...
| 每个数据库的最大存储空间 (GB) | #N/A | 2 | 2 | 2 | 2 | 2 | 2 | #N/A | 
||||||||
...
| 每个池的最大内存中 OLTP 存储 (GB) | 1 | #N/A | 4 | 10 | 12 | 
...
| 每个池的 eDTU 数 | **125** | **250** | **500** | **1000** |
|:---|---:|---:|---:| ---: | ---: | 
| 每个池包含的存储 (GB) | 250 | 500 | 750 | 750 |
| 每个池的最大存储选择 (GB)* | 250, 500, 750, 1024 | 500, 750, 1024 | 750, 1024 | 1024 | 
| 每个池的最大内存中 OLTP 存储 (GB) | 1 | #N/A | 4 | 10 |
...
| 每个数据库的最小 eDTU 数选择 | 0, 25, 50, 75, 125 | 0, 25, 50, 75, 125, 250 | 0, 25, 50, 75, 125, 250, 500 | 0, 25, 50, 75, 125, 250, 500, 1000 |
| 每个数据库的最大 eDTU 数选择 | 25, 50, 75, 125 | 25, 50, 75, 125, 250 | 25, 50, 75, 125, 250, 500 | 25, 50, 75, 125, 250, 500, 1000 | 
| 每个数据库的最大存储 (GB)* | 1024 | 1024 | 1024 | 1024 | 
||||||||
...
>
> \* 在高级层中，以下区域目前提供的存储超出 1 TB：美国东部 2、美国西部、美国弗吉尼亚州政府、西欧、德国中部、东南亚、日本东部、澳大利亚东部、加拿大中部和加拿大东部。 
>
>\*\* 标准池中每个数据库的最小/最大 eDTU 数至少为 200 eDTU 的为预览版。
>
