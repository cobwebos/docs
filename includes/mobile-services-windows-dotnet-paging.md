...
6.  （可选）使用消息检查软件（例如浏览器开发人员工具或 [Fiddler][Fiddler]）来查看发送到移动服务的请求的 URI。
...
7.  使用以下代码，再一次更新 "RefreshTodoItems" 方法：
    ...
    > [WACOM.NOTE] 本教程将硬编码分页值传递给 "Take" 和 "Skip" 方法，因此使用的是简化的方案。在实际应用程序中，你可以对页导航控件或类似的 UI 使用类似于上面的查询，让用户导航到上一页和下一页。你还可以调用 "IncludeTotalCount" 方法，以获取服务器上的可用项总数，以及分页的数据。
