...
| 安全存储密钥和机密。 | 为应用程序所有者提供安全的集中管理选项来简化机密和密钥的管理。 这种管理可以减少意外泄密或透露的风险。 Azure Key Vault 可以安全地将密钥存储在已通过 FIPS 140-2 级别2认证的硬件安全模块（Hsm）中。 如果需要使用 FIPs 140.2 级别3来存储密钥和机密，可以使用[Azure 专用 HSM](../articles/dedicated-hsm/overview.md)。 | - |
