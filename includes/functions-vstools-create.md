...
1. 输入项目的名称，并选择“创建”  。 函数应用名称必须像 C# 命名空间一样有效，因此请勿使用下划线、连字符或任何其他的非字母数字字符。
...
1. 在“新建 Azure Functions 应用程序”  中，使用以下选项：
    ...
    > [!NOTE]
    > 请确保将“授权级别”设置为 `Anonymous`。  如果选择默认级别 `Function`，需要在请求中提供[函数密钥](../articles/azure-functions/functions-bindings-http-webhook-trigger.md#authorization-keys)才能访问函数终结点。
