...
如果要授予不同的`--role`权限, 可以选择修改[az ad sp create-rbac][az-ad-sp-create-for-rbac]命令中的值。 有关角色的完整列表，请参阅 [ACR 角色和权限](https://github.com/Azure/acr/blob/master/docs/roles-and-permissions.md)。
...
以下脚本使用[az role 赋值 create][az-role-assignment-create]命令向在`SERVICE_PRINCIPAL_ID`变量中指定的服务主体授予*请求*权限。 如果要授予不同的访问级别，请调整 `--role` 值。
