...
## 将计算、网络和存储集成到 Azure Resource Manager 部署模型下的优点

借助 Azure Resource Manager 部署模型，可以轻松利用预建的应用程序模板或构建一个应用程序模板，在 Azure 上部署和管理计算、网络和存储资源。在本部分中，我们将了解通过 Azure Resource Manager 部署模型部署资源的优点。
...
| 负载平衡 | 云服务的创建为部署的虚拟机提供了一个隐式负载平衡器。 | 负载平衡器是 Microsoft.Network 提供程序提供的一个资源。需要负载平衡的虚拟机的主网络接口应该引用负载平衡器。负载平衡器既可以是内部的，也可以是外部的。[了解详细信息。](../articles/resource-groups-networking.md) |
...
|DNS 名称| 云服务会得到一个隐式的全局唯一 DNS 名称。例如：`mycoffeeshop.cloudapp.net`。 | DNS 名称是可在一个公共 IP 地址资源中指定的可选参数。FQDN 将采用以下格式 - `<domainlabel>.<region>.cloudapp.azure.com`。 |
...
Azure PowerShell 将拥有两种部署模式 – **AzureServiceManagement** 模式和 **AzureResourceManager** 模式。AzureResourceManager 模式现在同时包含用于管理虚拟机、虚拟网络和存储帐户的 cmdlet。你可以在[此处](../articles/powershell-azure-resource-manager.md)阅读详细内容。

### Azure CLI

Azure 命令行接口 (Azure CLI) 将拥有两种部署模式 – **AzureServiceManagement** 模式和 **AzureResourceManager** 模式。AzureResourceManager 模式现在同时包含用于管理虚拟机、虚拟网络和存储帐户的命令。你可以在[此处](../articles/xplat-cli-azure-resource-manager.md)阅读详细内容。
...
你可以在[此处](https://msdn.microsoft.com/library/azure/dn790568.aspx)找到有关计算、网络和存储资源提供程序的 REST API 的详细说明文档。
