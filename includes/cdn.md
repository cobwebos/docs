...
1. 登录到 [Azure 经典门户]。
2. 在左下角单击“新建”。 在“新建”对话框中，选择“数据服务”，然后单击“存储”，再单击“快速创建”。
...
5. 终结点创建后将出现在订阅的终结点的列表中。 该列表视图显示用于访问缓存内容的 URL 以及原始域。 
    ...
    原始域是 CDN 缓存内容的位置。 原始域可以是存储帐户或云服务；在本例中，我们使用存储帐户。 根据您指定的缓存控制设置或缓存网络的默认启发，存储内容被缓存到边缘服务器。 
...
* 您可将容器设为专用的而不是公用的。 有关详细信息，请参阅[限制对容器和 Blob 的访问](https://azure.microsoft.com/documentation/articles/storage-manage-access-to-resources/#restrict-access-to-containers-and-blobs)。
