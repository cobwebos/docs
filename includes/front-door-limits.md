...
* HTTP 请求转发到后端后，Front Door 将等待来自后端的第一个数据包 30 秒。 然后，它会向客户端返回 503 错误。 此值可通过 API 中的字段"发送 RecvTimeout 秒"进行配置。
    * 对于缓存方案，此超时是不可配置的，因此，如果缓存了请求，并且从前门或后端的第一个数据包需要 30 多秒的时间，则将 504 错误返回到客户端。 
...
* 来自运行状况探测 URL 的最大 HTTP 响应标头大小 - 4，096 字节 - 指定运行状况探测的所有响应标头的最大长度。 
