...
* *管理/控制平面*描述进入管理/控制平面或用于修改资源的 API 的调用。 例如，创建 VM、重启 VM 以及将虚拟网络更新成新子网等操作均可管理正在运行的资源。 它们并不直接影响到 VM 的连接。
...
> [!NOTE]
> 这是幂等操作。 如果失败，请重试操作。 如果继续失败，请创建支持票证或在[Microsoft Q 上创建一](https://docs.microsoft.com/answers/index.html)个论坛&A
>
>
