...
如果使用的是移动应用，请参阅 [为移动应用添加推送通知]教程，然后选择顶层的平台。

如果要使用 Java 或 PHP，请参阅[如何通过 Java/PHP 使用通知中心]。 可使用[通知中心 REST 接口]通过任何后端发送通知。
...
5. 在上面的代码中，将 `<hub name>` 和 `<connection string with full access>` 占位符替换为通知中心名称和从通知中心仪表板获取的 *DefaultFullSharedAccessSignature* 的连接字符串。
6. 在 **Main** 方法中添加以下行：
