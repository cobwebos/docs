...
将 Azure 备份服务与托管磁盘配合使用，创建具有基于时间的备份、轻松 VM 还原和备份保留策略的备份作业。 托管存储仅支持使用本地冗余存储 (LRS) 作为复制选项。 数据的三个副本保留在同一个区域中。 对于区域性灾难恢复，必须使用 [Azure 备份服务](../articles/backup/backup-introduction-to-azure-backup.md)和作为备份保管库的 GRS 存储帐户来备份不同区域中的 VM 磁盘。 当前 Azure 备份支持最大 4TB 的磁盘大小，请参阅[即时还原](../articles/backup/backup-instant-restore-capability.md)获取对 4TB 磁盘的支持。 有关详细信息，请参阅[为具有托管磁盘的 VM 使用 Azure 备份服务](../articles/backup/backup-introduction-to-azure-backup.md#using-managed-disk-vms-with-azure-backup)。
...
下面是可用于高级托管磁盘的磁盘大小，用星号表示的大小当前处于预览状态：

| **高级 SSD 托管<br>磁盘类型** | **P4** | **P6** | **P10** | **P15** | **P20** | **P30** | **P40** | **P50** | **P60*** | **P70*** | **P80*** |
|------------------|---------|---------|--------|--------|--------|----------------|----------------|----------------|----------------|----------------|----------------|
...
下面是可用于标准 SSD 托管磁盘的磁盘大小，用星号表示的大小当前处于预览状态：

| 标准 SSD 托管<br>磁盘类型 | **E4** | **E6** | E10 | E15 | E20 | E30 | E40 | E50 | **E60*** | **E70*** | **E80*** |
|------------------|---------|---------|--------|--------|--------|----------------|----------------|----------------|----------------|----------------|----------------|
| 磁盘大小        | 32 GiB | 64 GiB | 128 GiB | 256 GiB | 512 GiB | 1,024 GiB (1 TiB) | 2,048 GiB (2 TiB) | 4,095 GiB (4 TiB) | 8,192 GiB (8 TiB) | 16,384 GiB (16 TiB) | 32,767 GiB (TiB) |

下面是可用于标准 HDD 托管磁盘的磁盘大小，用星号表示的大小当前处于预览状态：
...
**出站数据传输**：[出站数据传输](https://azure.microsoft.com/pricing/details/data-transfers/)（传出 Azure 数据中心的数据）会产生带宽使用费。
