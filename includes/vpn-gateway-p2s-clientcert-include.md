...
- **自签名根证书：** 按照以下 P2S 证书文章中的步骤操作，以便您创建的客户端证书与 P2S 连接兼容。 这些文章中的步骤可生成兼容的客户端证书： 
  ...
  * [制作证书说明](../articles/vpn-gateway/vpn-gateway-certificates-point-to-site-makecert.md)：如果您无法访问 Windows 10 计算机以生成证书，请使用 MakeCert。 虽然 MakeCert 已弃用，但仍可使用它来生成证书。 可以将生成的证书安装在任何受支持的 P2S 客户端上。
