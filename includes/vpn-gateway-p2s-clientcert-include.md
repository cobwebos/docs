...
- **企业证书：**
  ...
  - 如果你使用的是企业证书解决方案，请使用通用名称值格式*名称 \@yourdomain .com*生成客户端证书。 而不要使用“域名\用户名”格式。
  - 请确保客户端证书基于“用户”证书模板，该模板将“客户端身份验证”列为用户列表中的第一项。 检查证书的方式是：双击证书，然后在“详细信息”选项卡中查看“增强型密钥用法”。

- **自签名根证书：** 按照以下 P2S 证书一文中的步骤进行操作，以便创建的客户端证书将与 P2S 连接兼容。 这些文章中的步骤可生成兼容的客户端证书： 
  ...
  * [MakeCert 说明](../articles/vpn-gateway/vpn-gateway-certificates-point-to-site-makecert.md)：如果无权访问用于生成证书的 Windows 10 计算机，请使用 MakeCert。 虽然 MakeCert 已弃用，但仍可使用它来生成证书。 可以将生成的证书安装在任何受支持的 P2S 客户端上。
