...
1. 配置程序包管理系统 (YUM) 以便能够安装 MongoDB。 创建 */etc/yum.repos.d/10gen.repo* 文件用于保存有关存储库的信息，然后添加以下内容：
...
3. 若要安装程序包，请运行以下命令以安装最新的 MongoDB 稳定版本及相关工具：
    ...
    下载和安装 MongoDB 时，请等待。
4. 创建数据目录。 默认情况下，MongoDB 将数据存储在 */data/db* 目录中，但用户必须创建该目录。 若要创建它，请运行：
...
6. 若要启动 MongoDB 命令行管理程序，请打开一个单独的 SSH 或 PuTTY 窗口并运行：
    ...
    通过 insert 创建数据库。
7. 在安装 MongoDB 后，您必须配置终结点才能远程访问 MongoDB。 在管理门户中，依次单击“虚拟机”、你的新虚拟机名称和“终结点”。
