...
| `Collection(Edm.DateTimeOffset)`、`Collection(Edm.Double)`、`Collection(Edm.Int32)`、`Collection(Edm.Int64)` | 使用 `eq`、`ne`、`lt`、`gt`、`le` 或 `ge` 进行比较 <br/><br/> 使用 `or` 将比较与其他子表达式组合 <br/><br/> 使用 `and` 将除 `ne` 以外的比较与其他子表达式组合 <br/><br/> `and` `or` 在[析取范式 Normal 窗体](https://en.wikipedia.org/wiki/Disjunctive_normal_form)中使用和组合的表达式（DNF） | 使用 `eq`、`ne`、`lt`、`gt`、`le` 或 `ge` 进行比较 <br/><br/> 使用 `and` 将比较与其他子表达式组合 <br/><br/> 使用 `or` 将除 `eq` 以外的比较与其他子表达式组合 <br/><br/> `and` `or` 在[联合 Normal 窗体](https://en.wikipedia.org/wiki/Conjunctive_normal_form)中使用和组合的表达式（.cnf） |
