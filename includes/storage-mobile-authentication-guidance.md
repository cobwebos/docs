...
> [!WARNING] 
> **请仅将共享密钥身份验证用于测试用途！** 为关联的存储帐户提供完全读/写访问权限的帐户名和帐户密钥将分发给下载你的应用的每个人。 这**不**是好的做法，因为你会面临向不受信任的客户端泄露密钥的风险。
> 
> 
...
* **帐户名** - 存储帐户的名称
* **帐户密钥** - 在 [Azure 门户](https://portal.azure.com)上，导航到存储帐户，然后单击“密钥”图标以查看此信息。
...
作为存储帐户所有者，需要为移动客户端生成要使用的 SAS。 若要生成 SAS，你可能需要编写单独的服务，该服务生成要分发给客户端的 SAS。 出于测试目的，可以使用 [Microsoft Azure 存储资源管理器](http://storageexplorer.com)或 [Azure 门户](https://portal.azure.com)来生成 SAS。 创建 SAS 时，可以指定 SAS 有效的时间间隔，以及 SAS 授予客户端的权限。
