...
[SSH](https://www.ssh.com/ssh/)是一种加密的连接协议，允许在不安全的连接上安全登录。 SSH 是在 Azure 中托管的 Linux VM 的默认连接协议。 虽然 SSH 本身提供加密连接，但是将密码用于 SSH 连接仍使 VM 易受到强力破解攻击或猜测密码。 使用 SSH 连接到 VM 的更安全和首选的方法是使用公钥对，也称为*SSH 密钥*。 
