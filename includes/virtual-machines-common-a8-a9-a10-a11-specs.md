...
计算密集型实例（A8、A9、H16r、H16mr、HB-ACCT-WC 和 HC）的一个子集为远程直接内存访问（RDMA）连接提供网络接口。 选择了通过 "r" （如 NC24rs 配置（NC24rs_v2 和 NC24rs_v3））指定的 N 系列大小也是 RDMA 支持的。 此接口是对可供其他 VM 大小使用的标准 Azure 网络接口的补充。 
