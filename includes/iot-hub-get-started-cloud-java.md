...
12. 在 **App** 类的 **main** 方法中添加以下代码。此代码将创建 **EventHubClient** 实例以连接到 IoT 中心上的事件中心兼容的终结点。然后，创建两个线程以从两个分区进行读取。将 **{youriothubkey}**、**{youreventhubcompatiblenamespace}** 和 **{youreventhubcompatiblename}** 替换为前面记下的值。**{youreventhubcompatiblenamespace}** 占位符的值来自**事件中心兼容的终结点**，其格式为 **xxxxnamespace**（即，从来自门户的事件中心兼容的终结点值中删除 **sb://** 前缀和 **.servicebus.windows.net** 后缀）。
