...
| 大小 | vCPU | 内存：GiB | 临时存储 (SSD) GB | 最大数据磁盘数 | 临时存储的最大吞吐量：IOPS/读取 MBps/写入 MBps | 最大 Nic 数/预期网络带宽（MBps） |
| -----|-----|-----|-----|-----|-----|-----|
...
<sup>3</sup>对于 "M416ms_v2" 和 "M416s_v2 大小"，请注意，对于以下映像仅提供初始支持： "GEN2： SUSE LINUX ENTERPRISE SERVER （SLES） 12 SP4 For SAP 应用程序"。
