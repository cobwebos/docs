...
[可用性区域](../articles/availability-zones/az-overview.md)扩展了在保持 VM 上应用程序和数据的可用性时的控制级别。 可用性区域是 Azure 区域中独特的物理位置。 每个区域由一个或多个数据中心组成，这些数据中心配置了独立电源、冷却和网络。 为确保复原能力，所有已启用的区域中必须至少有三个单独的区域。 区域中可用性区域的物理隔离可以在发生数据中心故障的情况下保护应用程序和数据。 区域冗余服务可跨可用性区域复制应用程序和数据，以防范单点故障。
...
可用性集是用于提供 VM 冗余和可用性的另一个数据中心配置。 数据中心内的这种配置可以确保在发生计划内或计划外维护事件时，至少有一个虚拟机可用，并满足 99.95% 的 Azure SLA 要求。 有关详细信息，请参阅[虚拟机的 SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/)。
...
> [!NOTE]
> 在某些情况下，同一可用性集中的 2 个 VM 可能具有相同的容错域。 可以通过转到可用性集并查看“容错域”列来确认这一点。
> 这可能是因为在部署 VM 时采用了下列顺序：
> - 部署第一个 VM
> - 停止/解除分配第一个 VM
> - 部署第二个 VM 在这种情况下，第二个 VM 的 OS 磁盘可能创建在与第一个 VM 相同的“容错域”上，因此第二个 VM 也会在同一容错域上。 
...
2. 在向存储帐户添加更多 VHD 之前，请查看 Azure 存储帐户中非托管磁盘的数量[限制](../articles/storage/blobs/scalability-targets-premium-page-blobs.md)
...
有关如何跨可用性区域实现负载均衡的教程，请参阅[使用 Azure CLI 跨所有可用性区域上实现 VM 负载均衡](../articles/load-balancer/load-balancer-standard-public-zone-redundant-cli.md)。
