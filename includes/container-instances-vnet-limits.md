---
author: dlepow
ms.service: container-instances
ms.topic: include
ms.date: 01/31/2020
ms.author: danlep
ms.openlocfilehash: edf7b7c7c7c94fd269f69bd8c5e13e53a6de1c03
ms.sourcegitcommit: 849bb1729b89d075eed579aa36395bf4d29f3bd9
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/28/2020
ms.locfileid: "77117828"
---
### <a name="regions-and-resource-availability"></a>区域和资源可用性

| 区域 | OS | 最大 CPU | 最大内存 (GB) | 存储器 (GB) |
| -------- | :---: | :---: | :-----------: | :---: |
| 澳大利亚东部、加拿大中部、美国中部、美国东部<sup>1</sup>、美国东部2、北欧、美国中南部<sup>1</sup>、东南亚、西欧、美国西部 2<sup>1</sup> | Linux | 4 | 16 | 50 |
| 日本东部 | Linux | 2 | 8 | 50 |
| 美国中北部、印度南部、美国西部 | Linux | 2 | 3.5 | 50 |

<sup>1</sup>用于生产工作负荷的容器组部署到虚拟网络的区域。 在其他区域，虚拟网络部署处于预览阶段。
