...
| JSON 实体 | 方法参数 | 描述 |
| ----- | ----- | ----- |
| `computeType` | NA | 计算目标。 对于 AKS，此值必须为 `aks`。 |
...
| `gpuCores` | `gpu_cores` | 为此 Web 服务分配的 GPU 内核数（每个容器副本）。 默认值为 1。 仅支持整数值。 |
| `livenessProbeRequirements` | NA | 包含运行情况探测要求的配置元素。 |
...
| &emsp;&emsp;`timeoutSeconds` | `timeout_seconds` | 活动探测超时后的秒数。默认值为 2 秒。 最小值为 1 |
