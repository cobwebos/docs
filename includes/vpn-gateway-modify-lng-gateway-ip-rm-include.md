...
### <a name="a-namegwipnoconnectionahow-to-modify-the-gateway-ip-address-no-gateway-connection"></a><a name="gwipnoconnection"></a>如何修改网关 IP 地址 - 无网关连接
...
### <a name="a-namegwipwithconnectionahow-to-modify-the-gateway-ip-address-existing-gateway-connection"></a><a name="gwipwithconnection"></a>如何修改网关 IP 地址 - 现有网关连接
如果网关连接已存在，首先需要删除该连接。 然后，可修改网关 IP 地址并重新创建一个新的连接。 这将导致 VPN 连接中断一段时间。

> [!IMPORTANT]
> 不要删除 VPN 网关。 如果执行此操作，则必须返回执行相应步骤重新创建它，并使用将分配到新创建网关的 IP 地址重新配置本地路由器。
> 
> 
