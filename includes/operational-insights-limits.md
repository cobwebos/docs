...
| 每个订阅的付费工作区数目 | 不适用 | 你受到的限制包括一个资源组中的资源数，以及一个订阅的资源组数 | 
...
| 类别 | 限制 | 注释
| --- | --- | --- |
| 数据收集器 API | 单个发布内容的最大大小为 30 MB<br>字段值的最大大小为 32 KB | 将较大的卷拆分为多个发布内容<br>超过 32 KB 的字段将被截断。 |
| 搜索 API | 针对非聚合数据返回&5000; 条记录<br>针对聚合数据返回&50000;0 条记录 | 聚合数据是指包含 `measure` 命令的搜索
