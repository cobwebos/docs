...
2. 单击**"推送"**选项卡，为**"权限"**选择**"仅经过身份验证的用户"**，然后单击**"编辑脚本"**。
	...
	这样，你便可以自定义推送通知注册回调函数。如果使用 Git 编辑源代码，可以在 `.\service\extensions\push.js` 中找到与此相同的注册函数。

3. 将现有的 **register** 类替换为以下代码：
	...
	这会向注册添加一个标记（已登录用户的 ID）。系统将验证提供的标记，以防止用户注册另一个用户的 ID。向此用户发送通知时，此设备以及该用户注册的其他设备将会收到该通知。
