...
CPU 时间（天）<sup>6</sup>|60 分钟|240 分钟|无限制，按标准[费率](/documentation/articles/app-service)</a>付费|无限制，按标准费率付费|无限制，按标准费率付费
内存（1 小时）|每个 App Service 计划 1024 MB|每个应用 1024 MB|不适用|不适用|不适用
带宽|165 MB|无限制，收取[数据传输费率](/documentation/articles/data-transfers)|无限制，收取数据传输费率|无限制，收取数据传输费率|无限制，收取数据传输费率
...
<sup>3</sup>专用实例可有不同的大小。有关更多详细信息，请参阅 [App Service 定价](home/features/web-site/#price)。可通过提出支持请求获得其他实例。  
<sup>4</sup>高级层在使用 App Service 环境时最多允许 50 个计算实例（取决于可用性）和 500 GB 的磁盘空间，否则为 20 个计算实例和 250 GB 的存储。  
<sup>5</sup>存储限制是跨相同 App Service 计划中所有应用的内容总大小。[App Service 环境](/documentation/articles/app-service-web-configure-an-app-service-environment#storage)中提供了更多存储选项  
