...
如果指定仅宽度或高度的查询参数，必应保持图像的纵横比。 如果指定了宽度和高度，则不会维持图像的原始纵横比，必应会将白色填充内容添加到图像的边框。 例如，如果在不裁剪的情况下将 480x359 大小的图像调整成 200x200 大小，则图像会充斥整个宽度，而高则包含 25 像素的白色填充内容，位于图像的顶部和底部。 如果图像大小为 359x480，则情形与上面类似，区别是图像的左右边缘包含白色填充内容。 如果裁剪图像，不会添加白色填充内容。  
