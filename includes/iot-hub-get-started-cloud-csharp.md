...
6. 将以下方法添加到 **Program** 类：
    ...
    此方法使用 **EventHubReceiver** 实例接收来自所有 IoT 中心设备到云接收分区的消息。请注意在创建 **EventHubReceiver** 对象时传递 `DateTime.Now` 参数的方式，使它只收到它启动后发送的消息。这很适合测试环境，因为这样可以看到当前的消息集，但在生产环境中，代码应该要确保它能处理所有消息。有关详细信息，请参阅 [How to process IoT Hub device-to-cloud messages][lnk-processd2c-tutorial]（如何处理 IoT 中心设备到云的消息）教程。
