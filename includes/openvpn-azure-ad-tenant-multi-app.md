...
在此步骤中，您将创建两个 Azure AD 租户用户：一个全局管理员帐户和一个主用户帐户。 主要用户帐户用作主要嵌入帐户（服务帐户）。 创建 Azure AD 租户用户帐户时，可以根据要创建的用户类型调整目录角色。 使用[此文](../articles/active-directory/fundamentals/add-users-azure-active-directory.md)中的步骤为 Azure AD 租户创建至少两个用户。 若要创建帐户类型，请务必更改“目录角色”：****
...
## <a name="4-register-additional-applications"></a><a name="register-apps"></a>4. 注册其他申请

在此步骤中，您可以为各种用户和组注册其他应用程序。

1. 在 Azure 活动目录下，单击**应用注册**，然后**单击 "新注册**"。

    ![Azure VPN](./media/openvpn-azure-ad-tenant-multi-app/app1.png)

2. 在 **"注册应用程序**"页上，输入**名称**。 选择所需的**受支持帐户类型**，然后单击"**注册**"。
...
7. 单击 **"公开 API"** 然后 **= 添加客户端应用程序**。  对于“客户端 ID”，请根据云输入以下值：****

    - 输入**41b23e61-6c1e-4545-b367-cd054e0ed4b44**用于 Azure**公共**
    - 输入 Azure**政府** **51bb15d4-3a4f-4ebf-9dca-40096fe32426**
    - 输入 Azure**德国** **538ee9e6-310a-468d-afef-ea97365856a9**
...
9. 复制“概述”页中的“应用程序(客户端) ID”。******** 您需要此信息来配置 VPN 网关。

    ![Azure VPN](./media/openvpn-azure-ad-tenant-multi-app/app6.png)

10. 重复此[寄存器附加应用程序](#register-apps)部分中的步骤，以创建安全要求所需的尽可能多的应用程序。 每个应用程序将关联到某个 VPN 网关，并可以包含一组不同的用户。 只能将一个应用程序关联到一个网关。

## <a name="5-assign-users-to-applications"></a><a name="assign-users"></a>5. 将用户分配给应用程序

将用户分配给您的应用程序。

1. 在**Azure AD-> 企业应用程序中**，选择新注册的应用程序并单击**属性**。 确保“需要用户分配?”设置为“是”。******** 单击“保存”。****

    ![Azure VPN](./media/openvpn-azure-ad-tenant-multi-app/user2.png)

2. 在应用页面上，单击 **"用户和组**"，然后单击 **"添加用户**"。
