...
---
为了在 Azure 文件共享中保护数据以防数据丢失或损坏，所有 Azure 文件共享都在写入每个文件时存储多个副本。 根据工作负荷的要求，你可以选择额外的冗余度。 Azure 文件目前支持以下数据冗余选项：
...
常规用途版本2（GPv2）存储帐户提供了两个 Azure 文件不支持的额外冗余选项：读取可访问的异地冗余存储（通常称为 RA-GRS）和读取可访问的异地区域冗余存储，通常是称为 "RA-GZRS"。 可以在存储帐户中预配这些选项集的 Azure 文件共享，但 Azure 文件不支持从次要区域读取。 部署到可读取访问的地理区域或地理区域冗余存储帐户中的 Azure 文件共享将分别计费为异地冗余或地域冗余存储。