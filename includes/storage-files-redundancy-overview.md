...
- **区域冗余**：区域冗余存储（通常称为 ZRS）表示每个文件都在三个不同的 Azure 存储群集中存储三次。 这三个不同的 Azure 存储群集每个将文件存储三次（就像本地冗余存储一样），并在物理上隔离在不同的 Azure*可用性区域*中。 可用性区域是 Azure 区域中独特的物理位置。 每个区域由一个或多个数据中心组成，这些数据中心配置了独立电源、冷却和网络。 在将写入存储写入到所有三个可用性区域中的存储群集之前，不会对其进行写入。 
