...
>[AZURE.NOTE] SQL 数据库的许多新功能只有在使用 [Azure Resource Manager (ARM) 部署模型](/documentation/articles/resource-group-overview)时才受支持。因此，示例使用基于 ARM 的 [Azure SQL 数据库 PowerShell cmdlet](https://msdn.microsoft.com/zh-cn/library/azure/mt574084.aspx)。向后兼容支持现有的经典部署模型 [Azure SQL 数据库（经典）cmdlet](https://msdn.microsoft.com/zh-cn/library/azure/dn546723.aspx)，但建议使用基于 ARM 的 cmdlet。
