...
1. 使用图像下面的表格中指定的函数应用设置。
    ...
    | **Function App 名称** | 全局唯一名称 | 用于标识新 Function App 的名称。 有效字符 `a-z` （不区分大小写）、`0-9`和 `-`。  |
...
1. 输入以下宿主设置。
    ...
    | 设置      | 建议的值  | 描述 |
    | ------------ | ---------------- | ----------- |
    | [存储帐户](../articles/storage/common/storage-quickstart-create-account.md) |  全局唯一名称 |  创建函数应用使用的存储帐户。 存储帐户名称必须为 3 到 24 个字符，并且只能包含数字和小写字母。 你还可以使用现有帐户，这必须满足[存储帐户要求](../articles/azure-functions/functions-scale.md#storage-account-requirements)。 |
    |**操作系统**| 首选操作系统 | 系统会根据运行时堆栈选择预先选择一个操作系统，但可以根据需要更改该设置。 |
