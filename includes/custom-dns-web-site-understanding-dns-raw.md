...
当你创建 Azure 网站时，DNS 名称将自动分配到 Web 应用。此名称采用 **&lt;yourwebappname&gt;.azurewebsites.net** 的格式。创建 DNS 记录时，还有一个虚拟 IP 地址可供使用，因此你可以创建指向 **.azurewebsites.net** 的记录，或者可以指向该 IP 地址。
...
CNAME 记录将*特定的* DNS 名称（例如 **mail.contoso.com** 或 **www.contoso.com**）映射到另一个（规范）域名。对于 Azure 网站，规范域名是 Web 应用的 **&lt;你的 Web 应用名称>.azurewebsites.net** 域名。一旦创建，CNAME 即为 **&lt;你的 Web 应用名称>.azurewebsites.net** 域名创建一个别名。CNAME 条目将自动解析为 **&lt;你的 Web 应用名称>.azurewebsites.net** 域名的 IP 地址，因此，如果 Web 应用的 IP 地址发生更改，你不必采取任何操作。
...
* **对于根域或通配符子域** - 将 **awverify** 的 DNS 名称映射为 **awverify.&lt;你的 Web 应用名称&gt;.azurewebsites.net**。

* **对于特定的子域** - 将 **awverify.&lt;子域>** 的 DNS 名称映射为 **awverify.&lt;你的 Web 应用名称&gt;.azurewebsites.net**。例如，如果 A 记录用于 **blogs.contoso.com**，则为 **awverify.blogs**。

此 CNAME 记录用于验证您正在尝试使用的您自己的域。此操作是对创建指向 Web 应用的虚拟 IP 地址的 A 记录的补充。

你可以按照以下步骤找到 Web 应用的 IP 地址以及 **awverify** 名称和 .**azurewebsites.net** 名称：

1. 在你的浏览器中，打开 [Azure 管理门户](https://manage.windowsazure.com)。
...
6. 在“管理自定义域”对话框中，你将看到 **awverify** 信息、当前分配的 **.azurewebsites.net** 域名和虚拟 IP 地址。保存此信息，因为将在创建 DNS 记录时使用它。
