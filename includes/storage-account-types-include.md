...
<div id="zone-redundant-storage"><sup>4</sup>区域冗余存储（ZRS）和异地冗余存储（GZRS/RA-GZRS）（预览版）仅适用于某些区域中的标准常规用途 V2、BlockBlobStorage 和 FileStorage 帐户。 有关 Azure 存储冗余选项的详细信息，请参阅<a href="https://docs.microsoft.com/azure/storage/common/storage-redundancy">Azure 存储冗余</a>。</div>
