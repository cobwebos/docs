...
4. <a name="congroup"></a>配置可用性组的 IP 资源。
...
5. <a name = "dependencyGroup"></a>使 SQL Server 可用性组资源依赖于客户端接入点。
...
6. <a name="listname"></a>使客户端接入点资源依赖于 IP 地址。
...
7. <a name="setparam"></a>在 PowerShell 中设置群集参数。
...
3. 记下 IP 地址的**名称**。 该项可能为 `Cluster IP Address`。 

4. <a name="setwsfcparam"></a>在 PowerShell 中设置群集参数。
