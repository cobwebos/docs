...
在托管磁盘上进行的下述控制平面操作可能涉及将磁盘从一个存储位置移动到另一个存储位置。 这是通过在后台复制数据来安排的，可能需要花费数小时才能完成，通常少于 24 小时，具体取决于磁盘中的数据量。 在此期间您的应用程序可能会遇到比平常的读取延迟高因为某些读取重定向到原始位置，并可以将长时间才能完成。 在此期间，对写入延迟没有影响。
...
Azure 高级存储提供了八个 GA 磁盘大小和当前处于预览状态的三种磁盘大小。 每种磁盘大小对 IOPS、带宽和存储空间设置了不同规格的限制。 选择正确的高级存储磁盘大小，具体取决于应用程序要求和高规格 VM 大小。 下表显示了 11 磁盘大小及其功能。 目前，仅托管磁盘支持 P4、P6、P15、P60、P70 和 P80 大小。
...
> [!WARNING]
> 大于 4 TiB 磁盘不支持磁盘缓存。 如果多个磁盘附加到 VM，每个磁盘都是 4 TiB 或较小将支持缓存。
>
...
请根据应用程序正在运行的工作负荷的类型，选择合适的条带大小。 对于随机的较小的 IO 请求，请使用较小的条带大小。 而对于大型顺序 IO 请求使用更大的条带大小。 对于要在高级存储上运行的应用程序，请找出相应的条带大小建议。 对于 SQL Server，如果工作负荷为 OLTP 工作负荷，请将条带大小配置为 64 KB；如果工作负荷为数据仓库型工作负荷，则请将条带大小配置为 256 KB。 请参阅 [Azure VM 上的 SQL Server 性能最佳实践](../articles/virtual-machines/windows/sql/virtual-machines-windows-sql-performance.md#disks-guidance)以了解更多信息。
