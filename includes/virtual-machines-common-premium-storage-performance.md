...
| **麦克斯。记忆** |顺利运行应用程序所需的内存量 |提交的在用字节数百分比 |使用 vmstat |
| **麦克斯。Cpu** |顺利运行应用程序所需的 CPU 速度 |处理器时间百分比 |%util |
...
| **VM 和磁盘规模限制** |所选 VM 大小的 IOPS 限制应大于由附加到它的存储磁盘驱动的 IOPS 总数。 |所选 VM 大小的吞吐量限制应大于已连接的高级存储磁盘所要求的总吞吐量。 |所选 VM 大小的规模限制必须大于已连接高级存储磁盘的总规模限制。 |
...
例如，以需要 16,000 IOPS 的应用程序为考虑对象。 要实现此性能，您需要一个标准\_D14 Azure IaaS VM，它可以使用 32 个标准存储 1 TB 磁盘最多提供 16，000 个 IOPS。 每个 1-TB 标准存储磁盘最多可以实现 500 个 IOPS。 此 VM 每月的估计成本将是 $1,570。 32 个标准存储磁盘每月的成本将是 $1,638。 每月估计的总成本将是 $3,208。
...
| CentOS | 7.1+ 或更高版本| 3.10.0-229.1.2.el7+ | [建议 LIS4](https://www.microsoft.com/download/details.aspx?id=55106) <br> 请参阅下一部分中的注释** |
| Red Hat Enterprise Linux (RHEL) | 6.8°、7.2°或更新 | &nbsp; | &nbsp; |
| Oracle | 6.0°、7.2°或更新 | &nbsp; | UEK4 或 RHCK |
| Oracle | 7.0-7.1 或更新 | &nbsp; | UEK4 或 RHCK 带[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
| Oracle | 6.4-6.7 或更新 | &nbsp; | UEK4 或 RHCK 带[LIS4](https://www.microsoft.com/download/details.aspx?id=55106) |
...
在某些情况下，上述命令也将升级内核。 如果需要内核更新，则可能需要在重新启动后再次运行上述命令，以完全安装 Microsoft-hyper-v 包。
