...
* AzureVPN
* 泛型
* OpenVPN（如果已启用网关上的 OpenVPN 和 Azure AD 身份验证设置。 有关 VPN 网关，请参阅[创建租户](../articles/vpn-gateway/openvpn-azure-ad-tenant.md)。 对于虚拟 WAN，请参阅[创建租户 - VWAN](../articles/virtual-wan/openvpn-azure-ad-tenant.md)。
...
添加连接时，请使用在上一步收集的“配置文件详细信息”页面的信息。 这些字段与以下信息相对应：
   ...
   * **受众：** 标识令牌的收件人资源
   * **发行人：** 标识发出令牌的安全令牌服务 （STS） 以及 Azure AD 租户
   * **租户：** 包含颁发令牌的目录租户的不可变的唯一标识符
   * **FQDN：** Azure VPN 网关上的完全限定域名 （FQDN）
   * **服务器机密：** VPN 网关预共享密钥
