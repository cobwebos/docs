...
5. 将现有的 **PushRegistrationHandler** 类替换为以下代码：
	...
	在注册期间将调用 **Register** 方法。这样，你便可以向注册添加一个标记（已登录用户的 ID）。系统将验证提供的标记，以防止用户注册另一个用户的 ID。向此用户发送通知时，此设备以及该用户注册的其他设备将会收到该通知。 

6. 展开 Controllers 文件夹，打开 TodoItemController.cs 项目文件，找到 **PostTodoItem** 方法，并将调用 **SendAsync** 的代码行替换为以下代码：
