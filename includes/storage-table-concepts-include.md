## <a name="what-is-table-storage"></a>表存储是什么
Azure 表存储存储大量结构化数据。 该服务是一个 NoSQL 数据存储，接受来自 Azure 云内部和外部的通过验证的呼叫。 Azure 表最适合存储结构化非关系型数据。 表存储的常见用途包括：
...
你可以使用表存储来存储和查询大型结构化非关系型数据和你的表会随着需求增加时。
