...
| <ul> <li> https:\//login.microsoftonline.com </li><li> https:\//secure.aadcdn.microsoftonline-p.com </li><li> https：\//login.live.com </li><li> https：\//graph.windows.net </li><li> https:\//login.windows.net </li><li> https：\//www.live.com </li><li> https：\//www.microsoft.com </li></ul> | OVF 设置需要对这些 Url 的访问权限。 它们由 Azure Active Directory 用于访问控制和标识管理
https:\//dev.mysql.com/get/Downloads/MySQLInstaller/mysql-installer-community-5.7.20.0.msi  | 完成 MySQL 下载。 </br> 在少数几个区域中，下载可能会重定向到 CDN URL。 如果需要，请确保 CDN URL 还列入允许列表。
|
...
下表汇总了配置服务器的容器要求。 如果要复制多个 VMware Vm，应查看[容量规划注意事项](../articles/site-recovery/site-recovery-plan-capacity-vmware.md)，并运行[Azure Site Recovery 部署规划器工具](../articles/site-recovery/site-recovery-deployment-planner.md)。
