...
第 1 代映像只能在数据磁盘上使用 GPT 分区，而不能在操作系统磁盘上使用。 操作系统磁盘必须使用 MBR 分区样式。

[第 2 代映像](https://docs.microsoft.com/azure/virtual-machines/linux/generation-2)可以在操作系统磁盘和数据磁盘上使用 GPT 分区。
...
什么是 Azure 磁盘预留？
磁盘预留是提前购买一年磁盘存储的选项，可降低总成本。 有关 Azure 磁盘预留的详细信息，请参阅我们有关该主题的文章：[了解预留折扣如何应用于 Azure 磁盘](../articles/cost-management-billing/reservations/understand-disk-reservations.md)。
...
如何应用 Azure 磁盘预留？    
磁盘预留遵循类似于预留虚拟机 (VM) 实例的模型。 不同之处在于，磁盘预留无法应用于不同的 SKU，而 VM 实例可以。 有关 VM 实例的详细信息，请参阅[通过 Azure 虚拟机预留实例节省成本](../articles/virtual-machines/linux/prepay-reserved-vm-instances.md)。     
...
只有 P15 或更高的高级 SSD 才支持共享磁盘。

如果我有现有的高级 SSD，是否可以对它启用共享磁盘？
...
除了磁盘限制之外，还会在 VM 级别施加 IO 限制。 请确保所使用的 VM 大小可以支持在磁盘上配置的级别。 有关 VM 施加的 IO 限制的详细信息，请参阅 [Azure 中 Windows 虚拟机的大小](../articles/virtual-machines/windows/sizes.md)。

是否可以对超级磁盘使用缓存级别？
不可以，超级磁盘不支持其他磁盘类型上所支持的不同缓存方法。 将磁盘缓存设置为“无”。

是否可以将超级磁盘附加到现有 VM？
可能可以，VM 必须处于支持超级磁盘的区域和可用性区域对中。 有关详细信息，请参阅[超级磁盘入门](../articles/virtual-machines/windows/disks-enable-ultra-ssd.md)。
...
在 [creationData](https://docs.microsoft.com/rest/api/compute/disks/createorupdate#creationdata) 的 [createOption](https://docs.microsoft.com/rest/api/compute/disks/createorupdate#diskcreateoption) 属性设置为“Upload”的情况下创建托管磁盘，然后可以将数据上传到它。

是否可以在 VM 处于上传状态时将磁盘附加到它？

不是。

是否可以对处于上传状态的托管磁盘拍摄快照？
...
不是。 提供对包含托管磁盘的 VM 的 Azure Site Recovery Azure 到 Azure 保护。
...
不支持，任何大小的标准 SSD 托管磁盘都无法用于非托管磁盘或页 blob。
...
是否可以将现有托管磁盘的大小从小于 4 TiB 调整到最新引入的磁盘大小（最大大小为 32 TiB）？

是的。

Azure 备份和 Azure Site Recovery 服务支持的最大磁盘大小是多少？

Azure 备份支持的最大磁盘大小为 32 TiB（对于加密磁盘为 4 TiB）。 Azure Site Recovery 支持的最大磁盘大小为 8 TiB。 Azure Site Recovery 中尚不支持最大为 32 TiB 的更大磁盘。
...
可以停止并启动磁盘附加到的 VM，也可以拆离并重新附加磁盘。 对于高级 SSD 和标准 SSD，较大磁盘大小的性能目标在正式发布时已提高。

哪些区域中支持 8 TiB、16 TiB 和 32 TiB 的托管磁盘大小？

全球 Azure、Microsoft Azure 政府和 Azure 中国世纪互联下的所有区域中都支持 8 TiB、16 TiB 和 32 TiB 磁盘 SKU。
