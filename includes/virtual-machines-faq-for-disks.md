...
若要实现标准 SSD 的磁盘吞吐量，并标准 HDD 大于 500 IOPS 和 60 MiB/s 的大磁盘大小（> 4 TiB），我们建议从以下 VM 大小之一部署新的 VM，以优化性能： B 系列、DSv2 系列、Dsv3 系列、ESv3 系列、Fs 系列、Fsv2 系列、M 系列、GS 系列、NCv2、NCv3 系列或 Ls 系列 Vm。 将较大的磁盘附加到不使用上述建议大小的现有 Vm 或 Vm 可能会降低性能。
