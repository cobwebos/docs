...
| 配额名称 | 范围 | 类型 | 超出时的行为 | 值 |
| --- | --- | --- | --- | --- |
| 每个 Azure 订阅的基本/标准命名空间数上限 |命名空间 |静态 |后续请求更多基本/标准命名空间会被门户拒绝。 |100|
| 每个 Azure 订阅的高级命名空间数上限 |命名空间 |静态 |后续请求更多高级命名空间会被门户拒绝。 |10 |
...
| 每个服务命名空间的分区主题/队列数 |系统范围 |静态 |将拒绝后续的在服务命名空间中创建新分区主题或队列的请求。 因此，如果是通过 [Azure 门户][Azure portal]配置的，将生成错误消息。 如果是通过管理 API 调用的，调用代码将收到 **QuotaExceededException** 异常。 |基本和标准层 - 100<br />[高级](../articles/service-bus-messaging/service-bus-premium-messaging.md) - 1,000（每个消息传送单元）<br/><br />在每个命名空间中，每个分区队列或主题的实体配额不会超过 10,000 个。 |
...
| 每个主题的订阅数 |系统范围 |静态 |将拒绝后续的为主题创建附加订阅的请求。 因此，如果是通过门户配置的，会显示错误消息。 如果是通过管理 API 调用的，调用代码将收到异常。 |2,000 |
...
| 每个命名空间、队列或主题的 [SharedAccessAuthorizationRule](/dotnet/api/microsoft.servicebus.messaging.sharedaccessauthorizationrule) 规则数 |实体、命名空间 |静态 |将拒绝后续的创建附加规则请求，且调用代码将收到异常。 |最大规则数：12。 <br /><br /> 在服务总线命名空间中配置的规则适用于该命名空间中的所有队列和主题。 |
