...
将以此模块为起点，从设备接受数据，并通过网关传递到云。 示例中，该模块：
...
上图中名为“协议 X 引入”的模块在源代码中称作“模拟设备”。

### <a name="mac-lt-gt-iot-hub-id-module"></a>MAC &lt;-&gt; IoT 中心 ID 模块

此模块会扫描具有 Mac 地址属性的消息。 该示例中，协议引入模块添加 MAC 地址属性。 如果模块发现这种属性，它会向消息添加另一个具有 IoT 中心设备密钥的属性。 然后模块会向链中的下一模块提供该消息。

开发人员在 MAC 地址和 IoT 中心标识之间设置映射，将模拟设备与 IoT 中心设备标识相关联。 开发人员手动将映射添加到模块配置中。

> [!NOTE]
> 此示例使用 MAC 地址作为唯一的设备标识符，将其与 IoT 中心设备标识相关联。 不过，你可以编写你自己的模块，让其使用其他的唯一标识符。 例如，设备可能具有唯一序列号或者遥测数据可能包括唯一的嵌入式设备名。

### <a name="iot-hub-communication-module"></a>IoT 中心通信模块

此模块采用具有 IoT 中心设备密钥属性（由前一模块分配）的消息。 模块使用 HTTP 协议将消息内容发送到 IoT 中心。 HTTP 是 IoT 中心理解的三大协议之一。

此模块会打开从网关到 IoT 中心的单个 HTTP 连接，而不是打开每个模拟设备的连接。 然后模块会通过该连接从所有模拟设备多路复用连接。 此方法使单个网关能够连接更多设备。
