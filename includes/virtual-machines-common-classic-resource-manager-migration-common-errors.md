...
| 托管服务 {hosted-service-name} 中的 VM {vm-name} 包含 Azure 资源管理器不支持的扩展 {extension-name}。 建议从 VM 中卸载该扩展，再继续迁移。 |XML 扩展，如 BGInfo 1。\*不支持 Azure 资源管理器中。 因此，无法迁移这些扩展。 如果将这些扩展保留安装在虚拟机上，则在完成迁移之前会自动将其卸载。 |
