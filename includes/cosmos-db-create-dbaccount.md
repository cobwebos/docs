...
3. 在“新建帐户”页上，输入新 Azure Cosmos DB 帐户的设置。 
    ...
    API|SQL (DocumentDB)|API 确定要创建的帐户的类型。 Azure Cosmos DB 提供四种 API 以满足应用程序的需求：Gremlin（图形）、MongoDB、SQL (Cosmos DB) 和表（键/值），每个目前都需要单独的帐户。 <br><br>之所以选择 **SQL (Cosmos DB)** 是因为，在本快速入门中将创建可使用 SQL 语法查询的文档数据库。<br><br>[详细了解 Cosmos DB API](../articles/cosmos-db/documentdb-introduction.md)|
...
4. 创建帐户需要几分钟时间。 创建帐户过程中，门户会显示“部署 Azure Cosmos DB”磁贴。
