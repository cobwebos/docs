...
获得批准后，请运行以下任一命令以确定要将超级 SSD 部署到美国东部 2 中的哪个区域：
...
在知道了要部署到哪个区域后，请按照本文中的部署步骤来部署第一个具有超级 SSD 盘的 VM。
...
此外，请参阅示例[创建具有多个超级 SSD 磁盘的 VM](https://aka.ms/UltraSSDTemplate)以了解如何创建具有多个超级 SSD 盘的 VM。

下面介绍了新的/修改的资源管理器模板更改：必须将 `Microsoft.Compute/virtualMachines` 和 `Microsoft.Compute/Disks` 的 apiVersion 设置为 `2018-06-01`（或之后）。

指定磁盘 Sku UltraSSD_LRS、磁盘容量、IOPS 和吞吐量 (MBps) 来创建超级 SSD 盘。 以下是创建 1,024 GiB（GiB = 2^30 字节）、80,000 IOPS 和 1,200 MBps（MBps = 每秒 10^6 字节）的磁盘的示例：
...
在 VM 的属性上添加一个额外功能来指示其超级 SSD 已启用（请参阅[示例](https://aka.ms/UltraSSDTemplate)以查看完整的资源管理器模板）：
