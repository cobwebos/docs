...
- Dv2 系列（原始 D 系列的后续产品）提供更强大的 CPU 和最优 CPU 到内存的配置，使其可适合大多数生产工作负荷。 Dv2 系列 CPU 比 D 系列 CPU 快大约 35%。 它基于最新代 Intel Xeon® E5 2673 v3 2.4 GHz (Haswell) 或 E5 2673 v4 2.3 GHz (Broadwell) 处理器，通过 Intel Turbo Boost 技术 2.0 可以达到 3.1 GHz。 Dv2 系列的内存和磁盘配置与 D 系列相同。
...
| 大小 | vCPU | 内存：GiB | 临时存储 (SSD) GiB | 最大数据磁盘数 | 最大缓存吞吐量和临时存储吞吐量：IOPS/MBps（以 GiB 为单位的缓存大小） | 最大非缓存磁盘吞吐量：IOPS/MBps | 最大 NIC 数/预期网络带宽 (MBps) |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Standard_DS1_v2 |第 |3.5 |7 |4 |4,000 / 32 (43) |3,200 / 48 |2 / 750 |
