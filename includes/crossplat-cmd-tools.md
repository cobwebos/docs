# <a name="how-to-use-the-azure-command-line-tools-for-mac-and-linux"></a>如何使用针对 Mac 和 Linux 的 Azure 命令行工具
本指南介绍如何使用适用于 Mac 和 Linux 的 Azure 命令行工具创建和管理 Azure 中的服务。 涉及的操作包括**安装工具**、**导入发布设置**、**创建和管理 Azure 网站**以及**创建和管理 Azure 虚拟机**。 有关完整的参考文档，请参阅[适用于 Mac 和 Linux 的 Azure 命令行工具文档][reference-docs]。 
...
有关支持的命令的完整列表，请在这些工具安装后在命令行中键入 `azure -help`，或参阅[参考文档][reference-docs]。
...
这将打开默认浏览器，并提示您登录到管理门户。 登录后，将会下载 `.publishsettings` 文件。 记下此文件的保存位置。
...
可以考虑将 `--ssh` 标志 (Linux) 或 `--rdp` 标志 (Windows) 传递到 `vm create` 以支持与新建的虚拟机的远程连接。
