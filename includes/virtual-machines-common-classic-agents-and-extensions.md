...
如果需要一个虚拟机实例来使用一个或多个 VM 扩展，该实例必须有安装的 VM 代理。 通过 Azure 门户创建的虚拟机映像和 **Marketplace** 提供的映像会自动在创建过程中安装 VM 代理。 如果虚拟机实例缺少 VM 代理，则可在创建虚拟机实例以后安装该 VM 代理。 也可安装随后上载的自定义 VM 映像中的代理。
...
* 用户使用 Azure 门户，然后从 **Marketplace** 选择一个映像，通过这种方式创建 VM 的实例。
