...
自定义 IPsec/IKE 策略在 Azure 标准 VPN 网关和高性能 VPN 网关上受支持。 **基本** SKU。

### <a name="how-many-policies-can-i-specify-on-a-connection"></a>在一个连接上可以指定多少个策略？
一个给定的连接只能指定一个策略组合。

### <a name="can-i-specify-a-partial-policy-on-a-connection-eg-only-ike-algorithms-but-not-ipsec"></a>能否在一个连接上指定部分策略？ （例如，仅指定 IKE 算法，不指定 IPsec）
...
| 流量选择器 | UsePolicyBasedTrafficSelectors** ($True/$False)                             |
|                  |                                                                             |
