...
3. 在 Microsoft 更新目录的搜索框中，输入你想要下载，例如的修补程序的知识库 (KB) 数**3179904**，然后单击**搜索**。
   
    该修补程序列表显示，例如， **StorSimple 8000 系列的累积软件捆绑更新 2.2**。
   
    ![搜索目录](./media/storsimple-install-update2-hotfix/HCS_SearchCatalog1-include.png)
4. 单击 **“添加”**。 该更新添加到购物篮中。
5. 上表中列出的任何其他的修补程序的搜索 (**3103616**， **3146621**)，并添加到购物篮的每个。
6. 单击**查看购物篮**。
7. 单击“下载” 。 指定或**浏览**到想要显示的下载的本地位置。 更新到指定位置下载并放置在具有相同名称的更新的子文件夹。 也可以将文件夹复制到可从设备访问的网络共享。
...
4. 类型**Y**当系统提示确认安装修补程序。
   ...
   > [!IMPORTANT]
   > 如果安装更新 2.2，仅安装二进制文件开头为 all hcsmdssoftwareudpate。 不要安装 Ci 和前面加上所有 cismdsagentupdatebundle MDS 代理更新。 如果不这样做将导致错误。 
...
1. 当软件更新已完成，请验证系统软件版本。 键入：
   ...
   * `MdsAgentVersion: 30.0.4698.16` 
     ...
     > [!IMPORTANT]
     > 你必须重新启动主动控制器通过`Restart-HcsController`cmdlet 之前应用剩余更新。 
     > 
     > 
2. 重复步骤 3-5 以安装剩余的常规模式修补程序。
   
   * ISCSI 更新 KB3146621
   * WMI 更新 KB3103616
3. 如果你要更新从更新 2，跳过此步骤。 如果你要更新从 Update 2 之前的版本，你将需要下载：
