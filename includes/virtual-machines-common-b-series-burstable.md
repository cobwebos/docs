...
**答**：当 VM 是"REDPLOYED"，VM 将移到另一个节点，累计信用额度将丢失。 如果 VM 已停止/已启动，但仍保留在同一节点，VM 将保留累积的信用额度。 只要 VM 在节点上重新启动，它就会获取初始信用额度，对于 Standard_B8ms，该信用额度为 240 分钟。
    
### <a name="q-what-happens-if-i-deploy-an-unsupported-os-image-on-b1ls"></a>问：如果我部署 B1ls 上不受支持的 OS 映像，会发生什么情况？
**答**：B1ls 仅支持 Linux 映像，如果另一个操作系统映像部署任何你可能不会获得最佳客户体验。
    
### <a name="q-why-is-there-no-pricing-information-for-b1ls-windows"></a>问：为何没有 B1ls windows 无定价信息？
**答**：B1ls 仅支持 Linux 映像，如果在部署任何其他 OS 映像可能无法获得最佳客户体验，但你将需要支付。
