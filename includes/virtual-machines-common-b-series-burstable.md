...
| Size             | vCPU  | 内存：GiB | 临时存储 (SSD) GiB | VM 的基本 CPU 性能 | VM 的最大 CPU 性能 | 初始额度 | 累积的积分/小时 | 最大累积积分 | 最大数据磁盘数 | 最大缓存吞吐量和临时存储吞吐量：IOPS/MBps | 最大非缓存磁盘吞吐量：IOPS/MBps | 最大 NIC 数 |          
|---------------|-------------|----------------|----------------------------|-----------------------|--------------------|--------------------|--------------------|----------------|----------------------------------------|-------------------------------------------|-------------------------------------------|----------|
...
请考虑 office 签出/应用程序。 在应用程序需要在营业时间内的 CPU 突发情况但不是太大的在空闲时间的计算能力。 在此示例中，工作负荷需要使用的 RAM，一起有效地使用 64GiB 16vCPU 虚拟机。

下表显示每小时的流量数据和图表是该流量的可视化表示形式。
...
![每小时流量数据图表](./media/virtual-machines-common-b-series-burstable/office-workload.png)

| 场景 | Time | CPU 使用率 （%） | 信用额度累积<sup>1</sup> | 可用的信用额度 |
| --- | --- | --- | --- | --- |
| B16ms 部署 | 部署 | 部署  | 480 （初始信用额度） | 480 |
| 没有流量 | 0:00 | 0 | 162 | 642 |
| 没有流量 | 1:00 | 0 | 162 | 804 |
| 没有流量 | 2:00 | 0 | 162 | 966 |
| 没有流量 | 3:00 | 0 | 162 | 1128 |
| 没有流量 | 4:00 | 0 | 162 | 1290 |
| 没有流量 | 5:00 | 0 | 162 | 1452 |
| 低流量 | 6:00 | 270 | 0 | 1452 |
| 员工进入办公室 （应用程序需要 80%的 vCPU） | 7:00 | 1280 | -606 | 846 |
| 员工继续来到 office （应用程序需要 80%的 vCPU） | 8:00 | 1280 | -606 | 240 |
| 低流量 | 9:00 | 270 | 0 | 240 |
...
| 正在签出 (应用程序需要 100 %vcpu) 的员工 | 17:00 | 1600 | -798 | 216 |
| 低流量 | 18:00 | 270 | 0 | 216 |
| 低流量 | 19:00 | 270 | 0 | 216 |
| 低流量 | 20:00 | 50 | 132 | 348 |
| 低流量 | 21:00 | 50 | 132 | 480 |
| 没有流量 | 22:00 | 0 | 162 | 642 |
| 没有流量 | 23:00 | 0 | 162 | 804 |

<sup>1</sup>信用额度累积/信用额度在一小时内使用等效于： `((Base CPU perf of VM - CPU Usage) / 100) * 60 minutes`。  

对于 D16s_v3 具有 16 Vcpu 和 64 GiB 内存的每小时薪金是 $0.936 每小时 (每月美元 673.92) 和 B16ms 与 16 Vcpu 和 64 GiB 内存速率是每小时 (每月美元 547.86) 0.794 美元。 <b> 这会导致节省 15%！</b>
...
### <a name="q-how-can-i-calculate-credits-accumulated-and-used"></a>问：如何计算累积和使用的信用额度？
**答**：可以使用以下公式： 

（基本 CPU 性能的 VM 的 CPU 使用率） / 100 = 信用额度银行或每分钟使用

在上面实例，例如您的基线是 20%，如果您使用累积的 cpu 的 10%（20%-10%) / 每分钟 100 = 0.1 信用额度。
