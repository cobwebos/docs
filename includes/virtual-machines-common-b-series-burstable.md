...
| 大小             | vCPU  | 内存：GiB | 临时存储 (SSD) GB | VM 的基本 CPU 性能 | VM 的最大 CPU 性能 | 初始信用额度 | 每小时盈余 | 最大学分 | 最大数据磁盘数 | 缓存和临时存储的最大吞吐量： IOPS/MBps | 非缓存磁盘最大吞吐量：IOPS / MBps | 最大 NIC 数 |          
|---------------|-------------|----------------|----------------------------|-----------------------|--------------------|--------------------|--------------------|----------------|----------------------------------------|-------------------------------------------|-------------------------------------------|----------|
...
| 方案 | 时间 | CPU 使用率（%） | 累计信用额度<sup>1</sup> | 可用信用额度 |
| --- | --- | --- | --- | --- |
...
**答**：我们将在未来几周内介绍2个新指标，**信用**指标将允许你查看 vm 已获多少个信用额度，**积分**指标将显示 vm 已从银行消耗了多少 CPU 信用额度。    你将能够从门户的指标窗格中查看这些指标，也可以编程方式通过 Azure Monitor API 查看这些指标。
...
### <a name="q-what-happens-if-i-run-out-of-credits"></a>问：如果用完了信用额度，会发生什么情况？
**答**：如果 VM 消耗了所有可用的信用额度，则其允许仅使用基线 CPU，而不能在该基线之上突发。 
