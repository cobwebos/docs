...
云网关是一个系统，它允许从多个不同的站点跨公共网络空间的设备或现场网关进行远程通信，通常面向基于云的控制和数据分析系统，即此类系统的联盟。 在某些情况下，云网关可立即提升特殊用途设备的终端（如平板电脑或手机）访问速度。 在此处所述的上下文中，“云”是指未绑定到附加设备或现场网关的同一站点的专用数据处理系统。 此外，在云区域中，用于防止物理访问目标的操作措施并不一定要向“公有云”基础结构公开。  
...
**篡改**：设备可以重新配置为在控制系统未知的状态下运行（在已知的校准参数之外），从而提供可能被误解的数据。

**欺骗/篡改/** 拒绝：如果不加以保护（消费者遥控器很少如此），攻击者可以匿名操作设备的状态。 遥控器就是很好的例子，它既可以遥控电视机，也是很流行的恶作剧工具。
