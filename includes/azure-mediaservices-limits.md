...
| 每个 AMS 帐户处于运行状态的流式处理终结点数</p></td> |2</p></td> |N/A<sup>1</sup> |
| 每个流式处理终结点的流式处理单位数 </p></td> |10 </p></td> |不适用<sup>1</sup> |
| 每个 AMS 帐户的媒体保留单位 (Ru) </p></td> |25 (S1, S2)<br/>10 (S3)</p></td> |不适用<sup>1</sup> |
| 存储帐户 | |1,000<sup>5</sup> |
| 策略 | |1,000,000<sup>6</sup> |
 
<sup>1</sup> 你可以通过开具支持票证，来请求更新此配额的限制。 不要通过创建更多 AMS 帐户来提高限制，而是应提交支持票证。 另请注意，S3 RU 在印度西部和巴西南部不可用。
