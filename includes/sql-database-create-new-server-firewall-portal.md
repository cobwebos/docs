...
8. 单击“保存”以创建服务器级防火墙规则。
	...
	>[AZURE.IMPORTANT] 你的客户端 IP 地址可能会不定时地更改，你可能需要创建新防火墙规则才能访问服务器。可使用“必应”[](http://www.bing.com/search?q=my%20ip%20address)查看 IP 地址。然后添加一个 IP 地址或一系列 IP 地址。有关详细信息，请参阅[管理防火墙设置](sql-database-configure-firewall-settings.md#manage-existing-server-level-firewall-rules-through-the-azure-portal)。
