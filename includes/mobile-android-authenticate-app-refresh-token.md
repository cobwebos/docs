在简单的情况下，我们的令牌缓存应该有效，但是，在令牌到期或撤销时，会发生什么情况？ 该应用未运行时，该令牌可能会到期。 这就意味着令牌缓存无效。 应用实际运行时，该令牌可能也已过期。 结果是 HTTP 状态代码 401“未授权”。 
...
在本节中，你将定义 ServiceFilter，它将检测 HTTP 状态码 401 响应，并触发令牌和令牌缓存的刷新。 此外，在身份验证期间，此 ServiceFilter 将阻止其他出站请求，以便这些请求可以使用刷新的令牌。
...
4. 在 ToDoActivity.java 文件中，将以下方法添加到 ToDoActivity 类。 此方法将触发等待，然后在身份验证完成时更新出站请求中的令牌。 
...
1. 在 ToDoActivity.java 文件中，更新 ToDoActivity 类的 `authenticate` 方法，使它接受一个布尔型参数，以允许强制刷新令牌和令牌缓存。 我们还需要在身份验证完成时，通知任何被阻止的线程以便其可以选取新令牌。
...
2. 在 ToDoActivity.java 文件中，在 ToDoActivity 类中，为新 `RefreshTokenCacheFilter` 类添加此代码：
...
1. 在 ToDoActivity.java 文件中，在 ToDoActivity 类中，为新 `ProgressFilter` 类添加此代码：
...
2. 在 ToDoActivity.java 文件中，更新 `onCreate` 方法，如下所示：
