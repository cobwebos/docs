...
简而言之，使用设备克隆将允许解决方案后端为托管的设备指定所需配置，不需要发送特定的命令。 此方法让设备负责确定更新其状态的最佳方式（对于特定设备条件影响即时执行特定命令的能力的 IoT 方案而言十分重要），同时继续向解决方案后端报告更新过程的当前状态和潜在的错误条件。 此模式可用来管理大量设备，因为它让解决方案后端能够跨所有设备完全掌握配置流程状态。
