...
有几种方法可用于验证容器是否正在运行。 找到相关容器的*外部 IP*地址和公开端口，并打开你喜欢的 web 浏览器。 使用以下各种请求 Url 来验证容器是否正在运行。 下面列出的示例请求 Url 是 `http://localhost:5000`的，但特定容器可能会有所不同。 请记住，你要依赖容器的*外部 IP*地址和公开的端口。
