...
| 旧的免费层<br>（已于 2016 年 4 月推出） | 500 MB | 7 天 | 当工作区达到 500 MB 的每日限制时，数据引入会停止，并在第二天开始时恢复。 一天的时间基于 UTC。 请注意，由 Azure 安全中心收集的数据不包含在这一 500 MB 的每日限制内，并在超出此限制后继续进行收集。  |
...
| 表中的最大列数         | 500 | |
| 列名称的最大字符数 | 500 | |
| 满负荷的区域 | 美国中西部 | 目前，不能在这个区域中新建工作区，因为该区域已达到临时容量限制。 此限制计划由2019年11月结束。 |
