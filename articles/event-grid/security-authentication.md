...
可以通过使用 Azure Active Directory 进行身份验证并授权事件网格将事件发布到终结点，从而保护 webhook 终结点。 你将需要创建一个 Azure Active Directory 应用程序，在应用程序授权事件网格中创建角色和服务主体，并将事件订阅配置为使用 Azure AD 应用程序。 [了解如何用事件网格配置 AAD](secure-webhook-delivery.md)。
