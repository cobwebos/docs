...
事件网格提供了用于管理事件订阅的两个内置角色。 这些角色是 `EventSubscription Contributor (Preview)` 和 `EventSubscription Reader (Preview)`。 实现事件域时，它们非常重要。 有关授予的操作的详细信息，请参阅[事件域 - 访问管理](event-domains.md#access-management)。
