...
2. **ValidationURL 握手（手动）** ：在某些情况下，无法访问终结点的源代码来实现 ValidationCode 握手。 如果使用第三方服务（例如 [Zapier](https://zapier.com) 或 [IFTTT](https://ifttt.com/)），则无法以编程方式使用验证码进行响应。
   ...
   提供的 URL 有效时间为5分钟。 在该时间内，事件订阅的预配状态为 `AwaitingManualAction`。 如果在5分钟内未完成手动验证，则设置状态将设置为 `Failed`。 你将必须在开始手动验证之前重新创建事件订阅。
...
在创建事件订阅的过程中，如果看到一条错误消息，如 "尝试验证提供的终结点 https： \//你的终结点，则此操作失败。 有关更多详细信息，请访问 https： \//esvalidation），它指示在验证握手中出现故障。 若要解决此错误，请验证以下各方面：
