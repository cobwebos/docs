...
| 属性名称 | 说明 |
| ---------------- | ------------ |
| `api__deliveryPolicyLimits__maxBatchSizeInBytes` | `ApproxBatchSizeInBytes`旋钮允许的最大值。 默认值是 `1_058_576`。
| `api__deliveryPolicyLimits__maxEventsPerBatch` | `MaxEventsPerBatch`旋钮允许的最大值。 默认值是 `50`。
| `broker__defaultMaxBatchSizeInBytes` | 仅`MaxEventsPerBatch`指定时的最大传递请求大小。 默认值是 `1_058_576`。
| `broker__defaultMaxEventsPerBatch` | 仅`MaxBatchSizeInBytes`指定了时要添加到批的最大事件数。 默认值是 `10`。
...
| 属性名称 | 说明 |
| ---------------- | ------------ |
| `metrics__reporterType` | 指标终结点的报告器类型。 默认值`none`为并禁用指标。 设置以`prometheus`启用 Prometheus 处于阐释格式的指标。