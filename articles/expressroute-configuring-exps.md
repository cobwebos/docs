<properties
   pageTitle="通过 Exchange 提供商配置 ExpressRoute 连接"
   description="本文将指导通过 Exchange 提供商 (EXP) 设置 ExpressRoute。"
...
- 以下虚拟网络要求： 
	...
	- 你的网络的 AS 编号。有关 AS 编号的详细信息，请参阅"自治系统 (AS) 编号"。
...
Windows PowerShell 是一个功能强大的脚本编写环境，可用于在 Azure 中控制和自动执行工作负荷的部署和管理。有关详细信息，请参阅 [MSDN](https://msdn.microsoft.com/library/windowsazure/jj156055.aspx) 中的 PowerShell 文档。
...
1. **为 ExpressRoute 导入 PowerShell 模块。**
	...
	该程序包包含以下 cmdlet：
	- Get-AzureDedicatedCircuitServiceProvider - 列出所有连接服务提供商及其位置和支持的带宽
	- Get-AzureDedicatedCircuit - 列出此订阅中创建的所有线路及其属性 
	- Get-AzureDedicatedCircuitLink - 列出特定线路的所有线路连接
	- New-AzureDedicatedCircuit - 用于创建新的专用线路
	- New-AzureDedicatedCircuitLink - 用于将线路连接到 VNet
	- Remove-AzureDedicatedCircuit - 用于删除线路
	- Remove-AzureDedicatedCircuitLink - 用于取消 VNet 与线路的连接
	- Get-AzureBGPPeering - 用于列出 BGP 会话的详细信息（仅适用于 ExpressRoute-Direct 线路）
	- New-AzureBGPPeering - 用于为线路创建新的 BGP 会话（仅适用于 ExpressRoute-Direct 线路）
	- Remove-AzureBGPPeering - 用于删除线路的 BGP 会话（仅适用于 ExpressRoute-Direct 线路）
	- Set-AzureBGPPeering - 用于更新线路的 BGP 参数（仅适用于 ExpressRoute-Direct 线路）
1. **获取支持的提供商、位置和带宽的列表。**
...
1. **发出对服务密钥的请求，并将该请求传递给你的 Exchange 提供商。**你将使用 PowerShell cmdlet 发出此请求。对于此示例，我们将使用 Equinix 作为 Exchange 服务提供商，并将在硅谷指定一条 1Gbps ExpressRoute 线路。如果你使用的是其他提供商和其他设置，请在发出请求时用该信息替换。
	...
	你可以随时使用 Get-AzureCircuit cmdlet 检索此信息。进行不带任何参数的调用将列出所有线路。你的服务密钥将在 ServiceKey 字段中列出。
		...
		ServiceProviderProvisioningState : NotProvisioned
		Status                           : Enabled
1. **将服务密钥发送给你的 Exchange 提供商。**你的服务提供商将使用服务密钥启用其连接端。
1. **请定期检查服务密钥的状态。**此状态在你的提供商已启用你的线路时会让你了解。启用了线路后， *ServiceProviderProvisioningState* 将显示为 *Provisioned*，如下例所示。
...
1. **配置虚拟网络的路由。**我们使用 BGP 会话交换路由，同时也确保我们具有高可用性。使用下面的示例为你的线路创建 BGP 会话。创建会话时，请替换为你自己的值。
...
1. **配置公共 IP 地址上托管的服务的路由。**我们使用 BGP 会话交换路由，同时也确保我们具有高可用性。使用下面的示例为你的线路创建 BGP 会话。创建会话时，请替换为你自己的值。
...
1. **配置虚拟网络和网关。**请参阅[为 ExpressRoute 配置虚拟网络和网关](https://msdn.microsoft.com/library/azure/dn643737.aspx)。请注意，网关子网必须是 /28 才能用于 ExpressRoute 连接。

1. **将你的网络连接到线路。**只有在你已确认线路已转为以下状态后，才继续按下面的说明进行操作：
	- ServiceProviderProvisioningState：Provisioned
	- Status：Enabled
