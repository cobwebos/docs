<properties
	pageTitle="在 Linux 和 Mac 上创建 SSH 密钥 | Microsoft Azure"
...
本文将创建 *ssh-rsa* 格式的密钥文件，它们是 Resource Manager 上的部署建议使用的文件，*ssh-rsa* 密钥是在[门户](https://portal.azure.com)上进行经典部署和 Resource Manager 部署所必需的。
...
```
`~/.ssh` 目录的列表。如果 `~/.ssh` 目录不存在，`ssh-keygen` 会创建它，并且还设置相应的所有权和文件模式。
...
`IdentityFile /home/ahmet/.ssh/id_id_rsa` = 要用于身份验证的 SSH 私钥和对应公钥。
