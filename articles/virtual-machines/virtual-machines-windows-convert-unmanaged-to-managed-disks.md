...
在开始之前，请确保已查看[规划迁移到托管磁盘](virtual-machines-windows-on-prem-to-azure.md#plan-for-the-migration-to-managed-disks)。
...
本部分将介绍如何将标准非托管磁盘上的现有 Azure VM 转换为高级托管磁盘。 要使用高级托管磁盘，你的 VM 必须使用支持高级存储的 [VM 大小](virtual-machines-windows-sizes.md)。


1.  首先，设置公共参数。 请确保你所选择的 [VM 大小](virtual-machines-windows-sizes.md)支持高级存储。
