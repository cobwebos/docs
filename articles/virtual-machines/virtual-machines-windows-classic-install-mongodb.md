...
[MongoDB][MongoDB] 是一个受欢迎的开源、高性能 NoSQL 数据库。本文将指导你完成以下过程：使用 [Azure 经典门户][AzurePortal]创建新的 Windows Server 虚拟机 (VM)，创建数据磁盘并将其附加到 VM，然后安装并配置 MongoDB。如果想使用 Azure 中现有的 VM，可以直接跳到[安装并配置 MongoDB](#install-and-run-mongo-on-win2k8-vm)。
