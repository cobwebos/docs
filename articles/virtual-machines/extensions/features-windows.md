...
有关支持的操作系统和安装说明的信息，请参阅 Azure[虚拟机代理](agent-windows.md)。
...
有关详细信息，请参阅[完整的资源管理器模板](https://github.com/Microsoft/dotnet-core-sample-templates/tree/master/dotnet-core-music-windows)。
...
在使用扩展的 Azure IaaS VM 上，在证书控制台中，您可能会看到具有主题**_为 Windows Azure CRP 证书生成器_** 的证书。 在经典 RDFE VM 上，这些证书的主题名称为**_Windows Azure 服务管理，用于扩展_**。

这些证书保护 VM 与其主机之间的通信，在传输扩展使用的受保护设置（密码、其他凭据）时。 证书由 Azure 结构控制器生成并传递到 VM 代理。 如果每天停止并启动 VM，则结构控制器可能会创建新的证书。 证书存储在计算机的个人证书存储中。 可以删除这些证书。 如果需要，VM 代理将重新创建证书。
...
1. 要检查 Windows 来宾代理日志，请查看在*C：\WindowsAzure_Logs_WaAppAgent.log*中预配扩展时的活动
...
在针对 VM 上运行 VM 扩展后，使用[Get-AzVM](https://docs.microsoft.com/powershell/module/az.compute/get-azvm)返回扩展状态。 *Substatuses[0]* 显示扩展预配成功，这意味着，该扩展已成功部署到 VM，但 VM 中的扩展执行失败 (*Substatuses[1]*)。
