...
## <a name="step-3-optional-create-a-storage-account"></a>步骤 3：（可选）创建存储帐户

在创建使用 [Azure 托管磁盘](../storage/storage-managed-disks-overview.md)或非托管磁盘的虚拟机时，目前有一个选项。 如果选择使用非托管磁盘，则必须创建一个[存储帐户](../storage/storage-introduction.md)，以便存储创建的虚拟机所使用的虚拟硬盘。 如果选择使用托管磁盘，则不需存储帐户。 存储帐户名称必须为 3 到 24 个字符，并且只能包含数字和小写字母。
...
现已做好所有的准备，接下来可以开始创建虚拟机。 可以使用[应用商店映像](virtual-machines-windows-cli-ps-findimage.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)、[自定义通用化（系统准备型）映像](virtual-machines-windows-create-vm-generalized.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)或[自定义专用（非系统准备型）映像](virtual-machines-windows-create-vm-specialized.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)创建虚拟机。 此示例使用应用商店提供的 Windows Server 映像。 
...
6. 如果使用非托管磁盘，请运行以下命令，以便定义 VM 硬盘的名称和位置；否则，请跳过此步骤。 非托管磁盘的虚拟硬盘文件存储在容器中。 以下命令在你创建的存储帐户中名为 **vhds/myOsDisk1.vhd** 的容器中创建该磁盘。
...
7. 将操作系统磁盘信息添加到 VM 配置。 此命令创建名为 **myOsDisk1** 的磁盘。
    ...
    如果使用托管磁盘，请运行以下命令，以便在配置中设置操作系统磁盘：
    ...
    如果使用非托管磁盘，请运行以下命令，以便在配置中设置操作系统磁盘：
