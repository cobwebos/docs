---
title: "使用负载平衡集群集化 MySQL | Microsoft Docs"
...
---
# <a name="using-load-balanced-sets-to-clusterize-mysql-on-linux"></a>使用负载平衡集群集化 Linux 上的 MySQL
[!INCLUDE [learn-about-deployment-models](../../includes/learn-about-deployment-models-classic-include.md)]

有关用于部署 MySQL 群集的 Resource Manager 模板，请参阅[此处](https://azure.microsoft.com/documentation/templates/mysql-replication/)。

本文旨在探索并说明可用于在 Microsoft Azure 上部署基于 Linux 的高度可用服务的不同方法，并作为入门书探索 MySQL Server 的高可用性。 [第 9 频道](http://channel9.msdn.com/Blogs/Open/Load-balancing-highly-available-Linux-services-on-Windows-Azure-OpenLDAP-and-MySQL)提供了此方法的视频演示。
...
创建第一个虚拟机后（从技术上讲，创建云服务时），我们将继续创建第二个虚拟机 `hadb02`。 对于第二个虚拟机，我们还将通过门户使用库中的 Ubuntu 13.10 VM，但我们将选择使用现有的云服务 `hadb.cloudapp.net` 而不是新建一个。 应为我们自动选择网络和可用性集。 也将创建 SSH 终结点。
...
## <a name="setting-up-the-cluster"></a>设置群集
...
### <a name="setting-up-drbd"></a>设置 DRBD
...
### <a name="mounting-the-drbd-resource"></a>装载 DRBD 资源
...
## <a name="setting-up-mysql"></a>设置 MySQL
...
### <a name="creating-the-mysql-load-balanced-set"></a>创建 MySQL 负载平衡集
返回到门户并浏览到 `hadb01` VM，然后然后“终结点”。 创建一个新终结点，从下拉列表中选择“MySQL (TCP 3306)”，并选中“新建负载均衡集”框。 我们将调用我们的负载平衡终结点 `lb-mysql`。 我们将保留大多数选项不动，但时间除外，我们会将其减少到 5（秒，最小值）

创建终结点后，我们将转到 `hadb02` 终结点并创建新的终结点，但我们将选择 `lb-mysql`，然后从下拉菜单中选择 MySQL。 还可以将 Azure CLI 用于此步骤。

此时，我们已具备对群集执行手动操作所需的一切条件。

### <a name="testing-the-load-balanced-set"></a>测试负载平衡集
...
## <a name="setting-up-corosync"></a>设置 Corosync
...
## <a name="setting-up-pacemaker"></a>设置 Pacemaker
...
![crm_mon 节点已停止](media/virtual-machines-linux-classic-mysql-cluster/image002.png)

并且此屏幕截图显示这两个节点（一个主节点和一个从节点）：

![crm_mon 操作主/从](media/virtual-machines-linux-classic-mysql-cluster/image003.png)
...
硬方式是通过门户关闭主 VM (hadb01) 或更改 VM 上的运行级别（即，停止、关闭），然后，我们将通过指示主节点将要关闭来帮助 Corosync 和 Pacemaker。 我们可以对此进行测试（适用于维护窗口），但我们也可以通过只冻结虚拟机来强制实施该方案。
