...
本文介绍如何将 SSH 服务器锁定到 Linux，以确保在安全方面符合最佳实践，并通过使用 SSH 密钥而非密码加快 SSH 登录过程。  为了进一步锁定 SSHD，我们将禁止根用户登录、限制可以通过已审批组列表登录的用户数（禁用 SSH 协议版本 1）、设置最小密钥位，以及配置空闲用户自动注销。  本文的要求如下：一个 Azure 帐户（[获取免费试用帐户](https://azure.microsoft.com/pricing/free-trial/)）以及 [SSH 公钥和私钥文件](virtual-machines-linux-mac-create-ssh-keys.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。
