---
title: "在 Azure 中 Windows VM 的部署模型之间迁移 | Microsoft 文档"
...
## <a name="detailed-guidance-on-migration"></a>详细迁移指南
...
| 负载平衡的终结点集 |负载平衡器 |在经典部署模型中，平台已经为每个云服务分配一个隐式负载平衡器。 在迁移期间，将创建新的负载均衡器资源，负载均衡终结点集将成为负载均衡器规则。 |
...
| 保留 IP |具有静态分配方法的公共 IP 地址 |与负载平衡器关联的保留 IP 将在迁移云服务或虚拟机的过程中一起迁移。 目前不支持进行未关联的保留 IP 迁移。 |
