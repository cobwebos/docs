...
可将 Powershell cmdlet 与 <!-- should be deleted --> ARM 或 <!-- should be deleted --> ASM 配合使用，以便打包、发布和监视 DSC 扩展部署。下面列出的 cmdlet 是 ASM 模块，<!-- should be deleted -->但是，可以将“Azure”替换为“AzureRM”以使用 ARM 模型。例如，`Publish-AzureVMDscConfiguration` 将使用 ASM，而 `Publish-AzureRmVMDscConfiguration` 将使用 ARM。<!-- should be deleted -->
...
**配置参数**：如果配置函数采用参数，请使用 `argumentName1=value1,argumentName2=value2` 格式在此处输入。<!-- should be deleted -->请注意，此处的格式与通过 PowerShell cmdlet 或 ARM 模板接受配置参数的方式不同。<!-- should be deleted -->
