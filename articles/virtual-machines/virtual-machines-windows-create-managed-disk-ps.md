...
在此示例中，创建一个空数据磁盘作为托管磁盘，并将其分配给参数 **$dataDisk2** 以供稍后使用。 将空数据磁盘附加到正在运行的 VM 且登录到该 VM 后，需要使用 diskmgmt.msc 或[远程使用 WinRM 和一个脚本](virtual-machines-windows-attach-disk-ps.md#initialize-the-disk)对其进行初始化。
...
- 如果已有 VM，可以[附加数据磁盘](virtual-machines-windows-attach-disk-portal.md)。
