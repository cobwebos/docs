...
可使用 **ssh** 客户端连接到 Azure 中的 Linux VM，并对其进行管理。 Windows 计算机通常未安装 **ssh** 客户端。 在以下程序包中包含可以安装的常见 Windows SSH 客户端：
...
> [!NOTE]
> 最新的 Windows 10 周年更新包含 Bash for Windows。 使用此功能可以对 Linux 运行 Windows 子系统以及访问 SSH 客户端等实用工具。 Bash for Windows 仍在开发，当前版本视为 beta 版本。 有关 Bash for Windows 的详细信息，请参阅 [Bash on Ubuntu on Windows](https://msdn.microsoft.com/commandline/wsl/about)（Windows 上的 Bash on Ubuntu）。
> 
> 
...
1. **ssh-rsa** 密钥是使用 [Azure 门户](https://portal.azure.com)进行任何部署所必需的，并且是使用 [Azure CLI](../xplat-cli-install.md) 进行 Resource Manager 部署所必需的。
   * 这些密钥通常是大多数人都需要的。
2. 使用[经典门户](https://manage.windowsazure.com)创建 VM 时需要 `.pem` 文件。 使用 [Azure CLI](../xplat-cli-install.md) 的经典部署也支持这些密钥。
...
3. 将在当前工作目录中创建新的私钥和证书。 最佳安全做法是，应在私钥上设置权限，以便只有你可以访问它：
...
7. 公钥将显示在“PuTTYgen”窗口的顶部。 创建 Linux VM 时，将此公钥复制并粘贴到 Azure 门户或 Azure Resource Manager 模板中。 也可以单击“保存公钥”将副本保存到计算机中：
    ...
    ![在 Azure 门户中创建 VM 时使用公钥](./media/virtual-machines-linux-ssh-from-windows/use-public-key-azure-portal.png)
