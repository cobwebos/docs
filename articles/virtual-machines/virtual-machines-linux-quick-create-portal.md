...
本文说明如何使用 [Azure 门户预览版](https://portal.azure.cn/)快速创建 Linux 虚拟机。唯一的要求是 [Azure 帐户](/pricing/1rmb-trial/)和 [SSH 公钥与私钥文件](/documentation/articles/virtual-machines-linux-mac-create-ssh-keys/)。

> [AZURE.NOTE] 如果选择使用密码来保护对你的 VM 的访问，则密码必须多于 12 个字符，并且必须至少包含一个大写字符、一个小写字符、一个特殊字符以及一个数字。


1. 使用你的 Azure 帐户标识登录到 Azure 门户预览版，然后单击左上角的“+ 新建”：
...
5. 在“设置”中，保留对“存储和网络”值的默认设置，然后单击“确定”查看摘要。请注意，磁盘类型已通过选择 DS1（**S** 表示 SSD）设置为高级 SSD。
