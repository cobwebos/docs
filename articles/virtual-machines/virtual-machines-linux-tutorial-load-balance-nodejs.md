...
# <a name="build-a-highly-available-application-on-linux-virtual-machines-in-azure"></a>在 Azure 中的 Linux 虚拟机上生成高可用性应用程序
...
若要完成本教程，请打开终端窗口，并确保已安装最新的 [Azure CLI 2.0](/cli/azure/install-azure-cli)。 如果尚未登录到 Azure 订阅，请使用 [az login](/cli/azure/#login) 登录，然后按照屏幕上的说明进行操作：
...
使用 [az vm availability-set create](/cli/azure/vm/availability-set#create) 创建可用性集。 以下示例创建一个名为 `myAvailabilitySet` 的可用性集：
...
若要在 Internet 上访问你的应用，请为负载均衡器分配公共 IP 地址。 使用 [az network public-ip create](/cli/azure/public-ip#create) 创建公共 IP 地址。 下例创建名为 `myPublicIP` 的公共 IP 地址：
...
使用 [az network lb create](/cli/azure/network/lb#create) 创建负载均衡器。 下例使用 `myPublicIP` 地址创建一个名为 `myLoadBalancer` 的负载均衡器：
...
使用 [az network lb probe create](/cli/azure/network/lb/probe#create) 创建运行状况探测器。 下例创建一个 `myHealthProbe` 运行状况探测器，用于监视每个 VM：
...
使用 [az network lb rule create](/cli/azure/network/lb/rule#create) 创建负载均衡器规则。 以下示例创建一个名为 `myLoadBalancerRule` 的规则、使用 `myHealthProbe` 运行状况探测器并平衡端口 `80` 上的流量：
...
若要为 VM 提供网络连接，请使用 [az network vnet create](/cli/azure/vnet#create) 创建虚拟网络。 以下示例创建一个名为 `myVnet` 的虚拟网络和名为 `mySubnet` 的子网：
...
若要允许 Web 流量到达你的应用，请使用 [az network nsg rule create](/cli/azure/network/nsg/rule#create) 创建网络安全组规则。 以下示例创建一个名为 `myNetworkSecurityGroupRule` 的网络安全组规则：
...
使用 [az network nic create](/cli/azure/network/nic#create) 创建虚拟 NIC。 以下示例创建三个虚拟 NIC。 （你在以下步骤中为应用创建的每个 VM 各使用一个虚拟 NIC）：
...
**cloud-init** 是自定义 VM 的常用方法。 可使用 **cloud-init** 安装程序包和写入文件。 由于 **cloud_init** 在初始部署期间运行，因此没有其他步骤可让你的应用运行。 VM 完成部署且该应用运行后，负载均衡器将开始分配流量。 有关使用 **cloud-init** 的详细信息，请参阅[在创建期间使用 cloud-init 自定义 Linux VM](virtual-machines-linux-using-cloud-init.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。
...
使用 [az vm create](/cli/azure/vm#create) 创建 VM。 以下示例创建三个 VM，并生成 SSH 密钥（如果它们尚不存在）：
...
使用 [az network nic ip-config address-pool remove](/cli/azure/network/nic/ip-config/address-pool#remove) 从后端地址池中删除 VM。 以下示例从 `myLoadBalancer` 中删除 **myVM2** 的虚拟 NIC：
...
执行 VM 维护后，或如果需要扩展容量，请使用 [az network nic ip-config address-pool add](/cli/azure/network/nic/ip-config/address-pool#add) 将 VM 添加到后端地址池。 以下示例将 **myVM2** 的虚拟 NIC 添加到 `myLoadBalancer`：
...
- [管理 Linux 虚拟机的可用性](virtual-machines-linux-manage-availability.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [Azure 负载均衡器概述](../load-balancer/load-balancer-overview.md)
- [使用网络安全组控制网络流量](../virtual-network/virtual-networks-nsg.md)
- [Azure CLI 示例脚本](virtual-machines-linux-cli-samples.md)
