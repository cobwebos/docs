...
>[!IMPORTANT]
> 当前在 Azure Marketplace 中提供的 RHEL 映像支持自带订阅（BYOS）或即用即付许可模式。 不支持在 BYOS 与即用即付许可之间进行[Azure 混合使用权益](../../windows/hybrid-use-benefit-licensing.md)和动态切换。 若要切换授权模式，你必须从相应的映像重新部署 VM。
