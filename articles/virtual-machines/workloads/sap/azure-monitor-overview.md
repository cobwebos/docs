...
下面是体系结构的主要重点：
 - **多实例**-客户可以在 VNET 中跨多个 sap sid （例如 HANA DB、HA 群集、Microsoft SQL server）为多个 sap sid 创建针对 SAP 解决方案的单个 Azure Monitor 资源的监视器。 
 - **多提供程序**-上述体系结构关系图以示例形式显示 SAP HANA 提供程序。 同样，客户可以为相应的组件（例如，HANA DB、HA 群集、Microsoft SQL server）配置其他提供程序，以便从这些组件收集数据。
...
## <a name="pricing"></a>定价
SAP 解决方案的 Azure Monitor 是免费产品（无许可费用）。 客户负责为托管资源组中的基础组件付费。
