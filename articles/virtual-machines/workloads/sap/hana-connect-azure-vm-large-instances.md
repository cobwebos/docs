...
当指时**地址空间**本文档中，为 Azure 虚拟网络允许使用的地址空间中。 此地址空间也是虚拟网络用于 BGP 路由传播的地址范围。 可在以下位置看到**地址空间**：

![Azure 门户中显示的 Azure 虚拟网络地址空间](./media/hana-overview-connectivity/image1-azure-vnet-address-space.png)

在上一示例中，使用 10.16.0.0/16，Azure 虚拟网络提供的宽广 IP 地址范围使用。 因此，此虚拟网络中后续子网中的所有 IP 地址范围可在该地址空间中包含其范围。 通常，对于 Azure 中的单个虚拟网络，我们不建议使用这么大的地址范围。 但是，让我们继续分析 Azure 虚拟网络中定义的子网：
...
在两个以前的图形中，**虚拟网络地址空间**同时涵盖**Azure VM 的子网 IP 地址范围**和的虚拟网络网关。
...
对于这些租户子网（VM 子网），可以使用偏好的任何命名标准。 但是，对于连接到 Azure 上的 SAP HANA（大型实例）ExpressRoute 线路的每个虚拟网络，**必须始终只有一个网关子网**。 **此网关子网必须命名为"GatewaySubnet"** 以确保正确放置 ExpressRoute 网关。

> [!WARNING] 
> 很重要，网关子网必须始终命名为"GatewaySubnet"。

可以使用多个 VM 子网和不连续的地址范围。 虚拟网络的**虚拟网络地址空间**必须涵盖这些地址范围。 可以是以聚合形式。 也可以是以 VM 子网和网关子网的确切范围列表形式。

下面汇总了有关要连接到 HANA 大型实例的 Azure 虚拟网络的重要事实：

- 必须提交**虚拟网络地址空间**到 Microsoft 时要执行的 HANA 大型实例的初始部署。 
...
已引入一些所需的部署 HANA 大型实例的 IP 地址范围。 但是，还有一些 IP 地址范围也很重要。 并非所有以下 IP 地址范围需要提交给 Microsoft。 但是，在发送初始部署请求之前，需要定义这些 IP 地址：

- **虚拟网络地址空间**：虚拟网络地址空间是在 Azure 虚拟网络中分配给地址空间参数的 IP 地址范围  。 这些网络连接到 SAP HANA 大型实例环境。 我们建议将此地址空间参数设置为多行值。 它应该由 Azure VM 的子网范围和 Azure 网关的子网范围组成。 此子网范围显示在前面的图中。 此范围不得与本地或服务器 IP 池或 ER-P2P 地址范围重叠。 如何获取这些 IP 地址范围？ 企业网络团队或服务提供商应提供一个或多个 IP 地址范围的网络内部未使用。 例如，Azure VM 的子网是 10.0.1.0/24，并将 Azure 网关子网的子网是 10.0.2.0/28。  我们建议你的 Azure 虚拟网络地址空间定义为：10.0.1.0/24 和 10.0.2.0/28。 尽管地址空间值可以聚合，仍我们建议使它们与子网范围匹配。 这样就可以避免在网络中其他较大的地址空间中重用空闲的 IP 地址范围。 虚拟网络地址空间是 IP 地址范围。  需要初始部署时，需要将其提交给 Microsoft。
...
最终需要在提交给 Microsoft 的可选 IP 地址范围：

- 如果您选择使用[ExpressRoute 全球覆盖](https://docs.microsoft.com/azure/expressroute/expressroute-global-reach)若要启用直接路由从本地到 HANA 大型实例单元，用户需要保留另一个/29 IP 地址范围。 此范围可能会与任何之前定义的其他 IP 地址范围重叠。
- 如果您选择使用[ExpressRoute 全球覆盖](https://docs.microsoft.com/azure/expressroute/expressroute-global-reach)若要启用直接路由从一个 Azure 区域中的 HANA 大型实例租户到另一个 Azure 区域中的另一个 HANA 大型实例租户，用户需要保留另一个/29 IP 地址范围. 此范围可能会与任何之前定义的其他 IP 地址范围重叠。

有关 ExpressRoute 全球覆盖和围绕 HANA 大型实例的使用情况详细信息，请检查文档：

- [SAP HANA （大型实例） 网络体系结构](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-network-architecture)
- [虚拟网络连接到 HANA 大型实例](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-connect-vnet-express-route)
...
此图形不显示的其他 IP 地址范围所需的使用 （可选） 的 ExpressRoute 全球覆盖。
...
在示例中，我们定义了一个涵盖 4096 个 IP 地址的较大范围，而不是为 Azure 虚拟网络的地址空间定义了两个较小范围。 定义这种较大的地址空间会使一些相当大的范围不被利用。 由于虚拟网络地址空间值用于 BGP 路由传播，使用未在本地或网络中其他位置使用的范围可能会导致路由问题。 此图形不显示的其他 IP 地址范围所需的使用 （可选） 的 ExpressRoute 全球覆盖。

我们建议你将与你使用的实际子网地址空间紧密对齐的地址空间。 如果需要，以后随时可以添加新的地址空间值，而不会导致虚拟网络出现故障。
