...
备注：在整个 SAP 世界内外的文档中，产品名称被引用为 Sybase ASE 或 SAP ASE，或者在某些情况下同时引用两者。 为了保持一致，我们在本文档中使用 SAP **ASE**的名称。

## <a name="operating-system-support"></a>操作系统支持
SAP 产品可用性矩阵包含每个 SAP 应用程序支持的操作系统和 SAP 内核组合。  完全支持 Linux 发行版 SUSE 12.x、SUSE 15.x、红帽 7.x。  甲骨文 Linux 作为 SAP ASE 的操作系统不受支持。  建议使用最新的 Linux 版本。 Windows 客户应使用 Windows 服务器 2016 或 Windows 服务器 2019 版本。  Windows 2012 等旧版本在技术上是受支持的，但始终建议使用最新的 Windows 版本。
...
Microsoft Azure 提供了许多不同的虚拟机类型，可运行最小的 SAP 系统和布局，也可运行包含成千上万用户的大型 SAP 系统和布局。 SAP 调整不同 SAP 认证 VM SKU 的 SAPS 编号，#1928533 在[SAP 支持说明](https://launchpad.support.sap.com/#/notes/1928533)中提供。

在 Windows 上安装 SAP ASE 的文档可在[适用于 Windows 的 SAP ASE 安装指南](https://help.sap.com/viewer/36031975851a4f82b1022a9df877280b/16.0.3.7/en-US/a660d3f1bc2b101487cbdbf10069c3ac.html)中找到

锁定内存中的页面是一种设置，它可以防止 SAP ASE 数据库缓冲区被分页。 此设置对于内存多的大型繁忙系统非常有用。 有关详细信息，请联系 BC-DB-SYB。 


## <a name="linux-operating-system-specific-settings"></a>Linux 操作系统特定设置
在 Linux VM`saptune`上，默认情况下应启用具有配置文件 SAP-ASE Linux 大页面，并且可以使用命令进行验证  

`cat /proc/meminfo` 

页面大小通常为 2048 KB。 有关详细信息，请参阅[Linux 上的文章"大页面](https://help.sap.com/viewer/ecbccd52e7024feaa12f4e780b43bc3b/16.0.3.7/en-US/a703d580bc2b10149695f7d838203fad.html)" 


## <a name="recommendations-on-vm-and-disk-structure-for-sap-ase-deployments"></a>有关 SAP ASE 部署的 VM 和磁盘结构的建议

SAP NetWeaver 应用程序 SAP ASE 支持 SAP[支持说明](https://launchpad.support.sap.com/#/notes/1928533)中列出的任何 VM 类型#1928533用于中型 SAP ASE 数据库服务器的典型 VM 类型包括 Esv3。  大型多字节数据库可以利用 M 系列 VM 类型。 通过启用 M 系列写入加速器，可以改进 SAP ASE 事务日志磁盘写入性能。 由于 SAP ASE 执行日志写入的方式，应谨慎使用 SAP ASE 测试写入加速器。  [查看 SAP 支持说明#2816580](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator)并考虑运行性能测试。  
写入加速器专为事务日志磁盘而设计。 磁盘级别缓存应设置为 NONE。 如果 Azure 写入加速器未显示与其他 DBMS 类似的改进，请不要感到惊讶。 根据 SAP ASE 写入事务日志的方式，Azure 写入加速器可能很少或根本没有加速。
建议为数据设备和日志设备单独使用磁盘。  系统数据库是 syb`saptools`安全性的，不需要专用磁盘，可以放置在包含 SAP 数据库数据和日志设备的磁盘上 

![SAP ASE 的存储配置](./media/dbms-guide-sap-ase/sap-ase-disk-structure.png)

### <a name="file-systems-stripe-size--io-balancing"></a>文件系统、条带大小& IO 平衡 
SAP ASE 将数据按顺序写入磁盘存储设备，除非另有配置。 这意味着具有四个设备的空 SAP ASE 数据库将仅将数据写入第一个设备。  仅当第一个设备已满时，其他磁盘设备才会写入。  每个 SAP ASE 设备的读取和写入 IO 量可能不同。 为了平衡所有可用 Azure 磁盘的磁盘 IO，需要使用 Windows 存储空间或 Linux LVM2。 在 Linux 上，建议使用 XFS 文件系统格式化磁盘。 应通过性能测试测试 LVM 条带大小。 128 KB 条带大小是一个很好的起点。 在 Windows 上，应测试 NTFS 分配单位大小 （AUS）。 64 KB 可用作起始值。 

建议配置自动数据库扩展，如 #1815695[配置 SAP 自适应服务器企业中自动数据库空间扩展](https://blogs.sap.com/2014/07/09/configuring-automatic-database-space-expansion-in-sap-adaptive-server-enterprise/)和 SAP[支持说明](https://launchpad.support.sap.com/#/notes/1815695)中所述。 

### <a name="sample-sap-ase-on-azure-virtual-machine-disk-and-file-system-configurations"></a>在 Azure 虚拟机、磁盘和文件系统配置上对 SAP ASE 进行示例 
下面的模板显示了 Linux 和 Windows 的示例配置。 在确认虚拟机和磁盘配置之前，请确保单个 VM 的网络和存储带宽配额足以满足业务需求。 还要记住，不同的 Azure VM 类型具有可附加到 VM 的不同最大磁盘数。 例如，E4s_v3 VM 具有 48 MB/秒存储 IO 吞吐量的限制。 如果数据库备份活动所需的存储吞吐量超过 48 MB/秒，则更大的 VM 类型具有更高的存储带宽吞吐量是不可避免的。 配置 Azure 存储时，还需要记住，特别是使用[Azure 高级存储](https://docs.microsoft.com/azure/virtual-machines/windows/premium-storage-performance)时，吞吐量和 IOPS 每 GB 容量确实会发生变化。 请参阅本文中有关此主题[的详细信息，Azure 中有哪些磁盘类型可用？](https://docs.microsoft.com/azure/virtual-machines/windows/disks-types) 特定 Azure VM 类型的配额记录在文章["内存优化的虚拟机大小](https://docs.microsoft.com/azure/virtual-machines/sizes-memory)"和链接到它的文章中。 

> [!NOTE]
>  如果 DBMS 系统从本地移动到 Azure，建议对 VM 执行监视并评估 CPU、内存、IOPS 和存储吞吐量。 将观察到的峰值值与上述文章中记录的 VM 配额限制进行比较

下面给出的示例是为了说明目的，可以根据个人需要加以修改。 由于 SAP ASE 的设计，数据设备的数量不如其他数据库重要。 本文档中详述的数据设备数量仅是指南。 
...
| 磁盘聚合 | 存储空间 | LVM2 | --- |
| 文件系统 | NTFS | Xfs |
| 格式块大小 | 需要工作负载测试 | 需要工作负载测试 | --- |
| * 和类型的数据磁盘 | 高级存储：2 x P10 （RAID0） | 高级存储：2 x P10 （RAID0）| 缓存 = 只读 |
| 日志磁盘的 * 和类型 | 高级存储：1 x P20  | 高级存储：1 x P20 | 缓存 = 无 |
| ASE 最大内存参数 | 90% 的物理 RAM | 90% 的物理 RAM | 假设单个实例 |
| 备份设备 * | 4 | 4| --- |
| 备份磁盘的 * 和类型 | 1 | 1 | --- |
...
| 文件系统 | NTFS | Xfs |
| 格式块大小 | 需要工作负载测试 | 需要工作负载测试 | --- |
| * 和类型的数据磁盘 | 高级存储：4 x P20 （RAID0） | 高级存储：4 x P20 （RAID0）| 缓存 = 只读 |
| 日志磁盘的 * 和类型 | 高级存储：1 x P20  | 高级存储：1 x P20 | 缓存 = 无 |
| ASE 最大内存参数 | 90% 的物理 RAM | 90% 的物理 RAM | 假设单个实例 |
| 备份设备 * | 4 | 4| --- |
| 备份磁盘的 * 和类型 | 1 | 1 | --- |
...
| 文件系统 | NTFS | Xfs |
| 格式块大小 | 需要工作负载测试 | 需要工作负载测试 | --- |
| * 和类型的数据磁盘 | 高级存储：4 x P30 （RAID0） | 高级存储：4 x P30 （RAID0）| 缓存 = 只读 |
| 日志磁盘的 * 和类型 | 高级存储：1 x P20  | 高级存储：1 x P20 | 缓存 = 无 |
| ASE 最大内存参数 | 90% 的物理 RAM | 90% 的物理 RAM | 假设单个实例 |
| 备份设备 * | 4 | 4| --- |
| 备份磁盘的 * 和类型 | 1 | 1 | --- |
...
| 文件系统 | NTFS | Xfs |
| 格式块大小 | 需要工作负载测试 | 需要工作负载测试 | --- |
| * 和类型的数据磁盘 | 高级存储：4+ x P30 （RAID0） | 高级存储：4+ x P30 （RAID0）| 缓存 = 只读，考虑 Azure 超级磁盘 |
| 日志磁盘的 * 和类型 | 高级存储：1 x P20  | 高级存储：1 x P20 | 缓存 = 无，请考虑 Azure 超磁盘 |
| ASE 最大内存参数 | 90% 的物理 RAM | 90% 的物理 RAM | 假设单个实例 |
| 备份设备 * | 16 | 16 | --- |
| 备份磁盘的 * 和类型 | 4 | 4 | 使用 LVM2/存储空间 |


### <a name="backup--restore-considerations-for-sap-ase-on-azure"></a>备份& Azure 上 SAP ASE 的还原注意事项
增加数据和备份设备的数量可提高备份和恢复性能。 建议将托管 SAP ASE 备份设备的 Azure 磁盘条带，如前面所示的表所示。 应注意平衡备份设备和磁盘的数量，并确保备份吞吐量不应超过 VM 吞吐量配额总数的 40%-50%。 建议将 SAP 备份压缩作为默认值。 更多详细信息请参阅文章：
...
请勿使用驱动器 D：*或 /temp 空间作为数据库或日志转储目标。

### <a name="impact-of-database-compression"></a>数据库压缩的影响
在 I/O 带宽可能成为限制因素的配置中，减少 IOPS 的度量值可能有助于扩展在 IaaS 方案中（如 Azure）中运行的工作负荷。 因此，我们建议确保先使用 SAP ASE 压缩，然后再将现有 SAP 数据库上传到 Azure。
...
数据和 LOB 压缩可以在 Azure 虚拟机托管的 VM 中运行，如同在本地运行一样。 有关如何检查压缩是否已在现有 SAP ASE 数据库中使用的详细信息，请查看[SAP 支持说明 1750510](https://launchpad.support.sap.com/#/notes/1750510)。 有关 SAP ASE 数据库压缩的更多详细信息，请查看[SAP 支持说明#2121797](https://launchpad.support.sap.com/#/notes/2121797)

## <a name="high-availability-of-sap-ase-on-azure"></a>Azure 上的 SAP ASE 高可用性 
HADR 用户指南详细介绍了 2 节点 SAP ASE"始终打开"解决方案的设置和配置。  此外，还支持第三个灾难恢复节点。 SAP ASE 支持许多高可用配置，包括共享磁盘和本机操作系统群集（浮动 IP）。 Azure 上唯一受支持的配置是使用没有浮动 IP 的故障管理器。  浮动 IP 地址方法在 Azure 上不起作用。  SAP 内核是一个"HA 感知"应用程序，了解主和辅助 SAP ASE 服务器。 SAP ASE 和 Azure 之间没有紧密的集成，不使用 Azure 内部负载均衡器。 因此，应遵循标准的 SAP ASE 文档，从[SAP ASE HADR 用户指南开始](https://help.sap.com/viewer/efe56ad3cad0467d837c8ff1ac6ba75c/16.0.3.7/en-US/a6645e28bc2b1014b54b8815a64b87ba.html) 

> [!NOTE]
> Azure 上唯一受支持的配置是使用没有浮动 IP 的故障管理器。  浮动 IP 地址方法在 Azure 上不起作用。 

### <a name="third-node-for-disaster-recovery"></a>灾难恢复的第三个节点
除了将 SAP ASE 始终打开用于本地高可用性之外，您可能希望将配置扩展到另一个 Azure 区域中的异步复制节点。 有关此类方案的文档可[在此处](https://techcommunity.microsoft.com/t5/running-sap-applications-on-the/installation-procedure-for-sybase-16-3-patch-level-3-always-on/ba-p/368199)找到。

## <a name="sap-ase-database-encryption--ssl"></a>SAP ASE 数据库加密& SSL 
SAP 软件预配管理器 （SWPM） 提供了在安装期间加密数据库的选项。  如果要使用加密，建议使用 SAP 完整数据库加密。  请参阅记录在：
...
- 将 VM 的磁盘 IOPS 和 VM 总聚合吞吐量配额与磁盘设计相匹配。  部署足够数量的磁盘
- 使用 Windows 存储空间或 Linux LVM2 聚合具有正确条带大小和文件系统的磁盘
- 为数据、日志、临时和备份目的创建足够数量的设备
- 考虑将超磁盘用于 X 大型系统 
- 在`saptune`Linux 操作系统上运行 SAP-ASE 
- 使用数据库加密保护数据库 – 在 Azure 密钥保管库中手动存储密钥 
- 在[Azure 检查表上](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-deployment-checklist)完成 SAP 
- 配置日志备份和完整备份 
- 测试 HA/DR、备份和恢复以及执行压力&体积测试 
- 确认自动数据库扩展工作 

## <a name="using-dbacockpit-to-monitor-database-instances"></a>使用 DBACockpit 监视数据库实例
对于使用 SAP ASE 作为数据库平台的 SAP 系统，可以将 DBACockpit 作为事务 DBACockpit 中的嵌入式浏览器窗口或作为 Webdynpro 来访问。 但是，监视和管理数据库的全部功能仅在 DBACockpit 的 Webdynpro 实现中可用。

与本地系统一样，需执行数个步骤，才能启用 DBACockpit 的 Webdynpro 实现所使用的所有 SAP NetWeaver 功能。 按照[SAP 支持说明#1245200，](https://launchpad.support.sap.com/#/notes/1245200)启用 Webdynpros 的使用并生成所需的网络。 按照上述说明的说明操作时，您还可以配置 Internet 通信管理器 （）`ICM`以及用于 http 和 https 连接的端口。 http 的默认设置如下所示：
...
> https：\//\<完全限定的主机名>：44300/sap/bc/webdynpro/sap/dba_cockpit
> 
> http：\//\<完全限定的主机名>：8000/sap/bc/webdynpro/sap/dba_cockpit
> 
> 
...
如果在仅云方案中部署了 VM，而本地和 Azure 之间没有跨界连接，则需要定义公共 IP 地址和 。 `domainlabel` VM 的公共 DNS 名称格式如下所示：

> `<custom domainlabel`>.`<azure region`>.cloudapp.azure.com
> 
> 

有关 DNS 名称的更多详细信息，请参阅 [此处][虚拟机-azurerm-对-azuresm]。
...
是有帮助的。 另一个有用的文档是[SAP 自适应服务器企业迁移和运行时最佳实践上的 SAP 应用程序](https://assets.cdn.sap.com/sapcom/docs/2016/06/26450353-767c-0010-82c7-eda71af511fa.pdf)。
