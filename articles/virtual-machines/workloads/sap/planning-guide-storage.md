...
| DBMS 数据量非 HANA | 不支持 | 受限（非生产） | 建议 | 建议 | 不支持 |
| DBMS 日志量非 HANA M/Mv2 VM 系列 | 不支持 | 受限（非生产） | 建议<sup>1</sup> | 建议 | 不支持 |
| DBMS 日志卷非 HANA 非 Mv2/虚拟机系列 | 不支持 | 受限（非生产） | 适用于最多中等工作负荷 | 建议 | 不支持 |


<sup>1</sup>使用[Azure 写入加速器](../../windows/how-to-enable-write-accelerator.md)适用于使用和的 Mv2 VM 系列的日志/重做日志卷<sup>2</sup>使用时，需要/hana/data 和/hana/log 和 
...
<sup>1</sup>使用[Azure 写入加速器](../../windows/how-to-enable-write-accelerator.md)FOR M/Mv2 VM 系列的日志/重做日志卷的使用情况
...
| --- | --- | --- | 
| OS 基本 VHD | 适合 | 所有系统 |
| 数据磁盘 | 适合 | 所有系统-[专门用于 SAP HANA](../../windows/how-to-enable-write-accelerator.md) |
...
| 每个磁盘的最大 IOPS | 20000[依赖磁盘大小](https://azure.microsoft.com/pricing/details/managed-disks/) | 还要考虑[VM 限制](../../linux/sizes.md) |
| 吞吐量 SLA | YES | - |
| 吞吐量线性到容量 | 方括号中的半线性 | [托管磁盘定价](https://azure.microsoft.com/pricing/details/managed-disks/) |
| HANA 认证 | YES | [专门为 SAP HANA](../../windows/how-to-enable-write-accelerator.md) |
| 可能的磁盘快照 | YES | - |
| 可能的 Azure 备份 VM 快照 | YES | [写入加速器](../../windows/how-to-enable-write-accelerator.md)缓存磁盘除外  |
| 成本 | MEDIUM | - |

Azure 高级存储不满足 SAP HANA 存储延迟 Kpi，其中包含 Azure 高级存储提供的通用缓存类型。 为了满足 SAP HANA 日志写入的存储延迟 Kpi，需要使用 Azure 写入加速器缓存，如[启用写入加速器](../../windows/how-to-enable-write-accelerator.md)一文中所述。 Azure 写入加速器受益于所有其他 DBMS 系统的事务日志写入和重做日志写入。 因此，建议在所有 SAP DBMS 部署中使用它。 对于 SAP HANA，Azure 写入加速器与 Azure 高级存储结合使用是必需的。
...
| --- | --- | --- | --- |
| 标准 HDD | [Azure 中 Linux Vm 的大小](../../linux/sizes.md) | [Azure 中 Windows Vm 的大小](../../windows/sizes.md) | 可能很难触摸中型或大型 Vm 的存储限制 |
| 标准 SSD | [Azure 中 Linux Vm 的大小](../../linux/sizes.md) | [Azure 中 Windows Vm 的大小](../../windows/sizes.md) | 可能很难触摸中型或大型 Vm 的存储限制 |
| 高级存储 | [Azure 中 Linux Vm 的大小](../../linux/sizes.md) | [Azure 中 Windows Vm 的大小](../../windows/sizes.md) | 易于访问的 IOPS 或存储吞吐量 VM 限制与存储配置 |
| 超磁盘存储 | [Azure 中 Linux Vm 的大小](../../linux/sizes.md) | [Azure 中 Windows Vm 的大小](../../windows/sizes.md) | 易于访问的 IOPS 或存储吞吐量 VM 限制与存储配置 |
| Azure NetApp 文件 | [Azure 中 Linux Vm 的大小](../../linux/sizes.md) | [Azure 中 Windows Vm 的大小](../../windows/sizes.md) | 存储流量使用网络吞吐量带宽而不是存储带宽！ |
