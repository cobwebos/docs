...
标准 HDD、标准 SSD、Azure 高级存储和超磁盘的 Microsoft Azure 存储将在三个不同的存储节点上的三个副本中保留基本 VHD （包含 OS）和 VM 附加数据磁盘或 Vhd。 故障转移到另一个副本，并在发生存储节点故障时对新副本进行种子设定是透明的。 作为此冗余的结果，**无**需在多个 Azure 磁盘上使用任何类型的存储冗余层。 这称为本地冗余存储 (LRS)。 对于 Azure 中的这些存储类型，LRS 是默认值。 [Azure NetApp 文件](https://azure.microsoft.com/services/netapp/)提供足够的冗余来实现与其他本机 Azure 存储相同的 sla。

还有其他几种冗余方法，这一文章适用于 Azure 提供的一些不同存储类型的[Azure 存储复制](https://docs.microsoft.com/azure/storage/common/storage-redundancy?toc=%2fazure%2fstorage%2fqueues%2ftoc.json)。 

### <a name="azure-managed-disks"></a>Azure 托管磁盘

托管磁盘是 Azure 资源管理器中的一种资源类型，可用于替代 Azure 存储帐户中存储的 Vhd。 托管磁盘会自动与它们附加到的虚拟机的 [可用性集] [虚拟机-管理可用性] 一致，因此可以提高虚拟机和虚拟机上运行的服务的可用性。 有关详细信息，请阅读[概述文章](https://docs.microsoft.com/azure/storage/storage-managed-disks-overview)。
...
> [!NOTE]
> 由于此原因以及通过托管磁盘专门提供的其他几项改进，我们要求使用 azure 块存储的虚拟机的新部署（除 Azure NetApp 文件之外的所有 Azure 存储空间）需要将 Azure 托管磁盘用于基本 VHD/OS 磁盘、包含 SAP 数据库文件的数据磁盘。 独立于是否通过可用性集部署 Vm，无论是在可用性区域还是独立于集和区域。 用于存储备份的磁盘不一定需要为托管磁盘。
...
在接下来的几个部分中，我们讨论了适用于上述三种方案的不同 Azure 存储类型及其可用性。 有关如何使用不同的 Azure 存储类型的一般分类说明，请参见[Azure 中的可用磁盘类型](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types)一文。 针对 SAP 工作负荷使用不同 Azure 存储类型的建议并不 majorly。

有关 S/4HANA 的 SAP NetWeaver/应用程序层对 Azure 存储类型的支持限制，请阅读[sap 支持说明 2015553](https://launchpad.support.sap.com/#/notes/2015553) ，了解 SAP HANA 认证和受支持的 azure 存储类型[SAP HANA azure 虚拟机存储配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)。
...
| --- | --- | --- | --- | --- | --- |
| OS 磁盘 | 不适用 |  受限（非生产） | 建议 | 不可能 | 不可能 |
| 全局传输目录 | 不支持 | 不支持 | 建议 | 建议 | 建议 |
| /sapmnt | 不适用 | 受限（非生产） | 建议 | 建议 | 建议 |
...
| DBMS 数据量非 HANA | 不受支持 | 受限（非生产） | 建议 | 建议 | 不受支持 |
| DBMS 日志量非 HANA M/Mv2 VM 系列 | 不受支持 | 受限（非生产） | 建议<sup>1</sup> | 建议 | 不受支持 |
| DBMS 日志卷非 HANA 非 Mv2/虚拟机系列 | 不受支持 | 受限（非生产） | 适用于最多中等工作负荷 | 建议 | 不受支持 |


<sup>1</sup>使用[Azure 写入加速器](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator)适用于使用和的 Mv2 VM 系列的日志/重做日志卷<sup>2</sup>使用时，需要/hana/data 和/hana/log 和 
...
| 延迟写入 | high | 中到高  | 低（毫秒<sup>1</sup>） | sub 毫秒 | sub 毫秒 |
...
<sup>1</sup>使用[Azure 写入加速器](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator)FOR M/Mv2 VM 系列的日志/重做日志卷的使用情况
...
这种类型的存储面向 DBMS 工作负荷，需要低单位数毫秒延迟的存储流量，在 Azure 高级存储的情况下，针对 IOPS 和吞吐量开销的 Sla 并不是存储在此类磁盘中的实际数据量，而是此类磁盘的大小类别，与存储在磁盘中的数据量无关。 你还可以在高级存储上创建不直接映射到[高级 SSD](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types#premium-ssd)文章所示大小类别中的磁盘。 本文的结论如下：
...
| --- | --- | --- | 
| OS 基本 VHD | 适合 | 所有系统 |
| 数据磁盘 | 适合 | 所有系统-[专门用于 SAP HANA](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator) |
...
| 每个磁盘的最大 IOPS | 20000[依赖磁盘大小](https://azure.microsoft.com/pricing/details/managed-disks/) | 还要考虑[VM 限制](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) |
| 吞吐量 SLA | YES | - |
| 吞吐量线性到容量 | 方括号中的半线性 | [托管磁盘定价](https://azure.microsoft.com/pricing/details/managed-disks/) |
| HANA 认证 | YES | [专门为 SAP HANA](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator) |
| 可能的磁盘快照 | YES | - |
| 可能的 Azure 备份 VM 快照 | YES | [写入加速器](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator)缓存磁盘除外  |
| 成本 | MEDIUM | - |

Azure 高级存储不满足 SAP HANA 存储延迟 Kpi，其中包含 Azure 高级存储提供的通用缓存类型。 为了满足 SAP HANA 日志写入的存储延迟 Kpi，需要使用 Azure 写入加速器缓存，如[启用写入加速器](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator)一文中所述。 Azure 写入加速器受益于所有其他 DBMS 系统的事务日志写入和重做日志写入。 因此，建议在所有 SAP DBMS 部署中使用它。 对于 SAP HANA，Azure 写入加速器与 Azure 高级存储结合使用是必需的。
...
Azure 超级磁盘为 Azure IaaS VM 提供高吞吐量、高 IOPS 和一贯低延迟的磁盘存储。 超磁盘的其他好处包括能够动态更改磁盘的 IOPS 和吞吐量以及工作负载，而无需重新启动虚拟机（VM）。 超磁盘适用于数据密集型工作负荷，如 SAP DBMS 工作负荷。 超磁盘只能用作数据磁盘，不能用作存储操作系统的基本 VHD 磁盘。 建议将 Azure 高级存储用作基于 VHD 磁盘的使用。

创建超磁盘时，可以定义三个维度：

- 磁盘的容量。 范围是从 4 GiB 到 65536 GiB
- 磁盘的预配 IOPS。 不同的最大值适用于磁盘容量。 阅读有关更多详细[信息的详细](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types#ultra-disk)信息
- 预配的存储带宽。 不同的最大带宽适用于磁盘的容量。 阅读有关更多详细[信息的详细](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types#ultra-disk)信息
...
**摘要：** Azure ultra 磁盘是适用于各种 SAP 工作负荷的低延迟的存储。 到目前为止，超磁盘只能与已通过可用性区域（区域部署）部署的 Vm 组合使用。 超磁盘目前不支持存储快照。 对于所有其他存储，超磁盘不能用于基本 VHD 磁盘。 超磁盘非常适合于 i/o 工作负荷很大程度地波动的情况，并且你想要将部署的存储吞吐量或 IOPS 调整到存储工作负荷模式，而不是调整带宽和 IOPS 的最大使用量。


## <a name="azure-netapp-files-anf"></a>Azure NetApp 文件（和）
[Azure NetApp 文件](https://azure.microsoft.com/services/netapp/)是 Microsoft 与 NetApp 之间协作的结果，旨在提供高性能的 AZURE 本机 NFS 和 SMB 共享。 重点是提供高带宽和低延迟存储，以实现 DBMS 部署方案，并且随着时间的推移，还可以通过 Azure 同时启用 NetApp 存储的典型操作功能。 NFS/SMB 共享以三个不同的服务级别提供，可区分存储吞吐量和价格。 [Azure NetApp 文件的服务级别](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-service-levels)一文中介绍了服务级别。 对于不同类型的 SAP 工作负荷，强烈建议使用以下服务级别：
...
- 在[Azure 虚拟机存储配置 SAP HANA](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)一文中所述，使用/hana/data 和/hana/log 卷和/或 nfs/hana/shared 卷的 nfs v2.0 共享和卷 SAP HANA 部署
...
- 在 SAP/sapmnt 目录的和上使用 NFS 的空间中，你通常会使用 Azure NetApp 文件强制执行的最小容量为 100 GiB 到 150 GiB。 但客户体验表明，12.8 MiB/秒的相关吞吐量（使用超高服务级别）可能不够，而且可能对 SAP 系统的稳定性产生负面影响。 在这种情况下，客户可以通过增加/sapmnt 卷的数量来避免出现问题，因此，该卷提供了更多的吞吐量。
...
| SAP sapmnt | 适合 | 所有系统 SMB （仅适用于 Windows）或 NFS （仅限 Linux） |
| 备份存储 | 适合 | - |
| 共享/共享磁盘 | YES | SMB 3.0、NFS v3 和 NFS v2。0 |
| 复原 | LRS | 磁盘无可用的 GRS 或 ZRS |
| 延迟 | 非常低 | - |
| IOPS SLA | YES | - |
| 从线性到容量的 IOPS | 严格线性  | 依赖于[服务级别](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-service-levels) |
| 吞吐量 SLA | YES | - |
| 吞吐量线性到容量 | 方括号中的半线性 | 依赖于[服务级别](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-service-levels) |
| HANA 认证 | YES | - |
| 可能的磁盘快照 | YES | - |
| 可能的 Azure 备份 VM 快照 | 是 | - |
| 成本 | 高于高级存储 | - |
...
| --- | --- | --- | --- |
| 标准 HDD | [Azure 中 Linux Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Azure 中 Windows Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | 可能很难触摸中型或大型 Vm 的存储限制 |
| 标准 SSD | [Azure 中 Linux Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Azure 中 Windows Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | 可能很难触摸中型或大型 Vm 的存储限制 |
| 高级存储 | [Azure 中 Linux Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Azure 中 Windows Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | 易于访问的 IOPS 或存储吞吐量 VM 限制与存储配置 |
| 超磁盘存储 | [Azure 中 Linux Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Azure 中 Windows Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | 易于访问的 IOPS 或存储吞吐量 VM 限制与存储配置 |
| Azure NetApp 文件 | [Azure 中 Linux Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Azure 中 Windows Vm 的大小](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | 存储流量使用网络吞吐量带宽而不是存储带宽！ |
...
有关特定的条带大小建议，请参阅不同 DBMS 的文档，例如[SAP HANA Azure 虚拟机存储配置](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)。
