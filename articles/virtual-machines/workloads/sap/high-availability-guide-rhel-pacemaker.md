---
title: 在 Azure 中的 RHEL 上设置起搏器 |微软文档
...
* 通用 RHEL 文档
  ...
  * [支持 RHEL 高可用性群集 - sbd 和 fence_sbd](https://access.redhat.com/articles/2800691)
* 特定于 Azure 的 RHEL 文档：
  ...
  * [在 RHEL 7.6 上使用独立队列服务器 2 （ENSA2） 配置 SAP S/4HANA ASCS/ERS](https://access.redhat.com/articles/3974941)
...
> [!NOTE]
> 红帽不支持软件模拟的监视器。 红帽不支持云平台上的 SBD。 有关详细信息，请参阅[RHEL 高可用性群集的支持策略 - sbd 和 fence_sbd](https://access.redhat.com/articles/2800691)。
> 在 Azure 上，起搏器红帽企业 Linux 群集唯一受支持的屏蔽机制是 Azure 围栏代理。  
...
1. **[A]** 注册
   ...
   请注意，通过将池附加到 Azure 应用商店 PAYG RHEL 映像，将有效地为 RHEL 使用支付双次计费：一次用于 PAYG 映像，一次用于所附加池中的 RHEL 授权。 为了缓解这种情况，Azure 现在提供 BYOS RHEL 映像。 更多信息可[在此处](../redhat/byos.md)获取。
...
1. **[A]** 安装 RHEL HA 附加产品
   ...
   > [!IMPORTANT]
   > 我们建议以下版本的 Azure Fence 代理（或更高版本）让客户从更快的故障转移时间中获益，如果资源停止失败或群集节点无法相互通信：  
   > RHEL 7.6： 栅栏代理-4.2.1-11.el7_6.8  
   > RHEL 7.5： 栅栏代理-4.0.11-86.el7_5.8  
   > RHEL 7.4： 栅栏代理-4.0.11-66.el7_4.12  
   > 有关详细信息，请参阅作为[RHEL 高可用性群集成员运行的 Azure VM 需要很长时间才能进行围栏保护，或者在 VM 关闭之前设置围栏失败/超时](https://access.redhat.com/solutions/3408711)。

   检查 Azure 围栏代理的版本。 如有必要，将其更新为等于或晚于上述版本的版本。
   ...
   > [!IMPORTANT]
   > 如果需要更新 Azure Fence 代理，并且如果使用自定义角色，请确保更新自定义角色以包括操作**Power Off**。 有关详细信息，请参阅[为围栏代理创建自定义角色](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/high-availability-guide-rhel-pacemaker#1-create-a-custom-role-for-the-fence-agent)。  
...
默认情况下，服务主体无权访问 Azure 资源。 您需要授予服务主体权限才能启动和停止群集的所有虚拟机（关闭）。 如果尚未创建自定义角色，可以使用 [PowerShell](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-powershell) 或 [Azure CLI](https://docs.microsoft.com/azure/role-based-access-control/role-assignments-cli) 来创建它
