...
- 对于每个 Azure 部署区域：
    ...
    - 使用[ExpressRoute 全局覆盖](https://docs.microsoft.com/azure/expressroute/expressroute-global-reach)线启用从本地到 HANA 大型实例单元的直接路由或不同 Azure 区域中的 HANA 大型实例单元之间的路由时可选，您需要另预留一个 /29 IP 地址范围。 此特定范围可能不会与您之前定义的任何其他 IP 地址范围重叠。
