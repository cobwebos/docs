---
title: RHEL 上的 Azure VM 上 SAP HANA 的高可用性 |微软文档
...
1. 输入以下参数：
    ...
    * **系统可用性**：选择“HA”。****
    * **管理员用户名、管理员密码或 SSH 密钥**：创建了一个新用户，可用于登录到计算机。
    * **子网 ID**：如果要将 VM 部署到已定义子网的现有 VNet 中，应为其分配 VM，则命名该特定子网的 ID。 ID 通常类似于 **/订阅/\<订阅 ID>/资源\<组/资源组名称>/提供商/Microsoft.Network/虚拟网络\</虚拟网络名称>/子网\</子网名称>**。 如果要创建新的虚拟网络，请将其留空
...
1. **[2]** 在第二个节点上配置系统复制：
   ...
   注册第二个节点以启动系统复制。 将以下命令运行为<个 hanasid\>adm：
...
1. **[1]** 检查复制状态
   ...
   检查复制状态并等待所有数据库同步。如果状态保持未知状态，请检查防火墙设置。
...
> [!NOTE]
> 上述配置中的超时只是示例，可能需要适应特定的 HANA 设置。 例如，如果启动 SAP HANA 数据库需要更长的时间，则可能需要增加启动超时。  
...
有关如何模拟网络故障的说明，请参阅[红帽知识库文章 79523。](https://access.redhat.com/solutions/79523) 此示例使用 net_breaker 脚本阻止对网络的所有访问。
...
* 要了解如何在 Azure（大型实例）上建立 SAP HANA 的高可用性和灾难恢复规划，请参阅[Azure 上的 SAP HANA（大型实例）高可用性和灾难恢复](hana-overview-high-availability-disaster-recovery.md)
