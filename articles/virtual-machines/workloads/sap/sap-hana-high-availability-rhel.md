---
title: RHEL 上的 Azure Vm 上 SAP HANA 的高可用性 |Microsoft Docs
...
1. 输入以下参数：
    ...
    * **子网 ID**：如果想要将 vm 部署到现有的 VNet，而该 VNet 定义了 vm 应分配到的子网，请命名该特定子网的 ID。 ID 通常类似于 **/subscriptions/\<订阅 ID>/resourcegroups/\<资源组名称>/providers/microsoft.network/virtualnetworks/\<虚拟网络名称>/subnets/\<子网名称>**。 如果要创建新的虚拟网络，请将其留空
...
1. **[1]** 检查复制状态
   ...
   检查复制状态，并等待所有数据库都同步。如果状态为 "未知"，请检查防火墙设置。
...
* 若要了解如何建立高可用性并规划 Azure 上的 SAP HANA （大型实例）的灾难恢复，请参阅[azure 上的 SAP HANA （大型实例）高可用性和灾难恢复](hana-overview-high-availability-disaster-recovery.md)
