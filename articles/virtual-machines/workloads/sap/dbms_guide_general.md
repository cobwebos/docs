...
* **PaaS**：平台即服务。
* **SaaS**：软件即服务。
* **Sap 组件**：单个 SAP 应用程序，例如 ERP Central COMPONENT （ECC）、业务仓库（BW）、解决方案管理器或 ENTERPRISE PORTAL （EP）。 SAP 组件可以基于传统的 ABAP 或 Java 技术，也可以基于非 NetWeaver 的应用程序，例如业务对象。
* **SAP 环境**：以逻辑方式分组的一个或多个 sap 组件，用于执行开发、质量保证、培训、灾难恢复或生产等业务功能。
* **SAP 横向**：此术语指的是客户 IT 环境中的整个 SAP 资产。 SAP 布局包括所有生产和非生产的环境。
* **Sap 系统**：一个 DBMS 层和应用程序层的组合，例如 sap ERP 开发系统、Sap 业务仓库测试系统或 sap CRM 生产系统。 在 Azure 部署中，不支持在本地与 Azure 之间分割这两个层。 因此，SAP 系统要么部署在本地，要么部署在 Azure 中。 可以在 Azure 中或本地部署 SAP 布局的不同系统。 例如，你可以在 Azure 中部署 SAP CRM 开发和测试系统，但在本地部署 SAP CRM 生产系统。
* **跨界**：描述将 vm 部署到在本地数据中心与 azure 之间建立了站点到站点、多站点或 azure ExpressRoute 连接的 azure 订阅的方案。 在一般的 Azure 文档中，此类部署也称为跨界方案。 
...
Azure 上的 SAP 工作负荷中还提供了其他相关文章。 从[Azure 上的 SAP 工作负载开始：](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/get-started)开始，然后选择你感兴趣的区域。
...
| [2002167] |Red Hat Enterprise Linux 7、windows：安装和升级 |
...
>
> 仅支持使用 MDADM 和逻辑卷管理器（LVM）在 Linux 上构建软件 RAID。 有关详细信息，请参见:
>
