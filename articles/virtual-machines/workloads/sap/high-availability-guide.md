...
Azure 虚拟机是面向在最短时间内需要计算、存储和网络资源的组织的解决方案，没有冗长的采购周期。 Azure 虚拟机可用于部署经典应用程序，如基于 SAP NetWeaver 的 ABAP、Java 和 ABAP+Java 堆栈。 无需额外本地资源即可扩展可靠性和可用性。 Azure 虚拟机支持跨界连接，因此可将 Azure 虚拟机集成到组织的本地域、私有云和 SAP 系统布局中。
...
图 1：Azure 中 SAP ASCS/SCS 的 Windows Server 故障转移群集配置示例
...
图 2：Azure 中没有共享磁盘的 Windows Server 故障转移群集配置
...
图 3：Azure 中使用 SIOS DataKeeper 的 Windows Server 故障转移群集配置
...
## <a name="2e3fec50-241e-441b-8708-0b1864f66dfa"></a>Azure 服务架构 (IaaS) 中的 SAP NetWeaver 高可用性
...
由于 Azure 存储帐户是潜在的单一故障点，因此务必至少拥有两个 Azure 存储帐户，并且至少要将两个虚拟机分配到其中。 在理想的设置中，运行 SAP 对话实例的每个虚拟机的磁盘都应部署在不同的存储帐户中。
...
图 6：Azure 中使用 SIOS DataKeeper 的 SAP ASCS/SCS 配置的 Windows Server 故障转移群集
...
* [使用资源管理器在 Azure 虚拟机中手动配置 Always On 可用性组][virtual-machines-windows-portal-sql-alwayson-availability-groups-manual]
...
图 8：SAP 高可用性体系结构模板 1，ASCS/SCS 和 DBMS 的专用群集
...
图 9：SAP 高可用性体系结构模板 2：包含 ASCS/SCS 的专用群集和 DBMS 的专用群集
...
图 10：SAP 高可用性体系结构模板 3：包含用于不同 ASCS/SCS 实例的专用群集
...
图 11：设置 SAP 高可用性 Azure 资源管理器参数
...
> [!NOTE]
> 网卡和 Azure 内部负载均衡器的所有 IP 地址默认为 **动态** 。 将它们更改为“静态”IP 地址。 本文稍后介绍操作方法。
>
>
...
3. 将内部负载均衡器 **pr1-lb-dbms** 的 IP 地址设置为 DBMS 实例的虚拟主机名 IP 地址。
   ...
   图 14：为 SAP ASCS/SCS 实例的内部负载均衡器设置静态 IP 地址
...
图 15：Azure 内部负载均衡器的默认 ASCS/SCS 负载均衡规则

将负载均衡器 **pr1-lb-dbms** 的 IP 地址设置为 DBMS 实例的虚拟主机名 IP 地址。

### <a name="fe0bd8b5-2b43-45e3-8295-80bee5415716"></a>更改 Azure 内部负载均衡器的 ASCS/SCS 默认负载均衡规则
...
2. 针对属于 SAP ASCS 或 SCS 实例的所有负载均衡规则，请更改以下值：
   ...
   * 后端端口
   ...
   图 16：更改 Azure 内部负载均衡器的 ASCS/SCS 默认负载均衡规则
...
2. 使用故障转移群集管理器设置故障转移群集。 在故障转移群集管理器中选择“创建群集”，只添加第一个群集节点 A 的名称。暂时不要添加第二个节点；将在后面的步骤中添加第二个节点。

   ![图 18：添加第一个群集节点的服务器或虚拟机名称][sap-ha-guide-figure-3007]

   图 18：添加第一个群集节点的服务器或虚拟机名称
...
6. 将群集虚拟主机名联机。
   ...
   图 25：群集核心服务启动并运行，并且使用正确的 IP 地址
...
- 使用 Windows 中的“添加角色和功能向导”，如图 39 所示。
  ...
  图 39：使用“添加角色和功能向导”安装 .NET Framework 3.5

  ![图 40：使用添加角色和功能向导安装 .NET Framework 3.5 时的安装进度栏][sap-ha-guide-figure-3029]

  图 40：使用“添加角色和功能向导”安装 .NET Framework 3.5 时的安装进度条
...
2. 输入管理和配置工具应连接到的第一个节点的名称或 TCP/IP 地址，并在第二个步骤中，插入第二个节点的相关数据。
   ...
   图 47：插入管理和配置工具应连接到的第一个节点的名称或 TCP/IP 地址，然后在第二步中插入第二个节点的相关数据
...
2. 若要定义分配给虚拟主机名的 IP 地址，请选择“DNS 管理器” **“域”。**  > 
   ...
   图 57：SAP ASCS/SCS 群集配置的新虚拟名称和 TCP/IP 地址
...
2. 若要保留 Azure 内部负载均衡器的默认端口，请选择：
   ...
   * **ABAP+Java 系统**：**ASCS** 实例编号 **00** 和 **SCS** 实例编号 **01**
   ...
   若要对 ABAP ASCS 实例使用00以外的实例号，并为 Java SCS 实例使用01，首先需要更改 Azure 内部负载均衡器默认负载均衡规则，如[更改 azure 内部负载均衡器的 ASCS/SCS 默认负载均衡规则][sap-ha-guide-8.9]中所述。
...
图 61：故障转移群集管理器：SAP <SID> 群集组在群集节点 A 上运行
...
图 62：在 SIOS DataKeeper 中，将本地卷从群集节点 A 复制到群集节点 B
...
4. 使用 Azure PowerShell 重新启动群集节点 A（这会启动将 SAP <*SID*> 群集组从节点 A 故障转移到节点 B 的自动故障转移）。
   ...
   图 63：在故障转移群集管理器中，SAP <SID> 群集组在群集节点 B 上运行
   ...
   图 64：SIOS DataKeeper 将本地卷从群集节点 B 复制到群集节点 A
