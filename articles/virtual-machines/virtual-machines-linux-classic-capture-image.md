...
> [!IMPORTANT] 
> Azure 提供两个不同的部署模型用于创建和处理资源：[Resource Manager 和经典模型](../azure-resource-manager/resource-manager-deployment-model.md)。 本文介绍如何使用经典部署模型。 Microsoft 建议大多数新部署使用资源管理器模型。 了解如何[使用 Resource Manager 模型执行这些步骤](virtual-machines-linux-capture-image.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)。
...
4. 键入 **Exit** 关闭 SSH 客户端。
   ...
   > [!NOTE]
   > 剩余步骤假定已在客户端计算机上[安装 Azure CLI](../xplat-cli-install.md)。 以下所有步骤也可以在 [Azure 经典门户][Azure classic portal]中执行。
...
9. 新映像现在会出现在映像列表中，可以用于配置任何新的 VM。 你可以使用以下命令来查看它：
   ...
   在 [Azure 经典门户][Azure classic portal]中，它显示在“映像”列表中。
