...
区域                                        | 可用性？
-----------------------------------------------|-----------------------------------------------
[全球所有公开上市的 Azure 区域](https://azure.microsoft.com/en-us/regions/)     | 正式版 
[Azure Government](https://azure.microsoft.com/en-us/overview/clouds/government/)              | 预览版 
[Azure 中国：](https://www.azure.cn/)                                                           | 预览版
[Azure 德国](https://azure.microsoft.com/en-us/overview/clouds/germany/)                    | 预览版
...
已对实例元数据服务进行了版本控制。 版本控制是必需的，当前版本为 `2017-04-02`。

> [!NOTE] 
> 支持的计划事件的早期预览版发布 {最新} 为 api-version。 此格式不再受支持，并且将在未来被弃用。

我们添加更新的版本时，早期版本仍可供访问以保持兼容性（如果你的脚本依赖于特定的数据格式）。 但是请注意，当前预览版本 (2017-03-01) 在服务正式发布之后可能不再可用。
...
scheduledevents | 目前为公共预览版，请查看 [scheduledevents](virtual-machines-scheduled-events.md)

## <a name="example-scenarios-for-usage"></a>用法的示例方案  

### <a name="tracking-vm-running-on-azure"></a>跟踪 Azure 上正在运行的 VM

作为服务提供商，你可能需要跟踪运行你的软件的 VM 数目，或者你的代理需要跟踪 VM 的唯一性。 为了能够获取 VM 的唯一 ID，请使用实例元数据服务的 `vmId` 字段。
...
- 详细了解由实例元数据服务提供的公共预览版 [scheduledevents](virtual-machines-scheduled-events.md) API。
