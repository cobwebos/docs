...
<properties
	...
	description="开始使用 Azure CLI 创建和管理你的第一个 Azure 虚拟机缩放集"
...
本教程演示如何使用 Ubuntu Linux 的最新修补版本创建 Linux 虚拟机的虚拟机缩放集，并自动缩放集中的虚拟机。为此，将使用 Azure PowerShell 创建 Azure Resource Manager 模板并部署它。有关模板的详细信息，请参阅[创作 Azure 资源管理器模板](/documentation/articles/resource-group-authoring-templates)。

在本教程中生成的模板非常类似于可在模板库中找到的模板。若要了解详细信息，请参阅 [Deploy a simple VM Scale Set with Linux VMs and a Jumpbox（使用 Linux VM 和 Jumpbox 部署简单的 VM 缩放集）](https://azure.microsoft.com/documentation/templates/201-vmss-linux-jumpbox/)。
...
1. **登录到 Azure** - 在命令行界面（Bash、终端、命令提示符）中，通过键入 `azure config mode arm` 确保你处于资源管理器模式，然后通过键入 `azure login` 来[使用你的工作或学校 ID 登录](/documentation/articles/xplat-cli-connect#use-the-log-in-method)，并按照提示完成你的 Azure 帐户的交互式登录体验。
...
10.	添加虚拟机缩放集资源，并指定将在缩放集中的所有虚拟机上安装的诊断扩展。此资源的许多设置都与虚拟机资源相似。他们的主要区别是：
...
请查看 [Autoscale a VM Scale Set running a Ubuntu/Apache/PHP app](https://github.com/Azure/azure-quickstart-templates/tree/master/201-vmss-lapstack-autoscale)（自动缩放运行 Ubuntu/Apache/PHP 应用的 VM 缩放集）模板，该模板设置 LAMP 堆栈以执行虚拟机缩放集的自动缩放功能。
