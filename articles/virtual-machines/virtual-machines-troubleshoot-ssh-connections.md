<properties
	pageTitle="无法通过 SSH 连接到 Azure VM | Microsoft Azure"
...
或者，你也可以发起 Azure 支持事件。请转到 [Azure 支持站点](http://azure.microsoft.com/support/options/)并单击“获取支持”。有关使用 Azure 支持的信息，请阅读 [Microsoft Azure 支持常见问题](http://azure.microsoft.com/support/faq/)。
...
1. 从 [Azure 预览门户](https://portal.azure.com)**重置远程访问**。单击“浏览全部”>“虚拟机(经典)”> 你的 Windows 虚拟机 >“重置远程访问”。

	![重置远程访问](./media/virtual-machines-troubleshoot-ssh-connections/Portal-SSH-Reset-Windows.png)

2. **重新启动**虚拟机。在 [Azure 预览门户](https://portal.azure.com)中，单击“全部浏览”>“虚拟机(经典)”> 你的 Windows 虚拟机 >“重新启动”。在 [Azure 管理门户](https://manage.windowsazure.com)中，打开“虚拟机”>“实例”，然后单击“重新启动”。

3. [**调整**虚拟机的大小](https://msdn.microsoft.com/library/dn168976.aspx)。

4. 按照[如何为基于 Linux 的虚拟机重置密码或 SSH](virtual-machines-linux-use-vmaccess-reset-password-or-ssh.md) 中的说明，在虚拟机上执行以下操作：
...
1. 使用 Azure CLI 或 Azure PowerShell，在命令行上为 Linux VM **重置 SSH 连接**。确保已安装 [Microsoft Azure Linux 代理](virtual-machines-linux-agent-user-guide.md) 2.0.5 或更高版本。
	...
	**使用 Azure CLI**
	...
	a.如果尚未安装 Azure CLI，请使用 `azure login` 命令[安装 Azure CLI 并连接到 Azure 订阅](../xplat-cli-install.md)。
...
2. 从门户**重新启动** Linux VM。在 [Azure 预览门户](https://portal.azure.com)中，单击“全部浏览”>“虚拟机”> 你的 Windows 虚拟机 >“重新启动”。

	![重新启动 V2](./media/virtual-machines-troubleshoot-ssh-connections/Portal-SSH-Restart-V2-Windows.png)
