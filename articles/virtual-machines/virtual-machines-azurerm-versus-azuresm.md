<properties
   pageTitle="计算、网络 和存储提供程序 | Microsoft Azure"
   description="Azure 资源管理器中计算、网络和存储资源提供程序（CRP、NRP 和 SRP）的概念性概述"
...
# Azure 资源管理器中的 Azure 计算、网络和存储提供程序

Azure 资源管理器包含计算、网络和存储功能，从根本上简化了在 IaaS 上运行的复杂应用程序的部署和管理。很多应用程序需要虚拟网络、存储帐户、虚拟机和网络接口等资源的组合。Azure 资源管理器允许你构造 JSON 模板，从而将所有这些资源作为单个应用程序一起部署和管理。

## 将计算、网络和存储集成到 Azure 资源管理器下的优点

借助 Azure 资源管理器，你可以轻松利用预建的应用程序模板或构建一个应用程序模板，在 Azure 上部署和管理计算、网络和存储资源。在本部分内容中，我们将了解通过 Azure 资源管理器部署资源的优点。
...
|DNS 名称| 云服务会得到一个隐式的全局唯一 DNS 名称。例如：`mycoffeeshop.chinacloudapp.cn`。 | DNS 名称是可在一个公共 IP 地址资源中指定的可选参数。FQDN 将采用以下格式 - `<domainlabel>.<region>.cloudapp.azure.com`。 |
...
Azure 门户将继续保留同时部署虚拟机和虚拟机（预览）的选项。Azure 门户还将允许自定义模板部署。
...
你已经构建的所有自动化和脚本将继续适用于在 Azure 服务管理模式下创建的现有虚拟机和虚拟网络。然而，必须更新这些脚本以使用新的架构，通过新的 Azure 资源管理器模式来创建相同的资源。请参阅有关如何修改你的 [Azure CLI 脚本](/documentation/articles/xplat-cli-azure-manage-vm-asm-arm)的详细信息。
