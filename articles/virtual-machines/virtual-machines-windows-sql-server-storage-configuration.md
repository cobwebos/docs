...
- [使用 AKV 集成创建 VM](https://github.com\Azure\azure-quickstart-templates\tree\master\201-vm-sql-full-keyvault)

## 现有 VM
对于现有的 SQL Server VM，可以在 Azure 门户中修改某些存储设置。选择 VM，转到“设置”区域，然后选择“SQL Server 配置”。“SQL Server 配置”边栏选项卡显示了 VM 当前的存储用量。此图显示了 VM 上存在的所有驱动器。对于每个驱动器，存储空间显示在四个部分：
