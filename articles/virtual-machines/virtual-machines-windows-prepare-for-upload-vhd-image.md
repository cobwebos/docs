...
6. 浏览到并选择新 VHD 文件的保存路径。
7. 单击“完成”关闭屏幕。

### <a name="convert-disk-using-powershell"></a>使用 PowerShell 转换磁盘
可以使用 Windows PowerShell 中的 [Convert-VHD](http://technet.microsoft.com/library/hh848454.aspx) cmdlet 转换虚拟磁盘。 启动 PowerShell 时，选择“以管理员身份运行”。 以下示例演示如何从 VHDX 转换为 VHD，从动态扩展磁盘转换为固定大小磁盘：

```powershell
Convert-VHD –Path c:\test\MY-VM.vhdx –DestinationPath c:\test\MY-NEW-VM.vhd -VHDType Fixed
```
将 -Path 的值替换为要转换的虚拟硬盘的路径，并将 -DestinationPath 的值替换为转换后磁盘的新路径和名称。

### <a name="convert-from-vmware-vmdk-disk-format"></a>从 VMware VMDK 磁盘格式转换
如果有 [VMDK 文件格式](https://en.wikipedia.org/wiki/VMDK)的 Windows VM 映像，可以使用 [Microsoft 虚拟机转换器](https://www.microsoft.com/download/details.aspx?id=42497)将其转换为 VHD。 有关详细信息，请参阅博客 [How to Convert a VMware VMDK to Hyper-V VHD](http://blogs.msdn.com/b/timomta/archive/2015/06/11/how-to-convert-a-vmware-vmdk-to-hyper-v-vhd.aspx)（如何将 VMware VMDK 转换为 Hyper-V VHD）。
...
1. 删除路由表中的所有静态持久性路由：
   ...
   * 若要查看路由表，请从命令提示符窗口运行 `route print`。
...
## <a name="set-services-startup-to-windows-default-values"></a>将服务启动设置为 Windows 默认值
...
1. 在 PowerShell 中运行以下命令，允许 WinRM 通过三个防火墙配置文件（“域”、“专用”和“公共”）并启用 PowerShell 远程服务：
   ...
   ```
2. 在命令提示符窗口中运行以下命令，确保已设置以下来宾操作系统防火墙规则：
...
1. 在命令提示符窗口中，运行 `winmgmt /verifyrepository` 以确认 Windows Management Instrumentation (WMI) 存储库是否一致。 如果存储库已损坏，请参阅博客文章 [WMI：存储库是否损坏？](https://blogs.technet.microsoft.com/askperf/2014/08/08/wmi-repository-corruption-or-not)
2. 设置启动配置数据 (BCD) 设置：
...
安装最新的 Windows 更新。 如果无法安装最新更新，请确保安装以下更新：
...
## <a name="run-sysprep--a-idstep23a"></a>运行 Sysprep <a id="step23"></a>    
如果想要创建一个要部署到多个 VM 的映像，则在将 VHD 上载到 Azure 之前，需要[通过运行 Sysprep 来通用化该映像](virtual-machines-windows-generalize-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)。 无需运行 Sysprep 即可使用专用 VHD。 有关详细信息，请参阅以下文章：
   
   * [使用 Sysprep 通用化 Windows 虚拟机](virtual-machines-windows-generalize-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)
