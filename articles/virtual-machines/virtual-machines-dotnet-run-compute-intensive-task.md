...
1. 在你的开发计算机（不必是你创建的虚拟机）上，下载 [Azure SDK for .NET](http://azure.microsoft.com/develop/net/)。
...
除了使用 TSPSolver 创建或删除队列外，你还可以使用 [Azure 门户](https://manage.windowsazure.com)创建或删除队列。访问 Azure 门户的 Service Bus 部分即可访问用于创建或删除队列以及检索连接字符串、颁发者和访问密钥的用户界面。你也可以查看 Service Bus 队列的仪表板，从而使你可以查看传入消息和传出消息的指标。
