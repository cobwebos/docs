...
这些步骤采用填空方法来创建 Azure PowerShell 命令集。如果你不熟悉 Azure PowerShell 或只想知道为成功的配置指定什么值，则此方法很有用。高级 Azure PowerShell 用户可以使用命令并将变量（以“$”开头的行）替换为他们自己的值。
...
通过将下面的一组命令块复制到新的文本文件或 PowerShell ISE 中，然后填写变量值并删除 < and > 字符来生成命令集的其余部分。请参阅本文末尾的两个[示例](#examples)，了解最终结果。
...
使用以下命令指定初始 Linux 用户名和密码（必需）。选择一个强密码。若要检查其强度，请参阅[密码检查器：使用强密码](https://www.microsoft.com/security/pc-security/password-checker.aspx)。
...
云服务的短名称是在 Azure 门户的 Azure 云服务列表中或 Azure 预览门户的资源组列表中显示的名称。
