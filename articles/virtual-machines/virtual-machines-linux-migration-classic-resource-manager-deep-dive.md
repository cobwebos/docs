---
title: "有关平台支持的从经典部署模型到 Azure Resource Manager 部署模型的迁移的技术深入探讨 | Microsoft Docs"
...
## <a name="detailed-guidance-on-migration-at-a-resource-and-feature-level"></a>有关资源和功能级别迁移的详细指导
...
| 负载平衡的终结点集 |负载平衡器 |在经典部署模型中，平台已经为每个云服务分配一个隐式负载平衡器。 在迁移期间，我们将创建新的负载平衡器资源，负载平衡终结点集将成为负载平衡器规则。 |
| 入站 NAT 规则 |入站 NAT 规则 |在迁移期间，VM 上定义的输入终结点将转换成负载平衡器下的入站网络访问转换规则。 |
| VIP 地址 |具有 DNS 名称的公共 IP 地址 |虚拟 IP 地址会变成公共 IP 地址并与负载平衡器关联。 |
...
| 保留 IP |具有静态分配方法的公共 IP 地址 |与负载平衡器关联的保留 IP 将在迁移云服务或虚拟机的过程中一起迁移。 目前不支持迁移未关联的保留 IP。 |
...
## <a name="illustration-of-a-simple-migration-walkthrough"></a>简单迁移的演练图解
