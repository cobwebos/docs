---
title: 使用 Azure 站点恢复将 Windows VM 迁移到 Azure 高级存储
...
* 移动服务是在想要复制的每个标准 VM 上部署的组件****。 它可以捕获标准 VM 上的数据写入，并将其转发到进程服务器。 阅读有关[复制的计算机先决条件](../../site-recovery/vmware-walkthrough-overview.md)。
...
2. 选择 **"创建资源** > **管理** > **备份和站点恢复 （OMS）"。** 或者，您可以选择**浏览** > **恢复服务保管库** > **添加**。
   >[!NOTE]
...
2. 转到**恢复服务保管库** > **设置** > **网站恢复** > **步骤 1：准备基础结构** > **保护目标**。
...
选择 **"准备基础结构** > **目标**"，并指定故障转移后要用于 VM 的部署模型。 可以根据方案选择“经典”或“Resource Manager”。********
...
设计 Azure 存储环境时，我们建议针对可用性集中的每个 VM 使用不同的存储帐户。 我们建议您遵循存储层中的最佳做法，[为每个可用性集使用多个存储帐户](../linux/manage-availability.md)。 将 VM 磁盘分配到多个存储帐户有助于改善存储可用性，以及在整个 Azure 存储基础结构中分配 I/O。
...
要检查复制是否完成，请选择"站点恢复"实例，然后选择 **"设置** > **复制项目**"。 此时会显示复制过程的状态和完成百分比。 
...
您可以在 **"设置** > **作业** > *YOUR_FAILOVER_PLAN_NAME"* 中查看测试故障转移的状态。 在窗格中，可以看到具体的步骤，以及成功/失败结果。 如果测试故障转移在执行任一步骤时失败，请选择该步骤查看错误消息。 
...
* [创建 Windows 服务器 VHD 并将其上载到 Azure](upload-generalized-managed.md)
