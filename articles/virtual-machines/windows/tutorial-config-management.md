...
ms.locfileid: "78372930"
---
# <a name="tutorial-monitor-changes-and-update-a-windows-virtual-machine-in-azure"></a>教程：在 Azure 中监视更改和更新 Windows 虚拟机
...
接下来，使用 [New-AzVM](https://docs.microsoft.com/powershell/module/az.compute/new-azvm) 创建 VM。 以下示例在 `myVM` 位置创建名为 `East US` 的 VM。 如果资源组 `myResourceGroupMonitor` 和支持的网络资源尚不存在，则会创建它们：
...
* **未尝试：未**安装更新。 基于定义的维护时段持续时间，没有足够的可用时间。
* **成功**：更新成功。
* **失败**：更新失败。
