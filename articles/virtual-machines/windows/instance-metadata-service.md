...
[公共美国中西部](https://azure.microsoft.com/regions/)     | 正式版   | 2017-04-02、2017-08-01、2017-12-01、2018-02-01、2018-04-02、2018-10-01
当有服务更新且/或有可用的新支持版本时，此表将更新。

> [!NOTE]
> 2018-10-01 目前正在推出，不久将在其他国家/地区推出。 
...
实例元数据在 http 终结点上响应 169.254.169.254。 实例元数据服务提供的部分方案是保证响应的数据来自 Azure。 我们对此信息的一部分进行签名，以便市场映像可以确保其映像在 Azure 上运行。
...
Nonce 是一个可选的 10 位字符串。 Nonce 可用于跟踪请求，如果未提供，则在响应编码字符串中返回当前 UTC 时间戳。
...
> 签名 blob 是 [pkcs7](https://aka.ms/pkcs7) 签名的文档版本。 它包含用于签名的证书以及 VM 细节，如 vmId、nonce、文档创建和到期的时间戳以及关于映像的计划信息。 计划信息只针对 Azure 市场位置映像填充。 可以从响应中提取证书，并用于验证响应是否有效，是否来自 Azure。

#### <a name="retrieving-attested-metadata-in-windows-virtual-machine"></a>检索 Windows 虚拟机中已验证的元数据

 “请求”可通过 Powershell 实用工具 `curl` 在 Windows 中检索实例元数据：
...
获得上面的签名后，可以验证签名是否来自 Microsoft。 还可以验证中间证书和证书链。
