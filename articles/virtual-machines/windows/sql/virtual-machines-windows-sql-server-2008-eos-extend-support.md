...
description: 通过将 SQL Server 实例迁移到 Azure 或购买扩展支持以将实例保留在本地，扩展对 SQL Server 2008 和 SQL Server 2008 R2 的支持。
...
SQL Server 2008 和 SQL Server 2008 R2 都达到了[其支持 （EOS） 生命周期的结束](https://www.microsoft.com/sql-server/sql-server-2008)。 由于许多客户仍在使用这两个版本，我们提供了多个选项让这些客户继续获得支持。 你可以将本地 SQL Server 实例迁移到 Azure 虚拟机 (VM)、迁移到 Azure SQL 数据库，或者将其保留在本地，但这需要购买扩展的安全更新。
...
**Windows Server 2008 R2 映像上有**一个即用即付 SQL Server 2008 R2 映像，可在 Azure 应用商店中使用。
...
> [!NOTE]
> 尽管 SQL Server**创建****和管理**边栏选项卡将使用 Azure 门户中的 SQL Server 2008 R2 映像，_但不支持_以下功能：自动备份、Azure 密钥保管库集成、R 服务和存储配置。

## <a name="licensing"></a>授权
即用即付 SQL Server 2008 R2 部署可以转换为[Azure 混合权益](https://azure.microsoft.com/pricing/hybrid-benefit/)。

要将基于软件保障 （SA） 的许可证转换为即用即付，客户应向 SQL VM[资源提供程序](virtual-machines-windows-sql-register-with-resource-provider.md)注册 。 注册后，SQL 许可证类型将在 Azure 混合权益和即用即付之间互换。

Azure VM 上的自安装 SQL Server 2008 或 SQL Server 2008 R2 实例可以注册到 SQL VM 资源提供程序，并将其许可证类型转换为即用即付。
...
SQL Server VM 的扩展安全更新在 SQL Server VM 注册到 SQL VM[资源提供程序](virtual-machines-windows-sql-register-with-resource-provider.md)后，通过 Microsoft 更新通道传递。 可以手动或自动下载修补程序。
...
* [支持结束](/sql/sql-server/end-of-support/sql-server-end-of-life-overview) & [扩展安全更新](/sql/sql-server/end-of-support/sql-server-extended-security-updates)