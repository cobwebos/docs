...
以下示例使用 PowerShell 在现有的 SQL Server VM 上配置自动修补。 **新建 AzVMSqlServerAutoPatchingConfig**命令将配置为自动更新新的维护时段。
...
若要禁用自动修补，请运行同一个脚本而无需 **-启用**参数**新建 AzVMSqlServerAutoPatchingConfig**。 缺少 **-Enable** 参数将向该命令发出指示以禁用此功能。
