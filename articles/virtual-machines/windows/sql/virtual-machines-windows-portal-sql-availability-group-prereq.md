...
本教程演示如何完成用于[在 Azure 虚拟机 (VM) 中创建 SQL Server AlwaysOn 可用性组](virtual-machines-windows-portal-sql-availability-group-tutorial.md)的必备组件配置。 完成先决条件后，将在单个资源组中拥有一台域控制器、两台 SQL Server VM 和一台见证服务器。
