...
ms.locfileid: "80060165"
---
# <a name="prerequisites-for-creating-always-on-availability-groups-on-sql-server-on-azure-virtual-machines"></a>在 Azure 虚拟机上的 SQL Server 上创建始终处于打开的可用性组的先决条件
...
![可用性组 (availability group)](./media/virtual-machines-windows-portal-sql-availability-group-tutorial/00-EndstateSampleNoELB.png)
...
需要一个 Azure 帐户。 您可以[打开一个免费的 Azure 帐户](https://signup.azure.com/signup?offer=ms-azr-0044p&appId=102&ref=azureplat-generic&redirectURL=https:%2F%2Fazure.microsoft.com%2Fget-started%2Fwelcome-to-azure%2F&correlationId=24f9d452-1909-40d7-b609-2245aa7351a6&l=en-US)或激活 Visual Studio[订阅者权益](https://docs.microsoft.com/visualstudio/subscriptions/subscriber-benefits)。
...
新的虚拟网络有一个子网，名为 **"管理员**"。域控制器使用此子网。 SQL Server VM 使用名为 SQL**** 的另一个子网。 配置此子网：
...
7. 单击 **“下一步”**，直到你到达 **“确认”** 部分。 选中“必要时自动重启目标服务器”**** 复选框。
8. 单击 **“安装”**。
...
14. 单击 **“下一步”** 以浏览向导中的其他页。 在“必备项检查”页上，确认看到以下消息：“所有先决条件检查都成功通过”。******** 查看任何适用的警告消息，但可继续安装。
...
8. 单击“确定”****，并单击“关闭”**** 提交更改。 你现在能够将该虚拟机加入到 **corp.contoso.com** 中。
...
| 虚拟机配置**设置** |**存储**：使用托管磁盘。<br/>**虚拟网络** = autoHAVNET<br/>**子网** = sqlsubnet(10.1.1.0/24)<br/>自动生成**公共 IP 地址**。<br/>**网络安全组** = 无<br/>**监视诊断** = 已启用<br/>**诊断存储帐户** = 使用自动生成的存储帐户<br/>**可用性集**= sql 可用性集<br/> |**存储**：使用托管磁盘。<br/>**虚拟网络** = autoHAVNET<br/>**子网** = sqlsubnet(10.1.1.0/24)<br/>自动生成**公共 IP 地址**。<br/>**网络安全组** = 无<br/>**监视诊断** = 已启用<br/>**诊断存储帐户** = 使用自动生成的存储帐户<br/>**可用性集**= sql 可用性集<br/> |**存储**：使用托管磁盘。<br/>**虚拟网络** = autoHAVNET<br/>**子网** = sqlsubnet(10.1.1.0/24)<br/>自动生成**公共 IP 地址**。<br/>**网络安全组** = 无<br/>**监视诊断** = 已启用<br/>**诊断存储帐户** = 使用自动生成的存储帐户<br/>**可用性集**= sql 可用性集<br/> |
...
1. 通过使用*\<计算机名称\>@DomainAdmin*帐户，通过远程桌面协议 （RDP） 连接到服务器。
...
在另一个 SQL Server VM 上重复上述步骤。
  ...
  >[!NOTE]
  > 此步骤，以及实际将 SQL Server VM 连接到故障转移群集，现在可以使用 Azure SQL [VM CLI](virtual-machines-windows-sql-availability-group-cli.md)和[Azure 快速启动模板](virtual-machines-windows-sql-availability-group-quickstart-template.md)进行自动化。
...
3. 对于“规则类型”，请选择“端口”。********
4. 对于端口，指定**TCP**并键入相应的端口号。 请参阅以下示例：
