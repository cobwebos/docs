---
title: "SQL Server 可用性组 - Azure 虚拟机 - 必备组件 | Microsoft 文档"
...
# <a name="complete-prerequisites-for-creating-always-on-availability-groups-in-azure-virtual-machines"></a>完成先备组件配置以便在 Azure 虚拟机中创建 Alwayson 可用性组

本教程说明如何完成必备组件配置，以便[在 Azure 虚拟机上中创建 SQL Server Always On 可用性组](virtual-machines-windows-portal-sql-availability-group-tutorial.md)。 完成必备组件配置后，将在单个资源组中获得一个域控制器、两个 SQL 服务器和一个见证服务器。
...
新虚拟网络包含一个名为 **Admin** 的子网。 域控制器使用此子网。 SQL Server 使用名为 **SQL** 的另一个子网。 配置此子网：
...
5. 创建第二个子网。 单击“+ 子网”。 
6. 在“添加子网”边栏选项卡中，通过在“名称”下键入 **sqlsubnet** 来配置子网。 Azure 将自动指定一个有效的**地址范围**。 请确认此地址范围中至少有 10 个地址。 在生产环境中，可能需要更多地址。 
7. 单击 **“确定”**。
...
在创建虚拟机之前，需要创建可用性集。 可用性集可减少计划内或计划外维护事件的停机时间。 Azure 可用性集是 Azure 置于物理容错域和更新域上的逻辑资源组。 容错域确保可用性集的成员具有不同的电源和网络资源。 更新域确保可用性集的成员不会同时停机进行维护。 [管理虚拟机的可用性](../manage-availability.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)
...
1. 单击“确定”，然后单击“关闭”提交更改。 现在可将该 VM 加入 **corp.contoso.com**。
...
## <a name="add-failover-cluster-features-to-both-sql-servers"></a>将故障转移群集功能添加到两个 SQL 服务器

若要添加故障转移群集功能，请在两个 SQL 服务器上执行以下步骤：
...
## <a name="a-nameendpoint-firewall-configure-the-firewall-on-each-sql-server"></a><a name="endpoint-firewall">在每个 SQL Server 上配置防火墙
