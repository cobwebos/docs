...
答：客户可以使用其现有 Windows Server 的值并 SQL Server 具有软件保障的许可证，也可以通过使用 Azure 混合权益来确定订阅许可证的资格。 Windows Server Datacenter 和 SQL Server Enterprise Edition 客户获得无限制的虚拟化（在受底层服务器的物理容量约束的主机上尽可能部署多个 Windows Server 虚拟机），并使用 Azure 混合权益。  Azure 专用主机中的所有 Windows Server 和 SQL Server 工作负荷也有资格用于 Windows Server 的扩展安全更新和 SQL Server 2008/R2，无额外收费。 
