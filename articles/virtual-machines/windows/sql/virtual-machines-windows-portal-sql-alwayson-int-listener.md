---
title: 配置可用性组侦听器&负载均衡器（Azure 门户）
...
ms.locfileid: "79096321"
---
# <a name="configure-a-load-balancer-for-an-availability-group-on-azure-sql-server-vms"></a>为 Azure SQL Server VM 上的可用性组配置负载均衡器
...
Azure 调用后端地址池*后端池*。 在本例中，后端池是可用性组中两个 SQL Server 实例的地址。 

1. 在资源组中，单击已创建的负载均衡器。 

2. 在“设置”中，单击“后端池”。********

3. 在**后端池**上，单击"**添加"** 以创建后端地址池。 

4. 在 **"添加后端池****"下，** 键入后端池的名称。
...
3. 使用“添加负载均衡规则”边栏选项卡配置负载均衡规则。**** 使用以下设置： 
   ...
   | **协议** |**Tcp** |
   | **港口** |*1433* |
   | **后端端口** |*1433.* 此值被忽略，因为此规则使用**浮动 IP（直接服务器返回）。** |
...
2. 使用**sqlcmd**实用程序测试连接。 例如，以下脚本通过具有 Windows 身份验证的侦听器建立与主副本的**sqlcmd**连接：
...
7. 使用以下设置添加运行状况探测：
   ...
   |**名称** |用于标识探测的名称。
   |**协议** |TCP
   |**港口** |一个未使用的 TCP 端口，必须在所有虚拟机上可用。 此端口不能用于任何其他用途。 两个侦听器不能使用相同的探测端口。 
   |**区间** |探测尝试之间的时间长短。 使用默认值 (5)。
   |**不正常的阈值** |在将虚拟机视为运行状况不正常之前连续阈值应失败的次数。
...
10. 使用以下设置配置新的负载均衡规则：
    ...
    |**协议** |TCP
    |**港口** |使用 SQL Server 实例正在使用的端口。 在不更改的情况下，默认实例使用端口 1433。 
    |**后端端口** |使用与“端口”相同的值****。
...
1. 通过以下设置创建负载均衡规则：
   ...
   |**协议** |TCP
   |**港口** |5022 - 用于 [分布式可用性组终结点侦听器](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/configure-distributed-availability-groups) 的端口。</br> 可为任何可用端口。  
...
- [在不同区域的 Azure 虚拟机上配置 SQL Server 始终打开可用性组](virtual-machines-windows-portal-sql-availability-group-dr.md)
