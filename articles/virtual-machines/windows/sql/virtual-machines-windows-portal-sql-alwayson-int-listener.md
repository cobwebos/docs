---
title: '& 负载均衡器（Azure 门户）配置可用性组侦听器'
...
---
# <a name="configure-a-load-balancer-for-an-availability-group-on-azure-sql-server-vms"></a>在 Azure 上为可用性组配置负载均衡器 SQL Server Vm
...
Azure 会调用后端地址池*后端池*。 在本例中，后端池是可用性组中两个 SQL Server 实例的地址。 

1. 在资源组中，单击已创建的负载均衡器。 

2. 在“设置”中，单击“后端池”。********

3. 在**后端池**上，单击 "**添加**" 创建后端地址池。 

4. 在 "**添加后端池**" 下的 "**名称**" 下，键入后端池的名称。
...
3. 使用“添加负载均衡规则”边栏选项卡配置负载均衡规则。**** 使用以下设置： 
   ...
   | 协议  |**TCP** |
   | 端口  |*1433* |
   | **后端端口** |*1433*。此值将被忽略，因为此规则使用**浮动 IP （直接服务器返回）**。 |
...
2. 中转到**AlwaysOn 高可用性** > **可用性** > 组**可用性组侦听器**。  
...
2. 使用**sqlcmd**实用工具测试连接。 例如，以下脚本通过侦听器与 Windows 身份验证建立了到主副本的**sqlcmd**连接：
