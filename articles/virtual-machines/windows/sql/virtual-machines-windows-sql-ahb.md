...
ms.locfileid: "52576939"
---
# <a name="how-to-change-the-licensing-model-for-a-sql-virtual-machine-in-azure"></a>如何在 Azure 中更改 SQL 虚拟机的许可模型
...
## <a name="register-legacy-sql-vm-with-new-resource-provider"></a>将旧 SQL VM 注册到新的资源提供程序
在许可模型之间进行切换是新的 SQL VM 资源提供程序 (Microsoft.SqlVirtualMachine) 提供的一项功能。 目前，若要切换许可模型，首先需将新提供程序注册到订阅，然后将旧 VM 注册到新的 SQL VM 资源提供程序。 
...
以下代码先将新的 SQL 资源提供程序注册到订阅，然后将旧 SQL VM 注册到新的资源提供程序。 
...
1. 导航到“订阅”，选择感兴趣的订阅。  
  ...
  ![选择订阅](media/virtual-machines-windows-sql-ahb/open-subscriptions.png)
...
可以通过 PowerShell 更改许可模型。  请确保在切换许可模型之前，SQL VM 已注册到新的 SQL 资源提供程序。 
...
```
  ...
  >[!NOTE]
  > 若要在许可证之间进行切换，必须使用新的 SQL VM 资源提供程序。 如果尝试在将 SQL VM 注册到新提供程序之前运行这些命令，则可能会遇到以下错误：`Get-AzureRmResource : The Resource 'Microsoft.SqlVirtualMachine/SqlVirtualMachines/AHBTest' under resource group 'AHBTest' was not found. The property 'sqlServerLicenseType' cannot be found on this object. Verify that the property exists and can be set. `如果看到该错误，请[将 SQL VM 注册到新的资源提供程序](#register-legacy-SQL-vm-with-new-resource-provider)。 
...
```
  ...
  >[!NOTE]
  >若要在许可证之间进行切换，必须使用新的 SQL VM 资源提供程序。 如果尝试在将 SQL VM 注册到新提供程序之前运行这些命令，则可能会遇到以下错误：`The Resource 'Microsoft.SqlVirtualMachine/SqlVirtualMachines/AHBTest' under resource group 'AHBTest' was not found. `如果看到该错误，请[将 SQL VM 注册到新的资源提供程序](#register-legacy-SQL-vm-with-new-resource-provider)。 
