...
根据 Microsoft 产品条款： "客户必须指出他们正在使用 Azure SQL 数据库（托管实例、弹性池和单一数据库）、Azure 数据工厂、SQL Server Integration Services 或 SQL Server 在 Azure 上配置工作负荷时 Azure 混合权益的虚拟机。"
...
### <a name="the-virtual-machine-vmname-has-more-than-one-nic-associated"></a>虚拟机 "\<vmname\>" 有多个关联的 NIC
