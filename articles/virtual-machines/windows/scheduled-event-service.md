...
连接到**myCollectorVM**并将 .zip 文件复制到虚拟机，并提取所有文件。 在 VM 上，打开 PowerShell 提示符。 将提示移入包含 `SchService.ps1` 的文件夹，例如： `PS C:\Users\azureuser\AzureScheduledEventsService-master\AzureScheduledEventsService-master\Powershell>`，并设置服务。
...
在任何时候，都可以使用开关 `–stop` 和 `–remove` 来停止/删除计划事件服务。
...
1. 选择 "**新建警报规则**"。 
1. 在 "**创建规则**" 页中，将 `collectorworkspace` 保留为**资源**。
1. 在 "**条件**" 下，*每当 "客户日志搜索" <login undefined> 时*，选择条目。 "**配置信号逻辑**" 页将打开。
