...
本文重点介绍如何创建使用**外部负载平衡**的侦听器。 如果要创建专用于虚拟网络的侦听器，请参阅本文的另一个版本，其中提供了设置[使用 ILB 的侦听器](virtual-machines-windows-classic-ps-sql-int-listener.md)的步骤
...
1. 对于外部负载平衡，你必须获取包含副本的云服务的公共虚拟 IP 地址。 登录到 Azure 经典门户。 导航到包含你的可用性组 VM 的云服务。 打开“仪表板”视图。
...
5. 在每个 VM 上重复此过程。 此脚本将使用云服务的 IP 地址来配置 IP 地址资源，同时设置探测端口等其他参数。 在 IP 地址资源联机后，它可以响应我们在本教程前面部分创建的负载平衡终结点在探测端口上的轮询。
...
若要从虚拟网络外部访问侦听器，必须使用外部/公共负载平衡（如本主题中所述）而不是 ILB，因为 ILB 只能在同一 VNet 中进行访问。 在连接字符串中指定云服务名称。 例如，如果云服务名为 *mycloudservice*，则 sqlcmd 语句将如下所示：
