---
title: "使用 PowerShell 在 Azure VM 中配置 Always On 可用性组 | Microsoft Docs"
...
在开始之前，考虑到你现在可以在 Azure Resource Manager 模型中完成此任务。 我们建议使用 Azure Resource Manager 模型来进行新的部署。 请参阅 [Azure 虚拟机上的 SQL Server Always On 可用性组](../sql/virtual-machines-windows-portal-sql-availability-group-overview.md)。

> [!IMPORTANT]
> 我们建议在大多数新部署中使用 Resource Manager 模型。 Azure 具有用于创建和处理资源的两个不同的部署模型： [资源管理器和经典](../../../azure-resource-manager/resource-manager-deployment-model.md)。 本文介绍使用经典部署模型。
...
此方案之所以是一个不错的选择是因为其在 Azure 上的简单性，而非其成本效益或其他因素。 例如，你可最大程度减少两副本可用性组的虚拟机数目，以便通过将域控制器作为两节点故障转移群集中的仲裁文件共享见证服务器来节省 Azure 中的计算时间。 通过此方法，上述配置中的 VM 数目可以减少一个。

本教程将介绍设置上述解决方案所需的步骤，但不详细阐述每一步的细节。 因此，本教程没有提供 GUI 配置步骤，而是使用 PowerShell 脚本带你迅速完成每个步骤。 本教程的假设条件如下：

* 你已经拥有一个带有虚拟机订阅的 Azure 帐户。
* 你已安装了 [Azure PowerShell cmdlet](/powershell/azureps-cmdlets-docs)。
...
4. 通过导入配置文件创建虚拟网络。
    ...
    配置文件包含以下 XML 文档。 简而言之，它指定了名为 **ContosoAG** 的地缘组中的一个名为 **ContosoNET** 的虚拟网络。 它具有地址空间 **10.10.0.0/16** 和两个子网 **10.10.1.0/24** 和 **10.10.2.0/24**，分别是前端子网和后端子网。 前端子网是你可以在其中放置客户端应用程序（例如 Microsoft SharePoint）的位置。 后端子网是你将在其中放置 SQL Server VM 的位置。 如果之前更改过 **$affinityGroupName** 和 **$virtualNetworkName** 变量，则还必须更改以下相应名称。
...
5. 创建与你创建的地缘组相关联的存储帐户，并在订阅中将其设置为当前存储帐户。
...
现在已成功预配了域控制器服务器。 接下来，你将在域控制器服务器上配置 Active Directory 域。 在本地计算机上使 PowerShell 窗口保持打开。 稍后将再次使用它来创建两个 SQL Server VM。
...
4. 通过启动远程桌面文件再次连接到域控制器服务器。 此时，以 **CORP\Administrator** 身份登录。
...
7. 接下来，运行以下命令为 **CORP\Install** 提供在域中创建计算机对象的权限。
    ...
    现在你已完成了 Active Directory 和用户对象的配置，接下来，你将创建两个 SQL Server VM 并将其加入到此域中。
...
4. 请等待每个 VM 预配完毕，并等待每个 VM 将其远程桌面文件下载到你的工作目录中。 `for` 循环会循环访问三个新 VM，并为每个 VM 执行上级大括号内的命令。
...
在本部分中，你需要修改将在故障转移群集和 SQL Server 安装中使用的三个服务器。 具体而言：
...
现在，你已准备就绪，可以执行启动操作了。 从 **ContosoQuorum** 开始，执行以下步骤：
...
最后，你已准备就绪，可以配置可用性组了。 你将使用 SQL Server PowerShell 提供程序在 **ContosoSQL1** 上执行所有工作。
