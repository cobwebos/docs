---
title: "备份 Azure Windows VM | Microsoft Docs"
...
5. 在本地计算机上，打开一个新选项卡，并转到 [Azure 门户](https://portal.azure.com)。
...
11. 在“步骤 2: 下载脚本以浏览并恢复文件”中，单击“下载可执行文件”按钮。 将文件保存到你的**下载**文件夹。
12. 在本地计算机上，打开**文件资源管理器**，导航到你的**下载**文件夹并复制所下载的 .exe 文件。 文件名将以你的 VM 名称作为前缀。 
13. 在你的 VM 上（通过 RDP 连接），将该 .exe 文件粘贴到 VM 的桌面。 
14. 导航到 VM 的桌面并双击该 .exe 文件。 这将启动一个命令提示符，然后将恢复点装载为你可以访问的文件共享。 完成该共享创建时，键入 **q** 以关闭命令提示符。
15. 在你的 VM 上，打开**文件资源管理器**，导航到用于该文件共享的驱动器号。
...
17. 在本地计算机上，打开你从中连接到 VM 的 IP 地址的浏览器选项卡，其中显示了 IIS 默认页面。 按 CTRL + F5 刷新浏览器页面。 现在，应该会看到图像已还原。
