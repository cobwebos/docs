...
第 2 代 VM 使用新的基于 UEFI 的启动体系结构，而不是第 1 代 VM 使用的基于 BIOS 的体系结构。 与第 1 代 VM 相比，第 2 代 VM 的启动时间和安装时间可能已缩短。 有关第 2 代 VM 的概述以及第 1 代与第 2 代之间的某些差异，请参阅[应在 Hyper-V 中创建第 1 代还是第 2 代虚拟机？](https://docs.microsoft.com/windows-server/virtualization/hyper-v/plan/should-i-create-a-generation-1-or-2-virtual-machine-in-hyper-v)。
...
对于第 2 代 VM，Azure 目前不支持本地 Hyper-V 所支持的某些特性。
...
### <a name="generation-1-vs-generation-2-features"></a>第 1 代与第 2 代的特性比较
...
### <a name="generation-1-vs-generation-2-capabilities"></a>第 1 代与第 2 代的功能比较
...
1. 选择支持 Gen2 的映像。
1. 单击“创建”。
1. 在“高级”选项卡中的“VM 代系”部分下，选择“第 2 代”选项  。
1. 在“基本信息”选项卡中的“实例详细信息”下，转到“大小”并打开“选择 VM 大小”边栏选项卡   。
1. 选择[支持的第 2 代 VM](#generation-2-vm-sizes)。
1. 执行整个 [Azure 门户创建流](quick-create-portal.md)完成 VM 的创建。
...
如果要创建使用 Windows Server 2012 作为 OS 的 VM，请选择第 1 代 (BIOS) 或第 2 代 (UEFI) VM SKU，如下所示：
...
* **第 2 代 VM 是否已在所有 Azure 区域中提供？**  
    是的。 但是，并非所有[第 2 代 VM 大小](#generation-2-vm-sizes)都已在每个区域中提供。 第 2 代 VM 的可用性取决于 VM 大小的可用性。
...
* **我有一个来自本地第 2 代 VM 的 .vhd 文件。我可以使用该 .vhd 文件在 Azure 中创建第 2 代 VM 吗？**
  是的，你可以将第 2 代 .vhd 文件带到 Azure，并使用该文件创建第 2 代 VM。 请使用以下步骤来执行该操作：
    1. 将 .vhd 上传到要在其中创建 VM 的区域内的某个存储帐户。
...
* **如何增大 OS 磁盘的大小？**  
  大于 2 TB 的 OS 磁盘是第 2 代 VM 的新配置。 默认情况下，第 2 代 VM 的 OS 磁盘小于 2 TB。 可将磁盘大小增大至 4 TB（建议的最大大小）。 使用 Azure CLI 或 Azure 门户可增大 OS 磁盘大小。 有关如何以编程方式扩展磁盘的信息，请参阅[调整磁盘大小](expand-os-disk.md)。

  若要在 Azure 门户中增大 OS 磁盘大小，请执行以下操作：
...
* **为什么在尝试创建第 2 代 VM 时，大小选择器中没有启用我的 VM 大小？**
    ...
    1. 确认“高级”选项卡中的“VM 代系”属性已设为“第 2 代”  。
    1. 确认你搜索的是[支持第 2 代 VM 的 VM 大小](#generation-2-vm-sizes)。
...
* 了解如何[准备 VHD](prepare-for-upload-vhd-image.md) 从本地系统上传到 Azure。
