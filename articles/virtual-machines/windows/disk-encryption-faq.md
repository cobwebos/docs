...
适用于 Windows VM 的 Azure 磁盘加密使用 Windows 的 BitLocker 功能为 OS 磁盘和数据磁盘提供全盘加密。 此外，[VolumeType 参数为 All](disk-encryption-windows.md#enable-encryption-on-a-newly-added-data-disk) 时，它提供临时磁盘加密。  加密内容从 VM 流向存储后端。 于是，为端到端加密提供由客户管理的密钥。
...
Azure 磁盘加密正式版支持 Azure 资源管理器模板、Azure PowerShell 和 Azure CLI。 不同的用户体验提供了灵活性。 可以通过三个不同的选项为 VM 启用磁盘加密。 有关 Azure 磁盘加密中提供的用户体验详细信息和分步指南，请参阅 [Windows 的 Azure 磁盘加密方案](disk-encryption-windows.md)。
...
若要轮换机密，只需要调用最初用于启用磁盘加密的命令，不过要指定一个不同的 Key Vault。 若要轮换密钥加密密钥，调用最初用于启用磁盘加密的命令，指定新的密钥加密。 

>[!WARNING]
> - 如果之前是通过指定 Azure AD 凭据对 [Azure AD 应用使用了 Azure 磁盘加密](disk-encryption-windows-aad.md)来加密此 VM，则必须继续使用此选项来加密 VM。 无法在此加密的 VM 上使用 Azure 磁盘加密，因为不支持此方案，也就是说尚不支持为此加密 VM 从 AAD 应用程序切换出来。

## <a name="how-do-i-add-or-remove-a-key-encryption-key-if-i-didnt-originally-use-one"></a>如果我最初并未使用密钥加密密钥，如何添加或删除密钥加密密钥？

若要添加密钥加密密钥，再次调用启用命令，传递密钥加密密钥参数。 若要删除密钥加密密钥，再次调用启用命令，而不提供密钥加密密钥参数。
...
Azure 磁盘加密具有先决条件。 若要创建 Azure Active Directory 应用程序、新建密钥保管库或设置现有的密钥保管库，以实现能启用加密并保护机密和密钥的磁盘加密访问，请参阅 [Azure AD 的 Azure 磁盘加密](disk-encryption-windows-aad.md)内容。 有关密钥加密密钥支持方案的详细信息，请参阅[使用 Azure AD 为 Azure 磁盘加密创建和部署密钥保管库](disk-encryption-key-vault-aad.md)。
...
Azure 备份提供了一种机制，用于在相同的订阅和区域内备份和还原加密的 VM。  相关说明，请参阅[通过 Azure 备份来备份和还原加密的虚拟机](../../backup/backup-azure-vms-encryption.md)。  当前不支持还原不同区域的加密 VM。  
