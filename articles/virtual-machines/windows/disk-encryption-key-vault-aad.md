...
**新版本的 Azure 磁盘加密消除了提供 Azure AD 应用程序参数以启用 VM 磁盘加密的要求。使用新版本时，不再需要在启用加密步骤期间提供 Azure AD 凭据。使用新版本，必须在没有 Azure AD 应用程序参数的情况下对所有新 VM 进行加密。要查看使用新版本启用 VM 磁盘加密的说明，请参阅 Azure[磁盘加密](disk-encryption-overview.md)。仍支持已使用 Azure AD 应用程序参数加密的 VM，应继续使用 AAD 语法进行维护。**
...
需要在 Azure 中正在运行的 VM 上启用加密时，Azure 磁盘加密将生成加密密钥并将其写入 Key Vault。 在 Key Vault 中管理加密密钥需要 Azure AD 身份验证。 为此，请创建 Azure AD 应用程序。 出于身份验证目的，可以使用基于客户端密钥的身份验证或[基于客户端证书的 Azure AD 身份验证](../../active-directory/authentication/active-directory-certificate-based-authentication-get-started.md)。
...
1. 使用 [New-AzADApplication](/powershell/module/az.resources/new-azadapplication) PowerShell cmdlet 创建 Azure AD 应用程序。 MyApplicationHomePage 和 MyApplicationUri 可以是所需的任意值。
...
2. [创建 Azure 活动目录应用程序](../../active-directory/develop/howto-create-service-principal-portal.md#create-an-azure-active-directory-application) 
...
> [!NOTE]
> Azure 磁盘加密要求为 Azure AD 客户端应用程序配置以下访问策略 -“WrapKey”__ 和“Set”__ 权限。
...
Azure AD 应用程序需有访问保管库中密钥或机密的权限。 使用[Set-AzKeyVaultAccessPolicy](/powershell/module/az.keyvault/set-azkeyvaultaccesspolicy) cmdlet 向应用程序授予权限，使用客户端 ID（在注册应用程序时生成）作为 __ServiceTheName_参数值。 若要了解详细信息，请参阅博客文章 [Azure Key Vault - Step by Step](https://blogs.technet.com/b/kv/archive/2015/06/02/azure-key-vault-step-by-step.aspx)（Azure Key Vault - 分步指南）。 
...
若要使用密钥加密密钥 (KEK) 来为加密密钥提供附加的安全层，请将 KEK 添加到 Key Vault。 使用 [Add-AzKeyVaultKey](/powershell/module/az.keyvault/add-azkeyvaultkey) cmdlet 在 Key Vault 中创建密钥加密密钥。 还可从本地密钥管理 HSM 导入 KEK。 有关详细信息，请参阅[密钥保管库文档](../../key-vault/key-vault-hsm-protected-keys.md)。 指定密钥加密密钥后，Azure 磁盘加密会使用该密钥包装加密机密，然后将机密写入 Key Vault。 
