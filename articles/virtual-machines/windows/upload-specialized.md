...
如果计划按原样使用专用 VHD 创建新的 VM，请确保完成以下步骤。 
  * 如果要使用托管磁盘，请查看[计划迁移到托管磁盘](on-prem-to-azure.md#plan-for-the-migration-to-managed-disks)。
...
专用 OS 可以是[上传到 Azure](upload-image.md) 的 VHD 或者[从现有 Azure VM 复制的 VHD](vhd-copy.md)。 
