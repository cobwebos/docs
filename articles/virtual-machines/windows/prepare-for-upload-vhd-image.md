...
### <a name="run-windows-system-file-checker-utility-run-sfc-scannow-on-os-prior-to-generalization-step-of-creating-customer-os-image"></a>在创建客户操作系统映像的通用化步骤之前，在操作系统上运行 Windows 系统文件检查器实用程序（运行 sfc/scannow）
...
在此命令中，将 `-Path` 的值替换为要转换的虚拟硬盘的路径。 将 `-DestinationPath` 的值替换为转换后磁盘的新路径和名称。
