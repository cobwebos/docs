---
title: 加密 Azure 中 Windows VM 上的磁盘
...
包含加密密钥和关联的计算资源（例如存储和 VM 本身）的 Azure Key Vault 必须位于同一区域。 使用 [New-AzKeyVault](https://docs.microsoft.com/powershell/module/az.keyvault/new-azkeyvault) 创建 Azure 密钥保管库，并启用该密钥保管库进行磁盘加密。 为*密钥库名称*指定唯一的密钥保管库名称，如下所示：
...
* 有关磁盘加密的详细信息，例如准备要上载到 Azure 的已加密自定义 VM，请参阅 [Azure Disk Encryption](../../security/fundamentals/encryption-overview.md)\（Azure 磁盘加密）。
