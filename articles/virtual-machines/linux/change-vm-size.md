...
1. 使用 [az vm list-vm-resize-options](/cli/azure/vm) 查看托管 VM 的硬件群集上可用的 VM 大小的列表。 以下示例列出资源组 `myVM` 区域中名为 `myResourceGroup` 的 VM 的 VM 大小：
...
3. 如果未列出所需的 VM 大小，则需要先使用 [az vm deallocate](/cli/azure/vm) 解除分配 VM。 然后，可以通过此过程将 VM 的大小调整为区域支持的任何可用大小，再启动该 VM。 以下步骤会将名为 `myVM` 的资源组中名为 `myResourceGroup` 的 VM 解除分配、调整大小并启动：
...
为了增加可伸缩性，请运行多个 VM 实例并进行横向扩展。有关详细信息，请参阅[自动缩放虚拟机规模集中的 Linux 计算机][scale-set]。 
