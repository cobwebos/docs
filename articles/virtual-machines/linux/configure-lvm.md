...
本文档介绍如何将多个数据磁盘合并成单个卷组，并创建条带化逻辑卷。 下面的步骤已通用化，适用于大多数分发。 在大多数情况下，Azure 上用于管理 LVM 的实用工具和工作流与其他环境中的基本上相同。 像往常一样，另请咨询 Linux 供应商以获得配合特定分发使用 LVM 的文档和最佳做法。
