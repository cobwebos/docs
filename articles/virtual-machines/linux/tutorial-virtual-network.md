...
本教程介绍了如何在虚拟网络中创建多个虚拟机 (VM)，以及如何在虚拟机之间配置网络连接。 完成后，将可从适用于 SSH 的端口 22 和适用于 HTTP 连接的端口 80 上的 Internet 访问“前端”VM。 具有 MySQL 数据库的“后端”VM 将被隔离，且只能从端口 3306 上的前端 VM 访问。
...
- myVMVNET - `myFrontendVM` 连接到的虚拟网络。
...
网络安全组 (NSG) 包含一系列安全规则，这些规则可允许或拒绝流向连接到 VNet 的资源的网络流量。 NSG 可与子网或附加到 VM 的单个 NIC 相关联。 使用 NSG 规则打开或关闭通过端口到 VM 的访问。 创建 `myFrontendVM` 时，会自动打开入站端口 22 进行 SSH 连接。
...
使用 [az network nsg create](https://docs.microsoft.com/cli/azure/network/nsg#create) 添加名为 `myBackendNSG` 的新网络安全组。 
...
设置一个端口，使 `myFrontendVM` 和 `myBackendVM` 在 VNet 中相互通信。 使用 `az network rule create` 添加一个 NSG 规则，仅允许从 `myVMSubnet` 到 `myBackendSubnet` 的通信：
...
使用 `az vm create` 和 `myBackendSubnet` 创建 `myBackendVM`：
...
后端 VM 旨在不被公开访问。 本部分介绍了如何使用 SSH 登录到 `myFrontendVM`，然后使用 SSH 从 `myFrontendVM` 登录到 `myBackendVM`。
...
从 `myFrontendVM` 连接到 `myBackendVM`：
...
安装 MySQL 以显示 `myBackendVM` 上的应用程序安装方式，但本教程中实际并未使用。
