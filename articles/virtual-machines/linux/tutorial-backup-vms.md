...
默认情况下，Azure 备份为 Linux VM 创建文件系统一致备份，但可以[使用前脚本和后脚本框架将其配置为创建应用程序一致备份](../../backup/backup-azure-linux-app-consistent.md)。 Azure 备份服务创建快照后，数据将传输到保管库。 为最大限度地提高效率，服务仅标识和传输自上次备份以后已更改的数据块。
