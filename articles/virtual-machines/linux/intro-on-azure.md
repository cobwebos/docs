...
---
# <a name="introduction-to-linux-on-azure"></a>Azure 上的 Linux 简介
本主题概述了在 Azure 云中使用 Linux 虚拟机的某些方面。 在使用库中的映像时，部署 Linux 虚拟机是一个非常简单的过程。

## <a name="authentication-usernames-passwords-and-ssh-keys"></a>身份验证：用户名、密码和 SSH 密钥
使用 Azure 经典门户创建 Linux 虚拟机时，系统会要求提供用户名、密码或 SSH 公钥。 在 Azure 上部署 Linux 虚拟机时，用户名的选择受到以下限制：不允许使用虚拟机中已经存在的系统帐户 (UID <100) 的名称，例如，“根”。
...
## <a name="obtaining-superuser-privileges-using-sudo"></a>使用 `sudo` 获取超级用户特权
...
## <a name="hostname-changes"></a>主机名更改
...
## <a name="virtual-machine-image-capture"></a>虚拟机映像捕获
