...
放置在 Azure 中 Linux VM 上的公钥默认存储在 ~/.ssh/id_rsa.pub 中，除非在创建密钥对时指定了不同的位置。 若要借助现有公钥使用 [Azure CLI 2.0](/cli/azure) 创建 VM，请结合 `--ssh-key-values` 选项使用 [az vm create](/cli/azure/vm#az-vm-create) 命令，来指定此公钥的值和（可选的）位置。 在以下命令中，请将 *VMname*、*RGname* 和 *keyFile* 替换为自己的值：
