---
title: 使用云 init 将用户添加到 Azure 上的 Linux VM
...
ms.locfileid: "78969242"
---
# <a name="use-cloud-init-to-add-a-user-to-a-linux-vm-in-azure"></a>使用 cloud-init 将用户添加到 Azure 中的 Linux VM
本文介绍如何在 Azure 中使用 [cloud-init](https://cloudinit.readthedocs.io) 在预配时将用户添加到 Linux 虚拟机 (VM) 或虚拟机规模集 (VMSS)。 Azure 预配资源后，此 cloud-init 脚本将在首次启动时运行。 有关云 init 如何在 Azure 中本机工作和支持的 Linux 发行版的详细信息，请参阅[云 init 概述](using-cloud-init.md)。
