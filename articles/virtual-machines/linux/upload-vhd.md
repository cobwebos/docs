...
使用 [az vm create](/cli/azure/vm#create) 从自定义或专用的磁盘创建新 VM 时，请**附加**该磁盘 (--attach-os-disk)，而不要指定自定义映像或 Marketplace 映像 (--image)。 以下示例使用从自定义 VHD 创建的、名为 *myManagedDisk* 的托管磁盘，创建名为 *myVM* 的 VM：
...
可以使用 [az storage account create](/cli/azure/storage/account#create) 为自定义磁盘和 VM 创建存储帐户。 
...
现在，使用 [az storage blob upload](/cli/azure/storage/blob#upload) 上传自定义磁盘。 可以页 Blob 的形式上传和存储自定义磁盘。
...
使用 [az disk create](/cli/azure/disk#create) 从 VHD 创建托管磁盘。 以下示例从已上传到命名存储帐户和容器的 VHD 创建名为 *myManagedDisk* 的托管磁盘：
...
现在，使用 [az vm create](/cli/azure/vm#create) 创建 VM，并将托管磁盘附加为 OS 磁盘 (--attach-os-disk)。 以下示例使用基于上传的 VHD 创建的托管磁盘创建名为 *myNewVM* 的 VM：
