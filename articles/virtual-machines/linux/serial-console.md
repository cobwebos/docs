...
场景          | 串行控制台中的操作                |  OS 适用性 
:------------------|:-----------------------------------------|:------------------
受损的 FSTAB 文件 | 按 `Enter` 键继续，然后使用文本编辑器修复 fstab 文件。 可能需要在单用户模式下执行此操作。 请参阅[如何修复 fstab 问题](https://support.microsoft.com/help/3206699/azure-linux-vm-cannot-start-because-of-fstab-errors)和[使用串行控制台访问 GRUB 和单用户模式](serial-console-grub-single-user-mode.md)获取入门信息。 | Linux 
...
与引导加载程序交互 | 通过串行控制台访问 GRUB/BCD。 参阅[使用串行控制台访问 GRUB 和单用户模式](serial-console-grub-single-user-mode.md)获取入门信息。 | Linux/Windows 
