...
description: 如何使用 Azure CLI 将 Azure 托管磁盘存储从标准转换为高级或高级到标准。
author: roygara
...
Azure 托管磁盘有四种磁盘类型：Azure 超 SSD（预览）、高级 SSD、标准 SSD 和标准 HDD。 可以根据性能需求在三种 GA 磁盘类型（高级 SSD、标准 SSD 和标准 HDD）之间切换。 您还不能从超级 SSD 切换到超级 SSD，必须部署新的 SSD。
