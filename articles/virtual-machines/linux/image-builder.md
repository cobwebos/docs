...
您还可以指定 `buildTimeoutInMinutes`。 默认值为240分钟，可以增加生成时间以允许较长的运行生成。
...
您可以根据需要修改此示例。 例如，可以增加 `buildTimeoutInMinutes` 的值，以允许较长的运行生成。 您可以使用文本编辑器（如 `vi`）在 Cloud Shell 中编辑该文件。
