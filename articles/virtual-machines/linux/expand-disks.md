...
> [!WARNING]
> 始终确保在文件系统处于正常状态，并确保在执行磁盘重设大小操作之前备份你的数据。 有关详细信息，请参阅[在 Azure 中备份 Linux VM](tutorial-backup-vms.md)。
...
1. 若要验证是否已调整大小的数据磁盘，请使用`df -h`。 以下示例输出显示数据驱动器 */dev/sdc1* 现在为 200 GB：
