...
只能对具有[支持的 VM 大小和操作系统](disk-encryption-overview.md#supported-vms-and-operating-systems)的虚拟机应用磁盘加密。 此外，还必须满足以下先决条件：
...
在所有情况下，都应该先[拍摄快照](snapshot-copy-managed-disk.md)和/或创建备份，再加密磁盘。 备份确保在加密过程中发生任何意外故障时可以使用恢复选项。 加密之前，需要备份包含托管磁盘的 VM。 备份后，可以运行 [Set-AzVMDiskEncryptionExtension cmdlet](/powershell/module/az.compute/set-azvmdiskencryptionextension) 通过指定 -skipVmBackup 参数来加密托管磁盘。 有关如何备份和还原已加密 VM 的详细信息，请参阅 [Azure 备份](../../backup/backup-azure-vms-encryption.md)一文。 

>[!WARNING]
> - 如果之前是结合使用 Azure 磁盘加密和 Azure AD 来加密 VM，必须继续使用这种方式来加密 VM。 有关详细信息，请参阅[结合使用 Azure 磁盘加密和 Azure AD（旧版）](disk-encryption-overview-aad.md)。 
>
> - 加密 Linux OS 卷时，应将 VM 视为不可用。 强烈建议在加密正在进行的过程中避免 SSH 登录，以免在加密过程中出现问题，导致需要访问的任何打开文件被拦截。 若要检查进度，请运行 [Get-AzVMDiskEncryptionStatus](/powershell/module/az.compute/get-azvmdiskencryptionstatus) PowerShell cmdlet 或 [vm encryption show](/cli/azure/vm/encryption#az-vm-encryption-show) CLI 命令。 对于 30GB 操作系统卷，此过程可能需要几小时才能完成，还需要额外的时间来加密数据卷。 除非使用“encrypt format all”选项，否则数据卷加密时间将与数据卷的大小和数量成比例。 
> - 在 Linux VM 上，仅支持对数据卷禁用加密。 如果 OS 卷已加密，则不支持对数据卷或 OS 卷禁用加密。  

## <a name="install-tools-and-connect-to-azure"></a>安装工具并连接到 Azure

可以通过 [Azure CLI](/cli/azure) 和 [Azure PowerShell](/powershell/azure/new-azureps-module-az) 来启用和管理 Azure 磁盘加密。 为此，必须在本地安装工具，并连接到 Azure 订阅。

### <a name="azure-cli"></a>Azure CLI

[Azure CLI 2.0](/cli/azure) 是用于管理 Azure 资源的命令行工具。 CLI 旨在提高数据查询灵活性、支持非阻塞进程形式的长时间操作，以及简化脚本编写。 可以按照[安装 Azure CLI](/cli/azure/install-azure-cli?view=azure-cli-latest) 中的步骤操作，在本地安装它。

 

若要[使用 Azure CLI 登录 Azure 帐户](/cli/azure/authenticate-azure-cli)，请运行 [az login](/cli/azure/reference-index?view=azure-cli-latest#az-login) 命令。
...
若要[使用 Azure PowerShell 登录 Azure 帐户](/powershell/azure/authenticate-azureps?view=azps-2.5.0)，请运行 [Connect-AzAccount](/powershell/module/az.accounts/connect-azaccount?view=azps-2.5.0) cmdlet。

```powershell
Connect-AzAccount
```

如果有多个订阅并要指定一个，请先运行 [Get-AzSubscription](/powershell/module/Az.Accounts/Get-AzSubscription) cmdlet 列出订阅，再运行 [Set-AzContext](/powershell/module/az.accounts/set-azcontext?view=azps-2.5.0) cmdlet：

```powershell
Set-AzContext -Subscription -Subscription <SubscriptionId>
```

运行 [Get-AzContext](/powershell/module/Az.Accounts/Get-AzContext) cmdlet 验证是否选择了正确的订阅。

若要确认是否已安装 Azure 磁盘加密 cmdlet，请运行 [Get-command](/powershell/module/microsoft.powershell.core/get-command?view=powershell-6) cmdlet：
...
>[!IMPORTANT]
 >启用 Azure 磁盘加密之前，必须在其外部创建基于托管磁盘的 VM 实例的快照和/或备份。 托管磁盘快照可以在门户中获取，也可以通过 [Azure 备份](../../backup/backup-azure-vms-encryption.md)获取。 备份确保在加密过程中发生任何意外故障时可以使用恢复选项。 备份后，可以运行 Set-AzVMDiskEncryptionExtension cmdlet 通过指定 -skipVmBackup 参数来加密托管磁盘。 除非已备份且已指定此参数，否则无法对基于托管磁盘的 VM 运行 Set-AzVMDiskEncryptionExtension 命令。 
>
>加密或禁用加密可能导致 VM 重新启动。 
>
...
通过安装并使用 [Azure CLI](/cli/azure/?view=azure-cli-latest) 命令行工具，可以在加密的 VHD 上启用磁盘加密。 可以在浏览器中结合 [Azure Cloud Shell](../../cloud-shell/overview.md) 使用这些 cmdlet，或者将它们安装在本地计算机上并在任何 PowerShell 会话中使用。 若要在 Azure 中现有或正在运行的 Linux VM 上启用加密，请运行以下 CLI 命令：

请运行 [az vm encryption enable](/cli/azure/vm/encryption?view=azure-cli-latest#az-vm-encryption-show) 命令，以在 Azure 中正在运行的虚拟机上启用加密。
...
请运行 [Set-AzVMDiskEncryptionExtension](/powershell/module/az.compute/set-azvmdiskencryptionextension) cmdlet，以在 Azure 中正在运行的虚拟机上启用加密。 请先拍摄[快照](snapshot-copy-managed-disk.md)和/或使用 [Azure 备份](../../backup/backup-azure-vms-encryption.md)对 VM 进行备份，再加密磁盘。 PowerShell 脚本中已指定 -skipVmBackup 参数，用于对正在运行的 Linux VM 进行加密。

-  **加密正在运行的 VM：** 以下脚本初始化变量，并运行 Set-AzVMDiskEncryptionExtension cmdlet。 资源组、VM 和密钥保管库已作为先决条件创建。 将 MyVirtualMachineResourceGroup、MySecureVM 和 MySecureVault 替换为你的值。 修改 -VolumeType 参数，以指定要加密哪些磁盘。
...
- **禁用磁盘加密：** 若要禁用加密，请运行 [Disable-AzVMDiskEncryption](/powershell/module/az.compute/disable-azvmdiskencryption) cmdlet。 只允许对 Linux VM 的数据卷禁用加密。
...
通过使用[资源管理器模板](https://github.com/Azure/azure-quickstart-templates/tree/master/201-encrypt-running-linux-vm-without-aad)，可以在 Azure 中现有或正在运行的 Linux VM 上启用磁盘加密。
...
| KeyVaultName | 加密密钥应上传到的密钥保管库的名称。 可以通过运行 cmdlet `(Get-AzKeyVault -ResourceGroupName <MyKeyVaultResourceGroupName>). Vaultname` 或 Azure CLI 命令 `az keyvault list --resource-group "MyKeyVaultResourceGroupName"` 来获取它。|
...
若要详细了解如何配置 Linux VM 磁盘加密模板，请参阅[适用于 Linux 的 Azure 磁盘加密](https://docs.microsoft.com/azure/virtual-machines/extensions/azure-disk-enc-linux)。
...
**EncryptFormatAll** 参数可以减少加密 Linux 数据磁盘所需的时间。 满足特定条件的分区与当前文件系统一起格式化，然后重新装载回在命令执行前的位置。 如果想要排除某个符合条件的数据磁盘，可以在运行命令之前卸载该磁盘。
 ...
 在此命令运行后，先前装载的所有驱动器都会被格式化，并在现在已空的驱动器之上启动加密层。 如果你选择这种方式，附加到 VM 的临时磁盘也会得到加密。 如果临时磁盘已重置，则会在下次机会时通过 Azure 磁盘加密解决方案为 VM 重新格式化和重新加密它。 一旦资源磁盘被加密，[Microsoft Azure Linux 代理](https://docs.microsoft.com/azure/virtual-machines/extensions/agent-linux)就无法管理资源磁盘和启用交换文件，但你可以手动配置交换文件。
...
> 如果 VM 的数据卷上存在所需的数据，则不应使用 EncryptFormatAll。 卸载磁盘可将其从加密项中排除。 首先应该在测试 VM 上试用 EncryptFormatAll，以了解功能参数及其影响，然后再尝试在生产 VM 上使用该参数。 EncryptFormatAll 选项会格式化数据磁盘，因此磁盘上的所有数据都会丢失。 在继续之前，请验证是否已正确卸载想要排除的磁盘。 </br></br>
 >如果在更新加密设置时设置此参数，可能会导致在实际加密之前重新启动。 在这种情况下，不妨还从 fstab 文件中删除你不想格式化的磁盘。 同样，在启动加密操作之前，应将想要加密并格式化的分区添加到 fstab 文件。 
...
### <a name="use-the-encryptformatall-parameter-with-azure-cli"></a>结合使用 EncryptFormatAll 参数和 Azure CLI
请运行 [az vm encryption enable](/cli/azure/vm/encryption#az-vm-encryption-enable) 命令，以在 Azure 中正在运行的虚拟机上启用加密。
...
### <a name="use-the-encryptformatall-parameter-with-a-powershell-cmdlet"></a>结合使用 EncryptFormatAll 参数和 PowerShell cmdlet
请结合使用 [Set-AzVMDiskEncryptionExtension](/powershell/module/az.compute/set-azvmdiskencryptionextension) cmdlet 和 EncryptFormatAll 参数。 

**使用 EncryptFormatAll 加密正在运行的 VM：** 例如，以下脚本初始化变量，并在运行 Set-AzVMDiskEncryptionExtension cmdlet 的同时使用 EncryptFormatAll 参数。 资源组、VM 和密钥保管库已作为先决条件创建。 将 MyVirtualMachineResourceGroup、MySecureVM 和 MySecureVault 替换为你的值。
...
### <a name="use-the-encryptformatall-parameter-with-logical-volume-manager-lvm"></a>结合使用 EncryptFormatAll 参数和逻辑卷管理器 (LVM) 
...
1. 选择分区标准，创建跨越整个驱动器的分区，然后格式化分区。 此处使用了 Azure 生成的符号链接。 使用符号链接可避免设备名更改所造成的问题。 有关详细信息，请参阅[排查设备名问题](troubleshoot-device-names-problems.md)一文。
...
请按照 Azure 磁盘加密相同脚本中的说明操作，准备可以在 Azure 中使用的预加密映像。 创建映像后，可使用下一部分中的步骤创建加密的 Azure VM。

* [准备预加密的 Linux VHD](disk-encryption-sample-scripts.md#prepare-a-pre-encrypted-linux-vhd)

>[!IMPORTANT]
 >启用 Azure 磁盘加密之前，必须在其外部创建基于托管磁盘的 VM 实例的快照和/或备份。 可以从门户创建托管磁盘的快照，也可以使用 [Azure 备份](../../backup/backup-azure-vms-encryption.md)。 备份确保在加密过程中发生任何意外故障时可以使用恢复选项。 备份后，可以运行 Set-AzVMDiskEncryptionExtension cmdlet 通过指定 -skipVmBackup 参数来加密托管磁盘。 除非已备份且已指定此参数，否则无法对基于托管磁盘的 VM 运行 Set-AzVMDiskEncryptionExtension 命令。 
>
> 加密或禁用加密可能导致 VM 重新启动。 
...
若要在加密的 VHD 上启用磁盘加密，可以运行 PowerShell cmdlet [Set-AzVMOSDisk](/powershell/module/Az.Compute/Set-AzVMOSDisk#examples)。 以下示例显示了一些常用参数。 
...
### <a name="enable-encryption-on-a-newly-added-disk-with-azure-cli"></a>使用 Azure CLI 在新添加的磁盘上启用加密
 ...
 如果 VM 之前是使用“All”进行加密，应将 --volume-type 参数的值保留为“All”。 All 包括 OS 和数据磁盘。 如果 VM 之前是使用卷类型“OS”进行加密，应将 --volume-type 参数的值更改为“All”，以便同时包含 OS 和新数据磁盘。 如果 VM 仅使用卷类型“Data”进行加密，则它可以保留为“Data”，如下所示。 添加新数据磁盘并将其附加到 VM 并不足以为加密做准备。 在启用加密之前，还必须格式化新附加的磁盘并将其正确装载在 VM 中。 在 Linux 上，磁盘必须使用[永久性块设备名称](troubleshoot-device-names-problems.md)装载在 /etc/fstab 中。  
...
### <a name="enable-encryption-on-a-newly-added-disk-with-azure-powershell"></a>使用 Azure PowerShell 在新添加的磁盘上启用加密
 当使用 PowerShell 加密适用于 Linux 的新磁盘时，需要指定新的序列版本。 序列版本必须唯一。 以下脚本生成序列版本的 GUID。 请先拍摄[快照](snapshot-copy-managed-disk.md)和/或使用 [Azure 备份](../../backup/backup-azure-vms-encryption.md)对 VM 进行备份，再加密磁盘。 PowerShell 脚本中已指定 -skipVmBackup 参数，用于对新添加的数据磁盘进行加密。
 

-  **加密正在运行的 VM 的数据卷：** 以下脚本初始化变量，并运行 Set-AzVMDiskEncryptionExtension cmdlet。 先决条件是事先创建资源组、VM 和密钥保管库。 将 MyVirtualMachineResourceGroup、MySecureVM 和 MySecureVault 替换为你的值。 -VolumeType 参数可接受的值为 All、OS 和 Data。 如果 VM 之前是使用卷类型“OS”或“All”进行加密，应将 -VolumeType 参数的值更改为“All”，以便同时包含 OS 和新数据磁盘。
...
- **使用 Azure PowerShell 禁用磁盘加密：** 若要禁用加密，请运行 [Disable-AzVMDiskEncryption](/powershell/module/az.compute/disable-azvmdiskencryption) cmdlet。 
     ```azurepowershell-interactive
...
- 当 OS 驱动器被加密时，禁用 Linux VM 的 OS 驱动器或数据驱动器上的加密。
...
- 动态卷。
- 临时 OS 磁盘。
- 加密共享/分布式文件系统，例如（但不限于）：DFS、GFS、DRDB 和 CephFS。
- 将加密的 VM 迁移到另一个订阅。
- 内核故障转储 (kdump)。
