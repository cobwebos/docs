...
description: 如何使用 Azure CLI 在 Linux 虚拟机上安装和使用 Docker 和合成。
author: cynthn
...
使用[Compose](https://github.com/docker/compose)，您可以使用简单的文本文件来定义由多个 Docker 容器组成的应用程序。 然后使用单个命令启动应用程序，该命令会执行部署定义的环境所需的所有操作。 作为示例，本文说明如何在 Ubuntu VM 上使用后端 MariaDB SQL 数据库快速设置 WordPress 博客。 也可以使用 Compose 设置更复杂的应用程序。
...
首先，使用 [az group create](/cli/azure/group) 为 Docker 环境创建资源组。 下面的示例在*东部*位置创建名为*myResourceGroup*的资源组：
