...
- 可以手动部署必要的 Azure 基础结构组件，然后按照 [OpenShift 容器平台文档](https://docs.openshift.com/container-platform)进行操作。
- 也可使用现有的[资源管理器模板](https://github.com/Microsoft/openshift-container-platform/)，该模板可简化 OpenShift 容器平台群集的部署流程。
- 另一种做法是使用 [Azure Marketplace 产品/服务](https://azuremarketplace.microsoft.com/marketplace/apps/osatesting.open-shift-azure-proxy)。
...
| `dataDiskSize` | 要附加到用于 Docker 卷 (的数据磁盘的大小（GB)  | 32、64、128、256、512、1024、2048 | 64 |
| `cnsGlusterDiskSize` | 要附加到 CNS 节点以供 glusterfs 使用的数据磁盘的大小（GB） ( | 32、64、128、256、512、1024、2048 | 128 |
| `adminUsername` |  (VM) 登录名和初始 OpenShift 用户的管理员用户名 |  | ocpadmin |
