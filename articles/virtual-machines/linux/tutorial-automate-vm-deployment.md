...
若要使 VM 能使用 Web 流量，请通过 [az vm open-port](/cli/azure/vm#az-vm-open-port) 从 Internet 打开端口 80：
...
现使用 [az vm create](/cli/azure/vm#az-vm-create) 创建 VM。 使用 `--secrets` 参数注入 Key Vault 中的证书数据。 与上个示例一样，使用 `--custom-data` 参数传入 cloud-init 配置：
...
若要使 VM 能使用安全的 Web 流量，请通过 [az vm open-port](/cli/azure/vm#az-vm-open-port) 从 Internet 打开端口 443：
