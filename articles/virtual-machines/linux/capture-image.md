...
1. 通过 SSH 连接到 VM，并使用 `waagent -deprovision` 取消设置。 +user 参数还会删除上次预配的用户帐户。 如果正在将帐户凭据收录到虚拟机，请省略此 +user 参数。 以下示例删除上次预配的用户帐户：
...
3. 使用 [az vm generalize](/cli/azure/vm#generalize) 通用化 VM。 如果使用了 [Packer](http://www.packer.io) 等工具来构建源 VM，请跳过此步骤，因为映像为通用映像。
...
执行下列步骤可取消设置现有 VM，解除分配并通用化 VM，然后创建映像。 可以使用此映像，在订阅内的任何资源组中创建 VM。 此过程使 [Azure 托管磁盘](../../storage/storage-managed-disks-overview.md)比非托管磁盘更具优势。 使用非托管磁盘时，只能在复制 VHD blob 的存储帐户中创建虚拟机。 使用托管磁盘时，所创建的映像资源可部署在整个订阅之中。

## <a name="step-1-remove-the-azure-linux-agent"></a>步骤 1：删除 Azure Linux 代理
要使 VM 做好进行通用化的准备，请使用 Azure VM 代理取消设置 VM，以删除文件和数据。 在目标 Linux VM 上，使用带 -deprovision 参数的 `waagent` 命令。 +user 参数还会删除上次预配的用户帐户。 如果正在将帐户凭据收录到虚拟机，请省略此 +user 参数。 以下示例删除上次预配的用户帐户。 有关详细信息，请参阅 [Azure Linux 代理用户指南](../windows/agent-user-guide.md)。
...
2. 在 SSH 窗口中，键入以下命令：
    ...
    ```
   > [!NOTE]
   > 仅在要捕获为映像的 VM 上运行此命令。 它无法保证中的所有敏感信息均已清除，也无法保证该映像适合再分发。
...
2. 使用 [az vm generalize](/cli//azure/vm#generalize) 通用化 VM。 如果使用了 [Packer](http://www.packer.io) 等工具来构建源 VM，请跳过此步骤，因为映像为通用映像。 以下示例在名为 myResourceGroup 的资源组中通用化名为 myVM 的 VM：
...
使用通过 [az vm create](/cli/azure/vm#create) 创建的映像来创建 VM。 以下示例从名为 myImage 的映像创建名为 myVMDeployed 的映像：
...
以下示例使用 [az vm create](/cli/azure/vm#create)，通过指定映像资源 ID，在与源映像不同的资源组中创建 VM：
