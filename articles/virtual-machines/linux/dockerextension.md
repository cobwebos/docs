...
---
# 在 Azure 中使用 Docker VM 扩展创建 Docker 环境
...
（可选）若要改为将控制返回提示符，并允许部署在后台继续运行，请将 `--no-wait` 标志添加到前一个命令。 此过程允许在 CLI 中执行其他工作，同时部署将持续几分钟。 
