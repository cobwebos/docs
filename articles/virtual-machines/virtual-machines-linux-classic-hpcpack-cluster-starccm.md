...
<properties
 pageTitle="在 Linux VM 上运行 STAR-CCM+ 与 HPC Pack | Azure"
...
本文将说明如何在 Azure 上部署 Microsoft HPC Pack 群集，并在与 InfiniBand 互连的多个 Linux 计算节点上运行 [CD-adapco STAR-CCM+](http://www.cd-adapco.com/products/star-ccm%C2%AE) 作业。

[AZURE.INCLUDE [了解部署模型](../includes/learn-about-deployment-models-both-include.md)]

Microsoft HPC Pack 可提供在 Azure 虚拟机群集上运行各种大型 HPC 和并行应用程序的功能，包括 MPI 应用程序。HPC Pack 还支持在 HPC Pack 群集中部署的 Linux 计算节点 VM 上运行 Linux HPC 应用程序。有关将 Linux 计算节点与 HPC Pack 配合使用的简介，请参阅 [Get started with Linux compute nodes in an HPC Pack cluster in Azure（Azure 的 HPC Pack 群集中的 Linux 计算节点入门）](/documentation/articles/virtual-machines-linux-classic-hpcpack-cluster)。

## 设置 HPC Pack 群集
从[下载中心](https://www.microsoft.com/download/details.aspx?id=44949)下载 HPC Pack IaaS 部署脚本，并在本地将其解压缩。

Azure PowerShell 是必备组件。如果未在本地计算机上配置 PowerShell，请阅读 [How to install and configure Azure PowerShell（如何安装和配置 Azure PowerShell）](/documentation/articles/powershell-install-configure)一文。

编写本文时，Azure 库中的 Linux 映像（包含 Azure 的 InfiniBand 驱动程序）适用于 SLES 12、CentOS 6.5 和 CentOS 7.1。本文假设使用的是 SLES 12。若要检索应用商店中支持 HPC 的所有 Linux 映像的名称，可以运行以下 PowerShell 命令：
...
以下模板将部署此类头节点，创建名为 **MyCluster.xml** 的 XML 文件，并将 **SubscriptionId**、**StorageAccount**、**Location**、**VMName** 和 **ServiceName** 的值替换成你的值。
...
将文件 **MyCluster.xml** 从本地计算机复制到头节点，并使用要部署的节点数 (<=20) 更新 **NodeCount** 标记。请确保在 Azure 配额中保留足够的可用核心。

在头节点上，复制 HPC Pack IaaS 部署脚本。

在权限提升的命令提示符下运行以下 Azure PowerShell 命令：

1.  运行 **Add-AzureAccount** 以连接到你的 Azure 订阅。

2.  如果你有多个订阅，请运行 **Get-AzureSubscription** 列出这些订阅。
...
遵循 [Get started with Azure File storage on Windows（在 Windows 上开始使用 Azure 文件存储）](..\storage\storage-dotnet-how-to-use-files.md)一文中的说明创建 Azure 文件共享。
...
HPC Pack 随附的一个有用工具是 clusrun 工具。可以使用此命令行工具，在一组计算节点上同时运行同一个命令。在本例中，将使用该工具装载 Azure 文件共享，然后持久保存该文件共享以避免重新启动。在头节点上的权限提升的命令提示符下运行以下命令。
...
## 更新 Linux 驱动程序
最终可能需要更新 Linux 计算节点的 InfiniBand 驱动程序。有关详细信息，请阅读 [Update the Linux RDMA drivers for SLES 12（更新 SLES 12 的 Linux RDMA 驱动程序）](/documentation/articles/virtual-machines-linux-classic-rdma-cluster#update-the-linux-rdma-drivers-for-sles-12)一文。
...
在测试中，我们使用了 Power-On-Demand 许可证令牌。对于该令牌，必须在命令行的 **-podkey** 选项中将 **$CDLMD\_LICENSE\_FILE** 环境变量设置为 **1999@flex.cd-adapco.com** 和密钥。

在完成一些初始化之后，该脚本将从 HPC Pack 设置的 **$CCP\_NODES\_CORES** 环境变量中提取节点列表，以构建 MPI 启动器使用的 hostfile。此 hostfile 将包含作业使用的计算节点名称列表（每行包含一个名称）。
...
* [在 Azure 中的 Linux 计算节点上使用 Microsoft HPC Pack 运行 NAMD](/documentation/articles/virtual-machines-linux-classic-hpcpack-cluster-namd)

* [Run OpenFOAM with Microsoft HPC Pack on a Linux RDMA cluster in Azure（在 Azure 中的 Linux RDMA 群集上运行 OpenFOAM 和 Microsoft HPC Pack）](/documentation/articles/virtual-machines-linux-classic-hpcpack-cluster-openfoam)
