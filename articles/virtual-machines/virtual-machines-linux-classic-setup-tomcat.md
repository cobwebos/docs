---
title: "在 Linux VM 上设置 Apache Tomcat | Microsoft Docs"
...
---
# <a name="how-to-set-up-tomcat7-on-a-linux-virtual-machine-with-microsoft-azure"></a>如何使用 Microsoft Azure 在 Linux 虚拟机上设置 Tomcat7
...
> [!IMPORTANT] 
> Azure 提供两个不同的部署模型用于创建和处理资源：[Resource Manager 和经典模型](../azure-resource-manager/resource-manager-deployment-model.md)。 本文介绍如何使用经典部署模型。 Microsoft 建议大多数新部署使用资源管理器模型。 有关使用 Open JDK 和 Tomcat 部署 Ubuntu VM 的 Resource Manager 模板，请参阅[此处](https://azure.microsoft.com/documentation/templates/openjdk-tomcat-ubuntu-vm/)。
...
假定读者已拥有 Azure 订阅。  如果没有，可以在 [http://azure.microsoft.com](https://azure.microsoft.com/) 上注册免费试用版。 如果已有 MSDN 订阅，请参阅 [Microsoft Azure 特价：MSDN、MPN 和 Bizspark 优惠](https://azure.microsoft.com/pricing/member-offers/msdn-benefits/?c=14-39)。 若要了解有关 Azure 的详细信息，请参阅[什么是 Azure？](https://azure.microsoft.com/overview/what-is-azure/)。
...
4. 在生成过程结束后，Puttygen.exe 将显示生成的密钥。 例如：  
   ![][2]
5. 在“密钥”中选择并复制公钥，然后将它保存在一个名为 publicKey.pem 的文件中。 不要单击“保存公钥”，因为保存的公钥的文件格式不同于所需的公钥。
6. 单击“保存私钥”，并将其保存到名为 privateKey.ppk 的文件中。

### <a name="step-2-create-the-image-in-the-azure-portal"></a>步骤 2：在 Azure 门户中创建映像。
在 [Azure 门户](https://portal.azure.com/)中，单击任务栏中的“新建”以创建映像，并根据需要选择 Linux 映像。 以下示例使用 Ubuntu 14.04 映像。
![][3]

对于“主机名”，指定你和 Internet 客户端将用于访问此虚拟机的 URL 的名称。 定义 DNS 名称的最后部分（例如 tomcatdemo），Azure 将生成 tomcatdemo.cloudapp.net 作为 URL。  

对于“SSH 身份验证密钥”，请从 **publicKey.pem** 文件中复制密钥值，其中包含由 puttygen 生成的公钥。  
![][4]
...
1. 在 Azure 门户中，单击“浏览” -> “虚拟机”，然后单击创建的虚拟机。  
   ![][5]
2. 若要将终结点添加到虚拟机，请单击“终结点”框。
   ![][6]
3. 单击 **“添加”**。  
   
   1. 对于**终结点**，请在“终结点”中键入终结点的名称，然后在“公用端口”中键入 80。  
      
      如果将其设置为 80，则无需在 URL 中包括端口号即可允许你访问 tomcat。 例如 http://tomcatdemo.cloudapp.net。    
      
      如果将其设置为其他值（例如 81），则需要将端口号添加到 URL 才能访问 tomcat。 例如 http://tomcatdemo.cloudapp.net:81/。
   2. 在“专用端口”中键入 8080。 默认情况下，tomcat 侦听 TCP 端口 8080。 如果更改了 tomcat 的默认侦听端口，应将专用端口更新为与 tomcat 侦听端口相同。  
      ![][7]
4. 单击“确定”将该终结点添加到虚拟机。
...
首先，从 Azure 门户获取你的虚拟机的 DNS 名称。 单击“浏览” -> “虚拟机”-> 虚拟机的名称 ->“属性”，然后查看“属性”磁贴的“域名”字段。  
...
单击“打开”。 此时可能会通过一个消息框提醒你。 如果已正确配置 DNS 名称和端口号，请单击“是”。
![][11]  
...
> [!NOTE] 
> 这两个 JDK 对于 Java API 中的类，几乎包含相同的代码，但用于虚拟机的代码实际上不同。 涉及到库时，OpenJDK 倾向于使用开放库，而 Oracle 倾向于使用已关闭的库。 但 Oracle JDK 包含更多类和一些修复的 bug，并且 Oracle JDK 比 OpenJDK 更稳定。
...
如果已安装 open-jdk，你应看到如下消息：![][14]

如果已安装 oracle-jdk，你应看到如下消息：![][15]
...
#### <a name="test"></a>测试：
若要检查 tomcat7 是否已成功安装，请浏览到 tomcat 服务器的 DNS 名称（http://tomcatexample.cloudapp.net/ 是本文中的示例 URL）。 如果看到如下页面，则 tomcat7 已正确安装。
![][16]

### <a name="install-other-tomcat-components"></a>安装其他 Tomcat 组件
...
## <a name="phase-4-configure-tomcat"></a>第 4 阶段：配置 Tomcat
在此阶段中，你可以管理 tomcat。
...
### <a name="permission-denied-when-upload-you-project-files-to-varlibtomcat7webapps"></a>将项目文件上载到 /var/lib/tomcat7/webapps/ 时，权限被拒绝
* **症状**  
  使用任何 SFTP 客户端（例如 FileZilla）连接到虚拟机并导航到 /var/lib/tomcat7/webapps/ 来发布站点时，收到如下错误消息：  
  
     状态: 正在列出目录 /var/lib/tomcat7/webapps    命令: put "C:\Users\liang\Desktop\info.jsp" "info.jsp"    错误: /var/lib/tomcat7/webapps/info.jsp: 打开以写入:  权限被拒绝    错误: 文件传输失败
* **可能的根本原因**你无权访问 /var/lib/tomcat7/webapps 文件夹。  
* **解决方案**  
  需要获得根帐户权限。 你可以将该文件夹的所有权从 root 更改为在设置计算机时使用的用户名。 下面是使用 azureuser 帐户名称的示例：  
