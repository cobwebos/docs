...
description: "在 Resource Manager 部署模型中，通过从本地上传的 VHD 在 Azure 中创建托管 VM，并使用托管磁盘。"
...
# <a name="create-a-new-vm-from-a-generalized-vhd-uploaded-to-azure-using-managed-disks"></a>使用托管磁盘从上传到 Azure 的通用化 VHD 中创建新的 VM

可通过上传 VHD（此 VHD 从本地虚拟化工具或其他云中导出）在 Azure 中创建新的 VM。 对新的 VM 使用[托管磁盘](../storage/storage-managed-disks-overview.md)可简化 VM 管理，并在 VM 放置于可用性集中时提供更好的可用性。 如果要上传将用于创建多个 Azure VM 的 VHD，必须先使用 [sysprep](virtual-machines-windows-generalize-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json) 通用化 VHD。 Sysprep 将从 VHD 中删除任何特定于计算机的信息和个人帐户信息。 
...
> [!IMPORTANT]
> 开始迁移到[托管磁盘](../storage/storage-managed-disks-overview.md)之前，请先查看[规划迁移到托管磁盘](virtual-machines-windows-on-prem-to-azure.md#plan-for-the-migration-to-managed-disks)。
>
