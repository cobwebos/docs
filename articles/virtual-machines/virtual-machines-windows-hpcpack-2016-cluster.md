...
1. 包含 Active Directory 域的高可用性群集
    ...
    ![AD 域中的 HA 群集](./media/virtual-machines-windows-hpcpack-2016-cluster/haad.png)



2. 不包含 Active Directory 域的高可用性群集

    ![不包含 AD 域的 HA 群集](./media/virtual-machines-windows-hpcpack-2016-cluster/hanoad.png)

3. 含有一个头节点的群集

   ![含有一个头节点的群集](./media/virtual-machines-windows-hpcpack-2016-cluster/singlehn.png)


## <a name="deploy-a-cluster"></a>部署群集

若要创建群集，请选择一个模板，然后单击“部署到 Azure”。 在 [Azure 门户](https://portal.azure.com)中，指定该模板的参数，如以下步骤中所示。 每个模板为 HPC 群集基础结构创建所需的全部 Azure 资源。 资源包括 Azure 虚拟网络、公共 IP 地址、负载平衡器（仅适用于高可用性群集）、网络接口、可用性集、存储帐户和虚拟机。
...
输入或修改模板参数的值。 单击每个参数旁边的图标可获得帮助信息。 另请参阅 [VM 可用大小](virtual-machines-windows-sizes.md)准则。
...
1. HPC Pack 群集完成部署后，请转到 [Azure 门户](https://portal.azure.com)。 单击“资源组”，然后查找已部署群集的资源组。 可找到头节点虚拟机。
    ...
    ![门户中的群集头节点](./media/virtual-machines-windows-hpcpack-2016-cluster/clusterhns.png)

2. 单击一个头节点（在高可用性群集中，单击任何一个头节点）。 在 **Essentials** 中，可以找到群集的公共 IP 地址或完整的 DNS 名称。

    ![群集连接设置](./media/virtual-machines-windows-hpcpack-2016-cluster/clusterconnect.png)
...
* 向群集提交作业。 请参阅[在 Azure 中将作业提交到 HPC（即 HPC Pack 群集）](virtual-machines-windows-hpcpack-cluster-submit-jobs.md)和[使用 Azure Active Directory 在 Azure 中管理 HPC Pack 2016 群集](virtual-machines-windows-hpcpack-cluster-active-directory.md)。
