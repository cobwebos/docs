<properties 
	pageTitle="使用 SSH 连接到 Linux 虚拟机 | Microsoft Azure" 
...
## 在 Linux 中生成 Microsoft Azure 兼容密钥 ##
...
前面的示例介绍了如何创建可用于 Microsoft Azure 的新密钥。在某些情况下，你可能已经拥有现有的 OpenSSH 兼容公钥和私钥对，并且希望将相同的密钥用于 Microsoft Azure。

OpenSSH 私钥可通过 `openssl` 实用工具直接读取。以下命令将采用现有的 SSH 私钥（在以下示例中为 id\_rsa），并创建 Microsoft Azure 所需的 `.pem` 公钥：
...
## 从 Linux 连接到 Microsoft Azure 虚拟机
