...
---
# <a name="different-ways-to-create-a-linux-vm-including-azure-cli-20-preview"></a>创建 Linux VM 的不同方式（包括使用 Azure CLI 2.0 预览版）
...
- [Azure CLI 2.0（预览版）](../xplat-cli-install.md)- 适用于资源管理部署模型的下一代 CLI

Azure CLI 2.0（预览版）可通过 npm 包、提供发行版的程序包或 Docker 容器跨平台使用。 请务必使用 **az login** 登录。

以下教程提供了有关使用 Azure CLI 2.0（预览版）的示例。 请阅读每篇文章，了解有关所示命令的详细信息：
...
* [使用 Azure CLI 2.0（预览版）创建 Linux VM](virtual-machines-linux-quick-create-cli.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
  ...
  * 本示例使用最新的 Debian 映像和名为 `id_rsa.pub` 的公钥在新资源组中创建 VM：
...
使用 `az vm image` CLI 命令，按发布者、发行版本和内部版本查看可用的内容。
