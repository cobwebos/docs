---
title: "在 VM 中配置 Oracle GoldenGate | Microsoft Docs"
description: "有关如何在 Azure Windows Server VM 中设置和实施 Oracle GoldenGate 以实现高可用性和灾难恢复的分步教程。"
...
本教程假设用户对 Oracle 数据库高可用性和灾难恢复的概念以及 [Oracle GoldenGate](http://docs.oracle.com/goldengate/1212/gg-winux/index.html) 具有理论和实践知识。 有关详细信息，请参阅 [Oracle 网站](http://www.oracle.com/technetwork/database/features/availability/index.html)。

此外，本教程假设你满足以下先决条件：

* 已经查看 [Oracle Virtual Machine images - Miscellaneous Considerations](virtual-machines-windows-classic-oracle-considerations.md?toc=%2fazure%2fvirtual-machines%2fwindows%2fclassic%2ftoc.json)（Oracle 虚拟机映像 - 其他注意事项）主题中的“高可用性和灾难恢复注意事项”部分。 请注意，Azure 支持独立的 Oracle 数据库实例，但目前不支持 Oracle 真正应用程序群集 (Oracle RAC)。
* 已从 [Oracle 下载](http://www.oracle.com/us/downloads/index.html)网站下载了 Oracle GoldenGate 软件。 你已选择产品包“Oracle Fusion Middleware – Data Integration”。 然后，你已选择适用于 Oracle 11g 数据库的 Oracle GoldenGate on Oracle v11.2.1 Media Pack for Microsoft Windows x64（64 位）。 接下来，请下载 Oracle GoldenGate V11.2.1.0.3 for Oracle 11g 64bit on Windows 2008（64 位）。
* 已使用 Windows Server 上的 Oracle 企业版在 Azure 中创建了两个虚拟机 (VM)。 确保这些虚拟机位于[相同的云服务](virtual-machines-linux-load-balance.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)和相同的[虚拟网络](https://azure.microsoft.com/documentation/services/virtual-network/)中，以确保它们可以通过持久性专用 IP 地址相互访问。
...
首先，通过 Azure 经典门户与站点 A 和站点 B 建立远程桌面连接。 打开 Windows 命令提示符，然后创建 Oracle GoldenGate 安装文件的主目录：
...
接下来，在站点 A 和站点 B 上的 %ORACLE\_HOME%\\database 文件夹中找到 INIT\<DatabaseSID\>.ORA 文件，并将以下数据库参数追加到 INITTEST.ora 中：
...
首先，通过 Azure 经典门户与站点 A 和站点 B 建立远程桌面连接。 使用 SQL*Plus 命令窗口将数据库切换到 archivelog 模式：
...
## <a name="3-create-all-necessary-objects-to-support-ddl-replication"></a>3.创建所有必要的对象以支持 DDL 复制
...
## <a name="4-configure-goldengate-manager-on-site-a-and-site-b"></a>4.在站点 A 和站点 B 上配置 GoldenGate Manager
...
## <a name="5-create-extract-group-and-data-pump-processes-on-site-a-and-site-b"></a>5.在站点 A 和站点 B 上创建 Extract Group 和 Data Pump 进程
### <a name="create-extract-and-data-pump-processes-on-site-a"></a>在站点 A 上创建 Extract 和 Data Pump 进程
需要在站点 A 和站点 B 上创建 Extract 和 Data Pump 进程。通过 Azure 经典门户与站点 A 和站点 B 建立远程桌面连接。 打开 GGSCI 命令解释器窗口。 在站点 A 上运行以下命令：
...
### <a name="add-replicat-on-site-b"></a>在站点 B 上添加 REPLICAT
...
### <a name="create-extract-and-data-pump-processes-on-site-b"></a>在站点 B 上创建 Extract 和 Data Pump 进程
...
### <a name="add-replicat-on-site-a"></a>在站点 A 上添加 REPLICAT
...
### <a name="start-extract-and-data-pump-processes-on-site-a"></a>在站点 A 上启动 Extract 和 Data Pump 进程
...
### <a name="start-extract-and-data-pump-processes-on-site-b"></a>在站点 B 上启动 Extract 和 Data Pump 进程
...
### <a name="start-replicat-process-on-site-a"></a>在站点 A 上启动 REPLICAT 进程
...
### <a name="start-replicat-process-on-site-b"></a>在站点 B 上启动 REPLICAT 进程
...
[Azure 的 Oracle 虚拟机映像](virtual-machines-linux-classic-oracle-images.md?toc=%2fazure%2fvirtual-machines%2flinux%2fclassic%2ftoc.json)
