...
在 Azure 虚拟机 （VM） 上排除问题时，可以使用本文中讨论的远程工具而不是使用远程桌面协议 （RDP） 连接到 VM。
...
>* PsExec 使用 TCP 端口 135 和 445。 因此，两个端口必须在防火墙上打开。

## <a name="run-command"></a>运行命令

有关如何使用 run 命令功能在 VM 上运行脚本的详细信息，请参阅[使用运行命令 在 Windows VM 中运行 PowerShell 脚本](../windows/run-command.md)。
...
* VM 已建立连接。
* Azure 虚拟机代理已安装完毕，并且按预期在 VM 上工作。
* 以前未在 VM 上安装该扩展。
 
  扩展仅在第一次使用脚本时注入脚本。 如果以后使用此功能，扩展将识别该功能已使用，并且不会上载新脚本。

将脚本上载到存储帐户，并生成自己的容器。 然后，在已连接到 VM 的计算机上的 Azure PowerShell 中运行以下脚本。
...
>
>对于 RDFE VM，必须有一个配备专用端口 (5986) 和公共端口的终结点。 然后，您还必须在 NSG 上打开面向公众的端口。
...
对于 Azure 资源管理器 VM，请使用门户中的运行命令运行启用RemotePS脚本：
...
* 虚拟网络或部署外部
  ...
  * 对于 Azure 资源管理器 VM，首先向公共 IP 地址添加 DNS 名称。 有关详细步骤，请参阅[在 Azure 门户中创建 Windows VM 的完全限定域名](../windows/portal-create-fqdn.md)。 然后，运行以下命令：
...
您还可以使用调用命令 cmdlet 在 VM 上远程运行脚本。
...
>
>对于 Azure 资源管理器 VM，您必须在 NSG 上打开端口 5986。 有关详细信息，请参阅“安全组”。 
>
>对于 RDFE VM，必须有一个配备专用端口 5986 和公共端口的终结点。 您还必须在 NSG 上打开该面向公众的端口。
...
3. 通过在 **"输入对象名称以选择**"框中输入目标 VM，按**主机名**或**动态 IP（** 首选）查找目标 VM。
...
- 有关 Enter-PSSession cmdlet 的详细信息，请参阅[输入 PSSession](https://technet.microsoft.com/library/hh849707.aspx)。
- 有关使用经典部署模型的 Windows 自定义脚本扩展的详细信息，请参阅[Windows 的自定义脚本扩展](../extensions/custom-script-classic.md)。
