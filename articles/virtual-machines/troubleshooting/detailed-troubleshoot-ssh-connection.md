...
要将组织边缘设备从失败原因中排除，请验证直接连接到 Internet 的计算机是否可以与 Azure VM 建立 SSH 连接。 如果通过站点到站点 VPN 或 Azure ExpressRoute 连接访问 VM，请跳到[来源 4:网络安全组](#nsg)。
...
> [!NOTE]
> 此来源仅适用于使用经典部署模型创建的 VM。 对于使用资源管理器创建的 Vm，请跳到[来源 4:网络安全组](#nsg)。
