...
与 VM 的串行控制台连接遇到错误： "已禁止" （SubscriptionNotEnabled）-订阅名称未定义，id \<订阅 id> 处于未定义的未启用状态 | 如果用户已在中创建了其 Cloud Shell 的存储帐户的订阅已禁用，则可能出现此问题。 若要缓解这种情况，请启动 Cloud Shell 并[执行必要的步骤](https://docs.microsoft.com/azure/cloud-shell/persisting-shell-storage#unmount-clouddrive-1)，为当前订阅中的 Cloud Shell 重新设置后备存储帐户。
