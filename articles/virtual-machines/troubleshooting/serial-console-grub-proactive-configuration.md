...
SysRq 功能可用于直接从 Azure 串行控制台重启崩溃或挂起的 VM，同时还有助于获取对 GRUB 菜单的访问权限，或者从另一个门户窗口或 ssh 会话重启 VM 可能会删除当前的控制台连接，从而使用于显示 GRUB 菜单的 GRUB 超时过期。
...
要修改的文件为/boot/grub/grub.conf。 此`timeout`值将确定显示 GRUB 的时间长度。
