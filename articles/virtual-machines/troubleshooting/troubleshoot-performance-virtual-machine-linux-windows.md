...
本文介绍虚拟机（VM）一般性能故障排除，通过监视和观察瓶颈，并为可能出现的问题提供可能的补救措施。
...
若要启用 VM 诊断，请前往 VM，单击 "**设置**"，然后单击 "**诊断**"。
...
首先，通过选择 VM 来确定 VM 使用的存储帐户（或帐户）。 单击 "**设置**"，然后单击 "**磁盘**"：

![单击 "设置"，然后单击 "磁盘"](media/troubleshoot-performance-virtual-machine-linux-windows/3-storage-disks-disks-selection.png)

在门户中，前往 VM 的存储帐户（或帐户）并完成以下步骤：

![选择 Blob 指标](media/troubleshoot-performance-virtual-machine-linux-windows/4-select-blob-metrics.png)
 
1. 选择**所有设置**。
2. 启用诊断。
3. 选择 "  **Blob*指标**"，并将保留期设置为**30**天。
4. 保存更改。
...
![选择“监视”](media/troubleshoot-performance-virtual-machine-linux-windows/5-observe-monitoring.png)
...
![检查 CPU 瓶颈](media/troubleshoot-performance-virtual-machine-linux-windows/6-cpu-bottleneck-time-range.png)

1. 编辑关系图。
2. 设置时间范围。
3. 接下来需要添加计数器：CPU 的来宾 OS 百分比
4. 保存。
...
如果已增加 VM，并且 CPU 仍在运行 95%，则确定此设置是否为可接受级别提供更好的性能或更高的应用程序吞吐量。 如果不是，则解决单个 application\process。
...
如果升级到更大的 VM，你会发现，在 100% 之前，你仍有持续稳定的增长，可以确定应用程序/进程并进行故障排除。
...
![查看监视中的存储帐户诊断](media/troubleshoot-performance-virtual-machine-linux-windows/7-virtual-machine-storage-account.png)
...
#### <a name="check-for-azure-storage-throttling---add-the-storage-account-metrics-throttlingerror"></a>检查 Azure 存储限制-添加存储帐户度量值：ThrottlingError
...
每个 VHD 最多可支持 60 MB/秒（IOPS 每个 VHD 不公开）。 查看数据，查看是否在 VM 级别使用磁盘读取和写入来达到了 VM 级别的最小的 VHD 吞吐量，并优化了 VM 存储配置以扩展过去的单个 VHD 限制。
