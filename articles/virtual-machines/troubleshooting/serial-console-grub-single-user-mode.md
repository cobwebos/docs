...
RHEL 原本就启用了 GRUB。 若要进入 GRUB，请运行来重新启动 VM， `sudo reboot` 然后按任意键。 应显示 GRUB 窗格。 如果不是这样，请确保 GRUB 文件（）中存在以下行 `/etc/default/grub` ：
...
尝试上述说明后，可能会有某种情况（如忘记密码），在 Ubuntu VM 中仍无法访问单用户模式。 你还可以将内核指示为 `/bin/bash` 作为 init 运行，而不是在系统 init 中运行。 此操作可提供 bash shell 并允许系统维护。 请遵照以下说明：
