---
title: "Azure Resource Manager 模板中的虚拟机 | Microsoft Azure"
...
- [名称](virtual-machines-linux-infrastructure-naming-guidelines.md)和凭据
- 磁盘和[操作系统设置](virtual-machines-windows-cli-ps-findimage.md)
- [网络接口](../virtual-network/virtual-networks-multiple-nics.md) 
- 启动诊断

## <a name="disks-and-images"></a>磁盘和映像
   
在 Azure 中，vhd 文件可以表示[磁盘或映像](virtual-machines-windows-about-disks-vhds.md)。 如果 vhd 文件中的操作系统专用于特定的 VM，则该文件称为磁盘。 如果 vhd 文件中的操作系统经过通用化，用于创建许多 VM，则该文件称为映像。   
...
磁盘的配置设置是使用 osDisk 元素分配的。 本示例定义了磁盘存储中的位置、磁盘的缓存模式，以及要从[平台映像](virtual-machines-windows-cli-ps-findimage.md)创建磁盘：
...
### <a name="create-new-virtual-machines-from-existing-disks"></a>从现有磁盘创建新虚拟机
...
### <a name="create-new-virtual-machines-from-a-custom-image"></a>从自定义映像创建新虚拟机

若要从[自定义映像](virtual-machines-windows-upload-image.md)创建虚拟机，请删除 imageReference 元素，然后定义以下磁盘设置：
...
可以选择将数据磁盘添加到 VM。 [磁盘数目](virtual-machines-windows-sizes.md)取决于要使用的操作系统磁盘的大小。 如果 VM 的大小设置为 Standard_DS1_v2，则可添加到 VM 的数据磁盘数目上限为&2;。 在本示例中，将向每个 VM 添加一个数据磁盘：
...
尽管[扩展](virtual-machines-windows-extensions-features.md)是独立的资源，但它们与 VM 密切相关。 可将扩展添加为 VM 的子资源，或添加为独立的资源。 本示例显示要将[诊断扩展](virtual-machines-windows-extensions-diagnostics-template.md)添加到 VM：
...
可在 VM 上安装许多扩展，但最有用的扩展也许是[自定义脚本扩展](virtual-machines-windows-extensions-customscript.md)。 在本示例中，首次启动每个 VM 时，将在 VM 上运行名为 start.ps1 的 PowerShell 脚本：
...
- 参考[使用 Resource Manager 模板创建 Windows 虚拟机](virtual-machines-windows-ps-template.md)部署创建的模板。
- 查看[使用 Azure Resource Manager 和 PowerShell 管理虚拟机](virtual-machines-windows-ps-manage.md)，了解如何管理创建的 VM。
