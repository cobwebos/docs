...
## Azure 虚拟机
借助 Azure 虚拟机，你可以采用灵活的方式部署各种计算解决方案。若要部署 Windows 或 Linux 虚拟机，可以通过映像库或我们不断增加的合作伙伴列表内的任何成员所创建的自定义映像来进行。你几乎可以在任何操作系统上以几乎任何语言部署几乎任何工作负荷。没有找到所需的映像？ 别担心，你也可以使用本地的自有映像。
...
## 后勤：区域、分发版、可用性、VM 大小和配额
### 区域
Azure 资源分布在世界各地的多个地理区域。一个“区域”代表位于单个地理区域的多个数据中心。截至 2016 年 1 月 1 日，这些区域包括：美洲 8 个，欧洲 2 个，亚太地区 6 个，中国大陆 2 个，印度 3 个。如需要所有 Azure 区域的完整列表，请参阅**[此处](https://azure.microsoft.com/regions/)**维护的现有和新宣布的区域列表。
...
## 可用性和 Azure SLA
为了使部署符合 99.95 的 VM 服务级别协议，必须部署两个或更多个在可用性集中运行工作负荷的 VM。这可确保 VM 分布在我们数据中心内的多个容错域，并使用不同的维护时段部署到主机。有关 SLA 的完整详细信息，可以查看此**[在线文章](https://azure.microsoft.com/support/legal/sla/virtual-machines/v1_0/)**。
...
注意：DS 系列和 GS 系列 VM 可以访问高级存储 - 适用于 I/O 密集型工作负荷的以 SSD 为后盾的高性能低延迟存储。高级存储只在某些区域可用。有关详细信息，请参阅 **[Premium Storage: High-performance storage for Azure virtual machine workloads（高级存储：适用于 Azure 虚拟机工作负荷的高性能存储）](/documentation/articles/storage-premium-storage)**。
