...
> [AZURE.WARNING]此过程将删除原始虚拟机（在捕获它后），它不应作为备份虚拟机的方法。执行此操作的一个可行方法是 Azure 备份，它在特定区域中作为预览版提供。有关详细信息，请参阅[备份 Azure 虚拟机](/documentation/articles/backup/backup-azure-vms)。认证合作伙伴提供了其他解决方案。若要了解当前提供的内容，请搜索 Azure 应用商店。
