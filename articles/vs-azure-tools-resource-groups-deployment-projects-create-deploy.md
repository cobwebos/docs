...
本文是使用 Visual Studio 2015 Update 2 和 Microsoft Azure SDK for .NET 2.9 编写的。如果你将 Visual Studio 2013 与 Azure SDK 2.9 配合使用，你的体验将大体相同。你可以使用从 2.6 或更高版本开始的 Azure SDK 版本；但是，你的体验可能会不同于本文所示内容。强烈建议你先安装最新版本的 [Azure SDK](/downloads/)，然后再开始执行步骤。
...
**storageType** 参数是使用允许的类型和默认类型预定义的。你可以保留或根据方案编辑这些值。如果你不想要允许任何人通过此模板部署 **Premium\_LRS** 存储帐户，只需将它从允许的类型中删除即可，如下所示。
...
1. 创建 Web 应用之后，将资源组项目中的引用添加到 Web 应用项目。
    ...
    - “其他属性”包含将要推送到 Azure 存储空间的 Web 部署包暂存位置。
    - “包含文件路径”包含创建包所在的路径。“包含目标”包含部署将要执行的命令。
...
1. 将资源组项目重新部署到资源组。这次还有一些新的参数。不需要为 **\_artifactsLocation** 或 **\_artifactsLocationSasToken** 提供值，因为它们是自动生成的。将文件夹和文件名称设置为包含部署包的路径。
