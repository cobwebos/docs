<properties
   pageTitle="创建和部署 Azure 资源组 Visual Studio 项目 | Microsoft Azure"
...
1. 在 Visual Studio 中，选择“文件”、“新建项目”，再选择“C#”或“Visual Basic”。然后选择“云”，再选择“Azure 资源组”项目。
    ...
    ![云部署项目](./media/vs-azure-tools-resource-groups-deployment-projects-create-deploy/IC796668.png)
...
1. 选择要部署到 Azure Resource Manager 的模板。可以看到，系统根据你要部署的项目类型提供了许多不同的选项。对于本主题，我们将选择“Web 应用 + SQL”模板。
    ...
    >[AZURE.NOTE] 可用模板列表可以在线检索，并且可能会更改。

    Visual Studio 将创建 Web 应用和 SQL 数据库的资源组部署项目。

1. 展开部署项目中的节点可查看创建的内容。
    ...
    |文件名|说明|
    |---|---|
    |Deploy-AzureResourceGroup.ps1|一个 PowerShell 脚本，用于调用 PowerShell 命令来部署到 Azure Resource Manager。<br />注意：Visual Studio 使用此 PowerShell 脚本来部署你的模板。对此脚本进行任何更改也会影响 Visual Studio 中的部署，因此请务必小心。|
    |WebSiteSQLDatabase.json|Resource Manager 模板，定义你要部署到 Azure 的基础结构，以及在部署期间可以提供的参数。它还定义各资源之间的依赖关系，以便按正确的顺序部署资源。|
    |WebSiteSQLDatabase.parameters.json|包含模板所需值的参数文件。你需要传入这些值来自定义每个部署。|
    |AzCopy.exe|一个工具，PowerShell 脚本使用它来将文件从本地存储置放路径复制到存储帐户容器。仅当你在配置部署项目以部署代码与模板时才使用此工具。|

    所有资源组部署项目都包含这四个基本文件。其他项目可能包含其他文件以支持其他功能。

## 自定义 Resource Manager 模板

可以通过修改 JSON 模板（描述要部署的资源）来自定义部署项目。JSON 是“JavaScript 对象表示法”的缩写，是一种易于使用的序列化数据格式。JSON 文件使用在每个文件顶部引用的架构。如果想要更好地了解该架构，可以下载并分析它。架构定义允许哪些元素、字段的类型和格式、可能的枚举值，等等。若要了解 Resource Manager 模板的元素，请参阅 [Authoring Azure Resource Manager templates](resource-group-authoring-templates.md)（创作 Azure Resource Manager 模板）。

若要使用模板，请打开 **WebSiteSQLDatabase.json**。

Visual Studio 编辑器提供了工具来帮助你编辑 Resource Manager 模板。“JSON 大纲”窗口可让你轻松查看模板中定义的元素。
...
可以通过选择“JSON 大纲”窗口顶部的“添加资源”按钮，或右键单击“资源”然后选择“添加新资源”，将新资源添加到模板。
...
**WebSitePicturesType** 参数是使用允许的类型和默认类型预定义的。你可以保留或根据方案编辑这些值。如果你不想要允许任何人通过此模板部署 **Premium\_LRS** 存储帐户，只需将它从允许的类型中删除即可，如下所示。
...
Visual Studio 还提供了 Intellisense 来帮助你了解编辑模板时可以使用哪些属性。例如，若要编辑 App Service 计划的属性，请导航到 **HostingPlan** 资源，并为 **properties** 添加新值。请注意，Intellisense 显示可用的值，并提供该值的描述。
...
现在你已准备好部署项目。部署 Azure 资源组项目时，会将它部署到 Azure 资源组，该组只是 Azure 中的资源（例如 Web 应用、数据库，等等）的逻辑分组。

1. 在部署项目节点的快捷菜单中，选择“部署”>“新建部署”。

    ![“部署”>“新建部署”菜单项](./media/vs-azure-tools-resource-groups-deployment-projects-create-deploy/IC796672.png)

    此时将显示“部署到资源组”对话框。

    ![部署到资源组对话框](./media/vs-azure-tools-resource-groups-deployment-projects-create-deploy/show-deployment.png)

1. 在“资源组”下拉框中，选择现有资源组或创建新组。若要创建资源组，请打开“资源组”下拉框，然后选择 **<Create New...>**。
...
1. 可以选择“编辑参数”按钮以编辑部署的参数。提供参数的值，然后选择“保存”按钮。
    ...
    “保存密码”选项表示密码将在 JSON 文件中以纯文本保存。此选项并不安全。

1. 选择“部署”按钮将项目部署到 Azure。可以在“输出”窗口中查看部署的进度。部署可能需要几分钟才能完成，具体取决于配置。如果出现提示，请输入数据库管理员密码。

    >[AZURE.NOTE] 系统可能会要求你安装 Azure PowerShell cmdlet。由于要有这些 cmdlet 才能部署 Azure 资源组，因此需要安装。
...
1. 如果你做了更改并想要重新部署项目，可以直接从 Azure 资源组项目的快捷菜单选择现有资源组。在快捷菜单中，选择“部署”，然后选择刚刚部署到的资源组。
...
1. 创建 Web 应用之后，将资源组项目中的引用添加到 Web 应用项目。

    ![添加引用](./media/vs-azure-tools-resource-groups-deployment-projects-create-deploy/add-reference.png)
    
    通过添加引用，可以将 Web 应用项目链接到资源组项目中，并设置三个重要属性。“其他属性”包含将要推送到 Azure 存储空间的 Web 部署包过渡位置。“包含文件路径”包含创建包所在的路径。“包含目标”包含部署将要执行的命令。默认值“生成并打包”可让部署生成并创建 Web 部署包 (package.zip)。不需要使用发布配置文件，因为部署将从属性中获取所需的信息来创建包。
    
      ![查看引用](./media/vs-azure-tools-resource-groups-deployment-projects-create-deploy/see-reference.png)
      
1. 将新资源添加模板，这次请选择“适用于 Web 应用的 Web 部署”。

    ![添加 Web 部署](./media/vs-azure-tools-resource-groups-deployment-projects-create-deploy/add-web-deploy.png)
    
1. 将资源组项目重新部署到资源组。这次还有一些新的参数。不需要提供 **\_artifactsLocation** 或 **\_artifactsLocationSasToken** 的值，因为它们是自动生成的。将文件夹和文件名称设置为包含部署包的路径。
...
部署完成后，你可以浏览到站点，并留意是否已部署默认的 ASP.NET 应用。
...
- 有关模板的详细信息，请参阅 [Authoring Azure Resource Manager templates](/documentation/articles/resource-group-authoring-templates)（创作 Azure Resource Manager 模板）。
