...
简而言之，尽管有很多注意事项需要考虑到提供高度可缩放的服务，但对于许多多租户应用程序而言，还存在许多目标和要求。 有些要求在特定情况下可能并不相关，并且各个目标和要求的重要性在各种情况下也会有所不同。 作为多租户应用程序的提供商，你还将拥有目标和需求，如满足租户的目标和要求、盈利能力、计费、多个服务级别、配置、可维护性监视以及自动化。
...
数据管理（如 Azure SQL 数据库或 Azure 存储服务，如表服务），该服务提供用于存储大量非结构化数据的服务和 Blob 服务，提供用于存储大量非结构化文本或二进制数据，如视频、音频和图像。

* 保护 SQL 数据库中的多租户数据 SQL Server 登录名。
* 通过指定容器级别访问策略对应用程序资源使用 Azure 表，你可以调整权限，而无需为使用共享访问签名保护的资源颁发新 URL。
...
* Azure Active Directory (Azure AD) 是一项基于 REST 的新型服务，它可为你的云应用程序提供标识管理和访问控制功能。 通过对应用程序资源使用 Azure AD，可以轻松地对用户进行身份验证和授权，以获取对你的 web 应用程序和服务的访问权限，同时允许将身份验证和授权功能从代码中分离出来。
* Azure Service Bus 为分布式应用程序和混合应用程序提供了安全消息传送和数据流功能，例如在 Azure 托管的应用程序与本地应用程序和服务之间进行通信，而不需要复杂的防火墙和安全基础结构。 对应用程序资源使用服务总线中继来访问作为终结点公开的服务可能属于租户（例如，托管在系统之外，如本地），也可能是专门为租户预配的服务（因为敏感的、特定于租户的数据跨它们传输。

**预配资源**

Azure 提供了许多方法来为应用程序预配新租户。 对于具有大量租户的多租户应用程序，通常必须通过启用自助服务配置来自动化此过程。
