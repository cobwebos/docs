...
ms.locfileid: "56961476"
---
# <a name="quickstart-create-an-aspnet-core-app-with-azure-app-configuration"></a>快速入门：使用 Azure 应用配置创建 ASP.NET Core 应用

Azure 应用配置是 Azure 中的托管配置服务。 通过它可以在与代码分离的一个位置轻松存储和管理所有应用程序设置。 本快速入门将介绍如何将服务合并到 ASP.NET Core web 应用中。 ASP.NET Core 使用一个或多个数据源的设置，生成基于单个键值的配置对象，称为配置提供程序，由应用程序指定。 由于应用配置的 .NET Core 客户端作为此类提供程序实现，因此服务就像是另一个数据源。
...
你将使用 [.NET Core命令行接口 (CLI)](https://docs.microsoft.com/dotnet/core/tools/) 创建新的 ASP.NET Core MVC Web 应用项目。 通过 Visual Studio 使用 .NET Core CLI 的优点是，它可用于 Windows、macOS 和 Linux 平台。

1. 为项目新建一个文件夹。 在此快速入门中，我们将它命名为 TestAppConfig。
...
你将向项目添加[机密管理器工具](https://docs.microsoft.com/aspnet/core/security/app-secrets)。 机密管理器工具存储敏感数据，以便用于项目树之外的开发工作。 此方法有助于防止意外共享源代码中的应用密码。
...
3. 将名为 ConnectionStrings:AppConfig 的机密添加到机密管理器。
    ...
    此机密将包含用于访问应用配置存储区的连接字符串。 将以下命令中的值替换为应用配置存储区的连接字符串。
    ...
    机密管理器将仅用于在本地测试 web 应用。 应用部署（例如部署到 [Azure 应用服务](https://azure.microsoft.com/services/app-service/web)）后，你将使用应用程序设置（例如应用服务中的连接字符串），而不会使用机密管理器存储连接字符串。
...
4. 打开 Program.cs 并更新 `CreateWebHostBuilder` 方法以通过调用 `config.AddAzureAppConfiguration()` 方法使用应用配置。
...
在本快速入门中，你已经创建了一个新的应用配置存储区，并将其用于 ASP.NET Core web 应用。 要了解有关使用应用配置的详细信息，请继续阅读下一个演示身份验证的教程。
