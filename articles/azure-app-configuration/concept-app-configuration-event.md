...
Azure 应用配置事件将发送到 Azure 事件网格，该网格通过丰富的重试策略和死信传递为应用程序提供可靠的传送服务。 若要了解详细信息，请参阅[事件网格消息传递和重试](https://docs.microsoft.com/azure/event-grid/delivery-and-retry)。
...
事件网格使用[事件订阅](../event-grid/concepts.md#event-subscriptions)将事件消息路由到订阅服务器。 Azure 应用配置事件订阅可能包含两种类型的事件：  
