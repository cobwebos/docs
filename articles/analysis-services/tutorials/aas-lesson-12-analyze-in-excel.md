...
本课中，将使用“在 Excel 中分析”功能打开 Microsoft Excel，自动创建到模型工作区的连接，并自动向工作表添加数据透视表。 “在 Excel 中分析”功能提供一种快速简单的方法在部署你的模型之前测试你的模型的功效。 本课中不执行任何数据分析。 本课的目的是让你（模型作者）熟悉可以用来测试模型设计的工具。   
...
本主题是表格建模教程的一部分，应当按顺序完成。 在执行本课中的任务之前，你应当已完成上一课：[第 11 课：创建角色](../tutorials/aas-lesson-11-create-roles.md)。  
  
## <a name="browse-using-the-default-and-internet-sales-perspectives"></a>使用默认透视和 Internet 销售透视进行浏览  
在这些首要任务中，你将使用默认透视（这包括所有模型对象）和“Internet 销售”透视浏览你之前创建的模型。 Internet 销售透视不包括 Customer 表对象。  
...
1.  单击“模型”菜单，然后单击“在 Excel 中分析”。  
  
2.  在“在 Excel 中分析”对话框中，使“当前 Windows 用户”保持选中状态，然后在“透视”下拉列表框中选择“Internet 销售”，然后单击“确定”。 
...
对任何表格模型而言，角色都是非常重要的部分。 如果没有一个可将用户作为其成员添加的角色，用户则无法使用模型访问和分析数据。 “在 Excel 中分析”功能为你提供了一种方法来测试已定义的角色。  
  
#### <a name="to-browse-by-using-the-sales-manager-user-role"></a>使用“销售经理”用户角色进行浏览  
  
1.  在 SSDT 中，单击“模型”菜单，然后单击“在 Excel 中分析”。  
