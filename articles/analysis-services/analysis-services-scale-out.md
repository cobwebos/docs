...
description: 使用横向扩展复制 Azure 分析服务服务器。然后，可以将客户端查询分布在横向扩展查询池中的多个查询副本之间。
author: minewiskan
...
通过横向扩展，客户端查询可以分布在*查询池*中的多个*查询副本*中，从而缩短高查询工作负载期间的响应时间。 也可将处理与查询池分开，确保客户端查询不受处理操作的负面影响。 可在 Azure 门户中配置横向扩展，也可使用 Analysis Services REST API 进行配置。
...
执行后续的横向扩展操作时（例如，将查询池中的副本数从两个增加到五个），新副本将与 Blob 存储中第二组文件内的数据合成。 不会发生同步。 如果在横向扩展后执行同步，则查询池中的新副本将水合两次 - 冗余水化。 执行后续的横向扩展操作时，请务必记住：
...
* 即使查询池中没有副本，也允许同步。 如果要使用主服务器上的处理操作中的新数据从零扩展到一个或多个副本，请首先在查询池中没有副本时执行同步，然后横向扩展。在横向扩展之前进行同步可避免新添加的副本的冗余水化。
...
默认情况下，查询副本将完全（而不是增量）重新水化。 补液分期分期进行。 它们一次分离并附加两个副本（假设至少有三个副本），以确保在任何给定时间至少有一个副本保持联机以查询。 在某些情况下，客户端可能需要在此过程进行时重新连接到其中一个联机副本。 通过使用（预览版）**副本同步模式**设置，现在可以指定并行发生的查询副本同步。 并行同步提供以下好处： 

- 显著缩短同步时间。 
- 在同步过程中，跨副本的数据更有可能是一致的。 
- 由于在整个同步过程中，所有副本上的数据库都保持联机状态，因此客户端不需要重新连接。 
- 内存中缓存仅使用更改的数据进行增量更新，这些数据可能比完全重新填充模型更快。 

#### <a name="setting-replicasyncmode"></a>设置副本同步模式

使用 SSMS 在高级属性中设置副本同步模式。 可能的值包括： 

- `1`（默认）：分阶段（增量）完全副本数据库补液。 
- `2`：并行优化同步。 

![RelicaSyncMode 设置](media/analysis-services-scale-out/aas-scale-out-sync-mode.png)

设置**ReplicaSyncMode_2**时，根据需要更新的缓存量，查询副本可能会占用额外的内存。 为了保持数据库联机且可用于查询，根据更改的数据量，该操作可能需要副本上的*内存增加一倍*，因为新旧段同时保存在内存中。 副本节点的内存分配与主节点相同，并且主节点上通常有额外的内存用于刷新操作，因此副本的内存可能不足。 此外，常见的方案是数据库在主节点上以增量方式更新，因此对内存加倍的要求应该并不常见。 如果 Sync 操作确实遇到内存不足错误，它将使用默认技术重试（一次附加/分离两个）。 
...
要确定是否需要为服务器横向扩展，请使用指标监视 Azure 门户中的[服务器](analysis-services-monitor.md)。 如果 QPU 经常超过上限，则表示针对模型的查询数量超出了计划的 QPU 限制。 查询线程池队列中的查询数量超过可用的 QPU 时，查询池作业队列长度指标也会增加。 

可监视的另一个很好指标是按 ServerResourceType 列出的平均 QPU。 此指标将主服务器的平均 QPU 与查询池进行比较。 
...
使用 Azure 监视器日志对已缩小的服务器资源进行更详细的诊断。 使用日志，可以使用日志分析查询按服务器和副本分解 QPU 和内存。 要了解更多信息，请参阅[分析服务诊断日志记录](analysis-services-logging.md#example-queries)中的示例查询。
...
对于最终用户客户端连接（如 Power BI 桌面、Excel 和自定义应用），请使用**服务器名称**。 
...
## <a name="scale-up-scale-down-vs-scale-out"></a>放大缩小字体功能 放大缩小字体功能
...
**解决方案：** 选择"**将处理服务器从查询池**"选项中分离出来时，使用默认连接字符串（不 ） 的`:rw`客户端连接将重定向到查询池副本。 如果查询池中的副本因尚未完成同步而尚未联机，则重定向的客户端连接可能会失败。 若要防止连接失败，执行同步时查询池中必须至少有两个服务器。 每个服务器单独同步，而其他服务器保持联机。 如果在处理期间选择在查询池中没有处理服务器，则可以选择将其从池中删除以进行处理，然后在处理完成后但在同步之前将其添加回池中。 可以使用内存和 QPU 指标来监视同步状态。
