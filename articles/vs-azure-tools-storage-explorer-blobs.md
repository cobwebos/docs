...
[Azure Blob 存储](storage/blobs/storage-dotnet-how-to-use-blobs.md)是一种服务，用于存储大量非结构化数据（如文本或二进制数据），这些数据可通过 HTTP 或 HTTPS 从世界任何地方访问。
...
3. 右键单击“Blob 容器”，并从上下文菜单中选择“创建 Blob 容器”。********
   ...
   ![“创建 Blob 容器”上下文菜单][0]
4. 此时会在“Blob 容器”文件夹下显示一个文本框。**** 输入 Blob 容器的名称。 有关命名 blob 容器的规则和限制的信息，请参阅[创建容器](storage/blobs/storage-quickstart-blobs-dotnet.md#create-a-container)。
...
[共享访问签名 （SAS）](storage/common/storage-dotnet-shared-access-signature-part-1.md)提供对存储帐户中资源的委派访问权限。
