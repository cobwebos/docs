...
2.  单击“通知”**** 即可查看可用的通知。
    ...
    -   **接近订阅配额限制** - 以下电子邮件收件人和用户会在订阅使用量接近使用量配额时收到电子邮件通知。
        ...
        > [!NOTE]
        > 通知仅[由订阅策略的配额](api-management-access-restriction-policies.md#SetUsageQuota)触发。 [按键策略的配额](api-management-access-restriction-policies.md#SetUsageQuotaByKey)不会生成通知。
