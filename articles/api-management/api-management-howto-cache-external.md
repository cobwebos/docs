...
## <a name="create-azure-cache-for-redis"></a><a name="create-cache"> </a>为雷瑞斯创建 Azure 缓存
...
> [!NOTE]
> **"使用从**"设置指定在 API 管理进行多区域配置的情况下，哪个 API 管理区域部署将与配置的缓存通信。 指定为**默认**的缓存将替代为具有区域值的缓存。
>
...
5. 选择 **"默认"** 或在"从下拉列表中**使用"** 字段中指定所需区域。
6. 单击“保存”。****
