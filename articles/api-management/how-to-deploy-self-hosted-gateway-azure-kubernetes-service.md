...
本文提供将 Azure API 管理的自承载网关组件部署到[Azure Kubernetes 服务](https://azure.microsoft.com/services/kubernetes-service/)的步骤。 有关将自承载网关部署到 Kubernetes 群集的详细说明，请参阅此[文档](how-to-deploy-self-hosted-gateway-kubernetes.md)。
...
```
11. 执行命令。 命令指示 AKS 群集运行容器，使用从 Microsoft 容器注册表下载的自承载网关的映像，并将该容器配置为公开 HTTP （8080）和 HTTPS （443）端口。
...
```
13. 运行以下命令来检查网关服务是否正在运行。 请注意，你的服务名称和 IP 地址将不同。
```console
...
* 了解[如何在云中配置和保存日志](how-to-configure-cloud-metrics-logs.md)
* * 了解[如何在本地配置和保存日志](how-to-configure-local-metrics-logs.md)
