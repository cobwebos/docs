...
| 角色          | 读取访问权限<sup>[1]</sup> | 写入访问权限<sup>[2]</sup> | 服务创建、删除、缩放，VPN 和自定义域配置 | 对旧版发布者门户拥有访问权限 | Description
| ------------- | ---- | ---- | ---- | ---- | ---- 
...
<sup>将所有管理 UI 从现有发布者门户迁移到 Azure 门户后，将提供 \* 服务编辑器角色。将发布者门户重构为仅包含与管理开发人员门户相关的功能后，将提供 "内容管理员" 角色。</sup>  
...
> [!NOTE]
> 为了能够在 Azure 门户中查看 API 管理实例，自定义角色必须包括 ```Microsoft.ApiManagement/service/read``` 操作。
