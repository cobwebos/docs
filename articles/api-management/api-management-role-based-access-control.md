...
API 管理目前提供了三个内置角色，不久之后会再添加两个角色。 可在不同的范围（包括订阅、资源组和单个 API 管理实例）分配这些角色。 例如，如果将"API 管理服务读取器"角色分配给资源组级别的用户，则用户具有对资源组内所有 API 管理实例的读取访问权限。 
...
| API 管理服务运营商 | ✓ | | ✓ | | 可以管理 API 管理服务，但不能管理实例。|
| API 管理服务编辑器<sup>*</sup> | ✓ | ✓ | |  | 可以管理 API 管理实体，但不能管理服务。|
| API 管理内容管理器<sup>*</sup> | ✓ | | | ✓ | 可以管理开发人员门户。 对服务和实体拥有只读访问权限。|

<sup>[1] 对 API 管理服务和实体（例如，API 和策略）拥有读取访问权限。</sup>

<sup>[2] 对 API 管理服务和实体拥有写入访问权限，但以下操作除外：实例创建、删除和缩放；VPN 配置；以及自定义域设置。</sup>

<sup>\*将所有管理员 UI 从现有发布者门户迁移到 Azure 门户后，服务编辑器角色将可用。内容管理器角色将在重构发布者门户后可用，以仅包含与管理开发人员门户相关的功能。</sup>  
...
创建自定义角色时，从某个内置角色着手会更为轻松。 编辑属性以添加 **Actions**、**NotActions** 或 **AssignableScopes**，然后将所做的更改保存为新角色。 以下示例从"API 管理服务读取器"角色开始，并创建一个名为"计算器 API 编辑器"的自定义角色。 可以将自定义角色分配给特定的 API。 因此，此角色仅有权访问该 API。 
...
若要详细了解 Azure 中基于角色的访问控制，请参阅以下文章：
  ...
  * [Azure 资源管理器资源管理器提供程序操作](../role-based-access-control/resource-provider-operations.md#microsoftapimanagement)
