---
title: "如何使用 Azure Active Directory 和 API 管理保护 Web API 后端"
...
此步骤使用 Visual Studio 2013 创建 Web API 后端。 视频中的此步骤从 1:30 开始。 若要在 Visual Studio 中创建 Web API 后端项目，单击“文件”->“新建”->“项目”，然后从“Web”模板列表中选择“ASP.NET Web 应用程序”。 在本视频中，此项目的名称为 **APIMAADDemo**。  。 
...
在“更改身份验证”对话框中配置所需设置，然后单击“确定”。

![更改身份验证][api-management-change-authentication]

单击“确定”后，Visual Studio 将尝试向 Azure AD 目录注册你的应用程序，并且系统可能会提示通过 Visual Studio 登录。 使用目录的管理帐户登录。

![登录 Visual Studio][api-management-sign-in-vidual-studio]

若要将此项目配置为 Azure Web API，请选中“在云中托管”，然后单击“确定”。

![新建项目][api-management-new-project-cloud]

系统可能会提示你需要登录 Azure 才能配置 Web 应用。
...
此示例中的 Web API 使用模型和控制器实现基本计算器服务。 若要为服务添加模型，请右键单击“解决方案资源管理器”中的“模型”，然后依次选择“添加”、“类”。 将类命名为 `CalcInput`，然后单击“添加”。
...
右键单击“解决方案资源管理器”中的“Controllers”文件夹，然后选择“添加”->“控制器”。 选择“Web API 2 控制器 - 空”，然后单击“添加”。 键入 **CalcController** 用作控制器名称，然后单击“添加”。
...
要将项目发布到 Azure，请右键单击 Visual Studio 中的“APIMAADDemo”项目，然后选择“发布”。 保留“发布 Web”对话框中的默认设置，然后单击“发布”。
...
在配置和发布 Web API 项目的过程中，将在 Azure AD 目录中创建适用于后端服务的新应用程序。 此步骤将向 Web API 后端授予权限，在视频中从 6:13 开始。
...
在 API 发布者门户（通过 Azure 门户访问）中配置 API。 若要访问，请单击 API 管理服务工具栏中的“发布者门户”。 如果尚未创建 API 管理服务实例，请参阅[管理第一个 API][管理第一个 API] 教程中的[创建 API 管理服务实例][创建 API 管理服务实例]。

![发布者门户][api-management-management-console]

可将操作[手动添加到 API](api-management-howto-add-operations.md)，也可以导入操作。 在此视频中，将以 Swagger 格式导入操作，从 6:40 开始。
...
通过本视频的第一步导航到 Azure AD 租户（此示例中的租户为“APIMDemo”），然后导航到“应用程序”选项卡。
...
选择“Web 应用程序和/或 Web API”，输入一个名称，然后单击下一步箭头。 此示例中使用“APIMDeveloperPortal”。

![新建应用程序][api-management-aad-new-application-devportal-1]

对于“登录 URL”，请输入 API 管理服务的 URL，然后追加 `/signin`。 此示例中使用“https://contoso5.portal.azure-api.net/signin”。

对于“应用 ID URL”，请输入 API 管理服务的 URL，然后追加某些唯一字符。 可根据需要输入任何字符，此示例中使用“https://contoso5.portal.azure-api.net/dp”。 配置所需“应用属性”后，单击复选标记创建应用程序。
...
可从为开发人员门户创建的 AAD 应用程序的“应用程序终结点”页面检索这些值。 若要访问终结点，请导航到 AAD 应用程序的“配置”选项卡，然后单击“查看终结点”。
...
若要获取“客户端 ID”，请导航到用于开发人员门户的 AAD 应用程序的“配置”选项卡，然后复制“客户端 ID”。

若要获取“客户端机密”，请单击“密钥”部分中的“选择持续时间”下拉列表，然后指定时间间隔。 此示例中使用 1 年。
...
下一步是配置开发人员门户 AAD 应用程序的权限。 单击“应用程序权限”并选中“读取目录数据”框。 单击“保存”保存此更改，然后单击“添加应用程序”。

![添加权限][api-management-add-devportal-permissions]

单击搜索图标，在框的开头键入“APIM”，选择“APIMAADDemo”，然后单击复选标记保存。

![添加权限][api-management-aad-add-app-permissions]

单击“APIMAADDemo”的“委派权限”，选中“访问 APIMAADDemo”框，然后单击“保存”。 这时开发人员门户应用程序便可访问后端服务。
...
单击左侧菜单中的“API”，然后单击“计算器”查看和配置其设置。

![计算器 API][api-management-calc-api]

导航到“安全”选项卡，选中“OAuth 2.0”复选框，从“授权服务器”下拉列表中选择所需的授权服务器，然后单击“保存”。
...
在开发人员门户中，导回到计算器服务的“添加两个整数”操作，然后单击“试用”。 请注意，“授权”部分中的新项与刚添加的授权服务器对应。
...
单击“发送”，然后记下**响应状态**“200 正常”和响应内容中的操作结果。
...
* 有关保护后端服务的其他方法，请参阅[使用证书进行相互身份验证](api-management-howto-mutual-certificates.md)和[通过 VPN 或 ExpressRoute 连接](api-management-howto-setup-vpn.md)。
