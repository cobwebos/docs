...
后端 API 导入到 API 管理 (APIM) 之后，APIM API 即成为后端 API 的外观。 在导入后端 API 时，源 API 和 APIM API 均相同。 通过 APIM，无需触及后端 API 即可根据需要自定义外观。 有关详细信息，请参阅[转换和保护 API](transform-api.md)。 
...
2. 从该列表中选择“OpenAPI 规范”，然后在弹出窗口中单击“完整”   。
    ...
    | **API URL 后缀**        | conference                                        | 此后缀附加到 API 管理服务的基础 URL。 API 管理通过其后缀区分 API，因此后缀对给定发布者上的每个 API 必须唯一。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
    | **产品**              | *无限制*                                        | 产品是一个或多个 API 的关联。 可在一个产品中包含多个 API，并通过开发人员门户将其提供给开发人员。 <br/>通过将 API 关联到某个产品（在本示例中为“无限制”）来发布该 API。  若要将此新 API 添加到产品，请键入产品名称（也可以稍后通过“设置”页执行此操作）。  多次重复此步骤以将 API 添加到多个产品。<br/>开发人员必须先订阅产品才能访问 API。 订阅时，他们会得到一个订阅密钥，此密钥对该产品中的任何 API 都有效。 <br/> 如果你创建了 APIM 实例，那么你已是管理员，因此订阅了每个产品。<br/> 默认情况下，每个 API 管理实例附带两个示例产品：**初学者**和**无限制**。 |
...
2. 按“测试”选项卡  。
3. 单击“GetSpeakers”  。 该页显示查询参数的字段（在此示例中为“无”），以及标头。 其中一个标头是“Ocp-Apim-Subscription-Key”，用于提供和此 API 关联的产品订阅密钥。 该密钥将被自动填充。
4. 按“发送”。 
