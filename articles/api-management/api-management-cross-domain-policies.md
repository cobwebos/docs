...
|名称|说明|必需|默认|
|----------|-----------------|--------------|-------------|
|allow-credentials|预检`Access-Control-Allow-Credentials`响应中的标头将设置为此属性的值，并且会影响客户端在跨域请求中提交凭据的能力。|否|false|
|preflight-result-max-age|预检`Access-Control-Max-Age`响应中的标头将设置为此属性的值，并且会影响用户代理缓存预处理响应的能力。|否|0|
