...
|allowed-methods|如果允许 GET 或 POST 之外的方法，则此元素是必需的。 包含 `method` 元素，用于指定支持的 HTTP 谓词。 该值`*`指示所有方法。|否|如果此部分不存在，则支持 GET 和 POST。|
...
|“属性”|描述|必选|默认|
|----------|-----------------|--------------|-------------|
|allow-credentials|预`Access-Control-Allow-Credentials`检响应中的标头将设置为此属性的值，并影响客户端在跨域请求中提交凭据的能力。|否|false|
|preflight-result-max-age|预`Access-Control-Max-Age`检响应中的标头将设置为此属性的值，并影响用户代理缓存飞行前响应的能力。|否|0|
