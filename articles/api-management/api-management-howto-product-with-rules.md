<properties
	pageTitle="使用 Azure API 管理保护 API | Microsoft Azure"
...
>[AZURE.NOTE] 如果已配置产品并想要在本教程中使用它，可以跳转到[配置调用速率限制和配额策略][]，并按照那里的教程使用产品来代替免费试用版产品。

若要开始，请在 Azure 经典门户中单击 API 管理服务所对应的“管理”。这将转到 API 管理发布者门户。

![发布者门户][api-management-management-console]

>如果尚未创建 API 管理服务实例，请参阅[在 Azure API 管理中管理第一个 API][] 教程中的[创建 API 管理服务实例][]。
...
>在 API 管理中，使用组来管理产品对开发人员的可见性。产品向组授予可见性，并且开发人员可以查看和订阅对他们所属的组可见的产品。有关详细信息，请参阅[如何创建和使用 Azure API 管理中的组][]。
...
>每个预先配置 Echo API 的 API 管理服务实例，都可用于试验和了解 API 管理。有关详细信息，请参阅[在 Azure API 管理中管理第一个 API][]。
...
>[AZURE.NOTE] 在本教程中，未为“免费试用版”产品启用“多个同时订阅”。如果是这样，会提示用户命名该订阅，如下面的示例所示。
...
>[AZURE.NOTE] 如果有多个订阅，请务必选择**免费试用版**的密钥，否则在前面步骤中配置的策略不会生效。
