...
若要从影响托管着 API 管理服务的区域的可用性问题中恢复，应随时准备好在另一区域中重建服务。 具体取决于恢复时间目标，可能想要保留备用服务在一个或多个区域中。 你可能尝试以维护其配置和内容与活动服务根据恢复点目标保持同步。 服务备份和还原功能为实现灾难恢复策略提供必要的构建基块。

备份和还原操作还可用于运行环境，例如开发和暂存之间复制 API 管理服务配置。 请注意该运行时数据，如用户和订阅将被复制，这可能并不合适。

本指南演示如何自动备份和还原操作以及如何确保成功进行身份验证的备份和还原请求由 Azure 资源管理器。

> [!IMPORTANT]
> 还原操作不会更改目标服务的自定义主机名配置。 我们建议使用相同的自定义主机名和 TLS 证书对于活动和备用服务，以便还原操作完成后，可以重定向到备用实例通过简单的 DNS CNAME 更改流量。
>
> 备份操作不会捕获在 Azure 门户中的分析边栏选项卡上显示的报表中使用预先聚合的日志数据。
...
-   `api-version` -将替换为 `2018-06-01-preview`
