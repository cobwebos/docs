...
* [将内部 VNET 中的 API 管理与应用程序网关集成](./api-management-howto-integrate-internal-vnet-appgateway.md)
...
**指南**：可以使用网络安全组 (NSG) 控制其中部署了 API 管理的子网的入站和出站流量。 将 NSG 部署到 API 管理子网，然后启用 NSG 流日志，并将日志发送到 Azure 存储帐户以进行流量审核。 还可以将 NSG 流日志发送到 Log Analytics 工作区，并使用流量分析来提供对 Azure 云中的流量流的见解。 流量分析的优势包括能够可视化网络活动、识别热点、识别安全威胁、了解流量流模式，以及查明网络不当配置。
...
* [如何将内部 VNET 中的 API 管理与应用程序网关集成](./api-management-howto-integrate-internal-vnet-appgateway.md)
...
* [如何将专用 API 公开给外部使用者](/azure/architecture/example-scenario/apps/publish-internal-apis-externally)

* [如何使用 Vnet 中的 API 管理](./api-management-using-with-vnet.md)
...
**指南**：使用虚拟网络 (Vnet) 服务标记在 API 管理子网中使用的网络安全组 (NSG) 上定义网络访问控制。 创建安全规则时，可以使用服务标记代替特定的 IP 地址。 在规则的相应源或目标字段中指定服务标记名称（例如 ApiManagement），可以允许或拒绝相应服务的流量。 Microsoft 会管理服务标记包含的地址前缀，并会在地址发生更改时自动更新服务标记。
...
**指南**：使用 Azure 活动日志监视网络资源配置，并检测与 Azure API 管理部署相关的网络资源的更改。 在 Azure Monitor 中创建当关键网络资源发生更改时触发的警报。
...
**指导**：在 Azure Monitor 中，根据组织的合规性规章设置 Log Analytics 工作区保留期。 将 Azure 存储帐户用于长期/存档存储。
...
* [如何监视和查看 Azure API 管理的日志](./api-management-howto-use-azure-monitor.md)
...
* [如何使用 PowerShell 获取 Azure AD 中分配给目录角色的用户的列表](/powershell/module/az.resources/get-azroleassignment?view=azps-3.7.0)

* [如何使用 PowerShell 获取 Azure AD 中的目录角色定义](/powershell/module/az.resources/get-azroledefinition?view=azps-3.7.0)

* [了解 Azure 安全中心的标识和访问建议](../security-center/recommendations-reference.md#recs-identity)
...
**指南**：尽可能使用 Azure AD 作为中央身份验证和授权系统。 AAD 通过对静态数据和传输中数据使用强加密来保护数据。 Azure AD 还会对用户凭据进行加盐、哈希处理和安全存储操作。
...
* [如何结合使用 OAuth 2.0 和 Azure Active Directory 与 API 管理来保护 API](./api-management-howto-protect-backend-with-aad.md)
...
* [如何获取 API 管理用户列表](/powershell/module/az.apimanagement/get-azapimanagementuser?view=azps-3.1.0)
...
**指导**：为开发、测试和生产实施单独的订阅和/或管理组。 Azure API 管理实例应由虚拟网络 (VNet)/子网进行分隔并适当标记。
...
* [管理 Azure API 管理中的 TLS 设置](./api-management-howto-manage-protocols-ciphers.md)

* [使用 Azure Active Directory 保护 Azure API 管理中的 API](./api-management-howto-protect-backend-with-aad.md)

* [使用 Azure Active Directory B2C 保护 Azure API 管理中的 API](./howto-protect-backend-frontend-azure-ad-b2c.md)
...
* [了解 Azure API 管理中的数据保护/静态加密](./api-management-security-controls.md#data-protection)
...
* [如何借助 Key Vault 密钥更新指南设置自定义域名](./configure-custom-domain.md)
...
* [有关 Blob 存储的安全建议](../storage/blobs/security-recommendations.md)

在 Key Vault 中启用“软删除”，以防止意外删除或恶意删除密钥。

* [如何启用 Key Vault 中的软删除](../storage/blobs/soft-delete-overview.md?tabs=azure-portal)
