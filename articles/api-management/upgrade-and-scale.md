...
每个单位的容量和价格取决于该单位所在的“层”  。 可在四个层之间选择：“开发人员”、“基本”、“标准”和“高级”     。 如果需要提高层中服务的容量，应添加单位。 如果当前在 API 管理实例中选择的层不允许添加更多的单元，则需要升级到更高级别的层。

每个单位的价格和可用功能（例如多区域部署）取决于你为 API 管理实例选择的层。 [定价详细信息](https://azure.microsoft.com/pricing/details/api-management/?ref=microsoft.com&utm_source=microsoft.com&utm_medium=docs&utm_campaign=visualstudio)一文介绍了每个单位的价格，以及每个层提供的功能。 
...
可以在四个层之间选择： "**开发人员**"、"**基本**"、"**标准**" 和 "**高级**"。 应将“开发人员”层用于评估服务；不应将其用于生产  。 “开发人员”层不附带 SLA，无法缩放此层（添加/删除单位）  。 

"**基本**"、"**标准**" 和 "**高级**" 是具有 SLA 和可缩放的生产层。 **基本**层是具有 SLA 的最便宜层，可将其扩展到两个单位，**标准**层最多可以扩展到四个单位。 可将任意数目的单位添加到“高级”层  。
...
可在任意两个层之间来回升级和降级。 升级或降级可能会删除某些功能，例如，从 "高级" 层降级到标准版或基本版时，会删除 Vnet 或多区域部署。
...
![Azure 门户中的规模管理 API 管理服务](./media/upgrade-and-scale/portal-scale.png)

1. 在[Azure 门户](https://portal.azure.com/)中导航到 API 管理服务。
2. 从菜单中选择 "**位置**"。
3. 单击要缩放的位置所在的行。
4. 指定新的**单位**数-使用滑块或键入数字。
5. 单击“应用”  。
...
1. 在[Azure 门户](https://portal.azure.com/)中导航到 API 管理服务。
2. 单击菜单中的 "**定价层**"。
3. 从下拉列表中选择所需的服务层。 更改后，使用滑块指定 API 管理服务的规模。
4. 单击 **“保存”** 。
