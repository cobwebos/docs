...
4. 此窗体的下一部分包含“授权的授权类型”、“授权终结点 URL”和“授权请求方法”设置。
    ...
    输入“授权终结点 URL”。 对于 Azure Active Directory，此 URL 将类似于以下 URL，其中 `<tenant_id>` 替换为 Azure AD 租户的 ID。
...
配置 OAuth 2.0 授权服务器并将 API 配置为使用该服务器以后，即可转到开发人员门户并调用 API 对其进行测试。 单击 "Azure API 管理实例**概述**" 页顶部菜单中的 "**开发人员门户（旧）** "。
