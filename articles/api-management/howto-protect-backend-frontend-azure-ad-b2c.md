...
我们将使用 OpenID Connect 协议和 API 管理 Azure AD B2C 与 API 管理一起使用 EasyAuth 来保护 Azure Functions 后端。
...
1. 从应用程序列表中打开后端 API，并选择 "*密钥*" 选项卡（在 "常规" 下），然后单击 "生成密钥" 以生成身份验证密钥
...
1. 然后，在 "标识提供者" 下，选中 "用户 ID 注册"，然后单击 "确定"。 
...
1. 将后端 API 的应用程序 ID （从 Azure AD B2C 粘贴到 "客户端 ID" 框中）
1. 将众所周知的开放 id 配置终结点从 "注册" 或 "登录" 策略粘贴到 "颁发者 URL" 框中（前面已记录此配置）。
1. 将之前用于 Azure AD B2C 应用程序的三个（或两个）应用程序 Id 添加到允许的令牌受众，此设置会告诉 EasyAuth 在接收的令牌中允许哪些 AUD 声明值。
1. 选择 "确定"，然后单击 "保存"。

   > [!NOTE]
   > 现在，你的函数 API 已部署，应针对未经授权的请求引发401或403错误，并应在出现有效请求时返回数据。
   > 但我们仍没有 IP 安全，如果你拥有有效密钥，你可以从任何位置调用它-理想情况下，我们希望强制所有请求都通过 API 管理。
   > 此外，如果你使用的是 API 管理消耗层，则你将无法通过 VIP 执行此锁定，因为对于该层没有专用的静态 IP，你将需要依赖通过共享机密功能密钥锁定 API 调用的方法，因此不可能执行步骤11-14。
...
需要将 CIDR 格式的地址块添加到 "IP 限制" 面板。 如果需要添加单个地址（如 API 管理 VIP），则需要将其添加为 xx. xx/32。

   > [!NOTE]
   > 现在，不能从任何位置通过 API 管理或地址调用函数 API。

## <a name="import-the-function-app-definition"></a>导入函数应用定义
1. 打开 API 管理门户边栏选项卡，并选择 API 管理实例。
...
1. 切换回 Azure 门户中的标准 Azure AD 租户，以便我们可以重新配置订阅中的项目并打开*API 管理边栏选项卡*，然后打开*实例*。
...
1. 检查*隐式身份验证*授予类型，并选择性地取消选中 "授权代码授予类型"。
1. 转到 "*授权*" 和 "*令牌*终结点" 字段，然后输入先前从众所周知的配置 xml 文档中捕获的值。
1. 向下滚动，并使用来自 Azure AD B2C 应用注册的函数 API 客户端 ID 填充名为 "resource" 的*附加正文参数*
