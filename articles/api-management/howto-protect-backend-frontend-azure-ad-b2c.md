...
1. 现在使用回复 url 占位符https://localhost，如，稍后我们将更新这些 url。
...
5. 切换回 "HttpTrigger1" 选项卡，单击 "获取函数 URL"，然后复制显示的 URL。
   ...
   > [!NOTE]
   > 刚创建的绑定只是告诉函数对刚刚复制的 URL 的匿名 http GET 请求做出响应。 （`https://yourfunctionappname.azurewebsites.net/api/hello?code=secretkey`）现在我们有一个可伸缩的无服务器 https API，它能够返回非常简单的负载。
...
1. 首先，让我们配置身份验证/授权，然后单击 function app 的名称（"Z &lt;&gt;函数" 图标旁），以显示 "概述" 页。
...
1. 将 "回复 URL" 条目设置为之前在 API 管理中配置身份验证代码授予的 redirect_uri 时记下的条目。
   ...
   既然已在上启用 OAuth 2.0 用户授权`Echo API`，开发人员控制台在调用 API 之前，会为用户获取访问令牌。

1. 在开发人员门户中浏览`Echo API`到下的任一操作，并选择 "**试用**" 以使你转到开发人员控制台。
1. 请注意，"**授权**" 部分中的新项与刚添加的授权服务器对应。
1. 从 "授权" 下拉列表中选择 "**授权代码**"，系统将提示登录到 Azure AD 租户。 如果已在帐户中登录，则可能不会出现提示。
1. 成功登录后，会将一个`Authorization: Bearer`标头添加到请求，并使用 Azure AD B2C 以 Base64 编码的访问令牌。 
