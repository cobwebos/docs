...
| 名称     | 类型   | 描述                                                                                               | 需要 |
|----------|--------|-----------------------------------------------------------------------------------------------------------|----------|
| 源   | string | 指定在其中发生错误的元素。 可以是策略或内置管道步骤名称。     | 是      |
| 原因   | string | 计算机友好错误代码，可以用在错误处理中。                                       | 否       |
| 消息  | string | 用户可读的错误说明。                                                                         | 是      |
| 范围    | string | 在其中发生错误的范围的名称，可以是以下值之一：“全局”、“产品”、“API”、“操作” | 否       |
| 部分  | string | 发生错误的节名称。 可能的值：“inbound”、“backend”、“outbound”或“on-error”。       | 否       |
| 路径     | string | 指定嵌套策略，例如“choose[3]/when[2]”。                                                        | 否       |
