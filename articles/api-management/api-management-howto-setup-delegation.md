...
一开始需先将 API 管理设置为通过委派终结点路由请求。 在 Azure 门户中，在 API 管理资源中搜索“安全性”，然后单击“委派”项 。 单击复选框以启用“委派登录和注册”。
...
2. 浏览器将重定向到委派终结点。
3. 委派终结点执行所需的产品订阅步骤。 由你来设计这些步骤。 它们可能包括重定向到其他请求计费信息的页面，需要提问更多问题，或者只需存储信息而不需任何用户操作。
...
1. 接收以下形式的请求：
   ...
   * **subscriptionId**：在 Unsubscribe 和 Renew 上 - 产品订阅的 ID 
   * **userId**：请求所针对的用户的 ID
   * **salt**：用于计算安全哈希的特殊 salt 字符串
   * **sig**：计算的安全哈希，用于与用户自行计算的哈希进行比较
...
4. 在这一端成功为用户订阅产品以后，即可[为订阅调用 REST API] 为用户订阅 API 管理产品。

## <a name="example-code"></a><a name="delegate-example-code"> </a>示例代码

下面的代码示例演示如何执行以下操作：

* 获取委派验证密钥，该密钥在发布服务器门户的委派屏幕中设置
* 创建一个 HMAC，然后用于验证签名，以证明传递的 returnUrl 的有效性。
...
[为订阅调用 REST API]: https://docs.microsoft.com/rest/api/apimanagement/2019-12-01/subscription/createorupdate
