...
|名称|说明|必须|默认|  
|----------|-----------------|--------------|-------------|  
|resource|字符串。 目标 web API 的应用 ID (受保护的资源) Azure Active Directory 中。|是|不适用|
|client-id|字符串。 Azure Active Directory 中用户分配的标识的应用 ID。|不适合|系统分配的标识|
|output-token-variable-name|字符串。 上下文变量的名称，它将令牌值接收为对象类型 `string`。 |不适合|空值|  
