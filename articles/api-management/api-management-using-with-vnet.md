...
4. 选择所需的访问类型：
   ...
   * **内部**：只能通过内部负载均衡器从虚拟网络内部访问 API 管理网关和开发人员门户。 网关可以访问虚拟网络中的资源。
     ...
     > [!IMPORTANT]
     > 将 Azure API 管理实例部署到 Resource Manager VNET 时，该服务必须位于除了 Azure API 管理实例之外不包含其他资源的专用子网中。 如果尝试将 Azure API 管理实例部署到包含其他资源的资源管理器 VNET 子网，则部署会失败。
     >
...
* **在 VNET 内部署现有 API 管理服务**：使用 cmdlet[更新 AzApiManagementRegion](/powershell/module/az.apimanagement/update-azapimanagementregion)若要将现有 Azure API 管理服务移到虚拟网络中的。
...
+ **SMTP 中继**：对于 SMTP 中继，在主机下解析出站网络连接`smtpi-co1.msn.com`， `smtpi-ch1.msn.com`， `smtpi-db3.msn.com`，`smtpi-sin.msn.com`和 `ies.global.microsoft.com`
...
+ **强制隧道流量发往本地防火墙使用 Express Route 或网络虚拟设备**:常见的客户配置是定义其自己默认路由 (0.0.0.0/0)，以强制所有流量从 API 管理委派到流通过在本地防火墙或网络虚拟设备的子网。 此流量流一定会中断与 Azure API 管理的连接，因为已在本地阻止出站流量，或者已 NAT 到不再与各种 Azure 终结点一起工作的一组无法识别的地址。 该解决方案需要执行几个事项：

  * 启用服务终结点上在其中部署 API 管理服务的子网。 [服务终结点][ ServiceEndpoints]需要为 Azure Sql、 Azure 存储、 Azure 事件中心和 Azure 服务总线启用。 启用终结点直接从 API 管理与这些服务的委派子网允许他们使用 Microsoft Azure 主干网络的服务流量提供最佳路由。 如果在使用强制隧道的 Api 管理服务终结点，上面的 Azure 服务流量不会强制隧道传输。 其他 API 管理服务依赖项流量会通过强制隧道重定向，不能为丢失或 API 管理服务将无法正常运行。
    
  * 所有控制平面流量从 Internet 到 API 管理服务管理终结点是通过一组特定的托管的 API 管理入站 Ip 都路由。 强制通过隧道发送流量时响应不会对称映射回这些入站源 Ip。 若要克服此限制，我们需要添加以下用户定义路由 ([Udr][UDRs]) 引导到 Azure 的流量通过设置为"Internet"这些主机路由的目标。 控制平面流量的入站 Ip 的集如下所示：
    
    > 13.84.189.17/32, 13.85.22.63/32, 23.96.224.175/32, 23.101.166.38/32, 52.162.110.80/32, 104.214.19.224/32, 13.64.39.16/32, 40.81.47.216/32, 51.145.179.78/32, 52.142.95.35/32, 40.90.185.46/32, 20.40.125.155/32

  * 有关其他 API 管理服务的依赖项都强制通过隧道发送，其应为以解析主机名，并向该终结点的方式。 其中包括
      - 指标和运行状况监视
      - Azure 门户诊断
      - SMTP 中继
      - 开发人员门户验证码
...
上面给定的计算的最小大小的子网时，可以在其中部署 API 管理为/29，它提供了三个 IP 地址。
