...
1. 转到 [Azure 门户](https://portal.azure.com)以查找你的 API 管理实例。 搜索并选择“API 管理服务”。
...
6. 如果选择了“外部”或“内部”，则会列出预配了 API 管理服务的所有区域 。 选择“位置”，然后选取其虚拟网络和子网  。 该虚拟网络中填充了在配置的区域中设置的 Azure 订阅中可用的经典和 Resource Manager 虚拟网络。
    ...
    然后，选择“应用”。 系统会使用新的虚拟网络和子网选项更新 API 管理实例的“虚拟网络”页。
...
> [!IMPORTANT]
> 如果从 VNET 中删除 API 管理或更改在其中部署的 API 管理，则之前使用的 VNET 可最多 6 小时保持锁定状态。 在此期间，无法删除该 VNET 或向其部署新资源。 对于使用 API 版本 2018-01-01 和更低版本的客户端，此行为是正确的。 对于使用 API 版本 2019-01-01 和更高版本的客户端，一旦删除关联的 API 管理服务，就会释放 VNET。
...
* **在 VNET 内部署现有 API 管理服务**：使用 cmdlet [Update-AzApiManagementRegion](/powershell/module/az.apimanagement/update-azapimanagementregion) 将现有 Azure API 管理服务移动到虚拟网络内。
...
+ **指标和运行状况监视**：到 Azure Monitor 终结点的出站网络连接，可在以下域中解析。 如表中所示，这些 URL 在 AzureMonitor 服务标记下表示，用于网络安全组。
...
+ **SMTP 中继**：在主机 `smtpi-co1.msn.com`、`smtpi-ch1.msn.com`、`smtpi-db3.msn.com`、`smtpi-sin.msn.com` 和 `ies.global.microsoft.com` 下解析的 SMTP 中继的出站网络连接
...
+ **Azure 负载均衡器**：允许来自服务标记 `AZURE_LOAD_BALANCER` 的入站请求不是 `Developer` SKU 的要求，因为我们仅在其后面部署一个计算单元。 但来自 [168.63.129.16](../virtual-network/what-is-ip-address-168-63-129-16.md) 的入站在升到更高的 SKU（如 `Premium`）时变得至关重要，因为负载均衡器的运行状况探测失败会导致部署失败。

+ **使用 Express Route 或网络虚拟设备强制将流量通过隧道传输至本地防火墙**：常见的客户配置会定义自己的默认路由 (0.0.0.0/0)，该路由会将来自 API 管理委托子网的所有流量强制穿过本地防火墙或流入网络虚拟设备。 此流量流一定会中断与 Azure API 管理的连接，因为已在本地阻止出站流量，或者已 NAT 到不再与各种 Azure 终结点一起工作的一组无法识别的地址。 该解决方案要求执行以下几项操作：

  * 在部署 API 管理服务的子网上启用服务终结点。 需要为 Azure SQL、Azure 存储、Azure EventHub 和 Azure ServiceBus 启用[服务终结点][ServiceEndpoints]。 直接启用从 API 管理委托子网到这项服务的终结点，使它们可以使用 Microsoft Azure 主干网络，从而为服务通信提供最佳路由。 如果将服务终结点与被强制进行隧道传输的 API 管理配合使用，则不会强制隧道传输以上 Azure 服务流量。 其他 API 管理服务依赖项流量会被强制进行隧道传输且不能丢失，否则 API 管理服务会功能失常。
    
  * 所有从 Internet 到 API 管理服务的管理终结点的控制平面流量均通过 API 管理托管的一组特定入站 IP 进行路由。 当流量被强制进行隧道传输时，响应不会对称地映射回这些入站源 IP。 为了克服此限制，我们需要添加以下用户定义的路由 ([UDR][UDRs])，通过将这些主机路由的目标设置为“Internet”来将流量传回 Azure。 [控制平面 IP 地址](#control-plane-ips)中记录了控制平面流量的入站 IP 集
...
* **增量更新**：对网络进行更改时，请参阅 [NetworkStatus API](https://docs.microsoft.com/rest/api/apimanagement/2019-12-01/networkstatus)，验证并确保 API 管理服务尚未丧失对所依赖的任何关键资源的访问权限。 连接状态应每 15 分钟更新一次。
...
每增加一个 API 管理的缩放单元，都需要增加两个 IP 地址。
...
IP 地址是按 Azure 环境划分的。 当允许入站请求时，使用“全球”标记的 IP 地址必须与“区域”特定 IP 地址一起列入允许列表 。
...
* [Azure 虚拟网络常见问题](../virtual-network/virtual-networks-faq.md)
