...
|`RegistrationDate`|日期时间|注册应用程序的日期和时间。|  
...
|`HasExpired`|布尔|订阅是否已到期。|  
|`IsRejected`|布尔|是否已拒绝订阅请求。|  
...
|`IsBasicAccount`|布尔|如果此帐户使用电子邮件和密码注册，则为 true；如果此帐户使用提供程序注册，则为 false。|  
...
|`RegistrationEnabled`|布尔|是否已启用注册。|  
|`DelegationEnabled`|布尔|是否已启用委派登录。|  
...
|`UserRegistrationTermsEnabled`|布尔|是否启用条款。|  
...
|属性|类型|描述|  
|--------------|----------|-----------------|  
|`PasswordConfirm`|布尔|[注册](api-management-page-controls.md#sign-up)注册控件使用的值。|  
|`Password`|字符串|用户帐户密码。|  
...
|`ConsentAccepted`|布尔|[注册](api-management-page-controls.md#sign-up)注册控件使用的值。|  
|`Email`|字符串|电子邮件地址。 不得为空，且必须在服务实例中唯一。 最大长度为 254 个字符。|  
|`FirstName`|字符串|名字。 不得为空。 最大长度为 100 个字符。|  
|`LastName`|字符串|姓氏。 不得为空。 最大长度为 100 个字符。|  
