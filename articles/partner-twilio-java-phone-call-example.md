...
以下示例演示了如何使用 Twilio 从 Azure 中托管的网页发起呼叫。 生成的应用程序将提示用户电话呼叫值，如下图所示。

![使用 Twilio 和 Java 的 Azure 呼叫窗体][twilio_java]

需要执行以下操作来使用本主题中的代码：

1. 获取 Twilio 帐户和身份验证令牌。 要开始使用 Twilio，请评估[https://www.twilio.com/pricing][twilio_pricing]定价。 您可以在 注册[https://www.twilio.com/try-twilio][try_twilio]。 有关 Twilio 提供的 API 的信息，[https://www.twilio.com/api][twilio_api]请参阅。
2. 获取 Twilio JAR。 在[https://github.com/twilio/twilio-java][twilio_java_github]，您可以下载 GitHub 源并创建自己的 JAR，或下载预构建的 JAR（无论是否具有依赖项）。
...
除了发起呼叫外，makecall.jsp 还可显示 Twilio 终结点、API 版本和呼叫状态。 例如以下屏幕截图：
...
* 可以使用 **RoleEnvironment.getConfigurationSettings** 从部署的配置设置中检索 Twilio 帐户 ID 和身份验证令牌，而不是在 makecall.jsp 中对这些值进行硬编码。 有关**角色环境**类的信息，请参阅在[JSP 中使用 Azure 服务运行时库][azure_runtime_jsp]。
* makecall.jsp 代码为**Url**变量分配 Twilio[https://twimlets.com/message][twimlet_message_url]提供的 URL。 此 URL 提供了一个 Twilio 标记语言 (TwiML) 响应，指示 Twilio 如何继续进行呼叫。 例如，返回的 TwiML 可以包含一个**&lt;&gt; Say**谓词，该谓词会导致文本与呼叫接收者通话。 可以构建自己的服务来响应 Twilio 的请求，而不使用 Twilio 提供的 URL；有关详细信息，请参阅[如何通过 Java 使用 Twilio 实现语音和短信功能][howto_twilio_voice_sms_java]。 有关 TwiML 的更多信息，请访问[https://www.twilio.com/docs/api/twiml][twiml]，有关**&lt;Say&gt;** 和其他 Twilio 动词的详细信息，请访问 。 [https://www.twilio.com/docs/api/twiml/say][twilio_say]
