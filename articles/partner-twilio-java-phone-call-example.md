...
1. 获取 Twilio 帐户和身份验证令牌。 若要开始 Twilio，请[https://www.twilio.com/pricing][twilio_pricing]评估定价。 你可以在[https://www.twilio.com/try-twilio][try_twilio]上进行注册。 有关 Twilio 提供的 API 的信息，请参阅[https://www.twilio.com/api][twilio_api]。
2. 获取 Twilio JAR。 在[https://github.com/twilio/twilio-java][twilio_java_github]，你可以下载 GitHub 源并创建自己的 jar，或下载预生成的 jar （有或没有依赖项）。
...
准备好部署到 Azure 之后，请针对云部署重新进行编译，部署到 Azure，并在浏览器中运行 http://*your_hosted_name*.cloudapp.net/TwilioCloud/callform.jsp（将 *your_hosted_name* 替换为自己的值）。
