<properties urlDisplayName="How to create a LAMP Stack with Windows Azure" pageTitle="如何使用 Microsoft Azure 创建 LAMP 堆栈" metaKeywords="Azure LAMP stack vm, LAMP stack vm, Azure LAMP stack" description="了解如何在 Microsoft Azure 中使用运行 Linux 的 Azure 虚拟机 (VM) 创建 LAMP 堆栈。" metaCanonical="" services="virtual-machines" documentationCenter="" title="" authors="ningk" solutions="" manager="timlt" editor="tysonn"/>
...
假定读者已拥有 Azure 订阅。如果没有，你可以在 [http://www.windowsazure.cn/](http://www.windowsazure.cn/pricing/1rmb-trial/) 中注册试用版。

如果你已有虚拟机，并且只想查找在不同 linux 分发上安装 LAMP 堆栈的基础知识，则除了本主题外，还可以参阅[在 Azure 中的 Linux 虚拟机上安装 LAMP 堆栈](virtual-machines-linux-install-lamp-stack.md)。
...
-	Web 服务器的根位于 **/var/www/html**。你应向需要将文件上载到此文件夹的用户授予特权。以下示例演示如何将写权限添加到名为 lampappgroup 的组并在此组中放置 azureuser 用户名：  
	...
	>[AZURE.NOTE] 如果要修改 /var/www/html/ 中的文件，你可能需要再次登录。
-	使用任何 SFTP 客户端（例如 FileZilla）连接到你的虚拟机（例如，lampdemo.chinacloudapp.cn）的 DNS 名称，并导航到 /**var/www/html** 以发布站点。![][18]
