...
1. Open a web browser, and browse to [http://www.windowsazure.cn](http://www.windowsazure.cn).
...
1. Sign in to the [Management Portal](http://manage.windowsazure.cn). You should see a navigation pane that looks like this.
...
**Note:** The schema and data values are taken from this [MSDN article](http://msdn.microsoft.com/zh-cn/library/azure/ee621790.aspx "MSDN article") and have been modified to work with SQL Database.

1. Go to the home page. In the [Management Portal](http://manage.windowsazure.cn), the **School** database appears in the list of items on the home page.
...
Open a new query window and then paste in the following script. Run the script to insert data. This script uses the [INSERT](http://msdn.microsoft.com/zh-cn/library/azure/ee336284.aspx) statement to add values to each column.
...
1. In the [Management Portal](http://manage.windowsazure.cn), select **SQL Databases**, click **Servers**, choose the server and then click the white arrow to open the
server page.
...
Using just Excel, you can import only one table at a time. A better approach is to use the PowerPivot for Excel add-in, which lets you import and work with multiple tables as a single data set. Working with PowerPivot is beyond the scope of this tutorial, but you can get more information in this topic about [PowerPivot for Excel](http://technet.microsoft.com/zh-cn/library/gg413471.aspx).
...
To actively manage your new database, consider installing and using SQL Server Management Studio. Management Studio is the primary database administration tool for managing SQL Server databases, including those running on Azure. Using Management Studio, you can save queries for future use, add new tables and stored procedures, and hone your Transact-SQL skills in a rich scripting environment that includes a syntax checker, intellisense, and templates. To get started, follow the instructions in [Managing SQL Databases Using SQL Server Management Studio](http://azure.microsoft.com/zh-cn/documentation/articles/sql-database-manage-azure-ssms/).

Fluency in the Transact-SQL query and data definition language is essential for database administrators. If you are new to Transact-SQL, start with the [Tutorial: Writing Transact-SQL Statements](http://msdn.microsoft.com/zh-cn/library/ms365303.aspx) to learn some basic skills.
...
* [Migrating Databases to SQL Database](http://msdn.microsoft.com/zh-cn/library/azure/ee730904.aspx)
* [Copying Databases in SQL Database](http://msdn.microsoft.com/zh-cn/library/azure/ff951624.aspx)
* [Deploy a SQL Server Database to an Azure Virtual Machine](http://msdn.microsoft.com/zh-cn/library/dn195938(v=sql.120).aspx)
