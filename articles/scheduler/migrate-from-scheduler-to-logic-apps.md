...
> [!IMPORTANT]
> [Azure 逻辑应用](../logic-apps/logic-apps-overview.md)正在替换[正在停用](#retire-date)的 Azure 计划程序。 要继续处理在计划程序中设置的作业，请按照本文尽快迁移到 Azure 逻辑应用。 
>
...
要了解更多信息，请参阅[什么是 Azure 逻辑应用？或](../logic-apps/logic-apps-overview.md)尝试在此快速入门中创建第一个逻辑应用：[创建第一个逻辑应用](../logic-apps/quickstart-create-first-logic-app-workflow.md)。
...
1. 对于请求触发器，您可以选择提供 JSON 架构，这有助于逻辑应用设计器了解对请求触发器的入站调用中包含的输入的结构，并使输出更易于在工作流中稍后选择。
   ...
   1. **在"输入或粘贴 JSON 有效负载示例**"下，提供示例有效负载，然后选择 **"完成**"，例如：
...
1. 输入要启动逻辑应用工作流的时间戳。
   ...
   单击**时间戳**框内时，将显示动态内容列表，以便可以选择从触发器中选择输出。
...
| 请求方法 | 代码 | Body | 标头 |
|----------------|-----|------|---------|
| **POST** | <*端点-URL*> | **raw** <p>**JSON(application/json)** <p>在“raw”框中，输入要在请求中发送的有效负载****。 <p>**注意**：此设置会自动配置“标头”值****。 | **键**：Content-Type <br>**值**：application/json |
|||||
...
> [!IMPORTANT]
>
> 如果以后要取消作业，请选择 **"标题"** 选项卡。在响应中查找并复制**x-ms-workflow-运行 id**标头值。 
>
...
1. 如果需要，可以设置更高级的计划。
   ...
   有关高级计划选项的详细信息，请参阅使用[Azure 逻辑应用创建和运行定期任务和工作流](../connectors/connectors-native-recurrence.md)。
...
**问**：Azure 计划程序何时停用？ <br>
**答**：Azure 计划程序计划于 2019 年 12 月 31 日完全停用。 有关在此日期之前采取的重要步骤和详细时间表，请参阅[将计划计划的退休日期延长至 2019 年 12 月 31 日](https://azure.microsoft.com/updates/extending-retirement-date-of-scheduler/)。 有关常规更新，请参阅[Azure 更新 - 计划程序](https://azure.microsoft.com/updates/?product=scheduler)。

**问**：服务停用后，我的工作集和工作会发生什么情况？ <br>
**A**： 所有计划程序作业集合和作业停止运行，并从系统中删除。
