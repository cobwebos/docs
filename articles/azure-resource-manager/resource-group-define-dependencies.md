---
title: "Resource Manager 模板中的依赖关系 | Microsoft Docs"
...
---
# <a name="defining-dependencies-in-azure-resource-manager-templates"></a>在 Azure 资源管理器模板中定义依赖关系
...
定义依赖关系时，可以包括资源提供程序命名空间和资源类型，以避免模糊。 例如，若要明确可能与其他资源具有相同名称的负载均衡器和虚拟网络，请使用以下格式：
...
尽管你可能倾向使用 dependsOn 来映射资源之间的关系，但请务必了解这样做的理由，因为它会影响部署性能。 例如，若要记录资源的互连方式，那么，dependsOn 方法并不合适。 部署之后，无法查询 dependsOn 元素中定义的资源。 通过使用 dependsOn，可以影响部署时间，因为 Resource Manager 不会并行部署两个具有依赖关系的资源。 若要记录资源之间的关系，请改为使用[资源链接](resource-group-link-resources.md)。
