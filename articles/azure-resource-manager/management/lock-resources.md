...
应用**ReadOnly**可能会导致意外的结果，因为某些不会修改资源的操作实际上需要锁阻止的操作。 **ReadOnly**锁可以应用于资源，也可以应用于包含资源的资源组。 **只读**锁阻止的一些常见操作示例包括：

* 存储帐户上的**ReadOnly**锁将阻止所有用户列出密钥。 列出密钥操作通过 POST 请求进行处理，因为返回的密钥可用于写入操作。
...
使用资源管理器模板部署锁定时，根据锁的作用域，对 "名称" 和 "类型" 使用不同的值。
...
有关在资源组中设置锁定的示例，请参阅[创建资源组并锁定它](https://github.com/Azure/azure-quickstart-templates/tree/master/subscription-level-deployments/create-rg-lock-role-assignment)。
...
若要获取有关某个锁的信息，请使用[AzResourceLock](/powershell/module/az.resources/get-azresourcelock)。 若要获取订阅中的所有锁，请使用：
