...
可以控制返回的有关预测更改的详细信息级别。 将 `ResultFormat` 参数设置为 `FullResourcePayloads` 以获取资源列表，其中会发生更改，以及有关将更改的属性的详细信息。 将 `ResultFormat` 参数设置为 `ResourceIdOnly` 以获取将更改的资源的列表。 默认值是 `FullResourcePayloads`。  
