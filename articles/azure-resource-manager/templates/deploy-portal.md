...
了解如何将 Azure[门户](https://portal.azure.com)与[Azure 资源管理器 （ARM） 模板](overview.md)一起部署 Azure 资源。 若要了解有关管理资源的信息，请参阅[通过 Azure 门户管理 Azure 资源](../management/manage-resources-portal.md)。
...
如果想要执行部署，但不使用市场中的任何模板，可以创建自定义模板来针对解决方案定义基础结构。 要了解如何创建模板，请参阅了解[ARM 模板的结构和语法](template-syntax.md)。
...
1. 此时，会看到用于创建模板的几个选项：
    ...
    - **常见模板**：有四个用于创建 Linux 虚拟机、Windows 虚拟机、Web 应用程序和 Azure SQL 数据库的常见模板。
...
- 要从部署或资源组导出模板，请参阅导出[ARM 模板](export-template-portal.md)。
- 要跨多个区域安全地推出服务，请参阅 Azure[部署管理器](deployment-manager-overview.md)。
