...
ms.locfileid: "80153261"
---
# <a name="deploy-resources-with-arm-templates-and-azure-powershell"></a>使用 ARM 模板和 Azure PowerShell 部署资源

了解如何将 Azure PowerShell 与 Azure 资源管理器 （ARM） 模板一起部署到 Azure。 有关部署和管理 Azure 解决方案的概念的详细信息，请参阅[模板部署概述](overview.md)。
...
要部署到**管理组**，请使用[New-AzManagementGroup 部署](/powershell/module/az.resources/New-AzManagementGroupDeployment)。
...
要部署到**租户**，请使用[新阿兹租户部署](/powershell/module/az.resources/new-aztenantdeployment)。
...
除非使用 Azure 云外壳部署模板，否则需要安装 Azure PowerShell 并连接到 Azure：
...
要部署外部模板，请使用**模板Uri**参数。 使用示例中的 URI 从 GitHub 部署示例模板。
...
前面的示例要求模板的 URI 可公开访问，它适用于大多数情况，因为模板应该不会包含敏感数据。 如果需要指定敏感数据（如管理员密码），请以安全参数的形式传递该值。 但是，如果不希望模板可公开访问，可以通过将其存储在专用存储容器中来保护它。 有关部署需要共享访问签名 (SAS) 令牌的模板的信息，请参阅[部署具有 SAS 令牌的专用模板](secure-template-with-sas-token.md)。 要浏览教程，请参阅[教程：在 ARM 模板部署中集成 Azure 密钥保管库](template-tutorial-use-key-vault.md)。

## <a name="deploy-from-azure-cloud-shell"></a>从 Azure 云外壳部署
...
要打开云外壳，请浏览到[https://shell.azure.com](https://shell.azure.com)，或从以下代码部分选择 **"试用"：**
...
要在不实际部署任何资源的情况下测试模板和参数值，请使用[测试-AzResourceGroup 部署](/powershell/module/az.resources/test-azresourcegroupdeployment)。 
