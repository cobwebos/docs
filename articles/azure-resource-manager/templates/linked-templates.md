...
ms.locfileid: "76119823"
---
# <a name="using-linked-and-nested-templates-when-deploying-azure-resources"></a>部署 Azure 资源时使用链接模版和嵌套模版

若要部署复杂的解决方案，你可以将模板分解为多个相关模板，然后通过主模板将它们一起部署。 相关模板可以是嵌入在主模板内的单独文件或模板语法。 本文使用术语 "**链接模板**" 引用从主模板链接到的单独模板文件。 它使用术语 "**嵌套模板**" 引用主模板内的嵌入模板语法。
...
若要嵌套模板，请将[部署资源](/azure/templates/microsoft.resources/deployments)添加到主模板。 在 "**模板**" 属性中，指定模板语法。
...
### <a name="scope-for-expressions-in-nested-templates"></a>嵌套模板中的表达式的作用域

使用嵌套模板时，可以指定是在父模板的作用域内还是在嵌套模板中计算模板表达式。 作用域确定如何解析参数、变量和资源（如[resourceGroup](template-functions-resource.md#resourcegroup)和[订阅](template-functions-resource.md#subscription)）。

通过 `expressionEvaluationOptions` 属性设置作用域。 默认情况下，`expressionEvaluationOptions` 属性设置为 `outer`，这意味着它使用父模板范围。 将 "值" 设置为 "`inner`" 以将表达式的作用域设置为嵌套的模板。
...
以下模板演示如何根据作用域解析模板表达式。 它包含一个名为 `exampleVar` 的变量，该变量在父模板和嵌套模板中都进行了定义。 它将返回变量的值。
...
下面的示例部署一个 SQL server，并检索用于该密码的密钥保管库机密。 作用域设置为 "`inner`"，因为它会动态创建密钥保管库 ID 并将其作为参数传递到嵌套模板。
...
> [!NOTE]
>
> 如果作用域设置为 `outer`，则不能在嵌套模板中已部署资源的嵌套模板的输出部分中使用 `reference` 函数。 若要返回嵌套模板中已部署资源的值，请使用内部范围或将嵌套模板转换为链接的模板。

## <a name="linked-template"></a>链接模板

若要链接模板，请将[部署资源](/azure/templates/microsoft.resources/deployments)添加到主模板。 在**templateLink**属性中，指定要包含的模板的 URI。 以下示例链接到部署新存储帐户的模板。
...
你可以在外部文件或内联中提供链接模板的参数。 提供外部参数文件时，请使用**parametersLink**属性：
...
若要使用嵌套模板创建资源的多个实例，请将 "copy" 元素添加到**Microsoft .resources/部署**资源的级别。 或者，如果范围是内部的，则可以在嵌套模板中添加副本。

以下示例模板演示了如何将 copy 与嵌套模板结合使用。
...
资源管理器将每个模板作为部署历史记录中的单独部署进行处理。 具有三个链接或嵌套模板的主模板在部署历史记录中显示为：
...
* 若要了解如何定义一个资源而创建多个实例，请参阅[在 Azure 资源管理器中创建多个资源实例](create-multiple-instances.md)。
