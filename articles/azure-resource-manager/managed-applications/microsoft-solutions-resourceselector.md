...
description: 描述 Azure 门户的 ResourceSelector UI 元素。 用于获取现有资源的列表。
author: tfitzmac
ms.topic: conceptual
ms.date: 07/13/2020
ms.author: tomfitz
...
在 `resourceType` 属性中，提供要在列表中显示的资源的资源提供程序命名空间和资源类型名称。

`filter`属性限制资源的可用选项。 可以按位置或订阅限制结果。 若要仅显示与 "基本信息" 中所选内容匹配的资源，请使用 `onBasics` 。 若要显示所有资源，请使用 `all` 。 默认值为 `all`。
