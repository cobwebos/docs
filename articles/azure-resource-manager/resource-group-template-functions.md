...
| 参数 | 必选 | 类型 | 说明 |
|:--- |:--- |:--- |:--- | 
|operand1 |是 |Integer |要添加的第一个数。 |
|operand2 |是 |Integer |要添加的第二个数。 |
...
| 参数 | 必选 | 类型 | 说明 |
|:--- |:--- |:--- |:--- |
| offset |否 |Integer |要添加到从零开始的迭代值的数。 |
...
| 参数 | 必选 | 类型 | 说明 |
|:--- |:--- |:--- |:--- |
| operand1 |是 |Integer |要乘以的第一个数。 |
| operand2 |是 |Integer |要乘以的第二个数。 |
...
### <a name="concat---string"></a>concat - 字符串
...
### <a name="length---string"></a>length - 字符串
`length(string)`
...
| 参数 | 必选 | 类型 | 说明 |
|:--- |:--- |:--- |:--- |
| originalValue |是 |String |用于跳过的字符串。 |
...
| 参数 | 必选 | 类型 | 说明 |
|:--- |:--- |:--- |:--- |
| originalValue |是 |String |要从中提取字符的值。 |
...
### <a name="concat---array"></a>concat - 数组
...
### <a name="length---array"></a>length - 数组
`length(array)`
...
以 **list** 开头的任何操作都可用作模板中的函数。 可用操作不仅包括 **listKeys**，也包括 **list**、**listAdminKeys** 和 **listStatus** 等操作。 若要确定存在列表操作的资源类型，请参阅资源提供程序的 [REST API 操作](/rest/api/)。
...
若要查找资源提供程序的列表操作，请使用以下 Azure CLI 命令和 JSON 实用工具 [jq](http://stedolan.github.io/jq/download/) 来仅筛选列表操作：
...
以下示例演示了如何检索网站和数据库的资源 ID。 网站在名为 **myWebsitesGroup** 的资源组中，数据库在此模板的当前资源组中。
