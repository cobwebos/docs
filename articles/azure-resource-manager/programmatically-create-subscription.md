---
title: 以编程方式创建 Azure 订阅 |Microsoft Docs
...
使用 `principalName` 属性标识想要对其收取订阅费用的帐户。 复制该帐户的 @no__t 0。 例如，如果你想要在 @no__t 0 注册帐户下创建订阅，则复制 @no__t。 此标识符是注册帐户的对象 ID。 将此值粘贴到某个位置，以便在下一步中将其用作 `enrollmentAccountObjectId`。
...
```
使用 `principalName` 属性标识想要对其收取订阅费用的帐户。 复制该帐户的 @no__t 0。 例如，如果你想要在 @no__t 0 注册帐户下创建订阅，则复制 @no__t。 将此对象 ID 粘贴到某个位置，以便在下一步中将其用作 `enrollmentAccountObjectId`。
...
发出以下请求，将 `<enrollmentAccountObjectId>` 替换为从第一步中复制的 `name` （```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```）。 若要指定所有者，请了解[如何获取用户对象 id](grant-access-to-create-subscription.md#userObjectId)。
...
首先，通过运行 `Install-Module Az.Subscription -AllowPrerelease` 安装此预览模块。 为了确保 `-AllowPrerelease` 有效，请从[获取 PowerShellGet 模块](/powershell/gallery/installing-psget)安装 PowerShellGet 最新版本。
...
运行下面的[az account create](/cli/azure/ext/subscription/account?view=azure-cli-latest#-ext-subscription-az-account-create)命令，将 `<enrollmentAccountObjectId>` 替换为在第一步中复制的 `name` （```747ddfe5-xxxx-xxxx-xxxx-xxxxxxxxxxxx```）。 若要指定所有者，请了解[如何获取用户对象 id](grant-access-to-create-subscription.md#userObjectId)。
...
```
使用 `displayName` 属性来确定要为其创建订阅的计费帐户。 请确保该帐户的 agreeementType 为*MicrosoftCustomerAgreement*。 复制帐户的 @no__t 0。  例如，如果要为 `Contoso` 计费帐户创建订阅，请复制 @no__t。 将此值粘贴到某个位置，以便在下一步中使用它。
...
发出以下请求，将 `<billingAccountName>` 替换为从第一步中复制的 `name` （```5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx```）。
...
使用 `invoiceSectionDisplayName` 属性来标识要为其创建订阅的发票部分。 复制 "`invoiceSectionId`，`billingProfileId`" 和 "发票" 部分的 @no__t 之一。 例如，如果要为 `Development` 发票部分创建类型为 `Microsoft Azure plan` 的订阅，则应复制 `/providers/Microsoft.Billing/billingAccounts/5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_2019-05-31/billingProfiles/PBFV-XXXX-XXX-XXX/invoiceSections/GJGR-XXXX-XXX-XXX`、`/providers/Microsoft.Billing/billingAccounts/5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_2019-05-31/billingProfiles/PBFV-xxxx-xxx-xxx` 和 @no__t。 将这些值粘贴到某个位置，以便在下一步中使用它们。
...
发出以下请求，将 `<invoiceSectionId>` 替换为从第二步（```/providers/Microsoft.Billing/billingAccounts/5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_2019-05-31/billingProfiles/PBFV-XXXX-XXX-XXX/invoiceSections/GJGR-XXXX-XXX-XXX```）复制的 @no__t。 需要传递从 API 的请求参数中的第二步复制的 @no__t 0 和 @no__t。 若要指定所有者，请了解[如何获取用户对象 id](grant-access-to-create-subscription.md#userObjectId)。
...
| `owners`      | No       | 字符串 | 要在创建订阅时作为 RBAC 所有者添加到订阅中的任何用户或服务主体的对象 ID。  |
| `costCenter` | No      | 字符串 | 与订阅关联的成本中心。 它将显示在使用情况 csv 文件中。 |
| `managementGroupId` | No      | 字符串 | 将订阅添加到的管理组的 ID。 若要获取管理组的列表，请参阅[管理组列表 API](https://docs.microsoft.com/rest/api/resources/managementgroups/list)。 使用 API 中管理组的 ID。 |
...
```
使用 `displayName` 属性来确定要为其创建订阅的计费帐户。 请确保该帐户的 agreeementType 为*MicrosoftPartnerAgreement*。 复制帐户的 @no__t 0。 例如，如果要为 `Contoso` 计费帐户创建订阅，请复制 @no__t。 将此值粘贴到某个位置，以便在下一步中使用它。

### <a name="find-customers-that-have-azure-plans"></a>查找具有 Azure 计划的客户

发出以下请求，将 `<billingAccountName>` 替换为从第一步（```5e98e158-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx```）复制的 @no__t，以列出计费帐户中可为其创建 Azure 订阅的所有客户。 
...
使用 `displayName` 属性来标识要为其创建订阅的客户。 复制客户的 @no__t 0。 例如，如果要为 @no__t 创建订阅，请复制 @no__t。 将此值粘贴到某个位置，以便在后续步骤中使用它。
...
发出以下请求，将 `<customerId>` 替换为从第二步（```/providers/Microsoft.Billing/billingAccounts/99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx/customers/2281f543-xxxx-xxxx-xxxx-xxxxxxxxxxxx```）复制的 @no__t，列出可供客户使用的所有分销商。
...
```
使用 `description` 属性标识将与订阅关联的分销商。 为经销商复制 `resellerId`。 例如，如果想要关联 `Wingtip`，请复制 @no__t。 将此值粘贴到某个位置，以便在下一步中使用它。

### <a name="create-a-subscription-for-a-customer"></a>为客户创建订阅

下面的示例为*Fabrikam 玩具*创建名为*Dev 团队订阅*的订阅，并将*Wingtip*经销商关联到该订阅。 万亿项

发出以下请求，将 `<customerId>` 替换为从第一步中复制的 `id` （```/providers/Microsoft.Billing/billingAccounts/99a13315-xxxx-xxxx-xxxx-xxxxxxxxxxxx:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx_xxxx-xx-xx/customers/2281f543-xxxx-xxxx-xxxx-xxxxxxxxxxxx```）。 传递从 API 的请求参数中的第二步复制的可选*resellerId* 。 
...
| `resellerId`      | No       | 字符串 | 将与订阅关联的分销商的 MPN ID。  |
