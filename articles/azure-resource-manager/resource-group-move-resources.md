...
移动资源仅将其移到新的资源组或订阅。 它不会更改该资源的位置。

## <a name="checklist-before-moving-resources"></a>移动资源前需查看的清单

移动资源之前需执行的一些重要步骤。 验证这些条件可以避免错误。

1. 你想要移动的资源必须支持移动操作。 这些资源都支持移动的列表，请参阅[移动对资源的操作支持](move-support-resources.md)。

1. 移动资源时，某些服务有特定的限制或要求。 如果已移动所有以下服务，检查在移动之前该指南。
   ...
   * [虚拟网络移动的指南](./move-limitations/virtual-network-move-limitations.md)
...
[验证移动操作](/rest/api/resources/resources/validatemoveresources)可以测试你的移动方案而无需实际移动资源。 使用此操作以检查迁移将会成功。 发送移动请求，会自动调用验证。 仅在需要以预先确定结果时，请使用此操作。 若要运行此操作，需要：
...
如果遇到错误，请参阅[移动到新的资源组或订阅的 Azure 资源的故障排除](troubleshoot-move.md)。
...
若要将现有资源移到另一个资源组或订阅，请使用[将资源移](/rest/api/resources/Resources/MoveResources)操作。
...
这些资源都支持移动的列表，请参阅[移动对资源的操作支持](move-support-resources.md)。
