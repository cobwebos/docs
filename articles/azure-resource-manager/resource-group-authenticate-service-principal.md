...
本文演示如何通过 [Azure PowerShell](/powershell/azure/overview) 为应用程序进行一切所需设置，使之能够使用自己的凭据和标识运行。
...
现在，需要以应用程序方式登录以执行相应操作。 对于用户名，请使用为应用程序创建的 `ApplicationId`。 对于密码，请使用在创建帐户时指定的密码。 
...
也可从要登录的 PowerShell 调用 REST 操作。 可以从身份验证响应中检索访问令牌，将其用于其他操作。 若要通过示例来了解如何通过调用 REST 操作来检索访问令牌，请参阅[创建请求](/rest/api/#create-the-request)。
