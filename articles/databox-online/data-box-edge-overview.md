...
- **使用 Azure 机器学习进行推理** - 借助 Azure Stack Edge，可以运行 ML 模型以获得可在数据发送到云之前执行的快速结果。 可以选择传输完整的数据集以继续重新训练并改进 ML 模型。 有关如何在 Azure Stack Edge 设备上使用 Azure ML 硬件加速模型的详细信息，请参阅[在 Azure Stack Edge 上部署 Azure ML 硬件加速模型](https://docs.microsoft.com/azure/machine-learning/service/how-to-deploy-fpga-web-service#deploy-to-a-local-edge-server)。
...
|离线上传     | 离线模式支持离线上传方案。|
|支持的协议     | 支持用于数据引入的标准 SMB 和 NFS 协议。 <br> 有关支持的版本的详细信息，请转到 [Azure Stack Edge 系统要求](data-box-edge-system-requirements.md)。|
...
* **Azure Stack Edge 资源** - Azure 门户中的一种资源，使用该资源可以通过 Web 界面（可从不同的地理位置访问该界面）管理 Azure Stack Edge 设备。 使用 Azure Stack Edge 资源可以创建和管理资源、查看和管理设备与警报，以及管理共享。  
    ...
    有关详细信息，请转到[为 Azure Stack Edge 设备创建订单](data-box-edge-deploy-prep.md#create-a-new-resource)。
...
* **Azure Stack Edge 本地 Web UI** - 使用本地 Web UI 可以运行诊断、关闭和重启 Azure Stack Edge 设备、查看复制日志，并联系 Microsoft 支持部门以提出服务请求。
    ...
    有关使用基于 Web 的 UI 的详细信息，请转到[使用基于 Web 的 UI 管理 Azure Stack Edge](data-box-edge-manage-access-power-connectivity-mode.md)。
...
- 查看 [Azure Stack Edge 系统要求](data-box-edge-system-requirements.md)。
- 了解 [Azure Stack Edge 限制](data-box-edge-limits.md)。
- 在 Azure 门户中部署 [Azure Stack Edge](data-box-edge-deploy-prep.md)。
