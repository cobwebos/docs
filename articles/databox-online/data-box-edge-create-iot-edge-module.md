...
---
# <a name="develop-a-c-iot-edge-module-to-move-files-on-data-box-edge"></a>开发 C# IoT 边缘模块，以在数据框边缘上移动文件
...
2. 文件事件生成器将为写入到本地共享的每个文件创建文件事件。 修改文件时也会生成文件事件。 随后，将文件事件发送到 IoT Edge 中心（在 IoT Edge 运行时中）。
...
- Data Box Edge 设备正在运行。
    ...
    - 该设备已配置 Edge 计算角色。
    有关详细信息，请转到为数据框边缘[配置计算](data-box-edge-deploy-configure-compute.md#configure-compute)。

- 以下开发资源：

    - [视觉工作室代码](https://code.visualstudio.com/)。
    - [C# 用于视觉工作室代码（由 OmniSharp 提供支持）扩展](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp)。
    - [Visual Studio 代码的 Azure IoT 边缘扩展。](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-edge)
    - [.NET Core 2.1 SDK](https://www.microsoft.com/net/download)。
    - [多克 CE](https://store.docker.com/editions/community/docker-ce-desktop-windows). 你可能需要创建一个帐户来下载并安装该软件。
...
3. 在命令调色板中，输入并运行命令**Azure IoT 边缘：新的 IoT 边缘解决方案**。 在命令面板中提供以下信息，以便创建解决方案：
...
1. 在 VS 代码资源管理器中，打开**的模块>文件复制模块>Program.cs**。
...
4. 在上一步之后，立即添加**FileEvent**类以定义消息正文。
...
5. 在**Init 方法**中，代码创建并配置一个**ModuleClient**对象。 该对象允许模块使用 MQTT 协议连接到本地 Azure IoT Edge 运行时，以发送并接收消息。 在 Init 方法中使用的连接字符串由 IoT Edge 运行时提供给模块。 代码注册 FileCopy 回调，以通过 input1**** 终结点从 IoT Edge 中心接收消息。 将**Init 方法**替换为以下代码。
...
6. 删除**PipeMessage 方法**的代码，并在其位置，插入**FileCopy**的代码。
...
7. 保存此文件。
8. 您还可以下载此项目[的现有代码示例](https://azure.microsoft.com/resources/samples/data-box-edge-csharp-modules/?cdn=disable)。 然后，您可以根据此示例中**program.cs**文件验证保存的文件。
...
1. 在 VSCode 中，前往终端>新终端，打开新的视觉工作室代码集成终端。
...
3. 提供凭据后，可将模块映像推送到 Azure 容器注册表。 在 VS Code 资源管理器中，右键单击 module.json 文件****，然后选择“生成并推送 IoT Edge 解决方案”****。
    ...
    *程序.cs（77，44）：警告CS1998：此异步方法缺少"await"运算符，并将同步运行。请考虑使用"await"运算符等待非阻塞 API 调用，或"等待 Task.Run（...）"在后台线程上执行 CPU 绑定工作。*
...
要在数据框边缘部署和运行此模块，请参阅[添加模块](data-box-edge-deploy-configure-compute.md#add-a-module)中的步骤。
