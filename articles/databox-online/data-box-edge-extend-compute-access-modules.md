---
title: 管理 Azure Data Box Edge 上的计算网络以访问模块 |Microsoft Docs
description: 描述如何通过外部 IP 扩展 Data Box Edge 上的计算网络以访问模块。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
---
# <a name="enable-compute-network-on-your-azure-data-box-edge"></a>启用 Azure Data Box Edge 上的计算网络

本文介绍 Azure Data Box Edge 上运行的模块如何访问在设备上启用的计算网络。

若要配置网络，请执行以下步骤：

- 启用 Data Box Edge 设备上的网络接口进行计算
- 添加模块以访问 Data Box Edge 上的计算网络
- 验证模块是否可以访问已启用的网络接口
...
- Data Box Edge 设备安装完成。
- 根据本教程中所述完成**配置计算**步骤[：将数据转换](data-box-edge-deploy-configure-compute-advanced.md#configure-compute)为设备上的 Azure Data Box Edge。 设备应有关联的 IoT 中心资源、IoT 设备和 IoT Edge 设备。
...
2. **启用**要用于连接到要在设备上运行的计算模块的网络接口。
    ...
    ![启用计算设置1](media/data-box-edge-extend-compute-access-modules/enable-compute-setting-1.png)
...
执行以下步骤，将 web 服务器应用模块添加到 Data Box Edge 设备上。

1. 中转到与 Data Box Edge 设备关联的 IoT 中心资源，然后选择 " **IoT Edge 设备**"。
2. 选择与 Data Box Edge 设备关联的 IoT Edge 设备。 在 "**设备详细信息**" 中，选择 "**设置模块**"。 在 "**添加模块**" 中，选择 " **+ 添加**"，然后选择 " **IoT Edge 模块**"。
3. 在 " **IoT Edge 自定义模块**" 边栏选项卡中：
    ...
    3. 在**容器 "创建选项**" 中，粘贴以下代码示例：  
        ...
        ![在 IoT Edge 自定义模块边栏选项卡中指定端口信息](media/data-box-edge-extend-compute-access-modules/module-information.png)
...
2. 连接到 web 服务器应用模块。 打开浏览器窗口并键入：
    ...
    ![验证通过指定端口连接到模块](media/data-box-edge-extend-compute-access-modules/verify-connect-module-1.png)
