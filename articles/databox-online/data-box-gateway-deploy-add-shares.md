...
2. 按“Windows + R”。在“运行”窗口中，指定 `\\<device IP address>`。 单击“确定”。 此时会打开文件资源管理器。 此时应能看到作为文件夹创建的共享。 选择并双击要查看其内容的共享（文件夹）。
...
> [使用本地 Web UI 管理 Data Box Gateway](http://aka.ms/dbg-docs)
