...
ms.locfileid: "78384732"
---
# <a name="tutorial-transform-data-with-azure-data-box-edge-for-advanced-deployment-flow"></a>教程：将数据与高级部署流的 Azure Data Box Edge 进行转换
...
|------------------|--------------------------------------------------|---------------------------------------|
| 适用对象     | IT 管理员                                | 开发人员                            |
| 类型             | 使用 Data Box Edge 服务部署模块      | 使用 IoT 中心服务部署模块 |
...
此 Edge 设备上不存在自定义模块。 可以添加自定义的或预生成的模块。 若要了解如何创建自定义模块，请参阅[为 Data Box Edge 设备开发 C# 模块](data-box-edge-create-iot-edge-module.md)。
...
4. 在“添加模块”下执行以下操作：
    ...
    2. 指定 IoT Edge 自定义模块的设置。 输入以下值。
        ...
        |要求凭据     | 如果选中此项，则会使用用户名和密码来检索具有匹配 URL 的模块。        |
