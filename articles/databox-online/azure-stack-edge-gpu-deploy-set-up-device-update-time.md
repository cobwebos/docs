---
title: 教程：在 Azure 门户中使用 GPU 连接、配置和激活 Azure Stack Edge 设备 | Microsoft Docs
description: 本教程介绍 Azure Stack Edge GPU 部署方法，指导你连接、设置和激活物理设备。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "89254502"
---
# <a name="tutorial-configure-device-settings-for-azure-stack-edge-with-gpu"></a>教程：使用 GPU 配置 Azure Stack Edge 的设备设置

本教程介绍如何使用板载 GPU 为 Azure Stack Edge 设备配置设备相关设置。 可以通过本地 Web UI 设置设备名称、更新服务器和时间服务器。
...
在 Azure Stack Edge 设备上使用 GPU 配置设备相关设置之前，请确保：

* 对于物理设备：

    - 已根据[安装 Azure Stack Edge](azure-stack-edge-gpu-deploy-install.md) 中详述的内容安装了物理设备。
    - 已根据[教程：使用 GPU 为 Azure Stack Edge 配置网络](azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md)中详述的内容在设备上配置了网络，同时启用并配置了计算网络。
...
1. 在“设备设置”磁贴上，为“设备”选择“配置”。

    ![本地 Web UI“设备”页](./media/azure-stack-edge-gpu-deploy-set-up-device-update-time/device-1.png)
    ...
    3. 若要验证并应用已配置的设备设置，请选择“应用”。
        ...
        ![本地 Web UI“设备”页](./media/azure-stack-edge-gpu-deploy-set-up-device-update-time/device-3.png)

    4. 更改设备名称和 DNS 域后，将创建 SMB 和 NFS 终结点。  

    5. 应用设置后，返回到“入门”。

## <a name="configure-update"></a>配置更新

1. 在“设备设置”磁贴上，为“更新”选择“配置”。 现在可以配置设备更新的下载位置。  

    ![本地 Web UI“更新服务器”页](./media/azure-stack-edge-gpu-deploy-set-up-device-update-time/update-1.png)
...
1. 在“设备设置”磁贴上，选择“时间”。 可以为设备选择时区以及主要和辅助 NTP 服务器。  
    ...
    ![本地 Web UI“时间”页](./media/azure-stack-edge-gpu-deploy-set-up-device-update-time/time-1.png)
...
若要了解如何为 Azure Stack Edge 设备配置证书，请参阅：
