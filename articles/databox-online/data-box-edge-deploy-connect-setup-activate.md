...
3. （可选）在左窗格中选择“Web 代理设置”，然后配置 Web 代理服务器。 尽管这一配置为可选操作，但如果使用 Web 代理，则只能在此页上进行配置。
   ...
   > [!NOTE]
   > 不支持代理自动配置（PAC）文件。 PAC 文件定义了 web 浏览器和其他用户代理可以如何自动选择适当的代理服务器（访问方法）来获取给定的 URL。
   > 尝试截获和读取所有流量（然后用其自己的证书对所有内容进行重新签名）的代理不兼容，因为代理的证书不受信任。
   > 通常，透明代理适用于 Azure Data Box Edge。
