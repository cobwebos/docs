---
title: 教程：准备 Azure 门户、数据中心环境以便部署 Azure Stack Edge GPU | Microsoft Docs
description: 本文是介绍如何部署 Azure Stack Edge GPU 的第一篇教程，涉及如何准备 Azure 门户。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "89254672"
---
# <a name="tutorial-prepare-to-deploy-azure-stack-edge-with-gpu"></a>教程：准备部署 Azure Stack Edge with GPU 

本文是完整部署 Azure Stack Edge with GPU 时必读的部署教程系列的第一篇教程。 本教程介绍如何准备 Azure 门户，以便部署 Azure Stack Edge 资源。
...
若要进行 Azure Stack Edge 部署，需要先准备环境。 环境准备就绪后，请执行必需步骤，如有必要，请执行可选步骤和过程来完全部署设备。 逐步部署说明将指示何时应执行每个必需步骤以及可选步骤。
...
|  | |
|**部署教程** |需要完成这些教程，才能在生产环境中部署 Azure Stack Edge 设备。 |
|**[1.在 Azure 门户中做好部署 Azure Stack Edge 的准备](azure-stack-edge-gpu-deploy-prep.md)** |在安装 Azure Stack Edge 物理设备之前创建并配置 Azure Stack Edge 资源。 |
|**[2.安装 Azure Stack Edge](azure-stack-edge-gpu-deploy-install.md)**|将 Azure Stack Edge 物理设备拆包、装入机架并布线。  |
|**[3.连接到 Azure Stack Edge](azure-stack-edge-gpu-deploy-connect.md)** |安装设备后，连接到设备本地 Web UI。  |
|**[4.配置 Azure Stack Edge 的网络设置](azure-stack-edge-gpu-deploy-configure-network-compute-web-proxy.md)** |配置网络，包括设备的计算网络和 Web 代理设置。   |
|**[5.配置 Azure Stack Edge 的设备设置](azure-stack-edge-gpu-deploy-set-up-device-update-time.md)** |分配设备名称和 DNS 域，配置更新服务器和设备时间。 |
|**[6.配置 Azure Stack Edge 的安全设置](azure-stack-edge-gpu-deploy-configure-certificates.md)** |为设备配置证书。 可使用设备生成的证书，或者使用自己的证书。   |
|**[7.激活 Azure Stack Edge](azure-stack-edge-gpu-deploy-activate.md)** |使用从服务中获取的激活密钥来激活设备。 现已准备好在设备设置 SMB 或 NFS 共享或通过 REST 进行连接。 |
...
现在可以开始收集 Azure Stack Edge 设备软件配置的相关信息。

## <a name="deployment-configuration-checklist"></a>部署配置清单

在部署设备之前，需要收集信息来配置 Azure Stack Edge 设备上的软件。 提前准备其中的一些信息有助于简化在环境中部署设备的过程。 使用 [Azure Stack Edge 部署配置清单](azure-stack-edge-gpu-deploy-checklist.md)，在部署设备时记下配置详细信息。
...
2. 在左窗格中，选择“+ 创建资源”。 搜索并选择“Azure Stack Edge/Data Box Gateway”。 选择“创建”。 如果遇到任何问题，请转至[排查订单问题](azure-stack-edge-troubleshoot-ordering.md)。

3. 选取要用于 Azure Stack Edge 设备的订阅。 选择要将此物理设备寄送到的国家/地区。 选择“显示设备”。

    ![创建资源 1](media/azure-stack-edge-gpu-deploy-prep/create-resource-1.png)

4. 选择设备类型。 在“Azure Stack Edge Commercial”下选择“Azure Stack Edge with GPU”，然后选择“注册”  。 

    ![创建资源 2](media/azure-stack-edge-gpu-deploy-prep/create-resource-2.png)

5. 此处将显示缩写形式。 填写表格，并选择“提交”。 Microsoft 将启用你的订阅。

    ![创建资源 3](media/azure-stack-edge-gpu-deploy-prep/create-resource-3.png)

5. 启用订阅后，你应该能继续创建资源。 在“选择设备类型”边栏选项卡中，选择“选择” 。
...
> [安装 Azure Stack Edge](./azure-stack-edge-gpu-deploy-install.md)
