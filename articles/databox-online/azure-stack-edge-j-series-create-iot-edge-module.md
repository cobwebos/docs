...
3. 提供：
   ...
   4. 选择“位置” 。 建议将此位置与 Azure Stack Edge 资源关联。
   5. 将“管理员用户”切换到“启用”。********
   6. 将 SKU 设置为“基本”****。
...
3. 在“命令面板”中，输入并运行 Azure IoT Edge：**New IoT Edge solution** 命令。 在命令面板中提供以下信息，以便创建解决方案：
    ...
    2. 提供解决方案的名称，或者接受默认的 **EdgeSolution**。
        ...
        ![创建新解决方案 1](./media/azure-stack-edge-j-series-create-iot-edge-module/create-new-solution-1.png)

    3. 选择“C# 模块”作为模块模板。****
    4. 将默认模块名称替换为你想要分配的名称，在此案例中，它是“FileCopyModule”****。
    
        ![创建新解决方案 2](./media/azure-stack-edge-j-series-create-iot-edge-module/create-new-solution-2.png)
    ...
    5. 将在上一部分创建的容器注册表指定为第一个模块的映像存储库。 将 **localhost:5000** 替换为复制的登录服务器值。
        ...
        ![创建新解决方案 3](./media/azure-stack-edge-j-series-create-iot-edge-module/create-new-solution-3.png)

4. 转到“文件”>“打开文件夹”****。

    ![创建新解决方案 4](./media/azure-stack-edge-j-series-create-iot-edge-module/create-new-solution-4.png)
...
5. 浏览并指向先前创建的文件夹 EdgeSolution****。 VS Code 窗口将加载你的 IoT Edge 解决方案空间及其五个顶级组件。 在本文中，不会编辑 .vscode**** 文件夹、.gitignore**** 文件、.env **** 文件和 deployment.template.json****。
    ...
    ![创建新解决方案 5](./media/azure-stack-edge-j-series-create-iot-edge-module/create-new-solution-5.png)
...
2. 在集成终端中输入以下命令，登录到 Docker。
    ...
    ![生成并推送 IoT Edge 解决方案](./media/azure-stack-edge-j-series-create-iot-edge-module/build-iot-edge-solution-1.png)
...
3. 提供凭据后，可将模块映像推送到 Azure 容器注册表。 在 VS Code 资源管理器中，右键单击 module.json 文件****，然后选择“生成并推送 IoT Edge 解决方案”****。
    ...
    ![构建和推送 IoT Edge 解决方案2](./media/azure-stack-edge-j-series-create-iot-edge-module/build-iot-edge-solution-2.png)
    ...
    ![选择平台](./media/azure-stack-edge-j-series-create-iot-edge-module/select-platform.png)
