---
title: 使用 Azure 门户排查 Azure Data Box Edge 问题 | Microsoft Docs
description: 介绍如何排查 Azure Data Box Edge 问题。
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
本文介绍如何排查 Azure Data Box Edge 上的问题。 
...
1. 在本地 Web UI 中，转到“故障排除”>“诊断测试”。**** 选择要运行的测试，然后单击“运行测试”。**** 这会运行测试来诊断网络、设备、Web 代理、时间或云设置可能存在的任何问题。 你将收到设备正在运行测试的通知。
    ...
    ![选择测试](media/data-box-edge-troubleshoot/run-diag-1.png)
 
2. 测试完成后，会显示结果。 

    ![查看测试结果](media/data-box-edge-troubleshoot/run-diag-2.png)

    如果未通过某项测试，会显示建议的措施的 URL。 可以单击该 URL 查看建议的措施。
 
    ![查看未通过测试的警告](media/data-box-edge-troubleshoot/run-diag-3.png)
...
1. 在本地 Web UI 中，转到“故障排除”>“支持”。**** 单击“创建支持包”。**** 系统随即开始收集支持包。 收集包可能需要几分钟时间。
    ...
    ![单击“添加用户”](media/data-box-edge-troubleshoot/collect-logs-1.png)
 
2. 创建支持包后，单击“下载支持包”。**** 压缩包将下载到所选的路径。 可以解压缩该包并查看系统日志文件。

    ![单击“添加用户”](media/data-box-edge-troubleshoot/collect-logs-2.png)
...
1. 若要查看错误文件，请转到共享，然后单击该共享以查看内容。 
      ...
      ![连接到共享内容并查看](media/data-box-edge-troubleshoot/troubleshoot-logs-1.png)

2. 单击“Microsoft Data Box Edge 文件夹”。__ 此文件夹包含两个子文件夹：
