...
1. 首先[在 IoT Central 中创建规则](howto-create-telemetry-rules-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)。 保存规则条件后，单击“Azure 逻辑应用”磁贴作为新操作。 单击“在 Azure 门户中创建”。 随即转到 Azure 门户以创建新的逻辑应用。 你可能需要登录 Azure 帐户。

1. 输入创建新逻辑应用所需的信息。 可以选择要将新逻辑应用预配到的 Azure 订阅。 它不必是创建 IoT Central 应用时所在的订阅。 单击“创建”。

    ![在 Azure 门户创建逻辑应用](./media/howto-build-azure-logic-apps-experimental/createinazureportal.png)

1. 成功创建逻辑应用后，将自动导航到逻辑应用设计器。 单击“空白逻辑应用”。 

    ![创建空白逻辑应用](./media/howto-build-azure-logic-apps-experimental/blanklogicapp.png)

1. 在设计器中，搜索“iot central”，并选择“在触发规则时”触发器。 使用登录 IoT Central 应用时所用的帐户登录连接器。 

    ![登录 IoT Central 连接器](./media/howto-build-azure-logic-apps-experimental/addtrigger.png)

1. 登录成功后，应该会出现一些字段。 从下拉列表中选择“应用程序”和“规则”。

    ![选择应用程序和规则](./media/howto-build-azure-logic-apps-experimental/pickappandrule.png)
...
1. 在操作中，选择“团队”和“通道”。 在“消息”字段中填写希望每条消息所要表述的内容。 可以包括 IoT Central 规则提供的动态内容，将设备名称和时间戳等重要信息传递给通知。
    ...
    ![逻辑应用编辑操作，此时动态窗格处于打开状态](./media/howto-build-azure-logic-apps-experimental/buildworkflow.png)
...
![IoT Central Device Explorer 设备 ID](./media/howto-build-azure-logic-apps-experimental/iotcdeviceid.png)


## <a name="next-steps"></a>后续步骤
了解如何使用 Microsoft Flow 来生成工作流以后，建议完成下一步：[管理设备](howto-manage-devices-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)。
