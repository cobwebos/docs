...
* [定义新设备类型](tutorial-define-device-type-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)（必需）
* [配置设备的规则和操作](tutorial-configure-rules-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)（可选）
* [自定义操作员的视图](tutorial-customize-operator-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)（可选）

## <a name="add-a-real-device"></a>添加真实设备

若要将真实设备添加到应用程序，请使用[定义新设备类型](tutorial-define-device-type-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)教程中创建的“连接的空调”设备模板。

1. 若要以操作员身份添加新设备，请在左侧导航菜单中选择“Device Explorer”：

   ![显示“连接的空调”的 Device Explorer 页](media/tutorial-add-device-experimental/explorer.png)

   “Device Explorer”会显示“连接的空调”设备模板，以及当构建人员创建该设备模板时自动创建的模拟设备。

2. 若要开始连接真实的连接空调设备，请依次选择“新建”、“真实”：

   ![开始添加新的真实连接空调设备](media/tutorial-add-device-experimental/newreal.png)

3. 输入设备 ID（须小写）或使用所建议的设备 ID。也可输入新设备的名称，然后选择“创建”。  

   ![重命名设备](media/tutorial-add-device-experimental/rename.png)
...
1. 在“设置”页上，请注意“设置温度”状态设置为“未更新”。 在真实设备连接到应用程序并确认它已处理该设置之前，设备一直会保持此状态。 
    ...
    ![设置显示正在同步](media/tutorial-add-device-experimental/settingssyncing.png)

2. 在新的、真实的已连接空调设备的“属性”页上，服务位置和上次维修日期都是设备的可编辑属性。 在设备连接到应用程序之前，序列号和固件版本字段为空。 这些是从设备发送的只读值，不能进行编辑。

    ![真实设备的设备属性](media/tutorial-add-device-experimental/setproperties1.png)
...
[后续步骤](#next-steps)部分提到的“操作说明”文章提供了更多完整示例，并介绍了其他编程语言的用法。 有关设备如何连接到 Azure IoT Central 的详细信息，请参阅[设备连接](concepts-connectivity-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)一文。
...
1. 应用程序中设备实例的连接字符串是根据 IoT Central 提供的设备信息生成的。
   ...
   ![显示“查看连接信息”链接的“设备”页](media/tutorial-add-device-experimental/connectionlink.png)

1. 在“设备连接”页面上，复制“范围 ID、设备 ID 和主密钥”并将其粘贴到文本编辑器中，然后保存。 下一步使用这些值。

   ![连接详细信息](media/tutorial-add-device-experimental/device-connect.png)
...
1. 应用程序会在控制台上列显输出：
   ...
   ![客户端应用程序输出](media/tutorial-add-device-experimental/output.png)

1. 大约 30 秒后，设备的“度量”页上会显示遥测数据：

   ![真实遥测数据](media/tutorial-add-device-experimental/realtelemetry.png)

1. 在“设置”页上，可以看到设置现已同步。 首次连接设备后，该设备会收到设置值，并确认更改：

   ![设置已同步](media/tutorial-add-device-experimental/settingsynced.png)

1. 在“设置”页上，将设备温度设置为 **95**，并选择“更新设备”。 示例应用程序会接收并处理此更改：

   ![接收和处理设置](media/tutorial-add-device-experimental/receivesetting.png)
...
1. 在“度量”页上可以看到，设备正在发送较高的温度值：
    ...
    ![温度遥测数据现在较高](media/tutorial-add-device-experimental/highertemperature.png)
...
* [管理设备](howto-manage-devices-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
* [使用设备集](howto-use-device-sets-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
* [创建自定义分析](howto-use-device-sets-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)

以设备开发人员身份了解如何：

* [准备和连接 DevKit](howto-connect-devkit-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
...
* [将泛型 Node.js 客户端连接到 Azure IoT Central 应用程序](howto-connect-nodejs-experimental.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
