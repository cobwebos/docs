...
ms.locfileid: "69880236"
---
# <a name="device-connectivity-in-azure-iot-central-preview-features"></a>Azure IoT Central 中的设备连接 (预览功能)
...
1. [使用 SAS 快速连接单个设备](#connect-a-single-device)
1. [使用 SAS 以大规模连接设备](#connect-devices-at-scale-using-sas)
1. [使用 x.509 证书大规模连接设备](#connect-devices-using-x509-certificates)这是建议用于生产环境的方法。
1. [直接连接而无需首先注册设备](#connect-without-registering-devices)
1. [使用 IoT 即插即用功能连接设备](howto-connect-pnp-device-pnp.md?toc=/azure/iot-central-pnp/toc.json&bc=/azure/iot-central-pnp/breadcrumb/toc.json)

## <a name="connect-a-single-device"></a>连接单个设备

当你尝试 IoT Central 或测试设备时, 此方法非常有用。 你可以使用 IoT Central 应用程序中的设备连接信息来生成设备的连接字符串。 有关详细步骤, 请参阅[如何生成用于连接到 Azure IoT Central 应用程序的设备连接字符串](howto-generate-connection-string.md?toc=/azure/iot-central-pnp/toc.json&bc=/azure/iot-central-pnp/breadcrumb/toc.json)。
...
1. 在 IoT Central 应用程序中, 添加并验证用于生成设备证书_的中间或根 x.509 证书_:
    ...
    - 导航到 "**管理" > 设备连接 "> 证书 (x.509)** ", 并添加要用于生成叶设备证书的 x.509 根证书或中间证书。
      ...
      ![连接设置](media/concepts-connectivity-pnp/connection-settings.png)
    ...
    - 验证证书所有权可确保证书的上载程序具有证书的私钥。 验证证书:
        ...
        - 上载已签名的验证证书, 然后选择 "**验证**"。
          ...
          ![连接设置](media/concepts-connectivity-pnp/verify-cert.png)
...
![连接设置](media/concepts-connectivity-pnp/device-connection-flow1.png)
...
1. 配置连接设置:
    ...
    ![连接设置 SAS](media/concepts-connectivity-pnp/connection-settings-sas.png)
...
若要详细了解如何连接 IoT 即插即用设备, 请参阅如何[连接 iot 即插即用设备](howto-connect-pnp-device-pnp.md?toc=/azure/iot-central-pnp/toc.json&bc=/azure/iot-central-pnp/breadcrumb/toc.json)。
...
1. 设备状态为 "未关联", 这意味着连接到 IoT Central 的设备没有关联的设备模板。 这通常在以下情况下发生:
...
每个设备都使用可以标识设备的唯一连接字符串进行连接。 设备只能连接到注册了该设备的 IoT 中心。 在 Azure IoT Central 应用程序中创建实际设备时, 应用程序将生成使用`dps-keygen`构造连接字符串所需的信息。
...
- [连接 IoT 即插即用设备](howto-connect-pnp-device-pnp.md?toc=/azure/iot-central-pnp/toc.json&bc=/azure/iot-central-pnp/breadcrumb/toc.json)
