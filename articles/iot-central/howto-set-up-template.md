...
3. 输入新设备模板的名称（例如 Refrigerator-1），然后选择“创建”。
...
选择“完成”后，度量列表中会出现“温度”度量。 片刻之后，可以看到模拟设备生成的温度数据的可视化效果。 创建设备模板时，会从该模板生成一个模拟设备，让你在连接物理/真实设备之前测试应用程序的行为。
...
设置用于控制设备。 应用程序的操作员可以使用设置来提供设备的输入。 可将多个设置添加到设备模板。这些设置将在“设置”选项卡上显示为可供操作员使用的磁贴。 可以添加多种类型的设置：数字、文本、日期、切换开关、选取列表和部分标签。 
...
例如，可以通过选择“编辑模板”并输入新的“数字”设置来添加新的风扇速度设置：
...
创建磁贴后，在屏幕右上角选择“完成”。 将真实设备连接到应用程序后，设置值将更改为已同步。

![包含磁贴“设计模式”开关的“设置”选项卡](./media/howto-set-up-template/settingstile.png)

## <a name="properties"></a>属性

属性是与设备关联的设备元数据，例如设备位置和序列号。 可将多个属性添加到设备模板，这些属性将在“属性”选项卡上显示为磁贴。可以添加多种类型的属性：数字、文本、日期、切换开关、设备属性、标签和位置。 操作员可以在创建设备时指定属性的值，并随时可以编辑这些值。 但是，设备属性是从设备发送到应用程序的只读数据，不可由操作员更改。 连接真实设备后，应用程序中的设备属性磁贴将会更新。 

有两种类别的属性：

- 设备向 IoT Central 应用程序报告的**设备**属性。 这些属性是设备报告的只读值，连接真实设备后将在应用程序中更新。 
- 仅存储在应用程序中的、可由操作员编辑的**应用程序**属性。 设备无法识别应用程序属性。

例如，可以通过选择“编辑模板”并输入新属性，来添加设备位置作为新的“文本”属性（一个应用程序属性）：

| 显示名称  | 字段名称 | 裁剪前导空格  | 裁剪尾随空格  | 区分大小写| 最小长度 | 最大长度 |
| --------------| -----------|---------| ---------|---- |----|----|
| 位置      | loc        | 关闭     |  关闭     | 混合  | 0 | 100|
...
创建磁贴后，可以更改应用程序属性值。 首先，在屏幕右上角选择“完成”。
...
3. 配置位置的显示名称、字段名称和初始值（可选）。 
    ...
    | 显示名称  | 字段名称 | 初始值 |
    | --------------| -----------|---------| 
    | 安装地址 | insta_address | Microsoft, 1 Microsoft Way, Redmond, WA 98052   |
...
连接真实设备后，将使用设备发送的值更新添加为设备属性的位置。 安装地址（即添加为应用程序属性的位置）是可编辑的磁贴。 现在，你已经配置了位置属性，将能够[添加一个地图以在设备仪表板中直观显示位置](#add-an-azure-maps-location-in-the-dashboard)。
...
选择“保存”和“完成”后，**Echo** 命令将以磁贴的形式显示，并在连接真实设备后可用于回显设备。 命令的字段名称必须与相应设备代码中的属性名称匹配，这样，命令才能成功运行。
...
操作员可以转到仪表板来查看有关设备的信息。 构建人员可在此页中添加磁贴，以帮助操作员了解设备的行为。 可将多个仪表板磁贴添加到设备模板。 可以添加多种类型的仪表板磁贴，例如图像、折线图、条形图、关键绩效指标 (KPI)、设置和属性，以及标签。
...
3. 指定标题。 以下示例指定标题“安装位置”，然后选择先前在“属性”选项卡下配置的位置属性。在以下示例中，已选择“安装地址”。
