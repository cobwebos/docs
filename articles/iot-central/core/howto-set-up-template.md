...
description: 本文将介绍如何作为构建者，如何在 Azure IoT 中央应用程序中创建新的 Azure IoT 设备模板。 其中介绍了如何为类型定义遥测、状态、属性和命令。
author: dominicbetts
ms.author: dobett
ms.date: 12/06/2019
ms.topic: how-to
...
[IoT 即插即用（预览）](../../iot-pnp/overview-iot-plug-and-play.md)使 IoT Central 能够集成设备，而无需编写任何嵌入式设备代码。 IoT 即插即用（预览版）的核心是描述设备功能的设备功能模型架构。 在 IoT Central 应用程序中，设备模板使用这些 IoT 即插即用（预览版）设备功能模型。
...
| “属性” | 遥测消息中字段的名称。 IoT Central 将根据显示名称生成此字段的值，但你可根据需要选择自己的值。 |
| 功能类型 | 遥测。 |
...
| “属性” | 属性的名称。 IoT Central 将根据显示名称生成此字段的值，但你可根据需要选择自己的值。 |
| 功能类型 | 属性。 |
...
| “属性” | 命令的名称。 IoT Central 将根据显示名称生成此字段的值，但你可根据需要选择自己的值。 |
| 功能类型 | 命令。 |
...
| “属性” | 云属性的名称。 IoT Central 将根据显示名称生成此字段的值，但你可根据需要选择自己的值。 |
| 语义类型 | 属性的语义类型，如温度、状态或事件。 选择的语义类型将决定以下哪些字段可用。 |
