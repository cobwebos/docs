...
![选择操作](media/howto-use-action-groups-pnp/chooseaction.png)

从 Azure 订阅中选择操作组：

![选择操作组](media/howto-use-action-groups-pnp/chooseactiongroup.png)

选择“保存”。 操作组现在显示在触发规则时要运行的操作的列表中：

![保存的操作组](media/howto-use-action-groups-pnp/savedactiongroup.png)
...
| Webhook     | {"schemaId"： "AzureIoTCentralRuleWebhook"，"data"： {[regular webhook 负载](howto-create-webhooks-pnp.md#payload)}} |
...
现在，你已了解如何结合使用操作组和规则，接下来是了解如何[管理你的设备](howto-manage-devices-pnp.md)。
