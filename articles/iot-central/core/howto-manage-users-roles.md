...
每个用户必须有一个用户帐户才能登录和访问 Azure IoT Central 应用程序。 Azure IoT 中心支持 Microsoft 帐户和 Azure 活动目录帐户。 Azure IoT Central 目前不支持 Azure Active Directory 组。
...
如果 IoT 中心用户 ID 从 Azure 活动目录中删除，然后重新添加，则用户将无法在 IoT 中央应用程序中登录。 要重新启用访问权限，IoT 管理中心管理员应删除并读取应用程序中的用户。
...
**生成器**角色中的用户可以管理应用的每个部分，但不能对"管理"或"连续数据导出"选项卡进行更改。
...
| 执行 | 查看 <br/> 其他依赖项：查看设备模板、设备实例和设备组;更新设备实例;在设备实例上执行命令 |
| 完全控制 | 查看、更新、创建、删除、执行 <br/> 其他依赖项：查看设备模板、设备实例和设备组;更新设备实例;在设备实例上执行命令 |
...
| ---- | -------- |
| 查看 | 无     |
| Export | 查看 <br/> 其他依赖项：查看设备模板、设备实例、设备组、仪表板、数据导出、品牌、帮助链接、自定义角色、规则 |
