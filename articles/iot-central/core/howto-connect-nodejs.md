...
在 Azure IoT Central 应用程序中，需要具有以下度量值的设备模板：设备属性、设置和命令。
...
记下 "**设备连接**" 页上的设备连接信息：**作用域 ID**、**设备 ID**和**主密钥**。 稍后在本操作方法指南中将这些值添加到设备代码中：
...
1. 向文件添加以下变量声明：
    ...
    使用之前记下的值更新占位符 `{your Scope ID}`、`{your Device ID}`和 `{your Primary Key}`。 在此示例中，将 `targetTemperature` 初始化为零，可以使用来自设备的当前读取或设备克隆中的值。
