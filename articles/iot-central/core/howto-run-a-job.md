...
5. 接下来，选择 "**属性**" 或 "**命令**" 作为要配置的作业的类型。 若要设置**属性**作业配置，请选择属性并设置其新值。 若要设置**命令**，或选择要运行的命令。 属性作业可以设置多个属性：

    > [!div class="mx-imgBorder"]
    > ![配置作业](./media/howto-run-a-job/configurejob.png)

6. 选择设备后，选择 "**运行**" 或 "**保存**"。 作业现在会显示在 "主**作业**" 页上。 在此页上，可以看到当前正在运行的作业，以及以前运行的或保存的作业的历史记录。 保存的作业可以随时打开以继续编辑或运行它：
...
若要复制某个现有作业，请将其打开，然后选择 "**复制**"。 此时会打开一个作业配置副本供你编辑，并且**复制**将追加到作业名称的末尾。 您可以保存或运行新作业：
...
若要查看作业和所有受影响的设备的状态，请选择该作业。 若要下载包含作业详细信息（包括设备列表及其状态值）的 CSV 文件，请选择 "**下载作业详细信息**"。 在每个设备名称旁边，你会看到以下状态消息之一：
