...
ms.locfileid: "55771802"
---
# <a name="export-your-data-in-azure-iot-central"></a>导出 Azure IoT Central 中的数据

*本主题适用于管理员。*

本文介绍如何使用 Azure IoT Central 中的连续数据导出功能将数据导出到你自己的 **Azure Blob 存储**、**Azure 事件中心**和 **Azure 服务总线**实例。 可以将“度量”、“设备”和“设备模板”导出至自己的目标，以便进行暖路径和冷路径分析。 可以将数据导出到 Blob 存储，以便在 Microsoft Power BI 中运行长期趋势分析，也可以将数据导出到事件中心和服务总线，以便使用 Azure 逻辑应用或 Azure Functions 以近实时方式转换和扩充数据。
...
详细了解[如何导出到 Blob 存储](howto-export-data-blob-storage.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)。
...
详细了解如何[导出到事件中心和服务总线](howto-export-data-event-hubs-service-bus.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)。
...
2. 在左侧菜单中，单击“连续数据导出”。
    ...
    ![创建新的 cde 事件中心](media/howto-export-data-experimental/export_menu.png)

3. 单击右上的“+ 新建”按钮。 从 **Azure Blob 存储**、**Azure 事件中心**或 **Azure 服务总线**中选择一个，作为导出的目标。 

    > [!NOTE] 
    > 每个应用的最大导出数目是 5。 

    ![创建新的连续数据导出](media/howto-export-data-experimental/export_new.png)
...
4. 在下拉列表框中，选择自己的**存储帐户/事件中心命名空间/服务总线命名空间**。 也可选取列表中的最后一个选项，即“输入连接字符串”。 
    ...
    ![创建新的 cde 事件中心](media/howto-export-data-experimental/export_create.png)
...
6. 若要启用连续数据导出，请确保将“数据导出”设置为“打开”。 选择“保存”。
  ...
  ![配置连续数据导出](media/howto-export-data-experimental/export_list.png)
...
> [将数据导出到 Azure Blob 存储](howto-export-data-blob-storage.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)

> [!div class="nextstepaction"]
> [将数据导出到 Azure 事件中心和 Azure 服务总线](howto-export-data-event-hubs-service-bus.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)

> [!div class="nextstepaction"]
> [如何在 Power BI 中直观显示数据](howto-connect-powerbi.md?toc=/azure/iot-central-experimental/toc.json&bc=/azure/iot-central-experimental/breadcrumb/toc.json)
