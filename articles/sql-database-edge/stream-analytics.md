...
4. 更新 SQL 数据库边缘模块配置，使其包含 DAC 包的共享访问 URI。 若要更新 SQL 数据库边缘模块，请执行以下步骤：
    ...
    6. 在**IoT Edge 自定义模块**"窗格中，选择"**设置模块克隆的所需属性**"。 将所需属性更新为包含`SQLPackage`选项的 URI，如以下示例中所示。
...
6. 在**IoT Edge 自定义模块**"窗格中，选择"**设置模块克隆的所需属性**"。 将所需属性更新为包含`ASAJobInfo`选项的 URI，如以下示例中所示。
