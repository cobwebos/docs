...
* 使用 [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms/) 或 [Azure Data Studio](/sql/azure-data-studio/download/) 连接到 SQL Database Edge。 运行 SQL 脚本创建数据库和表。
...
* 使用 SQLPackage.exe 将 DAC 包文件部署到 SQL Database Edge 容器。 将 SqlPackage 文件 URI 指定为模块所需属性配置的一部分可以自动化此过程。 你还可以直接使用 SqlPackage.exe 客户端工具将 DAC 包部署到 SQL Database Edge。
...
7. 对于“链接服务”，选择“新建”，然后完成下列步骤   ：
    ...
    2. 在“服务器名称”下，输入 SQL Database Edge 服务器详细信息  。
...
本教程中的 Azure 数据工厂管道每隔一小时将数据从 SQL Database Edge 实例上的表复制到 Azure Blob 存储中的某个位置。 若要了解如何在其他方案中使用数据工厂，请参阅这些[教程](../data-factory/tutorial-copy-data-portal.md)。
