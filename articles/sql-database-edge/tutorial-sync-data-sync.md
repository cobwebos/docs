...
description: 了解如何在 Azure SQL Database Edge 与 Azure SQL 数据同步之间同步数据
...
ms.locfileid: "73509191"
---
# <a name="tutorial-sync-data-from-sql-database-edge-to-azure-sql-database-using-sql-data-sync"></a>教程：使用 SQL 数据同步将数据从 SQL Database Edge 同步到 Azure SQL 数据库

在本教程中，你将了解如何使用“SQL 数据同步”同步组  ，以增量方式将数据从 Azure SQL Database Edge 同步到 Azure SQL 数据库。 使用 SQL 数据同步这项基于 Azure SQL 数据库的服务，可以跨多个 SQL 数据库和 SQL Server 实例双向同步选定数据。 有关 Azure SQL 数据同步的详细信息，请参阅 [Azure SQL 数据同步](../sql-database/sql-database-sync-data.md)。

由于 Azure SQL Database Edge 是在最新版本的 [Microsoft SQL Server 数据库引擎](/sql/sql-server/sql-server-technical-documentation/)上构建的，因此，适用于本地 SQL Server 实例的任何数据同步机制还可以用于将数据同步到边缘设备上运行的 SQL Database Edge 实例或从该实例同步数据。

## <a name="prerequisites"></a>先决条件

本教程需要使用 [Azure SQL 数据同步代理](../sql-database/sql-database-data-sync-agent.md)配置的 Windows 计算机。
...
* 将 Azure SQL Database Edge 实例注册到 Azure SQL 数据同步代理。 有关详细信息，请参阅[添加本地 SQL Server 数据库](../sql-database/sql-database-get-started-sql-data-sync.md#add-on-prem)。
...
1. 使用 Azure 门户创建同步组。 若要创建同步组，请参阅[使用 Azure 门户创建同步组](../sql-database/sql-database-get-started-sql-data-sync.md#create-sync-group)。 同一个中心  数据库可用于创建多个不同的同步组，以将不同 SQL Database Edge 实例中的数据同步到 Azure 中的一个或多个 SQL 数据库。

2. 将同步成员添加到同步组。 若要将成员添加到同步组，请参阅[将成员添加到 SQL 数据同步组](../sql-database/sql-database-get-started-sql-data-sync.md#add-sync-members)。

3. 配置同步组以选择将作为此同步的一部分的表。 若要配置同步组，请参阅[配置同步组](../sql-database/sql-database-get-started-sql-data-sync.md#add-sync-members)。
