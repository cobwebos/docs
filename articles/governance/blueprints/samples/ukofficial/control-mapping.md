...
以下映射适用于**英国官方**和**英国 NHS**控件。 使用右侧的导航栏可直接跳转到特定的控制映射。 许多的映射控制措施都是使用 [Azure Policy](../../../policy/overview.md) 计划实施的。 若要查看完整计划，请在 Azure 门户中打开“策略”，并选择“定义”页。 然后，查找并选择 " **\[预览版"\] AUDIT UK 官方 AND UK NHS controls，并部署特定的 VM 扩展以支持审核要求**内置策略计划。
...
- \[预览\]：部署要求以审核 Linux Vm，以允许无密码的帐户进行远程连接
- \[预览\]：审核没有密码的帐户的 Linux Vm
- \[预览\]：审核允许无密码的帐户进行远程连接的 Linux Vm
