...
- \[预览\]：审核 Function App 的远程调试状态
- \[预览\]：审核 Web 应用程序的远程调试状态
- \[预览\]：审核 API 应用的远程调试状态
- \[预览\]：审核 Linux VM 是否允许通过没有密码的帐户进行远程连接
- \[预览\]：部署 VM 扩展以审核 Linux VM 是否允许从没有密码的帐户进行远程连接
- 审核对存储帐户的不受限的网络访问
...
- \[预览\]：在 Azure 安全中心监视未经审核的 SQL 服务器
- 审核诊断设置
...
- \[预览\]：审核 Linux VM 是否有不使用密码的帐户
- \[预览\]：部署 VM 扩展，审核 Linux VM 是否有不使用密码的帐户
- \[预览\]：审核 Linux VM 是否已将密码文件权限设为 0644
- \[预览\]：审核 Windows VM 是否使用可逆加密来存储密码
- \[预览\]：部署 VM 扩展，审核 Linux VM 是否已将密码文件权限设为 0644
...
- \[预览\]：审核 Windows VM 是否能够重用之前的 24 个密码
- \[预览\]：审核 Windows VM 的最长密码期限是否为 70 天
- \[预览\]：审核 Windows VM 的最短密码期限是否为 1 天
- \[预览\]：审核 Windows VM 是否已启用密码复杂性设置
- \[预览\]：审核 Windows VM 是否已将最短密码长度限制为 14 个字符
- \[预览\]：审核 Windows VM 是否使用可逆加密来存储密码
- \[预览\]：部署 VM 扩展，审核 Windows VM 是否能够重用之前的 24 个密码
- \[预览\]：部署 VM 扩展，审核 Windows VM 的最长密码期限是否为 70 天
- \[预览\]：部署 VM 扩展，审核 Windows VM 的最短密码期限是否为 1 天
- \[预览\]：部署 VM 扩展，审核 Windows VM 是否已启用密码复杂性设置
- \[预览\]：部署 VM 扩展，审核 Windows VM 是否已将最短密码长度限制为 14 个字符
- \[预览\]：部署 VM 扩展，审核 Windows VM 是否使用可逆加密来存储密码
...
- 审核没有高级数据安全的 SQL 服务器
- 在 SQL 服务器上部署高级数据安全
- \[预览\]：审核 Azure 安全中心的虚拟机规模集上的 OS 漏洞
...
- \[预览\]：监视面向虚拟机的 Internet，获取网络安全组流量强化建议
...
- \[预览\]：审核 Azure 安全中心的虚拟机规模集上是否缺少系统更新
- \[预览\]：审核 Azure 安全中心的虚拟机规模集上的 OS 漏洞
...
- \[预览\]：审核 Azure 安全中心的虚拟机规模集上的终结点保护解决方案
