---
title: 示例-PCI DSS v3.2.1 蓝图-部署步骤
description: 部署的支付卡行业数据安全标准 v3.2.1 蓝图示例的步骤。
services: blueprints
author: DCtheGeek
ms.author: dacoulte
ms.date: 06/24/2019
...
1. 查找**PCI DSS v3.2.1**下的蓝图示例_其他示例_，然后选择**使用此示例**。

1. 输入该蓝图示例的“基本信息”： 

   - **蓝图名称**：提供您的 PCI DSS v3.2.1 蓝图示例副本的名称。
...
现已在环境中创建蓝图示例的副本。 该副本在创建后处于“草稿”模式，必须先将其**发布**，然后才能分配和部署它。  蓝图示例的副本可以进行自定义您的环境和需求，但该修改可能会将其移离 PCI DSS v3.2.1 标准。

1. 选择“所有服务”，然后在左窗格中搜索并选择“策略”。   在“策略”页上选择“蓝图”。  

1. 在左侧选择“蓝图定义”页。  使用筛选器找到蓝图示例的副本，然后选择它。

1. 选择页面顶部的“发布蓝图”。  在右侧的新窗格中，提供蓝图示例副本的**版本**。 以后做出修改时，此属性非常有用。 提供**更改说明**如"第一个版本发布的 PCI DSS v3.2.1 蓝图示例"。 然后选择页面底部的“发布”。 
...
|项目名称|项目类型|参数名称|描述|
|-|-|-|-|
|[预览]:审核 PCI v3.2.1:2018 控件并将其部署特定的 VM 扩展以支持审核要求|策略分配|资源类型的列表 | 审核诊断设置所选的资源类型。 默认值是选择所有资源| 
|允许的位置|策略分配|允许的位置的列表|允许使用任何资源部署到的数据中心位置的列表。 此列表在全球可自定义对所需的 Azure 位置。 选择你想要允许的位置。| 
|允许的资源组的位置|策略分配 |允许的位置 |此策略可用于限制你的组织可以创建资源组中的位置。 用于强制执行异地符合性要求。| 
|在 SQL server 上部署审核|策略分配|保留天数|在数天的数据保留。 默认值为 180 但 PCI 需要 365。| 
|在 SQL server 上部署审核|策略分配|存储帐户的资源组名称|审核数据库将事件写入审核日志 （在其中创建 SQL Server 将由该区域中的所有服务器共享每个区域中将创建存储帐户） 在 Azure 存储帐户中。| 

## <a name="next-steps"></a>后续步骤

现在，已学习了部署 PCI DSS v3.2.1 蓝图示例的步骤，请访问以下文章以了解有关概述和控件映射：

> [!div class="nextstepaction"]
> [PCI DSS v3.2.1 蓝图-概述](./index.md)
> [PCI DSS v3.2.1 蓝图-控件映射](./control-mapping.md)
