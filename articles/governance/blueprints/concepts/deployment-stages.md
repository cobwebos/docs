...
ms.locfileid: "74463552"
---
# <a name="stages-of-a-blueprint-deployment"></a>蓝图部署的阶段

部署蓝图后，Azure 蓝图服务将执行一系列操作来部署蓝图中定义的资源。 本文详细介绍了每个步骤涉及的内容。

通过向订阅分配蓝图或[更新现有分配](../how-to/update-existing-assignments.md)来触发蓝图部署。 在部署过程中，蓝图会执行以下高级步骤：
...
在分配过程中选择[系统分配的托管标识](../../../active-directory/managed-identities-azure-resources/overview.md)时，蓝图会创建标识，并向托管标识授予[所有者](../../../role-based-access-control/built-in-roles.md#owner)角色。 如果[升级了现有分配，则](../how-to/update-existing-assignments.md)蓝图将使用之前创建的托管标识。
...
部署的访问失败通常是向托管标识授予的访问级别。 蓝图服务管理**系统分配**的托管标识的安全生命周期。 但是，用户负责管理**用户分配**的托管标识的权限和生命周期。

## <a name="blueprint-service-and-system-assigned-managed-identity-rights-are-revoked"></a>已撤销蓝图服务和系统分配的托管标识权限

部署完成后，蓝图将从订阅撤消**系统分配**的托管标识的权限。 然后，蓝图服务将撤消其对订阅的权限。 权限删除可防止蓝图成为订阅的永久所有者。
