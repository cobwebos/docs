...
- Azure PowerShell 1.5.0 or higher. 若尚未安装，请遵循[这些说明](/powershell/azure/install-az-ps)。
...
- **Path**: Output folder path. 此参数是可选的。 If not specified, the package is created in current directory.
...
For Linux policies, include the property **AttributesYmlContent** in your configuration and overwrite the values accordingly. The Guest Configuration agent automatically creates the YaML file used by InSpec to store attributes. 请参阅以下示例。
