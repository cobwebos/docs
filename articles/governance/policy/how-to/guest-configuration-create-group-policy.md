---
title: 如何从 Windows 组策略基准创建来宾配置策略定义
...
在创建自定义策略定义之前，最好阅读[Azure 策略来宾配置](../concepts/guest-configuration.md)中的概念性概述信息。 若要了解如何创建适用于 Linux 的自定义来宾配置策略定义，请参阅[如何创建适用于 linux 的来宾配置策略](./guest-configuration-create-linux.md)。 若要了解如何创建适用于 Windows 的自定义来宾配置策略定义，请参阅[如何创建适用于 windows 的来宾配置策略](./guest-configuration-create.md)。 

审核 Windows 时，来宾配置使用 [Desired State Configuration](/powershell/scripting/dsc/overview/overview) (DSC) 资源模块创建配置文件。 DSC 配置定义了计算机应处于的条件。 如果对配置的评估不**符合**，则会触发策略影响*auditIfNotExists* 。 [Azure 策略来宾配置](../concepts/guest-configuration.md)仅审核计算机内部的设置。
...
DSC 社区已发布[BaselineManagement 模块](https://github.com/microsoft/BaselineManagement)，可将导出的组策略模板转换为 DSC 格式。 BaselineManagement 模块与 GuestConfiguration cmdlet 一起为 Windows 创建 Azure 策略来宾配置包组策略内容。 有关使用 BaselineManagement 模块的详细信息，请参阅文章[快速入门：将组策略转换为 DSC](/powershell/scripting/dsc/quickstarts/gpo-quickstart)。 

在本指南中，我们将逐步介绍从组策略对象（GPO）创建 Azure 策略来宾配置包的过程。 虽然本演练概述了 Windows Server 2019 安全基线的转换，但该过程也可以应用于其他 Gpo。  
...
若要在 PowerShell 中安装**DSC**、 **GuestConfiguration**、**基线管理**和相关 Azure 模块：
...
1. 为创建一个目录，并从 Windows 安全符合性工具包下载 Windows Server 2019 安全基线。
...
1. 使用**MapGuidsToGpoNames.ps1**验证服务器2019基线内容。
...
## <a name="convert-from-group-policy-to-azure-policy-guest-configuration"></a>从组策略转换为 Azure 策略来宾配置

接下来，使用来宾配置和基线管理模块将下载的服务器2019基线转换为来宾配置包。 
...
1. 在创建策略内容包之前，重命名、重新格式化和运行转换后的脚本。
...
> [!IMPORTANT]
> 必须**始终**使用结合了_AuditIfNotExists_和_DeployIfNotExists_策略的计划来分配来宾配置策略定义。 如果只分配了 AuditIfNotExists 策略，则不会部署必备组件，并且策略始终显示“0”个服务器是符合的。
