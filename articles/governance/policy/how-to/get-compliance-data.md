...
如果已将包含 `AzureActivity` 解决方案的 [Log Analytics](../../../log-analytics/log-analytics-overview.md) 工作区绑定到订阅，则还可以使用简单的 Kusto 查询和 `AzureActivity` 表，查看评估周期中的不符合结果。 有了 Log Analytics 中的详细信息，就可以对警报进行配置，监视不符合情况。
