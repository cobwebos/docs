...
- Azure 门户：
  ...
  1. 在 Azure 门户中单击“所有服务”，然后搜索并选择“策略”，启动 Azure Policy 服务。********
     ...
     :::image type="content" source="../media/rego-for-aks/search-policy.png" alt-text="在所有服务中搜索策略" border="false":::

  1. 选择 "Azure 策略" 页左侧的 "**加入预览版**"。

     :::image type="content" source="../media/rego-for-aks/join-aks-preview.png" alt-text="加入 AKS preview 的策略" border="false":::
...
1. 安装适用_0.4.0_于 AKS 的 Azure CLI 预览扩展的版本 0.4.0 `aks-preview`，：
   ...
   > [!NOTE]
   > 如果你以前安装了_aks-预览_扩展，请使用`az extension update --name aks-preview`命令安装任何更新。
...
- Azure 门户
  ...
  1. 选择 Kubernetes 服务页面左侧的 "**策略（预览）** "。
     ...
     :::image type="content" source="../media/rego-for-aks/policies-preview-from-aks-cluster.png" alt-text="AKS 群集中的策略" border="false":::

  1. 在主页中，选择 "**启用外接程序**" 按钮。

     :::image type="content" source="../media/rego-for-aks/enable-policy-add-on.png" alt-text="启用 Azure Policy for AKS 外接程序" border="false":::
...
- Azure 门户
  ...
  1. 在主页中，选择 "**禁用外接程序**" 按钮。
     ...
     :::image type="content" source="../media/rego-for-aks/disable-policy-add-on.png" alt-text="禁用 AKS 外接程序的 Azure 策略" border="false":::
