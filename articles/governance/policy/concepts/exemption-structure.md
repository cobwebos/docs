...
Azure 策略免除 (预览版) 功能用于从计划或定义的评估中 _免除_ 资源层次结构或单个资源。 不计入总体符合性，但不能评估或临时 _弃权的资源_ 。 有关详细信息，请参阅 [了解 Azure 策略中的作用域](./scope.md)。 Azure 策略免除仅适用于 [资源管理器模式](./definition-structure.md#resource-manager-modes) ，不适用于 **资源提供程序模式**。
