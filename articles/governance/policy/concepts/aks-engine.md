...
1. 为群集服务主体创建角色分配
   ...
   - 通过 Azure CLI 将 "Policy Insights 数据写入程序（预览版）" 角色分配分配给群集服务主体应用 ID （从上一步开始，值_aadClientID_ ）。 将`<subscriptionId>`替换为你的订阅`<aks engine cluster resource group>` ID，将替换为 AKS 引擎自托管 Kubernetes 群集所在的资源组。
...
- 在现有群集中安装 Helm 图表
  ...
  1. 使用 Helm 图表安装外接程序。 将`<subscriptionId>`替换为你的订阅`<aks engine cluster resource group>` ID，将替换为 AKS 引擎自托管 Kubernetes 群集所在的资源组。
...
若要查看 Azure 策略外接程序日志，请`kubectl`使用：
...
网关守卫（_网关控制器-管理器_）通常在`gatekeeper-system`或`kube-system`命名空间中，但可以在不同的命名空间中，具体取决于它的部署方式。

若要查看网关守卫日志`kubectl`，请使用：
