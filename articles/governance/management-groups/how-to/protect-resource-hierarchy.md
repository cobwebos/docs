...
1. 选择 " **更改默认管理组** " 按钮。
   ...
   > [!NOTE]
   > 如果 " **更改默认管理组** " 按钮处于禁用状态，则所查看的管理组不是根管理组，或安全主体没有更改层次结构设置所需的权限。

1. 从层次结构中选择管理组，并使用 " **选择** " 按钮。

### <a name="set-default-management-group-with-rest-api"></a>设置默认管理组与 REST API

若要在 REST API 中配置此设置，请调用 [层次结构设置](/rest/api/resources/hierarchysettings) 终结点。 为此，请使用以下 REST API URI 和正文格式。 将 `{rootMgID}` 替换为根管理组 ID，将 `{defaultGroupID}` 替换为将成为默认管理组的管理组 ID：
...
### <a name="set-require-authorization-in-portal"></a>在门户中设置需要授权
...
1. 切换 **要求创建新管理组的权限。** 选项为 on。

   > [!NOTE]
   > 如果 **需要创建新管理组的权限。** 切换功能处于禁用状态，正在查看的管理组不是根管理组，或安全主体没有更改层次结构设置所需的权限。

### <a name="set-require-authorization-with-rest-api"></a>Set 需要授权与 REST API

若要在 REST API 中配置此设置，请调用 [层次结构设置](/rest/api/resources/hierarchysettings) 终结点。 为此，请使用以下 REST API URI 和正文格式。 此值是布尔值，因此请为该值提供 true 或 false。 值 true 允许这种保护管理组层次结构的方法：
