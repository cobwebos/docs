...
使用 Azure Resource Graph 资源管理器可以直接在 Azure 门户中保存 Resource Graph 查询。 查询有两种：_专用_和_共享_。 专用查询保存在 Azure 门户设置中。 而共享查询是 Azure 资源管理器资源，可以使用基于角色的访问控制 (RBAC) 进行管理，并使用资源锁进行保护。 这两种类型的查询都是静态加密的。
