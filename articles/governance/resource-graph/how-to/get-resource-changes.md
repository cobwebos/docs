...
- 在事件管理期间了解潜在相关的更改。__ 查询特定时段内的更改事件，以及评估更改详细信息。
...
有关从基础结构层到应用程序部署的更改的详细信息，请参阅在 Azure 监视器[中使用应用程序更改分析（预览）。](../../../azure-monitor/app/change-analysis.md)

> [!NOTE]
> Resource Graph 中的更改详细信息适用于资源管理器属性。 有关在虚拟机内跟踪更改，请参阅 Azure 自动化的[更改跟踪](../../../automation/automation-change-tracking.md)或 Azure 策略的[VM 来宾配置](../../policy/concepts/guest-configuration.md)。
...
- **资源**\[Id\]必需 ：要查找更改的 Azure 资源。
- \]**需要间隔**\[：具有_开始_日期和_结束_日期的属性，用于何时使用**祖鲁时区 （Z）** 检查更改事件。
- **提取属性更改**（可选）：如果响应对象包含属性更改，则设置布尔属性。
...
- **changeType** - 描述已针对 **beforeSnapshot** 和 **afterSnapshot** 之间的整个更改记录检测到的更改的类型。 值为：_创建_、_更新_和_删除_。 仅当 **changeType** 为 _Update_ 时，才包括 **propertyChanges** 属性数组。
- **propertyChanges** - 此属性数组提供了 **beforeSnapshot** 和 **afterSnapshot** 之间已更新的所有资源属性的详细信息：
  - **propertyName** - 已更改的资源属性的名称。
  - **changeCategory** - 描述哪类更改者执行了更改。 值为：_系统和__用户_。
...
- **资源 Id**：要比较更改的 Azure 资源。
- **更改 Id**： 从**资源更改**中收集的**资源 Id**的唯一更改事件。
