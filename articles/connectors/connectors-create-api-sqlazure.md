---
title: 连接到 SQL 服务器或 Azure SQL 数据库
...
* 本地计算机上安装[的本地数据网关](../logic-apps/logic-apps-gateway-install.md)和[Azure 门户中为这些方案创建的 Azure 数据网关资源](../logic-apps/logic-apps-gateway-connection.md)：

  * 逻辑应用不在[集成服务环境 （ISE）](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md)中运行。

  * 逻辑应用*确实*在集成服务环境中运行，但您必须为 SQL Server 连接使用 Windows 身份验证。 对于此方案，请使用 SQL Server 连接器的非 ISE 版本以及数据网关，因为 ISE 版本不支持 Windows 身份验证。
