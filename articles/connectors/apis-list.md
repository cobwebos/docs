...
> [!TIP]
> （本主题中的）[A-Z 完整列表](#az)列出了所有可以用在逻辑应用中的连接器。 [连接器 swagger](/connectors/) 列出每个连接器的 swagger 详细信息。
...
连接器还提供可在工作流中使用的**操作**。 例如，可以通过逻辑应用查找数据，然后再在逻辑应用中使用该数据。 更具体地说，可以在 SQL 数据库中查找客户数据，然后使用该客户数据生成工作流。 
