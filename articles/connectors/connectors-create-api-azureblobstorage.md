...
> [!IMPORTANT]
> 如果逻辑应用都位于同一区域，则无法直接访问防火墙后面的存储帐户。 作为解决方法，您可以将逻辑应用和存储帐户放在不同的区域。 有关将 Azure 逻辑应用访问启用到防火墙后面的存储帐户的详细信息，请参阅本主题后面的[防火墙后面的访问存储帐户](#storage-firewalls)部分。

<a name="blob-storage-limits"></a>

## <a name="limits"></a>限制

* 默认情况下，Azure Blob 存储操作可以读取或写入*50 MB 或较小的*文件。 要处理大于 50 MB 但高达 1024 MB 的文件，Azure Blob 存储操作支持[消息分块](../logic-apps/logic-apps-handle-large-messages.md)。 **获取 blob 内容**操作隐式使用分块。

* Azure Blob 存储触发器不支持分块。 请求文件内容时，触发器仅选择 50 MB 或更小的文件。 若要获取大于 50 MB 的文件，请遵循以下模式：

  * 使用返回文件属性的 Azure Blob 存储触发器，例如**添加或修改 Blob 时（仅限属性）。**

  * 使用 Azure Blob 存储**获取 Blob 内容**操作遵循触发器，该操作读取完整文件并隐式使用分块。
...
此示例演示如何在存储容器中添加或更新 Blob 的属性时，使用 **"何时添加或修改 Blob时"触发器启动**逻辑应用工作流。

1. 在[Azure 门户](https://portal.azure.com)或可视化工作室中，创建一个空白逻辑应用，该应用将打开逻辑应用设计器。 此示例使用 Azure 门户。

2. 在搜索框中，输入“azure blob”作为筛选器。 在触发器列表中，选择所需的触发器。

   此示例使用此触发器：**添加或修改 Blob 时（仅限属性）**
...
3. 如果系统提示输入连接详细信息，请[立即创建 Blob 存储连接](#create-connection)。 或者，如果连接已存在，请提供触发器所需的信息。
   ...
   2. 在文件夹列表中，选择直角括号 （），**>** 然后浏览，直到找到并选择所需的文件夹。
...
或者，如果连接已存在，请提供操作所需的信息。
   ...
   2. 根据 Blob 的**ID**号查找并选择所需的文件。 您可以在前面描述的 Blob 存储触发器返回的 Blob 元数据中找到此**ID**号。
...
1. 当系统提示您创建连接时，请提供以下信息：

   | properties | 必选 | “值” | 描述 |
   |----------|----------|-------|-------------|
   | **连接名称** | 是 | <*连接名称*> | 为连接创建的名称 |
   | **存储帐户** | 是 | <*存储帐户*> | 从列表中选择存储帐户。 |
   ||||
...
有关此连接器的更多技术详细信息，例如连接器的 Swagger 文件所述的触发器、操作和限制，请参阅[连接器的参考页](https://docs.microsoft.com/connectors/azureblobconnector/)。
...
## <a name="access-storage-accounts-behind-firewalls"></a>在防火墙后面访问存储帐户

您可以通过限制防火墙[和防火墙规则](../storage/common/storage-network-security.md)的访问，将网络安全添加到 Azure 存储帐户。 但是，此设置会给 Azure 和其他需要访问存储帐户的 Microsoft 服务带来挑战。 数据中心中的本地通信会抽象内部 IP 地址，因此无法设置带有 IP 限制的防火墙规则。 有关详细信息，请参阅[配置 Azure 存储防火墙和虚拟网络](../storage/common/storage-network-security.md)。

以下是使用 Azure Blob 存储连接器或其他解决方案从 Azure 逻辑应用访问防火墙后面的存储帐户的各种选项：

* Azure 存储 Blob 连接器

  * [访问其他区域的存储帐户](#access-other-regions)
  * [通过受信任的虚拟网络访问存储帐户](#access-trusted-virtual-network)

* 其他解决方案

  * [使用托管标识将存储帐户访问为受信任服务](#access-trusted-service)
  * [通过 Azure API 管理访问存储帐户](#access-api-management)

<a name="access-other-regions"></a>

### <a name="problems-accessing-storage-accounts-in-the-same-region"></a>访问同一区域中的存储帐户时遇到问题

逻辑应用不能直接访问防火墙后面的存储帐户，因为它们都位于同一区域。 作为解决方法，将逻辑应用放在与存储帐户不同的区域中，并授予对[区域中托管连接器的出站 IP 地址](../logic-apps/logic-apps-limits-and-config.md#outbound)的访问权限。

> [!NOTE]
> 此解决方案不适用于 Azure 表存储连接器和 Azure 队列存储连接器。 相反，要访问表存储或队列存储，请使用内置的 HTTP 触发器和操作。

<a name="access-trusted-virtual-network"></a>

### <a name="access-storage-accounts-through-a-trusted-virtual-network"></a>通过受信任的虚拟网络访问存储帐户

您可以将存储帐户放在您管理的 Azure 虚拟网络中，然后将该虚拟网络添加到受信任的虚拟网络列表中。 要让逻辑应用通过[受信任的虚拟网络](../virtual-network/virtual-networks-overview.md)访问存储帐户，您需要将该逻辑应用部署到[集成服务环境 （ISE），该环境](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md)可以连接到虚拟网络中的资源。 然后，您可以将该 ISE 中的子网添加到受信任的列表中。 Azure 存储连接器（如 Blob 存储连接器）可以直接访问存储容器。 此设置与使用 ISE 中的服务终结点具有相同的体验。

<a name="access-trusted-service"></a>

### <a name="access-storage-accounts-as-a-trusted-service-with-managed-identities"></a>使用托管标识将存储帐户访问为受信任服务

要通过防火墙授予 Microsoft 受信任的服务对存储帐户的访问权限，可以在这些服务的存储帐户上设置异常。 此解决方案允许支持[托管标识的](../active-directory/managed-identities-azure-resources/overview.md)Azure 服务进行身份验证，以将防火墙后面的存储帐户作为受信任的服务访问。 具体而言，对于全局多租户 Azure 中的逻辑应用访问这些存储帐户，首先在逻辑应用上[启用托管标识支持](../logic-apps/create-managed-service-identity.md)。 然后，在逻辑应用中使用 HTTP 操作或触发器，并将[其身份验证类型设置为使用逻辑应用的托管标识](../logic-apps/create-managed-service-identity.md#authenticate-access-with-managed-identity)。 对于此方案，*只能使用*HTTP 操作或触发器。

要设置异常和托管标识支持，请按照以下一般步骤操作：

1. 在存储帐户上，在 **"设置"** 下，选择**防火墙和虚拟网络**。 在 **"允许从 "访问**"下，选择 **"选定网络"** 选项，以便显示相关设置。

1. 在 **"例外**"下，选择 **"允许受信任的 Microsoft 服务访问此存储帐户**"，然后选择"**保存**"。

   ![选择允许 Microsoft 受信任服务的异常](./media/connectors-create-api-azureblobstorage/allow-trusted-services-firewall.png)

1. 在逻辑应用的设置中，[启用对托管标识的支持](../logic-apps/create-managed-service-identity.md)。

1. 在逻辑应用的工作流中，添加和设置 HTTP 操作或触发器以访问存储帐户或实体。

   > [!IMPORTANT]
   > 对于传出 HTTP 操作或对 Azure 存储帐户的触发器调用，请确保请求标头`x-ms-version`包含要在存储帐户上运行的操作的属性和 API 版本。 有关详细信息，请参阅[使用托管标识进行身份验证访问权限](../logic-apps/create-managed-service-identity.md#authenticate-access-with-managed-identity)和[Azure 存储服务的版本化](https://docs.microsoft.com/rest/api/storageservices/versioning-for-the-azure-storage-services#specifying-service-versions-in-requests)。

1. 在此操作[中，选择要用于身份验证的托管标识](../logic-apps/create-managed-service-identity.md#authenticate-access-with-managed-identity)。

<a name="access-api-management"></a>

### <a name="access-storage-accounts-through-azure-api-management"></a>通过 Azure API 管理访问存储帐户

如果使用 API[管理的](../api-management/api-management-key-concepts.md)专用层，则可以使用 API 管理和允许后者的 IP 地址通过防火墙在存储 API 前面。 基本上，将 API 管理使用的 Azure 虚拟网络添加到存储帐户的防火墙设置中。 然后使用 API 管理操作或 HTTP 操作调用 Azure 存储 API 即可。 但是，如果选择此选项，则需自行处理身份验证过程。 有关详细信息，请参阅[简单的企业集成体系结构](https://aka.ms/aisarch)。
