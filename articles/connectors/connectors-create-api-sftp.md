---
title: '连接到 SFTP 帐户 (弃用) '
description: 使用 Azure 逻辑应用自动执行监视、创建、管理、发送和接收 SFTP 服务器文件的任务和过程
services: logic-apps
...
有关触发器、操作和限制（请参阅连接器的 OpenAPI（以前称为 Swagger）说明）的技术详细信息，请查看连接器的[参考页](/azure/data-factory/connector-sftp)。
