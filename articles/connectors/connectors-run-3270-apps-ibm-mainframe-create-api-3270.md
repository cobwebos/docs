...
* 推荐：[集成服务环境 （ISE）](../logic-apps/connect-virtual-network-vnet-isolated-environment.md) 
...
* **捕获**：在此模式下，使用大型机应用记录完成特定任务所需的屏幕，例如，获取银行余额。

* **导航**：在此模式下，您可以指定如何浏览大型机应用屏幕的计划或路径，以便执行特定任务。

* **方法**：在此模式下，定义描述屏幕导航路径的方法，例如`GetBalance`，描述屏幕导航路径。 还可以选择每个屏幕中要用作方法的输入和输出参数的字段。

### <a name="unsupported-elements"></a>不支持的元素

设计工具不支持以下元素：

* 部分 IBM 基本映射支持 （BMS） 映射：如果导入 BMS 地图，则设计工具将忽略部分屏幕定义。
* 输入/出参数：无法定义输入/出参数。
* 菜单处理：预览期间不支持
* 阵列处理：预览期间不支持
...
#### <a name="example-identify-repeated-screens"></a>示例：识别重复的屏幕
...
1. 添加方法的所有参数后，请定义每个参数的以下属性：
   ...
   | **字段填充方法** | 参数支持以下填充类型，必要时留空： <p><p>- **类型**：在字段中按顺序输入字符。 <p>- **填充**：将字段的内容替换为字符，必要时用空格填充。 <p>- **擦除类型**：清除字段，然后按顺序在字段中输入字符。 |
   | **格式字符串** | 某些参数数据类型使用格式字符串，告知 3270 连接器如何将屏幕中的文本转换为 .NET 数据类型： <p><p>- **日期时间**： 日期时间格式字符串遵循[.NET 自定义日期和时间格式字符串](https://docs.microsoft.com/dotnet/standard/base-types/custom-date-and-time-format-strings)。 例如，日期 `06/30/2019` 使用格式字符串 `MM/dd/yyyy`。 <p>- **十进制**： 十进制格式字符串使用[COBOL 图片子句](https://www.ibm.com/support/knowledgecenter/SS6SG3_5.2.0/com.ibm.cobol52.ent.doc/PGandLR/ref/rlddepic.html)。 例如，数字 `100.35` 使用格式字符串 `999V99`。 |
   |||
...
1. 在搜索框下，选择“企业”。**** 在搜索框中，输入“3270”作为筛选器。 从操作列表中选择此操作：**通过 TN3270 连接运行大型机程序**
...
1. 如果尚不存在任何连接，请提供连接的所需信息，然后选择“创建”。****
   ...
   |----------|----------|-------|-------------|
   | **连接名称** | 是 | <*连接名称*> | 连接名称 |
   | **集成帐户 ID** | 是 | <*集成帐户名称*> | 集成帐户的名称 |
   | **集成帐户 SAS URL** | 是 | <*集成帐户-SAS-URL*> | 集成帐户的共享访问签名 (SAS) URL，可以在 Azure 门户中基于集成帐户的设置生成此 URL。 <p>1. 在您的集成帐户菜单中，**在"设置"** 下，选择**回拨 URL**。 <br>2. 在右侧窗格中，复制**生成的回调 URL**值。 |
   | **服务器** | 是 | <*TN3270 服务器名称*> | TN3270 服务的服务器名称 |
   | **港口** | 否 | <*TN3270 服务器端口*> | TN3270 服务器使用的端口。 如果留空，则连接器将使用 `23` 作为默认值。 |
   | **设备类型** | 否 | <*IBM 终端模型*> | 要仿真的 IBM 终端的机型名称或型号。 如果留空，则连接器将使用默认值。 |
   | **代码页** | 否 | <*代码页编号*> | 主机的代码页号。 如果留空，则连接器将使用 `37` 作为默认值。 |
   | **逻辑单元名称** | 否 | <*逻辑单位名称*> | 要从主机请求的特定逻辑单元名称 |
   | **启用 SSL？** | 否 | 打开或关闭 | 打开或关闭 TLS 加密。 |
...
1. 提供操作的所需信息：
   ...
   | properties | 必选 | “值” | 描述 |
   |----------|----------|-------|-------------|
   | **Hidx 名称** | 是 | <*HIDX 文件名*> | 选择要使用的 3270 HIDX 文件。 |
   | **方法名称** | 是 | <*方法名称*> | 选择要使用的 HIDX 文件中的方法。 选择方法后，“添加新参数”列表将会显示，可以从中选择要对该方法使用的参数。**** |
   ||||
...
有关此连接器的更多技术详细信息，例如连接器的 Swagger 文件所述的触发器、操作和限制，请参阅[连接器的参考页](https://docs.microsoft.com/connectors/si3270/)。
