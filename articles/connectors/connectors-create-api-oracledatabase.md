...
* 安装本地数据网关。 [从逻辑应用连接到本地数据](../logic-apps/logic-apps-gateway-connection.md)一文列出了相关步骤。 若要连接到 Oracle 数据库，或者连接到安装了 Oracle DB 的 Azure VM，网关是必需的。 
    ...
    > [!NOTE]
    > 本地数据网关的作用好似一架桥，提供本地数据（不在云中的数据）与逻辑应用之间的安全数据传输。 可以将同一网关用于多个服务和多个数据源。 因此，你可能只需要安装一次网关。

* 将 Oracle 客户端与本地数据网关安装在同一计算机上。 请确保从 Oracle 安装适用于 .NET 的64位 Oracle 数据提供程序：  
...
**缓解措施**：请确保网关正在安装了该网关的本地计算机上运行，并确保该网关可以连接到 Internet。  建议不要在可能已关闭或睡眠的计算机上安装网关。 还可以重启本地数据网关服务（PBIEgwService）。
