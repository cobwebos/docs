...
可以使用触发器从服务总线获取响应，并使输出可供逻辑应用中的其他操作使用。 还可以让其他操作使用服务总线操作的输出。 如果你不太熟悉服务总线和逻辑应用，请查看[什么是 Azure 服务总线？](../service-bus-messaging/service-bus-messaging-overview.md) [什么是 Azure 逻辑应用？](../logic-apps/logic-apps-overview.md)
...
1. 若要将触发器添加到空白逻辑应用，请在搜索框中输入“Azure 服务总线”作为筛选器。 在触发器列表下，选择所需的触发器。 
   ...
   例如, 若要在将新项发送到服务总线队列时触发逻辑应用, 请选择此触发器:**在队列中收到消息时 (自动完成)**
