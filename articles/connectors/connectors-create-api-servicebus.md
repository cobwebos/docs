...
description: 创建自动任务和工作流，这些任务和工作流通过使用 Azure 逻辑应用中的 Azure 服务总线发送和接收邮件
services: logic-apps
...
使用[Azure 逻辑应用](../logic-apps/logic-apps-overview.md)和[Azure 服务总线](../service-bus-messaging/service-bus-messaging-overview.md)连接器，可以创建自动任务和工作流来传输数据，例如销售和采购订单、日志以及组织跨应用程序进行库存移动。 连接器不仅会监视、发送和管理消息，而且还能对队列、会话、主题、订阅等执行操作，例如：
...
1. 转到服务总线的命名空间。** 在命名空间页上的“设置”下，选择“共享访问策略”。******** 在 **"声明"** 下，检查您是否具有该命名空间**的管理**权限。
...
1. 在搜索框中，输入“Azure 服务总线”作为筛选器。 在触发器列表中，选择所需的触发器。
   ...
   某些触发器（如**一个或多个消息到达队列（自动完成）触发器时**，可以返回一个或多个消息。 这些触发器在触发时返回的消息数至少为 1，至多为触发器的**最大消息计数**属性指定的消息数。
...
1. 在要添加操作的步骤下，选择“新建步骤”。****
   ...
   或者将鼠标指针移到这些步骤之间的箭头上，以便在步骤之间添加操作。 选择显示的加号**+**（），然后选择 **"添加操作**"。
...
有关触发器、操作和限制（请参阅连接器的 OpenAPI（以前称为 Swagger）说明）的其他技术详细信息，请查看连接器的[参考页](/connectors/servicebus/)。 有关 Azure 服务总线消息传递的信息，请参阅[什么是 Azure 服务总线](../service-bus-messaging/service-bus-messaging-overview.md)？
