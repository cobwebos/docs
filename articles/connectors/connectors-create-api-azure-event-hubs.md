...
1. 在 "命名空间" 菜单的 "**设置**" 下，选择 "**共享访问策略**"。 在“声明”下，检查你是否有该命名空间的“管理”权限。
...
1. 在搜索框中，输入“事件中心”作为筛选器。 从 "触发器" 列表中，选择此触发器：事件**在事件中心内可用时-事件**中心
...
1. 在触发器中，提供有关要监视的事件中心的信息。 有关更多属性，请打开 "**添加新参数**" 列表。 选择参数会将该属性添加到触发器卡。
...
1. 在操作中，提供有关要发送的事件的信息。 有关更多属性，请打开 "**添加新参数**" 列表。 选择参数会将该属性添加到操作卡。
