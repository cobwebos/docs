...
或者，如果连接已存在，请提供触发器所需的信息。
   ...
   | properties | 必选 | “值” | 描述 |
   |----------|----------|-------|-------------|
   | 搜索查询 | 是 | <*搜索字*> | 输入要使用的搜索关键字。 |
   | 市场 | 是 | <*现场*> | 搜索区域设置。 默认为“zh-CN”，但可以选择另一个值。 |
   | 安全搜索 | 是 | <*搜索级别*> | 用于排除成人内容的筛选级别。 默认为“中等”，但可以选择另一个级别。 |
   | Count | 否 | <*结果计数*> | 返回指定数量的结果。 默认为 20，但可以指定另一个值。 实际返回的结果数可能会少于指定的数量。 |
   | Offset | 否 | <*跳过值*> | 返回结果前要跳过的结果数 |
   |||||
