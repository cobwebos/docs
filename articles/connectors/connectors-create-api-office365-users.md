...
description: 使用 Azure 逻辑应用自动执行获取和管理 Office 365 用户配置文件中配置文件的任务和工作流
services: logic-apps
...
将此连接器添加到逻辑应用时，必须登录到 Office 365 用户帐户，以便 Azure 逻辑应用可以连接到您的帐户。
...
有关触发器、操作和限制的技术详细信息（由连接器的 Swagger 描述）描述，请查看[连接器的参考页](/connectors/officeusers/)。
