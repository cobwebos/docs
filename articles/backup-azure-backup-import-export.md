...
借助 Azure 备份和 Azure 导入/导出，可以简单直接地通过磁盘将数据脱机上载到 Azure。而不是通过网络传输初始的完整副本，备份数据将写入 *暂存位置*。暂存位置可以是直接附加存储，也可以是网络共享。使用  *Azure 导入/导出工具*完成初始复制后，此数据将写入 SATA 驱动器，最终传送到 Azure 数据中心。根据初始备份的大小，可能需要一个或多个 SATA 驱动器来完成此操作。用于这些方案的 Azure 导入/导出工具。将备份写入磁盘后，可将其传送到最接近的数据中心，以便上载到 Azure。然后，Azure 备份会将备份数据复制到备份保管库并计划增量备份。

## 先决条件

1. 你必须熟悉[此处](storage-import-export-service.md)列出的 Azure 导入导出工作流。
...
3. 下载中的 Azure 发布文件设置[此处](https://manage.windowsazure.com/publishsettings)你计划以备份我们的数据的计算机上。

4. 准备 *暂存位置*这可能是网络共享或计算机上的其他驱动器。确保临时位置具有足够的磁盘空间来保存你的初始副本。对于如如果你尝试进行备份 500GB 文件服务器，确保临时区域是至少为 500GB（尽管将使用较少）。暂存区域是 '短时存储'并暂时在此工作流期间使用。
...
5. SATA 驱动器编写器连接到计算机上启用 BitLocker。

6. 下载 Azure 导入/导出工具从[此处](http://go.microsoft.com/fwlink/?LinkID=301900&clcid=0x409)到 SATA 驱动器编写器连接到计算机。


## 工作流
本部分中提供的信息是用于完成**脱机备份**工作流，因此可以传递到 Azure 数据中心你的数据，并将其上载到 Azure 存储。如果你有关于导入服务或进程的任何方面的问题，请参阅引用导入服务概述[上面](storage-import-export-service.md)。

### 启动脱机备份

1. 作为计划备份的一部分，你会遇到下面的屏幕（在 Windows Server 中，Windows 客户端或 SCDPM）。 <br/>
  ![ImportScreen][1]
...
+ **暂存位置** - 向其写入初始备份副本的临时存储位置。这可能是网络共享上或在本地计算机上。

+ **Azure 导入作业名称** - 作为完成此工作流的一部分，你将需要创建 *导入作业*在 Azure 门户中（在文档的更高版本的一部分中介绍）。提供了你计划更高版本在 Azure 门户中使用的输入。

+ **Azure 发布设置** - 一个 XML 文件，其中包含有关你订阅的个人资料信息。它还包含到你的订阅关联的安全凭据。可以从下载该文件[此处](https://manage.windowsazure.com/publishsettings)。提供发布设置文件的本地路径。

+ **Azure 订阅 ID** -提供计划来启动 Azure 导入作业的 Azure 订阅 ID。如果你有多个 Azure 订阅，使用与导入作业关联的 ID。
...
完成工作流，然后选择**立即备份**在 Azure 备份 mmc 中，若要启动脱机备份副本。初始备份写入到临时区域作为此步骤的一部分。<br/>

  ![Backup now][2]

通过单击启用相应的工作流中 SCDPM**保护组**，然后选择**创建恢复点**选项。这一切后跟选择**在线保护**选项。<br/>
  ![DPM Backup now][9]

操作完成后，在临时位置中创建 *。AIBBlob* 和 *。BaseBlob* 文件。 <br/>
![Output][3]

### 准备 SATA 驱动器

1. 下载 [Microsoft Azure 导入/导出工具](http://go.microsoft.com/fwlink/?linkid=301900&clcid=0x409)到 *复制计算机*。确保可从你打算运行下一个命令集的计算机访问的临时位置（在上一步）。如果需要，请在复制计算机可以与源计算机相同。
...
|/id:<*SessionId*> | 会话 ID 将标识 *复制会话*。它用于确保准确恢复中断的复制会话。复制会话中复制的文件将存储在以目标驱动器上的会话 ID 命名的目录中。|
| /sk:<*StorageAccountKey*> | 将数据导入到的存储帐户的帐户密钥。|
|/ BlobType | 指定 **PageBlob**，仅当指定 PageBlob 选项时，此工作流将会成功。这不是默认选项，并应在此命令中所述。 |
...
### 在 Azure 门户中创建导入作业
1. 导航到你的存储帐户[管理门户](https://manage.windowsazure.com/)，然后单击**导入/导出**，然后在**创建导入作业**的任务窗格中。 <br/>
![Portal][5]
...
## 后续步骤
+ 如有 Azure 导入/导出工作流方面的任何问题，请参阅此[文章](storage-import-export-service.md)。

+ 如有工作流方面的任何问题，请参阅 Azure 备份[常见问题](backup-azure-backup-faq.md)的"脱机备份"部分
