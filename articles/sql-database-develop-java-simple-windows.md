<properties
	pageTitle="在 Windows 上配合使用 Java 和 JDBC 连接到 SQL Database"
	description="演示了一个可以用来连接到 Azure SQL Database 的 Java 代码示例。该示例使用 JDBC，并在 Windows 客户端计算机上运行。"
...
# 在 Windows 上配合使用 Java 和 JDBC 连接到 SQL Database


本主题演示了一个可以用来连接到 Azure SQL Database 的 Java 代码示例。该 Java 示例依赖于 Java 开发工具包 (JDK) 版本 1.8。该示例将使用 JDBC 驱动程序连接到 Azure SQL Database。
...
本主题中的 Java 代码示例假设 Azure SQL Database 数据库中已存在以下测试表。
...
该代码示例将使用连接字符串创建 `Connection` 对象。有关查找连接字符串的详细信息，请参阅[创建你的第一个 Azure SQL Database](sql-database-get-started)。
