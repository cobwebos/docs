...
> 注意：通过引入[对应用服务环境的 ILB支持](/documentation/articles/app-service-environment-with-internal-load-balancer/)，可以将 ASE 配置为不可从外围网络访问，而仅可供专用网络访问。

## 配置 App Service 环境 ##
若要配置 App Service 环境，请参阅有关该主题的[文档](/documentation/articles/app-service-web-how-to-create-an-app-service-environment/)。创建 App Service 环境后，可在此环境中创建 [Web Apps](/documentation/articles/app-service-web-overview/)、[API Apps](/documentation/articles/app-service-api-apps-why-best-platform/) 和 [Mobile Apps](/documentation/articles/app-service-mobile-value-prop/)，这些应用将受下一部分中配置的 WAF 保护。
...
### 将终结点添加云服务 ###
云服务中有两个以上的 WAF VM 实例之后，即可使用 [Azure 门户预览版](https://portal.azure.cn/)添加应用程序使用的 HTTP 和 HTTPS 终结点，如下图所示。
...
使用浏览器浏览到云服务上的管理终结点。如果云服务名为 test.chinacloudapp.cn，则浏览到 http://test.chinacloudapp.cn:8000 即可访问此终结点。你应会看到与下面类似的登录页，在此页上，可以使用你在 WAF VM 设置阶段指定的凭据登录。
...
## 配置 Azure 流量管理器（可选） ##
如果多个区域中都有你的应用程序，则你可以使用 [Azure 流量管理器](/documentation/articles/traffic-manager-overview/)对这些区域进行负载平衡。为此，可以使用流量管理器配置文件中 WAF 的云服务名称在 [Azure 经典管理门户](https://manage.windowsazure.cn)中添加终结点，如下图所示。

![流量管理器终结点][TrafficManagerEndpoint]

如果你的应用程序需要身份验证，请确保有某个资源不需要任何身份验证，使流量管理器能够 ping 出应用程序的可用性。可以在 [Azure 经典管理门户](https://manage.windowsazure.cn)的“配置”部分下配置 URL，如下所示。
