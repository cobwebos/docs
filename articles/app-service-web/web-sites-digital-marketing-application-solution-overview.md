<properties 
	...
	description="本指南提供如何使用 Azure App Service Web Apps 创建数字市场营销活动的技术概述。这包括部署、社交媒体集成、缩放策略和监视。" 
	editor="jimbe" 
	manager="wpickett" 
	authors="cephalin" 
...
> [AZURE.NOTE]本指南介绍了一些与在 Azure App Service Web Apps 中运行数字市场营销活动一致的最常见领域和任务。但是，还有其他你可以在 App Service Web Apps 中实现的常见解决方案。若要查看这些解决方案，请参阅[全球网络影响力](web-sites-global-web-presence-solution-overview.md)和[业务应用程序](web-sites-business-application-solution-overview.md)中的其他指南。

## 从头开始创建或引入现有资产

快速从库中的常用 CMS 创建新的 Web 应用或者从各种语言和框架将现有 Web 资产引入到 App Service Web Apps。

Azure 应用商店提供了来自流行网站内容管理系统 (CMS) 的模板，例如[Orchard]、[Umbraco]、Drupal 和 [WordPress]。你可以使用你最喜欢的 CMS 风格创建 Web 应用。你可以从各种数据库后端选择以满足你的需要，包括 [Azure SQL 数据库]和 [MySQL].
...
通过直接从现有的源控件连续发布并在 App Service Web Apps 中运行 A/B 测试保持敏捷

在 Web 应用的计划、原型制作和早期部署过程中，你及你的客户可以通过[将其部署到过渡槽]在市场活动应用正式运行前查看该网站的实际工作版本。通过将源控件与 App Service Web Apps 相集成，你可以[连续发布]到过渡槽，并且在准备就绪时，将其切换到生产环境且无需停机。
...
App Service Web Apps 中的数字市场营销活动可以通过使用 Facebook 和 Twitter 等常用的提供程序进行身份验证，与社交媒体相集成。有关针对 ASP.NET 应用程序的此方法的示例，请参阅[创建包含身份验证和 SQLDB 的 ASP.NET MVC 应用并将其部署到 Azure App Service]。
...
- [App Service Web Apps 文档](/services/app-service/web/)
- [适用于 Azure App Service Web Apps 的学习路线图](websites-learning-map.md)
...
[创建包含身份验证和 SQLDB 的 ASP.NET MVC 应用并将其部署到 Azure App Service]: web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database.md
