...
本教程说明如何通过从流行的云存储服务（例如 Dropbox 与 OneDrive）同步内容，来部署到 [Azure Web 应用](/documentation/services/web-sites/)。

## <a name="overview"></a>内容同步部署概述

按需内容同步部署的后盾是与 Azure Web 应用集成的 [Kudu 部署引擎](https://github.com/projectkudu/kudu/wiki)。在 [Azure 门户](https://portal.azure.cn)上，你可以在云存储中指定一个文件夹，然后使用该文件夹中的应用代码和内容，并通过单击按钮同步到 Azure。内容同步利用 Kudu 进程来完成生成和部署。
    
## <a name="contentsync"></a>如何启用内容同步部署
若要从 [Azure 门户](https://portal.azure.cn)启用内容同步，请遵循以下步骤：

1. 在 Azure 门户上的应用边栏选项卡中，单击“设置”>“部署源”。单击“选择源”，然后选择“OneDrive”或“Dropbox”作为部署源。 
