...
<properties
	pageTitle="从 Azure 库创建 Web 应用 | Azure"
	description="了解如何使用 Azure 门户从 Azure 库创建新的 WordPress Web 应用。"
...
Azure 库提供了由 Microsoft、第三方公司和开源软件计划开发的各种流行 Web 应用。例如，WordPress、Umbraco CMS、Drupal 等。这些 Web 应用基于各种常用的框架，例如此 WordPress 示例中的 [PHP]，以及 [.NET]、[Node.js]、[Java]、[Python] 等。若要从 Azure 库创建 Web 应用，你所需要的唯一软件就是用于 [Azure 门户]的浏览器。

在本教程中，你将了解如何：

* 在 Azure 库中查找应用程序模板。
* 在 Azure 中根据模板创建 Web 应用。
* 配置适用于新 Web 应用和数据库的 Azure 设置。

在本教程中，你将从 Azure 库部署一个 WordPress 博客站点。完成本教程中的步骤后，你将在云中启动并运行自己的 WordPress 站点。

![示例 WordPress Wep 应用仪表板][WordPressDashboard]

在本教程中部署的 WordPress 站点使用 MySQL 作为数据库。如果想要改用 SQL 数据库作为数据库，请参阅 [Project Nami（项目 Nami）]，Azure 库中也提供了该项目。
...
1. 随后将显示 WordPress 设置边栏选项卡，你可以使用它来完成以下步骤：

	![配置 WordPress Web 应用设置][ConfigStart]

1. 在“Web 应用”框中输入 Web 应用的名称。

	该名称在 chinacloudsites.cn 域中必须是唯一的，因为 Web 应用的 URL 将是 *{名称}*.chinacloudsites.cn。如果你输入的名称不是唯一的，则会在文本框中显示一个红色的感叹号。
...
1. 选择“资源组”或新建一个。
	...
	有关资源组的详细信息，请参阅 [Using the Azure Portal to manage your Azure resources（使用 Azure 门户管理 Azure 资源）][ResourceGroups]。

	![配置 Web 应用的资源组][ConfigResourceGroup]

1. 选择“App Service 计划/位置”或新建一个。

	有关 App Service 计划的详细信息，请参阅 [Azure App Service plans overview（Azure App Service 计划概述）][AzureAppServicePlans]。
...
1. 随后将打开站点的“仪表板”页，其中显示了你提供的信息。
	...
	![查看 WordPress 仪表板][WordPressDashboard]

## 后续步骤

在本教程中，你已了解如何从 Azure 库创建和部署一个示例 Web 应用。

有关如何使用 Azure Web Apps 的详细信息，请参阅页面左侧（针对宽屏浏览器窗口）或页面顶部（针对窄屏浏览器窗口）的链接。

有关在 Azure 上开发 WordPress Web 应用的详细信息，请参阅 [Developing WordPress on Azure Web App（在 Azure Web 应用中开发 WordPress）][WordPressOnAzure]。
...
[Try Azure Web App（试用 Azure Web 应用）]: https://tryappservice.azure.com/
...
[Project Nami（项目 Nami）]: http://projectnami.org/
