...
在本教程中，我们将使用 [Python Tools for Visual Studio] 通过一个 PTVS 样本模板创建简单的轮询 Web 应用。 本教程也作为[视频](https://www.youtube.com/watch?v=GJXDGaEPy94)提供。

轮询 Web 应用定义其存储库的抽象，因此您可以轻松地在不同类型存储库（内存中、Azure 表存储、MongoDB）之间进行切换。

我们将了解如何创建 Azure 存储帐户、如何将 Web 应用配置为使用 Azure 表存储，以及如何将 Web 应用发布到 [Azure App Service Web 应用](http://go.microsoft.com/fwlink/?LinkId=529714)中。

请参阅 [Python 开发人员中心]以获取更多文章，这些文章介绍了如何通过 PTVS（使用 Bottle、Flask 和 Django Web 框架）、MongoDB、Azure 表存储、MySQL 和 SQL 数据库服务开发 Azure App Service Web 应用。 虽然本文将着重介绍 App Service，但步骤与 [Azure 云服务]的开发步骤类似。
...
1. 在 Visual Studio 中，右键单击解决方案资源管理器中的项目节点，然后选择“属性”。 单击“调试”选项卡。
...
4. 使用 `F5`运行应用程序。 使用“创建样本轮询”创建的轮询以及通过投票提交的数据将在 Azure 表存储中进行序列化。
...
1. 在 [Azure 门户]中，依次单击“浏览” > “应用服务”> Web 应用名称，打开 Web 应用的边栏选项卡。
...
5. 你应该会看到 Web 应用使用 **Azure 表存储**存储库按预期方式运行。
