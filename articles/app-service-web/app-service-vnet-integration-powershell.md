...
在 Azure Web 应用中，可以将应用（Web、移动或 API）连接到订阅中的虚拟网络 (VNet)。此功能称为 VNet 集成。

VNet 集成功能在新门户中有用户界面 (UI)，可让你与使用经典部署模型或 Azure Resource Manager 部署模型部署的虚拟网络集成。如果想要详细了解该功能，请参阅 [Integrate your app with an Azure Virtual Network（将应用与 Azure 虚拟网络集成）](/documentation/articles/web-sites-integrate-with-vnet)。
