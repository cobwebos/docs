<properties 
	pageTitle="如何使用 Azure Resource Manager 模板创建 ILB ASE | Azure" 
...
# 如何使用 Azure Resource Manager 模板创建 ILB ASE
...
-  *dnsSuffix* ：此参数定义要分配给 ASE 的默认根域。在 Azure App Service 的公共变体中，所有 Web 应用的默认根域均为 *chinacloudsites.cn* 。不过，ILB ASE 位于客户虚拟网络的内部，因此不适合使用公共服务的默认根域，而应当具有适合在公司的内部虚拟网络中使用的默认根域。例如，假定的 Contoso Corporation 可能会将 *internal-contoso.com* 的默认根域用于只能在 Contoso 虚拟网络内解析和访问的应用。
...
- *existingAseLocation* ：包含 ILB ASE 部署所在的 Azure 区域的文本字符串。例如“China East”。
...
- *certificateName* ：用户自己选择的好记字符串标识符，用于标识证书。此名称用作 *Microsoft.Web/certificates* 实体（表示 SSL 证书）的 Azure Resource Manager 唯一标识符的一部分。
...
若要开始使用 App Service 环境，请参阅 [App Service 环境简介](/documentation/articles/app-service-app-service-environment-intro/)
