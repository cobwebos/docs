<properties 
	pageTitle="使用 Active Directory 在 Azure App Service 中进行身份验证" 
	description="了解部署到 Azure App Service Web Apps 的业务线应用程序的不同身份验证和授权选项" 
...
# 使用 Active Directory 在 Azure App Service 中进行身份验证 #

[Azure 网站](/documentation/services/web-sites)通过支持单一登录 (SSO) 的用户启用企业业务线应用程序方案，允许你在本地环境或公共 Internet 访问应用程序。可以将它与 [Azure Active Directory](/documentation/services/active-directory) (AAD) 或本地安全令牌服务 (STS)（如 Active Directory 联合身份验证服务 (AD FS)）集成，以便对内部 Active Directory (AD) 用户进行身份验证并正确授权。
...
-	[Azure AD](/documentation/articles/web-sites-dotnet-lob-application-azure-ad) - 可以使用 Azure AD 为网站实施身份验证和授权。使用 Azure AD 作为标识提供程序具有以下特征：
	...
	-	通过使用 [Azure AD 图形 API](http://msdn.microsoft.com/library/azure/hh974476.aspx)，应用程序能够访问 Azure AD 中的目录数据。
