...
本教程演示如何构建安全的 ASP.NET MVC 5 Web 应用，以便用户能够使用 Facebook 或 Google 凭据进行登录。 该应用是一个简单的联系人列表，使用 ADO.NET Entity Framework 进行数据库访问。 你需要将该应用部署到 [Azure App Service](http://go.microsoft.com/fwlink/?LinkId=529714)。 
...
6. 可选 - 更改“Web 应用名称”框中的值（请参见下图）。
    ...
    Web 应用的 URL 将为 {名称}.azurewebsites.net，因此该名称在 azurewebsites.net 域中必须唯一。 配置向导建议在项目名称“ContactManager”的后面追加一个数字以使名称保持唯一，在本教程中你也可以这样做。
...
这就是创建将要部署到 Azure 的应用程序目前所需的全部操作。 
...
3. 将 Contact.cs 文件的内容替换为以下代码。
   ...
   **Contact** 类定义将为每个联系人存储的数据，外加数据库所必需的一个主键：*ContactID*。
...
9. 使用 Google 或 Facebook 登录。 这会将 Google 或 Facebook 帐户添加到 **canEdit** 角色。 如果你收到 HTTP 400 错误和消息“请求中的重定向 URI: https://contactmanager{我的版本}.azurewebsites.net/signin-google 与注册的重定向 URI 不匹配”，则你必须等待传播所做的更改。 如果在几分钟后收到此错误，请验证 URI 是否正确。
...
2. 右键单击 Web 应用，并选择“停止”。 
    ...
    也可以从“Azure 门户”[](https://portal.azure.com/)中转到 Web 应用的边栏选项卡，然后单击边栏选项卡顶部的“停止”图标。
...
* [使用 Facebook 和 Google OAuth2 创建 ASP.NET MVC 5 应用](http://www.asp.net/mvc/tutorials/mvc-5/create-an-aspnet-mvc-5-app-with-facebook-and-google-oauth2-and-openid-sign-on)一文包含有关如何将配置文件数据添加到用户注册数据库的说明，以及有关将 Facebook 用作身份验证提供程序的说明。
