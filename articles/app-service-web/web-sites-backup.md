<properties 
	pageTitle="在 Azure App Service 中备份 Web 应用" 
	description="了解如何在 Azure App Service 中创建 Web Apps 的备份。" 
...
有关从备份中还原 Azure Web 应用的信息，请参阅[还原 Azure Web 应用](/zh-cn/documentation/articles/web-sites-restore/)。
...
* 备份和还原功能要求站点为“标准”模式。有关使用“标准”模式缩放 Web 应用的详细信息，请参阅[如何缩放网站](/zh-cn/documentation/articles/web-sites-scale/)。 
...
-	你[设置每周备份](web-sites-backup.md#configure-automated-backups) Web 应用，其中包含永远不会更改的静态内容，例如旧的博客文章或映像。
...
3. 使用 [ftp](web-sites-deploy.md#ftp) 或任何其他方法将此文件上载到站点的 `D:\home\site\wwwroot` 的目录。如果你愿意，则可以直接在 `http://{yourapp}.scm.azurewebsites.net/DebugConsole` 中创建文件并在那里插入内容。
...
>[AZURE.NOTE]采用[还原定期备份](web-sites-restore.md)的相同方式还原站点的部分备份。还原过程会执行正确的操作。
>
...
-	为 Web 应用创建[过渡槽](web-sites-staged-publishing.md)。
-	在过渡槽上还原 Web 应用的完整备份。
-	基于完整备份和过渡槽还原最新的部分备份。
-	测试还原，以查看过渡应用是否正常工作。
-	将过渡 Web 应用[交换](web-sites-staged-publishing.md#Swap)为生产槽。
