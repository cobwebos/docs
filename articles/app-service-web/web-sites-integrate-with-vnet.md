...
该集成功能允许你的 Web 应用访问虚拟网络中的资源，但不允许通过虚拟网络访问你的 Web 应用。在一些标准情况下，你的 Web 应用需要访问数据库或 Web 服务，它们运行于你的虚拟网络甚至数据中心的虚拟机中。它不允许您安装驱动器。它目前还不支持启用与虚拟网络的身份验证系统相集成的功能。尽管该功能还处于预览状态，但到达 GA 之间将继续进行改进。
...
1.	如果 Web Apps 正基于**标准**层中的 App Service 计划运行，那么它们只能连接到虚拟网络。免费、共享和基本计划中的 Web Apps 无法连接到虚拟网络。
2.	如果你的目标虚拟网络已经存在，必须在连接到 Web 应用之前借助动态路由网关使网络处于点到站点启用状态。如果网关由静态路由配置，那么你将无法启用点到站点虚拟专用网络 (VPN)。
3.	你最多只有 5 个在 App Service 计划中配置的网络。Web 应用一次只能连接到一个网络。同一 App Service 计划中任意数量的 Web 应用均可使用这 5 个网络。  

您可以选择连接到新的或现有的虚拟网络。如果创建新的网络，网关将预先配置。请注意，创建和配置新的虚拟网络将耗费几分钟的时间。

如果你的 Web 应用不在标准层 App Service 计划中，用户界面可帮助你了解并访问你应该升级的定价层。

![](./media/web-sites-integrate-with-vnet/upgrade-to-standard.png)

## 系统的工作方式 ##
实际上，该功能使用点到站点 VPN 技术将你的 Web 应用连接到虚拟网络。Azure App Service 中的 Web Apps 具有多租户系统体系结构，与虚拟机中的情况一样，它可以阻止直接在虚拟网络中预配 Web 应用。通过基于点到站点技术的构建，我们将网络访问限制在托管 Web 应用的那台虚拟机。在那些 Web 应用主机上会进一步限制对网络的访问，因此你的 Web 应用仅可以访问将其配置为可访问的网络。

将网络保护在仅需要访问的 Web 应用范围之内而采取的措施可以防止创建 SMB 连接。尽管您可以访问远程资源，但这不包括安装远程驱动器。

![](./media/web-sites-integrate-with-vnet/how-it-works.png)
 
如果尚未使用虚拟网络配置 DNS 服务器，您需要使用 IP 地址。请确保您所需的终结点的端口能够越过防火墙显示。测试连接时，目前可用的唯一方法是使用 Web 应用或 Web 作业调用你所需的终结点。目前无法通过 Kudu 控制台使用 ping 或 nslookup 等工具。这点将会在不久的将来有所改进。

## 连接到预先存在的网络 ##
若要将 Web 应用连接到虚拟网络，请转到你的 Web 应用边栏选项卡，单击网络部分的虚拟网络磁贴，然后选择其中一个预先存在的网络。
...
在上图中，您可以看到名为 cantConnectVnet 的网络为灰色，无法进行选择。原因有两点。这意味着你无法在网络上启用点到站点 VPN，或你尚未在虚拟网络中预配动态路由网关。如果这两点得到满足，你将能够选择将虚拟网络与你的 Web 应用进行集成。

## 创建并连接到新的虚拟网络 ##
除了连接到预先存在的虚拟网络，你还可以通过 Azure 门户用户界面创建新的虚拟网络，并自动连接。若要创建新的虚拟网络，请按照相同路径访问虚拟网络用户界面，然后选择创建新的虚拟网络。您可以打开用户界面命名网络、指定地址空间，并为虚拟网络要使用的 DNS 服务器设置地址。
...
网络联接到该 Web 应用后，Web 应用将能够通过 TCP 或 UDP 访问虚拟网络中的资源。如果你想要访问本地系统（从站点到站点 VPN 到虚拟网络均可用）中的资源，你需要为自己的公司网络添加路由，以允许流量从网络转到在虚拟网络中配置的点到站点地址。
...
##管理虚拟网络连接##
通过访问 App Service 计划的边栏选项卡，你可以看到目前在 App Service 计划中与 Web 应用关联的所有虚拟网络列表。你最多有 5 个与标准层 App Service 计划关联的网络。
...
## 访问本地资源 ##
使用已配置了站点到站点 VPN 的虚拟网络时，你需要多执行一个步骤，以能够从你的 Web 应用访问本地资源。本地网络需添加路由，以支持流量从你的网络转到在虚拟网络中配置的点到站点地址。若要查看点到站点连接性的 IP 范围，请转到 Azure 门户的网络区域，如下所示。

![](./media/web-sites-integrate-with-vnet/vpn-to-onpremise.png)

## 证书 ##
为了与你的虚拟网络建立安全的连接，需要交换证书。如下所示，你可以看到 Web Apps 通过当前网络门户生成的公用证书的指纹。

![](./media/web-sites-integrate-with-vnet/vpn-to-onpremise-certificate.png)

如果证书因为某种原因（比如意外从网络门户中删除）而不同步，连接性将会被破坏。如要解决问题，可在你的 Web 应用的虚拟网络用户界面中执行同步连接操作，这样将重新建立连接。
...
## 与混合连接的比较和对比 ##
Web Apps 还可提供另外一项功能：混合连接，该功能在某些方面与虚拟网络集成类似。尽管它们的有些使用案例相互重叠，但两者无法相互替代。您可以借助混合连接在网络组合中建立与多个应用程序终结点的连接。虚拟网络功能够将你的 Web 应用连接到虚拟网络，该虚拟网络还可以连接到你的本地网络。如果资源全部位于网络之中，该功能将十分有效。
