---
title: "Azure 应用服务中的试验部署（beta 测试）"
description: "本端到端教程说明如何试验应用中的新功能或者对更新进行 beta 测试。 其中汇集了持续发布、槽、流量路由和 Application Insights 集成等应用服务功能。"
...
---
# <a name="flighting-deployment-beta-testing-in-azure-app-service"></a>Azure 应用服务中的试验部署（beta 测试）
本教程说明如何通过集成 [Azure 应用服务](http://go.microsoft.com/fwlink/?LinkId=529714)和 [Azure Application Insights](/services/application-insights/) 的各种功能来进行*试验部署*。
...
在本教程中，你将学习如何集成以下方案，以在生产环境中测试应用服务应用：

* [将生产流量路由](app-service-web-test-in-production-get-start.md)到 beta 应用
* [检测应用](../application-insights/app-insights-web-track-usage.md)以获取有用的度量值
...
* Visual Studio 2015 - 可以下载[社区版](https://www.visualstudio.com/en-us/products/visual-studio-express-vs.aspx)。
...
* 基本了解以下知识：
  * [Azure Resource Manager](../azure-resource-manager/resource-group-overview.md) 模板部署（请参阅[通过可预测的方式在 Azure 中部署复杂应用程序](app-service-deploy-complex-application-predictably.md)）
...
## <a name="set-up-your-production-web-app"></a>设置生产 Web 应用
> [!NOTE]
...
4. 创建本地复本后，请导航到 *&lt;repository_root>*\ARMTemplates 并运行具有唯一后缀的 deploy.ps1 脚本，如下所示：

     .\deploy.ps1 –RepoUrl https://github.com/<your_fork>/todoapp.git -ResourceGroupSuffix <your_suffix>
5. 出现提示时，键入所需的用户名和密码来访问数据库。 请记住数据库凭据，因为在更新资源组时需要再次指定凭据。
...
7. 脚本完成后，返回以浏览前端的地址 (http://ToDoApp*&lt;your_suffix>*.azurewebsites.net/)，查看在生产环境中运行的应用程序。
...
## <a name="investigate-instrument-your-client-app-for-monitoringmetrics"></a>调查：检测客户端应用的监视/度量值
1. 在 Visual Studio 中打开 *&lt;repository_root>*\src\MultiChannelToDo.sln。
2. 右键单击解决方案 >“管理解决方案的 NuGet 包” > “还原”，以还原所有 Nuget 包。
3. 右键单击“MultiChannelToDo.Web” > “添加 Application Insights 遥测” > “配置设置”>“将资源组更改为 ToDoApp”*&lt;“your_suffix”>* > “将 Application Insights 添加到项目”。
4. 在 Azure 门户中，打开 **MultiChannelToDo.Web** Application Insight 资源的边栏选项卡。 然后，在“应用程序运行状况”部分中，单击“了解如何收集浏览器页面加载数据”> 复制代码。
5. 将复制的 JS 检测代码添加到 *&lt;repository_root>*\src\MultiChannelToDo.Web\app\Index.cshtml，放在右 `<heading>` 标记前面。 它应包含 Application Insight 资源的唯一检测键。
...
6. 将以下代码添加到正文底部，以将鼠标单击自定义事件发送到 Application Insights：
...
7. 在 Git Shell 中，提交更改并将其发送到在 GitHub 中的分叉。 然后，等待客户端刷新浏览器。

       git add -A :/
       git commit -m "add AI configuration for client app"
       git push origin master
8. 将已部署的应用程序更改交换到生产环境：

     .\swap –Name ToDoApp<your_suffix>
9. 浏览到你配置的 Application Insights 资源。 单击“自定义事件”。
...
根据 ToDoApp 应用程序代码，**BUTTON** 事件对应“提交”按钮，**INPUT** 事件对应文本框。 到目前为止一切正常。 不过，点击次数似乎很多，但点击待办事项（**LI** 事件）的次数却很少。
...
### <a name="instrument-your-server-app-for-monitoringmetrics"></a>检测服务器应用的监视/度量值
这有一点离题，因为本教程中演示的方案只处理客户端应用。 不过，出于完整性，我们将设置服务器端应用。

1. 右键单击“MultiChannelToDo” > “添加 Application Insights 遥测” > “配置设置”>“将资源组更改为 ToDoApp”*&lt;“your_suffix”>* > “将 Application Insights 添加到项目”。
2. 在 Git Shell 中，提交更改并将其发送到在 GitHub 中的分叉。 然后，等待客户端刷新浏览器。

       git add -A :/
       git commit -m "add AI configuration for server app"
       git push origin master
3. 将已部署的应用程序更改交换到生产环境：

     .\swap –Name ToDoApp<your_suffix>

就这么简单！

## <a name="investigate-add-slot-specific-tags-to-your-client-app-metrics"></a>调查：将槽特定的标记添加到客户端应用度量值
...
由于要收集客户端行为的相关数据，因此需要在 index.cshtml 中[将遥测初始设置式添加到 JavaScript 代码](../application-insights/app-insights-api-filtering-sampling.md)。 例如，如果想要测试服务器端性能，也可以在服务器代码中运行类似的动作（请参阅[自定义事件和度量值的 Application Insights API](../application-insights/app-insights-api-custom-events-metrics.md)）。
...
2. 然后，在 Azure 中将此自定义属性添加为 Web 应用的[槽设置](web-sites-staged-publishing.md#AboutConfiguration)。 为此，请在 Git Shell 会话中运行以下命令。
...
5. 单击“收藏夹”按钮，将当前的“指标资源管理器”设置保存到**自定义事件：生产**之类的项。 以后你可以在此视图与部署槽视图之间轻松切换。
   ...
   > [!TIP]
   > 若要获得更强大的分析功能，请考虑将 [Application Insights 资源与 Power BI 集成](../application-insights/app-insights-export-power-bi.md)。
   >
   >

### <a name="add-slot-specific-tags-to-your-server-app-metrics"></a>将槽特定的标记添加到服务器应用度量值
同样，出于完整性，我们将设置服务器端应用。 不同于以 JavaScript 进行检测的客户端应用，服务器应用的槽特定标记以 .NET 代码进行检测。

1. 打开 *&lt;repository_root>*\src\MultiChannelToDo\Global.asax.cs。 将以下代码块添加到命名空间右括号前面。
...
4. 提交代码更改，并将其发送到 GitHub 上的分叉，然后等待用户使用新的应用（需要刷新浏览器）。 新属性大约需要 15 分钟才出现在 Application Insights `MultiChannelToDo` 资源中。

        git add -A :/
        git commit -m "add environment property to AI events for server app"
        git push origin master

## <a name="update-set-up-your-beta-branch"></a>更新：设置 beta 分支
1. 打开 *&lt;repository_root>*\ARMTemplates\ProdAndStagetest.json，并找到 `appsettings` 资源（搜索 `"name": "appsettings"`）。 资源共有 4 项，分别用于各个槽。
...
3. 在同一文件中，查找 `slotconfignames` 资源（搜索 `"name": "slotconfignames"`）。 资源共有 2 项，分别用于各个应用。
4. 针对每项 `slotconfignames` 资源，将 `"environment"` 添加到 `appSettingNames` 数组的末尾。 别忘了在上一行末尾处使用逗号。

    你已将 `environment` 应用设置链接到其各自用于两个应用的部署槽。  
5. 在 Git Shell 会话中运行以下命令，其中具有前面使用的相同资源组后缀。

        git checkout -b beta
        git push origin beta
        .\deploy.ps1 -RepoUrl https://github.com/<your_fork>/ToDoApp.git -ResourceGroupSuffix <your_suffix> -SlotName beta -Branch beta
6. 出现提示时，请指定与前面相同的 SQL 数据库凭据。 然后，当系统询问是否要更新资源组时键入 `Y`，然后按 `ENTER`。

    脚本完成后，原始资源组中的所有资源都保留下来，但在其中创建名为“beta”的新位置，且其配置与一开始创建的“过渡”槽相同。

   > [!NOTE]
   > 这种创建不同部署环境的方法，与[使用 Azure 应用服务进行敏捷软件开发](app-service-agile-software-development.md)中的方法不同。 使用此方法时，将创建具有部署槽的部署环境，而使用后一种方法时，将创建具有资源组的部署环境。 使用资源组来管理部署环境可让你将开发人员排除在生产环境以外，但无法轻松在生产环境中进行测试，而这一点却可以通过槽来轻松做到。
   >
   >
...
## <a name="update-push-your-updates-to-the-beta-app"></a>更新：将更新发送到 beta 应用
返回到你想要改进的应用。

1. 确保目前你位于 beta 分支中

        git checkout beta
2. 在 *&lt;repository_root>*\src\MultiChannelToDo.Web\app\Index.cshtml 中找到 `<li>` 标记，并添加 `style="cursor:pointer"` 属性，如下所示。

    ![](./media/app-service-web-test-in-production-controlled-test-flight/07-change-cursor-style-on-li.png)
3. 提交并推送到 Azure。
4. 通过导航到 http://todoapp*&lt;your_suffix>*-beta.azurewebsites.net/，验证更改是否已反映在 beta 槽中。 如果未看到更改，请刷新浏览器以获取新的 javascript 代码。
...
2. 现在导航到 http://ToDoApp*&lt;your_suffix>*.azurewebsites.net。 50% 的流量现在应该重定向到 beta 槽。
3. 在 Application Insights 资源中，使用 environment="beta" 筛选度量值。

   > [!NOTE]
   > 如果将此筛选的视图另存为收藏夹，你可以在生产视图与 beta 视图之间轻松切换指标资源管理器视图。
   >
   >
...
## <a name="go-live-move-your-new-code-into-production"></a>实现：将新代码移到生产环境
现在可将更新移到生产环境。 有利的是，现在你知道更新在发送到生产环境*之前*已经过验证。 因此，现在可以放心进行部署。 由于是对 AngularJS 客户端应用进行更新，因此只有客户端代码受到验证。 如果要对后端 Web API 应用进行更改，可以用同样的方式轻松验证更改。
...
2. 运行 Git 命令：
        ...
        git merge beta
        git push origin master
3. 等待几分钟，让新的代码部署到过渡槽，然后启动 http://ToDoApp*&lt;your_suffix>*-staging.azurewebsites.net，以验证新的更新是否已在过渡槽中准备就绪。 请记住，分叉的主要分支链接到应用的过渡槽。
4. 现在，将过渡槽交换到生产环境中
...
## <a name="summary"></a>摘要
Azure 应用服务可让中小型企业轻松地在生产环境中测试其客户端应用，这在过去只能在大型企业中进行。 希望本教程提供的信息可帮助你结合 Azure 应用服务和 Application Insights 以成功进行试验部署，甚至在 DevOps 领域进行其他生产测试。
