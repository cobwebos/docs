...
4. 当添加记录时，必须首先选择记录类型。
	...
	![选择记录类型](./media/web-sites-custom-domain-name/godaddy-selectrecordtype.png)

	接下来，你必须提供“主机”（自定义域或子域）和所“指向”内容。

	![添加区域记录](./media/web-sites-custom-domain-name/godaddy-addzonerecord.png)

	* 在添加 **A（主机）记录**时，必须将“主机”字段设置为 **@**（代表根域名，如 **contoso.com**）、*（用于匹配多个子域的通配符），或者是你要使用的子域（例如 **www**）。 必须将“指向”字段设置为你的 Azure 网站的 IP 地址。
