...
本教程演示如何通过 [Azure 管理门户](https://manage.windowsazure.cn/)[在 Azure 中创建 Java Web 应用](/documentation/services/web-sites/)。Azure 管理门户是可用于管理 Azure 资源的 Web 界面。
...
3. 创建一个应用，然后手动复制和编辑配置文件
	...
	* 你想要使用在 Azure 中不受支持的 Java 版本，且想要自行上载它。
	...
	对于这样的情况，你可以通过 Azure 管理门户创建一个应用，然后手动提供相应的运行时文件。在本示例中，这些文件将占用你 App Service 计划的存储空间配额。有关详细信息，请参阅[将自定义 Java Web 应用上载到 Azure](/documentation/articles/web-sites-java-custom-upload)。
