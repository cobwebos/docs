<properties
	pageTitle="通过 Memcache 协议将 Azure 网站中的网站连接到 Redis 缓存 | Microsoft Azure"
	description="使用 Memcache 协议将 Azure App Service 中的网站连接到 Redis 缓存"
...
# 通过 Memcache 协议将 Azure 网站中的网站连接到 Redis 缓存

在本文中，你将学习如何使用 [Memcache][13] 协议将 [Azure 网站](/documentation/services/web-sites/)中的 WordPress 网站连接到 [Azure Redis 缓存][12]。如果你的现有网站使用 Memcached 服务器作为内存中缓存，则你可以将其迁移到 Azure 网站，并使用 Microsoft Azure 中的第一方缓存解决方案，其中对你的应用程序代码进行少量更改或无更改。此外，你可以利用现有的 Memcache 专业知识，在使用 Azure Redis 缓存作为内存中缓存的 Azure 网站中创建高度可缩放的分布式应用，同时使用常用的应用程序框架，例如 .NET、PHP、Node.js、Java 和 Python。

Azure 网站使用网站 Memcache 填充码启用该应用程序方案，这是充当 Memcache 代理的本地 Memcached 服务器，用于缓存对 Azure Redis 缓存的调用。这使得使用 Memcache 协议通信的任何应用可以使用 Redis 缓存缓存数据。此 Memcache 填充码在协议级别运行，因此只要它使用 Memcached 协议通信，就可以供任何应用程序或应用程序框架使用。

## 先决条件

网站 Memcache 填充码可以与任何应用程序一起使用，前提是使用 Memcached 协议通信。就此特定示例来说，引用应用程序是一个可缩放的 WordPress 站点，它可以从 Azure 应用商店设置。
...
你需要创建的第二个应用设置是 **REDIS\_KEY** 应用设置。此设置提供了安全访问 Redis 缓存实例所需的身份验证令牌。可以通过以下 PowerShell 命令检索 REDIS\_KEY 应用设置所需的值。
...
最后一个应用设置用于启用网站中的 Memcache 填充码，以便使用 REDIS\_HOST 和 REDIS\_KEY 来连接到 Azure Redis 缓存并转发缓存调用。将应用设置的密钥设置为 **MEMCACHESHIM\_REDIS\_ENABLE**，并将其值设置为 **true**。
...
下载网站中启用的 PHP 版本的非线性安全 (NTS) x86 链接。（默认为 PHP 5.4）

![PHP PECL 网站 Memcache 包](./media/web-sites-connect-to-redis-using-memcache-protocol/8-php-pecl-memcache-package.png)

### 启用 php\_memcache 扩展

下载文件之后，将 **php\_memcache.dll** 解压缩并上载到 **d:\\home\\site\\wwwroot\\bin\\ext\** 目录。php\_memcache.dll 上载到网站之后，需要启用 PHP 运行时的扩展。若要在 Azure 管理门户中启用 Memcache 扩展，请打开网站的“配置”，然后添加键为 **PHP\_EXTENSIONS**、值为 **bin\\ext\\php\_memcache.dll** 的新应用设置。

> [AZURE.NOTE] 如果网站需要加载多个 PHP 扩展，则 PHP\_EXTENSIONS 的值应为由逗号分隔的 DLL 文件相对路径的列表。
...
>[AZURE.NOTE] 按照此博客中关于[如何启用网站中的站点扩展][8]的说明，安装 Visual Studio Team Services。
...
启用网站 Memcache 填充码的所有步骤现已完成。剩下的唯一一件事是，验证 Redis 缓存实例是否正在填充数据。
...
列出密钥的调用应返回一个值。如果没有，请尝试导航到网站，然后重试。

## 结束语

祝贺你！ WordPress 应用现在有了集中式的内存中缓存，这将有助于提高吞吐量。请记住，网站 Memcache 填充码可以与任何 Memcache 客户端结合使用，无需考虑编程语言或应用程序框架。若要提供反馈或者提问有关网站 Memcache 填充码的问题，请在 [MSDN 论坛][10]或 [Stackoverflow][11] 上发布相关文章。
