<properties
	pageTitle="创建内部负载平衡器并将其与应用服务环境搭配使用 | Azure"
...
应用服务环境 (ASE) 功能是 Azure App Service 的一个高级服务选项，可提供多租户戳记中不会提供的增强型配置功能。ASE 功能实质上是在 Azure 虚拟网络 (VNet) 中部署 Azure App Service。若要更好地理解 App Service 环境所提供的功能，请阅读文档：[什么是 App Service 环境][WhatisASE]。如果不了解在 VNet 中操作的优点，请阅读 [Azure 虚拟网络常见问题解答][virtualnetwork]。
...
如果 ASE 的名称为 ***contosotest***，该 ASE 中的应用名称为 ***mytest***，则子域的格式为 ***contosotest.p.chinacloudsites.cn***，该应用的 URL 为 ***mytest.contosotest.p.chinacloudsites.cn***。
...
2. 选择订阅
3. 选择或创建资源组
4. 选择或创建应用服务计划 (ASP)。如果创建新的 ASP，请选择 ASE 作为位置并选择希望在其中创建 ASP 的工作线程池。创建 ASP 时，选择 ASE 作为位置并选择工作线程池。指定应用的名称时，会看到应用名称下面的子域替换为 ASE 的子域。
5. 选择“创建”。如果希望应用显示在仪表板上，应选中“固定到仪表板”复选框。
...
创建 ASE 之后，用户会注意到子域显示指定的子域，且“设置”菜单中有一个名为“ILB 证书”的新项。在设置 ASE 的证书之前，无法通过 HTTPS 访问 ASE 中的应用。
...
8.	在该 VM 上使用浏览器转到 https://mytestapp.ilbase.com， 如果使用自签名证书，则必须接受安全性不足的问题。
