<properties
	...
	description="本教程演示如何使用 Azure 表服务存储 Azure App Service Web Apps 中托管的 Node.js 应用程序的数据。"
	tags="azure-portal"
...
**express** 创建的其中一个文件是 **package.json**。此文件包含模块依赖项的列表。之后，当你将应用程序部署到 App Service Web Apps 时，此文件会确定需要在 Azure 上安装的模块。
...
> [AZURE.NOTE]还可以使用 [Azure 门户](https://portal.azure.com)执行这些步骤。请参阅[在 Azure App Service 中生成和部署 Node.js Web 应用]。
>
...
2. 使用以下命令创建新的 Web 应用。
	...
	> [AZURE.NOTE]如果这是你的订阅的第一个 App Service Web 应用，系统会指示你使用 Azure 门户创建 Web 应用。有关详细信息，请参阅[在 Azure App Service 中生成和部署 Node.js Web 应用]。
...
3. 将提交的文件推送到 App Service Web 应用：
