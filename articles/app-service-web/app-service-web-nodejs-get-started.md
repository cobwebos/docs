---
title: "开发适用于 Azure 应用服务的 Node.js Web 应用 | Microsoft 文档"
...
---
# <a name="develop-nodejs-web-apps-for-azure-app-service"></a>开发适用于 Azure 应用服务的 Node.js Web 应用
...
- [Azure CLI 1.0](app-service-web-nodejs-get-started-cli-nodejs.md) - 适用于经典部署模型和资源管理部署模型的 CLI
- [Azure CLI 2.0（预览版）](app-service-web-nodejs-get-started.md)- 适用于资源管理部署模型的下一代 CLI
...
## <a name="create-and-configure-a-simple-nodejs-app-for-azure"></a>创建和配置简单的适用于 Azure 的 Node.js 应用
...
6. 从应用程序根目录打开 ./config/config.js 文件，并将生产端口更改为 `process.env.port`；`config` 对象中的 `production` 属性应类似下面的示例：
    ...
    > [!NOTE] 
    > 默认情况下，Azure 应用服务使用 `production` 环境变量 (`process.env.NODE_ENV="production"` 运行 Node.js 应用程序。
    > 因此，Azure 中的 Node.js 应用可以使用此处的配置在 iisnode 侦听的默认端口上响应 Web 请求。
    >
    >
...
8. 保存所做的更改，然后初始化应用程序根目录中的 Git 存储库并提交代码：
   
        git add .
        git add -f config
        git commit -m "{your commit message}"

## <a name="deploy-your-nodejs-app-to-azure"></a>将 Node.js 应用部署到 Azure

1. 登录到 Azure（需 [Azure CLI 2.0 预览版](#prereq)）：
...
Azure App Service 使用 [iisnode] 运行 Node.js 应用。 使用 Azure CLI 2.0 预览版和 Kudu 引擎（Git 部署），用户在通过命令行开发和部署 Node.js 应用时可以获得顺畅的体验。 

* 可以在根目录中创建一个 iisnode.yml 文件，用于自定义 iisnode 属性。 所有可配置的设置都记录在[此处](https://github.com/tjanczuk/iisnode/blob/master/src/samples/configuration/iisnode.yml)。
...
如需 Azure 应用服务中所有受支持的 Node.js/NPM 版本的已更新列表，请访问适合你的应用的以下 URL：
