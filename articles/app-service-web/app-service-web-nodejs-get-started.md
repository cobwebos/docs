<properties
	pageTitle="在 Azure App Service 中 Node.js Web 应用入门"
...
1. 更改为 ASM 模式，然后登录到 Azure（为此需要 [Azure CLI](#prereq)）：
...
2. 确保你仍在应用的根目录中，然后使用下一个命令，以唯一的应用名称在 Azure 中创建应用服务应用资源；例如：http://{appname}.azurewebsites.net
