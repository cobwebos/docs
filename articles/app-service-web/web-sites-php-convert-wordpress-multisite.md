...
本教程假定已存在 WordPress 安装。 如果没有，请按照[在 Azure 中从库中创建 WordPress 网站][website-from-gallery]中提供的指导进行操作。

通常，将现有 WordPress 单站点安装转换为 Multisite 非常简单，此处的许多初始步骤直接来自 [WordPress Codex](http://codex.wordpress.org) 上的 [Create A Network][wordpress-codex-create-a-network]（创建网络）页面。
...
* [采用 MySQL 和 FTP 的 PHP 网站][website-w-mysql-and-ftp-ftp-setup]
* [采用 MySQL 和 Git 的 PHP 网站][website-w-mysql-and-git-git-setup]
...
有关子域和子目录设置的详细信息，请参阅 WordPress Codex 上的 [Types of multisite network][wordpress-codex-types-of-networks]（多站点网络的类型）一文。
...
WordPress Multisite 当前没有用于映射自定义域的内置方法。 但是，可以利用一个称为 [WordPress MU 域映射][wordpress-plugin-wordpress-mu-domain-mapping]的插件来添加该功能。 登录到网站的“网络管理员”部分，并安装“WordPress MU 域映射”插件。
