...
**解答**：我们不提供使用新数据更新现有模型的功能。
如果你有新数据集且想要自定义现有模型，则必须使用新旧数据集重新适应。
新旧数据集必须合并为单个 .zip（如果是声音数据）或 .txt 文件（如果是语言数据），适应完成后，需要取消部署新更新的模型，获取新的终结点

**问题**：如果所需并发高于默认值应该如何操作？ 

**解答**：以被称为缩放单元的 5 个并发请求为增量纵向扩展模型。 每个缩放单元可确保模型可同时处理 5 个音频流。 最多可购买 100 个缩放单元（或 500 个并发请求）。
...
**解答**：在创建部署期间，可选择关闭跟踪，此时不会记录音频或听录。 否则，通常会将请求记录在 Azure 中并安全存储。 如果还有禁止使用自定义语音服务的进一步隐私问题，请与我们联系。
...
**问题**：数据报告显示存在失败的话语。 这有问题吗？

**解答**：如果只有少数话语未能成功导入，则不会出现问题。
...
**解答**：必须转录数据。 可以自行转录或使用专业听录服务进行转录。 一些服务使用专业转录器，其他使用众包。 我们还可根据要求推荐听录服务。
...
**问题**：我现在知道自定义模型的测试结果，如何确定这个数字是好还是不好？
...
**解答**：对于这个问题很难给出准确的答案，因为这取决于应用程序中使用的词汇和短语与初始语言模型存在多大差异。 对于所有新字词，请尽可能多地提供这些字的使用示例，这一点非常有用。 对于应用程序中使用的常用短语，在语言数据中包括短语也非常有用，因为这也会通知系统侦听这些术语。
