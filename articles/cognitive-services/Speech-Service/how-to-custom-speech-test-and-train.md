...
在测试 Microsoft 语音识别的准确性或训练自定义模型时，需要音频和文本数据。 在此页上，我们将介绍自定义语音模型所需的数据类型。

## <a name="data-diversity"></a>数据多样性

用于测试和定型自定义模型的文本和音频需要包含一组不同的扬声器和方案的示例，您需要模型来识别它们。
为自定义模型测试和定型收集数据时，请考虑以下因素：

* 你的文本和语音音频数据需要涵盖你的用户在与模型交互时将进行的一类口头表述。 例如，可引发并降低温度要求的模型会对用户提出请求此类更改的陈述进行训练。
* 你的数据需要包含模型需要识别的所有语音差异。 许多因素可能会改变语音，包括口音、方言、语言混合、年龄、性别、语音音调、压力水平和当天时间。
* 你必须在将使用模型的不同环境（室内、户外、公路噪音）中包括示例。
* 必须使用生产系统将使用的硬件设备收集音频。 如果您的模型需要识别不同质量的记录设备上记录的语音，则您提供的用于训练模型的音频数据还必须表示这些不同的方案。
* 稍后，您可以将更多数据添加到模型中，但请注意，将数据集保持在不同并代表您的项目需求。
* 如果包含*不*在自定义模型识别范围内的数据，则可能会损害总体识别质量，因此不包含模型不需要转录的数据。

在这种情况下训练的模型在这些方案中只能很好地执行。 仔细选择表示你需要自定义模型识别的方案的完整范围的数据。

> [!TIP]
> 从一组小型示例数据开始，这些数据与模型将遇到的语言和噪声相匹配。
> 例如，在同一个硬件上记录一个小但有代表性的音频样本，并将其放在生产方案中，你的模型会在该环境中找到。
> 小型的代表数据的数据集可能会在你投资收集更大的数据集进行培训之前公开问题。
