...
ms.locfileid: "72675604"
---
# <a name="quickstart-recognize-speech-by-using-a-cross-platform-xamarin-app-that-uses-the-speech-sdk"></a>快速入门：通过使用语音 SDK 的跨平台 Xamarin 应用识别语音
...
本文介绍如何使用适用于通用 Windows 平台 (UWP)、Android 和 iOS 的 Xamarin 开发一个使用 Azure 认知服务[语音 SDK](speech-sdk.md) 的跨平台 C# 应用程序。 该程序将通过设备的麦克风实时将语音转录为文本。 该应用程序使用[语音 SDK NuGet 包](https://aka.ms/csspeech/nuget)和任意版本的 Microsoft Visual Studio 2019 构建。
...
* 针对 iOS： 
    * 一台 iOS 设备 (ARM64) 或 iOS 模拟器 (x64)，[可用于开发](https://docs.microsoft.com/xamarin/ios/get-started/installation/device-provisioning/)，并配备正常工作的麦克风。
...
1. 接下来，在 Visual Studio 中双击 helloworld.UWP 项目下的 `Package.appxmanifest` 文件。 确保在“功能”下选中“麦克风”，然后保存文件。  
   ...
   > [!NOTE] 
   > 如果看到警告“以下证书文件不存在: helloworld.UWP_TemporaryKey.pfx”，请查看[语音转文本](quickstart-csharp-uwp.md)示例以了解详细信息。
