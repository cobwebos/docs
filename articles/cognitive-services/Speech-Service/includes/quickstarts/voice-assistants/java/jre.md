...
1. 在“新建 Java 类”窗口中，在“包”字段内输入 **speechsdk.quickstart**，在“名称”字段内输入 _Main_。   
...
1. 调用 `DialogServiceConnector` 方法将 `connectAsync()` 连接到 Direct Line 语音。 若要测试机器人，可以调用 `listenOnceAsync` 方法以从麦克风发送音频输入。 此外，还可以使用 `sendActivityAsync` 方法以序列化字符串的形式发送自定义活动。 这些自定义活动可以提供机器人在聊天中使用的其他数据。
...
1. 在“新建 Java 类”窗口中，在“包”字段中输入 **speechsdk.quickstart**，在“名称”字段中输入 _ActivityAudioStream_。   
