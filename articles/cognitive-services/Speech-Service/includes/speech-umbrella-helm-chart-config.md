...
|参数|描述|默认|
| -- | -- | -- | -- |
| `speechToText.enabled` | 是否启用**语音到文本**服务。 | `true` |
...
| `speechToText.verification.image.arguments` | 用于执行*测试使用*docker 映像的参数。 Helm 测试窗格在运行时`helm test`将这些参数传递给容器。 | `"./speech-to-text-client"`<br/> `"./audio/whatstheweatherlike.wav"` <br/> `"--expect=What's the weather like"`<br/>`"--host=$(SPEECH_TO_TEXT_HOST)"`<br/>`"--port=$(SPEECH_TO_TEXT_PORT)"` |
| `textToSpeech.enabled` | 是否启用**文本到语音转换**服务。 | `true` |
