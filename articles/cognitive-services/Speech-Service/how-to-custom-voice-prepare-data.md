---
title: 如何为自定义语音 - 语音服务准备数据
titleSuffix: Azure Cognitive Services
description: 使用语音服务为您的品牌创建自定义语音。 在你提供工作室录制内容和关联的脚本后，服务将生成针对录制的语音优化的独特语音模型。 使用此语音可以合成产品、工具和应用程序中的语音。
...
准备好为应用程序创建自定义的“文本转语音”语音后，第一步是收集音频录制内容和关联的脚本，开始训练语音模型。 语音服务使用此数据创建唯一的语音调整以匹配录音中的语音。 训练该语音后，可以开始合成应用程序中的语音。
...
| 数据类型 | 描述 | 何时使用 | 所需的其他服务 | 用于训练模型的数量 | 区域设置 |
| --------- | ----------- | ----------- | --------------------------- | ----------------------------- | --------- |
...
| **长音频 + 脚本 (beta)** | 长的未分段音频文件（长度超过 20 秒）集合 (.zip)，与包含所有口语的脚本 (.txt) 配对。 | 有音频文件和匹配的脚本，但它们未分段成言语。 | 分段（使用批量听录）。<br>根据需要进行音频格式转换。 | 无硬要求  | [所有自定义语音区域设置](language-support.md#customization) |
| **仅限音频 (beta)** | 不包含脚本的音频文件集合 (.zip)。 | 只有音频文件，没有脚本。 | 分段 + 脚本生成（使用批量听录）。<br>根据需要进行音频格式转换。| 无硬要求 | [所有自定义语音区域设置](language-support.md#customization) |
...
> [!TIP]
> 如果生成生产型“文本转语音”语音，请在选择表述（或撰写脚本）时既要考虑到语音覆盖率，又要考虑到效率。 无法获得所需的结果？ [请与自定义语音团队联系](mailto:speechsupport@microsoft.com)，详细了解如何让我们咨询。
...
所有音频文件应分组成一个 .zip 文件。 把 .wav 文件和 .mp3 文件放入一个音频 zip 中是可以的。 例如，您可以上传一个 zip 文件，其中包含名为"kingstory.wav"的音频文件，45 秒长，另一个音频名为"queenstory.mp3"，200 秒长。 处理后，所有 .mp3 文件将转换为 .wav 格式。
