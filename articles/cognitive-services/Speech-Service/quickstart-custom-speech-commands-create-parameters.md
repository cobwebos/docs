...
1. 创建一个新参数，用于表示用户是要打开还是关闭设备
   ...
   | ------------------ | ------------------- | --------------------------------------------------------------------------------------------------------- |
   | 名称               | 麦克风               | 参数的描述性名称                                                                     |
   | 为全局          | unchecked           | 指示此参数的值是否全局应用于项目中的所有命令的复选框 |
   | 必选           | checked             | 一个复选框，该复选框指示在完成命令之前是否需要此参数的值          |
   | 响应模板  | "-开或 off？"      | 在未知情况下询问此参数值的提示                                       |
   | 类型               | String              | 参数的类型，例如数字、字符串或日期时间                                               |
   | Configuration      | 字符串列表         | 对于字符串，字符串列表将输入限制为一组可能值                                      |
   | 字符串列表值 | on, off             | 对于字符串列表参数，为可能的值集及其同义词                                |

   - 接下来，再次`+`选择图标以添加一个表示设备名称的第二个参数。 对于本示例，电视和风扇
...
> [!TIP]
> 示例句子编辑器使用大括号来引用参数。 - `turn {OnOff} the {SubjectDevice}`使用 tab 自动补全引用前面创建的参数。

> [!div class="mx-imgBorder"]
> ![带参数的示例句子](media/custom-speech-commands/create-parameter-sentences.png)
...
1. 添加新条件，并选择 "必需参数"。 同时`OnOff`选择和`SubjectDevice`
