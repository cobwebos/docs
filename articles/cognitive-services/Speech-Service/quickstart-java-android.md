...
本文介绍如何使用 Azure 认知服务语音 SDK 将语音转为文本，进而开发适用于 Android 的 Java 应用程序。

该应用程序基于语音 SDK Maven 包和 Android Studio 3.3。 语音 SDK 目前与具有 32 位或 64 位 ARM 和 Intel x86/x64 兼容处理器的 Android 设备兼容。
...
![用户界面](media/sdk/qs-java-android-11-gui.png)
...
1. 另外，将字符串 `YourServiceRegion` 替换为与订阅关联的[区域](regions.md)。 例如，将 `westus` 用于免费试用版订阅。
