...
除了语音服务使用的通用模型外, 还可以创建特定于你的体验的自定义声音、语言和发音模型。 下面是自定义选项的列表:

| 模型 | 描述 |
|-------|-------------|
| [声学模型](how-to-customize-acoustic-models.md) | 如果你的应用程序、工具或设备在特定环境 (如汽车或工厂中的特定录制条件) 中使用, 则创建自定义声音模型非常有用。 示例包括带有口音的讲话、特定的背景噪音，或使用特定的麦克风录制音频。 |
