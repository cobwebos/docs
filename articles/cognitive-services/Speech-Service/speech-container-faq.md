...
请考虑一下 `ja-JP` 容器和最新模型。 声音模型是最苛刻的 CPU，而语言模型则要求最大的内存。 当我们对使用进行了基准处理时，如果音频流入实时 (如麦克风) ，则大约需要 0.6 CPU 核心来处理单个语音到文本请求。 如果你的音频比实时 (（如从文件) ）的速度更快，则该使用情况可以)  (1.2 x 内核。 同时，下面列出的内存是用于解码语音的操作内存。 它不 *会* 考虑将驻留在文件缓存中的语言模型的实际完整大小。 `ja-JP`这是一个额外的 2 gb; 对于 `en-US` ，它可能更 (6-7 GB) 。

如果有内存不足的计算机，并且您尝试在该计算机上部署多种语言，则文件缓存可能已满，并且操作系统会强制在页面模型中进出页面。对于正在运行的脚本，这可能会造成灾难性后果，并可能导致性能下降和其他性能影响。

此外，我们预先打包了具有 [高级矢量扩展 (AVX2) ](speech-container-howto.md#advanced-vector-extension-support) 指令集的计算机的可执行文件。 具有 AVX512 指令集的计算机将需要为该目标生成代码，而10个语言启动10个容器可能会暂时耗尽 CPU。 此类消息会出现在 docker 日志中：
...
<details>
<summary>
<b>是否可以帮助进行容量规划和本地的语音到文本容器的成本估算？</b>
</summary>

**答案：** 对于批处理模式下的容器容量，每个解码器可以通过两个 CPU 内核（对于单个识别）实时处理 2-3 倍。 建议不要为每个容器实例保留两个以上的并发识别，但建议在负载均衡器后面运行更多的容器实例，以确保可靠性/可用性。

尽管我们可以让每个容器实例运行更多的解码器。 例如，我们可以在八个核心计算机上设置每个容器实例的7个解码器， (每个) 的时间超过2倍，从而产生15x 吞吐量。 有一个需要 `DECODER_MAX_COUNT` 注意的参数。 在极端情况下，会出现可靠性和延迟问题，吞吐量会大幅增加。 对于麦克风，它将为1x 实时。 对于单个识别，总体使用量应为大约一个核心。
...
| 实例  | vCPU (s)  | RAM    | 临时存储 | AHB 的即用即付 | 1年准备金，AHB (% 节省)  | 3年保留的 AHB (% 储蓄)  |
|-----------|---------|--------|--------------|------------------------|-------------------------------------|--------------------------------------|
| `DS13 v2` | 8       | 56 GiB | 112 GiB      | $ 0.598/小时            | $ 0.3528/小时 (~ 41% )                  | $ 0.2333/小时 (~ 61% )                   |

根据设计参考 (两个群集的5个 Vm 来处理 1 K 小时/天的音频批处理) ，1年的硬件成本将是：

> 2 (群集) * 5 个 (Vm 每个群集) * $ 0.3528/小时 * 365 (天) * 24 (小时) = $ 31K/年
...
支付给 Microsoft 的维护费用取决于服务的服务级别和内容。 它的不同之处是，在涉及到现场服务时，将基本级别 $ 29.99/月份用于基本级别。 用于服务/维护的粗略数字是 $ 300/小时。 不包括人员成本。 其他基础结构成本 (例如存储、网络和负载均衡器) 不包括在内。
...
**答案：**`speech_recognition_language=<YOUR_LANGUAGE>`如果使用的是碳客户端，应在请求中显式配置。
...
自定义容器本质上是不支持基于 Halide 或 ONNX 的声音模型 (这是自定义培训门户) 中的默认设置。 这是因为自定义模型未进行加密，我们不想公开 ONNX 模型。语言模型是正确的。 客户需要为自定义培训显式选择一个较旧的非 ONNX 模型。 不会影响准确性。 模型大小可能大于 100 MB)  (。

> 支持模型 > 20190220 (4.5 统一) 
...
云不同于容器。 云可以由多个聚合容器组成 (有时称为微服务) 。 因此，有一个 LUIS 容器，然后有一个语音容器-两个单独的容器。 语音容器只执行语音。 LUIS 容器只会 LUIS。 在云中，因为这两个容器都是已知的，并且对于远程客户端进入云、执行语音、返回、再次转到云并执行 LUIS，我们提供了一项功能，该功能允许客户端转到语音、离开云中，然后转到 LUIS 并返回到客户端。 这种情况下，即使在此方案中，语音 SDK 也会转到带有音频的语音云容器，然后语音云容器将与 LUIS cloud 容器进行通信。 LUIS 容器没有接受音频 (的概念，如果 LUIS 容器接受流音频，LUIS 是基于文本的服务) 。 使用本地时，我们不能保证我们的客户已部署两个容器，我们不会在我们的客户本地中的容器之间进行协调，并且如果这两个容器都部署在本地上（如果它们更本地到客户端），则不会负担首先回到客户端，让客户转到 LUIS。
...
**答案：** 这是以下内容的合成：
- 尝试为容器口述终结点的人， (我不确定其 URL 是如何的) 
- 1个<sup>st</sup> 方终结点是容器中的终结点。
- 1个<sup>st</sup> 方终结点返回了 speech 消息，而不是 `speech.hypothesis` 消息中的3个<sup>rd</sup> 部件终结点返回的消息。
- "碳快速入门" 全部使用 `RecognizeOnce` (交互模式) 
- 使用带有断言的，而 `speech.fragment` 要求它们的消息不会以交互模式返回。
- 在发布版本中，将断言激发 (终止进程) 。

解决方法是在代码中使用连续识别切换到，或 (更快) 连接到容器中的交互式或连续终结点。
...
SDK 1.8 附带了正确的修补程序，它具有本地支持 (将选取正确的终结点，因此，我们不会比联机服务) 更糟。 同时，还提供了持续识别的示例，为什么不指向它呢？
...
**答案：** 以下是现有模型所需的一些大致数字 (会改变，因为我们将在 GA) ：
...
**答案：** `RecognizeOnce()` 在交互模式下，仅处理最多15秒的音频，因为模式适用于语音命令，其中最谈话应为 short。 如果使用 `StartContinuousRecognition()` 听写或会话，则不会限制15秒。
...
- 解码器可以完成大约2到3倍的实时工作。 为此，单个识别的总体 CPU 使用率将接近两个内核。 这就是我们不建议为每个容器实例保留两个以上的活动连接的原因。 极端情况是在八核计算机（如）中将大约10个解码器置于8个核心计算机上 `DS13_V2` 。 对于容器版本1.3 及更高版本，可以尝试设置一个参数 `DECODER_MAX_COUNT=20` 。
...
旧容器没有用于使用 API 的所需终结点 `FromHost` 。 如果容器用于版本1.3，则应使用以下代码：
...
```
 ...
 `SetSpeechSynthesisVoiceName`由于具有更新的文本到语音引擎的容器需要语音名称，因此调用函数。
...
- 对于单个识别 (交互模式) 使用自定义终结点 (，则为; `SpeechConfig` 使用终结点参数) ，请参阅 `speech_recognize_once_from_file_with_custom_endpoint_parameters()` 。
- 对于连续识别 (对话模式) ，只需进行修改即可使用如上所示的自定义终结点，请参阅 `speech_recognize_continuous_from_file()` 。
- 若要在上面的示例中启用听写 (仅当你确实需要它时) ，请在创建之后 `speech_config` 添加代码 `speech_config.enable_dictation()` 。
...
> 参数：宿主 (必需的) 订阅密钥 (可选的，如果你可以使用该服务而不) 。

Host 的格式为 `protocol://hostname:port` ，其中 `:port` 是可选的 (参见下面) ：
