...
包含发音条目的 .txt 文件的大小限制为 1 兆字节 (MB)。 通常情况下，无需通过此文件上传大量数据。 大多数自定义发音文件可能只有几千字节 (KB) 大小。 对于所有区域设置，.txt 文件的编码都应为 UTF-8 BOM。 对于英语区域设置，ANSI 也是可接受的。
