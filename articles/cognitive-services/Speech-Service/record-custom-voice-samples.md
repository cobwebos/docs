...
指导配音员清楚地发音。 脚本的每个字词都要按书面形式发音。 声音不应该被省略或模糊在一起，这是常见的在随意的语音，*除非他们已经写在脚本中的方式*。
...
> [!TIP]
> 如果使用大量的话语，单个话语可能不会对生成的自定义语音产生明显的影响。 只需记下任何带有问题的话语，将它们从数据集中排除，并查看自定义语音的结果如何，可能更适宜。您始终可以返回工作室，并在以后录制丢失的样品。
