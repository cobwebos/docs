...
语音容器使客户能够构建一个经过优化的语音应用程序体系结构，以利用强大的云功能和边缘位置。 我们现在支持的四个语音容器是：**语音到文本**、**自定义语音到文本**、**文本到语音**转换和**自定义文本到语音**。
...
> [!IMPORTANT]
> [`Billing`](#billing-configuration-setting)[`Eula`](#eula-setting)和[`ApiKey`](#apikey-configuration-setting)设置一起使用，您必须为所有三个设置提供有效值;因此，您必须为所有三个设置提供有效值。否则您的容器将无法启动。 有关使用这些配置设置实例化容器的详细信息，请参阅[计费](speech-container-howto.md#billing)。
...
标准语音容器不使用输入或输出装载来存储训练或服务数据。 但是，自定义语音容器依赖于卷装载。
...
自定义语音容器使用[卷装载](https://docs.docker.com/storage/volumes/)来保留自定义模型。 您可以通过将`-v`（或`--volume`） 选项添加到 Docker [run](https://docs.docker.com/engine/reference/commandline/run/)命令来指定卷装载。

首次下载自定义模型时，新模型被引入为自定义语音容器 Docker 运行命令的一部分。 自定义语音容器的序列`ModelId`运行将使用以前下载的模型。 如果未提供卷装载，则无法保留自定义模型。

卷安装设置由三个颜色`:`分隔字段组成：

1. 第一个字段是主机上的卷的名称，例如_C：\input_。
2. 第二个字段是容器中的目录，例如 _/usr/local/模型_。
3. 第三个字段（可选）是一个逗号分隔的选项列表，有关详细信息，请参阅[使用卷](https://docs.docker.com/storage/volumes/)。
...
此命令将主机_C：\输入_目录装载到容器 _/usr/local/模型_目录。

> [!IMPORTANT]
> 卷装载设置仅适用于**自定义语音到文本**和**自定义文本到语音转换**容器。 标准 **"语音到文本"** 和 **"文本到语音"** 容器不使用卷装载。
...
- **参数顺序**：除非熟悉 Docker 容器，否则不要更改参数的顺序。
...
### <a name="logging-example-for-speech-to-text"></a>记录语音到文本的示例
...
### <a name="logging-example-for-custom-speech-to-text"></a>记录自定义语音到文本的示例
...
### <a name="basic-example-for-text-to-speech"></a>文本到语音的基本示例
...
### <a name="logging-example-for-text-to-speech"></a>记录文本到语音转换的示例
...
### <a name="logging-example-for-custom-text-to-speech"></a>记录自定义文本到语音转换的示例
