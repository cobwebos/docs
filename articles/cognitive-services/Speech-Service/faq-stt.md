...
**问：我意识到我犯了一个错误。如何取消正在进行的数据导入或模型创建？**
...
**如果**单独提交每个通道（其自己的文件中的每个通道），则将在每个文件的持续时间内向您收费。 如果你提交单个文件，其中每个声道都一起多路复用，则按单个文件的持续时间对你收费。 有关定价的详细信息，请参阅 [Azure 认知服务定价页](https://azure.microsoft.com/pricing/details/cognitive-services/speech-services/)。
...
- 部署模型的区域；
- 已部署模型的终结点 ID：
  - 转到[自定义语音门户](https://aka.ms/customspeech)；
  - 视需要登录；
  - 选择项目和部署；
  - 选择需要增加其并发性的终结点；
  - 复制 `Endpoint ID`。
...
## <a name="tenant-model-custom-speech-with-office-365-data"></a>租户模型（自定义语音与 Office 365 数据）

**问：租户模型中包含哪些信息以及如何创建？**

**答：** 租户模型是使用[公共组](https://support.office.com/article/learn-about-office-365-groups-b565caa1-5c40-40ef-9915-60fdb2d97fa2)电子邮件和文档构建的，组织中的任何人都可以看到这些电子邮件和文档。

**问：租户模型改进了哪些语音体验？**

**答：** 启用、创建和发布租户模型后，它用于提高使用语音服务构建的任何企业应用程序的识别性;还将用户 AAD 令牌传递给企业，指示成员资格。

在为语音服务应用程序创建租户模型时，Office 365 中内置的语音体验（如听写和 PowerPoint 字幕）不会更改。
