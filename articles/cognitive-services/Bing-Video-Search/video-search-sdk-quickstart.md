...
ms.locfileid: "55152479"
---
# <a name="quickstart-bing-video-search-sdk-with-c"></a>快速入门：必应视频搜索 SDK 与 C# 

必应视频搜索 SDK 包含 REST API 的功能，可用于 Web 请求和分析结果。

Git Hub 上提供 [C# 必应视频搜索 SDK 示例的源代码](https://github.com/Azure-Samples/cognitive-services-dotnet-sdk-samples/tree/master/BingSearchv7/BingVideoSearch)。
...
若要使用必应视频搜索 SDK 设置控制台应用程序，请浏览到 Visual Studio 中的解决方案资源管理器中的“`Manage NuGet Packages`”选项。  添加 `Microsoft.Azure.CognitiveServices.Search.VideoSearch` 程序包。

安装 [[NuGet 视频搜索 SDK 程序包]](https://www.nuget.org/packages/Microsoft.Azure.CognitiveServices.Search.VideoSearch/1.2.0)还将安装依赖项，包括：
...
若要创建 `VideoSearchAPI` 客户端实例，请添加 using 指令：
```
...
分析结果，然后验证结果数，并输出第一个视频结果的 ID、名称和 URL。
```
...
依据查询文本“Bellevue Trailer”搜索未变更、简短和 1080p 分辨率的视频。  验证结果数，并输出第一个视频结果的 ID、名称和 URL。
...
搜索“Bellevue Trailer”视频，然后搜索有关第一个视频的详细信息。
```
