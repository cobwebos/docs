...
3. 创建名为的 `SearchVideos()` 新方法，其中包含 API 终结点主机和路径、订阅密钥与搜索词的变量。 此方法返回 `SearchResults` 对象。 `host` 可以是下面的全局终结点，也可以是资源的 Azure 门户中显示的[自定义子域](../../../cognitive-services/cognitive-services-custom-subdomains.md)终结点。
