...
在以下部分中，你将使用 Azure Cloud Shell 环境或 Azure PowerShell 使用自定义子域创建新的沉浸式读者资源，然后在 Azure 租户中配置 Azure AD。 完成初始配置后，你将调用 Azure AD 以获取访问令牌，这与使用沉浸式读者 SDK 时的操作方式类似。 如果遇到问题，每个部分都提供了每个 Azure PowerShell 命令的可用选项的链接。
