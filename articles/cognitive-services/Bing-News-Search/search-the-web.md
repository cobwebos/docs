...
若要生成仅限新闻的搜索结果页，以便查找与用户的搜索查询相关的新闻，请调用此 API，而不是调用更常规的[必应 Web 搜索 API](../bing-web-search/search-the-web.md)。 如果需要新闻和其他类型的内容，例如网页、图像和视频，则请调用必应 Web 搜索 API。
...
以下展示对上一查询的响应。 必须按响应中提供的顺序显示每篇新闻文章。 如果该文章有群集的文章，则应表明存在相关的文章并在用户请求查看这些文章时显示它们。
...
## <a name="getting-todays-top-news"></a>获取当天的头条新闻
...
## <a name="getting-news-by-category"></a>按类别获取新闻
...
## <a name="getting-headline-news"></a>获取标题新闻
...
## <a name="getting-trending-news"></a>获取热门新闻
