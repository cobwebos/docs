...
使用 [docker run](https://docs.docker.com/engine/reference/commandline/run/) 命令运行容器。 有关如何获取 [、](#gathering-required-parameters)、`{COMPUTER_VISION_ENDPOINT_URI}` 和 `{COMPUTER_VISION_API_KEY}` 值的详细信息，请参阅`{FORM_RECOGNIZER_ENDPOINT_URI}`收集必需的参数`{FORM_RECOGNIZER_API_KEY}`。

[ 命令的](form-recognizer-container-configuration.md#example-docker-run-commands)示例`docker run`可用。
