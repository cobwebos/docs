...
若要降低复杂性并轻松地将自定义表单识别器模型集成到工作流自动化过程或其他应用程序中，可以使用简单的 REST API 来调用该模型。 只需要五个表单文档（或一个空表单和两个填写表单），因此您可以快速、准确地获得结果，并根据特定内容进行定制。 而无需进行繁琐的手动干预，也不需要具备丰富的数据科学专业知识。 它不要求标记或标注数据。
...
| 函数 | 功能 |
|----------|----------|
| 表单识别器 | <li>处理 PDF、PNG 和 JPG 文件<li>使用至少五种相同布局的自定义模型进行训练 <li>提取键值对和表信息 <li>使用 Azure 认知服务计算机视觉 API 文本识别功能从表单内部的图像中检测和提取印刷体文本<li>无需进行标注或标记 |
...
| 计算机视觉 API 资源 | 若要处理扫描的文档和图像，需要计算机视觉资源。 可以访问作为 Azure 资源（REST API 或 SDK）或 *cognitive-services-recognize-text* [容器](../Computer-vision/computer-vision-how-to-install-containers.md#get-the-container-image-with-docker-pull)提供的文本识别功能。 使用此功能需要按普通费率付费。 <br><br>将计算机视觉资源（Azure 云或认知服务容器）的 API 密钥和终结点交用。 使用此 API 键，终结点为 **[COMPUTER_VISION_API_KEY]** 和 **[COMPUTER_VISION_ENDPOINT_URI]**。<br><br> 如果使用 *cognitive-services-recognize-text* 容器，请确保：<br><br>表单识别器容器的计算机视觉密钥是在 *cognitive-services-recognize-text* 容器的计算机视觉 `docker run` 命令中指定的密钥。<br>计费终结点是容器的终结点（例如 `http://localhost:5000`）。 如果在同一台主机上同时使用计算机视觉容器和表单识别器容器，不能使用默认端口 *5000* 将两者同时启动。 |
| 表单识别器资源 | 若要使用这些容器，必须具有：<br><br>用于获取关联的 API 密钥和终结点 URI 的 Azure**窗体识别器**资源。 这两个值在 Azure 门户**窗体识别器**概述和密钥页上都可用，启动容器需要这两个值。<br><br>**[FORM_RECOGNIZER_API_KEY]**： 密钥页上的两个可用资源键之一<br><br>**[FORM_RECOGNIZER_ENDPOINT_URI]**： 概述页上提供的终结点 |

> [!NOTE]
> 计算机视觉资源名称应为单个单词，没有连字符`-`或任何其他特殊字符。 此限制是为了确保表单识别器和识别文本容器的兼容性。

## <a name="gathering-required-parameters"></a>收集所需的参数

所有认知服务容器都需要三个主要参数。 最终用户许可协议 （EULA） 必须具有 的值。 `accept` 此外，还需要终结点 URL 和 API 密钥。

### <a name="endpoint-uri-computer_vision_endpoint_uri-and-form_recognizer_endpoint_uri"></a>端点`{COMPUTER_VISION_ENDPOINT_URI}`URI 和`{FORM_RECOGNIZER_ENDPOINT_URI}`
...
### <a name="keys-computer_vision_api_key-and-form_recognizer_api_key"></a>钥匙`{COMPUTER_VISION_API_KEY}`和`{FORM_RECOGNIZER_API_KEY}`
...
![获取两个密钥之一，供以后使用](../containers/media/keys-copy-api-key.png)
...
| 识别文本 | 1 核，8 GB 内存 | 2 个内核，8 GB 内存 |
...
## <a name="get-the-container-images-with-the-docker-pull-command"></a>使用 Docker 拉取命令获取容器映像

**表单识别器**和**识别文本**产品提供的容器映像在以下容器注册表中都有：
...
您需要两个容器，请注意，**识别器文本**容器在[本文外部是详细介绍的。](../Computer-vision/computer-vision-how-to-install-containers.md#get-the-container-image-with-docker-pull)
...
使用 [docker run](https://docs.docker.com/engine/reference/commandline/run/) 命令运行容器。 有关如何获取`{COMPUTER_VISION_ENDPOINT_URI}``{COMPUTER_VISION_API_KEY}``{FORM_RECOGNIZER_ENDPOINT_URI}`和`{FORM_RECOGNIZER_API_KEY}`值的详细信息，请参阅[收集所需参数](#gathering-required-parameters)。
