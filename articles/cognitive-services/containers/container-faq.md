...
**答：** 客户可以选择何时更新已部署的容器。 容器将使用标准[Docker 标记](https://docs.docker.com/engine/reference/commandline/tag/)（如 `latest`）进行标记，以指示最新版本。 我们鼓励客户在发布时请求最新版本的容器，签入[Azure 容器注册表 webhook](../../container-registry/container-registry-webhook.md) ，详细了解如何在更新映像时获得通知。
