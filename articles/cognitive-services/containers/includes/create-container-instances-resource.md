...
2. 在 "**基本**信息" 选项卡上，输入以下详细信息：

    |设置|值|
    |--|--|
    |订阅|选择订阅。|
    |资源组|选择可用的资源组，或创建一个新的资源组，如 `cognitive-services`。|
    ...
    |OS 类型|`Linux`|
    |大小|将大小更改为特定认知服务容器的建议建议：<br>2 CPU 内核数<br>4GB
...
4. 在 "**高级**" 选项卡上，为 Azure 容器实例资源的容器计费设置输入所需的**环境变量**：
    ...
    | 密钥 | 值 |
    |--|--|
    |`apikey`|从资源的 "**密钥**" 页中复制。 它是一个32的字母数字字符串，其中不含空格或短划线，`xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx`。|
    |`billing`|从资源的 "**概述**" 页中复制。|
    |`eula`|`accept`|

5. 单击 "**查看和创建**"
6. 验证通过后，单击 "**创建**" 以完成创建过程
7. 成功部署资源后，已准备就绪
