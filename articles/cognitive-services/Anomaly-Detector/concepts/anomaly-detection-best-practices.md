...
## <a name="when-to-use-batch-entire-or-latest-last-point-anomaly-detection"></a>何时使用 batch （整个）或最新（最后一个）点异常检测
...
* 批处理检测仅创建和应用一个模型，每个点的检测在整个序列的上下文中完成。 如果时序数据在无季节性的情况下上下趋势，则模型可能会丢失一些更改（数据的 dip 和峰值）。 同样，某些不太重要的更改点可能不会被视为足以合并到模型中的重要部分。
...
如果以非标准时间间隔对数据进行采样，则可以通过在请求中添加`customInterval`特性来指定数据。 例如，如果序列每5分钟进行一次采样，则可将以下内容添加到 JSON 请求：
...
缺少的数据点在平均分布式时序数据集中常见，尤其是具有精细粒度的数据点（一小采样间隔）。 例如，每隔几分钟采样一次数据。 如果数据中的预期点数不到10%，则不会对检测结果产生负面影响。 请考虑根据数据的特性（例如，从较早的时间段、线性内插或移动平均线中替代数据点）来填充数据中的空白。
...
如果你知道时序数据具有季节性模式（按固定间隔发生），可以提高准确性和 API 响应时间。 

在构造`period` JSON 请求时指定，可将异常检测滞后时间降低到50%。 `period`是一个整数，该整数指定时序用来重复模式的数据点的数量。 例如，每日包含一个数据点的`period`时间序列的数据点为`7`，而每小时有一个点（具有相同的每周模式）的`period` `7*24`时序将具有一个。 如果你不确定数据的模式，则无需指定此参数。

为获得最佳结果，请`period`提供4个值得的数据点，再加上一个额外的数据点。 例如，上述按周模式列出的每小时数据应在请求正文中提供673数据点（`7 * 24 * 4 + 1`）。

### <a name="sampling-data-for-real-time-monitoring"></a>为实时监视采样数据

如果流式处理数据在较短的时间间隔（例如，几秒钟或几分钟）采样，则发送推荐数量的数据点可能会超出异常检测程序 API 的最大允许值（8640数据点）。 如果数据显示稳定的季节性模式，请考虑以更大的时间间隔（如小时）发送时序数据的示例。 以这种方式采样你的数据也会明显缩短 API 响应时间。 
