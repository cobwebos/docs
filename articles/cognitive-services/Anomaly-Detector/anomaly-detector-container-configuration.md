...
`Billing` 设置指定 Azure 上的_异常探测器_资源的终结点 URI，该 URI 用于计量容器的计费信息。 您必须为此配置设置指定一个值，并且该值必须是 Azure 上_异常探测器_资源的有效终结点 URI。

可以在以下位置找到此设置：

* Azure 门户：**异常检测程序的**概述，已标记 `Endpoint`
