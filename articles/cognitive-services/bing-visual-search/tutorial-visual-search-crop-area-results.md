...
变量 `VisualSearchRequest` 包含关于图像的裁剪区域及其 URL 的信息。 `VisualSearchMethodAsync()` 方法获取结果。
...
必应视觉搜索结果为 `ImageTag` 对象。  每个标记都包含 `ImageAction` 对象的列表。  每个 `ImageAction` 均包含一个 `Data` 字段，它是取决于操作类型的值列表。
...
## <a name="get-urls-for-pagesincluding-actiontype-images"></a>获取用于 PagesIncluding ActionType 图像的 URL
