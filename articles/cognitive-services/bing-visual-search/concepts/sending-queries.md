...
- 使用从上一个调用中从映像到[必应映像搜索 API](https://docs.microsoft.com/rest/api/cognitiveservices/bing-images-api-v7-reference)终结点之一的见解令牌。
- 发送图像的 URL。
- 上传图像（以二进制格式）。

## <a name="bing-visual-search-requests"></a>必应视觉搜索请求

如果向可视化搜索发送图像令牌或 URL，以下代码段将显示必须在 POST 正文中包含的 JSON 对象：
...
`imageInfo` 对象必须包含 `url` 或 `imageInsightsToken` 字段，但不可同时包含两者。 将`url`字段设置为可 Internet 访问图像的 URL。 受支持的最大图像大小为 1 MB。
...
`filters` 对象包含站点筛选器（请参阅 `site` 字段），可用于将相似图像和相似产品限制为特定域。 例如，如果图像是 Surface Book，则可以设置为`site``www.microsoft.com`。
...
您可以选择将`enableEntityData`标题中的属性设置为，`true`以获取有关所上载图像中主实体的详细信息，包括指向 Web 的链接和归因信息。 默认情况下，此字段`false`为。
...
如果上载本地图像，以下代码段将显示必须包含在 POST 正文中的表单数据。 窗体数据必须包括`Content-Disposition`标头。 它的 `name` 参数必须设置为“image”，`filename` 参数可以设置为任何字符串。 标头`Content-Type`可以设置为任何常用的图像哑剧类型。 窗体的内容是图像的二进制数据。 可以上传的最大图像大小为 1 MB。 宽度或高度不得超过 1,500 像素。
...
以下代码段显示传递图像令牌和感兴趣区域的完整图像见解请求。 从上一次调用 /image/搜索中，您将获得见解令牌：
...
如果存在适用于图像的见解，响应包含一个或多个 `tags`（包含见解）。 该`image`字段包含输入图像的见解标记：
...
`tags` 字段包含显示名称和操作（见解）列表。 其中一个标记包含 `displayName` 字段（设置为空字符串）。 此标记包含默认见解，如包含图像的网页、视觉上相似的图像、在图像中找到的项的购物源。 由于整个图像都值得关注，因此默认见解标记不包括感兴趣区域的边界框：
...
有关默认见解的列表，请参阅[默认见解标记](../default-insights-tag.md)。

剩余的标记包含用户可能感兴趣的其他见解。 例如，如果图像包含文本，其中一个标记可能包含 TextResults 见解（包含已识别文本）。 或者，如果必应识别图像中的实体（即文化上知名/受欢迎的人员、地点或事物），则其中一个标记可能会标识该实体。 视觉搜索还可以返回一组派生自输入图像的不同术语（标记）。 这些标记使用户能够探索图像中的概念。 例如，如果输入图像是一位著名运动员，其中一个标记可能是体育（包含指向体育图像的链接）。
...
由于标记的 `displayName` 字段包含 ##TextRecognition，因此不要将其用作 UX 中的类别标题。 这适用于以 ## 开头的任何显示名称。 而是使用操作的显示名称。
...
如果图像包含公认的实体，如文化上知名/受欢迎的人员、地点或事物，则其中一个标记可能包含实体见解。 仅当`mainEntity`标头`data`中`enableEntityData`的属性设置为`Content-Type``true`时，和 字段才可用。
...
- [教程：创建可视化搜索单页 Web 应用](../tutorial-bing-visual-search-single-page-app.md)
