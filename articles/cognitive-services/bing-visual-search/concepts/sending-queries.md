...
剩余的标记包含用户可能感兴趣的其他见解。 例如，如果图像包含文本，其中一个标记可能包含 TextResults 见解（包含已识别文本）。 或者，如果 Bing 识别某个实体 (即，在图像中有一个区域性熟知的人员、地点或事物) ，则其中一个标记可能会标识实体。 视觉搜索还可以返回一组派生自输入图像的不同术语（标记）。 这些标记使用户能够浏览在图像中找到的概念。 例如，如果输入图像是一位著名运动员，其中一个标记可能是体育（包含指向体育图像的链接）。
