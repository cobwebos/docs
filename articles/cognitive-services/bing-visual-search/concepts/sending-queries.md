...
`filters` 对象包含站点筛选器（请参阅 `site` 字段），可用于将相似图像和相似产品限制为特定域。 例如，如果图像是 Surface Book，则可以将 `site` 设置为 `www.microsoft.com`。
...
如果图像包含服务识别的文本，其中一个标记将包含 TextResults 见解（操作）。 见解的 `displayName` 包含识别的文本：
