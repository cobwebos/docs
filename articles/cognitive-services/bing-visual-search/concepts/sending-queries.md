...
`imageInfo` 对象必须包含 `url` 或 `imageInsightsToken` 字段，但不可同时包含两者。 `url`将字段设置为可访问 Internet 的映像的 URL。 受支持的最大图像大小为 1 MB。
...
以下是请求应指定的标头。 `X-MSEdge-ClientID` `User-Agent` `X-MSEdge-ClientIP` `X-Search-Location`和标头是唯一必需的标头, 但你还应包括、、和。 `Ocp-Apim-Subscription-Key` `Content-Type`
