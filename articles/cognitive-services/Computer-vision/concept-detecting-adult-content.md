...
你可以通过 "[分析映像](https://westus.dev.cognitive.microsoft.com/docs/services/5adf991815e1060e6355ad44/operations/56f91f2e778daf14a499e1fa)API" 检测成人内容。 将`Adult`的值添加到**visualFeatures**查询参数时，API 会在其 JSON 响应中返回&mdash;`isAdultContent`三`isRacyContent`个布尔`isGoryContent` &mdash;属性：和。 此方法还返回相应的&mdash;`adultScore`属性`racyScore`， `goreScore` &mdash;以及表示每个相应类别的零和一个的置信度得分。
