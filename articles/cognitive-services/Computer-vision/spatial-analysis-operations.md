...
|---------|---------|---------|
| `id` | 字符串| 相机 ID|
| `timestamp` | 日期| 发出 JSON 有效负载时的 UTC 日期|
| `width` | int | 视频帧宽度|
| `height` | int | 视频帧高度|
| `frameId` | int | 帧标识符|
...
为了获得更好的 Gpu 性能和利用率，你可以使用图形实例在多个相机上部署任何空间分析操作。 下面是一个示例，用于在5个 (5) 相机上运行 spatialanalysis-personcount 操作。
