...
1. 在“我的应用”列表中，选择“+ 新建用于对话的应用”  。

1. 在对话框中，将应用程序命名为 `Home Automation`。 选择“英语”作为区域性  。 说明是可选的，不会用于创作或预测。 创建 LUIS 应用时，预测资源也是可选的。 选择“完成”  。

    LUIS 创建应用程序。 将应用发布到生产环境时，应该分配一个预测资源，使应用能够处理多个请求。
...
1. 在右上方导航中选择“测试”。  1. 在“交互式测试”窗格中键入 `Turn off the lights` 之类的测试言语，然后按 Enter。
