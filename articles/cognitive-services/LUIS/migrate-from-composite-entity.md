---
title: 升级复合实体-LUIS
description: 在 LUIS 门户中，通过升级过程将复合实体升级到机器学习的实体。
ms.topic: how-to
ms.date: 05/04/2020
...
ms.locfileid: "83599421"
---
# <a name="upgrade-composite-entity-to-machine-learned-entity"></a>将复合实体升级到机器学习的实体

将复合实体升级到机器学习的实体，以生成一个实体，该实体接收更完整的预测，并提供更好的 decomposability 来调试实体。

## <a name="current-version-model-restrictions"></a>当前版本模型限制

升级过程会根据在应用中找到的现有复合实体，在应用的新版本中创建计算机学习的实体。 这包括复合实体子级和角色。 此过程还会将示例最谈话中的标签切换为使用新实体。
...
* 为每个复合实体创建新的机器学习实体。
* 子实体：
    * 如果子实体仅用于复合实体，则只会将其添加到机器学习的实体。
    * 如果子实体在复合中使用 _，并_作为单独的实体（如示例最谈话中所示），则它将作为实体添加到版本，并作为子实体添加到新的机器学习的实体。
    * 如果子实体使用角色，则每个角色都将转换为具有相同名称的子实体。
    * 如果子实体为非计算机学习的实体（正则表达式、列表实体或预生成的实体），则将使用相同的名称创建一个新的子实体，而新的子实体将具有添加了所需功能的非计算机学习实体的功能。
* 名称保留，但在相同的子实体/同级级别必须是唯一的。 请参阅[唯一命名限制](luis-boundaries.md#name-uniqueness)。
* 示例最谈话中的标签通过子实体切换到新机器学习的实体。

使用下图来了解模型如何变化：

|旧对象|新建对象|备注|
|--|--|--|
|复合实体|采用结构的计算机学习实体|这两个对象都是父对象。|
...
1. 完成此过程后，您将使用新的、计算机学习的实体的新训练版本。 选择 "**尝试新实体**" 以查看新实体。
