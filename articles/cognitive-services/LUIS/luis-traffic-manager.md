...
1. 在 Azure 门户中，创建两个语言理解密钥，一个位于 **，一个位于** `West US``East US`。 使用上一节中创建的名为 `luis-traffic-manager` 现有资源组。 
...
由于每个 LUIS 终结点需要自己的路径，因此也需要其自己的流量管理器配置文件。 若要跨配置文件进行管理，请创建[嵌套流量管理器_体系结构_](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-nested-profiles)。 一个父配置文件指向子配置文件，并管理它们之间的流量。
