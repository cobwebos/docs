---
title: 教程：了解如何使用 Ruby 将陈述添加到 LUIS 应用 | Microsoft Docs
description: 本教程介绍如何使用 Ruby 调用 LUIS 应用。
...
ms.locfileid: "36266050"
---
# <a name="tutorial-add-utterances-to-app-using-ruby"></a>教程：使用 Ruby 将陈述添加到应用 
在本教程中，我们将编写一个程序，以使用 Ruby 中的创作 API 将陈述添加到意向。
...
* 现有的 LUIS [**应用程序 ID**](./luis-get-started-create-app.md)。 应用程序仪表板中显示了应用程序 ID。 在运行 `add-utterances.rb` 中的代码之前，包含 `utterances.json` 文件中所用的意向和实体的 LUIS 应用程序必须存在。 本文中的代码不会创建意向和实体。 它只会添加现有意向和实体的陈述。 
* 接收陈述的应用程序中的**版本 ID**。 默认 ID 为“0.1”
* 在 VSCode 中创建名为 `add-utterances.rb` 的文件项目。

> [!NOTE] 
> `utterances.json`[LUIS-Samples **Github 存储库**中提供了完整的 `add-utterances.cs` 文件和示例 ](https://github.com/Microsoft/LUIS-Samples/blob/master/documentation-samples/authoring-api-samples/ruby/) 文件。


## <a name="write-the-ruby-code"></a>编写 Ruby 代码
...
> [!NOTE]
> 在运行 `add-utterances.rb` 中的代码之前，包含 `utterances.json` 文件中所用的意向和实体的 LUIS 应用程序必须存在。 本文中的代码不会创建意向和实体。 它只会添加现有意向和实体的陈述。
...
仅结合 utterance.json 作为参数调用 `add-utterances.rb` 会添加 LUIS，但不会基于新陈述训练 LUIS。
````
