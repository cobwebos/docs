---
title: 教程：了解如何使用 Node.js 调用语言理解 (LUIS) 应用 | Microsoft Docs
...
ms.locfileid: "36265398"
---
# <a name="tutorial-call-a-luis-endpoint-using-javascript"></a>教程：使用 JavaScript 调用 LUIS 终结点
...
4. 返回 [LUIS](luis-reference-regions.md) 网站，使用 Azure 帐户登录。 
    ...
    [![](media/luis-get-started-node-get-intent/app-list.png "“创建应用列表”的屏幕截图")](media/luis-get-started-node-get-intent/app-list.png)
...
1. 示例应用的终结点采用以下格式：`https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/df67dcdb-c37d-46af-88e1-8b97951ca1c2?subscription-key=<YOUR_API_KEY>&verbose=false&q=turn%20on%20the%20bedroom%20light`。 复制该 URL，并将 `subscription-key` 字段的值替换为自己的订阅密钥。
...
## <a name="consume-a-luis-result-using-the-endpoint-api-with-javascript"></a>在 JavaScript 中通过终结点 API 使用 LUIS 结果 
...
2. 在以下代码行中，将 `"YOUR SUBSCRIPTION KEY"` 替换为自己的订阅密钥：`xhrObj.setRequestHeader("Ocp-Apim-Subscription-Key","YOUR SUBSCRIPTION KEY");`

3. 使用 Web 浏览器打开保存的文件。  此时应会弹出一个警报窗口，指出 `Detected the following intent: TurnOn`。

![指出 TurnOn 的弹出窗口](./media/luis-get-started-node-get-intent/popup-turn-on.png)
