...
4. 在[发布](PublishApp.md)页上，选中“包含所有预测意向评分”复选框。 选中复选框后，将返回所有意向。 如果取消选中该复选框，则仅返回最高意向。 

5. [发布](PublishApp.md)应用。 发布应用后，可使用 HTTPS 终结点对其进行测试。 
...
> * 通过[终结点测试](PublishApp.md#test-your-published-endpoint-in-a-browser)，可以查看精确的 LUIS 响应 JSON。 

## <a name="get-the-entity-score-with-the-endpoint-test"></a>使用终结点测试获取实体得分
若要查看实体得分，请[发布模型](PublishApp.md)并查询终结点。 
