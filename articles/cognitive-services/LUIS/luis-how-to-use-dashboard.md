...
对于在意向中不包含任何示例言语的应用（称为仅限模式的应用），此页不会提供相关的分析。__ 
...
|问题|图表颜色|说明|
|--|--|--|
|数据不平衡|-|当示例言语的数量存在显著的差异时，将出现此问题。 所有意向需要有大致相同的示例言语数量 - None 意向除外。__ None 意向的数量应该只占应用中言语总数的 10%-15%。<br><br> 如果数据不平衡但意向准确度超过特定的阈值，则不会将这种不平衡报告为一种问题。<br><br>**请从此问题着手 - 它可能是其他问题的根本原因。**|
...
|错误的预测|Red|未针对标记的意向（示例言语所在的意向）预测示例言语时，会出现此问题。|
...
分析页不指示何时使用[模式](luis-concept-patterns.md)或[短语列表](luis-concept-feature.md)。 如果确实添加它们，它可以帮助进行不正确或不明确的预测，但无助于数据不平衡。 
