...
ms.locfileid: "80879318"
---
# <a name="install-and-run-luis-docker-containers"></a>安装并运行 LUIS docker 容器
 
语言理解 （LUIS） 容器加载您训练或发布的语言理解模型。 作为[LUIS 应用](https://www.luis.ai)，docker 容器提供从容器的 API 终结点访问查询预测。 可以从容器收集查询日志并将其上载回"语言理解"应用，以提高应用的预测准确性。
...
1. 使用所需的输入装入点__ 和计费设置[运行容器](#run-the-container-with-docker-run)。 提供 `docker run` 命令的多个[示例](luis-container-configuration.md#example-docker-run-commands)。 
...
输入装入点目录可以同时包含应用的**** 生产、过渡**** 和版本控制模型****。 所有包均已装载。
...
|打包要求|详细信息|
|--|--|
|Azure __ 认知服务资源实例|支持的区域包括<br><br>美国西部 (`westus`)<br>西欧 (`westeurope`)<br>澳大利亚东部 (`australiaeast`)|
...
要下载已发布的包，请参阅此处的[API 文档][download-published-package]。 如果下载成功，响应是一个 LUIS 包文件。 将文件保存在为容器的输入装入点指定的存储位置中。 
...
要下载版本化包，请参阅此处的[API 文档][download-versioned-package]。 如果下载成功，响应是一个 LUIS 包文件。 将文件保存在为容器的输入装入点指定的存储位置中。 
...
* 从 LUIS 容器映像运行容器
* 从*C：\input*的输入装载加载 LUIS 应用，位于容器主机上
* 分配两个 CPU 内核和 4 千兆字节 (GB) 的内存
* 公开 TCP 端口 5000，并为容器分配伪 TTY
* 将容器和 LUIS 日志保存到位于容器主机上的*C：\输出*处的输出装载
...
若要对过渡**** 环境执行查询，请将路由中的 `production` 替换为 `staging`：
...
LUIS 容器使用 Azure 帐户中的认知服务__ 资源向 Azure 发送账单信息。 
...
* 有关已知功能限制，请参阅[LUIS 容器限制](luis-container-limitations.md)。
