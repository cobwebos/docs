...
|文本匹配实体|目的|
|--|--|
|[列表实体](#add-list-entities-for-exact-matches)|规范名称和同义词作为替代形式的列表|
...
在下面的查询文本中，有两个位置，每个位置都由围绕它的单词（例如`to`和`from`）在语义上指定： 

`Pick up the package from Seattle and deliver to New York City.`

在此过程中， `origin`将`destination`和角色添加到预生成的 geographyV2 实体。
...
1. 在 "**类型角色 ...** " 文本框中，输入角色`Origin`的名称，然后输入。 添加第二个角色名称`Destination` ，然后输入。 
