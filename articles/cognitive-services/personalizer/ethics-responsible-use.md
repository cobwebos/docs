...
若要让人和社会认识到 AI 的全部潜力，需以特定方式对实现进行设计，使之赢得那些将 AI 添加到其应用程序中的人们的信任，以及使用 AI 构建的应用程序的用户的信任。 这些指南旨在帮助你以特定方式实现个性化体验创建服务，而该方式有助于你培养他人对你的公司和服务的信任。 请务必暂停，了解个性化对人员生活的影响的信息。 如有疑问，请寻求指导。
...
可以使用以下内容作为入门清单，并根据自己的方案对其进行自定义和优化。 本文档包含两个主要部分：第一个部分专门用于在为 Personalizer 选择方案、功能和回报时突出显示责任使用注意事项。 第二部分介绍一系列的价值。Microsoft 认为在在构建 AI 系统时应该考虑到这些价值，并提供可行的建议和应该避免的风险，说明你在使用个性化体验创建服务时会对它们造成什么影响。
...
* 选择可应用个性化的用例。
* 构建[奖励函数](concept-rewards.md)。
* 选择将哪些与上下文和可能的操作有关的[特征](concepts-features.md)用于个性化。
...
在特定情况下使用个性化体验创建服务时，会提高造成负面影响或缺乏透明性的可能性，例如，“奖励”取决于许多长期的复杂因素，如果将这些因素过度简化成一种即时奖励，可能会为个人带来不希望的结果。 这些因素往往被视为 "后果性的" 选择，或者涉及危害风险的选择。 例如：


* **财务**：针对贷款、金融和保险产品的个性化产品/服务，其中风险因素基于个人不知道的数据、无法获取或不能争议。
* **教育**：对学校课程和教育机构的排名进行个性化，其中的建议可能会传播偏差并减少用户对其他选项的认识。
* **民主和市政参与**：为具有影响观点的目标的用户个性化内容是 manipulative 的。
* **第三方奖励评估**：根据用户的后一方评估（而不是由用户自己的行为生成的奖励）个性化奖励。
* **Intolerance**： Personalizer 的浏览行为可能导致损害。
...
* **用户统计**信息：有关性别、性别、年龄、种族、宗教的功能：出于法规原因，某些应用程序中可能不允许这些功能，因为个性化设置将传播归纳和偏向。 关于上面所说的传播偏见的行为，一个例子是在发布工程方面的工作招聘广告时，不向老年人或特定性别的受众显示该广告。
* **区域设置信息**：在世界各地的许多地方，位置信息 (如邮政编码、邮政编码或邻居名称) 可以与收入、种族和宗教高度关联。
* **用户理解公平**：即使在应用程序做出合理决策的情况下，用户觉察在应用程序中显示的内容的效果会以似乎与将歧视的功能关联的方式进行更改。
* **功能中的意外偏差**：可以使用只影响总体子集的功能引入的偏移类型。 如果特征是通过算法生成的（例如，使用图像分析提取图片中的项目，或通过文本分析发现文本中的实体），则需特别注意这一点。 让自己注意那些用于创建这些特征的服务的特征。
...
* 问问问问问问问问问：如果我使用此信息来个性化他们的内容，我的用户会很乐意， 如果向他们展示突出显示或隐藏某些项目的具体决策过程，我会乐意吗？
...
*AI 系统应安全可靠地执行*。 对于个性化体验创建服务，请注意以下事项：
...
*AI 系统应安全，并尊重隐私*。 使用个性化体验创建服务时，请遵守以下准则：
...
* 请参阅 Microsoft 在2018年1月发布的 AI 开发 AI 的六个原则， [以后计算](https://news.microsoft.com/futurecomputed/)
...
[功能：操作和上下文](concepts-features.md)。
