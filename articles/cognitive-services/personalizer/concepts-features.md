...
在以下 JSON 中，`user`、`state` 和 `device` 是特征命名空间。 公共预览说明:目前, 我们强烈建议使用基于 UTF-8 并以不同字母开头的功能命名空间的名称。 `user`例如,、`s`和`u`从、和`d`开始。 `device` `state` 当前具有具有相同第一个字符的命名空间可能会导致用于机器学习的索引出现冲突。
