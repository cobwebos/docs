...
* 内容，视频是 `Documentary`、`Movie` 还是 `TV Series`，或者某个零售商品在商店中是否有货。__
* 当前时间段，例如星期几。__

个性化程序不规定、限制或修复您可以发送的操作和上下文的功能：
...
* 对于分类要素，不需要定义可能的值，也不需要预先定义数值的范围。

## <a name="supported-feature-types"></a>支持的特征类型

个性化体验创建服务支持字符串、数字和布尔类型的特征。

### <a name="how-choice-of-feature-type-affects-machine-learning-in-personalizer"></a>功能类型的选择如何影响个性化中的机器学习

* **字符串**：对于字符串类型，键和值的每个组合都会在个性化机器学习模型中创建新的权重。 
* **数字**：当数字应按比例影响个性化结果时，应使用数值。 这是非常依赖于方案。 在简化示例中，例如个性化零售体验时，NumberOfPetsOfs 可能是一个数字功能，因为您可能希望拥有 2 或 3 只宠物的人对个性化结果的影响两次或三倍于拥有 1 只宠物。 基于数值单位但含义不是线性的要素（如年龄、温度或人员高度）最好编码为字符串，并且通常可以使用范围来提高要素质量。 例如，年龄可以编码为"年龄"："0-5"，"年龄"："6-10"等。
* 发送的值为"false"的**布尔**值就像它们根本不发送一样。
...
可以根据自己的约定来命名特征命名空间，只要它们是有效的 JSON 键即可。 命名空间用于将要素组织成不同的集，以及消除具有类似名称的要素的歧义。 可以将命名空间视为添加到要素名称的"前缀"。 不能嵌套命名空间。


在以下 JSON 中，`user`、`state` 和 `device` 是特征命名空间。 

> [!Note]
> 目前，我们强烈建议对基于 UTF-8 的要素命名空间使用名称，并使用不同的字母开头。 `user`例如，`state``device`和 开头`u`，`s`和`d`。 目前具有相同第一个字符的命名空间可能会导致用于机器学习的索引发生冲突。

JSON 对象可以包括嵌套的 JSON 对象和简单的属性/值。 仅当数组项为数字时，才能包含数组。 
...
### <a name="restrictions-in-character-sets-for-namespaces"></a>命名空间的字符集中的限制

用于命名命名空间的字符串必须遵循一些限制： 
* 不能是单码。
* 可以使用一些带有代码< 256 的可打印符号来命名。 
* 不能使用代码< 32（不可打印）、32（空格）、58（冒号）、124（管道）和 126-140 的代码符号。
* 它不应以下划线"*"开头，否则该功能将被忽略。
...
位置信息通常还受益于创建更广泛的分类。 例如，纬度经坐标（如 Lat：47.67402° N，长：122.12154° W）过于精确，并强制模型将纬度和经度作为不同的尺寸来学习。 当您尝试根据位置信息进行个性化设置时，它有助于对较大扇区中的位置信息进行分组。 一个简单的方法是为 Lat-Long 数字选择适当的舍入精度，并通过将它们组合成一个字符串，将纬度和经度组合成"区域"。 例如，在大约几公里宽的区域中，表示 47.67402° N、长：122.12154° W 的好方法是"位置"："34.3， 12.1"。


#### <a name="expand-feature-sets-with-extrapolated-information"></a>使用推理出的信息扩展特征集

评估可从已有信息派生的未探索属性也可以获得更多的特征。 例如，在虚构的电影列表个性化中，周末与工作日是否引起用户的不同行为？ 可将时间扩展为包含“周末”或“工作日”属性。 国家法定节假日是否会推动观众对某类电影的兴趣？ 例如，在相关的场合中，可以使用“万圣节”属性。 雨天是否会明显影响许多人选择观看电影？ 如果使用时间和地点，则天气服务可以提供此类信息，你可以将此信息添加为额外的特征。 
...
* 具有_事件_ID。 如果您已经拥有事件 ID，则应提交该 ID。 如果您没有事件 ID，请不要发送，个人化器会为您创建一个，并在 Rank 请求的响应中返回它。 ID 与 Rank 事件关联，而不是与用户关联。 如果创建 ID，GUID 效果最佳。 
* 具有特征列表。
...
在某些情况下，只能稍后在业务逻辑中确定是否要向用户显示排名 API 调用的最终操作。__ 对于这种情况，应使用非活动事件。__
...
上下文信息取决于每个应用程序和用例，但往往包括如下所述的信息：__
