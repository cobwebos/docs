---
title: 使用离线评估方法 - 个性化程序
titleSuffix: Azure Cognitive Services
description: 本文将解释如何使用脱机评估来衡量应用的有效性并分析学习循环。
...
使用脱机评估方法可以在不更改代码或影响用户体验的情况下，测试和评估个性化体验创建服务的有效性。 离线评估使用从应用程序发送到排名和奖励 API 的过去数据来比较不同等级的表现。
...
执行脱机评估后，可以看到个性化体验创建服务使用新策略与使用当前联机策略时的比较有效性。 然后，您可以通过下载该学习策略并将其上载到"模型和策略"面板中，使其在个性化设置中立即生效。 您也可以下载它供将来分析或使用。

评价中包括的现行政策：

| 学习设置 | 目的|
|--|--|
|**在线政策**| 个性化体验创建服务中当前使用的学习策略 |
|**基线**|应用程序的默认值（由在 Rank 调用中发送的第一个操作确定）|
...
* 是否意外包含了任何特征？ 例如：用户身份信息、重复 ID 等。
* 出于法规或负责使用方面的考量，是否不应使用任何不需要的特征进行个性化？ 是否有特征可以代理（即，紧密镜像或关联）不需要的特征？


## <a name="next-steps"></a>后续步骤

[配置个性化程序](how-to-settings.md)
[运行脱机评估](how-to-offline-evaluation.md)了解[个性化程序的工作原理](how-personalizer-works.md)
