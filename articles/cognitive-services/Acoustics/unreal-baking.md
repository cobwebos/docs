...
* **[项目]/Content/Acoustics/[级别名称]\_AcousticsData.ace**：此文件创建于烘培模拟过程中，包含运行时所用的查找数据，以呈现场景的音效。 使用“探测”选项卡上的字段可更改此文件的位置和名称  。如果要在创建此文件后重命名此文件，请从 Unreal 项目中删除 UAsset，在文件资源管理器中将此文件重命名为 Unreal 之外的文件，然后将此文件重新导入到 Unreal 中以生成新的 UAsset。 重命名 UAsset 本身将无法正常工作。
