...
此示例包括演示场景 Assets/AcousticsDemo/ProjectAcousticsDemo.unity。 此场景包含单个空间化的音频源，从浮点多维数据集（在“层次结构”中名为 AudioHolder）播放。 为了帮助编写泛型导航脚本，Main Camera 为 CameraHolder 对象的子级。 

![层次结构视图](media/SampleHierarchyView.png)

已制作场景，并且该场景包含与“层次结构”中的 MicrosoftAcoustics 预制件相关联的 ACE 文件。 

通过单击 Unity 编辑器中的播放按钮，侦听场景的声音效果。 使用 W、A、S、D 和鼠标进行移动。 若要比较含音响效果和不含音响效果的场景的声音效果，请单击鼠标左键或主控制器按钮。 若要循环各种声源，请单击鼠标右键或控制器上的后退按钮。
