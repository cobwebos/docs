...
当你使用[审阅工具](https://contentmoderator.cognitive.microsoft.com)注册时, 系统将提示你为你的帐户选择一个 Azure 区域。 这是因为,[查看工具](https://contentmoderator.cognitive.microsoft.com)会为 Azure 内容审查器服务生成免费试用密钥;需要使用此密钥从 REST 调用或客户端 SDK 访问任何服务。 可以通过选择 "**设置** > **凭据**" 查看密钥和 API 终结点 URL。
