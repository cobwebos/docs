...
使用内容审查器的机器辅助文本审查和[人机闭环](Review-Tool-User-Guide/human-in-the-loop.md) (human-in-the-loop) 功能来审查文本内容。

企业可以使用文本审查服务，根据其策略和阈值来阻止、审批或评审内容。 文本审查服务可以辅助需要合作伙伴、员工和使用者生成文本内容的环境的人工审查。 这些环境包括聊天室、讨论区、聊天机器人、电子商务目录、文档，等等。 

API 会扫描传入文本（最大 1024 字符）中的亵渎内容、对可能不需要的文本进行分类（预览版）、自动更正文本，并检测潜在的个人身份信息 (PII)。 它还会匹配自定义的字词列表。 自动更正功能可帮助捕获到有意拼错的单词。 处理内容后，服务会返回详细的响应。 使用该响应可以在评审工具中创建人工评审、撤消某项评审，或执行其他操作。

服务响应包含以下信息：

- 亵渎内容：根据多种语言的内置亵渎字词列表执行基于字词的匹配
- 分类：在机器的辅助下分类成三个类别
- 个人身份信息 (PII)
- 自动更正的文本
- 原始文本
...
内容审查器的机器辅助**文本分类功能**仅支持**英语**，可帮助检测可能不需要的内容。 标记的内容可能会被视为不适合，具体取决于上下文。 除了传达每个类别的可能性，它可能会建议对内容进行人工评审。 该功能使用训练的模型来识别可能的辱骂、贬损或歧视性语言。 要评审的内容包括俚语、缩写词、冒犯性言语，以及有意拼错的单词。 
...
尽管在默认情况下，全局字词列表能够很好地满足大部分需要，但你可能想要根据自己的具体业务需求筛选字词。 例如，你可能想要从用户的发布内容中，筛选出所有竞争品牌名称。 你的允许文本内容阈值可能不同于默认列表。
