...
好在有一些方法可以“隐藏”听录时间，即在用户说出话语后一部分的同时，对话语的前一部分执行听录。 例如，通过将拆分为 100 毫秒的 10 个区块 1 第二个查询文本并反过来对每个区块执行脚本，超过 450 脚本所需的总 500 毫秒的可以为"隐藏"，以便用户不会意识到听录是正在执行时他们说到。 联系此示例时，请记住该服务对音频的前一个 100 毫秒执行听录，而此时用户正在说出下一个 100 毫秒，所以当用户停止讲话时，该服务只需要听录大约 100 毫秒的音频来制作结果。
