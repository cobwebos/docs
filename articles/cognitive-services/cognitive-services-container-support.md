...
|[人脸][fa-containers] |F0，S0|**人脸** |检测图像中的人脸并标识属性，包括人脸特征（例如，鼻子和眼睛）、性别、年龄和其他计算机预测的面部特征。 除检测外，“人脸”还可以使用置信评分检查同一/不同图像中的两张人脸，或根据数据库比较人脸，以查看是否已存在类似或相同的人脸。 还可以使用共享视觉特征将类似人脸整理为许多组。<br>[请求访问权限](Face/face-how-to-install-containers.md#request-access-to-the-private-container-registry) |
|[窗体识别器][fr-containers] |F0，S0|**窗体识别器** |窗体理解应用机器学习技术，从表单中识别和提取键值对和表。<br>[请求访问权限](https://aka.ms/FormRecognizerContainerRequestAccess)|
...
> [!IMPORTANT]
> 目前，你必须完成注册过程才能访问以下容器，在此过程中，你可以填写和提交包含有关你的问题、你的公司以及你要实现容器的用例。 获得访问权限并提供凭据后，可以从 Azure 容器注册表托管的专用容器注册表中提取容器映像。
> * [异常探测器](Anomaly-Detector/anomaly-detector-container-howto.md#request-access-to-the-container-registry)
