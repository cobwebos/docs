...
使用[CIDR 表示法](https://tools.ietf.org/html/rfc4632) `16.17.18.0/24`或以单个 IP 地址（如`16.17.18.19`）提供允许的 internet 地址范围。

   > [!Tip]
   > 不支持使用“/31”或“/32”前缀大小的小型地址范围。 这些范围应使用单独的 IP 地址规则配置。

IP 网络规则仅适用于**公共 Internet** IP 地址。 IP 规则不允许使用为专用网络保留的 IP 地址范围（如 [RFC 1918](https://tools.ietf.org/html/rfc1918#section-3) 中所定义）。 专用`10.*`网络包括以、 `172.16.*`  -  `172.31.*`和`192.168.*`开头的地址。
