...
description: 从自定义愿景团队获取有关新版本的最新信息。
...
- 改进了与 Azure 订阅相关的门户用户体验，从而更轻松地选择 Azure 目录。

## <a name="april-18-2019"></a>2019 年 4 月18 日 

- 为视觉 AI 开发工具包添加了对象检测导出。
- UI 调整，包括项目搜索。

## <a name="april-3-2019"></a>2019年4月3日

- 将每张图像的边界框数限制增加到 200。 
- 错误修复，包括导出到 TensorFlow 的模型的大量性能更新。 

## <a name="march-26-2019"></a>2019 年 3 月 26 日

- 自定义视觉服务已在 Azure 上输入了"通用"！
- 添加了高级培训功能，并添加了新的机器学习后端，以提高性能，尤其是在具有挑战性的数据集和细粒度分类方面。 通过高级培训，您可以指定用于培训的计算时间预算，自定义视觉将实验性地确定最佳培训和增强设置。 对于快速迭代，您可以继续使用现有的快速培训。
- 引入了 3.0 API。 2019 年 10 月 1 日宣布即将弃用 3.0 API。 有关如何入门的示例，请参阅[.Net、Python、](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/csharp-tutorial)[节点](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/node-tutorial)[、Java](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/java-tutorial)或[Python](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/python-tutorial)[Go](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/go-tutorial)的文档快速入门。
- 在 3.0 API 中将"默认迭代"替换为发布/取消发布。
- 添加了新的模型导出目标。 Dockerfile 导出已升级，以支持树莓派 3 的 ARM。 导出支持已添加到[愿景 AI 开发工具包中。](https://visionaidevkit.com/).
- S0 层每个项目的标记限制增加到 500。 S0 层每个项目的图像限制增加到 100，000。
- 已删除成人域。 建议改为常规域。
- 已宣布的通用可用性[定价](https://azure.microsoft.com/pricing/details/cognitive-services/custom-vision-service/)。  

## <a name="february-25-2019"></a>2019 年 2 月 25 日

- 宣布有限试用项目（未与 Azure 资源关联的项目）的结束，因为自定义视觉即将完成其迁移到 Azure 公共预览。 从 2019 年 3 月 25 日开始，CustomVision.ai网站将仅支持查看与 Azure 资源关联的项目，例如免费的自定义视觉资源。 到 2019 年 10 月 1 日，您仍可以通过自定义视觉 API 访问现有的有限试用项目。 这将让您有时间更新使用自定义 Vision 编写的任何应用的 API 密钥。 2019 年 10 月 1 日之后，您尚未移动到 Azure 的任何有限试用项目都将被删除。

## <a name="january-22-2019"></a>2019 年 1 月 22 日

- 为新的 Azure 区域添加了支持：美国西部 2、美国东部、美国东部 2、西欧、北欧、东南亚、澳大利亚东部、印度中部、英国南部、日本东部和美国中北部。 对美国中南部的持续支持。
...
- 向网站添加了“移至 Azure”功能，以便更轻松地升级有限试用版项目以链接到 Azure。 资源链接项目（F0 或 S0。您可以在产品的"设置"页面上找到此内容。  
- 添加了导出到 ONNX 1.2 的功能，以支持 Windows ML 的 Windows 2018 10 月更新版本。
错误修复，包括具有特殊字符的 ONNX 导出。
...
- 错误修复&后端改进。
- 启用多类分类，用于图像正好具有一个标签的项目。 在多类模式的预测中，概率将总和为 1（所有图像都分类到指定的标记中）。
...
- 输入付费预览并登上 Azure 门户。 现在，可将项目附加到 F0（免费）或 S0（标准）层的 Azure 资源。 引入了 S0 层项目，最多允许 100 个标记和 25,000 张图像。
...
- 导出到 Android （TensorFlow） 除了以前发布的导出到 iOS （CoreML.） 之外，这允许导出经过训练的紧凑模型在应用程序中脱机运行。
