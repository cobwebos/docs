...
可导出压缩域生成的模型在本地运行。 根据所选的域，模型性能会有所不同。 在下表中，我们报告了有关 Intel 桌面 CPU 和 NVidia GPU 1 的模型大小和推断时间 \[ \] 。 
...
选择 " _视觉 AI 开发工具包_ " 时， _通用_、 _特征点_和 _零售_ ，而不是 _食品_ compact 域可用于映像分类，而 _常规 (compact) _ 和 _常规 (compact) [S1]_ 可用于对象检测。

>[!NOTE]
>用于对象检测的__常规 (compact) __域需要特殊的后处理逻辑。 有关详细信息，请参阅导出的 zip 包中的示例脚本。 如果需要没有后处理逻辑的模型，请使用 __常规 (compact) [S1]__。
