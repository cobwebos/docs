...
使用喜欢的 IDE 或编辑器创建一个新 Python 项目，或在桌面上创建新的文件夹。 将此代码片段复制到项目/文件夹的 `dictionary-lookup.py` 文件中。
...
文本翻译全局终结点设置为 `endpoint`。 `path` 设置 `dictionary/lookup` 路由并确定我们需使用 API 的版本 3。
