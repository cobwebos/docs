...
* 仅统计：没有神经网络可用于此语言。
* 神经可用：有一个神经系统可用。 使用参数 `category=generalnn` 可访问神经系统。
* 神经默认值：神经是默认翻译系统。 使用参数 `category=smt` 可访问适用于 Microsoft Translator Hub 的统计系统。
* 仅限神经：仅提供神经翻译。

**V3 Translator API** V3 Translator API 在默认情况下为神经，统计系统仅在没有神经系统时才可用。

> [!NOTE]
> 目前，自定义翻译器中提供了神经语言的子集，并且我们正在逐步添加其他语言。 [查看当前在自定义翻译器中提供的语言](#customization)。
...
以下语言可用于自定义[翻译](https://aka.ms/CustomTranslator)器的英语或英语自定义。
