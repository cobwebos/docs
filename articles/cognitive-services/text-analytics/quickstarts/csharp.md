...
在应用程序的 `Program` 类中，为资源的 Azure 终结点和订阅密钥创建变量。 在静态构造函数中，从环境变量 `TEXT_ANALYTICS_SUBSCRIPTION_KEY` 和 `TEXT_ANALYTICS_ENDPOINT` 获取这些值。 如果在开始编辑应用程序后创建了这些环境变量，则需要关闭并重新打开用于访问这些变量的编辑器、IDE 或 shell。
...
在应用程序的 `Main` 方法中，创建用于访问文本分析终结点的凭据。  稍后将定义 `Main` 方法调用的方法。
