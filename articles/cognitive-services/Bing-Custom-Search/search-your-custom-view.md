...
若要创建搜索查询，请为终结点输入参数值。 请注意，门户中显示的参数可能会根据你选择的终结点而更改。 请参阅[自定义搜索 API 参考](https://docs.microsoft.com/rest/api/cognitiveservices-bingsearch/bing-custom-search-api-v7-reference#query-parameters)有关详细信息。 若要更改搜索实例使用的订阅、 添加相应的订阅密钥，并更新相应的市场和/或语言参数。
...
## <a name="change-your-bing-custom-search-subscription"></a>更改你的必应自定义搜索订阅

可以更改与必应自定义搜索实例而无需创建一个新实例关联的订阅。 若要让 API 调用发送到新的订阅收费，请在 Azure 门户中创建新的必应自定义搜索资源。 在对 API 请求，以及实例的自定义配置的 id。 使用新的订阅密钥
