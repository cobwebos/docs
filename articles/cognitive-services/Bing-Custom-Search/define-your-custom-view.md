...
| 切片名称 | 描述                                                                                                                                                                                                                                                                                                |
|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Domain     | 域切片包括在 Internet 域中找到的所有内容。 例如，`www.microsoft.com`。 省略 `www.` 会导致必应同时搜索该域的子域。 例如，如果您指定`microsoft.com`，必应也会返回的结果`support.microsoft.com`或`technet.microsoft.com`。 |
...
* 在中**固定**选项卡上，输入要固定到其对应的查询和前，网页的 URL。

* 在“预览”窗格中，输入搜索查询并单击“搜索”  。 找到你想要固定查询，并单击的网页**固定到顶端**。 该网页和查询将添加到“固定”列表  。
