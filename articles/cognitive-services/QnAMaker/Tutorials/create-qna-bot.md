...
description: 本教程介绍如何从现有知识库的“发布”页创建 QnA 聊天机器人。 此机器人使用 Bot Framework SDK v4。 无需编写任何代码即可生成该机器人，系统会自动提供所有代码。
...
1.  输入用于创建机器人的设置：
    ...
    |设置|值|目的|
    |--|--|--|
    |机器人句柄|`qna-maker-central-us-bot`|这是机器人的 Azure 资源名称。|
    |订阅|查看用途。|请选择创建 QnA Maker 资源时所用的同一个订阅。|
    |资源组|`docs`|与机器人相关的所有 Azure 资源使用的资源组。|
    |位置|`west us`|机器人的 Azure 资源位置。|
    |定价层|`F0`|Azure 机器人服务的免费层。|
    |应用程序名称|`qna-maker-central-us-bot-app`|这是一个 Web 应用，仅用于为机器人提供支持。 此应用名称不能与 QnA Maker 服务已使用的名称相同。 不支持与任何其他资源共享 QnA Maker 的 Web 应用。|
    ...
    > ![使用这些设置创建知识库机器人。](../media/qnamaker-tutorials-create-bot/create-bot-from-published-knowledge-base.png)
