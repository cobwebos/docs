...
导入知识库将替换现有知识库的内容。 导入需要包含`.tsv`问题和答案的结构化文件。 此信息有助于 QnA Maker 将问题答案集分组，并将其属性设置为特定的数据源。
...
可以采用`.tsv`文件格式创建多轮会话。 此格式提供了通过分析以前的聊天日志（使用其他进程，而不是使用 QnA Maker）来创建多轮会话的功能，然后通过自动化`.tsv`创建该文件。 导入文件以替换现有知识库。
...
* 或在 QnA Maker 门户中创建集，保存并导出该知识库，以获取有关如何表示该集的示例。
