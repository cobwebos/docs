...
你的用户可能会输入带有对话文本`How do I add a toner cartridge to my printer?`样式的问题或关键字搜索 (如)。 `toner cartridge` 知识库应同时具有这两种样式, 以便正确返回最佳答案。 如果不确定客户输入了哪些关键字, 请使用 Application Insights 数据来分析查询。
