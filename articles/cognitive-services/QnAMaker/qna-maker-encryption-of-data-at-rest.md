...
2. 创建 QnA Maker 资源时，它会自动与 Azure 搜索实例相关联。 这不能与 CMK 一起使用。 若要使用 CMK，需要将在步骤1中创建的新创建的 Azure 搜索实例关联起来。 具体而言，需要在 QnA Maker 资源中`AzureSearchAdminKey`更新`AzureSearchName`和。
