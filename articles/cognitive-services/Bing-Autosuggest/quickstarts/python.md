---
title: 将必应自动推荐 API 与 Python 配合使用快速入门 | Microsoft Docs
...
本文展示了如何将[必应自动推荐 API](https://azure.microsoft.com/services/cognitive-services/autosuggest/) 与 Python 配合使用。 必应自动推荐 API 根据用户在搜索框中输入的部分查询字符串返回建议查询的列表。 通常情况下，每当用户在搜索框中键入新字符时均会调用此 API，然后搜索框的下拉列表中会显示建议。 本文展示了如何发送请求，以针对 *sail* 返回建议的查询字符串。
