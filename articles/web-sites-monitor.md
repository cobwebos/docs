<properties linkid="manage-services-how-to-monitor-websites" urlDisplayName="如何监视" pageTitle="如何监视网站 - Azure 服务管理" metaKeywords="Azure monitoring  Websites, Azure Management Portal Monitor, Azure monitoring" description="了解如何使用管理门户中的"监视"页来监视 Azure 网站。" metaCanonical="" services="web-sites" documentationCenter="" title="How to Monitor  Websites" authors="timamm" solutions="" manager="paulettm" editor="mollybos" />
...
1. 在[Azure 管理门户](http://manage.windowsazure.cn/)中，在网站的管理页面单击**监视器**选项卡以显示**监视器**管理页面。默认情况下，**监视器**页面上的图表显示的度量值与**仪表板**页面上图表所显示的度量值相同。 

2. 若要查看网站的其他度量值，请单击页面底部的**添加度量值**，以显示**选择度量值**对话框。 

3. 单击以选择要在**监视器**页面上显示的其他度量值。 

4. 选择了想要添加到**监视器**页面的度量值后，单击底部的复选标记。 

5. 将度量值添加到**监视器**页面之后，单击以启用/禁用各度量值旁边的圆形选项框来给页面顶部的图表添加/移除度量值。

6. 若要从**监视器**页移除度量值，请选中要移除的度量值，然后单击页面底部的**删除度量值**图标。

## <a name="howtoreceivealerts"></a>如何：接收来自网站度量值的警报
在**标准**网站模式中，您可以收到基于网站监视度量值的警报。该警报功能要求你首先配置用于监视的 Web 终结点，你可以在**配置**页的**监视**部分中进行此配置。然后，在 Azure 管理门户的**设置**页上，可以创建规则，当所选度量值达到指定的值时，触发警报。你还可以选择在触发警报时发送电子邮件。有关更多信息，请参见[如何：在 Azure 中接收警报通知和管理警报规则](http://go.microsoft.com/fwlink/?LinkId=309356)。  

## <a name="howtoviewusage"></a>如何：查看网站的使用率配额

网站可以配置为通过网站模式**缩放**管理页运行任意一种**共享**或**标准**模式。每个 Azure 订阅均有权访问为在**共享**网站模式中每个区域运行最多 100 个网站所提供的一组资源。对于为此目的而向每个网站订阅提供的该组资源可由同一地理区域中配置为在**共享**模式中运行的其他网站共享。由于共享这些资源是为了供其他网站使用，因此所有订阅对这些资源的使用是受限的。订阅使用这些资源存在限制，该限制以各网站**仪表板**管理页面的使用率概述部分下列出的使用率配额形式表示。

>[AZURE.NOTE] 当网站配置为以**标准**模式运行时，会被分配专用资源，资源大小等同于[Azure 的虚拟机和云服务大小][vmsizes]的表中的**小型**（默认）、**中型**或**大型**虚拟机大小。对于能够以**标准**模式运行的网站的订阅，没有资源限制。但是，每个区域可创建的**标准**模式网站的数量是 500。
 
### 如何：查看配置为共享网站模式的网站的使用率配额 ###
若要确定网站对资源使用率配额的影响程度，请执行下列步骤：

1. 打开网站的**仪表板**管理页。
2. 在**使用情况概述**部分显示了您各自的 Web 托管计划的使用率配额，它是以下内容的一个子集：
	-	**输出的数据**、**CPU 时间**和**内存** - 当超过配额时，Azure 将会停止，以留出当前配额间隔的剩余部分。Azure 将在下一配额间隔开始时启动该网站。
	-	**文件系统存储** - 当达到配额时，文件系统存储仍然可以访问以执行读取操作，但所有其他写入操作（包括正常网站活动所需的写入操作）将会被阻止。文件使用量降低或将该网站移入配额更高的 Web 托管计划时，写入操作将恢复。
	-	**链接的资源** - 在此处还显示所有链接的网站资源的配额，例如数据库或存储。   
...
- 将网站移动到级别更高的 Web 托管计划以充分利用更高的配额。例如，**基本**和**标准**计划的唯一配额是文件系统存储。 
- 随着网站实例的数量增加，超过共享资源配额的可能性将随之增加。在适当情况下，请考虑在即将达到共享资源配额时回缩网站的其他实例。


## <a name="howtoconfigdiagnostics"></a>如何：为网站配置诊断和下载日志

在网站的**配置**管理页启用诊断。有两种诊断类型：**"应用程序诊断**和**网站诊断**。
...
> [AZURE.NOTE] **应用程序诊断（文件系统）**、**应用程序诊断（表存储）**和**应用程序诊断（blob 存储）**可同时启用，但各自具有单独的日志级别配置。例如，您可能想要将错误和警告记录到存储中作为长期的日志记录解决方案，同时启用在检测应用程序代码后使用详细级别进行文件系统日志记录，以便解决问题。

> [AZURE.NOTE] 也可以从 Azure PowerShell 中使用 **Set-AzureWebsite cmdlet** 来启用诊断。
> 
> 如果尚未安装 Azure PowerShell，或者尚未将其配置为使用 Azure 订阅，请参阅[如何使用 Azure PowerShell](/zh-cn/documentation/articles/install-configure-powershell/)。

> [AZURE.NOTE] 应用程序日志记录依赖于你的应用程序生成的日志信息。用于生成日志信息的方法以及信息格式是特定于编写您的应用程序所使用的语言的。有关使用应用程序日志记录的语言特定的信息，请参见以下文章：
> 
...
- **Web 服务器日志记录** - 启用"Web 服务器日志记录"可使用 W3C 扩展日志文件格式保存网站日志。Web 服务器日志记录为对你的网站的所有传入请求生成一个记录，记录中包含客户端 IP 地址、请求的 URI、响应的 HTTP 状态代码以及客户端的用户代理字符串之类的信息。可将日志保存到 Azure 存储帐户或文件系统中。
 ...
 默认情况下，Web 服务器日志永远不会被删除。若要指定在其之后将自动删除日志的时段，请选择**设置保留**并且在**保留期**框中输入保留日志的天数。"Azure 存储空间"和"文件系统"选项中都提供了这一设置。
...
> [AZURE.IMPORTANT]在网站上提出诸多要求的日志记录和跟踪。建议在重现所排查的问题后关闭日志记录和跟踪功能。
...
1. 打开网站的**仪表板**管理页，然后记下在**诊断日志**下列出的 FTP 站点以及在**部署用户**下列出的帐户。FTP 站点是日志文件所在的位置，而"部署用户"下方列出的帐户可用于对 FTP 站点进行身份验证。
...
1. 从**开始屏幕**或**开始菜单**，搜索**Azure PowerShell**。右键单击**Azure PowerShell**项，并选择**以管理员身份运行**。

	> [AZURE.NOTE] 如果未安装**Azure PowerShell**，请参阅[Azure PowerShell Cmdlet 入门](http://msdn.microsoft.com/library/windowsazure/jj554332.aspx)以了解安装和配置信息。
...
- **详细的错误信息：**包括有关错误的信息，如 <em>模块</em>、 <em>处理程序</em>、 <em>错误代码</em>和 <em>请求的 URL</em>。

- **最可能的原因：**列出了几个可能导致该错误的原因。

- **可以尝试下列操作：**列出了用于解决错误所报告的问题的可能的解决方案。

- **链接和更多信息**：提供有关错误的其他摘要信息，并且还包括指向其他资源（如 Microsoft 知识库文章）的链接。

**日志文件类型：Web 服务器日志记录**

- 位置:/LogFiles/http/RawLogs。使用[ W3C 扩展日志格式](http://go.microsoft.com/fwlink/?LinkID=90561)设置文件中存储的信息的格式。Azure 网站不使用 s-computername、s-ip 和 cs-version 字段。
...
1.	打开**网站**。单击要配置的网站的名称。
2.	单击**配置**选项卡。 
3.     转到**监视**部分以输入你的终结点设置。
4.	输入终结点的名称。
5.	输入您想要监视的网站一部分的 URL。例如，[http://contoso.chinacloudsites.cn/archive](http://contoso.chinacloudsites.cn/archive)。 
6.	从列表中选择一个或多个地理位置。
...
9.	在最左侧的服务栏中单击**管理服务**。
10.	单击底部的**添加规则**。
11.	在**服务类型**中，选择**网站**，然后选择您之前配置了终结点监视的网站。单击**下一步**。
12.	在**度量值**中，现在，您可以为配置的终结点选择其他度量值。例如：**响应时间（主页/美国：伊利诺斯州 - 芝加哥）**。选择响应时间度量值，然后**阈值**中键入 3，以指定 3 秒的阀值。
