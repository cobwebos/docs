...
|  location | 字符串 | 否 | 资源位置。 |
|  标记 | 对象 | 否 | 资源标记。 |
|  identity | 对象 | 否 | 容器组的标识（如果已配置）。 - [ContainerGroupIdentity 对象](#ContainerGroupIdentity) |
|  properties | 对象 | 是 | [ContainerGroupProperties 对象](#ContainerGroupProperties) |
...
|  名称 | 类型 | 必选 | 值 |
|  ---- | ---- | ---- | ---- |
|  type | 枚举 | 否 | 容器组所使用的标识的类型。 类型 "SystemAssigned，UserAssigned" 同时包含隐式创建的标识和一组用户分配的标识。 类型 "None" 将删除容器组中的任何标识。 -SystemAssigned、UserAssigned、SystemAssigned、UserAssigned、None |
|  userAssignedIdentities | 对象 | 否 | 与容器组关联的用户标识列表。 用户标识字典键引用将是以下形式的 Azure 资源管理器资源 Id： "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}"。 |
...
|  imageRegistryCredentials | 数组 | 否 | 从中创建容器组的图像注册表凭据。 - [ImageRegistryCredential 对象](#ImageRegistryCredential) |
|  restartPolicy | 枚举 | 否 | 重启容器组中所有容器的策略。 - `Always` 始终重启-在失败时重新启动 `OnFailure`-`Never` 从不重新启动。 -Always、OnFailure、Never |
|  地址 | 对象 | 否 | 容器组的 IP 地址类型。 - [IpAddress 对象](#IpAddress) |
|  osType | 枚举 | 是 | 容器组中的容器所需的操作系统类型。 -Windows 或 Linux |
|  volumes | 数组 | 否 | 此容器组中的容器可以装载的卷的列表。 - [卷对象](#Volume) |
|  诊断 | 对象 | 否 | 容器组的诊断信息。 - [ContainerGroupDiagnostics 对象](#ContainerGroupDiagnostics) |
|  NetworkProfile | 对象 | 否 | 容器组的网络配置文件信息。 - [ContainerGroupNetworkProfile 对象](#ContainerGroupNetworkProfile) |
|  dnsConfig | 对象 | 否 | 容器组的 DNS 配置信息。 - [DnsConfiguration 对象](#DnsConfiguration) |
...
|  properties | 对象 | 是 | 容器实例的属性。 - [ContainerProperties 对象](#ContainerProperties) |
...
|  ---- | ---- | ---- | ---- |
|  name | 字符串 | 是 | 卷的名称。 |
|  azureFile | 对象 | 否 | Azure 文件卷。 - [AzureFileVolume 对象](#AzureFileVolume) |
|  emptyDir | 对象 | 否 | 空目录卷。 |
|  secret | 对象 | 否 | 机密卷。 |
|  gitRepo | 对象 | 否 | Git 存储库卷。 - [GitRepoVolume 对象](#GitRepoVolume) |
...
|  名称 | 类型 | 必选 | 值 |
|  ---- | ---- | ---- | ---- |
|  logAnalytics | 对象 | 否 | 容器组日志分析信息。 - [LogAnalytics 对象](#LogAnalytics) |
...
|  searchDomains | 字符串 | 否 | 容器组中用于主机名查找的 DNS 搜索域。 |
|  options | 字符串 | 否 | 容器组的 DNS 选项。 |
...
|  名称 | 类型 | 必选 | 值 |
|  ---- | ---- | ---- | ---- |
|  图像 | 字符串 | 是 | 用于创建容器实例的图像的名称。 |
|  command | 数组 | 否 | 要在容器实例中以 exec 形式执行的命令。 -string |
|  ports | 数组 | 否 | 容器实例上的已公开端口。 - [因此 containerport 对象](#ContainerPort) |
|  environmentVariables | 数组 | 否 | 要在容器实例中设置的环境变量。 - [value 对象](#EnvironmentVariable) |
|  资源 | 对象 | 是 | 容器实例的资源要求。 - [ResourceRequirements 对象](#ResourceRequirements) |
|  volumeMounts | 数组 | 否 | 容器实例可用的卷装入。 - [VolumeMount 对象](#VolumeMount) |
|  livenessProbe | 对象 | 否 | 活动探测。 - [ContainerProbe 对象](#ContainerProbe) |
|  readinessProbe | 对象 | 否 | 准备情况探测。 - [ContainerProbe 对象](#ContainerProbe) |
...
|  名称 | 类型 | 必选 | 值 |
|  ---- | ---- | ---- | ---- |
|  目录 | 字符串 | 否 | 目标目录名称。 不得包含或以 ".." 开头。  如果提供了 "."，则卷目录将是 git 存储库。  否则，如果指定了此项，则该卷将在子目录中包含具有给定名称的 git 存储库。 |
|  repository | 字符串 | 是 | 存储库 URL |
|  revision | 字符串 | 否 | 指定修订的提交哈希。 |
...
|  metadata | 对象 | 否 | Log analytics 的元数据。 |
...
|  限制 | 对象 | 否 | 此容器实例的资源限制。 - [ResourceLimits 对象](#ResourceLimits) |
...
|  名称 | 类型 | 必选 | 值 |
|  ---- | ---- | ---- | ---- |
|  exec | 对象 | 否 | [ContainerExec 对象](#ContainerExec)的执行命令 |
|  httpGet | 对象 | 否 | [ContainerHttpGet 对象](#ContainerHttpGet)的 Http Get 设置 |
...
|  GPU | 对象 | 否 | 此容器实例的 GPU 请求。 - [GpuResource 对象](#GpuResource) |
...
|  GPU | 对象 | 否 | 此容器实例的 GPU 限制。 - [GpuResource 对象](#GpuResource) |
...
|  名称 | 类型 | 必选 | 值 |
|  ---- | ---- | ---- | ---- |
|  command | 数组 | 否 | 要在容器中执行的命令。 -string |
