...
重启策略的指定方式取决于容器实例的创建方式，例如，是使用 Azure CLI、Azure PowerShell cmdlet 还是 Azure 门户。 在 Azure CLI 中, 在调用`--restart-policy` [az container create][az-container-create]时指定参数。
...
若要查看操作中的 "重新启动" 策略, 请从 Microsoft [wordcount][aci-wordcount-image]映像创建一个容器实例, 并`OnFailure`指定重新启动策略。 此示例容器运行一个 Python 脚本，默认情况下，该脚本会分析莎士比亚著作[哈姆雷特](http://shakespeare.mit.edu/hamlet/full.html)中的文本，将 10 个最常见的单词写入 STDOUT，然后退出。

用以下[az container create][az-container-create]命令运行示例容器:
...
当示例容器的状态显示为 *Terminated* 后，可以通过查看容器日志来查看该容器的任务输出。 运行[az container logs][az-container-logs]命令查看脚本的输出:
...
基于任务的方案 (如批处理包含多个容器的大型数据集) 可以在运行时利用自定义[环境变量](container-instances-environment-variables.md)或[命令行](container-instances-start-command.md)。
