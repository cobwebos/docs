...
* [az 容器创建][az-container-create]命令：使用`--command-line`参数传递字符串。 示例：`--command-line "python myscript.py arg1 arg2"`）。

* [新 AzureRm 容器组][new-azurermcontainergroup]Azure PowerShell cmdlet：传递带有参数`-Command`的字符串。 示例：`-Command "echo hello"`。

* Azure 门户：在容器配置的命令**重写**属性中，提供逗号分隔的字符串列表，而不提供引号。 示例：`python, myscript.py, arg1, arg2`）。 
