...
运行以下 [az keyvault set-policy](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-set-policy) 命令在 Key Vault 上设置访问策略。 以下示例允许用户分配的标识从 Key Vault 中获取机密：
...
运行以下 [az keyvault set-policy](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-set-policy) 命令在 Key Vault 上设置访问策略。 以下示例允许系统托管标识从 Key Vault 中获取机密：
