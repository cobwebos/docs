---
title: 使用 Azure Migrate 中的服务器评估来评估要迁移到 Azure VM 的 Hyper-V VM
description: 了解如何使用服务器评估来评估要迁移到 Azure VM 的 Hyper-V VM。
ms.topic: tutorial
ms.date: 09/14/2020
ms.custom: MVC
...
ms.locfileid: "90089137"
---
# <a name="tutorial-assess-hyper-v-vms-for-migration-to-azure"></a>教程：评估要迁移到 Azure 的 Hyper-V VM

在迁移到 Azure 的过程中，将评估本地工作负载，以衡量云的就绪性、确定风险以及估算成本和复杂性。

本文介绍如何使用“Azure Migrate:服务器评估”工具来评估发现的 Hyper-V 虚拟机 (VM) 是否适合迁移到 Azure。服务器评估”工具评估本地 VMware VM。
...
- 在按照本教程来评估要迁移到 Azure VM 的计算机之前，请确保已发现要评估的计算机：
    - 若要使用 Azure Migrate 设备发现计算机，请[遵循本教程](tutorial-discover-hyper-v.md)。 
