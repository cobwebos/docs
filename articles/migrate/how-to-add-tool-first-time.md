...
Azure Migrate 提供了一个中心，用于跟踪对 Azure 的本地应用程序、工作负荷以及专用/公有云 Vm 的发现、评估和迁移。 该中心提供 Azure Migrate 工具，用于评估和迁移，以及其他工具和独立软件供应商 (ISV) [产品](migrate-services-overview.md#isv-integration) 。 
...
5. 在 " **选择评估工具**" 中，添加一个评估工具。 如果不需要评估工具，请选择 "**跳过添加评估工具**"  >  **Next**。 
2. 在 " **选择迁移工具**" 中，根据需要添加迁移工具。 如果你现在不需要迁移工具，请选择 "**立即跳过添加迁移工具**"  >  **Next**。
