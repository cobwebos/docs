...
- 只能在地理位置为“美国”的区域创建一个 Azure Migrate 项目。 这不会影响迁移计划的制定。可以迁移到任何 Azure 目标位置。 迁移项目的地域只用来存储在本地环境中发现的元数据。 [详细了解](https://docs.microsoft.com/azure/migrate/resources-faq#discovery-and-assessment) Azure Migrate 收集的元数据。 收集的元数据存储在所选的 [Azure 地域](https://azure.microsoft.com/global-infrastructure/geographies/)中的区域之一中：
