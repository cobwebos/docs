...
**从 Hyper-V 迁移到 Azure**：Azure Migrate 目前仅支持评估要迁移到 Azure 的 VMware 虚拟机。 Azure Migrate 对 Hyper-V 的支持已在规划中。 在此期间，可以使用 ASR 部署规划器。 在 Azure Migrate 实现 Hyper-V 支持后，可以使用 Azure Migrate 来规划 Hyper-V 工作负荷的迁移。

**将 VMware/Hyper-V 灾难恢复到 Azure**：如果想要使用 Azure Site Recovery (ASR) 在 Azure 中实现灾难恢复 (DR)，请使用 ASR 部署规划器进行 DR 规划。 ASR 部署规划器会针对本地环境执行深度的、特定于 ASR 的评估。 它会提供所需的建议，让 ASR 成功执行复制、虚拟机故障转移等 DR 操作。  
...
对于依赖项可视化，如果在 VM 上安装了代理，则依赖项代理收集的数据将存储在美国的某个 OMS 工作区中，该工作区是在用户的订阅中创建的。 一旦用户在其订阅中删除该 OMS 工作区，就会删除这些数据。 [了解详细信息](https://docs.microsoft.com/azure/migrate/concepts-dependency-visualization)。
