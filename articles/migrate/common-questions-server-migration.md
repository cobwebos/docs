...
无代理复制情况下的 Azure Migrate 设备将压缩数据，并在上传之前加密。 数据通过 https 上的安全通信通道传输并使用 TLS 1.2 或更高版本。 此外，当你将数据保存到云中（静态加密）时，Azure 存储会自动对其进行加密。  
...
目前，可以同时迁移每个 vCenter Server 的 300 Vm。 成批迁移10个虚拟机。
...
如果设置了多个设备，则所提供的 vCenter 帐户上的 Vm 之间不会有重叠。 此类重叠的发现是不受支持的方案。
