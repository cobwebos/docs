...
1. 通过检查 Azure Migrate 的设备的哈希值来验证是否已正确下载设备 .OVA 文件。 请参阅[本文](https://docs.microsoft.com/azure/migrate/tutorial-assessment-vmware)来验证哈希值。 如果哈希值不匹配，请再次下载 OVA 文件并重试部署。
...
### <a name="the-appliance-could-not-be-registered-successfully-to-the-azure-migrate-project-error-id-60052"></a>设备无法成功注册到 Azure Migrate 项目 (错误 ID:60052)

此错误是由于用于注册设备的 Azure 帐户权限不足引起的。 确保用于注册设备的 Azure 用户帐户在订阅上至少具有 "参与者" 访问权限。 [详细了解](https://docs.microsoft.com/azure/migrate/migrate-appliance#appliance-deployment-requirements)所需的 Azure 角色和权限。

### <a name="the-appliance-could-not-be-registered-successfully-to-the-azure-migrate-project-error-id-60039"></a>设备无法成功注册到 Azure Migrate 项目 (错误 ID:60039)

找不到你所选择的用于注册设备的 Azure Migrate 项目, 导致注册失败。 中转到 Azure 门户, 并检查资源组中是否存在该项目。 如果项目不存在, 请在资源组中创建新的 Azure Migrate 项目, 并再次注册设备。 [了解](https://docs.microsoft.com/azure/migrate/how-to-add-tool-first-time#create-a-project-and-add-a-tool)有关创建新 Azure Migrate 项目的详细信息。

### <a name="azure-key-vault-management-operation-failed-error-id-60030-60031"></a>Azure 密钥保管库管理操作失败 (错误 ID:60030、60031)

确保用于注册设备的 Azure 用户帐户在订阅上至少具有 "参与者" 访问权限。 同时, 检查该帐户是否有权访问错误消息中指定的 Key Vault, 然后重试该操作。 如果问题持续出现，请联系 Microsoft 支持。 [详细了解](https://docs.microsoft.com/azure/migrate/migrate-appliance#appliance-deployment-requirements)所需的 Azure 角色和权限。

### <a name="discovery-could-not-be-initiated-due-to-the-error-the-operation-failed-for-the-given-list-of-hosts-or-clusters-error-id-60028"></a>由于错误而无法启动发现。 给定主机或群集列表的操作失败 (错误 ID:60028)

由于访问或检索 VM 信息时出现问题, 无法在错误中列出的主机上启动发现;添加的其余主机已成功添加。 使用**add host**选项再次添加错误中的主机。 如果出现验证错误, 请查看更正指南以修复错误, 然后再次尝试**保存并启动发现**。

### <a name="azure-active-directory-aad-operation-failed-the-error-occurred-while-creating-or-updating-the-aad-application-error-id-60025"></a>Azure Active Directory (AAD) 操作失败。 创建或更新 AAD 应用程序时出错 (错误 ID:60025)

用于注册设备的 Azure 用户帐户无权访问错误消息中指定的 AAD 应用程序。 检查你是否为 AAD 应用程序的所有者。 [了解](https://docs.microsoft.com/azure/migrate/migrate-appliance#appliance-deployment-requirements)有关 AAD 应用程序权限的详细信息。


## <a name="discovery-issues"></a>发现问题

### <a name="i-started-discovery-but-i-dont-see-the-discovered-vms-on-azure-portal-server-assessment-and-server-migration-tiles-show-a-status-of-discovery-in-progress"></a>我已经开始发现, 但在 Azure 门户看不到发现的虚拟机。 服务器评估和服务器迁移磁贴显示状态 "正在发现"
...
问题 | 更正
--- | ---
启动类型不受支持 | Azure 不支持引导类型为 EFI 的 VM。 在运行迁移之前, 建议将启动类型转换为 BIOS。 <br/><br/>你可以使用 Azure Migrate 服务器迁移来执行此类 Vm 的迁移, 因为它会在迁移过程中将 VM 的启动类型转换为 BIOS。
...
操作系统位数不受支持 | 采用 32 位 OS 的 VM 可以在 Azure 中引导，但建议在迁移到 Azure 之前将 VM 的 OS 从 32 位升级到 64 位。
...
磁盘大小超出限制 | Azure 支持大小最高为 4 TB 的磁盘。 在迁移之前将磁盘压缩至小于 4 TB。
...
“Azure Migrate:服务器评估目前不支持基于企业协议 (EA) 的定价。 解决方法是使用“即用即付”作为 Azure 服务并使用“折扣”属性来指定你所享受的任何自定义折扣。 [了解有关如何自定义评估的详细信息](https://aka.ms/migrate/selfhelp/eapricing)。
...
服务器评估中存在一个已知缺陷，即无法检测本地 VM 上安装的 Linux OS 的次要版本（例如，对于 RHEL 6.10，当前服务器评估仅将 RHEL 6 作为 OS 版本检测）。 由于 Azure 仅支持特定版本的 Linux，因此 Linux VM 目前在服务器评估中标记为“有条件就绪”。 可以通过查看[Azure Linux 支持文档](https://aka.ms/migrate/selfhost/azureendorseddistros), 手动确保本地 VM 上运行的 Linux 操作系统是否已在 azure 中认可。 验证了支持的发行版后，可以忽略此警告。
...
服务器评估中的磁盘大小调整取决于两个评估属性：大小调整标准和存储类型。 如果大小调整条件为 "基于性能" 且存储类型设置为 "自动", 则会将磁盘的 IOPS 和吞吐量值视为标识目标磁盘类型 (标准 HDD、标准 SSD 或高级磁盘)。 然后，考虑到本地磁盘的大小要求，建议使用磁盘类型中的磁盘 SKU。 如果大小调整条件是 "基于性能" 且存储类型为 "高级", 则建议使用 Azure 中的高级磁盘 SKU, 根据本地磁盘的 IOPS、吞吐量和大小需求进行推荐。 大小调整条件为“作为本地”大小大小调整且存储类型为“标准 HDD”、“标准 SSD”或“高级”时，使用相同的逻辑来执行磁盘大小调整。

例如, 如果你有一个包含 32 GB 内存的本地磁盘, 但该磁盘的聚合读取和写入 IOPS 为 800 IOPS, 则服务器评估将建议使用高级磁盘类型 (由于 IOPS 要求较高), 然后建议使用磁盘 SKU, 这可支持所需 IOPS 和大小。 本示例中最接近的匹配项将是 P15（256 GB，1100 IOPS）。 因此，即使本地磁盘所需的大小为 32 GB，服务器评估也建议使用更大的磁盘，因为本地磁盘的 IOPS 要求很高。

### <a name="why-does-my-assessment-report-say-percentageofcoresutilizedmissing-or-percentageofmemoryutilizedmissing-for-some-vms"></a>为什么某些 Vm 的评估报表显示 "PercentageOfCoresUtilizedMissing" 或 "PercentageOfMemoryUtilizedMissing"？
当 Azure Migrate 设备无法收集本地 Vm 的性能数据时, 将列出上述问题。 如果 Vm 在你创建评估 (最后一天/一周/一个月) 的持续时间内处于关闭状态, 则可能会发生这种情况, 因为设备在关机时无法收集 VM 的性能数据。 如果仅缺少内存计数器, 但要尝试评估 Hyper-v Vm, 请检查是否已在这些 Vm 上启用动态内存。 目前存在一个已知问题, Azure Migrate 设备无法为未启用动态内存的 Vm 收集内存利用率。 请注意, 此问题仅适用于 Hyper-v Vm, 不适用于 VMware Vm。 如果缺少任何性能计数器, 请 Azure Migrate:服务器评估将回退到分配的内核/内存, 并相应地建议 VM 大小。
...
服务器评估目前仅考虑 Windows 计算机的 OS 许可证成本，目前不考虑 Linux 计算机的 OS 许可证成本。
...
这些属性仅适用于“基于性能”的大小调整。 服务器评估不断收集本地计算机的性能数据，并使用它来针对 Azure 中的 VM SKU 和磁盘 SKU 提出建议。 下面是服务器评估收集性能数据的方法：
- Azure Migrate 设备连续对本地环境进行配置, 以便每隔20秒为 VMware Vm 收集实时利用率数据, 每隔30秒对 Hyper-v Vm 收集一次。
- 设备对时长 20/30 秒的样本进行汇总，以每隔 10 分钟创建单个数据点。 为了创建单个数据点，设备从所有时长 20/30 秒的样本中选择峰值并将其发送到 Azure。
- 在服务器评估中创建评估时，根据性能持续时间和性能历史记录百分位值，确定代表性利用率值。 例如, 如果性能历史记录为一周, 百分比利用率为 95, Azure Migrate 将按升序对最后一周的所有10分钟样本点进行排序, 然后选择 95% 作为代表值。
第 95 个百分位数值可以确保忽略任何离群值，如果选取第 99 个百分位数，则可以包括这些离群值。 如果希望选择该期间的峰值使用率并且不希望错过任何离群值，则应选择第 99 个百分位数作为百分位数利用率。
