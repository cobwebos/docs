...
ms.locfileid: "80336797"
---
# <a name="set-up-an-appliance-for-vmware-vms"></a>为 VMware VM 设置设备

本文介绍如何使用[Azure 迁移：服务器评估](migrate-services-overview.md#azure-migrate-server-assessment-tool)工具设置 Azure 迁移设备以进行评估，以及如何使用[Azure 迁移：服务器迁移](migrate-services-overview.md#azure-migrate-server-migration-tool)工具进行无代理迁移。
...
您可以使用下载的 OVA 模板或使用 PowerShell 安装脚本为 VMware VM 评估设置 Azure 迁移设备。 本文介绍如何使用 OVA 模板设置设备。 如果要使用脚本设置设备，请按照[本文](deploy-appliance-script.md)中的说明操作。
...
2. 提供设备的语言、时区和密码。
3. 在可以连接到 VM 的任何计算机上打开浏览器，并打开设备 Web 应用的**URL：https://*设备名称或 IP 地址*：44368**。
...
2. 在“用户名”和“密码”中，指定设备用来发现 vCenter Server 上的 VM 的只读帐户凭据。******** 可以通过限制对 vCenter 帐户的访问，来限定发现范围。 [了解详情](set-discovery-scope.md)。
