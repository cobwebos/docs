...
设备需要连接到 vCenter Server，才能发现 Vm 的配置和性能数据。
...
2. 在“用户名”和“密码”中，指定设备用来发现 vCenter Server 上的 VM 的只读帐户凭据。 请确保该帐户具有[所需的发现权限](migrate-support-matrix-vmware.md#assessment-vcenter-server-permissions)。可以通过相应地限制对 vCenter 帐户的访问来确定发现的范围。[在此处](tutorial-assess-vmware.md#scoping-discovery)了解范围发现的详细信息。
3. 单击“验证连接”，确保设备可以连接到 vCenter Server。

### <a name="specify-vm-credentials"></a>指定 VM 凭据
对于应用程序、角色和功能的发现以及可视化 Vm 的依赖项，你可以提供可访问 VMware Vm 的 VM 凭据。 你可以为 Windows Vm 添加一个凭据，为 Linux Vm 添加一个凭据。 [详细了解](https://docs.microsoft.com/azure/migrate/migrate-support-matrix-vmware#assessment-vcenter-server-permissions)所需的访问权限。

> [!NOTE]
> 此输入是可选的，需要它才能启用应用程序发现和无代理依赖项可视化。

1. 在 "**发现 vm 上的应用程序和依赖项**" 中，单击 "**添加凭据**"。
2. 选择**操作系统**。
3. 提供凭据的友好名称。
4. 在 "**用户名**" 和 "**密码**" 中，指定至少具有 vm 上的来宾访问权限的帐户。
5. 单击“添加”。

指定 vCenter Server 和 VM 凭据（可选）后，单击 "**保存并启动发现**" 以启动本地环境的发现。

大约 15 分钟后，已发现的 VM 的元数据将显示在门户中。 发现已安装的应用程序、角色和功能需要一些时间，持续时间取决于要发现的 Vm 的数量。 对于 500 Vm，应用程序清单在 Azure Migrate 门户中显示大约1小时。
