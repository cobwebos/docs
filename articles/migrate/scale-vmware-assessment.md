...
- **规划用于发现的帐户**： Azure Migrate 设备使用有权访问 vCenter Server 的帐户，以便发现用于评估和迁移的 vm。 如果发现超过10000个 Vm，请设置多个帐户。
...
- 如果你的环境在租户之间共享，并且你想要单独发现每个租户，则可以将对该设备用于发现的 vCenter 帐户的访问范围进行限定。 
    - 如果租户共享主机，你可能想要按 VM 文件夹来确定范围。 如果 vCenter 帐户有权访问 vCenter VM 文件夹级别，则 Azure Migrate 无法发现 Vm。 若要按 VM 文件夹限定发现范围，确保在 VM 级别为 vCenter 帐户分配只读访问权限即可实现此目的。 在[此处](tutorial-assess-vmware.md#set-the-scope-of-discovery)详细了解如何限定发现范围。
