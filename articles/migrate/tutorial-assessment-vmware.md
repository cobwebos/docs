...
4. 创建新的资源组。
5. 指定创建项目的区域，单击“创建”。 从本地 VM 收集的元数据将存储在此区域中。 对于此预览版，只能在“美国中西部”区域创建 Azure Migrate 项目。 但是，仍可以计划任意目标 Azure 位置的迁移。 
...
> [!NOTE]
> Azure Migrate 提供的成本估算适用于将本地 VM 作为 Azure 服务架构 (IaaS) VM 运行。 它不考虑任意平台即服务 (PaaS) 或服务型软件 (SaaS) 成本。 
