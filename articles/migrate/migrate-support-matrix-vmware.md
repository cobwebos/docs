...
**地域** | [查看](migrate-support-matrix.md#supported-geographies)支持的地理位置。
...
Azure Migrate：服务器评估可以发现应用、角色和功能。 发现您的应用程序清单后，您可以确定和规划为本地工作负荷定制的迁移路径。 Azure Migrate：服务器评估通过使用 WMI 和 SSH 调用，提供无代理发现，使用计算机来宾凭据远程访问计算机。

**支持** | **详细信息**
--- | ---
支持的计算机 | 本地 VMware VM
计算机操作系统 | 所有 Windows 和 Linux 版本
凭据 | 目前支持所有 Windows 服务器使用一个凭据，并支持为所有 Linux 服务器使用一个凭据。 为 Windows Vm 创建来宾用户帐户，为所有 Linux Vm 创建常规/普通用户帐户（非 sudo 访问权限）。
应用发现的计算机限制 | 每个设备10000。 每个项目35000
