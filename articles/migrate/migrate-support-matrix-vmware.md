...
ms.locfileid: "83849359"
---
# <a name="support-matrix-for-vmware-assessment"></a>VMware 评估支持矩阵 

本文总结了在使用 [Azure Migrate：服务器评估](migrate-services-overview.md#azure-migrate-server-assessment-tool)工具评估要迁移到 Azure 的 VMware VM 时的先决条件和支持要求。 若要将 VMware VM 迁移到 Azure，请查阅[迁移支持矩阵](migrate-support-matrix-vmware-migration.md)。

若要评估 VMware VM，请创建 Azure Migrate 项目，然后将服务器评估工具添加到项目中。 添加评估工具后，部署 [Azure Migrate 设备](migrate-appliance.md)。 此设备持续发现本地虚拟机，并向 Azure 发送虚拟机元数据和性能数据。 完成发现后，你将发现的虚拟机收集到组中，然后对组运行评估。
...
除了发现虚拟机外，服务器评估还可以发现虚拟机上运行的应用、角色和功能。 通过发现应用清单，可以确定和计划为本地工作负荷定制的迁移路径。 
...
**vCenter 凭据** | 应用发现需要 vCenter Server 帐户，帐户具有只读访问权限，并需要为“虚拟机”>“来宾操作”启用的特权。
**VM 凭据** | 应用发现目前支持对所有 Windows 服务器使用一个凭据，并对所有 Linux 服务器使用一个凭据。<br/><br/> 你为 Windows VM 创建来宾用户帐户，并为所有 Linux VM 创建常规/普通用户帐户（非 sudo 访问权限）。
**VMware 工具** | 必须在要发现的 VM 上安装并运行 VMware 工具。 <br/> VMware 工具版本必须高于 10.2.0。
...
**vCenter Server** | 要发现和评估的虚拟机必须由 vCenter Server 版本 5.5、6.0、6.5 或 6.7 托管。
**权限（评估）** | vCenter Server 只读帐户。
**权限（应用发现）** | vCenter Server 帐户，帐户具有只读访问权限，并需要为“虚拟机”>“来宾操作”启用的特权。
**权限（依赖关系可视化）** | vCenter Server 帐户，帐户具有只读访问权限，并需要为“虚拟机” > “来宾操作”启用的特权。
...
ESXi 主机（应用发现/无代理依赖关系分析） | 若要执行[应用发现](how-to-discover-applications.md)或[无代理依赖关系分析](concepts-dependency-visualization.md#agentless-analysis)，那么设备会在 TCP 端口 443 上连接到 ESXi 主机，以发现应用，并在 VM 上运行无代理依赖关系可视化。
...
## <a name="agentless-dependency-analysis-requirements"></a>无代理依赖关系分析要求
