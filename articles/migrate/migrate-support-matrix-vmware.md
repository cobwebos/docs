...
本文汇总了使用 Azure Migrate：服务器评估] （迁移-服务-概述--------------------------------------------- 如果要将 VMware Vm 迁移到 Azure，请参阅[迁移支持矩阵](migrate-support-matrix-vmware-migration.md)。
...
**权限（依赖项可视化）** | 中心服务器帐户具有只读访问权限，并且为**虚拟机** > **来宾操作**启用了特权。
...
**Windows 帐户** |  对于依赖项分析，Azure Migrate 设备需要本地或域管理员帐户才能访问 Windows Vm。
