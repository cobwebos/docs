...
ms.locfileid: "80336876"
---
# <a name="support-matrix-for-vmware-assessment"></a>VMware 评估的支持矩阵 

本文总结了为迁移到 Azure 做准备评估 VMware VM 的先决条件和支持要求。 如果要将 VMware VM 迁移到 Azure，请查看[迁移支持矩阵](migrate-support-matrix-vmware-migration.md)。
...
**支持** | **详细信息**
--- | ---
**项目限制** | 可以在 Azure 订阅中创建多个项目。<br/><br/> 您可以在单个[项目中](migrate-support-matrix.md#azure-migrate-projects)发现和评估多达 35，000 个 VMware VM。 项目可以包括 VMware VM、物理服务器和超 VM，每个虚拟机都达到评估限制。
...
除了发现计算机之外，Azure 迁移：服务器评估还可以发现在计算机上运行的应用、角色和功能。 通过发现应用清单，您可以识别和规划针对本地工作负荷量身定制的迁移路径。 

**支持** | **详细信息**
--- | ---
**支持的计算机** | 当前仅支持本地 VMware VM 的应用发现。
...
Azure 迁移使用[Azure 迁移设备](migrate-appliance.md)进行发现和评估。 VMware 的设备使用导入到 vCenter 服务器的 OVA 模板部署。 
