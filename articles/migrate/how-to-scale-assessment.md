...
如果你有多台 vCenter Server，每台 vCenter Server 中的虚拟机数少于 1500，但所有 vCenter Server 中的 VM 总数多于 1500，则需要创建多个迁移项目（一个迁移项目只能容纳 1500 台 VM）。 可以通过为每台 vCenter Server 创建迁移项目并对发现进行拆分来实现此目的。 可以使用单个收集器来发现每台 vCenter Server（逐台）。 如果希望同时启动发现，则还可以部署多台设备并且并行运行发现。
