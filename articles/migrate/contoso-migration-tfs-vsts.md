...
![迁移流程](./media/contoso-migration-tfs-vsts/migration-process.png) 
...
5. **试运行**：试运行导入可以帮助测试集合迁移。 试运行具有有限的生命，在生产迁移运行之前会被删除。 它们在设定的时间后会自动删除。 导入完成后收到的成功电子邮件中包括了有关何时将删除试运行的说明。 请注意并相应地制定计划。
...
1. [下载并安装](https://azure.microsoft.com/features/storage-explorer/) Azure 存储资源管理器。
    ...
    ![上传](./media/contoso-migration-tfs-vsts/backup5.png)

4. 连接到其订阅并找到他们为迁移创建的存储帐户 (**contosodevmigration**)。 创建一个新的 blob 容器 **vstsmigration**。

    ![上传](./media/contoso-migration-tfs-vsts/backup6.png)

5. 将要上传的 DACPAC 文件指定为块 blob。

    ![上传](./media/contoso-migration-tfs-vsts/backup7.png)
    
7. 上传文件后，单击文件名 >“生成 SAS”。 在存储帐户下展开 blob 容器，选择包含导入文件的容器，然后单击“获取共享访问签名”。

    ![上传](./media/contoso-migration-tfs-vsts/backup8.png)

8. 接受默认设置，然后单击“创建”。 这会将访问权限启用 24 小时。

    ![上传](./media/contoso-migration-tfs-vsts/backup9.png)

9. 复制共享访问签名 URL，以便供 TFS 迁移工具使用。

    ![上传](./media/contoso-migration-tfs-vsts/backup10.png)
