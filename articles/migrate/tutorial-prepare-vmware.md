...
- 设置评估帐户  。 Azure Migrate 需要访问 vCenter Server 来发现用于评估的 VM。 需要一个只读帐户才能进行 Azure Migrate 访问。
...
Azure Migrate 需访问 vCenter Server 来发现用于评估和无代理迁移的 VM。 若只进行评估，请设置一个适用于 vCenter Server 的只读帐户。
