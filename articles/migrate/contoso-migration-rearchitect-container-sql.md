...
本文演示 Contoso 如何在 Azure 中迁移并重新架构其 SmartHotel 应用。 Contoso 将应用前端 VM 迁移到 Azure Windows 容器，将应用数据库迁移到 Azure SQL 数据库。
...
**DevOps** | Contoso 希望使用代码生成和发布管道的 Visual Studio Team Services (VSTS) 迁移到 DevOps 模型。
...
- 对于应用 Web 层，Contoso 已决定使用 Visual Studio 将其转换为 Windows 容器。
...
- 为了实施 DevOps 管道，Contoso 将配合 Git 存储库使用 VSTS 进行源代码管理 (SCM)。  将使用自动生成和发行来生成代码，并将其部署到 Azure 容器注册表和 Azure Service Fabric。
...
**注意事项** | **详细信息**
--- | ---
**优点** | 要想迁移到 Azure Service Fabric，需要对 SmartHotel 应用代码进行更改。 但是，工作量很小，可以使用 Service Fabric SDK 工具进行更改。<br/><br/> 迁移到 Service Fabric 后，Contoso 今后能够快速开始开发可添加到应用程序中的微服务，且不会给原始代码库带来风险。<br/><br/> Windows 容器提供了与一般容器相同的优点。 它们提高了敏捷性、可移植性和控制能力。<br/><br/> Contoso 可以利用适用于 SQL Server 和 Windows Server 的 Azure 混合权益利用其在软件保障方面的投资。<br/><br/> 迁移后，不再需要支持 Windows Server 2008 R2。 [了解详细信息](https://support.microsoft.com/lifecycle)。<br/><br/> Contoso 可为应用的 Web 层配置多个实例，使其不再是单一故障点。<br/><br/> 他们不再依赖于老化的 SQL Server 2008 R2。<br/><br/> SQL 数据库支持 Contoso 的技术要求。 Contoso 管理员使用数据库迁移助手对本地数据库进行了评估，发现它是兼容的。<br/><br/> SQL 数据库具有内置的容错功能，Contoso 不需要设置此功能。 这将确保数据层不再是单一故障转移点。
...
2. 他们预配了一个 Azure SQL 实例，并将 SmartHotel 数据库迁移到它。
...
> * **步骤 4：管理 Service Fabric 证书**：Contoso 设置证书，使 VSTS 能够访问群集。
> * **步骤 5：使用 DMA 迁移数据库**：使用数据库迁移助手迁移应用数据库。
> * **步骤 6：设置 VSTS**：Contoso 在 VSTS 中设置新项目，并将代码导入 Git 存储库。
> * **步骤 7：转换应用**：Contoso 使用 Visual Studio 和 SDK 工具将应用转换为容器。
...
SmartHotel 容器将在 Azure Service Fabric 群集中运行。 Contoso 管理员按如下所述创建 Service Fabric 群集：
...
Contoso 需要提供群集证书，使 VSTS 能够访问群集。 Contoso 管理员对此进行设置。
...
3. 将指纹复制到某个文本文件供稍后参考。
4. 现在，添加一个在群集上用作管理客户端证书的客户端证书。 这样，VSTS 便可连接到群集，以便在发布管道中部署应用。 为此，可在门户中打开 KeyVault，并选择“证书” > “生成/导入”。
...
8. 对于 VSTS 部署，需要确定证书的 Base64 值。 可以使用 PowerShell 在本地开发人员工作站上执行此检查。 将输出粘贴到某个文本文件供稍后使用。
...
现在，Contoso 管理员可以使用 DMA 迁移 SmartHotel 数据库。
...
## <a name="step-6-set-up-vsts"></a>步骤 6：设置 VSTS

Contoso 需要为应用程序构建 DevOps 基础结构和管道。  为此，Contoso 管理员需创建新的 VSTS 项目、导入其代码，然后生成并发布管道。

1.   在 Contoso VSTS 帐户中创建新项目 (**ContosoSmartHotelRearchitect**)，然后选择“Git”作为版本控制机制。
...
3. 导入代码后，将 Visual Studio 连接到存储库，并使用团队资源管理器克隆代码。
    ...
    ![连接到存储库](./media/contoso-migration-rearchitect-container-sql/vsts3.png)
...
## <a name="step-8-build-and-release-pipelines-in-vsts"></a>步骤 8：VSTS 中的生成和发布管道

Contoso 管理员现在配置 VSTS 来执行生成和发布过程，以实施 DevOps 工作。

1. 在 VSTS 中，单击“生成和发布” > “新建管道”。

    ![新建管道](./media/contoso-migration-rearchitect-container-sql/pipeline1.png)

2. 选择“VSTS Git”和相关的存储库。
...
4. 更改标记映像以生成映像，并将任务配置为使用预配的 ACR。
...
7. 单击“保存并排队”以开始生成。
8. 生成成功后，转到发布管道。 在 VSTS 中，单击“发布” > “新建管道”。

    ![发布管道](./media/contoso-migration-rearchitect-container-sql/pipeline6.png)    

9. 选择“Azure Service Fabric 部署”模板，并命名环境 (**SmartHotelSF**)。

    ![环境](./media/contoso-migration-rearchitect-container-sql/pipeline7.png)

10. 提供管道名称 (**ContosoSmartHotelRearchitect**)。 对于环境，单击“1 个阶段，1 个任务”以配置 Service Fabric 部署。
...
12. 在“添加 Service Fabric 服务连接”中，配置连接和身份验证设置，供 VSTS 在部署应用时使用。 可在 Azure 门户中找到群集终结点。Contoso 添加了 **tcp://** 作为前缀。
...
18. 部署完成后，SmartHotel 现在将运行 Service Fabric。

    ![发布](./media/contoso-migration-rearchitect-container-sql/publish4.png)

19. 为连接到应用，Contoso 将流量定向到其 Service Fabric 节点前面的 Azure 负载均衡器的公共 IP 地址。
...
1. 将代码提交并推送到 VSTS。 这会启动生成和发布管道。

2. 完成生成和部署后，SmartHotel 将运行 Service Fabric。 Servie Fabric 管理控制台现在显示了三个服务。
...
在本文中，Contoso 通过将应用前端 VM 迁移到 Service Fabric 在 Azure 中重构了 SmartHotel 应用。 应用数据库已迁移到 Azure SQL 数据库。
