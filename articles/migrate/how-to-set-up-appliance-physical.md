...
2. 运行以下命令以生成压缩文件的哈希：
    ...
    - 政府云的示例使用情况：```  C:\>CertUtil -HashFile C:\Users\administrator\Desktop\AzureMigrateInstaller-Server-USGov.zip MD5 ```
3.  验证哈希值：
...
1. 将压缩文件解压缩到托管设备的服务器上的某个文件夹中。  确保没有在现有 Azure 迁移设备上的计算机上运行脚本。
2. 使用管理（提升）权限在上述服务器上启动 PowerShell。
3. 将 PowerShell 目录更改为从下载的压缩文件中提取内容的文件夹。
4. 通过运行以下命令，运行名为“AzureMigrateInstaller.ps1”的脚本****：

    - 对于公共云：``` PS C:\Users\administrator\Desktop\AzureMigrateInstaller> AzureMigrateInstaller.ps1 ```
    - 对于 Azure 政府：``` PS C:\Users\Administrators\Desktop\AzureMigrateInstaller-Server-USGov>AzureMigrateInstaller.ps1 ```

    脚本将在成功完成时启动设备 Web 应用程序。

如果遇到任何问题，可以在 C：_程序数据\微软 Azure_logs_AzureMigrateScenarioInstaller_<em>时间戳</em>.log 访问脚本日志以进行故障排除。
...
2. 指定**操作系统**、凭据的友好名称以及用户名和密码。 然后单击 **“添加”**。
