...
ms.locfileid: "86106150"
---
# <a name="access-and-identity-options-for-azure-kubernetes-service-aks"></a>Azure Kubernetes 服务 (AKS) 的访问和标识选项

可以通过不同的方式进行身份验证，控制访问权限/授权并保护 Kubernetes 群集。 使用 Kubernetes 基于角色的访问控制（RBAC），可以仅向用户、组和服务帐户授予对所需资源的访问权限。 使用 Azure Kubernetes 服务（AKS），可以使用 Azure Active Directory 和 Azure RBAC 进一步增强安全和权限结构。 这些方法可帮助你保护群集访问，并仅向开发人员和操作员提供最低要求的权限。
...
> [!NOTE]
> Microsoft/AKS 采取的任何群集操作都是以内置 Kubernetes 角色 `aks-service` 和内置角色绑定来进行的 `aks-service-rolebinding` 。 此角色允许 AKS 对群集问题进行故障排除，但是不能修改权限，也不能创建角色或角色绑定，或其他高权限操作。 仅在具有实时（JIT）访问权限的活动支持票证下启用角色访问。 阅读有关[AKS 支持策略](support-policies.md)的详细信息。
...
使用 OpenID Connect 向 AKS 群集提供 Azure AD 身份验证。 OpenID Connect 是构建在 OAuth 2.0 协议顶层的标识层。 有关 OpenID Connect 的详细信息，请参阅 [Open ID connect 文档] [开放式 id-connect]。 在 Kubernetes 群集内部， [Webhook 令牌身份验证][webhook-token-docs]用于验证身份验证令牌。 Webhook 令牌身份验证作为 AKS 群集的一部分进行配置和管理。
...
1. Kubectl 使用 Azure AD 客户端应用程序，通过[OAuth 2.0 device authorization grant flow](https://docs.microsoft.com/azure/active-directory/develop/v2-oauth2-device-code)登录用户。
...
1. [访问 Azure 订阅上的 AKS 资源](#azure-rbac-to-authorize-access-to-the-aks-resource)。 此过程允许你使用 AKS Api 控制扩展或升级群集，并请求 kubeconfig。
2. 访问 Kubernetes API。 此访问权限由[KUBERNETES RBAC](#kubernetes-role-based-access-controls-rbac) （传统）或通过将[AZURE RBAC 与 AKS 集成以实现 Kubernetes 授权](#azure-rbac-for-kubernetes-authorization-preview)控制

### <a name="azure-rbac-to-authorize-access-to-the-aks-resource"></a>授权访问 AKS 资源的 Azure RBAC

使用 Azure RBAC，可为用户（或标识）提供对一个或多个订阅中的 AKS 资源的精细访问权限。 例如，你可以使用[Azure Kubernetes 服务参与者角色](../role-based-access-control/built-in-roles.md#azure-kubernetes-service-contributor-role)，以允许你执行扩展和升级群集等操作。 其他用户可以拥有仅授予管理 kubeconfig 请求权限的[Azure Kubernetes Service 群集管理员角色](../role-based-access-control/built-in-roles.md#azure-kubernetes-service-cluster-admin-role)。
...
| Azure Kubernetes 服务 RBAC 管理员  | 允许在命名空间内授予管理员访问权限。 允许对命名空间（或群集范围）中的大多数资源进行读/写访问，包括在命名空间中创建角色和角色绑定的功能。 此角色不允许对资源配额或命名空间本身具有写入权限。 |
