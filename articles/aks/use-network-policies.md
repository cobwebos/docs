...
在 Kubernetes 中运行最新的基于微服务的应用程序时，通常想要控制哪些组件可以相互通信。 最小特权原则应该应用于流量在 Azure Kubernetes 服务 (AKS) 群集中的各 Pod 之间的流动方式。 假设你可能想要阻止直接流向后端应用程序的流量。 借助 Kubernetes 中的网络策略功能，可以为群集中各 Pod 之间的入口和出口流量定义规则。

本文介绍如何安装网络策略引擎并创建 Kubernetes 网络策略，来控制 AKS 中各 Pod 之间的流量流。 网络策略应仅用于 AKS 中基于 Linux 的节点和 Pod。
...
> [!TIP]
> 如果在预览期间使用了网络策略功能，建议[创建一个新的群集](#create-an-aks-cluster-and-enable-network-policy)。
> 
> 如果希望继续使用在预览期间使用了网络策略的现有测试群集，请将群集升级到新的 Kubernetes 版本，以获取最新的正式发布版本，然后部署以下 YAML 清单以修复崩溃的指标服务器和 Kubernetes 仪表板。 只有使用了 Calico 网络策略引擎的群集才需进行此修复。
>
> 作为安全方面的最佳做法，请[查看此 YAML 清单的内容][calico-aks-cleanup]，以了解部署到 AKS 群集中的项。
>
> `kubectl delete -f https://raw.githubusercontent.com/Azure/aks-engine/master/docs/topics/calico-3.3.1-cleanup-after-upgrade.yaml`

## <a name="overview-of-network-policy"></a>网络策略概述

默认情况下，AKS 群集中的所有 Pod 都可以无限制地发送和接收流量。 为了提高安全性，可定义用来控制流量流的规则。 例如，后端应用程序通常只公开给所需的前端服务。 或者，数据库组件只能由与其连接的应用程序层访问。

网络策略是一种 Kubernetes 规范，用于为 Pod 之间的通信定义访问策略。 通过使用网络策略，可以定义一组有序的规则来发送和接收流量，并将其应用于与一个或多个标签选择器匹配的 Pod 集合。

这些网络策略规则定义为 YAML 清单。 网络策略可以包含在同时也创建部署或服务的更大清单中。

### <a name="network-policy-options-in-aks"></a>AKS 中的网络策略选项

Azure 提供两种实现网络策略的方法。 创建 AKS 群集时，请选择网络策略选项。 创建群集后，无法更改策略选项：

* Azure 自身的实现，称为 Azure 网络策略。
* Calico 网络策略，一种由 [Tigera][tigera] 构建的开源网络和网络安全解决方案。

这两种实现都使用 Linux IPTables 来强制实施指定的策略。 策略被转换为允许和禁止的 IP 对集。 然后，这些 IP 对作为 IPTable 筛选规则进行编程。
...
| 符合 Kubernetes 规范 | 支持所有策略类型 |  支持所有策略类型 |
| 其他功能                      | 无                       | 由全局网络策略、全局网络集和主机终结点组成的扩展策略模型。 有关使用 `calicoctl` CLI 管理这些扩展功能的详细信息，请参阅 [calicoctl 用户参考][calicoctl]。 |
| 支持                                  | 受 Azure 支持和工程团队支持 | Calico 社区支持。 有关其他付费支持的详细信息，请参阅 [Project Calico 支持选项][calico-support]。 |
| 日志记录                                  | 在 IPTables 中添加/删除的规则记录在每个主机的 /var/log/azure-npm.log 下 | 有关详细信息，请参阅 [Calico 组件日志][calico-logs] |

## <a name="create-an-aks-cluster-and-enable-network-policy"></a>创建 AKS 群集并启用网络策略

若要查看网络策略的实际运行情况，让我们创建并扩展定义流量流的策略：

* 拒绝流向 Pod 的所有流量。
* 允许基于 Pod 标签的流量。
* 允许基于命名空间的流量。

首先创建一个支持网络策略的 AKS 群集。 

> [!IMPORTANT]
>
> 创建群集后，才能启用网络策略功能。 无法在现有 AKS 群集上启用网络策略。

若要使用 AKS 网络策略，必须使用 [Azure CNI 插件][azure-cni]并定义自己的虚拟网络和子网。 有关如何规划所需子网范围的更多详细信息，请参阅[配置高级网络][use-advanced-networking]。 Calico 网络策略既可与此同一 Azure CNI 插件一起使用，也可与 Kubenet CNI 插件一起使用。
...
创建群集需要几分钟时间。 等群集准备就绪后，使用 [az aks get-credentials][az-aks-get-credentials] 命令将 `kubectl` 配置为连接到 Kubernetes 群集。 此命令将下载凭据，并将 Kubernetes CLI 配置为使用这些凭据：
...
对于示例应用程序环境和流量规则，我们首先创建一个名为 development 的命名空间，以运行示例 Pod：
...
另外创建一个 Pod 并附加终端会话，以测试能否成功访问默认 NGINX 网页：
...
你已确认可以在示例后端 Pod 上使用基本的 NGINX 网页，接下来创建一个拒绝所有流量的网络策略。 创建名为 `backend-policy.yaml` 的文件并粘贴以下 YAML 清单。 此清单使用 podSelector 将策略附加到具有 app:webapp,role:backend 标签的 Pod，例如示例 NGINX Pod 。 入口下未定义任何规则，因此将拒绝流向 Pod 的所有入站流量：
...
让我们看看能否在后端 Pod 上再次使用 NGINX 网页。 创建另一个测试 Pod，并附加一个终端会话：
...
在上一节中，我们计划了后端 NGINX Pod 并创建了拒绝所有流量的网络策略。 让我们创建一个前端 Pod 并更新网络策略，以允许来自前端 Pod 的流量。
...
> [!NOTE]
> 此网络策略使用 namespaceSelector 和 podSelector 元素作为入口规则 。 YAML 语法对于入口规则能否累加很重要。 在此示例中，两个元素必须匹配要应用的入口规则。 1\.12 之前的 Kubernetes 版本可能无法正确解释这些元素并按预期限制网络流量。 有关此行为的详细信息，请参阅[针对选择器的行为和来自选择器的行为][policy-rules]。
...
计划标记为 app=webapp,role=frontend 的 Pod，并附加终端会话：
...
网络策略允许来自标记为 app: webapp,role: frontend 的 Pod 的流量，但应拒绝其他所有流量。 让我们测试一下另一个没有这些标签的 Pod 是否可以访问后端 NGINX Pod。 创建另一个测试 Pod，并附加一个终端会话：
...
在上述示例中，你创建了拒绝所有流量的网络策略，然后更新了策略以允许来自具有特定标签的 Pod 的流量。 另一个常见的需求是将流量限制为仅在给定命名空间内。 如果上面的示例针对的是 development 命名空间中的流量，请创建一个网络策略，阻止来自其他命名空间（例如，production）的流量到达 Pod 。
...
让我们更新入口规则 namespaceSelector 部分，以仅允许来自 development 命名空间的流量 。 编辑 backend-policy.yaml 清单文件，如以下示例所示：
...
在更复杂的示例中，可以定义多个入口规则，例如使用 namespaceSelector，然后使用 podSelector 。
...
拒绝来自 production 命名空间的流量后，在 development 命名空间中再次计划一个测试 Pod，并附加终端会话 ：
...
由于该 Pod 在与网络策略中允许的内容匹配的命名空间中计划，因此允许该流量。 以下示例输出显示了返回的默认 NGINX 网页：
