...
- **LoadBalancer** - 创建 Azure 负载均衡器资源、配置外部 IP 地址并将请求的 Pod 连接到负载均衡器后端池。 为了允许客户的流量到达应用程序，在所需的端口上创建负载平衡规则。 
...
关于 DNS，库贝内特和 Azure CNI 插件 DNS 由在 AKS 中运行的守护进程集 CoreDNS 提供。 有关库伯奈斯上的 CoreDNS 的详细信息，请参阅[自定义 DNS 服务](https://kubernetes.io/docs/tasks/administer-cluster/dns-custom-nameservers/)。 CoreDNS 按默认值配置为将未知域转发到节点 DNS 服务器，换句话说，转发到部署 AKS 群集的 Azure 虚拟网络的 DNS 功能。 因此，Azure DNS 和专用区域将适用于在 AKS 中运行的窗格。
...
* 如果手动为 AKS 群集创建虚拟网络资源，则在配置自己的 UDR 或服务终结点时，支持您。
* 如果 Azure 平台自动为 AKS 群集创建虚拟网络资源，则不支持手动更改这些 AKS 管理的资源以配置自己的 UDR 或服务终结点。
...
还可以配置入口控制器，以便在对 AKS 群集中的容器发出请求时保留客户端源 IP。 当客户端的请求通过入口控制器路由到 AKS 群集中的容器时，该请求的原始源 IP 将不可用到目标容器。 如果启用客户端源 IP 保留，则可以在请求标头中的 *X-Forwarded-For* 下使用客户端的源 IP。** 如果您在入口控制器上使用客户端源 IP 保留，则不能使用 SSL 传递。 可对其他服务（例如 *LoadBalancer* 类型的服务）使用客户端源 IP 保留和 SSL 直通。
