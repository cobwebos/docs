...
借助 Azure CNI，每个 pod 都可以从子网获取 IP 地址，并且可以直接访问。 这些 IP 地址在网络空间必须是唯一的，并且必须事先计划。 每个节点都有其支持的最大 pod 数的配置参数。 然后，为该节点预先保留每个节点的等效 IP 地址数。 此方法需要更多的计划，并且经常会耗尽 IP 地址或者在应用程序需求增长时需要在更大的子网中重新生成群集。
