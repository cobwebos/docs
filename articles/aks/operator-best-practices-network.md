...
* **Kubenet 网络**-Azure 管理的虚拟网络资源，如群集部署，并使用[kubenet][kubenet]的 Kubernetes 插件。
* **Azure CNI 网络**-部署到现有的虚拟网络，并使用[Azure 容器网络接口 (CNI)][cni-networking]的 Kubernetes 插件。 Pod 接收可以路由到其他网络服务或本地资源的各个 Ip。
...
有关 AKS 服务主体的委派的详细信息，请参阅[委派到其他 Azure 资源的访问权限][sp-delegation]。

每个节点和 Pod 在接收自己的 IP 地址时，请规划 AKS 子网的地址范围。 子网必须大到足以为每个部署的节点、Pod 和网络资源提供 IP 地址。 每个 AKS 群集必须位于自己的子网中。 要允许连接到 Azure 中的本地网络或对等互连网络，请勿使用与现有网络资源重叠的 IP 地址范围。 每个节点使用 kubenet 和 Azure CNI 网络运行的 Pod 数量存在默认限制。 若要处理纵向扩展事件或群集升级，还需要可在分配的子网中使用的其他 IP 地址。 此额外的地址空间是尤为重要，如果使用 Windows Server 容器 （目前以预览版在 AKS 中），因为这些节点池需要应用最新安全修补程序升级。 Windows Server 节点的详细信息，请参阅[升级在 AKS 中的节点池][nodepool-upgrade]。

若要计算的 IP 地址需要，请参阅[在 AKS 中的配置 Azure CNI 网络][advanced-networking]。
...
Kubenet 适用于小型开发或测试工作负荷，因为无需从 AKS 群集单独创建虚拟网络和子网。 流量较低或者将工作负荷直接迁移到容器中的简单网站，也可以受益于使用 kubenet 网络部署的 AKS 群集的简单性。 对于大多数生产部署，应计划和使用 Azure CNI 网络。 此外可以[配置你自己的 IP 地址范围和虚拟网络在使用 kubenet][aks-configure-kubenet-networking]。
...
入口控制器是在 AKS 节点上运行的守护程序并监视传入请求。 然后根据入口资源中定义的规则分配流量。 最佳常见的入口控制器基于 [NGINX]。 AKS 不会限制您对特定控制器，以便您可以使用其他控制器，如[Contour][contour], [HAProxy][haproxy]，或[Traefik][traefik]。

必须在 Linux 节点上计划入口控制器。 Windows Server 节点 （目前以预览版在 AKS 中） 不应运行入口控制器。 在 YAML 清单或部署 Helm 图表中的节点选择器用于指示资源应在基于 Linux 的节点上运行。 有关详细信息，请参阅[使用节点选择器来控制其中 pod 安排在 AKS 中][concepts-node-selectors]。
...
**最佳实践指导原则**-若要扫描潜在攻击的传入流量，使用如下所示的 web 应用程序防火墙 (WAF) [Barracuda WAF for Azure][barracuda-waf]或 Azure 应用程序网关。 这些更高级的网络资源还可以将流量路由到 HTTP 和 HTTPS 连接或基本 SSL 终端之外。
...
负载均衡器或入口资源继续在 AKS 群集中运行以进一步优化流量分配。 通过资源定义，可以将应用程序网关可以作为入口控制器进行集中管理。 若要开始，[创建应用程序网关入口控制器][app-gateway-ingress]。
...
若要使用网络策略，必须在创建 AKS 群集时启用此功能。 无法在现有 AKS 群集上启用网络策略。 请提前进行规划以确保在群集上启用网络策略并根据需要使用它们。 网络策略应仅用于基于 Linux 的节点和 AKS 中的 pod。
...
若要开始使用策略，请参阅[保护使用 Azure Kubernetes 服务 (AKS) 中的网络策略的 pod 之间的流量][use-network-policies]。
...
堡垒主机的管理网络也应受到保护。 使用[Azure ExpressRoute][expressroute] or [VPN gateway][vpn-gateway]连接到本地网络，并使用网络安全组控制访问。

## <a name="next-steps"></a>后续步骤

本文重点介绍网络连接性和安全性。 有关在 Kubernetes 中的网络基础知识的详细信息，请参阅[网络应用程序在 Azure Kubernetes 服务 (AKS) 的概念][aks-concepts-network]
