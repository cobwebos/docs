---
title: 查看 Azure Kubernetes 服务中的 kubelet 日志（AKS）
description: 了解如何在 Azure Kubernetes Service （AKS）节点的 kubelet 日志中查看疑难解答信息
...
ms.locfileid: "77595376"
---
# <a name="get-kubelet-logs-from-azure-kubernetes-service-aks-cluster-nodes"></a>从 Azure Kubernetes 服务 (AKS) 群集节点获取 kubelet 日志

在运行 AKS 群集的过程中，你可能需要查看日志来解决问题。 内置到 Azure 门户可以查看[AKS 群集中][azure-container-logs] [AKS 主组件][aks-master-logs]或容器的日志。 有时，可能需要从 AKS 节点获取*kubelet*日志，以便进行故障排除。

本文介绍如何使用 `journalctl` 查看 AKS 节点上的*kubelet*日志。
...
首先，创建与需要在其上查看*kubelet*日志的节点建立 SSH 连接。 [SSH 到 Azure Kubernetes Service （AKS）群集节点][aks-ssh]文档中详细介绍了此操作。
