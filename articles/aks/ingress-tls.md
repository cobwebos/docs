...
本文介绍如何部署[NGINX 入口控制器][nginx-ingress]in an Azure Kubernetes Service (AKS) cluster. The [cert-manager][cert-manager]项目用于自动生成并配置[let 's Encrypt][允许加密]证书。 最后，在 AKS 群集中运行两个应用程序（可通过单个 IP 地址访问其中的每个应用程序）。
...
- [创建使用 let 's Encrypt，自动生成具有静态公共 IP 地址的 TLS 证书入口控制器][aks-ingress-static-tls]
