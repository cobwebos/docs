...
现在使用 访问 Kubernets 入口控制器的地址，`curl`例如*http://10.240.0.42*。 提供自己的内部 IP 地址，该地址是在本文第一步中部署入口控制器时指定的。
...
现在向地址添加 */hello 世界两个*路径，如*http://10.240.0.42/hello-world-two*。 第二个使用自定义标题的演示应用程序已返回，如以下精简版示例输出中所示：
