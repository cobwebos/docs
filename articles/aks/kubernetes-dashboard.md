...
有关 Kubernetes 仪表板的详细信息，请参阅 [Kubernetes Web UI 仪表板][kubernetes-dashboard]。 AKS 使用开源仪表板的2.0 版及更高版本。

> [!WARNING]
> **AKS 仪表板外接程序设置为弃用。** 
> * 默认情况下，Kubernetes 仪表板对于运行 Kubernetes 版本低于1.18 的群集处于启用状态。
> * 默认情况下，将为在 Kubernetes 1.18 或更高版本上创建的所有新群集禁用仪表板加载项。 
 > * 从预览版 Kubernetes 1.19 开始，AKS 将不再支持安装托管 kube-仪表板加载项。 
 > * 启用了外接程序的现有群集不会受到影响。 用户将继续以用户安装的软件的形式手动安装开放源仪表板。

## <a name="before-you-begin"></a>开始之前

本文档中详细介绍的步骤假定已创建 AKS 群集，并且已建立 `kubectl` 与群集的连接。 如果需要创建 AKS 群集，请参阅 [AKS 快速入门][aks-quickstart]。

还需要安装并配置 Azure CLI 版本2.6.0 或更高版本。 运行  `az --version`  即可查找版本。 如果需要进行安装或升级，请参阅 [安装 Azure CLI][install-azure-cli]。

## <a name="disable-the-kubernetes-dashboard"></a>禁用 Kubernetes 仪表板

**默认情况下，在 K8s 1.18 以前的群集上启用**kube 加载项。 可以通过运行以下命令来禁用此加载项。
...
若要在群集上启动 Kubernetes 仪表板，请使用[az aks browse][az-aks-browse]命令。 此命令要求在群集上安装 kube 加载项，默认情况下，运行的版本低于 Kubernetes 1.18 的群集上包含该加载项。
...
> [!NOTE]
> 如果在中看不到 "仪表板" `http://127.0.0.1:8001` ，可以手动路由到以下地址。 1.16 或更高版本的群集使用 https，需要单独的终结点。
> * K8s 1.16 或更高版本：`http://127.0.0.1:8001/api/v1/namespaces/kube-system/services/https:kubernetes-dashboard:/proxy`
> * K8s 1.15 及更低：`http://127.0.0.1:8001/api/v1/namespaces/kube-system/services/kubernetes-dashboard:/proxy`
...
> [!IMPORTANT]
> 在[Kubernetes 仪表板](https://github.com/kubernetes/dashboard/releases/tag/v1.10.1)或 Kubernetes v 1.16 的 v 1.10.1 中，由于[该版本中的安全修补程序](https://github.com/kubernetes/dashboard/pull/3400)，不能再用于检索资源。 因此，没有身份验证信息的请求会返回401未经授权的错误。 从服务帐户检索的持有者令牌仍可像在此[Kubernetes 仪表板示例](https://kubernetes.io/docs/tasks/access-application-cluster/web-ui-dashboard/#accessing-the-dashboard-ui)中一样使用，但这会影响与较旧版本相比，仪表板加载项的登录流。
>
>如果仍在1.16 之前运行版本，仍可以向 "kubernetes" 服务帐户授予权限，但**不建议这样做**：
> ```console
> kubectl create clusterrolebinding kubernetes-dashboard --clusterrole=cluster-admin --serviceaccount=kube-system:kubernetes-dashboard
> ```

提供的初始屏幕需要 kubeconfig 或令牌。 这两个选项都需要资源权限，以在仪表板中显示这些资源。

![登录屏幕](./media/kubernetes-dashboard/login.png)

**使用 kubeconfig**

对于启用 Azure AD 和非 Azure AD 的群集，可以传入 kubeconfig。 确保访问令牌有效，如果令牌已过期，可以通过 kubectl 刷新令牌。

1. 将 admin kubeconfig 设置为`az aks get-credentials -a --resource-group <RG_NAME> --name <CLUSTER_NAME>`
...
1. 对于**启用了非 Azure AD 的群集**，请运行 `kubectl config view` 并复制与群集的用户帐户关联的令牌。
1. 登录时，粘贴到 "令牌" 选项。    
1. 单击 `Sign In`

对于启用 Azure AD 的群集，请通过以下命令检索 AAD 令牌。 验证是否已在命令中替换资源组和群集名称。
