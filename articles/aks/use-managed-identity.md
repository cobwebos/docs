...
description: '了解如何在 Azure Kubernetes Service (AKS 中使用托管标识) '
...
| kubelet | AKS 群集名称-agentpool | 用 Azure 容器注册表进行身份验证 (ACR)  | 节点资源组的 "读取者" 角色 | 目前不支持
| 加载项 | AzureNPM | 无需标识 | 不可用 | 不适合
| 加载项 | AzureCNI 网络监视 | 无需标识 | 不可用 | 不适合
| 加载项 | azurepolicy (守卫)  | 无需标识 | 不可用 | 不适合
| 加载项 | azurepolicy | 无需标识 | 不可用 | 不适合
| 加载项 | Calico | 无需标识 | 不可用 | 不适合
| 加载项 | 仪表板 | 无需标识 | 不可用 | 不适合
| 加载项 | HTTPApplicationRouting | 管理所需的网络资源 | 节点资源组的 "读取者" 角色，DNS 区域的参与者角色 | 不适合
| 加载项 | 入口应用程序网关 | 管理所需的网络资源| 节点资源组的参与者角色 | 不适合
| 加载项 | omsagent | 用于将 AKS 指标发送到 Azure Monitor | 监视指标发布者角色 | 不适合
| 加载项 | 虚拟节点 (ACIConnector)  | 管理 (ACI) 的 Azure 容器实例所需的网络资源 | 节点资源组的参与者角色 | 不适合
