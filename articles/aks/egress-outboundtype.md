...
* 在预览期间，只能在创建群集时定义 `outboundType`，以后不能更新。
* 预览期间，`outboundType` AKS 群集应使用 Azure CNI。 Kubenet 是可配置的，使用情况需要将路由表手动关联到 AKS 子网。
* 设置 `outboundType` 要求 AKS 群集的 `vm-set-type` `VirtualMachineScaleSets`，`load-balancer-sku` 为 `Standard`。
* 将 `outboundType` 设置为 `UDR` 的值需要用户定义的路由，该路由具有群集的有效出站连接。
* 将 `outboundType` 设置为 `UDR` 的值意味着路由到负载平衡器的入口源 IP 可能与群集的传出出口目标地址**不匹配**。

## <a name="overview-of-outbound-types-in-aks"></a>AKS 中的出站类型概述

可以使用类型为 "负载均衡器" 或 "用户定义路由" 的唯一 `outboundType` 自定义 AKS 群集。
...
如果设置 `loadBalancer`，AKS 将自动完成以下设置。 负载均衡器用于通过 AKS 分配的公共 IP 进行传出。 `loadBalancer` 的出站类型支持 `loadBalancer`类型的 Kubernetes 服务，该服务需要从 AKS 资源提供程序创建的负载均衡器出口。
...
下面是默认情况下在 AKS 群集中部署的网络拓扑，该网络拓扑使用 `loadBalancer`的 `outboundType`。
...
如果设置 `userDefinedRouting`，AKS 不会自动配置出口路径。 应由**用户**执行以下操作。
...
现在，将下面的 `APPID` 和 `PASSWORD` 替换为上一命令输出自动生成的服务主体 appid 和服务主体密码。 我们将引用 VNET 资源 ID 来向服务主体授予权限，以便 AKS 可以将资源部署到其中。
...
最后，可以将 AKS 群集部署到专用于群集的现有子网中。 要部署到的目标子网是通过环境变量（`$SUBNETID`）定义的。
...
使用 kubectl 应用部署内部负载均衡器，并指定 YAML 清单的名称：
