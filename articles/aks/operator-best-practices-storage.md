...
**最佳做法指南**-使用适用于存储类型的适当工具备份数据，如 Velero 或 Azure Site Recovery。 验证这些备份的完整性和安全性。

当应用程序存储和使用永久存储在磁盘或文件中的数据时，需要定期备份或创建数据的快照。 Azure 磁盘可以使用内置快照技术。 在执行快照操作之前，您可能需要查找您的应用程序以刷新写入磁盘。 [Velero][velero]可以备份持久卷以及其他群集资源和配置。 如果无法[从应用程序中删除状态][remove-state]，请从永久卷备份数据，并定期测试还原操作，以验证数据完整性和所需的过程。
