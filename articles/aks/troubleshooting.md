...
## <a name="im-receiving-errors-that-my-cluster-is-in-failed-state-and-upgrading-or-scaling-will-not-work-until-it-is-fixed"></a>我收到了群集处于失败状态的错误，在修复之前，升级或缩放将无法工作
...
1. 在群集超出 `failed` 状态之前，`upgrade` 和 `scale` 操作都不会成功。 常见的根本问题和解决方法包括：
    ...
    * 缩放具有高级网络的群集和**子网（网络）资源不足**。 若要解决此问题，请先将群集缩放回配额内的稳定目标状态。 然后[，执行以下步骤，请求增加资源配额](../azure-resource-manager/resource-manager-quota-errors.md#solution)，然后再尝试再次增加超出初始配额限制。
2. 解决升级失败的根本原因后，群集应处于 "已成功" 状态。 验证成功状态后，重试原始操作。

## <a name="im-receiving-errors-when-trying-to-upgrade-or-scale-that-state-my-cluster-is-being-currently-being-upgraded-or-has-failed-upgrade"></a>尝试升级或缩放时，收到错误：群集当前正在升级或升级失败
...
若要帮助诊断问题，请运行 `az aks show -g myResourceGroup -n myAKSCluster -o table` 以检索群集上的详细状态。 根据结果：

* 如果群集正在积极升级，请等待操作终止。 如果该操作成功，请再次重试先前失败的操作。
* 如果群集升级失败，请按照上一部分中所述的步骤进行操作。

## <a name="can-i-move-my-cluster-to-a-different-subscription-or-my-subscription-with-my-cluster-to-a-new-tenant"></a>是否可以将我的群集移到不同的订阅或使用我的群集将我的订阅转移到新租户？

如果已将 AKS 群集移到另一个订阅或拥有新租户订阅的群集，群集将丢失功能，因为角色分配和服务主体权限丢失。 **AKS 不支持跨订阅或租户移动群集**，因为存在此约束。
...
*此故障排除帮助是从 aka.ms/aks-naming-rules*

命名限制由 Azure 平台和 AKS 实现。 如果某个资源名称或参数中断了这些限制，则会返回一个错误，要求你提供不同的输入。 以下常见命名准则适用：

* AKS *MC_* 资源组名称结合了资源组名称和资源名称。 @No__t_0 的自动生成的语法不能超过80个字符。 如果需要，请减小资源组名称或 AKS 群集名称的长度。
