...
如果使用 Azure 容器注册表（ACR）作为容器映像存储，则需要向 AKS 群集的服务主体授予权限以读取和提取映像。 目前，建议的配置是使用[az aks create][az-aks-create]或 [az aks update] [az-aks] 命令与注册表集成，并为服务主体分配适当的角色。 有关详细步骤，请参阅通过 azure[容器注册表从 Azure Kubernetes 服务进行身份验证][aks-to-acr]。
