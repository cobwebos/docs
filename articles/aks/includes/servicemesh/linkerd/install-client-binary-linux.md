...
@No__t_0 客户端二进制文件在客户端计算机上运行，并允许与 Linkerd 服务网格交互。 使用以下命令在 Linux 上基于 bash 的 shell 或[适用于 linux 的 Windows 子系统][install-wsl]上安装 Linkerd `linkerd` 客户端二进制文件。 这些命令可将 `linkerd` 客户端二进制文件复制到 `PATH` 中的标准用户程序位置。
