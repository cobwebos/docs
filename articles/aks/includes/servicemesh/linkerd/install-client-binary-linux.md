...
`linkerd` 客户端二进制文件在客户端计算机上运行，用来与 Linkerd 服务网格交互。 在 Linux 或`linkerd`适用于 Linux 的 Windows 子系统[上的基于 bash 的 shell 中使用以下命令安装 Linkerd ][install-wsl] 客户端二进制文件。 这些命令可将 `linkerd` 客户端二进制文件复制到 `PATH` 中的标准用户程序位置。
