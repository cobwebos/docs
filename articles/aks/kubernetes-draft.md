...
若要将基本 Tiller 部署到 AKS 群集，请使用[helm init][helm-init]命令。 如果你的群集未启用 RBAC，则删除 `--service-account` 参数和值。
...
如果你尚未在本地计算机上配置草稿，请运行 `draft init`：
...
还需要将草稿配置为使用 ACR 的*loginServer* 。 以下命令使用 `draft config set` 将 `mydraftacr.azurecr.io` 用作注册表。
...
更新 `charts/example-java/values.yaml` 以创建*LoadBalancer*服务。 将*群集 ip*的值从 "*类型*" 更改为 " *LoadBalancer*"。
...
保存所做的更改，关闭文件，然后运行 `draft up` 以重新运行该应用程序。
...
使用*外部 IP*在浏览器中导航到应用程序的负载均衡器，查看示例应用程序。 在上面的示例中，`52.175.224.118`了 IP。

## <a name="iterate-on-the-application"></a>循环访问应用程序

你可以通过在本地进行更改来循环访问应用程序，然后重新运行 `draft up`。
