...
使用 Azure Dev Spaces 直接针对 AKS 群集开发、调试和测试应用程序。 在整个应用程序生命周期，团队中的开发人员共同协作进行生成和测试。 可以继续使用现有的工具，例如 Visual Studio 或 Visual Studio Code。 为 Dev 空间安装了一个扩展，该扩展提供了一个选项，用于在 AKS 群集中运行和调试应用程序。
