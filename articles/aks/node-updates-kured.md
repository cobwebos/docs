---
title: 通过 kured 处理 Linux 节点重新启动
...
你可以使用自己的工作流和进程来重启节点，或使用 `kured` 安排该进程。 使用 `kured`，可以部署在群集每个 Linux 节点上运行 Pod 的 [DaemonSet][DaemonSet]。 DaemonSet 中的这些 pod 会监视 */var/run/reboot-required*文件是否存在，然后启动一个进程来重新启动节点。
...
若要部署`kured` DaemonSet，请安装以下官方 Kured Helm 图表。 这将创建一个角色和群集角色、绑定和一个服务帐户，然后使用`kured`部署 DaemonSet。
...
也可以为 `kured` 配置其他参数，例如与 Prometheus 或 Slack 集成。 有关其他配置参数的详细信息，请参阅[Kured Helm chart][kured-install]。
