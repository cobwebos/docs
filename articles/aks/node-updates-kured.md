...
使 Windows Server 节点（当前在 AKS 中预览）保持最新的过程略有不同。 Windows 服务器节点不会接收每日更新。 相反，您将执行 AKS 升级，该升级部署具有最新基本窗口服务器映像和修补程序的新节点。 有关使用 Windows 服务器节点的 AKS 群集，请参阅[升级 AKS 中的节点池][nodepool-upgrade]。
...
你可以使用自己的工作流和进程来重启节点，或使用 `kured` 安排该进程。 使用 `kured`，可以部署在群集每个 Linux 节点上运行 Pod 的 [DaemonSet][DaemonSet]。 守护程序中的这些窗格监视*存在 /var/运行/重新启动所需的*文件，然后启动进程以重新启动节点。
...
要部署`kured`守护进程集，请安装以下官方的 Kured Helm 图表。 这将创建角色和群集角色、绑定和服务帐户，然后使用`kured`部署守护进程集。
...
更新过程完成后，可使用带有  参数的 [kubectl get nodes`--output wide`][kubectl-get-nodes] 命令查看节点的状态。 通过此附加输出，可发现基础节点的 KERNEL-VERSION 会有所差异，如以下示例输出所示**。 在上一步中已更新 *aks-nodepool1-28993262-0*，并显示内核版本为 *4.15.0-1039-azure*。 尚未更新的 *aks-nodepool1-28993262-1* 节点显示的内核版本为 *4.15.0-1037-azure*。
...
有关使用 Windows 服务器节点的 AKS 群集，请参阅[升级 AKS 中的节点池][nodepool-upgrade]。
