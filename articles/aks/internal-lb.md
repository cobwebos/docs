...
> [!NOTE]
> Azure 负载均衡器以两种 SKU 提供：“基本”和“标准”   。 默认情况下*基本*SKU 时的服务清单用于在 AKS 上创建负载均衡器使用。 有关详细信息，请参阅[Azure 负载均衡器 SKU 的比较][azure-lb-comparison]。
...
还需安装并配置 Azure CLI 2.0.59 或更高版本。 运行  `az --version` 即可查找版本。 如果你需要安装或升级，请参阅 [安装 Azure CLI][install-azure-cli]。
...
部署内部负载均衡器使用[kubectl 应用][kubectl-apply]并指定 YAML 清单的名称：
...
创建 AKS 群集时，可以指定高级网络设置。 此方法允许将群集部署到现有 Azure 虚拟网络和子网中。 一种方案是将 AKS 群集部署到连接到本地环境的专用网络，并运行仅在内部可访问的服务。 有关详细信息，请参阅配置与你自己虚拟网络子网[Kubenet][use-kubenet] or [Azure CNI][advanced-networking]。
...
> [!NOTE]
> 可能需要向 AKS 群集的服务主体授予针对部署了 Azure 虚拟网络资源的资源组的“网络参与者”  角色。 查看与服务主体[az aks show][az-aks-show]，如`az aks show --resource-group myResourceGroup --name myAKSCluster --query "servicePrincipalProfile.clientId"`。 若要创建角色分配，请使用[az 角色分配创建][az-role-assignment-create]命令。
