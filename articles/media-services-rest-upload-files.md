<properties 
	pageTitle="使用 REST API 将文件上载到 Media Services 帐户" 
...
# 使用 REST API 将文件上载到 Media Services 帐户
[WACOM.INCLUDE [media-services-selector-upload-files](../includes/media-services-selector-upload-files.md)]

本文是介绍 [Media Services 视频点播工作流](/zh-cn/documentation/articles/media-services-video-on-demand-workflow)的系列教程的一部分。前一个主题是[使用 REST API 连接到 Media Services](/zh-cn/documentation/articles/media-services-rest-connect_programmatically/)。
...
>成功连接到 https://media.chinacloudapi.cn 后，将收到一个 301 重定向，指定另一个 Media Services URI。必须根据[使用 REST API 连接到 Media Services](/zh-cn/documentation/articles/media-services-rest-connect_programmatically/) 中所述对新的 URI 执行后续调用。 
 

## <a id="upload"></a>使用 REST API 创建新资产并上载视频文件

在 Media Services 中，可以将数字文件上载到资产中。[资产](https://msdn.microsoft.com/zh-cn/library/azure/hh974277.aspx)实体可以包含视频、音频、图像、缩略图集合、图文轨迹和隐藏字幕文件（以及有关这些文件的元数据。）将文件上载到资产后，相关内容即安全地存储在云中供后续处理和流式处理。 
...
- **None** = **0**：将不使用加密。这是默认值。请注意，使用此选项时，你的内容在传送过程中或静态存储过程中都不会受到保护。
	如果计划使用渐进式下载交付 MP4，则使用此选项。 

- **StorageEncrypted** = **1**：指定是否要使用 AES-256 位加密法对文件加密以方便上载和存储。

- **CommonEncryptionProtected** = **2**：指定是否要上载使用常见加密法（例如 PlayReady）保护的文件。 

- **EnvelopeEncryptionProtected** = **4**：指定是否要上载使用 AES 文件加密的 HLS。请注意，Transform Manager 必须已对文件进行编码和加密。

如果资产要使用加密，则你必须根据以下主题中所述创建 **ContentKey** 并将其链接到你的资产：[如何创建 ContentKey](/zh-cn/documentation/articles/media-services-rest-create-contentkey)。请注意，将文件上载到资产后，需要使用加密**资产**期间获取的值更新 **AssetFile** 上的加密属性。使用 **MERGE** HTTP 请求完成此操作。 
...
[AssetFile](http://msdn.microsoft.com/library/azure/hh974275.aspx) 实体表示 Blob 容器中存储的视频或音频文件。一个资产文件始终与一个资产关联，而一个资产则可能包含一个或多个资产文件。如果资产文件对象未与 BLOB 容器中的数字文件关联，则 Media Services Encoder 任务将失败。
...
## 后续步骤
将资产上载到 Media Services 后，请转到[如何获取媒体处理器][]主题。

[如何获取媒体处理器]: /zh-cn/documentation/articles/media-services-get-media-processor/
