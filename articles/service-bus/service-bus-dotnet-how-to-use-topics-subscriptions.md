...
以下代码演示了如何使用 [`CreateFromConnectionString`](https://msdn.microsoft.com/library/microsoft.servicebus.messaging.topicclient.createfromconnectionstring.aspx) API 调用为上面创建的 **TestTopic** 主题创建 [`TopicClient`](https://msdn.microsoft.com/zh-cn/library/microsoft.servicebus.messaging.topicclient.aspx) 对象：
...
服务总线主题支持[最大为 256 Kb 的消息](service-bus-quotas.md)（标头最大为 64 Kb，其中包括标准和自定义应用程序属性）。一个主题中包含的消息数量不受限制，但消息的总大小受限制。此主题大小是在创建时定义的，上限为 5 GB。如果启用了分区，则上限更高。有关详细信息，请参阅[分区消息实体](https://msdn.microsoft.com/library/azure/dn520246.aspx)。
