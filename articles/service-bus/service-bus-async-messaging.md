...
对于这两种情况，自然或人为灾难都可能导致此问题发生。若要解决此问题并确保你仍可发送消息，可以使用[成对命名空间][]，以允许在主位置恢复正常时能够将消息发送到第二个位置。有关详细信息，请参阅[《Best practices for insulating applications against Service Bus outages and disasters》][]（使应用程序免受服务总线中断和灾难影响的最佳实践）。
