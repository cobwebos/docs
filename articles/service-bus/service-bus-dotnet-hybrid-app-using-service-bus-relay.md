...
5.  在确保命名空间名称可用后，选择应承载您的命名空间的国家或地区（确保使用在其中部署计算资源的同一国家/地区）。
    ...
    > [AZURE.IMPORTANT] 选取要选择用于部署应用程序的相同区域。这将为你提供最佳性能。
...
8. 现在必须配置新 Web 应用的 Azure 资源。请按照[配置新 Web 应用的 Azure 资源](/documentation/articles/#configure-azure-resources-for-a-new-web-app)一节中的所有步骤操作。然后，返回到本教程并继续执行下一步。
