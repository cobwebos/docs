...
> [AZURE.NOTE]你可以对 [`NamespaceManager`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.namespacemanager.aspx) 对象使用 [`QueueExists`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.namespacemanager.queueexists.aspx) 方法，以检查具有指定名称的队列是否已位于服务命名空间中。
...
从队列接收消息的主要方法是使用 [`QueueClient`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.queueclient.aspx) 对象。`QueueClient` 对象可在两种不同模式下工作：[`ReceiveAndDelete` 和 `PeekLock`](https://msdn.microsoft.com/zh-cn/library/azure/microsoft.servicebus.messaging.receivemode.aspx)。
