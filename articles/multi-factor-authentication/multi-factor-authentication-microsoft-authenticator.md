...
Microsoft Authenticator 应用在以下帐户中提供额外安全级别：Azure 帐户（例如，bsimon@contoso.onmicrosoft.com), 本地工作帐户（例如，bsimon@contoso.com), 或 Microsoft 帐户（例如，bsimon@outlook.com).
...
### <a name="add-an-account-to-the-app-by-using-the-qr-code-scanner"></a>使用 QR 码扫描仪将帐户添加到应用
...
3. 打开 Microsoft Authenticator 应用。 在“帐户”屏幕上，选择 **+**，然后指定要添加工作帐户或学校帐户。
    ...
    ![用于指定工作帐户或学校帐户的屏幕](./media/multi-factor-authentication-azure-authenticator/scan.png)
...
5. 稍候片刻，帐户即会激活。 激活完成后，选择“与我联系”。  此时会将通知或验证码发送到手机。  选择“验证”。
    ...
    ![在其中选择“验证”以登录的屏幕](./media/multi-factor-authentication-azure-authenticator/verify.png)
6. 如果公司需要 PIN 才能批准登录验证，请输入它。

    ![用于输入 PIN 的框](./media/multi-factor-authentication-end-user-first-time/scan3.png)
7. 填入 PIN 条目后，选择“关闭”。 此时，验证应已成功。
...
5. 在应用的相应框中输入该代码和 URL。
    ...
    ![用于输入代码和 URL 的屏幕](./media/multi-factor-authentication-azure-authenticator/addaccount2.png)
...
### <a name="add-an-account-to-the-app-by-using-touch-id"></a>使用 Touch ID 将帐户添加到应用
...
## <a name="uninstall-the-old-azure-authentication-app"></a>卸载旧的 Azure Authentication 应用
将所有帐户添加到新应用后，可以从手机中卸载旧应用。

## <a name="delete-an-account"></a>删除帐户
若要从 Microsoft Authenticator 应用删除某个帐户，请选择该帐户，然后选择“删除”。
