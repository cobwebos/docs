...
| 绑定 DN 或绑定用户名 |输入绑定到 LDAP 目录时要使用的帐户的用户记录的可分辨名称。<br><br>仅当绑定类型为“简单”或“SSL”时使用绑定的可分辨名称。  <br><br>输入在绑定类型为 Windows 时，绑定到 LDAP 目录时要使用的 Windows 帐户的用户名。  如果留空，已登录用户的帐户将用于绑定。 |
