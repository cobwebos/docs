...
# <a name="getting-started-with-an-azure-multi-factor-auth-provider"></a>Azure 多重身份验证提供程序入门
...
## <a name="what-is-an-azure-multi-factor-auth-provider"></a>什么是 Azure 多重身份验证提供程序？
...
使用以下步骤创建 Azure 多重身份验证提供程序。 Azure 多重身份验证提供程序只能在 Azure 经典门户中创建。 如果不能登录到 Azure 经典门户，请确保 Azure AD 租户[与 Azure 订阅相关联](../active-directory/active-directory-how-subscriptions-associated-directory.md)。 
...
7. 填写以下字段，并选择“创建” 。
   ...
   3. **目录** - 与多重身份验证提供程序关联的 Azure Active Directory 租户。 请注意以下事项：
      * 无需 Azure AD 目录即可创建多重身份验证提供程序。 如果仅计划下载 Azure 多重身份验证服务器或 SDK，请将此框留空。
      * 多重身份验证提供程序必须与 Azure AD 目录相关联才能利用高级功能。
      * 只能将一个多重身份验证提供程序与任意一个 Azure AD 目录相关联。  
...
## <a name="manage-your-multi-factor-auth-provider"></a>管理多重身份验证提供程序
