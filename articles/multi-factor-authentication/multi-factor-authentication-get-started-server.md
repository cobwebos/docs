...
![MFA 服务器电子邮件配置](./media/multi-factor-authentication-get-started-server/email1.png)

在“电子邮件内容”选项卡中，可以看到可供选择的电子邮件模板。 根据为用户配置的执行双步验证的方式，选择最合适的模板。

![MFA 服务器电子邮件模板](./media/multi-factor-authentication-get-started-server/email2.png)
...
4. 突出显示右侧的所有用户，**并单击“导入**。  此时应会显示一个弹出窗口，指出操作已成功。  关闭导入窗口。
   ...
   ![MFA 服务器用户导入](./media/multi-factor-authentication-get-started-server/import2.png)
