...
通过 Azure 的网络策略服务器 (NPS) 扩展，可使组织使用提供双重验证的基于云的 [Azure 多重身份验证 (MFA)](multi-factor-authentication-get-started-server-rdg.md) 来保护远程身份验证拨入用户服务 (RADIUS) 客户端身份验证。
...
11. 在“指定加密设置”窗口中，接受默认设置，然后选择“下一步”。
    ...
    ![“指定加密设置”窗口](./media/nps-extension-vpn/image8.png)

12. 在“指定领域名”窗口中，为领域名留空，接受默认设置，然后选择“下一步”。

    ![“指定领域名”窗口](./media/nps-extension-vpn/image9.png)

13. 在“完成新的拨号或虚拟专用网络连接和 RADIUS 客户端”窗口中，选择“完成”。

    ![“完成新的拨号或虚拟专用网络连接和 RADIUS 客户端”窗口](./media/nps-extension-vpn/image10.png)
...
3. 在“路由和远程访问”窗口中，右键单击“\<服务器名称> (本地)”，然后选择“属性”。
    ...
    ![路由和远程访问窗口](./media/nps-extension-vpn/image14.png)
...
7. 在“添加 RADIUS 服务器”窗口中，进行以下操作：
    ...
    ![“添加 RADIUS 服务器”窗口](./media/nps-extension-vpn/image16.png)
...
5. 在“添加 VPN 连接”窗口中的“VPN 提供程序”框中，选择“Windows(内置)”，根据需要完成剩余字段，然后选择“保存”。 
    ...
    ![“添加 VPN 连接”窗口](./media/nps-extension-vpn/image17.png)
...
10. 在“安全”选项卡上，确保仅选中“Microsoft CHAP 版本 2 (MS-CHAP v2)”，然后选择“确定”。
    ...
    ![“允许这些协议”选项](./media/nps-extension-vpn/image20.png)
...
![“要求用户匹配”设置](./media/nps-extension-vpn/image34.png)
...
4. 在“适用于 Azure MFA 设置的 NPS 扩展”窗口中，查看软件许可条款、选择“我同意许可条款和条件”复选框，然后选择“安装”。
    ...
    ![“适用于 Azure MFA 设置的 NPS 扩展”窗口](./media/nps-extension-vpn/image36.png)
 
5. 在“适用于 Azure MFA 设置的 NPS 扩展”窗口中，选择“关闭”。  

    ![“设置成功”确认窗口](./media/nps-extension-vpn/image37.png) 
...
4. 输入 Azure AD 管理员凭据和密码，然后选择“登录”。 
    ...
    ![“PowerShell 登录”窗口](./media/nps-extension-vpn/image39.png)
...
![“Windows 设置 VPN”窗口](./media/nps-extension-vpn/image42.png)
...
![PowerShell 安全事件查看器](./media/nps-extension-vpn/image44.png)
 
此外，你还可以查看“安全日志”或“网络策略和访问服务”自定义视图，如下所示：

![“网络策略服务器”日志](./media/nps-extension-vpn/image45.png)

在安装了适用于 Azure 多重身份验证的 NPS 扩展的服务器上，可以在 Application and Services Logs\Microsoft\AzureMfa 位置找到特定于此扩展的事件查看器应用程序日志。 

    _Get-WinEvent -Logname Security_ | where {$_.ID -eq '6272'} | FL

![事件查看器“事件数”窗格](./media/nps-extension-vpn/image46.png)
...
![显示失败登录事件的安全日志](./media/nps-extension-vpn/image47.png)

以下是 Azure 多重身份验证日志中的一个相关事件：

![“Azure 多重身份验证”日志](./media/nps-extension-vpn/image48.png)
...
[获取 Azure 多重身份验证](multi-factor-authentication-versions-plans.md)
