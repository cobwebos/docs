...
只要你用户的许可证包含 Azure 多重身份验证，则不需执行任何操作来启用 Azure MFA。 你可以在单个用户的基础上，开始要求进行双重验证。 启用 Azure MFA 的许可证是：
- Azure 多重身份验证
- Azure Active Directory Premium
- 企业移动性 + 安全性

如果这三个许可证你都没有，或者你的许可证数量不足以涵盖所有用户，则也没有问题。 只需执行一个额外的步骤，即可在目录中[创建多重身份验证提供程序](multi-factor-authentication-get-started-auth-provider.md)。
