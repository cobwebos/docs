...
有关详细信息和要求，请查看 Azure VMware 解决方案特定的 [应用程序网关](./protect-avs-web-apps-with-app-gateway.md) 文章。
...
作为最佳安全方案，请在中心虚拟网络中部署 [Microsoft Azure 堡垒](../bastion/index.yml) 服务。 Azure 堡垒提供对 Azure 中部署的 Vm 的无缝 RDP 和 SSH 访问，无需为这些资源预配公共 IP 地址。 预配 Azure 堡垒服务后，可以从 Azure 门户访问所选 VM。 建立连接后，会打开一个新的选项卡，其中显示了可在其中访问 Azure VMware 解决方案私有云管理平面的 Jumpbox 桌面。
...
出于标识目的，最佳方法是使用共享服务子网在中心部署至少一个 AD 域控制器。 理想情况下，在区域分发方式或 VM 可用性集中实现其中两项。 请参阅 [Azure 体系结构中心](/azure/architecture/reference-architectures/identity/adds-extend-domain) 将本地 AD 域扩展到 Azure。
