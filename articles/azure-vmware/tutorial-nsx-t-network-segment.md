...
完成本教程需要有权访问 vCenter 的 Azure VMware 解决方案私有云和 NSX-T Manager 管理界面。 请参阅[教程：在 Azure 中为 VMware 私有云配置网络](tutorial-configure-networking.md)。
...
1. 在私有云的 NSX-T Manager 中，选择“网络”。
   ...
   :::image type="content" source="media/nsxt/nsxt-network-overview.png" alt-text="在私有云的 NSX-T Manager 中，选择“网络”。":::
...
1. 在“NSX-T 网段概述”页中，选择“添加网段”。 三个网段作为私有云预配的一部分创建而成，不能用于 VM。  要用于此目的，需要添加新的网段。
   ...
   :::image type="content" source="media/nsxt/nsxt-segments-overview.png" alt-text="在“NSX-T 网段概述”页中，选择“添加网段”。":::
...
本教程中创建了用于 vCenter 中 VM 的 NSX-T 网段。 现在可以使用[教程：创建内容库以在 Azure VMware 解决方案中部署 VM](tutorial-deploy-vm-content-library.md)，以便在 vCenter 中创建内容库，并在本教程中创建的网络上预配 VM。
