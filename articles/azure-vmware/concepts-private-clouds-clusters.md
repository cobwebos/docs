...
Microsoft 负责对 NSX-t 设备（例如，NSX-T Manager 和 NSX-T 边缘）进行生命周期管理。 Microsoft 还负责引导网络配置，例如创建第0层的网关和启用北南部路由。 作为 Azure VMware 解决方案私有云的管理员，你需要负责 NSX-T SDN 配置。 例如，网络段、分布式防火墙规则、第1层网关和负载均衡器。
