...
Azure 订阅、Azure VMware 解决方案私有云、vSAN 群集和主机之间存在逻辑关系。 在此图中，将显示单个 Azure 订阅中的两个私有云。 私有云代表一个开发环境和一个生产环境，每个环境都有自己的私有云。 在每个私有云中，都有两个群集。 为了显示开发环境的更低潜在需求，使用较小容量主机的小型群集。 以下各部分介绍了所有这些概念。
...
Microsoft 负责对 NSX-t 设备（例如，NSX-T Manager 和 NSX-T 边缘）进行生命周期管理。 Microsoft 还负责引导网络配置，例如创建第0层的网关，并启用 North-South 路由。 作为 Azure VMware 解决方案私有云的管理员，你需要负责 NSX-T SDN 配置。 例如，网络段、分布式防火墙规则、第1层网关和负载均衡器。
