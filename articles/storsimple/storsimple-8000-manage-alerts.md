...
为设备启用电子邮件通知后，每次发生关键警报时，通知列表成员都将收到电子邮件消息。 消息将从 *"简单警报-无答复\@mail.windowsazure.com*发送，并将描述警报条件。 收件人可以单击“取消订阅”**** 将自己从电子邮件通知列表中删除。
...
2. 在“常规设置”**** 边栏选项卡中，转到“警报设置”****，并设置以下项：
   ...
   3. 在“其他电子邮件收件人”**** 字段中，输入应接收警报通知的所有其他收件人的电子邮件地址。 输入*某人\@somewhere.com*格式的名称。 使用分号分隔电子邮件地址。 对于每个设备，最多可以配置 20 个电子邮件地址。 
...
| 警报文本 | 事件 | 详细信息/建议的操作 |
|:--- |:--- |:--- |
| 创建本地卷 <*卷名称*> 失败。 |卷创建作业失败。 <*与故障错误代码对应的错误消息*>。 |连接问题可能会导致无法成功完成空间创建操作。 本地固定卷是丰富预配的，创建空间的过滤涉及到将分层卷分层到云中。 如果连接没有问题，则可能是已用完设备上的本地空间。 在重试此操作之前，请确定设备上是否还有空间。 |
...
| <*卷组名称*的本地快照的本地空间消耗接近> |备份策略的本地快照可能很快就会耗尽空间，并且会失效以免主机写入失败。 |在与此备份策略组关联的卷中，频繁创建本地快照和数据频繁进出导致设备上的本地空间很快耗尽。 删除不再需要的所有本地快照。 此外，将此备份策略的本地快照计划更新为以较低的频率创建本地快照，并确保定期创建云快照。 如果不采取这些措施，这些快照的本地空间可能很快耗尽，系统会自动将其删除，以确保继续成功处理主机写入。 |
...
| 检测到“Data0”的 IP 地址重复。 | |系统检测到 IP 地址“10.0.0.1”冲突。 设备*\<设备上*的网络资源"Data0">处于脱机状态。 确保此网络中的其他任何实体未使用此 IP 地址。 若要排查网络问题，请转到[使用 Get-NetAdapter cmdlet 进行故障排除](storsimple-8000-troubleshoot-deployment.md#troubleshoot-with-the-get-netadapter-cmdlet)。 请联系网络管理员帮助解决此问题。 如果问题持续出现，请联系 Microsoft 支持。 |
| “Data0”的 IPv4（或 IPv6）地址已脱机。 | |设备 设备设备上的前缀长度*\<"22"1>* 处于脱机状态。 确保此接口连接到的交换机端口运行正常。 若要排查网络问题，请转到[使用 Get-NetAdapter cmdlet 进行故障排除](storsimple-8000-troubleshoot-deployment.md#troubleshoot-with-the-get-netadapter-cmdlet)。 |
| 无法连接到身份验证服务。 |数据路径错误 |用于身份验证的 URL 不可访问。 请确保防火墙规则包括为 StorSimple 设备指定的 URL 模式。 有关 Azure 门户中的 URL 模式的详细信息，请转到\/https：/aka.ms/ss-8000-network-reqs。 如果使用 Azure 政府云，请转到 https 中的\/URL 模式：/aka.ms/ss8000-gov-network-reqs。|
