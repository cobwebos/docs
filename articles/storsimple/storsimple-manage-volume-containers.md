...
* **加密** – 可以为每个卷容器定义的加密密钥。 此密钥用于对从 StorSimple 设备发送到云的数据进行加密。 军用级 AES 256 位密钥与用户输入的密钥配合使用。 为保护你的数据安全，建议你始终启用云存储加密。
* **存储帐户** – 关联到你的云存储服务提供商的存储帐户。 同一卷容器中的所有卷都共享此存储帐户。 可以从现有列表中选择一个存储帐户，也可以在创建卷容器时创建一个新帐户然后为该帐户指定访问凭据。
