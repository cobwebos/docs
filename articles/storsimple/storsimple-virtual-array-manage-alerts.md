---
title: 查看和管理 StorSimple 虚拟阵列的警报
...
为虚拟阵列启用电子邮件通知后，每次发生关键警报时，通知列表成员都将收到电子邮件消息。 消息将从 *"简单警报-无答复\@mail.windowsazure.com*发送，并将描述警报条件。 收件人可以单击“取消订阅”**** 将自己从电子邮件通知列表中删除。
...
3. 在“常规设置”**** 边栏选项卡中，转到“警报设置”**** 部分，并设置以下项：
   ...
   3. 在“其他电子邮件收件人”**** 字段中，输入应接收警报通知的所有其他收件人的电子邮件地址。 输入*某人\@somewhere.com*格式的名称。 使用分号分隔电子邮件地址。 对于每个虚拟设备，最多可以配置 20 个电子邮件地址。
...
* **警告** – 如果得不到解决，此类条件可能变成关键级别的。 应针对这种情况进行调查，并采取任何必要的措施来解决问题。
* **信息** – 此类警报包含可用于跟踪和管理你的系统的信息。
...
单击 **“清除”** 时，你有机会提供有关警报的评论，以及你采用的解决问题的步骤。
...
* **状态** – 你可以显示 **“活动”** 或 **“已清除”** 警报。 活动警报仍在系统上触发，而已清除的警报已由管理员手动清除，或由于系统以新信息更新警报条件而以编程方式清除。
* **严重性** – 你可以显示所有严重级别（关键、警告、信息）的警报，或者仅显示某个严重级别的警报，例如仅显示关键警报。
* **源** - 可以显示来自所有源的警报，或只显示来自服务或者某一个或所有虚拟设备的警报。
* **时间范围** – 通过指定 **“起始”** 和 **“结束”** 日期和时间戳，可以查看你感兴趣的时间段内的警报。
...
| 警报文本 | 事件 | 详细信息/建议的操作 |
|:--- |:--- |:--- |
| 设备 <设备名**> 未连接到云。 |已命名的设备无法连接到云。 |无法连接到云。 这可能由以下原因之一造成：<ul><li>设备上的网络设置可能出现问题。</li><li>存储帐户凭据可能出现问题。</li></ul>有关排查连接问题的详细信息，请转到设备的[本地 web UI](storsimple-ova-web-ui-admin.md)。 |
...
| 您正在耗尽<*设备名称*\>上的预配磁盘空间。 |磁盘空间警告。 |预配磁盘空间不足。 要释放空间，请考虑将工作负荷移动到另一个卷或共享，或者删除数据。 |
...
| 您<*设备名称*\>上的本地预留磁盘空间不足。 |响应时间较长。 |<*设备名称*\>的总预配大小的 10% 保留在本地设备上，而您现在在预留空间上运行不足。 <*设备名称*\>上的工作负载正在生成更高的改动率，或者您可能最近迁移了大量数据。 这可能导致性能降低。 若要解决此问题，请考虑采取以下措施之一：<ul><li>增加此设备的云带宽。</li><li>减少工作负荷或将工作负荷移到其他卷或共享。</li></ul> |

### <a name="security-alerts"></a>安全警报

| 警报文本 | 事件 | 详细信息/建议的操作 |
|:--- |:--- |:--- |
| <*设备名称*\>的密码将在<*天内*\>过期。 |密码警告。 |您的密码将在<*天内*\>过期。 请考虑更改密码。 有关详细信息，请转到 [更改 StorSimple 虚拟阵列设备管理员密码](storsimple-virtual-array-change-device-admin-password.md)。 |
