<properties 
   pageTitle="部署 StorSimple 设备 (Update 2) | Microsoft Azure"
...
# 部署本地 StorSimple 设备 (Update 2)

> [AZURE.SELECTOR]
- [Update 2](../articles/storsimple/storsimple-deployment-walkthrough-u2.md)
- [Update 1](../articles/storsimple/storsimple-deployment-walkthrough-u1.md)
- [版本公开上市](../articles/storsimple/storsimple-deployment-walkthrough.md)
...
- [解包、装载机架，并将电缆接到 8600](storsimple-8600-hardware-installation.md)

你需要有管理员权限才能完成安装和配置过程。建议你在开始之前查看配置清单。部署和配置过程可能需要一些时间才能完成。

> [AZURE.NOTE] Microsoft Azure 网站上发布的 StorSimple 部署信息仅适用于 StorSimple 8000 系列设备。如需 7000 系列设备的完整信息，请转到：[http://onlinehelp.storsimple.com/](http://onlinehelp.storsimple.com)。如需 7000 系列的部署信息，请参阅 [StorSimple 系统快速入门指南](http://onlinehelp.storsimple.com/111_Appliance/)。
...
以下部分说明了 StorSimple Manager 服务和 StorSimple 设备配置的先决条件。

### 对于 StorSimple Manager 服务
...
- StorSimple Manager 服务已启用你的 Microsoft Azure 订阅。应通过[企业协议](https://azure.microsoft.com/pricing/enterprise-agreement/)购买订阅。
...
> [AZURE.IMPORTANT] 如果未启用服务的自动创建存储帐户，则在成功创建服务后，将需要创建至少一个存储帐户。在创建卷容器时将会使用此帐户。
>
...
## 步骤 3︰通过 Windows PowerShell for StorSimple 配置和注册设备

使用 Windows PowerShell for StorSimple 来完成 StorSimple 设备的初始设置，如以下过程所述。你需要使用终端模拟软件来完成此步骤。如需更多信息，请参阅[使用 PuTTY 连接到设备串行控制台](#use-putty-to-connect-to-the-device-serial-console)。

[AZURE.INCLUDE [storsimple-configure-and-register-device-u1](../../includes/storsimple-configure-and-register-device-u1.md)]

## 步骤 4：完成最低版本的设备安装

有关 StorSimple 设备的最低版本设备配置，将需要：
...
> [AZURE.IMPORTANT] StorSimple Manager 可以创建既精简又完全预配的卷。但是不能创建部分预配的卷。
...
备份可提供卷的时间点保护，并可提高可恢复性，同时最大限度地减少恢复时间。你可以在 StorSimple 设备上执行两种类型的备份：本地快照和云快照。上述每种备份类型都可以是**计划**或**手动**的。
...
若要连接到 Windows PowerShell for StorSimple，需要使用终端模拟软件，如 PuTTY。你可以在访问设备时，直接通过串行控制台或从远程计算机上打开 telnet 会话来使用 PuTTY。

[AZURE.INCLUDE [使用 PuTTY 连接到设备串行控制台](../../includes/storsimple-use-putty.md)]
...
2.	已创建一个用于扫描可用更新的作业。如果更新可用，**扫描更新**则变更为**安装更新**。单击“**安装更新**”。

3.	将创建更新作业。通过导航到**作业**监视更新状态。

	> [AZURE.NOTE] 更新作业启动时，状态立即显示为 50%。仅在更新作业完成后，状态才会更改为 100%。更新过程没有实时状态。
...
请执行以下步骤，以获取正在运行 Windows Server® 2012 的 Windows 主机的 iSCSI 限定名称 (IQN)。

[AZURE.INCLUDE [创建手动备份](../../includes/storsimple-get-iqn.md)]

## 创建手动备份

请在管理门户执行以下步骤，以在 StorSimple 设备上为单独的卷创建一个按需手动备份。

[AZURE.INCLUDE [创建手动备份](../../includes/storsimple-create-manual-backup.md)]
...
- 使用 [StorSimple Manager 服务](storsimple-manager-service-administration.md)来管理 StorSimple 设备。
