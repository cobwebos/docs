...
ms.locfileid: "80299009"
---
# <a name="connect-remotely-to-your-storsimple-8000-series-device"></a>远程连接到 StorSimple 8000 系列设备

## <a name="overview"></a>概述

可以通过 Windows PowerShell 远程连接到设备。 采取这种方式连接时，不会看到菜单。 （仅当使用设备上的串行控制台进行连接时，才会看到菜单。通过 Windows PowerShell 远程处理，您可以连接到特定的运行空间。 也可以指定显示语言。
...
4. 现在可选择使用 HTTP 进行连接。 （默认值是通过 HTTPS 进行连接。确保选择了 HTTP。
...
3. 会收到使用 HTTP 连接到设备的安全漏洞的相关通知。 收到提示时，键入 **Y** 确认。
...
2. 在“安全设置”**** 边栏选项卡中，单击“远程管理”****。
3. 将 **“启用远程管理”** 设置为 **“是”**。
4. 现在可以选择使用 HTTPS 进行连接。 （默认值是通过 HTTPS 进行连接。确保选择了 HTTPS。
...
* [将 .cer 文件导入客户端或远程主机的根存储](#to-import-the-certificate-on-the-remote-host)。
...
1. 右键单击 .cer 文件，然后选择 **“安装证书”**。 这会启动证书导入向导。
   
    ![证书导入向导 1](./media/storsimple-remote-connect/HCS_CertificateImportWizard1.png)
2. 对于 **“存储位置”**，请选择 **“本地计算机”**，然后单击 **“下一步”**。
3. 选择“将所有证书放入下列存储”****，并单击“浏览”****。 导航至远程主机的根存储，然后单击 **“下一步”**。
...
3. 输入之前保存的设备序列号。 将此设备序列号映射到 IP 地址，如下图所示。 对于控制器 0 和控制器 1，请在序列号（CN 名称）末尾附加 **Controller0** 和 **Controller1**。
...
使用 Windows PowerShell 和 TLS 从远程主机或客户端在设备上输入 SSAdmin 会话。 SSAdmin 会话映射到设备的[串行控制台](storsimple-8000-windows-powershell-administration.md#connect-to-windows-powershell-for-storsimple-via-the-device-serial-console)菜单中的选项 1。

在想要从中建立远程 Windows PowerShell 连接的计算机上执行以下过程。

#### <a name="to-enter-an-ssadmin-session-on-the-device-by-using-windows-powershell-and-tls"></a>使用 Windows PowerShell 和 TLS 在设备上输入 SSAdmin 会话
1. 以管理员身份启动 Windows PowerShell 会话。 如果使用 Windows 10 客户端，默认情况下，Windows 远程管理服务设置为手动启动。 可能需要通过键入以下命令启动该服务：
...
6. 您需要等待几分钟，然后通过 TLS 通过 HTTPS 连接到您的设备。 会看到一条指示已连接到设备的消息。
   
    ![使用 HTTPS 和 TLS 进行 PowerShell 远程处理](./media/storsimple-remote-connect/HCS_PSRemotingUsingHTTPSAndSSL.png)
