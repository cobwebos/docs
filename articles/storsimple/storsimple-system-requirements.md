...
| --- | --- | --- | --- | --- |
| TCP 80 (HTTP)<sup>3</sup> |出 |WAN |否 |<ul><li>出站端口用于 Internet 访问以检索更新。</li><li>出站 Web 代理由用户配置。</li><li>若要允许系统更新，还必须为控制器的固定 IP 打开此端口。</li></ul> |
| TCP 443 (HTTPS)<sup>3</sup> |出 |WAN |是 |<ul><li>出站端口用于访问云中的数据。</li><li>出站 Web 代理由用户配置。</li><li>若要允许系统更新，还必须为控制器的固定 IP 打开此端口。</li><li>此端口还可以在执行垃圾回收的两个控制器中使用。</li></ul> |
...
| 5985 |In |LAN |否 |StorSimple Snapshot Manager 使用入站端口与 StorSimple 设备进行通信。<br>通过 HTTP 远程连接到 Windows PowerShell for StorSimple 时也能使用此端口。 |
...
路由跃点数与将数据路由到指定网络的接口和网关相关联。 如果路由协议了解到存在到达同一目标的多个路径，它会使用路由跃点数来计算给定目标的最佳路径。 路由度量越低，优先级越高。

在 StorSimple 的上下文中，如果配置多个网络接口和网关以传送通道流量，路由跃点数将发挥作用来确定使用这些接口的相对顺序。 用户不能更改路由跃点数。 但是，可以使用 `Get-HcsRoutingTable` cmdlet 在 StorSimple 设备上打印出路由表（和跃点数）。 了解[解决 StorSimple 部署](storsimple-troubleshoot-deployment.md)中有关 Get-HcsRoutingTable cmdlet 的详细信息。
...
**从 Update 1 开始的版本以及 Update 2 之前的版本**

这包括软件版本 1、1.1、1.2 等。 确定基于路由跃点数的顺序的方法如下所示：

   *DATA 0 > 上次配置的 10 GbE 网络接口 > 其他 10 GbE 网络接口 > 上次配置的 1 GbE 网络接口 > 其他 1 GbE 网络接口*

   在 Update 1 中，DATA 0 的路由跃点数最低；因此，所有云流量都通过 DATA 0 路由。 如果 StorSimple 设备上有多个启用云的网络接口，请记录下来。

**从 Update 2 开始的版本**

Update 2 已进行几项与网络有关的改进，路由跃点数也已更改。 该行为如下所述。

* 已分配给网络接口一组预先确定的值。     
* 请考虑如下所示的示例表，其中列出了在各种网络接口已启用云或禁用云但具有配置网关时向它们分配的值。 请注意此处分配的值只是示例值。
...
* 通过网络接口路由云流量的顺序是：
    ...
    *Data 0 > Data 1 > Date 2 > Data 3 > Data 4 > Data 5*
    ...
    请考虑具有两个启用云的网络接口 Data 0 和 Data 5 的 StorSimple 设备。 Data 1 到 Data 4 是禁用云但具有配置网关的接口。 该设备路由的流量顺序是：
...
* 如果启用 iSCSI 的网络接口发生 VIP 失败，将执行 3 次重试，其中有 2 秒的延迟。 从以前版本开始此行为保持不变。 如果所有的 iSCSI 网络接口均失败，会发生控制器故障转移（伴随有重新启动）。
* 发生 VIP 失败时，StorSimple 设备上也会引发警报。 有关详细信息，请转到[警报快速参考](storsimple-manage-alerts.md)。
* 对于重试，iSCSI 优先于云。
  
    请考虑下面的示例：StorSimple 设备已启用两个网络接口，即 Data 0 和 Data 1。 Data 0 是启用云的接口，而 Data 1 既启用云又启用了 iSCSI。 此设备上没有为云或 iSCSI 启用其他网络接口。
  
    如果 Data 1 失败，假定它是最后一个 iSCSI 网络接口，会导致将控制器故障转移到另一个控制器上的 Data 1。

### <a name="networking-best-practices"></a>网络最佳实践
除上述网络要求外，为了获得 StorSimple 解决方案的最佳性能，请遵循以下最佳实践：

* 确保 StorSimple 设备具有始终可用的、专用的 40 Mbps 带宽（或更高带宽）。 此带宽不应（或应确保通过使用 QoS 策略分配）与任何其他应用程序共享。
* 确保到 Internet 的网络连接始终可用。 到设备的偶发性或不可靠 Internet 连接（包括无 Internet 连接）将导致不支持的配置。
* 通过在设备上使用专用的网络接口来访问 iSCSI 和云，从而隔离 iSCSI 和云流量。 有关详细信息，请参阅如何在 StorSimple 设备上[修改网络接口](storsimple-modify-device-config.md#modify-network-interfaces)。
...
* 如果 PCM 发生故障，立即请求更换。
* 只有在进行更换并准备好安装时，才能移除发生故障的 PCM。
* 不要同时移除两个 PCM。 PCM 模块包括备用电池模块。 同时移除两个 PCM 将导致在没有电池保护的情况下关机，这不会保存设备状态。 有关电池的详细信息，请转到[维护备用电池模块](storsimple-battery-replacement.md#maintain-the-backup-battery-module)。

#### <a name="controller-modules"></a>控制器模块
StorSimple 设备包括冗余的热插拔控制器模块。 控制器模块以主动/被动方式运行。 在任何给定的时间，一个控制器模块处于主动状态并且正在提供服务，而另一个控制器模块就会处于被动状态。 如果主动控制器模块发生故障或被移除，被动控制器模块会通电并开始运行。 每个控制器模块都有足够的容量，能够为整个机箱提供服务。 必须安装两个控制器模块以确保高可用性。

* 请确保始终都安装有两个控制器模块。
* 如果控制器模块发生故障，立即请求更换。
* 只有在进行更换并准备好安装时，才能移除发生故障的控制器模块。 移除模块所需时间过长的话，将会影响气流，进而影响系统冷却。
* 请确保到两个控制器模块的网络连接相同并且连接的网络接口具有相同的网络配置。
* 如果控制器模块发生故障或需要更换，在更换发生故障的控制器模块之前，请确保另一个控制器模块处于主动状态。 若要验证控制器是否处于主动状态，请转到[识别设备上的主动控制器](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device)。
...
* 如果 EBOD 机箱控制器模块发生故障，请确保在更换发生故障的模块之前另一个控制器模块处于主动状态。 若要验证控制器是否处于主动状态，请转到[识别设备上的主动控制器](storsimple-controller-replacement.md#identify-the-active-controller-on-your-device)。
