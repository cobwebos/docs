<properties 
   pageTitle="部署 StorSimple 设备 (Update 1) | Microsoft Azure"
   description="介绍部署 StorSimple Update 1 设备和服务的步骤和最佳实践。"
   services="storsimple"
...
# 部署本地 StorSimple 设备 (Update 1)

> [AZURE.SELECTOR]
- [Update 2](../articles/storsimple/storsimple-deployment-walkthrough-u2.md)
- [Update 1](../articles/storsimple/storsimple-deployment-walkthrough-u1.md)
- [正式版](../articles/storsimple/storsimple-deployment-walkthrough.md)

## 概述

欢迎使用 Microsoft Azure StorSimple 设备部署。本部署教程适用于 StorSimple 8000 系列 Update 1.0。本系列教程说明如何配置 StorSimple 设备，并包含配置清单、配置先决条件和详细配置步骤。
...
> [AZURE.NOTE] Microsoft Azure 网站上发布的 StorSimple 部署信息仅适用于 StorSimple 8000 系列设备。有关 5000 和 7000 系列设备的完整信息，请转到：[http://onlinehelp.storsimple.com/](http://onlinehelp.storsimple.com)。有关 5000 和 7000 系列的部署信息，请参阅 [StorSimple System Quick Start Guide](http://onlinehelp.storsimple.com/111_Appliance/)（StorSimple 系统快速入门指南）。
...
| | |
| **逐步部署** | 需要完成这些步骤才能在生产中部署 StorSimple 设备。 |
...
| 步骤 4：完成最低版本的设备安装</br>可选：更新 StorSimple 设备。 | 使用管理服务来完成设备安装，并启用以使其能够提供存储。 |
...
| | | | |
| **配置和注册设备** | Data 0 网络设置 | Data 0 IP 地址：</br>子网掩码：</br>网关：</br>主要 DNS 服务器：</br>主要 NTP 服务器：</br>Web 代理服务器 IP/FQDN（可选）：</br>Web 代理端口：| |
...
| | 服务数据加密密钥 | 当设备通过 Windows PowerShell for StorSimple 注册管理服务时将创建此密钥。复制此密钥，并将它存储在安全的位置。| |
| | | | |
...
使用 Windows PowerShell for StorSimple 来完成 StorSimple 设备的初始设置，如以下过程所述。需要使用终端模拟软件来完成此步骤。如需更多信息，请参阅[使用 PuTTY 连接到设备串行控制台](#use-putty-to-connect-to-the-device-serial-console)。
...
卷容器具有其中所包含的所有卷的存储帐户、带宽和加密设置。需要创建卷容器后才能开始在 StorSimple 设备上预配卷。
...
> [AZURE.IMPORTANT] StorSimple Manager 只能创建既精简预配的卷。无法创建完全预配或部分预配的卷。
...
> [AZURE.NOTE] StorSimple 虚拟设备不支持 MPIO。
