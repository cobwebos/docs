...
本文介绍 StorSimple 与 Veeam 的集成，以及集成这两种解决方案时的最佳实践。 此外，还提供了有关如何设置 Veeam，以实现与 StorSimple 的最佳集成的建议。 我们将遵循 Veeam 最佳实践以及备份架构师与管理员的意见，最合理地设置 Veeam，旨在满足个人的备份要求和服务级别协议 (SLA)。

本文虽然介绍了配置步骤和重要概念，但并未提供循序渐进的配置或安装指南。 本文假设基本组件与基础结构工作正常，随时能够支持本文中所述的操作。

### <a name="who-should-read-this"></a>目标读者

本文中的信息主要面向具有存储、Windows Server 2012 R2、以太网、云服务和 Veeam 相关知识的备份管理员、存储管理员和存储架构师。
...
**StorSimple 分层**
![Stor 简单分层图](./media/storsimple-configure-backup-target-using-veeam/image1.jpg)
...
有关 Veeam 的安装最佳实践，请参阅 [Veeam 备份和复制最佳实践](https://bp.veeam.expert/)，并阅读 [Veeam 帮助中心（技术文档）](https://www.veeam.com/documentation-guides-datasheets.html)中的用户指南。
...
- 不要使用跨区卷（由 Windows 磁盘管理创建）。 跨区卷不受支持。
...
### <a name="veeam-best-practices"></a>Veeam 最佳实践
...
-   关闭作业验证。 如果需要，应在最新的备份作业之后计划验证。 必须知道，此作业会影响备份持续时间。
-   打开媒体预分配。
-   确保打开并行处理。
-   关闭压缩。
-   关闭备份作业中的重复数据删除。
...
### <a name="assign-storsimple-volumes-to-a-veeam-backup-job"></a>将 StorSimple 卷分配到 Veeam 备份作业

对于主备份目标方案，请创建包含主 Veeam StorSimple 卷的每日备份作业。 对于辅助备份目标方案，请使用直接附加存储 (DAS)、网络附加存储 (NAS) 或简单磁盘捆绑 (JBOD) 存储创建每日备份作业。
...
8.  将计划设置为在每日的指定时间运行一次。

    ![Veeam 管理控制台，新建备份作业 - 计划页](./media/storsimple-configure-backup-target-using-veeam/veeamimage15.png)
...
### <a name="assign-storsimple-volumes-to-a-veeam-copy-job"></a>将 StorSimple 卷分配到 Veeam 复制作业
...
- 对于[SQL 服务器](https://www.veeam.com/microsoft-sql-server-explorer.html)
- 对于[共享点](https://www.veeam.com/microsoft-sharepoint-recovery-explorer.html)
...
| Veeam 服务器故障 | 备份和还原操作会中断。 | 重建备份服务器并执行数据库还原，如[Veeam 帮助中心（技术文档）中详细说明的那样](https://www.veeam.com/documentation-guides-datasheets.html)。  | 必须在灾难恢复站点重建或还原 Veeam 服务器。 将数据库还原到最近的时间点。 如果还原的 Veeam 数据库未与最新的备份作业同步，则需要编制索引和目录。 这种索引和目录重新扫描过程可能会导致扫描所有备份集并将其从云层提取到本地设备层。 这会进一步消耗时间。 |
