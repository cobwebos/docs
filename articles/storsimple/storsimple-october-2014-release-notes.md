...
| 2 |支持包 |在以前的版本，如果在运行 Windows PowerShell **Export-hcssupportpackage** cmdlet 来检索基板管理控制器 (BMC) 记录时，操作失败，发生以下警告:"该操作成功在此控制器上，但由于以下错误对等控制器上失败。 请检查对等节点是否正常以及当前节点是否可以连接到对等节点。“ 此问题现在已修复。 |是 |否 |
...
| 6 |固件更新 |在以前的发行版中，设备固件更新作业失败并且显示一个错误，指出 cmdlet 不可识别，因此更新失败。 控制器会进入恢复模式。 此问题现在已修复。 |是 |否 |
