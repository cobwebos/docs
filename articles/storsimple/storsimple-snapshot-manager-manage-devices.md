...
1. 要将设备连接到 StorSimple Snapshot Manager，请按照“添加或替换设备”中的说明进行操作。 连接到设备后，StorSimpleSnapshot Manager 会做出如下响应：
   ...
   * 如果设备可用，则 StorSimple Snapshot Manager 会添加一个连接。 选择该设备后，它会显示在“结果”窗格中，而且状态字段指示该设备为“可用”。 StorSimple Snapshot Manager 会导入为该设备配置的任何卷组，前提是这些卷组具有关联的备份。 备份策略将不导入。 没有关联备份的卷组将不被导入。
