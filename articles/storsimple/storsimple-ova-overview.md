...
Microsoft Azure StorSimple 虚拟阵列是一个集成式存储解决方案，用于管理虚拟机监控程序中运行的本地虚拟阵列与 Microsoft Azure 云存储之间的存储任务。 虚拟阵列是一种经济高效、易于管理的文件服务器或 iSCSI 服务器解决方案，可以消除与企业存储和数据保护相关的很多问题和开支。 虚拟阵列尤其适合远程办公室/分支机构方案。

本主题提供虚拟阵列的概述 - 下面是一些其他资源：
...
| 卷大小 |分层：500 GB 到 5 TB；本地固定：50 GB 到 500 GB |
| 卷大小 |分层：最多 5 TB；本地固定：最多 500 GB |
| 快照 |崩溃一致 |
| 项目级恢复 |是；用户可从共享还原 |
...
|方案     |工作负载     |支持      |限制               |
|-------------|-------------|---------------|---------------------------|
|ROBO 协作 |文件共享     |是      |请参阅[文件服务器的最大限制](storsimple-ova-limits.md)。<br></br>请参阅[受支持的 SMB 版本的系统要求](storsimple-ova-system-requirements.md)。| 所有版本     |
...
保证特定分层共享或卷的数据具有其自己的本地层空间。 （该共享或卷的总预配空间的大约 10%）。 尽管这降低了该共享或卷所在的虚拟阵列上的可用存储，但它确保了单个共享或卷的分层不会受到其他共享或卷的分层需求的影响。 因此一个共享或卷上的非常繁忙的工作负荷无法将所有其他工作负荷强制施加到云。 
