...
* 还原需求。 如果以新操作的形式执行还原，则调整大小时应考虑还原所需的空间。 还原到的目标是相同大小或更大的共享或卷。
* 应分配一些缓冲区来应对意外增长。
...
* 虚拟阵列上最多可以创建 16 个卷/共享。 如果固定在本地，卷/共享容量可以介于 50 GB 和 2 TB 之间。 如果分层，卷/共享容量必须介于 500 GB 和 20 TB 之间。 
* 创建卷时，请将预期的数据使用量以及将来的增长量包含考虑。 尽管卷以后无法扩展，但始终可以还原到更大的卷。
* 创建卷后，无法缩小 StorSimple 上的卷大小。
* 在写入 StorSimple 上的分层卷时，如果卷数据达到特定的阈值（相对于保留给卷的本地空间），IO 将受到限制。 继续写入此卷会让 IO 明显变慢。 尽管可以写入已超过预配容量的分层卷（系统不会主动阻止用户写入超出预配容量的数据），但会看到警报通知，指出已超过订阅量。 如果看到该警报，请务必采取补救措施，例如删除卷数据，或者将卷还原到更大的卷（目前不支持扩展卷）。
