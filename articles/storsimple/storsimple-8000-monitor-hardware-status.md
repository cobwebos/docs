...
| Metis |不适用 |逻辑运算 |共享 |不适用 |显示电池的状态：它们是否需要充电以及是否已接近报废。 |
| 群集 |不适用 |逻辑运算 |共享 |不适用 |显示在两个集成的控制器模块之间创建的群集的状态。 |
| 群集节点 |不适用 |逻辑运算 |共享 |不适用 |指示属于群集的一部分的控制器的状态。 |
| 群集仲裁 |不适用 |逻辑运算 | |不适用 |指示 HDD 存储池中存在多数磁盘成员关系。 |
| HDD 数据空间 |不适用 |逻辑运算 |共享 |不适用 |硬盘驱动器 (HDD) 存储池中用于数据的存储空间。 |
| HDD 管理空间 |不适用 |逻辑运算 |共享 |不适用 |HDD 存储池中保留的用于管理任务的空间。 |
| HDD 仲裁空间 |不适用 |逻辑运算 |共享 |不适用 |HDD 存储池中保留的用于群集仲裁的空间。 |
| HDD 更换空间 |不适用 |逻辑运算 |共享 |不适用 |HDD 存储池中保留的用来更换控制器的空间。 |
| SSD 数据空间 |不适用 |逻辑运算 |共享 |不适用 |固态驱动器 (SSD) 存储池中用于数据的存储空间。 |
| SSD NVRAM 空间 |不适用 |逻辑运算 |共享 |不适用 |SSD 存储池中专用于 NVRAM 逻辑的存储空间。 |
| HDD 存储池 |不适用 |逻辑运算 |共享 |不适用 |显示基于设备 HDD 创建的逻辑存储池的状态。 |
| SSD 存储池 |不适用 |逻辑运算 |共享 |不适用 |显示基于设备 SSD 创建的逻辑存储池的状态。 |
...
| 电源 [0-1] |PCM |物理 |共享 |是 |设备背部的两个 PCM 中的每个电源都存在对应的一行。 |
| 冷却 [0-1] |PCM |物理 |共享 |是 |两个 PCM 中的四个冷却风扇中每个都存在对应的一行。 |
| 本地存储 [HDD] |不适用 |逻辑运算 |共享 |不适用 |显示基于设备 HDD 创建的逻辑存储池的状态。 |
