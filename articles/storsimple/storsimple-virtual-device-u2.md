<properties 
   pageTitle="StorSimple 虚拟设备 Update 2 | Microsoft Azure"
...
# 在 Azure 中部署和管理 StorSimple 虚拟设备
...
|-----------------------|---------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| **最大容量** | 30 TB | 64 TB |
| **Azure VM** | Standard\_A3（4 核，7 GB 内存） | Standard\_DS3（4 核，14 GB 内存） |
...
| **存储类型** | 为本地磁盘使用 Azure 标准存储<br></br>了解如何[创建标准存储帐户]() | 为本地磁盘使用 Azure 高级存储<sup>2</sup> <br></br>了解如何[创建高级存储帐户](storage-premium-storage.md#create-and-use-a-premium-storage-account-for-a-virtual-machine-data-disk) |
...
- 使用与虚拟设备位于同一区域中的[存储帐户](storsimple-manage-storage-accounts.md#add-a-storage-account)。使用不同区域中的存储帐户可能导致性能不佳。可以配合虚拟设备使用标准或高级存储帐户。如何创建[标准存储帐户]()或[高级存储帐户](storage-premium-storage.md#create-and-use-a-premium-storage-account-for-a-virtual-machine-data-disk)的详细信息
...
[AZURE.INCLUDE [创建虚拟设备](../../includes/storsimple-create-virtual-device-u2.md)]
...
执行以下步骤配置和注册 StorSimple 虚拟设备。[AZURE.INCLUDE [配置和注册虚拟设备](../../includes/storsimple-configure-register-virtual-device.md)]
...
>[AZURE.NOTE] 对虚拟设备而言，Windows 主机就是 Azure 虚拟机。
...
[AZURE.INCLUDE [通过 HTTP 为虚拟设备配置远程管理](../../includes/storsimple-configure-remote-management-http-device.md)]
...
>[AZURE.WARNING] **为了增强安全性，强烈建议在连接到终结点时使用 HTTPS，并在完成 PowerShell 远程会话之后删除终结点。**
...
[AZURE.INCLUDE [在虚拟设备上创建公共终结点](../../includes/storsimple-create-public-endpoints-virtual-device.md)]
...
创建并配置 StorSimple 虚拟设备后，可以开始使用它。可以在虚拟设备上使用卷容器、卷和备份策略，如同在物理 StorSimple 设备上所做的一样。唯一差别在于，必须确保从设备列表中选择虚拟设备。有关虚拟设备的各种管理任务的分步过程，请参阅 [use the StorSimple Manager service to manage a virtual device](storsimple-manager-service-administration.md)（使用 StorSimple Manager 服务管理虚拟设备）。
...
- **软件更新** - 可以查看上次更新软件的日期，以及任何更新状态消息。若要检查是否有新的更新，可以使用页面底部的“扫描更新”按钮来执行手动扫描。如果有可用的更新，请单击“安装更新”进行安装。虚拟设备上只有一个接口，这意味着在应用更新时会造成轻微的服务中断。虚拟设备将关闭并重新启动（如果需要），以应用任何已发布的更新。有关分步过程，请转到[更新设备](storsimple-update-device.md#install-regular-updates-via-the-azure-classic-portal)。
...
[AZURE.INCLUDE [停止和重新启动虚拟设备](../../includes/storsimple-stop-restart-virtual-device.md)]
...
[AZURE.INCLUDE [关闭虚拟设备](../../includes/storsimple-shutdown-virtual-device.md)]

[AZURE.INCLUDE [删除虚拟设备](../../includes/storsimple-delete-virtual-device.md)]
