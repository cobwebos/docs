...
StorSimple Snapshot Manager 的主要功能是允许你以快照的形式创建 StorSimple 卷的应用程序一致备份副本。 然后在一个称为*备份目录*的 XML 文件中列出快照。 备份目录按卷组组织快照，然后按本地快照或云快照对其进行组织。 
...
可通过展开“**作用域**”窗格内“**备份目录**”中的节点查看备份目录 ，然后展开卷组。

* 如果单击卷组名称，“**结果**”窗格将显示可用于卷组的本地快照和云快照数目。 
* 如果单击“**本地快照**”或“**云快照**”，“**结果**”窗格会显示以下有关每个备份快照的信息（具体取决于“**视图**设置”）： 
  ...
  * “**导入**”– 是否已导入备份。 “**True**”指示在 StorSimple Snapshot Manager 中配置该设备时，已从 StorSimple Manager 服务导入备份；“**False**”指示备份未导入，但已由 StorSimple Snapshot Manager 创建。 （可以轻松地识别导入的卷组，因为添加的后缀可以标识导入的卷组来自哪个设备。）
    
    ![备份目录](./media/storsimple-snapshot-manager-manage-backup-catalog/HCS_SSM_Backup_catalog.png)
* 如果展开“**本地快照**”或“**云快照**”，然后单击单个快照名称，“**结果**” 窗格会显示以下有关所选快照的信息：
...
如果尚未进行操作，则创建一个卷和卷组，然后删除该卷。 默认情况下，StorSimple Snapshot Manager 在允许删除卷之前会对其进行备份。 如果无意中删除卷或出于某种原因需要对数据进行恢复，则此预防措施可以避免数据丢失。 
...
2. 在“**作用域**”窗格中，展开“**备份目录**”节点，展开卷组，然后单击“**本地快照**”或“**云快照**”。 备份快照的列表将在“**结果**”窗格中显示。 
3. 找到要还原的备份，右键单击，然后单击“**还原**”。 
   
    ![还原备份目录](./media/storsimple-snapshot-manager-manage-backup-catalog/HCS_SSM_Restore_BU_catalog.png) 
4. 在“确认”页上查看详细信息，键入“**确认**”，然后单击“**确定**”。 StorSimple Snapshot Manager 使用备份来还原卷。 
   
    ![还原确认消息](./media/storsimple-snapshot-manager-manage-backup-catalog/HCS_SSM_Restore_volume_msg.png) 
5. 可以在还原操作允许时对其进行监视。 在“**作用域**”窗格中，展开“**作业**”节点，然后单击“**正在运行**”。 作业详细信息将在“**结果**”窗格中显示。 还原作业完成后，作业详细信息会传输到“**过去 24 小时**”列表。
...
1. 单击桌面图标启动 StorSimple Snapshot Manager。
2. 在“**作用域**”窗格中，展开“**备份目录**”节点，展开卷组，然后单击“**云快照**”。 备份列表将在“**结果**”窗格中显示。
3. 找到要克隆的卷或卷组，右键单击卷或卷组名称，然后单击“**克隆**”。 “**克隆云快照**”对话框随即出现。
   
    ![克隆云快照](./media/storsimple-snapshot-manager-manage-backup-catalog/HCS_SSM_Clone.png) 
4. 如下所示，完成“**克隆云快照**”对话框： 
   
   1. 在“**名称**”文本框中，键入克隆的卷名称。 此名称将在“**卷**”节点中显示。 
   2. （可选）选择“**驱动器**”，然后从下拉列表中选择驱动器号。 
   3. （可选） 选择“**文件夹 (NTFS)**”，键入文件夹路径或单击“浏览”并选择该文件夹的位置。 
   4. 单击“创建” 。
5. 完成克隆过程后，必须初始化克隆的卷。 启动服务器管理器，然后启动磁盘管理。 有关详细说明，请参阅 [装载卷](storsimple-snapshot-manager-manage-volumes.md#mount-volumes)。 将其初始化之后，该卷将在“**作用域**”窗格内的“**卷**”节点中列出。  如果看不到列出的卷，刷新卷列表（右键单击“**卷**”节点，然后单击“**刷新**”）。
...
1. 单击桌面图标启动 StorSimple Snapshot Manager。
2. 在“**作用域**”窗格中，展开“**备份目录**”节点，展开卷组，然后单击“**本地快照**”或“**云快照**”。 快照列表将在“**结果**”窗格中显示。 
3. 右键单击要删除的快照，然后单击“**删除**”。
4. 在出现的确认消息中单击“**确认**”。 

## <a name="recover-a-file"></a>恢复文件
如果文件被意外从卷中删除，可以通过以下方法恢复文件：检索删除日期之前的快照，使用该快照创建克隆卷，然后将该文件从克隆卷复制到原来的卷。

#### <a name="prerequisites"></a>先决条件
在开始之前，请确保你具有卷组的最新备份。 然后，删除存储在该卷组中一个卷上的某个文件。 最后，使用以下步骤从备份中还原已删除的文件。 

#### <a name="to-recover-a-deleted-file"></a>恢复已删除的文件
1. 单击桌面上的 StorSimple Snapshot Manager 图标。 StorSimple Snapshot Manager 控制台窗口随即出现。 
2. 在“**作用域**”窗格中，展开“**备份目录**”节点，然后浏览到包含已删除文件的快照。 通常情况下，应选择在删除前不久创建的快照。 
3. 找到要克隆的卷，右键单击，然后单击“**克隆**”。 “**克隆云快照**”对话框随即出现。
...
4. 如下所示，完成“**克隆云快照**”对话框： 
   ...
   3. （可选） 选择“**文件夹 (NTFS)**”，键入文件夹路径或单击“**浏览**”并选择该文件夹的位置。 
   4. 单击“创建” 。 
5. 完成克隆过程后，必须初始化克隆的卷。 启动服务器管理器，然后启动磁盘管理。 有关详细说明，请参阅 [装载卷](storsimple-snapshot-manager-manage-volumes.md#mount-volumes)。 将其初始化之后，该卷将在“**作用域**”窗格内的“**卷**”节点中列出。 
   
     如果看不到列出的卷，刷新卷列表（右键单击“**卷**”节点，然后单击“**刷新**”）。
6. 打开包含克隆卷的 NTFS 文件夹，展开“**卷**”节点，然后再打开克隆卷。 找到要恢复的文件，并将其复制到主卷。
