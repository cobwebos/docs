---
title: 用作备份目标的 StorSimple 8000 系列与 Backup Exec 的集成 | Microsoft 文档
...
本文介绍 StorSimple 与 Veritas Backup Exec 的集成，以及集成这两种解决方案时的最佳实践。 此外，还提供了有关如何设置 Backup Exec，以实现与 StorSimple 的最佳集成的建议。 我们将遵循 Veritas 最佳实践以及备份架构师与管理员的意见，最合理地设置 Backup Exec，旨在满足个人的备份要求和服务级别协议 (SLA)。

本文虽然介绍了配置步骤和重要概念，但并未提供循序渐进的配置或安装指南。 本文假设基本组件与基础结构工作正常，随时能够支持本文中所述的操作。

### <a name="who-should-read-this"></a>目标读者

本文中的信息主要面向具有存储、Windows Server 2012 R2、以太网、云服务和 Backup Exec 相关知识的备份管理员、存储管理员和存储架构师。

## <a name="supported-versions"></a>支持的版本

-   [Backup Execc 16 和更高版本](https://www.veritas.com/content/support/en_US/article.100040087)
...
由于 StorSimple 是与 Azure 云集成的解决方案，因此 StorSimple 需要与 Azure 云建立有效的连接。 此连接用于如下所述的操作：云快照、管理、元数据传输，以及在 Azure 云存储中将不经常访问的旧数据分层。

为使该解决方案保持最佳性能，我们建议遵循以下网络最佳实践：

-   用于将 StorSimple 层连接到 Azure 的链路必须满足带宽要求。 为此，请向基础结构交换机应用必要的服务质量 (QoS) 级别，使之与 RPO 和恢复时间目标 (RTO) SLA 相符。
...
- 不要使用跨区卷（由 Windows 磁盘管理创建）。 跨区磁盘不受支持。
- 使用 NTFS 和 64-KB 分配大小格式化卷。
- 将 StorSimple 卷直接映射到 Backup Exec 服务器。
    - 为物理服务器使用 iSCSI。
    - 为虚拟服务器使用直通磁盘。

## <a name="best-practices-for-storsimple-and-backup-exec"></a>StorSimple 和 Backup Exec 的最佳实践
...
- 在任务管理器中关闭默认的[Windows Server 维护](https://msdn.microsoft.com/library/windows/desktop/hh848037.aspx)。 通过以下方式之一执行此操作：
  ...
  - 通过 Windows Sysinternals 下载 [PsExec](https://technet.microsoft.com/sysinternals/bb897553.aspx)。 下载 PsExec 后，以管理员身份运行 Azure PowerShell，键入：
    ```powershell
...
### <a name="backup-exec-best-practices"></a>Backup Exec 最佳实践

-   Backup Exec 必须安装在服务器的本地驱动器上，而不能安装在 StorSimple 卷上。
-   将 Backup Exec 存储的**并发写入操作**设置为允许的最大值。
    -   将 Backup Exec 存储的**块和缓冲区大小**设置为 512 KB。
    -   打开 Backup Exec 存储的**缓冲读取和写入**。
...
最常用的备份保留策略类型之一为祖父、父和子 (GFS) 策略。 在 GSF 策略中，增量备份每日执行，完整备份每周和每月执行。 此策略会导致生成六个 StorSimple 分层卷。 一个卷包含每周、每月和每年完整备份。 另外五个卷存储每日增量备份。
...
## <a name="set-up-backup-exec-storage"></a>设置 Backup Exec 存储
...
7.  每次分配卷结束时，请根据 [StorSimple 和 Backup Exec 的最佳实践](#best-practices-for-storsimple-and-backup-exec)中的建议更改存储设备设置。

    ![Backup Exec 管理控制台，存储设备设置页](./media/storsimple-configure-backup-target-using-backup-exec/image12.png)

8.  重复步骤 1-7，直到完成向 Backup Exec 分配 StorSimple 卷。
...
### <a name="assign-storsimple-volumes-to-a-backup-exec-backup-job"></a>将 StorSimple 卷分配到 Backup Exec 备份作业
...
### <a name="assign-storsimple-volumes-to-a-backup-exec-archive-and-deduplication-job"></a>将 StorSimple 卷分配到 Backup Exec 存档和重复数据删除作业
...
6.  选择“确定”  。
    ...
    ![显示备份定义属性的屏幕截图。](./media/storsimple-configure-backup-target-using-backup-exec/image24.png)
...
> [!NOTE]
> 在删除 StorSimple 快照之前，请仔细评估造成的合规性与数据保留后果。 有关如何运行备份后脚本的详细信息，请参阅 [Backup Exec 文档](https://www.veritas.com/support/en_US/article.100032497.html)。
...
1. [安装 Azure PowerShell](/powershell/azure/)。
...
3. 在运行脚本的服务器上，以管理员身份运行 PowerShell。 确保运行脚本时使用 `-WhatIf $true`，以查看该脚本会导致哪些变化。 完成验证后，立即传递 `-WhatIf $false`。 运行下列命令：
   ...
   ```
4. 通过编辑 Backup Exec 作业选项的前处理-后处理命令，将脚本添加到 Backup Exec 中的备份作业。

   ![Backup Exec 控制台，备份选项，前处理和后处理命令选项卡](./media/storsimple-configure-backup-target-using-backup-exec/image25.png)
...
从 StorSimple 设备还原的过程类似于从任何块存储设备还原。 还原已在云中分层的数据以云的速度进行。 对于本地数据，还原以设备本地磁盘的速度进行。 有关如何执行还原的信息，请参阅 Backup Exec 文档。 我们建议遵循 Backup Exec 还原最佳实践。
...
| 站点发生故障，导致备份服务器和 StorSimple 丢失 | 备份和还原操作会中断。 | 首先还原 StorSimple，然后还原 Backup Exec。 | 首先还原 StorSimple，然后还原 Backup Exec。 如果在恢复设备后需要执行还原，则需要将云中的完整工作集检索到新设备。 所有操作都以云的速度进行。 |
