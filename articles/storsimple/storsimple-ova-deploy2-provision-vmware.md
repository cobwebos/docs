---
title: "部署 StorSimple 虚拟阵列 - 在 VMware 中预配"
...
---
# <a name="deploy-storsimple-virtual-array---provision-a-virtual-array-in-vmware"></a>部署 StorSimple 虚拟阵列 - 在 VMware 中预配虚拟阵列
![](./media/storsimple-ova-deploy2-provision-vmware/vmware4.png)

## <a name="overview"></a>概述
本预配教程适用于运行 2016 年 3 月公开发行 (GA) 版的 StorSimple 虚拟阵列（也称 StorSimple 本地虚拟设备或 StorSimple 虚拟设备）。 本教程介绍如何在运行 VMware ESXi 5.5 及更高版本的主机系统上预配和连接 StorSimple 虚拟阵列。 本文适用于在 Azure 经典门户和 Microsoft Azure 政府云中部署 StorSimple 虚拟阵列的情况。

需要管理员权限才能预配和连接虚拟设备。 完成预配和初始设置可能需要大约 10 分钟。

## <a name="provisioning-prerequisites"></a>预配先决条件
下面是在运行 VMware ESXi 5.5 及更高版本的主机系统上预配虚拟设备的先决条件。
...
* 已从 Azure 门户下载 VMware 的虚拟设备映像。 有关详细信息，请参阅[步骤 3：下载虚拟设备映像](storsimple-ova-deploy1-portal-prep.md#step-3-download-the-virtual-device-image)。
...
* 已查看部署 StorSimple 虚拟设备的网络要求，并已根据要求配置数据中心网络。 有关详细信息，请参阅 [StorSimple 虚拟阵列系统要求](storsimple-ova-system-requirements.md)。
...
1. 复制系统中的虚拟设备映像。 该映像是通过 Azure 经典门户下载的映像。 
   
   1. 确保这是已下载的最新映像文件。 如果以前下载过此映像，请再下载一次，确保其为最新映像。 最新映像有两个文件（而不是一个）。
   2. 记下复制映像的位置，因为在以后的过程中需用到它。
...
4. 首先需将 VMDK 上载到 ESXi 服务器。 导航到右窗格中的“配置”选项卡。 在“硬件”下选择“存储”。
...
8. 在工具栏中，单击 ![](./media/storsimple-ova-deploy2-provision-vmware/image7.png) 图标创建新的文件夹。 指定文件夹名称，将其记录下来。 稍后创建虚拟机时需用到此文件夹名称（建议的最佳做法）。 单击 **“确定”**。
...
10. 单击上载图标 ![](./media/storsimple-ova-deploy2-provision-vmware/image10.png)，然后选择“上载文件”。
    
    ![](./media/storsimple-ova-deploy2-provision-vmware/image11.png)
11. 此时应浏览并指向已下载的 VMDK 文件。 将有两个文件。 选择要上载的文件。
    
    ![](./media/storsimple-ova-deploy2-provision-vmware/image12m.png)
12. 单击“打开”。 此时会开始将 VMDK 文件上载到指定的数据存储。 上载文件可能需要数分钟的时间。
13. 上载完成后，可以在数据存储的指定文件夹中看到该文件。 
    
    ![](./media/storsimple-ova-deploy2-provision-vmware/image14.png)
    
    此时需将第二个 VMDK 文件上载到同一数据存储。
...
18. 在“虚拟机版本”页上，选择“虚拟机版本: 8”。 请注意，版本 8 到 11 均受支持。
...
31. 在“创建磁盘”页上，将“磁盘大小”更改为 500 GB（或 500 GB 以上）。 在“磁盘预配”下，选择“精简预配”。 单击“资源组名称” 的 Azure 数据工厂。
...
34. 此时会返回到“虚拟机属性”页。 新硬盘已添加到虚拟机。 单击“完成” 。
...
1. 完成初始设置并启动设备以后，将会显示设备横幅文本。 记下显示在横幅文本中的 IP 地址和 URL，以便管理设备。 需使用该 IP 地址连接到虚拟设备的 Web UI 并完成本地设置和注册。
   
   ![](./media/storsimple-ova-deploy2-provision-vmware/image45.png)
2. （可选）仅当你是在政府云中部署设备时，才执行此步骤。 此时会在设备上启用美国联邦信息处理标准 (FIPS) 模式。 FIPS 140 标准定义的加密算法已经过批准，可以用于美国联邦政府计算机系统来保护敏感数据。
