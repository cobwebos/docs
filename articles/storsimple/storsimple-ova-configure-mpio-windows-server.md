---
title: "在 StorSimple Virtual Array 主机上配置 MPIO | Microsoft Docs"
description: "介绍了如何为连接到运行 Windows Server 2012 R2 的主机的StorSimple Virtual Array 配置多路径 I/O (MPIO)。"
services: storsimple
documentationcenter: 
author: alkohli
manager: carmonm
...
要实现 StorSimple 解决方案的高可用性，请在连接到 StorSimple 1200 Virtual Array（也称为本地虚拟设备）的 Windows Server 主机上配置 MPIO。 然后，主机服务器可以包容链路、网络或接口故障。 
...
* 应当将虚拟阵列配置为 iSCSI 服务器。 若要了解详细信息，请参阅[将虚拟阵列设置为 iSCSI 服务器](storsimple-ova-deploy3-iscsi-setup.md)。 应当在阵列上启用一个或多个网络接口。   
* 应当可以从 Windows Server 主机访问虚拟阵列上的网络接口。
* 应当在 StorSimple Virtual Array 上创建一个或多个卷。 若要了解详细信息，请参阅在 StorSimple 1200 Virtual Array 上[添加卷](storsimple-ova-deploy3-iscsi-setup.md#step-3-add-a-volume)。 在此过程中，我们在虚拟阵列上创建了 3 个卷（2 个本地固定卷和 1 个分层卷，如下所示）。
  
    ![MPIO0](./media/storsimple-ova-configure-mpio-windows-server/mpio0.png)

### <a name="hardware-configuration-for-storsimple-virtual-array"></a>StorSimple Virtual Array 的硬件配置
下图显示了此过程中为实现 Windows Server 主机和 StorSimple Virtual Array 的高可用性和负载平衡多路径而使用的硬件配置。  

![MPIO 硬件配置](./media/storsimple-ova-configure-mpio-windows-server/1200hardwareconfig.png)

如上图所示：

* 预配在 Hyper-V 上的 StorSimple Virtual Array 是一个配置为 iSCSI 服务器的单节点活动设备。
* 阵列上启用了两个虚拟网络接口。 在 1200 虚拟阵列的本地 Web UI 中，通过导航到“网络设置”来验证是否启用了两个网络接口，如下所示：
  
    ![1200 上启用的网络接口](./media/storsimple-ova-configure-mpio-windows-server/mpio9.png)
...
3. 在“发现目标门户”对话框中，执行以下操作：
   ...
   * 输入 StorSimple Virtual Array 上启用的第一个网络接口的 IP 地址。 默认情况下，此接口为 **Ethernet**。 
   * 单击“确定”以返回到“iSCSI 发起程序属性”对话框。
...
6. 单击“连接”来与 StorSimple 阵列建立 iSCSI 会话。 此时将显示“连接到目标”对话框。 选中“启用多路径”复选框。 单击“高级”。
...
11. 在“高级设置”对话框中：                                        
    ...
    * 在“本地适配器”下拉列表中，选择“Microsoft iSCSI 发起程序”。
    * 在“发起程序 IP”下拉列表中，选择与主机对应的 IP 地址。 在本例中，你将阵列上的两个网络接口连接到主机上的单个网络接口。 因此，此接口与为第一个会话提供的接口相同。
    * 在“目标门户 IP”下拉列表中，选择阵列上启用的第二个数据接口的 IP 地址。
    * 单击“确定”以返回到“iSCSI 发起程序属性”对话框。 你已添加了与目标之间的第二个会话。
      
       ![mpio11](./media/storsimple-ova-configure-mpio-windows-server/mpio11.png)
    * 在添加所需的会话（路径）后，在“iSCSI 发起程序属性”对话框中，选择目标并单击“属性”。 在“属性”对话框的“会话”选项卡上，记下与可能的路径排列对应的四个会话标识符。 若要取消会话，请选中会话标识符旁边的复选框，然后单击“断开连接”。
    * 若要查看会话内存在的设备，请选择“设备”选项卡。 若要为所选设备配置 MPIO 策略，请单击“MPIO”。 **
    * 详细信息** 对话框随即将出现。在“MPIO”选项卡上，可以选择合适的“负载平衡策略”设置。****还可以查看**活动**或**备用** 路径类型。
...
14. 通过导航到“服务器管理器”>“仪表板”>“计算机管理”打开“计算机管理”。 在左窗格中，单击“存储”>“磁盘管理”。 此主机可以看到的在 StorSimple Virtual Array 上创建的卷将作为新磁盘出现在“磁盘管理”下。
15. 初始化磁盘并创建新卷。 在格式化过程中，选择 64 KB 作为分配单元大小 (AUS)。 为所有可用卷重复此过程。
    
    ![磁盘管理](./media/storsimple-ova-configure-mpio-windows-server/mpio20.png)
16. 在“磁盘管理”下，右键单击“磁盘”并选择“属性”。
17. 在“多路径磁盘设备属性”对话框中，单击“MPIO”选项卡。
    
    ![磁盘属性](./media/storsimple-ova-configure-mpio-windows-server/mpio21.png)
...
详细了解[使用 StorSimple Manager 服务管理 StorSimple Virtual Array](storsimple-ova-manager-service-administration.md)。
