...
2. 在“添加卷”边栏选项卡中：
    ...
    6. 在“连接的主机”字段中，单击箭头。 
        ...
        ![连接的主机](./media/storsimple-8000-manage-volumes-u2/step5createvol2.png)

    7. 在“连接的主机”边栏选项卡中，选择现有的 ACR 或添加新的 ACR。 如果选择新 ACR，请提供 ACR 的**名称**，提供 Windows 主机的 **iSCSI 限定名称** (IQN)。 如果没有 IQN，请转到 [获取 Windows Server 主机的 IQN](#get-the-iqn-of-a-windows-server-host)。 单击“创建” 。 使用指定的设置创建卷。
...
1. 转到 StorSimple Device Manager 服务，然后单击“设备”。 从设备的表格列表中，选择包含所要修改的卷的设备。 单击“设置”>“卷”。
...
5. 单击“保存”以保存更改。 出现确认提示时，单击“是”。 Azure 门户将显示更新卷消息。 成功更新卷后，它将显示一条成功消息。
...
如果要将分层卷转换为本地固定卷，StorSimple 会先验证设备上是否有足够的空间，然后开始转换。 如果空间不足，将显示错误，并取消该操作。 
...
如果需要额外的空间来预配其他卷，可以将本地固定卷更改为分层卷。 将本地固定卷转换为分层卷时，设备上的可用容量将根据释放的容量大小增加。 如果连接问题导致无法将卷从本地类型转换为分层类型，本地卷将展示分层卷的属性，直到完成转换为止。 这是因为，可能有一部分数据溢出到云中。 这些溢出的数据继续占用设备上的本地空间，在重启并完成操作之前无法释放。
