---
title: 教程：使用 Azure DevOps Starter 为现有代码创建 CI/CD 管道
description: Azure DevOps Starter 可让你轻松地开始在 Azure 上入门。 借助 DevOps Projects，只需通过几个简单的步骤即可使用自己的代码和 GitHub 存储库启动 Azure 服务上的应用。
services: vsts
...
Azure DevOps Starter 提供了一个简化的过程，用于创建到 Azure 的持续集成（CI）和持续交付（CD）管道。 你可以引入自己的现有代码和 Git 存储库，或选择一个示例应用程序。
...
Azure DevOps Starter Azure Pipelines 中创建 CI/CD 管道。 可以创建新的 Azure DevOps 组织，或使用现有的组织。 Azure DevOps Starter 还会在所选的 Azure 订阅中创建 Azure 资源。
...
1. 从下拉菜单中选择一个**应用程序运行时**和**应用程序框架**，然后选择“下一步”。**** 应用程序框架决定可用的 Azure 服务部署目标类型。
...
1. 在“Azure 订阅”中选择订阅，然后在“Web 应用”中输入名称，或使用默认值。******** 选择一个**位置**，然后选择“完成”。**** 几分钟后，"DevOps 入门部署概述" 会显示在 Azure 门户中。

1. 选择 "**前往资源**" 以查看 DevOps 初学者仪表板。 在右上角，将“项目”**** 固定在仪表板上，以便快速访问。 Azure DevOps Starter 自动配置 CI 生成和发布触发器。 代码保留在 GitHub 存储库或其他外部存储库中，示例应用设置在“Azure DevOps 组织”中的某个存储库内。**** Azure DevOps Starter 运行生成并将应用部署到 Azure。
...
1. 从 DevOps 入门仪表板中，选择 "**存储库**"。 GitHub 存储库将在新的浏览器选项卡中打开。对应用程序进行更改，然后选择 "**提交更改**"。

1. 片刻之后，系统会在 Azure Pipelines 中启动生成。 你可以在 DevOps 入门仪表板中监视生成状态。 你还可以通过从 DevOps 入门仪表板中选择 "**生成管道**" 选项卡，在 Azure DevOps 组织中监视它。

## <a name="examine-the-azure-pipelines-cicd-pipeline"></a>检查 Azure Pipelines CI/CD 管道

Azure DevOps Starter 会自动在 Azure Pipelines 中配置 CI/CD 管道。 根据需要浏览和自定义管道。 若要了解生成和发布管道，请执行以下操作：

1. 从 "DevOps" 初学者面板中，选择 "**生成管道**"。
...
1. 该生成会执行各种任务，例如，从存储库提取源、还原依赖项，以及发布用于部署的输出。 在右侧的 "**名称**" 下，将生成管道名称更改为更具描述性的名称。 选择 "**保存 & 队列**"，然后选择 "**保存**"。 输入注释，然后再次选择“保存”。****

   ![Azure DevOps 生成页](_img/azure-devops-project-github/buildpage.png)

1. 若要查看生成的最近更改的审核记录，请选择 "**历史记录**" 选项卡。 Azure DevOps 跟踪对生成管道所做的任何更改，并允许你比较版本。

1. 选择 "**触发器**" 选项卡。 Azure DevOps Projects 会自动创建具有一些默认设置的 CI 触发器。 可以设置“启用持续集成”之类的触发器，以便每次提交代码更改时都运行生成。**** 还可以设置触发器，以将生成计划为在特定的时间运行。

## <a name="clean-up-resources"></a>清理资源

不再需要本教程中创建的 Azure 应用服务和相关资源时，可将其删除。 在 DevOps Projects 仪表板上使用**删除**功能。
