...
* 自动创建 Azure 资源，例如 Azure Cosmos DB、Application Insights、应用服务和应用服务计划。
...
DevOps Projects 在 Azure Pipelines 中创建 CI/CD 管道。 可以创建新的 Azure DevOps 组织，或使用现有的组织。 DevOps Projects 也在所选的 Azure 订阅中创建 Azure 资源，例如 Azure Cosmos DB、Application Insights、应用服务和应用服务计划。
...
1. 在搜索框中键入 **DevOps Projects**，然后单击“添加”。
   ...
   ![DevOps 项目](_img/azure-devops-project-cosmos-db/devops-project.png)

1. 选择 **Node.js** 作为运行时，然后选择“下一步”。 在“选择应用程序框架”下，选择“Express.js”。

1. 为 **Cosmos DB** 启用“添加数据库”部分，然后单击“下一步”。

    ![添加数据库](_img/azure-devops-project-cosmos-db/add-database.png)

    Cosmos DB 支持各种应用程序框架，例如 **Express.js**、**示例 Node.js 应用**和 **Sail.js**。 在本教程中，让我们考虑 **Express.js**。

1. 选择 Azure 服务来部署应用程序。 你有不同的服务，例如 Windows Web 应用、Kubernetes 服务和用于容器的 Web 应用。 在本教程中，我们将使用 **Windows Web 应用**。 单击“下一步”。
...
1. 配置过程在几分钟后完成。 此过程会在你的 Azure DevOps 组织的 Git 存储库中设置一个示例 Node.js 应用，创建 Azure Cosmos DB、应用服务、应用服务计划和 Application Insights，执行 CI/CD 管道，并将应用部署到 Azure。

   完成所有操作后，Azure DevOps Projects 仪表板会显示在 Azure 门户中。 也可直接从 Azure 门户中的“所有资源”直接转到 DevOps Projects 仪表板。

   在此仪表板中可以查看 Azure DevOps 代码存储库、CI/CD 管道和 Azure Cosmos DB。 可在 Azure DevOps 管道中配置其他 CI/CD 选项。 在右侧，选择要查看的“Azure Cosmos DB”。

## <a name="examine-the-azure-cosmos-db"></a>检查 Azure Cosmos DB

DevOps Projects 会自动配置 Cosmos DB，你可以浏览和自定义它。 若要了解 Cosmos DB，请执行以下操作：

1. 转到 DevOps Projects 仪表板。

    ![DevOps Projects 仪表板](_img/azure-devops-project-cosmos-db/devops-project-dashboard.png)

1. 在右侧，选择“Cosmos DB”。 此时会打开 Cosmos DB 的窗格。 可在此视图中执行各种操作，例如监视和搜索日志的操作。
...
1. 单击“生成”下的超链接。 浏览器标签页会显示新项目的生成管道。
    ...
    ![构建](_img/azure-devops-project-cosmos-db/build.png)
...
1. 在“删除”图标的右侧，选择“持续部署触发器”。 此发布管道已启用 CD 触发器，每次有新的生成项目可用时，此触发器就会执行部署。 （可选）可以禁用此触发器，这样就需要手动执行部署。
...
现在，可以使用可将最新工作部署到 Azure 应用服务的 CI/CD 过程，与某个团队展开应用协作。 每次对 Git 存储库进行更改都会在 Azure DevOps 中启动一个生成，CD 管道可以执行到 Azure 的部署。 遵循本部分所述的过程，或使用其他方法提交对存储库所做的更改。 例如，可以在常用工具或 IDE 中克隆 Git 存储库，然后将更改推送到此存储库。

1. 在 Azure DevOps 菜单中选择“存储库”|“文件”，然后转到你的存储库。

1. 存储库已包含的代码基于在创建过程中选择的应用程序语言。 打开 **Application/views/index.pug** 文件。

1. 选择“编辑”，然后对**第 15 行**进行更改。 例如，可以将它更新为“我的第一个目标为 Azure Cosmos DB 支持的 Azure 应用服务的部署”。
