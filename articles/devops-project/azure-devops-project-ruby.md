...
description: Azure DevOps Starter 可让你轻松地开始在 Azure 上入门。 可以快速启动 Azure 服务上的 Ruby Web 应用。
ms.prod: devops
...
Azure DevOps Starter Azure Repos 中创建 CI/CD 管道。 可以创建新的 Azure DevOps 组织，或使用现有的组织。 DevOps Starter 还会在所选的 Azure 订阅中创建 Azure 资源。
...
1. 在浏览器中，请切换到 DevOps 入门仪表板。 此时应有一个生成正在进行。 所做的更改会自动通过 CI/CD 管道进行生成和部署。
