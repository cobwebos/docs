---
title: 快速入门：使用 Azure DevOps Projects 创建用于 Ruby on Rails 的 CI/CD 管道
description: 可以通过 Azure DevOps Projects 轻松地在 Azure 上开始操作。 可以快速启动 Azure 服务上的 Ruby Web 应用。
ms.prod: devops
...
ms.locfileid: "52264516"
---
# <a name="create-a-cicd-pipeline-for-ruby-on-rails-by-using-azure-devops-projects"></a>使用 Azure DevOps Projects 创建用于 Ruby on Rails 的 CI/CD 管道

使用 Azure DevOps Projects 为 Ruby on Rails 应用配置持续集成 (CI) 和持续交付 (CD)。 DevOps Projects 可以简化 Azure DevOps 生成和发布管道的初始配置。

如果没有 Azure 订阅，可以通过 [Visual Studio Dev Essentials](https://visualstudio.microsoft.com/dev-essentials/) 免费获取一个。

## <a name="sign-in-to-the-azure-portal"></a>登录到 Azure 门户

Azure DevOps Projects 在 Azure Repos 中创建 CI/CD 管道。 可以创建新的 Azure DevOps 组织，或使用现有的组织。 DevOps Projects 还可以在所选的 Azure 订阅中创建 Azure 资源。
...
1. “Linux 上的 Web 应用”是默认的部署目标。  
    也可选择“适用于容器的 Web 应用”。 以前选择的应用程序框架规定了此处提供的 Azure 服务部署目标的类型。 
...
Azure DevOps Projects 在 Azure Pipelines 或 GitHub 中创建一个 Git 存储库。 若要查看存储库并对应用进行代码更改，请执行以下操作：
...
Azure DevOps Projects 自动在你的 Azure DevOps 组织中配置一个完整的 CI/CD 管道。 根据需要浏览和自定义管道。 若要了解 Azure DevOps 生成和发布管道，请执行以下操作：
...
不再需要本快速入门中创建的 Azure 应用服务实例和相关资源时，可将其删除。 请使用 DevOps Projects 仪表板上的“删除”功能来这样做。
