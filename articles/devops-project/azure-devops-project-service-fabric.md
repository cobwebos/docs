...
description: Azure DevOps Starter 可让你轻松地开始在 Azure 上入门。 使用 DevOps Projects，可通过几个快速步骤将 ASP.NET Core 应用部署到 Azure Service Fabric。
ms.author: mlearned
ms.manager: gwallace
ms.prod: devops
...
DevOps Starter Azure Pipelines 中创建 CI/CD 管道。 可以创建新的 Azure DevOps 组织，或使用现有的组织。 DevOps Starter 还会在所选的 Azure 订阅中创建 Azure 资源，例如 Service Fabric 群集。
...
1. 退出 Azure 配置区域，然后选择“完成”。   
    ...
    完成所有此项后，"Azure 门户中将显示 DevOps 入门仪表板。 你还可以直接从 Azure 门户中的**所有资源**中转到 DevOps 入门仪表板。 
...
DevOps Starter 会自动在 Azure Pipelines 中配置 CI/CD 管道。 可以浏览和自定义此管道。 若要了解此管道，请执行以下操作：
...
1. 在右上角选择“提交”，然后再次选择“提交”以推送更改。********  
    片刻之后，系统会启动生成，然后执行发布以部署所做的更改。 可以通过 Azure DevOps 实时日志记录在 DevOps 入门仪表板或浏览器中监视生成状态。

1. 发布完成后，刷新应用以验证更改。

## <a name="clean-up-resources"></a>清理资源

测试后，可以清理资源，以避免产生费用。 不再需要本教程中创建的 Azure Service Fabric 群集和相关资源时，可将其删除。 为此，请使用 DevOps 入门仪表板上的**删除**功能。
