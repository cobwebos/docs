---
title: 使用 Azure DevOps 项目创建用于 Java 的 CI/CD 管道 | 快速入门
...
ms.locfileid: "37966874"
---
# <a name="create-a-cicd-pipeline-for-java-with-the-azure-devops-project"></a>使用 Azure DevOps 项目创建用于 Java 的 CI/CD 管道

Azure DevOps 项目提供一种简化的体验，适合在 Visual Studio Team Services (VSTS)（Microsoft 推出的适用于 Azure 的 DevOps 解决方案）中为 Java 应用创建 Azure 资源并设置持续集成 (CI) 和持续交付 (CD)管道。  
...
1. 默认的示例框架为 **Spring**。 保留默认设置，然后选择“下一步”。  
...
1. 创建**新的** VSTS 帐户，或者选择**现有的**帐户。  为 VSTS 项目选择一个**名称**。  选择 **Azure 订阅**、**位置**，然后为应用程序选择一个**名称**。  完成后，选择“完成”。
    ...
    ![输入 VSTS 信息](_img/azure-devops-project-java/vstsazureinfo.png)
...
Azure DevOps 项目会自动配置 CI 生成和发布触发器。  现在可以与 Java 应用团队协作，通过 CI/CD 过程自动将最新工作部署到网站。
...
1. 在浏览器左侧导航到 **src/main/webapp/index.html** 文件。
