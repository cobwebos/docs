---
title: 使用 Azure DevOps 项目创建用于 Python 的 CI/CD 管道 | 快速入门
...
ms.locfileid: "47405643"
---
# <a name="create-a-cicd-pipeline-for-python-with-the-azure-devops-project"></a>使用 Azure DevOps 项目创建用于 Python 的 CI/CD 管道

Azure DevOps 项目提供一种简化的体验，适合在 Azure DevOps Services 中为 Python 应用创建 Azure 资源并设置持续集成 (CI) 和持续交付 (CD)管道。  
...
1. **项目仪表板**会在数分钟内加载到 Azure 门户中。  首先会在 Azure DevOps Services 组织的存储库中设置示例应用程序，然后执行生成并将应用程序部署到 Azure。  可以通过此仪表板查看**代码存储库**、**Azure CI/CD 管道**以及 **Azure 中的应用程序**。  在仪表板的右侧，选择“浏览”即可查看正在运行的应用程序。

    ![仪表板视图](_img/azure-devops-project-python/dashboardnopreview.png) 
    
Azure DevOps 项目会自动配置 CI 生成和发布触发器。  现在可以与 Python 应用团队协作，通过 CI/CD 过程自动将最新工作部署到网站。
...
1. 在浏览器左侧导航到 **app/templates/app/index.html** 文件。
