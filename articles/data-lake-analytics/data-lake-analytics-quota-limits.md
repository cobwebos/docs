...
了解如何调整和增加 Azure Data Lake Analytics (ADLA) 帐户中的配额和限制。 了解这些限制将帮助您了解 U-SQL 作业行为。 所有配额限制都是软限制，因此随时可通过联系 Azure 支持增加最大限制。
...
* 如果已有四个作业正在运行，每个作业使用 8 AU，提交需要 8 AU 的第五个作业时，该作业会在作业队列中等待，直到有 8 AU 可用。
    ...
    ![Azure 数据湖分析限制和配额页](./media/data-lake-analytics-quota-limits/adjust-quota-limits.png)

**每个作业的最大分析单位 （A） 数量：250**个，默认 32 个

这是每个作业可以在您的帐户中分配的最大数量。 除非提交者受到计算策略（作业提交限制）的影响，这会给每个作业提供更多 AU，否则分配超过此限制的作业将被拒绝。 此值的上限是帐户的 AU 限制。
