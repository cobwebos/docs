...
**本地计算机**帐户是一个共享的本地计算帐户，它含有单个本地数据根文件夹，作为本地存储帐户。 数据根文件夹默认位于 **C:\Users\<username>\AppData\Local\USQLDataRoot**。 它还可以通过**工具** > **数据湖** > **选项和设置**进行配置。
...
|本地数据根文件夹|永久本地文件夹。 通过**工具** > **数据湖** > **选项和设置配置**。|为在 U-SQL 项目工作目录下的每个本地运行创建的临时文件夹。 重新生成或重新运行时会清理该文件夹。|
...
- [如何测试 Azure 数据湖分析代码](data-lake-analytics-cicd-test.md)。
