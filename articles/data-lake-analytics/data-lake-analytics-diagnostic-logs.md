...
2. 打开 Data Lake Analytics 帐户，在 Data Lake Analytics 帐户边栏选项卡上单击“设置”，然后单击“诊断设置”。
3. 在“诊断”边栏选项卡上，作出以下更改来配置诊断日志记录。

    ![启用诊断日志记录](./media/data-lake-analytics-diagnostic-logs/enable-diagnostic-logs.png "Enable diagnostic logs")

   * 将“状态”设置为“开启”，启用诊断日志记录。
   * 可选择以两种不同的方式存储/处理数据。
     * 选择“导出到事件中心”将日志数据流式传输到 Azure 事件中心。 具有下游处理管道来实时分析传入日志时使用此选项。 若选择此选项，必须提供要使用的 Azure 事件中心的详细信息。
     * 选择“导出到存储帐户”，将日志存储到 Azure 存储帐户。 如果想要将数据存档，请使用此选项。 如果选择此选项，必须提供一个 Azure 存储帐户，以将日志保存到其中。
   * 指定是要获取审核日志还是请求日志，或者两者都获取。
   * 指定数据必须保留的天数。
   * 单击“保存” 。

启用诊断设置后，可在“诊断日志”选项卡上查看日志。
...
1. 从 Data Lake Analytics 帐户“设置”边栏选项卡上，单击“诊断日志”。

    ![查看诊断日志记录](./media/data-lake-analytics-diagnostic-logs/view-diagnostic-logs.png "View diagnostic logs")
...
1. 打开与日志记录的 Data Lake Analytics 关联的 Azure 存储帐户边栏选项卡，然后单击 Blob。 “Blob 服务”边栏选项卡上会列出两个容器。
    ...
    ![查看诊断日志记录](./media/data-lake-analytics-diagnostic-logs/view-diagnostic-logs-storage-account.png "View diagnostic logs")
