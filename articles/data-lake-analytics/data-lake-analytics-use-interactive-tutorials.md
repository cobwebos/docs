<properties 
   pageTitle="使用 Azure 门户交互式教程学习 Data Lake Analytics 和 U SQL | Azure" 
...
Azure 门户为你提供了学习 Data Lake Analytics 入门的交互式教程。本文将介绍如何通过本教程学习来分析网站日志。


>[AZURE.NOTE] 如果想要通过使用 Visual Studio 完成同一个教程，请参阅[使用 Data Lake Analytics 分析网站日志](data-lake-analytics-analyze-weblogs.md)。还有更多可添加到门户的交互式教程。
...
每个 Data Lake Analytics 帐户都有一个从属 [Azure Data Lake Store](../data-lake-store/data-lake-store-overview.md) 帐户。该帐户称作默认 Data Lake Store 帐户。可以事先或在创建 Data Lake Analytics 帐户时，创建 Data Lake Store 帐户。在本教程中，将使用 Analytics 帐户创建 Data Lake Store 帐户
...
3. 单击“**Marketplace**”磁贴。
3. 在“**全部**”边栏选项卡上的搜索框中键入 **Azure Data Lake Analytics**，然后按 **ENTER**。应该会在列表中看到 **Azure Data Lake Analytics**。
4. 从列表中单击“**Azure Data Lake Analytics**”。
5. 单击边栏选项卡底部的“**创建**”。
6. 键入或选择以下内容：
	...
	- **资源组**。选择现有的 Azure 资源组或创建新的资源组。应用程序通常由许多组件构成，例如 Web 应用、数据库、数据库服务器、存储和第三方服务。你可以使用 Azure 资源管理器 (ARM) 以组（称为 Azure 资源组）的方式处理应用程序中的资源。你可以通过一个协调的操作为应用程序部署、更新、监视或删除所有资源。你可以使用一个模板来完成部署，该模板适用于不同的环境，例如测试、过渡和生产。你可以通过查看整个组的累积费用，明确了解组织的帐单开支。有关详细信息，请参阅 [Azure 资源管理器概述](resource-group-overview.md)。
	- **位置**。为 Data Lake Analytics 帐户选择 Azure 数据中心。
7. 选择“**固定到启动板**”。这是出于按照本教程操作的需要。
8. 单击“创建”。将转到门户启动板。一个新的磁贴添加主页并带有显示为“部署 Azure Data Lake Analytics”的标签。需要花一些时间来创建一个 Data Lake Analytics 帐户。帐户创建后，门户将在新的边栏选项卡上打开该帐户。
...
5. 从“**交互式教程**”边栏选项卡上，单击“**网站日志分析**”。门户将在新的门户边栏选项卡中打开本教程。
5. 单击“**1 简介**”，然后按照说明进行操作
