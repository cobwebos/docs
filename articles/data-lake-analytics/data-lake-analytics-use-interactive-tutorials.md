---
title: "使用 Azure 门户交互式教程学习 Data Lake Analytics 和 U-SQL | Microsoft 文档"
...
7. 键入或选择以下内容：
   ...
   * **资源组**。 选择现有的 Azure 资源组或创建新的资源组。 应用程序通常由许多组件构成，例如 Web 应用、数据库、数据库服务器、存储和第三方服务。 你可以使用 Azure 资源管理器 (ARM) 以组（称为 Azure 资源组）的方式处理应用程序中的资源。 你可以通过一个协调的操作为应用程序部署、更新、监视或删除所有资源。 你可以使用一个模板来完成部署，该模板适用于不同的环境，例如测试、过渡和生产。 你可以通过查看整个组的累积费用，明确了解组织的帐单开支。 有关详细信息，请参阅 [Azure Resource Manager 概述](../azure-resource-manager/resource-group-overview.md)。 
   * **位置**。 为 Data Lake Analytics 帐户选择 Azure 数据中心。 
8. 选择“ **固定到启动板**”。 这是出于按照本教程操作的需要。
9. 单击“创建” 。 将转到门户启动板。 一个新的磁贴添加主页并带有显示为“部署 Azure Data Lake Analytics”的标签。 创建 Data Lake Analytics 帐户需要花费一点时间。 帐户创建后，门户将在新的边栏选项卡上打开该帐户。
