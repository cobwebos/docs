...
使用前面的任务所描述的过程在 Azure 上创建 Linux VM。确保此 VM 是独立 VM，因为我们会将此 VM 用作访问 Cassandra 群集的客户端。我们将先从 github 中安装 Node.js、NPM 和 [cassandra-client][]，然后从该 VM 连接到 Cassandra 群集：
