---
title: 使用 Azure IoT 中心设备配置服务的自定义分配策略
description: 如何将自定义分配策略与 Azure IoT 中心设备配置服务 （DPS） 一起使用
author: wesmc7777
ms.author: wesmc
ms.date: 11/14/2019
ms.topic: conceptual
...
通过使用自定义分配策略，当设备预配服务提供的策略不符合方案要求时，可以定义自己的分配策略。
...
在本节中，您可以使用 Azure 云外壳创建预配服务和两个 IoT 中心，表示**Contoso Toasters 部门和** **Contoso 热泵部门**。

> [!TIP]
> 本文中使用的命令在美国西部位置创建预配服务和其他资源。 我们建议在与你最靠近的区域中创建支持设备预配服务的资源。 若要查看可用位置的列表，可以运行 `az provider show --namespace Microsoft.Devices --query "resourceTypes[?resourceType=='ProvisioningServices'].locations | [0]" --out table` 命令，也可以转到[Azure 状态](https://azure.microsoft.com/status/)页，在其中搜索“设备预配服务”。 在命令中，位置可以指定为一个单词或多字格式;例如：西部、美国西部、美国西部等。该值不区分大小写。 如果使用多个单词的格式来指定位置，请将值置于引号中，例如 `-- location "West US"`。
>

1. 在 Azure Cloud Shell 中，使用 [az group create](/cli/azure/group#az-group-create) 命令创建资源组。 Azure 资源组是在其中部署和管理 Azure 资源的逻辑容器。

    下面的示例在*西部*区域创建名为*contoso-us-resource-group*的资源组。 建议对本文中创建的所有资源使用该组。 此方法将使清理完成后更容易。

    ```azurecli-interactive 
    az group create --name contoso-us-resource-group --location westus
    ```

2. 使用 Azure 云外壳使用[az iot dps 创建](/cli/azure/iot/dps#az-iot-dps-create)命令创建设备预配服务。 该预配服务将添加到 *contoso-us-resource-group*。

    下面的示例在*西部*位置创建名为*contoso 预配服务-1098*的预配服务。 必须使用唯一的服务名称。 在服务名称中的 **1098** 位置构成你自己的后缀。
...
3. 在 Azure Cloud Shell 中，使用 [az iot hub create](/cli/azure/iot/hub#az-iot-hub-create) 命令创建 Contoso 烤箱分区**** IoT 中心。 IoT 中心将被添加到 contoso-us-resource-group**。
    ...
    下面的示例*在西部*位置创建名为*contoso-toasters-hub-1098*的 IoT 中心。 必须使用唯一的中心名称。 在中心名称中的 1098**** 位置构成你自己的后缀。 自定义分配策略的示例代码要求使用中心名称中的 `-toasters-`。
...
4. 在 Azure Cloud Shell 中，使用 [az iot hub create](/cli/azure/iot/hub#az-iot-hub-create) 命令创建 Contoso 热泵分区**** IoT 中心。 此 IoT 中心也将被添加到 contoso-us-resource-group**。
    ...
    下面的示例*在西部*位置创建名为*contoso-热泵-集线器-1098*的 IoT 中心。 必须使用唯一的中心名称。 在中心名称中的 1098**** 位置构成你自己的后缀。 自定义分配策略的示例代码要求使用中心名称中的 `-heatpumps-`。
...
3. 在“函数应用”创建页上的“基本信息”选项卡下，输入新函数应用的以下设置，然后选择“查看 + 创建”：************
    ...
    **资源组**：选择**contoso-us 资源组**，以保持本文中创建的所有资源在一起。

    **函数应用名称**：输入唯一的函数应用名称。 本示例使用 **contoso-function-app-1098**。

    **发布**：验证**是否选择了代码**。

    **运行时堆栈**：从下拉列表中选择 **.NET 核心**。
...
1. 下载 [CMake 生成系统](https://cmake.org/download/)。
    ...
    在开始`CMake`安装**之前**，在计算机上安装 Visual Studio 的先决条件（Visual Studio 和"具有C++"工作负载的桌面开发）非常重要。 满足先决条件并验证下载内容后，安装 CMake 生成系统。
...
2. 在 Visual Studio 菜单上，选择 > **"调试开始而不调试**"以运行解决方案。 **Debug** 在重建项目的提示中，选择 **"是**"以在运行之前重建项目。
...
2. 在 Visual Studio 菜单上，选择 > **"调试开始而不调试**"以运行解决方案。 **Debug** 对于重新生成项目的提示，请选择“是”，以便在运行项目之前重新生成项目****。

    以下输出是模拟热泵设备成功启动并连接到预配服务实例的示例，该实例通过自定义分配策略分配给 Contoso 热泵 IoT 中心：
...
1. 登录到 Azure[门户](https://portal.azure.com)并选择**资源组**。
...
* 要了解更多取消预配，请参阅[如何取消预配以前已自动预配的设备](how-to-unprovision-devices.md) 
