...
* 为所使用的[证明机制](concepts-security.md#attestation-mechanism)获取设备安全项目：
    ...
    * [**X.509**](/azure/iot-dps/concepts-security)：
        * 个人注册：来自物理设备或 SDK [DICE](/azure/iot-dps/concepts-security) 仿真器的[叶证书](https://azure.microsoft.com/blog/azure-iot-supports-new-security-hardware-to-strengthen-iot-security/)。
...
* 注册组是共享 X.509 证书的常见证明机制的一组设备的条目，由**根证书**或[中间证书](https://docs.microsoft.com/azure/iot-dps/concepts-security#root-certificate)签名[](https://docs.microsoft.com/azure/iot-dps/concepts-security#intermediate-certificate)。 建议对共享所需初始配置的大量设备，或者全部转到同一租户的设备使用注册组。 请注意，只能注册使用 X.509 证明机制作为“注册组”的设备。 
    ...
    1. 使用 ```EnrollmentGroup``` 创建的唯一 ```attestation``` 创建一个新的 ```enrollmentGroupId``` 变量。  （可选）可设置 ```Device ID```、```IoTHubHostName```、```ProvisioningStatus``` 等参数。
    2. 使用 ```createOrUpdateEnrollmentGroup``` 在后端应用程序中调用服务 SDK API ```EnrollmentGroup``` 以创建注册组。
...
* 单独注册是用于单个设备注册的条目。 个人注册可使用 X.509 证书或 SAS 令牌（来自物理或虚拟 TPM）作为证明机制。 对于需要唯一初始配置的设备或仅能通过 TPM 或虚拟 TPM 使用 SAS 令牌作为证明机制的设备，建议为其使用个人注册。 单独注册可能会指定所需 IoT 中心设备 ID。
    ...
    2. 通过使用 ```IndividualEnrollment``` 创建的唯一 ```attestation``` 作为输入来创建一个新的 ```registrationId``` 变量，前者在设备上或从 TPM 模拟器生成。  （可选）可设置 ```Device ID```、```IoTHubHostName```、```ProvisioningStatus``` 等参数。
    3. 使用 ```createOrUpdateIndividualEnrollment``` 在后端应用程序中调用服务 SDK API ```IndividualEnrollment``` 以创建单个注册。
...
* 可通过使用 **调用服务 SDK API** 来删除单个注册```deleteIndividualEnrollment``````registrationId```。
* 可通过使用 **调用服务 SDK API** 来删除组注册```deleteEnrollmentGroup``````enrollmentGroupId```。
...
2. 使用 ```runBulkOperation``` 调用服务 SDK API ```BulkOperationMode``` 以实现所需操作和用于单个注册的变量。 支持四种模式：创建、更新、updateIfMatchEtag 和删除。
