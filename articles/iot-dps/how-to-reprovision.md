---
title: 重新调配 Azure IoT 中心设备预配服务中的设备
...
3. 在“选择在重新预配到其他 IoT 中心时要如何处理设备数据”下，选择下述一个重新配置策略****：
    ...
    * **重新预配并重置为初始配置**：当与注册项关联的设备提交新的预配请求时，此策略将执行操作。 根据注册项配置，可将设备重新分配给其他 IoT 中心。 如果设备正在更改 IoT 中心，则将删除初始 IoT 中心内的设备注册。 向新的 IoT 中心提供预配设备时预配服务实例接收到的初始配置数据。 在迁移期间，设备的状态将报告为**分配**。
...
- 要了解更多取消预配，请参阅[如何取消预配以前自动预配的设备](how-to-unprovision-devices.md) 
