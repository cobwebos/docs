---
title: 从 Azure IoT 中心设备预配服务中取消注册设备
description: 如何取消注册设备以防止通过 Azure IoT 中心设备预配服务 （DPS） 进行预配
author: wesmc7777
ms.author: wesmc
ms.date: 04/05/2018
ms.topic: conceptual
...
单独登记适用于单个设备，可使用 X.509 证书或 SAS 令牌（在真实或虚拟 TPM 中）作为证明机制。 （使用 SAS 令牌作为证明机制的设备只能通过单个注册进行预配。要将具有单独注册的设备列入黑名单，可以禁用或删除其注册条目。 
...
6. 滚动到“添加注册”**** 页底部，对“启用项”**** 开关选择“禁用”****，然后选择“保存”****。 
    ...
    [![使用禁用的个人注册条目在门户中禁用组注册中的设备](./media/how-to-revoke-device-access-portal/disable-individual-enrollment-in-enrollment-group.png)](./media/how-to-revoke-device-access-portal/disable-individual-enrollment-in-enrollment-group.png#lightbox)
