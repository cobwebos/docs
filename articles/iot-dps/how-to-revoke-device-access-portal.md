...
## <a name="disallow-devices-by-using-an-individual-enrollment-entry"></a>使用单个注册条目禁止设备

单独登记适用于单个设备，可使用 X.509 证书或 SAS 令牌（在真实或虚拟 TPM 中）作为证明机制。 使用 SAS 令牌作为其证明机制的 (设备只能通过单个注册进行设置。 ) 若要禁用具有单个注册的设备，可以禁用或删除其注册条目。 

若要通过禁用注册项来暂时禁止设备，请执行以下操作： 

1. 登录 Azure 门户，选择左侧菜单中的“所有资源”。 
2. 在资源列表中，选择要禁止其设备的预配服务。
...
若要通过删除其注册项来永久禁止设备，请执行以下操作：

1. 登录 Azure 门户，选择左侧菜单中的“所有资源”。 
2. 在资源列表中，选择要禁止其设备的预配服务。
3. 在预配服务中，选择“管理登记”，然后选择“单独登记”选项卡   。
4. 选中要禁用的设备的注册项旁边的复选框。 
...
## <a name="disallow-an-x509-intermediate-or-root-ca-certificate-by-using-an-enrollment-group"></a>禁止使用注册组使用 x.509 中间或根 CA 证书

X.509 证书通常排列在证书信任链中。 如果链中任何阶段的证书被盗用，则信任受损。 必须禁止证书，以防止设备预配服务在包含该证书的任何链中下游预配设备。 若要详细了解 X.509 证书和及如何用其预配服务，请参阅 [X.509 证书](./concepts-security.md#x509-certificates)。 

注册组是共享 X.509 证书的常见证明机制的设备的项，由同一个中间或根 CA 签名。 登记组项配置了与中间或根 CA 相关联的 X.509 证书。 由证书链中包含该证书的设备共享的任何配置值（如孪生状态和 IoT 中心连接）。 若要禁止证书，可以禁用或删除其注册组。

若要通过禁用注册组来暂时禁止证书，请执行以下操作： 

1. 登录 Azure 门户，选择左侧菜单中的“所有资源”。 
2. 在资源列表中，选择要禁止签名证书的预配服务。
...
若要通过删除注册组永久禁止证书，请执行以下操作：

1. 登录 Azure 门户，选择左侧菜单中的“所有资源”。 
2. 在资源列表中，选择要禁止其设备的预配服务。
3. 在预配服务中，选择“管理登记”，然后选择“登记组”选项卡   。
4. 选中要禁止的证书的注册组旁边的复选框。 
...
## <a name="disallow-specific-devices-in-an-enrollment-group"></a>禁止注册组中的特定设备
...
1. 登录 Azure 门户，选择左侧菜单中的“所有资源”。 
2. 在资源列表中，选择包含要禁止的设备的注册组的预配服务。
3. 在预配服务中，选择“管理登记”，然后选择“单独登记”选项卡   。
4. 选择顶部的“添加单个注册”  按钮。 
5. 在“添加注册”  页上，选择“X.509”  作为设备的证明机制  。

    上传设备证书，并输入不允许的设备的设备 ID。 对于证书，要使用设备上安装的已签名最终实体证书。 设备使用已签名的最终实体证书进行身份验证。

    ![设置不允许设备的设备属性](./media/how-to-revoke-device-access-portal/disable-individual-enrollment-in-enrollment-group-1.png)
