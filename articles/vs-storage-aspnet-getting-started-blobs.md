<properties title="Getting Started with Azure Storage" pageTitle="Azure 存储入门" metaKeywords="Azure, Getting Started, Storage" description="" services="storage" documentationCenter="" authors="kempb" />
...
#####获取存储连接字符串
在你可以使用 Blob 执行任何操作之前，你需要先获取将存放 Blob 的存储帐户的连接字符串。你可以使用 **CloudStorageAccount** 类型来表示你的存储帐户信息。对于 ASP.NET 项目，你可以使用 **ConfigurationManager** 类型从 Azure 服务配置检索你的存储连接字符串和存储帐户信息，如以下代码所示。
