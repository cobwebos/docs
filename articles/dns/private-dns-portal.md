---
title: 使用 Azure 门户创建 Azure DNS 专用区域
description: 在本教程中，将在 Azure DNS 中创建并测试专用 DNS 区域和记录。 这是有关使用 Azure 门户创建和管理第一个专用 DNS 区域和记录的分步指南。
services: dns
author: vhorne
ms.service: dns
ms.topic: tutorial
...
ms.locfileid: "67147828"
---
# <a name="tutorial-create-an-azure-dns-private-zone-using-the-azure-portal"></a>教程：使用 Azure 门户创建 Azure DNS 专用区域

本教程将引导你完成使用 Azure 门户创建第一个专用 DNS 区域和记录的步骤。
...
如果你愿意，可以使用 [Azure PowerShell](private-dns-getstarted-powershell.md) 或 [Azure CLI](private-dns-getstarted-cli.md) 完成本教程中的步骤。
