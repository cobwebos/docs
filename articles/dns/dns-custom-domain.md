...
单击"**Storage** > **存储存储帐户**"，选择存储帐户并单击 **"自定义域**"，即可导航回存储帐户。 键入创建而不在文本框中没有 asverify 前缀的别名，选中"**使用间接 CNAME 验证**"，然后单击"**保存**"。 完成此步骤后，返回 DNS 区域，并创建不带 asverify 前缀的 CNAME 记录。  此后，可以安全删除带有 cdnverify 前缀的 CNAME 记录。
