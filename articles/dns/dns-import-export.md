---
title: "使用 CLI 将域区域文件导入和导出到 Azure DNS | Microsoft Docs"
...
本文将引导你使用 Azure CLI 导入和导出 Azure DNS 的 DNS 区域文件。
...
Azure DNS 支持通过使用 Azure 命令行接口 (CLI) 导入和导出区域文件。 Azure CLI 是用于管理 Azure 服务的跨平台命令行工具。 它适用于 Windows、Mac 和 Linux 平台，可以从 [Azure 下载页](https://azure.microsoft.com/downloads/)获取。 跨平台支持对导入和导出区域文件尤其重要，因为最常见的名称服务器软件 [BIND](https://www.isc.org/downloads/bind/) 通常在 Linux 上运行。

## <a name="obtain-your-existing-dns-zone-file"></a>获取现有的 DNS 区域文件

将 DNS 区域文件导入 Azure DNS 之前，需要获取区域文件的副本。 此文件的来源将取决于当前托管 DNS 区域的位置。

* 如果你的 DNS 区域由合作伙伴服务（如域注册机构、专用的 DNS 托管提供商或备用云提供商）托管，则该服务应提供下载 DNS 区域文件的功能。
* 如果 DNS 区域是在 Windows DNS 上托管的，则区域文件的默认文件夹是 **%systemroot%\system32\dns**。 在 DNS 服务管理控制台的“**常规**”选项卡上，也会显示每个区域文件的完整路径。
* 如果 DNS 区域是通过使用 BIND 托管的，则在 BIND 配置文件 **named.conf** 中会指定每个区域的区域文件位置。

**使用 GoDaddy 的区域文件**

从 GoDaddy 下载的区域文件会包含一些非标准格式。 你需要先更正格式，然后再将这些区域文件导入 Azure DNS。 每个 DNS 记录 RData 中的 DNS 名称会指定为完全限定名称，但结尾没有 "."。这意味着其他 DNS 系统会将它们解释为相对名称。 你需要先进行编辑，以将结尾 "." 附加到其名称的区域文件，然后再将其导入 Azure DNS。

## <a name="import-a-dns-zone-file-into-azure-dns"></a>将 DNS 区域文件导入 Azure DNS

导入区域文件时将会在 Azure DNS 中新建一个区域（如果区域尚不存在）。 如果区域已存在，则区域文件中的记录集必须与现有的记录集合并。

### <a name="merge-behavior"></a>合并行为

* 默认情况下，将合并现有的和新的记录集。 合并的记录集内的相同记录会执行重复数据删除。
* 或者，通过指定 `--force` 选项，导入过程会将当前的记录集替换为新的记录集。 不会删除导入的区域文件中没有相应记录集的现有记录集。
...
* 除非指定 `--force` 参数，否则导入的 CNAME 记录将不替换现有的同名 CNAME 记录。
...
* `$TTL` 指令是可选的并受支持。 如果未提供 `$TTL` 指令，将会导入没有显式 TTL 的记录，并为其设置默认 TTL 3600 秒。 当同一个记录集的两个记录指定不同的 TTL 时，将会使用较低的值。
...
如果资源组中不存在具有此名称的区域，将为你创建一个。 如果区域已存在，则导入的记录集将与现有的记录集合并。 若要覆盖现有的记录集，请使用 `--force` 选项。
...
4. 在使用 Azure DNS 服务之前，必须注册你的订阅才可使用 Microsoft.Network 资源提供程序。 （每个订阅只需进行一次注册操作。）
...
6. 若要将文件 **contoso.com.txt** 中的区域 **contoso.com** 导入到资源组 **myresourcegroup** 中的新 DNS 区域，请运行命令 `azure network dns zone import`。<BR>此命令将会加载并分析区域文件。 此命令会在 Azure DNS 服务上执行一系列命令，以便创建区域和区域中的所有记录集。 此命令还将在控制台窗口中报告进度，以及任何错误或警告。 由于记录集是以序列方式创建的，导入大型区域文件可能需要几分钟。
...
验证该区域已正确导入后，你将需要更新 DNS 委派以指向 Azure DNS 名称服务器。 有关详细信息，请参阅文章[更新 DNS 委派](dns-domain-delegation.md)。
...
和区域导入一样，你必须先登录，选择订阅，然后配置 Azure CLI 以使用 Resource Manager 模式。
...
4. 若要将资源组 **myresourcegroup** 中现有的 Azure DNS 区域 **contoso.com** 导出至文件 **contoso.com.txt**（在当前文件夹中），请运行 `azure network dns zone export`。 此命令将调用 Azure DNS 服务，以枚举该区域中的记录集，并将结果导出到兼容 BIND 的区域文件。
