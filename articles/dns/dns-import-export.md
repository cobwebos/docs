...
> [!NOTE]
> 从 GoDaddy 下载的区域文件会包含一些非标准格式。 你需要先更正格式，然后再将这些区域文件导入 Azure DNS。
>
> RDATA 中每个 DNS 记录的 DNS 名称会指定为完全限定名称，但结尾没有“.”这意味着其他 DNS 系统会将它们解释为相对名称。 你需要先进行编辑，以将结尾 "." 附加到其名称的区域文件，然后再将其导入 Azure DNS。
>
