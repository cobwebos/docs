<properties
	pageTitle="升级到 SQL Database V12"
	description="介绍如何从先前版本的 Azure SQL Database 升级到 Azure SQL Database V12。"
...
[注册](https://manage.windowsazure.cn) SQL Database V12，以利用 Microsoft Azure 上的下一代 SQL Database。你首先需要订阅 Microsoft Azure。你可以注册 [Azure 试用版](/pricing/1rmb-trial)并查看[价格](/home/features/sql-database/#price)信息。


## 升级到 SQL Database V12 的步骤
...
| 4.选择要将哪个服务器复制到已启用 SQL Database Update 的新服务器。 | ![显示 SQL Server 的列表][4] |
| 5.单击“最新的 SQL Database Update V12”。 | ![预览版最新功能][5] |
| 6.单击“升级此服务器”。 | ![将 SQL Server 升级到预览版][6] |


> [AZURE.NOTE]选择升级选项后，服务器及其内部的数据库将启用 SQL Database V12 功能，而你无法撤消该操作。若要将服务器升级到 SQL Database V12，你需要购买基本、标准或高级服务层。有关服务层的详细信息，请参阅[将 SQL Database Web/企业数据库升级到新服务层](sql-database-upgrade-new-service-tiers)。


> [AZURE.IMPORTANT]SQL Database V12（预览版）不支持地域复制。有关详细信息，请参阅[规划和准备升级到 Azure SQL Database V12 预览版](sql-database-v12-plan-prepare-upgrade)。
...
- 验证过程完成后，你将看到数据库名称的列表，这些数据库需要你根据升级到 SQL Database V12 的要求执行相应的操作。
 - **只有针对其中的每个数据库执行了相应的操作，才能升级到 SQL Database V12**。
...
可以使用 Powershell cmdlet 来启动、停止或监视从 Azure SQL Database V11 或其他任何低于 V12 的版本到 V12 的升级。
...
- [规划和准备升级到 SQL Database V12](sql-database-v12-plan-prepare-upgrade)
