...
1. 在“安装扩展”选项卡上指定以下值，然后选择“确定”。********
    ...
    - **运行列表**- 输入添加到计算机的 Chef 运行列表。 此值可以留空。
    - **验证客户端名称**- 输入 Chef 验证客户端名称。 例如， `tarcher-validator`.
    - **验证密钥** - 选择包含启动计算机时使用的验证密钥的文件。
    - **客户端配置文件**- 为 Chef-客户端选择配置文件。 此值可以留空。
    - **Chef 客户端版本** - 输入要安装的 Chef 客户端版本。 此值可以留空，从而安装最新版本。
    - **SSL 验证模式** - 选择“无”或“对等”。******** 对于演示，选择了“无”。**
    - **Chef 环境** - 输入此节点所属的 Chef 环境。 此值可以留空。
    - **加密数据包密钥**- 选择包含此计算机需要访问的加密数据包的机密的文件。 此值可以留空。
    - **Chef 服务器 SSL 证书**- 选择分配给 Chef 服务器的 SSL 证书。 此值可以留空。
...
1. 显示 **"创建**"选项卡时，您将看到所选和输入的选项的摘要。 验证信息以及**使用条款**，然后选择 **"创建**"。
