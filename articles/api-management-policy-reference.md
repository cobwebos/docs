...
本主题提供以下 API 管理（预览版）策略的参考信息。有关添加和配置策略的信息，请参阅 [API 管理中的策略][]。

-   [访问限制策略][]

    -   [用量配额][] - 可让你强制实施可续订或生存期调用量和/或带宽配额。
    -   [速率限制][] - 通过限制调用和/或带宽占用率来防止 API 用量尖峰。
    -   [调用方 IP 限制][] - 筛选（允许/拒绝）来自特定 IP 地址和/或地址范围的调用。
-   [内容转换策略][]

    -   [设置 HTTP 标头][] - 向现有的响应和/或请求标头赋值，或者添加新的响应和/或请求标头。
    -   [将 XML 转换为 JSON][] - 将请求或响应正文从 XML 转换为“JSON”或“遵从 XML”格式的 JSON。
    -   [替换正文中的字符串][] - 查找请求或响应子字符串并将其替换为不同的子字符串。
    -   [设置查询字符串参数][] - 添加、删除请求查询字符串参数或替换其值。
-   [缓存策略][]

    -   [存储到缓存][] - 根据指定的缓存配置来缓存响应。
    -   [从缓存中获取][] - 执行缓存查找，并返回有效的缓存响应（如果有）。
-   [其他策略][]

    -   [重写 URI][] - 将请求 URL 从其公用格式转换为 Web 服务所需的格式。
    -   [在内容中屏蔽 URLS][] - 重写（屏蔽）响应正文和位置标头中的链接，使其通过代理指向等效的链接。
    ...
    -   [CORS][] - 向操作或 API 添加跨源资源共享 (CORS) 支持，以便从基于浏览器的客户端执行跨域调用。
...
**说明：**
根据指定的缓存设置来缓存响应。必须已设置相应的[从缓存中获取][]策略。
...
**说明：**
执行缓存查找，并返回有效的缓存响应（如果有）。正确响应来自 API 使用者的缓存验证请求。必须已设置相应的[存储到缓存][]策略。
