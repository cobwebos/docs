<properties linkid="develop-php- Website-with-storage" urlDisplayName="Web w/ Storage" pageTitle="使用表存储的 PHP 网站 - Azure 教程" metaKeywords="Azure table storage PHP, Azure PHP  Website, Azure PHP  Website, Azure PHP tutorial, Azure PHP example" description="本教程演示如何创建 PHP 网站并在后端使用 Azure 表存储服务。" metaCanonical="" services="web-sites,storage" documentationCenter="PHP" title="Create a PHP  Website using Azure Storage" authors="" solutions="" manager="" editor="" />
...
![Azure PHP  Website][ws-storage-app]
...
* 然后，调用 *createTable* 以传入表的名称。与其他 NoSQL 表存储相似，Azure 表不需要架构。
	...
	可在此处找到错误代码和消息：[http://msdn.microsoft.com/library/azure/dd179438.aspx][msdn-errors]
...
* 若要在 Azure 表中查询存储在 *tasks* 表中存储的"所有实例"，可以调用仅传递表名称的*queryEntities* 方法。在下面的"更新实体"一节中，你还将了解如何传递查询特定实体的筛选器。
...
* 更新实体的第一步是从表中获取实体：
	...
	如您所见，传入查询筛选器的格式为 `Key eq 'Value'`。有关查询语法的完整说明，请参阅[此处][msdn-table-query-syntax]。
...
只需调用 `deleteItem` 即可完成对项目的删除。传入值为 **PartitionKey** 和 **RowKey**，二者共同构成了实体的主密钥。创建一个名为 **deleteitem.php** 的文件并插入以下代码：
...
3. 依次单击"数据服务"、"存储"、"快速创建"。
	...
	输入一个 **URL** 值并在 "区域"下拉列表中选择您的网站的数据中心。单击对话框底部的"创建存储帐户"按钮。
...
7. 打开 **init.php** 并将 `[YOUR_STORAGE_ACCOUNT_NAME]` 和 `[YOUR_STORAGE_ACCOUNT_KEY]` 替换为上一步中记录的帐户名和密钥。保存文件。
...
3. 单击"计算"、"网站"和"快速创建"。
	...
	输入一个 **URL** 值并在  "区域"下拉列表中选择您的网站的数据中心。单击对话框底部的"创建新网站"按钮。

	![Fill in  Website details][ Website-quick-create-details]

	创建了网站之后，您将看到文本"*网站 '[SITENAME]' 创建成功完成"。现在，您可以启用 Git 发布。
...
2. 打开 GitBash（或终端，如果 Git 在您 `PATH` 中），将目录更改为应用程序的根目录，并运行以下命令（**注意：**这些步骤与"创建 Azure 网站并设置 Git 发布"部分末尾所记录的步骤相同）：
...
3. 浏览到 **http://[your_Website_domain]/createtable.php** 以创建应用程序的表。
4. 浏览到 **http://[your_Website_domain]/index.php** 以开始使用应用程序。
...
3. 浏览到 **http://[your_Website_domain]/index.php** 以查看所做的更改。 
...
[msdn-errors]: http://msdn.microsoft.com/library/azure/dd179438.aspx



[msdn-table-query-syntax]: http://msdn.microsoft.com/library/azure/dd894031.aspx
