...
## <a name="example"></a>示例
例如，假设我们在做将信息发送到 Azure Blob 存储的[导出数据][编写器]模块的实验。 我们将定义名为“Blob 路径”的 Web 服务参数，该参数允许 Web 服务用户在服务受访问时更改指向 Blob 存储的路径。

1. 在机器学习工作室中，单击[导出数据][编写器]模块，将其选中。 属性显示在实验画布右侧的“属性”窗格中。
2. 指定存储类型：
...
3. 单击“指向以容器参数开头的 blob 的路径”右侧的图标。 如下所示：
   ...
   “属性”窗格底部的“Web 服务参数”下添加了名为“指向以容器参数开头的 blob 的路径”的条目。 这即是 Web 服务参数，此时已与此[导出数据][编写器]模块参数关联。
...
访问 Web 服务时，Web 服务的用户现在可为[导出数据][编写器]模块指定新目标。
...
有关访问机器学习 Web 服务的详细信息，请参阅[如何使用发布的机器学习 Web 服务](machine-learning-consume-web-services.md)。
