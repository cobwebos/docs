...
可以通过将 `PARAMETERS.py` 中的变量 `classifier` 从 `svm` 更改为 `dnn` 来训练神经网络（而不是 SVM）。 然后，如第 1 部分所述，需要再次执行除数据准备（步骤 1）和 SVM 训练（步骤 3）之外的所有脚本。 DNN 优化需要使用 GPU。 如果找不到 GPU，或者如果 GPU 已锁定（例如，被之前的 CNTK 运行锁定），则脚本 `2_refineDNN.py` 会引发一个错误。 DNN 训练可能会在某些 GPU 上引发内存不足的错误，但可以通过减少小批量大小（`PARAMETERS.py` 中的变量 `cntk_mb_size`）来避免此错误。
