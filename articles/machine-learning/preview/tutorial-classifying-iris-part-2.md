...
（可选）可以尝试对本地 Docker 容器运行脚本。 为此，需在本地的 Windows 或 macOS 计算机上安装并启动 Docker 引擎（Community Edition 已足够）。 详细阅读 [Docker 安装说明](https://docs.docker.com/engine/installation/)。

若要尝试调度脚本，使之在远程 Azure VM 或 HDInsight Spark 群集的 Docker 容器中运行，可以遵循[创建基于 Ubuntu 的 Azure 数据科学虚拟机或 HDI 群集的说明](how-to-create-dsvm-hdi.md)。
...
9. 在“作业”面板上的同一作业状态中，单击紧靠在“已完成”状态和开始时间上方的蓝色文本“iris_sklearn.py [n]”（_n_ 为运行编号）。 此时会打开“运行属性”页，其中显示了“运行属性”信息、“输出”文件、任何“视觉对象”，以及该特定运行的“日志”。 
