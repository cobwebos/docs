---
title: "如何在 Azure Machine Learning Workbench 中查找准确度最高且持续时间最短的运行 | Microsoft Docs"
...
---
# <a name="how-to-find-runs-with-the-best-accuracy-and-lowest-duration"></a>如何查找准确性最高且持续时间最短的运行
假定有多个运行，一个用例是查找具有最高准确度的运行。 一种方法是通过命令行界面 (CLI) 使用 [JMESPath](http://jmespath.org/)查询。 有关在 Azure CLI 中使用 JMESPath 的详细信息，请参阅[此文章](https://docs.microsoft.com/cli/azure/query-azure-cli?view=azure-cli-latest)。 在下面的示例中，创建了准确度值为 0、0.98、1 和 1 的四个运行。 当 `Threshold = .03` 时，如果运行处于范围 `[MaxAccuracy-Threshold, MaxAccuracy]` 中，则会对运行进行筛选。

## <a name="sample-data"></a>样本数据
如果没有具有 `Accuracy` 值的现有运行，则以下步骤将生成用于查询的运行：

首先，在 Workbench 中创建一个 python 文件，将其命名为 `log_accuracy.py`，然后将以下代码粘贴到其中：
```python
...
## <a name="querying-the-run-history"></a>查询运行历史记录
第一个命令查找最大准确度值。
```powershell
az ml history list --query '@[?Accuracy != null] | max_by(@, &Accuracy).Accuracy'
```

使用此最大准确度值 `1` 和阈值 `0.03`，第二个命令将使用 `Accuracy` 对运行进行筛选，然后按 `duration` 以升序对运行进行排序。
```powershell
