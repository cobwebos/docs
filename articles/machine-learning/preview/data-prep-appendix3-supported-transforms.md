...
description: "本文提供可用于在 Azure 机器学习中准备数据的转换的完整列表"
...
Azure 机器学习中的转换使用给定格式的数据、对数据执行操作（例如更改数据类型），然后以新格式生成数据。 每个转换有其自身的接口和行为。 可以通过数据流中的步骤将多个转换链接在一起，从而对数据执行复杂且可重复的转换。 这是数据准备功能的核心所在。

下面是可在 Azure 机器学习中使用的转换列表。 

## <a name="column-selection"></a>列选择 
下面列出的许多转换都适用于一个或多个列。 若要选择多个列，请使用 **Ctrl** 键；若要选择某个范围内的列，请使用 **Shift** 键。
...
右键菜单中只会提供对所选数据类型有效的转换。 主菜单提供所有转换，但禁用了与所选列不相关的转换。
...
若要使用此转换，请选择一个或多个列。 按示例添加一个新的（空白）派生列。 在派生列中键入想要看到的示例（假设它派生自其他列），“按示例”技术尝试填充列中的所有其他单元格。 

对于复杂的示例，有必要提供多个示例。 为此，请选择另一个单元格并键入另一个示例。

“按示例”技术使用所选列来尝试确定示例的含义。 调用此转换时，如果没有选择任何列，则使用当前行的所有单元格。 仅选择需要的列可以生成更准确的结果。

可以在调用转换前选择列。 启动转换编辑器后，每个列顶部的复选框会指示已选择哪些列作为输入。 可以使用列标题中的复选框来添加或删除可选择的列。

有关“按示例派生列”转换的详细说明以及更多示例，请参阅：[按示例派生列参考](data-prep-derive-column-by-example.md)  

## <a name="split-column-by-example"></a>按示例拆分列
此转换选取现有列并使用“按示例”引擎尝试将该列拆分为 *n* 个其他列。 可以在后续生成的列上运行自动拆分。

有关“按示例拆分列”转换的详细说明以及更多示例，请参阅：[按示例拆分列参考](data-prep-split-column-by-example.md)

## <a name="expand-json"></a>扩展 JSON
...
使用此转换时，将会分析单个列中的值的相似性并将其组合成聚集。 对于每个群集，都有一个规范值（该值是替换群集中所有实例的值）和实例值。 可以删除整个聚集，并可以编辑规范值。 可以从给定群集中删除实例。 此外，可以更改用于将实例组合成聚集的相似性分数阈值筛选器。

默认情况下，此转换将所有群集实例值替换为该群集的规范值，从而创建包含新值的新列。 还可以将每个实例的相似性分数添加到新列（可对其命名）以便稍后在数据流中使用。

## <a name="replace-values"></a>替换值
使用此转换可将一个字符串替换为另一个字符串。 源字符串可以是部分字符串或完整单元格；可向单个或多个列应用替换。 搜索字符串支持搜索特殊字符和常规字符。 

## <a name="replace-na-values"></a>替换 NA 值
使用此转换可将各种不同形式的 NA（N/A、NA、null、NaN 等）或空字符串替换为单个值，使其保持一致。 此转换支持一个或多个列。 仅当选中某列时才列出此转换，未选中任何列时将不会在主转换菜单上显示。

## <a name="replace-missing-values"></a>替换缺失值
此转换将缺失数据替换为单个值。 此转换支持一个或多个列。 仅当选中某列时才列出此转换，未选中任何列时将不会在主转换菜单上显示。

## <a name="replace-error-values"></a>替换错误值
此转换将误差替换为单个值。 此转换支持一个或多个列。 仅当选中某列时才列出此转换，未选中任何列时将不会在主转换菜单上显示。

## <a name="trim-string"></a>裁剪字符串
此转换从一个或多个列中删除前导和尾部“空白”字符（包括空格、制表符等）。

## <a name="adjust-precision"></a>调整精度
使用此转换可为数字列设置小数位数。

## <a name="rename-column"></a>重命名列
此转换更改选定列的名称。 还可以通过单击列名称，在列标题中以内联方式调用此转换。

## <a name="remove-column"></a>删除列
此转换删除选定的列。 此转换适用于单列或多列。 

## <a name="keep-column"></a>保留列
此转换仅保留选定的列。 此转换适用于单列或多列。

## <a name="handle-path-column"></a>图柄路径列
在导入文件期间，“添加数据源”向导会自动向数据集添加路径列。 它包含形成数据集路径的完全限定文件名。 此转换从数据集添加列或删除多余的列。

## <a name="convert-field-type-to-numeric"></a>将字段类型转换为数字
此转换将列类型更改为数字。 如果存在非整数数据，可以指定分隔符。 默认情况下，此转换不会提示输入分隔符；可使用“编辑”菜单项调用编辑器。 此转换适用于单列或多列。

## <a name="convert-field-type-to-date"></a>将字段类型转换为日期
此转换将列类型更改为日期。 使用默认日期/时间格式，但可以使用 `strftime` 指令对其重写。 还可以在前面附加包含固定日期的时间值。

默认情况下，此转换不会提示输入格式；可在最终步骤中使用“编辑”菜单项调用编辑器。 此转换适用于单列或多列。

仅能将介于 9-22-1677 和 4-11-2262 之间的日期转换为日期类型。

## <a name="convert-field-type-to-boolean"></a>将字段类型转换为布尔值
此转换将列类型更改为 true/false。 此转换支持将多个值映射为 true 或 false；可以编辑这些映射。 它还支持将值映射到一个 true/false 映射表中不存在的常量。 此转换适用于单列或多列。

## <a name="convert-field-type-to-string"></a>将字段类型转换为字符串
此转换将列类型更改为字符串。 此转换适用于单列或多列。

## <a name="convert-unix-timestamp-to-datetime"></a>将 Unix 时间戳转换为日期时间
此转换识别 Unix 时间戳格式（即使它在数据中表示为字符串），并在数据准备过程中将其正确地转换为日期类型。

## <a name="filter"></a>筛选器
此转换支持基于一个或多个列中的值筛选行。 筛选条件依赖于每个列的数据类型，因此，可按“包含”或“不包含”条件来筛选字符串列。 可按“大于”和“小于”条件来筛选数字列。

如果从主菜单或通过右键单击列标题来调用筛选转换，可将失败行分支到其他数据流。 然后，主数据流继续在筛选**入**行中执行，并创建包含筛选**出**的所有行的新数据流。

## <a name="use-first-row-as-headers"></a>将第一行用作标头
此转换将数据集中的第一行提升为列名称。 如果某些列在第一行中不包含数据，则系统会自动生成名称。 使用此转换意味着最早从行 2 开始导入数据。 根据跳过行设置，导入启动甚至可以在数据集中进一步细化。 它还可用于删除提升并仅使用自动生成的名称。

## <a name="join"></a>Join
此转换用于将两个数据流联接起来。 可以选择将哪个联接输出作为结果、联接中的成功行、联接中的“左”失败行或联接中的“右”失败行。

联接向导从单个数据流启动，并将该数据流选作联接的一侧。 然后，它会提示输入另一个数据流作为联接的另一侧。 选择这两个数据流后，向导要求选择接点每侧的单列以进行联接。 如果联接需要多列，则在启动向导前创建派生列，以创建仅供联接使用的新（串连）列。 该向导尝试建议联接键，如果可能，将自动生成派生列。

完成联接后，会显示联接的 Sankey 图示视图。 彼此相关的线条宽度表示通过联接流的该部分移动的行数。 在右侧面板中，可以单独选择成功的行、左侧失败行或右侧失败行。 还可以只选择一个分支。

## <a name="append-rows"></a>追加行
此转换将另一个数据流中的数据追加到当前数据流。 它会按位置映射列，以便在末尾添加新行。

## <a name="append-columns"></a>追加列
此转换将另一个数据流中的新列追加到当前数据流。 它在右侧添加新列；不会尝试排列行中的数据。
...
打开编辑器后，请从列标题向上拖动到左上角的面板中。 要聚合的列显示在此处。 此面板是分层的，因此可以执行嵌套聚合。 右上角的编辑器面板用于选择应用于列的聚合。 一个列可聚合一次或多次。 选择至少一个聚合后，右下方的网格在其聚合窗体中预览数据。 

此转换类似于 `ANSI-SQL GROUP BY`。

## <a name="remove-duplicates"></a>删除重复项
此转换删除一个或多个选定列中存在重复值的整行。 如果未选择任何列，则删除的行正是所有列值都相同的行。

## <a name="sort"></a>排序
此转换对数据排序。 可按一个或多个列排序，每个列可按升序（默认）或降序（可在编辑器中更改）排序。

此转换类似于 `ANSI-SQL ORDER BY`。

## <a name="output-transforms"></a>输出转换
以下转换会输出数据。 可在单个流中包含多个写入块，以便在不同的时间点写出数据。
...
以下转换使用脚本 (Python) 执行核心产品中缺少的功能。 使用其中任一转换前，请阅读[使用 Python 扩展性](data-prep-python-extensibility-overview.md)。

### <a name="add-column-script"></a>添加列（脚本）
通过此转换可以使用 Python 表达式将列添加到数据。
