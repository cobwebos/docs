---
title: "有关如何使用 Azure 机器学习数据准备执行 API 的深度指南 | Microsoft Docs"
...
要在 Python 中使用数据源或数据准备包，请执行以下操作：
- 导航到项目的“数据”选项卡。
- 右键单击相应的源。
- 选择“生成数据访问代码文件”。
...
- `run_on_data(user_config, package_path, dataflow_idx=0, secrets=None, spark=None)` - 基于内存数据源运行指定的数据流，并将结果作为数据帧返回。 `user_config` 参数是一个字典，它将数据源（.dsource 文件）的绝对路径映射到以一列列表的形式表示的内存中数据源。
