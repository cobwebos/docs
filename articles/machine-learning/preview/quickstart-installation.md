...
5. 选择“创建”，开始试验帐户和模型管理帐户的创建过程。
   ...
   创建帐户可能需要一些时间。 单击 Azure 门户工具栏上的钟形图标即可查看部署过程的状态。
...
1. 在第一个屏幕上选择“登录”，使用 Azure Machine Learning Workbench 进行身份验证。 使用在 Azure 门户中使用过的相同凭据创建试验帐户和模型管理帐户。 

   在你登录后，Workbench 会使用其在 Azure 订阅中找到的第一个试验帐户。  Workbench 使用找到的第一个试验帐户，并显示与该帐户关联的所有工作区和项目。 
...
现在可以在本地计算机上运行 **iris_sklearn.py** 脚本了。 默认情况下，此脚本是“鸢尾花分类”项目模板随附的。 此脚本使用常用 Python [scikit-learn](http://scikit-learn.org/stable/index.html) 库中的[逻辑回归](https://en.wikipedia.org/wiki/logistic_regression)算法来生成模型。
...
1. 在“参数”文本框中输入 **0.01**。 此数字在脚本代码中用于设置正则化率。 此值用于配置线性回归模型的定型方式。 

1. 选择“运行”，开始在计算机上执行脚本。 **iris_sklearn** 作业会立即显示在右侧的“作业”面板中，方便你监视脚本的执行情况。

   祝贺你！ 你已在 Azure Machine Learning Workbench 中成功运行了 Python 脚本。

1. 使用 **0.001** 到 **10** 范围内的不同参数值重复执行步骤 2 - 3 多次。 每个执行作业都会显示在“作业”窗格中。

1. 检查运行历史记录，方法是：先选择“运行”视图，然后选择“运行”列表中的“iris_sklearn.py”，以便显示此脚本的运行历史记录。 

   ![运行历史记录仪表板](media/quickstart-installation/run_view.png)

   其中显示了在 **iris_sklearn.py** 中执行的每个运行。 运行历史记录仪表板还显示最常见的指标、一组默认图形和每个运行的指标列表。 

1. 可以使用齿轮或筛选器图标，通过对配置执行排序、筛选和调整操作来自定义此视图。
...
可以在 Workbench 的工具栏中使用“文件”-->“打开命令提示符”来启动 azure-cli-ml CLI。

可以在 azure-cli-ml CLI 中使用 --help 参数获取与命令相关的帮助。
...
现在已创建必需的 Azure 机器学习帐户并安装 Azure Machine Learning Workbench 应用程序。 在该应用程序中，已创建项目、运行脚本并浏览脚本的运行历史记录。
...
>[!NOTE]
> 虽然你已经有了模型管理帐户，但尚未将环境设置为适合部署 Web 服务的环境。  了解如何设置[部署环境](deployment-setup-configuration.md)。