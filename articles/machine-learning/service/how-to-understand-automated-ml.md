...
log_loss|这是（多项式） 逻辑回归及其扩展（例如神经网络）中使用的损失函数，在给定概率分类器的预测的情况下，定义为真实标签的负对数可能性。 对于包含 true 标签 ca-yt 的单个样本，{0,1} 在 ca-yt = 1 的情况下估计概率 yp，日志丢失为-log P （&#124;ca-yt yp） =-（ca-yt log （yp） + （1-ca-yt） log （））。|[计算](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.log_loss.html)|None|
