...
| properties | 描述 |
| ----- | ----- |
| `azureml.git.repository_uri` | 从中克隆存储库的 URI。 |
| `mlflow.source.git.repoURL` | 从中克隆存储库的 URI。 |
| `azureml.git.branch` | 提交运行时的活动分支。 |
| `mlflow.source.git.branch` | 提交运行时的活动分支。 |
| `azureml.git.commit` | 提交给运行的代码的提交哈希。 |
| `mlflow.source.git.commit` | 提交给运行的代码的提交哈希。 |
| `azureml.git.dirty` | 如果提交处于脏，则为 `True`;否则 @no__t 为-1。 |
...
1. 选择 "__日志__"，然后展开 "__日志__" 和 " __azureml__ " 条目。 选择以 __### @ no__t-2azure__开头的链接。
...
@No__t-0 CLI 命令可用于从运行中检索属性。 例如，以下命令将返回名为 `train-on-amlcompute` 的试验中最后一次运行的属性：
