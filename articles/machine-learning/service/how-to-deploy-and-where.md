...
+ **使用 SDK**
  ...
  使用 SDK 训练模型时，可以接收[Run](https://review.docs.microsoft.com/python/api/azureml-core/azureml.core.run.run?view=azure-ml-py&branch=master)或[AutoMLRun](https://review.docs.microsoft.com/python/api/azureml-train-automl/azureml.train.automl.run.automlrun?view=azure-ml-py&branch=master)对象，具体取决于模型的定型方式。 每个对象都可用于注册由试验运行创建的模型。
  ...
  + 从`azureml.core.Run`对象注册模型：
    ...
    `model_path`引用模型的云位置。 在此示例中，使用单个文件的路径。 若要在模型注册中包含多个文件`model_path` ，请将设置为包含这些文件的目录。 有关详细信息，请参阅[register_model](https://review.docs.microsoft.com/python/api/azureml-core/azureml.core.run.run?view=azure-ml-py&branch=master#register-model-model-name--model-path-none--tags-none--properties-none--model-framework-none--model-framework-version-none--description-none--datasets-none----kwargs-)引用。
  ...
  + 从`azureml.train.automl.run.AutoMLRun`对象注册模型：
    ...
    在此示例中， `metric`未`iteration`指定和参数，这将导致注册具有最佳主要指标的迭代。 使用从运行返回的值，而不是模型名称。`model_id`

    有关详细信息，请参阅[AutoMLRun. register_model](https://review.docs.microsoft.com/python/api/azureml-train-automl/azureml.train.automl.run.automlrun?view=azure-ml-py&branch=master#register-model-description-none--tags-none--iteration-none--metric-none-) reference。
...
推理配置可以使用 Azure 机器学习环境来定义部署所需的软件依赖项。 利用环境，你可以创建、管理和重复使用培训和部署所需的软件依赖项。 下面的示例演示如何从工作区加载环境，然后将其用于推理配置：
...
在将模型部署为服务之前，您可能需要对其进行分析，以确定最佳的 CPU 和内存要求。 可以使用 SDK 或 CLI 分析模型。 下面的示例演示如何从 SDK 使用分析：

> [!IMPORTANT]
> 使用分析时，所提供的推理配置无法引用 Azure 机器学习环境。 而是使用`conda_file` `InferenceConfig`对象的参数定义软件依赖项。
...
在某些情况下，可能需要在不部署模型的情况下创建 Docker 映像。 例如，在你计划[部署到 Azure App Service](how-to-deploy-app-service.md)时。 或者，你可能想要下载映像并在本地 Docker 安装上运行。 甚至可能需要下载用于生成映像的文件、对其进行检查、修改并手动生成。

模型打包使你可以执行这两项操作。 它打包了将某个模型托管为 web 服务所需的所有资产，并使你可以下载完全生成的 Docker 映像或生成它所需的文件。 可以通过两种方式使用模型打包：

* __下载打包模型__：下载包含模型的 Docker 映像以及将其作为 web 服务托管所需的其他文件。
* __生成 dockerfile__：下载生成 Docker 映像所需的 dockerfile、model、entry 脚本和其他资产。 然后，你可以在本地生成映像之前检查文件或进行更改。

这两个包都可用于获取本地 Docker 映像。 

> [!TIP]
> 创建包类似于部署模型，因为它使用已注册的模型和推理配置。

> [!IMPORTANT]
> 在本地下载完全生成的映像或构建映像等功能需要在开发环境中[进行工作。](https://www.docker.com)

### <a name="download-a-packaged-model"></a>下载打包模型

下面的示例演示如何在工作区的 Azure 容器注册表中生成已注册的映像：
...
创建包后，可以使用`package.pull()`将映像提取到本地 Docker 环境。 此命令的输出将显示映像的名称。 例如， `Status: Downloaded newer image for myworkspacef78fd10.azurecr.io/package:20190822181338` 。 下载后，使用`docker images`命令列出本地映像：
...
若要使用此映像启动本地容器，请使用以下命令从 shell 或命令行启动命名容器。 将值替换为从命令返回的映像 ID： `docker images` `<imageid>`

```bash
docker run -p 6789:5001 --name mycontainer <imageid>
```

此命令启动最新版本的映像`myimage`。 它将6789的本地端口映射到 web 服务正在侦听的容器中的端口（5001）。 它还为容器指定`mycontainer`名称，从而使其更易于停止。 启动后，你可以将请求提交`http://localhost:6789/score`给。

### <a name="generate-dockerfile-and-dependencies"></a>生成 dockerfile 和依赖项

下面的示例演示如何下载 dockerfile、模型以及在本地构建映像所需的其他资产。 `generate_dockerfile=True`参数指示需要文件，而不是完全生成的映像：
...
此代码将生成映像所需的文件下载到`imagefiles`目录。 保存文件中包含的 dockerfile 引用存储在 Azure 容器注册表中的基本映像。 在本地 Docker 安装上生成映像时，必须使用地址、用户名和密码对此注册表进行身份验证。 使用以下步骤来构建使用本地 Docker 安装的映像：

1. 在 shell 或命令行会话中，使用以下命令通过 Azure 容器注册表对 Docker 进行身份验证。 使用`<address>` `<username>` `<password>`检索的值替换、和： `package.get_container_registry()`
...
有关其他编程语言中的客户端的更多示例，请参阅[使用部署为 web 服务的模型](how-to-consume-web-service.md)。
