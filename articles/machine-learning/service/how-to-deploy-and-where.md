...
- [Azure 容器实例 (ACI)](#aci)：快速部署。 适用于开发或测试。
- [Azure Kubernetes 服务 (AKS)](#aks)：适用于大规模的生产部署。 提供了自动缩放和快速的响应时间。
- [Azure IoT Edge](#iotedge)：在 IoT 设备上部署模型。 推断在设备上进行。
- [现场可编程门列阵 (FPGA)](#fpga)：用于实时推断的超低延迟。

本文档的其余部分详细介绍了以上每个选项。

## <a id="aci"></a>Azure 容器实例
...
## <a id="aks"></a>Azure Kubernetes 服务

对于大规模的生产方案，请使用 Azure Kubernetes 服务 (AKS)。 可以使用现有 AKS 群集，也可使用 Azure 机器学习 SDK、CLI 或 Azure 门户创建新群集。
...
使用 IoT 设备，可更加快速地在设备上执行评分，而无需将数据发送到云进行评分。 使用 Azure IoT Edge，可在边缘设备上托管模型。 如果需要一个或多个以下功能，请将模型部署到 IoT Edge：
- 在本地处理优先级任务，甚至无需云连接
- 所用生成数据太大，无法快速从云端拉取
...
## <a id="fpga"></a>现场可编程门阵列 (FPGA)

使用 Project Brainwave 提供支持的硬件加速模型，可以实时推断请求的超低延迟。 Project Brainwave 可加快部署于 Azure 云中现场可编程门阵列上的深度神经网络 (DNN) 的速度。 常用 DNN 可用作传输学习的特征化器，也可以使用基于你自己的数据训练的权重进行自定义。
