...
| [设计器](ui-concept-visual-interface.md)（预览） | 拖放机器学习建模工具（以前称为设计器） | Enterprise 版 |
...
+ **Bug 修复和改进**
  + **azure-cli-ml**
    + CLI 现在支持模型打包。
    + 添加了数据集 CLI。 有关详细信息，请参阅：`az ml dataset --help`
...
+ **Bug 修复和改进**
  ...
  + **azureml-core**
    ...
    + 不再需要 "gpu_support";AzureML 现在会自动检测并使用 nvidia docker 扩展（如果可用）。 未来的版本会将其删除。
