...
* **将文件存储在 Azure 机器学习[数据](https://docs.microsoft.com/python/api/azureml-core/azureml.data?view=azure-ml-py)存储中。** 这会阻止试验延迟问题, 并具有从远程计算目标访问数据的优点, 这意味着身份验证和装载由 Azure 机器学习服务管理。 详细了解如何将数据存储指定为源目录, 并将文件上传到数据存储中的数据存储文章中的[访问数据](how-to-access-data.md)。
