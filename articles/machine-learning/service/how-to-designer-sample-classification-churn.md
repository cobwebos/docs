...
![管道图形](./media/how-to-designer-sample-classification-predict-churn/pipeline-graph.png)
...
- 原始数据集具有多个缺失值。 使用 "**清理缺失数据**" 模块将缺失值替换为0。
    ...
    ![清理数据集](./media/how-to-designer-sample-classification-predict-churn/cleaned-dataset.png)

- 功能和相应的改动在不同的数据集中。 使用 "**添加列**" 模块将标签列追加到特征列。 第一列**Col1**是标签列。 从可视化结果中可以看到，数据集不均衡。 有比正示例更多的负数（-1）示例（+ 1）。 稍后，我们将使用**SMOTE**模块增加常见情况。

    ![添加列数据集](./media/how-to-designer-sample-classification-predict-churn/added-column1.png)
...
![评估结果](./media/how-to-designer-sample-classification-predict-churn/evaluate-result.png)
