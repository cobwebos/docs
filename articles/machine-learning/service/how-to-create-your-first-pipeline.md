...
description: 使用适用于 Python 的 Azure 机器学习 SDK 创建和运行机器学习管道。  管道用于创建和管理将机器学习 (ML) 阶段（例如数据准备、模型训练、模型部署和推断）拼接在一起的工作流。
...
ms.locfileid: "53269033"
---
# <a name="create-and-run-a-machine-learning-pipeline-using-azure-machine-learning-sdk"></a>使用 Azure 机器学习 SDK 创建和运行机器学习管道
...
管道使用远程计算目标进行计算，以及存储与该管道关联的中间数据和最终数据。  管道可以在支持的 [Azure 存储](https://docs.microsoft.com/azure/storage/)位置读取和写入数据。
...
* 配置一个指向驻留在数据存储中的数据或者可在数据存储中访问的数据的 `DataReference` 对象。
...
创建工作区时，默认会将 [Azure 文件存储](https://docs.microsoft.com/azure/storage/files/storage-files-introduction)和 [Blob 存储](https://docs.microsoft.com/azure/storage/blobs/storage-blobs-introduction)附加到工作区。  Azure 文件存储是工作区的“默认数据存储”，但你也可以使用 Blob 存储作为数据存储。  详细了解 [Azure 存储选项](https://docs.microsoft.com/azure/storage/common/storage-decide-blobs-files-disks)。 
...
将数据文件或目录上传到数据存储，以便可以在管道中访问它们。  此示例使用 Blob 存储版本的数据存储：
...
在 Azure 机器学习中，计算（或计算目标）是指将要在机器学习管道中执行计算步骤的计算机或群集。 例如，可以创建 Azure 机器学习计算用于运行步骤。
...
现在，可以开始定义管道步骤。 可以通过 Azure 机器学习 SDK 使用许多内置步骤。 其中最基本的步骤是 `PythonScriptStep`，它在指定的计算目标中执行一个 Python 脚本。
...
提交管道时，将检查每个步骤的依赖项，并将指定为源目录的文件夹的快照上传到 Azure 机器学习服务。  如果未指定源目录，则上传当前的本地目录。
...
* 步骤在步骤定义中指定的计算目标上运行。 
* 创建项目，例如日志、stdout 和 stderr、指标，以及步骤指定的输出。 然后上传这些项目并将其保存在用户的默认数据存储中。

![以管道的形式运行试验](./media/how-to-create-your-first-pipeline/run_an_experiment_as_a_pipeline.png)
...
所有已发布的管道都有一个 REST 终结点，该终结点可以从非 Python 客户端等外部系统调用管道的运行。 在批量评分和重新训练方案中，此终结点提供一种“受控的可重复性”方式。

若要调用上述管道的运行，需要使用 [AzureCliAuthentication 类](https://docs.microsoft.com/python/api/azureml-core/azureml.core.authentication.azurecliauthentication?view=azure-ml-py)中所述的 Azure Active Directory 身份验证标头令牌
