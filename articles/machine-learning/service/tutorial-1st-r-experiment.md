...
1. 按照[安装说明](https://azure.github.io/azureml-sdk-for-r/articles/installation.html)执行以下操作：
    + 安装 Anaconda
    + 安装 `azuremlsdk`
    + 安装用于 Python 的 Azure 机器学习 SDK

1. 从 [GitHub](https://github.com/Azure/azureml-sdk-for-r/tree/master/vignettes/train-and-deploy-with-caret) 获取三个教程文件。  将这些文件保存到 **tutorials** 目录中。

2. 使用以下步骤创建 Azure 机器学习工作区并下载其配置文件。


### <a name="create-a-workspace"></a>创建工作区

Azure 机器学习工作区是云中的基础资源，用于试验、训练和部署机器学习模型。 它将 Azure 订阅和资源组关联到 SDK 中一个易于使用的对象。 如果你已有 Azure 机器学习工作区，请[跳转到下一部分](#config)。 否则，请立即创建一个。

[!INCLUDE [aml-create-portal](../../../includes/aml-create-in-portal.md)]

### <a name="config"></a> 下载 config.json 文件

1. 在工作区页的顶部，选择“下载 config.json”。 

    ![下载 config.json 文件](media/tutorial-1st-r-experiment/download-config.png)  

1. 将此文件添加到 **tutorials** 目录。

现在可以开始运行本教程。

我们建议使用 RStudio 运行本教程。 在 RStudio 中，选择“文件”>“新建项目”>“现有目录”，然后选择前面创建的“tutorials”目录。 

> [!Important]
> 本文的余下部分包含 **train-and-deploy-to-aci.Rmd** 文件中所示的相同内容。  
> 如果你有 RMarkdown 方面的经验，可随意使用该文件中的代码。  或者，可将该文件或本文中的代码片段复制/粘贴到 R 脚本或命令行中。
