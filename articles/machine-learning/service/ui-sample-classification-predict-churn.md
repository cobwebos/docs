...
因为我们正在尝试回答 "哪一个问题？" 这称为分类问题。 不过, 您可以在此试验中应用相同的步骤, 以解决任何类型的机器学习问题, 无论是回归、分类、群集等。
...
用于此试验的数据来自 KDD 杯2009。 数据集包含50000行和230功能列。 该任务旨在预测使用这些功能的客户的变动量、亲和力和向上销售情况。 有关数据和任务的详细信息, 请参阅[KDD 网站](https://www.kdd.org/kdd-cup/view/kdd-cup-2009)。
...
- 原始数据集包含大量缺少值。 我们使用 "**清理缺失数据**" 模块将缺失值替换为0。

    ![清理数据集](./media/ui-sample-classification-predict-churn/cleaned-dataset.png)

- 功能和相应的改动、亲和力以及销售标签位于不同的数据集中。 我们使用 "**添加列**" 模块将标签列附加到功能列。 第一列**Col1**是标签列。 其余列、 **Var1**、 **Var2**等是功能列。

    ![添加列数据集](./media/ui-sample-classification-predict-churn/added-column1.png)

- 我们使用**拆分数据**模块将数据集拆分为定型集和测试集。

    然后, 将提升决策树二进制分类器与默认参数一起使用, 以生成预测模型。 我们每个任务都生成一个模型, 即每个模型预测向上销售、亲和力和变动。

## <a name="results"></a>结果

可视化 "**评估模型**" 模块的输出, 以查看该模型在测试集上的性能。 对于向上销售的任务, ROC 曲线显示模型确实比随机模型更好。 曲线 (AUC) 下的区域为0.857。 阈值为 0.5, 精度为 0.7, 召回为 0.463, F1 分数为0.545。
