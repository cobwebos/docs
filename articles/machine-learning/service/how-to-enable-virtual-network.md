...
1. 在虚拟网络后创建计算实例（例如机器学习计算实例），或将计算实例附加到工作区（例如 HDInsight 群集、虚拟机或 Azure Kubernetes Service 群集）。 计算实例可以用于试验或模型部署。

   有关详细信息，请参阅[使用机器学习计算实例](#amlcompute)，[使用虚拟机或 HDInsight 群集](#vmorhdi)，并使用本文中的[Azure Kubernetes Service](#aksvnet)部分。
...
1. 在 "__防火墙和虚拟网络__" 页上，执行以下操作：
    - 选择“所选网络”。
    - 在 "__虚拟网络__" 下，选择 "__添加现有虚拟网络__" 链接。 此操作将添加计算实例所在的虚拟网络（请参阅步骤1）。

        > [!IMPORTANT]
        > 存储帐户必须与用于定型或推理的计算实例位于同一虚拟网络中。
...
> 创建工作区时，会自动预配默认存储帐户。
>
> 对于非默认存储帐户，此`storage_account` [ `Workspace.create()`函数](https://docs.microsoft.com/python/api/azureml-core/azureml.core.workspace(class)?view=azure-ml-py#create-name--auth-none--subscription-id-none--resource-group-none--location-none--create-resource-group-true--friendly-name-none--storage-account-none--key-vault-none--app-insights-none--container-registry-none--default-cpu-compute-target-none--default-gpu-compute-target-none--exist-ok-false--show-output-true-)中的参数允许你按 Azure 资源 ID 指定自定义存储帐户。
...
> * 如果工作区的 Azure 存储帐户还在虚拟网络中受保护，则它们必须位于与 Azure 机器学习计算实例相同的虚拟网络中。
...
你还可以使用 Azure 机器学习 SDK 将 Azure Kubernetes 服务添加到虚拟网络中。 如果虚拟网络中已有 AKS 群集，请将其附加到工作区，如[如何部署到 AKS](how-to-deploy-to-aks.md)中所述。 以下代码在名为`default` `mynetwork`的虚拟网络的子网中创建新的 AKS 实例：
