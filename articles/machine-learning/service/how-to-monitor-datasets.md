...
| 设置 | 说明 | 提示 | 可变 | 
| ------- | ----------- | ---- | ------- |
| 启用 | 启用或禁用数据集监视管道上的计划 | 禁用此选项可通过回填设置分析历史数据。 它可以在创建数据集监视器后启用。 | 是 | 
| Latency | 数据到达数据集所需的时间（以小时为单位）。 例如，如果数据在 SQL 数据库 "我的数据集" 中的封装时间为三天，则将延迟设置为72。 | 创建数据集监视器后无法更改 | 否 | 
...
| ------- | ----------- | ---- |
| 开始日期 | 回填作业的开始日期。 | | 
| 结束日期 | 回填作业的结束日期。 | 此时间不能超过 31 * 从开始日期起的时间单位。 在现有数据集监视器上，可以回填指标分析历史数据或将指标替换为更新的设置。 |
...
还会在一段时间内分析目标数据集。 每个功能的基线分布之间的统计距离与目标数据集的随时间进行比较，这在概念上类似于数据偏移量，但这是针对单个功能的。 最小值、最大值和平均值也可用。 
...
| 数值 | int、float | 的数据类型，不满足分类功能的条件。 | 如果 > 15% 的值为 null，则删除功能。 | 
