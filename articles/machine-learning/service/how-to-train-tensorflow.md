...
ms.locfileid: "46974215"
---
# <a name="how-to-train-tensorflow-models"></a>如何训练 TensorFlow 模型

对于使用 TensorFlow 的深度神经网络 (DNN) 训练，Azure 机器学习提供了估算器的自定义 TensorFlow 类。 Azure SDK 的 TensorFlow 估算器（不与 [`tf.estimator.Estimator`](https://www.tensorflow.org/api_docs/python/tf/estimator/Estimator) 类合并）可使你轻松地为 Azure 计算上的单节点和分布式运行提交 TensorFlow 训练作业。

## <a name="single-node-training"></a>单节点训练
使用 TensorFlow 估算器进行训练与使用[基础估算器](how-to-train-ml-models.md)类似，因此，请首先请阅读操作说明文章并确保你了解其中介绍的概念。
...
* `conda_packages`：要通过训练脚本所需的 conda 安装的 Python 包列表。 在这种情况下，训练脚本使用 `sklearn` 来加载数据，因此请指定要安装的包。  
构造函数具有名为 `pip_packages` 的另一个参数，可以将其用于任何所需的 pip 包
* `use_gpu`：将此标记设置为 `True` 以利用 GPU 进行训练。 默认为 `False`。
...
1. 使用 [Horovod](https://github.com/uber/horovod) 框架进行基于 MPI 的分布式训练
2. 通过参数服务器方法进行的本机[分布式 TensorFlow](https://www.tensorflow.org/deploy/distributed)
...
* `process_count_per_node`：要在每个节点上运行的进程（或“工作线程”）数。 此参数默认为 `1`
* `distributed_backend`：用于启动由估算器通过 MPI 提供的分布式训练的后端。 此参数默认为 `None`。 如果要使用 MPI（和 Horovod）执行并行或分布式训练（例如，`node_count`>1 或 `process_count_per_node`>1，或两者），请设置 `distributed_backend='mpi'`。 Azure 机器学习使用的 MPI 实现是 [Open MPI](https://www.open-mpi.org/)。
...
* `worker_count`：工作线程数。 此参数默认为 `1`
* `parameter_server_count`：参数服务器数。 此参数默认为 `1`
* `distributed_backend`：用于分布式训练的后端。 此参数默认为 `None`。 要通过参数服务器进行分布式训练，需要设置 `distributed_backend='ps'`
