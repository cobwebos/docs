...
ms.locfileid: "46977020"
---
# <a name="how-to-train-pytorch-models"></a>如何训练 PyTorch 模型

对于使用 PyTorch 的深度神经网络 (DNN) 训练，Azure 机器学习提供了估算器的自定义 PyTorch 类。 Azure SDK 的 PyTorch 估算器使你能够轻松为 Azure 计算上的单节点和分布式运行提交 PyTorch 训练作业。

## <a name="single-node-training"></a>单节点训练
使用 PyTorch 估算器进行训练与使用[基础估算器](how-to-train-ml-models.md)类似，因此，请首先请阅读操作说明文章并确保你了解其中介绍的概念。
...
由于你使用的是 PyTorch 估算器，因此，用于训练的容器将默认包含 PyTorch 包，以及在 CPU 和 GPU 上进行训练所需的相关依赖项。
