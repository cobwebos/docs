...
在本教程中, 已经提供训练`pytorch_train.py`脚本。 在实践中, 您可以采用任何自定义训练脚本, 并使用 Azure 机器学习服务运行它。
...
但是, 如果你想要使用 Azure 机器学习服务跟踪和指标功能, 则必须在训练脚本中添加少量代码。 可以在中`pytorch_train.py`找到指标跟踪的示例。
...
[`PyTorch`](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.dnn.pytorch?view=azure-ml-py)估计器还支持跨 CPU 和 GPU 群集的分布式培训。 您可以轻松地运行分布式 PyTorch 作业, Azure 机器学习服务将管理业务流程。
...
本文介绍了如何在 Azure 机器学习服务上使用 PyTorch 训练和注册深度学习和神经网络。 若要了解如何部署模型, 请继续学习我们的模型部署一文。
