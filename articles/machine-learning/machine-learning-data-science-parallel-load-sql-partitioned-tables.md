...
根据映射到在上一步中创建的数据库文件组的数据架构，创建分区表。 当将数据批量导入到分区表时，将根据分区方案在文件组之中分布记录，如下所述。
...
* [创建分区表](https://msdn.microsoft.com/library/ms174979.aspx)（根据你的数据架构），并指定用于对表进行分区的分区方案和约束字段，例如：
...
* 你可以使用 BCP、BULK INSERT 或其他方法（如 [SQL Server 迁移向导](http://sqlazuremw.codeplex.com/)）。 提供的示例使用 BCP 方法。
...
* 如果你从多个表中提取数据进行建模，请在联接键上创建索引来提高联接性能。
* [创建索引](https://technet.microsoft.com/library/ms188783.aspx)（群集或非群集）为每个分区锁定相同文件组，例如：
  ...
  > [!NOTE]
  > 你可以选择创建索引后再批量导入数据。 进行批量导入之前创建索引将会降低数据加载速度。
  > 
  > 

## <a name="advanced-analytics-process-and-technology-in-action-example"></a>操作示例中的高级分析流程和技术
有关 Cortana Analytics 过程中使用公用数据集的端到端演练示例，请参阅[操作中的 Cortana Analytics 过程：SQL Server](machine-learning-data-science-process-sql-walkthrough.md)。
