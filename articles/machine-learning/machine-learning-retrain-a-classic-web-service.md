...
部署的预测 Web 服务是默认的评分终结点。 默认终结点与原始训练和评分实验保持同步，因此不能替换默认终结点的训练模型。 若要重新训练 Web 服务，必须将新的终结点添加到 Web 服务。 
...
3. 单击 **“添加”**。
4. 键入新终结点的名称及说明。 选择日志记录级别以及是否启用示例数据。 有关日志记录的详细信息，请参阅[为机器学习 Web 服务启用日志记录](machine-learning-web-services-logging.md)。

### <a name="use-the-azure-classic-portal-to-add-an-endpoint"></a>使用 Azure 经典门户添加终结点
1. 登录到[经典 Azure 门户](https://manage.windowsazure.com)。
2. 在左侧菜单中，单击“机器学习”。
3. 在“名称”下，单击你的工作区，然后单击“Web 服务”。
4. 在“名称”下，单击“人口普查模型 [预测实验]”。
5. 单击页面底部的“添加终结点”。 有关添加终结点的详细信息，请参阅[创建终结点](machine-learning-create-endpoint.md)。 

## <a name="update-the-added-endpoints-trained-model"></a>更新已添加终结点的训练模型
若要完成重新训练过程，必须更新已新添加的终结点的训练模型。

* 如果使用经典 Azure 门户添加了新终结点，可以在门户中单击新终结点的名称，然后单击“更新资源”链接以获取更新终结点的模型所需的 URL。
...
如果在完成更新终结点之前你的 SAS 令牌到期，必须使用作业 ID 执行 GET 操作，才能获取全新令牌。
