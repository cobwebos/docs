...
> [!NOTE]
> 本教程介绍各种基础知识：如何将模块拖放到试验中、如何将模块连接到一起、如何运行试验，以及如何查看结果。 我们不讨论机器学习的常规主题，不讨论如何选择和使用工作室中包括的 100 多种内置算法和数据操作模块。
>
...
- [机器学习算法备忘](machine-learning-algorithm-cheat-sheet.md)和[如何选择 Microsoft Azure 机器学习的算法](machine-learning-algorithm-choice.md) - 可供用户下载的海报及深度讨论工作室算法的相关文章。
...
在本机器学习教程中，需遵循以下五个基本步骤在机器学习工作室中构建试验，以便创建模型、对模型定型以及对模型进行评分：
...
![单击输出端口，然后选择“可视化”][select-visualize]
<br/>
***单击输出端口，然后选择“可视化”***
...
若要找到一组理想的特征来创建预测模型，需要针对要解决的问题进行试验，并且具有相关知识。 有些特征比其他特征更适合用于预测目标。 另外，某些特征与其他特征有很强的关联性，可以将其删除。 例如，city-mpg（市区油耗）和 highway-mpg（高速公路油耗）密切相关，因此可以保留一个，删除另一个，不会对预测对明显影响。

让我们构建一个模型，它使用数据集中的一部分特征。 可在以后返回来选择不同的特征，再次运行试验，然后确认是否获得了理想的结果。 不过，让我们先尝试使用以下特征：
...
4. 在“开头为”下面，单击“没有列”。 在筛选器行中，选择“包括”和“列名”，然后在文本框中选择列名列表。 这会指示模块不要传入任何列（特征），我们指定的列除外。
...
> [!TIP]
> 若需详细了解不同类型的机器学习算法和使用时机，可以观看“适合初学者的数据科学”系列的第一个视频：[数据科学回答的五大问题](machine-learning-data-science-for-beginners-the-5-questions-data-science-answers.md)。 也可参阅带信息图的[机器学习基础知识及算法示例](machine-learning-basics-infographic-with-algorithm-examples.md)，或查看[机器学习算法备忘](machine-learning-algorithm-cheat-sheet.md)。
