---
title: 适用于 Azure 机器学习的 Azure 安全基线
...
适用于 Microsoft Azure 机器学习的 Azure 安全基线包含有助于改进部署安全状况的建议。 此服务的基线摘自 [Azure 安全基准版本 1.0](../security/benchmarks/overview.md)，其中提供了有关如何根据我们的最佳做法指导保护 Azure 上的云解决方案的建议。 有关详细信息，请参阅 [Azure 安全基线概述](../security/benchmarks/security-baselines-overview.md)。
...
**指南**： Azure 机器学习依赖于其他 Azure 服务计算资源。 计算资源（计算目标）用于训练和部署模型。 可以在虚拟网络中创建这些计算目标。 例如，你可以使用 Azure Virtual 机器学习计算实例来训练模型，然后将该模型部署到 Azure Kubernetes Service (AKS) 。 可以通过在 Azure 虚拟网络中隔离 Azure 机器学习定型和推理作业来保护机器学习生命周期。
...
**指南**： Azure 机器学习依赖于其他 Azure 服务计算资源。 将网络安全组分配给作为机器学习部署创建的网络。 

启用网络安全组流日志，并将日志发送到 Azure 存储帐户进行审核。 你还可以将流日志发送到 Log Analytics 工作区，然后使用流量分析提供对 Azure 云中流量模式的见解。 流量分析的一些优点是可以可视化网络活动、识别热点和安全威胁、了解流量流模式以及查明网络配置错误。
...
**指南**：你可以使用 HTTPS 来保护与 Azure 机器学习部署的 web 服务的通信。 Web 服务部署在 Azure Kubernetes Services (AKS) 或 Azure 容器实例 (ACI) 并保护客户端提交的数据。 你还可以使用专用 IP with AKS 来限制评分，以便仅虚拟网络后面的客户端可以访问 web 服务。
...
**指南**：在与机器学习实例相关联的虚拟网络上启用 DDoS 保护，以防范分布式拒绝服务 (DDoS) 攻击。 使用 Azure 安全中心集成威胁检测来检测与已知的恶意或未使用的 Internet IP 地址的通信。

在启用了基于威胁智能的筛选并将其配置为针对恶意网络流量进行 "警报和拒绝" 时，在组织的每个网络边界部署 Azure 防火墙。

- [如何配置 DDoS 防护](../virtual-network/manage-ddos-protection.md)

- [将 Azure 防火墙后的工作区用于 Azure 机器学习](how-to-access-azureml-behind-firewall.md)

- [有关 Azure 安全中心威胁检测的详细信息](/azure/security-center/security-center-alerts-service-layer)
...
**指南**：对于安装在 Azure 机器学习服务中的适当扩展的任何 vm，你可以启用网络观察程序数据包捕获来调查异常活动。 
...
**指南**：对于需要访问 Azure 机器学习帐户的资源，可以使用虚拟网络服务标记来定义网络安全组或 Azure 防火墙上的网络访问控制。 创建安全规则时，可以使用服务标记代替特定的 IP 地址。 通过在规则的相应 "源" 或 "目标" 字段中指定服务标记名称 (例如 AzureMachineLearning) ，可以允许或拒绝相应服务的流量。 Microsoft 会管理服务标记包含的地址前缀，并会在地址发生更改时自动更新服务标记。

Azure 机器学习服务在虚拟网络中记录其计算目标的服务标记列表，可帮助最大程度地降低复杂性，你可以在网络管理中将其用作指导原则。
...
**指南**：定义和实现与 Azure 策略关联的 Azure 机器学习命名空间的网络资源的标准安全配置。 使用 "Default-machinelearning-southcentralus" 和 "Microsoft 网络" 命名空间中的 Azure 策略别名创建自定义策略，以便审核或强制实施机器学习命名空间的网络配置。 
...
**指南**：对与 Azure 机器学习部署关联的网络资源使用标记，以便按照分类逻辑组织它们。

对于支持 "说明" 字段的 Azure 机器学习虚拟网络中的资源，请使用它来记录允许进出网络流量的规则。
...
**指南**：使用 Azure 活动日志监视网络资源配置，并检测与 Azure 机器学习相关的网络资源的更改。 在 Azure Monitor 中创建当关键网络资源发生更改时触发的警报。
...
**指南**： Microsoft 维护用于 Azure 资源的时间源，如日志中的时间戳 Azure 机器学习。
...
**指南**：通过 Azure Monitor 引入日志来聚合 Azure 机器学习生成的安全数据。 在 Azure Monitor 中，使用 Log Analytics 工作区来查询和执行分析，并使用 Azure 存储帐户长期和存档存储。 或者，可以启用数据并将其加入 Azure Sentinel 或第三方安全事件和事件管理 (SIEM)。
...
**指南**：如果计算资源由 microsoft 所有，则 microsoft 负责收集和监视它。 

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的任何计算资源，请使用 Azure 安全中心来监视操作系统。 
...
**指南**：在 Azure Monitor 中，根据组织的符合性规定，设置与 Azure 机器学习实例关联的 Log Analytics 工作区的日志保持期。
...
**指南**：分析和监视日志中的异常行为，并定期查看 Azure 机器学习中的结果。 使用 Azure Monitor 和 Log Analytics 工作区来查看日志数据并执行查询。

或者，你可以将和机载数据启用到 Azure Sentinel 或第三方 SIEM。 

- [如何在 Log Analytics 工作区中执行 Azure 机器学习查询](monitor-azure-machine-learning.md#analyzing-log-data)
...
- [有关 Azure 机器学习警报的详细信息](monitor-azure-machine-learning.md#alerts)

- [如何发出 Log Analytics 工作区日志数据的警报](../azure-monitor/learn/tutorial-response.md)
...
**指南**：如果计算资源由 microsoft 所有，则 microsoft 负责部署 Azure 机器学习服务的反恶意软件。 
...
**指南**：不适用;Azure 机器学习不会处理或产生与 DNS 相关的日志。
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请使用 Azure 安全中心为 Azure 虚拟机启用安全事件日志监视。 Azure 安全中心可在所有受支持的 Azure Vm 上预配 Log Analytics 代理，以及在启用自动预配的情况下创建的所有新代理。 或者，你可以手动安装该代理。 代理启用进程创建事件4688和事件4688内的命令行字段。 虚拟机上创建的新进程由事件日志记录，并由安全中心的检测服务监视。
...
**指南**：你可以使用 Azure 门户中资源的 "标识和访问管理" 选项卡来配置基于角色的访问控制 (RBAC) ，并在 Azure 机器学习资源上维护库存。 角色将应用到 Active Directory 中的用户、组、服务主体和托管标识。 对于个人和组，可使用内置角色或自定义角色。

Azure 机器学习为 Azure 机器学习中的常见管理方案提供内置 RBAC。 在 Azure Active Directory (Azure AD) 中具有配置文件的个人可以将这些 RBAC 角色分配给用户、组、服务主体或托管标识，以授予或拒绝对 Azure 机器学习资源的资源和操作的访问权限。
...
**指南**：通过 Azure Active Directory (Azure AD) 控制对机器学习资源的访问管理。 Azure AD 没有默认密码。
...
- [了解机器学习默认角色的详细信息](how-to-assign-roles.md#default-roles)
...
**指南**：机器学习与 Azure Active Directory 集成，请使用 Azure Active Directory SSO，而不是为每个服务配置单独的独立凭据。 使用 Azure 安全中心标识和访问建议。  
...
**指南**：启用 Azure Active Directory 多重身份验证，并遵循 Azure 安全中心的标识和访问建议。
...
在 Azure 中，角色访问的作用域可以限定为多个级别。 对于机器学习，可以在工作区级别管理角色，例如，你拥有工作区的所有者访问权限可能不具有对包含工作区的资源组的所有者访问权限。 这将提供更精细的访问控制，以在同一资源组中分隔角色。 
...
当环境中发生可疑活动或不安全活动时，使用 Azure Active Directory (Azure AD) Privileged Identity Management (PIM) 生成日志和警报。
...
**指南**：通过 Azure 机器学习部署的 Web 服务仅支持 TLS 版本1.2，该版本强制在传输过程中加密数据。
...
**指南**： Azure 机器学习支持使用 Azure Active Directory (Azure AD) 对机器学习资源的请求进行授权。 使用 Azure AD，你可以使用 Azure 基于角色的访问控制 (RBAC) 向安全主体（可以是用户或应用程序服务主体）授予权限。
...
Microsoft 管理机器学习的底层基础结构，并实施了严格控制来防止客户数据丢失或泄露。
...
**指南**： Azure 机器学习将快照、输出和日志存储在 Azure Blob 存储帐户中，并将其绑定到 Azure 机器学习工作区和订阅。 Azure Blob 存储中存储的所有数据已通过 Microsoft 管理的密钥静态加密。 你还可以在机器学习服务中将存储在 Azure Blob 存储中的数据与你自己的密钥加密。 

- [静态数据加密 Azure 机器学习](concept-enterprise-security.md#encryption-at-rest)
...
**指南**：将 Azure Monitor 与 Azure 活动日志结合使用，以创建 Azure 机器学习和其他关键或相关资源的生产实例发生更改的警报。
...
**指南**：如果计算资源由 microsoft 所有，则 microsoft 负责管理 Azure 机器学习服务的漏洞。 

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请按照 Azure 安全中心提供的建议，在你的 Azure 虚拟机、容器映像和 SQL server 上执行漏洞评估。
...
**指南**：如果计算资源由 microsoft 所有，则 microsoft 负责管理 Azure 机器学习服务的修补程序。 

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于组织拥有的任何计算资源，请使用 Azure 自动化更新管理确保 Windows 和 Linux Vm 上安装了最新的安全更新。 对于 Windows 虚拟机，请确保已启用 Windows 更新并将其设置为自动更新。
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于组织拥有的计算资源，请使用第三方修补程序管理解决方案。 已在其环境中使用 Configuration Manager 的客户也可以使用 System Center Updates Publisher，从而使他们能够将自定义更新发布到 Windows Server 更新服务。 这允许更新管理将使用 Configuration Manager 作为其更新存储库的计算机与第三方软件进行修补。
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请按照 Azure 安全中心提供的建议在 Azure 虚拟机、容器映像和 SQL server 上执行漏洞评估。 按一致的间隔导出扫描结果，并将结果与先前的扫描进行比较，以验证是否已修正了漏洞。 使用 Azure 安全中心建议的漏洞管理建议时，可以通过透视到所选解决方案的门户来查看历史扫描数据。
...
**指南**：使用 Azure 资源关系图可以在订阅中查询和发现资源 (如计算、存储、网络、端口和协议等 ) 。  确保适当的 (读取租户中的) 权限，并枚举所有 Azure 订阅以及订阅中的资源。
...
**指南**：将标记应用于 Azure 资源，将元数据添加到根据分类逻辑组织。
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于组织拥有的计算资源，请使用 Azure 自动化更改跟踪和清单来自动收集 Windows 和 Linux Vm 中的清单信息。 可从 Azure 门户获取软件名称、版本、发布者和刷新时间。 若要获取软件安装日期和其他信息，请启用来宾级别诊断，并将 Windows 事件日志定向到 Log Analytics 工作区。

- [如何为 VM 启用 Azure 自动化清单收集](../automation/automation-tutorial-installed-software.md)
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于组织拥有的计算资源，请使用 Azure 安全中心的文件完整性监视 (FIM) 来确定 Vm 上安装的所有软件。 可以使用的另一种方法，而不更改跟踪是将其与 FIM 结合使用，以便从 Linux 和 Windows Vm 收集清单。 

可以实现自己的未授权软件删除过程。 你还可以使用第三方解决方案来确定未批准的软件。

如果不再需要 Azure 资源，请将其删除。

- [如何使用文件完整性监视](../security-center/security-center-file-integrity-monitoring.md#using-file-integrity-monitoring)
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请使用 Azure 安全中心自适应应用程序控制，以确保仅执行经过授权的软件，并且阻止所有未经授权的软件在 Azure 虚拟机上执行。
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的任何计算资源，请使用 Azure 安全中心自适应应用程序控件指定规则可能或不适用于的文件类型。

如果自适应应用程序控件不符合要求，则实现第三方解决方案。
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于组织拥有的计算资源，根据脚本类型，可以使用特定于操作系统的配置或第三方资源来限制用户在 Azure 计算资源中执行脚本的能力。  你还可以使用 Azure 安全中心的自适应应用程序控制，以确保仅执行授权的软件，并且阻止所有未经授权的软件在 Azure 虚拟机上执行。
...
**指南**：通过 Azure 策略为 Azure 机器学习服务定义和实施标准安全配置。 使用 "Default-machinelearning-southcentralus" 命名空间中的 Azure 策略别名创建自定义策略，以便审核或强制执行 Azure 机器学习服务的配置。

Azure 资源管理器可以 JavaScript 对象表示法 (JSON) 中导出模板，应查看该模板以确保配置满足组织的安全要求。

你还可以使用 Azure 安全中心提供的建议作为你的 Azure 资源的安全配置基线。

Azure 机器学习完全支持用于跟踪工作的 Git 存储库;可以直接将存储库克隆到共享工作区文件系统，在本地工作站上使用 Git，并确保安全配置适用于作为机器学习环境一部分的代码资源。
...
**指南**：如果计算资源由 microsoft 所有，则 microsoft 负责 Azure 机器学习服务的操作系统安全配置。

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于组织拥有的计算资源，请使用 Azure 安全中心建议在所有计算资源上维护安全配置。  此外，你可以使用自定义的操作系统映像或 Azure 自动化状态配置来建立你的组织所需的操作系统的安全配置。
...
Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请按照 Azure 安全中心的建议执行有关 Azure 计算资源的漏洞评估。  此外，你可以使用 Azure 资源管理器模板、自定义的操作系统映像或 Azure 自动化状态配置来维护你的组织所需的操作系统的安全配置。   与 Azure 自动化状态配置结合的 Microsoft 虚拟机模板可以帮助满足和维护安全要求。 

请注意，由 Microsoft 发布的 Azure Marketplace 虚拟机映像由 Microsoft 进行管理和维护。 
...
**指南**：如果将自定义 Azure 策略定义用于机器学习或相关资源，请使用 Azure Repos 安全地存储和管理你的代码。
...
**指南**： Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请使用 Azure 基于角色的访问控制 (RBAC) ，以确保只有经过授权的用户才能访问你的自定义映像。 使用 Azure 共享映像库你可以将映像共享到组织内的不同用户、服务主体或 Azure AD 组。 将容器映像存储在 Azure 容器注册表中，并使用 RBAC 确保只有获得授权的用户才有权访问。
...
**指南**：使用 "default-machinelearning-southcentralus" 命名空间中的 Azure 策略别名创建自定义策略，以对系统配置进行警报、审核和强制执行。 另外，开发一个用于管理策略例外的流程和管道。
...
**指南**：如果计算资源由 microsoft 所有，则 microsoft 负责保护 Azure 机器学习服务的配置部署。

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请在任何云或本地数据中心中使用适用于所需状态配置的 Azure 自动化状态配置 (DSC) 节点。 可以轻松登记计算机、为其分配声明性配置并查看显示每台计算机是否符合指定的所需状态的报告。 

- [如何启用 Azure 自动化状态配置](../automation/automation-dsc-onboarding.md)
...
**指南**：如果计算资源由 microsoft 所有，则 microsoft 负责对 Azure 机器学习服务进行自动安全配置监视。

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请使用 Azure 安全中心计算 &amp; 应用，并按照 vm 和服务器的建议和容器进行操作。
...
**指南**：将托管标识与 Azure Key Vault 结合使用，简化云应用程序的密钥管理。

Azure 机器学习支持通过客户托管的密钥进行数据存储加密，你需要管理密钥轮换并废除每个组织的安全和符合性要求。 

使用 Azure Key Vault 将机密安全传递到定型脚本中的远程运行，而不是明文。

- [Azure 机器学习客户管理的密钥](concept-enterprise-security.md#azure-blob-storage)
...
**指南**： Azure 机器学习支持内置角色和创建自定义角色的功能。 使用托管标识在 Azure AD 中通过自动管理的标识来提供 Azure 服务。 通过托管标识，你可以对任何支持 Azure AD 身份验证的服务进行身份验证，包括 Key Vault，而无需在代码中包含任何凭据。

 
- [管理对 Azure 机器学习工作区的访问权限](how-to-assign-roles.md)

- [如何配置 Azure 资源的托管标识](../active-directory/managed-identities-azure-resources/qs-configure-portal-windows-vm.md)
...
**指南**：在支持 Azure 服务的基础主机上启用了 Microsoft 反恶意软件 (例如 Azure 机器学习) ，但它不会在客户内容上运行。

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的计算资源，请使用适用于 Azure 的 Microsoft 反恶意软件持续监视和保护你的资源。 对于 Linux，请使用第三方反恶意软件解决方案。 此外，使用 Azure 安全中心的数据服务威胁检测来检测已上传到存储帐户的恶意软件。

- [如何配置适用于 Azure 的 Microsoft 反恶意软件](../security/fundamentals/antimalware.md)
...
**指南**：在支持 Azure 服务的底层主机上启用了 Microsoft 反恶意软件 (例如 Azure 机器学习) ，但它不会在客户内容上运行。
...
**指南**：为支持 azure 服务 (的基础主机（例如，azure 机器学习) 启用和维护 Microsoft 反恶意软件，但不会在客户内容上运行。

Azure 机器学习跨不同计算资源甚至是自己的计算资源提供不同的支持。 对于你的组织拥有的任何计算资源，请按照 Azure 安全中心中的建议，计算 &amp; 应用，以确保所有终结点都是最新的签名。 对于 Linux，请使用第三方反恶意软件解决方案。

- [如何部署适用于 Azure 的 Microsoft 反恶意软件](../security/fundamentals/antimalware.md)
...
**指南**：机器学习服务中的数据恢复管理是通过数据管理连接的数据存储。 请确保按照每个客户组织策略备份数据的已连接存储的数据恢复指导原则进行操作。
...
**指南**：机器学习服务中的数据备份是通过数据管理连接的数据存储。 为 Vm 启用 Azure 备份，并配置所需的频率和保持期。 在 Azure Key Vault 中备份客户托管的密钥。
...
**指南**：机器学习服务中的数据备份验证是通过数据管理连接的数据存储。 定期对 Azure 备份中的内容执行数据还原。 确保可以还原已备份的客户管理的密钥。
...
**指南**：对于本地备份，将使用在备份到 Azure 时提供的密码提供静态加密。 使用基于角色的访问控制来保护备份和客户管理的密钥。 

在 Key Vault 中启用软删除和清除保护，以防止意外或恶意删除密钥。 如果将 Azure 存储用于存储备份，请启用软删除以在 blob 或 blob 快照被删除时保存和恢复数据。
