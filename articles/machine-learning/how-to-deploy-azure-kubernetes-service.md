...
有关设置、和的详细信息， `autoscale_target_utilization` `autoscale_max_replicas` `autoscale_min_replicas` 请参阅 [AksWebservice](https://docs.microsoft.com/python/api/azureml-core/azureml.core.webservice.akswebservice?view=azure-ml-py&preserve-view=true) 模块参考。
...
Azure 安全中心跨混合云工作负荷提供统一的安全管理和高级威胁防护。 你应允许 Azure 安全中心扫描资源并遵循其建议。 有关详细信息，请参阅 [Azure Kubernetes 服务与安全中心的集成](https://docs.microsoft.com/azure/security-center/azure-kubernetes-service-integration)。
