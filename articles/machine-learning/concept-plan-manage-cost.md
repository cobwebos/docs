...
虽然 Enterprise edition 处于预览阶段，但没有 ML 附加费。 企业版公开上市后，会有机器学习附加费（用于培训和推断）。  有关更多详细信息，请[Azure 机器学习定价](https://azure.microsoft.com/pricing/details/machine-learning/)。
...
按不同时间间隔查看图形和表中的成本。 一些示例如下：日、当前、上个月和年。 还查看成本与预算和预测成本。 随着时间的推移切换到较长的视图，可帮助你确定支出趋势并了解超支可能发生的位置。 如果已创建预算，请查看他们超出的位置。  
...
Azure 机器学习用户可以使用托管 Azure 机器学习计算群集（也称为 AmlCompute）。 AmlCompute 支持多种 GPU 和 CPU 选项。 AmlCompute 是由 Azure 机器学习代表你的订阅进行内部托管的，但在 Azure IaaS 云规模上提供相同的企业级安全性、合规性和监管。
...
根据工作负荷要求自动缩放群集有助于降低成本，因此只需使用所需的内容即可。 AmlCompute 群集旨在根据工作负荷的需求动态缩放。 群集可以扩展到预配的最大节点数和为订阅指定的配额范围内。 每次运行完成时，群集将释放节点并自动缩放到指定的最小节点计数。

除了设置最小和最大节点数以外，还可以调整节点在缩减之前处于空闲状态的时间量。 默认情况下，"缩减之前的空闲时间" 设置为120秒。

+ 如果执行更少的迭代试验，请缩短此时间以节省成本。 
+ 如果你执行高度迭代的开发/测试试验，你可能需要增加此项，以便在每次对定型脚本或环境进行更改后，不会向上下扩展。
...
与其他 Azure 计算资源非常类似，AmlCompute 附带了固有的[配额（或限制）配置](how-to-manage-quotas.md#azure-machine-learning-compute)。 此配额为 VM 系列（例如 Dv2 系列、NCv3 系列），并因每个订阅的区域而异。 订阅从较小的默认值开始，你可以使用此设置来控制订阅中可用的 Amlcompute 资源量。 

同时为订阅中的每个工作区配置[工作区级别配额，按 VM 系列](/how-to-manage-quotas.md#workspace-level-quota)。 这样，你就可以更精细地控制每个工作区可能产生的成本，并限制某些 VM 系列。 

若要在工作区级别设置配额，请从[Azure 门户](https://portal.azure.com)开始。  选择订阅中的任何工作区，并在左窗格中选择 "**使用情况 + 配额**"。 然后选择 "**配置配额**" 选项卡以查看配额。 你需要订阅范围的权限来设置此配额，因为这是一个影响多个工作区的设置。

## <a name="set-run-auto-termination-policies"></a>设置运行自动终止策略 

将定型运行配置为限制其持续时间，或在某些情况下提前终止它们（特别是在使用 Azure 机器学习的内置超参数优化或自动机器学习功能时）。 

下面是几个选项：
* 定义在 RunConfiguration 中`max_run_duration_seconds`调用的参数，以控制在所选计算（本地或远程云计算）上运行的最长持续时间。
* 对于[超参数优化](how-to-tune-hyperparameters.md#early-termination)，请从 Bandit 策略、中间停止策略或截断选择策略中定义早期终止策略。 此外，还可以使用参数（例如`max_total_runs`或`max_duration_minutes` ）进一步控制各种超参数扫描。
...
Azure 允许在虚拟机规模集、Batch 和机器学习服务中使用过量的未利用容量作为低优先级 Vm。 这些分配是预 emptible 的，但与专用 Vm 相比，其价格会降低。 通常，对于批处理工作负荷，建议使用低优先级 Vm，或者通过重新提交（对于批处理推断）或通过重新启动（用于对检查点的深度学习培训）来恢复中断。
...
Azure 保留 VM 实例提供了另一种方法，可通过提交到一年或三年来实现计算资源的巨大节约。 这些折扣范围可达72% 的即用即付价格，并直接应用于每月的 Azure 帐单。

Azure 机器学习计算本身就支持保留实例。 因此，如果您购买了一年或三年的预订实例，我们将根据 Azure 机器学习中使用的托管计算自动应用该预订实例折扣，无需任何其他设置。
