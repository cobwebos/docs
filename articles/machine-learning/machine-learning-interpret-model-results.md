...
本主题介绍预测解释，而不是模型评估。 有关如何评估模型的详细信息，请参阅[如何在 Azure 机器学习中评估模型性能](machine-learning-evaluate-model-performance.md)。

如果你不熟悉 Azure 机器学习，并需要创建简单实验的帮助以开始，请参阅 Azure 机器学习工作室中的[在 Azure 机器学习工作室中创建一个简单的实验](machine-learning-create-experiment.md)。
...
![鸢尾花实验的屏幕截图](./media/machine-learning-interpret-model-results/1.png)
...
![鸢尾花双类分类实验的结果](./media/machine-learning-interpret-model-results/2.png)
...
在了解了预测结果并判断其为优良后，可以将实验发布为 Web 服务，以便可以将其部署在各种应用程序中，并对任何新的鸢尾花调用它以获取类预测。 若要了解如何将训练实验更改为评分实验并将其发布为 Web 服务，请参阅[发布 Azure 机器学习 Web 服务](machine-learning-walkthrough-5-publish-web-service.md)。 此过程提供评分实验，如图 3 所示。

![评分实验的屏幕截图](./media/machine-learning-interpret-model-results/3.png)
...
![测试解释评分模型](./media/machine-learning-interpret-model-results/4_1.png)

![评分测试结果](./media/machine-learning-interpret-model-results/5.png)
...
![字母识别示例](./media/machine-learning-interpret-model-results/5_1.png)

在训练数据中，有 16 个从手写字母图像中提取的特征。 26 个字母形成了 26 个类。 图 6 显示一个实验，该实验将训练多类分类模型进行字母识别，并对测试数据集上的相同特征集进行预测。

![字母识别多类分类实验](./media/machine-learning-interpret-model-results/6.png)
...
![R 代码示例](./media/machine-learning-interpret-model-results/8.png)

图 8。 用于提取评分标签及标签的关联概率的 R 代码

![实验结果](./media/machine-learning-interpret-model-results/9.png)

图 9。 字母识别多类分类问题的最终评分实验

发布和运行 Web 服务并输入某些输入特征值后，返回的结果类似于图 10。 此手写字母及其提取的 16 个特征预测为“T”，概率为 0.9715。

![测试解释评分模块](./media/machine-learning-interpret-model-results/9_1.png)

![测试结果](./media/machine-learning-interpret-model-results/10.png)

图 10. 多类分类的 Web 服务结果

## <a name="regression"></a>回归
回归问题不同于分类问题。 在分类问题中，你将尝试预测离散类，如鸢尾花所属的类。 但是，正如以下回归问题示例所示，你将尝试预测连续变量，如一辆汽车的价格。

**示例实验**

使用汽车价格预测作为回归的示例。 你将尝试根据特征预测汽车的价格，包括型号、燃料类型、车身类型和驱动轮。 实验显示在图 11 中。

![汽车价格回归实验](./media/machine-learning-interpret-model-results/11.png)
...
![汽车价格回归问题的评分实验](./media/machine-learning-interpret-model-results/13.png)

图 13. 汽车价格回归问题的评分实验

运行 Web 服务，返回的结果类似于图 14。 此汽车的预测价格为 15085.52 美元。

![测试解释评分模块](./media/machine-learning-interpret-model-results/13_1.png)

![评分模块结果](./media/machine-learning-interpret-model-results/14.png)
...
![鸢尾花聚类问题实验](./media/machine-learning-interpret-model-results/15.png)
...
![可视化聚类结果](./media/machine-learning-interpret-model-results/17.png)
...
![鸢尾花聚类问题的评分实验](./media/machine-learning-interpret-model-results/18.png)

图 18. 鸢尾花聚类问题的评分实验

运行 Web 服务后，返回的结果类似于图 19。 此花卉预测为在群集 0 中。

![测试解释评分模块](./media/machine-learning-interpret-model-results/18_1.png)

![评分模块结果](./media/machine-learning-interpret-model-results/19.png)
...
六列中的第一列显示查找相关用户所需的给定用户 ID，由输入数据提供。 其他五列存储该用户的预测相关用户，以相关度降序排序。 例如，在第一行中，客户 U1048 的最相关的客户为 U1051，然后依次为 U1066、U1044、U1017 和 U1072。
...
六列中的第一列表示查找相关项目所需的给定项目 ID，由输入数据提供。 其他五列存储该项目的预测相关项目，以相关度降序排序。 例如，在第一行中，项目 135026 的最相关的项目为 135074，然后依次为 135035、132875、135055 和 134992。
...
![餐厅推荐问题的评分实验](./media/machine-learning-interpret-model-results/25.png)

图 25. 餐厅推荐问题的评分实验

运行 Web 服务，返回的结果类似于图 26。 对用户 U1048 最推荐的五个餐厅为 134986、135018、134975、135021 和 132862。

![推荐器系统服务的示例](./media/machine-learning-interpret-model-results/25_1.png)

![示例实验结果](./media/machine-learning-interpret-model-results/26.png)
