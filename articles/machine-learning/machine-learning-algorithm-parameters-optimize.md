...
![查找最佳参数集](./media/machine-learning-algorithm-parameters-optimize/fig1.png)
...
![双类提升决策树，单个参数](./media/machine-learning-algorithm-parameters-optimize/fig2.png)

 或者，使用**使用范围生成器**定义网格的最大和最小网格点和生成的总点数。 默认情况下，参数值按线性刻度生成。 但是，如果“对数刻度”处于选中状态，这些值将在对数刻度中生成（即相邻点的比率是常量，而不是它们的差）。 对于整数参数，可使用连字符定义范围。 例如，“1-10”是指介于 1 到 10（均含）之间的所有整数构成参数集。 也支持混合模式。 例如，参数集“1-10、20、50”将包括整数 1-10、20 和 50。
...
该模块还具有一个可选数据集输入。 使用折叠信息将数据集连接到必需数据集输入。 如果未向数据集分配任何折叠信息，那么默认情况下将自动执行 10 个折叠交叉验证。 如果未执行折叠分配且在可选数据集端口提供验证数据集，则将选择训练-测试模式且第一个数据集将用于为每个参数组合训练模型。

![提升决策树分类器](./media/machine-learning-algorithm-parameters-optimize/fig6a.png)

然后将在验证数据集上评估模型。 模块的左侧输出端口显示不同的指标作为参数值的函数。 右输出端口根据所选指标提供对应于最佳模型的已训练模型（本例中为**准确性**）。  

![验证数据集](./media/machine-learning-algorithm-parameters-optimize/fig6b.png)

你可以通过可视化右侧输出端口查看所选的确切参数。 保存为已训练模型后，此模型可用于对测试集进行评分或可操作性 Web 服务。
