...
ms.locfileid: "78933303"
---
# <a name="tutorial-predict-automobile-price-with-the-designer-preview"></a>教程：通过设计器预测汽车价格（预览）
...
>
>若要找到该示例，请转到工作区中的设计器。 在 "**新建管道**" 部分，选择 "**示例 1-回归：汽车价格预测（基本）** "。
...
现在，你的管道已全部设置，你可以提交管道运行来训练机器学习模型。 在设计器中生成管道时，可以随时提交管道运行。 你可以执行此操作来检查你的工作情况，并按预期验证管道功能。
...
1. 在画布右侧的模块详细信息窗格中，选择“输出”> 图形图标 **可视化图标** 查看结果。![](./media/tutorial-designer-automobile-price-train-score/visualize-icon.png)
...
* **平均绝对错误（MAE）** ：绝对错误的平均次数。 误差是指预测值与实际值之间的差。
* **根本方差方形错误（RMSE）** ：在测试数据集上进行预测的平均平方误差的平方根。
* **相对绝对误差**：相对于实际值与所有实际值平均值之间的绝对差异的绝对误差平均值。
* **相对平方误差**：相对于实际值与所有实际值平均值之间的平方差异的平方误差平均值。
* **确定系数**：也称为 R 平方值，这一统计指标指示模型适合数据的程度。
