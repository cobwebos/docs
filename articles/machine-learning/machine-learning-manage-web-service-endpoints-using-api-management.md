...
* AzureML 实验的工作区、服务和 api_key 均部署为 Web 服务。 单击[此处](machine-learning-create-experiment.md)了解如何创建 AzureML 实验的详细信息。 单击[此处](machine-learning-publish-a-machine-learning-web-service.md)了解如何将 AzureML 实验部署为 Web 服务的详细信息。 另外，附录 A 说明了如何创建并测试简单 AzureML 实验并将其部署为 Web 服务。

## <a name="create-an-api-management-instance"></a>创建 API 管理实例
下面是使用 API 管理来管理 AzureML Web 服务的步骤。 首先创建服务实例。 登录[经典门户](https://manage.windowsazure.com/)，然后依次单击“新建” > “应用服务” > “API 管理” > “创建”。
...
单击顶部菜单的“API”，然后单击“AzureML 演示 API”，查看可用操作。
...
![发送](./media/machine-learning-manage-web-service-endpoints-using-api-management/send.png)
...
首先，使用所选浏览器导航到 [https://studio.azureml.net/](https://studio.azureml.net/)，然后输入凭据以登录。 接下来，创建新的空白实验。

![搜索实验模板](./media/machine-learning-manage-web-service-endpoints-using-api-management/search-experiment-templates.png)
...
单击“选择数据集中的列”，然后单击“启动列选择器”并选择“Col2”。 单击复选标记应用这些更改。
...
测试 RRS 的另一种方法是从客户端代码入手。 如果在仪表板上单击“请求/响应”，并滚动到底部，你将看到 C#、Python 和 R 的示例代码。你还将看到 RRS 请求的语法，包括请求 URI、标头和正文。

本指南介绍了运行中的 Python 示例。 你将使用实验的“工作区”、“服务”和“api_key”修改该示例。
...
在仪表板上单击“批处理执行”并滚动到底部。 你将看到 C#、Python 和 R 的示例代码。你还将看到提交作业、启动作业、获取作业状态或结果以及删除作业的 BES 请求语法。

本指南介绍了运行中的 Python 示例。 你需要使用实验的“工作区”、“服务”和“api_key”修改该示例。 此外，还需要修改“存储帐户名称”、“存储帐户密钥”和“存储容器名称”。 最后，需要修改“输入文件”和“输出文件”的位置。
