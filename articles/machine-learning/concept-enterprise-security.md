...
此 Cosmos DB 实例是在订阅中的 Microsoft 托管资源组中创建的，以及所需的任何资源。 托管资源组的命名格式为 `<AML Workspace Resource Group Name><GUID>`。 如果 Azure 机器学习工作区使用专用终结点，则还会为 Cosmos DB 实例创建一个虚拟网络。 此 VNet 用于保护 Cosmos DB 和 Azure 机器学习之间的通信。

> [!IMPORTANT]
> * 请勿删除包含此 Cosmos DB 实例的资源组，或自动在此组中创建的任何资源。 如果需要删除资源组、Cosmos DB 实例等，则必须删除使用该资源组的 Azure 机器学习工作区。 删除关联的工作区时，将删除资源组、Cosmos DB 实例和其他自动创建的资源。
> * 此 Cosmos DB 帐户的默认[请求单位数](../cosmos-db/request-units.md)设置为“8000” 。 不支持更改此值。
> * 不能提供自己的 VNet 以便与创建的 Cosmos DB 实例一起使用。 也无法修改虚拟网络。 例如，你无法更改它使用的 IP 地址范围。
