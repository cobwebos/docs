...
Azure 容器实例不支持磁盘加密。 如果需要磁盘加密，我们建议改为部署到[Azure 库伯内斯服务实例](how-to-deploy-azure-kubernetes-service.md)。 在这种情况下，您可能还希望使用 Azure 机器学习对基于角色的访问控件的支持，以防止部署到订阅中的 Azure 容器实例。
