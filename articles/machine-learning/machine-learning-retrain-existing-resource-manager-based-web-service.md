...
在实验画布的底部，单击“设置 Web 服务”，然后选择“部署 Web 服务 [全新]”。 Azure 机器学习 Web 服务门户可打开“部署 Web 服务”页。 为 Web 服务键入名称，并选择付款计划，然后单击“部署”。 只可将 Batch 执行方法用于创建训练模型。
...
按照注释中指定的方式添加 NuGet 包 Microsoft.AspNet.WebApi.Client。 若要将引用添加到 Microsoft.WindowsAzure.Storage.dll，可能首先需要[为 Azure 存储服务安装客户端库](https://www.nuget.org/packages/WindowsAzure.Storage)。
...
若要更新 Azure 存储信息，必须检索 Azure 经典门户存储帐户的存储帐户名称、密钥和容器信息，然后更新相应的信息。运行实验后，生成的工作流应类似于以下内容：
...
若要修改训练模型定义以使用新训练的模型，必须先使用 [Export-AzureRmMlWebService](https://msdn.microsoft.com/library/azure/mt767935.aspx) cmdlet 将其导出到 JSON 格式的文件。
