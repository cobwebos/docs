...
在 Azure 机器学习中创建的神经网络模型的体系结构可通过使用 Net# 广泛自定义。 你可以：  
...
若要同时定义两个或多个常量，可将标识符名称和值括在括号内，并使用分号隔开。 例如：  
...
可训练层（隐藏或输出层）的层声明可选择包括输出函数（也称为激活函数），这将分类模型默认为 **sigmoid**，回归模型默认为 **linear**。 （即使使用默认值，为了清楚起见，也可以显式声明激活函数。）
...
* **Weights**：（可选）定义捆绑的初始权重。 值必须是浮点值的元组，其长度为内核数乘以每个内核的权重数，将在本文后面部分中定义。 默认权重是随机生成的。  
...
**警报**：目标层中的节点对应于是内核的中央节点的神经元。 例如，如果 KernelShape[d] 为奇数，则 *KernelShape[d]/2* 与中央内核节点相对应。 如果 *KernelShape[d]* 为偶数，则中心节点位于 *KernelShape[d]/2 - 1*。 因此，如果 **Padding**[d] 为 False，那么第一个和最后一个 *KernelShape[d]/2* 节点在目标层中没有对应节点。 若要避免这种情况，可将 **Padding** 定义为 [true, true, …, true]。  
...
* **Padding** 的默认值为 False，因此目标层的每个维度中只有 10 个节点。 若要包括一个与源层中每个节点对应的目标层中的节点，可添加 Padding = [true, true, true]；然后将 RN1 的大小更改为 [5, 12, 12]。  
...
* 第一行定义输入层（名为 *Data*）。 使用 **auto** 关键字时，神经网络将自动包括输入示例中的所有功能列。 
