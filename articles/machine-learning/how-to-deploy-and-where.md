...
> [!TIP]
> 注册模型时，需要提供云位置（来自定型运行）或本地目录的路径。 此路径只是在注册过程中查找要上载的文件。 它不需要与条目脚本中使用的路径匹配。 有关详细信息，请参阅[在输入脚本中查找模型文件](#locate-model-files-in-your-entry-script)。
...
+ **使用 SDK**
  ...
  使用 SDK 训练模型时，可以接收[Run](https://docs.microsoft.com/python/api/azureml-core/azureml.core.run.run?view=azure-ml-py) Object 或[AutoMLRun](/python/api/azureml-train-automl-client/azureml.train.automl.run.automlrun)对象，具体取决于模型的定型方式。 每个对象都可用于注册由试验运行创建的模型。
...
* **& 源代码依赖项的条目脚本**。 此脚本接受请求，使用模型为请求评分，并返回结果。
    ...
    >
    > * Web 服务和 IoT Edge 部署无法访问工作区数据存储或数据集。 如果部署的服务需要访问存储在部署之外的数据（例如 Azure 存储帐户中的数据），则必须使用相关的 SDK 开发自定义代码解决方案。 例如，[用于 Python 的 Azure 存储 SDK](https://github.com/Azure/azure-storage-python)。
    >
...
有关如何保护 web 服务部署的详细信息，请参阅[使用 SSL 保护 web 服务](how-to-secure-web-service.md#enable)。
...
每个已部署的 web 服务都提供 REST 终结点，因此可以使用任何编程语言创建客户端应用程序。
如果为服务启用了基于密钥的身份验证，则需要提供服务密钥作为请求标头中的令牌。
如果已为服务启用基于令牌的身份验证，则需要提供 Azure 机器学习的 JSON Web 令牌（JWT）作为请求标头中的持有者令牌。 

主要区别在于，**密钥是静态的，可以手动重新生成**，并且**令牌需要在过期时刷新**。 Azure 容器实例和 Azure Kubernetes 服务部署的 web 服务支持基于密钥的身份验证，基于令牌的身份验证**仅**适用于 Azure Kubernetes 服务部署。 有关详细信息和特定代码示例，请参阅身份验证的操作[方法](how-to-setup-authentication.md#web-service-authentication)。
...
`Status: Downloaded newer image for myworkspacef78fd10.azurecr.io/package:20190822181338` 列中的一个值匹配。 
