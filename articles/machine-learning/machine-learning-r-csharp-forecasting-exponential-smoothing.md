...
有多种方法可以自动方式使用该服务（[此处](http://microsoftazuremachinelearning.azurewebsites.net/etsForecasting.aspx)为示例应用）。
...
这是一个非常简单的 ETS 预测示例。 从上面的代码示例中可看出，没有进行错误捕获，并且该服务假定所有变量都是连续/正值，频率应为大于 1 的整数。 日期和值向量的长度应相同。 日期变量应遵循格式 mm/dd/yyyy。
