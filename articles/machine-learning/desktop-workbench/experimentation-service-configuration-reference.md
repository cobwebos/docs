...
ms.locfileid: "34831306"
---
# <a name="azure-machine-learning-experimentation-service-configuration-files"></a>Azure 机器学习试验服务配置文件

在 Azure Machine Learning (Azure ML) Workbench 中运行脚本时，由“aml_config”文件夹中的文件控制执行行为。 此文件夹位于你的项目文件夹根目录下。 请务必了解这些文件的内容，以便以最佳方式实现你期望的执行结果。
...
**PrepareEnvironment**：当设置为 true 时，此属性将指示试验服务根据指定为初始运行一部分的 conda 依赖项来准备 conda 环境。 仅当针对 Docker 环境执行时，此属性才有效。 如果针对本地环境运行，此设置无任何影响。 

**TrackedRun**：此标志指示试验服务是否跟踪 Azure ML Workbench 运行历史记录基础结构中的运行。 默认值为 _true_。 
...
详细了解[试验服务配置](experimentation-service-configuration.md)。
