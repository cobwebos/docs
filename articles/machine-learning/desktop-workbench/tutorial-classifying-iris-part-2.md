...
1. 创建 VM 后，可将 VM 附加为执行环境，方法是生成 `.runconfig` 和 `.compute` 文件对。 请使用以下命令生成这两个文件。 
 ...
 让我们将新计算目标命名为 `myvm`。
   ...
   >[!NOTE]
   >除了用户名和密码身份验证，还可以使用 `--private-key-file` 和 `--private-key-passphrase` 选项（后者为可选）指定私钥和相应的通行短语（如果有）。 如果要使用创建 DSVM 时使用的私钥，则应指定 `--use-azureml-ssh-key` 选项。
