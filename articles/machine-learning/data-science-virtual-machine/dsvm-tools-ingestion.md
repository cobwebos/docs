...
| 支持的 DSVM 版本      | Windows      |
| 典型用法      | 将多个 blob 从 Azure 存储复制到 Azure Data Lake Store。      |
...
| 典型用法      | 从 Azure 存储、Azure Data Lake Store 导出数据或将数据导入其中      |
|  如何使用/运行它？    |   打开命令提示符，然后键入 `az` 获取帮助。    |
| 到示例的链接      | [使用 Azure CLI](https://docs.microsoft.com/cli/azure/?viee-cli-latest)     |
...
| 支持的 DSVM 版本      | Windows      |
| 典型用法      | 将文件复制到 blob 存储，在帐户之间复制 blob。      |
|  如何使用/运行它？    |   打开命令提示符，然后键入 `azcopy` 获取帮助。    |
| 到示例的链接      | [Windows 上的 AzCopy](https://docs.microsoft.com/en-us/azure/storage/common/storage-use-azcopy)      |
| DSVM 上的相关工具      | AdlCopy     |
...
| 支持的 DSVM 版本      | Windows      |
| 典型用法      | 将文件从虚拟机导入 CosmosDB，将数据从 Azure 表存储导入 CosmosDB，或将数据从 SQL Server 数据库导入到 CosmosDB。     |
...
| 支持的 DSVM 版本      | Windows      |
| 典型用法      | 将 CSV 文件导入到 SQL Server 表中，将 SQL Server 表导出到文件。      |
...
| 到示例的链接      | [数据管理网关](https://msdn.microsoft.com/library/dn879362.aspx)      |
