...
| 如何使用或运行它      | 作为 UI 工具。 在 Windows 上，启动命令提示符，运行 R，然后在 R 内运行 `rattle()`。 在 Linux 上，使用 X2Go 进行连接，启动终端，运行 R，然后在 R 中运行 `rattle()`。 |
...
| 典型用途      | 一般计算机学习库      |
| 如何配置或安装它？      |  使用 GPU 支持安装   |
| 如何使用或运行它      | 作为 Python 库（2.7 和3.5）、R 包和路径命令行工具（适用于 Windows 的 `C:\dsvm\tools\xgboost\bin\xgboost.exe`，Linux @no__t 为-1）    |
