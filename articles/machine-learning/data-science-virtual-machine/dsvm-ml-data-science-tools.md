...
| 到示例的链接      | [Weka 示例](http://www.cs.waikato.ac.nz/ml/weka/documentation.html) |
...
| 支持的 DSVM 版本     | Windows、Linux     |
| 典型用法      | R 适用的常规用户界面数据挖掘工具    |
| 如何使用/运行它？      | 用户界面工具。 在 Windows 上，启动命令提示符，运行 R，然后在 R 中运行 `rattle()`。 在 Linux 上，使用 X2Go 连接，启动终端，运行 R，然后在 R 中运行 `rattle()`。 |
| 到示例的链接      | [Rattle](https://togaware.com/onepager/) |
