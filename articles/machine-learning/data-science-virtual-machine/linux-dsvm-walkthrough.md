...
* 使用打开的 XFCE 会话在计算机上安装[**X2Go**](https://wiki.x2go.org/doku.php) 。 有关详细信息, 请参阅[安装和配置 X2Go 客户端](linux-dsvm-intro.md#install-and-configure-the-x2go-client)。
...
* **Word\_高频\_* 字*** 等列指示电子邮件中与*word*匹配的单词的百分比。 例如, 如果**word\_频率\_** 为**1**, 则电子邮件中所有单词的 1% 都是。
...
1. 选择“执行”。 你应在单个图形窗口中看到两个密度图, 在这种情况下,在电子邮件中显示的词比_删除_更频繁。
...
这会带来一些有趣的相关性: 例如,_技术_与_HP_和_实验室_密切相关。 由于数据集赞助者的区号为 650, 因此它也强烈关联到_650_ 。

"**浏览**" 窗口中提供了单词之间的关联的数值。 例如, 要注意的是, 该_技术_与和_金钱_有负面关联 。
