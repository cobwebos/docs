...
假设我们想要使用尺寸、动力、引擎规格等功能来预测汽车的价格。 这是典型的回归问题，其中目标变量（*价格*）是一个连续数值。 我们可以调整线性回归模型，在给定汽车的功能值的情况下，可以预测该汽车的价格。 此回归模型可用于为我们训练所用的相同数据集评分。 获得预测的汽车价格后，可以通过查看预测偏离实际价格的平均数量来评估模型性能。 为了说明这一点，我们使用了 ML Studio （经典）的 "保存的数据**集**" 部分中提供的 "*汽车价格数据（原始）" 数据集*。
...
在二元分类方案中，目标变量只有两个可能的结果，例如： {0, 1} 或 {false, true}、{negative, positive}。 假设提供一个具有人口统计和雇用变量的成年员工数据集，并要求预测收入水平，此处的收入水平是一个带有值 {“<=50 K”, “>50 K”} 的二进制变量。 换而言之，负类表示年收入小于或等于 50K 的员工，正类表示所有其他员工。 和在回归方案中一样，我们可以训练一个模型、评分一些数据，并评估结果。 此处的主要区别在于经典版 Azure 机器学习 Studio 计算和输出的指标。 为了说明收入级别预测方案，我们将使用[成人](https://archive.ics.uci.edu/ml/datasets/Adult)数据集创建工作室（经典）试验，并评估双类逻辑回归模型（一种常用的二进制分类器）的性能。
...
因此，计算捕获评估的更具体方面的其他指标很有帮助。 在详细讨论此类指标之前，了解二元分类评估的混淆矩阵很重要。 训练集中的类标签只能采用两个可能的值，我们通常将它们称为正值或负值。 分类器正确预测的正和负实例分别称为真正 (TP) 和真负 (TN)。 同样，错误分类的实例称为假正和 (FP) 和假负 (FN)。 混淆矩阵只是一个表，其中显示了每个这四个类别下的实例数。 Azure 机器学习 Studio 的经典版自动确定数据集中两个类中的哪一个是正类。 如果类标签是布尔值或整数，则向标记为“true”或“1”的实例分配正类。 如果标签是字符串（例如，对于收入数据集），则按字母顺序对标签排序，并选择第一级作为负类，而第二级为正值。
