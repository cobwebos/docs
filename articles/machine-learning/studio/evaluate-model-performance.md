...
因此，计算捕获评估的更具体方面的其他指标很有帮助。 在详细讨论此类指标之前，了解二元分类评估的混淆矩阵很重要。 训练集中的类标签只能采用两个可能的值，我们通常将它们称为正值或负值。 分类器正确预测的正和负实例分别称为真正 (TP) 和真负 (TN)。 同样，错误分类的实例称为假正和 (FP) 和假负 (FN)。 混淆矩阵只是一个表，其中显示了每个这四个类别下的实例数。 Azure 机器学习 Studio （经典）会自动确定数据集中两个类中的哪一个是正类。 如果类标签是布尔值或整数，则向标记为“true”或“1”的实例分配正类。 如果标签是字符串（例如，对于收入数据集），则按字母顺序对标签排序，并选择第一级作为负类，而第二级为正值。
