...
* 请求-响应服务 （RRS）：一种低延迟、高度可扩展的服务，可对单个数据记录进行评分。
* 批处理执行服务 （BES）：一个异步服务，用于对一批数据记录进行评分。
...
在 **"配置"** 页上，您可以更改说明、标题、更新存储帐户密钥以及为 Web 服务启用示例数据。
...
您可以通过在 **"输入 SCHEMA"、"****输出架构**"和 **"Web 服务参数**"下为每个列输入一个字符串，为输入数据、输出数据和 Web 服务参数提供说明。 这些描述用于为 Web 服务提供的示例代码文档中。
...
要更新 Web 服务，请打开用于部署 Web 服务的原始预测实验，并通过单击 **"保存 AS"** 创建可编辑的副本。 进行更改，并单击“部署 Web 服务”****。

由于之前已部署此实验，系统会询问是要覆盖（经典 Web 服务）还是更新（新 Web 服务）现有服务。 单击 **"是**"或 **"更新**"会停止现有 Web 服务，部署新的预测实验将部署在其位置。
...
[新的 Web 服务]: #deploy-it-as-a-new-web-service
