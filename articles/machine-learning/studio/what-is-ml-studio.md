...
Microsoft Azure 机器学习工作室是一个协作型拖放式工具，可以使用它在数据上构建、测试和部署预测分析解决方案。 机器学习工作室将模型作为web服务发布，可以方便地由定制应用程序或Excel等BI工具使用。

机器学习工作室是满足数据科学、预测分析、云资源和数据处理的工作区。

[!INCLUDE [machine-learning-free-trial](../../../includes/machine-learning-free-trial.md)]

## <a name="the-machine-learning-studio-interactive-workspace"></a>机器学习工作室交互式工作区
要开发预测分析模型，通常需要使用一个或多个数据源中的数据，通过各种数据操作和统计函数来转换和分析数据，并生成一组结果。 开发此类模型是一个迭代过程。 在修改各种函数及其参数时，结果将趋于一致，直到您确信您拥有了一个训练有素的、有效的模型。

**Azure 机器学习工作室** 提供一个交互式的、可视化的工作区，可以轻松地在预测分析模型上构建、测试和迭代。 可以将***数据集***和分析***模块***拖放到交互式画布，将它们连接在一起构成***试验***，然后在机器学习工作室中运行该实验。 要在模型设计上迭代，您需要编辑试验，如果需要的情况下则保存一个副本，并重新运行该试验。 准备就绪后，可以将***训练实验***转换为***预测试验***，然后将其作为 ***Web 服务***发布，以便其他人可以访问您的模型。

不需要编程，只需要可视化地连接数据集和模块来构建预测分析模型。
...
![Azure 机器学习工作室示意图：创建试验、读取多个数据源的数据、写入评分的数据、写入模型。][ml-studio-overview]
...
* **项目** - 表示单个项目的实验、数据集、笔记本和其他资源的集合
* **实验** - 已创建、运行和保存为草稿的实验
* **WEB 服务** - 从实验中部署的 Web 服务
* **笔记本** - 创建的 Jupyter 笔记本
* **数据集** - 已上载到工作室的数据集
* **训练模型** - 已在实验中训练并保存在工作室中的模型
* **设置** - 用于配置帐户和资源的设置集合。
...
试验由向分析模块提供数据的数据集组成，将这些数据集连接在一起构成一个预测分析模型。 具体来说，一个有效的试验具有以下特征：
...
### <a name="modules"></a>模块
模块是可对数据执行的算法。 机器学习工作室有许多模块，从数据输入功能到培训、评分和验证过程。 以下是一些包含模块的例子：

* [转换为 ARFF][convert-to-arff] - 将 .NET 序列化数据集转换为属性关系文件格式 (ARFF)。
* [计算基本统计信息][elementary-statistics] - 计算基本统计信息，例如平均值、标准差等。
* [线性回归][linear-regression] - 创建在线梯度下降线性回归模型。
* [评分模型][score-model] - 得分训练分类或回归模型。

构建试验时，可以从画布左侧可用的模块列表中选择。  

模块可能提供一组参数用于配置模块的内部算法。 在画布上选择模块时，模块的参数会显示在画布右侧的“属性”窗格中。 可以在该窗格中修改参数来优化模型。
...
机器学习的术语有点难懂。 下面提供了重要术语的定义。 可以使用下面的“注释”部分告诉我们还要阐释哪些术语。
...
通过数据训练模型时，将使用已知数据集，根据数据特征对模型进行调整，以求获得最准确的结果。 在 Azure 机器学习中，模型是由处理训练数据的算法模块和功能模块(例如计分模块)构建的。

在监督式学习中，如果要训练诈骗检测模型，可以使用一组标记为诈骗或有效的交易。 随机分割数据集，并使用一部分数据训练模型，使用另一部分测试或评估模型。
...
* **模块**：机器学习工作室模型中的功能组件，例如，用于输入和编辑小型数据集的“输入数据”模块。 算法也是机器学习工作室中的一种模块类型。
...
* **预测**：预测是来自机器学习模型的一个或多个预测值。 有时称为“预测评分”。 但是，预测评分不是模型的最终输出。 评分之后还要对模型进行评估。
