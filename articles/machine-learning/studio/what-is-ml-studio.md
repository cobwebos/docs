...
[Azure 机器学习](../overview-what-is-azure-ml.md)提供了两个 SDK **和** Azure 机器学习设计器，以快速准备数据、训练和部署机器学习模型。 此设计器提供与工作室（经典版）类似的拖放体验。 但是，不像工作室（经典版）的专用计算平台，此设计器使用你自己的计算资源，并且已完全集成到 Azure 机器学习中。

下面是一个快速比较：

|| 机器学习工作室（经典版） | Azure 机器学习 |
|---| --- | --- |
| 拖放界面 | 是 | 是 - [Azure 机器学习设计器](../concept-designer.md) |
