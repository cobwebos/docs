...
使用 "[导入数据][import-data]" 模块，试验时，可以从各种在线数据源导入数据。

> [!NOTE]
> 本文提供了有关[导入数据][import-data]模块的一般信息。 有关可访问的数据类型、格式、参数以及常见问题的答案的详细信息，请参阅 "[导入数据][import-data]" 模块的 "模块参考" 主题。

通过使用 "[导入数据][import-data]" 模块，你可以在试验运行时从多个联机数据源之一访问数据：
...
若要在 Studio （经典）试验中访问联机数据源，请将[导入数据][import-data]模块添加到试验中。 然后，在“属性”下选择“启动导入数据向导”以根据分步指导说明来选择并配置数据源。 另外，也可以手动在“属性”下选择“数据源”并提供访问数据所需的参数。

下表列举了支持的联机数据源。 此表还汇总了支持的文件格式，以及用于访问数据的参数。

> [!IMPORTANT]
> 目前，[导入数据][import-data]和[导出数据][export-data]模块只能从使用经典部署模型创建的 Azure 存储读取和写入数据。 换言之，目前尚不支持可提供热存储访问层或冷存储访问层的新式 Azure Blob 存储帐户类型。
>
