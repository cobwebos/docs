...
+ **Padding** 的默认值为 False，因此目标层的每个维度中只有 10 个节点。 要包括一个与源层中每个节点对应的目标层中的节点，可添加 Padding = [true, true, true]；并将 RN1 的大小更改为 [5, 12, 12]。
...
+ 卷积的实参数量为三：元组的长度 `InputShape`、`KernelShape`、`Stride`和 `Sharing`。
