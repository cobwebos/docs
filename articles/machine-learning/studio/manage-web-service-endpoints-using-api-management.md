...
Azure API 管理是一项 Azure 服务，支持通过定义用户访问权限、使用限制和仪表板监视管理 REST API 终结点。 有关更多详细信息，请参阅 [Azure API 管理站点](https://azure.microsoft.com/services/api-management/)。 若要从 Azure API 管理着手，请参阅[导入和发布指南](/azure/api-management/import-and-publish)。 这份额外指南（作为本指南的依据）涵盖更多主题，包括通知配置、层定价、响应处理、用户身份验证、创建产品、开发人员订阅和使用仪表板。
...
4. 单击左侧的 **"响应 ADD"，** > **ADD**选择**200 个"确定**"。
5. 单词“保存”****，保存此操作。
...
5. 单击左侧的 **"响应 ADD"，** > **ADD**选择**200 个"确定**"。
6. 单击“保存”  。
...
6. 单击左侧的 **"响应 ADD"，** > **ADD**选择**200 个"确定**"。
7. 单击“保存”  。
...
可以直接从开发人员门户调用操作，这样可以方便地查看和测试 API 的操作。 在此步骤中，您将调用添加到**AzureML 演示 API**的**RRS 执行**方法。 
...
首先，使用您选择的浏览器，导航到：[https://studio.azureml.net/](https://studio.azureml.net/)并输入凭据登录。 接下来，创建新的空白实验。
...
AzureML Web 服务包含 RSS（请求/响应服务）和 BES（批处理执行服务）终结点。 RSS 适用于同步执行。 BES 适用于异步作业执行。 若要通过下面的 Python 源示例测试 Web 服务，可能需要下载并安装 Azure SDK for Python（请参阅[如何安装 Python](/azure/developer/python/azure-sdk-install)）。
