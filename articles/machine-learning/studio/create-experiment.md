...
本教程介绍如何在首次使用工作室时创建机器学习试验。 此试验将测试一个分析模型，该模型基于汽车品牌和技术规格等不同变量来预测汽车的价格。

> [!NOTE]
> 本教程介绍各种基础知识：如何将模块拖放到试验中、如何将它们连接到一起、如何运行试验以及查看结果。 我们不讨论机器学习这一宽泛主题，也不讨论如何选择和使用工作室中包括的 100 多种内置算法和数据操作模块。
>
>如果您是机器学习的初学者，可先观看视频系列：[适合初学者的数据科学](data-science-for-beginners-the-5-questions-data-science-answers.md)。 该视频系列使用日常语言和概念对机器学习进行了很好的介绍。
>
...
可以反复迭代进行模型设计，不断地编辑和运行试验，直至获得所要的结果。 模型准备就绪以后，可将其作为 ***Web 服务*** 发布，方便其他用户向其发送新的数据并获得相应的预测结果。

## <a name="open-machine-learning-studio"></a>打开机器学习工作室

若要开始使用学习工作室，请转到 [https://studio.azureml.net](https://studio.azureml.net)。 如果以前登录过机器学习工作室，请单击“登录”。 否则，请单击“在此注册”并选择免费或付费选项。
...
若要进行机器学习，首先需获取数据。
可以使用机器学习工作室所包含的多个示例数据集，也可以从多种数据源导入数据。 本示例将使用工作区中包含的示例数据集“汽车价格数据(原始)”。
...
数据集通常需要经过一些预处理才能进行分析。 例如，您可能已注意到，在多个行的列中存在缺失值。 需要清除这些缺失值，使模型能够正确分析数据。 在本例中，将删除包含缺失值的所有行。 此外，“标准化损失”列包含较大比例的缺失值，因此要将该列从模型中完全排除。

> [!TIP]
> 使用大多数模块时，都必须从输入数据中清除缺失值。

首先添加一个彻底删除“标准化损失”列的模块，然后添加另一个删除任何有缺失数据的行的模块。
...
3. 单击[选择数据集中的列][select-columns] 模块，并单击“属性”窗格中的“启动列选择器”。
    ...
    - 在下拉列表中，选择“排除”和“列名称”，并在文本框内部单击。 此时会显示列的列表。 选择“标准化损失”，该列随即添加到文本框中。
    - 单击复选标记（“确定”）按钮，关闭列选择器（右下角）。

    ![启动列选择器，排除“标准化损失”列][launch-column-selector]
    <br/>
    ***启动列选择器，排除“标准化损失”列***

    此时“选择数据集中的列”的属性窗格指示它将传入数据集中的所有列，但“标准化损失”除外。

    ![属性窗格显示“标准化损失”列已排除][showing-excluded-column]
    <br/>
    ***属性窗格显示“标准化损失”列已排除***

    > [!TIP]
    可以双击模块并输入文本，为模块添加注释。 这有助于快速查看模块在实验中的运行情况。 在本例中，请双击[选择数据集中的列][select-columns]模块，并键入注释“排除标准化损失”。
