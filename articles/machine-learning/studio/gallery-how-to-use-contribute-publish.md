...
1. 在库中，输入描述性的**名称**和**标记**。 突出显示使用过的技术或要解决的实际问题。 描述性实验标题的一个示例是"二进制分类：Twitter 情绪分析"。
...
**我在实验中使用的数据集会发生什么情况？数据集是否也在库中发布？**
...
**我有一个实验，使用导入数据模块从 Azure HDInsight 或 SQL Server 中提取数据。它使用我的凭据来检索数据。我可以发布这种实验吗？如何确保我的凭据不会共享？**
