...
* **自定义模块**-可将自定义模块下载到工作室（经典）工作区，以便在自己的实验中使用。
...
你可以将**试验**、**自定义模块**和**Jupyter 笔记本**下载到你自己的机器学习 Studio （经典）工作区中，以用于开发你自己的解决方案。
...
如果需要，可在已发布到库的实验中对工作流（模块、参数等）进行更改。 在机器学习 Studio （经典）中，进行所需的更改，然后再次发布。 已发布的实验将随更改进行更新。
