---
title: 重新训练机器学习工作室模型 - Azure | Microsoft Docs
...
重新训练可能会经常发生。 使用“编程重新训练 API”功能，可以使用“重新训练 API”以编程方式重新训练模型，并使用新训练的模型更新 Web 服务。 
...
在这些情况下，编程 API 提供了一种便捷的方法，允许你或 API 的使用者创建一个客户端，可以一次性或定期地使用自己的数据重新训练模型。 然后，他们可以评估重新训练的结果，并更新 Web 服务 API 以使用新训练的模型。

> [!NOTE]
> 如果有现有的训练实验和新 Web 服务，建议检查重新训练现有预测 Web 服务，而不要按照下一节中提到的步骤。
> 
> 
...
* [使用 PowerShell 从一个实验中创建多个机器学习模型和 Web 服务终结点](create-models-and-endpoints-with-powershell.md)
