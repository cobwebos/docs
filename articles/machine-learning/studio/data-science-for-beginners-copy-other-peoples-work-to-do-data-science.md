---
title: 复制其他人的数据科学示例
titleSuffix: Azure Machine Learning Studio (classic)
description: 数据科学的商业机密：让他人完成工作。 从 Azure AI 库获取机器学习示例。
services: machine-learning
ms.service: machine-learning
ms.subservice: studio
ms.topic: conceptual
author: sdgilley
ms.author: sgilley
ms.custom: seodec18
ms.date: 03/22/2019
ms.openlocfilehash: 248696188c2f93104987e395479fdee49cdecf94
ms.sourcegitcommit: c22327552d62f88aeaa321189f9b9a631525027c
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/04/2019
ms.locfileid: "73493154"
---
# <a name="copy-other-peoples-work-to-do-data-science"></a>复制其他人的工作以执行数据科研
## <a name="video-5-data-science-for-beginners-series"></a>视频 5：适合初学者的数据科学系列
数据科学的商业机密之一是让其他人完成工作。 在 Azure AI 库中查找适合用于自己的机器学习试验的聚类分析算法示例。

> [!IMPORTANT]
> “Cortana Intelligence 库”已重命名为“Azure AI 库”。 因此，本文中的文本和图像与视频略有不同，视频中使用原来的名称。
>

若要充分利用此系列内容，请观看所有视频。 [转到视频列表](#other-videos-in-this-series)
<br>

> [!VIDEO https://channel9.msdn.com/Blogs/Azure/data-science-for-beginners-series-copy-other-peoples-work-to-do-data-science/player]
>
>

## <a name="other-videos-in-this-series"></a>本系列中的其他视频
“适合初学者的数据科学”对数据科学进行简要介绍，共五个短视频。

* 视频 1：[数据科学可解答的 5 个问题](data-science-for-beginners-the-5-questions-data-science-answers.md)（5 分 14 秒）
* 视频 2：[数据是否可用于数据科学？](data-science-for-beginners-is-your-data-ready-for-data-science.md) （4 分 56 秒）
* 视频 3：[提出一个可用数据回答的问题](data-science-for-beginners-ask-a-question-you-can-answer-with-data.md)（4 分 17 秒）
* 视频 4：[使用简单的模型预测答案](data-science-for-beginners-predict-an-answer-with-a-simple-model.md)（7 分 42 秒）
* 视频 5：复制他人的工作以执行数据科研

## <a name="transcript-copy-other-peoples-work-to-do-data-science"></a>抄本：复制他人的工作以执行数据科研
欢迎观看“适合初学者的数据科学”系列中的第五个视频。

在本视频中，将发现一个可查找示例的位置，可以在其中查找可借用来作为自己工作起点的示例。 如果先观看了本系列先前的视频，可能会更充分地利用此视频。

数据科学的商业机密之一是让其他人完成工作。

## <a name="find-examples-in-the-azure-ai-gallery"></a>在 Azure AI 库中查找示例

Microsoft 有一个称为[Azure 机器学习 Studio （经典）](https://azure.microsoft.com/services/machine-learning-studio/)的基于云的服务。 它提供了一个工作区，可在其中使用不同的机器学习算法进行试验，并且在已制定出解决方案时，可以 Web 服务启动它。

此服务的一部分被称为 **[Azure AI 库](https://gallery.azure.ai/)** 。 它包含一些资源，其中包括用户已构建并提供给其他人使用的 Azure 机器学习 Studio （经典）试验或模型的集合。 这些试验是利用他人的想法和工作成果，开始自己的解决方案的绝佳方式。 欢迎每个人前往浏览。

![Azure AI 库](./media/data-science-for-beginners-copy-other-peoples-work-to-do-data-science/azure-ai-gallery.png)

如果在顶部单击“试验”，可看到库中最新和最常用的试验。 在屏幕顶部，单击“浏览全部”，可在其余的试验中进行搜索，还可输入搜索词并选择搜索筛选器。

## <a name="find-and-use-a-clustering-algorithm-example"></a>查找并使用聚类分析算法示例
例如，假设想要查看聚类分析的工作原理的示例，可搜索“聚类分析扫描”试验。

![搜索聚类分析试验](./media/data-science-for-beginners-copy-other-peoples-work-to-do-data-science/search-for-clustering-experiments.png)

下面是某用户向库提供的一个有趣试验。

![聚类分析试验](./media/data-science-for-beginners-copy-other-peoples-work-to-do-data-science/clustering-experiment.png)

单击该实验，可获取一个网页，它描述了此参与者完成的工作以及一些结果。

![聚类分析试验描述页](./media/data-science-for-beginners-copy-other-peoples-work-to-do-data-science/clustering-experiment-description-page.png)

请注意显示 "**在工作室中打开" 的链接（经典）** 。

!["在 Studio 中打开" 按钮](./media/data-science-for-beginners-copy-other-peoples-work-to-do-data-science/open-in-studio.png)

我可以单击该操作，它会直接转到**Azure 机器学习 Studio （经典）** 。 它将复制一份试验，并将其放入自己的工作区中。 这包括参与者的数据集、他们所做的所有处理、所用的所有算法以及保存结果的方法。

![在机器学习 Studio （经典）-群集算法示例中打开库实验](./media/data-science-for-beginners-copy-other-peoples-work-to-do-data-science/cluster-experiment-open-in-studio.png)

现在我已有起始点。 我可以将他们的数据置换为我自己的数据，并自行调整模型。 这给了我一个良好的开端，让我可以根据他人的工作来生成我自己的内容，这些人真正了解他们的工作内容。

## <a name="find-experiments-that-demonstrate-machine-learning-techniques"></a>查找演示机器学习技术的试验
[Azure AI 库](https://gallery.azure.ai)中有一些其他试验，专门为数据科学新手提供操作方法示例。 例如，库中有一个试验演示了如何处理缺失值（[Methods for handling missing values](https://gallery.azure.ai/Experiment/Methods-for-handling-missing-values-1)（处理缺失值的方法））。 它指导通过 15 种不同的方法来替代空值，并讨论了每种方法的优势以及使用时间。

![机器学习 Studio 中打开的库试验（经典）-缺少值的方法](./media/data-science-for-beginners-copy-other-peoples-work-to-do-data-science/experiment-methods-for-handling-missing-values.png)

可在 [Azure AI 库](https://gallery.azure.ai)中查找工作试验，并将这些工作试验用作自己解决方案的起始点。

请务必查看 "适用于初学者的数据科学" 中的其他视频，Microsoft Azure 机器学习工作室（经典）。

## <a name="next-steps"></a>后续步骤
* [试用 Azure 机器学习 Studio （经典）进行第一次数据科学试验](create-experiment.md)
* [获取 Microsoft Azure 上的机器学习的简介](/azure/machine-learning/preview/overview-what-is-azure-ml)
