...
Azure 应用商店提供的 web 应用模板可创建自定义 web 应用，该应用了解 web 服务的输入数据及所需结果。 所需操作就是授予 web 应用访问 web 服务和数据的权限，余下操作由模板完成。
...
5. 首次运行 web 应用时，会要求提供 **API Post URL** 和 **API 密钥**。
...
6. Web 应用以当前 web 服务设置显示其“Web 应用配置”页面。 可在此处更改 web 应用使用的设置。
   ...
   > 
    ...
    完成后，单击“保存更改”，并单击“转到主页”。

7. 可以在主页中输入要发送到 Web 服务的值。 操作完成后，单击“提交”，将返回结果。

如果希望返回“配置”页，请转到 web 应用的 `setting.aspx` 页。 例如：`http://carprediction.azurewebsites.net/setting.aspx.`。会提示再次输入 API 密钥 - 需要密钥来访问该页面和更新设置。

与其他 web 应用一样，可以在 Azure 门户暂停、重启或删除 web 应用。 只要应用正在运行，就可以浏览至主页网址并输入新值。
...
因此，需要一个 Azure 存储容器来保存 web 应用程序返回的结果，需要准备好的输入数据。

![使用 BES web 模板的过程][image2]

1. 请按照与 RRS 模板相同的步骤创建 BES Web 应用，不同的是需转到 [Azure ML Batch Execution Service Web App Template](https://azure.microsoft.com/marketplace/partners/microsoft/azuremlbeswebapptemplate/)（Azure ML 批处理执行服务 Web 应用模板）打开 Azure 应用商店上的 BES 模板，并单击“创建 Web 应用”。
...
* 通过机器学习工作室创建一个机器学习实验，请参阅 [Create your first experiment in Azure Machine Learning Studio](create-experiment.md)（在 Azure 机器学习工作室中创建第一个试验）
* 如何将机器学习实验部署为 web 服务，请参阅 [Deploy an Azure Machine Learning web service](publish-a-machine-learning-web-service.md)（部署 Azure 机器学习 web 服务）
