...
此处已编号的每个端对端演练展示了**特定方案**在团队数据科学过中的各步骤。 他们展示了如何将云、本地工具以及服务结合到一个工作流或管道中，以创建智能应用程序。
...
[操作中的团队数据科学过程：在 1TB 的数据集上使用 Azure HDInsight Hadoop 群集](machine-learning-data-science-process-hive-criteo-walkthrough.md)演练呈现了一个端对端方案，该方案使用 [Azure HDInsight Hadoop 群集](https://azure.microsoft.com/services/hdinsight/)存储、浏览并描绘工程师数据特征，从公用 [Criteo](http://labs.criteo.com/downloads/download-terabyte-click-logs/) 数据集下载示例数据。

## <a name="data-science-using-python-with-spark-on-azure"></a>Azure 上使用 Python 与 Spark 的数据科学
[Azure HDInsight 上使用 Spark 的数据科学](machine-learning-data-science-spark-overview.md)演练在端对端方案中使用了团队数据科学过程，该方案使用 [Azure HDInsight Spark 群集](https://azure.microsoft.com/services/hdinsight/)从纽约市公用计程车车程和费用数据集存储、浏览工程师数据并描绘其特征。 

## <a name="data-science-using-scala-with-spark-on-azure"></a>Azure 上使用 Scala 与 Spark 的数据科学
[Azure 上使用 Scala 与 Spark 的数据科学](machine-learning-data-science-process-scala-walkthrough.md)演练介绍了如何使用通过 Spark 可缩放机器学习库 (MLlib) 和 Azure HDInsight Spark 群集上的 SparkML 包使用针对受监督的机器学习任务的 Scala。 它将指导你完成构成[数据科学过程](http://aka.ms/datascienceprocess)的任务：数据引入和浏览、可视化、特征工程、建模和模型使用。 生成的模型包括逻辑和线性回归、随机林和梯度提升树。

## <a name="use-azure-data-lake-storage-and-analytics"></a>使用 Azure Data Lake 存储和分析
