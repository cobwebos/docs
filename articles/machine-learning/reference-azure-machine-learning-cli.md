...
* 开始运行试验。 使用此命令时，请指定 .runconfig 文件的名称 \*（如果您正在查看文件系统，则为 .runconfig）。
    ...
    >
    > 在此[JSON 文件](https://github.com/microsoft/MLOps/blob/b4bdcf8c369d188e83f40be8b748b49821f71cf2/infra-as-code/runconfigschema.json)中可以找到完整的 .runconfig 架构。 该架构通过每个对象的 `description` 键自行记录。 此外，还有一些可能值的枚举，以及末尾的模板代码段。
...
+ 弃用数据集：
    ...
    有关详细信息，请参阅[az ml dataset 弃用](https://docs.microsoft.com/cli/azure/ext/azure-cli-ml/ml/dataset?view=azure-cli-latest#ext-azure-cli-ml-az-ml-dataset-archive)。
...
+ 重新激活已存档或弃用的数据集：
    ...
    有关详细信息，请参阅[az ml dataset 重新激活](https://docs.microsoft.com/cli/azure/ext/azure-cli-ml/ml/dataset?view=azure-cli-latest#ext-azure-cli-ml-az-ml-dataset-archive)。
...
+ 取消注册数据集：
    ...
    有关详细信息，请参阅[az ml dataset 取消注册](https://docs.microsoft.com/cli/azure/ext/azure-cli-ml/ml/dataset?view=azure-cli-latest#ext-azure-cli-ml-az-ml-dataset-archive)。
...
如果使用 `az ml environment scaffold` 命令，它将生成一个模板 `azureml_environment.json` 文件，可以修改该模板并将其用于使用 CLI 创建自定义环境配置。 顶层对象松映射到 Python SDK 中的[`Environment`](https://docs.microsoft.com/python/api/azureml-core/azureml.core.environment(class)?view=azure-ml-py)类。 
...
| `inferencingStackVersion` | `string` | 指定添加到映像的推断堆栈版本。 若要避免添加推断堆栈，请将此字段 `null`。 有效值： "最新"。 |
