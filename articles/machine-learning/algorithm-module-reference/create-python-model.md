...
> [!WARNING]
> 目前，不能将此模块连接到**微调模型超参数**模块，或将 Python 模型的评分结果传递给[评估模型](evaluate-model.md)。 如果需要优化超参数或评估模型，可以使用 "[执行 Python 脚本](execute-python-script.md)" 模块编写自定义 Python 脚本。
