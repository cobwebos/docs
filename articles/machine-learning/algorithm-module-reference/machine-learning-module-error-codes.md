...
## <a name="error-0013"></a>错误0013  
 ...
 当定型模型与连接计分模块不兼容时, 会发生此错误。 <!--For example, connecting the output of [Train Matchbox Recommender](train-matchbox-recommender.md) to [Score Model](score-model.md) (instead of [Score Matchbox Recommender](score-matchbox-recommender.md)) will generate this error when the experiment is run.  -->
...
|数据集中的列数超出了允许的限制。|  
|"{0}" 中的数据集中的列数超出了允许值。|  
|"{0}" 中的数据集中的列数超出了允许的 "{1}" 的限制。|  
|"{0}" 中的数据集中的列数超出了{1}允许的{2}"" 限制 ""。|  
...
|未能转换: {0}。|  
|无法在行{1}上{0}转换:。|  
|无法将类型{0}的列转换为行{2}类型{1}的列。|  
|无法将类型{0}为的{2}列 "" 转换为行{3}类型{1}的列。|  
|无法将类型{0}为的{2}列 "" 转换为行{3} {4}类型{1}的列 ""。| 
...
|所选数值列的总数和分类和字符串列中的唯一值 (当前{0}) 应至少为{1}|  
...
|------------------------|  
|无法分析 GitHub URL。|  
|无法分析 GitHub URL (存储库名称\\后应为 "\\\blob" 或 "\tree"):{0}|  
...
|------------------------|  
|指定的服务器名称无法解析|  
|无法解析指定{0}的 documents.azure.com|
