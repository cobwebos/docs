...
1. 使用“联接类型”**** 下拉列表指定应当如何组合数据集。  
    ...
    * **左外部联接**：*左外部联接*返回从左表中的所有行联接行。 如果左表中的某行在右表中没有匹配的行，则对于来自右表的所有列，返回的行将包含缺失值。 你还可以为缺失值指定替换值。  
  
    * **完全外部联接**：*完全外部联接*返回左表 （**表 1**） 和右表 （**表2）** 的所有行。  
  
         对于任一表中在另一表中没有匹配行的每个行，结果中将包括含缺失值的一个行。  
  
    * **左半联接**：当键列的值匹配时，*左半联接*仅返回左表中的值。  
