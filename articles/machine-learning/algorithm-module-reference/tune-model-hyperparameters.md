...
术语“参数”和“超参数”可能会造成混淆。**** 模型*的参数*是您在模块右侧窗格中设置的参数。 简单而言，此模块将对指定的参数设置执行参数扫描。** 它将会习得一组最佳的超参数，而这些超参数对于每个特定的决策树、数据集或回归方法可能有所不同。__ 查找最佳配置的过程有时称为“优化”。** 
...
5.  在"调模型超参数"的右侧面板中，为**参数扫描模式**选择值。 此选项控制参数的选择方式。

    - **整个网格**：当您选择此选项时，模块会循环在系统预定义的网格上，以尝试不同的组合并识别最佳学员。 如果你不知道最佳的参数设置是什么，并想要尝试所有可能的值组合，则此选项很有用。

    - **随机扫描**：当您选择此选项时，模块将在系统定义的范围内随机选择参数值。 必须指定要让模块执行的最大运行次数。 如果你想要使用所选的指标提高模型性能，同时仍要节省计算资源，则此选项很有用。    

6.  对于“标签列”，打开列选择器以选择单个标签列。****

7.  选择运行次数：

    1. **随机扫描时的最大运行次数**：如果选择随机扫描，则可以使用参数值的随机组合指定应训练模型的次数。
...
+ 要查看最佳模型的一组精度指标，请右键单击模块，然后选择 **"可视化**"。
...
+ **整个网格**：使用整个网格的选项意味着每个组合都经过测试。 此选项是最彻底的，但所需的时间最长。 
