...
1.  对于“阈值集”****，从下拉列表中选择以下选项之一。 这些选项确定如何为可接受的值以及必须剪切的值设置上限和下限。  
    ...
    - **剪辑峰**：按峰值裁剪值时，只指定上边界。 将替换大于该上限值的值。
  
    -  **剪辑子峰值**：按子峰值剪辑值时，仅指定较低边界。 将替换小于该下限值的值。  
  
    - **剪辑峰和子峰**：当您按峰值和子峰值裁剪值时，可以指定上边界和下边界。 超出该范围的值将被替换。 不会更改与限值匹配的值。
  
1.  根据在上一步中所做的选择，可以设置以下阈值： 

    + **较低的阈值**：仅在选择 **"剪辑子峰值**"时显示
    + **上限**：仅在选择 **"剪辑峰**"时显示
    + **阈值**： 仅在选择**剪辑峰和子峰**时显示
...
1.  定义替换值。
    ...
    + **峰值的替代值**：定义值以替换大于指定阈值的所有列值。  
    + **副峰的替换值**：定义用作小于指定阈值的所有列值的值。  
    ...
    -   **均值**： 将剪切的值替换为列值的平均值。 在对值进行剪切之前计算平均值。  
  
    -   **中位数**： 将剪切的值替换为列值的中位数。 在对值进行剪切之前计算中值。   
  
    -   **缺少**。 将剪切值替换为缺失（空）值。  
  
1.  **添加指示器列**：如果要生成一个新列，告诉您指定的裁剪操作是否应用于该行中的数据，请选择此选项。 如果要测试一组新的剪切和替换值，此选项会很有用。  
  
1. **覆盖标志**：指示您希望如何生成新值。 默认情况下，**剪切值**会构建一个新列，其中峰值已剪切为所需阈值。 新值会覆盖原始列。  
  
    若想保留原始列并添加含剪切值的新列，请取消选择此选项。  
  
1.  提交管道。  
  
    右键单击 **"剪辑值"** 模块并**选择"可视化"** 或选择模块，然后切换到右侧面板中的 **"输出"** 选项卡，单击 **"端口输出**"中的直方图图标，查看值并确保裁剪操作符合您的预期。  
...
1.  对于“上限阈值”，选择“百评分”；对于“百评分数”，键入 90。************  
  
1.  对于“上限替换值”，选择“缺失值”。********  
  
1.  对于“下限阈值”，选择“百评分”；对于“百评分数”，键入 10。************  
  
1.  对于“下限替换值”，选择“缺失值”。********  
  
1.  取消选择“覆盖标志”选项，然后选择“添加指示器列”选项。********  
