...
ms.locfileid: "80064246"
---
# <a name="edit-metadata-module"></a>“编辑元数据”模块

本文介绍 Azure 机器学习设计器（预览版）中包含的一个模块。

使用"编辑元数据"模块更改与数据集中的列关联的元数据。 使用“编辑元数据”模块后，数据集的值和数据类型将会更改。
...
1. 在 Azure 机器学习设计器中，将"编辑元数据"模块添加到管道中，并连接要更新的数据集。 您可以在 **"数据转换**"类别中找到该模块。
  
1. 单击模块右侧面板中的 **"编辑"列**，然后选择要处理的列或列集。 可以按名称或索引单独选择列，也可以按类型选择一组列。  
...
1. 若要更改 Azure 机器学习在模型中使用数据的方式，请使用“字段”选项。****
    ...
    + **功能**：使用此选项将列标记为仅在要素列上运行的模块中的要素。 默认情况下，最初会将所有列视为特征。  
  
    + **标签**：使用此选项来标记标签，该标签也称为可预测属性或目标变量。 许多模块要求数据集中刚好存在一个标签列。
...
1. 使用以下选项可以清除前面所做的选择，并将元数据还原为默认值。  
    ...
    + **清除标签**：使用此选项从指定的列中删除**标签**元数据。  
  
    + **清除分数**：使用此选项从指定的列中删除**分数**元数据。  
