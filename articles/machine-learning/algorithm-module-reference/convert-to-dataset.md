...
3.  在“操作”**** 下拉列表中，指示是否要在保存数据集之前对数据执行任何清理操作：  
    ...
    - **设置缺失值**：将特定值设置为数据集中缺少的值。 默认占位符是问号字符 (?)，但你可以使用“自定义缺失值”**** 选项来输入其他值。 例如，如果你为“自定义缺失值”输入了 **Taxi******，则数据集中的所有 **Taxi** 实例都将更改为缺失值。
  
    - **替换值**：使用此选项指定要替换为任何其他精确值的单一精确值。 可以通过设置“替换”**** 方法来替换缺失值或自定义值：

      - **缺少**：选择此选项可替换输入数据集中缺少的值。 对于“新值”****，请输入要用来替换缺失值的值。
      - **自定义**：选择此选项可替换输入数据集中的自定义值。 对于“自定义值”****，请输入要查找的值。 例如，如果数据包含用作缺失值占位符的字符串 `obs`，则输入 `obs`。 对于“新值”****，请输入用来替换原始字符串的新值。
...
+  要使用新名称保存生成的数据集，请在模块右侧面板中的"**输出"** 选项卡下的图标 **"注册"数据集中**选择。  
