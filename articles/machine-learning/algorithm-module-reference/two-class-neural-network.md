...
3.  对于“隐藏层规范”****，请选择要创建的网络体系结构的类型。  
    ...
    -   **完全连接的情况**：使用为双类神经网络定义的默认神经网络体系结构，如下所示：
...
10. 选择“随机选择示例”选项可在迭代间随机选择事例。**** 如果取消选择此选项，则每次运行管道时，都会以完全相同的顺序处理事例。
  
11. 对于“随机数种子”，键入一个值来用作种子****。
  
     指定种子值对于确保同一管道的运行可重复性非常重要。  否则，将使用系统时钟值作为种子，这可能会导致每次运行管道时产生略微不同的结果。
