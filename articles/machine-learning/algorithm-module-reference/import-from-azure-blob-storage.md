---
title: 从 Azure Blob 存储导入：模块引用
titleSuffix: Azure Machine Learning service
description: 了解本主题介绍了如何使用 Azure 机器学习 service 中的 "从 Azure Blob 存储导入" 模块从 Azure blob 存储中读取数据，以便你可以在机器学习管道中使用数据。
...
1. 将**导入数据**模块添加到管道。 可以在接口中的**数据输入和输出**中找到此模块
...
5. 如果你的数据位于**专用**帐户，则必须提供凭据，包括帐户名称和密钥。
    ...
    - 对于 "**帐户名称**"，请键入或粘贴包含要访问的 blob 的帐户的名称。
        ...
        例如，如果存储帐户的完整 URL 是 `http://myshared.blob.core.windows.net` 的，则键入 `myshared`。

    - 对于 "**帐户密钥**"，请粘贴与帐户关联的存储访问密钥。

        如果你不知道访问密钥，请参阅本文中的 "管理 Azure 存储帐户" 部分：[关于 Azure 存储帐户](https://docs.microsoft.com/azure/storage/storage-create-storage-account)。
...
6. 对于**容器、目录或 blob 的路径**，键入要检索的特定 blob 的名称。
    ...
    因此，在**容器、目录或 blob**的字段路径中，你将键入： `trainingdata/data01.csv`

    若要导入多个文件，可以使用通配符 `*` （星号）或 `?` （问号）。
    ...
    > [!NOTE]
    > 如果选择了选项，请**使用缓存的结果**，对容器中的文件所做的任何更改都不会触发对管道中的数据的刷新。
...
7. 对于 " **blob 文件格式**"，请选择一个选项来指示存储在 blob 中的数据的格式，以便 Azure 机器学习可以相应地处理数据。 支持以下格式：
    ...
    - **TSV**：制表符分隔值（TSV）是由许多机器学习工具使用的格式。 如果数据已包含标题行，请确保选择 "**文件具有标题行**" 选项，否则标头将被视为数据行。
