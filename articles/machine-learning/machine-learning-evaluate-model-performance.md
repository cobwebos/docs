...
ms.openlocfilehash: e3596731639d0cd8884c269d228f264bd83b400c
...
[评估模型][evaluate-model]预期评分数据集为输入（如果要比较 2 个不同模型的性能，则为 2）。 这意味着需要使用[训练模型][train-model]模块训练模型并使用[评分模型][score-model]模块对某些数据集进行预测，然后才能评估结果。 评估基于评分标签/概率以及真实标签，所有这些由[评分模型][score-model]模块输出。
