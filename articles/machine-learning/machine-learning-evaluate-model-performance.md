...
本主题演示如何在 Azure 机器学习工作室中评估模型的性能，并提供可用于此任务的指标的简短说明。 显示了三个常见的监督学习方案： 
...
Azure 机器学习通过其两个主要的机器学习模块支持模型评估：[评估模型][评估模型]和[交叉验证模型][cross-validate-model]。 这些模型允许查看模型在一些机器学习中常用的指标和统计信息方面性能如何。

## <a name="evaluation-vs-cross-validation"></a>评估与交叉验证
评估和交叉验证是测量模型性能的标准方法。 它们都生成可检查或与其他模型比较的评估指标。

[评估模型][评估模型]预期评分数据集为输入（如果要比较 2 个不同模型的性能，则为 2）。 这意味着需要使用[训练模型][train-model]模块训练模型并使用[评分模型][评分模型]模块对某些数据集进行预测，然后才能评估结果。 评估基于评分标签/概率以及真实标签，所有这些由[评分模型][评分模型]模块输出。
...
在以下部分中，我们将使用[评估模型][评估模型]和[交叉验证模型][cross-validate-model]模块生成简单的回归和分类模型，并评估其性能。
...
运行实验后，可单击[评估模型][评估模型]模块的输出端口，并选择“可视化”查看评估结果。 可用于回归模型的评估指标包括：*平均绝对误差*、*根平均绝对误差*、*相对绝对误差*、*相对方误差*和*决定系数*。
...
如下面的图 5 所示连接端口，并将[训练模型][train-model]模块的标签列设置为*收入*。
...
### <a name="inspecting-the-evaluation-results"></a>检查评估结果
运行实验后，可单击[评估模型][评估模型]模块的输出端口，并选择“可视化”查看评估结果（图 7）。 可用于二元分类模型的评估指标包括：*准确性*、*精度*、*召回率*、*F1 分数*和 *AUC*。 此外，模块输出一个显示真正、假负、假正和真负的混淆矩阵，以及 *ROC*、*精度/召回率*和*升力*曲线。
...
单击[导入数据][import-data]模块并将*数据源*属性设置为*通过 HTTP 的 Web URL*，并将 *URL* 设置为 http://archive.ics.uci.edu/ml/machine-learning-databases/iris/iris.data。
...
### <a name="inspecting-the-evaluation-results"></a>检查评估结果
运行实验并单击[评估模型][评估模型]的输出端口。 在此情况下，评估结果以混淆矩阵形式显示。 矩阵显示所有 3 个类的实际和预测实例。
