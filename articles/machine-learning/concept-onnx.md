...
[ONNX 运行时](https://onnxruntime.ai)是一种用于将 ONNX 模型部署到生产环境的高性能推理引擎。 它针对云和 Edge 进行了优化，适用于 Linux、Windows 和 Mac。 在 c + + 中，它还具有用于在各种环境下使用的 C、Python、c #、Java 和 Javascript （Node.js） Api。 ONNX 运行时支持 DNN 和传统的 ML 模型，并与不同硬件（例如 TensorRT 上的 NVidia Gpu、OpenVINO 上的、Windows 等）集成。 通过使用 ONNX 运行时，可以从大量的生产级优化、测试和不断改进中受益。

ONNX 运行时用于高规模 Microsoft 服务，例如必应、Office 和 Azure 认知服务。 性能提升取决于许多因素，但这些 Microsoft 服务已看到__CPU 平均性能__大幅提高。 除了 Azure 机器学习服务之外，ONNX 运行时还在支持机器学习工作负荷的其他产品中运行，包括：
+ Windows：运行时作为[windows 机器学习](https://docs.microsoft.com/windows/ai/windows-ml/)的一部分内置于 windows，并在数百万台设备上运行。 
+ Azure SQL 产品系列：对[AZURE Sql Edge](https://docs.microsoft.com/azure/azure-sql-edge/onnx-overview)和[azure sql 托管实例](https://docs.microsoft.com/azure/azure-sql/managed-instance/machine-learning-services-overview)中的数据运行本机计分。
...
可通过以下几种方式获取 ONNX 模型：
+ 在 Azure 机器学习中训练新的 ONNX 模型（请参阅本文底部的示例）或使用[自动机器学习功能](concept-automated-ml.md#automl--onnx)
...
许多模型（包括图像分类、对象检测和文本处理）都可以表示为 ONNX 模型。 如果遇到无法成功转换的模型的问题，请在使用的相应转换器的 GitHub 中指出问题。 可以继续使用现有的格式模型，直到问题得到解决。
...
有关创建和部署 ONNX 模型的 Python 笔记本示例，请参阅[如何使用 azureml/部署/onnx](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/deployment/onnx) 。
