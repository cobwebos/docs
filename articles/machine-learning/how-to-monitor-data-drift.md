...
使用 Azure 机器学习，可以监视 AKS 上部署的模型的输入，并将此数据与模型的定型数据集进行比较。 固定时间间隔，推理数据是[快照和分析](https://docs.microsoft.com/python/api/azureml-core/azureml.core.dataset.dataset?view=azure-ml-py)的，然后针对基线数据集进行计算，以生成数据偏移分析： 

+ 度量数据偏移量（称为偏移系数）。
+ 按功能测量数据偏移量，指示哪些功能导致了数据偏移。
