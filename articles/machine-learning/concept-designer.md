...
ms.locfileid: "83654936"
---
# <a name="what-is-azure-machine-learning-designer-preview"></a>什么是 Azure 机器学习设计器（预览版）？ 
[!INCLUDE [applies-to-skus](../../includes/aml-applies-to-enterprise-sku.md)]

借助 Azure 机器学习设计器，你可以在交互式画布上直观地连接[数据集](#datasets)和[模块](#module)，以创建机器学习模型。 若要了解如何开始使用设计器，请参阅[教程：使用设计器预测汽车价格](tutorial-designer-automobile-price-train-score.md)

![Azure 机器学习设计器示例](./media/concept-designer/designer-drag-and-drop.gif)

设计器使用 Azure 机器学习[工作区](concept-workspace.md)来组织共享资源，例如：
...
设计器提供了用于生成、测试和部署机器学习模型的可视画布。 使用设计器，可以：
...
+ 将管道[发布](#publish)到 REST 管道终结点，以使用不同的参数和数据集提交新的管道运行。
    + 发布训练管道，在更改参数和数据集时重复使用单个管道来训练多个模型。
    + 发布批量推理管道，通过使用以前训练的模型对新数据进行预测。
+ 将实时推理管道[部署](#deploy)到实时终结点，以对新数据进行实时预测。

![设计器中的训练、批量推理和实时推理的工作流关系图](./media/concept-designer/designer-workflow-diagram.png)

## <a name="pipeline"></a>管道

[管道](concept-azure-machine-learning-architecture.md#ml-pipelines)包含连接在一起的数据集和分析模块。 管道有许多用途：可以创建训练单个模型的管道，或训练多个模型的管道。 可以创建实时或批量进行预测的管道，或者创建仅清理数据的管道。 借助管道，可以重复使用你的工作并组织你的项目。

### <a name="pipeline-draft"></a>管道草稿

在设计器中编辑管道时，进度将另存为“管道草稿”。 可以通过添加或删除模块、配置计算目标、创建参数等，随时编辑管道草稿。
...
每次运行管道时，管道及其结果的配置都会作为管道运行存储在工作区中。 可以返回到任何管道运行，以出于故障排除或审核目的进行检查。 克隆管道运行，以创建供你编辑的新管道草稿。

管道运行分组到试验中，以组织运行历史记录[](concept-azure-machine-learning-architecture.md#experiments)。 可以设置每个管道运行的试验。 

## <a name="datasets"></a>数据集

使用机器学习数据集可以轻松地访问和处理数据。 此设计器中包含多个示例数据集供你进行试验。 可以在需要时[注册](how-to-create-register-datasets.md)多个数据集。
...
使用工作区中的计算资源来运行管道，并将已部署的模型作为实时终结点或管道终结点进行托管（用于批量推理）。 支持的计算目标为：
...
若要执行实时推理，必须将管道部署为实时终结点。 实时终结点在外部应用程序和评分模型之间创建接口。 对实时终结点的调用会将预测结果实时返回到应用程序。 若要调用实时终结点，需要传递在部署终结点时创建的 API 密钥。 终结点基于 REST，这是一种流行的 Web 编程项目的体系结构。
...
还可以将管道发布到管道终结点。 与实时终结点类似，管道终结点允许你使用 REST 调用从外部应用程序提交新的管道运行。 但是，无法使用管道终结点实时发送或接收数据。

已发布的管道是灵活的，它们可用于训练或重新训练模型、[执行批量推理](how-to-run-batch-predictions-designer.md)、处理新数据等。 可以将多个管道发布到单个管道终结点，并指定要运行的管道版本。
...
可视界面（预览版）已更新，现在为 Azure 机器学习设计器（预览版）。 设计器已重建为使用基于管道的后端，该后端与 Azure 机器学习的其他功能完全集成。 

由于这些更新，可视界面的一些概念和术语已更改或重命名。 有关最重要的概念更改，请参阅下表。 
...
可以在设计器中将现有的可视界面试验和 Web 服务转换为管道和实时终结点。 使用以下步骤来迁移可视界面资产：
