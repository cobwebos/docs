...
2. 它会删除明显不需要的模块。 在本示例中，这包括[拆分数据][split]、第二个[评分模型][score-model]和[评估模型][evaluate-model]模块。
...
* **分数** -在此示例中，拆分模块用于将数据流划分为测试数据集和定型数据集。 预测性实验不需要此模型，可将其删除。 同样，第二个[评分模型][score-model]模块和[评估模型][evaluate-model]模块用于比较测试数据的结果，因此预测性实验也不需要这些模块。 然而，其余[评分模型][score-model]模块，就需要通过 Web 服务返回分数结果。
