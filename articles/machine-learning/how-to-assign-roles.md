...
| 角色 | 访问级别 |
| --- | --- |
| **读者** | 工作区中的只读操作。 读者可以在工作区中列出和查看资产（包括[数据存储](how-to-access-data.md)凭据），但不能创建或更新这些资产。 |
...
> [!NOTE]
> Azure Active Directory B2B，"az ml 工作区共享" 命令对联合帐户不起作用。 请使用 Azure UI 门户而不是命令。


## <a name="azure-machine-learning-operations"></a>Azure 机器学习操作

为许多操作和任务 Azure 机器学习内置操作。 有关完整列表，请参阅[Azure 资源提供程序操作](/azure/role-based-access-control/resource-provider-operations#microsoftmachinelearningservices)。
...
在 Azure CLI 中运行以下命令。 的 `<role-name>` 格式应与上述命令返回的格式相同。
