...
description: 了解组成 Azure 机器学习的体系结构、术语、概念和工作流。
...
![Azure 机器学习的体系结构和工作流](./media/concept-azure-machine-learning-architecture/workflow.png)
...
1. **训练**
    + 使用 Python 或 R 语言或者使用可视化设计器开发机器学习训练脚本 。
    + 创建和配置**计算目标**。
    + 将脚本提交到配置的计算目标以在该环境中运行。 在训练期间，脚本可以读取或写入数据存储。 系统将训练期间生成的日志和输出保存为工作区中的运行，分组到“试验”下  。

1. **包** - 找到满意的运行后，在模型注册表中注册持久化模型。

1. **验证** - 查询试验以了解当前和过去的运行中已记录的指标。 如果指标未指示所需结果，请循环回到步骤 1 并循环访问脚本。

1. **部署** - 开发一个使用该模型的评分脚本，并将该模型部署为 Azure 中的 Web 服务或部署到 IoT Edge 设备  。

1. **监视** - 监视已部署模型的训练数据集和推理数据之间的数据偏移。 必要时，请返回步骤 1，使用新的训练数据重新训练模型。

## <a name="tools-for-azure-machine-learning"></a>Azure 机器学习工具

为 Azure 机器学习使用以下工具：

+  使用[适用于 Python 的 Azure 机器学习 SDK](https://docs.microsoft.com/python/api/overview/azure/ml/intro?view=azure-ml-py) 与任何 Python 环境中的服务进行交互。
+ 使用[适用于 R 的 Azure 机器学习 SDK](https://azure.github.io/azureml-sdk-for-r/reference/index.html) 与任何 R 环境中的服务进行交互。
+ 使用 [Azure 机器学习 CLI](https://docs.microsoft.com/azure/machine-learning/reference-azure-machine-learning-cli) 自动执行机器学习活动。
+ 使用 [Azure 机器学习设计器（预览版）](concept-designer.md)在不编写代码的情况下执行工作流步骤。
+ [多模型解决方案加速器](https://aka.ms/many-models)（预览版）以 Azure 机器学习为基础，能用于训练、操作和管理数百甚至数千个机器学习模型。
...
[工作区](concept-workspace.md)是 Azure 机器学习的顶级资源。 它提供了一个集中的位置，用于处理使用 Azure 机器学习时创建的所有项目。 你可以和他人共享一个工作区。 有关工作区的详细说明，请参阅[什么是 Azure 机器学习工作区](concept-workspace.md)。
...
一次运行就是执行一次训练脚本。 一个试验通常包含多次运行。

Azure 机器学习记录所有运行，并在试验中存储以下信息：
...
如果你启动训练运行（其中源目录为本地 Git 存储库），存储库的相关信息存储在运行历史记录中。 这适用于使用估算器、ML 管道或脚本运行提交的运行。 还适用于从 SDK 或机器学习 CLI 提交的运行。
...
使用机器学习管道可以创建和管理将各个机器学习阶段整合到一起的工作流。 例如，一个管道可能包括数据准备、模型训练、模型部署以及推理/评分多个阶段。 每个阶段可以包含多个步骤，每个步骤都能够以无人参与方式在各种计算目标中运行。 

管道步骤可重复使用，如果这些步骤的输出未发生更改，可以运行它们，而无需重新运行前面的步骤。 例如，如果数据未更改，你可以重新训练模型，不需要重新运行成本高昂的数据准备步骤。 管道还使数据科学家能在处理机器学习工作流的不同区域时协作。
...
模型通过 Azure 机器学习中的运行生成。 还可以使用在 Azure 机器学习外部训练的模型。 你可以在 Azure 机器学习工作区中注册模型。

Azure 机器学习与框架无关。 创建模型时，可以使用任何常用的机器学习框架，例如 Scikit-learn、XGBoost、PyTorch、TensorFlow 和 Chainer。
...
> [!TIP]
> 注册的模型是组成模型的一个或多个文件的逻辑容器。 例如，如果你有一个存储在多个文件中的模型，可以在 Azure 机器学习工作区中将这些文件作为一个模型注册。 注册后，下载或部署注册的模型就能收到注册的所有文件。
...
我们使用 Azure ML 环境来指定用于为数据准备、模型训练和模型服务创建可重现的环境的配置（Docker、Python、Spark 等）。 这些环境是 Azure 机器学习工作区中带版本的托管实体，用于跨不同的计算目标实现可重现、可审核且可移植的机器学习工作流。

你可以对本地计算使用环境对象，从而开发训练脚本，对 Azure 机器学习计算重复使用同一环境进行大规模的模型训练，甚至使用相同的环境部署模型。 
...
为了便于通过常用的框架训练模型，使用估算器类能轻松地构造运行配置。 可以创建一个泛型[估算器](https://docs.microsoft.com/python/api/azureml-train-core/azureml.train.estimator?view=azure-ml-py)，并通过该估算器提交使用你选择的任何学习框架（例如 scikit-learn）的训练脚本。
...
* [使用 Azure 机器学习大规模训练和注册 TensorFlow 模型](how-to-train-tensorflow.md)。
* [使用 Azure 机器学习大规模训练和注册 Chainer 模型](how-to-train-ml-models.md)。

### <a name="endpoints"></a>终结点

终结点是将模型实例化为可在云中托管的 Web 服务或用于集成设备部署的 IoT 模块。

#### <a name="web-service-endpoint"></a>Web 服务终结点

将模型部署为 Web 服务时，可以在 Azure 容器实例、Azure Kubernetes 服务或 FPGA 上部署终结点。 你从模型、脚本和关联的文件创建该服务。 将它们放置到包含模型的执行环境的基础容器映像中。 映像具有负载均衡的 HTTP 终结点，可接收发送到 Web 服务的评分请求。

如果已选择启用此功能，Azure 通过收集 Application Insights 遥测数据或模型遥测数据帮助监视 Web 服务。 遥测数据仅供你访问，并且存储在 Application Insights 和存储帐户实例中。
...
部署的 IoT 模块终结点是包括模型、关联脚本或应用程序以及任何其他依赖项的 Docker 容器。 在边缘设备上使用 Azure IoT Edge 部署这些模块。
...
Azure 机器学习计算实例（以前称为 Notebook VM）是一种完全托管的基于云的工作站，其中包括为机器学习安装的多个工具和环境。 计算实例可用作训练和推理作业的计算目标。 对于大型任务，具有多节点缩放功能的 [Azure 机器学习计算群集](how-to-set-up-training-targets.md#amlcompute)是更好的计算目标选择。

详细了解[计算实例](concept-compute-instance.md)。

### <a name="datasets-and-datastores"></a>数据集和数据存储

使用 Azure 机器学习数据集（预览版）可以更轻松地访问和处理数据。 数据集管理各种方案（例如模型的训练和管道的创建）中的数据。 使用 Azure 机器学习 SDK，你可以访问底层存储、浏览数据以及管理不同数据集定义的生命周期。

数据集提供多种方法来处理采用常用格式的数据，例如使用 `from_delimited_files()` 或 `to_pandas_dataframe()`。

有关详细信息，请参阅[创建和注册 Azure 机器学习数据集](how-to-create-register-datasets.md)。  更多使用数据集的示例，请参阅[示例笔记本](https://github.com/Azure/MachineLearningNotebooks/tree/master/how-to-use-azureml/work-with-data/datasets-tutorial)。

数据存储是通过 Azure 存储帐户进行的存储抽象。 数据存储可以使用 Azure blob 容器或 Azure 文件共享作为后端存储。 每个工作区都有默认数据存储，并且你可以注册其他数据存储。 使用 Python SDK API 或 Azure 机器学习 CLI 可从数据存储中存储和检索文件。

### <a name="compute-targets"></a>计算目标

使用[计算目标](concept-compute-target.md)可指定运行训练脚本或托管服务部署的计算资源。 这个位置可以是本地计算机或基于云的计算资源。
