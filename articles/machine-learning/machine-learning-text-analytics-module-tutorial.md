...
你可以使用 Azure 机器学习构建和实现文本分析模型的操作化。 这些模型可以帮助你解决诸如文档分类或情绪分析的问题。
...
在本教程中，当我们使用“Amazon 书评数据集”逐步解说情绪分析模型时，你可以学到这些步骤（请参阅研究报告 ”Biographies, Bollywood, Boom-boxes and Blenders: Domain Adaptation for Sentiment Classification”，作者：Association of Computational Linguistics (ACL) 的 John Blitzer、Mark Dredze 和 Fernando Pereira，2007 年）。此数据集是由评论分数（1-2 或 4-5）和自由格式文字组成。 目标是预测评论分数：低 (1-2) 或高 (4-5)。

你可以在 Cortana Intelligence 库中找到本教程中涵盖的实验：
...
![创建标签](./media/machine-learning-text-analytics-module-tutorial/create-label.png)

然后，我们使用[预处理文本](https://msdn.microsoft.com/library/azure/mt762915.aspx)模块清理文本。 清除操作可减少数据集中的干扰信息，帮助你找到最重要的功能，并提高最终模型的准确性。 我们会删除非索引字，如“the”或“a”等常见字词，以及数字、特殊字符、重复的字符、电子邮件地址和 URL。 此外，我们还将文本转换为小写、按词形进行归并并检测句子边界，然后在预处理文本中用“|||”符号表示。

![预处理文本](./media/machine-learning-text-analytics-module-tutorial/preprocess-text.png)

如果要使用非索引字的自定义列表怎样操作？ 你可以将其作为可选输入传递。 你还可以使用自定义 C# 语法正则表达式来替换子字符串，并按词性（名词、动词或形容词）删除字词。

预处理完毕后，我们将数据拆分为定型和测试集。

## <a name="step-2-extract-numeric-feature-vectors-from-pre-processed-text"></a>步骤2：从预处理文本中提取数值特征向量
若要生成文本数据模型，通常需要将自由格式文本转换为数字特征向量。 在此示例中，我们使用[从文本提取 N 元语法功能](https://msdn.microsoft.com/library/azure/mt762916.aspx)模块，将文本数据转换为此类格式。 此模块使用一列以空格分隔的字词，并计算出现在数据集中的字典的字词，或 N 元语法的字词。 然后，它将计算每条记录中每个字词或 N 元语法出现的次数，并从这些计数创建特征向量。 在本教程中，我们将 N 元语法大小设置为 2，这样我们的特征向量可包含单个字词和两个后续字词的组合。

![提取 N 元语法](./media/machine-learning-text-analytics-module-tutorial/extract-ngrams.png)

我们会将 TF*IDF（词频逆文档频率）权重应用于 N 元语法计数。 此方法将添加单个记录中频繁出现的字词的权重，但极少针对整个数据集执行此操作。 其他选项包括二进制、TF 和图形权重。

此类文本功能通常具有较高的维度。 例如，假设语料库具有 100,000 个唯一字词，如果使用 N 元语法，则特征空间具有 100,000 维度或更多。 “提取 N 元语法特征”模块提供了一组用于减少维度的选项。 你可以选择排除过短或过长的字词，或不太常见或太频繁而具有重要预测值的字词。 在本教程中，我们会排除出现在少于 5 个记录或超过 80% 的记录的 N 元语法。
...
然后，我们使用[二元逻辑回归](https://msdn.microsoft.com/library/azure/dn905994.aspx)来预测我们的目标：高或低评论分数。 在这种情况下，文本分析问题已转换为常规分类问题。 你可以使用 Azure 机器学习中提供的工具来改进模型。 例如，你可以实验不同的分类器，以了解如何它们给出结果的准确度，或使用超参数调整改进准确度。

![定型和评分](./media/machine-learning-text-analytics-module-tutorial/scoring-text.png)
...
若要设置预测实验，我们先将 N 元语法词汇保存为数据集，并使用来自实验定型分支的定型逻辑回归模型。 然后，我们使用“另存为”来保存实验，以创建预测实验的实验图。 我们从实验中删除“拆分数据”模块和定型分支。 然后，我们将上一阶段保存的 N 元语法词汇和模型分别连接到“提取 N 元语法特征”和“评分模型”模块。 此外，我们还删除“评估模型”模块。

我们将“选择数据集中的列”模块插入到“预处理文本”模块前面，以删除标签列，并取消选择“评分模块”中的“将评分列附加到数据集”选项。 这样一来，Web 服务不会请求正在尝试进行预测的标签，也不对响应中的输入特征作出回应。

![预测性实验](./media/machine-learning-text-analytics-module-tutorial/predictive-text.png)
