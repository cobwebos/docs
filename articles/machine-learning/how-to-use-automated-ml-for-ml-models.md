...
自动应用数据保护程序，以帮助您识别数据的潜在问题（例如缺少值、类不平衡），并帮助采取纠正措施以改进结果。 有许多最佳实践可用，可以应用，以实现可靠的结果。 

下表描述了当前支持的数据守护轨，以及用户在提交实验时可能会遇到的关联状态。

护栏|状态|触发&nbsp;条件&nbsp;
---|---|---
缺少&nbsp;归&nbsp;因值 |**Passed** <br> <br> “固定”****|    任何输入&nbsp;列中没有缺失值 <br> <br> 某些列缺少值
交叉验证|**完成**|如果未提供显式验证集
高&nbsp;基数&nbsp;特征&nbsp;检测|    **Passed** <br> <br>**完成**|    未检测到高基数特征 <br><br> 检测到高基数输入列
类余额检测    |**Passed** <br><br><br>**提醒** |课程在培训数据中是平衡的;如果每个类在数据集中具有良好的表示形式，则数据集被视为平衡的，按样本的数量和比率衡量 <br> <br> 培训数据中的课程不平衡
时间序列数据一致性|**Passed** <br><br><br><br> “固定”**** |<br> 分析了选定的[视界、滞后、滚动窗口]值，未检测到潜在的内存不足问题。 <br> <br>已分析选定的[视界、延迟、滚动窗口]值，并可能导致实验内存不足。 延迟或滚动窗口已关闭。
