---
title: 使用 autoML & 部署创建模型
titleSuffix: Azure Machine Learning
description: 通过 Azure 机器学习创建、查看和部署自动化机器学习模型。
...
本文介绍了如何创建、浏览和部署自动化机器学习模型，而无需在 Azure 机器学习的 studio 界面中编写单行代码。 自动机器学习是为您选择要用于特定数据的最佳机器学习算法的一个过程。 此过程使你能够快速生成机器学习模型。 [详细了解自动化机器学习](concept-automated-ml.md)。
 
有关端到端示例，请尝试[使用 Azure 机器学习的自动 ML 接口创建分类模型的教程](tutorial-first-experiment-automated-ml.md)。 

对于基于 Python 代码的体验，请使用 Azure 机器学习 SDK[配置自动化机器学习试验](how-to-configure-auto-train.md)。
...
1. 登录到 https://ml.azure.comAzure 机器学习。 
