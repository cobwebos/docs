...
> [!NOTE]
> 自动机器学习特征化（例如功能规范化、处理丢失的数据或将文本转换为数字）的步骤成为基础模型的一部分。 使用模型进行预测时，在训练过程中应用的相同特征化步骤会自动应用于输入数据。
...
> [!NOTE]
> 如果计划将 AutoML 创建的模型导出到[ONNX 模型](concept-onnx.md)，则仅支持用星号（"*"）指示的特征化选项。 [详细了解如何将模型转换为 ONNX](concept-automated-ml.md#use-with-onnx)。

|特征化步骤| 说明 |
| ------------- | ------------- |
|删除高基数或者无差异的特征* |从定型集和验证集删除这些功能。 适用于所有缺少值的功能，这些值在所有行中都具有相同的值，或具有较高的基数（例如，哈希、Id 或 Guid）。|
...
|群集距离|在所有数值列上训练 k 平均值聚类分析模型。 为每个分类的质心生成*k*个新功能（每个群集一个新的数字功能），其中包含每个样本的距离。|

## <a name="data-guardrails"></a>数据护栏

*数据 guardrails*有助于识别数据的潜在问题（例如，缺失值或[类不平衡](concept-manage-ml-pitfalls.md#identify-models-with-imbalanced-data)）。 它们还可帮助你采取纠正措施来提高结果。
...
高基数特征处理 |Passed <br><br><br> 完成| 已分析你的输入，但未检测到高基数功能。 了解有关[高基数功能检测](#automatic-featurization)的详细信息。 <br><br> 在输入中检测到了高基数功能，已对其进行处理。
...
内存问题检测 |Passed <br><br><br><br> 完成 |<br> 已分析选定值（范围、延迟、滚动窗口），但未检测到任何可能的内存不足问题。 了解有关时序[预测配置](https://docs.microsoft.com/azure/machine-learning/how-to-auto-train-forecast#configure-and-run-experiment)的详细信息。 <br><br><br>已分析选定值（范围、延迟、滚动窗口），并可能会导致实验用尽内存。 延迟或滚动窗口配置已关闭。
...
|--|--|
|列用途更新|重写指定列的功能类型。|
|转换器参数更新 |更新指定转换器的参数。 目前支持*Imputer* （平均值、最常、中间值和*HashOneHotEncoder*）。|
|删除列 |指定要从特征化中删除的列。|
