...
若要将此 `CustomAddRows` 函数公开，作为 Azure 机器学习模块，必须创建 XML 定义文件来指定**自定义添加行**模块的外观和行为。 
...
若要在机器学习工作区中进行注册，请转到机器学习工作室中的工作区，单击底部的“+新建”按钮，然后选择“模块”->“从 ZIP 包”，上传新的**自定义添加行**模块。

![上传 Zip](./media/machine-learning-custom-r-modules/upload-from-zip-package.png)
...
**ColumnPicker**：列选择参数。 此类型在 UX 中显示为列选择器。 此处的**属性**元素用于指定端口 ID，将从此端口选择列，其中目标端口类型必须为 *DataTable*。 列选择结果将以字符串列表传送到 R 函数，其中包括选中列的名称。 
...
例如，你想从数据集中删除带有 NA 的行，在将其输出到 CustomAddRows 之前，你已将具有此功能的 R 函数写入了 RemoveDupNARows.R 文件：
