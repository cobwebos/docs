---
title: 在虚拟网络中启用 Azure 机器学习 studio
titleSuffix: Azure Machine Learning
description: 使用 Azure 机器学习 studio 来访问存储在虚拟网络中的数据。
...
> [!div class="checklist"]
> - 从虚拟网络内部的资源访问 studio。
> - 为工作室授予对存储在虚拟网络内的数据的访问权限。
> - 了解存储安全受工作室的影响。

本文是5部分系列中的第五部分，指导你完成保护 Azure 机器学习工作流的工作。 强烈建议您通读第 [一部分： VNet 概述](how-to-network-security-overview.md) 以首先了解总体体系结构。 
...
+ [已启用专用链接的现有 Azure 机器学习工作区](how-to-secure-workspace-vnet.md#secure-the-workspace-with-private-endpoint)。

+ 现有的 [Azure 存储帐户已添加虚拟网络](how-to-secure-workspace-vnet.md#secure-azure-storage-accounts)。

## <a name="access-the-studio-from-a-resource-inside-the-vnet"></a>从 VNet 中的资源访问工作室
...
1. 若要创建新的数据存储，请选择“+ 新建数据存储”。
    ...
    若要更新现有的数据存储，请选择数据存储，然后选择 " __更新凭据__"。

1. 在 "数据存储" 设置中，选择 __"是"__ 以  __允许 Azure 机器学习服务使用工作区管理的标识来访问存储__。


这些步骤使用基于 Azure 资源的访问控制 (RBAC) 将工作区管理的标识作为 __读取器__ 添加到存储服务。 “读取者”访问权限允许工作区检索防火墙设置，并确保数据不会离开虚拟网络。
...
使用托管标识访问存储服务会影响某些安全注意事项。 这些注意事项对你要访问的存储帐户类型是唯一的。 本部分介绍每种存储帐户类型的更改。

### <a name="azure-blob-storage"></a>Azure Blob 存储

对于 __Azure Blob 存储__，还会将工作区托管标识作为 [Blob 数据读取器](../role-based-access-control/built-in-roles.md#storage-blob-data-reader) 添加，以便它能够从 blob 存储中读取数据。
...
若要使用 RBAC，请将工作区管理的标识添加到 [Blob 数据读取器](../role-based-access-control/built-in-roles.md#storage-blob-data-reader) 角色。 有关详细信息，请参阅[基于角色的访问控制](../storage/blobs/data-lake-storage-access-control.md#role-based-access-control)。

若要使用 Acl，可以向工作区管理的标识分配访问权限，就像其他任何安全原则一样。 有关详细信息，请参阅[文件和目录上的访问控制列表](../storage/blobs/data-lake-storage-access-control.md#access-control-lists-on-files-and-directories)。
...
本文是由四部分构成的虚拟网络系列的可选部分。 若要了解如何保护虚拟网络，请参阅其余文章：
