...
此快速入门教程有助于你使用 R 编程语言快速开始扩展 Azure 机器学习的功能。 按照此 R 编程教程在 Azure 机器学习中创建、测试和执行 R 代码。 当你完成本教程时，将在 Azure 机器学习中使用 R 语言创建完整的预测解决方案。  
...
可以在[此处下载][下载]本文使用的数据和 R 脚本。 此数据最初是从威斯康星州大学提供的信息中合成的，网址为 http://future.aae.wisc.edu/tab/production.html。

### <a name="organization"></a>组织
我们将通过几个步骤来了解如何在 Azure 机器学习环境中创建、测试并执行分析和数据操作 R 代码。  

* 首先我们探讨在 Azure 机器学习工作室环境中使用 R 语言的基础知识。
* 然后我们讨论在 Azure 机器学习环境中数据、R 代码和图形的 I/O 的各个方面。
...
参考图 1，让我们看一些使用[执行 R 脚本] [execute-r-script] 模块的机器学习工作室环境的主要部分。

* 用于试验的模块在中间窗格中显示。
* 右窗格的上半部分包含一个窗口可查看和编辑 R 脚本。  
* 右窗格的下半部分显示[执行 R 脚本] [execute-r-script] 的某些属性。 可以通过单击此窗格中的适当位置查看错误和输出日志。

当然，我们会在本文的其余部分更详细地讨论[执行 R 脚本] [execute-r-script]。

使用复杂的 R 函数时，建议在 RStudio 中编辑、测试和调试。 与任何软件开发一样，逐步扩展你的代码，并在小型简单的测试用例上测试代码。 然后将函数剪切并粘贴到[执行 R 脚本] [execute-r-script] 模块的 R 脚本窗口中。 此方法允许你利用 RStudio 集成开发环境 (IDE) 和 Azure 机器学习的强大功能。  

#### <a name="execute-r-code"></a>执行 R 代码
单击“运行”按钮运行此试验时，将执行[执行 R 脚本] [execute-r-script] 模块中的任何 R 代码。 执行完成后，在[执行 R 脚本] [execute-r-script] 图标上将显示一个对勾。

#### <a name="defensive-r-coding-for-azure-machine-learning"></a>适用于 Azure 机器学习的防御性 R 编码
如果使用 Azure 机器学习为 Web 服务开发 R 代码，你一定会对代码如何处理意外的数据输入和异常进行规划。 为了保持代码的清晰度，在显示的大部分代码示例中未包含太多检查或异常处理方面的代码。 但是，后面我会提供几个使用 R 的异常处理功能的函数示例。  

如果需要 R 异常处理的更全面的内容，我建议你阅读[附录 B — 深入阅读](#appendixb)中列出的由 Wickham 创作的书籍中的相关部分。
...
若要检查 R 中的任何对象的值，可以将这些值打印到 output.log 文件。 检查对象的值的规则本质上与交互式 R 会话中的相同。 例如，如果在行上键入一个变量名，则该对象的值将打印到 output.log 文件。  
...
如果你此时不了解此代码的最后一行，请继续阅读。 本文剩余部分将全面讨论在 Azure 机器学习环境中使用 R。
...
如果你是 RStudio 的新手，应下载并安装桌面版。 可以在 http://www.rstudio.com/products/RStudio/ 处找到适用于 Windows、Mac OS 和 Linux/UNIX 的 RStudio 下载。 按照提供的说明在台式计算机上安装 RStudio。  
...
* 启动 Azure 机器学习工作室环境。
* 单击屏幕左下方的“+ 新建”，并选择“数据集”。
* 选择“从本地文件”，然后单击“浏览”以选择文件。
* 请确保选择了**具有标题 (.csv) 的通用 CSV 文件**作为数据集的类型。
* 单击复选标记。
* 上载数据集后，可单击“数据集”选项卡查看新数据集。  

#### <a name="create-an-experiment"></a>创建试验
现在，机器学习工作室中已有一些数据，我们需要创建一个试验来执行分析。  

* 单击左下方的“+ 新建”，然后依次选择“实验”和“空白实验”。
* 可以命名试验，方法是选择和修改页面顶部的“在...创建的试验”标题。 例如，将其改为“CA 乳品分析”。
* 在试验页面的左侧，展开“保存的数据集”，然后选择“我的数据集”。 可看到之前上载的 **cadairydata.csv**。
...
让我们看看已加载到试验中的数据。 在该试验中，单击 **cadairydata.csv 数据集**的输出，然后选择“可视化”。 可看到类似图 4 的内容。  

![cadairydata.csv 数据集摘要][4]

*图 4.cadairydata.csv 数据集摘要*

在此视图中可看到大量有用的信息。 可以看到该数据集的前几个行。 如果选择一列，“统计信息”部分显示有关该列的详细信息。 例如，“特征类型”行显示了 Azure 机器学习工作室分配给列的数据类型。 在开始执行任何重要的工作之前，像这样快速浏览数据是一种很好的数据完好性检查方式。

### <a name="first-r-script"></a>第一个 R 脚本
让我们在 Azure 机器学习工作室中创建第一个要测试的简单的 R 脚本。 我已在 RStudio 中创建并测试了以下脚本。  
...
现在，需要将此脚本移到 Azure 机器学习工作室。 我只需要剪切并粘贴。 但是，在该例中，我通过 zip 文件传输 R 脚本。
...
[执行 R 脚本] [execute-r-script] 模块可能有三个输入。 可以使用任何一个或所有输入，具体取决于你的应用程序。 使用没有任何输入的 R 脚本也是非常合理的。  
...
> [!NOTE]
> Azure 机器学习认为 zip 中的文件位于 src / 目录中，因此文件名需要有目录名称前缀。 例如，如果 zip 包含位于该 zip 根目录下的文件 `yourfile.R` 和 `yourData.rdata`，那么在使用 `source` 和 `load` 时将这两个文件处理为 `src/yourfile.R` 和 `src/yourData.rdata`。
> 
> 
...
1. 将 R 脚本保存到 R 文件。 调用脚本文件“simpleplot.R”。 内容如下：
        ...
        ## Azure Machine Learning Studio
        maml.mapOutputPort('cadairydata')
2. 创建一个 zip 文件，并将脚本复制到此 zip 文件。 在 Windows 上，右键单击该文件并选择“发送到”，然后选择“压缩文件夹”。 此时将创建包含"simpleplot.R"文件的新的 zip 文件。
3. 将文件添加到机器学习工作室中的**数据集**，并指定类型为 **zip**。 现在可在数据集中看到此 zip 文件。
4. 将 zip 文件从**数据集**拖放到**机器学习工作室画布**。
5. 将 **zip 数据**输出图标连接到[执行 R 脚本] [execute-r-script] 模块的**脚本包**输入。
6. 在[执行 R 脚本] [execute-r-script] 模块的代码窗口中键入具有 zip 文件名的 `source()` 函数。 在本例中我键入 `source("src/simpleplot.R")`。  
7. 请确保单击“保存”。
...
单击“运行”按钮执行试验。 执行完成后，单击“执行 R 脚本 [execute-r-script]”模块，然后单击“属性”窗格上的“查看 output.log”。[] 浏览器中打开新的页面，其中显示 output.log 文件的内容。 向下滚动页面时会看到类似下面的内容：
...
运行试验之后，单击结果 Dataset1 输出端口，然后单击“可视化”。 可看到类似图 6 的内容。
...
若要查看 R 设备输出，请单击该端口，然后单击“可视化”。 在图 7 中可看到 R 脚本的标准输出和标准错误。
...
R 是一个具有动态类型的语言，这表示可以根据需要将数据类型从一种类型强制转换为另一种类型。 R 的基本数据类型包括数字、逻辑和字符。 因子类型用于以简洁方式存储分类数据。 可以在[附录 B — 深入阅读](#appendixb)的参考文献中找到有关数据类型的更多信息。

将外部源中的表格数据读入 R 后，最好检查列的结果类型。 你可能想要字符类型的列，但在许多情况下则显示为因子，反之亦然。 例如，在另一些情况下，你认为应是数值的列却表示为字符数据。如字符“1.23”，而不是浮点数字 1.23。  
...
我删除了创建散点图矩阵的行，并添加了将“Month”列转换为因子的行。 在试验中我只需将 R 代码剪切并粘贴到[执行 R 脚本] [execute-r-script] 模块的代码窗口中。 你也可以更新 zip 文件，并将其上载到 Azure 机器学习工作室中，但这需要几个步骤。  
...
现在 Month 的类型显示为**具有 14 个级别的因子**。 由于一年只有 12 个月，因此这是一个问题。 你还可以查看该类型在结果数据集端口的“可视化”中为“分类”。
...
R 数据框支持强大的筛选功能。 对行或列使用逻辑筛选器可以将数据集划分为多个子集。 在许多情况下，都需要复杂的筛选条件。 [附录 B — 深入阅读](#appendixb)中的参考文献包含筛选数据框的许多示例。  

我们要对数据集进行一点筛选。 如果看一下 cadairydata 数据框中的列，会发现两个不必要的列。 第一列只包含一行数字，不是很有用。 第二列 Year.Month 包含冗余的信息。 使用以下 R 代码可轻松地排除这两列。

> [!NOTE]
> 本节从现在开始，我只展示在[执行 R 脚本] [execute-r-script] 模块中添加的附加代码。 我将在 `str()` 函数**前面**添加一个新行。 我使用此函数验证 Azure 机器学习工作室中的结果。
> 
> 
...
本节我们将对数据框的一些列的值执行简单转换。 R 语言支持几乎所有值的转换。 [附录 B — 深入阅读](#appendixb)中的参考文献包含许多示例。
...
如果你不习惯于 R 中的防御性编程，那么所有这些代码可能看起来有点让人难以招架。 我向你讲解以下主要步骤：
...
如前所述，时序对象是一系列具有时间索引的数据值。 R 时序对象用于创建和管理时间索引。 使用时序对象有几个优点。 时序对象可以为你省去管理封装在对象中的时序索引值的许多细节。 此外，时序对象允许使用许多时序方法进行绘图、打印、建模等。
...
以下显示的 `ts.detrend()` 函数执行了这两个操作。 以下两行代码对数据进行去趋势处理，然后标准化值。
...
构建数据框后，我们需要创建训练数据集。 该数据包含所有观测值，2013 年的 12 个月的数据除外，2013 年数据是测试数据集。 以下代码将数据框划分为不同子集，并创建乳品生产和价格变量的图形。 然后，我创建了四个生产和价格变量的图形。 在 `Map()` 函数中使用匿名函数定义图形的一些参数，然后遍历其他两个参数的列表。 如果觉得此处适合使用 for 循环，那么你是对的。 但是，由于 R 是函数式语言，因此我将向你展示函数式方法。
...
![加利福尼亚州乳品生产和价格数据的第一个时序图](./media/machine-learning-r-quickstart/unnamed-chunk-161.png)

![加利福尼亚州乳品生产和价格数据的第二个时序图](./media/machine-learning-r-quickstart/unnamed-chunk-162.png)

![加利福尼亚州乳品生产和价格数据的第三个时序图](./media/machine-learning-r-quickstart/unnamed-chunk-163.png)

![加利福尼亚州乳品生产和价格数据的第四个时序图](./media/machine-learning-r-quickstart/unnamed-chunk-164.png)

*图 22.加利福尼亚州乳品生产和价格数据的时序图*
...
![显示趋势模型的加利福尼亚州牛奶生产数据](./media/machine-learning-r-quickstart/unnamed-chunk-18.png)

*图 23.显示趋势模型的加利福尼亚州牛奶生产数据*
...
![包含季节性效应的加利福尼亚州牛奶生产模型](./media/machine-learning-r-quickstart/unnamed-chunk-20.png)

*图 24.包含季节性效应的加利福尼亚州牛奶生产模型*

图 24 中显示的数据拟合相当鼓舞人心。 趋势和季节性效应（月度变体）看起来合理。

作为模型的另一项检查，让我们看看残差。 以下代码从两个模型中计算预测值、计算季节性模型的残差，然后绘制训练数据的残差图。
...
![训练数据的季节性模型的残差](./media/machine-learning-r-quickstart/unnamed-chunk-21.png)
...
![季节性模型的第一个诊断图](./media/machine-learning-r-quickstart/unnamed-chunk-221.png)

![季节性模型的第二个诊断图](./media/machine-learning-r-quickstart/unnamed-chunk-222.png)

![季节性模型的第三个诊断图](./media/machine-learning-r-quickstart/unnamed-chunk-223.png)

![季节性模型的第四个诊断图](./media/machine-learning-r-quickstart/unnamed-chunk-224.png)
...
和“值转换”一节中讨论的 `log.transform()` 函数一样，该函数也存在很多错误检查和异常恢复代码。 两者采用的原理是相同的。 在 `tryCatch()` 函数中包装的两个位置中完成操作。 首先，对时间系列取幂，因为我们在使用值的对数。 其次，计算实际的 RMS 误差。  

配备一个函数来测量 RMS 误差，然后生成并输出包含 RMS 误差的数据框。 我们将单独为趋势模型包括项和具有季节性因子的完整模型包括项。 以下代码通过使用构造的两个线性模型完成此作业。
...
1. 创建项目
   ...
   我建议你按照文档中的说明进行操作，并为文档中的 R 代码示例创建项目。  
2. 编辑和执行 R 代码
...
* 《Advanced R》（高级 R）：作者是 Hadley Wickham。如果你想要深入了解 R 主题，可以查阅本书。 本书的联机版本的免费获取网址为 http://adv-r.had.co.nz/。

可以在时序分析的 CRAN 任务视图中找到 R 时序包的目录，网址为 http://cran.r-project.org/web/views/TimeSeries.html。 有关特定时序对象包的信息，请参考此包的文档。

由 Paul Cowpertwait 和 Andrew Metcalfe 所著的《Introductory Time Series with R》（在时序分析中使用 R 简介）一书介绍了如何在时序分析中使用 R。 还有更多的理论性文字提供了 R 示例。
...
<!--links-->
[appendixa]: #appendixa
[下载]: https://azurebigdatatutorials.blob.core.windows.net/rquickstart/RFiles.zip
