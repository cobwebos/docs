...
本文概述了如何将本地 SQL Server 数据导入 Azure 机器学习。 假定你熟悉工作区、模块、数据集、实验*等* Azure 机器学习概念。
...
* 网关计算机的建议配置至少为 2 GHz、4 核、8 GB RAM 和 80 GB 磁盘。
* 如果主机计算机进入休眠状态，网关将无法响应数据请求。 因此，安装网关之前，请在计算机上配置相应的电源计划。 如果计算机配置为休眠，网关安装将显示一条消息。
...
* 网关不需要与数据源位于同一台计算机上，但网关离数据源较近可减少网关连接到数据源的时间。 建议在不同于托管本地数据源的计算机上安装网关，以便网关和数据源不会因资源而竞争。
* 如果已在计算机中安装了为 Power BI 或 Azure 数据工厂方案提供服务的网关，那么在其他计算机上安装用于 Azure 机器学习的单独网关。
  ...
  >
  >
* 即使对其他数据使用 Azure ExpressRoute，也需要为 Azure 机器学习使用数据管理网关。 即使使用 ExpressRoute 也应该将数据源视为本地数据源（位于防火墙之后），并使用数据管理网关在机器学习和数据源之间建立连接。

有关安装先决条件、安装步骤和故障排除提示的详细信息，请参阅[使用数据管理网关在本地资源和云之间移动数据](../data-factory/data-factory-move-data-between-onprem-and-cloud.md)一文。
...
4. 在“新建数据网关”对话框中，输入“网关名称”，还可添加“说明”。 单击右下角的箭头，即可转到配置的下一步。
...
8. 如果已安装了网关，请运行数据管理网关配置管理器、单击“更改密钥”、粘贴已复制到剪贴板的“网关注册密钥” ****，然后单击“确定”。
9. 安装完成后，将显示 Microsoft 数据管理网关配置管理器的“注册网关”对话框。 粘贴上述已复制到剪贴板的“网关注册密钥”，然后单击“注册”。
...
12. 在 Microsoft 数据管理网关配置管理器中，切换到“证书”选项卡。 此选项卡上指定的证书可用于加密/解密在门户上指定的本地数据存储的凭据。 这是生成的默认证书。 Microsoft 建议将此证书更改为在你的证书管理系统中备份的自己的证书。 单击“更改”以改为使用你自己的证书。
...
7. 单击“用户名和密码”下的“输入值”，然后输入你的数据库凭据。 可以使用 Windows 集成身份验证或 SQL Server 身份验证，具体取决于配置本地 SQL Server 的方式。
   ...
   消息“必填值”将更改为带有绿色核选标记的“值已设置”。 只需输入凭据一次，除非数据库信息或密码发生更改。 Azure 机器学习使用你在安装网关时提供的证书，来加密云中的凭据。 Azure 不会存储未加密的本地凭据。

   ![](media\\machine-learning-use-data-from-an-on-premises-sql-server\\import-data-properties-entered.png)
8. 若要运行实验，请单击“运行”。

实验运行结束后，通过单击“导入数据”模块的输出端口并选择“可视化”，即可可视化从数据库中导入的数据。
