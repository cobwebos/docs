...
+ **Bug 修复与改进**
  ...
  + **azureml-core**
    ...
    + 对于数据存储. register_azure_blob_container 和数据存储。 register_azure_file_share 仅 (支持 SAS 令牌) 的选项，我们已更新字段的文档字符串 `sas_token` 以包括典型的读取和写入方案的最低权限要求。
    + 弃用 _with_auth get_mlflow_tracking_uri ( # A1 中的参数
  + **azureml-mlflow**
