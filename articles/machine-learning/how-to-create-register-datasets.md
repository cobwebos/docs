...
ms.locfileid: "82231877"
---
# <a name="create-azure-machine-learning-datasets"></a>创建 Azure 机器学习数据集

[!INCLUDE [aml-applies-to-basic-enterprise-sku](../../includes/aml-applies-to-basic-enterprise-sku.md)]

本文介绍如何创建 Azure 机器学习数据集，以访问本地或远程试验的数据。 若要了解数据集适合 Azure 机器学习的总体数据访问工作流，请参阅[安全访问数据](concept-data.md#data-workflow)一文。
...
> [!NOTE]
> 某些数据集类依赖于 [azureml-dataprep](https://docs.microsoft.com/python/api/azureml-dataprep/?view=azure-ml-py) 包。 对于 Linux 用户，仅以下分发版支持这些类： Red Hat Enterprise Linux、Ubuntu、Fedora 和 CentOS。
...
如果你使用的是 Pandas，则没有理由使用超过1个 vCPU，因为这就是它将使用的全部。 通过 Modin 和 Dask/Ray，你可以轻松地在单个 Azure 机器学习计算实例/节点上对多个个 vcpu 进行并行化，只需将更改`import pandas as pd`为`import modin.pandas as pd`，即可向外扩展到大群集。 
 
如果无法为数据获得足够大的虚拟空间，则可以使用以下两个选项：使用 Spark 或 Dask 等框架对数据 "内存不足" 执行处理，即，数据帧按分区加载到 RAM 分区并进行处理，并最终收集最终结果。 如果这太慢，Spark 或 Dask 允许向外扩展到仍可使用交互的群集。 

## <a name="dataset-types"></a>数据集类型

根据用户在训练中使用数据集的方式，存在两种数据集类型：

* [TabularDataset](https://docs.microsoft.com/python/api/azureml-core/azureml.data.tabulardataset?view=azure-ml-py) 表示表格格式的、通过分析提供的文件或文件列表所创建的数据。 使用此类数据集可将数据具体化为 Pandas 或 Spark 数据帧。 可从 .csv、 `TabularDataset` tsv、. parquet、. jsonl 文件和 SQL 查询结果中创建对象。 有关完整列表，请参阅 [TabularDatasetFactory 类](https://aka.ms/tabulardataset-api-reference)。
...
创建数据集时，将会创建对数据源位置的引用及其元数据的副本。 由于数据保留在其现有位置，因此不会产生额外的存储成本。 你可以通过使用`TabularDataset` Python `FileDataset` SDK 或 Azure 机器学习 studio 中https://ml.azure.com的来创建和数据集。
...
下面的代码按名称获取工作区现有工作区和所需的数据存储区。 然后，将数据存储和文件位置传递给`path`参数，以创建新的 TabularDataset `weather_ds`。
...
默认情况下，在创建 TabularDataset 时，将自动推断列数据类型。 如果推断出的类型与预期不符，可以使用以下代码指定列类型。 此参数`infer_column_type`仅适用于从分隔文件创建的数据集。 还可以[详细了解支持的数据类型](https://docs.microsoft.com/python/api/azureml-core/azureml.data.dataset_factory.datatype?view=azure-ml-py)。

> [!IMPORTANT] 
> 如果存储位于虚拟网络或防火墙后面，则仅支持通过 SDK 创建数据集。 若要创建数据集，请确保在`validate=False` `infer_column_types=False` `from_delimited_files()`方法中包含参数和。 这会绕过初始验证检查，并确保可以从这些安全文件创建数据集。 
...
| |PassengerId|Survived|Pclass|“属性”|Sex|年龄|SibSp|Parch|Ticket|车费|Cabin|Embarked
-|-----------|--------|------|----|---|---|-----|-----|------|----|-----|--------|
...
使用`TabularDatasetFactory`类[`from_sql_query()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.dataset_factory.tabulardatasetfactory?view=azure-ml-py#from-sql-query-query--validate-true--set-column-types-none--query-timeout-30-)中的方法从 Azure SQL 数据库读取数据：
...
使用`TabularDataset`类[`with_timestamp_columns()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.tabulardataset?view=azure-ml-py#with-timestamp-columns-timestamp-none--partition-timestamp-none--validate-false----kwargs-)的方法可以指定时间戳列并按时间启用筛选。 有关详细信息，请参阅[使用 NOAA 天气数据的表格时序相关 API 演示](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/work-with-data/datasets-tutorial/timeseries-datasets/tabular-timeseries-dataset-filtering.ipynb)。
...
使用`FileDatasetFactory`类[`from_files()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.dataset_factory.filedatasetfactory?view=azure-ml-py#from-files-path--validate-true-)的方法可加载任意格式的文件，并可创建未注册的 FileDataset。 如果存储位于虚拟网络或防火墙后面，请在`validate =False` `from_files()`方法中设置参数。 这会绕过初始验证步骤，并确保可以从这些安全文件创建数据集。
...
若要完成创建过程，请将数据集注册到工作区。 使用[`register()`](https://docs.microsoft.com/python/api/azureml-core/azureml.data.abstract_dataset.abstractdataset?view=azure-ml-py#register-workspace--name--description-none--tags-none--create-new-version-false-)方法将数据集注册到你的工作区，以便与其他人共享数据集，并在工作区中的试验间重复使用这些数据集：
...
可以检索 `TabularDataset` 或 `FileDataset` 形式的某些类，这样就可以直接操控和/或下载文件。 其他类只能通过使用`get_tabular_dataset()`或`get_file_dataset()`函数**之一来获取数据集**。 下面的代码示例演示了这些类类型的一些示例。
...
选择数据集对应的磁贴将其选中。 （您可以选择使用搜索栏进行筛选。）选择 "**下一步**"。
...
可以在本地以及在 Azure 机器学习计算等计算群集上远程访问已注册的数据集。 若要跨试验访问已注册的数据集，请使用以下代码按名称访问工作区和已注册的数据集。 默认情况下， [`get_by_name()`](https://docs.microsoft.com/python/api/azureml-core/azureml.core.dataset.dataset?view=azure-ml-py#get-by-name-workspace--name--version--latest--) `Dataset`类上的方法返回注册到工作区中的最新版本的数据集。
