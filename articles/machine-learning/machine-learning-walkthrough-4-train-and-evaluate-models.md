...
- - -
使用 Azure 机器学习工作室创建机器学习模型的优势之一是，能够在单个实验中一次性尝试多种模型并比较结果。 此类型的实验有助于找到最适合解决你的问题的解决方案。

在我们在此演练中开发的实验中，将创建两个不同类型的模型，然后比较其评分结果，从而确定我们希望用于最终实验的算法。  

可从多种模型中进行选择。 若要查看可用的模型，请在模块面板中展开“机器学习”节点，然后展开“初始化模型”及其下面的节点。 为实现本实验的目的，我们将选择[两类支持向量机][two-class-support-vector-machine] (SVM) 和[两类提升决策树][two-class-boosted-decision-tree]模块。    
...
3. 将左侧[执行 R 脚本][execute-r-script]模块的左侧输出连接到[训练模型][train-model]模块的右侧输入端口（我们在此演练的[第 3 步](machine-learning-walkthrough-3-create-new-experiment.md)中已决定使用来自“拆分数据”模块左侧的数据进行培训）。
...
2. 在“选择单个列”对话框中，在“可用列”下的搜索字段中键入“信用风险”，在下方选择“信用风险”，然后单击向右箭头按钮 (**>**) 将“信用风险”移动到“选定列”。 
...
2. 右键单击[训练模型][train-model]模块，选择“复制”，然后右键单击画布并选择“粘贴”。 [训练模型][train-model]模块的副本具有与原始模块相同的列选择。
...
2. 单击“启动列选择器”、为“开始”选择“没有列”、在第一个下拉列表中选择“包括”、在第二个下拉列表中选择“列类型”，然后在第三个下拉列表中选择“数值”。 这将指定所有数值列（和唯一数值）均已转换。

3. 单击此行右侧的加号 (+)，这将创建一行下拉列表。 在第一个下拉列表中选择“排除”，在第二个下拉列表中选择“列名称”，然后在文本字段中输入“信用风险”。 这将指定应忽略“信用风险”列（我们需要执行此操作，因为此列是数值，因此，如果不排除，也将转换）。
...
若要运行此实验，请单击画布下面的“运行”按钮。 可能需要几分钟时间。 每个模块上的旋转指示符显示它正在运行，模块完成后，将显示一个绿色对号。 当所有模块都有一个对号时，表示该实验已完成运行。

实验现在看起来应当与下图类似：  

![评估两种模型][3]

若要检查结果，请单击[评估模型][evaluate-model]模块的输出端口，然后选择“可视化”。  
...
在图表右侧，单击“已评分数据”或“要比较的已评分数据集”，突出显示关联的曲线并在下方显示关联的度量值。 在曲线图例中，“已评分数据集”对应于[评估模型][evaluate-model]模块的左侧输入端口；在本例中，这是提升决策树模型。 “要比较的已评分数据集”对应于右侧输入端口；在本例中，这是 SVM 模型。 单击其中一个标签后，该模型的曲线将突出显示，并且将显示相应的度量值，如下图所示。  

![模型的 ROC 曲线][4]

通过检查这些值，你可以确定哪个模型提供的结果与你所需的结果最接近。 你可以返回，通过更改不同模型中的参数值来迭代进行你的实验。 

对这些结果进行解释以及对模型性能进行优化的科学与艺术不在此演练的范围内。 若要获得更多帮助，可以阅读以下文章：
- [如何在 Azure 机器学习中评估模型性能](machine-learning-evaluate-model-performance.md)
- [选择参数来优化 Azure 机器学习中的算法](machine-learning-algorithm-parameters-optimize.md)
- [在 Azure 机器学习中解释模型结果](machine-learning-interpret-model-results.md)
