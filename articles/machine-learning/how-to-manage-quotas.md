...
ms.locfileid: "83680383"
---
# <a name="manage--increase-quotas-for-resources-with-azure-machine-learning"></a>管理和增加 Azure 机器学习资源的配额
[!INCLUDE [applies-to-skus](../../includes/aml-applies-to-basic-enterprise-sku.md)]

本文为 [Azure 机器学习](overview-what-is-azure-ml.md)用户详细介绍了订阅的 Azure 资源的预配置限制。 还介绍了如何为每种类型的资源请求增加配额。 这些限制用于防止由于欺诈导致的预算超支，并且符合 Azure 容量限制。

与其他 Azure 服务一样，与 Azure 机器学习关联的某些资源存在限制。 这些限制包括工作区的数量上限以及用于模型训练或推断/评分的实际基础计算的限制等[](concept-workspace.md)。 
...
对于每个 Azure 订阅，可以跨服务或以独立方式拥有虚拟机的数量有限。 基于总核心数和每个系列的区域级别也同样有此限制。

虚拟机核心数既有区域总数限制，也有区域按大小系列（Dv2、F 等）限制，这两种限制单独实施。 例如，某个订阅的美国东部 VM 核心总数限制为 30，A 系列核心数限制为 30，D 系列核心数限制为 30。 该订阅可以部署 30 个 A1 VM，或者 30 个 D1 VM，或者同时部署这二者，但其总数不能超过 30 个核心（例如，10 个 A1 VM 和 20 个 D1 VM）。
...
可用资源：
+ 每个区域的专用核心数的默认限制为 24 - 300，具体取决于具有较高的 EA 和 CSP 产品/服务类型默认值的订阅产品/服务类型。  可增加每个订阅的专用核心数，每个 VM 系列的数量各不相同。 某些专用 VM 系列（例如 NCv2、NCv3 或 ND 系列）的默认值从零核心开始。 请通过提出配额请求来联系 Azure 支持以讨论增加选项。

+ 每个区域的低优先级核心数的默认限制为 100 - 3000，具体取决于具有较高的 EA 和 CSP 产品/服务类型默认值的订阅产品/服务类型。 可增加每个订阅的低优先级核心数，该数量是 VM 系列中的单个值。 请联系 Azure 支持以讨论增加选项。

+ 每个区域的群集数的默认限制为 200。 这些限制在训练群集和计算实例（出于配额目的而被视为单个节点群集）之间共享。 如果请求增加的配额超出此限制，请与 Azure 支持部门联系。
...
| 低优先级节点上的最长作业生存期 | 7 天<sup>2</sup> |
| 每个节点的最大参数服务器数 | 1 |

<sup>1</sup> 最长生存期是指运行从开始到结束的时间。 已完成的运行会无限期保存；最长生存期内未完成的运行的数据不可访问。
<sup>2</sup> 存在容量约束时，低优先级节点上的作业可能会被抢占。 我们建议在作业中实施检查点操作。
...
- 每个订阅已发布管道的博客触发计划的基于计划运行数与 blob 拉取数之和的最大数量每月为 100,000
...
为了更好地管理各种[工作区](concept-workspace.md)之间的 Azure 机器学习计算目标 (Amlcompute) 的资源分配，我们引入了一项功能，该功能允许你分发订阅级别配额（按 VM 系列），并在工作区级别配置这些配额。 默认行为是所有工作区都具有与任何 VM 系列的订阅级别配额相同的配额。 但是，随着工作区数量的增加以及不同优先级的工作负载开始共享相同的资源，用户需要一种方法来更好地共享容量并避免资源争用问题。 Azure 机器学习通过允许用户在每个工作区中设置特定 VM 系列的最大配额，提供带有它的托管计算产品/服务的解决方案。 这类似于在工作区之间分发容量，用户也可以选择过度分配来推动最大使用率。 

若要在工作区级别设置配额，请在订阅中转到任何工作区，并在左窗格中单击“使用情况 + 配额”。 然后选择“配置配额”选项卡以查看配额，展开任何 VM 系列，并对该 VM 系列下列出的任何工作区设置配额限制。 请记住，不能设置负值或大于订阅级别配额的值。 此外，正如你所看到的，默认情况下，为所有工作区分配了整个订阅配额，以允许充分利用分配的配额。
...
>
> 必须拥有订阅级别权限才能在工作区级别设置配额。 此操作是强制执行的，以使各个工作区所有者不会编辑或增加其配额，不会开始侵占为其他工作区保留的资源。 因此，订阅管理员最适合跨工作区分配和分发这些配额。
...
1. 你现在可以在订阅级别视图和工作区级别视图之间切换：
    + 订阅视图：使用该视图可以查看按 VM 系列列出的核心配额的使用情况，从而按工作区展开它，进一步按实际群集名称展开它。 此视图最适合快速了解特定 VM 系列的核心使用情况的详细信息，以查看按工作区列出的细分，以及进一步按每个工作区的基础群集列出的细分。 此视图中的常规约定是（使用情况/配额），其中使用情况是当前纵向扩展的核心数，配额是资源可扩展到的逻辑最大核心数。 对于每个工作区，配额将是工作区级别配额（如上所述），它表示特定 VM 系列中可扩展到的最大核心数。 同样，对于群集，配额实际上是与群集可扩展到的由 max_nodes 属性定义的最大节点数相关的核心数。

    + 工作区视图：使用该视图可以查看按工作区列出的核心配额的使用情况，从而按 VM 系列展开它，进一步按实际群集名称展开它。 此视图最适合快速了解特定工作区的核心使用情况的详细信息，以查看按 VM 系列列出的细分，以及进一步按每个系列的基础群集列出的细分。
