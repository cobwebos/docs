...
+ 默认限制因产品/服务类别类型而异，例如免费试用、即用即付和 VM 系列，如 Dv2、F、G 等。
...
1. 现在，可以在订阅级视图与工作区级视图之间切换：
    + **订阅视图：** 这允许您按 VM 系列查看核心配额的使用情况，按工作区扩展核心配额，然后按实际群集名称进一步扩展核心配额。 此视图最适合用于快速了解特定 VM 系列的核心用量详细信息，以查看各工作区的细分，以及按每个工作区的基础群集进一步提供的细分。 此视图中的一般约定是“(用量/配额)”，其中，用量是当前的已扩展核心数，配额是资源可以扩展到的逻辑最大核心数。 对于每个**工作区**，配额是工作区级配额（如前所述），表示特定 VM 系列可扩展到的最大核心数。 同理，对于**群集**，配额实际上是对应于群集可扩展到的最大节点数（由 max_nodes 属性定义）的核心数。

    + **工作区视图：** 这允许您按工作区查看核心配额的使用情况，按 VM 系列扩展核心配额，并通过实际群集名称进一步扩展核心配额。 此视图最适合用于快速了解特定工作区的核心用量详细信息，以查看各 VM 系列的细分，以及按每个系列的基础群集进一步提供的细分。

## <a name="request-quota-increases"></a>请求增加配额

如果要将限制或配额提高到默认限制以上，[请免费打开在线客户支持请求](https://ms.portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest/)。
