...
预测维护是一个内容非常丰富的领域，经常部署了业务问题，这些问题可能来自预测建模视角的许多不同角度。 在接下来的部分中，我们提供了用于为不同业务问题建模的主要技术，这些问题可以用预测维护解决方案回答。 虽然有相似之处，但每个模型都有自己的方法来构造标签，对它们都有详细的描述。 作为附加的资源，可以参考 Azure 机器学习提供的示例试验中的预测性维护模板。 资源部分提供了此模板在线资料的链接。 可以看到如何使用上面讨论的一些特性工程技术和下一节中描述的建模技术来预测使用 Azure 机器学习的飞机引擎故障。
...
这会带来高训练准确性，但是对未看见的测试数据，其性能可能会非常差。 相反，随机欠采样从多数类中选择随机样本，并从训练数据集中删除这些样本。 然而，从多数类中删除样本可能导致分类器错过与多数类相关的重要概念。 混合采样即少数类过度采样，与此同时多数类欠采样，这是另一种可行的方法。 还有许多其他更复杂的抽样技术可用，并且针对类不平衡的有效采样方法是一个流行的研究领域，它受到持续关注，并获得了来自不同渠道的贡献。 使用不同技术来决定最有效的方法，通常这由数据科学家通过研究和实验来决定，并且它高度依赖数据属性。 此外，确保采样方法仅应用于训练集而非测试集尤为重要。
