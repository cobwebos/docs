...
[Azure 数据工厂 DotNet 活动](../../data-factory/v1/data-factory-use-custom-activities.md)
...
* 到此管道的**输入数据**通常会被丢弃，因为输出数据对输入具有完全保真度 - 它只是被更好地存储（分区）以供后续使用。
...
一旦管道成功执行，会在“connectedcar”容器下的存储帐户中看到以下已生成的分区。
...
在本节中，将了解如何结合 Azure 流分析、Azure 机器学习、Azure 数据工厂和 Azure HDInsight 来丰富关于车辆运行状况和驾驶习惯的高级分析。 此处有三个小节：
...
此模式也适用于希望将业务线 (LoB) 应用程序与实时分析流集成的场景，如警报、通知和消息传递。
