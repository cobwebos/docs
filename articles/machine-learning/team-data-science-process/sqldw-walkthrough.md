...
**安装 Visual Studio 和 SQL Server Data Tools。** 有关说明，请参阅[Visual Studio 2019 FOR SQL 数据仓库](../../sql-data-warehouse/sql-data-warehouse-install-visual-studio.md)入门。
...
> [!NOTE]
> 若要避免架构名称与 Azure Azure Synapse Analytics 中已存在的架构名称冲突，请在直接从 SQLDW 文件中读取参数时，将 SQLDW 文件中的架构名称作为默认架构添加到架构名称中每个运行的名称。 PowerShell 脚本可能会提示输入架构名称︰该名称可以由用户自行指定。
>
>
...
下面的查询联接 **nyctaxi\_trip** 和 **nyctaxi\_fare** 表，生成一个二元分类标签 **tipped**，多类分类标签 **tip\_class**，并从完整联接的数据集中提取样本。 采样是通过检索基于提取时间的行程的子集来完成的。  可以复制此查询，然后将其直接粘贴到[Azure 机器学习 Studio （经典）](https://studio.azureml.net) [导入数据][导入数据模块，]，以便从 Azure 中的 SQL 数据库实例进行直接数据引入。 此查询将排除具有不正确（0，0）坐标的记录。
...
2. 保留计划用于在新的 Azure Synapse 分析表中进行建模的抽样和工程数据，并使用 Azure 机器学习中的 "[导入数据模块，][导入数据模块，]" 模块中的新表。 之前步骤中的 PowerShell 脚本已为你执行此任务。 可以直接从“导入数据”模块中的此表读取。
...
1. 使用 "**数据输入和输出**" 部分提供的 "[导入数据模块，][导入数据模块，]" 模块，将数据导入 Azure 机器学习 Studio （经典）。 有关详细信息，请参阅[导入数据][导入数据模块，]模块参考页。
...
下图列出了直接从 Azure Synapse Analytics 数据库读取数据的二进制分类试验的示例（请记住，将表名称替换 nyctaxi_trip，nyctaxi_fare 然后按在演练）。 可以针对多类分类和回归问题构建类似实验。
