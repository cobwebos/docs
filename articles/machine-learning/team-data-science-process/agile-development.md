...
- 为项目涉及的工作项进行*冲刺 （sprint） 规划*。
- 将*工作项*添加到冲刺 （sprint） 中。
...
在 TDSP 冲刺 （sprint） 规划框架中，有四种常用*的工作项*类型：*功能*、*用户情景*、*任务*和*Bug。* 所有工作项的积压工作 (backlog) 都处于项目级别，而不是 Git 存储库级别。 

下面是工作项类型的定义：

- **功能**：功能对应于项目参与。 与客户端不同的协定是不同的功能，最好将项目的不同阶段视为不同的功能。 如果选择*\<客户端名称>-\<参与名称>* 等架构来命名您的功能，则可以轻松地从名称本身识别项目的上下文和参与度。
  
- **用户情景**：用户情景是端到端完成功能所需的工作项。 用户情景示例包括：
  - 获取数据 
  - 浏览数据 
  - 生成功能
  - 生成模型
...
- **任务**：任务是完成特定用户情景所需的可分配工作项。 例如，用户情景“获取数据”中的任务可能是**：
  - 获取 SQL Server 凭据
  - 将数据上传到 SQL 数据仓库
  
- **Bug**： Bug 是现有代码或文档中的问题，必须修复以完成任务。 如果 Bug 是由于缺少工作项引起的，则可以将其升级为用户情景或任务。 
...
在"功能"下，您可以添加用户情景来描述完成项目所需的主要步骤。 
...
要将任务添加到用户故事，请选择"用户故事**+**"项的旁边，然后选择 **"任务**"。 在任务中填写标题和其他信息。
...
1. 在所有**进程中**，选择 **"积压工作级别**"选项卡。在 **"项目组合积压工作**"下，选择 Epic 旁边的 **...** **（禁用的），** 然后选择 **"编辑/重命名**"。 
...
1. 要在 TDSP 项目下添加工作项，请选择**+** 项目的下一个，然后选择要创建的工作项的类型。 
   
   ![选择数据科学工作项类型](./media/agile-development/17-dsworkitems1.png)
   
1. 在新工作项中填写详细信息，然后选择“保存并关闭”****。
   
1. 继续选择工作项**+** 旁边的符号，以添加新的 TDSP 阶段、子阶段和任务。 
...
[与 Git 的协作编码](collaborative-coding-with-git.md)描述了如何使用 Git 作为共享代码开发框架为数据科学项目进行协作代码开发，以及如何将这些编码活动与敏捷流程计划的工作联系起来。
