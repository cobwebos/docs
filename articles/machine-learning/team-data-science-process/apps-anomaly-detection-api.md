...
默认情况下，部署将有一个免费的开发/测试计费计划，其中包括1000个交易/月和2个计算小时/月。  可以根据需求升级到其他计划。  在[此处](https://azure.microsoft.com/pricing/details/machine-learning/)的“生产 Web API 定价”下提供了有关不同计划的定价的详细信息。
...
若要调用该 API，需要知道终结点位置和 API 密钥。  这两个要求以及用于调用 API 的示例代码可从[Azure 机器学习 Studio （经典） web 服务](https://services.azureml.net/webservices/)页获取。  导航到所需的 API，并单击“使用”选项卡可找到它们。  可以将 API 调用为 Swagger API （即，使用 URL 参数`format=swagger`）或作为非 Swagger api （即，不包含`format` url 参数）。  代码示例使用 Swagger 格式。  下面是采用非 Swagger 格式的示例请求和响应。  这些示例针对季节性终结点。  非季节性终结点与它类似。
...
下图显示 Score API 可检测到的异常情况示例。 此时序有两个不同的级别更改，三个峰值。 红点显示检测到的更改级别的时间，而黑色的点显示检测到的峰值。
![Score API][1]

### <a name="detectors"></a>检测程序
异常情况检测 API 支持三个广义类别的检测程序。 可在下表中找到特定输入的参数和每个检测程序输出结果的详细信息。
...
| 慢速趋势检测程序 |慢速趋势检测程序 |根据每组的敏感度，检测慢速正面发展趋势 |*trenddetector：* 探测器分数上的阈值（默认值：3.25，3.25 –5是要从中选择的合理范围;敏感程度较低的 |tscore：浮动数字，表示趋势的异常分数 |
| 级别更改检测程序 | 双向级别更改检测程序 |根据每组的敏感度，检测向下和向上级别的更改 |*bileveldetector：* 探测器分数上的阈值（默认值：3.25，3.25 –5是要从中选择的合理范围;敏感程度较低的 |rpscore：浮动数字，表示向上和向下级别更改的异常分数 |
...
下图显示在季节性时序中检测到的异常情况示例。 时序有一个峰值（第一个黑点）、两个 dip （第二个黑点和一端的点）和一个级别更改（红点）。 在从序列中移除季节性组件后，时间序列中间的 dip 和级别更改只是明显。
![Seasonality API][2]
...
| tspikedetector.sensitivity |TSpike 检测程序的敏感度 |3 |integer |1-10 |3-5（值越小，敏感度越高） |
| zspikedetector.sensitivity |ZSpike 检测程序的敏感度 |3 |integer |1-10 |3-5（值越小，敏感度越高） |
| seasonality.enable |是否要执行季节性分析 |是 |boolean |true、false |时间序列依赖项 |
...
| ProcessedData |以下选项之一： <ul><li>如果检测到重大季节性变化并选中非季节性选项，随季节波动调整时间序列；</li><li>如果检测到重大季节性变化并选中非季节性选项，随季节波动调整时间序列，并去除其趋势</li><li>否则，此选项与 OriginalData 相同。</li> |
| TSpike |二进制指示符指示 TSpike 检测程序是否检测到峰值 |
| ZSpike |二进制指示符指示 ZSpike 检测程序是否检测到峰值 |
