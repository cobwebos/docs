...
> [!NOTE]
> 此项处于维护状态。 我们建议你使用由 Azure 认知服务下的机器学习算法库支持的[异常检测器 API 服务](https://azure.microsoft.com/services/cognitive-services/anomaly-detector/)来检测业务、运营和 IoT 指标中的异常。
...
要使用 API，必须将其部署到 Azure 订阅，在该订阅中它将作为 Azure 机器学习 Web 服务进行托管。  可以从 [Azure AI 库](https://gallery.cortanaintelligence.com/MachineLearningAPI/Anomaly-Detection-2)执行此操作。  这会将两个 Azure 机器学习工作室（经典）Web 服务（及其相关资源）部署到 Azure 订阅 - 一个用于异常情况检测（包含季节性检测），另一个不包含季节性检测。  部署完成后，便能从 [Azure 机器学习工作室（经典）Web 服务](https://services.azureml.net/webservices/)页管理 API。  在该页中，能够查找终结点位置、API 密钥以及调用 API 的示例代码。  [此处](https://docs.microsoft.com/azure/machine-learning/machine-learning-manage-new-webservice)提供了更详细的说明。

## <a name="scaling-the-api"></a>缩放 API
默认情况下，您的部署将有一个免费的 Dev/Test 计费计划，其中包括 1，000 个事务/月和 2 个计算小时/月。  可以根据需求升级到其他计划。  在[此处](https://azure.microsoft.com/pricing/details/machine-learning/)的“生产 Web API 定价”下提供了有关不同计划的定价的详细信息。
...
Web 服务通过 HTTPS 提供基于 REST 的 API，可通过不同方式使用，包括 Web 或移动应用程序、R、Python、Excel 等。 通过 REST API 调用将时间序列数据发送到此服务，并运行下面描述的三种异常类型的组合。

## <a name="calling-the-api"></a>调用 API
若要调用该 API，需要知道终结点位置和 API 密钥。  这两个要求以及调用 API 的示例代码可从[Azure 机器学习工作室（经典）Web 服务](https://services.azureml.net/webservices/)页面获得。  导航到所需的 API，并单击“使用”选项卡可找到它们。  您可以将 API 称为 Swagger API（即使用 URL 参数`format=swagger`）或非斯瓦格 API（即没有`format`URL 参数）。  代码示例使用 Swagger 格式。  下面是采用非 Swagger 格式的示例请求和响应。  这些示例针对季节性终结点。  非季节性终结点与它类似。
...
要查看该`ColumnNames`字段，必须在请求中包含`details=true`作为 URL 参数。  请参阅下表，了解下述每个字段背后的含义。
...
下图显示 Score API 可检测到的异常情况示例。 此时间序列有两个不同的级别更改和三个峰值。 红点显示检测到的更改级别的时间，而黑色的点显示检测到的峰值。
![Score API][1]

### <a name="detectors"></a>检测程序
异常检测 API 支持三大类探测器。 可在下表中找到特定输入的参数和每个检测程序输出结果的详细信息。
...
| 慢速趋势检测程序 |慢速趋势检测程序 |根据每组的敏感度，检测慢速正面发展趋势 |*趋势检测器.灵敏度：* 探测器分数的阈值（默认值：3.25、3.25 + 5 是可供选择的合理范围;越高，敏感性越低） |tscore：浮动数字，表示趋势的异常分数 |
| 级别更改检测程序 | 双向级别更改检测程序 |根据每组的敏感度，检测向下和向上级别的更改 |*双水平检测器.灵敏度：* 探测器分数的阈值（默认值：3.25、3.25 + 5 是可供选择的合理范围;越高，敏感性越低） |rpscore：浮动数字，表示向上和向下级别更改的异常分数 |
...
| detectors.spikesdips | 是仅检测峰值、仅检测 dip，还是两者都检测 |推送、请求和匿名 |枚举 |两者、峰值、Dip |推送、请求和匿名 |
...
| postprocess.tailRows |保留输出结果中的最新数据点的数量 |0 |integer |0（保留所有数据点），或指定要保留在结果中的点的数量 |空值 |
...
下图显示在季节性时序中检测到的异常情况示例。 时间序列有一个尖峰（第一个黑点），两个凹陷（第二个黑点，一个在末尾），一个水平变化（红点）。 只有在从序列中删除季节性组件后，时间序列中间的跌度和水平变化才能识别。
![Seasonality API][2]
...
| preprocess.aggregationFunc |用于将数据聚合到指定的 AggregationInterval 函数 |平均值 |枚举 |平均值、总和、长度 |空值 |
| preprocess.replaceMissing |用户输入缺失的数据值 |lkv（最近一次的值） |枚举 |零、lkv、平均值 |空值 |
| detectors.historywindow |用于记录异常分数计算结果（在数据点的 #） |500 |integer |10-2000 |时间序列依赖项 |
| detectors.spikesdips | 是仅检测峰值、仅检测 dip，还是两者都检测 |推送、请求和匿名 |枚举 |两者、峰值、Dip |推送、请求和匿名 |
| bileveldetector.sensitivity |双向级别更改检测程序的敏感度。 |3.25 |double |无 |3.25-5（值越小，敏感度越高） |
...
| seasonality.transform |是否应在应用异常检测前删除季节性（和）趋势组件 |deseason |枚举 |none, deseason, deseasontrend |空值 |
| postprocess.tailRows |保留输出结果中的最新数据点的数量 |0 |integer |0（保留所有数据点），或指定要保留在结果中的点的数量 |空值 |
...
| ProcessedData |以下任一选项： <ul><li>如果检测到重大季节性变化并选中非季节性选项，随季节波动调整时间序列；</li><li>如果检测到重大季节性变化并选中非季节性选项，随季节波动调整时间序列，并去除其趋势</li><li>否则，此选项与原始数据相同</li> |
| TSpike |二进制指示符指示 TSpike 检测程序是否检测到峰值 |
| ZSpike |二进制指示符指示 ZSpike 检测程序是否检测到峰值 |
