...
ms.locfileid: "83656543"
---
# <a name="machine-learning-anomaly-detection-api"></a>机器学习异常情况检测 API

> [!NOTE]
> 此项处于维护状态。 建议使用由 Azure 认知服务下的机器学习算法库支持的[异常检测器 API 服务](https://azure.microsoft.com/services/cognitive-services/anomaly-detector/)来检测业务、运营和 IoT 指标中的异常。

## <a name="overview"></a>概述
[异常情况检测 API](https://gallery.azure.ai/MachineLearningAPI/Anomaly-Detection-2) 是一个示例，使用 Azure 机器学习构建，检测时序数据和均匀分布在时间中的数字值的异常情况。
...
> [!NOTE]
> 请尝试由 [此 API](https://gallery.azure.ai/MachineLearningAPI/Anomaly-Detection-2) 提供的“IT 异常 Insights 解决方案”
>
...
若要调用该 API，需要知道终结点位置和 API 密钥。  这两项要求以及调用该 API 的示例代码可从 [Azure 机器学习工作室（经典）Web 服务](https://services.azureml.net/webservices/)页获得。  导航到所需的 API，并单击“使用”选项卡可找到它们。  可以像调用 Swagger API 一样调用该 API（即使用 URL 参数 `format=swagger`），也可以像调用非 Swagger API 一样调用该 API（即不使用 `format` URL 参数）。  代码示例使用 Swagger 格式。  下面是采用非 Swagger 格式的示例请求和响应。  这些示例针对季节性终结点。  非季节性终结点与它类似。
...
下图显示 Score API 可检测到的异常情况示例。 此时间序列具有两个不同级别的更改和三个峰值。 红点显示检测到的更改级别的时间，而黑色的点显示检测到的峰值。
![Score API][1]

### <a name="detectors"></a>检测程序
异常情况检测 API 支持检测程序中的三个主要类别。 可在下表中找到特定输入的参数和每个检测程序输出结果的详细信息。
...
下图显示在季节性时序中检测到的异常情况示例。 时间序列都有一个最大值（第一个黑点）、两个低值（第二个黑点和结尾处的黑点）和一个级别更改（红点）。 从序列中删除季节性组件后才可识别时间序列中间的低值和级别更改。
![Seasonality API][2]
