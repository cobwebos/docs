...
本教程介绍步骤 1：创建项目存储库，以及步骤 2：从团队 ProjectTemplate 存储库中设置种子项目存储库。 

有关步骤 3：为项目创建功能工作项，步骤 4：为项目阶段添加情景，请参阅[数据科学项目的敏捷开发](agile-development.md)。

有关步骤 5：创建和自定义存储/分析资产，并在必要时共享，请参阅[创建团队数据和分析资源](team-lead-tasks.md#create-team-data-and-analytics-resources)。

有关步骤 6：设置项目存储库的安全控制，请参阅[添加团队成员并配置权限](team-lead-tasks.md#add-team-members-and-configure-permissions)。
...
要克隆存储库并修改本地计算机或数据科学虚拟机 （DSVM） 上的内容，或设置 Azure 文件存储并将其装载到 DSVM，还需要考虑以下检查表：
...
1. 转到团队的项目**摘要**页面*\//\<https：服务器\<名称>/组织名称>/\<团队名称>，* 例如**https：/dev.azure.com/DataScienceUnit/MyTeam\/**，并从左侧导航中选择**Repos。** 
...
1. 在“导入 Git 存储库”**** 对话框中，选择“Git”作为“源类型”，然后在“克隆 URL”下输入 **TeamTemplate** 存储库的 URL。************ URL 是*https：\//\<服务器名称\<>/组织名称>/\<团队名称>/_git/\<团队模板存储库名称>*。 例如：**https:\//dev.azure.com/DataScienceUnit/MyTeam/_git/TeamTemplate**。 
