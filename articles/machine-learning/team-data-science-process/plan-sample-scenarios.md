...
最后一节介绍了用于选择适合您的数据和目标的示例方案**的决策树**。
...
## <a name="scenario-1-small-to-medium-tabular-dataset-in-local-files"></a><a name="smalllocal"></a>方案\#1：本地文件中的中小型表格数据集
![小型到中型本地文件][1]
...
1. 将数据上载到 Azure 存储容器。
1. 预处理和清理 IPython 笔记本中的数据，从 Azure 存储容器访问数据。
1. 将数据转换为已清理的表格形式。
...
## <a name="scenario-4-small-to-medium-dataset-of-local-files-targeting-sql-server-in-an-azure-virtual-machine"></a><a name="smalllocaltodb"></a>方案 \#4：本地文件中针对 Azure 虚拟机中 SQL Server 的小型到中型数据集
...
1. 将数据上载到 Azure 存储容器。
1. 使用 IPython 笔记本在 Azure 存储容器中预处理和清理数据。
1. 如果需要，将数据转换为已清理的表格形式。
...
1. 将数据加载到在 Azure VM 上运行的 SQL Server 数据库。
   ...
   * 创建数据库和目标表。
   * 使用某个大容量导入方法从本地 VM 文件加载数据。
1. 浏览数据，根据需要创建功能。 不需要在数据库表中具体化这些功能。 仅请注意创建它们所需的查询。
1. 如果需要和/或想要，请确定数据示例大小。
1. 登录到 [Azure 机器学习工作室](https://studio.azureml.net/)。
1. 使用[导入数据][import-data]模块直接从 SQL Server 中读取数据。 粘贴提取字段、创建要素和采样数据的必要查询（如果需要）直接在["导入数据"][import-data]查询中。
1. 开始使用引入的数据集生成 Azure 机器学习实验流。

## <a name="scenario-5-large-dataset-in-local-files-target-sql-server-in-azure-vm"></a><a name="largelocaltodb"></a>方案\#5：本地文件中的大型数据集，Azure VM 中的目标 SQL 服务器
...
1. （可选）预处理并清理数据。
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。  访问 Azure blob 中的数据，预处理并清理 IPython Notebook 中的数据。
   
    b.保留“数据库类型”设置，即设置为“共享”。  如果需要，将数据转换为已清理的表格形式。
...
1. 将数据加载到在 Azure VM 上运行的 SQL Server 数据库。
    ...
    b.保留“数据库类型”设置，即设置为“共享”。  如果数据尚未保存，请从 Azure 存储容器将数据文件下载到本地 VM 文件夹。
...
## <a name="scenario-6-large-dataset-in-a-sql-server-database-on-premises-targeting-sql-server-in-an-azure-virtual-machine"></a><a name="largedbtodb"></a>方案\#6：本地 SQL Server 数据库中的大型数据集，以 Azure 虚拟机中的 SQL Server 为目标
...
1. 使用一种数据导出方法将数据从 SQL Server 导出到转储文件。
   ...
   > 
   > 
1. 将转储文件上载到 Azure 存储容器。
1. 将数据加载到在 Azure 虚拟机上运行的 SQL Server 数据库。
   
   a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。  登录到 SQL Server VM。
   
   b.保留“数据库类型”设置，即设置为“共享”。  将数据文件从 Azure 存储容器下载到本地 VM 文件夹。
...
#### <a name="additional-azure-resources-azure-virtual-machine-sql-server--ipython-notebook-server"></a>其他 Azure 资源：Azure 虚拟机（SQL Server/IPython Notebook 服务器）
要复制 SQL Server VM 中的完整 SQL Server 数据库，假定数据库可暂时处于脱机状态，应将数据库从一个位置/服务器复制到另一个。 您可以使用 SQL 服务器管理工作室对象资源管理器，或使用等效的 Transact-SQL 命令。
...
1. 将数据加载到 Azure HDInsight Hadoop 群集中的 Hive 数据库。
   ...
   > 
   > 
1. 浏览数据，并根据需要在 Hadoop 命令行中创建功能。 不需要在数据库表中具体化这些功能。 仅请注意创建它们所需的查询。
...
1. 登录到 [Azure 机器学习工作室](https://studio.azureml.net/)。
1. 使用[导入数据][import-data]模块直接从 `Hive Queries` 中读取数据。 粘贴提取字段、创建要素和采样数据的必要查询（如果需要）直接在["导入数据"][import-data]查询中。
1. 开始使用上传的数据集对 Azure 机器学习实验流进行采样。

## <a name="decision-tree-for-scenario-selection"></a><a name="decisiontree"></a>用于方案选择的决策树
---
下面的图表总结了上述的方案以及让你采用每个详细方案的高级分析流程和技术选择。 数据处理、探索、功能工程和采样可在一种或多种方法/环境中（在源、中间和/或目标环境中）进行，并可根据需要进行迭代。 此图只用于列举某些可能流，并不提供详尽的枚举。
