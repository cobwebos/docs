...
>[AZURE.NOTE] 下列说明概述了使用 Visual Studio Team Services (VSTS) 设置 TDSP 团队环境所需的步骤。 我们指定了如何使用 VSTS 完成这些任务，因为这是我们在 Microsoft 中实现 TDSP 的方法。 如果你的组使用了其他代码托管平台，团队主管需要完成的任务通常不会发生变化。 但是完成这些任务的方法将会有所不同。


## <a name="repositories-and-directories"></a>存储库和目录

本教程使用了存储库和目录的缩写名称。 这些名称可便于掌握存储库和目录之间的操作。 以下部分使用了此表示法（R 表示 Git 存储库，D 表示 DSVM 上的本地目录）：
...
- 你的计算机必须已安装 Git。 如果使用的是数据科学虚拟机 (DSVM)，则已预装 Git，你可以继续操作了。 否则，请参阅[平台和工具附录](platforms-and-tools.md#appendix)。  
- 如果使用的是 Windows DSVM，则需要在你的计算机上安装 [Git 凭据管理器 (GCM)](https://github.com/Microsoft/Git-Credential-Manager-for-Windows)。 在 README.md 文件中，向下滚动到“下载并安装”部分，然后单击“最新安装程序”。 此操作会将你转到最新安转程序页。 在此处下载 .exe 安装程序并运行该程序。 
- 如果使用的是 Linux DSVM，则在你的 DSVM 上创建一个 SSH 公钥，然后将它添加到你的组 VSTS 服务器。 有关 SSH 的详细信息，请参阅[平台和工具附录](platforms-and-tools.md#appendix)中的“创建 SSH 公钥”部分。 
...
以下是 Team Data Science Process 定义的角色和任务的详细说明的链接：

- [数据科学团队的组管理员的任务](group-manager-tasks.md)
- [数据科学团队的团队主管的任务](team-lead-tasks.md)
- [数据科学团队的项目主管的任务](project-lead-tasks.md)
- [数据科学团队的项目单个参与者](project-ic-tasks.md)
