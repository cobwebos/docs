...
GroupCommon 项目的摘要页随即打开********。 页面 URL 是*\//\<https：服务器名称\<>/组织名称>/组公共*。
...
- **组公共存储库**：数据科学单元中的多个团队可以采用的通用存储库，用于许多数据科学项目。 
- **团队**存储库：数据科学单元内特定团队的存储库。 这些存储库特定于团队需求，可用于该团队内的多个项目，但不足以使数据科学部门中的多个团队使用。
- **项目**存储库：特定项目的存储库。 此类存储库通用程度比较低，可能不足以供一个团队内的多个项目或数据科学部门内的其他团队使用。
...
1. 在"**项目设置"** 页上，选择左侧导航中的**存储库**下的**存储库**以查看两个组存储库：**组项目模板**和**组实用程序**。
