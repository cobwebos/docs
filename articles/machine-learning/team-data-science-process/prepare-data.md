...
预处理和清理数据是数据集可用于模型培训之前必须执行的重要任务。 原始数据通常具有干扰性且不可靠，还可能缺少值。 使用此类数据进行建模会产生误导性结果。 这些任务是 Team Data Science Process (TDSP) 的一部分，通常对用于发现和计划所需预处理的数据集进行初步探索。 有关 TDSP 过程的详细说明，请参阅 [Team Data Science Process](overview.md) 中概述的步骤。
...
有关在 Azure 机器学习工作室（经典）内完成的数据探索和预处理的示例，请参阅[预处理数据](https://azure.microsoft.com/documentation/videos/preprocessing-data-in-azure-ml-studio/)视频。
...
* **不一致的数据记录**。 查看允许的值范围。 例如，如果数据包含学生 GPA（成绩点平均值），请检查 GPA 是否在指定范围内，例如 0+4。

当您发现数据问题时，**需要执行处理步骤**，这通常涉及清理缺失值、数据规范化、离散化、文本处理以删除和/或替换可能影响数据对齐的嵌入字符、常见字段中的混合数据类型等。
...
* **数据清理**：填充缺失值，检测并删除嘈杂的数据和异常值。
* **数据转换**：规范化数据以降低维数和干扰。
* **数据减少**：对数据记录或属性进行采样，便于数据处理。
* **数据离散化**：将连续属性转换为分类属性，便于使用某些机器学习方法。
* **文本清理**：删除可能导致数据不对齐的嵌入字符，例如，选项卡分隔的数据文件中的嵌入选项卡，嵌入可能中断记录的新行。例如。
...
数据规范化将数值重新缩放到指定范围。 常用的数据规范化方法包括：
...
**表格数据中的文本字段**可能包含影响列对齐和/或记录边界的字符。 例如，选项卡分隔文件中的嵌入选项卡会导致列不对齐，而嵌入的新行字符会中断记录行。 编写或阅读文本时的文本编码处理不当会导致信息丢失、无意中引入不可读的字符（如空字符），还可能影响文本解析。 若要清理文本字段以确保正确对齐和/或从非结构化或半结构化文本数据中提取结构化数据，可能需要进行仔细的分析和编辑。

**数据浏览**支持提前预览数据。 执行该步骤时会发现多个数据问题，可应用相应的方法解决这些问题。  提问非常重要，例如问题的源是什么以及问题是如何引入的。 此过程还可以帮助您确定需要解决这些步骤所需的数据处理步骤。 识别最终用例和角色也可用于确定数据处理工作的优先级。
