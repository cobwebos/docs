...
1.  在已解压缩的文件夹中，打开应用配置文件 RealtimeDashboardApp.exe.config。使用手动操作说明中的值替换事件中心、Azure Blob 存储和 Azure 机器学习服务连接中的应用设置。 保存所做更改。

1. 运行应用程序 RealtimeDashboardApp.exe。 在登录窗口中，输入有效的 Power BI 凭据。 
...
1. 在 Power BI 中生成新的数据集。 选择“ConnectedCarsRealtime”数据集。
...
1. 在“字段”选项卡上，选择“vin”。 在“可视化”选项卡上，选择“卡”可视化。  
...
1. 在“字段”选项卡上，选择“市/县”和“vin”。 在“可视化”选项卡上，选择“地图”可视化。 将“vin”拖到“值”区域。 将“市/县”拖到“图例”区域。 

    ![卡可视化](./media/cortana-analytics-playbook-vehicle-telemetry-powerbi-dashboard/connected-cars-3.4c.png)

1. 在“可视化”选项卡上，选择“格式”部分。 选择“标题”，将“文本”更改为“按市/县划分的运行中的车辆”。
...
1. 在“字段”选项卡上，将“模型”拖到“轴”区域。 将“vin”拖到“值”区域。 然后按照“vin 的计数”对可视化排序。 将图表“标题”更改为“按模型划分的需要维护的车辆”。 

1. 在“可视化”选项卡的“字段” ![Fields-image](./media/cortana-analytics-playbook-vehicle-telemetry-powerbi-dashboard/connected-cars-3.4field.png) 部分中，将“vin”拖到“颜色饱和度”。
...
1. 在“可视化”选项卡上，选择“仪表”可视化。 将“速度”拖到“值”、“最小值”和“最大值”区域。

   ![车辆运行状况统计信息](./media/cortana-analytics-playbook-vehicle-telemetry-powerbi-dashboard/connected-cars-3.4w.png)  

1. 在“值”区域中，将“速度”的默认聚合更改为“平均值”。

1. 在“最小值”区域中，将“速度”的默认聚合更改为“最小值”。

1. 在“最大值”区域中，将“速度”的默认聚合更改为“最大值”。
...
1. 在此处下载用于批处理仪表板的名为“ConnectedCarsPbiReport.pbix”的 Power BI 模板。
...
1. 将“数据库”保留为 connectedcar。

    ![数据库](./media/cortana-analytics-playbook-vehicle-telemetry-powerbi-dashboard/12-set-powerbi-database.png)

1. 选择“确定”。

1. 默认选择“Windows 凭据”选项。 通过选择右侧的“数据库”选项卡，将其更改为“数据库凭据”。

1. 输入在部署设置期间指定的 Azure SQL 数据库的“用户名”和“密码”。

    ![数据库凭据](./media/cortana-analytics-playbook-vehicle-telemetry-powerbi-dashboard/13-provide-database-credentials.png)

1. 选择“连接”。

1. 对右侧窗格中显示的剩下三个查询重复前面的步骤。 然后更新数据源连接详细信息。

1. 选择“关闭并加载”。 Power BI Desktop 文件数据集连接到 SQL 数据库表。

1. 选择“关闭”关闭 Power BI Desktop 文件。
...
1. 上传 Power BI Desktop 文件。 选择“获取数据” > “文件获取” > “本地文件”。

1. 转到“ConnectedCarsPbiReport.pbix”。

1. 文件完成上传后，返回到 Power BI 工作空间。 将创建数据集、报表和空白仪表板。  

1. 将图表固定到 Power BI 中名为“车辆遥测分析仪表板”的新仪表板上。 选择前面已创建的空白仪表板，然后转到“报表”部分。 选择新上传的报表。  
...
1. 将鼠标悬停在每个图表上。 根据以下已完成的仪表板示例提供的命名，对每个图表重新命名：
...
1. 创建了本文档中提及到的所有报表之后，最终完成的仪表板应如以下示例所示： 
