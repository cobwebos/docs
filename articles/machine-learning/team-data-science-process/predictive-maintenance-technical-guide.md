...
description: 解决方案模板的技术指南，适用于航空、公用事业和运输的预测性维护。
...
ms.locfileid: "79087434"
---
# <a name="technical-guide-to-the-solution-template-for-predictive-maintenance-in-aerospace"></a>用于航空的预测性维护的解决方案模板技术指南
...
解决方案模板旨在加速构建 E2E 演示的过程。 已部署的模板使用必要的组件预配订阅，然后生成它们之间的关系。 它还使用从数据生成器应用程序中的数据示例来设定数据管道的种子（会在部署解决方案模板后在本地计算机上下载和安装数据生成器应用程序）。 生成器中的数据将生成数据管道，并开始生成机器学习预测，然后可在 Power BI 仪表板上将其可视化。
...
2. 使用 Power BI Desktop 更新冷路径报表文件的数据源。
   ...
   * 会看到两个表：**RemainingUsefulLife** 和 **PMResult**。 选择第一个表，并在右侧“查询设置”![](./media/predictive-maintenance-technical-guide/icon-query-settings.png)面板的“应用的步骤”下的“源”旁边单击**查询设置图标**。 忽略显示的任何警告消息。
   ...
   * 单击第二个表 **PMResult**，在右侧“查询设置”![](./media/predictive-maintenance-technical-guide/icon-navigation.png)面板的“应用的步骤”下的“源”旁边单击**导航图标**，根据以上步骤更新服务器和数据库名称并单击“确定”。
...
3. （可选）将冷路径仪表板发布到 [Power BI online](https://www.powerbi.com/)。 此步骤需要 Power BI 帐户（或 Office 365 帐户）。
   ...
   * 打开报表后，请单击![大头针图标](./media/predictive-maintenance-technical-guide/icon-pin.png)以将所有可视化效果固定到仪表板。 要查找详细说明，请参阅[从报表将磁贴固定到 Power BI 仪表板](https://support.powerbi.com/knowledgebase/articles/430323-pin-a-tile-to-a-power-bi-dashboard-from-a-report)。
     ...
     <br/>
   * 要计划数据刷新，请将鼠标悬停在**PredictiveMaintenanceAerospace**数据集上，单击](./media/predictive-maintenance-technical-guide/icon-elipsis.png) ![省略号图标，然后选择 "**计划刷新**"。
     <br/>
