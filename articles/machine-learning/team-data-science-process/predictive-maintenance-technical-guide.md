...
description: 用于航空航天、公用事业和运输预测性维护的解决方案模板技术指南。
...
ms.locfileid: "79477164"
---
# <a name="technical-guide-to-the-solution-template-for-predictive-maintenance-in-aerospace"></a>航空航天预测性维护解决方案模板技术指南
...
解决方案模板旨在加快构建 E2E 演示的过程。 已部署的模板使用必要的组件为订阅提供配置，然后构建它们之间的关系。 它还使用从数据生成器应用程序中的数据示例来设定数据管道的种子（会在部署解决方案模板后在本地计算机上下载和安装数据生成器应用程序）。 生成器中的数据将生成数据管道，并开始生成机器学习预测，然后可在 Power BI 仪表板上将其可视化。
...
部署解决方案时，它会激活 Azure 服务，包括事件中心、流分析、HDInsight、数据工厂和机器学习。 体系结构图展示了如何构造航天工业预测性维护解决方案模板。 可以通过在与解决方案部署一起创建的解决方案模板图表上单击 Azure 门户中的服务对其进行调查（HDInsight 除外，该服务是在要求相关管道活动运行时按需预配的，且随后进行了删除）。
...
对于此模板，使用的数据源是从成功部署后在本地运行的已下载桌面应用程序生成的。
...
使用 HDInsight 运行[Hive](https://blogs.msdn.com/b/bigdatasupport/archive/2013/11/11/get-started-with-hive-on-hdinsight.aspx)脚本（由 Azure 数据工厂协调），以提供使用 Azure 流分析资源存档的原始事件的聚合。
...
使用 Azure 流分析资源通过从数据流读取数据并将数据输入任意数量的源来提供近乎实时的分析。
...
[Azure 数据工厂](https://azure.microsoft.com/documentation/services/data-factory/)服务协调数据的移动和处理。 在航天工业预测性维护的解决方案模板中，数据工厂由 3 个[管道](../../data-factory/concepts-pipelines-activities.md)组成，使用不同的技术移动和处理数据。  可以打开随解决方案部署创建的解决方案模板图示底部的数据工厂节点来访问数据工厂。 数据集下的错误之所以发生，是因为启动数据生成器之前已部署数据工厂。 这些错误可以忽略，不会阻止数据工厂运行。
...
* 管道的最后一个步骤是将数据（例如机器学习的预测数据）写入 SQL 数据库。 最多可能需要 3 个小时，数据才会出现在 SQL 数据库中。 监视 SQL 数据库中有多少数据的方法之一是使用 [Azure 门户](https://portal.azure.com/)。 在左侧面板上，找到 SQL![数据库](./media/predictive-maintenance-technical-guide/icon-SQL-databases.png)SQL 图标并单击它。 然后查找自己的数据库 **pmaintenancedb** 并单击它。 在底部的下一页，单击"管理"。
...
Power BI 作为数据源连接到 Azure SQL 数据库，其中存储预测结果。 

注意： 
1.    部署解决方案时，将在 3 小时内在数据库中显示预测。 生成器下载附带的 pbix 文件包含某些种子数据，因此需要立即创建 Power BI 仪表板。 
2.    在此步骤中，先决条件是下载并安装免费软件 Power [BI 桌面](https://powerbi.microsoft.com/documentation/powerbi-desktop-get-the-desktop/)。
...
1. 获取数据库凭据。
   ...
   * 您将看到显示 Azure 门户页的新浏览器选项卡/窗口。 单击左侧面板中的“资源组”****。
...
2. 使用 Power BI Desktop 更新冷路径报表文件的数据源。
   ...
   * 下一个弹出窗口的左侧窗格中出现了两个选项（“Windows”**** 和“数据库”****）。 单击 **"数据库"，** 填写 **"用户名"** 和 **"密码"（** 首次部署解决方案并创建 Azure SQL 数据库时输入的用户名和密码）。 在“选择要将这些设置应用到的级别”****** 中，选中数据库级别选项。 然后单击 **"连接"。**
...
3. （可选）将冷路径仪表板发布到 [Power BI online](https://www.powerbi.com/)。 此步骤需要 Power BI 帐户（或 Office 365 帐户）。
   ...
   * 要创建新仪表板，请单击左侧窗格的**+****"仪表板"** 部分旁边的符号。 为新仪表板输入名称“Predictive Maintenance Demo”。
   ...
   * 要计划数据刷新，请将鼠标悬停在 **PredictiveMaintenanceAerospace** 数据集上，单击![省略号图标](./media/predictive-maintenance-technical-guide/icon-elipsis.png)，然后选择“计划刷新”****。
     <br/>
     > [!NOTE]
     > 如果看到警告消息，请单击 **"编辑凭据"，** 并确保数据库凭据与步骤 1 中描述的凭据相同。
     <br/>
...
1. 在 Azure 流分析 (ASA) 中添加 Power BI 输出。
   ...
   * ASA 查询有三个输出，即**飞机监控、****飞机警报**和**飞行时间**。 您可以通过单击查询选项卡来查看查询。对应于每个表，您需要向 ASA 添加输出。 添加第一个输出时 (**aircraftmonitor**)，请确保“输出别名”、“数据集名称”和“表名称”都相同 (**aircraftmonitor**)。************ 重复以上步骤为 **aircraftalert** 和 **flightsbyhour** 添加输出。 将三个输出表全部添加完成并启动 ASA 作业后，应收到一条确认消息（“成功启动流分析作业 maintenancesa02asapbi”）。
...
1. 下面是创建上面的一个磁贴的一些步骤 - "传感器 11 的 Fleet 视图与阈值 48.26"磁贴：
   ...
   * 单击此折线图右上角的 **"针图视觉"** 图标。 可能会显示“固定到仪表板”窗口让你选择仪表板。 选择“预测性维护演示”，然后单击“固定”。
   * 将鼠标悬停在仪表板上的此磁贴上，单击右上角的"编辑"图标将其标题更改为"传感器 11 的 Fleet 视图与阈值 48.26"，并将副标题更改为"跨车队随时间的平均值"。
