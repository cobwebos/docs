...
description: 在 SQL Server 中使用 R、Python 和 T-SQL 进行预测分析的示例。
...
[使用 SQL Server](sql-walkthrough.md)演练显示了如何构建和部署机器学习分类和回归模型。  这些数据是一个公开可用的纽约市出租车旅行和票价数据集。


## <a name="predict-taxi-tips-using-microsoft-r-with-sql-server"></a>在 SQL Server 中使用 Microsoft R 预测出租车小费 

[使用 SQL Server R 服务](https://msdn.microsoft.com/library/mt612857.aspx)演练演示如何构建 R 模型并将其部署到 SQL Server。 本演练旨在向 R 开发人员介绍 R Services（数据库内）。
