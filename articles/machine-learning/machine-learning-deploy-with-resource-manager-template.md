...
通过提供部署带有验证和重试机制的互连组件的可扩展方法，使用 Azure Resource Manager 部署模板可节约时间。 例如，为了设置 Azure 机器学习工作区，需要先配置 Azure 存储帐户，然后部署工作区。 想象为数百个工作区手动执行此操作的样子。 更轻松的替代方法是使用 Azure Resource Manager 模板部署 Azure 机器学习工作区及其所有依赖项。 本文将引导你逐步完成此过程。 有关 Azure Resource Manager 的整体概述，请参阅 [Azure Resource Manager 概述](../azure-resource-manager/resource-group-overview.md)。
