<properties
	pageTitle="使用 Live Connect 对应用进行身份验证 (Windows Phone) | Microsoft Azure"
...
5. 在上述代码段中，请将占位符 `<microsoft-account-client-id>` 替换为你从应用程序的 Microsoft 帐户注册中获取的客户端 ID。

6. 注释掉或删除现有 **OnNavigatedTo** 方法覆盖中对 **RefreshTodoItems** 方法的调用。

	这可以防止在对用户进行身份验证之前加载数据。接下来，将要添加一个按钮用于启动登录过程。

7. 将以下代码段添加到 MainPage 类：
...
8. 在应用程序项目中，打开 MainPage.xaml 项目文件，然后在 **TitlePanel** 中 **TextBlock** 元素的后面添加 **Button** 元素：
