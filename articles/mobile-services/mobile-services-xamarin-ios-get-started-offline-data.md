<properties 
	pageTitle="在移动服务 (Xamarin iOS) 中使用脱机数据 | 移动开发人员中心" 
...
Azure 移动服务脱机同步允许最终用户，当无法访问网络时，与本地数据库交互。若要在你的应用程序中使用这些功能，请将 `MobileServiceClient.SyncContext` 初始化到本地存储。然后，通过 `IMobileServiceSyncTable` 接口引用表。本部分将指导完成脱机同步 `QSTodoService.cs` 中的相关代码。
