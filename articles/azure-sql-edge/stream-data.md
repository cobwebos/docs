...
description: 了解 Azure SQL Edge （预览版）中的数据流。
keywords: ''
services: sql-edge
ms.service: sql-edge
ms.topic: conceptual
...
ms.locfileid: "84669607"
---
# <a name="data-streaming-in-azure-sql-edge-preview"></a>Azure SQL Edge（预览版）中的数据流式处理

Azure SQL Edge （预览版）提供以下选项来实现数据流： 

- 部署在 Azure 中创建的 Azure 流分析 edge 作业。 有关详细信息，请参阅[部署 Azure 流分析作业](deploy-dacpac.md)。
- 使用 T-sql 流式处理在 Azure SQL Edge 中创建流式处理作业，而无需在 Azure 中配置流式处理作业。 

尽管可以使用这两个选项来实现 Azure SQL Edge 中的数据流，但只能使用其中一个选项。 使用这两种方法时，可能会影响数据流式处理操作的运行情况。

T-sql 流式处理是本文的重点。 它提供实时数据流式处理、分析和事件处理，可同时分析和处理来自多个源的大量快速流式处理数据。 T-sql 流式处理是通过使用在 Microsoft Azure 中支持[Azure 流分析](https://docs.microsoft.com/azure/stream-analytics/stream-analytics-introduction)的同一高性能流式处理引擎生成的。 此功能支持一组类似的功能，这些功能由在边缘上运行的 Azure 流分析提供。
