...
---
# <a name="back-up-and-restore-databases-in-azure-sql-edge"></a>备份和还原 Azure SQL Edge 中的数据库 

Azure SQL Edge 构建在最新版本的 Microsoft SQL 数据库引擎上。 它提供的备份和还原数据库功能与在容器中运行 Linux 上的 SQL Server 和 SQL Server 中的功能类似。 备份和还原组件提供了保护存储在 Azure SQL Edge 数据库中的数据的基本保护。 
...
Azure SQL Edge 支持备份到页 blob 和块 blob。 有关详细信息，请参阅 [备份到块 blob 和页 blob](https://docs.microsoft.com/sql/relational-databases/backup-restore/sql-server-backup-to-url#blockbloborpageblob)。 在下面的示例中，将数据库 *IronOreSilicaPrediction* 备份到块 blob。 

1. 若要将备份配置为阻止 blob，请首先生成一个可用于在 Azure SQL Edge 上创建 SQL Server 凭据 (SAS) 令牌的共享访问签名。 此脚本创建与存储访问策略相关联的 SAS。 有关详细信息，请参阅 [共享访问签名，第1部分：了解 SAS 模型](https://azure.microsoft.com/documentation/articles/storage-dotnet-shared-access-signature-part-1/)。 此脚本还编写在 SQL Server 上创建凭据时所需的 T-SQL 命令。 以下脚本假设已有一个 Azure 订阅和一个存储帐户，以及一个用于备份的存储容器。
...
在 Azure SQL Edge 中，可以从本地磁盘、网络位置或 Azure Blob 存储帐户进行还原。 有关 SQL Server 中的还原和恢复的详细信息，请参阅 [还原和恢复概述](https://docs.microsoft.com/sql/relational-databases/backup-restore/restore-and-recovery-overview-sql-server)。 有关 SQL Server 中的简单恢复模式的概述，请参阅 [ (简单恢复模式) 中完成数据库还原 ](https://docs.microsoft.com/sql/relational-databases/backup-restore/complete-database-restores-simple-recovery-model)。

> [!IMPORTANT] 
> 无法在 Microsoft SQL Server 或 Azure SQL 的实例上还原在 Azure SQL Edge 中创建的数据库。 此外，可以在 Azure SQL Edge 上还原在 Microsoft SQL Server 或 Azure SQL 上创建的数据库，前提是该数据库不包含 Azure SQL Edge 不支持的任何功能。 
