...
Azure SQL Edge 容器目前基于 Ubuntu 16.04，因此仅支持在运行 Ubuntu 16.04（建议）或 Ubuntu 18.04 的 Docker 主机上运行。 Azure SQL Edge 还可以在其他操作系统主机上运行，例如，Linux 的其他分发版或在 Windows 上（使用 Docker CE 或 Docker EE），但 Microsoft 未对这些配置进行广泛测试。
