...
确定目标之后，通知中心服务将通知推送到设备平台的“推送通知服务”。 示例包括适用于 Apple 的 Apple Push Notification 服务 (APNs) 和适用于 Google 的 Firebase Cloud Messaging (FCM)。 通知中心推送跨多批注册拆分的通知。 通知中心基于你在 Azure 门户的“配置通知中心”下设置的凭据，向各自的推送通知服务验证身份。 然后，推送通知服务将通知转发到各自的*客户端设备*。
...
若要成功将通知发送到各自的推送通知服务，通知中心服务需要在开发人员的应用程序环境中对自身进行身份验证。 开发人员可在各自的平台 （Google、 Apple、 Windows，等） 创建开发人员帐户。 然后，开发人员向从中获取凭据的平台注册自己的应用程序。
...
若要获取有关针对注册的失败传递尝试的更多错误信息，可以使用通知中心 REST API [按消息遥测：获取通知消息遥测数据](https://msdn.microsoft.com/library/azure/mt608135.aspx)并[PNS 反馈](https://msdn.microsoft.com/library/azure/mt705560.aspx)。 有关示例代码，请参阅[发送 REST 示例](https://github.com/Azure/azure-notificationhubs-samples/tree/master/dotnet/SendRestExample)。
...
由于平台通知服务非常强大，这些通知往往在几秒内就能从推送通知服务到达设备。 如果推送通知服务进行限制，通知中心服务会应用指数回退策略。 如果推送通知服务在 30 分钟内都无法访问，我们在过期并永久删除消息中有一个策略。
...
可查看和管理中心的所有注册，这些注册已经按照平台、本机或模板注册、任意标记、推送通知服务标识符、注册 ID 以及过期日期进行分类。 还可以在此页面中编辑注册。 它是用于编辑标记特别有用。

右键单击你**通知中心**中**服务器资源管理器**，然后选择**诊断**。 

![Visual Studio 的服务器资源管理器-诊断菜单](./media/notification-hubs-diagnosing/diagnose-menu.png)

请参阅以下页面： 

![Visual Studio 的诊断页](./media/notification-hubs-diagnosing/diagnose-page.png)
