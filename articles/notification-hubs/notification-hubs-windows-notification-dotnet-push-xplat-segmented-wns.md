...
本主题演示如何使用 Azure 通知中心将突发新闻通知广播到 Windows 应用商店或 Windows Phone 8.1（非 Silverlight）应用。 如果要以 Windows Phone 8.1 Silverlight 为目标，请参阅 [Windows Phone](notification-hubs-windows-phone-push-xplat-segmented-mpns-notification.md) 版本。 

完成此过程后，可注册感兴趣的突发新闻类别，之后你仅会收到这些类别的推送通知。 这对于许多应用（例如 RSS 阅读器或针对乐迷的应用）很常见，在此方案中，必须将通知发送给以前声明过对其感兴趣的用户组。 

在通知中心创建注册时，可通过加入一个或多个标记来启用广播方案。 将通知发送到标记时，已注册该标记的所有设备将接收通知。 标记只是一些字符串，因此不必提前设置。 有关标记的详细信息，请参阅[通知中心路由和标记表达式](notification-hubs-tags-segment-push-message.md)。
...
本主题以你在[通知中心入门][get-started]中创建的应用为基础。 开始本教程之前，必须先完成[通知中心入门][get-started]中的内容。
...
3. 将以下代码复制到新的 Notifications 类中：
    ...
    由于你可能需要注册多个模板（例如一个模板针对 toast 通知，一个模板针对磁贴），因此请提供模板名称（例如“simpleWNSTemplateExample”）。 为模板命名以便更新或删除模板。
...
1. 在 Visual Studio 中，选择 F5 编译并启动应用。  
...
我们已通过本文了解了如何按类别广播突发新闻。 建议完成以下教程，该教程侧重介绍其他高级通知中心方案：

* [使用通知中心广播本地化的突发新闻]此教程介绍如何将突发新闻应用扩展为允许发送本地化的通知。
