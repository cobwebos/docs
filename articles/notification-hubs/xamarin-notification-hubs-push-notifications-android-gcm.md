...
本教程说明如何使用 Azure 通知中心将推送通知发送到 Xamarin.Android 应用程序。 将创建一个空白 Xamarin.Android 应用，它使用 Google Cloud Messaging (GCM) 接收推送通知。 完成后，能够使用通知中心将推送通知广播到运行应用的所有设备。 [NotificationHubs 应用][GitHub]示例中提供了完成的代码。
...
通知中心现在已配置为使用 GCM，并且你有连接字符串，既可用于注册应用以接收通知，又可用于发送推送通知。

## <a name="connect-your-app-to-the-notification-hub"></a>将应用连接到通知中心
首先，创建新项目。
1. 在 Xamarin Studio 中选择“新建解决方案” > “Android 应用”，然后选择“下一步”。
...
### <a name="add-the-required-components-to-your-project"></a>将所需的组件添加到项目
Xamarin 组件应用商店中提供的 Google Cloud Messaging Client 可以简化在 Xamarin.Android 中支持推送通知的过程。

1. 在 Xamarin.Android 应用中右键单击“组件”文件夹，并选择“获取更多组件”。
...
1. 收集有关 Android 应用和通知中心的以下信息：
   ...
   * **GoogleProjectNumber**：在 Google 开发人员门户上，从应用的概览中获取此项目编号值。 在门户上创建应用时，已事先记下了这个值。
   * **侦听连接字符串**：在 [Azure 经典门户]中的仪表板上，选择“查看连接字符串”。 复制此值的 *DefaultListenSharedAccessSignature* 连接字符串。
   * **中心名称**：这是中心在 [Azure 经典门户]上的名称。 例如 *mynotificationhub2*。
...
3. 将以下 using 语句添加到 **MainActivity.cs**：
        ...
        using Android.Util;
        using Gcm.Client;
4. 将一个实例变量添加到 `MainActivity` 类，用于在运行应用时显示警报对话框。
   
        public static MainActivity instance;
5. 在 **MainActivity** 类中创建以下方法：
...
6. 在 **MainActivity.cs** 的方法 `OnCreate` 中，初始化 `instance` 变量并添加对 `RegisterWithGCM` 的调用：
...
7. 创建新类 **MyBroadcastReceiver**。
   ...
   > 
   > 
8. 将以下 using 语句添加到 **MyBroadcastReceiver.cs**（引用前面添加的组件和程序集）：
...
9. 在 **MyBroadcastReceiver.cs** 中，在 **using** 语句和 **namespace** 声明之间添加以下权限请求：
...
10. 在 **MyBroadcastReceiver.cs** 中，更改 **MyBroadcastReceiver** 类以匹配以下内容：
...
11. 在 **MyBroadcastReceiver.cs** 中添加名为 **PushHandlerService** 的一个类，该类从 **GcmServiceBase** 派生。 确保将 **Service** 属性应用到类：
...
12. **GcmServiceBase** 实现方法 **OnRegistered()**、**OnUnRegistered()**、**OnMessage()**、**OnRecoverableError()** 和 **OnError()**。 我们的 **PushHandlerService** 实现类必须重写这些方法，在响应与通知中心的交互时会激发这些方法。
13. 在 **PushHandlerService** 中使用以下代码重写 **OnRegistered()** 方法：
    ...
    > [!NOTE]
    > 在上述 **OnRegistered()** 代码中，应注意可指定标签来注册特定消息传送通道。
    > 
    > 
14. 在 **PushHandlerService** 中使用以下代码重写 **OnMessage** 方法：
...
15. 将以下 **createNotification** 和 **dialogNotify** 方法添加到 **PushHandlerService**，以便在收到通知时告诉用户。
...
16. 重写抽象成员 **OnUnRegistered()**、**OnRecoverableError()** 和 **OnError()**，以便能够编译代码。
...
> [!IMPORTANT]
> 为了接收推送通知，必须在 Android 虚拟设备上设置 Google 帐户。 （在模拟器中，导航到“设置”，并选择“添加帐户”）。此外，请确保模拟器已连接到 Internet。
> 
> [!NOTE]
...
1. 在“工具”中，依次选择“打开 Android 模拟器管理器”、设备、“编辑”。
   
      ![][18]
2. 在“目标”中选择“Google API”，然后选择“确定”。
   
      ![][19]
3. 在顶部工具栏中选择“运行”，然后选择应用。 这会启动模拟器并运行该应用程序。
...
通常，推送通知是在后端服务（例如，移动服务，或者通过兼容库的 ASP.NET）中发送的。 如果后端没有可用的库，则你也可以使用 REST API 直接发送通知消息。
...
在本教程的后续小节中，将使用 .NET 控制台应用和移动服务通过节点脚本来发送通知。

#### <a name="optional-send-notifications-by-using-a-net-app"></a>（可选）使用 .NET 应用发送通知
在本部分，将使用 .NET 控制台应用发送通知

1. 创建新的 Visual C# 控制台应用程序：
   
      ![][20]
2. 在 Visual Studio 中选择“工具” > “NuGet 包管理器”，然后选择“包管理器控制台”。
...
6. 在 **Main** 方法中添加下列行：
         ...
         Console.ReadLine();
7. 按 F5 键以运行应用。 应会在应用中收到通知。
   
      ![][21]

#### <a name="optional-send-notifications-by-using-a-mobile-service"></a>（可选）使用移动服务发送通知
1. 按照 [移动服务入门]操作。
2. 登录到 [Azure 经典门户]并选择移动服务。
3. 选择顶部的“计划程序”选项卡。
   
      ![][22]
4. 创建新的计划作业，插入名称，并选择“按需”。
   
      ![][23]
5. 创建作业时，选择该作业名称。 然后选择顶部栏上的“脚本”选项卡。
6. 在计划程序函数中插入以下脚本。 确保将占位符替换成先前获取的通知中心名称和 *DefaultFullSharedAccessSignature* 的连接字符串。 选择“保存”。
...
7. 选择底部栏上的“运行一次”。 应接收 toast 通知。

## <a name="next-steps"></a>后续步骤
在这个简单的示例中，将通知广播到所有 Android 设备。 要针对特定客户，请参考教程 [使用通知中心将通知推送到用户]。 如果要按兴趣组划分用户，可以阅读 [使用通知中心发送突发新闻]。 在[通知中心指南]和[针对 Android 的通知中心操作指南]中了解有关如何使用通知中心的详细信息。
