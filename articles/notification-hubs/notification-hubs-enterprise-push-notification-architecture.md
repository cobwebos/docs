...
一个常见需求是在后端系统中发生了用户感兴趣的事件时，通过其移动应用程序向用户发送推送通知。 例如，在 iPhone 具有银行应用的银行客户想要时借方一定从该帐户或在 intranet 方案中，在 Windows Phone 上具有了预算审批应用的财务部门员工希望收到通知 若要接收审批请求时通知你。
