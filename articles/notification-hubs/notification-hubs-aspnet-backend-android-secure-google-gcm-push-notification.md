...
请务必注意，在之前的流程（以及本教程中）中，假设此设备会在用户登录后在本地存储中存储身份验证令牌。 此方法可以保证无缝的体验，因为该设备可以使用此令牌检索通知的安全有效负载。 如果应用程序未在设备上存储身份验证令牌，或者如果这些令牌可能已过期，那么此设备应用程序在收到推送通知时应显示提示用户启动此应用程序的一般性通知。 然后应用会对用户进行身份验证，并显示通知有效负载。
