...
在 Azure 通知中心，中心是最小的资源/实体。 它通常映射到一个应用程序，并且可以为我们支持的应用程序的每个平台通知系统 （PNS） 保留一个证书。 该应用程序可以是混合应用程序，也可以是本机和跨平台应用程序。

**命名空间**是通知中心的集合。 每个命名空间通常包含多个中心，这些中心是相关的，用于特定目的。 例如，您可以分别有三个不同的命名空间来用于开发、测试和生产目的。

您可以将命名空间与**可用**、**基本**或**标准**定价层相关联。 可以将层用于符合要求的命名空间。 以下部分介绍如何更改通知中心命名空间的定价层。

## <a name="use-azure-portal"></a>使用 Azure 门户

使用 Azure 门户时，可以更改命名空间页上或中心页上的命名空间的定价层。 在中心页上更改它时，实际上是在命名空间级别更改它。 它更改命名空间以及命名空间中的所有中心的定价层。

### <a name="change-tier-on-the-namespace-page"></a>在命名空间页上更改层

以下过程演示如何更改命名空间页上的命名空间的定价层。 更改命名空间的层时，它将应用于该命名空间中的所有中心。

1. 登录 [Azure 门户](https://portal.azure.com)。
2. 选择左侧菜单上**的所有服务**。
3. 在“物联网”部分选择“通知中心命名空间”。******** 如果选择文本旁边的星号 （），`*`则它将添加到**FAVORITES**下的左侧导航栏中。 它可以帮助您更快地访问命名空间页面。 将其添加到收藏夹后，选择**通知中心命名空间**。
...
7. 更改定价层，然后单击 **"选择**"按钮。
8. 您可以在**警报**中查看层更改操作的状态。
9. 切换到“概览”页。**** 确认在“基本信息”部分可以看到“定价层”字段的新层。********
10. 此步骤是可选的。 选择命名空间中的任何中心。 确认可以在“基本信息”部分看到同一定价层。**** 您应该看到命名空间中所有中心的相同定价层。

### <a name="change-tier-on-the-hub-page"></a>在中心页上更改层

以下过程演示如何更改中心页上的命名空间的定价层。 即使是从中心页开始执行这些步骤，但实际上也会更改命名空间以及命名空间中的所有中心的定价层。
