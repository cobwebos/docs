...
Apple 现在要求开发人员通过 APNS API 中的新 `apns-push-type` 标头将通知标识为警报或背景通知。 按照[Apple 的文档](https://developer.apple.com/documentation/usernotifications/setting_up_a_remote_notification_server/sending_notification_requests_to_apns)： "此标头的值必须准确反映通知的有效负载的内容。 如果存在不匹配的情况，或者如果所需系统上缺少标头，Apn 可能会返回错误，延迟通知的传递，或将其全部删除。 "
