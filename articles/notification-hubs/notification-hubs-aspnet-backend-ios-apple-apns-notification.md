...
2. 在 Main.storyboard 中添加对象库中的组件，如下面的屏幕截图中所示。
	...
	+ **WNS**：标签和开关，用于已在中心设置 Windows 通知服务时，启用将通知发送到 Windows 通知服务。请参阅 [Windows 入门](notification-hubs-windows-store-dotnet-get-started)教程。
	+ **GCM**：标签和开关，用于已在中心设置 Google Cloud Messaging 时，启用将通知发送到 Google Cloud Messaging。请参阅 [Android 入门](notification-hubs-android-get-started.md)教程。
