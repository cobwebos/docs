...
* **捆绑 ID**：也称为“应用 ID”。 这是应用程序的捆绑标识符；例如 `com.example.myapp`。 请注意，可对多个应用使用一个密钥。 此值在发送通知时将映射到 `apns-topic` HTTP 标头，并用于将特定应用程序指定为目标。 请注意，不能显式设置的值 `apns-topic` 。
