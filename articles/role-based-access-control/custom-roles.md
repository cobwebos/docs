...
自定义角色存储在 Azure Active Directory (Azure AD) 目录中，可以在订阅之间共享。 至多可将每个目录**5000**自定义角色。 （对于专用云，例如 Azure 政府版、 Azure Germany 和 Azure 中国 21Vianet 限制为 2000年自定义角色。）可以使用 Azure PowerShell、Azure CLI 或 REST API 创建自定义角色。
