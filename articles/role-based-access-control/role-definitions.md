...
以前，基于角色的访问控制不用于数据操作。 数据操作的授权根据资源提供程序的不同而异。 用于管理操作的基于角色的访问控制授权模型已扩展到数据操作。
...
下面是[存储 Blob 数据读取器](built-in-roles.md#storage-blob-data-reader)角色定义，其中包括 `Actions` 和 `DataActions` 属性中的操作。 使用此角色可以读取 Blob 容器以及基础 Blob 数据。
...
为了更好地了解管理和数据操作的工作原理，让我们考虑一个具体的示例。 在订阅范围为 Alice 分配了[所有者](built-in-roles.md#owner)角色。 Bob 在存储帐户范围内分配了 "[存储 Blob 数据参与者](built-in-roles.md#storage-blob-data-contributor)" 角色。 下图演示了此示例。
...
由于 Alice 在订阅范围内有一个通配符（`*`）操作，因此其权限会向下继承，使其能够执行所有管理操作。 Alice 可以读取、写入和删除容器。 但是，Alice 在不采取其他步骤的情况下无法执行数据操作。 例如，默认情况下，Alice 无法读取容器内的 blob。 若要读取 blob，Alice 必须检索存储访问密钥并使用它们来访问 blob。

Bob 的权限仅限于[存储 Blob 数据参与者](built-in-roles.md#storage-blob-data-contributor)角色中指定的 `Actions` 和 `DataActions`。 Bob 可以基于角色执行管理和数据操作。 例如，Bob 可以读取、写入和删除指定存储帐户中的容器，还可以读取、写入和删除 blob。
