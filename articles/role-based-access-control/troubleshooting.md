...
- 如果尝试创建新的自定义角色时收到错误消息“角色定义超限。 无法创建更多的角色定义(代码:RoleDefinitionLimitExceeded)”，请删除未在使用的任何自定义角色。 Azure 支持最大**5000**租户中的自定义角色。 （Azure 政府、Azure 德国、Azure 中国世纪互联等专用云的限制为 2000 个自定义角色。）
...
## <a name="role-assignments-without-a-security-principal"></a>而无需安全主体的角色分配

列出您使用 Azure PowerShell 的角色分配时, 可能会看到一个空的分配`DisplayName`和一个`ObjectType`设置为未知。 例如， [Get AzRoleAssignment](/powershell/module/az.resources/get-azroleassignment)返回类似于以下的角色分配：
...
同样，列出时使用 Azure CLI 将角色分配，你可能会看到一个空的分配`principalName`。 例如， [az 角色分配列表](/cli/azure/role/assignment#az-role-assignment-list)返回类似于以下的角色分配：
...
将角色分配给安全主体 （用户、 组、 服务主体或托管的标识） 和更高版本删除该安全主体时，会发生这些角色分配。 这些角色分配不会显示在 Azure 门户中，并且它不会使其保持一个问题。 但是，如果你愿意，您可以删除这些角色分配。

若要删除这些角色分配，请使用[删除 AzRoleAssignment](/powershell/module/az.resources/remove-azroleassignment)或[az 角色分配删除](/cli/azure/role/assignment#az-role-assignment-delete)命令。

在 PowerShell 中，如果你尝试删除使用的对象 ID 和角色定义名称和多个角色分配的角色分配中的匹配你的参数，你将收到错误消息："提供的信息不映射到角色分配"。 下面显示的错误消息的示例：
...
如果收到此错误消息，请确保还指定`-Scope`或`-ResourceGroupName`参数。
