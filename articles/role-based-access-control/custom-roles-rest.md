...
如果[Azure 资源的内置角色](built-in-roles.md)不能满足组织的特定需求，则可以创建自己的自定义角色。 本文介绍如何使用 REST API 列出、创建、更新或删除自定义角色。
...
1. 将 *{角色定义 Id}* 替换为自定义角色的 GUID 标识符。

1. 在请求正文中，将 *[角色定义 Id]* 替换为 GUID 标识符。

1. 如果是`assignableScopes`订阅或资源组，请将 *[订阅 Id]* 或 *[资源组]* 实例替换为标识符。

1. 如果是`assignableScopes`管理组，请将 *{groupId}* 实例替换为管理组标识符。 将管理组添加到`assignableScopes`当前处于预览状态。
