...
| [Avere 运算符](#avere-operator) | 由 Avere vFXT 群集用来管理群集 |
| [Azure 事件中心数据所有者](#azure-event-hubs-data-owner) | 允许对 Azure 事件中心资源进行完全访问。 |
| [Azure 事件中心数据接收方](#azure-event-hubs-data-receiver) | 允许接收对 Azure 事件中心资源的访问权限。 |
| [Azure 事件中心数据发送者](#azure-event-hubs-data-sender) | 允许发送对 Azure 事件中心资源的访问权限。 |
...
| [Azure Maps 数据读取器（预览）](#azure-maps-data-reader-preview) | 授予对 Azure maps 帐户中读取映射相关数据的访问权限。 |
...
| [Azure 服务总线数据所有者](#azure-service-bus-data-owner) | 允许对 Azure 服务总线资源进行完全访问。 |
| [Azure 服务总线数据接收方](#azure-service-bus-data-receiver) | 允许接收对 Azure 服务总线资源的访问权限。 |
| [Azure 服务总线数据发送者](#azure-service-bus-data-sender) | 允许发送对 Azure 服务总线资源的访问权限。 |
| [Azure Stack 注册所有者](#azure-stack-registration-owner) | 允许管理 Azure Stack 注册。 |
| [备份参与者](#backup-contributor) | 允许你管理备份服务，但无法创建保管库并向其他人授予访问权限 |
...
| [认知服务数据读取器（预览）](#cognitive-services-data-reader-preview) | 允许你读取认知服务数据。 |
...
| [Cosmos DB 运算符](#cosmos-db-operator) | 允许你管理 Azure Cosmos DB 帐户，但不能访问其中的数据。 阻止访问帐户密钥和连接字符串。 |
...
| [逻辑应用参与者](#logic-app-contributor) | 允许你管理逻辑应用，但不能更改它们的访问权限。 |
| [逻辑应用操作员](#logic-app-operator) | 允许读取、启用和禁用逻辑应用，但不允许对其进行编辑或更新。 |
...
| [空间锚定帐户参与者](#spatial-anchors-account-contributor) | 允许你管理帐户中的空间锚，但不能删除它们 |
| [空间锚定帐户所有者](#spatial-anchors-account-owner) | 允许你管理帐户中的空间锚，包括删除它们 |
| [空间锚定帐户读取器](#spatial-anchors-account-reader) | 允许您在帐户中查找和读取空间锚的属性 |
| [SQL DB 参与者](#sql-db-contributor) | 允许管理 SQL 数据库，但不允许访问这些数据库。 此外，不允许管理其安全相关的策略或其父 SQL 服务器。 |
| [SQL 托管实例参与者](#sql-managed-instance-contributor) | 允许你管理 SQL 托管实例和所需的网络配置，但不能向其他人提供访问权限。 |
...
| [存储帐户参与者](#storage-account-contributor) | 允许管理存储帐户。 提供对帐户密钥的访问权限，它可用于通过共享密钥授权访问数据。 |
| [存储帐户密钥操作员服务角色](#storage-account-key-operator-service-role) | 允许列出和重新生成存储帐户访问密钥。 |
| [存储 Blob 数据参与者](#storage-blob-data-contributor) | 读取、写入和删除 Azure 存储容器和 blob。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
| [存储 Blob 数据所有者](#storage-blob-data-owner) | 提供对 Azure 存储 blob 容器和数据的完全访问权限，包括分配 POSIX 访问控制。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
| [存储 Blob 数据读者](#storage-blob-data-reader) | 阅读并列出 Azure 存储容器和 blob。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
| [存储队列数据参与者](#storage-queue-data-contributor) | 读取、写入和删除 Azure 存储队列和队列消息。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
| [存储队列数据消息处理程序](#storage-queue-data-message-processor) | 查看、检索和删除 Azure 存储队列中的消息。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
| [存储队列数据消息发送程序](#storage-queue-data-message-sender) | 将消息添加到 Azure 存储队列。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
| [存储队列数据读取器](#storage-queue-data-reader) | 阅读并列出 Azure 存储队列和队列消息。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
> | **操作** |  |
> | Microsoft.containerregistry/注册表/项目/删除 | 删除容器注册表中的项目。 |
> | **不操作** |  |
> | *无* |  |
> | **DataActions** |  |
> | *无* |  |
...
> | Microsoft.Network/virtualNetworks/subnets/joinViaServiceEndpoint/action | 将存储帐户或 SQL 数据库等资源加入到子网。 Not 可报警。 |
...
> | **操作** |  |
> | 区块链/blockchainMembers/transactionNodes/read | 获取或列出现有的区块链成员事务节点。 |
> | **不操作** |  |
> | *无* |  |
> | **DataActions** |  |
> | 区块链/blockchainMembers/transactionNodes/connect/action | 连接到区块链成员事务节点。 |
> | **NotDataActions** |  |
> | *无* |  |
...
## <a name="cognitive-services-data-reader-preview"></a>认知服务数据读取器（预览）
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
> | **说明** | 允许你读取认知服务数据。 |
...
> | | |
> | --- | --- |
> | **说明** | 允许你管理 Azure Cosmos DB 帐户，但不能访问其中的数据。 阻止访问帐户密钥和连接字符串。 |
...
> | Databox/位置/validateAddress/action | 验证送货地址，并提供备用地址（如有）。 |
> | Microsoft.ResourceHealth/availabilityStatuses/read | 获取指定范围内所有资源的可用性状态 |
> | Microsoft.Support/* | 创建和管理支持票证 |
> | **不操作** |  |
> | *无* |  |
...
> | Microsoft.devtestlab/实验室/ensureCurrentUserProfile/action | 确保当前用户在实验室中有有效的配置文件。 |
...
> | Microsoft.Network/loadBalancers/backendAddressPools/join/action | 加入负载均衡器后端地址池。 Not 可报警。 |
> | Microsoft.Network/loadBalancers/inboundNatRules/join/action | 加入负载均衡器入站 nat 规则。 Not 可报警。 |
> | Microsoft.Network/networkInterfaces/*/read | 读取网络接口（例如，此网络接口所属的所有负载均衡器）的属性 |
> | Microsoft.Network/networkInterfaces/join/action | 将虚拟机加入到网络接口。 Not 可报警。 |
> | Microsoft.Network/networkInterfaces/read | 获取网络接口定义。  |
> | Microsoft.Network/networkInterfaces/write | 创建网络接口，或更新现有的网络接口。  |
> | Microsoft.Network/publicIPAddresses/*/read | 读取公共 IP 地址的属性 |
> | Microsoft.Network/publicIPAddresses/join/action | 加入公共 ip 地址。 Not 可报警。 |
...
> | Microsoft HDInsight/群集/getGatewaySettings/操作 | 获取 HDInsight 群集的网关设置 |
> | Microsoft HDInsight/群集/updateGatewaySettings/操作 | 更新 HDInsight 群集的网关设置 |
> | Microsoft HDInsight/群集/配置/* |  |
> | Microsoft.Resources/subscriptions/resourceGroups/read | 获取或列出资源组。 |
> | Microsoft.Resources/deployments/operations/read | 获取或列出部署操作。 |
> | Microsoft.Insights/alertRules/* | 创建和管理 Insights 警报规则 |
> | Microsoft.Authorization/*/read | 读取角色和角色分配 |
...
> | | |
> | --- | --- |
> | **说明** | 允许读取、启用和禁用逻辑应用，但不允许对其进行编辑或更新。 |
...
## <a name="spatial-anchors-account-contributor"></a>空间锚定帐户参与者
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
> | **说明** | 允许你管理帐户中的空间锚，但不能删除它们 |
...
> | *无* |  |
> | **DataActions** |  |
> | MixedReality/SpatialAnchorsAccounts/create/action | 创建空间锚 |
> | MixedReality/SpatialAnchorsAccounts/discovery/read | 发现附近的空间锚 |
> | MixedReality/SpatialAnchorsAccounts/properties/read | 获取空间定位点的属性 |
> | MixedReality/SpatialAnchorsAccounts/query/read | 定位空间锚 |
> | MixedReality/SpatialAnchorsAccounts/submitdiag/read | 提交诊断数据以帮助改进 Azure 空间锚定服务的质量 |
> | MixedReality/SpatialAnchorsAccounts/write | 更新空间锚属性 |
> | **NotDataActions** |  |
> | *无* |  |

## <a name="spatial-anchors-account-owner"></a>空间锚定帐户所有者
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
> | **说明** | 允许你管理帐户中的空间锚，包括删除它们 |
...
> | **DataActions** |  |
> | MixedReality/SpatialAnchorsAccounts/create/action | 创建空间锚 |
> | MixedReality/SpatialAnchorsAccounts/delete | 删除空间锚 |
> | MixedReality/SpatialAnchorsAccounts/discovery/read | 发现附近的空间锚 |
> | MixedReality/SpatialAnchorsAccounts/properties/read | 获取空间定位点的属性 |
> | MixedReality/SpatialAnchorsAccounts/query/read | 定位空间锚 |
> | MixedReality/SpatialAnchorsAccounts/submitdiag/read | 提交诊断数据以帮助改进 Azure 空间锚定服务的质量 |
...
## <a name="spatial-anchors-account-reader"></a>空间锚定帐户读取器
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
> | **说明** | 允许您在帐户中查找和读取空间锚的属性 |
...
> | Microsoft.Insights/metricDefinitions/read | 读取指标定义 |
> | **不操作** |  |
> | ManagedInstances/currentSensitivityLabels/* |  |
> | ManagedInstances/recommendedSensitivityLabels/* |  |
> | ManagedInstances/数据库/架构/表/列/sensitivityLabels/* |  |
> | ManagedInstances/securityAlertPolicies/* |  |
> | ManagedInstances/sensitivityLabels/* |  |
...
> | | |
> | --- | --- |
> | **说明** | 允许你管理 SQL 托管实例和所需的网络配置，但不能向其他人提供访问权限。 |
...
> | | |
> | --- | --- |
> | **说明** | 允许管理存储帐户。 提供对帐户密钥的访问权限，它可用于通过共享密钥授权访问数据。 |
...
> | | |
> | --- | --- |
> | **说明** | 读取、写入和删除 Azure 存储容器和 blob。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
> | Microsoft.Storage/storageAccounts/blobServices/containers/blobs/read | 返回 blob 或 blob 列表。 |
> | Microsoft.Storage/storageAccounts/blobServices/containers/blobs/write | 写入 blob。 |
> | **NotDataActions** |  |
> | *无* |  |
...
> | | |
> | --- | --- |
> | **说明** | 提供对 Azure 存储 blob 容器和数据的完全访问权限，包括分配 POSIX 访问控制。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
> | | |
> | --- | --- |
> | **说明** | 阅读并列出 Azure 存储容器和 blob。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
> | *无* |  |
> | **DataActions** |  |
> | StorageAccounts/fileServices/文件共享/files/read | 返回文件/文件夹或文件/文件夹的列表。 |
> | StorageAccounts/fileServices/文件共享/files/write | 返回写入文件或创建文件夹的结果。 |
> | StorageAccounts/fileServices/文件共享/files/delete | 返回文件/文件夹的删除结果。 |
> | **NotDataActions** |  |
> | *无* |  |

## <a name="storage-file-data-smb-share-elevated-contributor"></a>存储文件数据 SMB 共享提升的参与者
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
...
> | StorageAccounts/fileServices/文件共享/files/write | 返回写入文件或创建文件夹的结果。 |
> | StorageAccounts/fileServices/文件共享/files/delete | 返回文件/文件夹的删除结果。 |
> | StorageAccounts/fileServices/文件共享/files/modifypermissions/action | 返回对文件/文件夹的修改权限的结果。 |
> | **NotDataActions** |  |
> | *无* |  |

## <a name="storage-file-data-smb-share-reader"></a>存储文件数据 SMB 共享读取器
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
...
> | | |
> | --- | --- |
> | **说明** | 读取、写入和删除 Azure 存储队列和队列消息。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
> | *无* |  |
> | **DataActions** |  |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/delete | 从队列中删除一条或多条消息。 |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/read | 查看或检索队列中的一条或多条消息。 |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/write | 向队列中添加消息。 |
> | **NotDataActions** |  |
> | *无* |  |

## <a name="storage-queue-data-message-processor"></a>存储队列数据消息处理程序
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
> | **说明** | 查看、检索和删除 Azure 存储队列中的消息。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
## <a name="storage-queue-data-message-sender"></a>存储队列数据消息发送程序
> [!div class="mx-tableFixed"]
> | | |
> | --- | --- |
> | **说明** | 将消息添加到 Azure 存储队列。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
> | *无* |  |
> | **DataActions** |  |
> | Microsoft.Storage/storageAccounts/queueServices/queues/messages/add/action | 向队列中添加消息。 |
> | **NotDataActions** |  |
> | *无* |  |
...
> | | |
> | --- | --- |
> | **说明** | 阅读并列出 Azure 存储队列和队列消息。 若要了解给定数据操作所需的操作，请参阅[调用 blob 和队列数据操作的权限](https://docs.microsoft.com/rest/api/storageservices/authenticate-with-azure-active-directory#permissions-for-calling-blob-and-queue-data-operations)。 |
...
> | Microsoft.Network/loadBalancers/backendAddressPools/join/action | 加入负载均衡器后端地址池。 Not 可报警。 |
> | Microsoft.Network/loadBalancers/inboundNatPools/join/action | 加入负载均衡器入站 NAT 池。 Not 可报警。 |
> | Microsoft.Network/loadBalancers/inboundNatRules/join/action | 加入负载均衡器入站 nat 规则。 Not 可报警。 |
> | Microsoft.Network/loadBalancers/probes/join/action | 允许使用负载均衡器的探测。 例如，使用此权限，VM 规模集的 healthProbe 属性可以引用探测。 Not 可报警。 |
> | Microsoft.Network/loadBalancers/read | 获取负载均衡器定义 |
...
> | System.web/hostingEnvironments/Join/Action | 联接应用服务环境 |
> | **不操作** |  |
> | *无* |  |
> | **DataActions** |  |
> | *无* |  |
