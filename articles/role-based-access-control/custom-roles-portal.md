---
title: 使用 Azure 门户（预览版）创建或更新 Azure 自定义角色-Azure RBAC
...
---
# <a name="create-or-update-azure-custom-roles-using-the-azure-portal-preview"></a>使用 Azure 门户（预览版）创建或更新 Azure 自定义角色

> [!IMPORTANT]
> 使用 Azure 门户的 Azure 自定义角色目前为公共预览版。
> 此预览版在提供时没有附带服务级别协议，不建议将其用于生产工作负荷。 某些功能可能不受支持或者受限。
> 有关详细信息，请参阅 [Microsoft Azure 预览版补充使用条款](https://azure.microsoft.com/support/legal/preview-supplemental-terms/)。

如果[Azure 内置角色](built-in-roles.md)不能满足组织的特定需求，则可以创建自己的 azure 自定义角色。 与内置角色一样，可以将自定义角色分配到订阅和资源组范围内的用户、组和服务主体。 自定义角色存储在 Azure Active Directory (Azure AD) 目录中，可以在订阅之间共享。 每个目录最多可包含5000个自定义角色。 可以使用 Azure 门户、Azure PowerShell、Azure CLI 或 REST API 创建自定义角色。 本文介绍如何使用 Azure 门户（当前为预览版）创建自定义角色。
...
1. 在 JSON 文件中，指定各个属性的值。 下面是添加了一些值的示例。 有关不同属性的信息，请参阅[了解角色定义](role-definitions.md)。
...
根据你选择的启动方式，你可能具有权限列表中的通配符（\*）的权限。 通配符（\*）将权限扩展到与您提供的字符串匹配的所有内容。 例如，假设你要添加与 Azure 成本管理和导出相关的所有权限。 你可以添加所有这些权限：
...
排除某个权限后，它将添加为 `NotActions` 或 `NotDataActions`。 有效的管理权限是通过添加所有 `Actions`，然后减去所有 `NotActions`来计算的。 通过添加所有 `DataActions`，然后减去所有 `NotDataActions`来计算有效的数据权限。
...
在 "可分配的**作用域**" 选项卡上，指定自定义角色可用于分配的位置，例如订阅或资源组。 根据您选择的启动方式，此选项卡可能会列出打开 "访问控制（IAM）" 页的范围。 不支持将可分配范围设置为根范围（"/"）。 对于此预览版，你无法将管理组添加为可分配的作用域。
