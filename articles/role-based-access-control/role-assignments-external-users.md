...
然后，管理员用户需要添加外部用户的电子邮件地址。 预期的行为不在现有租户中显示外部用户。 邀请外部用户后，它们将下可见**订阅 > 访问控制 (IAM)** 的当前分配 RBAC 角色的订阅范围内的所有当前用户。
...
由于在组织外部，新用户在“默认租户 Azure”目录中没有任何现有的属性。 外部用户同意后，将创建这些记录中记录与订阅相关联的目录分配到角色。
