---
title: 为 Azure 资源创建或更新自定义角色 Azure PowerShell
description: 了解如何使用 Azure PowerShell 使用 Azure 资源的基于角色的访问控制（RBAC）列出、创建、更新或删除自定义角色。
...
若要列出自定义角色定义，请使用[AzRoleDefinition](/powershell/module/az.resources/get-azroledefinition)。 这与用于内置角色的命令相同。
...
以下示例显示创建“虚拟机操作员”自定义角色的另一种方式。 首先，创建一个新 `PSRoleDefinition` 对象。 在 `perms` 变量中指定操作，然后将操作设置为 `Actions` 属性。 通过从 `NotActions`虚拟机参与者`NotActions`内置角色读取 [ 设置 ](built-in-roles.md#virtual-machine-contributor) 属性。 由于[虚拟机参与者](built-in-roles.md#virtual-machine-contributor)没有任何 `NotActions`，因此不需要此行，但它显示了从另一个角色检索信息的方式。
