---
title: "服务管理模式下的 Azure CLI 命令 | Microsoft Docs"
...
若要在命令行中查看当前的命令语法和选项，请键入 `azure help`；若要显示特定命令的帮助，请键入 `azure help [command]`。 还可以在创建和管理具体 Azure 服务的说明文档中找到 CLI 示例。
...
## <a name="setting-asm-mode"></a>设置 asm 模式
...
## <a name="manage-your-account-information-and-publish-settings"></a>管理帐户信息和发布设置
CLI 可以连接到你的帐户的一种方式是使用你的 Azure 订阅信息。 （有关其他选项，请参阅 [Connect to an Azure subscription from the Azure CLI](xplat-cli-connect.md)（从 Azure CLI 连接到 Azure 订阅）。）可以从 Azure 经典门户中的发布设置文件中获取此信息，如下所述。 可以导入发布设置文件作为永久性本地配置设置，CLI 会将此设置用于后续操作。 你只需导入你的发布设置一次。
...
此命令导入 publishsettings 文件或证书，以便可在将来的会话中由工具使用。
...
此命令删除已导入的存储的发布设置。 如果在此计算机上用完了该工具，并且希望确保在将来的会话中，不能通过自己的帐户使用该工具，请使用此命令。
...
### <a name="commands-to-manage-your-affinity-groups"></a>管理地缘组的命令
...
### <a name="commands-to-manage-your-account-environment"></a>管理帐户环境的命令
...
## <a name="commands-to-manage-your-classic-virtual-machines"></a>用于管理经典虚拟机的命令
...
此命令创建 Azure 虚拟机。 默认情况下，创建的每个虚拟机 (VM) 位于其自身的云服务中。 可以使用此处所述的 -c 选项指定将虚拟机添加到现有云服务。

vm create 命令与 Azure 经典门户一样，只会在生产部署环境中创建虚拟机。 目前没有用于在云服务的过渡部署环境中创建虚拟机的选项。 如果你的订阅没有现有的 Azure 存储帐户，此命令将创建一个。
...
如果计划使用 SSH 来管理部署的 Linux 虚拟机（通常都是如此），则必须在创建虚拟机时通过 -e 选项启用 SSH。 在创建该虚拟机后无法启用 SSH。
...
**-w, --virtual-network-name** 指定要在其中添加新虚拟机的虚拟网络。 可从 Azure 经典门户设置和管理虚拟网络。<br />
...
## <a name="commands-to-manage-your-azure-virtual-machine-endpoints"></a>用于管理 Azure 虚拟机终结点的命令
...
## <a name="commands-to-manage-your-azure-virtual-machine-images"></a>用于管理 Azure 虚拟机映像的命令
虚拟机映像是所捕获的、可根据需要进行复制的已配置虚拟机。

**vm image list [options]**

此命令获取虚拟机映像的列表。 有三种类型的映像：Microsoft 创建的映像（以“MSFT”作为前缀）、第三方创建的映像（以供应商的名称作为前缀）以及创建的映像。 若要创建映像，你可以捕获现有虚拟机或从上载到 Blob 存储的自定义 .vhd 创建映像。 有关使用自定义 .vhd 的更多信息，请参见 VM 映像创建。
...
> [!NOTE]
> 目前，此命令仅支持上载固定的 .vhd 文件。 若要上载动态 .vhd 文件，请使用[适用于 Go 的 Azure VHD 实用工具](https://github.com/Microsoft/azure-vhd-utils-for-go)。
> 
> 

某些系统会施加每进程文件描述符限制。 如果超出此限制，工具将显示文件描述符限制错误。 可以使用 -p &lt;number> 参数再次运行该命令，减少最大并行上载数。 默认的最大并行上载数为 96。
...
此命令上载和注册 Azure 磁盘。 必须指定 --blob-url、--location 或 --affinity-group。 如果将此命令与 [source-path] 结合使用，将上载指定的 .vhd 文件并创建映像。 然后你可以使用 vm disk attach 将此映像附加到虚拟机。
...
此命令将数据磁盘附加到 Azure 虚拟机。 在此示例中，20 是要附加的新磁盘的大小（以 GB 为单位）。 你可以选择使用 Blob URL 作为显式指定要创建的目标 Blob 的最后一个参数。 如果不指定 Blob URL，将自动生成一个 Blob 对象。
...
此命令将数据磁盘与 Azure 虚拟机分离。 &lt;lun> 用于标识要分离的磁盘。 要在分离某个磁盘之前获取与该磁盘关联的磁盘的列表，请使用 &lt;vm-name>。

    ~$ azure vm disk detach my-vm 2
    info:   Executing command vm disk detach
    info:   vm disk detach command OK

## <a name="commands-to-manage-your-azure-cloud-services"></a>用于管理 Azure 云服务的命令
...
## <a name="commands-to-manage-your-azure-certificates"></a>用于管理 Azure 证书的命令
Azure 服务证书是连接到你的 Azure 帐户的 SSL 证书。 有关 Azure 证书的详细信息，请参阅 [Manage Certificates](http://msdn.microsoft.com/library/azure/gg981929.aspx)（管理证书）。
...
## <a name="commands-to-manage-your-web-apps"></a>用于管理 Web 应用的命令
...
### <a name="commands-to-manage-your-web-app-application-settings"></a>用于管理 Web 应用应用程序设置的命令
...
### <a name="commands-to-manage-your-web-app-certificates"></a>用于管理 Web 应用证书的命令
...
### <a name="commands-to-manage-your-web-app-connection-strings"></a>用于管理 Web 应用连接字符串的命令
...
### <a name="commands-to-manage-your-web-app-default-documents"></a>用于管理 Web 应用默认文档的命令
...
### <a name="commands-to-manage-your-web-app-deployments"></a>用于管理 Web 应用部署的命令
...
### <a name="commands-to-manage-your-web-app-domains"></a>用于管理 Web 应用域的命令
...
### <a name="commands-to-manage-your-web-app-handler-mappings"></a>用于管理 Web 应用处理程序映射的命令
...
### <a name="commands-to-manage-your-web-jobs"></a>用于管理 Web 作业的命令
...
### <a name="commands-to-manage-your-web-jobs-history"></a>用于管理 Web 作业历史记录的命令
...
### <a name="commands-to-manage-your-web-app-diagnostics"></a>用于管理 Web 应用诊断的命令
...
### <a name="commands-to-manage-your-web-app-repositories"></a>用于管理 Web 应用存储库的命令
...
### <a name="commands-to-manage-your-web-app-scaling"></a>用于管理 Web 应用缩放的命令
**site scale mode [options] &lt;mode> [name]**

**site scale instances [options] &lt;instances> [name]**

## <a name="commands-to-manage-azure-mobile-services"></a>用于管理 Azure 移动服务的命令
...
### <a name="a-namemobileservicesacommands-to-manage-mobile-service-instances"></a><a name="Mobile_Services"></a>用于管理移动服务实例的命令
...
* **--sqlLocation `<location>``<location>`：在特定的 ** 中创建 SQL 服务器；默认为移动服务的位置。
...
> [!NOTE]
> 当重新生成密钥时，使用旧密钥的客户端可能无法访问你的移动服务。 当重新生成应用程序密钥时，应使用新密钥值更新你的应用程序。
> 
> 
...
### <a name="a-namemobileconfigurationacommands-to-manage-mobile-service-configuration"></a><a name="Mobile_Configuration"></a>用于管理移动服务配置的命令
...
### <a name="a-namemobiletablesacommands-to-manage-mobile-service-tables"></a><a name="Mobile_Tables"></a>用于管理移动服务表的命令
...
### <a name="a-namemobilescriptsacommands-to-manage-scripts"></a><a name="Mobile_Scripts"></a>用于管理脚本的命令
本部分中的命令用于管理属于移动服务的服务器脚本。 有关详细信息，请参阅 [Work with server scripts in Mobile Services](https://github.com/Azure/azure-mobile-services/blob/master/docs/mobile-services-how-to-use-server-scripts.md)（使用移动服务中的服务器脚本）。
...
该文件的名称必须由表和操作名称组成。 它必须位于相对于命令执行位置的 table 子文件夹中。 也可以使用 **-f `<file>`** 或 **--file `<file>`** 参数指定其他包含要注册的脚本的文件的文件名和路径。
...
### <a name="a-namemobilejobsacommands-to-manage-scheduled-jobs"></a><a name="Mobile_Jobs"></a>用于管理已计划作业的命令
本部分中的命令用于管理属于移动服务的已计划作业。 有关详细信息，请参阅 [Schedule jobs](http://msdn.microsoft.com/library/windowsazure/jj860528.aspx)（计划作业）。
...
> [!NOTE]
> 创建的新作业处于禁用状态，因为还必须上载脚本。 使用 **mobile script upload** 命令上载脚本，使用 **mobile job update** 命令启用作业。
> 
> 
...
### <a name="a-namemobilescaleacommands-to-scale-a-mobile-service"></a><a name="Mobile_Scale"></a>用于缩放移动服务的命令
本部分中的命令用于缩放移动服务。 有关详细信息，请参阅 [Scaling a mobile service](http://msdn.microsoft.com/library/windowsazure/jj193178.aspx)（缩放移动服务）。
...
### <a name="commands-to-enable-preview-features-for-your-mobile-service"></a>用于为移动服务启用预览版功能的命令
...
此命令为移动服务启用指定的预览版功能。 一旦启用，便无法为移动服务禁用预览版功能。

### <a name="commands-to-manage-your-mobile-service-apis"></a>用于管理移动服务 API 的命令
...
### <a name="commands-to-manage-your-mobile-application-app-settings"></a>用于管理移动应用程序的应用程序设置的命令
...
## <a name="manage-tool-local-settings"></a>管理工具本地设置
...
## <a name="commands-to-manage-service-bus"></a>用于管理 Service Bus 的命令
...
## <a name="commands-to-manage-your-storage-objects"></a>用于管理存储对象的命令
### <a name="commands-to-manage-your-storage-accounts"></a>用于管理存储帐户的命令
...
### <a name="commands-to-manage-your-storage-account-keys"></a>用于管理存储帐户密钥的命令
...
### <a name="commands-to-manage-your-storage-container"></a>用于管理存储容器的命令
...
### <a name="commands-to-manage-your-storage-blob"></a>用于管理存储 blob 的命令
...
## <a name="commands-to-manage-sql-databases"></a>用于管理 SQL 数据库的命令
使用这些命令来管理你的 Azure SQL 数据库

### <a name="commands-to-manage-sql-servers"></a>用于管理 SQL Server 的数据库
...
### <a name="commands-to-manage-your-sql-server-firewall-rules"></a>管理 SQL Server 防火墙规则的命令
...
## <a name="commands-to-manage-your-virtual-networks"></a>用于管理虚拟网络的命令
使用这些命令来管理你的虚拟网络
...
若要进行高级网络配置，请在本地导出你的网络配置。 导出的网络配置包括 DNS 服务器设置、虚拟网络设置、本地网络站点设置和其他设置。
