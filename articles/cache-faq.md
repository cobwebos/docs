...
可以在 [Azure 门户](https://manage.windowsazure.cn)中监视 Microsoft Azure Redis 缓存实例。你可以查看度量值、将度量值图表固定到启动面板、自定义监视图表的日期和时间范围、在图表中添加和删除度量值，以及设置符合特定条件时发出的警报。借助这些工具，你可以监视 Azure Redis 缓存实例的运行状况，以及管理缓存应用程序。有关监视缓存的详细信息，请参阅[监视 Azure Redis 缓存](https://msdn.microsoft.com/zh-cn/library/azure/dn763945.aspx)。
...
-	[使用 Azure Redis 缓存的常见缓存模式](cache-howto-common-cache-patterns)
