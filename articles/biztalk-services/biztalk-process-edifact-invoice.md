...
* **suspended** - 所有挂起的消息将路由到此队列。 如果消息在处理期间失败，则会被挂起。
...
3. 在此屏幕中，输入服务总线 ACS 命名空间、颁发者名称和颁发者密钥。
   ...
   ![][2]  
4. 将出现消息框提示：会在服务总线命名空间中创建三个队列。 单击“确定”。
...
2. 在新的合作伙伴页中，输入“Contoso”作为合作伙伴名称，并单击“保存”。
...
2. 在新协议的“常规设置”页上，指定值，如下图所示，并单击“继续”。
...
1. 在 Visual Studio 解决方案中，展开 InvoiceProcessingBridge 项目，并打开“MessageFlowItinerary.bcs”文件。
...
4. 在“消息类型”框中，单击加号 (**+**) 按钮指定传入消息的架构。 由于 EAI 桥的传入消息始终是内部发票，因此将其设置为“INHOUSEINVOICE”。
   ...
   ![][8]  
5. 单击“Xml 转换”图形，并在属性框中，单击“映射”属性的省略号 (**...**) 按钮。 在“映射选择”对话框中，选择“INHOUSEINVOICE_to_D93AINVOIC”转换文件，并单击“确定”。
...
2. 在 Visual Studio 解决方案资源管理器中，右键单击“InvoiceProcessingBridge”**项目，并单击“部署”**。
3. 如图所示提供值，并单击“部署”。 单击“连接信息”，可从 BizTalk 服务仪表板中获取 BizTalk 服务的 ACS 凭据。
...
3. 在下个窗口中，提供 BizTalk 服务订阅的 ACS 凭据及在其中部署 EAI 和 EDI（接收）桥的终结点。
   ...
   ![][12]  
4. 在下个窗口中，单击“Contoso”下的“发送内部发票”按钮。 在文件对话框中，打开 INHOUSEINVOICE.txt 文件。 检查文件的内容，并单击“确定”发送该发票。
   
   ![][13]  
5. 几秒钟后，Northwind 接收到该发票。 单击“查看消息”链接，查看 Northwind 接收的发票。 请注意，Northwind 接收的发票采用标准 EDIFACT 架构，而 Contoso 发送的发票采用内部架构。
   
   ![][14]  
6. 选择该发票，并单击“发送回单”。 请注意，弹出对话框显示接收的发票与发送的回单中交换 ID 相同。 在“发送回单”对话框中单击“确定”。
...
2. 对于批次名称，输入“InvoiceBatch”、提供说明，并单击“下一步”。
...
4. 指定批次释放条件。 从下拉框中，选择“MessageCountBased”并针对“计数”指定“3”。 这意味着，将向 Northwind 发送包含三条消息的一个批次。 单击“下一步”。
   ...
   ![][18]  
5. 查看摘要，并单击“保存”。 单击“部署”以重新部署协议。
6. 返回“教程客户端”，单击“发送内部发票”，并按照提示发送发票。 会发现 Northwind 未收到发票，因为不满足批次大小。 再重复此步骤两次，以确保向 Northwind 发送三条发票消息。 这样便可满足 3 条消息的批次释放条件，现在应可在 Northwind 处看到发票。
