...
在本教程中，我们将使用 Azure Redis 缓存更新 [Azure Redis 缓存的 ASP.NET 快速入门](cache-web-app-howto.md)中创建的 *ContosoTeamStats* ASP.NET Web 应用，以包含使用[缓存端模式](https://docs.microsoft.com/azure/architecture/patterns/cache-aside)的排行榜。 该示例应用程序显示数据库中的团队统计信息列表，并演示如何通过不同的方法使用 Azure Redis 缓存在缓存中存储和检索数据，以提高性能。 完成本教程后，将有一个运行的 Web 应用，该应用可以对数据库执行读写操作，已通过 Azure Redis 缓存进行优化，并且托管在 Azure 中。
