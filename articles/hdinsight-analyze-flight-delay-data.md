...
# 使用 HDInsight 中的 Hadoop 分析航班延误数据

Hive 提供了通过类似 SQL 的脚本语言（称作 *[HiveQL][hadoop-hiveql]*）运行 MapReduce 作业的方法，此方法可用于对大量数据进行汇总、查询和分析。 

Azure HDInsight 的主要优势之一就是隔离数据存储和计算。HDInsight 将 Azure Blob 存储用于数据存储。普通的 MapReduce 进程可分成 3 个部分：

1. **将数据存储在 Azure Blob 存储中。**这可能是连续进程。例如，将天气数据、传感器数据、Web 日志以及此案例中的航班延误数据存储到 Blob 存储中。
2. **运行作业。**在处理数据时，你可以运行 PowerShell 脚本（或客户端应用程序）来设置 HDInsight 群集、运行作业，然后删除群集。作业会将输出数据存储到 Azure Blob 存储。即使在删除群集之后，输出数据仍会保留。因此，你只需要对已耗用的部分付费。
3. **从 Blob 存储检索输出**，或在此教程中将数据导出到 Azure SQL Database。
...
- 运行 Sqoop 作业将 Hive 作业输出导出至 Azure SQL Database。
- 删除 HDInsight 群集。 

在附录中，你可以找到有关上载航班延误数据、创建/上载 Hive 查询字符串和针对 Sqoop 作业准备 Azure SQL Database 的说明。
...
* Azure 订阅。有关获取订阅的详细信息，请参阅[购买选项][azure-purchase-options]、[会员优惠][azure-member-offers]或[免费试用][azure-free-trial]。

### 了解 HDInsight 存储

HDInsight 中的 Hadoop 群集将 Azure Blob 存储用于数据存储。有关详细信息，请参阅[将 Azure Blob 存储与 HDInsight 配合使用][hdinsight-storage]。 

设置 HDInsight 群集时，请将 Azure 存储帐户的某个 Blob 存储容器指定为默认文件系统，就像在 Hadoop 分布式文件系统 (HDFS) 中一样。此存储帐户称为 *默认存储帐户*，Blob 容器称为 *默认 Blob 容器*或 *默认容器*。默认存储帐户必须与 HDInsight 群集位于同一个数据中心。删除 HDInsight 群集不会删除默认容器或默认存储帐户。

除了默认存储帐户，在设置期间，其他 Azure 存储帐户也可以绑定到 HDInsight 群集。这种绑定会将存储帐户和存储帐户密钥添加到配置文件，因此，群集可以在运行时访问这些存储帐户。有关添加其他存储帐户的详细信息，请参阅[在 HDInsight 中设置 Hadoop 群集][hdinsight-provision]。 

Azure Blob 存储语法为：

	wasb[s]://<ContainerName>@<StorageAccountName>.blob.core.chinacloudapi.cn/<path>/<filename>

>[AZURE.NOTE] Blob 存储路径是虚拟路径。有关详细信息，请参阅[将 Azure Blob 存储与 HDInsight 配合使用][hdinsight-storage]。 

存储在默认容器中的文件可以使用以下任一 URI 从 HDInsight 进行访问（以 flightdelays.hql 为例）：

	wasb://mycontainer@mystorageaccount.blob.core.chinacloudapi.cn/tutorials/flightdelays/flightdelays.hql
	wasb:///tutorials/flightdelays/flightdelays.hql
	/tutorials/flightdelays/flightdelays.hql

若要直接从存储帐户访问文件，文件的 Blob 名称为：
...
本教程将使用来自[美国研究与技术创新管理部门 (RITA) - 运输统计局][rita-website]的航班准时表现数据。数据已上载至具有"公用"Blob 访问权限的 Azure Blob 存储容器。因为它是公用 Blob 容器，你不需要将此存储帐户绑定到运行 Hive 脚本的 HDInsight 群集。HiveQL 脚本也会上载到相同的 Blob 容器。如果想要了解如何将数据放入/上载到你自己的存储帐户，以及如何创建/上载 HiveQL 脚本文件，请参阅[附录 A](#appendix-a) 和[附录 B](#appendix-b)。
...
<tr><td>wasb://flightdelay@hditutorialdata.blob.core.chinacloudapi.cn/2013Data</td><td>Hive 作业的输入的数据。这些数据已上载到具有公共访问权限的 Azure Blob 存储帐户。<a href="#appendix-a">附录 A</a> 提供了有关获取数据和将数据上载到你自己的 Azure Blob 存储帐户的说明。</td></tr>
...
有关详细信息，请参阅 [HDInsight：Hive 内部表和外部表简][cindygross-hive-tables]。

> [AZURE.NOTE] HiveQL 语句之一可创建 Hive 外部表。Hive 外部表将数据文件保留在原始位置。Hive 内部表将数据文件移到 hive\warehouse。Hive 内部表要求数据文件位于默认容器中。对于存储在默认 Blob 容器之外的数据，你必须使用 Hive 外部表。
...
Hadoop MapReduce 属于批处理。运行 Hive 作业时，最具成本效益的方法是设置作业的群集，并在作业完成之后删除作业。以下脚本覆盖了整个过程。有关设置 HDInsight 群集和运行 Hive 作业的详细信息，请参阅[在 HDInsight 中设置 Hadoop 群集][hdinsight-provision]和[将 Hive 与 HDInsight 配合使用][hdinsight-use-hive]。 

**使用 Windows PowerShell 运行 Hive 查询**

1. 根据[附录 C](#appendix-c) 中的说明，为 Sqoop 作业输出创建 Azure SQL Database 和表。
2. 准备参数：
	...
	<tr><td>$sqlDatabaseServerName</td><td>Azure SQL Database 服务器名称。必须是现有的服务器。有关创建服务器的信息，请参阅<a href="#appendix-c">附录 C</a>。</td></tr>
	<tr><td>$sqlDatabaseUsername</td><td>Azure SQL Database 服务器登录名。</td></tr>
	<tr><td>$sqlDatabasePassword</td><td>Azure SQL Database 服务器登录密码。</td></tr>
	<tr><td>$sqlDatabaseName</td><td>Sqoop 将数据导出到的 SQL Database。默认名称是 HDISqoop。Sqooop 作业输出的表名称为 AvgDelays。 </td></tr>
	</table>
...
6. 连接到 SQL Database，并在 AvgDelays 表中按城市查看平均航班延迟：
...
上载数据文件和 HiveQL 脚本文件（请参阅[附录 B](#appendix-b)）需要进行规划。思路是在设置 HDInsight 群集之前存储数据文件和 HiveQL 文件，并运行 Hive 作业。可以使用两个选项：

- **使用 HDInsight 群集将使用做为默认文件系统的相同 Azure 存储帐户。**因为 HDInsight 群集将会有存储帐户访问密钥，你不需要进行任何其他更改。
- **使用与 HDInsight 群集默认文件系统不同的 Azure 存储帐户。**如果存在这种情况，你必须修改[设置 HDInsight 群集和运行 Hive/Sqoop 作业](#runjob)中的 Windows PowerShell 脚本的设置部分， 以将存储帐户包含为额外的存储帐户。有关说明，请参阅[在 HDInsight 中设置 Hadoop 群集][hdinsight-provision]。这样，HDInsight 群集就会知道存储帐户的访问密钥。
...
2. 在该页面上，选择以下值：
	...
	<tr><td>字段</td><td>*Year*, *FlightDate*, *UniqueCarrier*, *Carrier*, *FlightNum*, *OriginAirportID*, *Origin*, *OriginCityName*, *OriginState*, *DestAirportID*, *Dest*, *DestCityName*, *DestState*, *DepDelayMinutes*, *ArrDelay*, *ArrDelayMinutes*, *CarrierDelay*, *WeatherDelay*, *NASDelay*, *SecurityDelay*, *LateAircraftDelay*（清除所有其他字段）</td></tr>
	</table>

3. 单击"下载"。 
4. 将文件提取到 **C:\Tutorials\FlightDelays\Data** 文件夹中。每个文件均为 CSV 文件且大小约为 60 GB。
...
路径 tutorials/flightdelays/data 是你在上载文件时创建的虚拟文件夹。验证是否有 12 个文件，每个月对应一个文件。
...
5. **计算平均天气延迟，并按城市名对结果进行分组。**它还会将结果输出到 Blob 存储。请注意，该查询将从数据中删除撇号，并且将排除其中的 **weather_delay** 值为 null 的行。这是必需操作，因为默认情况下，Sqoop（在本教程后面部分使用）不会适当地处理这些值。
...
3. 将以下脚本复制并粘贴到脚本窗格中：
	...
	- **$hqlLocalFileName** - 脚本会先将 HiveQL 脚本文件保存在本地，然后才上载到 Blob 存储。这是文件名。默认值为 <u>C:\tutorials\flightdelays\flightdelays.hql</u>。
	- **$hqlBlobName** - 这是 Azure Blob 存储中使用的 HiveQL 脚本文件 Blob 名称。默认值为 tutorials/flightdelays/flightdelays.hql。因为文件会直接写入 Azure Blob 存储，所以 Blob 名称的开头不是"/"。如果你要从 Blob 存储访问文件，必须在文件名的开头添加"/"。
	- **$srcDataFolder** 和 **$dstDataFolder** - = "tutorials/flightdelays/data" 
 = "tutorials/flightdelays/output"


---
## <a id="appendix-c"></a>附录 C - 针对 Sqoop 作业输出准备 Azure SQL Database
...
1. 准备参数：
	...
	<table border="1">
	<tr><th>变量名</th><th>说明</th></tr>
	<tr><td>$sqlDatabaseServerName</td><td>Azure SQL Database 服务器的名称。不输入任何值会创建新的服务器。</td></tr>
	<tr><td>$sqlDatabaseUsername</td><td>Azure SQL Database 服务器登录名。如果 $sqlDatabaseServerName 是现有的服务器，登录名和登录密码将用来向服务器进行身份验证。否则会创建新的服务器。</td></tr>
	<tr><td>$sqlDatabasePassword</td><td>Azure SQL Database 服务器登录密码。</td></tr>
...
3. 将以下脚本复制并粘贴到脚本窗格中：
	...
	>[AZURE.NOTE] 脚本使用具象状态传输 (REST) 服务 http://bot.whatismyipaddress.com 来检索外部 IP 地址。IP 地址用于创建 SQL Database 服务器的防火墙规则。  

	该脚本中使用的某些变量：

	- **$ipAddressRestService** - 默认值为 http://bot.whatismyipaddress.com。这是用来获取外部 IP 地址的公共 IP 地址 REST 服务。如果需要，你可以使用其他服务。使用此服务检索的外部 IP 地址将用于创建 Azure SQL Database 服务器的防火墙规则，使你能够从工作站访问数据库（通过 Windows PowerShell 脚本）。
	- **$fireWallRuleName** - 这是 Azure SQL Database 服务器防火墙规则名称。默认名称为 <u>FlightDelay</u>。如果需要，你可以将它重命名。
	- **$sqlDatabaseMaxSizeGB** - 只有在创建新的 Azure SQL Database 服务器时才会使用此值。默认值为 10GB。10GB 对于本教程来说已足够。
...
现在你已了解如何执行以下操作：将文件上载到 Azure Blob 存储、使用 Azure Blob 存储中的数据填充 Hive 表、运行 Hive 查询以及使用 Sqoop 将数据从 HDFS 导出到 Azure SQL Database。若要了解更多信息，请参阅以下文章：
