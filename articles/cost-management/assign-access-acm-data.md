...
目前，Azure 成本管理具有有限的支持进行跨租户身份验证。 在某些情况下尝试跨租户进行身份验证时可能会收到**访问被拒绝**中成本分析错误。 如果配置基于角色的访问控制 (RBAC) 到另一个租户的订阅，然后尝试查看成本数据，则可能会出现此问题。

*若要解决此问题*:配置跨租户 RBAC 后，等待一个小时。 然后，尝试在成本分析中查看成本或授予这两个租户中的用户对成本管理访问权限。  
