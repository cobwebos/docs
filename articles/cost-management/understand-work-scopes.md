...
- [**成本管理参与者**](../role-based-access-control/built-in-roles.md#cost-management-contributor) -可以查看成本和管理成本配置。
- [**成本管理读取者**](../role-based-access-control/built-in-roles.md#cost-management-reader) – 可以查看成本数据和配置。
...
- **查看节约成本的建议**– 成本管理访问者和参与者不默认情况下有权访问的建议。 建议访问需要读取访问权限的单个资源。 请考虑授予[读取器](../role-based-access-control/built-in-roles.md#reader)或[特定于服务的角色](../role-based-access-control/built-in-roles.md#built-in-role-descriptions)。
