...
如果未指定前缀，则默认前缀是为报表指定的名称。 日期范围具有 `/report-name/date-range/` 格式。
12. 对于**时间单位**，选择 "**每小时**"。
...
7. 在 "**外部 id**" 中，输入在 AWS 角色与 Azure 成本管理之间共享密码的外部 id。 在成本管理中的**新连接器**页上也使用同一外部 ID。 Microsoft 建议在输入外部 ID 时使用强密码策略。
...
1. 输入**成本和使用情况报告**。
2. 选择 > **读取** > **DescribeReportDefinitions**的**访问级别**。 此步骤允许成本管理读取当前定义的报表，并确定它们是否与报表定义必备项匹配。
3. 选择 "**添加其他权限**"。
...
1. 选择 "**选择服务**"。
2. 输入 " **S3**"。
3.  > **列表**中选择 "**访问级别**" > **ListBucket**。 此操作获取 S3 存储桶中的对象的列表。
...
1. 输入**组织**。
2.  > **列表**中选择 "**访问级别**" > **ListAccounts**。 此操作获取帐户的名称。
...
5. 在 "**创建 AWS 连接器**" 页上的 "**显示名称**" 中，输入连接器的名称。  
    创建 AWS 连接器 ![页面的示例](./media/aws-integration-setup-configure/create-aws-connector01.png)
6. 还可以选择默认管理组。 它将存储所有发现的链接帐户。 稍后可以进行设置。
7. 如果你想要确保在预览版过期时连续操作，请在 "**计费**" 部分选择 "**在公开上市时自动向1% 收费"** 。 如果选择 "自动" 选项，则必须选择计费订阅。
...
创建连接器后，建议为其分配访问控制。 向用户分配新发现的作用域的权限： AWS 合并的帐户和 AWS 链接的帐户。 创建连接器的用户是连接器的所有者、合并的帐户以及所有链接的帐户。
