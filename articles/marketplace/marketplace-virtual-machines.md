...
ms.locfileid: "82858312"
---
# <a name="publishing-guide-for-virtual-machine-offers"></a>虚拟机产品/服务的发布指南

发布虚拟机（VM）映像是将解决方案发布到 Azure Marketplace 的主要方法之一。 使用本指南来了解此类产品/服务的要求。 

虚拟机产品/服务是通过 Azure Marketplace 部署并计费的事务产品。 用户看到的对操作的调用*现在立即获得*。

## <a name="free-trial"></a>免费试用 

若要安排用户测试您的产品/服务，请在使用自带许可证（BYOL）计费模式时访问有限条款的软件许可证。 

## <a name="test-drive"></a>体验版

可以通过基础结构即服务（IaaS）或软件即服务（SaaS）应用部署一个或多个虚拟机。 "*测试驱动器*发布" 选项的优点是通过合作伙伴托管的指导教程自动设置虚拟机或整个解决方案。 使用测试驱动器，你的客户可以评估 Vm，而无需额外付费。 客户无需成为现有的 Azure 客户便可与试用体验合作。 

若要开始，请在[testdrive](mailto:amp-testdrive@microsoft.com)上通过电子邮件联系我们。 
...
将虚拟设备部署到与客户关联的订阅时，请使用*虚拟机*产品/服务类型。 Vm 使用即用即付或自带许可（BYOL）许可模式完全启用了商务。 Microsoft 托管商业事务并代你向客户计费。 可获得的好处是能够利用客户与 Microsoft 之间达成的首选付费关系（包括任何企业协议）。
...
| 与 Azure 兼容的虚拟硬盘 (VHD) | 必须基于 Windows 或 Linux 构建 VM。 有关创建 VHD 的详细信息，请参阅： <ul> <li>[Azure 上的 linux 分发](https://docs.microsoft.com/azure/virtual-machines/linux/endorsed-distros)版（适用于 linux vhd）。</li> <li>[创建与 Azure 兼容的 VHD](./partner-center-portal/azure-vm-create-offer.md) （适用于 Windows vhd）。</li> </ul> |  
