...
第一个也是最重要的字段是切换是否要显示表单的"测试"，并显示所有必需的字段，以便您填写。选择 **"否"时，** 窗体将禁用，如果禁用"测试驱动器"重新发布，则"测试驱动器"将从生产中删除。
...
1. 单击**访问控制 （IAM）。**
1. 单击 **"角色分配**"选项卡。 ![Azure 活动目录，添加新的访问控制主体](./media/azure-resource-manager-test-drive/SetupSub7_1.jpg)
1. 单击 **"添加角色分配**"。
1. 将角色设置为“参与者”****。
...
> [!CAUTION]
> 不能使用 Azure 应用注册预览，因为它当前不生成 base64 编码密钥。
