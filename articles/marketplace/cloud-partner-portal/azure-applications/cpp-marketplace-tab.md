...
ms.locfileid: "73827562"
---
# <a name="azure-application-marketplace-tab"></a>Azure 应用程序的“市场”选项卡

使用“市场”选项卡来介绍 Azure 应用程序并提供营销资产。 此选项卡包含以下形式：概述、营销项目、潜在客户管理和法律。
...
| **市场营销标识符\*** | 与此套餐关联的唯一 URL，通常包含组织名称和解决方案名称，最大长度为 50 个字符。 为服务选择一个短且易记的营销标识符。 此项将用在此套餐的市场 URL 中。 例如，如果你的发布者 ID 是 "contoso"，并且你的营销标识符是 "Sampleapp.exe"，则 Azure Marketplace 中的产品/服务的 URL 将 https://azuremarketplace.microsoft.com/marketplace/apps/contoso.sampleApp  
| **预览订阅 Id\*** | 为预览器添加 1 到 100 个订阅标识符。 这些由允许列表订阅的订阅将有权访问你的产品/服务，但在发布后，它将在发布前提供预览版。          |
...
| **中型\***       | 中徽标：90x90 像素（PNG 格式）    |
| **大型\***        | 大徽标：115x115 像素（PNG 格式）   |
| **宽\***         | 宽徽标：255x115 像素（PNG 格式）    |
| **主图**           | 可选的英雄徽标： PNG 格式的815x290 像素。 **注意：** 上传特大图标后，无法将其删除。 |
...
| **添加视频**      | 可选，视频显示在你的产品详细信息页上。 它们能够十分直观地展示应用程序的作用及其工作原理。 若要添加视频，请执行以下操作： <ul><li>选择“+ 添加视频”，打开“视频”窗口</li><li>**名称** - 输入名称/标题（最大长度为 100 字符）。</li><li>**链接**-输入承载视频的网站的 URL （YouTube 或 Vimeo）</li><li>**缩略**图-上传缩略图。 图像必须为 PNG 格式，大小为 533 x 324 像素。</li></ul>          |
|  |  |
...
下图显示了客户在搜索结果中单击产品/服务的磁贴后，该产品/服务在 Marketplace 中的显示方式。
...
>[!TIP]
>选择信息图标以查看此消息： "选择将在其中存储潜在顾客的系统。 [在此处](https://docs.microsoft.com/azure/marketplace/cloud-partner-portal-orig/cloud-partner-portal-get-customer-leads)了解如何连接到 CRM 系统。
