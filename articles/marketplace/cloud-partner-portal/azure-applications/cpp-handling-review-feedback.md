---
title: 处理 Azure 应用程序评审反馈 |Azure Marketplace
...
![通过 Azure DevOps 反馈发布步骤](./media/pub-flow-vsts-access.png)
...
1. 收集以下信息：
    ...
    - "拉取请求" 链接，例如：可以从通知消息或401响应页的地址检索此 URL `https://solutiontemplates.visualstudio.com/marketplacesolutions/_git/contoso/pullrequest/<number>`。
...
5. 在“步骤 2/2”页上，指定事件标题（例如 **），并提供在上述第一个步骤中收集的信息。** `Request Azure DevOps access`  阅读并接受协议，然后选择“提交”。
...
2. 可有可无在窗口右侧的 "**策略**" 部分中，单击 "问题" 消息（在本示例中为 "**策略验证失败**"）以调查问题的低级详细信息，包括关联的日志文件。  错误通常显示在日志文件的底部。
