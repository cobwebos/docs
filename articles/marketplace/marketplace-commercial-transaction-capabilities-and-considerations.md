---
title: Microsoft 商用 marketplace 功能
description: 本文介绍了 Microsoft 商业应用商店事务处理选项的定价、计费、开票和支出注意事项。
author: dsindona
...
具有现有商业功能的发布者可以选择 "列表"、"试用" 和 "自带许可证" （BYOL）发布选项来进行促销和用户获取。 借助这些选项，Microsoft 不会直接参与发布者的软件许可证事务，也不会产生关联的事务费用。 发布者负责支持软件许可证交易的各个方面，包括但不限于：订单、履行、计量、计费、开具发票、付款和费用收集。 借助列表和试用发布选项，发布者保留 100% 的收集自客户的发布者软件许可费。

### <a name="transact-publishing-option"></a>交易发布选项

"事务发布" 选项利用了 Microsoft 商业功能，并提供了从发现和评估到购买和实现的端到端体验。 针对现有的 Microsoft 订阅或信用卡对事务/服务进行计费，使 Microsoft 可以代表发布者来托管云 marketplace 交易。
...
* 订阅定价：软件许可费用以每月或每年的形式显示，按固定费率或按座位计费。 这仅适用于 SaaS 应用和 Azure 应用程序-托管的应用。
...
Microsoft 使客户能够订购、许可和使用出版商软件，经受 Microsoft 的商业市场和发布者的最终用户许可协议的条款和条件。 发布者必须提供最终用户许可协议，或在创建产品/服务时选择[标准合同](./standard-contract.md)。
...
*    如果为 SaaS 服务启用了 "通过 Microsoft 销售" 选项，则会得到以下成本结构。
