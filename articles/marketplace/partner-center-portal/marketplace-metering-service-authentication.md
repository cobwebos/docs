...
适用的产品/服务类型是采用托管应用程序计划类型的 SaaS 和 Azure 应用程序。  

使用用于身份验证的预定义固定应用程序 ID 提交自定义计量器。
...
|  **属性名称**  |  **必需**  |  **说明**          |
|  ------------------ |--------------- | ------------------------  |
|  `Grant_type`       |   True         | 授权类型。 默认值是 `client_credentials`。 |
|  `Client_id`        |   True         | 与 Azure AD 应用关联的客户端/应用标识符。|
|  `client_secret`    |   True         | 与 Azure AD 应用关联的密码。  |
|  `Resource`         |   True         | 为其请求令牌的目标资源。 默认值是 `20e940b3-4c77-4b0b-9a53-9e16a1b010a7`。  |
| | | |
...
托管应用程序可以包含从虚拟机到 Azure Functions 在内的各种类型的资源。  有关如何使用各种服务的托管标识进行身份验证的详细信息，请参阅[如何使用 Azure 资源的托管标识](https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview#how-can-i-use-managed-identities-for-azure-resources)。
