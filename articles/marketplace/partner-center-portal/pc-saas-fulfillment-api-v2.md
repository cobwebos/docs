...
商业市场通过最终客户购买 SaaS 订阅后，管理其整个生命周期。  它使用登录页、履单、操作 Api 和 webhook 作为一种机制来驱动实际的 SaaS 订阅激活和使用情况、更新和订阅取消。  最终客户的帐单基于 Microsoft 维护的 SaaS 订阅的状态。 
...
最终客户 (或 CSP) 购买 marketplace 中的 SaaS 产品/服务后，应通知该发布者购买，以便在发布方端为最终用户创建和配置新的 SaaS 帐户。

若要创建帐户，请执行以下操作：

1. 客户需要单击 " **配置** " 按钮，该按钮在 Microsoft AppSource 或 Azure 门户成功购买后可用于 SaaS 产品/服务。 或者在购买后，客户很快就会收到的电子邮件中。
2. 然后，在新的浏览器选项卡中打开 "登录" 页 URL，并使用令牌参数 (商业 marketplace 购买标识令牌) ，Microsoft 将通知合作伙伴。
...
> [!NOTE]
>如果 SSO 登录要求管理员授予对应用的权限，则合作伙伴中心提供的产品/服务的说明必须公开管理员级别的访问权限。 这是为了符合 [商业应用商店认证策略](https://docs.microsoft.com/legal/marketplace/certification-policies#10003-authentication-options)。
...
#### <a name="active-subscribed"></a>Active (订阅的) 
...
此操作表示 Microsoft 和发布者已处理现有活动 SaaS 订阅的更新。 可以通过以下方式启动此类更新：

- 商业应用商店中的客户。
- 来自商业应用商店的 CSP。
- 来自发布者的 SaaS 网站 (的客户不会应用于 CSP) 购买。
...
在订阅期限结束时 () 一个月或一年后，Microsoft 将自动续订 SaaS 订阅。  对于所有 SaaS 订阅，自动续订设置的默认值均为 *true* 。 将继续用定期节奏续订活动 SaaS 订阅。 续订订阅后，Microsoft 不会通知发布者。 客户可以通过 M365 管理门户或 Azure 门户关闭 SaaS 订阅的自动续订。  在这种情况下，将在当前帐单期限结束时自动取消 SaaS 订阅。  客户还可以在任何时间点取消 SaaS 订阅。
...
通过解析终结点，发布者可以将 marketplace 购买标识令牌交换 (称为已[购买但尚未激活](#purchased-but-not-yet-activated-pendingfulfillmentstart)的*令牌*) 到持续购买的 SaaS 订阅 ID 及其详细信息。
...
将为 SaaS 订阅购买的现有计划更新为 (公有或私有) 的新计划。  当发布服务器上为 marketplace 购买的 SaaS 订阅更改了计划时，发布服务器必须调用此 API。

只能为活动订阅调用此 API。  任何计划都可以更改为任何其他现有计划 (公有或私有) ，但不能更改为其自身。  对于私有计划，必须在合作伙伴中心的计划受众中定义客户的租户。
...
>[!Note]
>Webhook url 服务必须全天候启动并运行，并且随时可以接收来自 Microsoft 时间的新呼叫。  Microsoft 对 webhook 调用重试策略 (500) 重试超过8小时的重试，但如果发布者不接受调用并返回响应，则该 webhook 通知的操作最终会在 Microsoft 端失败。
...
*更改计划*、 *更改数量*和 *取消订阅* 操作都是从发布方测试的。  从 Microsoft 开始，可以从 Azure 门户和管理中心 (门户中触发 *取消订阅* ，Microsoft AppSource 购买) 。  只能从管理中心触发*更改数量和计划*。
