...
**默认容器创建选项**。 容器创建选项直接用于创建 IoT 边缘模块 Docker 容器。 IoT 边缘支持 Docker 引擎 API 创建容器选项。 查看[List 容器](https://docs.docker.com/engine/api/v1.30/#operation/ContainerList)中的所有选项。 创建选项字段必须有效 JSON、非转义字符和少于 512 个字符。
