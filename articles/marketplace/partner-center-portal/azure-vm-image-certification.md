...
> [!NOTE]
> 我们将 Azure VM 产品的管理从云合作伙伴门户迁移到合作伙伴中心。 在迁移产品/服务之前，请继续按照云合作伙伴门户中[为 Azure 密钥保管库创建证书](https://docs.microsoft.com/azure/marketplace/cloud-partner-portal/virtual-machine/cpp-create-key-vault-cert)的说明进行管理产品/服务。

本文介绍如何在商业市场中测试和提交虚拟机 （VM） 映像，以确保它满足最新的 Azure 应用商店发布要求。

在提交 VM 产品/服务之前完成以下步骤：

1. 创建和部署证书。
2. 使用通用映像部署 Azure VM。
3. 运行验证。

## <a name="create-and-deploy-certificates-for-azure-key-vault"></a>为 Azure 密钥保管库创建和部署证书

本节介绍如何创建和部署将 Windows 远程管理 （WinRM） 连接设置为 Azure 托管虚拟机所需的自签名证书。
...
| $vmName | 计划 Azure 虚拟机的名称。 |
| $certname | 证书的名称;必须匹配计划 VM 的完全限定域名。 |
| $certpassword | 证书的密码必须与计划 VM 使用的密码匹配。 |
| | |
...
> [!TIP]
> 在这些步骤期间保持相同的 Azure PowerShell 控制台会话打开并运行，以保留各种参数的值。

> [!WARNING]
> 如果保存此脚本，则仅将其保存在安全位置，因为它包含安全信息（密码）。

#### <a name="create-the-azure-key-vault-to-store-the-certificate"></a>创建 Azure 密钥保管库以存储证书

将下面的模板内容复制到本地计算机上的文件。 在下面的示例脚本中，此资源为`C:\certLocation\keyvault.json`。
...
本节介绍如何部署通用 VHD 映像以创建新的 Azure VM 资源。 对于此过程，我们将使用提供的 Azure 资源管理器模板和 Azure PowerShell 脚本。

### <a name="prepare-an-azure-resource-manager-template"></a>准备 Azure 资源管理器模板

将以下 Azure 资源管理器模板进行 VHD 部署复制到名为 VHDtoImage.json 的本地文件。 下一个脚本将请求本地计算机上的位置使用此 JSON。
...
| sNameForPublicIP | 公共 IP 的 DNS 名称;必须小写。 |
...
复制和编辑以下脚本以提供 和`$storageaccount``$vhdUrl`变量的值。 执行它，从现有通用 VHD 创建 Azure VM 资源。
...
Azure 认证测试工具在本地 Windows 计算机上运行，但测试基于 Azure 的 Windows 或 Linux VM。 它证明用户 VM 映像可以与 Microsoft Azure 一起使用，并且满足有关准备 VHD 的指南和要求。 该工具的输出是兼容性报告，您将上载到合作伙伴中心门户以请求 VM 认证。

1. 下载并安装最新的[“Azure 认证”的认证测试工具](https://www.microsoft.com/download/details.aspx?id=44299)。
2. 打开认证工具，然后选择 **"开始新测试**"。
3. 在“测试信息”屏幕中，为测试运行输入**测试名称**。****
4. 选择 VM**的平台**，Windows 服务器或 Linux。 所选的平台会影响剩余的选项。
5. 如果 VM 正在使用此数据库服务，请选择 **"Azure SQL 数据库测试"** 复选框。
...
2. 如果使用基于密码的身份验证，请输入 VM **DNS 名称**、**用户名**和**密码**的值。 您还可以更改默认**的 SSH 端口**号。

    ![Azure 认证测试工具，Linux VM 映像的密码身份验证](media/avm-cert2.png)

3. 如果使用基于密钥文件的身份验证，请输入“VM DNS 名称”、“用户名”和“私钥”位置的值。************ 您还可以包含**密码**或更改默认**SSH 端口**号。
...
在认证工具中为 VM 映像指定参数值后，选择 **"测试连接**"以创建与 VM 的有效连接。 验证连接后，选择“下一步”**** 启动测试。 测试完成后，测试结果将显示在表中。 "状态"列显示每个测试的（通过/失败/警告）。 如有任何测试失败，则不会认证该映像。__ 在这种情况下，请查看要求和失败消息，进行建议的更改，然后再次运行测试。

自动测试完成后，在 **"常规****评估和****内核自定义**"屏幕的两个选项卡上提供有关 VM 映像的其他信息，然后选择 **"下一步**"。

最后一个屏幕允许您提供更多信息，例如 Linux VM 映像的 SSH 访问信息，以及查找异常时任何失败评估的说明。

最后，选择 **"生成报告"** 以下载已执行测试用例的测试结果和日志文件以及对调查表的回答。 将结果保存在与 VHD 相同的容器中。
