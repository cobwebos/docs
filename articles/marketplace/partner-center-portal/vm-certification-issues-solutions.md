...
ms.locfileid: "85958525"
---
# <a name="virtual-machine-certification---issues-and-solutions"></a>虚拟机认证 - 问题和解决方案

将虚拟机（VM）映像发布到 Azure Marketplace 时，Azure 团队会验证 VM 映像，确保其 bootability、安全性和 Azure 兼容性。 如果任何高质量的测试失败，则发布将失败并出现一条包含错误的消息。

本文说明了 VM 映像的常见错误消息。 它还讨论相关解决方案：
...
当你使用属于另一发布服务器的基本映像并且已更新该映像时，将发生此错误。 在这种情况下，你将不能发布映像。
...
5. 请在 VM 的 " **vm 扩展**" 选项卡上，验证**Linux 诊断扩展**。
6. 如果 "状态"**设置**为 "成功"，则已通过扩展测试用例。
7. 如果状态为 "**预配失败**"，则扩展测试用例失败，需要设置强化标志。

   ![预配成功](./media/vm-certification-issues-solutions-2.png)

   如果 VM 扩展失败，请参阅[使用 Linux 诊断扩展监视指标和日志](../../virtual-machines/extensions/diagnostics-linux.md)以启用它。 如果你不希望启用 VM 扩展，请联系支持团队并要求他们禁用该扩展。

## <a name="virtual-machine-provisioning-issue"></a>虚拟机预配问题

提交你的产品/服务之前，请检查是否已严格遵循 VM 的预配过程。 若要查看用于预配 VM 的 json 格式，请参阅[Azure 虚拟机（VM）映像证书](azure-vm-image-certification.md)。
...
如果由于软件符合性而拒绝 windows 映像请求，则可能已创建安装了 SQL server 的 Windows 映像，而不是从 Azure Marketplace 获取相关的 SQL 版本基本映像。
...
有关详细信息，请参阅[创建 Azure 虚拟机技术资产](create-azure-vm-technical-asset.md#create-a-vm-image-using-an-approved-base)）以选择已批准的基准。

## <a name="tool-kit-test-case-execution-failed"></a>工具包测试用例执行失败

Microsoft 认证工具包将帮助你执行测试用例，验证你的 VHD/映像是否与 Azure 环境兼容。

下载[Microsoft 认证工具包](azure-vm-image-certification.md)。

## <a name="linux-test-cases"></a>Linux 测试用例

下面是该工具包将执行的 Linux 测试用例。 说明中说明了测试验证。

|S.No|测试用例|description|
|---|---|---|
|1|Bash 历史记录|在创建 VM 映像之前，应清除 Bash 历史记录文件。|
|2|Linux 代理版本|应安装 Azure Linux 代理2.2.41 和更高版本。|
...
|6|OpenSSL 版本|OpenSSL 版本应大于或等于 v 0.9.8。|
|7|Python 版本|强烈建议使用 Python 2.6 版。|
|8|客户端活动间隔|将 ClientAliveInterval 设置为180。 在应用程序需要时，可以将其设置为30到235之间。 如果要为最终用户启用 SSH，则必须按照说明设置此值。|
|9|OS 体系结构|仅支持 64 位操作系统。|
|10|自动更新|确定是否启用 Linux 代理自动更新。|

### <a name="common-errors-found-while-executing-the-previous-test-cases"></a>执行前面的测试用例时发现常见错误
...
|2|Bash 历史记录测试用例|如果提交图像的 bash 历史记录大小超过 1 KB，将会出现错误。 大小限制为 1 KB，以确保不会在 bash 历史记录文件中捕获任何潜在的敏感信息。|若要解决此问题，请将 VHD 装载到任何其他工作 VM，并进行任何更改（例如，删除 `.bash` 历史记录文件），以将大小减小到小于或等于 1 KB。|
|3|必需的内核参数测试用例|如果**console**的值未设置为**ttyS0**，则会收到此错误。 执行以下命令进行检查：<br>`cat /proc/cmdline`|将**console**的值设置为**ttyS0** ，然后重新提交请求。|
...
下面是工具包将执行的 Windows 测试用例。 说明中说明了测试验证。
...
|10|BitLocker 驱动器加密|操作系统不支持操作系统硬盘，但可在数据磁盘上使用 BitLocker 驱动器加密。|
...
虚拟机允许访问基础操作系统时，请确保 VHD 大小对于 VHD 而言足够大。 由于磁盘无法在无停机时间的情况下扩展，因此请使用介于30到 50 GB 之间的磁盘大小。

|VHD 大小|实际占用的大小|解决方案|
|---|---|---|
|>500 TiB|不适用|请联系支持团队以获取异常批准。|
|250-500 TiB|>200 GiB 与 blob 大小不同|请联系支持团队以获取异常批准。|

> [!NOTE]
> 较大的磁盘大小会产生较高的成本，并将在预配和复制步骤中产生延迟。 由于此延迟和成本，支持团队可能会寻找异常批准的理由。
...
若要查看 Windows Server 修补版本，请参阅下表，了解 OS 详细信息及其支持的最低版本。 

可以通过或验证图像文件版本 `C:\windows\system32\drivers\srv.sys` `srv2.sys` 。

> [!NOTE]
> WindowsServer2019 没有任何必需的版本要求。
...
|WindowsServer2019|不可用|

## <a name="sack-vulnerability-patch-verification"></a>SACK 漏洞修补程序验证

提交 Linux 映像时，可能会因为内核版本问题而拒绝请求。

使用已批准版本更新内核，并重新提交请求。 可以在下表中找到已批准的内核版本。 版本号应等于或大于下面所列的版本号。

如果映像未与以下某个内核版本一起安装，请用正确的修补程序更新映像。 可从以下链接中找到详细信息。 在用以下必需的修补程序更新映像后，请求支持团队提供必要的批准：
...
按照准则，将原始磁盘转换为 VHD 时，必须确保原始磁盘大小为 1 MB 的倍数。 有关详细信息，请参阅有关[非认可分发的信息](../../virtual-machines/linux/create-upload-generic.md)

## <a name="vm-access-denied"></a>拒绝访问 VM

如果在 VM 上执行测试用例时遇到访问被拒绝问题，可能是因为没有足够的特权执行测试用例。

检查是否为运行自检案例的帐户启用了正确的访问权限。 如果不是，则启用访问以执行测试用例。 如果你不想启用访问权限，你可以与支持团队共享自测用例结果。

## <a name="download-failure"></a>下载失败
    
请参阅下表，了解使用 SAS URL 下载 VM 映像时的任何问题。

|S.NO|错误|reason|解决方案|
|---|---|---|---|
|1|找不到 Blob|VHD 可以从指定位置删除或移动|| 
|2|Blob 正在使用中|VHD 正在由另一个内部进程使用|使用 SAS URL 下载时，VHD 应处于已使用状态。|
|3|无效 SAS URL|该 VHD 的关联 SAS URL 不正确。|获取正确的 SAS URL。|
|4|Invalid Signature|该 VHD 的关联 SAS URL 不正确。|获取正确的 SAS URL。|
|6|HTTP 条件头|无效的 SAS URL。|获取正确的 SAS URL。|
|7|VHD 名称无效|检查 VHD 名称中是否存在任何特殊字符，例如 **%** 或 **""**|通过删除特殊字符重命名 VHD 文件|
...
当使用多个数据磁盘提交请求，但未按顺序排列时，这会被视为映射问题。 例如，如果有三个数据磁盘，则编号顺序必须是**0、1、2**。 任何其他订单将被视为映射问题。

请重新提交请求，并对数据磁盘进行适当的排序。

## <a name="incorrect-os-mapping"></a>OS 映射不正确

创建映像后，可以将其映射或分配到错误的 OS 标签。 例如，如果在创建映像时选择了 " **windows** " 作为 OS 名称的一部分，则 OS 磁盘只应随 Windows 一起安装。 这同样适用于 Linux。
...
以下过程将通用化 Linux VM，并将其重新部署为单独的 VM。

在 SSH 窗口中，输入以下命令：`sudo waagent -deprovision+user`

Windows:

Windows 映像是通过通用的 `sysreptool` 。

有关此工具的详细信息，请参阅[Sysprep （系统准备）概述]( https://docs.microsoft.com/windows-hardware/manufacture/desktop/sysprep--system-preparation--overview)

## <a name="datadisk-error"></a>DataDisk 错误

有关与数据磁盘相关的错误的解决方案，请参阅下表。

|错误|reason|解决方案|
|---|---|---|
|`DataDisk- InvalidUrl:`|此错误的原因可能是提交产品/服务时为 LUN 指定的数字无效。|验证数据磁盘的 LUN 编号序列是否位于 "合作伙伴中心" 中。|
|`DataDisk- NotFound:`|此错误可能是由于数据磁盘未位于指定的 SAS URL|验证数据磁盘是否位于请求中指定的 SAS URL。|

## <a name="remote-access-issue"></a>远程访问问题

如果没有为 Windows 映像启用 RDP 选项，您将收到此错误。 

提交之前，对 Windows 映像启用 RDP 访问。
