...
5.  在 "*生成计划的流*" 窗口下，对 "间隔" 和 "小时" 的 "频率" 下的 "*重复每次*选择"。 此外，为流指定一个名称（如果需要）。 选择“创建”。
    ...
    >[!Note]
    >虽然此示例使用了1小时的间隔，但你可以选择最适合你的业务需求的时间间隔和频率。
...
11. 在 " **Azure 表存储**" 窗口中，提供以下字段的信息并选择 "**创建**"：
    ...
    * *共享存储密钥*-提供 Azure 表的存储帐户的密钥值。 可以在存储帐户的 "**访问密钥**" 页中找到此项。
    ...
    单击 "创建" 后，将看到 "*获取实体*" 窗口。 选择 "**显示高级选项**" 并为以下字段提供信息：
       ...
       * *表*-选择 Azure 表存储的名称（有关如何配置 azure 表的步骤6中的说明）。 在此示例中选择 "marketplaceleads" 表后，下一屏幕捕获会显示提示。
...
19. 在**Office 365 Outlook**窗口中，为以下字段提供信息：
    ...
    1. **Body** -添加要包括在每封电子邮件中的文本（可选），然后将其粘贴 `body('Get_entities')?['value']`。
...
20. 选择“保存”以保存流。 Microsoft Flow 将自动测试流中的错误。 如果没有任何错误，则流在保存后将开始运行。
...
在流运行后管理流非常简单。 可完全控制流。 例如，可以停止它、编辑它、查看运行历史记录，然后获取分析。 下一个屏幕截图显示了可用于管理流的选项。 
...
如果你没有收到任何潜在客户电子邮件通知，则表示新的潜在客户尚未添加到 Azure 表。 如果出现任何流故障，你将收到一封电子邮件，如下一个屏幕捕获中的示例。
