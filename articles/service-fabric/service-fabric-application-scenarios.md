...
Service Fabric 的独特体系结构使你可以在应用程序中执行近实时数据分析、内存中计算、并行事务和事件处理。 您可以根据不断变化的资源需求轻松将应用程序扩展到或扩展。

有关构建应用程序的设计指南，请阅读[Azure 服务结构上的微服务体系结构](https://docs.microsoft.com/azure/architecture/reference-architectures/microservices/service-fabric)以及[使用 Service Fabric 进行应用程序设计的最佳做法](service-fabric-best-practices-applications.md)。

对于以下类型的应用程序，请考虑使用 Service Fabric 平台：

* **数据收集、处理和 IoT**：Service Fabric 可处理大规模数据，并通过其有状态的服务具有低延迟。 它可以帮助你处理数百万台设备上的数据（其中的设备数据和计算数据共置在一起）。

    使用服务交换矩阵构建物联网服务的客户包括[霍尼韦尔](https://customers.microsoft.com/story/honeywell-builds-microservices-based-thermostats-on-azure)[、PCL建筑](https://customers.microsoft.com/story/pcl-construction-professional-services-azure)、[克雷斯特龙](https://customers.microsoft.com/story/crestron-partner-professional-services-azure)、[宝马](https://customers.microsoft.com/story/bmw-enables-driver-mobility-via-azure-service-fabric/)、[施耐德电气](https://customers.microsoft.com/story/schneider-electric-powers-engergy-solutions-on-azure-service-fabric)和[网状系统](https://customers.microsoft.com/story/mesh-systems-lights-up-the-market-with-iot-based-azure-solutions)。

* **游戏和基于会话的交互式应用程序**：如果应用程序需要低延迟读取和写入（如在线游戏或即时消息），则服务交换矩阵非常有用。 通过 Service Fabric，可生成这些交互式有状态应用程序，而无需创建单独的存储或缓存。 有关[在游戏服务中使用 Service Fabric](https://docs.microsoft.com/gaming/azure/reference-architectures/multiplayer-synchronous-sf) 的设计指导，请访问 [Azure 游戏解决方案](https://azure.microsoft.com/solutions/gaming/)。
...
* **数据分析和工作流处理**：必须可靠地处理事件或数据流的应用程序受益于 Service Fabric 中优化的读取和写入。 Service Fabric 还支持应用程序处理管道，其中的结果必须能够可靠地传递到下一个处理阶段而不会丢失。 这些管道包括交易和财务系统，其中的数据一致性和计算保证至关重要。
...
* **对数据的计算**：Service Fabric 使您能够构建执行密集型数据计算的有状态应用程序。 Service Fabric 允许在应用程序中共置处理（计算）和数据。 
...
* **高度可用的服务**：服务结构通过创建多个辅助服务副本提供快速故障转移。 节点、进程或单独的服务因硬件或其他故障而不可用时，其中一个辅助副本会提升为主副本，将对服务的损失降到最低。
...
下图演示了设计有状态应用程序与无状态应用程序之间的差异。 通过利用[可靠的服务和](service-fabric-reliable-services-introduction.md)[可靠的参与者](service-fabric-reliable-actors-introduction.md)编程模型，有状态的服务降低了应用程序复杂性，同时实现了高吞吐量和低延迟。

下面是一个使用无状态服务的示例应用程序：![使用无状态服务的应用程序][Image1]

下面是一个使用有状态服务的示例应用程序：![使用有状态服务的应用程序][Image2]

## <a name="next-steps"></a>后续步骤

* 使用服务交换矩阵[可靠服务和](service-fabric-reliable-services-quick-start.md)[可靠参与者](service-fabric-reliable-actors-get-started.md)编程模型开始构建无状态和有状态的服务。
* 有关在[Azure 上构建微服务](https://docs.microsoft.com/azure/architecture/microservices/)的指导，请访问 Azure 体系结构中心。
