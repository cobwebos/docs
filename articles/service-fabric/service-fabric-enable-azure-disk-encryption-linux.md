---
title: 为 Service Fabric Linux 群集启用磁盘加密 | Microsoft Docs
...
```
4. **Azure Key Vault**：在与虚拟机规模集相同的订阅和区域中创建 KeyVault，并在 KeyVault 上使用其 PS cmdlet 设置访问策略“EnabledForDiskEncryption”。 也可以使用 Azure 门户中的 KeyVault UI 设置该策略： 
```Powershell
...
遵照相关步骤和指南[将应用程序部署到群集](https://docs.microsoft.com/en-us/azure/service-fabric/service-fabric-quickstart-containers-linux)
...
另外，用户可以登录到 Linux 群集 VM 并运行 LSBLK 命令。 输出应在装入点列中显示添加的数据磁盘，并且对于添加的数据磁盘，“类型”列应显示为“加密”。
