...
使用系统运行状况报告，不仅可以查看群集和应用程序功能，还能标记问题。 对于应用程序和服务，系统运行状况报告从 Service Fabric 的角度验证实体得到实现并且正常运行。 报表不提供服务的业务逻辑的任何运行状况监视或未响应的进程的检测。 用户服务可以使用其逻辑的特有信息来丰富运行状况数据。
...
如果某些种子节点不正常， **System.FM**将报告群集级别警告。 种子节点是维护基础群集可用性的节点。 这些节点有助于通过在某些类型的网络故障期间，与其他节点建立租约并充当决胜属性来确保群集保持启动状态。 如果大多数种子节点在群集中关闭，它们不会恢复，则群集会自动关闭。 

如果种子节点状态为 "已关闭"、"已删除" 或 "未知"，则该节点不正常。
种子节点状态的警告报告将列出所有不正常的种子节点及其详细信息。

* **SourceID**： System.FM
* **属性**： SeedNodeStatus
* **后续步骤**：如果此警告显示在群集中，请按照以下说明进行修复：对于运行 Service Fabric 版本6.5 或更高版本的群集：对于 Azure 上的 Service Fabric 群集，在种子节点关闭后，Service Fabric 将尝试自动将其更改为非种子节点。 若要进行此操作，请确保主节点类型中的非种子节点数量大于或等于向下种子节点数。 如有必要，请将更多节点添加到主节点类型以实现此目的。
根据群集状态，可能需要一段时间来解决此问题。 完成此操作后，会自动清除 "警告" 报告。

对于 Service Fabric 独立群集，若要清除警告报告，所有种子节点都需要变为正常状态。 根据种子节点不正常的原因，需要执行不同的操作：如果种子节点已关闭，用户需要将该种子节点置于上;如果已删除或未知种子节点，则[需要从群集中删除](https://docs.microsoft.com/azure/service-fabric/service-fabric-cluster-windows-server-add-remove-nodes)此种子节点。
所有种子节点都变为正常时，会自动清除警告报告。

对于运行 Service Fabric 版本低于6.5 的群集：在这种情况下，需要手动清除警告报告。 **在清除报告之前，用户应确保所有种子节点都变为正常**：如果种子节点已关闭，用户需要将该种子节点置于上; 如果种子节点已删除或未知，则需要从群集中删除该种子节点。
所有种子节点都变为正常后，使用 Powershell 中的以下命令来[清除警告报告](https://docs.microsoft.com/powershell/module/servicefabric/send-servicefabricclusterhealthreport)：
