---
title: "使用 Windows 安全性保护 Windows 上运行的群集 | Microsoft Docs"
...
---
# <a name="secure-a-standalone-cluster-on-windows-using-windows-security"></a>使用 Windows 安全性保护 Windows 上的独立群集
...
使用 **ClusterIdentity** 配置[节点到节点安全性](service-fabric-cluster-security.md#node-to-node-security)。 若要在节点之间建立信任关系，这些节点必须能够相互识别。 为此，可创建一个域组，将所有节点包括在群集中。 该组名应在 **ClusterIdentity** 中指定。 有关详细信息，请参阅[在 Active Directory 中创建组](https://msdn.microsoft.com/library/aa545347(v=cs.70).aspx)。  
...
以下示例**安全性**部分配置 Windows 安全性，并指定 *ServiceFabric/clusterA.contoso.com* 中的计算机位于群集中，同时还指定 *CONTOSO\usera* 拥有管理客户端访问权限：
...
> [!NOTE]
> 不应在域控制器上部署 Service Fabric。 使用计算机组或 gMSA 时，请确保 ClusterConfig.json 不包括域控制器的 IP。
> 
> 
