---
title: 使用 Azure 监视器日志设置监视
description: 了解如何设置 Azure 监视器日志以可视化和分析事件以监视 Azure 服务结构群集。
author: srrengar
ms.topic: conceptual
ms.date: 02/20/2019
ms.author: srrengar
...
---
# <a name="set-up-azure-monitor-logs-for-a-cluster"></a>为群集设置 Azure 监视器日志

要监视群集级别的事件，建议使用 Azure Monitor 日志。 可以通过 Azure 资源管理器、PowerShell 或 Azure 市场设置 Log Analytics 工作区。 如果维护部署的更新资源管理器模板供将来使用，请使用同一模板来设置 Azure 监视器日志环境。 如果部署的群集已启用诊断，则通过市场部署更方便。 如果你在要部署到的帐户中没有订阅级别访问权限，请通过 PowerShell 或资源管理器模板进行部署。

> [!NOTE]
> 要将 Azure 监视器日志设置为监视群集，需要启用诊断才能查看群集级或平台级事件。 有关详细信息，请参阅[如何在 Windows 群集中设置诊断](service-fabric-diagnostics-event-aggregation-wad.md)和[如何在 Linux 群集中设置诊断](service-fabric-diagnostics-oms-syslog.md)
...
如果使用 Windows，请继续执行以下步骤，将 Azure 监视器日志连接到存储群集事件的存储帐户。 

>[!NOTE]
>服务结构分析解决方案仅支持 Windows 群集。 对于 Linux 群集，请查看我们有关如何[为 Linux 群集设置 Azure 监视器日志](service-fabric-diagnostics-oms-syslog.md)的文章。  
...
7. 选择“确定”，将工作区连接到群集的日志****。
    ...
    ![将存储帐户日志添加到 Azure 监视器日志](media/service-fabric-diagnostics-event-analysis-oms/add-storage-account.png)
...
## <a name="deploy-azure-monitor-logs-with-azure-resource-manager"></a>使用 Azure 资源管理器部署 Azure 监视器日志
...
## <a name="deploy-azure-monitor-logs-with-azure-powershell"></a>使用 Azure PowerShell 部署 Azure 监视器日志

您还可以使用 命令通过 PowerShell`New-AzOperationalInsightsWorkspace`部署日志分析资源。 要使用此方法，请确保已安装 [Azure PowerShell](https://docs.microsoft.com/powershell/azure/install-Az-ps)。 使用此脚本新建 Log Analytics 工作区，并向其添加 Service Fabric 解决方案： 
...
完成后，按照上一节中的步骤将 Azure 监视器日志连接到相应的存储帐户。

还可添加其他解决方案或使用 PowerShell 对 Log Analytics 工作区进行其他修改。 要了解更多信息，请参阅[使用 PowerShell 管理 Azure 监视器日志](../azure-monitor/platform/powershell-workspace-configuration.md)。
...
* [使用视图设计器在 Azure 监视器日志中创建自定义视图](../azure-monitor/platform/view-designer.md)
