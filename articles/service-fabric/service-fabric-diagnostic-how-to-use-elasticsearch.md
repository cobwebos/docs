<properties
   pageTitle="将 Elasticsearch 用作 Service Fabric 应用程序跟踪存储 | Microsoft Azure"
...
## 介绍
本文介绍 [Azure Service Fabric](/documentation/services/service-fabric/) 应用程序如何使用 **Elasticsearch** 和 **Kibana** 来存储、索引和搜索应用程序跟踪。[Elasticsearch](https://www.elastic.co/guide/index.html) 是开源的、分布式和可缩放的实时搜索和分析引擎，很适合执行此任务。它可以安装在 Microsoft Azure 中运行的 Windows 和 Linux 虚拟机上。Elasticsearch 可以非常高效地处理使用**Windows 事件跟踪 (ETW)** 之类的技术所生成的 *结构化* 跟踪。
...
>[AZURE.NOTE] ES-MultiNode 模板和相关联的脚本目前支持 Elasticsearch 1.7 版。日后将增加对 Elasticsearch 2.0 的支持。
...
1. 如果你尚未安装它们，请安装 [**Azure PowerShell 模块**](http://go.microsoft.com/fwlink/p/?linkid=320376)。出现提示时，请单击“运行”，再单击“安装”。
>[AZURE.NOTE] Azure PowerShell 正在 Azure PowerShell 1.0 版中进行重大变更。CreateElasticSearchCluster 目前设计为用于 Azure PowerShell 0.9.8，不支持 Azure PowerShell 1.0 预览版。日后将提供 Azure PowerShell 1.0 兼容脚本。
...
3. 假设 Git 已安装，但未包含在系统路径中，请打开 Microsoft Azure PowerShell 窗口并运行下列命令：
...
4. 确保你已登录到 Azure（通过 [**Add-AzureAccount**](https://msdn.microsoft.com/zh-cn/library/azure/dn790372.aspx) cmdlet），并且已选择应该用来创建 Elasticsearch 群集的订阅 ([**Select-AzureSubscription**](https://msdn.microsoft.com/zh-cn/library/azure/dn790367.aspx))。
...
|参数名称 |说明|
|-----------------------  |--------------------------|
|dnsNameForLoadBalancerIP |此名称用于为 Elasticsearch 群集创建公开可见的域名系统 (DNS) 名称（通过将 Azure 区域域附加到提供的名称）。例如，如果此参数值为“myBigCluster”，而所选的 Azure 区域是美国西部，则为群集产生的 DNS 名称会是 **myBigCluster.westus.cloudapp.azure.com**。<br /><br />对于与 Elasticsearch 群集相关联的许多项目的名称，例如数据节点名称，此名称也会成为这些名称的根。|
|storageAccountPrefix |将为 Elasticsearch 群集创建的存储帐户的前缀。<br /><br /> 模板的当前版本使用一个共享存储帐户，但在未来可能变更。|
|adminUsername |用于管理 Elasticsearch 群集的管理员帐户的名称（将自动生成对应的 SSH 密钥）。|
|dataNodeCount |Elasticsearch 群集中的节点数。当前版本的脚本不会区分数据与查询节点；所有节点会同时扮演这两种角色。|
|dataDiskSize |将分配给每个数据节点的数据磁盘大小（以 GB 为单位）。每个节点将会收到 4 个数据磁盘，专供 Elasticsearch 服务使用。|
|region |应该放置 Elasticsearch 群集的 Azure 区域的名称。|
|esClusterName |Elasticsearch 群集的内部名称。<br /><br />除非你打算在相同的虚拟网络上运行多个 Elasticearch 群集，否则就需要更改此默认值。ES-MultiNode 模板目前不支持这么做。|
|esUserName esPassword |将配置为可访问 Elasticsearch 群集的用户的凭据（受限于 HTTP 基本身份验证）。|

你现在可以开始运行脚本。发出下列命令：
```powershell
CreateElasticSearchCluster -ResourceGroupName <es-group-name>
```
其中 `<es-group-name>` 是将包含所有群集资源的 Azure 资源组的名称。

>[AZURE.NOTE] 如果从 Test-AzureResourceGroup cmdlet 收到 NullReferenceException，表示你忘记登录到 Azure (`Add-AzureAccount`)。
...
2. *成熟度、稳定性和已证实的性能*
    ...
    * 平台供应商所开发的代理（例如 Microsoft Azure 诊断代理）都经过严格的测试和实战锻炼。
...
### Service Fabric 2015 年 11 月预览版和 Microsoft.Diagnostics.Tracing NuGet 包
使用 Service Fabric 2015 年 11 月预览版生成的应用程序面向 **.NET Framework 4.5.1**。这是 Azure 在预览版支持的最高 .NET Framework 版本。可惜，这个版本的框架缺少 Microsoft.Diagnostics.Listeners 库所需的某些 EventListener API。因为 EventSource（Fabric 应用程序中形成日志记录 API 基础的组件）和 EventListener 紧密结合，每个使用 Microsoft.Diagnostics.Listeners 库的项目都必须使用替代的 EventSource 实现。此实现由 Microsoft 创作的 **Microsoft.Diagnostics.Tracing NuGet 包**所提供。此包与框架中包含的 EventSource 完全后向兼容，因此，除了更改引用的命名空间，应该不需要更改任何代码。
...
当 Microsoft Azure 支持 **.NET Framework 4.6** 时，就不需要这些步骤。
