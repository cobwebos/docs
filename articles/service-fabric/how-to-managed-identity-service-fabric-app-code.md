---
title: 在应用程序中使用托管标识
description: 如何在 Azure 中使用托管标识 Service Fabric 应用程序代码来访问 Azure 服务。 此功能目前以公共预览版提供。
ms.topic: article
ms.date: 10/09/2019
...
- "MSI_ENDPOINT"： localhost 终结点、完整路径、API 版本以及与该服务的托管标识对应的参数
- "MSI_SECRET"：身份验证代码，该代码是不透明的字符串，用于唯一表示当前节点上的服务

> [!NOTE]
> 名称 "MSI_ENDPOINT" 和 "MSI_SECRET" 引用以前指定的托管标识（"托管服务标识"），现已弃用。 这些名称还与其他支持托管标识的 Azure 服务使用的等效环境变量名称一致。
...
| `api-version` | 查询字符串参数，指定托管标识令牌服务的 API 版本;目前 `2019-07-01-preview`唯一接受的值，并且可能会发生更改。 |
| `resource` | 一个查询字符串参数，表示目标资源的应用 ID URI。 这将反映为已颁发令牌的 `aud` （受众）声明。 此示例请求一个令牌访问 Azure Key Vault，其应用 ID URI 为 https：\//keyvault.azure.com/。 |
...
| `resource` | 为其颁发访问令牌的资源，通过请求的 `resource` 查询字符串参数指定;对应于标记的 "aud" 声明。 |
...
| message | 错误的详细说明。 **错误说明随时可能会更改。不要依赖于错误消息本身。**|
...
通常，唯一可重试错误代码为429（请求过多）;内部服务器错误/5xx 错误代码可能是可重试的，尽管原因可能是永久性的。 

限制限制适用于对托管标识子系统所进行的调用次数（特别是 "上游" 依赖项（托管标识 Azure 服务或安全令牌服务）。 Service Fabric 在管道中的各个级别缓存令牌，但对于所涉及的组件的分布式性质，调用方可能会遇到不一致的限制响应（即，在应用程序的一个节点/实例上受到限制，而不是在为同一标识请求令牌时在不同的节点上）。设置限制条件后，来自同一应用程序的后续请求可能会失败，并出现 HTTP 状态代码429（请求过多），直到清除该条件。  
