...
内存限制（以 MB 为单位）。 必须为正整数。 代码包（容器或进程）无法分配超出此限制的内存，尝试执行此操作会引发内存不足异常。
...
内存调控软限制（以 MB 为单位），仅当在节点上检测到内存争用时才强制执行此限制。 必须为正整数。
...
可用 CPU 的可用百分比（仅适用于 Windows）。 必须为正整数。 如果为服务包指定了 CPU 限制，将有效忽略此参数。
...
以能够使用的 IOps 计的最大 IO 速率（读取和写入）。 必须为正整数。
...
以字节为单位的内核内存限制。  必须为正整数。  请注意，这是特定于 Linux 的，Windows 上的 Docker 在进行此设置的情况下会出错。
...
/dev/shm 的大小（以字节为单位）。 如果省略此项，系统会使用 64MB。  必须为正整数。  请注意，这是特定于 Linux 的，但在指定此项的情况下，Docker 不会出错。  系统会直接忽略它。
