...
|内容|0 个元素，4 个属性|
|defined|globally|
|名称|ConfigPackageDescriptionType|
...
声明一个按 Name 特性命名的文件夹，该文件夹位于包含 Settings.xml 文件的 PackageRoot 中。 此文件包含进程用户定义的键值对设置，进程可在运行时读回这些设置。 升级期间，如果仅更改了 ConfigPackage 版本，则不重启正在运行的进程。 相反，回调会向进程通知配置设置已更改，以便可以重新动态加载这些设置。
...
使用在 clusterManifest.xml 中指定的默认存储库凭据，而不是在 ApplicationManifest.xml 的“RepositoryCredentials”标记中指定的凭据。
        

|属性|值|
|---|---|
|名称|UseDefaultRepositoryCredentials|
|type|xs:string|
|use|可选|
|default|false|

#### <a name="usetokenauthenticationcredentials"></a>UseTokenAuthenticationCredentials
使用 MSI 令牌身份验证（或自定义的指定终结点）来获取用于身份验证的令牌。
        

|属性|值|
|---|---|
|名称|UseTokenAuthenticationCredentials|
|type|xs:string|
|use|可选|
|default|false|

#### <a name="isolation"></a>隔离
容器的隔离模式。 有效值为默认、进程或 hyperv（仅支持 Windows 容器）。
...
一个标志，指示在终止容器的情况下，是会自动删除容器 (true) 还是不会 (false)。  如果为 false，SF 会手动删除容器。
...
|内容|0 个元素，1 个属性|
|defined|globally|
|名称|ContainerNetworkPolicyEndpointBindingType|
...
|内容|0 个元素，3 个属性|
|defined|globally|
|名称|EnvironmentVariableOverrideType|
...
|属性|值|
|---|---|
|名称|CentralSecretServiceReplicatorEndpoint|
|type|[InternalEndpointType](#internalendpointtype-complextype)|
|minOccurs|0|
...
|属性|值|
|---|---|
|名称|EventStoreServiceReplicatorEndpoint|
|type|[InternalEndpointType](#internalendpointtype-complextype)|
|minOccurs|0|
...
|属性|值|
|---|---|
|名称|GatewayResourceManagerReplicatorEndpoint|
|type|[InternalEndpointType](#internalendpointtype-complextype)|
|minOccurs|0|
...
|内容|0 个元素，2 个属性|
|defined|globally|
|名称|ServiceFabricRuntimeAccessPolicyType|
...
|属性|值|
|---|---|
|名称|RemoveServiceFabricRuntimeAccess|
|type|xs:boolean|
|default|false|
...
|属性|值|
|---|---|
|名称|UseServiceFabricReplicatedStore|
|type|xs:boolean|
|default|false|
...
|属性|值|
|---|---|
|名称|ServiceFabricRuntimeAccessPolicy|
...
|内容|1 个元素，2 个属性|
|defined|globally|
|名称|ServicePackageContainerPolicyType|
