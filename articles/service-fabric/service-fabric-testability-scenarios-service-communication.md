<properties
   pageTitle="可测试性：服务通信 | Microsoft Azure"
...
在本示例中，我们将 `QuorumLossMode` 设置为 `PartialQuorumLoss` 以指出我们希望引入仲裁丢失而不关闭所有副本。这样就仍然能够进行读操作。若要测试整个分区不可用的情形，可将此开关设置为 `FullQuorumLoss`。
