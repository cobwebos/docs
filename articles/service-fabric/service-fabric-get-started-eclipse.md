...
1. 请确保已安装最新 Eclipse **Neon** 和最新 Buildship（1.0.17 或更高版本）。 可以选择``Help => Installation Details``，检查已安装组件的版本。 可以使用[此处][buildship-update]的说明更新 Buildship。 若要检查 Eclipse Neon 是否为最新版本并根据情况进行更新，可转到``Help => Check for Updates``。

2. 若要安装 Service Fabric 插件，请选择``Help => Install New Software...``。
...
> [!NOTE]
> 如果在 Eclipse 上安装或更新 Service Fabric Eclipse 插件耗时很长，则是因为 Eclipse 每次都会尝试提取发生在所有更新站点（已注册到 Eclipse 实例）上的所有新更改的元数据。 因此，若要加快速度，可以使用这样一种小技巧：转到`Available Software Sites`，取消选中所有选项，只保留指向 Service Fabric 插件位置 `http://dl.windowsazure.com/eclipse/servicefabric` 的选项。
>

## <a name="create-service-fabric-application-using-eclipse"></a>使用 Eclipse 创建 Service Fabric 应用程序

1. 转到 ``File => New => Other``。 选择``Service Fabric Project``。 单击``Next``。
...
3. 从可用模板集（执行组件、无状态、容器或来宾可执行文件）中选择“服务模板”。 单击``Next``。

    ![Service Fabric 新建项目第 3 页][create-application/p3]

4. 在此页上输入服务名称和/或相关的服务详细信息，然后单击``Finish``。

    ![Service Fabric 新建项目第 4 页][create-application/p4]

5. 创建第一个 Service Fabric 项目时，系统会询问是否需要设置 Service Fabric 透视，请选择``yes``继续。
...
* 右键单击上述刚创建的 Service Fabric 应用程序。 在上下文菜单中选择``Service Fabric``选项。 此时会显示一个带多个选项的子菜单， 如下所示：
  ...
  - ``Build Application``会在不清除的情况下生成应用程序
  - ``Rebuild Application``会执行应用程序的干净生成
  - ``Clean Application``会清除应用程序中的已生成项目


* 也可选择通过此菜单部署、取消部署以及发布应用程序。
  - ``Deploy Application``会部署到本地群集
  - ``Publish Application...``会询问你在 ``Local.json`` 和 ``Cloud.json`` 之间选择哪个发布配置文件。 这些 JSON 文件用于存储连接到本地群集或云 (Azure) 群集所需的信息（例如连接终结点和安全信息）。

  ![Service Fabric 右键菜单][publish/Publish]

* 还可以通过另一种方式，使用 Eclipse 运行配置来部署 Service Fabric 应用程序。

  1. 选择``Run => Run Configurations``。 选择``Grade Project``下的 ``ServiceFabricDeployer`` 运行配置。
  2. 在右窗格的``Arguments``选项卡下，指定“本地”或“云”作为 ``publishProfile``。 默认设置为“本地”。 若要部署到远程/云群集，请选择“云”。
  3. 相应地编辑 `Local.json` 或 `Cloud.json`，根据情况输入终结点详细信息和安全凭据，确保在发布配置文件中填充正确的信息。
  4. 确保右窗格中``Grade Project``下的``Working Directory``指向要部署的应用程序。 如果否，则请单击``Workspace...``按钮，选择所需应用程序。
  5. 单击“应用”，然后单击“运行”。
...
2. 选择`Add ServiceFabric Service`选项，系统会引导你完成后续步骤，将服务添加到项目。
...
1. 选择``Run => Run Configurations``。 单击左窗格中``Grade Project``左侧的小箭头。
2. 右键单击``ServiceFabricDeployer``，选择``Duplicate``。 为此配置提供一个新名称，例如 ``ServiceFabricUpgrader``。
3. 在右侧面板的``Arguments``选项卡下，将``-Pconfig='deploy'``更改为``-Pconfig=upgrade``，然后单击``Apply``。
4. 现在，你已创建和保存用于升级应用程序的运行配置，该配置可以按需``Run``。 该配置将用于从 application-manifest 文件中获取最近更新的 application-type 版本。
