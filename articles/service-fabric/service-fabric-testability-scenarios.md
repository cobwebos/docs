<properties
   pageTitle="运行混沌测试。"
   description="本文介绍 Microsoft 提供的固有 Service Fabric 方案。"
...
# 可测试性方案
大型分布式系统，例如云基础结构，在本质上都是不可靠的。Service Fabric 使开发人员能够编写出可以在不可靠基础结构上运行的服务。若要编写高质量的服务，开发人员需要能够引入这种不可靠的基础结构来测试其服务的稳定性。Service Fabric 使开发人员能够引入故障操作，在存在故障情况下测试服务。然而，定向模拟故障只能做到这种程度。为了让测试更进一步，Service Fabric 提供了固有的测试方案。这些方案在整个群集模拟连续交叉出现的故障，包括常规故障和非常规故障，时间跨度很长。一旦配置好故障率和故障类型，它就可以作为一个客户端工具运行，通过 C# API 或 PowerShell 在群集和服务中生成故障。作为可测试性功能的一部分，我们提供以下方案。
...
混沌测试在指定时间段内运行故障和群集验证的多次循环。让群集达到稳定状态以及验证成功所用的时间也是可配置的。若我们在群集验证中遇到一次故障，则方案失败。例如，考虑一个设置为运行 1 小时且最多 3 个并发故障的测试。该测试将引入 3 个故障，然后验证群集运行状况。该测试将循环进行以上步骤，直到群集变得不正常或 1 小时的时间已过。如果在任何一次循环中群集变得不正常，即在配置的时间内不稳定，则测试将引发异常而失败。此异常指出系统出现问题，并且需要进一步调查。当前，混沌测试故障生成引擎只引入安全故障。这意味着在没有外部故障的情况下，绝对不会出现仲裁丢失或数据丢失。

### 重要的配置选项
 - **TimeToRun**：测试在成功完成之前将要运行的总时间。测试也可以提前完成来代替验证失败。
 - **MaxClusterStabilizationTimeout**：在测试失败之前，等待群集变得正常的最大时间。执行的检查包括群集运行状况是否正常、服务运行状况是否正常、对于服务分区而言目标副本集是否达到设定的大小以及是否没有 InBuild 副本。
...
### 重要的配置选项
 ...
 - **MaxServiceStabilizationTimeout**：在测试失败之前，等待群集变得正常的最大时间。执行的检查包括服务运行状况是否正常、对于所有分区而言目标副本集是否达到设定的大小以及是否没有 InBuild 副本。
