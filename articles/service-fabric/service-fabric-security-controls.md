...
description: 了解 Azure 服务结构的安全控件。 包括内置安全控制清单。
author: msmbaldwin
ms.topic: conceptual
ms.date: 09/04/2019
ms.author: mbaldwin
...
| 授权| 是 | 通过 SFRP 进行针对调用的标识和访问管理 (IAM)。 直接调用群集终结点支持两个角色：用户角色和管理员。客户可以将 API 映射到任一角色。 |

## <a name="data-protection"></a>数据保护

| 安全控制 | Yes/No | 说明 |
|---|---|--|
| 服务器端静态加密：微软管理的密钥 | 是 | 客户拥有群集以及作为群集构建基础的虚拟机规模集。 可以在虚拟机规模集上启用 Azure 磁盘加密。 |
