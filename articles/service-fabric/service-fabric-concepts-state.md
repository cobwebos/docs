...
# 服务状态
**服务状态**是服务正常运行所必需的数据。这是服务为正常工作而读取和写入的数据结构和变量。
...
在 Service Fabric 中，第一个服务被称为无状态服务。第二个服务则称为有状态服务。

## 存储服务状态
状态可以外部化或与操作状态的代码共存。通常使用外部的数据库或存储区进行状态的外部化。在我们的计算器示例中，这可能是 SQL 数据库（当前结果存储在表中）。计算总和的每个请求均将在此行上执行更新。
