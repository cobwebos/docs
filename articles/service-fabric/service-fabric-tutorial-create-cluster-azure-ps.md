...
以下过程创建单节点（单虚拟机）预览版 Service Fabric 群集，该群集由位于 Key Vault 中的自签名证书保护。 单节点群集无法扩展超出一个虚拟机，并且预览版群集无法升级到较新版本。 
...
7. 使用默认设置，但务必勾选“将此密钥标记为可导出” 复选框（在“私钥保护”步骤中）。 本教程中稍后将 Azure 容器注册表配置为 Service Fabric 群集身份验证时，Visual Studio 需要导出该证书。

8. 现在可以在浏览器中打开 Service Fabric Explorer。 URL 是 PowerShell CmdLet 输出中的 ManagementEndpoint，例如 https://mycluster.westeurope.cloudapp.azure.com:19080 
