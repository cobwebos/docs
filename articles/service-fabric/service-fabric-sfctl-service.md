...
description: 了解 sfctl，Azure 服务结构命令行接口。 包括用于管理服务、服务类型和服务包的命令列表。
author: jeffj6123
ms.topic: reference
ms.date: 1/16/2020
ms.author: jejarry
...
| --move-cost | 指定服务的移动成本。 可能的值是\:"零"、"低"、"中"、"高"、"非常高"。 |
...
| --placement-policy-list | 服务的放置策略以及任何关联域名的 JSON 编码列表。 策略\:`NonPartiallyPlaceService`可以是一个或多个`PreferPrimaryDomain`， `RequireDomain`、 `RequireDomainDistribution`. |
...
| --服务安置时间 | 副本在报告该生成之前可以保留 InBuild 的持续时间将卡住。 仅适用于有状态服务。 |
