<properties
   pageTitle="Service Fabric Reliable Actors KVSActorStateProvider 配置概述"
   description="了解有关配置类型“KVSActorStateProvider”的 Service Fabric 有状态 Actor 的信息"
...
通过更改 Visual Studio 程序包根目录下的“Config”文件夹中生成的“settings.xml”文件，可以为指定的问题 Actor 修改 KVSActorStateProvider 的默认配置。
...
### 配置名称
请参阅[复制安全](/documentation/articles/service-fabric/service-fabric-replication-security)

## 复制器配置
复制器配置用于配置负责使 Actor 状态提供程序状态高度可靠的复制器。默认配置由 Visual Studio 模板生成，并应已足够。本部分介绍了可用于调整复制器的其他配置。
### 部分名称
...
## 存储配置
存储配置用于配置本地存储，该存储用于保持正在复制的状态。默认配置由 Visual Studio 模板生成，并应已足够。本部分介绍了可用于调整本地存储的其他配置。
### 部分名称
