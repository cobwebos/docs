---
title: 在 Azure 上创建基于执行组件的服务 Service Fabric
...
* 执行组件服务项目 (HelloWorld)。 此项目定义要托管执行组件的 Service Fabric 服务。 它包含执行组件的实现 HelloWorld.cs。 执行组件实现是派生自基类型 `Actor` 的一个类，用于实现 MyActor.Interfaces项目中定义的接口。 执行组件类还必须实现一个接受 `ActorService` 实例和 `ActorId` 并将其传递给基本 `Actor` 类的构造函数。
...
5. 客户端项目还需要对接口项目的引用。  在 ActorClient 项目中，右键单击 "**依赖关系**"，然后单击 "**添加引用 ...** "。 **> 解决方案**中选择项目（如果尚未选择），然后勾选 " **HelloWorld**" 旁边的复选框。  单击“确定”。
...
按 F5在 Service Fabric 开发群集中本地生成、部署和运行应用程序。  在部署过程中，可以在“**输出**”窗口中查看进度。
