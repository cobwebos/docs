...
|---|---|
|name|终结点|
|type|[端点类型](service-fabric-service-model-schema-complex-types.md#endpointtype-complextype)|
...
|特性|“值”|
|---|---|
|type|[图像类型](service-fabric-service-model-schema-complex-types.md#imagetype-complextype)|
|content|0 个元素，0 个属性|
...
|特性|“值”|
|---|---|
|type|[参数类型](service-fabric-service-model-schema-complex-types.md#parametertype-complextype)|
|content|0 个元素，0 个属性|
...
指定帐户类型：本地用户、域用户、网络服务、本地服务、托管服务帐户或本地系统。  默认值为 LocalUser。 会在部署应用程序的计算机上创建本地用户帐户。 默认情况下，这些帐户的名称与此处指定的名称不同。 相反，它们是动态生成的并带有随机密码。 受支持的本地系统帐户类型为 LocalUser、NetworkService、LocalService 和 LocalSystem。 在 Azure Active Directory 是可用的 Windows Server 部署上支持域帐户。
