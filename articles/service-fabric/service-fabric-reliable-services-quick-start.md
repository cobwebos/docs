...
然后创建一个名为的无状态服务项目*HelloWorldStateless*:
...
有状态服务具有与无状态服务相同的入口点。 主要区别是是否有能够可靠地存储状态的*状态提供程序*。 Service Fabric 附带状态提供程序实现调用[Reliable Collections](service-fabric-reliable-services-reliable-collections.md)，后者可以创建复制通过可靠状态管理器的数据结构。 有状态可靠服务默认使用此状态提供程序。
