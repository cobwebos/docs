...
|-- MongoDB
    |-- bin
        ...
        |-- anybinary.exe
```
由于 Service Fabric 需要使用类似于下面的命令来启动 MongoDB，因此打包 MongoDB 时，你需要使用 `/ma` 参数。
...
|--[yourtargetdirectory]
    ...
    |-- ApplicationManifest.xml
```
如你所见，工具已将新文件夹“MongoDB”添加到包含 MongoDB 二进制文件的目录中。 打开 `ApplicationManifest.xml` 文件后，你会发现包现在包含 Node.js 应用程序和 MongoDB。 以下代码会显示应用程序清单的内容。
