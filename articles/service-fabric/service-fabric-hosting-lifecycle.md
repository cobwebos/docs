...
**ServiceTypeDisableFailureThreshold**确定 (激活、下载) 故障的次数，并将 ServiceType 计划为列入阻止列表。 因此，第一次激活/下载失败或 CodePackage 崩溃应触发 ServiceType 列入阻止列表计划。 **ServiceTypeDisableGraceInterval** config 确定了在该节点上 ServiceType 最后标记为列入阻止列表的宽限时间间隔。 请注意，对于这种情况，激活/下载/CodePackage 重启应仍在内部重试模式，并通过托管子系统进行跟踪。 重试，例如： CodePackage 将计划在崩溃后再次启动，否则 Service Fabric 将再次尝试下载包。
...
回退值始终 (RetryTime， **ActivationMaxRetryInterval**) ，此值可以是基于**ActivationRetryBackoffExponentiationBase** config 的常量、线性或指数。
...
**ActivationMaxRetryInterval**控制故障后，Service Fabric) 等待的最长时间 (等待
...
Service Fabric 始终使用与 CodePackage 崩溃) 相同的线性回退 (在激活过程中发现错误。 这意味着激活操作将在 (0 + 10 + 20 + 30 + 40) = 100 秒后放弃， (第一次重试是立即) 。 不重试此激活后。
    
最大激活回退可以是**ActivationMaxRetryInterval**并重试**activationmaxfailurecount 次**。

### <a name="download-failure"></a>下载失败
如果在下载过程中遇到错误，Service Fabric 始终使用线性回退。 这意味着激活操作将在 (0 + 10 + 20 + 30 + 40) = 100 秒后放弃， (第一次重试是立即) 。 此下载不重试。 用于下载的线性回退等同于 ContinuousFailureCount ***DeploymentRetryBackoffInterval** ，可以最大为**DeploymentMaxRetryInterval**。 与激活一样，下载操作可以重试**activationmaxfailurecount 次**。
...
* 在激活的情况下，当 Service Fabric 系统需要在节点上放置一个副本时，RA (ReconfigurationAgent) 会要求承载子系统激活应用程序，并每15秒 (**RAPMessageRetryInterval**) 重试激活请求。 为了使 Service Fabric 系统知道是否已禁用 ServiceType，托管的激活操作需要的时间比重试时间间隔和**ServiceTypeDisableGraceInterval**长。 例如：让群集将配置**activationmaxfailurecount 次**设置为5，将**ActivationRetryBackoffInterval**设置为1秒。这意味着激活操作将在 (0 + 1 + 2 + 3 + 4) = 10 秒后放弃 (第一次重试是立即) ，并且在该宿主放弃重试之后。 在这种情况下，激活操作将完成，并将在15秒后重试。 发生这种情况是因为 Service Fabric 在15秒内完成了所有重试。 因此，ReconfigurationAgent 中的每次重试都将在托管子系统中创建新的激活操作，该模式将保持重复，并且不会在该节点上禁用 ServiceType。 由于无法在节点 Sf 系统的组件 FM 上禁用 ServiceType (FailoverManager) 不会将副本移到其他节点。
> 
...
示例1：假设 Deactivator 在执行**扫描)  (** 。 下一次扫描将位于2T。 假设 ServicePackage 激活发生在 T + 1 中。 此 ServicePackage 不承载副本，因此需要将其停用。 为了使 ServicePackage 成为停用的候选项，它需要处于 "无副本" 的状态（至少为 "T"）。 也就是说，它有资格在 2T + 1 上停用。 因此，2T 上的扫描将找不到此 ServicePackage 作为停用的候选项。 下一次停用周期3T 会将此 ServicePackage 计划为停用，因为现在它在没有时间 T 的副本状态。  
...
> [!NOTE]
> 因此 ServicePackage 可以在 (**DeactivationScanInterval**到 2 ***DeactivationScanInterval**) + **DeactivationGraceInterval**ExclusiveModeDeactivationGraceInterval 之间的任何位置停用 / **ExclusiveModeDeactivationGraceInterval**。 
>
...
对于 ex：假设发生减量，并计划在 2T + X (**DeactivationGraceInterval** / **ExclusiveModeDeactivationGraceInterval**) 上停用。 如果在这段时间内宿主获取一个 IncrementUsage，这意味着创建一个副本，则取消停用。
...
**ServiceTypeDisableFailureThreshold**：默认值为1。 失败计数的阈值，在此阈值后，将通知 FM (FailoverManager) 禁用该节点上的服务类型，并尝试另一个节点进行放置。
