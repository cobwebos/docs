...
|MaxTimeoutRetryBuffer | 以秒为单位的时间，默认值为 600 |动态|指定以秒为单位的时间范围。 因超时而发生内部重试时的最大操作超时值为 `<Original Time out> + <MaxTimeoutRetryBuffer>`。 以 MinOperationTimeout 为增量添加额外超时时间。 |
...
|AADTokenEndpointFormat|string，默认值为“”|静态|为非默认环境（例如 Azure 政府 "https： \//microsoftonline/{0}"）指定的 AAD 令牌终结点（默认为 Azure 商业） |
