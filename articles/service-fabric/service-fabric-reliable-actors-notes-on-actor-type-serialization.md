...
ms.locfileid: "75349306"
---
# <a name="notes-on-service-fabric-reliable-actors-type-serialization"></a>有关 Service Fabric Reliable Actors 类型序列化的说明
所有方法的参数、执行组件接口中每个方法返回的任务的结果类型以及存储在执行组件的状态管理器中的对象必须是[数据协定可序列化](/dotnet/framework/wcf/feature-details/types-supported-by-the-data-contract-serializer)的。 这同样适用于[执行组件事件接口](service-fabric-reliable-actors-events.md)中定义的方法的参数。 （执行组件事件接口方法始终返回 void。）
...
Reliable Actor 框架使用 DataContract 序列化。 因此，必须分别使用**数据协定**和**数据成员**属性对自定义数据对象及其成员进行点带。
