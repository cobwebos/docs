...
metadata 用于描述安全群集，可根据具体的情况进行设置。 ClusterCredentialType 和 ServerCredentialType 确定群集与节点将要实现的安全类型。 对于基于证书的安全性，可以将其设置为*X509* ，对于基于 Active Directory 的安全性，则设置为*Windows* 。 security 节的余下设置基于安全类型。 若要了解如何填充 security 节的余下设置，请参阅[独立群集中基于证书的安全性](service-fabric-windows-cluster-x509-security.md)，或[独立群集中的 Windows 安全性](service-fabric-windows-cluster-windows-security.md)。
...
```
所有可用的加载项功能都可在[Service Fabric REST API 参考](https://docs.microsoft.com/rest/api/servicefabric/sfrp-model-addonfeatures)中查看。
