<properties
   pageTitle="Reliable Actors 生命周期 | Microsoft Azure"
...
> [AZURE.TIP] Fabric Actors 运行时发出一些[与执行组件激活和停用相关的事件](/documentation/articles/service-fabric-reliable-actors-diagnostics#actor-activation-and-deactivation-events)。它们可用于进行诊断和性能监视。
...
- 在 T = 7 时的执行组件方法调用将空闲时间重置为 0，并延迟此执行组件的垃圾回收。
