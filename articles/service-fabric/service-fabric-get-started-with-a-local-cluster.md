...
> [AZURE.NOTE] 本文假设您已[设置开发环境](service-fabric-get-started.md)。
...
3. 最后，看看 WordCountService 的分区列表：
    ...
    > [AZURE.NOTE] 要了解有关 Service Fabric Explorer 的详细信息，请参阅[使用 Service Fabric Explorer 可视化群集](service-fabric-visualizing-your-cluster.md)。
...
当使用本地群集来开发应用程序时，经常会发现自己一直在执行编写代码、调试、更改代码、调试等快速迭代。为了帮助优化此过程，本地群集可采用以下两种模式运行：1 节点或 5 节点。这两种群集模式各有优点。5 节点群集模式使你能够使用真正的群集。可以使用多个实例和服务副本测试故障转移方案。1 节点群集模式进行了优化，可快速执行部署和注册服务，从而使用 Service Fabric 运行时帮助你快速验证代码。

1 节点群集模式和 5 节点群集模式都不是仿真器或模拟器。它运行多台计算机群集上使用的相同平台代码。

> [AZURE.NOTE] 在 SDK 版本 5.2 和更高版本中提供了此功能。

若要将群集模式更改为 1 节点群集，可以按以下方式使用 Service Fabric 本地群集管理器或者使用 PowerShell：
...
> [AZURE.WARNING] 在更改群集模式时，当前群集会从系统中删除，并将创建一个新的群集。数据必须已经保存到群集中，在更改群集模式时，它们将会被删除。
