...
* `ReliableDictionary`枚举使用按键排序的排序数据结构。 为了使枚举有效，将提交添加到临时哈希表中，然后将其移到主排序的数据结构后检查点。 添加/更新/删除操作具有最佳的 O （1）和最差的 case 运行时（log n），在验证检查是否存在密钥的情况下。 获取的可能是 O （1）或 O （log n），这取决于你是从最近的提交还是从旧的提交中进行读取。
