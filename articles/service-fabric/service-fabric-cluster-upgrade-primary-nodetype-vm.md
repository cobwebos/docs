---
title: 升级 Azure Service Fabric 群集 VM 的 SKU 或 OS | Microsoft Docs
description: 了解如何在 Service Fabric 群集的主节点类型中升级虚拟机。
...
ms.locfileid: "34659185"
---
# <a name="upgrade-the-primary-node-type-vms-of-a-service-fabric-cluster"></a>升级 Service Fabric 群集的主节点类型 VM
本文介绍如何升级 Azure 中运行的 Service Fabric 群集的主节点类型虚拟机。  Service Fabric 群集是一组通过网络连接在一起的虚拟机或物理计算机，微服务会在其中部署和管理。 属于群集一部分的计算机或 VM 称为节点。 虚拟机规模集是一种 Azure 计算资源，用于将一组 VM 作为一个集进行部署和管理。 Azure 群集中定义的每个节点类型[设置为独立的规模集](service-fabric-cluster-nodetypes.md)。 然后可以单独管理每个节点类型。 创建 Service Fabric 群集后，可以纵向缩放群集节点类型（更改节点的资源）或升级节点类型 VM 的操作系统。  随时可以缩放群集，即使该群集上正在运行工作负荷。  在缩放群集的同时，应用程序也会随之自动缩放。
