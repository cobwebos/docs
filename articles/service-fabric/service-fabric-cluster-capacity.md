...
* 主节点类型的**最小 VM 数**由您选择的**可靠性层**决定。 默认可靠性层为“白银”。 请参阅[群集的可靠性特征](https://docs.microsoft.com/azure/service-fabric/service-fabric-cluster-capacity#the-reliability-characteristics-of-the-cluster)以获取更多详细信息。  
...
| ---------------- |  ----------------------------  | ---------------------------------------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 金卡             | 5                              | 专用于单个客户的完整节点 SKU（例如 L32s、GS5、G5、DS15_v2、D15_v2） | 可延迟到 Service Fabric 群集批准 | 每个 UD 可以暂停 2 小时，提供额外的时间让副本从之前的故障中恢复 |
| 银卡           | 5                              | 单核或更多核心的 VM，至少 50 GB 的本地 SSD                      | 可延迟到 Service Fabric 群集批准 | 任何时候都无法延迟                                                    |
| 铜卡           | 1                              | VM，至少 50 GB 的本地 SSD                                              | 不会因为 Service Fabric 群集延迟           | 任何时候都无法延迟                                                    |
...
- 采用更安全的方式进行 VM SKU 更改（向上/向下扩展）：更改虚拟机规模集的 VM SKU 需要许多步骤和注意事项。 可以遵循以下过程来避免常见问题。
    - **对于非主节点类型：** 建议创建新的虚拟机规模集，修改服务放置约束以包括新的虚拟机规模集/节点类型，然后将旧的虚拟机规模集实例计数降低到零，一次一个节点（这是为了确保删除节点不会影响群集的可靠性）。
    - **对于主节点类型：** 如果您选择的 VM SKU 处于容量状态，并且希望更改为更大的 VM SKU，请按照我们关于[主节点类型的垂直缩放的指导](https://docs.microsoft.com/azure/service-fabric/service-fabric-scale-up-node-type)进行。 
...
* [节点类型与虚拟机规模集的关系](service-fabric-cluster-nodetypes.md)
