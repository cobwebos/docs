...
> [!NOTE]
> 将主节点类型减少到小于最小数量会使群集不稳定，甚至使其关闭。 这可能会对应用程序和系统服务造成数据丢失。
> 
> 
...
> [!NOTE]
> 添加 VM 需要时间，请不要期待马上就有结果。 请提前规划增加容量的时间，在副本/服务实例可以使用 VM 容量之前留出超过 10 分钟的时间让一切就绪。
> 
...
Service Fabric 系统服务在群集中以主节点类型运行。 横向缩减主节点类型时，切勿在横向缩减实例数时使其少于[可靠性层](service-fabric-cluster-capacity.md)所需的数目。 
...
对节点执行这三个步骤后，即可将其从规模集中移除。 如果使用除[铜][durability]以外的任意持续性层，在移除规模集实例时会完成这些步骤。
...
Service Fabric Explorer 中列出的节点是 Service Fabric 系统服务（特别是 FM）在群集具有的节点数方面所了解的信息的反映。 横向缩减虚拟机规模集时，VM 已删除，但是 FM 系统服务仍然认为节点（映射到已删除的 VM）会恢复。 因此 Service Fabric Explorer 会继续显示该节点（尽管运行状况状态可能是错误或未知）。
