...
* **运营**Service Fabric 和群集执行的高级操作，包括即将启动的节点的事件、正在部署的新应用程序或升级回滚等。在此处[查看事件的完整](service-fabric-diagnostics-event-generation-operational.md)列表。  
...
* **使用代理**  
这是从计算机中收集性能的首选方法，因为代理通常有可以收集的可能性能指标列表，并且选择要收集或更改的指标是一个相对简单的过程。 有关 Azure 监视器的 Azure 监视器在 Service Fabric 的[Azure 监视器日志](service-fabric-diagnostics-event-analysis-oms.md)中提供 Azure 监视器日志，并[设置日志分析代理](../log-analytics/log-analytics-windows-agent.md)以了解有关日志分析代理的更多信息，该代理是能够获取群集 VM 和已部署容器的性能数据的监视代理之一。
...
* 阅读有关服务结构的[Azure 监视器日志集成](service-fabric-diagnostics-event-analysis-oms.md)，以收集群集诊断并创建自定义查询和警报
