...
默认情况下，产品收集 Service Fabric 使用情况的遥测数据来改进产品。 作为安装程序一部分运行的最佳做法分析器检查是否连接到[https://vortex.data.microsoft.com/collect/v1](https://vortex.data.microsoft.com/collect/v1)。 如果不可访问，安装会失败，除非选择不使用遥测。

1. 遥测管道每天尝试将以下数据上传到[https://vortex.data.microsoft.com/collect/v1](https://vortex.data.microsoft.com/collect/v1)一次。 这是尽力而为的上传，不会影响群集功能。 仅从运行故障转移管理器的主节点发送遥测数据。 其他节点不发送遥测数据。
2. 遥测数据由以下项组成：
