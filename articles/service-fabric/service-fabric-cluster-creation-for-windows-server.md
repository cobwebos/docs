...
### <a name="step-1-create-the-cluster"></a>第 1 步：创建群集

#### <a name="scenario-a-create-an-unsecured-local-development-cluster"></a>方案 A：创建不安全的本地开发群集
...
2. 创建群集：运行*CreateServiceFabricCluster.ps1*脚本，以在配置中的每台计算机上部署服务结构群集。 
    ```powershell
...
#### <a name="scenario-c-create-an-offline-internet-disconnected-cluster"></a>方案 C：创建脱机（互联网断开连接）群集
...
[Service Fabric Explorer](service-fabric-visualizing-your-cluster.md) 是一项很好的工具，适用于可视化群集和管理应用程序。  服务结构资源管理器是在群集中运行的服务，您可以通过导航到[http://localhost:19080/Explorer](http://localhost:19080/Explorer)访问 使用浏览器访问该服务。
...
默认情况下，产品收集 Service Fabric 使用情况的遥测数据来改进产品。 作为设置的一部分运行的最佳做法分析器检查与[https://vortex.data.microsoft.com/collect/v1](https://vortex.data.microsoft.com/collect/v1)的连接。 如果不可访问，安装会失败，除非选择不使用遥测。

1. 遥测管道尝试每天将以下数据上载到[https://vortex.data.microsoft.com/collect/v1](https://vortex.data.microsoft.com/collect/v1)一次。 这是尽力而为的上传，不会影响群集功能。 仅从运行故障转移管理器的主节点发送遥测数据。 其他节点不发送遥测数据。
2. 遥测数据由以下项组成：
