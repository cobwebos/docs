...
服务的 `InstanceCount` 指示需要运行的所需实例数。 当任何（或全部）实例发生故障时，Service Fabric 会通过在其他节点上自动创建替换实例来做出响应。 Service Fabric 将继续创建替换项，直至服务恢复到所需的实例计数。
另一个示例，如果无状态服务具有 `InstanceCount`-1，这意味着应在群集的每个节点上运行一个实例。 如果其中一些实例发生故障，Service Fabric 将检测该服务是否处于所需状态，并且将尝试在缺少它们的节点上创建实例。

#### <a name="stateful-services"></a>有状态服务
有两种类型的有状态服务：
1. 具有持久化状态的有状态
2. 具有非持久化状态的有状态（状态存储在内存中）

有状态服务故障恢复取决于有状态服务的类型以及服务具有多少个副本以及多少个副本失败。

什么是仲裁？
在有状态服务中，将在副本（Primary 和次要副本）之间复制传入数据。 如果大多数副本都接收到数据，则会将数据视为已提交仲裁（对于5个副本，3个为 "_仲裁_"）。 这意味着，在任何给定的时间，都可以保证至少具有最新数据的副本仲裁。 如果副本失败（假设2个副本中有2个副本失败），我们可以使用仲裁值计算是否可以恢复（由于5个副本中还有3个副本仍在运行，保证至少有1个副本具有完整的数据）。

什么是仲裁丢失？
当副本的仲裁失败时，分区被声明为处于仲裁丢失状态。 假设一个分区有5个副本，这意味着至少有3个副本具有完整的数据。 如果副本的仲裁（3出5）出现故障，则 Service Fabric 无法确定剩余副本（2 out 5）是否有足够的数据来还原分区。
...
2. 确定仲裁丢失是否具有永久性
   - 大多数情况下，故障是暂时性的。 重启进程、节点和 VM，网络分区修复。 但是，故障有时是永久性的。 
     - 对于没有永久性状态的服务，仲裁或多个副本故障会立即导致永久性仲裁丢失。 当 Service Fabric 检测到有状态非持久性服务的仲裁丢失时，它将通过声明（潜在）数据丢失来立即继续执行步骤3。 继续进行数据丢失非常有意义，因为 Service Fabric 知道没有必要再等待副本回来，因为即使是因为服务的非持久化性质，此数据也会丢失。
     - 对于有状态持久性服务，仲裁或更多副本的故障会导致 Service Fabric 等待副本恢复并还原仲裁。 这会对服务的受影响分区（或“副本集”）的任何写入造成服务中断。 但是，仍有可能可以读取，其一致性保证降低。 Service Fabric 等待仲裁恢复的默认时间是无限的，因为处理是（潜在的）数据丢失事件并伴有其他风险。
3. 确定有无实际数据丢失，并从备份还原
   ...
   1. 首先，记录 `OnDataLossAsync` 已被触发，并发出任何必要的管理警报。
   1. 此时，通常需要暂停并等待进一步决策和要采取的手动操作。 这是因为即使备份可用，也可能需要时间准备。 例如，如果两个不同的服务协调信息，则可能需要修改这些备份，以确保发生还原后，这两个服务所关注的信息一致。 
   1. 通常还有一些其他遥测或服务消耗。 此元数据可能包含在其他服务或日志中。 此信息可用于确定主要副本是否收到并处理了任何调用，这些调用未存在于备份中或未复制到此特定副本中。 可能需要重播或向备份添加这些调用才能进行恢复。  
   1. 将剩余副本的状态与任何可用备份中所含的状态进行比较。 如果使用 Service Fabric 可靠集合，则可使用工具和进程执行此操作，如[本文](service-fabric-reliable-services-backup-restore.md)所述。 目标是查看副本中的状态是否充足，或查看副本可能丢失的内容。
   1. 完成比较以及必要的还原后，如果更改了任何状态，则服务代码应返回 true。 如果副本确定它是该状态的最佳可用副本，并且未执行任何更改，则返回 false。 True 表示任何其他剩余副本现在可能与此副本不一致。 应删除这些副本并根据此副本重新生成。 False 表示未进行任何状态更改，因此其他副本可以保留其所含内容。 
...
由于暂时性故障，副本可能会间歇停机。 等待一段时间，Service Fabric 会尝试将其打开。 如果副本的运行时间超过预期持续时间，请遵循以下故障排除步骤。
- 副本可能会崩溃。 检查副本级别运行状况报告和应用程序日志。 收集故障转储并采取必要的措施进行恢复。
- 副本进程可能已停止响应。 检查应用程序日志以验证这一点。 收集进程转储，然后终止无响应进程。 Service Fabric 将创建替换过程，并尝试将副本重新导入。
- 承载副本的节点可能已关闭。 重新启动基础 VM 以使节点启动。

在某些情况下，无法恢复副本例如，驱动器可能出现故障或计算机物理上没有响应。 在这些情况下，需要告知 Service Fabric 不等待副本恢复。
如果使服务处于联机状态，则不能使用这些方法，在这种情况下，应该尽力恢复物理计算机。

以下步骤可能会导致数据丢失-请确保在以下步骤之前：
   
> [!NOTE]
> 对于特定的分区，使用这些方法的方法_不是使用_目标方法。 
>

- 使用 `Repair-ServiceFabricPartition -PartitionId` 或 `System.Fabric.FabricClient.ClusterManagementClient.RecoverPartitionAsync(Guid partitionId)` API。 此 API 允许指定要从 QuorumLoss 恢复的分区 ID，但可能会丢失数据。
- 如果你的群集遇到导致服务进入仲裁丢失状态的频繁故障，可能_会有数据丢失_，则指定适当的[QuorumLossWaitDuration](https://docs.microsoft.com/powershell/module/servicefabric/update-servicefabricservice?view=azureservicefabricps)值可帮助你的服务自动恢复。 在执行恢复之前，Service Fabric 将等待提供的 QuorumLossWaitDuration （默认为无限）。 _不建议使用_此方法，因为这可能会导致意外的数据丢失。
...
在极少数情况下，物理数据中心可能会由于电源或网络连接中断而暂时不可用。 在这些情况下，Service Fabric 群集和该数据中心或 Azure 区域中的服务不可用。 但会保留你的数据。 对于在 Azure 中运行的群集，可以在[azure 状态页][azure-status-dashboard]上查看有关中断的更新。 物理数据中心遭到部分或完全损坏（这种情况很少见）时，托管在此处的任何 Service Fabric 群集或其中的服务可能会丢失。 这包括未在该数据中心或区域外部备份的任何状态。
...
Service Fabric 具有种子节点的概念。 种子节点可以维护基础群集的可用性。 这些节点有助于通过在某些类型的网络故障期间，与其他节点建立租约并充当决胜属性来确保群集保持启动状态。 如果随机故障删除了群集中的大部分种子节点，并且未将其恢复，则群集联合环会折叠，因为丢失了种子节点仲裁并且群集出现故障。 在 Azure 中，Service Fabric 资源提供程序管理 Service Fabric 群集配置，并且默认情况下，在主节点类型的容错域和升级域之间分配种子节点;如果将主节点标记为 "白银" 或 "黄金" 持续性，则当你通过在主 nodetype 中进行缩放或手动删除种子节点删除种子节点时，群集将尝试从可用的主节点升级另一个非种子节点如果你的可用容量低于你的主节点类型所需的群集可靠性级别，容量将会失败。
