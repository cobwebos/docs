...
![Service Fabric Explorer 中的容错域分布的节点][sfx-cluster-map]
</center>
...
Service Fabric 具有种子节点的概念。 种子节点可以维护基础群集的可用性。 这些节点有助于通过在某些类型的网络故障期间，与其他节点建立租约并充当决胜属性来确保群集保持启动状态。 如果随机故障删除群集中的大部分种子节点，并且无法将其后，群集联合身份验证令牌环会折叠，因为丢失了种子节点仲裁和群集失败。 在 Azure 中，Service Fabric 资源提供程序管理 Service Fabric 群集配置，并在主节点类型的容错域和升级域; 默认情况下分配种子节点如果主 nodetype 被标记为银级或是金级持久性，当您删除种子节点，通过在主 nodetype 中缩放或手动删除种子节点，群集将尝试从主节点类型可提升另一个非种子节点容量，并且如果有不好用的容量与能力群集可靠性级别需要主节点类型将失败。
