...
* 在服务结构节点上运行但不作为服务交换矩阵*服务实现的内部*监视器。
* 从 Service Fabric 群集*外部*探测资源的外部监视程序（例如，监视服务（如 Gomez）。
...
* **运行状况报告发送间隔**：从将报表添加到客户端的时间到发送到运行状况管理器的时间之间的延迟。 用于在单条消息中批处理报告，而不会针对每个报告发送一条消息。 批处理可以提升性能。 默认值：30 秒。
* **运行状况报告重试发送间隔**：运行状况客户端向运行状况管理器重新发送累积运行状况报告的时间间隔。 默认值：30 秒，最小：1 秒。
* **运行状况操作超时**：发送给运行状况管理器的报告消息的超时时间。 如果消息超时，运行状况客户端就会不断重试，直到运行状况管理器确认报告已处理为止。 默认值：2 分钟。
...
客户端上的缓冲会将报告的唯一性纳入考虑范围。 例如，如果特定的错误报告器针对相同实体的相同属性每秒产生 100 个报告，则会以最后一个版本取代所有报告。 客户端队列中最多存在一个这样的报表。 如果配置了批处理，则发送到运行状况管理器的报告数目仅为每个发送间隔发送一份报告。 这是最后添加的报告，可反映实体的最新状态。
...
确定监视器详细信息后，应该确定可唯一标识它的源 ID。 如果多个相同类型的监视器存留于群集中，它们必须报告不同的实体，如果它们报告相同的实体，请使用不同的源 ID 或属性。 如此报告才可并存。 运行状况报告的属性应捕获受监视的条件。 （对于上面的示例，属性可以是**共享大小**。）如果多个报表应用于同一条件，则属性应包含一些允许报表共存的动态信息。 例如，如果需要监视多个共享，属性名称可以是 **ShareSize-sharename**。
...
[服务结构运行状况监测简介](service-fabric-health-introduction.md)
