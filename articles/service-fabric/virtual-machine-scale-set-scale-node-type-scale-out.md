...
向现有群集添加新节点类型（由虚拟机缩放集支持）类似于[升级主节点类型](service-fabric-scale-up-node-type.md)，但您不会使用相同的 NodeTypeRef;显然不会禁用任何主动使用的虚拟机扩展集，如果不更新主节点类型，也不会丢失群集可用性。 
