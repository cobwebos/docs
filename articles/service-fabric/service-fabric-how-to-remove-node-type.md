...
7. 删除 Service Fabric 节中的节点类型。
    ...
    - 查找与 Service Fabric 部分中的节点类型相关的部分。
    - 删除与节点类型对应的部分。
    - 仅适用于银色和更高耐用性群集，更新模板中的群集资源，并配置运行状况策略，以忽略 fabric：/System `applicationDeltaHealthPolicies`应用程序运行`properties`状况，方法是在群集资源下添加，如下所示。 下面的策略应忽略现有错误，但不允许新的运行状况错误。 
    ...
    - 部署已修改的 Azure 资源管理器模板。 * * 此步骤将需要一段时间，通常长达两个小时。 此升级会将设置更改为 InfrastructureService，因此需要重新启动节点。 在这种情况`forceRestart`下，将忽略。 
