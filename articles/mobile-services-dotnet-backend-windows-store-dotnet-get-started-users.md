...
<li><p>可选）完成[注册 Windows 应用商店应用程序包以进行 Microsoft 身份验证][注册 Windows 应用商店应用程序包以进行 Microsoft 身份验证]中的步骤。</a>.</p>
...
<ol start="3">
<li><p>在 Visual Studio 2013 中，打开你在完成[移动服务入门][移动服务入门]教程后创建的项目。</p></li>
...
> [WACOM.NOTE] 如果你向移动服务注册了 Windows 应用商店应用程序包信息，则应通过为 *useSingleSignOn* 参数提供值 "true" 来调用 [LoginAsync][LoginAsync] 方法。如果你不执行此操作，则每次调用 login 方法时，系统仍会向你的用户提供登录提示。
