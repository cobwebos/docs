...
1. 若要手动安装模块，请下载源代码并将项目目录的内容解压缩到`$env:ProgramFiles\WindowsPowerShell\Modules folder`。 或者，运行以下命令，使用 PowerShellGet 从 PowerShell 库安装（在 PowerShell 5.0 中）：
...
1. 在 PowerShell 控制台中，导航到保存该`.ps1`文件的文件夹。
...
3. 这会`localhost.mof file`在名为`C:\dsc`的新文件夹中创建。
