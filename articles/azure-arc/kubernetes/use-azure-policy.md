...
2. 选择“+ 策略定义”。
3. 将“定义位置”设置为订阅或管理组。  这会确定可使用策略定义的最广泛范围。
4. 为策略提供“名称”和“说明”。
5. 在类别下，选择“新建”，并写入“Kubernetes 群集 - Azure Arc”
6. 在“策略规则”编辑框中，复制/粘贴此[示例策略定义](https://raw.githubusercontent.com/Azure/arc-k8s-demo/master/policy/Ensure-GitOps-configuration-for-Kubernetes-cluster.json)的内容。
7. 保存。

完成使其成为内置策略的工作后，便不需要创建自定义策略定义的这一步骤。
...
2. 找到刚创建的定义，并选择它。
3. 在页面操作中，选择“分配”。
4. 将“范围”设置为将应用策略分配的管理组、订阅或资源组。
5. 如果要从策略范围中排除任何资源，请设置“排除”。
6. 为策略分配提供可以用于轻松识别它的“名称”和“说明”。
7. 确保“策略实施”设置为“已启用”。
8. 选择“**下一页**”。
9. 设置将在 `sourceControlConfiguration` 创建过程中使用的参数值。
10. 选择“**下一页**”。
11. 启用“创建修正任务”。
12. 确保选中“创建托管标识”，并确保标识具有“参与者”权限。  有关所需权限的详细信息，请参阅[此文档](https://docs.microsoft.com/azure/governance/policy/assign-policy-portal)和[此文档中的注释](https://docs.microsoft.com/azure/governance/policy/how-to/remediate-resources)。
13. 选择“查看 + 创建”。
...
1. 在 Azure 门户中，导航到你的某个 `connectedCluster` 资源，然后在边栏的“设置”部分中，选择“策略”。 （AKS 托管群集的 UX 尚未实现，但即将推出。）
