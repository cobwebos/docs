...
* 安装支持 Azure Arc 的 Kubernetes CLI 扩展需要 Azure CLI 版本 2.3 +。 [安装 Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli?view=azure-cli-latest)或更新到最新版本，以确保 Azure CLI 版本 2.3 +。
...
* **使用 Azure CLI 删除**：以下 Azure CLI 命令可用于启动对已启用 Azure Arc Kubernetes 资源的删除。
  ...
  ```
  这会删除 `Microsoft.Kubernetes/connectedCluster` Azure 中的资源和任何关联的 `sourcecontrolconfiguration` 资源。 Azure CLI 使用 helm uninstall 删除群集上运行的代理。
