...
### <a name="q-will-my-data-box-disk-cross-country-borders-during-shipping"></a>Q. 我的 Data Box Disk 在运输过程中是否将跨越国家/地区边界？
...
A. 随 Data Box Disk 提供的工具集包含三个工具：
 - **Data Box Disk 解锁工具**：使用此工具解锁从 Microsoft 提供的加密磁盘。 使用此工具解锁磁盘时，需要提供从 Azure 门户中的 Data Box Disk 订单中获得的密钥。 
 - **Data Box Disk 验证工具**：使用此工具根据 Azure 命名约定验证大小、格式和 blob 名称。 它还生成复制的数据的校验和，然后使用校验和来验证上传到 Azure 的数据。
 - **Data Box Disk 剥离复制工具**：使用此工具时，使用多个磁盘时，需要在所有磁盘之间进行拆分和复制的大型数据集。 此工具当前可用于 Windows。 不支持将此工具与托管磁盘配合使用。 此工具也会在复制数据时进行验证，因此可以在使用此工具时跳过验证步骤。
...
A. 支持 Data Box Disk Azure 文件，但无法与 Azure 文件同步完美配合使用。如果文件数据与 Azure 文件同步一起使用，则不保留元数据。
...
A.  数据复制完成后，可以运行 `DataBoxDiskValidation.cmd`DataBoxDiskImport*文件夹中提供的* 来生成校验和，以用于验证。 如果有多个磁盘，则需要针对每个磁盘打开命令窗口，并运行此命令。 请注意，根据具体的数据大小，此操作可能需要花费很长时间（大约数小时数）。
...
### <a name="q-i-copied-data-to-manageddisk-folder-i-dont-see-any-managed-disks-with-the-resource-group-specified-for-managed-disks-was-my-data-uploaded-to-azure-and-how-can-i-locate-it"></a>Q. 我将数据复制到了 ManagedDisk 文件夹中。 我看不到任何托管磁盘有为托管磁盘指定的资源组。 我的数据是否已上传到 Azure？如何才能找到它？
