...
---
# <a name="azure-data-box-disk-frequently-asked-questions"></a>Azure 数据框磁盘：常见问题
...
A. 随 Data Box Disk 提供的工具集包含三个工具：
 - **数据盒磁盘解锁工具**：使用此工具解锁从微软发货的加密磁盘。 使用此工具解锁磁盘时，需要提供从 Azure 门户中的 Data Box Disk 订单中获得的密钥。 
 - **数据框磁盘验证工具**：使用此工具根据 Azure 命名约定验证大小、格式和 blob 名称。 它还生成复制的数据的校验和，然后使用校验和来验证上传到 Azure 的数据。
 - **数据盒磁盘拆分复制工具**：当您使用多个磁盘并且具有需要拆分和复制到所有磁盘的大型数据集时，请使用此工具。 此工具当前可用于 Windows。 不支持将此工具与托管磁盘配合使用。 此工具也会在复制数据时进行验证，因此可以在使用此工具时跳过验证步骤。
...
A. 数据框磁盘支持 Azure 文件，但不适用于 Azure 文件同步。如果文件数据与 Azure 文件同步一起使用，则不保留元数据。
