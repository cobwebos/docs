...
|错误消息  |建议的操作 |
|---------|---------|
|AzCopy 命令在显示以下错误之前，似乎挂起了 20 分钟： <br>分析源位置 `https://<accountname>.blob.<serialnumber>.microsoftdatabox.com/<cntnr>` 出错。 没有此类设备或地址|检查是否已在 `/etc/hosts` 将终结点名称 `<accountname>.blob.<serialnumber>.microsoftdatabox.com` 添加到 hosts 文件。|
