...
A. 是的。 可以同时连接到 Data Box 上的两个 10-GbE 接口来复制数据。 有关如何复制数据的详细信息，请转到[教程：将数据复制到 Azure 数据框](data-box-deploy-copy-data.md)。
...
A. 如果系统故障指示器 LED 灯亮起，则表示系统不正常。 [有关后续步骤，请与 Microsoft 支持部门联系](data-box-disk-contact-microsoft-support.md)。
...
### <a name="q-can-i-pick-up-my-data-box-order-myself-can-i-return-the-data-box-via-a-carrier-that-i-choose"></a>Q. 我可以自己拿起数据框订单吗？ 我能否通过您选择的运营商返回数据盒？
A. 是的。 Microsoft 还仅在 US Gov 区域提供自我管理的发货。 下数据盒订单时，您可以选择自管理的装运选项。 要拿起数据盒设备，请执行以下步骤：
    
1. 下订单后，将处理订单并准备数据框。 将通过电子邮件通知你订单已准备好进行提货。 
...
1. 一旦数据副本完成且没有错误，请运行 **"准备发货**"。 准备完成后，您将在设备的本地 Web UI 中收到一个代码。 复制并保存代码。
2. 关闭设备并拆下连接电缆。
3. 将设备随附的电源线卷好并安全地放在设备后面。
4. 向[Azure 数据框操作团队](mailto:adbops@microsoft.com)发送电子邮件，并为他们提供之前保存的代码。 他们为您提供有关何时何地放下设备的信息。
