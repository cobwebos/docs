...
A. 是的。 Microsoft 还仅在 US Gov 区域提供自我管理的发货。 在下 Data Box 订单时，可以选择“自我管理的发货”选项。 若要提取 Data Box 设备，请执行以下步骤：
    
1. 下完订单后，将处理订单并准备好 Data Box。 将通过电子邮件通知你订单已准备好进行提货。 
...
数据复制完成后，请执行以下步骤来退回设备：

1. 数据复制完成且无错误后，运行“准备交付”。 准备完成后，将在设备的本地 Web UI 中收到一个代码。 复制并保存该代码。
2. 关闭设备并拔下连接电缆。
3. 将设备随附的电源线卷好并安全地放在设备后面。
4. 向 [Azure Data Box 运营团队](mailto:adbops@microsoft.com)发送电子邮件，并向他们提供你之前保存的代码。 他们会为你提供有关何时何地投递设备的信息。
