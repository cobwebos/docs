...
description: 描述如何使用本地 web UI 管理 Data Box 和 Data Box Heavy 设备
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
---
# <a name="use-the-local-web-ui-to-administer-your-data-box-and-data-box-heavy"></a>使用本地 web UI 管理你的 Data Box 和 Data Box Heavy

本文介绍了可以在 Data Box 和 Data Box Heavy 设备上执行的一些配置和管理任务。 你可以通过 Azure 门户 UI 和设备的本地 web UI 来管理 Data Box 和 Data Box Heavy 设备。 本文重点介绍可使用本地 Web UI 执行的任务。

Data Box 和 Data Box Heavy 的本地 web UI 用于设备的初始配置。 你还可以使用本地 web UI 关闭或重新启动设备、运行诊断测试、更新软件、查看复制日志，并为 Microsoft 支持部门生成日志包。 在具有两个独立节点的 Data Box Heavy 设备上，你可以访问与设备的每个节点对应的两个单独的本地 web Ui。
...
你可以使用本地 web UI 关闭或重新启动设备。 在重启之前，建议使共享依次在主机和设备上脱机。 这能在最大程度上减少发生数据损坏的可能性。 关闭设备时，请确保当前未执行数据复制。
...
物料清单（BOM）或清单文件包含复制到 Data Box 或 Data Box Heavy 的文件的列表。 这些文件是在你准备要发运的设备时生成的。

在开始之前，请确保设备已完成**准备交付**步骤。 遵循以下步骤下载 BOM 或清单文件：

1. 中转到设备的本地 web UI。 你将看到设备已完成准备交付。 设备准备完成后，设备状态将显示为“已准备好寄送”。
...
> [!NOTE]
> 在 Data Box Heavy 上，会有两组文件列表（BOM 文件）对应于设备上的两个节点。
...
此文件包含在 Data Box 或 Data Box Heavy 上复制的所有文件的列表。 在此文件中，*crc64* 值与针对相应文件生成的校验和相关。
...
- 了解如何[通过 Azure 门户管理 Data Box 和 Data Box Heavy](data-box-portal-admin.md)。
