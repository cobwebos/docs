...
## <a name="data-box-service-limits"></a>Data Box 服务限制
 ...
 - Data Box 服务在 "[区域可用性](data-box-disk-overview.md#region-availability)" 列出的 Azure 区域中提供。
 - Data Box 磁盘支持单个存储帐户。
...
- 不要直接将数据复制到磁盘中。 将数据复制到预先创建的*BlockBlob*、*PageBlob*和*AzureFile*文件夹中。
- *BlockBlob* 和 *PageBlob* 下的文件夹是容器。 例如，容器创建为 *BlockBlob/container* 和 *PageBlob/container*。
- 如果云中存在与要复制的对象同名的 Azure 对象（如 blob），Data Box Disk 将在云中将该文件重命名为文件（1）。
...
- 将数据上传到 Azure 文件时，不会保留文件元数据和 NTFS 权限。 例如，在复制数据时，不会保留文件的*上次修改*的属性。
...
|-------------------|-----------------------------------------------------------|
| 块 Blob        | ~ 4.75 TiB                                                 |
| 页 blob         | 8 TiB <br> （每个上传的文件的格式必须为512字节，否则上传会失败。 <br> VHD 和 VHDX 的对齐方式均为512字节。） |
...
| 实体 | 约定                                             |
|-------------------|-----------------------------------------------------------|
| 托管磁盘名称       | <li> 名称长度必须介于1到80个字符之间。 </li><li> 名称必须以字母或数字开头，以字母、数字或下划线结尾。 </li><li> 该名称只能包含字母、数字、下划线、句点或连字符。 </li><li>   名称不应包含空格或 `/`。                                              |
