---
title: 对 Azure Data Box，Azure 数据大量框上的问题进行故障排除 |Microsoft Docs
description: 介绍如何将数据复制到这些设备时看到在 Azure Data Box 和 Azure 数据框大量的问题进行疑难解答。
services: databox
author: alkohli
ms.service: databox
ms.subservice: pod
...
---
# <a name="troubleshoot-issues-related-to-azure-data-box-and-azure-data-box-heavy"></a>对与 Azure Data Box 和 Azure 数据框大量相关的问题进行故障排除

本文详细介绍了使用 Azure 数据 Boxn 或 Azure 数据框高时可能会看到如何对问题进行故障排除信息。
...
**建议的解决方法：** 数据已复制到其中的数据框或数据框高 share(SMB/NFS) 下的文件夹将成为你的存储帐户中的 Azure 容器。 

- 上**连接和复制**页设备本地 web UI、 下载和查看问题的错误文件，以确定该文件夹名称。
- 更改，确保在数据框或数据框大量共享下的文件夹名称：
...
**建议的解决方法：** 上**连接和复制**页上的本地 web UI、 下载和查看的错误文件。
...
**建议的解决方法：** 在数据框或数据框高仅支持这些文件是 512 字节的页 blob 共享对齐 （例如 VHD/VHDX）。 复制到页 blob 共享任何数据作为页 blob 上载到 Azure。
