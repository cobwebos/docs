...
你可能需要更改订单状态电子邮件收件人用户。 例如，当设备已妥投或提货时，需要告知某个用户。 另一个用户可能需要在数据复制已完成，以便其可以验证数据源中删除它之前是 Azure 存储帐户中时得到通知。 在这种情况下，可以编辑通知详细信息。
...
|已完成，但有错误| 数据复制已完成，但在复制期间发生错误。 <br> 请使用 Azure 门户中提供的路径查看复制日志。   |
