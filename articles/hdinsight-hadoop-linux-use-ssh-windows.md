...
* 适用于 Windows 客户端的 **PuTTY** 和 **PuTTYGen**。这些实用程序可通过 <a href="http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html" target="_blank">http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html</a> 获得。
...
上述每种方法都将需要公钥。有关创建基于 Linux 的 HDInsight 群集的完整信息，请参阅<a href="/documentation/articles/hdinsight-hadoop-provision-linux-clusters/" target="_blank">设置基于 Linux 的 HDInsight 群集</a>。
...
1. 从 <a href="http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html" target="_blank">http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html</a> 安装 Pageant。此实用程序用于缓存 PuTTY 的 SSH 密钥。
...
3. 在"浏览"对话框出现时，选择包含密钥的 .ppk 文件，然后单击**"打开"**。此时将向 Pageant 添加密钥，在连接到群集时，Pageant 将向 PuTTY 提供该密钥。
    ...
    > [AZURE.IMPORTANT] 如果你使用了 SSH 密钥保护帐户，则必须完成以前的步骤，然后才能连接到从节点。
...
7. 在建立连接后，使用以下方式检索你的群集节点列表。将  *ADMINPASSWORD* 替换为你的群集管理员帐户的密码。将  *CLUSTERNAME* 替换为你的群集名称。
...
9. 在建立会话后，你的 PuTTY 会话的提示符将从  `username@headnode` 更改为  `username@workernode`，指示你已连接到从节点。你此时运行的任何命令都将在从节点上运行。

10. 在从节点上执行完操作后，使用  `exit` 命令来关闭从节点的会话。这将使你返回到  `username@headnode` 提示符。
...
在 HDInsight 群集上访问使用群集中头节点或辅助节点内部域名的基于 Web 的服务时，此功能特别有用。例如，Ambari 网页上的某些部分使用类似于 **headnode0.mycluster.d1.internal.chinacloudapp.cn** 的内部域名。这些名称无法从群集外部解析，但是，通过 SSH 传送的请求将源自于群集内部，并可正确解析。

使用以下步骤创建 SSH 隧道，并将浏览器配置为使用该隧道连接到群集：

1. 打开 PuTTY，并输入你的连接信息，如以前的[连接到基于 Linux 的群集](#connect-to-a-linux-based-hdinsight-cluster) 部分中所述。
...
4. 选择**"URL 模式"**选项卡，然后选择**"添加新模式"**。使用以下设置来定义模式，然后单击**"确定"**：
	...
	* **URL 模式** - **\*headnode\*** - 这可以定义将任何 URL 与其中包含的单词 **headnode** 匹配的模式。
