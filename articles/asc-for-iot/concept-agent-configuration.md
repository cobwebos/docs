...
ms.locfileid: "58541912"
---
# <a name="configure-security-agents"></a>配置安全代理
...
本文提供如何配置适用于使用的代理与 ASC 有关 IoT 的说明。
...
IoT 安全代理相关的属性位于的 azureiotsecurity 模块所需的属性部分中的代理配置对象，每个 ASC。 

若要修改的配置，创建和修改在 azureiotsecurity 模块孪生标识此对象。 如果代理配置对象不存在 azureiotsecurity 模块孪生中，所有安全代理属性值都设置为默认值。 
...
所有自定义属性必须设置内部 azureiotsecurity 模块孪生中的代理配置对象。 

设置属性覆盖默认值。 若要设置的属性，添加到与所需的值的配置对象的属性键。 
...
|系统信息 |eventPrioritySystemInformation |低 |真 |系统信息的快照 (例如：操作系统或 CPU）。| 
