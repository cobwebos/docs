...
用于 IoT 的 azure 安全中心会扫描 Azure 资源和 IoT 设备，并提供安全建议来减小攻击面。
...
| 严重性 | 名称                                                      | “数据源” | 说明                                                                                                                                                                                           |
|----------|-----------------------------------------------------------|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 中   | 打开设备上的端口                                      | Agent       | 在设备上找到侦听终结点。                                                                                                                                                        |
| 中   | 在其中一个链中找到的可许可防火墙策略。 | Agent       | 已找到允许的防火墙策略（输入/输出）。 默认情况下，防火墙策略应拒绝所有流量，并定义规则以允许与设备进行必要的通信。                               |
| 中   | 找到输入链中的可许可防火墙规则     | Agent       | 已发现防火墙中包含广泛 IP 地址或端口的许可模式的规则。                                                                                    |
| 中   | 找到输出链中的可许可防火墙规则    | Agent       | 已发现防火墙中包含广泛 IP 地址或端口的许可模式的规则。                                                                                   |
...
| 严重性 | 名称                                    | “数据源” | 说明                                                                       |
|----------|-----------------------------------------|-------------|-----------------------------------------------------------------------------------|
| 低      | 代理发送未利用消息          | Agent       | 10% 或更多的安全消息在过去24小时内小于 4 KB。  |
| 低      | 安全克隆配置不最佳 | Agent       | 安全克隆配置不是最佳的。                                        |
| 低      | 安全克隆配置冲突    | Agent       | 安全克隆配置中标识了冲突。 |                          |
|
...
| 中   | 默认 IP 筛选器策略应为 deny                  | IoT 中心     | IP 筛选器配置应为允许的流量定义规则，默认情况下，默认情况下，拒绝所有其他流量。                                                                                                     |
| 中   | IP 筛选器规则包含大型 IP 范围                   | IoT 中心     | 允许 IP 筛选规则源 IP 范围太大。 过度许可的规则可以将 IoT 中心暴露给恶意参与者。                                                                                       |
