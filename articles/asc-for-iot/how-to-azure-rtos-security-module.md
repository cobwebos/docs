...
| 名称 | 类型 | 默认 | 详细信息 |
| - | - | - | - |
| ASC_SECURITY_MODULE_ID | String | --- | 设备的唯一标识符  |
...
| ASC_COLLECTOR_NETWORK_ACTIVITY_CAPTURE_UNICAST_ONLY | 布尔 | true | 仅捕获单播传入数据包，设置为 false 时捕获也广播和多播 |
...
| 名称 | 类型 | 默认 | 详细信息 |
| - | - | - | - |
| collector_heartbeat_enabled | 布尔 | ON | 启用检测信号收集器 |
| collector_network_activity_enabled | 布尔 | ON | 启用网络活动收集器 |
| collector_system_information_enabled | 布尔 | ON | 启用系统信息收集器 |
...
尽管可选且不是必需的，但如果想要进一步调查设备事件和活动，启用和配置 Log Analytics 可能会很有帮助。 阅读有关如何 [在 Azure 安全中心 For IoT 服务](how-to-security-data-access.md#log-analytics) 中设置和使用 Log Analytics 以了解详细信息。 
