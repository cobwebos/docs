...
IoT 安全代理 Azure 安全中心从本地设备收集数据和系统事件，并将此数据发送到 Azure 云进行处理和分析。 安全代理收集多种类型的设备事件，包括新进程和新的连接事件。 新流程和新连接事件可能都会在一秒内合法地在设备上发生，虽然对于强大和全面的安全性很重要，但安全代理被迫发送的消息数可能快速达到或超过 IoT 中心配额和成本限制。 但是，这些事件包含非常有价值的安全信息，这些信息对于保护您的设备至关重要。
