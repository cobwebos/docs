...
时序见解环境是收集和存储设备数据的地方。 存储在时序见解环境中以后，[时序见解资源管理器](time-series-quickstart.md)和[时序见解查询 API](/rest/api/time-series-insights/ga-query-api) 就可以用来查询和分析数据。 IoT 中心是所有设备（模拟设备或物理设备）用来安全连接 Azure 云以及向其传输数据的连接点。 [时序见解概述](time-series-insights-overview.md)中指出，Azure IoT 中心还可充当事件源，用于将数据流式传输到时序见解环境。 本教程使用 [IoT 解决方案加速器](/azure/iot-accelerators/)来生成示例遥测数据并将其流式传输到 IoT 中心。
...
1. 在“时序见解环境”页上，填写必需的参数： 
   ...
   参数|说明
   ---|---
   **环境名称** | 为时序见解环境选择唯一名称。 该名称由时序见解资源管理器和查询 API 使用。
   **订阅** | 订阅是 Azure 资源的容器。 选择要为其创建时序见解环境的订阅。
   **资源组** | 资源组是 Azure 资源的容器。 为时序见解环境资源选择现有的资源组或创建新的资源组。
   **位置** | 为时序见解环境选择数据中心区域。 为了避免带宽成本和延迟的提高，最好是将时序见解环境与其他 IoT 资源保留在同一区域。
...
1. 转到针对时序见解环境创建的资源组的“概览”页，然后选择时序见解环境： 
...
1. 在“新建事件源”页上输入所需的参数。 
   ...
   **时间戳属性名称** | 指定为 `timestamp`。
   ...
   完成后，单击“创建”以添加事件源。  返回资源组的“概览”页时，会看到新的“时序见解事件源”资源以及时序见解环境资源。 
...
1. 在时序见解环境的“概览”页上单击“时序见解资源管理器 URL”，打开时序见解资源管理器。  
...
1. 时序见解资源管理器将会加载，并使用 Azure 门户帐户进行身份验证。 初始视图出现后，可以在图表区域中看到，时序见解环境中确实填充了模拟的遥测数据。 若要根据更小的时间范围进行筛选，请选择左上角的下拉列表。 然后输入足够大的时间范围，以涵盖设备模拟的整个持续时间。 然后单击搜索放大镜。
...
1. 缩小时间范围可在图表中放大 IoT 中心和时序见解环境中的不同数据传输高峰期。 另请注意右上角的“流式处理完成”文本，其中显示了找到的事件总数。  还可以拖动“间隔大小”滑块来控制图表上的绘图粒度。 
...
本教程创建了多个正常运行的 Azure 服务来支持时序见解环境和设备模拟解决方案。 如果你想要放弃和/或延期完成本系列教程，我们建议删除所有资源，以免产生不必要的费用。

在 Azure 门户的左侧菜单中：

1. 单击“资源组”图标，然后选择针对时序见解环境创建的资源组。  在页面顶部单击“删除资源组”，输入资源组的名称，然后单击“删除”。  
