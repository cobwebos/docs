...
ms.locfileid: "87046452"
---
# <a name="diagnose-and-solve-issues-in-your-azure-time-series-insights-gen1-environment"></a>诊断并解决 Azure 时序见解 Gen1 环境中的问题
...
### <a name="cause-c-the-provided-consumer-group-isnt-exclusive-to-azure-time-series-insights"></a>原因 C：提供的使用者组不是 Azure 时序见解所独有的

注册 IoT 中心或事件中心时，必须设置用于读取数据的使用者组。 不能共享此使用者组。 如果共享此使用者组，底层 IoT 中心或事件中心会随机自动断开某个读取者的连接。 为 Azure 时序见解提供唯一的使用者组，以供读取。

### <a name="cause-d-the-environment-has-just-been-provisioned"></a>原因 D：环境刚完成预配

数据将在环境中的几分钟内显示，在第一次创建数据后，数据将显示在 Azure 时序见解资源管理器中。
...
如果在创建包含数据的事件源之后预配了环境，则经常会发生[限制](time-series-insights-environment-mitigate-latency.md)问题。 Azure IoT 中心和 Azure 事件中心将数据存储最长 7 天。 Azure 时序见解总是以事件源（先进先出或*FIFO*）中的最早事件开始。

例如，如果在连接到 S1 （单单元 Azure 时序见解环境）时，事件源中存在5000000事件，Azure 时序见解将每日读取大约1000000事件。 它可能类似于 Azure 时序见解正在经历5天的延迟。 但是，实际原因是环境受到了限制。

如果事件源中有旧事件，可通过以下两种方式之一来解决限制：

- 更改事件源的保留限制，以帮助删除不想显示在 Azure 时序见解中的旧事件。
- 预配一个更大的环境大小（以单位数计），以提高旧事件的吞吐量。 在上面的示例中，如果在某一天将同一 S1 环境增大到 5 个单位，则环境在一天内就能赶上进度。 如果稳定状态事件的生产每日为1000000或更少事件，可以在 Azure 时序见解捕获后将事件容量减少到一个单元。
...
* 如果每个事件中心消息都平展到单个 Azure 时序见解事件，则不会发生限制。
* 如果每个事件中心消息都平展到 100 Azure 时序见解事件，则每分钟应引入4600事件。
...
1. 将 SKU 容量增大到允许的最大值（本例中为 10）。 增大容量后，流入进程很快就能开始赶上进度。 增加容量需要付费。 若要直观显示你的捕获速度，可以在[Azure 时序见解资源管理器](https://insights.timeseries.azure.com)中查看可用性图表。

2. 消除滞后问题之后，将 SKU 容量降低至正常流入速率。

## <a name="problem-data-was-showing-previously-but-is-no-longer-showing"></a>问题：以前显示数据，但现在不再显示

如果 Azure 时序见解不再引入数据，但事件仍流式传输到 Iot 中心或事件中心，则请考虑此可能的原因。

### <a name="cause-a-your-hub-access-key-was-regenerated-and-your-environment-needs-to-be-updated"></a>原因 A：重新生成了中心访问密钥，环境需要更新

如果创建事件源时提供的密钥不再有效，则会出现此问题。 你会在中心看到遥测数据，但 Azure 时序见解中不会收到任何入口接收的消息。 如果你不确定是否重新生成了密钥，可以在事件中心的活动日志中搜索“创建或更新命名空间授权规则”。 对于 IoT 中心，请搜索“创建或更新 IotHub 资源”。

若要用新密钥更新 Azure 时序见解环境，请在 Azure 门户中打开中心资源，然后复制新密钥。 请参阅 Azure 时序见解资源，并选择 "**事件源**"：
...
若要确保正确捕获时间戳属性名称并使其正常工作，最简单的方法是使用 Azure 时序见解资源管理器。 在 Azure 时序见解资源管理器中，使用图表，选择输入时间戳属性名称后的时间段。 右键单击选定项，然后选择“浏览事件”。

第一个列标头应是时间戳属性名称。 在单词 **Timestamp** 旁边，将显示 **($ts)** 。

不会显示以下值：

- *（abc）*：指示 Azure 时序见解正在将数据值作为字符串读取。
- *日历图标*：表示 Azure 时序见解正在将数据值读取为日期时间值。
- *#*：指示 Azure 时序见解正在将数据值作为整数读取。
