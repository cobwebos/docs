...
### <a name="learn-about-common-time-series-insights-customer-challenges-and-mitigationsbr"></a>了解常见的时间序列洞察客户挑战和缓解措施。</br>
...
在创建具有数据的事件源后预配环境时，[限制](time-series-insights-environment-mitigate-latency.md)是一个常见问题。 Azure IoT 中心和 Azure 事件中心将数据存储最长 7 天。 时序见解始终最先处理事件源中的最早事件（先进先出 (*FIFO*)）。
...
将基于环境 SKU 类型和容量对环境进行强制限制。 环境中的所有事件源都共享此容量。 如果 IoT 中心或事件中心的事件源将数据推送到强制限制之外，您将遇到限制和延迟。
...
为了高度了解拼合逻辑的工作原理，请阅读[支持的 JSON 形状](./how-to-shape-query-json.md)。

#### <a name="recommended-resolutions-for-excessive-throttling"></a>针对过度限制的建议解决方法

若要解决延迟问题，请增加环境的 SKU 容量。 有关详细信息，请阅读[缩放时间序列见解环境](time-series-insights-how-to-scale-your-environment.md)。
...
TSI 不再引入数据，但事件仍在流式传输到 Iot 中心或事件中心

### <a name="cause-a-your-hub-access-key-was-regenerated-and-your-environment-needs-updating"></a>原因 A：您的中心访问密钥已重新生成，您的环境需要更新
...
使用新键在 Azure 门户中打开中心资源并复制新密钥，请更新时间序列见解环境。 导航到 TSI 资源，然后单击事件源。 
...
- *（abc）*：指示时序见解正在将数据值读取为字符串。
- *日历图标*：指示时间序列见解正在将数据值读为*日期时间*。
- *#*：指示时序见解正在将数据值读为整数。
