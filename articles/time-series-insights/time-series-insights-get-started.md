...
description: 了解如何使用 Azure 门户创建新的时序见解环境。
...
1. 填写必需的参数。 下表解释了每个参数：
   ...
   [![创建时序见解资源组](media/time-series-insights-get-started/tsi-configure-and-create.png)](media/time-series-insights-get-started/tsi-configure-and-create.png#lightbox)
   
   设置|建议的值|描述
   ---|---|---
   环境名称 | 唯一的名称 | 此名称在[时序资源管理器](https://insights.timeseries.azure.com)中代表环境
   Subscription | 你的订阅 | 如果有多个订阅，最好是选择包含事件源的订阅。 时序见解可以自动检测存在于同一订阅中的 Azure IoT 中心和事件中心资源。
   Resource group | 创建新资源组或使用现有的资源组 | 资源组是结合使用的 Azure 资源的集合。 可以选择现有的资源组，例如，包含事件中心或 IoT 中心的资源组。 或者，如果此资源与其他资源不相关，则可以创建新资源组。
   Location | 最靠近事件源的位置 | 最好是选择包含事件源数据的同一个数据中心位置，以尽量避免增加跨界和跨区域的带宽费用，以及将数据移出区域时增大延迟。
   定价层 | S1 | 选择所需的吞吐量。 若要尽量降低费用并获得入门容量，请选择 S1。
   Capacity | 第 | 容量是应用于入口速率、存储容量和所选 SKU 相关成本的乘数。  可以在创建环境后更改其容量。 若要尽量降低费用，请选择 1 个单位的容量。 
  
1. 选择“创建”开始执行预配过程。 这可能需要几分钟时间才能完成。

1. 若要监视部署过程，请选择“通知”符号（钟形图标）。

   [![观看通知](media/time-series-insights-get-started/tsi-deploy-notifications.png)](media/time-series-insights-get-started/tsi-deploy-notifications.png#lightbox)

1. 在资源**概述**中，验证你的部署配置设置。

   [![创建时序见解固定到仪表板](media/time-series-insights-get-started/tsi-verify-deployment.png)](media/time-series-insights-get-started/tsi-verify-deployment.png#lightbox)

1. **（可选）** 选择右上角的 "**固定" 图标**，以便在将来轻松访问时序见解环境。
