---
title: 规划 Gen2 环境-Azure 时序见解 |Microsoft Docs
description: 配置、管理、规划和部署 Azure 时序见解 Gen2 环境的最佳实践。
author: deepakpalled
ms.author: dpalled
manager: diviso
...
本文介绍了使用 Azure 时序见解 Gen2 计划和快速入门的最佳实践。
...
Azure 时序见解采用即用即付业务模型。 有关费用和容量的详细信息，请阅读[Azure 时序见解定价](https://azure.microsoft.com/pricing/details/time-series-insights/)。

## <a name="the-gen2-environment"></a>Gen2 环境
...
## <a name="review-azure-time-series-insights-gen2-limits"></a>查看 Azure 时序见解 Gen2 限制
...
可以选择最多三个键以唯一区分资源。 有关详细信息，请参阅选择时序 ID 和[引入规则](concepts-json-flattening-escaping-rules.md)[的最佳实践](./time-series-insights-update-how-to-id.md)。
...
您可以验证将事件发送到 Azure 时序见解的方式。 理想情况下，事件非规范化且高效。
...
- 若要规划业务恢复配置选项，请查看 [Azure 顾问](../advisor/advisor-overview.md)。
- 详细了解 Azure 时序见解 Gen2 中的[数据引入](./concepts-ingestion-overview.md)。
- 查看 Azure 时序见解 Gen2 中的[数据存储](./concepts-storage.md)一文。
