...
- 若要根据 Azure 磁盘加密的使用方式设置和配置 Azure 密钥保管库，请参阅本文附录中的**根据 Azure 磁盘加密的使用方式设置和配置 Azure 密钥保管库**部分。

- 若要根据 Azure 磁盘加密的使用方式在 Azure Active Directory 中设置和配置 Azure AD 应用程序，请参阅本文附录中的**在 Azure Active Directory 中设置 Azure AD 应用程序**部分。

- 若要设置和配置 Azure AD 应用程序的密钥保管库访问策略，请参阅本文附录中的**为 Azure AD 应用程序设置密钥保管库访问策略**部分。
...
- 若要运行任何 Azure 磁盘加密 PowerShell cmdlet，必须先安装 Azure PowerShell 1.0.1 版：

	- 若要安装 Azure PowerShell 并将其与 Azure 订阅相关联，请参阅[如何安装和配置 Azure PowerShell](powershell-install-configure.md)。

	- 如果上述选项尚未更新，因而不包含 Azure PowerShell 1.0.1 版，则你也可以从[此处](https://github.com/Azure/azure-powershell/releases/tag/v1.0.1-November2015)直接安装 Azure PowerShell 1.0.1

**注意：**本文档假设你已了解基本概念，例如模块、cmdlet 和会话。有关详细信息，请参阅 [Windows PowerShell 入门](https://technet.microsoft.com/library/hh857337.aspx)
...
### 在 Azure 中现有/正在运行的 IaaS Windows VM 上启用加密
...
| AADClientSecret | ​有权可将机密写入密钥保管库的 Azure AD 应用的客户端机密 |
| **​**keyVaultResourceID | 用于标识 ARM 中密钥保管库资源的 ResourceID。可以使用 PowerShell cmdlet 获取该 ID：(Get-AzureRmKeyVault -VaultName &lt;yourKeyVaultName&gt; -ResourceGroupName &lt;yourResourceGroupName&gt;).ResourceId |
| ​keyVaultURL | ​BitLocker 密钥应上载到的密钥保管库的 URL。可以使用 cmdlet 获取该 ID：(Get-AzureRmKeyVault -VaultName &lt;yourKeyVaultName&gt; -ResourceGroupName &lt;yourResourceGroupName&gt;).VaultURI |
| ​ keyEncryptionKeyURL | ​用于加密所生成 BitLocker 密钥的密钥加密密钥的 URL。这是可选的 |
...
### 在 Azure 中现有/正在运行的 IaaS Linux VM 上启用加密
...
可以使用 Azure 管理（预览）门户、[PowerShell cmdlet](https://msdn.microsoft.com/library/azure/mt622700.aspx) 或 CLI 命令获取加密状态。以下部分将说明如何使用 Azure 管理（预览）门户和 CLI 命令来获取加密状态。

#### 使用 Azure 管理预览门户获取已加密 IaaS VM 的加密状态

可以从 Azure 管理预览门户获取 IaaS VM 的加密状态。通过 https://portal.azure.com/ 登录到 Azure 预览门户，在左侧菜单中单击虚拟机链接以查看订阅中虚拟机的摘要视图。可以从订阅下拉列表中选择订阅名称，以筛选虚拟机视图。单击位于虚拟机页面菜单顶部的列。从“选择列”边栏选项卡中选择“磁盘加密”列，然后单击“更新”。你应会看到，对于每个 VM，磁盘加密列会显示加密状态“已启用”或“未启用”，如下图所示。
