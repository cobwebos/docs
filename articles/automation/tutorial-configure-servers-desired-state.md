...
在本教程中，我们使用一个简单的[DSC 配置](/powershell/scripting/dsc/configurations/configurations)，以确保在 VM 上安装 IIS。
...
- 运行 Windows Server 2008 R2 或更高版本的 Azure 资源管理器 VM（不是经典）。 有关创建 VM 的说明，请参阅[在 Azure 门户中创建第一个 Windows 虚拟机](../virtual-machines/virtual-machines-windows-hero-tutorial.md)。
...
- 熟悉所需状态配置 (DSC)。 有关 DSC 的信息，请参阅[Windows 电源外壳所需的状态配置概述](/powershell/scripting/dsc/overview/overview)。

## <a name="support-for-partial-configurations"></a>支持部分配置

Azure 自动化状态配置支持使用[部分配置](/powershell/scripting/dsc/pull-server/partialconfigs)。 在此方案中，DSC 配置为独立管理多个配置，并且每个配置都从 Azure 自动化中检索。 但是，每个自动化帐户只能为一个节点分配一个配置。 这意味着，如果您对一个节点使用两个配置，则需要两个自动化帐户。

有关如何从拉取服务注册部分配置的详细信息，请参阅[部分配置的文档](https://docs.microsoft.com/powershell/scripting/dsc/pull-server/partialconfigs#partial-configurations-in-pull-mode)。

有关团队如何协同协作以使用配置作为代码协作管理服务器的详细信息，请参阅[了解 DSC 在 CI/CD 管道中的角色](/powershell/scripting/dsc/overview/authoringadvanced)。

## <a name="log-in-to-azure"></a>登录 Azure

使用[Connect-AzAccount](https://docs.microsoft.com/powershell/module/Az.Accounts/Connect-AzAccount?view=azps-3.7.0) cmdlet 登录到 Azure 订阅，并按照屏幕上的说明进行操作。
...
在文本编辑器中，键入以下内容并将其本地保存为**TestConfig.ps1**。
...
调用[导入-阿兹自动化Dsc配置](https://docs.microsoft.com/powershell/module/Az.Automation/Import-AzAutomationDscConfiguration?view=azps-3.7.0)cmdlet 将配置上载到自动化帐户。
...
必须先将 DSC 配置编译为节点配置，然后才能将它分配给节点。 请参阅[DSC 配置](/powershell/scripting/dsc/configurations/configurations)。

调用[启动-AzAutomationD编译作业](https://docs.microsoft.com/powershell/module/Az.Automation/Start-AzAutomationDscCompilationJob?view=azps-3.7.0)cmdlet，将`TestConfig`配置编译为自动化帐户中指定的`TestConfig.WebServer`节点配置。
...
调用[注册-AzAutomationDscNode](https://docs.microsoft.com/powershell/module/Az.Automation/Register-AzAutomationDscNode?view=azps-3.7.0) cmdlet 将 VM 注册到 Azure 自动化状态配置作为托管节点。 
...
使用[注册-AzAutomationDscNode](/powershell/module/azurerm.automation/register-azurermautomationdscnode) cmdlet 将 VM 注册为托管节点并指定配置属性。 例如，可以通过指定`ApplyOnly`为`ConfigurationMode`属性的值来指定计算机的状态仅应用一次。 状态配置不会尝试在初始检查后应用配置。

```powershell
Register-AzAutomationDscNode -ResourceGroupName 'MyResourceGroup' -AutomationAccountName 'myAutomationAccount' -AzureVMName 'DscVm' -ConfigurationMode 'ApplyOnly'
```

还可以指定 DSC 使用 属性`ConfigurationModeFrequencyMins`检查配置状态的频率。 有关 DSC 配置设置的详细信息，请参阅[配置本地配置管理器](/powershell/scripting/dsc/managing-nodes/metaConfig)。
...
这将指定节点`TestConfig.WebServer`配置到注册的 DSC 节点`DscVm`。 默认情况下，每隔 30 分钟会检查一次 DSC 节点是否符合节点配置。 有关如何更改合规性检查间隔的信息，请参阅[配置本地配置管理器](/powershell/scripting/dsc/managing-nodes/metaConfig)。

## <a name="check-the-compliance-status-of-a-managed-node"></a>检查托管节点的符合性状态

您可以使用[Get-AzAutomationDscNode 报告](https://docs.microsoft.com/powershell/module/Az.Automation/Get-AzAutomationDscNodeReport?view=azps-3.7.0)cmdlet 获取有关托管节点的合规性状态的报告。
