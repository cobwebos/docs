...
鉴于本文的目的，需要拥有 Azure 自动化帐户和 AWS 订阅。 有关设置 Azure 自动化帐户和使用 AWS 订阅凭据对其进行配置的详细信息，请参阅[使用 Amazon Web Services 配置身份验证](automation-config-aws-account.md)。  继续操作之前应使用 AWS 订阅凭据创建或更新此帐户，因为我们会在下面的步骤中引用此帐户。
...
2. 会转到 Azure 登录页，身份验证后，会路由到 Azure 门户，其中显示了以下页。<br><br> ![“导入模块”页](./media/automation-scenario-aws-deployment/deploy-aws-powershell-module-parameters.png)
3. 从“资源组”下拉列表中选择资源组，并在“参数”窗格上提供以下信息：
...
6. 单击“资产”磁贴，并在“资产”窗格上选择“模块”磁贴。
7. 在“模块”页上，可在列表中看到 AWSPowerShell 模块。
