...
运行混合 Runbook 辅助角色的计算机应满足最低硬件要求，才能指定它托管此功能。 否则，在 Runbook 执行过程中，由于其他后台进程的资源使用率和争用，该计算机将变为过度使用，从而导致 runbook 作业延迟或超时。
...
* **oms.conf** - 此进程是辅助角色管理器进程，它直接从 DSC 启动。
