...
3. 在“设置”边栏选项卡的“监视”分区下，选择“警报规则”。
4. 在“警报规则”边栏选项卡中，单击“添加警报”。

此时将打开“添加警报规则”边栏选项卡，可以在其中配置警报条件，并选择下列一个或所有选项：向某人发送电子邮件、使用 Webhook 将警报转发到另一个系统，和/或作为响应运行自动化 Runbook 以尝试补救问题。
...
> [!NOTE]
> 在此版本中，可以从服务提供的三个 Runbook 中选择 –“重新启动 VM”、“停止 VM”或“删除 VM”（删除它）。  将来的版本可让你选择其他 Runbook 或你自己的某个 Runbook。
> 
> 

![可供选择的 Runbook](media/automation-azure-vm-alert-integration/RunbooksToChoose.png)

在三个可用的 Runbook 中选择一个后，将显示“自动化帐户”下拉列表，你可以选择用于运行 Runbook 的自动化帐户。 Runbook 需在 Azure 订阅中的[自动化帐户](automation-security-overview.md)上下文中运行。 你可以选择已创建的自动化帐户，也可以自行创建新的自动化帐户。
...
> [!NOTE]
> 在订阅范围内创建了服务主体，并向其分配了“参与者”角色。 需要分配此角色，以便帐户有权运行自动化 Runbook 来管理 Azure VM。  创建自动化帐户和/或服务主体是一次性的事件。 创建后，你可以使用该帐户来对其他 Azure VM 警报运行 Runbook。
> 
> 
...
完成配置后，将看到 Runbook 名称出现在“添加警报规则”边栏选项卡中。
