...
3. 迁移完成后，将需要更新从现有订阅中引用资源或订阅 ID 的任何 runbook 或变量。   
...
2. 请注意，在“移动资源”边栏选项卡中，它提供与你的自动化帐户和资源组相关的资源。  从下拉列表中选择“订阅”和“资源组”，或选择“创建新的资源组”选项，并在提供的字段中输入新的资源组名称。  
3. 查看并选中该复选框以确认你了解在移动资源后，将需要更新工具和脚本，以使用新的资源 ID，然后单击“确定”。<br> ![“移动资源”边栏选项卡](media/automation-migrate-account-subscription/automation-move-resources-blade.png)<br>   

此操作将需要几分钟才能完成。  “通知”中将显示发生的每个操作的状态 - 验证、迁移，然后最后是完成的时间。     

## <a name="to-move-the-automation-account-using-powershell"></a>使用 PowerShell 移动自动化帐户
若要将现有自动化资源移到另一个资源组或订阅，请使用 **Get-AzureRmResource** cmdlet 获取特定自动化帐户，然后使用 **Move-AzureRmResource** cmdlet 执行移动。
...
执行上面的代码示例后，将提示你确认要执行此操作。  单击“是”并允许继续执行脚本后，执行迁移时将不会收到任何通知。  
