...
<properties 
	pageTitle="启动和停止虚拟机 - Graph | Azure"
...
这是此方案的图形 Runbook 版本。也可以使用 [PowerShell 工作流 Runbook](/documentation/articles/automation-solution-startstopvm-psworkflow/) 获取。

## 获取方案

此方案包含两个可通过以下链接下载的图形 Runbook。请参阅此方案的 [PowerShell 工作流版本](/documentation/articles/automation-solution-startstopvm-psworkflow/)，以获取 PowerShell 工作流 Runbook 的链接。
...
下载 Runbook 之后，可以使用[图形 Runbook 过程](/documentation/articles/automation-graphical-authoring-intro/#graphical-runbook-procedures)中的过程导入它们。
...
| 资产类型 | 默认名称 | 说明 |
|:---|:---|:---|:---|
| [凭据](/documentation/articles/automation-credentials/) | AzureCredential | 包含帐户凭据，该帐户有权在 Azure 订阅中启动和停止虚拟机。 |
| [变量](/documentation/articles/automation-variables/) | AzureSubscriptionId | 包含你的 Azure 订阅的订阅 ID。 |

## 使用方案

### Parameters

每个 Runbook 具有以下[输入参数](/documentation/articles/automation-starting-a-runbook/#runbook-parameters)。必需参数必须提供值，其他参数则可根据要求选择性地提供值。
...
你可以使用[在 Azure 自动化中启动 Runbook](/documentation/articles/automation-starting-a-runbook/) 中的任何方法来启动本文中的任一 Runbook。
...
下图显示在示例图形 Runbook 中使用 **StartAzureClassicVM** 作为[子 Runbook](/documentation/articles/automation-child-runbooks/)。其中使用了下表中的条件链接。

| 链接 | 条件 |
|:---|:---|
| 成功链接 | $ActivityOutput['StartAzureClassicVM'] -like "* has been started" |
| 错误链接 | $ActivityOutput['StartAzureClassicVM'] -notlike "* has been started" |
...
Runbook 首先使用活动来设置将要用于 Runbook 剩余部分的[凭据](/documentation/articles/automation-configuring/#configuring-authentication-to-azure-resources)和 Azure 订阅。
...
**Add-AzureAccount** 设置将用于 Runbook 剩余部分的凭据。它从**获取 Azure 凭据**中检索的凭据资产必须有权启动和停止 Azure 订阅中的虚拟机。使用的订阅由 **Select-AzureSubscription** 选择，此活动使用来自**获取订阅 ID** 的订阅 ID。
...
方案是创建**合并 VM** 来运行 **Write-Output** cmdlet。该 cmdlet 的 **InputObject** 参数是结合前面两个活动的输入所形成的 PowerShell 表达式。其中只有一个活动运行，因此只有一组输出。**Start-AzureVM** 可以使用该输出作为输入参数。
...
根据 Runkbook，后续活动使用 **Start-AzureVM** 或 **Stop-AzureVM** 来尝试启动或停止 Runbook。由于活动的前面有管道链接，因此对**合并 VM** 返回的每个对象运行一次。链接是条件性的，仅当虚拟机的 *RunningState* 在 **Start-AzureVM** 中是 *Stopped*，而在 **Stop-AzureVM** 中是 *Started* 时，活动才运行。如果不满足此条件，则运行**通知已启动**或**通知已停止**，以使用 **Write-Output** 发送消息。
...
Runbook 的最后一个步骤是发送输出，而不论是否成功提交每个虚拟机的启动或停止请求。由于各自有一个 **Write-Output** 活动，我们根据条件链接确定运行哪一个。如果 *OperationStatus* 是 *Succeeded*，则运行**通知 VM 已启动**或**通知 VM 已停止**。如果 *OperationStatus* 是任何其他值，运行**通知无法启动**或**通知无法停止**。
