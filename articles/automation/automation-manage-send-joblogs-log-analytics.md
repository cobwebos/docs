...
来自 Azure 自动化的诊断将在 Azure Monitor 日志中创建两种类型的记录，并将其标记为 **AzureDiagnostics**。 以下查询使用升级的 Azure Monitor 日志查询语言。 有关旧查询语言和新的 Azure Kusto 查询语言之间的常见查询的信息，请访问[旧到新 Azure Kusto 查询语言备忘单](https://docs.loganalytics.io/docs/Learn/References/Legacy-to-new-to-Azure-Log-Analytics-Language)
...
| RunbookName_s |Runbook 的名称。 |
| Caller_s |谁启动了该操作。 可能的值为电子邮件地址或计划作业的系统。 |
| StreamType_s |作业流的类型。 可能的值包括：<br>- Progress（进度）<br>- Output（输出）<br>- Warning（警告）<br>- Error<br>- Debug（调试）<br>- Verbose |
| Tenant_g | GUID，用于为 Caller 标识租户。 |
| JobId_g |用作 Runbook 作业 ID 的 GUID。 |
