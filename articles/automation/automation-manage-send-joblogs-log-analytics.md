...
description: 本文演示如何将作业状态和 runbook 作业流发送到 Azure 的 Azure Monitor 日志，以提供附加见解和管理。
services: automation
...
ms.locfileid: "56820852"
---
# <a name="forward-job-status-and-job-streams-from-automation-to-azure-monitor-logs"></a>将作业状态和作业流从自动化转发到 Azure Monitor 日志

自动化可以将 Runbook 作业状态和作业流发送到 Log Analytics 工作区。 此过程不涉及工作区链接，并且完全独立。 可在 Azure 门户中或使用 PowerShell 查看单个作业的作业日志和作业流，这使用户可执行简单的调查。 现在使用 Azure Monitor 日志可以：
...
* Log Analytics 工作区。 有关详细信息，请参阅[开始使用 Azure Monitor 日志](../log-analytics/log-analytics-get-started.md)。 
...
## <a name="set-up-integration-with-azure-monitor-logs"></a>设置与 Azure Monitor 日志集成
...
运行此脚本后，可能需要在您可以看到新 JobLogs 或 JobStreams 正在编写的 Azure Monitor 日志中记录一小时。
...
来自 Azure 自动化的诊断 Azure Monitor 日志中创建两个类型的记录，并标记为**AzureDiagnostics**。 以下查询使用 Azure Monitor 日志到升级版的查询语言。 有关旧查询语言和新的 Azure Kusto 查询语言之间的常见查询的信息，请访问[旧到新 Azure Kusto 查询语言备忘单](https://docs.loganalytics.io/docs/Learn/References/Legacy-to-new-to-Azure-Log-Analytics-Language)
...
现在，你已开始自动化作业日志发送到 Azure Monitor 日志，我们来看可以在 Azure Monitor 日志对这些日志执行的操作。
...
1. 从 Log Analytics 工作区的概述页上，单击**查看日志**。
...
通过将自动化作业状态和流数据发送到 Azure Monitor 日志，您可以更好地了解自动化作业的状态：
+ 设置警报，以便在出现问题时获得通知。
+ 使用自定义视图和搜索查询直观地显示 Runbook 结果、Runbook 作业状态，以及其他相关的关键指标。  

Azure Monitor 日志提供更直观地自动化作业，可帮助更快地解决事件。  
