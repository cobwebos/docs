...
自动化可以将 Runbook 作业状态和作业流发送到 Log Analytics 工作区。 此过程不涉及工作区链接，并且完全独立。 可在 Azure 门户中或使用 PowerShell 查看单个作业的作业日志和作业流，这使用户可执行简单的调查。 现在有了 Azure Monitor 日志，您可以：
...
运行此脚本后，可能需要一小时才能开始在写入 Azure Monitor 日志中查看记录。
...
Azure Automation 中的诊断在 Azure Monitor 日志中创建两种类型的记录，并将其标记为**AzureDiagnostics**。 以下查询使用升级的查询语言来 Azure Monitor 日志。 有关旧查询语言和新的 Azure Kusto 查询语言之间的常见查询的信息，请访问[新的 Azure Kusto 查询语言](https://docs.loganalytics.io/docs/Learn/References/Legacy-to-new-to-Azure-Log-Analytics-Language)备忘单
...
| ResultType |Runbook 作业的状态。 可能的值包括：<br>- New（新）<br>-已创建<br>- Started（已启动）<br>- Stopped（已停止）<br>- Suspended（已暂停）<br>- Failed（失败）<br>- Completed（已完成） |
...
Azure Monitor 日志可为自动化作业提供更高的操作可见性，有助于更快地解决事件。
