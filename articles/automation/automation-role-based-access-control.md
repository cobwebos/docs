...
|自动化作业操作员|自动化作业操作员角色，可创建和管理使用自动化 runbook 的作业。|
|自动化 Runbook 操作员|自动化 Runbook 操作员角色，可以读取 runbook 属性。 此外可创建 runbook 的作业。|
...
自动化作业操作员在自动化帐户范围授予。 允许操作员权限管理帐户中的作业。
...
自动化 Runbook 操作员角色在 Runbook 范围授予。 自动化 Runbook 操作员可以查看 Runbook 名称。 与在自动化帐户的范围的自动化作业 Operator 结合使用此权限使操作员能够执行特定 runbook 的自动化运算符操作。 下表显示了授予角色的权限：
