...
|写入 shoebox 帐户      | Microsoft.Automation/automationAccounts/write        | Account        |
...
以下部分介绍如何通过[门户](#configure-rbac-using-the-azure-portal)和[PowerShell](#configure-rbac-using-powershell)在自动化帐户上配置 RBAC。
...
2. 单击左上角的访问**控制 （IAM）** 控件以打开访问控制 （IAM） 页面。 您可以使用此页面添加新用户、组和应用程序来管理自动化帐户，并查看可为自动化帐户配置的现有角色。
3. 单击“角色分配”选项卡****。
...
3. 键入要在 **"选择"** 字段中授予权限的用户的用户名。 从列表中选择用户，然后单击"**保存**"。
...
[Get-AzureRmRoleDefinition](/previous-versions/azure/mt603792(v=azure.100)) 列出 Azure Active Directory 中提供的所有 RBAC 角色。 可以使用此 cmdlet 与*Name*参数一起列出特定角色可以执行的所有操作。
...
[Get-AzureRmRoleAssignment](/previous-versions/azure/mt619413(v=azure.100)) 列出指定作用域中的 Azure AD RBAC 角色分配。 没有任何参数，此 cmdlet 返回订阅下的所有角色分配。 使用 *"展开主体组"* 参数列出指定用户以及用户所属的组的访问分配。
...
使用[New-AzureRmRole 分配](/previous-versions/azure/mt603580(v=azure.100))将对用户、组和应用程序的访问分配给特定作用域。
...
使用 [Remove-AzureRmRoleAssignment](/previous-versions/azure/mt603781(v=azure.100)) 从特定范围中删除指定用户、组或应用程序的访问权限。
...
在前面的示例中，将"要删除的用户的登录 ID"、"订阅 ID"、"资源组名称"和"自动化帐户名称"替换为您的帐户详细信息。 在继续删除用户角色分配之前，提示确认时选择 **"是**"。
...
脚本运行后，让用户登录到 Azure 门户并查看**所有资源**。 在列表中，用户可以查看已添加为自动化 Runbook 运算符的运行簿。
