...
本文介绍如何使用 runbook 为环境中的 VM 启用[更新管理](automation-update-management.md)功能。 若要大规模启用 Azure VM，必须使用更新管理启用现有 VM。 
...
2. 选择 Log Analytics 工作区，然后单击“启用”。 启用更新管理时，将显示蓝色横幅。 
    ...
    ![启用更新管理](media/automation-onboard-solutions/update-onboard.png)
...
3. 从列表中选择 VM，然后单击“启用”以设置 VM 进行更新。 
   ...
   ![为 VM 启用更新管理](media/automation-onboard-solutions/enable-update.png)
...
5. 单击“确定”，将 Runbook 导入到自动化帐户中。
   ...
   ![导入 runbook 以进行设置](media/automation-onboard-solutions/import-from-gallery.png)
