...
利用图形创作，无需编写基本的 Windows PowerShell 或 PowerShell 工作流代码，就能为 Azure 自动化创建 runbook。 可将活动从 cmdlet 和 Runbook 的库添加到画布，将它们链接到一起，然后对其进行配置以形成工作流。 如果你曾经使用过 System Center Orchestrator 或 Service Management Automation （SMA），图形创作应该非常熟悉。 本文介绍在开始创建图形 Runbook 之前需要了解的概念。
...
所有 cmdlet 都会有提供其他参数的选项。 这些是 PowerShell 通用参数或其他自定义参数。 图形编辑器提供了一个文本框，可以在其中使用 PowerShell 语法提供参数。 例如，若要使用`Verbose` common 参数，应该指定。 `-Verbose:$True`
...
例如，下面的代码使用名为`NumberOfDays`的 runbook 输入变量执行日期计算。 然后，它会将计算的日期时间值作为输出发送，供 runbook 中的后续活动使用。
...
在链接上指定条件时，只有当条件解析为 True 时才会运行目标活动。 通常会在条件`ActivityOutput`中使用变量来检索源活动的输出。

对于管道链接，必须为单个对象指定条件。 Runbook 针对源活动的每个对象输出对条件进行评估。 然后，它将为满足条件的每个对象运行目标活动。 例如，对于的源活动`Get-AzVM`，你可以使用以下条件管道链接语法来仅检索资源组中名为 Group1 的虚拟机。
...
Runbook 使用三个不同的序列链接来验证输入参数`VMName`的值， `ResourceGroupName`并确定要执行的相应操作。 可能的操作是启动单个 VM、启动资源组中的所有 Vm，或者启动订阅中的所有 Vm。 对于与`Connect to Azure` `Get single VM`之间的序列链接，此处为条件逻辑：
...
例如，runbook 中`Start-AzVM`的活动将启动所有虚拟机。 它有两个条件链接。 第一个条件链接使用表达式`$ActivityOutput['Start-AzVM'].IsSuccessStatusCode -eq $true`来筛选`Start-AzVM`活动是否成功完成。 第二个条件链接使用表达式`$ActivityOutput['Start-AzVM'].IsSuccessStatusCode -ne $true`来筛选`Start-AzVm`活动是否未能启动虚拟机。

![条件链接示例](media/automation-graphical-authoring-intro/runbook-conditional-links.png)

在第一个链接后使用的任何活动`Get-AzureVM`仅检索在运行时`Get-AzureVM`启动的虚拟机。 遵循第二个链接的任何活动仅获取运行时`Get-AzureVM`已停止的虚拟机。 任何按第三个链接进行的活动都会获取所有虚拟机，不管这些虚拟机的运行状态如何。
...
第二个数据访问机制使用如下所示的语法，检索 PowerShell 表达式数据源或包含`ActivityOutput`变量的工作流脚本活动中活动的输出。 如果输出是一个对象，则 runbook 可以指定单个属性。
...
检查点仅在图形 PowerShell 工作流 runbook 中启用，在图形 runbook 中不可用。 如果 runbook 使用 Azure cmdlet，则它应遵循`Connect-AzAccount`活动的任何检查点活动。 如果 runbook 已挂起，则使用 connect 操作，并且必须在另一辅助角色上从此检查点重新启动。
...
创作 runbook 时，可以通过单击 runbook 工具栏上的 "**输入和输出**" 来访问其输入参数。 这种情况下将打开输入和输出控件，可以通过单击“添加输入”**** 在该控件中编辑现有输入参数或创建新的参数。
...
| 必需 | 可选。 指定是否必须为参数提供值的设置。 如果选择`yes`，则在启动 runbook 时必须提供一个值。 如果选择`no`此值，则启动 runbook 时不需要一个值，并且可以使用默认值。 如果没有为每个没有定义默认值的必需参数提供值，则无法启动 runbook。 |
| 默认值 | 可选。 如果在 runbook 启动时未传入参数，则为参数使用该值。 若要设置默认值，请`Custom`选择。 如果`None`不想提供任何默认值，请选择此值。 |
...
若要在 runbook 中使用上一个活动的输出，请使用`ActivityOutput`具有以下语法的变量。
...
例如，以下条件确定名`Get-AzureVM`为的活动中的虚拟机当前是否已停止。
...
可以使用[逻辑运算符](https://technet.microsoft.com/library/hh847789.aspx)（如`-and`或`-or`）在 runbook 中加入多个条件。 例如，以下条件会检查上一示例中的虚拟机是否处于停止或停止状态。
...
下面的示例使用名`Get Twitter Connection`为的活动的输出来填充哈希表。
...
Azure 自动化中用于管理 Azure 资源的 Runbook 将需要通过 Azure 进行身份验证。 [运行方式帐户](automation-create-runas-account.md)（也称为服务主体）是自动化 runbook 用来访问订阅中的 Azure 资源管理器资源的默认机制。 可以通过将使用 PowerShell `AzureRunAsConnection` [get-automationconnection](https://technet.microsoft.com/library/dn919922%28v=sc.16%29.aspx) cmdlet 的连接资产添加到画布，将此功能添加到图形 runbook。 还可以添加[AzAccount](/powershell/module/az.accounts/connect-azaccount) cmdlet。 下面的示例阐释了这种情况。

![运行方式身份验证活动](media/automation-graphical-authoring-intro/authenticate-run-as-account.png)

使用`Get Run As Connection`名为`AzureRunAsConnection`的`Get-AutomationConnection`常量值数据源配置活动或。

![运行方式连接配置](media/automation-graphical-authoring-intro/authenticate-runas-parameterset.png)

下一个活动`Connect-AzAccount`添加已验证的运行方式帐户，以便在 runbook 中使用。

![AzAccount 参数集](media/automation-graphical-authoring-intro/authenticate-conn-to-azure-parameter-set.png)

>[!NOTE]
>对于 PowerShell Runbook，`Add-AzAccount` 和 `Add-AzureRMAccount` 是 `Connect-AzAccount` 的别名。 请注意，这些别名不适用于图形 Runbook。 图形 runbook 只能使用`Connect-AzAccount`自身。
...
某些订阅者使用[Azure AD 的用户帐户](automation-create-aduser-account.md)创建一个自动化帐户，用于管理 azure 经典部署或 azure 资源管理器资源。 若要维护这些订阅服务器的向后兼容性，runbook 中使用的身份验证`Add-AzureAccount`机制是包含[凭据资产](automation-credentials.md)的 cmdlet。 资产表示有权访问 Azure 帐户的 Active Directory 用户。

你可以为图形 runbook 启用此功能，方法是将凭据资产添加到画布，然后再添加`Add-AzureAccount`一个活动，该活动使用凭据资产进行输入。 请参阅以下示例。

![身份验证活动](media/automation-graphical-authoring-intro/authentication-activities.png)

Runbook 必须在其开始和每个检查点之后进行身份验证。 因此，您必须在`Add-AzureAccount`执行任何`Checkpoint-Workflow`活动后使用活动。 不需要使用其他凭据活动。
