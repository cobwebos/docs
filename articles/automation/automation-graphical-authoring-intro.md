...
可以将此功能添加到图形 Runbook，只需将凭据资产添加到画布，并完成 Add-AzureAccount 活动即可。  Add-AzureAccount 使用凭据活动作为其输入。  下面的示例对此进行了演示。
