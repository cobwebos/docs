...
<properties 
    pageTitle="Azure 自动化中的图形创作 | Azure"
...
可以在添加 Runbook 时选择“导入”选项，将图形 Runbook 或图形 PowerShell 工作流 Runbook 文件导入。当你选择要导入的文件时，可以保留同一个**名称**，或提供一个新名称。“Runbook 类型”字段会在评估所选文件后显示 Runbook 的类型。如果你尝试选择其他不正确的类型，则会显示一条消息，告知你可能存在冲突，并且可能会在转换过程中出现语法错误。
...
对于序列链接，只会对条件评估一次，因为会返回单个包含源活动中所有对象输出的数组。因此，不能使用序列链接来进行管道链接这样的筛选，但可以通过该链接来直接确定是否会运行下一活动。试以下面的活动集（位于我们的 Start VM Runbook 中）为例。 <br> ![使用序列的条件链接](./media/automation-graphical-authoring-intro/runbook-conditional-links-sequence.png)<br>
...
可以通过在任何活动上选择“检查点 Runbook”，在图形 PowerShell 工作流 Runbook 中设置[检查点](/documentation/articles/automation-powershell-workflow/#checkpoints)。这会导致在运行活动之后设置检查点。
...
Azure 自动化中用于管理 Azure 资源的 Runbook 将需要通过 Azure 进行身份验证。新的[运行方式帐户](/documentation/articles/automation-sec-configure-azure-runas-account/)功能（也称为服务主体）是在订阅中使用自动化 Runbook 来访问 Azure Resource Manager 资源的默认方法。你可以将此功能添加到图形 Runbook，只需向画布添加 **AzureRunAsConnection** 连接资产即可，该资产使用 PowerShell [Get-AutomationConnection](https://technet.microsoft.com/zh-cn/library/dn919922%28v=sc.16%29.aspx) cmdlet 和 [Add-AzureRmAccount](https://msdn.microsoft.com/zh-cn/library/mt619267.aspx) cmdlet。下面的示例对此进行了演示。<br>![运行方式身份验证活动](./media/automation-graphical-authoring-intro/authenticate-run-as-account.png)<br>
...
为了确保后向兼容性，以便使用 [Azure AD 用户帐户](/documentation/articles/automation-sec-configure-aduser-account/)创建了自动化帐户的订户能够管理 Azure 服务管理 (ASM) 或 Azure Resource Manager 资源，所使用的身份验证方法必须是带有[凭据资产](http://msdn.microsoft.com/zh-cn/library/dn940015.aspx)的 Add-AzureAccount cmdlet，该资产代表具有 Azure 帐户访问权限的 Active Directory 用户。
...
图形编写的优点之一是提供以 PowerShell 的基本知识创建 Runbook 的能力。目前，你确实需要熟悉一下 PowerShell 才能填充某些[参数值](#activities)和设置[链接条件](#links-and-workflow)。本部分提供 PowerShell 表达式的快速简介供不熟悉的用户参考。[使用 Windows PowerShell 编写脚本](http://technet.microsoft.com/zh-cn/library/bb978526.aspx)中提供了 PowerShell 的完整详细信息。
...
使用[比较运算符](https://technet.microsoft.com/zh-cn/library/hh847759.aspx)来比较值或确定值是否与指定的模式匹配。比较将返回 $true 或 $false 值。
...
可以使用[逻辑运算符](https://technet.microsoft.com/zh-cn/library/hh847789.aspx)（例如 **-and** 或 **-or**）添加多个条件。例如，以下条件将检查上述示例中同一虚拟机的状态是“已停止”还是“正在停止”。
...
[哈希表](http://technet.microsoft.com/zh-cn/library/hh847780.aspx)是返回一组值时很有用的名称/值对。某些活动的属性可能是哈希表而不是简单值。哈希表有时也称为字典。
...
- 若要开始使用图形 Runbook，请参阅[我的第一个图形 Runbook](/documentation/articles/automation-first-runbook-graphical/)
- 若要了解有关 Runbook 类型、其优点和限制的详细信息，请参阅 [Azure 自动化 Runbook 类型](/documentation/articles/automation-runbook-types/)
- 若要了解如何使用自动化运行方式帐户进行身份验证，请参阅[配置 Azure 运行方式帐户](/documentation/articles/automation-sec-configure-azure-runas-account/)
