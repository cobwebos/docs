...
在 Azure 门户中创建自动化帐户时，将自动创建 **"运行为"** 帐户。 此帐户执行以下任务：

* 在 Azure Active Directory (Azure AD) 中创建服务主体。
* 创建证书。
* 向参与者分配基于角色的访问控制 (RBAC)，以便使用 Runbook 管理 Azure 资源管理器资源。

通过为您创建此帐户，您可以快速开始构建和部署 Runbook 以支持您的自动化需求。

## <a name="permissions-required-to-create-an-automation-account"></a>创建自动化帐户所需的权限

若要创建或更新自动化帐户，并完成本文所述的任务，必须具有以下特权和权限：

* 要创建自动化帐户，必须将 Azure AD 用户帐户添加到具有与`Microsoft.Automation`资源所有者角色等值的权限的角色。 有关详细信息，请参阅[Azure 自动化 中的基于角色的访问控制](automation-role-based-access-control.md)。
...
如果在添加到订阅的全局管理员/Coadministrator 角色之前，您不是订阅的 Active Directory 实例的成员，则作为来宾将添加到 Active Directory。 在这种情况下，您可以在"添加自动化帐户"窗格上看到此消息：`You do not have permissions to create.`

如果首先将用户添加到全局管理员/Coadministrator 角色，则可以从订阅的活动目录实例中删除该用户。 您可以将用户读取为"活动目录中的用户"角色。
...
1. 在"添加自动化帐户"窗格中，在 **"名称"** 字段中输入新自动化帐户的名称。 选择此名称后，无法更改该名称。 

    > [!NOTE]
    > 自动化帐户名称对于每个区域和资源组是唯一的。 已删除的自动化帐户的名称可能无法立即使用。

1. 如果您有多个订阅，请使用 **"订阅"** 字段指定用于新帐户的订阅。
1. 对于“资源组”，请输入或选择新的或现有的资源组。****
1. 对于“位置”，请选择一个 Azure 数据中心位置。****
1. 对于"**创建 Azure 运行为帐户"** 选项，请确保选择 **"是**"，然后单击"**创建**"。
