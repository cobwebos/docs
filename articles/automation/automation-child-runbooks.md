...
内联调用的子 Runbook 的参数可以是任意数据类型（包括复杂对象），并且不会进行 [JSON 序列化](automation-starting-a-runbook.md#runbook-parameters)，因为使用 Azure 管理门户或 Start-AzureRmAutomationRunbook cmdlet 启动 Runbook 时会进行这种序列化。
