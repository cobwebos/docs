...
可以使用 Azure 自动化中的更新管理解决方案来管理 Azure、本地环境和其他云环境中的 Windows 和 Linux 计算机的操作系统更新。 可以快速评估所有代理计算机上可用更新的状态，并管理为服务器安装所需更新的过程。
...
解决方案管理的每台 Windows 计算机都在 "**混合辅助角色组**" 窗格中列出为自动化帐户的**系统混合辅助角色组**。 解决方案使用*主机名 FQDN_GUID*命名约定。 不能在帐户中通过 Runbook 将这些组作为目标进行操作。 如果尝试，则尝试失败。 这些组旨在仅支持此管理解决方案。
...
> [!NOTE]
> 如果将 Operations Manager 1807 或2019管理组连接到管理组中配置为收集日志数据的代理的 Log Analytics 工作区，则需要重写以下规则，以便使用更新管理：重写参数**IsAutoRegistrationEnabled**并将其设置为**True** ： **AzureAutomation**规则。

有关如何更新解决方案管理包的详细信息，请参阅[将 Operations Manager 连接到 Azure Monitor 日志](../azure-monitor/platform/om-agents.md)。

> [!NOTE]
> 对于具有操作管理器代理的计算机，若要通过更新管理完全管理，则必须将代理更新为适用于 Windows 或 Linux 的 Log Analytics 代理。 若要了解如何更新代理，请参阅[如何升级 Operations Manager 代理](https://docs.microsoft.com/system-center/scom/deploy-upgrade-agents)。 在使用 Operations Manager 的环境中，你必须运行 System Center Operations Manager 2012 R2 UR 14 或更高版本。
