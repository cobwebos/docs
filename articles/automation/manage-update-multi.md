---
title: 在 Azure 自动化中管理多个 VM 的更新
description: 本文介绍如何管理多个 VM 的更新。
services: automation
...
ms.locfileid: "83834622"
---
# <a name="manage-updates-for-multiple-vms"></a>管理多个 VM 的更新

可以使用 Azure 自动化更新管理来管理 Windows 和 Linux VM 的更新和修补程序。 可以从 [Azure 自动化](automation-offering-get-started.md)帐户执行以下操作：

- 启用要进行更新管理的 VM。
- 评估可用更新的状态。
- 计划安装所需的更新。
- 查看部署结果，验证是否已成功将更新应用到所有启用了“更新管理”的 VM。

要了解更新管理的系统要求，请参阅[更新管理客户端要求](automation-update-management.md#client-requirements)。

## <a name="prerequisites"></a>先决条件

* 一个安装了其中一个受支持的操作系统的 VM 或计算机。
* 对启用了更新管理的 Linux VM 的更新存储库的访问权限。

## <a name="enable-update-management-for-azure-vms"></a>为 Azure VM 启用更新管理
...
3. 选择要启用的 VM，并在“启用更新管理”下选择“启用”。

    ![“启用更新管理”对话框](./media/manage-update-multi/update-enable.png)

    操作完成后，会在 VM 上启用更新管理。

## <a name="enable-update-management-for-non-azure-vms-and-computers"></a>为非 Azure VM 和计算机启用更新管理

需要在运行于企业网络或其他云环境中的 VM 上安装适用于 Windows 和 Linux 的 Log Analytics 代理，才能为其启用更新管理。 请参阅 [Log Analytics 代理概述](../azure-monitor/platform/log-analytics-agent.md)，了解将代理部署到在 Azure 之外托管的计算机所要满足的系统要求和支持的方法。

## <a name="view-computers-attached-to-your-automation-account"></a>查看附加到自动化帐户的计算机

为计算机启用更新管理后，可以通过选择“计算机”来查看计算机信息。 可以查看有关计算机的以下信息：计算机名称、符合性状态、环境、OS 类型、已安装的关键更新和安全更新、已安装的其他更新和更新代理准备情况。
...
- `Not assessed`：未在预期的时间范围内收到计算机的更新评估数据。 对于 Linux 计算机，预期的时间范围是过去 1 小时内。 对于 Windows 计算机，预期的时间范围是过去 12 小时内。
...
安装在 VM 和计算机上的代理收集有关更新的数据。 代理将该数据发送到 Azure 更新管理。
...
对于 Linux 计算机，符合性扫描默认情况下每小时执行一次。 如果 MMA 代理重启，则会在 15 分钟内启动符合性扫描。
...
若要为一台或多台 VM 计划新的更新部署，请在“更新管理”下选择“计划更新部署”。 

在“新建更新部署”窗格中，指定以下信息：

- **Name**：输入用于标识更新部署的唯一名称。
...
- **要更新的计算机**：选择“已保存的搜索”、“已导入的组”或“计算机”，进而选择要更新的计算机。
   ...
   >[!NOTE]
   >选择“保存的搜索”选项不会返回计算机标识，而仅返回其名称。 如果多个资源组中有多个具有相同名称的 VM，则会在结果中返回这些 VM。 建议使用“要更新的组”选项，以确保包含符合条件的唯一 VM。
...
- **更新分类**：选择要在更新部署中包括的软件的类型。 有关分类类型的说明，请参阅[更新分类](automation-view-update-assessments.md#work-with-update-classifications)。 分类类型：
  - 关键更新
  - 安全更新
  - 更新汇总
  - 功能包
...
- 要包含/排除的更新 - 这会打开“包含/排除”页。 要包含或排除的更新位于单独的选项卡上。 有关如何处理包含项的详细信息，请参阅[计划更新部署](automation-tutorial-update-management.md#schedule-an-update-deployment)。
...
“更新结果”窗格会显示 VM 的更新数和部署结果。 右侧的表格详细列出了每个更新的细目以及安装结果。 安装结果可能是以下值之一：
...
* 如果需要搜索更新日志，请参阅[查询更新管理日志](automation-update-management-query-logs.md)。
