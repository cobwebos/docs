<properties
    ...
    manager="jwhit"
    editor=""
	keywords="powershell 工作流, powershell 工作流示例, powershell 工作流, 工作流 powershell"/>
<tags
...
5.	在本例中，我们将要创建一个PowerShell 工作流 Runbook，因此请选择“PowerShell 工作流”作为“Runbook 类型”。<br>
...
1.	我们的 Runbook 目前是空的，只有必需的工作流关键字、Runbook 名称以及括住整个工作流的大括号。<br>
...
11.	单击“作业”打开此 Runbook 的“作业”窗格。这将列出此 Runbook 创建的所有作业。由于我们只运行该作业一次，应该只会看到一个列出的作业。<br>
...
6.	单击“启动”以启动测试。测试完成后，你应该会从帐户收到显示基本信息的输出。其中确认凭据有效。<br>
![身份验证](media/automation-first-runbook-textual/runbook-auth-results.png)
...
5.	停止在上一步中启动的虚拟机。
6.	单击“启动”以启动 Runbook。键入要启动的虚拟机的 **VMName** 和 **ResourceGroupName**。<br>
