...
使用前脚本和后脚本，你可以在 Azure 自动化帐户中（任务前）和之后（在任务后）更新部署之前运行 PowerShell runbook。 Pre 和后脚本在 Azure 上下文中运行，而不是在本地运行。 预脚本在更新部署开始时运行。 后期脚本在部署结束时和任何已配置的重启之后运行。

## <a name="runbook-requirements"></a>Runbook 要求

对于要用作前脚本或后脚本的 runbook，必须将 runbook 导入到自动化帐户中并发布。 若要了解有关此过程的详细信息，请参阅[发布 runbook](manage-runbooks.md#publish-a-runbook)。

## <a name="using-a-pre-script-or-post-script"></a>使用前脚本或后脚本
...
选择 "更新部署" "运行" 后，将会向前面和后脚本显示更多详细信息。 此时会提供运行时的脚本源的链接。

![部署运行结果](./media/pre-post-scripts/deployment-run.png)

## <a name="passing-parameters"></a>传递参数

在配置前脚本和后脚本时，可以像计划 runbook 一样传入参数。 参数是在创建更新部署时定义的。 Pre 和后脚本支持以下类型：
...
除了标准 runbook 参数外，还提供了另一个参数： **SoftwareUpdateConfigurationRunContext**

此参数是一个 JSON 字符串，如果您在前脚本或后脚本中定义参数，则更新部署会自动传入此参数。 参数包含有关更新部署的信息，这是[SOFTWAREUPDATECONFIGURATIONS API](/rest/api/automation/softwareupdateconfigurations/getbyname#updateconfiguration)返回的信息的子集。 
...
|SoftwareUpdateConfigurationSettings.duration     | 更新部署的最长持续时间 `PT[n]H[n]M[n]S` 每 ISO8601 运行一次;也称为*维护时段*。          |
...
示例均基于在下面的示例中定义的基本模板。 此模板可用于创建自己的 runbook，以便与前脚本和后脚本一起使用。 包括在 Azure 中进行身份验证和处理 `SoftwareUpdateConfigurationRunContext` 参数所需的逻辑。
...
Pre 和后期任务作为 runbook 在自动化帐户中运行，而不是直接在部署中的计算机上运行。 之前和之后的任务也在 Azure 上下文中运行，不能访问非 Azure 计算机。 以下部分介绍如何直接与计算机交互，无论它们是 Azure Vm 还是非 Azure 计算机。

### <a name="interacting-with-azure-machines"></a>与 Azure 计算机交互

Pre 和 post 任务作为 runbook 运行，不在部署中的 Azure Vm 上本地运行。 要与 Azure Vm 交互，你必须具有以下各项：

* 一个运行方式帐户
* 要运行的 runbook

要与 Azure 计算机交互，你应使用[AzureRmVMRunCommand](/powershell/module/azurerm.compute/invoke-azurermvmruncommand) Cmdlet 与 azure vm 进行交互。 有关如何执行此操作的示例，请参阅 runbook 示例[更新管理–使用 run 命令运行脚本](https://gallery.technet.microsoft.com/Update-Management-Run-40f470dc)。

### <a name="interacting-with-non-azure-machines"></a>与非 Azure 计算机交互

Pre 和后任务在 Azure 上下文中运行，并且没有访问非 Azure 计算机的权限。 若要与非 Azure 计算机交互，必须具有以下各项：
...
* 使用 pre 和 post 脚本时，不能向参数传递布尔值、对象或数组。 如果这样做，runbook 将会失败。 有关支持的类型的完整列表，请参阅[传递参数](#passing-parameters)。
