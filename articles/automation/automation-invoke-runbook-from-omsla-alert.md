...
*  保存警报后，警报会触发和调用 Webhook，发送到 Runbook 的 WebhookData 包含 .SearchResult。
* 如果创建警报并将其配置为调用 Runbook（这也会创建 Webhook），则警报触发时，会将 WebhookData 发送到包含 .SearchResults 的 Runbook。

因此在上面的代码示例中，如果警报调用 Webhook，则需获取 .SearchResult；如果警报直接调用 Runbook，则需获取 .SearchResults。

## <a name="example-walkthrough"></a>示例演练

我们使用以下可以启动 Windows 服务的示例图形 Runbook 来演示上述操作。<br><br> ![启动 Windows 服务的图形 Runbook](media/automation-invoke-runbook-from-omsla-alert/automation-runbook-restartservice.png)<br>
