...
下表中的 cmdlet 用于通过 Windows PowerShell 创建和管理自动化证书资产。可在自动化 Runbook 中使用的 [Azure PowerShell 模块](powershell-install-configure)已随附了这些 cmdlet。
...
此示例将对使用连接对象向服务进行身份验证的 Send-**TwilioSMS activity** 使用 **UseConnectionObject** 参数集。必须在此处使用[管道链接](automation-graphical-authoring-intro#links-and-workflow)，因为序列链接将返回一个集合，其中包含不应在 Connection 参数中使用的单个对象。
