...
可以通过以下方式在 Azure 自动化状态配置中编译所需状态配置（DSC）配置：
...
**AzAutomationDscCompilationJob**返回可用于跟踪作业状态的编译作业对象。 然后，可以使用此编译作业对象与[AzAutomationDscCompilationJob](/powershell/module/az.automation/get-azautomationdsccompilationjob)来确定编译作业的状态，并使用[AzAutomationDscCompilationJobOutput](/powershell/module/az.automation/get-azautomationdscconfiguration)查看其流（输出）。 下面的示例启动 SampleConfig 配置的编译，等待它完成，然后显示其流。
...
下面的示例使用两*个名为*"功能名称" 和 " *IsPresent*" 的参数来确定编译期间生成的 ParametersExample 节点配置中的属性值。
...
通过 **ConfigurationData** 可在使用 PowerShell DSC 时分开结构化配置与任何环境特定配置。 有关详细信息，请参阅[从 POWERSHELL DSC 中的 "位置" 分隔 "内容](https://blogs.msdn.com/b/powershell/archive/2014/01/09/continuous-deployment-using-dsc-with-minimal-change.aspx)"。

> [!NOTE]
> 在 Azure 自动化状态配置中进行编译时，可以在 Azure PowerShell 中使用**ConfigurationData** ，但不能在 Azure 门户中使用。
...
Azure Automation 中的 DSC 配置可以使用**get-automationpscredential** Cmdlet 引用自动化凭据资产。 如果配置具有指定**PSCredential**对象的参数，请使用**get-automationpscredential** ，方法是将 Azure 自动化凭据资产的字符串名称传递到 cmdlet 来检索凭据。 然后对需要**PSCredential**对象的参数使用该对象。 在后台将检索具有该名称的 Azure 自动化凭据资产并将其传递给配置。 下面的示例演示了这种操作方案。
...
可以告诉 PowerShell DSC，使用配置数据在生成的节点配置 Mof 中以纯文本形式输出凭据是正确的。 应通过**ConfigurationData**为 DSC 配置中显示的每个节点块名称传递 `PSDscAllowPlainTextPassword = $true`，并使用凭据。
...
> [!NOTE]
> 编译完成后，你可能会收到错误消息 "由于已导入" Microsoft PowerShell 管理单元 "，未导入" Microsoft PowerShell. 管理 "模块。 您可以放心地忽略此消息。

## <a name="compiling-your-dsc-configuration-in-windows-powershell"></a>在 Windows PowerShell 中编译 DSC 配置

还可以导入已在 Azure 外部编译的节点配置 (MOF)。 导入包括从开发人员工作站或在[Azure DevOps](https://dev.azure.com)等服务中进行编译。 此方法具有多项优势，包括性能和可靠性。

Windows PowerShell 中的编译还提供了用于对配置内容进行签名的选项。 DSC 代理在托管节点上本地验证已签名的节点配置。 验证可确保应用于节点的配置来自授权源。
...
可以从开发人员工作站或在生成服务（例如[Azure DevOps](https://dev.azure.com)）中执行此过程。 然后，可以将配置编译时生成的 MOF 文件导入到 Azure 状态配置服务中。
