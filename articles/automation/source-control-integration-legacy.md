...
6. 设置源代码管理后，会在自动化帐户中创建两个[变量资产](automation-variables.md)。 此外，已授权的应用程序将添加到 GitHub 帐户。
...
3. 单击“签入”时，会通过一条确认消息进行提示，请单击“是”以继续操作。  
    ...
    ![一个对话框，该对话框用于确认签入到源代码管理](media/source-control-integration-legacy/automation-CheckinMessage.png)
4. 签入启动源代码管理 Runbook：**Sync-MicrosoftAzureAutomationAccountToGitHubV1**。 此 Runbook 将连接到 GitHub 并将 Azure 自动化中的更改推送到存储库。 若要查看已签入的作业历史记录，请返回到“源代码管理集成”选项卡，单击以打开“存储库同步”页面。 此页面将显示所有源代码管理作业。  选择要查看的作业，并单击以查看详细信息。  
...
1. 从设置源代码管理的自动化帐户中，打开 "**源代码管理集成/存储库同步**" 页面，然后单击 "**同步**"。 系统将提示确认消息，单击 **"是"** 以继续。  
    ...
    ![带有消息确认将同步所有 runbook 的 "同步" 按钮](media/source-control-integration-legacy/automation-SyncButtonwithMessage.png)
...
2. 同步启动 Runbook：**Sync-MicrosoftAzureAutomationAccountFromGitHubV1**。 此 Runbook 将连接到 GitHub 并将存储库中的更改提取到 Azure 自动化。 此操作的“存储库同步”页面中应该会显示一个新作业。 若要查看同步作业的详细信息，请单击以打开作业详细信息页面。  
    ...
    ![显示 GitHub 存储库上同步作业的同步结果的窗口](media/source-control-integration-legacy/automation-SyncRunbook.png)

    > [!NOTE]
    > 从源代码管理进行的同步针对当前在源代码管理中的**所有** Runbook，覆盖当前存在于自动化帐户中的 Runbook 草稿版本。 用于同步的 Git 等效命令行指令为 **git pull**

![显示挂起的源代码管理同步作业中所有日志的窗口](media/source-control-integration-legacy/automation-AllLogs.png)
...
若要断开与 GitHub 帐户的连接，请打开“存储库同步”页面，并单击“断开连接”。 断开与源代码管理的连接后，前面同步的 Runbook 仍会保留在自动化帐户中，但不会启用“存储库同步”页面。  
  ...
  ![显示断开源代码管理的 "断开连接" 按钮的窗口](media/source-control-integration-legacy/automation-Disconnect.png)
