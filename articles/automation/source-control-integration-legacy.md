...
如果已有想要链接到 Azure 自动化的 GitHub 帐户和存储库，请登录到现有帐户并从下面的步骤 2 开始。 否则，导航到[GitHub，](https://github.com/)注册新帐户，然后[创建新的存储库](https://help.github.com/articles/create-a-repo/)。
...
2. “源代码管理”**** 页面随即打开，可以在其中配置 GitHub 帐户详细信息。 下面是要配置的参数的列表：  
   ...
   | Runbook 文件夹路径 |Runbook 文件夹路径可指定 GitHub 存储库中的路径，以便从中推送或提取代码。 必须以 **/foldername/subfoldername** 格式输入路径。 只有 Runbook 文件夹路径中的 Runbook 才同步到自动化帐户。 Runbook 文件夹路径的子文件夹中的 Runbook **不会**同步。 用于**/** 同步存储库下的所有 Runbook。 |
...
6. 设置源代码管理后，将在自动化帐户中创建两个[变量资产](automation-variables.md)。 此外，会将一个已授权的应用程序添加到 GitHub 帐户。
   ...
   * **自动化源代码管理**已作为已授权的应用程序添加到 GitHub 帐户。 要查看应用程序：从 GitHub 主页导航到**配置文件** > **设置** > **应用程序**。 此应用程序可让 Azure 自动化将 GitHub 存储库同步到自动化帐户。  
...
1. 从设置源代码控制的自动化帐户中，打开**源代码管理集成/存储库同步**页面，然后单击 **"同步**"。 系统会提示您收到确认消息，单击"**是**"以继续。  
