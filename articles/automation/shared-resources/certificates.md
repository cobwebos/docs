...
对于 AzureRM，下表中的 cmdlet 用于通过 Windows PowerShell 创建和管理自动化凭据资产。 它们作为[AzureRM.自动化模块](/powershell/azure/overview)的一部分发货，该模块可用于自动化手册和 DSC 配置。

|Cmdlet|描述|
|:---|:---|
|[获取 AzureRm 自动化证书](/powershell/module/azurerm.automation/get-azurermautomationcertificate)|检索有关要在 Runbook 或 DSC 配置中使用的证书的信息。 只能从 Get-AutomationCertificate 活动中检索证书本身。|
|[新 AzureRm 自动化证书](/powershell/module/azurerm.automation/new-azurermautomationcertificate)|将新证书创建到 Azure 自动化中。|
[删除 AzureRm 自动化证书](/powershell/module/azurerm.automation/remove-azurermautomationcertificate)|从 Azure自动化中删除证书。|
|[设置-AzureRm自动化证书](/powershell/module/azurerm.automation/set-azurermautomationcertificate)|设置现有证书的属性，包括上传证书文件和设置 .pfx 的密码。|
