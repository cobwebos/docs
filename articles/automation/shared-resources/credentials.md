...
## <a name="azure-powershell-az-cmdlets"></a>Azure 电源壳 Az cmdlet

对于 Azure PowerShell Az 模块，下表中的 cmdlet 用于使用 Windows PowerShell 创建和管理自动化凭据资产。 它们作为[AzureAz.自动化模块](/powershell/azure/new-azureps-module-az?view=azps-1.1.0)的一部分发货，该模块可用于自动化运行簿和 DSC 配置。

| Cmdlet | 描述 |
|:--- |:--- |
| [获取阿兹自动化证书](/powershell/module/az.automation/get-azautomationcredential?view=azps-3.3.0) |检索有关凭据资产的信息。 此 cmdlet 不会返回 PSCredential 对象。  |
...
以下示例命令演示了如何创建新的自动化凭据。 首先创建了一个具有名称和密码的 PSCredential 对象，然后使用该对象创建凭据资产。 或者，可以使用 **Get-Credential** cmdlet 并按照提示键入名称和密码。
...
在 Runbook 或 DSC 配置中使用 **Get-AutomationPSCredential** 活动检索凭据资产。 这将返回 [PSCredential 对象](/dotnet/api/system.management.automation.pscredential)，你可以将其用于需要 PSCredential 参数的活动或 cmdlet。 还可以检索要单独使用的凭据对象的属性。 该对象具有用户名和安全密码属性，你也可以使用 **GetNetworkCredential** 方法返回将提供不安全版本的密码的 [NetworkCredential](/dotnet/api/system.net.networkcredential) 对象。

> [!NOTE]
> **获取阿兹自动化凭据**不会返回可用于身份验证的**PS 凭据**。 它仅提供有关凭据的信息。 如果需要在 runbook 中使用凭据，则必须使用 **Get-AutomationPSCredential** 检索 **PSCredential** 对象。
...
您还可以使用凭据对 Azure 进行[连接-AzAccount](/powershell/module/az.accounts/connect-azaccount?view=azps-3.3.0)。 在大多数情况下，您应该使用["运行即"帐户](../manage-runas-account.md)，并使用[Get-AzAutomationConnection](../automation-connections.md)检索它。
