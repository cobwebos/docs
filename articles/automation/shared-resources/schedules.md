...
可以在 Azure 门户中或使用 PowerShell 为 runbook 创建新计划。
...
4. 通过选择“一次”或“定期”来选择该计划是运行一次，还是按计划重复运行。 如果选择“一次”，请指定“开始时间”，并单击“创建”。 如果选择“定期”，请指定“开始时间”，并且在“重复间隔”中选择想要 runbook 重复运行的频率（按“小时”、按“天”、按“周”或按“月”）。
    1. 如果选择 "**周**"，则会提供一个列表，其中列出了要从中进行选择的一周中的几天。 根据需要选择天数。 计划的第一次运行将在开始时间之后选择的第一天进行。 例如，若要选择周末计划，请选择 "**星期六**" 和 "**星期日**"。

       ![设置周末定期计划](../media/schedules/week-end-weekly-recurrence.png)

    2. 如果选择 "**月**"，则会提供不同的选项。 对于 "**每月发生次数**" 选项，请选择 "**月份日期** **" 或 "工作日"** 。 如果选择 "**月份日期**"，则会显示一个日历，允许你根据需要选择任意数量的日期。 如果你选择的日期不是当前月份出现的第31号，则计划将不会运行。 如果希望计划在最后一天运行，请在“在月份的最后一天运行”下选择“是”。 如果选择“每周天数”，则会显示“重复间隔”选项。 选择“第一”、“第二”、“第三”、“第四”或“最后”。 最后选择一天进行重复。

       ![每月的第一天、第15天和最后一天的计划](../media/schedules/monthly-first-fifteenth-last.png)

5. 完成后，单击“创建”。

### <a name="to-create-a-new-schedule-with-powershell"></a>使用 PowerShell 创建新计划

可使用 [New-AzureRmAutomationSchedule](/powershell/module/azurerm.automation/new-azurermautomationschedule) cmdlet 创建计划。 必须指定计划的开始时间以及运行频率。 下面的示例演示如何创建许多不同的计划方案。
...
#### <a name="create-a-recurring-schedule"></a>创建循环计划

下面的示例命令演示了如何创建一个定期计划，该计划在每天的1： 00 (运行一次。
...
#### <a name="create-a-weekly-recurring-schedule"></a>创建每周循环计划

下面的示例命令演示了如何创建仅在工作日运行的每周计划。
...
#### <a name="create-a-weekly-recurring-schedule-for-weekends"></a>为周末创建每周重复计划

下面的示例命令演示了如何创建仅在周末运行的每周计划。
...
#### <a name="create-a-recurring-schedule-for-first-15th-and-last-days-of-the-month"></a>为每月的第一天、第15天和最后一天创建重复计划

下面的示例命令演示了如何创建在一个月的第1天、第15天和最后一天运行的定期计划。
...
3. 如果 runbook 当前未链接到计划，则将提供创建新计划或链接到现有计划的选项。
4. 如果 runbook 具有参数，则可以选择 "**修改运行设置（默认值： Azure）** " 选项，此时会显示 "**参数**" 窗格，可在其中输入信息。
...
禁用计划后，链接到该计划的所有 Runbook 不再按该计划运行。 可以手动禁用计划，也可以在创建带频率的计划时设置其过期时间。 一旦达到过期时间，就会禁用该计划。
...
> [!NOTE]
> 如果要禁用启动时间过去的计划，则必须将开始日期更改为将来的某个时间，然后再保存。
