...
ms.locfileid: "87423790"
---
# <a name="use-azure-private-link-to-securely-connect-networks-to-azure-automation-preview"></a>使用 Azure 专用链接将网络安全连接到 Azure 自动化（预览版）
...
设置自动化帐户资源之前，请考虑网络隔离要求。 评估你的虚拟网络对公共 internet 的访问权限，以及对你的自动化帐户的访问限制（包括设置专用链接组范围以便在与自动化帐户集成的情况下 Azure Monitor 日志）。 还包括对自动化服务[DNS 记录](./automation-region-dns-records.md)作为计划的一部分的检查，以确保支持的功能正常运行。
