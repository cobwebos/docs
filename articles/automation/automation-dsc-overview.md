...
使用 Azure 自动化 DSC，可以[创作和管理 PowerShell 所需状态配置](https://technet.microsoft.com/library/dn249918.aspx)、导入 [DSC 资源](https://technet.microsoft.com/library/dn282125.aspx)并生成 DSC 节点配置（MOF 文档），所有这一切均在云中完成。 这些 DSC 项将放置在 Azure 自动化 [DSC 拉取服务器](https://technet.microsoft.com/library/dn249913.aspx)中，以便目标节点自动接收配置，符合所需状态，并报告回其符合性。 Azure 自动化的目标可以是云中或本地的虚拟机或物理计算机。 
...
PowerShell DSC 引入了名为“配置”的新概念。 使用配置，你可以通过 PowerShell 语法来定义你的环境所需的状态。 若要使用 DSC 配置环境，请先使用配置关键字后跟标识符来定义 Windows PowerShell 脚本块，然后使用大括号 ({}) 来为该块定界。

![替换文字](./media/automation-dsc-overview/AADSC_1.png)

在配置块内，可以定义节点配置块，以便为环境中应配置为完全相同的一组节点（计算机）指定所需配置。 节点配置通过这种方式来表示一个或多个节点需要采用的一种“角色”。 节点配置块以节点关键字开头。 紧跟此关键字的是角色的名称，可以是变量或表达式。 在角色名称之后，使用大括号 {} 来分隔节点配置块。
...
有关配置关键字的更详细信息，请参阅：[DSC 配置](https://msdn.microsoft.com/en-us/powershell/dsc/configurations "DSC 配置")
...
编译 DSC 配置时，会生成一个或多个节点配置，具体取决于配置中的节点块。 节点配置等同于“MOF”或“配置文档”。 节点配置代表一个“角色”（例如 Web 服务器或辅助角色），一个或多个节点将采用该“角色”的所需状态，或针对该“角色”的所需状态检查合规性。 Azure 自动化 DSC 中的节点配置名称采用“Configuration Name.NodeConfigurationBlockName”格式。
...
DSC 节点是由 DSC 管理其配置的任何计算机。 此计算机可以是 Windows 或 Linux Azure VM、本地 VM/物理主机或其他公有云中的计算机。 节点执行要成为的节点配置并保持与它们定义的所需状态符合。 节点还向报表服务器报告回其配置和符合性状态。

Azure 自动化 DSC 使登记节点操作轻松且灵活。 Azure 自动化 DSC 允许更改分配给每个节点的节点配置。 在节点配置更改发生时，节点将自动通过更改其配置并开始针对其新角色报告来担任不同角色。 

### <a name="resource"></a>资源

DSC 资源是可用于定义 Windows PowerShell 所需状态配置 (DSC) 配置的构建基块。 DSC 附带了一组内置资源，例如，用于文件和文件夹、服务器功能和角色、注册表设置、环境变量以及服务和进程的资源。 若要了解内置 DSC 资源的完整列表以及如何使用它们，请参阅 [Built-In Windows PowerShell Desired State Configuration Resources](https://msdn.microsoft.com/powershell/dsc/builtinresource)（内置 Windows PowerShell Desired State Configuration 资源）。

还可以将 DSC 资源作为 PowerShell 模块的一部分导入，以便扩展内置 DSC 资源的集合。 如果 DSC 节点要执行的节点配置包含对非默认资源的引用，则该节点会从 DSC 拉取服务器中拉取这些资源。 若要了解如何创建自定义资源，请参阅[Build Custom Windows PowerShell Desired State Configuration Resources](https://msdn.microsoft.com/powershell/dsc/authoringresource)（构建自定义 Windows PowerShell Desired State Configuration 资源）。
...
* 升级到 WMF5 之前，如果计算机已注册为 Azure 自动化 DSC 的节点，请从 Azure 自动化 DSC 将其注销，并且在完成 WMF 5 安装之后重新注册。 在重新注册之前，请删除 $env:windir\system32\configuration\DSCEngineCache.mof 文件
* 如果将 WMF 5 RTM 安装在 WMF 5 Production Preview 之上，DSC 可能无法正常工作。 若要解决此问题，请在提升权限的 PowerShell 会话（以管理员身份运行）中运行以下命令：`mofcomp $env:windir\system32\wbem\DscCoreConfProv.mof`
* Azure 自动化 DSC 目前不支持进行部分式或复合式 DSC 配置。 但是，可以导入 DSC 复合资源并像在本地 PowerShell 中一样在 Azure 自动化 DSC 配置中使用，使配置可重复使用。
...
* 当将某个节点首次注册到 Azure 自动化帐户，或将该节点更改为映射到不同的节点配置服务器端时，其状态为“符合”。 即使该节点的状态不符合它现在所映射到的节点配置，也会发生这种情况。 注册以后或者节点配置映射更改以后，如果该节点发送了首个报告，则节点状态将是可以信任的。
