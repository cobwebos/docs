...
首先要注意的是，该资源的类型与资源组`Microsoft.Authorization/roleAssignments`的类型并不相同。  而资源类型则遵循模式`{provider-namespace}/{resource-type}/providers/roleAssignments`。 在这种情况下，资源类型将`Microsoft.DevTestLab/labs/providers/roleAssignments`为。
...
角色定义 ID 是现有角色定义的字符串标识符。 角色 ID 的格式`/subscriptions/{subscription-id}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id}`为。 
...
### <a name="scope"></a>范围
作用域指定应应用角色分配的资源或资源组。 对于资源，范围的格式为： `/subscriptions/{subscription-id}/resourceGroups/{resource-group-name}/providers/{provider-namespace}/{resource-type}/{resource-name}`。 `subscription().subscriptionId`该模板使用函数`subscription-id`填充部件，并使用`resourceGroup().name`模板函数填充该`resource-group-name`部件。 使用这些函数意味着你要向其分配角色的实验室必须存在于当前订阅中，以及模板部署所属的同一资源组。 最后一个部分`resource-name`是实验室的名称。 此值通过此示例中的模板参数接收。 
...
如简介中所述，创建新的 Azure 角色分配，以便将用户添加到实验室的**开发测试实验室用户**角色。 在 PowerShell 中，可以使用[new-azurermroleassignment](/powershell/module/azurerm.resources/new-azurermroleassignment?view=azurermps-6.13.0) cmdlet 来执行此操作。 此 cmdlet 具有多个可选参数，以提供灵活性。 可以`ObjectId`将`SigninName`、或`ServicePrincipalName`指定为要授予其权限的对象。  
...
若要`ResourceName`指定要向其授予权限的资源，可以通过将、 `ResourceType` `ResourceGroup`或的组合指定给该`scope`参数。 无论使用哪种参数组合，都可以为 cmdlet 提供足够的信息来唯一标识 Active Directory 对象（用户、组或服务主体）、作用域（资源组或资源）以及角色定义。

## <a name="use-azure-command-line-interface-cli"></a>使用 Azure 命令行接口（CLI）
在 Azure CLI 中，使用`az role assignment create`命令将实验室用户添加到实验室。 有关 Azure CLI cmdlet 的详细信息，请参阅[使用 RBAC 和 Azure CLI 管理对 Azure 资源的访问权限](../role-based-access-control/role-assignments-cli.md)。

被授予访问权限的对象可以由`objectId`、 `signInName`和`spn`参数指定。 要向其授予访问`scope`权限的实验室`resource-name`，可以通过 url 或、 `resource-type`和`resource-group`参数的组合来识别。
