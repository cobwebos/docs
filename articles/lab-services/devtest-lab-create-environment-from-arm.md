...
- 可以使用参数 `_artifactsLocation` 和 `_artifactsLocationSasToken` 来构造 parametersLink URI 值，以允许开发测试实验室自动管理嵌套模板。 有关详细信息，请参阅[将测试环境的嵌套的 Azure 资源管理器模板部署](deploy-nested-template-environments.md)。
