...
创建实验室时，它将在虚拟网络的子网中创建。  默认情况下，创建此子网时会将“启用共享公共 IP”**** 设置为“是”。**  此配置会为整个子网配置一个公共 IP 地址。  若要深入了解如何配置虚拟网络和子网，请参阅[在 Azure 开发测试实验室中配置虚拟网络](devtest-lab-configure-vnet.md)。
...
在此实验室中创建的任何 VM 都将默认设置为使用共享 IP。  创建 VM 时，可以在**IP 地址配置**下的 **"高级设置"** 页中观察到此设置。
...
- 专用：创建每个 VM 使用的专用 IP 地址****。 您无法通过远程桌面直接从 Internet 连接到此 VM。
