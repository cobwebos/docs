...
1. 登录到有问题的虚拟机。
2. 将该脚本复制到本地虚拟机中，或在下`C:\Packages\Plugins\Microsoft.Compute.CustomScriptExtension\<version>`的虚拟机上进行查找。 这是下载项目脚本的位置。
...
- **检查默认实验室的存储帐户**（即创建实验室时创建的第一个存储帐户，其名称通常以字母 "a" 开头，以多位数（即\<labname\>#）结束。
    1. 导航到实验室的资源组。
...
如果未发生上述任何错误，并且仍无法应用项目，则可以提供 Azure 支持事件。 转到[Azure 支持站点](https://azure.microsoft.com/support/options/)并选择 "**获取支持**"。
