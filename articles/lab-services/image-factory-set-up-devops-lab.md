...
采用你的团队的图像工厂的下一步是了解有哪些可用。 映像工厂脚本和模板是可公开推出[开发测试实验室 GitHub 存储库](https://github.com/Azure/azure-devtestlab/tree/master/Scripts/ImageFactory)。 下面是各个部分的概述：
...
所有 PowerShell、 模板和映像工厂的配置中的都位置[公共开发测试实验室 GitHub 存储库](https://github.com/Azure/azure-devtestlab/tree/master/Scripts/ImageFactory)。 获取代码到新的团队项目的最快方法是导入存储库。 这将拉在整个开发测试实验室存储库中 （所以您会收到额外的文档和示例）。 
