...
ms.locfileid: "85445009"
---
# <a name="set-up-a-lab-to-teach-data-science-with-python-and-jupyter-notebooks"></a>使用 Python 和 Jupyter 笔记本设置实验室来讲授数据科学
本文概述了如何在实验室服务中设置模板虚拟机（VM），以及如何使用[Jupyter 笔记本](http://jupyter-notebook.readthedocs.io/)以及学生如何连接到他们的虚拟机（vm）上的笔记本所需的工具。
...
- [Visual Studio Code](https://code.visualstudio.com/)：集成开发环境（IDE），在编写和测试笔记本时提供丰富的交互式体验。 有关详细信息，请参阅[在 Visual Studio Code 中使用 Jupyter 笔记本](https://code.visualstudio.com/docs/python/jupyter-support)。
...
2. 输入用于连接到 VM 的密码。 （可能需要为 X2Go 的权限授予绕过防火墙的连接。）
3.  现在应会看到 Ubuntu Data Science VM 的图形界面。


#### <a name="ssh-tunnel-to-jupyter-server-on-the-vm"></a>VM 上的 Jupyter 服务器的 SSH 隧道
某些学生可能需要直接从其本地计算机直接连接到其 Vm 内的 Jupyter 服务器。 SSH 协议允许在本地计算机和远程服务器（在我们的示例中为学生的实验室 VM）之间进行端口转发，以便在服务器上的特定端口上运行的应用程序通过**隧道**连接到本地计算机上的映射端口。 学员应遵循以下步骤，通过 SSH 隧道连接到实验室 Vm 上的 Jupyter 服务器：
...
我们来介绍此类的可能的成本估算。 我们将使用一类25名学生。 计划的类时间有20小时。 此外，每个学生在计划的类时间之外为家庭作业或分配获取10小时配额。 所选的 VM 大小为小 GPU （计算），即139实验室单位。 如果要使用小型（20个实验室单位）或中型大小（42实验室单位），可以将下面公式中的实验室单元部分替换为正确的数字。  

下面是此类的可能的成本估算示例：25名学生 * （20个计划小时 + 10 个配额小时） * 139 实验室单位 * 0.01 美元/小时 = 1042.5 USD
