...
如果您的环境或虚拟机通过自动化创建，是在一个最后一个位置以查找错误的信息。 这是 Azure 资源管理器模板部署日志。 通过自动化创建实验室资源时，它通常是通过 Azure 资源管理器模板部署。 请参阅[https://github.com/Azure/azure-devtestlab/tree/master/Samples ](https://github.com/Azure/azure-devtestlab/tree/master/Samples)有关创建开发测试实验室资源的示例 Azure 资源管理器模板。 
