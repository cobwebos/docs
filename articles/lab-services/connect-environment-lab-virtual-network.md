---
title: 连接到 Azure 开发测试实验室的实验室的 vnet 环境 |Microsoft Docs
description: 了解如何连接的环境 （如 Service Fabric 群集） 到 Azure 开发测试实验室的实验室的虚拟网络
...
下面是连接实验室的子网的简单环境模板。 在此示例中，`DTLSubnetId`参数表示的实验室所在的子网的 ID。 分配给： `$(LabSubnetId)`，这会自动解除通过开发测试实验室的实验室的子网的 id。 **子网**的属性**网络接口**此定义中的 vm 设置为`DTLSubnetId`以便加入同一子网。 
