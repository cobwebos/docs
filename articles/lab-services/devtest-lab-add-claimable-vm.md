---
title: 在 Azure 开发人员测试实验室中创建和管理可索赔 VM |微软文档
...
用户声明 VM 后，DevTest Labs 将启动计算机并将其移动到实验室用户的"我的虚拟机"列表中。 这意味着实验室用户现在将在此计算机上具有所有者权限。 此步骤所需的时间可能因启动时间以及索赔事件期间执行的任何其他自定义操作而异。 一旦索赔，机器不再在可索赔池中可用。  
...
当用户取消声明 VM 时，他们不再拥有该特定实验室 VM 的所有者权限，并且可在将 VM 重新保留到池的状态下的任何其他实验室用户声明该 VM。 
