...
> [!IMPORTANT] 
> 启用**远程桌面连接**仅打开 Linux 计算机上的**RDP**端口。 作为教师, 你作为教师首次使用 SSH 连接到 Linux 计算机, 并安装 RDP 和 GUI 包, 以便以后可以使用 RDP 连接到 Linux 计算机。 然后,**发布**该映像, 以便学生可以通过 RDP 登录到学生 Linux vm。 
