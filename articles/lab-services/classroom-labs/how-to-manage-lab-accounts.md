...
4. 在“创建实验室帐户”页面的“基本信息”选项卡上，执行以下操作： 
    ...
    5. 对于“允许实验室创建者选择实验室位置”字段，请指定是否希望实验室创建者能够选择实验室位置。 默认情况下，此选项已禁用。 此选项禁用时，实验室创建者无法为他们正在创建的实验室指定位置。 实验室在距离实验室帐户最近的地理位置进行创建。 此选项启用时，实验室创建者可以在创建实验室时选择位置。 有关详细信息，请参阅[允许 lab creator 选取实验室的位置](allow-lab-creator-pick-lab-location.md)。 

        ![创建实验室帐户-> 基础知识](../media/how-to-manage-lab-accounts/create-lab-account-basics.png)
5. 选择页面底部的 "**下一步：高级**" 以导航到 "**高级**" 选项卡，然后执行以下步骤： 
    ...
    2. 指定是否想要在用户断开连接时**自动关闭 Windows 虚拟机**。 指定虚拟机在自动关机之前应等待用户重新连接的时间。 
    ...
    8. 为实验室中的 VM 指定**地址范围**。 地址范围应为无类域间路由（CIDR）表示法（例如： 10.20.0.0/23）。 将在此地址范围内创建实验室中的虚拟机。 有关详细信息，请参阅[在实验室中为 Vm 指定地址范围](how-to-configure-lab-accounts.md#specify-an-address-range-for-vms-in-the-lab)  

        ![创建实验室帐户-> 高级](../media/how-to-manage-lab-accounts/create-lab-account-advanced.png)  
6. 选择页面底部的 "**下一步：标记**" 以切换到 "**标记**" 选项卡。添加要与实验室帐户关联的任何标记。 标记是名称/值对，通过将相同的标记应用到多个资源和资源组，可以对资源进行分类并查看合并的帐单。 有关详细信息，请参阅[使用标记来组织 Azure 资源](../../azure-resource-manager/management/tag-resources.md)。

    ![创建实验室帐户-> 标记](../media/how-to-manage-lab-accounts/create-lab-account-tags.png)
7. 选择此页底部的 "**查看 + 创建**"，切换到 "**查看**" 和 "创建" 选项卡。 
4. 查看此页上的摘要信息，然后选择 "**创建**"。 

    ![创建实验室帐户-> 标记](../media/how-to-manage-lab-accounts/create-lab-account-review-create.png)
5. 等待部署完成，展开 "**后续步骤**"，并选择 "**中转到资源**"，如下图所示： 

    你还可以选择工具栏上的**电铃图标**（**通知**），确认部署已成功，然后选择 "**前往资源**"。 
...
> [!NOTE]
> 你还可以使用 LabServices PowerShell 模块（预览版）来管理实验室帐户。 有关详细信息，请参阅[GitHub 上的 LabServices 主页](https://github.com/Azure/azure-devtestlab/tree/master/samples/ClassroomLabs/Modules/Library)。
