...
可以通过两种方式启动选定的 Vm （一个或多个）。 第一种方法是在列表中选择 VM 或 Vm，然后在工具栏上选择 "**启动**"。 第二种方法是在列表中选择 VM 或 vm，在其中一个行的 "**状态**" 列中选择 "下拉"，然后选择 "**启动**"。 

![启动所选 Vm](../media/how-to-set-virtual-machine-passwords/start-selected-vms.png)

同样，可以使用 "**状态**" 列中的下拉列表或工具栏上的 "**停止**" 来停止一个或多个 vm。 
