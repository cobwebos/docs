...
需要设置特定的注册表项，以便告诉适用于 android 的 Visual Studio 仿真程序（Android Sdk 位于该位置）。  若要设置所需的注册表项，请运行以下脚本。  下面的 PowerShell 脚本假定 Android Sdk 的默认安装位置。  如果你的 Android Sdk 安装在其他位置，请在运行该`$androidSdkPath`脚本之前修改的值。
...
25名\*学生（20个计划 + 10 个配额）小时数 * 55 实验室单位 * 0.01 美元/小时 = 412.5 usd
