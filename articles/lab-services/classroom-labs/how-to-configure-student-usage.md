...
1. 选择左侧菜单上的“用户”  。
2. 选择**每个用户的配额：** 工具栏上。 
3. 上**每个用户的配额**页上，指定你想要为每个用户授予 （学生） 的小时数： 
    1. **0 小时 （仅计划）** 。 请仅在计划时间或作为实验室所有者在 Vm 上打开它们时，用户可以使用其虚拟机。

        ![零小时数 – 计划的时间](../media/how-to-configure-student-usage/zero-hours.png)
    1. **每个用户的实验室小时总数**。 用户可以将其虚拟机使用的小时 （指定为此字段） 的集编号**除了计划的时间**。 如果选择了此选项，请在文本框中输入**小时数**。 
...
> [!IMPORTANT]
> 在发送前的注册链接到学生，教师必须设置为类计划，如果他们选择 0 配额小时数或指定实验室配额小时数。
>
