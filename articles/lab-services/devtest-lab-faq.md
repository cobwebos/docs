...
description: 本文提供了有关 Azure 开发人员测试实验室的一些常见问题 （FAQ） 的解答。
...
我们的 DevTest Labs 团队博客已于 2019 年 3 月 20 日停用。 

### <a name="where-can-i-track-feature-updates-from-now-on"></a>从现在起，我在哪里可以跟踪功能更新？
从现在起，我们将在 Azure 博客和 Azure 更新上发布功能更新和信息性博客文章。 这些博客文章还将链接到我们的文档，只要有需要。

订阅[开发人员测试实验室 Azure 博客](https://azure.microsoft.com/blog/tag/azure-devtest-labs/)和[开发人员测试实验室 Azure 更新](https://azure.microsoft.com/updates/?product=devtest-lab)，随时了解 DevTest 实验室中的新功能。

### <a name="what-happens-to-the-existing-blog-posts"></a>现有的博客文章会发生什么情况？
我们目前正在努力将现有的博客文章（不包括中断更新）迁移到[我们的 DevTest Labs 文档](devtest-lab-overview.md)。 弃用 MSDN 博客时，它将重定向到 DevTest Labs 的文档概述。 重定向后，您可以在"筛选者"标题中搜索要查找的文章。 我们还没有迁移所有职位，但应在本月底完成。 
...
我们的推特句柄：[@azlabservices](https://twitter.com/azlabservices)
...
Microsoft 帐户可用于通过 Microsoft 设备和服务执行的几乎所有操作。 它是用于登录 Skype、Outlook.com、OneDrive、Windows 电话、Azure 和 Xbox Live 的电子邮件地址和密码。 单一帐户意味着文件、照片、联系人和设置可跟随用户移动到任何设备。
...
“无忧的自助服务”是指开发人员和测试人员按需创建自己的环境。 管理员可以放心地知道，DevTest Labs 可以帮助设置适当的访问，最大限度地减少浪费和控制成本。 管理员可以指定允许的 VM 大小、最大 VM 数量以及 VM 的启动和关机时间。 开发测试实验室还可简化成本监视和警报设置，可帮助用户始终注意实验室资源的使用方式。

### <a name="how-can-i-use-devtest-labs"></a>如何使用开发测试实验室？
DevTest Labs 在需要开发或测试环境时非常有用，并且希望快速重现这些环境，或者使用节省成本的策略对其进行管理。
...
- **实验室用户**：实验室用户可查看所有实验室资源（例如 VM、策略和虚拟网络）。 但是，实验室用户无法修改其他用户创建的策略或任何 VM。
...
由于作用域是分层的，因此在用户具有某个作用域的权限时，系统会自动向该用户授予该作用域内每个较低级别的作用域的权限。 例如，如果向用户分配了订阅所有者角色，那么用户可访问订阅中的所有资源。 这些资源包括 VM、虚拟网络和实验室。 订阅所有者会自动继承实验室所有者的角色。 但是反过来则不适用。 实验室所有者具有访问实验室的权限，其低于订阅级别的作用域。 因此，实验室所有者无法看到 VM、虚拟网络或实验室外部的任何其他资源。
...
中央 IT 应仅拥有所需的内容，并使项目和应用程序团队具有所需的控制级别。 通常情况下，这意味着中心 IT 部门拥有订阅并负责核心 IT 功能，例如网络配置。 订阅的**所有者**集应该小。 这些所有者可以在有需要时提名其他所有者，或应用订阅级策略，例如"无公共 IP"。

部分用户（例如第 1 层或第 2 层支持人员）可能需要在整个订阅中进行访问。 在这种情况下，建议为这些用户提供管理资源所需的“参与者”访问权限，但不提供用户访问权限，也不调整策略。****

开发测试实验室资源应该由那些接近项目/应用程序团队的所有者拥有。 这是因为他们了解他们对机器和所需软件的要求。 在大多数组织中，此开发测试实验室资源的所有者通常是项目/开发主管。 此所有者可以管理实验室环境中的用户和策略，可以管理开发测试实验室环境中的所有 VM。

项目/应用程序团队成员应添加到**开发人员测试实验室用户角色**。 这些用户可以创建虚拟机（前提是遵循实验室和订阅级别的策略）。 他们也可以管理自己的虚拟机。 他们不能管理属于其他用户的虚拟机。

有关详细信息，请参阅[Azure 企业基架 – 规范性的订阅治理文档](/azure/architecture/cloud-adoption/appendix/azure-scaffold)。


### <a name="how-do-i-create-a-role-to-allow-users-to-do-a-specific-task"></a>如何创建角色以允许用户执行特定任务？
...
- 开发自定义映像、自定义项目和部署过程，以便在使用 Active Directory 定义的安全领域内进行协调。 此方法会强制实施企业边界，并规定一套通用的环境控制措施。 开发人员在整个过程中开发和遵循安全开发生命周期时，可以考虑这些针对环境的控制措施。 目标还旨在提供一个不过度限制、可能阻碍开发的环境，而是提供一套合理的控制。 包含实验室虚拟机的组织单位 (OU) 的组策略可以是生产环境中的所有组策略的子集。 或者，它们可以是一个用于适当缓解任何所识别到的风险的补充集。
...
远程访问的第一层控件是通过未直接连接到实验室的 VPN 连接应用远程访问策略。

第二个控制层是应用组策略对象集，以防止通过远程桌面进行复制粘贴。 可以实施一个网络策略来禁止环境中的出站服务，例如环境外部的 FTP 和 RDP 服务。 用户定义的路由可能会强制所有 Azure 网络流量返回本地，但路由不能考虑所有可能允许上传数据的 URL，除非通过代理进行控制以扫描内容和会话。 在支持开发测试实验室的虚拟网络中，公共 IP 可能限制为禁止桥接外部网络资源。

最终，需要在组织中应用相同类型的限制，这将考虑所有可能采用的可移动媒体或外部 URL 方法，这些方法可以接受内容帖子。 评审和实施安全策略时，请咨询安全专家。 有关更多建议，请参阅 [Microsoft 网络安全性](https://www.microsoft.com/security/default.aspx?&WT.srch=1&wt.mc_id=AID623240_SEM_sNYnsZDs)。

## <a name="lab-configuration"></a>实验室配置

### <a name="how-do-i-create-a-lab-from-a-resource-manager-template"></a>如何从资源管理器模板模板创建实验室？
我们提供了[实验室 Azure 资源管理器模板的 GitHub 存储库](https://azure.microsoft.com/resources/templates/101-dtl-create-lab)，可按原样部署，也可对其进行修改，为实验室创建自定义模板。 每个模板都有一个链接，用于部署实验室，因为它在您自己的 Azure 订阅中。 或者，可自定义模板，并[使用 PowerShell 或 Azure CLI 进行部署](../azure-resource-manager/templates/deploy-powershell.md)。


### <a name="can-i-have-all-virtual-machines-to-be-created-in-a-common-resource-group-instead-having-each-machine-in-its-own-resource-group"></a>是否可以将所有虚拟机创建在公共资源组中，而不是在自己的资源组中创建每台计算机？
可以，作为实验室所有者，您可以让实验室为您处理资源组分配，也可以在指定的公共资源组中创建所有虚拟机。

单独的资源组方案：
-   DevTest Labs 为您旋转的每个公共/专用 IP 虚拟机创建新的资源组
-   DevTest Labs 为属于相同大小的共享 IP 计算机创建资源组。

常见资源组方案：
-   所有虚拟机都在指定的公共资源组中旋转。 为实验室了解更多[资源组分配](https://aka.ms/RGControl)。
...
### <a name="how-do-i-determine-the-ratio-of-users-per-lab-and-the-overall-number-of-labs-that-are-needed-across-an-organization"></a>如何确定每个实验室的用户比例以及整个组织所需的实验室总数？
我们建议将同一开发项目关联的业务单位和开发组关联到同一实验室。 这样，便可以对这两个组应用相同类型的策略、映像和关闭策略。

可能还需要考虑地理边界。 例如，美国东北部的开发人员可以使用在东部 US2 中预配的实验室。 另外，位于德克萨斯州达拉斯市和科罗拉多州丹佛市的开发人员可能会定向为使用美国中南部的资源。 如果能够与外部第三方展开协作，则可为这些开发人员分配未由内部开发人员使用的实验室。

您还可以在 Azure DevOps 项目中为特定项目使用实验室。 然后，通过允许访问这两组资源的指定 Azure Active Directory 组应用安全性。 分配到该实验室的虚拟网络可以是用于整合用户的另一个边界。

### <a name="how-can-we-prevent-the-deletion-of-resources-within-a-lab"></a>如何防止删除实验室中的资源？
我们建议在实验室级别设置适当的权限，以便只有经过授权的用户才能删除资源或更改实验室策略。 应将开发人员放置在“开发测试实验室用户”组中。**** 开发人员主管或基础结构主管应是“开发测试实验室所有者”。**** 我们建议您只有两个实验室所有者。 此策略根据代码存储库的扩展而扩展，以避免损坏。 实验室用户有权使用资源，但不能更新实验室策略。 请参阅以下文章，其中列出了每个内置组在实验室中拥有的角色和权限：[在 Azure 开发测试实验室中添加所有者和用户](devtest-lab-add-devtest-user.md)。

### <a name="how-do-i-share-a-direct-link-to-my-lab"></a>如何向我的实验室共享直接链接？

1. 在[Azure 门户](https://portal.azure.com)中，转到实验室。
2. 从浏览器复制**实验室 URL，** 然后与实验室用户共享。
...
### <a name="why-cant-i-see-vms-on-the-virtual-machines-page-that-i-see-in-devtest-labs"></a>为什么我无法在 DevTest 实验室中看到的虚拟机页面上看到 VM？
在 DevTest 实验室中创建 VM 时，您被授予访问该 VM 的权限。 您可以在实验室页面和 **"虚拟机"** 页上查看 VM。 分配给**DevTest 实验室所有者**角色的用户可以在实验室的 **"所有虚拟机"** 页上查看实验室中创建的所有 VM。 但是，具有**DevTest Labs 用户角色**的用户不会自动被授予对其他用户创建的 VM 资源的读取访问权限。 因此，这些 VM 不会显示在 **"虚拟机"** 页上。
...
- 您可以在创建 VM 时[生成资源管理器模板](devtest-lab-add-vm.md#save-azure-resource-manager-template)，并从[Windows PowerShell 部署资源管理器模板](../azure-resource-manager/templates/deploy-powershell.md)。
- 您还可以指定在虚拟机创建期间要创建的计算机的多个实例。 要了解有关创建虚拟机多个实例的更多详细信息，请参阅有关[创建实验室虚拟机](devtest-lab-add-vm.md)的文档。
...
2.  在 DevTest 实验室实验室内创建[自定义映像](devtest-lab-create-template.md)。
3.  在实验室中从自定义映像创建 VM。
...
要在 Azure 中开发或测试 Windows 客户端操作系统映像（Windows 7 或更高版本），请执行以下步骤之一：

- [购买 MSDN 订阅](https://www.visualstudio.com/products/how-to-buy-vs)。
- 如果您有企业协议，请使用[企业开发/测试产品/产品/服务](https://azure.microsoft.com/offers/ms-azr-0148p)创建 Azure 订阅。
...
作为实验室所有者，可以从 Azure 门户中的实验室中删除 VM。 还可以使用 PowerShell 脚本删除实验室中的所有 VM。 在下面的示例中，在**要更改注释的值**下，修改参数值。 可以从 Azure 门户中的实验室窗格中检索订阅 Id、实验室资源组和实验室名称值。
...
通过使用[DevTest Labs](devtest-lab-test-env.md)一文中"环境"功能中提到的步骤，将资源管理器模板部署到开发人员测试实验室环境中。 从根本上说，将资源管理器模板签入 Git 存储库（Azure Repos 或 GitHub），并将[模板的专用存储库](devtest-lab-test-env.md)添加到实验室。 如果使用 DevTest Labs 托管开发计算机，则此方案可能很有用，但如果构建临时环境（代表生产环境）则可能很有用。

还值得注意的是，每个实验室或每个用户选项的虚拟机数仅限制在实验室本身中本机创建的计算机数，而不是由任何环境（资源管理器模板）创建。
...
在[图像工厂模式上](https://sec.ch9.ms/ch9/8e8a/9ea0b8d4-b803-4f23-bca4-4808d9368e8a/dtlimagefactory_mid.mp4)查看此视频。 此方案是一个高级方案，提供的脚本都只是示例脚本。 如果需要进行任何更改，则需要管理和维护在环境中使用的脚本。

有关创建映像工厂的详细信息，请参阅[在 Azure DevTest 实验室中创建自定义映像工厂](image-factory-create.md)。
...
通常，此方案中的决定因素是成本和重用。 如果在某些情况下，许多用户/实验室需要一个在基本映像之上包含大量软件的图像，则可以通过创建自定义映像来降低成本。 这意味着映像创建一次。 它减少了虚拟机设置时间以及由于在设置时运行虚拟机而产生的成本。

但是，需要注意的另一个因素是软件包的更改频率。 如果运行每日内部版本，并要求该软件位于用户的虚拟机上，请考虑使用公式而不是自定义映像。

有关更深入的说明，请参阅在 DevTest 实验室[中比较自定义图像和公式](devtest-lab-comparing-vm-base-image-types.md)。
...
- 安装和设置应用程序可能需要数小时，这不能有效地利用在 Azure 应用商店映像上添加的计算时间。
- 开发人员和测试人员需要快速访问虚拟机，并希望最大限度地缩短新虚拟机的设置时间。
- 所有计算机必须具备的合规性或法规条件（例如安全策略）。
- 使用自定义图像不应掉以轻心。 它们会带来额外的复杂性，因为您现在必须管理这些基础映像的 VHD 文件。 此外，还需要使用软件更新定期修补这些基础映像。 这些更新包括新的操作系统 (OS) 更新，以及软件包本身所需的任何更新或配置更改。

## <a name="artifacts"></a>项目

### <a name="what-are-artifacts"></a>项目是什么？
项目是用于将最新位或开发工具部署到 VM 的可自定义元素。 创建 VM 时，将项目附加到 VM。 预配 VM 后，项目可以部署和配置 VM。 在我们的[公共 GitHub 存储库](https://github.com/Azure/azure-devtestlab/tree/master/Artifacts)中提供了各种预先存在的工件。 也可以[创作自己的项目](devtest-lab-artifact-author.md)。
...
- 如果组织的治理策略需要隔离，并且外部存储库不受组织的直接配置管理，则应部署私有工件存储库。 在此过程中，可以复制公共存储库的初始副本，并将其与专用存储库集成。 然后可以禁用公共存储库，使组织中的任何人都不再可以访问它。 此方法会强制组织中的所有用户只能使用组织批准的单个存储库，并可以最大程度地减少配置偏差。
...
- 在 Azure`All DevTest Labs Developers`活动目录中创建在集中管理的组。 应将参与项目开发的任何开发人员置于此组中。
...
如果使用 Azure DevOps，则可以使用[DevTest Labs 任务扩展程序](https://marketplace.visualstudio.com/items?itemName=ms-azuredevtestlabs.tasks)在 DevTest 实验室中自动执行发布管道。 可使用此扩展执行的任务包括：
...
如果您的 VM 需要与现有基础结构进行交互，请考虑在 DevTest Labs 环境中使用现有的虚拟网络。 如果使用 ExpressRoute，您可能希望最小化 VNet/子网的数量，以便不会分割分配给订阅的 IP 地址空间。

请考虑在此处使用 VNet 对等互连模式 （[中心分支模型](/azure/architecture/reference-architectures/hybrid-networking/hub-spoke)）。 此方法支持跨订阅的 vnet/子网通信。 除此之外，每个开发测试实验室环境可以有其自己的虚拟网络。
...
- **入站流量**– 如果虚拟机没有公共 IP 地址，则 Internet 无法访问该 IP 地址。 常见方法是确保设置订阅级策略，以便任何用户都无法创建公共 IP 地址。
- **出站流量**– 如果要防止虚拟机直接访问公共互联网并强制流量通过公司防火墙，则可以使用强制路由通过快速路由或 VPN 路由本地流量。

> [!NOTE]
> 如果您有一个代理服务器，无需代理设置即可阻止流量，则不要忘记向实验室的项目存储帐户添加异常。
...
若一个资源是另一个资源的父资源，则该父资源必须存在才可创建子资源。 如果父资源不存在，您将看到**父资源未找到**消息。 如果未指定与父资源的依赖关系，子资源可能会在父资源之前部署。
...
活动日志中会捕获 VM 部署错误。 您可以在实验室 VM 页上的资源菜单上的 **"审核日志**"或 **"虚拟机诊断"** 下找到实验室 VM 活动日志（从"我的虚拟机"列表中选择 VM 后，该页将出现）。
...
### <a name="why-do-i-get-location-is-not-available-for-resource-type-error-when-trying-to-create-a-lab"></a>为什么在尝试创建实验室时，我会收到"位置不适合资源类型"的错误？
当您尝试创建实验室时，您可能会看到类似于以下错误的错误消息：
...
在["按区域可用的产品](https://azure.microsoft.com/global-infrastructure/services/)"页上检查 Azure 区域中资源类型的可用性。 如果资源类型在特定区域不可用，DevTest Labs 不支持在该区域创建实验室。 创建实验室时选择其他区域。

#### <a name="option-2"></a>方法 2
如果资源类型在您所在区域可用，请检查其是否已注册到您的订阅。 它可以在订阅所有者级别完成，[如本文](../azure-resource-manager/management/resource-providers-and-types.md)所示。
