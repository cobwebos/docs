...
Twitter 句柄： [@azlabservices](https://twitter.com/azlabservices)
...
安全访问由基于角色的访问控制（RBAC）决定。 要了解访问权限的工作方式，最好先了解 RBAC 定义的权限、角色与作用域之间的差异。
...
> [!NOTE]
> 如果实验室用户是具有 Microsoft 帐户但不是组织 Active Directory 实例成员的外部用户，则用户在尝试访问共享链接时可能会看到一条错误消息。 如果外部用户看到错误消息，请让该用户首先在 Azure 门户的右上角选择其名字。 然后，在菜单的 "目录" 部分中，用户可以选择实验室所在的目录。
...
- 在集中管理的 Azure Active Directory 中创建名为 `All DevTest Labs Developers` 的组。 应将参与项目开发的任何开发人员置于此组中。
