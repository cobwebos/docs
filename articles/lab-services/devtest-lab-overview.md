...
借助 Azure 开发测试实验室，团队开发人员无需等待批准即可有效地自行管理虚拟机（Vm）和 PaaS 资源。
...
- 通过使用资源管理器模板在实验室上下文内浏览 Azure，加速空资源组（沙盒）。
...
- [在实验室中设置策略](devtest-lab-get-started-with-lab-policies.md)，例如每个用户或每个实验室的 vm 数。 
...
使用预置的插件或 API 直接从你喜欢的[持续集成（CI）工具](devtest-lab-integrate-ci-cd-vsts.md)、集成开发环境（IDE）或自动发布管道设置开发/测试环境。 你还可以使用全面的命令行工具。
