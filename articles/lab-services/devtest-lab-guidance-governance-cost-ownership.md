...
- 可以访问库中唯一的开发/测试映像，包括 Windows 8.1 和 Windows 10
 
只有活动的 Visual Studio 订阅者（标准订阅、年度云订阅和月度云订阅）可使用企业开发/测试订阅中运行的 Azure 资源， 但最终用户也可访问该应用程序来提供反馈或执行验收测试。 在此订阅内使用的资源限制为开发和测试应用程序，且不提供运行时间保证。
