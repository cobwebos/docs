...
description: "本文介绍如何使用 Microsoft Monitoring Agent (MMA) 将本地基础结构中托管的 Windows 计算机连接到 OMS。"
...
OMS Agent for Linux 通过 TCP 端口 443 与 OMS 服务进行出站通信；如果计算机连接到防火墙或代理服务器以通过 Internet 进行通信，则请查看[配置代理和防火墙设置](log-analytics-proxy-firewall.md)，了解需要应用哪些配置更改。  如果使用 System Center 2016 - Operations Manager 或 Operations Manager 2012 R2 监视计算机，该计算机可以与 OMS 服务进行多宿主连接，以便收集数据并将数据转发到该服务，且仍受 Operations Manager 监视。  受 Operations Manager 管理组监视并与 OMS 集成的 Linux 计算机不通过管理组接收数据源配置和转发收集的数据。  
...
## <a name="install-the-oms-agent-for-linux"></a>安装 OMS Agent for Linux
...
### <a name="bundle-command-line-arguments"></a>捆绑包命令行参数
```
Options:
...
#### <a name="to-install-the-agent-packages-and-onboard-at-a-later-time"></a>安装代理包并在以后载入
```
sudo sh ./omsagent-1.3.0-1.universal.x64.sh --upgrade
```

#### <a name="to-extract-the-agent-packages-from-the-bundle-without-installing"></a>从捆绑包中提取代理包，但不安装
```
...
可在以下文件中设置代理配置：`/etc/opt/microsoft/omsagent/proxy.conf`可直接创建或编辑此文件，但必须可由 omsagent 用户读取。 例如：
```
...
## <a name="manage-omsagent-daemon"></a>管理 omsagent 守护程序
...
### <a name="debian--ubuntu"></a>Debian 和 Ubuntu
```
...
新功能！ 如果是时间差导致载入失败，最新版 OMS Agent for Linux 现在会通知用户使用正确的工作区 ID 和工作区密钥指令重新载入
