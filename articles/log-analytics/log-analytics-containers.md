...
---
# <a name="containers-preview-solution-in-log-analytics"></a>Log Analytics 中的容器（预览）解决方案
...
使用该解决方案，可以查看容器主机上正在运行的容器以及容器中运行的映像。 可以查看详细审核信息，它显示了与容器一起使用的命令。 并且，用户可以通过查看和搜索集中式日志来排查容器问题，而无需远程查看 Docker 或 Windows 主机。 可以在主机上找到可能具有干扰性并且占用过多资源的容器。 并且，可以查看容器的集中式 CPU、内存、存储器、网络使用情况和性能信息。 在运行 Windows 的计算机上，可以集中比较 Windows Server、Hyper-V 和 Docker 容器中的日志。
...
| 服务<br>Fabric | | | 是 | 是 | 是 | 是 | 是 | 是 | 是 | 是 | 
| Red Hat Open<br>Shift | | 是 | | 是 | 是| 是 | 是 | 是 | | 是 | 
| Windows Server<br>（独立） | | | 是 | 是 | 是 | 是 | 是 | 是 | | 是 |
| Linux Server<br>（独立） | | 是 | | 是 | 是 | 是 | 是 | 是 | | 是 |
...
1. 从 [Azure 应用商店](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.ContainersOMS?tab=Overview)或使用[从解决方案库中添加 Log Analytics 解决方案](log-analytics-add-solutions.md)中所述的过程，将容器解决方案添加到 OMS 工作区。

2. 通过 OMS 来安装和使用 Docker。  可根据操作系统选择以下方法：
...
需要在容器主机上配置以下设置，然后才能监视容器。

## <a name="linux-container-hosts"></a>Linux 容器主机

安装 Docker 之后，请使用以下容器主机设置来配置代理以供 Docker 使用。 首先，需要 OMS 工作区 ID 和密钥，可通过切换到 [OMS 经典门户](https://mms.microsoft.com)来找到它们。  在“概述”页面上，从顶部菜单中选择“设置”，然后导航到“连接的源\Windows 服务器”。  可在“工作区 ID”和“主键”的右侧看到值。  将它们复制并粘贴到喜爱的编辑器中。    
...
在 OMS 门户中启用解决方案后，会看到**容器**磁贴显示有关容器主机和在主机中运行的容器的摘要信息。
...
例如，通过将搜索查询中的“**正在运行**”更改为“**停止**”，可以对搜索查询进行修改，使其显示所有已停止的容器，而不是正在运行的容器。

## <a name="troubleshoot-by-finding-a-failed-container"></a>通过查找失败的容器进行故障排除
如果容器退出时带有非零退出代码，则 OMS 会将其标记为“**失败**”。 可以在“**失败容器**”边栏选项卡中查看环境中的错误和故障的概述。
...
4. 接下来，找到运行此映像的容器。 在搜索查询中键入以下内容。
   `Type=ContainerInventory <ImageID>` 此时会显示日志。 你可以滚动查看失败的容器。  
...
* 选择一个最近失败的映像，并找到它的错误日志。 首先，通过使用 **ContainerInventory** 搜索查找运行该映像的容器名称。 例如，搜索 `Type=ContainerInventory ubuntu Failed`  
  ...
  记下**名称**旁边容器的名称，并搜索这些日志。 在此示例中，它是 `Type=ContainerLog adoring_meitner`。
...
从一或两个示例开始生成查询，并修改它们以适应环境，这通常很有用。 可以首先尝试使用“**重要查询**”边栏选项卡，它可以帮助你构建更高级的查询。
