...
Log Analytics 的**自定义字段**功能使你可以通过添加自己的可搜索字段来扩展 OMS 存储库中的现有记录。  自动填充自定义字段的数据是从同一记录的其他属性中提取的。

![自定义字段概述](media/log-analytics-custom-fields/overview.png)

例如，以下示例记录将有用数据隐藏在事件描述中。  将此数据提取到单独的属性中，使其可供排序和筛选等操作使用。
...
创建自定义字段时，Log Analytics 必须了解将使用哪些数据填充其值。  将使用由 Microsoft Research 开发的 FlashExtract 技术来快速找出此数据。  不需要你提供确切说明，Log Analytics 就会明白要从你提供的示例中提取的数据。
...
2. 为了提取要填充自定义字段的数据，请选择 Log Analytics 将用于充当模型的记录。  确定要从该记录中提取的数据，然后 Log Analytics 将使用此信息为所有类似记录确定用于填充自定义字段的逻辑。
3. 单击该记录的任何文本属性左侧的按钮，然后选择“字段提取自”。
4. 将打开“字段提取向导”，所选记录会显示在“主示例”列中。  将为所选属性中值相同的那些记录定义自定义字段。  
5. 如果所选内容不完全是所需要的，可选择其他字段来缩小条件范围。  若要更改条件的字段值，必须先取消匹配所需条件的记录，然后再选择其他记录。

### <a name="step-2---perform-initial-extract"></a>步骤 2 - 执行初始提取。
在完成确定将具有自定义字段的记录后，请确定要提取的数据。  Log Analytics 将使用此信息在类似记录中确定类似模式。  在此之后的步骤中，将可以验证结果，并提供更多详细信息供 Log Analytics 在其分析中使用。

1. 突出显示示例记录中要用于填充自定义字段的文本。  然后将显示一个对话框，用于命名字段并执行初始提取。  将自动附加字符 **\_CF**。
2. 单击“提取”以执行已收集记录的分析。  
...
5. 如果对结果满意，请单击“保存提取”。  自定义字段现已定义，但它尚未添加到任何记录。
6. 等待收集匹配指定条件的新记录，然后再次运行日志搜索。 新记录应具有自定义字段。
...
有两种方法可以删除自定义字段。  第一种方法是在查看上述完整列表时，对每个字段使用“删除”选项。  另一种方法是检索记录，然后单击字段左侧的按钮。  菜单中将有一个选项可用于删除自定义字段。
...
我们需要 **RenderedDescription** 属性中出现的服务名称，然后选择该属性旁边的按钮。
...
我们从“WMI 性能适配器”记录开始。  单击该记录的编辑图标，然后单击“修改此突出显示”。  

![修改突出显示](media/log-analytics-custom-fields/modify-highlight.png)

我们会增加突出显示以包括单词 **WMI**，然后重新运行该提取。  
...
滚动到含有 DPMRA 服务的记录，然后采用相同过程对该记录进行修正。
