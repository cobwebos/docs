...
下例中，请创建一个指标度量警报规则，当查询中某个值超过阈值的 90% 时，该规则为每台计算机对象创建一个警报。
...
5. 使用以下信息配置警报规则：  
   ...
   c. “警报频率”用于指定应运行查询的频率。  对于本示例，请指定 5 分钟，这在指定的时间范围内。  
   d.单击“下一步”。 选择 **“指标度量”** ，输入 **90** 作为 *“聚合值”* ，并输入 *3* 作为 **“触发警报基数”**    
   e. 在“操作”下禁用电子邮件通知。
6. 单击“**保存**”完成警报规则。 该警报会立即开始运行。<br><br> ![警报规则示例](media/log-analytics-tutorial-response/log-analytics-alert-01.png)
...
6. 单击“**保存**”完成警报规则。 该警报会立即开始运行。<br><br> ![使用电子邮件通知的警报规则](media/log-analytics-tutorial-response/log-analytics-alert-02.png)
