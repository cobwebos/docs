...
1. 请确保使用的 OMS 工作区和用来创建所有群集资源的 Azure 订阅关联（包括存储帐户）。 请参阅 [Log Analytics 入门](log-analytics-get-started.md) 了解有关创建 OMS 工作区的信息。
2. 配置 OMS 以收集和查看 Service Fabric 日志。
3. 在工作区中启用 Service Fabric 解决方案。

## <a name="configure-oms-to-collect-and-view-service-fabric-logs"></a>配置 OMS 以收集和查看 Service Fabric 日志
在本节中将了解如何配置 OMS 以检索 Service Fabric 日志。 日志允许你使用 OMS 门户查看、分析和解决群集或该群集中运行的应用程序和服务中的问题。

> [!NOTE]
> 必须将 Azure 诊断扩展配置为将日志上载到与 OMS 搜索目标匹配的存储表。 请参阅 [如何使用 Azure 诊断收集日志](../service-fabric/service-fabric-diagnostics-how-to-setup-wad.md) 了解有关如何收集日志的详细信息。 本文中的配置设置示例将显示存储表的名称。 在群集上设置“诊断”且并将日志上载到存储帐户后，下一步是配置 OMS 以收集这些日志。
>
>

请确保先更新“**template.json**”中的“**EtwEventSourceProviderConfiguration**”部分，以为新的 EventSources 添加条目，然后再通过运行“**deploy.ps1**”应用配置更新。 要上载的表与 (ETWEventTable) 相同。 目前，OMS 仅可以读取此表中的应用程序 ETW 事件。 但是，正在开发对自定义 ETW 表的支持。
...
### <a name="configure-an-oms-workspace-to-show-the-cluster-logs"></a>配置 OMS 工作区以显示群集日志
如上所述创建 OMS 工作区之后，接下来请配置工作区，以便从 Azure 存储表中提取诊断扩展从群集上载的日志。 运行以下 PowerShell 脚本以实现此操作：
...
配置 OMS 工作区读取存储帐户中的 Azure 表后，登录 Azure 门户，并在“**全部资源**”中选择“OMS 工作区”。 选择后，应看到连接到该 OMS 工作区的存储帐户日志的数量。 选择“**存储帐户日志**”磁贴，并在存储帐户日志列表中验证存储帐户是否已连接到该 OMS 工作区：

![存储帐户日志](./media/log-analytics-service-fabric/sf1.png)

## <a name="enable-the-service-fabric-solution"></a>启用 Service Fabric 解决方案
使用以下脚本向 OMS 工作区添加解决方案。 使用已与要在其中启用 Service Fabric 解决方案的 OMS 工作区关联的 Azure 订阅，在 PowerShell 中运行脚本。
...
启用解决方案后，Service Fabric 磁贴将添加到“OMS 概述”页面，并附带值得注意的问题视图，例如 runAsync 故障和最近 24 小时内发生的取消等。

![Service Fabric 磁贴](./media/log-analytics-service-fabric/sf2.png)

### <a name="view-service-fabric-events"></a>查看 Service Fabric 事件
单击“**Service Fabric**”磁贴打开 Service Fabric 仪表板。 仪表板包含下表中的列。 每个列按照指定时间范围内符合该列条件的计数列出了前十个事件。 可以通过在每一列右下方单击“查看全部”或单击列标题来运行提供整个列表的日志搜索。
...
## <a name="troubleshoot-your-service-fabric-and-oms-configuration"></a>Service Fabric 和 OMS 配置故障排除
如果因无法在 OMS 中查看事件数据而需要检查 OMS 配置，请使用以下脚本。 该脚本读取 Service Fabric 诊断配置，检查要写入表中的数据，并验证是否将 OMS 配置为从表中读取。
