---
title: "Azure Log Analytics PowerShell 示例 | Microsoft Docs"
description: "Azure Log Analytics PowerShell 示例"
services: log-analytics
documentationcenter: 
author: mgoedtel
manager: carmonm
editor: tysonn
tags: 
ms.assetid: 
ms.service: log-analytics
ms.devlang: na
ms.topic: article
ms.tgt_pltfrm: na
ms.workload: na
ms.date: 09/14/2017
ms.author: magoedte
ms.custom: mvc
ms.openlocfilehash: 5fc4d1620706019ad7f0ba7affbdcf9f3688e1a1
ms.sourcegitcommit: 6699c77dcbd5f8a1a2f21fba3d0a0005ac9ed6b7
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/11/2017
---
# <a name="azure-log-analytics-powershell-samples"></a>Azure Log Analytics PowerShell 示例

下表包含 PowerShell 脚本示例的链接，这些脚本示例可用于在 Log Analytics 中执行各种功能。

| | |
|---|---|
|**创建工作区**||
| [创建 Log Analytics 工作区](scripts/log-analytics-powershell-sample-create-workspace.md?toc=%2fpowershell%2fmodule%2ftoc.json) | 在 Log Analytics 中创建工作区。|
| | |

