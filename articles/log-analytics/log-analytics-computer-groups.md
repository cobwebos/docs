...
description: "Log Analytics 中的计算机组允许你为一组特定的计算机设定日志搜索的范围。  本文介绍用于创建计算机组的不同方法以及如何在日志搜索中使用这些方法。"
...
Log Analytics 中的计算机组允许你为一组特定的计算机设定[日志搜索](log-analytics-log-searches.md)的范围。  每个组使用你定义的查询或通过从不同源导入组填充计算机。  当日志搜索中包括组时，结果仅限于与组中的计算机匹配的记录。
...
从日志搜索创建的计算机组将包含由定义的搜索查询返回的所有计算机。  每次使用计算机组都会运行此查询，以便反映创建组之后所做的任何更改。

使用以下过程从日志搜索创建计算机组。

1. [创建日志搜索](log-analytics-log-searches.md)，它将返回计算机列表。  搜索必须通过使用查询中类似 **Distinct Computer** 或 **measure count() by Computer** 等内容返回一组不同的计算机。  
2. 单击屏幕顶部的“**保存**”按钮。
3. 选择“**是**”**将此查询另存为计算机组：**。
4. 为组键入**名称**和**类别**。  如果已存在具有相同名称和类别的搜索，则会提示你进行覆盖。  不同类别中可以有采用相同名称的多个搜索。 
...
将 Log Analytics 配置为导入 Active Directory 组成员身份时，它将使用 OMS 代理分析任何加入域的计算机的组成员身份。  在 Log Analytics 中为 Active Directory 中的每个安全组创建一个计算机组，并且每台计算机会添加到与它们所属的安全组对应的计算机组。  此成员身份每 4 小时持续更新一次。  

将 Log Analytics 配置为从 Log Analytics“**设置**”中的“**计算机组**”菜单导入 Active Directory 安全组。  选择“**自动化**”，然后选择“**从计算机导入 Active Directory 组成员身份**”。  无需进一步的配置。
...
在将 Log Analytics 配置为导入 Active Directory 组成员身份时，它将使用 OMS 代理分析任何计算机的目标组成员身份。  如果使用客户端目标，连接到 OMS 并且属于任何 WSUS 目标组的所有计算机会将其组成员身份导入到 Log Analytics。 如果使用服务器端目标，OMS 代理应安装在 WSUS 服务器上，以使组成员身份信息导入到 OMS。  此成员身份每 4 小时持续更新一次。 
...
运行搜索时，首先解析搜索中包含的任何计算机组的成员。  如果组基于日志搜索，那么在执行顶级日志搜索前运行搜索以返回组成员。
