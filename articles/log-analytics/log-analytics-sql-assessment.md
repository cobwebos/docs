...
![SQL 运行状况检查符号](./media/log-analytics-sql-assessment/sql-assessment-symbol.png)
...
![SQL 运行状况检查磁贴的图像](./media/log-analytics-sql-assessment/sql-healthcheck-summary-tile.png)

![SQL 运行状况检查仪表板的图像](./media/log-analytics-sql-assessment/sql-healthcheck-dashboard-01.png)
...
* 一个 Log Analytics 工作区，用于在 Azure 门户中通过 Azure Marketplace 添加 SQL 运行状况检查解决方案。  只有 Azure 订阅中的管理员或参与者才能安装该解决方案。 
...
1. 如果该服务器尚不受 System Center 2016 - Operations Manager 或 Operations Manager 2012 R2 的监视，请安装 [Microsoft Monitoring Agent (MMA)](log-analytics-windows-agents.md)。
...
2. 使用以下查询列出对于环境中计算机失败的建议。
    ...
    下面是显示日志搜索查询的屏幕截图：<br><br> ![失败的建议](./media/log-analytics-sql-assessment/sql-assess-failed-recommendations.png)<br>
