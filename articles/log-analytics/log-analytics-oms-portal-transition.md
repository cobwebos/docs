...
- 使用 OMS 门户创建的更新计划，可能不会反映在 Azure 门户上的更新管理仪表板的计划更新部署或更新作业历史记录中。 此差距预期在 2018 年 6 月底予以解决。
- 只能通过 OMS 门户启用自定义日志预览版功能。 在 2018 年 6 月底，将为所有工作区自动启用此功能。
...
从 6 月 25 日到 7 月，开始将访问控制权限从 OMS 门户自动转换到 Azure 门户权限。 转换完成后，OMS 门户用户管理部分会将用户路由到 Azure 中的访问控制 (IAM)。 
