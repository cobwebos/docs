...
参阅 [Log Analytics 用户从 OMS 门户过渡到 Azure 门户时的常见问题](../log-analytics/log-analytics-oms-portal-faq.md)，了解有关如何过渡到 Azure 门户的信息。 欢迎向 LAUpgradeFeedback@microsoft.com 发送任何反馈、问题或忧虑。
...
不再能够使用 OMS 门户创建新工作区。 请遵照[在 Azure 门户中创建 Log Analytics 工作区](log-analytics-quick-create-workspace.md)中的指导，在 Azure 门户中创建新工作区。
...
由于支持[跨资源查询](log-analytics-cross-workspace-search.md)，因此不再需要使用这种方法来复制数据。 在这种情况下，现有的 Application Insights 解决方案将被弃用。 从 10 月开始，无法将新的 Application Insights 资源链接到 Log Analytics 工作区。 在 2019 年 1 月 15 日之前，现有的链接和仪表板将继续正常运行。
