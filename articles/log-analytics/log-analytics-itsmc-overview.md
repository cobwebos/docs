...
通过 ITSMC，你可以：
  ...
  - 在所选 ITSM 工具中将操作警报与偶发事故管理做法进行集成。
...
> 默认情况下，ITSMC 每 24 小时刷新连接数据一次。 若要即时刷新连接的数据以获取执行的任何编辑或模板更新，可单击连接旁边显示的“刷新”按钮。
...
配置连接器后，它将开始从所连接的 ITSM 产品/服务中收集数据。 初始同步应在几分钟内完成，具体要取决于 ITSM 产品/服务中的事件数和更改请求数。

> [!NOTE]
> - 通过 ITSMC 解决方案从 ITSM 产品中导入的数据将在 Log Analytics 中显示为 ServiceDesk_CL 类型的日志记录。
> - 日志记录包含一个名为 ServiceDeskWorkItemType_s 的字段，用于表示事件或更改请求（两种从 ITSM 产品中导入的数据）。

## <a name="data-synced-from-itsm-product"></a>从 ITSM 产品同步的数据
事件和更改请求会从 ITSM 产品同步到 Log Analytics 工作区。
...
## <a name="itsmc-integration-with-other-oms-solutions"></a>ITSMC 与其他 OMS 解决方案集成

ITSM 连接器目前支持与服务映射解决方案集成。

服务映射自动发现 Windows 和 Linux 系统上的应用程序组件并映射服务之间的通信。 它允许如所想一般作为提供重要服务的互连系统查看服务器。 服务映射显示任何 TCP 连接的体系结构中服务器、进程和端口之间的连接，只需安装代理，无需任何其他配置。
...
通过使用 ITSMC 解决方案，可配置 OMS 警报，以在连接的 ITSM 工具中触发工作项的创建。 请按以下过程操作：
...
此外，还可从日志记录直接在连接的 ITSM 源中创建工作项。 请按以下过程操作：
...
1.  如果从连接源的 UI 建立连接失败，并出现“保存连接时出错”消息，请执行以下步骤：
 - 对于 ServiceNow、Cherwell 和 Provance 连接，请确保正确输入了每个连接的用户名、密码、客户端 ID 和客户端密码。
        检查在相应 ITSM 产品中是否拥有足够的权限来建立连接。
 - 对于 Service Manager 连接，请确保成功部署了 Web 应用并创建了混合连接。 若要验证是否已成功建立与本地 Service Manager 计算机的连接，请访问建立[混合连接](log-analytics-itsmc-connections.md#configure-the-hybrid-connection)文档中详细介绍的 Web 应用 URL。
