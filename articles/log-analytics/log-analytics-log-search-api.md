...
本指南提供的基本教程介绍了如何使用 Operations Management Suite (OMS) 中的 Log Analytics 搜索 REST API，提供的示例向你演示如何使用这些命令。 本文中的某些示例参考了 Operational Insights，它是以前版本的 Log Analytics 的名称。

## <a name="overview-of-the-log-search-rest-api"></a>日志搜索 REST API 概述
Log Analytics 搜索 REST API 为 RESTful，可通过 Azure Resource Manager API 访问。 在本文档中，你会看到通过 [ARMClient](https://github.com/projectkudu/ARMClient)（可简化 Azure Resource Manager API 调用的开源命令行工具）访问 API 的示例。 ARMClient 和 PowerShell 的使用是访问 Log Analytics 搜索 API 的许多选项之一。 另一个选项是使用 Operational Insights 的 Azure PowerShell 模块，其中包含了用于访问搜索的 cmdlet。 使用这些工具可以利用 RESTful Azure Resource Manager API 对 OMS 工作区进行调用并执行它们所包含的搜索命令。 API 将以 JSON 格式输出搜索结果，从而允许你以编程方式使用许多不同的方法来搜索结果。

可以通过 [.NET 的库](https://msdn.microsoft.com/library/azure/dn910477.aspx)以及 [REST API](https://msdn.microsoft.com/library/azure/mt163658.aspx) 访问 Azure Resource Manager。 查看链接的网页，了解详细信息。

## <a name="basic-log-analytics-search-rest-api-tutorial"></a>Log Analytics 搜索 REST API 基本教程
### <a name="to-use-the-arm-client"></a>使用 ARM 客户端
1. 安装 [Chocolatey](https://chocolatey.org/)，它是适用于 Windows 的开源软件包管理器。 以管理员身份打开命令提示符窗口，然后运行以下命令：
   
    ```
    @powershell -NoProfile -ExecutionPolicy unrestricted -Command "iex ((new-object net.webclient).DownloadString('https://chocolatey.org/install.ps1'))" && SET PATH=%PATH%;%ALLUSERSPROFILE%\chocolatey\bin
    ```
2. 通过运行以下命令来安装 ARMClient：
   
    ```
    choco install armclient
    ```

### <a name="to-perform-a-simple-search-using-the-armclient"></a>使用 ARMClient 执行简单搜索
1. 登录到你的 Microsoft 帐户或 OrgID 帐户：
...
> [!NOTE]
> 当搜索返回“Pending”状态时，则可以通过此 API 轮询已更新的结果。 6 分钟后，将从缓存中清除搜索结果，并返回 HTTP Gone。 当初始搜索请求立即返回“Successful”状态时，不会将它添加到缓存（导致此 API 在查询时返回 HTTP Gone）。 HTTP 200 结果的内容将采用与仅含有已更新值的初始搜索请求相同的格式。
> 
> 

### <a name="saved-searches---rest-only"></a>保存的搜索 - 仅限 REST
**请求已保存搜索的列表：**
...
| properties.displayName |**必需**。 用户定义的查询的显示名称。 当建模为 Azure 资源时，这将是一个标记。 |
| properties.category |**必需**。 用户定义的查询的类别。 当建模为 Azure 资源时，这将是一个标记。 |

> [!NOTE]
> 当针对工作区中保存的搜索进行轮询时，Log Analytics 搜索 API 当前返回用户创建的已保存搜索。 此时，该 API 不会返回解决方案提供的已保存搜索。 今后将会添加该功能。
> 
> 
...
下面介绍的方法适用于查看在工作区中收集的数据的所有日志类型的字段。 例如，如果想要知道事件类型中是否存在名为“Computer”的字段，可以使用该方法进行查找并确认。
...
结合使用 Put 方法和已保存搜索唯一 ID 新建计算机组。 使用现有的计算机组 ID 时，将修改该 ID。 在 OMS 控制台中创建计算机组时，将通过组和名称创建 ID。

用于组定义的查询必须返回一组计算机，才能使组正常工作。  建议在查询末尾添加 *| Distinct Computer*，以确保返回正确数据。
