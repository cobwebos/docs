...
在[搜索字段和方面参考部分](#search-field-and-facet-reference)，你可以了解在搜索中返回的字段，以及可以帮助你深入了解类似数据类别的方面。
...
系统中的每个数据段都具有 *TimeGenerated* 属性，用于表示记录的原始日期和时间。 此外，某些类型的数据还会有更多个日期/时间字段（例如，*LastModified*）。

Log Analytics 中的时间线图表/时间选择器显示一段时间内基于 *TimeGenerated* 字段的结果分布（根据正在运行的当前查询）。 日期/时间字段具有特定的字符串格式，可在查询中用于将查询限制为特定的时间范围。 你也可以使用语法来指代相对时间间隔（例如，“between 3 days ago and 2 hours ago”）。
...
前一个命令只返回 *TimeGenerated* 的值正好为“12:20 on October 1, 2013”的记录。 这也可能不会返回任何结果，不过，你明白其作用就可以了。

现在，分析器还支持助记日期/时间值。

同样，这可能也不会产生任何结果，因为数据不能如此快的通过系统传递。

这些示例都是用于相对和绝对日期的基本构件。 在后面的三小节中，我们将将以使用相对日期范围为例讲解如何在更高级的筛选器中使用这些字段。
...
| 运算符 | 说明 |
| --- | --- |
| / |将日期/时间取整为指定的单位。 示例：NOW/DAY 将当前日期/时间取整为当天的午夜。 |
| + 或 - |按指定的单位数偏移日期/时间。 示例：NOW+1HOUR 将当前日期/时间向前偏移一小时。2013-10-01T12:00-10DAYS 将日期值向后偏移 10 天。 |

你可以将日期/时间数学运算符链在一起，例如：
...
### <a name="field-facets"></a>字段方面
通过使用字段方面，你可以指定特定字段的搜索条件及其确切的值，而不是为整个索引中的各个术语编写“自定义文本”查询。 在前面几段，我们已在几个示例中使用过此语法。 此处，我们将介绍更复杂的示例。
...
可以使用正则表达式关键字，针对具有正则表达式的字段指定搜索条件。  在[使用正则表达式筛选 Log Analytics 中的日志搜索](log-analytics-log-searches-regex.md)中获取可在正则表达式中使用的语法的完整说明。
...
按特定字段对结果进行排序。 asc/desc 前缀是可选的。 如果省略，则采用 *asc* 排列顺序。 如果查询不显式使用*排序*命令，则对 **TimeGenerated** 进行降序排序是默认行为，并且将始终会先返回最新结果。
...
| 度量统计函数 | 说明 |
| --- | --- |
| *aggregateFunction* |聚合函数的名称（不区分大小写）。 支持以下聚合函数：COUNT MAX MIN SUM AVG STDDEV COUNTDISTINCT PERCENTILE## 或 PCT##（## 是介于 1 到 99 之间的任何数字） |
| *aggregatedField* |待聚合的字段。 此字段对于 COUNT 聚合函数是可选的，但必须是现有数字字段 SUM、MAX、MIN、AVG STDDEV 或 PERCENTILE## 或 PCT##（## 是介于 1 到 99 之间的任何数字）。 aggregatedField 可以是该扩展支持的任何函数。 |
| *fieldAlias* |用于已计算的聚合值（可选）别名。 如果未指定，该字段名称将为 AggregatedValue。 |
| *groupField* |结果集分组所依据的字段名称。 |
| *Interval* |时间间隔的格式：**nnnNAME**，其中 nnn 是正整数。 **NAME** 是间隔名称。 支持的间隔名称包括（区分大小写）：MILLISECOND[S] SECOND[S] MINUTE[S] HOUR[S] DAY[S] MONTH[S] YEAR[S] |
...
按照 Computer 对处理器时间百分比进行分组，并返回每隔 1 小时的平均值。
...
按计算机对处理器时间百分比进行分组，并返回每一个小时的最小值、平均值、第 75 百分位数和最大值。
...
依次按计算机和实例名称对处理器时间百分比进行分组，并返回每一个小时的最小值、平均值、第 75 百分位数和最大值
...
说明：此语法允许你创建聚合并将该聚合中的值列表提供给另一个将寻找具有这些值的事件的外部（主）搜索。 可以通过将内部搜索括在大括号内，并提供其结果作为使用 IN 运算符的外部搜索中字段的可能值来执行此操作。
...
**说明**：返回针对给定字段的每个唯一值找到的第一个文档。

**示例**

    Type=Event | Dedup EventID | sort TimeGenerated DESC

上面的示例针对每个 EventID 返回一个事件（自从我们在 TimeGenerated 上使用 DESC 起最新的一个）

### <a name="extend"></a>Extend
**说明**：允许在查询中创建运行时字段。 如果希望执行聚合，还可以在 Extend 后使用 Measure 命令。
...
| 和 |当且仅当其所有操作数的计算结果为 true，才返回值为 true。 |`and(not(exists(popularity)),exists(price))` |
...
| def |def 是默认值的简称。 返回字段“field”的值，或如果该字段不存在，则返回指定的默认值并生成第一个值，其中 `exists()==true`。 |`def(rating,5)` - 此 def() 函数返回分级，或如果文档中未指定分级，则返回 5 <br> `def(myfield, 1.0)` - 等效于 `if(exists(myfield),myfield,1.0)` |
| deg |将弧度转换为度 |`deg(x)` |
...
| if |启用条件函数查询。 在 `if(test,value1,value2)` 中，test 是或是指一个逻辑值或返回逻辑值（TRUE 或 FALSE）的表达式。  test 产生的值为 TRUE 时，`value1` 是函数返回的值。 test 产生的值为 FALSE 时，`value2` 是函数返回的值。 表达式可以是输出布尔值的任何函数，或者甚至是返回数字值的函数，值为 0 时将解释为 false 或字符串，或者值为空字符串时解释为 false。 |`if(termfreq(cat,'electronics'),popularity,42)` - 此函数检查每个文档以查看它是否在 cat 字段中包含术语“electronics”。 如果是，则返回 popularity 字段的值，否则返回的值为 42。 |
...
| strdist |计算两个字符串之间的距离。 使用 Lucene 拼写检查器 StringDistance 接口，并且支持该包中所有可用的实现，而且允许应用程序通过 Solr 的资源加载功能插入其自身。 strdist 采用 `(string1, string2, distance measure)`。 距离度量的可能值有： <br>jw：Jaro-Winkler<br>edit：Levenstein 或 Edit distance<br>ngram：NGramDistance（如果指定）也可以选择传入 ngram 大小。 默认值为 2。<br>FQN：用于实现 StringDistance 接口的完全限定的类名称。 必须具有无参数构造函数。 |`strdist("SOLR",id,edit)` |
...
| tan |返回角度的正切值 |`tan(x)` |
| tanh |返回角度的双曲正切值 |`tanh(x)` |

## <a name="search-field-and-facet-reference"></a>搜索字段和方面参考
使用日志搜索查找数据时，结果将显示各个字段和方面。 但是，你将看到的一些信息可能不会显得非常具体。 可以使用以下信息帮助你了解结果。

| 字段 | 搜索类型 | 说明 |
| --- | --- | --- |
| TenantId |全部 |用于对数据进行分区 |
| TimeGenerated |全部 |用于驱动时间线、时间选择器（在搜索和其他屏幕中）。 它表示生成数据段的时间（通常在代理上）。 该时间以 ISO 格式表示，并且始终为 UTC。 在“类型”基于现有检测（即日志中的事件）的情况下，这通常是记录日志条目/行/记录的实际时间；对于通过管理包或在云中生成的某些其他类型（即建议/警报/更新代理等），这是收集此新数据段（具有某种配置的快照）的时间，或在其基础上生成建议/警报的时间。 |
| EventID |事件 |Windows 事件日志中的 EventID |
| EventLog |事件 |由 Windows 记录的事件的事件日志 |
| EventLevelName |事件 |严重/警告/信息/成功 |
| EventLevel |事件 |严重/警告/信息/成功的数值（改用 EventLevelName 进行更简单/更具可读性的查询） |
| SourceSystem |全部 |数据的来源（就服务的“连接”模式而言，即 Operations Manager、OMS（数据在云中生成）、Azure 存储（来自 WAD 的数据）等） |
| ObjectName |PerfHourly |Windows 性能对象名称 |
| InstanceName |PerfHourly |Windows 性能计数器实例名称 |
| CounteName |PerfHourly |Windows 性能计数器名称 |
| ObjectDisplayName |PerfHourly、ConfigurationAlert、ConfigurationObject、ConfigurationObjectProperty |在 Operations Manager 中按性能收集规则确定的对象显示名称，或由操作见解发现的对象显示名称，或针对其生成警报的对象显示名称 |
| RootObjectName |PerfHourly、ConfigurationAlert、ConfigurationObject、ConfigurationObjectProperty |在 Operations Manager 中按性能收集规则确定的父级对象的父级（在双重托管关系中，即 Windows 计算机托管 SqlInstance，SqlInstance 托管 SqlDatabase）显示名称，或由操作见解发现的对象显示名称，或针对其生成警报的对象显示名称 |
| 计算机 |大多数类型 |数据所属的计算机名称 |
| DeviceName |ProtectionStatus |数据所属的计算机名称（与“Computer”相同） |
| DetectionId |ProtectionStatus | |
| ThreatStatusRank |ProtectionStatus |威胁状态级别是威胁状态的数字表示形式，与 HTTP 响应代码类似，我们已经在数字之间留出了间隙（这就是为什么没有威胁是 150，也不是 100 或 0），以便我们获得一些空间来添加新的状态。 当我们对威胁状态和保护状态进行汇总时，我们想要显示在所选的时间段内计算机所处于的最差状态。 我们可以使用数字设置不同状态的级别，以便我们可以查找具有最大数字的记录。 |
...
| ManagementGroupName |大多数类型 |Operations Manager 连接的代理的管理组名称；否则将为 null/空 |
...
| PublishDate |RequiredUpdate |更新是何时在 Microsoft 更新上发布的？ |
| 服务器 |RequiredUpdate |数据所属的计算机名称（与“Computer”相同） |
| 产品 |RequiredUpdate |更新应用于的产品 |
| UpdateClassification |RequiredUpdate |更新的类型（更新汇总、Service Pack 等） |
| KBID |RequiredUpdate |介绍此最佳实践或更新的知识库文章 ID |
...
| DaysSinceLastUpdate |UpdateAgent |此代理在多少天前（相对于此记录的“TimeGenerated”）安装的来自 WSUS/Microsoft 更新的任何更新？ |
...
| KBLink |建议 |描述此最佳实践或更新的知识库文章的 URL |
| RecommendationStatus |建议 |建议将少数类型的记录“更新”，而不仅仅是添加到搜索索引。 此状态将更改建议是活动/打开还是 Log Analytics 检测到已得到解决。 |
...
| Percentile95 |PerfHourly |性能计数器每小时聚合的每小时间隔的第 95 个百分位数 |
...
| csMethod |W3CIISLog |HTTP 请求中的客户端所使用的 HTTP 方法（GET/POST 等）。 原始日志中的 cs-method 字段 |
| cIP |W3CIISLog |HTTP 请求来自的客户端 IP 地址。 原始日志中的 c-ip 字段 |
| csUserAgent |W3CIISLog |由客户端（浏览器或其他）声明的 HTTP 用户代理。 原始日志中的 cs-user-agent |
| scStatus |W3CIISLog |由服务器返回到客户端的 HTTP 状态代码（200/403/500 等）。 原始日志中的 cs-status |
...
| csVersion |W3CIISLog |请求中使用的 HTTP 协议版本（即“HTTP/1.1”） |
| csCookie |W3CIISLog |Cookie 信息 |
| csReferer |W3CIISLog |用户上次访问的站点。 此站点提供指向当前站点的链接。 |
| csHost |W3CIISLog |所请求的主机头（即“www.mysite.com”） |
...
| ConfigChangeType |ConfigurationChange |更改的类型（WindowsServices/软件等） |
