...
你可以将 System Center Configuration Manager 连接到 Log Analytics 以同步收集数据。 这就使得来自配置管理器部署的数据可在 OMS 中使用。
...
6. 在 OMS 中，以计算机组的形式[从配置管理器导入收集的数据](#import-collections)。
7. 在 OMS 中，以[计算机组](log-analytics-computer-groups.md)的形式查看配置管理器中的数据。
...
下列过程将介绍如何为 Azure 管理门户提供访问 OMS 的权限。 具体而言，你必须对资源组中的用户授予*参与者角色*。 继而，才可以使 Azure 管理门户将配置管理器连接到 OMS。

> [!NOTE]
> 必须为配置管理器指定到 OMS 的权限。 否则，在配置管理器中使用配置向导时会收到一条错误消息。
>
>

1. 打开 [Azure 门户](https://portal.azure.com/)，然后单击“**浏览**” > “**Log Analytics (OMS)**”，打开 Log Analytics (OMS) 边栏选项卡。  
2. 在“**Log Analytics (OMS)**”边栏选项卡上，单击“**添加**”，打开“**OMS 工作区**”边栏选项卡。  
   ![OMS 边栏选项卡](./media/log-analytics-sccm/sccm-azure01.png)
3. 在“**OMS 工作区**”边栏选项卡上，提供以下信息，然后单击“**确定**”。
   ...
   * **定价层**  
     ...
     > [!NOTE]
     > 以上示例将创建一个新的资源组。 该资源组仅用于为配置管理器提供到在此示例中的 OMS 工作区的权限。
     >
     >
4. 单击“**浏览**” > “**资源组**”，打开“**资源组**”边栏选项卡。
5. 在“**资源组**”边栏选项卡中，单击上面创建的资源组，打开“&lt;资源组名称&gt;”设置边栏选项卡。  
...
8. 在“**添加访问**”边栏选项卡中，单击“**选择角色**”，然后选择“**参与者**”角色。  
   ![选择角色](./media/log-analytics-sccm/sccm-azure05.png)  
9. 单击“**添加用户**”，选择“配置管理器用户”，单击“**选择**”，然后单击“**确定**”。  
...
1. 在配置管理器的“**管理**”工作区中，选择“**OMS 连接器**”。 这将打开**添加 OMS 连接向导**。 选择“**下一步**”。
2. 在“**常规**”屏幕上，确认已完成以下操作，并且具有每个项的详细信息，然后选择“**下一步**”。
   ...
   2. 在 Azure 管理门户中，你已在 Azure Active Directory 中为已注册的应用创建了应用密钥。  
   3. 在 Azure 管理门户中，你已为注册的 Web 应用提供访问 OMS 的权限。  
      ![连接到 OMS 向导常规页](./media/log-analytics-sccm/sccm-console-general01.png)
3. 在“**Azure Active Directory**”屏幕上，通过提供“**租户**”、“**客户端 ID**”，以及“**客户端密钥**”来配置到 OMS 的连接设置，然后选择“**下一步**”。  
   ![连接到 OMS 向导 Azure Active Directory 页](./media/log-analytics-sccm/sccm-wizard-tenant-filled03.png)
4. 如果已成功完成所有其他过程，则“**OMS 连接配置**”屏幕上的信息将自动出现在此页中。 其中应显示用于“**Azure 订阅**”、“**Azure 资源组**”，以及“**Operations Management Suite 工作区**”的连接设置信息。  
   ![连接到 OMS 向导 OMS 连接页](./media/log-analytics-sccm/sccm-wizard-configure04.png)
5. 此向导将使用已输入的信息连接到 OMS 服务。 选择要与 OMS 同步的设备集合，然后单击“**添加**”。  
   ![选择集合](./media/log-analytics-sccm/sccm-wizard-add-collections05.png)
6. 在“**摘要**”屏幕验证连接设置，然后选择“**下一步**”。 “**进度**屏幕”将显示连接状态，最终应为**完成**。
...
1. 在配置管理器中，导航到“**云服务**”，然后选择“**OMS 连接器**”，打开“**OMS 连接属性**”页。
2. 在此页中，单击“**Azure Active Directory**”选项卡，查看你的“**租户**”、“**客户端 ID**”、“**客户端密钥过期**”。 如果“**客户端密钥**”已过期，则对其进行“**验证**”。
...
将 OMS 连接添加到配置管理器并在运行配置管理器服务连接点站点系统角色的计算机上安装代理之后，下一步是从 OMS 的配置管理器中以计算机组的形式导入集合。

启用导入后，每隔 3 小时检索一次集合成员身份信息，以保持最新的集合成员身份。 你可以随时选择禁用导入。

1. 在 OMS 门户中，单击“**设置**”。
2. 单击“**计算机组**”选项卡，然后单击“**SCCM**”选项卡。
3. 选择“**导入 Configuration Manager 集合成员身份**”，然后单击“**保存**”。  
...
将 OMS 连接添加到配置管理器并在运行配置管理器服务连接点站点系统角色的计算机上安装了代理之后，来自代理的数据将发送到 OMS。 在 OMS 中，配置管理器集合以[计算机组](log-analytics-computer-groups.md)的形式显示。 可以从“**设置**”中“**计算机组**”下方的“**配置管理器**”页查看这些组。

在导入集合后，可以看到已检测到的具有集合成员身份的计算机数。 此外还可以看到已导入的集合数。

![计算机组 - SCCM 选项卡](./media/log-analytics-sccm/sccm-computer-groups02.png)

单击上面任意一项，都将会打开“搜索”，相应显示所有已导入的组，或者属于每个组的所有计算机。 使用[日志搜索](log-analytics-log-searches.md)，可以针对配置管理器数据开始深度分析。
