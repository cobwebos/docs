...
ms.locfileid: "32178664"
---
# <a name="understanding-alerts-in-log-analytics"></a>了解 Log Analytics 中的警报

> [!NOTE]
> Log Analytics 中的警报将[扩展到 Azure 中](../monitoring-and-diagnostics/monitoring-alerts-extend.md)。 仍然可以根据本文中的信息，在 Azure 门户中定义那些使用 Log Analytics 搜索的警报的详细信息。

Log Analytics 中的警报标识 Log Analytics 存储库中的重要信息。 本文将讨论必须根据查询数据的收集频率、可能由网络延迟或处理能力造成的数据引入随机延迟以及将数据提交到 Log Analytics 工作区而进行的设计决策。 本文还详细说明了 Log Analytics 中警报规则的作用原理，并介绍了不同类型警报规则的差异。
...
- 收集频率决定了 OMS 代理将数据发送到 Log Analytics 的频率。 例如，如果收集频率为 10 分钟一次，且系统中没有其他延迟，则在添加到存储库并可由 Log Analytics 搜索之前，传输数据的时间戳可能是 0 到 10 分钟之间的任意值。
...
- **频率**。  指定查询应运行的频率和可用于提高正常情况下警报响应速度的频率。 该值可介于 5 分钟到 24 小时之间，且应小于或等于警报时间窗口。  如果该值大于时间范围，则会有记录缺失的风险。<br>如果目标对于 30 分钟及以下的延迟可靠，而正常延迟为 10 分钟，则应将时间窗口设置为 1 小时，将频率值设置为 10 分钟。 对于有 10 分钟引入延迟的数据，这将在警报数据生成后 10 到 20 分钟触发警报。<br>为避免由于时间窗口过宽而对相同数据创建多个警报，可使用“阻止警报”选项至少在时间窗口内阻止警报。
...
| Type |*Alert* |
| SourceSystem |*OMS* |
| *Object*  | [“指标度量”警报](#metric-measurement-alert-rules)有一个针对组字段的属性。 例如，如果日志搜索按“计算机”进行组合，则警报记录会有一个“计算机”字段，其值为计算机的名称。
| AlertName |警报的名称。 |
