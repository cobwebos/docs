...
description: "几分钟内即可在 Microsoft Operations Management Suite (OMS) 中启动并运行 Log Analytics。"
...
可以通过 OMS 网站创建免费 OMS 工作区。 还可使用 Microsoft Azure 订阅创建 OMS 工作区。 两种工作区在功能上等效，但免费 OMS 工作区每天只能向 OMS 服务发送 500 MB 的数据。 如果使用 Azure 订阅，还可以使用该订阅来访问其他 Azure 服务。 无论使用哪种方法创建工作区，都通过 Microsoft 帐户或组织帐户创建工作区。
...
* 需要付费的 Microsoft Azure 订阅才能使用完整的 Log Analytics。 如果你还没有 Azure 订阅，可以先创建一个 [免费帐户](https://azure.microsoft.com/free/) ，这样就可以访问任何 Azure 服务。 也可在 [Operations Management Suite](http://microsoft.com/oms) 网站创建免费 OMS 帐户，然后单击“免费试用”。
* OMS 工作区
* 要从中收集数据的每台 Windows 计算机都必须运行 Windows Server 2008 SP1 或更高版本
* [防火墙](log-analytics-proxy-firewall.md)可访问 OMS Web 服务的地址
* [OMS Log Analytics 转发器](https://blogs.technet.microsoft.com/msoms/2016/03/17/oms-log-analytics-forwarder)（网关）服务器，当计算机无法访问 Internet 时，它将通信从服务器转发到 OMS
...
* 确定计算机可直接访问 Internet。 如果不能，则需通过网关服务器访问 OMS Web 服务站点。 所有访问都通过 HTTPS 进行。
...
## <a name="sign-up-in-3-steps-using-the-operations-management-suite"></a>使用 Operations Management Suite 分 3 步进行注册
1. 转到 [Operations Management Suite](http://microsoft.com/oms) 网站，然后单击“免费试用”。 使用 Microsoft 帐户（如 Outlook.com）或公司或教育机构提供的组织帐户登录，以使用 Office 365 或其他 Microsoft 服务。
...
3. 接下来，可以创建新 Azure 订阅，或链接到现有 Azure 订阅。 若要继续使用免费试用版，请单击“稍后再说”。  
...
有关通过 Operations Management Suite 设置工作区并将现有 Azure 帐户链接到所创建的工作区的详细信息，请参阅[管理对 Log Analytics 的访问](log-analytics-manage-access.md)。

## <a name="sign-up-quickly-using-microsoft-azure"></a>使用 Microsoft Azure 快速注册
1. 转到 [Azure 门户](https://portal.azure.com)并登录，浏览服务列表，然后选择“Log Analytics (OMS)”。  
...
3. 单击“创建”即可在 Azure 门户中看到工作区详细信息。       
    ![工作区详细信息](./media/log-analytics-get-started/oms-onboard-workspace-details.png)         
4. 单击“OMS 门户”链接，打开包含新工作区的 Operations Management Suite 网站。
...
## <a name="optionally-connect-servers-directly-to-the-operations-management-suite-by-installing-an-agent"></a>通过安装代理，直接服务器连接到 Operations Management Suite（可选）
