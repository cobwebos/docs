...
| SrcIP_s | 源 IP 地址 | Will be blank in case of AzurePublic and ExternalPublic flows |
| DestIP_s | 目标 IP 地址 | Will be blank in case of AzurePublic and ExternalPublic flows |
| VMIP_s | IP of the VM | Used for AzurePublic and ExternalPublic flows |
| PublicIP_s | 公共 IP 地址 | Used for AzurePublic and ExternalPublic flows |
...
| MACAddress_s | MAC 地址 | MAC address of the NIC at which the flow was captured |
...
| Subscription1_s | 订阅 ID | Subscription ID of virtual network/ network interface/ virtual machine to which the source IP in the flow belongs to |
| Subscription2_s | 订阅 ID | Subscription ID of virtual network/ network interface/ virtual machine to which the destination IP in the flow belongs to |
...
| Region1_s | Azure 区域 | Azure region of virtual network/ network interface/ virtual machine to which the source IP in the flow belongs to |
| Region2_s | Azure 区域 | Azure region of virtual network to which the destination IP in the flow belongs to |
...
| FlowCount_d | 已弃用。 Total flows that matched the same four-tuple. In case of flow types ExternalPublic and AzurePublic, count will include the flows from various PublicIP addresses as well.
