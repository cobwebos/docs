...
1. 位于 "FlowIntervalStartTime_t" 和 "FlowIntervalEndTime_t" 之间的所有流日志在存储帐户中以一分钟的时间间隔捕获为 blob，然后再由流量分析处理。
...
| SubType_s | FlowLog | 流日志的子类型。 仅使用 "FlowLog"，SubType_s 的其他值用于产品的内部工作原理 |
...
| FlowStartTime_t | UTC 格式的日期和时间 |  流的第一次出现（将进行聚合）在 "FlowIntervalStartTime_t" 和 "FlowIntervalEndTime_t" 之间的流日志处理间隔中。 此流基于聚合逻辑进行聚合 |
| FlowEndTime_t | UTC 格式的日期和时间 | 在 "FlowIntervalStartTime_t" 和 "FlowIntervalEndTime_t" 之间的流日志处理间隔中，最后一次出现的流（将被聚合）。 就流日志 v2 而言，此字段包含开始具有相同的四元组的最后一个流（在原始流记录中标记为 "B"）的时间。 |
...
| NSGRules_s | \<索引值0） >\|\<NSG_RULENAME >\|\<\|\<> Flow 状态\|\<> FlowCount ProcessedByRule |  允许或拒绝此流的 NSG 规则 |
...
| VM_s | < resourcegroup_Name >\/\<NetworkInterfaceName > | 与网络接口关联的虚拟机 NIC_s |
...
| ConnectionName_s | \<SubscriptionID >/\<ResourceGroupName >/\<ConnectionName > | 连接名称。 对于 flowtype P2S，这将被格式化为 <gateway name>_<VPN Client IP> |
...
| DestPublicIPs_s | \<DESTINATION_PUBLIC_IP >\|FLOW_STARTED_COUNT >\|\<FLOW_ENDED_COUNT >\|\<OUTBOUND_PACKETS >\|\<INBOUND_PACKETS >\|\<OUTBOUND_BYTES >\|\<INBOUND_BYTES > < | 按栏分隔的条目 |

### <a name="notes"></a>说明

1. 对于 AzurePublic 和 ExternalPublic 流，客户拥有的 Azure VM IP 将在 VMIP_s 字段中填充，同时在 PublicIPs_s 字段中填充公共 IP 地址。 对于这两种流类型，应使用 VMIP_s 和 PublicIPs_s，而不是 SrcIP_s 和 DestIP_s 字段。 对于 AzurePublic 和 ExternalPublicIP 地址，我们将进一步聚合，以便将引入的记录数量降至客户 log analytics 工作区。（此字段即将弃用，应根据 azure VM 是流中的源还是目标来使用 SrcIP_ 和 DestIP_s）
...
1. 某些字段名称追加 \_s 或 \_d 中。 这并不表示源和目标，但分别表示数据类型字符串和十进制。
