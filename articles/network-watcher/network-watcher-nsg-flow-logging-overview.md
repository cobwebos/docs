...
- 自我管理密钥轮换：如果您更改/旋转存储帐户的访问密钥，NSG 流日志将停止工作。 要解决此问题，必须禁用并重新启用 NSG 流日志。

**在附加到资源的所有 NSG 上启用 NSG 流日志记录**：Azure 中的流日志记录在 NSG 资源上配置。 一个流只与一个 NSG 规则相关联。 如果利用多个 NSG，则我们建议在应用资源子网或网络接口的所有 NSG 上启用 NSG 流日志记录，以确保记录所有流量。 有关详细信息，请参阅如何在网络安全组中[评估流量](../virtual-network/security-overview.md#how-traffic-is-evaluated)。

**流日志记录成本**：NSG 流日志记录根据生成的日志量计费。 流量较高时，流日志的量和相关成本可能会增大。 NSG 流日志定价不包括基本的存储成本。 将保留策略功能与 NSG 流日志记录一起使用意味着在较长时间内产生单独的存储成本。 如果不需要使用保留策略功能，我们建议将此值设置为 0。 有关详细信息，请参阅[网络观察程序定价](https://azure.microsoft.com/pricing/details/network-watcher/)和[Azure 存储定价](https://azure.microsoft.com/pricing/details/storage/)以获取其他详细信息。

**从 Internet IP 记录到没有公共 IP 的 VM 的入站流**：没有通过与 NIC 关联的公共 IP 地址作为实例级公共 IP 或作为基本负载均衡器后端池的一部分、使用[默认 SNAT](../load-balancer/load-balancer-outbound-connections.md#defaultsnat)并由 Azure 分配 IP 地址以方便出站连接的公共 IP 地址的 VM。 因此，如果流被发送到分配给 SNAT 的端口范围内的端口，您可能会看到来自 Internet IP 地址流的流日志条目。 虽然 Azure 不允许这些流流向 VM，但尝试将记录并按设计显示在网络观察程序的 NSG 流日志中。 我们建议使用 NSG 显式阻止不需要的入站互联网流量。

**无状态流的字节和数据包计数不正确**：[网络安全组 （NSG）](https://docs.microsoft.com/azure/virtual-network/security-overview)作为[状态防火墙](https://en.wikipedia.org/wiki/Stateful_firewall?oldformat=true)实现。 但是，许多控制流量的默认/内部规则都是以无状态方式实现的。 由于平台限制，不记录无状态流的字节和数据包计数（即，通过无状态规则的流量流），它们仅记录状态流。 因此，NSG 流日志（和流量分析）中报告的字节数和数据包数可能不同于实际流。 这一限制定于 2020 年 6 月修复。
