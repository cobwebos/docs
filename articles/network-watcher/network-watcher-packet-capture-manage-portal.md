...
如果某个网络安全组已关联到网络接口或该网络接口所在的子网，请确保存在允许上述端口的规则。 
...
3. 选择“添加”以创建数据包捕获。 可以选择以下属性的值：
   - **订阅**：虚拟机，你想要创建数据包捕获，以用于该订阅是在中。
   - **资源组**：虚拟机的资源组。
   - **目标虚拟机**：你想要创建的数据包捕获虚拟机。
   - **数据包捕获名称**:数据包捕获名称。
   - **存储帐户或文件**:选择**存储帐户**，**文件**，和 / 或。 如果选择“文件”，则捕获将写入虚拟机中的某个路径。
   - **本地文件路径**:在其中保存数据包捕获的虚拟机上的本地路径 (仅当*文件*处于选中状态)。 该路径必须是有效路径。 如果使用 Linux 虚拟机，则路径必须以 */var/captures* 开头。
   - **存储帐户**：选择一个现有的存储帐户，如果所选*存储帐户*。 仅当选择了“存储”时，此选项才可用。
   ...
   - **时间限制 （秒）**:停止数据包捕获之前的时间限制。 默认值为 18,000 秒。
   - 筛选（可选） 选择“+ 添加筛选器”
     - **协议**：要筛选的数据包捕获协议。 可用值为 TCP、UDP 和 Any。
     - **本地 IP 地址**:将数据包捕获筛选的数据包的本地 IP 地址与此值的匹配。
     - **本地端口**:将数据包捕获筛选的数据包的本地端口与此值的匹配。
     - **远程 IP 地址**:将数据包捕获筛选的数据包的远程 IP 地址与此值的匹配。
     - **远程端口**:将数据包捕获筛选的数据包的远程端口与此值的匹配。
