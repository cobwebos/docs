---
title: "使用 Azure 网络观察程序管理数据包捕获 - REST API | Microsoft 文档"
...
使用网络观察程序数据包捕获功能，可以创建捕获会话以跟踪进出虚拟机的流量。 我们为捕获会话提供了筛选器，确保仅捕获所需的流量。 数据包捕获有助于以主动和被动方式诊断网络异常。 其他用途包括收集网络统计信息、获取网络入侵信息、调试客户端与服务器之间的通信，等等。 由于能够远程触发数据包捕获，此功能可减轻在所需计算机上手动运行数据包捕获的负担，从而节省宝贵的时间。
...
本方案通过调用网络观察程序 Rest API 来运行“IP 流验证”。 通过 PowerShell 调用 REST API 时，使用的是 ARMclient。 可以按照 [ARMClient on Chocolatey](https://chocolatey.org/packages/ARMClient)（Chocolatey 上的 ARMClient）中的说明，在 chocolatey 上找到 ARMClient
