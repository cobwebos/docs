---
title: 针对 Azure Cache for Redis 的最佳做法
...
ms.locfileid: "80478606"
---
# <a name="best-practices-for-azure-cache-for-redis"></a>针对 Azure Cache for Redis 的最佳做法 
 ...
 * **请记住，Redis 是内存中的数据存储。**  [此文](cache-troubleshoot-data-loss.md)概述了可能发生数据丢失的一些情况。

 * **开发您的系统，以便它可以处理连接波动**[，因为修补和故障转移](cache-failover.md)。
 ...
 * **Redis 最适合较小的值**，因此请考虑将较大的数据切入多个键。  [此 Redis 介绍文章](https://stackoverflow.com/questions/55517224/what-is-the-ideal-value-size-range-for-redis-is-100kb-too-large/)中列出了一些应该仔细考虑的因素。  阅读[此文章](cache-troubleshoot-client.md#large-request-or-response-size)以了解较大值可能引起的问题示例。
 ...
 * [Asp.Net会话状态提供程序](https://gist.github.com/JonCole/925630df72be1351b21440625ff2671f#file-redis-bestpractices-session-state-provider-md)
 ...
 * **首先使用 `redis-benchmark.exe`** 以在编写自己的性能测试之前感受可能的吞吐量/延迟。  [可在此处找到](https://redis.io/topics/benchmarks) Redis 基准文档。  请注意，redis-基准测试不支持 TLS，因此在运行测试之前，您必须[通过门户启用非 TLS 端口](cache-configure.md#access-ports)。  [可在此处找到 Windows 兼容版本的 redis-benchmark.exe](https://github.com/MSOpenTech/redis/releases)
 * 用于测试的客户端 VM 应与 Redis 缓存实例**位于同一区域**。
 * **我们建议对客户端使用 Dv2 VM 系列**，因为它们具有更好的硬件，并且会提供最佳结果。
 * 确保所用客户端 VM 的计算和带宽资源 *至少与要测试的缓存相同。** 
 * 如果您在 Windows 上，则在客户端计算机上**启用 VRSS。**  [请参阅此处了解详细信息](https://technet.microsoft.com/library/dn383582(v=ws.11).aspx)。  PowerShell 脚本示例：
...
**预测试设置**：使用下面列出的延迟和吞吐量测试命令所需的数据准备缓存实例。
> redis-基准.exe-h yourcache.redis.cache.windows.net -您的 Accesskey -t SET -n 10-d 1024 

**测试延迟**：使用 1k 有效负载测试 GET 请求。
> redis-基准.exe-h yourcache.redis.cache.windows.net - 您的 Accesskey -t GET -d 1024 -P 50 -c 4

**要测试吞吐量：** 具有 1k 有效负载的管道 GET 请求。
> redis-基准.exe-h yourcache.redis.cache.windows.net - 您的 Accesskey -t GET -n 1000000-d 1024 -P 50 -c 50
