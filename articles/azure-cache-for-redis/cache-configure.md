...
**最大内存预留**设置配置为非缓存操作（如故障转移期间复制）保留的内存量（以 MB 表示）。 设置此值能够在负载变化时具有更一致的 Redis 服务器体验。 对于写入密集型工作负荷，应将此值设置为较高。 为此类操作保留内存后，将无法存储缓存数据。
...
每个定价层都有不同的客户端连接、内存和带宽的限制。 如果缓存持续一段时间接近这些度量值的最大容量，即会提供建议。 有关 **"建议"** 工具查看的指标和限制的详细信息，请参阅下表：
...
要升级缓存，请单击"**立即升级**"以更改定价层并[缩放](#scale)缓存。 有关选择定价层的详细信息，请参阅[应使用哪种 Azure Redis 缓存套餐和大小？](cache-faq.md#what-azure-cache-for-redis-offering-and-size-should-i-use)
...
### <a name="locks"></a>锁
“锁定”**** 部分可用来锁定订阅、资源组或资源，以防止组织中的其他用户意外删除或修改关键资源。 有关详细信息，请参阅使用[Azure 资源管理器锁定资源](../azure-resource-manager/management/lock-resources.md)。
