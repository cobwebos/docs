...
description: "使用这篇文章可帮助你规划、生成然后传输你自己的受 HSM 保护的密钥，以便与 Azure 密钥保管库一起使用。"
services: key-vault
documentationcenter: 
author: cabailey
manager: mbaldwin
...
使用以下命令启动 Azure PowerShell 会话，并登录你的 Azure 帐户：
        ...
        Add-AzureAccount
在弹出的浏览器窗口中，输入你的 Azure 帐户用户名和密码。 然后，使用 [Get-azuresubscription](https://msdn.microsoft.com/library/azure/dn790366.aspx) 命令：
...
确保 Thales 工具位于路径（**%nfast_home%\bin** 和 **%nfast_home%\python\bin**）下。 例如，请键入以下内容：
...
### <a name="step-32-validate-the-downloaded-package"></a>步骤 3.2：验证下载的程序包
...
### <a name="step-33-create-a-new-key"></a>步骤 3.3：新建密钥
...
运行此命令时，请将 *contosokey* 替换为你在[生成密钥](#step-3-generate-your-key)步骤的**步骤 3.3：新建密钥**中指定的相同值。

系统会要求你插入安全体系的管理员卡。

此命令完成后，你会看到 **Result: SUCCESS**，同时权限减少的密钥副本位于名为 key_xferacId_<contosokey> 的文件中。

### <a name="step-42-inspect-the-new-copy-of-the-key"></a>步骤 4.2：检查密钥的新副本
（可选）请运行 Thales 实用工具来确认新密钥的最低权限：
...
* kmfile-dump.exe:
  ...
  运行这些命令时，将 contosokey 替换为你在[生成密钥](#step-3-generate-your-key)步骤的**步骤 3.3：新建密钥**中指定的相同值。

### <a name="step-43-encrypt-your-key-by-using-microsofts-key-exchange-key"></a>步骤 4.3：使用 Microsoft 的密钥交换密钥加密密钥
...
* 将 *contosokey* 替换为你用于在[生成密钥](#step-3-generate-your-key)步骤的**步骤 3.3：新建密钥**中生成密钥的标识符。
...
此操作成功完成后，它会显示 **Result: SUCCESS**，并且具有以下名称的当前文件夹中出现一个新文件：TransferPackage-*ContosoFirstHSMkey*.byok

### <a name="step-44-copy-your-key-transfer-package-to-the-internet-connected-workstation"></a>步骤 4.4：将密钥传输包复制到连接 Internet 的工作站
...
在最后一步中，在连接 Internet 的工作站上，使用 [Add-AzureKeyVaultKey](https://msdn.microsoft.com/library/azure/dn868048\(v=azure.300\).aspx) cmdlet 上传你从断开连接的工作站中复制到 Azure 密钥保管库 HSM 的密钥传输包：
