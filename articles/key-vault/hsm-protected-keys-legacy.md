...
nCipher Security 是一家委托 Datacard 公司，是通用 HSM 市场的领导者，通过为其关键业务信息和应用程序提供信任、完整性和控制，为世界领先的组织提供支持。 nCipher 的加密解决方案保护新兴技术（云、物联网、区块链、数字支付），并帮助满足新的合规性要求，使用全球组织目前所依赖的相同成熟技术来防范其威胁敏感数据、网络通信和企业基础设施。 nCipher 为关键业务应用程序提供信任，确保数据的完整性，并始终让客户完全控制。

微软与 NCipher 安全公司合作，以增强 HSM 的最先进的状态。 这些增强功能可使你能够获得托管服务的典型优势，而且无需放弃对密钥的控制权。 具体来说，这些增强功能可以让 Microsoft 管理 HSM，这样你就不必费心管理了。 作为云服务，Azure 密钥保管库无需通知即可扩大，以满足组织的使用高峰需求。 同时，密钥也会在 Microsoft 的 HSM 内部获得保护：你生成密钥并将其传输到 Microsoft 的 HSM，所以可以保留对密钥生命周期的控制权。
...
如果使用 nCipher nShield 边缘，则更改模式：1。 使用“模式”按钮突出显示所需模式。 2. 几秒钟内，按住“清除”按钮几秒钟。 如果模式发生更改，新模式的 LED 指示灯将停止闪烁并保持亮起。 状态 LED 可能会不规则地闪烁几秒钟，并在设备就绪时规则闪烁。 否则，设备保持在当前模式，相应的模式 LED 亮起。
