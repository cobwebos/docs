...
---
# <a name="import-hsm-protected-keys-for-key-vault-legacy"></a>为 Key Vault 导入受 HSM 保护的密钥（旧版）
...
nCipher 安全是一家 Entrust Datacard 公司，是一般用途的 HSM 市场，可通过向其业务关键信息和应用程序提供信任、完整性和控制来实现全球领先的组织。 nCipher 的加密解决方案确保新兴技术安全（云、IoT、区块链、数字付款），并使用全球组织所依赖的同一经验证的技术来防范其敏感数据、网络通信和企业基础结构的威胁。 nCipher 提供对业务关键应用程序的信任，确保数据的完整性，并使客户始终处于完全控制状态-今天、明天。

Microsoft 与 nCipher 安全性合作，为 Hsm 提高了作品的状态。 这些增强功能可使你能够获得托管服务的典型优势，而且无需放弃对密钥的控制权。 具体来说，这些增强功能可以让 Microsoft 管理 HSM，这样你就不必费心管理了。 作为云服务，Azure Key Vault 在短期内扩展，以满足你组织的使用高峰。 同时，你的密钥在 Microsoft 的 Hsm 内受到保护：你可以通过生成密钥并将其传送到 Microsoft 的 Hsm 来保持对密钥生命周期的控制。
...
### <a name="step-21-prepare-the-disconnected-workstation-with-ncipher-nshield-hsm"></a>步骤2.1：通过 nCipher nShield HSM 准备断开连接的工作站

在 Windows 计算机上安装 nCipher 支持软件，然后将 nCipher nShield HSM 连接到该计算机。

确保 nCipher 工具在你的路径（**% nfast_home% \ bin**）中。 例如，请键入以下内容：
...
如果使用的是 nCipher nShield Edge，则更改模式：1。 使用“模式”按钮突出显示所需模式。 2. 几秒钟内，按住“清除”按钮几秒钟。 如果模式发生变化，则新模式的 LED 将停止闪烁并保持发亮。 状态 LED 可能会不规则地闪烁几秒钟，并在设备就绪时规则闪烁。 否则，设备保持在当前模式，相应的模式 LED 亮起。
...
此程序可在 %NFAST_KMDATA%\local\world（与 C:\ProgramData\nCipher\Key Management Data\local 文件夹对应）下创建一个**安全体系**文件。 你可以对仲裁使用不同的值，但在我们的示例中，系统会提示你输入三个空白卡和 pin。 然后，任何两个卡都会提供对安全体系的完全访问权限。 这些卡变成新安全体系的**管理员卡集**。

> [!NOTE]
> 如果 HSM 不支持较新的密码套件 DLf3072s256mRijndael，则可以将 --cipher-suite=DLf3072s256mRijndael 替换为 --cipher-suite=DLf1024s160mRijndael
> 
> 通过 nCipher 软件12.50 版附带的 new-world 创建的安全体系与此 BYOK 过程不兼容。 可以使用两个选项：
...
> [!NOTE]
> 若要验证下载的包，HSM 必须处于连接状态，并且必须有安全体系（例如刚刚创建的包）。
...
1. 根据所在的地理区域或 Azure 实例，键入以下项之一，以运行 verifykeypackage.py 脚本：
   ...
   * 对于瑞士：
     ...
     > [!TIP]
     > NCipher nShield 软件包括位于% NFAST_HOME% \ python\bin 的 python
     >
     >
...
命令完成后，会看到**Result： SUCCESS** ，并且具有降低的权限的密钥副本位于名为 key_xferacId_\<contosokey> 的文件中。
...
### <a name="step-42-encrypt-your-key-by-using-microsofts-key-exchange-key"></a>步骤4.2：使用 Microsoft 的密钥交换密钥加密密钥
