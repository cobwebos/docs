...
ms.locfileid: "81429728"
---
# <a name="import-hsm-protected-keys-for-key-vault-legacy"></a>导入密钥保管库（旧版）的受 HSM 保护的密钥
...
* 用于加密密钥的密钥交换密钥 (KEK) 是在 Azure 密钥保管库 HSM 内部生成的，且不可导出。 HSM 会强制使 HSM 外部没有清晰版本的 KEK。 此外，该工具集还包括来自 nCipher 的证明，证明 KEK 不可导出，并且生成于由 nCipher 制造的正版 HSM 中。
* 该工具集包括来自 nCipher 的证明，即 Azure 密钥保管库安全世界也生成在 nCipher 制造的正版 HSM 上。 此证明可证明 Microsoft 使用的是正版硬件。
...
nCipher Security 是一家委托 Datacard 公司，是通用 HSM 市场的领导者，通过为其关键业务信息和应用程序提供信任、完整性和控制，为世界领先的组织提供支持。 nCipher 的加密解决方案保护新兴技术（云、物联网、区块链、数字支付），并帮助满足新的合规性要求，使用全球组织目前所依赖的相同成熟技术来抵御其敏感数据、网络通信和企业基础架构的威胁。 nCipher 为关键业务应用程序提供信任，确保数据的完整性，并始终让客户完全控制。

微软与 NCipher 安全公司合作，以增强 HSM 的最先进的状态。 这些增强功能可使你能够获得托管服务的典型优势，而且无需放弃对密钥的控制权。 具体来说，这些增强功能可以让 Microsoft 管理 HSM，这样你就不必费心管理了。 作为云服务，Azure 密钥保管库会在短时间内扩展，以满足组织的使用高峰。 同时，密钥在 Microsoft 的 HSM 中受到保护：您保留对密钥生命周期的控制，因为您生成密钥并将其传输到 Microsoft 的 HSM。
...
### <a name="step-21-prepare-the-disconnected-workstation-with-ncipher-nshield-hsm"></a>步骤 2.1：使用 nCipher nShield HSM 准备断开连接的工作站

在 Windows 计算机上安装 nCipher 支持软件，然后将 nCipher nShield HSM 附加到该计算机。

确保 nCipher 工具位于您的路径中 **（%nfast_home %nfast_home%\bin**）。 例如，请键入以下内容：

  ```cmd
  set PATH=%PATH%;"%nfast_home%\bin"
  ```

有关详细信息，请参阅 nShield HSM 中包含的用户指南。
...
如果使用 nCipher nShield 边缘，则更改模式：1。 使用“模式”按钮突出显示所需模式。 2. 几秒钟内，按住“清除”按钮几秒钟。 如果模式发生更改，新模式的 LED 将停止闪烁并保持亮起。 状态 LED 可能会不规则地闪烁几秒钟，并在设备就绪时规则闪烁。 否则，设备保持在当前模式，相应的模式 LED 亮起。

### <a name="step-32-create-a-security-world"></a>步骤 3.2：创建安全体系

启动命令提示符并运行 nCipher 新世界程序。

   ```cmd
    new-world.exe --initialize --cipher-suite=DLf3072s256mRijndael --module=1 --acs-quorum=2/3
   ```

此程序可在 %NFAST_KMDATA%\local\world（与 C:\ProgramData\nCipher\Key Management Data\local 文件夹对应）下创建一个**安全体系**文件。 您可以为仲裁使用不同的值，但在我们的示例中，系统会提示您为每个值输入三张空白卡和引脚。 然后，任何两个卡都会提供对安全体系的完全访问权限。 这些卡变成新安全体系的**管理员卡集**。

> [!NOTE]
> 如果 HSM 不支持较新的密码套件 DLf3072s256mRijndael，则可以将 --cipher-suite=DLf3072s256mRijndael 替换为 --cipher-suite=DLf1024s160mRijndael
> 
> 使用 nCipher 软件版本 12.50 附带的 nCipher 软件版本创建的安全世界与此 BYOK 过程不兼容。 可以使用两个选项：
> 1) 将 nCipher 软件版本降级到 12.40.2，以创建一个新的安全世界。
> 2) 联系 nCipher 支持，并要求他们提供 12.50 软件版本的修补程序，允许您使用 12.40.2 版本的新世界.exe，该版本与本 BYOK 过程兼容。
...
* 工具集中中包含的密钥交换密钥是从正版 nCipher nShield HSM 生成的。
* 工具集中中包含的安全世界哈希已在正版 nCipher nShield HSM 中生成。
* 密钥交换密钥不可导出。

> [!NOTE]
> 要验证下载的包，HSM 必须连接、通电，并且必须有一个安全世界（例如您刚刚创建的包）。
...
1. 根据所在的地理区域或 Azure 实例，键入以下项之一，以运行 verifykeypackage.py 脚本：
   ...
   * 对于瑞士：
     ...
     > [!TIP]
     > nCipher nShield 软件包括在 %NFAST_HOME%\python_bin 的 python
     >
     >
2. 确认看到以下指示验证成功的消息：**Result: SUCCESS**

此脚本验证签名者链到 nShield 根键。 此根密钥的哈希嵌入到脚本中，其值应为 **59178a47 de508c3f 291277ee 184f46c4 f1d9c639**。 您也可以通过访问[nCipher 网站](https://www.ncipher.com/products/key-management/cloud-microsoft-azure/validation)单独确认此值。

现在，您可以创建新密钥。

### <a name="step-35-create-a-new-key"></a>步骤 3.5：新建密钥

使用 nCipher nShield**生成密钥程序生成密钥**。
...
> [!IMPORTANT]
> 稍后将自己的密钥传送到 Azure 密钥保管库时，Microsoft 就无法将此密钥导出返回给你，因此，请务必安全地备份密钥和安全体系。 请与[nCipher](https://www.ncipher.com/about-us/contact-us)联系，了解备份密钥的指南和最佳实践。
>
...
命令完成后，您将看到**结果：成功**，并且具有缩小权限的密钥副本位于名为 key_xferacId_\<contosokey>的文件中。

您可以使用 nCipher nShield 实用程序使用以下命令检查 ACLS：
...
### <a name="step-42-encrypt-your-key-by-using-microsofts-key-exchange-key"></a>步骤 4.2：使用微软密钥交换密钥加密密钥
