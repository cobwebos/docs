...
* 这是一个 RSA-HSM 密钥（4096位或3072位或2048位）
* 它将使用固定 key_ops （仅限 "import"），这只允许在 BYOK 期间使用它
* 必须位于将导入目标密钥的同一个保管库中
...
客户使用 HSM 供应商提供的 BYOK 工具和文档完成步骤3。 它会生成一个密钥传输 Blob （一个 "byok" 文件）。
...
客户将使用 HSM 供应商提供的 BYOK 工具来创建密钥传输 blob （作为 "BYOK" 文件存储）。 KEK 公钥（作为 pem 文件）将是此工具的一个输入。
...
传输 blob 的格式使用 JSON Web 加密紧凑序列化（RFC7516）作为将所需的元数据传递给服务进行正确解密的载体。
...
JSON blob 存储在扩展名为 "byok" 的文件中，以便在使用 "AzKeyVaultKey" （PSH）或 "az keyvault key import" （CLI）命令时，Azure PowerShell/CLI 客户端可以正确地将其处理。

### <a name="step-4-upload-key-transfer-blob-to-import-hsm-key"></a>步骤4：上传密钥传输 blob 以导入 HSM 密钥

客户将密钥传输 Blob （"byok" 文件）传输到联机工作站，然后运行**az keyvault key import**命令，将此 Blob 作为新的 HSM 支持的密钥导入 Key Vault 中。 
...
* [创建密钥](https://docs.microsoft.com/rest/api/keyvault/createkey/createkey)
* [Get key （仅密钥属性和公钥）](https://docs.microsoft.com/rest/api/keyvault/getkey/getkey)
* [导入密钥](https://docs.microsoft.com/rest/api/keyvault/importkey/importkey)
...
* [az keyvault key 下载](https://docs.microsoft.com/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-download)
* [az keyvault key import](https://docs.microsoft.com/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-import)


## <a name="next-steps"></a>后续步骤
* 分步 BYOK 说明：[将 HSM 保护的密钥导入 Key Vault （BYOK）](hsm-protected-keys-byok.md)
