...
- **托管服务标识**：虽然 Azure Key Vault 可用于安全存储凭据以及其他密钥和机密，但代码需要向密钥保管库证明身份才能检索它们。 托管服务标识为 Azure 服务提供了 Azure AD 中的自动托管标识，更巧妙地解决了这个问题。 可以使用此标识向支持 Azure AD 身份验证的密钥保管库或任何服务证明身份，而无需在代码中放入任何凭据。 有关详细信息，请参阅 [Azure 资源的托管服务标识](../active-directory/managed-service-identity/overview.md)。

    ![托管服务标识原理图](./media/key-vault-whatis/msi.png)
