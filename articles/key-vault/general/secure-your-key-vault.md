...
对于身份验证，这两个平面都使用 Azure Active Directory (Azure AD)。 对于授权，管理平面使用基于 Azure 角色的访问控制 (RBAC) ，而数据平面使用 Key Vault 访问策略和 Azure RBAC (预览) 。
...
- **仅限应用程序**：应用程序表示服务或后台作业。 此标识是需要定期从密钥保管库访问证书、密钥或机密的应用程序的最常见方案。 要使此方案正常运行， `objectId` 必须在访问策略中指定应用程序的，并且 `applicationId` _不_ 能指定或必须是 `null` 。
- **仅用户**：用户从租户中注册的任何应用程序访问密钥保管库。 此类访问的示例包括 Azure PowerShell 和 Azure 门户。 要使此方案正常工作， `objectId` 必须在访问策略中指定用户的，并且 `applicationId` _不_ 能指定或必须是 `null` 。
- **应用程序加用户** (有时称为 _复合标识_) ：用户需要从特定应用程序访问密钥保管库 _，并且_ 应用程序必须使用代表身份验证 (OBO) flow 才能模拟用户。 要使此方案正常工作， `applicationId` `objectId` 必须在访问策略中指定和。 `applicationId`标识所需的应用程序，并 `objectId` 标识该用户。 此选项当前不可用于数据平面 Azure RBAC (预览) 
...
应用程序通过终结点访问平面。 两个平面的访问控制独立运行。 若要授予应用程序访问权限以使用密钥保管库中的密钥，请使用 Key Vault 访问策略或 Azure RBAC (preview) 授予数据平面访问权限。 若要授予用户对密钥保管库属性和标记的读取访问权限，但不授予对数据（密钥、机密或证书）的访问权限，请使用 RBAC 授予管理平面访问权限。
...
在管理平面中，你将使用 Azure RBAC)  (Azure 基于角色的访问控制来授权调用方可以执行的操作。 在 Azure RBAC 模型中，每个 Azure 订阅都有一个 Azure AD 的实例。 可以从此目录向用户、组和应用程序授予访问权限。 授予访问权限以管理 Azure 订阅中使用 Azure 资源管理器部署模型的资源。
...
可以通过设置密钥保管库的 Key Vault 访问策略授予数据平面访问权限。 若要设置这些访问策略，用户、组或应用程序必须具有该密钥保管库管理平面的 `Contributor` 权限。
...
Azure 基于角色的访问控制是一种替代权限模型，可用于控制对单个密钥保管库启用的 Azure Key Vault 数据平面的访问。 Azure RBAC 权限模型是专用的，一旦设置，保管库访问策略将变为非活动状态。 Azure Key Vault 定义一组 Azure 内置角色，这些角色包含用于访问密钥、机密或证书的公用权限集。

将 Azure 角色分配到 Azure AD 安全主体后，Azure 会向该安全主体授予对这些资源的访问权限。 可以将访问权限限定为订阅级别、资源组、密钥保管库或单个密钥、机密或证书。 Azure AD 安全主体可以是用户、组、应用程序服务主体，也可以是 [Azure 资源的托管标识](../../active-directory/managed-identities-azure-resources/overview.md)。

使用 Azure RBAC 权限对保管库访问策略的主要优势是集中式访问控制管理，它与 Privileged Identity Management (PIM) 相集成。 Privileged Identity Management 提供基于时间和基于审批的角色激活，用于缓解所关注资源上出现的访问权限过度、不必要或滥用的风险。


## <a name="firewalls-and-virtual-networks"></a>防火墙和虚拟网络

为了增加安全性，你可以配置防火墙和虚拟网络规则。 可以将 Key Vault 防火墙和虚拟网络配置为默认拒绝访问来自所有网络的流量（包括 Internet 流量）。 可以向来自特定 Azure 虚拟网络和公共 Internet IP 地址范围的流量授予访问权限，为应用程序构建安全的网络边界。
...
在此示例中，我们正在开发一个应用程序，该应用程序使用 TLS/SSL 的证书、用于存储数据的 Azure 存储以及用于加密 Azure 存储中的数据的 RSA 2048 位密钥。 我们的应用程序在 Azure 虚拟机 (VM)（或虚拟机规模集）中运行。 我们可以使用密钥保管库来存储应用程序机密。 我们可以存储应用程序用于通过 Azure AD 进行身份验证的启动证书。

我们需要对以下存储密钥和机密的访问权限：
- **TLS/SSL 证书**：用于 TLS/SSL。
- **存储密钥**：无法访问存储帐户。
- **RSA 2048 位密钥**：用于通过 Azure 存储进行包装/解包数据加密密钥。
- **应用程序托管标识**：用于对 Azure AD 进行身份验证。 授予对 Key Vault 的访问权限后，应用程序可以提取存储密钥和证书。

我们需要定义以下角色，以指定可以管理、部署和审核应用程序的用户：
- **安全团队**：CSO（首席安全官）办公室中的 IT 人员或类似参与者。 安全团队负责机密的适当保管。 机密可以包括 TLS/SSL 证书、用于加密的 RSA 密钥、连接字符串和存储帐户密钥。
...
- 添加密钥和机密。
- 为灾难恢复创建密钥备份。
- 设置 Key Vault 访问策略，并分配角色以向用户和应用程序授予特定操作的权限。
- 定期滚动密钥和机密。

**开发人员和操作人员**
- 从安全团队获取启动和 TLS/SSL 证书的引用 (指纹) 、存储密钥 (机密 URI) 和 RSA 密钥 (密钥 URI) 用于包装/解包。
- 开发和部署应用程序以编程方式访问证书和机密。
...
| --- | --- | --- | --- |
| 安全团队 | 密钥保管库参与者 | 证书：所有操作 <br> 密钥：所有操作 <br> 机密：所有操作 | Key Vault 管理员 (预览版)  |
| 开发人员和&nbsp;操作人员 | 密钥保管库部署权限<br><br> **注意**：此权限允许已部署的 VM 从密钥保管库提取机密。 | 无 | 无 |
| 审核人员 | 无 | 证书：列表 <br> 密钥：列出<br>机密：列出<br><br> **注意**：此权限让审核员能够检查日志中未发出的密钥和机密的属性（标记、激活日期、到期日期）。 | Key Vault Reader (预览版)  |
| Azure 存储帐户 | 无 | 密钥： get、list、wrapKey、unwrapKey <br> | Key Vault 加密服务加密 |
| 应用程序 | 无 | 机密：获取、列出 <br> 证书：获取、列出 | Key Vault Reader (预览版) ，Key Vault 机密用户 (预览版)  |

三个团队角色需要访问其他资源的权限以及密钥保管库权限。 若要部署 Vm (或 Azure App Service) 的 Web 应用功能，开发人员和操作员需要部署访问权限。 审核员需要具有对存储密钥保管库日志的存储帐户的“读取”访问权限。
...
[分配 Azure 角色以访问密钥、机密和证书](rbac-guide.md)
