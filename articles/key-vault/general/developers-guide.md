...
[Key Vault REST API 参考](/rest/api/keyvault/)。
...
[适用于 Key Vault 的 Java SDK](/java/api/overview/azure/keyvault)
...
- [从 Web 应用程序使用 Azure Key Vault](../secrets/quick-create-net.md) - 此教程有助于你了解如何从 Azure 中的 Web 应用程序使用 Azure Key Vault。 
...
- [访问位于防火墙后面的密钥保管库](access-behind-firewall.md) -若要访问密钥保管库，密钥保管库客户端应用程序必须能够访问各种功能的多个终结点。
...
- [如何使用端到端密钥轮替和审核设置 Key Vault](../secrets/key-rotation-log-monitoring.md) - 逐步介绍如何设置 Azure Key Vault 的密钥轮替和审核。
...
- [向多个应用程序授予 Key Vault 的访问权限](group-permissions-for-apps.md)密钥保管库访问控制策略支持多达 1024 个条目。 但是，可以创建一个 Azure Active Directory 安全组。 将所有关联的服务主体添加到此安全组，并为此安全组授予密钥保管库的访问权限。
