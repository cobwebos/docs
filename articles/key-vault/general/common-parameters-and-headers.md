...
- HTTP `Host` 标头必须始终存在并且必须指定保管库主机名。 示例：`Host: contoso.vault.azure.net`。 请注意，大多数客户端技术将从 URI 填充 `Host` 标头。 例如，`GET https://contoso.vault.azure.net/secrets/mysecret{...}` 会将 `Host` 设置为 `contoso.vault.azure.net`。 这意味着，如果使用 `GET https://10.0.0.23/secrets/mysecret{...}` 等原始 IP 地址访问 Key Vault，`Host` 标头的自动值将是错误的，并且必须手动确保 `Host` 标头包含保管库主机名。
