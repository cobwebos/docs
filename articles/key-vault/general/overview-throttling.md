...
1. 如果使用密钥保管库存储服务的凭据，请检查该服务是否支持 Azure AD 身份验证以直接进行身份验证。 这减少了密钥保管库的负载，提高了可靠性并简化了代码，因为密钥保管库现在可以使用 Azure AD 令牌。  许多服务已转向使用 Azure AD Auth。 请参阅支持 Azure[资源的托管标识的服务中的](../../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-managed-identities-for-azure-resources)当前列表。
