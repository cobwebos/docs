...
Key Vault 最多支持 1024 个访问策略条目，每个条目可向“主体”授予一组不同的权限： 例如，这是[适用于 .NET 的 Azure Key Vault 客户端库快速入门](../secrets/quick-create-net.md)中的控制台应用访问 Key Vault 的方式。

有关 Key Vault 访问控制的完整详细信息，请参阅 [Azure Key Vault 安全性：标识和访问管理](overview-security.md#identity-and-access-management)。 有关访问控制的完整详细信息，请参阅： 

- [“键”](../keys/index.yml)
- [机密访问控制](../secrets/index.yml)
- [证书访问控制](../certificates/index.yml)
...
第二种方式是在终端窗口中创建服务主体。 在 Azure CLI 中，使用 [az ad sp create-for-rbac](/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac) 命令，并以“http://&lt;my-unique-service-principal-name&gt;”的格式为 -n 标志提供唯一服务主体名称。
