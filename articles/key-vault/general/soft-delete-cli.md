...
Azure Key Vault 的软删除功能允许恢复已删除的保管库和保管库对象。 软删除将具体探讨以下方案：
...
- Azure CLI - 如果环境没有此设置，请参阅[使用 Azure CLI 管理 Key Vault](manage-with-cli2.md)。
...
默认情况下，自动对所有密钥保管库启用软删除。 自 2020 年 12 月 31 日起，无法在禁用软删除的情况下创建新的密钥保管库。
...
若要在创建保管库时启用软删除和清除保护，请使用 [az keyvault create](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-create) 命令：
