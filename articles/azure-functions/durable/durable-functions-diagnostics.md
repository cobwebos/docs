...
Azure Functions Durable 扩展还会发出跟踪事件，用于跟踪业务流程的端到端执行。** 可以使用 Azure 门户中的[应用程序见解分析](../../azure-monitor/app/analytics.md)工具查找和查询这些跟踪事件。
...
* **appName**：函数应用的名称。 当您有多个功能应用共享同一应用程序见解实例时，此字段非常有用。
* **slotName**：运行当前函数应用的[部署槽位](../functions-deployment-slots.md)。 当您利用部署槽来对业务流程进行版本时，此字段非常有用。
...
* **state**：实例的生命周期执行状态。 有效值包括：
  ...
  * **Completed**：函数已成功完成。
  * **Failed**：函数失败并出错。
* **reason**：与跟踪事件关联的其他数据。 例如，如果某个实例正在等待外部事件通知，则此字段指示该实例正在等待的事件的名称。 如果函数失败，此字段将包含错误详细信息。
* **isReplay**：指示跟踪事件是否用于重播执行的布尔值。
* **extensionVersion**：持久任务扩展的版本。 报告扩展中可能存在的 Bug 时，版本信息尤为重要。 如果长时间运行的实例在运行时发生更新，它可能会报告多个版本。
* **sequenceNumber**：事件的执行序列号。 与时间戳组合使用可以帮助按执行时间对事件进行排序。 *请注意，如果主机在实例正在运行时重新启动，则此数字将重置为零，因此始终先按时间戳然后按 sequenceNumber 排序很重要。*

可跟踪发送到应用程序见解的数据的详细程度可以在`logger``logging``host.json`文件的 （函数 1.x） 或 （函数 2.0） 部分进行配置。
...
从持久函数 2.0 开始，.NET 协调器函数还可以选择创建一`ILogger`个，在重播期间自动筛选出日志语句。 此自动筛选使用`IDurableOrchestrationContext.CreateReplaySafeLogger(ILogger)`API 完成。
...
* **重播**：当收到新输入时，协调器功能会定期[重播](durable-functions-orchestrations.md#reliability)。 此行为意味着协调器函数的单个*逻辑*执行可能导致多次击中同一断点，尤其是在函数代码早期设置时。
* **Await**：`await`每当在协调器函数中遇到 时，它都会将控制权生成回持久任务框架调度程序。 如果这是第一次遇到特定任务`await`，*则永远不会*恢复关联的任务。 由于任务永远不会恢复，因此无法*跨步执行*await（Visual Studio 中的 F10）。 仅当任务正在重播时，才能跳过。
* **消息超时**：内部持久函数使用队列消息来推动协调器、活动和实体函数的执行。 在多 VM 环境中，长时间中断调试可能会使另一个 VM 拾取消息，从而导致重复执行。 正则队列触发器函数也存在此行为，但必须在此上下文中指出，因为队列属于实现细节。
* **停止和启动**：持久函数中的消息在调试会话之间保留。 如果在执行持久函数时停止调试并终止本地主机进程，该函数可能会在将来的调试会话中自动重新执行。 此行为在不需要时可能会令人困惑。 在调试会话之间清除[内部存储队列](durable-functions-perf-and-scale.md#internal-queue-triggers)中的所有消息是避免此行为的一种技术。

> [!TIP]
> 在协调器函数中设置断点时，如果只想在非重播执行时中断，则可以设置条件断点，该断点仅在 为`IsReplaying``false`时中断。

## <a name="storage"></a>存储

默认情况下，Durable Functions 在 Azure 存储中存储状态。 此行为意味着您可以使用[Microsoft Azure 存储资源管理器](https://docs.microsoft.com/azure/vs-azure-tools-storage-manage-with-storage-explorer)等工具检查业务流程的状态。
...
> [了解有关 Azure 函数中监视的更多详细信息](../functions-monitoring.md)
