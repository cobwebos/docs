...
> * 计算持久计时器的触发时间时，请始终在 .NET 中使用 `CurrentUtcDateTime` 而非 `DateTime.UtcNow`，在 JavaScript 中使用 `currentUtcDateTime` 而非 `Date.now` 或 `Date.UTC`。 有关详细信息，请参阅[协调器函数代码约束](durable-functions-code-constraints.md)一文。
...
> [!NOTE]
> 前面的 C# 示例以 Durable Functions 2.x 为目标。 对于 Durable Functions 1.x，必须使用 `DurableOrchestrationContext` 而不是 `IDurableOrchestrationContext`。 有关不同版本之间的差异的详细信息，请参阅[持久函数版本](durable-functions-versions.md)一文。
...
> [!WARNING]
> 如果代码`CancellationTokenSource`不会等待完成，请使用`cancel()`（.NET） 或调用返回`TimerTask`的 （JavaScript） 取消持久计时器。 在所有未完成任务都完成或取消之前，Durable Task Framework 不会将业务流程的状态更改为“已完成”。
...
有关如何在协调器函数中实现超时的更深入示例，请参阅["人机交互&超时 - 电话验证](durable-functions-phone-verification.md)"一文。
