...
若要调试任何 PowerShell 函数，该函数需要停止才能附加调试器。 `Wait-Debugger` cmdlet 将停止执行并等待调试器。

只需在 `if` 语句上添加对 `Wait-Debugger` cmdlet 的调用，如下所示：
...
使用 `Wait-Debugger` 时，现在可以使用 Visual Studio Code 或 PowerShell 控制台调试函数。
...
验证是否在要附加调试器的函数中设置 `Wait-Debugger`。  添加 `Wait-Debugger` 后，可以使用 Visual Studio Code 调试 function app。
...
你会注意到响应不会立即返回。 这是因为 `Wait-Debugger` 已附加调试器，并且 PowerShell 执行会尽快进入中断模式。 这是由于 BreakAll 的[概念](#breakall-might-cause-your-debugger-to-break-in-an-unexpected-place)，稍后将对此进行说明。 按下 "`continue`" 按钮后，调试器会立即在 `Wait-Debugger`后中断行。
...
>[!NOTE]
> 本部分假设你已阅读[Azure Functions Core Tools 文档](functions-run-local.md)并知道如何使用 `func host start` 命令来启动函数应用。

打开控制台，`cd` 到函数应用的目录中，并运行以下命令：
...
其中一个控制台充当客户端。 从此，你调用 `Invoke-RestMethod` 来触发函数。 例如，你可以运行以下命令：

```powershell
Invoke-RestMethod "http://localhost:7071/api/HttpTrigger?Name=Functions"
```

你会注意到，它不会返回响应，这是 `Wait-Debugger`的结果。 PowerShell 运行空间现在正在等待要附加的调试程序。 接下来，我们来吧。
...
请记下表中项的 `ProcessId`，其中 `ProcessName` 为 `dotnet`。 此过程是函数应用。
...
您可以再次调用相同的函数（例如，使用 `Invoke-RestMethod`），调试器将在 `Wait-Debugger` 命令后立即中断。
...
在实际调用你的 `run.ps1` 脚本之前，Azure Functions 运行时将运行几个命令，因此，调试器可能会在 `Microsoft.Azure.Functions.PowerShellWorker.psm1` 或 `Microsoft.Azure.Functions.PowerShellWorker.psd1`内中断。
