...
使用此选项可以将推送包含到 Azure function app 的 zip 文件。 （可选） 还可以指定要在中启动应用程序[运行从包](run-functions-from-deployment-package.md)模式。
...
部署包含 function app 的 Linux 容器映像。
...
可以将函数应用连接到 git 存储库，以便对该存储库中的代码的任何更新会触发部署。 有关详细信息，看一看[Kudu Wiki](https://github.com/projectkudu/kudu/wiki/VSTS-vs-Kudu-deployments)。
...
允许从本地计算机代码推送到 Azure Functions 使用 Git。
...
可以将内容同步 Dropbox 和 OneDrive 中到 Azure Functions。
...
使用基于门户的编辑器，可直接编辑在 function app 上的文件 (实质上部署任何时候单击**保存**)。
...
<sup>*</sup> 对于使用专用计划在 Linux 上的函数，仅 HTTP 和计时器触发器的启用了门户编辑。
