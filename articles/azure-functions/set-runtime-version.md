...
函数应用在特定版本的 Azure Functions 运行时上运行。 在预览版中有两个主要版本： 1.x[和](functions-versions.md)2.x。 默认情况下，函数应用基于 2.x 版的运行时创建。 本文介绍如何在 Azure 中将函数应用配置为在所选的版本上运行。 有关如何为特定版本配置本地开发环境的信息，请参阅[在本地编码和测试 Azure Functions](functions-run-local.md)。
...
> [!NOTE]
> 如果你固定到 Azure Functions 的特定版本，然后尝试使用 Visual Studio 发布到 Azure，则会弹出一个对话框窗口，提示你更新到最新版本或取消发布。 若要避免出现此情况，请在 `<DisableFunctionExtensionVersionUpdate>true</DisableFunctionExtensionVersionUpdate>` 文件中添加 `.csproj` 属性。
...
可以使用 `FUNCTIONS_EXTENSION_VERSION`az functionapp config appsettings set[ 命令更新函数应用中的 ](/cli/azure/functionapp/config/appsettings) 设置。
...
可以通过在前面代码示例中选择“试一试”[](../cloud-shell/overview.md)运行这个来自 **Azure Cloud Shell** 的命令。 还可以在执行 [az login](/cli/azure/install-azure-cli) 登录后使用 [Azure CLI 在本地](/cli/azure/reference-index#az-login)执行此命令。
