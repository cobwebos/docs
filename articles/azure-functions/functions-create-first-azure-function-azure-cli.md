...
```
默认情况下，使用的 Function App 附带消耗托管计划，这意味着，系统会根据函数的需要动态添加资源，只需在运行函数时付费。 有关详细信息，请参阅[选择适当的托管计划](functions-scale.md)。 
...
--manual-integration 
```
设置部署源后，Azure CLI 会显示类似于以下示例的信息（为方便阅读，已删除 null 值）：
...
![函数响应会显示在浏览器中。](./media/functions-create-first-azure-function-azure-cli/functions-azure-cli-function-test-curl.png)  
...
![函数响应会显示在浏览器中。](./media/functions-create-first-azure-function-azure-cli/functions-azure-cli-function-test-browser.png)  
