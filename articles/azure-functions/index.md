---
title: Azure Functions Documentation | Microsoft Azure
description: Azure Functions Documentation
services: azure-functions
documentationcenter: 
author: kriscrider
manager: carolz
editor: 
tags: 
ms.assetid: 4d991e72-f48c-482c-acf8-cde0c066f667
ms.service: azure-functions
ms.devlang: na
ms.topic: landing page
ms.tgt_pltfrm: na
ms.workload: na
/06/2016ms.date: 11
ms.author: kriscr
translationtype: Human Translation
ms.sourcegitcommit: 888608c01793e08902e5a8daa15066bc87b10077
ms.openlocfilehash: 0045f92c039ea3b44a177648f5e11864f4905af6


---
# <a name="azure-functions-documentation"></a>Azure Functions Documentation

Azure Functions is a solution for easily running small pieces of code, or "functions," in the cloud. Azure Functions is a solution to process events with a serverless code architecture.  Azure Functions supports multiple languages and allows you to write only the code you need without worrying about application or infrastructure to run it.

<ul class="panelContent cardsA">
    <li>
        <a href="functions-overview">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="images/placeholder.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <p>Learn about Azure Functions</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="https://azure.microsoft.com/en-us/get-started/?b=16.43#">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="images/placeholder.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <p>Get started with Azure Functions (Video)</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="https://azure.microsoft.com/en-us/documentation/videos/index/?services=azure-functions ">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="images/placeholder.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <p>Azure Functions Video Library</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
    <li>
        <a href="functions-create-first-azure-function">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="images/placeholder.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <p>Create an Azure Function</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
   <li>
        <a href="functions-create-an-azure-connected-function">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="images/placeholder.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <p>Create an Azure Connected function</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>             
       <li>
       <a href="functions-create-a-web-hook-or-api-function">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="images/placeholder.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <p>Create a Web hook or API function</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>
   <li>
        <a href="functions-create-an-event-processing-function">
        <div class="cardSize">
            <div class="cardPadding">
                <div class="card">
                    <div class="cardImageOuter">
                        <div class="cardImage">
                            <img src="images/placeholder.svg" alt="" />
                        </div>
                    </div>
                    <div class="cardText">
                        <p>Create an Event Processing function</p>
                    </div>
                </div>
            </div>
        </div>
        </a>
    </li>     
</ul>

---
# <a name="reference"></a>Reference

## <a name="nodejs"></a>Node.js
[Azure Functions Node.js developer reference](functions-reference-node)

## <a name="c"></a>C#
[Azure Functions C# developer reference](functions-reference-csharp)

## <a name="f"></a>F#
[Auzre Functions F# Developer reference](functions-reference-fsharp)

---
<div class="downloadHolder">
    <a href="https://msdn.microsoft.com/">
        <div class="img"></div>
        <div class="text">
Download Azure Functions Documentation </div>
    </a>
</div>



<!--HONumber=Nov16_HO2-->


