...
在本地计算机上开发功能，并使用核心工具将其发布到 Azure 后，请遵循以下基本步骤：

> [!div class="checklist"]
> * [安装核心工具和依赖项。](#v2)
> * [使用特定于语言的模板创建 function app 项目。](#create-a-local-functions-project)
...
版本2.x 要求在初始化项目时为其选择默认语言。 在版本2.x 中，添加的所有函数都使用默认语言模板。 在版本 1.x 中，每次创建函数时都要指定语言。
...
默认情况下，将项目发布到 Azure 时，这些设置不会自动迁移。 `--publish-local-settings`发布时[使用 ](#publish) 开关确保已将这些设置添加到 Azure 中的函数应用。 请注意，**ConnectionStrings** 中的值永远不会发布。
...
如果没有为[`AzureWebJobsStorage`]设置有效的存储连接字符串，并且未使用模拟器，则会显示以下错误消息：
...
若要运行 Functions 项目，请运行 Functions 主机。 宿主为项目中的所有函数启用触发器。 启动命令因项目语言而异。
...
| **`--language-worker`** | 用于配置语言辅助角色的参数。 例如，可以通过提供[调试端口和其他所需的参数](https://github.com/Azure/azure-functions-core-tools/wiki/Enable-Debugging-for-language-workers)来启用语言辅助角色调试。 版本1.x 不支持。 |
...
>[!IMPORTANT]
>在本地运行时，不会对 HTTP 终结点强制执行授权。 这意味着所有本地 HTTP 请求都将作为 `authLevel = "anonymous"` 处理。 有关详细信息，请参阅 [HTTP 绑定](functions-bindings-http-webhook-trigger.md#authorization-keys)一文。
...
以下 cURL 命令使用查询字符串中传递的 name`MyHttpTrigger`_参数从 GET 请求触发_ quickstart 函数。
...
对于除 HTTP 触发器和 webhook 和事件网格触发器之外的所有类型的函数，可以通过调用管理终结点在本地测试函数。 在本地服务器上通过 HTTP POST 请求调用此终结点会触发该函数。 

若要在本地测试事件网格触发函数，请参阅[利用查看器 web 应用进行本地测试](functions-bindings-event-grid-trigger.md#local-testing-with-viewer-web-app)。
...
>[!IMPORTANT]
> 只有版本1.x 的工具支持 `func run` 命令。 有关详细信息，请参阅主题[如何指向 Azure Functions 运行时版本](set-runtime-version.md)。

在版本1.x 中，你还可以使用 `func run <FunctionName>` 直接调用函数，并为函数提供输入数据。 此命令类似于在 Azure 门户中使用“测试”选项卡运行函数。
...
此命令发布到 Azure 中的现有函数应用。 如果你尝试发布到你的订阅中不存在的 `<FunctionAppName>`，将会出现错误。 若要了解如何使用 Azure CLI 从命令提示符或终端窗口创建函数应用，请参阅[为无服务器执行创建函数应用](./scripts/functions-cli-create-serverless.md)。 默认情况下，此命令使用[远程生成](functions-deployment-technologies.md#remote-build)并部署你的应用程序，以便[从部署包中运行](run-functions-from-deployment-package.md)。 若要禁用此建议的部署模式，请使用 `--nozip` 选项。
...
了解如何使用 Azure Functions Core Tools [Microsoft 了解模块](https://docs.microsoft.com/learn/modules/develop-test-deploy-azure-functions-with-core-tools/)Azure Functions Core Tools 是[开放源代码并托管在 GitHub 上](https://github.com/azure/azure-functions-cli)来开发、测试和发布 Azure Functions。  
