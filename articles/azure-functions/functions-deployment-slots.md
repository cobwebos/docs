...
- **不同的环境用于不同目的**：使用不同的插槽，您可以在交换到生产槽或暂存槽之前区分应用实例。
- **预热**：部署到插槽而不是直接部署到生产允许应用程序在上线前预热。 此外，使用槽可以降低 HTTP 触发的工作负荷的延迟。 实例在部署之前将会预热，这可以减少新部署的函数的冷启动。
- **简单的回退**：在与生产交换后，具有以前暂存应用的插槽现在具有以前的生产应用。 如果交换到生产槽的更改不符合预期，你可以立即反向交换，以恢复“上次已知正常的实例”。

## <a name="swap-operations"></a>交换操作

在交换期间，一个槽被视为源，另一个槽被视为目标。 源槽包含已应用到目标槽的应用程序实例。 以下步骤可确保在交换期间目标槽不会发生停机：

1. **应用设置：** 目标槽中的设置将应用于源槽的所有实例。 例如，生产设置将应用到过渡实例。 应用的设置包括以下类别：
    - [特定于槽的](#manage-settings)应用设置和连接字符串（如果适用）
    - [连续部署](../app-service/deploy-continuous-deployment.md)设置（如果启用）
    - [应用服务身份验证](../app-service/overview-authentication-authorization.md)设置（如果已启用）

1. **等待重新启动和可用性：** 交换将等待源槽中的每个实例完成其重新启动并可用于请求。 如果有任何实例无法重启，交换操作将会还原对源槽所做的所有更改，并停止操作。

1. **更新路由：** 如果源插槽上的所有实例都成功预热，则两个插槽通过切换路由规则完成交换。 完成此步骤后，目标槽（例如生产槽）包含先前已在源槽中预热的应用。

1. **重复操作：** 现在，源槽以前在目标槽中具有预交换应用，则通过应用所有设置并重新启动源槽的实例来执行相同的操作。
...
1. 验证交换的配置设置，然后单击 **"交换**![Azure 函数"部署槽](./media/functions-deployment-slots/azure-functions-deployment-slots-swap-config.png)
...
- [删除](https://docs.microsoft.com/cli/azure/functionapp/deployment/slot?view=azure-cli-latest#az-functionapp-deployment-slot-delete)
- [列表](https://docs.microsoft.com/cli/azure/functionapp/deployment/slot?view=azure-cli-latest#az-functionapp-deployment-slot-list)
- [交换](https://docs.microsoft.com/cli/azure/functionapp/deployment/slot?view=azure-cli-latest#az-functionapp-deployment-slot-swap)
