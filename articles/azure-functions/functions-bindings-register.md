...
将示例中的 `<TARGET_VERSION>` 替换为特定包版本，例如 `3.0.0-beta5`。 有效版本以[NuGet.org](https://nuget.org)在各个包页上列出。对应于函数运行时 1.x 或 2.x 的主要版本在绑定的参考文章中指定。

如果使用 `Install-Package` 来引用绑定，则无需使用[扩展捆绑](#extension-bundles)。 此方法特定于在 Visual Studio 中生成的类库。

## <a name="c-class-library-with-visual-studio-code"></a><a name="vs-code"></a>C# 类库，带视觉工作室代码
