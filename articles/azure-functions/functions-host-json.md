...
下面的示例*host.js*版本 2.x + 的文件中指定了所有可能的选项（不包括仅供内部使用的选项）。
...
有关完整的 JSON 结构，请参阅前面的[示例 host.js的文件](#sample-hostjson-file)。

> [!NOTE]
> 日志采样可能会导致一些执行不会显示在 Application Insights 监视器边栏选项卡中。 若要避免日志采样，请将添加 `excludedTypes: "Request"` 到 `samplingSettings` 值。
...
| includedTypes | Null | 要进行采样的以分号分隔的类型列表。空列表隐含了所有类型。 此处列出的替代类型中列出了类型 `excludedTypes` 。 可识别的类型为： `Dependency` 、 `Event` 、 `Exception` 、 `PageView` 、 `Request` 和 `Trace` 。 采样指定类型的实例;未指定或隐含的类型将在不采样的情况下传输。 |
...
| enableW3CDistributedTracing | true | 启用或禁用对 W3C 分布式跟踪协议的支持（并打开旧版相关架构）。 如果为 true，则默认情况下启用 `enableHttpTriggerExtendedInfoCollection` 。 如果 `enableHttpTriggerExtendedInfoCollection` 为 false，则此标志仅适用于传出请求，而不应用于传入请求。 |
| enableResponseHeaderInjection | true | 启用或禁用向响应中注入多组件相关标头。 启用注入允许 Application Insights 在使用多个检测密钥时构造应用程序映射。 如果为 true，则默认情况下启用 `enableHttpTriggerExtendedInfoCollection` 。 如果为 false，则此设置不适用 `enableHttpTriggerExtendedInfoCollection` 。 |

### <a name="applicationinsightssnapshotconfiguration"></a>Applicationinsights.config. snapshotConfiguration

有关快照的详细信息，请参阅[调试 .net 应用中的异常](../azure-monitor/app/snapshot-debugger.md)和[排查 Application Insights Snapshot Debugger 或查看快照](../azure-monitor/app/snapshot-debugger-troubleshoot.md)中的异常。
...
| maximumSnapshotsRequired | 3 | 为单个问题收集的最大快照数，范围为1到999。 在应用程序中，可能会将问题视为单个 throw 语句。 为问题收集的快照数达到此值后，就不会再为该问题收集更多的快照（请参阅 `problemCounterResetInterval` ），并 `thresholdForSnapshotting` 再次达到限制。 |
| problemCounterResetInterval | 24:00:00 | 在一分钟到7天内重置问题计数器的频率。 达到此间隔后，所有问题计数都将重置为零。 已达到用于执行快照的阈值但尚未在中生成快照数的现有问题 `maximumSnapshotsRequired` 仍处于活动状态。 |
...
| -- | -- | -- |
| 消耗 | 5 | 10 |
| 高级<sup>1</sup> | 30 | -1 （未绑定）<sup>2</sup> |
| 专用（应用服务） | 30 | -1 （未绑定）<sup>2</sup> |

<sup>1</sup>只有60分钟才保证高级计划执行，但在技术上不受限制。   
<sup>2</sup>如果值为 `-1` ，则指示未绑定的执行，但建议保留固定的上限。
...
|logLevel|不适用|一个对象，它定义了用于筛选应用中的函数的日志类别。 版本2.x 和更高版本按照 ASP.NET Core 布局进行日志类别筛选。 此设置允许你筛选特定函数的日志记录。 有关详细信息，请参阅 ASP.NET Core 文档中的[日志筛选](/aspnet/core/fundamentals/logging/?view=aspnetcore-2.1#log-filtering)。 |
...
此值指示 host.js上的的架构版本。 `"version": "2.0"`对于面向 v2 运行时或更高版本的函数应用，版本字符串是必需的。 V2 与 v3 之间的架构更改没有 host.js。
...
## <a name="override-hostjson-values"></a>覆盖值 host.js

在某些情况下，你可能希望在特定环境的 host.js文件中配置或修改特定设置，而无需更改文件本身的 host.js。  您可以重写特定 host.js值，将其创建为应用程序设置的等效值。 当运行时找到格式的应用程序设置时 `AzureFunctionsJobHost__path__to__setting` ，它会覆盖位于 JSON 中的设置的等效 host.js`path.to.setting` 。 表示为应用程序设置时， `.` 用于指示 JSON 层次结构的点（）将替换为双下划线（ `__` ）。 

例如，假设你想要在本地运行时禁用 Application insights 采样。 如果更改了本地 host.js文件以禁用 Application Insights，则在部署过程中可能会将此更改推送到生产应用。 执行此操作的更安全方法是改为在文件中创建应用程序设置 `"AzureFunctionsJobHost__logging__applicationInsights__samplingSettings__isEnabled":"false"` `local.settings.json` 。 你可以在以下文件中看到此 `local.settings.json` 内容，这不会发布：
