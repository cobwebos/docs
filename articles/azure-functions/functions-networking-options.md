...
description: Azure Functions 中可用的所有网络选项的概述。
...
* 可以在[应用服务环境](../app-service/environment/intro.md)中运行函数。 此方法将函数部署到虚拟网络中，并提供完整网络控制和隔离。
...
可以使用 IP 限制定义被允许或拒绝访问应用的 IP 地址的列表（按优先级排序）。 该列表可以包含 IPv4 和 IPv6 地址。 如果存在一个或多个条目，则在列表末尾会存在一个隐式的“全部拒绝”。 IP 限制适用于所有函数托管选项。

> [!NOTE]
> 实施网络限制后，只能从虚拟网络内使用门户编辑器，或者仅当将用于访问 Azure 门户的计算机的 IP 地址放入安全收件人列表中时才能使用。 不过，仍然可以从任何计算机访问“平台功能”选项卡上的任何功能。
...
Azure Functions 中的虚拟网络集成将共享基础结构与应用服务 Web 应用结合使用。 若要了解有关这两种类型的虚拟网络集成的详细信息，请参阅：
...
目前，如果 key vault 使用服务终结点进行保护，则 [Key Vault 引用](../app-service/app-service-key-vault-references.md)不起作用。 若要使用虚拟网络集成连接到 key vault，需要在应用程序代码中调用 Key Vault。
...
[混合连接](../service-bus-relay/relay-hybrid-connections-protocol.md)是 Azure 中继的一项功能，可用于访问其他网络中的应用程序资源。 使用混合连接可以从应用访问应用程序终结点。 无法使用它访问应用程序。 在除消耗计划之外的所有计划中，混合连接可用于 Windows 上运行的函数。

在 Azure Functions 中使用时，每个混合连接与单个 TCP 主机和端口组合相关联。 这意味着，混合连接终结点可以位于任何操作系统和任何应用程序上，只要能够访问 TCP 侦听端口即可。 混合连接功能不知道、也不关心应用程序协议或者要访问的内容是什么。 它只提供网络访问。

若要了解详细信息，请参阅[混合连接的应用服务文档](../app-service/app-service-hybrid-connections.md)。 Azure Functions 支持这些相同配置步骤。
...
* [了解有关虚拟网络与应用服务/Functions 的集成的详细信息](../app-service/web-sites-integrate-with-vnet.md)
* [了解有关 Azure 中的虚拟网络的详细信息](../virtual-network/virtual-networks-overview.md)
* [通过应用服务环境启用更多网络功能和控制](../app-service/environment/intro.md)
