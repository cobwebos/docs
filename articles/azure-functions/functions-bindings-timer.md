...
本文介绍了如何在 Azure Functions 中对计时器触发器进行配置和编写代码。 Azure Functions 支持计时器触发器。 计时器触发器基于计划调用函数一次或重复调用函数。 
...
`schedule` 的值是 [CRON 表达式](http://en.wikipedia.org/wiki/Cron#CRON_expression)，其中包含 6 个字段：`{second} {minute} {hour} {day} {month} {day of the week}`。 许多在线的 cron 表达式会忽略 `{second}` 字段。 如果从其中一个进行复制，需要调整额外的 `{second}` 字段。 有关具体示例，请参阅下面的[计划示例](#examples)。

CRON 表达式使用的默认时区为协调世界时 (UTC)。 如果希望 CRON 表达式基于其他时区，请为名为 `WEBSITE_TIME_ZONE` 的 Function App 创建新的应用设置。 将值设置为所需时区的名称，如 [Microsoft 时区索引](https://msdn.microsoft.com/library/ms912391.aspx)中所示。 

例如，东部标准时间是 UTC-05:00。 如果希望计时器触发器每天在东部时间上午 10:00 触发，可使用表示 UTC 时区的以下 CRON 表达式：
