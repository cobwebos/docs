---
title: 适用于 Azure Functions 的 Java 开发人员参考
...
```
在上述示例中，`queryValue` 绑定到 Http 请求 URL `name` 中的查询字符串参数 `http://{example.host}/api/metadata?name=test`。 下面是另一个示例，演示如何从队列触发器元数据绑定到 `Id`。
