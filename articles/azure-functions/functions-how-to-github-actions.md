...
你可以使用[az ad sp 创建-rbac](/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-for-rbac&preserve-view=true)命令[Azure CLI](/cli/azure/)来创建[服务主体](../active-directory/develop/app-objects-and-service-principals.md#service-principal-object)。 使用 Azure 门户中 [Azure Cloud Shell](https://shell.azure.com) 或选择 " **试用** " 按钮来运行此命令。
...
下面的示例使用和的版本 1 `functions-action` `service principal` 进行身份验证。 工作流设置 Windows .NET 环境。 
