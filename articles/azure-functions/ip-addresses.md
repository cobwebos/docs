...
- 删除 SSL 绑定，如[证书续订](../app-service/configure-ssl-certificate.md#renew-certificate)期间。

当函数应用在[消耗计划](functions-scale.md#consumption-plan)中运行时，即使没有执行[上述](#inbound-ip-address-changes)任何操作，入站 IP 地址也可能会更改。
...
当函数应用在[消耗计划](functions-scale.md#consumption-plan)中运行时，出站 IP 地址也可能会更改，即使未执行[以上列出](#inbound-ip-address-changes)的操作也是如此。
