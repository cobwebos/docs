...
了解如何创建在 Blob 存储容器中上传到或更新文件时触发的函数。
...
已成功创建新的 function app。

:::image type="content" source="./media/functions-create-storage-blob-triggered-function/function-app-create-success.png" alt-text="已成功创建 Function App。" border="true":::
...
1. 选择 "**函数**"，然后选择 " **+ 添加**" 以添加新函数。
   ...
   :::image type="content" source="./media/functions-create-storage-blob-triggered-function/function-app-quickstart-choose-template.png" alt-text="选择 Azure 门户中的函数模板。" border="true":::

1. 选择 " **Azure Blob 存储" 触发器**模板。
...
1. 返回到 Azure 门户中，浏览到函数，展开页面底部的“日志”**** 并确保日志流式处理未暂停。
    ...
    :::image type="content" source="./media/functions-create-storage-blob-triggered-function/functions-storage-log-expander.png" alt-text="展开 Azure 门户中的日志。" border="true":::

1. 在单独的浏览器窗口中，在 Azure 门户中中转到资源组，并选择存储帐户。

1. 选择 "**容器**"，然后选择 "**示例工作项**" 容器。

    :::image type="content" source="./media/functions-create-storage-blob-triggered-function/functions-storage-container.png" alt-text="在 Azure 门户中，请参阅示例-工作项容器。" border="true":::

1. 选择 "**上传**"，然后选择文件夹图标，选择要上传的文件。
...
1. 浏览到本地计算机上的文件（如图像文件），选择该文件。 选择 "**打开**"，然后**上传**。
