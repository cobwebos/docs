...
以下示例演示一个从特定数据库和集合触发的[预编译 C# 函数](functions-dotnet-class-library.md)。
...
### <a name="trigger---c-script"></a>触发器 - C# 脚本
...
### <a name="trigger---javascript"></a>触发器 - JavaScript
...
对于[预编译 C#](functions-dotnet-class-library.md) 函数，请使用 NuGet 包 [Microsoft.Azure.WebJobs.Extensions.DocumentDB](http://www.nuget.org/packages/Microsoft.Azure.WebJobs.Extensions.DocumentDB) 中定义的 [CosmosDBTrigger](https://github.com/Azure/azure-webjobs-sdk-extensions/blob/master/src/WebJobs.Extensions.DocumentDB/Trigger/CosmosDBTriggerAttribute.cs) 特性。
...
| **leaseConnectionStringSetting** | **LeaseConnectionStringSetting** | （可选）应用设置的名称，该应用设置包含指向保留租用集合的服务的连接字符串。 未设置时，使用 `connectionStringSetting` 值。 在门户中创建绑定时，将自动设置该参数。 |
...
>[!NOTE] 
>用于租用集合的连接字符串必须具有写入权限。

## <a name="input"></a>输入

DocumentDB API 输入绑定会检索一个或多个 Azure Cosmos DB 文档，并将其传递给函数的输入参数。 可根据调用函数的触发器确定文档 ID 或查询参数。 
...
* [预编译 C#](#input---c-example-2)
* [C# 脚本](#input---c-script-example-2)
* [JavaScript](#input---javascript-example-2)

### <a name="input---c-example-2"></a>输入 - C# 示例 2

以下示例演示一个执行 SQL 查询的[预编译 C# 函数](functions-dotnet-class-library.md)：
...
以下示例演示 *function.json* 文件中的一个 DocumentDB 输入绑定以及使用该绑定的 [C# 脚本函数](functions-reference-csharp.md)。 该函数使用队列触发器自定义查询参数检索 SQL 查询指定的多个文档。
...
以下示例演示 *function.json* 文件中的一个 DocumentDB 输入绑定以及使用该绑定的 [JavaScript 脚本函数](functions-reference-node.md)。 该函数使用队列触发器自定义查询参数检索 SQL 查询指定的多个文档。
...
对于[预编译 C#](functions-dotnet-class-library.md) 函数，请使用 NuGet 包 [Microsoft.Azure.WebJobs.Extensions.DocumentDB](http://www.nuget.org/packages/Microsoft.Azure.WebJobs.Extensions.DocumentDB) 中定义的 [DocumentDB](https://github.com/Azure/azure-webjobs-sdk-extensions/blob/master/src/WebJobs.Extensions.DocumentDB/DocumentDBAttribute.cs) 特性。
...
|**id**    | **Id** | 要检索的文档的 ID。 此属性支持绑定参数。 有关详细信息，请参阅[绑定到绑定表达式中的自定义输入属性](functions-triggers-bindings.md#bind-to-custom-input-properties-in-a-binding-expression)。 不要同时设置 **id** 和 **sqlQuery** 属性。 如果上述两个属性都未设置，则会检索整个集合。 |
...
||**PartitionKey**|指定用于查找分区键值。 可以包含绑定参数。|
...
DocumentDB API 输出绑定允许将新文档写入 Azure Cosmos DB 数据库。 
...
以下示例演示一个使用队列存储消息中提供的数据，将文档添加到数据库的[预编译 C# 函数](functions-dotnet-class-library.md)。
...
以下示例演示 *function.json* 文件中的一个 DocumentDB 输出绑定以及使用该绑定的 [C# 脚本函数](functions-reference-csharp.md)。 该函数使用一个用于某个队列的队列输入绑定，该队列以下列格式接收 JSON：
...
以下示例演示 *function.json* 文件中的一个 DocumentDB 输出绑定以及使用该绑定的 [F# 函数](functions-reference-fsharp.md)。 该函数使用一个用于某个队列的队列输入绑定，该队列以下列格式接收 JSON：
...
以下示例演示 *function.json* 文件中的一个 DocumentDB 输出绑定以及使用该绑定的 [JavaScript 函数](functions-reference-node.md)。 该函数使用一个用于某个队列的队列输入绑定，该队列以下列格式接收 JSON：
...
||**PartitionKey** |当 `CreateIfNotExists` 为 true 时，将定义所创建集合的分区键路径。|
||**CollectionThroughput**| 当 `CreateIfNotExists` 为 true 时，将定义所创建集合的[吞吐量](../cosmos-db/set-throughput.md)。|
|**连接**    |**ConnectionStringSetting** |内含 Azure Cosmos DB 连接字符串的应用设置的名称。        |
