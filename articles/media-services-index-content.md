...
本文是介绍 [Media Services 视频点播工作流](/zh-cn/documentation/articles/media-services-video-on-demand-workflow)的系列教程的一部分。 

使用 Azure Media Indexer，可以使媒体文件内容可供搜索，并为隐藏的字幕和关键字生成全文本脚本。你可以只处理一个媒体文件，也可以一次处理多个媒体文件。你还可以为 Internet 上公开提供的文件编制索引，方法是在清单文件中指定这些文件的 URL。

>[WACOM.NOTE] 在编制内容的索引时，请确保使用语音极其清晰的媒体文件（没有背景音乐、噪音、特效音或麦克风电流嘶嘶声）。适当内容的某些示例包括：录制的会议、讲座或演示内容。以下内容可能不适合用于编制索引：电影、电视剧、混合了音频和声音特效的任何内容、带有背景噪音（电流嘶嘶声）的不当录制内容。
...
本主题介绍如何创建索引作业，以便**为资产编制索引**、**为多个文件编制索引**，以及**为 Internet 上公开的文件编制索引**。

有关最新的 Azure Media Indexer 更新，请参阅 [Media Services 博客](http://azure.microsoft.com/blog/topics/media-services/)。
...
## 为来自 Internet 的文件编制索引

对于 Internet 上公开提供的媒体文件，你也可以为这些文件编制索引，而不用将其复制到 Azure 存储空间。你可以使用清单文件来指定媒体文件的 URL。有关详细信息，请参阅 [Azure Media Indexer 的任务预设](https://msdn.microsoft.com/zh-cn/library/azure/dn783454.aspx)。

请注意，HTTP 和 HTTPS URL 协议受支持。

以下方法和配置将创建为 Internet 上的媒体文件编制索引的作业。
...
在通过 http 或 https 下载 Internet 文件时，索引器支持使用用户名和密码进行的基本身份验证。

你可以在任务配置中指定 **username** 和 **password**，如 [Azure Media Indexer 的任务预设](https://msdn.microsoft.com/zh-cn/library/azure/dn783454.aspx)中所述。
