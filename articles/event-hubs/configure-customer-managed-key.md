...
为启用了 BYOK 的命名空间设置诊断日志, 可以在使用客户管理的密钥对命名空间进行加密时, 提供有关操作的必需信息。 可以启用这些日志, 稍后将其流式传输到事件中心, 或通过 log analytics 进行分析或流式传输到存储, 以执行自定义的分析。 若要了解有关诊断日志的详细信息, 请参阅[Azure 诊断日志概述](../azure-monitor/platform/diagnostic-logs-overview.md)。
