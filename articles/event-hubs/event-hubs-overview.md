...
尽管分区是可识别的并且可以直接向其发送数据，但最好不要向特定分区发送数据。 但是，可以使用[事件发布者](#event-publisher)和[发布者策略](#capacity-and-security)部分中介绍的更高级构造。
...
### <a name="common-publisher-tasks"></a>常见的发布者任务
本部分介绍事件发布者的常见任务。
