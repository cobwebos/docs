...
群集预配和收费**容量单位 (Cu)**，有预分配的数量的 CPU 和内存资源。 你可以购买的每个分类的 1、 2、 4、 8、 12、 16 或 20 个 Cu。 多少可以引入和流式处理每个 CU 取决于多种因素，例如生产者和使用者，有效负载形状出口数速率 （请参阅下面有关更多详细信息的基准校验结果）。 
...
| 分区 | 每个命名空间的 40 | 2000 每 CU，每个事件中心的 1024 |
...
载入到专用的自助服务体验处于预览状态，通过它可以创建 1 CU 在以下区域中的群集：
  - 加拿大中部
  - 西欧
  - 美国中部
  - 美国东部
...
我们正在添加新的区域，但在此期间如果首选的区域不在列表中，请提交支持请求到[事件中心团队](https://ms.portal.azure.com/#create/Microsoft.Support)下*技术 > 事件中心 > 配额 > 为请求专用 SKU*。 专用计划独一无二，用户可从事件中心产品团队处获得适合自己的灵活部署，提供一种亲身实践操作体验。 
...
#### <a name="how-do-i-create-a-cluster-larger-than-1-cu"></a>如何创建一个群集大于 1 CU？

在自助服务体验的预览版本，可以请求以增加你的群集，在创建群集之后。 创建 1 CU 群集后，请联系事件中心支持通过填写[支持请求](https://ms.portal.azure.com/#create/Microsoft.Support)下*技术 > 配额 > 对向上缩放或专用群集向下缩放请求*。 在我们 GA 版本中，你将能够直接通过门户将群集向上扩展。 
