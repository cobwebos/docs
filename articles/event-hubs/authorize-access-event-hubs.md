...
事件中心资源的 Azure Active Directory （Azure AD）集成提供了基于角色的访问控制（RBAC），以便对客户端对资源的访问进行精细控制。 你可以使用基于角色的访问控制（RBAC）向安全主体授予权限，这可能是用户、组或应用程序服务主体。 通过 Azure AD 对安全主体进行身份验证，以返回 OAuth 2.0 令牌。 令牌可用于授权请求访问事件中心资源。
