...
* [Microsoft.Azure.EventHubs.Processor](/dotnet/api/microsoft.azure.eventhubs.processor)
  * 此库添加了其他功能，可用于跟踪已处理的事件，并为从事件中心读取的最简单方法。
...
### <a name="create-an-event-hub-client"></a>创建事件中心
...
接收器将绑定到特定分区，因此为了接收事件中心内的所有事件，你将需要创建多个实例。 一般来说，好的做法是以编程方式获取分区信息，而不是对分区 ID 进行硬编码。 为此，可以使用 [**GetRuntimeInformationAsync**](/dotnet/api/microsoft.azure.eventhubs.eventhubclient#Microsoft_Azure_EventHubs_EventHubClient_GetRuntimeInformationAsync) 方法。
...
由于事件永远不会从事件中心删除（而只会过期），因此你需要指定正确的起始点。 下面的示例演示可能的组合。
