...
Azure 事件中心使您能够在[Azure Blob 存储](https://azure.microsoft.com/services/storage/blobs/)或 Azure[数据存储第 1 代或](https://azure.microsoft.com/services/data-lake-store/)第 2 代帐户中自动捕获事件中心中的流数据，从而增加了指定时间或大小间隔的灵活性。 设置捕获极其简单，无需管理费用即可运行它，并且可以使用事件中心[吞吐量单位](event-hubs-scalability.md#throughput-units)自动进行缩放。 事件中心捕获是在 Azure 中加载流式处理数据的最简单方法，并可让用户专注于数据处理，而不是数据捕获。
...
事件中心捕获使您能够指定自己的 Azure Blob 存储帐户和容器，或用于存储捕获数据的 Azure 数据湖存储帐户。 这些帐户可以与事件中心在同一区域中，也可以在另一个区域中，从而增加了事件中心捕获功能的灵活性。
...
[阿帕奇钻：Azure Blob 存储插件][Apache Drill: Azure Blob Storage Plugin]
...
[Apache Spark][Apache Spark] 是“用于大规模数据处理的统一分析引擎”。 它支持不同的语言，包括 SQL，并且可以轻松地访问 Azure Blob 存储。 在 Azure 中运行 Apache Spark 有几个选项，每个选项都提供了对 Azure Blob 存储的轻松访问：

- [HDInsight：在 Azure 存储中的地址文件][HDInsight: Address files in Azure storage]
- [Azure 数据块：Azure Blob 存储][Azure Databricks: Azure Blob Storage]
- [Azure 库伯奈斯服务](https://docs.microsoft.com/azure/aks/spark-job) 
...
Apache Avro 针对 [Java][Java] 和 [Python][Python] 提供了完整的快速入门指南。 您还可以阅读"[开始使用事件中心捕获](event-hubs-capture-python.md)"一文。
...
[缩放时流式传输：事件中心捕获]:https://github.com/yorek/streaming-at-scale/tree/master/event-hubs-capture
