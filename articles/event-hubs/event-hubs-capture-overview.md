...
可以创建 Azure 事件网格订阅，其中事件中心命名空间作为其源。 以下教程演示了如何使用事件网格和 Azure Functions，将事件中心作为源和 Azure Functions 应用创建事件网格订阅，并将 [捕获的事件中心数据迁移到 Azure Synapse 分析](store-captured-data-data-warehouse.md)。
