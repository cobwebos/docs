...
虽然可以标识分区并向其直接发送数据，但并不建议直接发送到分区， 相反，可以使用中引入的更高级构造[事件发布者](#event-publishers)和容量部分。 
...
## <a name="scaling-with-event-hubs"></a>使用事件中心进行缩放
...
事件中心的吞吐量容量由“吞吐量单位”控制。 吞吐量单位是预先购买的容量单位。 单个吞吐量，可以：
...
分区让您的小数位数下游处理。 由于分区使用者模型，事件中心提供具有分区，您可以向外缩放时同时处理您的活动。 事件中心可以具有最多 32 个分区。

我们建议您权衡 1 对 1 吞吐量单位和分区数目以实现最佳缩放。 单个分区都有保证的入口和出口的最多为 1 个吞吐量单位。 虽然您可能能够实现更高吞吐量的分区，不保证性能。 这就是原因我们强烈建议中的事件中心的分区数为大于或等于吞吐量单位数。

您知道所需的吞吐量单位数和最小数量的分区，但您应在多少分区具有给定计划需要的总吞吐量？ 选择根据你想要实现的下游并行度，以及将来的吞吐量需求的分区数。 不在事件中心中拥有的分区数收费。
