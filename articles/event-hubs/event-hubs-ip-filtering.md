...
ms.locfileid: "81393143"
---
# <a name="configure-ip-firewall-rules-for-an-azure-event-hubs-namespace"></a>为 Azure 事件中心命名空间配置 IP 防火墙规则
默认情况下，只要请求附带了有效的身份验证和授权，就可以从 Internet 访问事件中心命名空间。 使用 IP 防火墙，可以将其进一步限制为采用 [CIDR（无类域间路由）](https://en.wikipedia.org/wiki/Classless_Inter-Domain_Routing)表示法的一组 IPv4 地址或 IPv4 地址范围。
...
>[!WARNING]
> 启用 IP 筛选可以防止其他 Azure 服务与事件中心交互。
>
...
1. 若要限制对特定网络和 IP 地址的访问，请选择 "**所选网络**" 选项。 在 "**防火墙**" 部分中，执行以下步骤：
    ...
    3. 指定是否**允许受信任的 Microsoft 服务跳过此防火墙**。 
        ...
        !["防火墙-所有网络" 选项已选中](./media/event-hubs-firewall/firewall-selected-networks-trusted-access-disabled.png)
