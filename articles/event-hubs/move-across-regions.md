...
如果 Azure 资源组中包含事件中心命名空间的其他资源，则可能需要在资源组级别导出该模板，以便可以通过一个步骤将所有相关资源移到新区域。 本文中的步骤说明如何将 **命名空间** 导出到模板。 将 **资源组** 导出到模板的步骤类似。 
...
若要提交更改并完成移动事件中心命名空间，请删除原始区域中的 **事件中心命名空间** 。 请确保在删除命名空间前处理了该命名空间中的所有事件。 
...
3. 在 " **删除命名空间** " 页上，通过键入 **命名空间名称**来确认删除，然后选择 " **删除**"。 
