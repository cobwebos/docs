...
无需生成自己的解决方案即可满足这些要求。 Azure 事件中心 SDK 提供此功能。 在 .NET 或 Java Sdk 中，使用事件处理客户端（EventProcessorClient），在 Python 和 JavaScript Sdk 中使用 EventHubConsumerClient。 在旧版 SDK 中，它是支持这些功能的事件处理器主机 (EventProcessorHost)。
...
默认情况下，处理事件的函数将按顺序在给定分区中调用。 后续事件和对此函数的调用将在事件泵继续在后台的其他线程上运行的同一个分区队列中调用。 请注意，可以同时处理来自不同分区的事件，并且必须同步跨分区访问的任何共享状态。
