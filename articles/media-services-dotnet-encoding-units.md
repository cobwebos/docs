...
Media Services 帐户与保留单元类型相关联，后者决定了编码作业的处理速度。你可以在以下保留单元类型之间进行选择：基本、标准或高级。例如，与"基本"保留单元类型相比，使用"标准"类型时，相同的编码作业运行速度更快。有关详细信息，请参阅 [Milan Gada](http://azure.microsoft.com/blog/author/milanga/) 撰写的"编码保留单元类型"。
