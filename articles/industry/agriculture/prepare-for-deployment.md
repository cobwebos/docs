...
4. 要安装 Azure FarmBeats 的区域。 目前，以下区域支持 Azure FarmBeats：美国中部、西欧、美国东部2、北欧、美国西部、东南亚、美国东部、澳大利亚东部、美国西部2。
5. 选择“确定”。
...
10. 成功完成部署后，你会收到以下输出链接：
 ...
 - **数据中心 URL**： Swagger 链接，用于试用 Azure FarmBeats api。
 - **快捷键 URL**：用于浏览 Azure FarmBeats 加速器的用户界面。
...
数据中心安装完成后，将收到 URL，以通过 Swagger 接口访问 Azure FarmBeats Api，格式为： https://\<yourdatahub/swagger >
...
加速器安装完成后，将收到 URL 来访问 Azure FarmBeats 用户界面，格式为： https://\<加速器->. appname>.azurewebsites.net
...
目前我们不支持使用安装程序自动卸载 Azure FarmBeats。 若要删除数据中心或加速器，请在 Azure 门户中删除安装了这些组件的资源组，或手动删除资源。

例如，如果在两个不同的资源组中部署了数据中心和加速器，则按如下所述删除这些资源组：

1. 登录到 Azure 门户。
2. 在右上角选择帐户，并切换到要在其中部署 Azure FarmBeats 的所需 Azure AD 租户。
