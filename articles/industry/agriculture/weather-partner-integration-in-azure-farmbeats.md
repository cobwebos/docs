...
天气合作伙伴需要提供 docker 映像/节目（带有下述规范），并在可由客户访问的容器注册表中托管 docker 映像。 天气合作伙伴需要为其客户提供以下信息：
...
如果已安装 FarmBeats，则可以访问 FarmBeats swagger，网址为`https://yourfarmbeatswebsitename-api.azurewebsites.net/swagger`

请注意，"-api" 将追加到你的 FarmBeats 网站名称。
API 终结点将为：`https://yourfarmbeatswebsitename-api.azurewebsites.net`
...
当客户在 FarmBeats 上启动 docker 注册时，应执行此组件。 将传递给此程序的参数（arg1，arg2）为：
...
- /**JobType**： FarmBeats 具有可扩展的作业管理系统。 作为天气预报，你将拥有不同的数据集/Api （例如 GetDailyForecasts），你可以在 FarmBeats 中将其启用为 JobType。 创建 JobType 后，客户可以触发该类型的作业，以获取其位置/相关场的天气数据（请参阅 JobType 和作业 Api in [FarmBeats Swagger](https://aka.ms/farmbeatsswagger)）。
...
- 作为作业的一部分，程序必须根据在启动过程中创建的/WeatherDataModel 创建 **/WeatherDataLocation** 。 **/WeatherDataLocation**对应于用户作为作业的参数提供的位置（lat/long）。
...
连接器 docker 应该能够在元数据上发送更新。 更新方案的示例包括：添加天气提供商的数据集中的新天气参数，以及功能（例如 添加了30天预测）
...
天气数据映射到一个规范消息，该消息将被推送到 Azure 事件中心进行处理。 Azure EventHub 是一项服务，可用于从连接的设备和应用程序引入实时数据（遥测）。 若要将天气数据发送到 FarmBeats，需要创建将消息发送到 FarmBeats 中的事件中心的客户端。 若要了解有关发送遥测数据的详细信息，请参阅向[事件中心发送遥测](https://docs.microsoft.com/azure/event-hubs/event-hubs-dotnet-standard-getstarted-send)数据
...
由于合作伙伴作业将在现有作业框架中运行，因此，在 FarmBeats 中记录的错误与其他预先存在的作业（如 GetFarmData、SensorPlacement 等）的错误相同。 ADF 管道内运行的 ADF 活动记录了 STDERR 和 STDOUT。 这两个文件都在 FarmBeats 资源组中的 "datahublogs-xxx" 存储帐户中可用。
