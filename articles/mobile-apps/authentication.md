...
[App Center](/appcenter/auth/) authentication 是一项基于云的标识管理服务，开发人员可以使用它来对用户进行身份验证和管理用户标识。 App Center 身份验证还与 Visual Studio App Center 的其他部分集成。 开发人员可以使用用户标识来查看其他服务中的[用户数据](/appcenter/data/index)，甚至可以将[推送通知发送给用户，而不是单独的设备](/appcenter/push/push-to-user#setting-user-identity)。 
