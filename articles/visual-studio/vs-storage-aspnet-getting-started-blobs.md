---
title: 通过 Visual Studio 开始使用 Azure Blob 存储（ASP.NET）
...
以下步骤创建一个方法，用于通过**使用 web.config 中**的连接字符串连接到存储帐户。这些步骤还会创建对容器的引用。  **Web.config** 中的连接字符串设置是以 `<storageaccountname>_AzureStorageConnectionString` 格式命名的。 
...
1. 如果没有 Blob 文件夹，则创建一个****。 从上下文菜单中，选择 "**添加** > **新文件夹**"。 将新文件夹命名为 *Blobs*。 
...
1. 有了 Blob 引用之后，可通过调用 Blob 引用对象的 `UploadFromStream` 方法，将任何数据流上传到该引用。 `UploadFromStream` 方法创建 Blob（如果不存在）或者覆盖它（如果存在）。 （将* &lt;文件上传>* 更改为要上传的文件的完全限定路径。）
...
1. 若要列出 Blob 容器中的 Blob，请使用 `CloudBlobContainer.ListBlobs` 方法。 `CloudBlobContainer.ListBlobs` 方法返回一个 `IListBlobItem` 对象，该对象可强制转换为`CloudBlockBlob`、`CloudPageBlob` 或 `CloudBlobDirectory` 对象。 以下代码片段枚举 blob 容器中的所有 blob。 每个 blob 均可基于其类型强制转换为相应的对象。 其名称（对于 **CloudBlobDirectory** 则为 URI）将添加到列表中。
    ...
    **blob** 字符串列表包含的值如下所示（使用前面的代码示例）：
    ...
    如*图所*示，此列表仅包含顶级实体，而不包含嵌套实体（ *baz.png 和 png*）。 若要列出 Blob 容器中的所有实体，请更改代码，以便在 **CloudBlobContainer.ListBlobs** 方法中为 **useFlatBlobListing** 参数传递 **true**。    
...
1. 若要下载 Blob，请使用 `CloudBlockBlob.DownloadToStream` 方法。 以下代码将 blob 的内容传输到一个流对象。 然后将该对象保存到一个本地文件中。 （将* &lt;本地文件名称>* 更改为表示要将 blob 下载到的位置的完全限定文件名。） 
...
本教程已介绍了如何使用 ASP.NET 在 Azure 存储中存储、列出和检索 Blob。 查看更多功能指南，以了解在 Azure 中存储数据的其他方式。
  ...
  * [Azure 表存储和 Visual Studio 连接服务入门（ASP.NET）](vs-storage-aspnet-getting-started-tables.md)
  * [Azure 队列存储和 Visual Studio 连接服务入门（ASP.NET）](vs-storage-aspnet-getting-started-queues.md)
