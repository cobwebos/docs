...
* 有关以编程方式操作队列的详细信息，请参阅[通过 .NET 开始使用 Azure 队列存储](../storage/queues/storage-dotnet-how-to-use-queues.md)。
...
要访问 ASP.NET Core 项目中的队列，需要将下列事项包含在任何访问 Azure 队列存储的 C# 源文件中。
...
要在现有队列中插入消息，请创建新的 **CloudQueueMessage** 对象，并调用 **AddMessageAsync** 方法。
