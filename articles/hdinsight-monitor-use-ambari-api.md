<properties 
	...
	description="使用 Apache Ambari API 设置、管理和监视 Hadoop 群集。Ambari 的直观操作员工具和 API 免除了 Hadoop 的复杂性。" 
	services="hdinsight" 
	documentationCenter="" 
	authors="mumian" 
	editor="cgronlun" 
...
[Apache Ambari][ambari-home] 用于设置、管理和监视 Apache Hadoop 群集。它包括一系列直观的操作员工具和一组免除 Hadoop 复杂性的可靠 API，可简化群集操作。有关这些 API 的详细信息，请参阅 [Ambari API 参考][ambari-api-reference]。
...
- 安装并配置有 Azure PowerShell 的**工作站**。有关说明，请参阅[安装和配置 Azure PowerShell][powershell-install]。若要执行 Azure PowerShell 脚本，必须以管理员身份运行 Azure PowerShell 并将执行策略设为  *RemoteSigned*。有关详细信息，请参阅[运行 Windows PowerShell 脚本][powershell-script]。

- （可选）[cURL][curl]。若要进行安装，请参阅 [cURL 版本和下载][curl-download]。

	>[AZURE.NOTE] 在 Windows 中使用 cURL 命令时，对选项值使用双引号而非单引号。

- **Azure HDInsight 群集**。有关群集设置的说明，请参阅[开始使用 HDInsight][hdinsight-get-started] 或[设置 HDInsight 群集][hdinsight-provision]。你将需要以下数据才能完成本教程：
...
下面是用于在  *HDInsight 3.1 群集*中获取 MapReduce 作业跟踪器信息的 Azure PowerShell 脚本。主要区别在于，我们从 YARN 服务（而非 MapReduce）中拉取这些详细信息。
...
下面是用于在 *HDInsight 2.1 群集*中获取 MapReduce 作业跟踪器信息的 Azure PowerShell 脚本：
...
**对于 10/8/2014 版本**：
在使用 Ambari 终结点时，
"https://{clusterDns}.azurehdinsight.cn/ambari/api/v1/clusters/{clusterDns}.azurehdinsight.cn/services/{servicename}/components/{componentname}"，*host_name* 字段将返回节点的完全限定域名 (FQDN) 而非主机名。在 10/8/2014 版本之前，此示例仅返回了 **headnode0**"。在 10/8/2014 版本之后，你将获取 FQDN "**headnode0.{ClusterDNS}.azurehdinsight.cn**"，如以上示例所示。需要这种改变以利于实现可以在一个虚拟网络 (VNET) 中部署多个群集类型（如 HBase 和 Hadoop）的方案。例如，使用 HBase 作为 Hadoop 的后端平台时，会发生这种情况。

## <a id="monitor"></a>监视 API 的 Ambari
...
<tr><td>获取主机组件</td><td><tt>/api/v1/clusters/&lt;ClusterName&gt;.azurehdinsight.cn/hosts/&lt;HostName&gt;/host_components
</tt></td><td>namenode、resourcemanager</td></tr>
<tr><td>获取主机组件信息</td><td><tt>/api/v1/clusters/&lt;ClusterName&gt;.azurehdinsight.cn/hosts/&lt;HostName&gt;/host_components/&lt;ComponentName&gt;
</tt></td><td>HostRoles、组件、主机、指标</td></tr>
<tr><td>获取配置</td><td><tt>/api/v1/clusters/&lt;ClusterName&gt;.azurehdinsight.cn/configurations 
</tt></td><td>配置类型：core-site、hdfs-site、mapred-site、hive-site</td></tr>
<tr><td>获取配置信息</td><td><tt>/api/v1/clusters/&lt;ClusterName&gt;.azurehdinsight.cn/configurations?type=&lt;ConfigType&gt;&tag=&lt;VersionName&gt; 
</tt></td><td>配置类型：core-site、hdfs-site、mapred-site、hive-site</td></tr>
</table>
