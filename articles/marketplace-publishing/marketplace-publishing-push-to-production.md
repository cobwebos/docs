---
title: 将产品/服务部署到 Azure 市场 | Microsoft Docs
...
3. 单击按钮“请求批准推送到生产”。 发出请求后，批准团队会执行最终审查，随后产品/服务会在 Azure 市场中可用。
...
> * 如果上述步骤都成功，产品/服务将批准用于生产。 此时，发布门户中的状态将变为“已列出”。 但是，此“已列出”状态并不表示过程已完成。 需要先完成以下步骤，产品/服务才能在 Azure 市场中可用。
...
* 在 SKU 选项卡下，仅当 SKU 是解决方案模板的一部分时，才可将“针对市场隐藏此 SKU，因为它始终应通过解决方案模板购买”选项标记为“是”。 在其他所有情况下，此选项应始终标记为“否”。
* 请记住：在 SKU 列出后，不应更改 SKU 可见性设置。 我们不支持此功能。
* 确保徽标遵守下面给出的 Azure 市场徽标准则。
* 产品/服务和 SKU 描述不应该相同。
* SKU 标题和套餐详细摘要不应该相同。
* SKU 标题和产品/服务摘要不应该相同。
...
* Azure 市场不支持对已列出 SKU 进行定价更改，因为它会影响现有客户的计费。 确保不在 SKU 可用的区域中更改已列出 SKU 的定价。 但是，可以添加新的 SKU 或将新区域添加到现有 SKU。
