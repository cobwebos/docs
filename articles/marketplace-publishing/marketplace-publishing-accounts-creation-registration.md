...
1. 在你公司的域中创建通讯组列表 (DL) 或安全组 (SG)。 通过使用 DL，可允许多个人接收对于报告付款信息很重要的电子邮件通知。 它还确保 Microsoft 帐户的所有权可以转让，并且不与个人绑定。
   按照如下说明操作。
   
   1. 将你的培训团队添加到 DL。
   2. 确保 DL/SG 是有效的电子邮件地址，并且能够接收电子邮件，因为付款、税务信息和报告会通过此帐户进行路由。
   3. 我们建议将 marketplace@partnercompany.com 之类的电子邮件地址用于 DL/SG。
2. 打开新的 Chrome Incognito 或 Internet Explorer InPrivate 浏览会话，以确保未登录到现有帐户。
3. 通过使用 [https://signup.live.com/signup.aspx](https://signup.live.com/signup.aspx) 链接，将步骤 1 中创建的 DL 注册为 Microsoft 帐户。 请根据以下说明操作。
...
4. 验证已发送到 DL 的电子邮件地址。
5. 现在，可以在 Microsoft 开发人员中心中使用新的 Microsoft 帐户。

## <a name="2-create-your-microsoft-developer-center-account"></a>2.创建 Microsoft 开发人员中心帐户
