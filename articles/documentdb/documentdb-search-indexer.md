...
`api-version` 是必需的。有效值包括 `2015-02-28` 或更高版本。请访问 [Azure 搜索中的 API 版本](../search/search-api-versions.md)，查看所有支持的搜索 API 版本。
...
如果你还没有目标 Azure 搜索索引，请创建一个。你可以从 [Azure 门户 UI](/documentation/articles/search-get-started/#test-service-operations) 或使用[创建索引 API](https://msdn.microsoft.com/library/azure/dn798941.aspx) 来执行此操作。
...
祝贺你！ 你已学习了如何使用 DocumentDB 索引器将 Azure DocumentDB 与 Azure 搜索进行集成。
 ...
 - 若要了解有关 Azure 搜索的详细信息，请参阅[搜索服务页](/services/search/)。
