...
## <a name="high-level-view-of-notification-process"></a>通知流程的高级视图
...
此 HTTP 操作会执行 GET 操作。  它会调用 API 应用 GetUtcDate 方法。 Uri 使用传入触发器主体的“GetUtcDate_HoursBack”属性。  “GetUtcDate_HoursBack”值在第一个逻辑应用中设置。 你将在稍后的教程中详细了解触发器逻辑应用。
