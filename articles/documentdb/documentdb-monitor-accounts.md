...
可以在 [Microsoft Azure 门户](https://portal.azure.cn/)中监视 DocumentDB 帐户。对于每个 DocumentDB 帐户，性能指标（如请求和服务器错误）和使用情况指标（如存储消耗）都可用。

## 在门户中查看性能指标 
1.	在新窗口打开 [Azure 门户](https://portal.azure.cn/)，单击“浏览”、“DocumentDB 帐户”，然后单击要查看其性能指标的 DocumentDB 帐户的名称。
2.	默认情况下，在“监视”可重用功能区中可以看到如下内容：
	*	当天的请求总数。
	*	当天使用的 RU 总计（[请求单位数](documentdb-request-units.md)）。
	*	使用的存储量。

	如果你的表显示**无可用数据**，请参阅[故障排除](#troubleshooting)部分。

	![“监视”可重用功能区的屏幕截图，它显示当天的请求总数和存储使用情况](./media/documentdb-monitor-accounts/documentdb-total-requests-and-usage.png)


3.	单击“请求数”、“RU 总计”或“存储”磁贴将打开一个详细的“指标”边栏选项卡。
...
## 在门户中设置警报
1.	在 [Azure 门户](https://portal.azure.cn/)中单击“浏览”、“DocumentDB 帐户”，然后单击要为其设置性能指标警报的 DocumentDB 帐户的名称。
...
2.	在“编辑图表”边栏选项卡上，在“时间范围”部分中，单击“过去一小时”，然后单击“确定”。  
...
## 后续步骤
若要了解有关 DocumentDB 容量的详细信息，请参阅 [Manage DocumentDB capacity（管理 DocumentDB 容量）](/documentation/articles/documentdb-manage)。
