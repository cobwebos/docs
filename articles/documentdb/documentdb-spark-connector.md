...
description: "使用本教程来了解 Azure Cosmos DB Spark 连接器：使用该连接器可将 Apache Spark 连接到 Azure Cosmos DB，以便在针对云设计的 Microsoft 多租户全球分布式的数据库系统中执行分布式聚合和数据科学。"
...
* [Azure Cosmos DB](http://documentdb.com)：专门为云设计的 Microsoft 多租户[全球分布式数据库系统](documentdb-distribute-data-globally.md)。 使用 Azure Cosmos DB，客户可跨数目不限的地理区域配置和弹性缩放吞吐量与存储。 该服务向开发员提供 99% 的低延迟、99.99% 的高可用性保证，以及[多个完善定义的一致性模型](documentdb-consistency-levels.md)。
...
![通过 pyCosmos DB 从 Spark 流到 Cosmos DB 的数据流](./media/documentdb-spark-connector/azure-documentdb-spark-pydocumentdb.png)
...
1. 通过 pyCosmos DB 建立从 Spark 主节点到 Cosmos DB 网关节点的连接。  请注意，用户只需指定 Spark 和 Cosmos DB 连接，连接到相应主节点和网关节点的事实对用户是透明的。
...
Spark 到 Cosmos DB 的连接器利用 [Azure Cosmos DB Java SDK](https://github.com/Azure/azure-documentdb-java)，并在 Spark 工作节点与 Cosmos DB 之间移动数据，如下图所示：

![Spark 到 Cosmos DB 的连接器中的数据流](./media/documentdb-spark-connector/azure-documentdb-spark-connector.png)

数据流如下所示：

1. 建立从 Spark 主节点到 Cosmos DB 网关节点的连接，以获取分区映射。  请注意，用户只需指定 Spark 和 Cosmos DB 连接，连接到相应主节点和网关节点的事实对用户是透明的。
2. 此信息将返回给 Spark 主节点。  此时，你应该能够分析查询，确定需要访问 Cosmos DB 中的哪些分区（及其位置）。
3. 此信息将传送到 Spark 工作节点 ...
4. 从而允许 Spark 工作节点直接连接到 Cosmos DB 分区以提取所需的数据，并将数据传送回到 Spark 工作节点中的 Spark 分区。

要点在于，Spark 与 Cosmos DB 之间的通信速度会大幅提高，因为数据移动在 Spark 工作节点与 Cosmos DB 数据节点（分区）之间发生。
...
本部分举例说明如何结合使用 Apache Spark 和 Azure Cosmos DB 来执行分布式聚合与分析。  请注意，根据 [Planet scale aggregates with Azure Cosmos DB](https://azure.microsoft.com/blog/planet-scale-aggregates-with-azure-documentdb/)（使用 Azure Cosmos DB 实现全球级聚合）博客文章中所述，Azure Cosmos DB 已能支持聚合，下面介绍了如何将它与 Apache Spark 配合使用，以进一步发挥它的优势。

请注意，这些聚合与 [Spark 到 Cosmos DB 的连接器笔记本](https://github.com/Azure/azure-documentdb-spark/blob/master/samples/notebooks/Spark-to-Cosmos DB_Connector.ipynb)相关。

### <a name="connecting-to-flights-sample-data"></a>连接到航班示例数据
在这些聚合示例中，我们要访问 DoctorWho Cosmos DB 数据库中存储的一些飞行性能数据。  若要连接到这些数据，需要利用以下代码片段：
...
我们还要使用此代码片段运行一个基本查询，以便将 Cosmos DB 中所需的筛选数据集传输到 Spark（后者可以执行分布式聚合）。  在本例中，我们要查询从西雅图 (SEA) 出发的航班。
