...
**客户托管服务**：拆分/合并将作为客户托管服务交付。您必须在 Microsoft Azure 订阅中部署并托管该服务。您从 NuGet 下载的程序包将包含一个要使用您的特定部署信息完成的配置模板。有关详细信息，请参阅[拆分/合并教程][拆分/合并教程]。由于服务在您的 Azure 订阅中运行，因此您可以控制和配置该服务的大多数安全设置。默认模板包括配置 SSL 的选项、基于证书的客户端身份验证、DoS 防护和 IP 限制。您可以在以下[灵活扩展安全注意事项][灵活扩展安全注意事项]文档中找到有关安全设置的详细信息。
...
将拆分/合并的服务模板预配置为将 WAD（Windows Azure 诊断）存储用于其他详细的日志记录和诊断存储。您可以通过拆分/合并的服务配置文件控制 WAD 的配置，例如存储帐户和凭据。该服务的 WAD 配置遵循[云服务基础][云服务基础]中的指南。它包括用于记录性能计数器、IIS 日志、Windows 事件日志和拆分/合并应用程序事件日志的定义。您可以从“资源管理器”树的 Azure 部分中的 Visual Studio 服务器资源管理器轻松访问这些日志：
...
对话框中的不同选项卡控制不同的日志类型，每个日志类型都有其各自的传输时间段设置。
WAD 中的日志和计数器通常需要对 Microsoft 团队可见，以免您的拆分/合并服务部署出现问题。可以使用一些工具（例如 [Azure 存储资源管理器][Azure 存储资源管理器]）来将 WAD 日志导入可轻松共享的 CSV 文件中。
