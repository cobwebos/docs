<properties 
	pageTitle="使用 REST API 连接到 Media Services 帐户" 
	description="本主题演示如何使用 REST API 连接到 Media Services。" 
...
# 使用 Media Services REST API 连接到 Media Services 帐户

本主题介绍如何在使用 Media Services REST API 编程时获取与 Microsoft Azure Media Services 的编程连接。
...
2. 连接到 Media Services URI 
	...
	>[WACOM.NOTE] 成功连接到 https://media.chinacloudapi.cn 后，将收到一个 301 重定向，指定另一个 Media Services URI。你必须对这个新 URI 进行后续调用。
...
>[WACOM.NOTE] 获取新 URI 后，应该使用该 URI 来与 Media Services 通信。 
