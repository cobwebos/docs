...
当向 GitHub 或 Azure DevOps 中的公共或私有 Git 存储库提交或发出拉取请求时，触发容器映像生成或多步骤任务。 例如，使用 Azure CLI 命令 [az acr task create][az-acr-task-create] 并指定 Git 存储库以及可选的分支和 Dockerfile，来配置生成任务。 当团队在存储库中更新代码时，ACR 任务创建的 Webhook 会触发存储库中定义的容器映像的生成。 
...
要配置源代码更新触发器，您需要为任务提供个人访问令牌 （PAT）， 以在公共或私有 GitHub 或 Azure DevOps 存储库中设置 Webhook。
...
ACR 任务真正增强容器生成工作流的强大功能来自于它检测*基本映像*的更新的能力。 大多数容器映像的一个功能，基本映像是一个或多个应用程序映像基于的父映像。 基本映像通常包含操作系统，有时包括应用程序框架。 

您可以设置 ACR 任务，以在生成应用程序映像时跟踪对基本映像的依赖项。 将更新的基础映像将推送到注册表时，或者在公共存储库（例如 Docker Hub）中更新基础映像时，ACR 任务可以基于该映像自动生成任何应用程序映像。
通过这种自动检测和重新生成，ACR 任务能够节省在正常情况下手动跟踪和更新引用已更新基础映像的每个应用程序映像所需的时间和精力。

了解有关 ACR 任务[的基本映像更新触发器](container-registry-tasks-base-images.md)的详细信息。 了解如何在教程中将基本映像推送到容器注册表时触发映像[生成，在 Azure 容器注册表中更新基本映像时自动生成容器映像](container-registry-tutorial-base-image-update.md)
...
| GitHub 主分支 | 公共或私有 GitHub 存储库的主分支（或其他默认）分支中的文件。  | `https://github.com/gituser/myapp-repo.git` |
| GitHub 分支 | 公共或私有 GitHub 存储库的特定分支。| `https://github.com/gituser/myapp-repo.git#mybranch` |
| GitHub 子文件夹 | 公共或私有 GitHub 存储库中的子文件夹中的文件。 示例显示了分支和子文件夹规范的组合。 | `https://github.com/gituser/myapp-repo.git#mybranch:myfolder` |
| GitHub 提交 | 公共或私有 GitHub 存储库中的特定提交。 示例显示提交哈希 （SHA） 和子文件夹规范的组合。 | `https://github.com/gituser/myapp-repo.git#git-commit-hash:myfolder` |
| Azure DevOps 子文件夹 | 公共或专用 Azure 回购中的子文件夹中的文件。 示例显示了分支和子文件夹规范的组合。 | `https://dev.azure.com/user/myproject/_git/myapp-repo#mybranch:myfolder` |
...
每个任务运行都会生成日志输出，检查该输出即可确定任务步骤是否已成功运行。 手动触发任务时，任务运行的日志输出将流式传输到控制台，并存储以供以后检索。 自动触发某个任务后（例如，提交源代码或更新基础映像后触发），只会存储任务日志。 在 Azure 门户中查看运行日志，或使用[az acr 任务日志](/cli/azure/acr/task#az-acr-task-logs)命令。

查看[和管理任务日志](container-registry-tasks-logs.md)。
