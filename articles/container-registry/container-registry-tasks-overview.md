...
使用熟悉的 `docker build` 格式，Azure CLI 中的[az acr build][az-acr-build]命令采用[上下文](#context-locations)（要生成的文件集），将其发送到 acr 任务，并在默认情况下将生成的映像推送到其注册表。
...
向 GitHub 或 Azure DevOps 中的公共或私有 Git 存储库提交代码或获取或更新拉取请求时，触发容器映像生成或多步骤任务。 例如，通过指定 Git 存储库以及（可选）分支和 Dockerfile，使用 Azure CLI 命令[az acr task create][az-acr-task-create]配置生成任务。 当你的团队更新存储库中的代码时，ACR 任务创建的 webhook 将触发存储库中定义的容器映像的生成。 
...
多步骤任务提供基于步骤的任务定义和执行，用于在云中构建、测试和修补容器映像。 [YAML 文件](container-registry-tasks-reference-yaml.md)中定义的任务步骤指定容器映像或其他项目的单独生成和推送操作。 它们还可以定义一个或多个容器的执行，每个步骤都使用容器作为其执行环境。
...
| GitHub 子文件夹 | 公共或专用 GitHub 存储库中的子文件夹内的文件。 示例显示分支和子文件夹规范的组合。 | `https://github.com/gituser/myapp-repo.git#mybranch:myfolder` |
| Azure DevOps 子文件夹 | 公共或私有 Azure 存储库中的子文件夹内的文件。 示例显示分支和子文件夹规范的组合。 | `https://dev.azure.com/user/myproject/_git/myapp-repo#mybranch:myfolder` |
...
默认情况下，ACR 任务为 Linux OS 和 amd64 体系结构生成映像。 指定 `--platform` 标记，以便为其他体系结构生成 Windows 映像或 Linux 映像。 指定 os/体系结构格式（例如 `--platform Linux/arm`）中的操作系统和支持的体系结构（可选）。 对于 ARM 体系结构，可以选择指定 OS/体系结构/变体格式的变体（例如 `--platform Linux/arm64/v8`）：
