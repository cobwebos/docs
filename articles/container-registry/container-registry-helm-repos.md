...
若要快速地管理和部署适用于 Kubernetes 的应用程序，可以使用[开放源代码 Helm 包管理器][helm]。 使用 Helm，将应用程序包定义为[图表](https://helm.sh/docs/topics/charts/)，并将其收集并存储在[Helm 图表存储库](https://helm.sh/docs/topics/chart_repository/)中。

本文介绍如何在 Azure 容器注册表中使用 Helm 3 或 Helm 2 安装来托管 Helm 图表存储库。 在许多情况下，您将为您开发的应用程序构建并上传您自己的图表。 有关如何生成自己的 Helm 图表的详细信息，请参阅[图表模板开发人员指南][develop-helm-charts]。 还可以从另一个 Helm 存储库存储现有的 Helm 图表。
...
* [Helm 3 客户](#use-the-helm-3-client)端- `helm chart`使用 Helm CLI 中的命令，将注册表中的图表作为[OCI 项目](container-registry-image-formats.md#oci-artifacts)管理
* [Helm 2 客户](#use-the-helm-2-client)端-在 Azure CLI 中使用[az acr Helm][az-acr-helm]命令将容器注册表作为 Helm 图表存储库添加和管理

### <a name="additional-information"></a>其他信息

* 在大多数情况下，我们建议使用 Helm 3 工作流和`helm chart`本机命令将图表作为 OCI 项目来管理。
...
- **Helm client 版本3.1.0 或更高版本**-运行`helm version`以查找当前版本。 有关如何安装和升级 Helm 的详细信息，请参阅[安装 Helm][helm-install]。
- **Kubernetes 群集**，你将在其中安装 Helm 图表。 如果需要，创建[Azure Kubernetes 服务群集][aks-quickstart]。 
- **Azure CLI 版本2.0.71 或更高版本**-运行`az --version`以查找版本。 如果需要进行安装或升级，请参阅[安装 Azure CLI][azure-cli-install]。
...
* 在注册表中将 Helm 3 图表存储为[OCI 项目](container-registry-image-formats.md#oci-artifacts)。 目前，Helm 3 对 OCI 的支持是*实验*性的。
* 使用`helm registry login`命令对注册表进行身份验证。
* 使用`helm chart` Helm CLI 中的命令在注册表中推送、提取和管理 Helm 图表
...
作为一个基本示例，请将目录更改`templates`为该文件夹，并首先删除其中的内容：
...
在`templates`文件夹中，创建一个名`configmap.yaml`为的文件，其中包含以下内容：
...
将目录更改为`hello-world`子目录。 然后，运行`helm chart save`以在本地保存图表副本，并使用注册表的完全限定名称（全部小写）以及目标存储库和标记创建别名。 

在以下示例中，注册表名称为*mycontainerregistry*，目标存储库为 " *hello-世界*"，目标图表标记为*v1*，但会将值替换为你的环境：
...
运行`helm chart list`确认是否已将图表保存在本地注册表缓存中。 输出类似于：
...
### <a name="authenticate-with-the-registry"></a>在注册表中进行身份验证

在 Helm `helm registry login` 3 CLI 中运行命令，使用适用于你的方案的凭据对[注册表进行身份验证](container-registry-authentication.md)。

例如，使用 "请求"[和 "推送" 权限](container-registry-auth-service-principal.md#create-a-service-principal)（AcrPush role）创建对注册表的 "Azure Active Directory" 服务主体。 然后向`helm registry login`提供服务主体凭据。 下面的示例使用环境变量提供密码：
...
在 Helm `helm chart push` 3 CLI 中运行命令，将图表推送到完全限定的目标存储库：
...
输出（在本示例中为缩写） `configMediaType`显示`application/vnd.cncf.helm.config.v1+json`了的：
...
若要将 Helm 图安装到 Kubernetes，图表必须位于本地缓存中。 在此示例中，首先`helm chart remove`运行以删除名为`mycontainerregistry.azurecr.io/helm/hello-world:v1`的现有本地图表：

```console
helm chart remove mycontainerregistry.azurecr.io/helm/hello-world:v1
```

运行`helm chart pull`下载 Azure 容器注册表中的图表，并将其下载到本地缓存：
...
若要查看存储库中导出的图表的信息，请`helm show chart`在导出图表的目录中运行该命令。
...
Helm 返回有关最新版本的图表的详细信息，如以下示例输出所示：
...
运行`helm install`安装已拉取到本地缓存并导出的 Helm 图表。 指定发布名称（如*myhelmtest*）或传递`--generate-name`参数。 例如：
...
若要验证安装，请运行`helm get manifest`命令。 该命令返回`configmap.yaml`模板文件中的 YAML 数据。

运行`helm uninstall`卸载群集上的图表版本：
...
### <a name="add-a-sample-chart-to-the-repository"></a>向存储库添加示例图表
...
键入`ls`以列出下载的图表，并记下文件名中包含的 Wordpress 版本。 `helm fetch stable/wordpress` 命令未指定特定版本，因此会获取最新** 版本。 在下面的示例输出中，Wordpress 图是版本*8.1.0*：
...
如果将图表存储在存储库中并在本地提供更新的索引，可以使用常规 Helm 客户端命令来搜索或安装。 若要查看存储库中的所有图表，请`helm search`使用命令，并提供自己的 Azure 容器注册表名称：
...
若要查看存储库中的特定图表的信息，可以使用`helm inspect`命令。
...
此外，还可以使用 Azure CLI [az acr helm show][az-acr-helm-show] 命令显示图表的信息。 同样，默认情况下会返回图表的最新** 版本。 可以追加`--version`以列出特定版本的图表，如*8.1.0*：
...
若要从存储库中删除一个图表，请使用 [az acr helm delete][az-acr-helm-delete] 命令。 指定图表名称（如*wordpress*）和要删除的版本，如*8.1.0*。
...
当你运行`helm search`时，将继续返回图表。 同样，Helm 客户端不会自动更新存储库中可用图表的列表。 若要更新 Helm 客户端存储库索引，请再次使用 [az acr helm repo add][az-acr-helm-repo-add] 命令：
...
* 详细了解如何在[Azure Kubernetes Service （AKS）](../aks/kubernetes-helm.md)中通过 Helm 安装应用程序。
