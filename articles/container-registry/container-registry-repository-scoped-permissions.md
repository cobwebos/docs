...
* 无法在启用了[匿名请求访问](container-registry-faq.md#how-do-i-enable-anonymous-pull-access)的注册表中创建作用域映射。
...
* **标记**以及生成的密码允许用户使用注册表进行身份验证。 你可以为令牌密码设置过期日期，或者随时禁用令牌。  
  ...
  |`content/write`     |  将数据写入存储库     | 使用 with `content/read`来推送项目 |
...
下面的示例在注册表*myregistry*中创建一个令牌，该令牌具有以下对`samples/hello-world`存储库`content/write`的`content/read`权限：和。 默认情况下，该命令会将默认令牌状态`enabled`设置为，但你可以随时将`disabled`状态更新为。
...
输出包含有关创建的命令的范围映射的详细信息。 你可以使用命名`MyToken-scope-map`的作用域映射将相同的存储库操作应用于其他令牌。 或者，稍后更新范围映射来更改关联令牌的权限。
...
若要创建令牌，请运行[az acr token create][az-acr-token-create] ，并指定*MyScopeMap* scope map。 如前面的示例所示，该命令会将默认令牌状态`enabled`设置为。
...
你可以使用 Azure 门户来创建令牌和作用域映射。 与`az acr token create` CLI 命令一样，你可以应用现有的作用域映射，或在通过指定一个或多个存储库和关联的操作创建令牌时创建作用域映射。 存储库尚不需要位于注册表中。 

下面的示例创建一个标记，并在`samples/hello-world`存储库中创建具有以下权限的范围映射： `content/write`和`content/read`。
...
1. 配置作用域映射：
    1. 为作用域映射输入名称和描述。 
    1. 在 **"存储库**" 下`samples/hello-world`，输入，然后在`content/write`"**权限**" 下选择`content/read`和。 然后选择 " **+ 添加**"。  
...
以下示例使用在本文前面创建的令牌对存储库执行常见操作：推送和拉取映像、删除映像以及列出存储库标记。 标记最初设置为`content/write` `content/read` `samples/hello-world`存储库上的推送权限（和操作）。

### <a name="pull-and-tag-test-images"></a>请求和标记测试映像

对于以下示例，请从 Docker `hello-world`中心`alpine`拉取和映像，并将其标记为注册表和存储库。
...
该令牌`samples/alpine`无权访问存储库，因此，以下推送尝试将失败并出现类似于以下的`requested access to the resource is denied`错误：
...
例如`MyToken-scope-map` ，在`content/write` `content/read` `samples/alpine`存储库中更新和操作，并删除`content/write` `samples/hello-world`存储库上的操作。  
...
1. 在 **"存储库**" 下`samples/alpine`，输入，然后在`content/write`"**权限**" 下选择`content/read`和。 然后选择 " **+ 添加**"。
1. 在 "**存储库**" 下，选择`samples/hello-world` " `content/write`**权限**"，然后选择。 再选择“保存”  。
...
由于作用域映射仅对`content/read` `samples/hello-world`存储库具有权限，因此对存储库的推送`samples/hello-world`尝试现在会失败：
 
```bash
docker push myregistry.azurecr.io/samples/hello-world:v1
```

从这两个存储库中提取映像将成功，因为`content/read`作用域映射提供两个存储库的权限：
...
使用以下[az acr repository delete][az-acr-repository-delete]命令删除`samples/alpine`存储库。 若要删除映像或存储库，该令牌不`docker login`会通过进行身份验证。 而是将令牌的名称和密码传递给命令。 下面的示例使用前面在本文中创建的环境变量：
...
若要读取存储库`samples/hello-world`中的元数据，请运行[az acr repository show-清单][az-acr-repository-show-manifests]或[az acr repository show-tags][az-acr-repository-show-tags]命令。 

若要读取元数据，则令牌不`docker login`通过进行身份验证。 而是将令牌的名称和密码传递到任一命令。 下面的示例使用前面在本文中创建的环境变量：
...
下面的示例为*MyToken*标记生成新的 password1 值，有效期为30天。 它将密码存储在环境变量`TOKEN_PWD`中。 此示例的格式适用于 bash shell。
...
使用 Azure CLI 运行[az acr token update][az-acr-token-update]命令，将设置`status`为： `disabled`
