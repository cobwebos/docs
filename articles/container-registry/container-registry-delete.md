...
> [!WARNING]
> 请谨慎使用以下示例脚本，已删除映像数据是无法恢复的。 如果系统按清单摘要（而不是映像名称）拉取映像，则不应运行这些脚本。 删除清单摘要后，这些系统即无法从注册表拉取映像。 不按清单拉取，而是考虑采用*建议的最佳做法*，即“唯一标记”方案[](container-registry-image-tag-version.md)。 
...
> [!WARNING]
> 请谨慎使用以下示例脚本，已删除映像数据是无法恢复的。 如果系统按清单摘要（而不是映像名称）拉取映像，则不应运行这些脚本。 删除无标的记映像后，这些系统即无法从注册表拉取映像。 不按清单拉取，而是考虑采用*建议的最佳做法*，即“唯一标记”方案[](container-registry-image-tag-version.md)。
