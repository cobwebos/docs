...
* **Azure 容器注册表任务** - 使用 [Azure 容器注册表任务](container-registry-tasks-overview.md)（ACR 任务）可简化在 Azure 中生成、测试、推送和部署映像的过程。 例如，使用 ACR 任务可以通过将 `docker build` 操作产生的负荷转移到 Azure 来将开发内部循环扩展到云。 配置生成任务以使其自动执行容器 OS 和框架修补管道，并使其在团队将代码提交到源代码管理时自动生成映像。
