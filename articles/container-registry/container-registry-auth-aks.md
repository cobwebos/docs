...
本文假定已创建 AKS 群集，并且可以使用 `kubectl` 命令行客户端访问群集。 但是，如果你想要创建群集并在创建群集时配置对容器注册表的访问权限，请参阅[教程：部署 AKS 群集 @ no__t-0 或通过[Azure 容器注册表从 Azure Kubernetes 服务进行身份验证](../aks/cluster-container-registry-integration.md)。
