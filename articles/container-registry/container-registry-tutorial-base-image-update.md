...
生成完成后，如果还要完成以下可选步骤，请记下“生成 ID”（例如，"eastus6"）。
...
然后，使用 `docker run` 在本地运行容器。 将 \<build-id\> 替换为上一步骤的输出（例如，"eastus5"）中找到的生成 ID。
...
如果想要执行以下可选步骤运行新生成的容器，以查看更新的版本号，请记下映像更新触发的生成的“生成 ID”值（在之前的输出中为 "eastus6"）。
