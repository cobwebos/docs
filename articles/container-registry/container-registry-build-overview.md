...
**ACR Build** 是 Azure 容器注册表中的一个功能套件。 它为 Linux、Windows 和 ARM 提供基于云的容器映像生成，并可以针对 Docker 容器自动执行 [OS 和框架修补](#automate-os-and-framework-patching)。
...
Azure CLI 中的 [az acr build][az-acr-build] 命令使用我们熟悉的 `docker build` 格式提取**上下文**（要生成的文件集），将其发送到 ACR Build 服务，并在完成后，默认将生成的映像推送到其注册表。

下表显示了 ACR Build 支持的上下文位置的几个示例：
...
ACR Build 还遵循异地复制的注册表，可让分散在各地的开发团队利用最靠近的复制注册表。
...
> [!NOTE]
> 在初始预览版中，仅当基本映像和应用程序映像驻留在同一 Azure 容器注册表或可公开访问的 Docker 中心存储库中时，基本映像更新才会触发生成。
