...
ms.locfileid: "72293900"
---
# <a name="build-and-push-an-image-from-an-app-using-a-cloud-native-buildpack"></a>使用云本机 Buildpack 从应用生成并推送映像

Azure CLI 命令 @no__t 使用[Buildpacks](https://buildpacks.io/)的[`pack`](https://github.com/buildpack/pack) CLI 工具来生成应用，并将其映像推送到 Azure 容器注册表中。 此功能提供了一个选项，可用于从 node.js、Java 和其他语言中的应用程序源代码快速构建容器映像，而无需定义 Dockerfile。
...
* Buildpack 生成器图像的名称，如 `cloudfoundry/cnb:0.0.12-bionic`。  

`az acr pack build` 支持 ACR 任务命令的其他功能，包括流式处理的[运行变量](container-registry-tasks-reference-yaml.md#run-variables)和[任务运行日志](container-registry-tasks-overview.md#view-task-logs)，并且还保存用于以后检索。

## <a name="example-build-nodejs-image-with-cloud-foundry-builder"></a>例如：用 Cloud Foundry 生成器生成 node.js 映像

下面的示例使用 `cloudfoundry/cnb:0.0.12-bionic` 生成器，在[Azure-Samples/nodejs-hello world](https://github.com/Azure-Samples/nodejs-docs-hello-world)存储库中的 node.js 应用程序中生成容器映像：
...
此示例生成包含 @no__t 标记的 @no__t 0 映像，并将其推送到*myregistry*容器注册表。 此处，将目标注册表名称显式预置到映像名称。 如果未指定，则会自动将注册表 URL 附加到映像名称之前。

@No__t 参数指定命令提取最新的生成器图像。
...
在最喜爱的浏览器中浏览到 `localhost:1337`，查看示例 web 应用。 按 @no__t，停止容器。

## <a name="example-build-java-image-with-heroku-builder"></a>例如：用 Heroku 生成器生成 Java 映像

下面的示例使用 `heroku/buildpacks:18` 生成器在[buildpack/sample](https://github.com/buildpack/sample-java-app)存储库中的 java 应用程序中生成容器映像：
...
此示例生成用命令的运行 ID 标记的 @no__t 0 图像，并将其推送到*myregistry*容器注册表。
...
在最喜爱的浏览器中浏览到 `localhost:8080`，查看示例 web 应用。 按 @no__t，停止容器。
