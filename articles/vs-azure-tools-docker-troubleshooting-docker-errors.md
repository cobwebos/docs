...
添加 docker 支持时，必须将 `.UseUrls(Environment.GetEnvironmentVariable("ASPNETCORE_URLS"))` 添加到 WebHostBuilder()。
如果在 `Program.cs` 中找不到 `Main()` 函数或新的 WebHostBuilder 类，则将显示一条警告。
在 docker 容器内运行时，除了 localhost 外，`.UseUrls()` 是允许 Kestrel 侦听传入流量所必需的。
完成后，典型代码如下所示：
...
UseUrls() 将 WebHost 配置为侦听传入 URL 流量。
[Docker Tools for Visual Studio](http://aka.ms/DockerToolsForVS) 在 dockerfile.debug/release 模式下配置环境变量，如下所示：
...
## 卷映射不正常工作
为启用编辑和刷新功能，卷映射配置为将项目的源代码共享到容器内的 .app 文件夹。
在主机上更改了文件时，容器 /app 目录会使用相同的目录。
在 docker-compose.debug.yml 中，以下配置启用卷映射
...
> [AZURE.NOTE] 使用 Linux VM 时，容器文件系统区分大小写。

如果无法看到内容，请尝试以下操作：

**Docker for Windows beta 版**
- 通过查看系统任务栏中的 `moby` 图标来确认 Docker for Windows 桌面应用是否正常运行（确保该图标为白色且显示正常）。
- 通过右键单击系统任务栏中的 `moby` 图标，选择设置并单击“管理共享驱动器...”来确认是否已配置卷映射
...
无法打开浏览器的原因可能是执行 `docker-compose-up` 期间出错。如要查看该错误，请执行以下步骤：
...
1. 添加 `-noexit` 参数，使该行类似于以下内容。此代码使 PowerShell 保持打开状态，方便查看错误。
