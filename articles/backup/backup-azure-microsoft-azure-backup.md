...
> [!NOTE]
> Azure 备份服务器现在可以保护 VMware VM 并提供改进的安全功能。 请按以下各部分所述安装该产品以及最新的 Azure 备份代理。 若要详细了解如何使用 Azure 备份服务器备份 VMware 服务器，请参阅[使用 Azure 备份服务器备份 VMware 服务器](backup-azure-backup-server-vmware.md)一文。 要了解安全功能，请参阅[Azure 备份安全功能文档](backup-azure-security-feature.md)。
>
>
...
> [!NOTE]
> Azure 有两个用于创建和使用资源的部署模型：[资源管理器和经典](../azure-resource-manager/management/deployment-models.md)。 本文提供有关还原使用 Resource Manager 模型部署的 VM 的信息和过程。
>
>
...
选择用于运行 Azure 备份服务器的服务器时，建议从 Windows Server 2016 Datacenter 或 Windows Server 2019 Datacenter 库映像开始。 [在 Azure 门户中创建第一个 Windows 虚拟机](../virtual-machines/virtual-machines-windows-hero-tutorial.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)一文提供了如何在 Azure 中开始使用建议的虚拟机的教程，即使以前从未使用过 Azure 也没关系。 服务器虚拟机 （VM） 的建议最低要求应该是：具有四个内核和 8 GB RAM Standard_A4_v2。
...
可以使用 Windows Server 重复数据删除来删除 DPM 存储中的重复数据。 了解有关在 Hyper-V VM 中部署时 [DPM 和重复数据删除](https://docs.microsoft.com/system-center/dpm/deduplicate-dpm-storage?view=sc-dpm-2019)如何配合工作的详细信息。
...
> * 作为群集节点的计算机
>
> Windows 服务器核心或 Microsoft Hyper-V 服务器不支持安装 Azure 备份服务器。
...
存储复制选项可让用户在异地冗余存储与本地冗余存储之间进行选择。 默认情况下，恢复服务保管库使用异地冗余存储。 如果此保管库是主保管库，请保留异地冗余存储这一存储选项。 如果想要一个更便宜、但持久性不太高的选项，请选择本地冗余存储。 有关[异地冗余](../storage/common/storage-redundancy-grs.md)和[本地冗余](../storage/common/storage-redundancy-lrs.md)存储选项的详细内容，请参阅 [Azure 存储复制概述](../storage/common/storage-redundancy.md)。
...
3. 默认情况下会打开“设置”**** 边栏选项卡。 如果已关闭，请单击“设置”打开“设置”边栏选项卡。
...
5. 在“备份目标”**** 边栏选项卡中，从“工作负荷的运行位置”**** 菜单中选择“本地”****。
    ...
    从"**要备份什么？"** 下拉菜单中，选择要使用 Azure 备份服务器保护的工作负载，然后单击"**确定**"。
...
7. 选择所有文件，并单击“**下一步**”。 下载 Microsoft Azure 备份下载页中的所有文件，并将所有文件放在同一个文件夹中。
    ...
    由于所有文件的下载大小> 3G，因此在 10Mbps 下载链接上，下载可能需要长达 60 分钟才能完成。
...
2. 在"欢迎"屏幕上，单击"**下一步**"按钮。 随后将转到“*先决条件检查*”部分。 在此屏幕上单击“检查”****，以确定是否符合 Azure 备份服务器的硬件和软件先决条件。 如果完全符合所有先决条件，将看到一条指明计算机符合要求的消息。 单击“**下一步**”按钮。

    ![Azure 备份服务器 - 欢迎页和先决条件检查](./media/backup-azure-microsoft-azure-backup/prereq/prereq-screen2.png)
3. Azure 备份服务器安装包与所需的相应 SQL Server 二进制文件捆绑在一起。 启动新的 Azure 备份服务器安装时，选择"**安装具有此设置的 SQL 服务器新实例**"选项，然后单击"**检查和安装**"按钮。 成功安装必备组件后，单击“**下一步**”。

    >[!NOTE]
    >如果要使用自己的 SQL 服务器，支持的 SQL Server 版本是 SQL Server 2014 SP1 或更高版本，2016 和 2017 年。  所有 SQL Server 版本应为标准版或企业版 64 位。
    >Azure 备份服务器不能与远程 SQL Server 实例配合使用。 Azure 备份服务器使用的实例需在本地。 如果使用现有的 SQL 服务器进行 MABS，MABS 设置仅支持使用 SQL 服务器*的命名实例*。
   ...
   **手动配置**
    ...
    **使用 SQL 2017 时的 SSRS 配置**
    ...
    对 SSRS 配置使用以下值：
    * 服务帐户："使用内置帐户"应为网络服务
    * Web 服务 URL："虚拟目录"应ReportServer_\<SQLInstance 名称>
    * 数据库：数据库名称应为报表服务器/\<SQLInstance 名称>
    * Web 门户 URL："虚拟目录"应Reports_\<SQLInstance 名称>
...
4. 提供 Microsoft Azure 备份服务器文件的安装位置，并单击“**下一步**”。
    ...
    备份到 Azure 需要有暂存位置。 请确保暂存位置的空间至少为要备份到云的数据的 5%。 在磁盘保护方面，安装完成之后需要配置独立的磁盘。 有关存储池的详细信息，请参阅[配置存储池和磁盘存储](https://docs.microsoft.com/previous-versions/system-center/system-center-2012-R2/hh758075(v=sc.12))。
...
10. 从新服务器 cd 的管理员命令行到 Microsoft Azure 备份安装位置和 bin 文件夹
    ...
    路径示例：C：\windows_system32>cd"c：程序文件\微软 Azure 备份\DPM_DPM\bin\"
...
如果您使用的是 ExpressRoute 微软对等互连，请选择以下服务/区域：

* Azure Active Directory (12076:5060)
* Microsoft Azure 区域（根据恢复服务保管库的位置）
* Azure 存储（根据恢复服务保管库的位置）

有关详细信息，请访问[ExpressRoute 路由要求](https://docs.microsoft.com/azure/expressroute/expressroute-routing)。
...
可以将 Azure 订阅从“*已过期*”或“*已取消预配*”状态更改为“*活动*”状态。 但是，当状态不*处于活动状态*时，这会影响产品行为：
...
3. 在受保护的服务器上更新保护代理。
4. 备份应会继续，而无需重启生产服务器。
5. 现在，可以开始保护数据。 如果在保护状态下升级到新式备份存储，则还可以选择备份要存储到的卷，并检查预配不足的空间。 [了解详情](backup-mabs-add-storage.md)。
...
您可以在此处获取有关[为 DPM 准备环境](https://docs.microsoft.com/system-center/dpm/prepare-environment-for-dpm?view=sc-dpm-2019)的详细信息。 其中还包含有关可在其上部署和使用 Azure 备份服务器的受支持配置的信息。 可以使用一系列 [PowerShell cmdlet](https://docs.microsoft.com/powershell/module/dataprotectionmanager/?view=systemcenter-ps-2016) 来执行各种操作。
