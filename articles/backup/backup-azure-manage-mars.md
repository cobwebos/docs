...
- **删除项**：使用此选项可以删除项，使其不会被备份。
  ...
  - 清除卷中的所有选择会导致旧的项目备份，并根据上一次备份时的保留设置进行保留，无需修改作用域。
...
- 单击“确定”以应用更改。****  如果在 Azure 门户中为恢复服务保管库启用了[安全功能](https://docs.microsoft.com/azure/backup/backup-azure-security-feature#enable-security-features)，则系统会提示你输入安全 PIN。 若要接收该 PIN，请遵循[此文](https://docs.microsoft.com/azure/backup/backup-azure-security-feature#authentication-to-perform-critical-operations)中列出的步骤。<br>
...
对于这些计算机，Azure 备份服务将根据备份策略中指定的保留规则确保最后一个恢复点不会过期（即，不会被修剪）。 因此，你可以安全地还原计算机。  考虑可以对备份数据执行的以下方案：
...
- 若要还原计算机，请执行以下步骤：
  - [将 VM 还原到备用目标计算机](backup-azure-restore-windows-server.md#use-instant-restore-to-restore-data-to-an-alternate-machine)
  - 用与源计算机相同的主机名重新创建目标计算机
  - 安装代理，并将其重新注册到同一保管库并采用相同的密码
  - 启动 MARS 客户端以根据你的需求扩展保留期
