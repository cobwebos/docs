...
不是。 备份成本独立于 VM 的成本。 详细了解 [Azure 备份定价](https://azure.microsoft.com/pricing/details/backup/)。
...
不是。 为按需备份作业指定保留期。 默认情况下，从门户触发时，该作业会保留 30 天。
...
### <a name="can-i-backup-or-restore-selective-disks-attached-to-a-vm"></a>我可以备份或还原连接到 VM 的选择性磁盘吗？
...
还原过程保持不变。 如果恢复点是 VM 具有非托管磁盘时的时间点，则可以[将磁盘还原为非托管](tutorial-restore-disk.md#unmanaged-disks-restore)磁盘。 如果 VM 有托管磁盘，则可以[将磁盘还原为托管磁盘](tutorial-restore-disk.md#managed-disk-restore)。 然后，可以从[这些磁盘创建 VM。](tutorial-restore-disk.md#create-a-vm-from-the-restored-disk)
