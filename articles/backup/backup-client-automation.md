...
设置备份策略之后，会根据计划进行备份。 也可以使用 [Start-OBBackup](https://docs.microsoft.com/powershell/module/msonlinebackup/start-obbackup) cmdlet 触发按需备份：
...
但是，存储在 Azure 中的数据会保留，并会根据你设置的保留策略保留下来。 较旧的恢复点会自动过时。
