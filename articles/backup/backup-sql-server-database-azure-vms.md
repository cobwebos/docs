---
title: 在 Azure Vm 中备份 SQL Server 数据库 |Microsoft Docs
description: 了解如何在 Azure Vm 中备份 SQL Server 数据库
ms.reviewer: vijayts
author: dcurwin
manager: carmonm
ms.service: backup
...
4. 专用于 SQL 2008 和 2008 R2,[添加注册表项](#add-registry-key-to-enable-registration)以启用服务器注册。 如果该功能已公开发布, 则不需要执行此步骤。
5. 检查是否没有为数据库启用任何其他备份解决方案。 在备份数据库之前, 禁用所有其他 SQL Server 备份。
...
1. 打开 Regedit
2. 创建注册表目录路径:HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\WorkloadBackup\TestHook (需要在 WorkloadBackup 下创建 "Key" TestHook, 这反过来又需要在 Microsoft 下创建)。
3. 在注册表目录路径下, 创建一个新的 "字符串值", 该字符串的名称为**AzureBackupEnableWin2K8R2SP1** , 值为:**True**

    ![用于启用注册的 RegEdit](media/backup-azure-sql-database/reg-edit-sqleos-bkp.png)

或者, 你可以通过使用以下命令运行 .reg 文件来自动执行此步骤:
...
3. 在“策略名称”处输入新策略的名称。
4. 在 "**完整备份策略**" 中, 选择 "**备份频率**"。 选择 "**每日**" 或 "**每周**"。
