...
- 在**物理服务器或本地 VM 上运行的 DPM** -除了磁盘和磁带备份外，还可以将数据备份到 Azure 中的备份保管库。
- **在 AZURE vm 上运行的 DPM** -通过 System Center 2012 R2 Update 3 或更高版本，你可以在 Azure vm 上部署 dpm。 可以将数据备份到附加到 VM 的 Azure 磁盘，或使用 Azure 备份将数据备份到备份保管库。
