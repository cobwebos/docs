...
若要在虚拟机上为 SQL Server 数据库配置保护，必须在该虚拟机上安装 **AzureBackupWindowsWorkload** 扩展。 如果收到错误 **UserErrorSQLNoSysadminMembership**，则表示 SQL 实例没有所需的备份权限。 若要解决此错误，请遵循[为非市场 SQL VM 设置权限](backup-azure-sql-database.md#set-vm-permissions)中的步骤。

## <a name="backup-type-unsupported"></a>不受支持的备份类型

| Severity | 描述 | 可能的原因 | 建议的操作 |
|---|---|---|---|
| 警告 | 为此数据库的当前设置不支持特定类型的关联的策略中存在的备份类型。 | <li>**Master DB**：仅完整数据库备份操作可以执行对 master 数据库;既不**差异**备份，也事务**日志**的备份。 </li> <li>简单恢复模式  中的任何数据库都不允许进行事务日志  备份。</li> | 将数据库设置修改为支持策略中的所有备份类型。 或者，将当前策略更改为只包含受支持的备份类型。 否则为将在计划备份期间跳过不受支持的备份类型或备份作业的即席备份将失败。
...
| 错误消息 | 可能的原因 | 建议的操作 |
|---|---|---|
| Azure 备份无法连接到 SQL 实例。 | Azure 备份无法连接到 SQL 实例。 | 使用 Azure 门户上错误菜单中的其他详细信息缩小根本原因的范围。 请参阅 [SQL 备份故障排除](https://docs.microsoft.com/sql/database-engine/configure-windows/troubleshoot-connecting-to-the-sql-server-database-engine)来解决错误。<br/><ul><li>如果默认 SQL 设置不允许远程连接，请更改设置。 请参阅以下文章，了解有关更改的设置的信息。<ul><li>[MSSQLSERVER_-1](/previous-versions/sql/sql-server-2016/bb326495(v=sql.130))</li><li>[MSSQLSERVER_2](/sql/relational-databases/errors-events/mssqlserver-2-database-engine-error)</li><li>[MSSQLSERVER_53](/sql/relational-databases/errors-events/mssqlserver-53-database-engine-error)</li></ul></li></ul><ul><li>如果出现登录问题，请参阅以下链接来解决问题：<ul><li>[MSSQLSERVER_18456](/sql/relational-databases/errors-events/mssqlserver-18456-database-engine-error)</li><li>[MSSQLSERVER_18452](/sql/relational-databases/errors-events/mssqlserver-18452-database-engine-error)</li></ul></li></ul> |
...
## <a name="files-size-limit-beyond-which-restore-happens-to-default-path"></a>超出的还原文件的大小限制为默认路径会发生的情况

上的文件数，但也在其名称和路径，不仅取决于文件的总字符串大小。 对于每个数据库文件，获取逻辑文件名称和物理路径。
可以使用下面提供的 SQL 查询：
...
如果上面给出的内容的字符串大小超过 20,000 个字节，数据库文件的存储方式，并在恢复期间你将不能设置为还原的目标文件路径。 文件将还原到 SQL Server 提供的默认 SQL 路径。

### <a name="override-the-default-target-restore-file-path"></a>重写默认目标还原文件路径

可以通过将一个 JSON 文件，其中包含映射到目标还原路径的数据库文件还原操作期间覆盖目标还原文件路径。 为此创建一个文件`database_name.json`并将其放在位置*C:\Program Files\Azure 工作负荷 Backup\bin\plugins\SQL*。
...
 
在上面的内容就可以使用下面提供的 SQL 查询的数据库文件的逻辑名称：
