...
若要安装代理，请**在 DPM 服务器**上已提升权限的 Azure PowerShell 控制台中运行以下命令：
...
代理将显示在已安装程序列表中。若要查看已安装的程序列表，请转到“控制台”>“程序”>“程序和功能”。
...
| /m | 选择启用 Microsoft Update | - | | /nu | 安装完成后不要检查更新 | - | 
...
### 注册到 Azure 备份服务
在可注册 Azure 备份服务之前，需要确保符合[先决条件](backup-azure-dpm-introduction)。你必须：
...
为了将备份转移到 Azure（DPM 将此称为联机备份），可将保留范围配置为[使用祖父-父-子方案 (GFS) 的长期保留](backup-azure-backup-cloud-as-tape)。也就是说，你可以定义组合保留策略，其中包括每日、每周、每月和每年保留策略。在此示例中，我们将创建一个用于表示所需复杂保留配置的数组，然后使用 [Set-DPMPolicyObjective](https://technet.microsoft.com/zh-cn/library/hh881762) cmdlet 配置保留范围。
...
## 后续步骤
有关适用于 DPM 的 Azure 备份的详细信息，请参阅 [Azure DPM 备份简介](backup-azure-dpm-introduction)
