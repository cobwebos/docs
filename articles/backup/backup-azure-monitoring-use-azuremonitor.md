...
打开日志分析工作区的**日志**部分，并为您自己的日志创建查询。 选择“新建警报规则”时，将打开 Azure Monitor 警报创建页，如下图所示。****
...
- 在所有解决方案中，其他信息（如备份项、策略、恢复点、存储等）每天至少推送*一次。*
...
- **定义拟合**：计划的备份活动与活动日志的最新定义不相符。 相反，它与[资源日志](https://docs.microsoft.com/azure/azure-monitor/platform/resource-logs-collect-workspace#what-you-can-do-with-platform-logs-in-a-workspace)对齐。 当通过活动日志通道传送数据发生变化时，这种相符性会导致意外的影响。
- **活动日志通道的问题**：在恢复服务保管库中，从 Azure 备份中泵送的活动日志遵循新模型。 遗憾的是，此项更改会影响 Azure 政府、Azure 德国和 Azure 中国世纪互联的活动日志生成。 如果这些云服务的用户在 Azure Monitor 中基于活动日志创建或配置了任何警报，将不会触发警报。 此外，在所有 Azure 公共区域，如果用户[将恢复服务活动日志收集到 Log Analytics 工作区中](https://docs.microsoft.com/azure/azure-monitor/platform/collect-activity-logs)，这些日志不会显示。
