...
6. 在 **"备份项目"** 窗格中，您可以查看受保护的 VM 列表。 在此示例中，保管库保护着一台虚拟机：demobackup。  

    ![查看"备份项目"窗格](./media/backup-azure-manage-vms/backup-items-blade-select-item.png)

7. 从保管库项的仪表板中，修改备份策略，运行按需备份，停止或恢复 VM 保护，删除备份数据，查看还原点，以及运行还原。

    !["备份项目"仪表板和"设置"窗格](./media/backup-azure-manage-vms/item-dashboard-settings.png)
...
3. 在 **"备份项目"** 窗格中，您可以查看受保护 VM 的列表以及具有最新还原点时间的最后一个备份状态。
...
2. 从“备份管理类型”**** 下，选择“Azure 虚拟机”****。 将显示 **"备份项目（Azure 虚拟机）"** 窗格。
3. 选择一个 VM，选择“立即备份”**** 来创建一个按需备份。 将显示"**立即备份"** 窗格。
...
如果在停止 VM 保护期间选择了["停止保护"并保留备份数据](#stop-protection-and-retain-backup-data)选项，则可以使用 **"恢复备份**"。 如果选择了“[停止保护并删除备份数据](#stop-protection-and-delete-backup-data)”选项或“[删除备份数据](#delete-backup-data)”，则此选项不可用。
...
为了保护数据，Azure 备份包括软删除功能。 使用软删除，即使在删除 VM 的备份（所有恢复点）后，备份数据也会保留 14 天。 有关详细信息，请参阅[软删除文档](https://docs.microsoft.com/azure/backup/backup-azure-security-feature-cloud)。
...
* 如果为 Azure 备份配置的 Azure VM 被删除或移动而不停止保护，则计划备份作业和按需（临时）备份作业都将因错误 UserErrorVmNotFoundV2 而失败。 备份预检查将仅对失败的按需备份作业（未显示失败的计划作业）显示为关键。
* 这些备份项在系统中保持活动状态，并遵守用户设置的备份和保留策略。 这些 Azure VM 的备份数据将根据保留策略保留。 过期的恢复点（最后一个恢复点除外）将根据备份策略中设置的保留范围进行清理。
* 如果用户不再需要删除资源的备份项/数据，因为最后一个恢复点会永久保留，并且用户按适用的备份定价收费，则建议用户删除主数据源不再存在的备份项目，以避免任何额外费用。
