...
- **备份本地计算机**：
    ...
    - 可以将本地计算机备份到备份服务器（System Center Data Protection Manager (DPM) 或 Microsoft Azure 备份服务器 (MABS)），然后将备份服务器备份到 Azure 中的 Azure 备份恢复服务保管库。
- **备份 Azure VM**：
    - 可以直接备份 Azure VM。 Azure 备份为在 VM 上运行的 Azure VM 代理安装一个备份扩展来执行此操作。 这将备份整个 VM。
    - 可以通过运行 MARS 代理来备份 Azure VM 上的特定文件和文件夹。
    - 可以将 Azure VM 备份到在 Azure 中运行的 MABS，然后将 MABS 备份到保管库。
...
**代理** | **详细信息** 
--- | --- 
**Microsoft Azure 恢复服务 (MARS) 代理** | 1) 在独立的本地 Windows 服务器上运行，可以备份文件、文件夹和系统状态<br/><br/> 2) 在 Azure VM 上运行，可以备份文件、文件夹和系统状态。<br/><br/>  3) 在 DPM/MABS 服务器上运行，可以将 DPM/MABS 本地存储磁盘备份到 Azure。 
...
**备份类型** | **详细信息** | **使用情况**
--- | --- | ---
**完整** | 备份包含整个数据源。 完整备份会占用更多的网络带宽。 | 用于初始备份。
...
**备份类型** | **详细信息** | **使用情况**
--- | --- | ---
**完整备份** | 完整数据库备份将备份整个数据库。 它包含特定数据库或者文件组或文件集中的所有数据，以及足以用于恢复数据的日志。 | 每天最多可以触发一次完整备份。<br/><br/> 可以选择每天或每周创建完整备份。
...
请注意，Azure VM 需要能够访问 Internet 才能执行控制命令。 如果是备份 VM 中的工作负荷（例如 SQL Server 备份），则备份数据也需要能够访问 Internet。 

![备份 Azure VM](./media/backup-architecture/architecture-azure-vm.png)

## <a name="architecture-direct-backup-of-on-premises-windows-machinesazure-vm-filesfolders"></a>体系结构：直接备份本地 Windows 计算机/Azure VM 文件/文件夹
...
2. 为 DPM/MABS 中的计算机或应用设置保护时，请选择备份到 MABS/DPM 本地磁盘进行短期存储，并备份到 Azure（联机保护）。 此外，可以指定何时要运行备份到本地 DPM/MABS 存储的操作，以及何时要运行联机备份到 Azure 的操作。
...
- 详细了解 [Windows](../virtual-machines/windows/managed-disks-overview.md) 和 [Linux](../virtual-machines/linux/managed-disks-overview.md) VM 的磁盘存储。
- 了解可用的[磁盘类型](../virtual-machines/windows/disks-types.md)，例如标准和高级。
