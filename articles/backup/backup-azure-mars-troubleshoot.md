...
- [检查另一个进程或防病毒软件是否干扰 Azure 备份](https://docs.microsoft.com/azure/backup/backup-azure-troubleshoot-slow-backup-performance-issue#cause-another-process-or-antivirus-software-interfering-with-azure-backup)。
...
**错误消息**：提供的无效保管库凭据。 该文件已损坏，或者没有与恢复服务关联的最新凭据。 (ID: 34513)

| 原因 | 建议的操作 |
| ---     | ---    |
| **保管库凭据无效** <br/> <br/> 保管库凭据文件可能已损坏或过期。 （例如，它们可能是在注册时的 48 以前下载的。）| 请从 Azure 门户上的恢复服务保管库下载新凭据。 （请参阅下载 MARS[代理部分中](https://docs.microsoft.com/azure/backup/install-mars-agent#download-the-mars-agent)的步骤 6。然后根据需要执行以下步骤： <ul><li> 如果已安装并注册 MARS，请打开 Microsoft Azure 备份代理 MMC 控制台，然后在“操作”窗格中选择“注册服务器”，以使用新凭据完成注册。******** <br/> <li> 如果新的安装失败，请尝试使用新凭据重新安装。</ul> **注意**：如果下载了多个保管库凭据文件，则只有最新的文件在接下来的 48 小时内有效。 我们建议下载新的保管库凭据文件。
| **代理服务器/防火墙正在阻止注册** <br/>或 <br/>**未建立 Internet 连接** <br/><br/> 如果计算机或代理服务器限制了 Internet 连接，并且你无法确保能够访问所需的 URL，则注册将会失败。| 请执行以下步骤：<br/> <ul><li> 与 IT 团队协作，确保系统已建立 Internet 连接。<li> 如果没有代理服务器，请确保在注册代理时不要选择代理选项。 [检查代理设置](#verifying-proxy-settings-for-windows)。<li> 如果你使用了防火墙/代理服务器，请与网络团队协作，确保这些 URL 和 IP 地址能够访问：<br/> <br> **Url**<br> `www.msftncsi.com` <br> .Microsoft.com <br> .WindowsAzure.com <br> .microsoftonline.com <br> .windows.net <br>**IP 地址**<br>  20.190.128.0/18 <br>  40.126.0.0/18 <br/></ul></ul>完成上述故障排除步骤后，再次尝试注册。<br></br> 如果要连接是通过 Azure ExpressRoute 进行的，请确保设置已按照[Azure ExpressRoute 支持](backup-support-matrix-mars-agent.md#azure-expressroute-support)中所述进行配置。
...
- 确保联机备份状态设置为“启用”。**** 若要验证状态，请执行以下步骤：
  ...
  1. 双击**Microsoft 在线备份**并转到 **"触发器"** 选项卡。
  1. 检查状态是否设置为“已启用”。**** 如果不是，请依次选择“编辑”、“已启用”、“确定”。************

- 确保为运行任务选择的用户帐户是服务器上的**SYSTEM**或**本地管理员的组**。 若要验证用户帐户，请转到“常规”选项卡并检查“安全性”选项。********
...
3. 转到**设备管理器** > **存储控制器**并找到微软**iSCSI 的发送器**。 如果找到，请直接转到步骤 7。

4. 如果找不到 Microsoft iSCSI 的开机器服务，请尝试在名为"未知设备 **"的设备管理器** > **存储控制器**下查找一个条目，该控制器名为 **"未知设备**"，硬件 ID **ROOT_ISCSIPRT**。
...
7. 转到**任务管理器** > **服务（本地）** > **微软 iSCSI 的分机服务**：
...
如果缓存文件夹（也称为临时文件夹）配置不正确、缺少先决条件或具有受限访问权限，则备份操作可能会失败。

### <a name="prerequisites"></a>先决条件

对于 MARS 代理操作的成功，缓存文件夹需要遵守以下要求：

- [确保划痕文件夹位置有 5% 到 10% 的可用卷空间](backup-azure-file-folder-backup-faq.md#whats-the-minimum-size-requirement-for-the-cache-folder)
- [确保临时文件夹位置有效且可访问](backup-azure-file-folder-backup-faq.md#how-to-check-if-scratch-folder-is-valid-and-accessible)
- [确保支持缓存文件夹上的文件属性](backup-azure-file-folder-backup-faq.md#are-there-any-attributes-of-the-cache-folder-that-arent-supported)
- [确保分配的卷影副本存储空间足以进行备份过程](#increase-shadow-copy-storage)
...
如果保护数据源所需的卷影副本存储空间不足，则备份操作可能会失败。 要解决此问题，请使用 vssadmin 增加受保护卷上的卷影副本存储空间，如下所示：

- 从提升的命令提示符检查当前卷影存储空间：<br/>
...
- scratch 文件夹。 其默认位置为 C：\程序文件\微软 Azure 恢复服务代理\划痕
- C：\程序文件\微软 Azure 恢复服务代理\Bin 处的 bin 文件夹
- CBengine.exe
- CSC.exe

## <a name="common-issues"></a>常见问题

本节介绍您在使用 MARS 代理时遇到的常见错误。

### <a name="salchecksumstoreinitializationfailed"></a>SalChecksumStoreInitializationFailed

错误消息 | 建议的操作 |
-- | --
Microsoft Azure 恢复服务代理无法访问存储在暂存位置的备份校验和 | 要解决此问题，请执行以下操作并重新启动服务器 <br/> - [检查是否有防病毒或其他进程锁定临时位置文件](#another-process-or-antivirus-software-blocking-access-to-cache-folder)<br/> - [检查临时位置是否有效且可供 mars 代理访问。](backup-azure-file-folder-backup-faq.md#how-to-check-if-scratch-folder-is-valid-and-accessible)

### <a name="salvhdinitializationerror"></a>SalVhdInitializationError

错误消息 | 建议的操作 |
-- | --
Microsoft Azure 恢复服务代理无法访问暂存位置，因此无法初始化 VHD | 要解决此问题，请执行以下操作并重新启动服务器 <br/> - [检查是否有防病毒或其他进程锁定临时位置文件](#another-process-or-antivirus-software-blocking-access-to-cache-folder)<br/> - [检查临时位置是否有效且可供 mars 代理访问。](backup-azure-file-folder-backup-faq.md#how-to-check-if-scratch-folder-is-valid-and-accessible)

### <a name="sallowdiskspace"></a>萨尔洛磁盘空间

错误消息 | 建议的操作 |
-- | --
由于临时文件夹所在的卷中存储空间不足，备份失败 | 要解决此问题，请验证以下步骤并重试操作：<br/>- [确保 MARS 代理是最新的](https://go.microsoft.com/fwlink/?linkid=229525&clcid=0x409)<br/> - [验证和解决影响备份临时空间的存储问题](#prerequisites)

### <a name="salbitmaperror"></a>SalBitmapError

错误消息 | 建议的操作 |
-- | --
找不到文件中的更改。 这可能是由于各种原因。 请重试该操作 | 要解决此问题，请验证以下步骤并重试操作：<br/> - [确保 MARS 代理是最新的](https://go.microsoft.com/fwlink/?linkid=229525&clcid=0x409) <br/> - [验证和解决影响备份临时空间的存储问题](#prerequisites)

## <a name="next-steps"></a>后续步骤

- 获取有关[如何使用 Azure 备份代理备份 Windows 服务器](tutorial-backup-windows-server-to-azure.md)的更多详细信息。
