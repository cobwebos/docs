...
1. 如果发生备份的计算机仍可用，请在 MARS 中使用[将数据恢复到同一计算机](backup-azure-restore-windows-server.md#recover-data-to-the-same-machine)，从所有旧的恢复点恢复。
2. 如果上面提到的计算机不可用，则使用[恢复到备用计算机](backup-azure-restore-windows-server.md#recover-to-an-alternate-machine)，使用另一台 MARS 计算机获取此数据。
