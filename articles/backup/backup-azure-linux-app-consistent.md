...
5. 根据下述信息配置 VMSnapshotScriptPluginConfig.json：****
    ...
    - **超时In秒**：为预脚本和后脚本指定单个超时（最大值可以是 1800）。
    ...
    - **脚本执行时间秒**：设置扩展在每个轮询到脚本执行之间的睡眠时间。 例如，如果值为 2，则扩展会每 2 秒检查一次前/后脚本执行是否完成。 它可以采用的最小值和最大值分别为 1 和 5。 该值应严格为一个整数。
...
[将 VM 备份配置为恢复服务保管库](https://docs.microsoft.com/azure/backup/backup-azure-arm-vms)
