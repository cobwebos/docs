...
5. 根据下述信息配置 VMSnapshotScriptPluginConfig.json：
    ...
    - **timeoutInSeconds**：指定操作前脚本和操作后脚本的单个超时值（最大值可以为1800）。
    ...
    - **ScriptsExecutionPollTimeSeconds**：设置扩展在每次轮询到脚本执行之间必须休眠的时间。 例如，如果值为2，则扩展会检查预处理/后脚本是否每隔2秒完成。 它可以采用的最小值和最大值分别为1和5。 该值应严格为一个整数。
