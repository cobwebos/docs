...
description: 使用 Azure 备份服务备份 SQL Server 数据库的简介
ms.topic: conceptual
ms.date: 01/30/2019
...
ms.locfileid: "77505932"
---
# <a name="back-up-sql-server-to-azure-as-a-dpm-workload"></a>将 SQL Server 备份到 Azure 作为 DPM 工作负荷

本文将引导您完成使用 Azure 备份备份 SQL Server 数据库的配置步骤。

要将 SQL Server 数据库备份到 Azure，需要一个 Azure 帐户。 如果没有，只需几分钟即可创建免费帐户。 有关详细信息，请参阅创建[Azure 免费帐户](https://azure.microsoft.com/pricing/free-trial/)。

要将 SQL Server 数据库备份到 Azure 并从 Azure 恢复它，需要执行：
...
开始之前，请确保满足使用 Azure 备份保护工作负荷[的先决条件](backup-azure-dpm-introduction.md#prerequisites-and-limitations)。 下面是一些先决条件任务： 
* 创建备份保管库。
* 下载保管库凭据。 
* 安装 Azure 备份代理。
* 将服务器注册到保管库。
...
1. 展开要备份的数据库所在的 SQL Server 计算机。 您将看到可以从该服务器备份的数据源。 展开**所有 SQL 共享**，然后选择要备份的数据库。 在此示例中，我们选择报表服务器$MSDPM2012 和报表服务器$MSDPM2012TempDB。 然后选择 **"下一步**"。
...
1. 选择“下一步”。 DPM 显示可用的总体存储空间。 它还显示了潜在的磁盘空间利用率。

    ![设置磁盘分配](./media/backup-azure-backup-sql/pg-storage.png)

    默认情况下，DPM 每个数据源（SQL Server 数据库）创建一个卷。 该卷用于初始备份副本。 在此配置中，逻辑磁盘管理器 （LDM） 将 DPM 保护限制为 300 个数据源（SQL Server 数据库）。 若要避免此限制，请选择“在 DPM 存储池中共置数据”。**** 如果使用此选项，DPM 会对多个数据源使用单个卷。 此设置允许 DPM 保护多达 2，000 个 SQL Server 数据库。

    如果选择 **"自动增长卷**"，则 DPM 可以随着生产数据的增长考虑增加的备份量。 如果不选择 **"自动增长卷**"，则 DPM 将备份存储限制为保护组中的数据源。

1. 如果您是管理员，您可以选择**通过网络自动**传输此初始备份，并选择传输时间。 或者选择**手动**传输备份。 然后选择 **"下一步**"。

    ![选择副本创建方法](./media/backup-azure-backup-sql/pg-manual.png)

    初始备份副本需要传输整个数据源（SQL Server 数据库）。 备份数据从生产服务器（SQL Server 计算机）移动到 DPM 服务器。 如果此备份较大，则通过网络传输数据可能会导致带宽拥塞。 因此，管理员可以选择使用可移动媒体**手动**传输初始备份。 或者，他们可以在指定时间**通过网络自动**传输数据。
...
1. 选择何时运行一致性检查。 然后选择 **"下一步**"。
    ...
    DPM 可以对备份点的完整性运行一致性检查。 它计算生产服务器上的备份文件的校验和（本示例中的 SQL Server 计算机）和 DPM 中该文件的备份数据。 如果检查发现冲突，则假定 DPM 中的备份文件已损坏。 DPM 通过发送对应于校验和不匹配的块来修复备份的数据。 由于一致性检查是一项性能密集型操作，因此管理员可以选择计划一致性检查或自动运行一致性检查。
...
1. 如果您是管理员，则可以选择适合组织策略的备份计划和保留策略。
    ...
    >
    > 使用 DPM 在本地磁盘备份完成后安排 Azure 备份。 遵循这种做法后，最新的磁盘备份将复制到 Azure。
    >
