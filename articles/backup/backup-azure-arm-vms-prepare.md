...
备份作业详细信息中的 2 个阶段，包括每个 VM 备份**快照**阶段后跟**将数据传输到保管库**阶段。<br/>
快照阶段可保证恢复点存储在一起的磁盘的可用性**即时还原**和可用于根据用户配置的快照保留 5 天的最大值。 传输数据到保管库以进行长期保留保管库中创建一个恢复点。 将数据传输到保管库仅启动后完成的快照阶段。

  ![备份作业状态](./media/backup-azure-arm-vms-prepare/backup-job-status.png)

有两个**的子任务**由后端，另一个用于前端的备份作业可以从检查运行**备份作业**详细信息边栏选项卡中与给定的相同：

  ![备份作业状态](./media/backup-azure-arm-vms-prepare/backup-job-phase.png)

**将数据传输到保管库**阶段可能需要好几天才能完成具体取决于磁盘的大小，每个磁盘和若干其他因素变化。
...
现在借助此功能，对于相同的 VM 中，两个备份可以并行运行，但在任一阶段 （快照，传输数据到保管库） 中只有一个子任务可以运行。 在方案处于进行中的备份作业导致第二天的备份失败会避免使用与此分离功能。 具有快照其后一天的备份时已完成**将数据传输到保管库**跳过早一天的备份作业是否处于正在进行状态。
在保管库中创建的增量恢复点将捕获在保管库中创建的最后一个恢复点从所有改动。 用户没有任何成本影响。
