...
2. 在“设置”边栏选项卡上，使用垂直滚动条向下滚动到“管理”部分。 单击“备份基础结构”以打开其边栏选项卡。 在“常规”部分中，单击“备份配置”打开其边栏选项卡。 在“备份配置”边栏选项卡中，选择保管库的存储复制选项。 默认情况下，保管库具有异地冗余存储。 如果更改存储复制类型，请单击“保存”。
    ...
    ![备份保管库列表](./media/backup-azure-arm-vms-prepare/full-blade.png)
     ...
     如果你使用 Azure 作为主要备份存储终结点，则继续使用异地冗余存储。 如果使用 Azure 作为非主要备份存储终结点，则选择本地冗余存储。 请参阅 [Azure 存储复制概述](../storage/storage-redundancy.md)部分，深入了解[异地冗余](../storage/storage-redundancy.md#geo-redundant-storage)和[本地冗余](../storage/storage-redundancy.md#locally-redundant-storage)存储选项。
...
| **操作** | **Windows** | **Linux** |
| --- | --- | --- |
| 安装 VM 代理 |下载并安装 [代理 MSI](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409)。 你需要有管理员权限才能完成安装。 |<li> 安装最新的 [Linux 代理](../virtual-machines/linux/agent-user-guide.md)。 你需要有管理员权限才能完成安装。 我们建议从分发存储库安装代理。 我们**不建议**直接从 github 安装 Linux VM 代理。  |
