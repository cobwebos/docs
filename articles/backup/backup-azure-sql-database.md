...
5. 在“备份目标”菜单中，选择“配置备份”。
    ...
    Azure 备份服务会显示包含独立数据库的所有 SQL Server 实例，以及 SQL Server Always On 可用性组。 若要查看 SQL Server 实例中的独立数据库，请选择实例名称旁边的 V 形图标。 同样，选择 Always On 可用性组旁边的 V 形图标查看数据库列表。 下图是独立实例和 Always On 可用性组的示例。
...
在“还原配置”菜单中，“服务器”下拉列表框仅显示已注册到恢复服务保管库的 SQL Server 实例。 如果所需的服务器不在该列表中，请参阅[发现 SQL Server 数据库](backup-azure-sql-database.md#discover-sql-server-databases)来查找该服务器。 在发现过程中，新服务器会注册到恢复服务保管库。<br>
若要还原 SQL DB，需要有以下权限：

* 针对恢复服务**保管库**（在其中进行还原）的**备份操作员**权限。
* 对**源 SQL VM**（已备份的 VM，将尝试从其中进行还原）的**参与者（写入）** 访问权限。
* 对目标 SQL VM（要将内容还原到其中的 VM；在使用原始位置恢复 (OLR) 时，与源 VM 是同一 VM）的**参与者（写入）** 访问权限。
