...
答 10.将停止任何当前配置的备份。你将需要向备份保管库注册该服务器，并且该服务器将被恢复服务视为新的服务器，因此，在注册后发生的第一个备份操作将是在备份中包含的所有数据的完整备份，而不是只备份自上次备份后发生的更改。但是，如果你需要执行某一恢复操作，则可以使用“从其他服务器恢复”恢复选项来恢复已备份的数据。
...
A2.是的，Azure 备份支持每天通过 Windows 服务器/客户端计划 3 个备份副本，每天通过 SCDPM 计划 2 个备份副本，每天针对 IaaS VM 进行 1 次备份。
...
A5.是的，Azure 备份保留结构允许你根据自己的要求十分灵活地定义保留策略。
...
A10.否。我们已经去除了对恢复点的限制。你可以根据需要创建任意数量的恢复点。

**问 11.为什么在备份中传输的数据量与我备份的数据量不相等？**<br/>
A11.备份的所有数据在传输之前都会经过压缩和加密。根据所备份的数据类型，你应该会获得 30-40% 压缩率。
