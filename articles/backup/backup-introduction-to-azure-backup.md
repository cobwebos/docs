...
**数据加密** - 该服务允许在公有云中安全地传输和存储数据。加密密码将存储在本地，绝不会传输或存储到 Azure 中。如有必要还原任何数据，只需具有加密密码或密钥即可。
...
Azure 备份保护本地和云端的数据。Azure Site Recovery 就虚拟机和物理服务器的复制、故障转移和故障恢复进行协调。这两个服务都很重要，因为灾难恢复解决方案需要让数据保持安全且可恢复（备份），同时在服务中断时使工作负荷保持可用 (Site Recovery)。
