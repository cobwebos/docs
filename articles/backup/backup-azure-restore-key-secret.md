...
* **备份加密的 VM** - 已使用 Azure 备份备份加密的 Azure VM。 有关如何备份已加密 Azure VM 的详细信息，请参阅[使用 PowerShell 管理 Azure VM 的备份和还原](backup-azure-vms-automation.md)一文。
...
使用先前生成的 JSON 文件来获取密钥名称和值，并将其他提供给设置密钥 cmdlet，以便将机密 (BEK) 放回密钥保管库。如果您的**VM 使用 BEK 和 KEK 进行加密，** 请使用这些 cmdlet。
...
将密钥和机密还原回密钥保管库后，请参阅[使用 PowerShell 管理 Azure VM 的备份和恢复](backup-azure-vms-automation.md#create-a-vm-from-restored-disks)文章，从还原的磁盘、密钥和机密创建加密 VM。
