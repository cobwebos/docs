...
### <a name="selecting-the-relevant-azure-vm"></a>选择相关的 Azure VM
 ...
 可以通过在订阅下[列出所有可保护项](https://docs.microsoft.com/rest/api/backup/backupprotectableitems/list)来确认“缓存”已完成，并在响应中找到所需的 VM。 [此操作的响应](#example-responses-1)还提供有关恢复服务如何标识 VM 的信息。  熟悉此模式后，可以跳过此步骤直接进入[启用保护](#enabling-protection-for-the-azure-vm)。
...
#### <a name="responses"></a><a name="responses-1"></a>反应
...
若更改受保护 VM 的策略，可以使用与[启用保护](#enabling-protection-for-the-azure-vm)相同的格式。 只需在[请求正文](#example-request-body)中提供新的策略 ID 并提交请求即可。 例如：要将 testVM 的策略从"默认策略"更改为"ProdPolicy"，请在请求正文中提供"ProdPolicy"ID。
...
#### <a name="responses"></a><a name="responses-2"></a>反应
...
> [!IMPORTANT]
> 为了防止意外删除方案，有一个[软删除功能可用于](use-restapi-update-vault-properties.md#soft-delete-state)恢复服务保管库。 如果保管库的软删除状态设置为启用，则删除操作不会立即删除数据。 它将保存14天，然后永久清除。 在这 14 天的期间，客户不收取存储费用。 要撤消删除操作，请参阅[撤消删除部分](#undo-the-stop-protection-and-delete-data)。

### <a name="undo-the-stop-protection-and-delete-data"></a>撤消停止保护并删除数据

撤消意外删除类似于创建备份项。 撤消删除后，将保留该项目，但不会触发将来的备份。

撤消删除是*PUT*操作，与[更改策略](#changing-the-policy-of-protection)和/或[启用保护](#enabling-protection-for-the-azure-vm)非常相似。 只需提供在[请求正文](#example-request-body)中用变量*rehydrate*来撤消删除的意图并提交请求。 例如：要撤消 testVM 的删除，应使用以下请求正文。
