...
description: 在本文中，了解如何使用 Azure 备份服务将 SAP HANA 数据库备份到 Azure 虚拟机。
ms.topic: conceptual
ms.date: 11/12/2019
...
ms.locfileid: "79248055"
---
# <a name="back-up-sap-hana-databases-in-azure-vms"></a>备份 Azure VM 中的 SAP HANA 数据库

SAP HANA 数据库是需要低恢复点目标 （RPO） 和长期保留的关键工作负载。 可以使用[Azure 备份](backup-overview.md)备份来备份在 Azure 虚拟机 （VM） 上运行的 SAP HANA 数据库。

本文演示如何将 Azure VM 上运行的 SAP HANA 数据库备份到 Azure 备份恢复服务保管库。
...
请参阅[先决条件](tutorial-backup-sap-hana-db.md#prerequisites)和[预注册脚本执行](tutorial-backup-sap-hana-db.md#what-the-pre-registration-script-does)哪些部分来设置数据库进行备份。
...
2. 单击“启动发现”****。 这会开始在保管库区域中发现未受保护的 Linux VM。
   ...
   * 发现后，未受保护的 VM 将显示在门户中，按名称和资源组列出。
   * 如果 VM 未按预期列出，请检查 VM 是否已在保管库中备份。
   * 多个 VM 可以具有相同的名称，但它们属于不同的资源组。

3. 在“选择虚拟机”中，单击脚本下载链接。此脚本可为 Azure 备份服务提供访问 SAP HANA VM 的权限以进行数据库发现****。
4. 在托管要备份的 SAP HANA 数据库的每个 VM 上运行脚本。
5. 在 VM 上运行脚本后，在 **"选择虚拟机"** 中，选择 VM。 然后单击“发现数据库”****。
...
## <a name="run-sap-hana-studio-backup-on-a-database-with-azure-backup-enabled"></a>在启用 Azure 备份的数据库上运行 SAP HANA 工作室备份

如果要获取使用 Azure 备份的数据库的本地备份（使用 HANA Studio），请执行以下操作：
...
* 了解如何[管理使用 Azure 备份备份的 SAP HANA 数据库](https://docs.microsoft.com/azure/backup/sap-hana-db-manage)
