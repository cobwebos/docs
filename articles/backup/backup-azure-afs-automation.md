...
3. 指定保管库存储使用的冗余类型。
   ...
   * 以下示例针对 **testvault** 设置为 [GeoRedundant](https://docs.microsoft.com/powershell/module/az.recoveryservices/set-azrecoveryservicesbackupproperty) 的 **Set-AzRecoveryServicesBackupProperties** 命令设置 **-BackupStorageRedundancy** 选项。
...
可以使用按需备份将快照保留 10 年。 计划程序可用于运行可选择保留期的按需 PowerShell 脚本，从而每周、每月或每年定期拍摄快照。 在拍摄常规快照时，请参阅使用 Azure 备份进行按[需备份的限制](https://docs.microsoft.com/azure/backup/backup-azure-files-faq#how-many-on-demand-backups-can-i-take-per-file-share)。
