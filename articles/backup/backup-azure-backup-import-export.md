<properties
   pageTitle="Azure 备份 — 使用 Azure 导入/导出服务进行离线备份或初始种子设定 | Azure"
...
5. 外部 SATA 驱动器编写器和外部 3.5 英寸 SATA 驱动器。只支持对 3.5 英寸 SATA II/III 硬盘驱动器使用导入/导出服务。不支持大于 8TB 的硬盘驱动器。你可以使用 SATA II/III USB 适配器在外部将 SATA II/III 磁盘连接到大多数计算机。检查的最新的一套驱动器支持的服务的 Azure 导入/导出文档。
...
2. 完成工作流，然后在 Azure 备份 mmc 中选择“立即备份”，以启动脱机备份副本。初始备份写入到临时区域作为此步骤的一部分。
    ...
    通过单击“保护组”并选择“创建恢复点”选项启用 SCDPM 中的相应工作流。随后选择“在线保护”选项。
...
7. 运送磁盘，然后输入要跟踪的发货状态的跟踪号。磁盘抵达数据中心后，将其复制到存储帐户并更新状态。

    ![完成状态](./media/backup-azure-backup-import-export/complete.png)
