...
> [!NOTE]
> 配置诊断后，初次数据推送完成可能需要长达24小时。 数据开始流向 LA 工作区后，您可能无法立即看到报表中的数据，因为当前部分日期的数据不会显示在报表中（[此处](https://docs.microsoft.com/azure/backup/configure-reports#conventions-used-in-backup-reports)提供了更多详细信息）。 因此，建议在将保管库配置为将数据发送到 Log Analytics 后，开始查看报表2天。
...
> [!NOTE]
> * 当前，报表的初始加载最多可能需要1分钟。
> * 恢复服务保管库只是备份报表的入口点。 在从保管库的边栏选项卡中打开 "备份报表" 工作簿后，可以看到在所有保管库中聚合的数据（通过选择一组相应的 LA 工作区）。
