...
在可注册 Azure 备份服务之前，需要确保符合[先决条件](backup-configure-vault.md)。 必须：

* 具备有效的 Azure 订阅
* 有一个备份保管库

要下载保管库凭据，请在 Azure PowerShell 控制台中运行 **Get-AzureRMBackupVaultCredentials** cmdlet，并将其存储在方便的位置，例如 *C:\Downloads\*。
...
结合适当的参数运行 [Get-OBRecoverableItem](https://technet.microsoft.com/library/hh770399.aspx) cmdlet 可以检索备份点列表。 在本示例中，我们将选择源卷 *D:* 的最新备份点，并使用它来恢复特定的文件。
