...
| LogBackupRetentionDuration      | 十进制数 | Azure VM 备份中的 SQL 日志备份的保留期 |
| MonthlyRetentionDaysOfTheMonth  | 文本           | 配置每月保留时的当月周次。  例如，First、Last 等。 |
...
| MonthlyRetentionWeeksOfTheMonth | 文本           | 配置每月保留时的当月周次。   例如，First、Last 等。 |
...
| WeeklyRetentionDuration         | 十进制数 | 已配置备份的每周保留持续时间（周） |
...
- [了解如何将诊断数据发送到 Log Analytics](https://aka.ms/AA6il6r)