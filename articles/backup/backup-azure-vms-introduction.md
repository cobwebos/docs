...
**准备备份** | 注意准备备份所需的时间。 备份准备时间包括安装或更新备份扩展的时间，以及按照备份计划触发快照的时间。
**数据传输** | 考虑 Azure 备份识别上一备份中的增量更改所需的时间。<br/><br/> 在增量备份中，Azure 备份通过计算块的校验和来确定更改。 如果更改了某个块，则会对其进行标记以传输到保管库。 该服务将分析已确定的块，以尝试进一步减少要传输的数据量。 评估所有已更改的块后，Azure 备份会将更改传输到保管库。<br/><br/> 创建快照之后，可能要经过一段滞后时间才会将它复制到保管库。 在高峰时段，将快照传输到保管库可能需要长达8个小时。 对于每日备份，VM 的备份时间小于 24 小时。
**初始备份** | 尽管增量备份的总备份时间少于 24 小时，但第一次备份可能并非如此。 初始备份所需的时间将取决于数据的大小以及处理备份的时间。
...
- 修改策略中设置的默认计划时间。 例如，如果策略中的默认时间是晚上 12:00，请将该时间递增几分钟，确保以最佳方式使用资源。
...
- 对于使用高级存储的 Vm 的备份，使用 "即时还原"，我们建议分配总分配存储空间的*50%* 可用空间（**仅限**第一次备份需要）。 第一次备份完成后，备份就不再需要 50% 的可用空间
