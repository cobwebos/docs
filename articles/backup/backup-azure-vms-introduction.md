...
- 从常规用途 v1 存储层（快照）还原将在几分钟内完成，因为快照位于同一个存储帐户中。 从常规用途 v2 存储层（保管库）还原可能需要数小时。 在常规用途 v1 存储中提供数据的情况下，我们建议你使用 "[即时还原](backup-instant-restore-capability.md)" 功能进行更快速的还原。 （如果必须从保管库还原数据，将需要更多时间。）
...
例如，采用 A2 标准大小的 VM，其中包含两个附加的数据磁盘，每个数据磁盘最大大小为 4 TB。 下表显示了每个磁盘上存储的实际数据：
