...
当 Azure 备份服务在计划的时间启动备份作业时，它将触发进行时间点快照拍摄所需的备份扩展。 对于 Windows VM，创建快照时，将借助卷影复制服务 (VSS) 来获取虚拟机中磁盘的一致性快照，不必关闭该虚拟机。 对于 Linux VM，客户可以编写自己的自定义脚本，这些脚本将在创建 VM 快照时调用以确保一致性，如本文后面部分所述。 
...
Azure 备份提供了一个框架，让客户可在 VM 备份的过程中通过执行自定义前脚本和后脚本灵活地控制其备份工作流和环境，从而确保应用程序一致性。 前脚本将在创建 VM 快照之前调用，后脚本将在 VM 快照完成后调用。 有关更多详细信息，请参阅[使用前脚本和后脚本的应用程序一致性 Linux VM 备份](https://docs.microsoft.com/azure/backup/backup-azure-linux-app-consistent)。 
> [!NOTE]
> Azure 备份仅调用客户编写的前脚本和后脚本，客户控制的这些脚本需要用来确保应用程序一致性。 如果前脚本和后脚本成功执行，Azure 备份会将恢复点标记为应用程序一致。 
>
...
| 一致性 | 基于 VSS | 解释和详细信息 |
| --- | --- | --- |
| 应用程序一致性 |是（对于 Windows）|应用程序一致性非常适合工作负荷，因为它可确保：<ol><li> VM *启动*。 <li>无数据损坏。 <li>无数据丢失。<li> 对于使用数据的应用程序，数据将保持一致，因为备份时会使用 VSS 或前/后脚本将应用程序纳入考虑。</ol> <li>*Windows VM* - 大多数 Microsoft 工作负荷都有 VSS 写入器，负责执行与数据一致性相关的工作负荷特定操作。 例如，Microsoft SQL Server 的 VSS 编写器可确保正确写入事务日志文件和数据库。 对于 Azure Windows VM 备份，若要创建应用程序一致恢复点，备份扩展必须调用 VSS 工作流且需在创建 VM 快照前完成调用。 若要确保 Azure VM 快照准确性，则也必须完成所有 Azure VM 应用程序的 VSS 编写器。 （了解 [VSS 基本信息](http://blogs.technet.com/b/josebda/archive/2007/10/10/the-basics-of-the-volume-shadow-copy-service-vss.aspx)，并深入了解其[工作原理](https://technet.microsoft.com/library/cc785914%28v=ws.10%29.aspx)详细信息）。 </li> <li> *Linux VM* - 客户可以执行[自定义前脚本和后脚本，以确保应用程序一致性](https://docs.microsoft.com/azure/backup/backup-azure-linux-app-consistent)。 </li> |
...
备份过程会尝试尽快完成备份作业。 为此，这将使用尽可能多的资源。 但是，*单个 Blob 的目标吞吐量*实施 I/O 操作总次数限制为每秒 60 MB。 为了最大限度加快速度，备份过程会尝试并行备份每个 VM 磁盘。 如果 VM 有 4 个磁盘，则服务会尝试并行备份所有 4 个磁盘。 决定从客户存储帐户传出备份流量的最重要因素是备份的**磁盘数**。
