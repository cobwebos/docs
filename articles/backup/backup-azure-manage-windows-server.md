...
ms.locfileid: "88263320"
---
# <a name="monitor-and-manage-recovery-services-vaults"></a>监视和管理恢复服务保管库

本文介绍如何使用恢复服务保管库的“概述”仪表板来监视和管理恢复服务保管库。 从列表中打开恢复服务保管库时，将打开所选保管库的“概述”仪表板。 仪表板提供有关该保管库的各种详细信息。 其中的磁贴显示：严重和警告性警报的状态、正在进行的和失败的备份作业，以及本地冗余存储 (LRS) 和异地冗余存储 (GRS) 使用量。 如果将 Azure VM 备份到保管库，则[“备份预检查状态”磁贴将显示所有严重或警告项](#backup-pre-check-status)。 下图是 **Contoso-vault** 的“概述”仪表板。 “备份项”磁贴显示有九个项已注册到保管库。
...
* Azure VM 的预检查状态。 有关预检查状态的完整信息，请参阅[备份预检查状态](#backup-pre-check-status)。
...
* **已通过**：此状态表示 VM 的配置可确保备份成功，无需采取纠正措施。
* **警告**：此状态表示 VM 的配置中存在一个或多个可能会导致备份失败的问题。 它提供可确保备份成功的建议步骤。 例如，未安装最新的 VM 代理可能会导致备份间歇性失败。 这种情况会提供警告状态。
* **严重**：此状态表示 VM 的配置中存在一个或多个会导致备份失败的严重问题，并提供可确保备份成功的必需步骤。 例如，对 VM 的 NSG 规则进行更新时导致的网络问题会导致备份失败，因为它会阻止 VM 与 Azure 备份服务通信。 这种情况会提供严重状态。
...
| 警告 | 当出现以下情况时，你将收到警告警报：备份作业完成但出现警告，例如，由于损坏问题而未备份超过100个文件，或者在) 成功备份大于1000000的文件时。 |
| 信息性 | 目前未使用信息性警报。 |
...
虚拟机列表包含有用的数据：关联的资源组、以前的[备份预检查](#backup-pre-check-status)、上次备份状态，以及最近还原点的日期。 单击最后一列中的省略号会打开用于触发常见任务的菜单。 列中提供的有用数据根据备份类型的不同而异。

![打开常见任务的省略号菜单](./media/backup-azure-manage-windows-server/ellipsis-menu.png)
...
![备份作业列表](./media/backup-azure-manage-windows-server/backup-jobs-list.png)
...
* 若要了解有关 Azure 备份的详细信息，请参阅 [Azure 备份概述](./backup-overview.md)
