...
备份预检查检查 VM 的配置中，有无可能对备份产生不利影响的问题。 它们聚合此信息，以便您可以直接从恢复服务保管库仪表板查看这些信息，并提供纠正措施建议，以确保成功进行文件一致性或应用程序一致性备份。 它们不需要基础设施，也没有额外费用。  

备份预检查作为 Azure VM 计划备份操作的一部分运行。 它们最后有以下状态之一：

* **已通过**：此状态指示 VM 的配置应导致成功的备份，并且无需采取任何纠正措施。
* **警告**： 此状态指示 VM 配置中的一个或多个可能导致备份*might*失败的问题。 它提供了确保成功备份*的建议*步骤。 例如，未安装最新的 VM 代理可能会导致备份间歇性失败。 这种情况将提供警告状态。
* **严重**：此状态指示 VM 配置中的一个或多个关键问题，*这些问题将导致*备份失败，并提供确保成功备份*的必要*步骤。 例如，由于 VM 的 NSG 规则更新而导致的网络问题将导致备份失败，因为它会阻止 VM 与 Azure 备份服务通信。 这种情况将提供一个严重状态。

按照以下步骤开始解决备份预检查在恢复服务保管库中的 VM 备份报告的任何问题。

* 在恢复服务保管库仪表板上选择**备份预检查状态 （Azure VM）** 磁贴。
* 选择备份预检查状态**为"严重**"或"**警告**"的任何 VM。 此操作将打开**VM 详细信息**窗格。
* 选择窗格顶部的窗格通知，以显示配置问题说明和补救措施。
...
虚拟机列表包含有用的数据：关联的资源组、以前的[备份预检查](https://docs.microsoft.com/azure/backup/backup-azure-manage-windows-server#backup-pre-check-status)、上次备份状态，以及最近还原点的日期。 单击最后一列中的省略号会打开用于触发常见任务的菜单。 列中提供的有用数据根据备份类型的不同而异。
