...
**到保管库的备份** | **Azure VM：** 一天一次<br/><br/>**受 DPM/MABS 保护的机器：** 一天两次<br/><br/> **使用 MARS 代理直接备份的计算机：** 一天三次
**在保管库之间备份** | 备份在一个区域内进行。<br/><br/> 在包含想要备份的 VM 的每个 Azure 区域中都需要有一个保管库。 无法备份到其他区域。
**移动保管库** | 您可以在订阅之间或同一订阅中的资源组之间[移动保管库](https://docs.microsoft.com/azure/backup/backup-azure-move-recovery-services-vault)。 但是，不支持跨区域移动保管库。
...
Azure 备份不支持根据 Azure VM 备份的夏令时时自动调整时钟。 它不会向前或向后移动备份小时。 为确保备份在所需时间运行，请根据需要手动修改备份策略。

## <a name="disk-deduplication-support"></a>磁盘重复数据删除支持

磁盘重复数据删除支持如下：

- 当您使用 DPM 或 MABS 备份运行 Windows 的超 V VM 时，磁盘重复数据消除在本地支持。 Windows Server 会在以备份存储形式附加到 VM 的虚拟硬盘 (VHD) 上执行主机级别的重复数据删除。
...
- 在后端，Azure 备份使用[Azure 存储服务加密](../storage/common/storage-service-encryption.md)，以保护静态数据。
...
Azure 备份添加了跨区域还原功能，以增强数据可用性和恢复能力，使客户完全控制将数据还原到辅助区域。 要配置此功能，请访问["设置跨区域还原"一文。](backup-create-rs-vault.md#set-cross-region-restore) 以下管理类型支持此功能：

| 备份管理类型 | 支持                                                    | 支持的区域 |
| ---------------------- | ------------------------------------------------------------ | ----------------- |
| Azure VM               | 是的。   支持用于磁盘小于 4 TB 的加密 VM 和 VM | 所有 Azure 公共区域。  |
| MARS 代理/内部 | 否                                                           | 空值               |
| SQL /SAP HANA          | 否                                                           | 空值               |
| Afs                    | 否                                                           | 空值               |
