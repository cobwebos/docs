...
Folder | 缓存文件夹应在删除了重复数据的卷上进行加密，或者在压缩的文件夹中进行加密，即稀疏文件或具有重新分析点的文件夹。
...
可以在本地计算机和 Azure Vm 上运行的某些操作系统上，使用 MARS 代理直接备份到 Azure。 操作系统必须是64位，并且应该运行最新的服务包和更新。 下表总结了这些操作系统：
...
受 BitLocker 保护的卷| 不支持 |在开始备份之前，必须解锁卷。
文件系统标识| 不支持 |仅支持 NTFS。
