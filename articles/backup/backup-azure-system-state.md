...
6. 在“资源组”部分：
   ...
   有关资源组的完整信息，请参阅 [Azure 资源管理器概述](../azure-resource-manager/management/overview.md)。
...
3. 从 "**要备份的内容"** 菜单中，选择 "**系统状态**"，然后单击 **"确定"** 。
...
2. 在恢复服务代理中单击“ **立即备份** ”，以通过网络完成初始种子设定。
    ...
    ![Windows Server 备份](./media/backup-try-azure-backup-in-10-mins/backup-now.png)
...
5. 单击“**关闭**”以关闭向导。 如果在备份过程完成之前关闭向导，向导将继续在后台运行。
    > [!NOTE]
    > MARS 代理会在每次系统状态备份之前触发 SFC/verifyonly 作为预检查的一部分。 这是为了确保在系统状态中备份的文件具有与 Windows 版本相对应的正确版本。 在[本文](https://docs.microsoft.com/windows-server/administration/windows-commands/sfc)中了解有关系统文件检查器（SFC）的详细信息。
    >
