...
ms.locfileid: "56431021"
---
# <a name="support-matrix-for-backup-with-microsoft-azure-backup-serversystem-center-dpm"></a>使用 Microsoft Azure 备份服务器/System Center DPM 进行备份时的支持矩阵

可以使用 [Azure 备份服务](backup-overview.md)备份本地计算机和工作负荷以及 Azure VM。 本文汇总了使用 Microsoft Azure 备份服务器 (MABS) 或 System Center Data Protection Manager (DPM) 和 Azure 备份服务备份计算机时的支持设置与限制。

## <a name="about-mabsdpm"></a>关于 MABS/DPM
...
Microsoft Azure 备份服务器 (MABS) 是可用于备份本地物理服务器、虚拟机 (VM) 及其上运行的应用以及 Azure VM 的服务器产品。

- MABS 基于 System Center Data Protection Manager (DPM)，两者提供类似的功能，但也存在几项差别：
    - 无需 System Center 许可证即可运行 MABS。
    - Azure 为 MABS 和 DPM 提供长期备份存储。 此外，DPM 允许在磁带上备份数据，以作长期存储。 MABS 不提供此功能。
- 从 [Microsoft 下载中心](https://www.microsoft.com/en-us/download/details.aspx?id=57520)下载 MABS 服务器。 它可以在本地运行，或者在 Azure 中的 Azure VM 上运行。
...
## <a name="mabsdpm-backup"></a>MABS/DPM 备份

使用 MABS/DPM 和 Azure 备份进行备份的工作原理如下：

1. 在要备份的每台计算机上安装 DPM/MABS 保护代理。 
2. 将计算机/应用备份到 DPM/MABS 上的本地存储。
...
**方案** | **代理** | **位置**
--- | --- | ---
**备份本地计算机/工作负荷** | DPM/MABS 保护代理在要备份的计算机上运行<br/><br/> MABS/DPM 服务器上的 MARS 代理 | MABS/DPM 必须在本地运行
**Azure VM/工作负荷的备份** | 受保护计算机上的 DPM/MABS 保护代理<br/><br/> MABS/DPM 上的 MARS 代理 | DPM/MABS 必须在 Azure VM 上运行。
...
**部署为 Azure VM** | 保护 Azure VM，以及这些 VM 上运行的工作负荷。 | Azure 中运行的 MABS/DPM 无法备份本地计算机。


## <a name="supported-mabs-and-dpm-operating-systems"></a>支持的 MABS 和 DPM 操作系统

Azure 备份可以备份运行以下任何操作系统的 MABS/DPM。 操作系统应该运行最新的服务包和更新。
...
**本地 MABS** | 支持的 64 位操作系统：<br/><br/> - MABS v3 和更高版本：Windows Server 2019（Standard、Datacenter、Essentials） <br/><br/> MABS v2 和更高版本：Windows Server 2016（Standard、Datacenter、Essentials）<br/><br/> 所有 MABS 版本：Windows Server 2012 R2/2012（Standard、Datacenter、Foundation）<br/><br/>所有 MABS 版本：Windows Storage Server 2012 R2/2012 (Standard/Workgroup)
...
**问题** | **详细信息**
--- | ---
**安装** | 应在单一用途计算机上安装 DPM/MABS。<br/><br/> 不要在域控制器、装有应用程序服务器角色的计算机、运行 Exchange Server 或 System Center Operations Manager 的计算机或群集节点上安装 DPM/MABS。<br/><br/> [查看](https://docs.microsoft.com/system-center/dpm/prepare-environment-for-dpm?view=sc-dpm-1807#dpm-server)所有 DPM 系统要求。
**域** | DPM 服务器/MABS 应加入域。 首先安装 DPM/MABS，然后将其加入域。 部署后，不支持将 DPM/MABS 移到新域。
...
组件 | **详细信息**
--- | --- 
**Azure Stack VM 上的 MABS** | 至少使用 A2 大小。 我们建议从 Azure 市场中提供的 Windows Server 2012 R2 或 Windows Server 2016 映像着手。<br/><br/> 不应在 MABS VM 上安装其他任何组件。
**MABS 存储** | 对 MAB VM 使用单独的存储帐户。 MABS 上运行的 MARS 代理需要使用临时存储作为缓存位置，以及保存从云中还原的数据。
**MABS 存储池** | MABS 存储池的大小取决于附加到 MABS VM 的磁盘数目和大小。 每个 Azure Stack VM 大小具有最大磁盘数。 例如，A2 包含 4 个磁盘。
**MABS 保留** | 不应在本地 MABS 磁盘上将备份数据保留 5 天以上。
**MABS 纵向扩展** | 若要纵向扩展部署，可以增加 MABS VM 的大小。 例如，从 A 系列增大到 D 系列。<br/><br/> 还可以确保通过定期备份到 Azure 来卸载数据，如有必要，可以部署更多的 MABS 服务器。
...
**支持的备份** | 要备份的 VM 支持以下操作系统：<br/><br/> - Windows Server Semi-Annual Channel (Datacenter/Enterprise/Standard)<br/><br/> Windows Server 2016/2012 R2/2012/2008 R2。
**Azure Stack VM 的 SQL 支持** | 备份 SQL Server 2016/2014/2012 SP1。<br/><br/> 备份和恢复数据库。
**Azure Stack VM 的 SharePoint 支持** | SharePoint 2016/2013/2010。<br/><br/> 备份和恢复场、数据库、前端与 Web 服务器。
**备份的 VM 的网络要求** | Azure Stack 工作负荷中的所有 VM 必须在同一个 VNet 中，并属于同一个订阅。
...
**备份** | **版本** | **MABS** | **详细信息**
--- | --- | --- | ---
**Windows 10、8.1、8、7**<br/><br/>（32/64 位） | MABS v3、v2、v1 | 本地 | 卷/共享/文件夹/文件<br/><br/> 支持已删除重复数据的卷<br/><br/> 卷必须至少为 1 GB，且采用 NTFS。
...
**Windows Server 2012 R2（Datacenter 和 Standard）**<br/><br/> 64/32 位 | MABS v3、v2、v1 | 本地/Azure VM。 | **本地保护**：卷/共享/文件夹/文件；系统状态/裸机<br/><br/> **Azure VMprotection**：卷/共享/文件夹/文件<br/><br/> 支持已删除重复数据的卷。
**Windows Server 2012 SP1（Datacenter 和 Standard）**<br/><br/> 64/32 位 | MABS v3、v2、v1<br/><br/> 必须安装 [Windows Management Framework 4.0](https://www.microsoft.com/download/details.aspx?id=40855)。 | 本地/Azure VM。 | **本地保护**：卷/共享/文件夹/文件；系统状态/裸机<br/><br/> **Azure VM 保护**：卷/共享/文件夹/文件<br/><br/> 支持已删除重复数据的卷。
**Windows Server 2012（Datacenter 和 Standard）**<br/><br/> 64/32 位 | MABS v1 | 本地/Azure VM。 | **本地保护**：卷/共享/文件夹/文件；系统状态/裸机<br/><br/> **Azure VM 保护**：卷/共享/文件夹/文件<br/><br/> 支持已删除重复数据的卷。
**Windows 2008 R2 SP1（Standard 和 Enterprise）**<br/><br/> 64/32 位 | MABS v3、v2、v1 支持。<br/><br/> 必须安装 [Windows Management Framework 4.0](https://www.microsoft.com/download/details.aspx?id=40855)。 | 本地/Azure VM。 |   **本地保护**：卷/共享/文件夹/文件；系统状态/裸机<br/><br/> **Azure VM 保护**：卷/共享/文件夹/文件<br/><br/> 支持已删除重复数据的卷。
**Windows 2008 R2（Standard 和 Enterprise）**<br/><br/> 64/32 位 | MABS v1。 对于 MABS v2/v3，OS 必须运行 SP1。 | 本地/Azure VM。 | **本地保护**：卷/共享/文件夹/文件；系统状态/裸机<br/><br/> **Azure VM 保护**：卷/共享/文件夹/文件<br/><br/> 支持已删除重复数据的卷。
**Windows Server 2008 SP2**<br/><br/> 64/32 位 | MABS v1、v2、v3 | 如果将 MABS 部署为本地物理机或 Hyper-V VM，则仅支持 v1。<br/><br/> 如果将 MABS 部署为 VMware VM，则支持 MABS v1、v2、v3。<br/><br/> 不支持 Azure VM 上运行的 MABS | 卷/共享/文件夹/文件；系统状态/裸机。
**Windows Storage Server 2008** | MABS v1、v2、v3 | 用作本地物理服务器/Hyper-V VM 的 MABS | 不支持 Azure VM 上运行的 MABS。 | 卷/共享/文件夹/文件；系统状态/裸机。
...
**SQL Server 2014**<br/><br/> **SQL Server 2012/SP1/SP2**<br/><br/> **SQL Server 2008 R2**<br/><br/> **SQL Server 2008** | MABS v3、v2、v1 | 本地/Azure VM。| 备份 SQL Server 数据库。<br/><br/> 支持 SQL Server 群集备份。<br/><br/>不支持 CSV 中存储的数据库。
**Exchange 2016**<br/><br/> **Exchange 2013**<br/><br/> **Exchange 2010** | MABS v3、v2、v1 | 本地。 | 备份独立 Exchange 服务器和 DAG 下的数据库<br/><br/> 恢复邮箱和 DAG 下的邮箱数据库。<br/><br/> 不支持 ReFS。<br/><br/> 备份非共享的磁盘群集。<br/><br/> 备份配置为连续复制的 Exchange 服务器。
**SharePoint 2016**<br/><br/> **Sharepoint 2013**<br/><br/> **SharePoint 2010** | MABS v3、v2、v1 | 本地/Azure VM | 备份场和前端 Web 服务器。<br/><br/> 恢复场、数据库、Web 应用、文件或列表项、SharePoint 搜索和前端 Web 服务器。<br/><br/> 无法使用 AlwaysOn 备份内容数据库的场。
**Windows Server 2016 上的 Hyper-V**<br/><br/> **Windows Server 2012 R2/2012** (Datacenter/Standard)<br/><br/> **Windows Server 2008 R2 SP1** | MABS v3、v2、v1 | 本地 | **Hyper-V 主机上的 MABS 代理**：备份整个 VM 和主机数据文件。 备份使用本地存储的 VM、群集中使用 CSV 存储的 VM，以及使用 SMB 文件服务器存储的 VM。<br/><br/> **来宾 VM 上的 MABS 代理**：备份 VM 上运行的工作负荷。 CSV。<br/><br/> **恢复**：VM、VHD/卷/文件夹/文件的项级恢复。<br/><br/> **Linux VM**：当 Hyper-V 在 Windows Server 2012 R2 和更高版本上运行时进行备份。 Linux VM 的恢复是针对整个计算机进行的。
**VMware VM：vCenter/vSphere ESXi 5.5/6.0/6.5** | MABS v3、v2、v1。<br/><br/> MABS v1 需要更新汇总 1 | 本地 | 在 CSV、NFS 和 SAN 存储中备份 VMware VM。<br/><br/> 恢复整个 VM。<br/><br/> Windows/Linux 备份。<br/><br/> 文件夹/文件的项级恢复（仅适用于 Windows VM）。<br/><br/> 不支持 VMware vApp。<br/><br/> Linux VM 的恢复是针对整个计算机进行的。
...
**备份** | **DPM** | **详细信息**
--- | --- | --- 
**Windows 10、8.1、8、7**<br/><br/>（32/64 位） | 仅限本地。<br/><br/> 使用 DPM 2012 R2 备份 Windows 10 时，我们建议安装 [Update 11](https://support.microsoft.com/help/3209592/update-rollup-12-for-system-center-2012-r2-data-protection-manager)。 | 卷/共享/文件夹/文件<br/><br/> 支持已删除重复数据的卷<br/><br/> 卷必须至少为 1 GB，且采用 NTFS。
...
**Windows Server 2012（Datacenter 和 Standard）**<br/><br/> 64/32 位 |  本地/Azure VM。 | **本地保护**：卷/共享/文件夹/文件；系统状态/裸机<br/><br/> **Azure VM 保护**：卷/共享/文件夹/文件<br/><br/> DPM 2012 R2 和更高版本支持已删除重复数据的卷。
...
**SharePoint 2016**<br/><br/> **Sharepoint 2013**<br/><br/> **SharePoint 2010** | DPM 2016 和更高版本上的 SharePoint 2016。<br/><br/>本地/Azure VM | 备份场和前端 Web 服务器。<br/><br/> 恢复场、数据库、Web 应用、文件或列表项、SharePoint 搜索和前端 Web 服务器。<br/><br/> 无法使用 AlwaysOn 备份内容数据库的场。
...
**VMware VM：vCenter/vSphere ESXi 5.5/6.0/6.5** | MABS v3、v2、v1。<br/><br/> DPM 2012 R2 需要 System Center 更新汇总 1 | 本地 | 在 CSV、NFS 和 SAN 存储中备份 VMware VM。<br/><br/> 恢复整个 VM。<br/><br/> Windows/Linux 备份。<br/><br/> 文件夹/文件的项级恢复（仅适用于 Windows VM）。<br/><br/> 不支持 VMware vApp。<br/><br/> Linux VM 的恢复是针对整个计算机进行的。
...
- [设置](https://docs.microsoft.com/system-center/dpm/install-dpm?view=sc-dpm-180
