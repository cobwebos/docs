...
# 备份 Azure 虚拟机
本文提供了备份现有 Azure 虚拟机的步骤，方便你按照公司的备份和保留策略来保护它们。

首先，在备份 Azure 虚拟机之前，需要注意一些事项。如果你尚未这样做，请完成[先决条件](backup-azure-vms-prepare.md)部分，对环境进行 VM 备份准备，然后再继续。

如果你正在寻找有关[在 Azure 中计划 VM 备份基础结构](backup-azure-vms-introduction.md)或 [Azure 虚拟机](https://azure.microsoft.com/documentation/services/virtual-machines/)的信息，请按照这些链接来查找该信息。

备份 Azure 虚拟机的过程包括三个主要步骤：

![备份 Azure 虚拟机的三个步骤](./media/backup-azure-vms/3-steps-for-backup.png)

>[AZURE.NOTE]虚拟机备份位于本地。你不能将一个区域的虚拟机备份到另一个区域的备份保管库。因此，对于每个需要备份 VM 的 Azure 区域，需要在该区域中至少创建 1 个备份保管库。
...
5. 选择要与备份关联的“保留期范围”。
    ...
    保留策略指定备份存储的时间长度，你可以根据进行备份的时间指定不同的保留策略。例如，在每个季末创建的备份点可能需要保留更长的时间（以便进行审核），而每天创建的备份点（用作操作恢复点）只需保留 90 天。
...
## 查看备份状态和详细信息
虚拟机受保护后，“仪表板”页摘要中的虚拟机计数也会递增。“仪表板”页还显示过去 24 小时内成功、失败和仍在进行的作业数目。单击任何一个类别可在“任务”页中深入查看该类别。
...
## 排查错误
如果你在备份虚拟机时遇到问题，请参阅此[疑难解答](backup-azure-vms-troubleshoot.md)指南以获取帮助。
