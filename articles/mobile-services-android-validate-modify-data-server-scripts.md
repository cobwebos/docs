<properties linkid="develop-mobile-tutorials-validate-modify-and-augment-data-android" urlDisplayName="Validate Data - Android" pageTitle="使用服务器脚本验证和修改数据 (Android) | 移动开发人员中心" metaKeywords="" description="了解如何从 Android 应用程序验证和修改使用服务器脚本发送的数据。" metaCanonical="" services="" documentationCenter="Mobile" title="Validate and modify data in Mobile Services by using server scripts" authors="ricksal" solutions="" manager="" editor="" />
...
4. 将现有脚本替换为以下函数，然后单击"保存"。
	...
	<b>注意</b>
	<p>在"脚本"选项卡中，依次单击"清除"和"保存"可以删除某个已注册的脚本<strong></strong><strong></strong><strong></strong>。</p></div>
...
1. 在[管理门户]中的"脚本"选项卡上，将当前的 Insert 脚本替换为以下函数，然后单击"保存"。
    ...
    <div class="dev-callout"><b>注意</b>
	<p>首次运行此 insert 脚本时，必须启用动态架构。启用动态架构后，移动服务在首次执行时会自动将 <strong>createdAt</strong> 列添加到 <strong>TodoItem</strong> 表。默认情况下，将为新的移动服务启用动态架构，发布应用程序之前应该禁用动态架构。</p>
    </div>
...
1. 在 Package Explorer 中打开 ToDoItem.java 文件，然后添加以下 **import** 语句：
...
2. 将以下代码添加到 **TodoItem** 类中的 private 字段定义：
    ...
    <div class="dev-callout"><b>注意</b>
	<p>批注 <code>SerializedName</code> 告知客户端要将应用程序中的新 <code>mCreatedAt</code> 属性映射到 TodoItem 表中定义的具有不同名称的 <code>createdAt</code> 列。如果你使用此批注，应用程序在对象上使用的属性名称可以不同于 SQL Database 中的列名称。如果不使用此批注，则会因大小写不同而发生错误。</p>
    </div>

2. 将以下方法添加到 ToDoItem 类，以获取并设置新的 mCreatedAt 属性：
...
5. 在 Package Explorer 中打开 ToDoItemAdapter.java 文件，然后添加以下 **import** 语句：

		import java.text.DateFormat;

6. 在 GetView 方法中添加以下代码：
...
7. 找到代码 `checkBox.setText(currentItem.getText());` 并将此代码行替换为以下代码：
...
7. 将现有的 **RefreshItemsFromTable** 方法替换为以下代码：
...
[使用脚本为用户授权]: /zh-cn/documentation/articles/mobile-services-android-authorize-users-in-scripts
