...
1. 选择 "**添加度量值**" 按钮可查看在同一图表中绘制多个度量值。
1. 选择 " **+ 新建图表**" 按钮，在一个视图中查看多个图表。
...
**指标** | **单位** | **聚合** | **指标描述**
|---|---|---|---|
连续导出已导出记录的数目    | 计数 | SUM | 从群集导出的记录总数。 |
连续导出最大迟到分钟数 |    计数   | 最多   | 导出记录的最大分钟数（分钟）。|
连续导出挂起计数 | 计数 | 最多   | 挂起导出操作的最大值。
连续导出结果    | 计数 |   计数   | 按结果连续导出操作的总数。 指标包含连续导出名称和数据库。 
导出利用率 |    百分比 | 最多   | 用于导出操作的已定义槽的使用情况。
| | | | |
...
**指标** | **单位** | **聚合** | **指标描述**
|---|---|---|---|
| 处理的事件数（适用于事件/IoT 中心） | 计数 | Max、Min、Sum | 从事件中心读取并由群集处理的事件总数。 事件被拆分为已拒绝的事件和群集引擎接受的事件。 |
| 引入延迟 | 秒数 | Avg、Max、Min | 数据引入的延迟时间，从在群集中接收数据到查询准备就绪。 引入延迟时间取决于引入方案。 |
| 引入结果 | 计数 | 计数 | 失败和成功的引入操作的总数。 使用 "**应用拆分**" 创建成功和失败结果的存储桶，并分析维度（**值** > **状态**）。|
| 引入卷（MB） | 计数 | Max、Sum | 压缩前引入到群集的数据的总大小（以 MB 为单位）。 |
| | | | |  
...
**指标** | **单位** | **聚合** | **指标描述**
|---|---|---|---|
| 查询持续时间 | 毫秒 | Avg、Min、Max、Sum | 接收到查询结果之前的总时间（不包括网络延迟）。 |
...
**指标** | **单位** | **聚合** | **指标描述**
|---|---|---|---|
流引入数据速率 |    计数   | RateRequestsPerSecond | 引入到群集的总数据量。 |
流摄取持续时间   | 毫秒  | Avg、Max、Min | 所有流式处理引入请求的总持续时间。 |
流式处理引入请求速率   | 计数 | Count、Avg、Max、Min、Sum | 流式引入请求总数。 |
流式处理引入结果 | 计数 | Avg   | 按结果类型的流式引入请求总数。 |
| | | | |
