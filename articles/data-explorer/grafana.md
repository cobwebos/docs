...
description: 本文介绍如何将 Azure 数据资源管理器设置为 Grafana 的数据源，然后将来自示例群集的数据可视化。
author: orspod
ms.author: orspodek
ms.reviewer: gabil
...
使用以下视频，了解如何使用 Grafana 的 Azure 数据资源管理器插件，将 Azure 数据资源管理器设置为 Grafana 的数据源，然后可视化数据。 
...
1. 在"主页仪表板"中，选择 **"警报** > **通知通道**"以创建新的通知通道
