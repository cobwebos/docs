...
Kafka Connect 是一项工具，用于在 Apache Kafka 和其他系统之间以可伸缩且可靠的方式流式传输数据。 可以通过它简单快速地定义连接器，将大量数据移进和移出 Kafka。 ADX Kafka 接收器充当 Kafka 的连接器。
...
1. 生成并运行应用。 应用将消息发送到 Kafka 群集，并且每十秒显示一次状态。
