...
description: 了解如何从 Azure CLI 管理 Azure Database for MySQL 服务器。
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: how-to
...
如果尚未创建服务器，请参阅此 [快速入门](quickstart-create-mysql-server-database-using-azure-cli.md) 创建一个。

## <a name="scale-compute-and-storage"></a>缩放计算和存储
你可以使用以下命令轻松扩展你的定价层、计算和存储。 你可以查看所有服务器操作你可以执行 [az mysql server 概述](/cli/azure/mysql/server?view=azure-cli-latest)
...
> [!Important]
> - 存储可以向上扩展 (但是，你不能缩小存储) 
> - 不支持从基本版扩展到常规用途或内存优化定价层。 你可以[使用 bash 脚本](https://techcommunity.microsoft.com/t5/azure-database-for-mysql/upgrade-from-basic-to-general-purpose-or-memory-optimized-tiers/ba-p/830404)或[使用 MySQL 工作台](https://techcommunity.microsoft.com/t5/azure-database-support-blog/how-to-scale-up-azure-database-for-mysql-from-basic-tier-to/ba-p/369134)手动增加规模
...
|[az mysql db 删除](/cli/azure/sql/db#az-mysql-db-delete)|```az mysql db delete -g myresourcegroup -s mydemoserver -n mydatabasename```|从服务器中删除数据库。 此命令不会删除你的服务器。 |
...
如果要删除 MySQL 单一服务器，可运行 [az MySQL server delete](/cli/azure/mysql/server#az-mysql-server-delete) 命令。
