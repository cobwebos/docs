...
登录到 [Azure 门户](https://portal.azure.com)。 Create an Azure Database for MySQL server with engine version 5.6, 5.7, or 8.0. 有关详细信息，请参阅[如何在门户中创建 Azure Database for MySQL 服务器](quickstart-create-mysql-server-database-using-azure-portal.md)或[如何使用 CLI 创建 Azure Database for MySQL 服务器](quickstart-create-mysql-server-database-using-azure-cli.md)。

Redirection is currently only supported when SSL is enabled. 有关如何配置 SSL 的详细信息，请参阅[通过 Azure Database for MySQL 使用 SSL](https://docs.microsoft.com/azure/mysql/howto-configure-ssl#step-3-enforcing-ssl-connections-in-azure)。 
