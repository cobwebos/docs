...
创建服务器之后，可以独立地更改 vCore 数、存储量和备份保留期。 创建服务器之后，不能更改定价层或备份存储类型。 vCore 数和备份保留期可以上下缩放。 存储大小只能增加。 可以通过门户或 Azure CLI 缩放资源。 有关使用 Azure CLI 进行缩放的示例，请参阅[使用 Azure CLI 监视和缩放 Azure Database for MySQL 服务器](scripts/sample-scale-server.md)。
