...
配置 MySQL Workbench，以便安全地通过 SSL 连接。 在“设置新连接”对话框上，导航到 MySQL Workbench 上的 **SSL** 选项卡。 在“SSL CA 文件:”字段中输入 **BaltimoreCyberTrustRoot.crt.pem** 的文件位置。
...
在 Azure 门户中，访问 Azure Database for MySQL 服务器，并单击“连接安全性”。 使用切换按钮来启用或禁用“强制实施 SSL 连接”设置。 然后单击“保存”。 Microsoft 建议始终启用“强制实施 SSL 连接”设置，以增强安全性。
...
## <a name="step-4-verify-ssl-connection"></a>步骤 4：验证 SSL 连接
执行 mysql status 命令，验证是否已使用 SSL 连接到 MySQL 服务器：
```dos
mysql> status
```
通过查看输出来确认连接已加密。 它应显示：**SSL: 使用的密码是 AES256-SHA** 
