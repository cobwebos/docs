...
2. 选择将日志输出到使用**日志\_输出**的位置。 若要将日志发送到本地存储并 Azure Monitor 诊断日志，请选择 "**文件**"。 若要仅将日志发送到 Azure Monitor 日志，请选择 "**无**"
...
如果**log_output**配置为 "File"，则可以通过[az mysql server-logs 下载](/cli/azure/mysql/server-logs#az-mysql-server-logs-download)命令从服务器下载单个日志文件。
