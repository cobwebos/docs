...
使用 SSL/TLS 并验证根证书的所有应用程序都需要更新根证书才能连接到 Azure Database for MySQL。 如果当前未使用 SSL/TLS，则不会影响应用程序的可用性。 可以通过 [此处](concepts-ssl-connection-security.md#ssl-default-settings)) 验证客户端应用程序是否尝试使用具有预定义的受信任证书颁发机构的 SSL 模式 (CA。
