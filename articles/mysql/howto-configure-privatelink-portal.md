---
title: 专用链接 - Azure 门户 - MySQL 的 Azure 数据库
description: 了解如何从 Azure 门户为 MySQL 配置 Azure 数据库的专用链接
author: kummanish
ms.author: manishku
ms.service: mysql
ms.topic: conceptual
...
---
# <a name="create-and-manage-private-link-for-azure-database-for-mysql-using-portal"></a>使用门户为 MySQL 创建和管理 Azure 数据库的专用链接

专用终结点是 Azure 中专用链接的构建基块。 它使 Azure 资源（例如虚拟机 (VM)）能够以私密方式来与专用链接资源通信。 在本文中，您将学习如何使用 Azure 门户在 Azure 虚拟网络中创建 VM，以及使用 Azure 专用终结点为 MySQL 服务器创建 Azure 数据库。
...
在本节中，您将创建虚拟网络和子网来承载用于访问专用链接资源的 VM（Azure 中的 MySQL 服务器）。
...
在本节中，您将在 Azure 中为 MySQL 服务器创建 Azure 数据库。 

1. 在 Azure 门户中屏幕的左上角，选择**为 MySQL****创建资源** > **数据库** > Azure 数据库 。

1. 在**MySQL 的 Azure 数据库中**提供了以下信息：
    ...
    | 资源组 | 选择**我的资源组**。 已在上一部分创建此内容。|
    | **服务器详细信息** |  |
    |服务器名称  | 输入*我的服务器*。 如果此名称已被使用，请创建唯一的名称。|
    | 管理员用户名| 输入所选的管理员名称。 |
    | 密码 | 输入所选密码。 密码长度必须至少为 8 个字符，且符合定义的要求。 |
    | 位置 | 选择要希望 MySQL 服务器驻留的 Azure 区域。 |
...
8. 在 **"创建专用终结点 - 配置**"中，输入或选择此信息：
    ...
    |**专用 DNS 集成**||
    |与专用 DNS 区域集成 |选择 **“是”**。 |
    |专用 DNS 区域 |选择 *（新建）私人链接.mysql.数据库.azure.com* |
    |||
...
4. 在 **"新建连接**"中，输入或选择此信息：
    ...
    | 用户名 | 输入 MySQL 服务器创建期间提供的用户名username@servername。 |
...
7. （可选）从 MySQL 服务器创建或查询信息。

8. 关闭远程桌面连接到 myVm。

## <a name="clean-up-resources"></a>清理资源
使用专用终结点 MySQL 服务器和 VM 完成后，请删除资源组及其包含的所有资源：
...
在此执行中，您可以在虚拟网络上创建了 VM、MySQL 的 Azure 数据库以及用于专用访问的专用终结点。 您从 Internet 连接到一个 VM，并使用专用链路安全地连接到 MySQL 服务器。 要了解有关私有终结点的更多内容，请参阅[什么是 Azure 专用终结点](https://docs.microsoft.com/azure/private-link/private-endpoint-overview)。
