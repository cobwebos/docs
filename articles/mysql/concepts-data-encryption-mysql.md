...
> [!NOTE]
> 此功能适用于所有 Azure Database for MySQL 支持 "常规用途" 和 "内存优化" 定价层的 Azure 区域。
...
![显示创建自己的密钥概述的关系图](media/concepts-data-access-and-security-data-encryption/mysqloverview.png)

要使 MySQL 服务器使用 Key Vault 中存储的客户托管密钥进行 DEK 加密，Key Vault 管理员向服务器提供以下访问权限：
...
* Key Vault 和 Azure Database for MySQL 必须属于同一 Azure Active Directory （Azure AD）租户。 不支持跨租户 Key Vault 和服务器交互。 以后移动资源要求您重新配置数据加密。
...
* 使用 get、wrapKey 和 unwrapKey 权限向密钥保管库 Azure Database for MySQL 授予对密钥保管库的访问权限，方法是使用其唯一的托管标识。 在 Azure 门户中，当在 MySQL 上启用数据加密时，将自动创建唯一标识。 有关使用 Azure 门户的详细分步说明，请参阅[配置 MySQL 的数据加密](howto-data-encryption-portal.md)。
...
* 确保 Key Vault 和 Azure Database for MySQL 位于同一区域，以确保更快地访问 DEK wrap 和解包操作。
...
使用存储在 Key Vault 中的客户托管密钥对 Azure Database for MySQL 进行加密后，任何新创建的服务器副本也会加密。 可以通过本地或异地还原操作或通过读取副本建立此新副本。 但是，可以更改副本以反映新客户的加密的托管密钥。 当客户管理的密钥更改时，服务器的旧备份将使用最新密钥开始。
...
了解如何[使用 Azure 门户通过 Azure database For MySQL 的客户托管密钥设置数据加密](howto-data-encryption-portal.md)。
