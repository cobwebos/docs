...
# 如何：配置资产传送策略
[WACOM.INCLUDE [media-services-selector-configure_asset_delivery_policy](../includes/media-services-selector-configure_asset_delivery_policy.md)]


Media Services 内容传送工作流中的步骤之一是为想要流式传输的资产配置传送策略。资产传送策略告知 Media Services 你希望如何传送资产：应该将资产动态打包成哪种流式处理协议（例如 MPEG DASH、HLS、平滑流或全部），是否要动态加密资产以及如何加密（信封或常用加密）。 

本主题介绍为何以及如何创建和配置资产传送策略。 

>[AZURE.NOTE]若要使用动态打包和动态加密，必须确保至少有一个缩放单位（也称为流式处理单位）。有关详细信息，请参阅[如何缩放 Media Services](/zh-cn/documentation/articles/media-services-manage-origins#scale_streaming_endpoints)。 
>
>此外，你的资产必须包含一组多码率 MP4 或多码率平滑流文件。   
...
根据配置资产传送策略的方式，你可以动态打包、动态加密和流式传输以下流式处理协议：平滑流、HLS、MPEG DASH 和 HDS 流。  
...
以下 HTTP 请求将创建一个资产传送策略，该策略指定不要应用动态加密，而使用以下任何协议传送流：MPEG DASH、HLS 和平滑流协议。 

有关创建 AssetDeliveryPolicy 时可以指定的值的信息，请参阅[定义 AssetDeliveryPolicy 时使用的类型](#types) 部分。   
...
在指定 DynamicEnvelopeEncryption 传送策略时，需要确保将你的资产链接到 EnvelopeEncryption 类型的内容密钥。有关详细信息，请参阅：[创建内容密钥](/zh-cn/documentation/articles/media-services-rest-create-contentkey)。
...
指定要在 HTTP 请求正文中获取的 URL 类型。如果要使用 PlayReady 保护内容，请使用 1（表示 keyDeliveryType）请求 Media Services PlayReady 许可证获取 URL：{"keyDeliveryType":1}。如果要使用信封加密保护内容，请为 keyDeliveryType 指定 2，以请求密钥获取 URL：{"keyDeliveryType":2}。
...
参阅[将资产与资产传送策略相链接](#link_asset_with_asset_delivery_policy)

## DynamicCommonEncryption 资产传送策略 

### 创建 CommonEncryption 类型的内容密钥，并将其链接到资产

在指定 DynamicCommonEncryption 传送策略时，需要确保将你的资产链接到 CommonEncryption 类型的内容密钥。有关详细信息，请参阅：[创建内容密钥](/zh-cn/documentation/articles/media-services-rest-create-contentkey))。
...
以下 HTTP 请求将创建 **AssetDeliveryPolicy**，该策略配置为将动态常用加密 (**DynamicCommonEncryption**) 应用到**平滑流式处理**协议（在本示例中，已阻止流式处理其他协议）。 
