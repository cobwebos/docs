...
若要查找这些值，请转到 Azure 门户，选择你的 Media Service 帐户，然后单击门户窗口底部的"管理密钥"图标。单击每个文本框旁边的图标将值复制到系统剪贴板中。


## 创建 CloudMediaContext 实例

若要开始针对 Media Services 编程，你需要创建一个代表服务器上下文的 **CloudMediaContext** 实例。**CloudMediaContext** 包括对各种重要集合的引用，这些集合包括作业、资产、文件、访问策略和定位符。

>[WACOM.NOTE] **CloudMediaContext** 类不是线程安全的。每个线程或每组操作均应创建一个新 CloudMediaContext。
