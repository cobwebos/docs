...
在管理门户中查看监视数据会产生相关的额外费用。有关详细信息，请参阅[存储分析和计费][存储分析和计费]。

## 目录

-   [如何：为存储帐户配置监视][如何：为存储帐户配置监视]
-   [如何：自定义仪表板以进行监视][如何：自定义仪表板以进行监视]
-   [如何：自定义“监视”页][如何：自定义“监视”页]
-   [如何：向度量值表中添加度量值][如何：向度量值表中添加度量值]
-   [如何：在“监视”页上自定义度量值图表][如何：在“监视”页上自定义度量值图表]
-   [如何：配置日志记录][如何：配置日志记录]

## 如何：为存储帐户配置监视

1.  在[管理门户][管理门户]中，单击**“存储”**，然后单击存储帐户名称以打开仪表板。
...
度量值存储在存储帐户中的以下 4 个表中：\$MetricsTransactionsBlob、\$MetricsTransactionsTable、\$MetricsTransactionsQueue 和 \$MetricsCapacityBlob。有关详细信息，请参阅[关于存储分析度量值][关于存储分析度量值]。
...
2.  单击**“监视”**。
    ...
    ![监视\_详细监视视图][监视\_详细监视视图]

    **说明**

    在选择度量值时应考虑成本。在刷新监视视图时会产生相关的事务和数据传出费用。有关详细信息，请参阅[存储分析和计费][存储分析和计费]。
...
诊断日志保存在你的存储帐户下名为 \$logs 的 Blob 容器中。有关访问 \$logs 容器的信息，请参阅[关于存储分析日志记录][关于存储分析日志记录]。
