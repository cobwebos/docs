...
本文将指导你完成使用 PowerShell cmdlet 重置 Azure VPN 网关。这些说明适用于经典部署模型。我们还没有 cmdlet 或 REST API 来为使用资源管理器部署模型创建的虚拟网络重置 VPN 网关。这些都正在开发中。你可以通过在预览门户中查看虚拟网络来确定 VPN 网关是否是使用典型部署模型创建的。使用典型部署模型创建的虚拟网络显示在预览门户的虚拟网络（典型）部分。
...
在两次重新启动之后，如果你仍然遇到跨界连接问题，请从 Azure 门户开具支持票证以联系 Microsoft Azure 支持。
