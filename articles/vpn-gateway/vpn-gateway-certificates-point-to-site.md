---
title: "创建点到站点连接的自签名证书：Azure | Microsoft Docs"
...
3. 在计算机上的“个人”证书存储中创建并安装证书。 以下示例将创建一个相应的 *.cer* 文件，在配置 P2S 时需要将此文件上传到 Azure。 使用想要用于证书的名称替换“ARMP2SRootCert”和“ARMP2SRootCert.cer”。<br><br>该证书将位于“证书”-“当前用户\个人\证书”中。
...
2. 修改并运行示例，生成客户端证书。
    * 将 *"ARMP2SRootCert"* 更改为生成客户端证书所用的自签名根证书。 确保使用的是根证书的名称，即创建根证书时指定的“CN=”值。
    * 将 *ClientCertificateName* 更改为生成客户端证书所用的名称。<br><br>如果未经修改就运行以下示例，个人证书存储中将有一个从根证书 ARMP2SRootCert 生成的客户端证书，名为 ClientCertificateName。
