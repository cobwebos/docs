...
>[AZURE.NOTE] 配置站点到站点连接时，需要为你的 VPN 设备提供面向公众的 IPv4 IP 地址。
...
| Citrix | NetScaler MPX，SDX，VPX |10\.1 及以上 | [集成说明](https://docs.citrix.com/zh-CN/netscaler/11-1/system/cloudbridge-connector-introduction/cloudbridge-connector-azure.html) | 不兼容 |
...
| 阶段 2 安全关联 (SA) 生命周期（吞吐量）| 102,400,000 KB | - | IPsec SA 加密和身份验证产品（按偏好顺序列出）| 1.ESP-AES256 2.ESP-AES128 3.ESP-3DES 4.N/A | 请参阅 *RouteBased 网关 IPsec 安全关联 (SA) 产品*（见下）| | 完全向前保密 (PFS) | 否 | 否 (*) | | 对等体存活检测 | 不支持 | 支持 |
