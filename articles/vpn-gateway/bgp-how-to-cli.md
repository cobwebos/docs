---
title: "在 Azure VPN 网关上配置 BGP：资源管理器：CLI | Microsoft Docs"
description: "本文指导完成使用 Azure 资源管理器和 CLI 通过 Azure VPN 网关配置 BGP。"
...
---
# <a name="how-to-configure-bgp-on-azure-vpn-gateways-using-cli"></a>如何使用 CLI 在 Azure VPN 网关上配置 BGP

本文帮助用户使用资源管理器部署模型和 CLI 在跨界站点到站点 (S2S) VPN 连接和 VNet 到 VNet 连接上启用 BGP。

## <a name="about-bgp"></a>关于 BGP

BGP 是通常在 Internet 上使用的，用于在两个或更多网络之间交换路由和可访问性信息的标准路由协议。 BGP 使 VPN 网关和本地 VPN 设备（称为 BGP 对等节点或邻域）能够交换“路由”。 路由会向这两个网关提供有关前缀可用性和可访问性的信息，以便通过所涉及的网关或路由器。 BGP 还可以通过将 BGP 网关从一个 BGP 对等节点获知的路由传播到所有其他 BGP 对等节点来允许在多个网络之间传输路由。
...
请求公共 IP 地址。 将向为 VNet 创建的 VPN 网关分配公共 IP 地址。
...
为 TestVNet1 创建虚拟网络网关。 BGP 需要基于路由的 VPN 网关。 还需要额外的参数“-Asn”，为 TestVNet1 设置 ASN（AS 编号）。 创建网关可能需要一些时间（45 分钟或更久）。 

如果使用“--no-wait”参数运行该命令，则不会显示任何反馈或输出。 “--no-wait”参数允许在后台创建网关， 但并不意味着 VPN 网关会立即创建完毕。
...
要建立跨界连接，需要创建本地网关来表示本地 VPN 设备，并创建连接将 Azure VPN 网关与本地网关连接在一起。 虽然这些步骤与创建其他连接的步骤类似，但它们包括指定 BGP 配置参数所需的其他属性。
...
* 提醒一下，在本地网络与 Azure VNet 之间必须使用不同的 BGP ASN。 如果它们是相同的，则需要更改 VNet ASN（如果本地 VPN 设备已使用该 ASN 与其他 BGP 邻域对等）。

请确保已完成此练习的[为 VPN 网关启用 BGP](#enablebgp)部分，并且仍与订阅 1 连接，然后再继续。 请注意，在此示例中会创建新的资源组。 另请注意本地网关的两个附加参数：Asn 和 BgpPeerAddress。
...
在此示例中，虚拟网关和本地网关位于不同的资源组中。 网关位于不同的资源组中时，必须指定两个网关的整个资源 ID，以便建立 VNet 之间的连接。
...
在此步骤中，创建从 TestVNet1 到 Site5 的连接。 如前所述，同一 Azure VPN 网关可以同时具有 BGP 连接和非 BGP 连接。 除非在连接属性中启用了 BGP，否则 Azure 不会为此连接启用 BGP，即使已在这两个网关上配置了 BGP 参数，也是如此。 确保将该订阅 ID 替换为自己的订阅 ID。
...
在本示例中，虚拟网络属于同一订阅。 可以在不同订阅之间设置 VNet 到 VNet 连接，有关更多详细信息，请参阅[配置 VNet 到 VNet 连接](vpn-gateway-howto-vnet-vnet-cli.md)。 请确保在创建连接时添加“-EnableBgp $True”，以启用 BGP。
...
为 TestVNet2 创建虚拟网络网关。 必须覆盖 Azure VPN 网关上的默认 ASN。 连接的 VNet 的 ASN 必须不同，才能启用 BGP 和传输路由。
...
在此步骤中，创建从 TestVNet1 到 Site5 的连接。 要为此连接启用 BGP，必须指定“--enable-bgp”参数。

在以下示例中，虚拟网关和本地网关位于不同的资源组中。 网关位于不同的资源组中时，必须指定两个网关的整个资源 ID，以便建立 VNet 之间的连接。 
...
创建从 TestVNet1 到 TestVNet2 的连接，以及从 TestVNet2 到 TestVNet1 的连接。 确保将该订阅 ID 替换为自己的订阅 ID。
...
完成这些步骤后，可在几分钟内建立连接。 完成 VNet 到 VNet 连接后，BGP 对等互连会话即可运行。

如果完成此练习的所有三个部分，可建立网络拓扑，如下图所示：
