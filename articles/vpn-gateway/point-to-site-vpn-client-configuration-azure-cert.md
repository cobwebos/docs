...
VPN 客户端配置文件包含在一个 zip 文件中。 配置文件提供本机 Windows、Mac IKEv2 VPN 或 Linux 客户端通过使用本机 Azure 证书身份验证的点对点连接连接到虚拟网络所需的设置。
...
2. 双击所需的包进行安装。 如果您看到智能屏幕弹出窗口，请单击 **"更多信息**"，然后**无论如何运行**。
3. 在客户端计算机上，导航到**网络设置**，然后单击**VPN**。 VPN 连接显示所连接到的虚拟网络的名称。 
...
* **VpnSettings.xml**，它包含服务器地址和隧道类型等重要设置。 
* **VpnServerRoot.cer**，其中包含在 P2S 连接设置期间验证 Azure VPN 网关所需的根证书。
...
3. 打开 **"网络首选项**"下的 **"网络**"对话框，然后单击 **"+"** 以创建新的 VPN 客户端连接配置文件，用于 P2S 连接到 Azure 虚拟网络。
...
以下说明是在 Ubuntu 18.0.4 上创建的。 Ubuntu 16.0.10 不支持 strongSwan GUI。 如果要使用 Ubuntu 16.0.10，必须使用[命令行](#linuxinstallcli)。 以下示例可能与你看到的屏幕不同，具体取决于所用的 Linux 和 strongSwan 版本。
...
5. 打开下载的客户端配置文件包含的 **Generic** 文件夹中的 **VpnSettings.xml** 文件。 查找名为**VpnServer**的标记并复制名称，以"azure 网关"开头，以".cloudapp.net"结尾。
