<properties 
   pageTitle="使用 Resource Manager 部署模型配置与虚拟网络的点到站点 VPN 连接 | Azure"
   description="通过创建点到站点 VPN 连接安全地连接到 Azure 虚拟网络。"
...
点到站点连接不需要 VPN 设备或面向公众的 IP 地址即可运行。有关点到站点连接的详细信息，请参阅 [VPN 网关常见问题](/documentation/articles/vpn-gateway-vpn-faq/#point-to-site-connections)和[关于跨界连接](/documentation/articles/vpn-gateway-cross-premises-options/)。
...
- 你需要安装 Azure Resource Manager PowerShell cmdlet（1.0.2 或更高版本）。有关安装 PowerShell cmdlet 的详细信息，请参阅[如何安装和配置 Azure PowerShell](/documentation/articles/powershell-install-configure/)。
...
10. 将受信任的证书添加到 Azure。最多可以添加 20 个证书。有关使用 *makecert* 创建自签名根证书的说明，请参阅[为点到站点配置使用自签名根证书](/documentation/articles/vpn-gateway-certificates-point-to-site/)。在将 Base64 编码 X.509 (.cer) 文件添加到 Azure 时，则是在告诉 Azure 信任该文件所代表的根证书。

	若要获取公钥，请将证书导出为 Base64 编码 X.509 (.CER) 文件。请记下导出到.cer 文件的文件路径。以下是获取证书的 Base64 字符串表示形式的一个示例。在此步骤中，需要使用自己的 .cer 文件路径。
...
使用点到站点连接连接到 Azure 的每个客户端必须符合两个要点：必须配置 VPN 客户端以建立连接，并且客户端上必须安装了客户端证书。Windows 客户端有可用的 VPN 客户端配置包。有关详细信息，请参阅 [VPN 网关常见问题](/documentation/articles/vpn-gateway-vpn-faq/#point-to-site-connections)。
...
2. 生成并安装从客户端计算机上的根证书创建的客户端证书 (*.pfx)。可以使用你所习惯的任何安装方法。如果您使用的是自签名根证书但不熟悉如何执行此操作，可以参考[为用点到站点配置使用自签名根证书](/documentation/articles/vpn-gateway-certificates-point-to-site/)。
...
证书用于对点到站点 VPN 的 VPN 客户端进行身份验证。以下步骤将引导你添加和删除根证书。在将 Base64 编码 X.509 (.cer) 文件添加到 Azure 时，则是在告诉 Azure 信任该文件所代表的根证书。

### 添加受信任的根证书

最多可以将 20 个受信任的根证书添加到 Azure。请遵循以下步骤来添加根证书。

1. 创建并准备要添加到 Azure 的新的根证书。
...
可以从 Azure 中删除受信任的根证书。删除某个受信任的证书时，从该证书生成的客户端证书将不能够再通过点到站点连接到 Azure，直到它们安装了在 Azure 中受信任的证书所生成的客户端证书为止。

1. 若要删除受信任的根证书，请修改下面的示例。
