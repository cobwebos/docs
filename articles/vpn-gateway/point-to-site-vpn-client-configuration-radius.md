---
title: Azure VPN 网关：创建&安装 VPN 客户端配置文件 - P2S RADIUS 连接
...
1. [为 P2S 连接设置 Azure VPN 网关](point-to-site-how-to-radius-ps.md)。
2. [设置 RADIUS 服务器进行身份验证](point-to-site-how-to-radius-ps.md#radius)。 
...
运行该命令将返回一个链接。 复制并粘贴到 Web 浏览器的链接以下载**VpnClient配置.zip**。 解压缩该文件，查看以下文件夹： 
...
2. 若要安装该包，请双击它。 如果您看到智能屏幕弹出窗口，请选择 **"更多信息** > **运行**"。
...
7. 在连接的“客户端”部分中，选择“EAP”作为“身份验证”方式，并输入用户名和密码。************ 可能需要选择右侧的锁形图标才能保存此信息。 然后，选择 **"保存**"。
...
1. 选择一个配置包，将其安装在客户端设备上。 对于 64 位处理器体系结构，请选择 **VpnClientSetupAmd64** 安装程序包。 对于 32 位处理器体系结构，请选择**VpnClientSetupX86**安装程序包。 如果您看到智能屏幕弹出窗口，请选择 **"更多信息** > **运行**"。 也可将要安装的包保存在其他客户端计算机上。
2. 每个客户端都需要使用客户端证书来进行身份验证。 安装客户端证书。 有关客户端证书的信息，请参阅[点对点的客户端证书](vpn-gateway-certificates-point-to-site.md)。 若要安装生成的证书，请参阅[在 Windows 客户端上安装证书](point-to-site-how-to-vpn-client-install-azure-cert.md)。
3. 在客户端计算机上，浏览到“网络设置”****，并选择“VPN”****。 VPN 连接显示所连接到的虚拟网络的名称。
...
3. 在“网络首选项”下打开“网络”对话框。******** 选择**+** 该选择可为 P2S 连接到 Azure 虚拟网络的 P2S 连接创建新的 VPN 客户端连接配置文件。
