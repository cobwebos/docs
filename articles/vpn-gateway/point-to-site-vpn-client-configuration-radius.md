...
### <a name="workflow"></a>工作流
  ...
  3. 获取适用于所选身份验证选项的 VPN 客户端配置，用其在 Windows 设备上设置 VPN 客户端。 这样即可通过 P2S 连接连接到 Azure VNet。
...
* 名为“WindowsAmd64”和“WindowsX86”的文件夹。 这些文件夹分别包含 Windows 64 位和 32 位安装程序包。 
* 名为“GenericDevice”的文件夹。 此文件夹包含的常规信息用于创建你自己的 VPN 客户端配置。 请忽略此文件夹。
* 如果已在创建虚拟网关时配置了 IKEv2，则会看到名为“Mac”的文件夹，其中包含名为“mobileconfig”的文件。 该文件用于配置 Mac 客户端。

可以检索已创建的客户端配置文件。 “Get-AzureRmVpnClientConfiguration”cmdlet 返回一个 URL（链接），用于下载 VpnClientConfiguration.zip 文件。 对 P2S VPN 配置（例如 VPN 协议类型或身份验证类型）进行更改时，该配置不会自动更新。 必须运行“New-AzureRmVpnClientConfiguration”cmdlet 来重新创建该配置。
...
### <a name="certfiles"></a>生成 VPN 客户端配置文件
...
* 名为“WindowsAmd64”和“WindowsX86”的文件夹。 这些文件夹分别包含 Windows 64 位和 32 位安装程序包。 
* 名为“GenericDevice”的文件夹。 此文件夹包含的常规信息用于创建你自己的 VPN 客户端配置。
...
### <a name="certmaccli"></a>设置 MAC VPN 客户端
...
## <a name="otherauth"></a>其他身份验证类型或协议
