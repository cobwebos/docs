...
> * [电源外壳](vpn-gateway-howto-point-to-site-rm-ps.md)
...
1. 在[Azure 门户](https://portal.azure.com)菜单或**主页**上，选择 **"创建资源**"。 此时会打开一个“新建”页面****。
...
7. 选择网关“大小”****。 大小为虚拟网关的网关 SKU。 在 Azure 门户中，默认 SKU 为“默认”****。 有关网关 SKU 的详细信息，请参阅[有关 VPN 网关设置](vpn-gateway-about-vpn-gateway-settings.md#gwsku)。
...
要从另一台客户端计算机（而不是用于生成客户端证书的计算机）创建 P2S 连接，请安装客户端证书。 安装客户端证书时，需要使用导出客户端证书时创建的密码。 通常，只需双击证书即可安装。 有关详细信息，请参阅[安装导出的客户端证书](vpn-gateway-certificates-point-to-site.md#install)。
...
6. 在“指纹”页中，将证书指纹以连续文本行的形式进行粘贴，不留空格****。 选择 **"确定"** 以完成。
