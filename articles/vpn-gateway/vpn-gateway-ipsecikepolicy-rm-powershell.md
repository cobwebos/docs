---
title: 用于 S2S VPN 的 IPsec/IKE 策略 & VNet 到 VNet 连接
...
如果没有配置 IPsec/IKE 策略，则命令（PS > $connection 6。IpsecPolicies）获取空返回。 这并不意味着未对连接配置 IPsec/IKE，而是表示没有自定义 IPsec/IKE 策略。 实际连接使用本地 VPN 设备和 Azure VPN 网关之间协商的默认策略。
