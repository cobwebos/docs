...
* 最新版本的 CLI 命令（2.0 或更高版本）。 有关安装 CLI 命令的信息，请参阅[安装 Azure CLI 2.0](/cli/azure/install-azure-cli) 和 [Azure CLI 2.0 入门](/cli/azure/get-started-with-azure-cli)。
...
如果还没有虚拟网络，请使用 [az network vnet create](/cli/azure/network/vnet#create) 命令创建一个。 创建虚拟网络时，请确保你指定的地址空间不与本地网络的任一个地址空间相重叠。 
...
使用 [az network local-gateway create](/cli/azure/network/local-gateway#create) 命令添加具有多个地址前缀的本地网关：
...
使用 [az network public-ip create](/cli/azure/network/public-ip#create) 命令请求动态公共 IP 地址。
...
使用 [az network vnet-gateway create](/cli/azure/network/vnet-gateway#create) 命令创建 VPN 网关。 如果使用“--no-wait”参数运行该命令，则不会显示任何反馈或输出。 此参数允许在后台创建网关。 创建网关大约需要 45 分钟时间。
...
- 虚拟网关的“公共 IP 地址”。 可以通过 Azure 门户、PowerShell 或 CLI 查看公共 IP 地址。 若要查找虚拟网关的公共 IP 地址，请使用 [az network public-ip list](/cli/azure/network/public-ip#list) 命令。 为了方便阅读，对输出进行了格式化，以表格式显示一系列公共 IP。
...
使用 [az network vpn-connection create](/cli/azure/network/vpn-connection#create) 命令创建连接。
