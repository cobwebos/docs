...
注意：你可以将虚拟网络配置为同时使用站点到站点连接和点到站点连接，前提是你使用动态路由网关创建站点到站点连接。
...
目前，点到站点连接仅适用于经典部署模型（也称为服务管理）。我们正在努力寻求针对 Azure 资源管理器部署模型的点到站点解决方案。当该解决方案可用时，我们会更新此页。
...
是的。如果虚拟网络有动态路由 VPN 网关，则这两种解决方案均可正常工作。我们不支持静态路由 VPN 网关中的点到站点连接。
...
静态路由网关实施基于策略的 VPN。基于策略的 VPN 会根据本地网络和 Azure VNet 之间的地址前缀的各种组合，加密数据包并引导其通过 IPsec 隧道。通常会在 VPN 配置中将策略（或流量选择器）定义为访问列表。

### 什么是基于路由的（动态路由）网关？

动态路由网关实时基于路由的 VPN。基于路由的 VPN 使用 IP 转发或路由表中的“路由”将数据包引导到相应的隧道接口中。然后，隧道接口会加密或解密出入隧道的数据包。基于路由的 VPN 的策略或流量选择器配置为任意到任意（或通配符）。
...
我们有一个网关服务，运行它即可启用跨界连接。我们需要从你的路由域获得 2 个 IP 地址，才能在你本地与云之间进行路由。我们需要你指定至少一个 /29 子网，从中我们可选取 IP 地址来设置路由。虽然你可以创建 /29 子网，但你必须知道，某些功能需要特定的网关大小。对于你想要配置的功能，请遵循网关子网要求。
...
### 能否将使用动态路由 VPN 的虚拟网络连接到另一个使用静态路由 VPN 的虚拟网络？

不能，两个虚拟网络都必须使用动态路由 VPN。
...
能，系统支持该操作。有关详细信息，请参阅[配置可共存的 ExpressRoute 连接和站点到站点 VPN 连接](/documentation/articles/expressroute-coexist)。

## 跨界连接和 VM

### 如果虚拟机位于虚拟网络中，而连接是跨界连接，应如何连接到该 VM？

有几个选择。如果你启用了 RDP 并且创建了终结点，则可以使用 VIP 连接到虚拟机。在这种情况下，你需要指定要连接到的 VIP 和端口。你需要配置用于流量的虚拟机端口。通常，你会转到 Azure 经典门户，并保存与你的计算机建立的 RDP 连接的设置。这些设置将包含所需的连接信息。
