...
---
# <a name="configure-a-vnet-to-vnet-connection-for-the-classic-deployment-model"></a>为经典部署模型配置 VNet 到 VNet 连接
...
本文逐步讲解如何执行相关步骤，使用经典部署模型（也称为服务管理）来创建虚拟网络并将其连接到一起。 以下步骤使用 Azure 经典门户来创建 VNet 和网关，使用 PowerShell 来配置 VNet 到 VNet 连接。 无法在门户中配置该连接。
...
### <a name="dns-servers-and-vpn-connectivity"></a>DNS 服务器和 VPN 连接
...
* 请不要选中 P2S 或 S2S 连接对应的任何复选框。 单击右下角的箭头转到下一屏幕。

### <a name="virtual-network-address-spaces"></a>虚拟网络地址空间
...
* **地址空间** - 包括“起始 IP”和“地址计数”。 请确保指定的地址空间不与本地网络的任一地址空间重叠。 本示例为 VNet1 使用 10.1.0.0/16。
* **添加子网** - 包括“起始 IP”和“地址计数”。 附加的子网不是必需的，但你可能需要为具有静态 DIP 的 VM 创建一个单独的子网。 或者，你可能需要在子网中拥有与其他角色实例分开的 VM。

**单击勾选标记**（位于该页右下角），随即会开始创建虚拟网络。 创建完成后，在“网络”页上，你将看到“状态”下面列出了“已创建”。

## <a name="step-3---create-vnet2"></a>步骤 3 - 创建 VNet2
接下来，重复前面的步骤创建另一个 VNet。 在后面的步骤中，将要连接两个 VNet。 可以参阅步骤 1 中的[示例设置](#step1)。 如果你的 VNet 已存在，则不需执行此步骤， 但需确保 IP 地址范围不与要连接到的任何其他 VNet 或本地网络的 IP 地址范围重叠。

## <a name="step-4---add-the-local-network-sites"></a>步骤 4 - 添加本地网络站点
...
### <a name="a-namelocalnetaadd-the-local-network-site-vnet1local"></a><a name="localnet"></a>添加本地网络站点 VNet1Local
1. 在屏幕的左下角，单击“新建” > “网络服务” > “虚拟网络” > “添加本地网络”。
2. 在“指定本地网络详细信息”页中，对于“名称”，请输入要使用的名称，该名称代表所要连接到的网络。 在此示例中，你可以使用“VNet1Local”来表示 VNet1 的 IP 地址范围和网关。
...
### <a name="add-the-local-network-site-vnet2local"></a>添加本地网络站点 VNet2Local
使用上述步骤创建本地网络站点“VNet2Local”。 如果需要，请参阅步骤 1 中[示例设置](#step1)中的值。

### <a name="configure-each-vnet-to-point-to-a-local-network"></a>将每个 VNet 配置为指向本地网络
...
## <a name="step-5---configure-a-gateway-for-each-vnet"></a>步骤 5 - 为每个 VNet 配置网关
...
1. 在“网络”页上，检查虚拟网络的“状态”列是否为“已创建”。
2. 在“名称”列中，单击虚拟网络的名称。 本示例使用“VNet1”。
...
## <a name="step-6---edit-the-local-network"></a>步骤 6 - 编辑本地网络
1. 在“本地网络”页上，单击要编辑的本地网络名称，然后单击页面底部的“编辑”。 对于“VPN 设备 IP 地址”，请输入对应于 VNet 的网关 IP 地址。 例如，对于 VNet1Local，请输入分配给 VNet1 的网关 IP 地址。 然后，单击页底部的箭头。
...
你可以将虚拟机添加到虚拟网络。 有关详细信息，请参阅[虚拟机文档](https://docs.microsoft.com/azure/#pivot=services&panel=Compute)。
