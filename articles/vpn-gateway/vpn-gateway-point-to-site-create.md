<properties
   pageTitle="使用经典门户配置与 Azure 虚拟网络的点到站点 VPN 网关连接 | Microsoft Azure"
...
本文将指导用户使用**经典门户**在**经典部署模型**中通过点到站点连接来创建 VNet。目前不能在 Azure 门户中为经典部署模型创建端到端配置。可在 Azure 门户中执行一些步骤，但下载 VPN 客户端配置仍需要使用 PowerShell。
...
7. 在“虚拟网络地址空间”页上，指定要用于虚拟网络的地址范围。这些都是动态 IP 地址 (DIPS)，将分配给部署到此虚拟网络的 VM 和其他角色实例。<br><br>所选范围不要与本地网络使用的范围重叠，这一点尤其重要。必须与网络管理员协调，该管理员可能需要从本地网络地址空间划分一个 IP 地址范围，供虚拟网络使用。
...
1. 在 Azure 经典门户的虚拟网络“证书”页上，单击“上传根证书”。
...
1. 在 Azure 经典门户的虚拟网络“仪表板”页上，导航到右下角的速览菜单。有关支持的客户端操作系统列表，请参阅 VPN 网关常见问题解答的[点到站点连接](vpn-gateway-vpn-faq.md#point-to-site-connections)部分。VPN 客户端包中含有用于配置 Windows 内置 VPN 客户端软件的配置信息。该程序包不安装额外的软件。设置特定于要连接到的虚拟网络。<br><br>请选择与要在其中进行安装的客户端操作系统对应的下载包：
