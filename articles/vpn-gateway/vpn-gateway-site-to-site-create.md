<properties
   pageTitle="使用 Azure 门户创建具有站点到站点 VPN 连接的虚拟网络 | Microsoft Azure"
...
# 使用 Azure 门户创建具有站点到站点 VPN 连接的虚拟网络

> [AZURE.SELECTOR]
- [Azure portal](vpn-gateway-site-to-site-create.md)
- [PowerShell - Azure Resource Manager](vpn-gateway-create-site-to-site-rm-powershell.md)

本文将指导你创建一个经典虚拟网络，并与本地网络建立站点到站点 VPN 连接。

Azure 目前有两种部署模式：经典部署模式和 Azure 资源管理器部署模式。根据用于部署虚拟网络的模型不同，配置步骤会有所不同。

这些说明适用于经典部署模型。如果你想要使用 Azure 资源管理器模型创建站点到站点 VPN 网关连接，请参阅[使用 Azure 资源管理器和 PowerShell 创建站点到站点 VPN 连接](/documentation/articles/vpn-gateway-create-site-to-site-rm-powershell)。


## 开始之前

- 请确保你要使用的 VPN 设备符合创建跨界虚拟网络连接所需达到的要求。有关详细信息，请参阅[关于用于虚拟网络连接的 VPN 设备](/documentation/articles/vpn-gateway-about-vpn-devices)。
...
单击页面底部的复选标记，此时将开始创建虚拟网络。创建完成后，在 Azure 门户的“网络”页上，你将看到“状态”下面列出了“已创建”。在创建 VNet 之后，你就可以配置虚拟网络网关。

## 配置虚拟网络网关

接下来，你将配置虚拟网络网关，以便创建安全的站点到站点连接。请参阅[在 Azure 门户中配置虚拟网络网关](vpn-gateway-configure-vpn-gateway-mp.md)。

## 后续步骤

你可以在下文中了解有关虚拟网络跨界连接的更多信息：[关于虚拟网络的安全跨界连接](/documentation/articles/vpn-gateway-cross-premises-options)。
...
如果你想要在经典虚拟网络与使用 Azure 资源管理器模式创建的虚拟网络之间配置连接，请参阅[将经典 VNet 连接到 Azure 资源管理器 VNet](../virtual-network/virtual-networks-arm-asm-s2s-howto.md)。
