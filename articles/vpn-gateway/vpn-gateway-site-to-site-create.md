...
# 使用 Azure 经典门户创建具有站点到站点连接的 VNet
...
本文介绍了如何使用**经典部署模型**和经典门户创建虚拟网络和到本地网络的站点到站点 VPN 连接。站点到站点连接可以用于跨界和混合配置。目前，无法使用 Azure 门户为经典部署模型创建端到端、站点到站点配置。
...
如果想要将多个 VNet 连接到一起，请参阅[为经典部署模型配置 VNet 到 VNet 连接](virtual-networks-configure-vnet-to-vnet-connection.md)。
...
- **DNS 服务器**：输入 DNS 服务器名称和 IP 地址，或从快捷菜单中选择一个以前注册的 DNS 服务器。此设置不创建 DNS 服务器。它允许指定要用于对此虚拟网络进行名称解析的 DNS 服务器。
