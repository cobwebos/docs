...
7. 对于**客户端地址空间**，使用资源管理器 VNet 的虚拟网络 IP 地址空间[的值](#connectoverview)。 此设置用于指定要路由到 Resource Manager 虚拟网络的地址空间。 在示例中，我们使用 192.168.0.0/16 作为 RMVNet 的地址范围。
...
### <a name="4-copy-the-virtual-network-gateway-public-ip-address"></a><a name="ip"></a>4. 复制虚拟网络网关公共 IP 地址
...
### <a name="3-create-a-local-network-gateway"></a><a name="createlng"></a>3. 创建本地网络网关
...
- **-共享密钥**是您生成和指定的值。 对于此示例，我们使用了 *abc123*，但可以生成更复杂的内容。 重要的是，在此处指定的值必须与创建从 Resource Manager 虚拟网络到经典虚拟网络的连接时指定的值相同。
...
## <a name="section-6---verify-your-connections"></a><a name="verify"></a>第 6 节：验证连接
