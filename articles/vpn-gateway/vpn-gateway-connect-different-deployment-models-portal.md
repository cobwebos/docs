...
以下步骤将指导您完成为每个 VNet 配置动态或基于路由的网关以及在网关之间创建 VPN 连接所需的设置。此配置不支持静态或基于策略的网关。在本文中，我们使用经典管理门户、Azure 门户和 PowerShell。目前，无法只使用 Azure 门户创建此配置。
...
在本部分中，我们将为经典 VNet 创建本地网络和网关。本部分中的说明使用经典管理门户。目前，Azure 门户不提供与经典 VNet 相关的所有设置。
...
可以使用经典管理门户、Azure 门户或 PowerShell 来验证连接。可使用以下步骤来验证连接。将值替换为你自己的值。
