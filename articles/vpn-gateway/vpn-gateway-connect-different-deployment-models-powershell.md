...
1. 在 PowerShell 控制台中，登录到您的 Azure 帐户。该 cmdlet 将提示你提供 Azure 帐户的登录凭据。登录后它会下载你的帐户设置，以便这些信息可供 Azure PowerShell 使用。您将使用 SM PowerShell cmdlet 完成此部分配置。
...
可以使用经典管理门户或使用 PowerShell 来创建虚拟网络网关。目前无法在 Azure 门户中创建经典网关。

#### 在经典管理门户中创建网关

1. 在[经典管理门户](https://manage.windowsazure.cn)中，转到“网络”，然后单击要为其创建虚拟网络网关的经典 VNet。这将打开 VNet 的主页。
2. 单击页面顶部的“仪表板”以切换到“仪表板”页面。
3. 在“仪表板”页的底部，单击“创建网关”，然后单击“动态路由”。
4. 单击“是”即可开始创建网关。
5. 等待网关创建完毕。有时可能需要 45 分钟或更长时间才能完成。
6. 创建网关后，可以在“仪表板”页面中查看网关 IP 地址。这是网关的公共 IP 地址。记下或复制此公共 IP 地址。将在后面为 Resource Manager VNet 配置创建本地网络的步骤中使用此地址。
7. 可以选择使用此 cmdlet 来检索网关设置和检查网关状态：`Get-AzureVirtualNetworkGateway`

#### 使用 PowerShell 创建网关

若要使用 PowerShell 创建网关，请使用下面的示例。
...
3. **请求一个公共 IP 地址**并将其分配到 Azure Resource Manager VNet VPN 网关。您无法指定要使用的 IP 地址。它会动态分配到网关。但是，这并不意味着 IP 地址会更改。Azure VPN 网关 IP 地址只在删除或重新创建网关时更改。它不会因为 Azure VPN 网关重设大小、重置或其他内部维护/升级而更改。<br>在此步骤中，我们还将设置一个在后面的步骤中使用的变量。
...
7. 通过运行以下命令**创建 Resource Manager VNet 网关**。`-VpnType` 必须是 *RouteBased*。
