---
title: 在地图上显示搜索结果 |微软 Azure 地图
description: 在本文中，您将了解如何使用 Microsoft Azure 地图 Web SDK 执行搜索请求，并在地图上显示结果。
...
可通过两种方法来搜索感兴趣的位置。 一种方法是使用服务模块发出搜索请求。 另一种方法是通过 Fetch API 向[Azure 映射模糊搜索](https://docs.microsoft.com/rest/api/maps/search/getsearchfuzzy) [API](https://fetch.spec.whatwg.org/)发出搜索请求。 下面将讨论这两种方法。
...
第三个代码块使用 [DataSource](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.source.datasource?view=azure-iot-typescript-latest) 类创建一个数据源对象并向其添加搜索结果。 某个[符号层](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.layer.symbollayer?view=azure-iot-typescript-latest)使用文本或图标来呈现作为符号包装在地图上 [DataSource](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.source.datasource?view=azure-iot-typescript-latest) 中的基于点的数据。  然后创建符号图层。 将数据源添加到符号层，然后将符号层添加到地图。

第四个代码块在[服务模块](how-to-use-services-module.md)中使用[SearchFuzzy](/javascript/api/azure-maps-rest/atlas.service.models.searchgetsearchfuzzyoptionalparams)方法。 它允许您通过[获取搜索模糊休息 API](https://docs.microsoft.com/rest/api/maps/search/getsearchfuzzy)执行自由形式文本搜索，以搜索兴趣点。 获取对搜索模糊 API 的请求可以处理模糊输入的任意组合。 然后使用 `geojson.getFeatures()` 方法提取响应中的 GeoJSON 特征集合，并将其添加到数据源，这将自动通过符号层在地图上呈现数据。

最后一个代码块使用地图的[SetCamera](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#setcamera-cameraoptions---cameraboundsoptions---animationoptions-)属性调整地图的摄像机边界。

搜索请求、数据源、符号图层和摄像机边界位于地图[的事件侦听器](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#events)内。 我们希望确保在地图完全加载后显示结果。


## <a name="make-a-search-request-via-fetch-api"></a>通过 Fetch API 发出搜索请求
...
在上面的代码中，第一个代码块构造地图对象。 它将身份验证机制设置以使用访问令牌。 有关说明，可以参阅[创建地图](./map-create.md)。

代码的第二个块创建一个 URL 以发出搜索请求。 它还创建两个数组来存储搜索结果的边界和引脚。

第三个代码块使用[Fetch API](https://fetch.spec.whatwg.org/)。 [Fetch API](https://fetch.spec.whatwg.org/)用于向[Azure 映射模糊搜索 API](https://docs.microsoft.com/rest/api/maps/search/getsearchfuzzy)发出请求以搜索感兴趣的点。 模糊搜索 API 可以处理任意组合的模糊输入。 然后，它处理并分析搜索响应，并将结果引脚添加到搜索引脚数组中。
...
最后一个代码块创建[边界框](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.data.boundingbox?view=azure-iot-typescript-latest)对象。 它使用结果数组，然后使用地图的[SetCamera](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#setcamera-cameraoptions---cameraboundsoptions---animationoptions-)调整地图的摄像机边界。 然后，它呈现结果引脚。

搜索请求、数据源、符号图层和摄像机边界设置在地图[的事件侦听器](https://docs.microsoft.com/javascript/api/azure-maps-control/atlas.map?view=azure-iot-typescript-latest#events)中，以确保在地图完全加载后显示结果。
