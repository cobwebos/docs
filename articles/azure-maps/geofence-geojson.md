---
title: Azure Maps 中的地理围栏 GeoJSON 数据格式 | Microsoft Docs
...
| 姓名 | 类型 | 必填  | 描述 |
| :------------ |:------------: |:---------------:| :-----|
| startTime | Datetime  | 真 | 有效时段的开始日期时间。 |
| endTime   | Datetime  | 真 |  有效时段的结束日期时间。 |
| recurrenceType | string | 假 |   时段的重复类型。 值可为 `Daily`、`Weekly`、`Monthly` 或 `Yearly`。 默认值为 `Daily`。|
| businessDayOnly | Boolean | 假 |  指示数据是否仅在工作日有效。 默认值为 `false`。|
