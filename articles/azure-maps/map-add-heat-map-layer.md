...
热度地图也称为点密度图，是一种类型的数据可视化。 它们用于用一系列颜色来表示数据的密度，并在地图上显示数据 "热点"。 热度地图是使用大量点呈现数据集的好方法。 
...
* `color`：指定如何着色热度地图。 颜色渐变是热度地图的常见功能。 您可以使用 `interpolate` 表达式来实现效果。 你还可以使用 `step` 表达式着色热度地图，将密度直观地分解为类似于等高线或雷达图样式图的范围。 这些调色板定义了从最小到最大密度值的颜色。 
...
缩放半径，使每个缩放级别翻倍，从而创建一个在所有缩放级别上都保持一致的热度地图。 若要应用此缩放，请将 `zoom` 与基 2 `exponential interpolation` 表达式一起使用，如下面的示例中所示。 缩放地图，查看热度地图如何随缩放级别一起缩放。
