...
1. 根据身份验证方法完成以下步骤之一。 
    ...
    * 如果你的应用程序对 Azure Maps Web SDK 使用用户令牌身份验证，则`oauth2AllowImplicitFlow`启用。 若要启用它，请在应用注册的**清单**部分中将`oauth2AllowImplicitFlow`设置为 true。 
...
如果你的应用程序使用服务器或应用程序身份验证，则从 Azure AD 令牌终结`https://login.microsoftonline.com`点请求令牌。 在请求中，使用以下详细信息： 
