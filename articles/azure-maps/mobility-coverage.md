...
Azure Maps [移动服务](https://aka.ms/AzureMapsMobilityService) 可提高具有公共传输功能的应用程序的开发时间，如中转路由和搜索附近的公共传输停止。 用户可以检索有关公交站点、线路和时刻表的详细信息。 通过出行服务，用户还可以检索停靠站点和线路的几何形状，停靠站点、线路和服务区的警报以及实时的公交到达时间和服务警报。 此外，出行服务还提供具有多式联运行程规划选项的路线规划功能。 多式联运行程规划在一个行程中融合了步行、骑行和公共交通选项。 用户还可以访问详细的多式联运分步行程。 
