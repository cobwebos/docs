---
title: 支持的 web 浏览器-Azure Maps |Microsoft Docs
description: 了解 Azure Maps Web SDK 支持的 web 浏览器
author: rbrundritt
ms.author: richbrun
ms.date: 03/25/2019
ms.topic: conceptual
...
Azure Maps Web SDK 支持下列 web 浏览器。
...
> [!TIP]
> 如果要嵌入地图在移动应用程序视图的 WebView 控件内，你可能会发现使用[npm 包的 Azure Maps Web SDK](https://www.npmjs.com/package/azure-maps-control)优于引用 CDN 托管版本的 SDK。 这将减少加载时间，因为 SDK 将用户的设备上已经有这些并不需要在运行时下载。
...
如果需要面向较旧的 web 浏览器可能不支持或提供有限的 WebGL 的支持，建议使用与开放源代码地图控件结合使用 Azure 地图服务，如[宣传册](https://leafletjs.com/)。 
