...
Read 函数的结果是一个`SpatialDataSet`对象。 此对象扩展 GeoJSON FeatureCollection 类。 它可以轻松地`DataSource`按原样传递到，以在地图上呈现其功能。 `SpatialDataSet`不仅包含功能信息，还可能包括 KML 地面叠加、处理度量值和其他详细信息，如下表中所述。
...
下面的代码演示如何读取空间数据集，并使用`SimpleDataLayer`类在地图上呈现该数据集。 该代码使用 URL 指向的 .GPX) 文件。
...
<iframe height='500' scrolling='no' title='将 KML 加载到地图上' src='//codepen.io/azuremaps/embed/XWbgwxX/?height=500&theme-id=0&default-tab=js,result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>请参阅<a href='https://codepen.io'>CodePen</a>上的 KML （<a href='https://codepen.io/azuremaps'>@azuremaps</a>），将笔<a href='https://codepen.io/azuremaps/pen/XWbgwxX/'>负载拖到 Azure Maps 地图</a>上。
</iframe>
...
<iframe height='700' scrolling='no' title='将空间文件拖放到地图上' src='//codepen.io/azuremaps/embed/zYGdGoO/?height=700&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>请参阅<a href='https://codepen.io'>CodePen</a>上的<a href='https://codepen.io/azuremaps/pen/zYGdGoO/'>将空间文件拖放到 map</a> by<a href='https://codepen.io/azuremaps'>@azuremaps</a>Azure Maps （）。
</iframe>
...
熟知[文本](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry)（WKT）是一种用于将空间几何表示为文本的开放地理空间信息联盟（OGC）标准。 许多地理空间系统使用 PostGIS 插件支持 WKT，例如 Azure SQL 和 Azure PostgreSQL。 与大多数 OGC 标准一样，坐标的格式设置为 "经度纬度"，以与 "x y" 约定保持一致。 例如，可以`POINT(-110 45)`使用 WKT 格式编写介于110和纬度45的点。

可以使用`atlas.io.ogc.WKT.read`函数读取已知文本，并使用函数编写该`atlas.io.ogc.WKT.write`文本。

## <a name="examples-of-reading-and-writing-well-known-text-wkt"></a>读取和写入熟知文本（WKT）的示例

下面的代码演示如何读取众所周知的文本字符串，并使用`POINT(-122.34009 47.60995)`气泡图层在地图上呈现该字符串。

<br/>

<iframe height='500' scrolling='no' title='读取已知文本' src='//codepen.io/azuremaps/embed/XWbabLd/?height=500&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>请参阅<a href='https://codepen.io'>CodePen</a>上的 "<a href='https://codepen.io/azuremaps/pen/XWbabLd/'>读取已知</a>Azure Maps<a href='https://codepen.io/azuremaps'>@azuremaps</a>文本"。
</iframe>
...
<iframe height='700' scrolling='no' title='读取和写入已知文本' src='//codepen.io/azuremaps/embed/JjdyYav/?height=700&theme-id=0&default-tab=result&embed-version=2&editable=true' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'>请参阅<a href='https://codepen.io'>CodePen</a>上的 "<a href='https://codepen.io/azuremaps/pen/JjdyYav/'>读取" 和 "写入常见</a>Azure Maps<a href='https://codepen.io/azuremaps'>@azuremaps</a>文本"。
</iframe>

## <a name="read-and-write-gml"></a>读取和写入 GML

GML 是一个空间 XML 文件规范，通常用作其他 XML 规范的扩展。 可以使用`atlas.io.core.GmlWriter.write`函数将 GeoJSON 数据编写为具有 GML 标记的 XML。 可以使用`atlas.io.core.GmlReader.read`函数读取包含 GML 的 XML。 Read 函数有两个选项：
...
`atlas.io.read`函数在检测到输入数据`atlas.io.core.GmlReader.read`为 xml 时将默认为函数，但数据不是其他支持空间 XML 格式之一。
