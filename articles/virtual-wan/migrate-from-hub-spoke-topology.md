...
![中心辐射](./media/migrate-from-hub-spoke-topology/hub-spoke.png)
**图： Azure 虚拟广域网**

成千上万的客户使用 Azure 虚拟数据中心（VDC）中心辐射型连接模型来利用 Azure 网络的默认可传递的路由行为，以便构建简单且可缩放的云网络。 Azure 虚拟 WAN 基于这些概念构建并引入了新功能，这些功能不仅允许在本地位置和 Azure 之间使用全局连接拓扑，而且还允许客户利用 Microsoft 网络的规模来增强现有的全球网络。
...
![Contoso 虚拟 wan 体系](./media/migrate-from-hub-spoke-topology/vwan-architecture.png)
结构**图： Azure 虚拟 wan 体系结构**
...
测试混合连接**图4： VDC 辐射到虚拟 wan 迁移**

### <a name="step-5-transition-connectivity-to-virtual-wan-hub"></a>步骤5：将连接转换到虚拟 WAN 集线器

![转换到虚拟 wan 集线器](./media/migrate-from-hub-spoke-topology/figure5.png)
的连接**图5： VDC 辐射到虚拟 wan 迁移**

**一个**。 删除从辐射虚拟网络到旧 VDC 集线器的现有对等互连。 步骤 a-c 完成之前，无法访问辐射虚拟网络中的应用程序。
...
![旧集线器成为共享服务轮辐](./media/migrate-from-hub-spoke-topology/figure6.png)
**图6： VDC 辐射到虚拟 WAN 迁移**

由于虚拟 WAN 中心是一个托管实体，不允许部署自定义资源（如虚拟机），因此共享服务块现在作为一个辐射虚拟网络存在，并通过 Azure 应用程序网关或网络虚拟化设备来承载 internet 入口等功能。 现在，共享服务环境与后端虚拟机之间的流量在虚拟 WAN 托管的中心内传输。
...
![最终状态体系结构和通信路径](./media/migrate-from-hub-spoke-topology/figure8.png)
**图：双重区域虚拟 WAN**
...
的安全性和策略控制**图：虚拟 WAN 中的 Azure 防火墙（受保护的虚拟中心）**
