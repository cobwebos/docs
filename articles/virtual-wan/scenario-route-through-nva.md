...
ms.locfileid: "85848099"
---
# <a name="scenario-route-traffic-through-an-nva"></a>方案：通过 NVA 路由流量

使用虚拟 WAN 虚拟中心路由时，有很多可用方案。 在此 NVA 方案中，目标是通过 NVA （网络虚拟设备）将流量路由到 VNet，将 VNet 路由到 vnet。 有关虚拟中心路由的详细信息，请参阅[关于虚拟中心路由](about-virtual-hub-routing.md)。
...
1. 标识 NVA 辐射 VNet 连接。 在**图 1**中，它们是**vnet 2 连接（Eastusconn）** 和**vnet 4 连接（weconn）**。
   ...
   * 从 VNET 7 和8到 VNET 4 NVA IP 
   ...
   不需要将 VNET 5、6、7和8直接连接到虚拟中心。 确保 Vnet 5、6、7、8中的 Nsg 允许 branch （VPN/ER/P2S）或 Vnet 连接到 theire 远程 Vnet 的流量。 例如，VNET 5、6必须确保 Nsg 允许连接到远程中心2的本地地址前缀和 Vnet 7、8的流量。 
