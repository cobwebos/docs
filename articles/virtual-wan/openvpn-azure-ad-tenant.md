...
description: 可以使用 Azure 虚拟 WAN 用户 VPN (点到站点) 通过 Azure AD 身份验证连接到 VNet
...
通过 IKEv2 协议连接到虚拟中心时，可以使用基于证书的身份验证或 RADIUS 身份验证。 但是，在使用 OpenVPN 协议时，还可以使用 Azure Active Directory 身份验证。 本文介绍如何使用 OpenVPN authentication 设置用于虚拟 WAN 用户 VPN (点到站点) 的 Azure AD 租户。
...
2. 接下来，向你的组织授予管理员许可，这允许 Azure VPN 应用程序登录和读取用户配置文件。 在浏览器的地址栏中复制并粘贴与部署位置相关的 URL：
