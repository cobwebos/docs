...
2. 选择**VPN （站点到站点）** 查找站点到站点网关， **expressroute**查找 expressroute 网关，或使用**用户 VPN （指向站点）** 查找点到站点网关。 在页面上，可以看到网关信息。 复制此信息。 稍后你将使用它来查看使用 Azure Monitor 的诊断。

3. 选择“指标”。

   :::image type="content" source="./media/logs-metrics/metrics.png" alt-text="指标":::
...
默认情况下会收集**活动日志**条目，并且可以在 Azure 门户中查看。 可以使用 Azure 活动日志（以前称为*操作日志*和*审核日志*）查看提交到 Azure 订阅的所有操作。
