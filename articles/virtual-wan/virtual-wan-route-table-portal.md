...
*  拥有网络虚拟设备 (NVA)。 网络虚拟设备是你选择的第三方软件，通常由 Azure 市场在虚拟网络中预配。

    * 必须向 NVA 网络接口分配专用 IP 地址。

    * NVA 未部署在虚拟中心中。 它必须部署在单独的虚拟网络中。

    *  NVA 虚拟网络可以有一个或多个虚拟网络连接到它。 在本文中，我们将 NVA 虚拟网络称为“间接辐射 VNet”。 这些虚拟网络可以使用 VNet 对等互连连接到 NVA VNet。 在上图中，VNet 1、VNet 2 和 NVA VNet 之间的 VNet 对等互连链路用黑色箭头表示。
*  你已创建两个虚拟网络。 它们将用作辐射 VNet。

    * VNet 辐射地址空间是：VNet1：10.0.2.0/24 和 VNet2：10.0.3.0/24。 如果需要有关如何创建虚拟网络的信息，请参阅[创建虚拟网络](../virtual-network/quick-create-portal.md)。
...
2. 单击要为其创建路由表的中心。
3. 单击“...”，然后单击“编辑虚拟中心” 。
4. 在“编辑虚拟中心”页上，向下滚动，然后选中“将表用于路由”复选框 。
5. 在“如果目标前缀为”列中，添加地址空间。 在“发送到下一个跃点”列中，添加 DMZ NVA 网络接口专用 IP 地址。
>[!NOTE]
>DMZ NVA 网络适用于本地中心。
>
...
从每个间接辐射 VNet（VNet1 和 VNet2）创建到中心的虚拟网络连接。 上图中用蓝色箭头表示这些虚拟网络连接。 然后，从 NVA VNet 创建到中心的 VNet 连接（图中的黑色箭头）。
