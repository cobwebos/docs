...
在**Azure 门户 >虚拟 WAN**中，从左侧的 "**监视**" 菜单中选择 "**见解（预览版）**"。 这会显示 " **Insights" 视图**，其中显示了虚拟 WAN 依赖关系映射和高级指标最小工作簿。
...
* 部署在中心（安全中心）中的 Azure 防火墙（包括第三方防火墙代理）。
* 在辐射 Vnet 中部署的第三方 NVA （网络虚拟设备）。

依赖关系图还显示间接连接的 Vnet （对等互连与虚拟 WAN 辐射 Vnet 相关的 VNet）。
