...
存储资源管理器要求使用密码管理器。 您可能必须手动连接到密码管理器。 可以通过运行以下命令，将存储资源管理器连接到系统的密码管理器：
...
存储资源管理器提供了多种连接到存储帐户的方式。 通常，可以：

* [登录到 Azure 以访问订阅及其资源](#sign-in-to-azure)
...
### <a name="attach-a-specific-resource"></a>附加特定资源
...
* [使用连接字符串](#use-a-connection-string)。 如果有到存储帐户的连接字符串，请使用此选项。 存储资源管理器支持密钥和[共享访问签名](storage/common/storage-dotnet-shared-access-signature-part-1.md)连接字符串。
...
* [使用 "名称" 和 "密钥](#use-a-name-and-key)"。 如果你知道某个帐户密钥到你的存储帐户，则可以使用此选项快速连接。 在 "存储帐户" 页中，通过选择 "**设置**" > " [Azure 门户](https://portal.azure.com)中的"**访问密钥**"查找密钥。
...
1. 如果你尚未执行此操作，请使用 "**添加 Azure 帐户**" 选项登录到有权访问该资源的 azure 帐户。 登录后，返回以**连接到 Azure 存储**。

1. 选择 "**通过 Azure Active Directory （Azure AD）添加资源**"，然后选择 "**下一步**"。

1. 选择 "Azure 帐户" 和 "租户"。 这些值必须具有对要附加到的存储资源的访问权限。 选择“**下一页**”。

1. 选择要附加的资源类型。 输入连接所需的信息。 

   在此页上输入的信息取决于要添加的资源类型。 请确保选择正确的资源类型。 输入所需信息后，选择 "**下一步**"。

1. 查看**连接摘要**，确保所有信息均正确无误。 如果是，请选择 "**连接**"。 否则，选择 "**上一步**" 返回到前一页，修复所有不正确的信息。
...
1. 选择 "**使用连接字符串**"，然后选择 "**下一步**"。

1. 为连接选择 "显示名称"，并输入连接字符串。 然后，选择“下一步”。
...
1. 选择 "**使用共享访问签名（SAS） URI**"，然后选择 "**下一步**"。

1. 为连接选择一个显示名称，然后输入共享访问签名 URI。 要附加的资源类型的服务终结点应自动填充。 如果你使用的是自定义终结点，则它可能不是。 选择“**下一页**”。
...
#### <a name="attach-to-a-local-emulator"></a>附加到本地模拟器
...
1. 启动模拟器。 输入命令 `AzureStorageEmulator.exe status` 显示每种服务类型的端口。
...
1. 选择 "**附加到本地模拟器**"，然后选择 "**下一步**"。

1. 为连接选择一个 "显示名称"，并为每个服务类型输入模拟器正在侦听的端口。 **附加到本地模拟器**会为大多数模拟器推荐默认端口值。 **文件端口**为空白，因为这两个官方仿真器当前均不支持文件服务。 如果正在使用的模拟器支持文件，可以输入要使用的端口。 然后，选择“下一步”。

1. 查看**连接摘要**，确保所有信息均正确。 如果是，请选择 "**连接**"。 否则，选择 "**上一步**" 返回到前一页，修复所有不正确的信息。
...
1. 选择 Azure Cosmos DB API，输入**连接字符串**数据，然后选择 **"确定"** 以连接 Azure Cosmos DB 帐户。 有关如何检索连接字符串的信息，请参阅[管理 Azure Cosmos 帐户](https://docs.microsoft.com/azure/cosmos-db/manage-account)。
...
1. 输入 URI，然后选择 **"确定"** 。 Data Lake Store 显示在 " **Data Lake Storage**下。
    ...
    ![连接到 Data Lake Store 结果](./media/vs-azure-tools-storage-manage-with-storage-explorer/storage-explorer-attach-data-lake-finished.png)
...
> [!NOTE]
> 若要加快搜索速度，请使用**帐户管理**取消选择不包含所要搜索的项的订阅。 您还可以右键单击某个节点，然后选择 "**从此处搜索**" 以从特定节点开始搜索。
>
>
