<properties
   pageTitle="将 Node.js 应用程序部署到 Azure 中的 Linux 虚拟机"
...
# 将 Node.js 应用程序部署到 Azure 中的 Linux 虚拟机
...
> 如果你没有 GitHub 帐户，可以[在此处](https://github.com/join)注册。

> 如果你没有 [Microsoft Azure](https://azure.microsoft.com/) 帐户，可以[在此处](https://azure.microsoft.com/pricing/free-trial/)注册免费试用版。如果你还没有 Microsoft 帐户，这也将引导你完成 [Microsoft 帐户](http://account.microsoft.com)的注册过程。或者，如果你订阅了 Visual Studio，则可以[激活你的 MSDN 权益](/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F)。

> 如果你的开发计算机上没有 Git，并且你使用的是 Macintosh 或 Windows 计算机，请从[此处](http://www.git-scm.com)安装 Git。如果你使用的是 Linux，请使用最适合你的机制安装 Git，如 `sudo apt-get install git`。
...
从主 (~) 目录开始，运行以下命令（将 *accountname* 替换为你的 GitHub 用户帐户名）：
...
此时将运行应用程序的 Web 前端，但还需要执行一个步骤才能从 Web 浏览器访问该应用程序。你创建的虚拟机受名为 *network security group* 的 Azure 资源保护，该资源是在你设置虚拟机时为你创建的。目前，该资源仅允许到端口 22 的外部请求路由到虚拟机，这将实现与计算机的 SSH 通信，但也仅限于此。因此，为了查看 TODO 应用程序（配置为在端口 8080 上运行），还需要打开此端口。
...
> [AZURE.NOTE]
此配置仅适用于本教程。它**不是**推荐的安全方案，不应在生产环境中使用。
...
### 配置你的开发计算机
接下来，我们将尝试把你的开发计算机连接到 Web 前端虚拟机。这与将虚拟机上的裸机存储库添加为远程存储库一样简单。运行以下命令以执行此操作（根据需要将 *user* 替换为你的 Web 前端虚拟机登录名、*machinename* 和 *region*）：
