...
“遥测”。有 4 个代表引擎属性的传感器。这些传感器一般标记为传感器 9、传感器 11、传感器 14 和传感器 15。这 4 个传感器代表足以从 RUL 的机器学习模型获取有用结果的遥测。此模型根据包含实际引擎传感器数据的公用数据集创建而来。有关如何根据原始数据集创建该模型的详细信息，请参阅 [Cortana Intelligence Gallery Predictive Maintenance Template（Cortana Intelligence 库预见性维护模板）][lnk-cortana-analytics]。
...
有关如何根据原始数据集创建该模型的详细信息，请参阅 [Cortana Intelligence Gallery Predictive Maintenance Template（Cortana Intelligence 库预见性维护模板）][lnk-cortana-analytics]。
...
Web 应用程序中的此页面会使用 PowerBI JavaScript 控件（请参阅 [PowerBI-visuals repository（PowerBI 可视化效果存储库）][lnk-powerbi]）以可视化方式呈现：
