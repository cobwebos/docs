...
1.  在你的 iOS 设备上登录到 [HockeyApp](https://rink.hockeyapp.net)。使用 Microsoft 登录按钮之一，并使用你注册会议时使用的同一 Microsoft 帐户电子邮件登录。（请勿使用电子邮件和密码字段。）
...
在 MyDriving 应用内部，你可以使用内置的 HockeyApp 反馈机制：只需摇动你的 iOS 和 Android 手机或使用“反馈”菜单命令即可。这将自动附加屏幕截图，这样我们便能了解你想说明的内容。如果不幸出现了故障，HockeyApp 将收集故障日志并告知我们。

你也可以在 [GitHub 上提出问题]，件或在下面留言（zh-cn 版）。
...
-   使用我们的 Azure 资源管理器脚本[创建和部署你自己的系统](/documentation/articles/iot-solution-build-system)。[MyDriving Reference Guide（MyDriving 参考指南）](http://aka.ms/mydrivingdocs)还将指导你了解你将进行最多自定义操作的区域。
