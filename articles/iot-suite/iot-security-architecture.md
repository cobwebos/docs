...
此上下文将“服务”定义为任何软件组件或模块，它通过现场网关或云网关来与设备交互、进行数据收集和分析，以及执行命令和控制等功能。  服务属于中介。 它们向网关和其他子系统，其标识下执行操作、 存储和分析数据、 自主将命令发送到基于数据见解或计划的设备和公开信息和控制向授权最终用户的功能。
