...
连接工厂允许你直接从云对工业设备发出命令并进行控制。 可以通过此功能来响应设备生成的警报。 例如，可以从云向设备发送命令。 可以在 OPC UA 服务器浏览器树的 **StationCommands** 节点中查找可用命令。 在本方案中，需打开慕尼黑生产线装配站的泄压阀。 若要使用命令和控制功能，必须通过“管理员”角色完成预配置解决方案的部署。
