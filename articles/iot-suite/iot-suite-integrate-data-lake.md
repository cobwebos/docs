...
1. 在 Azure 门户中，单击“Cloud Shell”按钮。
    ...
    ![门户启动图标](media/iot-suite-integrate-data-lake/portal-launch-icon.png)
...
1. 单击“创建资源”，从 Marketplace 中选择“物联网”，然后单击“流分析作业”。
    ...
    ![新建流分析作业](media/iot-suite-integrate-data-lake/new-stream-analytics-job.png)
...
1. 单击“创建”。
    ...
    ![创建流分析作业](media/iot-suite-integrate-data-lake/create-stream-analytics-job.png)
...
1. 在“概述”页上单击“输入”。
    ...
    ![概述页](media/iot-suite-integrate-data-lake/stream-analytics-overview.png)

1. 单击“添加流输入”，从下拉列表中选择“IoT 中心”。

    ![添加输入](media/iot-suite-integrate-data-lake/stream-analytics-add-input.png)

1. 在“新建输入”选项卡上，提供 **IoTHub** 输入别名。

1. 在“使用者组”下拉列表中，选择前面创建的使用者组。 本文使用 **streamanalyticsjob**。

    ![选择输入](media/iot-suite-integrate-data-lake/stream-analytics-new-input.png)

1. 单击“ **保存**”。

1. 在“概述”页上单击“输出”。

    ![添加 Data Lake Store](media/iot-suite-integrate-data-lake/stream-analytics-overview-2.png)

1. 单击“添加”，从下拉列表中选择“Data Lake Store”。

    ![添加输出](media/iot-suite-integrate-data-lake/stream-analytics-output.png)
...
1. 在“日期格式”字段中，输入 **/streaming/{date}/{time}**。 保留默认日期格式 YYYY/MM/DD 和时间格式 HH。
    ...
    ![提供文件夹结构](media/iot-suite-integrate-data-lake/stream-analytics-new-output.png)

1. 单击“授权”。

    必须授权 Data Lake Store 向流分析作业授予对文件系统的写入访问权限。

    ![向 Data Lake Store 提供流分析授权](media/iot-suite-integrate-data-lake/stream-analytics-out-authorize.png)
...
1. 在“概述”选项卡上，单击“编辑查询”。
    ...
    ![编辑查询](media/iot-suite-integrate-data-lake/stream-analytics-edit-query.png)
...
1. 在查询编辑器中，将 [YourOutputAlias] 和 [YourInputAlias] 占位符替换为前面定义的值。
    ...
    ![流分析查询](media/iot-suite-integrate-data-lake/stream-analytics-query.png)
...
1. 在“概述”选项卡上，单击“启动”。
    ...
    ![启动流分析作业](media/iot-suite-integrate-data-lake/stream-analytics-start.png)
...
1. 单击“启动”。
    ...
    ![选择自定义日期](media/iot-suite-integrate-data-lake/stream-analytics-start-custom.png)

    等到作业进入运行中状态，如果看到可以来自查询的错误，请务必检查语法是否正确。

    ![作业正在运行](media/iot-suite-integrate-data-lake/stream-analytics-running.png)
...
1. 在数据资源管理器中，向下钻取到 **/streaming** 文件夹。 将会看到以 YYYY/MM/DD/HH 格式创建的文件夹。
    ...
    ![浏览流数据](media/iot-suite-integrate-data-lake/data-lake-store-data-explorer.png)

    将会看到一些 json 文件，每小时各有一个文件。

    ![浏览流数据](media/iot-suite-integrate-data-lake/data-lake-store-file-preview.png)
