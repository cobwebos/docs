...
> [!NOTE]
> 如果在 Azure 云服务（Web 角色或辅助角色）中托管应用程序，并且这是你首次部署应用程序，Azure 将尝试使用在部署环境中所安装的 Node.js 的版本（如果该版本与 Azure 中提供的默认版本之一相匹配）。
>
>
...
还可使用 **Get-AzureServiceProjectRoleRuntime** 检索托管为云服务的应用程序的可用 Node.js 版本列表。  始终确认项目所依赖的 Node.js 版本是否在此列表中。
...
4. 下载用于应用程序的特定版本 **node.exe**（Windows 版本）。 例如，以下命令使用 **curl** 下载版本 0.8.1：
...
了解如何指定应用程序使用的 Node.js 版本后，请了解[如何使用模块]、[构建并部署 Node.js 网站](app-service/app-service-web-get-started-nodejs.md)以及[如何使用针对 Mac 和 Linux 的 Azure 命令行工具]。
