...
在本教程中，你将利用便捷的 Azure 开放数据集和功能强大的 Azure 机器学习服务来创建回归模型，以预测纽约出租车车费价格。 你可以轻松下载公开发布的出租车、节假日和天气数据，并使用 Azure 机器学习服务配置自动化机器学习试验。 此过程接受定型数据和配置设置，并自动循环访问不同特征规范化/标准化方法、模型和超参数设置的组合，以实现最佳模型。

在本教程中，你将学习如何执行以下任务：

- 配置 Azure 机器学习服务工作区
- 创建本地 Python 环境
- 使用 Azure 开放数据集访问、转换和联接数据
- 定型自动化机器学习回归模型
- 计算模型准确度
...
首先，创建用于保留出租车数据的数据帧。 如果是在非 Spark 环境中，开放数据集仅允许一次下载一个月的数据，并利用一些类来避免较大数据集出现 `MemoryError`。 若要下载一年的出租车数据，请以迭代方式一次提取一个月的数据，并在将数据追加到 `green_taxi_df` 前，先从各月的数据中随机采样 2000 个样本，以免数据帧膨胀。 然后，预览数据。

>[!NOTE]
> 开放数据集有镜像类，可用于无需考虑数据大小和内存的 Spark 环境中。
...
至此，初始数据已加载。是时候定义函数了，以根据接取日期/时间字段创建各种基于时间的特征。 这将新建月份、日期、周几和时段字段，并允许模型将基于时间的季节性考虑在内。 此函数还添加国家/地区代码的静态特征，以联接节假日数据。 请对数据帧使用 `apply()` 函数，以迭代方式将 `build_time_features()` 函数应用于出租车数据中的每一行。
...
删除建模或其他特征生成不需要的一些列。 重命名接取时间字段，并使用 `pandas.Series.dt.normalize` 将时间转换为午夜。 对所有时间特征都这样做，这样稍后在每日粒度级别将数据集联接在一起时，就可以将日期/时间组件用作键了。
...
### <a name="enrich-with-holiday-data"></a>使用节假日数据进行扩充
...
### <a name="enrich-with-weather-data"></a>使用天气数据进行扩充
...
> [!NOTE]
> 本教程中的示例使用 Pandas 函数和自定义聚合来合并数据，但开放数据集 SDK 有专门用于轻松合并和扩充数据集的类。 有关这些设计模式的代码示例，请参阅[笔记本](https://github.com/Azure/OpenDatasetsNotebooks/blob/master/tutorials/data-join/04-nyc-taxi-join-weather-in-pandas.ipynb)。
...
从汇总统计信息中可以看到，有几个字段包含离群值或会降低模型准确度的值。 首先，将纬度/经度字段筛选为位于用于筛选天气数据的同一边界内。 `tripDistance` 字段包含一些错误数据，因为最小值为负值。 `passengerCount` 字段也包含错误数据，因为最大值为 210 个乘客。 最后，`totalAmount` 字段包含负值，这在模型上下文中毫无意义。
...
现在，使用已准备的数据来定型自动化机器学习模型。 首先，将 `final_df` 拆分为特征（x 值）和标签（y 值，在此模型中为出租车车费）。
...
现在，使用 `scikit-learn` 库中的 `train_test_split()` 函数将数据拆分为训练集和测试集。 `test_size` 参数决定了分配用于测试的数据的百分比。 `random_state` 参数设置随机数生成器的种子。这样一来，定型-测试拆分是有确定性的。
...
结合使用 `get()` 函数和订阅及工作区信息，加载 Azure 机器学习服务工作区。 在工作区内，创建用于存储和监视模型运行的试验。
...
使用 `AutoMLConfig` 类，创建试验的配置对象。 附加定型数据，并指定控制定型过程的设置和参数。 这些参数的用途如下：

* `task`：要运行的试验的类型。
* `X`：定型特征。
* `y`：定型标签。
* `iterations`：要运行的迭代次数。 每次迭代都尝试不同特征规范化/标准化方法和不同模型（使用多个超参数设置）的组合。
* `primary_metric`：要在模型定型期间优化的主要指标。 最佳拟合模型是根据此指标进行选择。
* `preprocess`：控制试验能否预处理输入数据（处理缺失数据、将文本转换为数字等）。
* `n_cross_validations`：在验证数据未指定的情况下，需执行的交叉验证拆分的数目。
...
提交试验，以供定型。 在提交试验以后，此过程会根据定义的约束循环访问不同的机器学习算法和超参数设置。 它通过优化定义的准确度指标来选择最佳拟合模型。 将 `automl_config` 对象传递到试验。 将输出设置为 `True`，以查看试验期间的进度。

提交试验后，便会看到定型过程的实时输出。 对于每次迭代，可以看到模型类型和特征规范化/标准化方法、运行持续时间以及定型准确度。 字段 `BEST` 根据指标类型跟踪运行情况最好的训练分数。
...
### <a name="retrieve-the-fitted-model"></a>检索拟合模型

在所有定型迭代结束时，自动化机器学习过程利用装袋算法或堆叠算法，从所有运行中创建集成算法。 将拟合集成模型检索到 `fitted_model` 变量中，并将最佳单次运行检索到 `best_run` 变量中。
...
使用拟合集成模型对测试数据集运行预测，以预测出租车车费。 函数 `predict()` 使用拟合模型，并预测 `X_test` 数据集的 y 值（即出租车车费）。
...
计算结果的均方根误差。 使用 `y_test` 数据帧，并将它转换为列表 `y_actual`，以与预测值进行比较。 函数 `mean_squared_error` 接受两个数组的值，计算两个数组之间的平均平方误差。 计算结果的平方根会生成与 y 变量（车费）单位相同的误差。 它大致指出了出租车车费预测值与实际车费的差值，但在很大程度上对大误差进行了加权处理。
...
* 若要查看更多代码示例，请参阅 Azure 开放数据集[笔记本](https://github.com/Azure/OpenDatasetsNotebooks)。
* 若要详细了解 Azure 机器学习服务中的自动化机器学习，请参阅[操作说明](https://docs.microsoft.com/azure/machine-learning/service/how-to-configure-auto-train)。
