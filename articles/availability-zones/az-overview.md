...
- **分区服务**– 将资源固定到特定区域（例如，虚拟机、托管磁盘、标准 IP 地址）或
...
> [!IMPORTANT]
> 可用性区域标识符（上图中的数字 1、2 和 3）以逻辑方式映射到每个订阅的实际物理区域。 这意味着给定订阅中的可用性区域 1 可能引用与不同订阅中的可用性区域 1 不同的物理区域。 因此，建议不要依赖不同订阅中的可用区域 ID 进行虚拟机放置。

## <a name="services-support-by-region"></a>按区域提供的服务支持
...
## <a name="services-resiliency"></a>服务恢复能力
所有 Azure 管理服务都设计为从区域级故障中具有弹性。 在故障范围内，与整个区域故障相比，区域内的一个或多个可用性区域故障的故障半径较小。 Azure 可以从区域或其他 Azure 区域中的管理服务的区域级故障中恢复。 Azure 在区域内一次执行一个区域的关键维护，以防止任何影响跨区域可用性区域部署的客户资源的失败。
...
- [为 Azure 宇宙数据库添加区域冗余区域](../cosmos-db/high-availability.md#availability-zone-support)
- [入门 Azure 缓存，用于雷瑞斯可用性区域](https://aka.ms/redis/az/getstarted)
- [创建 Azure Active Directory 域服务实例](../active-directory-domain-services/tutorial-create-instance.md)
- [创建使用可用性区域的 Azure 库伯奈斯服务 （AKS） 群集](../aks/availability-zones.md)
