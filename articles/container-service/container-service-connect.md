...
对于 DC/OS 和 Docker Swarm，我们建议你创建一个从本地计算机通往群集管理系统的安全外壳 (SSH) 隧道。 建立隧道后，可以运行使用 HTTP 终结点的命令，通过本地系统查看 Orchestrator 的 Web 界面（如果可用）。 


## <a name="prerequisites"></a>先决条件

* [部署在 Azure 容器服务中](container-service-deployment.md)的 Kubernetes、DC/OS 或 Docker Swarm 群集。
* SSH RSA 私钥文件，对应于在部署期间添加到群集的公钥。 这些命令假设 SSH 私钥位于计算机上的 `$HOME/.ssh/id_rsa` 中。 有关详细信息，请参阅适用于 [macOS 和 Linux](../virtual-machines/linux/mac-create-ssh-keys.md) 或 [Windows](../virtual-machines/linux/ssh-from-windows.md) 的说明。 如果 SSH 连接不起作用，可能需要[重置 SSH 密钥](../virtual-machines/linux/troubleshoot-ssh-connection.md)。
...
2. 单击“容器服务”资源，然后单击“概览”。 群集的“主 FQDN”将显示在“概要”下面。 保存此名称以供稍后使用。 
...
现在，你可以在本地系统上使用 Docker 命令行界面 (Docker CLI) 访问 Docker Swarm 群集。 如需安装说明，请参阅 [Install Docker](https://docs.docker.com/engine/installation/)（安装 Docker）。
