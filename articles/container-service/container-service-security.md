---
title: "Azure 容器服务中的容器安全性 | Microsoft Docs"
...
Azure 容器服务中提供的每个协调器具有其自身的安全注意事项。 例如，应该限制对容器服务中的协调器节点进行直接 SSH 访问。 而是应该使用每个协调器的 UI 或命令行工具（例如，适用于 Kubernetes 的`kubectl`），在不访问主机的情况下管理容器环境。 有关详细信息，请参阅[与 Kubernetes、DC/OS 或 Docker Swarm 群集建立远程连接](container-service-connect.md)。
