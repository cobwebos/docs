...
3. 若要将负载均衡器配置为接受发往部署的外部流量，请结合 `--type=LoadBalancer` 运行 `kubectl expose`。 以下命令公开端口 80 上的 Nginx 服务器：
...
若要从 YAML 或 JSON [服务配置文件](https://kubernetes.io/docs/user-guide/services/operations/#service-configuration-file)部署 Kubernetes 容器应用，请通过将以下行添加到服务规范来指定外部负载均衡器：
