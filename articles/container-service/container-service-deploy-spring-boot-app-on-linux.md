...
[Spring Framework] 是一种开放源代码解决方案，可帮助 Java 开发人员创建企业级应用程序。 基于该平台构建的一个更热门的项目是 [Spring Boot]，该项目提供简化的方法来创建独立的 Java 应用程序。

[Docker] 是开源解决方案，可帮助开发人员自动部署、扩展和管理在容器中运行的应用程序。
...
1. 可选步骤：如果想要在端口 80 上（而不是在默认端口 8080 上）运行嵌入式 Tomcat 服务器，（例如，如果要在本地测试 Spring Boot 项目），可以通过执行以下步骤配置端口：

   a.在“横幅徽标”下面，选择“删除上传的徽标”。 将目录更改为 resources 目录；例如：
   ```
   cd src/main/resources
   ```

   b.在“磁贴徽标”下面，选择“删除上传的徽标”。 在文本编辑器中打开 application.yml 文件。
...
1. 当显示“创建容器注册表”边栏选项卡时，输入你的“注册表名称”和“资源组”，为“管理员用户”选择“启用”，然后单击“创建”。
...
1. 当显示“Linux 上的 Web 应用”边栏选项卡时，输入以下信息：

   a.在“横幅徽标”下面，选择“删除上传的徽标”。 为“应用名称”输入唯一名称；例如：“wingtiptoyslinux”。

   b.在“磁贴徽标”下面，选择“删除上传的徽标”。 从下拉列表中选择一个订阅。
...
> [!NOTE]
>
> Azure 会将 Internet 请求自动映射到在标准端口 80 或 8080 上运行的嵌入式 Tomcat 服务器。 但是，如果已将嵌入式 Tomcat 服务器配置为在自定义端口上运行，则需要将一个环境变量添加到 Web 应用（为嵌入式 Tomcat 服务器定义端口）。 为此，请按照以下步骤操作：
>
> 1. 浏览到 [Azure 门户]并登录。
> 
