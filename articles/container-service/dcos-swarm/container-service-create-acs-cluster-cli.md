...
### <a name="log-in-to-your-account"></a>登录到帐户
```azurecli
az login 
```
...
建议为每个群集创建资源组。 指定一个[提供](https://azure.microsoft.com/regions/services/) Azure 容器服务的 Azure 区域。 例如：
...
除非使用各自的开关进行覆盖，其他输入都设置为默认值（请查看以下屏幕）。 例如，协调器默认设置为 DC/OS。 在没有指定的情况下，会根据群集名称创建 DNS 名称前缀。
...
如果在默认位置有一个 SSH RSA 公钥文件 `id_rsa.pub`（或者为 [OS X 和 Linux](../../virtual-machines/linux/mac-create-ssh-keys.md) 或 [Windows](../../virtual-machines/linux/ssh-from-windows.md) 创建了一个），则可使用以下类似命令：
...
> [!TIP]
> [Kubernetes 演练](../kubernetes/container-service-kubernetes-walkthrough.md)介绍了如何使用默认值通过 `az acs create` 创建 Kubernetes 群集。
>
...
```
此命令不会删除在创建容器服务时创建的所有资源（网络和存储）。 要轻松删除所有资源，建议将每个群集部署在不同资源组中。 然后，在不再需要群集时删除资源组。
