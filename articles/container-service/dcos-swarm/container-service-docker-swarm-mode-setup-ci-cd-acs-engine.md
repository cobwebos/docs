...
在 Azure DevOps 中可以[跨环境管理发布](https://azure.microsoft.com/services/devops/pipelines/)。 可以启用持续部署，确保顺利地将应用程序部署到不同的环境（例如开发、测试、预生产和生产）。 可以创建一个代表 Azure 容器服务 Docker Swarm 模式群集的环境。
