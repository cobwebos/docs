...
Open a new query window and then paste in the following script. Run the script to insert data. This script uses the [INSERT](http://msdn.microsoft.com/zh-cn/library/windowsazure/ee336284.aspx) statement to add values to each column.
...
Using just Excel, you can import only one table at a time. A better approach is to use the PowerPivot for Excel add-in, which lets you import and work with multiple tables as a single data set. Working with PowerPivot is beyond the scope of this tutorial, but you can get more information on this [Microsoft web site](http://www.microsoft.com/zh-cn/server-cloud/solutions/business-intelligence/default.aspx#fbid=9ZH5wGSDgf0).
<!--
