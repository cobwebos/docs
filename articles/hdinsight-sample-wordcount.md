...
# 在 HDInsight 中的 Hadoop 群集上运行 MapReduce 单词计数示例
...
- 你必须具有 Azure 帐户。有关注册帐户的选项，请参阅 [azure-trial](/pricing/1rmb-trial/) 页。

- 你必须已经设置了 HDInsight 群集。有关可用于创建这种群集的各种不同方法的说明，请参阅 [Azure HDInsight 入门][hdinsight-get-started]或[设置 HDInsight 群集](hdinsight-provision-clusters)。

- 你必须已经安装了 Azure PowerShell，并且已将其配置为可用于你的帐户。有关如何进行此配置的说明，请参阅[安装和配置 Azure PowerShell][powershell-install-configure]。
...
4. 运行以下命令，将 MapReduce 作业输出从 Blob 下载到工作站：
	...
	*/example/data/WordCountOutput* 文件夹是你在运行 MapReduce 作业时指定的输出文件夹。 *part-r-00000* 是 MapReduce 作业输出的默认文件名。此文件将下载到本地文件夹中的相同文件夹结构。例如，在以下屏幕快照中，当前文件是 C 根文件夹。该文件将下载到 *C:\example\data\WordCountOutput* 文件夹。
...
<h2><a id="java-code"></a>WordCount MapReduce 程序的 Java 代码</h2>
