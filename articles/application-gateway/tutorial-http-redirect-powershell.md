...
本教程需要 Azure PowerShell 模块 3.6 或更高版本。 可以运行 `Get-Module -ListAvailable AzureRM` 来查找版本。 如果需要升级，请参阅[安装 Azure PowerShell 模块](/powershell/azure/azurerm/install-azurerm-ps)。 若要运行本教程中的命令，还需要运行 `Connect-AzureRmAccount` 以创建与 Azure 的连接。
