...
创建[应用程序网关](application-gateway-multi-site-overview.md)时可以使用 Azure Powershell 配置[多个网站的托管](application-gateway-introduction.md)。 本教程中使用虚拟机规模集创建后端池。 然后，基于所拥有的域配置侦听器和规则，以确保 Web 流量可到达池中的相应服务器。 本教程假定你拥有多个域，并使用示例 *www.contoso.com* 和 *www.fabrikam.com*。
...
如果选择在本地安装并使用 PowerShell，则本教程需要 Azure PowerShell 模块版本 1.0.0 或更高版本。 若要查找版本，请运行 `Get-Module -ListAvailable Az`。 如果需要进行升级，请参阅 [Install Azure PowerShell module](/powershell/azure/install-az-ps)（安装 Azure PowerShell 模块）。 如果在本地运行 PowerShell，则还需运行 `Connect-AzAccount` 以创建与 Azure 的连接。
...
使用 *New-AzApplicationGatewayBackendAddressPool* 为应用程序网关创建名为 *contosoPool* 和 [fabrikamPool](/powershell/module/az.network/new-azapplicationgatewaybackendaddresspool) 的后端池。 使用 [New-AzApplicationGatewayBackendHttpSettings](/powershell/module/az.network/new-azapplicationgatewaybackendhttpsetting) 配置池的设置。
...
使用 *New-AzApplicationGatewayHttpListener* 以及前面创建的前端配置和前端端口创建名为 *contosoListener* 和 [fabrikamListener](/powershell/module/az.network/new-azapplicationgatewayhttplistener) 的侦听器。 侦听器需要使用规则来了解哪个后端池用于传入流量。 使用 *New-AzApplicationGatewayRequestRoutingRule* 创建名为 *contosoRule* 和 [fabrikamRule](/powershell/module/az.network/new-azapplicationgatewayrequestroutingrule) 的基本规则。
