...
**原因：** 建立 TCP 连接并完成 TLS 握手后（如果启用了 TLS），应用程序网关会将探测作为 HTTP GET 请求发送到后端服务器。 如上所述，默认探测针对 \<协议\>://127.0.0.1:\<端口\>/，并将 200 至 399 范围内的响应状态代码视为“正常”。 如果服务器返回任何其他状态代码，则将服务器标记为“不正常”并显示此消息。
...
对于要信任的 TLS/SSL 证书，后端服务器的证书必须由应用程序网关的受信任存储区中包含的 CA 颁发。 如果该证书不是由受信任的 CA 颁发（例如，使用了自签名证书），则用户应将颁发者的证书上传到应用程序网关。
...
**原因：** 在将服务器视为正常之前，应用程序网关 v2 的端到端 SSL 要求验证后端服务器的证书。
要使 TLS/SSL 证书受信任，必须由应用程序网关的受信任存储区中包含的 CA 颁发后端服务器证书。 如果该证书不是由受信任的 CA 颁发（例如，使用了自签名证书），则用户应将颁发者的证书上传到应用程序网关。
...
**原因：** 应用程序网关检查后端 HTTP 设置中指定的主机名是否与后端服务器的 TLS/SSL 证书所显示的 CN 的名称相匹配。 这是 Standard_v2 和 WAF_v2 SKU 的行为。 Standard 和 WAF SKU 的服务器名称指示 (SNI) 设置为后端池地址中的 FQDN。
...
**原因：** 每个证书都带有有效性范围，且 HTTPS 连接不安全，除非服务器的 TLS/SSL 证书有效。 当前日期必须在**生效**和**失效**日期范围内。 否则，证书将被视为无效，并成为一个安全隐患。在这种情况下，应用程序网关会将后端服务器标记为“不正常”。

**解决方案：** 如果你的 TLS/SSL 证书已过期，请与你的供应商一起续订证书，并将服务器设置更新为新证书。 如果它是自签名证书，则必须生成有效的证书，并将根证书上传到应用程序网关的 HTTP 设置。 为此，请执行以下步骤：
...
**消息：** 无法验证后端证书的有效性。 若要找出原因，请检查与错误代码 {errorCode} 关联的消息的 OpenSSL 诊断
