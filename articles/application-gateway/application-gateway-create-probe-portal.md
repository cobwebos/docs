...
1. 登录到 Azure[门户](https://portal.azure.com)。 如果您还没有帐户，您可以注册一[个月的免费试用](https://azure.microsoft.com/free)
...
4. 在“添加运行状况探测”页上填写探测的所需信息，并在完成后选择“确定”。********
   ...
   |**Host**|例如  contoso.com|此值是应用程序服务器上运行的虚拟主机的名称（不同于 VM 主机名）。 探测发送到 (协议)://(主机名):(http 设置中的端口)/urlPath。  仅当应用程序网关上配置了多站点时，此项设置才适用。 如果应用程序网关是为单个站点配置的，请输入“127.0.0.1”。|
   ...
   |超时(秒)****|30|探头在超时前等待的时间量。如果在此超时期间未收到有效的响应，则探测器将标记为失败。 超时间隔必须足够长，以便进行 http 调用，确保后端运行状况页可用。 请注意，超时值不能大于此探测设置中使用的“间隔”值，或者要与此探测关联的 HTTP 设置中的“请求超时”值。|
|**不正常的阈值**|3|系统认为不正常的连续失败尝试次数。 阈值可以设置为 1 或更大值。|
