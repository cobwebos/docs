...
当您使用 Internet Explorer 中的短名称 URL 访问应用程序网关时，您启用了基于 Cookie 的关联设置，[http://website](http://website/)例如：
...
为了解决此问题，应该使用 FQDN 来访问应用程序网关。 例如，使用[http://website.com](https://website.com/)或[http://appgw.website.com](http://appgw.website.com/)。
...
1. 在 [Azure 门户](https://portal.azure.com/)中找到你的资源，然后单击“诊断日志”。****
   ...
   对于应用程序网关，有三个日志可用：访问日志、性能日志、防火墙日志
...
- **请求查询**– 这指示收到请求的目标服务器。
- **服务器路由**：收到请求的后端池实例。
- X-AzureApplicationGateway-LOG-ID****：用于请求的相关 ID。 它可用于排查后端服务器上的流量问题。 例如：X-Azure 应用程序网关-CACHE-HIT=0&服务器-路由=10.0.2.4。

  - 服务器状态****： 应用程序网关接收从后端的 HTTP 响应代码。
...
4. 最有可能的是，您将对解密的 HTTPS 流量感兴趣，并且您可以通过选择**工具** > **Fiddler 选项**来启用 HTTPS 解密，并选中"解密**HTTPS 流量**"复选框。
...
6. 重现问题后，通过选择"**File** > **文件保存** > **所有会话"** 来保存文件以供审阅。 . 
...
- **示例 A：** 您找到一个会话日志，该会话日志请求是从客户端发送的，并且该日志转到应用程序网关的公共 IP 地址，单击此日志以查看详细信息。  右侧下框中的数据是应用程序网关返回给客户端的数据。 选择"RAW"选项卡并确定客户端是否收到"**设置-Cookie：ARRAffinity=** *ARRAffinity值*"。 如果未看到任何 Cookie，则表示未设置会话相关性，或者应用程序网关未将 Cookie 应用回到客户端。
...
- **示例 B：** 下一个会话日志后跟前一个日志是客户端响应回应用程序网关，该网关已设置 ARRAAFFINITY。 如果 ARRAffinity Cookie ID 匹配，则数据包应会发送到前面使用的同一个后端服务器。 检查 http 通信的后面几行，以查看客户端的 ARRAffinity Cookie 是否在发生变化。
