---
title: Azure 应用程序网关的 Azure 安全基线
description: Azure 应用程序网关的 Azure 安全基线
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/05/2020
...
ms.locfileid: "84485520"
---
# <a name="azure-security-baseline-for-azure-application-gateway"></a>Azure 应用程序网关的 Azure 安全基线

Azure 应用程序网关的 Azure 安全基线包含有助于改进部署安全状况的建议。
...
**指南**：请确保所有虚拟网络 Azure 应用程序网关子网部署都将网络安全组（NSG）与特定于应用程序受信任端口和源的网络访问控制结合使用。 虽然 Azure 应用程序网关上支持网络安全组，但必须遵守某些限制和要求，以便 NSG 和 Azure 应用程序网关正常运行。

* [了解有关将 Nsg 与 Azure 应用程序网关配合使用的限制和要求](https://docs.microsoft.com/azure/application-gateway/configuration-overview)
...
**指南**：对于与 Azure 应用程序网关子网关联的网络安全组（nsg），启用 NSG 流日志，并将日志发送到存储帐户以进行流量审核。 还可以将 NSG 流日志发送到 Log Analytics 工作区，并使用流量分析来提供有关 Azure 云中流量流的见解。 流量分析的优势包括能够可视化网络活动、识别热点、识别安全威胁、了解流量流模式，以及查明网络不当配置。

注意：在某些情况下，与 Azure 应用程序网关子网关联的 NSG 流日志不会显示已允许的流量。 如果配置与以下情况相符，则不会在 NSG 流日志中看到允许的流量：
- 你已部署应用程序网关 v2
- 应用程序网关子网上有 NSG
- 已在该 NSG 上启用 NSG 流日志
...
**指南**：在关键 Web 应用程序之前部署 Azure Web 应用程序防火墙（WAF），以便对传入流量进行额外检查。 Web 应用程序防火墙（WAF）是一项服务（Azure 应用程序网关的一项功能），可提供对 web 应用程序的集中保护，并防范常见的漏洞。 Azure WAF 通过检查入站 web 流量来阻止攻击（例如 SQL 注入、跨站点脚本、恶意软件上传和 DDoS 攻击），从而帮助保护 Azure App Service web 应用。 WAF 基于 OWASP（开放 Web 应用程序安全项目）核心规则集 3.1（仅限 WAF_v2）、3.0 和 2.2.9 中的规则。
...
**指南**：在与 Azure 应用程序网关的生产实例关联的 Azure 虚拟网络上启用 ddos 标准保护，以防范 ddos 攻击。 使用 Azure 安全中心集成威胁情报拒绝与已知的恶意 IP 地址的通信。
...
**指南**：使用虚拟网络服务标记定义网络安全组或 Azure 防火墙上的网络访问控制。 创建安全规则时，可以使用服务标记代替特定的 IP 地址。 通过在规则的相应 "源" 或 "目标" 字段中指定服务标记名称（例如 GatewayManager），可以允许或拒绝相应服务的流量。 Microsoft 会管理服务标记包含的地址前缀，并会在地址发生更改时自动更新服务标记。

对于与 Azure 应用程序网关子网关联的网络安全组（Nsg），必须在 TCP 端口65503-65534 上为应用程序网关 v1 SKU 允许传入 Internet 流量，并允许 v2 SKU 的 TCP 端口65200-65535 （目标子网为 Any and source as GatewayManager service 标记）。 此端口范围是进行 Azure 基础结构通信所必需的。 这些端口受 Azure 证书的保护（处于锁定状态）。 外部实体（包括这些网关的客户）无法在这些终结点上通信。
...
**指南**：定义和实现与 Azure 应用程序网关部署相关的网络设置的标准安全配置。 使用 "Microsoft 网络" 命名空间中的 Azure 策略别名创建自定义策略，以便审核或强制实施 Azure 应用程序网关、Azure 虚拟网络和网络安全组的网络配置。 你还可以使用内置策略定义。
...
**指南**：对与 Azure 应用程序网关子网关联的网络安全组（nsg）和与网络安全和流量流相关的任何其他资源使用标记。 对于单个 NSG 规则，请使用“说明”字段针对允许流量传入/传出网络的任何规则指定业务需求和/或持续时间等。
...
**指南**：使用 Azure 活动日志监视网络资源配置，并检测与 Azure 应用程序网关部署相关的网络设置和资源的更改。 在 Azure Monitor 中创建警报，以便在对关键网络设置或资源发生更改时触发。
...
**指南**： Microsoft 维护用于 Azure 资源的时间源，例如 Azure App Service。
...
**指南**：对于控制平面审核日志记录，启用 Azure 活动日志诊断设置，并将日志发送到 Log Analytics 工作区、azure 事件中心或 azure 存储帐户。 使用 Azure 活动日志数据，您可以确定用于保护 Azure 应用程序网关子网的 Azure 应用程序网关和相关资源（如网络安全组（Nsg））在控制平面级别执行的任何写入操作（PUT、POST、DELETE）的 "操作内容、操作人员和操作时间"。

除了活动日志之外，还可以为 Azure 应用程序网关部署配置诊断设置。 诊断设置用于配置将资源的平台日志和指标导出到所选目标（存储帐户、事件中心和 Log Analytics）的流导出。

Azure 应用程序网关还提供与 Azure 应用程序 Insights 的内置集成。 Application Insights 收集日志、性能和错误数据。 Application Insights 会自动检测性能异常，并提供强大的分析工具来帮助你诊断问题并了解你的 web 应用的使用情况。 可以启用连续导出，将遥测从 Application Insights 导出到一个集中的位置，以使数据的长度超过标准保持期。

* [如何启用 Azure 活动日志的诊断设置](https://docs.microsoft.com/azure/azure-monitor/platform/diagnostic-settings-legacy)

* [如何为 Azure 应用程序网关启用诊断设置](https://docs.microsoft.com/azure/application-gateway/application-gateway-diagnostics)
...
**指南**：启用 Azure 活动日志诊断设置和 Azure 应用程序网关的诊断设置，并将日志发送到 Log Analytics 工作区。 在 Log Analytics 中执行查询以搜索术语、识别趋势、分析模式以及基于收集的数据提供其他许多见解。

对于所有部署的网络资源（包括 Azure 应用程序的网关），使用 Azure Monitor 来全面了解所有已部署的网络资源的运行状况和指标。
...
* [如何将 Azure Monitor 用于网络](https://docs.microsoft.com/azure/azure-monitor/insights/network-insights-overview)
...
**指南**：在关键 Web 应用程序之前部署 Azure Web 应用程序防火墙（WAF） v2 SKU，以便对传入流量进行额外检查。 Web 应用程序防火墙（WAF）是一项服务（Azure 应用程序网关的一项功能），可提供对 web 应用程序的集中保护，并防范常见的漏洞。 Azure WAF 通过检查入站 web 流量来阻止攻击（例如 SQL 注入、跨站点脚本、恶意软件上传和 DDoS 攻击），从而帮助保护 Azure App Service web 应用。 WAF 基于 OWASP（开放 Web 应用程序安全项目）核心规则集 3.1（仅限 WAF_v2）、3.0 和 2.2.9 中的规则。

启用 Azure 活动日志诊断设置和 Azure WAF 的诊断设置，并将日志发送到 Log Analytics 工作区。 在 Log Analytics 中执行查询以搜索术语、识别趋势、分析模式以及基于收集的数据提供其他许多见解。 可以根据 Log Analytics 工作区查询来创建警报。

对于所有部署的网络资源（包括 Azure 应用程序的网关），使用 Azure Monitor 来全面了解所有已部署的网络资源的运行状况和指标。 在 "网络 Azure Monitor" 控制台中，可以查看和创建 Azure 应用程序网关的警报。
...
**指南**：在关键 Web 应用程序之前部署 Azure Web 应用程序防火墙（WAF） v2，以便对传入流量进行额外检查。 Web 应用程序防火墙（WAF）是一项服务（Azure 应用程序网关的一项功能），可提供对 web 应用程序的集中保护，并防范常见的漏洞。 Azure WAF 通过检查入站 web 流量来阻止攻击（例如 SQL 注入、跨站点脚本、恶意软件上传和 DDoS 攻击），从而帮助保护 Azure App Service web 应用。

为 Azure 应用程序网关部署配置诊断设置。 诊断设置用于配置将资源的平台日志和指标导出到所选目标（存储帐户、事件中心和 Log Analytics）的流导出。
...
**指南**：不适用;Azure 应用程序网关不会处理或产生用户可访问的 DNS 相关日志。
...
**指南**：通过 AZURE ACTIVE DIRECTORY （AD）控制对 Azure 应用程序的网关的控制平面访问。 Azure AD 没有默认密码。
...
**指南**：使用 Azure 应用注册（服务主体）来检索令牌，该令牌可用于通过 API 调用与 Azure 应用程序网关进行交互。
...
**指南**：Azure AD 提供有助于发现陈旧帐户的日志。 此外，请使用 Azure 标识访问评审来有效管理组成员身份、对企业应用程序的访问和角色分配。 可以定期评审用户的访问权限，确保只有适当的用户才持续拥有访问权限。
...
**指南**：你有权访问 Azure AD 登录活动、审核和风险事件日志源，以便与任何 SIEM/监视工具集成。
...
**指南**：使用 Azure AD Identity Protection 和风险检测功能来配置对检测到的与用户标识相关的可疑操作的自动响应。 还可以将数据引入 Azure Sentinel 中以便进一步调查。
...
**指南**：不适用;客户密码箱不适用于 Azure 应用程序的网关。
...
**指导**：为开发、测试和生产实施单独的订阅和/或管理组。 确保所有虚拟网络 Azure 应用程序网关子网部署都将网络安全组（NSG）应用到特定于应用程序受信任端口和源的网络访问控制。 虽然 Azure 应用程序网关上支持网络安全组，但必须遵守某些限制和要求，以便 NSG 和 Azure 应用程序网关正常运行。
...
**指南**：请确保所有虚拟网络 Azure 应用程序网关子网部署都将网络安全组（NSG）与特定于应用程序受信任端口和源的网络访问控制结合使用。 限制仅限受信任位置的出站流量，以帮助减轻数据渗透的威胁。 虽然 Azure 应用程序网关上支持网络安全组，但必须遵守某些限制和要求，以便 NSG 和 Azure 应用程序网关正常运行。
...
**指南**：为 Azure 应用程序网关配置具有 TLS 的端到端加密。
...
**指南**：不适用，Azure 应用程序网关不会将客户数据存储在静态。

Microsoft 为 Azure 应用程序网关管理底层基础结构，并实施了严格控制来防止客户数据丢失或泄露。
...
**指南**：使用 AZURE ACTIVE DIRECTORY （AD）基于角色的访问控制（RBAC）来控制对 Azure 应用程序网关控制平面（Azure 门户）的访问。
...
**指南**：将 Azure Monitor 与 Azure 活动日志结合使用，为生产 Azure 应用程序网关实例以及其他关键或相关资源的发生更改创建警报。
...
**指南**：当前不可用;Azure 安全中心的漏洞评估尚不适用于 Azure 应用程序网关。

Microsoft 对基础平台进行了扫描和修补。 查看可用于 Azure 应用程序网关的安全控制，以减少与服务配置相关的漏洞。
...
**指南**：尚不可用;Azure 安全中心的漏洞评估尚不适用于 Azure 应用程序网关。
...
**指南**：使用托管标识通过 AZURE ACTIVE DIRECTORY （AD）中的自动托管标识提供 Azure 应用程序网关。 使用托管标识可以向支持 Azure AD 身份验证的任何服务（包括 Key Vault）进行身份验证，无需在代码中放入任何凭据。

使用 Azure Key Vault 安全地存储证书。 Azure Key Vault 是由平台管理的机密存储，可以用来保证机密、密钥和 SSL 证书的安全。 Azure 应用程序网关支持与 Key Vault 集成，从而获取附加到支持 HTTPS 的侦听器上的服务器证书。 此支持仅限于应用程序网关 v2 SKU。

* [如何使用 Azure PowerShell 配置 Key Vault 证书的 SSL 终止](https://docs.microsoft.com/azure/application-gateway/configure-keyvault-ps)
...
**指南**：使用 Azure Web 应用程序防火墙（WAF）时，可以配置 WAF 策略。 WAF 策略包含两种类型的安全规则：由客户创作的自定义规则，以及作为 Azure 托管预配置规则集的集合的托管规则集。 Azure 托管的规则集可轻松针对一组常见的安全威胁来部署保护。 由于此类规则集由 Azure 托管，因此这些规则会根据需要进行更新以预防新的攻击签名。
...
**指南**： Azure 应用程序关不存储客户数据。 但是，如果使用自定义 Azure 策略定义，请使用 Azure DevOps 或 Azure Repos 来安全地存储和管理你的代码。
...
**指南**：在 Azure Key Vault 中备份客户托管证书。

* [如何在 Azure 中备份 key vault 证书](https://docs.microsoft.com/powershell/module/azurerm.keyvault/backup-azurekeyvaultcertificate)
