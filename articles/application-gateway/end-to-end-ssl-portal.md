...
description: 了解如何使用 Azure 门户使用端到端 TLS 加密创建应用程序网关。
...
---
# <a name="configure-end-to-end-tls-by-using-application-gateway-with-the-portal"></a>将应用程序网关与门户一起使用来配置端到端 TLS

本文介绍如何使用 Azure 门户通过 Azure 应用程序网关 v1 SKU 配置端到端传输层安全 （TLS） 加密（以前称为安全套接字层 （SSL） 加密。
...
要使用应用程序网关配置端到端 TLS，需要网关的证书。 后端服务器也需要证书。 网关证书用于根据 TLS 协议规范派生对称密钥。 然后，可以使用该对称密钥来加密和解密发送到网关的流量。 

对于端到端 TLS 加密，必须在应用程序网关中允许右后端服务器。 若要允许这种访问，请将后端服务器的公共证书（也称为“身份验证证书”(v1) 或“受信任的根证书”(v2)）上传到应用程序网关。 添加证书后，可确保应用程序网关仅与已知后端实例通信。 此配置可进一步保护端到端通信。

要了解更多信息，请参阅[使用应用程序网关 的 TLS 终止和端到端 TLS 概述](https://docs.microsoft.com/azure/application-gateway/ssl-overview)。

## <a name="create-a-new-application-gateway-with-end-to-end-tls"></a>使用端到端 TLS 创建新的应用程序网关

要使用端到端 TLS 加密创建新的应用程序网关，您需要首先启用 TLS 终止，同时创建新的应用程序网关。 此操作允许 TLS 加密，以便客户端和应用程序网关之间的通信。 然后，需要在 HTTP 设置中，将后端服务器的证书放入“安全接收方”列表。 此配置允许 TLS 加密，用于应用程序网关和后端服务器之间的通信。 这实现了端到端 TLS 加密。

### <a name="enable-tls-termination-while-creating-a-new-application-gateway"></a>在创建新应用程序网关的同时启用 TLS 终止

要了解更多信息，请参阅[在创建新应用程序网关时启用 TLS 终止](https://docs.microsoft.com/azure/application-gateway/create-ssl-portal)。
...
要使用端到端 TLS 加密配置现有应用程序网关，必须首先在侦听器中启用 TLS 终止。 此操作允许 TLS 加密，以便客户端和应用程序网关之间的通信。 然后，将后端服务器的这些证书放入 HTTP 设置中的“安全接收方”列表内。 此配置允许 TLS 加密，用于应用程序网关和后端服务器之间的通信。 这实现了端到端 TLS 加密。

您需要使用具有 HTTPS 协议的侦听器和启用 TLS 终止的证书。 可以使用符合这些条件的现有侦听器，或者创建新的侦听器。 如果选择前一个选项，则可以忽略以下"在现有应用程序网关中启用 TLS 终止"部分，并直接移动到"为后端服务器添加身份验证/受信任的根证书"部分。
...
5. 上传要用于客户端和应用程序网关之间的 TLS 终止的 PFX 证书。
