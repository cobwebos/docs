...
以下与请求和响应的计时相关的指标可用。 通过分析这些度量值，你可以确定应用程序在中的应用程序是否由于 WAN、应用程序网关、应用程序网关和后端之间的网络或应用程序性能而减慢。
...
- **当前容量单位**
   ...
   使用的容量单位计数。 容量单位用于衡量除固定成本以外，还需支付的费用。 容量单元计算单元、持续连接和吞吐量有三个 qps。 每个容量单位最多包含：1个计算单元，或2500持续连接或 2.22-Mbps 吞吐量。
