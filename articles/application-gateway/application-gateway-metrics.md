...
- **后端连接时间**
  ...
  这包括网络延迟，以及后端服务器的 TCP 堆栈建立新连接所用的时间。 在 TLS 的情况下，它还包括握手所花费的时间。 
...
同理，如果“应用程序网关总时间”出现高峰，但“后端最后一个字节响应时间”稳定，则可能表示应用程序网关出现性能瓶颈，或者客户端与应用程序网关之间的网络出现瓶颈。**** 此外，如果*客户端 RTT*也有相应的峰值，则表明降级是由于客户端和应用程序网关之间的网络。
...
- **当前容量单位数**
   ...
   用于负载平衡流量的容量单位计数。 容量单位有三个决定因素 - 计算单位、持久连接和吞吐量。 每个容量单元最多由：1 个计算单元或 2500 个持久连接或 2.22-Mbps 吞吐量组成。
...
- **当前连接**
   ...
   从客户端到应用程序网关处于活动状态的并发连接的总数
   
- **估计计费容量单位**

  使用 v2 SKU，定价模型由消耗驱动。 容量单位测量在固定价格的基础上按消耗量计收的费用。 *估计计费容量单位*指示用于计费的容量单位数。 计算此值时，*当前容量单位*（负载平衡流量所需的容量单位）和*固定计费容量单位*（保留预配的最小容量单位）之间的值越大。
...
- **固定计费容量单位**
  ...
  根据应用程序网关配置中*的最低规模单位*设置（一个实例转换为 10 个容量单位）保留的最小容量单位数。
   
 - **每秒新连接数**

   从客户端到应用程序网关以及从应用程序网关到后端成员每秒建立的新 TCP 连接的平均数量。
...
- **每个正常主机每分钟的请求数**
  ...
  后端池中每个正常成员在一分钟内收到的平均请求数。 您必须使用*后端池 HttpSettings*维度指定后端池。  
...
### <a name="alert-rules-on-metrics"></a>指标上的警报规则

可基于资源的指标启动警报规则。 例如，如果应用程序网关的吞吐量在指定时间段内高于、低于或等于阈值，警报即可调用 webhook 或给管理员发送电子邮件。

以下示例指导创建警报规则，以在吞吐量违反阈值时给管理员发送电子邮件：

1. 选择 **"添加指标警报**"以打开 **"添加规则**"页。 您还可以从指标页到达此页面。

   ![“添加指标警报”按钮][6]

2. 在 **"添加规则"** 页上，填写名称、条件和通知节，然后选择 **"确定**"。
...
要了解有关 Webhook 以及如何将其与警报一起使用它们的更多内容，请访问[在 Azure 指标警报上配置 Webhook。](../azure-monitor/platform/alerts-webhooks.md)

## <a name="next-steps"></a>后续步骤

* 使用 [Azure Monitor 日志](../azure-monitor/insights/azure-networking-analytics.md)可视化计数器和事件日志。
* [使用 Power BI 博客文章可视化 Azure 活动日志](https://blogs.msdn.com/b/powerbi/archive/2015/09/30/monitor-azure-audit-logs-with-power-bi.aspx)。
