...
- **当前容量单位**
   ...
   用于对流量进行负载平衡的容量单位计数。 容量单元计算单元、持续连接和吞吐量有三个 qps。 每个容量单位的最大数目为：1个计算单元、2500持续连接或 2.22-Mbps 吞吐量。
...
- **估计的计费容量单位**
  ...
  使用 v2 SKU，定价模型由消耗驱动。 容量单位用于衡量除固定成本以外，还需支付的费用。 *估计的计费容量单位*指示预估计费使用的容量单位数。 这是在*当前容量单位*（对流量进行负载平衡所需的容量单位）和固定的可*计费容量单位*（设置的最小容量单位）之间进行较大的值计算得出的。
...
- **固定的可计费容量单位**
  ...
  在应用程序网关配置中，根据*最小缩放单位*设置（一个实例转换为10个容量单位）保留预配的容量单元的最小数目。
