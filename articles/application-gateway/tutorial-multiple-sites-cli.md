...
如果选择在本地安装并使用 CLI，本文要求运行 Azure CLI 2.0.4 或更高版本。 要查找版本，请运行 `az --version`。 如果需要安装或升级，请参阅[安装 Azure CLI](/cli/azure/install-azure-cli)。
...
添加使用[az 网络应用程序网关地址池创建](/cli/azure/network/application-gateway/address-pool#az-network-application-gateway-address-pool-create)来包含后端服务器所需的后端池
```azurecli-interactive
