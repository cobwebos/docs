...
在此示例中，**Description**、**InstanceCount** 和 **GatewaySize** 是可选参数。 InstanceCount**** 的默认值为 2****，最大值为 10****。 **网关大小的**默认值为 **"中等**"。 其他可用值为 Small 和 Large。 **虚拟 IP**和**DnsName**显示为空，因为网关尚未启动。 这些值在网关进入运行状态后创建。
