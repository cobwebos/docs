...
* [指标](application-gateway-metrics.md)：应用程序网关具有多个指标，可帮助您验证系统是否按预期运行。
...
此页面将列出后端池中的每个成员（无论是 NIC、IP，还是 FQDN）。 将显示后端池名称、端口、后端 HTTP 设置名称和运行状况。 运行状况的有效值为 **"健康**、**不健康**"和 **"未知**"。
...
默认情况下，Azure 会生成活动日志。 日志可在 Azure 事件日志存储中保留 90 天。 通过阅读["查看事件"和"活动日志](../monitoring-and-diagnostics/insights-debugging-with-events.md)"一文，了解有关这些日志的更多内容。
