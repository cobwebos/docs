...
- **区域冗余**：应用程序网关或 WAF 部署可以跨多个可用区域，无需在每个区域中使用流量管理器预配单独的应用程序网关实例。 可以选择一个或多个区域来部署应用程序网关实例，以便更灵活地应对区域故障。 应用程序的后端池可以通过类似方式分布在多个可用性区域中。
...
每个容量单元最多由：1 个计算单元或 2500 个持久连接或 2.22-Mbps 吞吐量组成。
...
应用程序网关Standard_v2预配，无需在固定容量为五个实例的手动缩放模式下自动缩放。

固定价格 = 744（小时） = $0.20 = $148.8 <br>
容量单位 = 744 （小时） = 每个实例 10 个容量单位 = 5 个实例 = 每个容量单位小时 0.008 美元 = 297.6 美元
...
容量单价 = 744（小时） = 最大值（连接/秒 25/50 计算单位，吞吐量容量单位 8.88/2.22） = $0.008 = 744 = 4 = 0.008 = $23.81

总价 = $148.8*23.81 = $172.61

如您所见，您只为四个容量单位收费，而不是为整个实例收费。 
...
应用程序网关standard_v2预配一个月，至少有五个实例。 假设没有流量和连接是短暂的，您的价格将是：

固定价格 = 744（小时） = $0.20 = $148.8

容量单价 = 744（小时） = 最大值（连接/秒 0/50 计算单位，吞吐量 0/2.22 容量单位） = $0.008 = 744 = 50 = 0.008 = $297.60

总价 = $148.80+297.60 = $446.4

在这种情况下，即使没有流量，也会为五个实例的全部费用收费。
...
容量单价 = 744（小时） = 最大值（连接/秒 25/50 计算单位，吞吐量 125/2.22 容量单位） = 0.008 美元 = 744 * 57 = 0.008 = $339.26

总价 = $148.80*339.26 = $488.06

在这种情况下，您需要为全部五个实例以及 7 个容量单位（即实例的 7/10）收费。  
...
容量单价 = 744（小时） = 最大值（计算单位最大值（连接/秒计算单位 25/50，WAF RPS 80/10），吞吐量容量单位 8.88/2.22） = 0.0144 * 744 * 8 = 0.0144 = $85.71
