...
ms.locfileid: "64947222"
---
# <a name="autoscaling-and-zone-redundant-application-gateway"></a>自动缩放和区域冗余的应用程序网关 

应用程序网关和 Web 应用程序防火墙 (WAF) 也是 Standard_v2 和 WAF_v2 SKU 提供性能增强功能并增加了对关键新功能，如自动缩放、 区域冗余和支持为静态 Vip 支持下提供的。 标准和 WAF SKU 下的现有功能继续支持中新的 v2 SKU，但有少数例外比较部分中列出。 
...
- **区域冗余**：应用程序网关或 WAF 部署可以跨多个可用性区域，无需预配每个区域使用流量管理器中的单独应用程序网关实例。 可以选择一个区域或多个区域来部署应用程序网关实例，以便确保区域故障复原。 应用程序的后端池可以通过类似方式分布在多个可用性区域中。
...
在以下区域均提供 Standard_v2 和 WAF_v2 SKU:美国中北部、美国中南部、美国西部、美国西部 2、美国东部、美国东部 2、美国中部、欧洲北部、欧洲西部、亚洲东南部、法国中部、英国西部、日本东部、日本西部。 即将推出对其他地区的支持。
...
最多包含每个容量单位：1 个计算单元，或 2500年持久连接或 2.22 Mbps 吞吐量。

计算单元的指南：

- **Standard_v2** -每个计算单元是能够每秒含 RSA 2048 位密钥的 TLS 证书的大约 50 个连接。
- **WAF_v2** -每个计算单元是能够每秒大约 10 个并发请求的 70%的流量的 70-30%组合请求不超过 2 KB 获取/发布和剩余更高版本。 WAF 性能当前不受响应大小。
...
一个月内预配应用程序网关 WAF_v2，在此期间它接收 25 新 SSL 连接数/秒，平均 8.88 Mbps 数据传输，并每秒 80 请求。 假设连接较短，而且，应用程序的计算单元计算每个计算单位支持 10 个 RPS，您的价格将为：
...
- **自动缩放**-可以通过启用自动缩放应用程序网关和 WAF v2 Sku 将增加或减少根据应用程序流量需求。 此模式提供了更好地为你的应用程序的弹性，无需猜测应用程序网关大小或实例计数。 此模式还可以通过不要求若要运行网关在预计的最大流量负载的预配的峰值容量来节省成本。 客户必须指定的最小值和 （可选） 最大实例计数。 最小容量可确保，应用程序网关和 WAF v2 执行不低于指定甚至在没有出现的流量的最小实例计数。 即使在没有任何流量的情况下，将为此最小容量计费您。 您还可以选择指定最大实例计数，以确保应用程序网关不能扩展到指定数量的实例。 将继续付费网关提供服务的流量。 实例计数的范围可以介于 0 到 125。 如果未指定，默认值为最大实例计数为 20。
- **手动**-或者，可以选择手动模式下，网关将不自动缩放。 在此模式下，如果将多个通信发送的哪些应用程序网关或 WAF 是能够处理，它可能导致流量丢失。 手动模式下使用指定的实例计数是必需的。 实例计数可能会不同于 1 到 125 实例。
...
> [!NOTE]
> 自动缩放和区域冗余的应用程序网关现在支持 SKU[默认运行状况探测](application-gateway-probe-overview.md#default-health-probe)自动监视后端池中的所有资源的运行状况并突出显示将被视为这些后端成员不正常。 默认运行状况探测将会自动为所有你尚未为其设置的任何自定义探测配置这些后端配置。 若要了解详细信息，请参阅[运行状况探测应用程序网关中](application-gateway-probe-overview.md)。
