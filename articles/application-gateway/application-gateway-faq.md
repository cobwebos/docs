...
可以。 可以在不中断的情况下，设置连接排出以更改后端池内的成员。 此设置允许你继续将现有连接发送到以前的目标，直到该连接关闭或可配置的超时过期。 连接排出仅等待当前正在进行的连接完成。 应用程序网关不能识别应用程序会话状态。
...
WAF 目前支持 CRS [2.2.9](application-gateway-crs-rulegroups-rules.md#owasp229)和[3.0](application-gateway-crs-rulegroups-rules.md#owasp30)。 这些规则为打开的 Web 应用程序安全项目（OWASP）所标识的大多数前10个漏洞提供基准安全： 
