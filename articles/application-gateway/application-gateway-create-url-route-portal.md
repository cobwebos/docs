...
本示例将创建三个虚拟机，用作应用程序网关的后端服务器。 还需要在虚拟机上安装 IIS, 以验证应用程序网关是否按预期方式工作。

1. 在 Azure 门户中，选择“创建资源”。
2. 在热门列表中选择 " **Windows Server 2016 Datacenter** "。
3. 输入虚拟机的以下值：

    - **资源组**中, 选择 "**新建**", 然后键入*myResourceGroupAG*。
    - **虚拟机名称**: *myVM1*
    - **区域**：*反馈美国东部*
    - **用户名**: *azureuser*
    - **密码**：*Azure123456!* -
...
6. 对于 "**虚拟网络**", 请选择 "**新建**", 然后键入虚拟网络的以下值:
   ...
   - 第一个子网名称的*myBackendSubnet*
   - *10.0.1.0/24* - 子网地址空间。
   - *myAGSubnet* -第二个子网名称。
   - *10.0.0.0/24* - 子网地址空间。
7. 选择“确定”。

8. 确保为子网选择了 "**网络接口**" 下的**myBackendSubnet** , 然后选择 **"下一步":管理**。
9. 选择“关闭”以禁用启动诊断。
10. 单击 "**查看 + 创建**", 查看 "摘要" 页上的设置, 然后选择 "**创建**"。
11. 再创建两个虚拟机*myVM2*和*myVM3* , 并将其放置在*MyVNet*虚拟网络和*myBackendSubnet*子网中。
...
1. 在“基本信息”选项卡上，输入这些值作为以下应用程序网关设置：
   ...
   - **区域**-选择 "美国**东部**"。
        ...
        ![新建应用程序网关：基本](./media/application-gateway-create-gateway-portal/application-gateway-create-basics.png)
...
3. 在 "**后端目标**" 下, 选择下拉列表中的 "**虚拟机**"。

5. 在 "**目标**" 下, 选择**myVM1**的网络接口。
6. 选择 **添加** 。
7. 重复此步骤, 使用*myVM2*作为目标添加*映像*后端池, 并使用*myVM3*作为目标添加*视频*后端池。
...
8. 对于 "**路径**", 请键入 */images/* \*。
9. 对于 "**路径规则名称**", 请键入*Images*。
10. 对于 " **HTTP 设置**", 请选择**myHTTPSetting**
11. 对于**后端目标**, 选择 "**映像**"。
12. 选择 "**添加**" 以保存路径规则, 并返回到 "**添加路由规则**" 选项卡。
13. 重复此步骤以添加其他视频规则。
14. 选择 "**添加**" 以添加路由规则并返回到 "**配置**" 选项卡。
15. 在完成时选择“下一步:标记”，然后选择“下一步:**审阅 + 创建”** 。
...
2. 复制该公共 IP 地址，并将其粘贴到浏览器的地址栏。 例如, http:\//52.188.72.175: 8080。
   ...
   端口8080上的侦听器将此请求路由到默认后端池。

3. 将 URL 更改为*http://&lt;&gt;ip 地址: 8080/images/test.htm*, 将 ip &lt;地址&gt;替换为你的 ip 地址, 你应看到类似于以下示例的内容:

    ![在应用程序网关中测试映像 URL](./media/application-gateway-create-url-route-portal/application-gateway-iistest-images.png)

   端口8080上的侦听器将此请求路由到*映像*后端池。

4. 将 URL 更改为*http://&lt;&gt;ip 地址: 8080/video/test.htm*, 将 ip &lt;地址&gt;替换为你的 ip 地址, 你应看到类似于以下示例的内容:

    ![在应用程序网关中测试视频 URL](./media/application-gateway-create-url-route-portal/application-gateway-iistest-video.png)

   端口8080上的侦听器将此请求路由到*视频*后端池。
