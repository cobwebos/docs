...
使用 [az network application-gateway address-pool create](/cli/azure/network/application-gateway#az-network_application_gateway_address_pool_create) 添加包含后端服务器所需的名为 *contosoPool* 和 *fabrikamPool* 的后端池。
...
使用 [az network application-gateway http-listener create](/cli/azure/network/application-gateway#az-network_application_gateway_http_listener_create) 添加路由流量所需的名为 *contosoListener* 和 *fabrikamListener* 的侦听器。
