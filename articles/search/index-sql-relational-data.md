---
title: 为导入和索引建模 SQL 关系数据
titleSuffix: Azure Cognitive Search
description: 了解如何为 Azure 认知搜索中的索引和全文搜索，对关系数据进行建模，并将其取消标准化为平面结果集。
author: HeidiSteen
manager: nitinme
ms.author: heidist
...
---
# <a name="how-to-model-relational-sql-data-for-import-and-indexing-in-azure-cognitive-search"></a>如何为 Azure 认知搜索中的导入和索引建模关系 SQL 数据

Azure 认知搜索接受平面行集作为[索引管道](search-what-is-an-index.md)的输入。 如果源数据源自 SQL Server 关系数据库中的联接表，本文将介绍如何构造结果集，以及如何在 Azure 认知搜索索引中对父子关系建模。
...
使用一对多关系的难题之一是，在联接的表上构建的标准查询将返回不规范的数据，这在 Azure 认知搜索情况下不能正常工作。 请考虑以下用于联接旅馆和房间的示例。
...
此查询的结果返回所有宾馆字段，后跟所有房间字段，并为每个房间值重复使用初步酒店信息。
   ...
   ![非规范化数据，添加会议室字段时的冗余宾馆数据](media/index-sql-relational-data/denormalize-data-query.png "非规范化数据，添加会议室字段时的冗余宾馆数据")


虽然此查询在表面上成功（提供所有数据都在一个平面行集中），但它在为预期的搜索体验交付正确的文档结构时失败。 在编制索引期间，Azure 认知搜索将为每个行引入创建一个搜索文档。 如果搜索文档看起来像上述结果，就会发现重复-7 个单独的单独文档用于克隆的凸透镜。 "对佛罗里达州的酒店" 的查询将只为 "克隆" 凸透镜返回7个结果，并将其他相关饭店推送到搜索结果中。
...
若要传递预期的搜索体验，你的数据集应为 Azure 认知搜索中的每个搜索文档包含一行。 在我们的示例中，我们希望每个酒店都有一行，但我们也希望用户能够搜索他们关心的其他房间相关的字段，例如每夜费率、床位的大小和数量，或海滩的视图，所有这些都是会议室详细信息的一部分。
...
2. 创建一个由父表（`SELECT * from dbo.Hotels$`）中的所有字段组成的视图，并添加一个新的*会议室*字段，其中包含嵌套查询的输出。 `SELECT * from dbo.Rooms$` 上的**FOR JSON AUTO**子句将输出作为 JSON 进行结构。 
...
> [!NOTE]
> 此方法假定嵌入的 JSON 小于 SQL Server 的[最大列大小限制](https://docs.microsoft.com/sql/sql-server/maximum-capacity-specifications-for-sql-server)。 如果数据不合适，可以尝试编程方法，如[示例：为 Azure 的 AdventureWorks 清单数据库建模认知搜索](search-example-adventureworks-modeling.md)。

 ## <a name="use-a-complex-collection-for-the-many-side-of-a-one-to-many-relationship"></a>为一对多关系的 "多" 方使用复杂集合

在 Azure 认知搜索端，使用嵌套 JSON 创建模型为一对多关系的索引架构。 您在上一节中创建的结果集通常对应于下面提供的索引架构（为了简洁起见，我们剪切了某些字段）。
...
给定上一个结果集和上述索引架构后，您将拥有成功索引操作所需的所有组件。 平展数据集满足索引要求，但仍保留详细信息。 在 Azure 认知搜索索引中，搜索结果将轻松进入基于宾馆的实体，同时保留各个房间及其属性的上下文。
