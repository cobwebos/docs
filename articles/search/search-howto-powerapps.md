...
1. 依次选择“+新建自定义连接器”、“从空白开始创建”。 
    ...
    :::image type="content" source="./media/search-howto-powerapps/1-3-create-blank.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 在“常规”页中输入信息：
   ...
   * 对于“基 URL”，只需输入“/”
    ...
    :::image type="content" source="./media/search-howto-powerapps/1-5-general-info.png" alt-text="“自定义连接器”菜单" border="true":::

1. 在“安全”页中，将“API 密钥”设置为“身份验证类型”，并将参数标签和参数名称都设置为“api-key”。 对于“参数位置”，请选择“标头”，如下所示。

    :::image type="content" source="./media/search-howto-powerapps/1-6-authentication-type.png" alt-text="“自定义连接器”菜单" border="true":::

1. 在“定义”页中，选择“+ 新建操作”以创建用于查询索引的操作。 输入值“查询”作为操作 ID 的摘要和名称。 输入说明，例如“查询搜索索引”。

    :::image type="content" source="./media/search-howto-powerapps/1-7-new-action.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 向下滚动。 在“请求”中，选择“+ 从示例导入”按钮，以配置对你的搜索服务的查询请求：
   ...
   * 对于“标头”，请键入 `Content-Type`。 
     ...
     Power Apps 将使用语法从查询中提取参数。 请注意，我们显式定义了搜索字段。 
       ...
       :::image type="content" source="./media/search-howto-powerapps/1-8-1-import-from-sample.png" alt-text="“自定义连接器”菜单" border="true":::

1. 单击“导入”以自动填充请求。 通过单击每个参数旁边的“…”符号来完成参数元数据的设置。 每次更新参数后，可以单击“后退”返回到“请求”页。

   :::image type="content" source="./media/search-howto-powerapps/1-8-2-import-from-sample.png" alt-text="“自定义连接器”菜单" border="true":::

1. 对于 search：将 `*` 设置为默认值，将 required 设置为 False，将 visibility 设置为 none。  

    :::image type="content" source="./media/search-howto-powerapps/1-10-1-parameter-metadata-search.png" alt-text="“自定义连接器”菜单" border="true":::

1. 对于 select：将 `HotelName,Description,Address/City` 设置为默认值，将 required 设置为 False，将 visibility 设置为 none。   

    :::image type="content" source="./media/search-howto-powerapps/1-10-4-parameter-metadata-select.png" alt-text="“自定义连接器”菜单" border="true":::

1. 对于 api-version：将 `2020-06-30` 设置为默认值，将 required 设置为 True，将 visibility 设置为 internal。   

    :::image type="content" source="./media/search-howto-powerapps/1-10-2-parameter-metadata-version.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 选择该连接器，展开操作列表，然后选择“查看属性”。
    ...
    :::image type="content" source="./media/search-howto-powerapps/1-11-1-test-connector.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 在“操作”中，单击“测试操作”按钮。 如果测试成功，应会看到 200 状态，并且在响应正文中，应会看到描述搜索结果的 JSON。
    ...
    :::image type="content" source="./media/search-howto-powerapps/1-11-2-test-connector.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 在左侧，展开“应用” > “+ 新建应用” > “画布”。  
    ...
    :::image type="content" source="./media/search-howto-powerapps/2-1-create-canvas.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 进入 Studio 后，选择“数据源”选项卡，然后单击刚刚创建的新连接器。 在本例中，该连接器名为 AzureSearchQuery。 单击“添加连接”。
   ...
   输入查询 API 密钥。
    ...
    :::image type="content" source="./media/search-howto-powerapps/2-3-connect-connector.png" alt-text="“自定义连接器”菜单" border="true":::

    现在，AzureSearchQuery 是可从应用程序使用的数据源。

1. 在“插入”选项卡上，将几个控件添加到画布。

    :::image type="content" source="./media/search-howto-powerapps/2-4-add-controls.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 插入以下元素：
   ...
   * 垂直库（将其命名为 galleryResults）
    ...
    :::image type="content" source="./media/search-howto-powerapps/2-5-controls-layout.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 要使“搜索”按钮发出查询，请将以下操作粘贴到 OnSelect 中： 
   ...
   以下屏幕截图显示了 OnSelect 操作的公式栏。
    ...
    :::image type="content" source="./media/search-howto-powerapps/2-6-search-button-event.png" alt-text="“自定义连接器”菜单" border="true":::
...
1. 将“垂直库”控件链接到完成上一步骤时创建的 azResult 集合。 
   ...
   * 单击“编辑字段”，并选择要可视化的字段。
    ...
    :::image type="content" source="./media/search-howto-powerapps/2-7-gallery-select-fields.png" alt-text="“自定义连接器”菜单" border="true":::   
 
1. 按 F5 预览应用。  

    :::image type="content" source="./media/search-howto-powerapps/2-8-3-final.png" alt-text="“自定义连接器”菜单" border="true":::    
...
<!--     Remember that the fields can be set to calculated values.
    ...
    :::image type="content" source="./media/search-howto-powerapps/2-8-2-final.png" alt-text="“自定义连接器”菜单" border="true":::         -->
