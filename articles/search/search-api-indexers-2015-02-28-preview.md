...
**索引器**是连接数据源与目标搜索索引的资源。 可通过以下方式使用索引器： 
...
**数据源**指定需要编制索引的数据、用于访问数据的凭据，以及用于启用 Azure 搜索的策略，以便有效地标识数据中的更改（如数据库表中已修改或已删除的行）。 数据源定义为独立的资源，以便于可供多个索引器使用。
...
所有服务请求都需要 HTTPS。 **创建数据源**请求可使用 POST 或 PUT 方法构造。 使用 POST 时，必须在请求正文中提供数据源名称以及数据源定义。 对于 PUT，该名称是 URL 的一部分。 如果数据源不存在，则创建它。 如果它已经存在，则将其更新到新定义。 
...
此策略仅可与表一起使用；不能与视图一起使用。 你需要对要使用的表启用更改跟踪，然后才能使用此策略。 有关说明，请参阅[启用和禁用更改跟踪](https://msdn.microsoft.com/library/bb964713.aspx)。    
...
数据删除检测策略旨在有效识别已删除的数据项。 当前，唯一受支持的策略是 `Soft Delete` 策略，它允许根据 `soft delete` 列的值或数据源中的属性标识删除的项。 可按如下方式指定此策略：
