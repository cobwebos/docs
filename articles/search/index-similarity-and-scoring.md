...
## <a name="featuresmode-parameter-preview"></a>featuresMode 参数（预览）

[搜索文档](https://docs.microsoft.com/rest/api/searchservice/preview-api/search-documents)请求具有一个新的[featuresMode](https://docs.microsoft.com/rest/api/searchservice/preview-api/search-documents#featuresmode)参数，该参数可提供有关在字段级别的相关性的其他详细信息。 而 `@searchScore` 是针对文档的全部计算（此查询上下文中的相关文档在此查询的上下文中是如此），通过 featuresMode 可以获取有关单个字段的信息，如结构中所示 `@search.features` 。 该结构包含查询中使用的所有字段（在查询中通过**searchFields**指定的字段，或索引中的可**搜索**字段中的所有字段）。 对于每个字段，可获得以下值：

+ 在字段中找到的唯一标记数
+ 相似性分数，或相对于查询项的字段内容的相似度量值
+ 字词频率，或在字段中找到查询字词的次数

对于目标为 "描述" 和 "标题" 字段的查询，包含的响应可能如下 `@search.features` 所示：
...
你可以在[自定义计分解决方案](https://github.com/Azure-Samples/search-ranking-tutorial)中使用这些数据点，或使用这些信息来调试搜索相关性问题。
