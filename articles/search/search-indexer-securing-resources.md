...
> [!NOTE]
> 除了上面列出的选项之外，对于受网络保护的 Azure 存储帐户，客户可以利用 Azure 认知搜索是 [受信任的 Microsoft 服务](https://docs.microsoft.com/azure/storage/common/storage-network-security#trusted-microsoft-services)这一事实。 这意味着，如果在存储帐户上启用了相应的基于角色的访问控制，则特定搜索服务可以绕过存储帐户的虚拟网络或 IP 限制，并可以访问存储帐户中的数据。 详细信息可在操作 [方法指南](search-indexer-howto-access-trusted-service-exception.md)中找到。 如果存储帐户或搜索服务无法移到不同的区域，则可以使用此选项而不是 IP 限制路由。

选择索引器应使用哪些安全访问机制时，请考虑以下约束：

- 所有 Azure 资源都不支持[服务终结点](https://docs.microsoft.com/azure/virtual-network/virtual-network-service-endpoints-overview)。
...
- 多租户环境的 IP 地址可通过 `AzureCognitiveSearch` 服务标记获得。 [Azure 服务标记](https://docs.microsoft.com/azure/virtual-network/service-tags-overview) 对每个服务都有一个已发布的 IP 地址范围-可通过 [发现 API (预览) ](https://docs.microsoft.com/azure/virtual-network/service-tags-overview#use-the-service-tag-discovery-api-public-preview) 或 [可下载的 JSON 文件](https://docs.microsoft.com/azure/virtual-network/service-tags-overview#discover-service-tags-by-using-downloadable-json-files)获取。 在这两种情况下，IP 范围按区域划分，你只能选取为在其中预配搜索服务的区域分配的 IP 范围。

对于某些数据源，可以直接使用服务标记，而不是枚举 IP 范围的列表， (仍需要显式使用) 搜索服务的 IP 地址。 这些数据源通过设置 [网络安全组规则](https://docs.microsoft.com/azure/virtual-network/security-overview)来限制访问，这种规则在本机上支持添加服务标记，这与 IP 规则（例如 Azure 存储、CosmosDB、azure SQL 等提供的 ip 规则不同） `AzureCognitiveSearch` 是支持直接使用服务标记（除了搜索服务 IP 地址）的数据源：
...
- [SQL 托管实例](https://docs.microsoft.com/azure/search/search-howto-connecting-azure-sql-mi-to-azure-search-using-indexers#verify-nsg-rules)
...
索引器可以利用 [专用终结点](https://docs.microsoft.com/azure/private-link/private-endpoint-overview) 来访问资源，访问被锁定以选择虚拟网络或没有启用任何公共访问权限的资源。
...
客户应调用搜索管理操作， [创建或更新 *共享的专用链接资源* API](https://docs.microsoft.com/rest/api/searchmanagement/sharedprivatelinkresources/createorupdate) ，以便创建到其安全资源的专用终结点连接 (例如，存储帐户) 。 通过此 (出站) 专用终结点连接的流量将仅源自搜索服务特定 "专用" 索引器执行环境中的虚拟网络。
...
然后，客户需要在其安全资源上查找此请求，并 "批准"。 通常，可以通过门户或 [REST API](https://docs.microsoft.com/rest/api/virtualnetwork/privatelinkservices/updateprivateendpointconnection)来完成此操作。
