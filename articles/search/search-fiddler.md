<properties
	pageTitle="如何使用 Fiddler 评估和测试 Azure 搜索 REST API | Microsoft Azure | 托管云搜索服务"
...
# 使用 Fiddler 评估和测试 Azure 搜索 REST API
> [AZURE.SELECTOR]
...
3. 在“编辑器”选项卡上，将构建如以下屏幕截图所示的请求。

  	![][1]

2. 选择“PUT”。

3. 输入一个 URL，其中指定服务 URL、请求属性和 api 版本。要记住的几个要点：
   + 使用 HTTPS 作为前缀。
...
4.	指定请求标头，并将 host 和 api-key 替换为对服务有效的值。
...
6.	单击“检查器”选项卡上，单击“标头”选项卡，然后选择 JSON 格式。应看到文档计数和存储大小（以 KB 为单位）。
