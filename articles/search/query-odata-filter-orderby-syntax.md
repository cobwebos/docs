...
- [$filter语法](search-query-odata-filter.md)
- [$orderby语法](search-query-odata-orderby.md)
- [$select语法](search-query-odata-select.md)
...
### <a name="escaping-special-characters-in-string-constants"></a>在字符串常量中转义特殊字符

OData 中的字符串常量由单个引号分隔。 如果需要构造具有可能本身包含单个引号的字符串常量的查询，则可以通过加倍引用来逃避嵌入的报价。

例如，具有未格式化撇号（如"Alice 的车"）的短语将在 OData 中表示为字符串常量`'Alice''s car'`。

> [!IMPORTANT]
> 以编程方式构造筛选器时，请务必记住从用户输入中转义字符串常量。 这是为了减轻[注入攻击](https://wikipedia.org/wiki/SQL_injection)的可能性，尤其是在使用过滤器实现[安全修整](search-security-trimming-for-azure-search.md)时。
...
**$orderby** 和 **$select** 参数都是较简单表达式的逗号分隔列表。 **$filter** 参数是由较简单的子表达式构成的布尔表达式。 这些子表达式使用逻辑运算符（如[`and`、`or`和`not`](search-query-odata-logical-operators.md)、比较运算符（如[`eq`、、、、`gt``lt`等](search-query-odata-comparison-operators.md)）和集合运算符（如[`any`和`all`](search-query-odata-collection-operators.md)）进行组合。
