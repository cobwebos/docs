---
title: 添加计分配置文件以在搜索结果中提高相关文档
titleSuffix: Azure Cognitive Search
description: 通过添加计分配置文件，提高 Azure 认知搜索结果的搜索排名分数。
manager: nitinme
author: Brjohnstmsft
ms.author: brjohnst
...
# <a name="add-scoring-profiles-to-an-azure-cognitive-search-index"></a>将计分配置文件添加到 Azure 认知搜索索引
 ...
 Azure 认知搜索使用默认评分来计算初始分数，但你可以通过*评分配置文件*自定义计算。 借助计分概要文件，可以更好地控制搜索结果中的项排名。 例如，建议根据创收能力提升项、提升新项或提升库存时间太长的项。  
...
```  
 ...
 此查询对“inn”一词进行搜索，并在当前位置中传递。 请注意，此查询包含其他参数，如 `scoringParameter`。 [搜索文档&#40;&#41;](https://docs.microsoft.com/rest/api/searchservice/Search-Documents)中介绍了查询参数 Azure 认知搜索 REST API。  
...
## <a name="when-to-use-custom-scoring"></a>何时使用自定义计分  
 ...
 基于相关性的排序也通过计分概要文件实现。 请考虑过去使用的可以按价格、日期、评分或相关性排序的搜索结果页。 在 Azure 认知搜索中，计分配置文件驱动 "相关性" 选项。 相关性定义由你控制，基于业务目标和希望提供的搜索体验类型断定。  
...
|`defaultScoringProfile`|在执行搜索请求时，如果未指定任何计分概要文件，则使用默认计分（仅限 [tf-idf](http://www.tfidf.com/)）。<br /><br /> 可以在此处设置默认的计分配置文件名称，导致 Azure 认知搜索在搜索请求中未提供特定配置文件时使用该配置文件。|  
...
##  <a name="bkmk_boostdur"></a>设置 boostingDuration  
 ...
 `boostingDuration` 必须设置为 XSD "dayTimeDuration" 值（ISO 8601 持续时间值的受限子集）的格式。 此模式的模式为： "P [nD] [T [nH] [nM] [nS]]"。  
