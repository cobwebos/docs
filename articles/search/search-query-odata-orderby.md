...
每个子句都有排序条件，可以选择后跟排序方向（`asc` 表示升序，或 `desc` 表示降序）。 如果不指定方向，则默认为升序。 排序条件可以是`sortable`字段的路径，也可以是对 或[`geo.distance`](search-query-odata-geo-spatial-functions.md)函数的[`search.score`](search-query-odata-search-score-function.md)调用。
