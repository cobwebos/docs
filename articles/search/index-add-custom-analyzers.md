...
自定义分析器是*文本分析器*的一种特定类型，包含现有 tokenizer 和可选筛选器的用户定义组合[](search-analyzers.md)。 通过以新方式组合 tokenizer 和筛选器，可以在搜索引擎中自定义文本处理以得到特定结果。 例如，可以使用*字符筛选器*创建自定义分析器，以在标记文本输入之前删除 HTML 标记。
...
可以使用 **REST API** 中的[测试分析器操作](https://docs.microsoft.com/rest/api/searchservice/test-analyzer)来查看分析器如何将给定文本分解成多个标记。
...
|类型|分析器类型来自受支持分析器列表。 请参阅下面**分析器**表中的 [analyzer_type](#AnalyzerTable) 列。|  
...
|-|-|  
|名称|它必须仅包含字母、数字、空格、短划线或下划线，只能以字母数字字符开头和结尾，且最多包含 128 个字符。|  
|类型|字符筛选器类型来自受支持字符筛选器列表。 请参阅下面**字符筛选器**表中的 [char_filter_type](#char-filters-reference) 列。|  
...
|-|-|  
|名称|它必须仅包含字母、数字、空格、短划线或下划线，只能以字母数字字符开头和结尾，且最多包含 128 个字符。|  
|类型|Tokenizer 名称来自受支持 tokenizer 列表。 请参阅下面 **Tokenizer** 表中的 [tokenizer_type](#Tokenizers) 列。|  
...
|-|-|  
|名称|它必须仅包含字母、数字、空格、短划线或下划线，只能以字母数字字符开头和结尾，且最多包含 128 个字符。|  
|类型|标记筛选器名称来自受支持标记筛选器列表。 请参阅下面**标记筛选器**表中的 [token_filter_type](#TokenFilters) 列。|  
