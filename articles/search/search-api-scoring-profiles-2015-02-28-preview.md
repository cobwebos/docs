...
Azure 搜索使用默认计分计算初始分数，但可以通过计分概要文件自定义计算。 计分概要文件使你能够更好地控制搜索结果中项的排名。 例如，你可能想要基于营收潜力提升项，或者提升较新的项，也可能提升库存时间太长的项。

计分概要文件属于索引定义的一部分，由字段、函数和参数组成。

为了让你大致了解计分概要文件的情况，以下示例演示了名为“geo”的简单概要文件。 此文件用于提升在 `hotelName` 字段中具有搜索词的项。 它还使用 `distance` 函数优先提升在当前位置十公里范围内的项。 如果有人搜索“inn”一词，而“inn”恰好是酒店名称的一部分，包含带有“inn”的酒店的文档将在搜索结果中的较高位置出现。
...
计分对排序结果集中的每项计算搜索分数。 搜索结果集中的每项都分配到一个搜索分数，然后从高到低排名。 分数较高的项返回应用程序。 默认返回前 50 个，但可以使用 `$top` 参数返回较少或较多的项（单个响应中最多 1000 个）。
...
搜索分数值可以在整个结果集中重复。 例如，你可能有分数为 1.2 的 10 项，分数为 1.0 的 20 项，分数为 0.5 的 20 项。 当多个命中具有相同的搜索分数时，得分相同项的顺序未定义，并且不稳定。 重新运行查询后，可能会看到项移位。 对于具有相同分数的两项，无法保证先显示哪一个。

## <a name="when-to-use-custom-scoring"></a>何时使用自定义计分
当默认排名行为不足以满足业务目标时，应创建一个或更多计分概要文件。 例如，你可能做出关于搜索相关性应倾向于新添加项的决定。 同样，你可能有一个包含利润率的字段，或其他某些指示营收潜力的字段。 提升为业务带来益处的命中可能是确定使用计分概要文件的一个重要因素。

基于相关性的排序也通过计分概要文件实现。 请考虑过去使用的可以按价格、日期、评分或相关性排序的搜索结果页。 在 Azure 搜索中，计分概要文件驱动“相关性”选项。 相关性定义由你控制，基于业务目标和你希望提供的搜索体验类型断定。
...
若要实现自定义计分行为，请将计分概要文件添加到定义该索引的架构。 在一个索引中可以具有最多 16 个计分概要文件（请参阅[服务限制](search-limits-quotas-capacity.md)），但在任何给定查询中，一次都只能指定一个概要文件。
...
### <a name="weights"></a>权重
计分概要文件的 `weights` 属性指定向字段分配相对权重的名称-值对。 在[示例](#example)中，albumTitle、genre 和 artistName 字段各自提升 1.5、5 和 2。 genre 为何比其他字段提升更高？ 如果对在某种程度上为同类的数据执行搜索（正如 `musicstoreindex` 中的“genre”一样），可能需要在相对权重中产生较大差异。 例如，在 `musicstoreindex` 中，“rock”既作为流派显示，又显示在采用相同组句方式的流派说明中。 如果你希望流派的权重在流派说明之上，genre 字段将需要更高的相对权重。

### <a name="functions"></a>函数
函数在特定上下文需要其他计算时使用。 有效的函数类型为 `freshness`、`magnitude`、`distance` 和 `tag`。 每个函数具有独有的参数。

* 当你希望按照新旧程度提升项时，应使用 `freshness`。 此函数仅可与 datetime 字段 (`Edm.DataTimeOffset`) 结合使用。 请注意 `boostingDuration` 特性仅用于 freshness 函数。
* 当你希望根据数值高低程度提升时，应使用 `magnitude`。 调用此函数的方案包括按照利润率、最高价格、最低价格或下载次数提升。 如果想要反转模式（例如，将价格较低项提升至价格较高项之上），可以将范围反转为从高到低。 假设价格范围从 100 美元到 1 美元，可以将 `boostingRangeStart` 设为 100、`boostingRangeEnd` 设为 1 以提升价格较低的项。 此函数仅可与 double 和 integer 字段结合使用。
* 当你希望按距离或地理位置提升时，应使用 `distance`。 此函数仅可与 `Edm.GeographyPoint` 字段结合使用。
* 当你希望按文档和搜索查询之间共有的标记提升时，应使用 `tag`。 此函数仅可与 `Edm.String` 和 `Collection(Edm.String)` 字段结合使用。
...
定义索引后，请通过上载索引架构（后跟文档）建立索引。 请参阅[创建索引](search-api-2015-02-28-preview.md#CreateIndex)和[添加或更新文档](search-api-2015-02-28-preview.md#AddOrUpdateDocuments)，获取有关这些操作的说明。 建立索引后，应获得适用于搜索数据的函数计分概要文件。
...
内插允许你定义从范围起始至范围结束的计分提升增量的斜率。 可以使用以下内插：

* `Linear`：对于最大和最小范围内的项，应用于项的提升将以恒定递减量完成。 Linear 是计分概要文件的默认内插。
* `Constant`：对于起始和结束范围内的项，将对排名结果应用恒定提升。
* `Quadratic`：与具有恒定递减提升的 Linear 内插相比，Quadratic 最初以较小的速度递减，然后在接近结束范围时，以明显更高的间隔递减。 tag 计分函数中不允许使用此内插选项。
