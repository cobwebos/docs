...
对存储、工作负荷以及索引数量和其他对象的最大限制取决于你是在**免费**、**基本**、**标准**还是**存储优化**定价层[预配 Azure 认知搜索](search-create-service-portal.md)。
...
| 每个文档&nbsp;<sup>2</sup>的所有复杂集合中的最大元素数 |3000 |3000 |3000 |3000 |3000 |3000 |3000 |3000 |
...
<sup>2</sup>每个文档的复杂集合中的元素数非常多，当前导致高存储利用率。 这是一个已知问题。 同时，3000的限制是所有服务层的安全上限。 仅对利用支持复杂类型字段（`2019-05-06`）的最早正式发布（GA） API 版本的索引操作强制实施此限制。 若要使可能使用较早预览 API 版本的客户端（支持复杂类型字段）不会中断，我们不会对使用这些预览 API 版本的索引操作强制实施此限制。 请注意，不应将预览 API 版本用于生产方案，我们强烈建议客户转到最新的 GA API 版本。

<a name="document-limits"></a>

## <a name="document-limits"></a>文档限制 

自 2018 年 10 月起，在任何区域的任何可计费层（基本、S1，S2、S3、S3 HD）创建的任何新服务都不再有任何文档数限制。 虽然在11月/12 月2017以后大多数区域都有无限制的文档计数，但在该日期之后，有几个区域会继续施加文档限制。 根据你创建搜索服务的时间和地点，你可能正在运行仍受文档数限制的服务。

若要确定服务是否有文档限制，请使用[REST API 获取服务统计信息](https://docs.microsoft.com/rest/api/searchservice/get-service-statistics)。 文档限制在响应中反映出来，`null` 指示没有限制。

> [!NOTE]
> 即使没有任何 SKU 特定的文档限制，每个索引仍会受到最大安全限制，以确保服务的稳定性。 此限制来自 Lucene。 每个 Azure 认知搜索文档在内部都作为一个或多个 Lucene 文档建立索引。 每个搜索文档的 Lucene 文档数取决于复杂集合字段中的元素总数。 每个元素都作为一个单独的 Lucene 文档进行索引。 例如，在复杂集合字段中包含3个元素的文档将被索引为 4 Lucene 文档，对于文档本身为3个，对于元素为3。 每个索引的最大 Lucene 文档数约为25000000000。
...
| Blob 索引器：从 blob 中提取的内容的最大字符数 |32,000 |64,000 |4&nbsp;百万 |8&nbsp;百万 |16&nbsp;百万 |空值 |4&nbsp;百万 |4&nbsp;百万 |
...
> [!NOTE]
> 如[索引限制](#index-limits)中所述，索引器还将在每个文档中强制执行3000个元素，每个文档从支持复杂类型（`2019-05-06`）的最新 GA API 版本开始。 这意味着，如果已使用以前的 API 版本创建索引器，则不会受到此限制。 为了保持最大兼容性，使用以前的 API 版本创建的索引器，然后使用 API 版本 `2019-05-06` 或更高版本进行更新，这些限制仍将被**排除**。 客户应注意具有非常大的复杂集合（如前文所述）的不利影响，我们强烈建议创建包含最新 GA API 版本的任何新索引器。

## <a name="synonym-limits"></a>同义词限制

允许的最大同义词映射数因定价层而异。 每个规则最多可以有20个扩展，其中，扩展是等效的。 例如，给定 "cat"，与 "猫"、"猫" 和 "felis" （猫的 genus）的关联将计为3个扩展。
...
<sup>1</sup>在 Azure 认知搜索中，请求正文受到 16 MB 的上限限制，对单个字段或集合的内容施加实际限制，但不受理论限制的限制（请参阅[支持的数据类型](https://docs.microsoft.com/rest/api/searchservice/supported-data-types)，了解有关字段组合和限制的详细信息）。
