...
+ **存储优化**层在专用计算机上运行，与**标准**层相比具有更多的总存储、存储带宽和内存。 优化的存储分为两个级别： L1 和 L2
...
<sup>2</sup>每个文档的复杂集合中的元素数非常多，当前导致高存储利用率。 这是已知问题。 同时，3000的限制是所有服务层的安全上限。 仅对利用支持复杂类型字段（`2019-05-06`）的最早正式发布（GA） API 版本的索引操作强制实施此限制。 若要使可能使用较早预览 API 版本的客户端（支持复杂类型字段）不会中断，我们不会对使用这些预览 API 版本的索引操作强制实施此限制。 请注意，不应将预览 API 版本用于生产方案，我们强烈建议客户转到最新的 GA API 版本。
...
用于调用[实体识别](cognitive-search-skill-entity-recognition.md)、[关键短语提取](cognitive-search-skill-keyphrases.md)、[情绪分析](cognitive-search-skill-sentiment.md)和[语言检测](cognitive-search-skill-language-detection.md)的文本分析资源的[AI 扩充管道](cognitive-search-concept-intro.md)受数据限制的限制。 记录的最大大小应为 50,000 个字符，通过 [`String.Length`](https://docs.microsoft.com/dotnet/api/system.string.length) 进行测量。 如果需要在将数据发送到情绪分析器之前拆分数据，请使用[文本拆分技能](cognitive-search-skill-textsplit.md)。
