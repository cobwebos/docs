...
<sup>1</sup> 免费和预览 SKU 不随附 SLA。 SKU 公开发布以后，就会强制实施 SLA。
...
1. 是否有**服务级别协议 (SLA)** 要求？ 将 SKU 决策缩小到基本或非预览标准。
...
> [!NOTE]
> 如果文档包含无关数据，存储要求可能会过高。 在理想情况下，文档仅包含可搜索的数据或元数据。 二进制数据不可搜索，应该分开存储（或许存储在 Azure 表或 blob 存储中），并且在索引中要有一个字段用于保存外部数据的 URL 参考。 个别文档的最大大小是 16 MB（如果在一次请求中批量上载了多个文档，则小于 16 MB）。 有关详细信息，请参阅 [Azure 搜索中的服务限制](search-limits-quotas-capacity.md)。
>
>
