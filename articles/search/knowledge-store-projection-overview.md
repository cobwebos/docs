...
description: 将 AI 扩充索引管道中的已扩充数据保存并形状到一个知识库，以便在除全文搜索以外的其他方案中使用。 知识存储目前为公共预览版。
manager: nitinme
author: vkurpad
ms.author: vikurpad
...
下图是[有关如何开始使用知识库](knowledge-store-howto.md)的示例中的示例。 如果某个案例包含多种观点，并且每种观点已通过标识其中包含的实体进行扩充，则你可以按如下所示为投影建模。
