...
description: 通过调用 Web API 扩展 Azure 认知搜索技能集的功能。 使用自定义 Web API 技能集成自定义代码。
manager: nitinme
author: luiscabrer
ms.author: luisca
...
---
# <a name="custom-web-api-skill-in-an-azure-cognitive-search-enrichment-pipeline"></a>Azure 认知搜索丰富管道中的自定义 Web API 技能

**自定义 Web API**技能允许您通过调用提供自定义操作的 Web API 终结点来扩展 AI 充实。 与内置技能类似，“自定义 Web API”**** 技能也有输入和输出。 Web API 根据输入在索引器运行时接收 JSON 有效负载，并输出 JSON 有效负载作为响应，以及成功状态代码。 响应应包含自定义技能指定的输出。 其他任何响应都被视为错误，并且不会执行任何扩充。
...
| 参数名称     | 描述 |
|--------------------|-------------|
| uri | _将 JSON_负载发送到的 Web API 的 URI。 只允许使用 https**** URI 方案 |
...
| degreeOfParallelism | （可选）指定后，指示索引器将与您提供的终结点并行发出的呼叫数。 如果终结点在请求负载过高的情况下出现故障，则可以降低此值;如果终结点能够接受更多请求，并且希望提高索引器的性能，则可以将其调高。  如果未设置，则使用默认值 5。 并行度最多可设置为 10 和最小 1。 |
...
“输出”对应于 Web API 返回的响应。 Web API 应仅返回_JSON_负载（通过查看`Content-Type`响应标头进行验证），并应满足以下约束：
