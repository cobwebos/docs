...
description: Azure 认知搜索查询中的 OData search.in 函数。
manager: nitinme
author: brjohnstmsft
ms.author: brjohnst
...
ms.locfileid: "72793283"
---
# <a name="odata-searchin-function-in-azure-cognitive-search"></a>Azure 中的 OData `search.in` 函数认知搜索

[OData 筛选器表达式](query-odata-filter-orderby-syntax.md)中的常见方案是检查每个文档中的单个字段是否等于多个可能值中的一个。 例如，以下是某些应用程序实现[安全修整](search-security-trimming-for-azure-search.md)的方式：通过检查包含一个或多个主体 id 的字段，将其用于表示发出查询的用户的主体 id 列表。 编写类似于下面的查询的一种方法是使用[`eq`](search-query-odata-comparison-operators.md)和[`or`](search-query-odata-logical-operators.md)运算符：

    group_ids/any(g: g eq '123' or g eq '456' or g eq '789')

不过，使用 `search.in` 函数可以更简单地编写这种方法：

    group_ids/any(g: search.in(g, '123, 456, 789'))

> [!IMPORTANT]
> 除了更简单、更易于阅读，使用 `search.in` 还可以提高[性能](#bkmk_performance)，并避免在筛选器中包含数以百计甚至上千个值时[筛选器的某些大小限制](search-query-odata-filter.md#bkmk_limits)。 出于此原因，我们强烈建议使用 `search.in`，而不是更复杂的相等表达式析取。
...
| 参数名称 | Type | 描述 |
| --- | --- | --- |
| `variable` | `Edm.String` | 字符串字段引用（如果在 `any` 或 `all` 表达式内使用 `search.in`，则为字符串集合字段的范围变量。 |
| `valueList` | `Edm.String` | 一个字符串，包含要与 `variable` 参数匹配的值的分隔列表。 如果未指定 `delimiters` 参数，则默认分隔符为空格和逗号。 |
| `delimiters` | `Edm.String` | 一个字符串，其中每个字符在分析 `valueList` 参数时被视为分隔符。 此参数的默认值是 `' ,'` 这意味着它们之间具有空格和/或逗号的任何值都将被分隔。 如果你需要使用空格和逗号以外的分隔符，因为你的值包括这些字符，则可以在此参数中指定替换分隔符，如 `'|'`。 |

<a name="bkmk_performance"></a>

### <a name="performance-of-searchin"></a>`search.in` 性能

如果使用 `search.in`，则当第二个参数包含数百个或数千个值的列表时，可以获得次秒级响应时间。 你可以传递到 `search.in`的项数没有显式限制，尽管你仍受最大请求大小的限制。 但是，延迟会随着值数量的增长而增加。

## <a name="examples"></a>示例

查找名称等于 "海平面 View motel" 或 "预算酒店" 的所有酒店。 短语包含空格，这是默认分隔符。 您可以使用单引号指定替代分隔符作为第三个字符串参数：  
