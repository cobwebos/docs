...
---
# <a name="example-multi-level-facets-in-azure-cognitive-search"></a>示例：Azure 认知搜索中的多级面
...
+ **分类/任何**`(x:x eq 'Accessories')`返回配件分支中的所有文档
+ **分类/任何**`(x:x eq 'Accessories|Bike Racks')`只返回在"附件"分支下具有自行车架子类别的文档。
...
[示例：为 Azure 认知搜索建模 AdventureWorks 清单数据库](search-example-adventureworks-modeling.md)