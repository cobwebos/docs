...
---
# <a name="upgrading-to-the-azure-search-net-sdk-version-20-preview"></a>升级到 Azure 搜索 .NET SDK 版本 2.0-preview
如果使用的是版本 1.1 或更早版本的 [Azure 搜索 .NET SDK](https://msdn.microsoft.com/library/azure/dn951165.aspx)，本文有助于你升级应用程序，以便使用下一个主要版本（即 2.0-preview）。

有关包括示例的 SDK 的更多常规演练，请参阅[如何使用 .NET 应用程序中的 Azure 搜索](search-howto-dotnet-sdk.md)。

2.0-preview 版的 Azure 搜索 .NET SDK 包含了某些针对早期版本进行的更改。 这些更改主要涉及次要版本，因此更改代码的工作量并不是太大。 有关如何更改代码以使用新版 SDK 的说明，请参阅[升级步骤](#UpgradeSteps)。

> [!NOTE]
> 如果使用的是版本 0.13-preview 或更早版本，则应该先升级到版本 1.1，然后再升级到版本 2.0-preview。 有关说明，请参阅[附录：升级到版本 1.1 的步骤](#UpgradeStepsV1)。
> 
> 

<a name="WhatsNew"></a>

## <a name="whats-new-in-version-20-preview"></a>版本 2.0-preview 中的新增功能
Azure 搜索 .NET SDK 的版本 2.0-preview 是面向 Azure 搜索 REST API 预览版（特别是 2015-02-28-preview）的第一个版本。 这使得可以在 .NET 应用程序中使用 Azure 搜索的许多预览版功能，如下所示：
...
首先，按以下方法操作更新 `Microsoft.Azure.Search` 的 NuGet 引用：使用 NuGet 程序包管理器控制台，或者在 Visual Studio 中右键单击项目引用，然后选择“管理 NuGet 程序包...”。 确保按以下方法操作支持预发行版程序包：在 Visual Studio 的 NuGet“管理程序包”窗口中选择“包含预发行版”，或者使用 `-IncludePrerelease` 开关（如果使用的是 NuGet 程序包管理器控制台）。
...
下一步是修复生成错误。 有关出错原因和修复方法的详细信息，请参阅[版本 2.0-preview 中的重大更改](#ListOfChanges)。

你可能会看到与过时方法或属性有关的其他生成警告。 这些警告将包含有关使用哪些功能来替换已弃用功能的说明。 例如，如果应用程序使用了 `SearchRequestOptions.RequestId` 属性，应该会收到显示 `"This property is deprecated. Please use ClientRequestId instead."` 的警告

在修复了任何生成错误后，可以对应用程序进行更改，以利用新功能（如果你愿意）。 有关 SDK 中的新功能的详细信息，请参阅[版本 2.0-preview 中的新增功能](#WhatsNew)。

<a name="ListOfChanges"></a>

## <a name="breaking-changes-in-version-20-preview"></a>版本 2.0-preview 中的重大更改
除重新生成应用程序之外，版本 2.0-preview 中只有一个可能需要更改代码的重大更改。

### <a name="indexesgetclient-return-type"></a>Indexes.GetClient 返回类型
`Indexes.GetClient` 方法具有新的返回类型。 之前，它返回 `SearchIndexClient`，但在版本 2.0-preview 中这已更改为 `ISearchIndexClient`。 这是为了支持想要通过返回 `ISearchIndexClient` 的模拟实现，来模拟测试单元的 `GetClient` 方法的客户。
...
我们欢迎你就 SDK 提供反馈。 如果遇到问题，请随时通过 [Azure 搜索 MSDN 论坛](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=azuresearch)向我们寻求帮助。 如果找到 Bug，可以在 [Azure .NET SDK GitHub 存储库](https://github.com/Azure/azure-sdk-for-net/issues)中提出问题。 务必在你的问题标题上加前缀“Search SDK:”。
...
> [!NOTE]
> 本章节仅适用于使用 Azure 搜索 .NET SDK 版本 0.13-preview 或更早版本的用户。
> 
> 
...
##### <a name="important-note-for-web-applications"></a>Web 应用程序的的重要说明
