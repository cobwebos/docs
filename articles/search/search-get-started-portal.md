...
## <a name="create-index"></a>创建索引以及将数据加载
...
* **可检索**意味着，它显示在搜索结果列表。 清除此复选框即可将单个字段标记为关闭搜索结果限制，例如当字段仅用于筛选器表达式时。 
* “可筛选”、“可排序”和“可查找”确定字段是否可用于筛选器、排序或方面导航结构。 
* **可搜索**意味着字段包含在全文搜索。 字符串可搜索。 数值字段和布尔字段通常标记为不可搜索。 
...
+ `search=*` 是空搜索。 空搜索会搜索所有内容。 提交空查询的原因之一是针对整个文档集进行筛选器或分面。 例如，您希望包含在索引中的所有城市分面导航结构。
