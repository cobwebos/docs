...
description: 内容提取、自然语言处理 （NLP） 和图像处理用于在 Azure 认知搜索索引中创建具有预定义的认知技能和自定义 AI 算法的内容。
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
---
# <a name="getting-started-with-ai-enrichment"></a>开始使用 AI 浓缩

AI 扩充是 Azure 认知搜索索引的功能，用于从图像、blob 和其他非结构化数据源中提取文本。 扩充和提取使您的内容在[索引](search-what-is-an-index.md)或[知识存储](knowledge-store-concept-intro.md)中更容易搜索。 提取和浓缩使用附加到索引管道的*认知技能*实现。 服务内置的认知技能分为以下几类： 

+ “自然语言处理”技能包括[实体识别](cognitive-search-skill-entity-recognition.md)、[语言检测](cognitive-search-skill-language-detection.md)、[关键短语提取](cognitive-search-skill-keyphrases.md)、文本操作、[情绪检测](cognitive-search-skill-sentiment.md)和 [PII 检测](cognitive-search-skill-pii-detection.md)****。 使用这些技能，非结构化文本将映射到索引中的可搜索和可筛选字段。

+ **图像处理**技能包括[光学字符识别 （OCR）](cognitive-search-skill-ocr.md)和[视觉特征](cognitive-search-skill-image-analysis.md)的识别，如面部检测、图像解释、图像识别（名人和地标）或图像定向等属性。 这些技能创建图像内容的文本表示形式，使其可以使用 Azure 认知搜索的查询功能进行搜索。

![扩充管道关系图](./media/cognitive-search-intro/cogsearch-architecture.png "扩充管道概述")

Azure 认知搜索中的认知技能基于认知服务 API 中的预先训练的机器学习模型：[计算机视觉](https://docs.microsoft.com/azure/cognitive-services/computer-vision/)和[文本分析](https://docs.microsoft.com/azure/cognitive-services/text-analytics/overview)。 
...
## <a name="when-to-use-ai-enrichment"></a>何时使用 AI 浓缩

如果原始内容为非结构化文本、图像内容或需要语言检测和翻译的内容，则应考虑使用内置认知技能。 通过内置认知技能应用 AI，可以对此内容进行解锁，在搜索和数据科学应用中提高其价值和实用性。 

此外，如果你有要集成到管道中的开源、第三方或第一方代码，则可以考虑添加自定义技能。 标识各种文档类型显著特征的分类模型属于此类别，但可以使用任何为内容增加价值的包。

### <a name="more-about-built-in-skills"></a>有关内置技能的详细信息

使用内置技能组装[的技能集](cognitive-search-defining-skillset.md)非常适合以下应用程序方案：
...
## <a name="steps-in-an-enrichment-pipeline"></a>浓缩管道中的步骤

浓缩管道基于[*索引器*](search-indexer-overview.md)。 索引器基于索引和数据源之间的字段到字段映射填充索引，以便进行文档破解。 技能，现在附加到索引器，根据您定义的技能集拦截和丰富文档。 编制索引后，可以使用所有[受 Azure 认知搜索支持的查询类型](search-query-overview.md)通过搜索请求来访问内容。  本部分引导索引器的新手完成这些步骤。

### <a name="step-1-connection-and-document-cracking-phase"></a>第 1 步：连接和文档开裂阶段

在管道开始时，您有非结构化文本或非文本内容（如图像、扫描的文档或 JPEG 文件）。 数据必须存在于可由索引器访问的 Azure 数据存储服务中。 索引器可以“破解”源文档，以从源数据提取文本。 文档破解是在索引过程中从非文本源中提取或创建文本内容的过程。
...
### <a name="step-2-cognitive-skills-and-enrichment-phase"></a>第2步：认知技能和浓缩阶段

浓缩是使用执行原子操作*的认知技能*进行的。 例如，破解 PDF 后，可以应用实体识别、语言检测或关键短语提取，在索引中生成源中本机不可用的新字段。 管道中使用的技能的集合统称为技能集。**  
...
在内部，管道生成扩充文档的集合。 可以确定要将扩充文档的哪些部分映射到搜索索引中可编制索引的字段。 例如，如果应用了关键短语提取和实体识别技能，则这些新字段将成为丰富文档的一部分，并可以映射到索引上的字段。 请参阅[注释](cognitive-search-concept-annotations-syntax.md)详细了解输入/输出的形成。
...
### <a name="step-3-search-index-and-query-based-access"></a>第 3 步：搜索索引和基于查询的访问
...
+ [示例：为 AI 扩充 （C#） 创建自定义技能](cognitive-search-create-custom-skill-example.md)
+ [快速入门：在门户演练中尝试 AI 扩充](cognitive-search-quickstart-blob.md)
