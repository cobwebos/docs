...
description: 添加新元素、更新现有元素或文档，或删除完全重建或部分索引中的过时文档，以刷新 Azure 认知搜索索引。
manager: nitinme
author: HeidiSteen
ms.author: heidist
...
| 删除字段 | 若要以物理方式删除字段的所有跟踪，必须重新生成索引。 当立即重新生成不可行时，您可以修改应用程序代码以禁用对"已删除"字段的访问，或使用[$select查询参数](search-query-odata-select.md)选择结果集中表示哪些字段。 实际上，当你应用省略了相关字段的架构时，字段定义和内容会一直保留在索引中，直至下次重新生成。 |
| 切换层 | 如果需要更多容量，则无法在 Azure 门户中就地升级。 必须创建新服务，必须在新服务中从头开始生成索引。 若要自动完成此过程，可以使用此 [Azure 认知搜索 .NET 示例存储库](https://github.com/Azure-Samples/azure-search-dotnet-samples)中的 **index-backup-restore** 示例代码。 此应用程序将备份索引到一系列 JSON 文件，然后在指定的搜索服务中重新创建索引。|

## <a name="update-conditions"></a>更新条件

可以在不影响现有物理结构的情况下进行许多其他修改。 具体而言，以下更改*不需要*重新生成索引。 对于这些更改，您可以使用更改[更新索引定义](https://docs.microsoft.com/rest/api/searchservice/update-index)。
...
在开发过程中，索引架构经常更改。 您可以通过创建可以删除、重新创建和使用小型代表性数据集快速重新加载的索引来规划它。

对于已投入生产的应用程序，建议创建一个与现有索引并排运行的新索引，以避免查询时停机。 应用程序代码提供重定向到新索引。

索引不会在后台运行，服务将平衡附加索引与正在进行的查询。 在索引期间，您可以监视门户中的[查询请求](search-monitor-queries.md)，以确保查询及时完成。

1. 确定是否需要重建。 如果只是添加字段，或更改与字段无关的索引的某些部分，则可能只是[更新定义](https://docs.microsoft.com/rest/api/searchservice/update-index)而无需删除、重新创建和完全重新加载它。

1. [获取索引定义](https://docs.microsoft.com/rest/api/searchservice/get-index)，以用于将来参考。

1. [删除现有索引](https://docs.microsoft.com/rest/api/searchservice/delete-index)，假定您没有并排运行新旧索引。 

   任何针对该索引的查询都会被立即删除。 请记住，删除索引是不可逆的，会破坏字段集合和其他构造的物理存储。 在放弃之前，请停下来思考其含义。 

1. [创建修订索引](https://docs.microsoft.com/rest/api/searchservice/create-index)，其中请求正文包括更改或修改的字段定义。
...
您可以使用[搜索资源管理器](search-explorer.md)或像[Postman](search-get-started-postman.md)这样的 Web 测试工具来检查更新的内容。

如果添加或重命名字段，请使用[$select](search-query-odata-select.md)返回该字段：`search=*&$select=document-id,my-new-field,some-old-field&$count=true`
