---
title: 教程： C#使用 .net 创建技能组合
titleSuffix: Azure Cognitive Search
description: 逐步完成显示 Azure 认知搜索扩充索引管道中的数据提取、自然语言和图像 AI 处理的示例代码。
manager: nitinme
author: MarkHeff
ms.author: maheff
...
本教程在免费服务中运行，但免费事务数目限制为每日 20 个文档。 如果要在同一天中多次运行此教程，请删除索引器以重置计数器。
...
若要在 Visual Studio 中安装 `Microsoft.Extensions.Configuration.Json` NuGet 包，请选择 "**工具**" > **NuGet 包管理器** > "**管理解决方案的 NuGet 包 ...** "。选择 "浏览"，然后搜索 `Microsoft.Extensions.Configuration.Json` NuGet 包。 找到此包后，依次选择它和项目，确认版本是否是最新稳定版，再选择“安装”。
...
通过调用 `DataSource`，新建 `DataSource.AzureBlobStorage` 实例。 `DataSource.AzureBlobStorage` 要求必须指定数据源名称、连接字符串和 Blob 容器名称。
