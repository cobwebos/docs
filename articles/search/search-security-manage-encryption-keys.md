...
description: 通过在 Azure Key Vault 中创建和管理的密钥，对 Azure 认知搜索中的索引和同义词映射进行服务器端加密补充。
manager: nitinme
author: NatiNimni
ms.author: natinimn
...
默认情况下，Azure 认知搜索使用[服务托管密钥](https://docs.microsoft.com/azure/security/fundamentals/encryption-atrest#data-encryption-models)来加密静态用户内容。 你可以使用在 Azure Key Vault 中创建和管理的密钥来补充使用额外加密层的默认加密。 本文将指导你完成这些步骤。
...
+ [Postman](search-get-started-postman.md)、 [Azure PowerShell](search-create-index-rest-api.md)和[Azure 认知搜索 SDK](https://aka.ms/search-sdk-preview)可用于调用预览版 REST API。 目前尚无门户或 .NET SDK 支持。
...
>[!Note]
> 由于通过客户管理密钥功能进行加密的性质，Azure 认知搜索将无法检索你的数据（如果你的 Azure 密钥保管库密钥已删除）。 为了防止意外 Key Vault 删除密钥导致的数据丢失，强烈建议你在所选的密钥保管库上启用软删除和清除保护。 有关详细信息，请参阅[Azure Key Vault 软删除](https://docs.microsoft.com/azure/key-vault/key-vault-ovw-soft-delete)。   
