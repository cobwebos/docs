...
description: 使用在 Azure 密钥保管库中创建和管理的密钥，在 Azure 认知搜索中补充索引和同义词映射上的服务器端加密。
manager: nitinme
author: NatiNimni
ms.author: natinimn
...
默认情况下，Azure 认知搜索使用[服务托管密钥](https://docs.microsoft.com/azure/security/fundamentals/encryption-atrest#data-encryption-models)对静态索引内容进行加密。 可以使用在 Azure Key Vault 中创建和管理的密钥，通过一个附加的加密层来补充默认加密。 本文将会讲解这些步骤。
...
密钥并不都需要在同一密钥保管库中。 单个搜索服务可以承载多个加密索引或同义词映射，每个索引或同义词映射都使用存储在不同密钥保管库中的客户托管加密密钥进行加密。  您还可以在同一服务中具有索引和同义词映射，这些索引和同义词映射未使用客户管理的密钥进行加密。 

> [!IMPORTANT] 
> 此功能在[REST API 版本 2019-05-06](https://docs.microsoft.com/rest/api/searchservice/)和[.NET SDK 版本 8.0 预览](search-dotnet-sdk-migration-version-9.md)版上可用。 当前不支持在 Azure 门户中配置客户托管加密密钥。 搜索服务必须在 2019 年 1 月之后创建，不能为免费（共享）服务。
...
+ [邮递员](search-get-started-postman.md)[、Azure 电源外壳](search-create-index-rest-api.md)和[Azure 认知搜索 SDK](https://aka.ms/search-sdk-preview)可用于调用 REST API。 目前没有针对客户管理的加密的门户支持。

>[!Note]
> 由于使用客户管理的密钥功能进行加密的性质，如果 Azure 密钥保管库密钥被删除，Azure 认知搜索将无法检索数据。 为了防止意外删除密钥保管库密钥而导致的数据丢失，**必须先**在密钥保管库中启用软删除和清除保护，然后才能使用它。 有关详细信息，请参阅 [Azure Key Vault 软删除](https://docs.microsoft.com/azure/key-vault/key-vault-ovw-soft-delete)。   
...
## <a name="advanced-use-an-externally-managed-azure-active-directory-application"></a><a name="aad-app"></a>高级：使用外部管理的 Azure 活动目录应用程序
