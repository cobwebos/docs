...
1. [登录到 Azure 门户](https://portal.azure.com)，然后打开搜索服务概述页面。 
...
若要在索引或同义词映射上添加客户管理的密钥，必须使用[搜索 REST API](/rest/api/searchservice/) 或 SDK。 门户不会公开同义词映射或加密属性。 使用有效的 API 时，索引和同义词映射都支持顶级 encryptionKey 属性。 
