...
下面的代码使用应用程序的配置文件（`app.config` 或 `web.config`）中存储的搜索服务名称和 API 密钥的值创建一个新 `SearchServiceClient`：
...
请注意，必须通过将 `IsKey` 设为 `true` 将类型为 `DataType.String` 的索引中的确切一个字段指定为“键”字段（请参见上例中的 `hotelId`）。
