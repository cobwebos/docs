...
分面导航是一个备用的搜索入口点。 它可以方便地替代手动键入复杂的搜索表达式。 方面可以帮助你找到你正在寻找的内容，同时确保没有获得零个结果。 作为开发人员，分面允许公开用于导航搜索库的最有用的搜索条件。 在在线零售应用程序中，分面导航通常基于品牌、分类（童鞋）、尺寸、价格、受欢迎程度和评级生成。 
...
-   **$filter=**  
    筛选器是一种功能强大的机制，用于根据特定记录属性的值限制搜索结果的大小。 A`$filter`是首先计算后, 跟生成的可用值和每个值的相应计数的分面逻辑
...
ph x="1" /> 索引属性在搜索索引中预定义，并在选定字段上设置以允许或拒绝在分面导航结构中使用。 如果没有 `"Facetable" = true`，则无法在分面导航中使用字段。
...
通过分面，应用程序明确表述的请求还应生成筛选器，以便根据分面值选择缩小候选记录集。 对于自行车商店，分面导航提供了就“有哪些颜色、制造商和类型的自行车”问题的线索， 同时筛选对“在此价格区间，具体有哪些红色的山地自行车”问题的答案。 当你单击"红色"，以指示应显示红色的产品时，包括应用程序发送第二个查询`$filter=Color eq ‘Red’`。
...
当分面查询添加筛选器时，你可能想要保留方面语句 (例如， `facet=Rating&$filter=Rating ge 4`)。 从技术上讲，不需要分面“分级”，但保留它会返回分级 4 和更高分级的分面值计数。 例如，如果单击"4"，该查询包含大于或等于"4"的筛选器，计数是返回为每个为 4 的分级和更高版本。  
...
由于分片体系结构，分面计数可能不准确。 每个搜索索引具有多个分片，每个分片报告按记录计数排序的前 N 个分面，并合并到单个结果中。 如果某些分片具有许多的匹配值，而其他人有较少的可能会发现一些方面值缺少或下计数在结果中。
...
它的常见若要查看筛选器，可帮助你选择应用商店、 饭馆或基于你的当前位置到其邻近的目标。 虽然此类型的筛选器看起来像分面导航，但它只是一个筛选器。 我们在此处针对专门查找该特定设计问题的实现建议的用户进行介绍。
