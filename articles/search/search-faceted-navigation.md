...
分面导航是一个备用的搜索入口点。 它可以方便地替代手动键入复杂的搜索表达式。 方面可以帮助您找到您要查找的内容，同时确保不会获得零结果。 作为开发人员，分面允许公开用于导航搜索索引的最有用的搜索条件。 在在线零售应用程序中，分面导航通常建立在品牌、部门（儿童鞋）、规模、价格、受欢迎程度和评级之上。 
...
如果不熟悉搜索开发，对分面导航定义的最佳理解是它显示了自定向搜索的可能性。 它是一种基于预定义筛选器的向下钻取搜索体验，用于通过点击点和单击操作快速缩小搜索结果范围。 

### <a name="interaction-model"></a>交互模型

分面导航的搜索体验是迭代的，因此让我们首先将其理解为响应用户操作展开的查询序列。
...
这两个查询都有效，但如果您正在寻找在西雅图有停车位的非汽车旅馆，则第二个问题是优越的。
...
可能用于分面导航的所有字段类型在默认情况下均为 `Facetable`。 此类字段类型包括`Edm.String` `Edm.DateTimeOffset`、和所有数值字段类型（基本上，除 外`Edm.GeographyPoint`，所有字段类型都是可面的，不能用于分面导航）。 

在生成索引时，分面导航的最佳做法是针对绝不应用作分面的字段显式关闭分面功能。  具体而言，应将单独值的字符串字段（例如 ID 或产品名称）设置为 `"Facetable": false`，以避免在分面导航中意外（和无效）使用它们。 在不需要的地方关闭面，有助于减小索引的大小，并且通常可以提高性能。
...
如您在示例架构中看到的，`Facetable`对于不应用作分面（如 ID 值）的字符串字段，将关闭。 在不需要的地方关闭面，有助于减小索引的大小，并且通常可以提高性能。
...
使用筛选器。 如果仅依赖搜索表达式，则词干可能会导致返回的文档，该文档在其任何字段中没有精确的分面值。
...
* `&facet=city,count:5` 指定仅返回在排序最高的结果中找到的前五个城市作为分面结果。 考虑一个搜索词为"机场"和 32 个匹配项的示例查询。 如果查询指定 `&facet=city,count:5`，分面结果中仅包含前五个搜索结果中具有最多文档的唯一城市。
...
* "`@odata.count`"<br/>
  在查询响应中，该值指示搜索结果中匹配项的数目。 平均而言，它大于所有分面结果的总和，因为存在与搜索词匹配但没有分面值匹配的项目。

**获取分面结果中的计数**

当向分面查询添加筛选器时，你可能希望保留分面语句（例如 `facet=Rating&$filter=Rating ge 4`）。 从技术上讲，不需要分面_评级，但保留它返回评级 4 和更高等级的分面值计数。 例如，如果单击“4”并且查询包括大于或等于“4”的筛选器，将针对每个等于和大于 4 的分级返回计数。  
...
Azure 认知搜索通过提供两种用于计算范围的方法，简化范围构造。 对于这两种方法，Azure 认知搜索会根据您提供的输入创建适当的范围。 例如，如果用户指定范围值为 10|20|30，它会自动创建范围 0-10、10-20、20-30。 应用程序可以选择性地删除任何空间隔。 
...
通常可以看到筛选器，这些筛选器可帮助您根据商店、餐厅或目的地的邻近位置来选择它。 虽然这种类型的筛选器可能看起来像分面导航，但它只是一个筛选器。 我们在此处针对专门查找该特定设计问题的实现建议的用户进行介绍。
...
观看[Azure 认知搜索深度潜水](https://channel9.msdn.com/Events/TechEd/Europe/2014/DBI-B410)。 在 45:25，演示了如何实现分面。
