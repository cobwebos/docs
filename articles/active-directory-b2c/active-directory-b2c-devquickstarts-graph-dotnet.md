...
> [!NOTE]
> 如果要从现有用户存储迁移的帐户的密码强度低于[Azure AD B2C 强制实施的强密码强度](active-directory-b2c-reference-password-complexity.md)，可以通过使用 `passwordPolicies` 属性中的 `DisableStrongPassword` 值来禁用强密码要求。 例如，可以按如下所示修改以前的 "创建用户" 请求： `"passwordPolicies": "DisablePasswordExpiration, DisableStrongPassword"`。
