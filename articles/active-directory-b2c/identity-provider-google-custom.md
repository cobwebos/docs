---
title: 使用自定义策略设置使用 Google 帐户进行登录
...
本文说明如何使用 Azure Active Directory B2C （Azure AD B2C）中的[自定义策略](custom-policy-overview.md)为使用 Google 帐户的用户进行登录。
...
7. 输入应用程序的**名称**。
8. 在“授权的 JavaScript 来源”中输入 **，然后在“授权的重定向 URI”中输入** 。`https://your-tenant-name.b2clogin.com``https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp` 将 your-tenant-name 替换为租户的名称。 输入租户名称时，必须全部使用小写字母，即使租户是使用大写字母在 Azure AD B2C 中定义的，也是如此。
8. 单击“创建”。
