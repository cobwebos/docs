...
在相关的文章中，我们已[创建一个 RESTful 服务](rest-api-claims-exchange-dotnet.md)，该服务可与 Azure Active Directory B2C (Azure AD B2C) 交互。

本文介绍如何使用客户端证书来限制对 Azure Web 应用 (RESTful API) 的访问。 此机制称为 TLS 相互身份验证或客户端证书身份验证。 只有包含适当证书的服务（例如 Azure AD B2C）能够访问你的服务。

>[!NOTE]
>也可以[使用 HTTP 基本身份验证](secure-rest-api-dotnet-basic-auth.md)保护 RESTful 服务。 但是，HTTP 基本身份验证的安全性被认为比客户端证书要低。 我们建议按本文中所述，使用客户端证书身份验证保护 RESTful 服务。
...
7. 在“密码”框中，键入证书的密码。
    ...
    ![在 Azure 门户中的 "创建密钥" 页上传策略密钥](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-upload.png)
...
5. 紧靠在 `<Metadata>` 元素右括号的后面添加以下 XML 片段：
    ...
    ![设置 ClientCertificate 身份验证 XML 元素](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-cert-tech-profile.png)
...
3. 在“名”框中键入 **Test** 来测试该过程。
    ...
    ![突出显示了 "给定名称" 文本框并显示输入验证错误](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-test.png)
...
![添加证书验证代码](./media/secure-rest-api-dotnet-certificate-auth/rest-api-netfw-secure-client-code.png)
...
如果需要对此步骤进行故障排除，请参阅[使用 Application Insights 收集日志](troubleshoot-with-application-insights.md)。

## <a name="optional-download-the-complete-policy-files-and-code"></a>（可选）下载完整的策略文件和代码
* 完成[自定义策略入门](custom-policy-get-started.md)演练后，我们建议你使用自己的自定义策略文件来构建方案。 我们已提供[示例策略文件](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw-secure-cert)用于参考。
