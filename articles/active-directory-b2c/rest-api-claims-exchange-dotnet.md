...
1. 在“新建 ASP.NET Web 应用程序”窗口中，选择“Web API”或“Azure API 应用”模板。

    ![在 Visual Studio 中选择 web API 模板](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-select-web-api.png)
...
1. 将类命名为 `InputClaimsModel`，并将以下属性添加到 `InputClaimsModel` 类：
...
1. 创建新模型 `OutputClaimsModel`，并将以下属性添加到 `OutputClaimsModel` 类：
...
1. 在“解决方案资源管理器”中，右键单击“控制器”文件夹，选择“添加”，并选择“控制器”。
    ...
    ![在 Visual Studio 中添加新控制器](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-1.png)

1. 在“添加基架”窗口中，选择“Web API 控制器 - 空”，再选择“添加”。

    ![选择 Visual Studio 中的 Web API 2 控制器-空](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-2.png)

1. 在“添加控制器”窗口中，将控制器命名为 **IdentityController**，选择“添加”。

    ![在 Visual Studio 中输入控制器名称](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-add-controller-3.png)

    基架会在“控制器”文件夹中创建名为 *IdentityController.cs* 的文件。

1. 如果 *IdentityController.cs* 文件尚未打开，请双击它，并将该文件中的代码替换为以下代码：
...
1. 在解决方案资源管理器中右键单击“Contoso.AADB2C.API”项目，选择“发布”。
    ...
    ![通过 Visual Studio 发布到 Microsoft Azure 应用服务](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-1.png)

1. 在“发布”窗口中，依次选择“Microsoft Azure 应用服务”、“发布”。

    ![通过 Visual Studio 创建新的 Microsoft Azure 应用服务](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-2.png)

    此时会打开“创建应用服务”窗口。 在此窗口中，创建在 Azure 中运行 ASP.NET Web 应用所需的全部 Azure 资源。

    > [!TIP]
    > 有关发布方法的详细信息，请参阅[在 Azure 中创建 ASP.NET Web 应用](../app-service/app-service-web-get-started-dotnet-framework.md)。

1. 在“Web 应用名称”框中，键入唯一的应用名称（有效字符为 a-z、0-9 和连字符 (-)）。 Web 应用的 URL 为 http://<app_name>.azurewebsites.NET，其中，*app_name* 是 Web 应用的名称。 可以接受自动生成的名称，它是唯一的。

    ![配置应用服务属性](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-publish-to-azure-3.png)

1. 若要开始创建 Azure 资源，请选择“创建”。
...
1. 对 SignUpOrSignIn.xml 文件重复上述步骤。
...
2. 打开已上传的信赖方 (RP) 自定义策略 **B2C_1A_signup_signin**，然后选择“立即运行”。
    ...
    ![Azure 门户中的 B2C_1A_signup_signin 自定义策略页](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-run.png)

3. 在“名”框中键入 **Test** 来测试该过程。
    Azure AD B2C 会在窗口顶部显示一条错误消息。

    ![在注册登录页上测试给定名称输入验证](./media/rest-api-claims-exchange-dotnet/aadb2c-ief-rest-api-netfw-test.png)
...
* 完成[自定义策略入门](custom-policy-get-started.md)演练后，我们建议你使用自己的自定义策略文件来构建方案。 我们已提供[示例策略文件](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/tree/master/scenarios/aadb2c-ief-rest-api-netfw)用于参考。
