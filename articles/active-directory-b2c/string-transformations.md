...
description: Azure Active Directory B2C Identity Experience Framework (IEF) 架构的字符串声明转换示例。
...
ms.locfileid: "83739067"
---
# <a name="string-claims-transformations"></a>字符串声明转换

[!INCLUDE [active-directory-b2c-advanced-audience-warning](../../includes/active-directory-b2c-advanced-audience-warning.md)]

本文演示了在 Azure Active Directory B2C (Azure AD B2C) 中使用 Identity Experience Framework 架构的字符串声明转换的过程。 有关详细信息，请参阅 [ClaimsTransformations](claimstransformations.md)。
...
| Item | TransformationClaimType | 数据类型 | 说明 |
| ---- | ----------------------- | --------- | ----- |
| OutputClaim | 本地化字符串的名称 | 字符串 | 调用此声明转换后生成的声明类型列表。 |
...
1. 进行声明转换时，指定要用本地化字符串设置的声明的列表。 `ClaimTypeReferenceId` 是对该策略的 ClaimsSchema 部分中已定义的 ClaimType 的引用。 `TransformationClaimType` 是在 `LocalizedString` 元素的 `StringId` 中定义的本地化字符串的名称。
1. 对于[自断言技术配置文件](self-asserted-technical-profile.md)或[显示控件](display-controls.md)输入或输出声明转换，请对声明转换进行引用。

![GetLocalizedStringsTransformation](./media/string-transformations/get-localized-strings-transformation.png)

以下示例从本地化字符串查找电子邮件主题、正文、代码消息和电子邮件的签名。 这些声明稍后由自定义电子邮件验证模板使用。

对英语（默认）和西班牙语定义本地化字符串。
...
使用此声明转换可从声明属性包中删除不必要的数据，以便减小会话 Cookie 大小。 以下示例将删除 `TermsOfService` 声明类型的值。
...
检查字符串声明 `claimToMatch` 和 `matchTo` 输入参数是否相等，并使用 `outputClaimIfMatched` 输入参数中提供的值设置输出声明，以及比较结果输出声明（此声明将基于比较结果设置为 `true` 或 `false`）。
...
| OutputClaim | outputClaim | 字符串 | 如果正则表达式匹配，则此输出声明包含 `outputClaimIfMatched` 输入参数的值。 如果无匹配，则为 NULL。 |
| OutputClaim | regexCompareResultClaim | boolean | 正则表达式匹配结果输出声明类型，将基于匹配结果设置为 `true` 或 `false`。 |
| OutputClaim| 声明名称| 字符串 | 如果将 extractGroups 输入参数设置为 true，则为调用此声明转换后生成的声明类型列表。 claimType 的名称必须与正则表达式组名称匹配。 | 
...
确定指定的子字符串是否出现在输入声明中。 结果是新布尔型 ClaimType，值为 `true` 或 `false`。 如果参数值出现在此字符串中，则为 `true`；否则为 `false`。
...
| OutputClaim | outputClaim | 字符串 | 调用此 ClaimsTransformation 后生成的 ClaimType。 如果子字符串出现在输入声明中，则为布尔值指示器。 |

使用此声明转换检查字符串声明类型是否包含任何子字符串。 以下示例检查 `roles` 字符串声明类型是否包含值“管理员”。
...
从指定位置的字符开始，提取字符串声明类型的部分内容，并返回指定数量的字符。
...
在声明类型字符串中搜索指定的值，并返回一个新的声明类型字符串，其中，当前字符串中指定字符串的所有匹配项将替换为另一个指定的字符串。

| Item | TransformationClaimType | 数据类型 | 说明 |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | inputClaim | 字符串 | 包含字符串的声明类型。 |
| InputParameter | oldValue | 字符串 | 要搜索的字符串。 |
| InputParameter | newValue | 字符串 | 要替换 `oldValue` 的所有匹配项的字符串 |
| OutputClaim | outputClaim | boolean | 等效于当前字符串（除了 oldValue 的所有实例都已替换为 newValue 外）的字符串。 如果在当前实例中找不到 oldValue，此方法返回未更改的当前实例。 |

例如，通过删除 `-` 字符来统一电话号码的格式：
...
连接指定字符串集合声明类型的元素，在每个元素或成员之间使用指定的分隔符。

| Item | TransformationClaimType | 数据类型 | 说明 |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | inputClaim | stringCollection | 一个包含要连接的字符串的集合。 |
| InputParameter | delimiter | 字符串 | 要用作分隔符的字符串，如逗号 (`,`)。 |
| OutputClaim | outputClaim | 字符串 | 一个由 `inputClaim` 字符串集合的成员组成的字符串，这些成员以 `delimiter` 输入参数分隔。 |

以下示例使用用户角色的字符串集合，并将其转换为采用逗号分隔符的字符串。 可以使用此方法在 Azure AD 用户帐户中存储字符串集合。 之后，当你从目录中读取该帐户时，使用 `StringSplit` 将使用逗号分隔符的字符串转换回字符串集合。
...
| InputParameter | delimiter | 字符串 | 要用作分隔符的字符串，如逗号 (`,`)。 |
| OutputClaim | outputClaim | stringCollection | 一个字符串，其元素包含此字符串中的子字符串，这些子字符串由 `delimiter` 输入参数分隔。 |

以下示例使用由用户角色组成的逗号分隔符字符串，并将其转换为字符串集合。
