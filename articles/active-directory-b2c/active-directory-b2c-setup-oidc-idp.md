---
title: 通过 OpenID Connect 设置注册和登录
...
[OpenID connect](active-directory-b2c-reference-oidc.md)是构建在 OAuth 2.0 基础之上的身份验证协议，可用于安全用户登录。 Azure AD B2C 中支持大多数使用此协议的标识提供程序。 本文介绍如何将自定义 OpenID Connect 标识提供者添加到用户流。

## <a name="add-the-identity-provider"></a>添加标识提供者

1. 以 Azure AD B2C 租户的全局管理员身份登录 [Azure 门户](https://portal.azure.com/)。
1. 通过单击顶部菜单中的 "**目录 + 订阅**" 筛选器并选择包含租户的目录，确保使用的是包含 Azure AD B2C 租户的目录。
1. 选择 Azure 门户左上角的“所有服务”，搜索并选择 **Azure AD B2C**。
1. 选择 "**标识提供者**"，然后选择 "**新建 OpenID connect 提供程序**"。

## <a name="configure-the-identity-provider"></a>配置标识提供者

每个 OpenID Connect 标识提供程序都说明了包含执行登录所需的大多数信息的元数据文档。 这包括要使用的 URL 和服务的公共签名密钥的位置等信息。 OpenID Connect 元数据文档始终位于以 `.well-known\openid-configuration` 结尾的的终结点上。 输入想要添加的 OpenID Connect 标识提供者的元数据 URL。
...
> [!NOTE]
> 客户端密码是可选项。 但是，如果你想要使用[授权代码流](https://openid.net/specs/openid-connect-core-1_0.html#CodeFlowAuth)，则必须输入客户端密码，该流使用机密来交换令牌的代码。
...
* **用户 ID**：输入提供已登录用户的*唯一标识符*的声明。
* **显示名称**：输入提供用户的*显示名称*或*全名*的声明。
* "**名称**"：输入提供用户的*名字*的声明。
* **姓**：输入提供用户的*姓氏*的声明。
