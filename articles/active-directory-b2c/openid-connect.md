...
由于它扩展了 OAuth 2.0，因此它还允许应用程序安全获取*访问令牌*。 可以使用 access_token 访问由[授权服务器](protocols-overview.md)保护的资源。 如果要构建在服务器上托管并通过浏览器访问的 web 应用程序，则建议使用 OpenID Connect。 有关令牌的详细信息，请参阅[中的令牌概述 Azure Active Directory B2C](tokens-overview.md)

Azure AD B2C 扩展了标准 OpenID Connect 协议，使其功能远远超出了简单的身份验证和授权。 它引入了[用户流参数](user-flow-overview.md)，使你可以使用 OpenID connect 向应用程序添加用户体验，例如注册、登录和配置文件管理。
