---
title: 在 Azure Active Directory B2C 中进行身份验证、注册、编辑配置文件 | Microsoft Docs
...
MSAL 还可用于跟踪用户的登录状态。 在此应用中，我们希望用户即使在关闭并重新打开应用后也能保持登录状态。 回到 `OnInitialized` 替代中，请使用 MSAL 的 `AcquireTokenSilent` 方法检查缓存的令牌：
