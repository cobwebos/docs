---
title: "Azure Active Directory B2C：使用自定义策略管理 SSO 和令牌自定义 | Microsoft Docs"
description: 
...
若要更改令牌生存期的设置，需要在想要影响的策略的信赖方文件中添加 `<ClaimsProviders>` 元素。  `<ClaimsProviders>` 元素是 `<TrustFrameworkPolicy>` 的子级。  在该元素中，需要放置影响令牌生存期的信息。  XML 如下所示：
