...
找到`<ClaimsProviders>`部分，然后添加以下 XML 代码片段。

您可以更改`IssuerUri`元数据的值。 这是 Azure AD B2C 的 SAML 响应中返回的颁发者 URI。 应将信赖方应用程序配置为在 SAML 断言验证期间接受颁发者 URI。
...
1. 将策略`PolicyId`的`PublicPolicyUri`和更改为_B2C_1A_signup_signin_saml_ ， `http://tenant-name.onmicrosoft.com/B2C_1A_signup_signin_saml`如下所示。
...
1. 在`<RelyingParty>`元素之前添加以下 XML 代码段。 此 XML 将覆盖_signuporsignin.xml_用户旅程的业务流程步骤7。 如果是从初学者包中的不同文件夹启动的，或通过添加或删除业务流程步骤自定义用户旅程，请确保数字（ `order`在元素中）与令牌颁发者步骤的用户旅程中指定的值对齐（例如，在其他 starter pack 文件夹中`LocalAccounts`，它的步骤号为4，对于为， `SocialAccounts`则为`SocialAndLocalAccountsWithMfa`9）。
...
1. 将`<RelyingParty>`元素中`<TechnicalProfile>`的整个元素替换为以下技术配置文件 XML。
...
1. 更新`tenant-name`为 Azure AD B2C 租户的名称。
...
Azure AD B2C 策略 IDP 元数据是 SAML 协议中用于公开 SAML 标识提供者配置的信息。 元数据定义服务的位置，例如登录和注销、证书、登录方法和其他信息。 以下 URL 提供了 Azure AD B2C 策略元数据。 将`tenant-name`替换为 Azure AD B2C 租户的名称， `policy-name`将替换为策略的名称（ID）：
...
`identifierUris`是一个字符串集合，其中包含在其 Azure AD B2C 租户中唯一标识 Web 应用的用户定义 URI。 服务提供商必须在 SAML 请求的`Issuer`元素中设置此值。
...
对于本教程，使用 SAML 测试应用程序，请使用以下值`samlMetadataUrl`：
...
如果未提供元数据 URI，则可以显式指定回复 URL。 此可选属性表示服务`AssertionConsumerServiceUrl`提供`SingleSignOnService`程序元数据中的（URL）， `BindingType`假定为`HTTP POST`。

如果在不使用服务提供程序元数据的情况下，选择在应用程序清单中配置回复 URL 和注销 URL，Azure AD B2C 将不会验证 SAML 请求签名，也不会加密 SAML 响应。

对于本教程，使用 SAML 测试应用程序时，将的`url`属性设置`replyUrlsWithType`为以下 JSON 代码片段中显示的值。
...
此可选`Logout`属性表示 url （`SingleLogoutService`信赖方元数据中的 url），假定`BindingType`为。 `Http-Redirect`

对于本教程，使用 SAML 测试应用程序，请将`logoutUrl`设置为`https://samltestapp2.azurewebsites.net/logout`：
...
选择 "**登录**"，并显示用户登录屏幕。 登录时，SAML 断言会退回到示例应用程序。
