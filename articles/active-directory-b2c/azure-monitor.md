...
使用 [Connect-AzAccount](/powershell/azure/authenticate-azureps) 登录到包含你的订阅的目录。 使用 `-tenant` 标志来强制对正确的目录进行身份验证。

```PowerShell
Connect-AzAccount -tenant contoso.onmicrosoft.com
```

使用 [Get-AzSubscription](/powershell/module/az.accounts/get-azsubscription) cmdlet 列出 Azure AD 租户下的、可由当前帐户访问的订阅。 记下要投影到 Azure AD B2C 租户的订阅的 ID。
