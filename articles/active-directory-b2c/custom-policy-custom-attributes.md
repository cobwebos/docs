...
当按照 [Azure Active Directory B2C：自定义策略入门](custom-policy-get-started.md)中的步骤进行操作时，你已下载并修改了名为 TrustFrameworkBase.xml、TrustFrameworkExtensions.xml、SignUpOrSignin.xml、ProfileEdit.xml 和 PasswordReset.xml 的[示例文件](https://github.com/Azure-Samples/active-directory-b2c-custom-policy-starterpack/archive/master.zip)。 在此步骤中，请对这些文件进行更多修改。
