...
*Client_id* 是之前记录为应用程序 ID 的值。 如果想要接收刷新令牌，则 *Offline_access* 是可选的。 使用的用户名和密码必须是来自 Azure AD B2C 租户中现有用户的凭据。
...
```
> [!NOTE]
> 在创建时通过图形 API 的用户，应用程序需要具有"openid"、"offline_access"和 Microsoft Graph 中的"配置文件"权限。
