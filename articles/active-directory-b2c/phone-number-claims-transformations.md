...
## <a name="convertphonenumberclaimtostring"></a>转换电话号码索赔字符串
...
| OutputClaim | 电话数字字符串 | 字符串 | 调用此声明转换后生成的 ClaimType。 |

在此示例中，具有 值类型的手机PhoneNumber声明`phoneNumber`将转换为值类型的手机电话声明， 其值类型为`string`。
...
| Item | TransformationClaimType | 数据类型 | 说明 |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | 电话数字字符串 | 字符串 |  电话号码的字符串声明。 电话号码必须采用国际格式，带有前导“+”和国家/地区代码。 如果提供输入`country`声明，电话号码为本地格式（没有国家/地区代码）。 |
| InputClaim | country | 字符串 | [可选]以 ISO3166 格式（两个字母的 ISO-3166 国家/地区代码）对电话号码的国家/地区代码的字符串声明。 |
...
| Item | TransformationClaimType | 数据类型 | 说明 |
| ---- | ----------------------- | --------- | ----- |
| InputClaim | phoneNumber | 字符串 | 电话号码的字符串声明。 电话号码必须采用国际格式，带有前导“+”和国家/地区代码。 |
| InputParameter | throwExceptionOnFailure | boolean | [可选]一个参数，指示当电话号码无效时是否引发异常。 默认值为 false。 |
| InputParameter | countryCodeType | 字符串 | [可选]一个参数，指示输出声明中国家/地区代码的类型。 可用值为 CallingCode（国家/地区的国际呼叫代码，例如 +1）或 ISO3166（两个字母的 ISO-3166 国家/地区代码）********。 |
