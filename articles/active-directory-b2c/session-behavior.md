...
- **单一登录配置**如果 B2C 租户中有多个应用程序和用户流，则可以使用 "**单一登录" 配置**属性来管理它们之间的用户交互。 可以将该属性设为以下设置之一：
...
5. 打开之前创建的用户流。
6. 选择“属性”。
7. 配置**web 应用会话生存期（分钟）** 、 **web 应用会话超时**、**单一登录配置**，并根据需要**在注销请求中要求 ID 令牌**。
