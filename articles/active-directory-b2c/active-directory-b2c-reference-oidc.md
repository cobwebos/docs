...
此时，要求用户完成策略的工作流。 这可能涉及用户输入其用户名和密码、用社交标识登录、注册目录，或任何其他若干步骤，具体取决于如何定义策略。

用户完成策略后，Azure AD 使用 `response_mode` 参数中指定的方法，将响应返回到位于所指示的 `redirect_uri` 参数的应用。 对于上述每种情况，响应均相同，与执行的策略无关。
...
如果你仅需要 Web 应用执行策略，则可以跳过下面几节。 这些章节仅适用于需要对 Web API 进行验证的调用，以及受到 Azure AD B2C 保护的 Web 应用。
