...
description: 了解如何将 Azure AD B2C authentication 与 TypingDNA 集成，以帮助根据用户键入模式进行身份验证和校对，提供了强制执行多重身份验证的 ID 验证解决方案，并有助于满足支付服务指令2（PSD2）的 SCA 要求。
author: gargi-sinha
manager: celestedg
...
在本演练中，了解如何将 Azure Active Directory B2C online 付款应用与 TypingDNA 应用集成在一起。 通过使用 TypingDNA 应用，Azure AD B2C 客户可以通过击键动态和强大的客户身份验证满足[支付服务指令 2](https://www.typingdna.com/use-cases/sca-strong-customer-authentication) （PSD2）事务要求。 [在此处](https://www.typingdna.com/)查找有关 TypingDNA 的详细信息。
...
2. 当用户提交页面时，TypingDNA 库将计算用户的键入特征。 然后，将信息插入 Azure AD B2C 呈现的隐藏文本字段。 此字段已隐藏为 CSS。  
    ...
    此示例包含 HTML 文件，其中包含 JavaScript 和 CSS 修改，并且由 `api.selfasserted.tdnasignin` 和 `api.selfasserted.tdnasignup` 内容定义引用。 请参阅[托管页面内容](https://docs.microsoft.com/azure/active-directory-b2c/custom-policy-ui-customization#hosting-the-page-content)以托管 HTML 文件。

3. 现在，在用户提交凭据时，Azure AD B2C 在声明包中具有键入模式。 它必须调用一个 API （你的 API），才能将此数据传递到 TypingDNA REST API 终结点。 此 API 包含在示例中（typingDNA）。
...
`saveTypingPattern`如果希望 Azure AD B2C （通过 API）调用 TypingDNA 保存类型模式终结点，则 API 必须返回声明。

存储库中的示例包含配置有以下属性的 API （TypingDNA）。
...
1. 托管所选提供程序的 TypingDNA-API 接口
2. 将和中的所有实例替换为 `apiKey` `apiSecret` 你的 TypingDNA 仪表板中的凭据，并在 TYPINGDNA-API-Interface 解决方案中
...
6. 替换策略文件中的 TenantId
7. 将所有 TypingDNA REST API 技术配置文件（TDNA-VerifyUser，，REST-TDNA-SaveUser）中的 ServiceURLs 替换为 TDNA API 接口 API 的终结点。
8. 将策略文件上传到你的租户。
