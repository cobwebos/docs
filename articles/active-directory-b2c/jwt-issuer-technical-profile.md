...
"**Name****协议"** 元素的名称属性需要设置为`None`。 将 **OutputTokenFormat** 元素设置为 `JWT`。
...
|刷新令牌用户旅程 Id| 否 | 应在[刷新访问令牌](authorization-code-flow.md#4-refresh-the-token)POST 请求到终结点期间执行的用户旅程的`/token`标识符。 |
