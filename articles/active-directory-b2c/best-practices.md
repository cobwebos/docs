...
|  |  |
|--|--|
| 在大多数情况下选择用户流 | Azure AD B2C 的标识体验框架是服务的核心强度。 策略充分描述了标识体验，例如注册、登录或配置文件编辑。 若要帮助设置最常见的标识任务，Azure AD B2C 门户应包括名为“用户流”的预定义且可配置的策略。 使用用户流，只需单击几下鼠标，就能在几分钟内创建极佳的用户体验。 [了解何时使用用户流与自定义策略](custom-policy-overview.md#comparing-user-flows-and-custom-policies)。|
...
| 限制 |  如果在短时间内从同一源发送过多请求，Azure AD B2C 会限制流量。 在负载测试时使用多个流量源，并`AADB2C90229`在应用程序中正常处理错误代码。 |
...
| [Microsoft 支持部门](support-options.md) | 为 Azure AD B2C 技术问题提供支持请求。 计费和订阅管理支持免费提供。 |
