...
ms.locfileid: "83679245"
---
# <a name="developer-notes-for-custom-policies-in-azure-active-directory-b2c"></a>Azure Active Directory B2C 中自定义策略的开发人员说明

Azure Active Directory B2C 中的自定义策略配置现已正式发布。 此配置方法面向构建复杂标识解决方案的高级标识开发人员。 自定义策略使 Identity Experience Framework 的强大功能在 Azure AD B2C 租户中可用。
使用自定义策略的高级标识开发人员应该计划投入一些时间来完成演练并阅读参考文档。

尽管大部分可用的自定义策略选项现已正式发布，但在软件生命周期的不同阶段，都有一些基本功能，如技术配置文件类型和内容定义 API。 更多功能即将发布。 下表指定了更为细致的可用性级别。

## <a name="features-that-are-generally-available"></a>正式发布的功能

- 使用自定义策略创作并上传自定义身份验证用户旅程。
    - 将用户旅程逐步描述为声明提供程序之间的交换。
    - 在用户旅程中定义条件分支。
- 在自定义身份验证用户旅程中与已启用 REST API 的服务进行互操作。
...
手动策略配置授予对 Azure AD B2C 基础平台的较低级别的访问权限，因此要求创建唯一的信任框架。 自定义标识提供者、信任关系、与外部服务的集成以及分步工作流的许多可能的排列组合方式需要一种系统化方式来设计和配置。

使用自定义策略功能集的开发人员应遵循以下准则：

- 熟悉自定义策略和密钥/机密管理的配置语言。 有关详细信息，请参阅 [TrustFrameworkPolicy](trustframeworkpolicy.md)。
- 取得方案和自定义集成的所有权。 记录你的工作并通知实时站点组织。
- 执行有序的方案测试。
...
## <a name="terms-for-features-in-public-preview"></a>公共预览版功能的条款

- 建议将公共预览版功能仅用于评估。
- 服务级别协议 (SLA) 不适用于公共预览版功能。
- 可通过普通支持渠道提出公共预览版功能支持请求。

## <a name="features-by-stage-and-known-issues"></a>按阶段的功能和已知问题

自定义策略/Identity Experience Framework 功能正处于持续而快速的开发中。 下表是功能和组件可用性的索引。
...
### <a name="identify-providers-federation"></a>标识提供程序联合 
...
| [第三方电子邮件服务提供程序](custom-email.md) |  |X  |  |  |
...
| 将 JSON 经 `client_assertion` 插入用户旅程 | X |  |  | 即将弃用。 |
| 将 JSON 插入用户旅程以作为 `id_token_hint` |  | X |  | 用于传递 JSON 的前向方法。 |
...
| [OAuth SSO 会话提供程序](custom-policy-reference-sso.md#oauthssosessionprovider)  |  | X |  |  |
| [单一注销](session-overview.md#sign-out)  |  | X |  |  |
...
| [Application Insights 事件日志](application-insights-technical-profile.md) |  | X |  | 用于监视生产环境中的用户流。 |
