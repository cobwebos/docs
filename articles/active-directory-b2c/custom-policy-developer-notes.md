...
| [REST API 与 OAuth2 持有者身份验证](secure-rest-api.md#oauth2-bearer-authentication) |  | X |  |  |
...
| 作为本地目录[Azure Active Directory](active-directory-technical-profile.md) |  |  | X |  |
...
| 通过将 JSON 插入用户旅程`client_assertion` | X |  |  | 即将弃用。 |
| 将 JSON 插入用户旅程`id_token_hint` |  | X |  | 用于传递 JSON 的前向方法。 |
| [向应用程序传递标识提供程序令牌](idp-pass-through-custom.md) |  | X |  | 例如，从 Facebook 到应用。 |
...
- 检查[可用于 Azure AD B2C Microsoft Graph 操作](microsoft-graph-operations.md)
