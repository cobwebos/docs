...
使用语言自定义可以根据的客户需求将用户旅程更改为不同的语言。  我们提供 37 种语言的翻译（请参阅[其他信息](#additional-information)）。  即使体验是针对一种语言提供的，你也可以根据需要自定义页面上的任何文本。  
...
3. **策略默认语言** - 如果未指定语言，或者指定了不支持的语言，用户旅程将翻译成策略默认语言
...
3. 随后你将转到“支持的语言”边栏选项卡。  在此处可以选择“添加资源”。
4. 选择想要支持的所有语言。  
