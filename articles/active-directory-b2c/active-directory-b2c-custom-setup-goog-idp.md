---
title: 在 Azure Active Directory B2C 中使用自定义策略将 Google+ 添加为 OAuth2 标识提供者 | Microsoft Docs
...
要将 Google+ 用作 Azure Active Directory (Azure AD) B2C 中的标识提供者，需要创建 Google+ 应用程序并向其提供合适的参数。 可在此处注册 Google+ 应用程序：[https://accounts.google.com/SignUp](https://accounts.google.com/SignUp)
...
13.  提供应用程序的**名称**，在**已授权 JavaScript 来源**字段中输入 `https://{tenant}.b2clogin.com`，并在**已授权重定向 URI** 字段输入 `https://{tenant}.b2clogin.com/te/{tenant}.onmicrosoft.com/oauth2/authresp`。 将 {tenant} 替换为租户名称（例如 contosob2c）。 **{tenant}** 值区分大小写。 单击“创建”。
