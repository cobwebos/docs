...
如果使用[自定义策略](custom-policy-overview.md)在应用程序中提供注册或登录、密码重置或配置文件编辑，请使用[策略文件自定义 UI](custom-policy-ui-customization.md)。

如果需要根据客户的决策提供动态内容，请使用自定义策略，根据查询字符串中发送的参数动态[更改页面内容](custom-policy-ui-customization.md#configure-dynamic-custom-page-content-uri)。 例如，可以基于从 Web 或移动应用程序传递的参数，更改 Azure AD B2C 注册或登录页面上的背景图像。
...
如果仅提供登录及其随附的密码重置页和验证电子邮件，请使用与[Azure AD 登录页](../active-directory/fundamentals/customize-branding.md)相同的自定义步骤。

如果客户尝试在登录之前编辑其配置文件，则他们会使用用于自定义 Azure AD 登录页的步骤将其重定向到您自定义的页面。
...
- Azure AD B2C 将 HTML 内容**合并**到您的页面中。 请勿复制并尝试更改 Azure AD B2C 提供的默认内容。 最好从头开始构建 HTML5 内容，将默认内容用作参考。
...
## <a name="company-branding-preview"></a>公司品牌（预览）

您可以使用 Azure 活动目录[公司品牌](../active-directory/fundamentals/customize-branding.md)，使用横幅徽标、背景图像和背景颜色自定义用户流页。

要自定义用户流页，请先在 Azure 活动目录中配置公司品牌，然后在 Azure AD B2C 中的用户流页面布局中启用它。
...
1. 在 **"管理**"下，选择 **"公司品牌**"。
1. 按照[将品牌添加到组织的 Azure 活动目录登录页中](../active-directory/fundamentals/customize-branding.md)的步骤操作。

在 Azure AD B2C 中配置公司品牌时，请记住以下事项：

* Azure AD B2C 中的公司品牌目前仅限于**背景图像**、**横幅徽标**和**背景颜色**自定义。 *不支持*公司品牌窗格中的其他属性，例如 **"高级设置**"中的属性。
* 在用户流页中，背景颜色在加载背景图像之前显示。 我们建议您选择与背景图像中的颜色紧密匹配的背景颜色，以便更流畅地获得加载体验。
* 横幅徽标将显示在用户启动注册用户流时发送给用户的验证电子邮件中。

### <a name="enable-branding-in-user-flow-pages"></a>在用户流页中启用品牌
...
![Azure 门户中的 Azure AD B2C 中的页面布局选择](media/customize-ui-overview/portal-02-page-layout-select.png)

此带注注的示例在使用"海洋蓝色"模板*的注册和登录*用户流页上显示自定义横幅徽标和背景图像：
...
要在自定义 HTML 中使用公司品牌资产，请在`<div id="api">`标记之外添加以下标记：
...
图像源替换为背景图像和横幅徽标的图像源。 如["开始使用自定义 HTML 和 CSS"](#get-started-with-custom-html-and-css)部分所述，使用 CSS 类对页面上的资产进行样式设置和定位。
