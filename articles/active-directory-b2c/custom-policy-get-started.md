...
1. 在 "**重定向 URI**" 下，选择 " `https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com` **Web**" `your-tenant-name` ，然后输入，其中是 Azure AD B2C 租户域名。
...
> [!TIP]
> 如果你的 XML 编辑器支持验证，则根据位于 starter `TrustFrameworkPolicy_0.3.0.0.xsd`包根目录中的 XML 架构验证文件。 XML 架构验证在上传之前会识别错误。
...
1. `SocialAndLocalAccounts/` **`TrustFrameworkExtensions.xml`** 在文件中，将的值替换`client_id`为 Facebook 应用程序 ID：
