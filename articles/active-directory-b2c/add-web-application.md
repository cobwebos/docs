...
1. 在“重定向 URI”下选择“Web”，然后输入 Azure AD B2C 会将应用程序请求的任何令牌返回到其中的终结点****。**** 在生产应用程序中，可以将重定向 URI 设置为类似`https://localhost:5000`的终结点。 在开发或测试过程中，你可以将`https://jwt.ms`其设置为，这是一个 Microsoft 拥有的 web 应用程序，它显示令牌的已解码内容（该令牌的内容绝不会离开你的浏览器）。 可以随时在注册的应用程序中添加和修改重定向 URI。
1. 选择“注册”  。
