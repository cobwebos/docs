---
title: 选择页面协定-Azure Active Directory B2C
...
无论你使用用户流或自定义策略，可以在 Azure Active Directory (Azure AD) B2C 策略中启用 JavaScript 客户端代码。 若要为应用程序启用 JavaScript，必须在[自定义策略](active-directory-b2c-overview-custom.md)中添加元素，选择页面协定，并在请求中使用 [b2clogin.com](b2clogin.md)。

页面协定是 Azure AD B2C 提供的元素与你提供的内容之间的关联。

本文讨论了如何通过在自定义策略中配置页面协定来在 Azure AD B2C 中选择页面协定。
...
页协定软件包会定期更新要包含在其页面元素中的修复和改进。 下面的更改日志指定每个版本中引入的更改。
