...
| 管理员帮助文本 | 0:1 | 声明类型的说明，有助于管理员了解其用途。 |
...
|date| 表示时间的瞬间，通常表示为一天的日期。 日期的值遵循 ISO 8601 约定。|
|dateTime|表示某个时刻，通常以日期和当天的时间表示。 日期的值遵循 ISO 8601 约定。|
|duration|表示以年、月、天、小时、分钟和秒为单位的时间间隔。 的格式为`PnYnMnDTnHnMnS`，其中`P`表示为正值或`N`负值。 `nY`是后跟字`Y`的年数。 `nMo`是以字面`Mo`后跟的月份数。 `nD`是后跟字`D`的天数。 示例：`P21Y`表示 21 年。 `P1Y2Mo`表示一年零两个月。 `P1Y2Mo5D`表示一年、两个月和五天。  `P1Y2M5DT8H5M620S`表示一年、两个月、五天、八小时、五分钟和二十秒。  |
|phoneNumber|表示电话号码。 |
|int| 表示介于 -2，147，483，648 和 2，147，483，647 之间的数字|
|long| 表示介于 -9，223，372，036，854，775，808 到 9，223，372，036，854，775，807 之间 |
|字符串| 将文本表示为 UTF-16 代码单元的序列。|
|stringCollection|表示 `string` 的集合。|
|用户身份| 表示用户标识。|
|用户身份集合|表示 `userIdentity` 的集合。|
...
| 特性 | 必选 | 描述 |
| --------- | -------- | ----------- |
| “属性” | 是 | Azure AD B2C 支持的有效协议的名称。 可能的值是：OAuth1、OAuth2、SAML2、OpenIdConnect。 |
...
枚**举**元素定义可供用户为用户界面中的声明（如`CheckboxMultiSelect`中`DropdownSingleSelect`的值或`RadioSingleSelect`中的值）选择的可用选项。 或者，您可以使用[本地化集合](localization.md#localizedcollections)元素定义和本地化可用选项。 要从声明**枚举**集合中查找项，请使用[GetMappedValue 从本地化集合](string-transformations.md#getmappedvaluefromlocalizedcollection)声明转换。
...
| HelpText | 否 | 如果正则表达式检查失败，则给用户的错误消息。 |
...
Azure AD B2C 支持各种用户输入类型，例如在手动输入声明类型的声明数据时可以使用的文本框、密码和下拉列表。 当您使用[自断言的技术配置文件](self-asserted-technical-profile.md)和[显示控件](display-controls.md)从用户收集信息时，必须指定**UserInputType。**

**用户输入类型**元素可用用户输入类型：

| UserInputType | 支持的声明类型 | 描述 |
| --------- | -------- | ----------- |
|CheckboxMultiSelect| `string` |多选下拉框。 声明值以所选值的逗号分隔符字符串表示。 |
|DateTimeDropdown | `date`, `dateTime` |下拉列表以选择天、月和年。 |
|DropdownSingleSelect |`string` |单个选择下拉框。 声明值是所选值。|
|EmailBox | `string` |电子邮件输入字段。 |
|Paragraph | `boolean`, `date`, `dateTime`, `duration`, `int`, `long`, `string`|仅在段落标记中显示文本的字段。 |
|密码 | `string` |密码文本框。|
|RadioSingleSelect |`string` | 单选按钮的集合。 声明值是所选值。|
|Readonly | `boolean`, `date`, `dateTime`, `duration`, `int`, `long`, `string`| 只读文本框。 |
|文本框 |`boolean`, `int`, `string` |单行文本框。 |
...
Paragraph**** 用户输入类型用于提供仅在段落标记中显示文本的字段。  例如，&lt;p&gt;文本&lt;/p&gt;。 **段落**用户输入类型的`OutputClaim`自断言技术配置文件，必须设置`Required`属性`false`（默认值）。
