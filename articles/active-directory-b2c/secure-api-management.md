...
您可以使用当前**的应用程序**体验或我们新的统一**应用注册（预览）** 体验来获取应用程序 ID。 [了解更多关于新体验](https://aka.ms/b2cappregintro)。
...
1. 在*Webapp1*或其他以前创建的应用程序 **（客户端）ID**列中记录该值。
...
## <a name="migrate-to-b2clogincom"></a>迁移到b2clogin.com

如果您有一个 APIM API 来验证由旧终结点`login.microsoftonline.com`颁发的令牌，则应迁移 API 和调用它的应用程序，以使用[b2clogin.com](b2clogin.md)颁发的令牌。

可以遵循以下一般过程来执行分阶段的迁移：

1. 在 APIM 入站策略中添加对 b2clogin.com和login.microsoftonline.com颁发的令牌的支持。
1. 一次更新一个应用程序，从b2clogin.com终结点获取令牌。
1. 一旦所有应用程序都正确从b2clogin.com获取令牌，请从 API 中删除对login.microsoftonline.com颁发的令牌的支持。

下面的示例 APIM 入站策略说明了如何接受b2clogin.com和login.microsoftonline.com颁发的令牌。 此外，它还支持来自两个应用程序的 API 请求。
...
您可以找到有关将基于 OWIN 的 Web API 及其应用程序迁移到b2clogin.com迁移[基于 OWIN 的 Web API 的信息b2clogin.com。](multiple-token-endpoints.md)
