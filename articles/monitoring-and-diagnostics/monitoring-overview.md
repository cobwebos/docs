...
基本监视提供对 Azure 资源的最基础必要的监视。 这些服务有一个最低配置要求，并收集高级监视服务所使用的核心遥测数据。    

### <a name="azure-monitor"></a>Azure 监视器
使用 [Azure Monitor](../monitoring-and-diagnostics/monitoring-overview-azure-monitor.md) 可收集[指标](../monitoring-and-diagnostics/monitoring-overview-metrics.md)、[活动日志](../monitoring-and-diagnostics/monitoring-overview-activity-logs.md)和[诊断日志](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md)，为 Azure 服务启用基本监视。 例如，可以通过活动日志了解新资源的创建或修改时间。 

可通过指标获取不同资源（甚至包括虚拟机中的操作系统）的性能统计信息。 可使用 Azure 门户中的某个资源管理器来查看此数据、将数据发送至 Azure Log Analytics 进行趋势和详细信息分析，或创建警报规则以主动发送关键问题的通知。
...
下列 Azure 服务提供丰富的功能，用于收集和分析监视数据。 这些服务以基本的监视功能为基础，并可利用 Azure 中的常用功能。 它们可以对收集的数据进行深入的分析，并提供有关应用程序和基础结构的独特见解。 它们在面向不同受众的方案上下文中呈现数据。
...
可以通过检测与 Application Insights 配合使用的应用程序来获得深入的见解。 然后即可快速确定并诊断错误，无需等待用户报告这些错误。 利用所收集的信息，可作出有关应用程序维护和优化的明智抉择。 
...
[Log Analytics](http://azure.microsoft.com/documentation/services/log-analytics) 在 Azure 监视中起着重要作用，它可以将来自不同资源的数据收集到一个存储库中。 你可以使用强大的查询语言对其中的数据进行分析。 

Application Insights 和 Azure 安全中心将数据存储在 Log Analytics 数据存储中，并使用其分析引擎。 数据是从 Azure Monitor、管理解决方案以及安装在虚拟机（云中或本地）上的代理收集的数据。 可以通过此共享功能全面了解自己的环境。 
...
* [ExpressRoute 监视器](https://azure.microsoft.com/en-in/blog/monitoring-of-azure-expressroute-in-preview/)：一种 NPM 功能，用于通过 Azure ExpressRoute 线路监视端到端连接和性能。
* 流量分析：一种基于云的解决方案，可查看云网络上的用户和应用程序活动。
* [DNS Analytics](https://docs.microsoft.com/en-us/azure/log-analytics/log-analytics-dns)：一种解决方案，可以根据 DNS 服务器提供与安全、性能和操作相关的见解。
...
下列 Azure 工具为高级监视服务提供关键功能。 多个服务共享它们，因此你可以跨多个服务利用这些常见功能和配置。
...
可以使用 [Azure 仪表板](../azure-portal/azure-portal-dashboards.md)，将不同类型的数据合并到 Azure 门户的单个窗格中， 然后将仪表板与其他 Azure 用户共享。 
...
也可向 Log Analytics 发送活动日志。 可以在其中使用数据来分析日志。这些数据是通过管理解决方案、虚拟机上的代理和其他源收集的。
...
* [Azure 诊断](../azure-diagnostics.md)：如果要尝试诊断云服务、虚拟机、虚拟机规模集或 Azure Service Fabric 应用程序中的问题。
