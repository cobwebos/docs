...
对 Azure 资源执行的所有操作都会由 Azure Resource Manager 进行完全审核（从创建和删除到授予或撤消访问权限）。 可在 Azure 门户中浏览这些日志，也可使用 [REST API](https://msdn.microsoft.com/library/azure/dn931927.aspx) 或 [.NET SDK](https://www.nuget.org/packages/Microsoft.Azure.Insights/) 以编程方式访问完整的事件集。
...
3. 这将打开一个边栏选项卡，其中显示过去 7 天影响任何订阅的所有事件。 顶部是一个按级别显示数据的图表，下部是日志的完整列表： ![所有事件](./media/insights-debugging-with-events/Insights_AllEvents.png)

> [!NOTE]
> 在 Azure 门户中，仅可查看 500 条给定订阅的最新事件。
> 
> 

1. 可以单击任何日志条目来查看组成它的事件。 例如，向资源组部署某种内容时，可能会创建或修改许多不同的资源。 对于每个条目，可以查看：
...
2. 从这里可以转到详细信息边栏选项卡，以查看该事件的具体细节。
...
3. 单击该磁贴以查看仅筛选到所选资源的事件。 可使用“筛选器”命令更改时间范围或应用更多特定筛选器。

## <a name="next-steps"></a>后续步骤
* 每当发生事件时[接收警报通知](insights-receive-alert-notifications.md)。
