...
1. 使用服务器的 IP 地址在浏览器中打开登录页 http://\<IP 地址\>:3000 或 \<DNSName>\:3000。 尽管默认端口为 3000，但请注意可能在设置期间选择了其他端口。 你应看到生成的 Grafana 服务器登录页。
    ...
    ![Grafana 登录屏幕](.\media\monitor-how-to-grafana\grafana-login-screen.png)
...
![Grafana 包括 Azure Monitor 插件](.\media\monitor-how-to-grafana\grafana-includes-azure-monitor-plugin-dark.png)
...
6. 选择“保存”，然后 Grafana 将为每个 API 测试凭据。 应看到类似于下面的消息：  
    ![已批准 Grafana 数据源配置](.\media\monitor-how-to-grafana\grafana-data-source-config-approved-dark.png)
...
3. 在仪表板上将显示一个空白图表。 单击面板标题，然后选择“编辑”，输入你想要在此图表中绘制的数据详细信息。
    ![Grafana 新图形](.\media\monitor-how-to-grafana\grafana-new-graph-dark.png)
...
4. 选择已配置的 Azure Monitor 数据源。
    * 收集 Azure Monitor 指标 - 在服务下拉列表中选择“Azure Monitor”。 随即将显示选择器列表，可在其中选择此图表中要监视的资源和指标。 若要收集 VM 的指标，请使用命名空间 Microsoft.Compute/VirtualMachines。 选择 VM 和指标后，即可开始在仪表板中查看其数据。
    ![适用于 Azure Monitor 的 Grafana 图形配置](.\media\monitor-how-to-grafana\grafana-graph-config-for-azure-monitor-dark.png)
    * 收集 Azure Log Analytics 数据 - 在下拉列表中选择“Azure Log Analytics”。 选择要查询的工作区并设置查询文本。 可在此处复制已有的任何 Log Analytics 查询，或新建一个查询。 在查询中键入时，IntelliSense 将显示并建议自动完成选项。 选择可视化类型“时间序列表”，并运行查询。
    ...
    ![适用于 Azure Log Analytics 的 Grafana 图形配置](.\media\monitor-how-to-grafana\grafana-graph-config-for-azure-log-analytics-dark.png)
...
![Grafana 配置变量](.\media\monitor-how-to-grafana\grafana-configure-variable-dark.png)
...
![Grafana 使用变量](.\media\monitor-how-to-grafana\grafana-use-variables-dark.png)
