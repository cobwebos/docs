...
3. 单击“活动日志”，确保显示活动日志部分。
    ...
    [**活动日志**](monitoring-overview-activity-logs.md)描述了在订阅资源上执行的所有操作。 通过活动日志，可确定对于订阅资源上的任何创建、更新或删除操作，“谁何时执行什么操作”。 例如，活动日志会显示某 Web 应用何时停止以及谁停止了该应用。 活动日志事件存储在平台中，可在 90 天内进行查询。
...
6. 返回到仪表板。 现在可看到仪表板中显示了查询（及结果数）。 如果你想快速查看订阅中最近发生的任何重大操作等，这就非常有用。 分配了新角色或删除了 VM。
   
    ![固定到仪表板的活动日志](./media/monitoring-get-started/monitor-act-log-db.png)
7. 返回到“监视器”磁贴并单击“度量”部分。 首先需要使用边栏选项卡顶部的下拉选项进行筛选和选择来选取资源。
   
    ![筛选度量资源](./media/monitoring-get-started/monitor-met-filter.png)
   
    所有 Azure 资源都会发出[**度量**](monitoring-overview-metrics.md)。 该视图将所有度量汇聚在单个面板中，便于你轻松了解当前资源的执行方式。
8. 选中某项资源后，边栏选项卡左侧会立即显示所有可用度量。 可以选择指标来立即绘制多个指标的图，并修改图类型和时间范围。 你还可以查看在此资源上设置的所有指标警报。
...
15. 如果你当前未使用 Application Insights 或 Log Analytics，则 Azure 监视器可能与你当前的监视、日志记录和警报产品建立了合作关系。 请参阅[合作伙伴页面](monitoring-partners.md)，了解集成方式的完整列表和说明。
