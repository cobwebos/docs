<properties 
	pageTitle="HDInsight 中的 HBase 概述 | Azure" 
	description="HDInsight 中的 HBase 简介、用例以及与其他数据库解决方案的比较。" 
	services="hdinsight" 
	documentationCenter="" 
	authors="mumian" 
	manager="paulettm" 
...
HBase 是一种基于 Hadoop 构建的 Apache 开放源代码 NoSQL 数据库。HBase 针对大量非结构化和结构化数据提供随机访问和强一致性。它是基于 Google 的 BigTable 建模的，是面向列系列的数据库。数据存储在表的各行中，行中的数据按列系列分组。HBase 是无架构数据库，也就是说，在使用其数据前，不必定义列以及列中存储的数据类型。开放源代码可进行线性伸缩，以处理上千节点上数 PB 的数据。开放源代码可依赖数据冗余、批处理以及 Hadoop 生态系统中的分布式应用程序提供的其他功能。

## Azure 中的 HDInsight HBase 是什么？
...
HDInsight 实施利用 HBase 的横向扩展架构来提供表自动分片、使读写操作保持高度的一致性，以及支持自动故障转移。性能可通过对读取使用内存中缓存并对写入使用高吞吐量流式处理来提高。可视化网络设置也适用于 HDInsight HBase。有关详细信息，请参阅[在 Azure 虚拟网络上设置 HDInsight 群集][hbase-provision-vnet]。
