...
> [!TIP]
> 如果与 Avere vFXT 群集一起创建新的 Azure Blob 容器，则该容器已配置并可供使用。

如果没有随群集创建新的 Blob 容器，或者想要添加其他硬件或基于云的存储系统，请按照这些说明进行操作。

有两个主要任务：

1. [创建一个核心文件程序](#create-a-core-filer)，该文件程序将 vFXT 群集连接到现有存储系统或 Azure 存储帐户容器。
...
"核心文件服务器"是后端存储系统的 vFXT 术语。 存储器可以是 NetApp 或 Isilon 等硬件 NAS 设备，也可以是云对象存储。 有关核心文件服务器的详细信息，请参阅[Avere 群集设置指南](https://azure.github.io/Avere/legacy/ops_guide/4_7/html/settings_overview.html#managing-core-filers)。

要添加核心文件管理器，请选择以下两种主要类型的核心文件管理器之一：

* [NAS 核心文件管理器](#nas-core-filer) - 介绍如何添加 NAS 核心文件管理器
* [Azure 存储云核心文件程序](#azure-blob-storage-cloud-core-filer)- 介绍如何将 Azure Blob 存储容器添加为云核心文件服务器
...
按照以下步骤添加 NAS 核心文件服务器：

1. 在 Avere 控制面板中，单击顶部的“设置”选项卡****。

1. 单击左侧**的核心文件服务器** > **管理核心文件服务器**。
...
要使用 Azure Blob 存储作为 vFXT 群集的后端存储，需要一个空容器作为核心文件程序添加。
...
> [!TIP]
> 如果在为 Azure 群集创建 Avere vFXT 时创建新的 Blob 容器，则部署模板会自动将容器配置为核心文件程序。 （如果使用创建脚本，则也是如此，该脚本可应请求提供。以后不需要配置核心文件服务器。
>
> 群集创建工具为您执行以下配置任务：
>
...
1. 创建新的 Blob 容器：单击概览页面上的**容器**，然后单击 **"容器**"。 使用任何容器名称，并确保访问权限设置为“专用”****。

   ![存储 blob 页，其中已圈出 _容器按钮，并在弹出窗口中创建新容器](media/avere-vfxt-new-blob.png)

1. 单击 **"设置**"下的 **"访问密钥**"获取 Azure 存储帐户密钥。 复制提供的密钥之一。
...
1. 接下来，创建核心文件管理器。 在 Avere 控制面板的左侧，单击 **"核心文件服务器** >  **管理核心文件器**"。
...
创建交汇点后，客户端使用命名空间路径从存储系统访问文件。

## <a name="next-steps"></a>后续步骤

* [装载 Avere vFXT 群集](avere-vfxt-mount-clients.md)
* 了解[将数据移动到新 Blob 容器](avere-vfxt-data-ingest.md)的有效方法
