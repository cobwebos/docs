...
6. 单击 **+** _前端主机_上的 "" 图标创建前端主机，为前门输入默认前端主机的全局唯一名称（ `\<**name**\>.azurefd.net` ）。 单击 "**添加**" 以继续进行下一步。
...
13. 添加另一个用于处理 HTTPS 流量的路由规则。 单击 " **+** 登录_路由规则_" 和 "提供路由名称" （例如 "DefaultForwardingRoute"），然后将 "_接受的协议_" 字段设置为 **"仅 HTTPS"**。 确保选择适当的_前端主机_。
...
在此示例中，将为 `www` 子域（例如）添加 CNAME 记录 `www.contosonews.com` 。
