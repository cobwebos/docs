...
ms.locfileid: "46958792"
---
# <a name="backends-and-backend-pools-in-azure-front-door-service"></a>Azure Front Door 服务中的后端和后端池
本文介绍了有关如何将应用程序部署与 Front Door 相对应的各种概念。 我们还将介绍 Front Door 配置中有关应用程序后端的不同术语的含义。
...
1. **路径**：为后端池中的所有后端发送探测请求的 URL 路径。 例如，如果你的其中一个后端是 `contoso-westus.azurewebsites.net`，并且路径设置为 `/probe/test.aspx`，则 Front Door 环境中（假设协议设置为 HTTP）会将运行状况探测请求发送到 http://contoso-westus.azurewebsites.net/probe/test.aspx。 
2. **协议**：定义是否通过 HTTP 或 HTTPS 协议将运行状况探测请求从 Front Door 发送到后端。
3. **间隔(秒)**：此字段定义向后端发送运行状况探测的频率，也就是说，每个 Front Door 环境中发送探测的间隔。 注意 - 如果你正在寻求更快的故障转移，请将此字段设置为较低值。 但是，值越低，你的后端将收到越多的运行状况探测量。 若要了解 Front Door 将在后端生成的探测量，让我们来看一个示例。 例如，间隔设置为 30 秒，并且在全球有大约 90 个 Front Door 环境或 POP。 因此，每个后端大约每秒收到 3-5 个探测请求。
...
1. **样本大小**：此属性标识我们需要考虑采用多少个运行状况探测样本来进行后端运行状况评估。
2. **成功样本大小**：此属性定义在上述“样本大小”中，我们需要检查多少个样本，才能成功将后端判定为正常。 
...
3. **延迟敏感度(额外延迟)**：“延迟敏感度”字段定义您是希望 Front Door 将请求发送到延迟度量方面处于敏感度范围内的后端，还是将请求转发到最近的后端。 阅读 Front Door 的[基于最少延迟的路由方法](front-door-routing-methods.md#latency)了解详细信息。
...
1. **后端主机类型**：想要添加的资源的类型。 如果是从应用服务、云服务或存储操作，Front Door 支持自动发现应用程序后端。 如果需要 Azure 中的不同资源，甚至非 Azure 后端，请选择“自定义主机”。 注意 - 在配置期间，API 不会验证后端是否可从 Front Door 环境访问，你需要确保可从 Front Door 访问后端。 
2. **订阅和后端主机名**：如果未选择“自定义主机”作为后端主机类型，则需要通过从用户界面中选择相应的订阅以及相应的后端主机名，缩小范围并进行选择。
3. **后端主机标头**：发送到后端的每个请求的主机标头值。 阅读[后端主机标头](#hostheader)了解详细信息。
4. **优先级**：如果想要使用主服务后端来处理所有流量，并提供备份来防范主后端或备份后端不可用的情况，可以为不同后端分配优先级。 阅读有关[优先级](front-door-routing-methods.md#priority)的详细信息。
5. **权重**：想要在一组后端中分布流量时，可以为不同后端分配权重，可以平均分布，也可以根据权重系数分布。 阅读有关[权重](front-door-routing-methods.md#weighted)的详细信息。
...
大多数应用程序后端（如 Web 应用、Blob 存储和云服务）要求主机标头与后端的域相匹配。 但是，路由到后端的前端主机将具有类似 www.contoso.azurefd.net 的不同主机名。 如果正在设置的后端要求主机标头与后端的主机名相匹配，应确保“后端主机标头”也具有后端的主机名。
