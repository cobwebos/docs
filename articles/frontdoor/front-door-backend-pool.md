...
- **后端主机类型**。 你想要添加的资源的类型。 第一道防线服务支持自动发现的应用后端，如果从应用服务、 云服务或存储。 如果您希望不同的资源在 Azure 或甚至非 Azure 后端中，选择**自定义主机**。
...
- **成功的样本大小**。 定义样本大小按前面所述，成功调用后端的正常运行所需的样本数。 例如，假设第一道防线运行状况探测间隔为 30 秒，样本大小为 5 秒，而成功的样本大小为 3 秒。 每次我们评估运行状况探测后端，我们看一下最后五个样本超过 150 秒 (5 x 30)。 至少三个成功探测所需声明为正常的后端。
