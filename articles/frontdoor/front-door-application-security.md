---
title: Azure 前门 - 应用程序层安全性 |微软文档
description: 本文可帮助您了解 Azure 前门如何保护和保护您的应用程序后端
services: frontdoor
documentationcenter: ''
author: sharad4u
...
Azure 前门提供 Web 应用程序保护功能，可保护 Web 应用程序免受网络攻击和常见 Web 漏洞漏洞利用，如 SQL 注入或跨站点脚本 （XSS）。 为 HTTP(S) 前端启用的 Front Door 的应用层安全性分布在全球并始终处于启用状态，可以阻止攻击者在 Azure 的网络边缘发动恶意攻击，使之远离你的后端。 Front Door 在安全性和性能优化方面进行了强化，可以为最终用户提供快速且安全的 Web 体验。
...
- **IP 允许列表和阻止列表：** 您可以配置自定义规则，以便根据客户端 IP 地址列表控制对 Web 应用程序的访问。 支持 IPv4 和 IPv6
