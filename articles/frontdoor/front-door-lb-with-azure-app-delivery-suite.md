...
| --------------- | ------------------------ |
|**任何协议：** 流量管理器在 DNS 层运行，因此可以路由任何类型的网络流量，比如 HTTP、TCP、UDP 等。 | **HTTP 加速：** 前门流量接近微软网络的边缘。  因此，HTTP(S) 请求在延迟和吞吐量方面有所改善，降低了 SSL 协商的延迟并使用热连接从 AFD 连接到应用程序。|
