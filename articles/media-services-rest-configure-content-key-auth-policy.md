...
如果你希望 Media Services 加密资产，则需要将加密密钥（**CommonEncryption** 或 **EnvelopeEncryption**）与资产相关联（如[此处](/zh-cn/documentation/articles/media-services-rest-create-contentkey/)所述），并配置密钥的授权策略（如本文所述）。 
...
若要配置令牌限制选项，你需要使用 XML 来描述令牌的授权要求。令牌限制配置 XML 必须符合[此部分](#schema)中所示的 XML 。
...
## 后续步骤
在配置内容密钥的授权策略后，请转到[如何配置资产传送策略](/zh-cn/documentation/articles/media-services-rest-configure-asset-delivery-policy/) 主题。
