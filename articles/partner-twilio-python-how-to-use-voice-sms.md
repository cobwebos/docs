...
使用 Twilio 服务且在 Azure 中运行的 Python 应用程序与任何其他使用 Twilio 服务的 Python 应用程序没有任何差别。 Twilio 服务是基于 REST 的且可通过几种方法从 Python 中调用，本文将重点介绍如何将 Twilio 服务与 [GitHub 提供的用于 Python 的 Twilio 库][twilio_python]一起使用。 有关使用 Python 的 Twilio 库的详细信息，[https://www.twilio.com/docs/libraries/python][twilio_lib_docs]请参阅。
...
* 从 GitHub 下载 Python 的 Twilio 库，[https://github.com/twilio/twilio-python][twilio_python]然后安装它，如下所示：
...
当应用程序发起对 Twilio API 的调用时，Twilio 会将请求发送到应返回 TwiML 响应的 URL。 上面的示例使用 Twilio 提供的 URL [https://twimlets.com/message][twimlet_message_url]。 （虽然 TwiML 专供 Twilio 使用，但可以在浏览器中查看它。 例如，单击以查看[https://twimlets.com/message][twimlet_message_url]空元素;单击`<Response>`以查看空元素。作为另一[https://twimlets.com/message?Message%5B0%5D=Hello%20World][twimlet_message_url_hello_world]`<Response>`个示例，单击以查看包含`<Say>`元素的元素。
