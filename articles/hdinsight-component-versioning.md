...
HDInsight 支持多个可随时部署的 Hadoop 群集版本。每个版本选项设置 Hortonworks 数据平台 (HDP) 分发的特定版本和该分发内包含的一组组件。下表中逐项列出了与每个 HDInsight 群集版本关联的组件版本。请注意，[Azure HDInsight][Azure HDInsight] 使用的默认群集版本当前是 2.1（基于 HDP 1.3）。
...
> [WACOM.NOTE] HDInsight 2.1 和 3.0 群集均运行在 Azure 来宾 OS [系列 4][系列 4]上，该系列使用 64 位版本的 Windows Server 2012 R2 并支持 .NET Framework 4.0、4.5 和 4.5.1。

### 有关版本控制的其他说明和信息

-   SQL Server JDBC 驱动程序由 HDInsight 在内部使用，不用于外部操作。如果你希望使用 ODBC 连接到 HDInsight，请使用 Microsoft Hive ODBC 驱动程序。有关使用 Hive ODBC 的详细信息，请参阅[使用 Microsoft Hive ODBC 驱动程序将 Excel 连接到 HDInsight][使用 Microsoft Hive ODBC 驱动程序将 Excel 连接到 HDInsight]。
...
-   HDInsight 群集版本 3.0 使用基于 [Hortonworks 数据平台 2.0][Hortonworks 数据平台 2.0] 的 Hadoop 分发。

-   HDInsight 群集版本 2.1 使用基于 [Hortonworks 数据平台 1.3][Hortonworks 数据平台 1.3] 的 Hadoop 分发。这是在使用 Azure HDInsight 门户时创建的默认 Hadoop 群集。

-   HDInsight 群集版本 1.6 使用基于 [Hortonworks 数据平台 1.1][Hortonworks 数据平台 1.1] 的 Hadoop 分发。

-   与 HDInsight 群集版本关联的组件版本可能在将来的 HDInsight 更新中更改。确定可用组件并验证正在使用哪些群集版本的一种方法是使用 Ambari REST API。GetComponentInformation 命令可用于检索有关服务组件的信息。有关详细信息，请参阅 [Ambari 文档][Ambari 文档]。获取此信息的另一个方法是使用远程桌面登录到群集并直接检查“C:\\apps\\dist”目录的内容。
