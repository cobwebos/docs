...
* 在 Azure 专用对等互连或 ExpressRoute 线路的 Microsoft 对等互连上启用双向转发检测（BFD）。 如果在 1 2018 年8月之后创建了 ExpressRoute 线路 10 2020，则在 Azure 专用对等互连上默认启用 BFD。 如果线路在此之前创建，则未启用 BFD。 可以通过禁用对等互连并重新启用它来启用 BFD。 
