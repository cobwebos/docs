...
- 使用中的有效 Azure 帐户。需有此帐户才能设置 ExpressRoute 线路。ExpressRoute 线路是 Azure 订阅中的资源。即使连接仅限于非 Azure Microsoft 云服务（例如 Office 365 服务和 CRM Online），也必须提供一个 Azure 订阅。
- 有效的 Office 365 订阅（如果使用 Office 365 服务）。有关详细信息，请参阅本文中的 [Office 365 的具体要求](#office-365-specific-requirements)部分。

## 连接服务提供商
- 你可以通过 [ExpressRoute 连接服务合作伙伴](/documentation/articles/expressroute-locations/#partners)连接到 Microsoft 云。有[三种方法](/documentation/articles/expressroute-introduction/#howtoconnect)可以在本地网络与 Microsoft 之间建立连接。 
- 即使你的提供商不是 ExpressRoute 连接服务合作伙伴，你也可以通过[云交换服务提供商](/documentation/articles/expressroute-locations/#nonpartners)连接到 Microsoft 云。

## 网络要求
- **冗余连接**：对于你和提供商之间的物理连接，没有冗余方面的要求。即使你只有[一个到云交换位置的物理连接](/documentation/articles/expressroute-faqs/#onep2plink)，Microsoft 也会要求在 Microsoft 的路由器和对等互连路由器之间建立冗余性的 BGP 会话。 
- **路由**：你或你的提供商需设置和管理针对[路由域](/documentation/articles/expressroute-circuit-peerings)的 BGP 会话，具体取决于你如何连接到 Microsoft 云。某些以太网连接服务提供商或云交换服务提供商可能会以增值服务的形式提供 BGP 管理。
- **NAT**：Microsoft 只能通过 Microsoft 对等互连方式接受公共 IP 地址。如果你使用的是本地网络中的专用 IP 地址，你或你的提供商需[使用 NAT](/documentation/articles/expressroute-nat) 将专用 IP 地址转换为公共 IP 地址。
- **QoS**：Skype for Business 的各种服务（例如语音、视频、文本）所要求的 QoS 处理方式各有差异。你和你的提供商应遵循 [QoS 要求](/documentation/articles/expressroute-qos)。
- **网络安全**：通过 ExpressRoute 连接到 Microsoft 云时，应考虑[网络安全](/documentation/articles/best-practices-network-security)。
...
- [CRM Online URLs（CRM Online URL）](https://support.microsoft.com/kb/2655102)和 [IP address ranges（IP 地址范围）](https://support.microsoft.com/kb/2728473)
