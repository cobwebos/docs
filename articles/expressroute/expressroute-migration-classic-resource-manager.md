...
* 验证是否具有最新版本的 Azure PowerShell 模块。 有关详细信息，请参阅[如何安装和配置 Azure PowerShell](/powershell/azure/overview)。 若要安装 PowerShell 服务管理模块（对于经典部署模型是必需的），请参阅[安装 Azure PowerShell 服务管理模块](/powershell/azure/servicemanagement/install-azure-ps)。
...
* 不支持 ExpressRoute 线路跨订阅迁移。 有关详细信息，请参阅[Microsoft。网络移动支持](../azure-resource-manager/management/move-support-resources.md#microsoftnetwork)。
