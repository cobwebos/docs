...
Azure 公共对等互连有1个与每个 BGP 会话关联的 NAT IP 地址。 通过 Microsoft 对等互连，可以配置自己的 NAT 分配，并使用路由筛选器进行选择性前缀播发。 公共对等互连是一种单向服务，使用该服务时，连接始终从你的 WAN 发起 Microsoft Azure 服务。 Microsoft Azure 服务无法通过此路由域发起到网络的连接。

启用公共对等互连后，可以连接到所有 Azure 服务。 我们不允许选择要将路由播发到的服务。 Microsoft 对等互连是双向连接，其中，可以从 Microsoft Azure 服务和 WAN 发起连接。 有关路由域和对等互连的详细信息，请参阅[ExpressRoute 线路和路由域](expressroute-circuit-peerings.md)。

## <a name="before"></a>准备工作

若要连接到 Microsoft 对等互连，你需要设置和管理 NAT。 连接服务提供商可以将 NAT 作为托管服务进行设置和管理。 如果计划在 Microsoft 对等互连上访问 Azure PaaS 和 Azure SaaS 服务，请务必正确设置 NAT IP 池大小。 有关适用于 ExpressRoute 的 NAT 详细信息，请参阅 [Microsoft 对等互连的 NAT 要求](expressroute-nat.md#nat-requirements-for-microsoft-peering)。 通过 Azure ExpressRoute （Microsoft 对等互连）连接到 Microsoft 时，你有多个 Microsoft 链接。 一个链接是现有的 Internet 连接，另一个是通过 ExpressRoute 的连接。 发往 Microsoft 一些流量可能通过 Internet 传输，但通过 ExpressRoute 返回，反之亦然。
...
在配置 Microsoft 对等互连之前，请参阅[具有多个网络路径的非对称路由](https://docs.microsoft.com/azure/expressroute/expressroute-asymmetric-routing)，以获取非对称路由注意事项。
...
默认情况下，在将路由筛选器附加到线路之前，新的 Microsoft 对等互连不会播发任何前缀。 创建路由筛选器规则时，可以指定要用于 Azure PaaS 服务的 Azure 区域的服务团体列表。 这为你提供了根据你的需求筛选路由的灵活性，如以下屏幕截图所示：
