---
title: 关于加密-Azure ExpressRoute |Microsoft Docs
...
否。 启用 MACsec 后，所有网络控制流量（例如，BGP 数据流量和客户数据流量）都将加密。 
...
是。 MACsec 保护你和 Microsoft 之间的物理连接。 IPsec 保护你和 Azure 上的虚拟网络之间的端到端连接。 您可以单独启用它们。 
...
是。 可以通过与 ExpressRoute 线路的 Microsoft 对等互连来设置此 IPsec 隧道。 遵循我们的[配置指南](site-to-site-vpn-over-microsoft-peering.md)。
...
否。 必须在 Azure 虚拟网络中部署第三方 VPN 网关，并在其与本地 VPN 网关之间建立 IPsec 隧道。
