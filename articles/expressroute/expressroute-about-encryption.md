...
不。 MACsec 使用一个实体（即 customer）拥有的密钥对物理链路上的所有流量进行加密。 因此，它仅可用于 ExpressRoute 直接访问。
...
不。 启用 MACsec 后，所有网络控制流量（例如，BGP 数据流量和客户数据流量）都将加密。 
...
不。 如果配置了 MACsec 并发生了键不匹配问题，则将失去与 Microsoft 的连接。 换句话说，我们不会回退到未加密的连接，从而公开你的数据。 
...
不。 必须在 Azure 虚拟网络中部署第三方 VPN 网关，并在其与本地 VPN 网关之间建立 IPsec 隧道。
