...
1. 为 ExpressRoute 导入 PowerShell 模块。
  ...
  你必须从 [PowerShell 库](http://www.powershellgallery.com/) 安装最新的 PowerShell 安装程序，并将 Azure Resource Manager 模块导入 PowerShell 会话，以便开始使用 ExpressRoute cmdlet。 你需要以管理员身份运行 PowerShell。
...
4. 配置线路的 Microsoft 对等互连。
  ...
  * MD5 哈希（如果选择使用）。 **这是可选的。**
  ...
  可以运行以下 cmdlet 为线路配置 Microsoft 对等互连：
