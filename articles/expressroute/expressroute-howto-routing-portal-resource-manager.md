...
可以为 ExpressRoute 线路配置 Azure 专用和 Microsoft 对等互连（新线路不推荐使用 Azure 公共对等互连）。 可以按照所选的任意顺序配置对等互连。 但是，必须确保一次只完成一个对等互连的配置。 有关路由域和对等互连的详细信息，请参阅[关于线路和对等互连](expressroute-circuit-peerings.md)。
