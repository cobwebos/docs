---
title: 部署 Azure 区块链工作台预览
description: 如何部署 Azure 区块链工作台预览
ms.date: 01/08/2020
ms.topic: article
ms.reviewer: brendal
...
---
# <a name="deploy-azure-blockchain-workbench-preview"></a>部署 Azure 区块链工作台预览

Azure 区块链工作台预览使用 Azure 应用商店中的解决方案模板进行部署。 该模板可以简化创建区块链应用程序所需的组件的部署。 部署后，Blockchain Workbench 提供对客户端应用的访问权限，以创建和管理用户与区块链应用程序。
...
* Azure Key Vault
* 服务总线
* SQL 数据库 （标准 S0） 和 SQL 逻辑服务器
* Azure 存储帐户（标准 LRS）
* 容量为 1 的虚拟机规模集
* 虚拟网络资源组（带负载均衡器、网络安全组、公共 IP 地址、虚拟网络）
* Azure 区块链服务。 如果您使用的是以前的区块链工作台部署，请考虑重新部署 Azure 区块链工作台以使用 Azure 区块链服务。
...
1. 选择**区块链** > **Azure 区块链工作台（预览版）。**
    ...
    | 数据库和区块链密码 | 指定密码，用于访问在部署过程中创建的数据库。 密码必须满足以下四个要求中的三个：长度必须在 12 & 72 个字符、1 个小写字符、1 个大写字符、1 个数字和 1 个特殊字符之间，不是数字符号（*）、百分比（）、逗号（、星\`形）、回引号（）、双引号（）、单引号（）、破折号（-）和半列（;) |
...
1. 在“高级设置”中，选择是要创建新的区块链网络，还是使用现有的权威证明区块链网络。****
    ...
    *创建新*选项将部署具有默认基本 sKU 的 Azure 区块链服务仲裁分类帐。

    ![新区块链网络的高级设置](media/deploy/advanced-blockchain-settings-new.png)

    | 设置 | 描述  |
    |---------|--------------|
    | Azure 区块链服务定价层 | 选择用于区块链工作台**的基本**或**标准**Azure 区块链服务层 |
    | Azure Active Directory 设置 | 选择“稍后添加”。****</br>注意：如果选择[预配置 Azure AD](#azure-ad-configuration) 或要重新部署，请选择“立即添加”。** |
    | VM 选择 | 为您的区块链网络选择首选的存储性能和 VM 大小。 如果使用具有较低服务限制的订阅（如 Azure 免费层），请选择较小的 VM（如标准 DS1 v2**）。 |
...
## <a name="blockchain-workbench-web-url"></a>区块链工作台网络网址
...
1. 在应用服务**概述**中，复制**URL**值，该值表示 Web URL 到已部署的区块链工作台。
...
您已完成 Azure 区块链工作台部署。 有关开始使用部署的建议，请参阅[后续步骤](#next-steps)。
...
Blockchain Workbench 部署要求注册 Azure AD 应用程序。 需要使用 Azure Active Directory (Azure AD) 来注册应用。 可以使用现有租户，也可以创建新租户。 如果使用现有的 Azure AD 租户，则需要拥有足够的权限才能在 Azure AD 租户中注册应用程序、授予图形 API 权限和允许来宾访问。 如果现有 Azure AD 租户中没有足够的权限，请创建新租户。


1. 登录到 Azure[门户](https://portal.azure.com)。
1. 在右上角选择自己的帐户，然后切换到所需的 Azure AD 租户。 租户应该是部署 Azure 区块链工作台的订阅的订阅管理员的租户，您有足够的权限注册应用程序。
1. 在左侧导航窗格中，选择“Azure Active Directory”服务****。 选择**应用注册** > **新注册**。

    ![应用注册](media/deploy/app-registration.png)

1. 提供显示**名称**，并**仅选择此组织目录中的帐户**。
...
1. 清单需要 GUID。 您可以使用 PowerShell 命令`[guid]::NewGuid()`或`New-GUID`cmdlet 生成 GUID。 还可以使用 GUID 生成器网站。
1. 对于您注册的应用程序，请在 **"管理**"部分中选择 **"清单**"。
1. 接下来，更新清单的应用**角色**部分。 替换为`"appRoles": []`提供的 JSON。 请务必将 **id** 字段的值替换为生成的 GUID。 
...
1. 在*区块链 API*应用注册中，选择**API 权限**。 默认情况下，将添加图形 API**用户.读取**权限。
1. 工作台应用程序需要读取用户的基本配置文件信息。 在 *"已配置的权限"* 中，选择 **"添加权限**"。 在**微软 API 中**，选择**微软图形**。
1. 由于工作台应用程序使用经过身份验证的用户凭据，请选择**委派权限**。
1. 在 *"用户"* 类别中，选择 **"用户.阅读基本"，所有**权限。

    ![Azure AD 应用注册配置，显示添加 Microsoft 图形用户.ReadBasic.所有委派权限](media/deploy/add-graph-user-permission.png)

    选择“添加权限”****。

1. 在 *"已配置的权限"* 中，选择 **"授予域管理员同意"，** 然后为验证提示选择 **"是**"。
...
1. 选择“身份验证”****。
1. 指定您在[区块链工作台 Web URL](#blockchain-workbench-web-url)部分检索到的 Azure 区块链工作台部署的主 Web URL。 回复 URL 带有 `https://` 前缀。 例如： `https://myblockchain2-7v75.azurewebsites.net`
