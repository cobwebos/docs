---
title: Azure 区块链工作台数据库视图
description: 可用 Azure 区块链工作台预览 SQL DB 数据库视图的概述。
ms.date: 09/05/2019
ms.topic: article
...
Azure 区块链工作台预览将数据从分布式分类账传递到*链外*SQL 数据库。 链外数据库使得可以使用 SQL 和现有工具（如[SQL Server Management Studio）](https://docs.microsoft.com/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-2017)与区块链数据进行交互。
...
| UploadedByUserExternalId         | nvarchar(255) | 否          | 上传应用程序的用户的外部标识符。 默认情况下，此 ID 是联合体 Azure 活动目录中的用户。                                                                                                |
...
| UserExternalId             | nvarchar(255) | 否          | 与角色关联的用户的外部标识符。 默认情况下，此 ID 是联合体 Azure 活动目录中的用户。                                                                     |
...
| UserExternalId           | nvarchar(255) | 否          | 与连接关联的用户的外部标识符。 默认情况下，此 ID 是联合体 Azure 活动目录中的用户。 |
| UserProvisioningStatus   | int           | 否          |标识用户的当前预配过程状态。 可能的值包括： <br />0 – API 已创建用户<br />1 – 已将密钥与数据库中的用户相关联<br />2 – 已完全预配用户 |
...
| ContractDeployedByUserId                 | int            | 否          | 部署合同的用户的外部标识符。 默认情况下，此 ID 是表示用户 Azure 活动目录 ID 的 guid。                                                                                                          |
| ContractDeployedByUserExternalId         | nvarchar(255)  | 否          | 部署合同的用户的外部标识符。 默认情况下，此 ID 是表示用户 Azure 活动目录 ID 的 guid。                                                                                                         |
...
| ContractDeployedByUserFirstName          | nvarchar(50)  | 是         | 部署合同的用户的名字 |
| ContractDeployedByUserLastName           | nvarchar(255) | 是         | 部署合同的用户的姓氏 |
| ContractDeployedByUserExternalId         | nvarchar(255) | 否          | 部署合同的用户的外部标识符。 默认情况下，此 ID 是在联合体 Azure 活动目录中表示其标识的 guid。                                                                                                                                                |
| ContractDeployedByUserEmailAddress       | nvarchar(255) | 是         | 部署合同的用户的电子邮件地址 |
...
| ContractActionExecutedByUserExternalId   | nvarchar(255) | 是         | 执行合同操作的用户的外部标识符。 默认情况下，此 ID 是在联合体 Azure 活动目录中表示其标识的 guid。 |
...
| TransactionIsWorkbenchTransaction        | bit           | 是         | 标识事务是否为 Azure 区块链工作台事务位 |
...
| StateName                          | nvarchar(50)  | 是         | 如果此属性包含协定的状态，则它是状态的显示名称。 如果此属性不与状态相关联，则值为 null。 |
| StateDisplayName                   | nvarchar(255) | 否          | 如果此属性包含状态，则它是状态的显示名称。 如果此属性不与状态相关联，则值为 null。 |
| StateValue                         | nvarchar(255) | 是         | 如果此属性包含状态，则它是状态值。 如果此属性不与状态相关联，则值为 null。 |
...
| StateName                          | nvarchar(50)  | 是         | 如果此属性包含状态，则它为状态的显示名称。 如果此属性不与状态相关联，则值为 null。 |
| StateDisplayName                   | nvarchar(255) | 否          | 如果此属性包含状态，则它是状态的显示名称。 如果此属性不与状态相关联，则值为 null。 |
| StateValue                         | nvarchar(255) | 是         | 如果此属性包含状态，则它是状态值。 如果此属性不与状态相关联，则值为 null。 |
...
|--------------------|---------------|-------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ID                 | int           | 否          | 用户的唯一标识符 |
| ExternalID         | nvarchar(255) | 否          | 用户的外部标识符。 默认情况下，此 ID 是表示用户 Azure 活动目录 ID 的 guid。 |
...
此视图呈现有关核心工作流元数据以及工作流函数和参数的详细信息。 它专为报告而设计，还包含有关与工作流关联的应用程序的元数据。 此视图包含来自多个基础表的数据，以促成工作流报告。 对于每个工作流，此视图包含以下数据：
...
此视图呈现有关核心工作流元数据以及工作流函数和参数的详细信息。 它专为报告而设计，还包含有关与工作流关联的应用程序的元数据。 此视图包含来自多个基础表的数据，以促成工作流报告。 对于每个工作流函数，此视图包含以下数据：
...
| WorkflowStateValue           | nvarchar(50)  | 否          | 状态的值 |
