...
对于那些刚刚进入区块链社区的人来说，该解决方案的发布是在 Azure 上以简单的可配置方式了解该技术的绝佳机会。 工作量证明是一种抗 Sybil 机制，利用计算成本对网络进行自我调节并允许公平参与。 这非常适合开放的匿名区块链网络，其中加密货币的竞争提升了网络安全性。 但是，在专用/联盟网络中，基础 Ether 没有值。 备用协议，概念验证的机构是更适合于其中所有达成一致意见参与方已知的允许网络且信誉好。 无需进行挖矿，权威证明更有效，同时仍保留拜占庭容错。
...
由于每个成员将具有多个同时运行的验证程序节点，并且每个节点必须具有允许的标识，务必验证程序可以安全地获取网络上唯一活动的标识。 若要简化此过程，我们已构建安全地保存生成的以太坊标识的每个成员的订阅中获取部署标识存储。 在部署后，业务流程容器将为每个验证程序生成的以太坊私钥，并将其存储在 Azure 密钥保管库。 在奇偶校验节点启动之前，它首先获取对未使用标识的租用，以确保该标识不会由另一个节点获取。 标识提供给为其授予开始创建块的权限的客户端。 如果托管 VM 发生故障，将释放标识租用，允许替换节点在将来恢复其标识。

### <a name="bootnode-registrar"></a>引导节点注册机构

为了便于连接，每个成员都可在[数据 API 终结点](#data-api)上托管一组连接信息。 此数据包括 bootnodes 作为对等互连节点提供联接的成员的列表。 作为此数据 API 的一部分，我们将此引导节点列表保持最新状态

### <a name="bring-your-own-operator"></a>自带运营商

联盟成员通常希望参与网络治理，但不希望运营和维护其基础结构。 与传统系统不同，整个网络中只有一个运营商与区块链系统的分散模型是背道而驰的。 每个联盟成员都可以将基础结构管理委托给他们选择的运营商，而不是雇用同一中介来运营网络。 这样，混合模型，其中可以选择每个成员运行自己的基础结构或向不同的合作伙伴的操作委托。 委托运营工作流的工作方式如下：
...
若要执行任何一种通过监管 DApp 交易，您将需要利用以太坊 wallet。  最简单的方法是使用 [MetaMask](https://metamask.io) 等浏览器内电子钱包；但是，由于它们是网络上部署的智能合同，因此还可以自动化与 Governance 合同的交互。

安装 MetaMask 后，在浏览器中导航到 Governance DApp。  可以在部署确认电子邮件中或通过部署输出中的 Azure 门户查找 URL。  如果您不具备安装浏览器内钱包你将无法再执行任何操作;但是，您仍可以读取管理员状态。  

#### <a name="becoming-an-admin"></a>成为管理员
如果您在网络部署的第一个成员，然后将自动成为管理员和奇偶校验节点将被列为验证程序。  如果您要加入网络，您将需要获取投票以管理员身份被大多数 （大于 50%）现有的管理员组。  如果你选择不成为管理员，则节点仍将同步并验证区块链；但是，它们不会参与块创建过程。 若要启动投票过程以成为管理员，请单击“提名”并输入 Ethereum 地址和别名。
...
#### <a name="admins"></a>管理员
“管理员”选项卡将显示当前管理员集并允许你投反对票。  一旦管理员将失去的详细信息比 50%的支持，它们将被删除为网络上的管理员。  此管理员拥有的任何验证程序节点将丢失验证程序状态并成为网络上的事务节点。  管理员可能会删除为任意数量的原因;但是，它由联盟预先同意的策略。

![管理员](./media/ethereum-poa-deployment/governance-dapp-admins.png)

#### <a name="validators"></a>验证程序
选择左侧菜单中的“验证程序”选项卡将显示此实例的当前已部署的奇偶校验节点及其当前状态（节点类型）。  由于此视图表示的当前已部署的联盟成员，每个联盟成员将在此列表中，有一组不同的验证程序。  如果这是新部署的实例，并且您没有添加您验证程序，将向您显示该选项为添加验证程序。  选择此项将自动选择的奇偶校验节点区域均衡的一组，并将其分配到验证程序集。  如果已部署的节点数大于允许的容量，则剩余节点将成为网络上的事务节点。
...
右上角是 Ethereum 帐户别名和 identicon。  如果你是管理员必须能够更新您的别名。
...
每个参数的详细说明如下：
  ...
  参数名称|描述|允许的值|默认值
  ---|---|---|---
联盟成员 ID|与参与联盟网络的每个成员相关联的 ID，用于配置 IP 地址空间以避免冲突。 在专用网络中，成员 ID 在同一网络中的不同组织之间应该是唯一的。  即使同一个组织部署到多个区域，也需要一个唯一的成员 ID。 记下此参数的值，因为稍后需要用来共享与其他联接的成员，以确保不会发生冲突。|0-255|NA
...
在监视边栏选项卡可以配置你的网络的 Azure Monitor 日志资源。 监视代理从网络收集并显示有用的指标和日志，从而提供快速检查网络运行状况或调试问题的功能。
...
[!INCLUDE [azure-monitor-log-analytics-rebrand](../../../includes/azure-monitor-log-analytics-rebrand.md)]
  ...
  ---|---|---|---
监视|启用监视的选项|启用或禁用|启用
连接到现有的 Azure Monitor 日志|创建新的 Azure Monitor 日志实例或加入现有实例|新建或加入现有|新建
监视位置 (连接到现有的 Azure Monitor 日志 = 创建新)|将部署新的 Azure Monitor 的记录实例的区域|所有 Azure 监视器都日志区域|NA
现有 log analytics 工作区 ID (连接到现有的 Azure Monitor 日志 = 加入现有)|工作区 ID 的现有 Azure Monitor 日志实例||NA
...
完成部署后，您可以访问所需的参数通过确认电子邮件或通过 Azure 门户。 这些参数中，可看到：
...
部署已成功完成并已预配所有资源后可以在资源组中查看的输出参数。
...
要扩展联盟，首先必须连接物理网络。
此首要步骤使用基于公共 IP 的部署，可顺畅完成。 如果部署 VPN 的后面，请参阅的部分[连接 VNet 网关](#connecting-vnet-gateways)执行新的成员部署的一部分的网络连接。  完成部署后，请使用 [Governance DApp](#governance-dapp) 成为网络管理员。
...
如果已使用默认的公共 IP 设置进行部署，则可以忽略此步骤。 在专用网络中，不同的成员通过 VNet 网关连接进行连接。 成员可以加入网络，并查看事务流量之前，现有成员必须执行其接受连接的 VPN 网关上的最终配置。 这意味着联接的成员的以太坊节点不会运行，直到建立的连接。 建议创建冗余的网络连接 （网格） 到联盟以减少单点故障的可能性。
...
现有成员必须运行以下 PowerShell 脚本来完成连接。 我们建议使用 Azure Cloud Shell 位于门户中的顶部右侧导航栏中。
...
出于安全因素，默认情况下，网络组安全规则拒绝 SSH 端口访问。 若要访问 PoA 网络中的虚拟机实例，您将需要更改此规则应用于\"允许\"
...
一旦部署，然后选择资源组中的实例。 可以在“概述”选项卡中找到用于访问流量管理器的 DNS 名称
...
对已部署网络中的每个区域重复此操作。 终结点后\"启用\"状态，它们将会自动加载和区域平衡的流量管理器 DNS 名称。 现在，可以在文档的其他步骤中使用此 DNS 名称替代 \[CONSORTIUM\_DATA\_URL\] 参数。

### <a name="data-api"></a>数据 API

每个联盟成员都拥有其他成员连接到网络所需的信息。 现有成员将提供该成员的部署前的 [CONSORTIUM_DATA_URL]。 部署后，加入的成员可从以下终结点的 JSON 接口检索信息：

`<CONSORTIUM_DATA_URL>/networkinfo`

响应将包含成员 （区块，验证设置协定 ABI，bootnodes） 和有用的信息加入现有成员 （验证程序地址） 的有用的信息。 我们鼓励使用此标准化来跨云提供程序扩展联盟。 此 API 返回具有以下结构的 JSON 格式的响应：
```json
{
  "$id": "",
  "type": "object",
...
解锁[个人 API](https://web3js.readthedocs.io/en/1.0/web3-eth-personal.html) 是不安全的操作。 机器人将侦听解锁的 Ethereum 帐户并尝试耗尽资金。 机器人假设这些帐户包含真实的以太，并试图第一个转走余额。 请勿在网络上启用个人 API。 而是对事务进行预签名，可以使用 MetaMask 之类的电子钱包进行手动签名，也可以使用[以编程方式与智能合同交互](#programmatically-interacting-with-a-smart-contract)部分中所述的方式以编程方式签名。
