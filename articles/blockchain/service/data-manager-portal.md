...
区块链数据管理器实例连接并监控 Azure 区块链服务事务节点。 只有有权访问事务节点的用户才能创建连接。 实例从事务节点捕获所有原始块和原始事务数据。 区块链数据管理器发布 **RawBlockAndTransactionMsg** 消息，它是从 web3 [getBlock](https://web3js.readthedocs.io/en/v1.2.0/web3-eth.html#getblock) 和 [getTransaction](https://web3js.readthedocs.io/en/v1.2.0/web3-eth.html#gettransaction) 查询返回的信息的超集。
