---
title: 配置区块链数据管理器-Azure CLI
description: 如何使用 Azure CLI 创建和管理区块链数据管理器
...
| 上层 | 与输入关联的观察程序的路径。 例如，**观察程序/mywatcher**。 |
...
出站连接将区块链数据发送到 Azure 事件网格。 可以将区块链数据发送到单个目标，或将区块链数据发送到多个目标。 对于任何给定的区块链数据管理器实例，区块链数据管理器支持多个事件网格主题出站连接。
...
| 上层 | 与输出关联的观察程序的路径。 例如，**观察程序/mywatcher**。 |
...
| 上层 | 与应用程序关联的观察程序的路径。 例如，**观察程序/mywatcher**。 |
