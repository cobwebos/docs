<properties
	pageTitle="数据目录入门 | Microsoft Azure"
...
4. 展开“定价”，选择 Azure 数据目录的**版本**（“免费”或“标准”）。![Azure 数据目录 - 选择版本](media/data-catalog-get-started/data-catalog-create-catalog-select-edition.png)
5. 展开“目录用户”，然后单击“添加”添加数据目录的用户。用户将自动添加到此组。![Azure 数据目录 - 用户](media/data-catalog-get-started/data-catalog-add-catalog-user.png)
6. 展开“目录管理员”，然后单击“添加”添加数据目录的其他管理员。用户将自动添加到此组。![Azure 数据目录 - 管理员](media/data-catalog-get-started/data-catalog-add-catalog-admins.png)
7. 单击“创建目录”创建组织的数据目录。创建数据目录后，将看到其主页。![Azure 数据目录 - 已创建](media/data-catalog-get-started/data-catalog-created.png)
...
6.	注册数据资产的元数据。在本示例中，将注册 AdventureWorks Production 命名空间中的 **Production/Product** 对象：
	...
	3. 单击**移动箭头** (**>**)。此操作将所有选定的对象移到“要注册的对象”列表中。
...
3. 确认是否在结果中看到全部四个表和数据库 (AdventureWorks2014)。可以单击工具栏上的按钮切换**网格视图**和**列表视图**，如下图所示。请注意，由于“突出显示”选项为“打开”，因此搜索结果中突出显示了搜索关键字。还可以指定搜索结果中**每页显示的结果数**。
...
有关发现数据资产的详细信息，请参阅 [How to discover data assets](data-catalog-how-to-discover.md)（如何发现数据资产）；有关搜索语法，请参阅 [Data Catalog Search syntax reference](https://msdn.microsoft.com/library/azure/mt267594.aspx)（数据目录搜索语法参考）。
...
3. 在“友好名称”中输入 **Product images**，在“描述”中输入 **Product photos for marketing materials**。
...
- 添加数据集级别的标记。在“标记”区域中单击“添加”。标记可以是用户标记或词汇标记。标准版的数据目录包包含帮助目录管理员定义中心业务分类的业务词汇。然后，目录用户可以使用词汇中的术语来批注数据资产。有关详细信息，请参阅 [How to set up the Business Glossary for Governed Tagging](data-catalog-how-to-business-glossary.md)（如何设置受管标记的业务词汇）
...
> [AZURE.NOTE] 请务必记住，Azure 数据目录不允许访问实际数据源，它只能方便发现和识别数据源。连接到数据源时，选择的客户端应用程序使用 Windows 凭据，或在必要时提示提供凭据。如果以前未获得数据源的访问权限，则必须先获得授权才能连接。
...
> [AZURE.NOTE] 本练习所述的管理功能只在标准版 Azure 数据目录中提供，免费版本不提供。在 Azure 数据目录中，可以获取数据资产的所有权、添加数据资产的共同所有者，以及设置数据资产的可见性。
...
4. 若要限制可见性，请选择“可见性”部分的“所有者和这些用户”，然后单击“添加”。在文本框中输入用户的电子邮件地址，然后按 **ENTER**。
...
> [AZURE.NOTE] 目录的默认行为是允许任何用户注册任何数据源，允许任何用户删除任何已注册的数据资产。标准版 Azure 数据目录中包含的管理功能提供其他选项，用于取得资产所有权、限制谁可以发现资产，以及谁可以删除资产。
...
- [How to register data assets](data-catalog-how-to-register.md)（如何注册数据资产）
- [How to discover data assets](data-catalog-how-to-discover.md)（如何发现数据资产）
- [How to annotate data assets](data-catalog-how-to-annotate.md)（如何批注数据资产）
- [How to document data assets](data-catalog-how-to-documentation.md)（如何记录数据资产）
- [How to connect to data assets](data-catalog-how-to-connect.md)（如何连接到数据资产）
- [How to manage data assets](data-catalog-how-to-manage.md)（如何管理数据资产）
