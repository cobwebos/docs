...
业务术语表仅在 Azure 数据目录标准版中可用。 数据目录的免费版不包含术语表，并且它不提供受治理标记的功能。
...
![数据目录 - 添加新术语表术语](./media/data-catalog-how-to-business-glossary/02-new-term.png)
...
在给出的层次结构级别里，术语必须是唯一的。 不允许重复名称。 层次结构中的级别数没有限制，但是三个或更少级别的层次结构通常更易于理解。
