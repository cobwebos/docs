...
* 分析师知道如何在他们支持的业务流程的上下文中使用数据。

每个观点都很有价值，数据目录使用众包方式捕获每个元数据和完全呈现已注册数据源。 使用数据目录门户，每个用户都可以添加和编辑自己的注释，同时可以查看其他用户提供的批注。
...
| 标记（术语表标记） |可在数据资产和属性/列级别中提供标记。 术语表标记是集中定义的术语表术语，可用于通过常见业务分类对数据资产或属性进行分类。 有关详细信息，请参阅  [如何为管辖标记设置业务术语表](data-catalog-how-to-business-glossary.md) |
