...
**Azure 数据目录**使 Adventure Works 财务团队能够就关键数据源进行协作，因此每个团队成员可以轻松地查找和使用所需的数据，并可以与整个团队共享他们的知识。
...
在选择试运行项目时，请选择其计划最引人注目且能解决现有业务问题的团队。 例如，某位业务分析师要从 SQL Server 数据库创建报表。 问题是，他们只是在与几个同事交谈后才意识到数据源。 最后，在浪费时间尝试查找要使用的数据源后，他们发现了一个 Excel 工作簿，其中包含每个数据源的说明。 尽管 Excel 工作簿充分描述了他们需要的表，但如果这些数据源在**Azure 数据目录**中注册并进行了说明，它们很快就会找到这些数据源。
...
**Azure 数据目录**解决的其中一个业务问题是将**数据生成者**连接到**数据使用者**。 其解决方式是作为企业数据源相关信息的中心存储库。 David 使用数据目录注册了 Adventure Works 和 SQL Server 数据源。 使用众包，任何发现此数据源的用户都可以分享她对数据的看法，以及使用他们发现的数据。 例如，Nancy 在搜索目录后找到数据源，并分享其关于数据的专业知识。  现在，组织中的其他人可以通过搜索数据目录从所分享的知识获益。
...
在本示例中，业务分析师 Nancy 使用 SQL Server Database 中的数据为其团队创建报告。 问题是，他们只是在与几个同事交谈后才意识到数据源。 如果在中央位置（如**Azure 数据目录**）中注册并进行注册和加带，他们很快就会找到这些数据源。
...
> [!NOTE]
> 我们已编写使用 **Azure 数据目录** API 的示例工具，将 Excel 工作簿迁移到数据目录。 有关数据目录 API 和示例工具的信息，请[下载即席工作簿代码示例](https://github.com/Azure-Samples/data-catalog-dotnet-excel-register-data-assets)，并查看 [Azure 数据目录 REST API](/rest/api/datacatalog/) 文档。
...
* **愿意失败**- 如果飞行员没有达到预期的结果，重新评估并确定要改变的领域 - 在转移到更大的范围之前修复试验中的问题。
