...
使用共享访问签名 (SAS) 是将对你存储帐户中 Blob、表和队列的受限访问权限授予其他客户端且不必公开帐户密钥的一种强大的方法。在有关共享访问签名的教程的第 1 部分中，我们将概要介绍 SAS 模型以及 SAS 最佳实践。本教程的[第 2 部分][第 2 部分]将逐步引导你完成使用 Blob 服务创建共享访问签名的过程。
...
10. **使用存储分析监视你的应用程序。**你可以使用日志记录和度量来观察由于你的 SAS 提供程序服务中的中断或者由于某一存储访问策略的无意中删除而导致的身份验证失败中的任何峰值情形。有关其他信息，请参阅 [Azure 存储空间团队博客][Azure 存储空间团队博客]。
