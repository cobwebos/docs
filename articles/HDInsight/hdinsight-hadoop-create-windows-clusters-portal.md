...
<properties
   pageTitle="在 HDInsight 中创建 Hadoop 群集 | Azure"
   	description="了解如何使用 Azure 管理门户创建 Azure HDInsight 的群集。"
   services="hdinsight"
   documentationCenter=""
   tags="azure-portal"
   authors="mumian"
...
# 在 HDInsight 中使用 Azure 管理门户创建基于 Windows 的 Hadoop 群集

[AZURE.INCLUDE [选择器](../includes/hdinsight-selector-create-clusters.md)]

了解如何在 HDInsight 中使用 Azure 管理门户创建 Hadoop 群集。Microsoft [Azure 管理门户](/documentation/articles/azure-portal-overview)是一个中心位置，你可以在其中预配和管理 Azure 资源。Azure 管理门户是可用于在 HDInsight 中创建基于 Linux 的或基于 Windows 的 Hadoop 群集的工具之一。有关其他群集创建工具和功能，请单击本页面顶部的相应选项卡，或参阅[群集创建方法](/documentation/articles/hdinsight-provision-clusters-v1#cluster-creation-methods)。
...
2. 单击“新建”，单击“数据分析”，然后单击“HDInsight”。
    ...
    ![在 Azure 管理门户中创建新群集](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.1.png "在 Azure 管理门户中创建新群集")
...
3. 键入或选择以下值：
  ...
  * **凭据**：配置 Hadoop 用户（HTTP 用户）的用户名和密码。如果你为该群集启用了远程桌面，则需要配置远程桌面用户的用户名、密码和帐户过期日期。在底部单击“选择”以保存更改。
	   	...
	   	![提供群集凭据](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.3.png "Provide cluster credentials")

  * **数据源**：创建新的 Azure 存储帐户，或选择现有的帐户，使其用作群集的默认文件系统。

   		![数据源边栏选项卡](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.4.png "Provide data source configuration")
  ...
  * **节点定价层**：设置该群集所需的辅助角色节点数。该群集的预估成本将显示在边栏选项卡内。
		...
		![节点定价层边栏选项卡](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.5.png "Specify number of cluster nodes")


  * “可选配置”用于选择群集的版本，以及配置其他可选设置，例如加入**虚拟网络**、设置**外部元存储**用于保存 Hive 和 Oozie 的数据、使用脚本操作来自定义要安装自定义组件的群集，或使用具有该群集的其他存储帐户。

  		* **HDInsight 版本**：选择要用于该群集的版本。有关详细信息，请参阅[HDInsight 群集版本](/documentation/articles/hdinsight-component-versioning-v1).
  		* **虚拟网络**：如果你想要将群集放入虚拟网络，请选择 Azure 虚拟网络和子网。  

			![虚拟网络边栏选项卡](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.6.png "Specify virtual network details")
		...
		* **外部元存储**：指定 Azure SQL 数据库用于存储与该群集关联的 Hive 和 Oozie 元数据。
			...
			![自定义元存储边栏选项卡](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.7.png "Specify external metastores")
  		...
  		* 如果你想要在创建群集时使用自定义脚本自定义群集，请选择“脚本操作”。有关脚本操作的详细信息，请参阅[使用脚本操作自定义 HDInsight 群集](/documentation/articles/hdinsight-hadoop-customize-cluster-v1)。在“脚本操作”边栏选项卡上提供如屏幕截图中所示的详细信息。
			...
			![脚本操作边栏选项卡](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.8.png "Specify script action")


    	* **Azure 存储密钥**：指定与该群集关联的其他存储帐户。在“Azure 存储密钥”边栏选项卡中，单击“添加存储密钥”，然后选择现有的存储帐户或创建新的帐户。
    

			![其他存储边栏选项卡](./media/hdinsight-provision-clusters-v1/HDI.CreateCluster.9.png "Specify additional storage accounts")


4. 单击“创建”。选择“固定到启动板”会将群集的磁贴添加到门户的启动板。该图标指示正在创建群集，完成创建后，将改为显示 HDInsight 图标。
...
5. 创建完成后，在启动板中单击群集磁贴，以启动群集边栏选项卡。群集边栏选项卡提供有关该群集的基本信息，如名称、其所属的资源组、位置、操作系统、群集仪表板 URL 等。
	...
	![群集边栏选项卡](./media/hdinsight-provision-clusters-v1/HDI.Cluster.Blade.png "群集属性")
	...
	* **删除**：删除 HDInsight 群集。
	* **快速启动** (![云和闪电图标 = 快速启动](./media/hdinsight-provision-clusters-v1/quickstart.png))：显示可帮助你开始使用 HDInsight 的信息。
	* **用户** (![用户图标](./media/hdinsight-provision-clusters-v1/users.png))：可让你设置 Azure 订阅上其他用户对此群集的_门户管理_权限。
	

		> [AZURE.IMPORTANT] 这_只会_影响在门户中对此群集的访问和权限，对于连接到 HDInsight 群集或将作业提交到其上的用户并没有作用。
		
	* **标记** (![标记图标](./media/hdinsight-provision-clusters-v1/tags.png))：标记可让你设置键/值对，以定义云服务的自定义分类。例如，你可以创建名为 __project__ 的键，然后对与特定项目关联的所有服务使用一个公用值。
