...
# 在基于 Linux 的 HDInsight 上部署和管理 Apache Storm 拓扑

在本文档中，你将了解有关如何在 HDInsight 群集上管理和监视基于 Linux 的 Storm 上运行的 Storm 拓扑的基本知识。

> [AZURE.IMPORTANT] 本文中的步骤需要使用 HDInsight 群集上基于 Linux 的 Storm。有关在基于 Windows 的 HDInsight 上部署和监视拓扑的详细信息，请参阅 [Deploy and manage Apache Storm topologies on Windows-based HDInsight（在基于 Windows 的 HDInsight 上部署和管理 Apache Storm 拓扑）](/documentation/articles/hdinsight-storm-deploy-monitor-topology/)

## 先决条件

* **HDInsight 群集上基于 Linux 的 Storm**：参阅 [Get started with Apache Storm on HDInsight（Apache Storm on HDInsight 入门）](/documentation/articles/hdinsight-apache-storm-tutorial-get-started/) 获取群集创建步骤
...
1. 使用 SSH 连接到 HDInsight 群集。将 **USERNAME** 替换为你的 SSH 登录名。将 **CLUSTERNAME** 替换为 HDInsight 群集名称：
...
2. 使用以下命令启动示例拓扑：
    ...
    > [AZURE.NOTE] 将拓扑提交到群集时，必须先复制包含群集的 jar 文件，然后再使用 `storm` 命令。可以从文件所在的客户端使用 `scp` 命令来实现此目的。例如 `scp FILENAME.jar USERNAME@CLUSTERNAME-ssh.azurehdinsight.cn:FILENAME.jar`
    >
...
`storm` 实用工具可让你从命令行使用正在运行的拓扑。下面是常用命令的列表。使用 `storm -h` 可以获取完整的命令行表。
...
> [AZURE.WARNING] 重新平衡某个拓扑首先会停用该拓扑，然后跨群集平均重新分布辅助角色，最后让拓扑返回到发生重新平衡之前的状态。因此，如果拓扑原本处于活动，则现在它将再次变为活动状态。如果它原本已停用，则现在它将保持停用状态。

    storm rebalance TOPOLOGYNAME

##使用 Storm UI 进行监视和管理

Storm UI 提供一个 Web 界面用于处理正在运行的拓扑，你的 HDInsight 群集随附了此界面。若要查看 Storm UI，请使用 Web 浏览器打开 __https://CLUSTERNAME.azurehdinsight.cn/stormui__，其中 __CLUSTERNAME__ 是群集的名称。
...
> [AZURE.IMPORTANT] Storm REST API 不能通过 Internet 公开使用，而必须使用与 HDInsight 群集头节点建立的 SSH 隧道来访问。有关创建和使用 SSH 隧道的信息，请参阅 [Use SSH Tunneling to access Ambari web UI, ResourceManager, JobHistory, NameNode, Oozie, and other web UI's（使用 SSH 隧道访问 Ambari Web UI、ResourceManager、JobHistory、NameNode、Oozie 和其他 Web UI）](/documentation/articles/hdinsight-linux-ambari-ssh-tunnel/)。
...
* __从 SSH 会话__：通过与群集建立的 SSH 会话使用命令 `headnode -f`。

* __从 Ambari Web__：从页面顶部选择“服务”，然后选择“Storm”。在“摘要”选项卡中，选择“Storm UI 服务器”。页面顶部将会显示运行 Storm UI 和 REST API 的节点的 FQDN。

* __从 Ambari REST API__：使用命令 `curl -u admin:PASSWORD -G "https://CLUSTERNAME.azurehdinsight.cn/api/v1/clusters/CLUSTERNAME/services/STORM/components/STORM_UI_SERVER"` 来检索有关运行 Storm UI 和 REST API 的节点的信息。将 __PASSWORD__ 替换为群集的管理员密码。将 __CLUSTERNAME__ 替换为群集名称。在响应中，“host\_name”条目包含节点的 FQDN。
...
现在，你已了解如何使用 Storm 仪表板部署和监视拓扑，接下来请了解如何[使用 Maven 开发基于 Java 的拓扑](/documentation/articles/hdinsight-storm-develop-java-topology/)。

有关更多示例拓扑的列表，请参阅 [Example topologies for Storm on HDInsight（Storm on HDInsight 的示例拓扑）](/documentation/articles/hdinsight-storm-example-topology/)。
