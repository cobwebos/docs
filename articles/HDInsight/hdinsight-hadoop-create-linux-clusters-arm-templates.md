...
了解如何使用 Azure Resource Manager (ARM) 模板创建 HDInsight 群集。有关详细信息，请参阅 [Deploy an application with Azure Resource Manager template](/documentation/articles/resource-group-template-deploy/)（使用 Azure Resource Manager 模板部署应用程序）。有关其他群集创建工具和功能，请单击本页顶部的相应选项卡，或参阅[群集创建方法](/documentation/articles/hdinsight-provision-clusters-v1/#cluster-creation-methods)。
...
有关详细信息，请参阅 [Deploy with PowerShell](/documentation/articles/resource-group-template-deploy/#deploy-with-powershell)（使用 PowerShell 进行部署）。
...
请参阅 [Deploy with the REST API](/documentation/articles/resource-group-template-deploy/#deploy-with-the-rest-api)（使用 REST API 进行部署）。
