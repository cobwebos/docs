...
如果未指定端口，SSH 将默认为端口 22，这会连接到 HDInsight 群集上的 headnode0。如果使用端口 23，将连接到 headnode1。有关头节点的详细信息，请参阅 [HDInsight 中的 Hadoop 群集的可用性和可靠性](hdinsight-high-availability-linux.md)。
...
4. 在辅助节点上执行完操作后，使用 `exit` 命令来关闭辅助节点的会话。这将使你返回到 `username@headnode` 提示符。
...
2. 将客户端程序（例如 Firefox）配置为使用 **localhost:9876** 作为 **SOCKS v5** 代理。下面是 Firefox 设置的外观：
	...
	![Firefox 设置图像](./media/hdinsight-hadoop-linux-use-ssh-unix/socks.png)
...
1. 在浏览器中打开 FoxyProxy 扩展。例如，在 Firefox 中，选择地址字段旁边的 FoxyProxy 图标。
	...
	![foxyproxy 图标](./media/hdinsight-hadoop-linux-use-ssh-unix/foxyproxy.png)

2. 选择**添加新代理**，再选择**常规**选项卡，然后输入代理名称 **HDInsightProxy**。

	![foxyproxy 常规](./media/hdinsight-hadoop-linux-use-ssh-unix/foxygeneral.png)
...
3. 选择**代理详细信息**选项卡并填充以下字段。
	...
	* **SOCKS v5** - 选择此项可设置代理所需的版本。
	...
	![foxyproxy 代理](./media/hdinsight-hadoop-linux-use-ssh-unix/foxyproxyproxy.png)
...
5. 在“FoxyProxy”对话框的顶部，将**选择模式**更改为**根据其预定义模式和优先级使用代理**，然后单击**关闭**。
	...
	![foxyproxy 选择模式](./media/hdinsight-hadoop-linux-use-ssh-unix/selectmode.png)
