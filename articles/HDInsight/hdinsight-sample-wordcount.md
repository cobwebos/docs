...
> [AZURE.NOTE]本文档中的步骤要求使用 Windows 客户端。有关使用基于 Linux 的 HDInsight 群集从 Linux、Unix 或 OS X 客户端使用该单词计数示例的步骤，请参阅[通过 SSH 将 MapReduce 与 HDInsight 上的 Hadoop 配合使用](hdinsight-hadoop-use-mapreduce-ssh.md)或[通过 Curl 将 MapReduce 与 HDInsight 上的 Hadoop 配合使用](hdinsight-hadoop-use-mapreduce-curl.md)。
