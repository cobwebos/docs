...
有关适用于装有 HDInsight 群集的 Jupyter 笔记本的自定义内核和 Spark magic 的详细信息，请参阅 [Kernels available for Jupyter notebooks with Apache Spark Linux clusters on HDInsight](/documentation/articles/hdinsight-apache-spark-jupyter-notebook-kernels)（适用于装有 HDInsight 上Apache Spark Linux 群集的 Jupyter 笔记本的内核）。
...
在本部分中，你将安装 Spark magic 以及 PySpark 和 Spark 内核，然后将这些内核配置为连接到 Azure HDInsight 中运行的 Apache Spark 群集。

1. 从 [Github](https://github.com/jupyter-incubator/sparkmagic/archive/publicpreview0.5.zip) 下载最新的 Spark magic 公共预览版。

2. 将下载的文件解压缩到磁盘上的某个位置。在以下说明中，此路径为 `$SPARKMAGIC_PATH`。

2. 运行以下命令

		pip install -r $SPARKMAGIC_PATH/requirements.txt  

3. 运行以下命令来安装 Spark magic。

		pip install -e $SPARKMAGIC_PATH

4. 安装 PySpark 和 Spark 内核。运行以下命令。
...
6. 验证是否可以使用 Jupyter 笔记本连接到群集，以及是否可以使用内核随附的 Spark magic。执行以下步骤。

	1. 创建新的笔记本。在右下角单击“新建”。你应会看到默认内核 **Python2**，以及安装的两个新内核 **PySpark** 和 **Spark**。
	...
	>[AZURE.TIP] 如果你想要更新笔记本配置以连接到不同的群集，请使用一组新值更新 config.json，如上述步骤 3 中所示。

## 为何要在计算机上安装 Jupyter？

你可能会出于多种原因而要在计算机上安装 Jupyter，然后将其连接到 HDInsight 上的 Spark 群集。

* 尽管 Azure HDInsight 中的 Spark 群集上已提供 Jupyter 笔记本，但在计算机上安装 Jupyter 可以选择在本地创建笔记本，根据正在运行的群集测试你的应用程序，然后将笔记本上载到群集。若要将笔记本上载到群集，可以使用群集上运行的 Jupyter 笔记本来上载，或者将它们保存到与群集关联的存储帐户中的 /HdiNotebooks 文件夹。有关如何在群集上存储笔记本的详细信息，请参阅 [Where are Jupyter notebooks stored?](/documentation/articles/hdinsight-apache-spark-jupyter-notebook-kernels#where-are-the-notebooks-stored)（Jupyter 笔记本存储在何处？）
...
>[AZURE.WARNING] 在本地计算机上安装 Jupyter 后，多个用户可以同时在同一个 Spark 群集上运行同一个笔记本。在这种情况下，将会创建多个 Livy 会话。如果你遇到问题并想要调试，则跟踪哪个 Livy 会话属于哪个用户将是一项复杂的任务。
