...
<properties
   pageTitle="使用 ARM 模板在 HDInsight 中创建基于 Windows 的 Hadoop 群集 | Azure"
...
# 使用 ARM 模板在 HDInsight 中创建基于 Windows 的 Hadoop 群集
...
可以在[附录 A](#appx-a-arm-template) 中找到用于创建 HDInsight 群集及所依赖的 Azure 存储帐户的 ARM 模板。使用文本编辑器将模板保存到工作站上的文件中。你将学习如何使用各种工具调用模板。
