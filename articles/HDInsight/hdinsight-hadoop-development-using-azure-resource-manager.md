...
有关其他用法信息，请参阅 [Customize Linux-based HDInsight clusters using Script Action（使用脚本操作自定义基于 Linux 的 HDInsight 群集）](/documentation/articles/hdinsight-hadoop-customize-cluster-v1)。

**与群集标识相关的 cmdlet：**

- **Add-AzureRmHDInsightClusterIdentity**：将群集标识添加到群集配置对象，使 HDInsight 群集能够访问 Azure 数据湖存储。请参阅 [Create an HDInsight cluster with Data Lake Store using Azure PowerShell（使用 Azure PowerShell 创建包含数据湖存储的 HDInsight 群集）](/documentation/articles/data-lake-store-hdinsight-hadoop-use-powershell)。
...
| 使用 .NET SDK 提交 Pig 作业 | 请参阅 [Submit Pig jobs（提交 Pig 作业）](/documentation/articles/hdinsight-hadoop-use-pig-dotnet-sdk-v1)|
