...
#####获取存储连接字符串
在你可以使用队列执行任何操作之前，你需要先获取将存放队列的存储帐户的连接字符串。你可以使用 **CloudStorageAccount** 类型来表示你的存储帐户信息。对于云服务项目，你可以使用 **CloudConfigurationManager** 类型从 Azure 服务配置检索你的存储连接字符串和存储帐户信息，如以下代码所示。
