---
title: "如何在 Azure RemoteApp 中重定向 USB 设备？ | Microsoft 文档"
...
设备重定向允许用户将连接到其计算机或平板电脑的 USB 设备与 Azure RemoteApp 中的应用一起使用。 例如，如果你通过 Azure RemoteApp 共享了 Skype，那么你的用户需要能够使用其摄像机设备。
...
第一个选项是使用[供供应商使用的系统定义的设备安装程序类](https://msdn.microsoft.com/library/windows/hardware/ff553426.aspx)。 选取与连接到本地计算机的设备最匹配的类。 对于数字相机来说，这可能是图像处理设备类或视频捕获设备类。 你将需要使用设备类做一些试验，找到适合本地连接的 USB 设备（在本例中是网络摄像机）的正确类 GUID。
...
你可以在同一个 cmdlet 中组合使用多个设备重定向。 例如︰要重定向本地存储和 USB 网络摄像机，cmdlet 看起来会如下所示︰

        Set-AzureRemoteAppCollection -CollectionName <collection name> -CustomRdpProperty "drivestoredirect:s:*`nusbdevicestoredirect:s:<Class Guid value>"

当按类 GUID 设置设备重定向时，会重定向与指定集合中的类 GUID 匹配的所有设备。 例如，如果在本地网络上有多个计算机使用同一个 USB 网络摄像机，那么你可以运行单个 cmdlet 来重定向所有网络摄像机。

## <a name="redirecting-a-usb-device-by-using-the-device-instance-id"></a>通过使用设备实例 ID 重定向 USB 设备
如果你需要更细粒度的控制并且希望按设备控制重定向，则可以使用 **USB\InstanceID** 重定向参数。

此方法最困难的部分是查明 USB 设备实例 ID。 你将需要访问计算机和特定 USB 设备。 然后，执行以下步骤：
...
4. 选择一个文件名和一个位置，例如MyConnection.rdp和This PC\Documents，然后保存文件。
