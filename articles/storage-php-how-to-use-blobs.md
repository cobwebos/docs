...
本指南将演示如何使用 Azure Blob 服务执行常见方案。示例是用 PHP 编写的并使用了 [Azure SDK for PHP][Azure SDK for PHP]。涉及的任务包括**上载**、**列出**、**下载**和**删除** Blob。有关 Blob 的详细信息，请参阅[后续步骤][后续步骤]部分。
...
-   [配置你的应用程序以访问 Blob 服务][配置你的应用程序以访问 Blob 服务]
...
有关 Blob 服务错误代码的详细信息，请参阅 [Blob 服务错误代码][Blob 服务错误代码]。
...
-   参阅位于以下位置的 PHP 页 Blob 示例：<https://github.com/WindowsAzure/azure-sdk-for-php-samples/blob/master/storage/PageBlobExample.php>
  ...
  [Azure SDK for PHP]: http://windowsazure.cn/zh-cn/documentation/articles/php-download-sdk/
  [后续步骤]: #NextSteps
  [什么是 Blob 存储]: #what-is
  [概念]: #concepts
