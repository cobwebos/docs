---
title: "Azure 资源运行状况常见问题 | Microsoft Docs"
...
# <a name="azure-resource-health-faq"></a>Azure 资源运行状况常见问题
了解 Azure 资源运行状况常见问题解答。

## <a name="frequently-asked-questions"></a>常见问题
* [什么是 Azure 资源运行状况？](#what-is-azure-resource-health)
* [资源运行状况的用途是什么？](#what-is-the-resource-health-intended-for)
* [资源运行状况执行何种运行状况检查？](#what-health-checks-are-performed-by-resource-health)
* [每个运行状态意味着什么？](#what-does-each-of-the-health-status-mean)
* [未知状态意味着什么？我的资源是否出了问题？](#what-does-the-unknown-status-mean-is-something-wrong-with-my-resource)
* [如果资源不可用，如何获取帮助？](#how-can-i-get-help-for-a-resource-that-is-unavailable)
* [资源运行状况是否会对平台问题导致的不可用与我的操作导致的不可用进行区分？](#does-resource-health-differentiate-between-unavailability-cased-by-platform-problems-versus-something-i-did)
* [能否将资源运行状况与监视工具集成？](#can-i-integrate-resource-health-with-my-monitoring-tools)
* [在何处查找资源运行状况？](#where-do-i-find-resource-health)
* [资源运行状况是否适用于所有资源类型？](#is-resource-health-available-for-all-resource-types)
* [如果我的资源显示可用，但我认为它不可用，该怎么办？](#what-should-i-do-if-my-resource-is-showing-available-but-i-believe-it-is-not)
* [资源运行状况是否适用于所有 Azure 区域？](#is-resource-health-available-for-all-azure-regions)
* [资源运行状况与服务运行状况仪表板或 Azure 门户服务通知有何不同？](#how-is-resource-health-different-from-the-service-health-dashboard-or-the-azure-portal-service-notifications)
* [是否需要激活每个资源的资源运行状况？](#do-i-need-to-activate-resource-health-for-each-resource)
* [是否需要为组织启用资源运行状况？](#do-we-need-to-enable-resource-health-for-my-organization)
* [是否免费提供资源运行状况？](#is-resource-health-available-free-of-charge)
* [资源运行状况提供的建议有哪些？](#what-are-the-recommendations-that-resource-health-provides)
...
1. 计划外事件，例如主机意外重新启动
2. 计划内事件，例如主机 OS 按计划更新
3. 用户操作触发的事件，例如用户重新启动虚拟机

## <a name="what-does-each-of-the-health-status-mean"></a>每个运行状态意味着什么？
有三种不同的运行状态：
1. 可用：Azure 平台中没有任何已知能够影响该资源的问题
2. 不可用：资源运行状况检测到影响资源的问题
3. 未知：资源运行状况无法确定资源的运行状况，因为已停止接收有关该资源的信息。 
...
资源运行状况是一项服务，旨在帮助你诊断和减轻影响资源的 Azure 服务问题。 虽然可以使用资源运行状况 API 来程序性地获取运行状态，但建议使用指标来监视资源。 一旦检测到问题，资源运行状况就可以帮助你确定根本原因，指导你完成解决问题所需的操作。 请访问 [Azure Monitor](https://docs.microsoft.com/azure/monitoring-and-diagnostics/)，详细了解如何使用指标来检查资源。

## <a name="where-do-i-find-resource-health"></a>在何处查找资源运行状况？
登录 Azure 门户以后，即可通过多种方式访问资源运行状况：
1. 导航到你的资源。 在左侧导航中，单击“资源运行状况”。
2. 转到 Azure Monitor 边栏选项卡。  在左侧导航中，单击“资源运行状况”。
3. 通过单击门户右上角的问号，然后选择“帮助 + 支持”，打开“帮助 + 支持”边栏选项卡。 该边栏选项卡打开后，单击“资源运行状况”

也可使用资源运行状况 API，获取你资源的运行状况的相关信息。
...
[Azure 状态](https://status.azure.com)和门户服务通知告知你影响大量客户（例如某个 Azure 区域的客户）的服务问题，而资源运行状况则会公开更细致的事件，这些事件仅与特定的资源相关。 例如，如果某个主机意外重新启动，资源运行状况仅提醒其虚拟机在该主机上运行的那些客户。
...
不能。  Azure 资源运行状况可以在 Azure 门户中访问，没有任何设置要求。
...
资源运行状况会根据运行状态为你提供建议，目的是减少你进行故障排除的时间。 这些建议针对可用资源，注重于如何解决客户遇到的最常见问题。 如果资源因 Azure 计划外事件而不可用，则会注重于为你提供恢复过程之中和之后的帮助。 
