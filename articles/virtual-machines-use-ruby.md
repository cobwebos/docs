...
[Azure 提供针对服务管理操作的 REST API][]，包括对 Azure 虚拟机的管理。Azure SDK for ruby 通过 **Azure::VirtualMachineSerivce** 类公开针对虚拟机的管理操作。使用该类可以访问通过 [Azure 管理门户][]提供的许多虚拟机管理功能。
...
你可以通过使用 Azure 跨平台命令行界面 (xplat-cli) 获取订阅 ID 和管理证书。有关安装和配置 xplat-cli 的信息，请参阅[安装和配置 Azure 跨平台命令行界面][]。
...
-   **:vm\_size** - 虚拟机的大小。这将确定内存大小、内核数目、带宽以及虚拟机的其他物理特性。有关可用大小和物理特性的信息，请参阅 [Azure 的虚拟机和云服务大小][]。
...
-   访问[虚拟机][]功能页
-   查看 MSDN 参考：[虚拟机][1]
-   了解如何[在虚拟机上托管 Ruby on Rails 应用程序][]
