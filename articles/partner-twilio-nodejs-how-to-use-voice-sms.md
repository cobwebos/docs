...
通过 Twilio，开发人员可[购买可编程电话号码][purchase_phone]，此类电话号码可用于收发呼叫和短信。  在某一 Twilio 号码接收到传入呼叫或文本时，Twilio 将向您的 Web 应用程序发送一个 HTTP POST 或 GET 请求，请求提供有关如何处理该呼叫或文本的说明。  你的服务器将使用 [TwiML][twiml] 响应 Twilio 的 HTTP 请求，TwiML 是一组简单的 XML 标记，包含有关如何处理呼叫或文本的说明。  稍后将显示 TwiML 的示例。
...
Twilio 提供一个客户端 SDK，它可以将任何桌面 Web 浏览器、iOS 应用程序或 Android 应用程序转换为 VoIP 电话。  在本文中，我们将重点介绍如何在浏览器中使用 VoIP 呼叫。  除了在浏览器中运行的 Twilio JavaScript SDK 以外，还必须使用服务器端应用程序（我们的 node.js 应用程序）向 JavaScript 客户端发布一个“功能标记”。  还可[在 Twilio 开发人员博客][voipnode]上深入了解如何将 VoIP 用于 node.js。

<a id="signup"/>

## <a name="sign-up-for-twilio-microsoft-discount"></a>注册 Twilio (Microsoft Discount)
使用 Twilio 服务之前，必须先[注册帐户][signup]。  Microsoft Azure 客户可享受特别折扣 - [请务必在此处注册][signup]！
...
选择您的 node.js 网站，然后单击“配置”链接。  如果您向下滚动一点，将会看到一个区域，您可以在该区域中为应用程序设置配置属性。  按照显示输入 Twilio 帐户凭据（[可在 Twilio 仪表板上找到][twilio_dashboard]）- 请确保将它们分别命名为“TWILIO_ACCOUNT_SID”和“TWILIO_AUTH_TOKEN”：
...
接下来，请创建一个 package.json，以便通过 [npm] 管理节点模块依赖项。  在与您在 Azure/node.js 教程中创建的“server.js”文件所处的相同级别上，创建一个名为“package.json”的文件。  在此文件中，放置以下代码：
...
我们将创建一个简单的表单，它将向我们选择的号码发出呼叫。  打开 server.js，然后输入以下代码。  请注意，在出现“CHANGE_ME”的地方放置您的 azure 网站的名称：
...
* [Votr - 一个具有 node.js 和 CouchDB 的实时 SMS 投票应用程序（三个部分）][votr]
