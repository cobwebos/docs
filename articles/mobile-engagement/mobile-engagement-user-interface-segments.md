...
在 UI 的“段”部分，可基于从应用程序获取的不同行为和分析对用户进行分段，并可通过“段 API”访问。 段首先计算它们会创建，并且它们重新计算基于最新的分析信息每 24 小时后 24 小时。 后计算出一段时，它显示的"Day 到一天的历史记录"图表每天。
...
使用段可对应用程序的最终用户进行细分。
对用户细分是一项重要的市场营销策略。 通过 Azure Mobile Engagement，可获取历史数据并创建自定义段。 借助这一强大的工具，可让你了解客户在应用程序中的体验。 可以很方便地对这些段进行分析，并将其用作推送目标。
一个常见的使用案例是：你想发送一条推送通知，鼓励最终用户在应用商店中对应用程序进行评级。 这种情况下，无需向所有最终用户发送通知，而是可以创建一个段，计算出上个月仅每天都在使用应用程序并且有很好用户体验的那部分用户。 当你发送较少、 更具针对性的推送通知时，你会获得更好的投资回报。
...
要以最佳方式使用段，必须使用“应用信息”标记的标签体系和代码部署计划在应用程序中完成 SDK 的自定义集成。
然后，请转到接口的主页上，选择你想，然后单击"段"部分的应用。

1. 选择“段”部分。
2. 单击“新建段”按钮创建新段。

## <a name="real-life-example-create-a-simple-segment-based-on-session-information"></a>真实示例：基于“会话”信息创建一个简单的段
针对上周使用应用至少 50 次的最终用户创建一个段。 从中找出每次会话在应用中至少停留了 30 秒的最终用户。 该结果会显示在应用中有正面体验的所有最终用户。 然后，创建的这个段可被用于向这些最终用户推送通知，让他们在应用商店中为应用评级。

 ![segments5][39]

1. 给段指定一个名称，以便在“段”列表中快速找到它。
2. 单击“创建”按钮。
...
5. 选择所需“次数”。 
6. 单击“下一步”。
   根据此示例的设置，匹配的用户在上一周内至少发起了 50 次会话。
   
   ![segments9][43]

为了对会话进行细分，可选择每个会话的长度作为条件。
