...
通过使用"新条件"按钮的特定条件目标受众是 Azure Mobile Engagement 可帮助你发送相关推送通知的功能最强大概念之一客户将响应而不是发送垃圾邮件的每个人。 可以基于标准条件限制访问群体，并模拟推送以确定有多少人将接收通知。
...
* * * Technicals: * * 可以面向基于可以分析和监视部分中看到的相同技术信息。 **另请参阅：**[UI 文档 - 分析][Link 15]、[UI 文档 - 监视器][Link 16]
* **位置：**用于"实时位置报告"地域隔离的应用程序可以使用地理位置作为条件来从 GPS 位置访问群体。 “简易区域位置报告”调用也可用于从手机位置确定访问群体（“实时位置报告”和“简易区域位置报告”必须从 SDK 中激活）。 **另请参阅：**[SDK 文档 - iOS - 集成][Link 5]、[SDK 文档 - Android - 集成][Link 5]
* **市场宣传反馈：**可以基于其反馈确定访问群体，这些反馈包括以前市场宣传通知的反馈乃至公告、投票和数据推送的市场宣传反馈。 这使您能够更好的目标受众两个或三个到达营销活动，不是你可以使用第一次。 通过将活动设置为不发送到已接收到特定的上一个活动的用户，还可用于筛选出已接收到含有类似内容的通知的用户。 甚至可以禁止仍处于活动状态的特定活动包含的用户接收新推送。 **另请参阅：**[UI 文档 - 市场宣传 - 推送内容][Link 29]
* **安装跟踪：**可以根据用户安装应用的位置跟踪信息。 **另请参阅：**[UI 文档 - 设置][Link 20]
* 
            **用户配置文件：**可以基于标准的用户信息确定目标，并且可以基于已创建的自定义应用信息确定目标。 这包括当前登录的用户和已回答特定问题的用户，已要求他们在应用自身中进行设置，而不是只是如何对以前的活动进行响应。 为应用定义的所有应用信息会显示在此列表上。
* 段：你还可以根据已基于包含多个条件的特定用户行为创建的段确定目标。 为应用定义的所有段会显示在此列表上。 **另请参阅：**[UI 文档 - 细分市场][Link 18]
...
3. 使应用程序设置为"true"当用户成功执行的应用内购买此应用程序信息 (通过使用 sendAppInfo ("inAppPurchase"，...) 函数)
4. 如果不想从应用程序中执行此操作，则可以通过使用设备 API 从后端执行此操作
5. 然后，只需使用将访问群体限制为将“inAppPurchase”设置为“true”的用户的条件来创建自己的公告
