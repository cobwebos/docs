...
有多种方法可获取令牌。 由于通常从云服务中调用 API，因此你需要使用 API 密钥。 在 Azure 术语中 API 密钥称为服务主体密码。 以下过程介绍一种手动设置的方法。
...
2. 安装 Azure PowerShell 后，使用以下命令以确保你已安装 **Azure 模块**：
    ...
    b. 如果在上述列表中找不到 Azure PowerShell 模块，则需要运行以下命令：
        ...
        Import-Module Azure 
3. 通过运行以下命令，并提供你的 Azure 帐户的用户名和密码，从 PowerShell 登录到 Azure Resource Manager，： 
   
        Login-AzureRmAccount
4. 如果你有多个订阅，则应运行以下命令：
   
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 获取所有订阅的列表，并复制你要使用的订阅的 SubscriptionId。 请确保此订阅具有你将使用 API 与之进行交互的 Mobile Engagement 应用的同一个订阅。 
...
6. 该脚本将要求你为 **principalName** 提供输入。 在此处提供你想使用的合适名称，以创建 Active Directory 应用程序（例如 APIAuth）。 
7. 在脚本完成后，它将显示以下四个值，你将需要以编程方式使用 AD 进行身份验证，因此，请务必复制它们。 
   
    **TenantId**、**SubscriptionId**、**ApplicationId** 和 **Secret**。
   
    你将使用 TenantId 作为 `{TENANT_ID}`、ApplicationId 作为 `{CLIENT_ID}`、Secret 作为 `{CLIENT_SECRET}`。
   
   > [!NOTE]
   > 你的默认安全策略可能会阻止你运行 PowerShell 脚本。 如果是这样，可以使用以下命令，暂时配置你的执行策略以允许脚本执行：
   > 
...
8. PS cmdlet 集应类似于如下所示。 
    ...
    ![][3]
9. 查看 Azure 管理门户，会有一个新的 AD 应用程序是使用你提供给“**显示我公司拥有的应用程序**”下名为 **principalName** 的脚本名称创建的。
...
至此，你拥有了一个有效的令牌，可以开始 API 调用。

1. 在每个 API 请求中，你需要传递在上节中获得的有效且未过期的令牌。
2. 你需要将某些参数插入到请求 URI，用于标识你的应用程序。 请求 URI 应类似如下所示
   
        https://management.azure.com/subscriptions/{subscription-id}/resourcegroups/{resource-group-name}/
        providers/Microsoft.MobileEngagement/appcollections/{app-collection}/apps/{app-resource-name}/
   
    若要获取参数，请单击你的应用程序名称并单击仪表板，你将看到如下页面和所有 3 个参数。
   
   * **1** `{subscription-id}`
   * **2** `{app-collection}`
   * **3** `{app-resource-name}`
   * **4** 除非你创建了一个新的资源组，否则你的资源组名称应该是 **MobileEngagement**。 
...
> 2. 如果你使用 Azure 经典门户创建了该应用，则需要使用不同于应用程序名称自身的应用程序资源名称。 如果你在 Azure 门户中创建了该应用，则应使用应用的自身名称（在新门户中创建的应用的应用程序资源名称与应用名称之间没有差异）。  
> 
> 
