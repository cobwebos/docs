<properties
	pageTitle="适用于 Windows Universal 应用的 Azure Mobile Engagement 入门"
...
[AZURE.INCLUDE [先决条件](../../includes/mobile-engagement-windows-store-prereqs.md)]
...
3. 复制之前为 Mobile Engagement 应用复制的连接字符串，将其粘贴到 `Resources\EngagementConfiguration.xml` 文件的 `<connectionString>` 与 `</connectionString>` 标记之间：
	...
	>[AZURE.TIP] 如果应用同时面向 Windows 和 Windows Phone 平台，仍应创建两个 Mobile Engagement 应用程序（每个受支持的平台一个）。两个应用可以确保创建正确的受众分段，并且可以适当地发送每个平台的定向通知。
...
> [AZURE.IMPORTANT] 如果页面重写了 `OnNavigatedTo` 方法，必须调用 `base.OnNavigatedTo(e)`。否则，该活动不会报告 `EngagementPage` 在其 `OnNavigatedTo` 方法内调用 `StartActivity`。这一点在 Windows Phone 项目中尤为重要，在该项目中，默认模板具有 `OnNavigatedTo` 方法。
...
3. 创建应用后，从左侧菜单导航到“服务”->“推送通知”。

	![][11]

4. 在“推送通知”部分中，单击“Live 服务站点”链接。

	![][12]

5. 导航到“推送凭据”部分。确保位于“应用设置”部分，复制“程序包 SID”和“客户端密码”

	![][13]

6. 导航到 Mobile Engagement 门户的“设置”，单击左侧的“原生推送”部分。单击“编辑”按钮，按如下所示输入“程序包安全标识符 (SID)”和“密钥”：
