...
3. 复制之前为 Mobile Engagement 应用复制的连接字符串，将其粘贴到 `Resources\EngagementConfiguration.xml` 文件的 `<connectionString>` 与 `</connectionString>` 标记之间：
    ...
    > [AZURE.IMPORTANT] NuGet 不会自动在 Windows 10 UWP 应用程序中复制 SDK 资源。 必须按照在安装 Nuget 包时显示的步骤手动进行操作。  
...
> 
> [!IMPORTANT]
> 有关 **Windows 10 通用应用**的信息，请使用[此推荐方法](mobile-engagement-windows-store-advanced-reporting.md#recommended-method-overload-your-codepagecode-classes)，而不是使用以上提及的方法。
> 
> 
