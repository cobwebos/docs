...
现在可以启用/禁用/筛选由 SDK 生成的控制台日志。 要自定义，请将属性 `EngagementAgent.Instance.TestLogEnabled` 更新为 `EngagementTestLogLevel` 枚举中提供的值之一，例如：
...
* 如果上一个覆盖正在工作，或者要手动集成 `WebView` 元素，可以决定保留现有文件，它仍将正常工作。
* 要更新到新的覆盖，请将资源中的整个 `overlay` 文件夹替换为 SDK 包中的新文件夹（UWP 应用：升级完成后，可以从 %USERPROFILE%\\.nuget\packages\MicrosoftAzure.MobileEngagement\3.4.0\content\win81\Resources 中获得新的覆盖文件夹）。
