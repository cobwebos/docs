...
### <a name="causes"></a>原因
* 可以使用 Analytics API、Monitor API 和 Segments API 查看 UI 中缺少的任何数据是否通过 API 可见。
* 如果 Azure Mobile Engagement SDK 未正确集成到应用中，则无法看到分析、分段、监视或仪表板中的信息。
...
* 筛选 UI 中的分析数据会显示此类型的所有示例，而不论应用是什么版本（例如，按名称筛选的“Crashes”将从应用的版本 1 和版本 2 显示）。
* 分析的时间段基于用户的设备设置中的日期，因此，手机日期未正确设置的用户可能会在错误的时间段中显示。
* 使用按钮“测试”推送时，不会记录任何服务器端数据，仅对于实际的推送活动，才会记录数据。
