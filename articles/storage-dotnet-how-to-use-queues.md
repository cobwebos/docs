...
> [WACOM.NOTE] 本指南适用于 Azure .NET 存储客户端库 2.x 及更高版本。建议使用的版本是存储客户端库 3.x，可通过 NuGet 或 Azure SDK for .NET 2.3 获得。请参阅下文中的 [如何：以编程方式访问队列存储][如何：以编程方式访问队列存储]，了解有关获得存储客户端库的详细信息。
