...
1. 选择“项目”，然后从项目列表中选择并配置要添加到基础映像的项目。 如果对开发测试实验室或配置项目不熟悉，请参阅[向 VM 中添加现有项目](devtest-lab-add-vm.md#add-an-existing-artifact-to-a-vm)部分，然后在完成后返回此处。
1. 选择“高级设置”来配置 VM 的网络选项和到期选项。 在“声明”选项下选择“是”，将虚拟机设为可声明。

  ![进行选择，使 VM 可声明。](./media/devtest-lab-add-vm/devtestlab-claim-VM-option.png)

1. 如果想要查看或复制 Azure Resource Manager 模板，请参阅[保存 Azure Resource Manager 模板](devtest-lab-add-vm.md#save-azure-resource-manager-template)部分，然后在完成后返回此处。
