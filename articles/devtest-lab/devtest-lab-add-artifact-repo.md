...
---
# <a name="add-a-git-repository-to-store-custom-artifacts-and-azure-resource-manager-templates"></a>添加 Git 存储库以存储自定义项目和 Azure 资源管理器模板
...
创建 VM 时，可以保存 Azure Resource Manager 模板，根据需要对其进行自定义，并在以后使用它轻松创建更多 VM。 必须创建自己的专用存储库来存储自定义 Azure Resource Manager 模板。  
...
要将存储库添加到实验室，必须先从存储库获取某些信息。 以下部分将指导获取在 GitHub 和 Visual Studio Team Services 上托管的存储库的某些信息。
...
4. 选择 GitHub 右上角的配置文件图像，并选择“设置”。
5. 在左侧的“个人设置”菜单中，选择“个人访问令牌”。
6. 选择“生成新令牌”。
7. 在“新的个人访问令牌”页上，输入“令牌说明”，并且接受“选择范围”默认项目，并选择“生成令牌”。
8. 保存生成的令牌，以备之后使用。
9. 现在可以关闭 GitHub。   
...
1. 打开团队集合主页（例如 `https://contoso-web-team.visualstudio.com`），并选择项目。
2. 在项目主页上，选择“代码”。
...
8. 在“创建个人访问令牌”页中：
   ...
   * 选择“所有范围”选项。
   * 选择“创建令牌”。
9. 完成后，新的标记将出现在“个人访问令牌”列表中。 选择“复制令牌”，并保存令牌值以备后用。
10. 继续转到[将实验室连接到存储库](#connect-your-lab-to-the-repository)部分。
...
* [如何对 Azure 开发测试实验室中的失败项目进行故障排除](devtest-lab-troubleshoot-artifact-failure.md)
