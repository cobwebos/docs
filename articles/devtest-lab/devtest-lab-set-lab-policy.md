...
Azure 开发测试实验室允许你通过管理每个实验室的策略（设置）来控制成本并尽量减少实验中的浪费。 本文将分步详细介绍如何设置每个策略。  
...
通过**每个用户的虚拟机**策略可指定个人用户可创建 VM 数量上限。 若用户在达到用户上限时尝试创建或索取 VM，则会出现错误消息，指出无法创建/索取 VM。 

1. 在实验室的“配置和策略”菜单上，选择“每个用户的虚拟机”。
   
    ![每个用户的虚拟机](./media/devtest-lab-set-lab-policy/max-vms-per-user.png)

1. 选择“是”以限制每个用户的 VM 数。 如果不希望限制每个用户的 VM 数，则选择“否”。 如果选择了“是”，请输入一个数字值，指示一个用户可创建或索取的 VM 数量上限。 

1. 选择“是”以限制可以使用 SSD（固态磁盘）的 VM 数。 如果不希望限制可以使用 SSD 的 VM 数，则选择“否”。 如果选择了“是”，请输入一个值，指示可以使用 SSD 创建的 VM 数量上限。 
...
1. 对于在指定的自动关闭时间之前 15 分钟发送通知的选项，指定“是”或“否”。 如果指定了“是”，请输入要接收通知的 webhook URL 终结点。 有关 webhook 的详细信息，请参阅[创建 webhook 或 API Azure 函数](../azure-functions/functions-create-a-web-hook-or-api-function.md)。 
