---
title: "应用服务中的 API 应用和 ASP.NET 入门 | Microsoft Docs"
...
---
# <a name="get-started-with-api-apps-aspnet-and-swagger-in-azure-app-service"></a>Azure 应用服务中的 API 应用、ASP.NET 和 Swagger 入门
...
* Visual Studio 2015 和 [用于 .NET 的 Azure SDK](https://azure.microsoft.com/downloads/archive-net-downloads/) - SDK 会自动安装 Visual Studio 2015（如果尚未安装）。
  ...
  * 在 Visual Studio 中，依次单击“帮助”->“关于 Microsoft Visual Studio”，并确保已安装“Azure App Service Tools v2.9.1”或更高版本。
    ...
    ![Azure 应用工具版本](./media/app-service-api-dotnet-get-started/apiversion.png)
...
3. 生成解决方案 (CTRL + SHIFT + B) 还原 NuGet 包。
   
    如果要在部署应用程序之前先查看应用程序的运行情况，可以在本地运行此应用程序。 确保 ToDoListDataAPI 是启动项目并运行解决方案。 应在浏览器中看到 HTTP 403 错误。

## <a name="use-swagger-api-metadata-and-ui"></a>使用 Swagger API 元数据和 UI
...
2. 在“发布 Web”向导的“配置文件”步骤中，单击“Microsoft Azure 应用服务”。
...
4. 在“应用服务”对话框中，选择要使用的 Azure **订阅**，然后单击“新建”。
...
5. 输入 *azurewebsites.net* 域中唯一的“API 应用名称”。 可以接受 Visual Studio 建议的默认名称。
   
    如果输入的名称已被使用，右侧会出现红色感叹号。
   
    API 应用的 URL 为 `{API app name}.azurewebsites.net`。
...
7. 单击“应用服务计划”下拉列表旁边的“新建”按钮。
    ...
    ![“创建 App Service”对话框](./media/app-service-api-dotnet-get-started/createas.png)
...
13. 单击“下一步”。
    ...
    下一个选项卡是“设置”选项卡（如下所示）。 可以在此处更改生成配置选项卡，部署用于[远程调试](../app-service-web/web-sites-dotnet-troubleshoot-visual-studio.md#remotedebug)的调试版本。 该选项卡还提供了多个“文件发布选项”： 
    
    * 删除目标处的其他文件
    * 在发布期间预编译
    * 从 App_Data 文件夹中排除文件
    
    在本教程中，不需要使用这些选项。 有关这些选项的作用的说明，请参阅 [How to: Deploy a Web Project Using One-Click Publish in Visual Studio](https://msdn.microsoft.com/library/dd465337.aspx)（如何：在 Visual Studio 中使用一键式发布来部署 Web 项目）。
14. 单击“下一步”。
    
    ![在“发布 Web”的“设置”选项卡中单击“下一步”](./media/app-service-api-dotnet-get-started/settingsnext.png)
    
    接下来是“预览”选项卡（如下所示），用于查看哪些文件即将从项目复制到 API 应用。 如果要将项目部署到前面已部署到的 API 应用，则只会复制已更改的文件。 如需查看要复制的项列表，请单击“ **开始预览** ”按钮。
15. 单击“发布” 。
...
20. 在“设置”边栏选项卡中，找到“API”部分并单击“API 定义”。
    ...
    使用“API 定义”边栏选项卡可以指定以 JSON 格式返回 Swagger 2.0 元数据的 URL。 `/swagger/docs/v1`。
...
4. 在“应用服务”对话框中，展开本教程中使用的资源组，选择 API 应用，然后单击“确定”。
    ...
    请注意，在返回“添加 REST API 客户端”对话框时，文本框中已填充前面在门户中看到的 API 定义 URL 值。
...
## <a name="create-an-api-app-to-host-the-middle-tier"></a>创建用于托管中间层的 API 应用
...
3. 在“应用服务”对话框中，单击“新建”。
4. 在“创建应用服务”对话框的“托管”选项卡中，接受默认的“API 应用名称”或者输入在 *azurewebsites.net* 域中唯一的名称。
5. 选择一直使用的 Azure **订阅** 。
...
## <a name="configure-the-middle-tier-to-call-the-data-tier"></a>配置用于调用数据层的中间层
...
1. 转到 [Azure 门户](https://portal.azure.com/)，然后导航到为托管 TodoListAPI（中间层）项目而创建的 API 应用的“API 应用”边栏选项卡。
2. 在 API 应用的“设置”边栏选项卡中，单击“应用程序设置”。
3. 在 API 应用的“应用程序设置”边栏选项卡中，向下滚动到“应用设置”部分并添加以下键和值。 该值将为你在本教程中发布的第一个 API 应用的 URL。
   
   | **键** | toDoListDataAPIURL |
   | --- | --- |
   | **值** |https://{数据层 API 应用名称}.azurewebsites.net |
...
2. 在浏览器的地址栏中，将“swagger”添加到 URL，然后按 Enter。 （URL 为 `http://{apiappname}.azurewebsites.net/swagger`。）
