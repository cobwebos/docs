...
3. 按照[入门](create-instance.md)指南重新创建托管域。
...
2. 修复子网的 IP 地址范围
   ...
   6. 单击要在表中编辑的子网。
   7. 更新地址范围并保存更改。
3. 按照[开始使用 Azure AD 域服务指南](create-instance.md)重新创建托管域。 请确保选取属于专用 IP 地址范围的虚拟网络。
...
此错误无法恢复。 若要解决此问题，必须[删除现有托管域](delete-aadds.md)并[重新创建托管域](create-instance.md)。
