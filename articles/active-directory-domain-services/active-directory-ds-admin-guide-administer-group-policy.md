...
3. 单击展开“林: contoso100.com”和“域”节点，以便查看托管域的组策略。 有两个内置的组策略对象 (GPO) - 托管域中的“AADDC 计算机”和“AADDC 用户”容器各一个。

    ![内置 GPO](./media/active-directory-domain-services-admin-guide/builtin-gpos.png)
4. 可以自定义这些内置 GPO，以便在托管域上配置组策略。 右键单击该 GPO，然后单击“编辑...”自定义内置 GPO。 可以使用组策略配置编辑器工具自定义该 GPO。

    ![编辑内置 GPO](./media/active-directory-domain-services-admin-guide/edit-builtin-gpo.png)
5. 现在可以使用**组策略管理编辑器**控制台编辑内置 GPO。 例如，下面的屏幕截图显示了如何自定义内置的“AADDC 计算机”GPO。
