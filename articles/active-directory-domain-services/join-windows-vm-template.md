...
1. 在 "**自定义部署**" 页上，输入以下信息以创建 WINDOWS Server VM 并将其加入到 Azure AD DS 托管域：
    ...
    | 域用户名           | 应用于将 VM 加入托管域的 Azure AD DS 托管域中的用户帐户，例如`contosoadmin@aaddscontoso.com`。 此帐户必须是 Azure AD DS 托管域的一部分。 |
...
1. 在 "**自定义部署**" 页上，输入以下信息以将 VM 加入到 Azure AD DS 托管域：
    ...
    | 域加入用户名     | 应用于将 VM 加入托管域的 Azure AD DS 托管域中的用户帐户，例如`contosoadmin@aaddscontoso.com`。 此帐户必须是 Azure AD DS 托管域的一部分。 |
