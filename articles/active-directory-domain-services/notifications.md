...
若要向你发出有关托管域的问题的警报，你可以配置电子邮件通知。 这些电子邮件通知指定出现警报的托管域，并提供检测时间和 Azure 门户中的 "运行状况" 页的链接。 然后，你可以按照提供的疑难解答建议解决这些问题。
...
Azure AD DS 发送电子邮件通知，以了解有关托管域的重要更新。 这些通知仅适用于影响服务并应立即解决的紧急问题。 每个电子邮件通知都由托管域上的警报触发。 这些警报也会显示在 Azure 门户中，并可在 " [AZURE AD DS 运行状况" 页][check-health]上查看。

Azure AD DS 不会发送电子邮件用于广告、更新或销售目的。

### <a name="when-will-i-receive-email-notifications"></a>为什么我会收到电子邮件通知？

当在托管域上发现[新警报][troubleshoot-alerts]时，将立即发送通知。 如果未解决警报，则会将其他电子邮件通知作为提醒每四天发送一次。
...
> [!WARNING]
> 更改通知设置时，将更新整个托管域的通知设置，而不是只更新自己的设置。
...
如果警报已解决，则会从 Azure 门户中清除警报。 最可能的原因是，接收电子邮件通知的其他人已在托管域上解决了警报，或者它已被 Azure 平台自动解决。

### <a name="why-can-i-not-edit-the-notification-settings"></a>为什么我无法编辑通知设置？

如果您无法访问 "Azure 门户中的" 通知设置 "页，则您没有编辑托管域的权限。 请与全局管理员联系，以获取编辑 Azure AD DS 资源或从收件人列表中删除的权限。
...
检查电子邮件中的 "垃圾邮件" 或 "垃圾邮件" 文件夹中是否有通知，并确保允许的发件人 `azure-noreply@microsoft.com` 。

## <a name="next-steps"></a>后续步骤

有关对可能报告的某些问题进行故障排除的详细信息，请参阅[在托管域上解决警报][troubleshoot-alerts]。
