---
title: 管理 DNS 的 Azure AD 域服务 |Microsoft Docs
description: 管理 Azure AD 域服务的 DNS
...
## <a name="task-1---create-a-domain-joined-virtual-machine-to-remotely-administer-dns-for-the-managed-domain"></a>任务 1 - 创建已加入域的虚拟机，以便远程管理托管域的 DNS
...
Azure AD 目录中的管理员无权通过远程桌面连接到托管域上的域控制器。 “AAD DC 管理员”组的成员可以使用已加入托管域的 Windows Server/客户端计算机中的 DNS 服务器工具对托管域的 DNS 进行远程管理。 DNS 服务器工具是远程服务器管理工具 (RSAT) 可选功能的一部分。

第一个任务是创建已加入托管域的 Windows Server 虚拟机。 有关说明，请参阅[将 Windows Server 虚拟机加入 Azure AD 域服务托管域](active-directory-ds-admin-guide-join-windows-vm.md)一文。

## <a name="task-2---install-dns-server-tools-on-the-virtual-machine"></a>任务 2 - 在虚拟机上安装 DNS 服务器工具
完成以下步骤，在已加入域的虚拟机上安装 DNS 管理工具。 有关[安装和使用远程服务器管理工具](https://technet.microsoft.com/library/hh831501.aspx)的详细信息，请参阅 Technet。
...
3. 若要连接到 VM，请打开下载的 RDP 文件。 出现提示时，请单击“连接”。  请使用属于“AAD DC 管理员”组的用户的凭据。 例如，“bob@domainservicespreview.onmicrosoft.com”。 你可能会在登录过程中收到证书警告。 单击“是”或“继续”以连接。
...
现在，你可以使用 Windows Server DNS 工具来管理托管域上的 DNS。
...
3. 在“连接到 DNS 服务器”  对话框中，单击“以下计算机”  ，并输入托管域的 DNS 域名（例如“contoso100.com”）。
