...
VM 需要其他一些包才能将 VM 加入托管域。 若要安装和配置这些包，请使用更新和安装域加入工具`apt-get`

在 Kerberos 安装过程中， *krb5.conf*包会提示所有大写的领域名称。 例如，如果托管域的名称为*aaddscontoso.com*，请输入*AADDSCONTOSO.COM*作为领域。 安装会 `[realm]` `[domain_realm]` 在 */etc/krb5.conf*配置文件中写入和节。 请确保将整个领域指定为全部大写：
...
1. 在*ntp*文件中，创建一个行以添加托管域的 DNS 名称。 在下面的示例中，添加了*aaddscontoso.com*的条目。 使用自己的 DNS 名称：

    ```console
    server aaddscontoso.com
    ```

    完成后，使用编辑器的命令保存并退出*ntp*文件。 `:wq`
...
1. 若要确保 VM 与托管域同步，需要执行以下步骤：
    ...
    * 启动 NTP 服务
    ...
    运行以下命令来完成这些步骤。 在命令中使用自己的 DNS 名称 `ntpdate` ：
...
既然已在 VM 上安装了所需的包，并且已配置了 NTP，请将 VM 加入托管域。

1. 使用 `realm discover` 命令发现托管域。 以下示例发现领域*AADDSCONTOSO.COM*。 以大写形式指定你自己的托管域名：
...
1. 现在使用命令初始化 Kerberos `kinit` 。 指定属于托管域的用户。 如果需要，请[将用户帐户添加到 Azure AD 中的组](../active-directory/fundamentals/active-directory-groups-members-azure-portal.md)。
...
将 VM 加入托管域需要一段时间。 以下示例输出显示 VM 已成功加入托管域：

```output
Successfully enrolled machine in realm
```

如果 VM 无法成功完成域加入过程，请确保 VM 的网络安全组允许 TCP + UDP 端口464上的出站 Kerberos 流量发送到托管域的虚拟网络子网。

如果收到错误 " *GSS 失败"。 次要代码可能提供详细信息（在 Kerberos 数据库中找不到服务器）*，打开文件 */etc/krb5.conf* ，并在部分中添加以下代码 `[libdefaults]` ，然后重试：
...
将 VM 加入托管域并配置为进行身份验证时，需要完成一些用户配置选项。 这些配置更改包括允许基于密码的身份验证，并在域用户首次登录时在本地 VM 上自动创建主目录。
...
1. 将*PasswordAuthentication*的行更新为 *"是"*：
    ...
    完成后，使用编辑器的命令保存并退出*sshd_conf*的文件 `:wq` 。
...
1. 将以下行添加到此文件中的行下面 `session optional pam_sss.so` ：

    ```console
    session required pam_mkhomedir.so skel=/etc/skel/ umask=0077
    ```

    完成后，使用编辑器的命令保存并退出*公用会话*文件 `:wq` 。

### <a name="grant-the-aad-dc-administrators-group-sudo-privileges"></a>为“AAD DC 管理员”组授予 sudo 特权

若要授予*AAD DC Administrators*组成员对 Ubuntu VM 的管理权限，请向 */etc/sudoers*添加一个条目。 添加后， *AAD DC 管理员*组的成员可以 `sudo` 在 Ubuntu VM 上使用命令。
...
1. 从控制台创建新的 SSH 连接。 使用命令（如）使用属于托管域的域帐户 `ssh -l` ， `contosoadmin@aaddscontoso.com` 然后输入 VM 的地址，例如*ubuntu.aaddscontoso.com*。 如果使用 Azure Cloud Shell，请使用 VM 的公共 IP 地址，而不使用内部 DNS 名称。
