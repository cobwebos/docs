...
[服务主体](../active-directory/develop/app-objects-and-service-principals.md)是 Azure 平台用于管理、更新和维护 Azure AD DS 托管域的应用程序。 如果删除了服务主体，Azure AD DS 托管域中的功能会受到影响。
...
如果需要的服务主体遭删除，Azure 平台无法执行自动化的管理任务。 Azure AD DS 托管域可能无法正确地应用更新或执行备份。
...
Azure AD DS 托管域的运行状况在两小时内自动自行更新，并删除警报。
...
删除这两个应用程序后，Azure 平台自动重新创建它们，并尝试恢复密码同步。 Azure AD DS 托管域的运行状况在两小时内自动自行更新，并删除警报。
