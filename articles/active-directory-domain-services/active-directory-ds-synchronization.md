...
Azure AD Connect 同步用于将用户帐户、组成员身份和凭据哈希同步到 Azure AD 租户。 用户帐户的属性，例如 UPN 和本地 SID（安全标识符），将会同步。 如果使用 Azure AD 域服务，则 NTLM 和 Kerberos 身份验证所需的旧凭据哈希也会同步到 Azure AD 租户。

如果配置了写回，则 Azure AD 目录中发生的更改将同步回到本地 Active Directory。 例如，如果使用 Azure AD 的自助密码更改功能更改了密码，则更改的密码将在本地 AD 域中更新。
...
相比之下，Azure AD 租户是一个简单得多的扁平命名空间。 为了使用户能够可靠地访问 Azure AD 保护的应用程序，需要解决不同林中用户帐户的 UPN 冲突。 Azure AD 域服务托管域与 Azure AD 租户非常类似。 因此，在托管域中会看到一个扁平的 OU 结构。 所有用户和组都存储在“AADDC 用户”容器中，不管它们是从哪个本地域或林同步的。 你可能在本地配置了分层 OU 结构。 但是，托管域仍采用简单的扁平 OU 结构。
...
* **计算机对象：**已加入本地域的计算机的计算机对象不会同步到你的托管域。 这些计算机未与托管域建立信任关系，它们仅属于本地域。 在托管域中，只能看到已显式加入该托管域的计算机的计算机对象。
...
| SAMAccountName |用户在 Azure AD 租户中的或自动生成的 mailNickname 属性 |SAMAccountName 属性源自 Azure AD 租户中的 mailNickname 属性。 如果多个用户帐户具有相同的 mailNickname 属性，将自动生成 SAMAccountName。 如果用户的 mailNickname 或 UPN 前缀长度超过 20 个字符，将自动生成 SAMAccountName，以满足 SAMAccountName 属性不超过 20 个字符的限制。 |
