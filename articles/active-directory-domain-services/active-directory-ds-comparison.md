...
使用 Azure AD 域服务可在 Azure 基础结构服务中部署工作负荷，无需考虑如何维护标识基础结构。 此托管服务不同于自行部署和管理的典型 Windows Server Active Directory 部署。 该服务在设计上易于部署，能够自动执行运行状况监视和补救，并针对云提供一个简单的标识基础结构。 我们正在持续改进该服务，以求添加对常见部署方案的支持。

若要确定是要使用 Azure AD 域服务，还是在 Azure 中运转和管理自己的 AD 基础结构 (DIY)：
...
| [**组策略**](active-directory-ds-comparison.md#group-policy) |简单 |完整 |
...
根据 Microsoft 的 AD 部署安全最佳实践安全锁定托管域。 这些最佳实践出自 AD 产品团队数十年的 AD 部署设计和支持经验。 在 DIY 部署中，需要执行特定的部署步骤才能锁定/保护部署。
...
无法通过配置托管域来设置与其他域的信任关系（入站/出站）。 因此，在资源林部署等方案中，或者在不想要将密码同步到 Azure AD 的情况下，无法使用 Azure AD 域服务。
...
* **将本地域扩展到 Azure：**可以使用 VPN/ExpressRoute 连接将 Azure 虚拟网络连接到本地网络，以便 Azure VM 可以加入本地 AD。 另一种做法是将 Azure 中本地域的副本域控制器升级为 VM。 然后，将它设置为通过与本地目录建立的 VPN/ExpressRoute 连接进行复制。 此部署模式可以有效地将本地域扩展到 Azure。
