...
4. **用于启用安全 LDAP 的证书**。
   ...
   * **建议** - 从企业 CA 或公共证书颁发机构获取证书。 这是更安全的配置选项。
...
1. **受信任的颁发者** - 证书必须由使用安全 LDAP 连接到域的计算机所信任的颁发机构颁发。 此颁发机构可以是组织的企业证书颁发机构，也可以是这些计算机所信任的公共证书颁发机构。
...
* 从证书颁发机构获取证书。 颁发机构可以是组织的企业 CA 或公共证书颁发机构。
* 创建自签名证书。
...
如果组织要部署企业公钥基础结构 (PKI)，则你需要从组织的企业证书颁发机构 (CA) 获取证书。 如果组织从公共证书颁发机构获取其证书，则你需要从该公共证书颁发机构获取安全 LDAP 证书。

请求证书时，请务必满足[安全 LDAP 证书的要求](#requirements-for-the-secure-ldap-certificate)中所述的要求。

> [!NOTE]
> 需要使用安全 LDAP 连接到托管域的客户端计算机必须信任 LDAPS 证书的颁发者。
> 
> 

### <a name="option-b---create-a-self-signed-certificate-for-secure-ldap"></a>选项 B - 为安全 LDAP 创建自签名证书
可以选择为安全 LDAP 创建自签名证书，但前提是：

* 组织中的证书并非由企业证书颁发机构颁发；或者
* 预计不会使用公共证书颁发机构颁发的证书。
...
在开始此任务之前，请确保已从企业证书颁发机构或公共证书颁发机构获取安全 LDAP 证书，或已创建自签名证书。
...
8. “配置”选项卡的“域服务”部分应该灰显，并且会处在“挂起...”状态几分钟。 在此期间，系统将验证 LDAPS 证书的正确性，同时为托管域配置安全 LDAP。
   ...
   > [!NOTE]
   > 为托管域启用安全 LDAP 大约需要 10 到 15 分钟时间。 如果提供的安全 LDAP 证书不符合所需的条件，则不会为目录启用安全 LDAP，并且会显示失败消息。 例如，域名不正确、证书已过期或即将过期，等等。
   > 
   > 
...
请配置外部 DNS 提供程序，使托管域的 DNS 名称（例如“contoso100.com”）指向此外部 IP 地址。 在本例中，需要创建以下 DNS 条目：
