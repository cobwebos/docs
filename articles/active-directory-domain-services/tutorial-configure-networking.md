...
在上一篇教程中，我们已创建一个托管域，该托管域对虚拟网络使用了一些默认的配置选项。 这些默认选项创建了一个 Azure 虚拟网络和虚拟网络子网。 提供托管域服务的 Azure AD DS 域控制器已连接到此虚拟网络子网。
...
1. 在 Azure 门户中，选择托管域的资源组，例如“myResourceGroup”。 在资源列表中选择默认的虚拟网络，例如 *aadds-vnet*。
...
1. 默认情况下，虚拟网络使用 Azure 提供的内置 DNS 服务器。 选择使用“自定义”DNS 服务器。 输入 Azure AD DS 域控制器的 IP 地址，通常是 *10.0.2.4* 和 *10.0.2.5*。 在门户上托管域的“概述”窗口中确认这些 IP 地址。
