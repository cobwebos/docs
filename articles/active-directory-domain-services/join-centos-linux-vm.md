---
title: 将 CentOS VM 加入 Azure AD 域服务 |微软文档
description: 了解如何配置 CentOS Linux 虚拟机并将其加入 Azure AD 域服务托管域。
...
本文介绍如何将 CentOS Linux VM 加入 Azure AD DS 托管域。
...
如果在 Azure 中具有现有的 CentOS Linux VM，请使用 SSH 连接到它，然后继续执行下一步[以开始配置 VM](#configure-the-hosts-file)。
...
* *centos*是要加入托管域的 CentOS VM 的主机名。
...
VM 需要一些其他包才能将 VM 加入 Azure AD DS 托管域。 要安装和配置这些包，请使用`yum`更新和安装域联接工具：
...
现在，所需的软件包安装在 VM 上，请将 VM 加入 Azure AD DS 托管域。
...
要授予 CentOS VM 上的*AAD DC 管理员*组管理权限，您需要向 */etc/sudoers*添加一个条目。 添加后 *，AAD DC 管理员*组的成员可以在 CentOS `sudo` VM 上使用该命令。
...
1. 从控制台创建新的 SSH 连接。 使用`ssh -l`命令使用属于托管域的域帐户，例如`contosoadmin@aaddscontoso.com`，然后输入 VM 的地址，如*centos.aaddscontoso.com*。 如果使用 Azure 云外壳，请使用 VM 的公共 IP 地址，而不是内部 DNS 名称。
