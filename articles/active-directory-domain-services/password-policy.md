...
> [!NOTE]
> 密码策略仅适用于使用资源管理器部署模型创建的托管域。 对于使用经典创建的旧托管域，请[从经典虚拟网络模型迁移到资源管理器][migrate-from-classic]。
...
* 在 Azure AD 租户中启用并配置 Azure Active Directory 域服务托管域。
  ...
  * 托管域必须使用资源管理器部署模型创建。 如果需要，请[从经典虚拟网络模型迁移到资源管理器][migrate-from-classic]。
...
细化密码策略（Fgpp）允许您将对密码和帐户锁定策略的特定限制应用于域中的不同用户。 例如，若要保护特权帐户，你可以应用更严格的帐户锁定设置，而不是常规的非特权帐户。 可以在托管域中创建多个 Fgpp，并指定优先级顺序以将其应用于用户。
...
对于在托管域中手动创建的用户帐户，还会通过默认策略应用以下其他密码设置。 这些设置不适用于从 Azure AD 同步的用户帐户，因为用户无法直接在 Azure AD DS 中更新其密码。
...
自定义密码策略将应用于托管域中的组。 此配置有效地覆盖默认策略。

若要创建自定义密码策略，请使用已加入域的 VM 中的 Active Directory 管理工具。 使用 Active Directory 管理中心可以查看、编辑和创建托管域中的资源，包括 Ou。

> [!NOTE]
> 若要在托管域中创建自定义密码策略，你必须登录到作为*AAD DC 管理员*组成员的用户帐户。
...
1. 在左窗格中，选择托管域，如*aaddscontoso.com*。
1. 打开 "**系统**" 容器，然后**密码设置容器**"。

    将显示托管域的内置密码策略。 你无法修改此内置策略。 而是创建一个自定义密码策略来替代默认策略。
...
1. 根据需要编辑其他密码策略设置。 请记住以下要点：
    ...
    * 仅限在托管域中手动创建的用户的密码复杂性、期限或过期时间等设置。
    * 帐户锁定设置适用于所有用户，但仅在托管域中生效，而不会在 Azure AD 本身生效。

    ![创建自定义细化密码策略](./media/password-policy/custom-fgpp.png)
...
1. 键入要向其应用策略的组的名称，然后选择 "**检查名称**" 以验证该组是否存在。
    ...
    ![搜索并选择要应用的组 FGPP](./media/password-policy/fgpp-apply-group.png)
