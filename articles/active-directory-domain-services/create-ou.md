...
Active Directory 域服务（AD DS）托管域中的组织单位（Ou）允许你以逻辑方式将对象分组，例如用户帐户、服务帐户或计算机帐户。 然后，可以将管理员分配给特定 Ou，并应用组策略来强制实施目标配置设置。

Azure AD DS 托管域包含以下两个内置 Ou：

* *AADDC 计算机*-包含已加入到托管域的所有计算机的计算机对象。
* *AADDC 用户*-包括 Azure AD 租户中从同步的用户和组。

当您创建和运行使用 Azure AD DS 的工作负荷时，可能需要为应用程序创建服务帐户以对其自身进行身份验证。 若要组织这些服务帐户，通常需要在托管域中创建自定义 OU，然后在该 OU 中创建服务帐户。

在混合环境中，在本地 AD DS 环境中创建的 Ou 不会同步到托管域。 托管域使用平面 OU 结构。 尽管所有用户帐户和组都是从不同的本地域或林进行同步，但即使已在此处配置了分层 OU 结构，所有用户帐户和组都将存储在*AADDC Users*容器中。
...
当你在托管域中创建自定义 Ou 时，你可以获得更多的管理灵活性，以便用户管理和应用组策略。 与本地 AD DS 环境相比，创建和管理托管域中的自定义 OU 结构时有一些限制和注意事项：
...
* 将创建*AADDC 用户*的默认 OU，其中包含 Azure AD 租户中所有已同步的用户帐户。
    * 不能将用户或组从 " *AADDC 用户*" OU 移到创建的自定义 ou。 仅可将在托管域中创建的用户帐户或资源移动到自定义 Ou 中。
* 在自定义 Ou 下创建的用户帐户、组、服务帐户和计算机对象在 Azure AD 租户中不可用。
    * 这些对象不会使用 Microsoft Graph API 或 Azure AD UI 中显示;它们仅在托管域中可用。

## <a name="create-a-custom-ou"></a>创建自定义 OU

若要创建自定义 OU，请使用已加入域的 VM 中的 Active Directory 管理工具。 使用 Active Directory 管理中心可以查看、编辑和创建托管域中的资源，包括 Ou。

> [!NOTE]
> 若要在托管域中创建自定义 OU，你必须登录到作为*AAD DC 管理员*组成员的用户帐户。
...
1. 在左窗格中，选择托管域，如*aaddscontoso.com*。 将显示现有 Ou 和资源的列表：

    ![在 Active Directory 管理中心中选择托管域](./media/create-ou/create-ou-adac-overview.png)

1. "**任务**" 窗格显示在 Active Directory 管理中心右侧。 在域下（如*aaddscontoso.com*），选择 "**新建 > 组织单位**"。

    ![选择用于在 Active Directory 管理中心中创建新 OU 的选项](./media/create-ou/create-ou-adac-new-ou.png)
...
1. 返回 Active Directory 管理中心，自定义 OU 现在已列出并可供使用：
    ...
    ![可在 Active Directory 管理中心中使用的自定义 OU](./media/create-ou/create-ou-done.png)
