...
VM 需要其他一些包才能将 VM 加入托管域。 若要安装和配置这些包，请使用更新和安装域加入工具 `yum` 。 RHEL 7、windows 和 RHEL 1.x 之间存在一些差异，因此请在本文的其余部分中使用适用于发行版版本的相应命令。
...
现在，所需的包已安装到 VM 上，请将 VM 加入托管域。 同样，请对 RHEL 发行版版本使用适当的步骤。
...
1. 使用 `adcli info` 命令发现托管域。 以下示例发现领域*ADDDSCONTOSO.COM*。 以大写形式指定你自己的托管域名：
...
1. 现在，配置 `/ect/krb5.conf` 并创建 `/etc/sssd/sssd.conf` 文件以使用 `aaddscontoso.com` Active Directory 域。
   请确保 `AADDSCONTOSO.COM` 将替换为你自己的域名：

    `/ect/krb5.conf`使用编辑器打开文件：

    ```console
    sudo vi /etc/krb5.conf
    ```

    更新 `krb5.conf` 文件以与以下示例匹配：
...
1. 用于 `authconfig` 指导 VM AD Linux 集成：
...
若要向*AAD DC Administrators*组成员授予 RHEL VM 的管理权限，请向 */etc/sudoers*添加一个条目。 添加后， *AAD DC 管理员*组的成员可以使用 `sudo` RHEL VM 上的命令。
...
1. 从控制台创建新的 SSH 连接。 使用命令（如）使用属于托管域的域帐户 `ssh -l` ， `contosoadmin@aaddscontoso.com` 然后输入 VM 的地址，例如*rhel.aaddscontoso.com*。 如果使用 Azure Cloud Shell，请使用 VM 的公共 IP 地址，而不使用内部 DNS 名称。
