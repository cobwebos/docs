...
在无法同步密码哈希的环境中，或者用户只使用智能卡登录，因此他们不知道密码的情况下，你可以在 Azure Active Directory 域服务 (AD DS) 中使用资源林。 资源林使用从 Azure AD DS 到一个或多个本地 AD DS 环境的单向出站信任。 这种信任关系可让用户、应用程序和计算机通过 Azure AD DS 托管域向本地域进行身份验证。 Azure AD DS 资源林目前为预览版。
...
* 使用资源林创建的并在 Azure AD 租户中配置的 Azure Active Directory 域服务托管域。
    * 如果需要，请[创建并配置 Azure Active Directory 域服务实例][create-azure-ad-ds-instance-advanced]。
    ...
    > [!IMPORTANT]
    > 确保使用资源林创建 Azure AD DS 托管域。  默认选项是创建用户林。  只有资源林才能创建对本地 AD DS 环境的信任。 此外，至少需要为托管域使用“企业版”SKU。  如果需要，请[更改 Azure AD DS 托管域的 SKU][howto-change-sku]。
...
若要从本地环境正确解析 Azure AD DS 托管域，可能需要向现有 DNS 服务器添加转发器。 如果尚未将本地环境配置为与 Azure AD DS 托管域进行通信，请在管理工作站中针对本地 AD DS 域完成以下步骤：
...
本地 AD DS 域需要 Azure AD DS 托管域的传入林信任。 必须手动在本地 AD DS 域中创建此信任，而不能在 Azure 门户中创建。
...
1. 选择创建“仅限此域”的信任。  在下一步骤中，你将在 Azure 门户中为 Azure AD DS 托管域创建信任。
...
配置本地 AD DS 域来解析 Azure AD DS 托管域并创建入站林信任后，现在请创建出站林信任。 此出站林信任完成本地 AD DS 域与 Azure AD DS 托管域之间的信任关系。

若要在 Azure 门户中为 Azure AD DS 托管域创建出站信任，请完成以下步骤：

1. 在 Azure 门户中，搜索并选择“Azure AD 域服务”，然后选择你的托管域，例如 aaddscontoso.com  
1. 从 Azure AD DS 托管域左侧的菜单中选择“信任”，然后选择“+ 添加”以添加信任。  

   > [!NOTE]
   > 如果看不到“信任”菜单选项，请在“属性”下检查“林类型”。    只有资源林才能创建信任。  如果林类型是“用户”，则无法创建信任。  目前没有任何办法可以更改 Azure AD DS 托管域的林类型。 需要删除托管域，然后重新创建资源林形式的托管域。
