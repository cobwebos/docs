...
| 目标资源 | 方案 |
|:---|:---|
|Azure 存储| 如果你的主要需求是出于存档目的存储安全审核事件，则应使用此目标。 其他目标可用于存档目的，但是这些目标提供的功能超出了存档的主要需求。 启用 Azure AD DS 安全审核事件之前，请先[创建一个 Azure 存储帐户](../storage/common/storage-quickstart-create-account.md?tabs=azure-portal#create-a-storage-account-1)。|
...
1. 在 Azure 门户顶部，搜索并选择**Azure AD 域服务**"。 选择你的托管域，例如 *contoso.com*。
