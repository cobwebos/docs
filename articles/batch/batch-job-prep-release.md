---
title: 创建任务以在计算节点上准备 & 完成作业-Azure Batch
...
在执行作业的任务之前，Batch 在计划运行任务的每个计算节点上执行作业准备任务。 默认情况下，批处理等待作业准备任务完成，然后再运行计划在节点上执行的任务。 但可以将该服务配置为不要等待。 如果节点重新启动，作业准备任务将再次运行。 你还可以禁用此行为。 如果有作业准备任务和作业管理器任务已配置，作业准备任务在作业管理器任务之前运行，就像对所有其他任务执行任务一样。 作业准备任务始终首先运行。
...
若要查看操作中的作业准备和释放任务，请查看 GitHub 上的[JobPrepRelease][job_prep_release_sample]示例项目。 此控制台应用程序将执行以下操作：
...
当你运行示例应用程序时，你可以使用[Azure 门户][portal]查看作业及其任务的属性，甚至可以下载作业任务修改的共享文本文件。
