...
作业准备任务只会在计划运行任务的节点上运行。例如，这可以防止未分配任务的节点不必要地执行准备任务，从而节省数据传输费用。当作业的任务数小于池中的节点数，或者在任务计数小于可能的并行任务总数的情况下启用了[并行任务执行](batch-parallel-node-tasks.md)，从而留出一些空闲节点时，便可以实现上述目的。
