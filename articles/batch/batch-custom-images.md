---
title: 从托管映像预配自定义池 - Azure 批处理 |微软文档
description: 从托管映像资源创建批处理池，以便使用应用程序的软件和数据预配计算节点。
services: batch
author: LauraBrenner
manager: evansma
ms.service: batch
...
ms.locfileid: "77020142"
---
# <a name="use-a-managed-image-to-create-a-pool-of-virtual-machines"></a>使用托管映像创建虚拟机池

要为 Batch 池的虚拟机 （VM） 创建自定义映像，可以使用[共享映像库](batch-sig-images.md)或*托管映像*资源。

> [!TIP]
> 在大多数情况下，应使用共享图像库创建自定义图像。 通过使用共享映像库，您可以更快地预配池、扩展大量 VM，并在预配 VM 时提高了可靠性。 要了解更多信息，请参阅[使用共享图像库创建自定义池](batch-sig-images.md)。
...
6. 指定其余必需的设置，包括**节点大小**、**目标专用节点**和低**优先级节点**，以及任何所需的可选设置。
...
通过使用[共享图像库](batch-sig-images.md)，您可以使用自定义图像以及更多共享图像副本创建更大的池。 使用共享映像时，池达到稳定状态所需的时间最多可以缩短 25%，VM 空闲延迟最多可以缩短 30%。

## <a name="considerations-for-using-packer"></a>使用包装机的注意事项

直接使用 Packer 创建托管映像资源只能通过用户订阅模式批处理帐户完成。 对于批处理服务模式帐户，您需要先创建 VHD，然后将 VHD 导入托管映像资源。 根据您的池分配模式（用户订阅或批处理服务），创建托管映像资源的步骤将有所不同。

确保用于创建托管映像的资源存在于引用自定义映像的任何池的生存期内。 否则可能会导致池分配失败和/或调整大小。

如果删除映像或基础资源，您可能会收到类似于：`There was an error encountered while performing the last resize on the pool. Please try resizing the pool again. Code: AllocationFailed`的错误。 如果收到此错误，请确保尚未删除基础资源。

有关使用打包器创建 VM 的详细信息，请参阅[使用打包器构建 Linux 映像](../virtual-machines/linux/build-image-with-packer.md)或使用[打包器生成 Windows 映像](../virtual-machines/windows/build-image-with-packer.md)。
