...
大规模地并行执行任务是 Azure Batch 的核心功能。此功能不仅可以在多个计算节点上运行任务，而且可以并行运行这些任务。利用 Batch，你可以横向和纵向扩展任务的并行执行规模。
...
你可以在Batch 解决方案中进行池级别的计算节点配置，以便并行执行任务。在创建池时，可以通过 Batch .NET API 设置 [CloudPool.MaxTasksPerComputeNode][maxtasks_net] 属性。在 Batch REST API 中，[maxTasksPerNode][maxtasks_rest] 元素是在创建池时在请求正文中设置的。
...
[Batch 资源管理器][batch_explorer]是 Azure Batch [示例应用程序][github_samples]之一，包含 *热度地图* 功能，提供任务执行可视化。执行 [ParallelTasks][parallel_tasks_sample] 示例应用程序时，可以使用“热图”功能来轻松可视化每个节点上并行任务的执行。
