---
title: 使用 Azure Pipelines 生成 & 部署 HPC 解决方案-Azure Batch
...
* **管道**文件夹，其中包含生成管道的定义。
* **可选**：将存储 .net 应用程序代码的**客户端应用程序**文件夹。 我们不会在示例中使用它，但在你自己的项目中，你可能希望通过客户端应用程序执行 HPC 批处理应用程序的运行。
...
1. 可以通过两种方法创建生成管道：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 [使用可视化设计器](https://docs.microsoft.com/azure/devops/pipelines/get-started-designer?view=azure-devops&tabs=new-nav)。 为此，请单击 "**新建管道**" 页上的 "使用可视化设计器"。

    b.保留“数据库类型”设置，即设置为“共享”。 [使用 YAML 生成](https://docs.microsoft.com/azure/devops/pipelines/get-started-yaml?view=azure-devops)。 可以通过单击 "新建管道" 页上的 "Azure Repos" 或 "GitHub" 选项来创建新的 YAML 管道。 或者，你可以在源代码管理中存储下面的示例，并通过单击 "可视化设计器"，然后使用 YAML 模板引用现有的 YAML 文件。
...
1. 导航到 "**变量**" 部分。 建议在管道中创建多个变量，因此不会在多个任务中输入相同的信息。 下面是此示例中使用的变量，以及它们如何影响部署。
    ...
    * **applicationStorageAccountName**：存储帐户的名称以保存 HPC 应用程序二进制文件
    ...
    * **位置**：要部署的 Azure 资源的位置
    * **resourceGroupName**：要创建的资源组的名称，以及你的资源将部署到的位置
    * **storageAccountName**：要保存链接资源管理器模板的存储帐户的名称
