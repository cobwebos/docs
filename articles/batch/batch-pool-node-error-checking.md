...
description: 本文介绍了可能发生的后台操作以及要检查的错误，以及在创建池和节点时如何避免这些错误。
services: batch
ms.service: batch
author: mscurrell
ms.author: markscu
...
你可以指定一个或多个容器上的容器引用。 Batch 会将指定的容器下载到每个节点。 节点[错误](https://docs.microsoft.com/rest/api/batchservice/computenode/get#computenodeerror)属性报告下载容器失败，并将节点状态设置为 "**不可用**"。
