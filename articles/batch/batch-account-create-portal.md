...
1. 输入“新 Batch 帐户”**** 设置。 查看以下详细信息。
    ...
    f. **池分配模式**：在 **"高级**设置"选项卡中，您可以将池分配模式指定为**批处理服务**或**用户订阅**。 对于大多数情况，请接受默认值“Batch 服务”****。
...
> [!NOTE]
> 确保在链接**的密钥保管库**资源的**Access 策略**下选择了**用于部署**的 Azure 虚拟机和**用于模板部署的 Azure 资源管理器**复选框。
> 
> ![](./media/batch-account-create-portal/key-vault-access-policy.png)在 Azure 门户中创建批处理帐户时，这不是强制性的。 默认情况下，选择该选项。
...
* [批量电源外壳 cmdlet](batch-powershell-cmdlets-get-started.md)
