...
若要使用应用程序包，必须 [将 Azure 存储帐户链接到](#link-a-storage-account) Batch 帐户。
...
* **包** -- 与此应用程序关联的版本号码。
* **默认版本** -- 如果在设置池的应用程序时未指定版本，系统将安装此版本。 此设置是可选的。
...
* **显示名称** - 指定在显示应用程序相关信息时（例如，通过 Batch 提供给客户的服务 UI 中），Batch 解决方案可以使用的“友好”名称。
...
> [!IMPORTANT]
> 如果应用程序包部署出于任何原因而失败，Batch 服务会将该节点标记为 [unusable][net_nodestate]，并且不会在该节点上计划执行任何任务。 在此情况下，应**重启**节点，以重新启动包部署。 重新启动节点也会在节点上再次启用任务计划。
> 
> 
