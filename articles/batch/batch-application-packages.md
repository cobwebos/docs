...
Batch 服务使用 Azure 存储将应用程序包存储为块 blob。 块 blob 数据[正常收费][storage_pricing]，且每个包的大小不能超过[最大块 blob 大小](../storage/common/storage-scalability-targets.md#azure-blob-storage-scale-targets)。 请务必考虑应用程序包的大小和数目，并定期删除过时的包以降低成本。
> 
> 
...
正如您所看到的字段匹配的那些**新的应用程序**窗口中，但**应用程序 ID**框处于禁用状态。 如前面创建新应用程序一样，指定新包的“版本”  ，浏览到“应用程序包”  .zip 文件，并单击“确定”  上传包。
...
[13]: ./media/batch-application-packages/package-file-structure.png "计算在 Azure 门户中的节点信息"
[14]: ./media/batch-application-packages/package-file-structure-node.png "在 Azure 门户中显示的计算节点上的文件"
