...
如果想要在计算节点上看到的应用程序包文件结构中，导航到在门户中的 Batch 帐户。 在 Batch 帐户中，导航到**池**。 选择包含您感兴趣的计算节点的池。

![在池中的节点][13]

选择池后，导航到计算节点上安装应用程序包。 在这里，应用程序包的详细信息都位于**应用程序**文件夹。 在计算节点上的其他文件夹包含其他文件，例如启动任务、 输出文件，错误输出，等等。
