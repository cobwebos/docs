...
* 将 `taskSlotsPerNode` 增加到 *2*，以便充分利用每个节点上的两个核心
...
输出显示 doAzureParallel 的“执行辅助角色”数。 此数是节点数乘以 `taskSlotsPerNode` 值。 如果已如前所述修改群集配置，则此数为 *10*。
...
![显示收盘价直方图的屏幕截图](media/tutorial-r-doazureparallel/closing-prices-local.png)
