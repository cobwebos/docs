...
通过使用容器，提供了一种简单的方式来运行批处理任务，而无需管理应用程序包和依赖项。 容器将应用程序部署为轻量级、可移植、自给自足的单元，可以在各种不同环境中运行。 例如，你可以在本地构建和测试容器，然后将容器映像上传到 Azure 或其他位置的注册表中。 容器部署模型可确保始终正确安装和配置应用程序的运行时环境，而不考虑在何处托管应用程序。 本教程介绍了如何使用 Batch .NET SDK 创建支持运行容器任务的计算节点池，以及如何在池上运行容器任务。
...
* 如果你提供自定义映像，则应用程序必须使用 Azure Active Directory (Azure AD) 身份验证才能运行基于容器的工作负载。 如果使用 Azure Marketplace 映像，则不需要 Azure AD 身份验证；共享密钥身份验证将起作用。 有关 Azure AD 的 Azure Batch 支持，请参阅[使用 Active Directory 对 Batch 服务解决方案进行身份验证](batch-aad-auth.md)。


## <a name="supported-virtual-machine-images"></a>受支持的虚拟机映像

你需要提供 Windows 或 Linux 映像才能创建 VM 计算节点池。
...
可以使用 [Docker 社区版 (CE)](https://www.docker.com/community-edition) 或 [Docker 企业版 (EE)](https://www.docker.com/enterprise-edition)。
...
若要访问 Azure RDMA 网络，请使用以下大小的 VM：A8、A9、H16r、H16mr 或 NC24r。 必需的 RDMA 驱动程序安装在 Azure Marketplace 中的 CentOS 7.3 HPC 和 Ubuntu 16.04 LTS 映像中。 可能还需要其他配置来运行 MPI 工作负载。 请参阅[在批处理池中使用支持 RDMA 或启用了 GPU 的实例](batch-pool-compute-intensive-sizes.md)。
...
此外，当你使用自定义 VM 映像时，需要向应用程序授予 IAM 访问控制，以访问 VM 映像。 在 Azure 门户中，打开“所有资源”，选择容器映像，并在映像边栏选项卡的“访问控制 (IAM)”部分，单击“添加”。 在“添加权限”边栏选项卡中指定“角色”，在“分配访问权限”中选择“Azure AD 用户、组或应用程序”，然后在“选择”中输入应用程序名称。

在应用程序中，使用 [BatchClient.Open](/dotnet/api/microsoft.azure.batch.batchclient.open#Microsoft_Azure_Batch_BatchClient_Open_Microsoft_Azure_Batch_Auth_BatchTokenCredentials_) 创建批处理客户端时，传递 Azure AD 身份验证令牌，如[使用 Active Directory 对批处理服务解决方案进行身份验证](batch-aad-auth.md)中所述。
...
批处理池是批处理执行作业任务时所在的计算节点集合。 在创建池时，与计算节点的 VM 配置一起提供。 [VirtualMachineConfiguration](/dotnet/api/microsoft.azure.batch.virtualmachineconfiguration) 对象包含对 [ContainerConfiguration](/dotnet/api/microsoft.azure.batch.containerconfiguration) 对象的引用。 若要在池上启用容器工作负载，请指定 `ContainerConfiguration` 设置。 此外，VM 配置也是在其中指定映像引用和映像节点代理 SKU ID 的位置，如以下示例所示。

对于池创建有多个选项。 可以创建一个带或不带预提取容器映像的池。 

可以通过拉取（或预提取）过程从 Docker 中心预加载容器映像，或在 Internet 上预加载另一个容器注册表。 预提取容器映像的优点是，当任务开始运行时，它们不必等待要下载的容器映像。 容器配置在创建池时将容器映像拉取到 VM。 然后，在池中运行的任务即可引用容器映像和容器运行选项列表。
...
若要在池上预提取容器映像，请将容器映像 (`containerImageNames`) 列表添加到 `ContainerConfiguration`，并指定映像列表名称。 下面的示例假定你使用自定义 Ubuntu 16.04 LTS 映像，从 [Docker 中心](https://hub.docker.com)预提取 TensorFlow 映像。 本示例包含一个在池节点的 VM 主机中运行的启动任务。 例如，你可以执行此操作来安装容器可访问的文件服务器。
...
当将任务设置为在计算节点上运行时，必须指定特定于容器的设置，如任务运行选项、要使用的映像和注册表。
