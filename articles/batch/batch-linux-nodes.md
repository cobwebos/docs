...
Batch 服务使用[虚拟机规模集](../virtual-machine-scale-sets/virtual-machine-scale-sets-overview.md)提供虚拟机配置中的计算节点。 可指定 [Azure 市场][vm_marketplace]中的一个映像，或者提供一个准备好的自定义映像。 有关自定义映像的更多详细信息，请参阅[使用共享映像库创建池](batch-sig-images.md)。
...
下面的代码片段示范如何使用[用于 Python 的 Microsoft Azure Batch 客户端库][py_batch_package]创建 Ubuntu Server 计算节点池。 Batch Python 模块的参考文档可以在读取文档的[azure.batch 包][py_batch_docs]中找到。

此代码片段显式创建 [ImageReference][py_imagereference]，并指定它的每个属性（publisher、offer、SKU、version）。 但是，在生产代码中，我们建议您使用[list_supported_images][py_list_supported_images]方法在运行时从可用映像和节点代理 SKU 组合中确定和选择。
...
如前所述，我们建议不要显式创建[ImageReference，][py_imagereference]而是使用[list_supported_images][py_list_supported_images]方法从当前支持的节点代理/商城映像组合中动态选择。 以下 Python 代码片段演示如何使用此方法。
...
以下代码段使用[池操作][net_pool_ops]。[List 支持图像][net_list_supported_images]方法，从当前支持的应用商店映像和节点代理 SKU 组合列表中选择。 这种做法非常有效，因为支持的组合列表可能随着时间改变。 通常情况下，添加支持的组合。
...
尽管前面的代码段使用[池操作][net_pool_ops]。[List 支持图像][net_list_supported_images]方法可动态列出和选择支持的图像和节点代理 SKU 组合（建议），还可以显式配置[ImageReference：][net_imagereference]
...
要获取 Batch 服务及其相应节点代理的所有受支持的应用商店虚拟机映像的列表，请使用您选择的相应语言 SDK 中的[list_supported_images][py_list_supported_images] （Python）、[列表支持图像][net_list_supported_images]（Batch .NET） 或相应的 API。
...
## <a name="pricing"></a>定价
Azure Batch 构建在 Azure 云服务和 Azure 虚拟机技术基础之上。 Batch 服务本身免费提供，这意味着您仅对 Batch 解决方案消耗的计算资源（以及所需的相关成本）收费。 如果选择“云服务配置”****，系统会根据[云服务定价][cloud_services_pricing]结构收费。 如果选择“虚拟机配置”****，系统会根据[虚拟机定价][vm_pricing]结构收费。
