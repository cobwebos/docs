...
<properties
	pageTitle="自动缩放 Azure 批处理池中的计算节点 | Microsoft Azure"
...
> [AZURE.NOTE]每个 Azure 批处理帐户限制为可用于处理的计算节点的最大数目。系统最多会创建限制数目的节点，因此不可能会达到公式指定的目标数。
...
> [AZURE.NOTE]自动缩放公式由 [Batch REST](https://msdn.microsoft.com/library/azure/dn820158.aspx) API 变量、类型、操作和函数组成。即使是在使用 [Batch .NET](https://msdn.microsoft.com/library/azure/mt348682.aspx) 库的时候，也会在公式字符串中使用这些组成元素。
...
在这个使用 [Batch .NET](https://msdn.microsoft.com/library/azure/mt348682.aspx) 库的代码段中，我们先对公式进行评估，然后将其应用到 [CloudPool](https://msdn.microsoft.com/library/azure/microsoft.azure.batch.cloudpool.aspx)。
