...
|元素名称|类型|说明|
|------------------|----------|-----------|
|`startTime`|日期/时间|任务开始运行的时间。 “Running”对应于**正在运行**状态，因此如果任务指定资源文件或应用程序包，则开始时间反映了任务开始下载或部署这些内容的时间。  如果任务已重启或重试，该时间是任务开始运行的最近时间。|
|`endTime`|日期/时间|任务完成的时间。|
