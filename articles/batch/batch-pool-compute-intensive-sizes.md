---
title: 将计算密集型 Azure VM 与批处理一起使用
description: 如何利用 Azure 批处理池中的 HPC 和 GPU 虚拟机大小。 了解操作系统依赖项，并查看几个方案示例。
documentationcenter: ''
author: LauraBrenner
manager: evansma
editor: ''
...
Batch 中计算密集型大小的 RDMA 或 GPU 功能仅在某些操作系统中支持。 （支持的操作系统列表是这些大小创建的虚拟机支持的操作系统的子集。根据创建 Batch 池的方式，您可能需要在节点上安装或配置其他驱动程序或其他软件。 下表总结了这些依存关系。 有关详细信息，请参阅链接的文章。 有关配置 Batch 池的选项，请参阅本文后面部分。
...
<sup>*</sup>支持 RDMA 的 N 系列尺寸还包括 NVIDIA 特斯拉 GPU
...
3. 按照以下步骤为批处理创建[共享图像库图像](batch-sig-images.md)。
4. 在支持 NC VM 的区域中创建 Batch 帐户。
5. 使用批处理 API 或 Azure 门户，[使用自定义映像](batch-sig-images.md)并使用所需的节点数和缩放创建池。 下表列出了映像的示例池设置：
...
1. 按照以下步骤为批处理创建[共享图像库图像](batch-sig-images.md)。
1. 使用批处理 API 或 Azure 门户，[使用共享映像库](batch-sig-images.md)创建池，并使用所需的节点数和缩放数创建池。 下表列出了映像的示例池设置：
...
## <a name="example-intel-mpi-on-a-linux-h16r-vm-pool"></a>示例：Linux H16r VM 池上的英特尔 MPI
