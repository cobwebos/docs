...
本文概述如何将 [Azure CLI 2.0 版](/cli/azure)与 Batch 配合使用。 请参阅 [Azure CLI 入门](/cli/azure/get-started-with-azure-cli)，大致了解如何将 CLI 与 Azure 配合使用。

## <a name="set-up-the-azure-cli"></a>设置 Azure CLI

可以在 [Azure Cloud Shell](../cloud-shell/overview.md) 中运行最新的 Azure CLI。 若要在本地安装 Azure CLI，请执行[安装 Azure CLI](/cli/azure/install-azure-cli) 中概述的步骤。
...
另请参阅 Azure CLI 参考文档，详细了解[适用于 Batch 的 Azure CLI 命令](/cli/azure/batch)。 
...
- **使用 Azure Active Directory (Azure AD) 身份验证** 
    ...
    Azure AD 的优势是提供基于角色的访问控制 (RBAC)。 使用 RBAC 时，用户的访问权限取决于分配给他们的角色，而不是是否拥有帐户密钥。 你可以管理 Azure 角色而不是帐户密钥，让 Azure AD 处理访问权限和身份验证。  
...
请参阅 [CLI 脚本示例](./scripts/batch-cli-sample-create-account.md)，了解用于完成常规任务的 Batch。 这些示例涵盖在用于 Batch 的 Azure CLI 中提供的许多命令，这些命令用于创建和管理帐户、池、作业和任务。
