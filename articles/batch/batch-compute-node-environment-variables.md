...
| AZ_BATCH_MASTER_NODE            | 在其上运行[多实例任务][multi_instance]的主要任务的计算节点的 IP 地址和端口。 请勿对 MPI 和 NCCL 通信使用此处指定的端口 - 它被保留用于 Azure Batch 服务。 改用变量 MASTER_PORT，方式是使用通过命令行参数传入的值设置（端口 6105 是适合的默认选项），或者使用 AML 设置的值（若已设置）。 | 多实例主要和子任务。 | `10.0.0.4:6000` |
...
| AZ_BATCH_NODE_LIST              | 分配给[多实例任务][multi_instance]的节点列表，采用 `nodeIP;nodeIP` 格式。 | 多实例主要和子任务。 | `10.0.0.4;10.0.0.5` |
...
| AZ_BATCH_TASK_SHARED_DIR | 对[多实例任务][multi_instance]的主要任务和每个子任务来说相同的目录路径。 该路径存在于运行多实例任务的每个节点上，在该节点上运行的任务命令（[协调命令][coord_cmd]和[应用程序命令][app_cmd]）对它具有读取/写入权限。 在其他节点上执行的子任务或主要任务不具有对此目录的远程访问权限（该目录不是“共享”的网络目录）。 | 多实例主要和子任务。 | C:\user\tasks\workitems\multiinstancesamplejob\job-1\multiinstancesampletask |
