...
批处理帐户是批处理服务中唯一标识的实体。所有处理都与一个 Batch 帐户相关联。当你使用 Batch 服务执行操作时，需要同时用到帐户名及其帐户密钥之一。可以[在 Azure 门户中创建和管理 Azure Batch 帐户](batch-account-create-portal.md)。
...
- **节点大小**
	...
	[Sizes for Cloud Services（云服务的大小）](../cloud-services/cloud-services-sizes-specs.md)中列出了**云服务配置**计算节点大小。Batch 支持 `ExtraSmall` 以外的所有云服务大小。
...
通过[自动缩放](batch-automatic-scaling.md)功能，你可以让 Batch 服务根据计算方案的当前工作负荷和资源使用状况动态缩放池中的计算节点数目。这样，便可做到只使用所需资源并可释放不需要的资源，因而能够降低运行应用程序的整体成本。
...
- 在 [Get started with the Azure Batch Library for .NET（适用于 .NET 的 Azure Batch 库入门）](batch-dotnet-get-started.md)中逐步演练一个示例 Batch 应用程序。另请参阅该教程的 [Python 版本](batch-python-tutorial.md)，其中介绍了如何在 Linux 计算节点上运行工作负荷。
