...
ms.locfileid: "74792095"
---
# <a name="install-on-premises-data-gateway-for-azure-logic-apps"></a>为 Azure 逻辑应用安装本地数据网关

在[从 Azure 逻辑应用连接到本地数据源](../logic-apps/logic-apps-gateway-connection.md)之前，请在本地计算机上下载并安装本地[数据网关](https://aka.ms/on-premises-data-gateway-installer)。 该网关充当桥，可在本地数据源和逻辑应用之间提供快速数据传输和加密。 可以将相同的网关安装与其他云服务配合使用，例如 Power BI、电源自动、电源应用和 Azure Analysis Services。 有关如何使用这些服务的网关的信息，请参阅以下文章：
...
* Azure 帐户和订阅。 如果没有包含订阅的 Azure 帐户，请[注册免费 azure 帐户](https://azure.microsoft.com/free/)。
  ...
  * 在网关安装过程中，你可以用 Azure 帐户登录，这会将网关安装链接到你的 Azure 帐户，并且仅将该帐户链接到该帐户。 稍后，在 Azure 门户中，你必须使用相同的 Azure 帐户，并在创建注册和声明你的网关安装的 Azure 网关资源时 Azure AD 租户。 在 Azure 逻辑应用中，本地触发器和操作使用网关资源连接到本地数据源。
...
* **相关注意事项**
  ...
  * 仅在本地计算机上安装本地数据网关，而不是域控制器。 不一定要在数据源所在的同一台计算机上安装网关。 对于所有数据源，只需一个网关，因此不需要为每个数据源安装网关。
  ...
  * 如果要将网关安装更新到最新版本，请先卸载最新的网关，以获得更清晰的体验。
...
1. 查看最终确认窗口中的信息。 此示例对逻辑应用、Power BI、电源应用和电源自动化使用同一帐户，因此该网关适用于所有这些服务。 准备就绪后，选择 "**关闭**"。
