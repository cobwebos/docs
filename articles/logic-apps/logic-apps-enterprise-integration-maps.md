...
若要在 Azure 逻辑应用中的企业集成方案的格式之间传输 XML 数据，逻辑应用可以使用映射，或更具体地说，即可扩展样式表语言转换（XSLT）映射。 映射是一个 XML 文档，描述如何将 XML 文档中的数据转换为另一种格式。 
...
* 如果映射引用了外部程序集，必须将该程序集和映射上传到集成帐户。 请确保[*先上传程序集*](#add-assembly)，然后上传引用该程序集的映射。
...
> [!NOTE]
> 如果更改您的程序集，则您还必须更新您的地图，而不管该地图是否已更改。
...
若要添加更大的程序集，可将程序集上传到 Azure 存储帐户中的 Azure Blob 容器。 根据你的 blob 容器是否具有公共读取访问权限，添加程序集的步骤会有所不同。 首先，检查你的 blob 容器是否具有公共读取访问权限，请执行以下步骤：[设置 blob 容器的公共访问级别](../vs-azure-tools-storage-explorer-blobs.md#set-the-public-access-level-for-a-blob-container)
...
1. 在 Blob 容器的快捷菜单中，选择“设置公共访问级别”。
   ...
   * 如果 blob 容器至少具有公共访问权限，请选择 "**取消**"，并按照以下步骤在此页上执行以下步骤：上[传到具有公共访问权限的容器](#public-access-assemblies)

     ![公共访问权限](media/logic-apps-enterprise-integration-schemas/azure-blob-container-public-access.png)

   * 如果 blob 容器没有公共访问权限，请选择 "**取消**"，并在此页后面的步骤中执行以下步骤：上[传到无公共访问权限的容器](#no-public-access-assemblies)
...
1. 在 "**地图类型**" 下，选择类型，例如： "**液体**"、" **xslt**"、" **xslt 2.0**" 或 " **xslt 3.0**"。
