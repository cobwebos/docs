...
1. 在搜索框中输入“Azure Functions”作为筛选器。 从 "操作" 列表中，选择此操作：**选择 Azure 函数**
...
1. 在函数定义编辑器中定义函数：
   ...
   1. 在 "**代码**" 框中，将你的代码添加到函数模板，包括你希望在函数完成运行后返回给逻辑应用的响应和有效负载。
...
1. 在 "**选择操作**" 下的 "搜索" 框中，输入 "azure 函数" 作为筛选器。 从 "操作" 列表中，选择此操作：**选择 Azure 函数**
