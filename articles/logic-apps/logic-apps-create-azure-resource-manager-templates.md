...
* [用 Visual Studio 创建逻辑应用](../logic-apps/quickstart-create-logic-apps-with-visual-studio.md)
...
使用此工具运行`Get-LogicAppTemplate`命令时, 命令首先通过 ARMClient 工具获取访问令牌, 通过管道将令牌传递给 PowerShell 脚本, 并将该模板创建为 JSON 文件。 有关该工具的详细信息, 请参阅[有关 Azure 资源管理器客户端工具的文章](https://blog.davidebbo.com/2015/01/azure-resource-manager-client.html)。
...
若要根据[azure 资源管理器客户端工具](https://github.com/projectkudu/ARMClient)中的令牌进行管道的建议, 请运行此命令, `$SubscriptionId`而不是 azure 订阅 ID:
...
| KeyVault | 否 | 描述如何处理可能的 key vault 值的枚举。 默认值为 `None`。 |
||||
