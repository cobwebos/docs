---
title: 在可视化工作室中构建第一个无服务器应用程序
description: 在可视化工作室中使用 Azure 逻辑应用和 Azure 功能构建、部署和管理无服务器应用
services: logic-apps
...
通过使用 Azure 中的无服务器工具和功能（如[Azure 逻辑应用](../logic-apps/logic-apps-overview.md)和[Azure 函数](../azure-functions/functions-overview.md)），可以快速开发和部署云应用。 本文介绍如何在 Visual Studio 中开始生成一个使用逻辑应用来调用 Azure 函数的无服务器应用。 若要详细了解 Azure 中的无服务器解决方案，请参阅[将 Azure Serverless 与 Functions 和逻辑应用配合使用](../logic-apps/logic-apps-serverless-overview.md)。
...
* 以下工具。 如果还没有这些工具，请下载并安装它们。
  ...
  * [Azure 函数核心工具](https://www.npmjs.com/package/azure-functions-core-tools)，用于本地调试功能。
...
若要开始，请为无服务器应用创建一个 [Azure 资源组项目](../azure-resource-manager/templates/create-visual-studio-deployment-project.md)。 在 Azure 中，在*资源组*中创建资源 ，这是用于组织、管理和部署整个应用的资源作为单个资产的逻辑集合。 对于 Azure 中的无服务器应用，你的资源组包含 Azure 逻辑应用和 Azure Functions 的资源。 详细了解 [Azure 资源组和资源](../azure-resource-manager/management/overview.md)。
...
1. 从 **"显示此位置列表中的模板**"中，选择**Azure 快速入门 （github.com/Azure/azure-quickstart-templates）。**
...
1. 在解决方案资源管理器中，从资源项目的快捷菜单中，选择 **"部署** > **新建**"。
