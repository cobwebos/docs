---
title: 创建和管理 B2B 解决方案的集成帐户-Azure 逻辑应用
description: 使用 Azure 逻辑应用创建、链接、移动和删除企业集成和 B2B 解决方案的集成帐户
services: logic-apps
...
ms.locfileid: "68607012"
---
# <a name="create-and-manage-integration-accounts-for-b2b-solutions-by-using-azure-logic-apps"></a>使用 Azure 逻辑应用创建和管理 B2B 解决方案的集成帐户

在可以使用[Azure 逻辑应用](../logic-apps/logic-apps-overview.md)构建[企业集成和 B2B 解决方案](../logic-apps/logic-apps-enterprise-integration-overview.md)之前, 需要创建一个集成帐户, 该帐户是一个单独的 Azure 资源, 可为你定义并用于逻辑应用工作流的集成项目。
...
1. 提供有关集成帐户的下列信息:
   ...
   | 属性 | 必填 | Value | 说明 |
   |----------|----------|-------|-------------|
   | **名称** | 是 | <integration-account-name> | 集成帐户的名称, 例如 "Fabrikam-集成" |
   ...
   | **Location** | 是 | <*Azure 区域*> | 用于存储集成帐户元数据的区域。 选择与逻辑应用相同的位置, 或在与集成帐户相同的位置创建逻辑应用。 对于本示例, 请使用 "美国西部"。 <p>**注意**：若要在[integration service 环境 (ISE)](../logic-apps/connect-virtual-network-vnet-isolated-environment-overview.md)中创建集成帐户, 请选择该 ISE 作为位置。 有关详细信息, 请参阅[在 ISE 中创建集成帐户](../logic-apps/add-artifacts-integration-service-environment-ise.md#create-integration-account-environment)。 |
...
若要使逻辑应用能够访问包含 B2B 项目的集成帐户, 必须先将集成帐户链接到逻辑应用。 逻辑应用和集成帐户必须存在于同一区域中。 对于此任务, 您可以按照本部分中的步骤使用 Azure 门户。
