...
若要提高调用 API 的安全性，可通过 Azure 门户设置 Azure Active Directory (Azure AD) 身份验证，这样便不需要更新代码。 或者，还可利用 API 代码要求并强制执行身份验证。

## <a name="authentication-options-for-your-api"></a>适用于 API 的身份验证选项

可通过以下方式提高调用自定义 API 的安全性：
