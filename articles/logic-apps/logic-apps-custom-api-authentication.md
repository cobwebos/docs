...
description: 如何设置身份验证以提高从 Azure 逻辑应用调用自定义 API 的安全性
services: logic-apps
...
为了提高对 API 调用的安全性，可以通过 Azure 门户设置 Azure 活动目录 （Azure AD） 身份验证，这样您就不必更新代码。 或者，还可利用 API 代码要求并强制执行身份验证。

## <a name="authentication-options-for-your-api"></a>适用于 API 的身份验证选项

您可以通过以下方式提高对自定义 API 调用的安全性：
...
2. 确认所在目录与 Web 应用或 API 应用相同。
   ...
   > [!TIP]
   > 要切换目录，请选择配置文件，然后选择其他目录。 或者，选择 **"概述** > **切换"目录**。
