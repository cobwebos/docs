...
| 参数 | 必须 | 类型 | 说明 |
| --------- | -------- | ---- | ----------- |
| <*expression1*>, <*expression2*>, ... | 是 | 布尔值 | 要检查的表达式 |
|||||
...
该示例还使用[parameters （）](#parameters)函数获取中的 xml 字符串 `'items'` ，并使用[xml （）](#xml)函数将字符串转换为 xml 格式。
