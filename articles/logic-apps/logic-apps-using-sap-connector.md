...
* 如果将 SNC 与 SSO 一起使用, 请确保网关正在以与 SAP 用户对应的用户身份运行。 若要更改默认帐户，请选择“更改帐户”并输入用户凭据。
...
1. 现在, 保存逻辑应用, 以便可以为逻辑应用生成终结点 URL。 在设计器工具栏上选择“保存”。
...
1. 如果系统提示输入连接详细信息，请立即创建 SAP 连接。 否则, 如果连接已存在, 请继续执行下一步, 以便可以设置 SAP 操作。
   ...
   **创建本地 SAP 连接**
   ...
   提供 SAP 服务器的连接信息。 对于“数据网关”属性，请选择在 Azure 门户中为网关安装创建的数据网关。 完成后, 选择 "**创建**"。 逻辑应用设置并测试连接, 以确保连接正常运行。
...
1. 在搜索框中，输入“响应”作为筛选器。 从 "**操作**" 列表中选择 "**响应**"。
...
1. 如果系统提示输入连接详细信息，请立即创建 SAP 连接。 如果连接已存在, 请继续执行下一步, 以便可以设置 SAP 操作。
...
## <a name="receive-idocs-packets-from-sap"></a>接收来自 SAP 的 Idoc 数据包
...
* 提供附加安全功能的 SNC 库与数据网关安装在同一台计算机上。 例如, [sapseculib](https://help.sap.com/saphelp_nw74/helpdata/en/7a/0755dc6ef84f76890a77ad6eb13b13/frameset.htm)、KERBEROS 和 NTLM。
   ...
   | 属性 | 描述 |
   |----------| ------------|
   | **SNC 库路径** | SNC 库的名称、相对于 NCo 安装位置的路径或绝对路径。 `sapsnc.dll`例如或`.\security\sapsnc.dll`或。 `c:\security\sapsnc.dll` |
   | **SNC SSO** | 通过 SNC 连接时, SNC 标识通常用于对调用方进行身份验证。 另一个选项是重写，以便可以使用用户和密码信息对调用方进行身份验证，但该行仍会加密。 |
   | **我的 SNC 名称** | 在大多数情况下可以省略此属性。 安装的 SNC 解决方案通常知道自己的 SNC 名称。 仅对于支持多个标识的解决方案, 你可能需要指定要用于此特定目标或服务器的标识。 |
   | **SNC 合作伙伴名称** | 后端 SNC 的名称。 |
   | **SNC 保护质量** | 用于 SNC 此特定目标或服务器通信的服务质量。 默认值由后端系统定义。 最大值由 SNC 使用的安全产品定义。 |
   |||

   > [!NOTE]
   > 不要在具有数据网关和 SNC 库的计算机上设置环境变量 SNC_LIB 和 SNC_LIB_64。 如果设置, 则它们优先于通过连接器传递的 SNC 库值。

<a name="safe-typing"></a>

## <a name="safe-typing"></a>安全类型化

创建 SAP 连接时，默认会使用强类型化通过针对架构执行 XML 验证来检查无效值。 此行为可帮助提前检测问题。 “安全类型化”选项用于实现后向兼容，它只会检查字符串长度。 如果选择 "**安全键入**", 则将 SAP 中的 dat 类型和 TIMS 类型视为字符串, 而不是作为其`xs:date` XML `xs:time`等效项`xmlns:xs="http://www.w3.org/2001/XMLSchema"`, 并在其中进行处理。 安全类型将影响所有架构生成的行为、"已发送" 负载和 "已收到" 响应的发送消息以及触发器。 
