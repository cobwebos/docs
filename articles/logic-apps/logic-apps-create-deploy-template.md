...
可以使用任意工具来部署模板，比如 PowerShell、REST API、[Azure DevOps Release Management](#team-services) 和 Azure 门户模板部署。
