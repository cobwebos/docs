...
|value|是|指定输出的值。|  
|type|是|指定已指定的值的类型。 可能的值类型为： <p>- `string`<br />- `securestring`<br />- `int`<br />- `bool`<br />- `array`<br />- `object`|
...
|actions|使表达式能够从运行时操作的其他 JSON 名称值对或输出中派生其值。 这些表达式显式声明某个操作依赖于另一个操作。 以下示例中 propertyPath 表示的属性是可选的。 如果未指定 propertyPath，则引用整个操作对象。 可以使用此元素或 conditions 元素来指定依赖关系，但不需要同时使用它们用于同一依赖资源。 <p>`actions('myAction').outputs.body.propertyPath` <p> **参数数目**：1 <p> **名称**：操作名称 <p> **说明**：必需。 需要获取其值的操作的名称。|  
|trigger|使表达式能够从运行时触发器的其他 JSON 名称值对或输出中派生其值。 以下示例中 propertyPath 表示的属性是可选的。 如果未指定 propertyPath，则引用整个触发器对象。 <p>`trigger().outputs.body.propertyPath` <p>在触发器的输入中使用时，该函数返回上一次执行的输出。 但是，在触发器的条件中使用时，`trigger` 函数将返回当前执行的输出。|  
...
|交集|返回在传入的数组与对象之间具有共同元素的单个数组或对象。 例如，以下函数返回 `[1, 2]`： <p>`intersection([1, 2, 3], [101, 2, 1, 10],[6, 8, 1, 2])` <p>该函数的参数可以是对象集或数组集（但不能是两者的混合）。 如果存在两个同名的对象，最终的对象中将显示使用该名称的最后一个对象。 <p> **参数数目**：1 ... *n* <p> **名称**：Collection *n* <p> **说明**：必需。 要计算的集合。 要在结果中显示某个对象，该对象必须在传入的所有集合中。|  
|union|返回其所有元素出现在传递给此函数的数组或对象中的单个数组或对象。 例如，以下函数返回 `[1, 2, 3, 10, 101]`： <p>`union([1, 2, 3], [101, 2, 1, 10])` <p>该函数的参数可以是对象集或数组集（但不能是两者的混合）。 如果最终的输出中存在两个同名的对象，最终的对象中将显示使用该名称的最后一个对象。 <p> **参数数目**：1 ... *n* <p> **名称**：Collection *n* <p> **说明**：必需。 要计算的集合。 显示在任意集合中的对象也会显示在结果中。|  
...
|startswith|检查字符串是否以某个值开头（不区分大小写）。 例如，以下函数返回 `true`： <p>`lastindexof('hello, world', 'hello')` <p> **参数数目**：1 <p> **名称**：字符串 <p> **说明**：必需。 可能包含该值的字符串。 <p> **参数数目**：2 <p> **名称**：字符串 <p> **说明**：必需。 可能为字符串开头的值。|  
|endswith|检查字符串是否以某个值结尾（不区分大小写）。 例如，以下函数返回 `true`： <p>`lastindexof('hello, world', 'world')` <p> **参数数目**：1 <p> **名称**：字符串 <p> **说明**：必需。 可能包含该值的字符串。 <p> **参数数目**：2 <p> **名称**：字符串 <p> **说明**：必需。 可能为字符串结尾的值。|  
...
|triggerFormDataValue|返回与表单数据或表单编码触发器输出中的键名匹配的单个值。  如果有多个匹配项，将会出错。  例如，以下函数返回 `bar`：`triggerFormDataValue('foo')`<br /><br />**参数数目**：1<br /><br />**名称**：键名<br /><br />**说明**：必需。 要返回的表单数据值的键名。|
...
|formDataValue|返回与表单数据或表单编码操作输出中的键名匹配的单个值。  如果有多个匹配项，将会出错。  例如，以下函数返回 `bar`：`formDataValue('someAction', 'foo')`<br /><br />**参数数目**：1<br /><br />**名称**：操作名称<br /><br />**说明**：必需。 包含表单数据或表单编码响应的操作的名称。<br /><br />**参数数目**：2<br /><br />**名称**：键名<br /><br />**说明**：必需。 要返回的表单数据值的键名。|
...
|min|可通过两种不同的模式来调用此函数。 <p>如果 `min` 采用数组，该函数将返回 `0`： <p>`min([0,1,2])` <p>或者，此函数可以采用值的逗号分隔列表，此时也返回 `0`： <p>`min(0,1,2)` <p> **注意**：所有值必须为数字，因此，如果参数为数组，该数组必须仅包含数字。 <p> **参数数目**：1 <p> **名称**：集合或值 <p> **说明**：必需。 要在其中查找最小值的值数组，或者集的第一个值。 <p> **参数数目**：2 ... *n* <p> **名称**：Value *n* <p> **说明**：可选 如果第一个参数是一个值，则你可以传递其他值，这样就会返回所有传递值的最小值。|  
|max|可通过两种不同的模式来调用此函数。 <p>如果 `max` 采用数组，该函数将返回 `2`： <p>`max([0,1,2])` <p>或者，此函数可以采用值的逗号分隔列表，此时也返回 `2`： <p>`max(0,1,2)` <p> **注意**：所有值必须为数字，因此，如果参数为数组，该数组必须仅包含数字。 <p> **参数数目**：1 <p> **名称**：集合或值 <p> **说明**：必需。 要在其中查找最大值的值数组，或者集的第一个值。 <p> **参数数目**：2 ... *n* <p> **名称**：Value *n* <p> **说明**：可选 如果第一个参数是一个值，则你可以传递其他值，这样就会返回所有传递值的最大值。|  
...
|rand|生成指定范围内的随机整数（包括两个边界）。 例如，以下函数可能返回 `42`： <p>`rand(-1000,1000)` <p> **参数数目**：1 <p> **名称**：最小值 <p> **说明**：必需。 可返回的最小整数。 <p> **参数数目**：2 <p> **名称**：最大值 <p> **说明**：必需。 此值为可返回的最大整数。|  
...
|工作流|此函数提供工作流本身在运行时的所有详细信息。 可以获取管理 API 中提供的任何信息，例如名称、位置和 resourceID。 有关这些属性的详细信息，请参阅 [Rest API](http://go.microsoft.com/fwlink/p/?LinkID=525617)。 例如，以下函数返回类似于 `westus` 的位置： <p>`workflow().location` <p> **注意**：目前仅支持在触发器中使用 `@workflow`。|
