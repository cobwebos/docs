...
每个 URL 都包含 `sp`、`sv` 和 @no__t 查询参数，如下表中所述：
...
在正文中，使用 JSON 日期字符串包含 `NotAfter`property。 该属性返回仅在 `NotAfter` 日期和时间之前有效的回调 URL。
...
> [!NOTE]
> 无论 IP 地址如何，你仍可以通过使用 `/triggers/<trigger-name>/run` 通过 Azure REST API 或通过 API 管理，来运行具有基于 HTTP 请求的触发器的逻辑应用。 但是，此方案仍需要针对 Azure REST API 进行身份验证。 所有事件都显示在 Azure 审核日志中。 请确保相应地设置访问控制策略。

#### <a name="restrict-incoming-ip-ranges-in-azure-resource-manager-template"></a>限制 Azure 资源管理器模板中的传入 IP 范围

如果使用[Azure 资源管理器模板](../logic-apps/logic-apps-create-deploy-template.md)自动执行逻辑应用部署，则可以通过 @no__t 使用逻辑应用资源定义中的 `triggers` 部分来指定 IP 范围，例如：
...
访问逻辑应用的运行历史记录时，逻辑应用会对访问进行身份验证，并提供指向逻辑应用运行中的请求和响应的输入和输出的链接。 但是，对于处理任何密码、机密、密钥或其他敏感信息的操作，你希望阻止其他人查看和访问该数据。 例如，如果逻辑应用从[Azure Key Vault](../key-vault/key-vault-overview.md)获取要在对 HTTP 操作进行身份验证时使用的机密，则需要将该机密从视图中隐藏。
...
如果使用[Azure 资源管理器模板](../logic-apps/logic-apps-create-deploy-template.md)自动执行逻辑应用部署，则可以通过 @no__t 使用逻辑应用资源定义中的 `contents` 部分来指定 IP 范围，例如：
...
在基础触发器或操作定义中，添加或更新 `runtimeConfiguration.secureData.properties` 数组，其中的一个或两个值均为：
...
例如，如果使用[Azure Active Directory](../logic-apps/logic-apps-workflow-actions-triggers.md#connector-authentication)对 HTTP 操作进行身份验证，则可以定义接受用于身份验证的客户端 ID 和客户端密钥的参数并确保它们的安全。 若要为逻辑应用定义这些参数，请在逻辑应用的工作流定义中使用 `parameters` 部分。 若要保护在编辑逻辑应用或查看运行历史记录时不希望显示的参数值，请使用 @no__t 0 或 @no__t 类型定义参数，并在必要时使用编码。 具有此类型的参数不会随资源定义一起返回，且在部署后查看资源时不可访问。 若要在运行时访问这些参数值，请使用工作流定义中的 @no__t 0 表达式。 此表达式仅在运行时进行计算，由[工作流定义语言](../logic-apps/logic-apps-workflow-definition-language.md)进行描述。
...
使用[Azure 资源管理器模板](../azure-resource-manager/resource-group-authoring-templates.md#parameters)自动执行部署时，可以通过使用 @no__t 和 @no__t 2 类型定义在部署时评估的安全模板参数。 若要定义模板参数，请使用模板的顶级 `parameters` 部分，该部分是独立的，不同于工作流定义的 @no__t 部分。 若要提供模板参数的值，请使用单独的[参数文件](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values)。
...
若要在逻辑应用的工作流定义中保护敏感信息，请使用受保护的参数，以便在保存逻辑应用后，此信息不可见。 例如，假设有一个 HTTP 操作需要使用用户名和密码的基本身份验证。 在工作流定义中，`parameters` 部分使用 @no__t 类型定义 @no__t 和 @no__t 参数。 然后，操作定义在 `authentication` 部分中引用这些参数。
...
逻辑应用的资源管理器模板包含多个 @no__t 0 部分。 若要保护密码、密钥、机密和其他敏感信息，请使用 @no__t 0 或 @no__t 类型在模板级别和工作流定义级别定义安全参数。 然后，你可以将这些值存储在[Azure Key Vault](../key-vault/key-vault-overview.md)中，并使用[参数文件](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values)来引用密钥保管库和机密。 然后，模板会在部署时检索该信息。 有关详细信息，请参阅[在部署时使用 Azure Key Vault 传递安全参数值](../azure-resource-manager/resource-manager-keyvault-parameter.md)。

下面是有关这些 `parameters` 部分的详细信息：

* 在模板的顶层，@no__t 0 部分定义模板在*部署*时使用的值的参数。 例如，这些值可能包含特定部署环境的连接字符串。 然后，可以将这些值存储在单独的[参数文件](../azure-resource-manager/resource-group-template-deploy.md#pass-parameter-values)中，这会使更改这些值变得更容易。

* 在逻辑应用的资源定义中，但在工作流定义外，@no__t 0 部分指定工作流定义的参数的值。 在本部分中，可以使用引用模板参数的模板表达式来分配这些值。 这些表达式是在部署时计算的。

* 在工作流定义中，@no__t 0 部分定义逻辑应用在运行时使用的参数。 然后，可以使用在运行时计算的工作流定义表达式来引用逻辑应用工作流中的这些参数。

此示例模板具有使用 @no__t 类型的多个安全参数定义：
