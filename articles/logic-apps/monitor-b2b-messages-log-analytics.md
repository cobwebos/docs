---
title: 使用 Azure 监视器监视 B2B 消息
description: 通过设置 Azure 监视器日志和收集 Azure 逻辑应用的诊断数据，对 AS2、X12 和 EDIFACT 消息进行故障排除
services: logic-apps
...
---
# <a name="set-up-azure-monitor-logs-and-collect-diagnostics-data-for-b2b-messages-in-azure-logic-apps"></a>在 Azure 逻辑应用中设置 Azure 监视器日志并收集 B2B 消息的诊断数据

在集成帐户中设置贸易伙伴之间的 B2B 通信后，这些合作伙伴可以使用 AS2、X12 和 EDIFACT 等协议交换消息。 要检查此通信是否可以按预期方式工作，可以为集成帐户设置[Azure 监视器日志](../azure-monitor/platform/data-platform-logs.md)。 [Azure 监视器](../azure-monitor/overview.md)可帮助您监视云和本地环境，以便更轻松地维护其可用性和性能。 通过使用 Azure 监视器日志，可以在[日志分析工作区](../azure-monitor/platform/resource-logs-collect-workspace.md)中记录和存储有关运行时数据和事件的数据，例如触发器事件、运行事件和操作事件。 对于消息，日志记录还会收集信息，例如：
...
Azure 监视器允许您创建[日志查询](../azure-monitor/log-query/log-query-overview.md)，以帮助您查找和查看此信息。 还可以[将此诊断数据与其他 Azure 服务](../logic-apps/monitor-logic-apps-log-analytics.md#extend-data)（如 Azure 存储和 Azure 事件中心）一起使用。

要为集成帐户设置日志记录，请在 Azure 门户中[安装逻辑应用 B2B 解决方案](#install-b2b-solution)。 此解决方案提供 B2B 消息事件的聚合信息。 然后，要启用日志记录和创建此信息的查询，应设置[Azure 监视器日志](#set-up-resource-logs)。

本文演示如何为集成帐户启用 Azure 监视器日志记录。
...
* 关联到逻辑应用的集成帐户。 [了解如何将集成帐户链接到逻辑应用](../logic-apps/logic-apps-enterprise-integration-create-integration-account.md)。

<a name="install-b2b-solution"></a>

## <a name="install-logic-apps-b2b-solution"></a>安装逻辑应用 B2B 解决方案

在 Azure 监视器日志可以跟踪逻辑应用的 B2B 消息之前，请将**逻辑应用 B2B**解决方案添加到日志分析工作区。

1. 在[Azure 门户](https://portal.azure.com)的搜索框中，输入`log analytics workspaces`，然后选择**日志分析工作区**。

   ![选择"日志分析工作区"](./media/monitor-b2b-messages-log-analytics/find-select-log-analytics-workspaces.png)
...
1. 在"概述"窗格中，**在"开始使用日志分析** > **配置监视解决方案**"下，选择 **"查看解决方案**"。
   ...
   ![在"概述"窗格中，选择"查看解决方案"](./media/monitor-b2b-messages-log-analytics/log-analytics-workspace.png)

1. 在"概述"窗格中，选择 **"添加**"。

   ![在概览窗格上，添加新解决方案](./media/monitor-b2b-messages-log-analytics/add-logic-apps-management-solution.png)

1. **市场**打开后，在搜索框中输入`logic apps b2b`，然后选择**逻辑应用 B2B**。

   ![从应用商店中，选择"逻辑应用管理"](./media/monitor-b2b-messages-log-analytics/select-logic-apps-b2b-solution.png)
...
1. 查看并确认要安装解决方案的日志分析工作区，然后选择"再次**创建**"。
   ...
   Azure 将解决方案部署到包含日志分析工作区的 Azure 资源组后，该解决方案将显示在工作区的摘要窗格中。 处理 B2B 消息时，将更新此窗格上的消息计数。
...
1. 在集成帐户的菜单上，在 **"监视"** 下，选择 **"诊断设置**"。 选择“添加诊断设置”。****
...
1. 要创建设置，请按照以下步骤操作：
   ...
   1. 在**日志**下，选择 **"集成帐户跟踪事件**"类别，该类别指定要记录的事件类别。

   1. 完成后，选择“保存”****。

   例如： 

   ![设置 Azure 监视器日志以收集诊断数据](./media/monitor-b2b-messages-log-analytics/send-diagnostics-data-log-analytics-workspace.png)

<a name="view-message-status"></a>

## <a name="view-message-status"></a>查看消息状态

运行逻辑应用后，您可以在日志分析工作区中查看有关这些消息的状态和数据。

1. 在[Azure 门户](https://portal.azure.com)搜索框中，查找并打开日志分析工作区。

1. 在工作区的菜单上，选择**工作区摘要** > **逻辑应用 B2B**。

   ![工作区摘要窗格](./media/monitor-b2b-messages-log-analytics/b2b-overview-messages-summary.png)

   > [!NOTE]
   > 如果逻辑应用 B2B 磁贴在运行后没有立即显示结果，请尝试选择 **"刷新"** 或等待一小段时间，然后再重试。

   默认情况下，“逻辑应用 B2B”磁贴显示一天的数据****。 要将数据范围更改为不同的间隔，请选择页面顶部的范围控件：
...
1. 当消息状态仪表板显示后，可以查看特定消息类型的更多详细信息（显示的也是一天数据）。 选择**AS2** **、X12**或**EDIFACT**的磁贴。
   ...
   ![查看邮件的状态](./media/monitor-b2b-messages-log-analytics/workspace-summary-b2b-messages.png)
...
| properties | 描述 |
|----------|-------------|
| **发送** | “接收设置”**** 中指定的来宾合作伙伴，或 AS2 协议的“发送设置”**** 中指定的托管合作伙伴 |
| **接收机** | “接收设置”**** 中指定的托管合作伙伴，或 AS2 协议的“发送设置”**** 中指定的来宾合作伙伴 |
...
| properties | 描述 |
|----------|-------------|
| **发送** | “接收设置”**** 中指定的来宾合作伙伴，或 X12 协议的“发送设置”**** 中指定的托管合作伙伴 |
| **接收机** | “接收设置”**** 中指定的托管合作伙伴，或 X12 协议的“发送设置”**** 中指定的来宾合作伙伴 |
...
| properties | 描述 |
|----------|-------------|
| **发送** | “接收设置”**** 中指定的来宾合作伙伴，或 EDIFACT 协议的“发送设置”**** 中指定的托管合作伙伴 |
| **接收机** | “接收设置”**** 中指定的托管合作伙伴，或 EDIFACT 协议的“发送设置”**** 中指定的来宾合作伙伴 |
