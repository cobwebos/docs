...
description: 了解如何使用使用 Azure 逻辑应用创建的自动化任务和工作流中的变量来存储、管理、使用和传递值
services: logic-apps
...
变量只在创建它们的逻辑应用实例中存在并保持全局性。 另外，它们保留在逻辑应用实例中的任何循环迭代之间。 引用变量时，请使用变量的名称作为标记，而不是操作的名称，这是引用操作输出的常用方法。

> [!IMPORTANT]
> 默认情况下，在"每个"循环中循环并行运行。 在循环中使用变量时，请[按顺序](../logic-apps/logic-apps-control-flow-loops.md#sequential-foreach-loop)运行循环，以便变量返回可预测的结果。

## <a name="prerequisites"></a>先决条件

* Azure 订阅。 如果没有订阅，[请注册免费的 Azure 帐户](https://azure.microsoft.com/free/)。
...
1. 在[Azure 门户](https://portal.azure.com)或可视化工作室中，在逻辑应用设计器中打开逻辑应用。
...
1. 提供有关变量的此信息，如下所述：
   ...
   | **名称** | 是 | <*变量名称*> | 要递增的变量的名称 |
   | **类型** | 是 | <*变量类型*> | 变量的数据类型 |
   | **价值** | 否 | <*起始值*> | 变量的初始值 <p><p>**提示**：尽管此值是可选的，但最好是设置此值，以便始终知道变量的起始值。 |
   |||||
...
如果从设计器切换到代码视图编辑器，下面是**初始化变量**操作在逻辑应用定义中显示的方式，该定义采用 JavaScript 对象表示法 （JSON） 格式：
...
例如，此表达式使用`variables()`函数从[本文中以前创建的](#append-value)数组变量中获取项。 函数`string()`以字符串格式返回变量的内容：`"1, 2, 3, red"`
...
要按常量值增加或*递增*变量，请向逻辑应用添加**增量变量**操作。 此操作仅适用于整数和浮点数变量。
...
1. 提供用于递增变量的以下信息：
   ...
   |----------|----------|-------|--------------|
   | **名称** | 是 | <*变量名称*> | 要递增的变量的名称 |
   | **价值** | 否 | <*增量值*> | 用于递增变量的值。 默认值为 1。 <p><p>**提示**：尽管此值是可选的，但最好是设置此值，以便始终知道用于递增变量的特定值。 |
   ||||
...
如果从设计器切换到代码视图编辑器，以下是**增量变量**操作在逻辑应用定义中显示的方式，该定义采用 JSON 格式：
...
1. 在触发器中，要检查附件并将这些附件传递到逻辑应用的工作流中，请为以下属性选择 **"是**"：
...
1. 添加[“初始化变量”操作](#create-variable)。**** 创建名为具有`Count`零起始值的整数变量。
...
1. 在该循环中，单击“从先前的步骤中选择一个输出”框。**** 当动态内容列表出现时，选择“附件”。****
   ...
   **附件**属性将包含来自触发器输出的电子邮件附件的数组传递到循环中。

1. 在 **"对于每个循环"** 中，选择 **"添加操作**"。
...
如果从设计器切换到代码视图编辑器，下面是**For 每个**循环与逻辑应用定义中的**增量变量**操作一起显示的方式，该操作采用 JSON 格式。
...
要按常量值减小或*递减*变量，请按照[增加变量](#increment-value)的步骤操作，但查找并选择 **"递减"变量**操作。 此操作仅适用于整数和浮点数变量。
...
|----------|----------|-------|--------------|
| **名称** | 是 | <*变量名称*> | 要递减的变量的名称 | 
| **价值** | 否 | <*增量值*> | 用于递减变量的值。 默认值为 1。 <p><p>**提示**：尽管此值是可选的，但最好是设置此值，以便始终知道用于递减变量的特定值。 |
||||| 

如果从设计器切换到代码视图编辑器，下面是 **"递减"变量**操作在逻辑应用定义（以 JSON 格式）中显示的方式。
...
如果从设计器切换到代码视图编辑器，下面是**Set 变量**操作显示在逻辑应用定义（以 JSON 格式）中的方式。 此示例将`Count`变量的当前值更改为另一个值。
...
|----------|----------|-------|--------------|
| **名称** | 是 | <*变量名称*> | 要更改的变量的名称 |
| **价值** | 是 | <*追加值*> | 要追加的值，可以是任何类型 |
|||||

如果从设计器切换到代码视图编辑器，下面是 **"追加到数组变量操作"** 显示在逻辑应用定义（以 JSON 格式）中的方式。 本示例创建一个数组变量，并将另一个值添加为数组中的最后一个项。 结果是包含此数组的已更新变量：`[1,2,3,"red"]`
