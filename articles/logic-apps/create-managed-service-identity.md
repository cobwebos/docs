...
1. 在逻辑应用菜单的“设置”下，选择“标识”********。 选择 > **"** > **保存**时**分配系统**"。 当 Azure 提示你确认时，请选择 **"是"**。
   ...
   | properties | 值 | 说明 |
   |----------|-------|-------------|
   | **对象 ID** | <*标识-资源 ID*> | 全局唯一标识符（GUID），表示 Azure AD 租户中逻辑应用的系统分配的标识 |
   ||||
...
若要自动创建和部署 Azure 资源（如逻辑应用），可以使用[azure 资源管理器模板](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md)。 若要在模板中为逻辑应用启用系统分配的托管标识，请将`identity`对象和`type`子属性添加到模板中逻辑应用的资源定义，例如：
...
当 Azure 创建逻辑应用资源定义时，对象`identity`将获取以下附加属性：
...
| 属性 (JSON) | 值 | 说明 |
|-----------------|-------|-------------|
| `principalId` | <*主体 ID*> | 托管标识的服务主体对象的全局唯一标识符（GUID），表示 Azure AD 租户中的逻辑应用。 此 GUID 有时显示为 "对象 ID" 或`objectID`。 |
| `tenantId` | <*Azure-AD-租户-ID*> | 全局唯一标识符（GUID），表示逻辑应用现在是其成员的 Azure AD 租户。 在 Azure AD 租户内，服务主体与逻辑应用实例具有相同名称。 |
||||
...
1. 在[Azure 门户](https://portal.azure.com)的任意页面上的 "搜索" 框中， `managed identities`输入，然后选择 "**托管标识**"。
...
1. 提供有关托管标识的信息，然后选择 "**创建**"，例如：
   ...
   | **订阅** | 是 | <*Azure-订阅-名称*> | 要使用的 Azure 订阅的名称 |
   | **资源组** | 是 | <*Azure-资源组-名称*> | 要使用的资源组的名称。 创建新组，或选择现有组。 此示例创建名为 "fabrikam-托管-标识-RG" 的新组。 |
   | **位置** | 是 | <*Azure 区域*> | 用于存储有关资源的信息的 Azure 区域。 此示例使用“美国西部”。 |
   |||||
...
1. 在逻辑应用菜单上的 "**设置**" 下，选择 "**标识**"，然后选择 "**用户分配** > 的**添加**"。
...
若要自动创建和部署 Azure 资源（如逻辑应用），可以使用[azure 资源管理器模板](../logic-apps/logic-apps-azure-resource-manager-templates-overview.md)，该模板支持[用户分配的身份验证标识](../active-directory/managed-identities-azure-resources/how-to-manage-ua-identity-arm.md)。 在模板的`resources`部分中，逻辑应用的资源定义需要以下各项：

* 一个`identity`对象，其`type`属性设置为`UserAssigned`

* 指定标识`userAssignedIdentities`的资源 ID 的子对象，该 ID 是具有`principalId`和`clientId`属性的另一个子对象。

此示例演示 HTTP PUT 请求的逻辑应用资源定义，并包含非参数化`identity`的对象。 对 PUT 请求和后续 GET 操作的响应还具有以下`identity`对象：
...
| 属性 (JSON) | 值 | 描述 |
|-----------------|-------|-------------|
| `principalId` | <*主体 ID*> | Azure AD 租户中用户分配的托管标识的全局唯一标识符（GUID） |
| `clientId` | <*client-ID*> | 逻辑应用的新标识的全局唯一标识符（GUID），用于运行时中的调用 |
||||

如果模板还包括托管标识的资源定义，则可以将`identity`对象参数化。 此示例演示了子`userAssignedIdentities`对象如何引用您`userAssignedIdentity`在模板的`variables`部分中定义的变量。 此变量引用用户分配的标识的资源 ID。
...
| 属性 (JSON) | 值 | 描述 |
|-----------------|-------|-------------|
| `tenantId` | <*Azure-AD-租户-ID*> | 表示用户分配的标识现在为成员的 Azure AD 租户的全局唯一标识符（GUID）。 在 Azure AD 租户中，服务主体的名称与用户分配的标识名称相同。 |
| `principalId` | <*主体 ID*> | Azure AD 租户中用户分配的托管标识的全局唯一标识符（GUID） |
| `clientId` | <*client-ID*> | 逻辑应用的新标识的全局唯一标识符（GUID），用于运行时中的调用 |
||||
...
1. 在资源的菜单中，选择 "**访问控制（IAM）** > "**角色分配**，你可以在其中查看该资源的当前角色分配。 在工具栏上，选择 "**添加** > " "添加**角色分配**"。
...
1. 如果尚未这样做，请添加[支持托管标识的触发器或操作](logic-apps-securing-a-logic-app.md#managed-identity-authentication)。
   ...
   | **标头** | 是，适用于 Azure 存储 | `x-ms-blob-type` = `BlockBlob` <p>`x-ms-version` = `2019-02-02` | Azure `x-ms-blob-type`存储`x-ms-version`操作所需的和标头值。 <p><p>**重要提示**：在 Azure 存储的传出 HTTP 触发器和操作请求中，标头`x-ms-version`要求要运行的操作的属性和 API 版本。 <p>有关详细信息，请参阅以下主题： <p><p>- [请求标头-快照 Blob](https://docs.microsoft.com/rest/api/storageservices/snapshot-blob#request) <br>- [Azure 存储服务的版本控制](https://docs.microsoft.com/rest/api/storageservices/versioning-for-the-azure-storage-services#specifying-service-versions-in-requests) |
...
1. 现在，将**身份验证**属性添加到 HTTP 操作。 从 "**添加新参数**" 列表中，选择 "**身份验证**"。
   ...
   > [!NOTE]
   > 并非所有触发器和操作支持都允许您添加身份验证类型。 有关详细信息，请参阅[针对出站调用添加身份验证](../logic-apps/logic-apps-securing-a-logic-app.md#add-authentication-outbound)。
...
1. 在某些触发器和操作上，还会显示**受众**属性，使你可以设置目标资源 ID。 将**受众**属性设置为[目标资源或服务的资源 ID](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication)。 否则，默认情况下，**受众**属性使用`https://management.azure.com/`资源 id，该 id 是 AZURE 资源管理器的资源 id。
   ...
   > [!IMPORTANT]
   > 请确保目标资源 ID 与 Azure Active Directory （AD）预期的值（包括任何必需的尾部斜杠）*完全匹配*。 例如，所有 Azure Blob 存储帐户的资源 ID 都需要尾部斜杠。 但是，特定存储帐户的资源 ID 不需要尾部斜杠。 检查[支持 Azure AD 的 Azure 服务的资源 id](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication)。

   此示例将**受众**属性设置为`https://storage.azure.com/` ，以便用于身份验证的访问令牌对所有存储帐户都有效。 但是，你还可以为特定的存储帐户指定`https://fabrikamstorageaccount.blob.core.windows.net`根服务 URL。
...
1. 在逻辑应用菜单上的 "**设置**" 下，选择 "**标识**"，然后按照标识的步骤进行操作：
   ...
   * 选择 > **"** > **保存**时**分配系统**"。 当 Azure 提示你确认时，请选择 **"是"**。
...
如果使用 Azure 资源管理器模板创建了逻辑应用的托管标识，请将该`identity`对象的`type`子属性设置为。 `None` 对于系统管理的标识，此操作还会从 Azure AD 删除主体 ID。
