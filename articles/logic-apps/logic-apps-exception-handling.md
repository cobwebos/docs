...
重试策略是最基本的异常和错误处理类型。 如果初始请求超时或失败（导致 429 或 5xx 响应的任何请求），此策略将定义是否应重试操作以及如何重试。 有三种类型的重试策略：`exponential`、`fixed` 和 `none`。 如果工作流定义中未提供重试策略，则使用默认策略。 可以在特定操作或触发器的**输入**中配置重试策略（如果可重试）。 同样，可在逻辑应用设计器中的“设置”下为给定块配置重试策略（如果适用）。

有关重试策略的限制的信息，请参阅[逻辑应用限制和配置](../logic-apps/logic-apps-limits-and-config.md)；有关支持的语法的详细信息，请参阅[“工作流操作和触发器”中的“重试策略”部分][retryPolicyMSDN]。

### <a name="exponential-interval"></a>指数间隔
`exponential` 策略类型将在指数级增长的范围内的随机时间间隔后重试失败的请求。 保证每次重试尝试以大于 **minimumInterval** 且小于 **maximumInterval** 的随机间隔发送。 将在以下范围内为每次重试生成统一随机变量（最大为 **count**）：
<table>
<tr><th> 随机变量范围 </th></tr>
<tr><td>
...
对于 `exponential` 类型策略，**count** 和 **interval** 是必需的，而 **minimumInterval** 和 **maximumInterval** 是可选提供的，分别用于替代 PT5S 和 PT1D 的默认值。

| 元素名称 | 必选 | 类型 | 说明 |
| ------------ | -------- | ---- | ----------- |
| type | 是 | String | `exponential` |
| 计数 | 是 | Integer | 重试次数必须介于 1 和 90 之间  |
| interval | 是 | String | [ISO 8601 格式](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations)的重试间隔必须介于 PT5S 和 PT1D 之间 |
| minimumInterval | 否| String | [ISO 8601 格式](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations)的重试最小间隔必须介于 PT5S 和 **interval** 之间 |
| maximumInterval | 否| String | [ISO 8601 格式](https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations)的重试最小间隔必须介于 **interval** 和 PT1D 之间 |

### <a name="fixed-interval"></a>固定间隔

`fixed` 策略类型将在发送下一个请求之前等待所提供的时间间隔重试失败的请求。
...
### <a name="default"></a>默认
如果未指定重试策略，则将使用默认策略。 默认策略是指数间隔策略，它以缩放幅度为 7.5 秒的指数级增长间隔（上限为 5 到 45 秒之间）最多发送 4 次重试。 此默认策略（未定义 **retryPolicy** 时使用）等效于以下示例 HTTP 工作流定义中的策略：
