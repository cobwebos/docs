...
上述模式非常适合于处理运行中的错误和异常，不过也可以独立于运行本身来标识和响应错误。 [Azure 诊断](../logic-apps/logic-apps-monitor-your-logic-apps.md)提供了一种将所有工作流事件（包括所有运行和操作状态）发送到 azure 存储帐户或使用[azure 事件](../event-hubs/event-hubs-about.md)中心创建的事件中心的简单方法。
