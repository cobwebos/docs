...
*协议*通过定义要在与企业到企业（B2B）消息交换时使用的特定行业标准协议，帮助组织和企业彼此无缝通信。 协议提供了共同的优势，例如：

* 使组织能够使用众所周知的格式交换信息。
* 提高 B2B 交易时的效率。
* 可以轻松地创建、管理和使用来生成企业集成解决方案。

本文介绍如何创建 AS2、EDIFACT 或 X12 协议，该协议可以在使用[Enterprise Integration Pack](../logic-apps/logic-apps-enterprise-integration-overview.md)和[Azure 逻辑应用](../logic-apps/logic-apps-overview.md)为 B2B 方案生成企业集成解决方案时使用。 创建协议后，可以使用 AS2、EDIFACT 或 X12 连接器来交换 B2B 消息。
...
* 至少两个已在集成帐户中创建的[贸易合作伙伴](../logic-apps/logic-apps-enterprise-integration-partners.md)。 协议要求主机伙伴和来宾伙伴。 双方伙伴都必须使用与要创建的协议相同的 "业务标识" 限定符，如 AS2、X12 或 EDIFACT。

* 可选：要在其中使用协议的逻辑应用，以及用于启动逻辑应用工作流的触发器。 若要仅创建集成帐户和 B2B 项目，不需要使用逻辑应用。 但是，在逻辑应用可以在集成帐户中使用 B2B 项目之前，你必须将集成帐户链接到逻辑应用。 如果不熟悉逻辑应用，请查看[什么是 Azure 逻辑应用](../logic-apps/logic-apps-overview.md)和[快速入门：创建第一个逻辑应用](../logic-apps/quickstart-create-first-logic-app-workflow.md)。

## <a name="create-agreements"></a>创建协议

1. 登录到 [Azure 门户](https://portal.azure.com)。
在 Azure 主菜单中，选择“所有服务”。 在搜索框中，输入 "集成" 作为筛选器。 在结果中，选择 "此资源：**集成帐户**"
...
* [Exchange EDIFACT 消息](logic-apps-enterprise-integration-edifact.md)
