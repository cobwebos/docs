...
在 Azure 预览门户中，所有新资源均创建在资源组中。即使你只创建了一个资源（例如网站），也必须决定是要将该资源添加到现有组，还是为该资源创建新组。

下图显示了带有 Application Insights、数据库服务器、数据库、App Service 计划和网站的资源组。

![资源组摘要](./media/resource-group-overview/resourcegroupsummary2.png)
...
从预览版门户中，可以轻松地查看成本、监视事件和管理警报。下图显示了某个组的合并帐户。
...
有关基于角色的访问控制的详细信息，请参阅 [Microsoft Azure 预览门户中基于角色的访问控制](/documentation/articles/role-based-access-control-configure)。

有关设置访问策略的示例，请参阅[管理和审核对资源的访问权限](/documentation/articles/resource-group-rbac.md)。
