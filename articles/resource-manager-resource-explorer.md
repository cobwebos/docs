...
[Azure 资源浏览器](https://resources.azure.com)是用于查看已在你的订阅中创建的资源的有用工具。通过使用此工具，可以了解组织资源的方式，并查看分配给每个资源的属性。你可以了解可用于某种资源类型的 REST API 操作和 PowerShell cmdlet 的相关信息，并可以通过界面发出命令。在创建资源管理器模板时，资源浏览器可能特别有用，因为使用它可以查看现有资源的属性。
...
当你熟悉 ARM 模板时，正文内容将开始看起来很熟悉！ 响应的 **properties** 部分将与可以在模板的 **properties** 节中提供的值匹配。
