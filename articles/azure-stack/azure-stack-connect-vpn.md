...
* Azure Stack 集成系统 （多节点） 直接连接到 internet 的部署。 在外部的公共 IP 地址范围必须是可以从公共 internet 直接访问。
...
网络配置示例表显示本文中用于示例的值。 可以使用这些值，或者你可以参考它们以更好地理解本文中的示例：
...
3. 从资源列表中选择“本地网络网关”。
4. 在“名称”中，键入 **Azs-GW**。
5. 在中**IP 地址**，键入在 Azure Stack 虚拟网络网关的网络配置表中前面列出的公共 IP 地址。
...
9. 在中**共享密钥 (PSK)**，类型**12345**，然后选择**确定**。

   >[!NOTE]
   >如果对共享密钥使用不同的值，请记住，必须在连接另一端上创建的共享密钥的值进行匹配。

10. 查看“摘要”部分，然后选择“确定”。

## <a name="create-a-virtual-machine"></a>创建虚拟机

现在在 Azure 中，创建虚拟机并将其放在虚拟网络中的 VM 子网上。

1. 在 Azure 门户中，选择“+ 创建资源”。
2. 转到“市场”，选择“计算”。
3. 在虚拟机映像列表中，选择“Windows Server 2016 Datacenter Eval”映像。
4. 在“基本”部分的“名称”中，键入 **AzureVM**。
5. 键入有效的用户名和密码。 此帐户用于登录到虚拟机在创建后。
6. 提供“订阅”、“资源组”和“位置”，并选择“确定”。
7. 在“大小”部分，为此实例选择一种虚拟机大小，然后选择“选择”。
8. 在中**设置**部分中，您可以使用默认设置。 在选择之前**确定**，确认：
...
4. 子网名称默认设置为 **GatewaySubnet**。 网关子网才能正常运行，则必须使用**GatewaySubnet**名称。
...
在 Azure 部署中，本地网关代表一个本地（位于用户位置）物理设备，可连接到 Azure 中的虚拟网关。 但是，在 Azure Stack 中的两端都是连接的虚拟网络网关。

更通用的说明为本地网络网关资源始终指示远程网关连接另一端。
...
6. 在中**IP 地址**，键入在 Azure 中的虚拟网络网关的公共 IP 地址**Azure GW PiP**。 此地址已显示在前面的网络配置表中。
7. 在“地址空间”中，键入 **10.100.0.0/24** 和 **10.100.1.0/24**，用作创建的 Azure VNET 的地址空间。
8. 验证你**订阅**，**资源组**，并**位置**值正确，，然后选择**创建**。
...
若要检查 VPN 连接，创建两个 Vm： 在 Azure 中，一个，另一个在 Azure Stack 中。 创建这些 VM 以后，即可使用它们通过 VPN 隧道发送和接收数据。

1. 在 Azure 门户中，选择“+ 创建资源”。
2. 转到“市场”，选择“计算”。
3. 在虚拟机映像列表中，选择“Windows Server 2016 Datacenter Eval”映像。
4. 在“基本”部分的“名称”中，键入 **Azs-VM**。
5. 键入有效的用户名和密码。 此帐户用于在创建后登录到 VM。
6. 提供“订阅”、“资源组”和“位置”，并选择“确定”。
7. 在“大小”部分，为此实例选择一种虚拟机大小，然后选择“选择”。
...
7. 键入 **ipconfig /all**。
8. 在输出中找到“IPv4 地址”，然后保存该地址供稍后使用。 这是从 Azure ping 的地址。 在示例环境中，该地址为 **10.1.0.4**，但用户环境中的该地址可能有所不同。 该地址应在此前创建的 **10.1.0.0/24** 子网范围内。
...
11. 从远程虚拟机的答复表示测试成功。 可以关闭虚拟机窗口。

你还应执行测试; 错误检测的更严格的数据传输例如，复制不同的大小调整两个方向中的文件。

### <a name="viewing-data-transfer-statistics-through-the-gateway-connection"></a>通过网关连接查看数据传输统计信息

如果你想要了解通过站点到站点连接传送的数据量，此信息现已推出**连接**部分。 也可利用此测试来验证刚发送的 ping 是否确实通过了 VPN 连接。

1. 在登录到 Azure Stack 中的用户虚拟机，使用你的用户帐户登录到用户门户。
