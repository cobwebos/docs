...
| 参数 | 说明 |
| --- | --- |
| **publisher** |用户使用时它们将映像部署的 VM 映像发布者名称段。 一个示例是**Microsoft**。 不包括空格或其他特殊字符在此字段中。 |
| **offer** |用户使用用户部署的 VM 映像时的 VM 映像提供名称段。 一个示例是**WindowsServer**。 不包括空格或其他特殊字符在此字段中。 |
...
| **title** |应用商店项的显示名称。 默认情况下，它设置为`Publisher-Offer-Sku`的 VM 映像的值。 |
| **说明** |应用商店项目的描述。 |
