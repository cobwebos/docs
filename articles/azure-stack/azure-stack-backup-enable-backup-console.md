...
description: "启用基础结构返回服务通过管理门户，以便可以还原 Azure 堆栈，如果故障。"
...
启用基础结构返回服务通过管理门户，以便 Azure 堆栈可以生成备份。 你可以使用这些备份 ups 故障时还原你的环境。
...
3. 键入的路径**备份存储位置**。 必须为文件共享托管在一个单独的设备的路径使用通用命名约定 (UNC) 字符串。 UNC 字符串指定的资源，如共享的文件或设备的位置。 对于服务，你可以使用 IP 地址。 若要确保备份数据的可用性发生灾难时，设备应在单独的位置。
    > [!Note]  
