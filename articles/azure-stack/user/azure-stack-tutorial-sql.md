...
请务必提供 Azure Stack 操作员的公共 ip 地址或完整的 FQDN 为以前创建 SQL AlwaysOn 可用性组的资源组时记录了 SQL 负载均衡器的公共 IP (**SQLPIPsql\<资源组名称\>**)。 此外，该运算符将需要知道 SQL Server 身份验证凭据用于访问 AlwaysOn 可用性组中的 SQL 实例。
