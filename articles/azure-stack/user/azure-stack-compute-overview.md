...
Azure 堆栈虚拟机 (VM) 是一种类型的 Azure 堆栈提供的按需、 可缩放计算资源。 通常情况下，如果需要以更大的力度（相对于其他控制选项）控制计算环境，则应选择 VM。 本文介绍创建 VM 之前的注意事项，以及 VM 的创建方法和管理方式。
...
你使用 VM 的大小由你想要运行的工作负载确定。 然后，选择的大小决定了处理能力、内存和存储容量等因素。 Azure 堆栈提供各种大小来支持多种类型的用途。
