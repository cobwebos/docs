...
本文介绍将替换为非热插拔的硬件组件的一般过程。 步骤将会有所不同的实际替换基于原始设备制造商 (OEM) 硬件供应商联系。 请参阅特定于 Azure 堆栈集成系统的详细步骤的供应商的字段可更换部件 (FRU) 文档。
...
Azure 堆栈运行状况和监视系统监视网络适配器和由存储空间直通控制的数据驱动器的运行的状况。 它无法监测其他硬件组件。 对于所有其他硬件组件，在监视硬件生命周期主机运行的解决方案的特定于供应商的硬件中引发警报。

## <a name="component-replacement-process"></a>组件更换过程

以下步骤提供作为组件更换过程的高级概述。 不用按照这些步骤而不会引用你的 OEM 提供 FRU 文档。
