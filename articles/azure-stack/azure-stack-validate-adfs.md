---
title: 验证 Azure Stack 的 ADFS 集成
description: 使用 Azure Stack 就绪性检查器来验证 Azure Stack 的 ADFS 集成。
...
ms.locfileid: "49947385"
---
# <a name="validate-adfs-integration-for-azure-stack"></a>验证 Azure Stack 的 ADFS 集成

使用 Azure Stack 就绪性检查器工具 (AzsReadinessChecker) 来验证你的环境准备好与 Azure Stack 的 ADFS 集成。 在开始数据中心集成之前或在 Azure Stack 部署之前，应验证 ADFS 集成。

就绪性检查器会验证下列项：

* *联合身份验证元数据*包含联合身份验证的有效 XML 元素。
* *ADFS SSL 证书*可以检索，并且可以生成信任链。 模具上 ADFS 必须信任的 SSL 证书链。 该证书必须由同一个签名*证书颁发机构*作为 Azure Stack 部署证书或由受信任的根颁发机构合作伙伴。 有关受信任的根颁发机构的合作伙伴的完整列表，请参阅： https://gallery.technet.microsoft.com/Trusted-Root-Certificate-123665ca
* *ADFS 签名证书*是受信任和不即将到期。
...
* ADFS 联合身份验证元数据 URL。 例如： `https://adfs.contoso.com/FederationMetadata/2007-06/FederationMetadata.xml`
* 联合身份验证元数据 XML 文件。 例如，FederationMetadata.xml

## <a name="validate-adfs-integration"></a>验证 ADFS 集成
...
1. 运行该工具后，查看输出。 确认，状态为正常的 ADFS 集成要求。 成功验证看起来类似于以下。
...
在生产环境中测试从运算符工作站信任的证书链不会考虑在 Azure Stack 基础结构中的 PKI 信任状况完全表示。 Azure Stack 戳记的公共 VIP 网络需要 PKI 基础结构连接到 CRL。
...
如果验证检查失败，则有关失败的详细信息将显示在 PowerShell 窗口中。 该工具还将记录到的信息*AzsReadinessChecker.log*。
...
**解析**– 导入就绪性检查器模块显式。 复制并将以下代码粘贴到 PowerShell 并更新\<版本\>使用当前安装的版本的版本号。
