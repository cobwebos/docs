...
ms.locfileid: "36938105"
---
# <a name="deploy-the-mysql-resource-provider-on-azure-stack"></a>部署 Azure 堆栈上的 MySQL 资源提供程序

使用 MySQL Server 资源提供程序作为 Azure 堆栈服务公开 MySQL 数据库。 MySQL 资源提供程序作为服务运行在 Windows Server 2016 服务器核心虚拟机 (VM) 上。

## <a name="prerequisites"></a>必备组件

有一些需要准备就绪可以部署 Azure 堆栈 MySQL 资源提供程序之前的几个先决条件。 若要满足这些要求，完成这篇文章中可以访问特权终结点 VM 的计算机上的步骤。
...
* 通过下载将在所需的 Windows Server core VM 添加到 Azure 堆栈 marketplace **Windows Server 2016 Datacenter 的服务器核心**映像。 也可以使用脚本创建 [Windows Server 2016 映像](https://docs.microsoft.com/azure/azure-stack/azure-stack-add-default-image)。 请确保在运行脚本时选择核心选项。
  ...
  >[!NOTE]
  >如需安装更新，可以将单个 .MSU 包放置在本地依赖项路径中。 如果多个。MSU 文件位于，MySQL 资源提供程序安装将失败。

* 下载二进制 MySQL 资源提供程序，然后运行自解压缩程序中，若要将内容提取到临时目录。

  >[!NOTE]
  >若要部署 MySQL 提供程序不具有 Internet 访问权限的系统上，复制[mysql 连接器 net 6.10.5.msi](https://dev.mysql.com/get/Download/sConnector-Net/mysql-connector-net-6.10.5.msi)到本地共享的文件。 当系统提示你输入它，请提供的共享名。 必须安装 Azure 和 Azure Stack PowerShell 模块。

* 资源提供程序有一个相应的 Azure Stack 最低内部版本。 请确保下载你正在运行的 Azure 堆栈的版本正确的二进制文件。
...
对于 ASDK，自签名的证书被创建此安装过程的一部分。 对于 Azure 堆栈集成系统，你必须提供适当的证书。 如需提供自己的证书，请将满足以下条件的 .pfx 文件放在 **DependencyFilesLocalPath** 中：

* \*.dbadapter.\<区域\>.\<外部 FQDN\> 的通配符证书，或采用 mysqladapter.dbadapter.\<区域\>.\<外部 FQDN\> 公用名的单个站点证书。
* 该证书必须受信任。 而无需中间证书必须存在的信任链。
* DependencyFilesLocalPath 中只存在单个证书文件。
* 文件名称不能具有任何特殊字符或空格。

## <a name="deploy-the-resource-provider"></a>部署资源提供程序

你已安装的所有先决条件后，运行**DeployMySqlProvider.ps1**脚本部署 MYSQL 资源提供程序。 DeployMySqlProvider.ps1 脚本会提取你下载的你的 Azure 堆栈版本的 MySQL 资源提供程序二进制文件的一部分。

> [!IMPORTANT]
> 在运行脚本的系统必须安装的.NET 运行时的最新版本的 Windows 10 或 Windows Server 2016 系统。

若要部署 MySQL 资源提供程序，打开新的提升权限的 PowerShell 控制台窗口，并将更改为你在哪里提取的 MySQL 资源提供程序二进制文件的目录。 我们建议使用新的 PowerShell 窗口，以避免潜在问题引起的已加载 PowerShell 模块。

运行**DeployMySqlProvider.ps1**脚本，完成以下任务：

* 将证书和其他项目上传到 Azure Stack 上的存储帐户。
* 将发布库包，以便你可以部署使用库的 MySQL 数据库。
* 发布用于部署宿主服务器的库包。
* 将部署使用 Windows Server 2016 core 映像下载，然后再安装 MySQL 资源提供程序的 VM。
* 注册映射到资源提供程序 VM 的本地 DNS 记录。
* 注册资源提供程序与本地 Azure 资源管理器的运算符和用户帐户。
* （可选） 在资源提供程序安装过程中安装的单个 Windows Server 更新。

> [!NOTE]
> MySQL 资源提供程序部署的启动时，则**system.local.mysqladapter**创建资源组。 可能需要最多 75 分钟才能完成到此资源组所需的四个部署。

### <a name="deploymysqlproviderps1-parameters"></a>DeployMySqlProvider.ps1 参数

你可以指定这些参数从命令行。 如果不这样做，或者如果任何参数验证失败，系统会提示你提供所需的参数。
...
| **DependencyFilesLocalPath** | 包含 [mysql-connector-net-6.10.5.msi](https://dev.mysql.com/get/Downloads/Connector-Net/mysql-connector-net-6.10.5.msi) 的本地共享路径。 如果提供其中一个路径，则也必须将证书文件放在此目录中。 | _可选_单个节点，_必需_多节点。 |
...
4. 在资源的摘要页上组概述，在消息**部署**应**3 Succeeded**。
