...
Azure 堆栈系统数据或遥测，会自动上载到 Microsoft 通过连接的用户体验。 Microsoft 团队主要是为了提高客户体验，以及为安全性、 运行状况、 质量和性能分析使用从 Azure 堆栈遥测收集的数据。
...
> [!NOTE]
> 若要支持遥测数据流，必须在你的网络中打开端口 443 (HTTPS)。 连接的用户体验和遥测组件连接到在 https://v10.vortex-win.data.microsoft.com Microsoft 数据管理服务。连接的用户体验和遥测组件还链接到 https://settings-win.data.microsoft.com 下载配置信息。

## <a name="privacy-considerations"></a>隐私注意事项
ETW 服务将返回到受保护的云存储路由遥测数据。 遥测数据的最小特权指南访问权限原则。 仅 Microsoft 人员具有有效的业务需要允许对遥测数据的访问。 Microsoft 客户的决定中所述的有限目的; 二是不共享个人客户数据与第三方除[Microsoft 隐私声明](https://privacy.microsoft.com/PrivacyStatement)。 商业报表与 Oem 和包含聚合的匿名信息的遥测信息的合作伙伴共享。 内部 Microsoft 团队包括隐私、 法律和数据管理利益干系人进行共享决策的数据。

Microsoft 认为中和实践信息最小化。 我们还尽量收集所需，信息和我们将其存储为仅只要它需要提供服务或分析。 很多有关如何工作的 Azure 堆栈系统和 Azure 服务的信息是在六个月内删除。 汇总或聚合的数据将保留更长一段。

我们了解隐私和安全的客户信息很重要。  我们已经仔细和全面的方法与客户隐私和与 Azure 堆栈的客户数据的保护。 IT 管理员可以控制以在任何时候自定义功能和隐私设置。 我们致力于透明度和信任十分明显：
...
遥测还有助于 Microsoft 能够更好地了解客户部署组件的方式、 使用的功能，并使用服务来实现业务目标。 获取该数据中的信息可帮助我们确定优先工程投资客户体验和工作负荷会直接影响的区域中。
...
建议不要关闭你的组织中的遥测，如遥测提供驱动器改进的产品功能和稳定性的数据。 我们知道，但是，，在某些情况下这可能有必要。 

在这些情况下，你可以配置通过使用注册表设置预部署或使用遥测终结点后期部署发送给 Microsoft 的遥测级别。

### <a name="asdk-set-telemetry-level-in-the-windows-registry"></a>ASDK： 在 Windows 注册表中设置遥测级别
...
- * * 有助于提供的了解哪些资源提供程序系统和虚拟机上安装并标识潜在的兼容性问题的兼容性数据。
...
关闭 Windows 和 Azure 堆栈遥测还将禁用 SQL 遥测。 有关 Windows Server 遥测设置的含义的详细信息，请参阅[Windows 遥测白皮书](https://aka.ms/winservtelemetry)。 
...
### <a name="asdk-and-multinode-enable-or-disable-telemetry-after-deployment"></a>ASDK 和多节点： 启用或禁用部署后的遥测
