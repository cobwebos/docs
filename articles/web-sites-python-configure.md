<properties linkid="develop-python-tutorials-web-sites-configuration" urlDisplayName="配置 Azure 网站的 Python" pageTitle="配置 Azure 网站的 Python" metaKeywords="" description="本教程介绍用于在 Azure 网站上创作并配置符合基本 Web 服务器网关接口 (WSGI) 的 Python 应用程序的各种方法。" metaCanonical="" services="web-sites" documentationCenter="Python" title="Configuring Python with Azure Websites" authors="" solutions="" manager="" editor="" />
...
Azure 库包含用于 Bottle、Django 和 Flask 框架的模板。如果您正在开发您的第一个 Azure 网站，或者您不熟悉 Git，我们建议您遵循以下教程之一，其中包括用于从 Windows 或 Mac 使用 Git 部署从库构建工作应用程序的分步说明：
...
如果您没有现成的网站，则可从 Azure 管理门户创建一个网站。单击左下角的"新建"按钮。将出现一个窗口。依次单击"计算"、"网站"和"快速创建"。
...
<h2><a name="wsgi-handler"></a>WSGI 处理程序</h2>
...
可以使用 `python app.py` 在本地运行此应用程序，方法是：在 Web 浏览器中浏览到 `http://localhost:5555`。


<h2><a name="virtual-environment"></a>虚拟环境</h2>
...
<h2><a name="package-management"></a>包管理</h2>
...
可以配置规则 `Static Files` 以为不同于 URL 中位置的磁盘上位置中的文件服务。在以下规则定义中，对 `http://pythonapp.chinacloudsites.cn/static/site.css` 的请求将为磁盘上 `\FlaskWebProject\static\site.css` 处的文件服务，而不为 `\static\site.css`处的文件服务。
...
`WSGI_ALT_VIRTUALENV_HANDLER` 是指定 WSGI 处理程序的位置。在上面的示例中，这是 `app.wsgi_app`，因为处理程序是根文件夹中的 `app.py` 中一个名为 `wsgi_app` 的函数。

可以自定义 `PYTHONPATH`，但是，如果通过在 requirements.txt 中指定所有依赖项将全部安装在虚拟环境中，则不需要对其更改。


<h2><a name="virtual-environment-proxy"></a>虚拟环境代理</h2>
...
<h2><a name="customize-git-deployment"></a>自定义 Git 部署</h2>
