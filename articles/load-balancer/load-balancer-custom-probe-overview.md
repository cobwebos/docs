...
ms.locfileid: "80477020"
---
# <a name="load-balancer-health-probes"></a>负载均衡器运行状况探测

将负载平衡规则与 Azure 负载均衡器一起使用时，需要指定运行状况探测，以允许负载均衡器检测后端终结点状态。  运行状况探测的配置和探测响应确定哪个后端池实例要接收新流。 可以使用运行状况探测来检测后端终结点上应用程序的故障。 还可以对运行状况探测生成自定义响应，并使用用于控制流的运行状况探测来管理负载或计划内停机。 运行状况探测失败时，负载均衡器停止向各个不正常的实例发送新流。 出站连接不受影响，仅入站连接受到影响。

运行状况探测支持多个协议。 可用的具体运行状况探测协议因负载均衡器 SKU 而异。  此外，服务的行为也因负载均衡器 SKU 而异，如下表所述：

| | 标准 SKU | 基本 SKU |
| --- | --- | --- |
| [探头类型](#types) | TCP、HTTP、HTTPS | TCP、HTTP |
...
>[!IMPORTANT]
>无论配置的超时阈值如何，如果服务器返回任何状态代码不是 HTTP 200 正常，或者连接通过 TCP 重置终止，HTTP（S） 负载均衡器运行状况探测将自动向下探测实例。

## <a name="probe-configuration"></a><a name="probes"></a>探头配置
...
可以通过一个示例来进一步演示该行为。 如果将探头响应数设置为 2，间隔设置为 5 秒，则意味着必须在 10 秒间隔内观察到 2 个探头超时故障。  由于在应用程序更改状态时发送探测的时间未同步，因此我们可以分两种情况来界定检测时间：

1. 如果应用程序在第一个探测器到达之前开始生成超时探测器响应，则这些事件的检测需要 10 秒（2 x 5 秒的间隔），加上应用程序开始发出超时信号以发出超时信号到第一个探测器到达时的时间。  可以假设此项检测花费的时间略微超过 10 秒。
2. 如果应用程序在第一个探测器到达后开始生成超时探测响应，则在下一个探测器到达（超时）加上另外 10 秒（2 x 5 秒间隔）之前，不会开始检测这些事件。  可以假设此项检测花费的时间略少于 15 秒。
...
您可以假设对超时探头响应的反应至少需要 10 秒到最多 15 秒才能对来自应用程序的信号变化做出反应。  此示例旨在演示发生的情况，但是，无法准确预测超出此示例所示大致指导范围的持续时间。

>[!NOTE]
>运行状况探测将探测后端池中的所有正在运行的实例。 如果实例已停止，则在再次启动实例之前不会探测该实例。
...
如果出现以下情况，TCP 探测将会失败：
* 实例上的 TCP 侦听器在超时期限内根本未做出响应。  探测器根据超时探测请求的数量进行标记，这些请求配置为在标记下探之前未应答。
* 探测从实例接收 TCP 重置。
...
* 探测端点在探测间隔最小和 30 秒超时期间根本不响应。 在探测标记为未运行并且达到所有超时间隔的总和之前，多个探测请求可能无应答。
* 探测终结点通过 TCP 重置关闭连接。
...
UDP 是无连接的，并且系统不会跟踪 UDP 的流状态。 如果任何后端终结点的运行状况探测失败，则现有 UDP 流将移动到后端池中的另一个正常实例。
...
除了负载均衡器运行状况探测之外，[以下操作使用此 IP 地址](../virtual-network/what-is-ip-address-168-63-129-16.md)：
...
## <a name="design-guidance"></a><a name="design"></a>设计指南

运行状况探测使服务具有复原能力，并使其可缩放。 错误的配置或不当的设计模式可能会影响服务的可用性与可伸缩性。 请通篇阅读本文档，并考虑在将此探测响应标记为停机或运行时对方案造成的影响，以及对应用程序方案可用性造成的影响。

为应用程序设计运行状况模型时，应该探测后端终结点上可以反映该实例以及所提供应用程序服务的运行状况的端口。____  应用程序端口和探测端口不必相同。  在某些方案中，探测端口可能需要不同于应用程序提供服务所用的端口。  
...
基本公共负载均衡器通过 Azure 监视器日志公开每个后端池汇总的运行状况探测状态。  Azure 监视器日志不适用于内部基本负载均衡器。  可以使用[Azure 监视器日志](load-balancer-monitor-log.md)检查公共负载均衡器探测运行状况和探测计数。 可以配合 Power BI 或 Azure Operation Insights 使用日志记录，以提供有关负载均衡器运行状况的统计信息。

## <a name="limitations"></a>限制

- HTTPS 探测不支持使用客户端证书的相互身份验证。
- 您应该假定启用 TCP 时间戳时运行状况探测将失败。
