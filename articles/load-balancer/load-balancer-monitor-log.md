...
7. **诊断设置**有三个选项。  你可以选择一个、两个或全部三个，并根据你的要求进行配置：
   ...
   * **发送到 Log Analytics**
    ...
    你将需要已为此进程创建的事件中心。  若要创建事件中心，请参阅 [Quickstart：使用 Azure 门户 @ no__t 创建事件中心
...
8. 在 "**诊断设置**" 窗格中的 "**日志**" 部分下，选中 "两者" 旁边的复选框：
   ...
   * **Insights-logs-loadbalancerprobehealthstatus**
