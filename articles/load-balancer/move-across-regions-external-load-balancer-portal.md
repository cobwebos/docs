...
3. 选择 **"设置** > **导出"模板**。
4. 选择“导出模板”下的“部署”。********
5. 选择**TEMPLATE** > **编辑参数**以在联机编辑器中打开参数.json 文件。
...
9.  选择**TEMPLATE** > **编辑模板**以在联机编辑器中打开模板.json 文件。
...
10. 若要编辑要将公共 IP 移到的目标区域，请更改 **resources** 下的 **location** 属性：
    ...
    若要获取区域位置代码，请参阅 [Azure 位置](https://azure.microsoft.com/global-infrastructure/locations/)。 区域代码是不带空格的区域名称。 例如，美国中部的代码是**中央代码**。
...
12. 还可以根据要求和需求更改模板中的其他参数：
    ...
    * **SKU**. 可以在配置中将公共 IP 的 SKU 从 standard 更改为 basic 或者从 basic 更改为 standard，只需在 template.json 文件中更改 **sku** 下的 **name** 属性即可：
        ...
        有关基本和标准 SKU 公共 IP 的区别，请参阅[创建、更改或删除公共 IP 地址](https://docs.microsoft.com/azure/virtual-network/virtual-network-public-ip-address)。
...
14. 选择**BASICS** > **订阅**以选择将部署目标公共 IP 的订阅。

15. 选择**BASICS** > **资源组**以选择将部署目标公共 IP 的资源组。 可以选择“新建”来为目标公共 IP 创建新的资源组。**** 请确保所选名称与现有源公共 IP 的源资源组不同。

16. 验证**BASICS** > **位置**是否设置为要部署公共 IP 的目标位置。
...
8.  选择**TEMPLATE** > **编辑模板**以在联机编辑器中打开模板.json 文件。
...
10. 若要获取区域位置代码，请参阅 [Azure 位置](https://azure.microsoft.com/global-infrastructure/locations/)。 区域代码是不带空格的区域名称。 例如，美国中部的代码是**中央代码**。
...
11. 还可以根据要求和需求更改模板中的其他参数：
    ...
    * **负载平衡规则**。 可以通过在 template.json 文件的 **loadBalancingRules** 节中添加或删除条目，在配置中添加或删除负载均衡规则：
    ...
    * **探头**. 可以通过在 template.json 文件的 **probes** 节中添加或删除条目，在配置中添加或删除负载均衡器的探测：
...
13. 选择**BASICS** > **订阅**以选择将部署目标外部负载均衡器的订阅。

15. 选择**BASICS** > **资源组**以选择将部署目标负载均衡器的资源组。 可以选择“新建”来为目标外部负载均衡器创建新的资源组。**** 或者，可以选择先前为公共 IP 创建的现有资源组。 请确保所选名称与现有源外部负载均衡器的源资源组不同。

16. 验证**BASICS** > **位置**是否设置为目标位置，您希望部署外部负载均衡器。
