...
description: 使用 Azure 资源管理器模板将 Azure 外部负载均衡器从一个 Azure 区域移到另一个 Azure 区域，使用 Azure 门户。
author: asudbring
...
以下步骤演示了如何使用资源管理器模板为移动准备外部负载均衡器，并使用 Azure 门户将外部负载均衡器配置移动到目标区域。  在此过程中，必须包含外部负载均衡器的公共 IP 配置，并且必须先完成此配置，然后才能移动外部负载均衡器。
...
5. 若要编辑外部负载均衡器名称的参数，请将源外部负载均衡器名称的属性**值**更改为目标外部负载均衡器的名称，并确保名称为引号：
...
11. 你还可以根据需要更改模板中的其他参数，并根据需要进行选择：
    ...
    * **出站规则**-可以通过编辑**outboundRules**文件中的 "属性 **"，在**配置中添加或删除出站规则：
...
17. 在 "**设置**" 下验证名称是否与上述 "参数编辑器" 中输入的名称相匹配。  验证是否为配置中的任何公共 Ip 填充了资源 Id。
...
若要提交更改并完成公共 IP 和外部负载均衡器的移动，请删除源公共 IP 和外部负载均衡器或资源组。 为此，请在门户中的仪表板中选择公共 IP 和外部负载均衡器或资源组，并在每个页面顶部选择 "**删除**"。
