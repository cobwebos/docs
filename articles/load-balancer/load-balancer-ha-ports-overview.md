...
<a node="diagram"></a>
![中心与分支虚拟网络的关系图，NVA 部署在 HA 模式下](./media/load-balancer-ha-ports-overview/nvaha.png)
...
您可以为后端资源配置*一个*公共标准负载均衡器资源，以及带有 HA 端口的单个内部标准负载均衡器。
...
- 不支持将 HA 端口负载平衡规则与指向相同后端 ip 配置的非 HA 端口负载平衡规则组合。
...
- [在内部标准负载均衡器上配置 HA 端口](load-balancer-configure-ha-ports.md)
