<properties
   ...
   description="利用 Azure Resource Manager 预览版，将 powershell 用于负载平衡器。对负载平衡器使用模板"
...
- 前端 IP 配置 – 单个负载平衡器可包含一个或多个前端 IP 地址（也称为虚拟 IP，即 VIP）。这些 IP 地址充当流量的入口。

- 后端地址池 – 即指与虚拟机网络接口卡 (NIC) 关联且要分配有负载的 IP 地址。

- 负载平衡规则 – 规则属性将给定的前端 IP 和端口组合映射到一组后端 IP 地址和端口组合。单个负载平衡器可拥有多个负载平衡规则。每个规则都包含前端 IP 和端口，以及与 VM 关联的后端 IP 和端口。
...
模板可包含以下项的定义：虚拟机、虚拟网络、可用性集、网络接口 (NIC)、存储帐户、负载平衡器、网络安全组和公开 IP。模板可用于创建复杂应用程序所需的一切内容。模板文件可签入到内容管理系统进行版本控制和协作。
...
- [VNET 中包含负载平衡器和负载平衡规则的 2 个 VM](http://go.microsoft.com/fwlink/?LinkId=544800)
...
- [How to create a load balancer using Azure Resource Manager（如何使用 Azure Resource Manager 创建负载平衡器）](/documentation/articles/load-balancer-get-started-internet-arm-ps/)
...
- [Load Balancer REST APIs（负载平衡器 REST API）](https://msdn.microsoft.com/zh-cn/library/azure/mt163651.aspx)
