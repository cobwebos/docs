...
description: 本文介绍如何将 Azure 基本内部负载平衡器升级到标准公共负载均衡器
...
[Azure 标准负载均衡器](load-balancer-overview.md)通过区域冗余提供了丰富的功能集和高可用性。 要了解有关负载均衡器 SKU 的更多，请参阅[比较表](https://docs.microsoft.com/azure/load-balancer/concepts-limitations#skus)。 由于标准内部负载均衡器不提供出站连接，因此我们提供了创建标准公共负载均衡器的解决方案。

升级分为四个阶段：

1. 将配置迁移到标准公共负载均衡器
2. 将 VM 添加到标准公共负载均衡器的后端池
3. 在出站连接的负载平衡器上创建出站规则
4. 为应避免从互联网到互联网的子网/VM 设置 NSG 规则
...
* 脚本支持需要出站连接的内部负载均衡器升级。 如果任何 VM 不需要出站连接，请参阅[此页面](upgrade-basicInternal-standard.md)进行最佳实践。
* 标准负载均衡器具有新的公共地址。 不可能将与现有基本内部负载平衡器关联的 IP 地址无缝移动到标准公共负载均衡器，因为它们具有不同的 SKU。
...
1. 检查所需的参数：
   ...
   * **新定位： [String]： 必需**– 这是将创建标准负载均衡器的位置。 我们建议将所选基本负载均衡器的相同位置继承到标准负载均衡器，以便更好地与其他现有资源关联。
...
### <a name="create-nsg-rules-for-vms-which-to-refrain-communication-from-or-to-the-internet"></a>为 VM 创建 NSG 规则，这些规则可避免与 Internet 通信
如果要避免 Internet 流量到达 VM，则可以在 VM 的网络接口上创建[NSG 规则](https://docs.microsoft.com/azure/virtual-network/manage-network-security-group)。
