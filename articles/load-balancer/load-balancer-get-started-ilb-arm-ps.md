...
> [!NOTE]
> Azure 提供两个用于创建和处理资源的部署模型：[资源管理器](../azure-resource-manager/resource-manager-deployment-model.md)和[经典](load-balancer-get-started-ilb-classic-ps.md)。 本文介绍如何使用资源管理器部署模型创建负载均衡器。 Microsoft 建议将 Azure 资源管理器用于大多数新的部署。
