...
- 对传入到虚拟机的 Internet 流量进行负载平衡。这称为[面向 Internet 的负载平衡](load-balancer-internet-overview.md)。
- 对虚拟网络中虚拟机之间的流量、云服务中虚拟机之间的流量或本地计算机和跨界虚拟网络中虚拟机之间的流量进行负载平衡。这称为[内部负载平衡](load-balancer-internal-overview.md)。
- 将外部流量转发到特定的虚拟机。

云中的所有资源都需要通过公共 IP 地址从 Internet 进行访问。Microsoft Azure 中的云基础结构对其资源使用不可路由的 IP 地址。为了与 Internet 通信，它对公共 IP 地址使用网络地址转换 (NAT)。

务必了解 Azure 经典[部署模型](../resource-manager-deployment-model.md)与 Resource Manager 部署模型之间的差异，因为负载平衡器在各个部署模型中的配置并不相同。
...
负载平衡器可以通过基于 Resource Manager 的 API 和工具进行管理。若要了解有关 Resource Manager 的详细信息，请参阅 [Resource Manager 概述](../resource-group-overview.md)。
