...
标准负载均衡器仅支持在后端池中具有标准 IP 地址的 VM。 在本部分，我们将在两个不同的区域（区域 1 和区域 2）中创建具有标准公共 IP 地址的两个 VM（*myVM1* 和 *myVM2*），这些 VM 将添加到前面创建的标准负载均衡器后端池。
...
1. 选择“查看 + 创建”。   
1. 检查设置，然后选择“创建”。
1. 遵循相应的步骤创建具有标准 SKU 公共 IP 地址 *myVM2-ip* 的另一个 VM，其名称为 *myVM2*，其“可用性区域”设置为 **2**，其他所有设置与 *myVM1* 相同。 
