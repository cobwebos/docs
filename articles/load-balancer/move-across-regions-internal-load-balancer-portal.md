...
5. 单击**TEMPLATE** > **编辑参数**以在联机编辑器中打开**参数.json**文件。
...
9. 单击**TEMPLATE** > **编辑模板**以在联机编辑器中打开**模板.json**文件。
...
11. 若要获取区域位置代码，请参阅 [Azure 位置](https://azure.microsoft.com/global-infrastructure/locations/)。  区域的代码是没有空格的区域名称，**美国** = **中部中心**。
...
14. 单击**BASICS** > **订阅**以选择将部署目标 VNET 的订阅。

15. 单击**BASICS** > **资源组**以选择将部署目标 VNET 的资源组。  可以单击“新建”来为目标 VNET 创建新的资源组。****  确保该名称不同于现有 VNET 的源资源组名称。

16. 验证**BASICS** > **位置**设置为您希望部署 VNET 的目标位置。
...
6. 若要编辑上面已移动的目标虚拟网络的值，必须先获取资源 ID，然后将其复制并粘贴到 **parameters.json** 文件中。 若要获取 ID：
    ...
    1. 登录到另一个浏览器选项卡或窗口中的[Azure 门户](https://portal.azure.com) > **资源组**。
...
7. 单击**TEMPLATE** > **编辑模板**以在联机编辑器中打开**模板.json**文件。
...
9.  若要获取区域位置代码，请参阅 [Azure 位置](https://azure.microsoft.com/global-infrastructure/locations/)。  区域的代码是没有空格的区域名称，**美国** = **中部中心**。
...
10. 也可选择更改模板中的其他参数，这些参数是可选的，具体取决于你的要求：
    ...
    * **入站 NAT 规则** - 可以通过在 **template.json** 文件的 **inboundNatRules** 节中添加或删除条目，来添加或删除负载均衡器的入站 NAT 规则：
        ...
        ```
        要完成入站 NAT 规则的添加或删除，必须在**模板.json**文件末尾作为**类型**属性存在或删除该规则：
...
13. 单击**BASICS** > **订阅**以选择将部署目标内部负载均衡器的订阅。

15. 单击**BASICS** > **资源组**以选择将部署目标负载均衡器的资源组。  可以单击“新建”来为目标内部负载均衡器创建新资源组，或选择前面为虚拟网络创建的现有资源组。****  请确保所选名称与现有源内部负载均衡器的源资源组不同。

16. 验证**BASICS** > **位置**设置为您希望部署内部负载均衡器的目标位置。
