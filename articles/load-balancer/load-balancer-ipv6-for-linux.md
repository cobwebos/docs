...
description: 在本文中，了解如何为 Linux VM 配置 DHCPv6。
...
3. 续订 IPv6 地址：
    ...
    ```
从 Ubuntu 17.10 开始，默认网络配置机制为[NETPLAN]( https://netplan.io)。  在安装/实例化时间，NETPLAN 从此位置的 YAML 配置文件读取网络配置：/_lib 等，运行\/netplan/_.yaml。

请在配置中为每个以太网接口包括*dhcp6：true*语句。  例如：
...
在早期启动期间，netplan"网络呈现器"将配置写入 /run，将设备控制移交给指定的网络守护进程有关 NETPLAN 的参考信息，请参阅https://netplan.io/reference。
