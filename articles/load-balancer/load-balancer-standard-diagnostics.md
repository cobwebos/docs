---
title: 包含指标、警报和资源运行状况的诊断 - Azure 标准负载均衡器
...
* **多维指标和警报**：通过[Azure 监视器](https://docs.microsoft.com/azure/azure-monitor/overview)为标准负载均衡器配置提供多维诊断功能。 可以监视、管理和排查标准负载均衡器资源问题。

* **资源运行状况**：Azure 门户中的"负载均衡器"页和"资源运行状况"页（在监视器下）公开了标准负载均衡器的资源运行状况部分。 

本文概要介绍这些功能，以及如何对标准负载均衡器使用这些功能。 

## <a name="multi-dimensional-metrics"></a><a name = "MultiDimensionalMetrics"></a>多维指标

Azure 负载均衡器通过 Azure 门户中的 Azure 指标提供多维指标，它可以帮助您获得对负载均衡器资源的实时诊断见解。 
...
| 已分配的 SNAT 端口 | 公共负载均衡器 | 标准负载均衡器报告每个后端实例分配的 SNAT 端口数 | 平均值。 |
| 已使用的 SNAT 端口 | 公共负载均衡器 | 标准负载均衡器报告每个后端实例使用的 SNAT 端口数。 | 平均值 | 
| 字节计数器 |  公共和内部负载均衡器 | 标准负载均衡器按前端报告处理的数据。 您可能会注意到字节在后端实例之间分布不均。 这是预料之中的，因为 Azure 的负载均衡器算法基于流 | 平均值 |
...
2. 设置适当的指标聚合类型。
3. （可选）配置需要的筛选和分组。
4. 可以选择配置时间范围和聚合。 默认情况下，时间以 UTC 显示。

  >[!NOTE] 
  >当将某些指标解释为数据每分钟采样一次时，时间聚合非常重要。 如果时间聚合设置为五分钟，并且指标聚合类型 Sum 用于 SNAT 分配等指标，则图形将显示总分配的 SNAT 端口的五倍。 
...
#### <a name="is-the-data-path-up-and-available-for-my-load-balancer-frontend"></a>数据路径是否已打开，并且可用于负载平衡器前端？
<details><summary>展开</summary>

数据路径可用性可用性指标描述区域内数据路径到 VM 所在的计算主机的运行状况。 此指标反映了 Azure 基础结构的运行状况。 使用此指标可以：
- 监视服务的外部可用性
- 深入分析和了解部署服务的平台是否正常，或者来宾 OS 或应用程序实例是否正常。
- 查明某个事件是与服务还是底层数据平面相关。 请勿将此指标与运行状况探测状态（"后端实例可用性"）混淆。
...
#### <a name="are-the-backend-instances-for-my-load-balancer-responding-to-probes"></a>负载均衡器的后端实例响应探测器了吗？
<details>
  <summary>展开</summary>
...
1. 选择**SNAT 使用情况**和/或**SNAT 分配**作为指标类型和**平均值**作为聚合
    * 默认情况下，这是分配给每个后端 VM 或 VMSS 的平均 SNAT 端口数，对应于映射到负载均衡器的所有前端公共 IP，这些端口通过 TCP 和 UDP 聚合。
    * 查看负载均衡器使用或分配的 SNAT 端口总数，请使用指标聚合**总和**
1. 筛选到特定的**协议类型**，一组后端**IP**和/或**前端 IP**。
1. 要监视每个后端或前端实例的运行状况，请应用拆分。 
    * 注释拆分只允许一次显示单个指标。 
1. 例如，要监视每台计算机 TCP 流的 SNAT 使用情况，按**平均值**聚合，按**后端 IP**拆分，按**协议类型**进行筛选。 

![SNAT 分配和使用](./media/load-balancer-standard-diagnostics/snat-usage-and-allocation.png)

*图：一组后端 VM 的平均 TCP SNAT 端口分配和使用*
...
<details>
  <summary>展开</summary>
通过在单个图表上使用数据路径可用性和运行状况探测状态指标的组合，可以确定查找问题并解决问题的位置。 可以确定 Azure 是否正常工作，并据此最终确定配置或应用程序是否为问题的根本原因。

可以使用运行状况探测指标来了解 Azure 如何根据提供的配置查看部署的运行状况。 在监视或确定原因时，查看运行状况探测始终是合理的第一个动作。

您可以更进一步，使用数据路径可用性指标来深入了解 Azure 如何查看负责特定部署的基础数据平面的运行状况。 结合使用两个指标，可以查明错误的所在位置，如以下示例所示：

![组合使用“数据路径可用性”和“运行状况探测状态”指标](./media/load-balancer-standard-diagnostics/lbmetrics-dipnvipavailability-2bnew.png)

*图：合并数据路径可用性和运行状况探测状态指标*

此图表显示以下信息：
- 承载 VM 的基础结构在过去不可用，在图表开始处显示为 0%。 稍后，基础结构正常，VM 可访问，多个 VM 置于后端。 此信息由数据路径可用性的蓝色跟踪指示，该跟踪后来为 100%。 
- 由紫色曲线指示的运行状况探测状态在图表的开头为 0%。 绿色圆圈区域突出显示运行状况探测状态变得正常，此时客户的部署能够接受新流。
...
| 未知 | 标准负载均衡器资源的资源运行状况尚未更新。<br>（*未知*状态还可能意味着资源未与您的标准负载均衡器连接。  |
