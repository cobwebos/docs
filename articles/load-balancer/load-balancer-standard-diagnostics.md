...
Azure 负载均衡器提供了通过在 Azure 门户中，新的 Azure 指标的新多维度指标，以及它帮助你深入了解实时诊断负载均衡器资源。 

各种标准负载均衡器配置提供以下指标：

| 指标 | 资源类型 | 描述 | 建议的聚合 |
| --- | --- | --- | --- |
| 数据路径可用性 （VIP 可用性）| 公共负载均衡器 | 标准负载均衡器持续运用从区域内部到负载均衡器前端，直到支持 VM 的 SDN 堆栈的数据路径。 只要保留正常实例，这种度量就会遵循应用程序负载均衡的流量所用的相同路径。 此外，还会验证客户使用的数据路径。 度量对于应用程序不可见，且不会干扰其他操作。| 平均值 |
...
在 Azure 门户公开通过指标页，可在特定资源的负载均衡器资源页和 Azure 监视器页面的负载均衡器度量值。 
...
若要获取标准负载均衡器资源的数据路径可用性：
1. 确保选择正确的负载均衡器资源。 
2. 在中**指标**下拉列表中，选择**数据路径可用性**。 
3. 在“聚合”  下拉列表中，选择“平均”  。 
4. 此外，前端 IP 地址或前端端口为维度的所需的前端 IP 地址或前端端口添加筛选器，然后将它们分组按所选维度。
...
出于诊断目的，可以使用[以及运行状况探测状态的数据路径可用性指标](#vipavailabilityandhealthprobes)。
...
若要获取标准负载均衡器资源运行状况探测状态：
1. 选择**运行状况探测状态**指标**Avg**聚合类型。 
2. 应用筛选器所需的前端 IP 地址或端口 （或两者）。
...
![合并数据路径可用性和运行状况探测状态指标](./media/load-balancer-standard-diagnostics/lbmetrics-dipnvipavailability-2bnew.png)

图：*合并数据路径可用性和运行状况探测状态指标*

此图表显示以下信息：
- 托管 Vm 的基础结构是不可用，并在图表的开始处的 0%。 更高版本，基础结构的正常和 Vm 是可访问，并在后端中放置多个 VM。 此信息指示数据路径可用性 （VIP 可用性），这是更高版本以 100%的蓝色轨迹。 
- 运行状况探测状态 （DIP 可用性），由紫色跟踪，位于图表的开始处的 0%。 在其中运行状况探测状态 （DIP 可用性） 变为正常运行，并在该位置将客户的部署的绿色突出显示的带圆圈的区域就能够接受新流量。
