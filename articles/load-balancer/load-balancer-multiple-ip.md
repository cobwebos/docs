---
title: "在多个 IP 配置上进行负载均衡 | Microsoft 文档"
...
本文介绍如何将 Azure 负载均衡器用于每个虚拟网络接口 (NIC) 的多个 IP 地址。 目前，对一个 NIC 的多个 IP 地址的支持是预览版的功能。 有关详细信息，请参阅本文中的[限制](#limitations)部分。 以下场景说明了如何通过负载均衡器使用此功能。
...
目前，辅助 IP 配置的负载均衡只能使用 Azure PowerShell 和 Azure CLI 进行配置。 该限制是暂时性的，以后随时可能更改。 重新访问此页以检查更新。
