...
| SYN 数据包 | 负载均衡器标准版不会终止 TCP 连接，也不会与 TCP 或 UDP 数据包流交互。 流及其握手始终位于源和 VM 实例之间。 若要更好地排查 TCP 协议方案的问题，可以使用 SYN 数据包了解做出了多少次 TCP 连接尝试。 该指标将报告接收到的 TCP SYN 数据包数目。 该指标也可反映尝试与服务建立连接的客户端。|
