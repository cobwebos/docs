...
你可以通过创建 OnDemand 流式处理定位符并生成流 URL 来流式传输自适应比特率 MP4 集。[对资产进行编码](/zh-cn/documentation/articles/media-services-rest-encode-asset) 主题说明了如何编码成自适应比特率 MP4 集。在创建定位符之前，应该根据[此主题](/zh-cn/documentation/articles/media-services-rest-configure-asset-delivery-policy) 中所述配置资产传送策略。 
...
使用创建定位符后返回的 **Path** 值生成平滑流、HLS 和 MPEG DASH URL。 
