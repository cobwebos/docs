...
>|----|----|----|------------|
>| Tomcat 全局错误 | tomcat. 错误 | 计数 | 处理的请求发生的错误数 |
...
>| 分配的应用内存 | jvm。已提交 | 字节 | 表示可保证由 JVM 使用的内存量。 JVM 可能会将内存释放给系统，并提交的时间可能低于 init。 提交的将始终大于或等于使用。 |
>| 已使用的应用内存 | jvm。已使用 | 字节 | 表示当前使用的内存量（以字节为单位）。 |
...
>| 最大可用的旧代数据大小 | jvm. 最大数据大小 | 字节 | 自启动 Java 虚拟机以来旧代内存池的最大内存使用量。 |
>| 旧代数据大小 | jvm. 数据大小 | 字节 | 完全 GC 后的旧代内存池的大小。 |
>| 提升为旧代数据大小 | jvm。已升级 | 字节 | 在 gc 到 gc 之后，旧代内存池的大小正增加的计数。 |
>| 提升为新代数据大小 | jvm。已分配 | 字节 | 增加了一个 GC 到下一个 GC 之前的空间增加的空间。 |
>| GC 暂停计数 | jvm （total-count） | 计数 | 此 JMV 开始后的总 GC 计数，包括年轻人和旧 GC。 |
...
>| Tomcat 请求总数 | tomcat. global. 请求（总数） | 计数 | Tomcat web 服务器处理的请求总数 |
...
>| 名称 | 弹簧制动器指标名称 | 单位 | 详细信息 |
>|----|----|----|------------|
>| Tomcat 会话最大活动计数 | tomcat。最大值 | 计数 | 同时处于活动状态的会话的最大数量 |
>| Tomcat 会话最大活动时间 | tomcat。最大值 | 毫秒 | 过期会话处于活动状态的最长时间（秒） |
>| Tomcat 创建的会话数 | tomcat。已创建 | 计数 | 已创建的会话数 |
>| Tomcat 过期的会话数 | tomcat 会话。已过期 | 计数 | 已过期的会话数 |
>| Tomcat 拒绝的会话数 | tomcat 会话。已拒绝 | 计数 | 由于达到了最大活动会话数而未创建的会话数。 |
