...
>| “属性” | 弹簧制动器指标名称 | 计价单位 | 详细信息 |
>|----|----|----|------------|
>| Tomcat 全局错误（不推荐使用）<br><br>（将在2020年7月1日后删除）  | tomcat. 错误 | 计数 | 处理的请求中发生的错误数 |
...
>| “属性” | 弹簧制动器指标名称 | 计价单位 | 详细信息 |
>|----|----|----|------------|
>| 系统 CPU 使用率百分比（不推荐使用）<br><br>（将在2020年7月1日后删除） | system.object。用法 | 百分比 | 整个系统的最近 CPU 使用率。 此值为 [0.0，1.0] 间隔中的双精度值。 如果值为0.0，则表示在观察到的最近一段时间内所有 Cpu 都处于空闲状态，而如果值为1.0，则表示在观察到最近一段时间内，所有 Cpu 都正在积极运行的时间为100%。|
...
>| 应用 CPU 使用率百分比（不推荐使用）<br><br>（2020年7月1日之后已删除） | 应用 CPU 使用率百分比 | 百分比 | Java 虚拟机进程的最近 CPU 使用率。 此值为 [0.0，1.0] 间隔中的双精度值。 如果值为0.0，则表示在观察到的最近一段时间内，任何 Cpu 都不会运行 JVM 进程中的线程，而值1.0 则表示所有 Cpu 都在观察到的最近一段时间内从 100 JVM 中主动运行了线程。 JVM 中的线程包括应用程序线程以及 JVM 内部线程。|
...
>| 已分配应用内存（已弃用）<br><br>（将在2020年7月1日后删除） | jvm。已提交 | 字节 | 表示可保证由 JVM 使用的内存量。 JVM 可能会将内存释放给系统，而提交的内存可能小于初始内存。 提交的内存将始终大于或等于使用的内存。 |
>| jvm。已提交 | jvm。已提交 | 字节 | 表示可保证由 JVM 使用的内存量。 JVM 可能会将内存释放给系统，并提交的时间可能低于 init。 提交的将始终大于或等于使用。 |
>| 使用的应用内存（已弃用）<br><br>（将在2020年7月1日后删除） | jvm。已使用 | 字节 | 表示当前使用的内存量（以字节为单位）。 |
>| jvm。已使用 | jvm。已使用 | 字节 | 表示当前使用的内存量（以字节为单位）。 |
>| 最大应用内存（不推荐使用）<br><br>（将在2020年7月1日后删除） | jvm. max | 字节 | 表示可用于内存管理的最大内存量。 如果定义了 max，则已使用和已提交的内存量将始终小于或等于最大值。 如果内存分配尝试增加已使用的内存（即使使用的内存仍为 true >，即使已使用 <= max），内存分配可能会失败（例如，当系统的虚拟内存不足时）。 |
...
>| 最大可用旧生成数据大小（不推荐使用）<br><br>（将在2020年7月1日后删除） | jvm. 最大数据大小 | 字节 | 自启动 Java 虚拟机以来旧代内存池的最大内存使用量。 |
>| jvm. 最大数据大小 | jvm. 最大数据大小 | 字节 | 自启动 Java 虚拟机以来旧代内存池的最大内存使用量。 |
>| 旧生成数据大小（已弃用）<br><br>（将在2020年7月1日后删除） | jvm. 数据大小 | 字节 | 完全 GC 后的旧代内存池的大小。 |
>| jvm. 数据大小 | jvm. 数据大小 | 字节 | 完全 GC 后的旧代内存池的大小。 |
>| 旧生成数据大小（已弃用）<br><br>（将在2020年7月1日后删除） | jvm。已升级 | 字节 | 在 gc 到 gc 之后，旧代内存池的大小正增加的计数。 |
>| jvm。已升级 | jvm。已升级 | 字节 | 在 gc 到 gc 之后，旧代内存池的大小正增加的计数。 |
>| 提升为年轻人生成数据大小（不推荐使用）<br><br>（将在2020年7月1日后删除） | jvm。已分配 | 字节 | 增加了一个 GC 到下一个 GC 之前的空间增加的空间。 |
>| jvm。已分配 | jvm。已分配 | 字节 | 增加了一个 GC 到下一个 GC 之前的空间增加的空间。 |
>| GC 暂停计数（已弃用）<br><br>（将在2020年7月1日后删除） | jvm （total-count） | 计数 | 此 JMV 开始后的总 GC 计数，包括年轻人和旧 GC。 |
>| jvm。 total. count | jvm （total-count） | 计数 | 此 JMV 开始后的总 GC 计数，包括年轻人和旧 GC。 |
>| GC 暂停总时间（已弃用）<br><br>（将在2020年7月1日后删除） | jvm （总计时间） | 毫秒 | 此 JMV 启动后使用的 GC 时间总数，包括年轻和旧 GC。 |
...
>| “属性” | 弹簧制动器指标名称 | 计价单位 | 详细信息 |
>|----|----|----|------------|
>| Tomcat 发送的总字节数（不推荐使用）<br><br>（2020年7月1日之后已删除） | tomcat. 已发送 | 字节 | 已发送的数据 Tomcat web 服务器量 |
>| tomcat. 已发送 | tomcat. 已发送 | 字节 | 已发送的数据 Tomcat web 服务器量 |
>| Tomcat 收到的总字节数（已弃用）<br><br>（2020年7月1日之后已删除） | tomcat。已接收 | 字节 | 已接收的数据 Tomcat web 服务器数量 |
>| tomcat。已接收 | tomcat。已接收 | 字节 | 已接收的数据 Tomcat web 服务器数量 |
>| Tomcat 请求总时间（已弃用）<br><br>（2020年7月1日之后已删除） | tomcat. 请求（总计时间） | 毫秒 | Tomcat web 服务器处理请求的总时间 |
>| Tomcat 请求总数（不推荐使用）<br><br>（2020年7月1日之后已删除） | tomcat. global. 请求（总数） | 计数 | Tomcat web 服务器处理的请求总数 |
>| tomcat. total. count | tomcat. global. 请求（总数） | 计数 | Tomcat web 服务器处理的请求总数 |
>| Tomcat 请求最大时间（不推荐使用）<br><br>（2020年7月1日之后已删除） | tomcat. max | 毫秒 | Tomcat web 服务器处理请求的最长时间 |
...
>| “属性” | 弹簧制动器指标名称 | 计价单位 | 详细信息 |
>|----|----|----|------------|
>| Tomcat 会话最大活动计数（已弃用）<br><br>（2020年7月1日之后已删除） | tomcat。最大值 | 计数 | 同时处于活动状态的会话的最大数量 |
>| tomcat。最大值 | tomcat。最大值 | 计数 | 同时处于活动状态的会话的最大数量 |
>| Tomcat 会话最大活动时间（已弃用）<br><br>（2020年7月1日之后已删除） | tomcat。最大值 | 毫秒 | 过期会话处于活动状态的最长时间（秒） |
>| tomcat。最大值 | tomcat。最大值 | 毫秒 | 过期会话处于活动状态的最长时间（秒） |
>| Tomcat 会话创建计数（已弃用）<br><br>（2020年7月1日之后已删除） | tomcat。已创建 | 计数 | 已创建的会话数 |
>| tomcat。已创建 | tomcat。已创建 | 计数 | 已创建的会话数 |
>| Tomcat 会话过期计数（已弃用）<br><br>（2020年7月1日之后已删除） | tomcat 会话。已过期 | 计数 | 已过期的会话数 |
>| tomcat 会话。已过期 | tomcat 会话。已过期 | 计数 | 已过期的会话数 |
>| Tomcat 会话已拒绝计数（已弃用）<br><br>（2020年7月1日之后已删除） | tomcat 会话。已拒绝 | 计数 | 由于达到了最大活动会话数而未创建的会话数。 |
