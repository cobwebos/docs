...
\*_若要提高限制，请打开[支持票证](https://azure.microsoft.com/support/faq/)。_
...
* `server.port`Git 存储库的配置文件中不允许使用。 将其添加到配置文件可能会导致应用程序无法从其他应用程序或 internet 中访问。
...
这取决于扩展资源所属的资源提供程序逻辑。 `Microsoft.AppPlatform`实例的扩展资源不属于同一命名空间，因此行为因不同的资源提供程序而异。 例如，删除/移动操作不会级联到**诊断设置**资源。 如果使用与删除的资源 ID 相同的资源 ID 预配新的 Azure 春季云实例，或者以前的 Azure 春季云实例被移回，则之前的**诊断设置**资源将继续扩展它。
