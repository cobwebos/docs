...
5. 添加的解决方案的新磁贴显示在 OMS 的“概述”页面上，OMS 服务处理数据后，便可以开始使用该解决方案。
...
* 通过 Azure 应用商店安装解决方案时，系统会提示提供一个 OMS 工作区和自动化帐户，并创建它们之间的链接。  
* 对于 Azure 应用商店外的解决方案，必须在安装解决方案之前链接 OMS 工作区和自动化帐户。  为此，可以在 Azure 应用商店中选择任何解决方案，并选择 OMS 工作区和自动化帐户。  无需实际安装解决方案，因为只要选择了 OMS 工作区和自动化帐户，就会创建链接。  创建链接后，可以对任何解决方案使用该 OMS 工作区和自动化帐户。 
