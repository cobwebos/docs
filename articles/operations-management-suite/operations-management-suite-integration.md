...
description: 除了使用 OMS 的标准功能外，还可以将其与其他管理应用程序和服务集成，以提供混合管理环境、提供特定于环境的自定义管理方案或向客户提供自定义管理体验。  本文概述了用于与 OMS 集成的不同选项以及指向提供详细技术信息的文章链接。
...
有多种方法可填充 OMS 存储库。  使用的方法取决于如下因素：源数据所在位置、数据格式和需要支持的客户端。  数据存储在存储库中后，数据收集方式没有任何区别。

以下部分介绍了用于填充 OMS 存储库的不同选项。

#### <a name="connected-sources-and-data-sources"></a>连接的源和数据源
连接的源是可为 OMS 存储库检索数据的位置。  数据源和解决方案在连接的源上运行，并定义收集的特定数据。  如果应用程序将数据写入这些数据源之一，则可通过配置数据源收集它。  例如，如果应用程序创建 Syslog 事件，Syslog 数据源可在 Linux 代理上收集它们。

* [Log Analytics 中的数据源](../log-analytics/log-analytics-data-sources.md)

#### <a name="solutions"></a>解决方案
解决方案扩展 OMS 的功能。  解决方案可以从连接的源收集数据，或者可以对已在存储库中收集的记录执行分析。  Microsoft 提供的每个解决方案都有一篇文章提供所收集数据的详细信息。

* [Log Analytics 中的解决方案](../log-analytics/log-analytics-add-solutions.md)

#### <a name="http-data-collector-api"></a>HTTP 数据收集器 API
Log Analytics HTTP 数据收集器 API 是 REST API，允许将 JSON 数据添加到 Log Analytics 存储库。  拥有的应用程序不通过其他数据源或解决方案之一提供数据时，可利用此 API。  它可用于从可调用 API 的任何客户端填充存储库，不依赖任何数据源或解决方案的收集计划。
...
视图设计器允许在 OMS 控制台中创建自定义视图，可在解决方案中向用户提供数据的可视化和分析。  每个视图包括显示在控制台主页上的磁贴和基于定义的日志搜索的许多可视化部分。

* [Log Analytics 视图设计器](../log-analytics/log-analytics-view-designer.md)

#### <a name="power-bi"></a>Power BI
Log Analytics 可将数据从 OMS 存储库自动导出到 Power BI，以便可以利用其可视化和分析工具。  按计划执行此导出操作，以便数据保持最新。 

* [将 Log Analytics 数据导出到 Power BI](../log-analytics/log-analytics-powerbi.md)

## <a name="automation"></a>自动化
OMS 可自动执行进程以响应收集的数据或执行其他管理功能。  它可能从应用程序收集数据并将其插入 OMS 存储库中，或者可以自动更正已知问题以响应在存储库中找到的数据。 
...
可将集成逻辑封装在自定义解决方案中，以在工作区或客户的工作区中运行。  解决方案中包括本文中的所有集成方法以及用于提供完整管理方案的其他资源。  打包解决方案中的资源，以便在删除解决方案后，它创建的所有资源都从 OMS 工作区和 Azure 订阅中删除。

例如，解决方案可以包括自动化 Runbook 以收集和处理数据，然后使用 HTTP 数据收集器 API 填充 Log Analytics 存储库。  还可以包括显示和分析所收集数据的自定义视图。  
