...
| 查询 | 说明 |
| --- | --- |
|基于 Windows 且需要更新的服务器计算机 |`Type:Update OSType!=Linux UpdateState=Needed Optional=false Approved!=false | measure count() by Computer` |
|需要更新的 Linux 服务器 | `Type:Update OSType=Linux UpdateState!="Not needed" | measure count() by Computer` |
...
| 特定计算机缺少的更新（请将相关值替换为你自己的计算机名称） |`Type=Update UpdateState=Needed Optional=false Computer="COMPUTER01.contoso.com" | select Computer,Title,KBID,Product,UpdateSeverity,PublishedDate` |
...
| 基于 Windows 且其更新在更新运行中失败的服务器计算机 | `Type:UpdateRunProgress InstallationStatus=failed | measure count() by Computer, Title, UpdateRunName` |
| 其更新在更新运行中失败的 Linux 服务器 |`Type:UpdateRunProgress InstallationStatus=failed | measure count() by Computer, Product, UpdateRunName` |
...
| 列出哪些更新部署对计算机进行了修改 |`Type:UpdateRunProgress | measure Count() by UpdateRunName` |
|在此更新运行中进行了更新的计算机（请将相关值替换为你的更新部署名称） |`Type:UpdateRunProgress UpdateRunName="DeploymentName" | measure Count() by Computer` |
