...
# <a name="service-map-integration-with-system-center-operations-manager"></a>服务映射与 System Center Operations Manager 的集成
  > [!NOTE]
  > 由于此功能处于个人预览状态，因此不应在生产系统上使用它。
  > 
  
Operations Management Suite 服务映射可自动发现 Windows 和 Linux 系统上的应用程序组件并映射服务之间的通信。 服务映射允许你如所想一般作为提供重要服务的互连系统查看服务器。 服务映射显示任何 TCP 连接的体系结构中服务器、进程和端口之间的连接，只需安装代理，无需任何其他配置。 有关详细信息，请参阅[服务映射文档](operations-management-suite-service-map.md)。
...
2. 在“连接配置”窗口中，输入服务主体的租户名称或 ID、应用程序 ID（也称为用户名或 clientID）和密码，然后单击“下一步”。 有关详细信息，请参阅[创建服务主体](#creating-a-service-principal)。

    ![“连接配置”窗口](media/oms-service-map/scom-config-spn.png)

3. 在“订阅选择”窗口中，选择 Azure 订阅、Azure 资源组（包含 Operations Management Suite 工作区的资源组）和 Operations Management Suite 工作区，然后单击“下一步”。

    ![Operations Manager 配置工作区](media/oms-service-map/scom-config-workspace.png)

4. 在“服务器选择”窗口中，配置包含要在 Operations Manager 与服务映射之间同步的服务器的服务映射服务器组。 单击“添加/删除服务器”。   
...
5. 可选：选择要与 Operations Management Suite 通信的管理服务器资源池，然后单击“添加工作区”。
    ...
    >[!NOTE]
    >默认同步间隔设置为 60 分钟。 可以配置重写来更改同步间隔。 还可以通过“创作”窗格手动将服务器添加到服务映射服务器组。 为此，请选择“组”，然后搜索“服务映射服务器组”。 下一次同步时，将会根据配置的同步间隔同步这些服务器的服务器映射。

## <a name="monitor-service-map"></a>监视服务映射
连接 Operations Management Suite 工作区后，Operations Manager 控制台的“监视”窗格中将显示新文件夹“服务映射”。

![Operations Manager 的“监视”窗格](media/oms-service-map/scom-monitoring.png)

“服务映射”文件夹包含三个节点：
* **活动警报**：列出 Operations Manager 与 Operations Management Suite 中服务映射解决方案之间的通信的所有相关活动警报。

    >[!NOTE]
    >这些警报不是 Operations Manager 中显示的 Operations Management Suite 警报。
...
当前设计存在以下问题和限制：
* 尽管可以通过“创作”窗格手动将服务器添加到服务映射服务器组，但只会在下一个同步周期从服务映射同步这些服务器的映射。 默认设置为 60 分钟，但你可以重写计时。 
* 可以连接到单个 Operations Management Suite 工作区。
