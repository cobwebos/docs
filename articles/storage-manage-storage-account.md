...
-   [如何：管理存储帐户复制][如何：管理存储帐户复制]
-   [如何：查看、复制和重新生成存储访问密钥][如何：查看、复制和重新生成存储访问密钥]
-   [如何：删除存储帐户][如何：删除存储帐户]
...
有关存储帐户复制的定价信息，请参阅[存储定价详细信息][存储定价详细信息]。

### 为存储帐户指定复制设置

1.  在 [Azure 管理门户][Azure 管理门户]中，单击**“存储”**，然后单击你的存储帐户名称以显示仪表板。
...
在[管理门户][管理门户]中，可使用仪表板或**“存储”**页上的**“管理密钥”**查看、复制和重新生成用于访问 Blob、表和队列服务的存储访问密钥。

### 复制存储访问密钥

你可以使用**“管理密钥”**复制要在连接字符串中使用的存储访问密钥。连接字符串需要在进行身份验证时使用存储帐户名称和密钥。有关配置连接字符串以访问 Azure 存储服务的信息，请参阅[配置连接字符串][配置连接字符串]。
...
2.  为你的存储帐户重新生成主访问密钥。在[管理门户][管理门户]中，从仪表板或“配置” 页，单击“管理密钥” 。单击主访问密钥下的**“重新生成”**，然后单击**“是”**确认你要生成新密钥。
