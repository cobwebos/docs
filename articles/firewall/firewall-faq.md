...
Azure 防火墙支持入站和出站筛选。 入站保护通常用于非 HTTP/S 协议。 例如 RDP、SSH 和 FTP 协议。 为了获得最佳入站 HTTP/S 保护，请使用 Web 应用程序防火墙（如[Azure Web 应用程序防火墙 （WAF））。](../web-application-firewall/overview.md)
...
若要安全访问 PaaS 服务，我们建议使用服务终结点。 可以选择在 Azure 防火墙子网中启用服务终结点，并在连接的分支虚拟网络中禁用它们。 这样，您就受益于这两个功能：服务终结点安全性和所有流量的中央日志记录。
...
能，可以在中心虚拟网络中使用 Azure 防火墙来路由和筛选两个分支虚拟网络之间的流量。 每个分支虚拟网络中的子网必须有一个 UDR，指向 Azure 防火墙作为此方案正常工作的默认网关。
...
支持强制隧道。 有关详细信息，请参阅[Azure 防火墙强制隧道（预览）。](forced-tunneling.md) 
...
## <a name="when-configuring-dnat-for-inbound-internet-network-traffic-do-i-also-need-to-configure-a-corresponding-network-rule-to-allow-that-traffic"></a>在为入站 Internet 网络流量配置 DNAT 时，我是否需要配置相应的网络规则以允许该流量？
...
## <a name="how-does-connection-draining-work"></a>连接排水如何工作？

对于任何计划维护，连接耗尽逻辑会正常更新后端节点。 Azure 防火墙等待 90 秒，以便关闭现有连接。 如果需要，客户端可以自动重新建立与另一个后端节点的连接。
...
Azure 防火墙的初始吞吐量容量为 2.5 - 3 Gbps，并且扩展到 30 Gbps。 它根据 CPU 使用率和吞吐量进行扩展。 请与支持部门联系，以提高防火墙的吞吐量。

## <a name="how-long-does-it-take-for-azure-firewall-to-scale-out"></a>Azure 防火墙横向扩展需要多长时间？

Azure 防火墙需要 5 到 7 分钟才能横向扩展。如果发生需要更快自动缩放的突发，请与支持部门联系，以提高防火墙的初始吞吐量容量。

在测试防火墙自动缩放时，应考虑以下几点：

- 单个 TCP 流量性能限制为 1.4 Gbps。 因此，性能测试需要建立多个 TCP 流。
- 性能工具必须不断建立新的连接，以便它们与扩展的后端防火墙实例连接。 如果测试在开始时建立一次连接，则这些连接将仅与初始后端实例连接。 即使防火墙扩展，您也不会看到任何性能提高，因为连接与初始实例相关联。
...
## <a name="can-i-exclude-a-fqdn-or-an-ip-address-from-azure-firewall-threat-intelligence-based-filtering"></a>是否可以从基于 Azure 防火墙威胁智能的筛选中排除 FQDN 或 IP 地址？

可以，您可以使用 Azure PowerShell 执行此操作：
