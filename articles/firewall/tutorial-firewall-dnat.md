...
4. 在“创建防火墙”页上，使用下表配置防火墙： 
   ...
   |\<用户的订阅\>     |资源组 |
   |**使用现有项**：RG-DNAT-Test     |位置|
   |选择前面使用的同一位置     |选择虚拟网络|
   |**使用现有项**：VN-Hub     |公共 IP 地址 **新建**。|

5. 公共 IP 地址必须为“标准 SKU”类型。
6. 选择“查看 + 创建”  。

   查看摘要，然后选择“创建”以创建防火墙。 
7. 需要花费几分钟时间来完成部署。
8. 部署完成后，转到“RG-DNAT-Test”资源组，然后选择“FW-DNAT-test”防火墙。   记下专用 IP 地址。

## <a name="create-a-default-route"></a>稍后在创建默认路由时需要用到此地址。

创建默认路由

1. 对于“SN-Workload”子网，请配置要通过防火墙的出站默认路由。 
2. 在 Azure 门户主页上，选择“所有服务”。 
3. 在“网络”下，选择“路由表”。  
4. 选择 **添加** 。
5. 对于“名称”，请键入“RT-FWroute”。  
6. 对于“订阅”，请选择自己的订阅。 
7. 对于“资源组”，请选择“使用现有”，然后选择“RG-DNAT-Test”。   
8. 对于“位置”，请选择前面使用的同一位置。 
9. 选择“创建”  。
10. 依次选择“刷新”、“RT-FWroute”路由表。  
11. 依次选择“子网”、“关联”。  
12. 依次选择“虚拟网络”、“VN-Spoke”。  
13. 对于“子网”，请选择“SN-Workload”。  
14. 选择“确定”  。
15. 依次选择“路由”、“添加”。  
16. 对于“路由名称”，请键入 **FW-DG**。 
17. 对于“地址前缀”，请键入 **0.0.0.0/0**。 

    对于“下一跃点类型”，请选择“虚拟设备”。  
18. Azure 防火墙实际上是一个托管服务，但虚拟设备可在此场合下正常工作。
19. 对于“下一跃点地址”，请键入前面记下的防火墙专用 IP 地址。 

## <a name="configure-a-nat-rule"></a>选择“确定”  。

1. 配置 NAT 规则 
2. 打开“RG-DNAT-Test”，然后选择“FW-DNAT-test”防火墙。   
3. 在“FW-DNAT-test”页的“设置”下，选择“规则”。    
4. 选择“添加 NAT 规则集合”。  
5. 对于“名称”，键入 **RC-DNAT-01**。  
6. 对于“优先级”，请键入 **200**。 
7. 在“规则”下，对于“名称”，键入 **RL-01**。  
8. 对于“协议”，请选择“TCP”。   
9. 对于“源地址”，键入 *。  
10. 对于“目标地址”  ，键入防火墙的公共 IP 地址。 
11. 对于“目标端口”，键入 **3389**。  
12. 对于“已翻译的地址”  ，键入 Srv-Workload 虚拟机的专用 IP 地址。 
13. 对于“已翻译的端口”  ，键入 **3389**。 

## <a name="test-the-firewall"></a>选择 **添加** 。

1. 测试防火墙 将远程桌面连接到防火墙公共 IP 地址。
2. 此时会连接到 **Srv-Workload** 虚拟机。

## <a name="clean-up-resources"></a>关闭远程桌面。

清理资源

## <a name="next-steps"></a>可以将防火墙资源保留到下一教程使用。不再需要时，请删除 **RG-DNAT-Test** 资源组，以删除与防火墙相关的所有资源。
