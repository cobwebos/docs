<properties linkid="develop-media-services-tutorials-smooth-streaming-windows-store-app" urlDisplayName="Smooth Streaming Windows Store App" pageTitle="平滑流式处理 Windows 应用商店应用程序 - Azure .NET 教程" metaKeywords="" description="了解如何使用 Azure Media Services 来创建一个 C# Windows 应用商店应用程序，该应用程序包含一个用于播放平滑流内容的 XML MediaElement 控件。" metaCanonical="" services="media-services" documentationCenter="" title="How to Build a Smooth Streaming Windows Store Application" authors="" solutions="" manager="" editor="" />
...
可从 MSDN 开发人员代码示例（代码库）下载每一课后生成的解决方案：[第 1 课](http://code.msdn.microsoft.com/Smooth-Streaming-Client-0bb1471f "A Simple Windows 8 Smooth Streaming Media Player")、[第 2 课](http://code.msdn.microsoft.com/A-simple-Windows-8-Smooth-ee98f63a "A Simple Windows 8 Smooth Streaming Media Player with a Slider Bar Control")、[第 3 课](http://code.msdn.microsoft.com/A-Windows-8-Smooth-883c3b44 "A Windows 8 Smooth Streaming Media Player with Stream Selection")和[第 4 课](http://code.msdn.microsoft.com/A-Windows-8-Smooth-aa9e4907 "A Windows 8 Smooth Streaming Media Player with Track Selection")。
...
有关开发 Windows 应用商店应用程序的详细信息，请参阅[开发适用于 Windows 8 的极佳应用程序](http://msdn.microsoft.com/windows/apps/br229512.aspx)。 
本课包含以下过程：
...
4.	在 <strong>mediaElement AdaptiveSourceOpened</strong> 方法的末尾，添加以下代码以订阅事件：
...
5.	在 **MainPage** 类的末尾，添加以下代码：
	...
	**注意：**CoreDispatcher 用于从非 UI 线程对 UI 线程进行更改。如果调度程序线程出现瓶颈，开发人员可以选择使用想要更新的 UI 元素提供的调度程序。例如：
