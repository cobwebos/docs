...
1. 从 Azure Databricks 工作区中，导航到 Databricks 群集。
    ...
    ![打开数据块群集](./media/connect-databricks-excel-python-r/open-databricks-cluster.png "打开数据块群集")

2. 在“配置”**** 选项卡下，单击“JDBC/ODBC”**** 选项卡并复制“服务器主机名”**** 和“HTTP 路径”**** 的值。 需要使用这些值来完成本文中的步骤。

    ![获取数据砖块配置](./media/connect-databricks-excel-python-r/get-databricks-jdbc-configuration.png "获取数据砖块配置")

3. 在计算机上，根据所用的应用程序启动 **ODBC 数据源**应用程序（32 位或 64 位）。 若要从 Excel 进行连接，请使用 32 位版本。 若要从 R 和 Python 进行连接，请使用 64 位版本。

    ![启动 ODBC](./media/connect-databricks-excel-python-r/launch-odbc-app.png "启动 ODBC 应用程序")

4. 在“用户 DSN”**** 选项卡下，单击“添加”****。 在“创建新数据源”对话框中，选择“Simba Spark ODBC 驱动程序”，然后单击“完成”。************

    ![启动 ODBC](./media/connect-databricks-excel-python-r/add-new-user-dsn.png "启动 ODBC 应用程序")
...
5. 在“Simba Spark ODBC 驱动程序”**** 对话框中，提供以下值：
    ...
    * 单击“测试”**** 以测试到 Azure Databricks 的连接。 单击“确定”**** 保存配置。
...
3. 如果系统提示输入凭据，对于用户名，输入 **token**。 对于密码，提供从 Databricks 工作区中检索到的令牌值。
    ...
    ![为数据砖块提供凭据](./media/connect-databricks-excel-python-r/excel-databricks-token.png "选择 DSN")

4. 从导航器窗口中，选择 Databricks 中要加载到 Excel 的表，然后单击“加载”。**** 

    ![将 dta 加载到 Excel 中](./media/connect-databricks-excel-python-r/excel-load-data.png "将 dta 加载到 Excel 中")
...
* 如果将适用于桌面的 RStudio 用作 IDE，则还要从[https://aka.ms/rclient/](https://aka.ms/rclient/)安装 Microsoft R 客户端。 
...
* 要了解从中将数据导入 Azure 数据块的源，请参阅[Azure 数据块的数据源](/azure/databricks/data/data-sources/index)
