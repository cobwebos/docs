---
title: 教程：使用 Azure Databricks 针对流数据执行情绪分析 | Microsoft Docs
description: 了解如何将 Azure Databricks 与事件中心以及认知服务 API 配合使用，以便对实时流数据运行情绪分析。
...
本教程介绍如何使用 Azure Databricks 针对实时数据流运行情绪分析。 使用 Azure 事件中心设置实时数据引入系统。 使用 Spark 事件中心连接器将事件中心的消息用到 Azure Databricks 中。 最后，使用 Microsoft 认知服务 API 对流数据运行情绪分析。 
...
若要接收实时推文流，必须在 Twitter 中创建一个应用程序。 按步骤创建一个 Twitter 应用程序，并记下稍后需要在本教程中使用的值。
...
本教程介绍如何使用 [Microsoft 认知服务文本分析 API](../cognitive-services/text-analytics/overview.md) 对实时推文流运行情绪分析。 在使用这些 API 之前，必须在 Azure 上创建 Microsoft 认知服务帐户，并检索使用文本分析 API 所需的访问密钥。
...
若要运行该 Notebook，请按 **SHIFT + ENTER**。 会显示一个输出，如以下代码片段所示。 输出中的每个事件是已引入到事件中心的实时推文。 
...
就这么简单！ 现已成功使用 Azure Databricks 将实时数据流式传输到 Azure 事件中心，通过事件中心连接器使用了流数据，然后对流数据运行了情绪分析。
