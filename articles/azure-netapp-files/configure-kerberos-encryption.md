...
ms.locfileid: "87535325"
---
# <a name="configure-nfsv41-kerberos-encryption-for-azure-netapp-files"></a>为 Azure NetApp 文件配置 NFSv 4.1 Kerberos 加密

Azure NetApp 文件支持采用 AES-256 加密的 Kerberos 模式（krb5.conf、krb5i 和 krb5p）中的 NFS 客户端加密。 本文介绍将 NFSv 4.1 卷与 Kerberos 加密配合使用所需的配置。

## <a name="requirements"></a>要求

以下要求适用于 NFSv 4.1 客户端加密： 

* Active Directory 域服务（AD DS）连接以促进 Kerberos 票证 
* 为客户端和 Azure NetApp 文件创建 DNS A/PTR 记录 NFS 服务器 IP 地址
* Linux 客户端  
    本文为 RHEL 和 Ubuntu 客户端提供指导。  其他客户端将使用类似的配置步骤。 
* NTP 服务器访问  
    你可以使用其中一个常用 Active Directory 域控制器（AD DC）域控制器。
...
2. 选择 "**导出策略**"，与卷的所需访问级别和安全选项（kerberos 5、kerberos 5I 或 Kerberos 5p）相匹配。   
...
* 在两个级别对性能影响进行了测试：低并发性（低负载）和高并发性（i/o 和吞吐量上限）。  
* 测试了三种类型的工作负荷：  
    * 小型操作随机读取/写入（使用 FIO）
    * 大型操作顺序读取/写入（使用 FIO）
...
主要有两个方面：光源负载和上限。 以下列表描述了安全设置和方案按方案列出的性能影响。 对安全参数进行所有比较 `sec=sys` 。
