...
> [!IMPORTANT] 
> 在注册 Azure NetApp 文件资源提供程序之前，您必须收到 Azure NetApp 文件团队的电子邮件，确认已被授予对服务的访问。 

在本文中，了解如何注册 Azure NetApp 文件，以便开始使用该服务。

## <a name="submit-a-waitlist-request-for-accessing-the-service"></a><a name="waitlist"></a>提交访问服务的等待列表请求

1. 通过[Azure NetApp 文件等待列表提交页](https://forms.office.com/Pages/ResponsePage.aspx?id=v4j5cvGGr0GRqy180BHbR8cq17Xv9yVBtRCSlcD_gdVUNUpUWEpLNERIM1NOVzA5MzczQ0dQR1ZTSS4u)提交访问 Azure NetApp 文件服务的等待列表请求。 

    等待列表注册不保证立即访问服务。 

2. 等待 Azure NetApp 文件团队的正式确认电子邮件，然后再继续执行其他任务。 
...
> [!NOTE] 
> 即使没有获得服务访问权限，您也能成功注册 NetApp 资源提供商。 但是，如果没有访问授权，创建 NetApp 帐户或任何其他 Azure NetApp 文件资源的任何 Azure 门户或 API 请求都将被拒绝，并出现以下错误：  
>
...
3. 在 Azure Cloud Shell 控制台中，输入以下命令来验证你的订阅是否已列入允许列表：
   ...
   `<SubID>` 是你的订阅 ID。
    ...
    如果未看到功能名称`Microsoft.NetApp/ANFGA`，则无法访问该服务。 停止此步骤。 按照["提交访问服务的等待列表请求](#waitlist)"中的说明进行操作，以请求服务访问，然后再继续。 
