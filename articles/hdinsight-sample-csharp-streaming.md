<properties
	pageTitle="HDInsight 中的 C# 流式处理单词计数 Hadoop 示例 | Azure"
...
> [AZURE.NOTE] 本教程中的步骤仅适用于基于 Windows 的 HDInsight 群集。有关基于 Linux 的 HDInsight 群集流式处理的示例，请参阅[开发适用于 HDInsight 的 Python 流式处理程序](hdinsight-hadoop-streaming-python)。
...
- 一个 Azure 帐户。有关注册帐户的选项，请参阅 [azure-trial](/pricing/1rmb-trial/) 页。

- 设置的 HDInsight 群集。有关可用于创建这类群集的不同方法的说明，请参阅[配置 HDInsight 群集](hdinsight-provision-clusters)。

- Azure PowerShell。必须对它进行配置，使其可用于你的帐户。有关如何进行此配置的说明，请参阅[安装和配置 Azure PowerShell][powershell-install-configure]。
...
<h2><a id="java-code"></a>Hadoop 流式处理的 C# 代码</h2>
