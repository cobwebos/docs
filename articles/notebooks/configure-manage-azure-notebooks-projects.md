...
Azure Notebooks 中的项目仪表板，可通过它管理文件和配置项目的特性：
...
- **Docker-compose.override.yml**：在第二个下拉列表中，选择用于使用 conda 环境的 Python 项目的*docker-compose.override.yml*文件。
   ...
   > [!WARNING]
   > 由于这是一个预览服务，因此当前存在一个已知问题，该设置不会 `Environment.yml` 按预期方式应用到你的项目。 中的项目和 Jupyter 笔记本不会加载当前指定的环境文件。
