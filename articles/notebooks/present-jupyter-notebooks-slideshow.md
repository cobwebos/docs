...
1. 对于每个单元格，选择五种类型之一：
    ...
    | Fragment | 在使用向下导航箭头时，单元格内容出现在上一张幻灯片或子幻灯片的上下文中（在使用向上箭头时，会删除片段）。 可以将片段与代码单元格一起使用，使该代码出现在幻灯片中，也可以使用多个片段使文本项目符号逐个出现（请参阅下一节中的示例）。 因为片段基于当前幻灯片而构建，所有多余碎片在浏览器窗口底部不可见。 |
    | 跳过 | 单元格不显示在幻灯片放映中。 |
    | 备注 | 单元格包含不显示在幻灯片放映中的演讲者备注。 |
...
1. 在代码单元格上，使用播放按钮运行代码；输出会出现在幻灯片上：
    ...
    > [!Tip]
    > 单元格输出被视为幻灯片放映中的单元格的一部分。 如果笔记本或幻灯片放映视图中运行单元格，则输出也会出现在其他视图中。 若要清除输出，请对当前单元) 格使用 "当前输出的**单元格** (明文" 命令; 对于所有单元格，请使用所有  >  **Current Outputs**  >  **Clear** **Cell**  >  **输出**  >  **清除** () 。
