...
*无服务器模式*：Azure SignalR 服务仅接受客户端连接的模式。 不允许服务器连接。
...
从理论上讲，Azure SignalR 服务容量受计算资源（CPU、内存和网络）的限制。 例如，与 Azure SignalR 服务建立更多连接会导致服务使用更多的内存。 对于较大的消息流量（例如，每条消息大于 2,048 字节），Azure SignalR 服务需要消耗更多的 CPU 周期来处理流量。 同时，Azure 网络带宽也对最大流量施加限制。

传输类型是影响性能的另一个因素。 三种类型为 [WebSocket](https://en.wikipedia.org/wiki/WebSocket)、[Server-Sent-Event](https://en.wikipedia.org/wiki/Server-sent_events) 和 [Long-Polling](https://en.wikipedia.org/wiki/Push_technology)。 
...
另一个因素是协议：JSON和[消息包](https://msgpack.org/index.html)。 MessagePack 较小，其传送速度比 JSON 要快。 不过，MessagePack 可能无法提高性能。 Azure SignalR 服务的性能对协议不敏感，因为在将消息从客户端转发到服务器或反向转发时，它不会对消息有效负载进行解码。
...
|     Broadcast             | 单元 1 | 单元 2 | 单元 5  | 单元 10 | 单元 20 | 单元 50  | 单元 100 |
|---------------------------|-------|-------|--------|--------|--------|---------|---------|
| 连接               | 1,000 | 2,000 | 5,000  | 10,000 | 20,000 | 50,000  | 100,000 |
...
- *消息大小*： 单个消息的大小（平均值）。 小于 1,024 字节的小型消息的性能影响与 1,024 字节消息类似。

- *发送间隔*：发送一条消息的时间。 通常发送每条消息需要 1 秒，即，每秒发送一条消息。 间隔越短，在一个时间段内发送的消息越多。 例如，每条消息花费 0.5 秒意味着每秒可发送两条消息。

- *连接*：每个层的 Azure SignalR 服务的已提交最大阈值。 如果连接数进一步增加，将遭到连接限制。
...
| Broadcast  | 消息大小 | 每秒入站消息数 | 连接 | 发送间隔 |
|---|---------------------|--------------------------|-------------|-------------------------|
...
- **小组**：每个组有 10 个连接。 组数 = 最大连接数 / 10。 例如，对于单元 1，如果连接计数为 1,000，则组数为 1000 / 10 = 100。

- **大组**：组数总是 10。 组成员计数 = 最大连接计数 / 10。 例如，对于单元 1，如果连接计数为 1,000，则每个组的成员数为 1000 / 10 = 100。
...
- 客户端 VM 大小：标准DDS2V2（2 个 vCPU，7G 内存）

- 应用服务器 VM 大小：标准F4sV2（4 个 vCPU，8G 内存）
