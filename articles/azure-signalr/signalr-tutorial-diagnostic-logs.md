...
connectionId | 连接的标识
transportType | 连接的传输类型。 允许的值包括： `Websockets` \| `ServerSentEvents` \|`LongPolling`
connectionType | 连接的类型。 允许值包括：`Server` \| `Client`。 `Server`：从服务器端建立的连接；`Client`：从客户端建立的连接
userId | 用户的标识
message | 日志事件的详细消息
...
3. 发布详细信息，尽可能具体：例如，应用程序服务器不发送消息、客户端连接断开等
4. 从服务器/客户端以及其他可能有用的材料收集的日志
5. [可选] 重现代码

> [!NOTE]
> 如果在 GitHub 中打开问题，请保持敏感信息（例如资源 ID、服务器/客户端日志）的私密性，仅私下发送给 Microsoft 组织中的成员。