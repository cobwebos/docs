...
Azure 使用的证书可以包含公钥。 证书具有指纹，它提供了一种可对证书进行明确识别的方法。 该指纹用于在 Azure [配置文件](cloud-services-configure-ssl-certificate-portal.md)中识别云服务应使用的证书。 
...
此实用工具已弃用，此处不再进行记录。 有关详细信息，请参阅[此 MSDN 文章](/windows/desktop/SecCrypto/makecert)。
