...
## 替代体系结构：网站和 Web 作业
本教程演示如何在 Azure 云服务中运行前端和后端。一种替代方法是在 [Azure 网站](/services/web-sites/)中运行前端，并为后端使用 [WebJobs](http://go.microsoft.com/fwlink/?LinkId=390226) 功能（目前以预览版提供）。有关如何使用 WebJobs 的教程，请参阅 [Azure WebJobs SDK 入门](../app-service-web/websites-dotnet-webjobs-sdk-get-started.md)。有关如何选择最适合你的方案的服务信息，请参阅 [Azure 网站、云服务和虚拟机比较](../app-service-web/choose-web-site-cloud-service-vm.md)。
...
在没有 Azure 订阅的情况下可以本地运行应用，但若要将应用程序部署到云中，则需要一个 Azure 订阅。如果没有帐户，可以[激活 MSDN 订户权益](/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A55E3C668)或[注册免费试用帐户](/pricing/free-trial/?WT.mc_id=A55E3C668)。
...
6. 如果你使用 Visual Studio 2015，请在 ContosoAdsWeb 项目的应用程序 *Web.config* 文件以及 ContosoAdsCloudService 项目的 *ServiceConfiguration.Local.cscfg* 文件中，更改 SQL Server 连接字符串。对于每种情况，请将“(localdb)\\v11.0”更改为“(localdb)\\MSSQLLocalDB”。
7. 按 Ctrl+F5 运行应用程序。
...
1. 在浏览器中，打开 [Azure 经典门户](http://manage.windowsazure.com)。
2. 单击“新建”>“计算”>“云服务”>“快速创建”。
...
1. 在 [Azure 经典门户](http://manage.windowsazure.com)中，依次单击“新建”>“数据服务”>“SQL 数据库”>“快速创建”。
...
1. 在 [Azure 经典门户](http://manage.windowsazure.com)中，依次单击“新建”>“数据服务”>“存储”>“快速创建”。
...
1. 在 ContosoAdsWeb 项目中，为应用程序 *Web.config* 文件打开 *Web.Release.config* 转换文件，删除包含 `<connectionStrings>` 元素的注释块，并在其原位置粘贴下面的代码。
    ...
    保持文件打开进行编辑。
2. 在 [Azure 经典门户](http://manage.windowsazure.com)中，依次单击左侧窗格中的“SQL 数据库”、你在本教程中创建的数据库、“仪表板”选项卡和“显示连接字符串”。
...
3. 在 *Web.Release.config* 转换文件中，删除 `{connectionstring}` 并将 Azure 经典门户中的 ADO.NET 连接字符串粘贴在其原位置。
...
8. 单击“设置”选项卡。
9. 将“服务配置”更改为“云”。
10. 选择 `ContosoAdsDbConnectionString` 设置的“值”字段，然后粘贴你从本教程上一部分复制的连接字符串。
...
4. 在“创建存储连接字符串”对话框中，单击“你的订阅”，选择你以前创建的存储帐户，然后单击“确定”。如尚未登录，将提示您输入 Azure 帐户凭据。
...
3. 在向导的“设置”步骤中，单击“下一步”。
    ...
    本教程可以使用“高级”选项卡中的默认设置。有关高级选项卡的信息，请参阅[发布 Azure 应用程序向导](http://msdn.microsoft.com/library/hh535756.aspx)。
4. 在“摘要”步骤中，单击“发布”。
...
> [!NOTE]
> 完成测试后，删除或停止云服务。即使你不使用云服务，也会产生费用，因为要为它保留虚拟机资源。如果保持运行云服务，找到你 URL 的任何人都可以创建和查看广告。在 [Azure 经典门户](http://manage.windowsazure.com)中，转到云服务的“仪表板”选项卡，然后单击页面底部的“删除”按钮。如果你只是想要暂时防止其他人访问站点，请单击“停止”。在这种情况下，会继续产生费用。当你不再需要 SQL 数据库和存储帐户时，可以遵循类似的过程将其删除。
> 
> 
...
9. 在“添加新项目”对话框中，选择左侧窗格中“Visual C#”下面的“Windows”，然后单击“类库”模板。
...
2. 在窗口顶部，选择“更新”。
3. 查找 *WindowsAzure.Storage* 包，如果它在列表中，请选择它并选择要在其中更新它的 Web 项目和辅助项目，然后单击“更新”。
