...
但是，您也可以在自己的域名（如**contoso.com）** 上公开应用程序。 此文章介绍了如何保留或配置云服务 Web 角色的自定义域名称。
...
CNAME 记录将*特定*域（如**contoso.com**或**\.www contoso.com）** 映射到规范的域名。 在这种情况下，规范域名是 Azure 托管应用程序的 **[myapp].cloudapp.net** 域名。 CNAME 创建后，将为 **[myapp].cloudapp.net** 创建一个别名。 CNAME 条目会自动解析为 **[myapp].cloudapp.net** 服务的 IP 地址，因此，如果该云服务的 IP 地址发生更改，无需执行任何操作。
...
*记录*将域（如**contoso.com**或**www\.contoso.com）***或通配符域*（如**\*.contoso.com）** 映射到 IP 地址。 使用 Azure 云服务时，映射到该服务的虚拟 IP。 因此，A 记录在 CNAME 记录上的主要好处是，您可以有一个使用通配符的条目，\*例如 **.contoso.com**，该条目将处理多个子域（如**mail.contoso.com、login.contoso.com**或**www\.contso.com）** 的请求。 **login.contoso.com**

> [!NOTE]
> 由于 A 记录映射到静态 IP 地址，因此它无法自动解析云服务 IP 地址的更改。 云服务使用的 IP 地址是在您第一次部署到空插槽（生产或暂存）时分配的。如果删除该槽的部署，Azure 将释放 IP 地址，并且将来对该槽的任何部署都可能获得新的 IP 地址。
> 
...
4. 如果要为 www customdomain.com 创建别名，还必须提供 CNAME 的域或子域别名，例如**\.www。** **www** 如果要为根域创建别名，它可能在注册商的 DNS 工具中列为'**\@** 符号'符号。
5. 然后，必须提供一个规范主机名，在此示例中它是应用程序的“cloudapp.net”**** 域。

例如，以下 CNAME 记录将从**\.www contoso.com**转发到已部署应用程序的自定义域名**contoso.cloudapp.net**的所有流量：
...
> [!NOTE]
> **www\.contoso.com**访问者永远不会看到真正的主机（contoso.cloudapp.net），因此转发过程对最终用户不可见。
> 
...
4. 选择或输入将使用此 A 记录的域或子域。 例如，如果要为**www\.customdomain.com**创建别名，请选择**www。** 如果希望为所有子域创建通配符条目，请输入“*****”。 这将涵盖所有子域名，如**mail.customdomain.com、login.customdomain.com**和**login.customdomain.com****\.www customdomain.com。**

    如果要为根域创建 A 记录，它可能在注册商的 DNS 工具中**\@** 列为''符号。
