...
创建云服务时，Azure 会将其分配给 **cloudapp.net** 的子域。例如，如果你的云服务名为"contoso"，你的用户将能够访问类似 http://contoso.cloudapp.net 的 URL 上的应用程序。Azure 还会将分配一个虚拟 IP 地址。
...
CNAME 记录将特定域（例如 **contoso.com** 或 **www.contoso.com**）映射到规范域名。在这种情况下，规范域名是你的 Azure 托管应用程序的 **[myapp].cloudapp.net** 域名。CNAME 创建后，将为 **[myapp].cloudapp.net** 创建一个别名。CNAME 条目将自动解析为你的 **[myapp].cloudapp.net** 服务的 IP 地址，因此，如果该云服务的 IP 地址发生更改，你无需采取任何措施。
...
1. 使用下列方法之一找到分配给你的云服务的 **.cloudapp.net** 域名。
    ...
    * 安装并配置 [Azure Powershell](../install-configure-powershell.md)，然后使用以下命令：
...
> 上述示例仅适用于 **www** 子域的流量。因为无法为 CNAME 记录使用通配符，所以必须为每个域/子域创建一个 CNAME。如果希望将子域（例如 *.contoso.com）的流量定向到您的 cloudapp.net 地址，则可以在 DNS 设置中配置 **URL 重定向** 或 **URL 转发** 条目，或者创建一个 A 记录。
...
>[AZURE.WARNING]
Azure 中的 IP 地址默认为动态 IP 地址。你将很可能想使用[保留 IP 地址](../virtual-network/virtual-networks-reserved-public-ip.md)以确保你的 IP 地址不会更改。
