...
CNAME 记录映射*特定*域，如**contoso.com**或**www\.contoso.com**，到规范域名。 在这种情况下，规范域名是 Azure 托管应用程序的 **[myapp].cloudapp.net** 域名。 CNAME 创建后，将为 **[myapp].cloudapp.net** 创建一个别名。 CNAME 条目会自动解析为 **[myapp].cloudapp.net** 服务的 IP 地址，因此，如果该云服务的 IP 地址发生更改，无需执行任何操作。

> [!NOTE]
> 某些域注册机构仅允许你将使用 CNAME 记录，例如 www 时映射子域\.contoso.com，而非根名称，例如 contoso.com。 有关 CNAME 记录的详细信息，请参阅由注册机构提供的文档、[CNAME 记录上的 Wikipedia 条目](https://en.wikipedia.org/wiki/CNAME_record)或 [IETF 域名 - 实现和规范](https://tools.ietf.org/html/rfc1035)文档。

### <a name="a-record"></a>A 记录
*A*记录映射域，如**contoso.com**或**www\.contoso.com**，*或通配符域*如 **\*。 contoso.com**，为 IP 地址。 使用 Azure 云服务时，映射到该服务的虚拟 IP。 CNAME 记录相比，A 记录的主要优势是，您可以有一个使用通配符，如的条目\* **。 contoso.com**，它将处理多个子域的请求，如**mail.contoso.com**， **login.contoso.com**，或**www\.contso.com**。
...
4. 您还必须提供域或子域别名为 CNAME，如**www**如果你想要创建的别名**www\.customdomain.com**。 如果希望为根域创建别名，它可能在注册机构的 DNS 工具中以符号“ **\@** ”的形式列出。
5. 然后，必须提供一个规范主机名，在此示例中它是应用程序的“cloudapp.net”  域。

例如，以下 CNAME 记录将转发所有流量从**www\.contoso.com**到**contoso.cloudapp.net**，你已部署的应用程序的自定义域名：
...
> [!NOTE]
> 访问者**www\.contoso.com**将永远不会看到真正的主机 (contoso.cloudapp.net)，因此转发过程对最终用户不可见。
> 
...
4. 选择或输入将使用此 A 记录的域或子域。 例如，选择**www**如果你想要创建的别名**www\.customdomain.com**。 如果希望为所有子域创建通配符条目，请输入“*****”。 这将涵盖所有子域，例如**mail.customdomain.com**， **login.customdomain.com**，并**www\.customdomain.com**。
