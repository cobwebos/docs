...
TexConv 始终**只生成一个输出**文件。 它可以使用**多个输入**文件来组合的输出。 对于程序集，它还需要一个**通道映射**，该映射告诉它要从哪个输入文件中获取哪个通道（*红色、绿色、蓝色*或*Alpha*），并将其移到输出图像的哪个通道。
...
### <a name="usage-srgb--gamma-correction"></a>用法（sRGB/伽玛更正）
