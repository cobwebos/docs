...
标准 DDoS 防护在 DDoS 攻击持续期间通过 [Azure Monitor](../azure-monitor/overview.md) 公开丰富的遥测数据。 可以针对 DDoS 防护使用的任何 Azure Monitor 指标配置警报。 通过 Azure Monitor 诊断界面的高级分析，可与 Splunk （Azure 事件中心）、 Azure Monitor 日志和 Azure 存储集成日志记录。
