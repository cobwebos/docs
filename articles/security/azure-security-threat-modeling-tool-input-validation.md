...
## <a name="a-iddisable-xsltadisable-xslt-scripting-for-all-transforms-using-untrusted-style-sheets"></a><a id="disable-xslt"></a>使用不受信任样式表针对所有转换禁用 XSLT 脚本
...
## <a name="a-idout-sniffingaensure-that-each-page-that-could-contain-user-controllable-content-opts-out-of-automatic-mime-sniffing"></a><a id="out-sniffing"></a>确保可能包含用户可控内容的每个页面能够选择不使用自动 MIME 探查
...
## <a name="a-idxml-resolutionaharden-or-disable-xml-entity-resolution"></a><a id="xml-resolution"></a>强化或禁用 XML 实体解析
...
## <a name="a-idapp-verificationaapplications-utilizing-httpsys-perform-url-canonicalization-verification"></a><a id="app-verification"></a>使用 http.sys 的应用程序执行 URL 规范化验证
...
## <a name="a-idcontrols-usersaensure-appropriate-controls-are-in-place-when-accepting-files-from-users"></a><a id="controls-users"></a>确保接受用户的文件时实施适当的控制
...
| 适用的技术 | 泛型 |
| 属性              | 不适用  |
| 参考              | [不受限制的文件上载](https://www.owasp.org/index.php/Unrestricted_File_Upload)、[文件签名表](http://www.garykessler.net/library/file_sigs.html) |
| 步骤 | <p>上载的文件会给应用程序带来重大的风险。</p><p>许多攻击的第一步就是在要攻击的系统中放入一些代码。 然后，攻击者只需找到一种执行该代码的方法。 使用文件上载可以帮助攻击者实现这第一个步骤。 不受限制的文件上载的后果多种多样，包括完全接管系统、使文件系统或数据库过载、将攻击转移到后端系统，以及简单的篡改。</p><p>这种后果取决于应用程序对上载的文件的处理，尤其是该文件的存储位置。 服务器端缺少对文件上载的验证。 应该针对文件上载功能实施以下安全控制：</p><ul><li>文件扩展名检查（应该只接受有效的一组受允许文件类型）</li><li>文件大小上限</li><li>不应将文件上载到 webroot；位置应是非系统驱动器上的某个目录</li><li>应该遵守命名约定，使上载文件的名称具有某种随机性，防止文件覆盖</li><li>将文件写入磁盘之前，应扫描它是否包含病毒</li><li>确保验证文件名和其他任何元数据（例如文件路径）是否包含恶意字符</li><li>应检查文件格式签名，防止用户上载伪装的文件（例如，通过将扩展名更改为 txt 来上载 exe 文件）</li></ul>| 
...
## <a name="a-idtypesafeaensure-that-type-safe-parameters-are-used-in-web-application-for-data-access"></a><a id="typesafe"></a>确保在 Web 应用程序中使用类型安全的参数进行数据访问
...
## <a name="a-idbinding-mvcause-separate-model-binding-classes-or-binding-filter-lists-to-prevent-mvc-mass-assignment-vulnerability"></a><a id="binding-mvc"></a>使用单独的模型绑定类或绑定筛选列表来防止 MVC 大规模分配漏洞
...
## <a name="a-idrenderingaencode-untrusted-web-output-prior-to-rendering"></a><a id="rendering"></a>在呈现之前为不受信任的 Web 输出编码
...
## <a name="a-idtypemodelaperform-input-validation-and-filtering-on-all-string-type-model-properties"></a><a id="typemodel"></a>针对所有字符串类型的 Model 属性执行输入验证和筛选
...
## <a name="a-idrichtextasanitization-should-be-applied-on-form-fields-that-accept-all-characters-eg-rich-text-editor"></a><a id="richtext"></a>应该针对接受所有字符的窗体字段（例如 RTF 编辑器）应用净化
...
## <a name="a-idinbuilt-encodeado-not-assign-dom-elements-to-sinks-that-do-not-have-inbuilt-encoding"></a><a id="inbuilt-encode"></a>不要将 DOM 元素分配到没有内置编码的接收器
...
## <a name="a-idredirect-safeavalidate-all-redirects-within-the-application-are-closed-or-done-safely"></a><a id="redirect-safe"></a>验证应用程序中的所有重定向是否闭合且安全
...
## <a name="a-idstring-methodaimplement-input-validation-on-all-string-type-parameters-accepted-by-controller-methods"></a><a id="string-method"></a>针对 Controller 方法接受的所有字符串类型参数实施输入验证
...
## <a name="a-iddos-expressionaset-upper-limit-timeout-for-regular-expression-processing-to-prevent-dos-due-to-bad-regular-expressions"></a><a id="dos-expression"></a>针对正则表达式处理设置超时上限，防止由于正则表达式错误而出现 DoS
...
## <a name="a-idhtml-razoraavoid-using-htmlraw-in-razor-views"></a><a id="html-razor"></a>避免在 Razor 视图中使用 Html.Raw
...
| 属性              | 不适用  |
| 参考              | 不适用  |
| 步骤 | ASP.Net 网页 (Razor) 执行自动 HTML 编码。 嵌入式代码片段 (@块）列显的所有字符串会自动进行 HTML 编码。 但是，调用 `HtmlHelper.Raw` 方法时，将返回未经 HTML 编码的标记。 如果使用 `Html.Raw()` 帮助器方法，它会绕过 Razor 提供的自动编码保护。|
...
## <a name="a-idstored-procado-not-use-dynamic-queries-in-stored-procedures"></a><a id="stored-proc"></a>不要在存储过程中使用动态查询
...
## <a name="a-idvalidation-apiaensure-that-model-validation-is-done-on-web-api-methods"></a><a id="validation-api"></a>确保针对 Web API 方法执行模型验证
...
## <a name="a-idstring-apiaimplement-input-validation-on-all-string-type-parameters-accepted-by-web-api-methods"></a><a id="string-api"></a>针对 Web API 方法接受的所有字符串类型参数实施输入验证
...
## <a name="a-idtypesafe-apiaensure-that-type-safe-parameters-are-used-in-web-api-for-data-access"></a><a id="typesafe-api"></a>确保在 Web API 中使用类型安全的参数进行数据访问
...
## <a name="a-idsql-docdbause-parametrized-sql-queries-for-documentdb"></a><a id="sql-docdb"></a>对 DocumentDB 使用参数化 SQL 查询
...
## <a name="a-idschema-bindingawcf-input-validation-through-schema-binding"></a><a id="schema-binding"></a>通过架构绑定执行 WCF 输入验证
...
## <a name="a-idparametersawcf--input-validation-through-parameter-inspectors"></a><a id="parameters"></a>WCF - 通过参数检查器执行输入验证
