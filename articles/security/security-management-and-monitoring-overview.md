...
你的 Microsoft 云服务的安全性是你与 Microsoft 之间的一个合作关系和共担责任。 共担责任意味着 Microsoft 负责 Microsoft Azure 及其数据中心的物理安全（通过使用上锁的入口门、护栏和保安等安全保护措施）。 此外，Azure 还在软件层提供了强级别的云安全，以满足其要求苛刻的客户的安全、隐私及合规性需求。

你拥有自己的数据和标识，你需负责对其进行保护，同时确保你的本地资源的安全性以及你所控制的云组件的安全性。 Microsoft 向你提供安全控件和功能，以帮助你保护自己的数据和应用程序。 你对安全的责任取决于云服务的类型。
...
基于角色的访问控制 (RBAC) 为 Azure 资源提供精细的访问管理。 使用 RBAC，你可以仅授予用户执行其作业所需的访问权限。  RBAC 还可以帮助你确保在用户离开组织时他们无法访问云中的资源。
...
此外，如果有访问特权的用户帐户被泄露，这个缺口可能会影响你的总体云安全性。 Azure AD Privileged Identity Management 可通过降低特权的暴露时间和加强对使用情况的了解来帮助解决此风险。  
...
Azure Active Directory (AD) 标识保护提供了可疑登录活动和潜在漏洞的统一视图来帮助保护你的企业。 标识保护基于诸如暴力攻击、泄漏凭据，以及从陌生位置和受感染设备登录等信号，检测用户和特权（管理）标识的可疑登录活动。

通过提供通知和建议的补救措施，标识保护有助于实时降低风险。 它会计算用户风险严重性，并且你可以配置基于风险的策略，以自动保护应用程序访问免受将来的威胁。
...
安全中心可通过以下方式帮助你优化和监视 Azure 资源的安全：

* 根据公司的安全需求和每个订阅中的应用程序类型或数据敏感度，使你能够为 Azure 订阅资源定义策略。
