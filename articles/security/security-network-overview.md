...
[强制隧道](https://www.petri.com/azure-forced-tunneling)是一种机制，可用于确保不允许你的服务启动到 Internet 上的设备的连接。 请注意，这不同于接受传入连接然后对其作出响应。 前端 Web 服务器需要响应来自 Internet 主机的请求，因此允许源自 Internet 的流量传入到这些 Web 服务器，而且允许这些 Web 服务器作出响应。
...
全局负载均衡可以为你提供这两个优点。 在 Azure 中，你可以使用 Azure 流量管理器获得全局负载均衡的优点。
...
* 内部名称解析 – 内部名称解析可供 Azure 虚拟网络、本地网络或两者上的服务使用。 用于内部名称解析的名称无法通过 Internet 访问。 为获得最佳的安全性，重要的是你的内部名称解析方案对于外部用户不可访问。
