...
ms.locfileid: "90059142"
---
# <a name="security-control-backup-and-recovery"></a>安全控制：备份和恢复
...
确保在发生意外事件后，备份系统和数据以保持业务连续性。 这应该是针对恢复点目标 (RPO) 和恢复时间目标 (RTO) 的任何目标的指导。
...
对于使用 Azure 备份的本地备份，将使用所提供的密码提供静态加密。 对于常规 Azure 服务备份，将使用 Azure 平台管理的密钥自动对备份数据进行加密。 你可以选择使用客户托管密钥加密备份。 在这种情况下，请确保密钥保管库中此客户托管的密钥也在备份范围内。 
...
- [事件准备](/) azure/云采用-框架/组织/云-安全-事件-准备
