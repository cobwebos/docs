...
| Azure Cosmos DB | <ul><li>[尽可能使用资源令牌连接到 Azure Cosmos DB](#resource-docdb)</li></ul> |
...
## <a id="sequential-logic"></a>处理业务逻辑流时有序执行步骤顺序
...
## <a id="rate-enumeration"></a>实施频率限制机制来防止枚举
...
## <a id="enumerable-browsing"></a>确保不可通过强行浏览来枚举或访问内容和资源
...
## <a id="sendonly-sas"></a>使用拥有仅限发送权限的 SAS 密钥来生成设备令牌
...
## <a id="sas-minimum-permissions"></a>使用拥有所需最低权限的 SAS 密钥连接到事件中心
...
| 属性              | 不适用  |
| 参考              | 不适用  |
| 步骤 | 资源令牌与 Cosmos DB 权限资源关联，可捕获数据库用户与该用户对某个特定 Cosmos DB 应用程序资源（例如，集合、文档）的权限之间的关系。 如果无法信任客户端（例如，移动或桌面客户端等最终用户应用程序）对主密钥或只读密钥的处理，请始终使用资源令牌访问 Cosmos DB。通过可以安全存储这些密钥的后端应用程序使用主密钥或只读密钥。|
...
## <a id="cluster-rbac"></a>使用 RBAC 限制客户端对群集操作的访问
...
## <a id="rbac-azure-manager"></a>使用 Azure Resource Manager 对 Azure 存储帐户启用基于角色的访问控制 (RBAC)
...
## <a id="rooting-detection"></a>实施隐式越狱或 root 检测
