...
* 最新 PowerShell 版本。
   ...
   后至少具有 PowerShell 5.0 中，您可以继续通过中的说明安装最新版本[安装 Azure PowerShell](/powershell/azure/install-az-ps)。
...
1. 获取所有可能的 Azure 位置并将名称添加到可以在后面的步骤中使用的变量：
    ...
    如果输入`$locations`此时，您看到位置名称而无需返回 Get AzLocation 的其他信息。
