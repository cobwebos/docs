...
FIDO2 在用作身份验证器和公钥/私钥加密的窗体因数之间提供了一个抽象层，用于启用内置平台验证器（如 Windows Hello 和安全密钥）以解析为私钥，并提供可用作标识符的公钥来访问外部资源。 FIDO2 安全密钥配备有自己的内置安全 enclave，用于存储私钥，并要求生物识别或 PIN 解锁。 凭据不能在服务中重复使用、重播或共享，并且不受网络钓鱼和 MiTM 攻击或服务器泄露的限制。
...
* 部署 Windows Hello 企业版（1909：保持最新）。
