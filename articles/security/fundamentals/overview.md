...
### <a name="features-to-secure-data-and-application"></a>保护数据和应用程序的功能
根据云服务模型，负责管理应用程序或服务的安全的人员需承担各种不同的责任。 Azure 平台中提供的功能可帮助用户通过内置功能以及可部署到 Azure 订阅中的合作伙伴解决方案来履行这些职责。

内置功能分为六个功能领域：操作、应用程序、存储、网络、计算和标识。 有关 Azure 平台中有关这六个区域中可用特性和功能的其他详细信息，请访问摘要信息。

## <a name="operations"></a>操作
本部分提供了关于安全操作中主要特性的其他信息以及有关这些功能的摘要信息。

### <a name="security-and-audit-dashboard"></a>“安全和审核”仪表板
[安全和审核解决方案](../../security-center/security-center-intro.md)借助[内置搜索查询](https://blogs.technet.microsoft.com/msoms/2016/01/21/easy-microsoft-operations-management-suite-search-queries/)找到需要关注的重要问题，从而提供有关组织的 IT 安全态势的全面观点。 [安全和审核](https://technet.microsoft.com/library/mt484091.aspx)仪表板是 Azure 监视器日志中与安全性相关的一切内容的主屏幕。 它提供计算机安全状态的高级洞见。 还允许查看过去 24 小时、7 天或任何自定义时间范围的所有事件。
...
[Azure 资源管理器](../../azure-resource-manager/management/deployment-models.md)使您能够作为一个组处理解决方案中的资源。 可以通过一个协调的操作为解决方案部署、更新或删除所有资源。 可以使用 [Azure 资源管理器模板](https://blogs.technet.microsoft.com/canitpro/2015/06/29/devops-basics-infrastructure-as-code-arm-templates/)来完成部署，该模板适用于测试、过渡和生产等不同环境。 Resource Manager 提供安全、审核和标记功能，以帮助你在部署后管理资源。

基于 Azure 资源管理器模板的部署因其标准的安全控制设置，有助于提高 Azure 中部署的解决方案的安全性，并且还可以集成到基于标准化模板的部署中。 这样可以降低手动部署期间可能发生的安全配置错误风险。

### <a name="application-insights"></a>Application Insights
[应用程序见解](https://docs.microsoft.com/azure/application-insights/)是面向 Web 开发人员的可扩展的应用程序性能管理 （APM） 服务。 用户可以使用 Application Insights 监视实时 Web 应用程序并自动检测性能异常。 Application Insights 内含强大的分析工具，有助于诊断问题并了解用户在应用中实际执行的操作。 它在应用程序运行时全程进行监视，包括测试期间以及发布或部署之后。
...
[Azure 监视器日志](https://azure.microsoft.com/documentation/services/log-analytics/)– 除了 Azure 资源外，还为本地和第三方基于云的基础结构（如 AWS）提供 IT 管理解决方案。 Azure 监视器中的数据可以直接路由到 Azure 监视器日志，以便您可以在一个位置查看整个环境的指标和日志。

Azure 监视器日志是取证和其他安全分析的有用工具，因为该工具使您能够使用灵活的查询方法快速搜索大量与安全相关的条目。 此外，本地[防火墙和代理日志可以导出到 Azure 中，并使用 Azure 监视器日志进行分析。](../../log-analytics/log-analytics-agent-windows.md)

### <a name="azure-advisor"></a>Azure 顾问
[Azure 顾问](../../advisor/index.yml)是一种个性化的云顾问，可帮助优化 Azure 部署。 它分析资源配置和使用情况遥测数据。 然后，它推荐解决方案，以帮助提高资源[的性能](../../advisor/advisor-performance-recommendations.md)、[安全性](../../advisor/advisor-security-recommendations.md)和[高可用性](../../advisor/advisor-high-availability-recommendations.md)，同时寻找[机会减少 Azure 的总体支出](../../advisor/advisor-cost-recommendations.md)。 Azure 顾问提供安全建议，可显著提高在 Azure 中部署的解决方案的总体安全状况。 这些建议来自于 [Azure 安全中心](../../security-center/security-center-intro.md)执行的安全分析。

### <a name="azure-security-center"></a>Azure 安全中心
[安全中心](../../security-center/security-center-intro.md)通过提高 Azure 资源的安全性并控制威胁，帮助您预防、检测和响应威胁。 它提供 Azure 订阅之间的集成安全监视和策略管理，帮助检测可能被忽略的威胁，且适用于广泛的安全解决方案生态系统。

此外，安全中心通过提供单个仪表板来显示可立即执行的警报和建议，从而有助于安全操作。 通常，您可以在安全中心控制台内单击一下来修复问题。
...
应用服务 Web 应用为 Web 服务器和 Web 应用程序中的日志记录信息提供诊断功能。 这些在逻辑上被分离为[Web服务器诊断](../../app-service/troubleshoot-diagnostic-logs.md)和[应用程序诊断](https://technet.microsoft.com/library/hh530058(v=sc.12).aspx)。 Web 服务器包括诊断和排查站点和应用程序这两大改进方面。
...
[应用程序诊断](../../app-service/troubleshoot-diagnostic-logs.md)允许您捕获 Web 应用程序生成的信息。 ASP.NET 应用程序可使用 [System.Diagnostics.Trace](https://msdn.microsoft.com/library/system.diagnostics.trace) 类将信息记录到应用程序诊断日志。 在应用程序诊断中，有两种主要类型的事件，即与应用程序性能相关的事件以及与应用程序故障和错误相关的事件。 故障和错误可以进一步分为连接性、安全性和故障问题。 故障问题通常与应用程序代码问题相关。
...
可以使用基于角色的访问控制 (RBAC) 来保护存储帐户。 对于想要强制实施数据访问的安全策略的组织来说，根据[了解](https://en.wikipedia.org/wiki/Need_to_know)和[最低特权](https://en.wikipedia.org/wiki/Principle_of_least_privilege)安全原则限制访问至关重要。 这些访问权限是通过将相应的 RBAC 角色分配给特定范围内的组和应用程序来授予的。 可以使用[内置 RBAC 角色](../../role-based-access-control/built-in-roles.md)（例如存储帐户参与者）将权限分配给用户。 可通过基于角色的访问控制 (RBAC)，控制借助 [Azure 资源管理器](../../storage/blobs/security-recommendations.md)模型访问存储帐户的存储密钥的情况。

### <a name="shared-access-signature"></a>共享访问签名
[共享访问签名 （SAS）](../../storage/common/storage-dotnet-shared-access-signature-part-1.md)提供对存储帐户中资源的委派访问权限。 使用 SAS，意味着可以授权客户端在指定时间段内，以一组指定权限有限访问存储帐户中的对象。 可以授予这些有限的权限，而不必共享帐户访问密钥。
...
-   [线加密](../../storage/blobs/security-recommendations.md)，如[Azure 文件共享](../../storage/files/storage-dotnet-how-to-use-files.md)的[SMB 3.0 加密](../../storage/blobs/security-recommendations.md)。
...
[用户定义的路由](../../virtual-network/virtual-networks-udr-overview.md)允许用户为进出单个虚拟机或子网的流量自定义入站和出站路径，以确保最安全的路由。 [强制隧道](https://www.petri.com/azure-forced-tunneling)是一种机制，可用于确保不允许服务启动与 Internet 上的设备的连接。
...
虽然网络安全组、用户定义的路由和强制隧道在 [OSI 模型](https://en.wikipedia.org/wiki/OSI_model)的网络层和传输层为用户提供了一定程度的安全性，但有时可能想要启用堆栈的更高级别安全性。 可以使用 Azure 合作伙伴安全设备解决方案访问这些增强的网络安全功能。 您可以通过访问[Azure 应用商店](https://azure.microsoft.com/marketplace/)并搜索"安全"和"网络安全"来查找最新的 Azure 合作伙伴网络安全解决方案。
...
此外，可以使用 Azure 中可用的[连接选项](../../vpn-gateway/index.yml)之一将虚拟网络连接到本地网络。 实际上，可以将网络扩展到 Azure，对 IP 地址块进行完全的控制，并享受企业级 Azure 带来的好处。
...
若要在 Azure 虚拟网络与本地站点之间发送网络流量，必须为 Azure 虚拟网络创建 VPN 网关。 [VPN 网关](../../vpn-gateway/vpn-gateway-about-vpngateways.md)是一种虚拟网络网关，通过公共连接发送加密流量。 也可以使用 VPN 网关在基于 Azure 网络结构的 Azure 虚拟网络之间发送流量。
...
使用 Microsoft [Azure 流量管理器](../../traffic-manager/traffic-manager-overview.md)，可以控制用户流量在不同数据中心内的服务终结点上的分布。 流量管理器支持的服务终结点包括 Azure VM、Web 应用和云服务。 也可将流量管理器用于外部的非 Azure 终结点。 流量管理器使用域名系统 （DNS） 根据[流量路由方法和](../../traffic-manager/traffic-manager-routing-methods.md)终结点的运行状况将客户端请求定向到最合适的终结点。
...
[Azure 负载均衡器](../../load-balancer/load-balancer-overview.md)可为应用程序提供高可用性和网络性能。 它是第 4 层（TCP、UDP）类型的负载均衡器，可在负载均衡集中定义的运行状况良好的服务实例之间分配传入流量。 可以将 Azure 负载均衡器配置为：
...
[域名系统](https://technet.microsoft.com/library/bb629410.aspx)（或 DNS）负责将网站或服务名称翻译（或解析）到其 IP 地址。 [Azure DNS](../../dns/dns-overview.md) 是 DNS 域的托管服务，它使用 Microsoft Azure 基础结构提供名称解析。 通过在 Azure 中托管域，可以使用与其他 Azure 服务相同的凭据、API、工具和计费来管理 DNS 记录。 DNS 支持“CIA”安全三因素的可用性方面。

### <a name="azure-monitor-logs-nsgs"></a>Azure 监视器记录 NSG
可以为 NSG 启用以下诊断日志类别：
...
[Azure 安全中心](../../security-center/security-center-intro.md)持续分析 Azure 资源的安全状态，以进行网络安全最佳实践。 当安全中心识别潜在的安全漏洞时，它会创建[建议](../../security-center/security-center-recommendations.md)，指导您完成配置所需控件以强化和保护资源的过程。
...
[Azure 磁盘加密](../azure-security-disk-encryption-overview.md)是一项新功能，可帮助您加密 Windows 和 Linux IaaS 虚拟机磁盘。 它应用 Windows 的行业标准 BitLocker 功能和 Linux 的 DM-Crypt 功能，为 OS 和数据磁盘提供卷加密。 该解决方案与 Azure Key Vault 集成，帮助用户控制和管理 Key Vault 订阅中的磁盘加密密钥和机密。 此解决方案还可确保虚拟机磁盘上的所有数据在 Azure 存储中静态加密。

### <a name="virtual-networking"></a>虚拟网络
虚拟机需要网络连接。 要支持该要求，Azure 要求将虚拟机连接到 Azure 虚拟网络。 Azure 虚拟网络是一个构建于物理 Azure 网络结构之上的逻辑构造。 每个逻辑[Azure 虚拟网络](../../virtual-network/virtual-networks-overview.md)与所有其他 Azure 虚拟网络隔离。 这种隔离可帮助确保部署中的网络流量对于其他 Microsoft Azure 客户不可访问。
...
[安全中心](../../security-center/security-center-intro.md)可帮助您预防、检测和响应威胁，并增强对 Azure 资源安全性的可见性和控制。 它提供对 Azure 订阅的集成安全监视和策略管理，帮助检测可能被忽略的威胁，且适用于广泛的安全解决方案生态系统。
...
| :------------- | :------------- |:------------- |:------------- |:------------- |
|   [目录对象](../../active-directory/active-directory-whatis.md)、[用户/组管理（添加/更新/删除）/基于用户的预配、设备注册](../../active-directory/active-directory-whatis.md)、[单一登录 （SSO）、](../../active-directory/active-directory-whatis.md)[云用户的自助服务密码更改](../../active-directory/active-directory-whatis.md)、[连接（将本地目录扩展到 Azure 活动目录的同步引擎）、](../../active-directory/active-directory-whatis.md)[安全/使用情况报告](../../active-directory/active-directory-whatis.md)       |   [基于组的访问管理/预配](../../active-directory/active-directory-whatis.md)、[云用户的自助服务密码重置](../../active-directory/active-directory-whatis.md)、[公司品牌（登录页面/访问面板自定义）、](../../active-directory/active-directory-whatis.md)[应用程序代理](../../active-directory/active-directory-whatis.md) [、SLA 99.9%](../../active-directory/active-directory-whatis.md) |  [自助服务组和应用管理/自助服务应用程序添加/动态组](../../active-directory/active-directory-whatis.md)、[自助服务密码重置/更改/解锁与本地写回](../../active-directory/active-directory-whatis.md)，[多重身份验证（云和本地 （MFA 服务器）） ，](../../active-directory/active-directory-whatis.md) [MIM CAL + MIM 服务器](../../active-directory/active-directory-whatis.md)，[云应用程序发现](../../active-directory/active-directory-whatis.md)，[连接运行状况](../../active-directory/active-directory-whatis.md)，[组帐户自动密码滚动](../../active-directory/active-directory-whatis.md)|    [身份保护](../../active-directory/identity-protection/overview.md)，[特权身份管理](../../active-directory/privileged-identity-management/pim-configure.md)|   [将设备加入 Azure AD、桌面 SSO、Azure AD 的 Microsoft 护照、管理员 BitLocker 恢复](../../active-directory/active-directory-whatis.md)[、MDM 自动注册、自助服务 BitLocker 恢复、通过 Azure AD 联接将其他本地管理员连接到 Windows 10 设备](../../active-directory/active-directory-whatis.md)|
...
- [Azure 活动目录标识保护](https://azure.microsoft.com/documentation/articles/active-directory-identityprotection/)是一种安全服务，它使用 Azure 活动目录异常检测功能，为风险检测和可能影响组织身份的潜在漏洞提供统一视图。
