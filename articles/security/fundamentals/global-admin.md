...
执行[如何要求对用户或组进行双重验证](https://docs.microsoft.com/azure/multi-factor-authentication/multi-factor-authentication-get-started-user-states)中列出的某个过程，开始使用 Azure MFA。 你可以选择对所有登录强制执行双重验证, 也可以创建条件性访问策略, 以便仅在有必要时才需要双重验证。
