...
如果无法从安全事件和事件管理 (SIEM) 供应商获取 [Azure Monitor](/azure/monitoring-and-diagnostics/monitoring-get-started) 连接器，应仅使用 Azure 日志集成。
...
* **两个系统**： 
  ...
  * 要监视的计算机。 这是作为 [Azure 虚拟机](/azure/virtual-machines/virtual-machines-windows-overview)运行的 VM。 来自此计算机的日志记录信息将发送到 Azure 日志集成服务计算机。
...
![安装窗格的屏幕截图，其中的遥测复选框已选中](./media/azure-log-integration-get-started/telemetry.png)
...
2. 导入 Azure 日志集成 cmdlet。 若要导入 cmdlet，请运行脚本 `LoadAzlogModule.ps1`。 输入 `.\LoadAzlogModule.ps1` 并按 Enter（请注意，此命令中使用了 **.\\** ）。 应该会看到如下图所示的内容：
   ...
   ![LoadAzlogModule.ps1 命令的输出屏幕截图](./media/azure-log-integration-get-started/loaded-modules.png)
...
4. 监视系统之前，需要用于 Azure 诊断的存储帐户的名称。 在 Azure 门户中，转到“虚拟机”。 查找要监视的 Windows 虚拟机。 在“属性”部分，选择“诊断设置”。  然后选择“代理”。 记下指定的存储帐户名称。 后面的步骤中需要此帐户名称。
   ...
   ![Azure 诊断设置窗格的屏幕截图](./media/azure-log-integration-get-started/storage-account-large.png) 

   ![“启用来宾级监视”按钮的屏幕截图](./media/azure-log-integration-get-started/azure-monitoring-not-enabled-large.png)
...
5. 现在，请返回到 Azure 日志集成计算机。 验证已安装 Azure 日志集成的系统是否连接到了存储帐户。 运行 Azure 日志集成服务的计算机需要访问存储帐户，以检索 Azure 诊断在每台受监视系统上记录的信息。 验证连接： 
   ...
   ![存储资源管理器中的存储帐户屏幕截图](./media/azure-log-integration-get-started/storage-explorer.png)
...
3. 在“筛选器”框中，输入“存储”。 然后选择“存储帐户”。
   ...
   ![显示“所有服务”中的存储帐户的屏幕截图](./media/azure-log-integration-get-started/filter.png)

4. 此时会显示存储帐户的列表。 双击分配给日志存储的帐户。

   ![显示存储帐户列表的屏幕截图](./media/azure-log-integration-get-started/storage-accounts.png)

5. 在“设置”下，选择“访问密钥”。

   ![显示菜单中“访问密钥”选项的屏幕截图](./media/azure-log-integration-get-started/storage-account-access-keys.png)
...
> [!NOTE]
> 必须先查看[入门](azure-log-integration-get-started.md)文章并完成其中的步骤，然后才能尝试本文中的步骤。
...
* [Azure 日志集成常见问题 (FAQ)](azure-log-integration-faq.md)。 此文提供有关 Azure 日志集成的常见问题的解答。
* [使用 Azure 日志集成来集成 Azure 安全中心警报](/azure/security-center/security-center-integrating-alerts-with-log-integration)。 此文介绍如何同步安全中心警报以及由 Azure 诊断和 Azure 活动日志收集的虚拟机安全事件。 使用 Azure Monitor 日志或 SIEM 解决方案同步日志。
