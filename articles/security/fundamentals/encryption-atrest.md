...
Microsoft Azure 包括根据公司的安全和合规性需求保护数据的工具。 本白皮书重点介绍：
...
静态加密为已存储的数据（静止的）提供数据保护。 对静态数据进行的攻击包括：试图获得存储数据的硬件的物理访问机会，然后盗用其中包含的数据。 在这种攻击中，服务器的硬盘驱动器在维护过程中可能处理不当，攻击者可以移除硬盘。 攻击者随后会将该硬盘驱动器置于受其控制的计算机中，尝试访问相关数据。
...
组织需要数据治理和合规性工作，可能也需要静态加密。 行业和政府法规（例如 HIPAA、PCI 和 FedRAMP）就数据保护和加密要求制定了具体的保障措施。 要符合这其中的许多法规，静态加密是一种必需的强制措施。 有关 Microsoft 的 FIPS 140-2 验证方法的详细信息，请参阅[联邦信息处理标准 （FIPS） 出版物 140-2](https://docs.microsoft.com/microsoft-365/compliance/offering-fips-140-2)。 
...
Azure 中受支持的加密模型分为两个主要组："客户端加密"和"服务器端加密"，如前所述。 Azure 服务始终建议使用独立于所用静态加密模型的安全传输（例如 TLS 或 HTTPS）。 因此，传输过程中的加密应由传输协议来处理，不应成为决定要使用的静态加密模型的主要因素。
...
使用 Microsoft 托管密钥的服务器端加密意味着服务对存储具有完全访问权限，并且可以管理密钥。 某些客户可能希望对密钥进行管理，因为他们觉得自己可以获得更高的安全性，但在评估此模型时，应该考虑与自定义密钥存储解决方案相关联的成本和风险。 在许多情况下，组织可能觉得本地解决方案的资源约束或风险会大于对静态加密密钥进行云管理的风险。  但是，对于需要控制加密密钥的创建或生命周期或让不同的人员管理服务的加密密钥的组织来说，此模型可能是不够的（即密钥管理与服务的总体管理模型隔离）。
...
- 完全控制所使用的密钥 – 加密密钥在客户控制下的密钥保管库中进行管理。
- 能够通过加密将多个服务转换成一个主服务
...
某些 Azure 服务启用了“托管自己的密钥 (HYOK)”密钥管理模型。 此管理模式在需要加密静态数据并管理 Microsoft 控制之外的专有存储库中的密钥的情况下非常有用。 在此模型中，服务必须从外部站点检索密钥。 性能和可用性担保会受影响，并且配置更复杂。 另外，由于服务可以在加密和解密操作过程中访问 DEK，此模型的总体安全保证类似于密钥在 Azure Key Vault 中由客户托管的情况。  因此，此模型不适合大多数组织，除非该组织有特定的密钥管理要求。 由于这些限制，大多数 Azure 服务不支持使用客户所控制硬件中的服务托管密钥的服务器端加密。
...
可以通过 [Always Encrypted](https://msdn.microsoft.com/library/mt163865.aspx) 功能启用对 Azure SQL 数据库数据的客户端加密。 Always Encrypted 使用由客户端创建和存储的密钥。 客户可以将主密钥存储在 Windows 证书存储、Azure Key Vault 或本地硬件安全模块中。 使用 SQL 服务器管理工作室，SQL 用户选择要使用哪个键来加密哪个列。
...
保护存储在 Azure 服务中的客户数据对于 Microsoft 来说至关重要。 所有 Azure 托管服务都会始终提供静态加密选项。 Azure 服务支持服务管理密钥、客户管理密钥或客户端加密。 Azure 服务正在广泛增强静态加密可用性，并计划在未来几个月进行预览和一般可用性的新选项。
