---
title: Azure 服务的安全属性
description: 用于评估 Azure 服务的安全属性清单
services: security
documentationcenter: ''
author: msmbaldwin
manager: barbkess
...
本文收集所选 Azure 服务的安全属性。 安全属性是 Azure 服务的质量或功能。 它有助于服务预防、检测和响应安全漏洞。
...
| 传输中加密（例如 ExpressRoute 加密、VNet 中加密，以及 VNet-VNet 加密） | 是 | [快速路由](/azure/expressroute/index)和 VNet 加密由 [Azure 网络](/azure/virtual-network/index)提供。 |
| 加密密钥处理（CMK、BYOK 等）| 否 | 所有加密密钥都是基于服务实例的，也是通过服务托管的。 |
| 列级加密（Azure 数据服务）| 不可用 | |
| 加密的 API 调用| 是 | 通过 [Azure 资源管理器](/azure/azure-resource-manager/index)经 TLS 进行管理平面调用。 需要有效 JSON web 令牌 (JWT)。  可以通过 TLS 以及某个受支持的身份验证机制（例如，客户端证书或 JWT）对数据平面调用进行保护。
 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 控制和管理平面日志记录和审核| 是 | [Azure Monitor 活动日志](/azure/azure-monitor/platform/activity-logs-overview) |
| 数据平面日志记录和审核| 是 | [Azure Monitor 诊断日志](/azure/azure-monitor/platform/diagnostic-logs-overview)和 (可选) [Azure 应用程序见解](/azure/azure-monitor/app/app-insights-overview)。|
...
## <a name="app-serviceazureapp-serviceapp-service-security-attributes"></a>[应用服务](/azure/app-service/app-service-security-attributes)

### <a name="preventative"></a>预防

| 安全属性 | 是/否 | 说明 |
|---|---|--|
| 静态加密（例如服务器端加密、带客户托管密钥的服务器端加密，以及其他加密功能） | 是 | 网站文件内容存储在 Azure 存储中，后者自动对内容进行静态加密。 请参阅[静态数据的 Azure 存储加密](/azure/storage/common/storage-service-encryption)。<br><br>客户提供的机密会进行静态加密。 机密在存储到应用服务配置数据库中时会进行静态加密。<br><br>本地附加的磁盘可以由网站选择性地用作临时存储 (D:\local and %TMP%)。 本地附加的磁盘不进行静态加密。 |
...
| 加密密钥处理（CMK、BYOK 等）| 是 | 客户可以选择将应用程序机密存储在 Key Vault 中，在运行时检索它们。 请参阅[使用应用服务和 Azure Functions 的 Key Vault 引用（预览版）](/azure/app-service/app-service-key-vault-references)。|
| 列级加密（Azure 数据服务）| 不可用 | |
| 加密的 API 调用| 是 | 可以通过基于 HTTPS 的 [Azure 资源管理器](/azure/azure-resource-manager/index)调用进行管理调用，以便配置应用服务。 |

### <a name="network-segmentation"></a>网络分段

| 安全属性 | 是/否 | 说明 |
|---|---|--|
| 服务终结点支持| 是 | 目前提供适用于应用服务的预览版。 请参阅 [Azure 应用服务访问限制](/azure/app-service/app-service-ip-restrictions)。 |
| VNet 注入支持| 是 | 应用服务环境是专用于注入到客户虚拟网络的单个客户的应用服务的专用实现。 请参阅[应用服务环境简介](/azure/app-service/environment/intro)。 |
| 网络隔离和防火墙支持| 是 | 对于应用服务的公共多租户变体，客户可以配置网络 ACL（IP 限制），锁定允许的入站流量。  请参阅 [Azure 应用服务访问限制](/azure/app-service/app-service-ip-restrictions)。  应用服务环境直接部署到虚拟网络中，因此可以通过 NSG 来确保安全。 |
| 强制隧道支持| 是 | 应用服务环境可部署到客户的虚拟网络中, 其中配置了强制隧道。 客户需要按照[将应用服务环境配置为强制隧道](/azure/app-service/environment/forced-tunnel-support)中的说明进行操作。 |

### <a name="detection"></a>检测

| 安全属性 | 是/否 | 说明|
|---|---|--|
| Azure 监视支持（Log Analytics、App Insights 等）| 是 | 应用服务与支持 Application Insights (完整 .NET Framework、.NET Core、Java 和 NODE.JS) 的语言的 Application Insights 集成。  请参阅[监视 Azure App Service 性能](/azure/azure-monitor/app/azure-web-apps)。 应用服务还会将应用程序指标发送到 Azure Monitor。 请参阅[在 Azure App Service 中监视应用](/azure/app-service/web-sites-monitor)。 |

### <a name="identity-and-access-management"></a>标识和访问管理

| 安全属性 | 是/否 | 说明|
|---|---|--|
| 身份验证| 是 | 客户可以构建基于应用服务的应用程序，这些应用程序自动集成 [Azure Active Directory (Azure AD)](/azure/active-directory/index) 以及其他与 OAuth 兼容的标识提供者；请参阅 [Azure 应用服务中的身份验证和授权](/azure/app-service/overview-authentication-authorization)。 对应用服务资产进行管理访问时，所有访问都可以通过组合使用经 Azure AD 验证的主体和 Azure 资源管理器 RBAC 角色进行控制。 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 控制和管理平面日志记录和审核| 是 | 在应用服务对象上执行的所有管理操作都通过 [Azure 资源管理器](/azure/azure-resource-manager/index)来完成。 可以通过门户和 CLI 获取这些操作的历史日志；请参阅 [Azure 资源管理器资源提供程序操作](/azure/role-based-access-control/resource-provider-operations#microsoftweb)和 [az monitor activity-log](/cli/azure/monitor/activity-log)。 |
...
## <a name="azure-resource-managerazureazure-resource-managerazure-resource-manager-security-attributes"></a>[Azure 资源管理器](/azure/azure-resource-manager/azure-resource-manager-security-attributes)
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 控制和管理平面日志记录和审核| 是 | 活动日志公开对资源执行的所有写入操作（PUT、POST、DELETE）；请参阅[查看活动日志来审核对资源的操作](/azure/azure-resource-manager/resource-group-audit)。 |
| 数据平面日志记录和审核| 不可用 | |
...
## <a name="azure-backupazurebackupbackup-security-attributes"></a>[Azure 备份](/azure/backup/backup-security-attributes)
...
| 安全属性 | 是/否 | 说明 |
|---|---|--|
| 静态加密（例如服务器端加密、带客户托管密钥的服务器端加密，以及其他加密功能） | 是 | 所有 Cosmos DB 数据库和备份默认已加密。请参阅 [Azure Cosmos DB 中的数据加密](/azure/cosmos-db/database-encryption-at-rest)。 不支持使用客户管理的密钥进行服务器端加密。 |
| 传输中加密（例如 ExpressRoute 加密、VNet 中加密，以及 VNet-VNet 加密）| 是 | 所有 Azure Cosmos DB 数据在传输中都会经过加密。 |
| 加密密钥处理（CMK、BYOK 等）| 否 |  |
| 列级加密（Azure 数据服务）| 是 | 只能在表 API 高级版中使用。 并非所有 API 都支持此功能。 请参阅 [Azure Cosmos DB 简介：表 API](/azure/cosmos-db/table-introduction)。 |
...
|---|---|--|
| 服务终结点支持| 是 |  |
| VNet 注入支持| 是 | 使用 VNet 服务终结点可将 Azure Cosmos DB 帐户配置为仅允许从虚拟网络 (VNet) 的特定子网进行访问。 还可以将 VNet 访问与防火墙规则相结合。  请参阅[从虚拟网络访问 Azure Cosmos DB](/azure/cosmos-db/vnet-service-endpoint)。 |
| 网络隔离和防火墙支持| 是 | 借助防火墙支持，可将 Azure Cosmos 帐户配置为仅允许从一组已批准的 IP 地址、某个 IP 地址范围和/或云服务进行访问。 请参阅[在 Azure Cosmos DB 中配置 IP 防火墙](/azure/cosmos-db/how-to-configure-firewall)。|
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| Azure 监视支持（Log Analytics、App Insights 等）| 是 | 发送到 Azure Cosmos DB 的所有请求将被记录。 支持 [Azure 监视](/azure/azure-monitor/overview)、Azure 指标和 Azure 审核日志记录。  可以记录对应于数据平面请求、查询运行时统计、查询文本和 MongoDB 请求的信息。 还可以设置警报。 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 跨域资源共享 (CORS) | 是 | 请参阅[配置跨域资源共享 (CORS)](/azure/cosmos-db/how-to-configure-cross-origin-resource-sharing)。 |


## <a name="event-hubsazureevent-hubsevent-hubs-security-attributes"></a>[事件中心](/azure/event-hubs/event-hubs-security-attributes)
...
| 加密密钥处理（CMK、BYOK 等）| 不可用 | |
| 列级加密（Azure 数据服务）| 不可用 | |
| 加密的 API 调用| 是 | 通过[Azure 资源管理器](/azure/azure-resource-manager/index)。 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| Azure 监视支持（Log Analytics、App Insights 等）| 是 | [有关公共基本负载均衡器](/azure/load-balancer/load-balancer-monitor-log), 请参阅 Azure Monitor 日志。 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 控制和管理平面日志记录和审核| 是 | [有关公共基本负载均衡器](/azure/load-balancer/load-balancer-monitor-log), 请参阅 Azure Monitor 日志。 |
| 数据平面日志记录和审核 | 不可用 |  |
...
## <a name="service-bus-messagingazureservice-bus-messagingservice-bus-messaging-security-attributes"></a>[服务总线消息传送](/azure/service-bus-messaging/service-bus-messaging-security-attributes)
...
| 加密密钥处理（CMK、BYOK 等）| 否 |   |
| 列级加密（Azure 数据服务）| 不可用 | |
| 加密的 API 调用| 是 | API 调用通过 [Azure 资源管理器](/azure/azure-resource-manager/index)和 HTTPS 进行。 |

### <a name="network-segmentation"></a>网络分段

| 安全属性 | 是/否 | 说明 |
|---|---|--|
| 服务终结点支持| 是（仅限高级层） | 仅[服务总线高级层](/azure/service-bus-messaging/service-bus-premium-messaging)支持 VNet 服务终结点。 |
| VNet 注入支持| 否 | |
| 网络隔离和防火墙支持| 是（仅限高级层） |  |
| 强制隧道支持| 否 |  |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| Azure 监视支持（Log Analytics、App Insights 等）| 是 | 通过 [Azure Monitor 和警报](/azure/service-bus-messaging/service-bus-metrics-azure-monitor)支持。 |

### <a name="identity-and-access-management"></a>标识和访问管理

| 安全属性 | 是/否 | 说明|
|---|---|--|
| 身份验证| 是 | 通过 [Azure Active Directory 托管服务标识](/azure/service-bus-messaging/service-bus-managed-service-identity)进行管理；请参阅[服务总线身份验证和授权](/azure/service-bus-messaging/service-bus-authentication-and-authorization)。|
| Authorization| 是 | 支持通过 [RBAC](/azure/service-bus-messaging/service-bus-role-based-access-control)（预览版）和 SAS 令牌进行授权；请参阅[服务总线身份验证和授权](/azure/service-bus-messaging/service-bus-authentication-and-authorization)。 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 控制和管理平面日志记录和审核| 是 | 操作日志可用；请参阅[服务总线诊断日志](/azure/service-bus-messaging/service-bus-diagnostic-logs)。  |
| 数据平面日志记录和审核| 否 |  |
...
## <a name="service-bus-relayazureservice-bus-relayservice-bus-relay-security-attributes"></a>[服务总线中继](/azure/service-bus-relay/service-bus-relay-security-attributes)
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 控制和管理平面日志记录和审核| 是 | 通过 [Azure 资源管理器](/azure/azure-resource-manager/index)。 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 配置管理支持（配置的版本控制等）| 是 | 通过 [Azure 资源管理器](/azure/azure-resource-manager/index)。|
...
SQL 数据库同时包含[单一数据库](/azure/sql-database/sql-database-single-index)和[托管实例](/azure/sql-database/sql-database-managed-instance)。 以下条目适用于这两个产品/服务, 但在其他情况下除外。

### <a name="preventative"></a>预防

| 安全属性 | 是/否 | 说明 |
|---|---|--|
| 静态加密（例如服务器端加密、带客户托管密钥的服务器端加密，以及其他加密功能） | 是 | 称为 "正在使用的加密", 如[Always Encrypted](/azure/sql-database/sql-database-always-encrypted)中所述。 服务器端加密使用[透明数据加密](/azure/sql-database/transparent-data-encryption-azure-sql)。|
| 传输中加密（例如 ExpressRoute 加密、VNet 中加密，以及 VNet-VNet 加密） | 是 | 使用 HTTPS。 |
| 加密密钥处理, 如 CMK 或 BYOK| 是 | 提供服务管理的和客户管理的密钥处理。 后者通过[Azure Key Vault](/azure/key-vault/index)提供。 |
| Azure 数据服务提供的列级加密| 是 | 通过 [Always Encrypted](/azure/sql-database/sql-database-always-encrypted) 进行。 |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| Azure 监视支持, 如 Log Analytics 或 Application Insights| 是 | SecureSphere 是 Imperva 的 SIEM 解决方案, 也通过[SQL 审核](/azure/sql-database/sql-database-auditing)通过[Azure 事件中心](/azure/event-hubs/index)集成来支持。 |
...
| 数据平面日志记录和审核 | 是 | 通过[SQL 审核](/azure/sql-database/sql-database-auditing) |
...
| 安全属性 | 是/否 | 说明|
|---|---|--|
| 预防：漏洞评估 | 是 | 请参阅 [SQL 漏洞评估服务可帮助你识别数据库漏洞](/azure/sql-database/sql-vulnerability-assessment)。 |
| 预防：数据发现和分类  | 是 | 请参阅 [Azure SQL 数据库和 SQL 数据仓库数据发现和分类](/azure/sql-database/sql-database-data-discovery-and-classification)。 |
| 检测：威胁检测 | 是 | 请参阅 [Azure SQL 数据库的高级威胁防护](/azure/sql-database/sql-database-threat-detection-overview)。 |


## <a name="storageazurestoragecommonstorage-security-attributes"></a>[存储](/azure/storage/common/storage-security-attributes)
...
| 加密密钥处理（CMK、BYOK 等）| 是 | 请参阅[在 Azure Key Vault 中使用客户托管密钥进行存储服务加密](/azure/storage/common/storage-service-encryption-customer-managed-keys?toc=%2fazure%2fstorage%2fblobs%2ftoc.json)。|
| 列级加密（Azure 数据服务）| 不可用 |  |
| 加密的 API 调用| 是 |  |
...
[Linux vm](/azure/virtual-machines/windows/virtual-machines-windows-security-attributes) | [Windows vm](/azure/virtual-machines/windows/virtual-machines-windows-security-attributes) | [虚拟机规模集](/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-security-attributes)
...
## <a name="vpn-gatewayazurevpn-gatewayvpn-gateway-security-attributes"></a>[VPN 网关](/azure/vpn-gateway/vpn-gateway-security-attributes)
