...
本文概述可与 Azure 存储配合使用的 Azure 安全功能。 Azure 存储是依赖于持续性、可用性和伸缩性来满足客户需求的现代应用程序的云存储解决方案。 Azure 存储提供全面的安全功能。 你可以：
...
* [传输级别加密](/azure/storage/common/storage-security-guide#encryption-in-transit)，例如将数据传入或传出 Azure 存储时使用的 HTTPS。
* [线路加密](/azure/storage/common/storage-security-guide#using-encryption-during-transit-with-azure-file-shares)，例如 Azure 文件共享的 SMB 3.0 加密。
* [客户端加密](/azure/storage/common/storage-security-guide#using-client-side-encryption-to-secure-data-that-you-send-to-storage)，在将数据传输到存储之前加密数据，以及从存储传出数据后解密数据。
...
* [存储服务加密](/azure/storage/common/storage-security-guide#encryption-at-rest)始终处于启用状态，并在数据写入 Azure 存储时自动加密数据。
* [客户端加密](/azure/storage/common/storage-security-guide#client-side-encryption)也提供静态加密功能。
* [Azure 磁盘加密](/azure/storage/common/storage-security-guide#using-azure-disk-encryption-to-encrypt-disks-used-by-your-virtual-machines)允许加密 IaaS 虚拟机使用的 OS 磁盘和数据磁盘。
...
* [静态数据的 Azure 存储服务加密](/azure/storage/common/storage-service-encryption)
...
有关 Azure 存储防火墙和虚拟网络的详细信息，请查看文章[配置 Azure 存储防火墙和虚拟网络](/azure/storage/common/storage-network-security)
