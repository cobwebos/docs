...
* 仅基于特定 Azure 库的 Linux 服务器分发和版本支持 Azure 磁盘加密。  有关当前受支持版本的列表，请参阅 [Azure 磁盘加密常见问题解答](https://docs.microsoft.com/en-us/azure/security/azure-security-disk-encryption-faq)。
...
* 若要启用 Azure 磁盘加密功能，IaaS VM 必须符合以下网络终结点配置要求：
  ...
  * IaaS VM 必须能够连接到托管 Azure 扩展存储库的 Azure 存储终结点和托管 VHD 文件的 Azure 存储帐户。
  ...
  >
  >配置并访问防火墙保护下的 Azure Key Vault (https://docs.microsoft.com/azure/key-vault/key-vault-access-behind-firewall)
...
> [!NOTE]
 > 除非已在 Azure 磁盘加密外部创建快照或备份，否则绝对不要使用 -skipVmBackup 参数。 
...
* 若要使用 Azure 备份服务来备份和还原加密的 VM，则使用 Azure 磁盘加密启用加密时，请使用 Azure 磁盘加密密钥配置加密 VM。 备份服务支持使用非 KEK 或 KEK 配置加密的 VM。 请参阅[如何通过 Azure 备份加密来备份和还原加密的虚拟机](https://docs.microsoft.com/azure/backup/backup-azure-vms-encryption)。

* 请注意，加密 Linux OS 卷时，目前需要在该过程结束时重新启动 VM。 通过门户、powershell 或 CLI 可以完成此操作。   要跟踪加密进度，请定期轮询由 Get-AzureRmVMDiskEncryptionStatus https://docs.microsoft.com/powershell/module/azurerm.compute/get-azurermvmdiskencryptionstatus 返回的状态消息。  加密完成后，此命令返回的状态消息会指示已完成。 例如，“ProgressMessage: OS disk successfully encrypted, please reboot the VM”（ProgressMessage: OS 磁盘已成功加密，请重新启动 VM），此时可以重新启动并使用 VM。  

* 适用于 Linux 的 Azure 磁盘加密要求数据磁盘加密之前在 Linux 中装载文件系统
...
1. 选择“所有服务”>“Azure Active Directory”

 ![Azure 磁盘加密](./media/azure-security-disk-encryption/aad-service.png)

2. 选择“应用注册”>“新应用程序注册”

 ![Azure 磁盘加密](./media/azure-security-disk-encryption/aad-app-registration.png)

3. 提供所需的信息，并创建应用程序：

 ![Azure 磁盘加密](./media/azure-security-disk-encryption/aad-create-app.png)

4. 选择新创建的应用程序以查看其属性，包括应用程序 ID。  要为应用程序创建密钥，请选择“设置”>“密钥”，添加密钥的描述和过期时间，然后单击“保存”

 ![Azure 磁盘加密](./media/azure-security-disk-encryption/aad-create-pw.png)

5. 复制生成的机密值并适当地保护。

 ![Azure 磁盘加密](./media/azure-security-disk-encryption/aad-save-pw.png)
...
> [!NOTE]
> 必须在新的 PowerShell 窗口中执行以下命令。 请不要使用 Azure PowerShell 或 Azure 资源管理器窗口执行这些命令。 提出这项建议是因为这些 cmdlet 位于 MSOnline 模块或 Azure AD PowerShell 中。
...
* ["101-Key-Vault-Create" 资源管理器模板](https://github.com/Azure/azure-quickstart-templates/tree/master/101-key-vault-create)
* [Azure PowerShell Key Vault cmdlet](/powershell/module/azurerm.keyvault/#key_vault)
* Azure 资源管理器
* 如何[保护 Key Vault](https://docs.microsoft.com/azure/key-vault/key-vault-secure-your-key-vault)
...
可通过 Azure 资源管理器或 Key Vault 接口添加 KEK。
...
| subnetName | VM NIC 所属的 VNet 中子网的名称。 |
| vmSize | VM 的大小 目前仅支持标准 A、D 和 G 系列。 |
| keyVaultResourceID | 在 Azure 资源管理器中标识 Key Vault 资源的资源 ID。 可使用 PowerShell cmdlet `(Get-AzureRmKeyVault -VaultName &lt;yourKeyVaultName&gt; -ResourceGroupName &lt;yourResourceGroupName&gt;).ResourceId` 获取它。 |
...
在此方案中，可以通过使用资源管理器模板、PowerShell cmdlet 或 CLI 命令启用加密。 以下部分详细介绍了如何通过资源管理器模板和 CLI 命令启用它。
...
下表列出了用于在正在运行的 IaaS VM 上禁用加密的资源管理器模板参数：
...
* VM 必须使用与 [Azure 磁盘加密常见问题解答](https://docs.microsoft.com/en-us/azure/security/azure-security-disk-encryption-faq#what-linux-distributions-does-azure-disk-encryption-support)中列出的 OS 磁盘加密兼容的分发版 
