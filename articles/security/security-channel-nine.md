...
他们认为软件会有更美好的前景，并为此感到兴奋。 在第 9 频道这个社区，你可以加入正在进行中的对话。

以下是第 9 频道上的精选 Azure 安全演示文稿。 请确保每月查看此页面，获取新的视频。

[使用 Barracuda 安全促进 Azure 的使用](https://channel9.msdn.com/events/Microsoft-Azure-Marketplace-ISV-Solutions-Webinar-Series/Webinar-1-Accelerating-Azure-Consumption-with-Barracuda-Security/Webinar-1-Accelerating-Azure-Consumption-with-Barracuda-Security)

查看如何使用 Barracuda 安全来保护你的 Azure 部署。

[Azure 安全中心 - 威胁检测](https://channel9.msdn.com/Shows/Azure-Friday/Azure-Security-Center-Threat-Detection)

通过 Azure 安全中心，你可以获得所有 Azure 资源的安全状态的核心情况。 可以快速验证适当的安全控制是否就位且配置正确。 Scott 与 Sarah Fender 进行交流，后者就如何将安全中心与威胁检测集成到一起做了介绍。

[Azure 安全中心概述](https://channel9.msdn.com/Shows/Azure-Friday/Azure-Security-Center-Overview)

通过 Azure 安全中心，你可以获得所有 Azure 资源的安全状态的核心情况。 可以快速验证适当的安全控制是否就位且配置正确。 Scott 与 Sara Fender 进行交流，后者就相关问题进行了全面的解释说明。
...
Jack Richins 向 [Scott](https://channel9.msdn.com/Niners/Glucose) 讲述了如何轻松地对他在虚拟机 Azure 实例上的 SQL Server 数据库进行加密。 它的简单程度超乎你的想像！
...
在 Azure 上部署的每个重要应用都有重要的机密 – 连接字符串、证书和密钥。 在管理这些机密时所犯下的愚蠢错误会导致严重的后果 – 泄露、中断、违反合规性。 最近的多项调查表明，愚蠢的错误导致的数据泄露要比对手多出四倍多。 在本会话中，我们将介绍一些管理重要应用机密的最佳实践。 这些最佳实践可能看起来很平常，甚至为许多开发人员所忽视。 我们还将介绍如何使用 Azure 密钥保管库来实施这些最佳实践。 另一个好处是，遵循以下这些实践可帮助你证明其遵从了诸如 SOC 等标准。 会话的前 10 分钟的级别是 100，它们适用于在任何平台上开发的任何云应用。 会话的其余部分的级别是 200～300，重点介绍你在 Azure 平台上构建的应用。

[与 Narayan Annamalai 一起使用网络安全组保护你的 Azure 虚拟网络的安全](https://channel9.msdn.com/Shows/Azure-Friday/Sucruing-your-Azure-Virtual-Network-using-Network-ACLs-with-Narayan-Annamalai)

高级项目经理 Narayan Annamalai 向 Scott 讲述了如何在 Azure 虚拟网络内使用网络安全组。 你可以通过子网和网络创建对 Azure 内的对象的控制访问！ 你将学习如何使用 PowerShell 在 Azure 内控制访问和创建组。
...
* 管理对 Office 365 特定于工作负荷的管理角色的保护
* 为特权角色激活配置 Azure 多重身份验证 (MFA)
* 测量和提高租户的安全状态
* 监视和修复安全发现
* 查看谁需要在定期重新认证工作流中保留特权角色
