---
title: "Azure 磁盘加密故障排除 | Microsoft Docs"
...
- 加密之前在 OS 中安装并运行了 SAP、MongoDB 或 Apache Cassandra 等大型应用程序。  扩展无法正常关闭这些应用程序；如果这些应用程序向 OS 驱动器保持开放文件句柄，则无法卸载驱动器，从而导致失败。
- 在启用加密的几乎同一时间内运行自定义脚本，或者在加密过程中在 VM 上进行其他任何更改。   如果资源管理器模板定义了多个同时执行的扩展，或者在执行磁盘加密的同时运行自定义脚本扩展或其他操作，则可能会发生这种情况。   序列化并隔离此类步骤可能会解决问题。
...
在某些情况下，Linux 磁盘加密看上去停滞在“OS 磁盘加密已启动”状态，同时 SSH 处于禁用状态。 此过程可能需要 3-16 小时才能完成，有时需要更长时间。  Linux OS 磁盘加密序列会暂时卸载 OS 驱动器、对整个 OS 磁盘执行逐块加密，然后以加密状态重新装载磁盘。   与 Windows 上的 Azure 磁盘加密不同，Linux 磁盘加密不允许在加密的同时并发使用 VM。  VM 的性能特征（包括磁盘大小，以及存储帐户是由标准存储还是高级 (SSD) 存储为后盾）可能会极大地影响到完成加密所需的时间。
