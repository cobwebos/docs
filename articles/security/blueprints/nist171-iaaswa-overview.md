...
本 Azure 安全性和符合性蓝图提供的指导可帮助客户在 Azure 中部署 Web 应用程序体系结构，用于实施一部分 NIST SP 800-171 控制措施。 此解决方案演示了客户可以满足特定安全性和符合性要求的方式，并且可以作为客户在 Azure 中构建和配置自己的 Web 应用程序的基础。
...
为了增强安全性，将通过 Azure 资源管理器以资源组的形式管理此解决方案中的所有资源。 Azure Active Directory 基于角色的访问控制用于控制对 Azure Key Vault 中部署的资源和密钥的访问。 系统运行状况通过 Azure Monitor 进行监视。 客户配置两个监视服务捕获日志并在单独的、可轻松导航的仪表板中显示系统运行状况。
...
**Azure 磁盘加密**：Azure 磁盘加密用于加密 Windows IaaS 虚拟机磁盘。 [Azure 磁盘加密](https://docs.microsoft.com/azure/security/azure-security-disk-encryption)利用 Windows 的 BitLocker 功能，为操作系统和数据磁盘提供卷加密。 此解决方案与 Azure Key Vault 集成，用于控制和管理磁盘加密密钥。
...
-   [Azure Active Directory](https://azure.microsoft.com/services/active-directory/) 是 Microsoft 提供的多租户、基于云的目录和标识管理服务。 解决方案的所有用户（包括访问 SQL Server 的用户）都在 Azure Active Directory 中创建。
...
- [Azure Active Directory Privileged Identity Management](https://docs.microsoft.com/azure/active-directory/active-directory-privileged-identity-management-getting-started) 使客户能够最大限度地减少有权访问特定资源的用户数量。  管理员可以使用 Azure Active Directory Privileged Identity Management 来发现、限制和监视特权标识及其对资源的访问。 还可以根据需要，使用此功能来实施按需、实时的管理访问。
...
**应用程序网关**：体系结构使用配置了 Web 应用程序防火墙并启用了 OWASP 规则集的应用程序网关，来降低安全漏洞风险。 其他功能包括：
...
此参考体系结构的数据流图可供[下载](https://aka.ms/nist171-iaaswa-tm)，也可以在下面找到。 此模型有助于客户在做出修改时了解系统基础结构中存在的潜在风险点。
...
[Azure 安全性和符合性蓝图 - NIST SP 800-171 IaaS Web 应用程序控制措施实施矩阵](https://aka.ms/nist171-iaaswa-cim)提供了有关 IaaS Web 应用程序体系结构解决哪些 NIST SP 800-171 措施控制的信息，包括实施方案如何满足每个所述控制措施的要求的详细说明。
