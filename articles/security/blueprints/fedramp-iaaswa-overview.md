...
![替换文字](images/fedramp-iaaswa-architecture.png?raw=true "Azure 安全性和符合性蓝图 - FedRAMP Web 应用程序自动化")
...
[控件实现表](https://aka.ms/blueprintwacim)（Excel 工作簿）列出了 FedRAMP High 基线需要的所有安全控件。 对于每个在客户责任表中指定了客户责任的控件（或控件子部分），该表明确了以下事项：1) 蓝图自动化是否实现该控件；2) 说明了该实现在符合控件要求方面做得如何。 [此处](fedramp-iaaswa-controls-overview.md)也提供了这些内容。
