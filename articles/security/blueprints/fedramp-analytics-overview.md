...
![FedRAMP 参考体系结构示意图的分析](images/fedramp-analytics-reference-architecture.png?raw=true "FedRAMP 参考体系结构示意图的分析")
...
![替换文字](images/fedramp-analytics-components.png?raw=true "FedRAMP 组件示意图的分析")
...
**网络安全组**：设置 [Nsg](../../virtual-network/virtual-network-vnet-plan-design-arm.md) 来管理流向部署资源和服务的流量。 网络安全组设置为默认拒绝方案，并且仅允许预配置访问控制列表 (ACL) 中包含的流量。
...
- **活动日志**：[活动日志](../../azure-monitor/platform/activity-logs-overview.md)提供有关对订阅中资源执行的操作的见解。
...
-   [安全和审核](../../security-center/security-center-intro.md)：安全和审核仪表板通过提供安全域、值得注意的问题、检测、威胁智能和常见安全性查询的指标，提供对资源安全性状态的高级见解。
-   [SQL 评估](../../azure-monitor/insights/sql-assessment.md)：SQL 运行状况检查解决方案定期评估服务器环境的风险和运行状况，并且为客户提供特定于已部署服务器基础结构的优先建议列表。
-   [Azure 活动日志](../../azure-monitor/platform/collect-activity-logs.md)：Activity Log Analytics 解决方案帮助客户跨所有 Azure 订阅分析 Azure 活动日志。
...
-   [反恶意软件评估](../../security-center/security-center-install-endpoint-protection.md)：反恶意软件解决方案报告恶意软件、威胁和防护状态。
-   [更新管理](../../automation/automation-update-management.md)：“更新管理”解决方案允许客户管理操作系统安全更新，包括可用更新的状态以及安装所需更新的过程。
