---
title: Azure 安全性和符合性蓝图：针对 FFIEC 的数据分析
description: Azure 安全性和符合性蓝图：针对 FFIEC 的数据分析
services: security
author: meladie
...
本文中的 Azure 安全性和符合性蓝图提供了相关指南，引导你在 Azure 中针对美国联邦金融机构检查委员会 (FFIEC) 规定的金融数据部署一个适合数据收集、存储且可检索的数据分析体系结构。
...
一旦数据上传到 Azure SQL 数据库并被 Azure 机器学习训练，操作用户和 SQL/数据管理员通过 Power BI 提取其摘要。 Power BI 直观地显示数据，并整合多个数据集的信息以提供更好的见解。 其高度适应性和与 Azure SQL 数据库的轻松集成可确保客户按照自己的业务需求进行配置，从而处理各种情况。
...
![针对 FFIEC 的分析参考体系图示](images/ffiec-analytics-architecture.png "针对 FFIEC 的分析参考体系图示")
...
此参考体系结构的数据流图可供[下载](https://aka.ms/ffiec-analytics-tm)，也可以在下面找到。 此模型有助于客户在做出修改时了解系统基础结构中存在的潜在风险点。

![针对 FFIEC 的分析威胁模型](images/ffiec-analytics-threat-model.png "针对 FFIEC 的分析威胁模型")
...
[Azure 安全性和符合性蓝图 - FFIEC 数据分析实现矩阵](https://aka.ms/ffiec-analytics-cim)介绍了 数据分析体系结构要应对的 FFIEC 要求，其中详细阐述了实施方案如何满足每个所述目标的要求。
