...
| 加密密钥处理（CMK、BYOK 等）| 是 | 客户控制其密钥保管库中的所有密钥。 如果指定了硬件安全模块 (HSM) 支持的密钥，FIPS 级别 2 HSM 保护密钥、 证书或密钥。 |
| 列级加密（Azure 数据服务）| 不适用 |  |
...
| 加密密钥处理（CMK、BYOK 等）| 是 | 请参阅[使用 Azure Key Vault 中客户托管密钥的存储服务加密](../storage/common/storage-service-encryption-customer-managed-keys.md)。|
| 列级加密（Azure 数据服务）| 不适用 |  |
| 加密的 API 调用| 是 |  |
