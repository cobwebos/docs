...
- 根据“需要知道”和“最低权限”安全原则限制访问。 对于想要实施数据访问安全策略的组织，限制访问是必须要做的事。 基于角色的访问控制 (RBAC) 可用于向特定范围的用户、组和应用程序分配权限。 请参阅[访问管理入门](../active-directory/role-based-access-control-what-is.md)，了解有关向用户授予应用程序访问权限的详细信息。
...
- 限制传入的源 IP 地址。 应用服务提供虚拟网络集成功能，帮助你通过网络安全组 (NSG) 限制传入的源 IP 地址。 如果你不熟悉 Azure 虚拟网络 (VNET)，可使用此功能将多个 Azure 资源放置在可以控制其访问权限但无法通过 Internet 路由的网络中。 请参阅[将应用与 Azure 虚拟网络集成](../app-service-web/web-sites-integrate-with-vnet.md)了解详细信息。
