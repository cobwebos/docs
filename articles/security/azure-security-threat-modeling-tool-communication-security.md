...
## <a id="ssl-appservice"></a>在 Azure App Service 中为自定义域配置 SSL 证书
...
## <a id="appservice-https"></a>强制要求发往 Azure App Service 的所有流量都通过 HTTPS 连接传输
...
## <a id="smb-shares"></a>使用 SMB 3.0 兼容的客户端来确保传输到 Azure 文件共享的数据经过加密
...
| 适用的技术 | 泛型 |
| 属性              | StorageType - 文件 |
| 参考              | [Azure 文件存储](../storage/storage-files-introduction.md)，[如何在 Windows 上装载 Azure 文件共享](../storage/storage-file-how-to-use-files-windows.md) |
| 步骤 | 使用 REST API 时，Azure 文件存储支持 HTTPS，但经常用作附加到 VM 的 SMB 文件共享。 SMB 2.1 不支持加密，因此只允许在 Azure 中的相同区域内连接。 但是，SMB 3.0 支持加密，并且可以配合 Windows Server 2016、Windows Server 2012 R2、Windows 8.1 和 Windows 10 使用，允许跨区域访问，甚至桌面上的访问。 |

## <a id="cert-pinning"></a>实现证书固定

| 标题                   | 详细信息      |
| ----------------------- | ------------ |
| 组件               | Azure 存储 | 
| SDL 阶段               | 构建 |  
| 适用的技术 | 通用、Windows Mobile |
| 属性              | 不适用  |
| 参考              | [证书和公钥绑定](https://www.owasp.org/index.php/Certificate_and_Public_Key_Pinning#.Net) |
| 步骤 | <p>证书固定可以防范基于 TLS 的中间人 (MITM) 攻击。 固定是将 TLS 连接与预期 X.509 证书或公钥相关联的过程。 这种关联可在首次成功连接（“首次使用时信任”）后发生或在这之前发生（需要预先加载）。</p><p>当对手尝试执行 TLS MITM 攻击时，客户端将进行检查以确保收到的证书/密钥与预期值相匹配。 因此，即使对手具有其他有效且受信任的证书，连接仍将失败。</p><p>证书固定可以在 .NET 下通过实现 [HttpWebRequest.ServerCertificateValidationCallback](https://docs.microsoft.com/dotnet/api/system.net.httpwebrequest.servercertificatevalidationcallback)（首选）或 [ServicePointManager.ServerCertificateValidationCallback](https://docs.microsoft.com/dotnet/api/system.net.servicepointmanager.servercertificatevalidationcallback) 委托来实现。 下例验证了硬编码公钥和算法，但在实际应用中，应将这些值存储在安全配置区域，并根据需要进行更新。</p>|
