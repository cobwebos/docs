...
* IaaS VM 根据客户控制的密钥和策略启动。 可在 Key Vault 中审核密钥和策略的使用方式。
...
* 在 Windows 虚拟机规模集上启用加密。
* 在 Linux 虚拟机规模集的数据驱动器上启用加密。
* 在 Windows IaaS VM 上禁用加密。
* 在 Linux IaaS VM 的数据驱动器上禁用加密。
* 在 Windows 虚拟机规模集上禁用加密。
* 在 Linux 虚拟机规模集的数据驱动器上禁用加密。
* 启用托管磁盘 VM 的加密。
...
* 标准层 VM：[A、D、DS、G、GS 和 F 等系列 IaaS VM](https://azure.microsoft.com/pricing/details/virtual-machines/)。 这些层中的 [Linux VM](azure-security-disk-encryption-faq.md#bkmk_LinuxOSSupport) 必须满足最小 7 GB 内存要求。
* 通过支持的 Azure 库映像在 Windows 和 Linux IaaS VM、托管磁盘和规模集 VM 上启用加密。
* 在 Windows IaaS VM、规模集 VM 和托管磁盘 VM 的 OS 和数据驱动器上禁用加密。
* 在 Linux IaaS VM、规模集 VM 和托管磁盘 VM 的数据驱动器上禁用加密。
...
* 适用于 Windows 的磁盘加密扩展。
* 适用于 Linux 的磁盘加密扩展。
* PowerShell 磁盘加密 cmdlet。
* Azure CLI 磁盘加密 cmdlet。
* Azure 资源管理器磁盘加密模板。

在运行 Windows 或 Linux OS 的 IaaS VM 上支持 Azure 磁盘加密解决方案。 有关支持的操作系统的详细信息，请参阅[先决条件](azure-security-disk-encryption-prerequisites.md)一文。
...
1. 向 Azure 平台授予访问权限，使其能够从 Key Vault 中读取加密密钥数据（Windows 系统的 BitLocker 加密密钥，和 Linux 的通行短语），从而在 IaaS VM 上启用加密。
...
1. 选择在 Azure 中运行的 IaaS VM 上禁用加密（解密），并指定解密配置。 可以通过 Azure 磁盘加密资源管理器模板、PowerShell cmdlet 或 Azure CLI 禁用加密。
   ...
   此步骤将对正在运行的 Windows IaaS VM 禁用 OS 和/或数据卷加密。 如前面部分所述，不支持对 Linux 禁用 OS 磁盘加密。 只要 OS 磁盘未加密，仅允许对 Linux VM 上的数据驱动器执行解密步骤。

1. Azure 更新 VM 服务模型后，IaaS VM 会被标记为已解密。 VM 的内容不再静态加密。
...
1. 提供 Azure AD 应用程序标识，将加密密钥材料写入 Key Vault。 执行此步骤后，可在步骤 2 所述方案中的 IaaS VM 上启用加密。
...
| KEK | 密钥加密密钥 (KEK) 是非对称密钥 (RSA 2048)，用于在需要时保护或包装机密。 可提供硬件安全模块 (HSM) 保护的密钥或软件保护的密钥。 有关详细信息，请参阅 [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) 文档。 |
