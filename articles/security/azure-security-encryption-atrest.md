...
在某些情况下，组织需要进行数据管理和确保符合性，则也需使用静态加密。 行业和政府法规（例如 HIPAA、PCI 和 FedRAMP）以及国际法规对数据的保护和加密以流程和策略的方式提出了特定的保护措施要求。 这其中的许多法规都要求将静态加密作为一种强制措施来管理和保护数据，否则为不合规。 
...
在实施静态加密时，通常使用多个加密密钥。 非对称加密用于建立信任和完成身份验证，这是访问和管理密钥所必需的。 对称加密在进行批量加密和解密时更有效，因此加密强度更大，性能更好。 另外，限制单个加密密钥的使用降低了密钥被盗用的风险，也降低了必须更换密钥时的重新加密成本。 为了发挥非对称加密和对称加密的优势并对使用和公开单个密钥的行为进行限制，Azure 静态加密模型使用由以下密钥类型组成的密钥层次结构：
...
客户端加密模型是指由服务或调用应用程序在资源提供程序或 Azure 外部执行的加密。 加密可以由 Azure 中的服务应用程序执行，也可以由在客户数据中心运行的应用程序执行。 不管哪种情况，在采用此加密模型时，Azure 资源提供程序都会收到加密的数据 blob，但却无法以任何方式解密数据，也无法访问加密密钥。 在此模型中，密钥管理由调用服务/应用程序执行，对 Azure 服务来说是完全不透明的。
...
使用 Microsoft 托管密钥的服务器端加密意味着服务具有存储的完全访问权限，并且可以管理密钥。 某些客户可能希望对密钥进行管理，因为他们觉得自己可以确保更高的安全性，但在评估此模型时，应该考虑与自定义密钥存储解决方案相关联的成本和风险。 在许多情况下，组织可能觉得本地解决方案的资源约束或风险会大于对静态加密密钥进行云管理的风险。  但是，此模型可能满足不了某些组织的需求，这些组织需要控制加密密钥的创建或生命周期，或者需要安排某个人来管理服务，安排另一个人来管理该服务的加密密钥（也就是说，这是一种将密钥管理与总体管理分开的服务管理模型）。
...
将客户托管密钥置于 Azure Key Vault 中的服务器端加密模型涉及到的操作是，服务会根据需要访问用于加密和解密的密钥。 可以通过访问控制策略来允许服务访问静态加密密钥，只需授予该服务实体接收密钥所需的访问权限即可。 可以为代表关联的订阅运行的 Azure 服务配置一个用于该订阅中该服务的标识。 该服务可以执行 Azure Active Directory 身份验证，然后会收到一个身份验证令牌，将服务本身标识为代表该订阅的服务。 然后，该服务可以将该令牌出示给 Key Vault，以便获取有权访问的密钥。
...
软件即服务 (SaaS) 客户通常会在每个服务中启用或提供静态加密。 Office 365 服务提供多个供客户验证或启用静态加密的选项。 有关 Office 365 服务的信息，请参阅“Data Encryption Technologies for Office 365”（适用于 Office 365 的数据加密技术）。
...
|----------------------------------|----------------|---------------------|------------------------------|------------------------------|--------|
|                                  |                |                     |                              |                              | 客户端 |
|                                  | **密钥管理** | 服务托管密钥 | 在 Key Vault 中由客户托管 | 在本地由客户托管 |        |
| 存储和数据库            |                |                     |                              |                              |        |
| 磁盘 (IaaS)                      |                | -                   | 是                          | 是*                         | -      |
