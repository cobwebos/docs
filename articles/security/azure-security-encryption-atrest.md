...
静态加密是常见的安全要求。 在 Azure 中，组织可以实施静态加密，不存在实施和管理成本，也没有自定义密钥管理解决方案的风险。 组织可以选择让 Azure 来全权管理静态加密。 另外，组织还可以通过各种选择来严格管理加密或加密密钥。
...
- 必须将密钥存储在安全位置，通过访问控制策略来限制对某些标识的访问并记录密钥使用情况。 数据加密密钥通常使用非对称加密来加密，目的是进一步限制访问。
...
|                                  |                    | **加密模型和密钥管理** |                   |                    |
|----------------------------------|--------------------|--------------------|--------------------|--------------------|
|                                  | **使用服务托管密钥的服务器端**     | **使用 Key Vault 中的客户管理密钥的服务器端**             |  **使用本地客户管理密钥的服务器端**                  | **使用客户端托管密钥的客户端**      |
| **存储和数据库**        |                    |                    |                    |                    |                    |
...
| Azure 流分析           | 是                | -                  | -                  | -                  |
| HDInsight（Azure Blob 存储）   | 是                | -                  | -                  | -                  |
| HDInsight (Data Lake Storage)    | 是                | -                  | -                  | -                  |
