...
| 产品/服务 | 项目 |
| --------------- | ------- |
| **Web 应用程序** | <ul><li>[仅使用经批准的对称块密码和键长度](#cipher-length)</li><li>[为对称加密法使用批准的块加密法模式和初始化向量](#vector-ciphers)</li><li>[使用批准的非对称算法、密钥长度和填充](#padding)</li><li>[使用经批准的随机数生成器](#numgen)</li><li>[不要使用对称流密码](#stream-ciphers)</li><li>[使用批准的 MAC/HMAC/键控哈希算法](#mac-hash)</li><li>[只使用批准的加密哈希函数](#hash-functions)</li></ul> |
| **数据库** | <ul><li>[使用强加密算法加密数据库中的数据](#strong-db)</li><li>[应该对 SSIS 包加密和数字签名](#ssis-signed)</li><li>[将数字签名添加到关键数据库安全功能](#securables-db)</li><li>[使用 SQL 服务器 EKM 保护加密密钥](#ekm-keys)</li><li>[如果不应向数据库引擎透露加密密钥，请使用 AlwaysEncrypted 功能](#keys-engine)</li></ul> |
| **IoT 设备** | <ul><li>[在 IoT 设备上安全地存储加密密钥](#keys-iot)</li></ul> | 
...
| **Dynamics CRM Outlook 客户端** | <ul><li>[确保设备管理策略到位，需要 PIN/密码/自动锁定并加密所有数据（例如 BitLocker）](#bitlocker)</li></ul> | 
| **标识服务器** | <ul><li>[使用标识服务器时确保滚动更新签名密钥](#rolled-server)</li><li>[确保在身份服务器中使用加密强客户端 ID、客户端密钥](#client-server)</li></ul> | 
...
| **属性**              | 空值  |
| **引用**              | 空值  |
| **步骤** | <p>产品必须使用 SHA-2 系列的哈希算法（SHA256、SHA384 和 SHA512）。 如果需要更短的哈希（例如 128 位输出长度）来适应出于缩短 MD5 哈希目的而设计的数据结构，产品团队可以截断 SHA2 哈希之一（通常为 SHA256）。 请注意，SHA384 是 SHA512 的截断版本。 不允许出于安全目的将加密哈希截断至 128 位以下。 新代码必须使用 MD2、MD4、MD5、SHA-0、 SHA-1 或 RIPEMD 哈希算法。 对于这些算法，哈希冲突在计算上是可行的，这会有效地破解这些算法。</p><p>为使加密灵活可控而允许的 .NET 哈希算法（按优先顺序列出）：</p><ul><li>SHA512Cng（符合 FIPS）</li><li>SHA384Cng（符合 FIPS）</li><li>SHA256Cng（符合 FIPS）</li><li>SHA512 托管（不符合 FIPS）（在调用 HashAlgorithm 时使用 SHA512 作为算法名称。</li><li>SHA384 管理（不符合 FIPS）（在调用 HashAlgorithm 时使用 SHA384 作为算法名称。</li><li>SHA256 托管（不符合 FIPS）（在调用 HashAlgorithm 时使用 SHA256 作为算法名称。</li><li>SHA512CryptoServiceProvider（符合 FIPS）</li><li>SHA256CryptoServiceProvider（符合 FIPS）</li><li>SHA384CryptoServiceProvider（符合 FIPS）</li></ul>| 
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [选择加密算法](https://technet.microsoft.com/library/ms345262(v=sql.130).aspx) |
...
## <a name="ssis-packages-should-be-encrypted-and-digitally-signed"></a><a id="ssis-signed"></a>应该对 SSIS 包加密和数字签名
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [使用数字签名标识包源](https://msdn.microsoft.com/library/ms141174.aspx)、[威胁和漏洞缓解措施（集成服务）](https://msdn.microsoft.com/library/bb522559.aspx) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [SQL Server 可扩展密钥管理 (EKM)](https://msdn.microsoft.com/library/bb895340)、[使用 Azure Key Vault (SQL Server) 可扩展密钥管理](https://msdn.microsoft.com/library/dn198405) |
...
| **引用**              | [始终加密（数据库引擎）](https://msdn.microsoft.com/library/mt163865) |
...
| **引用**              | [Windows IoT Core 上的 TPM](https://developer.microsoft.com/windows/iot/docs/tpm)、[设置 Windows IoT Core 上的 TPM](https://docs.microsoft.com/windows/iot-core/secure-your-device/setuptpm)、[Azure IoT 设备 SDK TPM](https://github.com/Azure/azure-iot-hub-vs-cs/wiki/Device-Provisioning-with-TPM) |
...
## <a name="generate-a-random-symmetric-key-of-sufficient-length-for-authentication-to-iot-hub"></a><a id="random-hub"></a>生成足够长度的随机对称密钥用于向 IoT 中心进行身份验证
...
## <a name="ensure-a-device-management-policy-is-in-place-that-requires-a-pinpasswordauto-lock-and-encrypts-all-data-eg-bitlocker"></a><a id="bitlocker"></a>确保设备管理策略到位，需要 PIN/密码/自动锁定并加密所有数据（例如 BitLocker）
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [标识服务器 - 密钥、签名和加密](https://identityserver.github.io/Documentation/docsv2/configuration/crypto.html) |
| **步骤** | 使用标识服务器时确保滚动更新签名密钥。 参考部分中的链接说明了如何在不中断依赖于标识服务器的应用程序的情况下规划密钥滚动更新。 |

## <a name="ensure-that-cryptographically-strong-client-id-client-secret-are-used-in-identity-server"></a><a id="client-server"></a>确保在身份服务器中使用加密强客户端 ID、客户端密钥
