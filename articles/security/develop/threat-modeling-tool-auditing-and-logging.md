...
| **Web 应用程序** | <ul><li>[确保对应用程序强制执行审核和日志记录](#auditing)</li><li>[确保已准备好日志旋转和分离](#log-rotation)</li><li>[确保应用程序不会记录敏感的用户数据](#log-sensitive-data)</li><li>[确保对审核与日志文件限制访问](#log-restricted-access)</li><li>[确保记录用户管理事件](#user-management)</li><li>[确保在系统中针对滥用提供内置防御机制](#inbuilt-defenses)</li><li>[在 Azure 应用服务中启用 Web 应用的诊断日志记录](#diagnostics-logging)</li></ul> |
...
| **IoT 云网关** | <ul><li>[确保在云网关上强制实施适当的审核和日志记录](#logging-cloud-gateway)</li></ul> |
