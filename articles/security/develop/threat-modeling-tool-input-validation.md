...
ms.locfileid: "68728063"
---
# <a name="security-frame-input-validation--mitigations"></a>安全框架：输入验证 | 缓解措施 
| 产品/服务 | 项目 |
| --------------- | ------- |
| **Web 应用程序** | <ul><li>[使用不受信任的样式表禁用所有转换的 XSLT 脚本](#disable-xslt)</li><li>[确保可能包含用户可控内容的每个页面能够选择不使用自动 MIME 探查](#out-sniffing)</li><li>[强化或禁用 XML 实体解析](#xml-resolution)</li><li>[使用 http.sys 的应用程序执行 URL 规范化验证](#app-verification)</li><li>[确保接受用户的文件时实施适当的控制](#controls-users)</li><li>[确保在 Web 应用程序中使用类型安全的参数进行数据访问](#typesafe)</li><li>[使用单独的模型绑定类或绑定筛选列表来防止 MVC 大规模分配漏洞](#binding-mvc)</li><li>[在呈现之前为不受信任的 Web 输出编码](#rendering)</li><li>[对所有字符串类型 模型属性执行输入验证和筛选](#typemodel)</li><li>[应该针对接受所有字符的表单域（例如 RTF 编辑器）应用净化](#richtext)</li><li>[不要将 DOM 元素分配给没有内置编码的接收器](#inbuilt-encode)</li><li>[验证应用程序内的所有重定向均已关闭或安全完成](#redirect-safe)</li><li>[针对 Controller 方法接受的所有字符串类型参数实施输入验证](#string-method)</li><li>[设置正则表达式处理的上限超时，以防止由于正则表达式错误而导致的 DoS](#dos-expression)</li><li>[避免在 Razor 视图中使用 Html.Raw](#html-razor)</li></ul> | 
| **数据库** | <ul><li>[请勿在存储过程中使用动态查询](#stored-proc)</li></ul> |
| **Web API** | <ul><li>[确保在 Web API 方法上完成模型验证](#validation-api)</li><li>[对 Web API 方法接受的所有字符串类型参数实现输入验证](#string-api)</li><li>[确保 Web API 中使用类型安全参数进行数据访问](#typesafe-api)</li></ul> | 
| **Azure Document DB** | <ul><li>[对 Azure Cosmos DB 使用参数化 SQL 查询](#sql-docdb)</li></ul> | 
| **WCF** | <ul><li>[通过架构绑定进行 WCF 输入验证](#schema-binding)</li><li>[WCF - 通过参数检查器执行输入验证](#parameters)</li></ul> |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [XSLT 安全](https://msdn.microsoft.com/library/ms763800(v=vs.85).aspx)、[XsltSettings.EnableScript 属性](https://msdn.microsoft.com/library/system.xml.xsl.xsltsettings.enablescript.aspx) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [IE8 安全性第五部分 - 全面保护](https://blogs.msdn.com/ie/archive/2008/07/02/ie8-security-part-v-comprehensive-protection.aspx)  |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [不受限制的文件上传](https://www.owasp.org/index.php/Unrestricted_File_Upload)、[文件签名表](https://www.garykessler.net/library/file_sigs.html) |
...
| **属性**              | 空值  |
| **引用**              | [如何在 ASP.NET 中防止跨站点脚本](https://msdn.microsoft.com/library/ms998274.aspx)、[跨站点脚本](https://cwe.mitre.org/data/definitions/79.html)、[XSS（跨站点脚本）预防速查表](https://www.owasp.org/index.php/XSS_(Cross_Site_Scripting)_Prevention_Cheat_Sheet) |
| **步骤** | 跨站点脚本（通常缩写为 XSS）是针对联机服务或者使用 Web 输入的任何应用程序/组件的攻击向量。 攻击者可能会利用 XSS 漏洞，通过有漏洞的 Web 应用程序在另一个用户的计算机上执行脚本。 使用恶意脚本可以窃取 Cookie，或者通过 JavaScript 篡改受害者的计算机。 通过验证用户输入，确保在网页中呈现之前其格式正确并经过编码，可以阻止 XSS。 可以使用 Web 保护库执行输入验证和输出编码。 对于托管代码（C、VB.NET\#等），根据用户输入的显示上下文，使用 Web 保护 （反 XSS） 库中的一种或多种适当的编码方法：| 
...
| **适用的技术** | 泛型、MVC5、MVC6 |
| **属性**              | 空值  |
| **引用**              | [添加验证](https://www.asp.net/mvc/overview/getting-started/introduction/adding-validation)、[验证 MVC 应用程序中的模型数据](https://msdn.microsoft.com/library/dd410404(v=vs.90).aspx)、[ASP.NET MVC 应用程序指导原则](https://msdn.microsoft.com/magazine/dd942822.aspx) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [为不安全的输入编码](https://msdn.microsoft.com/library/ff647397.aspx#paght000003_step3)、[HTML 净化器](https://github.com/mganss/HtmlSanitizer) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [OAuth 2.0 授权框架 - 开放重定向程序](https://tools.ietf.org/html/rfc6749#section-10.15) |
...
| **适用的技术** | 泛型、MVC5、MVC6 |
| **属性**              | 空值  |
| **引用**              | [验证 MVC 应用程序中的模型数据](https://msdn.microsoft.com/library/dd410404(v=vs.90).aspx)、[ASP.NET MVC 应用程序指导原则](https://msdn.microsoft.com/magazine/dd942822.aspx) |
...
| **适用的技术** | 泛型、Web 窗体、MVC5、MVC6  |
| **属性**              | 空值  |
| **引用**              | [默认Regex匹配超时属性](https://msdn.microsoft.com/library/system.web.configuration.httpruntimesection.defaultregexmatchtimeout.aspx) |
...
| **属性**              | 空值  |
| **引用**              | 空值  |
| 步骤 | ASP.NET网页 （Razor） 执行自动 HTML 编码。 嵌入式代码片段（@ 块）打印的所有字符串会自动进行 HTML 编码。 但是，调用 `HtmlHelper.Raw` 方法时，将返回未经 HTML 编码的标记。 如果使用 `Html.Raw()` 帮助器方法，它会绕过 Razor 提供的自动编码保护。|
...
| **适用的技术** | MVC5、MVC6 |
| **属性**              | 空值  |
| **引用**              | [ASP.NET Web API 中的模型验证](https://www.asp.net/web-api/overview/formats-and-model-binding/model-validation-in-aspnet-web-api) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [宣布推出 Azure Cosmos DB 中的 SQL 参数化](https://azure.microsoft.com/blog/announcing-sql-parameterization-in-documentdb/) |
