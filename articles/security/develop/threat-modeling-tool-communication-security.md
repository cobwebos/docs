...
| **Azure 数据工厂** | <ul><li>[将本地 SQL 服务器连接到 Azure 数据工厂时使用数据管理网关](#sqlserver-factory)</li></ul> |
...
| **数据库** | <ul><li>[确保 SQL 服务器连接加密和证书验证](#sqlserver-validation)</li><li>[强制以加密形式来与 SQL Server 通信](#encrypted-sqlserver)</li></ul> |
| **Azure 存储** | <ul><li>[确保与 Azure 存储之间的通信通过 HTTPS 进行](#comm-storage)</li><li>[如果无法启用 HTTPS，则在下载 blob 后验证 MD5 哈希](#md5-https)</li><li>[使用 SMB 3.0 兼容客户端确保对 Azure 文件共享的传输中数据加密](#smb-shares)</li></ul> |
...
| **用于 Redis 的 Azure 缓存** | <ul><li>[确保与 Redis 的 Azure 缓存的通信通过 SSL](#redis-ssl)</li></ul> |
...
## <a name="check-service-account-privileges-and-check-that-the-custom-services-or-aspnet-pages-respect-crms-security"></a><a id="priv-aspnet"></a>检查服务帐户特权，并检查自定义服务或 ASP.NET 页面是否遵循 CRM 的安全性
...
## <a name="use-data-management-gateway-while-connecting-on-premises-sql-server-to-azure-data-factory"></a><a id="sqlserver-factory"></a>将本地 SQL 服务器连接到 Azure 数据工厂时使用数据管理网关
...
| **适用的技术** | 泛型 |
| **属性**              | 链接服务类型 - Azure 和本地 |
| **引用**              |[在本地和 Azure 数据工厂之间移动数据](https://azure.microsoft.com/documentation/articles/data-factory-move-data-between-onprem-and-cloud/#create-gateway)，[数据管理网关](https://azure.microsoft.com/documentation/articles/data-factory-data-management-gateway/) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [IdentityServer3 - 密钥、签名和加密](https://identityserver.github.io/Documentation/docsv2/configuration/crypto.html)、[IdentityServer3 - 部署](https://identityserver.github.io/Documentation/docsv2/advanced/deployment.html) |
...
## <a name="configure-ssl-certificate-for-custom-domain-in-azure-app-service"></a><a id="ssl-appservice"></a>在 Azure 应用服务中为自定义域配置 SSL 证书
...
| **引用**              | [为 Azure 应用服务中的应用启用 HTTPS](../../app-service/configure-ssl-bindings.md) |
...
| **适用的技术** | 泛型 |
| **属性**              | EnvironmentType - Azure |
| **引用**              | [对 Azure 应用服务强制执行 HTTPS](../../app-service/configure-ssl-bindings.md#enforce-https) |
...
## <a name="enable-http-strict-transport-security-hsts"></a><a id="http-hsts"></a>启用 HTTP 严格传输安全性 (HSTS)
...
| **适用的技术** | SQL Azure  |
| **属性**              | SQL 版本 - V12 |
| **引用**              | [有关为 SQL 数据库编写安全连接字符串的最佳做法](https://social.technet.microsoft.com/wiki/contents/articles/2951.windows-azure-sql-database-connection-security.aspx#best) |
...
| **引用**              | [启用数据库引擎的加密连接](https://msdn.microsoft.com/library/ms191192)  |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [Azure 存储传输级加密 – 使用 HTTPS](https://azure.microsoft.com/documentation/articles/storage-security-guide/#_encryption-in-transit) |
...
| **适用的技术** | 泛型 |
| **属性**              | StorageType - Blob |
| **引用**              | [Windows Azure Blob MD5 概述](https://blogs.msdn.microsoft.com/windowsazurestorage/2011/02/17/windows-azure-blob-md5-overview/) |
...
| **适用的技术** | 泛型 |
| **属性**              | StorageType - 文件 |
| **引用**              | [Azure 文件存储](https://azure.microsoft.com/blog/azure-file-storage-now-generally-available/#comment-2529238931)、[Windows 客户端的 Azure 文件存储 SMB 支持](https://azure.microsoft.com/documentation/articles/storage-dotnet-how-to-use-files/#_mount-the-file-share) |
...
| **属性**              | 空值  |
| **引用**              | [证书和公钥绑定](https://www.owasp.org/index.php/Certificate_and_Public_Key_Pinning#.Net) |
...
## <a name="enable-https---secure-transport-channel"></a><a id="https-transport"></a>启用 HTTPS - 安全传输通道
...
| **属性**              | 空值  |
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx)、[巩固王国](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_transport_security_enabled) |
...
| **适用的技术** | .NET Framework 3 |
| **属性**              | 空值  |
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff650862.aspx) |
| **步骤** | <ul><li>**说明：** 当保护级别设置为“none”时，将禁用消息保护。 保密性和完整性是使用适当的设置级别实现的。</li><li>**建议：**<ul><li>当 `Mode=None` 时 - 禁用消息保护</li><li>当 `Mode=Sign` 时 - 将消息签名但不加密；当数据完整性非常重要时应使用该设置</li><li>当 `Mode=EncryptAndSign` 时 - 将消息签名并加密</li></ul></li></ul><p>请考虑禁用加密，仅当只是需要验证信息的完整性而不关心机密性时，才为消息签名。 对于需要验证原始发送者但不传输任何敏感数据的操作或服务约定，这种做法可能很有用。 降低保护级别时，请注意消息不包含任何个人数据。</p>|
...
## <a name="wcf-use-a-least-privileged-account-to-run-your-wcf-service"></a><a id="least-account-wcf"></a>WCF：使用最低特权帐户运行 WCF 服务
...
| **适用的技术** | .NET Framework 3 |
| **属性**              | 空值  |
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff648826.aspx ) |
...
| **适用的技术** | MVC5、MVC6 |
| **属性**              | 空值  |
| **引用**              | [在 Web API 控制器中强制 SSL](https://www.asp.net/web-api/overview/security/working-with-ssl-in-web-api) |
...
## <a name="ensure-that-communication-to-azure-cache-for-redis-is-over-ssl"></a><a id="redis-ssl"></a>确保与 Azure Redis 缓存之间的通信通过 SSL 进行
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [Azure Redis SSL 支持](https://azure.microsoft.com/documentation/articles/cache-faq/#when-should-i-enable-the-non-ssl-port-for-connecting-to-redis) |
...
## <a name="secure-device-to-field-gateway-communication"></a><a id="device-field"></a>保护设备与现场网关之间的通信
...
## <a name="secure-device-to-cloud-gateway-communication-using-ssltls"></a><a id="device-cloud"></a>使用 SSL/TLS 保护设备与云网关之间的通信
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [选择通信协议](https://azure.microsoft.com/documentation/articles/iot-hub-devguide/#messaging) |
