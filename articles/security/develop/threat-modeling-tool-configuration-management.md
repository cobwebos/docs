...
| **适用的技术** | 泛型 |
| **属性**              | N/A  |
| **参考**              | [ASP.NET 调试概述](https://msdn.microsoft.com/library/ms227556.aspx)、[ASP.NET 跟踪概述](https://msdn.microsoft.com/library/bb386420.aspx)、[如何：为 ASP.NET 应用程序启用跟踪](https://msdn.microsoft.com/library/0x5wc973.aspx)、[如何：为 ASP.NET 应用程序启用调试](https://msdn.microsoft.com/library/e8z01xdh(VS.80).aspx) |
...
## <a id="validate-aspnet"></a>在 ASP.NET 页面中启用 ValidateRequest 属性
...
| **步骤** | 防火墙系统可帮助阻止对计算机资源进行未经授权的访问。 要通过防火墙访问 SQL Server 数据库引擎的实例，必须将运行 SQL Server 的计算机上的防火墙配置为允许这种访问 |
...
## <a id="field-bit-locker"></a>使用 bit-locker 加密 IoT 现场网关的 OS 和其他分区
