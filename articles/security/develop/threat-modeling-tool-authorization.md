...
| 产品/服务 | 项目 |
| --------------- | ------- |
| **计算机信任边界** | <ul><li>[确保配置适当的 Acl 来限制对设备上的数据进行未经授权的访问](#acl-restricted-access)</li><li>[确保将特定于用户的敏感应用程序内容存储在用户配置文件目录中](#sensitive-directory)</li><li>[确保以最低特权运行部署的应用程序](#deployed-privileges)</li></ul> |
| **Web 应用程序** | <ul><li>[处理业务逻辑流时强制执行顺序步骤顺序](#sequential-logic)</li><li>[实施频率限制机制来防止枚举](#rate-enumeration)</li><li>[确保实施适当的授权并遵循最低权限原则](#principle-least-privilege)</li><li>[业务逻辑和资源访问授权决策不应该基于传入的请求参数](#logic-request-parameters)</li><li>[确保内容和资源无法通过强制浏览进行枚举或访问](#enumerable-browsing)</li></ul> |
| **Database** | <ul><li>[确保使用最低特权帐户连接到数据库服务器](#privileged-server)</li><li>[实现行级别安全性 RLS 来防止租户访问彼此的数据](#rls-tenants)</li><li>[Sysadmin 角色应该只包括必要的有效用户](#sysadmin-users)</li></ul> |
...
| **Dynamics CRM 门户** | <ul><li>[执行门户帐户的安全建模，请注意门户的安全模型与 CRM 的其余部分不同](#portal-security)</li></ul> |
...
| **Web API** | <ul><li>[在 ASP.NET Web API 中实现适当的授权机制](#authz-aspnet)</li></ul> |
...
| **特性**              | SQL 版本 - V12，SQL 版本 - MsSQL2016 |
...
| **步骤** | 在某些业务方案中，Azure 表存储可能需要存储面向不同当事方的敏感数据。 例如，与不同国家/地区相关的敏感数据。 在这种情况下，可以通过指定分区和行键范围来构建 SAS 签名，使用户能够访问特定国家/地区的数据。| 
...
| **SDL 阶段**               | 构建 |  
| **适用的技术** | 泛型、MVC5 |
| **特性**              | 不适用，标识提供者 - ADFS，标识提供程序 - Azure AD |
