...
ms.locfileid: "68728381"
---
# <a name="security-frame-authorization--mitigations"></a>安全框架：授权 | 缓解措施 
| 产品/服务 | 项目 |
| --------------- | ------- |
| **计算机信任边界** | <ul><li>[确保配置了适当的 ACL 以限制对设备上数据的未授权访问](#acl-restricted-access)</li><li>[确保敏感用户特定的应用程序内容存储在用户配置文件目录中](#sensitive-directory)</li><li>[确保以最低特权运行部署的应用程序](#deployed-privileges)</li></ul> |
| **Web 应用程序** | <ul><li>[在处理业务逻辑流时强制执行顺序步进顺序](#sequential-logic)</li><li>[实施频率限制机制来防止枚举](#rate-enumeration)</li><li>[确保适当的授权到位，并遵循最低特权原则](#principle-least-privilege)</li><li>[业务逻辑和资源访问授权决策不应基于传入请求参数](#logic-request-parameters)</li><li>[确保内容和资源不会通过强制浏览进行枚举或访问](#enumerable-browsing)</li></ul> |
| **数据库** | <ul><li>[确保使用最低特权帐户连接到数据库服务器](#privileged-server)</li><li>[实现行级安全 RLS，以防止租户访问彼此的数据](#rls-tenants)</li><li>[Sysadmin 角色应该只包括必要的有效用户](#sysadmin-users)</li></ul> |
...
| **Dynamics CRM 门户** | <ul><li>[执行门户帐户的安全建模，同时牢记门户的安全模型与 CRM 的其余部分不同](#portal-security)</li></ul> |
...
| **WCF** | <ul><li>[WCF 中的弱类引用](#weak-class-wcf)</li><li>[WCF-实现授权控制](#wcf-authz)</li></ul> |
| **Web API** | <ul><li>[在ASP.NET Web API 中实施适当的授权机制](#authz-aspnet)</li></ul> |
| **IoT 设备** | <ul><li>[如果设备支持需要不同权限级别的各种操作，应在设备中执行授权检查](#device-permission)</li></ul> |
| **IoT 现场网关** | <ul><li>[如果现场网关支持需要不同权限级别的各种操作，则在现场网关中执行授权检查](#field-permission)</li></ul> |
...
## <a name="implement-rate-limiting-mechanism-to-prevent-enumeration"></a><a id="rate-enumeration"></a>实施频率限制机制来防止枚举
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [SQL 数据库权限层次结构](https://msdn.microsoft.com/library/ms191465)、[SQL 数据库安全对象](https://msdn.microsoft.com/library/ms190401) |
...
| **引用**              | [SQL Server 行级别安全性 (RLS)](https://msdn.microsoft.com/library/azure/dn765131.aspx) |
...
| **引用**              | [Iot 集线器访问控制](https://azure.microsoft.com/documentation/articles/iot-hub-devguide/#Security) |
| **步骤** | 向连接到云网关（IoT 中心）的各种组件提供最低特权。 典型示例 – 设备管理/预配组件使用 registryread/write，事件处理器 (ASA) 使用服务连接。 各个设备使用设备凭据建立连接|

## <a name="use-a-send-only-permissions-sas-key-for-generating-device-tokens"></a><a id="sendonly-sas"></a>使用拥有仅限发送权限的 SAS 密钥来生成设备令牌
...
## <a name="connect-to-event-hub-using-sas-keys-that-have-the-minimum-permissions-required"></a><a id="sas-minimum-permissions"></a>使用拥有所需最低权限的 SAS 密钥连接到事件中心
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [使用角色分配管理对 Azure 订阅资源的访问权限](https://azure.microsoft.com/documentation/articles/role-based-access-control-configure/)  |
...
| **适用的技术** | 泛型 |
| **属性**              | 环境 - Azure |
| **引用**              | [适用于 Service Fabric 客户端的基于角色的访问控制](https://azure.microsoft.com/documentation/articles/service-fabric-cluster-security-roles/) |
...
| **适用的技术** | 泛型 |
| **属性**              | StorageType - 表 |
| **引用**              | [如何使用 SAS 来委派对 Azure 存储帐户中对象的访问权限](https://azure.microsoft.com/documentation/articles/storage-security-guide/#_data-plane-security) |
| **步骤** | 在某些业务方案中，Azure 表存储可能需要存储面向不同当事方的敏感数据。 例如，与不同国家/地区相关的敏感数据。 在这种情况下，可以通过指定分区和行键范围来构造 SAS 签名，以便用户可以访问特定于特定国家/地区的数据。| 
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [如何使用基于角色的访问控制 (RBAC) 来保护存储帐户](https://azure.microsoft.com/documentation/articles/storage-security-guide/#management-plane-security) |
...
## <a name="implement-implicit-jailbreak-or-rooting-detection"></a><a id="rooting-detection"></a>实施隐式越狱或 root 检测
...
| **属性**              | 空值  |
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx)、[巩固王国](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_weak_class_reference) |
...
| **引用**              | [ASP.NET Web API 中的身份验证和授权](https://www.asp.net/web-api/overview/security/authentication-and-authorization-in-aspnet-web-api) |
