...
| DisableRemediation | boolean | ScanType = CustomScan | False | true |  | 如果选中：1），则忽略文件排除项。 2）扫描存档文件。 3）检测后不应用操作。 4）检测后不写入事件日志项。 5）自定义扫描的检测不显示在用户界面中。 6）控制台输出将显示自定义扫描的检测列表。
| BootSectorScan | boolean | ScanType = CustomScan | False | false |  | 如果选中，将启用启动扇区扫描。
| 参数 | 字符串 | InputType = Custom | True | -ScanType DisableRemediation-File $ （Build.stagingdirectory） |  | 命令行参数，其中-File 的参数是一个绝对路径，或者是在生成代理上预定义的 $ （Build.stagingdirectory）的相对路径。 注意：如果未提供自变量作为最后一个参数，则默认为 $ （Build.stagingdirectory）。 你还可以提供 MpCmdRun 工具允许的自变量。<br/><br/>有关此工具的命令行参数的更多详细信息，请输入<strong>-h</strong>或<strong>-？</strong> 并执行生成任务。
...
| 参数 | 字符串 | InputType = 命令行 | True |  |  | 要执行的标准 Binskim 命令行参数。 将删除并替换输出路径。<br>有关此工具的命令行参数的更多详细信息，请在 "参数" 字段中输入**帮助**，然后执行生成任务。
...
| AnalyzeSymPath | 字符串 | InputType = Basic && 函数 = 分析 | False |  |  | 目标的符号文件的路径。
| AnalyzeConfigPath | 字符串 | InputType = Basic && 函数 = 分析 | False | default |  | 策略文件的路径，该文件将用于配置分析。 传递值 "default" 以使用内置设置。
| AnalyzePluginPath | 字符串 | InputType = Basic && 函数 = 分析 | False |  |  | 将对分析集中的所有目标调用的插件的路径。
| AnalyzeRecurse | boolean | InputType = Basic && 函数 = 分析 | False | true |  | 在计算文件说明符参数时递归到子目录中。
...
| DumpRecurse | boolean | InputType = Basic && 函数 = 转储 | False | true |  | 在计算文件说明符参数时递归到子目录中。
| DumpVerbose | boolean | InputType = Basic && 函数 = 转储 | False | true |  | 发出详细输出。 生成的综合性报表旨在提供符合性方案的相应证据。
...
| batchSize | 字符串 | 通用 | False |  |  | 用于并行运行凭据扫描器的并发线程数。 （默认值为20）<br/>值必须在1-2147483647 范围内。
| regexMatchTimeoutInSeconds | 字符串 | 通用 | False |  |  | 在放弃检查之前，尝试搜索者匹配所用的时间（以秒为单位）。<br/>添加``-Co RegexMatchTimeoutInSeconds=<Value>``到命令行。
| fileScanReadBufferSize | 字符串 | 通用 | False |  |  | 读取内容时缓冲区大小（以字节为单位）。 （默认值为524288）<br/>添加``-Co FileScanReadBufferSize=<Value>``到命令行。
| maxFileScanReadBytes | 字符串 | 通用 | False |  |  | 内容分析期间要从给定文件中读取的最大字节数。 （默认值为104857600）<br/>添加``-Co MaxFileScanReadBytes=<Value>``到命令行。
...
| AccountId | 字符串 | 通用 | True |  |  | 标识帐户的 GUID。 可以从帐户 URL 检索它。
| BinariesURL | 字符串 | 通用 | True |  |  | 要由模糊化计算机用来下载二进制文件的以分号分隔的 Url 列表。 确保 Url 可公开使用。
| SeedsURL | 字符串 | 通用 | False |  |  | 要由模糊化计算机用来下载种子的以分号分隔的 Url 列表。 请确保 Url 是公开的。
| OSPlatformType | 列表 | 通用 | True | Windows | Linux、Windows | 要在其上运行模糊化作业的计算机的 OS 平台类型。
| WindowsEdition | 字符串 | OSPlatformType = Windows | True | Server 2008 R2 |  | 要在其上运行模糊化作业的计算机的 OS 版本。
| LinuxEdition | 字符串 | OSPlatformType = Linux | True | Redhat |  | 要在其上运行模糊化作业的计算机的 OS 版本。
| PreSubmissionCommand | 字符串 | 通用 | False |  |  | 需要在测试计算机上运行的脚本，以便在提交模糊化作业之前安装测试目标程序及其依赖项。
| SeedDirectory | 字符串 | 通用 | True |  |  | 包含种子的模糊化计算机上的目录的路径。 有关详细信息，请参阅[Seed File Directory](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/03-choose-seed-files#seed-file-directory) 。
| SeedExtension | 字符串 | 通用 | True |  |  | 种子的文件扩展名。
| TestDriverExecutable | 字符串 | 通用 | True |  |  | 模糊计算机上目标可执行文件的路径。 有关详细信息，请参阅[EPE 的完整路径](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/02-choose-exe#full-path-to-the-epe)。
| TestDriverExeType | 列表 | 通用 | True | x86 | amd64，x86 | 目标可执行文件体系结构。
| TestDriverParameters | 字符串 | 通用 | True | "% testfile.txt%" |  | 传递给测试目标可执行文件的命令行参数。 请注意，会自动将 **"% testfile.txt%"** 符号（包括双引号）替换为测试驱动程序预期要分析的目标文件的完整路径，并且是必需的。 有关详细信息，请参阅[命令行参数](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/02-choose-exe#command-line-arguments)。
| ClosesItself | boolean | 通用 | True | true |  | 检查测试驱动程序是否在完成后终止自身（例如，测试驱动程序将分析输入文件并立即退出）;如果需要强行关闭测试驱动程序（例如，测试驱动程序是一种 GUI 应用程序，在分析输入后，其主窗口仍保持打开状态）。 有关详细信息，请参阅[自我终止](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/05-scope-exe-lifetime#self-termination)。
| MaxDurationInSeconds | 字符串 | 通用 | True | 5 |  | 测试驱动程序的最长持续时间（秒）。 提供目标程序分析输入文件所需的最长预期时间的估计。 此估计越精确，模糊化运行的效率就越高。 有关详细信息，请参阅[预期的最大执行持续时间](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/05-scope-exe-lifetime#maximum-expected-execution-duration)。
| CanRunRepeat | boolean | 通用 | True | true |  | 检查是否可以重复运行测试驱动程序，而无需依赖于持久化/shared 全局状态。 有关详细信息，请参阅[从头开始运行](https://docs.microsoft.com/security-risk-detection/how-to/submit-windows-fuzzing-job/04-describe-exe-characteristics#runs-from-scratch)。
...
| msBuildCommandline | 字符串 | userProvideBuildInfo = = msBuildInfo | True |  |  | 用于编译解决方案或项目的完整生成命令行。<br/><br/>注意：命令行应以**msbuild.exe**或**dotnet**的完整路径开头。<br/>将使用 $ （Build.sourcesdirectory）作为工作目录运行该命令。
...
| customRuleset | 字符串 | rulesetName = Custom | False |  |  | 要使用的规则集的可访问路径。 相对路径将规范化为源存储库（`$(Build.SourcesDirectory)`）的根。<br/><br/>如果规则集指定`Rules` ， `Actions`并将`Error`设置为，则生成任务将失败。 若要使用这样做的规则集，请`Continue on error`签入生成任务的`Control Options`。
...
| RulesDirectory | 字符串 | RuleLibrary = = custom | True |  |  | 一个可访问目录，其中包含可在 TSLint 中使用的其他 TSLint 规则。
...
| 项目 | 字符串 | FileSelectionType = = projectFile | True |  |  | [Tsconfig.json](http://www.typescriptlang.org/docs/handbook/tsconfig-json.html)文件的路径，该文件指定要在其上运行 TSLint 的 TypeScript 文件。 路径相对于`Build.SourcesDirectory`值。
| TypeCheck | boolean | RuleLibrary！ = microsoft && FileSelectionType = = projectFile | False | true |  | 在运行 linting 规则时启用类型检查器。
| ExcludeFiles | 字符串 | 通用 | False |  |  | 指示要从 linting 中排除的文件的[glob](https://www.npmjs.com/package/glob) 。 路径相对于`Build.SourcesDirectory`值。 可以指定多个值，用分号分隔。
...
| NodeMemory | 字符串 | 通用 | False |  |  | 要分配给节点以运行 TSLint 的显式内存量（Mb）。 示例：8000<br/><br/>映射到节点`--max_old_space=<value>`的 CLI 选项，这是`v8 option`。
...
| TypeScriptVersionCustom | 字符串 | TypeScriptVersion = = custom | True | 最新 |  | 要下载和使用的[typescript](https://www.npmjs.com/package/typescript)版本。<br/>**注意：** 这需要与用于编译代码的 TypeScript 版本相同。
...
| TargetPath | 字符串 | ArtifactType = FilePath | False | \\my\share\$（DefinitionName）<br>\$（BuildNumber） |  | 要将文件复制到的文件共享
| AllTools | boolean | 通用 | True | true |  | 发布所有安全开发工具生成任务生成的结果。
| 反恶意软件 | boolean | AllTools = false | True | true |  | 发布反恶意软件生成任务生成的结果。
| BinSkim | boolean | AllTools = false | True | true |  | 发布 BinSkim 生成任务生成的结果。
| CredScan | boolean | AllTools = false | True | true |  | 发布由 Credential 扫描器生成任务生成的结果。
| MSRD | boolean | AllTools = false | True | true |  | 为 MSRD 生成任务启动的 MSRD 作业发布作业信息和作业 url。 MSRD 作业长时间运行，并提供单独的报表。
| RoslynAnalyzers | boolean | AllTools = false | True | false |  | 发布 Roslyn 分析器生成任务生成的结果。
| TSLint | boolean | AllTools = false | True | true |  | 发布 TSLint 生成任务生成的结果。 请注意，报表仅支持 JSON 格式的 TSLint 日志。 如果选择了不同的格式，请相应地更新 TSLint 生成任务。
| ToolLogsNotFoundAction | 列表 | 通用 | True | 标准 | 错误，无，标准，警告 | 当找不到所选工具（或所有工具均已选中）的日志时要执行的操作，表示此工具未运行。<br/><br/>**选项：**<br/>**无：** 将消息写入到详细输出流，只能通过将 VSTS 变量**system**设置为**true**。<br/>**Standard：** （默认值）写入标准输出消息，该消息未找到该工具的日志。<br/>**警告：** 写入一个黄色警告消息，该工具未找到该工具的任何日志，这会在 "生成摘要" 页上显示为警告。<br/>**错误：** 写入红色错误消息并引发异常，从而中断生成。 使用此选项可确保使用各个工具选项来确保运行的工具。
...
| TsvFile | boolean | 通用 | False | true |  | 生成一个 tsv 文件（制表符分隔值），每个查找结果对应一行，并为结果分隔信息。
| HtmlFile | boolean | 通用 | False | true |  | 生成 html 报表文件。
...
| ToolLogsNotFoundAction | 列表 | 通用 | True | 标准 | 错误，无，标准，警告 | 当找不到所选工具（或所有工具均已选中）的日志时要执行的操作，表示此工具未运行。<br/><br/>**选项：**<br/>**无：** 将消息写入到详细输出流，只能通过将 VSTS 变量**system**设置为**true**。<br/>**Standard：** （默认值）写入标准输出消息，该消息未找到该工具的日志。<br/>**警告：** 写入一个黄色警告消息，该工具未找到该工具的任何日志，这会在 "生成摘要" 页上显示为警告。<br/>**错误：** 写入红色错误消息并引发异常，从而中断生成。 使用此选项可确保使用各个工具选项来确保运行的工具。
| CustomLogsFolder | 字符串 | 通用 | False |  |  | 分析工具日志所在的基文件夹;单个日志文件将位于此路径下的每个工具之后名为的子文件夹中。
