...
| **Web 应用程序** | <ul><li>[确保不要在浏览器中缓存敏感内容](#cache-browser)</li><li>[加密包含敏感数据的 Web 应用配置文件的部分](#encrypt-data)</li><li>[在敏感窗体和输入中显式禁用自动填充 HTML 特性](#autocomplete-input)</li><li>[确保用户屏幕上显示的敏感数据经过屏蔽](#data-mask)</li></ul> | 
| **数据库** | <ul><li>[实施动态数据屏蔽，限制透露给非特权用户的敏感数据](#dynamic-users)</li><li>[确保以加盐哈希格式存储密码](#salted-hash)</li><li>[确保数据库列中的敏感数据已加密](#db-encrypted)</li><li>[确保启用数据库级加密 (TDE)](#tde-enabled)</li><li>[确保数据库备份经过加密](#backup)</li></ul> | 
...
| **Azure IaaS VM 信任边界** | <ul><li>[使用 Azure 磁盘加密加密虚拟机使用的磁盘](#disk-vm)</li></ul> | 
...
| **移动客户端** | <ul><li>[加密写入到手机本地存储的敏感或 PII 数据](#pii-phones)</li><li>[在分发到最终用户之前，对生成的二进制文件进行模糊处理](#binaries-end)</li></ul> | 
| **WCF** | <ul><li>[将客户端凭据类型设置为证书或 Windows](#cert)</li><li>[WCF - 安全模式未启用](#security)</li></ul> | 
...
| **属性**              | 空值  |
| **引用**              | 空值  |
| **步骤** | 浏览器可能会出于缓存和历史记录的目的存储信息。 这些缓存的文件存储在某个文件夹中，例如，如果使用的是 Internet Explorer，该文件夹为“Internet 临时文件”。 再次浏览这些页面时，浏览器会从缓存显示这些页面。 如果向用户显示敏感信息（如其地址、信用卡详细信息、社会保险号或用户名），则此信息可以存储在浏览器的缓存中，因此可以通过检查浏览器的缓存或通过只需按下浏览器的"后退"按钮。 将缓存控制响应标头值设置为所有页面的"无存储"。 |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [如何：使用 DPAPI 加密 ASP.NET 2.0 中的配置部分](https://msdn.microsoft.com/library/ff647398.aspx)、[指定受保护的配置提供程序](https://msdn.microsoft.com/library/68ze1hb2.aspx)、[使用 Azure Key Vault 保护应用程序机密](https://azure.microsoft.com/documentation/articles/guidance-multitenant-identity-keyvault/) |
...
## <a name="explicitly-disable-the-autocomplete-html-attribute-in-sensitive-forms-and-inputs"></a><a id="autocomplete-input"></a>在敏感窗体和输入中显式禁用自动填充 HTML 特性
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [MSDN：自动完成特性](https://msdn.microsoft.com/library/ms533486(VS.85).aspx)、[在 HTML 中使用自动完成](https://msdn.microsoft.com/library/ms533032.aspx)、[HTML 净化漏洞](https://technet.microsoft.com/security/bulletin/MS10-071)、[又是自动完成？](https://blog.mindedsecurity.com/2011/10/autocompleteagain.html) |
...
| **引用**              | [动态数据屏蔽](https://msdn.microsoft.com/library/mt130841) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [使用 .NET 加密 API 的密码哈希](https://docs.asp.net/en/latest/security/data-protection/consumer-apis/password-hashing.html) |
...
| **适用的技术** | 泛型 |
| **属性**              | SQL 版本 - 所有 |
| **引用**              | [加密 SQL Server 中的敏感数据](https://technet.microsoft.com/library/ff848751(v=sql.105).aspx)、[如何：加密 SQL Server 中的数据列](https://msdn.microsoft.com/library/ms179331)、[使用证书加密](https://msdn.microsoft.com/library/ms188061) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [了解 SQL Server 透明数据加密 (TDE)](https://technet.microsoft.com/library/bb934049(v=sql.105).aspx) |
...
| **引用**              | [SQL 数据库备份加密](https://msdn.microsoft.com/library/dn449489) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [使用 Azure 磁盘加密来加密虚拟机所用的磁盘](https://azure.microsoft.com/documentation/articles/storage-security-guide/#_using-azure-disk-encryption-to-encrypt-disks-used-by-your-virtual-machines) |
...
| **适用的技术** | 泛型 |
| **属性**              | 环境 - Azure |
| **引用**              | [管理 Service Fabric 应用程序中的机密](https://azure.microsoft.com/documentation/articles/service-fabric-application-secret-management/) |
...
| **适用的技术** | 通用、Xamarin  |
| **属性**              | 空值  |
| **引用**              | [使用 Microsoft Intune 策略管理设备上的设置和功能](https://docs.microsoft.com/mem/intune/configuration/)、[Keychain Valet](https://components.xamarin.com/view/square.valet) |
...
| **适用的技术** | 泛型 |
| **属性**              | 空值  |
| **引用**              | [Crypto Obfuscation For .Net](https://www.ssware.com/cryptoobfuscator/obfuscator-net.htm) |
...
| **属性**              | 安全模式 - 传输，安全模式 - 消息 |
| **引用**              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx)、[巩固王国](https://vulncat.fortify.com/en/detail?id=desc.config.dotnet.wcf_misconfiguration_weak_class_reference)、[WCF 安全基础知识 - CoDe 杂志](https://www.codemag.com/article/0611051) |
