...
| **Database** | <ul><li>[实施动态数据屏蔽，限制透露给非特权用户的敏感数据](#dynamic-users)</li><li>[确保以加盐哈希格式存储密码](#salted-hash)</li><li>[确保数据库列中的敏感数据已加密](#db-encrypted)</li><li>[确保启用数据库级加密 (TDE)](#tde-enabled)</li><li>[确保数据库备份经过加密](#backup)</li></ul> | 
...
| **Azure IaaS VM 信任边界** | <ul><li>[使用 Azure 磁盘加密来加密虚拟机使用的磁盘](#disk-vm)</li></ul> | 
...
| **移动客户端** | <ul><li>[加密写入到手机本地存储的敏感或 PII 数据](#pii-phones)</li><li>[在分发给最终用户之前，对生成的二进制文件进行模糊处理](#binaries-end)</li></ul> | 
...
| **特性**              | 空值  |
| **参考**              | 空值  |
| **步骤** | 浏览器可能会出于缓存和历史记录的目的存储信息。 这些缓存的文件存储在某个文件夹中，例如，如果使用的是 Internet Explorer，该文件夹为“Internet 临时文件”。 再次浏览这些页面时，浏览器会从缓存显示这些页面。 如果向用户显示敏感信息（如他们的地址、信用卡详细信息、社会安全号码或用户名），则可以将此信息存储在浏览器的缓存中，从而可以通过检查浏览器的缓存或只需按浏览器的 "后退" 按钮来检索。 为所有页面将缓存控制响应标头值设置为 "无商店"。 |
...
| **参考**              | [动态数据屏蔽](https://msdn.microsoft.com/library/mt130841) |
...
| **SDL 阶段**               | 构建 |  
| **适用的技术** | SQL Azure、OnPrem |
| **特性**              | SQL 版本 - V12，SQL 版本 - MsSQL2014 |
