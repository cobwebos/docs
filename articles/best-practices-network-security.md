...
[示例 1：构建外围网络 （也称为 DMZ、 外围安全区域或屏蔽子网） 来帮助保护网络安全组 (Nsg) 的应用程序。](#example-1-build-a-perimeter-network-to-help-protect-applications-with-nsgs)</br>
[示例 2：构建外围网络，以帮助通过防火墙和 Nsg 保护应用程序。](#example-2-build-a-perimeter-network-to-help-protect-applications-with-a-firewall-and-nsgs)</br>
...
[示例 4：添加混合连接与站点到站点虚拟设备虚拟专用网络 (VPN)。](#example-4-add-a-hybrid-connection-with-a-site-to-site-virtual-appliance-vpn)</br>
[示例 5：添加混合连接与站点到站点 Azure VPN 网关。](#example-5-add-a-hybrid-connection-with-a-site-to-site-azure-vpn-gateway)</br>
[示例 6:添加与 ExpressRoute 的混合连接。](#example-6-add-a-hybrid-connection-with-expressroute)</br>
未来几个月内，本文档中会加入有关在虚拟网络之间添加连接、高可用性和服务链接的示例。

## <a name="microsoft-compliance-and-infrastructure-protection"></a>Microsoft 合规性与基础结构保护
为了帮助组织符合监管个人数据收集和使用的国家、地区和行业特定要求的标准，Microsoft 提供 40 多项认证和鉴定。 最全面的集合，囊括所有云服务提供商。
...
1.    **DDoS 保护**:DDoS 保护是 Azure 保护 Azure 平台本身受到大规模的基于 Internet 的攻击的物理网络的一个层。 这些攻击利用多个“傀儡”节点尝试瘫痪 Internet 服务。 Azure 使用一个强大的 DDoS 保护网来保护所有入站、出站和跨 Azure 区域连接。 此 DDoS 保护层没有用户可配置的属性，客户也无法对其进行访问。 DDoS 保护层保护 Azure 平台免受大规模攻击，并监视出站流量和跨 Azure 区域流量。 使用 VNet 上的网络虚拟设备，可以通过客户配置其他弹性层，防止受到规模较小且不会妨碍平台级保护的攻击。 举一个进行 DDoS 攻击的例子：如果某个面向 Internet 的 IP 地址遭受大规模的 DDoS 攻击，Azure 会检测攻击源，清理带攻击性的流量，阻止其访问预期的目标。 在几乎所有情况下，受攻击的终结点不会受攻击的影响。 在极少数情况下，如果某个终结点受影响，则只影响到受攻击终结点的流量，到其他终结点的流量不受影响。 因此，其他客户和服务也不会受该攻击的影响。 必须注意的是，Azure DDoS 仅监视大规模攻击。 可能会出现平台级保护阈值尚未超出，而特定服务却已不堪重负的情况。 例如，在 Azure 平台级 DDoS 保护将某个威胁记录在册之前，DDoS 攻击可能已导致单个 A0 IIS 服务器上的网站脱机。

2.  **公共 IP 地址**:公共 IP 地址 （通过服务终结点、 公共 IP 地址、 应用程序网关和其他公共 IP 地址向 internet 路由到所需的资源的 Azure 功能已启用） 允许云服务或资源组，为具有公共 Internet IP地址和公开的端口。 终结点使用网络地址转换 (NAT) 将流量路由到 Azure 虚拟网络上的内部地址和端口。 此路径是外部流量进入虚拟网络的主要方式。 可以对公共 IP 地址进行配置，确定可以传入哪种流量、如何在虚拟网络上转换该流量以及要将它路由到何处。
...
* **多层拓扑**:虚拟网络允许客户通过分配子网并为指定的不同元素或"层"工作负荷的单独的地址空间来定义多层拓扑。 这些逻辑分组和拓扑可让客户根据工作负荷类型来定义不同的访问策略，以及控制各层之间的流量。
* **跨界连接**:客户可以在 Azure 中建立跨界虚拟网络和多个本地站点或其他虚拟网络之间的连接。 客户可以使用 VNet 对等互连、Azure VPN 网关、第三方网络虚拟设备或 ExpressRoute 来构造连接。 Azure 支持使用标准 IPsec/IKE 协议和 ExpressRoute 专用连接的站点到站点 (S2S) VPN。
...
* **网络虚拟设备**Azure Marketplace 中：Azure Marketplace 和 VM 映像库中提供了防火墙、 负载均衡器和 IDS/IPS 等安全设备。 客户可将这些设备部署到其虚拟网络，特别是安全边界（包括外围网络子网），以实现多层安全网络环境。
...
* **子网体系结构：** 指定虚拟网络，使整个子网专门作为外围网络中，同一虚拟网络中其他子网分开。 这样分隔可确保外围网络与其他内部或专用子网层之间的流量流经防火墙或 IDS/IPS 虚拟设备。  要将此流量转发到虚拟设备，需在边界子网上使用用户定义的路由。
* **NSG:** 外围网络子网本身应该打开以允许与 Internet 进行通信，但这并不意味着客户应该绕过 Nsg。 请遵循常用的安全实践，将曝露于 Internet 的网络接触面减到最小。 管制允许访问部署的远程地址范围，或特定的应用程序协议和打开的端口。 但在某些情况下，不可能实现完全锁定。 例如，如果客户在 Azure 中有外部网站，外围网络应该允许从任何公共 IP 地址，传入 web 请求，但只应该打开 web 应用程序端口：端口 80 上的 TCP 和/或端口 443 上的 TCP。
* **路由表：** 外围网络子网本身应该能够直接与 Internet 通信，但不是应该允许未通过防火墙或安全设备的后端或本地网络的直接通信。
* **安全设备配置：** 为了路由和检查外围网络和受保护的网络、 防火墙、 IDS 和 IPS 等安全设备的其余部分之间的数据包的设备可能有多个宿主。 外围网络和后端子网可能有独立的 NIC。 外围网络中的 NIC 使用相应的 NSG 和路由表直接与 Internet 相互通信。 对于相应的后端子网，连接到后端子网的 NIC 有更受限制的 NSG 和路由表。
* **安全设备功能：** 通常在外围网络中部署的安全设备执行以下功能：
  * 防火墙:实施防火墙规则或对传入请求的访问控制策略。
  * 威胁检测和防护：检测并缓解来自 Internet 的恶意攻击。
  * 审核和日志记录：维护详细的记录供审核和分析。
  * 反向代理：将传入请求重定向到相应的后端服务器。 此重定向涉及到将前端设备（通常是防火墙）上的目标地址映射并转换成后端服务器地址。
  * 正向代理：提供 NAT，并审核从虚拟网络中发起，到 Internet 的通信。
  * 路由器：转发虚拟网络中的传入和跨子网流量。
  * VPN 设备：作为客户的本地网络与 Azure 虚拟网络之间建立跨界 VPN 连接的跨界 VPN 网关。
...
* 单一边界：前端外围网络中，虚拟网络和 Internet 之间的一个。
* 两个边界：一个在外围网络中，而另一个外围网络子网和 Azure 虚拟网络中的后端子网之间的 Internet 端。
* 三个边界：一个在外围网络的 Internet 端，在外围网络与后端子网之间的一个后, 端子网与本地网络之间的一个。
* N 个边界：数目可变。 可以在给定网络中应用的安全边界没有数量限制，具体取决于安全要求。
...
## <a name="examples-building-security-boundaries-with-azure-virtual-networks"></a>示例：构建使用 Azure 虚拟网络的安全边界
...
- 单个资源组
- 包含两个子网的虚拟网络：“FrontEnd”和“BackEnd”
- 应用到这两个子网的网络安全组
...
* 单个资源组
* 具有三个子网的虚拟网络：“SecNet”、“FrontEnd”和“BackEnd”
...
* 外部规则（针对入站流量）：
  ...
  2. RDP 规则 （针对每个 Windows 服务器）：这四个规则 （每个服务器一个） 允许通过 RDP 单个服务器管理。 根据所用的网络虚拟设备功能，也可将四项 RDP 规则合并为一项规则。
  3. 应用程序流量规则：有两个这些规则针对前端 web 流量，第一个和第二个后端流量 （例如，web 服务器到数据层）。 这些规则的配置取决于网络体系结构（服务器的放置位置）和流量流动行为（流量的流动方向，以及使用的端口）。
...
* 内部规则（针对虚拟网络内部流量）
  1. 出站到 Internet 规则：此规则允许来自任何网络传递到所选网络的流量。 此规则通常是防火墙上已有的但处于禁用状态的默认规则。 对于本示例，应启用此规则。
  2. DNS 规则：此规则仅允许 DNS（端口 53）流量传递到 DNS 服务器。 在此环境中，阻止了大部分由前端流往后端的流量。 此规则专门允许来自任何本地子网的 DNS。
  3. 子网到子网规则：此规则是后端子网连接到前端子网 （但不能反向） 上的任何服务器上允许的任何服务器。
* 防故障规则（针对不符合上述任一规则的流量）：
  1. 拒绝所有流量规则：此拒绝规则始终应 （在优先级方面），最后一个规则，这种情况下如果流量的流动行为无法符合上述规则的任何它删除此规则。 此规则为默认规则，通常已设置到位并处于活动状态。 此规则通常不需做任何修改。
