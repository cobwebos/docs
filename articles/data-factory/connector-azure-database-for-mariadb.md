...
| properties | 说明 | 必选 |
|:--- |:--- |:--- |
| type | 类型属性必须设置为 **：Azure MariaDB** | 是 |
...
有关可用于定义数据集的节和属性的完整列表，请参阅[数据集](concepts-datasets-linked-services.md)一文。 本部分提供 Azure Database for MariaDB 数据集支持的属性列表。

若要从 Azure Database for MariaDB 复制数据，需要支持以下属性：

| properties | 说明 | 必选 |
|:--- |:--- |:--- |
| type | 数据集的类型属性必须设置为 **：AzureMariaDBTable** | 是 |
...
| properties | 说明 | 必选 |
|:--- |:--- |:--- |
| type | 必须将复制活动源的类型属性设置为 **：AzureMariaDBSource** | 是 |
