...
> [!NOTE]
> Parquet/分隔文本格式**AzureDataLakeStoreSource**作为仍受支持类型的复制活动源下一节中提到的是为了向后兼容。 建议以使用从长远看，此新模型和创作 UI 的 ADF 已切换为生成这些新类型。
...
> [!NOTE]
> Parquet/分隔文本格式**AzureDataLakeStoreSink**作为仍受支持类型复制活动接收器中下一节提到的是为了向后兼容。 建议以使用从长远看，此新模型和创作 UI 的 ADF 已切换为生成这些新类型。
