...
1.  添加 Azure 资源管理器部署任务：
    ...
    f.  选择 **...** 在 "**模板参数**" 框旁边，选择参数文件。 你选择的文件将取决于你是创建副本还是使用默认文件 ARMTemplateParametersForFactory。

    g.  选择 **...** 在 "**替代模板参数**" 框旁，输入目标数据工厂的信息。 对于来自 Azure Key Vault 的凭据，请在双引号之间输入密钥的名称。 例如，如果密码的名称为 cred1，请输入此值的 **"$ （cred1）"** 。
