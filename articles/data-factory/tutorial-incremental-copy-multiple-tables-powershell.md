...
---
# <a name="incrementally-load-data-from-multiple-tables-in-sql-server-to-azure-sql-database"></a>以增量方式将数据从 SQL Server 中的多个表加载到 Azure SQL 数据库
...
3. **创建包含以下活动的管道：** 
    ...
    1. 创建一个 **ForEach** 活动，循环访问一个列表，其中的源表名称是作为参数传递到管道的。 对于每个源表，它会调用以下活动，为该表执行增量加载。 
    2. 创建两个**查找**活动。 使用第一个查找活动检索最后一个水印值。 使用第二个查找活动检索新的水印值。 这些水印值会传递到复制活动。 
    3. 创建**复制活动**，用于复制源数据存储中其水印列值大于旧水印值且小于新水印值的行。 然后，该活动将源数据存储中的增量数据作为新文件复制到 Blob 存储。 
    4. 创建**存储过程活动**，用于更新下一次管道运行的水印值。 
...
* **Azure SQL 数据库**。 使用 Azure SQL 数据库作为**接收器**数据存储。 如果没有 Azure SQL 数据库，请参阅[创建 Azure SQL 数据库](../sql-database/sql-database-get-started-portal.md)一文获取创建步骤。 

### <a name="create-source-tables-in-your-sql-server-database"></a>在 SQL Server 数据库中创建源表

1. 启动 **SQL Server Management Studio**，连接到本地 SQL Server。 
...
2. 在“服务器资源管理器”中，右键单击数据库，然后选择“新建查询”。
3. 对 Azure SQL 数据库运行以下 SQL 命令，以便创建名为 `customer_table` 和 `project_table` 的表。  
...
### <a name="create-another-table-in-azure-sql-database-to-store-the-high-watermark-value"></a>在 Azure SQL 数据库中创建另一个表，用于存储高水印值
...
运行以下命令，在 Azure SQL 数据库中创建存储过程。 此存储过程在每次管道运行后更新水印的值。 
...
通过运行以下查询，在 Azure SQL 数据库中创建两个存储过程和两个数据类型：这些是用于将数据从源表合并到目标表中。
...
### <a name="create-sql-server-linked-service"></a>创建 SQL Server 链接服务。
在此步骤中，请将本地 SQL Server 链接到数据工厂。

1. 在 **C:\ADFTutorials\IncCopyMultiTableTutorial** 文件夹中创建名为 **SqlServerLinkedService.json** 的 JSON 文件，其中包含以下内容。根据连接到 SQL Server 时所使用的**身份验证**选择适当的节。 如果本地文件夹不存在，请创建。 
...
2. 在 **Azure PowerShell** 中切换到 **C:\ADFTutorials\IncCopyMultiTableTutorial** 文件夹。
...
1. 在 **C:\ADFTutorials\IncCopyMultiTableTutorial** 文件夹中，创建包含以下内容的名为 **AzureSQLDatabaseLinkedService.json** 的 JSON 文件：（如果文件夹 ADF 不存在，请创建该文件夹）。 将 **&lt;server&gt;、&lt;database name&gt;、&lt;user id&gt; 和 &lt;password&gt;** 分别替换为自己的 Azure SQL Server 名称、数据库名称、用户 ID 和密码，然后保存文件。 
...
在此步骤中，请创建表示数据源和数据目标的数据集， 以及存储水印的位置。
...
1. 在同一文件夹中，创建包含以下内容的名为 **SourceDataset.json** 的 JSON 文件： 
    ...
    表名为虚拟名称。 管道中的复制活动使用 SQL 查询来加载数据，而不是加载整个表。 
1.  运行 Set-AzureRmDataFactoryV2Dataset cmdlet 创建数据集：SourceDataset
...
1. 在同一文件夹中创建包含以下内容的名为 **SinkDataset.json** 的 JSON 文件：tableName 是由管道在运行时动态设置的。 管道中的 ForEach 活动循环访问一个包含表名的列表，每一次迭代都将表名传递到此数据集。 
...
1. 通过**查找活动**检索旧的水印值（初始值或上次迭代中使用的值）。
2. 通过**查找活动**检索新的水印值（源表中水印列的最大值）。
3. 通过**复制活动**将这两个水印值之间的数据从源数据库复制到目标数据库。 
4. 通过**存储过程活动**更新旧水印值，以便在下一迭代的第一步使用该值。 
...
1. 在同一文件夹中创建包含以下内容的参数文件 **Parameters.json**：
...
在 SQL Server Management Studio 中对目标 Azure SQL 数据库运行以下查询，验证数据是否已从源表复制到目标表。 
...
对源 SQL Server 数据库运行以下查询，在 customer_table 中更新现有行，并将新行插入到 project_table 中。 
...
2. 按照[监视管道](#monitor-the-pipeline)部分的说明监视管道运行。 当管道状态为“正在进行”时，可以在“操作”下看到另一操作链接，用于取消管道运行。 
    ...
    ![管道运行](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-pipeline-runs-6.png)    
3. 单击“刷新”对列表进行刷新，直到管道运行成功。 

    ![管道运行](media\tutorial-incremental-copy-multiple-tables-powershell\monitor-pipeline-runs-succeded-7.png)
4. （可选）单击“操作”下的“查看活动运行”链接（图标），查看与此管道运行相关联的所有活动运行。 

## <a name="review-final-results"></a>查看最终结果
...
请注意 PersonID 3 对应的 Name 和 LastModifytime 的新值。 
...
>[使用更改跟踪技术，以增量方式将 Azure SQL 数据库中的数据加载到 Azure Blob 存储](tutorial-incremental-copy-multiple-tables-powershell.md)
