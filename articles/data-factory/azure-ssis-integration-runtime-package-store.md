...
使用包部署模型时，可以选择是否要使用包存储区来预配 Azure-SSIS IR。 它们在 Azure SQL 托管实例托管的文件系统、Azure 文件或 MSDB 之上提供包管理层。 Azure-SSIS IR 包存储允许你通过 SQL Server Management Studio (SSMS) 导入/导出/删除/运行包以及监视/停止正在运行的包，类似于[旧版 SSIS 包存储](https://docs.microsoft.com/sql/integration-services/service/package-management-ssis-service?view=sql-server-2017)。 
...
连接到 SSMS 上的 Azure-SSIS IR 后，可以右键单击任何包存储、文件夹或包以弹出菜单，并选择 "**新建文件夹**"、"**导入包**"、"**导出包**"、"**删除**" 或 "**刷新**"。
   ...
   *  选择“导入包”可将包从“文件系统”、“SQL Server (MSDB)”或旧版“SSIS 包存储”导入到你的包存储中。
      ...
      > [!NOTE]
      > 将 SSIS 包导入 Azure-SSIS IR 包存储区只能逐个完成，只需将它们复制到基础 MSDB/文件系统/Azure 文件中，同时保留其 SQL Server/SSIS 版本。 
      >
      > 由于 Azure-SSIS IR 当前的默认兼容性级别为140，这等于**SQL Server 2017**，因此，在其上执行较低版本的包会在运行时将它们升级到 SSIS 2017 包。 不支持执行更高版本的包。
      >
      > 此外，由于旧 SSIS 包存储区绑定到特定的 SQL Server 版本并仅在 SSMS 上可访问该版本，因此需要先使用指定的 SSMS 版本将旧 SSIS 包存储中的较低版本的包导出到文件系统中，然后才能使用 SSMS 2019 或更高版本将它们导入 Azure-SSIS IR 包存储区中。
      >
      > 或者，若要在切换其保护级别时将多个 SSIS 包导入 Azure-SSIS IR 包存储，可以使用[dtutil](https://docs.microsoft.com/sql/integration-services/dtutil-utility?view=sql-server-2017)命令行实用工具，请参阅使用[dtutil 部署多个包](#deploying-multiple-packages-with-dtutil)。
   ...
   *  选择“导出包”可将包从你的包存储导出到“文件系统”、“SQL Server (MSDB)”或旧版“SSIS 包存储”中。
      ...
      根据要导入到的“包位置”，选择相关的“服务器”/“身份验证类型”，在必要时输入访问凭据，并选择“包路径”。 导出包时，如果已对其进行加密，请先输入密码对其进行解密，然后可以更改其保护级别，例如，避免存储任何敏感数据或通过用户密钥或密码对数据进行加密。

      > [!NOTE]
      > 从 Azure-SSIS IR 包存储导出 SSIS 包时，只能通过一种方式进行复制，而无需切换其保护级别即可只复制它们，同时保留其 SQL Server/SSIS 版本，否则会将它们升级到 SSIS 2019 或更高版本的包。
      >
      > 由于 Azure-SSIS IR 当前的默认兼容性级别为140，这等于**SQL Server 2017**，因此，在其上执行较低版本的包会在运行时将它们升级到 SSIS 2017 包。 不支持执行更高版本的包。
      >
      > 或者，若要在切换其保护级别时从 Azure-SSIS IR 包存储中导出多个 SSIS 包，可以使用[dtutil](https://docs.microsoft.com/sql/integration-services/dtutil-utility?view=sql-server-2017)命令行实用工具，请参阅使用[dtutil 部署多个包](#deploying-multiple-packages-with-dtutil)。
...
连接到 SSMS 上的 Azure-SSIS IR 后，可以右键单击任何存储的包以弹出菜单并选择 "**运行包**"。  这将打开“执行包实用工具”对话框，你可以在其中将 Azure-SSIS IR 上的包执行操作配置为 ADF 管道中的“执行 SSIS 包”活动。
...
**执行包实用工具**对话框的 "**常规**"、"**配置**"、"**执行选项**" 和 "**日志记录**" 页对应于 "执行 SSIS 包" 活动的 "**设置**" 选项卡 在这些页面上，你可以输入包的加密密码，并访问包配置文件的信息。 你还可以输入包执行凭据和属性以及日志文件夹的访问信息。  “执行包实用工具”对话框的“设置值”页面对应于“执行 SSIS 包”活动的“属性替代”选项卡，你可以在其中输入要替代的现有包属性。 有关详细信息，请参阅[将 SSIS 包作为 ADF 管道中的“执行 SSIS 包”活动运行](https://docs.microsoft.com/azure/data-factory/how-to-invoke-ssis-package-ssis-activity)。

当你选择 "**执行**" 按钮时，将自动生成并触发带有 "执行 SSIS 包" 活动的新 ADF 管道。 如果已存在具有相同设置的 ADF 管道，则会重新运行它，并且不会生成新的管道。 ADF 管道和“执行 SSIS 包”活动将分别命名为 `Pipeline_SSMS_YourPackageName_HashString` 和 `Activity_SSMS_YourPackageName`。
...
连接到 SSMS 上的 Azure-SSIS IR 后，可以右键单击它以弹出菜单，并选择 "**前往 Azure 数据工厂门户**" 或 "**刷新**"。
...
## <a name="deploying-multiple-packages-with-dtutil"></a>通过 dtutil 部署多个包

若要提升 & 将本地 SSIS 工作负载转移到 ADF 中的 SSIS，同时保持旧的包部署模型，需要将包从文件系统、由 SQL Server 承载的 MSDB 或旧 SSIS 包存储区部署到 azure 文件中，MSDB 由 Azure SQL 托管实例或 Azure-SSIS IR 包存储区承载。 同时，还应将用户密钥的保护级别从加密切换到未加密或通过密码加密（如果尚未这样做）。

你可以使用 SQL Server/SSIS 安装附带的[dtutil](https://docs.microsoft.com/sql/integration-services/dtutil-utility?view=sql-server-2017)命令行实用工具来成批部署多个包。 它绑定到特定的 SSIS 版本，因此，如果你使用它来部署更低版本的包，而不切换其保护级别，则只需复制它们，同时保留其 SSIS 版本。 如果你使用它来部署它们并同时切换其保护级别，则它会将它们升级到其 SSIS 版本。

 由于 Azure-SSIS IR 当前的默认兼容性级别为140，这等于**SQL Server 2017**，因此，在其上执行较低版本的包会在运行时将它们升级到 SSIS 2017 包。 不支持执行更高版本的包。

因此，为了避免运行时升级，在包部署模型中 Azure-SSIS IR 部署要在上运行的包应使用 SQL Server/SSIS 2017 安装附带的 dtutil 2017。 出于此目的，你可以下载并安装免费的[SQL Server/SSIS 2017 开发人员版](https://go.microsoft.com/fwlink/?linkid=853016)。 安装完成后，可在此文件夹中找到 dtutil 2017： `YourLocalDrive:\Program Files\Microsoft SQL Server\140\DTS\Binn` 。

### <a name="deploying-multiple-packages-from-file-system-on-premises-into-azure-files-with-dtutil"></a>通过 dtutil 将多个包从本地文件系统部署到 Azure 文件

 若要将多个包从文件系统部署到 Azure 文件并同时切换其保护级别，可以在命令提示符处运行以下命令。 请替换特定于您的情况的所有字符串。
...
若要在批处理文件中运行上述命令，请将替换 `%f` 为 `%%f` 。

若要将来自文件系统的旧式 SSIS 包存储区中的多个包部署到 Azure 文件并同时切换其保护级别，可以使用相同的命令，但将替换 `YourLocalDrive:\...\YourPackageFolder` 为旧版 SSIS 包存储区使用的本地文件夹： `YourLocalDrive:\Program Files\Microsoft SQL Server\YourSQLServerDefaultCompatibilityLevel\DTS\Packages\YourPackageFolder` 。 例如，如果旧 SSIS 包存储区绑定到 SQL Server 2016，请参阅 `YourLocalDrive:\Program Files\Microsoft SQL Server\130\DTS\Packages\YourPackageFolder` 。  可以 `YourSQLServerDefaultCompatibilityLevel` 从[SQL Server 默认兼容级别的列表](https://docs.microsoft.com/sql/t-sql/statements/alter-database-transact-sql-compatibility-level?view=sql-server-ver15#arguments)中找到的值。

如果已在 Azure 文件上配置 Azure-SSIS IR 包存储，则在你连接到 SSMS 2019 或更高版本上的 Azure-SSIS IR 时，已部署的包将显示在其中。

### <a name="deploying-multiple-packages-from-msdb-on-premises-into-msdb-in-azure-with-dtutil"></a>通过 dtutil 将本地 MSDB 中的多个包部署到 Azure 中的 MSDB

 若要将来自 msdb 的 SQL Server 或旧式 SSIS 包存储中托管的多个包部署到 Azure SQL 托管实例承载的 MSDB 中，并同时切换其保护级别，可以连接到 ssms 上的 SQL Server，右键单击 `Databases->System Databases->msdb` ssms**对象资源管理器**上的节点以打开**新的查询**窗口，然后运行以下 t-sql 脚本。 请替换特定于您的情况的所有字符串：  
...
该脚本将为 MSDB 中的所有包生成 dtutil 命令行，你可以对其进行多重选，复制 & 粘贴，然后在命令提示符下运行。
...
如果已在 MSDB 上配置 Azure-SSIS IR 包存储，则在你连接到 SSMS 2019 或更高版本上的 Azure-SSIS IR 时，已部署的包将显示在其中。

### <a name="deploying-multiple-packages-from-msdb-on-premises-into-azure-files-with-dtutil"></a>通过 dtutil 将多个包从本地 MSDB 部署到 Azure 文件

 若要将来自 msdb 的 SQL Server 或旧式 SSIS 包存储中托管的多个包部署到 Azure 文件并同时切换其保护级别，可以连接到 SSMS 上的 SQL Server，右键单击 `Databases->System Databases->msdb` ssms**对象资源管理器**上的节点，以打开**新的查询**窗口，然后运行以下 t-sql 脚本。 请替换特定于您的情况的所有字符串：  
