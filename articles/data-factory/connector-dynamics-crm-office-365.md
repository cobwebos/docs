...
本文概述了如何使用 Azure 数据工厂中的复制活动从 Dynamics 365/Dynamics CRM 复制数据， 是基于总体介绍复制活动的[复制活动概述](copy-activity-overview.md)一文进行扩展的。
...
| username | 指定要连接到 Dynamics 的用户名称。 | 是 |
...
有关可用于定义活动的各部分和属性的完整列表，请参阅[管道](concepts-pipelines-activities.md)一文。 本部分提供 Dynamics 源支持的属性列表。

### <a name="dynamics-as-source"></a>作为源的 Dynamics

要从 Dynamics 复制数据，请将复制活动中的源类型设置为“DynamicsSource”。 复制活动源部分支持以下属性：
