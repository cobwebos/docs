...
具体而言，此 Oracle 连接器支持以下版本的 Oracle 数据库：
...
要从 Oracle 复制数据，请将复制活动中的源类型设置为“OracleSource”。 复制活动源部分支持以下属性：
...
| writeBatchSize | 缓冲区大小达到 writeBatchSize 时会数据插入 SQL 表。<br/>允许的值为：整数（行数）。 |否（默认为 10000） |
