...
具有更多核心的 Integration Runtime 增加了 Spark 计算环境中节点的数量，并提供更多的处理能力来读取、写入和转换数据。 ADF 数据流为计算引擎使用 Spark。 Spark 环境适用于内存优化资源。
...
### <a name="decrease-cluster-compute-start-up-time-with-ttl"></a>降低群集计算启动时间和 TTL

"数据流属性" 下的 "Azure IR 中有一个属性，可让你为工厂创建群集计算资源池。 利用此池，可以按顺序提交要执行的数据流活动。 建立池后，按需 Spark 群集将需要1-2 分钟时间来执行作业。 资源池的初始设置需要大约6分钟。 指定想要在生存时间（TTL）设置中维护资源池的时间长度。

## <a name="optimizing-for-azure-sql-database-and-azure-sql-data-warehouse-synapse"></a>针对 Azure SQL 数据库和 Azure SQL 数据仓库的优化 Synapse
...
* 批大小指示 ADF 将数据存储在 Spark 内存的集中，而不是逐行存储。 批大小是一个可选设置，如果计算节点的大小不正确，则可能会用尽计算节点上的资源。 如果未设置此属性，则将使用 Spark 缓存批处理默认值。
...
若要避免对数据流进行逐行处理，请在 Azure SQL 数据库和 Azure SQL DW 接收器的 "设置" 选项卡中设置**批大小**。 如果设置了批大小，则 ADF 会根据提供的大小处理批次数据库写入。 如果未设置此属性，则将使用 Spark 缓存批处理默认值。
...
管理数据流中的联接性能是一项很常见的操作，您将在数据转换的整个生命周期中执行此操作。 在 ADF 中，数据流不要求在联接之前对数据进行排序，因为这些操作在 Spark 中作为哈希联接执行。 但是，可以通过适用于联接、存在和查找转换的 "广播" 联接优化提高性能。

这会通过将联接关系的任一端的内容向下推送到 Spark 节点来避免实时洗牌。 这非常适合用于引用查找的小型表。 可能不适用于节点内存的较大表不适合进行广播优化。

对于包含多个联接操作的数据流，推荐的配置是将 "自动" 设置为 "自动"，并使用内存优化的 Azure Integration Runtime 配置。 如果在数据流执行期间遇到内存不足错误或广播超时，可以关闭广播优化。 但是，这将导致执行数据流的速度变慢。 或者，您可以指示数据流仅在联接的左侧和/或右侧下推。
