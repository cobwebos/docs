...
* 若要将更多的数据缓存在内存中，请尝试使用**内存优化**群集。 内存优化群集的每核心价位比计算优化群集更高，但可能会提高转换速度。 如果在执行数据流时遇到内存不足错误，请切换到内存优化的 Azure IR 配置。
...
每次转换时，都可以在“优化”选项卡中设置希望数据工厂使用的分区方案。良好的做法是先在保留默认分区和优化的情况下测试基于文件的接收器。
...
对于包含许多联接操作的数据流，建议的配置是将“广播”优化始终设置为“自动”，并使用内存优化的 Azure Integration Runtime 配置。 如果在执行数据流期间遇到内存不足错误或广播超时，可以关闭广播优化。 但是，这会导致数据流执行速度变慢。 （可选）可以指示数据流仅下推联接的左侧或右侧，或者同时下推两侧。
