...
数据工厂服务提供用于存储、处理和数据移动服务的可靠且完整视图。 此服务提供的监视仪表板帮助可用于执行以下任务： 
...
数据工厂的图示视图提供单个窗格来监视和管理数据工厂及其资产。

若要查看数据工厂的图示视图，请在数据工厂主页单击“图示”。

![图示视图](./media/data-factory-monitor-manage-pipelines/diagram-view.png)

可以放大、缩小、缩放到合适大小、缩放到 100%、锁定图示布局以及自动放置管道和表。 还可看到数据沿袭信息（显示选定项的上下游项）。

### <a name="activities-inside-a-pipeline"></a>管道中的活动
1. 右键单击该管道，然后单击“打开管道”查看管道中的所有活动以及该活动的输入和输出数据集。 如果管道包含多个活动且用户想要了解单个管道的操作沿袭，此功能非常有用。
...
3. 通过在左上角痕迹导航中单击数据工厂链接，可向后导航到数据工厂主页。
   
    ![向后导航到数据工厂](./media/data-factory-monitor-manage-pipelines/navigate-back-to-data-factory.png)

### <a name="view-state-of-each-activity-inside-a-pipeline"></a>查看管道中每个活动的状态
通过查看活动生成的任意数据集状态，可查看该活动的当前状态。 

例如：在以下示例中，已成功运行 **BlobPartitionHiveActivity** 且生成了名为 **PartitionedProductsUsageTable** 的数据集，该数据集处于“就绪”状态。
...
<tr>
<td>ActivityResume</td><td>活动暂停，且在恢复活动之前无法运行切片。</td>
</tr>
<tr>
...
<tr>
<td>-</td><td>未能生成和/或验证切片。</td>
</tr>
<td>就绪</td><td>-</td><td>切片已就绪，可供使用。</td>
</tr>
<tr>
<td>已跳过</td><td>无</td><td>切片未经处理。</td>
</tr>
<tr>
<td>无</td><td>-</td><td>用于以不同状态存在但已被重置的切片。</td>
</tr>
</table>
...
如果已多次执行切片，则会在“活动运行”列表中看到多行。 通过在“活动运行”列表中单击运行条目，可以查看有关活动运行的详细信息。 列表将显示所有日志文件以及一条错误消息（如有）。 此功能对查看和调试日志，而无需离开数据工厂非常有用。

![活动运行详细信息](./media/data-factory-monitor-manage-pipelines/activity-run-details.png)

如果切片状态不是“就绪”，可以在“未就绪的上游切片”列表中看到未就绪且阻碍当前切片运行的上游切片。 如果切片处于“等待”状态，并且你希望了解等待中切片的上游依赖关系，此功能非常有用。
...
数据工厂中的数据集状态转换流：“等待”->“正在进行/正在验证”->“就绪/失败”

切片刚开始处于“等待”状态，满足前提条件后执行。 然后，活动开始执行，切片进入“正在进行”状态。 活动执行可能成功，也可能失败。 根据执行结果，切片被标记为“就绪”或“失败”。 
...
例如：如以下屏幕截图所示，已在 **productrecgamalbox1dev** 数据工厂中通过 **PartitionProductsUsagePipeline** 识别问题，准备挂起该管道。
...
如果活动在管道中运行失败，则管道所生成数据集将因此故障而处于错误状态。 可以在 Azure 数据工厂使用以下机制调试错误并对其进行故障排除。

#### <a name="use-azure-portal-to-debug-an-error"></a>使用 Azure 门户调试错误：
1. 在“表”边栏选项卡中，单击“状态”设置为“失败”的问题切片。
   
   ![包含问题切片的“表”边栏选项卡](./media/data-factory-monitor-manage-pipelines/table-blade-with-error.png)
2. 在“数据切片”边栏选项卡中，单击运行失败的活动。
   
   ![出现错误的数据切片](./media/data-factory-monitor-manage-pipelines/dataslice-with-error.png)
3. 在“活动运行详细信息”边栏选项卡中，可以下载与 HDInsight 处理关联的文件。 单击“状态/stderr 下载”可下载包含错误详细信息的错误日志文件。
...
2. 运行 **Get-AzureRmDataFactorySlice** 命令查看切片及其状态。 可看到“失败”状态的切片。        
...
3. 现在，运行 **Get-AzureRmDataFactoryRun** cmdlet 来获取有关切片活动运行的详细信息。
...
4. 可看到包含错误详细信息的输出（类似以下内容）：
    ...
    Type                    :
    ```
5. 可通过从输出中看到的 ID 值运行 **Save-AzureRmDataFactoryLog** cmdlet，并使用用于 cmdlet 的 **-DownloadLogsoption** 下载日志文件。
...
在切片因策略失败（例如：数据不可用）而未通过验证的情况下，在命令栏上单击“验证”按钮，可以解决故障并再次验证。
![修复错误，然后验证](./media/data-factory-monitor-manage-pipelines/fix-error-and-validate.png)

### <a name="using-azure-powershell"></a>使用 Azure PowerShell
通过使用 Set-AzureRmDataFactorySliceStatus cmdlet，可以重新运行切片。 有关该 cmdlet 的语法和其他详细信息，请参阅 [Set-AzureRmDataFactorySliceStatus](https://msdn.microsoft.com/library/mt603522.aspx) 主题。 

**示例：**以下示例将 Azure 数据工厂“WikiADF”中表“DAWikiAggregatedData”的所有切片状态设置为“等待”。
...
创建、更新或删除 Azure 资源（例如，数据工厂）时 Azure 会记录用户事件。 可以基于这些事件创建警报。 利用数据工厂，可以捕获各种指标并创建指标相关警报。 建议将事件用于实时监视，将指标用于历史查询。 

### <a name="alerts-on-events"></a>事件相关警报
Azure 事件可提供有关 Azure 资源中所发生情况的有用见解。 创建、更新或删除 Azure 资源（例如，数据工厂）时 Azure 会记录用户事件。 使用 Azure 数据工厂时，以下情况会生成事件：

* 创建/更新/删除 Azure 数据工厂。
* 数据处理（称为运行次数）已开始/完成。
* 创建和删除按需 HDInsight 群集。

可以根据这些用户事件创建警报，并将其配置为向订阅管理员和共同管理员发送电子邮件通知。 此外，针对需接收电子邮件通知且满足条件的其他用户，可指定其电子邮件地址。 如果希望收到故障通知，但不持续监视数据工厂，此功能非常有用。

> [!NOTE]
> 当前，门户不显示事件警报。 使用[监视和管理应用](data-factory-monitor-manage-app.md)查看所有警报。
> 
> 
...
如果不想收到特定故障的警报，可从 JSON 定义删除 **subStatus**。
...
#### <a name="deploying-the-alert"></a>部署警报
...
#### <a name="retrieving-the-list-of-azure-resource-group-deployments"></a>检索 Azure 资源组部署的列表
...
#### <a name="troubleshooting-user-events"></a>用户事件故障排除
...
3. 在“事件”边栏选项卡中，可看到有关事件的详细信息，以及筛选事件等。 
...
请参阅 [Azure Insight Cmdlet](https://msdn.microsoft.com/library/mt282452.aspx) 一文，了解可用于添加/获取/删除警报的 PowerShell cmdlet。 下面是使用 **Get-AlertRule** cmdlet 的几个示例： 
...
* 如果在门户边栏选项卡上可看到警报生成事件，但无法接收电子邮件通知，请检查是否已将指定的电子邮件地址设置为接收来自外部发件人的电子邮件。 警报电子邮件可能已被电子邮件设置阻止。

### <a name="alerts-on-metrics"></a>指标相关警报
利用数据工厂，可以捕获各种指标并创建指标相关警报。 在数据工厂中，可对切片监视和创建基于以下指标的警报。

* 失败的运行次数
* 成功的运行次数

这些指标非常有用，借此可大致了解数据工厂中的失败和成功运行总数。 每次运行切片都会发出指标。 每小时将这些指标聚合并推送到存储帐户。 因此，若要启用指标，请设置存储帐户。

#### <a name="enabling-metrics"></a>启用指标：
...
在“诊断”边栏选项卡中，单击“打开”并选择存储帐户，然后保存。

![诊断边栏选项卡](./media/data-factory-monitor-manage-pipelines/diagnostics-blade.png)

保存后，可能需要一个小时才能在“监视”边栏选项卡看到指标，因为指标聚合每小时发生一次。

### <a name="setting-up-alert-on-metrics"></a>设置指标相关警报：
单击“数据工厂指标”边栏选项卡： 
...
* 选择指标（失败的运行次数与成功的运行次数）。
* 指定条件和阈值。   
* 指定时间段。 
* 指定是否向所有者、参与者和读者发送电子邮件。
* 等等。 

![“数据工厂指标”边栏选项卡 - 添加警报](./media/data-factory-monitor-manage-pipelines/add-an-alert-rule.png)

成功添加警报规则后，关闭边栏选项卡，可在“指标”页看到新警报。 

![“数据工厂指标”边栏选项卡 - 添加警报](./media/data-factory-monitor-manage-pipelines/failed-alert-in-metric-blade.png)

还可在“警报”磁贴上看到警报数。 单击“警报”磁贴。

![“数据工厂指标”边栏选项卡 - 警报规则](./media/data-factory-monitor-manage-pipelines/alert-rules-tile-rules.png)

在“警报”边栏选项卡中，可看到现有的任何警报。 若要添加警报，请在工具栏上单击“添加警报”。

![“警报规则”边栏选项卡](./media/data-factory-monitor-manage-pipelines/alert-rules-blade.png)

### <a name="alert-notifications"></a>警报通知：
警报规则与条件匹配后，可收到警报被激活的电子邮件。 问题得以解决且警报条件不再匹配时，可收到警报已解决的电子邮件。

此行为与事件中的行为不同，事件中对警报规则限定的每个故障均发送通知。

### <a name="deploying-alerts-using-powershell"></a>使用 PowerShell 部署警报
...
## <a name="move-data-factory-to-a-different-resource-group-or-subscription"></a>将数据工厂移动到其他资源组或订阅
