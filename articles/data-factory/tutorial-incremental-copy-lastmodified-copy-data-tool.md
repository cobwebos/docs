...
3. 在“源数据存储”**** 页上，完成以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 选择“+ 创建新连接”添加一个连接。****
...
4. 在“选择输入文件或文件夹”页中完成以下步骤：****
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 浏览并选择 **source** 文件夹，然后选择“选择”。****

    ![选择输入文件或文件夹](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/choose-input-file-folder.png)

    b.保留“数据库类型”设置，即设置为“共享”。 在**文件加载行为**下，选择**增量加载：上次修改日期**。

    c. 勾选“二进制副本”，然后选择“下一步”。********

     ![选择输入文件或文件夹](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/check-binary-copy.png)

5. 在 **"目标数据存储"** 页上，选择您创建的**Azure Blob 存储**。 这是与源数据存储相同的存储帐户。 然后选择 **"下一步**"。

6. 在“选择输出文件或文件夹”页中完成以下步骤：****

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 浏览并选择 **destination** 文件夹，然后选择“选择”。****
...
10. 请注意，界面中已自动选择左侧的“监视”选项卡。**** 应用程序切换到 **"监视器"** 选项卡。您将看到管道的状态。 选择“刷新”可刷新列表。**** 单击 PIPELINE **NAME**下的链接以查看活动运行详细信息或重新运行管道。 

    ![刷新列表并选择“查看活动运行”](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs1.png)

11. 该管道只包含一个活动（复制活动），因此只显示了一个条目。 有关复制操作的详细信息，请选择**活动名称**列下方**的"详细信息**"链接（眼镜图标）。 有关属性的详细信息，请参阅[复制活动概述](copy-activity-overview.md)。 
...
13. 要返回 **"管道运行"** 视图，请选择 **"所有管道运行**"，然后等待再次自动触发同一管道。  

    ![选择“所有管道运行”](./media/tutorial-incremental-copy-lastmodified-copy-data-tool/monitor-pipeline-runs4.png)

14. 当第二个管道运行完成时，按照上述相同的步骤查看活动运行详细信息。  

    您将看到一个文件（file1.txt）已从**源**容器复制到 Blob 存储帐户**的目标**容器。
