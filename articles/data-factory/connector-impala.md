...
可以将数据从 Impala 复制到任何支持的接收器数据存储。 有关复制活动支持为源或接收器的数据存储的列表，请参阅[支持数据存储](copy-activity-overview.md#supported-data-stores-and-formats)表。
...
| enableSsl | 指定是否使用 TLS 加密到服务器的连接。 默认值是 **false**秒。  | 否 |
...
| allowHostNameCNMismatch | 指定在通过 TLS 连接时是否需要 CA 颁发的 TLS/SSL 证书名称与服务器的主机名匹配。 默认值是 **false**秒。  | 否 |
...
| properties | 说明 | 必选 |
|:--- |:--- |:--- |
| type | 数据集的类型属性必须设置为 **：ImpalaObject** | 是 |
