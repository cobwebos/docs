...
description: Azure 数据工厂映射数据流表达式函数语言
author: kromerm
ms.author: makromer
...
ms.locfileid: "59362026"
---
# <a name="mapping-data-flow-data-transformation-expressions"></a>映射数据的流数据转换表达式
...
添加一对字符串或数字。 将日期添加到日期数。 将类似类型的数组追加到另一个数组。 与相同 + 运算符
* ``add(10, 20) -> 30``
* ``10 + 20 -> 30``
...
在流中按名称选择列的值。 如果有多个匹配项，则返回第一个匹配项。 如果没有匹配项则返回 NULL 值。 返回的值必须是其中一个类型转换函数 （TO_DATE，TO_STRING...） 转换的类型。在设计时已知的列名称应只需通过其名称进行寻址。 不支持计算的输入，但可以使用的参数替换
...
在流中选择某列的值按其相对位置 (从 1 开始)。 如果超出界限位于它返回一个 NULL 值。 返回的值必须是其中一个类型转换函数 （TO_DATE，TO_STRING...） 转换的类型不支持计算的输入，但可以使用的参数替换
...
根据备用条件应用一个值或另一个值。 如果输入个数为偶数，另对于最后一个条件为 NULL
...
获取值的聚合计数。 如果指定的可选列，则它会忽略 NULL 值计数
* ``count(custId) -> 100``
...
根据条件获取值的聚合计数。 如果指定可选的列，则它会忽略 NULL 值计数
...
获取当前时间戳，启动作业以运行与本地时区
* ``currentTimestamp() -> 12-12-2030T12:12:12``
*********************************
<code>currentUTC</code>
==============================
<code><b>currentUTC([<i>&lt;value1&gt;</i> : string]) => timestamp</b></code><br/><br/>
获取 UTC 格式的当前时间戳。 可以“GMT”、“PST”、“UTC”、“America/Cayman”格式传递可选的时区。 默认值为当前时区
...
计算某个值在值组中的排名。 结果是 1 加上前面的行数，或者等于当前行在分区中的顺序。 值不会在序列中生成空隙。 即使数据未排序和查找的更改中的值有效密集排名
...
始终返回 false 值。 如果没有名为 'false' 的列名称，请使用函数 syntax(false())
* ``isDiscounted == false()``
* ``isDiscounted() == false``
*********************************
<code>first</code>
...
获取列组的第一个值。 如果省略第二个参数 ignoreNulls，它被假定 false
...
将从 UTC 转换为时间戳。 可以选择性地传递 'GMT'、 '太平洋标准时间、 'UTC'、 ' America/开曼窗体中的时区。 默认值为当前时区
...
返回输入值列表中的最大值。 如果所有输入都均为 null，则返回 null
...
将每个单词的第一个字母转换为大写。 单词标识为由空格分隔
* ``initCap('cool iceCREAM') -> 'Cool IceCREAM'``
*********************************
<code>instr</code>
==============================
<code><b>instr(<i>&lt;string&gt;</i> : string, <i>&lt;substring to find&gt;</i> : string) => integer</b></code><br/><br/>
在字符串中查找子字符串的位置（从 1 开始）。 0 是返回如果未找到
* ``instr('great', 'eat') -> 3``
* ``instr('microsoft', 'o') -> 7``
* ``instr('good', 'bad') -> 0``
*********************************
...
检查该行是否标记为删除。 采用多个输入的流转换为可以传递将索引 （从 1 开始） 的流。 流索引的默认值为 1
* ``isDelete() -> true``
* ``isDelete(1) -> false``
*********************************
<code>isError</code>
...
检查该行是否标记为错误。 采用多个输入的流转换为可以传递将索引 （从 1 开始） 的流。 流索引的默认值为 1
* ``isError() -> true``
* ``isError(1) -> false``
*********************************
<code>isIgnore</code>
...
检查该行是否标记为忽略。 采用多个输入的流转换为可以传递将索引 （从 1 开始） 的流。 流索引的默认值为 1
* ``isIgnore() -> true``
* ``isIgnore(1) -> false``
*********************************
<code>isInsert</code>
...
检查该行是否标记为插入。 采用多个输入的流转换为可以传递将索引 （从 1 开始） 的流。 流索引的默认值为 1
* ``isInsert() -> true``
* ``isInsert(1) -> false``
*********************************
<code>isMatch</code>
...
检查查找时是否匹配该行。 采用多个输入的流转换为可以传递将索引 （从 1 开始） 的流。 流索引的默认值为 1
* ``isMatch() -> true``
* ``isMatch(1) -> false``
*********************************
<code>isNull</code>
...
检查该行是否标记为更新。 采用多个输入的流转换为可以传递将索引 （从 1 开始） 的流。 流索引的默认值为 1
* ``isUpdate() -> true``
* ``isUpdate(1) -> false``
*********************************
<code>kurtosis</code>
...
获取当前行之前由第一个参数计算的 n 行的值。 第二个参数是要回顾的行数，默认值为 1。 如果不返回的值为 null，除非指定了默认值的行数
* ``lag(amount, 2) -> 60``
...
获取列组的最后一个值。 如果省略第二个参数 ignoreNulls，它被假定 false
* ``last(sales) -> 523.12``
...
获取当前行之后由第一个参数计算的 n 行的值。 第二个参数是要期待的行数，默认值为 1。 如果不返回的值为 null，除非指定了默认值的行数
* ``lead(amount, 2) -> 60``
...
从索引 1 处开始提取包含字符数的子字符串。 相同的子字符串 (str，1，n)
* ``left('bojjus', 2) -> 'bo'``
...
从特定的位置开始，在字符串中查找子字符串的位置（从 1 开始）。 如果省略位置，则视为从字符串开头查找。 0 是返回如果未找到
* ``locate('eat', 'great') -> 3``
...
在左侧裁剪掉前导和尾随字符的字符串。 如果未指定第二个参数，则裁剪掉空格。 否则剪裁第二个参数中指定的任何字符
* ``ltrim('!--!wor!ld!', '-!') -> 'wor!ld!'``
*********************************
<code>max</code>
==============================
...
NTile 函数将每个窗口分区的行分割为从 1 到 `n` 的 `n` 桶。 桶值最大相差 1。 如果分区中的行数不能均匀分割成桶数，则余值将逐个分布在每个桶中，从第一个桶开始。 NTile 函数的 tertiles 计算特别有用，在初始化期间四分位数、 十分位数和其他常见的汇总统计函数计算两个变量：正则存储桶的大小将具有一个额外的行添加到它。 这两个变量基于当前分区的大小。 在计算过程中，该函数将跟踪当前行号、当前桶号，以及发生桶更改的行号 (bucketThreshold)。 如果当前行号达到桶的阈值，则桶值将会加 1，阈值将按桶大小增加（如果当前桶已填充，则额外加 1）。
* ``nTile() -> 1``
...
返回下一个唯一序列。 数是仅在一个分区内连续和 partitionId 前缀
...
返回 NULL 值。 如果没有名为 null 的列名称，请使用函数 syntax(null())。 使用任何操作将导致为空
...
计算某个值在值组中的排名。 结果是 1 加上前面的行数，或者等于当前行在分区中的顺序。 值将在序列中生成空隙。 级别工作甚至时不对数据进行排序和查找的更改中的值
* ``rank(salesQtr, salesAmt) -> 1``
*********************************
<code>regexExtract</code>
==============================
<code><b>regexExtract(<i>&lt;string&gt;</i> : string, <i>&lt;regex to find&gt;</i> : string, [<i>&lt;match group 1-based index&gt;</i> : integral]) => string</b></code><br/><br/>
提取给定正则表达式模式的匹配子字符串。 最后一个参数标识匹配组，如果省略，则默认为 1。 使用<regex>'(back quote) 来匹配而不转义的字符串
...
检查字符串是否与给定的正则表达式模式相匹配。 使用<regex>'(back quote) 来匹配而不转义的字符串
...
使用给定的字符串中的另一个子字符串替换所有匹配的正则表达式模式 '<regex>'(back quote) 来匹配而不转义的字符串
...
从右侧提取包含字符数的子字符串。 相同的子字符串 (str，LENGTH(str)-n，n)
* ``right('bojjus', 2) -> 'us'``
...
舍入给定可选的小数位数和可选的舍入模式。 如果省略小数位数，则它是默认设置为 0。  如果省略模式，则默认为 ROUND_HALF_UP(5)。 舍入的值包括 1-ROUND_UP 2-ROUND_DOWN 3-ROUND_CEILING 4-ROUND_FLOOR 5-ROUND_HALF_UP 6-ROUND_HALF_DOWN 7-ROUND_HALF_EVEN 8-ROUND_UNNECESSARY
* ``round(100.123) -> 100.0``
* ``round(2.5, 0) -> 3.0``
...
将分配编号从 1 开始的窗口中的行的顺序行
* ``rowNumber() -> 1``
*********************************
<code>rpad</code>
==============================
<code><b>rpad(<i>&lt;string to pad&gt;</i> : string, <i>&lt;final padded length&gt;</i> : integral, <i>&lt;padding&gt;</i> : string) => string</b></code><br/><br/>
在右侧使用提供的填充内容填充字符串，直到达到特定的长度。 如果该字符串为等于或大于的长度，则它将被不视为执行任何操作* ``rpad('great', 10, '-') -> 'great-----'``
* ``rpad('great', 4, '-') -> 'great'``
* ``rpad('great', 8, '<>') -> 'great<><'``
*********************************
...
在右侧裁剪掉前导和尾随字符的字符串。 如果未指定第二个参数，则裁剪掉空格。 否则剪裁第二个参数中指定的任何字符
* ``rtrim('!--!wor!ld!', '-!') -> '!--!wor!ld'``
*********************************
<code>second</code>
==============================
...
计算的基元数据类型给出的位长度的值 0(256)，可以仅是不同的列集的 sha-2 摘要 224、 256、 384、 512。 它可用于计算行的指纹
...
可将值转换 (不，true、 y、 是，"1") 为 true 和 (f、 'false'，' n、 否，"0") 为 false 和 NULL 的任何其他值
* ``toBoolean('true') -> true``
* ``toBoolean('n') -> false``
...
将字符串转换为给定的可选日期格式的日期。 所有可能的格式，请参阅 Java SimpleDateFormat。 如果省略日期格式，则接受以下组合。 [ yyyy, yyyy-[M]M, yyyy-[M]M-[d]d, yyyy-[M]M-[d]d, yyyy-[M]M-[d]d, yyyy-[M]M-[d]dT* ]
...
将任何数字或字符串转换为浮点值。 可选的 Java 十进制格式可用于转换。将截断任何双
...
将任何数字或字符串转换为整数值。 可选的 Java 十进制格式可用于转换。将截断任何 long、 float、 double
* ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
...
将任何数字或字符串转换为长值。 可选的 Java 十进制格式可用于转换。将截断任何 float、 double
* ``toLong(123) -> 123``
* ``toLong('123') -> 123``
* ``toLong('$123', '$###') -> 123``
*********************************
...
将任何数字或字符串转换为短值。 可选的 Java 十进制格式可用于转换。将截断任何整数，long、 float、 double
* ``toShort(123) -> 123``
* ``toShort('123') -> 123``
* ``toShort('$123', '$###') -> 123``
*********************************
...
将基元数据类型转换为字符串。 对于数字和日期，可以指定格式。 如果未指定，则选择系统默认值。对数字使用 Java 十进制格式。 所有可能的日期格式; 数据格式，请参阅 Java SimpleDateFormat默认格式是年-月-日
* ``toString(10) -> '10'``
...
根据可选的时间戳格式将字符串转换为日期。 所有可能的格式，请参阅 Java SimpleDateFormat。 如果省略该时间戳，则默认模式 yyyy-[M] M-[d] d hh: mm: [.f...] 使用
...
将时间戳转换为 UTC。 可以“GMT”、“PST”、“UTC”、“America/Cayman”格式传递可选的时区。 默认值为当前时区
...
将字符串中的一组字符替换为另一组字符。 字符具有 1 对 1 替换
...
裁剪掉前导和尾随字符的字符串。 如果未指定第二个参数，则裁剪掉空格。 否则剪裁第二个参数中指定的任何字符
* ``trim('!--!wor!ld!', '-!') -> 'wor!ld'``
*********************************
<code>true</code>
==============================
<code><b>true() => boolean</b></code><br/><br/>
始终返回 true 值。 如果没有名为 'true' 的列名称，请使用函数 syntax(true())
* ``isDiscounted == true()``
* ``isDiscounted() == true``
*********************************
<code>typeMatch</code>
...
匹配列的类型。 仅可在模式 expressions.number 匹配短、 整数、 长整型、 双精度，float 或 （十进制） 整数匹配简单地说，整数、 长而小数部分匹配 double、 float、 decimal 和日期时间的匹配项的日期或时间戳类型
...
Java HotSpot(TM) 64 位服务器 VM 警告： 忽略选项 MaxPermSize = 2048m;8.0 [成功] 中已不再支持的总时间：5 s，已完成 4:17:11 PM 2019 年 4 月 3 日 > 到 D:\kadfdataflow\DataflowExecutor\target\scala-2.11\classes runMain com.microsoft.dataflow.FunctionPrinter [信息] 编译 1 Scala 源...[错误]D:\kadfdataflow\DataflowExecutor\src\main\scala\com\microsoft\dataflow\FunctionPrinter.scala:92： 类型的应用程序不允许为后缀运算符 [error] typeOf[SLICE.type]，[error] ^ [error] 其中一个错误，找不到 [错误] (数据流 / 编译： 编译) 编译失败 [错误] 总时间：24 s，已完成 4:43:43 PM 2019 年 4 月 3 日 >> 到 D:\kadfdataflow\DataflowExecutor\target\scala-2.11\classes runMain com.microsoft.dataflow.FunctionPrinter [信息] 编译 1 Scala 源...[信息]运行 com.microsoft.dataflow.FunctionPrinter 侦听传输 dt_socket 地址：8000
*********************************
<code>abs</code>
==============================
<code><b>abs(<i>&lt;value1&gt;</i> : number) => number</b></code><br/><br/>
...
==============================
<code><b>toInteger(<i>&lt;value&gt;</i> : any, [<i>&lt;format&gt;</i> : string]) => integer</b></code><br/><br/>
将任何数值或字符串转换为一个整数值。 可选的 Java 十进制格式可用于转换。将截断任何 long、 float、 double
* ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
