...
将从 UTC 转换为时间戳。 可以选择性地传递 'GMT'、 '太平洋标准时间、 'UTC'、 ' America/开曼窗体中的时区。 默认为当前时区 * ``fromUTC(currentTimeStamp()) -> 12-12-2030T19:18:12``
...
根据条件应用一个值或另一个值。 如果未指定另一个是它被视为 NULL。 这两个值必须兼容（数字、字符串...）* ``iif(custType == 'Premium', 10, 4.5)``
* ``iif(amount > 100, 'High')``
...
获取当前行之前由第一个参数计算的 n 行的值。 第二个参数是要回顾的行数，默认值为 1。 如果行数不多，则返回 null 值，除非指定了默认值 * ``lag(amount, 2) -> 60``
...
获取当前行之后由第一个参数计算的 n 行的值。 第二个参数是要期待的行数，默认值为 1。 如果行数不多，则返回 null 值，除非指定了默认值 * ``lead(amount, 2) -> 60``
...
返回 NULL 值。 如果没有名为 null 的列名称，请使用函数 syntax(null())。 使用该函数的任何操作将导致 NULL * ``custId = NULL (for derived field)``
* ``custId == NULL -> NULL``
* ``'nothing' + NULL -> NULL``
* ``10 * NULL -> NULL'``
* ``NULL == '' -> NULL'``
...
舍入给定可选的小数位数和可选的舍入模式。 如果省略小数位数，则它是默认设置为 0。  如果省略模式，则默认为 ROUND_HALF_UP(5)。 舍入运算的值包括：1 - ROUND_UP；2 - ROUND_DOWN；3 - ROUND_CEILING；4 - ROUND_FLOOR；5 - ROUND_HALF_UP；6 - ROUND_HALF_DOWN；7 - ROUND_HALF_EVEN；8 - ROUND_UNNECESSARY * ``round(100.123) -> 100.0``
* ``round(2.5, 0) -> 3.0``
...
根据可选的时间戳格式将字符串转换为日期。 所有可能的格式，请参阅 Java SimpleDateFormat。 如果省略时间戳，则使用默认模式 yyyy-[M]M-[d]d hh:mm:ss[.f...] * ``toTimestamp('2016-12-31 00:12:00') -> 2012-8-8T00:12:00``
...
==============================
<code><b>toInteger(<i>&lt;value&gt;</i> : any, [<i>&lt;format&gt;</i> : string]) => integer</b></code><br/><br/>
将任何数值或字符串转换为一个整数值。 可以使用可选的 Java 十进制格式进行转换。截断任何长整型数、浮点数和双精度数 * ``toInteger(123) -> 123``
* ``toInteger('123') -> 123``
* ``toInteger('$123', '$###') -> 123``
*********************************
<code>toLong</code>
