...
NTile 函数将每个窗口分区的行分割为从 1 到 `n` 的 `n` 桶。 桶值最大相差 1。 如果分区中的行数不能均匀分割成桶数，则余值将逐个分布在每个桶中，从第一个桶开始。 NTile 函数是 tertiles、 四分位数、 十分位数和其他常见的摘要统计信息的计算特别有用。 在初始化期间，该函数将计算两个变量：正则存储桶的大小将具有一个额外的行添加到它。 这两个变量基于当前分区的大小。 在计算过程中，该函数将跟踪当前行号、当前桶号，以及发生桶更改的行号 (bucketThreshold)。 如果当前行号达到桶的阈值，则桶值将会加 1，阈值将按桶大小增加（如果当前桶已填充，则额外加 1）。
* ``nTile() -> 1``
