...
1. 选择如何连接 SQL 托管实例，即是通过专用终结点，还是通过公共终结点：
    ...
    - 通过公共终结点
        ...
        Azure SQL 托管实例可通过[公共终结点](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-public-endpoint-configure)进行连接。 若要允许 SQL 托管实例和 Azure-SSIS IR 之间的流量，必须满足入站和出站要求：
