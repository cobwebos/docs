<properties
	pageTitle="构建第一个数据工厂 (Visual Studio) | Microsoft Azure"
...
4. 在解决方案资源管理器中，右键单击“链接服务”，指向“添加”，然后单击“新建项”。
...
3. 将 **JSON** 替换为以下代码片段。
	...
	> [AZURE.IMPORTANT] 将 **storageaccountname** 替换为存储帐户名。
...
18. 在“解决方案资源管理器”中，右键单击该项目，然后单击“发布”。
...
21. 在“配置数据工厂”页中执行以下步骤：
	1. 选择“新建数据工厂”选项。
	2. 输入数据工厂的唯一**名称**。例如：**FirstDataFactoryUsingVS09152016**。该名称必须全局唯一。
	
		> [AZURE.IMPORTANT] 如果在发布时收到错误：“数据工厂名称 ‘FirstDataFactoryUsingVS’ 不可用”，请更改名称（例如 yournameFirstDataFactoryUsingVS）。有关数据工厂项目命名规则，请参阅 [Data Factory - Naming Rules](data-factory-naming-rules.md)（数据工厂 - 命名规则）主题。
	3. 在“订阅”字段中选择正确的订阅。
	 
		> [AZURE.IMPORTANT] 如果未看到任何订阅，请确保使用属于订阅管理员或共同管理员的帐户登录。
	4. 为要创建的数据工厂选择**资源组**。
	5. 为数据工厂选择**区域**。
...
### 使用图示视图监视管道
6. 登录到 [Azure 门户](https://portal.azure.com/)，执行以下操作：
