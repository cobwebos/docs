...
|"\@{pipeline().parameters.myString}"| 返回字符串形式的 `foo`。|  
|"\@pipeline().parameters.myNumber"| 返回`42`数字*形式的* 。|  
|"\@{pipeline().parameters.myNumber}"| 返回`42`字符串*形式的* 。|  
...
|substring|返回字符串中的字符子集。 例如，以下表达式：<br /><br /> `substring('somevalue-foo-somevalue',10,3)`<br /><br /> 返回：<br /><br /> `foo`<br /><br /> **参数数目**：1<br /><br /> **名称**：字符串<br /><br /> **说明**：必需。 要从中获取子字符串的字符串。<br /><br /> **参数数目**：2<br /><br /> **名称**：起始索引<br /><br /> **说明**：必需。 参数 1 中子字符串的起始索引。 起始索引是从零开始的。 <br /><br /> **参数数目**：3<br /><br /> **名称**：长度<br /><br /> **说明**：必需。 子字符串的长度。|  
...
|indexof|在字符串中不区分大小写查找值的索引。 索引是从零开始的。 例如，以下表达式返回 `7`: `indexof('hello, world.', 'world')`<br /><br /> **参数数目**：1<br /><br /> **名称**：字符串<br /><br /> **说明**：必需。 可能包含该值的字符串。<br /><br /> **参数数目**：2<br /><br /> **名称**：字符串<br /><br /> **说明**：必需。 要搜索其索引的值。|  
|lastindexof|在字符串中不区分大小写查找值的最后一个索引。 索引是从零开始的。 例如，以下表达式返回 `3`: `lastindexof('foofoo', 'foo')`<br /><br /> **参数数目**：1<br /><br /> **名称**：字符串<br /><br /> **说明**：必需。 可能包含该值的字符串。<br /><br /> **参数数目**：2<br /><br /> **名称**：字符串<br /><br /> **说明**：必需。 要搜索其索引的值。|  
...
|交集|返回在传递给此函数的数组与对象之间具有共同元素的单个数组或对象。 例如，以下函数返回 `[1, 2]`：<br /><br /> `intersection([1, 2, 3], [101, 2, 1, 10],[6, 8, 1, 2])`<br /><br /> 该函数的参数可以是对象集或数组集（但不能是两者的混合）。 如果存在两个同名的对象，最终的对象中会显示使用该名称的最后一个对象。<br /><br /> **参数数目**：1 ... *n*<br /><br /> **名称**：Collection *n*<br /><br /> **说明**：必需。 要计算的集合。 要在结果中显示某个对象，该对象必须在传入的所有集合中。|  
|union|返回其所有元素出现在传递给此函数的数组或对象中的单个数组或对象。 例如，此函数返回 `[1, 2, 3, 10, 101]:`<br /><br /> :  `union([1, 2, 3], [101, 2, 1, 10])`<br /><br /> 该函数的参数可以是对象集或数组集（但不能是两者的混合）。 如果最终的输出中存在两个同名的对象，最终的对象中会显示使用该名称的最后一个对象。<br /><br /> **参数数目**：1 ... *n*<br /><br /> **名称**：Collection *n*<br /><br /> **说明**：必需。 要计算的集合。 显示在任意集合中的对象会显示在结果中。|  
...
|和|如果两个参数均为 true，则返回 true。 两个参数需是布尔值。 以下表达式返回 `false`:  `and(greater(1,10),equals(0,0))`<br /><br /> **参数数目**：1<br /><br /> **名称**：布尔值 1<br /><br /> **说明**：必需。 必须为 `true` 的第一个参数。<br /><br /> **参数数目**：2<br /><br /> **名称**：布尔值 2<br /><br /> **说明**：必需。 必须为 `true` 的第二个参数。|  
...
|Array|将参数转换为数组。  例如，以下表达式返回 `["abc"]`: `array('abc')`<br /><br /> **参数数目**：1<br /><br /> **名称**：值<br /><br /> **说明**：必需。 要转换为数组的值。|
...
|mul|返回两个数字相乘的结果。 例如，以下返回 `103.33`:<br /><br /> `mul(10,10.333)`<br /><br /> **参数数目**：1<br /><br /> **名称**：被乘数 1<br /><br /> **说明**：必需。 要与**被乘数 2** 相乘的数字。<br /><br /> **参数数目**：2<br /><br /> **名称**：被乘数 2<br /><br /> **说明**：必需。 要与**被乘数 1** 相乘的数字。|  
|div|返回两个数字相除的结果。 例如，以下返回 `1.0333`:<br /><br /> `div(10.333,10)`<br /><br /> **参数数目**：1<br /><br /> **名称**：被除数<br /><br /> **说明**：必需。 要与**除数**相除的数字。<br /><br /> **参数数目**：2<br /><br /> **名称**：除数<br /><br /> **说明**：必需。 要与**被除数**相除的数字。|  
...
|min|可通过两种不同的模式来调用此函数：`min([0,1,2])` min 在此取一个数组。 此表达式返回 `0`。 或者，此函数可以采用值的逗号分隔列表：`min(0,1,2)` 此函数也会返回 0。 注意：所有值都必须为数字，因此，如果参数为数组，该数组必须仅包含数字。<br /><br /> **参数数目**：1<br /><br /> **名称**：集合或值<br /><br /> **说明**：必需。 要查找最小值，它可以是一组值，也可以是一组值的第一个值。<br /><br /> **参数数目**：2 ... *n*<br /><br /> **名称**：Value *n*<br /><br /> **说明**：可选 如果第一个参数是一个值，则可以传递其他值，并返回所有传递的值的最小值。|  
|max|可通过两种不同的模式来调用此函数：`max([0,1,2])`<br /><br /> max 在此处采用一个数组。 此表达式返回 `2`。 或者，此函数可以采用值的逗号分隔列表：`max(0,1,2)` 此函数会返回 2。 注意：所有值都必须为数字，因此，如果参数为数组，该数组必须仅包含数字。<br /><br /> **参数数目**：1<br /><br /> **名称**：集合或值<br /><br /> **说明**：必需。 要查找最大值，它可以是一组值，也可以是一组值的第一个值。<br /><br /> **参数数目**：2 ... *n*<br /><br /> **名称**：Value *n*<br /><br /> **说明**：可选 如果第一个参数是一个值，则可以传递其他值，这样就会返回所有传递值的最大值。|  
...
|adddays|将整数天数添加到传入的字符串时间戳。 天数可为正数或负数。 默认情况下，结果是采用 ISO 8601 格式 ("o") 的字符串，除非提供了格式说明符。 例如 `2015-02-23T13:27:36Z`：<br /><br /> `adddays('2015-03-15T13:27:36Z', -20)`<br /><br /> **参数数目**：1<br /><br /> **名称**：时间戳<br /><br /> **说明**：必需。 包含时间的字符串。<br /><br /> **参数数目**：2<br /><br /> **名称**：天<br /><br /> **说明**：必需。 要添加的天数。 可为负数（减去相应的天数）。<br /><br /> **参数数目**：3<br /><br /> **名称**：格式<br /><br /> **说明**：可选 [单个格式说明符](https://msdn.microsoft.com/library/az4se3k1%28v=vs.110%29.aspx)或[自定义格式模式](https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx)，指示如何设置此时间戳值的格式。 如果未提供格式，则使用 ISO 8601 格式 ("o")。|  
|formatDateTime|返回日期格式的字符串。 默认情况下，结果是采用 ISO 8601 格式 ("o") 的字符串，除非提供了格式说明符。 例如 `2015-02-23T13:27:36Z`：<br /><br /> `formatDateTime('2015-03-15T13:27:36Z', 'o')`<br /><br />若要在 "yyyy/MM/dd" 中设置日期的格式，请使用 formatDateTime （utcnow （），"yyyy/MM/dd"）。</br>若要在日期后面追加名称，请使用 @concat（"foo-"、"/"、formatDateTime （utcnow （）、"yyyy/MM/dd"））。<br><br> **参数数目**：1<br /><br /> **名称**：日期<br /><br /> **说明**：必需。 包含日期的字符串。<br /><br /> **参数数目**：2<br /><br /> **名称**：格式<br /><br /> **说明**：可选 [单个格式说明符](https://msdn.microsoft.com/library/az4se3k1%28v=vs.110%29.aspx)或[自定义格式模式](https://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx)，指示如何设置此时间戳值的格式。 如果未提供格式，则使用 ISO 8601 格式 ("o")。 |  
