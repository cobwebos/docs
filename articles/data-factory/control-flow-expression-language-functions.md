...
在以下示例中，BlobDataset 采用名为 **path** 的参数。 其值用于通过使用 expression： `dataset().path` 来设置**folderPath**属性的值。 
...
本[教程](https://azure.microsoft.com/mediahandler/files/resourcefiles/azure-data-factory-passing-parameters/Azure%20data%20Factory-Whitepaper-PassingParameters.pdf)将指导您如何在管道和活动之间以及活动之间传递参数。
...
|分钟|可通过两种不同的模式来调用此函数：`min([0,1,2])` min 在此取一个数组。 此表达式返回 `0`。 或者，此函数可以采用值的逗号分隔列表：`min(0,1,2)` 此函数也会返回 0。 注意：所有值都必须为数字，因此，如果参数为数组，该数组必须仅包含数字。<br /><br /> **参数数目**：1<br /><br /> **名称**：集合或值<br /><br /> **说明**：必需。 要查找最小值，它可以是一组值，也可以是一组值的第一个值。<br /><br /> **参数数目**：2 ... *n*<br /><br /> **名称**：Value *n*<br /><br /> **说明**：可选 如果第一个参数是一个值，则可以传递其他值，并返回所有传递的值的最小值。|  
