---
title: 基于时间分区的文件名以增量方式复制新文件
description: 创建 Azure 数据工厂，然后使用复制数据工具基于时间分区的文件名以增量方式加载新文件。
...
ms.locfileid: "75982624"
---
# <a name="incrementally-copy-new-files-based-on-time-partitioned-file-name-by-using-the-copy-data-tool"></a>使用复制数据工具以增量方式复制基于时间分区文件名的新文件

在本教程中，我们将使用 Azure 门户创建数据工厂。 然后，使用 "复制数据" 工具创建一个管道，该管道可根据从 Azure Blob 存储到 Azure Blob 存储的时间分区文件名将新文件增量复制到 Azure Blob 存储。
...
1. 创建一个名为**source**的容器。  在容器中创建一个文件夹路径为**2019/02/26/14** 。 创建一个空的文本文件，并将其命名为**file1**。 将 file1 上传到存储帐户中的文件夹路径**源/2019/02/26/14** 。  可以使用各种工具（例如 [Azure 存储资源管理器](https://storageexplorer.com/)）来执行这些任务。

    ![上传文件](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/upload-file.png)

    > [!NOTE]
    > 请将文件夹名称调整为 UTC 时间。  例如，如果当前 UTC 时间为2:03 年2月 26 2019 日，则可以按**source/{Year}/{Month}/{Day}/{Hour}** /的规则创建**源/2019/02/26/14** /文件夹路径。

2. 创建一个名为**destination**的容器。 可以使用各种工具（例如 [Azure 存储资源管理器](https://storageexplorer.com/)）来执行这些任务。
...
2. 在 "**属性**" 页上，执行以下步骤：
    ...
    b.保留“数据库类型”设置，即设置为“共享”。 在**任务节奏或任务计划**下，选择 "**按计划定期运行**"。

    c. 在 "**触发器类型**" 下，选择 "**翻转窗口**"。

    d.单击“下一步”。 在 "**重复周期**" 下，输入**1 小时**。
...
6. 在 "**选择输出文件或文件夹**" 页上，执行以下步骤：

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 浏览并选择**目标**文件夹，然后单击 "**选择**"。

    ![选择输出文件或文件夹](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/choose-output-file-folder.png)

    b.保留“数据库类型”设置，即设置为“共享”。 将动态文件夹路径编写为**source/{year}/{month}/{day}/{小时}** /，并按如下所示更改格式：
...
10. 请注意，界面中已自动选择左侧的“监视”选项卡。  自动触发管道时需要等待管道运行（大约一小时后）。  当它运行时，"**操作**" 列包含用于查看活动运行详细信息的链接以及用于重新运行管道的链接。 选择 "**刷新**" 以刷新列表，并在 "**操作**" 列中选择 "**查看活动运行**" 链接。

    ![监视管道运行](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs1.png)
11. 该管道只包含一个活动（复制活动），因此只显示了一个条目。 你可以看到，源文件（file1）已从**源/2019/02/26/14/** **目标/2019/02/26/14/** 中复制，具有相同的文件名。  

    ![监视管道运行](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs2.png)

    你还可以使用 Azure 存储资源管理器（ https://storageexplorer.com/) 来验证文件。

    ![监视管道运行](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs3.png)
12. 再创建一个新名称为**file2**的空文本文件。 将 file2 文件上传到存储帐户中的文件夹路径**源/2019/02/26/15** 。   可以使用各种工具（例如 [Azure 存储资源管理器](https://storageexplorer.com/)）来执行这些任务。

    ![监视管道运行](./media/tutorial-incremental-copy-partitioned-file-name-copy-data-tool/monitor-pipeline-runs4.png)

    > [!NOTE]
    > 你可能会注意到需要创建新的文件夹路径。 请将文件夹名称调整为 UTC 时间。  例如，如果当前 UTC 时间为3:20 年2月 26 2019 日下午，则可以按照 **{Year}/{Month}/{Day}/{Hour}/** 的规则创建**源/2019/02/26/15/** 的文件夹路径。
