...
本文概述了如何在 Azure 数据工厂中使用复制活动在 Azure DocumentDB 和其他数据存储之间移动数据。 本文基于[数据移动活动](data-factory-data-movement-activities.md)一文，其中总体概述了如何结合使用复制活动和受支持的数据存储进行数据移动。

以下示例演示如何将数据复制到 Azure DocumentDB 和 Azure Blob 存储，以及如何从中复制数据。 但是，可以从任何源将数据**直接**复制到任何受支持的接收器。 有关详细信息，请参阅[使用复制活动移动数据](data-factory-data-movement-activities.md)中的“支持的数据存储和格式”部分。  
...
此 DocumentDB 连接器支持从 DocumentDB 单分区集合和已分区集合复制数据，以及将数据复制到 DocumentDB 单分区集合和已分区集合。 不支持 [MongoDB 的 DocDB](../documentdb/documentdb-protocol-mongodb.md)。
...
2. [AzureStorage](data-factory-azure-blob-connector.md) 类型的链接服务。
3. [DocumentDbCollection](#azure-documentdb-dataset-type-properties) 类型的输入[数据集](data-factory-create-datasets.md)。
...
5. 包含复制活动的一个[管道](data-factory-create-pipelines.md)，该复制活动使用 [DocumentDbCollectionSource](#azure-documentdb-copy-activity-type-properties) 和 [BlobSink](data-factory-azure-blob-connector.md#azure-blob-copy-activity-type-properties)。
...
5. 包含复制活动的一个[管道](data-factory-create-pipelines.md)，该复制活动使用 [BlobSource](data-factory-azure-blob-connector.md#azure-blob-copy-activity-type-properties) 和 [DocumentDbCollectionSink](#azure-documentdb-copy-activity-type-properties)。
...
## <a name="azure-documentdb-dataset-type-properties"></a>Azure DocumentDB 数据集类型属性
