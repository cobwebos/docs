...
- 从 SAP HANA 源并行复制。 有关详细信息，请参阅[SAP HANA 部分的并行副本](#parallel-copy-from-sap-hana)。
...
有关可用于定义数据集的节和属性的完整列表，请参阅[数据集](concepts-datasets-linked-services.md)一文。 本部分提供 SAP HANA 数据集支持的属性列表。
...
>[!TIP]
>通过使用数据分区高效地从 SAP HANA 中引入数据，[请从 SAP HANA 部分的并行副本](#parallel-copy-from-sap-hana)中学习详细信息。
...
## <a name="parallel-copy-from-sap-hana"></a>来自 SAP HANA 的并行副本

数据工厂 SAP HANA 连接器提供内置数据分区，以并行复制来自 SAP HANA 的数据。 可以在复制活动的“源”表中找到数据分区选项。****
