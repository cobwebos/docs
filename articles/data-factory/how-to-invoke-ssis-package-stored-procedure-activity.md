...
1. 要触发管道运行，请单击工具栏上的 **"触发"，** 然后单击 **"立即触发**"。 
...
5. 若要创建数据工厂，请运行下面的 **Set-AzDataFactoryV2** cmdlet，使用 $ResGrp 变量中的 Location 和 ResourceGroupName 属性： 
...
3. 运行**集-AzDataFactoryV2链接服务**cmdlet 以创建链接服务 **：AzureSql数据库链接服务**。 
...
2. 要创建管道：**运行SSIS包管道**，运行**集阿兹达工厂V2管道**cmdlet。
