...
| rowDelimiter     | 用于分隔文件中的行的单个字符或 "\r\n"。<br>默认值为读取时的以下任何值 **： ["\r\n"、"\r"、"\n"]** 、 **"n" 或 "\r\n"** （通过分别映射数据流和复制活动进行写入）。 <br>将 @no__t 设置为 "无分隔符（空字符串）" 时，还必须将 @no__t 设置为 "无分隔符（空字符串）"，这意味着将整个内容视为单个值。 | No       |
...
| escapeChar       | 用于转义带引号的值中的引号的单个字符。<br>默认值为**反斜杠 `\`** 。 <br>对于映射数据流，`escapeChar` 不能是空字符串。 <br/>对于复制活动，当 `escapeChar` 定义为空字符串时，还必须将 @no__t 设置为空字符串，在这种情况下，请确保所有列值都不包含分隔符。 | No       |
...
| compressionLevel | 压缩比。 <br>允许的值为 "**最佳**" 或 "**最快**"。<br>@no__t 0**速度最快：** 应尽快完成压缩操作，即使生成的文件未进行最佳压缩。<br>- **最优**：应以最佳方式压缩压缩操作，即使操作需要更长的时间才能完成。 有关详细信息，请参阅 [Compression Level](https://msdn.microsoft.com/library/system.io.compression.compressionlevel.aspx)（压缩级别）主题。 | No       |
