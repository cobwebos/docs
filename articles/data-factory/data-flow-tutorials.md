---
title: 映射数据流视频教程
description: 介绍映射数据流概念的视频教程列表
author: djpmsft
ms.author: daperlov
ms.service: data-factory
ms.topic: tutorial
ms.date: 03/04/2020
ms.openlocfilehash: bb996b0abc16a3d6171c8f7ab2ac0e64dd0c1075
ms.sourcegitcommit: be53e74cd24bbabfd34597d0dcb5b31d5e7659de
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 03/11/2020
ms.locfileid: "79096798"
---
# <a name="mapping-data-flow-video-tutorials"></a>映射数据流视频教程

下面是由 Azure 数据工厂团队创建的映射数据流教程视频列表。 

随着产品的不断更新，当前 Azure 数据工厂用户体验中已添加了一些功能或不同功能。

## <a name="getting-started"></a>入门

[Azure 数据工厂中的映射数据流入门](http://youtu.be/MFw7t_8tuV4)

## <a name="debugging-and-developing-mapping-data-flows"></a>调试和开发映射数据流

[调试和测试映射数据流](https://www.youtube.com/watch?v=3ANxyvDGfjA)。

[数据浏览](https://www.youtube.com/watch?v=En1ztyh5GaA)

[数据预览快速操作](https://www.youtube.com/watch?v=CP0TnNmaLA0)

[监视和管理映射数据流性能](https://www.youtube.com/watch?v=fktIWdJiqTk)

[基准计时](http://youtu.be/6CSbWm4lRhw?hd=1)

## <a name="transformation-overviews"></a>转换概述

[聚合转换](http://youtu.be/jdL75xIr98I)

[更改行转换](https://www.youtube.com/watch?v=4ktoohwptmQ)

[派生列转换](https://www.youtube.com/watch?v=FFCbU4ujCiY)

[联接转换](https://www.youtube.com/watch?v=zukwayEXRtg)

[自联接模式](https://www.youtube.com/watch?v=Dx1kANfnvmk&feature=youtu.be&hd=1)

[查找转换](https://www.youtube.com/watch?v=9U-0VPU2ZPU)

[透视转换](https://www.youtube.com/watch?v=Tua14ZQA3F8&t=34s)

[透视转换：映射偏移列](https://youtu.be/5MygzCX0wnM)

[选择转换](http://youtu.be/F9VjQ_YyRyU?hd=1)

[选择转换：基于规则的映射](https://youtu.be/5lf1lh1qMwU)

[代理键转换](https://www.youtube.com/watch?v=ISpegL9CbTM)

[联合转换](http://youtu.be/_Et6mg1tEr8?hd=1)

[逆透视转换](http://youtu.be/KFYkxcpB8b0?hd=1)

[窗口转换](https://www.youtube.com/watch?v=m6zgbtY5AYQ)

[筛选转换](https://youtu.be/OhbKDOXSfeE)

[有条件拆分转换](http://youtu.be/W1lQHojhKZw?hd=1)

[存在转换](http://youtu.be/GS8JVgNBMfs?hd=1)

[动态联接和动态查找](https://youtu.be/CMOPPie9bXM)

[平展转换](https://youtu.be/VY2tFQJoAXE)

## <a name="source-and-sink"></a>源和接收器

[读取和写入 JSON](https://www.youtube.com/watch?v=yY5aB7Kdhjg)

[Parquet 和分隔文本文件](https://youtu.be/V_2a60j2Kjo)

[CosmosDB 连接器](http://youtu.be/plp1etT2ftY?hd=1)

[推断分隔文本文件中的数据类型](https://www.youtube.com/watch?v=nJjRzlFktlA)

[读取和写入分区文件](https://www.youtube.com/watch?v=7Q-db4Qgc4M)

[转换和创建多个 SQL 表](https://www.youtube.com/watch?v=Sj15Yjwai1A)

[在数据湖中对文件进行分区](https://youtu.be/VNWv-MvLQ_0)

[数据仓库加载模式](https://www.youtube.com/watch?v=7mLqwtmeQFg)

## <a name="optimizing-mapping-data-flows"></a>优化映射数据流

[数据世系](https://www.youtube.com/watch?v=5KvqYF-y93s)

[优化提示](https://www.youtube.com/watch?v=a2KtwUJngHo)

[使用参数循环访问文件](http://youtu.be/uEgz0ptYRDM?hd=1)

[缩短启动时间](https://youtu.be/FFCbU4ujCiY?t=528)

[SQL DB 性能](https://youtu.be/iyZT5CY3V_4)

[日志记录和审核](https://www.youtube.com/watch?v=UC__LOyuf8M)

## <a name="mapping-data-flow-scenarios"></a>映射数据流方案

[模糊查找](http://youtu.be/7gdwExjHBbw)

[暂存数据模式](https://youtu.be/mZLKdyoL3Mo)

[清理地址模式](https://youtu.be/axEYbuU3lmw)

[重复数据删除](https://www.youtube.com/watch?v=OLenvYwg__I)

[合并文件](http://youtu.be/WbDTBAyYte8)

[缓慢变化的维度类型 1：覆盖](http://youtu.be/Rz2zx5GRbrA)

[缓慢变化的维度类型 2：历史记录](http://youtu.be/123CptslKvU)

[事实数据表加载](http://youtu.be/ABG3X9pgFPQ)

[使用增量数据加载模式转换本地 SQL Server](https://youtu.be/IN-4v0e7UIs)

[参数化](https://www.youtube.com/watch?v=vpuuQcFojt8)

[非重复行和行计数](https://youtu.be/ryYo8UFUgTI)

[处理截断错误](http://youtu.be/sPpcSiKQz34)

[智能数据路由](https://youtu.be/PIGw-Z-0upw)

[敏感数据的数据掩码](https://www.youtube.com/watch?v=OFd4LeiTmfs)

[逻辑模型与物理模型](http://youtu.be/K5tgzLjEE9Q)

[检测源数据更改](http://youtu.be/CaxIlI7oXfI?hd=1)

## <a name="data-flow-expressions"></a>数据流表达式

[日期/时间表达式](https://www.youtube.com/watch?v=uboyCZ25r_E&feature=youtu.be&hd=1)

[拆分数组和 Case 语句](https://www.youtube.com/watch?v=DHNH8ZO7YjI)

[使用字符串内插和参数的乐趣](https://youtu.be/hb3-cn2CMgM)
