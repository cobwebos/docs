...
每小时从新的 blob 获取数据一次（频率：小时，间隔：1）。 根据处理中切片的开始时间，动态评估 blob 的文件夹路径和文件名。 文件夹路径使用开始时间的年、月和日部分，文件名使用开始时间的小时部分。 “external”: “true” 设置将告知数据工厂服务：表位于数据工厂外且不由数据工厂中的活动生成。
...
| compression |指定数据的压缩类型和级别。 支持的类型为：**GZip**、**Deflate** 和 **BZip2**，支持的级别为：**Optimal** 和 **Fastest**。 **AvroFormat** 或 **OrcFormat** 格式的数据目前不支持进行压缩设置。 有关详细信息，请参阅[压缩支持](#compression-support)部分。 |否 |
...
* **Type：**压缩编解码器，可以是 **GZIP**、**Deflate** 或 **BZIP2**。  
...
假设将示例数据集用作复制活动的输出。 复制活动通过使用最佳比率的 GZIP 编解码器压缩输出数据，然后将压缩的数据写入 Azure Data Lake Store 中名为 pagecounts.csv.gz 的文件。   

在输入数据集 JSON 中指定压缩属性时，管道将从源读取压缩的数据。 在输出数据集 JSON 中指定属性时，复制活动可以将压缩的数据写入目标中。 下面是一些示例方案：

* 从 Azure Data Lake Store 读取 GZIP 压缩的数据，将其解压缩，然后将结果数据写入 Azure SQL 数据库。 在此情况下，可以使用压缩 JSON 属性定义输入 Azure Data Lake Store 数据集。
* 从来自本地文件系统的纯文本文件读取数据、使用 GZip 格式进行压缩并将压缩的数据写入到 Azure Data Lake Store。 在此情况下，可以使用压缩 JSON 属性定义输出 Azure Data Lake 数据集。  
* 从 Azure Data Lake Store 读取 GZIP 压缩的数据，将其解压缩、使用 BZIP2 将其压缩，然后将结果数据写入 Azure Data Lake Store。 将输入和输出数据集的压缩类型集分别设置为 GZIP 和 BZIP2。   
