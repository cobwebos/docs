...
| --- | --- |
| 活动名称 | 管道中的活动的名称 |
| 活动类型 | 活动的类型，如**复制**、**执行数据流**或**AzureML 执行管道** |
...
将任何管道活动属性提升为用户属性，使其成为受监视的实体。 例如，您可以将管道中复制活动的**源****和目标**属性提升为用户属性。 选择“自动生成”****，为复制活动生成“源”**** 和“目标”**** 用户属性。
...
| 运行状态 | 筛选器按状态运行：**成功**"、"**失败**"、"**已排队**"、"**已取消"** 或 **"正在进行**"。 |
| 批注 | 按应用到每个管道的标记进行筛选 |
| 运行次数 | 筛选是否要查看重新运行的管道 |
...
### <a name="rerun-from-failed-activity"></a>从失败活动重新运行

如果活动失败、超时或已取消，则可以通过**从失败活动中选择"重新运行**"来从该失败活动重新运行管道。

![重新运行失败活动](media/monitor-visually/rerun-failed-activity.png)
...
可在数据工厂中发出有关受支持指标的警报。 在数据工厂监视页上选择 **"监视** > **警报&指标**以开始。
