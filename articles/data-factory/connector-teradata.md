...
description: 利用数据工厂服务的 Teradata 连接器, 你可以将数据从 Teradata 数据库复制到数据工厂支持的数据存储作为接收器。
...
本文概述如何使用 Azure 数据工厂中的复制活动从 Teradata 数据库复制数据。 它基于[复制活动概述](copy-activity-overview.md)进行构建。
