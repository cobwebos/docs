...
| avroCompressionCodec | 写入 Avro 文件时要使用的压缩编解码器。 从 Avro 文件中读取数据时，数据工厂会根据文件元数据自动确定压缩编解码器。<br>支持的类型为 "**none**" （默认值）、"**deflate**" 和 "**snappy**"。 注意：当前复制活动在读取/写入 Avro 文件时不支持 Snappy。 | 是       |
