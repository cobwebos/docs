...
有关可用于定义数据集的各部分和属性的完整列表，请参阅数据集一文。 本部分提供文件系统数据集支持的属性列表。
...
| folderPath | 文件夹路径。 不支持通配符筛选器。 有关示例，请参阅 [Sample linked service and dataset definitions](#sample-linked-service-and-dataset-definitions)（链接服务和数据集定义示例）。 |否 |
