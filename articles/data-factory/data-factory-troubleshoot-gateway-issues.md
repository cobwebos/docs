...
> [!NOTE]
> 有关网关的详细信息，请参阅[数据管理网关](data-factory-data-management-gateway.md)一文。 请参阅[在本地与云之间移动数据](data-factory-move-data-between-onprem-and-cloud.md)一文，了解通过使用网关将数据从本地 SQL Server 数据库移动到 Azure blob 存储的演练。 
> 
> 

## <a name="failed-to-install-or-register-gateway"></a>无法安装或注册网关
### <a name="1-problem"></a>1.问题
在安装/注册网关，尤其是在下载网关安装文件时，会出现此错误消息。 
...
请检查防火墙/代理服务器设置，查看是否是设置阻止了计算机网络连接到[下载中心](https://download.microsoft.com/)，并相应地更新设置。 

或者，可在其他可访问下载中心的计算机上从[下载中心](https://www.microsoft.com/en-US/download/details.aspx?id=39717)下载最新网关的安装文件，然后将安装程序文件复制到网关主机计算机，最后手动运行安装程序文件来安装/更新网关。
...
### <a name="resolution"></a>解决方法
在 Azure 门户中验证是否使用的是正确的网关密钥。 如果需要，可重新生成密钥并使用该密钥注册网关。
...
### <a name="cause"></a>原因
输入网关密钥的内容或格式不正确。 其中一个原因可能是（使用无效密钥）仅从门户复制了副本的一部分。

### <a name="resolution"></a>解决方法
在 Azure 门户中生成网关密钥，并使用复制按钮复制整个密钥，然后将其粘贴在此窗口来注册网关。
...
### <a name="cause"></a>原因
网关密钥已重新生成或已在 Azure 门户中删除了网关。

### <a name="resolution"></a>解决方法
如果网关仍然存在，请在 Azure 门户中重新生成网关密钥，并使用复制按钮复制整个密钥，然后将其粘贴在此窗口来注册网关。 否则，重新创建网关，然后重新开始。
...
### <a name="resolution"></a>解决方法
如果已通过使用“网关配置管理器”的“设置”选项卡上的“导出”按钮，从原始网关计算机中导出了凭据证书，则请在此处使用该证书。

恢复网关时不能跳过此阶段。 如果证书丢失，则需从门户中删除网关，然后重新创建新的网关。  此外，需要通过重新输入凭据来更新所有与网关相关的链接服务。 
...
### <a name="resolution"></a>解决方法
按照本文档中[代理服务器注意事项](#proxy-server-considerations)部分中的说明，通过配置管理器配置代理设置。
...
网关联机但功能受限时，可能无法使用数据工厂复制向导创建用于向/从本地数据存储复制数据的数据管道。 解决办法是使用 Azure 门户、Visual Studio 或 Azure PowerShell 中的数据工厂编辑器。
...
* 允许网关计算机和企业防火墙上的 IP 地址出站规则。 可通过 Windows 事件日志查找 IP 地址 (ID == 401)：尝试了以其访问权限 XX.XX.XX.XX:9350 禁止的方式访问套接字
...
### <a name="problem"></a>问题
尝试使用门户上的凭据管理器输入新链接服务的凭据或更新现有链接服务的凭据时，可能会出现此错误。

`Error: The data store '<Server>/<Database>' cannot be reached. Check connection settings for the data source.`

看到此错误时，配置管理器的设置页可能如下所示：

![无法访问数据库](media/data-factory-troubleshoot-gateway-issues/database-cannot-be-reached.png)

### <a name="cause"></a>原因
网关计算机上的 SSL 证书可能已丢失。 网关无法加载当前用于 SSL 加密的证书。 还可能在事件日志中看到类似于这样的错误消息：“无法从云服务获取网关设置。 请检查网关密钥和网络连接。 （无法加载具有指纹的证书。）”
...
### <a name="1-problem"></a>1.问题
在 Azure 门户中设置管道后可能会出现下面的“UserErrorFailedToConnectToSqlserver”失败。 
...
如果目标数据库为 Azure SQL 数据库，请一并检查 Azure SQL Server 防火墙。

请参阅下面的部分，了解测试本地数据存储连接的步骤。 

## <a name="data-store-connection-or-driver-related-errors"></a>数据存储连接或驱动程序相关错误
如果看到数据存储连接或驱动程序相关错误，请执行以下步骤： 

1. 在网关计算机上启动“数据管理网关配置管理器”。
2. 切换到“诊断”选项卡
3. 使用此网关组在“测试对本地数据源的连接”中为字段选择或输入适当的值
4. 单击“测试连接”，查看是否可使用连接信息和凭据从网关计算机连接到本地数据源。 如果安装驱动程序后测试连接仍然失败，请重新启动网关以便它应用最新的更改
...
联系 Microsoft 支持部门以获取解决网关问题的帮助时，可能会要求你共享网关日志。 通过网关的发布，只需在网关配置管理器中点击两个按钮便可轻松共享所需的网关日志。    

1. 切换到网关配置管理器的“诊断”选项卡。
   
    ![数据管理网关 - 诊断选项卡](media/data-factory-troubleshoot-gateway-issues/data-management-gateway-diagnostics-tab.png)
2. 单击“发送日志”链接查看以下对话框： 
...
4. （可选）单击“隐私”查看 Microsoft 在线服务隐私声明。 
5. 如若对要上传的内容感到满意，便可单击“发送日志”将过去 7 天内的日志发送给 Microsoft，以便进行故障排除。 可看到“发送日志”操作的状态如下图所示：
   
    ![数据管理网关 - 发送日志状态](media/data-factory-troubleshoot-gateway-issues/data-management-gateway-send-logs-status.png)
6. 操作完成后，会看到如下图所示的对话框：
   
    ![数据管理网关 - 发送日志状态](media/data-factory-troubleshoot-gateway-issues/data-management-gateway-send-logs-result.png)
7. 记下“报告 ID”，并将其与 Microsoft 支持部门共享。 此报告 ID 用于查找你已经上传以进行故障排除的网关日志。  此报告 ID 也会保存在事件查看器中以供参考。  可通过查找事件 ID“25”找到该报表 ID，并查看日期和时间。
...
* 手动安装并注册网关；
* 尝试在配置管理器上使用重新生成的密钥注册网关； 
* 尝试发送日志，且无法连接网关主机服务；

此类情况下，可将网关日志保存为 zip 文件，之后在联系 Microsoft 支持部门时将其共享。 例如在注册网关时收到错误，如下图所示：   
...
可以在 Windows 事件日志中的网关日志内找到详细信息。 可通过“应用程序和服务日志” > “数据管理网关”下的“Windows 事件查看器”进行查找。 解决网关相关问题时，在事件查看器中查找错误级别事件。
