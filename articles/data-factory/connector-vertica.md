---
title: 使用 Azure 数据工厂（Beta 版本）从 Vertica 复制数据 | Microsoft Docs
...
ms.locfileid: "34617845"
---
# <a name="copy-data-from-vertica-using-azure-data-factory-beta"></a>使用 Azure 数据工厂（Beta 版本）从 Vertica 复制数据
...
| connectionString | 用于连接到 Vertica 的 ODBC 连接字符串。 将此字段标记为 SecureString 以安全地将其存储在数据工厂中或[引用存储在 Azure Key Vault 中的机密](store-credentials-in-key-vault.md)。 | 是 |
...
有关可用于定义数据集的各部分和属性的完整列表，请参阅[数据集](concepts-datasets-linked-services.md)一文。 本部分提供 Vertica 数据集支持的属性列表。

要从 Vertica 复制数据，请将数据集的 type 属性设置为“VerticaTable”。 此类型的数据集中没有任何其他特定于类型的属性。
...
有关可用于定义活动的各部分和属性的完整列表，请参阅[管道](concepts-pipelines-activities.md)一文。 本部分提供 Vertica 源支持的属性列表。

### <a name="vertica-as-source"></a>以 Vertica 作为源

要从 Vertica 复制数据，请将复制活动中的源类型设置为“VerticaSource”。 复制活动**源**部分支持以下属性：
