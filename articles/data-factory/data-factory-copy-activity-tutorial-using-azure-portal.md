---
title: 教程：使用 Azure 门户创建包含复制活动的管道 | Microsoft Docs
...
3. 请注意，在“新建数据工厂”边栏选项卡中，“添加到启动板”处于选中状态。
4. 在“新建数据工厂”边栏选项卡中单击“创建”。
   ...
   > 
   > 
5. 在创建过程中单击左侧的“通知”中心可以查看通知。如果“通知”边栏选项卡处于打开状态，请单击“X”将其关闭。
6. 完成创建后，将显示图中所示的“数据工厂”边栏选项卡。
...
4. 单击工具栏上的“部署”，部署 AzureStorageLinkedService。确认标题栏中显示了“已成功创建链接服务”消息。
...
1. 单击工具栏上的“部署”，创建并部署 **EmpTableFromBlob** 表。确认编辑器标题栏中显示了“已成功创建表”消息。
...
1. 单击工具栏上的“部署”，创建并部署 **EmpSQLTable** 表。
...
2. 将右窗格中的 JSON 替换为以下 JSON 代码片段：
   ...
   * 在 **transformation** 节中，**BlobSource** 指定为源类型，**SqlSink** 指定为接收器类型。
     ...
     有关 JSON 属性的详细信息，请参阅 [JSON Scripting Reference](http://go.microsoft.com/fwlink/?LinkId=516971)（JSON 脚本参考）。
3. 单击工具栏上的“部署”，创建并部署 **ADFTutorialPipeline**。确认显示了“已成功创建管道”消息。
4. 现在，请单击“X”关闭“编辑器”边栏选项卡。再次单击“X”关闭包含工具栏和树视图的“ADFTutorialDataFactory”边栏选项卡。如果看到“将丢弃未保存的编辑”消息，请单击“确定”。
5. 随即返回 **ADFTutorialDataFactory** 的“数据工厂”边栏选项卡。
...
6. 请注意，到目前为止，数据切片已生成并已“就绪”，因为 **emp.txt** 文件始终在 Blob 容器中：**adftutorial\\input**。确认底部的“最近失败的切片”部分中没有任何切片。
