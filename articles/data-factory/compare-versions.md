...
| 活动 | “活动”用于定义在管道中对数据执行的操作。 支持数据移动（复制活动）和数据转换活动（例如 Hive、Pig 和 MapReduce）。 | 在数据工厂的当前版本中，活动仍然是在管道内定义的操作。数据工厂的当前版本引入了新的[控制流活动](concepts-pipelines-activities.md#control-activities)。 可以在控制流（循环和分支）中使用这些活动。 在 V1 中受支持的数据移动和数据转换活动在当前版本中也受支持。 在当前版本中，在定义转换活动时可以不使用数据集。 |
