...
- **建议**：更新 Azure 函数以返回有效的 JSON 有效负载。 例如， C#函数可以返回 \"Id `(ActionResult)new<OkObjectResult("{`\"：\"123\"`}");`。
...
- **建议**：考虑限制提交到 HDInsight 的并发作业的数目。 如果作业正在由同一活动提交，则请参阅数据工厂活动并发性。 更改触发器，将并发管道运行分散到不同的时间。 请参阅 HDInsight 文档，根据错误的建议调整 `templeton.parallellism.job.submit`。
...
- **建议**：作业已成功提交到 HDInsight。 群集上的失败。 打开 HDInsight Ambari UI 中的作业和日志，或从存储中打开该文件，如错误消息所述。 文件显示错误详细信息。
...
- **建议**： hdinsight 设置失败时，此错误来自 hdinsight 服务。 联系 HDInsight 团队并提供按需群集名称。
...
- **建议**：受支持的 HTTP 方法为 PUT、POST、GET 和 DELETE。
...
- **建议**：检查访问的资源的用户权限。 使用 Fiddler 或 Postman 检查 URL 的功能。
