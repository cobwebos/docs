...
| JSON 元素 | 描述 | Type | 允许的值 | 需要 |
|:--- |:--- |:--- |:--- |:--- |
| 类型 | 触发器的类型。 类型为固定值 "TumblingWindowTrigger"。 | 字符串 | "TumblingWindowTrigger" | 是 |
...
| **interval** | 一个正整数，表示 **frequency** 值对应的时间间隔，决定了触发器的运行频率。 例如，如果 **interval** 为 3，**frequency** 为“hour”，则触发器每 3 小时重复触发一次。 <br/>**注意**：最小窗口间隔为15分钟。 | Integer | 正整数。 | 是 |
| **startTime**| 第一个匹配项，可以是过去的时间。 第一个触发器间隔是 (**startTime**, **startTime** + **interval**)。 | 日期/时间 | 一个日期时间值。 | 是 |
| **endTime**| 最后一个匹配项，可以是过去的时间。 | 日期/时间 | 一个日期时间值。 | 是 |
| **delay** | 延迟窗口数据处理开始的时间量。 管道运行在预期的执行时间加上 **delay** 的量之后启动。 **delay** 的定义是：在预期的执行时间过后，触发器在触发新的运行之前等待的时间。 **delay** 不改变窗口 **startTime**。 例如，值为 00:10:00 的 **delay** 意味着 10 分钟的延迟。 | Timespan<br/>(hh:mm:ss)  | 一个时间跨度值，默认值为 00:00:00。 | No |
...
| **retryPolicy: Count** | 将管道运行标记为“失败”之前的重试次数。  | Integer | 一个整数，其默认值为 0（不重试）。 | No |
| **retryPolicy: intervalInSeconds** | 重试之间的延迟（以秒为单位指定）。 | Integer | 秒数，其默认值为 30。 | No |
| **dependsOn：键入** | TumblingWindowTriggerReference 的类型。 如果设置了依赖关系，则为必需。 | 字符串 |  "TumblingWindowTriggerDependencyReference", "SelfDependencyTumblingWindowTriggerReference" | No |
| **dependsOn： size** | 依赖项翻转窗口的大小。 | Timespan<br/>(hh:mm:ss)  | 正 timespan 值，其中默认为子触发器的窗口大小  | No |
| **dependsOn： offset** | 依赖项触发器的偏移量。 | Timespan<br/>(hh:mm:ss) |  自依赖项中必须为负的 timespan 值。 如果未指定任何值，则窗口与触发器本身相同。 | 自依赖项：是<br/>其他：否  |
...
2. 使用**AzDataFactoryV2Trigger** cmdlet 创建触发器：

    ```powershell
    Set-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger" -DefinitionFile "C:\ADFv2QuickStartPSH\MyTrigger.json"
    ```
    
3. 使用**AzDataFactoryV2Trigger** cmdlet 确认触发器的状态为 "**已停止**"：
...
5. 使用**AzDataFactoryV2Trigger** cmdlet 确认触发器的状态为 "**已启动**"：

    ```powershell
    Get-AzDataFactoryV2Trigger -ResourceGroupName $ResourceGroupName -DataFactoryName $DataFactoryName -Name "MyTrigger"
    ```

6. 使用**AzDataFactoryV2TriggerRun** cmdlet 获取触发器在 Azure PowerShell 中运行。 若要获取有关触发器运行的信息，请定期执行以下命令。 更新 **TriggerRunStartedAfter** 和 **TriggerRunStartedBefore** 值以匹配触发器定义中的值：
