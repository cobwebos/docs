...
6. **Blob 路径以开头**， **blob 路径**以属性结尾，可用于指定要为其接收事件的容器、文件夹和 blob 名称。 事件触发器需要至少定义这些属性中的一个。 可以为“Blob 路径开头为”和“Blob路径结尾为”属性使用各种模式，如本文中后面的示例所示。
    ...
    * **Blob 路径以以下结尾：** Blob 路径必须以文件名或扩展名结束。 有效值包括 `shoes.csv` 和 `.csv`。 容器和文件夹名称是可选的，但在指定时，它们必须由 `/blobs/` 段分隔。 例如，名为 "orders" 的容器的值可以是 `/orders/blobs/2018/april/shoes.csv`。 若要在任何容器中指定一个文件夹，请省略前导 "/" 字符。 例如，`april/shoes.csv` 将在任何容器中名为 "四月" 的文件夹 a 中的任何名为 `shoes.csv` 的文件上触发事件。 
...
8. 完成配置后，请单击 "**下一步"：数据预览**。 此屏幕显示事件触发器配置匹配的现有 blob。 请确保已有特定的筛选器。 配置过于广泛的筛选器会匹配创建/删除的大量文件，这可能会显著影响你的成本。 筛选条件验证完成后，单击 "**完成**"。

    ![事件触发器数据预览](media/how-to-create-event-trigger/event-based-trigger-image3.png)

9. 若要将管道附加到此触发器，请跳到管道画布并单击 "**添加触发器**"，然后选择 "**新建/编辑**"。 出现侧导航时，单击 "**选择触发器 ...** " 下拉列表，然后选择所创建的触发器。 单击 "**下一步：数据预览**" 以确认配置正确 **，然后单击**"验证数据预览"。
...
在前面的示例中，当在容器示例-数据中的文件夹事件测试中创建以 .csv 结尾的 blob 路径时，将触发器配置为激发。 **FolderPath**和**fileName**属性捕获新 blob 的位置。 例如，将 MoviesDB 添加到路径示例-数据/事件测试时，`@triggerBody().folderPath` 的值为 `sample-data/event-testing`，`@triggerBody().fileName` 的值为 `moviesDB.csv`。 示例中将这些值映射到管道参数 `sourceFolder` 和 `sourceFile`，可以在整个管道中将它们用作 `@pipeline().parameters.sourceFolder` 和 `@pipeline().parameters.sourceFile`。
...
| **blobPathBeginsWith** | blob 路径必须使用为要触发的触发器提供的模式开头。 例如，`/records/blobs/december/` 只会触发 `december` 容器下 `records` 文件夹中的 blob 触发器。 | String   | | 必须为其中至少一个属性提供值：`blobPathBeginsWith` 或 `blobPathEndsWith`。 |
| **blobPathEndsWith** | blob 路径必须使用为要触发的触发器提供的模式结尾。 例如，`december/boxes.csv` 只会触发 `boxes` 文件夹中名为 `december` 的 blob 的触发器。 | String   | | 必须为其中至少一个属性提供值：`blobPathBeginsWith` 或 `blobPathEndsWith`。 |
...
> [!IMPORTANT]
> 每当指定容器和文件夹、容器和文件或容器、文件夹和文件时，都必须包含路径的 `/blobs/` 段，如以下示例所示。 对于**blobPathBeginsWith**，数据工厂 UI 会自动在触发器 JSON 中的文件夹和容器名称之间添加 `/blobs/`。
...
| **Blob 路径开头** | `/containername/blobs/foldername/file.txt` | 接收 `file.txt` 容器下的 `foldername` 文件夹中名为 `containername` 的 blob 事件。 |
| **Blob 路径结尾** | `file.txt` | 接收任何路径中名为 `file.txt` 的 blob 事件。 |
| **Blob 路径结尾** | `/containername/blobs/file.txt` | 接收容器 `file.txt` 下名为 `containername` 的 blob 事件。 |
| **Blob 路径结尾** | `foldername/file.txt` | 接收任何容器下 `file.txt` 文件夹中名为 `foldername` 的 blob 事件。 |
