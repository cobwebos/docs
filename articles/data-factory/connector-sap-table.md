...
| `systemNumber` | SAP 系统的系统编号。<br/>用于连接到 SAP 应用程序服务器。<br/>允许值：表示为字符串的两位小数。 | 否 |
...
| `partitionLowerBound`              | `partitionColumnName` 中指定的用于继续分区的列的最小值。 （注意：`partitionLowerBound`当分区选项为`PartitionOnInt`时，不能为"0" | 否       |
