...
ms.locfileid: "74930834"
---
# <a name="copy-data-from-phoenix-using-azure-data-factory"></a>使用 Azure 数据工厂从 Phoenix 复制数据 

本文概述了如何使用 Azure 数据工厂中的复制活动从 Phoenix 复制数据。 是基于总体介绍复制活动的[复制活动概述](copy-activity-overview.md)一文进行扩展的。
...
| port | Phoenix 服务器用来侦听客户端连接的 TCP 端口。 默认值为 8765。 如果连接到 Azure HDInsights，请指定端口 443。 | No |
| httpPath | 对应于 Phoenix 服务器的部分 URL。 （即，/gateway/sandbox/phoenix/version）。 如果使用 HDInsights 群集，请指定 `/hbasephoenix0`。  | No |
| authenticationType | 用于连接到 Phoenix 服务器的身份验证机制。 <br/>允许的值为：**Anonymous**、**UsernameAndPassword**、**WindowsAzureHDInsightService** | 是 |
| username | 用于连接到 Phoenix 服务器的用户名。  | No |
