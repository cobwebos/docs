...
对于在使用 Parquet 文件序列化/反序列化的自承载 IR 上运行的复制，ADF 通过首先检查是否*`(SOFTWARE\JavaSoft\Java Runtime Environment\{Current Version}\JavaHome)`* 存在 JRE 的注册表来定位 Java 运行时，如果*`JAVA_HOME`* 未找到，则再次检查 OpenJDK 的系统变量。
