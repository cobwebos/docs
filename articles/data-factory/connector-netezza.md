...
| 属性 | 说明 | 必须 |
|:--- |:--- |:--- |
| SecurityLevel | 驱动程序用于连接到数据存储区的安全级别 (SSL/TLS)。 示例：`SecurityLevel=preferredSecured`。 支持的值是：<br/>- **仅安全**（**onlyUnSecured**）：驱动程序不使用 TLS。<br/>- **首选不安全（preferredUnSecured）（默认）**：如果服务器提供了选择，则驱动程序不使用 TLS。 <br/>- **首选的安全（preferredSecured）**：如果服务器提供了选择，则驱动程序将使用 TLS。 <br/>- **仅安全（onlySecured）**：除非 TLS 连接可用，否则驱动程序不会连接。 | 否 |
| CaCertFile | 服务器使用的 TLS/SSL 证书的完整路径。 示例： `CaCertFile=<cert path>;`| 是，如果启用 TLS |
