...
| 将数据复制到 Azure SQL 数据库或 Azure Cosmos DB |4 到 16，具体取决于接收器 Azure SQL 数据库或 Cosmos DB 的层 （Dtu/Ru 数） 个 |
| 所有其他复制方案 | 4 |
...
> [!TIP]
> 复制之间基于文件的存储的数据时，默认行为会给您的最佳吞吐量。 默认行为是自动确定根据源文件模式。
...
* 当从不基于文件的 （除非已启用数据分区的源 Oracle 数据库） 的存储复制数据到基于文件的存储，数据移动服务将忽略**parallelCopies**属性。 即使指定了并行性，在此情况下也不适用。
...
目前，不能复制通过不同的自承载 Ir，具有和不暂存复制的情况下都不连接的两个数据存储之间的数据。 对于此类方案中，可以配置两个显式链接的复制活动要复制到过渡环境，来源，然后从过渡到接收器。
