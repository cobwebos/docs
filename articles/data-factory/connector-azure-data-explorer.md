...
>[!TIP]
>有关使用 Azure 数据资源管理器连接器的演练，请参阅[使用 Azure 数据工厂将数据复制到 azure 数据资源管理器](../data-explorer/data-factory-load-data.md)。
...
2. 在 Azure 数据资源管理器中授予服务主体适当的权限。 有关角色和权限的详细信息，请参阅[管理 Azure 数据资源管理器数据库权限](../data-explorer/manage-database-permissions.md)，并演练管理权限。 一般情况下，你需要
...
| database | 数据库的名称。 | 是 |
| 租户 | 指定应用程序的租户信息（域名或租户 ID）。 在 [Kusto 连接字符串](https://docs.microsoft.com/azure/kusto/api/connection-strings/kusto#application-authentication-properties)中，这通常称为“机构/授权 ID”。 可以将鼠标指针悬停在 Azure 门户右上角来检索它。 | 是 |
...
| noTruncation | 指示是否截断返回的结果集。 默认情况下，在500000记录或 64 MB 后，结果将被截断。 强烈建议截断以确保活动的行为正常。 |否 |
