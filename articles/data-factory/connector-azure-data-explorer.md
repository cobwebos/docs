...
>[!NOTE]
>使用数据工厂 UI 创作时，登录用户帐户用于列出 Azure 数据资源管理器群集、数据库和表。 如果您没有这些操作的权限，请手动输入名称。
...
| queryTimeout | 查询请求超时前的等待时间。默认值为 10 分钟（00：10：00）;允许的最大值为 1 小时（01：00：00）。 | 否 |
...
| 其他属性 | 属性包，可用于指定 Azure 数据资源管理器接收器尚未设置的任何引入属性。 具体来说，它可用于指定引入标记。 从[Azure 数据浏览数据引入文档](https://kusto.azurewebsites.net/docs/management/data-ingestion/index.html)中了解更多信息。 | 否 |
