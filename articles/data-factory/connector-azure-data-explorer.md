...
| 终结点 | Azure 数据资源管理器群集的终结点 URL，格式为 `https://<clusterName>.<regionName>.kusto.windows.net`。 | 是 |
| 数据库 | 数据库的名称。 | 是 |
...
| noTruncation | 指示是否截断返回的结果集。 默认情况下, 在500000记录或64MB 后, 结果将被截断。 强烈建议对活动的适当行为进行截断。 |否 |
