...
如果将 Azure SQL 数据库与可以通过加入虚拟网络来托管 SSISDB 的虚拟网络服务器终结点/托管实例（预览版）配合使用，则还必须将 Azure-SSIS 集成运行时加入同一虚拟网络。 使用 Azure 数据工厂版本 2（预览版）可将 Azure-SSIS 集成运行时加入虚拟网络。 有关详细信息，请参阅[将 Azure-SSIS 集成运行时加入虚拟网络](join-azure-ssis-integration-runtime-virtual-network.md)。
