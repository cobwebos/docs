...
| 主机 | Presto 服务器的 IP 地址或主机名。 （即 192.168.222.160）  | 是 |
...
要从 Presto 复制数据，请将数据集的 type 属性设置为“PrestoObject”。 在此类型的数据集中没有任何其他特定于类型的属性。
...
有关可用于定义活动的各个部分和属性的完整列表，请参阅[管道](concepts-pipelines-activities.md)一文。 本部分提供 Presto 数据源支持的属性列表。
