...
> * [版本 1 - 正式版](data-factory-onprem-teradata-connector.md)
> * [版本 2 - 预览版](../connector-teradata.md)

> [!NOTE]
> 本文适用于数据工厂版本 1（即正式版 (GA)）。 如果使用数据工厂服务版本 2（预览版），请参阅 [V2 中的 Teradata 连接器](../connector-teradata.md)。
...
管道包含配置为使用输入和输出数据集，且计划每小时运行一次的复制活动。 在管道 JSON 定义中，“源”类型设置为 **RelationalSource**，“接收器”类型设置为 **BlobSink**。 为 **query** 属性指定的 SQL 查询选择复制过去一小时的数据。
