...
| enableServerCertificateValidation | 指定当源为 HTTPS Web 服务器时，是否启用服务器 SSL 证书验证。 当 HTTPS 服务器使用自签名证书时，将其设置为 **false**。 | No<br /> （默认值为 true） |
| gatewayName | 用于连接本地 HTTP 源的数据管理网关实例的名称。 | 是（如果从本地 HTTP 源复制数据） |
| encryptedCredential | 用于访问 HTTP 终结点的已加密凭据。 在复制向导中或使用 **ClickOnce** 对话框配置身份验证信息时，将自动生成该值。 | No<br /> （仅当从本地 HTTP 服务器复制数据时才适用） |
...
| relativeUrl | 包含数据的资源的相对 URL。 未指定路径时，仅使用链接服务定义中指定的 URL。 <br><br> 若要构造动态 URL，可以使用[数据工厂函数和系统变量](data-factory-functions-variables.md)。 示例：**relativeUrl**: **$$Text.Format('/my/report?month={0:yyyy}-{0:MM}&fmt=csv', SliceStart)** 。 | No |
| requestMethod | HTTP 方法。 允许的值为 **GET** 和 **POST**。 | No <br />（默认值为 **GET**） |
| additionalHeaders | 附加的 HTTP 请求标头。 | No |
| requestBody | HTTP 请求的正文。 | No |
| format | 如果要*从 HTTP 终结点按原样检索数据*而不分析它，请跳过 **format** 设置。 <br><br> 如果要在复制期间分析 HTTP 响应内容，下面是支持的格式类型：**TextFormat**、**JsonFormat**、**AvroFormat**、**OrcFormat** 和 **ParquetFormat**。 有关详细信息，请参阅[文本格式](data-factory-supported-file-and-compression-formats.md#text-format)、[JSON 格式](data-factory-supported-file-and-compression-formats.md#json-format)、[Avro 格式](data-factory-supported-file-and-compression-formats.md#avro-format)、[Orc 格式](data-factory-supported-file-and-compression-formats.md#orc-format)和 [Parquet 格式](data-factory-supported-file-and-compression-formats.md#parquet-format)。 |No |
...
| properties | 描述 | 需要 |
| -------- | ----------- | -------- |
| httpRequestTimeout | 用于获取响应的 HTTP 请求的超时 （TimeSpan 值）。 这是获取响应而不是读取响应数据的超时。 | No<br />（默认值：**00:01:40**） |
