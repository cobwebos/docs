...
本文创建了一个包含转换活动（HDInsight 活动）的管道，可在按需 HDInsight 群集上运行 Hive 脚本。 若要了解如何使用“复制”活动将数据从 Blob 存储复制到 Azure SQL 数据库，请参阅[教程：将数据从 Blob 存储复制到 SQL 数据库](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md)。
