...
| typeProperties | 每种类型（例如 Azure Blob、Azure SQL 表）的类型属性各不相同。 有关支持的类型及其属性的详细信息，请参阅[数据集类型](#Type)。 |是 |NA |
...
* **tableName** type 属性（特定于 AzureSqlTable 类型）设置为 MyTable。
...
| style |指定是否应在间隔的开头/结尾生成切片。<ul><li>StartOfInterval</li><li>EndOfInterval</li></ul>如果**frequency**设置为**Month**，并且**style**设置为**EndOfInterval**，则会在每月的最后一天生成切片。 若将“style”**** 设置为“StartOfInterval”****，则会在每月的第一天生成切片。<br/><br/>如果**frequency**设置为**Day**，并且**style**设置为**EndOfInterval**，则会在一天的最后一小时生成切片。<br/><br/>如果**frequency**设置为**Hour**，并且**style**设置为**EndOfInterval**，则会在该小时结束时生成切片。 例如，对于下午 1 点到下午 2 点期间的切片，则在下午 2 点生成切片。 |否 |EndOfInterval |
