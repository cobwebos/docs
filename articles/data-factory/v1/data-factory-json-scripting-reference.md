...
在下面的示例中，数据集有三个列：`slicetimestamp`、`projectname` 和 `pageviews`，并且它们的类型分别为：String、String 和 Decimal。
...
#### <a name="example-blobsource"></a>例如：**BlobSource**
```json
{
    "name": "SamplePipeline",
    "properties": {
...
#### <a name="example-azuredatalakestoresource"></a>例如：AzureDataLakeStoreSource
...
| writeBatchSize |向 Azure Cosmos DB 服务发送创建文档的并行请求数。<br/><br/>将数据复制到 Azure Cosmos DB 以及从其中复制数据时，可使用此属性对性能进行微调。 增加 writeBatchSize 时，由于会向 Azure Cosmos DB 发送更多的并行请求，因此可以获得更好的性能。 但是，需要避免可能会引发以下错误消息的限制：“请求速率大”。<br/><br/>限制由多种因素决定，包括文档大小、文档中的术语数、目标集合的索引策略等。对于复制操作，可以使用更佳的集合（例如 S3）实现最大吞吐量（2,500 请求单位/秒）。 |Integer |否（默认值：5） |
...
#### <a name="example-json-for-using-sql-authentication"></a>例如：用于使用 SQL 身份验证的 JSON
...
#### <a name="example-sample-dataset-with-prefix"></a>例如：带前缀的示例数据集
...
| encryptedCredential |指定可以通过运行新建 AzDataFactoryEncryptValue cmdlet 获取的加密的凭据。 |否（如果选择在纯文本中指定 userid 和密码） |
...
#### <a name="example-using-port-enablessl-enableservercertificatevalidation"></a>例如：使用端口、enableSsl、enableServerCertificateValidation
...
#### <a name="example-using-basic-authentication"></a>例如：使用基本身份验证
...
| --- | --- | --- |
| url | Web 服务器的基 URL | 是 |
| authenticationType | 指定身份验证类型。 允许值包括：**Anonymous**、**Basic**、**Digest**、**Windows**、**ClientCertificate**。 <br><br> 有关这些身份验证类型的其他属性和 JSON 示例，请分别参阅此表格下面的部分。 | 是 |
...
#### <a name="example-using-clientcertificate-authentication"></a>例如：使用 ClientCertificate 身份验证
...
| requestMethod | Http 方法。 允许的值为 **GET** 或 **POST**。 | 否。 默认为 `GET`。 |
| additionalHeaders | 附加的 HTTP 请求标头。 | 否 |
| requestBody | HTTP 请求的正文。 | 否 |
| format | 如果只想要**从 HTTP 终结点按原样检索数据**而不分析它，请跳过此格式设置。 <br><br> 如果要在复制期间分析 HTTP 响应内容，则支持以下格式类型：TextFormat、JsonFormat、AvroFormat、OrcFormat、ParquetFormat      。 有关详细信息，请参阅[文本格式](data-factory-supported-file-and-compression-formats.md#text-format)、[Json 格式](data-factory-supported-file-and-compression-formats.md#json-format)、[Avro 格式](data-factory-supported-file-and-compression-formats.md#avro-format)、[Orc 格式](data-factory-supported-file-and-compression-formats.md#orc-format)和 [Parquet 格式](data-factory-supported-file-and-compression-formats.md#parquet-format)部分。 |否 |
| compression | 指定数据的压缩类型和级别。 支持的类型包括：**GZip**、**Deflate**、**BZip2** 和 **ZipDeflate**。 支持的级别为：“最佳”和“最快”   。 有关详细信息，请参阅 [Azure 数据工厂中的文件和压缩格式](data-factory-supported-file-and-compression-formats.md#compression-support)。 |否 |
...
可以对使用的凭据进行加密[新建 AzDataFactoryEncryptValue](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactoryencryptvalue) cmdlet。
...
| version |HDInsight 群集的版本。 若要了解详细信息，请参阅 [Azure 数据工厂中支持的 HDInsight 版本](data-factory-compute-linked-services.md#supported-hdinsight-versions-in-azure-data-factory)。 |否 |
...
| osType |操作系统的类型。 允许值包括：Windows（默认值）和 Linux |否 |
...
[存储过程活动](#stored-procedure-activity) | 可在数据工厂管道中使用存储过程活动来调用以下数据存储之一的存储过程：你的企业或 Azure VM 中的 Azure SQL 数据库、Azure SQL 数据仓库、SQL Server 数据库。
...
在 Azure 机器学习工作室批处理执行活动 JSON 定义中，可以指定以下属性。 活动的类型属性必须是：**AzureMLBatchExecution**。 必须创建 Azure 机器学习链接的服务首先和它的名称指定的值为**linkedServiceName**属性。 将活动类型设置为 AzureMLBatchExecution 时，**typeProperties** 节支持以下属性：
...
在该 JSON 示例中，部署的 Azure 机器学习 Web 服务使用读取器和编写器模块从 Azure SQL 数据库读取数据，或将数据写入 Azure SQL 数据库。 此 Web 服务公开以下四个参数：数据库服务器名称、数据库名称、服务器用户帐户名称和服务器用户帐户密码。
...
在 Azure 机器学习工作室更新资源活动 JSON 定义中，可以指定以下属性。 活动的类型属性必须是：**AzureMLUpdateResource**。 必须创建 Azure 机器学习链接的服务首先和它的名称指定的值为**linkedServiceName**属性。 将活动类型设置为 AzureMLUpdateResource 时，**typeProperties** 节支持以下属性：
...
| 属性 | 说明 | 需要 |
|:--- |:--- |:--- |
| AssemblyName | 程序集的名称。 在此示例中，它是：**MyDotnetActivity.dll**。 | 是 |
| EntryPoint |实现 IDotNetActivity 接口的类名称。 在此示例中，它是：**MyDotNetActivityNS.MyDotNetActivity**，其中，MyDotNetActivityNS 是命名空间，MyDotNetActivity 是类。  | 是 |
| PackageLinkedService | Azure 存储链接服务的名称，指向包含自定义活动 zip 文件的 Blob 存储。 在此示例中，它是：**AzureStorageLinkedService**。| 是 |
