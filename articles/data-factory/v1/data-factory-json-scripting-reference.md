...
| 类型 | 数据集的类型。 指定 Azure 数据工厂支持的类型之一（例如：AzureBlob、AzureSqlTable）。 有关数据工厂支持的所有数据存储和数据集类型，请参阅[数据存储](#data-stores)部分。 |
...
| Property | 描述 | 必需 |
| --- | --- | --- |
| storedProcedureName |指定 Azure SQL 数据库或 Azure SQL 数据仓库中的存储过程的名称，该存储过程由输出表使用的链接服务表示。 |是 |
