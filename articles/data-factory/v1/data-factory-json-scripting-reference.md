...
| 活动 | 包含活动列表。 | 是 |
| start |管道的开始日期-时间。 必须为 [ISO 格式](https://en.wikipedia.org/wiki/ISO_8601)。 例如：2014-10-14T16:32:41。 <br/><br/>可指定本地时间，如 EST 时间。 下面是一个示例：`2016-02-27T06:00:00**-05:00`（美国东部标准时间上午 6 点）。<br/><br/>start 和 end 属性共同指定管道的活动期限。 仅在此活动期限内生成输出切片。 |否<br/><br/>如果要指定 end 属性值，必须指定 start 属性值。<br/><br/>创建管道时，开始和结束时间均可为空。 必须指定这两个值，才能设置管道运行的活动期限。 如果在创建管道时未指定开始和结束时间，则可以使用"设置-AzDataFactoryPipelineActivePeriod"cmdlet 设置它们。 |
...
* **定义**部分用于指定作为 Hive 配置值（例如`${hiveconf:inputtable}`），`${hiveconf:partitionedtable}`传递到 hive 脚本的运行时设置。
...
| anchorDateTime |定义计划程序用于计算数据集切片边界的时间中的绝对位置。 <br/><br/><b>注意</b>：如果 AnchorDateTime 的日期部分比频率更细粒度，则忽略更细粒度的零件。 <br/><br/>例如，如果<b>间隔</b><b>是每小时</b>（频率：小时和间隔：1）和<b>AnchorDateTime</b>包含<b>分钟和秒</b>，则忽略 AnchorDateTime 的<b>分钟和秒</b>部分。 |否 |01/01/0001 |
...
| fileName |blob 的名称。 fileName 可选，并且区分大小写。<br/><br/>如果指定文件名，则活动（包括复制）将对特定 Blob 起作用。<br/><br/>如果未指定 fileName，则复制将包括输入数据集的 folderPath 中所有的 Blob。<br/><br/>未为输出数据集指定 fileName 时，生成的文件的名称将采用以下格式：（`Data.<Guid>.txt`例如： Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt） |否 |
...
如果要从 Azure Blob 存储复制数据，请将复制活动的**源类型**设置为**BlobSource**，并在**源**部分中指定以下属性：
...
| properties | 描述 | 允许的值 | 必选 |
| --- | --- | --- | --- |
| copyBehavior |源为 BlobSource 或 FileSystem 时，请定义复制行为。 |<b>保留层次结构</b>：保留目标文件夹中的文件层次结构。 从源文件到源文件夹的相对路径与从目标文件到目标文件夹的相对路径相同。<br/><br/><b>FlattenHierarchy：</b>源文件夹中的所有文件都位于目标文件夹的第一级。 目标文件具有自动生成的名称。 <br/><br/><b>MergeFiles：（默认值）</b>将源文件夹中的所有文件合并到一个文件中。 如果指定文件/Blob 名称，则合并的文件名称将为指定的名称；否则，会自动生成文件名。 |否 |
...
| properties | 描述 | 必选 |
|:--- |:--- |:--- |
| type | 类型属性必须设置为 **：AzureDataLakeStore** | 是 |
...
| subscriptionId | 数据湖存储所属的 Azure 订阅 ID。 | 接收器所需 |
...
| sessionID | OAuth 授权会话中的 OAuth 会话 ID。 每个会话 ID 是唯一的，只能使用一次。 使用数据工厂编辑器时，会自动生成此设置。 | 是（对于用户凭据身份验证） |
...
| fileName |Azure Data Lake Sore 中文件的名称。 fileName 可选，并且区分大小写。 <br/><br/>如果指定 filename，则活动（包括复制）将对特定文件起作用。<br/><br/>如果未指定 fileName，则复制将包括输入数据集的 folderPath 中的所有文件。<br/><br/>未为输出数据集指定 fileName 时，生成的文件的名称将采用以下格式：（`Data.<Guid>.txt`例如： Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt） |否 |
| partitionedBy |partitionedBy 是一个可选属性。 它可用于指定时序数据的动态 folderPath 和 filename。 例如，folderPath 可针对每小时的数据参数化。 |否 |
| format | 支持以下格式类型：**TextFormat**、**JsonFormat**、**AvroFormat**、**OrcFormat** 和 **ParquetFormat**。 请将格式中的“type”属性设置为上述值之一****。 有关详细信息，请参阅[文本格式](data-factory-supported-file-and-compression-formats.md#text-format)、[Json 格式](data-factory-supported-file-and-compression-formats.md#json-format)、[Avro 格式](data-factory-supported-file-and-compression-formats.md#avro-format)、[Orc 格式](data-factory-supported-file-and-compression-formats.md#orc-format)和 [Parquet 格式](data-factory-supported-file-and-compression-formats.md#parquet-format)部分。 <br><br> 如果想要在基于文件的存储之间按原样复制文件****（二进制副本），可以在输入和输出数据集定义中跳过格式节。 |否 |
| compression | 指定数据的压缩类型和级别。 支持的类型为：GZip、Deflate、BZip2 和 ZipDeflate****************。 支持的级别为：最佳和最快********。 有关详细信息，请参阅 [Azure 数据工厂中的文件和压缩格式](data-factory-supported-file-and-compression-formats.md#compression-support)。 |否 |
...
| properties | 描述 | 允许的值 | 必选 |
| --- | --- | --- | --- |
| copyBehavior |指定复制行为。 |<b>保留层次结构</b>：保留目标文件夹中的文件层次结构。 从源文件到源文件夹的相对路径与从目标文件到目标文件夹的相对路径相同。<br/><br/><b>拼合层次结构</b>：源文件夹中的所有文件都在目标文件夹的第一级创建。 创建目标文件时，自动生成名称。<br/><br/><b>合并文件</b>：将源文件夹中的所有文件合并到一个文件。 如果指定文件/Blob 名称，则合并的文件名称将为指定的名称；否则，会自动生成文件名。 |否 |
...
| writeBatchSize |向 Azure Cosmos DB 服务发送创建文档的并行请求数。<br/><br/>将数据复制到 Azure Cosmos DB 以及从其中复制数据时，可使用此属性对性能进行微调。 增加 writeBatchSize 时，由于会向 Azure Cosmos DB 发送更多的并行请求，因此可以获得更好的性能。 但是，需要避免可能会引发“请求速率大”的错误消息的限制。<br/><br/>限制由多种因素决定，包括文档大小、文档中的术语数、目标集合的索引策略等。对于复制操作，可以使用更好的集合（例如 S3）具有最可用的吞吐量（2，500 个请求单位/秒）。 |Integer |否（默认值：5） |
...
### <a name="linked-service"></a>链接服务
要定义 Azure 认知搜索链接服务，请设置链接到**Azure 搜索**的服务**的类型**，并在**typeProperties**部分中指定以下属性：
...
### <a name="dataset"></a>数据集
要定义 Azure 认知搜索数据集，请将数据集**的类型**设置为**AzureSearchIndex**，并在**typeProperties**部分中指定以下属性：
...
| WriteBatchSize | 当缓冲区大小达到 writeBatchSize 时，将数据上载到搜索索引中。 | 1 到 1,000。 默认值为 1000。 | 否 |
...
您可以使用**New-AzDataFactoryEncryptValue** cmdlet 加密凭据，并在连接字符串中使用它们，如以下示例所示（**加密凭据**属性）：
...
### <a name="linked-service"></a>链接服务
您可以使用本地**文件服务器**链接服务将本地文件系统链接到 Azure 数据工厂。 下表提供了特定于本地文件服务器链接服务的 JSON 元素的说明。
...
| enableServerCertificateValidation |指定在 SSL/TLS 通道上使用 FTP 时是否启用服务器 TLS/SSL 证书验证 |否 |true |
...
#### <a name="example-basic-authentication-with-encrypted-credential"></a>示例：**使用加密凭据进行基本身份验证**
...
| enableServerCertificateValidation | 指定是否启用服务器 TLS/SSL 证书验证（如果源是 HTTPS Web 服务器） | 不启用，默认值为 true |
...
| gatewayName |网关名称 - 数据工厂服务应使用此网关连接到本地 OData 服务。 仅指定从内部 OData 源复制数据时。 |否 |
...
您可以使用["新建-AzData 工厂加密值](https://docs.microsoft.com/powershell/module/az.datafactory/new-azdatafactoryencryptvalue)"cmdlet 加密凭据。
...
| 计算环境 | 活动 |
| --- | --- |
| [按需 HDInsight 群集](#on-demand-azure-hdinsight-cluster)或[自己的 HDInsight 群集](#existing-azure-hdinsight-cluster) |[.NET 自定义活动](#net-custom-activity)，[蜂巢活动](#hdinsight-hive-activity)，[猪活动](#hdinsight-pig-activity)，[地图减少活动](#hdinsight-mapreduce-activity)， Hadoop 流活动，[火花活动](#hdinsight-spark-activity) |
...
| [Azure SQL 数据库](#azure-sql-database) [、Azure SQL 数据仓库](#azure-sql-data-warehouse) [、SQL 服务器](#sql-server-stored-procedure) |[存储过程](#stored-procedure-activity) |

## <a name="on-demand-azure-hdinsight-cluster"></a>按需 Azure HDInsight 群集
Azure 数据工厂服务可自动创建基于 Windows/Linux 的按需 HDInsight 群集，以处理数据。 群集创建在与该群集相关联的存储帐户（JSON 中的 linkedServiceName 属性）所在的同一区域中。 您可以在此链接的服务上运行以下转换活动： [.NET 自定义活动](#net-custom-activity)、 [Hive 活动](#hdinsight-hive-activity)、[猪活动](#hdinsight-pig-activity)、 [MapReduce 活动](#hdinsight-mapreduce-activity)、 Hadoop 流活动 、 [Spark 活动](#hdinsight-spark-activity).
...
可以创建 Azure HDInsight 链接服务，以向数据工厂注册自己的 HDInsight 群集。 您可以在此链接的服务上运行以下数据转换活动： [.NET 自定义活动](#net-custom-activity)、 [Hive 活动](#hdinsight-hive-activity)、[猪活动](#hdinsight-pig-activity)、 [MapReduce 活动](#hdinsight-mapreduce-activity)、 Hadoop 流活动、 [Spark 活动](#hdinsight-spark-activity)。
...
| sessionID |OAuth 授权会话中的会话 ID。 每个会话 ID 是唯一的，只能使用一次。 使用数据工厂编辑器时，会自动生成此 ID。 |是 |
...
## <a name="sql-server-stored-procedure"></a>SQL 服务器存储过程
...
活动 | 描述
-------- | -----------
[HDInsight 蜂巢活动](#hdinsight-hive-activity) | 数据工厂管道中的 HDInsight Hive 活动会在自己的或基于 Windows/Linux 的按需 HDInsight 群集上执行 Hive 查询。
...
- **rootPath**设置为**adfspark\\pyFiles，** 其中 adfspark 是 Azure Blob 容器，pyFiles 是该容器中的精细文件夹。 在此示例中，Azure Blob 存储是与 Spark 群集关联的存储。 可将文件上传到不同的 Azure 存储。 如果这样做，请创建 Azure 存储链接服务，将该存储帐户链接到数据工厂。 然后，将该链接的服务的名称指定为 **sparkJobLinkedService** 属性的值。 请参阅“Spark 活动属性”，了解有关 Spark 活动支持的此属性及其他属性的详细信息。
...
您可以在 Azure 机器学习工作室批处理执行活动 JSON 定义中指定以下属性。 活动的类型属性必须是：**AzureMLBatchExecution**。 必须首先创建 Azure 机器学习链接服务，并将该服务的名称指定为**链接的 ServiceName**属性的值。 将活动类型设置为 AzureMLBatchExecution 时，**typeProperties** 节支持以下属性：
...
您可以在 Azure 机器学习工作室更新资源活动 JSON 定义中指定以下属性。 活动的类型属性必须是：**AzureMLUpdateResource**。 必须首先创建 Azure 机器学习链接服务，并将该服务的名称指定为**链接的 ServiceName**属性的值。 将活动类型设置为 AzureMLUpdateResource 时，**typeProperties** 节支持以下属性：
...
输出数据集为存储过程活动指定**计划**（每小时、每周、每月等）。 输出数据集必须使用**链接服务**，其指代 Azure SQL 数据库或 Azure SQL 数据仓库或要在其中运行存储过程的 SQL Server 数据库。 输出数据集可以作为传递存储过程的结果的方法，以便由管道中的另一个活动（[链接活动](data-factory-scheduling-and-execution.md#multiple-activities-in-a-pipeline)）进行后续处理。 但是，数据工厂不会自动将存储过程的输出写入此数据集。 它是写入输出数据集指向的 SQL 表的存储过程。 在某些情况下，输出数据集可以是**虚拟数据集**，它仅用于指定运行存储过程活动的计划。
