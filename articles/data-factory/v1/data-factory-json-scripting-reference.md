...
| anchorDateTime |定义计划程序用于计算数据集切片边界的时间中的绝对位置。 <br/><br/><b>注意</b>：如果 AnchorDateTime 的日期部分比频率更精细，则忽略更精细的部分。 <br/><br/>例如，如果<b>间隔</b>为<b>每小时</b>（frequency： hour，interval：1），而<b>AnchorDateTime</b>包含<b>分钟和秒</b>，则将忽略 AnchorDateTime 的<b>分钟和秒</b>部分。 |否 |01/01/0001 |
...
| 授权 | 单击“数据工厂编辑器”**** 中的“授权”**** 按钮，并输入会自动生成的授权 URL 分配给此属性的凭据。 | 是（对于用户凭据身份验证）|
...
| 授权 |在数据工厂编辑器中单击“授权”**** 按钮并完成 OAuth 登录后，会自动检索授权代码。 |是 |
...
输出数据集为存储过程活动指定**计划**（每小时、每周、每月等）。 输出数据集必须使用**链接服务**，其指代 Azure SQL 数据库或 Azure SQL 数据仓库或要在其中运行存储过程的 SQL Server 数据库。 输出数据集可以作为一种方法，用于传递存储过程的结果，以供管道中的其他活动（[链接活动](data-factory-scheduling-and-execution.md#multiple-activities-in-a-pipeline)）进行后续处理。 但是，数据工厂不会自动将存储过程的输出写入此数据集。 它是写入输出数据集指向的 SQL 表的存储过程。 在某些情况下，输出数据集可以是**虚拟数据集**，它仅用于指定运行存储过程活动的计划。
