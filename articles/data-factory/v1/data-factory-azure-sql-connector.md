...
此示例每小时将时间序列数据（每小时、每天等）从 Azure SQL 数据库中的表复制到 blob。 对于这些示例中使用的 JSON 属性，在示例后的部分对其进行描述。
...
示例每隔一小时将时序数据（每小时、每天等）复制到 Azure SQL 数据库中的表。 对于这些示例中使用的 JSON 属性，在示例后的部分对其进行描述。
