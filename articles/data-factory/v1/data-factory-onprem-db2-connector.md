...
使用复制向导时，会自动创建数据工厂链接服务、数据集和管道实体的 JSON 定义。 使用工具或 API（.NET API 除外）时，可使用 JSON 格式定义数据工厂实体。 JSON 示例：将数据从 DB2 复制到 Azure Blob 存储显示用于从本地 DB2 数据存储复制数据的数据工厂实体的 JSON 定义。
