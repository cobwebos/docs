...
1.  导入用于 ETL 的笔记本。 将下面的转换笔记本导入 Databricks 工作区。 （不必与下面的位置相同，但请记住稍后选择的路径。）在 URL 字段中输入以下 url，从以下 URL 导入笔记本： `https://adflabstaging1.blob.core.windows.net/share/Transformations.html`。 选择“导入”。
