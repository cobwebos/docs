...
本文概述了如何使用 Azure 数据工厂中的复制活动从 Azure 数据存储第 2 代复制数据，以及使用数据流转换 Azure 数据湖存储 Gen2 中的数据。 若要了解 Azure 数据工厂，请阅读[介绍性文章](introduction.md)。
...
- 从 Azure 数据存储第 1 代/Gen2 复制时[保留 ACL。](#preserve-acls)
...
2. 向服务主体授予适当的权限。 查看有关权限在"数据存储 Gen2"中如何工作的示例，这些操作来自[文件和目录上的访问控制列表](../storage/blobs/data-lake-storage-access-control.md#access-control-lists-on-files-and-directories)

    - **作为源**：在存储资源管理器中，至少授予所有上游文件夹和文件系统**的执行**权限，以及文件复制的**读取**权限。 或者，在访问控制 (IAM) 中至少授予“存储 Blob 数据读取者”角色****。
    - **作为接收器**：在存储资源管理器中，至少授予所有上游文件夹和文件系统**的执行**权限，以及接收器文件夹的**写入**权限。 或者，在访问控制 (IAM) 中至少授予“存储 Blob 数据参与者”角色****。

>[!NOTE]
>如果使用数据工厂 UI 创作服务主体，并且服务主体未在 IAM 中设置"存储 Blob 数据读取器/参与者"角色，则在执行测试连接或浏览/导航文件夹时，请选择"测试连接到文件路径"或"从指定路径浏览"，并指定具有 **"读取 + 执行**"权限的路径以继续。
...
>[!NOTE]
>如果使用数据工厂 UI 创作，并且托管标识未在 IAM 中使用"存储 Blob 数据读取器/参与者"角色设置，则在执行测试连接或浏览/导航文件夹时，请选择"测试连接到文件路径"或"从指定路径浏览"，并指定具有 **"读取 + 执行**"权限的路径以继续。
...
| Property                 | 说明                                                  | 必选                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | 下`storeSettings`的类型属性必须设置为**AzureBlobFSReadSettings**。 | 是                                           |
...
| Property                 | 说明                                                  | 必选 |
| ------------------------ | ------------------------------------------------------------ | -------- |
| type                     | 下`storeSettings`的类型属性必须设置为**AzureBlobFSWrite设置**。 | 是      |
...
| 块大小因MB | 指定用于将数据写入 ADLS Gen2 的 MB 块大小。 [了解有关块 Blob 的更多](https://docs.microsoft.com/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs#about-block-blobs)。 <br/>允许的值**介于 4 到 100 MB 之间**。 <br/>默认情况下，ADF 会根据您的源存储类型和数据自动确定块大小。 对于非二进制复制到 ADLS Gen2，默认块大小为 100 MB，以便最多适合 4.95 TB 数据。 当数据不大时，它可能是最佳的，尤其是在使用自托管集成运行时时，网络不佳导致操作超时或性能问题。 您可以显式指定块大小，同时确保块SizeInMB_50000足够大以存储数据，否则复制活动运行将失败。 | 否 |
...
## <a name="preserve-acls-from-data-lake-storage-gen1gen2"></a><a name="preserve-acls"></a>保留数据湖存储第 1 代/第 2 代中的 ACL

将文件从 Azure 数据存储库存储第 1 代/第 2 代复制到 Gen2 时，可以选择保留 POSIX 访问控制列表 （ACL） 以及数据。 从保留[ACL 从数据存储库第 1 代/第 2 代 到第 2 代](copy-activity-preserve-metadata.md#preserve-acls)。
...
在映射数据流中转换数据时，可以从 Azure 数据存储 Gen2 中读取和写入 JSON、Avro、分隔文本或 Parquet 格式的文件。 有关详细信息，请参阅映射数据流功能中的[源转换](data-flow-source.md)和[接收器转换](data-flow-sink.md)。

### <a name="source-transformation"></a>源转换

在源转换中，可以从 Azure 数据湖存储 Gen2 中的容器、文件夹或单个文件进行读取。 "**源"选项**选项卡允许您管理文件读取的方式。 
...
在接收器转换中，可以写入 Azure 数据湖存储 Gen2 中的容器或文件夹。 通过 **"设置"** 选项卡，您可以管理文件的写入方式。
...
| fileName | 指定“folderPath”下的文件的“名称或通配符筛选器”。 如果没有为此属性指定任何值，则数据集会指向文件夹中的所有文件。 <br/><br/>对于筛选器，允许的通配符为：`*`（匹配零个或更多字符）和 `?`（匹配零个或单个字符）。<br/>- 示例 1：`"fileName": "*.csv"`<br/>- 示例 2：`"fileName": "???20180427.txt"`<br/>如果实际文件名内具有通配符或此转义符，请使用 `^` 进行转义。<br/><br/>当未为输出数据集指定文件名并在活动接收器中未指定**保留层次结构**时，复制活动将自动生成具有以下模式的文件名："*数据"。活动运行 ID GUID]。[GUID 如果扁平层次结构]。[格式（如果已配置）。"[压缩如果配置]*"，例如，"数据.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt.gz"。 如果使用表名称而不是查询从表格源进行复制，则名称模式为“[table name].[format].[compression if configured]”，例如“MyTable.csv”。** |否 |
...
| recursive | 指示是要从子文件夹中以递归方式读取数据，还是只从指定的文件夹中读取数据。 当 recursive 设置为 true 且接收器是基于文件的存储时，将不会在接收器上复制或创建空的文件夹或子文件夹。<br/>允许的值**为 true（** 默认值）和**false**。 | 否 |
| maxConcurrentConnections | 可以同时连接到数据存储的连接数。 仅在要限制与数据存储的并发连接时指定。 | 否 |
