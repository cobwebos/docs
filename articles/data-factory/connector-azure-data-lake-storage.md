...
>[!NOTE]
>在使用 PolyBase 将数据加载到 SQL 数据仓库中，如果您的源数据湖存储第 2 代配置与虚拟网络终结点，必须使用托管的标识身份验证所需的 PolyBase。 请参阅[托管的标识身份验证](#managed-identity)详细的配置先决条件部分。
...
2. 向服务主体授予适当的权限。 了解有关权限中的数据湖存储第 2 代中的工作方式[对文件和目录访问控制列表](../storage/blobs/data-lake-storage-access-control.md#access-control-lists-on-files-and-directories)

    - **作为源**：在存储资源管理器，请至少授予**Execute**权限以及源文件系统时，从开始**读取**要复制的文件的权限。 或者，在访问控制 (IAM)，授予至少**存储 Blob 数据读取器**角色。
    - **作为接收器**：在存储资源管理器，请至少授予**Execute**权限接收器文件系统中，从开始连同**编写**接收器文件夹的权限。 或者，在访问控制 (IAM)，授予至少**存储 Blob 数据参与者**角色。

>[!NOTE]
>向列表名称开头的文件夹从帐户级别，或若要测试连接，您需要设置的权限被授予对服务主体**存储帐户的"存储 Blob 数据读取者"权限在 IAM**。 对于以下情况需要这样做：
>- **复制数据工具**创作复制管道。
>- 在创作期间使用**数据工厂 UI** 测试连接和浏览文件夹。 
>如果你担心在帐户级别授权，在创作时，跳过测试连接，并授予权限的父路径然后选择要浏览从指定路径的输入。 只要通过复制文件的适当权限授予服务主体，将复制活动的工作原理。
...
2. 向托管标识授予适当的权限。 了解权限中的数据湖存储第 2 代中的工作原理的详细信息[对文件和目录访问控制列表](../storage/blobs/data-lake-storage-access-control.md#access-control-lists-on-files-and-directories)。
...
>[!IMPORTANT]
>如果使用 PolyBase 将数据载入数据湖存储第 2 代从 SQL 数据仓库对数据湖存储第 2 代中使用托管的标识身份验证时，请确保还遵循步骤 1 和 2 中的[本指南](../sql-database/sql-database-vnet-service-endpoint-rule-overview.md#impact-of-using-vnet-service-endpoints-with-azure-storage)为 1) 注册 SQL使用 Azure Active Directory (Azure AD) 的数据库服务器和 2） 将存储 Blob 数据参与者角色分配给你的 SQL 数据库服务器;数据工厂处理其余部分。 如果你的数据湖存储第 2 代配置与 Azure 虚拟网络终结点，若要使用 PolyBase 将数据加载，您必须使用托管的标识身份验证所需的 PolyBase。
