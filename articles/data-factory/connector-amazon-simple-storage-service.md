...
- **对于复制活动执行：**： `s3:GetObject`和`s3:GetObjectVersion` ，用于 Amazon S3 对象操作。
- **对于数据工厂 GUI 创作**： `s3:ListAllMyBuckets` `s3:ListBucket` / `s3:GetBucketLocation`对于 "测试连接" 和 "浏览/导航文件路径" 等操作，另外还需要用于 Amazon S3 存储桶操作的权限。 如果不想授予这些权限，请跳过“链接服务创建”页面中的测试连接，并直接在“数据集设置”中指定路径。
...
| 属性                 | 说明                                                  | 必需                                                    |
| ------------------------ | ------------------------------------------------------------ | ----------------------------------------------------------- |
| type                     | 下`storeSettings`的 type 属性必须设置为**AmazonS3ReadSettings**。 | 是                                                         |
