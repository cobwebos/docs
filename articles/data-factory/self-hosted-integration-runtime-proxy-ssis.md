...
description: 了解如何将自承载 Integration Runtime 配置为 Azure-SSIS Integration Runtime 的代理。
...
ms.locfileid: "68737558"
---
# <a name="configure-self-hosted-ir-as-a-proxy-for-azure-ssis-ir-in-adf"></a>在 ADF 中将自承载 IR 配置为 Azure-SSIS IR 的代理
本文介绍如何在 azure 数据工厂 (ADF) 中使用配置为代理的自承载 IR 在 Integration Runtime Azure 上运行 SQL Server Integration Services (SSIS) 包 (IR)。  此功能允许你在本地访问数据而无需将[AZURE SSIS IR 加入虚拟网络](https://docs.microsoft.com/azure/data-factory/join-azure-ssis-integration-runtime-virtual-network)。  当你的公司网络的配置/限制策略过于复杂时, 可以将 Azure SSIS IR 插入其中。

此功能会将包含包含本地数据源的数据流任务的包拆分为两个过渡任务: 第一个在自承载 IR 上运行的第一个任务是将数据从本地数据源移到 Azure Blob 存储中的临时区域, 同时然后, 在 Azure SSIS IR 上运行的第二个将数据从临时区域移到预期的数据目标。

此功能还提供其他优点/功能, 因为它允许你在 Azure 不支持的区域中预配自承载 IR, 同时允许你的数据源防火墙上的自承载 IR 的公共静态 IP 地址, 等等。

## <a name="prepare-self-hosted-ir"></a>准备自承载 IR
若要使用此功能, 首先需要创建一个 ADF 并在其下预配 Azure SSIS IR (如果尚未这样做, 请遵循[如何预配 AZURE SSIS ir](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure)一文)。

接下来, 需要在使用[如何创建自承载 ir](https://docs.microsoft.com/azure/data-factory/create-self-hosted-integration-runtime)一文的同一 ADF 中预配自己的自承载 ir。
...
## <a name="configure-azure-ssis-ir-with-self-hosted-ir-as-a-proxy"></a>使用自承载 IR 将 Azure SSIS IR 配置为代理
准备好用于过渡的自承载 IR 和 Azure Blob 存储链接服务之后, 现在可以使用自承载 IR 将新的/现有的 Azure SSIS IR 配置为 ADF 门户/应用上的代理。  如果现有的 Azure SSIS IR 正在运行, 请在执行此操作之前将其停止, 然后重新启动。

在 "**高级设置**" 页上, 请检查 "**设置自承载 Integration Runtime 作为 Azure-SSIS Integration Runtime 的代理**" 复选框, 选择要进行暂存的自承载 IR 和 Azure Blob 存储链接服务, 并指定 Blob**暂存路径**的容器名称 (如果需要)。

![使用自承载 IR 将 Azure SSIS IR 配置为代理](media/self-hosted-integration-runtime-proxy-ssis/shir-advanced-settings-ssisir.png)
...
在 Azure SSIS IR 上运行的第二个过渡任务不会单独计费, 但运行的 Azure SSIS IR 将按[AZURE SSIS ir 定价](https://azure.microsoft.com/pricing/details/data-factory/ssis/)一文中的规定进行计费。

## <a name="current-limitations"></a>当前限制
- 目前仅支持 OLEDB/平面文件连接管理器和 OLEDB/平面文件源。
- 目前仅支持用**帐户密钥**/**SAS URI**/**服务主体**身份验证配置的 Azure Blob 存储链接服务。
- 目前支持在预配 Azure SSIS IR 的同一 ADF 下预配自承载 IR。
