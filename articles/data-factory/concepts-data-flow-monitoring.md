...
数据流![监视]数据流(media/data-flow/mon001.png "监视")

你将在此级别上看到统计信息，其中包括运行时间和状态。 活动级别的运行 ID 不同于管道级别的运行 ID。 上一级别的运行 ID 适用于管道。 单击眼镜图标可以查看有关数据流执行的深入详细信息。

数据流![监视]数据流(media/data-flow/mon002.png "监视")

在图形节点监视视图中，将会看到数据流图形的简化只读版本。

数据流![监视]数据流(media/data-flow/mon003.png "监视")
...
数据流![监视]数据流(media/data-flow/mon004.png "监视")
