...
| **serviceBusRuleId** |String | 要在其中为流式诊断日志创建事件中心的服务总线命名空间的服务总线规则 ID。 规则 ID 的格式 `{service bus resource ID}/authorizationrules/{key name}`。|
...
| **timeGrain** | String | 以 ISO 8601 持续时间格式捕获的指标粒度。 属性值必须为 `PT1M`，该值指定一分钟。 |
| **enabled**| Boolean | 指定是否为此资源启用指标或日志类别的集合。 |
...
| --- | --- | --- | --- |
| **Level** |String | 诊断日志的级别。 对于 "活动-运行日志"，将属性值设置为4。 | `4` |
| **correlationId** |String | 用于跟踪特定请求的唯一 ID。 | `319dc6b4-f348-405e-b8d7-aafc77b73e77` |
| **time** | String | 以 timespan UTC 格式 `YYYY-MM-DDTHH:MM:SS.00000Z`事件的时间。 | `2017-06-28T21:00:27.3534352Z` |
...
|**category**| String | 诊断日志的类别。 将此属性 (Property) 的值设置为 `ActivityRuns`。 | `ActivityRuns` |
|**level**| String | 诊断日志的级别。 将此属性 (Property) 的值设置为 `Informational`。 | `Informational` |
|**operationName**| String | 活动的名称及其状态。 如果活动为启动检测信号，则属性值为 `MyActivity -`。 如果活动为最终检测信号，则属性值为 `MyActivity - Succeeded`。 | `MyActivity - Succeeded` |
|**pipelineName**| String | 管道的名称。 | `MyPipeline` |
|**activityName**| String | 活动的名称。 | `MyActivity` |
|**start**| String | 活动的开始时间以 timespan UTC 格式运行。 | `2017-06-26T20:55:29.5007959Z`|
|**end**| String | 活动的结束时间以 timespan UTC 格式运行。 如果诊断日志显示某个活动已经开始但尚未结束，则该属性值为 `1601-01-01T00:00:00Z`。 | `2017-06-26T20:55:29.5007959Z` |
...
|**resourceId**| String | 与数据工厂资源关联的 ID。 | `/SUBSCRIPTIONS/<subID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |
|**category**| String | 诊断日志的类别。 将此属性 (Property) 的值设置为 `PipelineRuns`。 | `PipelineRuns` |
|**level**| String | 诊断日志的级别。 将此属性 (Property) 的值设置为 `Informational`。 | `Informational` |
|**operationName**| String | 管道的名称及其状态。 管道运行完成后，属性值 `Pipeline - Succeeded`。 | `MyPipeline - Succeeded`。 |
|**pipelineName**| String | 管道的名称。 | `MyPipeline` |
|**start**| String | 活动的开始时间以 timespan UTC 格式运行。 | `2017-06-26T20:55:29.5007959Z`。 |
|**end**| String | 活动的结束时间以 timespan UTC 格式运行。 如果诊断日志显示某个活动已经开始但尚未结束，则该属性值为 `1601-01-01T00:00:00Z`。  | `2017-06-26T20:55:29.5007959Z` |
|**status**| String | 管道运行的最终状态。 可能的属性值 `Succeeded` 和 `Failed`。 | `Succeeded`|
...
|**category**| String | 诊断日志的类别。 将此属性 (Property) 的值设置为 `PipelineRuns`。 | `PipelineRuns` |
|**level**| String | 诊断日志的级别。 将此属性 (Property) 的值设置为 `Informational`。 | `Informational` |
|**operationName**| String | 触发器的名称，其最终状态指示是否已成功激发触发器。 如果检测信号成功，则属性值为 `MyTrigger - Succeeded`。 | `MyTrigger - Succeeded` |
|**triggerName**| String | 触发器的名称。 | `MyTrigger` |
|**triggerType**| String | 触发器的类型。 可能的属性值 `Manual Trigger` 和 `Schedule Trigger`。 | `ScheduleTrigger` |
|**triggerEvent**| String | 触发器的事件。 | `ScheduleTime - 2017-07-06T01:50:25Z` |
|**start**| String | 以 timespan UTC 格式触发触发器的开始时间。 | `2017-06-26T20:55:29.5007959Z`|
|**status**| String | 显示触发器是否成功激发的最终状态。 可能的属性值 `Succeeded` 和 `Failed`。 | `Succeeded`|
...
|----------------------|---------------------------------|----------|----------------------|-------------------------------------------------------|
| PipelineSucceededRuns | 成功的管道运行数指标 | 计数    | 总计                | 在一分钟时间范围内成功的管道运行总数。 |
| PipelineFailedRuns   | 失败的管道运行数指标    | 计数    | 总计                | 分钟窗口中失败的管道运行总数。    |
| ActivitySucceededRuns | 成功的活动运行数指标 | 计数    | 总计                | 在一分钟时间范围内成功的活动运行总数。  |
| ActivityFailedRuns   | 失败的活动运行数指标    | 计数    | 总计                | 分钟窗口中失败的活动运行总数。     |
| TriggerSucceededRuns | 成功的触发器运行数指标  | 计数    | 总计                | 在一分钟时间范围内成功的触发器运行总数。   |
| TriggerFailedRuns    | 失败的触发器运行数指标     | 计数    | 总计                | 分钟窗口内失败的触发器运行总数。      |

若要访问指标，请完成[Azure Monitor 数据平台](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-metrics)中的说明。

> [!NOTE]
> 只发出已完成的、触发的活动和管道运行事件。 正在进行，并且**不**发出沙盒/调试运行。 
