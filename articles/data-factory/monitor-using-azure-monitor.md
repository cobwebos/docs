...
有关更多详细信息，请参阅 [Azure Monitor 概述](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-azure-monitor)。
...
1. 为设置指定名称，选择“发送到 Log Analytics”，然后从 **Log Analytics 工作区**中选择一个工作区。
    ...
    * 在_特定于资源_的模式中，来自 Azure 数据工厂的诊断日志将流入_ADFActivityRun_、 _ADFPipelineRun_、 _ADFTriggerRun_、 _ADFSSISIntegrationRuntimeLogs_、 _ADFSSISPackageEventMessageContext_、 _ADFSSISPackageEventMessages_、 _ADFSSISPackageExecutableStatistics_、 _ADFSSISPackageExecutionComponentPhases_和_ADFSSISPackageExecutionDataStatistics_表中。

      你可以选择与工作负荷相关的各种日志以发送到 Log Analytics 表。 例如，如果根本不使用 SQL Server Integration Services （SSIS），则无需选择任何 SSIS 日志。 如果要记录 SSIS Integration Runtime （IR）启动/停止/维护操作，则可以选择 "SSIS IR 日志"。 如果仅通过 T-sql 调用 SSIS 包执行，则只能选择 "SSIS 包日志"。 如果通过 ADF 管道中的 "执行 SSIS 包" 活动调用 SSIS 包执行，则可以选择 "所有日志"。

    * 如果选择 " _AllMetrics_"，则将提供用于监视/引发警报的 ADF 实体计数/大小、活动/管道/触发器运行、INTEGRATION RUNTIME （IR） CPU 使用率/内存/节点计数/队列，以及用于执行 ssis 包执行和 ssis IR 启动/停止操作的指标。
...
安装 Azure 数据工厂分析将在所选 "Log Analytics" 工作区的 "工作簿" 部分中创建一组默认视图。 这将导致启用以下度量值：
...
| SSISPackageExecutionCanceled         | 已取消 SSIS 包执行指标  | 计数    | 总计                | 在一分钟时间范围内取消的 SSIS 包执行总数。 |
...
> [!NOTE]
> 仅发出已完成、已触发的活动和管道运行事件。 **不**会发出正在进行的运行和沙盒/调试运行。 另一方面，将发出来自 SSIS 包执行的所有事件，包括已完成的事件、正在进行的事件，并通过 SSMS/SQL Server 代理/其他指定的工具上的 T-sql 调用，或在 ADF 管道中执行 SSIS 包活动的触发/沙盒/调试运行。
...
这些是 SSIS Integration Runtime （IR）启动/停止/维护操作的日志属性/属性。
...
| Property                   | 类型   | 描述                                                   | 示例                        |
| -------------------------- | ------ | ------------------------------------------------------------- | ------------------------------ |
| **time**                   | String | UTC 格式的事件时间：`YYYY-MM-DDTHH:MM:SS.00000Z` | `2017-06-28T21:00:27.3534352Z` |
...
| **level**                  | String | 诊断日志级别                                  | `Informational` |
...
这些是与 SSIS IR 上的 SSIS 包执行生成的事件消息相关的条件的日志属性/属性。 它们将与显示多个 SSIS 包属性的运行时值的[ssis 目录（SSISDB）事件消息上下文表/视图](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-message-context?view=sql-server-ver15)传达类似的信息。 它们是在选择 `Basic/Verbose` 日志记录级别时生成的，适用于调试/符合性检查。
...
| -------------------------- | ------ | -------------------------------------------------------------------- | ------------------------------ |
| **time**                   | String | UTC 格式的事件时间：`YYYY-MM-DDTHH:MM:SS.00000Z`        | `2017-06-28T21:00:27.3534352Z` |
| **operationName**          | String | 此元素设置为 。`YourSSISIRName-SSISPackageEventMessageContext`       | `mysqlmissisir-SSISPackageEventMessageContext` |
...
| **integrationRuntimeName** | String | SSIS IR 的名称                                             | `MySSISIR` |
| **level**                  | String | 诊断日志级别                                         | `Informational` |
| **operationId**            | String | 用于在 SSISDB 中跟踪特定操作的唯一 ID          | `1`（1表示与 SSISDB 中未存储的包相关的操作） |
| **contextDepth**           | String | 事件消息上下文的深度                              | `0`（0表示在包执行开始之前的上下文，1表示发生错误时的上下文，并随着上下文从错误中的发生进一步增加而增加） |
| **packagePath**            | String | 作为事件消息上下文源的包对象的路径      | `\Package` |
| **contextType**            | String | 作为事件消息上下文源的包对象类型      | `60`（请参阅[更多上下文类型](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-message-context?view=sql-server-ver15#remarks)） |
...
这些是 ssis IR 上的 SSIS 包执行生成的事件消息的日志属性/属性。 它们将类似信息作为[SSISDB 事件消息表/视图](https://docs.microsoft.com/sql/integration-services/system-views/catalog-event-messages?view=sql-server-ver15)进行传递，此表/视图显示事件消息的详细文本/元数据。 它们是在任何日志记录级别生成的，但除外 `None` 。
...
| **messageType**            | String | 事件消息的类型                                     | `70`（请参阅[更多消息类型](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks)） |
| **messageSourceType**      | String | 事件消息源的类型                              | `20`（请参阅[更多消息源类型](https://docs.microsoft.com/sql/integration-services/system-views/catalog-operation-messages-ssisdb-database?view=sql-server-ver15#remarks)） |
| **message**                | String | 事件消息的文本                                     | `MyPackage:Validation has started.` |
| **packageName**            | String | 已执行的包文件的名称                             | `MyPackage.dtsx` |
| **名**              | String | 相关运行时事件的名称                                 | `OnPreValidate` |
...
这些是 ssis IR 上的 SSIS 包执行生成的可执行统计信息的日志属性/属性，其中可执行文件是包控制流中的容器/任务。 它们将类似信息作为[SSISDB 可执行统计表/视图](https://docs.microsoft.com/sql/integration-services/system-views/catalog-executable-statistics?view=sql-server-ver15)进行传递，此表/视图为每个正在运行的可执行文件（包括迭代）显示一行。 它们是在任何日志记录级别生成 `None` 的，除了用于识别任务级瓶颈/故障。
...
| **integrationRuntimeName** | String | SSIS IR 的名称                                         | `MySSISIR` |
| **level**                  | String | 诊断日志级别                                     | `Informational` |
| **executionId**            | String | 用于在 SSISDB 中跟踪特定执行的唯一 ID      | `1`（1表示与 SSISDB 中未存储的包有关的执行） |
| **executionPath**          | String | 从父包到已执行组件的完整路径          | `\Transformation\Data Flow Task`（此路径还捕获组件迭代） |
...
| **executionResult**        | String | 运行可执行文件的结果                                 | `0`（0表示成功，1表示失败，2表示完成，3表示取消） |
| **executionValue**         | String | 通过运行可执行文件返回的用户定义值            | `1` |
| **resourceId**             | String | ADF 资源的唯一 ID                               | `/SUBSCRIPTIONS/<subscriptionID>/RESOURCEGROUPS/<resourceGroupName>/PROVIDERS/MICROSOFT.DATAFACTORY/FACTORIES/<dataFactoryName>` |

#### <a name="ssis-execution-component-phases-log-attributes"></a>SSIS 执行组件阶段日志属性

这些是由 SSIS IR 上的 SSIS 包执行生成的数据流组件的运行时统计信息的日志属性/属性。 它们将类似信息作为[SSISDB 执行组件阶段表/视图传达，此表/视图](https://docs.microsoft.com/sql/integration-services/system-views/catalog-execution-component-phases?view=sql-server-ver15)显示了所有执行阶段中数据流组件所用的时间。 它们是在选择 `Performance/Verbose` 日志记录级别时生成的，用于捕获数据流执行统计信息。
...
这些是数据在数据流管道的每个阶段（从上游到下游组件，由 SSIS IR 上的 SSIS 包执行生成）的日志属性/属性。 它们将类似信息作为[SSISDB 执行数据统计表/视图](https://docs.microsoft.com/sql/integration-services/system-views/catalog-execution-data-statistics?view=sql-server-ver15)传达，显示通过数据流任务移动的数据的行计数。 它们是在选择 `Verbose` 日志记录级别时生成的，用于计算数据流吞吐量。
...
若要提升 & SQL Server Integration Services （SSIS）工作负载，可以在支持以下[项的 Azure 数据工厂（ADF）中预配 SSIS Integration Runtime （IR）](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure) ：
...
预配后，可以[使用 Azure PowerShell 或 ADF 门户的**监视**中心来检查 SSIS IR 操作状态](https://docs.microsoft.com/azure/data-factory/monitor-integration-runtime#azure-ssis-integration-runtime)。 使用项目部署模型时，SSIS 包执行日志存储在 SSISDB 内部表/视图中，因此可以使用 SQL Server Management Studio （SSMS）之类的指定工具对其进行查询、分析和直观显示。 使用包部署模型时，可以将 SSIS 包执行日志作为 CSV 文件存储在文件系统/Azure 文件中，这些文件仍需要使用其他指定的工具进行分析和处理，然后才能对其进行查询、分析和直观显示。

现在，在[Azure Monitor](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform)集成的情况下，可以在 Azure 门户上查询、分析并直观显示从 ssis IR 操作和 ssis 包执行生成的所有指标和日志，同时也可以在这些指标和日志上发出警报。

### <a name="configure-diagnostic-settings-and-workspace-for-ssis-operations"></a>为 SSIS 操作配置诊断设置和工作区

若要将从 SSIS IR 操作和 SSIS 包执行生成的所有指标和日志发送到 Azure Monitor，请按照提供的分步说明为[ADF 配置诊断设置和工作区](https://docs.microsoft.com/azure/data-factory/monitor-using-azure-monitor#configure-diagnostic-settings-and-workspace)。

### <a name="ssis-operational-metrics"></a>SSIS 操作指标

SSIS 操作[度量](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform-metrics)值是性能计数器/数值，用于描述特定时间点的 ssis IR 启动/停止操作和 ssis 包执行的状态。 它们是[Azure Monitor 中 adf 指标](https://docs.microsoft.com/azure/data-factory/monitor-using-azure-monitor#data-factory-metrics)的一部分，包括 adf 实体计数/大小、活动/管道/触发器运行以及 IR CPU 使用率/内存/节点计数/队列的。
...
SSIS 操作[日志](https://docs.microsoft.com/azure/azure-monitor/platform/data-platform-logs)是由 ssis IR 操作和 ssis 包执行生成的事件，这些事件提供了有关已识别的任何问题的足够上下文/信息，并且对根本原因分析很有用。 

在 Azure Monitor 上为 ADF 配置诊断设置和工作区时，可以选择相关的 SSIS 操作日志，并将其发送到基于 Azure 数据资源管理器的 Log Analytics，在该日志中，可以[使用丰富的查询语言](https://docs.microsoft.com/azure/azure-monitor/log-query/log-query-overview)、 [Azure 仪表板上的演示文稿](https://docs.microsoft.com/azure/azure-monitor/learn/tutorial-app-dashboards)和[近乎实时的警报](https://docs.microsoft.com/azure/azure-monitor/platform/alerts-log)进行分析。
...
选择的 SSIS 包执行日志始终发送到 Log Analytics，而不考虑调用方法，例如，在启用了 Azure 的 SQL Server Data Tools （SSDT）上，通过 SSMS/SQL Server 代理/其他指定工具上的 T-sql，或在 ADF 管道中执行 SSIS 包活动的触发/沙盒/调试运行。

查询日志分析上的 SSIS 包执行日志时，可以使用 OperationId/ExecutionId/CorrelationId 属性联接它们。 对于与 SSISDB 中**未**存储的包相关的所有操作/执行，OperationId/ExecutionId 始终设置为1。

![查询 Log Analytics 上的 SSIS 包执行日志](media/data-factory-monitor-oms/log-analytics-query.png)
