...
> [!TIP]
> 了解如何复制活动将源数据映射到从接收器[复制活动中的架构映射](copy-activity-schema-and-type-mapping.md)。
...
>[!NOTE]
>数据工厂引入了新的 Parquet 格式对，请参阅[Parquet 格式](format-parquet.md)文章的详细信息。 基于文件的数据存储区数据集上的以下配置仍支持作为-用于向后 compabitility。 建议以使用新的模型继续。
