...
7. 通过选择递**归文件和****二进制复制**选项来指定复制行为。 选择“**下一页**”。
...
要评估从 Azure 数据存储第 1 代升级到 Azure 数据存储湖存储第 2 代，请参阅[将大数据分析解决方案从 Azure 数据存储库第 1 代升级到 Azure 数据存储湖存储第 2 代](../storage/blobs/data-lake-storage-migrate-gen1-to-gen2.md)。 以下各节介绍使用数据工厂将数据升级到数据湖存储第 1 代的最佳做法。
