...
映射数据流可提供完全可视的体验，无需编码。 数据流在执行群集上运行，用于横向扩展的数据处理。 Azure 数据工厂处理所有代码转换、路径优化和数据流作业的执行。
...
此操作将带您到数据流画布，您可以在其中创建转换逻辑。 选择 **"添加源**"以开始配置源转换。 有关详细信息，请参阅[源转换](data-flow-source.md)。
...
当您开始在 ADF 中处理数据流时，您希望打开浏览器 UI 顶部的数据流的"调试"开关。 这将启动 Spark 群集，用于交互式调试、数据预览和管道调试执行。 您可以通过选择自定义[Azure 集成运行时](concepts-integration-runtime.md)来设置正在使用的群集的大小。 上次数据预览或上次调试管道执行后，调试会话最多保持活动状态长达 60 分钟。

使用数据流活动操作管道时，ADF 将使用与"运行打开"属性中[的活动](control-flow-execute-data-flow-activity.md)关联的 Azure 集成运行时。

默认的 Azure 集成运行时是一个小型的 4 核单辅助节点群集，允许您以最低的成本预览数据并快速执行调试管道。 如果要对大型数据集执行操作，则设置较大的 Azure IR 配置。

通过在 Azure IR 数据流属性中设置 TTL，可以指示 ADF 维护群集资源 （VM） 池。 此操作可加快后续活动的作业执行速度。
...
如果在管道中并行执行数据流，ADF 将根据附加到每个活动的 Azure 集成运行时中的设置为每个活动启动单独的 Spark 群集。 要在 ADF 管道中设计并行执行，请添加数据流活动，而 UI 中没有优先级约束。

在这三个选项中，此选项可能在最短的时间内执行。 但是，每个并行数据流同时在单独的群集上执行，因此事件的顺序是非确定性的。

如果要在管道内并行执行数据流活动，建议不要使用 TTL。 此操作是因为同时使用相同的 Azure 集成运行时并行执行数据流会导致数据工厂的多个暖池实例。

##### <a name="overload-single-data-flow"></a>过载单数据流

如果将所有逻辑放在单个数据流中，ADF 将同一作业执行上下文放在单个 Spark 群集实例上。

此选项可能更具挑战性，因为您的业务规则和业务逻辑可能会混杂在一起。 此选项也无法提供多少可重用性。

##### <a name="execute-data-flows-serially"></a>串行执行数据流

如果在管道中串行执行数据流活动，并且在 Azure IR 配置上设置了 TTL，则 ADF 重用计算资源 （VM），从而加快后续执行时间。 您仍然会为每个执行接收新的 Spark 上下文。

在这三个选项中，此操作可能需要最长的时间才能端到端执行。 但它确实提供了每个数据流步骤中逻辑操作的完全分离。
...
要更改任何转换上的分区，请选择 **"优化**"选项卡并选择"**设置分区**单选"按钮。 您会收到一系列分区选项。 最佳分区方法因数据卷、候选键、空值和基数而异。 
...
Azure 数据工厂生成列哈希以生成统一的分区，以便具有相似值的行属于同一分区。 使用哈希选项时，测试可能的分区偏斜。 可以设置物理分区数目。

##### <a name="dynamic-range"></a>动态范围

动态范围使用基于您提供的列或表达式的 Spark 动态范围。 可以设置物理分区数目。 

##### <a name="fixed-range"></a>固定范围

生成为分区数据列中的值提供固定范围的表达式。 为了避免分区偏斜，在使用此选项之前，应充分了解数据。 为表达式输入的值用作分区函数的一部分。 可以设置物理分区数目。

##### <a name="key"></a>密钥

如果您对数据基数有很好的了解，则密钥分区可能是一个不错的策略。 键分区为列中的每个唯一值创建分区。 无法设置分区数，因为该编号基于数据中的唯一值。

#### <a name="inspect"></a>检查

"**检查"** 选项卡提供要转换的数据流的元数据的视图。 您可以看到列计数、列更改、添加列、数据类型、列顺序和列引用。 **检查**是元数据的只读视图。 无需启用调试模式即可在 **"检查"** 窗格中查看元数据。
