...
如果您有活动的数据流调试会话，这些会话将显示在活动调试窗口的底部。 您可以选择活动数据流会话并停止相应的群集。

![活动数据流调试运行的示例列表](media/data-flow/dfsessions.png)

## <a name="monitoring-debug-runs"></a>监视调试运行

使用**调试**功能启动的测试运行在 **"监视器"** 选项卡的列表中不可用。您只能在 **"监视器"** 选项卡中看到使用 **"立即触发**"、**计划**或**翻滚窗口**触发器触发的运行。您可以在管道画布的 **"输出"** 窗口中看到使用**调试**功能启动的最后一次测试运行。
