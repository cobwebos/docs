...
ms.locfileid: "74929331"
---
# <a name="copy-data-from-hive-using-azure-data-factory"></a>使用 Azure 数据工厂从 Hive 复制数据 

本文概述了如何使用 Azure 数据工厂中的复制活动从 Hive 复制数据。 是基于总体介绍复制活动的[复制活动概述](copy-activity-overview.md)一文进行扩展的。
...
| serverType | Hive 服务器的类型。 <br/>允许的值为：**HiveServer1**、**HiveServer2**、**HiveThriftServer** | No |
...
| tableName | 包含架构部分的表的名称。 支持此属性是为了向后兼容。 对于新工作负荷，请使用 `schema` 和 `table`。 | 否（如果指定了活动源中的“query”） |
