...
| enableServerCertificateValidation | 指定在通过 SSL/TLS 通道使用 FTP 时是否启用服务器 TLS/SSL 证书验证。<br/>允许的值是：true****（默认）、false****。 | 否 |
...
| 属性                 | 说明                                                  | 必需                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| type                     | 下`storeSettings`的 type 属性必须设置为**FtpReadSettings**。 | 是                                           |
