...
您可以将数据从任何受支持的源数据存储复制到搜索索引中。 有关复制活动支持作为源/接收器的数据存储列表，请参阅[支持的数据存储](copy-activity-overview.md#supported-data-stores-and-formats)表。

## <a name="getting-started"></a>入门

[!INCLUDE [data-factory-v2-connector-get-started](../../includes/data-factory-v2-connector-get-started.md)]

以下各节提供有关用于定义特定于 Azure 认知搜索连接器的数据工厂实体的属性的详细信息。
...
> [!IMPORTANT]
> 将云数据存储中的数据复制到搜索索引时，在 Azure 认知搜索链接服务中，需要引用 Azure 集成运行时，其中具有 connactVia 中的显式区域。 将区域设置为搜索服务所在的区域。 从 [Azure Integration Runtime](concepts-integration-runtime.md#azure-integration-runtime) 了解更多信息。
...
有关可用于定义数据集的节和属性的完整列表，请参阅[数据集](concepts-datasets-linked-services.md)一文。 本节提供 Azure 认知搜索数据集支持的属性列表。
...
有关可用于定义活动的各部分和属性的完整列表，请参阅[管道](concepts-pipelines-activities.md)一文。 本节提供 Azure 认知搜索源支持的属性列表。

### <a name="azure-cognitive-search-as-sink"></a>Azure 认知搜索作为接收器
...
| writeBatchSize | 当缓冲区大小达到 writeBatchSize 时，将数据上载到搜索索引中。 有关详细信息，请参阅 [WriteBatchSize 属性](#writebatchsize-property)。<br/><br/>允许的值为：整数 1 到 1,000；默认值为 1000。 | 否 |

### <a name="writebehavior-property"></a>WriteBehavior 属性

AzureSearchSink 在写入数据时执行 upsert 操作。 换句话说，在编写文档时，如果搜索索引中已存在文档密钥，Azure 认知搜索将更新现有文档，而不是引发冲突异常。
...
Azure 认知搜索服务支持将文档编写为批处理。 每批次可包含 1 到 1,000 个操作。 每个操作处理一个文档以执行上传/合并操作。
...
当前不支持其他数据类型，例如复杂类型。 有关 Azure 认知搜索支持的数据类型的完整列表，请参阅[支持的数据类型（Azure 认知搜索）。](https://docs.microsoft.com/rest/api/searchservice/supported-data-types)
