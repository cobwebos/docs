---
title: 通过 Azure 数据工厂使用控制表从数据库进行增量复制
...
- *Data_Destination_Folder_Path* 或 *Data_Destination_Table_Name* 是目标存储中要将数据复制到的位置。
...
9. 选择“存储过程”。 对于“存储过程名称”，请选择“[update_watermark]”。 依次选择“导入参数”、“添加动态内容”。  
...
15. 可有可无如果选择 "SQL 数据仓库" 作为数据目标，则还必须提供到用于过渡的 Azure Blob 存储的连接，SQL 数据仓库 Polybase 需要此连接。 确保已在 Blob 存储中创建容器。
