...
| recursive | 表明从子文件夹中以递归方式删除数据，还是只从指定文件夹中删除数据。  | 不能。 默认为 `false`。 |
| maxConcurrentConnections | 用于删除文件夹或文件而同时连接到“存储”库的连接数。   |  不能。 默认为 `1`。 |
