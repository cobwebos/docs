...
| -------- | ------------------------------------------------------------ | -------- |
| 名称     | 源或接收器列的名称。                           | 是      |
| 序号  | 列索引。 从 1 开始。 <br>应用并使用分隔不带标头行的文本时所需。 | 否       |
| 路径     | 若要提取或映射的每个字段的 JSON 路径表达式。 适用于层次结构数据例如 MongoDB/REST。<br>对于根对象下的字段，JSON 路径以开头根 $;有关选择的数组中的字段，`collectionReference`从数组元素的属性，JSON 路径开始。 | 否       |
