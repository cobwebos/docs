...
> [!NOTE]
> Parquet/分隔文本格式**BlobSource**作为仍受支持类型下一节中所述的复制活动源的是向后兼容性。 建议以使用从长远看，此新模型和创作 UI 的 ADF 已切换为生成这些新类型。
...
> [!NOTE]
> Parquet/分隔文本格式**BlobSink**类型复制活动接收器中下一节提到仍支持作为-为是为了向后兼容。 建议以使用从长远看，此新模型和创作 UI 的 ADF 已切换为生成这些新类型。
