...
description: 了解如何使用数据工厂将数据复制到 Blob 存储，以及如何转换 Blob 存储中的数据。
ms.author: jingwang
author: linda33wj
manager: shwang
ms.reviewer: craigg
...
>[!IMPORTANT]
>如果你使用 PolyBase 将 Blob 中的数据加载到 SQL 数据仓库，则在使用 Blob 的托管标识身份验证时，请确保你还遵循[本指南](../sql-database/sql-database-vnet-service-endpoint-rule-overview.md#impact-of-using-vnet-service-endpoints-with-azure-storage)中的步骤1和步骤2，将 sql 数据库服务器注册到 Azure Active Directory （Azure AD），2）将 "存储 Blob 数据参与者" 角色分配到 sql 数据库服务器;其余的是由数据工厂处理的。 如果将 Blob 存储配置为使用 Azure 虚拟网络终结点，使用 PolyBase 加载数据时，必须使用 PolyBase 所需的托管标识身份验证。
...
| container  | Blob 容器。                                          | 是      |
| folderPath | 给定容器下的文件夹的路径。 如果要使用通配符筛选文件夹，请跳过此设置并在活动源设置中指定。 | No       |
| fileName   | 给定容器和 folderPath 下的文件名。 如果要使用通配符筛选文件，请跳过此设置并在活动源设置中指定。 | No       |
...
> [!NOTE]
> 对于 Parquet/分隔文本格式，在下一部分中提到的**BlobSource**类型复制活动源仍受支持，以实现向后兼容性。 建议你继续使用此新模型，并且 ADF 创作 UI 已切换为生成这些新类型。
