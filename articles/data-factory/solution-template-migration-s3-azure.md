...
该模板包含五个活动：
- **查找**将检索尚未从外部控制表复制到 Azure Data Lake Storage Gen2 的分区。 表名是*s3_partition_control_table*的，从表中加载数据的查询是 *"SELECT PARTITIONPREFIX From s3_partition_control_table WHERE SuccessOrFailure = 0"* 。
...
该模板包含两个参数：
- **AWS_S3_bucketName**是你要从中迁移数据的 AWS S3 上的 bucket 名称。 如果要迁移 AWS S3 上多个存储桶中的数据，则可以在外部控制表中另外添加一个列，以存储每个分区的存储桶名称，还可以更新管道以便相应地从该列中检索数据。
...
- **Lookup**从外部控制表检索上次复制作业运行时间，以便可以通过 LastModifiedTime 识别新文件或更新文件。 表名称是*s3_partition_delta_control_table*的，从表中加载数据的查询是 *"Select max （LastModifiedTime） From s3_partition_delta_control_table where PartitionPrefix = ' @ {JobRunTime} ' and SuccessOrFailure = 1"* 。
...
2. 在控制表的同一个 Azure SQL 数据库中创建一个存储过程。 
    ...
    > [!NOTE]
    > 存储过程的名称是 sp_update_partition_success。 它将由 ADF 管道中的 SqlServerStoredProcedure 活动调用。
...
2. 在控制表的同一个 Azure SQL 数据库中创建一个存储过程。 
    ...
    > [!NOTE]
    > 存储过程的名称是 sp_insert_partition_JobRunTime_success。 它将由 ADF 管道中的 SqlServerStoredProcedure 活动调用。
