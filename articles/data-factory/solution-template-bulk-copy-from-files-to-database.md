...
4. 你会看到所创建的管道，如以下示例中所示：
    ...
    > [!NOTE]
    > 如果在上述步骤 2 中选择了“Azure Synapse Analytics (以前称为 SQL DW)”作为数据目标，则需要按 SQL 数据仓库 Polybase 的要求，输入用于暂存的 Azure Blob 存储的连接。 如以下屏幕截图所示，模板会自动为 Blob 存储生成存储路径。 检查是否在管道运行后创建了容器。
