...
- **Azure 数据工厂**：如果没有，请按照步骤[创建数据工厂](quickstart-create-data-factory-portal.md#create-a-data-factory)。

- **具有目标类型"数据库表"的 SAP BW 开放集线器目标 （OHD）：** 要创建 OHD 或检查 OHD 是否正确配置用于数据工厂集成，请参阅本文的[SAP BW 开放集线器目标配置](#sap-bw-open-hub-destination-configurations)部分。
...
6. 如果需要，请指定筛选器。 如果你的 OHD 仅包含采用单个请求 ID 的单个数据传输进程 (DTP) 执行中的数据，或者你确定 DTP 已完成并想要复制数据，请清除“排除最后一个请求”复选框。****
   ...
   请在本文的 [SAP BW Open Hub 目标配置](#sap-bw-open-hub-destination-configurations)部分详细了解这些设置。 选择“验证”以仔细检查要返回的数据。**** 然后选择 **"下一步**"。
...
9. 在“选择输出文件或文件夹”页上，输入 **copyfromopenhub** 作为输出文件夹名称。**** 然后选择 **"下一步**"。
...
11. 在“设置”页上，展开“性能设置”。******** 为“复制并行度”输入一个值（例如 5），表示从 SAP BW 加载数据的并行度。**** 然后选择 **"下一步**"。

    ![配置复制设置](media/load-sap-bw-data/configure-copy-settings.png)

12. 在“摘要”**** 页上查看设置。 然后选择 **"下一步**"。
...
2. 在模板的主页上，选择或创建以下三个连接，然后选择窗口右下角的“使用此模板”。****
   ...
   - **Azure Blob 存储**：在本演练中，我们使用 Azure Blob 存储来存储高水印，这是*最大复制的请求 ID。*
   - **SAP BW 开放中心**：这是从中复制数据的源。 有关详细配置，请参考前面的完整复制演练。
   - **Azure 数据存储第 2 代**：这是将数据复制到的接收器。 有关详细配置，请参考前面的完整复制演练。

   ![从 SAP BW 模板执行增量复制](media/load-sap-bw-data/incremental-copy-from-sap-bw-template.png)

3. 此模板生成一个管道，其中涉及以下三个活动，并使它们在成功时链接：*查找*、*复制数据和* *Web*。

   转到管道**参数**选项卡。您将看到需要提供的所有配置。

   ![从 SAP BW 配置执行增量复制](media/load-sap-bw-data/incremental-copy-from-sap-bw-pipeline-config.png)

   - **SAPOpenHub 目标名称**：指定打开中心表名以从中复制数据。

   - **Data_Destination_Container**： 指定要将数据复制到的目标 Azure 数据存储湖存储 Gen2 容器。 如果该容器不存在，则数据工厂的复制活动将在执行期间创建一个容器。
  
   - **Data_Destination_Directory**： 指定 Azure 数据湖存储 Gen2 容器下的文件夹路径以将数据复制到 。 如果该路径不存在，则数据工厂的复制活动将在执行期间创建一个路径。
  
   - **高水印Blob容器**：指定容器以存储高水位线值。

   - **高水印BlobDirectory：** 指定容器下的文件夹路径以存储高水位线值。

   - **高水印Blob名称**：指定 blob 名称以存储高水印值，如`requestIdCache.txt`。 在 Blob 存储中，转到 HighWatermarkBlobContainer+HighWatermarkBlobDirectory+HighWatermarkBlobName 的相应路径，例如“container/path/requestIdCache.txt”**。 创建包含内容 0 的 Blob。

      ![Blob 内容](media/load-sap-bw-data/blob.png)

   - **逻辑AppURL：** 在此模板中，我们使用 WebActivity 调用 Azure 逻辑应用来设置 Blob 存储中的高水位线值。 或者，可以使用 Azure SQL 数据库来存储该值。 使用存储过程活动来更新该值。
...
1. 创建 Open Hub 目标。 可以在 SAP Transaction RSA1 中创建 OHD，这会自动创建所需的转换和数据传输过程。 使用以下设置：
   ...
   - **对象类型**：您可以使用任何对象类型。 此处我们使用 **InfoCube** 作为示例。
   - **目标类型**：选择**数据库表**。
   - **表的键**：选择**技术密钥**。
   - **提取**：选择 **"保留数据并将记录插入到表中**"。
...
- 在 OHD 中：将**提取**选项设置为**删除数据和插入记录**。 否则，在 BW 进程链中重复 DTP 时，会多次提取数据。
...
- 在数据工厂的 BW 打开集线器连接器中：关闭**排除上次请求**。 否则不会提取任何内容。
