...
1. 启动**记事本**并粘贴以下 HQL 脚本。 该 Hive 脚本会创建两个表：**WebLogsRaw** 和 **WebLogsPartitioned**。 在菜单上单击“文件”并选择“另存为”。 切换到硬盘上的 **C:\adfgetstarted** 文件夹。 为“保存类型”****字段选择**所有文件 (*.*)**。在“文件名”****中输入 **partitionweblogs.hql**。确认对话框底部的“编码”****字段设置为 **ANSI**。如果没有，则将其设置为 **ANSI**。  
