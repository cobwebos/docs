...
| clusterSshUserName         | SSH 的用户名远程连接到群集的节点（适用于 Linux）。 | 否       |
...
如果为这些属性指定了错误的值，则可能会收到以下**错误：** 未能创建群集。 异常: 无法完成群集创建操作。 操作失败，代码为 '400'。 群集保持为 'Error' 状态。 消息: “PreClusterCreationValidationFailure”。 收到此错误时，请确保使用的是**虚拟机的大小**一文中的表中的 [CMDLET 和 API](../virtual-machines/linux/sizes.md) 名称。        
...
| isEspEnabled      | 如果 HDInsight 群集启用了*企业安全性套餐*，请指定“true”[](https://docs.microsoft.com/azure/hdinsight/domain-joined/apache-domain-joined-architecture)。 默认值为“false”。 | 否       |
| connectVia        | 用于将活动分发到此链接服务的集成运行时。 可以使用 Azure 集成运行时或自托管集成运行时。 如果未指定，则使用默认 Azure 集成运行时。 <br />对于启用企业安全性套餐（ESP）的 HDInsight 群集，请使用自我托管的集成运行时，该运行时向群集提供直线，或者应将其部署在与 ESP HDInsight 群集相同的虚拟网络中。 | 否       |
