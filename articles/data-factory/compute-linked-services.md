...
| [Azure 机器学习工作室](#azure-machine-learning-studio-linked-service) | [机器学习活动：批处理执行和更新资源](transform-data-using-machine-learning.md) |
| [Azure 机器学习服务](#azure-machine-learning-service-linked-service) | [Azure 机器学习执行管道](transform-data-machine-learning-service.md) |
...
| [Azure Function](#azure-function-linked-service)         | [Azure Function 活动](control-flow-azure-function-activity.md)
>  
...
| clusterType                  | 要创建的 HDInsight 群集的类型。 允许的值是“hadoop”和“spark”。 如果未指定，默认值为 hadoop。 无法按需创建启用企业安全性套餐的群集，请改用[现有群集/自带计算](#azure-hdinsight-linked-service)。 | No       |
| 版本                      | HDInsight 群集的版本。 如果未指定较高的值，则使用当前 HDInsight 定义的默认版本。 | No       |
| hostSubscriptionId           | 用于创建 HDInsight 群集的 Azure 订阅 ID。 如果未指定，则使用 Azure 登录上下文的订阅 ID。 | No       |
| clusterNamePrefix           | HDI 群集名称的前缀，将自动在群集名称末尾追加时间戳| No       |
| sparkVersion                 | 群集类型为“Spark”时的 spark 版本 | No       |
| additionalLinkedServiceNames | 指定 HDInsight 链接服务的其他存储帐户，使数据工厂服务能够代为注册它们。 这些存储帐户必须与 HDInsight 群集位于同一区域中，该群集是在与 linkedServiceName 指定的存储帐户相同的区域中创建的。 | No       |
| osType                       | 操作系统的类型。 允许的值为：Linux 和 Windows（仅适用于 HDInsight 3.3）。 默认值为 Linux。 | No       |
| hcatalogLinkedServiceName    | 指向 HCatalog 数据库的 Azure SQL 链接服务的名称。 将 Azure SQL 数据库用作元存储以创建按需 HDInsight 群集。 | No       |
| connectVia                   | 用于将活动分发到此 HDInsight 链接服务的集成运行时。 对于按需的 HDInsight 链接服务，它仅支持 Azure 集成运行时。 如果未指定，则使用默认 Azure Integration Runtime。 | No       |
| clusterUserName                   | 用于访问群集的用户名。 | No       |
| clusterPassword                   | 用于访问群集的安全字符串类型密码。 | No       |
| clusterSshUserName         | SSH 的用户名远程连接到群集的节点（适用于 Linux）。 | No       |
| clusterSshPassword         | 用于 SSH 远程连接到群集节点的安全字符串类型密码（适用于 Linux）。 | No       |
| scriptActions | 在按需创建群集期间指定 [HDInsight 群集自定义](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-customize-cluster-linux)的脚本。 <br />目前，Azure 数据工厂的用户界面创作工具仅支持指定 1 个脚本操作，但你可以通过 JSON 解决此限制（在 JSON 中指定多个脚本操作）。 | No |
...
| isEspEnabled      | 如果 HDInsight 群集启用了[企业安全性套餐](https://docs.microsoft.com/azure/hdinsight/domain-joined/apache-domain-joined-architecture)，请指定“true”。 默认值为“false”。 | No       |
| connectVia        | 用于将活动分发到此链接服务的集成运行时。 可以使用 Azure 集成运行时或自托管集成运行时。 如果未指定，则使用默认 Azure Integration Runtime。 <br />对于启用企业安全性套餐（ESP）的 HDInsight 群集，请使用自我托管的集成运行时，该运行时向群集提供直线，或者应将其部署在与 ESP HDInsight 群集相同的虚拟网络中。 | No       |
...
| connectVia        | 用于将活动分发到此链接服务的集成运行时。 可以使用 Azure 集成运行时或自托管集成运行时。 如果未指定，则使用默认 Azure Integration Runtime。 | No       |

## <a name="azure-machine-learning-studio-linked-service"></a>Azure 机器学习 Studio 链接服务
创建 Azure 机器学习 Studio 链接服务，以向数据工厂注册机器学习 batch 计分终结点。
...
| updateResourceEndpoint | 用于使用定型模型文件更新预测 Web 服务的 Azure 机器学习 Web 服务终结点的更新资源 URL | No                                       |
...
## <a name="azure-machine-learning-service-linked-service"></a>Azure 机器学习服务链接服务
创建 Azure 机器学习服务链接服务，以便将 Azure 机器学习服务工作区连接到数据工厂。

> [!NOTE]
> 目前仅支持 Azure 机器学习服务链接服务的服务主体身份验证。
...
| properties               | 描述                              | 需要                                 |
| ---------------------- | ---------------------------------------- | ---------------------------------------- |
| Type                   | 类型属性应设置为： **AzureMLService**。 | 是                                      |
| subscriptionId         | Azure 订阅 ID              | 是                                      |
| resourceGroupName      | name | 是                                      |
| mlWorkspaceName        | Azure 机器学习服务工作区名称 | 是  |
| servicePrincipalId     | 指定应用程序的客户端 ID。     | No |
| servicePrincipalKey    | 指定应用程序的密钥。           | No |
| tenant                 | 指定应用程序的租户信息（域名或租户 ID）。 可将鼠标悬停在 Azure 门户右上角进行检索。 | 如果已指定 updateResourceEndpoint，则为必需 | No |
| connectVia             | 用于将活动分发到此链接服务的集成运行时。 可以使用 Azure 集成运行时或自托管集成运行时。 如果未指定，则使用默认 Azure Integration Runtime。 | No |    
...
| existingClusterId    | 现有群集的群集 ID，用于在其上运行所有作业。 该群集应该是已创建的交互式群集。 如果群集停止响应，则可能需要手动重启该群集。 Databricks 建议在新群集上运行作业，以提高可靠性。 可以通过 Databricks 工作区 ->“群集”->“交互式群集名称”->“配置”->“标记”找到交互式群集的群集 ID。 [更多详细信息](https://docs.databricks.com/user-guide/clusters/tags.html) | No 
| instancePoolId    | Databricks 工作区中现有池的实例池 ID。  | No  |
| newClusterVersion    | 群集的 Spark 版本。 它会在 Databricks 中创建作业群集。 | No  |
| newClusterNumOfWorker| 此群集应该拥有的工作节点的数目。 群集有一个 Spark 驱动程序和 num_workers 个执行器，总共有 num_workers + 1 个 Spark 节点。 格式为 Int32 的字符串（如 "1"）表示 numOfWorker 为1或 "1:10" 表示自动缩放，最小为1，最大值为10。  | No                |
| newClusterNodeType   | 此字段通过单个值对提供给此群集中的每个 Spark 节点的资源进行编码。 例如，可以针对内存或计算密集型工作负荷设置和优化 Spark 节点。 新群集需要此字段                | No               |
...
| newClusterInitScripts| 新群集的一组可选的、用户定义的初始化脚本。 指定 init 脚本的 DBFS 路径。 | No  |
...
创建 Azure 函数链接服务，并将其与[Azure function 活动](control-flow-azure-function-activity.md)一起使用，以在数据工厂管道中运行 Azure Functions。 Azure 函数的返回类型必须是有效的 `JObject`。 （请记住， [JArray](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_Linq_JArray.htm) *不*是 `JObject`。）除 `JObject` 以外的任何返回类型都将失败，并且引发用户错误*响应内容不是有效的 JObject*。
