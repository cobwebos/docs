...
| linkedServiceName            | 由按需群集用于存储和处理数据的 Azure 存储链接服务。 HDInsight 群集在创建时与此 Azure 存储帐户位于同一区域。 Azure HDInsight 会限制可在其支持的每个 Azure 区域中使用的核心总数。 确保在 Azure 区域中有足够的内核配额来满足所需的 clusterSize。 有关详细信息，请参阅[使用 Hadoop、Spark、Kafka 等在 HDInsight 中设置群集](../hdinsight/hdinsight-hadoop-provision-linux-clusters.md)<p>目前，无法创建使用 Azure Data Lake Storage (Gen 2) 作为存储的按需 HDInsight 群集。 如果想要存储在 Azure Data Lake Storage (Gen 2) 中处理的来自 HDInsight 的结果数据，请使用复制活动将数据从 Azure Blob 存储复制到 Azure Data Lake Storage (Gen 2)。 </p> | 是      |
...
| clusterNamePrefix           | HDI 群集名称的前缀，会自动在群集名称末尾追加时间戳| 否       |
...
| linkedServiceName | Azure 存储链接服务（指 HDInsight 群集使用的 Azure Blob 存储）的名称。 <p>目前，不能为此属性指定 Azure Data Lake Storage (Gen 2) 链接服务。 如果 HDInsight 群集有权访问 Data Lake Store，则可从 Hive/Pig 脚本访问 Azure Data Lake Storage (Gen 2) 中的数据。 </p> | 是      |
...
可以创建 Azure 机器学习工作室链接服务，以向数据工厂注册机器学习批处理计分终结点。
...
| updateResourceEndpoint | Azure 机器学习 Web 服务终结点的更新资源 URL 用于使用经过训练的模型文件更新预测性 Web 服务 | 否                                       |
...
创建 Azure 函数链接服务，并将其与 [Azure Function 活动](control-flow-azure-function-activity.md)一起使用，以在数据工厂管道中运行 Azure Functions。 Azure 函数的返回类型必须是有效的 `JObject`。 （请记住：[JArray](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_Linq_JArray.htm) 不是 `JObject`。）`JObject` 以外的任何返回类型都会失败，并引发用户错误“响应内容不是有效的 JObject”。
