...
### <a name="monitor"></a>监视
成功地构建和部署数据集成管道后，提供优化数据的业务值，你想要监视计划的活动和管道，以了解成功/失败率。 Azure 数据工厂通过 Azure 门户上的 Azure Monitor、API、PowerShell、Microsoft Operations Management Suite (OMS) 和运行状况面板，对管道监视提供内置支持。
...
数据工厂具有 30 种以上的连接器，因此能够将混合和异类环境中的数据载入 Azure。  有关内部测试和优化建议的最新性能结果，请参阅[性能和优化指南](copy-activity-performance.md)。 此外，我们为在专用网络环境中安装的自承载集成运行时启用了高可用性和扩展性，以解决大量第 1 层企业客户希望获得更高可用性和扩展性的需求。
...
管道运行是管道执行的实例。 管道运行通常是通过将自变量传递给管道中定义的参数来实例化的。 自变量可手动传递，也可在触发器定义中传递
