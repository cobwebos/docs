...
# 教程：使用 Azure PowerShell 创建移动数据的数据工厂管道
...
- 安装 **Azure PowerShell**。 遵循[如何安装和配置 Azure PowerShell](../powershell-install-configure.md) 中的说明。
...
2. 在数据工厂中创建**链接服务**。 本步骤创建两类链接服务：Azure 存储和 Azure SQL 数据库。 
    ...
    AzureStorageLinkedService 链接将 Azure 存储帐户链接到数据工厂。 你根据[先决条件](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md)创建了一个容器并将数据上传到该存储帐户。   

    AzureSqlLinkedService 将 Azure SQL 数据库链接到数据工厂。 从 Blob 存储复制的数据存储在该数据库中。 你根据[先决条件](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md)在该数据库中创建了一个 SQL 表。   
3. 在数据工厂中创建输入和输出**数据集**。  
...
3. 运行 **New-AzureRmDataFactory** cmdlet，创建名为 **ADFTutorialDataFactoryPSH** 的数据工厂：  
    ...
    ```
    此名称可能已使用。 因此，可以通过添加前缀或后缀（例如：ADFTutorialDataFactoryPSH05152017）使数据工厂的名称变得唯一，然后再次运行该命令。  
