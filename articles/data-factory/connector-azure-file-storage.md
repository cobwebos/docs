...
>[!TIP]
>当使用 ADF UI 创作时，可以用于创建链接的服务，哪些下方生成类型找到"Azure 文件存储"的特定条目`FileServer`对象。
...
若要向 / 从 Azure 文件存储中复制数据**Parquet 或带分隔符的文本格式**，请参阅[Parquet 格式](format-parquet.md)并[分隔符的文本格式](format-delimited-text.md)基于格式的数据集上的文章和受支持的设置。 用于 Azure 文件存储下支持以下属性`location`中基于格式的数据集的设置：
...
若要向 / 从 Azure 文件存储中复制数据**ORC/Avro/JSON/二进制格式**，支持以下属性：
...
若要将数据从 Azure 中的文件存储复制**Parquet 或带分隔符的文本格式**，请参阅[Parquet 格式](format-parquet.md)和[分隔符的文本格式](format-delimited-text.md)上基于格式的复制活动的文章源和受支持的设置。 用于 Azure 文件存储下支持以下属性`storeSettings`基于格式的复制源中的设置：
...
若要将数据从 Azure 中的文件存储复制**ORC/Avro/JSON/二进制格式**，将复制活动中支持以下属性**源**部分：
...
将数据复制到 Azure 文件存储在**Parquet 或带分隔符的文本格式**，请参阅[Parquet 格式](format-parquet.md)并[分隔符的文本格式](format-delimited-text.md)基于格式的复制活动接收器上的文章和受支持的设置。 用于 Azure 文件存储下支持以下属性`storeSettings`格式基于复制接收器中的设置：
...
若要将数据复制到 Azure 文件存储在**ORC/Avro/JSON/二进制格式**，在支持以下属性**接收器**部分：
