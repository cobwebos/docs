...
ms.locfileid: "81415802"
---
# <a name="load-data-from-office-365-by-using-azure-data-factory"></a>使用 Azure 数据工厂从 Office 365 加载数据

[!INCLUDE[appliesto-adf-xxx-md](includes/appliesto-adf-xxx-md.md)]

本文介绍如何使用数据工厂将 Office 365 中的数据载入 Azure Blob 存储。__ 可以遵循类似的步骤将数据复制到 Azure Data Lake Gen1 或 Gen2。 请参阅 [Office 365 连接器文章](connector-office-365.md)，了解有关从 Office 365 复制数据的一般信息。
...
2. 在“新建数据工厂”页中，为下图中所示的字段提供值****：
    ...
    * **名称**：输入 Azure 数据工厂的全局唯一名称。 如果收到错误"数据工厂名称*LoadFromOffice365Demo*不可用"，请输入数据工厂的其他名称。 例如，可以使用名称 _**yourname**_**LoadFromOffice365Demo**。 请重试创建数据工厂。 有关数据工厂项目的命名规则，请参阅[数据工厂命名规则](naming-rules.md)。
...
3. 现在，您位于"复制活动配置"选项卡中。单击 Office 365 数据集旁边的 **"编辑"** 按钮以继续数据配置。
...
8. 现在返回**管道** > **源选项卡**，继续配置 Office 365 数据提取的其他属性。  用户范围和用户范围筛选器是可选谓词，你可以定义这些谓词来限制要从 Office 365 中提取的数据。 有关如何配置这些设置的说明，请参阅 [Office 365 数据集属性](https://docs.microsoft.com/azure/data-factory/connector-office-365#dataset-properties)部分。
...
如果这是您第一次请求此上下文的数据（正在访问的哪个数据表的组合，哪个目标帐户是要加载到的数据，哪个用户标识正在发出数据访问请求），您将看到复制活动状态为 **"正在进行**"，并且只有在单击"操作"下的"详细信息"链接时，您才会看到状态为 **"重新设置同意**"。  在继续执行数据提取之前，数据访问审批者组的成员需要在 Privileged Access Management 中审批该请求。

_请求同意的状态：_
![活动执行详细信息 - 请求同意](./media/load-office-365-data/activity-details-request-consent.png) 
