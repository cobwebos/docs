...
设置 Azure-SSIS IR 后，你可以使用熟悉的工具在 Azure 中部署和运行包。 这些工具包括 SQL Server Data Tools、SQL Server Management Studio 和命令行工具，如 `dtinstall`、`dtutil`和 `dtexec`。
...
2. 在**Integration Runtime 安装程序**的 "**常规设置**" 页上，完成以下步骤。
   ...
   ![常规设置](./media/tutorial-create-azure-ssis-runtime-portal/general-settings.png)
    ...
    d. 对于 "**节点大小**"，请选择 integration runtime 群集中节点的大小。 仅显示支持的节点大小。 如果要运行多个计算密集型或内存密集型包，请选择一个较大的节点大小（向上缩放）。
...
3. 在 " **SQL 设置**" 页上，完成以下步骤。
   ...
   a. 选中 "**创建 SSIS 目录 ...** " 复选框，以选择要在 Azure-SSIS IR 上运行的包的部署模型。 您将选择项目部署模型，其中，包部署到您的数据库服务器托管的 SSISDB，或包部署模型，其中，包部署到文件系统、文件共享或 Azure 文件中。 
   ...
   h.如果该值不存在，请单击“添加行”。 对于 "**目录数据库服务层**"，请选择用于托管 SSISDB 的数据库服务器的服务层。 选择 "基本"、"标准" 或 "高级" 层，或选择弹性池名称。 
...
4. 在 "**高级设置**" 页上，完成以下步骤。
   ...
   a. 对于**每个节点的最大并行**执行数，请选择 integration runtime 群集中每个节点并发运行的最大包数。 仅显示支持的包数。 如果要使用多个核心来运行计算或内存密集型单个大型包，请选择一个较小的数字。 如果要在单个内核中运行一个或多个小包，请选择一个较大的数字。
...
5. 选中 "为**您的 Azure-SSIS Integration Runtime 选择要加入的 VNet 并允许 ADF 创建特定网络资源**" 复选框，以选择是否要将集成运行时加入到虚拟网络。 
   ...
   d. 对于“VNet 名称”，请选择虚拟网络的名称。 此虚拟网络应该与用于包含虚拟网络服务终结点的 Azure SQL 数据库服务器或虚拟网络中的托管实例相同，以便托管 SSISDB。 此虚拟网络应该与连接到本地网络的虚拟网络相同。
...
8. 在 "**摘要**" 页上，查看所有设置设置，将建议的文档链接做成书签，然后选择 "**完成**" 以开始创建集成运行时。
    ...
    >
    > 如果使用 SSISDB，数据工厂服务将连接到数据库服务器来准备 SSISDB。 它还会配置虚拟网络的权限和设置（如果已指定），并将 Azure-SSIS IR 加入虚拟网络中。
    > 
...
使用 [New-AzResourceGroup](../azure-resource-manager/resource-group-overview.md) 命令创建 [Azure 资源组](/powershell/module/az.resources/new-azresourcegroup)。 资源组是在其中以组的形式部署和管理 Azure 资源的逻辑容器。
...
如果不将 Azure SQL 数据库服务器与虚拟网络服务终结点一起使用，或者不将托管实例与专用终结点一起用于托管 SSISDB，或者需要访问本地数据，则可以省略 `VNetId` 和 `Subnet` 参数，或者为其传递空值它们. 如果将自承载 IR 配置为用于访问本地数据的 Azure-SSIS IR 代理，则也可以省略它们。 否则，你不能省略这些值，并且必须从虚拟网络配置传递有效值。 有关详细信息，请参阅将[Azure-SSIS IR 加入到虚拟网络](https://docs.microsoft.com/azure/data-factory/join-azure-ssis-integration-runtime-virtual-network)。
...
- 对于具有专用终结点的 Azure SQL 数据库服务器，服务器终结点格式 `<server name>.database.windows.net`。
- 对于具有专用终结点的托管实例，服务器终结点格式 `<server name>.<dns prefix>.database.windows.net`。
- 对于具有公共终结点的托管实例，服务器终结点格式 `<server name>.public.<dns prefix>.database.windows.net,3342`。 

如果不使用 SSISDB，则可以将包部署到文件系统、文件共享或 Azure 文件中。 然后，你可以使用 `dtinstall`、`dtutil`和 `dtexec` 命令行工具在 Azure-SSIS IR 上运行它们。 有关详细信息，请参阅[部署 SSIS 包](/sql/integration-services/packages/deploy-integration-services-ssis-projects-and-packages#deploy-packages-to-integration-services-server)。 
...
- [管理 Azure-SSIS IR](manage-azure-ssis-integration-runtime.md)。 此文介绍如何停止、启动或删除 Azure-SSIS IR。 还介绍如何通过添加更多节点来横向扩展 Azure-SSIS IR。
