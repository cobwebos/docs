---
title: 使用 PowerShell 增量加载数据
description: 此 PowerShell 脚本演示如何使用 Azure 数据工厂将数据从 Azure SQL 数据库增量复制到 Azure Blob 存储。
...
| [Set-AzDataFactoryV2Pipeline](/powershell/module/az.datafactory/Set-Azdatafactoryv2pipeline) | 在数据工厂中创建管道。 管道包含执行特定操作的一个或多个活动。 在此管道中，复制活动在 Azure Blob 存储中将数据从一个位置复制到另一个位置。 |
