---
title: 内存中技术
description: 在 Azure SQL 数据库和 Azure SQL 托管实例中，内存中技术极大地提高了事务和分析工作负荷的性能。
...
ms.locfileid: "85985184"
---
# <a name="optimize-performance-by-using-in-memory-technologies-in-azure-sql-database-and-azure-sql-managed-instance"></a>使用 Azure SQL 数据库和 Azure SQL 托管实例中的内存中技术优化性能
[!INCLUDE[appliesto-sqldb-sqlmi](includes/appliesto-sqldb-sqlmi.md)]

使用内存中技术，可以提高应用程序的性能，并可能降低数据库成本。

## <a name="when-to-use-in-memory-technologies"></a>何时使用内存中技术

通过使用内存中技术，可以在各种工作负荷上实现性能改进：
...
## <a name="benefits-of-in-memory-technology"></a>内存中技术的优点

由于查询和事务处理效率更高，内存中技术还有助于降低成本。 用户通常不需要升级数据库的定价层即可实现性能提升。 在某些情况下，你甚至可能会降低定价层，同时仍然可以看到内存中技术的性能改进。
...
本文介绍特定于 Azure SQL 数据库和 Azure SQL 托管实例的内存中 OLTP 和列存储索引的各个方面，还包括示例：

- 介绍这些技术对存储和数据大小限制的影响。
- 介绍如何控制使用这些技术的数据库在不同定价层之间的移动。
- 你将看到两个示例，演示如何使用内存中 OLTP 以及列存储索引。

有关内存中 SQL Server 的详细信息，请参阅：
...
内存中 OLTP 技术通过将所有数据保存在内存中，提供极快的数据访问操作。 它还使用专用索引、查询本机编译和无闩锁数据访问来提高 OLTP 工作负荷的性能。 可通过两种方式来组织内存中 OLTP 数据：
...
有关内存中 OLTP 的快速入门教程：[快速入门1：用于提高 T-sql 性能的内存中 Oltp 技术](/sql/relational-databases/in-memory-oltp/survey-of-initial-areas-in-in-memory-oltp)（可帮助您入门的另一篇文章）
...
如果查询返回 **1**，则此数据库支持内存中 OLTP。 下面的查询标识了在将数据库降级到常规用途、标准或基本之前需要删除的所有对象：
...
内存中 OLTP 包括用于存储用户数据的内存优化表。 这些表必需在内存可容纳的范围内。 因为你直接在 SQL 数据库中管理内存，所以我们具有用户数据配额的概念。 这种概念称为*内存中 OLTP 存储*。

每个受支持的单一数据库定价层和每个弹性池定价层都包括一定量的内存中 OLTP 存储。

- [基于 DTU 的资源限制-单个数据库](database/resource-limits-dtu-single-databases.md)
...
若要详细了解如何监视内存中 OLTP 存储利用率并在几乎达到 cap 时配置警报，请参阅[监视内存中存储](in-memory-oltp-monitor-space.md)。
...
但是，将层降级可能会对数据库造成负面影响。 如果数据库包含内存中 OLTP 对象，则从“业务关键”层降级到“常规用途”层（或者从“高级”层降级到“标准”或“基本”层）时，影响就尤为明显。 降级后，内存优化表不可用（即使它们保持可见）。 降低弹性池的定价层或将具有内存中技术的数据库移动到常规用途、标准或基本弹性池时，需要考虑相同的注意事项。

> [!Important]
> “常规用途”、“标准”或“基本”层不支持内存中 OLTP。 因此，不能将包含任何内存中 OLTP 对象的数据库移动到其中一层。

将数据库降级到常规用途、标准或基本模式之前，请删除所有内存优化表和表类型，以及所有本机编译的 T-sql 模块。

*业务关键层中的缩减资源*：内存优化表中的数据必须在与数据库或托管实例的层关联的内存中 OLTP 存储中，或者在弹性池中可用。 如果尝试缩减层或将数据库移到可用内存中 OLTP 存储不足的池，操作将会失败。
...
- **非聚集列存储**：其中的数据存储在传统的行存储表中，有一个采用列存储格式的索引用于执行分析查询。 使用此模型可以实现混合事务分析处理 (HTAP)：可以针对事务工作负荷运行高性能的实时分析。 针对优化的行存储表执行 OLTP 查询以访问少量的行，同时，针对更适合用于扫描和分析的列存储索引执行 OLAP 查询。 查询优化器基于查询动态选择行存储或列存储格式。 非聚集列存储索引无法减少数据大小，因为原始数据集按原样保留在原始行存储表中。 但是，其他列存储索引的大小应该比同等的 B 树索引要小几个量级。

> [!Note]
> 内存中列存储技术仅在内存中保留处理时所需的数据，不能装入内存的数据将存储在磁盘上。 因此，内存中列存储结构中的数据量可能会超出可用内存量。

有关该技术的深入介绍视频：

- [列存储索引：来自 Ignite 2016 的内存中分析视频](https://blogs.msdn.microsoft.com/sqlserverstorageengine/20../../columnstore-index-in-memory-analytics-i-e-columnstore-index-videos-from-ignite-2016/)
...
使用非聚集列存储索引时，仍以传统行存储格式存储基础表。 因此，存储节省的成本与聚集列存储索引的容量不同。 但是，如果使用单个列存储索引取代众多传统非聚集索引，则仍可整体减少表的存储占用。

### <a name="changing-service-tiers-of-databases-containing-columnstore-indexes"></a>更改包含列存储索引的数据库的服务层级

如果目标层低于 S3，可能无法将*单个数据库降级为基本或标准*。 只有“业务关键”/“高级”定价层、“标准”层、“S3”及更高的层支持列存储索引，“基本”层则不支持。 将数据库降级到不受支持的层或级别时后，列存储索引不可用。 系统会保留列存储索引，但永远不会利用索引。 如果后来又升级回到受支持的层或级别，列存储索引立即可供再次利用。
