...
description: 使用存储空间直通通过 Azure 虚拟机上的 SQL Server 创建故障转移群集实例（FCI）。
...
---
# <a name="create-an-fci-with-storage-spaces-direct-sql-server-on-azure-vms"></a>使用存储空间直通创建 FCI （SQL Server Azure Vm）
[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

本文介绍如何通过在 Azure 虚拟机（Vm）上使用 SQL Server[存储空间直通](/windows-server/storage/storage-spaces/storage-spaces-direct-overview)来创建故障转移群集实例（FCI）。 存储空间直通充当基于软件的虚拟存储区域网络（VSAN），它在 Windows 群集中的节点（Azure Vm）之间同步存储（数据磁盘）。 
...
1. 使用远程桌面协议（RDP）连接到第一台虚拟机，该帐户是本地管理员的成员并且有权在 Active Directory 中创建对象的域帐户。 使用此帐户完成余下的配置。
...
存储空间直通的磁盘需要为空。 它们不能包含分区或其他数据。 若要清除磁盘，请按照[部署存储空间直通](https://docs.microsoft.com/windows-server/storage/storage-spaces/deploy-storage-spaces-direct?redirectedfrom=MSDN#step-31-clean-drives)中的说明进行操作。
...
1. [创建卷](https://technet.microsoft.com/windows-server-docs/storage/storage-spaces/hyper-converged-solution-using-storage-spaces-direct#step-36-create-volumes)。
   ...
   存储空间直通中的某个功能可自动创建存储池（如果已启用此功能）。 接下来，可以创建卷。 PowerShell cmdlet `New-Volume` 自动完成卷的创建过程。 此过程包括格式化、将卷添加到群集，以及创建 CSV。 此示例创建一个 800 千兆字节 (GB) 的 CSV：
...
1. 选择“新建 SQL Server 故障转移群集安装”。 遵照向导中的说明安装 SQL Server FCI。
   ...
   FCI 数据目录需位于群集存储中。 使用存储空间直通，它不是共享磁盘，而是每个服务器上的卷的装入点。 存储空间直通会在两个节点之间同步该卷。 卷作为 CSV 提供给群集。 使用数据目录的 CSV 装入点。
...
- Azure 虚拟机支持 Windows Server 2019 上的 Microsoft 分布式事务处理协调器（MSDTC），Csv 上有存储和[标准负载均衡器](../../../load-balancer/load-balancer-standard-overview.md)。
