---
title: 教程：可用性组的先决条件
description: 本教程介绍如何在 Azure 虚拟机上配置创建 SQL Server Always On 可用性组的先决条件。
...
ms.locfileid: "84669352"
---
# <a name="prerequisites-for-creating-always-on-availability-groups-on-sql-server-on-azure-virtual-machines"></a>在 Azure 虚拟机上的 SQL Server 上创建 Always On 可用性组的先决条件

[!INCLUDE[appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

本教程演示如何完成[在 Azure 虚拟机（vm）上创建 SQL Server Always On 可用性组](availability-group-manually-configure-tutorial.md)的先决条件。 完成先决条件后，会在单个资源组中拥有一个域控制器、两个 SQL Server Vm 和一个见证服务器。
...
3. 选择“虚拟网络”。
4. 在**虚拟网络**上，选择 "**资源管理器**" 部署模型，然后选择 "**创建**"。
...
1. 在仪表板上，选择创建的资源组 " **SQL-HA-RG**"。 在“资源”下的资源组中找到网络。

    如果不显示**SQL-HA-RG** ，请通过选择 "**资源组**" 并按资源组名称进行筛选来查找它。

2. 在资源列表中选择 " **autoHAVNET** "。 
...
创建虚拟机前，需创建可用性集。 可用性集可减少计划内或计划外维护事件的停机时间。 Azure 可用性集是 Azure 置于物理容错域和更新域上的逻辑资源组。 容错域确保可用性集的成员具有不同的电源和网络资源。 更新域确保可用性集的成员不会同时停机进行维护。 有关详细信息，请参阅[管理虚拟机的可用性](../../../virtual-machines/linux/manage-availability.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)。

需要两个可用性集。 一个用于域控制器， 另一个用于 SQL Server VM。

若要创建可用性集，请前往资源组，并选择 "**添加**"。 通过键入“可用性集”筛选结果。 在结果中选择 "**可用性集**"，然后选择 "**创建**"。
...
3. 选择**Windows Server 2016 Datacenter**。 在**Windows Server 2016 Datacenter**中，验证是否**资源管理器**了部署模型，然后选择 "**创建**"。 
...
1. 在门户中打开 **SQL-HA-RG** 资源组，并选择 **ad-primary-dc** 计算机。 在**ad-primary-dc**上，选择 "**连接**" 以打开用于远程桌面访问的 RDP 文件。
...
6. 选择“Active Directory 域服务”和“DNS 服务器”角色。  出现提示时，添加这些角色所需的任何其他功能。
   ...
   > [!NOTE]
   > Windows 会警告没有静态 IP 地址。 如果要测试配置，请选择 "**继续**"。 对于生产方案，请在 Azure 门户中将 IP 地址设置为静态，或[使用 PowerShell 设置域控制器计算机的静态 IP 地址](../../../virtual-network/virtual-networks-reserved-private-ip.md)。
   >

    ![“添加角色”对话框](./media/availability-group-manually-configure-prerequisites-tutorial-/23-addroles.png)

7. 选择 "**下一步**"，直到你到达 "**确认**" 部分。 选中“必要时自动重启目标服务器”复选框。
8. 选择“安装”。
...
12. 在 "**所有服务器任务详细信息**" 对话框的 "**操作**" 列中，选择 "**将此服务器提升为域控制器**"。
...
15. 选择“安装”。 **ad-primary-dc** 虚拟机会自动重新启动。
...
1. 在 Azure 门户中，选择 "虚拟网络"。

2. 在 "**设置**" 下，选择 " **DNS 服务器**"。

3. 选择 "**自定义**"，然后键入主域控制器的专用 IP 地址。
...
1. 在门户中打开 **SQL-HA-RG** 资源组，并选择 **ad-secondary-dc** 计算机。 在**ad-辅助站点**上，选择 "**连接**" 以打开用于远程桌面访问的 RDP 文件。
2. 使用已配置的管理员帐户 (BUILTIN\DomainAdmin) 和密码 (Contoso!0000) 登录到 VM。
3. 将首选 DNS 服务器地址更改为域控制器的地址。
4. 在 "**网络和共享中心**" 中，选择网络接口。

   ![网络接口](./media/availability-group-manually-configure-prerequisites-tutorial-/26-networkinterface.png)

5. 选择“属性”。
6. 选择 " **Internet 协议版本4（TCP/IPv4）** "，然后选择 "**属性**"。
7. 选择 "**使用以下 dns 服务器地址**"，然后在 "**首选 DNS 服务器**" 中指定主域控制器的地址。
8. 选择 **"确定"**，然后单击 "**关闭**" 以提交更改。 现在可将该 VM 加入 **corp.contoso.com**。
...
15. 选择黄色警告栏上的 "**更多**" 链接。
16. 在 "**所有服务器任务详细信息**" 对话框的 "**操作**" 列中，选择 "**将此服务器提升为域控制器**"。
...
22. 选择 "**下一步**"，直到对话框达到**必备项**检查。 然后选择 "**安装**"。

服务器完成配置更改后，请重启服务器。

### <a name="add-the-private-ip-address-to-the-second-domain-controller-to-the-vpn-dns-server"></a>将专用 IP 地址添加到 VPN DNS 服务器的第二个域控制器
...
7. 选择“确定”  ，然后再次选择“确定”  。 关闭“corp”属性窗口。
...
再创建三个虚拟机。 该解决方案需要两个具有 SQL Server 实例的虚拟机。 第三个虚拟机充当见证服务器。 Windows Server 2016 可以使用[云见证](https://docs.microsoft.com/windows-server/failover-clustering/deploy-cloud-witness)。 但是，为了与以前的操作系统保持一致，本文为见证服务器使用虚拟机。  

在继续操作之前，请考虑以下设计决策。

* **存储：Azure 托管磁盘**

   将 Azure 托管磁盘用作虚拟机存储。 Microsoft 建议为 SQL Server 虚拟机使用托管磁盘。 托管磁盘在后台处理存储。 此外，当使用托管磁盘的虚拟机位于同一可用性集中时，Azure 会分发存储资源以提供适当冗余。 有关其他信息，请参阅 [Azure 托管磁盘概述](../../../virtual-machines/linux/managed-disks-overview.md)。 有关可用性集中托管磁盘的具体信息，请参阅[为可用性集中的 VM 使用托管磁盘](../../../virtual-machines/linux/manage-availability.md#use-managed-disks-for-vms-in-an-availability-set)。

* **网络：生产环境中的专用 IP 地址**

   本教程对虚拟机使用公共 IP 地址。 公共 IP 地址可通过 internet 直接连接到虚拟机，并使配置步骤更简单。 在生产环境中，Microsoft 建议仅使用专用 IP 地址，以减少 SQL Server 实例 VM 资源的漏洞涉及面。

### <a name="create-and-configure-the-sql-server-vms"></a>创建并配置 SQL Server VM

接下来，创建三个 Vm-两个 SQL Server Vm，另外一个 VM 用于附加群集节点。 若要创建每个 Vm，请返回到**SQL-HA**资源组，然后选择 "**添加**"。 搜索相应的库项，选择 "**虚拟机**"，然后选择 "**从库中**"。 参考下表中的信息创建 VM：
...
> [!NOTE]
> 此处建议的计算机大小适用于在 Azure 虚拟机中测试可用性组。 若要获得生产工作负荷的最佳性能，请参阅[Azure 虚拟机中 SQL Server](performance-guidelines-best-practices.md?toc=%2fazure%2fvirtual-machines%2fwindows%2ftoc.json)的 SQL Server 计算机大小和配置的建议。
>
...
7. 看到 "欢迎使用 corp.contoso.com 域" 消息时，请选择 **"确定**"。
8. 选择 "**关闭**"，然后在弹出对话框中选择 "**立即重新启动**"。
...
- **SQL Server VM**： SQL Server 的默认实例的端口1433。
- **Azure 负载均衡器探测：** 任何可用的端口。 示例中经常使用 59999。
- **数据库镜像端点：** 任何可用的端口。 示例中经常使用 5022。
...
2. 在左窗格中，选择“入站规则”。 在右侧窗格中，选择 "**新建规则**"。
3. 对于“规则类型”，请选择“端口”。 
...
8. 在 "**名称**" 页上，在 "**名称**" 文本框中指定规则名称（如**Azure LB 探测**），然后选择 "**完成**"。
...
* [在 Azure 虚拟机上创建 SQL Server Always On 可用性组](availability-group-manually-configure-tutorial.md)
