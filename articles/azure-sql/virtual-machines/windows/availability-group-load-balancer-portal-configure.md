...
若要完成此任务，需要在使用 Resource Manager 运行的 Azure 虚拟机上部署一个 SQL Server 可用性组。 这两个 SQL Server 虚拟机必须属于同一个可用性集。 可以使用 [Microsoft 模板](availability-group-azure-marketplace-template-configure.md)在 Resource Manager 中自动创建可用性组。 此模板会自动创建内部负载均衡器。 
