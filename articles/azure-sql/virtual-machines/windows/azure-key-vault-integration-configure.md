...
> * [资源管理器](azure-key-vault-integration-configure.md)
...
启用此功能后，它会自动安装 SQL Server 连接器，配置 EKM 提供程序以访问 Azure Key Vault，并创建允许你访问保管库的凭据。 在前面提到的本地文档列出的步骤中，可以看到此功能自动完成步骤 2 和步骤 3。 仍需手动执行的唯一操作是创建密钥保管库和密钥。 之后，会自动进行 SQL Server VM 的整个设置。 在此功能完成设置后，可以执行 Transact-SQL (T-SQL) 语句，以按照通常的方式加密数据库或备份。
...
> [!NOTE]
> 还可以使用模板配置密钥保管库集成。 有关详细信息，请参阅[用于 Azure 密钥保管库集成的 Azure 快速入门模板](https://github.com/Azure/azure-quickstart-templates/tree/master/101-vm-sql-existing-keyvault-update)。
