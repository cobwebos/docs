...
- 需要 SQL Server 所需版本的安装介质。 如果客户有[软件保障](https://www.microsoft.com/licensing/licensing-programs/software-assurance-default)，则可以从[批量许可中心](https://www.microsoft.com/Licensing/servicecenter/default.aspx)获取其安装介质。 没有软件保障的客户可以使用具有更高版本 SQL Server 的 Azure Marketplace SQL Server VM 映像（通常位于 C:\SQLServerFull 中）。
...
1. 在开始此过程之前，请备份数据库，包括系统（tempdb 除外）和用户数据库。 你还可以使用 Azure 备份服务创建与应用程序一致的 VM 级别备份。
...
1. 导出所有必需的服务器级对象（如服务器触发器、角色、登录名、链接服务器、作业、凭据和证书）。
...
1. 导入所有必需的服务器级对象（在步骤3中导出）。
1. 从头开始重新创建所有必要的用户数据库（通过使用创建的脚本或步骤4中的文件）。
