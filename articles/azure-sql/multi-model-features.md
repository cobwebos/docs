...
ms.locfileid: "85979992"
---
# <a name="multi-model-capabilities-of-azure-sql-database--sql-managed-instance"></a>& SQL 托管实例的 Azure SQL 数据库的多模型功能
[!INCLUDE[appliesto-sqldb-sqlmi](includes/appliesto-sqldb-sqlmi.md)]

使用多模型数据库，可以存储和处理多种数据格式（如关系数据、图形、JSON/XML 文档、键值对等）中表示的数据。

## <a name="when-to-use-multi-model-capabilities"></a>何时使用多模型功能

[AZURE SQL 系列产品](azure-sql-iaas-vs-paas-what-is-overview.md)旨在与关系模型结合使用，以便在大多数通用应用程序的大部分情况下提供最佳性能。 但是，Azure SQL 系列产品并不仅限于关系数据。 使用 Azure SQL 系列产品，可以使用与关系模型紧密集成的各种非关系格式。
在以下情况下，应考虑使用 Azure SQL 系列产品的多模型功能：
...
- [XML 功能](#xml-features)：在数据库中存储 XML 数据并为其编制索引，并使用本机 XQuery/XPath 运算来处理 XML 数据。 Azure SQL 系列产品具有处理 XML 数据的专用内置 XML 查询引擎。
...
在以下部分中，可以了解 Azure SQL 系列产品的最重要多模型功能。

## <a name="graph-features"></a>图形功能

Azure SQL 系列产品提供了图形数据库功能，可为数据库中的多对多关系建模。 图形是节点（或顶点）和边缘（或关系）的集合。 节点表示实体（例如，某个人或组织），边缘表示该实体连接的两个节点之间的关系（例如，爱好或朋友）。 下面是使图形数据库独一无二的某些功能：
...
[Graph 关系和图形查询功能](/sql/relational-databases/graphs/sql-graph-overview)已集成到 transact-sql 中，并获得使用 SQL Server 数据库引擎作为基础数据库管理系统的好处。
...
Azure SQL 系列产品允许分析和查询以 JavaScript 对象表示法[（JSON）](https://www.json.org/)格式表示的数据，并将关系数据导出为 JSON 文本。
...
使用 Azure SQL 系列产品可轻松地处理 JSON 数据，并将数据库与新式服务集成，并提供以下用于处理 JSON 数据的函数：
...
[JSON](/sql/relational-databases/json/json-data-sql-server)是一种核心 SQL Server 数据库引擎功能。
...
可在 Azure SQL 系列产品中使用的空间对象可用于分析和查询以 JavaScript 对象表示法[（JSON）](https://www.json.org/)格式表示的数据，并将关系数据导出为 JSON 文本。
例如[Point](/sql/relational-databases/spatial/point)、 [LineString](/sql/relational-databases/spatial/linestring)、[多边形](/sql/relational-databases/spatial/polygon)等。

Azure SQL 系列产品还提供专用的[空间索引](/sql/relational-databases/spatial/spatial-indexes-overview)，可用于改进空间查询的性能。

[空间支持](/sql/relational-databases/spatial/spatial-data-sql-server)是核心 SQL Server 数据库引擎功能。

## <a name="xml-features"></a>XML 功能

SQL Server 数据库引擎提供了一个功能强大的平台，用于为半结构化数据管理开发丰富的应用程序。 对 XML 的支持已集成到数据库引擎的所有组件中，包括以下各项：
...
[XML](/sql/relational-databases/xml/xml-data-sql-server)是一项核心 SQL Server 数据库引擎功能。
...
Azure SQL 系列产品不具有支持键值对的专用类型或结构，因为键-值结构可以本地表示为标准关系表：
...
Azure SQL 系列产品中的多模型功能也是在 Azure SQL 系列产品间共享的核心 SQL Server 数据库引擎功能。 有关这些功能的更多详细信息，请访问 SQL 关系数据库文档页：
