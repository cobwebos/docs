...
Azure SQL 托管实例几乎提供最新 SQL Server（企业版）数据库引擎具备的所有安全功能：

- 在隔离的环境中限制访问
- 使用需要标识的身份验证机制：Azure Active Directory (Azure AD) 和 SQL 身份验证
...
若要了解详细信息，请参阅 [Azure SQL 托管实例概述](sql-managed-instance-paas-overview.md)。 
...
- 托管实例
  - 遵循以下文章：[快速入门：创建托管实例](instance-create-quickstart.md)
- 能够访问托管实例，并且[为托管实例预配了 Azure AD 管理员](../database/authentication-aad-configure.md#provision-azure-ad-admin-sql-managed-instance)。 若要了解更多信息，请参阅以下文章：
  - [将应用程序连接到托管实例](connect-application-instance.md)
  - [SQL 托管实例连接体系结构](connectivity-architecture-overview.md)
  - [使用 SQL 配置和管理 Azure Active Directory 身份验证](../database/authentication-aad-configure.md)

## <a name="limit-access"></a>限制访问 

可以通过专用 IP 地址访问托管实例。 与在隔离的 SQL Server 环境中非常类似，应用程序或用户需要访问 SQL 托管实例网络 (VNet) 才能建立连接。 有关详细信息，请参阅[将应用程序连接到 SQL 托管实例](connect-application-instance.md)。

还可以在托管实例上配置服务终结点，该终结点允许使用与 Azure SQL 数据库相同的方式进行公共连接。
有关详细信息，请参阅[在 Azure SQL 托管实例中配置公共终结点](public-endpoint-configure.md)。

> [!NOTE]
> 即使启用了服务终结点，[Azure SQL 数据库防火墙规则](../database/firewall-configure.md)也不适用。 Azure SQL 托管实例使用自己的[内置防火墙](management-endpoint-verify-built-in-firewall.md)来管理连接。

## <a name="create-an-azure-ad-server-principal-login-using-ssms"></a>使用 SSMS 创建 Azure AD 服务器主体（登录名）

第一个 Azure AD 服务器主体（登录名）可以由标准 SQL 管理员帐户（非 Azure AD）创建，该帐户是 `sysadmin`，或者是在设置过程中创建的托管实例的 Azure AD 管理员。 有关详细信息，请参阅[为 SQL 托管实例预配 Azure Active Directory 管理员](../database/authentication-aad-configure.md#provision-azure-ad-admin-sql-managed-instance)。

有关如何连接到 SQL 托管实例的示例，请参阅以下文章：

- [快速入门：将 Azure VM 配置为连接到 SQL 托管实例](connect-vm-instance-configure.md)
- [快速入门：配置从本地到 SQL 托管实例的点到站点连接](point-to-site-p2s-configure.md)

1. 在 [SQL Server Management Studio](point-to-site-p2s-configure.md#connect-with-ssms) 中使用充当 `sysadmin` 或 SQL 托管实例的 Azure AD 管理员的标准 SQL 登录帐户（非 Azure AD）登录到托管实例。
...
1. 在 SQL Server Management Studio 中使用 Azure AD 服务器主体（登录名）连接到托管实例。 输入 SQL 托管实例主机名。 若要在 SSMS 中进行身份验证，可在使用 Azure AD 帐户登录时从三个选项中选择：
...
1. 在查询窗口中，使用以下语法为另一个 Azure AD 帐户创建登录名：
    ...
    此示例为 Azure AD 用户 bob@aadsqlmi.net 创建登录名。该用户的 aadsqlmi.net 域已与 Azure AD aadsqlmi.onmicrosoft.com 域相联合。

    执行以下 T-SQL 命令。 联合的 Azure AD 帐户是本地 Windows 登录名和用户的 SQL 托管实例替代项。
...
1. 为 Azure AD 中的某个组创建 SQL 托管实例登录名。 Azure AD 中必须存在该组，才能将该登录名添加到 SQL 托管实例。 请参阅[使用 Azure Active Directory 创建基本组并添加成员](../../active-directory/fundamentals/active-directory-groups-create-azure-portal.md)。 创建组 _mygroup_ 并在其中添加成员。

1. 在 SQL Server Management Studio 中打开新的查询窗口。

    此示例假设 Azure AD 中存在名为 mygroup 的组。 执行以下命令：
...
在 SQL 托管实例中对单个数据库授权的方式非常类似于 SQL Server 中数据库的授权。 可以基于数据库中的现有登录名创建用户并为其提供对该数据库的权限，或者将该用户添加到数据库角色。
...
1. 也支持基于组的形式的 Azure AD 服务器主体（登录名）创建 Azure AD 用户。
    ...
    以下示例为 Azure AD 实例中的 Azure AD 组 mygroup 创建登录名。
...
SQL 托管实例支持模拟 Azure AD 服务器级主体（登录名）。
...
## <a name="use-cross-database-queries"></a>使用跨数据库查询
...
## <a name="additional-supported-scenarios"></a>其他受支持方案

- Azure AD 服务器主体（登录名）支持 SQL 代理管理和作业执行。
- Azure AD 服务器主体（登录名）可以执行数据库备份和还原操作。
- [审核](auditing-configure.md)与 Azure AD 服务器主体（登录名）和身份验证事件相关的所有语句。
- 属于 `sysadmin` 服务器角色成员的 Azure AD 服务器主体（登录名）可以建立专用管理员连接。
- 支持 Azure AD 服务器主体（登录名）使用 [sqlcmd 实用工具](/sql/tools/sqlcmd-utility)和 [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms) 工具。
...
请参阅 [SQL 托管实例安全功能](sql-managed-instance-paas-overview.md#security-features)一文，全面了解保护数据库的各种方式。 此文介绍了以下安全功能：
