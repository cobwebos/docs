---
title: 构建可缩放的云数据库
description: 通过弹性数据库客户端库构建可缩放的 .NET 数据库应用。
...
使用 Azure SQL 数据库的可缩放工具和功能，可以轻松扩大数据库。 特别是，可以使用**弹性数据库客户端库**来创建和管理扩大的数据库。 利用此功能，你可以使用数百甚至数千个数据库在 Azure SQL 数据库中轻松开发分片应用程序。
...
4. [迁移要扩展的现有数据库](elastic-convert-to-use-elastic-tools.md)
5. [依赖于数据的路由](elastic-scale-data-dependent-routing.md)
...
12. [将弹性数据库客户端库与实体框架配合使用](elastic-scale-use-entity-framework-applications-visual-studio.md)
...
16. [弹性数据库工具常见问题](elastic-scale-faq.md)
...
- **分片映射管理**：创建一个称为 "分片映射管理器" 的特殊数据库。 分片映射管理是一种使应用程序能够管理其分片相关元数据的功能。 开发人员可使用此功能将数据库注册为分片（描述各个分片键或键范围到这些数据库的映射），并随着数据库的数量和组成发展来维护此元数据，以反映容量更改。 如果没有弹性数据库客户端库，则在实现分片时需要花费大量时间来编写管理代码。 有关详细信息，请参阅[分片映射管理](elastic-scale-shard-map-management.md)。

- **数据依赖型路由**：假设将一个请求传入应用程序。 基于请求的分片键值，应用程序必须根据该键值判断正确的数据库。 接着，它会与数据库建立连接来处理请求。 借助数据依赖型路由，能够通过对应用程序的分片映射的单个简单调用打开连接。 数据相关的路由是基础结构代码的另一个区域，现已通过弹性数据库客户端库中的功能进行了介绍。 有关详细信息，请参阅[数据依赖型路由](elastic-scale-data-dependent-routing.md)。
...
通常，使用弹性数据库工具的客户在提交分片本地操作时，可以获得完整的 T-sql 功能，而不是具有其自己的语义的跨分片操作。



## <a name="next-steps"></a>后续步骤

- 弹性数据库客户端库（[Java](https://search.maven.org/#search%7Cga%7C1%7Ca%3A%22azure-elasticdb-tools%22)、 [.net](https://www.nuget.org/packages/Microsoft.Azure.SqlDatabase.ElasticScale.Client/)）-**下载**库。

- [弹性数据库工具入门](elastic-scale-get-started.md)-尝试演示客户端功能的**示例应用**。
