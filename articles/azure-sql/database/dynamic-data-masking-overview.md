---
title: 动态数据屏蔽
description: 动态数据掩码通过对 Azure SQL 数据库、Azure SQL 托管实例和 Azure Synapse Analytics 的非特权用户屏蔽敏感数据来限制敏感数据的公开
...
例如，呼叫中心服务代表可以根据呼叫者的信用卡号的多个数字来识别其身份，但这些数据项不应完全透露给服务代表。 可以定义掩码规则，对任意查询的结果集中任何信用卡号除最后四位数以外的其他所有数字进行掩码。 作为另一个示例，可以定义适当的数据掩码来保护个人数据，使开发人员无需违反法规遵从性法规便可查询生产环境以进行故障排除。

## <a name="dynamic-data-masking-basics"></a>动态数据掩码基础知识

在 Azure 门户中设置动态数据屏蔽策略，方法是在 "SQL 数据库配置" 窗格中选择 "**安全性**" 下的 "**动态数据掩码**" 边栏选项卡。 此功能不能使用 Azure Synapse （使用 PowerShell 或 REST API）或 SQL 托管实例的门户进行设置。 有关详细信息，请参阅 [Dynamic Data Masking](/sql/relational-databases/security/dynamic-data-masking)。
...
* **从屏蔽中排除的 sql 用户**-一组可以在 sql 查询结果中获取非屏蔽数据的 sql 用户或 Azure AD 标识。 始终不会对拥有管理员权限的用户进行掩码，这些用户可以看到没有任何掩码的原始数据。
...
### <a name="data-masking-policies"></a>数据屏蔽策略
