---
title: 使用 Excel 进行连接
description: 了解如何将 Microsoft Excel 连接到 Azure SQL 数据库或 Azure SQL 托管实例中的数据库。 将数据导入 Excel 以进行报告和数据探索。
...
ms.locfileid: "84267505"
---
# <a name="connect-excel-to-a-database-in-azure-sql-database-or-azure-sql-managed-instance-and-create-a-report"></a>将 Excel 连接到 Azure SQL 数据库或 Azure SQL 托管实例中的数据库，并创建报表
[!INCLUDE[appliesto-sqldb-sqlmi](../includes/appliesto-sqldb-sqlmi.md)]

您可以将 Excel 连接到数据库，然后导入数据，并根据数据库中的值来创建表和图表。 在本教程中，将设置 Excel 与数据库表之间的连接，保存用于存储 Excel 的数据和连接信息的文件，然后根据数据库值创建分析数据透视图。

在开始之前，您需要创建一个数据库。 如果没有，请参阅在[AZURE SQL 数据库中创建数据库](single-database-create-quickstart.md)和[创建服务器级 IP 防火墙](firewall-create-server-level-portal-quickstart.md)，以便在几分钟内使数据库中的示例数据正常运行。
...
1. 若要将 Excel 连接到 SQL 数据库中的数据库，请打开 Excel，然后创建新的工作簿或打开现有的 Excel 工作簿。
...
4. 在 " **SQL Server 数据库**" 对话框中，选择左侧的 "**数据库**"，然后输入要连接到的服务器的**用户名**和**密码**。 选择“连接”以打开“导航器”********。
...
建立连接后，有多种加载数据的方式可供选择。 例如，以下步骤将基于在 SQL 数据库中的数据库中找到的数据创建透视图表。
...
* 了解如何[与 SQL Server Management Studio 进行连接和查询](connect-query-ssms.md)，以便进行高级查询和分析。
* 了解 [弹性池](elastic-pool-overview.md)的优点。
* 了解如何[创建连接到后端上的 AZURE SQL 数据库的 web 应用程序](../../app-service/app-service-web-tutorial-dotnet-sqldatabase.md)。
