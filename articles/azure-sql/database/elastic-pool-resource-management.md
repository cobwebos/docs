...
|指标名称|说明|推荐的平均值|
|----------|--------------------------------|------------|
|`avg_instance_cpu_percent`|与弹性池关联的 SQL 进程的 CPU 使用率，由基础操作系统衡量。 在每个数据库的 " [sys.dm_db_resource_stats](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) " 视图中，在数据库的 " [sys.elastic_pool_resource_stats](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-elastic-pool-resource-stats-azure-sql-database) " 视图中可用 `master` 。 此指标还会发出到 Azure Monitor，其中命名为[named](https://docs.microsoft.com/azure/azure-monitor/platform/metrics-supported#microsoftsqlserverselasticpools) `sqlserver_process_core_percent` ，可在 Azure 门户中查看。 此值对于同一弹性池中的每个数据库都是相同的。|低于70%。 偶尔会出现最多90% 的短暂的情况。|
...
|`oom_per_second`|内存不足 (OOM 在弹性池中) 错误，这是内存压力的指示器。 在 [sys.dm_resource_governor_resource_pools_history_ex](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-resource-governor-resource-pools-history-ex-azure-sql-database) 视图中可用。 请参阅示例查询 [示例](#examples) 以计算此指标。|0|
...
|[sys.dm_resource_governor_workload_groups](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-resource-governor-workload-groups-transact-sql)|返回工作负荷组的累计工作负荷组统计信息和当前配置。 此视图可以与列上的 sys.dm_resource_governor_resource_pools 联接 `pool_id` ，以获取资源池信息。|
