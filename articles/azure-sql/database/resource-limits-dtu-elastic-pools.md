...
对于 Azure SQL 数据库弹性池，下表显示了每个服务层提供的资源和计算大小。 你可以使用以下内容设置服务层、计算大小和存储量：
...
| 每个池的最大内存中 OLTP 存储 (GB) | 不适用 | 空值 | 空值 | 空值 | 空值 | 空值 | 空值 | 空值 |
...
| 每个池的最大内存中 OLTP 存储 (GB) | 不适用 | 空值 | 空值 | 空值 | 空值 | 空值 |
...
| 每个池的最大内存中 OLTP 存储 (GB) | 不适用 | 空值 | 空值 | 空值 | 空值 |
