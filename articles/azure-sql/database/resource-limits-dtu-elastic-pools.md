...
向弹性池提供的资源可能会超过为相同数量的 Dtu 提供给弹性池之外单个数据库的资源。 这意味着，弹性池的 eDTU 使用率可能小于池中数据库的 DTU 使用率的总和，具体取决于工作负荷模式。 例如，在一个极端情况下，如果弹性池中只有一个数据库的数据库 DTU 利用率为100%，则池 eDTU 利用率对于某些工作负荷模式可能会是50%。 即使已设置每个数据库的显式最大 DTU，也可能会发生这种情况。 在这种情况下，与具有匹配服务目标的单一数据库的 DTU 消耗相同的方式限制了池数据库 DTU 消耗。
