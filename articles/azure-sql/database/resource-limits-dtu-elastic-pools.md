...
* 有关 Azure SQL 数据库的 DTU 购买模型资源限制，请参阅 [DTU 资源限制 - Azure SQL 数据库](resource-limits-vcore-elastic-pools.md)。
...
对于相同数量的 Dtu，为弹性池提供的资源可能会超过向弹性池之外的单一数据库提供的资源。 这意味着，弹性池的 eDTU 使用率可能小于池中数据库的 DTU 使用率的总和，具体取决于工作负荷模式。 例如，在一个极端情况下，如果弹性池中只有一个数据库的数据库 DTU 利用率为100%，则池 eDTU 利用率对于某些工作负荷模式可能会是50%。 即使每个数据库的最大 DTU 保持为给定池大小的最大支持值，也可能会发生这种情况。
...
| 每个池的最大内存中 OLTP 存储 (GB) | 空值 | N/A | N/A | N/A | N/A | N/A |
| 每个池的最大数据库数<sup>2</sup> | 100 | 200 | 500 | 500 | 500 | 500 |
| 每个池的最大并发工作线程数（请求数） <sup>3</sup> | 100 | 200 | 400 | 600 | 800 | 1600 |
...
| 每个数据库的最大 DTU 数选项 | 10, 20, 50 | 10, 20, 50, 100 | 10, 20, 50, 100, 200 | 10, 20, 50, 100, 200, 300 | 10, 20, 50, 100, 200, 300, 400 | 10, 20, 50, 100, 200, 300, 400, 800 |
| 每个数据库的最大存储空间 (GB) | 500 | 750 | 1024 | 1024 | 1024 | 1024 |
||||||||

<sup>1</sup>请参阅[SQL 数据库定价选项](https://azure.microsoft.com/pricing/details/sql-database/elastic/)，详细了解由于预配的任何额外存储导致的额外成本。

<sup>2</sup>有关更多注意事项，请参阅[密集弹性池中的资源管理](elastic-pool-resource-management.md)。

<sup>3</sup>有关每个数据库的最大并发工作线程数（请求数），请参阅[单一数据库资源限制](resource-limits-vcore-single-databases.md)。 例如，如果弹性池使用 Gen5 且每个数据库的最大 vCore 数设置为 2，则最大并发辅助角色数目值为 200。  如果每个数据库的最大 vCore 数设置为 0.5，则最大并发辅助角色数目值为 50，因为 Gen5 上每个 vCore 的最大并发辅助角色数为 100。 对于每个数据库的最大 vCore 设置小于 1 个 vCore 或更少的其他情况，最大并发辅助角色数会相应重新缩放。
...
| 每个池的最大内存中 OLTP 存储 (GB) | 空值 | N/A | N/A | N/A | N/A |
| 每个池的最大数据库数<sup>2</sup> | 500 | 500 | 500 | 500 | 500 |
| 每个池的最大并发工作线程数（请求数） <sup>3</sup> | 2400 | 3200 | 4000 | 5000 | 6000 |
...
| 每个数据库的最大 DTU 数选项 | 10, 20, 50, 100, 200, 300, 400, 800, 1200 | 10, 20, 50, 100, 200, 300, 400, 800, 1200, 1600 | 10, 20, 50, 100, 200, 300, 400, 800, 1200, 1600, 2000 | 10, 20, 50, 100, 200, 300, 400, 800, 1200, 1600, 2000, 2500 | 10, 20, 50, 100, 200, 300, 400, 800, 1200, 1600, 2000, 2500, 3000 |
...
| 每个池的最大并发辅助进程数（请求） <sup>3</sup> | 200 | 400 | 800 | 1600 | 2400 |
| 每个池的最大并发会话数<sup>3</sup> | 30000 | 30000 | 30000 | 30000 | 30000 |
...
| 每个池的最大并发工作线程数（请求数） <sup>3</sup> | 3200 | 4000 | 4800 | 5600 | 6400 |
| 每个池的最大并发会话数<sup>3</sup> | 30000 | 30000 | 30000 | 30000 | 30000 |
...
| 每个数据库的最大 DTU 数选项 | 25, 50, 75, 125, 250, 500, 1000, 1750 | 25, 50, 75, 125, 250, 500, 1000, 1750 | 25, 50, 75, 125, 250, 500, 1000, 1750 | 25, 50, 75, 125, 250, 500, 1000, 1750 | 25, 50, 75, 125, 250, 500, 1000, 1750, 4000 |
| 每个数据库的最大存储空间 (GB) | 1024 | 1024 | 1024 | 1024 | 1024 |
|||||||
