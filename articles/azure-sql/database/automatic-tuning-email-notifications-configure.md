...
1. 在 "Runbook" 窗格中，键入 runbook 的名称（在本示例中，使用 "**AutomaticTuningEmailAutomation**"），选择 runbook 类型作为**PowerShell**并写入此 runbook 的说明以描述其用途。
...
1. 在 "**编辑 PowerShell Runbook**" 窗格内的菜单树中选择 "**runbook**"，然后展开视图，直到看到 Runbook 的名称（在此示例中为 "**AutomaticTuningEmailAutomation**"）。 选择此 runbook。
1. 在 "编辑 PowerShell Runbook" （从数字1开始）的第一行中，复制并粘贴以下 PowerShell 脚本代码。 此 PowerShell 脚本按原样提供，可帮助你入门。 修改脚本以满足需求。
...
2. 创建操作以从已执行的 PowerShell 脚本检索输出
   ...
   - 填充所需字段（类似于创建以前的作业）-填充 Azure 订阅 ID、资源组和自动化帐户（在 "自动化帐户" 窗格中输入）。
...
> [!TIP]
> 要将自动电子邮件发送给不同的收件人，请创建单独的流。 在这些附加的流中，更改 "收件人" 字段中的收件人电子邮件地址和 "主题" 字段中的电子邮件主题行。 在 Azure 自动化中，通过自定义 PowerShell 脚本（例如更改 Azure 订阅 ID）创建新的 runbook 可以进一步自定义自动方案，例如，通过电子邮件将单独的收件人发送给单独订阅的自动优化建议。
