...
description: 本文讨论了 Azure SQL 数据库和 Azure SQL 托管实例使用的基于 vCore 的购买模型中的常规用途和业务关键服务层。
...
ms.locfileid: "85986597"
---
# <a name="azure-sql-database-and-azure-sql-managed-instance-service-tiers"></a>Azure SQL 数据库和 Azure SQL 托管实例服务层
[!INCLUDE[appliesto-sqldb-sqlmi](../includes/appliesto-sqldb-sqlmi.md)]

Azure SQL 数据库和 Azure SQL 托管实例基于 SQL Server 数据库引擎体系结构，该体系结构针对云环境进行了调整，以确保99.99% 的可用性，即使存在基础结构故障，也是如此。 两个服务层由 Azure SQL 数据库和 Azure SQL 托管实例使用，每个层都有不同的体系结构模型。 这些服务层级为：

- [常规用途](service-tier-general-purpose.md)，设计用于预算导向型工作负荷。
- [业务关键](service-tier-business-critical.md)，适用于具有高延迟的低延迟工作负荷，并可进行故障恢复和快速故障转移。

Azure SQL 数据库具有额外的服务层： 
...
下表介绍了最新一代（第 5 代）服务层级之间的主要差异。 请注意，SQL 数据库和 SQL 托管实例中的服务层特征可能有所不同。
...
| | SQL 托管实例池 | 2，4，8，16，24，32，40，64，80 Vcore | 不适用 | N/A |
...
| | SQL 托管实例  | [每个 vCore 24 GB](../managed-instance/resource-limits.md#service-tier-characteristics) | 不适用 | 最大 4 TB - [受存储大小限制](../managed-instance/resource-limits.md#service-tier-characteristics) |
...
| | SQL 托管实例 | [每个 vCore 3 MB/秒（最大 22 MB/秒）](../managed-instance/resource-limits.md#service-tier-characteristics) | 不适用 | [每个 vCore 4 MB/秒（最大 48 MB/秒）](../managed-instance/resource-limits.md#service-tier-characteristics) |
|**可用性**|All| 99.99% |  [99.95%（具有一个次要副本），99.99%（具有更多副本）](service-tier-hyperscale-frequently-asked-questions-faq.md#what-slas-are-provided-for-a-hyperscale-database) | 99.99% <br/> [99.995%（具有区域冗余单一数据库）](https://azure.microsoft.com/blog/understanding-and-leveraging-azure-sql-database-sla/) |
...
有关详细信息，请参阅 Azure SQL 数据库中的服务层[（vCore）](resource-limits-vcore-single-databases.md)、[单个 azure SQL 数据库（dtu）](resource-limits-dtu-single-databases.md)、[共用 azure sql 数据库（dtu）](resource-limits-dtu-single-databases.md)和[azure sql 托管实例](../managed-instance/resource-limits.md)页面之间的详细区别。
...
以下因素会影响用于数据文件和日志文件的存储量，并适用于常规用途和业务关键。 有关“超大规模”服务层级中的数据和日志存储的详细信息，请参阅[“超大规模”服务层级](service-tier-hyperscale.md)。
...
- [针对 AZURE SQL 数据库中的共用数据库的基于 vCore 的资源限制](resource-limits-vcore-elastic-pools.md)。
