---
title: 处理暂时性错误
description: 了解如何在连接到 Azure SQL 数据库、Azure SQL 托管实例和 Azure Synapse Analytics 时排查、诊断和防止 SQL 连接错误或暂时性错误。
...
本文介绍如何防止、排查、诊断和减少客户端应用程序在与 Azure SQL 数据库、Azure SQL 托管实例和 Azure Synapse Analytics 交互时遇到的连接错误和暂时性错误。 了解如何配置重试逻辑、生成连接字符串以及调整其他连接设置。

<a id="i-transient-faults" name="i-transient-faults"></a>

## <a name="transient-errors-transient-faults"></a>暂时性错误（暂时性故障）

暂时性错误（也称为暂时性故障）的根本原因很快就能自行解决。 当 Azure 系统快速地将硬件资源转移到负载均衡更好的各种工作负荷时，偶尔会发生暂时性错误。 其中的大多数重新配置事件在 60 秒内就能完成。 在此重新配置时间范围内，在 SQL 数据库中连接到数据库时可能会遇到问题。 应该构建连接到您的数据库的应用程序，以预见这些暂时性错误。 为了处理这些错误，可应用程序代码中实现重试逻辑，而不是以应用程序错误的形式呈现给用户。
...
请重试 SQL 数据库和 SQL 托管实例连接或重新建立连接，具体取决于以下各项：
...
在偶尔会遇到暂时性错误的客户端程序中包含重试逻辑可以让它变得更稳健。 当程序通过第三方中间件与 SQL 数据库中的数据库进行通信时，请向供应商询问中间件是否包含暂时性错误的重试逻辑。
...
- 当 SQL 数据库或 SQL 托管实例 `UPDATE` 语句由于暂时性错误而失败时，请先建立新的连接，然后重试更新。 重试逻辑必须确保整个数据库事务完成，或整个事务已回滚。
...
有关使用 ADO.NET 的客户端的阻塞期的讨论，请参阅[连接池（ADO.NET）](https://msdn.microsoft.com/library/8xx3tyca.aspx)。
...
- [通过 ADO.NET 将弹性连接到 Azure SQL][step-4-connect-resiliently-to-sql-with-ado-net-a78n]
- [将弹性连接到 Azure SQL with PHP][step-4-connect-resiliently-to-sql-with-php-p42h]
...
如果你的客户端程序通过使用 .NET Framework 类**SqlClient**连接到 SQL 数据库中的数据库，请使用 .net 4.6.1 或更高版本（或 .net Core），以便你可以使用其连接重试功能。 有关此功能的详细信息，请参阅[此网页](https://docs.microsoft.com/dotnet/api/system.data.sqlclient.sqlconnection)。
...
- **ConnectRetryCount**： &nbsp; &nbsp; 默认值为1。 范围为 0 到 255。
- **ConnectRetryInterval**： &nbsp; &nbsp; 默认值为10秒。 范围为 1 到 60。
- **连接超时**： &nbsp; &nbsp; 默认值为15秒。 范围为 0 到 2147483647。
...
## <a name="connections-to-your-database-in-sql-database"></a>连接到 SQL 数据库中的数据库

<a id="c-connection-string" name="c-connection-string"></a>

### <a name="connection-connection-string"></a>连接：连接字符串

连接到数据库所需的连接字符串与用于连接 SQL Server 的字符串稍有不同。 可以从[Azure 门户](https://portal.azure.com/)复制数据库的连接字符串。
...
必须将 SQL 数据库配置为接受来自托管客户端程序的计算机的 IP 地址的通信。 若要设置此配置，可以通过 [Azure 门户](https://portal.azure.com/)编辑防火墙设置。
...
1. 打开控制面板。
2. 选择 **"所有控制面板项" "**  >  **Windows 防火墙**  >  **高级设置**" "  >  **出站规则**  >  **操作**"  >  **新规则**。
...
- 将立即重试 Azure SQL 的连接打开尝试，从而提高已启用云的应用的性能。
...
如果程序无法连接到 SQL 数据库中的数据库，则可以使用一个诊断选项来尝试使用实用程序进行连接。 理想情况下，该实用工具将使用程序所用的同一个库进行连接。
...
在 Windows 上， [PortQry.exe](https://www.microsoft.com/download/details.aspx?id=17148)实用程序可能会很有用。 下面是查询 SQL 数据库中的数据库上的端口情况并在便携式计算机上运行的示例执行：
...
Enterprise Library 6 （EntLib60）是 .NET 类的框架，可帮助你实现云服务的可靠客户端，其中一项是 SQL 数据库。 若要查找 EntLib60 可以提供帮助的各个领域的相关专题，请参阅 [Enterprise Library 6 - 2013 年 4 月](https://msdn.microsoft.com/library/dn169621%28v=pandp.60%29.aspx)。
...
在**包含 microsoft.practices.enterpriselibrary.transientfaulthandling. microsoft.practices.enterpriselibrary.transientfaulthandling.testsupport**中：
...
- 免费书籍下载： [Microsoft Enterprise Library 第2版开发人员指南](https://www.microsoft.com/download/details.aspx?id=41145)。
- 最佳实践：[有关重试的一般性指南](/azure/architecture/best-practices/transient-faults)深入探讨了重试逻辑。
...
- [连接池（ADO.NET）](https://docs.microsoft.com/dotnet/framework/data/adonet/sql-server-connection-pooling)
- [*重试*是 Apache 2.0 许可的通用重试库，它以 Python 编写，](https://pypi.python.org/pypi/retrying)可以简化将重试行为添加到几乎所有操作的任务。
