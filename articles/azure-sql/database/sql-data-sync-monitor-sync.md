...
description: 了解如何使用 Azure Monitor 日志监视 SQL 数据同步。
...
您不再需要单独浏览每个同步组的日志来查找问题。 可以通过使用自定义 Azure Monitor 视图在一个位置监视所有订阅中的所有同步组。 此视图显示对 SQL 数据同步客户很重要的信息。

![数据同步监控仪表板](./media/sql-data-sync-monitor-sync/sync-monitoring-dashboard.png)
...
4.  在“基于以下项生成警报”中，请选择“指标度量值”********。
    ...
    2.  在“大于”**** 后，输入接收通知之前等待的阈值。 数据同步中应出现暂时性错误。若要减少干扰，请将阈值设置为5。
...
-   每个同步组的磁贴，其中显示错误数、成功数和警告数，以及最近的错误消息。
...
-   [SQL 数据库概述](../../sql-database/sql-database-technical-overview.md)
