...
在这个由三部分组成的教程系列的第二部分中，你将在 R 中构建一个 K-Means 模型来执行聚类分析。 在本系列的下一部分中，将使用 Azure SQL 数据库机器学习服务（预览版）在 Azure SQL 数据库的数据库中部署此模型。
...
在[第二部分](clustering-model-prepare-data-tutorial.md)中，我们学习了如何准备来自数据库的数据以执行聚类分析。

在[第三部分](clustering-model-deploy-tutorial.md)中，将了解如何创建可以基于新数据在 R 中执行聚类分析的存储过程。

## <a name="prerequisites"></a>先决条件

* 本教程的第二部分假定你已完成[**第一部分**](clustering-model-prepare-data-tutorial.md)及其先决条件。
...
![肘形图](./media/clustering-model-build-tutorial/elbow-graph.png)
...
使用[第一部分](clustering-model-prepare-data-tutorial.md#separate-customers)中定义的变量给出四种群集平均值：
...
如果不打算继续学习本教程，请从服务器中删除 tpcxbb_1gb 数据库。
...
> [教程：使用 Azure SQL 数据库机器学习服务（预览版）在 R 中部署聚类模型](clustering-model-deploy-tutorial.md)
