---
title: 使用智能见解排查性能问题
description: 智能见解有助于排查 Azure SQL 数据库和 Azure SQL 托管实例性能问题。
...
ms.locfileid: "85986369"
---
# <a name="troubleshoot-azure-sql-database-and-azure-sql-managed-instance-performance-issues-with-intelligent-insights"></a>排查 Azure SQL 数据库和 Azure SQL 托管实例性能问题智能见解
[!INCLUDE[appliesto-sqldb-sqlmi](../includes/appliesto-sqldb-sqlmi.md)]

本页提供有关 Azure SQL 数据库和 Azure SQL 托管实例通过[智能见解](intelligent-insights-overview.md)资源日志检测到的性能问题的信息。 可以将指标和资源日志流式传输到[Azure Monitor 日志](../../azure-monitor/insights/azure-sql.md)、 [azure 事件中心](../../azure-monitor/platform/resource-logs-stream-event-hubs.md)、 [azure 存储](metrics-diagnostic-telemetry-logging-streaming-export-configure.md#stream-into-azure-storage)或第三方解决方案，以用于自定义 DevOps 警报和报告功能。
...
智能见解会根据查询执行等待时间、错误或超时自动检测性能问题。 智能见解将检测到的性能模式输出到资源日志。 下表汇总了可检测的性能模式。

| 可检测性能模式 | Azure SQL Database | Azure SQL 托管实例 |
| :------------------- | ------------------- | ------------------- |
...
| [工作负荷增加](intelligent-insights-troubleshoot-performance.md#workload-increase) | 检测到工作负荷增大，或数据库上的工作负荷持续累积。 这会影响性能。 | 检测到工作负荷增加。 这会影响数据库性能。 |
| [内存压力](intelligent-insights-troubleshoot-performance.md#memory-pressure) | 请求内存授予的辅助角色需要等待统计大量时间的内存分配，或已增加请求内存授予的辅助角色的累计量。 这会影响性能。 | 请求内存授予的工作线程会等待内存分配相当长的时间（就统计学意义来说）。 这会影响数据库性能。 |
| [锁定](intelligent-insights-troubleshoot-performance.md#locking) | 检测到的数据库锁定过多，会影响性能。 | 检测到过度的数据库锁定，这影响数据库性能。 |
...
| [缺少索引](intelligent-insights-troubleshoot-performance.md#missing-index) | 检测到缺少索引会影响性能。 | 检测到索引缺失，这影响数据库性能。 |
| [新建查询](intelligent-insights-troubleshoot-performance.md#new-query) | 检测到新的查询影响整体性能。 | 检测到新查询，这影响数据库的总体性能。 |
| [等待时间延长统计信息](intelligent-insights-troubleshoot-performance.md#increased-wait-statistic) | 检测到的数据库等待时间增加会影响性能。 | 检测到数据库等待时间延长，这影响数据库的性能。 |
...
| [弹性池 DTU 不足](intelligent-insights-troubleshoot-performance.md#elastic-pool-dtu-shortage) | 弹性池中的可用 Edtu 不足会影响性能。 | 对于 Azure SQL 托管实例不可用，因为它使用 vCore 模型。 |
...
| [慢速客户端](intelligent-insights-troubleshoot-performance.md#slow-client) | 应用程序客户端运行缓慢，无法以足够快的速度使用数据库的输出。 这会影响性能。 | 应用程序客户端运行缓慢，无法以足够快的速度使用数据库的输出。 这会影响数据库性能。 |
...
> [!TIP]
> 对于数据库的连续性能优化，启用[自动优化](automatic-tuning-overview.md)。 此内置智能功能持续监视数据库、自动调整索引并应用查询执行计划更正。
>
...
Azure SQL 数据库中的资源通常称为[DTU](service-tiers-dtu.md)或[VCore](service-tiers-vcore.md)资源，azure sql 托管实例上的资源称为 vCore 资源。 如果检测到的查询性能下降是由于达到所度量资源限制而造成的，则认为出现了“达到资源限制”模式。

会话限制资源表示数据库的可用并发登录数。 当连接到数据库的应用程序已达到数据库的可用并发登录数时，就会识别此性能模式。 如果应用程序尝试使用的会话数超过了可在数据库中使用的数目，则会影响查询性能。

达到工作线程限制是一种特殊的达到资源限制的情况，因为可用工作线程数不会计入 DTU 或 vCore 用量。 数据库中达到工作线程限制可能会导致资源特定的等待时间延长，从而导致查询性能降低。

### <a name="troubleshooting"></a>疑难解答

诊断日志会输出影响性能和资源消耗百分比的查询的查询哈希。 可以使用此信息作为优化数据库工作负荷的入手点。 具体而言，可以通过添加索引优化那些导致性能降低的查询。 也可以让工作负荷分配更均衡，从而优化应用程序。 如果无法减少工作负荷或进行优化，请考虑增加数据库订阅的定价层，以增加可用的资源量。

如果已达到可用会话限制，可以通过减少数据库登录次数来优化应用程序。 如果无法减少从应用程序到数据库的登录数，请考虑增加数据库订阅的定价层。 也可以将数据库拆分成多个数据库并进行移动，使工作负荷的分配更为均衡。

有关解决会话限制的更多建议，请参阅[如何处理最大登录数的限制](https://blogs.technet.microsoft.com/latam/20../../how-to-deal-with-the-limits-of-azure-sql-database-maximum-logins/)。 请参阅[服务器上的资源限制概述](resource-limits-logical-server.md)，了解有关服务器和订阅级别的限制的信息。
...
由于数据库无法处理工作负荷，因此，在其更严重的窗体中，工作负荷可能会持续进行。 结果就是工作负荷不断增大，即出现工作负荷堆积情况。 因此，工作负荷等待执行的时间也增加。 这种情况代表最严重的数据库性能问题之一。 可通过监视已中止工作线程数的增加情况来检测此问题。

### <a name="troubleshooting"></a>疑难解答

诊断日志会输出执行时间已延长的查询数，以及对工作负荷增加影响最大的查询的查询哈希。 可以使用此信息作为优化工作负荷的入手点。 一开始可以确定对工作负荷增加影响最大的查询，这很有用。

可以考虑将工作负荷更均衡地分配到数据库。 考虑通过添加索引来优化影响性能的查询。 也可将工作负荷分配到多个数据库中。 如果无法解决这些解决方案，请考虑增加数据库订阅的定价层，以增加可用的资源量。
...
内存压力表示存在大量请求内存授予的工作线程的性能情况。 大容量导致内存使用率高的情况，即数据库无法有效地将内存分配给请求它的所有工作线程。 此问题的最常见原因之一与数据库的可用内存量有关。 另一方面，工作负荷增加导致工作线程数增加和内存压力增大。

内存压力增大时，更严重者会出现内存堆积的情况。 这种情况表明，请求内存授予的工作线程数超出释放内存的查询数。 请求内存授予的工作线程数还可能会持续增加（堆积），因为数据库引擎无法有效分配足够的内存来满足需求。 内存堆积的情况代表最严重的数据库性能问题之一。
...
还可以减少工作负荷，方法是对其进行优化，或者将其分配到多个数据库。 也可将工作负荷分配到多个数据库中。 如果无法使用这些解决方案，请考虑提高数据库的定价层，以增加可用于数据库的内存资源量。
...
这种可检测的性能模式表示所选查询执行计划的并行化程度超出了预期。 查询优化器可以通过并行执行查询来提高工作负荷性能，从而尽可能提高工作效率。 在某些情况下，处理查询的并行工作线程等待相互同步和合并结果所花费的时间，比使用较少的并行工作线程（有时甚至是一个工作线程）执行相同查询的时间还要长。

专家系统会对比基线期间来分析当前的数据库性能。 它会确定此前运行的某个查询是否运行得比以前更慢，因为查询执行计划的并行化程度比本来应有的程度更高。

MAXDOP 服务器配置选项用于控制并行执行相同查询的 CPU 内核数。
...
将 MAXDOP 服务器配置选项设置为零（0）作为默认值表示数据库可以使用所有可用的 CPU 内核来并行执行单个查询。 将 MAXDOP 设置为一 (1) 表示只能将一个核心用于单个查询的执行。 实际上，这意味着并行功能处于关闭状态。 根据具体的情况、数据库的可用核心数与诊断日志信息，可将 MAXDOP 选项优化成用于并行查询执行且能够解决具体问题的核心数。
...
由于 pagelatch 是一个内部控制机制，因此它会自动确定何时使用它们。 应用程序决策（包括架构设计）可以影响因闩锁的确定性行为而导致的 Pagelatch 行为。
...
> [!TIP]
> 您是否知道，内置智能可以自动管理数据库的最佳性能索引？
>
> 对于连续性能优化，建议启用[自动优化](automatic-tuning-overview.md)。 这种独特的内置智能功能持续监视数据库，并为数据库自动调整和创建索引。
>
...
在 Azure SQL 数据库中，考虑使用[Query Performance Insight](query-performance-insight-use.md)。

## <a name="increased-wait-statistic"></a>增加的等待统计信息
...
[Azure 弹性池资源](elastic-pool-overview.md)用作多个数据库之间共享的可用资源池，以实现缩放。 如果弹性池中的可用 eDTU 资源不够大，无法支持池中的所有数据库，则系统就会检测到“弹性池 DTU 不足”性能问题。
...
这种可检测的性能模式表示数据库利用不太理想的查询执行计划的情况。 欠佳的计划通常会导致查询执行时间延长，从而导致当前查询和其他查询的等待时间变长。

数据库引擎确定执行查询所需的查询执行计划的成本最低。 当查询和工作负荷的类型发生变化时，有时现有的计划不再有效，或者数据库引擎没有做出良好的评估。 作为一种纠正措施，可以手动强制查询执行计划。

这种可检测的性能模式合并了三种不同的计划回归情况：新计划回归、旧计划回归和现有计划更改的工作负荷。 ** 诊断日志的“详细信息”属性中提供了出现的计划回归的特定类型。

新计划回归条件是指数据库引擎开始执行新的查询执行计划的状态，这不如旧计划有效。 旧计划回归条件是指数据库引擎从使用新的、更高效的计划到旧计划的状态，这不如新计划有效。 现有计划更改的工作负荷回归表示这样一种状态：不断交替使用旧计划和新计划，天平逐渐倾向于性能不佳的计划。
...
> [!TIP]
> 您是否知道，内置智能功能可以自动管理数据库的最佳查询执行计划？
>
> 对于连续性能优化，建议启用[自动优化](automatic-tuning-overview.md)。 此内置智能功能持续监视数据库，并自动调整和创建数据库的最佳查询执行计划。
...
这种可检测的性能模式指示在数据库发送结果时，使用数据库的客户端无法使用数据库输出的情况。 由于数据库未将执行的查询的结果存储在缓冲区中，因此它会减缓并等待客户端使用传输的查询输出，然后再继续。 这种情况还可能与网络相关，该网络的速度不够快，无法将输出从数据库传输到使用方客户端。
...
这种可检测的性能模式表示数据库订阅的定价层已降级的情况。 由于可供数据库使用的资源 (DTU) 减少，系统检测到当前数据库性能相比过去七天的基线有所下降。

此外，可能会出现这样的情况：数据库订阅的定价层已降级，然后在较短的时间内升级到更高的层。 检测到这种暂时性的性能降低时，会将检测结果作为定价层降级和升级输出到诊断日志的 details 节中。

### <a name="troubleshooting"></a>疑难解答

如果你缩减了定价层，因此可以使用 Dtu，并且你对性能感到满意，则无需执行任何操作。 如果您缩减了定价层，并且您对数据库性能不满意，则可以减少数据库工作负荷，或考虑将定价层提升到更高的级别。
