---
title: 设置 SQL 数据同步
description: 本教程演示如何设置 Azure SQL 数据同步
...
ms.locfileid: "85963948"
---
# <a name="tutorial-set-up-sql-data-sync-between-databases-in-azure-sql-database-and-sql-server"></a>教程：在 Azure SQL 数据库中的数据库和 SQL Server 之间设置 SQL 数据同步
[!INCLUDE[appliesto-sqldb](../includes/appliesto-sqldb.md)]

在本教程中，你将了解如何通过创建包含 Azure SQL 数据库和 SQL Server 实例的同步组来设置 SQL 数据同步。 同步组进行了自定义配置，并根据设置的计划进行同步。
...
有关如何配置 SQL 数据同步的 PowerShell 示例，请参阅[如何在 SQL 数据库中的数据库之间](scripts/sql-data-sync-sync-data-between-sql-databases.md)或[在 Azure sql 数据库中的数据库之间进行同步和 SQL Server](scripts/sql-data-sync-sync-data-between-azure-onprem.md)
...
1. 请在 SQL 数据库中转到[Azure 门户](https://portal.azure.com)查找数据库。 搜索并选择“SQL 数据库”****。

    ![搜索数据库，Microsoft Azure 门户](./media/sql-data-sync-sql-server-configure/search-for-sql-databases.png)

1. 选择要用作数据同步的中心数据库的数据库。

    ![从 "数据库" 列表中选择 "Microsoft Azure 门户](./media/sql-data-sync-sql-server-configure/select-sql-database.png)
...
1. 在“同步到其他数据库”页中，选择“新建同步组”********。 “新建同步组”页随即打开，其中突出显示“创建同步组(步骤 1)”********。
   ...
   | **冲突解决** | 选择“中心胜出”或“成员胜出”********。<br/><br/>“中心胜出”表示发生冲突时，中心数据库中的数据将覆盖成员数据库中的冲突数据****。<br/><br/>“成员胜出”表示发生冲突时，成员数据库中的数据将覆盖中心数据库中的冲突数据****。 |

   > [!NOTE]
   > Microsoft 建议新建空的数据库，以用作“同步元数据数据库”****。 SQL 数据同步在此数据库中创建表，并经常运行工作负载。 此数据库作为所选区域和订阅中的所有同步组的**同步元数据数据库**共享。 如果不删除区域中的所有同步组和同步代理，则无法更改该数据库或其名称。
...
在 "**中心数据库**" 部分中，输入中心数据库所在服务器的现有凭据。 请勿在此部分中输入新** 凭据。
...
在 "**成员数据库**" 部分中，选择 "**添加 azure sql 数据库**"，将 azure sql 数据库中的数据库添加到同步组。 此时，“配置 Azure SQL 数据库”**** 页随即打开。
  ...
  | **Azure SQL 数据库** | 选择 "SQL 数据库" 中的现有数据库。 |
  | **同步方向** | 选择“双向同步”、“向中心同步”或“从中心同步”************。 |
  | **用户名**和**密码** | 输入成员数据库所在服务器的现有凭据。 请勿在此部分中输入新** 凭据。 |
...
在 "**成员数据库**" 部分中，选择 "**添加本地数据库**" （可选）将 SQL Server 数据库添加到同步组。 随即打开“配置本地”页，可以在其中执行以下操作****：
...
1. 在“选择同步代理”页中，选择是使用现有代理还是创建代理****。
   ...
   1. 通过提供的链接下载 Data Sync Agent，并将其安装在 SQL Server 所在的计算机上。 还可以直接从[Azure SQL 数据同步代理](https://www.microsoft.com/download/details.aspx?id=27693)下载代理。
...
1. 在 SQL Server 计算机上，找到并运行客户端同步代理应用程序。
   ...
   ![数据同步客户端代理应用程序](./media/sql-data-sync-sql-server-configure/datasync-preview-clientagent.png)
    ...
    1. 在“同步元数据数据库配置”**** 对话框中，粘贴从 Azure 门户复制的代理密钥。 还提供元数据数据库所在的服务器的现有凭据。 （如果创建了元数据数据库，则此数据库与中心数据库位于同一服务器上。）选择 **"确定"** ，并等待配置完成。
