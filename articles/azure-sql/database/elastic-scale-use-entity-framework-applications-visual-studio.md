...
创建这些数据库后，使用服务器名称、数据库名称和凭据填充**Program.cs**中的占位符以连接到数据库。 在 Visual Studio 中构建解决方案。 在生成过程中，Visual Studio 会下载弹性数据库客户端库、Entity Framework 和暂时性故障处理所需的 NuGet 包。 确保已为解决方案启用还原 NuGet 包。 可以通过右键单击 Visual Studio 解决方案资源管理器中的解决方案文件来启用此设置。
...
使用实体框架的数据库连接通常通过 **DbContext** 的子类来管理。 通过从 **DbContext** 派生创建这些子类。 这是你定义 **DbSets** 的位置，它可为应用程序实现支持数据库的 CLR 对象的集合。 在数据依赖型路由的上下文中，可以标识多个有用的属性，这些属性不一定会为其他 EF Code First 应用程序方案保存：
...
自动架构管理是实体框架提供的一项便利。 在使用弹性数据库工具的应用程序的上下文中，会希望保留此功能以在数据库添加到分片应用程序时，将架构自动设置为新创建的分片。 主要的使用案例是增加使用 EF 的分片应用程序的数据层的容量。 依靠 EF 的架构管理功能可减少在 EF 上构建的分片应用程序的数据库管理工作。
