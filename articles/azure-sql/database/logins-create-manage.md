...
description: 了解 Azure SQL Database、SQL 托管实例和 Azure Synapse 如何使用登录名和用户帐户对用户进行身份验证。 此外，还将了解如何授予数据库角色并显式授予登录名和用户执行操作和查询数据的权限。
...
ms.locfileid: "85986277"
---
# <a name="authorize-database-access-to-sql-database-sql-managed-instance-and-azure-synapse-analytics"></a>授权数据库访问 SQL Database、SQL 托管实例和 Azure Synapse Analytics
[!INCLUDE[appliesto-sqldb-sqlmi-asa](../includes/appliesto-sqldb-sqlmi-asa.md)]

本文介绍：

- 用于配置 Azure SQL 数据库、Azure SQL 托管实例和 Azure Synapse Analytics （以前称为 Azure SQL 数据仓库）的选项，使用户能够执行管理任务并访问存储在这些数据库中的数据。
- 最初创建新服务器后的访问和授权配置。
- 如何在 master 数据库和用户帐户中添加登录名和用户帐户，然后向这些帐户授予管理权限。
- 如何在用户数据库中添加与登录名或包含的用户帐户关联的用户帐户。
- 使用数据库角色和显式权限在用户数据库中配置具有权限的用户帐户。

> [!IMPORTANT]
> Azure SQL 数据库、Azure SQL 托管实例和 Azure Synapse 中的数据库在本文的其余部分中统称为数据库，服务器引用的[服务器](logical-servers.md)管理 Azure SQL 数据库和 azure Synapse 的数据库。
...
- [Azure Active Directory 身份验证](authentication-aad-overview.md)
  ...
  使用此身份验证方法，用户提交用户帐户名称，并请求服务使用存储在 Azure Active Directory （Azure AD）中的凭据信息。

**登录名和用户**：数据库中的用户帐户可以与存储在 master 数据库中的登录名相关联，也可以是存储在单个数据库中的用户名。

- 登录名是 master 数据库中的单个帐户，一个或多个数据库中的用户帐户可以关联到该帐户****。 使用登录名时，用户帐户的凭据信息将与该登录名一起存储。
- **用户帐户**是任何数据库中的个人帐户，但不一定要链接到登录名。 使用未关联到登录名的用户帐户时，凭据信息将与该用户帐户一起存储。
...
首次部署 Azure SQL 时，为该登录名指定管理员登录名和关联的密码。 此管理帐户称为“服务器管理员”。**** 在部署期间，将在 master 数据库和用户数据库中进行以下登录名和用户配置：
...
- 此登录名对所有数据库拥有完全管理权限，充当[服务器级主体](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/principals-database-engine)。 该登录名具有所有可用的权限，无法受到限制。 在 SQL 托管实例中，此登录名添加到[sysadmin 固定服务器角色](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/server-level-roles)中（此角色在 Azure SQL 数据库中不存在）。
...
> [!IMPORTANT]
> 在创建管理员登录名后，不能对其进行更改。 若要重置服务器管理员的密码，请转到 [Azure 门户](https://portal.azure.com)，单击“SQL Server”，从列表中选择服务器，然后单击“重置密码”********。 若要为 SQL 托管实例重置密码，请前往 Azure 门户，单击该实例，然后单击 "**重置密码**"。 也可以使用 PowerShell 或 Azure CLI。

## <a name="create-additional-logins-and-users-having-administrative-permissions"></a>创建拥有管理权限的其他登录名和用户

此时，你的服务器或托管实例仅配置为使用单个 SQL 登录名和用户帐户进行访问。 若要创建其他拥有完全管理权限或部分管理权限的登录名，可使用以下选项（取决于部署模式）：

- **创建具有完全管理权限的 Azure Active Directory 管理员帐户**

  启用 Azure Active Directory 身份验证并创建 Azure AD 管理员登录名。 可以使用完全管理权限将一个 Azure Active Directory 帐户配置为 Azure SQL 部署的管理员。 此帐户可以是个人帐户或安全组帐户。 如果要使用 Azure AD 帐户连接到 SQL 数据库、SQL 托管实例或 Azure Synapse，则**必须**配置一个 Azure AD 管理员。 有关为所有 Azure SQL 部署类型启用 Azure AD 身份验证的详细信息，请参阅以下文章：

  - [将 Azure Active Directory 身份验证用于使用 SQL 进行身份验证](authentication-aad-overview.md)
  - [使用 SQL 配置和管理 Azure Active Directory 身份验证](authentication-aad-configure.md)

- **在 SQL 托管实例中，创建具有完全管理权限的 SQL 登录名**

  - 在 master 数据库中创建其他 SQL 登录名。
...
- **在 SQL 数据库中，创建具有有限管理权限的 SQL 登录名**
  ...
  - 使用 ALTER ROLE 语句将用户帐户添加到 `dbmanager` `loginmanager` 数据库中的和/或角色中 `master` （对于 Azure Synapse，请使用[sp_addrolemember](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql)语句）。 [ALTER ROLE](https://docs.microsoft.com/sql/t-sql/statements/alter-role-transact-sql)

  > [!NOTE]
  > `dbmanager`和 `loginmanager` 角色与**not** SQL 托管实例部署无关。

  对于 Azure SQL 数据库，这些[特殊的 master 数据库角色](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/database-level-roles#special-roles-for--and-)的成员具有创建和管理数据库的权限，或者用于创建和管理登录名的权限。 在属于 `dbmanager` 角色的成员的用户创建的数据库中，该成员将映射到 `db_owner` 固定数据库角色，并可以使用 `dbo` 用户帐户登录到该数据库以及对其进行管理。 这些角色在 master 数据库外部没有显式权限。

  > [!IMPORTANT]
  > 不能在 SQL 数据库中创建具有完全管理权限的其他 SQL 登录名。
...
- **创建用户帐户**
  ...
  在数据库中创建用户需要访问的用户帐户（也称为[包含的用户](/sql/relational-databases/security/contained-database-users-making-your-database-portable)）。

  - 通过 SQL 数据库，您始终可以创建这种类型的用户帐户。
  - 使用支持[Azure AD 服务器主体](authentication-aad-configure.md#create-contained-users-mapped-to-azure-ad-identities)的 sql 托管实例，你可以创建用户帐户对 SQL 托管实例进行身份验证，而无需将数据库用户创建为包含的数据库用户。

  使用此方法时，用户身份验证信息将存储在每个数据库中，并自动复制到异地复制的数据库。 但是，如果同一帐户存在于多个数据库中，并且你使用的是 Azure SQL 身份验证，则必须将密码保持手动同步。 此外，如果用户在不同的数据库中具有密码不同的帐户，则可能会很难记住这些密码。

> [!IMPORTANT]
> 若要创建映射到 Azure AD 标识的包含用户，必须使用 Azure SQL 数据库中的数据库中的管理员身份登录 Azure AD 帐户。 在 SQL 托管实例中，具有权限的 SQL 登录名 `sysadmin` 还可以创建 Azure AD 登录名或用户。
...
> [!TIP]
> 有关包括在 Azure SQL 数据库中创建用户的安全教程，请参阅[教程：保护 AZURE Sql 数据库](secure-database-tutorial.md)。
...
- **固定数据库角色**
  ...
  - 若要将用户添加到固定数据库角色：
    ...
    - Azure Synapse，请使用[sp_addrolemember](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql)语句。 有关示例，请参阅 [sp_addrolemember 示例](https://docs.microsoft.com/sql/t-sql/statements/alter-role-transact-sql)。
...
- 使用 Azure Active Directory 身份验证时，请将 Azure Active Directory 用户放入 Azure Active Directory 安全组。 为该组创建包含数据库用户。 将一个或多个数据库用户作为成员添加到具有适用于该用户组的特定权限的自定义或内置数据库角色。
...
有关所有 Azure SQL 数据库和 SQL 托管实例安全功能的概述，请参阅[安全性概述](security-overview.md)。
