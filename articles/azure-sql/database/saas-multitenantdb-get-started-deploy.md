...
- 安装最新的 Azure PowerShell。 有关详细信息，请参阅[Azure PowerShell][link-azure-get-started-powershell-41q]入门。
...
- *User：* **af1**  *（其首字母缩写，加上一个数字）。如果第二次部署应用，请使用不同的值（例如 af2）。）*
- *资源组：* **wingtip-af1** *（wingtip 指示这是分片多租户应用程序。追加用户名 af1 会将资源组名称与它包含的资源的名称相关联。）*
...
1. 输入该部署所需的参数值。
    ...
    - 对于 "**资源组**"，选择 "**新建**"，然后为资源组提供一个**名称**（区分大小写）。
        - 从下拉列表中选择一个**位置**。
    - 对于“用户”，**** 建议选择短的“用户”值。****
...
脚本位于中 *。 \\Wingtipticketssaas-dbpertenant 提供了-Wingtipticketssaas-multitenantdb-主 \\ 学习模块 \\ *文件夹。
...
1. 在 Web 浏览器中打开“事件中心”****：
   - http://events.wingtip-mt.&lt; &gt; trafficmanager.net &nbsp; *（ &lt; 将 user 替换 &gt; 为部署的用户值。）*
...
- http://events.wingtip-mt.&lt; &gt; trafficmanager.net/*fabrikamjazzclub*
...
1. 在*POWERSHELL ISE*中打开 ... \\学习模块 \\ 实用程序 \\ *Demo-LoadGenerator.ps1*脚本。
...
1. 在 [Azure 门户](https://portal.azure.com)中，浏览到资源组列表。 打开你在部署应用程序时创建的资源组。
...
2. 浏览到**tenants1 &lt; &gt; 用户**服务器，并单击**salixsalsa**数据库。 可以看到这个只包含一个租户的数据库中的资源利用情况。
...
- 若要了解弹性池，请参阅：
  ...
  - [弹性池有助于管理和缩放 Azure SQL 数据库中的多个数据库](elastic-pool-overview.md)
...
现在，请尝试[预配和编录教程](saas-multitenantdb-provision-and-catalog.md)。
