...
description: 查询性能监视识别 Azure SQL 数据库中的单个和共用数据库的最 CPU 消耗和长时间运行的查询。
...
ms.locfileid: "86042877"
---
# <a name="query-performance-insight-for-azure-sql-database"></a>适用于 Azure SQL 数据库的 Query Performance Insight
[!INCLUDE[appliesto-sqldb](../includes/appliesto-sqldb.md)]

Query Performance Insight 为单一数据库和池数据库提供智能查询分析。 它有助于确定工作负荷中资源消耗最多的资源和长时间运行的查询。 这可帮助你查找要优化的查询，以提高工作负载的整体性能，并有效地使用你要支付的资源。 Query Performance Insight 提供以下功能，帮助你花费更少的时间来排查数据库性能问题：

* 深入了解数据库资源（DTU）消耗
* 按 CPU、持续时间和执行计数排名靠前的数据库查询的详细信息（性能改进的潜在优化候选项）
* 能够向下钻取查询的详细信息，以查看查询文本和资源使用情况的历史记录
* 显示[数据库顾问](database-advisor-implement-performance-recommendations.md)的性能建议的批注

![Query Performance Insight](./media/query-performance-insight-use/opening-title.png)

## <a name="prerequisites"></a>先决条件

查询性能见解要求已在数据库上启用 [Query Store](https://msdn.microsoft.com/library/dn817826.aspx)。 默认情况下，会自动为 Azure SQL 数据库中的所有数据库启用该功能。 如果查询存储未运行，Azure 门户将提示启用它。
...
2. 在左侧菜单中，打开**智能性能**  >  **Query Performance Insight**。
   ...
   ![菜单上的 Query Performance Insight](./media/query-performance-insight-use/tile.png)

3. 在第一个选项卡上，查看资源占用排名靠前的查询列表。
4. 选择单个查询以查看其详细信息。
5. 打开**智能性能**  >  **性能建议**并检查是否有任何性能建议可用。 有关内置性能建议的详细信息，请参阅[Azure SQL 数据库顾问](database-advisor-implement-performance-recommendations.md)。
6. 使用滑块或缩放图标更改观察间隔。

   ![性能仪表板](./media/query-performance-insight-use/performance.png)

> [!NOTE]
> 要使 Azure SQL Database 在 Query Performance Insight 中呈现信息，查询存储需要捕获几个小时的数据。 如果在某段时间内数据库没有活动，或查询存储不处于活动状态，则当 Query Performance Insight 显示该时间范围时，图表将为空。 如果查询存储未运行，可随时启用它。 有关详细信息，请参阅[查询存储最佳做法](https://docs.microsoft.com/sql/relational-databases/performance/best-practice-with-the-query-store)。
>

如需数据库性能建议，请在 Query Performance Insight 导航边栏选项卡中选择 [建议](database-advisor-implement-performance-recommendations.md)。
...
1. 使用复选框选择或清除单个查询，以在图表中包含或排除它们。
   ...
   ![排名靠前的查询](./media/query-performance-insight-use/top-queries.png)
...
1. 选择排位靠前查询列表中的任意查询。
    ...
    ![排名靠前查询列表](./media/query-performance-insight-use/details.png)
...
例如，许多数据驱动的网站为每个用户请求大量访问数据库。 虽然连接池有助于提高服务器的网络流量和处理负载，但会降低性能。 一般而言，请将往返保持在最低限度。
...
这些图标是注释。 它们显示[Azure SQL 数据库顾问](database-advisor-implement-performance-recommendations.md)的性能建议。 通过将鼠标悬停在注释上，可以获得有关性能建议的摘要信息。
