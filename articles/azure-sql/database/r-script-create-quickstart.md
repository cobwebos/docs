...
- [Azure SQL 数据库中的数据库](single-database-create-quickstart.md)，使用[服务器级防火墙规则](firewall-create-server-level-portal-quickstart.md)
...
1. 使用 `SELECT` 语句来查询表。
    ...
    **结果**
    ...
    ![RTestData 表的内容](./media/r-script-create-quickstart/select-rtestdata.png)
...
1. 运行以下 R 脚本。 它使用 `SELECT` 语句从表中检索数据，通过 R 运行时传递数据，并以数据帧的形式返回数据。 `WITH RESULT SETS` 子句定义 SQL 数据库的返回数据表的架构，添加列名 *NewColName*。
    ...
    **结果**
    ...
    ![从表返回数据的 R 脚本的输出](./media/r-script-create-quickstart/r-output-rtestdata.png)
...
1. 还可以仅使用没有输入数据的 R 脚本（`@input_data_1` 设置为空白）生成值。
    ...
    **结果**
    ...
    ![使用 @script 作为输入的查询结果](./media/r-script-create-quickstart/r-data-generated-output.png)
...
![R 中安装的包](./media/r-script-create-quickstart/r-installed-packages.png)
...
> [通过 Azure SQL 数据库机器学习服务（预览版）使用 R 创建并训练预测模型](r-train-score-model-create-quickstart.md)

有关使用 R 的 Azure SQL 数据库机器学习服务（预览版）的详细信息，请参阅以下文章。

- [使用 R 的 Azure SQL 数据库机器学习服务（预览版）](machine-learning-services-overview.md)
- [使用机器学习服务（预览版）在 Azure SQL 数据库中编写高级 R 函数](machine-learning-services-functions.md)
- [在 Azure SQL 数据库机器学习服务中使用 R 和 SQL 数据（预览版）](machine-learning-services-data-issues.md)
