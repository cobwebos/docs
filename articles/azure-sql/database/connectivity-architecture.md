...
description: 本文档介绍 azure 中的数据库连接的 Azure SQL 数据库连接体系结构，以及 azure 之外的 azure SQL 数据库连接体系结构。
...
ms.locfileid: "85833666"
---
# <a name="azure-sql-database-and-azure-synapse-analytics-connectivity-architecture"></a>Azure SQL 数据库和 Azure Synapse 分析连接体系结构
[!INCLUDE[appliesto-sqldb-asa](../includes/appliesto-sqldb-asa.md)]

本文介绍了将网络流量定向到 Azure SQL 数据库或 Azure Synapse Analytics 中的服务器的各种组件的体系结构。 它还介绍了不同的连接策略，以及这些策略如何影响从 Azure 内部连接的客户端以及从 Azure 外部连接的客户端。
...
以下步骤描述了如何建立到 Azure SQL 数据库的连接：

- 客户端连接到网关，后者使用公共 IP 地址并侦听端口 1433。
- 该网关根据有效的连接策略将流量重定向或代理到适当的数据库群集。
- 在数据库群集中，会将流量转发到相应的数据库。

## <a name="connection-policy"></a>连接策略

SQL 数据库和 Azure Synapse 中的服务器支持服务器的连接策略设置的以下三个选项：
...
- **代理：** 在此模式下，所有连接都通过 Azure SQL 数据库网关进行代理，从而提高延迟并降低吞吐量。 若要通过连接来使用此模式，客户端需满足以下条件：在端口 1433 上允许从客户端到 Azure SQL 数据库网关 IP 地址的出站通信。
...
我们强烈建议使用 `Redirect` 连接策略而不要使用 `Proxy` 连接策略，以最大程度地降低延迟和提高吞吐量。 但是，你将需要满足如上所述允许网络流量的其他要求。 如果客户端为 Azure 虚拟机，则可将网络安全组 (NSG) 与[服务标记](../../virtual-network/security-overview.md#service-tags)配合使用来实现它。 如果客户端从本地工作站进行连接，则可能需要联系网络管理员，让其允许网络流量通过公司防火墙。

## <a name="connectivity-from-within-azure"></a>从 Azure 内连接

如果从 Azure 内部连接，则连接默认具有 `Redirect` 连接策略。 策略是 `Redirect` 指在 TCP 会话建立到 AZURE Sql 数据库之后，客户端会话将被重定向到正确的数据库群集，并将来自 AZURE SQL 数据库网关的目标虚拟 IP 更改为该群集的目标虚拟 IP。 此后，所有后续数据包绕过 Azure SQL 数据库网关，直接传输到群集。 下图演示了此流量流。
...
下表按区域列出了网关的 IP 地址。 若要连接到 SQL 数据库或 Azure Synapse，需要在区域的**所有**网关之间允许网络流量。
...
| 美国西部 2            | 13.66.226.202, 40.78.240.8  |
|                      |                    |

## <a name="next-steps"></a>后续步骤

- 有关如何为服务器更改 Azure SQL 数据库连接策略的信息，请参阅[conn-policy](https://docs.microsoft.com/cli/azure/sql/server/conn-policy)。
