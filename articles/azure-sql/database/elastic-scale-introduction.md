...
* [弹性事务](elastic-transactions-overview.md)：此功能允许您运行跨多个数据库的事务。 弹性数据库事务适用于使用 ADO .NET 的 .NET 应用程序，并且与你熟悉的使用 [System.Transaction](https://msdn.microsoft.com/library/system.transactions.aspx)类的编程体验相集成。

下图显示了一种体系结构，它包含与数据库集合有关的弹性数据库功能  。

在此图中，数据库颜色表示架构。 颜色相同的数据库具有相同的架构。

1. 使用分片体系结构在 Azure 上托管一组**SQL 数据库**。
2. **弹性数据库客户端库** 用于管理分片集。
...
传统上，此类方案的工作方式是通过在较大规模的服务器中投资来支持应用程序。 但是，在预定义商品硬件上执行所有处理的云中，此选项受限制。 将数据和处理负载分散到许多结构相同的数据库（一种称为“分片”的向外缩放模式），无论在成本还是弹性上，都是超越传统向上缩放方法的另一种选择。
