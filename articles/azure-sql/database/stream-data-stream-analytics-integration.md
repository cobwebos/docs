...
ms.locfileid: "84345353"
---
# <a name="stream-data-into-azure-sql-database-using-azure-stream-analytics-integration-preview"></a>使用 Azure 流分析集成（预览版）将数据流式传输到 Azure SQL 数据库
...
> [!IMPORTANT]
> Azure 流分析作业可以输出到 Azure SQL 数据库、Azure SQL 托管实例或 Azure Synapse Analytics （以前称为 Azure SQL 数据仓库）。 有关详细信息，请参阅[输出](../../stream-analytics/stream-analytics-define-outputs.md#sql-database)。
...
1. 登录到 Azure 门户。
2. 导航到要在其中引入流式传输数据的数据库。 选择 "**流分析（预览版）**"。
...
3. 若要开始将流式传输数据引入到此数据库中，请选择 "**创建**" 并为流式处理作业指定一个名称，然后选择 "**下一步：输入**"。
    ...
    ![创建流分析作业](./media/stream-data-stream-analytics-integration/create-job.png)
...
4. 输入事件源详细信息，然后选择 "**下一步：输出**"。
   ...
   - **事件中心使用者组**（默认为新建）：指定使用者组名称  
     ...
     ![创建流分析作业](./media/stream-data-stream-analytics-integration/create-job-output.png)
