...
description: 了解如何通过使用动态管理视图监视 Microsoft Azure SQL 数据库和 Azure SQL 托管实例来检测和诊断常见性能问题。
...
有关动态管理视图的详细信息，请参阅[动态管理视图和函数（transact-sql）](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/system-dynamic-management-views)。
...
在 SQL Server 和 Azure SQL 托管实例的实例中，动态管理视图返回服务器状态信息。 在 Azure SQL 数据库中，只返回与当前逻辑数据库相关的信息。
...
使用以下查询来识别数据和日志 IO 用量。 如果数据或日志 IO 大于80%，则意味着用户已使用 SQL 数据库服务层的可用 IO。
...
### <a name="identify-high-memory-consuming-statements"></a>标识消耗大量内存的语句
...
你可以使用[SQL 数据库 Query Performance Insight](query-performance-insight-use.md)来监视 Azure SQL 数据库资源使用情况。 对于 Azure SQL 数据库和 Azure SQL 托管实例，可以使用[查询存储](https://msdn.microsoft.com/library/dn817826.aspx)进行监视。
...
可以使用[sys. server_resource_stats](/sql/relational-databases/system-catalog-views/sys-server-resource-stats-azure-sql-database)返回 Azure SQL 托管实例的 CPU 使用情况、IO 和存储数据。 在五分钟的间隔内收集和聚合数据。 每15秒报告一次。 返回的数据包括 CPU 使用率、存储大小、IO 利用率和托管实例 SKU。 历史数据保留大约 14 天。
...
**Master**数据库中的[sys.databases resource_stats](https://msdn.microsoft.com/library/dn269979.aspx)视图提供了额外的信息，可帮助您监视数据库在其特定服务层和计算大小方面的性能。 每 5 分钟收集一次数据，并且会保留大约 14 天。 此视图对于数据库使用资源的方式的长期历史分析非常有用。

下图显示一周内每小时的 P2 计算大小高级数据库的 CPU 资源使用情况。 此图形从周一开始，显示 5 个工作日，并显示周末（应用程序上很少发生的情况）。

![数据库资源使用](./media/monitoring-with-dmvs/sql_db_resource_utilization.png)
...
数据库引擎在每个服务器中的**master**数据库的**sys.databases resource_stats**视图中公开每个活动数据库的使用的资源信息。 表中的数据以 5 分钟为间隔收集而得。 对于“基本”、“标准”和“高级”服务层级，数据可能需要再耗费 5 分钟才会出现在表中，以使此数据更有利于历史分析而非接近实时的分析。 查询 **sys.resource_stats** 视图，以查看数据库的最近历史记录和验证你选择的预留是否提供了所需的性能。

> [!NOTE]
> 在 Azure SQL 数据库上，必须连接到**master**数据库才能在以下示例中查询**sys.databases resource_stats。**
...
下一个示例演示了使用**sys. resource_stats**目录视图获取有关数据库如何使用资源的信息的不同方法：
...
3. 使用每个资源指标的平均值和最大值信息，可以评估工作负荷与所选计算大小的适合程度。 通常情况下，**sys.resource_stats** 中的平均值提供了一个用于目标大小的良好基准。 它应该是主要测量标杆。 例如，你可能正在使用 S2 计算大小的“标准”服务层级。 CPU 和 IO 读写的平均使用百分比低于 40%，平均辅助角色数低于 50，平均会话数低于 200。 工作负荷可能适合 S1 计算大小。 很容易查看数据库是否在辅助角色和会话限制内。 若要查看数据库是否适合 CPU 和读写数等更小的计算大小，请将更小计算大小的 DTU 数除以当前计算大小的 DTU 数，并将结果乘以 100：
   ...
   | 24.5 |100.00 |
    ...
    如果对于三个资源维度中的任何一个维度，此查询返回的值小于99.9%，请考虑转到下一个更高的计算大小，或使用应用程序优化技术来减少数据库上的负载。

4. 本练习还应将未来预计的工作负荷增加考虑在内。

对于弹性池，可以使用本节中所述的方法来监视池中的单个数据库。 但你还可以在总体上监视池。 有关信息，请参阅[监视和管理弹性池](elastic-pool-overview.md)。

### <a name="maximum-concurrent-requests"></a>最大并发请求数

若要查看并发请求数，请在你的数据库上运行以下 Transact-sql 查询：
...
若要分析 SQL Server 数据库的工作负荷，请修改此查询以筛选要分析的特定数据库。 例如，如果有一个名为 MyDatabase 的本地数据库，此 TRANSACT-SQL 查询返回该数据库中的并发请求计数：
...
若要查看当前的活动会话数，请在您的数据库上运行以下 Transact-sql 查询：
...
如果要分析 SQL Server 工作负荷，请将查询修改为集中于特定数据库。 如果正在考虑将数据库移到 Azure，此查询可帮助你确定数据库可能的会话需求。
...
可以通过查询[sys.databases resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database)查看和查看**active_session_count**列来获取会话的历史统计信息。
...
缓慢或长时间运行的查询会造成过多的资源消耗并会导致查询受阻。 受阻的原因可能是应用程序设计欠佳、查询计划不良、缺乏有用的索引等。 您可以使用 "sys. dm_tran_locks" 视图获取数据库中当前锁定活动的相关信息。 有关代码示例，请参阅[sys. dm_tran_locks （transact-sql）](https://msdn.microsoft.com/library/ms190345.aspx)。
