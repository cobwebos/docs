...
- **灵活性**
  ...
  应用程序需要在 Azure SQL 数据库中的单个数据库限制之外灵活地进行扩展。 根据需要使用该工具将数据移到新的数据库，同时保留完整性。
...
- **合并以实现缩减**
  ...
  由于业务的季节性，需要缩减容量。 当业务减少时，使用该工具可减少到更少的缩放单元。 弹性缩放拆分/合并服务中的 "合并" 功能涵盖了这一要求。
...
- **分片映射**
  ...
  请求参数的下一部分包含有关分片映射和托管分片映射的数据库的信息。 具体而言，需要提供托管 shardmap 的服务器和数据库的名称、用于连接到分片映射数据库的凭据以及分片映射的名称。 当前，该操作仅接受一个凭据集。 这些凭据需要具有足够的权限，才能对分片映射和分片上的用户数据执行更改。
...
- **目标分片（拆分）**
  ...
  在你提供了有关拆分操作的源的信息后，你需要通过提供目标的服务器和数据库名称来定义数据复制的目标位置。
...
一般而言，更好的性能将从更高、更高性能的服务层。 为更高服务层级分配更高的 IO、CPU 和内存有利于拆分/合并服务在使用的批量复制和删除操作。 因此，在定义的有限时间段内仅为这些数据库提高服务层级。
