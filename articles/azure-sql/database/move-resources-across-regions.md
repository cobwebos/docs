...
ms.locfileid: "85982136"
---
# <a name="move-resources-to-new-region---azure-sql-database--azure-sql-managed-instance"></a>将资源移到新区域-Azure SQL 数据库 & Azure SQL 托管实例
[!INCLUDE[appliesto-sqldb-sqlmi](../includes/appliesto-sqldb-sqlmi.md)]

本文介绍如何将数据库或托管实例移到新区域的一般工作流。

## <a name="overview"></a>概述

在各种情况下，你希望将现有数据库或托管实例从一个区域移到另一个区域。 例如，你要将业务扩展到新的区域，并想要为新的客户群优化。 或者出于合规性原因，需要将运营部门转移到另一个区域。 或 Azure 发布了一个新区域，它可提供更好的邻近性并改善客户体验。  
...
1. 为每个源服务器创建目标服务器。
1. 使用[PowerShell](scripts/create-and-configure-database-powershell.md)配置具有适当例外的防火墙。  
1. 配置具有正确登录名的服务器。 如果你不是订阅管理员或 SQL 服务器管理员，请让管理员为你分配所需的权限。 有关详细信息，请参阅[灾难恢复后如何管理 AZURE SQL 数据库安全性](active-geo-replication-security-configure.md)。
1. 如果使用透明数据加密对数据库进行加密，并在 Azure Key Vault 中使用自己的加密密钥，请确保在目标区域中预配正确的加密材料。 有关详细信息，请参阅[Azure Key Vault 中客户托管密钥的 AZURE SQL 透明数据加密](transparent-data-encryption-byok-overview.md)。
1. 如果启用了数据库级审核，请将其禁用，并改为启用服务器级审核。 故障转移后，数据库级审核需要跨区域流量，这在移动后不是所需要的或可能的。
1. 对于服务器级审核，请确保：
   - 已将包含现有审核日志的存储容器、Log Analytics 或事件中心移到目标区域。
   - 已在目标服务器上配置审核。 有关详细信息，请参阅[SQL 数据库审核入门](../../azure-sql/database/auditing-overview.md)。
1. 如果实例具有长期保留策略 (LTR)，则现有的 LTR 备份将与当前服务器保持关联。 由于目标服务器不同，因此你可以通过使用源服务器访问源区域中的较旧的 LTR 备份，即使服务器已删除也是如此。
...
1. 在源服务器与目标服务器之间创建[故障转移组](failover-group-add-single-database-tutorial.md#2---create-the-failover-group)。  
...
- **ReplicationState = 0** （种子设定）指示数据库尚未设定种子，尝试进行故障转移会失败。

### <a name="test-synchronization"></a>测试同步

**ReplicationState**之后 `2` ，使用辅助终结点连接到每个数据库或数据库的子集， `<fog-name>.secondary.database.windows.net` 并对数据库执行任何查询，以确保连接性、正确的安全配置和数据复制。

### <a name="initiate-the-move"></a>启动移动

1. 使用辅助终结点 `<fog-name>.secondary.database.windows.net` 连接到目标服务器。
1. 在完全同步的情况下，使用 [Switch-AzSqlDatabaseFailoverGroup](/powershell/module/az.sql/switch-azsqldatabasefailovergroup) 将辅助托管实例切换为主托管实例。 此操作将成功，或将回滚。
1. 使用 `nslook up <fog-name>.secondary.database.windows.net` 验证该命令是否已成功完成，确定 DNS CNAME 条目指向目标区域的 IP 地址。 如果 switch 命令失败，则不会更新 CNAME。
...
1. 删除密钥保管库、审核存储容器、事件中心、Azure Active Directory （Azure AD）实例和其他依赖资源，以停止对这些资源进行计费。
...
1. 配置具有正确登录名的服务器。 如果你不是订阅管理员或服务器管理员，请与管理员联系，以分配所需的权限。 有关详细信息，请参阅[灾难恢复后如何管理 AZURE SQL 数据库安全性](active-geo-replication-security-configure.md)。
1. 如果使用透明数据加密对数据库进行加密，并在 Azure Key Vault 中使用自己的加密密钥，请确保在目标区域中预配正确的加密材料。
...
1. 如果实例具有长期保留策略 (LTR)，则现有的 LTR 备份将与当前服务器保持关联。 由于目标服务器不同，因此你将能够使用源服务器访问源区域中的较旧的 LTR 备份，即使服务器已被删除也是如此。
...
1. 在源服务器上的每个弹性池与目标服务器上的对应弹性池之间创建一个单独的[故障转移组](failover-group-add-elastic-pool-tutorial.md#3---create-the-failover-group)。
1. 将池中的所有数据库添加到该故障转移组中。

    系统会自动启动已添加的数据库的复制。 有关详细信息，请参阅[包含弹性池的故障转移组的最佳实践](auto-failover-group-overview.md#best-practices-for-sql-database)。
...
**ReplicationState**为后 `2` ，使用辅助终结点连接到每个数据库或数据库子集， `<fog-name>.secondary.database.windows.net` 并对数据库执行任何查询，以确保连接性、正确的安全配置和数据复制。
...
1. 使用 [Remove-AzSqlServer](/powershell/module/az.sql/remove-azsqlserver) 删除源服务器。
1. 删除密钥保管库、审核存储容器、事件中心、Azure AD 实例以及其他相关资源，以停止对这些资源进行计费。

## <a name="move-a-managed-instance"></a>移动托管实例

### <a name="verify-prerequisites"></a>验证先决条件

1. 对于每个源托管实例，请在目标区域中创建一个相同大小的 SQL 托管实例目标实例。  
1. 为托管实例配置网络。 有关详细信息，请参阅[网络配置](../managed-instance/how-to-content-reference-guide.md#network-configuration)。
1. 使用正确的登录名配置目标 master 数据库。 如果你不是订阅或 SQL 托管实例管理员，请与管理员联系，以分配所需的权限。
1. 如果使用透明数据加密对数据库进行加密，并在 Azure Key Vault 中使用自己的加密密钥，请确保源区域和目标区域中都存在具有相同加密密钥的 Azure Key Vault。 有关详细信息，请参阅[Azure Key Vault 中的客户托管密钥的透明数据加密](transparent-data-encryption-byok-overview.md)。
1. 如果为托管实例启用了审核，请确保：
    - 已将包含现有日志的存储容器或事件中心移到目标区域。
    - 已在目标实例上配置审核。 有关详细信息，请参阅[SQL 托管实例的审核](../managed-instance/auditing-configure.md)。
1. 如果你的实例具有长期保留策略（LTR），则现有的 LTR 备份仍与当前实例相关联。 由于目标实例不同，因此可以使用源实例在源区域中访问较旧的 LTR 备份，即使该实例已删除也是如此。
...
在每个源托管实例和 SQL 托管实例的相应目标实例之间创建故障转移组。
...
- **ReplicationState = 2** (CATCH_UP) 表示数据库已同步，可以安全故障转移。
- **ReplicationState = 0** （种子设定）指示数据库尚未设定种子，尝试故障转移会失败。
...
1. 使用辅助终结点连接到目标托管实例 `<fog-name>.secondary.database.windows.net` 。
1. 在完全同步的情况下，使用 [Switch-AzSqlDatabaseFailoverGroup](/powershell/module/az.sql/switch-azsqldatabasefailovergroup?view=azps-2.3.2) 将辅助托管实例切换为主托管实例。 此操作将成功，或将回滚。
1. 使用 `nslook up <fog-name>.secondary.database.windows.net` 验证该命令是否已成功完成，确定 DNS CNAME 条目指向目标区域的 IP 地址。 如果 switch 命令失败，则不会更新 CNAME。
...
在数据库迁移后[管理](manage-data-after-migrating-to-database.md)数据库。
