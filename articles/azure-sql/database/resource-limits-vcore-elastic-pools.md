...
您可以使用[Azure 门户](elastic-pool-manage.md#azure-portal)、 [PowerShell](elastic-pool-manage.md#powershell)、 [Azure CLI](elastic-pool-manage.md#azure-cli)或[REST API](elastic-pool-manage.md#rest-api)设置服务层、计算大小（服务目标）和存储量。

> [!IMPORTANT]
> 有关缩放指导和注意事项，请参阅[缩放弹性池](elastic-pool-scale.md)。
...
如果弹性池的所有 vCore 繁忙，则池中的每个数据库将接收相同数量的计算资源来处理查询。 Azure SQL 数据库通过确保计算时间的相同切片，在数据库之间提供资源共享的公平。 弹性池资源共享公平性是在将每个数据库的 vCore 最小值设为非零值时，对另外为每个数据库保证的任意资源量的补充。
...
> [!NOTE]
> 弹性池中各个数据库的资源限制通常与池外部的单一数据库的资源限制相同，这些池的计算大小相同（服务目标）。 例如，GP_Gen4_1 数据库的最大并发工作进程数为 200 个。 因此，GP_Gen4_1 池中数据库的最大并发工作进程数也是 200 个。 请注意，GP_Gen4_1 池中的并发工作进程总数为 210 个。
