...
|列存储支持|是|是|是|是|是|是|
|内存中 OLTP 存储 (GB)|空值|空值|空值|不可用|不可用|空值|
...
|副本数|1|1|1|1|1|1|
|Multi-AZ|空值|空值|空值|不可用|空值|空值|
|读取横向扩展|空值|空值|空值|不可用|不可用|空值|
|随附的备份存储|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|1 倍数据库大小|
...
|列存储支持|是|是|是|是|是|是|是|
|内存中 OLTP 存储 (GB)|空值|空值|不可用|不可用|不可用|不可用|空值|
...
|副本数|1|1|1|1|1|1|1|
|Multi-AZ|空值|空值|不可用|空值|空值|空值|空值|
|读取横向扩展|空值|空值|不可用|不可用|不可用|不可用|不可用|
...
|副本数|1|1|1|1|1|1|1|
|Multi-AZ|不可用|空值|空值|空值|空值|空值|空值|
...
|每个池的最大日志速率 (MBps)|30|30|30|30|30|
|每个池的最大并发辅助进程数（请求） <sup>3</sup>|400|500|600|700|800|
...
|每个池的最大数据库数<sup>1</sup>|500|500|500|500|500|
|列存储支持|是|是|是|是|是|是|
|内存中 OLTP 存储 (GB)|不可用|不可用|不可用|不可用|不可用|空值|
...
|每个池的最大日志速率 (MBps)|30|30|30|30|30|30|
|每个池的最大并发辅助进程数（请求） <sup>3</sup>|900|1000|1200|1600|1800|3600|
|每个池的最大并发登录数（请求数） <sup>3</sup>|1800|2000|2400|3200|3600|7200|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|
|每个数据库的最小/最大弹性池 vCore 选项|0-18|0-20|0-24|0-32|0-36|0-72|
|副本数|1|1|1|1|1|1|
|Multi-AZ|空值|不可用|不可用|不可用|不可用|不可用|
...
|每个池的最大并发辅助进程数（请求） <sup>3</sup>|800|1,000|1,200|1,400|1,600|1,800|
...
|每个池的最大并发辅助进程数（请求） <sup>3</sup>|2,000|2,400|3,200|6,400|12,800|
