...
# ExpressRoute 先决条件  
若要使用 ExpressRoute 连接到 Azure，你需要确认是否满足以下先决条件：

- Microsoft Azure 帐户
- 与上述[支持列表](https://msdn.microsoft.com/library/azure/dn957919.aspx)中的网络服务提供商 (NSP) 或 Exchange 提供商 (EXP) 的关系，通过这些提供商便于实现连接要求。你必须与网络服务提供商或 Exchange 提供商存在现有业务关系。你必须确保所使用的服务与 ExpressRoute 兼容。 
...
- 与 Microsoft 帐户团队联系。我们建议你与 Microsoft 帐户团队联系。帐户团队可以与你和你的服务提供商协作，以优先处理你的请求。
...
- 用于路由配置的 IP 地址和 AS 编号。 
	- 你可以使用私有 AS 编号。如果你选择这样做，则该编号必须大于 65000。有关 AS 编号的详细信息，请参阅[自治系统 (AS) 编号](http://www.iana.org/assignments/as-numbers/as-numbers.xhtml)。
