...
5. While the specifics of each registrar vary, in general you map *from* your custom domain name (such as **contoso.com**,) *to* the Azure Web Site domain name (**contoso.chinacloudsites.cn**) or the Azure Web Site virtual IP address.

	* CNAME records will always map to the Azure Web Sites domain - **contoso.chinacloudsites.cn**. So you will be mapping *from* a domain such as **www** *to* your **&lt;yourwebsitename&gt;.chinacloudsites.cn** address.
	
		> [WACN.NOTE] If you will be using an A record, you must also add a CNAME record with one of the following configurations:
		> 
		> * Maps *from* **www** *to* your **&lt;yourwebsitename&gt;.chinacloudsites.cn**.
		> 
		> OR
		> 
		> * Maps *from* **awverify.www** *to* **awverify.&lt;yourwebsitename&gt;.chinacloudsites.cn**.
		> 
	...
	* A records will always map to the Azure Web Sites virtual IP address. So you are mapping *from* a domain such as **www** *to* the web site's virtual IP address.
		...
		> [WACN.NOTE] To map a root domain (such as **contoso.com**,) to an Azure Web Site, you will often map from '**@**', or a blank entry to the virtual IP address. To create a wildcard mapping that maps all sub-domains to the virtual IP address, you will usually map from '*' to the virtual IP address.
		> 
