<properties
	pageTitle="Azure Insights: Azure Insights PowerShell 快速入门示例。| Microsoft Azure"
...
## 配置诊断日志
许多 Azure 服务提供其他日志和遥测，包括 Azure 网络安全组、软件负载平衡器、密钥保管库、Azure 搜索服务和逻辑应用，并且可以对它们进行配置，以在 Azure 存储帐户中保存数据。仅在资源级别执行该操作，存储帐户应与配置诊断设置的目标资源处于相同的区域中。
