<properties
	pageTitle="Azure Insights：使用审核日志在 Azure Insights 中发送电子邮件和 webhook 警报通知。| Microsoft Azure"
...
### 示例电子邮件规则
必须设置电子邮件规则、webhook 规则，然后告诉这些规则在审核日志事件发生时启动。可以在 [Azure Insights PowerShell 快速入门示例](insights-powershell-samples.md#alert-on-audit-log-event)处查看使用 PowerShell 的示例。
