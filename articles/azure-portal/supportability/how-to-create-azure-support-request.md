...
可以在 Azure 门户获取**帮助和支持**。 它可通过全局标头或服务的资源菜单获得。 你必须具有相应的权限，然后才能发出支持请求。
...
在 "**新支持请求**" 的 "**基本**信息" 选项卡上，使用选择器开始告诉我们问题。 首先，您将确定问题类型的一些常规类别，并选择相关订阅。 若要获取帮助，下一步是选择服务（例如 "运行 Windows 的虚拟机"）和资源（虚拟机的名称）。 简要描述你自己的问题，然后选择 "**问题类型**" 以获取更具体的信息。
...
接下来，我们收集有关问题的其他详细信息。 在此步骤中提供详尽的详细信息可帮助我们将你的支持请求路由到正确的代理，并让他们开始诊断问题。

最好是让我们知道问题的开始时间，以及重现该问题的步骤。 你还可以选择上传文件，如日志文件或诊断输出。

获得有关你遇到的问题的所有信息后，你可以选择如何获取支持。 在 "**详细信息**" 选项卡的 "**支持方法**" 部分中，选择影响的严重性。 提供首选联系方法、与你联系的良好时间以及你的支持语言。
...
支持代理将使用你指定的方法与你联系。 有关初始响应时间的信息，请查阅[支持范围和响应能力](https://azure.microsoft.com/support/plans/response/)。
...
在此页上，可以按订阅、创建日期（UTC）和状态筛选支持请求。 另外，还可以在此页面上对支持请求进行排序和搜索操作。

选择支持请求以查看详细信息，包括严重性和支持代理响应所需的时间。

如果要更改请求的严重性，请选择 "**业务影响**"。 显示可用于分配的严重性列表。
