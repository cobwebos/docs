...
2. 在拥有远程桌面连接后，通过使用桌面上的图标来启动 **Hadoop 命令行**。
...
4. 通过应用正则表达式从每个记录中仅提取日志记录级别来转换数据：
	...
	转换后，你可以使用 **DUMP** 来查看数据。在这种情况下为 `DUMP LEVELS;`。
...
6. 你也可以通过使用 `STORE` 语句保存转换结果。例如，以下语句会将 `RESULT` 保存到群集的默认存储容器中的 **/example/data/pigout** 目录：
