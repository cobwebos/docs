...
如果可用存储小于预配存储大小的 5%，则将标记为只读 100 GB 预配存储的服务器。 对于预配存储超出 100 GB 的服务器，如果可用存储少于 5 GB，则会将其标记为只读。
...
我们建议你启用存储自动增长或设置警报，以便在服务器存储接近阈值时通知你，避免进入只读状态。 有关详细信息，请参阅有关[如何设置警报](howto-alert-metric.md)的文档。

### <a name="storage-auto-grow"></a>存储自动增长

存储自动增长可防止服务器耗尽存储空间并变为只读。 如果启用了存储自动增长，存储会在不影响工作负荷的情况下自动增长。 对于配置存储小于 100 GB 的服务器，当可用存储低于预配存储的 10% 时，预配存储大小将增加 5 GB。 对于具有 100 GB 以上预配存储的服务器，当可用存储空间低于预配存储大小的 10 GB 时，预配存储大小将增加 5%。 适用上面指定的最大存储限制。

例如，如果已预配了 1000 GB 的存储，并且实际利用率超过 990 GB，则服务器存储大小将增加到 1050 GB。 或者，如果已预配 10 GB 的存储，则当可用存储少于 1 GB 时，存储大小会增加到 15 GB。

请记住，存储只能增加，不能减少。

## <a name="backup"></a>备份

服务自动对服务器进行备份。 您可以选择 7 到 35 天的保留期。 通用和内存优化服务器可以选择具有用于备份的异地冗余存储。 在[概念文章中了解有关备份的更多内容](concepts-backup.md)。
