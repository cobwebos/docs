...
Azure Database for MariaDB 使用 MariaDB server 的社区版。 因此，它与多种编程语言和驱动程序兼容。 MariaDB 的 API 和协议与 MySQL 使用的 API 和协议兼容。 这意味着，使用 MySQL 的连接器还应使用 MariaDB。

其目标是支持 MariaDB 驱动程序的三个最新版本，并与开源社区的作者一起不断改善 MariaDB 驱动程序的功能和可用性。 下表提供了已测试并发现与 Azure Database for MariaDB 10.2 兼容的驱动程序列表：
...
前往 | https://github.com/go-sql-driver/mysql/releases | 1.3、1。4 | 1.2 及以下版本 | 在版本1.3 的连接字符串中使用 `allowNativePasswords=true`。 版本1.4 包含一个修补程序，并且不再需要 `allowNativePasswords=true`。
...
兼容性优势也适用于数据库管理工具。 只要数据库操作在用户权限范围内运行，现有工具就会继续使用 Azure Database for MariaDB。 下表列出了已测试并发现与 Azure Database for MariaDB 10.2 兼容的三个常见数据库管理工具：
