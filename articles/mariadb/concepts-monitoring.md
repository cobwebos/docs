...
可以在服务器上启用慢查询日志。 还可以通过 Azure 诊断日志在 Azure Monitor 日志、事件中心和存储帐户中使用这些日志。 若要了解有关日志记录的详细信息，请访问 [服务器日志](concepts-server-logs.md)页。

## <a name="query-store"></a>查询存储

[查询存储](concepts-query-store.md)跟踪一段时间内的查询性能，包括查询运行时统计信息和等待事件。 此功能保留了**mysql**架构中的查询运行时性能信息。 你可以通过各种配置旋钮控制数据的收集和存储。
...
**计划内维护通知**可用于接收有关即将进行的计划内维护到 Azure Database for MariaDB 的警报。 这些通知与[服务运行状况的](../service-health/overview.md)计划内维护集成，允许你在同一位置查看订阅的所有计划内维护。 它还有助于将通知扩展到不同资源组的适当受众，因为你可能有不同的联系人负责不同的资源。 你将在事件之前的即将到来的维护72小时内收到通知。
