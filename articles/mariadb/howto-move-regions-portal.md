...
description: 使用读取副本和 Azure 门户将 Azure Database for MariaDB 服务器从一个 Azure 区域移到另一个区域。
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: how-to
...
将现有 Azure Database for MariaDB 服务器从一个区域移到另一个区域的方案有多种。 例如，你可能想要将生产服务器作为灾难恢复计划的一部分转移到另一个区域。

你可以使用 Azure Database for MariaDB[跨区域读取副本](concepts-read-replicas.md#cross-region-replication)来完成移动到另一个区域。 为此，请先在目标区域中创建一个读取副本。 接下来，停止到读取副本服务器的复制，使其成为接受读和写流量的独立服务器。 
...
- 只读副本功能仅适用于“常规用途”或“内存优化”定价层中的 Azure Database for MariaDB 服务器。 请确保源服务器在这些定价层中的一个。

- 请确保你的 Azure Database for MariaDB 源服务器位于要从中移动的 Azure 区域。
...
你可能想要删除源 Azure Database for MariaDB 服务器。 为此，请按照以下步骤操作：
...
在本教程中，你通过使用 Azure 门户将 Azure Database for MariaDB 服务器从一个区域移到另一个区域，然后清理了不需要的源资源。 
