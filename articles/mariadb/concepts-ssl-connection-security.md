...
在某些情况下，应用程序需要从受信任的证书颁发机构生成的本地证书文件 (CA) 证书文件安全地进行连接。 要连接到 Azure Database for MariaDB 服务器的证书位于上 https://www.digicert.com/CACerts/BaltimoreCyberTrustRoot.crt.pem 。 
...
> [!Note] 
> 默认情况下，Azure Database for MariaDB 不会强制) 设置 (最低 TLS 版本 `TLSEnforcementDisabled` 。
>
