---
title: Azure Database for MariaDB 中查询存储
description: 本文介绍 Azure 数据库中的查询存储功能 for MariaDB
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
ms.locfileid: "67079387"
---
# <a name="monitor-azure-database-for-mariadb-performance-with-query-store"></a>有关使用查询存储的 MariaDB 性能监视 Azure 数据库
...
Azure Database for Mariadb 中的查询存储功能提供了一种方法来跟踪一段时间内的查询性能。 通过帮助快速查找运行时间最长且资源最密集的查询，查询存储可简化性能故障排除。 查询存储自动捕获查询和运行时统计信息的历史记录，并保留它们以供查看。 它按时间范围分隔数据，以便可以查看数据库使用模式。 为所有用户、 数据库和查询数据存储在**mysql**架构数据库在 Azure Database for MariaDB 实例中的。
...
使用 [Azure 门户](howto-server-parameters.md)要获取或设置某个参数的不同值。

## <a name="views-and-functions"></a>视图和函数

使用以下视图和函数查看并管理查询存储。 中的任何人[选择权限公共角色](howto-create-users.md#create-additional-admin-users)可以使用这些视图以查看查询存储中的数据。 这些视图中才有**mysql** 数据库。
...
- 如果 MariaDB 服务器有参数`default_transaction_read_only`，查询存储无法捕获数据。
