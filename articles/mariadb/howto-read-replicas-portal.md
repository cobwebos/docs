---
title: 在 Azure Database for MariaDB 中创建和管理读取副本
description: 本文介绍如何使用门户在 Azure Database for MariaDB 中设置和管理读取副本
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
ms.locfileid: "67876308"
---
# <a name="how-to-create-and-manage-read-replicas-in-azure-database-for-mariadb-using-the-azure-portal"></a>如何使用 Azure 门户在 Azure Database for MariaDB 中创建和管理读取副本

在本文中, 你将学习如何使用 Azure 门户在 Azure Database for MariaDB 服务中创建和管理读取副本。
...
> [!IMPORTANT]
> 读取副本功能仅适用于常规用途或内存优化定价层中的 Azure Database for MariaDB 服务器。 请确保主服务器位于其中一个定价层中。
...
6. 选择副本服务器的位置。 可以在任何 Azure 区域中创建副本。 默认位置与主服务器相同。
...
1. 在 Azure 门户中, 选择您的主 Azure Database for MariaDB 服务器。 
...
2. 从“概览”  中，选择“删除”  。
   ...
   ![Azure Database for MariaDB-删除 master](./media/howto-read-replica-portal/delete-master-overview.png)

3. 键入主服务器的名称，然后单击“删除”  以确认删除主服务器。  

   ![Azure Database for MariaDB-删除 master](./media/howto-read-replica-portal/delete-master-confirm.png)
...
5. 查看所选时间范围的复制延迟时间。 下图显示了较大工作负荷的最近30分钟。
