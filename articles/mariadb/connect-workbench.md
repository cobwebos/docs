...
2.  在“设置新连接”对话框的“参数”选项卡上，输入以下信息：
    ...
    | 用户名 |  服务器管理员登录名 | 输入在创建 Azure Database for MariaDB 实例时使用的服务器管理员登录用户名。 示例用户名为 **myadmin@mydemoserver**。 如果不记得服务器管理员登录名，请完成上一部分的步骤，以便获取连接信息。 格式为 username@servername。
