---
title: 访问审核日志 - Azure CLI - MariaDB 的 Azure 数据库
description: 本文介绍如何从 Azure CLI 配置和访问 MariaDB Azure 数据库中的审核日志。
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
可以从 Azure CLI[为 MariaDB 审核日志](concepts-audit-logs.md)配置 Azure 数据库。
...
1. 通过更新**audit_log_egitvents**参数选择要记录[的事件类型](concepts-audit-logs.md#configure-audit-logging)。
    ```azurecli-interactive
...
1. 通过更新**audit_log_include_users**参数添加要包括的任何特定 MariaDB 用户进行日志记录。 通过提供 MariaDB 用户名来指定用户。
    ```azurecli-interactive
