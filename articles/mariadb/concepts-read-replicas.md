...
* 区域可用性：MariaDB 的 Azure 数据库在美国西部 2、法国中部、阿联酋北部和德国中部可用。 但是，它们配对的区域不可用。
...
参数[`event_scheduler`](https://mariadb.com/kb/en/library/server-system-variables/#event_scheduler)锁定在副本服务器上。
