---
title: 读取 Azure Database for MariaDB 中的副本
description: 本文介绍 Azure Database for MariaDB 的读取副本。
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
ms.locfileid: "67874898"
---
# <a name="read-replicas-in-azure-database-for-mariadb"></a>读取 Azure Database for MariaDB 中的副本

使用读取副本功能, 可以将 Azure Database for MariaDB 服务器中的数据复制到只读服务器。 可将主服务器中的数据复制到最多 5 个副本。 副本使用 MariaDB 引擎的二进制日志 (binlog) 基于文件位置的复制技术 (包含全局事务 ID (GTID)) 以异步方式更新。 要详细了解 binlog 复制，请参阅 [binlog 复制概述](https://mariadb.com/kb/en/library/replication-overview/)。

> [!IMPORTANT]
> 可以在主服务器所在的区域或所选的任何其他 Azure 区域创建只读副本。 读取副本 (同一区域和跨区域) 当前为公共预览版。
...
只读副本可以增强灾难恢复计划。 如果存在区域性灾难, 并且你的主服务器不可用, 则可以将工作负荷定向到其他区域中的副本。 为此, 首先允许副本接受使用 "停止复制" 功能的写入。 然后可以通过更新连接字符串来重定向应用程序。 在[停止复制](#stop-replication)部分了解详细信息。
...
您可以使用副本的主机名和有效用户帐户连接到副本, 就像在常规 Azure Database for MariaDB 服务器上一样。 对于名称为 **myreplica**、管理员用户名为 **myadmin** 的服务器，可以使用 mysql CLI 连接到副本：
...
此指标是使用 MariaDB 的`seconds_behind_master` `SHOW SLAVE STATUS`命令中可用的指标计算得出的。
