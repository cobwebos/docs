<properties linkid="develop-java-tutorials-web-site-custom_upload" urlDisplayName="上载自定义 Java 网站" pageTitle="将自定义 Java 网站上载到 Azure" metaKeywords="" description="本教程介绍了如何将自定义 Java 网站上载到 Azure。" metaCanonical="" services="web-sites" documentationCenter="Java" title="Upload a custom Java  Website to Azure" videoId="" scriptId="" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" />

# 将自定义 Java 网站上载到 Azure

本主题介绍了如何将自定义 Java 网站上载到 Azure。包括适用于 Java 网站的信息以及特定应用程序的示例。

请注意，Azure 提供了一种使用 Azure 门户配置 UI 和 Azure 应用程序库创建 Java 网站的方法，如[Azure 网站和 Java 入门
](../web-sites-java-get-started)。此教程适用于无需使用 Azure 配置 UI 或 Azure 应用程序库的方案。  
...
3. 在**d:\home\site\wwwroot**中创建 web.config，并在其中粘贴以下内容：
    ...
    此时应重新启动网站使所做更改生效。   连接到 [http://yoursite/hudson](http://yoursite/hudson)，启动 Hudson。
...
值得注意的是，JRE_HOME 环境变量将在上述 web.config 中指定为指向 64 位 JDK。默认为 32 位，但由于 Liferay 可能需要大量内存，因此建议使用 64 位 JDK。

做完上述更改后，重新启动运行 Liferay 的网站，然后打开 [http://yoursite](http://yoursite)。可从网站根目录访问 Liferay 门户。 
