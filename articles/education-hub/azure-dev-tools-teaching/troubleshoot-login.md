...
访问用于教学的 Azure 开发工具要求用户拥有 Microsoft 帐户（MSA）。 如果用户的帐户尚不是 MSA 或已链接到 MSA，则学生将自动定向到创建 MSA。 如果域与 Active Directory 相关联，则该域下的所有帐户都已被视为 MSA。
...
如果你使用大学电子邮件地址（例如 `John.Smith@university.edu` ）登录，则需要使用该电子邮件地址创建一个 Microsoft 帐户。 创建帐户是免费的，只需几分钟即可完成。 如果有 Microsoft 帐户，则可以使用单个用户名和密码自动登录到所有 Microsoft 产品。

### <a name="use-a-personal-email-address"></a>使用个人电子邮件地址
如果你使用个人电子邮件地址（例如 `John.Smith@email.com` ）登录，但也有大学电子邮件地址，请尝试使用大学电子邮件地址。 如果你以前使用个人电子邮件地址登录到你的机构的 webstore 或没有大学电子邮件地址，则需要创建一个 Microsoft 帐户，并将其链接到你的个人电子邮件地址。

## <a name="use-an-existing-microsoft-account"></a>使用现有 Microsoft 帐户
如果学生具有现有 Microsoft 帐户（例如，Xbox），则他们可以将该帐户连接到 Azure 开发工具帐户。
...
下一次登录时，你可以使用你的大学电子邮件地址来访问用于教学的 Azure 开发工具。
    ...
    :::image type="content" source="media/troubleshoot-accounts/manage-sign-in-add-email.png" alt-text="管理你登录到 Microsoft 的方式。" border="false":::
