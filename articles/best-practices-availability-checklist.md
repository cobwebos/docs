...
- 通过本地和地理冗余**利用数据复制**。Azure 存储空间中的数据会自动复制，以防止基础结构故障时发生数据丢失，你可以配置这种复制的某些方面。例如，可以在多个地理区域中复制数据的只读副本（称为全局读取访问冗余存储，简称 RA-GRS）。请注意，使用 RA-GRS 会产生额外的费用。有关详细信息，请参阅 [Azure Storage Pricing（Azure 存储空间定价）](https://azure.microsoft.com/pricing/details/storage/)。
...
- **启用高可用性选项来维护 Azure Redis 缓存的辅助副本。** 使用 Azure Redis 缓存时，请选择“标准”选项来维护内容的辅助副本。有关详细信息，请参阅 [Create a cache in Azure Redis Cache（在 Azure Redis 缓存中创建缓存）](https://msdn.microsoft.com/library/dn690516.aspx)。
