<properties
   pageTitle="可用性核对清单 | Microsoft Azure"
...
- 通过本地和地理冗余**利用数据复制**。Azure 存储空间中的数据会自动复制，以防止基础结构故障时发生数据丢失，你可以配置这种复制的某些方面。例如，可以在多个地理区域中复制数据的只读副本（称为全局读取访问冗余存储，简称 RA-GRS）。请注意，使用 RA-GRS 会产生附加费用，请参阅 Microsoft 网站上的 [Azure 存储空间定价](http://azure.microsoft.com/pricing/details/storage/)页以获取详细信息。
...
- **使用定期备份和时间点还原**，并确保符合恢复点目标 (RPO)。定期自动备份未保留在其他位置的数据，并确认可以在发生故障时可靠还原数据和应用程序本身。数据复制并不是备份功能，因为故障、错误或恶意操作造成的错误和不一致性将会复制到所有存储中。备份过程必须是安全的，这样才能保护传输中和存储中的数据。数据库或部分数据存储通常可以使用事务日志恢复到以前的某个时间点。Microsoft Azure 针对存储在 Microsoft Azure SQL 数据库中的数据提供备份工具。数据将导出到 Microsoft Azure Blob 存储上的备份包，并且可下载到存储的安全本地位置。
- **启用高可用性选项来维护 Redis 缓存的辅助副本。** 使用 Redis 缓存时，请选择“标准”选项来维护内容的辅助副本。有关详细信息，请参阅 Microsoft 网站上的[在 Azure Redis 缓存中创建缓存](https://msdn.microsoft.com/zh-cn/library/dn690516.aspx)页。
