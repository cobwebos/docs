...
本教程是有关创建 HBase 地域复制的[系列教程][hdinsight-hbase-geo-replication]的第二部分。
...
2.	输入：
	- **名称**：Contoso-DNS-EU
	- **DNS SERVER IP ADDRESS**:10.1.0.4 - IP 地址必须匹配 DNS 服务器虚拟机的 IP 地址。
...
5.	输入以下信息：
	- **DNS 域**：输入 Contoso-DNS-EU 的 DNS 后缀。例如：Contoso-DNS-US.b5.internal.chinacloudapp.cn。
