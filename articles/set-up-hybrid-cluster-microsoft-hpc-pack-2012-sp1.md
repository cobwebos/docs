...
本教程假定你之前未使用过计算群集或 HPC Pack。它只是为了出于演示目的帮助你快速部署混合群集。有关在生产环境中以更高层级部署混合 HPC Pack 群集的注意事项和步骤，请参阅[详细指南][]。

> [WACOM.NOTE] Azure 为你的计算资源提供了适合不同工作负载的[大小范围][]。例如，A8 和 A9 实例结合了某些 HPC 应用程序所需要的高性能和对低延迟、高吞吐量应用程序网络的访问。有关信息，请参阅 [A8 和 A9 计算密集型实例：使用 HPC Pack 快速启动][]。
...
-   运行某个版本的 Windows Server 2012 R2 或 Windows Server 2012 的本地计算机。该计算机将是 HPC 群集的头节点。如果你尚未运行 Windows Server，可下载并安装[评估版][]。
...
-   HPC Pack 2012 R2 的安装文件，它是免费提供的。[下载][]完整安装包并将文件复制到头节点计算机或某个网络位置。选择语言与你所安装的 Windows Server 的语言相同的安装文件。
...
使用[管理门户][]来执行以下 Azure 订阅步骤。之所以需要些步骤，是因为之后你可以从本地头节点部署 Azure 节点。
...
-   创建 Azure 存储帐户
    ...
    > [WACOM.NOTE] 另外请记下你的 Azure 订阅 ID，后面将会需要用到它。在你的 Azure [帐户信息][]中可以找到此 ID。
...
10. 在创建节点模板向导中，执行以下操作：
    ...
    c. 在“提供订阅信息”页面上，输入 Azure 订阅 ID（在 Azure [帐户信息][]中）。然后，在“管理证书”中，单击“浏览”并选择“默认 HPC Azure 管理”。然后单击**“下一步”**。
...
3.  在“指定新节点”页上，选择你之前创建的 Azure 节点模板（默认称作“默认 AzureNode 模板”）。然后指定大小为“小型”的 **2** 个节点，然后单击“下一步”。
    ...
    有关可用虚拟机大小的详细信息，请参阅 [Azure 的虚拟机和云服务大小][]。
...
4.  若要确认角色实例正在运行，请在[管理门户][]中单击“云服务”，单击你的云服务的名称，然后单击“实例”。
...
4.  若要确认角色实例不再在 Azure 继续运行，请在[管理门户][]中单击“云服务”，单击你的云服务的名称，然后单击“实例”。将不会有任何实例部署在生产环境中。
...
-   [HPC Pack 2012 R2 和 HPC Pack 2012][]
-   [使用 Microsoft HPC Pack 迸发到 Azure][详细指南]
-   [将应用程序部署到 Azure 节点][]
-   [Azure 虚拟机中的 Microsoft HPC Pack][]
