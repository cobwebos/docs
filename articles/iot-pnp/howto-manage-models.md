...
- 查找 IoT 即插即用设备功能型号。 你可以使用这些模型[快速构建 IoT ready 设备，并将它们与解决方案集成](./quickstart-connect-pnp-device-solution-node.md)。
...
1. 打开 Visual Studio Code，使用**Ctrl + Shift + P**，键入并选择**IoT 即插即用：打开模型存储库**。
...
1. 若要在公司模型存储库和公共模型存储库之间切换，请按**Ctrl + Shift + P**，键入并选择**IoT 即插即用：注销模型存储库**。 然后，使用**IoT 即插即用：再次打开模型存储库**命令。
