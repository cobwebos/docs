...
Azure CLI 提供了用于管理 IoT 即插即用公用和公司模型存储库中的设备功能模型和接口的命令。
...
1. 您可以选择**打开公共模型存储库**或**打开组织模型存储库**。 对于公司模型存储库, 需要输入模型存储库连接字符串。 你可以在**公司存储库**的 "**连接字符串**" 选项卡上的[Azure IoT 认证门户](https://preview.catalog.azureiotsolutions.com)中找到此连接字符串。
