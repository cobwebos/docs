...
按照上一部分中的步骤配置后，每次重新启动 Eclipse 时，都将自动从 Azure 帐户中注销。 但是，如果要在不重新启动 Eclipse 的情况下注销 Azure 帐户，请使用以下步骤。
...
按照上一部分中的步骤配置后，每次重新启动 Eclipse 时，Azure 工具包都会将你自动登录到 Azure 帐户。 但是，若要注销 Azure 帐户并禁止 Azure 工具包将你自动登录，请使用以下步骤。
...
如果在使用 Eclipse 时从 Azure 中注销，则需要重新配置用于 Eclipse 的 Azure 工具包以使用已创建的凭据文件，然后才能自动登录到 Azure 帐户。 以下步骤将引导你完成配置 Azure 工具包以使用现有凭据文件。
