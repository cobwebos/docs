...
> [!IMPORTANT]
> 区域冗余数据库和弹性池目前仅在选择区域中的高级和业务关键服务层中支持。 在使用业务关键层，选择 Gen5 计算硬件时才可用区域冗余配置。 有关支持区域冗余数据库的区域的最新信息，请参阅[按区域的服务支持](../availability-zones/az-overview.md#services-support-by-region)。  
