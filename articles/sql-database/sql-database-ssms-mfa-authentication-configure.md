---
title: 配置多重身份验证
description: 了解如何在 SSMS 中对 SQL 数据库和 Azure Synapse Analytics 使用多重身份验证
...
本主题演示如何将 Azure Active Directory 多重身份验证 (MFA) 用于 SQL Server Management Studio。 将 SSMS 或 SqlPackage 连接到 Azure [SQL 数据库](sql-database-technical-overview.md)和[azure Synapse Analytics](../synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is.md)时，可以使用 Azure AD MFA。 有关 Azure SQL 数据库多重身份验证的概述，请参阅[使用 SQL 数据库和 Azure Synapse 进行通用身份验证（对 MFA 的 SSMS 支持）](sql-database-ssms-mfa-authentication.md)。

> [!NOTE]
> 本主题适用于 Azure SQL server 以及在 Azure SQL server 上创建的 SQL 数据库和 Azure Synapse 数据库。 为简单起见，在提到 SQL 数据库和 Azure Synapse 时，本文统称 SQL 数据库。
...
2. **配置 MFA** -有关分步说明，请参阅[什么是 Azure 多重身份验证？](../active-directory/authentication/multi-factor-authentication.md)、[使用 azure SQL 数据库的条件性访问（MFA）和 azure Synapse](sql-database-conditional-access.md)。 （完整条件访问需要高级 Azure Active Directory （Azure AD）。 标准 Azure AD 提供有限 MFA。）
3. 为**Azure AD 身份验证配置 SQL 数据库或 Azure Synapse** -有关分步说明，请参阅[使用 Azure Active Directory 身份验证连接到 SQL 数据库或 azure Synapse](sql-database-aad-authentication.md)。
...
以下步骤演示如何使用最新的 SSMS 连接到 SQL 数据库或 SAzure Synapse。
...
3. 如果要以来宾用户身份进行连接，则不再需要填写来宾用户的“AD 域名或租户 ID”字段，因为 SSMS 18. x 或更高版本会自动识别它。 有关详细信息，请参阅[通过 SQL 数据库和 Azure Synapse 进行通用身份验证（对 MFA 的 SSMS 支持）](sql-database-ssms-mfa-authentication.md)。
...
4. 通常，对于 SQL 数据库和 Azure Synapse，必须单击 "**选项**" 并在 "**选项**" 对话框中指定数据库。 （如果连接的用户是来宾用户（如 joe@outlook.com），则必须选中该框并在“选项”中添加当前 AD 域名或租户 ID。 请参阅[通过 SQL 数据库和 Azure Synapse 进行通用身份验证（对 MFA 的 SSMS 支持）](sql-database-ssms-mfa-authentication.md)。 然后单击“连接”  。  
...
- 有关 Azure SQL 数据库多重身份验证的概述，请参阅使用[SQL 数据库和 Azure Synapse 进行通用身份验证（对 MFA 的 SSMS 支持）](sql-database-ssms-mfa-authentication.md)。  
