...
```
### <a name="factors-influencing-query-plan-changes"></a>因素影响查询计划更改

查询执行计划重新编译可能会导致不同于最初缓存生成的查询计划。 有多种原因导致为什么现有原始计划可能会自动重新编译：
- 查询所引用的架构中的更改
- 对查询所引用的表的数据更改 
- 对查询上下文选项的更改 

可以从多种原因，包括实例重新启动缓存弹出已编译的计划，数据库范围的配置更改、 内存压力和显式请求，以清除缓存。 此外，使用 RECOMPILE 提示意味着不会缓存计划。

重新编译 （或后缓存逐出的全新编译） 仍会导致在生成中从一个最初观察到相同的查询执行计划。  如果，但是，有对相比以前或原始计划的计划的更改，以下是对查询执行计划的更改原因的最常见解释：

- **更改物理设计**。 例如，新的索引创建，更有效地涵盖查询的要求可能会使用新编译，如果查询优化器决定它是更优，利用该新的索引不是使用第一个版本的最初选择的数据结构执行查询。  对被引用对象的任何物理更改可能会导致在编译时新计划选择。

- **服务器资源之间的差异**。 在一个计划，在"系统 A"和"系统 B"– 可用性的资源，如可用处理器的数量不同的方案中可能会影响哪些计划获取生成。  例如，如果一个系统具有更多的处理器，则可能选择并行计划。 

- **不同的统计信息**。 与被引用对象关联的统计信息已更改，或采用不同于原始系统的统计信息。  如果统计信息更改并重新编译时，查询优化器将在时间中使用该特定点处的统计信息。 修改后的统计信息可能具有明显不同的数据分布和不是这种情况在原始编译的频率。  这些更改用于估计基数估计 （预计流经查询逻辑树的行数）。  更改的基数估计可能导致我们选择不同的物理运算符和关联的操作顺序。  即使是轻微更改统计信息可能导致更改后的查询执行计划。

- **已更改的数据库兼容性级别或基数估计器版本**。  新的策略，并可能会导致不同的查询执行计划的功能，可以启用对数据库的兼容性级别的更改。  不在数据库兼容性级别禁用或启用跟踪标志 4199 或更改数据库作用域内配置也可以影响 QUERY_OPTIMIZER_HOTFIXES 的状态查询在编译时执行计划选项。  跟踪标志 9481 (强制旧版 CE) 和 2312 （强制执行默认 CE） 还会影响计划。 
