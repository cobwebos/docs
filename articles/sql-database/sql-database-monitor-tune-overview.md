...
若要监视 Azure 中的 SQL 数据库的性能，首先需要监视相对于所选数据库性能级别使用的资源。 监视以下资源：
 ...
 - **等待统计信息**：使用 [sys.dm_os_wait_stats (Transact-SQL)](/sql/relational-databases/system-dynamic-management-views/sys-dm-os-wait-stats-transact-sql) 确定查询的等待时间。 查询可能正在等待资源，发生了队列等待或外部等待。 
