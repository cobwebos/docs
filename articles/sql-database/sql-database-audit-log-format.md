...
| connection_id | 空值 | 服务器中的连接 ID | GUID | 空值 |
...
| 空值 | is_server_level_audit_s | 指示此审核是否处于服务器级别的标志 | 空值 | 字符串 |
...
| server_principal_name | server_principal_name_s | 当前登录 | sysname | 字符串 |
...
| statement | statement_s | 已执行的 t-sql 语句（如果有） | nvarchar(4000) | 字符串 |
| succeeded | succeeded_s | 指示触发事件的操作是否成功。 对于除登录名和批处理之外的事件，此选项仅报告权限检查是成功还是失败，而不是操作。 1 = 成功，0 = 失败 | bit | 字符串 |
