...
实例池与常规托管实例（*单一实例*）具有相似的体系结构。 为了支持 [Azure 虚拟网络中的部署（vnet）](../virtual-network/virtual-network-for-azure-services.md) 并为客户提供隔离和安全性，实例池还依赖于 [虚拟群集](sql-database-managed-instance-connectivity-architecture.md#high-level-connectivity-architecture)。 虚拟群集表示在客户的虚拟网络子网中部署的一组专用虚拟机。
