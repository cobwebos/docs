...
本文将介绍 SQL 数据库在性能、缩放性和易管理性方面的核心概念与功能，并提供链接让你进一步了解详细信息。 如果已准备就绪，可以在几分钟内[创建第一个 SQL 数据库](sql-database-get-started.md)，或者[创建弹性池](sql-database-elastic-pool-create-portal.md)。 如果想深入了解，请观看这个时长 30 分钟的视频。
...
许多业务和应用只要能够创建独立数据库并按需调高或调低性能即可，尤其是当使用模式相对容易预测时。 但如果有无法预测的使用模式，则管理成本和业务模式就会变得相当困难。

[弹性池](sql-database-elastic-pool.md) 可解决此问题。 概念很简单。 可向池分配性能，并且仅需为池的总体性能付费，而无需为独立数据库的性能付费。 无需调整数据库的性能。 池中的数据库称为 *弹性数据库*，可自动增大和减小来满足需求。 弹性数据库会使用该池，但不会超出其限制，因此即使数据库的使用情况仍不可预测，你的成本也仍是可预测的。 此外，你可以 [向池添加和删除数据库](sql-database-elastic-pool-manage-portal.md)，将应用从少量数据库扩展到数千个，而一切费用不会超出由你控制的预算范围。 若要深入了解如何通过弹性池设计 SaaS 应用程序的模式，请参阅 [具有 Azure SQL 数据库的多租户 SaaS 应用程序的设计模式](sql-database-design-patterns-multi-tenancy-saas-applications.md)。

无论使用单一数据库还是弹性数据库，你都可以随时更改选择。 可将独立数据库与弹性池混合使用，并快速而轻松地更改独立数据库和池的服务层来适应你的情况。 而且，凭借 Azure 的功能和作用范围，你可以将其他 Azure 服务与 SQL 数据库搭配使用以满足独特的应用程序设计需求，提高成本和资源效益，发掘新的商机。

但是，要如何比较数据库和数据库池的相对性能？ 当调高和调低性能时，如何知道该在何处停止？ 答案是依靠内置的性能监视和警报工具，并配合使用基于数据库事务单位 (DTU)（适用于独立数据库）和弹性 DTU (eDTU)（适用于弹性数据库和数据库池）的性能评级，快速评估根据你的当前需求或项目性能需求向上或向下缩放带来的影响。 有关详细信息，请参阅 [SQL 数据库选项和性能：了解每个服务层提供的功能](sql-database-service-tiers.md) 。
...
* 有关独立数据库和弹性池的成本比较与计算器，请参阅[定价页](https://azure.microsoft.com/pricing/details/sql-database/)。
