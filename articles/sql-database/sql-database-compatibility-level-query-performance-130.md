...
> 
> 在 2016 年 6 月中旬之前创建的数据库具有在其创建时的兼容级别。 从早期版本的 Azure SQL 数据库迁移的数据库的兼容级别为 100 或 110。 2016 年 6 月中旬之前创建的服务器的 master 数据库的兼容级别为创建服务器时的兼容级别，并且无法更改。 master 数据库和用户数据库之间不同的兼容级别不会影响到用户数据库的功能或性能。
> 
