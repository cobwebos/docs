<properties 
   ...
   description="时间点还原, Microsoft Azure SQL 数据库, 还原数据库, 恢复数据库, Azure PowerShell" 
...
本教程说明如何在 [Azure PowerShell](/documentation/articles/install-configure-powershell) 中使用时间点还原来还原 Azure SQL 数据库Azure SQL 数据库针对基本、标准和高级服务层提供内置备份，以支持自助时间点还原。
...
必须使用基于证书的身份验证来运行以下 cmdlet。有关详细信息，请参阅[如何安装和配置 Azure PowerShell](/documentation/articles/powershell-install-configure.md#use-the-certificate-method) 中的*使用证书方法*。
