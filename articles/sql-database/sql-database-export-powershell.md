...
[New-AzureRmSqlDatabaseExport](https://docs.microsoft.com/powershell/resourcemanager/azurerm.sql/v2.7.0/new-azurermsqldatabaseexport) cmdlet 向服务提交导出数据库请求。 根据数据库的大小，导出操作可能需要一些时间才能完成。

> [!IMPORTANT]
> 若要确保获得事务处理一致的 BACPAC 文件，应首先[创建数据库的副本](scripts/sql-database-copy-database-to-new-server-powershell.md)，然后导出该数据库副本。
> 
> 
...
运行 [New-AzureRmSqlDatabaseExport](https://docs.microsoft.com//powershell/resourcemanager/azurerm.sql/v2.7.0/new-azurermsqldatabaseexport) 后，可运行 [Get-AzureRmSqlDatabaseImportExportStatus](https://docs.microsoft.com/powershell/resourcemanager/azurerm.sql/v2.7.0/get-azurermsqldatabaseimportexportstatus) 来查看请求的状态。 如果请求后立即运行，通常会返回“**状态: 处理中**”。 显示“状态: 成功”时，表示导出完毕。
...
* 若要了解如何使用 Powershell 导入 Azure SQL 数据库，请参阅[使用 PowerShell 导入 BACPAC](scripts/sql-database-import-from-bacpac-powershell.md)。
