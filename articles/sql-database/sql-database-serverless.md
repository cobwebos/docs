...
Azure SQL 数据库无服务器 (预览版) 是单个数据库的计算层, 可根据工作负荷需求和帐单自动根据每秒使用的计算量来缩放计算。 当只对存储计费并在活动返回时自动恢复数据库时, 无服务器计算层还会在非活动期间自动暂停数据库。
...
使用`MaxVcore`参数在 PowerShell 中使用[AzSqlDatabase](https://docs.microsoft.com/powershell/module/az.sql/set-azsqldatabase)命令来修改 max vcore。

### <a name="minimum-vcores"></a>最小 vCore 数

#### <a name="use-powershell"></a>使用 PowerShell

修改 min vcore 是通过在 PowerShell `MinVcore`中使用参数[AzSqlDatabase](https://docs.microsoft.com/powershell/module/az.sql/set-azsqldatabase)命令来执行的。

### <a name="autopause-delay"></a>自动暂停延迟

#### <a name="use-powershell"></a>使用 PowerShell

修改 autopause 延迟是通过在 PowerShell `AutoPauseDelayInMinutes`中使用参数[AzSqlDatabase](https://docs.microsoft.com/powershell/module/az.sql/set-azsqldatabase)命令来执行的。
...
每秒每 vCore 的成本的 vCore 单位价格。 请参考 [Azure SQL 数据库定价页](https://azure.microsoft.com/pricing/details/sql-database/single/)，获取给定区域的特定单位价格。
