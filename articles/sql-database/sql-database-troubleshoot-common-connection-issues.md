...
正在移动（或重新配置）Azure 数据库时发生该错误，并且应用程序失去与 SQL 数据库的连接。 之所以会发生 SQL 数据库重新配置事件是因为有计划内事件（例如，软件升级）或计划外事件（例如，进程故障或负载均衡）。 大多数重新配置事件的生存期通常较短，并且应在最多 60 秒内完成。 但是，偶尔这些事件可能需要更长时间才能完成，例如当一个大型事务导致长时间运行的恢复时。
