---
title: Azure SQL 数据库 Premium RS 服务层停用 | Microsoft Docs
...
2018 年 2 月，Microsoft 已宣布不会发布 Azure SQL 数据库中 Premium RS 服务层的正式版，并且在 2019 年 1 月 31 日之后不再为其提供支持。 支持截止时间已推迟到 2019 年 6 月 30 日。 本文介绍从 Premium RS 服务层迁移到另一服务层的选项。 2019 年 6 月 30 日之后，Microsoft 会自动将 Premium RS 数据库迁移到最符合 Premium RS 数据库性能要求的正式版服务层。

以下是可能适用于 Premium RS 客户的迁移目标和定价选项：

- vCore 服务层

  [基于 vCore 的购买模型](sql-database-service-tiers-vcore.md)中的“常规用途”和“业务关键”服务层。 这两个服务层已推出正式版。 基于 vCore 的购买模型还提供“超大规模”服务层（公共预览版），该服务层按需适应工作负荷的需求，可自动将每个数据库最大扩展到 100 TB。 “超大规模”服务层提供的 IO 性能与[基于 DTU 的购买模型](sql-database-service-tiers-dtu.md)中的“高级”服务层相当，价格与 Premium RS 服务层比较接近。
- 开发/测试定价
...
## <a name="act-now-to-migrate-your-premium-rs-databases-to-alternative-sql-database-service-tiers"></a>立即将 Premium RS 数据库迁移到备用的 SQL 数据库服务层

请查看本文中的指导以及相关定价和文档，以确定 Premium RS 工作负荷的适当迁移目标。

## <a name="migrate-compute-intensive-workloads-and-save"></a>迁移计算密集型工作负荷并实现成本节省

对于计算密集型 Premium RS 工作负荷，我们建议迁移到基于 vCore 的正式版“常规用途”服务层，并使用面向 SQL Server 的 Azure 混合权益和预留容量产品/服务，以优于许可证涵盖费率的价格实现更大的节省。 如果想要保留使用基于 DTU 的购买选项，可将计算密集型 Premium RS 数据库迁移到“标准”服务层，与采用 Premium RS 正式版定价（如果已发布正式版）相比，这样仍可以实现节省。

> [!WARNING]
> 与采用当前的 Premium RS 定价相比，将 Premium RS 工作负荷迁移到基于 DTU 的“高级”服务层可能会增大每月成本。 我们建议考虑使用提供 Azure 混合权益和预留容量定价的“超大规模”或“业务关键”层，以维持与 Premium RS 类似或更低的成本。
...
基于 vCore 的购买模型中的服务层享受特别优惠，与许可证涵盖的定价相比，最多可将成本节省 80%。 与附带[面向 SQL Server 的 Azure 混合权益](https://azure.microsoft.com/pricing/hybrid-benefit/)的许可证涵盖定价相比，附带积极软件保障的 SQL Server Standard 或 Enterprise 版许可证最多可将成本节省 55%。 可将混合权益和 [Azure SQL 数据库预留容量](sql-database-reserved-capacity.md)定价相结合，在提前承诺使用一年或三年时，最多将成本节省 80%。  立即从 Azure 门户激活两项权益。
...
## <a name="migration-from-a-premium-rs-service-tier-to-a-service-tier-in-either-the-dtu-or-the-vcore-model"></a>从 Premium RS 服务层迁移到 DTU 或 vCore 模型中的服务层

### <a name="migration-of-a-database"></a>迁移数据库

将数据库从 Premium RS 服务层迁移到 DTU 或 vCore 模型中的服务层类似于在 Premium RS 服务层之间升级或降级。
