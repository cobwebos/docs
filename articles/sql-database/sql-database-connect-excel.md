...
本文介绍如何将 Excel 连接到 Azure SQL 数据库并基于数据库中的数据创建报告。你首先需要有一个 SQL 数据库。如果你没有，请参阅[创建你的第一个 SQL 数据库](sql-database-get-started.md)，以在几分钟内获取数据库，并让其中的示例数据正常运行。本文基于该文中的示例数据撰写，但你可以对自己的数据执行类似的步骤。
...
3.	在“连接到数据库服务器”对话框中，以格式 **<*servername*>.database.windows.net** 键入托管要连接到的逻辑服务器的**服务器名称**。例如，**adventureserver.database.windows.net**。
