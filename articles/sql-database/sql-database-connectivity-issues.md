...
* **在尝试连接期间发生暂时性错误**：延迟数秒后重试连接。
* **执行 SQL 查询命令期间发生暂时性错误**：不要立即重试命令。 应在一定的延迟之后建立新的连接。 然后重试命令。
...
如果程序通过第三方中间件与 SQL 数据库通信，请咨询供应商该中间件是否包含暂时性错误的重试逻辑。

<a id="principles-for-retry" name="principles-for-retry"></a>

### <a name="principles-for-retry"></a>重试原则
* 如果错误是暂时性的，请重试打开连接。
* 不要直接重试由于暂时性错误而失败的 SQL SELECT 语句。
  * 而是建立新的连接，并重试 SELECT。
* SQL UPDATE 语句由于暂时性错误而失败时，请先建立新的连接，然后重试 UPDATE。
