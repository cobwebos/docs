...
可以更改托管实例默认 DNS 区域 *.database.windows.net。* 

要使用另一个 DNS 区域而不是默认值，例如 *.contoso.com*： 
...
如果这两种硬件代系都可以在预配托管实例的区域中使用，则可以在硬件代系之间自动联机切换。 在这种情况下，您可以检查[vCore 模型概述页面](sql-database-service-tiers-vcore.md)，解释如何在硬件生成之间切换。
...
可以使用[Azure 成本管理解决方案](/azure/cost-management/)执行此操作。 导航到 Azure[门户](https://portal.azure.com)中的**订阅**，然后选择**成本分析**。 

使用 **"累积成本"** 选项，然后按**资源类型**筛选为`microsoft.sql/managedinstances`。 
  
...
除了 NSG 规则之外，内置防火墙还保护网络层上的实例。 在应用程序层上，通信受证书保护。
  
...
## <a name="mitigate-data-exfiltration-risks"></a>降低数据渗漏风险  

**如何降低数据渗漏风险？**

为了降低任何数据泄露风险，建议客户应用一组安全设置和控制：
...
为了更好地了解与部署 Azure SQL 数据库托管实例相关的收益、成本和风险，还有一个 Forrester 的研究[：MI 的总体经济影响](https://azure.microsoft.com/resources/forrester-tei-sql-database-managed-instance)。
...
不支持使用 IP 地址连接到托管实例。 托管实例主机名映射到托管实例虚拟群集前面的负载均衡器。 由于一个虚拟群集可以承载多个托管实例，因此如果不指定其名称，就无法将连接路由到适当的托管实例。
...
- 将非对称密钥保护器上传到 Azure Key Vault (AKV)，并将托管实例指向该保护器。** 此方法类似于自带密钥 (BYOK) TDE 用例，该用例也使用 AKV 集成来存储加密密钥。 如果您不想将密钥用作加密密钥保护程序，并且只想使该密钥可用于托管实例以还原加密数据库，请按照[设置 BYOK TDE](https://docs.microsoft.com/azure/sql-database/transparent-data-encryption-azure-sql#manage-transparent-data-encryption)的说明操作，不要选中复选框 *，使所选密钥成为默认 TDE 保护器*。
