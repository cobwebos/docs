...
> [!IMPORTANT]
> 迁移的证书仅用于还原 TDE 保护的数据库。 还原完成后，已迁移的证书会被另一个保护程序（服务托管证书或密钥保管库中的非对称密钥）替换为不同的保护程序，具体取决于在实例上设置的透明数据加密的类型。
...
- [安装和更新](https://docs.microsoft.com/powershell/azure/install-az-ps)Azure PowerShell 模块。
