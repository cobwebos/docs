---
title: "PowerShell：管理长期数据库备份保留 (Azure) | Microsoft Docs"
description: "关于如何使用 PowerShell 在 Azure 恢复服务保管库中配置、管理和还原自动 Azure SQL 数据库备份的长期保留的快速参考"
...
---
# <a name="configure-long-term-retention-of-database-backups-in-an-azure-recovery-services-vault-using-powershell"></a>使用 PowerShell 在 Azure 恢复服务保管库中配置数据库备份的长期保留
本主题介绍如何使用 PowerShell 在 Azure 恢复服务保管库中配置、管理和还原自动备份的长期保留。 还可以[使用 Azure 门户](sql-database-manage-long-term-backup-retention-portal.md)执行这些任务。

有关长期备份保留的详细信息，请参阅[长期备份保留](sql-database-long-term-retention.md)。

> [!TIP]
> 有关教程，请参阅[使用 PowerShell 进行数据保护和恢复的备份和还原指南](sql-database-get-started-backup-recovery-powershell.md)。
>

## <a name="configure-long-term-backup-retention-using-powershell"></a>使用 PowerShell 配置长期备份保留

配置长期保留需要使用最新的 [Azure PowerShell](https://docs.microsoft.com/powershell/azureps-cmdlets-docs/) 执行以下步骤：

1. 在与 SQL 数据库服务器相同的区域、订阅和资源组中创建 Azure 恢复服务保管库。 （[New-AzureRmRecoveryServicesVault](https://docs.microsoft.com/powershell/resourcemanager/azurerm.recoveryservices/v2.3.0/new-azurermrecoveryservicesvault)、[Set-AzureRmRecoveryServicesBackupProperties](https://docs.microsoft.com/powershell/resourcemanager/azurerm.recoveryservices/v2.3.0/set-azurermrecoveryservicesbackupproperties)）
2. 将 Azure SQL Server 注册到保管库 ([Set-AzureRmSqlServerBackupLongTermRetentionVault](https://docs.microsoft.com/powershell/resourcemanager/azurerm.sql/v2.3.0/set-azurermsqlserverbackuplongtermretentionvault))
3. 创建保留策略 ([New-AzureRmRecoveryServicesBackupProtectionPolicy](https://docs.microsoft.com/powershell/resourcemanager/azurerm.recoveryservices.backup/v2.3.0/new-azurermrecoveryservicesbackupprotectionpolicy))
4. 向需要长期备份保留的数据库应用保护策略 ([Set-AzureRmSqlDatabaseBackupLongTermRetentionPolicy](https://docs.microsoft.com/powershell/resourcemanager/azurerm.sql/v2.3.0/set-azurermsqldatabasebackuplongtermretentionpolicy))
...
## <a name="restore-from-long-term-backup-retention-using-powershell"></a>使用 PowerShell 从长期备份保留还原

从长期保留备份还原数据库需要安装最新的 [Azure PowerShell](https://docs.microsoft.com/powershell/azureps-cmdlets-docs/) 并使用以下 cmdlet：
...
## <a name="view-long-term-backup-retention-information-using-the-powershell"></a>使用 PowerShell 查看长期备份保留信息

查看长期保留的备份需要安装最新的 [Azure PowerShell](https://docs.microsoft.com/powershell/azureps-cmdlets-docs/) 并使用以下 cmdlet：
...
- 若要使用 Azure 门户管理长期备份保留中的备份，请参阅[使用 Azure 门户管理长期备份保留](sql-database-manage-long-term-backup-retention-portal.md)
