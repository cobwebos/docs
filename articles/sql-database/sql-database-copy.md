...
> [!NOTE]
> 创建数据库副本时，将使用[自动数据库备份](sql-database-automated-backups.md)。

## <a name="logins-in-the-database-copy"></a>数据库副本中的登录名

将某个数据库复制到同一 SQL 数据库服务器时，可以在这两个数据库上使用相同的登录名。 用于复制该数据库的安全主体将成为新数据库上的数据库所有者。 

将数据库复制到其他 SQL 数据库服务器时，在目标服务器上启动复制操作的安全主体将成为新数据库的所有者。 

无论目标服务器是什么，所有数据库用户、其权限及其安全标识符 （SID） 都将复制到数据库副本。 使用[包含的数据库用户](sql-database-manage-logins.md)进行数据访问可确保复制的数据库具有相同的用户凭据，以便在副本完成后，您可以立即使用相同的凭据访问它。

如果使用服务器级登录进行数据访问并将数据库复制到其他服务器，则基于登录的访问可能不起作用。 这可能是因为目标服务器上不存在登录名，或者因为它们的密码和安全标识符 （SID） 不同。 要了解如何在将数据库复制到其他 SQL 数据库服务器时管理登录名，请参阅[灾难恢复后如何管理 Azure SQL 数据库安全性](sql-database-geo-replication-security-config.md)。 复制到其他服务器后，在重新映射其他用户之前，只有与数据库所有者关联的登录名，或者服务器管理员才能登录到复制的数据库。 要在复制操作完成后解析登录并建立数据访问，请参阅[解析登录](#resolve-logins)名。
...
数据库副本是异步操作，但在接受请求后立即创建目标数据库。 如果需要取消仍在进行的复制操作，请使用 [Remove-AzSqlDatabase](/powershell/module/az.sql/new-azsqldatabase) cmdlet 删除目标数据库。
...
数据库副本是异步操作，但在接受请求后立即创建目标数据库。 如果需要取消仍在进行的复制操作，请使用 [az sql db delete](/cli/azure/sql/db#az-sql-db-delete) 命令删除目标数据库。
...
使用服务器管理员登录或创建要复制的数据库的登录登录到主数据库。 要使数据库副本成功，不是服务器管理员的登录名必须是`dbmanager`角色的成员。 有关登录名和链接到服务器的详细信息，请参阅[管理登录名](sql-database-manage-logins.md)。

开始复制源数据库与[创建数据库...作为语句的副本](https://docs.microsoft.com/sql/t-sql/statements/create-database-transact-sql?view=azuresqldb-current#copy-a-database)。 T-SQL 语句继续运行，直到数据库复制操作完成。

> [!NOTE]
> 终止 T-SQL 语句不会终止数据库复制操作。 要终止操作，删除目标数据库。
>

### <a name="copy-a-sql-database-to-the-same-server"></a>将 SQL 数据库复制到同一台服务器

使用服务器管理员登录或创建要复制的数据库的登录登录到主数据库。 要成功进行数据库复制，不是服务器管理员的登录名必须是`dbmanager`角色的成员。
...
登录到要创建新数据库的目标服务器的主数据库。 使用与源服务器上的源数据库的数据库所有者具有相同名称和密码的登录名。 目标服务器上的登录也必须是`dbmanager`角色的成员，或者是服务器管理员登录。
...
> [!IMPORTANT]
> 必须配置两个服务器的防火墙，以允许来自发出 T-SQL CREATE DATABASE 的客户端的 IP 的入站连接...作为命令的副本。

### <a name="copy-a-sql-database-to-a-different-subscription"></a>将 SQL 数据库复制到不同的订阅

您可以使用将[SQL 数据库复制到其他服务器](#copy-a-sql-database-to-a-different-server)部分中的步骤，使用 T-SQL 将数据库复制到其他订阅中的 SQL 数据库服务器。 请确保使用的登录名与源数据库的数据库所有者具有相同的名称和密码。 此外，登录必须是源服务器和目标服务器上`dbmanager`的角色或服务器管理员的成员。

> [!NOTE]
> [Azure 门户](https://portal.azure.com)PowerShell 和 Azure CLI 不支持数据库复制到其他订阅。

### <a name="monitor-the-progress-of-the-copying-operation"></a>监视复制操作的进度

通过查询[sys.数据库](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-databases-transact-sql)[、dm_database_copies](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-database-copies-azure-sql-database)和[sys.dm_operation_status](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-operation-status-azure-sql-database)视图来监视复制过程。 复制正在进行时，新数据库的sys.数据库视图**的state_desc**列设置为**COPYING**。

* 如果复制失败，新数据库的state_desc数据库视图**的列将**设置为 **"可疑"。** 对新数据库执行 DROP 语句并稍后重试。
* 如果复制成功，则新数据库的 sys.database 视图**的state_desc**列将设置为**ONLINE**。 复制已完成并且新数据库是一个常规数据库，可独立于源数据库进行更改。

> [!NOTE]
> 如果决定在复制过程中取消复制，请对新数据库执行 [DROP DATABASE](https://docs.microsoft.com/sql/t-sql/statements/drop-database-transact-sql) 语句。

> [!IMPORTANT]
> 如果需要创建服务目标比源小得多的副本，目标数据库可能没有足够的资源来完成种子设定过程，并且可能导致复制操作失败。 在这种情况下，请使用异地还原请求在不同服务器和/或不同区域中创建副本。 有关详细信息，请参阅[使用数据库备份恢复 Azure SQL 数据库](sql-database-recovery-using-backups.md#geo-restore)。

## <a name="resolve-logins"></a>解析登录名

在目标服务器上新数据库联机后，使用[ALTER USER](https://docs.microsoft.com/sql/t-sql/statements/alter-user-transact-sql?view=azuresqldb-current)语句将用户从新数据库重新映射到目标服务器上的登录。 若要解析孤立用户，请参阅[孤立用户疑难解答](https://docs.microsoft.com/sql/sql-server/failover-clusters/troubleshoot-orphaned-users-sql-server)。 另请参阅[灾难恢复后如何管理 Azure SQL 数据库安全性](sql-database-geo-replication-security-config.md)。

新数据库中的所有用户都保持他们在源数据库中已有的权限。 启动数据库副本的用户将成为新数据库的数据库所有者。 复制成功后，在重新映射其他用户之前，只有数据库所有者才能登录到新数据库。
