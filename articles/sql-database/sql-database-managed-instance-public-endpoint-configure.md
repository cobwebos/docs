...
- 在托管实例上启用公共终结点需要由托管实例管理员完成。可在 SQL 托管实例资源的 **"概述"** 页上找到托管实例管理员。
- 允许使用需要由网络管理员完成的网络安全组进行流量。有关详细信息，请参阅[网络安全组权限](../virtual-network/manage-network-security-group.md#permissions)。
...
运行以下 PowerShell 命令。 将**订阅 ID**替换为订阅 ID。 将 **rg-name** 替换为托管实例的资源组，将 **mi-name** 替换为托管实例的名称。
...
1. 如果托管实例的配置页仍然打开，请导航到 **"概述"** 选项卡。否则，请返回**SQL 托管实例**资源。 选择“虚拟网络/子网”链接，转到虚拟网络配置页。****
...
1. 请注意，公共终结点主机名的格式<mi_name>。**公共**.<dns_zone>.database.windows.net，用于连接的端口为 3342。
