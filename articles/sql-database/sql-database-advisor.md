---
title: 针对单个数据库和池数据库的数据库顾问性能建议
description: Azure SQL 数据库为单个数据库和池数据库提供建议，这些数据库可以提高 Azure SQL 数据库中的查询性能。
...
---
# <a name="database-advisor-performance-recommendations-for-single-and-pooled-databases"></a>针对单个数据库和池数据库的数据库顾问性能建议

Azure SQL 数据库与应用程序一起自行学习和进行适应性调整。 对于单个数据库和池数据库，SQL 数据库具有许多数据库顾问，这些顾问提供自定义建议，使您能够最大限度地提高性能。 这些数据库顾问持续评估和分析使用历史记录，并根据有助于提高性能的工作负载模式提供建议。

## <a name="performance-overview"></a>性能概述

性能概述提供数据库性能摘要，并帮助您进行性能调整和故障排除。

![Azure SQL 数据库性能概述](./media/sql-database-performance/performance-overview-annotated.png)

- “建议”**** 磁贴提供数据库的优化建议明细（如果建议超出 3 条，则将显示前 3 条）。 单击此磁贴将带您到**[性能建议选项](sql-database-advisor-portal.md#viewing-recommendations)**。
- “**优化活动**”磁贴提供正在进行的和已完成的数据库优化操作摘要，可快速查看优化活动的历史记录。 单击此磁贴可转到数据库的完整优化历史记录视图。
- **"自动调整"** 磁贴显示数据库**[的自动调优配置](sql-database-automatic-tuning-enable.md)**（自动应用于数据库的调优选项）。 单击此磁贴可打开自动化配置对话框。
...
适用于 Azure SQL 数据库中的单个数据库和池数据库的性能建议选项包括：
...
| **参数化查询建议（预览）** - 建议强制参数化的情况下，你有一个或多个查询不断重新编译，但最终具有相同的查询执行计划。 | 是 | 否 |
...
要应用绩效建议，请参阅[应用建议](sql-database-advisor-portal.md#applying-recommendations)。 要查看建议的状态，请参阅[监视操作](sql-database-advisor-portal.md#monitoring-operations)。
...
使用建议创建的索引始终都会标记为 auto_created。 通过查看[sys.index 视图](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-indexes-transact-sql)，可以查看自动创建哪些索引。 自动创建的索引不会阻止 ALTER/RENAME 命令。
...
如果 CPU、数据 IO 或日志 IO 在前 30 分钟内高于 80%，则创建索引建议将延迟。 如果创建索引后可用存储低于 10%，则建议会进入错误状态。 如果几天后自动优化仍认为索引会很有益，此过程再次启动。
