...
本文介绍如何使用[SQL 服务器管理工作室 （SSMS）](https://msdn.microsoft.com/library/hh213248.aspx)中的[始终加密向导](https://msdn.microsoft.com/library/mt459280.aspx)，使用数据库加密来保护 SQL 数据库中的敏感数据。 它还演示如何将加密密钥存储在 Windows 证书存储中。
...
* 使用 SSMS 中的"始终加密"向导创建[始终加密的密钥](https://msdn.microsoft.com/library/mt163865.aspx#Anchor_3)。
...
* Azure 帐户和订阅。 如果没有，请注册免费[试用](https://azure.microsoft.com/pricing/free-trial/)。
...
* [视觉工作室](https://www.visualstudio.com/downloads/download-visual-studio-vs.aspx)。
...
1. 打开 SSMS。 （单击 **"连接** > **数据库引擎**"以打开"**连接到服务器"** 窗口（如果窗口未打开）。
...
加密每位患者的“SSN”**** 和“出生日期”**** 信息。 **SSN**列将使用确定性加密，它支持相等查找、联接和分组。 **BirthDate** 列将使用随机加密，该加密不支持操作。

将**SSN**列的**加密类型**设置为**确定性**，将 **"出生日期"** 列设置为**随机化**。 单击“下一步”****。
...
您可以通过访问**诊所** > **安全** > **始终加密密钥**来验证 SSMS 中密钥的创建。 现在，可以看到向导生成的新密钥了。
