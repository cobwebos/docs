...
ms.locfileid: "80238662"
---
# <a name="accelerated-database-recovery"></a>加速数据库恢复

**加速数据库恢复 （ADR）** 是一种 SQL 数据库引擎功能，通过重新设计 SQL 数据库引擎恢复过程，大大提高了数据库可用性，尤其是在存在长时间运行的事务的情况下。 ADR 目前可用于 Azure SQL 数据库的单个、弹性池和托管实例，以及 Azure SQL 数据仓库中的数据库（当前处于预览状态）。 ADR 的主要优势在于：
...
- **主动日志截断**
  ...
  使用 ADR 时，事务日志会被大幅截断，即使在存在活动长时间运行的事务时也是如此，从而防止事务日志失去控制。
...
基于此设计，SQL 数据库引擎从意外重启中恢复所需的时间（大致）与故障时系统中时间最长的活动事务的大小成正比。 恢复需要回滚所有未完成的事务。 所需的时间长度与事务已执行的工作及其处于活动状态的时间成正比。 因此，SQL Server 恢复过程可能需要很长时间才能存在长时间运行的事务（例如针对大型表的大型批量插入操作或索引生成操作）。

此外，基于此设计，取消/回滚大型事务也可能需要很长时间，因为它使用与上述流程相同的撤消恢复阶段。

此外，SQL 数据库引擎无法截截事务日志时，有长时间运行的事务，因为他们的相应日志记录是恢复和回滚过程所必需的。 由于 SQL 数据库引擎的这种设计，一些客户曾经面临这样的问题：事务日志的大小会变大并消耗大量的驱动器空间。
...
- **分析阶段**
  ...
  添加为非版本化操作的 sLog 和复制日志记录，该过程与以前相同。
...
- 具有长时间运行事务的工作负载。
- 已看到活动事务导致事务日志显著增长的工作负载。  
- 由于 SQL Server 长时间运行恢复（如意外的 SQL Server 重新启动或手动事务回滚），经历了长时间数据库不可用的工作负载。
