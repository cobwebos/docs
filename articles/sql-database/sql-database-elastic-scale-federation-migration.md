...
对于弹性数据库工具 API，将使用 **RangeShardMap** 类中的 **OpenConnectionForKey** 方法，通过[数据相关路由](sql-database-elastic-scale-data-dependent-routing.md)建立与特定分片的连接。
...
本部分中的这些步骤是必要的，但是可能不适用于出现的所有迁移方案。有关详细信息，请参阅[弹性数据库工具的概念性概述](sql-database-elastic-scale-introduction.md)和 [API 参考](http://go.microsoft.com/?linkid=9862604)。
