...
通过 [C#](sql-database-get-started-csharp.md) 或 [PowerShell](sql-database-get-started-powershell.md) 执行这些相同任务。
...
5. 如果有多个订阅，请选择一个订阅。
6. 在“资源组”下，单击“新建”，为第一个资源组提供名称，如“my-resource-group”。绿色的对勾表示已提供有效名称。

    ![新 SQL 数据库 4](./media/sql-database-get-started/sql-database-new-database-4.png)

7. 在“选择源”下，单击“示例”，然后在“选择示例”下，单击“AdventureWorksLT [V12]”。
...
13. 在“密码”和“确认密码”下，为服务器主体登录帐户提供密码，如 "p@ssw0rd1"。绿色的对勾表示已提供有效密码。
...
18. 在“SQL 数据库”边栏选项卡的“定价层”下，单击“S2 标准”，然后单击“基本”，为第一个数据库选择开销最少的定价层。之后可以随时更改定价层。
...
>[AZURE.IMPORTANT]部署完成后，就在 Azure 中创建好新的 Azure SQL 服务器和数据库。使用 SQL Server 工具无法连接到新服务器或数据库，除非创建服务器防火墙规则，对 Azure 外部的连接开放 SQL 数据库防火墙。
