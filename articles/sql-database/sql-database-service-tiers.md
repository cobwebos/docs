...
>[AZURE.NOTE]Web 和 Business Edition 即将停用。了解如何[升级 Web 和 Business Edition](/documentation/articles/sql-database-upgrade-new-service-tiers)。如果你打算继续使用 Web 和 Business Edition，请阅读[版本停用常见问题](http://azure.microsoft.com/pricing/details/sql-database/web-business/)。
...
若要更好地了解 DTU，请参阅本主题中的 [DTU 部分](/documentation/articles/#understanding-dtus)。
...
### 用于弹性数据库池的服务层
除了创建和缩放单一数据库外，你还可以选择管理[弹性数据库池](sql-database-elastic-pool)中的多个数据库。弹性数据库池中的所有数据库共享一组公用资源。性能特征由*弹性数据库事务单位* (eDTU) 数度量。与单一数据库一样，弹性数据库池有三个服务层：**基本**、**标准**和**高级**。对于弹性数据库，这三个服务层仍定义整体性能限制和多个功能。
...
## 后续步骤
[SQL 数据库定价](http://azure.microsoft.com/pricing/details/sql-database/)中提供了有关这些层的价格详细信息。
...
现在，你已了解有关 SQL 数据库层的信息，可使用[免费试用版](http://azure.microsoft.com/pricing/free-trial/)来试用这些层，并了解[如何创建你的第一个 SQL 数据库](/documentation/articles/sql-database-get-started)！
