...
> [!IMPORTANT]
> [超大规模数据库（预览版）](sql-database-hyperscale.md)仅可使用基于 vCore 的购买模型，并且仅适用于逻辑服务器的单一数据库或托管实例内的单一数据库。 
