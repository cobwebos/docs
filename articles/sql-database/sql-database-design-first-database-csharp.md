...
4. 单击“服务器”即可使用现有的数据库服务器，或者创建并配置新的数据库服务器。 选择现有服务器或单击“创建新服务器”，在“新建服务器”表单中填写以下信息：
...
SQL 数据库服务会在服务器级别创建 IP 防火墙。 此防火墙会阻止外部应用程序和工具连接到服务器和服务器上的任何数据库，除非防火墙规则允许其 IP 通过防火墙。 若要启用与单一数据库的外部连接，必须首先为 IP 地址（或 IP 地址范围）添加 IP 防火墙规则。 按照这些步骤创建 [SQL 数据库服务器级 IP 防火墙规则](sql-database-firewall-configure.md)。
