...
在 Azure 中, 可将 SQL Server 工作负荷作为托管服务 ([PaaS](https://azure.microsoft.com/overview/what-is-paas/)) 或托管基础结构 ([IaaS](https://azure.microsoft.com/overview/what-is-iaas/)) 运行。 在 PaaS 中，有多个部署选项，每个部署选项中有多个服务层级。 在 PaaS 与 IaaS 之间作出抉择时，需要提出的一个重要问题是，你是要自行管理数据库、应用修补程序、创建备份，还是将这些操作委托给 Azure？
