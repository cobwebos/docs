...
- [**SQL 托管实例**](sql-database-managed-instance.md)：最适合大多数迁移到云。 托管实例是系统和用户数据库的集合，其中包含一组随时可直接迁移的共享资源。 最适合用于需要使用最新的 SQL Server 稳定功能，并且在进行极少量更改的情况下迁移到云中的新应用程序或现有本地应用程序。 托管实例类似于 Microsoft SQL Server[数据库引擎](https://docs.microsoft.com/sql/database-engine/sql-server-database-engine-overview)的实例，该引擎为数据库和其他实例范围功能提供共享资源。 托管实例支持从本地迁移数据库，所需的数据库更改极少或没有。 此选项提供 Azure SQL 数据库的所有 PaaS 优势，但同时也添加了以前只在 SQL VM 中提供的功能。 这包括本机虚拟网络 (VNet)，以及与本地 SQL Server 的接近 100% 的兼容性。 托管实例提供完全的 SQL Server 访问和功能兼容性，可用于将 SQL Server 迁移到 Azure。


- [**SQL 虚拟机**](../virtual-machines/windows/sql/virtual-machines-windows-sql-server-iaas-overview.md)：最适合需要操作系统级访问的迁移和应用程序。 SQL 虚拟机随时可直接迁移，适用于需要快速迁移到云中且只需进行极少量更改甚至不需要任何更改的现有应用程序。 SQL 虚拟机针对要迁移到 Azure 的 SQL Server 实例和底层 OS 提供全面的管理控制度。 想要快速完成开发和测试方案，但又不想购买本地 SQL Server 非生产硬件。 SQL 虚拟机属于“基础结构即服务 (IaaS)”行业类别，可让你在 Azure 云中的完全托管式虚拟机 (VM) 上运行 SQL Server。** SQL 虚拟机也在 Microsoft 所拥有、托管及维护的标准化硬件上运行。 使用 SQL 虚拟机时，您可以立即为 SQL Server 映像中已包含的 SQL Server 许可证付费，也可以轻松使用现有许可证。 还可以根据需要停止或恢复 VM。 在云中安装和托管的 SQL Server 将在 Azure 上运行的 Windows Server 或 Linux 虚拟机上运行，也称为基础结构即服务 (IaaS)。 SQL 虚拟机非常适合用于迁移本地 SQL Server 数据库和应用程序，无需进行任何数据库更改。 所有当前 SQL Server 版本都可安装在 IaaS 虚拟机中。 与 SQL 数据库和 SQL 托管实例之间的最重要差别在于，SQL Server VM 允许完全控制数据库引擎。 可以选择维护/修补开始时间、将恢复模式更改为简单模式或批量记录模式、根据需要暂停或启动服务，并且可以完全自定义 SQL Server 数据库引擎。 控制度的提高也意味着在管理虚拟机方面需要承担更大的责任。
...
- [成本](#cost) - PaaS 和 IaaS 选项都包括基本价格，其中涵盖了底层基础结构和许可证的费用。 但是，使用 IaaS 选项时，需要投入额外的时间和资源来管理数据库，而 PaaS 的价格已包括这些管理功能的费用。 IaaS 选项使您能够在不使用资源来降低成本时关闭资源，而 PaaS 版本始终处于运行，除非您删除资源并在需要时重新创建资源。
...
无论是现金不足的新公司，或是在预算有限的情况下运作的已成立公司的小组，有限资金经常是决定数据库托管方式的主要考虑因素。 在本节中，您将了解 Azure 中有关这两个关系数据库选项的计费和许可基础知识：SQL 数据库和 SQL 虚拟机。 还将了解如何计算应用程序总成本。
...
- 使用 SQL 数据库，您可以从基本层从 5 美元/月开始的各种价格中选择适合您需求的服务层。
...
对于**SQL 虚拟机**，Microsoft 提供的 SLA 可用性为 99.95%，仅涵盖虚拟机。 此 SLA 不涵盖 VM 上运行的进程（例如 SQL Server），并且要求在可用性集中托管至少两个 VM 实例。 有关最新信息，请参阅 [VM SLA](https://azure.microsoft.com/support/legal/sla/virtual-machines/)。 为了在 VM 中实现数据库高可用性 (HA)，应在 SQL Server 中配置一个受支持的高可用性选项，例如 [Always On 可用性组](https://docs.microsoft.com/sql/database-engine/availability-groups/windows/always-on-availability-groups-sql-server)。 使用支持的高可用性选项不会提供更多 SLA，但使用户能够实现 >99.99% 的数据库可用性。

### <a name="time-to-move-to-azure"></a><a name="market"></a>迁移到 Azure 的时机

当开发人员的工作效率和新解决方案的快速上市时间至关重要时 **，SQL 数据库**（单个数据库或弹性池）是云设计应用程序的正确解决方案。 此选项提供类似于编程 DBA 的功能，非常适合云架构师和开发员，因为它能降低管理基础操作系统和数据库的需求。
...
- [请参阅第一个 Azure SQL 数据库](sql-database-single-database-get-started.md)以开始使用 SQL 数据库。
