...
你首先需要有一个 Azure 中的 SQL 数据库。你可以按照[开始使用 Microsoft Azure SQL 数据库](/documentation/articles/sql-database-get-started)中的说明快速创建一个 SQL 数据库。此处的示例基于你参考该文章创建的 AdventureWorks 示例数据库，但直到执行查询前，这些步骤适用于任何 SQL 数据库。
...
## 运行示例查询
连接完成后，可以运行示例查询。如果没有使用[开始使用 Microsoft Azure SQL 数据库](/documentation/articles/sql-database-get-started)中的 AdventureWorks 示例创建数据库，此查询将不起作用。请直接跳到“后续步骤”以了解详细信息。
