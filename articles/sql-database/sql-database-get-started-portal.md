...
1. 在“SQL 数据库”窗体中，键入或选择以下值： 
   
   - **数据库名称**：键入 mySampleDatabase。
...
1. 选择“服务器”，然后选择“创建新的服务器”。 
   
1. 在“新建服务器”窗体中，键入或选择以下值： 
   
   - **服务器名称**：键入 mySQLServer。
   ...
   >[!IMPORTANT]
   >请牢记或记录服务器管理员登录名和密码，以便可以登录服务器和数据库以获取此快速入门和其他快速入门。 如果忘记了登录名或密码，可在“SQL 服务器”页上获取登录名或重置密码。 若要打开“SQL 服务器”页，请在创建数据库后在数据库“概述”页上选择服务器名称。
...
1. 在数据库的 SQL 数据库页的左侧菜单中，选择“查询编辑器(预览)”。 
   ...
   ![登录查询编辑器](./media/sql-database-get-started-portal/query-editor-login.png)
...
若要转到[后续步骤](#next-steps)并了解如何使用多种不同方法连接和查询数据库，请保留此资源组、SQL 服务器和 SQL 数据库。 

使用完这些资源后，可以通过如下方式将其删除：

1. 在 Azure 门户的左侧菜单中，选择“资源组”，然后选择“myResourceGroup”。
1. 在资源组页上，选择“删除资源组”。 
1. 在字段中键入 myResourceGroup ，然后选择“删除”。

## <a name="next-steps"></a>后续步骤

- 必须创建服务器级防火墙规则，以从本地或远程工具连接到 Azure SQL 数据库。 有关详细信息，请参阅[创建服务器级防火墙规则](sql-database-get-started-portal-firewall.md)。
- 创建服务器级防火墙规则后，可以使用多种不同的工具和语言[连接和查询](sql-database-connect-query.md)数据库。 
