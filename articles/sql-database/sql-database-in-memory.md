...
最好规划在 Azure VM 上运行 ostress.exe。在 AdventureWorksLT 数据库所在的同一 Azure 地理区域中创建 [Azure 虚拟机](http://azure.microsoft.com/documentation/services/virtual-machines/)。但你可以改为在便携式计算机上运行 ostress.exe。
