...
description: 了解 Azure SQL 数据库和 Azure Synapse Analytics 如何使用登录名和用户帐户对用户进行身份验证。 还了解如何通过数据库角色和显式权限来授权登录名和用户执行操作和查询数据。
...
ms.locfileid: "82106434"
---
# <a name="authorizing-database-access-to-authenticated-users-to-sql-database-and-azure-synapse-analytics-using-logins-and-user-accounts"></a>使用登录名和用户帐户向经过身份验证的用户授权数据库访问 SQL 数据库和 Azure Synapse Analytics

本文介绍以下内容：

- 用于配置 Azure SQL 数据库和 Azure Synapse Analytics （以前称为 Azure SQL 数据仓库）以使用户能够执行管理任务和访问存储在这些数据库中的数据的选项。
...
> [!IMPORTANT]
> Azure SQL 数据库和 Azure Synapse Analytics （以前称为 Azure SQL 数据仓库）中的数据库将在本文的其余部分统称为数据库或 Azure SQL （以简化）。

## <a name="authentication-and-authorization"></a>身份验证和授权

[**身份验证**](sql-database-security-overview.md#authentication)是证明用户身份的过程。 用户使用用户帐户连接到数据库。
当用户尝试连接到数据库时，它们会提供用户帐户和身份验证信息。 用户使用以下两种身份验证方法之一进行身份验证：
...
- [SQL 身份验证](https://docs.microsoft.com/sql/relational-databases/security/choose-an-authentication-mode#connecting-through-sql-server-authentication)。
  ...
  使用此身份验证方法时，用户将提交一个用户帐户名和关联的密码来建立连接。 对于链接到登录名的用户帐户，此密码存储在 master 数据库中，或存储在包含*未*链接到登录名的用户帐户的数据库中。
- [Azure Active Directory 身份验证](sql-database-aad-authentication.md)

  使用此身份验证方法，用户提交用户帐户名称，并请求服务使用存储在 Azure Active Directory 中的凭据信息。

**登录名和用户**：在 Azure SQL 中，数据库中的用户帐户可以与存储在 master 数据库中的登录名相关联，也可以是存储在单个数据库中的用户名。

- **登录名**是 master 数据库中的一个帐户，可以将一个或多个数据库中的用户帐户链接到该帐户。 登录时，用户帐户的凭据信息与登录名一起存储。
- **用户帐户**是任何数据库中可能存在但不需要链接到登录名的个人帐户。 使用未链接到登录名的用户帐户，凭据信息将与用户帐户一起存储。

使用数据库角色和显式权限管理访问数据和执行各种操作的[**授权**](sql-database-security-overview.md#authorization)。 授权是指分配给用户的权限，并确定允许用户执行的操作。 授权由用户帐户的数据库[角色成员身份](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/database-level-roles)和[对象级权限](https://docs.microsoft.com/sql/relational-databases/security/permissions-database-engine)控制。 作为最佳实践，应向用户授予所需的最低权限。

## <a name="existing-logins-and-user-accounts-after-creating-a-new-database"></a>创建新数据库后的现有登录名和用户帐户

创建第一个 Azure SQL 部署时，为该登录名指定管理员登录名和关联的密码。 此管理帐户称为**服务器管理员**。在部署过程中，主数据库和用户数据库中的登录名和用户的以下配置将会发生：
...
此时，仅使用单个 SQL 登录名和用户帐户对 Azure SQL 实例进行配置。 若要创建具有完全或部分管理权限的其他登录名，可以使用以下选项（具体取决于部署模式）：
...
- **在单个或共用部署中，创建具有有限管理权限的 SQL 登录名**
  ...
  - 使用[ALTER SERVER role](https://docs.microsoft.com/sql/t-sql/statements/alter-server-role-transact-sql)语句将`dbmanager`用户帐户`loginmanager`添加到`master`数据库中的和/或角色中（对于 Azure Synapse 分析，请使用[sp_addrolemember](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-addrolemember-transact-sql)语句）。

  > [!NOTE]
  > `dbmanager`和`loginmanager`角色与托管实例**部署无关。**

  对于单个或共用数据库，这些[特殊的 master 数据库角色](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/database-level-roles#special-roles-for--and-)的成员使用户拥有创建和管理数据库的权限，或者用于创建和管理登录名的权限。 在由作为`dbmanager`角色成员的用户创建的数据库中，成员将映射到`db_owner`固定数据库角色，并可使用`dbo`用户帐户登录和管理该数据库。 这些角色没有 master 数据库外的显式权限。
...
> [!IMPORTANT]
> 若要创建映射到 Azure AD 标识的包含用户，必须使用 SQL 数据库中的管理员 Azure AD 帐户进行登录。 在托管实例中，具有`sysadmin`权限的 SQL 登录名还可以创建 Azure AD 的登录名或用户。
