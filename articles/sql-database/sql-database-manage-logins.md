...
| 差异点 | 本地 SQL Server | Azure SQL Database |
|------------------------------------------------|-----------------------------------------------------------------------------|--------------------------------------------------|
| 管理服务器级别安全性的位置 | SQL Server Management Studio 的“对象资源管理器”中的“安全性”文件夹 | master 数据库以及通过 Azure 门户 |
| 用于创建登录名的服务器级别安全角色 | securityadmin 固定服务器角色。有关详细信息，请参阅“服务器级别角色” | master 数据库中的 loginmanager 数据库角色 |
...
| 用于创建数据库的服务器级别角色 | dbcreator 固定数据库角色。有关详细信息，请参阅“服务器级别角色” | master 数据库中的 dbmanager 数据库角色 |
...
master 数据库将跟踪有权创建数据库或其他登录名的登录名。只要你 CREATE、ALTER 或 DROP 登录名或数据库，就必须连接到 master 数据库。**master** 数据库还具有 ``sys.sql_logins`` 和 ``sys.databases`` 视图，可供你查看登录名和数据库。
...
为了使用你创建的登录名连接到 Windows Azure SQL 数据库，你必须首先通过使用 ``CREATE USER`` 语句为每个登录名授予数据库级别权限。有关详细信息，请参阅“向登录名授予数据库级别权限”。
...
SQL 数据库中的这一数据库级别权限模型与 Windows Azure SQL Server 的本地实例中的权限模型相同。有关详细信息，请参阅 SQL ServerSQL Server 联机丛书中的下列主题。
...
> [AZURE.NOTE]Windows Azure SQL 数据库中与安全相关的 Transact-SQL 语句在可用的参数方面可能稍有不同。有关详细信息，请参阅 [Azure SQL 数据库 Transact-SQL 参考](/documentation/articles/sql-database-transact-sql-information)。
