<properties
	pageTitle="SQL 数据库的弹性数据库池参考 | Microsoft Azure" 
...
- 在使用 [Azure 门户](https://manage.windowsazure.cn)、[PowerShell](/documentation/articles/sql-database-elastic-pool-powershell) 和 .NET 客户端库（仅限 Azure Resource Manager）时，支持创建和管理弹性数据库池；而[经典门户](https://manage.windowsazure.cn)和服务管理命令不受支持。
