...
本文列出了当前以公共预览版提供的 SQL 数据库功能。 有关 SQL 数据库更新和改进，请参阅[SQL 数据库服务更新](https://azure.microsoft.com/updates/?product=sql-database)。 有关其他 Azure 服务的更新和改进，请参阅[服务更新](https://azure.microsoft.com/updates)。
...
| 具有单个数据库和弹性池的 R 服务/机器学习 |有关详细信息，请参阅[Azure SQL 数据库中的机器学习服务](https://docs.microsoft.com/sql/advanced-analytics/what-s-new-in-sql-server-machine-learning-services?view=sql-server-2017#machine-learning-services-in-azure-sql-database)。|
|SQL Analytics|有关详细信息，请参阅[Azure SQL 分析](../azure-monitor/insights/azure-sql.md)。|
...
| Feature | 详细信息 |
| ---| --- |
| <a href="/azure/sql-database/sql-database-instance-pools">实例池</a> | 将较小的 SQL 实例迁移到云的便捷且经济高效的方法。 |
...
- [服务辅助子网配置](https://azure.microsoft.com/updates/service-aided-subnet-configuration-for-managed-instance-in-azure-sql-database-available/)管理子网配置的安全便捷方法，在托管实例控制数据流量时确保管理流量的不间断流动
- [透明数据加密 （TDE） 带自带密钥 （BYOK）](https://azure.microsoft.com/updates/general-avilability-transparent-data-encryption-with-customer-managed-keys-for-azure-sql-database-managed-instance/)支持自带密钥 （BYOK） 方案，以进行静态数据保护，并允许组织为密钥和数据分开管理职责。
...
2019 年上半年在托管实例部署模型中启用了以下功能：
  ...
  - 托管实例在以下 Azure 政府区域中可用于 GA（美国德克萨斯州州长、亚利桑那州州长）以及中国北 2 和中国东部 2。 它还可用于以下公共区域：澳大利亚中部、澳大利亚中部 2、巴西南部、法国南部、阿联酋中部、阿联酋北部、南非北部、南非西部。
...
|[@querysp_send_db_mail不支持参数](#-parameter-not-supported-in-sp_send_db_mail)|2019 年 4 月|无解决方法||
...
|[不支持同一实例中的两个数据库上的事务作用域](#transaction-scope-on-two-databases-within-the-same-instance-isnt-supported)||具有解决方法||
|[CLR 模块和链接的服务器有时无法引用本地 IP 地址](#clr-modules-and-linked-servers-sometimes-cant-reference-a-local-ip-address)||具有解决方法||
|从 Azure Blob 存储还原数据库后，未使用 DBCC CHECKDB 验证数据库一致性。||已解决|2019 年 11 月|
...
如果将非 sysadmin 登录名添加到任何 [SQL 代理固定数据库角色](https://docs.microsoft.com/sql/ssms/agent/sql-server-agent-fixed-database-roles)，则会出现以下问题：需要向主存储过程授予显式 EXECUTE 权限才能使这些登录名正常工作。 如果遇到此问题，将显示错误消息"对象上的 EXECUTE 权限被拒绝<object_name>（Microsoft SQL 服务器，错误：229）"。

**解决方法**：将登录添加到 SQL 代理固定数据库角色之一：SQLAgentUserRole、SQLAgentReaderRole 或 SQLAgent仲裁者Role后，对于添加到这些角色的每个登录者，将执行以下 T-SQL 脚本，以显式授予列出的存储过程的 EXECUTE 权限。
...
**解决方法：** 使用[SQL Server 管理工作室](/sql/relational-databases/in-memory-oltp/monitor-and-troubleshoot-memory-usage#bkmk_Monitoring)[监视内存中 OLTP 存储使用情况](https://docs.microsoft.com/azure/sql-database/sql-database-in-memory-oltp-monitoring)，以确保工作负载使用不超过可用内存。 提高基于 vCore 数的内存限制，或者优化工作负荷，让其使用较少的内存。
...
**解决方法**：等待还原过程完成，或者如果创建或更新服务层操作具有更高的优先级，则取消还原过程。
...
**解决方法**：定期运行`ALTER RESOURCE GOVERNOR RECONFIGURE`或作为 SQL 代理作业的一部分运行，该作业在实例启动时执行 SQL 任务，如果使用[资源调控器](/sql/relational-databases/resource-governor/resource-governor)。
...
**解决方法：** 在更新服务层并在更新服务层之前停止任何使用跨数据库服务代理对话框的对话的活动，并在更新后重新初始化它们。 如果在更改服务层级后存在任何未传递的剩余消息，请从源队列中读取消息，然后将其重新发送到目标队列。
...
**解决方法**： 使用 sys.database 视图从物理数据库名称解析实际数据库名称，以 GUID 数据库标识符的形式指定
...
**解决方法：** 使用[SqlConnection.更改数据库（字符串）](/dotnet/api/system.data.sqlclient.sqlconnection.changedatabase)在连接上下文中使用另一个数据库，而不是使用两个连接。
