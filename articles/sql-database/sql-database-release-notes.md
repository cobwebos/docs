...
- [服务辅助子网配置](https://azure.microsoft.com/updates/service-aided-subnet-configuration-for-managed-instance-in-azure-sql-database-available/)管理子网配置的一种安全且方便的方式，当托管实例可确保管理流量中断时控制数据流量
- [带有创建自己的密钥（BYOK）的透明数据加密（TDE）](https://azure.microsoft.com/updates/general-avilability-transparent-data-encryption-with-customer-managed-keys-for-azure-sql-database-managed-instance/)可为静态数据保护启用自带密钥（BYOK）方案，并使组织能够分隔密钥和数据的管理职责。
...
|[不支持模拟 Azure AD 登录类型](#impersonification-of-azure-ad-login-types-is-not-supported)|7月2019|无解决方法||
|[@querysp_send_db_mail 中不支持参数](#-parameter-not-supported-in-sp_send_db_mail)|Apr 2019|无解决方法||
...
|使用安全连接为外部（非 Azure）邮件服务器数据库邮件功能||已解决|2019 年 10 月|
|托管实例中不支持包含的数据库||已解决|2019 年 8 月|

### <a name="permissions-on-resource-group-not-applied-to-managed-instance"></a>对资源组的权限不适用于托管实例

应用于资源组（RG）的托管实例参与者 RBAC 角色不会应用到托管实例，因此不起作用。

**解决方法**：安装程序在订阅级别为用户托管实例参与者角色。
...
如果将非 sysadmin 登录名添加到任何 [SQL 代理固定数据库角色](https://docs.microsoft.com/sql/ssms/agent/sql-server-agent-fixed-database-roles)，则会出现以下问题：需要向主存储过程授予显式 EXECUTE 权限才能使这些登录名正常工作。 如果遇到此问题，将显示错误消息 "对象 <上的 EXECUTE 权限被拒绝 object_name> （Microsoft SQL Server，错误：229）"。
...
**解决方法**：使用 sys.databases 视图解析物理数据库名称中的实际数据库名称，以 GUID 数据库标识符的形式指定
