...
[恢复管理器](https://docs.microsoft.com/dotnet/api/microsoft.azure.sqldatabase.elasticscale.shardmanagement.recovery.recoverymanager)类使ADO.NET应用程序能够轻松检测和更正分片数据库环境中全局分片映射 （GSM） 和本地分片映射 （LSM） 之间的任何不一致。
...
* [使用弹性数据库工具入门](sql-database-elastic-scale-get-started.md)  
...
在删除分片前，分片映射反映了 GSM 中的分片位置。 由于已删除分片，假设这是特意的，而且分片键范围已不再使用中。 如果不是这种情况，则可以执行时间点还原。 以从较早的时间点还原分片。 （在这种情况下，请查看以下部分以检测分片不一致。要恢复，请参阅[时间点恢复](sql-database-recovery-using-backups.md)。
...
* *分片MapName*参数是分片映射名称。 仅当多个分片映射由同一分片映射管理器管理时才是必需的。 可选。
