---
title: 将数据库迁移到单个/共用数据库 SQL Server
...
- 在完成之前，创建快照会锁定相关联的表，因此请适当进行快照计划。
