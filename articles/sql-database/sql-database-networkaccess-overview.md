---
title: 网络访问控件
description: 概述适用于 Azure SQL 数据库和 Azure Synapse Analytics 的网络访问控制，以管理访问权限，并配置单个或共用数据库。
...
> [!NOTE]
> 本文适用于 Azure SQL server，以及在 Azure SQL server 上创建的 SQL 数据库和 Azure Synapse 分析数据库。 为简单起见，在同时引用 SQL 数据库和 Azure Synapse 分析时，将使用 SQL 数据库。
...
> [!TIP]
> 尽管指定 Location 参数，但 AzNetworkServiceTag 返回 SQL 服务标记的全局范围。 请务必将范围筛选为托管同步组所用中心数据库的区域
