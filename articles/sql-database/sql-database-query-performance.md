...
> [!NOTE]
> Query Store 需要捕获数小时的数据才能使 SQL 数据库提供查询性能见解。 如果在某段时间内数据库没有任何活动，或查询存储不处于活动状态，则在显示该期间时图表是空的。 如果查询存储未运行，随时可以启用它。   
> 
> 
