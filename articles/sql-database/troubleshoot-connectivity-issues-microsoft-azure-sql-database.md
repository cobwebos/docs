...
description: 解决与 Microsoft Azure SQL 数据库的连接问题
...
ms.locfileid: "71974417"
---
# <a name="troubleshooting-connectivity-issues-with-microsoft-azure-sql-database"></a>解决与 Microsoft Azure SQL 数据库的连接问题

与 Azure SQL 数据库连接失败时，会出现错误消息。 连接问题可能是 SQL Azure 数据库重新配置、防火墙设置、连接超时或不正确的登录信息造成的。 此外，如果达到了某些 Azure SQL Database 资源的最大限制，则无法连接到 Azure SQL 数据库。
...
2. 如果不存在任何已知的中断，请在[Microsoft Azure 支持网站](http://azure.microsoft.com/support/options)上打开支持案例。
...
之所以出现此问题，是因为应用程序无法连接到服务器。

若要解决此问题，请尝试执行以下部分中的步骤（按顺序），其中标题为**修复常见连接问题的步骤**。
...
### <a name="error-40-could-not-open-a-connection-to-sql-server"></a>错误40：无法打开与 SQL Server 的连接

**详细错误**

``A network-related or instance-specific error occurred while establishing a connection to SQL Server. The server was not found or was not accessible. Verify that the instance name is correct and that SQL Server is configured to allow remote connections. (provider: Named Pipes Provider, error: 40 - Could not open a connection to SQL Server)``

### <a name="error-10053-a-transport-level-error-has-occurred-when-receiving-results-from-the-server"></a>错误10053：在接收来自服务器的结果时发生传输级错误。

**详细错误**

``10053: A transport-level error has occurred when receiving results from the server. (Provider: TCP Provider, error: 0 - An established connection was aborted by the software in your host machine)``

出现这些问题是因为应用程序无法连接到服务器。

若要解决此问题，请尝试执行以下部分中的步骤（按顺序），其中标题为**修复常见连接问题的步骤**。

## <a name="cannot-connect-to--servername--due-to-firewall-issues"></a>由于防火墙问题，无法连接到 < servername >

### <a name="error-40615-cannot-connect-to--servername-"></a>错误 40615：无法连接到 < servername >

若要解决此问题，请[使用 Azure 门户在 SQL 数据库上配置防火墙设置。](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure)
...
## <a name="unable-to-login-to-the-server-errors-18456-40531"></a>无法登录到服务器（错误18456、40531）
...
若要解决此问题，请与服务管理员联系以提供有效的 SQL 用户名和密码。

服务管理员通常可使用以下步骤添加登录名：

1. 使用 SQL Server Management Studio （SSMS）登录到服务器。
2. 使用以下 SQL 查询来检查是否已禁用该登录名：
...
4. 如果 SQL 登录用户名不存在，请使用 SSMS 创建它：

   1. 双击“安全性”将其展开。 
   2. 右键单击“登录”，然后选择“新建登录名”。 
   3. 可以在生成的带占位符的脚本中，编辑和运行以下 SQL 查询：
   ...
   ```       
5. 双击“数据库”。 
6. 选择一个数据库，以便向用户授予该数据库的权限。
7. 双击“安全性”。 
8. 右键单击“用户”，然后选择“新建用户”。 
9. 可以在生成的带占位符的脚本中，编辑和运行以下 SQL 查询： 
...
### <a name="systemdataentitycoreentityexception-the-underlying-provider-failed-on-open"></a>System.web. EntityException：基础提供程序打开失败。
...
由于连接或查询问题，可能发生这些异常。 若要确认此错误是由连接问题引起的，请检查下面的 "**确认错误是否是由于连接问题引起的"** 一节。

由于应用程序无法连接到服务器，导致连接超时。 若要解决此问题，请尝试执行以下部分中的步骤（按顺序），其中标题为**修复常见连接问题的步骤**。

## <a name="transient-errors-errors-40197-40545"></a>暂时性错误（错误40197、40545）

### <a name="error-40197-the-service-has-encountered-an-error-processing-your-request-please-try-again-error-code--code-"></a>错误40197：该服务在处理请求时遇到错误。 请重试。 错误代码 < 代码 >

之所以出现此问题，是因为在后端重新配置/故障转移过程中遇到暂时性错误。

若要解决此问题，请等待一小段时间，然后重试。 除非此问题仍然存在，否则不需要支持案例。

最佳做法是确保重试逻辑已经到位。 有关重试逻辑的详细信息，请参阅对[SQL 数据库的暂时性故障和连接错误进行故障排除](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues)。

## <a name="connection-terminated-due-to-hitting-some-system-defined-limit"></a>由于达到了某个系统定义的限制，连接终止
...
* 验证是否存在长时间运行的查询：
  ...
  > [!NOTE]
  > 这是一种最简单的方法，可能不一定能解决问题。

  1. 通过执行以下 SQL 查询，检查[sys.databases _exec_requests](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql)视图以查看任何阻塞请求：
...
* 如果在处理阻塞和长时间运行的查询时数据库一直达到其限制，请考虑升级到新的预览版本（如[标准版或高级版](https://azure.microsoft.com/pricing/details/sql-database/)）之一。
...
### <a name="error-40501-the-service-is-currently-busy"></a>错误40501：服务当前正忙
...
以下步骤可帮助你解决问题，或提供可考虑的其他选项。

1. 使用 Azure 门户中的仪表板检查数据库的当前大小。

   > [!NOTE]
   > 若要确定哪些表消耗最多的空间和可能的清理候选项，可以使用以下 SQL 查询：
...
3. 如果数据库大小已超过版本所支持的最大大小，则可以执行以下步骤之一：
   1. 执行正常的数据库清理活动（使用截断/删除等清理不需要的数据，或使用 SSIS、bcp 等将数据移出）
   2. 请将数据分区或删除、删除索引或查阅文档以找到可能的解决方案。 
...
### <a name="error-40549-session-is-terminated-because-you-have-a-long-running-transaction"></a>错误40549：由于有长时间运行的事务，已终止会话。

**详细错误**

``40549: Session is terminated because you have a long-running transaction. Try shortening your transaction.``

如果重复遇到此错误消息，请尝试以下步骤来解决此问题： 

1. 通过执行以下 SQL 脚本，检查 sys.databases _exec_requests 视图，以查看具有较高值的已打开会话：
...
* 此问题是由于插入、更新或删除操作导致的。 尝试通过实现批处理或拆分为多个较小的事务来减少立即操作的行数。
* 发生此问题的原因是索引重新生成操作。 请确保遵循以下公式：表中受影响的行数 * （以字节 + 80 更新的字段的平均大小 +） < 2 GB

  > [!NOTE]
  > 对于索引重新生成，应将更新字段的平均大小替换为平均索引大小。
...
之所以出现此问题，是因为该帐户没有对 master 数据库的访问权限。 但是，默认情况下 SQL Server Management Studio （SSMS）尝试连接到 master 数据库。

若要解决此问题，请执行以下步骤：

1. 在 SSMS 的登录屏幕上，单击 "**选项**"，然后单击 "**连接属性**"。 
2. 在 "**连接到数据库**" 中，键入用户的默认数据库名称作为默认登录数据库，然后单击 "**连接**"。
...
## <a name="confirm-whether-an-error-is-due-to-a-connectivity-issue"></a>确认错误是否由连接问题引起

若要确认错误是否是由于连接问题导致的，请查看用于显示调用的帧的堆栈跟踪，这些调用会打开类似于下面的连接（请注意，对**SqlConnection**类的引用）：
...
当异常由于查询问题而发生时，你会注意到一个类似于下面的调用堆栈（请注意对**SqlCommand**类的引用）。 在这些情况下，请[调整查询](http://blogs.msdn.com/b/sqlblog/archive/2013/11/01/is-my-query-running-fine-in-the-cloud.aspx)。
...
1. 请确保在应用程序服务器上启用了 TCP IP 作为客户端协议。 有关详细信息，请参阅[配置客户端协议](https://docs.microsoft.com/sql/database-engine/configure-windows/configure-client-protocols)。 在未安装 SQL Server 工具的应用程序服务器上，检查是否已通过运行**cliconfg.exe** （SQL Server 客户端网络实用工具）启用了 TCP IP。 
2. 检查应用程序的连接字符串，以确保正确配置该应用程序。 例如，请确保连接字符串指定了正确的端口（1433）和完全限定的服务器名称。
...
3. 请尝试增加连接**超时值**。 Microsoft 建议使用至少为30秒的连接超时。 
4. 使用[SQL Server Management Studio （SSMS）](https://docs.microsoft.com/azure/sql-database/sql-database-connect-query-ssms)、UDL 文件、ping 和 telnet 来测试应用程序服务器和 Azure SQL 数据库之间的连接。 有关详细信息，请参阅排查[连接问题](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues#diagnostics) [SQL Server 连接问题](https://support.microsoft.com/help/4009936/solving-connectivity-errors-to-sql-server)和诊断。

   > [!NOTE]
   > 作为故障排除步骤，你还可以尝试在不同的客户端计算机上测试连接。

5. 最佳做法是确保重试逻辑已经到位。 有关重试逻辑的详细信息，请参阅对[SQL 数据库的暂时性故障和连接错误进行故障排除](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues)。

如果前面的步骤不能解决你的问题，请尝试收集更多数据并联系支持人员。 如果你的应用程序是云服务，则启用日志记录。 此步骤返回失败的 UTC 时间戳。 此外，SQL Azure 返回跟踪 ID。 [Microsoft 客户支持服务](http://azure.microsoft.com/support/options/)可使用此信息。 

有关如何启用日志记录的详细信息，请参阅[在 Azure App Service 中启用 web 应用的诊断日志记录](https://azure.microsoft.com/documentation/articles/web-sites-enable-diagnostic-log/)。
