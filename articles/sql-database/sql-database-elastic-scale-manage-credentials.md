...
[弹性数据库客户端库](http://www.nuget.org/packages/Microsoft.Azure.SqlDatabase.ElasticScale.Client/)将凭据用于不同类型的操作 - 在创建或操作[分片映射管理器](sql-database-elastic-scale-shard-map-management.md)、引用现有的分片映射管理器获取有关分片的信息，以及连接到分片时尤其如此。下面讨论了用于这些类型的操作的凭据。
