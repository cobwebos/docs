...
- **混合数据同步：** 通过数据同步，可以在本地数据库和 Azure SQL 数据库之间保持数据同步，以启用混合应用程序。 此功能可能会吸引在考虑迁移到云中，并希望启用 Azure 应用程序的客户。
...
- **全球分布式应用程序：** 许多企业跨越多个地区，甚至几个国家/地区。 为了最大限度地缩短网络延迟时间，最好将数据存储在靠近的区域中。 借助 SQL 数据同步，可轻松同步世界各地区域中的数据库。
...
- **同步数据库**包含数据同步的元数据和日志。同步数据库必须是位于与中心数据库相同的区域中的 Azure SQL 数据库。 同步数据库的创建者和所有者均为客户。
...
由于 SQL 数据同步是基于触发器的服务，因此无法保证事务一致性。 Microsoft 保证最终进行所有更改，并且数据同步不会导致数据丢失。
...
- [监视 Azure SQL 数据与 Azure 监视器日志同步](sql-database-sync-monitor-oms.md)
- [Azure SQL 数据同步问题疑难解答](sql-database-troubleshoot-data-sync.md)
