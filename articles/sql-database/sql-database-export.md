...
- 要使导出在事务上保持一致，必须确保在导出期间未发生写入活动，或者从 Azure SQL 数据库[的事务一致性副本](sql-database-copy.md)导出。
