...
本主题提供可以用来向 Azure SQL 数据库发出 [CREATE ASSEMBLY](http://msdn.microsoft.com/zh-cn/library/ms189524.aspx) 语句的 C# 代码示例。对于 SQL Database，FROM 子句不能接受托管数据库的本地计算机上的简单格式路径。替代的做法是先将程序集 DLL 的二进制位编码成包含十六进制数的长字符串。然后，将该字符串指定为 FROM 子句中的值。
