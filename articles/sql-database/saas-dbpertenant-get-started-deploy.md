...
1. 登录到你的 Azure 帐户，如有必要，选择要使用的订阅。
...
1. 如果 `$OneTime = $false`，则负载生成器将启动后台作业，然后继续运行。 每隔 10 秒钟，它将监视是否预配了任何新租户。 如果设置了 `$OneTime = $true`，则负载生成器将启动后台作业，然后停止在前台运行。 在本教程中，保留为 `$OneTime = $false`。
...
1. 打开保存租户数据库的 tenants1-dpt-&lt;USER&gt; 服务器。
