---
title: 设置托管实例（Azure 门户）的警报和通知
description: 使用 Azure 门户创建 SQL 托管实例警报，当满足指定的条件时，该警报可以触发通知或自动化。
...
本文介绍如何使用 Azure 门户为 Azure SQL 托管实例数据库中的数据库设置警报。 当某些指标（例如实例存储大小或 CPU 使用率）达到阈值时，警报可以向您发送电子邮件或调用 Web 挂钩。 本文还提供了有关设置警报期限的最佳做法。
