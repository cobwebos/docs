...
SQL 数据库使用[自动数据库备份](sql-database-automated-backups.md)为数据库恢复提供三个选项。 可在数据库备份[保留期](sql-database-service-tiers.md)期间将数据库还原到：
...
还可以使用[自动数据库备份](sql-database-automated-backups.md)在任何区域中的逻辑服务器上创建[数据库副本](sql-database-copy.md)。 还可以将 SQL 数据库服务器配置为存储 Azure 恢复服务保管库的 Azure SQL 数据库备份，存储期可长达 10 年。 请参阅[长期备份保留](sql-database-long-term-retention.md)。
