...
若要使用 Azure 门户将单个或共用的已删除数据库恢复到删除时间，请打开 "服务器概述" 页，然后选择 "**已删除的数据库**"。 选择要还原的已删除数据库，然后键入将使用从备份还原的数据创建的新数据库的名称。
...
有关演示如何还原已删除实例数据库的示例 PowerShell 脚本，请参阅[使用 PowerShell 还原托管实例上的已删除数据库](sql-database-managed-instance-point-in-time-restore.md#restore-a-deleted-database)
...
当数据库因托管区域中的事件而不可用时，异地还原是默认的恢复选项。 可以将数据库还原到任何其他区域的服务器中。 创建备份后，将其异地复制到其他区域中的 Azure Blob 时会出现延迟。 因此，还原的数据库最多可以在原始数据库后一小时。 下图显示了从另一个区域中的最后一个可用备份进行的数据库还原。
