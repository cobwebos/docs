<properties 
	pageTitle="使用 Azure 预览门户升级到 Azure SQL 数据库 V12 | Microsoft Azure" 
	description="介绍如何使用 Azure 预览门户升级到 Azure SQL 数据库 V12，包括如何升级 Web 和企业数据库，以及如何升级 V11 服务器并将其数据库直接迁移到弹性数据库池。" 
...
# 使用 Azure 预览门户升级到 Azure SQL 数据库 V12
...
- **升级所有 Web 和企业数据库**：请参阅下面的[升级所有 Web 和企业数据库](/documentation/articles/sql-database-upgrade-server-portal#upgrade-all-web-and-business-databases)部分，或[使用 PowerShell 来升级数据库和服务器](sql-database-upgrade-server-powershell)。
...
- **如果客户端在 Azure VM 上，请打开这些端口**：如果客户端程序连接到 SQL 数据库 V12，而客户端运行在 Azure 虚拟机 (VM) 上，则必须打开虚拟机上的端口范围 11000-11999 和 14000-14999。有关详细信息，请参阅 [SQL 数据库 V12 的端口](sql-database-develop-direct-route-ports-adonet-v12)。



## 开始升级

1. 在 [Azure 预览门户](https://manage.windowsazure.cn)中，通过选择“浏览全部”>“SQL 服务器”并选择所需的服务器，浏览到要升级的服务器。
...
在 [Azure 预览门户](https://ms.portal.azure.com/)中浏览到 V12 服务器并单击“添加池”。
...
除了监视单个数据库之外，你还可以[使用门户](sql-database-elastic-pool-portal#monitor-and-manage-an-elastic-database-pool)或通过 [PowerShell](/documentation/articles/sql-database-elastic-pool-powershell#monitoring-elastic-databases-and-elastic-database-pools) 监视弹性数据库池。
...
- [创建弹性数据库池](sql-database-elastic-pool-portal)，并将部分或全部数据库添加到池中。
