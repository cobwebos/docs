---
title: 使用 Azure Monitor 日志监视 Azure SQL 数据同步 |Microsoft Docs
...
再也无需单独仔细查看每个同步组的日志就能查找问题。 通过使用自定义 Azure Monitor 视图，可以从任何订阅在一个位置监视所有同步组。 此视图显示对 SQL 数据同步客户很重要的信息。
...
不再需要在 Azure 门户中手动或通过 PowerShell、REST API 检查日志。 与[Azure Monitor 日志](https://docs.microsoft.com/azure/log-analytics/log-analytics-overview)，可以创建警报，直接转到需要查看发生错误时的人员的电子邮件地址。
...
实现自定义的 Azure Monitor 中的日志监视解决方案的 SQL 数据同步不会超过一小时通过执行以下操作：

需要配置三个组件：

-   SQL 数据同步日志数据输送到 Azure Monitor 日志 PowerShell runbook。

-   Azure Monitor 警报的电子邮件通知。
...
使用托管在 Azure 自动化中的 PowerShell runbook 拉取 SQL 数据同步日志数据并将其发送到 Azure Monitor 日志。 已包含示例脚本。 作为先决条件，需要有一个 Azure 自动化帐户。 然后，你需要创建 runbook 并安排它运行。 
...
10. 请为你的帐户和 SQL 数据同步配置执行所需的更改。 （如需更多详细信息，请参阅示例脚本。）
    ...
    3.  Azure 监视器将记录信息。 在 Azure 门户 | 设置 | 连接的源中查找此信息。 有关将数据发送到 Azure Monitor 日志的详细信息，请参阅[将数据发送到 HTTP 数据收集器 API （预览版） 使用 Azure Monitor 日志](../azure-monitor/platform/data-collector-api.md)。
...
此步骤中创建的 Azure Monitor 视图以可视化方式监视所有指定的同步组。 该视图包括多个组件：
...
**Azure Monitor 日志：** 可能会有与 Azure Monitor 日志，具体取决于你的使用情况关联的成本。 免费层包括每日 500 MB 的引入数据。 在大多数情况下，此解决方案预计每天引入的数据不超过 500 MB。 若要减少使用，请使用 runbook 中包含的“仅失败筛选”。 如果你每天使用的数据超过 500 MB，请升级到付费层，以避免在达到此限制时停止分析的风险。 有关详细信息，请参阅[Azure Monitor 日志定价](https://azure.microsoft.com/pricing/details/log-analytics/)。
