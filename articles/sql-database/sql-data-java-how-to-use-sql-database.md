...
## 概念
由于 Azure SQL Database 是基于 SQL Server 技术构建的，因此通过 Java 访问 SQL Database 与通过 Java 访问 SQL Server 非常相似。您可以本地部署应用程序（使用 SQL Server），然后通过仅更改连接字符串连接到 SQL Database。您可以对应用程序使用 SQL Server JDBC 驱动程序。但是，SQL Database 和 SQL Server 之间有一些可能影响您的应用程序的差别。有关详细信息，请参阅[指导原则和限制（SQL 数据库）](http://msdn.microsoft.com/library/windowsazure/ff394102.aspx)。
