...
| 130 | 内存中表的并行查询计划 | 如果查询针对的是内存中表，可以使用多个线程并可以并行运行，这意味着表通过 **MEMORY\_OPTIMIZED = YES** 子句创建。并行性可以使查询运行得更快。<br/><br/>常规 Transact-SQL 和用户存储过程支持此增强功能。但编译成 DLL 的本机存储过程不支持此功能。 |


## 列存储索引功能要求最低级别 130


在兼容性级别 130 上，查询优化器增强功能可以产生新的查询计划。对于涉及到列存储索引的已更改计划，更改通常涉及下列情况之一：
...
| 130 是最低<br/>必要<br/>级别 | 查询区域，<br/>列存储索引 | <br/>查询计划<br/>改进的详细信息 |
| :-- | :-- | :-- |
| 130 | 函数查询 | 在以下情况下，通过切换到批处理模式，性能得以改进：<br/><br/>• 涉及排序。<br/><br/>• 聚合*多个*不同的函数<br/>（以下列表中两个不同的加重号，每个一个函数）：<br/>&nbsp;&nbsp;&nbsp;▫ **COUNT** *或* **COUNT\_BIG**<br/>&nbsp;&nbsp;&nbsp;▫ **AVG** *或* **SUM**<br/>&nbsp;&nbsp;&nbsp;▫ **CHECKSUM\_AGG**<br/>&nbsp;&nbsp;&nbsp;▫ **STDEV** *或* **STDEVP**<br/><br/>• Window 聚合函数<br/>（在 [MSDN 的这篇文档](http://msdn.microsoft.com/library/ms189461.aspx)、以及 [Kathi Kellenberger 的这篇文档](http://www.bidn.com/blogs/KathiKellenberger/sql-server/4397/what-is-a-window-aggregate-function)中进行了介绍）：<br/>&nbsp;&nbsp;&nbsp;▫ **COUNT**、**COUNT\_BIG**、**SUM**、**AVG**、**MIN**、**MAX**、**CLR**<br/><br/>• Window [用户定义的](http://msdn.microsoft.com/library/ms131057.aspx)聚合：<br/>&nbsp;&nbsp;&nbsp;▫ [**CHECKSUM\_AGG**](http://msdn.microsoft.com/library/ms188920.aspx)、[**STDEV**](http://msdn.microsoft.com/library/ms190474.aspx)、[**STDEVP**](http://msdn.microsoft.com/library/ms176080.aspx)、[**VAR**](http://msdn.microsoft.com/library/ms186290.aspx)、[**VARP**](http://msdn.microsoft.com/library/ms188735.aspx)、[**GROUPING**](http://msdn.microsoft.com/library/ms178544.aspx)<br/><br/>• Window 聚合分析函数：<br/>&nbsp;&nbsp;&nbsp;▫ [**LAG**](http://msdn.microsoft.com/library/hh231256.aspx)、[**LEAD**](http://msdn.microsoft.com/library/hh213125.aspx)、[**FIRST\_VALUE**](http://msdn.microsoft.com/library/hh213018.aspx)、[**LAST\_VALUE**](http://msdn.microsoft.com/library/hh231517.aspx)、[**PERCENTILE\_CONT**](http://msdn.microsoft.com/library/hh231473.aspx)、 [**PERCENTILE\_DISC**](http://msdn.microsoft.com/library/hh231327.aspx)、[**CUME\_DIST**](http://msdn.microsoft.com/library/hh231078.aspx)、[**PERCENT\_RANK**](http://msdn.microsoft.com/library/hh213573.aspx) |
| 130 | 单线程串行查询计划 | 在单个线程上执行的查询可以在批处理模式下运行。这会使查询的执行速度更快。<br/><br/>查询计划可能会设计为单线程，或者查询可能会在 **MAXDOP 1** 下运行。 |
| 130 | 并行插入 | 查询计划可以并行执行一些插入操作。<br/<br/>本主题后面的[示例](#ExampleQueryParallelCciByCompatLevel)演示了此并行性。 |
| 130 | 反半联接 | 此运算符现在可以在批处理模式下运行。 |


## 常规功能要求最低级别 120


| 120 是最低<br/>必要<br/>级别 | 查询区域，<br/>常规 | <br/>查询计划<br/>改进的详细信息 |
| :-- | :-- | :-- |
...
| 120 | [创建和管理内存优化对象的存储](http://msdn.microsoft.com/zh-cn/library/dn133174.aspx) | 内存中 OLTP 引擎集成在 SQL Server 中。这让你可以在同一数据库中同时拥有 **MEMORY\_OPTIMIZED** 表和基于磁盘的传统表。 |
| 120 | [内存中 OLTP 的 Transact-SQL 支持](http://msdn.microsoft.com/zh-cn/library/dn133180.aspx) | 少量的 Transact-SQL 命令已经过改进，可以支持内存中的联机事务处理 (OLTP)。<br/><br/>例如，[CREATE PROCEDURE](http://msdn.microsoft.com/library/ms187926.aspx) 命令的新 **NATIVE\_COMPILATION** 关键字。 |
...
| 120 | 在聚合函数的表扫描阶段，<br/>聚合下推 | 通过完成查询计划中前面的过渡计算，使更少的数据需要复制到后面的阶段，可以提高性能。<br/><br/>适用于 **MIN**、**MAX**、**SUM**、**COUNT**、**AVG**。<br/><br/>仅适用于数据类型为 8 个字节或更少的情况，但不适用于字符串。 |
