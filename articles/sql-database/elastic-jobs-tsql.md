...
默认情况下，作业代理将创建输出表以存储返回的结果。 因此，与`CREATE TABLE`输出凭据关联的数据库主体必须至少具有以下权限：在数据库`ALTER`、、、、、、、、`SELECT``INSERT``DELETE`[索引目录视图](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-indexes-transact-sql)`SELECT`上。
...
[ ** \@target_group_name ]**"目标group_name"  
...
[ ** \@job_execution_id ] [** job_execution_id输出  
...
[ ** \@target_group_id =** = target_group_id输出 如果成功创建，则分配给作业的目标组标识号。 target_group_id 是类型为 uniqueidentifier 的输出变量，默认值为 NULL。
...
|[作业步骤](#jobsteps-view)     |     显示每项作业的当前版本中的所有步骤。    |
...
|**生命 周期**| nvarchar(50)|指示作业状态的值：“已创建”、“处理中”、“失败”、“成功”、“跳过”、“SucceededWithSkipped”|
...
|**job_id**|    uniqueidentifier    |作业的唯一 ID。|
|**job_version**    |int    |作业的版本（每次修改作业时都自动对其进行更新）。|
|**描述**    |nvarchar(512)| 对作业的说明。 enabled 为 bit    指示作业是已启用还是已禁用。 1 指示作业已启用，0 指示作业已禁用。|
...
|**命令**|   nvarchar(max)|  将要由弹性作业按 command_type 执行的命令。|
