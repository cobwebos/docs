...
| [Azure Active Directory (AAD) 身份验证](sql-database-aad-authentication.md) | 可以。 仅限 AAD 用户。 | 可以。 包括服务器级 AAD 登录名。 |
...
| [将数据库还原到 SQL Server](https://docs.microsoft.com/sql/relational-databases/backup-restore/back-up-and-restore-of-sql-server-databases#restore-data-backups) | 不能。 使用 BACPAC 或 BCP 而不是本机还原。 | 否，因为托管实例中使用的 SQL Server 数据库引擎的版本高于本地使用的任何 SQL Server RTM 版本。 改用 BACPAC、BCP 或事务复制。 |
...
| [TempDB](https://docs.microsoft.com/sql/relational-databases/databases/tempdb-database) | 可以。 [每个数据库的每个核心为 32GB 大小](sql-database-vcore-resource-limits-single-databases.md)。 | 可以。 [整个 GP 层的每个 vCore 为 24GB 大小，受 BC 层上的实例大小限制](sql-database-managed-instance-resource-limits.md#service-tier-characteristics)  |
...
| Windows 身份验证 | 否 | 否 |
| [Windows Server 故障转移群集](https://docs.microsoft.com/sql/sql-server/failover-clusters/windows/windows-server-failover-clustering-wsfc-with-sql-server) | 不能。 每个数据库附带提供[高可用性](sql-database-high-availability.md)的其他技术。 [简要介绍如何使用 Azure SQL 数据库确保业务连续性](sql-database-business-continuity.md)中对灾难恢复进行了讨论 | 不能。 每个数据库附带提供[高可用性](sql-database-high-availability.md)的其他技术。 [简要介绍如何使用 Azure SQL 数据库确保业务连续性](sql-database-business-continuity.md)中对灾难恢复进行了讨论 |
...
| [自动备份](sql-database-automated-backups.md) | 可以。 完整备份每 7 天一次，差异备份每 12 小时一次，日志备份每 5-10 分钟一次。 | 可以。 完整备份每 7 天一次，差异备份每 12 小时一次，日志备份每 5-10 分钟一次。 |
...
| 备份保留 | 可以。 默认为 7 天，最长 35 天。 | 可以。 默认为 7 天，最长 35 天。 |
...
| 公共 IP 地址 | 可以。 访问权限可以使用防火墙或服务终结点来限制。  | 可以。 需要显式启用，且必须在 NSG 规则中启用端口 3342。 可根据需要禁用公共 IP。 有关更多详细信息，请参阅[公共终结点](sql-database-managed-instance-public-endpoint-securely.md)。 | 
...
| 资源池 | 是，用作[弹性池](sql-database-elastic-pool.md) | 可以。 单个托管实例可以有多个共享同一资源池的数据库。 此外，还可以在可共享资源的[实例池（预览版）](sql-database-instance-pools.md)中部署多个托管实例。 |
...
| [Query Performance Insights (QPI)](sql-database-query-performance.md) | 是 | 不能。 使用 SQL Server Management Studio 和 Azure Data Studio 中的内置报告。 |
