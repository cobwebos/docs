...
例如，当您创建新的 Azure SQL 服务器名为*mysqlserver*时，SQL 数据库防火墙会阻止对服务器公共终结点的所有访问（该服务器可在*mysqlserver.database.windows.net*访问）。
...
### <a name="server-level-ip-firewall-rules"></a>服务器级别 IP 防火墙规则
  ...
  这些规则允许客户端访问整个 Azure SQL 服务器，即同一 SQL 数据库服务器内的所有数据库。 这些规则存储在 *master* 数据库中。 对于 Azure SQL Server，最多可以有 128 个服务器级别 IP 防火墙规则。 如果启用了 **"允许 Azure 服务和资源访问此服务器**设置"，则这计为 Azure SQL Server 的单个防火墙规则。
...
若要允许 Azure 内部托管的应用程序连接到 SQL 服务器，必须启用 Azure 连接。 在应用程序尝试从 Azure 连接到你的数据库服务器时，防火墙将验证是否允许 Azure 连接。 若要直接从 Azure 门户边栏选项卡中将其打开，可以设置防火墙规则，也可以在“防火墙和虚拟网络”设置中将“允许 Azure 服务和资源访问此服务器”切换为“启用”。************ 如果不允许连接，则该请求将不会访问 Azure SQL 数据库服务器。
...
此时会打开服务器的概述页。 它显示完全限定的服务器名称（如*mynewserver20170403.database.windows.net），* 并提供进一步配置的选项。
...
要删除服务器级 IP 防火墙规则，请执行*sp_delete_firewall_rule*存储过程。 以下示例删除规则 *ContosoFirewallRule*：
...
```
> [!TIP]
> 对于$servername指定服务器名称而不是完全限定的 DNS 名称，例如指定**mysqldbserver**而不是**mysqldbserver.database.windows.net**
...
- **本地防火墙配置：**
  ...
  在计算机可以访问 SQL 数据库之前，可能需要在计算机上创建针对 TCP 端口 1433 的防火墙例外。 若要在 Azure 云边界内部建立连接，可能需要打开其他端口。 有关详细信息，请参阅端口的"SQL 数据库：外部与内部"部分[，这些端口ADO.NET 4.5 和 SQL 数据库超过 1433。](sql-database-develop-direct-route-ports-adonet-v12.md)
...
- 确认公司网络环境允许来自 Azure 数据中心使用的计算 IP 地址范围（包括 SQL 范围）的入站通信。 可能需要将这些 IP 地址添加到允许列表。 请参阅[微软 Azure 数据中心 IP 范围](https://www.microsoft.com/download/details.aspx?id=41653)。  
...
- 有关可能需要打开的其他端口的信息，请参阅 1433 以外的端口的"SQL 数据库：外部与内部"部分[，用于ADO.NET 4.5 和 SQL 数据库](sql-database-develop-direct-route-ports-adonet-v12.md)
