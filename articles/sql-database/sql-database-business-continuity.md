...
为了缓解本地硬件和软件故障, SQL 数据库包括一个[高可用性体系结构, 该体系结构](sql-database-high-availability.md)可保证自动从这些故障中恢复到 99.995% 的可用性 SLA。  
...
您可以使用自动数据库备份将数据库还原到过去某个时间点。 这样, 你就可以从人为错误引起的数据损坏中恢复。 使用流程图还原, 可以在同一服务器中创建新的数据库, 该数据库表示出现损坏事件之前的数据状态。 对于大多数数据库, 还原操作所需的时间不会超过12个小时。 恢复非常大或非常活跃的数据库可能需要更长的时间。 有关恢复时间的详细信息，请参阅[数据库恢复时间](sql-database-recovery-using-backups.md#recovery-time)。 
