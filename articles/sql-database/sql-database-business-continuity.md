...
ms.locfileid: "67442039"
---
# <a name="overview-of-business-continuity-with-azure-sql-database"></a>使用 Azure SQL 数据库确保业务连续性的相关概述

Azure SQL 数据库中的**业务连续性**是指在遇到中断（尤其是计算基础结构的中断）时，使企业能够继续运营的机制、策略和过程。 在大多数情况下，Azure SQL 数据库将会处理云环境中可能发生的中断事件，并让应用程序和业务流程保持运行。 但是，有不能由 SQL 数据库自动如处理某些中断性事件：
...
- 通常由应用程序 bug 或人为失误导致的数据损坏或删除。 此类故障是特定于应用程序，并通常不能检测到的数据库服务。
- 可能由自然灾害造成的数据中心服务中断。 针对这种情况，需要采取某种程度的异地冗余，并将应用程序故障转移到备用数据中心。
- 升级或维护错误，意外的问题的过程中发生计划内基础结构维护或升级可能需要快速回滚到之前的数据库状态。

若要缓解的本地硬件和软件故障，SQL 数据库包括[高可用性体系结构](sql-database-high-availability.md)，这样可以保证都能达到 99.995%与这些故障自动恢复的可用性 SLA。  

若要防止你的业务的数据丢失，SQL 数据库会自动创建完整数据库备份每周、 差异数据库备份每隔 12 小时和事务日志备份每 5-10 分钟。 备份存储在 RA-GRS 存储时间至少 7 天的所有服务层。 除了基本的所有服务层为 35 天最多都支持时间点还原，可配置的备份保留的期。 

SQL 数据库还提供了若干业务连续性功能，可用于迁移非计划的各种方案。 

- 使用[时态表](sql-database-temporal-tables.md)可以从任意时间点还原行版本。
- [内置自动备份](sql-database-automated-backups.md)并[时间点还原](sql-database-recovery-using-backups.md#point-in-time-restore)使您能够完整数据库还原到某个时间点的时间内配置的保持期的时间向上到 35 天。
...
## <a name="recover-a-database-within-the-same-azure-region"></a>在同一 Azure 区域内的将数据库恢复

自动数据库备份可用于将数据库还原到某个点，在过去的时间。 这种方式可以从数据损坏引起的人为错误恢复。 时间点还原，可表示的数据损坏事件之前的状态在同一服务器中创建新的数据库。 对于大多数数据库的还原操作将采用少于 12 小时。 可能需要更长时间才能恢复非常大或非常活跃的数据库。 有关恢复时间的详细信息，请参阅[数据库恢复时间](sql-database-recovery-using-backups.md#recovery-time)。 

如果支持的最大备份保留期的时间点还原 (PITR) 是不够的应用程序，则可以通过配置数据库的长期保留 (LTR) 策略扩展它。 有关详细信息，请参阅[长期备份保留](sql-database-long-term-retention.md)。

## <a name="recover-a-database-to-another-azure-region"></a>将数据库恢复到另一个 Azure 区域
...
制定业务连续性计划时，需了解应用程序在中断事件发生后完全恢复之前的最大可接受时间。 应用程序完全恢复所需的时间称为恢复时间目标 (RTO)。 您还需要了解最新数据更新 （时间间隔） 应用程序可以容忍的最大段从计划外中断性事件恢复时丢失。 潜在的数据丢失称为恢复点目标 (RPO)。

不同的恢复方法提供不同级别的 RPO 和 RTO。 可以选择特定的恢复方法，或使用 achicethe 完整的应用程序恢复到的方法的组合。 下表比较了 RPO 和 RTO 的每个恢复选项。
...
您可以选择使用数据库备份和活动异地复制根据应用程序的需求的组合。 为独立数据库和弹性池使用这些业务连续性功能的设计注意事项的讨论，请参阅[为云灾难恢复设计应用程序](sql-database-designing-cloud-solutions-for-disaster-recovery.md)和[弹性池灾难恢复策略](sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool.md)。
