...
-  如果需要硬件安全模块或创建自己的密钥 (BYOK) 技术进行透明数据加密，请考虑使用 [Azure SQL 透明数据加密：支持创建自己的密钥](transparent-data-encryption-byok-azure-sql.md)。
...
> [!IMPORTANT]
> 管理 Azure 中的数据库和逻辑服务器由门户用户帐户的角色分配控制。 有关本文的详细信息，请参阅 [Azure 门户中基于角色的访问控制](../role-based-access-control/overview.md)。 使用防火墙规则控制访问权限不适用于 **Azure SQL 数据库托管实例**。 有关所需网络配置的详细信息，请参阅以下有关[连接到托管实例](sql-database-managed-instance-connect-app.md)的文章。
...
- **SQL 身份验证**
  ...
  此身份验证方法使用用户名和密码。 在为数据库创建逻辑服务器时，已指定一个包含用户名和密码的“服务器管理员”登录名。 通过这些凭据，可以使用数据库所有者（即“dbo”）的身份通过服务器上任何数据库的身份验证。
