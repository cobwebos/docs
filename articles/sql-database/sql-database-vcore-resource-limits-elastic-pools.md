...
|最大数据 IOPS *|400|800|1200|1600|2000|2400|
|最大对数速率（MBps）|4.7|9.4|14.1|18.8|23.4|28.1|
...
|存储类型|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|最大数据 IOPS *|2800|3200|3600|4000|6400|9600|
|最大对数速率（MBps）|32.8|37.5|37.5|37.5|37.5|37.5|
...
|最大数据 IOPS *|800|1600|2400|3200|4000|4800|5600|
|最大对数速率（MBps）|9.4|18.8|28.1|37.5|37.5|37.5|37.5|
...
\*任意单个数据库的最大并发工作线程（请求） \*，请参阅[单一数据库资源限制](sql-database-vcore-resource-limits-single-databases.md)。 例如，如果弹性池使用的是 Gen5，并且每个数据库的最大 vCore 数设置为2，则最大并发工作线程值为200。  如果 "每个数据库的最大 vCore 数" 设置为0.5，则 "最大并发工作线程数" 值为50，因为每个 vCore 的最大并发辅助进程数为100，每个数据库的最大并发辅助进程数为1个或更少，最大并发辅助进程数同样 重新缩放.
...
|存储类型|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|高级（远程）存储|
|IO 延迟（近似）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|5-7 毫秒（写入）<br>5-10 毫秒（读取）|
|最大数据 IOPS * |6,400|7,200|8,000|9,600|12,800|16,000|32,000|
|最大对数速率（MBps）|37.5|37.5|37.5|37.5|37.5|37.5|37.5|
...
|最大数据 IOPS *|9,000|13,500|18,000|22500|27000|
|最大对数速率（MBps）|20|30|40|50|60|
...
|最大数据 IOPS *|31500|36,000|40500|45,000|72000|90,000|
|最大对数速率（MBps）|70|80|80|80|80|80|
...
|存储类型|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|本地 SSD|
|IO 延迟（近似）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|1-2 毫秒（写入）<br>1-2 毫秒（读取）|
|最大数据 IOPS *|18,000|27000|36,000|45,000|54,000|63000|
|最大对数速率（MBps）|60|90|120|120|120|120|
...
|最大数据 IOPS *|72000|81000|90,000|108000|144000|180,000|256000|
|最大对数速率（MBps）|120|120|120|120|120|120|120|
|每个池的最大并发辅助进程数（请求） * *|1680|1890|2100|2520|3360|4200|8400|
|每个池的最大并发登录数（请求） * *|1680|1890|2100|2520|3360|4200|8400|
|最大并发会话数|30,000|30,000|30,000|30,000|30,000|30,000|30,000|
