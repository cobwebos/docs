...
对于使用许多资源的大容量应用场景，可以使用 [Log Analytics](saas-dbpertenant-log-analytics.md)。 这是单独的 Azure 服务，可针对发出的诊断日志以及日志分析工作区中收集的遥测进行分析。 Log Analytics 可收集多个服务的遥测，可查询和设置警报。
