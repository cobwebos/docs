...
- 单个数据库或弹性池的存储价格是指将数据存储和事务日志存储量乘以服务层的存储单价。 `tempdb` 的成本包括在价格中。 有关存储价格的详细信息，请参阅[SQL 数据库定价](https://azure.microsoft.com/pricing/details/sql-database/)。
