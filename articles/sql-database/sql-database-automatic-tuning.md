...
如果是通过 T-sql 应用优化建议，则自动性能验证和反向机制不可用。 以这种方式应用的建议将保持活动状态，并在24-48 小时的优化建议列表中显示。 将在 24-48 小时内保持活动状态并显示在建议列表中。 如果你想要更快地从列表中删除建议，可以将其从 Azure 门户中丢弃。
