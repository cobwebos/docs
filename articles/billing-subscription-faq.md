...
- [为什么不能编辑或添加订阅详细信息？](#why-cannot-i-edit-or-add-details-to-my-subscription)
...
- [什么是试用订阅，它能够持续多长时间？](#what-is-a-trial-subscription-and-how-long-does-it-last)
- [使用试用帐户时可访问哪些资源？](#what-resources-are-available-when-using-a-trial-account)
...
- [如何编辑我的 Azure 订阅的支付信息？](#how-can-i-edit-my-payment-information-for-my-azure-subscriptions)
...
按照 [Azure 发票](https://azure.microsoft.com//pricing/invoicing/)中的步骤提交通过发票付款的请求。在你的请求获得批准之后，将为你提供有关如何针对发票付款方式设置订阅的说明。
...
如果你更换了信用卡，但号码保持不变，则只需更新现有信用卡的详细信息即可，例如失效日期。如果你的卡号变了，或者你想要使用其他卡，则需添加新卡。你可以在下面找到更新或添加信用卡的说明：[如何更改用于支付 Azure 订阅的信用卡](billing-how-to-change-credit-card.md)。
...
**计划内维护（经典门户）**：不管是单实例配置还是多实例配置，Azure 都会提前发送电子邮件通信，提醒你即将执行计划内维护（单实例提前 1 周，多实例提前 48 小时）。电子邮件将发送到订阅中提供的帐户管理员、服务管理员和共同管理员的电子邮件帐户。

**注意：**如需特定于虚拟机的详细信息，请参阅文章：[Azure 虚拟机的计划内维护](virtual-machines/virtual-machines-linux-planned-maintenance.md)。

**计划外维护：**如果 Azure 平台上发生非计划服务中断，则将在我们的[服务仪表板](http://status.azure.com/)上进行状态更新，并且客户也将在 [Azure 经典门户](https://manage.windowsazure.com/)中收到通知。
...
- 你的订阅类型是“免费试用”或某项成员产品/服务 (MSDN/Open/BizSpark/DreamSpark)，因此没有信用卡或与之关联的其他付款方式。
- 还未到生成发票的日期。
...
[无法登录管理 Azure 订阅](billing-cannot-login-subscription.md)

**注意** 请确保你以正确的管理员身份登录：帐户管理员只能访问[帐户中心](https://account.windowsazure.com/)，而服务管理员 (SA) 和共同管理员 (CA) 则只能访问 [Azure 门户](https://portal.azure.com/)或 [Azure 经典门户](https://manage.windowsazure.com/)。
...
可通过 [Azure 帐户中心](https://account.windowsazure.com/Home/Index)访问 Microsoft Azure 帐户。从这里，你可以查看到属于该帐户且将有资格使用支持的订阅的列表。
...
在 6 个月的期限结束时，你的计划将使用相同的付款方式，自动续订 6 个月相同的 Azure 支持计划。作为帐户所有者，你可以选择通过 [Azure 帐户中心](https://account.windowsazure.com/Home/Index)关闭自动续订功能。

如果你不希望自动续订，请登录 [Azure 帐户中心](https://account.windowsazure.com/Home/Index)，选择相应的支持计划，单击“更改续订详细信息”，并取消自动续订的默认选项。

### 我如何取消 Azure 支持计划？

帐户所有者可取消 Azure 支持计划，方式是登录 [Azure帐户中心](https://account.windowsazure.com/Home/Index)，依次选择“帐户中心”和“订阅”。如果你拥有订阅，则将显示你的订阅列表。选择你希望取消的订阅。然后，将显示订阅的详细信息，并在屏幕右侧随附一个选项列表。选择“取消”并继续操作。如果你没有订阅，则将显示“你未拥有任何订阅”的消息。

请注意，Azure 支持计划的期限为 6 个月，且 Microsoft 要求有持续的资金承诺才会延续期限。在期限到期前取消，不会免除剩余的承诺，也不会就任何预支付金额提供退款。
...
你现在可以轻松地在 Microsoft Azure 帐户中心对即用即付、MSDN、Action Pack 或 BizSpark 订阅执行此操作。我们添加了相应的功能让你将订阅转让给另一用户。换言之，你现在可以更改所拥有的任何即用即付、MSDN、Action Pack 或 BizSpark 订阅的帐户管理员。请注意，接收方的用户帐户必须是在同一国家/地区，而且你不能转让与应用商店购买相关联的订阅。
...
你可以将即用即付订阅转成 [12 个月预付款优惠](https://azure.microsoft.com/offers/ms-azr-0026p/)。根据此优惠，预付款购买 12 个月期限的 Azure 服务时，将可享受 5% 的 Azure 服务价格折扣。在 [Azure 帐户中心](https://account.windowsazure.com/Subscriptions)转到你的订阅，然后单击“改用其他优惠”。这是目前唯一可以通过帐户中心改用的优惠。另外，若要改用其他优惠，你也可以联系 [Azure 支持](https://azure.microsoft.com/support/options/)。
...
2.	单击“编辑详细信息”以更新接收通知的电子邮件地址。

### 如何编辑我的 Azure 订阅的支付信息？
...
2.	在“订阅”页上，单击你想要更新其付款方式的订阅。
3.	在“订阅摘要”页上，单击“更改付款方式”。“更改付款方式”工具将在单独的窗口中显示。

**注意**：你还可从 Microsoft Azure 经典门户访问帐户中心。为此，请单击你的帐户名称，然后单击“查看我的帐单”。
...
**注意**：“更改付款方式”工具当前不允许删除用作付款方式的现有信用卡。有关如何删除信用卡的信息，请参阅本文中的常见问题“如何删除不再用作 Azure 付款方式的信用卡？”。

### 为什么不能编辑或添加订阅详细信息？

若要查看和编辑 Azure 帐户信息，必须以帐户管理员身份登录 Azure 帐户中心。如果你是 AA 但仍然无法编辑订阅，请提交支持票证以请求协助。若要创建支持票证，请参阅文章：[如何创建针对 Azure 计费和订阅问题的支持票证](billing-how-to-create-billing-support-ticket.md)。
...
是的。利用我们的新[支出限制功能](https://azure.microsoft.com/pricing/spending-limits/)，注册免费试用、MSDN、MPN 或 BizSpark 产品/服务的客户可以使用 Azure 而完全不必担心被收费，只要他们将支出限制功能保持打开状态。
...
Azure 试用订阅将给予你一个以美元计的信用额度，你可以在 30 天内根据需要将其用于 Microsoft 服务。若要查看服务免费试用信用额度可用于和不可用于的服务，请参阅[免费试用促销活动页面](https://azure.microsoft.com/offers/ms-azr-0044p/)。

可在[此处](http://azure.microsoft.com/pricing/free-trial/)注册试用订阅。

### 使用试用帐户时可访问哪些资源？

你将收到 200 美元的 Azure 信用额度以及免费试用版。现在你可以选择如何使用你的 Azure 信用额度。请参阅[免费试用促销活动页面](https://azure.microsoft.com/offers/ms-azr-0044p/)以了解详细信息。
