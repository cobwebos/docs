...
无论你具有 S1、S2 或 S3 性能级别的集合，还是具有单个分区集合（所有这些集合都具有 20 GB 的存储空间），都可以使用 Azure Cosmos DB 数据迁移工具将数据迁移到具有几乎无限制的存储的已分区集合。 有关分区集合优势的信息，请参阅 [Azure Cosmos DB 中的分区和缩放](sql-api-partition-data.md)。 
...
3.  [请求单位](request-units.md)数。 了解不同操作类型（例如读取、写入和查询）的吞吐量消耗。
