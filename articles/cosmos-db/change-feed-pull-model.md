...
您可以使用请求`FeedIterator`模型创建来处理更改源。 最初创建时`FeedIterator`，可以在中指定一个可选`StartTime`的。 `ChangeFeedRequestOptions` 如果未指定， `StartTime`将为当前时间。

`FeedIterator`分为两种风格。 除了返回实体对象的以下示例之外，还可以获取`Stream`支持的响应。 流使你可以在不先进行反序列化的情况下读取数据，而是在客户端资源上保存。
...
下面是一个示例`FeedIterator` ，用于获取返回的： `Stream`
...
获取容器的 FeedRanges 列表时，每个`FeedRange` [物理分区](partition-data.md#physical-partitions)会获得一个列表。

然后`FeedRange`，你可以创建`FeedIterator`来并行处理跨多个计算机或线程的更改源。 与上面演示如何获取整个容器的单个`FeedIterator`示例不同，你可以使用`FeedRange`来获取多个 FeedIterators，它可以并行处理更改源。
