...
| Maximum number of distinct (logical) partition keys | 无限制 |
| Maximum storage per container | 无限制 |
| Maximum storage per database | 无限制 |
...
| 资源 | 默认限制 |
| --- | --- |
| Maximum number of databases | 无限制 |
| Maximum number of containers per database (or account) | 无限制 |
...
| 资源 | 默认限制 |
| --- | --- |
| Maximum master token expiry time | 15 分钟  |
...
| Maximum clock skew for token authorization| 15 分钟 |
...
| Maximum containers per subscription (SQL, Gremlin, Table API) | 第 |
