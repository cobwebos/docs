...
使用 **"生存时间"** 或"TTL"时，Azure Cosmos DB 提供了在特定时间段后自动从容器中删除项目的能力。 默认情况下，可以在容器级别设置生存时间，并基于每个项替代该值。 在容器或项级别设置 TTL 后，Azure Cosmos DB 会在一段时间（自上次修改项的时间开始算起）后自动删除这些项。 配置的生存时间值以秒为单位。 配置 TTL 后，系统会基于 TTL 值自动删除已过期的项，不需要客户端应用程序显式发出的 delete 操作。
...
1. **容器的生存时间**（使用 `DefaultTimeToLive` 设置）：
   ...
   - 如果存在，并且该值设置为"-1"，则等于无穷大，并且默认情况下项目不会过期。

   - 如果存在且值设置为某个数字 *"n"，* 则项目将在上次修改时间后 *"n"* 秒过期。
...
* 如果 TTL 在容器上设置为 *"n"，* 则该容器中的项将在*n*秒后过期。  如果同一容器中的项有自身的生存时间且 TTL 设置为 -1（表示不会过期），或者某些项使用不同的数字替代了生存时间设置，则这些项会根据其自己的已配置 TTL 值过期。 
