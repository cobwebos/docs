...
使用由 Microsoft 管理的密钥（**服务托管密钥**）自动并无缝加密存储在 Azure Cosmos 帐户中的数据。 或者，你可以选择使用所管理的密钥（**客户管理的密钥**）来添加另一层的加密。

![围绕客户数据进行加密的层](./media/how-to-setup-cmk/cmk-intro.png)

必须将客户托管密钥存储在[Azure Key Vault](../key-vault/general/overview.md)中，并为使用客户托管密钥启用的每个 Azure Cosmos 帐户提供密钥。 此密钥用于加密该帐户中存储的所有数据。
...
1. 在“选择主体”下，选择“未选择任何项”。******** 然后，搜索**Azure Cosmos DB**主体并将其选中（若要更容易查找，还可以按主体 id 搜索： `a232010e-820c-4083-83bb-3ace5fc29d0b`对于除 azure 政府区域外的任何 AZURE 区域，主体 id 为`57506a73-e302-42a9-b869-6f12d9ec29e9`）。 最后，选择底部的 "**选择**"。 如果**Azure Cosmos DB**主体不在列表中，则可能需要重新注册**Microsoft DocumentDB**资源提供程序，如本文的[注册资源提供程序](#register-resource-provider)部分中所述。
...
Azure Cosmos DB [定期自动备份](./online-backup-and-restore.md)帐户中存储的数据。 此操作会备份已加密的数据。 若要使用还原的备份，需要提供备份时使用的加密密钥。 这意味着，不会进行任何吊销，并且仍将启用备份时使用的密钥版本。
