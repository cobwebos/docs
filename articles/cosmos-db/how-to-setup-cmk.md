...
- **客户托管的密钥（CMK）** -可以选择使用自己的密钥来添加另一层加密。

必须将客户托管的密钥存储在[Azure Key Vault](../key-vault/key-vault-overview.md)中，并为使用客户管理的密钥启用的每个 Azure Cosmos 帐户提供密钥。 此密钥用于加密存储在该帐户中的所有数据。

> [!NOTE]
> 目前，客户托管的密钥仅适用于新的 Azure Cosmos 帐户，你应在帐户创建过程中对其进行配置。
...
将客户托管的密钥与 Azure Cosmos DB 结合使用时，需要在计划用于承载加密密钥的 Azure Key Vault 实例上设置两个属性。 这些属性包括**软删除**和不**清除**。 默认情况下，这些属性不会启用，你可以使用 PowerShell 或 Azure CLI 来启用它们。
...
1. 在 "**密钥权限**" 下拉列表菜单中，选择 "**获取**、**解包**密钥和**包装密钥**" 权限：

   ![选择正确的权限](./media/how-to-setup-cmk/portal-akv-add-ap-perm2.png)

1. 在 "**选择主体**" 下，选择 "**未**选择"。 然后，搜索 " **Azure Cosmos DB**主体" 并将其选中。 最后，单击底部的 "**选择**" （如果**Azure Cosmos DB**主体不在列表中，则可能需要按照本文[注册资源提供程序](#register-resource-provider)部分中所述，重新注册**Microsoft DocumentDB**资源提供程序）：
...
1. 选择 "**生成/导入**"，提供新密钥的名称，选择 RSA 密钥大小（建议最少为3072，以获得最佳安全性），然后选择 "**创建**"：

   ![新建密钥](./media/how-to-setup-cmk/portal-akv-gen.png)

1. 创建密钥后，选择新创建的密钥，然后选择其当前版本。

1. 复制密钥的**密钥标识符**（最后一个正斜杠后的部分除外）：
...
- 传递从**PropertyObject**中**keyVaultKeyUri**属性下的之前复制的 Azure Key Vault 密钥的 URI
...
可以。 为了考虑使用客户管理的密钥管理数据加密和解密所需的额外计算负载，对 Azure Cosmos 帐户执行的所有操作都消耗25% 的[请求单位](./request-units.md)。
...
### <a name="how-does-customer-managed-keys-affect-a-backup"></a>客户托管的密钥如何影响备份？

Azure Cosmos DB 会[定期自动备份](./online-backup-and-restore.md)帐户中存储的数据。 此操作可备份加密的数据。 若要使用还原的备份，则需要在备份时使用的加密密钥。 这意味着，不应进行任何吊销，并且仍将启用备份时使用的密钥版本。
