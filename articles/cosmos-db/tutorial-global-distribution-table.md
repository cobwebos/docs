...
为了利用[全局分发](../documentdb/documentdb-distribute-data-globally.md)，客户端应用程序可以指定要用于执行文档操作的区域优先顺序列表。 对于预览版 Azure 存储 SDK，这可通过在应用配置中设置 `TablePreferredLocations` 配置值完成。 Azure 存储 SDK 会根据 Azure Cosmos DB 帐户配置、当前区域可用性和指定的优先顺序列表，选择最佳的终结点来执行写入和读取操作。
