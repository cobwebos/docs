---
title: Azure Cosmos 容器的全球分布式事务和分析（在专用预览中）存储
...
|用于访问数据的 API  |   SQL、蒙戈DB、卡桑德拉、格雷姆林、表格等。       | Apache Spark         |
|保留期（生存时间或 TTL）   |  由策略驱动，使用 `DefaultTimeToLive` 属性在 Azure Cosmos 容器中配置。       |   由策略驱动，使用 `ColumnStoreTimeToLive` 属性在 Azure Cosmos 容器中配置。      |
|每 GB 价格    |   查看[定价页面](https://azure.microsoft.com/pricing/details/cosmos-db/)     |   查看[定价页面](https://azure.microsoft.com/pricing/details/cosmos-db/)        |
|存储事务的价格    |  查看[定价页面](https://azure.microsoft.com/pricing/details/cosmos-db/)         |   查看[定价页面](https://azure.microsoft.com/pricing/details/cosmos-db/)        |
...
对 Azure Cosmos DB 执行 ETL 操作不会产生任何开销。 每个 Azure Cosmos 容器以事务和分析存储引擎为后盾，事务与分析存储引擎之间的数据传输是在数据库引擎内部执行的，无需任何网络跃点。 与传统的分析解决方案相比，产生的延迟和成本要低得多。 您还可以为事务和分析工作负载提供单个全局分布式存储系统。  
...
### <a name="globally-distributed-multi-master-analytical-storage"></a>全球分布式多主分析存储
...
在给定区域中，事务工作负载针对容器的事务/行存储进行操作。 另一方面，分析工作负载针对容器的分析/列存储运行。 这两个存储引擎独立运行，在工作负荷之间提供严格的性能隔离。
