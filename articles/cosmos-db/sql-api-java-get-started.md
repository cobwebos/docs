...
## <a name="step-2-clone-the-github-project"></a><a id="GitClone"></a>步骤 2：克隆 GitHub 项目
...
可以使用 [DocumentClient](databases-containers-items.md#azure-cosmos-databases) 类的 [createDatabase](/java/api/com.microsoft.azure.documentdb.documentclient.createdatabase) 方法创建 Azure Cosmos DB **数据库**。 数据库是跨集合分区的 JSON 文档存储的逻辑容器。

    Database database = new Database();
    database.setId("familydb");
    this.client.createDatabase(database, null);

## <a name="step-5-create-a-collection"></a><a id="CreateColl"></a>步骤 5：创建集合
> [!WARNING]
> **createCollection** 创建一个具有保留吞吐量的新集合，这会牵涉到价格问题。 有关详细信息，请访问[定价页](https://azure.microsoft.com/pricing/details/cosmos-db/)。
> 
> 

可以使用 [DocumentClient](/java/api/com.microsoft.azure.documentdb.documentclient.createcollection) 类的 **createCollection** 方法创建集合。 集合是 JSON 文档和相关联的 JavaScript 应用程序逻辑的容器。
...
## <a name="step-6-create-json-documents"></a><a id="CreateDoc"></a>第 6 步：创建 JSON 文档
可以使用 [DocumentClient](/java/api/com.microsoft.azure.documentdb.documentclient.createdocument) 类的 **createDocument** 方法创建文档。 文档是用户定义的（任意）JSON 内容。 现在，我们可以插入一个或多个文档。 如果已有要在数据库中存储的数据，则可以使用 Azure Cosmos DB 的[数据迁移工具](import-data.md)将数据导入数据库。
...
## <a name="step-8-replace-json-document"></a><a id="ReplaceDocument"></a>步骤 8：替换 JSON 文档
...
## <a name="step-9-delete-json-document"></a><a id="DeleteDocument"></a>步骤 9：删除 JSON 文档
...
## <a name="step-11-run-your-java-console-application-all-together"></a><a id="Run"></a>步骤 11：一起运行 Java 控制台应用程序！
