...
|还原已删除的数据|可以使用自动联机备份来恢复大约 30 天内意外删除的数据。 <br><br>[通过 Azure Cosmos DB 在自动联机备份和还原](online-backup-and-restore.md)中了解详细信息|
...
|使用强密码的管理帐户|难以相信，我们竟然还要提到这项要求。但与我们的某些竞争产品不同，在 Azure Cosmos DB 中，不带密码的管理帐户是根本不受允许的。<br><br> 默认情况下，通过基于 TLS 和 HMAC 机密的身份验证进行安全融入。|
|安全和数据保护认证| 有关认证的最新数据列表，请参阅具有所有认证（搜索 Cosmos）的整个 [Azure 符合性站点](https://www.microsoft.com/en-us/trustcenter/compliance/complianceofferings)以及最新 [Azure 符合性文档](https://gallery.technet.microsoft.com/Overview-of-Azure-c1be3942)。 若要进一步了解2018年4月25日，请阅读 post [Azure #CosmosDB：安全、私有、符合性，其中包括 SOC 1/2 类型2、HITRUST、PCI DSS 级别1、ISO 27001、HIPAA、FedRAMP 高，等等。
