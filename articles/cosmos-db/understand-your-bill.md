...
利用 Azure Cosmos DB 免费层，你将获得帐户中的第一个 400 RU/s 和 5 GB 存储空间，适用于帐户级别。 任何 RU/秒和超过 400 RU/秒和 5 GB 的存储将按定价页的定期定价费率进行计费。 在帐单上，你将看不到免费 400 Ru/秒和 5 GB 的费用或行项，只显示了免费层所涵盖的 RU/秒和存储空间。 400 RU/s 适用于任何类型的 RU/s 预配吞吐量、自动缩放和多主机。  
...
### <a name="billing-example---container-or-database-with-autoscale-throughput"></a>计费示例-具有自动缩放吞吐量的容器或数据库
- 假设在免费层帐户中创建启用了自动缩放的数据库或容器，最大 RU/秒为 4000 RU/s。 此资源将在 400 RU/s-4000 RU/秒之间自动缩放。 
...
- 注意：如果各区域中的 RU/s 或存储的单位价格不同，则免费层 400 RU/秒和 5 GB 将反映创建该帐户的区域的费率。
