...
ms.locfileid: "83647727"
---
# <a name="understand-your-azure-cosmos-db-bill"></a>了解 Azure Cosmos DB 帐单

Azure Cosmos DB 是完全托管的云原生数据库服务，仅针对预配的吞吐量和消耗的存储收费，从而简化计费。 与本地或 IaaS 托管的替代方案相比，无需额外的许可费、硬件、使用成本或设施成本。 若想使用 Azure Cosmos DB 的多区域功能，与现有本地或 IaaS 解决方案相比，数据库服务可显著降低成本。
...
* 假定创建一个预配吞吐量为 2,500 RU/秒的容器。容器在一个月内存在 24 小时（例如，在创建容器后 24 小时将其删除）。  
...
使用 Azure Cosmos DB 免费层，你将在帐户中获得每秒前 400 RU 的免费吞吐量和 5 GB 的免费存储（适用于帐户级别）。 除每秒 400 RU 和 5 GB 以外的任何每秒 RU 和存储都将按定价页的常规定价费率计费。 在帐单上，你不会看到免费的每秒 400 RU 和 5 GB 的费用或行项，而只会看到免费层涵盖范围之外的每秒 RU 和存储的费用。 每秒 400 RU 的免费吞吐量适用于任何类型的每秒 RU - 预配吞吐量、自动缩放和多主数据库。  

### <a name="billing-example---container-or-database-with-provisioned-throughput"></a>计费示例 - 具有预配吞吐量的容器或数据库
- 假定在免费层帐户中创建了一个数据库或容器，并预配了 400 RU/秒的吞吐量和 5 GB 的存储。
- 你的帐单不会显示有关此资源的任何费用。 每小时成本和每月成本将为 0 美元。
- 现在，假定在同一帐户中添加了另一个数据库或容器，并将其预配为 1000 RU/秒的吞吐量和 10 GB 的存储。
- 你的帐单现在会显示 1000 RU/秒吞吐量和 10 GB 存储的费用。 

### <a name="billing-example---container-with-autoscale-throughput"></a>计费示例 - 具有自动缩放吞吐量的容器
- 假定在免费层帐户中创建了一个支持自动缩放的容器，并将 RU/秒的最大值预配为 4000 RU/秒。 则该资源将在 400 RU/秒 - 4000 RU/秒之间自动缩放。 
- 假定在 1 - 10 小时中，该资源 RU/秒的最小值为 400 RU/秒。 在第 11 个小时，该资源纵向扩展到 1000 RU/秒，然后在该小时内又缩减到 400 RU/秒。
...
### <a name="billing-example---multi-region-single-write-region-account"></a>计费示例 - 多区域的单一写入区域帐户
- 假定在免费层帐户中创建了一个数据库或容器，并预配了 1200 RU/秒的吞吐量和 10 GB 的存储。 我们将该帐户复制到 3 个区域，并且我们拥有一个单主机（单一写入区域）帐户。
- 如果不使用免费层，我们共需要支付 3 * 1200 RU/秒 = 3600 RU/秒和 3 * 10 GB = 30 GB 存储的费用。
...
### <a name="billing-example---multi-region-multi-master-multiple-write-region-account"></a>计费示例 - 多区域多主机（多写入区域）帐户

此示例反映了 2019 年 12 月 1 日之后创建的帐户的[多主机定价](https://azure.microsoft.com/pricing/details/cosmos-db/)。 
- 假定在免费层帐户中创建了一个数据库或容器，并预配了 1200 RU/秒的吞吐量和 10 GB 的存储。 我们将该帐户复制到 3 个区域，并且我们拥有一个多主机（多写入区域）帐户。 
- 如果不使用免费层，我们共需要支付 3 * 1200 RU/秒 = 3600 RU/秒和 3 * 10 GB = 30 GB 存储的费用。
