...
ms.locfileid: "78674305"
---
# <a name="understand-your-azure-cosmos-db-bill"></a>了解 Azure Cosmos DB 帐单

Azure Cosmos DB 是完全托管的云原生数据库服务，仅针对预配的吞吐量和消耗的存储收费，从而简化计费。 与本地或 IaaS 托管的替代项相比，没有额外的许可费用、硬件、实用程序成本或设施成本。 考虑 Azure Cosmos DB 的多区域功能时，与现有的本地或 IaaS 解决方案相比，数据库服务可大大降低成本。
...
* 在720小时（24小时 * 30 天）内，如果500小时预配吞吐量为 1200 RU/秒，而剩余的220小时预配吞吐量为 22200 RU/秒，则每月帐单将显示： 500 x $ 0.096/hour + 220 x $ 1.776/小时 = $ 438.72/月。
...
* 然后，你的帐单将更改为： 1550 * $0.008 = $ 12.40/小时。  

* 每月720小时，如果300小时预配吞吐量为 120-K RU/秒，而剩余的420小时预配吞吐量为 155-K RU/秒，则每月帐单将显示： 300 x $ 9.60/小时 + 420 x $ 12.40/hour = $2880 + $5208 = $ 8088/月。 
...
*同时假设你在美国西部的容器中每月出口 100 GB 数据，将数据复制到美国东部、北欧和东亚。按数据传输费率对出口收费。*
...
### <a name="billing-example-azure-cosmos-account-with-multi-master-database-level-throughput-including-dedicated-throughput-mode-for-some-containers"></a>计费示例：具有多主数据库级吞吐量的 Azure Cosmos 帐户，其中包括针对某些容器的专用吞吐量模式
...
| | |2个其他区域的吞吐量帐单：美国东部、北欧（所有区域均可写）  |`(2 + 1) * (60 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |2,880 美元  |
...
| | |2个其他区域的吞吐量帐单：美国东部、北欧（所有区域均可写）  |`(2 + 1) * (120 K RU/sec /100 * $0.016) * 100 hours = $5,760`  |5,760 美元  |
...
| | |2个其他区域的吞吐量帐单：美国东部、北欧（所有区域均可写）  |`(2 + 1) * (140 K RU/sec /100 * $0.016-) * 100 hours = $6,720` |6,720 美元 |
...
| | |2个其他区域的吞吐量帐单：美国东部、北欧（所有区域均可写）  |`(1 + 1) * (90 K RU/sec /100 * $0.016) * 100 hours = $2,880`  |2,880 美元  |
...
| | |2个其他区域的吞吐量帐单：美国东部、北欧（所有区域均可写）  |`(1 + 1) * (40 K RU/sec /100 * $0.016) * 100 hours = $1,280`  |1,280 美元  |
...
| | |2个其他区域的吞吐量帐单：美国东部、北欧（所有区域均可写）  |`(1 + 1) * (120 K RU/sec /100 * $0.016) * 200 hours = $1,280`  |7,680 美元  |
...
| | |2个其他区域的吞吐量帐单：美国东部、北欧（所有区域均可写）  |`(1 + 1) * (70 K RU/sec /100 * $0.016) * 20 hours = $448`  |224 美元  |
|| |**每月总成本**  | |**38,688 美元**   |

## <a name="billing-examples-with-free-tier-accounts"></a>免费层帐户计费示例
利用 Azure Cosmos DB 免费层，你将获得帐户中的第一个 400 RU/s 和 5 GB 存储空间，适用于帐户级别。 任何 RU/秒和超过 400 RU/秒和 5 GB 的存储将按定价页的定期定价费率进行计费。 在帐单上，你将看不到免费 400 Ru/秒和 5 GB 的费用或行项，只显示了免费层所涵盖的 RU/秒和存储空间。 400 RU/s 适用于任何类型的 RU/s 预配吞吐量、autopilot （预览版）和多主机。  

### <a name="billing-example---container-or-database-with-provisioned-throughput"></a>计费示例-具有预配吞吐量的容器或数据库
- 假设我们在具有 400 RU/s 和 5 GB 存储空间的免费层帐户中创建一个数据库或容器。
- 你的帐单不会显示此资源的任何费用。 每小时和每月成本将是 $0。
- 现在，让我们假设在同一帐户中，添加另一个具有 1000 RU/s 和 10 GB 存储空间的数据库或容器。
- 帐单现在会显示 1000 RU/秒和 10 GB 存储空间的费用。 

### <a name="billing-example---container-or-database-with-autopilot-throughput-preview"></a>计费示例-具有 autopilot 吞吐量的容器或数据库（预览版）
- 假设在免费层帐户中创建一个启用了 autopilot 的数据库或容器，最大 RU/秒为 4000 RU/s。 此资源将在 400 RU/s-4000 RU/秒之间自动缩放。 
- 假设在1到1小时10，资源至少为 400 RU/s。 在第11个小时内，资源会在一小时内扩展到 1000 RU/秒，然后再向下移动到 400 RU/秒。
- 在小时1到10中，你将为吞吐量计费 $0，因为 400 RU/秒是免费级别。 
- 在小时11中，将按有效的 1000 RU/s-400 RU/s = 600 RU/秒付费，因为这是一小时内的最大 RU/秒。 这将是一小时 100 RU/s 的6个单位，因此该小时的总吞吐量成本将是6个单位 * $0.012 = $0.072。 
- 除前 5 GB 以外的任何存储都将按标准存储费率计费。 

### <a name="billing-example---multi-region-single-write-region-account"></a>计费示例-多区域单一写入区域帐户
- 假设在免费层帐户中创建一个具有 1200 RU/s 和 10 GB 存储空间的数据库或容器。 我们将帐户复制到3个区域，并且我们有一个单主机（单一写入区域）帐户。
- 除了免费层，我们将按 3 * 1200 RU/s = 3600 RU/秒和 3 * 10 GB = 30 GB 的存储计费。
- 使用免费层折扣后，在删除 400 RU/秒和 5 GB 存储后，我们将按单一写入区域速率和 25 GB 存储空间的有效 3200 RU/秒（32单位）向你收费。
- RU/s 的每月费用为：32单位 * $0.008 * 24 小时 * 31 天 = $190.46。 每月存储成本为： 25 GB * 0.25/GB = $6.25。 总开销为 $190.46 + $6.25 = $196.71。
- 注意：如果各区域中的 RU/s 或存储设备价格不同，则免费层 400 RU/秒和 5 GB 将反映创建该区域的帐户的速率。

### <a name="billing-example---multi-region-multi-master-multiple-write-region-account"></a>计费示例-多区域多主机（多个写入区域）帐户

此示例反映了2019年12月1日之后创建的帐户的[多主机定价](https://azure.microsoft.com/pricing/details/cosmos-db/)。 
- 假设在免费层帐户中创建一个具有 1200 RU/s 和 10 GB 存储空间的数据库或容器。 我们将帐户复制到3个区域，并且我们有一个多主机（多个写入区域）帐户。 
- 除了免费层，我们将按 3 * 1200 RU/s = 3600 RU/秒和 3 * 10 GB = 30 GB 的存储计费。
- 对于免费层折扣，在删除 400 RU/秒和 5 GB 存储后，我们将按多个写入区域速率和 25 GB 的存储量，以有效的 3200 RU/秒（32单位）为单位收费。
- RU/s 的每月费用为：32单位 * $0.016 * 24 小时 * 31 天 = $380.93。 每月存储成本为： 25 GB * 0.25/GB = $6.25。 总开销为 $380.93 + $6.25 = $387.18。
...
Total RU/sec： 500 + 400 = 900 每小时费用： 900/100 * $0.008 = $0.072 预计的每月吞吐量（假设31天）： $0.072 * 24 * 31 = $53.57

**每月总成本**

月度总成本 = 月度存储成本 + 月度吞吐量成本；月度总成本 = 25.00 美元 + 53.57 美元 = 78.57 美元

*定价可能因区域而异。有关最新定价，请参阅[定价页](https://azure.microsoft.com/pricing/details/cosmos-db/)。*
...
* 吞吐量成本（即用即付）： 100000 RU/秒/100 * $ 0.008/hour * 8760 小时/年 = $70080 
