...
1.  单击“高级选项”并设置以下值：
    *  **REST API 版本**：选择操作的[REST API 版本](https://docs.microsoft.com/rest/api/cosmos-db/)。 默认值为2015-12-16。 如果容器具有[大分区键](large-partition-keys.md)，并且需要 REST API 版本2018-12-31：
        ...
        - 右键单击以添加新的**字符串**值：
            ...
            - 值数据：**1**
             @ No__t-2Registry Editor settings @ no__t-3
...
以下步骤使用**表分隔符**映射方法为一个或多个容器中的数据创建一个架构。 如果容器包含异类数据类型，我们建议使用此采样方法。 可以使用此方法将采样范围限定为一组属性及其相应值。 例如，如果某个文档包含“Type”属性，可将采样范围限定为此属性的值。 采样的最终结果为针对每个指定 Type 的值返回的一组表。 例如，Type = Car 将生成 Car 表，Type = Plane 将生成 Plane 表。
