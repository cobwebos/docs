...
在此视频中通过 Azure Cosmos DB 首席项目经理 Denny Lee 了解详细信息。 
...
使用 **jars** 命令可以包含 **azure-cosmosdb-spark** 所需的两个 JAR（该 JAR 本身以及 Azure Cosmos DB SQL Java SDK）并排除 **scala-reflect**，使其不会干扰 Livy 调用（Jupyter notebook > Livy > Spark）。
