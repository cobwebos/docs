...
- **容量模型**：Azure Cosmos DB 上的数据库容量基于基于吞吐量的模型。 此模型基于[每秒请求单位数](request-units.md)，此单位表示每秒可对集合执行的数据库操作次数。 此容量可以在[数据库或集合级别](set-throughput.md)分配，也可以在分配模型上进行预配，也可以使用[AutoPilot 模型](provision-throughput-autopilot.md)进行预配。

- **请求单位**：每个数据库操作在 Azure Cosmos DB 中都有关联的请求单位 （R）成本。 执行操作时，将从在给定的秒可用的请求单位级别中减去此成本。 如果请求所需的 RU 数超过了当前分配的每秒 RU 数，可以使用两个选项来解决此问题 - 增加 RU 数量，或等待下一秒开始，然后重试操作。

- **弹性容量**：给定集合或数据库的容量可以随时更改。 这样，数据库就能弹性适应工作负荷的吞吐量要求。

- **自动分片**：Azure Cosmos DB 提供一个自动分区系统，该系统只需要分片（或分区键）。 [自动分区机制](partition-data.md)在所有 Azure Cosmos DB API 之间共享，允许通过水平分配进行无缝的数据缩放和全面缩放。
...
## <a name="estimate-the-throughput-need-for-your-workloads"></a><a id="estimate-throughput"></a>估计工作负载的吞吐量需求
...
下面是影响所需 RU 数的关键因素：
- **文档大小**：随着物料/文档大小的增加，用于读取或写入物料/文档的 R 并会增加。

- **文档属性计数**：创建或更新文档所消耗的 RU 数与该文档的属性数目、复杂性和长度相关。 您可以通过[限制索引属性的数量](mongodb-indexing.md)来减少写入操作的请求单位消耗。
