...
在 Azure 中，PaaS 服务（如 Azure 流分析、Azure 函数）都是与 Azure Cosmos DB 结合使用的。 要从此类 IP 地址不可用的服务中启用对 Azure Cosmos DB 数据库帐户的访问，必须通过[配置 IP 访问控制策略](#configure-ip-policy)，将 IP 地址为 0.0.0.0 的 IP 地址添加到与 Azure Cosmos DB 数据库帐户关联的 IP 地址的允许列表中。  这将确保 Azure PaaS 服务可以访问具有此规则的 Azure Cosmos DB 帐户。 
