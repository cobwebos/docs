---
title: Azure Cosmos DB Gremlin API 中的分区 | Microsoft Docs
...
ms.locfileid: "49078216"
---
# <a name="using-a-partitioned-graph-in-azure-cosmos-db"></a>在 Azure Cosmos DB 中使用分区图形

Azure Cosmos DB 中 Gremlin API 的重要功能之一是通过横向可缩放性处理大规模图形。 此过程是通过 [Azure Cosmos DB 中的分区功能](partition-data.md#how-does-partitioning-work)实现的。该功能利用在存储和吞吐量方面可独立缩放的容器。 Azure Cosmos DB 支持跨所有 API 的以下容器类型：
...
- 如果容器的大小预期会超过 10 GB 和/或分配的每秒请求单位数 (RU/s) 需要超过 10,000，则必须设置分区。

- 顶点和边缘作为 JSON 文档存储在 Azure Cosmos DB Gremlin API 容器的后端中。

- **顶点需要分区键**。 此键通过哈希算法确定在哪个分区中存储顶点。 此分区键的名称是不包含空格或特殊字符的单字字符串，是创建新容器时在门户中使用 `/partitioning-key-name` 格式定义的。
...
下面是在无限制容器中使用分区图形时为了确保获得最高效性能和可伸缩性而应该遵守的准则：
...
本文概述了有关使用 Azure Cosmos DB Gremlin API 进行分区的概念和最佳做法。 
