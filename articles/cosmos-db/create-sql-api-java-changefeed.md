...
Azure Cosmos DB 更改源提供事件驱动的接口，用于在响应文档插入时触发操作。 此功能具有很多用途。 例如，在读取和写入负载繁重的应用程序中，更改源的主要用途是在引入文档时创建容器的实时具体化视图。  具体化视图容器包含相同的数据，但为了提高读取效率，该容器已分区，因此，可以提高应用程序的读取和写入效率。

管理更改源事件的工作主要由 SDK 中内置的更改源处理器库负责。 此库足够强大，如果需要，它可以在多个工作线程之间分配更改源事件。 你只需为更改源库提供一个回调。
...
> 可以选择使用 Java SDK 4.0 或 Java SDK 3.7.0 来学习本快速入门。 若要使用 Java SDK 3.7.0，请在终端中键入 ```git checkout SDK3.7.0```。  否则，请保持使用 ```master``` 分支，其默认为 Java SDK 4.0。

在存储库目录中打开终端。 运行以下命令生成应用
...
1. 看到以下内容时，请按 Enter
    ...
    然后，在浏览器中返回到 Azure 门户数据资源管理器。 将会看到，已添加了包含三个空容器的数据库 GroceryStoreDatabase：  
...
1. 在终端中，现在应会看到一条提示
    ...
    **Java SDK 3.7.0**
    ...
    在浏览器中返回到 Azure 门户数据资源管理器。 在“InventoryContainer-leases”容器下，单击“项”以查看其内容。   将会看到，更改源处理器已填充了租约容器，即，处理器已在 InventoryContainer 的某些分区中为 ```SampleHost_1``` 工作线程分配了一个租约。 
...
1. 让代码运行 5-10 秒。 然后返回到 Azure 门户数据资源管理器，并导航到“InventoryContainer”>“项”。  应会看到，项正在插入到库存容器；请记下分区键 (```id```)。

    ![源容器](media/create-sql-api-java-changefeed/cosmos_items.JPG)

1. 现在，请在数据资源管理器导航到“InventoryContainer-pktype”>“项”。  这是具体化视图 - 此容器中的项会镜像 InventoryContainer，因为它们是由更改源以编程方式插入的。  记下分区键 (```type```)。 因此，此具体化视图已针对 ```type``` 查询筛选进行优化，但它在 InventoryContainer 中效率不高，因为此容器是按 ```id``` 分区的。 

    ![具体化视图](media/create-sql-api-java-changefeed/cosmos_materializedview2.JPG)

1. 我们将同时从 InventoryContainer 和 InventoryContainer-pktype 删除某个文档，为此只需使用一个 ```upsertItem()``` 调用即可。   首先查看 Azure 门户数据资源管理器。 我们将删除 ```/type == "plums"``` 的文档；下面以红框突出显示了此项
