...
* 将默认 ResponseContinuationTokenLimitInKB 设置为1kb。 默认情况下，我们会将此限制为1kb，以避免长标头（node.js 具有全局标头大小限制）。 用户可以将此字段设置为允许使用更长的标头，这有助于后端优化查询执行。
...
* 将 _ts 类型从 string 转换为数字（#252）（#295）
* 修复请求费用聚合（#289）
* 允许空字符串分区键（#277）
* 将字符串添加到冲突查询类型（#237）
...
* 添加了对会话一致性的支持。
* 此 SDK 版本需要[Azure Cosmos DB 模拟器](https://aka.ms/cosmosdb-emulator)的最新版本。
...
* 现可控制跨分区查询的并行度。
* 添加了选项，用于在针对 Azure Cosmos DB 模拟器运行时禁用 TLS 验证。
...
* 对限制添加了重试策略支持。 （限制请求收到请求速率太大的异常，错误代码429。）默认情况下，遇到错误代码429时，为每个请求 Azure Cosmos DB 重试九次，并将在响应标头中 retryAfter 时间。 如果想要忽略重试之间由服务器返回的 retryAfter 时间，现在可以对 ConnectionPolicy 对象设置固定的重试间隔时间，并将其作为 RetryOptions 属性的一部分。 Azure Cosmos DB 现在对每个要中止的请求等待最多 30 秒（不考虑重试次数），并返回对错误代码 429 作出的响应。 还可以在 ConnectionPolicy 对象的 RetryOptions 属性中替代该时间。
...
* 修复问题 [#100](https://github.com/Azure/azure-documentdb-node/issues/100) - 专用 HTTPS 代理：避免因 Azure Cosmos DB 用途而修改全局代理。 为所有 lib 请求使用专用代理。
...
> [!WARNING]
> 适用于 SQL API 的节点客户端**SDK 的所有版本 1.x**将于**2020 年8月30日**停用。 这只会影响客户端节点 SDK，而不会影响服务器端脚本（存储过程、触发器和 Udf）。
> 
>
<br/>
