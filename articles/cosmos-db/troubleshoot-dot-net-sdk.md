...
使用直接/TCP 连接时往往会发生 RequestTimeout，但在网关模式下也可能会发生。 这些错误是常见的已知原因，并提供有关如何解决此问题的建议。

* CPU 利用率较高，导致延迟和/或请求超时。 客户可以纵向扩展主机以便为其提供更多的资源，或者可将负载分散到更多的计算机。
* 套接字/端口可用性可能较低。 在 Azure 中运行时，使用 .NET SDK 的客户端可能会遇到 Azure SNAT (PAT) 端口耗尽的情况。 若要减少遇到此问题的机率，请使用最新版本 2.x 或 3.x 的 .NET SDK。 这是为什么建议始终运行最新 SDK 版本的示例。
...
如果没有[超时](#request-timeouts)，且诊断显示单个请求，在和`ResponseTime` `RequestStartTime`之间存在很大的差异，例如，在此示例中为 >300 毫秒）：
