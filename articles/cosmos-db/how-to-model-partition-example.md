...
- **[C4]** 为帖子点赞
- **[Q5]** 列出帖子的点赞数
- **[第6季度]** 列出以简短形式创建的*x*最新帖子（源）

在此阶段，我们尚未考虑每个实体（用户、帖子等）将要包含的详细信息。 此步骤通常是针对关系存储进行设计时首先要处理的步骤之一，因为我们必须弄清楚这些实体将如何在表、列、外键等方面进行转换。文档数据库在写入时不强制实施任何架构，这远不是什么问题。
...
同样，我们的初始查询不会筛选`posts`容器的分区键，这触发了代价高昂的扇出。这一个更糟糕，因为我们的目标是一个更大的结果集，并排序结果与`ORDER BY`子句，这使得它更昂贵的请求单位。
...
## <a name="v3-making-sure-all-requests-are-scalable"></a>V3：确保所有请求都是可伸缩的
...
| **[第 1 季度]** | 2 毫秒 / 1 RU | 2 毫秒 / 1 RU | 2 毫秒 / 1 RU |
| **[C2]** | 9 毫秒 / 8.76 RU | 9 毫秒 / 8.76 RU | 9 毫秒 / 8.76 RU |
| **[Q2]** | 9 毫秒 / 19.54 RU | 2 毫秒 / 1 RU | 2 毫秒 / 1 RU |
| **[第3季度]** | 130 毫秒 / 619.41 RU | 28 毫秒 / 201.54 RU | 4 毫秒 / 6.46 RU |
