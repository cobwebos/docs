...
Azure Cosmos DB 提供五个妥善定义的一致性级别供你选择，以便在一致性、性能和可用性之间进行权衡。 非常一致性确保在配置了 Cosmos 帐户的每个区域中以同步方式复制数据并保持持久性。 此配置提供最高的持久性级别，不过，会降低性能和可用性。 如果应用程序想要在不影响性能的情况下根据需要控制/放宽数据持久性，可以在应用层采用自定义同步，以实现所需的持久性级别。

下图直观描绘了自定义同步模型。

![自定义同步](./media/how-to-custom-synchronization/custom-synchronization.png)

在此方案中，Cosmos 容器是多个大洲的多个区域进行全局复制。 针对此方案中的所有区域使用非常一致性会影响性能。 为确保在不影响写入延迟的情况下提高数据持久性级别，应用程序可以使用两个共享同一会话令牌的客户端。
