...
在此方案中，Azure Cosmos 容器将在跨多个大洲的多个区域中进行全局复制。 针对此方案中的所有区域使用非常一致性会影响性能。 为确保在不影响写入延迟的情况下提高数据持久性级别，应用程序可以使用两个共享同一[会话令牌](how-to-manage-consistency.md#utilize-session-tokens)的客户端。
