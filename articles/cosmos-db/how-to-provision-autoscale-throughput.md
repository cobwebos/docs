...
可以使用[CQL 命令](manage-scale-cassandra.md#use-autoscale)、 [Azure CLI](cli-samples.md)或[Azure 资源管理器模板](resource-manager-samples.md)来预配 Cassandra API Azure Cosmos DB 帐户。

## <a name="azure-cosmos-db-api-for-mongodb"></a>用于 MongoDB 的 Azure Cosmos DB API

可以使用[mongodb 扩展命令](mongodb-custom-commands.md)、 [Azure CLI](cli-samples.md)或[AZURE 资源管理器模板](resource-manager-samples.md)为 mongodb API Azure Cosmos DB 帐户设置自动缩放。
