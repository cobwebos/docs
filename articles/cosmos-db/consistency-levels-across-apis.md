...
与 AzureCosmos DB 不同，Apache Cassandra 本身不提供精确定义的一致性保证。  Apache Cassandra 提供了写入一致性级别和读取一致性级别，以实现高可用性、一致性和延迟折衷。 使用 Azure Cosmos DB 的 Cassandra API 时： 
