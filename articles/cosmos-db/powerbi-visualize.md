...
* 在你的 Cosmos DB 帐户中访问我们的演示帐户或数据。
  ...
  * 或者，若要创建你自己帐户，请参阅[使用 Azure 门户创建 Azure Cosmos DB 数据库帐户](https://azure.microsoft.com/documentation/articles/create-account/)。 然后，要获取类似于本教程中使用的示例火山数据（但不包含 GeoJSON 块），请参阅 [NOAA 站点](https://www.ngdc.noaa.gov/nndc/struts/form?t=102557&s=5&d=5)，然后使用 [Azure Cosmos DB 数据迁移工具导入数据](import-data.md)。
...
![通过 Power BI 连接器完成此 Power BI 教程，你将能够使用 Power BI Desktop 火山报表对数据进行可视化](./media/powerbi-visualize/power_bi_connector_pbireportfinal.png)
...
6. 如下所示指定你想要从其中检索数据的 Cosmos DB 帐户终结点 URL，然后单击“确定”。 可以在 Azure 门户的[“密钥”](manage-account.md#keys)边栏选项卡中的 URI 框检索 URL，或者可以使用演示帐户信息，其中 URL 为 `https://analytics.documents.azure.com`。 
...
7. 如果你是首次连接到此终结点，则将提示你输入帐户密钥。  可以在 Azure 门户的[“只读密钥”](manage-account.md#keys)边栏选项卡中的“主密钥”框检索密钥，或者可以使用演示帐户信息，其中密钥为 `MSr6kt7Gn0YRQbjd6RbTnTt7VHc5ohaAFu7osF0HdyQmfR+YhwCH2D2jcczVIR1LNK3nMPNBD31losN7lQ/fkw==`。 输入帐户密钥并单击“连接”。
   
    我们建议你在生成报表时使用只读密钥。  这将防止主密钥不必要地暴露于潜在的安全风险中。 只读密钥从 Azure 门户的“密钥”[](manage-account.md#keys)边栏选项卡中可用，或者你可以使用上文提供的演示帐户信息。
...
11. 此时我们已经完成了将数据平展到表格格式中。  可以利用查询编辑器中所有可用的功能在 Cosmos DB 中形成和转换数据。  如果正在使用示例，可以通过在“主页”功能区上更改“数据类型”，将“海拔”数据类型更改为“整数”。
    ...
    ![Azure Cosmos DB Power BI 连接器的 Power BI 教程 - 更改列类型](./media/powerbi-visualize/power_bi_connector_pbichangetype.png)
12. 单击“关闭并应用”以保存数据模型。
...
![PowerBI.com 中“立即刷新”的屏幕截图](./media/powerbi-visualize/power-bi-refresh-now.png)

对于计划刷新，请执行以下操作。

1. 单击操作列表中的“计划刷新”。 

    ![PowerBI.com 中计划刷新的屏幕截图](./media/powerbi-visualize/power-bi-schedule-refresh.png)
2. 在“设置”页上，展开“数据源凭据”。 
3. 单击“编辑凭据”。 
