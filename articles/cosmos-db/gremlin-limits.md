...
运算符深度** | **400** |  遍历中的唯一步骤的总数。 例如，```g.V().out()```运算符计数为 2：V（） 和 out（），```g.V('label').repeat(out()).times(100)```运算符深度为 3：V（）、重复（）和 out（），```.times(100)```因为是运算符的```.repeat()```参数。
*并行性程度* | **32** | 发往存储层的单个请求中查询的存储分区的最大数。 具有数百个分区的图将受到此限制的影响。
