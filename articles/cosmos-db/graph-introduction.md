...
* [统包式全局分配](distribute-data-globally.md)
* 在全球范围内[灵活缩放吞吐量和存储](partition-data.md)
* 99% 的情况下低至个位数的毫秒级延迟
...
- 自动索引编制，可使用即时查询。
- 可优化的一致性级别。
- 综合 SLA，包括为所有单区域帐户和具有松散一致性的所有多区域帐户提供的 99.99% 可用性 SLA，以及为所有多区域数据库帐户提供的 99.999% 读取可用性。
