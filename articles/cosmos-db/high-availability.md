...
- 发生区域性服务中断时，单区域帐户可能会失去可用性。 始终建议 **至少设置两个区域** (最好的方法是使用 Azure Cosmos 帐户) 至少两个写入区域，以确保在任何时候都能实现高可用性。
...
- Azure Cosmos 帐户使用具有两个或更少读取区域的强一致性 (这包括读取 & 写入区域) 会在读取区域中断期间丢失写入可用性，但会保持剩余区域的读取可用性。
...
- 即使在 Azure 区域永久无法恢复的情况下，如果多区域 Azure Cosmos 帐户配置了 *强* 一致性，也不会造成数据丢失。 在永久无法恢复的写入区域（配置有有限过期一致性的多区域 Azure Cosmos 帐户）中，可能的数据丢失时段限制为过期时段 (*k* 或 *t*) 其中 k = 100000 更新，T = 5 分钟。 对于会话、一致前缀和最终一致性级别，潜在的数据丢失时段限制为最多 15 分钟。 有关 Azure Cosmos DB 的 RTO 和 RPO 目标的详细信息，请参阅[一致性级别和数据持续性](consistency-levels-tradeoffs.md#rto)
