...
description: 本文介绍了如何通过在与 Azure Cosmos 帐户关联的所有区域中以透明 Azure Cosmos DB 方式复制数据来提供高可用性
author: markjbrown
...
- 即使在不常发生的事件中，当 Azure 区域永久恢复时，如果你的多区域 Cosmos 帐户配置了默认的 "*强*" 一致性级别，则不会丢失数据。 在永久无法恢复的写入区域中，对于配置了有限过期一致性的多区域 Cosmos 帐户，可能的数据丢失时段限制为过期窗口（*K*或*T*）;对于会话、一致前缀和最终一致性级别，潜在的数据丢失时段限制为最长5秒。 
...
- 即使 Cosmos 帐户具有高可用性，应用程序也不一定能够正常保持高可用性。 若要测试应用程序的端到端高可用性，请[使用 Azure CLI 或 Azure 门户定期调用手动故障转移](how-to-manage-database-account.md#manual-failover)，作为应用程序测试或灾难恢复（DR）演练的一部分。
