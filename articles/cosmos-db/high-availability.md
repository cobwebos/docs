...
将区域添加到新的或现有的 Azure Cosmos 帐户时, 可以启用区域冗余。 目前, 只能使用 Azure 门户、PowerShell 和 Azure 资源管理器模板来启用区域冗余。 若要在 Azure Cosmos 帐户上启用区域冗余, 应将`isZoneRedundant`标志设置为`true` , 以指定特定位置。 可以在 "位置" 属性中设置此标志。 例如, 以下 powershell 代码片段为 "东南亚" 区域启用区域冗余:
