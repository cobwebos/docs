...
若要配置共享租约容器，需要在触发器中进行的唯一额外配置是在使用C#或 `leaseCollectionPrefix`[属性](../azure-functions/functions-bindings-cosmosdb-v2-trigger.md)时添加 `LeaseCollectionPrefix`[属性](../azure-functions/functions-bindings-cosmosdb-v2-trigger.md#attributes-and-annotations)（如果使用的是 JavaScript）。 属性的值应是特定触发器的逻辑描述符。
...
对于 JavaScript，可以使用 `function.json` 属性在 `leaseCollectionPrefix` 文件上应用配置：
