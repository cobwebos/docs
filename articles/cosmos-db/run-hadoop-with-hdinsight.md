...
description: "了解如何使用 Azure Cosmos DB 和 Azure HDInsight 运行简单的 Hive、Pig 和 MapReduce 作业。"
services: cosmos-db
author: dennyglee
manager: jhubbard
editor: mimig
...
> [!IMPORTANT] 
> Spark 到 Azure Cosmos DB 的连接器是用于将 Azure HDInsight 连接到 Azure Cosmos DB 的建议选项。 有关详细信息，请参阅[使用 Spark 到 Azure Cosmos DB 的连接器加速实时大数据分析](spark-connector.md)。
...
建议通过观看以下视频来入门，在视频中，我们通过使用 Cosmos DB 和 HDInsight 来运行 Hive 作业。

> [!VIDEO https://channel9.msdn.com/Blogs/Azure/Use-Azure-DocumentDB-Hadoop-Connector-with-Azure-HDInsight/player]
>
>

然后，返回到本文，在这里可获得有关如何对 Cosmos DB 数据运行分析作业的完整详细信息。
...
没有时间完成教程，只想获得有关 Hive、Pig 和 MapReduce 的完整示例 PowerShell 脚本？ 没问题，可在[此处][hdinsight-samples]获得。 此下载还包含对于这些示例的 hql、pig 及 java 文件。
...
请输入： </br> <strong>https://portalcontent.blob.core.windows.net/scriptaction/documentdb-hadoop-installer-v04.ps1</strong>。</td></tr>
         <tr><td>标头</td>
             <td>单击复选框运行 PowerShell 脚本到标头节点上。</br></br>选中此复选框
             <strong></strong>。</td></tr>
         <tr><td>辅助角色</td>
             <td>单击复选框运行 PowerShell 脚本到辅助角色节点上。</br></br>选中此复选框
             <strong></strong>。</td></tr>
         <tr><td>Zookeeper</td>
             <td>单击复选框运行 PowerShell 脚本到 Zookeeper 上。</br></br>无需
             <strong></strong>。
             </td></tr>
         <tr><td>parameters</td>
             <td>根据脚本的需要，指定参数。</br></br>无需参数
             <strong></strong>。</td></tr>
     </table>
...
## <a name="RunHive"></a>步骤 3：使用 Azure Cosmos DB 和 HDInsight 运行 Hive 作业
> [!IMPORTANT]
> 必须使用配置设置填写所有由 < > 表示的变量。
>
>
...
2. <p>让我们开始构造查询字符串。 我们将编写 Hive 查询，该查询采用来自 Azure Cosmos DB 集合的所有文档系统生成的时间戳 (_ts) 和唯一 ID (_rid)，按分钟计算所有文档，并将结果存储回新 Azure Cosmos DB 集合。</p>
    ...
    <p>首先，从 Azure Cosmos DB 集合创建 Hive 表。 将以下代码片段添加到 PowerShell 脚本窗格中从 #1 开始的代码片段<strong>之后</strong>。 请确保包含可选的查询参数，以便将文档调整为 just_ts 和 _rid。</p>
...
1. 接下来，让我们为输出集合创建 Hive 表。 输出文档属性将为月、日、小时、分钟和发生次数总数。

   > [!NOTE]
   > **再次，命名 DocumentDB.outputCollections 不是一个错误。** 是，我们允许将多个集合作为输出添加： </br>
   > '*DocumentDB.outputCollections*' = '*\<DocumentDB Output Collection Name 1\>*,*\<DocumentDB Output Collection Name 2\>*' </br> 仅使用逗号将集合名称隔开，无需空格。 </br></br>
...
2. <p>让我们开始构造查询字符串。 我们将编写 Pig 查询，该查询采用来自 Azure Cosmos DB 集合的所有文档系统生成的时间戳 (_ts) 和唯一 ID (_rid)，按分钟计算所有文档，并将结果存储回新 Azure Cosmos DB 集合。</p>
    <p>首先，从 Cosmos DB 将文档加载到 HDInsight 中。 将以下代码片段添加到 PowerShell 脚本窗格中从 #1 开始的代码片段<strong>之后</strong>。 请确保向可选的 DocumentDB 查询参数添加了查询，以便将文档调整到 just_ts 和 _rid。</p>

   > [!NOTE]
   > 是，我们允许将多个集合作为输入添加： </br>
   > '*\<DocumentDB Input Collection Name 1\>*,*\<DocumentDB Input Collection Name 2\>*'</br> 仅使用逗号将集合名称隔开，无需空格。 </b>
   >
   >
...
4. 最后，让我们将结果存储到我们新的输出集合。
   ...
   > [!NOTE]
   > 是，我们允许将多个集合作为输出添加： </br>
   > '\<DocumentDB Output Collection Name 1\>,\<DocumentDB Output Collection Name 2\>'</br> 仅使用逗号将集合名称隔开，无需空格。</br>
...
* [使用 Azure Cosmos DB 开发 Java 应用程序][sql-api-java-application]
