---
title: Azure Cosmos DB 中的不同 Api 的常见问题
...
[请求单位](request-units.md) (RU) 是 Azure Cosmos DB 中吞吐量的衡量单位。 1RU 吞吐量对应于获取 1 KB 文档的吞吐量。 在 Azure Cosmos DB 中进行的每个操作（包括读取、写入、SQL 查询和存储过程执行）都具有一个确定性的 RU 值，该值基于完成该操作所需的吞吐量。 无需考虑 CPU、IO 和内存，以及它们会怎样影响应用程序吞吐量，而是从单个 RU 度量值的角度进行考虑。

可以配置每个 Azure Cosmos 容器，每个容器的吞吐量为每秒的每秒 ru 数。 对于任何规模的应用程序，都可以将单个请求设为基准以测量其 RU 值，并预配容器以处理所有请求的请求单位总和。 也可以根据应用程序的发展需求，相应地增加或减少容器的吞吐量。 有关请求单位的详细信息以及确定容器需求方面的帮助，请尝试[吞吐量计算器](https://www.documentdb.com/capacityplanner)。
...
Azure Cosmos DB 是 Microsoft 提供的全球分布式多模型数据库服务。 如果多模型意味着 Azure Cosmos DB 支持多个 Api 和多个数据模型，则不同的 Api 将不同的数据格式用于存储和网络协议。 例如，SQL 使用 JSON，MongoDB 使用 BSON，表使用 EDM，Cassandra 使用 CQL，Gremlin 使用 GraphSON。 因此，建议使用相同的 API 对给定帐户中的数据进行所有访问。
...
也可以使用 [Azure Cosmos DB 模拟器](local-emulator.md)在本地免费开发和测试应用程序，而无需创建 Azure 订阅。 如果对应用程序在 Azure Cosmos DB 模拟器中的工作情况感到满意，可以转为在云中使用 Azure Cosmos DB 帐户。
...
Azure 门户中提供了 Azure Cosmos DB。 首先注册 Azure 订阅。 注册后，可以将 Azure Cosmos DB 帐户添加到 Azure 订阅。

### <a name="what-is-a-master-key"></a>什么是主密钥？

主密钥是用于访问帐户的所有资源的安全令牌。 拥有此密钥的人对数据库帐户中的所有资源具有读取和写入访问权。 分发主密钥时要格外小心。 在[Azure 门户][azure-portal]的 "**密钥**" 边栏选项卡中提供主要主密钥和辅助主密钥。 有关密钥的详细信息，请参阅[查看、复制和重新生成访问密钥](manage-with-cli.md#list-account-keys)。
...
容器级和数据库级吞吐量预配是不同的产品，在这两者之间切换需要将数据从源迁移到目标。 这意味着你需要创建新的数据库或新的容器，然后通过使用[批量执行程序库](bulk-executor-overview.md)或[Azure 数据工厂](../data-factory/connector-azure-cosmos-db.md)来迁移数据。
...
有关详细信息，请参阅 Azure Cosmos DB[服务配额](concepts-limits.md)，以及[每个容器和数据库](set-throughput.md#comparison-of-models)项目的限制。
...
* 对于 SQL API，容器映射到容器。
* 对于 MongoDB 帐户的 Cosmos DB API，容器映射到集合。
...
你可以使用[Azure 门户](https://portal.azure.com)创建数据库，如[添加容器](create-sql-api-java.md#add-a-container)、 [Azure Cosmos DB sdk](sql-api-sdk-dotnet.md)之一或[REST api](/rest/api/cosmos-db/)中所述。
...
SQL API 通过 JavaScript 存储过程和触发器支持语言集成式事务。 脚本内的所有数据库操作均在快照隔离下执行。 如果它是单分区容器，则执行的作用域为容器。 如果容器已分区，则执行的作用域为容器中具有相同分区键值的文档。 文档版本 (ETag) 的快照是在事务开始时获取的，且只有当脚本成功运行时才会提交。 如果 JavaScript 引发错误，则会回滚事务。 有关详细信息，请参阅 [Azure Cosmos DB 的服务器端 JavaScript 编程](stored-procedures-triggers-udfs.md)。
...
是，因为 Azure Cosmos DB 是 RESTful 服务，而资源链接固定不变，所以可以缓存。 SQL API 客户端可以指定 "If-无-Match" 标头，用于对任何类似资源的文档或容器进行读取，然后在服务器版本发生更改后更新其本地副本。
...
这是 JavaScript 的限制。 JavaScript 使用 IEEE 754 中指定的双精度浮点数格式数字，并且它可以安全地在-（2<sup>53</sup> -1）和 2<sup>53</sup>-1 （即9007199254740991）之间保存数字。
...
Azure Cosmos DB 的 MongoDB API 是一种有线协议兼容性层，它允许应用程序使用现有的、社区支持的 Sdk 和 MongoDB 驱动程序轻松透明地与本机 Azure Cosmos 数据库引擎进行通信。 开发人员现在可以使用现有的 MongoDB 工具链和技能来构建利用 Azure Cosmos DB 的应用程序。 开发人员受益于 Azure Cosmos DB 的独特功能，其中包括具有多主机复制的全局分发、自动索引、备份维护、财务支持的服务级别协议（Sla）等。

### <a name="how-do-i-connect-to-my-database"></a>如何连接到我的数据库？

若要通过 Azure Cosmos DB 的用于 MongoDB 的 API 连接到 Cosmos 数据库，最快捷的方法是使用 [Azure 门户](https://portal.azure.com)。 转到你的帐户，然后在左侧导航菜单上单击“快速启动”。 快速入门是获取代码片段以连接到数据库的最佳方式。
...
|---------------------|-------|--------------|-----------|
| TooManyRequests     | 16500 | 使用的请求单位总数大于容器的预配请求单位率，已受到限制。 | 考虑从 Azure 门户中对分配给一个容器或一组容器的吞吐量进行缩放，或者重试。 |
| ExceededMemoryLimit | 16501 | 作为一种多租户服务，操作已超出客户端的内存配额。 | 通过限制性更强的查询条件缩小操作的作用域，或者通过 [Azure 门户](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade)联系支持人员。 <br><br>示例： <em>&nbsp;&nbsp;&nbsp;&nbsp;getCollection （"users"）。聚合（[<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$match： {name： " <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$sort： {age：-1}}<br>&nbsp;&nbsp;&nbsp;&nbsp;])</em>) |
...
[Azure 门户][azure-portal]提供了 Azure Cosmos DB 表 API。 首先必须注册 Azure 订阅。 注册成功后，可将 Azure Cosmos DB 表 API 帐户添加到 Azure 订阅，然后向帐户添加表。
...
* 表 API 返回的查询结果未按分区键/行键顺序排列，因为它们位于 Azure 表存储中。
* 行键最多只能包含 255 个字节
* 批最多只能包含 2 MB
* 目前不支持 CORS。
* Azure 表存储中的表名称不区分大小写，但它们处于 Azure Cosmos DB 表 API
...
* 需要后端的容器或容器组提供更大的吞吐量。 例如，发现已用吞吐量超过预配吞吐量，且吞吐量已达到限制。 有关详细信息，请参阅[设置 Azure Cosmos 容器的吞吐量](set-throughput.md)。
...
Azure Cosmos DB 利用[水平分区](partition-data.md)自动满足增加存储和吞吐量的需求。 工作负荷的最大吞吐量和存储容量取决于与给定容器关联的分区数。 不过，Gremlin API 容器具有一组特定的指导原则，可确保大规模获得适当的性能体验。 有关分区和最佳做法的详细信息，请参阅 [Azure Cosmos DB 中的分区](partition-data.md)一文。

### <a name="how-can-i-protect-against-injection-attacks-using-gremlin-drivers"></a>如何使用 Gremlin 驱动程序防范注入式攻击？

大多数本机 Apache Tinkerpop Gremlin 驱动程序允许选项提供用于执行查询的参数字典。 这里提供了有关如何执行此操作的示例（分别以 [Gremlin.Net](https://tinkerpop.apache.org/docs/3.2.7/reference/#gremlin-DotNet) 和 [Gremlin Javascript](https://github.com/Azure-Samples/azure-cosmos-db-graph-nodejs-getting-started/blob/master/app.js) 编写）。
...
上述配置文件的输出显示了获取顶点对象、边缘对象和工作数据集的大小所花费的时间。 这与 Azure Cosmos DB 查询的标准成本度量相关。

## <a id="cassandra"></a> Cassandra API

### <a name="what-is-the-protocol-version-supported-by-azure-cosmso-db-cassandra-api-is-there-a-plan-to-support-other-protocols"></a>Azure Cosmso DB 支持的协议版本是什么 Cassandra API？ 是否打算支持其他协议？
...
出于可用性和低延迟的原因，Azure Cosmos DB 提供现成的全局分发。 不需要设置副本或其他内容。 在提供性能保证的任何区域中，所有写入始终都持久仲裁提交。
...
使用 CQLv3 的 Apache Cassandra SDK 的客户端驱动程序用于客户端程序。 如果使用其他驱动程序或者遇到问题，请向 [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com) 发送邮件。
...
目前，Azure Cosmos DB 针对云环境提供了优化的体验，且不产生操作开销。 如果需要配对，请向 [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com) 发送邮件并在其中提供你的方案的说明。 我们正在致力于帮助将本地/不同云 Cassandra 群集与 Cosomos DB 的 Cassandra API 配对。
...
Cassandra API 计划支持辅助索引，以帮助对某些属性创建选择性索引。 
...
### <a name="azure-cosmos-db-as-a-platform-seems-to-have-lot-of-capabilities-such-as-change-feed-and-other-functionality-will-these-capabilities-be-added-to-the-cassandra-api"></a>作为平台 Azure Cosmos DB 似乎有许多功能，例如更改源和其他功能。 这些功能是否将添加到 Cassandra API 中？
