...
* **按小时付费：** 对于需要在任何时间点进行缩放的大规模工作负荷，只需按小时收费。 应用程序中的工作负荷通常会在一年中的不同时间因查询的数据有所变化。 使用 Azure Cosmos DB 可按需进行扩展或缩减，只有所需的资源才产生费用。 不能搭配本地或 IaaS 托管的系统使用此模型，因为没有任何方法可以每隔一小时解除硬件。 在这种情况下，使用 Azure Cosmos DB 可能会将成本平均节省 10 到 14 倍。
