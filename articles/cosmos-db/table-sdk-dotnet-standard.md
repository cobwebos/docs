...
### <a name="a-name0110-preview0110-preview"></a><a name="0.11.0-preview"/>0.11.0-预览
* 对 CloudTableClient 的配置方式进行了更改。 现在，它在构造过程中使用 TableClientConfiguration 对象。 TableClientConfiguration 提供不同的属性来配置客户端行为，具体取决于 Cosmos DB 表 API 还是 Azure 存储表 API 上的目标终结点。
* 添加了对 TableQuery 的支持，以在自定义列上按排序顺序返回结果。 此功能仅在 Cosmos DB 表终结点上受支持。
* 添加了对不同结果类型公开 RequestCharges 的支持。 此功能仅在 Cosmos DB 表终结点上受支持。
...
| [0.11.0-预览](#0.11.0-preview) |2019年3月5日 |--- |
