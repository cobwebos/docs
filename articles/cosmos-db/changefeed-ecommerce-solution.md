...
Azure Cosmos DB 更改源是一种机制，用于在创建或修改这些记录时从 Azure Cosmos 容器获取连续和增量的记录源。 更改源支持的工作原理是侦听容器中发生的任何更改。 然后，它会按照所更改文档的修改顺序输出这些文档的排序列表。 若要详细了解更改源，请参阅[使用更改源](change-feed.md)一文。 
...
2. **宇宙数据库：** 生成的数据存储在 Azure Cosmos 容器中。  

3. **更改源：** 更改源将侦听对 Azure Cosmos 容器的更改。 每次将新文档添加到集合时（即，发生了用户查看商品、将商品添加到购物车或购买商品等事件时），更改源将触发一个 [Azure 函数](../azure-functions/functions-overview.md)。  
...
1. 转到[Azure 门户](https://portal.azure.com/)，查找模板部署创建的**Azure Cosmos DB 帐户**。  
...
创建新文档或在 Cosmos 容器中修改当前文档时，更改源会自动将修改的文档添加到其集合历史记录中。 现在，我们生成并运行一个用于处理更改源的 Azure 函数。 在创建的集合中创建或修改文档时，更改源会触发该 Azure 函数。 然后，该 Azure 函数会将修改后的文档发送到事件中心。
...
7. 如果导航到[Azure 门户](https://portal.azure.com/)，则导航到资源组中的 Cosmos DB 帐户，然后导航到**数据资源管理器**，您将看到在**更改源实验室集合**中导入的随机数据。
...
1. 从[Azure 门户](https://portal.azure.com/)导航到资源组，然后导航到**流作业 1（** 您在预实验室中创建的流分析作业）。  
...
6. 在“可视化效果类型”字段中，从下拉菜单中选择“簇状条形图”。******** 在“轴”下面添加操作。**** 跳过“图例”，不要添加任何内容。**** 然后，在下一节"**值**"下，添加**vg**。选择 **"下一步**"，然后标题图表，然后选择 **"应用**"。 仪表板上应会出现一个新图表！  
...
现在，我们知道可以如何使用新的数据分析工具来连接实际的电子商务站点。 为了构建电子商务网站，请使用 Azure Cosmos 数据库来存储产品类别（女性、男性、Unisex）、产品目录和最受欢迎的项目列表的列表。

1. 导航回[Azure 门户](https://portal.azure.com/)，然后导航到**Cosmos DB 帐户**，然后导航到**数据资源管理器**。  

   在具有固定存储容量**的更改源实验室** - **产品和****类别**下添加两个集合。
...
要删除在本实验期间创建的资源，请导航到[Azure 门户](https://portal.azure.com/)上的资源组，然后从页面顶部的菜单**中选择"删除资源组**"，然后按照提供的说明进行操作。
