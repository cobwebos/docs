---
title: 有关自动缩放模式的常见问题 Azure Cosmos DB
...
使用自动缩放模式中预配的吞吐量，Azure Cosmos DB 会根据使用情况自动管理和缩放容器或数据库的 RU/秒。 本文解答了有关自动缩放的常见问题。

## <a name="frequently-asked-questions"></a>常见问题

### <a name="is-autoscale-mode-supported-for-all-apis"></a>所有 Api 是否支持自动缩放模式？
是，所有 Api 都支持自动缩放模式： Core （SQL）、Gremlin、Table、Cassandra 和 API for MongoDB。

### <a name="is-autoscale-mode-supported-for-multi-master-accounts"></a>多主帐户是否支持自动缩放模式？
是的，多主机帐户支持自动缩放模式。 添加到 Cosmos 帐户的每个区域都提供最大 RU/s。 
...
可在新的容器和使用 Azure 门户创建的数据库上启用自动缩放模式。 

### <a name="is-there-cli-or-sdk-support-to-create-containers-or-databases-with-autoscale-mode"></a>在自动缩放模式下创建容器或数据库是否支持 CLI 或 SDK？
目前，只能通过 Azure 门户创建自动缩放模式的资源。 尚不支持 CLI 和 SDK。

### <a name="can-i-enable-autoscale-on-an-existing-container-or-a-database"></a>能否对现有容器或数据库启用自动缩放？
目前，可以在创建新容器和数据库时启用自动缩放。 目前尚不支持在现有容器和数据库上启用自动缩放模式。 可以使用 [Azure 数据工厂](../data-factory/connector-azure-cosmos-db.md)或[更改源](change-feed.md)将现有容器迁移到新容器。 

### <a name="can-i-turn-off-autoscale-mode-on-a-container-or-database"></a>能否在容器或数据库上关闭自动缩放模式？
是的，你可以通过切换到预配的吞吐量的 "手动" 选项来关闭自动缩放。 在当前版本中，在从自动缩放模式切换到手动模式后，不能再次为同一资源启用自动缩放。 

### <a name="is-autoscale-mode-supported-for-shared-throughput-databases"></a>共享吞吐量数据库是否支持自动缩放模式？
是，共享吞吐量数据库支持自动缩放模式。 若要启用此功能，请在创建数据库时选择自动缩放模式和 "**设置吞吐量**" 选项。 
...
对于启用了自动缩放模式的共享吞吐量数据库，允许的集合数为：最小值（25，最大 RU/秒，即 database/1000）。 例如，如果数据库的最大吞吐量为 20,000 RU/s，则数据库可以有 MIN(25, 20,000 RU/s / 1000) = 20 个集合。 
...
在自动缩放模式下，可根据传入流量，即时增加或减少 ru/秒范围内的 RU/s。 计费时间间隔为 1 小时，即，按某一小时内的最高 RU/s 收费。

### <a name="can-i-specify-a-custom-max-throughput-rus-value-for-autoscale-mode"></a>能否为自动缩放模式指定自定义的最大吞吐量（RU/s）值？
...
是的。 在容器的 "**缩放 & 设置**" 选项或数据库的 "**缩放**" 选项中，可以为自动缩放模式选择更高的最大 RU/秒。 这是一个异步增加操作，随着服务预配更多资源来支持更高的规模，该操作可能需要一段时间才能完成（通常为 4-6 小时，具体取决于所选的 RU/s）。 
...
* 了解[在自动缩放模式下预配吞吐量的好处](provision-throughput-autoscale.md#autoscale-benefits)。
