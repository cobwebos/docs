...
数据库事务提供一种安全且可预测的编程模型来处理数据的并发更改。 传统关系数据库（如 SQL Server）允许您使用存储过程和/或触发器编写业务逻辑，并将其发送到服务器以直接在数据库引擎内执行。 对于传统的关系数据库，需要使用两种不同的编程语言（例如 JavaScript、Python、 C#、Java 等）和数据库本身执行的事务编程语言（例如 t-sql）来处理这两种编程语言。
...
存储在 Azure Cosmos 容器中的每个项都具有系统定义的 `_etag` 属性。 每次更新项时，`_etag` 的值都由服务器自动生成和更新。 `_etag` 可用于提供 `if-match` 请求标头的客户端，以允许服务器决定是否可以有条件地更新项。 `if-match` 标头的值与服务器上 `_etag` 的值相匹配，则将更新项。 如果 `if-match` 请求标头的值不再是最新的，则服务器将拒绝该操作并出现 "HTTP 412 前置条件失败" 响应消息。 然后，客户端可以重新提取项以获取服务器上项的当前版本，或重写服务器中项的版本及其自身 `_etag` 值。 此外，可以将 `_etag` 与 `if-none-match` 标头一起使用，以确定是否需要资源的重新提取。

每次更新项时，项的 `_etag` 值都会发生更改。 对于 "替换项" 操作，必须将 `if-match` 显式表示为请求选项的一部分。 有关示例，请参阅 [GitHub](https://github.com/Azure/azure-cosmos-dotnet-v3/blob/master/Microsoft.Azure.Cosmos.Samples/Usage/ItemManagement/Program.cs#L578-L674) 中的示例代码。 对于存储过程所涉及的所有写入项，将隐式检查 `_etag` 值。 如果检测到任何冲突，则存储过程将回滚该事务并引发异常。 通过此方法，将以原子方式应用存储过程中的所有写入内容或不应用任何写入内容。 这是应用程序重新应用更新并重试原始客户端请求的信号。
