...
Cosmos DB 旨在提供可预测的性能。 创建容器时，可以根据每秒**[请求单位](request-units.md) (RU) 数保留吞吐量，为每分钟 RU 数留出潜在的余量**。 将为每个请求分配请求单位费用，该费用与系统资源（如操作使用的 CPU、内存和 IO）的数量成正比。 读取 1-KB 会话一致性文档将消耗一个请求单位。 读取为 1 RU 时不考虑存储的项数量或同时运行的并发请求数。 较大的项要求更高的请求单位数，具体取决于大小。 如果知道实体大小及为应用程序提供支持需要的读取次数，则可以设置应用程序读取所需的那个吞吐量。 
