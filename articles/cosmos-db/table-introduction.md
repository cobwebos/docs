...
| 全球分布 | 具有一个可选可读辅助读取区域以实现高可用性的单一区域。 | 从 1 个到任意多个区域进行[统包式全局分发](distribute-data-globally.md)。 支持在全球各地随时[自动和手动故障转移](high-availability.md)。 多主数据库功能，允许任何区域接受写入操作。 |
