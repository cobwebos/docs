<properties pageTitle="在 Azure PowerShell 中使用资源管理器模式的常见 VM 任务" description="为你提供在 Azure PowerShell 中使用资源管理器模式的常见 VM 任务的命令" services="virtual-machines" documentationCenter="windows" authors="KBDAzure" manager="timlt" editor=""/>

<tags ms.service="virtual-machines" ms.date="04/22/2015" wacn.date=""/>

# 在资源管理器模式下使用 Azure PowerShell 的常见 VM 任务
...
每天执行的许多管理 VM 的任务都可通过在 Azure 资源管理器模式下使用 Azure PowerShell cmdlet 自动执行。本文提供较简单任务的示例命令，并提供演示更复杂任务的命令的文章链接。
...
如果使用资源组名称“MyRG”和 VM 名称“MyVM”运行命令，应看到如下内容：
