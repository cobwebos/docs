<properties
   pageTitle="了解 Apache Storm on HDInsight | Azure"
   description="Learn 如何使用 Apache Storm on HDInsight 在 Azure 云中构建实时数据分析。"
   services="hdinsight"
   documentationCenter=""
   authors="Blackmist"
   manager="paulettm"
...
# Storm on HDInsight 概述

Apache Storm on HDInsight 可让你使用 [Apache Hadoop](http://hadoop.apache.org) 在 Azure 环境中创建分布式实时数据处理解决方案。
...
* 与其他 Azure 服务（包括事件中心、Azure 虚拟网络、SQL Database、Blob 存储和 DocumentDB）集成

	* 通过使用 Azure 虚拟网络中组合多个 HDInsight 群集的功能：创建使用 HDInsight、HBase 或 Hadoop 群集的分析管道

有关使用 Apache Storm 的公司列表，请参阅[使用 Apache Storm 的公司](https://storm.apache.org/documentation/Powered-By.html)。
...
下面是一个演示如何使用 Azure 门户设置 Storm on HDInsight 群集的示例。你也可以使用 [Azure PowerShell](install-configure-powershell) 设置 Storm 群集。
...
Apache Storm 群集运行的是**拓扑**，而不是你在 HDInsight 或 Hadoop 中可能熟悉的 MapReduce 作业。Storm 群集包含两种节点：运行 **Nimbus** 的头节点和运行 **Supervisor** 的辅助节点
...
* **工作进程**：运行一部分**拓扑**。正在运行的拓扑分布在整个群集的许多工作进程上。

* **拓扑**：定义处理数据**流**的计算图形。与 MapReduce 作业不同，拓扑运行到你停止它们为止。

* **流**：无限制的**元组**集合。流由 **spout** 和 **bolt** 生成，并且由 **bolt** 使用。
...
* **Bolt**：使用**流**，对**元组**执行处理，并且可以发出**流**。Bolt 还负责将数据编写到外部存储，比如队列、HDInsight HBase、blob 或其他数据存储。

* **Apache Thrift**：一个软件框架，用于可扩展的跨语言服务开发。可用于构建在 C++、Java、Python、PHP、Ruby、Erlang、Perl、Haskell、C#、Cocoa、JavaScript、Node.js、Smalltalk 及其他语言间工作的服务。
...
* [为 Apache Storm on HDInsight 开发基于 Java 的拓扑](hdinsight-storm-develop-java-topology)

* [使用 HDInsight 上的 Storm 和 HBase 分析传感器数据](hdinsight-storm-sensor-data-analysis)
