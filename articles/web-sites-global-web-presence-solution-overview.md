<properties linkid=" Websites-global-web-presence" urlDisplayName="在 Azure 网站上打造全球网络影响力" pageTitle="在 Azure 网站上打造全球网络影响力" metaKeywords="" description="本指南提供了如何在 Azure 网站上托管您的组织的 (.COM) 网站的技术概述。内容包括部署、自定义域、SSL 和监视。" metaCanonical="http://www.windowsazure.cn/zh-cn/documentation/articles/web-sites-global-web-presence-solution-overview/" services="" documentationCenter="" title="Create a Global Web Presence on Azure  Websites" authors="jroth" solutions="" manager="paulettm" editor="mollybos" />
...
本指南提供了如何在 Azure 上托管您的组织的 (.COM) 网站的技术概述。此方案也称作全球网络影响力。本指南重点介绍使用[ Azure 网站][websitesoverview]，因为网站是在 Azure 上创建、迁移、缩放和管理 Web 应用程序的最快和最简单方法。但是，某些应用程序要求适合于运行 IIS 的[Azure 云服务][csoverview]或[Azure 虚拟机][vmoverview]。它们还是托管 Web 应用程序的极佳选择。如果你处于初始计划阶段，请查看文档 [Azure 网站、云服务和虚拟机：何时使用何种产品？][chooseservice]。如果没有针对使用云服务或虚拟机的要求，我们建议使用网站来发挥您的全球网络影响力。本文的剩余部分将提供将网站用于此情形的指南。 
...
> [AZURE.NOTE]
> 本指南展示为面向公众的 .COM 站点开发而进行调整的一些最常见领域和任务。但是，还有可在特定实施中使用的其他一些 Azure 网站功能。若要查看这些功能，还请参阅数字市场营销活动 <a href="/zh-cn/documentation/articles/web-sites-digital-marketing-application-solution-overview/">和业务应用程序</a> 和 <a href="/zh-cn/documentation/articles/web-sites-business-application-solution-overview">其他指南</a>。

<!--
> 如果您想要在注册帐户之前开始使用 Azure 网站，请转到 <a href="https://trywebsites.chinacloudsites.cn/">https://trywebsites.chinacloudsites.cn</a>其中您可以在 Azure 网站中立即免费创建短期 ASP.NET 入门网站。无需信用卡，没有承诺。
-->
## <a name="createwebsite"></a>创建 Azure 网站
使用 Azure 管理门户，您可以通过几种方式创建新的 Azure 网站。在门户的底部单击**新建**按钮时，将出现以下对话框：

![GlobalWebCreate][GlobalWebCreate]

有三个选项用于创建新网站：**"快速创建**、**自定义创建**和**从库中**。对于上述每个选项，您应选择适合于您的主要用户群的 Azure 区域。

如果您正在迁移现有网站，**自定义创建选项**允许您创建或关联 SQL Database 或 MySQL 数据库。此选项还提供为部署指定若干源代码管理选项的功能，例如 GitHub 或 Team Foundation Server (TFS)。如果您已经在使用源代码管理机制管理您的网站，那么这是一种针对部署设置 Azure 网站的快速方法。

<!--
**从库中**选项允许您使用若干框架之一（例如 Drupal 或 WordPress）设置新网站。这可以有助于快速设置新网站，然后在所选框架内对新网站进行自定义。
-->
...
这里的许多部署任务都涉及使用 Azure 管理门户中的信息。转到您的网站，选择**仪表板**选项卡，然后查看**速览**部分。下面的屏幕快照显示若干选项。

![GlobalWebQuickGlance][GlobalWebQuickGlance]

某些源代码管理工具和 FTP 客户端要求用户名/密码访问。对于新网站，不自动创建凭据。不过，您可以通过单击**重置部署凭据**轻松地创建凭据。在完成后，您可以通过在同一个**仪表板**页将这些凭据与**FTP 主机名**一起使用，使用任何 FTP 客户端部署您的网站。
...
还可以选择通过源代码管理服务（例如 GitHub 或 TFS Online）进行部署。单击针对**从源代码管理设置部署**的选项。然后，按照针对您选择的源代码管理系统或服务的说明执行。有关从本地 Git 存储库进行发布的分步说明，请参见[从源代码管理发布到 Azure 网站][publishingwithgit]。

如果您计划使用 Visual Studio 创建和管理您的网站，则可以选择直接从 Visual Studio 发布。一个方法是单击**下载发布配置文件**选项。这允许你保存可导入到 Visual Studio 中以便进行 Web 发布的 publishsettings 文件。 

> [AZURE.NOTE]
> 请务必确保 <i>publishsettings</i> 文件安全并且处于源代码管理之外，因为它包含针对部署以及任何链接的数据库连接字符串的用户名和密码。

也可以将订阅信息直接导入到 Visual Studio 中。有关示例，请考虑 Visual Studio 中的本地 ASP.NET 项目。右键单击 Web 项目，然后选择**发布**。通过**发布 Web**对话框中的**导入**按钮，您可以导入包含您的 Azure 订阅设置的文件或者导入从网站仪表板下载的 publishsettings 文件。下面的屏幕快照显示这些选项。
...
尽管这些步骤提供了您部署 .COM 网站所需项目，但还应为管理后续内容发布周期创建计划。这些选项可以涵盖从开始执行自定义解决方案到不频繁更改的网站的定期重新部署再到功能完备的内容管理系统 (CMS) 的方方面面。如果您在创建新网站，则应注意，在库中有一些选项使用现有 CMS 框架，例如[Drupal][drupal]或[Umbraco][umbraco]。
...
在 Azure 管理门户中，在**缩放**选项卡上确认您在**共享**或标准模式下运行（**免费**网站不支持自定义域）。然后转到**配置**选项卡，单击**管理域**按钮。这使您能够将网站域自定义域名相关联。 

![GlobalWebWebMatrix][GlobalWebWebMatrix]

在列表中放置您的自定义域之前，必须首先转到您的 DNS 提供程序并且创建您的自定义域 (www.contoso.com) 的 CNAME 记录，该记录指向您的 Azure 网站的 URL (contoso.chinacloudsites.cn)。在此传播后，您可以在前面屏幕快照中显示的对话框中输入自定义域。显示指向该网站的 www.contoso.com 的 CNAME 记录  可确保您有权使用该网站的  网站。此时，您可以在对话框的底部创建具有 IP 地址的 A 记录。
...
Azure 网站自动提供与实际网站 URL 的安全连接。例如，如果您的网站是 http://contoso.chinacloudsites.cn， 则只需通过将"http"更改为"https"，例如**https**://contoso.chinacloudsites.cn，就可以通过 SSL 进行连接。

但是，如果您在使用自定义域名，则必须采取措施通过网站的 Azure 管理门户上载证书和启用 SSL。下面的步骤提供此过程的摘要，但您可以在主题[为 Azure 网站配置 SSL 证书][ssl]中找到详细说明。
...
一旦从证书颁发机构获取证书后，就不能简单地以相同方式将其上载到 Azure。你必须使用 openssl 命令生成 .pfx 文件。该 openssl 命令是 OpenSSL 项目的一部分。这些源在[OpenSSL 网站][openssl]上分发，但您通常也可以在 Internet 上找到该工具的预先编译的版本。在下面的示例中，使用证书 myserver.crt 和私钥文件 myserver.key 创建了一个 .pfx 文件。

	openssl pkcs12 -export -out myserver.pfx -inkey myserver.key -in myserver.crt

若要将该证书上载到 Azure，请首先转到**缩放**选项卡，确认您正在**标准**模式下运行。**免费**或**共享**模式不支持针对自定义域的 SSL。在**配置**选项卡上，选择**上载证书**按钮。
...
对于在**标准**模式下运行的网站，您还可以启用终结点监视和警报。在**配置**选项卡上，转到**监视**部分，并且配置终结点。此终结点从您指定的一个或多个位置运行，并且定期尝试访问您的网站。时间和错误信息都会收集。 

在**监视器**选项卡中，此终结点在出现时会显示响应时间。如果您选择终结点度量值，则可以通过单击**添加规则**图标，添加警报规则。
...
如果您发现网站要求缩放，则可以在**缩放**选项卡上手动执行缩放，或者通过自动缩放预览执行缩放。该"缩放"选项卡提供向上扩展（更大的专用计算机）或向外扩展（相同大小的附加共享实例或专用实例）的选择。但是，自动缩放预览仅支持向外扩展。有关此选项的更详细信息，请参见有关网站监视的更多信息，以及参见[数字市场营销活动][scenariodigitalmarketing]方案的"根据用户要求进行缩放"部分。另请参见[如何监视网站][howtomonitor]。
...
<tr>
   ...
   <td valign="top">- <a href="/zh-cn/documentation/articles/choose-web-site-cloud-service-vm/">网站、云服务和虚拟机：何时使用何种产品？</a></td>
</tr>
<tr>
...
<tr>
  ...
  [howtocreate 网站]:/zh-cn/documentation/articles/web-sites-dotnet-get-started
