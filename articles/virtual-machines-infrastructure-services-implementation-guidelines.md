<properties pageTitle="Azure 基础结构服务实现准则" description="了解用于在 Azure 基础结构服务中部署 IT 工作负荷的关键设计和实施准则。" documentationCenter="" services="virtual-machines" authors="JoeDavies-MSFT" manager="timlt" editor=""/>

<tags ms.service="virtual-machines" ms.date="05/29/2015" wacn.date=""/>

# Azure 基础结构服务实现准则
 
Azure 是一个实现开发/测试或概念证明配置的极好平台，因为它只需很少的投资即可测试实现你的解决方案的特定方法。但是，你必须能够将用于开发/测试或概念证明环境的简便做法与用于全功能生产就绪的 IT 工作负荷实现的更难且更详细的做法区分开来。

本指南将列出规划对 Azure 中的 IT 基础结构或工作负荷取得成功至关重要的许多方面。此外，它还通过提供创建所需资源的顺序来帮助在 Azure 平台上实现解决方案。虽然存在一定的灵活性，但 Microsoft 建议你按此顺序来进行规划和决策。
...
建立良好的命名约定并遵循特定的系统顺序在 Azure 中创建资源可极大地减轻管理负担，并增加任何实现项目的成功机会。

> [AZURE.NOTE]地缘组已弃用，本文未介绍其用法。有关详细信息，请参阅[关于区域虚拟网络和地缘组](https://msdn.microsoft.com/zh-CN/library/azure/jj156085.aspx)。

## 1.命名约定

良好的命名约定应在 Azure 中创建任何项目之前就准备好。命名约定可确保所有资源都具有可预测的名称，这可帮助减轻与管理对这些资源关联的管理负担。
...
创建特定资源时，Azure 将使用某些默认设置来简化对关联到这些资源的资源的管理。例如，在为新云服务创建第一个虚拟机时，Azure 管理门户将尝试将虚拟机的名称用于虚拟机的新云服务的名称。
...
例如，下面是托管计算引擎的云服务的两个可能名称：

- Svc-CalculationEngine（前缀）
- CalculationEngine-Svc（后缀）
...
Azure 组件、服务或产品 | Svc 用于云服务，VNet 用于虚拟网络 | 根据资源提供支持的产品。
角色 | sql、ora、sp、iis | 根据 VM 的角色。
实例 | 01、02、03 等。 | 适用于可能具有多个实例的资源。例如，云服务中经过负载平衡的 Web 服务器。
		
建立命名约定时，请确保这些命名约定明确说明要对每种类型的资源使用哪些词缀，以及在哪个位置使用（前缀还是后缀）。

### 日期

许多时候，从资源名称确定创建日期很重要。Microsoft 建议使用 YYYYMMDD 日期格式。此格式可确保不仅会记录完整的日期，而且名称只是日期不同的两个资源也会同时按字母和按时间顺序进行排序。
...
名称应尽可能具有描述性，以确保名称可以提供足够的信息来确定它所引用的资源。

### 计算机名称

当管理员创建虚拟机时，Microsoft Azure 将要求他们提供虚拟机名称。Microsoft Azure 将使用虚拟机名称作为 Azure 虚拟机资源名称。Azure 将使用同一名称作为虚拟机上安装的操作系统的计算机名称。但是，这些名称可能并非始终相同。

在基于已包含操作系统的 .VHD 文件创建虚拟机的情况下，Microsoft Azure 中的虚拟机名称可能不同于虚拟机的 OS 计算机名称。这种情况可能会增加虚拟机管理难度，我们不建议这样做。始终确保 Azure 虚拟机资源名称与分配给该虚拟机的操作系统的计算机名称相同。
...
存储帐户具有适用于其名称的特殊规则。你只能使用小写字母和数字，并且所分配的连接到服务（Blob、表或队列）的名称和默认域 (core.chinacloudapi.cn) 应呈现全局范围内有效的唯一 DNS 名称。例如，如果存储帐户名为 mystorageaccount，则以下相应的 URL 应为有效的唯一 DNS 名称：
...
Azure 构建基块是 Azure 通常为利用 PaaS 功能的应用程序提供的应用程序级别服务，虽然 IaaS 资源可以利用一些诸如 Azure SQL、流量管理器等工具。
...
Microsoft 通过可下载的文件为企业协议中的单个帐户或所有帐户提供详细的计费信息。你可以处理此文件，例如，使用 Excel。此过程将引入数据、对将多个级别的层次结构编码为单独列的资源进行分区，并使用数据透视表或 PowerPivot 提供动态报告功能。
...
存储是任何 Azure 解决方案必不可少的组成部分，因为它不仅提供应用程序级服务，而且还是支持虚拟机的基础结构的一部分。
 
Azure 提供两种类型的存储。标准存储授予你访问 Blob 存储、表存储、队列存储和文件存储的权限：高级存储专为高性能应用程序（例如 AlwaysOn 群集中的 SQL Server）设计，当前仅支持 Azure 虚拟机磁盘。

存储帐户将绑定到可伸缩性目标。要熟悉当前 Azure 存储空间限制，请参阅 [Microsoft Azure 订阅和服务限制、配额和约束](azure-subscription-service-limits#storage-limits)。另请参阅 [Azure 存储空间可伸缩性和性能目标](storage-scalability-targets)。

Azure 使用一个操作系统磁盘、一个临时磁盘和零个或更多可选数据磁盘创建虚拟机。操作系统磁盘和数据磁盘是 Azure blob，而临时磁盘则由计算机所在的节点的本地存储提供支持。这使得临时磁盘不适用于在系统回收过程中必须保留的数据，因为计算机可能会以无提示方式从一个节点迁移到另一个节点，从而丢失该磁盘中的任何数据。不要在临时驱动器上存储任何内容。

由于 blob 的最大大小为 1024 GB，并且必须包含 VHD 文件的元数据（页脚），因此操作系统磁盘和数据磁盘的最大大小为 1023 GB（1 GB 为 10243 个字节）。你可以通过在 Windows 中实现磁盘条带化来超越此限制。

### 条带化的磁盘
除了提供相关功能来创建大于 1023 GB 的磁盘外，在许多情况下，对数据磁盘使用条带化还可增强性能，因为允许多个 blob 支持单个卷的存储。这将并行执行所需的 I/O 以将数据写入单个磁盘和从单个磁盘读取数据。

Azure 将对可用的数据磁盘量和带宽加以限制，具体取决于虚拟机大小。有关详细信息，请参阅 [Azure 的虚拟机和云服务大小](https://msdn.microsoft.com/zh-CN/library/azure/dn197896.aspx)。
...
Microsoft 建议你从为每个存储帐户部署一个虚拟机开始。

### 存储布局设计

为了实现这些策略，以便为虚拟机的磁盘子系统实现良好性能，IT 工作负荷或基础结构通常会利用多个存储帐户。这些帐户将托管多个 VHD blob。在某些情况下，多个 blob 将关联到虚拟机中的单个卷。

这种情况可能会为管理任务添加复杂性。为存储设计有效的策略（包括基础磁盘和关联的 VHD blob 的相应命名）是关键。

### 实现准则会扼要重述存储

决策：

- 你需要进行磁盘条带化以创建大于 500 TB 的磁盘吗？
- 你需要进行磁盘条带化以获得工作负荷的最佳性能吗？
- 你需要使用哪一组存储帐户来托管你的 IT 工作负荷或基础结构？

任务：

- 使用命名约定创建存储帐户集。可以使用 Azure 预览门户、Azure 管理门户或 **New-AzureStorageAccount** PowerShell cmdlet。

## 4.云服务

云服务是 Azure 中的基本构建基块，同时用于 PaaS 和 IaaS 服务。对于 PaaS，云服务表示其实例可以相互通信的角色关联。云服务将关联到一个公共虚拟 IP (VIP) 地址和一个负载平衡器，后者将接受来自 Internet 的传入流量，并将该流量负载平衡到配置为接收该流量的角色。
...
Microsoft Azure 将公开云服务名称，因为这些名称会关联到域“chinacloudapp.cn”中 VIP。为了改善应用程序的用户体验，应根据需要配置虚名称来替换完全限定的云服务名称。这通常使用公共 DNS 中将资源的公共 DNS 名称（例如，www.contoso.com）映射到托管资源的云服务（例如，托管 www.contoso.com 的 Web 服务器的云服务）的 DNS 名称的 CNAME 记录完成。
...
- 使用命名约定创建云服务集。可以使用 Azure 管理门户或 **New-AzureService** PowerShell cmdlet。
...
- 使用命名约定创建虚拟网络。可以使用 Azure 预览门户或 Azure 管理门户。
...
在 Azure IaaS 中，角色的概念并不重要，因为每个 IaaS 虚拟机表示包含单个实例的角色。为了提示 Azure 不要同时关闭两个或更多关联的计算机（例如，为了对它们所在的节点进行 OS 更新），引入了可用性集的概念。可用性集告知 Azure 不要同时关闭同一可用性集中的所有计算机，以防止服务中断。可用性集的虚拟机成员具有 99.95% 的正常运行时间服务级别协议。
...
为了减少管理负担，Azure 管理门户将使用计算机名称作为关联的云服务的默认名称建议（如果客户在虚拟机创建向导中选择创建新的云服务）。
...
- 使用命名约定定义每个虚拟机名称。
- 使用 Azure 预览门户、Azure 管理门户、**New-AzureVM** PowerShell cmdlet、Azure CLI，或使用资源管理器模板创建虚拟机。
...
- Contoso 使用 [IT 工作负荷]-[位置]-[Azure 资源]作为前缀。在此示例中，"azfae"（Azure 财务分析引擎）为 IT 工作负荷的名称，"use"（中国东部）是位置，因为 Contoso 的大多数初始客户都位于美国东海岸。
- 存储帐户使用 contosoazfaeusesa[说明] 请注意 contoso 已添加到前缀以提供唯一性，并且存储帐户名称不支持使用连字符。
- 云服务使用 contoso-azfae-use-cs-[说明] 请注意 ccontoso 已添加到前缀以提供唯一性。
...
Contoso 已使用以下 PowerShell 命令创建这两个存储帐户：

	New-AzureStorageAccount -StorageAccountName "contosoazfaeusesawebapp" -Location "China East"
	New-AzureStorageAccount -StorageAccountName "contosoazfaeusesasqlclust" -Location "China East" -Type Premium_LRS
...
- **contoso-azfae-use-cs-frontend** 用于前端 Web 服务器
- **contoso-azfae-use-cs-backend** 用于后端应用程序服务器、SQL Server 群集服务器和域控制器

Contoso 使用以下 PowerShell 命令创建了云服务：

	New-AzureService -Service "contoso-azfae-use-cs-frontend" -Location "China East"
	New-AzureService -Service "contoso-azfae-use-cs-backend" -Location "China East"
...
- **azfae-use-vm-sql01** 用于 SQL Server 群集中的第一个 SQL Server
- **azfae-use-vm-sql02** 用于 SQL Server 群集中的第二个 SQL Server
...
这些 Azure PowerShell 命令使用这个以前创建的存储帐户、云服务和虚拟网络的配置创建虚拟机。
...
[Microsoft Azure 订阅和服务限制、配额和约束](azure-subscription-service-limits#storage-limits)

[Azure 的虚拟机和云服务大小](https://msdn.microsoft.com/zh-CN/library/azure/dn197896.aspx)

[Azure 存储空间可伸缩性和性能目标](storage-scalability-targets)

[云平台集成框架（Azure 体系结构模式）](azure-architectures-cpif-overview)
