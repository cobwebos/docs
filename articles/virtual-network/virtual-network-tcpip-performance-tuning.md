---
title: Azure VM 的 TCP/IP 性能调优 |微软文档
...
#### <a name="the-dont-fragment-bit-in-an-ip-packet"></a>IP 数据包中的"不碎片位"

"不碎片 （DF）"位是 IP 协议标头中的标志。 DF 位指示发送方与接收方之间的路径中的网络设备不得将数据包分段。 设置此位的原因有很多。 （有关一个示例，请参阅本文的"路径 MTU 发现"部分。当网络设备收到具有"不碎片"位集的数据包，并且该数据包超过设备的接口 MTU 时，标准行为是设备丢弃数据包。 设备会将一条 ICMP Fragmentation Needed（需要 ICMP 分段）消息发回给数据包的原始源。
...
片段通常被网络防火墙等安全设备丢弃，或者网络设备的接收缓冲区耗尽时丢弃。 当网络设备的接收缓冲区耗尽时，网络设备会尝试重组分段的数据包，但没有资源可用于存储和恢复数据包。
...
启用 LSO 后，Azure 客户在执行数据包捕获时可能会看到较大的帧大小。 这些大型帧大小可能导致一些客户认为碎片正在发生，或者大型 MTU 正在使用时不使用。 以太网适配器可以使用 LSO 将更大的最大片段大小 (MSS) 播发到 TCP/IP 堆栈，以创建更大的 TCP 数据包。 然后，整个未分段的帧将转发到以太网适配器，并在 VM 上执行的数据包捕获中显示。 但是，根据以太网适配器的 MTU，数据包将被以太网适配器分解成许多较小的帧。
...
PMTUD 过程的效率低下，会影响网络性能。 如果发送的数据包超过网络路径的 MTU，需要使用更低的 MSS 重新传输该数据包。 如果发送方未收到 ICMP Fragmentation Needed 消息，原因可能是路径中存在网络防火墙（通常称为“PMTUD 黑洞”），因此发送方不知道它需要降低 MSS 并持续重新传输数据包。** 这就是为什么我们不建议增加 Azure VM MTU 的原因。
...
|-|-|-|-|
|**Route**|**距离**|**单向时间**|**RTT**|
|纽约到旧金山|4，148 公里|21 毫秒|42 毫秒|
|纽约到伦敦|5，585 公里|28 毫秒|56 毫秒|
|纽约到悉尼|15，993 公里|80 毫秒|160 毫秒|
...
让我们以纽约到旧金山为例。 两者之间的直线距离为 4,148 公里。 将该值插入公式，会得到以下结果：
...
Windows 可为不同的连接类型设置不同的缩放因子。 （连接类别包括数据中心、互联网等。您可以使用`Get-NetTCPConnection`PowerShell 命令查看窗口缩放连接类型：
...
通过允许来宾 VM 绕过主机并直接使用主机的 SmartNIC 建立数据路径，加速网络提高了性能。 加速网络的部分优势如下：

- **更低的延迟/每秒更高的数据包 （pps）：** 从数据路径中删除虚拟交换机消除了数据包在主机中用于策略处理所花费的时间，并增加了可在 VM 中处理的数据包数。

- **减少抖动**：虚拟交换机处理取决于需要应用的策略量和执行处理的 CPU 的工作负载。 将策略实施卸载到硬件消除了这种可变性，因为可以将数据包直接传送到 VM，消除了主机与 VM 之间的通信，以及所有的软件中断和上下文切换。

- **CPU 利用率降低**：绕过主机中的虚拟交换机会导致处理网络流量的 CPU 利用率降低。
...
在 VM 上启用加速网络后，若要获得最佳性能，需要启用 RSS。 RSS 还可以为不使用加速网络的 VM 提供优势。 有关如何确定 RSS 是否已启用及其启用方法的概述，请参阅[优化 Azure 虚拟机的网络吞吐量](https://aka.ms/FastVM)。
...
套接字处于 TIME_WAIT 状态的时间长短是可配置的。 该值的范围为 30 秒到 240 秒。 套接字是有限的资源，在任意给定时间可使用的套接字数目是可配置的。 （可用套接字的数量通常约为 30，000 个。如果使用可用的套接字，或者客户端和服务器TIME_WAIT设置不匹配，并且 VM 尝试在TIME_WAIT状态下重用套接字，则新连接将失败，因为 TCP SYN 数据包被静默丢弃。
...
传入流量不直接计算，或者说不直接受到限制。 但是，还有其他因素，如 CPU 和存储限制，可能会影响虚拟机处理传入数据的能力。

加速网络旨在改进网络性能（包括延迟、吞吐量和 CPU 使用率）。 加速网络可以提高虚拟机的吞吐量，但它只能达到虚拟机分配的带宽。
...
- **加速网络**：虽然此功能有助于达到已发布的限制，但它不会改变限制。
...
- **延迟**：两个目的地之间的往返时间可能受中间网络问题、不采用"最短"距离路径的流量以及次优对等路径的影响。

- **数据包丢失**：数据包丢失可能由网络拥塞、物理路径问题和网络设备性能不佳引起。

- **MTU 大小/碎片**：路径上的碎片可能导致数据到达或数据包到达顺序过大的延迟，从而影响数据包的传递。
