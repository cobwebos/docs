...
> [AZURE.IMPORTANT]建议你尽可能地使用网络安全组 (NSG) 而不是 ACL。若要了解有关 NSG 的详细信息，请参阅[什么是网络安全组](virtual-network-nsg)。

可以使用 PowerShell 或在管理门户中配置 ACL。若要使用 PowerShell 配置网络 ACL，请参阅[使用 PowerShell 管理终结点的访问控制列表 (ACL)](https://msdn.microsoft.com/zh-cn/library/azure/dn376543.aspx)。若要使用管理门户配置网络 ACL，请参阅[如何设置虚拟机的终结点](virtual-machines-set-up-endpoints)。
...
[如何使用 PowerShell 管理终结点的访问控制列表 (ACL)](virtual-networks-acl-powershell)
