...
在 2017 年 11 月 15 日之后创建的即用即付或 Microsoft 合作伙伴网络订阅中存在一个技术限制，它会阻止这些订阅中的 VM 直接发送电子邮件。 如需将电子邮件从 Azure VM 直接发送到外部电子邮件提供商（不使用经身份验证的 SMTP 中继），可以请求去除该限制。 Microsoft 将会根据自行的判断审批请求，并且只会在执行额外的防欺诈检查后才授予权限。 若要发出请求，请使用以下问题类型打开支持案例：**技术** > **虚拟网络** > **连接** > **无法发送电子邮件（SMTP/端口25）**。 请确保添加详细信息，解释为什么要直接向电子邮件提供商发送邮件而不是使用经身份验证的中继。
