...
对于在2017年11月15日之后创建的即用即付或 Microsoft 合作伙伴网络订阅，将会阻止从这些订阅中的 Vm 直接发送的电子邮件。 如需将电子邮件从 Azure VM 直接发送到外部电子邮件提供商（不使用经身份验证的 SMTP 中继），可以请求去除该限制。 请求将由 Microsoft 自行自行审核和批准，并且仅在进行其他反欺诈检查后才被授予。 若要发出请求，请使用以下问题类型打开支持案例：**技术** > **虚拟网络** > **连接** > **无法发送电子邮件（SMTP/端口25）**。 请确保添加详细信息，解释为什么要直接向电子邮件提供商发送邮件而不是使用经身份验证的中继。

在从 Azure 门户中免除了即用即付或 Microsoft 合作伙伴网络订阅并且 Vm 已从 "启动" &，将免除该订阅中的所有 Vm。 豁免仅适用于请求的订阅，并且仅适用于直接路由到 Internet 的虚拟机流量。 不支持通过 Azure PaaS 服务（如 [Azure 防火墙](https://azure.microsoft.com/services/azure-firewall/)）路由端口 25 流量。
...
如果在2017年11月15日之后创建了 MSDN、Azure Pass、Azure 开放许可、教育、BizSpark、Azure 主办方、Azure 学生版、免费试用版或任何 Visual Studio 订阅，你将有技术限制，这些技术限制会阻止从这些订阅中的 Vm 直接发送到电子邮件提供商的电子邮件。 此限制是为了防止滥用邮件。 不接受去除此限制的请求。
...
如果仍需要帮助，请[联系支持](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade)人员以快速解决你的问题，方法是使用以下问题类型：**订阅管理**问题类型：**允许端口25电子邮件流的请求**。
