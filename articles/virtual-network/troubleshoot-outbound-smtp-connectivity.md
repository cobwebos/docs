...
在 2017 年 11 月 15 日之后创建的即用即付或 Microsoft 合作伙伴网络订阅中存在一个技术限制，它会阻止这些订阅中的 VM 直接发送电子邮件。 如需将电子邮件从 Azure VM 直接发送到外部电子邮件提供商（不使用经身份验证的 SMTP 中继），可以请求去除该限制。 Microsoft 将会根据自行的判断审批请求，并且只会在执行额外的防欺诈检查后才授予权限。 用于发出请求，请使用下面的问题类型打开支持案例：**订阅管理**问题类型：**若要启用端口 25 电子邮件流的请求**。 请确保添加详细信息，解释为什么要直接向电子邮件提供商发送邮件而不是使用经身份验证的中继。

即用即付或 Microsoft 合作伙伴网络订阅得到豁免，并且 Vm 了已停止和已启动从 Azure 门户后，该订阅中的所有 Vm 会被都免除，今后。 例外特性仅适用于请求订阅。
...
如果您使用这些订阅类型，建议您在本文前面所述使用 SMTP 中继服务，或更改你的订阅类型。

## <a name="cloud-service-provider-csp"></a>云服务提供商 (CSP)

如果您使用的 Azure 资源通过 CSP，可以请求如果无法使用安全的 SMTP 中继与 Microsoft 代表你创建的解除锁定免除请求的 CSP。

## <a name="need-help-contact-support"></a>需要帮助？ 联系支持人员

如果仍需帮助，请[请联系支持人员](https://portal.azure.com/?#blade/Microsoft_Azure_Support/HelpAndSupportBlade)以通过使用下面的问题类型快速解决问题：**订阅管理**问题类型：**若要启用端口 25 电子邮件流的请求**。
