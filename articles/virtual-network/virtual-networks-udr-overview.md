...
|默认 |活动 |192.168.0.0/16      |无                      |                   |
 ...
 Subnet2 的路由表包含所有 Azure 创建的默认路由，以及可选的 VNet 对等互连和虚拟网关可选路由。 向虚拟网络添加网关和对等互连时，Azure 向虚拟网络中的所有子网添加了可选路由。 Azure 中删除从 10.0.0.0/8、 192.168.0.0/16 和 100.64.0.0/10 地址前缀的路由*Subnet1*路由表添加到用户定义路由 0.0.0.0/0 地址前缀为时*Subnet1*.  
