...
>[AZURE.NOTE] 在 Azure 中有两种部署模式：经典（也称为服务管理）和 Azure 资源管理器 (ARM)。不能将经典 VNet 添加到地缘组，或创建为区域 VNet。如果你在地缘组中有一个 VNet，建议你[将它迁移到区域 VNet](virtual-networks-migrate-to-regional-vnet.md)。
...
- **连接**。VNet 通过使用站点到站点 VPN 连接或 ExpressRoute 连接可以彼此连接，甚至可以连接到本地数据中心。若要了解有关 VPN 网关的详细信息，请访问[关于 VPN 网关](../vpn-gateway/vpn-gateway-about-vpngateways.md)。若要了解有关 ExpressRoute 的详细信息，请访问 [ExpressRoute 技术概述](../expressroute/expressroute-introduction.md)。
...
## 限制
一个订阅中允许的虚拟网络数有限制，请参阅 [Azure 网络限制](../azure-subscription-service-limits.md#networking-limits)，了解详细信息。

## 定价
在 Azure 中使用虚拟网络不需支付额外的费用。在 Vnet 中启动的计算实例将按标准费率计费，如 [Azure VM 定价](https://azure.microsoft.com/pricing/details/virtual-machines/)中所述。在 VNet 中使用的 [VPN 网关](https://azure.microsoft.com/pricing/details/vpn-gateway/)和[公共 IP 地址](https://azure.microsoft.com/pricing/details/ip-addresses/)也将按标准费率计费。
