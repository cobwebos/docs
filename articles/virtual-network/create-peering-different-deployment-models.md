...
5. 输入 `login-azurermaccount` 命令，登录 Azure，创建虚拟网络 (Resource Manager)。 用于登录的帐户必须拥有创建虚拟网络对等互连的必要权限。 有关权限列表，请参阅[虚拟网络对等互连权限](virtual-network-manage-peering.md#requirements-and-constraints)。
