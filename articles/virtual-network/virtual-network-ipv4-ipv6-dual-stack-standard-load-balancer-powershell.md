...
description: 本文介绍如何使用 Azure Powershell 在 Azure 虚拟网络中部署具有标准负载均衡器的 IPv6 双重堆栈应用程序。
...
ms.locfileid: "72174462"
---
# <a name="deploy-an-ipv6-dual-stack-application-in-azure---powershell-preview"></a>在 Azure 中部署 IPv6 双重堆栈应用程序-PowerShell （预览版）

本文介绍如何使用 Azure 中的标准负载均衡器部署双堆栈（IPv4 + IPv6）应用程序，该应用程序包括双重堆栈虚拟网络和子网、具有双重（IPv4 + IPv6）前端配置的标准负载均衡器、包含 Nic 的虚拟机双 IP 配置、网络安全组和公共 Ip。
...
使用[AzLoadBalancer](/powershell/module/az.network/new-azloadbalancer)创建标准负载均衡器。 下面的示例使用在前面的步骤中创建的 IPv4 和 IPv6 前端 IP 配置、后端池和负载均衡规则，创建名为*myLoadBalancer*的公共标准负载均衡器：
...
使用 [New-AzVirtualNetwork](/powershell/module/az.network/new-azvirtualnetwork) 创建虚拟网络。 以下示例使用*mySubnet*创建名为*dsVnet*的虚拟网络：
...
获取资源组中的所有网络接口对象，以使用 `get-AzNetworkInterface` 汇总此部署中使用的 IP。 此外，还可获取负载均衡器的 IPv4 和 IPv6 终结点的前端地址，`get-AzpublicIpAddress`。
