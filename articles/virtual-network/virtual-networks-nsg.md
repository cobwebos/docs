...
* **VIRTUAL_NETWORK**（如果使用的是 Azure Resource Manager，则为 **VirtualNetwork**）此默认标记表示你的所有网络地址空间。 它包括虚拟网络地址空间（Azure 中定义的 CIDR 范围）以及所有连接的本地地址空间和连接的 Azure VNet（本地网络）。
* **AZURE_LOADBALANCER**（如果使用的是 Azure Resource Manager，则为 **AzureLoadBalancer**）此默认标记表示 Azure 的基础结构负载均衡器。 这将转换到 Azure 数据中心 IP，从中进行 Azure 的运行状况探测。
