...
>[AZURE.NOTE] 不支持将基于终结点的 ACL 和网络安全组置于相同 VM 实例上。如果你想要使用 NSG，但已有了终结点 ACL，则请先删除该终结点 ACL。有关如何执行此操作的信息，请参阅[使用 PowerShell 管理终结点的访问控制列表 (ACL)](virtual-networks-acl-powershell.md)。
...
|**Source port range**|要与规则匹配的源端口范围|单个端口号（从 1 到 65535）、端口范围（例如 1-65635）或 *（针对所有端口）|源端口可以是暂时的。除非客户端程序使用特定端口，在大多数情况下请使用"*"。<br/>尽可能尝试使用端口范围以避免需要多个规则<br/>多个端口或端口范围不能使用逗号进行分组
...
|Azure 门户|![是](./media/virtual-network-nsg-overview/green.png)|[![是][green]](virtual-networks-create-nsg-arm-pportal.md)|
...
|ARM 模板|![否](./media/virtual-network-nsg-overview/red.png)|[![是][green]](virtual-networks-create-nsg-arm-template.md)|

|**键**|支持 ![是](./media/virtual-network-nsg-overview/green.png)。|不支持 ![否](./media/virtual-network-nsg-overview/red.png)。|
|---|---|---|
...
>[AZURE.IMPORTANT] 在设计解决方案之前，请确保查看所有[与 Azure 中的网络服务相关的限制](../azure-subscription-service-limits.md#networking-limits)。可以通过开具支持票证增加某些限制。
