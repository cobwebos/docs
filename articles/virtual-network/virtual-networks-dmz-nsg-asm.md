---
title: "外围网络示例 – 使用 NSG 构建简单的外围网络 | Microsoft 文档"
...
---
# <a name="example-1-build-a-simple-dmz-with-nsgs"></a>示例 1 – 使用 NSG 构建简单的外围网络
[返回安全边界最佳实践页面][主页]
...
下面更详细地讨论了每个规则（**注意**：以下列表中以货币符号开头的任何项（例如 $NSGName）均为本文“参考”部分的脚本中的用户定义变量）：
...
2. 本示例中的第一个规则允许所有内部网络之间的 DNS 流量发往后端子网上的 DNS 服务器。 该规则有一些重要参数：
   ...
   * “Action”表示是要阻止还是允许受此规则影响的流量。
         ...
         Get-AzureNetworkSecurityGroup -Name $NSGName | `
             ...
             -Protocol *
3. 此规则允许 RDP 流量从 Internet 发往 VNET 中任一子网上任何服务器的 RDP 端口。 此规则使用两种特殊地址前缀：“VIRTUAL_NETWORK”和“INTERNET”。 这样就可以轻松处理较大类别的地址前缀。
...
#### <a name="allowed-web-to-web-server"></a>（*允许*）从 Web 访问 Web 服务器
...
#### <a name="denied-web-dns-lookup-on-dns-server"></a>（*拒绝*）在 DNS 服务器上执行 Web DNS 查找
...
#### <a name="denied-web-to-sql-access-through-firewall"></a>（*拒绝*）从 Web 通过防火墙访问 SQL
...
如有需要，请修改用户定义的变量。 运行脚本，然后根据上面“示例 1”部分中所述的防火墙规则设置说明操作。
