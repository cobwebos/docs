...
- 如果使用 Azure 命令行接口 (CLI) 命令来完成本文中的任务，请运行 [Azure Cloud Shell](https://shell.azure.com/bash) 中的命令，或从计算机运行 CLI。 本教程需要 Azure CLI 2.0.31 或更高版本。 运行 `az --version` 查找已安装的版本。 如果需要安装或升级，请参阅[安装 Azure CLI](/cli/azure/install-azure-cli)。 如果在本地运行 Azure CLI，则还需运行 `az login` 以创建与 Azure 的连接。
...
可将所需的任意数量的[专用](#private)和[公共](#public) [IPv4](#ipv4) 地址添加到网络接口，只要不超过 [Azure 限制](../azure-resource-manager/management/azure-subscription-service-limits.md?toc=%2fazure%2fvirtual-network%2ftoc.json#azure-resource-manager-virtual-networking-limits)一文中列出的限制即可。 您可以将专用 IPv6 地址添加到现有网络接口的一个[辅助 IP 配置](#secondary)（只要没有现有的辅助 IP 配置）。 每个网络接口最多可能具有一个 IPv6 专用地址。 您可以选择将公共 IPv6 地址添加到 IPv6 网络接口配置中。 有关使用 IPv6 地址的详细信息，请参阅 [IPv6](#ipv6)。
...
- 还可能具有分配给它的[公共](#public) IPv4 地址。 您不能将公共 IPv6 地址分配给主 （IPv4） IP 配置。 
...
专用[IPv4](#ipv4)或 IPv6 地址使虚拟机能够与虚拟网络或其他连接网络中的其他资源进行通信。 
...
您可以（可选）将公共或私有静态 IPv4 或 IPv6 地址分配给 IP 配置。 若要详细了解 Azure 如何分配静态公共 IPv4 地址，请参阅[公共 IP 地址](virtual-network-public-ip-address.md)。
...
可以为网络接口的辅助 IP 配置分配零个或一个专用 [IPv6](#ipv6) 地址。 网络接口不能具有任何现有的辅助 IP 配置。 每个网络接口最多可能具有一个 IPv6 专用地址。 您可以选择将公共 IPv6 地址添加到 IPv6 网络接口配置中。
