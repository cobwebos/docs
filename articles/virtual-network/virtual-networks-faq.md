...
受支持的最小 IPv4 子网为 /29，最大为 /8（使用 CIDR 子网定义）。  IPv6 子网的大小必须完全为 /64。  
...
是的，VNet 可以是仅 IPv4 或双堆栈 （IPv4+IPv6）。  有关详细信息，请参阅[Azure 虚拟网络的 IPv6 概述](./ipv6-overview.md)。
...
通过某些 Azure PaaS 服务（如 Azure 存储和 Azure SQL 数据库）部署的资源可以通过使用虚拟网络服务终结点或 Azure 专用链路限制对 VNet 的网络访问。 有关详细信息，请参阅[虚拟网络服务终结点概述](virtual-network-service-endpoints-overview.md)[、Azure 专用链接概述](../private-link/private-link-overview.md)
...
如果通过全局 VNet 对等互连对互连两个不同区域中的两个虚拟网络，则无法通过负载均衡器的前端 IP 连接到基本负载均衡器后面的资源。 此限制对于标准负载均衡器不存在。
以下资源可以使用基本负载均衡器，这意味着您无法通过负载均衡器的前端 IP 通过全局 VNet 对等互连来达到它们。 但是，如果允许，您可以使用全局 VNet 对等互连直接通过其专用 VNet IP 访问资源。 
- 位于基本负载均衡器后的 VM
- 采用基本负载均衡器的虚拟机规模集 
- Redis 缓存 
- 应用程序网关 (v1) SKU
...
如果对等互连连接处于 *"已启动"* 状态，这意味着您只创建了一个链接。 必须创建双向链接才能成功建立连接。 例如，若要从 VNet A 对等互连到 VNet B，必须创建从 VNetA 到 VNetB 以及从 VNetB 到 VNetA 的链接。 创建两个链接将更改状态到 *"已连接*"。
...
如果您的 VNet 对等互连连接处于*断开连接*状态，则表示创建的链接之一已被删除。 若要重新建立对等互连连接，需要删除该链路并重新创建它。
...
### <a name="why-is-my-peering-connection-in-a-disconnected-state"></a>为什么我的对等互连处于*断开连接*状态？
...
虚拟网络 TAP 预览在所有 Azure 区域都可用。 受监视的网络接口、虚拟网络 TAP 资源和收集器或分析解决方案必须部署在同一区域中。
...
受监视的网络接口仅能拥有一个 TAP 配置。 请咨询各个[合作伙伴解决方案](virtual-network-tap-overview.md#virtual-network-tap-partner-solutions)，了解将 TAP 流量的多个副本流式传输到您选择的分析工具的功能。
...
某些服务（如 SQL 和 CosmosDB）允许通过**忽略缺失VNet服务终结点**标志对上述序列进行异常。 将标志设置为**True**后，可以在 Azure 服务端设置 VNet ACL，在网络端设置服务终结点之前。 Azure 服务提供此标志以帮助客户在 Azure 服务上配置特定的 IP 防火墙，由于源 IP 从公共 IPv4 地址更改为专用地址，因此打开网络端的服务终结点会导致连接性下降。 在网络端设置服务终结点之前，在 Azure 服务端设置 VNet ACL 可帮助避免连接性下降。
...
### <a name="can-an-on-premises-devices-ip-address-that-is-connected-through-azure-virtual-network-gateway-vpn-or-expressroute-gateway-access-azure-paas-service-over-vnet-service-endpoints"></a>通过 Azure 虚拟网络网关 （VPN） 或 ExpressRoute 网关连接的本地设备的 IP 地址能否通过 VNet 服务终结点访问 Azure PaaS 服务？
...
### <a name="does-azure-active-directory-azure-ad-support-vnet-service-endpoints"></a>Azure 活动目录 （Azure AD） 是否支持 VNet 服务终结点？

Azure 活动目录 （Azure AD） 不支持本机服务终结点。 支持 VNet 服务终结点的 Azure 服务的完整列表[可在此处](https://docs.microsoft.com/azure/virtual-network/virtual-network-service-endpoints-overview)看到。 请注意，支持服务终结点的服务下列出的"Microsoft.AzureActiveDirectory"标记用于支持 ADLS 第 1 代的服务终结点。 对于 ADLS 第 1 代，Azure 数据湖存储第 1 代的虚拟网络集成利用虚拟网络和 Azure 活动目录 （Azure AD） 之间的虚拟网络服务终结点安全性在访问令牌中生成其他安全声明。 然后，系统会使用这些声明对 Data Lake Storage Gen1 帐户进行虚拟网络身份验证，然后允许访问。 了解有关[Azure 数据湖存储第 1 代 VNet 集成的更多](../data-lake-store/data-lake-store-network-security.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
