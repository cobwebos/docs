...
是的。 Azure 在每个子网中保留 5 个 IP 地址。 它们分别为 x. x. x. x. x. x. x. x. x. x. x. x. x. x. x. x。 在 Azure 服务的每个子网中保留了 x. x. x. x. x. x. x. x. x. x. x. x。   
- x.x.x.x：网络地址
...
是的。 可以随时更改 VNet 的 DNS 服务器列表。 如果更改 DNS 服务器列表，则需要对 VNet 中所有受影响的 Vm 执行 DHCP 租约续订，以使新的 DNS 设置生效。 对于运行 Windows OS 的 Vm，你可以通过在 VM 上直接键入 `ipconfig /renew` 来执行此操作。 对于其他 OS 类型，请参阅针对特定 OS 类型的 DHCP 租约续订文档。 
...
是的。 可以使用 ASE （应用服务环境）在 VNet 中部署 Web 应用，使用 VNet 集成将应用的后端连接到 Vnet，并将入站流量锁定到使用服务终结点的应用。 有关详细信息，请参阅以下文章：
...
是的。 全球 VNet 对等互连可以将不同区域中的 VNet 对等互连。 全局 VNet 对等互连在所有 Azure 公共区域、中国云区域和政府云区域中均可用。 不能将 Azure 公共区域的全局对等互连到国家/地区云区域。
