---
title: "创建、更改或删除 Azure 虚拟网络子网 | Microsoft Docs"
...
如果不熟悉虚拟网络，建议在创建、更改或删除子网之前阅读 [Azure 虚拟网络概述](virtual-networks-overview.md)和[创建、更改或删除虚拟网络](virtual-network-manage-network.md)。 所有可连接到虚拟网络的 Azure 资源都已连接到虚拟网络中的子网。 通常情况下，在一个虚拟网络中创建多个子网以：
- 筛选子网之间的流量。 可将网络安全组应用于子网，筛选连接到虚拟网络的所有资源（如虚拟机）的入站和出站网络流量。 若要了解有关如何创建网络安全组的详细信息，请参阅[创建网络安全组](virtual-networks-create-nsg-arm-pportal.md)。
...
- 如果打算使用 Azure CLI 命令来完成本文中所述的任务，首先必须[安装和配置 Azure CLI](/cli/azure/install-azure-cli?toc=%2fazure%2fvirtual-network%2ftoc.json)。 确保安装最新版本的 Azure CLI。 若要获取 Azure CLI 命令的相关帮助，请输入 `az <command> --help`。

## <a name="create-subnet"></a>创建子网

若要创建子网，请执行以下操作：

1. 使用已分配订阅的“网络参与者”角色权限（最低权限）的帐户登录到[门户](https://portal.azure.com)。 若要了解有关如何将角色和权限分配给帐户的相关信息，请参阅[用于 Azure 基于角色的访问控制的内置角色](../active-directory/role-based-access-built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor)。
...
6. 在“添加子网”边栏选项卡中，输入以下参数的值：
    ...
    - 网络安全组：可选择将现有网络安全组关联到子网，控制子网的入站和出站网络流量筛选。 网络安全组必须存在于与虚拟网络相同的订阅和位置。 必须使用 Resource Manager 部署模型创建。 若要了解有关如何创建网络安全组的详细信息，请参阅[网络安全组](virtual-networks-create-nsg-arm-pportal.md)。
    - 路由表：可选择将现有的路由表关联到子网，控制目标为其他网络的网络流量路由。 路由表必须存在于与虚拟网络相同的订阅和位置。 必须使用 Resource Manager 部署模型创建。 若要了解有关如何创建路由表的详细信息，请参阅[用户定义的路由](virtual-network-create-udr-arm-ps.md)。
...
|PowerShell|[](/powershell/module/azurerm.network/new-azurermvirtualnetworksubnetconfig?view=azurermps-3.8.0?toc=%2fazure%2fvirtual-network%2ftoc.json)New-AzureRmVirtualNetworkSubnetConfig、[](/powershell/module/azurerm.network/add-azurermvirtualnetworksubnetconfig?view=azurermps-3.8.0?toc=%2fazure%2fvirtual-network%2ftoc.json)Add-AzureRmVirtualNetworkSubnetConfig|

## <a name="change-subnet"></a>更改子网设置

通过管理连接到子网的资源可以更改网络安全组、路由表和对子网的用户访问权限。 若要了解有关这些设置的详细信息，请参阅[创建子网](#create-subnet)中的步骤 6。 若要更改子网的地址空间，必须首先删除任何连接到该子网的资源。 删除资源所采取的步骤因资源而异。 若要了解有关如何删除已连接到子网的资源的相关信息，请阅读针对要删除的每种资源类型的相关文档。 若要更改子网的地址范围，请执行以下操作：
...
|PowerShell|[](/powershell/module/azurerm.network/set-azurermvirtualnetworksubnetconfig?view=azurermps-3.8.0?toc=%2fazure%2fvirtual-network%2ftoc.json)Set-AzureRmVirtualNetworkSubnetConfig|


## <a name="delete-subnet"></a>删除子网

只有在没有资源连接到子网的情况下，才能删除该子网。 如果有资源连接到子网，则必须先删除连接到子网的资源，然后才能删除该子网。 删除资源所采取的步骤因资源而异。 若要了解有关如何删除已连接到子网的资源的相关信息，请阅读针对要删除的每种资源类型的相关文档。 若要删除子网，请执行以下操作：
...
若要创建 VM 并将其连接到子网，请参阅[创建虚拟网络并连接 VM](virtual-network-get-started-vnet-subnet.md#a-namecreate-vmsacreate-virtual-machines)。
