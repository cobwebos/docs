---
title: 将 Azure VM （经典）或云服务角色实例移动到不同的子网
...
为何要将 VM 移到另一个子网？ 当旧子网太小并且由于该子网中存在正在运行的 VM 而无法扩展时，子网迁移很有用。 在这种情况下，可以创建一个更大的新子网，将 VM 迁移到新子网，并在迁移完成后可删除空的旧子网。
...
如果为 VM 指定了静态内部专用 IP，则必须清除该设置，才能将 VM 移到新子网。 在这种情况下，请使用以下代码：
