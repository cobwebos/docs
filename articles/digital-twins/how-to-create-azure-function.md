...
3. 向函数添加身份验证代码（以便能够访问 Azure 数字孪生）
4. 将函数应用发布到 Azure
...
Function app 使用[适用于 .net 的 Azure IoT 数字克隆客户端库（c #）](https://github.com/Azure/azure-sdk-for-net/tree/master/sdk/digitaltwins/Azure.DigitalTwins.Core)与 Azure 数字孪生交互。 若要使用 SDK，需要在项目中包含以下包：
...
若要将函数应用发布到 Azure，请在解决方案资源管理器中右键选择函数项目（而非解决方案），然后选择 "*发布" （）*。
...
* 附加到 Azure 数字孪生的事件网格终结点，用于处理来自 Azure 数字孪生本身的消息（例如，属性更改消息、中的[数字孪生](concepts-twins-graph.md)生成的遥测消息或生命周期消息）
