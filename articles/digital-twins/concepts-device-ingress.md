---
title: 设备连接和遥测入口 - Azure 数字孪生 |微软文档
description: 了解如何在 Azure 数字孪生中连接、上载和发送来自 IoT 设备的遥测数据。
ms.author: alinast
...
有关如何将设备带上板载的分步指南，请阅读[部署和配置数字孪生的教程](tutorial-facilities-setup.md)。 步骤概览：
...
- 创建匹配程序，根据条件筛选遥测消息。
- 创建[用户定义的函数](concepts-user-defined-functions.md)，并将其分配给图形中用于自定义处理遥测消息的空间。
...
| **创建时间Utc** | `string` | 否 | [ISO 8601](https://www.iso.org/iso-8601-date-and-time-format.html) 格式的日期字符串，用于标识有效负载的采样时间。 例如，`2018-09-20T07:35:00.8587882-07:00` 。 |
| **相关性 Id** | `string` | 否 | 用于跟踪整个系统中的事件的 UUID。 例如，`cec16751-ab27-405d-8fe6-c68e1412ce1f` 。
...
- 要了解 Azure 数字孪生数据处理和用户定义的函数功能，请阅读[Azure 数字孪生数据处理和用户定义的函数](concepts-user-defined-functions.md)。
