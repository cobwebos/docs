...
输出窗口会显示用户定义的函数如何运行，以及如何截获来自设备模拟的事件。 
   ...
   ![UDF 的输出](./media/tutorial-facilities-udf/udf-running.png)

如果满足受监视的条件，用户定义的函数会将空间的值设置为我们[此前](#udf)看到的的相关消息。 该消息由 `GetAvailableAndFreshSpaces` 函数输出到控制台。
