...
解决方案开发人员可以通过 Postman 客户端指定 HTTP 请求（*POST*、 *GET*、 *UPDATE*、 *PATCH*和*DELETE*）的类型、要调用的 API 终结点，以及使用 TLS。 Postman 还支持添加 HTTP 请求标头、参数、表单数据和正文。
...
1. 按照[快速入门](quickstart-view-occupancy-dotnet.md#set-permissions-for-your-app)中的步骤创建和配置 Azure Active Directory 应用程序。 或者，你可以重复使用现有的应用注册。
    ...
    [![配置新的 Postman 重定向 URI](media/how-to-configure-postman/authentication-redirect-uri.png)](media/how-to-configure-postman/authentication-redirect-uri.png#lightbox)

1. 现在，将**重定向 URI**添加`https://www.getpostman.com/oauth2/callback`到。
...
1. 在 "**标头**" 选项卡下，添加具有值`multipart/mixed`的 HTTP 请求标头密钥**内容类型**。
...
1. 在 "**正文**" 选项卡`form-data`下，选择。 
1. 通过分配**键**名称来添加每个文件， `File`并选择。
