...
ms.locfileid: "85392193"
---
# <a name="dtdl-client-side-parser-library"></a>DTDL 客户端分析器库

使用基于 JSON 的数字孪生定义语言（DTDL）定义 Azure 数字孪生中的[模型](concepts-models.md)。 对于分析模型非常有用的情况，在 NuGet.org 上以客户端库的形式提供 DTDL 分析库： [DigitalTwins](https://nuget.org/packages/Microsoft.Azure.DigitalTwins.Parser/)。

此库提供对 DTDL 定义的模型访问，本质上充当 DTDL 的 c # 反射的等效项。 此库可独立于任何[Azure 数字孪生 SDK](how-to-use-apis-sdks.md)使用，尤其适用于在可视化或文本编辑器中进行 DTDL 验证。 在尝试将模型定义文件上传到服务之前，此方法有助于确保模型定义文件有效。
...
有可用的示例代码可以验证模型文档，以确保 DTDL 有效。 它在 DTDL 分析器库的基础上构建，并与语言无关。 在此处查找： [DTDL 验证器示例](https://docs.microsoft.com/samples/azure-samples/dtdl-validator/dtdl-validator)。

验证程序示例可用作命令行实用工具，用于验证 DTDL 文件的目录树。 它还提供交互模式。 源代码显示了如何使用分析器库的示例。
...
## <a name="use-the-parser-library-in-code"></a>在代码中使用分析器库

您还可以直接使用分析器库，以便在自己的应用程序中验证模型或生成动态模型驱动的 UI、仪表板和报表。
