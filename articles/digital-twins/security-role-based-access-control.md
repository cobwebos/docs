...
使用 Azure 数字孪生可以精确地控制空间图中的特定数据、资源和操作。 它通过将角色和权限管理称为[基于角色的访问控制](https://docs.microsoft.com/azure/role-based-access-control/)（RBAC）来实现此目的。 RBAC 包括“角色”和“角色分配”。____ 角色标识权限的级别。 角色分配将角色与用户或设备相关联。
