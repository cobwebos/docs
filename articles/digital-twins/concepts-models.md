...
有一个示例可用于验证模型文档，以确保 DTDL 是有效的。 它在 DTDL 分析器库的基础上构建，并与语言无关。 在此处查找： [DTDL 验证器示例](https://docs.microsoft.com/samples/azure-samples/dtdl-validator/dtdl-validator)。

或者，有关分析器库的详细信息（包括如何直接使用它的示例），请参阅[操作方法：分析和验证模型](how-to-use-parser.md)。
