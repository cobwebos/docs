...
ms.locfileid: "50959471"
---
# <a name="security-best-practices"></a>安全最佳实践

Azure 数字孪生安全性支持对 IoT 图中特定资源和操作的精确访问。 它通过精细的角色和权限管理（称为基于角色的访问控制）实现这一点。

Azure 数字孪生还使用 Azure IoT 上提供的其他安全功能，这包括 Azure Active Directory (Azure AD)。 为此，配置 Azure 数字孪生应用涉及使用当前建议的许多相同 [Azure IoT 安全实践](https://docs.microsoft.com/azure/iot-fundamentals/iot-security-best-practices?context=azure/iot-hub/)。
...
> * 加密已保存、已存储或持久性数据。
> * 要求定期更改或刷新密码或密钥。
> * 仔细按角色限制访问权限和权限。 请参阅“基于角色的访问控制最佳做法”部分。
> * 使用功能强大的加密。 要求使用长密码，并使用安全的协议以及双重身份验证。

监视 IoT 资源来查看处于常见操作范围之外的离群值、威胁或资源参数。 使用 Azure Analytics 来管理监视。
...
Azure 数字孪生使用 Azure AD 对用户进行身份验证并保护应用程序。 Azure AD 支持各种新式体系结构的身份验证。 所有这些体系结构都以行业标准协议（例如 OAuth 2.0 或 OpenID Connect）。 保护 Azure AD 的 IoT 空间的几个关键做法包括：

> [!div class="checklist"]
> * 将 Azure AD 应用机密和密钥存储在一个安全位置，例如 [Azure Key Vault](https://azure.microsoft.com/services/key-vault/)。
