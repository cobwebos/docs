...
请务必注意，本文档未介绍深奥的数据科研过程，及其数学和技术层面。 可以在 [Azure 机器学习服务文档](https://azure.microsoft.com/services/machine-learning/)和[博客](https://blogs.microsoft.com/blog/tag/azure-machine-learning/)中找到这些详细信息。
...
一个用于计算需求预测解决方案财务权益的基本公式是：![用于计算需求预测解决方案财务权益的基本公式](media/cortana-analytics-playbook-demand-forecasting-energy/financial-benefit-formula.png)
...
例如：如果需求预测解决方案在每天上午 8:00 生成新的预测，则我们需要确保在此时间点之前已完全引入过去 24 小时内收集到的所有数据，甚至必须包括最后一小时的数据。
...
* **缺失值** – 指未收集到特定测量值的情况。 在此，基本请求是在任何指定的期间，缺失值比率都不应该超过 10%。 如果单个值缺失，则该值应该使用预定义的值（例如：“9999”）而非“0”（可能是有效的度量值）来表示。
...
**独立度量值特征：** 独立特征包括我们想要在模型中用作预测指标的所有数据元素。 在此我们排除需要预测的依赖特征。
...
**依赖项特征：** 依赖项特征是模型要预测的数据列。 使用[监督式机器学习](https://en.wikipedia.org/wiki/Supervised_learning)时，需先使用依赖特征（也称为标签）来训练模型。 这样可让模型从依赖特征关联的数据中学习模式。 在能源需求预测中，通常需要预测实际需求，因此将它用作依赖特征。

**处理缺少的值：** 在数据准备阶段，我们将需要确定处理缺少的值的最佳策略。 我们主要使用不同的统计[数据插补法](https://en.wikipedia.org/wiki/Imputation_\(statistics\))实现此目的。 以能源需求预测而言，通常从以前可用的数据点使用移动平均来推算缺失值。
...
> [!NOTE]
> 本部分并非机器学习和预测的概述，只是简单探讨需求预测常用的建模技巧。 有关时序预测的详细信息和教材，强烈建议阅读在线丛书 [Forecasting: principles and practice](https://www.otexts.org/)（预测：原理和实践）。
>
>

#### <a name="ma-moving-average"></a>**MA（移动平均）**
...
#### <a name="ets-exponential-smoothing"></a>**ETS（指数平滑）**
指数平滑 (ETS) 由一系列方法组成，使用最近数据点的加权平均来预测下一个数据点。 其思路是将较高权重分配给较新的值，而对于较旧的测量值，逐渐降低此权重。 此方法系列包括许多不同的方法，其中一些方法可处理数据的季节性，例如 Holt-Winters 季节性方法。

还有一些方法也会考虑数据的季节性。

#### <a name="arima-auto-regression-integrated-moving-average"></a>**ARIMA（自动回归集成移动平均）**
