...
> [AZURE.SELECTOR]
- [Windows PowerShell](role-based-access-control-powershell)
- [Cross-Platform CLI](role-based-access-control-xplat-cli)
...
有关详细信息，请参阅[将 Windows PowerShell 与资源管理器配合使用](powershell-azure-resource-manager)。
...
有关详细信息，请参阅[如何安装和配置 Azure PowerShell](install-configure-powershell)。
...
要在哪个范围内分配：有三个级别的范围
   ...
   - 当前订阅
   - 资源组，若要获取资源组的列表，请键入 `PS C:> Get-AzureResourceGroup`
   - 资源，若要获取资源的列表，请键入 `PS C:> Get-AzureResource`
...
- [使用 XPLAT CLI 配置基于角色的访问控制](role-based-access-control-xplat-cli)
