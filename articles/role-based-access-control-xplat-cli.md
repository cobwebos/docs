...
<div><a href="/documentation/articles/role-based-access-control-powershell">Windows PowerShell</a><a href="/documentation/articles/role-based-access-control-xplat-cli">跨平台 CLI</a></div>
...
在本教程中，你将学习如何使用 Azure 跨平台命令行界面 (xplat-cli) 来管理基于角色的访问控制。它将指导你完成创建和检查角色分配的过程。
...
- Azure 跨平台命令行界面版本 0.8.8 或更高版本。若要安装最新版本并将其与你的 Azure 订阅相关联，请参阅[安装和配置 Azure 跨平台命令行界面](xplat-cli)。
- 另请阅读以下教程以熟悉如何在 Azure 跨平台命令行界面中设置和使用 Azure 资源管理器：[将 Azure 跨平台命令行界面用于资源管理器](xplat-cli-azure-resource-manager)
...
有关详细信息，请参阅[将 Azure 跨平台命令行界面用于资源管理器](xplat-cli-azure-resource-manager)

若要连接到 Azure 订阅，请键入：

    azure login -u <username>

在命令行提示符下，输入 Azure 帐户密码（仅使用组织帐户）。Xplat-cli 将使用此帐户获取你拥有的所有订阅并将自己配置为使用第一个订阅作为默认订阅。请注意，使用基于角色的访问控制，你只有通过成为共同管理员或具有某种角色分配才能获取你有某些权限的订阅。
...
有关详细信息，请参阅[安装和配置 Azure 跨平台命令行界面](xplat-cli)。
...
- [Azure 中基于角色的访问控制](role-based-access-control-configure)
- [安装并配置 Azure 跨平台命令行界面](xplat-cli)
- [将 Azure 跨平台命令行界面用于资源管理器](xplat-cli-azure-resource-manager)
- [使用资源组来管理 Azure 资源](azure-preview-portal-using-resource-groups)：了解如何在 Azure 管理门户中创建和管理资源组。
- [使用 Windows PowerShell 配置基于角色的访问控制](role-based-access-control-powershell)
