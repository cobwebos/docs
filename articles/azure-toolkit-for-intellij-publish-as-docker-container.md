...
# <a name="how-to-publish-a-web-app-as-a-docker-container-using-the-azure-toolkit-for-intellij"></a>如何使用用于 IntelliJ 的 Azure 工具包将 Web 应用发布为 Docker 容器

Docker 容器广泛用于部署 Web 应用程序，开发人员可在其中将其所有项目文件和依赖项整合成单个包，以便部署到服务器。 用于 IntelliJ 的 Azure 工具包可以添加用于部署到 Microsoft Azure 的*发布为 Docker 容器*功能，为 Java 开发人员简化了部署过程。本文中的步骤将引导你完成将应用程序作为 Docker 容器发布到 Azure 的过程。
...
> [!NOTE]
>
> 若要发布 Web 应用，需要创建一个随时可用于部署的项目。 有关其他信息，请参阅本文稍后的[有关创建项目的其他信息](#artifacts)。
>
> 此外，在完成部署向导至少一次后，再次运行向导时，在本演练中指定的大部分设置将用作默认值。
>
...
1. 使用以下方法之一启动“发布为 Docker 容器”向导：
   ...
   * 在“项目”工具窗口中右键单击你的项目，然后依次单击“Azure”、“发布为 Docker 容器”： ![发布为 Docker 容器][PUB01]

   * 在 IntelliJ 工具栏中单击“发布组”菜单，然后单击“发布为 Docker 容器”： ![发布为 Docker 容器][PUB02]

1. 出现“在 Azure 上部署 Docker 容器”向导时，你将看到如下图所示的对话框：![在 Azure 上部署 Docker 容器][PUB03]

   a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“Docker 映像名称”文本框中输入 Docker 主机的唯一名称。 （向导会自动创建名称，但如果需要，你可以修改该名称。）

   b. “主机”窗口将显示已创建的所有 Docker 主机。
      * 如果你尚未创建任何 Docker 主机，对话框的此部分将是空白的。
      * 如果已创建 Docker 主机，可以选择将 Web 应用部署到现有主机；否则，请遵循以下步骤创建新的 Docker 主机。

1. 若要创建新的 Docker 主机，请单击绿色加号 (**+**)；此时将启动“创建 Docker 主机”对话框。
      ![在 Azure 向导中部署 Docker 容器][PUB04a]
   ...
   b.保留“数据库类型”设置，即设置为“共享”。 指定上述选项后，请单击“下一步”。
...
Azure 工具包随即开始将 Web 应用部署到 Azure 中的某个 Docker 容器。除非已将 IntelliJ 配置为在后台部署，否则会出现一个对话框，其中显示部署进度。 
![部署进度][PUB09]

<a name="artifacts"></a>
## <a name="additional-information-about-creating-artifacts"></a>有关创建项目的其他信息

若要创建随时可用于部署的项目，请使用以下步骤：
...
1. 单击绿色加号 (**+**) 添加项目，然后单击“Web 应用程序: 项目”。
   ![添加项目][ART02]

1. 为项目命名（切勿添加“.war”扩展名），然后单击“确定”。
   ![项目属性][ART03]
