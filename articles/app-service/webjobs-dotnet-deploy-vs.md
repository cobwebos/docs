...
本文介绍如何使用 Visual Studio 将控制台应用程序项目作为 [Azure WebJob](https://go.microsoft.com/fwlink/?LinkId=390226) 部署到[应用服务](overview.md)中的 Web 应用。 有关如何使用[Azure 门户](https://portal.azure.com)部署 Web 作业的信息，请参阅使用[Web 作业运行后台任务](webjobs-create.md)。
...
### <a name="enable-webjobs-deployment-for-an-existing-console-application-project"></a><a id="convert"></a> 为现有控制台应用程序项目启用 WebJobs 部署
...
#### <a name="enable-automatic-webjobs-deployment-with-a-web-project"></a><a id="convertlink"></a>使用 Web 项目启用自动 WebJobs 部署
...
#### <a name="enable-webjobs-deployment-without-a-web-project"></a><a id="convertnolink"></a> 不使用 Web 项目启用 WebJobs 部署
...
### <a name="create-a-new-webjobs-enabled-project"></a><a id="create"></a>创建已启用 WebJobs 的新项目
若要创建已启用 Web 作业的新项目，可以使用控制台应用程序项目模板，并根据[上一节](#convert)所述启用 Web 作业部署。 或者，可以使用 Web 作业新建项目模板：

* [将 Web作业新项目模板用于独立的 Web 作业](#createnolink)
...
* [对链接到 Web 项目的 Web 作业使用 Web 作业新项目模板](#createlink)
...
#### <a name="use-the-webjobs-new-project-template-for-an-independent-webjob"></a><a id="createnolink"></a> 为独立的 WebJob 使用 WebJobs 新建项目模板
1. 单击 **"文件** > **新项目**"，然后在 **"新项目**"对话框中单击**云** > **Azure Web 作业 （.NET 框架）。**
   
    ![显示了 Web 作业模板的“新建项目”对话框](./media/webjobs-dotnet-deploy-vs/np.png)
2. 按照前述说明，[将控制台应用程序项目设为独立的 Web 作业项目](#convertnolink)。

#### <a name="use-the-webjobs-new-project-template-for-a-webjob-linked-to-a-web-project"></a><a id="createlink"></a> 为链接到 Web 项目的 WebJob 使用 WebJobs 新建项目模板
...
### <a name="the-add-azure-webjob-dialog"></a><a id="configure"></a>添加 Azure Web 作业对话框
使用“添加 Azure WebJob”对话框可以输入你的 WebJob 的 WebJob 名称和运行模式。**** 

![“添加 Azure Web 作业”对话框](./media/webjobs-dotnet-deploy-vs/aaw2.png)

此对话框中的字段对应于 Azure 门户的“添加 WebJob”**** 对话框中的字段。 有关详细信息，请参阅[使用 WebJobs 运行后台任务](webjobs-create.md)。
...
> * 如果部署 WebJob，然后决定要更改 WebJob 和重新部署的类型，则需要删除*Webjob-发布设置.json*文件。 这样，Visual Studio 就会再次显示发布选项，使你可以更改 WebJob 的类型。
...
可以编辑此文件目录，Visual Studio 将提供 IntelliSense。 文件架构存储在，[https://schemastore.org](https://schemastore.org/schemas/json/webjob-publish-settings.json)并可在那里查看。  
...
可以编辑此文件目录，Visual Studio 将提供 IntelliSense。 文件架构存储在，[https://schemastore.org](https://schemastore.org/schemas/json/webjobs-list.json)并可在那里查看。

### <a name="deploy-a-webjobs-project"></a><a id="deploy"></a>部署 WebJobs 项目
已链接到 Web 项目的 Web 作业项目会通过 Web 项目自动部署。 有关 Web 项目部署的信息，请参阅左侧**导航中的"部署应用"指南** > **Deploy app**。
