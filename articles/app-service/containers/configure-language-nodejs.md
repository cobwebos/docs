...
Node.js 应用必须部署所有必需的 NPM 依赖项。 部署[Git 存储库](../deploy-local-git.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json)时，或在启用生成`npm install --production` [过程的情况](../deploy-zip.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json)下，应用服务部署引擎（Kudu）会自动运行。 但是，如果使用[FTP/S](../deploy-ftp.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json)部署文件，则需要手动上传所需的包。
...
1. 无`npm install`需任何标志即可运行，其中`preinstall`包括`postinstall` npm 和脚本， `devDependencies`也会进行安装。
1. 如果`npm run build`在*包*中指定了生成脚本，则运行。
1. 如果`npm run build:azure`生成： azure 脚本是在*包 json*中指定的，则运行。
1. 如果由指定， `POST_BUILD_SCRIPT_PATH`则运行自定义脚本。

> [!NOTE]
> 如[npm 文档](https://docs.npmjs.com/misc/scripts)中所述，在`prebuild`指定`postbuild`的之前和之后`build`分别运行的脚本。 `preinstall`并`postinstall`分别在之前和`install`之后运行。
...
若要使用`npm start`启动应用程序，只需确保`start` *包 json*文件中有脚本。 例如：
...
在大多数情况下，你的应用程序不需要进行额外配置。 如果你的应用程序运行时使用的是*进程的 json*文件（默认或自定义）， `script`则它必须具有 json 根中的属性。 例如：
...
默认情况下，Kudu `npm install --production`在识别 node.js 应用时运行。 如果应用需要任何常用的自动化工具，如 Grunt、Bower 或 Gulp，则需要提供一个[自定义部署脚本](https://github.com/projectkudu/kudu/wiki/Custom-Deployment-Script)来运行该工具。
...
打开*deploy.sh*并找到如下`Deployment`所示的部分：
...
此部分结束于运行`npm install --production`。 添加代码部分，需要*在* `Deployment`节末尾运行所需的工具：
...
- 在生产模式下本地测试应用。 应用服务在生产模式下运行 node.js 应用，因此，你需要确保你的项目在生产模式下以本地方式按预期方式工作。 例如：
    - 根据*包*的不同，可能会为生产模式（`dependencies`与`devDependencies`）安装不同的包。
    - 某些 web 框架可以在生产模式下以不同的方式部署静态文件。
    - 在生产模式下运行时，某些 web 框架可能使用自定义的启动脚本。
- 在开发模式下，在应用服务中运行应用。 例如[，在中，可以](https://meanjs.org/)通过[设置`NODE_ENV`应用设置](../configure-common.md?toc=%2fazure%2fapp-service%2fcontainers%2ftoc.json#configure-app-settings)，将应用程序设置为运行时中的开发模式。
