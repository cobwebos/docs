---
title: 配置 Linux Java 应用程序
description: 了解如何为应用配置预构建的 Java 容器。 本文介绍最常见的配置任务。
keywords: azure 应用程序服务， Web 应用程序， linux， oss， java， java ee， jee， javaee
author: bmitchell287
manager: barbkess
ms.devlang: java
ms.topic: article
...
Linux 上的 Azure 应用服务允许 Java 开发人员在完全托管的基于 Linux 的服务上快速构建、部署和扩展其 Tomcat 或 Java 标准版 （SE） 打包的 Web 应用程序。 可以在命令行或者 IntelliJ、Eclipse 或 Visual Studio Code 等编辑器中使用 Maven 插件部署应用程序。

本指南为在应用服务中使用内置 Linux 容器的 Java 开发人员提供了关键概念和说明。 如果您从未使用过 Azure 应用服务，请按照[Java 快速入门](quickstart-java.md)操作。

## <a name="deploying-your-app"></a>部署应用

可以使用[Azure 应用服务的 Maven 插件](/java/api/overview/azure/maven/azure-webapp-maven-plugin/readme)来部署 .jar 和 .war 文件。 [Azure Toolkit for IntelliJ](/java/azure/intellij/azure-toolkit-for-intellij) 或 [Azure Toolkit for Eclipse](/java/azure/eclipse/azure-toolkit-for-eclipse) 还支持通过流行的 IDE 进行部署。
...
通过 Azure 门户或 [Azure CLI](/cli/azure/webapp/log#az-webapp-log-config) 启用[应用程序日志记录](../troubleshoot-diagnostic-logs.md?toc=/azure/app-service/containers/toc.json#enable-application-logging-windows)，以将应用服务配置为向本地文件系统或 Azure Blob 存储写入应用程序的标准控制台输出和标准控制台错误流。 在完成配置并经过 12 个小时后，将禁用记录到应用服务本地文件系统实例。 如果需要保留日志更长时间，请将应用程序配置为向 Blob 存储容器写入输出。 您的 Java 和 Tomcat 应用日志可以在 */home/LogFiles/应用程序/* 目录中找到。
...
内置Java映像基于[阿尔卑斯Linux](https://alpine-linux.readthedocs.io/en/latest/getting_started.html)操作系统。 使用`apk`包管理器安装任何故障排除工具或命令。

### <a name="flight-recorder"></a>网络流量记录器

应用服务上的所有 Linux Java 映像都安装了祖鲁飞行记录器，因此您可以轻松连接到 JVM 并启动探查器录制或生成堆转储。

#### <a name="timed-recording"></a>时记录

要开始使用，SSH 进入应用服务并`jcmd`运行该命令以查看正在运行的所有 Java 进程的列表。 除了 jcmd 本身之外，还应看到 Java 应用程序使用进程 ID 号 （pid） 运行。
...
执行下面的命令以开始 30 秒的 JVM 录制。 这将分析 JVM 并在主目录中创建名为*jfr_example.jfr*的 JFR 文件。 （将 116 替换为 Java 应用的 pid。

```shell
jcmd 116 JFR.start name=MyRecording settings=profile duration=30s filename="/home/jfr_example.jfr"
```

在 30 秒间隔内，您可以通过运行`jcmd 116 JFR.check`来验证正在发生的录制。 这将显示给定 Java 进程的所有录制。

#### <a name="continuous-recording"></a>连续录制

您可以使用祖鲁飞行记录器持续分析 Java 应用程序，对运行时性能的影响最小（[源](https://assets.azul.com/files/Zulu-Mission-Control-data-sheet-31-Mar-19.pdf)）。 为此，请运行以下 Azure CLI 命令以创建名为JAVA_OPTS的应用设置，并具有必要的配置。 启动应用时，JAVA_OPTS应用设置的内容将传递给`java`该命令。

```azurecli
az webapp config appsettings set -g <your_resource_group> -n <your_app_name> --settings JAVA_OPTS=-XX:StartFlightRecording=disk=true,name=continuous_recording,dumponexit=true,maxsize=1024m,maxage=1d
```

录制开始后，您可以随时使用 命令`JFR.dump`转储当前录制数据。
...
### <a name="analyzing-recordings"></a>分析录音
...
适用于 Linux 的 Azure 应用服务支持通过 Azure 门户和 CLI 进行开箱即用的调优和自定义。 请查看以下文章了解非特定于 Java 的 Web 应用配置：
...
要在 Tomcat 和 Java SE 环境中设置分配的内存或其他 JVM 运行时选项，请创建`JAVA_OPTS`一个使用这些选项命名的[应用设置](../configure-common.md?toc=/azure/app-service/containers/toc.json#configure-app-settings)。 应用服务 Linux 在启动时，会将此设置作为环境变量传递给 Java 运行时。
...
如果要部署 JAR 应用程序，则应将其命名为*app.jar，* 以便内置映像能够正确标识你的应用。 （Maven 插件会自动进行此重命名。如果您不希望将 JAR 重命名为*app.jar，* 则可以上载带有命令的 shell 脚本来运行 JAR。 然后，将此脚本的完整路径粘贴到门户“配置”部分的[启动文件](app-service-linux-faq.md#built-in-images)文本框中。 启动脚本不从放置它的目录运行。 因此，请始终使用绝对路径在启动脚本中引用文件（例如：`java -jar /home/myapp/myapp.jar`）。
...
如果 Java 应用程序特别大，则应增加启动时间限制。 为此，请创建一个应用程序设置，`WEBSITES_CONTAINER_START_TIME_LIMIT`并将其设置为应用服务在超时之前应等待的秒数。最大值为`1800`秒。
...
Spring Boot 开发人员可以使用 [Azure Active Directory Spring Boot Starter](/java/azure/spring-framework/configure-spring-boot-starter-java-app-with-azure-active-directory?view=azure-java-stable) 通过熟悉的 Spring Security 注释和 API 来保护应用程序。 请确保在*应用程序.属性*文件中增加最大标头大小。 我们建议值为 `16384`。
...
默认情况下，在容器启动时，[上载到 App Service Linux](../configure-ssl-certificate.md)的任何公共或私有证书都将加载到相应的 Java 密钥存储中。 上传证书后，需要重新启动应用服务才能将其加载到 Java 密钥存储中。 公共证书加载到 的密钥存储中`$JAVA_HOME/jre/lib/security/cacerts`，私有证书存储在 中`$JAVA_HOME/lib/security/client.jks`。

使用 Java 密钥存储中的证书加密 JDBC 连接可能需要其他配置。 请参阅您选择的 JDBC 驱动程序的文档。
...
要初始化对象`import java.security.KeyStore`，请使用密码加载密钥存储文件。 两个密钥存储的默认密码为"更改"。
...
您可以手动将证书加载到密钥存储。 创建应用设置 ，`SKIP_JAVA_KEYSTORE_LOAD`其值为`1`禁用应用服务自动将证书加载到密钥存储中。 通过 Azure 门户上载到应用服务的所有公共证书都存储在`/var/ssl/certs/`下。 私有证书存储在 下`/var/ssl/private/`。

您可以通过打开到应用服务的[SSH 连接](app-service-linux-ssh-support.md)并运行命令`keytool`来交互或调试 Java 密钥工具。 有关命令列表，请参阅["关键工具"文档](https://docs.oracle.com/javase/8/docs/technotes/tools/unix/keytool.html)。 有关 KeyStore API 的更多信息，请参阅[官方文档](https://docs.oracle.com/javase/8/docs/api/java/security/KeyStore.html)。
...
4. [SSH 进入您的应用服务实例](app-service-linux-ssh-support.md)，并创建新的目录 */home/网站/wwwroot/apm*。
5. 将未打包的 NewRelicJava 代理文件上载到 */home/站点/wwwroot/apm*下的目录中。 代理文件应位于 */home/site/wwwroot/apm/newrelic* 中。
...
3. [SSH 进入您的应用服务实例](app-service-linux-ssh-support.md)，并创建新的目录 */home/网站/wwwroot/apm*。
...
默认情况下，应用服务希望您的 JAR 应用程序被命名为*app.jar*。 如果它具有此名称，它将自动运行。 对于 Maven 用户，您可以通过在`<finalName>app</finalName>`*pom.xml*`<build>`节中包括 JAR 名称来设置 JAR 名称。 您可以通过设置`archiveFileName`属性[在 Gradle 中执行相同的操作](https://docs.gradle.org/current/dsl/org.gradle.api.tasks.bundling.Jar.html#org.gradle.api.tasks.bundling.Jar:archiveFileName)。

如果要对 JAR 使用不同的名称，还必须提供执行 JAR 文件的[启动命令](app-service-linux-faq.md#built-in-images)。 例如，`java -jar my-jar-app.jar` 。 您可以在门户、配置>常规设置下或使用名为 的应用程序`STARTUP_COMMAND`设置设置启动命令的值。

### <a name="server-port"></a>服务器端口

App Service Linux 将传入请求路由到端口 80，因此应用程序也应侦听端口 80。 您可以在应用程序的配置（如 Spring*的应用程序.属性*文件）或启动命令（例如。 `java -jar spring-app.jar --server.port=80` 请参阅以下常见 Java 框架的文档：

- [Spring Boot](https://docs.spring.io/spring-boot/docs/current/reference/html/howto-properties-and-configuration.html#howto-use-short-command-line-arguments)
- [斯帕克贾瓦](http://sparkjava.com/documentation#embedded-web-server)
...
- [顶点](https://vertx.io/docs/vertx-core/java/#_start_the_server_listening)
- [夸库斯](https://quarkus.io/guides/application-configuration-guide)
...
#### <a name="shared-server-level-resources"></a>共享服务器级资源

添加共享的服务器级数据源需要编辑 Tomcat 的服务器.xml。 首先，上载[启动脚本](app-service-linux-faq.md#built-in-images)，并在**配置** > **启动命令**中将路径设置为脚本。 您可以使用[FTP](../deploy-ftp.md)上载启动脚本。

启动脚本将[xsl 转换为](https://www.w3schools.com/xml/xsl_intro.asp)server.xml 文件，并将生成的 xml 文件输出`/usr/local/tomcat/conf/server.xml`到 。 启动脚本应通过 apk 安装 libxslt。 您的 xsl 文件和启动脚本可以通过 FTP 上传。 下面是一个启动脚本示例。
...
下面提供了一个示例 xsl 文件。 示例 xsl 文件向 Tomcat server.xml 添加了一个新的连接器节点。
...
最后，将驱动程序 JAR 放在 Tomcat 类路径中并重新启动应用服务。

1. 将 JDBC 驱动程序文件放入 */home/tomcat/lib* 目录，确保它们可供 Tomcat 类加载器使用。 （如果此目录不存在，请创建该目录。要将这些文件上载到应用服务实例，请执行以下步骤：
...
要连接到 Spring Boot 应用程序中的数据源，我们建议创建连接字符串并将其注入*到应用程序.*

1. 在应用服务页面的"配置"部分中，为字符串设置名称，将 JDBC 连接字符串粘贴到值字段中，并将类型设置为"自定义"。 您可以选择将此连接字符串设置为插槽设置。

    此连接字符串可供我们的应用程序作为名为 的环境变量访问`CUSTOMCONNSTR_<your-string-name>`。 例如，我们在上面创建的连接字符串将命名为`CUSTOMCONNSTR_exampledb`。

2. 在*应用程序.属性*文件中，使用环境变量名称引用此连接字符串。 对于我们的示例，我们将使用以下内容。

    ```yml
    app.datasource.url=${CUSTOMCONNSTR_exampledb}
    ```

有关本主题的详细信息，请参阅[有关数据访问](https://docs.spring.io/spring-boot/docs/current/reference/html/howto-data-access.html)和[外部化配置](https://docs.spring.io/spring-boot/docs/current/reference/html/boot-features-external-config.html)的 Spring Boot 文档。

## <a name="use-redis-as-a-session-cache-with-tomcat"></a>使用 Redis 作为与 Tomcat 的会话缓存

您可以将 Tomcat 配置为使用外部会话存储，如[Redis 的 Azure 缓存](/azure/azure-cache-for-redis/)。 这使您能够在用户转移到应用的另一个实例时保留用户会话状态（如购物车数据），例如发生自动缩放、重新启动或故障转移时。

要将 Tomcat 与 Redis 一起使用，必须将应用配置为使用[持久管理器](https://tomcat.apache.org/tomcat-8.5-doc/config/manager.html)实现。 以下步骤使用[Pivotal 会话管理器：redis 存储](https://github.com/pivotalsoftware/session-managers/tree/master/redis-store)作为示例来解释此过程。
...
1. 打开 Bash 终端，`<variable>=<value>`并用于设置以下每个环境变量。
    ...
    | 区域                   | 托管应用的区域的名称。                           |
    | REDIS_CACHE_NAME         | Redis 实例的 Azure 缓存的名称。                           |
    | REDIS_PORT               | Redis 缓存侦听的 SSL 端口。                             |
    | REDIS_PASSWORD           | 实例的主访问密钥。                                  |
    | REDIS_SESSION_KEY_PREFIX | 指定用于标识来自应用的会话键的值。 |
    ...
    通过查看服务**实例的属性或****访问键**部分，可以在 Azure 门户上查找名称、端口和访问密钥信息。

2. 创建或更新应用的*src/主/webapp/META-INF/context.xml*文件，包含以下内容：
    ...
    此文件指定并配置应用的会话管理器实现。 它使用您在上一步中设置的环境变量来将您的帐户信息从源文件中排除。

3. 使用 FTP 将会话管理器的 JAR 文件上载到应用服务实例，并将其放在 */home/tomcat/lib*目录中。 有关详细信息，请参阅使用[FTP/S 将应用部署到 Azure 应用服务](https://docs.microsoft.com/azure/app-service/deploy-ftp)。

4. 禁用应用服务实例的[会话关联 Cookie。](https://azure.microsoft.com/blog/disabling-arrs-instance-affinity-in-windows-azure-web-sites/) 可以通过导航到应用，然后设置 **"配置>常规设置> ARR 关联**设置为**Off，** 从 Azure 门户执行此操作。 或者，您可以使用以下命令：

    ```azurecli
    az webapp update -g <resource group> -n <webapp name> --client-affinity-enabled false
    ```

    默认情况下，应用服务将使用会话关联 Cookie 来确保具有现有会话的客户端请求路由到应用程序的同一实例。 此默认行为不需要任何配置，但在重新启动应用实例或将流量重新路由到其他实例时，它无法保留用户会话状态。 禁用[现有 ARR 实例关联](https://azure.microsoft.com/blog/disabling-arrs-instance-affinity-in-windows-azure-web-sites/)配置以关闭基于会话 Cookie 的路由时，允许配置的会话存储在不受干扰的情况下运行。

5. 导航到应用服务实例**的属性部分，** 并查找**其他出站 IP 地址**。 这些表示应用的所有可能的出站 IP 地址。 复制这些，以便在下一步中使用。

6. 对于每个 IP 地址，请在 Azure 缓存中为 Redis 实例创建防火墙规则。 您可以在 Redis 实例的**防火墙**部分在 Azure 门户上执行此操作。 为每个规则提供唯一的名称，并将 **"开始 IP 地址"** 和 **"结束 IP 地址**"值设置为相同的 IP 地址。

7. 导航到 Redis 实例的高级**设置**部分，并将**仅通过 SSL 允许访问**设置为 **"否**"。 这使应用服务实例能够通过 Azure 基础结构与 Redis 缓存进行通信。

8. 更新`azure-webapp-maven-plugin`应用*pom.xml*文件中的配置，以参考您的 Redis 帐户信息。 此文件使用以前设置的环境变量来将帐户信息从源文件中排除。
...
你的应用现在将使用 Redis 缓存进行会话管理。

有关可用于测试这些说明的示例，请参阅 GitHub 上的[缩放状态 java-Web 应用](https://github.com/Azure-Samples/scaling-stateful-java-web-app-on-azure)在 azure 存储库。
...
- Java SE 运行时环境：用于运行打包为 Java 存档 (JAR) 文件的应用程序。 支持的版本是 Java 8 和 11。

### <a name="jdk-versions-and-maintenance"></a>JDK 版本和维护

Azul Zulu Enterprise 内部版 OpenJDK 是适用于 Azure 和 Azure Stack 的 OpenJDK 的免费、多平台、生产就绪型发行版，由 Microsoft 及 Azul Systems 提供支持。 这些版本包含构建和运行 Java SE 应用程序所需的所有组件。 你可以从[JavaJDK安装JDK](https://aka.ms/azure-jdks)安装。
