---
title: 混合连接
description: 了解如何在 Azure App Service 中创建和使用混合连接以访问不同网络中的资源。
author: ccompy
...
对于对基础 Azure 服务总线中继拥有足够权限的任何用户，可以将现有混合连接添加到其他应用服务 Web 应用。 这意味着，如果必须阻止其他人重复使用相同的混合连接（例如，当目标资源是没有任何其他安全措施以防止未经授权的访问）的服务时，必须锁定对 Azure 的访问权限服务总线中继。

在 Azure 门户中尝试将其添加到他们的 Web 应用时，任何有 `Reader` 访问中继的用户都将可以_查看_混合连接，但当他们无权检索用于建立中继连接的连接字符串时，它们将无法_添加_。 为了成功添加混合连接，它们必须具有 `listKeys` 权限（`Microsoft.Relay/namespaces/hybridConnections/authorizationRules/listKeys/action`）。 在中继上包含此权限的 `Contributor` 角色或任何其他角色将允许用户使用混合连接，并将其添加到自己的 Web 应用。
