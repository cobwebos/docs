...
1. 使用 Azure 帐户登录来获取混合连接可与你的订阅。 HCM 不继续使用 Azure 以外的帐户。 
1. 选择订阅。
1. 选择 HCM 要中继的混合连接。
![混合连接的屏幕截图][9]
...
在应用服务中， **tcpping**可以通过高级工具 (Kudu) 控制台调用命令行工具。 此工具可以告知你是否能够访问 TCP 终结点，但不会告知你是否能够访问混合连接终结点。 在控制台中针对混合连接终结点使用此工具时，只能确认混合连接是否使用了“主机:端口”组合。  

如果必须将命令行客户端终结点，也可以测试从应用控制台的连接。 例如，可以通过使用 curl 测试与 web 服务器终结点的访问权限。
