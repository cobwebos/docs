...
可以通过应用门户或混合连接服务门户创建混合连接。  强烈建议通过应用门户创建要在应用中使用的混合连接。  要创建混合连接，请转到 [Azure 门户][portal]，并进入应用的 UI。  选择“网络”>“配置混合连接终结点”。  从此处，可以看到为应用配置的混合连接  
...
就像在应用视图中一样，可以通过单击查看有关混合连接的详细信息。  在此处所示的属性中，不仅可以看到应用视图提供的所有信息，而且还能看到同一应用服务计划中的其他多少个应用正在使用该混合连接。
...
此工具可在 Windows server 2008 R2 和更高版本的 Windows 上运行。  安装后，HCM 以服务的形式运行。  此服务基于配置的终结点连接到 Azure 服务总线中继。  从 HCM 建立的连接是与端口 80 和 443 建立的出站连接。    
...
如果想要让订阅外部的某人托管给定混合连接的 HCM 实例，可与他（她）共享该混合连接的网关连接字符串。  在 [Azure 门户][portal]上的混合连接属性中可以看到该字符串。 要使用该字符串，请在 HCM 中单击“手动配置”按钮，并粘贴网关连接字符串。


## <a name="troubleshooting"></a>故障排除 ##

如果与运行中的应用程序建立了混合连接并且至少为一个 HCM 配置了混合连接，门户中的状态会显示为“已连接”。  如果不是显示为“已连接”，则意味着应用已关闭，或者 HCM 无法通过端口 80 或 443 连接到 Azure。  
...
可以通过控制台调用应用服务中一个名为 tcpping 的工具。  此工具可以告知你是否能够访问 TCP 终结点，但不会告知你是否能够访问混合连接终结点。  在控制台中针对混合连接终结点使用此工具时，如果 ping 命令成功，结果只会告知已经为应用配置了一个使用“主机:端口”组合的混合连接。  
...
要将 BizTalk 混合连接添加到应用，请在 [Azure 门户][portal]中转到应用，并单击“网络”>“配置混合连接终结点”。  在“经典混合连接”表中，单击“添加经典混合连接”。  此处会显示 BizTalk 混合连接的列表。  
