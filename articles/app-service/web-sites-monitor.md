...
ms.locfileid: "79500437"
---
# <a name="monitor-apps-in-azure-app-service"></a>监视 Azure 应用服务中的应用
[Azure 应用服务](https://go.microsoft.com/fwlink/?LinkId=529714)为[Azure 门户](https://portal.azure.com)中的 Web 应用、移动和 API 应用提供了内置的监视功能。

在 Azure 门户中，您可以查看应用和应用服务计划的*配额*和*指标*，并设置基于规则的*警报*和*自动缩放*指标。
...
| **IO 每秒其他操作数** | 应用进程发出未读取或写入操作的 I/O 操作的速率。|
...
| **专用字节数** | 专用字节是应用进程分配的当前内存大小（以字节为单位）无法与其他进程共享。|
| **Requests** | 请求总数，不考虑是否导致 HTTP 状态代码。 |
...
| **数据** | 计划的所有实例使用的平均输入带宽量。 |
| **数据输出** | 计划的所有实例使用的平均输出带宽量。 |
| **磁盘队列长度** | 在存储上排队的读取和写入请求的平均数量。 高磁盘队列长度表示应用可能由于磁盘 I/O 过多而变慢。 |
...
**CPU 时间**：对于在"免费"或"共享"计划中托管的应用很有用，因为它们的配额之一在应用使用的 CPU 分钟数中定义。

**CPU 百分比**：对于在基本、标准和高级计划中托管的应用很有用，因为它们可以横向扩展。CPU 百分比很好地指示所有实例的总体使用情况。
...
要查找配额，请选择 **"设置** > **配额**"。 在图表中，可以查看： 
1. 配额名称。
1. 配额的重置时间间隔。
1. 配额的当前限制。
1. 配额的当前值。

![Azure 门户][metrics]中的指标图表可以直接从资源**概述**页访问指标。 在这里，您将看到表示某些应用指标的图表。

单击这些图表中的任何一个将带您到指标视图，您可以在其中创建自定义图表、查询不同的指标等。 
...
托管在基本或更高应用服务计划中的应用支持自动缩放。 使用自动缩放可以配置用于监视应用服务计划指标的规则。 规则可以增加或减少实例计数，并根据需要提供更多的资源。 规则还有助于避免过度预配应用，从而节省资金。
