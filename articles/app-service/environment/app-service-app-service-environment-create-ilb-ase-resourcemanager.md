...
3. 上传的 SSL 证书显式分配给 ILB ASE 作为其“默认”SSL 证书。  当应用使用分配给 ASE 的常见根域寻址时，此 SSL 证书将用于 ILB ASE 上应用的 SSL 流量（例如`https://someapp.mycustomrootcomain.com`）
...
然后必须将生成的 .pfx 文件转换为 base64 字符串，因为会使用 Azure 资源管理器模板上传 SSL 证书。  Azure 资源管理器模板是文本文件，因此必须将 .pfx 文件转换为 base64 字符串，才能纳入为模板的参数。
...
一旦填入 *azuredeploy.parameters.json* 文件，就可以使用以下 Powershell 代码片段配置默认 SSL 证书。  更改文件 PATH，以匹配 Azure 资源管理器模板文件在计算机上的位置。  此外，切记提供自己的 Azure 资源管理器部署名称值和资源组名称值。
...
完成模板后，可通过 HTTPS 访问 ILB ASE 上的应用，并使用默认 SSL 证书保护连接。  如果 ILB ASE 上的应用使用应用程序名称与默认主机名的组合来寻址，则会使用默认 SSL 证书。  例如*https://mycustomapp.internal-contoso.com*，将使用 =*.internal-contoso.com 的*默认 SSL 证书。
