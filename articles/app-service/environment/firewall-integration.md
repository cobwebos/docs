...
如果使用通过防火墙设备进行的出站流量部署 ASE, 则需要更改 ASE 子网上的路由。 路由在 IP 级别运行。 如果你不小心定义你的路由, 则可以将 TCP 回复流量从另一个地址强制发送到源。 这称为非对称路由, 并将中断 TCP。

必须已定义路由, 以便与 ASE 的入站流量可以通过与流量传入相同的方式回复。 这适用于入站管理请求, 适用于入站应用程序请求。
