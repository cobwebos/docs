---
title: 锁定出站流量
description: 了解如何与 Azure 防火墙集成，为应用服务环境中的出站流量提供保护。
author: ccompy
...
有许多可用于管理 ASE 的入站终结点。 无法通过防火墙设备发送入站管理流量。 此流量的源地址是已知的，并已在[应用服务环境管理地址](https://docs.microsoft.com/azure/app-service/environment/management-addresses)文档中发布。 还有一个名为 AppServiceManagement 的服务标记，可以与网络安全组 (NSG) 一起使用，以保护入站流量。
...
部署出站流量会经过防火墙设备的 ASE 需要更改 ASE 子网上的路由。 路由在 IP 级别运行。 如果在定义路由时不谨慎，就可能强制 TCP 将流量从另一个地址回复给源。 如果回复地址与发送到的地址不同，会产生非对称路由问题，这会破坏 TCP。

路由的定义必须能够使去往 ASE 的入站流量可以按原路回复到该流量来时的地方。 必须为入站管理请求和入站应用程序请求定义路由。

进出 ASE 的流量必须遵循以下约定

* 如果使用防火墙设备，则不支持发往 Azure SQL、存储和事件中心的流量。 这些流量必须直接发送到这些服务。 实现此目的的方法是为这三项服务配置服务终结点。 
* 路由表规则的定义必须能够使入站管理流量按其传入路径发回其传入位置。
* 路由表规则的定义必须能够使入站应用程序流量按其传入路径发回其传入位置。 
* 离开 ASE 的所有其他流量都可以通过路由表规则发送到防火墙设备。

![使用 Azure 防火墙的 ASE 连接流][5]

## <a name="locking-down-inbound-management-traffic"></a>锁定入站管理流量

如果还没有为 ASE 子网分配 NSG，请创建一个 NSG。 在 NSG 中，将第一个规则设置为允许端口 454、455 上通行来自名为 AppServiceManagement 的服务标记的流量。 允许来自 AppServiceManagement 标记的访问这一规则是管理 ASE 时需要设置的唯一公共 IP 规则。 该服务标记背后的地址仅用于管理 Azure 应用服务。 流经这些连接路径的管理流量是经过加密的，并使用身份验证证书进行保护。 此通道上的典型流量包括客户发起的命令和运行状况探测等。 

通过门户生成的包含新子网的 ASE 配置了包含 AppServiceManagement 标记的允许规则的 NSG。  

ASE 还需要允许收发端口 16001 上来自负载均衡器标记的入站请求。 端口 16001 上来自负载均衡器的请求是负载均衡器和 ASE 前端之间的“保持活动状态”检查。 如果阻止端口 16001，ASE 将无法保持正常运行。

## <a name="configuring-azure-firewall-with-your-ase"></a>在 ASE 中配置 Azure 防火墙 

使用 Azure 防火墙锁定现有 ASE 的传出流量的步骤如下：

1. 为 ASE 子网中的 SQL、存储和事件中心启用服务终结点。 若要启用服务终结点，请转到网络门户 >“子网”，然后从服务终结点下拉列表中选择“Microsoft.EventHub”、“Microsoft.SQL”和“Microsoft.Storage”。 为 Azure SQL 启用服务终结点后，还必须为应用的所有 Azure SQL 依赖项配置服务终结点。 
...
1. 在 Azure 防火墙 UI >“规则”>“网络规则集合”中，选择“添加网络规则集合”。 提供名称、优先级，并设置“允许”。 在“IP 地址”的“规则”部分提供名称，选择“任何”，将源和目标地址设置为 *，将端口设置为 123。 此规则允许系统使用 NTP 执行时钟同步。 以相同的方式针对端口 12000 创建另一个规则，以帮助诊断任何系统问题。 

   ![添加 NTP 网络规则][3]
   
1. 在 Azure 防火墙 UI >“规则”>“网络规则集合”中，选择“添加网络规则集合”。 提供名称、优先级，并设置“允许”。 在“服务标记”下的“规则”部分提供名称，选择**任何**协议，将源地址设置为 *，选择 AzureMonitor 的服务标记，并将端口设置为 80443。 此规则允许系统向 Azure Monitor 提供运行状况和指标信息。
...
对于 US Gov 区域的 ASE，请按照本文档中[在 ASE 中配置 Azure 防火墙](https://docs.microsoft.com/azure/app-service/environment/firewall-integration#configuring-azure-firewall-with-your-ase)部分的说明为 ASE 配置 Azure 防火墙。
...
Linux 在 US Gov 区域中不可用，因此未在可选配置中列出。
