...
要注册多个 Azure 应用服务终结点（全部在相同的 Azure 区域中运行），最简单的方法是使用 Powershell [Azure 资源管理器流量管理器支持][ARMTrafficManager]。  
...
最后一个必要步骤是将应用的自定义域指向流量管理器域。  对于示例应用，这意味着指向 `scalable-ase-demo.trafficmanager.net` 的 `www.scalableasedemo.com`。  此步骤必须以管理自定义域的域注册机构来完成。  
...
流量管理器及 DNS 配置的最终结果是 `www.scalableasedemo.com` 的请求经历以下顺序的流程：
