...
应用服务环境是横向横向横向横向扩展的理想平台。 选择支持已知请求速率的应用服务环境配置后，开发人员可以以"cookie cutter"方式部署其他应用服务环境，以达到所需的峰值负载容量。
...
* **应用的命名约定：** 由于将部署多个应用实例，每个部署的应用实例都要有名称。  有一项鲜为人知、但非常方便的应用服务环境功能，是多个应用服务环境可以使用相同的应用名称。  由于每个应用服务环境都有唯一的域后缀，开发人员可以选择在每个环境中重复使用完全相同的应用名称。  例如，开发人员可以指定如下应用：myapp.foo1.p.azurewebsites.net、myapp.foo2.p.azurewebsites.net、myapp.foo3.p.azurewebsites.net*myapp.foo2.p.azurewebsites.net*等。 *myapp.foo3.p.azurewebsites.net* *myapp.foo1.p.azurewebsites.net* 对于示例应用，尽管每个应用实例都有唯一的名称。  所用的应用实例名称是 *webfrontend1*、*webfrontend2* 和 *webfrontend3*。
