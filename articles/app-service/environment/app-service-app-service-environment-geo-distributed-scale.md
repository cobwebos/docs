...
* **应用的自定义域：** 客户访问应用时使用的自定义域名是什么？  对于示例应用，自定义域名 `www.scalableasedemo.com`
...
最后一个必要步骤是将应用的自定义域指向流量管理器域。  对于示例应用程序，这意味着指向 `scalable-ase-demo.trafficmanager.net``www.scalableasedemo.com`。  此步骤必须以管理自定义域的域注册机构来完成。  
...
在此示例中，自定义域是 `www.scalableasedemo.com`的，并且每个应用程序实例都具有与之关联的自定义域。
...
流量管理器和 DNS 配置的最终结果是 `www.scalableasedemo.com` 的请求将按以下顺序进行：
