---
title: 配置 ASE v1
description: 应用服务环境 v1 的配置、管理和监视。 此文档仅提供给使用旧版 v1 ASE 的客户。
author: ccompy
...
**设置** > **属性**： 当您打开 ASE 边栏选项卡时，"**设置"** 边栏选项卡会自动打开。 位于顶部的是“属性”****。 此处显示了许多与“基本信息”**** 重复的项，但“虚拟 IP 地址”**** 和“出站 IP 地址”**** 是非常有用的项。

![“设置”边栏选项卡和“属性”][4]

**设置** > **IP 地址**：当您在 ASE 中创建 IP 安全套接字层 （SSL） 应用时，您需要一个 IP SSL 地址。 要获取该地址，ASE 需要拥有一些可分配的 IP SSL 地址。 创建的 ASE 最初有一个 IP SSL 地址用于此目的，但可以添加更多地址。 额外的 IP SSL 地址需要付费，如[应用服务定价][AppServicePricing]（在有关 SSL 连接的一节中）所述。 额外的价格是 IP SSL 价格。

**设置** > **前端池** / **辅助池**：除了提供控件以完全扩展该资源池外，每个资源池边栏选项卡都提供了仅在该资源池上查看信息的功能。  
...
要在特定资源池中使用手动或自动缩放功能，请访问 **"设置** > **前端池** / **辅助工作池**"（视情况而定）。 然后打开要更改的池。 转到 **"设置** > **横向扩展**"或 **"向上缩放****设置** > "。 通过“扩大”**** 边栏选项卡可控制实例数量。 使用“增加”**** 可控制资源大小。  
...
要开始使用应用服务环境，请参阅[如何创建应用服务环境](app-service-web-how-to-create-an-app-service-environment.md)。
