...
应用服务环境是[Azure App Service](../overview.md)的[高级][PremiumTier]服务计划选项, 可提供完全隔离和专用的环境, 以便大规模安全运行 Azure App Service 应用, 包括 Web 应用、移动应用和 API 应用。  
...
每个工作线程池包含分配给[应用服务计划][AppServicePlan]的计算资源, 而这些资源又包含一个或多个 Azure App Service 应用。  由于应用服务环境中可能有多达三个不同的工作线程池，因此可以灵活地为每个工作线程池选择不同的计算资源。  
...
有关应用服务环境中支持的可用计算资源大小的详细信息, 请参阅[应用服务定价][AppServicePricing]页, 并查看高级定价层中应用服务环境的可用选项。

## <a name="virtual-network-support"></a>虚拟网络支持

可以在 Azure 资源管理器虚拟网络 **或**经典部署模型虚拟网络中创建应用服务环境 ([有关虚拟网络的详细信息][MoreInfoOnVirtualNetworks])。  由于应用服务环境始终位于虚拟网络中（更准确地说，是位于虚拟网络的子网中），因此可以利用虚拟网络的安全功能来控制入站和出站网络通信。  
...
应用还经常需要访问公司资源，例如内部数据库和 Web 服务。  常见的做法是让这些终结点仅可用于在 Azure 虚拟网络中流动的内部网络流量。  将应用服务环境加入到与内部服务相同的虚拟网络后, 在该环境中运行的应用可以访问这些服务, 包括可通过[站点到站点][SiteToSite]和[Azure ExpressRoute][ExpressRoute]连接访问的终结点。

有关应用服务环境如何使用虚拟网络和本地网络的更多详细信息, 请参阅以下文章:[网络体系结构][NetworkArchitectureOverview]、[控制入站流量][ControllingInboundTraffic]和[安全连接到后端][SecurelyConnectingToBackends]。 
...
有关将应用服务环境与 ExpressRoute 结合使用的详细信息, 请参阅以下文章: [Express Route 和应用服务环境][NetworkConfigDetailsForExpressRoute]。
