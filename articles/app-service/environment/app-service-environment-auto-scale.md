...
Frank 是企业的一位 sysadmin，他已将所管理的部分工作负荷迁移到应用服务环境。
...
Frank 非常熟悉该应用程序。 他们知道，负载高峰时段介于 9:00 AM 和 6:00 PM 之间，因为这是员工在办公室内使用的业务线（LOB）应用程序。 高峰时间过后，使用量会下降，此时用户完成了当天的工作。 高峰时间之外仍会有一些负载，因为用户可以使用其移动设备或家用电脑远程访问该应用。 生产型应用服务计划已配置为按以下规则根据 CPU 使用率自动缩放：
...
根据生产型应用服务计划的工作日配置文件的“自动缩放 - 纵向缩减”规则，该计算如下所示：

![基于“自动缩放 - 纵向缩减”规则的工作日应用服务计划膨胀率。][Equation3]

根据生产型应用服务计划的周末配置文件的“自动缩放 - 纵向缩减”规则，该计算公式解释为：  

![基于“自动缩放 - 纵向缩减”规则的周末应用服务计划膨胀率。][Equation4]
...
计数减少幅度可调整为纵向缩减时应用服务计划膨胀率的 1/2 倍到 1 倍之间。
