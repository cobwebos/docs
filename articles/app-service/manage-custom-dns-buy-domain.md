...
> [!NOTE]
> 自定义域的 "**不安全**" 标签意味着尚未绑定到 SSL 证书, 从浏览器到自定义域的任何 HTTPS 请求都将收到错误或警告, 具体取决于浏览器。 若要配置 SSL 绑定, 请参阅[为 Azure App Service 购买并配置 ssl 证书](web-sites-purchase-ssl-web-site.md)。
>
...
* 如果禁用自动续订:90天, 在域过期之前, 会向你发送续订通知电子邮件, 并在门户中激活 "**续订域**" 按钮。
