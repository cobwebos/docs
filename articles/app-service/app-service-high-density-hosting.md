...
1. 在应用服务计划中将按站点缩放标志设置为 true。

1. 将 **numberOfWorkers** 属性设置为 **1**，创建新站点并将其分配给该应用服务计划。这样会在该辅助池中产生可能的最高密度。

1. 可以按站点独立配置辅助角色数，根据需要授予其他资源。例如，使用率高的站点可通过将 **numberOfWorkers** 设置为 **3** 来提高该应用的处理能力，使用率低的站点则可将 **numberOfWorkers** 设置为 **1**。
