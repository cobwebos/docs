...
如果没有现成的 Web 应用，可从 [Azure 门户](https://portal.azure.com)中创建一个。 在左上角，单击“创建资源” > “Web + 移动” > “Web 应用”。
...
当 Azure 在存储库的根目录中检测到 requirements.txt 文件时，会自动创建名为 `env` 的虚拟环境。 仅第一次部署会执行此操作，也可以在所选的 Python 运行时发生更改后进行任何部署的过程中执行此操作。
...
在上述示例中，磁盘上静态文件的位置应与 URL 中的位置匹配。 也就是说，`http://pythonapp.azurewebsites.net/static/site.css` 的请求将为磁盘上 `\static\site.css` 处的文件服务。
