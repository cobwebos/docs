...
通过 Visual Studio，用户可以访问 [Azure 门户](https://go.microsoft.com/fwlink/?LinkId=529715)中提供的一部分应用管理功能和配置设置。 在本节中，您将看到使用**服务器资源管理器**可用的内容。 若要了解最新的 Azure 集成功能，请同时试用**云资源管理器**。 可以从“视图”**** 菜单打开这两个窗口。
...
3. 展开包含在[在 Azure 应用服务中创建 ASP.NET 应用](app-service-web-get-started-dotnet-framework.md)中创建的应用的资源组，右键单击该应用节点，并单击“查看设置”****。
    ...
    有关此窗口中的应用设置和连接字符串框的信息，请参阅 Azure[应用服务：应用程序字符串和连接字符串的工作原理](https://azure.microsoft.com/blog/windows-azure-web-sites-how-application-strings-and-connection-strings-work/)。
...
1. 在**解决方案资源管理器**中，右键单击项目，然后单击 **"发布**"。
...
1. 在“服务器资源管理器”中，右键单击应用，并单击“附加调试器”********。
    ...
    浏览器会自动打开运行在 Azure 中的主页。 可能需要等待大约 20 秒，以便 Azure 针对调试设置服务器。 此延迟只在 48 小时内首次于应用上运行调试模式时出现。 如果在同一时段再次开始调试，不会发生延迟。
...
## <a name="remote-debugging-webjobs"></a><a name="remotedebugwj"></a> 远程调试 WebJobs
...
1. 打开*控制器\HomeController.cs*，并将`Index`和`About`替换`Contact`， 和 方法替换为以下代码`Trace`，以便添加`using`语句`System.Diagnostics`和 语句：
...
1. 按 Ctrl+F5 运行应用程序。
1. 在浏览器窗口的地址栏中，向 URL 添加*trace.axd，* 然后按 Enter（URL 类似于`http://localhost:53370/trace.axd`）。
...
7. 现在，在显示“联系人”**** 页面的浏览器窗口中，单击“主页”****，并依次单击“关于”**** 和“联系人”****。
    ...
    在本部分中，已通过使用应用设置启用和禁用日志记录。 此外，还可以通过修改 Web.config 文件启用和禁用跟踪侦听器。 然而，修改 Web.config 文件会导致应用域回收，而通过应用配置启用日志记录则不会出此情况。 如果问题重现需要花费较长时间或呈间歇性，回收应用域可能会“修复”该问题并强迫你一直等到其再次出现。 通过启用 Azure 中的诊断，可以立即开始捕获错误信息，并且不需要回收应用域。
...
4. 在 [Azure 门户中应用的“设置”页面上，单击“部署凭据”，并输入新的用户名和密码](https://portal.azure.com) ********。
...
* [微软 Q 上的 Azure 论坛&A](https://docs.microsoft.com/answers/topics/azure-webapps.html)。
...
Microsoft TechNet 网站包含["使用失败的请求跟踪](https://www.iis.net/learn/troubleshoot/using-failed-request-tracing)"部分，该部分可能有助于了解如何使用这些日志。 然而，该文档主要着重于在 IIS 中配置失败请求跟踪，并不适用于 Azure 应用服务。
