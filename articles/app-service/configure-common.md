...
对于 ASP.NET 和 ASP.NET Core 开发人员而言，在应用服务中设置应用设置类似于在 Web.config`<appSettings>` *或 appsettings.json* *中的* 内进行设置，但应用服务中的值会替代 Web.config 或 appsettings.json 中的值。 可以在 Web.config 或 appsettings.json 中保留开发设置（例如，本地 MySQL 密码），但在应用服务中保留生产机密（例如 Azure MySQL 数据库密码）会更安全。 相同的代码在本地调试时使用开发设置，部署到 Azure 时使用生产机密。
...
> [!NOTE]
> 也可以使用 [Key Vault 引用](/azure/key-vault/)从 [Key Vault](app-service-key-vault-references.md) 解析应用设置。
...
对于 ASP.NET 和 ASP.NET Core 开发人员而言，在应用服务中设置连接字符串类似 `<connectionStrings>` 于在 web.config 中设置连接*字符串，但*在应用服务中设置的值会覆盖*web.config*中的连接字符串。您可以在应用服务中安全地保存*web.config*中的开发设置（例如，数据库文件）和生产机密（例如，SQL 数据库凭据）。 相同的代码在本地调试时使用开发设置，部署到 Azure 时使用生产机密。
...
例如，可以使用环境变量 *的形式访问名为*connectionstring1`MYSQLCONNSTR_connectionString1` 的 MySql 连接字符串。 有关特定语言堆栈的步骤，请参阅：
...
> [!NOTE]
> 也可以使用 [Key Vault 引用](/azure/key-vault/)从 [Key Vault](app-service-key-vault-references.md) 解析连接字符串。
...
- **平台设置**：允许你为托管平台配置设置，包括：
    ...
    - **Always On**：即使在没有流量的情况，也要加载应用。 对于连续性 WebJobs 或使用 CRON 表达式触发的 WebJobs，它是必需的。
    ...
    - **ARR 关联**：在多实例部署中，请确保将客户端路由到会话生存期内的同一实例。 对于无状态应用程序，请将此选项设置为“关闭”。
...
每个应用具有已映射到 `/`（代码的默认部署位置）的默认根路径 (`D:\home\site\wwwroot`)。 如果应用根位于其他文件夹中，或者存储库包含多个应用程序，则你可以在此处编辑或添加虚拟应用程序和目录。 单击“新建虚拟应用程序或目录”。
...
- [设置 Azure 应用服务中的过渡环境]
- [使用 Azure App Service 中的 SSL 绑定保护自定义 DNS 名称](configure-ssl-bindings.md)
