---
title: 在门户中配置应用
description: 了解如何在 Azure 门户中配置应用服务应用的常见设置。 应用设置、连接字符串、平台、语言堆栈、容器等。
...
在[Azure 门户]中，导航到应用的 "管理" 页。 在应用程序的左侧菜单中，单击 "**配置**" > **应用程序设置**。

![应用程序设置](./media/configure-common/open-ui.png)

对于 ASP.NET 和 ASP.NET Core 开发人员而言，在应用服务中设置应用设置类似*于在 web.config 或* *appsettings*中 `<appSettings>` 设置应用设置，但应用服务中的值*会替代 web.config 或* *appsettings*中的设置。 你可以在 web.config 或*appsettings*中保留开发设置（例如，本地 MySQL 密码），*但在应用*服务中，可以确保生产机密（例如，Azure MySQL 数据库密码）安全。 当你在本地调试时，相同的代码使用你的开发设置，并且在部署到 Azure 时，它将使用你的生产机密。
...
### <a name="show-hidden-values"></a>显示隐藏值

默认情况下，在门户中隐藏应用设置的值以获得安全。 若要查看应用设置的隐藏值，请单击该设置的**值**字段。 若要查看所有应用设置的值，请单击 "**显示值**" 按钮。
...
对于 ASP.NET 和 ASP.NET Core 开发人员而言，在应用服务中设置连接字符串类似 `<connectionStrings>` 于在 web.config 中设置连接*字符串，但*在应用服务中设置的值会覆盖*web.config*中的连接字符串。您可以在应用服务中安全地保存*web.config*中的开发设置（例如，数据库文件）和生产机密（例如，SQL 数据库凭据）。 当你在本地调试时，相同的代码使用你的开发设置，并且在部署到 Azure 时，它将使用你的生产机密。

对于其他语言堆栈，更好的做法是使用[应用设置](#configure-app-settings)，因为连接字符串需要可变键中的特殊格式，以便访问这些值。 但这是一个例外，不过，如果你在应用程序中配置了某些 Azure 数据库类型的连接字符串，则会将该应用与应用一起备份。 有关详细信息，请参阅[备份的内容](manage-backup.md#what-gets-backed-up)。 如果不需要此自动备份，请使用应用设置。

在运行时，连接字符串作为环境变量提供，以下列连接类型作为前缀：
...
默认情况下，在门户中隐藏连接字符串的值以获得安全。 若要查看连接字符串的隐藏值，只需单击该字符串的**值**字段。 若要查看所有连接字符串的值，请单击 "**显示值**" 按钮。
...
在此处，你可以为应用程序配置一些常见设置。 某些设置要求你[纵向扩展到更高的定价层](manage-scale-up.md)。

- **堆栈设置**：运行应用的软件堆栈，包括语言和 SDK 版本。 对于 Linux 应用和自定义容器应用，还可以设置可选的启动命令或文件。
...
- **调试**：启用[ASP.NET](troubleshoot-dotnet-visual-studio.md#remotedebug)、 [ASP.NET Core](/visualstudio/debugger/remote-debugging-azure)或 node.js 应用的远程[调试。](containers/configure-language-nodejs.md#debug-remotely) 此选项将在48小时后自动关闭。
...
在[Azure 门户]中，导航到应用的 "管理" 页。 在应用程序的左侧菜单中，单击 "**配置** > **默认文档**"。

![常规设置](./media/configure-common/open-documents.png)

默认文档是网站的根 URL 中显示的网页。 使用的是列表中的第一个匹配文件。 若要添加新的默认文档，请单击 "**新建文档**"。 别忘了单击 "**保存**"。

如果应用使用根据 URL 路由的模块，而不是提供静态内容，则无需默认文档。

## <a name="configure-path-mappings"></a>配置路径映射

在[Azure 门户]中，导航到应用的 "管理" 页。 在应用程序的左侧菜单中，单击 "**配置**" > **路径映射**"。
...
对于 Windows 应用，你可以自定义 IIS 处理程序映射和虚拟应用程序和目录。

处理程序映射允许添加自定义脚本处理器，以处理特定文件扩展名的请求。 若要添加自定义处理程序，请单击 "**新建处理程序**"。 配置处理程序，如下所示：

- **扩展名**。 要处理的文件扩展名，例如 *\*php*或*handler.fcgi*。
- **脚本处理器**。 脚本处理器的绝对路径。 与文件扩展名匹配的文件请求由脚本处理器处理。 使用路径 `D:\home\site\wwwroot` 表示应用的根目录。
- **参数**。 脚本处理器的可选命令行参数。

每个应用都具有映射到 `D:\home\site\wwwroot`的默认根路径（`/`），默认情况下会部署代码。 如果你的应用程序根目录位于其他文件夹中，或者如果你的存储库有多个应用程序，则可以在此处编辑或添加虚拟应用程序和目录。 单击 "**新建虚拟应用程序或目录**"。
