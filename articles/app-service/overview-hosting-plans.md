...
应用服务计划的定价层确定了所提供的应用服务功能和计划费用。__ 定价层有以下几个类别：
...
- **隔离**：此层在专用 Azure 虚拟网络上运行专用 Azure VM。 它在计算隔离的基础上提供与应用的网络隔离。 此层提供最大的横向扩展能力。
...
在 **"免费**"和 **"共享**"层中，应用在共享 VM 实例上接收 CPU 分钟数，并且无法横向扩展。在其他层中，应用运行并缩放如下。
...
- 在“共享”层中，每个应用遵循 CPU 分钟配额，因此，每个应用会根据 CPU 配额产生小时费用。****__
