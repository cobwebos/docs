...
在 ASEv2 中，有一个称为“隔离”的只适用于 ASEv2 的新定价 SKU。 ASEv2 中托管的所有应用服务计划都在“隔离”定价 SKU 中。 除了应用服务计划的价格以外，ASE 本身也会收取一笔统一的费用。 此价格不会根据 ASE 的大小改变。 

其他潜在费用与调整前端缩放比率或前端大小相关。 可以调整缩放比率以便更快地添加前端。 但是，需要支付所有不会自动添加到系统中的附加核心的费用。 同样，如果为前端选择更大的大小，则需要支付所有不会自动分配的核心的费用。 例如，如果将缩放比率调整到 10，则会为应用服务计划中的每 10 个实例添加一个前端。 该统一费用涵盖针对每 15 个实例添加 1 个前端的缩放比率。 如果缩放比率为 10，则需要支付针对 10 个应用服务计划实例添加的第三个前端的费用。 如果达到了 15 个实例，则不需要支付该费用，因为前端会自动添加。
...
2. 输入 ASE 的名称，确认想要将它删除。 删除 ASE 时，会同时删除它包含的所有内容。 
