...
SCM 站点名称能将用户带到 Kudu 控制台，在 Azure 门户中称为**高级门户**。 Kudu 控制台允许查看环境变量、浏览磁盘、使用控制台等等。 有关详细信息，请参阅[用于 Azure App Service 的 Kudu 控制台][Kudu]。 

在多租户应用服务和外部 ASE 中，Azure 门户和 Kudu 控制台之间有单一登录。 然而，对于 ILB ASE，需要使用发布凭据登录到 Kudu 控制台。

基于 Internet 的 CI 系统（比如 GitHub 和 Visual Studio Team Services）不支持 ILB ASE，因为发布终结点不可访问 Internet。 需要该用使用拉取模式的 CI 系统，如 Dropbox。
