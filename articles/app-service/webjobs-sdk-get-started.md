...
1. 安装 `Microsoft.Azure.WebJobs.Extensions` NuGet 包的最新稳定版1.x 版本，其中包括 `Microsoft.Azure.WebJobs`。
...
宿主是侦听触发器和调用函数的函数的运行时容器。 以下步骤创建一个实现[`IHost`](/dotnet/api/microsoft.extensions.hosting.ihost)的宿主，该主机是 ASP.NET Core 中的泛型主机。
...
1. 右键单击该项目，选择 "**添加** > **新项 ...** "，选择 "**类**"，将新C#的类文件命名为*Functions.cs*，然后选择 "**添加**"。
...
1. 在新文件中，添加一个 @no__t 字段，如以下示例中所示：
...
* 将[server.loggerfactory](./webjobs-sdk-how-to.md#logging-and-monitoring)对象置于 @no__t 1 块中，以确保在主机退出时刷新日志输出。
