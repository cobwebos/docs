...
如果应用服务计划设置为**基本** SKU 或更高级别，可以控制 VM 的**大小**和规模计数。
...
创建应用时，应考虑创建资源组。 另一方面，如果此应用是某一大型应用程序的组件，则在分配给该大型应用程序的资源组内创建该应用。
...
“更改应用服务计划”将打开“应用服务计划”选择器。 此时，可以选取要移入此应用的现有计划。

> [!IMPORTANT]
> 根据下列条件筛选挑选出的应用服务计划 UI：
> - 存在于同一资源组内
> - 存在于同一地理区域中
> - 存在于同一 Web 空间里
>
> Web 空间是应用服务中的逻辑构造，用于定义服务器资源的分组。 地理区域（例如美国西部）包含诸多 Web 空间，旨在通过应用服务分配客户。 当前无法在 Web 空间之间移动应用服务资源。
>
