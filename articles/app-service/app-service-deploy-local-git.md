...
3. 如果这是第一次在 Azure 中设置存储库，则需要为其创建登录凭据。 将使用它们从本地 Git 存储库登录到 Azure 的存储库和推送更改。 在应用的边栏选项卡中，单击“部署”>“部署凭据”，并配置部署用户名和密码。 完成后，单击“保存”。
...
1. 在 Azure 门户的应用边栏选项卡中，单击“Git URL”的“设置”>“属性”。
    ...
    **Git URL** 是从本地存储库到部署的远程引用。 在后续步骤中，将要用到此 URL。
...
- - -
**症状**：无通用引用且未指定任何引用；不采取任何措施。 或许应指定一个分支，例如“master”。
...
[Azure 命令行接口]: https://azure.microsoft.com/en-us/documentation/articles/xplat-cli-azure-resource-manager/
