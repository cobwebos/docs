...
1. 一种版本支持与在同一区域中的 Vnet 集成。 这种形式的该功能需要在同一区域中的 VNet 的子网
2. 另一个版本可与其他区域中的 Vnet 或经典 Vnet 集成。 此版本的功能需要虚拟网络网关部署到 VNet。
...
* 应用和 VNet 必须位于同一区域中
* 一个地址用于每个应用服务计划实例。 由于子网大小在分配后不能更改，因此在使用子网时，其大小应该超出你的最大规模大小。 建议的大小是包含 32 个地址的 /27，因为它所容纳的应用服务计划可以扩大到 20 个实例。
* 不能删除包含集成的应用的 VNet。 您必须首先删除集成 

若要使用的同一区域中的资源管理器 VNet 使用 VNet 集成功能：
...
该功能仅支持每个辅助角色的一个虚拟接口。  每个辅助角色的一个虚拟接口意味着每个应用服务计划的一个虚拟接口。 所有相同的应用服务计划中的应用可以使用相同的 VNet 集成，但如果需要连接到其他 VNet，你将需要创建另一个应用服务计划。 使用的虚拟接口不是客户可以直接访问的资源。
...
**路由** 在 VNet 中定义的路由，用于将流量从应用导入 VNet。 如果需要将其他出站流量发送到 VNet 中，则可以在此处添加地址块。 此 capabilty 只适用于网关需要 VNet 集成。
