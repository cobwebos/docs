...
ms.locfileid: "67795923"
---
# <a name="integrate-your-app-with-an-azure-virtual-network"></a>将应用与 Azure 虚拟网络进行集成
本文档介绍 Azure 应用服务虚拟网络集成功能以及如何将它设置为中的应用[Azure 应用服务](https://go.microsoft.com/fwlink/?LinkId=529714)。 [Azure 虚拟网络][VNETOverview](Vnet) 允许你在非 internet 可路由网络中放置多个 Azure 资源。  
...
本文档将完成两个 VNet 集成功能，即在多租户应用服务中使用。 如果您的应用程序处于[应用服务环境][ASEintro]，然后，它已在 VNet 中并不需要使用 VNet 集成功能来访问同一 VNet 中的资源。 对所有应用服务网络功能的信息，请参阅[网络功能的应用服务](networking-features.md)

有两个窗体到 VNet 集成功能

1. 一种版本支持与在同一区域中的 Vnet 集成。 这种形式的功能要求在同一区域中的 VNet 的子网。 此功能仍处于预览阶段，但存在一些注意事项如下所示支持适用于 Windows 应用生产工作负荷。
2. 另一个版本可与其他区域中的 Vnet 或经典 Vnet 集成。 此版本的功能需要虚拟网络网关部署到 VNet。 这是点到站点基于 VPN 的功能，仅支持与 Windows 应用程序。
...
| 想要访问经典 VNet 或另一个区域中的 VNet 中的资源 | 网关所需的 VNet 集成 |
...
* 此功能要求未使用/27 使用 32 个地址或以上资源管理器 VNet 中子网
* 应用和 VNet 必须位于同一区域中
* 不能删除带有集成应用的 VNet。 您必须首先删除集成 
* 您可以每个应用服务计划中只有一个区域的 VNet 集成。 在同一应用服务计划中的多个应用程序可以使用相同的 VNet。 

为每个应用服务计划实例使用一个地址。 如果缩放成 5 个实例应用程序，则使用 5 个地址。 由于分配后，不能更改子网大小，因此必须使用足够大以容纳您的应用程序可能会达到任何规模的子网。 使用 32 个地址/27 是建议的大小，可以容纳的高级应用服务计划缩放到 20 个实例。
...
启用 VNet 集成后，您的应用程序仍将像平时一样的同一通道通过 internet 的出站调用。 应用属性门户中列出的出站地址仍是您的应用程序使用的地址。 对服务终结点保护的服务或 RFC 1918 地址的调用更改为你的应用是什么，将进入你的 VNet。 
...
1. [创建网关子网][creategatewaysubnet]VNet 中。  
...
* **同步网络**。 同步网络操作仅适用于依赖于网关的 VNet 集成功能。 执行同步网络操作可确保证书和网络信息保持同步。如果添加或更改 VNet 的 DNS，则需执行“同步网络”操作  。 此操作会重启所有使用此 VNet 的应用。
...
> [!NOTE]
> 网关需要 VNet 集成功能不会将应用与包含 ExpressRoute 网关的 VNet。 即使 ExpressRoute 网关配置中[共存模式][VPNERCoex] the VNet Integration doesn't work. If you need to access resources through an ExpressRoute connection, then you can use the regional VNet Integration feature or an [App Service Environment][ASE]，在 VNet 中运行。 
> 
> 
...
* 数据传输费用 - 数据传出会产生费用，即使 VNet 在同一数据中心也是如此。 中介绍了这些费用[数据传输定价详细信息][DataPricing]。 
* VPN 网关费用 - 点到站点 VPN 所需的 VNet 网关会产生费用。 详细信息位于[VPN 网关定价][VNETPricing]页。
