...
description: 在 Azure 应用服务中将应用与 Azure 虚拟网络集成。
author: ccompy
...
ms.locfileid: "84170777"
---
# <a name="integrate-your-app-with-an-azure-virtual-network"></a>将应用与 Azure 虚拟网络集成

本文介绍 Azure 应用服务 VNet 集成功能，并叙述如何对 [Azure 应用服务](https://go.microsoft.com/fwlink/?LinkId=529714)中的应用设置该功能。 使用 [Azure 虚拟网络][VNETOverview] (VNet) 可将多个 Azure 资源置于无法通过 Internet 路由的网络中。
...
> [!NOTE]
> 如果在 Linux 应用的菜单中，“网络”边栏选项卡处于禁用状态（灰显），表示该功能当前不可用。
>

1. 转到应用服务门户中的“网络”UI。 在“VNet 集成”下选择“单击此处可配置” 。
...
1. 下拉列表包含订阅内位于相同区域中的所有 Azure 资源管理器虚拟网络。 下面是所有其他区域中资源管理器虚拟网络的列表。 选择要集成的 VNet。
   ...
   * 若要与经典 VNet 集成，请选择“单击此处连接到经典 VNet”，而不是“虚拟网络”下拉列表 。 选择需要的经典虚拟网络。 目标 VNet 必须已预配一个启用了点到站点连接的虚拟网络网关。
...
需要网关的 VNet 集成支持连接到其他区域中的 VNet 或连接到经典虚拟网络。 需要网关的 VNet 集成：

* 使一个应用一次只能连接一个 VNet。
* 允许在应用服务计划中最多集成 5 个 VNet。
* 允许在应用服务计划中由多个应用使用同一个 VNet，而不影响应用服务计划可使用的总数。 如果在同一个应用服务计划中有六个应用使用相同的 VNet，计为使用一个 VNet。
* 由于网关上的 SLA，可实现 99.9% 的 SLA。
* 使应用能使用配置 VNet 所用的 DNS。
* 需要一个配置了 SSTP 点到站点 VPN 的基于虚拟网络路由的网关，然后才能连接到应用。

不能将需要网关的 VNet 集成用于：

* Linux 应用。
* 与 Azure ExpressRoute 连接的 VNet。
* 访问服务终结点保护的资源。
* 支持 ExpressRoute 以及点到站点/站点到站点 VPN 的共存网关。
...
如果是创建用于应用服务 VNet 集成的网关，不需要上传证书。 创建网关可能需要 30 分钟。 要将应用与 VNet 集成，必须先预配网关。

### <a name="how-gateway-required-vnet-integration-works"></a>需要网关的 VNet 集成的工作原理

需要网关的 VNet 集成基于点到站点 VPN 技术。 点到站点 VPN 将网络访问限制为访问托管应用的虚拟机。 应用受到限制，只能通过混合连接或 VNet 集成仅将流量外发到 Internet。 在门户中将应用配置为使用需要网关的 VNet 集成时，系统代你管理一个复杂的协商，用于在网关和应用程序端创建并分配证书。 于是，用于托管应用的辅助角色能直接连接到所选 VNet 中的虚拟网络网关。

![需要网关的 VNet 集成的工作原理][6]

### <a name="access-on-premises-resources"></a>访问本地资源

应用可以通过与具备站点到站点连接的 VNet 集成来访问本地资源。 如果使用需要网关的 VNet 集成，请使用点到站点地址块更新本地 VPN 网关路由。 先设置站点到站点 VPN，接着应通过用于配置该 VPN 的脚本来正确地设置路由。 如果在创建站点到站点地址后才添加点到站点 VPN，则需手动更新路由。 具体操作信息取决于每个网关，在此不作说明。 你不能使用站点到站点 VPN 连接配置 BGP。
...
1. 在应用所连接的 VNet 上添加对等互连连接。 在添加对等互连连接时，启用“允许虚拟网络访问”并选择“允许转发流量”和“允许网关传输”  。
1. 在与连接的 VNet 进行对等互连的 VNet 上添加对等互连连接。 在目标 VNet 上添加对等互连连接时，启用“允许虚拟网络访问”并选择“允许转发流量”和“允许远程网关”  。
...
与 VNet 连接和断开连接都在应用级别进行的。 可能影响多个应用的 VNet 集成的操作是在应用服务计划级别执行的。 从应用 >“网络” > “VNet 集成”门户，可以获取有关 VNet 的详细信息 。 在“应用服务计划” > “网络” > “VNet 集成”中，可以看到应用服务计划级别的类似信息  。

在 VNet 集成实例的应用视图中，能够执行的唯一操作是断开应用与当前所连接到的 VNet 的连接。 若要断开应用与 VNet 的连接，请选择“断开连接”。 断开与 VNet 的连接后，应用会重启。 断开连接操作不会更改 VNet。 子网或网关不会被删除。 如果那时想删除 VNet，请先将断开应用与 VNet 的连接并删除其中的资源，例如网关。

应用服务计划 VNet 集成 UI 显示应用服务计划中的应用使用的所有 VNet 集成。 要查看每个 VNet 的详细信息，请选择感兴趣的 VNet。 此时可为需要网关的 VNet 集成执行两项操作：

* **同步网络**：同步网络操作仅用于依赖网关的 VNet 集成功能。 执行同步网络操作确保了证书与网络信息是同步的。如果添加或更改 VNet 的 DNS，请执行同步网络操作。 此操作重启使用此 VNet 的任何应用。 如果你使用的是属于不同订阅的应用和 VNet，此操作无效。
...
如果证书或网络信息发生了更改，请选择“同步网络”。 选择“同步网络”会导致应用与 VNet 之间的连接出现短暂的中断。 虽然应用不会重启，但失去连接会导致站点功能失常。
...
* **应用服务计划定价层费用**：应用需要属于“标准”、“高级”或“高级 V2”应用服务计划。 有关这些费用的详细信息，请参阅[应用服务定价][ASPricing]。
* **数据传输费用**：存在数据流出量费用，即使 VNet 在同一数据中心也是如此。 [数据传输定价详细信息][DataPricing]中对这些费用进行了说明。
* **VPN 网关费用**：点到站点 VPN 所需的虚拟网络网关会产生费用。 有关详细信息，请参阅 [VPN 网关定价][VNETPricing]。
...
对于需要网关的 VNet 集成，你可以使用 PowerShell 将应用服务与 Azure 虚拟网络集成。 对于准备好运行的脚本，请参阅[将 Azure 应用服务中的应用连接到 Azure 虚拟网络](https://gallery.technet.microsoft.com/scriptcenter/Connect-an-app-in-Azure-ab7527e3)。
