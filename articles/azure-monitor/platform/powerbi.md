...
首先创建一个[日志搜索](../../azure-monitor/log-query/log-query-overview.md)，其从 Log Analytics 返回要填充 Power BI 数据集的数据。  随后将该查询导出为 Power BI Desktop 可以使用的 [Power Query (M) 语言](https://msdn.microsoft.com/library/mt807488.aspx)。
...
2. 如果你正在使用日志搜索门户，请单击“Power BI”。  如果你正在使用 Analytics 门户，请选择“导出” > “Power BI Query (M)”。  这两种选项都会将查询导出到一个名为“PowerBIQuery.txt”的文本文件。 
    ...
    ![导出日志搜索](media/powerbi/export-logsearch.png) ![导出日志搜索](media/powerbi/export-analytics.png)
...
* 了解有关[日志搜索](../../azure-monitor/log-query/log-query-overview.md)的信息以生成可导出到 Power BI 的查询。
