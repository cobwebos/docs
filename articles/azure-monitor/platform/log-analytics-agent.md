...
> [!NOTE]
> 您可能还会看到日志分析代理称为 Microsoft 监视代理 （MMA） 或 OMS Linux 代理。
...
Azure 监视器中的[Azure 诊断扩展](diagnostics-extension-overview.md)还可用于从 Azure 虚拟机的来宾操作系统收集监视数据。 根据您的要求，您可以选择使用任一或两者。 有关 Azure 监视器代理的详细比较，请参阅[Azure 监视器代理的概述](agents-overview.md)。 
...
## <a name="costs"></a>成本
日志分析代理不收取任何费用，但可能会对引入的数据产生费用。 [使用 Azure 监视器日志检查管理使用情况和成本](manage-cost-storage.md)，了解日志分析工作区中收集的数据定价的详细信息。
...
| [自定义日志](data-sources-custom-logs.md)           | 来自 Windows 和 Linux 计算机上的文本文件的事件。 |

## <a name="data-destinations"></a>数据目标
日志分析代理将数据发送到 Azure 监视器中的日志分析工作区。 Windows 代理可以多居化，将数据发送到多个工作区和系统中心操作管理器管理组。 Linux 代理只能发送到单个目标。

## <a name="other-services"></a>其他服务
Linux 和 Windows 的代理不仅用于连接到 Azure 监视器，它还支持 Azure 自动化以承载混合 Runbook 辅助角色和其他服务，如[更改跟踪](../../automation/change-tracking.md)、[更新管理和](../../automation/automation-update-management.md) [Azure 安全中心](../../security-center/security-center-intro.md)。 有关混合 Runbook 辅助角色的详细信息，请参阅 [Azure 自动化混合 Runbook 辅助角色](../../automation/automation-hybrid-runbook-worker.md)。  
...
* Windows 代理支持[FIPS 140 标准](https://docs.microsoft.com/windows/security/threat-protection/fips-140-validation)，而 Linux 代理不支持它。  

如果使用 System Center Operations Manager 2012 R2 或更高版本：

* 每个操作管理器管理组只能[连接到一个工作区](om-agents.md)。
...
| | [用于 VM 的 Azure Monitor](../insights/vminsights-enable-overview.md) | 使用 Azure 监视器为 VM 启用监视时，它将安装日志分析扩展和代理。 |
| | [Azure 安全中心自动预配](../../security-center/security-center-enable-data-collection.md) | Azure 安全中心可以在所有受支持的 Azure VM 上预配日志分析代理，以及启用它监视安全漏洞和威胁时创建的任何新代理。 如果启用，将预配任何没有已安装代理的新或现有 VM。 |
| 混合 Windows 计算机| [手动安装](agent-windows.md) | 从命令行安装 Microsoft 监视代理。 |
| | [Azure 自动化 DSC](agent-windows.md#install-the-agent-using-dsc-in-azure-automation) | 使用 Azure 自动化 DSC 自动安装。 |
| | [具有 Azure Stack 的资源管理器模板](https://github.com/Azure/AzureStack-QuickStart-Templates/tree/master/MicrosoftMonitoringAgent-ext-win) | 如果在数据中心中部署了 Microsoft Azure 堆栈，请使用 Azure 资源管理器模板。| 
| 混合 Linux 计算机| [手动安装](../../azure-monitor/learn/quick-collect-linux-computer.md)|调用 GitHub 上托管的包装器脚本安装 Linux 代理。 | 
| System Center Operations Manager|[将 Operations Manager 与 Log Analytics 集成](../../azure-monitor/platform/om-agents.md) | 配置操作管理器和 Azure 监视器日志之间的集成，以便将从报告到管理组的 Windows 计算机收集的数据转发。|  
...
* 仅支持服务器版本，不支持客户端版本。  
* 对任何[Azure Linux 认可的发行版](../../virtual-machines/linux/endorsed-distros.md)进行重点支持。 请注意，在新的发行版/版本是 Azure Linux 支持版本和日志分析 Linux 代理支持它之间可能存在一些延迟。
* 列出的每个主版本支持所有的次版本。
* 超出制造商终止支持日期的版本不受支持。  
* 不支持新版本的 AMI。  
* 默认仅支持运行 SSL 1.x 的版本。
...
为了确保传输到 Azure 监视器日志的数据的安全性，我们强烈建议您配置代理至少使用传输层安全 （TLS） 1.2。 我们发现旧版 TLS/安全套接字层 (SSL) 容易受到攻击，尽管目前出于向后兼容，这些协议仍可正常工作，但我们**不建议使用**。  有关其他信息，请查看[使用 TLS 1.2 安全地发送数据](data-security.md#sending-data-securely-using-tls-12)。 


## <a name="sha-2-code-signing-support-requirement-for-windows"></a>SHA-2 Windows 的代码签名支持要求
Windows 代理将于 2020 年 5 月 18 日开始专门使用 SHA-2 签名。 此更改将影响客户使用旧版操作系统上的日志分析代理作为任何 Azure 服务的一部分（Azure 监视器、Azure 自动化、Azure 更新管理、Azure 更改跟踪、Azure 安全中心、Azure 哨兵、Windows Defender ATP）。 除非您在旧版操作系统版本（Windows 7、Windows Server 2008 R2 和 Windows Server 2008）上运行代理，否则更改不需要任何客户操作。 在 2020 年 5 月 18 日之前，运行在旧版操作系统版本的客户必须在其计算机上执行以下操作，否则其代理将停止将数据发送到其日志分析工作区：

1. 为您的操作系统安装最新的服务包。 所需的服务包版本包括：
    - Windows 7 SP1
    - Windows Server 2008 SP2
    - Windows Server 2008 R2 SP1

2. 安装 SHA-2 签名 Windows 更新，如[2019 年 SHA-2 代码签名支持 Windows 和 WSUS](https://support.microsoft.com/help/4472027/2019-sha-2-code-signing-support-requirement-for-windows-and-wsus)中所述
...
> [!NOTE]
> 如果在密码中使用特殊字符（如""），\@则会收到代理连接错误，因为值解析不正确。  若要解决此问题，请使用 [URLDecode](https://www.urldecoder.org/) 等工具在 URL 中对密码进行编码。  
