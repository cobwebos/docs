...
- **时间段**。  指定查询的时间范围。 查询仅返回在当前时间的这个范围内创建的记录。 时间段限制为日志查询提取的数据以防止滥用，并规避日志查询中使用的任何时间命令（如 ago）。 <br>*例如，如果时间段设置为 60 分钟，并且查询在 1：15 PM 运行，则仅返回在 12：15 PM 和 1：15 PM 之间创建的记录以执行日志查询。现在，如果日志查询使用时间命令（如之前 （7d），日志查询将仅运行 12：15 PM 和 1：15 PM 之间的数据 - 就像数据仅存在过去 60 分钟一样。而不是日志查询中指定的七天数据。*

- **频率**.  指定应运行查询的频率。 可以是介于 5 分钟到 24 小时之间的任何值。 应等于或小于时间段。  如果该值大于时间段，则会有记录缺失的风险。<br>*例如，考虑 30 分钟的时间段和 60 分钟的频率。 如果查询在 1：00 运行，它将返回 12：30 和 1：00 PM 之间的记录。 下次运行查询的时间是 2：00，此时它将返回 1：30 和 2：00 之间的记录。 在 1：00 和 1：30 之间创建的任何记录都不会被计算。*
...
某些情况下，可能需要在没有事件的情况下创建警报。  例如，进程可能记录常规事件以指明其运行正常。  如果它不在特定时间段内记录某个事件，则应创建警报。  在这种情况下，您将阈值设置为**小于 1**。
...
由于已将警报配置为超出阈值两次以上才触发，因此我们看到只有 srv02 和 srv03 符合此标准。 因此，将针对 srv02 和 srv03 创建单独的警报，因为它们在多个时间箱中两次超过 90% 的阈值。 如果*基于：参数的触发警报*配置为*连续违规*选项，则**仅**针对 srv03 触发警报，因为它连续三次从 1：10 到 1：20 超过阈值。 不会为 srv02 触发警报，因为它只在从 1:10 到 1:15 这个时间范围内连续两个时间段超出阈值。****
