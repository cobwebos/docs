...
ms.locfileid: "75751089"
---
# <a name="create-diagnostic-setting-to-collect-platform-logs-and-metrics-in-azure"></a>创建诊断设置以在 Azure 中收集平台日志和指标
Azure 中的[平台日志](platform-logs-overview.md)，包括 azure 活动日志和资源日志，提供 azure 资源及其所依赖的 azure 平台的详细诊断和审核信息。 本文详细介绍了如何创建和配置诊断设置，以便将平台日志发送到不同的目标。

> [!IMPORTANT]
> 在创建诊断设置以收集活动日志之前，应先禁用任何旧配置。 有关详细信息，请参阅[通过旧设置收集 Azure 活动日志](diagnostic-settings-legacy.md)。

每个 Azure 资源都需要其自己的诊断设置，该设置定义以下内容：

- 发送到设置中所定义目标的日志和指标数据类别。 不同资源类型的可用类别将有所不同。
- 用于发送日志的一个或多个目标。 当前目标包括 Log Analytics 工作区、事件中心和 Azure 存储。
 
单个诊断设置只能定义每个目标中的一个。 如果要将数据发送到多个特定目标类型（例如，两个不同的 Log Analytics 工作区），请创建多个设置。 每个资源最多可以有 5 个诊断设置。
...
1. 在 Azure 门户中配置诊断设置的位置取决于资源。
   ...
   - 对于单个资源，请在资源菜单中单击 "**监视**" 下的 "**诊断设置**"。

        ![诊断设置](media/diagnostic-settings/menu-resource.png)

    - 对于一个或多个资源，请在 "Azure Monitor" 菜单中单击 "**设置**" 下的 "**诊断设置**"，然后单击 "资源"。
    
        ![诊断设置](media/diagnostic-settings/menu-monitor.png)

    - 对于活动日志，请在 " **Azure Monitor** " 菜单中单击 "**活动日志**"，然后单击 "**诊断设置**"。 请确保禁用活动日志的任何旧配置。 有关详细信息，请参阅[禁用现有设置](diagnostic-settings-legacy.md#disable-existing-settings)。
...
5. 选中要发送到指定目标的每个数据类别的复选框。 每个 Azure 服务的类别列表将各不相同。
...
> [!IMPORTANT]
> 不能将此方法用于 Azure 活动日志。 相反，请[在 Azure Monitor 中使用 "创建诊断设置"，使用资源管理器模板](diagnostic-settings-template.md)创建资源管理器模板并使用 PowerShell 进行部署。
...
请参阅[使用资源管理器模板在 Azure Monitor 中创建诊断设置](diagnostic-settings-template.md)，以使用资源管理器模板创建或更新诊断设置。

## <a name="next-steps"></a>后续步骤

* [阅读有关 Azure 平台日志的详细信息](platform-logs-overview.md)
