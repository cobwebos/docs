...
1. 在 [Azure 门户](https://portal.azure.com)中，选择“监视”  。 “监视器”  窗格将所有监视设置和数据合并到一个视图中。
...
1. 定义一个操作列表。 为每个操作提供以下信息：
    ...
    1. **常见警报架构**:您可以选择启用[常见警报架构](https://aka.ms/commonAlertSchemaDocs)，它提供一个可扩展的优势和 Azure Monitor 中服务的所有警报的统一警报有效负载。
...
> [!NOTE]
> 请参阅[监视的订阅服务限制](https://docs.microsoft.com/azure/azure-subscription-service-limits#azure-monitor-limits)有关以下各项的每个数字限制。  

### <a name="azure-app-push-notifications"></a>Azure 应用推送通知
操作组中，可以有有限的数量的 Azure 应用操作。
...
操作组中，可以有有限的数量的 ITSM 操作。 

### <a name="logic-app"></a>逻辑应用
操作组中，可以有有限的数量的逻辑应用操作。

### <a name="function"></a>函数
通过函数 API，目前要求 v2 函数应用，将"AzureWebJobsSecretStorageType"设置为"文件"将应用配置读取函数应用配置为操作的函数密钥。 有关详细信息，请参阅 [Functions V2 中对密钥管理的更改]( https://aka.ms/funcsecrets)。
...
操作组中，可以有有限的数量的 Runbook 操作。 

### <a name="sms"></a>SMS
请参阅[速率限制信息](./../../azure-monitor/platform/alerts-rate-limiting.md)并[短信警报行为](../../azure-monitor/platform/alerts-sms-behavior.md)有关其他重要信息。
...
Webhook 使用以下规则进行重试。 当返回的 HTTP 状态代码为 408、429、503、504 或者 HTTP 终结点未响应时，最多可以重试 2 次 Webhook 调用。 首次重试在 10 秒后发生。 第二次重试在 100 秒后发生。 在失败两次后，操作组在 30 分钟内不会再调用该终结点。 
...
操作组中，可以有有限的数量的 Webhook 操作。

#### <a name="secure-webhook"></a>安全 Webhook
**保护 Webhook 功能当前处于预览状态。**

操作组 Webhook 操作，可以充分利用 Azure Active Directory 来保护你的操作组和受保护的 web API （webhook 终结点） 之间的连接。 利用此功能的整体工作流如下所述。 有关 Azure AD 应用程序和服务主体的概述，请参阅[Microsoft 标识平台 (v2.0) 概述](https://docs.microsoft.com/azure/active-directory/develop/v2-overview)。

1. 创建受保护 web API 的 Azure AD 应用程序。 请参阅 https://docs.microsoft.com/azure/active-directory/develop/scenario-protected-web-api-overview 。
    - 配置受保护的 API 调用的守护程序。
    
1. 启用要使用 Azure AD 应用程序的操作组。

    > [!NOTE]
    > 您必须是属于[Azure AD 应用程序管理员角色](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-assign-admin-roles#available-roles)若要执行此脚本。
    
    - 修改 PowerShell 脚本的 Connect-azuread 调用，以使用 Azure AD 租户 id。
    - 修改 PowerShell 脚本的变量 $myAzureADApplicationObjectId，以使用 Azure AD 应用程序的对象 ID
    - 运行修改后的脚本。
    
1. 配置操作组 Webhook 操作。
    - 从脚本复制值 $myApp.ObjectId 和 Webhook 操作定义中的应用程序对象 ID 字段中输入。
    
    ![安全的 Webhook 操作](./media/action-groups/action-groups-secure-webhook.png)
