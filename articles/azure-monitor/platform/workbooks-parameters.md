...
4. 在弹出的 "新建参数" 窗格中，输入：
    ...
    2. 显示名称： `Time Range` *（但__显示名称__可以包含空格、特殊字符、表情符号等）*  
...
2. 打开 "_时间范围_" 下拉箭头，然后`Time Range`从底部的 "参数" 部分选择选项。
...
_In 文本_部分使用了参数`label`的，而不是它的值。 参数会根据其类型公开各种此类选项，例如，时间范围选取器允许值、标签、查询、开始、结束和颗粒。

使用`Previews` _编辑参数_窗格的部分来查看参数的扩展选项：
