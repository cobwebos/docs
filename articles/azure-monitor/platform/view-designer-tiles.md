---
title: Azure 监视器中视图设计器磁贴的参考指南 |微软文档
...
| 单位 |指定由查询返回的值的单位。 此信息用于显示在图表上指示值类型的标签，并可用于转换值。 **单位类型**指定单位的类别，并定义可用的**当前单位类型**值。 如果在 **"转换"** 中选择值，则数值将从 **"当前单位"** 类型转换为 **"转换为**类型"。 |
