...
很多团队需要严格控制对监视数据和设置的访问。 例如，如果你的团队成员专门使用监视（支持工程师、DevOps 工程师），或者如果你使用托管服务提供商，则你可能想要向他们授予仅监视数据的访问权限，同时限制其创建、修改或删除资源。 本文介绍如何快速地将内置监视 RBAC 角色应用到 Azure 中的用户，或者为需要有限的监视权限的用户构建自己的自定义角色。 然后讨论与 Azure Monitor 相关资源的安全注意事项，以及如何限制对它们所含数据的访问。
...
* Log Analytics 工作区中列出监视包。
* 在 Log Analytics 工作区中检索和执行保存的搜索。
* 检索 Log Analytics 的工作区存储配置。
...
* 在 Log Analytics 工作区中启用或禁用监视包。
* 在 Log Analytics 工作区中创建和删除并执行保存的搜索。
* 创建和删除 Log Analytics 工作区存储配置。

\*用户还必须单独授予对目标资源（存储帐户或事件中心命名空间）的 ListKeys 权限，才能设置日志配置文件或诊断设置。
...
2. 如果使用者需要能够获取密钥即席性，请向用户授予该事件中心的 ListKeys 操作。 这对于需要能够设置传输到事件中心的诊断设置或日志配置文件的用户来说也是必需的。 例如，可以创建 RBAC 规则：
...
![Azure 存储设置 "对话框](./media/roles-permissions-security/secured-storage-example.png) Azure Monitor 被视为" 受信任的 Microsoft 服务 "之一。如果允许受信任的 Microsoft 服务访问安全存储，则 Azure Monitor 将有权访问受保护的存储帐户;在这些受保护的条件下启用将 Azure Monitor 资源日志、活动日志和指标写入存储帐户。 这还会使 Log Analytics 能够从受保护的存储中读取日志。   
