...
| 层 | Data | 方法 |
|:---|:---|:---|
| [Azure 租户](data-sources.md#azure-tenant) | Azure Active Directory 审核日志 | 在 AAD 租户上配置租户诊断设置。 请[参阅教程：将 Azure Active Directory 日志流式传输到 Azure 事件](../../active-directory/reports-monitoring/tutorial-azure-monitor-stream-logs-to-event-hub.md)中心以获取详细信息。 |
...
| Splunk | [Splunk 的 Azure Monitor 外接](https://splunkbase.splunk.com/app/3534/)程序是 Splunkbase 中可用的开放源代码项目。 [Splunk 的 Azure Monitor 加载](https://github.com/Microsoft/AzureMonitorAddonForSplunk/wiki/Azure-Monitor-Addon-For-Splunk)项中提供了文档。<br><br> 如果无法在 Splunk 实例中安装外接程序，如使用代理或在 Splunk 云上运行，则可以使用[Azure Function For Splunk](https://github.com/Microsoft/AzureFunctionforSplunkVS)将这些事件转发到 Splunk HTTP 事件收集器，该函数由事件中心。 |
