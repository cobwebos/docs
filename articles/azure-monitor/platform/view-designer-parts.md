---
title: Azure 监视器中视图设计器部件的参考指南 |微软文档
...
名称和值分隔符是用于将列表查询中的文本属性分析为多个值的单字符分隔符。 如果指定分隔符，则可以为每个字段提供名称，由 **"名称"** 框中的同一分隔符分隔。
...
点击导航定义在单击标题或视图中的列表项时要执行的操作。  这将在[日志分析](../../azure-monitor/log-query/portals.md)中打开查询或启动另一个视图。
