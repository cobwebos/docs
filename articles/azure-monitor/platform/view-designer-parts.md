...
description: 通过使用 Azure Monitor 中的视图设计器，你可以创建在 Azure 门户中显示的自定义视图，并在 Log Analytics 工作区中包含有关数据的多种可视化效果。 本文针对自定义视图中可用的可视化部件，提供设置方面的参考指南。
...
ms.locfileid: "72931993"
---
# <a name="reference-guide-to-view-designer-visualization-parts-in-azure-monitor"></a>Azure Monitor 中的视图设计器可视化部件的参考指南
通过使用 Azure Monitor 中的视图设计器，你可以在 Azure 门户中创建各种自定义视图，这些视图可帮助你可视化 Log Analytics 工作区中的数据。 本文针对自定义视图中可用的可视化部件，提供设置方面的参考指南。
...
| [数字和列表](#number-and-list-part) |标头显示单个数字，其中显示日志查询中的记录数。 列表显示查询的前十个结果，其中的图形指示数值列的相关值或其随时间的变化。 |
| [两个数字和列表](#two-numbers-and-list-part) |标头显示两个数字，用于显示单独日志查询中的记录数。 列表显示查询的前十个结果，其中的图形指示数值列的相关值或其随时间的变化。 |
...
> [!NOTE]
> 视图中的部件基于 Log Analytics 工作区中的[日志查询](../log-query/log-query-overview.md)。 它们目前不支持[跨资源查询](../log-query/cross-workspace-query.md)从 Application Insights 检索数据。

## <a name="list-of-queries-part"></a>查询列表部分
查询列表部分显示日志查询列表。 可以选择每个查询以显示其结果。 默认情况下，视图将包括单个查询，可以选择“+ 查询”添加其他查询。
...
标头显示单个数字，其中显示日志查询中的记录数。 列表显示查询的前十个结果，其中的图形指示数值列的相关值或其随时间的变化。
...
标头具有两个数字，用于显示单独日志查询中的记录数。 列表显示查询的前十个结果，其中的图形指示数值列的相关值或其随时间的变化。
...
| 设置           | 描述 |
|:--|:--|
| 日志搜索(自动) | 选择标题项时要运行的日志查询。  这是项所基于的相同日志查询。
...
* 了解用于支持可视化部分中的查询的[日志查询](../log-query/log-query-overview.md)。
