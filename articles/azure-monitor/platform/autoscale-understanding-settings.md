...
| metricTrigger | statistic | timeGrain 时间段内的聚合方法。 例如，**统计 = "平均值"** 和 **"时间粒" = "PT1M"** 意味着指标应每 1 分钟聚合一次，通过获取平均值。 此属性规定指标的采样方式。 |
| metricTrigger | timeWindow | 查找指标的时间范围。 例如，**时间窗口 = "PT10M"** 意味着每次自动缩放运行时，它都会查询过去 10 分钟的指标。 使用该时间范围可将指标规范化，避免对暂时性的峰值作出反应。 |
| metricTrigger | timeAggregation | 用于聚合已采样指标的聚合方法。 例如，**时间聚合 = "平均值"** 应通过获取平均值来聚合采样指标。 上例取 10 个 1 分钟样本并求其平均值。 |
...
如果触发了一个或多个横向扩展规则，自动缩放将计算由每个规则**的缩放操作**确定的新容量。 然后，它会扩展到这些容量的最大值，以确保服务可用性。
...
如果未触发横向扩展规则，自动缩放将评估所有比例内缩放规则（带方向的规则 **="减少"）。** 如果触发了所有缩减规则，自动缩放只会执行某个缩减操作。

自动缩放计算由每个规则**的规模操作**确定的新容量。 然后，选择可产生其中最大容量的缩放操作，以确保服务可用性。
