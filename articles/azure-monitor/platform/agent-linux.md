...
适用于 Linux 的 Log Analytics 代理由多个包组成。 发布文件包含以下包，这些包可通过使用`--extract`参数运行 shell 捆绑包获得：
...
* Sudoers*包含*文件是在中`/etc/sudoers.d/omsagent`创建的。 这会`omsagent`授权重新启动 syslog 和 omsagent 守护程序。 如果在安装的 sudo 版本中不支持 sudo *include*指令，则这些项将写入`/etc/sudoers`。
* 修改 syslog 配置，以将事件子集转发到代理。 有关详细信息，请参阅[配置 Syslog 数据收集](data-sources-syslog.md)。

在被监视的 Linux 计算机上，代理将列为`omsagent`。 `omsconfig`是适用于 Linux 配置代理的 Log Analytics 代理，每5分钟查找一次新的门户端配置。 新的和更新的配置将应用于位于的代理配置文件`/etc/opt/microsoft/omsagent/conf/omsagent.conf`。
...
2. 使用`--install`参数安装捆绑包。 若要在安装过程中载入 Log Analytics 工作区， `-w <WorkspaceID>`请`-s <workspaceKey>`提供前面复制的和参数。

    >[!NOTE]
    >如果已安装了适用`--upgrade`于 Linux 的 system Center Operations Manager agent，则需要使用参数（如 omi、scx、omsconfig 或其旧版本）。 
...
如果 Linux 计算机需要通过代理服务器进行通信才能 Log Analytics，则可以通过包括`-p [protocol://][user:password@]proxyhost[:port]`在命令行上指定此配置。 *Protocol*属性接受`http`或`https`，而*proxyhost*属性接受代理服务器的完全限定的域名或 IP 地址。 
