...
- 指标：CPU 百分比
- 条件类型：静态
- 时间聚合（基于原始指标值运行的统计信息。 支持的时间聚合为最小、最大值、平均值、总计、计数：平均值
- 期间（检查指标值的回望窗口）：在过去 5 分钟中
- 频率（如果满足条件，指标警报检查的频率）：1 分钟
- 运算符：大于
- 阈值：70
...
到目前为止，已了解了如何使用单个指标警报监视与单个 Azure 资源相关的一个或多个指标时序。 很多时候，你可能希望将同一预警规则应用于许多资源。 Azure 监视器还支持使用一个指标警报规则监视同一 Azure 区域中存在的资源的多个资源（相同类型）。 

此功能当前支持以下 Azure 云中以下服务的平台指标（不是自定义指标）：
...
您可以通过以下三种方式之一指定单个指标警报规则的监视范围。 例如，对于虚拟机，您可以将作用域指定为：  

- 订阅中一个 Azure 区域中的虚拟机列表
