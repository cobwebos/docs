...
3. 在 "**定义警报条件**" 下, 提供以下信息并选择 "**完成**":
   ...
   - **警报目标:** 若要查看和选择新警报的目标, 请按**资源类型**使用**按订阅** / 筛选筛选器。 从显示的列表中选择资源或资源组。
...
若要使用 Azure 资源管理器模板创建活动日志警报, 请创建类型`microsoft.insights/activityLogAlerts`的资源。 然后，填充所有相关属性。 下面是创建活动日志警报的模板:
...
```
之前的示例 JSON 可以保存为 (例如, sampleActivityLogAlert), 并可使用[Azure 资源管理器在 Azure 门户中](../../azure-resource-manager/resource-group-template-deploy-portal.md)进行部署。
...
1. [az monitor activity-log alert scope](https://docs.microsoft.com/cli/azure/monitor/activity-log/alert/scope)：为创建的活动日志警报规则添加作用域。
...
若要检索一个活动日志警报规则资源, 请使用 Azure CLI [命令 az monitor monitor activity 日志 alert](https://docs.microsoft.com/cli/azure/monitor/activity-log/alert#az-monitor-activity-log-alert-show
)show。 若要查看资源组中的所有活动日志警报规则资源, 请使用[az monitor 活动日志警报列表](https://docs.microsoft.com/cli/azure/monitor/activity-log/alert#az-monitor-activity-log-alert-list)。
