...
有时需要导出所选行的全部内容，而不只是导出特定的列。 在这种情况下， `Field to export`请在上面的步骤7.1 中将该属性保留为未设置。 工作簿会将整行内容导出为作为参数的 json。 

在引用 KQL 控件上，使用`todynamic`函数分析 json 并访问各个列。

 ## <a name="grid-cell-clicks"></a>网格单元格单击
工作簿允许作者通过一种特殊类型的网格列呈现器来添加`link renderer`交互性，称为。 链接呈现器根据单元格的内容将网格单元转换为超链接。 工作簿支持多种类型的链接呈现器，其中包括允许打开资源概述边栏选项卡、属性包查看器、App Insights 搜索、使用情况、事务跟踪等。
...
6. 单击 "_列设置_" 打开 "设置" 窗格。
7. 在 "_列_" 部分中，设置：
    1. _示例_-列呈现器`Link`：，要打开的`Cell Details`视图：，链接标签：`Sample`
    2. _计数_-列呈现器`Bar`：，调色板： `Blue`，最小值：`0`
    3. _请求_-列呈现器：`Automatic`
    4. 单击 "_保存并关闭_" 以应用更改
8. 单击网格中的`Sample`某个链接。 这将打开一个属性窗格，其中包含抽样请求的详细信息。
...
5. 检查设置`Make this item conditionally visible`
    1. 如果`ShowDetails`参数值`equals`为，则此项可见`Yes`
6. 单击 "_完成编辑_" 以提交更改。
7. 单击 "工作簿" 工具栏上的 "_完成编辑_" 以进入读取模式。
8. 将参数`ShowDetails`的值转换为`No`。 请注意，下图将消失。

下图显示`ShowDetails`了`Yes`

![显示图表可见位置的条件可见性的图像](./media/workbooks-interactive/conditional-visibility.png)

下图显示了隐藏的情况， `ShowDetails`其中`No`
