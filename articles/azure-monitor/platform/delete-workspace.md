...
> [!NOTE]
> 如果要替换软删除行为并永久删除工作区，请按照[永久删除工作区](#permanent-workspace-delete)中的步骤操作。

删除工作区时需谨慎，因为其中可能会含有对服务操作产生不利影响的重要数据和配置。 查看将数据存放在 Log Analytics 中的代理、解决方案和其他 Azure 服务和源，例如：
...
若要永久删除工作区，请将[工作区 – 删除](https://docs.microsoft.com/rest/api/loganalytics/workspaces/delete) REST 请求与强制标记配合使用：
...
或者，可以从 Azure REST 文档站点执行操作：
1.  导航到[工作区 – 删除](https://docs.microsoft.com/rest/api/loganalytics/workspaces/delete) REST API，然后单击“试用”。 
2.  输入要永久删除的工作区的详细信息
3.  输入值为 true 的新参数 force 
4.  单击值右侧的“+”图标。 这会将 force = true 添加到请求中的 URI
5.  单击“运行”按钮
