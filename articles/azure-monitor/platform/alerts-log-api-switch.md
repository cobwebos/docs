...
description: 旧 savedSearch 概述基于 Log Analytics 警报 API 和过程，以切换到新的 ScheduledQueryRules API，解决常见客户问题的详细信息的警报规则。
author: msvijayn
...
- 管理日志警报使用[Powershell cmdlet](alerts-log.md#managing-log-alerts-using-powershell)
...
用户可以自由使用[旧 Log Analytics 警报 API](api-alerts.md) 或新 [scheduledQueryRules API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules)。 通过任一 API 创建的警报规则都*只能由同一 API 进行管理* - 以及从 Azure 门户进行管理。 默认情况下，Azure Monitor 将继续使用[旧式 Log Analytics 警报 API](api-alerts.md)用于从 Azure 门户中为现有工作区的 Log Analytics 中创建任何新的警报规则。 作为[宣布或 2019 年 6 月 1 日之后创建的新日志工作区](https://azure.microsoft.com/updates/switch-api-preference-log-alerts/)-将自动使用新[scheduledQueryRules API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules)通过默认包括在 Azure 门户中。

下面汇总了首选项切换为 scheduledQueryRules API 的影响：

- 通过编程接口为管理日志警报而进行的所有交互现在都必须改用 [scheduledQueryRules](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules) 完成。 有关详细信息，请参阅，[通过 Azure 资源模板的示例使用](alerts-log.md#managing-log-alerts-using-azure-resource-template)和[示例使用通过 PowerShell](alerts-log.md#managing-log-alerts-using-powershell)
...
从[旧 Log Analytics 警报 API](api-alerts.md) 移动警报规则的过程不涉及以任何方式更改警报定义、查询或配置。 你的警报规则和监视不受影响，警报不会在切换期间或之后停止或停滞。 唯一更改是 API 首选项并且有权通过一个新的 API 在规则中的更改。

> [!NOTE]
> 一旦用户选择要切换到新的首选项[scheduledQueryRules API](https://docs.microsoft.com/rest/api/monitor/scheduledqueryrules)，则不能重新选择或恢复为使用较旧的[旧式 Log Analytics 警报 API](api-alerts.md)。
