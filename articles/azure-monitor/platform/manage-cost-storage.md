...
ms.locfileid: "64918987"
---
# <a name="manage-usage-and-costs-with-azure-monitor-logs"></a>管理使用情况和成本与 Azure Monitor 日志

> [!NOTE]
> 本文介绍如何通过设置 Log Analytics 工作区的数据保留期来控制您的成本在 Azure 监视器中。  请参阅以下文章了解相关信息。
> - [监视使用情况及预估成本](usage-estimated-costs.md)介绍了如何针对不同的定价模型查看多个 Azure 监视功能的使用情况及预估成本。 它还介绍了如何更改定价模型。

Azure Monitor 日志到缩放并支持收集、 索引和存储在企业中的每日来自任何源的数据海量设计，或在 Azure 中部署。  尽管这可能是组织的主要驱动力，但成本效益最终是基本驱动力。 为此，务必了解 Log Analytics 工作区的成本不基于仅在数据收集，还是依赖计划选择，并且您选择多长时间存储从连接的源生成数据的卷上。  
...
Azure Monitor 日志使我们可以很容易理解什么成本就很可能基于最近使用模式。 若要执行此操作，请使用**Log Analytics 使用情况和估计成本**来查看和分析数据使用情况。 显示每个解决方案收集的数据量、保留的数据量，并根据引入的数据量和已包含量之外的其他保留量来估算成本。
...
从“使用情况和估计成本”页面，可以查看当月的数据量。 这包括 Log Analytics 工作区中收到和保留的所有数据。  单击**用法的详细信息**顶部的页上，若要查看包含信息的使用情况仪表板上的源、 计算机和产品/服务的数据量趋势。 若要查看和设置每日上限或修改保留期，请单击“数据量管理”。
...
可以配置工作区的每日上限并限制每日引入量，但请谨慎设置，因为目标是避免达到每日限制。  否则，会丢失该天剩余时间的数据，这可能会影响其功能依赖于工作区中提供的最新数据的其他 Azure 服务和解决方案。  因此，需要具有在支持 IT 服务的资源的运行状况受到影响时监视和接收警报的能力。  每日上限旨在用作一种方法来管理从所管理的资源的数据量意外的增长并保持了限制，或者当你想要限制工作区产生计划外的费用。  
...
以下步骤介绍如何配置一个限制来管理每日引入 Log Analytics 工作区的数据量。  
...
3. 在窗格中，移动滑块以增加或减少天数，然后单击“确定”。  如果位于“免费”层，则不能修改数据保留期，需要升级到付费层才能控制这一项设置。
    ...
    ![更改工作区数据保留期设置](media/manage-cost-storage/manage-cost-change-retention-01.png)

## <a name="legacy-pricing-tiers"></a>旧版定价层

2018 年 7 月 1 日以前签订企业协议的客户或者已在订阅中创建 Log Analytics 工作区的客户，仍可以访问免费计划。 如果订阅未绑定到现有的 EA 注册，那么，2018 年 4 月 2 日后，在新订阅中创建工作区时免费层不再可用。  数据被限制为七天的保留期*免费*层。  旧*独立*或*每个节点的*过去 31 天内的层，以及当前 2018年单一定价层，收集的数据是可用。 “免费”层的每日引入限制为 500 MB。如果发现一直超出允许的量，可将工作区更改为另一计划，从而收集超出该限制的数据。 
...
如果你想要将你的工作区移动到当前的定价层，则需要更改订阅的监视[Azure Monitor 中的定价模型](usage-estimated-costs.md#moving-to-the-new-pricing-model)这将更改该订阅中的所有工作区的定价层。

> [!NOTE]
> 您可以了解有关设置定价层的详细信息时[使用 Azure 资源管理器模板](template-workspace-configuration.md#create-a-log-analytics-workspace)创建工作区中，并确保你的 Azure 资源管理器模板部署将无论是否成功订阅旧的或新的定价模型。 
...
当数据收集停止时，是 OperationStatus**警告**。 数据收集启动时，是 OperationStatus **Succeeded**。 下表描述了数据收集停止的原因以及用于恢复数据收集的建议操作：  
...
若要得到通知，当数据收集停止时，请使用中所述的步骤*创建每日数据上限*警报数据收集停止时收到通知。 使用中所述的步骤[创建操作组](action-groups.md)若要配置警报规则的一个电子邮件、 webhook 或 runbook 操作。 

## <a name="troubleshooting-why-usage-is-higher-than-expected"></a>排查使用量超出预期的原因

使用量较高是由下面的一个或两个原因引起的：
- 更多节点比预期将数据发送到 Log Analytics 工作区
- 更多的数据比预期要发送到 Log Analytics 工作区
...
若要获取发送计算机的列表**计费数据类型**（某些数据类型是免费的），利用`_IsBillable`[属性](log-standard-properties.md#_isbillable):
...
更改`subscriptionId`到`resourceGroup`将显示通过 Azure 资源组的可计费引入的数据卷。 
...
## <a name="create-an-alert-when-data-collection-is-high"></a>数据收集较高时创建警报
