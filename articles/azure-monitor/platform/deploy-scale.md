...
[诊断设置](./diagnostic-settings.md)将资源日志和指标从 Azure 资源收集到多个位置，通常是收集到 Log Analytics 工作区，该工作区允许使用[日志查询](../log-query/log-query-overview.md)和[日志警报](alerts-log.md)来分析数据。 使用 Policy 可在每次创建资源时自动创建诊断设置。
...
## <a name="azure-monitor-for-vms-and-virtual-machine-agents"></a>用于 VM 的 Azure Monitor 和虚拟机代理
[用于 VM 的 Azure Monitor](../insights/vminsights-overview.md)是 Azure Monitor 用于监视虚拟机和虚拟机规模集的主要工具。 若要启用用于 VM 的 Azure Monitor 必须在每个客户端上安装 Log Analytics 代理和依赖项代理。 你还可以自行安装 Log Analytics 代理以支持其他监视方案。 使用 Azure 策略可以确保在创建每个虚拟机时配置每个虚拟机，而不是手动执行这些任务。
...
| [预览]：应在 Linux Azure Arc 计算机上安装 Log Analytics 代理 |如果在列表中定义了 VM 映像（OS），并且未安装代理，则会报告混合 Azure Arc 计算机不符合 Linux Vm 的要求。 |
| [预览]：应在 Windows Azure Arc 计算机上安装 Log Analytics 代理 |如果在列表中定义了 VM 映像（OS），并且未安装代理，则会将混合 Azure Arc 计算机报告为不符合 Windows Vm。 |
