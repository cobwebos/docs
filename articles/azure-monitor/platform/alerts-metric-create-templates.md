...
ms.locfileid: "53344943"
---
# <a name="create-a-metric-alert-with-a-resource-manager-template"></a>使用 Resource Manager 模板创建指标警报

本文介绍如何在 Azure Monitor 中使用 [Azure 资源管理器模板](../../azure-resource-manager/resource-group-authoring-templates.md)配置[较新的指标警报](../../monitoring-and-diagnostics/monitoring-near-real-time-metric-alerts.md)。 使用资源管理器模板可以通过编程方式在多个环境中设置一致且可重现的警报。 较新的指标警报当前适用于[这套资源类型](../../monitoring-and-diagnostics/monitoring-near-real-time-metric-alerts.md#metrics-and-dimensions-supported)。

> [!IMPORTANT]
> 为指标警报指定的资源管理器模板不能用于以下资源类型：Microsoft.OperationalInsights/workspace；因为 Log Analytics 中对指标的支持处于预览状态。 若要通过资源模板使用预览功能，可联系 [Azure 警报反馈](mailto:azurealertsfeedback@microsoft.com)
...
将下面的 json 保存为 simplemetricalert.parameters.json 并根据需要对其进行修改。
...
可以通过 PowerShell 或 Azure CLI 使用模板和参数文件创建指标警报。
...
## <a name="resource-manager-template-for-a-more-advanced-metric-alert"></a>更高级指标警报的资源管理器模板

较新的指标警报支持根据多维指标发出警报，且支持多个标准。 可以使用以下模板创建根据维度指标发出的更高级指标警报，并指定多个标准。

为进行本次演练，请将下面的 json 保存为 advancedmetricalert.json。
...
为进行本次演练，请将下面的 json 保存并修改为 advancedmetricalert.parameters.json。
...
## <a name="resource-manager-template-for-metric-alert-that-monitors-multiple-resources"></a>用于监视多个资源的指标警报的资源管理器模板

上一部分中已介绍了一些示例 Azure 资源管理器模板，它们用来创建用于监视单个资源的指标警报。 Azure Monitor 现在支持使用单个指标警报规则监视多个资源。 此预览版功能当前只能通过 Azure 资源管理器模板和 REST API 使用，并且仅支持在虚拟机上使用。 很快将支持通过门户创建和编辑此类规则。
...
### <a name="all-virtual-machines-in-one-or-more-resource-groups"></a>一个或多个资源组中的所有虚拟机

此模板将创建一个指标警报规则，用于监视一个或多个资源组中的所有虚拟机（在单个 Azure 区域中）的 CPU 百分比。

将下面的 json 保存为 all-vms-in-resource-group.json 以用于此演练。
...
可以将上述模板与下面的参数文件配合使用。
将下面的 json 保存并修改为 all-vms-in-resource-group.parameters.json 以用于此演练。
...
### <a name="all-virtual-machines-in-a-subscription"></a>订阅中的所有虚拟机

此模板将创建一个指标警报规则，用于监视单个订阅中的所有虚拟机（在单个 Azure 区域中）的 CPU 百分比。

将下面的 json 保存为 all-vms-in-subscription.json 以用于此演练。
...
可以将上述模板与下面的参数文件配合使用。
将下面的 json 保存并修改为 all-vms-in-subscription.parameters.json 以用于此演练。
...
### <a name="a-list-of-virtual-machines"></a>虚拟机列表

此模板将创建一个指标警报规则，用于监视单个订阅中的虚拟机列表（在单个 Azure 区域中）的 CPU 百分比。

将下面的 json 保存为 list-of-vms.json 以用于此演练。
...
可以将上述模板与下面的参数文件配合使用。
将下面的 json 保存并修改为 list-of-vms.parameters.json 以用于此演练。
