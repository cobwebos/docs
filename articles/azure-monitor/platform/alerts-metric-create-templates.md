...
为进行本次演练，请将下面的 json 保存为 simplestaticmetricalert.json。
...
将下面的 json 保存为 simplestaticmetricalert.parameters.json 并根据需要对其进行修改。
...
## <a name="template-for-a-simple-dynamic-thresholds-metric-alert"></a>用于简单动态阈值指标警报的模板

若要使用 Resource Manager 模板创建警报，需要创建 `Microsoft.Insights/metricAlerts` 类型的资源，并填充所有相关属性。 下面是创建指标警报规则的示例模板。

为进行本次演练，请将下面的 json 保存为 simpledynamicmetricalert.json。
...
将下面的 json 保存为 simpledynamicmetricalert.parameters.json 并根据需要对其进行修改。
...
为进行本次演练，请将下面的 json 保存并修改为 advancedstaticmetricalert.parameters.json。
...
为进行本次演练，请将下面的 json 保存并修改为 advanceddynamicmetricalert.parameters.json。
...
上一部分中已介绍了一些示例 Azure 资源管理器模板，它们用来创建用于监视单个资源的指标警报。 Azure Monitor 现在支持使用单个指标警报规则监视多个资源。 此预览版功能当前只能通过 Azure 资源管理器模板和 REST API 使用，并且仅支持在虚拟机上使用。
...
可以将上述模板与下面的参数文件配合使用。
将下面的 json 保存并修改为 all-vms-in-resource-group-static.parameters.json 以用于此演练。
...
可以根据当前工作目录，通过 PowerShell 或 Azure CLI 使用模板和参数文件创建静态指标警报。
...
### <a name="dynamic-thresholds-alert-on-all-virtual-machines-in-one-or-more-resource-groups"></a>一个或多个资源组中所有虚拟机上的动态阈值警报

此模板将创建动态阈值指标警报规则，用于监视一个或多个资源组中的所有虚拟机（在单个 Azure 区域中）的 CPU 百分比。

将下面的 json 保存为 all-vms-in-resource-group-dynamic.json 以用于此演练。
...
可以将上述模板与下面的参数文件配合使用。
将下面的 json 保存并修改为 all-vms-in-resource-group-dynamic.parameters.json 以用于此演练。
...
可以将上述模板与下面的参数文件配合使用。
将下面的 json 保存并修改为 all-vms-in-subscription-dynamic.parameters.json 以用于此演练。
...
可以将上述模板与下面的参数文件配合使用。
将下面的 json 保存并修改为 list-of-vms-dynamic.parameters.json 以用于此演练。
