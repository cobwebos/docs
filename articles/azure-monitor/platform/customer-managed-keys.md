...
此资源用作 Key Vault 与工作区之间的中间标识连接。 收到订阅处于允许列表状态的确认后，请在工作区所在的区域创建 Log Analytics*群集*资源。 Application Insights 和 Log Analytics 需要单独的群集资源。 *群集*资源的类型在创建时通过将 "clusterType" 属性设置为 "LogAnalytics" 或 "applicationinsights.config" 来定义。 不能更改群集资源类型。
