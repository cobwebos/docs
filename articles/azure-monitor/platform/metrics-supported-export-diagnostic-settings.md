...
默认情况下，Azure 监视器提供[平台指标](data-platform-metrics.md)，无需配置。 它提供了几种与平台指标进行交互的方法，包括在门户中绘制图表、通过 REST API 访问它们或使用 PowerShell 或 CLI 查询它们。 有关 Azure 监视器的整合指标管道当前可用的平台指标的完整列表，请参阅[支持指标](metrics-supported.md)。 要查询和访问这些指标，请使用[2018-01-01 api 版本](https://docs.microsoft.com/rest/api/monitor/metricdefinitions)。 其他指标可在门户或旧版 API 中使用。

您可以通过以下两种方式之一将平台指标从 Azure 监视器管道导出到其他位置。
1. 使用[诊断设置](diagnostic-settings.md)发送到日志分析、事件中心或 Azure 存储。
2. 使用[指标 REST API](https://docs.microsoft.com/rest/api/monitor/metrics/list)

由于 Azure 监视器后端的复杂性，并非所有指标都可以使用诊断设置进行导出。 下表列出了可以使用诊断设置导出和不能导出的内容。
...
## <a name="metrics-exportable-table"></a>指标可导出表 
...
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubDroppedEvents  |  删除的事件中心事件数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubRejectedEvents  |  拒绝的事件中心事件数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubSuccessfulEvents  |  成功的事件中心事件数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubThrottledEvents  |  限制的事件中心事件数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubTimedoutEvents  |  超时的事件中心事件数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubTotalBytesSent  |  事件中心事件大小  |  字节  |  总计 | 
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubTotalEvents  |  事件中心事件总数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.ApiManagement/service  |  EventHubTotalFailedEvents  |  失败的事件中心事件数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.AppPlatform/Spring  |  TomcatSessionCreatedCount  |  Tomcat 创建的会话数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.AppPlatform/Spring  |  TomcatSessionExpiredCount  |  Tomcat 过期的会话数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.AppPlatform/Spring  |  TomcatSessionRejectedCount  |  Tomcat 拒绝的会话数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.Automation/automationAccounts  |  TotalUpdateDeploymentMachineRuns  |  更新部署计算机运行总数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  已完成的作业  |  已完成的作业  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  提交的作业  |  提交的作业  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  正在退出的核心数  |  正在退出的核心数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  正在退出的节点数  |  正在退出的节点数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  占用的核心数  |  占用的核心数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  占用的节点数  |  占用的节点数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  配额利用率百分比  |  配额利用率百分比  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  核心总数  |  核心总数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  节点总数  |  节点总数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  不可用的核心数  |  不可用的核心数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.BatchAI/workspaces  |  不可用的节点数  |  不可用的节点数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Blockchain/blockchainMembers  |  ConnectionHandled  |  已处理的连接数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.Blockchain/blockchainMembers  |  ProcessedBlocks  |  已处理的块数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.Blockchain/blockchainMembers  |  ProcessedTransactions  |  已处理的事务数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.Blockchain/blockchainMembers  |  QueuedTransactions  |  已排队的事务数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.CognitiveServices/accounts  |  CharactersTrained  |  训练的字符数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachines  |  数据磁盘队列深度  |  数据磁盘队列深度（预览版）  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachines  |  入站流数  |  入站流数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachines  |  OS 磁盘队列深度  |  OS 磁盘队列深度（预览版）  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachines  |  出站流数  |  出站流数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets  |  数据磁盘队列深度  |  数据磁盘队列深度（预览版）  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets  |  入站流数  |  入站流数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets  |  OS 磁盘队列深度  |  OS 磁盘队列深度（预览版）  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets  |  出站流数  |  出站流数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets/virtualMachines  |  数据磁盘队列深度  |  数据磁盘队列深度（预览版）  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets/virtualMachines  |  入站流数  |  入站流数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets/virtualMachines  |  OS 磁盘队列深度  |  OS 磁盘队列深度（预览版）  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Compute/virtualMachineScaleSets/virtualMachines  |  出站流数  |  出站流数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.ContainerRegistry/registries  |  SuccessfulPullCount  |  成功提取计数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.ContainerRegistry/registries  |  SuccessfulPushCount  |  成功推送计数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.ContainerRegistry/registries  |  TotalPullCount  |  提取总数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.DataFactory/factories  |  ActivityCancelledRuns  |  已取消的活动运行指标数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.DataFactory/factories  |  FactorySizeInGbUnits  |  总工厂大小（以 GB 为单位）  |  Count  |  最大值 | 
...
| **是**  | 否 |  Microsoft.DataFactory/factories  |  PipelineCancelledRuns  |  已取消的管道运行指标数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.DataFactory/factories  |  TriggerCancelledRuns  |  已取消的触发器运行指标数  |  Count  |  总计 | 
...
| **是**  | **是** |  微软.设备/帐户  |  数字双胞胎.遥测.节点  |  数字双胞胎节点遥测占位符  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.Devices/IotHubs  |  c2d.commands.egress.abandon.success  |  丢弃的 C2D 消息数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.Devices/IotHubs  |  c2d.commands.egress.complete.success  |  已完成的 C2D 消息传递次数  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.Devices/IotHubs  |  c2d.commands.egress.reject.success  |  拒绝的 C2D 消息数  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.Devices/IotHubs  |  C2DMessagesExpired  |  过期的 C2D 消息数（预览版）  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.Devices/IotHubs  |  EventGridDeliveries  |  事件网格传递次数（预览版）  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.DocumentDB/databaseAccounts  |  删除虚拟网络  |  删除虚拟网络  |  Count  |  Count | 
...
| 否  | 否 |  Microsoft.EnterpriseKnowledgeGraph/services  |  SuccessCount  |  成功计数  |  Count  |  Count | 
| 否  | 否 |  Microsoft.EnterpriseKnowledgeGraph/services  |  SuccessLatency  |  成功延迟  |  毫秒  |  平均值 | 
| 否  | 否 |  Microsoft.EnterpriseKnowledgeGraph/services  |  TransactionCount  |  事务计数  |  Count  |  Count | 
...
| **是**  | **是** |  Microsoft.EventHub/namespaces  |  INMSGS  |  传入的消息数（已过时）（已弃用）  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.EventHub/namespaces  |  INREQS  |  传入的请求数（已弃用）  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.EventHub/namespaces  |  INTERR  |  内部服务器错误数（已弃用）  |  Count  |  总计 | 
| **是**  | **是** |  Microsoft.EventHub/namespaces  |  MISCERR  |  其他错误（已弃用）  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.EventHub/namespaces  |  OUTMSGS  |  传出的消息数（已过时）（已弃用）  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.HDInsight/clusters  |  NumActiveWorkers  |  活动辅助角色数目  |  Count  |  最大值 | 
| **是**  | 否 |  Microsoft.HDInsight/clusters  |  缩放请求  |  缩放请求  |  Count  |  最大值 | 
...
| 否  | 否 |  Microsoft.Insights/Components  |  dependencies/failed  |  依赖项调用失败次数  |  Count  |  Count | 
| 否  | 否 |  Microsoft.Insights/Components  |  exceptions/browser  |  浏览器异常  |  Count  |  Count | 
| **是**  | **是** |  Microsoft.Insights/Components  |  exceptions/count  |  异常  |  Count  |  Count | 
...
| **是**  | **是** |  Microsoft.Kusto/Clusters  |  ContinuousExportNumOfRecordsExported  |  连续导出 - 导出记录数  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.Kusto/Clusters  |  EventsProcessedForEventHubs  |  处理的事件数（适用于事件中心/IoT 中心）  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.Kusto/Clusters  |  SteamingIngestRequestRate  |  流引入请求速率  |  Count  |  RateRequestsPerSecond | 
...
| **是**  | **是** |  Microsoft.Kusto/Clusters  |  StreamingIngestResults  |  流引入结果  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  已完成的运行数  |  已完成的运行数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  正在退出的核心数  |  正在退出的核心数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  正在退出的节点数  |  正在退出的节点数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  失败的模型部署数  |  失败的模型部署数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  已启动的模型部署数  |  已启动的模型部署数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  成功的模型部署数  |  成功的模型部署数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  失败的模型注册数  |  失败的模型注册数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  成功的模型注册数  |  成功的模型注册数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  占用的核心数  |  占用的核心数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  占用的节点数  |  占用的节点数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  配额利用率百分比  |  配额利用率百分比  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  已启动的运行数  |  已启动的运行数  |  Count  |  总计 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  核心总数  |  核心总数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  节点总数  |  节点总数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  不可用的核心数  |  不可用的核心数  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.MachineLearningServices/workspaces  |  不可用的节点数  |  不可用的节点数  |  Count  |  平均值 | 
...
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  GCPauseTotalCount  |  GC 暂停计数  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  GCPauseTotalTime  |  GC 暂停总时间  |  毫秒  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  MaxOldGenMemoryPoolBytes  |  最大可用的旧代数据大小  |  字节  |  平均值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  OldGenMemoryPoolBytes  |  旧代数据大小  |  字节  |  平均值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  OldGenPromotedBytes  |  提升为旧代数据大小  |  字节  |  最大值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  服务Cpu使用百分比  |  服务 CPU 使用率  |  百分比  |  平均值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  服务内存已提交  |  服务内存分配  |  字节  |  平均值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  服务内存最大值  |  服务内存最大值  |  字节  |  最大值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  服务内存已使用  |  使用的服务内存  |  字节  |  平均值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  SystemCpuUsagePercentage  |  系统 CPU 使用率百分比  |  百分比  |  平均值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatErrorCount  |  Tomcat 全局错误  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatReceivedBytes  |  Tomcat 收到的总字节数  |  字节  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatRequestMaxTime  |  Tomcat 请求最大时间  |  毫秒  |  最大值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatRequestTotalCount  |  Tomcat 请求总数  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatRequestTotalTime  |  Tomcat 请求总时间  |  毫秒  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatResponseAvgTime  |  Tomcat 请求平均时间  |  毫秒  |  平均值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatSentBytes  |  Tomcat 发送的总字节数  |  字节  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatSessionActiveCurrentCount  |  Tomcat 会话活动计数  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatSessionActiveMaxCount  |  Tomcat 会话最大活动计数  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatSessionAliveMaxTime  |  Tomcat 会话最大活动时间  |  毫秒  |  最大值 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatSessionCreatedCount  |  Tomcat 创建的会话数  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatSessionExpiredCount  |  Tomcat 过期的会话数  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  TomcatSessionRejectedCount  |  Tomcat 拒绝的会话数  |  Count  |  总计 | 
| **是**  | 否 |  微软.微服务4春天/应用程序集群  |  YoungGenPromotedBytes  |  提升为新代数据大小  |  字节  |  最大值 | 
...
| 否  | 否 |  Microsoft.Network/applicationGateways  |  CapacityUnits  |  当前容量单位数  |  Count  |  平均值 | 
| 否  | 否 |  Microsoft.Network/applicationGateways  |  ClientRtt  |  客户端 RTT  |  毫秒  |  平均值 | 
| 否  | 否 |  Microsoft.Network/applicationGateways  |  ComputeUnits  |  当前计算单位数  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.OperationalInsights/workspaces  |  Average_Avg. 磁盘秒数/读取  |  平均磁盘秒/读取  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.OperationalInsights/workspaces  |  Average_Avg. 磁盘秒数/读取  |  平均磁盘秒/读取  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.OperationalInsights/workspaces  |  Average_Avg. 磁盘秒数/传输  |  平均磁盘秒/传输  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.OperationalInsights/workspaces  |  Average_Avg. 磁盘秒数/写入  |  平均磁盘秒/写入  |  Count  |  平均值 | 
| **是**  | 否 |  Microsoft.OperationalInsights/workspaces  |  Average_Avg. 磁盘秒数/写入  |  平均磁盘秒/写入  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Sql/servers/databases  |  app_cpu_billed  |  计费的应用 CPU  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.Sql/servers/databases  |  cpu_used  |  已用 CPU  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.Sql/servers/elasticPools  |  cpu_used  |  已用 CPU  |  Count  |  平均值 | 
...
| 否  | 否 |  Microsoft.Sql/servers/elasticPools  |  database_cpu_used  |  已用 CPU  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.StorageCache/caches  |  ClientIOPS  |  总客户端 IOPS  |  Count  |  平均值 | 
...
| **是**  | 否 |  Microsoft.StorageCache/caches  |  StorageTargetIOPS  |  总 StorageTarget IOPS  |  Count  |  平均值 | 
...
| **是**  | 否 |  microsoft.storagesync/storageSyncServices  |  StorageSyncSyncSessionAppliedFilesCount  |  已同步的文件数  |  Count  |  总计 | 
...
| **是**  | 否 |  microsoft.storagesync/storageSyncServices/syncGroups  |  SyncGroupSyncSessionAppliedFilesCount  |  已同步的文件数  |  Count  |  总计 | 
...
| **是**  | 否 |  microsoft.storagesync/storageSyncServices/syncGroups/serverEndpoints  |  ServerEndpointSyncSessionAppliedFilesCount  |  已同步的文件数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.VMwareCloudSimple/virtualMachines  |  DiskReadOperations  |  磁盘读取操作次数  |  Count  |  总计 | 
...
| **是**  | 否 |  Microsoft.VMwareCloudSimple/virtualMachines  |  DiskWriteOperations  |  磁盘写入操作次数  |  Count  |  总计 | 
...
| **是**  | **是** |  Microsoft.Web/serverfarms  |  TcpClosing  |  TCP 正在关闭  |  Count  |  平均值 | 
...
| **是**  | **是** |  Microsoft.Web/serverfarms  |  TcpSynSent  |  发送的 TCP Syn  |  Count  |  平均值 | 
