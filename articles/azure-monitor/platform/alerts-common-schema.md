...
通用警报架构将目前 Azure 中的警报通知的使用体验进行了标准化。 从历史上看，Azure 中的三种警报类型（指标、日志和活动日志）都有自己的电子邮件模板、Webhook 架构等。使用通用警报架构，您现在可以使用一致的架构接收警报通知。

任何警报实例都会描述**受影响的资源**和**警报原因**。这些实例在以下部分的通用架构中进行介绍：
* **要点**：一组**标准化字段**，在所有警报类型中很常见，用于描述警报所使用**的资源**以及其他常见警报元数据（例如，严重性或描述）。 
* **警报上下文**：一组描述**警报原因的**字段，字段**因警报类型**而异。 例如，指标警报在警报上下文中会有指标名称和指标值之类的字段，而活动日志警报会包含生成警报的事件的相关信息。 
...
这意味着可以减少集成数，大大简化其管理和维护过程。__ 另外，未来的警报有效负载扩充（例如，自定义、诊断扩充等）只会出现在通用架构中。
...
| 电子邮件 | 一个一致且详细的电子邮件模板，可以轻松快捷地诊断问题。 嵌入式深度链接指向门户中的警报实例和受影响的资源，确保你可以快速跳转到修正过程。 |
...
- [Webhook/逻辑应用/Azure 函数/自动化 Runbook 的常见警报架构定义。](https://aka.ms/commonAlertSchemaDefinitions)
