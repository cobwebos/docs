...
Azure 监视器中的计算机组允许您将[日志查询](../log-query/log-query-overview.md)限定为一组特定的计算机。  每个组使用定义的查询或通过从不同源导入组填充计算机。  当日志查询中包括组时，结果仅限于与组中的计算机匹配的记录。
...
| 配置管理器 | 从 Microsoft 终结点配置管理器导入集合，并在 Azure 监视器中为每个集合创建一个组。 |
