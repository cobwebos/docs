...
| VM 存储 | 用于 VM 的 Azure Monitor 将 heath 状态信息存储在自定义位置。 除了[Azure 资源运行状况 REST API](/rest/api/resourcehealth/)外，此功能仅适用于 Azure 门户中的用于 VM 的 Azure Monitor。 | [了解 Azure 虚拟机的运行状况](../insights/vminsights-health.md)<br>[Azure 资源运行状况 REST API](https://docs.microsoft.com/rest/api/resourcehealth/) |
...
| Azure Kubernetes 服务 | 为了获得近乎实时的体验，Azure Monitor 容器直接在 Azure 门户中显示 Azure Kubernetes 服务中的数据。 | [如何使用用于容器的 Azure Monitor（预览版）实时查看容器日志](../insights/container-insights-live-logs.md) |
