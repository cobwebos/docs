...
[服务映射](../insights/service-map.md)和[用于 VM 的 Azure Monitor](../../azure-monitor/insights/vminsights-overview.md) 要求在 Windows 和 Linux 虚拟机上有一个依赖项代理。 它集成了 Log Analytics 代理，用于收集在虚拟机上运行的进程的已发现数据，以及外部进程依赖项的已发现数据。 它将此数据存储在 Azure Monitor 中，并将发现的互连组件可视化。  
