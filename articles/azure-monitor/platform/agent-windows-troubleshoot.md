...
如果代理通过代理服务器或防火墙通信，某些限制可能会阻止源计算机和 Azure Monitor 服务发起的通信。 如果通信被阻止，由于配置错误，在尝试安装代理时，与工作区的注册可能会失败，或将代理安装后配置为向其他工作区报告。 注册成功后，代理通信可能会失败。 本部分将介绍排查此类 Windows 代理问题的方法。
