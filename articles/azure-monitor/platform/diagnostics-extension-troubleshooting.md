...
Azure 诊断提供可在 Azure 门户中显示的指标数据。 如果在门户中看到数据时遇到问题，请检查 Azure 诊断存储\*帐户中的 WADMetrics 表，以查看是否有相应的指标记录，并确保[资源提供程序](https://docs.microsoft.com/azure/azure-resource-manager/resource-manager-supported-services)Microsoft.Insights 已注册。

此处，表的 PartitionKey 是资源 ID、虚拟机或虚拟机规模集****。 RowKey 是指标名称（也称为性能计数器名称）****。

如果资源 ID 不正确，请检查“诊断配置” > “指标” > “ResourceId”，以查看是否已正确设置资源 ID**** ************。

如果没有特定指标的数据，请检查**诊断配置** > **性能计数器**以查看是否包括指标（性能计数器）。 默认启用以下计数器：
...
如果存储帐户配置正确，请远程访问计算机并验证*诊断插件.exe*和*MonAgentCore.exe*是否正在运行。 如果它们未运行，请按照 Azure 诊断中的步骤[不启动](#azure-diagnostics-is-not-starting)。
...
- ETW 跟踪：远程访问 VM 并安装 PerfView****。  在 PerfView 中，运行**文件** > **用户命令** > **侦听 etwprovder1** > **etwprovider2，** 等等。 侦听命令区分大小写，ETW 提供程序的逗号分隔列表之间不能有空格****。 如果命令未能运行，可选择 Perfview 工具右下角的“日志”按钮，查看尝试运行的内容以及结果****。  假设输入正确，则会弹出一个新窗口。 几秒钟后，即可看到 ETW 跟踪信息。
...
- 确保**诊断监视器跟踪侦听器**在 Web.config 或 app.config 中配置。 默认情况下，这在云服务项目中配置。 然而，某些客户将其注释掉了，导致诊断不收集相关跟踪语句。

- 如果未从 **"启动"或"****运行**"方法写入日志，请确保**诊断监视器跟踪侦听器**位于 app.config 中。 默认情况下，它位于 Web.config 中，但仅适用于在 w3wp.exe 中运行的代码。 因此需将其置于 app.config 中，以捕获在 WaIISHost.exe 中运行的跟踪。

- 确保使用的是 Diagnostics.Trace.TraceXXX，而不是 Diagnostics.Debug.WriteXXX******** 从发布版本中删除了 Debug 语句。

- 确保编译的代码实际上具有**诊断.跟踪行**（使用反射器、ildasm 或 ILSpy 进行验证）。 **诊断.跟踪**命令将从已编译的二进制文件中删除，除非您使用 TRACE 条件编译符号。 这是使用 msbuild 构建项目时常会发生的问题。   
...
- 存储中的数据是否有英文计数器名称。 如果计数器名称不是英文，门户指标图表将无法识别它。 **** 缓解措施：将系统帐户的计算机语言更改为英语。 为此，请选择 **"控制面板** > **区域** > **管理** > **复制设置**"。 接下来，取消选择“欢迎界面和系统帐户”，以免将自定义语言应用到系统帐户****。

- 如果在性能计数器名称中使用通配符 (\*)，则在将性能计数器发送到 Azure 存储接收器时，门户将无法关联已配置和已收集的计数器。 **缓解**：为了确保可以使用通配符，并让门户展开 （\*）， 将性能计数器路由到 Azure 监视器接收器。
