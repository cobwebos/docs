...
> [!IMPORTANT]
> 自定义字段增加了 Log Analytics 工作区中收集的数据量，这可能会增加成本。 有关详细信息，请参阅[使用 Azure Monitor 日志管理使用情况和成本](manage-cost-storage.md#pricing-model)。
...
1. 突出显示示例记录中要用于填充自定义字段的文本。  然后会显示一个对话框，用于命名字段和设置字段的数据类型，以及执行初始提取。  将自动追加字符** \_ CF** 。
2. 单击“提取”**** 以执行已收集记录的分析。  
