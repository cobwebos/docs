...
|DataProcessed|已处理的数据|字节|总计|遍历防火墙的数据量|无维度|
|FirewallHealthState|防火墙运行状况状态|百分比|平均值|指示防火墙的运行状况|状态，原因|
|SNATPortUtilization|SNAT 端口利用率|百分比|平均值|防火墙使用的 SNAT 端口的百分比|无维度|
