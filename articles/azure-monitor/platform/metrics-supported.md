...
|MemoryUsage|内存: 内存用量|字节|Average|服务器进程的内存使用量（在计算清理器内存价格时使用）。 等于计数器 Process\PrivateBytes 加上内存映射的数据的大小，并且将忽略由 xVelocity 内存中分析引擎 (VertiPaq) 映射或分配的超出了 xVelocity 引擎内存限制的任何内存。|ServerResourceType|
...
|Quota|内存: Quota|字节|Average|当前内存配额（字节）。 内存配额也称为内存授予或内存保留。|ServerResourceType|
...
|Duration|网关请求的总持续时间|毫秒|Average|网关请求的总持续时间，以毫秒为单位|位置、主机名|
|容量|容量|Percent|Average|ApiManagement 服务的利用率指标|Location|
...
|Latency|Latency|毫秒|Average|延迟（毫秒）。|ApiName，操作名称、 区域|
...
|每磁盘读取字节数/秒|数据磁盘读取字节数/秒 （已弃用）|每秒计数|Average|监视期间从单个磁盘读取的字节数/秒|SlotId|
|每磁盘写入字节数/秒|数据磁盘写入字节数/秒 （已弃用）|每秒计数|Average|监视期间写入单个磁盘的字节数/秒|SlotId|
|每磁盘读取操作数/秒|数据磁盘读取操作数/秒 （已弃用）|每秒计数|Average|从单个磁盘监视期间的读取 IOPS|SlotId|
|每磁盘写入操作数/秒|数据磁盘写入操作数/秒 （已弃用）|每秒计数|Average|监视期间从单个磁盘写入 IOPS|SlotId|
|每磁盘 QD|数据磁盘 QD （已弃用）|Count|Average|数据磁盘队列深度(或队列长度)|SlotId|
|OS 每磁盘读取字节数/秒|OS 磁盘读取字节数/秒 （已弃用）|每秒计数|Average|在 OS 磁盘监视期间从单个磁盘读取的字节数/秒|无维度|
|OS 每磁盘写入字节数/秒|OS 磁盘写入字节数/秒 （已弃用）|每秒计数|Average|在 OS 磁盘监视期间写入单个磁盘的字节数/秒|无维度|
|OS 每磁盘读取操作数/秒|OS 磁盘读取操作数/秒 （已弃用）|每秒计数|Average|从单个磁盘监视期间 OS 磁盘读取 IOPS|无维度|
|OS 每磁盘写入操作数/秒|OS 磁盘写入操作数/秒 （已弃用）|每秒计数|Average|在 OS 磁盘监视期间从单个磁盘写入 IOPS|无维度|
|OS 每磁盘 QD|OS 磁盘 QD （已弃用）|Count|Average|OS 磁盘队列深度(或队列长度)|无维度|
|数据磁盘读取字节数/秒|数据磁盘读取字节数/秒（预览版）|每秒计数|Average|监视期间从单个磁盘读取的字节数/秒|LUN|
|数据磁盘写入字节数/秒|数据磁盘写入字节数/秒（预览版）|每秒计数|Average|监视期间写入单个磁盘的字节数/秒|LUN|
|数据磁盘读取操作数/秒|数据磁盘读取操作数/秒（预览版）|每秒计数|Average|从单个磁盘监视期间的读取 IOPS|LUN|
|数据磁盘写入操作数/秒|数据磁盘写入操作数/秒（预览版）|每秒计数|Average|监视期间从单个磁盘写入 IOPS|LUN|
|数据磁盘队列深度|数据磁盘队列深度 （预览版）|Count|Average|数据磁盘队列深度(或队列长度)|LUN|
|OS 磁盘读取字节数/秒|OS 磁盘读取字节数/秒（预览版）|每秒计数|Average|在 OS 磁盘监视期间从单个磁盘读取的字节数/秒|无维度|
|OS 磁盘写入字节数/秒|OS 磁盘写入字节数/秒（预览版）|每秒计数|Average|在 OS 磁盘监视期间写入单个磁盘的字节数/秒|无维度|
|OS 磁盘读取操作数/秒|OS 磁盘读取操作数/秒（预览版）|每秒计数|Average|从单个磁盘监视期间 OS 磁盘读取 IOPS|无维度|
|OS 磁盘写入操作数/秒|OS 磁盘写入操作数/秒（预览版）|每秒计数|Average|在 OS 磁盘监视期间从单个磁盘写入 IOPS|无维度|
|OS 磁盘队列深度|OS 磁盘队列深度 （预览版）|Count|Average|OS 磁盘队列深度(或队列长度)|无维度|
|入站的流|入站的流 （预览版）|Count|Average|入站的流的数字的当前流中的入站方向 （转到 VM 的流量）|无维度|
|出站流|出站流 （预览版）|Count|Average|出站流的数字的当前流中的出站方向 （传出 VM 的流量）|无维度|
|入站的流最大的创建速率|入站的流最大的创建速率 （预览版）|每秒计数|Average|入站流量 （流量进入虚拟机） 的最大的创建速率|无维度|
|出站流最大的创建速率|出站流最大的创建速率 （预览版）|每秒计数|Average|最大的创建速率的出站流 （传出 VM 的流量）|无维度|
...
|磁盘读取操作次数/秒|磁盘读取操作次数/秒|每秒计数|Average|磁盘读取 IOPS|VMName|
|磁盘写入操作次数/秒|磁盘写入操作次数/秒|每秒计数|Average|磁盘写入 IOPS|VMName|
|剩余 CPU 信用额度|剩余 CPU 信用额度|Count|Average|可用来集中使用的总信用点数|无维度|
|已用 CPU 信用额度|已用 CPU 信用额度|Count|Average|虚拟机使用的总信用点数|无维度|
|每磁盘读取字节数/秒|数据磁盘读取字节数/秒 （已弃用）|每秒计数|Average|监视期间从单个磁盘读取的字节数/秒|SlotId|
|每磁盘写入字节数/秒|数据磁盘写入字节数/秒 （已弃用）|每秒计数|Average|监视期间写入单个磁盘的字节数/秒|SlotId|
...
|OS 每磁盘写入操作数/秒|OS 磁盘写入操作数/秒 （已弃用）|每秒计数|Average|在 OS 磁盘监视期间从单个磁盘写入 IOPS|无维度|
|OS 每磁盘 QD|OS 磁盘 QD （已弃用）|Count|Average|OS 磁盘队列深度(或队列长度)|无维度|
|数据磁盘读取字节数/秒|数据磁盘读取字节数/秒（预览版）|每秒计数|Average|监视期间从单个磁盘读取的字节数/秒|LUN VMName|
|数据磁盘写入字节数/秒|数据磁盘写入字节数/秒（预览版）|每秒计数|Average|监视期间写入单个磁盘的字节数/秒|LUN VMName|
|数据磁盘读取操作数/秒|数据磁盘读取操作数/秒（预览版）|每秒计数|Average|从单个磁盘监视期间的读取 IOPS|LUN VMName|
|数据磁盘写入操作数/秒|数据磁盘写入操作数/秒（预览版）|每秒计数|Average|监视期间从单个磁盘写入 IOPS|LUN VMName|
|数据磁盘队列深度|数据磁盘队列深度 （预览版）|Count|Average|数据磁盘队列深度(或队列长度)|LUN VMName|
|OS 磁盘读取字节数/秒|OS 磁盘读取字节数/秒（预览版）|每秒计数|Average|在 OS 磁盘监视期间从单个磁盘读取的字节数/秒|VMName|
|OS 磁盘写入字节数/秒|OS 磁盘写入字节数/秒（预览版）|每秒计数|Average|在 OS 磁盘监视期间写入单个磁盘的字节数/秒|VMName|
|OS 磁盘读取操作数/秒|OS 磁盘读取操作数/秒（预览版）|每秒计数|Average|从单个磁盘监视期间 OS 磁盘读取 IOPS|VMName|
|OS 磁盘写入操作数/秒|OS 磁盘写入操作数/秒（预览版）|每秒计数|Average|在 OS 磁盘监视期间从单个磁盘写入 IOPS|VMName|
|OS 磁盘队列深度|OS 磁盘队列深度 （预览版）|Count|Average|OS 磁盘队列深度(或队列长度)|VMName|
|入站的流|入站的流 （预览版）|Count|Average|入站的流的数字的当前流中的入站方向 （转到 VM 的流量）|VMName|
|出站流|出站流 （预览版）|Count|Average|出站流的数字的当前流中的出站方向 （传出 VM 的流量）|VMName|
|入站的流最大的创建速率|入站的流最大的创建速率 （预览版）|每秒计数|Average|入站流量 （流量进入虚拟机） 的最大的创建速率|VMName|
|出站流最大的创建速率|出站流最大的创建速率 （预览版）|每秒计数|Average|最大的创建速率的出站流 （传出 VM 的流量）|VMName|
...
|MemoryUsage|内存用量|字节|Average|总内存使用量（以字节为单位）。|containerName|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|TotalPullCount|总请求计数|Count|Average|总计中拉取的映像数|无维度|
|SuccessfulPullCount|成功的请求计数|Count|Average|成功映像拉取的数量|无维度|
|TotalPushCount|总的推送计数|Count|Average|总共推送映像的次数|无维度|
|SuccessfulPushCount|成功的推送计数|Count|Average|推送次数成功图像|无维度|
...
|storage_used|已用的存储量|字节|Average|已用的存储量|无维度|
...
|d2c.endpoints.latency.builtIn.events|路由：消息/事件的消息延迟|毫秒|Average|消息进入 IoT 中心与遥测消息进入内置终结点（消息/事件）之间的平均延迟时间（毫秒）。 此指标只启动时启用了路由 (https://aka.ms/iotrouting) 的 IoT 中心。|无维度|
...
|browserTimings/networkDuration|页面加载网络连接时间|毫秒|Average|用户请求和网络连接之间的时间。 包括 DNS 查找和传输连接。|无维度|
|browserTimings/processingDuration|客户端处理时间|毫秒|Average|从接收文档的最后一个字节到 DOM 加载完之间的时间。 可能仍在处理异步请求。|无维度|
|browserTimings/receiveDuration|接收响应时间|毫秒|Average|第一个和最后一个字节之间的时间，或直至断开连接的时间。|无维度|
|browserTimings/sendDuration|发送请求时间|毫秒|Average|网络连接和接收第一个字节之间的时间。|无维度|
|browserTimings/totalDuration|浏览器页面加载时间|毫秒|Average|从用户请求一直到 DOM、样式表、脚本和映像加载完之间的时间。|无维度|
...
|performanceCounters/exceptionsPerSecond|异常率|每秒计数|Average|报告给窗口的已处理和未处理的异常的计数，这些异常包括 .NET 异常和转换为 .NET 异常的非托管异常。|cloud/roleInstance|
|performanceCounters/processIOBytesPerSecond|进程 IO 率|每秒字节数|Average|每秒读取和写入文件、网络和设备的总字节数。|cloud/roleInstance|
...
|performanceCounters/processPrivateBytes|进程专用字节|字节|Average|以独占方式分配给受监视应用程序进程的内存。|cloud/roleInstance|
...
| ContinuousExportNumOfRecordsExported | 连续导出中导出的记录数 | Count | 总计 | 导出过程导出操作中写入每个存储项目的记录数  | 无 |
| ExportUtilization | 导出利用率 | Percent | 最大值 | 导出利用率 | 无 |
| ContinuousExportPendingCount | 挂起计数的连续导出 | Count | 最大值 | 挂起持续数导出已准备好执行作业 | 无 |
| ContinuousExportMaxLatenessMinutes | 连续导出最大延迟分钟 | Count | 最大值 | 以分钟为单位的所有连续导出的最长的时间是挂起和已准备好执行 | 无 |
...
|失败的运行次数|失败的运行次数|Count|总计|此工作区失败的运行数|应用场景|
...
|Average_Total Bytes|字节数总计|Count|Average|Average_Total Bytes|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|Average_Users|用户|Count|Average|Average_Users|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
|Average_Avg. 磁盘秒数/读取|平均磁盘秒数/读取|Count|Average|Average_Avg. 磁盘秒数/读取|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
|Average_Avg. 磁盘秒数/写入|平均磁盘秒数/写入|Count|Average|Average_Avg. 磁盘秒数/写入|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|Average_Free Megabytes|可用 MB 数|Count|Average|Average_Free Megabytes|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
|Average_% Free Space|可用空间百分比|Count|Average|Average_% Free Space|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
|Average_Available MBytes|可用兆字节数|Count|Average|Average_Available MBytes|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|ActiveMessages|队列/主题中的活动消息计数。 （预览版）|Count|Average|队列/主题中的活动消息计数。 （预览版）|EntityName|
...
|workers_percent|辅助角色百分比|Percent|Average|辅助角色百分比|无维度|
...
|allocated_data_storage|已分配的数据空间|字节|Average|已分配的数据空间|无维度|
...
|storage_limit|数据最大大小|字节|Average|存储限制|无维度|
|eDTU_used|已用的 eDTU|Count|Average|已用的 eDTU|无维度|
|storage_used|已用数据空间|字节|Average|已用的存储量|无维度|
|xtp_storage_percent|内存中 OLTP 存储百分比|Percent|Average|内存中 OLTP 存储百分比|无维度|
|cpu_limit|CPU 限制|Count|Average|CPU 限制|无维度|
|cpu_used|已用 CPU|Count|Average|已用 CPU|无维度|
|allocated_data_storage|已分配的数据空间|字节|Average|已分配的数据空间|无维度|
...
|句柄数|句柄计数|Count|Average|句柄计数|实例|
|线程数|线程计数|Count|Average|线程计数|实例|
|PrivateBytes|专用字节|字节|Average|专用字节|实例|
...
|TotalFrontEnds|前端总数|Count|Average|前端总数|无维度|
|SmallAppServicePlanInstances|小型应用服务计划工作线程数|Count|Average|小型应用服务计划工作线程数|无维度|
|MediumAppServicePlanInstances|中型应用服务计划工作线程数|Count|平均值|中型应用服务计划工作线程数|无维度|
|LargeAppServicePlanInstances|大型应用服务计划工作线程数|Count|Average|大型应用服务计划工作线程数|无维度|

## <a name="microsoftwebhostingenvironmentsworkerpools"></a>Microsoft.Web/hostingEnvironments/workerPools

|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|WorkersTotal|工作线程总数|Count|Average|工作线程总数|无维度|
|WorkersAvailable|可用工作线程数|Count|Average|可用工作线程数|无维度|
|WorkersUsed|使用的工作线程数|Count|Average|使用的工作线程数|无维度|
|CpuPercentage|CPU 百分比|Percent|Average|CPU 百分比|实例|
|MemoryPercentage|内存百分比|Percent|Average|内存百分比|实例|
