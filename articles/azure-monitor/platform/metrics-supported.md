...
2. 使用[诊断设置](diagnostic-settings.md)将平台指标路由到 
    - Azure 存储
    - Azure Monitor 日志 (，因而 Log Analytics) 
    - 事件中心，即如何将它们获取到非 Microsoft 系统 

使用诊断设置是路由指标的最简单方法，但存在一些限制： 

- **不可导出的部分**-所有指标都可使用 REST API 进行导出，但有些指标无法使用诊断设置导出，因为 Azure Monitor 后端中存在复杂的情况。 通过下表中的*诊断设置可导出*的列列出了可通过这种方式导出的指标。  

- **多维指标**-当前不支持通过诊断设置将多维度指标发送到其他位置。 多维指标将按平展后的单维指标导出，并跨维值聚合。 例如：可以基于每个队列级别浏览和绘制事件中心上的“传入消息”指标。 但是，当通过诊断设置导出时，该指标将表示为事件中心的所有队列中的所有传入消息。

## <a name="guest-os-and-host-os-metrics"></a>来宾操作系统和主机操作系统指标

> [!WARNING]
> 在 Azure 虚拟机、Service Fabric 和云服务中运行的来宾操作系统)  (来宾操作系统的指标**不**会在此处列出。 必须通过在或来宾操作系统中运行的一个或多个代理收集来宾 OS 指标。  来宾 OS 指标包括跟踪来宾 CPU 百分比或内存使用率的性能计数器，两者都经常用于自动缩放或警报。 
>
> **主机操作系统度量值可用，如下所示。** 它们不相同。 主机操作系统指标与托管来宾操作系统会话的 Hyper-v 会话相关。 

> [!TIP]
> 最佳做法是使用和配置[Azure 诊断扩展](diagnostics-extension-overview.md)，将来宾 OS 性能指标发送到存储平台指标的同一 Azure Monitor 指标数据库。 该扩展通过[自定义指标](metrics-custom-overview.md)API 路由来宾 OS 指标。 然后，你可以对诸如平台指标之类的来宾 OS 指标进行图表、警报和其他使用。 或者，还可以使用 Log Analytics 代理向 Azure Monitor 日志/Log Analytics 发送来宾 OS 指标。 可以在其中与非指标数据一起查询这些指标。 
...
> [!IMPORTANT] 
> 此最新更新将添加一个新列，并将度量值重新排序为字母。 添加信息意味着下表的底部可能有水平滚动条，具体取决于浏览器窗口的宽度。 如果您认为缺少信息，请使用滚动条来查看表的完整信息。
...
|---|---|---|---|---|---|---|
|CleanerCurrentPrice|是|内存:清理器当前价格|Count|平均值|内存的当前价格，$/字节/时间，标准化为 1000。|ServerResourceType|
...
|CommandPoolBusyThreads|是|线程:命令池繁忙线程数|Count|平均值|命令线程池中的繁忙线程数。|ServerResourceType|
|CommandPoolIdleThreads|是|线程:命令池空闲线程数|Count|平均值|命令线程池中的空闲线程数。|ServerResourceType|
|CommandPoolJobQueueLength|是|命令池作业队列长度|计数|平均值|命令线程池队列中的作业数。|ServerResourceType|
|CurrentConnections|是|连接:当前连接数|Count|平均值|当前已建立的客户端连接的数量。|ServerResourceType|
|CurrentUserSessions|是|当前用户会话数|Count|平均值|当前已建立的用户会话数。|ServerResourceType|
|LongParsingBusyThreads|是|线程:长分析繁忙线程数|Count|平均值|长分析线程池中的繁忙线程数。|ServerResourceType|
|LongParsingIdleThreads|是|线程:长分析空闲线程数|Count|平均值|长分析线程池中的空闲线程数。|ServerResourceType|
|LongParsingJobQueueLength|是|线程:长分析作业队列长度|Count|平均值|长分析线程池队列中的作业数。|ServerResourceType|
...
|mashup_engine_qpu_metric|是|M 引擎 QPU|Count|平均值|糅合引擎进程的 QPU 使用率|ServerResourceType|
...
|ProcessingPoolBusyNonIOThreads|是|线程:处理池繁忙非 I/O 线程数|Count|平均值|处理线程池中正在运行非 I/O 作业的线程数。|ServerResourceType|
|ProcessingPoolIdleIOJobThreads|是|线程:处理池空闲 I/O 作业线程数|Count|平均值|处理线程池中可用于 I/O 作业的空闲线程数。|ServerResourceType|
|ProcessingPoolIdleNonIOThreads|是|线程:处理池空闲非 I/O 线程数|Count|平均值|处理线程池中专用于非 I/O 作业的空闲线程数。|ServerResourceType|
|ProcessingPoolIOJobQueueLength|是|线程:处理池 I/O 作业队列长度|Count|平均值|处理线程池队列中的 I/O 作业数。|ServerResourceType|
|ProcessingPoolJobQueueLength|是|处理池作业队列长度|Count|平均值|处理线程池的队列中的非 I/O 作业数。|ServerResourceType|
|qpu_metric|是|QPU|计数|平均值|QPU。 S1 范围为 0-100，S2 范围为 0-200，S4 范围为 0-400|ServerResourceType|
|QueryPoolBusyThreads|是|查询池忙线程数|Count|平均值|查询线程池中的忙线程数。|ServerResourceType|
|QueryPoolIdleThreads|是|线程:查询池空闲线程数|Count|平均值|处理线程池中可用于 I/O 作业的空闲线程数。|ServerResourceType|
|QueryPoolJobQueueLength|是|线程:查询池作业队列长度|Count|平均值|查询线程池队列中的作业数。|ServerResourceType|
|Quota|是|内存:Quota|字节|平均值|当前内存配额（字节）。 内存配额也称为内存授予或内存预留。|ServerResourceType|
|QuotaBlocked|是|内存:阻止的配额|Count|平均值|在其他内存配额被释放之前已阻止的当前的配额请求数。|ServerResourceType|
...
|ShortParsingBusyThreads|是|线程:短分析繁忙线程数|Count|平均值|短分析线程池中的繁忙线程数。|ServerResourceType|
|ShortParsingIdleThreads|是|线程:短分析空闲线程数|Count|平均值|短分析线程池中的空闲线程数。|ServerResourceType|
...
|TotalConnectionFailures|是|连接失败总数|Count|平均值|失败的连接尝试总数。|ServerResourceType|
|TotalConnectionRequests|是|连接请求总数|Count|平均值|连接请求总数。 这些是到达的请求。|ServerResourceType|
...
|Duration|是|网关请求的总持续时间|毫秒|平均值|网关请求的总持续时间，以毫秒为单位|位置、主机名|
|EventHubDroppedEvents|是|删除的事件中心事件数|Count|总计|由于达到队列大小限制而跳过的事件数|位置|
|EventHubRejectedEvents|是|拒绝的事件中心事件数|Count|总计|拒绝的事件中心事件（配置不当或未授权）数|位置|
|EventHubSuccessfulEvents|是|成功的事件中心事件数|Count|总计|成功的事件中心事件数|位置|
|EventHubThrottledEvents|是|限制的事件中心事件数|Count|总计|限制的事件中心事件数|位置|
|EventHubTimedoutEvents|是|超时的事件中心事件数|Count|总计|超时的事件中心事件数|位置|
|EventHubTotalBytesSent|是|事件中心事件大小|字节|总计|事件中心事件的总大小，以字节为单位|位置|
|EventHubTotalEvents|是|事件中心事件总数|Count|总计|发送到事件中心的事件数|位置|
|EventHubTotalFailedEvents|是|失败的事件中心事件数|Count|总计|失败的事件中心事件数|位置|
|FailedRequests|是|失败的网关请求数（已弃用）|Count|总计|网关请求失败数 - 请改用包含 GatewayResponseCodeCategory 维度的多维请求指标|位置、主机名|
|NetworkConnectivity|是|资源的网络连接状态（预览）|计数|平均值|API 管理服务中从属资源类型的网络连接状态|位置，ResourceType|
|OtherRequests|是|其他网关请求数（已弃用）|Count|总计|其他网关请求数 - 请改用包含 GatewayResponseCodeCategory 维度的多维请求指标|位置、主机名|
|请求|是|请求|Count|总计|包含多个维度的网关请求指标|Location、Hostname、LastErrorReason、BackendResponseCode、GatewayResponseCode、BackendResponseCodeCategory、GatewayResponseCodeCategory|
|SuccessfulRequests|是|成功的网关请求数（已弃用）|Count|总计|成功的网关请求数 - 请改用包含 GatewayResponseCodeCategory 维度的多维请求指标|位置、主机名|
|TotalRequests|是|网关请求总数（已弃用）|Count|总计|网关请求数 - 请改用包含 GatewayResponseCodeCategory 维度的多维请求指标|位置、主机名|
|UnauthorizedRequests|是|未经授权的网关请求数（已弃用）|Count|总计|未经授权的网关请求数 - 请改用包含 GatewayResponseCodeCategory 维度的多维请求指标|位置、主机名|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|HttpIncomingRequestCount|是|HttpIncomingRequestCount|计数|计数|传入的 http 请求总数。|StatusCode，身份验证|
|HttpIncomingRequestDuration|是|HttpIncomingRequestDuration|计数|平均值|http 请求延迟。|StatusCode，身份验证|
|ThrottledHttpRequestCount|是|ThrottledHttpRequestCount|Count|Count|限制 http 请求。|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|jvm. 数据大小|是|jvm. 数据大小|字节|平均值|完全 GC 后的旧代内存池大小|部署、AppName、Pod|
|jvm. 最大数据大小|是|jvm. 最大数据大小|字节|平均值|旧代内存池的最大大小|部署、AppName、Pod|
|jvm。已分配|是|jvm。已分配|字节|最大值|运行一个 GC 之后、运行下一个 GC 之前，新代内存池大小增加的递增量|部署、AppName、Pod|
|jvm。已升级|是|jvm。已升级|字节|最大值|GC 之前、GC 之后，旧代内存池大小正增加的计数|部署、AppName、Pod|
|jvm。 total. count|是|jvm。 total. count|Count|总计|GC 暂停计数|部署、AppName、Pod|
|jvm： total. time|是|jvm： total. time|毫秒|总计|GC 暂停总时间|部署、AppName、Pod|
|jvm。已提交|是|jvm。已提交|字节|平均值|分配给 JVM 的内存（字节）|部署、AppName、Pod|
|jvm. max|是|jvm. max|字节|最大值|可用于内存管理的最大内存量（以字节为单位）|部署、AppName、Pod|
|jvm。已使用|是|jvm。已使用|字节|平均值|已使用的应用内存（字节）|部署、AppName、Pod|
|进程。使用情况|是|进程。使用情况|百分比|平均值|应用 JVM CPU 使用率百分比|部署、AppName、Pod|
|system.object。用法|是|system.object。用法|百分比|平均值|整个系统的最近 CPU 使用率|部署、AppName、Pod|
|tomcat. 错误|是|tomcat. 错误|Count|总计|Tomcat 全局错误|部署、AppName、Pod|
|tomcat。已接收|是|tomcat。已接收|字节|总计|Tomcat 收到的总字节数|部署、AppName、Pod|
|tomcat. avg. avg. avg. avg. time|是|tomcat. avg. avg. avg. avg. time|毫秒|平均值|Tomcat 请求平均时间|部署、AppName、Pod|
|tomcat. max|是|tomcat. max|毫秒|最大值|Tomcat 请求最大时间|部署、AppName、Pod|
|tomcat. total. count|是|tomcat. total. count|Count|总计|Tomcat 请求总数|部署、AppName、Pod|
|tomcat. 总计. 时间|是|tomcat. 总计. 时间|毫秒|总计|Tomcat 请求总时间|部署、AppName、Pod|
|tomcat. 已发送|是|tomcat. 已发送|字节|总计|Tomcat 发送的总字节数|部署、AppName、Pod|
|tomcat 会话。当前|是|tomcat 会话。当前|Count|总计|Tomcat 会话活动计数|部署、AppName、Pod|
|tomcat。最大值|是|tomcat。最大值|Count|总计|Tomcat 会话最大活动计数|部署、AppName、Pod|
|tomcat。最大值|是|tomcat。最大值|毫秒|最大值|Tomcat 会话最大活动时间|部署、AppName、Pod|
|tomcat。已创建|是|tomcat。已创建|Count|总计|Tomcat 创建的会话数|部署、AppName、Pod|
|tomcat 会话。已过期|是|tomcat 会话。已过期|Count|总计|Tomcat 过期的会话数|部署、AppName、Pod|
|tomcat 会话。已拒绝|是|tomcat 会话。已拒绝|计数|总计|Tomcat 拒绝的会话数|部署、AppName、Pod|
|tomcat.threads.config 最大值|是|tomcat.threads.config 最大值|Count|总计|Tomcat 配置最大线程计数|部署、AppName、Pod|
|tomcat。当前|是|tomcat。当前|Count|总计|Tomcat 当前线程计数|部署、AppName、Pod|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|TotalJob|是|作业总数|Count|总计|作业总数|Runbook、状态|
...
|TotalUpdateDeploymentRuns|是|更新部署运行总数|Count|总计|软件更新部署运行总数|SoftwareUpdateConfigurationName、Status|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|CoreCount|否|专用核心计数|Count|总计|批处理帐户中的专用核心总数|无维度|
|CreatingNodeCount|否|正在创建的节点计数|Count|总计|正在创建的节点数目|无维度|
|IdleNodeCount|否|空闲节点计数|计数|总计|空闲节点数目|无维度|
|JobDeleteCompleteEvent|是|作业删除完成事件数|Count|总计|已成功删除的作业总数。|jobId|
|JobDeleteStartEvent|是|作业删除启动事件数|Count|总计|已请求删除的作业总数。|jobId|
|JobDisableCompleteEvent|是|作业禁用完成事件数|Count|总计|已成功禁用的作业总数。|jobId|
|JobDisableStartEvent|是|作业禁用启动事件数|Count|总计|已请求禁用的作业总数。|jobId|
|JobStartEvent|是|作业启动事件数|Count|总计|已成功启动的作业总数。|jobId|
|JobTerminateCompleteEvent|是|作业终止完成事件数|Count|总计|已成功终止的作业总数。|jobId|
|JobTerminateStartEvent|是|作业终止启动事件数|计数|总计|已请求终止的作业总数。|jobId|
|LeavingPoolNodeCount|否|正在退出池的节点计数|Count|总计|正在退出池的节点数目|无维度|
|LowPriorityCoreCount|否|低优先级核心计数|Count|总计|批处理帐户中的低优先级核心总数|无维度|
|OfflineNodeCount|否|脱机节点计数|Count|总计|脱机节点数目|无维度|
|PoolCreateEvent|是|池创建事件数|Count|总计|已创建的池总数|poolId|
|PoolDeleteCompleteEvent|是|池删除完成事件数|Count|总计|已完成的池删除活动总数|poolId|
|PoolDeleteStartEvent|是|池删除启动事件数|计数|总计|已启动的池删除活动总数|poolId|
|PoolResizeCompleteEvent|是|池调整大小完成事件数|Count|总计|已完成的池调整大小活动总数|poolId|
|PoolResizeStartEvent|是|池调整大小启动事件数|Count|总计|已启动的池调整大小活动总数|poolId|
|PreemptedNodeCount|否|已占用节点计数|Count|总计|已占用节点数|无维度|
|RebootingNodeCount|否|正在重新启动的节点计数|Count|总计|正在重新启动的节点数目|无维度|
|ReimagingNodeCount|否|正在重置映像的节点计数|Count|总计|正在重置映像的节点数目|无维度|
|RunningNodeCount|否|正在运行的节点计数|Count|总计|正在运行的节点数目|无维度|
|StartingNodeCount|否|正在启动的节点计数|Count|总计|正在启动的节点数目|无维度|
|StartTaskFailedNodeCount|否|启动任务失败的节点计数|Count|总计|启动任务失败的节点数目|无维度|
|TaskCompleteEvent|是|任务完成事件数|Count|总计|已完成的任务总数|poolId，jobId|
|TaskFailEvent|是|任务失败事件数|Count|总计|处于失败状态的已完成任务总数|poolId，jobId|
|TaskStartEvent|是|任务启动事件数|Count|总计|已启动的任务总数|poolId，jobId|
|TotalLowPriorityNodeCount|否|低优先级节点计数|Count|总计|批处理帐户中的低优先级节点总数|无维度|
|TotalNodeCount|否|专用节点计数|计数|总计|批处理帐户中的专用节点总数|无维度|
|UnusableNodeCount|否|不可用的节点计数|Count|总计|不可用的节点数目|无维度|
|WaitingForStartTaskNodeCount|否|正在等待启动任务的节点计数|Count|总计|正在等待启动任务完成的节点数目|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|活动核心数|是|活动核心数|Count|平均值|活动核心数|方案，ClusterName|
|活动节点数|是|活动节点数|计数|平均值|正在运行的节点数目|方案，ClusterName|
|空闲核心数|是|空闲核心数|Count|平均值|空闲核心数|方案，ClusterName|
|空闲节点数|是|空闲节点数|Count|平均值|空闲节点数目|方案，ClusterName|
|已完成的作业|是|已完成的作业|Count|总计|已完成的作业数|方案，ClusterName，ResultType|
|提交的作业|是|提交的作业|Count|总计|提交的作业数|方案，ClusterName|
|正在退出的核心数|是|正在退出的核心数|Count|平均值|正在退出的核心数|方案，ClusterName|
|正在退出的节点数|是|正在退出的节点数|Count|平均值|正在退出的节点数|方案，ClusterName|
|占用的核心数|是|占用的核心数|计数|平均值|占用的核心数|方案，ClusterName|
|占用的节点数|是|占用的节点数|Count|平均值|已占用节点数|方案，ClusterName|
|配额利用率百分比|是|配额利用率百分比|Count|平均值|已利用的配额百分比|方案、ClusterName、VmFamilyName、VmPriority|
|核心总数|是|核心总数|Count|平均值|核心总数|方案，ClusterName|
|节点总数|是|节点总数|Count|平均值|节点总数|方案，ClusterName|
|不可用的核心数|是|不可用的核心数|Count|平均值|不可用的核心数|方案，ClusterName|
|不可用的节点数|是|不可用的节点数|Count|平均值|不可用的节点数目|方案，ClusterName|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|BroadcastProcessedCount|是|广播处理计数|Count|平均值|处理的事务数|节点、通道、类型、状态|
|ConnectionAccepted|是|已接受的连接数|Count|总计|已接受的连接数|节点|
|ConnectionActive|是|活动连接数|Count|平均值|活动连接数|节点|
|ConnectionHandled|是|已处理的连接数|Count|总计|已处理的连接数|节点|
|ConsensusEtcdraftCommittedBlockNumber|是|已提交共识 Etcdraft 块号|计数|平均值|已提交的最新块的块号|Node、信道|
|CpuUsagePercentageInDouble|是|CPU 使用率百分比|百分比|最大值|CPU 使用率百分比|节点|
|EndorserEndorsementFailures|是|Endorser 认可失败|Count|平均值|签署失败的次数。|Node、信道、chaincode、chaincodeerror|
|GossipLeaderElectionLeader|是|Gossip 领袖选举负责人|Count|总计|对等是领导 (1) 或 (0) |Node、信道|
|GossipMembershipTotalPeersKnown|是|Gossip 成员资格总数已知|Count|平均值|已知对等方总数|Node、信道|
|GossipStateHeight|是|Gossip 状态高度|Count|平均值|当前分类帐高度|Node、信道|
|IOReadBytes|是|IO 读取字节数|字节|总计|IO 读取字节数|节点|
|IOWriteBytes|是|IO 写入字节数|字节|总计|IO 写入字节数|节点|
|LedgerTransactionCount|是|帐目交易计数|Count|平均值|处理的事务数|Node、信道、transaction_type、chaincode、validation_code|
...
|PendingTransactions|是|挂起的事务数|Count|平均值|挂起的事务数|节点|
|ProcessedBlocks|是|已处理的块数|Count|总计|已处理的块数|节点|
|ProcessedTransactions|是|已处理的事务数|Count|总计|已处理的事务数|节点|
|QueuedTransactions|是|已排队的事务数|计数|平均值|已排队的事务数|节点|
|RequestHandled|是|已处理的请求数|Count|总计|已处理的请求数|节点|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|cachehits|是|缓存命中数|Count|总计||ShardId|
|cachehits0|是|缓存命中数(分片 0)|Count|总计||无维度|
|cachehits1|是|缓存命中数(分片 1)|Count|总计||无维度|
|cachehits2|是|缓存命中数(分片 2)|Count|总计||无维度|
|cachehits3|是|缓存命中数(分片 3)|Count|总计||无维度|
|cachehits4|是|缓存命中数(分片 4)|Count|总计||无维度|
|cachehits5|是|缓存命中数(分片 5)|计数|总计||无维度|
|cachehits6|是|缓存命中数(分片 6)|Count|总计||无维度|
|cachehits7|是|缓存命中数(分片 7)|Count|总计||无维度|
|cachehits8|是|缓存命中数(分片 8)|Count|总计||无维度|
|cachehits9|是|缓存命中数(分片 9)|Count|总计||无维度|
|cacheLatency|是|缓存延迟毫秒数（预览）|Count|平均值||ShardId|
|cachemisses|是|缓存未命中数|Count|总计||ShardId|
|cachemisses0|是|缓存未命中数(分片 0)|Count|总计||无维度|
|cachemisses1|是|缓存未命中数(分片 1)|Count|总计||无维度|
|cachemisses2|是|缓存未命中数(分片 2)|Count|总计||无维度|
|cachemisses3|是|缓存未命中数(分片 3)|计数|总计||无维度|
|cachemisses4|是|缓存未命中数(分片 4)|Count|总计||无维度|
|cachemisses5|是|缓存未命中数(分片 5)|Count|总计||无维度|
|cachemisses6|是|缓存未命中数(分片 6)|Count|总计||无维度|
|cachemisses7|是|缓存未命中数(分片 7)|Count|总计||无维度|
|cachemisses8|是|缓存未命中数(分片 8)|计数|总计||无维度|
|cachemisses9|是|缓存未命中数(分片 9)|Count|总计||无维度|
...
|connectedclients0|是|连接的客户端数(分片 0)|Count|最大值||无维度|
|connectedclients1|是|连接的客户端数(分片 1)|Count|最大值||无维度|
|connectedclients2|是|连接的客户端数(分片 2)|Count|最大值||无维度|
|connectedclients3|是|连接的客户端数(分片 3)|Count|最大值||无维度|
|connectedclients4|是|连接的客户端数(分片 4)|Count|最大值||无维度|
|connectedclients5|是|连接的客户端数(分片 5)|Count|最大值||无维度|
|connectedclients6|是|连接的客户端数(分片 6)|Count|最大值||无维度|
|connectedclients7|是|连接的客户端数(分片 7)|Count|最大值||无维度|
|connectedclients8|是|连接的客户端数(分片 8)|Count|最大值||无维度|
|connectedclients9|是|连接的客户端数(分片 9)|计数|最大值||无维度|
|错误|是|错误|Count|最大值||ShardId, ErrorType|
|evictedkeys|是|逐出的密钥数|Count|总计||ShardId|
|evictedkeys0|是|逐出的密钥数(分片 0)|Count|总计||无维度|
|evictedkeys1|是|逐出的密钥数(分片 1)|Count|总计||无维度|
|evictedkeys2|是|逐出的密钥数(分片 2)|计数|总计||无维度|
|evictedkeys3|是|逐出的密钥数(分片 3)|Count|总计||无维度|
|evictedkeys4|是|逐出的密钥数(分片 4)|Count|总计||无维度|
|evictedkeys5|是|逐出的密钥数(分片 5)|Count|总计||无维度|
|evictedkeys6|是|逐出的密钥数(分片 6)|Count|总计||无维度|
|evictedkeys7|是|逐出的密钥数(分片 7)|Count|总计||无维度|
|evictedkeys8|是|逐出的密钥数(分片 8)|Count|总计||无维度|
|evictedkeys9|是|逐出的密钥数(分片 9)|Count|总计||无维度|
|expiredkeys|是|过期的密钥数|Count|总计||ShardId|
|expiredkeys0|是|过期的密钥数(分片 0)|Count|总计||无维度|
|expiredkeys1|是|过期的密钥数(分片 1)|计数|总计||无维度|
|expiredkeys2|是|过期的密钥数(分片 2)|Count|总计||无维度|
|expiredkeys3|是|过期的密钥数(分片 3)|Count|总计||无维度|
|expiredkeys4|是|过期的密钥数(分片 4)|Count|总计||无维度|
|expiredkeys5|是|过期的密钥数(分片 5)|Count|总计||无维度|
|expiredkeys6|是|过期的密钥数(分片 6)|计数|总计||无维度|
|expiredkeys7|是|过期的密钥数(分片 7)|Count|总计||无维度|
|expiredkeys8|是|过期的密钥数(分片 8)|Count|总计||无维度|
|expiredkeys9|是|过期的密钥数(分片 9)|Count|总计||无维度|
|getcommands|是|获取数|Count|总计||ShardId|
|getcommands0|是|Get 数(分片 0)|Count|总计||无维度|
|getcommands1|是|Get 数(分片 1)|Count|总计||无维度|
|getcommands2|是|Get 数(分片 2)|Count|总计||无维度|
|getcommands3|是|Get 数(分片 3)|Count|总计||无维度|
|getcommands4|是|Get 数(分片 4)|Count|总计||无维度|
|getcommands5|是|Get 数(分片 5)|计数|总计||无维度|
|getcommands6|是|Get 数(分片 6)|Count|总计||无维度|
|getcommands7|是|Get 数(分片 7)|Count|总计||无维度|
|getcommands8|是|Get 数(分片 8)|Count|总计||无维度|
|getcommands9|是|Get 数(分片 9)|Count|总计||无维度|
|operationsPerSecond|是|每秒操作数|Count|最大值||ShardId|
|operationsPerSecond0|是|每秒操作数（分片 0）|Count|最大值||无维度|
|operationsPerSecond1|是|每秒操作数（分片 1）|Count|最大值||无维度|
|operationsPerSecond2|是|每秒操作数（分片 2）|Count|最大值||无维度|
|operationsPerSecond3|是|每秒操作数（分片 3）|Count|最大值||无维度|
|operationsPerSecond4|是|每秒操作数（分片 4）|计数|最大值||无维度|
|operationsPerSecond5|是|每秒操作数（分片 5）|Count|最大值||无维度|
|operationsPerSecond6|是|每秒操作数（分片 6）|Count|最大值||无维度|
|operationsPerSecond7|是|每秒操作数（分片 7）|Count|最大值||无维度|
|operationsPerSecond8|是|每秒操作数（分片 8）|Count|最大值||无维度|
...
|setcommands|是|设置数|Count|总计||ShardId|
|setcommands0|是|Set 数(分片 0)|Count|总计||无维度|
|setcommands1|是|Set 数(分片 1)|Count|总计||无维度|
|setcommands2|是|Set 数(分片 2)|Count|总计||无维度|
|setcommands3|是|Set 数(分片 3)|Count|总计||无维度|
|setcommands4|是|Set 数(分片 4)|Count|总计||无维度|
|setcommands5|是|Set 数(分片 5)|Count|总计||无维度|
|setcommands6|是|Set 数(分片 6)|Count|总计||无维度|
|setcommands7|是|Set 数(分片 7)|Count|总计||无维度|
|setcommands8|是|Set 数(分片 8)|Count|总计||无维度|
|setcommands9|是|Set 数(分片 9)|Count|总计||无维度|
|totalcommandsprocessed|是|总操作数|Count|总计||ShardId|
|totalcommandsprocessed0|是|总操作数(分片 0)|Count|总计||无维度|
|totalcommandsprocessed1|是|总操作数(分片 1)|Count|总计||无维度|
|totalcommandsprocessed2|是|总操作数(分片 2)|Count|总计||无维度|
|totalcommandsprocessed3|是|总操作数(分片 3)|Count|总计||无维度|
|totalcommandsprocessed4|是|总操作数(分片 4)|Count|总计||无维度|
|totalcommandsprocessed5|是|总操作数(分片 5)|Count|总计||无维度|
|totalcommandsprocessed6|是|总操作数(分片 6)|Count|总计||无维度|
|totalcommandsprocessed7|是|总操作数(分片 7)|Count|总计||无维度|
|totalcommandsprocessed8|是|总操作数(分片 8)|Count|总计||无维度|
|totalcommandsprocessed9|是|总操作数(分片 9)|Count|总计||无维度|
|totalkeys|是|总密钥数|Count|最大值||ShardId|
|totalkeys0|是|总密钥数（分片 0）|Count|最大值||无维度|
|totalkeys1|是|总密钥数（分片 1）|Count|最大值||无维度|
|totalkeys2|是|总密钥数（分片 2）|Count|最大值||无维度|
|totalkeys3|是|总密钥数（分片 3）|Count|最大值||无维度|
|totalkeys4|是|总密钥数（分片 4）|Count|最大值||无维度|
|totalkeys5|是|总密钥数（分片 5）|Count|最大值||无维度|
|totalkeys6|是|总密钥数（分片 6）|Count|最大值||无维度|
|totalkeys7|是|总密钥数（分片 7）|Count|最大值||无维度|
|totalkeys8|是|总密钥数（分片 8）|Count|最大值||无维度|
|totalkeys9|是|总密钥数（分片 9）|Count|最大值||无维度|
...
|BlobCount|否|Blob 计数|Count|平均值|存储帐户的 Blob 服务中的 Blob 数。|BlobType、Tier|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|可用性|是|可用性|百分比|平均值|存储服务或指定的 API 操作的可用性百分比。 可用性通过由 TotalBillableRequests 值除以适用的请求数（其中包括引发意外错误的请求）计算得出。 所有意外错误都会导致存储服务或指定的 API 操作的可用性下降。|GeoType、ApiName、Authentication、文件共享|
|流出量|是|流出量|字节|总计|流出的数据量（以字节为单位）。 此数字包括从外部客户端到 Azure 存储流出的数据量，以及流出 Azure 中的数据量。 因此，此数字不反映计费的流出量。|GeoType、ApiName、Authentication、文件共享|
|FileCapacity|否|文件容量|字节|平均值|存储帐户的文件服务使用的存储量（以字节为单位）。|FileShare|
|FileCount|否|文件计数|Count|平均值|存储帐户的文件服务中的文件数。|FileShare|
...
|流入量|是|流入量|字节|总计|流入的数据量（以字节为单位）。 此数字包括从外部客户端到 Azure 存储流入的数据量，以及流入 Azure 中的数据量。|GeoType、ApiName、Authentication、文件共享|
|SuccessE2ELatency|是|成功 E2E 延迟|毫秒|平均值|向存储服务或指定的 API 操作发出的成功请求的端到端延迟（以毫秒为单位）。 此值包括在 Azure 存储中读取请求、发送响应和接收响应确认所需的处理时间。|GeoType、ApiName、Authentication、文件共享|
|SuccessServerLatency|是|成功服务器延迟|毫秒|平均值|由 Azure 存储用于处理成功请求的延迟（以毫秒为单位）。 此值不包括 SuccessE2ELatency 中指定的网络延迟。|GeoType、ApiName、Authentication、文件共享|
|事务|是|事务|Count|总计|向存储服务或指定的 API 操作发出的请求数。 此数值包括成功和失败的请求数，以及引发错误的请求数。 针对不同类型的响应数使用 ResponseType 维度。|ResponseType、GeoType、ApiName、Authentication、文件共享|
...
|QueueCount|是|队列计数|Count|平均值|存储帐户的队列服务中的队列数。|无维度|
...
|TableCount|是|表计数|Count|平均值|存储帐户的表服务中的表数。|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|BlockedCalls|是|阻止的调用数|Count|总计|超过速率或配额限制的调用数。|ApiName、OperationName、Region|
|CharactersTrained|是|训练的字符数|Count|总计|训练的字符总数。|ApiName、OperationName、Region|
|CharactersTranslated|是|转换的字符|Count|总计|传入的文本请求中的字符总数。|ApiName、OperationName、Region|
|ClientErrors|是|客户端错误数|Count|总计|引发客户端错误（HTTP 响应代码 4xx）的调用数。|ApiName、OperationName、Region|
...
|ProcessedImages|是|已处理的图像|计数|总计| 图像处理的事务数。|ApiName、功能、UsageChannel、Region|
|ServerErrors|是|服务器错误数|Count|总计|引发服务内部错误（HTTP 响应代码 5xx）的调用数。|ApiName、OperationName、Region|
|SpeechSessionDuration|是|语音会话持续时间|秒|总计|语音会话的总持续时间（以秒计）。|ApiName、OperationName、Region|
|SuccessfulCalls|是|成功调用数|Count|总计|成功调用数。|ApiName、OperationName、Region|
|TotalCalls|是|总调用数|Count|总计|调用总数。|ApiName、OperationName、Region|
|TotalErrors|是|错误总数|Count|总计|引发错误响应（HTTP 响应代码 4xx 或 5xx）的调用总数。|ApiName、OperationName、Region|
|TotalTokenCalls|是|令牌调用总数|Count|总计|令牌调用的总数。|ApiName、OperationName、Region|
|TotalTransactions|是|总事务|Count|总计|事务总数。|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|已用 CPU 信用额度|是|已用 CPU 信用额度|Count|平均值|虚拟机使用的总信用点数|无维度|
|剩余 CPU 信用额度|是|剩余 CPU 信用额度|Count|平均值|可用来集中使用的总信用点数|无维度|
...
|磁盘写入操作次数/秒|是|磁盘写入操作次数/秒|每秒计数|平均值|磁盘写入 IOPS|无维度|
|入站流数|是|入站流数|Count|平均值|入站流数是入站方向的当前流（传入 VM 的流量）的数目|无维度|
...
|OS 磁盘队列深度|是|OS 磁盘队列深度（预览版）|Count|平均值|OS 磁盘队列深度(或队列长度)|无维度|
...
|OS 每磁盘 QD|是|OS 磁盘 QD (已弃用)|Count|平均值|OS 磁盘队列深度(或队列长度)|无维度|
...
|出站流|是|出站流|Count|平均值|出站流数是出站方向的当前流（传出 VM 的流量）的数目|无维度|
|出站流最大创建速率|是|出站流最大创建速率|每秒计数|平均值|出站流（传出 VM 的流量）的最大创建速率|无维度|
|每磁盘 QD|是|数据磁盘 QD (已弃用)|Count|平均值|数据磁盘队列深度(或队列长度)|SlotId|
...
|已用 CPU 信用额度|是|已用 CPU 信用额度|Count|平均值|虚拟机使用的总信用点数|无维度|
|剩余 CPU 信用额度|是|剩余 CPU 信用额度|Count|平均值|可用来集中使用的总信用点数|无维度|
|数据磁盘队列深度|是|数据磁盘队列深度（预览版）|Count|平均值|数据磁盘队列深度(或队列长度)|LUN、VMName|
...
|入站流数|是|入站流数|Count|平均值|入站流数是入站方向的当前流（传入 VM 的流量）的数目|VMName|
...
|OS 磁盘队列深度|是|OS 磁盘队列深度（预览版）|Count|平均值|OS 磁盘队列深度(或队列长度)|VMName|
...
|OS 每磁盘写入字节数/秒|是|OS 磁盘写入字节数/秒(已弃用)|每秒计数|平均值|OS 磁盘监视期间每秒写入到单个磁盘的字节数|无维度|
|OS 每磁盘写入操作数/秒|是|OS 磁盘写入操作数/秒(已弃用)|每秒计数|平均值|OS 磁盘监视期间从单个磁盘执行的写入 IOPS|无维度|
|出站流|是|出站流|Count|平均值|出站流数是出站方向的当前流（传出 VM 的流量）的数目|VMName|
...
|已用 CPU 信用额度|是|已用 CPU 信用额度|Count|平均值|虚拟机使用的总信用点数|无维度|
|剩余 CPU 信用额度|是|剩余 CPU 信用额度|Count|平均值|可用来集中使用的总信用点数|无维度|
|数据磁盘队列深度|是|数据磁盘队列深度（预览版）|Count|平均值|数据磁盘队列深度(或队列长度)|LUN|
|数据磁盘读取字节数/秒|是|数据磁盘读取字节数/秒（预览版）|每秒计数|平均值|监视期间每秒从单个磁盘读取的字节数|LUN|
|数据磁盘读取操作数/秒|是|数据磁盘读取操作数/秒（预览版）|每秒计数|平均值|监视期间从单个磁盘执行的读取 IOPS|LUN|
|数据磁盘写入字节数/秒|是|数据磁盘写入字节数/秒（预览版）|每秒计数|平均值|监视期间每秒写入到单个磁盘的字节数|LUN|
|数据磁盘写入操作数/秒|是|数据磁盘写入操作数/秒（预览版）|每秒计数|平均值|监视期间从单个磁盘执行的写入 IOPS|LUN|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|CpuUsage|是|CPU 使用率|Count|平均值|所有核心的 CPU 使用率（以 millicore 为单位）。|containerName|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|AgentPoolCPUTime|是|AgentPool CPU 时间|秒|总计|AgentPool CPU 时间（秒）|无维度|
|RunDuration|是|运行持续时间|毫秒|总计|运行持续时间，以毫秒为单位|无维度|
|SuccessfulPullCount|是|成功提取计数|Count|平均值|成功的映像提取次数|无维度|
|SuccessfulPushCount|是|成功推送计数|Count|平均值|成功的映像推送次数|无维度|
|TotalPullCount|是|提取总数|Count|平均值|映像提取总次数|无维度|
|TotalPushCount|是|推送总数|Count|平均值|映像推送总次数|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|kube_node_status_allocatable_cpu_cores|否|托管群集中可用 CPU 内核的总数|Count|平均值|托管群集中可用 CPU 内核的总数|无维度|
|kube_node_status_allocatable_memory_bytes|否|托管群集中可用内存的总量|字节|平均值|托管群集中可用内存的总量|无维度|
|kube_node_status_condition|否|各种节点条件的状态|Count|平均值|各种节点条件的状态|condition、status、status2、node|
|kube_pod_status_phase|否|依据阶段的 Pod 数|Count|平均值|依据阶段的 Pod 数|阶段、命名空间、Pod|
|kube_pod_status_ready|否|就绪状态下的 Pod 数|Count|平均值|就绪状态下的 Pod 数|命名空间、Pod|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|FailedRequests|是|失败的请求数|Count|总计|获取自定义资源提供程序的可用日志|HttpMethod、CallPath、StatusCode|
|SuccessfullRequests|是|成功的请求数|Count|总计|自定义提供程序发出的成功请求|HttpMethod、CallPath、StatusCode|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|FailedRuns|是|失败的运行次数|Count|总计||pipelineName、activityName|
|SuccessfulRuns|是|成功的运行次数|Count|总计||pipelineName、activityName|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ActivityCancelledRuns|是|已取消的活动运行指标数|Count|总计||ActivityType、PipelineName、FailureType、Name|
|ActivityFailedRuns|是|失败的活动运行数指标|Count|总计||ActivityType、PipelineName、FailureType、Name|
|ActivitySucceededRuns|是|成功的活动运行数指标|Count|总计||ActivityType、PipelineName、FailureType、Name|
|FactorySizeInGbUnits|是|总工厂大小（以 GB 为单位）|Count|最大值||无维度|
...
|IntegrationRuntimeQueueLength|是|集成运行时队列长度|Count|平均值||IntegrationRuntimeName|
|MaxAllowedFactorySizeInGbUnits|是|允许的最大工厂大小（以 GB 为单位）|Count|最大值||无维度|
|MaxAllowedResourceCount|是|允许的最大实体计数|Count|最大值||无维度|
|PipelineCancelledRuns|是|已取消的管道运行指标数|Count|总计||FailureType、Name|
|PipelineFailedRuns|是|失败的管道运行数指标|Count|总计||FailureType、Name|
|PipelineSucceededRuns|是|成功的管道运行数指标|Count|总计||FailureType、Name|
|ResourceCount|是|实体总数|Count|最大值||无维度|
|TriggerCancelledRuns|是|已取消的触发器运行指标数|Count|总计||Name、FailureType|
...
|ReadRequests|是|读取请求数|Count|总计|帐户的数据读取请求计数。|无维度|
|TotalStorage|是|总存储|字节|最大值|帐户中存储的数据总量。|无维度|
|WriteRequests|是|写入请求数|Count|总计|帐户的数据写入请求计数。|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|active_connections|是|活动连接数|Count|平均值|活动连接数|无维度|
|backup_storage_used|是|已用的备份存储|字节|平均值|已用的备份存储|无维度|
|connections_failed|是|失败的连接数|Count|总计|失败的连接数|无维度|
...
|seconds_behind_master|是|复制延迟（秒）|Count|最大值|复制延迟（秒）|无维度|
...
|cpu_percent|是|CPU 百分比|百分比|平均值|CPU 百分比|无维度|
|iops|是|IOPS|Count|平均值|每秒 IO 操作次数|无维度|
|memory_percent|是|内存百分比|百分比|平均值|内存百分比|无维度|
|network_bytes_egress|是|网络传出|字节|总计|跨活动连接数的网络传出|无维度|
|network_bytes_ingress|是|网络传入|字节|总计|跨活动连接数的网络传入|无维度|
|storage_percent|是|存储空间百分比|百分比|平均值|存储空间百分比|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|c2d.commands.egress.abandon.success|是|丢弃的 C2D 消息数|Count|总计|设备丢弃的云到设备消息数|无维度|
...
|c2d.methods.failure|是|失败的直接方法调用数|Count|总计|所有失败直接方法调用的计数。|无维度|
...
|c2d.methods.success|是|成功的直接方法调用数|Count|总计|所有成功的直接方法调用的计数。|无维度|
|c2d.twin.read.failure|是|后端的失败孪生读取数|Count|总计|由后端发起的所有失败孪生读取的计数。|无维度|
|c2d.twin.read.size|是|后端的孪生读取的响应大小|字节|平均值|由后端发起的所有成功的孪生读取的平均、最小和最大大小。|无维度|
|c2d.twin.read.success|是|后端的成功孪生读取数|Count|总计|由后端发起的所有成功孪生读取的计数。|无维度|
|c2d.twin.update.failure|是|后端的失败孪生更新数|Count|总计|由后端发起的所有失败孪生更新的计数。|无维度|
|c2d.twin.update.size|是|后端的失败孪生更新大小|字节|平均值|由后端发起的所有成功孪生更新的平均、最小和最大大小。|无维度|
|c2d.twin.update.success|是|后端的成功孪生更新数|Count|总计|由后端发起的所有成功孪生更新的计数。|无维度|
|C2DMessagesExpired|是|过期的 C2D 消息数（预览版）|计数|总计|过期的云到设备消息数|无维度|
|配置|是|配置指标|计数|总计|配置操作的指标|无维度|
|connectedDeviceCount|否|连接设备数（预览）|Count|平均值|已连接到 IoT 中心的设备数目|无维度|
|d2c.endpoints.egress.builtIn.events|是|路由：消息传送到消息/事件的次数|Count|总计|IoT 中心路由成功将消息传送到内置终结点（消息/事件）的次数。|无维度|
|d2c.endpoints.egress.eventHubs|是|路由：消息传送到事件中心的次数|Count|总计|IoT 中心路由成功将消息传送到事件中心终结点的次数。|无维度|
|d2c.endpoints.egress.serviceBusQueues|是|路由：消息传送到服务总线队列的次数|Count|总计|IoT 中心路由成功将消息传送到服务总线队列终结点的次数。|无维度|
...
|d2c.endpoints.egress.storage.blobs|是|路由：将 Blob 传送到存储的次数|Count|总计|IoT 中心路由将 Blob 传送到存储终结点的次数。|无维度|
...
|d2c.telemetry.egress.dropped|是|路由：遥测消息删除次数 |Count|总计|由于终结点消亡，IoT 中心路由删除消息的次数。 此值不会统计已传送到回退路由的消息，因为已删除的消息不会传送到回退路由。|无维度|
|d2c.telemetry.egress.fallback|是|路由：消息传送到回退路由的次数|Count|总计|IoT 中心路由将消息传送到与回退路由关联的终结点的次数。|无维度|
|d2c.telemetry.egress.invalid|是|路由：遥测消息不兼容|Count|总计|消息由于与终结点不兼容而无法由 IoT 中心路由传送的次数。 此值不包括重试次数。|无维度|
|d2c.telemetry.egress.orphaned|是|路由：遥测消息孤立次数 |计数|总计|消息由于与任何路由规则（包括回退规则）都不匹配而被 IoT 中心路由孤立的次数。 |无维度|
|d2c.telemetry.egress.success|是|路由：遥测消息传送次数|Count|总计|使用 IoT 中心路由将消息成功传送到所有终结点的次数。 如果某条消息已路由到多个终结点，则每成功传送一次，此值就会加 1。 如果某条消息多次路由到同一终结点，则每成功传送一次，此值就会加 1。|无维度|
|d2c.telemetry.ingress.allProtocol|是|遥测消息发送尝试次数|Count|总计|尝试发送到 IoT 中心的、设备到云的遥测消息数|无维度|
|d2c.telemetry.ingress.sendThrottle|是|限制错误数|Count|总计|由于设备吞吐量限制而导致的限制错误数|无维度|
|d2c.telemetry.ingress.success|是|发送的遥测消息数|计数|总计|成功发送到 IoT 中心的、设备到云的遥测消息数|无维度|
|d2c.twin.read.failure|是|设备的失败孪生读取数|Count|总计|由设备发起的所有失败孪生读取的计数。|无维度|
|d2c.twin.read.size|是|设备的孪生读取的响应大小|字节|平均值|由设备发起的所有成功的孪生读取的平均、最小和最大大小。|无维度|
|d2c.twin.read.success|是|设备的成功孪生读取数|Count|总计|由设备发起的所有成功孪生读取的计数。|无维度|
|d2c.twin.update.failure|是|设备的失败孪生更新数|Count|总计|由设备发起的所有失败的孪生更新的计数。|无维度|
|d2c.twin.update.size|是|设备的孪生更新的大小|字节|平均值|由设备发起的所有成功孪生更新的平均、最小和最大大小。|无维度|
|d2c.twin.update.success|是|设备的成功孪生更新数|Count|总计|由设备发起的所有成功的孪生更新的计数。|无维度|
|dailyMessageQuotaUsed|是|已使用的消息总数|Count|最大值|今天使用的消息总数|无维度|
...
|devices.connectedDevices.allProtocol|是|连接的设备数（已弃用） |Count|总计|已连接到 IoT 中心的设备数目|无维度|
|devices.totalDevices|是|设备总数（已弃用）|Count|总计|已注册到 IoT 中心的设备数目|无维度|
...
|jobs.cancelJob.failure|是|失败的作业取消数|Count|总计|用来取消作业的调用失败的次数。|无维度|
|jobs.cancelJob.success|是|成功的作业取消数|Count|总计|用来取消作业的调用成功的次数。|无维度|
|jobs.completed|是|已完成的作业|Count|总计|所有已完成的作业的计数。|无维度|
|jobs.createDirectMethodJob.failure|是|方法调用作业的创建失败数|Count|总计|直接方法调用作业创建失败的所有次数。|无维度|
|jobs.createDirectMethodJob.success|是|方法调用作业的创建成功数|Count|总计|直接方法调用作业创建成功的所有次数。|无维度|
|jobs.createTwinUpdateJob.failure|是|孪生更新作业创建失败数|Count|总计|孪生更新作业创建失败的所有次数。|无维度|
|jobs.createTwinUpdateJob.success|是|孪生更新作业创建成功数|Count|总计|孪生更新作业创建成功的所有次数。|无维度|
|jobs.failed|是|失败的作业数|Count|总计|所有失败的作业的计数。|无维度|
|jobs.listJobs.failure|是|对列出作业的失败调用数|Count|总计|对列出作业的所有失败调用的计数。|无维度|
|jobs.listJobs.success|是|对列出作业的成功调用数|计数|总计|对列出作业的所有成功调用的计数。|无维度|
|jobs.queryJobs.failure|是|失败的作业查询数|Count|总计|对查询作业的所有失败调用的计数。|无维度|
|jobs.queryJobs.success|是|成功的作业查询数|Count|总计|对查询作业的所有成功调用的计数。|无维度|
|RoutingDataSizeInBytesDelivered|是|以字节为单位的路由传递消息大小 (预览) |字节|总计|IoT 中心发送到终结点的消息的总大小（以字节为单位）。 您可以使用 "终结点" 和 "EndpointType" 维度来查看传递到不同终结点的消息大小（以字节为单位）。 对于每个传递的消息，指标值都会增加，包括如果消息传递到多个终结点，或者消息传递到同一终结点多次。|EndpointType、终结点、RoutingSource|
|RoutingDeliveries|是|路由传递（预览版）|计数|总计|IoT 中心尝试使用路由将消息传送到所有终结点的次数。 若要查看成功或失败尝试的次数，请使用 Result 维度。 若要查看失败的原因，例如无效、已删除或孤立，请使用 FailureReasonCategory 维度。 还可以使用 EndpointName 和 EndpointType 维度来了解传递到不同终结点的消息数。 每次传递尝试，指标值都会增加 1，包括将消息传递到多个终结点或将消息多次传递到同一终结点。|EndpointType、终结点、FailureReasonCategory、Result、RoutingSource|
|RoutingDeliveryLatency|是|路由传递延迟（预览版）|毫秒|平均值|消息进入 IoT 中心与遥测消息进入终结点之间的平均延迟（毫秒）。 可以使用 EndpointName 和 EndpointType 维度来了解不同终结点的延迟。|EndpointType、终结点、RoutingSource|
|totalDeviceCount|否|设备总数（预览）|计数|平均值|已注册到 IoT 中心的设备数目|无维度|
|twinQueries.failure|是|失败的孪生查询数|Count|总计|所有失败孪生查询的计数。|无维度|
|twinQueries.resultSize|是|孪生查询结果大小|字节|平均值|所有成功孪生查询的结果大小的平均值、最小值和最大值。|无维度|
|twinQueries.success|是|成功的孪生查询数|Count|总计|所有成功孪生查询的计数。|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|AttestationAttempts|是|证明尝试次数|Count|总计|已尝试的设备证明次数|ProvisioningServiceName、Status、Protocol|
|DeviceAssignments|是|已分配设备|计数|总计|已分配给 IoT 中心的设备数|ProvisioningServiceName、IotHubName|
|RegistrationAttempts|是|注册尝试次数|Count|总计|已尝试的设备注册次数|ProvisioningServiceName、IotHubName、Status|
...
|AutoscaleMaxThroughput|否|自动缩放最大吞吐量|Count|最大值|自动缩放最大吞吐量|DatabaseName、CollectionName|
|AvailableStorage|否| (弃用) 可用存储|字节|总计|2020年9月结束时，将从 Azure Monitor 中删除 "可用存储"。 Cosmos DB 集合存储大小目前是无限制的。 唯一的限制是每个逻辑分区键的存储大小为 20 gb。 可以在诊断日志中启用 PartitionKeyStatistics，以了解 top 分区键的存储占用情况。 有关 Cosmos DB 存储配额的详细信息，请查看此文档 https://docs.microsoft.com/azure/cosmos-db/concepts-limits 。 弃用后，仍在弃用的指标上定义的剩余警报规则将在弃用日期之后自动禁用。|CollectionName、DatabaseName、Region|
|CassandraConnectionClosures|否|Cassandra 连接关闭数|计数|总计|已关闭的 Cassandra 连接数，按 1 分钟粒度报告|Region、ClosureReason|
|CassandraConnectorAvgReplicationLatency|否|Cassandra 连接器 Average ReplicationLatency|毫秒|平均值|Cassandra 连接器 Average ReplicationLatency|无维度|
|CassandraConnectorReplicationHealthStatus|否|Cassandra 连接器复制运行状况状态|Count|Count|Cassandra 连接器复制运行状况状态|NotStarted，ReplicationInProgress，Error|
|CassandraKeyspaceCreate|否|已创建 Cassandra 密钥空间|Count|Count|已创建 Cassandra 密钥空间|ResourceName |
...
|CassandraRequestCharges|否|Cassandra 请求费用|Count|总计|发出的 Cassandra 请求所消耗的 RU 数|DatabaseName、CollectionName、Region、OperationType、ResourceType|
|CassandraRequests|否|Cassandra 请求数|Count|计数|发出的 Cassandra 请求数|DatabaseName、CollectionName、Region、OperationType、ResourceType、ErrorCode|
|CassandraTableCreate|否|已创建 Cassandra 表|Count|Count|已创建 Cassandra 表|Context.resourcename、ChildResourceName、 |
|CassandraTableDelete|否|Cassandra 表已删除|计数|计数|Cassandra 表已删除|Context.resourcename、ChildResourceName、 |
|CassandraTableThroughputUpdate|否|Cassandra 表吞吐量已更新|计数|计数|Cassandra 表吞吐量已更新|Context.resourcename、ChildResourceName、 |
|CassandraTableUpdate|否|Cassandra 表已更新|计数|计数|Cassandra 表已更新|Context.resourcename、ChildResourceName、 |
...
|DocumentCount|否|文档计数|Count|总计|按 5 分钟粒度报告的文档总数|CollectionName、DatabaseName、Region|
|DocumentQuota|否|文档配额|字节|总计|按 5 分钟粒度报告的存储配额总量|CollectionName、DatabaseName、Region|
|GremlinDatabaseCreate|否|已创建 Gremlin 数据库|Count|Count|已创建 Gremlin 数据库|ResourceName |
...
|GremlinGraphCreate|否|已创建 Gremlin 图|Count|Count|已创建 Gremlin 图|Context.resourcename、ChildResourceName、 |
|GremlinGraphDelete|否|已删除 Gremlin 图形|计数|计数|已删除 Gremlin 图形|Context.resourcename、ChildResourceName、 |
|GremlinGraphThroughputUpdate|否|已更新 Gremlin 图形吞吐量|计数|计数|已更新 Gremlin 图形吞吐量|Context.resourcename、ChildResourceName、 |
|GremlinGraphUpdate|否|已更新 Gremlin 图形|计数|计数|已更新 Gremlin 图形|Context.resourcename、ChildResourceName、 |
...
|MongoCollectionCreate|否|已创建 Mongo 收集|Count|Count|已创建 Mongo 收集|Context.resourcename、ChildResourceName、 |
|MongoCollectionDelete|否|已删除 Mongo 集合|计数|计数|已删除 Mongo 集合|Context.resourcename、ChildResourceName、 |
|MongoCollectionThroughputUpdate|否|已更新 Mongo 集合吞吐量|计数|计数|已更新 Mongo 集合吞吐量|Context.resourcename、ChildResourceName、 |
|MongoCollectionUpdate|否|Mongo 集合已更新|计数|计数|Mongo 集合已更新|Context.resourcename、ChildResourceName、 |
...
|MongoDBDatabaseCreate|否|已创建 Mongo 数据库|Count|Count|已创建 Mongo 数据库|ResourceName |
|MongoDBDatabaseUpdate|否|已更新 Mongo 数据库|计数|计数|已更新 Mongo 数据库|ResourceName |
|MongoRequestCharge|是|Mongo 请求费用|Count|总计|Mongo 已消耗的请求单位|DatabaseName、CollectionName、Region、CommandName、ErrorCode、Status|
|MongoRequests|是|Mongo 请求|Count|计数|已发出的 Mongo 请求数|DatabaseName、CollectionName、Region、CommandName、ErrorCode、Status|
...
|ProvisionedThroughput|否|预配的吞吐量|Count|最大值|预配的吞吐量|DatabaseName、CollectionName|
...
|SqlContainerCreate|否|已创建 Sql 容器|Count|Count|已创建 Sql 容器|Context.resourcename、ChildResourceName、 |
|SqlContainerDelete|否|Sql 容器已删除|计数|计数|Sql 容器已删除|Context.resourcename、ChildResourceName、 |
|SqlContainerThroughputUpdate|否|已更新 Sql 容器吞吐量|计数|计数|已更新 Sql 容器吞吐量|Context.resourcename、ChildResourceName、 |
|SqlContainerUpdate|否|已更新 Sql 容器|计数|计数|已更新 Sql 容器|Context.resourcename、ChildResourceName、 |
|SqlDatabaseCreate|否|已创建 Sql 数据库|Count|计数|已创建 Sql 数据库|ResourceName |
...
|TableTableCreate|否|已创建 AzureTable 表|Count|Count|已创建 AzureTable 表|ResourceName |
...
|TotalRequests|是|请求总数|Count|计数|已发出的请求数|DatabaseName、CollectionName、Region、StatusCode、OperationType、Status|
|TotalRequestUnits|是|总请求单位数|Count|总计|已消耗的请求单位|DatabaseName、CollectionName、Region、StatusCode、OperationType、Status|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|DeadLetteredCount|是|死信事件数|Count|总计|与此事件订阅匹配的死信事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName、DeadLetterReason|
...
|DeliverySuccessCount|是|发送的事件数|Count|总计|发送到此事件订阅的事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName|
|DestinationProcessingDurationInMs|否|目标处理持续时间|毫秒|平均值|目标处理持续时间（毫秒）|主题、EventSubscriptionName、DomainEventSubscriptionName|
|DroppedEventCount|是|删除的事件数|Count|总计|与此事件订阅匹配的已删除事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName、DropReason|
|MatchedEventCount|是|匹配的事件数|Count|总计|与此事件订阅匹配的事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName|
|PublishFailCount|是|发布失败的事件数|Count|总计|未能发布到此主题的事件总数|主题，ErrorType，错误|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ActiveConnections|否|ActiveConnections|Count|平均值|Microsoft.EventHub 的活动连接总数。|无维度|
|AvailableMemory|否|可用内存|百分比|最大值|事件中心群集的可用内存，以总内存量的百分比表示。|角色|
|CaptureBacklog|否|捕获积压工作(backlog)。|Count|总计|捕获有关 Microsoft.EventHub 的积压工作(backlog)。|无维度|
|CapturedBytes|否|已捕获的字节数。|字节|总计|Microsoft.EventHub 已捕获的字节数。|无维度|
|CapturedMessages|否|已捕获的消息数。|Count|总计|Microsoft.EventHub 已捕获的消息数。|无维度|
|ConnectionsClosed|否|已关闭的连接数。|Count|平均值|Microsoft.EventHub 已关闭的连接数。|无维度|
|ConnectionsOpened|否|打开的连接数。|Count|平均值|Microsoft.EventHub 打开的连接数。|无维度|
|CPU|否|CPU|百分比|最大值|事件中心群集的 CPU 使用率（百分比）|角色|
|IncomingBytes|是|传入字节数。|字节|总计|Microsoft.EventHub 传入的字节数。|无维度|
|IncomingMessages|是|传入消息数|Count|总计|Microsoft.EventHub 传入的消息数。|无维度|
|IncomingRequests|是|传入请求数|Count|总计|Microsoft.EventHub 传入的请求数。|无维度|
|OutgoingBytes|是|传出字节数。|字节|总计|Microsoft.EventHub 传出的字节数。|无维度|
|OutgoingMessages|是|传出消息数|Count|总计|Microsoft.EventHub 传出的消息数。|无维度|
|QuotaExceededErrors|否|超过限额错误。|Count|总计|Microsoft.EventHub 的超过限额错误数。|无维度|
|ServerErrors|否|服务器错误数。|Count|总计|Microsoft.EventHub 的服务器错误数。|无维度|
|大小|否|大小|字节|平均值|EventHub 的大小（以字节为单位）。|角色|
|SuccessfulRequests|否|成功的请求数|Count|总计|Microsoft.EventHub 成功请求数。|无维度|
|ThrottledRequests|否|限制的请求数。|Count|总计|Microsoft.EventHub 限制的请求数。|无维度|
|UserErrors|否|用户错误数。|Count|总计|Microsoft.EventHub 用户错误数。|无维度|
...
|CaptureBacklog|否|捕获积压工作(backlog)。|Count|总计|捕获有关 Microsoft.EventHub 的积压工作(backlog)。|EntityName|
|CapturedBytes|否|已捕获的字节数。|字节|总计|Microsoft.EventHub 已捕获的字节数。|EntityName|
|CapturedMessages|否|已捕获的消息数。|Count|总计|Microsoft.EventHub 已捕获的消息数。|EntityName|
|ConnectionsClosed|否|已关闭的连接数。|Count|平均值|Microsoft.EventHub 已关闭的连接数。|EntityName|
|ConnectionsOpened|否|打开的连接数。|Count|平均值|Microsoft.EventHub 打开的连接数。|EntityName|
|EHABL|是|存档积压工作消息数（已弃用）|Count|总计|命名空间积压工作中的事件中心存档消息数（已弃用）|无维度|
|EHAMBS|是|存档消息吞吐量（已弃用）|字节|总计|命名空间中的事件中心存档消息吞吐量（已弃用）|无维度|
|EHAMSGS|是|存档消息数（已弃用）|Count|总计|命名空间中的事件中心存档消息数（已弃用）|无维度|
...
|EHOUTMSGS|是|传出消息（已弃用）|Count|总计|命名空间的传出消息总数（已弃用）|无维度|
|FAILREQ|是|失败的请求数（已弃用）|计数|总计|命名空间的失败请求总数（已弃用）|无维度|
|IncomingBytes|是|传入字节数。|字节|总计|Microsoft.EventHub 传入的字节数。|EntityName|
|IncomingMessages|是|传入消息数|Count|总计|Microsoft.EventHub 传入的消息数。|EntityName|
|IncomingRequests|是|传入请求数|Count|总计|Microsoft.EventHub 传入的请求数。|EntityName|
|INMSGS|是|传入的消息数（已过时）（已弃用）|Count|总计|命名空间的传入消息总数。 此指标已弃用。 请改用传入消息指标（已弃用）|无维度|
|INREQS|是|传入的请求数（已弃用）|Count|总计|命名空间的传入发送请求总数（已弃用）|无维度|
|INTERR|是|内部服务器错误数（已弃用）|Count|总计|命名空间的内部服务器错误总数（已弃用）|无维度|
|MISCERR|是|其他错误（已弃用）|Count|总计|命名空间的失败请求总数（已弃用）|无维度|
|OutgoingBytes|是|传出字节数。|字节|总计|Microsoft.EventHub 传出的字节数。|EntityName|
|OutgoingMessages|是|传出消息数|Count|总计|Microsoft.EventHub 传出的消息数。|EntityName|
|OUTMSGS|是|传出的消息数（已过时）（已弃用）|Count|总计|命名空间的传出消息总数。 此指标已弃用。 请改用传出消息指标（已弃用）|无维度|
|QuotaExceededErrors|否|超过限额错误。|Count|总计|Microsoft.EventHub 的超过限额错误数。|EntityName、 |
|ServerErrors|否|服务器错误数。|Count|总计|Microsoft.EventHub 的服务器错误数。|EntityName、 |
|大小|否|大小|字节|平均值|EventHub 的大小（以字节为单位）。|EntityName|
|SuccessfulRequests|否|成功的请求数|Count|总计|Microsoft.EventHub 成功请求数。|EntityName、 |
|SUCCREQ|是|成功的请求数（已弃用）|Count|总计|命名空间的成功请求总数（已弃用）|无维度|
|SVRBSY|是|服务器繁忙错误数（已弃用）|Count|总计|命名空间的服务器繁忙错误总数（已弃用）|无维度|
|ThrottledRequests|否|限制的请求数。|Count|总计|Microsoft.EventHub 限制的请求数。|EntityName、 |
|UserErrors|否|用户错误数。|Count|总计|Microsoft.EventHub 用户错误数。|EntityName、 |
...
|GatewayRequests|是|网关请求数|Count|总计|网关请求数|HttpStatus|
|NumActiveWorkers|是|活动辅助角色数目|Count|最大值|活动辅助角色数目|MetricName|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|MetricThreshold|是|指标阈值|Count|平均值|自动缩放运行时已配置的自动缩放阈值。|MetricTriggerRule|
|ObservedCapacity|是|观察到的容量|计数|平均值|自动缩放执行时报告的容量。|无维度|
|ObservedMetricValue|是|观察到的指标值|Count|平均值|执行自动缩放时计算的值|MetricTriggerSource|
|ScaleActionsInitiated|是|启动的缩放操作|Count|总计|缩放操作的方向。|ScaleDirection|
...
|dependencies/count|否|依赖项调用|Count|计数|应用程序对外部资源所进行的调用计数。|依赖项/类型、依赖项/performanceBucket、依赖项/成功、依赖项/目标、依赖项/resultCode、操作/合成、cloud/roleInstance、云/对象类型|
|dependencies/duration|是|依赖项持续时间|毫秒|平均值|应用程序对外部资源所进行的调用持续时间。|依赖项/类型、依赖项/performanceBucket、依赖项/成功、依赖项/目标、依赖项/resultCode、操作/合成、cloud/roleInstance、云/对象类型|
|dependencies/failed|否|依赖项调用失败次数|Count|计数|应用程序对外部资源所进行的依赖项调用失败的计数。|依赖项/类型、依赖项/performanceBucket、依赖项/目标、依赖项/resultCode、操作/合成、云/roleInstance、云/工作项|
|exceptions/browser|否|浏览器异常|Count|计数|浏览器中所引发未捕获异常的计数。|云/拥有|
|exceptions/count|是|例外|计数|计数|所有未捕获异常的已合并计数。|cloud/roleName, cloud/roleInstance, client/type|
|exceptions/server|否|服务器异常|Count|计数|服务器应用程序中引发的未捕获的异常计数。|cloud/roleName, cloud/roleInstance|
|pageViews/count|是|页面视图|Count|计数|页面视图计数。|操作/综合，云/工作群|
|pageViews/duration|是|页面视图加载时间|毫秒|平均值|页面视图加载时间|操作/综合，云/工作群|
...
|performanceCounters/requestsInQueue|是|应用程序队列中的 HTTP 请求|Count|平均值|应用程序请求队列的长度。|cloud/roleInstance|
...
|requests/failed|否|失败的请求|Count|计数|标记为失败的 HTTP 请求的计数。 在大多数情况下这些请求的响应代码 >= 400 且不等于 401。|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|traces/count|是|跟踪|Count|计数|跟踪文档计数|trace/severityLevel, operation/synthetic, cloud/roleName, cloud/roleInstance|
...
|ServiceApiHit|是|服务 API 命中总计|Count|计数|服务 API 命中总数|ActivityType, ActivityName|
|ServiceApiLatency|是|总体服务 API 延迟|毫秒|平均值|服务 API 请求的总体延迟|ActivityType，ActivityName，StatusCode，StatusCodeClass|
|ServiceApiResult|是|服务 API 结果总计|Count|计数|服务 API 结果总数|ActivityType，ActivityName，StatusCode，StatusCodeClass|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|BatchBlobCount|是|批处理 Blob 计数|Count|平均值|聚合批次中用于引入的数据源数。|数据库|
|BatchDuration|是|批处理持续时间|秒|平均值|引入流中聚合阶段的持续时间。|数据库|
|BatchesProcessed|是|处理的批处理|Count|平均值|为引入聚合的批处理数。 批处理完成原因：批处理是否已达到批处理时间、数据大小或批处理策略设置的文件数限制|Database、SealReason|
|BatchSize|是|批大小|字节|平均值|引入的聚合批中的未压缩预期数据大小。|数据库|
...
|ContinuousExportNumOfRecordsExported|是|连续导出 – 导出的记录数|计数|总计|导出的记录数，在导出操作期间对每个写入的存储项目激发|ContinuousExportName，数据库|
|ContinuousExportPendingCount|是|连续导出挂起计数|Count|最大值|做好执行准备的挂起的连续导出作业数|无维度|
|ContinuousExportResult|是|连续导出结果|计数|计数|指示连续导出是成功还是失败|ContinuousExportName，结果，数据库|
|CPU|是|CPU|百分比|平均值|CPU 使用率级别|无维度|
|EventsProcessedForEventHubs|是|处理的事件数（适用于事件中心/IoT 中心）|Count|总计|从事件中心/IoT 中心引入时，由群集处理的事件数|EventStatus|
|ExportUtilization|是|导出利用率|百分比|最大值|导出利用率|无维度|
|IngestionLatencyInSeconds|是|引入延迟（秒）|秒|平均值|从源（例如消息位于事件中心）到群集的引入时间（秒）|无维度|
|IngestionResult|是|引入结果|Count|计数|引入操作的数量|IngestionResultDetails|
|IngestionUtilization|是|引入利用率|百分比|平均值|群集中已使用引入槽的比率|无维度|
|IngestionVolumeInMB|是|引入量 (MB)|Count|总计|已引入群集的数据总量 (MB)|无维度|
|InstanceCount|是|实例计数|计数|平均值|实例总计数|无维度|
|KeepAlive|是|保持活动状态|Count|平均值|完整性检查表明群集对查询做出了响应|无维度|
|QueryDuration|是|查询持续时间|毫秒|平均值|队列持续时间（秒）|QueryStatus|
|SteamingIngestRequestRate|是|流引入请求速率|Count|RateRequestsPerSecond|流引入请求速率（每秒请求数）|无维度|
|StreamingIngestDataRate|是|流引入数据速率|Count|平均值|流引入数据速率（MB/秒）|无维度|
|StreamingIngestDuration|是|流引入持续时间|毫秒|平均值|流引入持续时间（毫秒）|无维度|
|StreamingIngestResults|是|流引入结果|Count|平均值|流引入结果|结果|
...
|ActionsCompleted|是|完成的操作数 |Count|总计|已完成的工作流操作数目。|无维度|
|ActionsFailed|是|失败的操作数 |Count|总计|失败的工作流操作数目。|无维度|
|ActionsSkipped|是|跳过的操作数 |Count|总计|已跳过的工作流操作数目。|无维度|
|ActionsStarted|是|启动的操作数 |Count|总计|已启动的工作流操作数目。|无维度|
|ActionsSucceeded|是|成功的操作数 |计数|总计|成功的工作流操作数目。|无维度|
|ActionSuccessLatency|是|操作成功延迟 |秒|平均值|已成功的工作流操作的延迟。|无维度|
|ActionThrottledEvents|是|操作限制事件数|Count|总计|工作流操作限制事件数目。|无维度|
...
|RunsCompleted|是|已完成的运行数|Count|总计|已完成的工作流运行数目。|无维度|
|RunsFailed|是|失败的运行数|Count|总计|失败的工作流运行数目。|无维度|
|RunsStarted|是|已启动的运行数|Count|总计|已启动的工作流运行数目。|无维度|
|RunsSucceeded|是|成功的运行数|Count|总计|成功的工作流运行数目。|无维度|
|RunStartThrottledEvents|是|运行启动限制事件|Count|总计|工作流运行启动限制事件数。|无维度|
|RunSuccessLatency|是|运行成功延迟|秒|平均值|已成功的工作流运行的延迟。|无维度|
|RunThrottledEvents|是|运行限制事件数|Count|总计|工作流操作或触发器限制事件数目。|无维度|
|TriggerFireLatency|是|触发器激发延迟 |秒|平均值|已激发的工作流触发器的延迟。|无维度|
|TriggerLatency|是|触发器延迟 |秒|平均值|已完成的工作流触发器的延迟。|无维度|
|TriggersCompleted|是|完成的触发器数 |Count|总计|已完成的工作流触发器数目。|无维度|
|TriggersFailed|是|失败的触发器数 |Count|总计|失败的工作流触发器数目。|无维度|
|TriggersFired|是|激发的触发器数 |计数|总计|已激发的工作流触发器数目。|无维度|
|TriggersSkipped|是|跳过的触发器数|Count|总计|已跳过的工作流触发器数目。|无维度|
|TriggersStarted|是|启动的触发器数 |Count|总计|已启动的工作流触发器数目。|无维度|
|TriggersSucceeded|是|成功的触发器数 |Count|总计|成功的工作流触发器数目。|无维度|
|TriggerSuccessLatency|是|触发器成功延迟 |秒|平均值|已成功的工作流触发器的延迟。|无维度|
|TriggerThrottledEvents|是|触发器限制事件数|Count|总计|工作流触发器限制事件数目。|无维度|
...
|ActionsSkipped|是|跳过的操作数 |Count|总计|已跳过的工作流操作数目。|无维度|
|ActionsStarted|是|启动的操作数 |Count|总计|已启动的工作流操作数目。|无维度|
|ActionsSucceeded|是|成功的操作数 |Count|总计|成功的工作流操作数目。|无维度|
|ActionSuccessLatency|是|操作成功延迟 |秒|平均值|已成功的工作流操作的延迟。|无维度|
|ActionThrottledEvents|是|操作限制事件数|Count|总计|工作流操作限制事件数目。|无维度|
|BillableActionExecutions|是|计费的操作执行数|Count|总计|计费的工作流操作执行数目。|无维度|
|BillableTriggerExecutions|是|计费的触发器执行数|Count|总计|计费的工作流触发器执行数目。|无维度|
|BillingUsageNativeOperation|是|本机操作执行的计费使用情况|Count|总计|已计费的本机操作执行次数。|无维度|
|BillingUsageStandardConnector|是|标准连接器执行的计费使用情况|Count|总计|已计费的标准连接器执行次数。|无维度|
|BillingUsageStorageConsumption|是|存储使用执行的计费使用情况|Count|总计|已计费的存储使用执行次数。|无维度|
|RunFailurePercentage|是|运行失败百分比|百分比|总计|失败的工作流运行百分比。|无维度|
|RunLatency|是|运行延迟|秒|平均值|已完成的工作流运行的延迟。|无维度|
|RunsCancelled|是|取消的运行数|计数|总计|已取消的工作流运行数目。|无维度|
|RunsCompleted|是|已完成的运行数|Count|总计|已完成的工作流运行数目。|无维度|
...
|RunSuccessLatency|是|运行成功延迟|秒|平均值|已成功的工作流运行的延迟。|无维度|
|RunThrottledEvents|是|运行限制事件数|Count|总计|工作流操作或触发器限制事件数目。|无维度|
|TotalBillableExecutions|是|计费的执行总数|Count|总计|计费的工作流执行数目。|无维度|
|TriggerFireLatency|是|触发器激发延迟 |秒|平均值|已激发的工作流触发器的延迟。|无维度|
|TriggerLatency|是|触发器延迟 |秒|平均值|已完成的工作流触发器的延迟。|无维度|
|TriggersCompleted|是|完成的触发器数 |Count|总计|已完成的工作流触发器数目。|无维度|
|TriggersFailed|是|失败的触发器数 |Count|总计|失败的工作流触发器数目。|无维度|
|TriggersFired|是|激发的触发器数 |Count|总计|已激发的工作流触发器数目。|无维度|
|TriggersSkipped|是|跳过的触发器数|Count|总计|已跳过的工作流触发器数目。|无维度|
|TriggersStarted|是|启动的触发器数 |Count|总计|已启动的工作流触发器数目。|无维度|
|TriggersSucceeded|是|成功的触发器数 |Count|总计|成功的工作流触发器数目。|无维度|
|TriggerSuccessLatency|是|触发器成功延迟 |秒|平均值|已成功的工作流触发器的延迟。|无维度|
...
|---|---|---|---|---|---|---|
|活动核心数|是|活动核心数|Count|平均值|活动核心数|方案，ClusterName|
|活动节点数|是|活动节点数|Count|平均值|活动节点数。 这些节点是正在运行作业的节点。|方案，ClusterName|
|请求取消运行数|是|请求取消运行数|计数|总计|为此工作区请求了取消的运行数。 当已收到用于运行的取消请求时，将更新计数。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|已取消运行数|是|已取消运行数|计数|总计|为此工作区取消的运行数。 成功取消运行后，计数将更新。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|已完成的运行数|是|已完成的运行数|Count|总计|已成功为此工作区完成的运行数。 当运行已完成并收集输出时，计数将更新。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|CpuUtilization|是|CpuUtilization|计数|平均值|CPU 节点上内存使用率的百分比。 使用率每分钟报告一次。|方案、runId、ClusterName|
|错误|是|错误|Count|总计|此工作区中的运行错误数。 每当运行遇到错误时，计数就会更新。|场景|
|失败的运行次数|是|失败的运行次数|Count|总计|此工作区的运行失败次数。 当运行失败时，将更新计数。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|完成状态运行数|是|完成状态运行数|计数|总计|为此工作区进入的运行已完成状态的数目。 当运行已完成但输出收集仍在进行时，将更新计数。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|GpuUtilization|是|GpuUtilization|计数|平均值|GPU 节点上内存使用率的百分比。 使用率每分钟报告一次。|方案、runId、ClusterName、DeviceId、|
|空闲核心数|是|空闲核心数|计数|平均值|空闲核心数|方案，ClusterName|
|空闲节点数|是|空闲节点数|Count|平均值|空闲节点数。 空闲节点是指未运行任何作业，但可以接受新作业（如果可用）的节点。|方案，ClusterName|
|正在退出的核心数|是|正在退出的核心数|Count|平均值|正在退出的核心数|方案，ClusterName|
|正在退出的节点数|是|正在退出的节点数|Count|平均值|正在退出的节点数。 正在退出的节点是指刚刚完成了作业处理，将进入空闲状态的节点。|方案，ClusterName|
|失败的模型部署数|是|失败的模型部署数|Count|总计|此工作区中失败的模型部署数|方案，StatusCode|
|已启动的模型部署数|是|已启动的模型部署数|Count|总计|此工作区中已启动的模型部署数|方案|
|成功的模型部署数|是|成功的模型部署数|Count|总计|此工作区中成功的模型部署数|方案|
|失败的模型注册数|是|失败的模型注册数|Count|总计|此工作区中失败的模型注册数|方案，StatusCode|
|成功的模型注册数|是|成功的模型注册数|Count|总计|此工作区中成功的模型注册数|方案|
|无响应运行数|是|无响应运行数|计数|总计|此工作区的运行数未响应。 当运行进入无响应状态时，将更新计数。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|未启动运行|是|未启动运行|计数|总计|此工作区的 "未开始运行" 状态。 当收到请求来创建运行但尚未填充运行信息时，将更新计数。 |方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|占用的核心数|是|占用的核心数|Count|平均值|占用的核心数|方案，ClusterName|
|占用的节点数|是|占用的节点数|Count|平均值|占用的节点数。 这些节点是已从可用节点池中取出的低优先级节点。|方案，ClusterName|
|准备的运行数|是|准备的运行数|计数|总计|正在为此工作区准备的运行数。 当正在准备运行环境时，计数会在运行进入准备状态时进行更新。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|预配运行数|是|预配运行数|计数|总计|为此工作区预配的运行数。 当运行正在等待计算目标创建或预配时，计数会更新。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|排队运行数|是|排队运行数|计数|总计|为此工作区排队的运行数。 当运行在计算目标中排队时，将更新计数。 等待必需的计算节点准备就绪时，可以 occure。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|配额利用率百分比|是|配额利用率百分比|Count|平均值|已利用的配额百分比|方案、ClusterName、VmFamilyName、VmPriority|
|已启动的运行数|是|已启动的运行数|计数|总计|为此工作区运行的运行数。 当运行开始在所需资源上运行时，将更新计数。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|启动运行数|是|启动运行数|计数|总计|为此工作区启动的运行数。 在请求创建运行并运行信息（如运行 Id）后，将更新计数|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|核心总数|是|核心总数|Count|平均值|核心总数|方案，ClusterName|
|节点总数|是|节点总数|Count|平均值|节点总数。 此总数包括一些活动节点、空闲节点、不可用的节点、占用的节点和正在退出的节点|方案，ClusterName|
|不可用的核心数|是|不可用的核心数|Count|平均值|不可用的核心数|方案，ClusterName|
|不可用的节点数|是|不可用的节点数|Count|平均值|不可用的节点数。 不可用的节点是由于存在某种不可解决的问题而无法正常运行的节点。 Azure 将回收这些节点。|方案，ClusterName|
|警告|是|警告|计数|总计|此工作区中的运行警告数。 每当运行遇到警告时，计数就会更新。|场景|
...
|使用情况|否|使用情况|Count|计数|API 调用计数|ApiCategory、ApiName、ResultType、ResponseCode|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|AssetCount|是|资产计数|Count|平均值|已在当前媒体服务帐户中创建的资产数量|无维度|
|AssetQuota|是|资产配额|Count|平均值|当前媒体服务帐户允许的资产数量|无维度|
|AssetQuotaUsedPercentage|是|资产配额已用百分比|百分比|平均值|当前媒体服务帐户中的资产已用百分比|无维度|
|ContentKeyPolicyCount|是|内容密钥策略计数|Count|平均值|当前媒体服务帐户中已创建的内容密钥策略数量|无维度|
|ContentKeyPolicyQuota|是|内容密钥策略配额|Count|平均值|当前媒体服务帐户允许的内容密钥策略数量|无维度|
...
|StreamingPolicyQuota|是|流式处理策略配额|Count|平均值|当前媒体服务帐户允许的流式处理策略数量|无维度|
...
|请求|是|请求|Count|总计|对流式处理终结点的请求。|OutputFormat，HttpStatusCode，ErrorCode|
...
|CbsVolumeBackupActive|是|卷备份活动状态|Count|平均值|当前卷的备份是否处于挂起状态。|无维度|
|CbsVolumeLogicalBackupBytes|是|已备份的逻辑字节|字节|平均值|已为此卷备份 Toatl 压缩/未加密的字节数。|无维度|
|CbsVolumeOperationComplete|是|操作状态|Count|平均值|上次备份/还原操作成功。|无维度|
|CbsVolumeOperationTransferredBytes|是|为操作传输的字节数|字节|平均值|上次备份/还原操作传输的总字节数。|无维度|
|CbsVolumeProtected|是|卷保护状态|Count|平均值|受云备份服务保护的卷。|无维度|
...
|XregionReplicationHealthy|是|卷复制状态是否正常|Count|平均值|关系的条件为1或0。|无维度|
|XregionReplicationLagTime|是|卷复制滞后时间|秒|平均值|镜像上的数据滞后于源的时间量（以秒为单位）。|无维度|
|XregionReplicationLastTransferDuration|是|卷复制上次传输持续时间|秒|平均值|完成最后一次传输所用的时间（以秒为单位）。|无维度|
|XregionReplicationLastTransferSize|是|卷复制的上次传输大小|字节|平均值|作为上次传输的一部分传输的总字节数。|无维度|
|XregionReplicationRelationshipProgress|是|卷复制进度|字节|平均值|当前传输操作传输的总数据量。|无维度|
|XregionReplicationRelationshipTransferring|是|卷复制传输|计数|平均值|卷复制的状态是否为 "正在传输"。|无维度|
|XregionReplicationTotalTransferBytes|是|卷复制总传输|字节|平均值|为关系传输的累计字节数。|无维度|
...
|BackendConnectTime|否|后端连接时间|毫秒|平均值|与后端服务器建立连接所花费的时间|侦听器、BackendServer、BackendPool、BackendHttpSetting|
|BackendFirstByteResponseTime|否|后端第一个字节响应时间|毫秒|平均值|从开始与后端服务器建立连接，到收到响应标头的第一个字节的间隔时间，大致相当于后端服务器的处理时间|侦听器、BackendServer、BackendPool、BackendHttpSetting|
|BackendLastByteResponseTime|否|后端最后一个字节响应时间|毫秒|平均值|从开始与后端服务器建立连接，到收到响应正文的最后一个字节的间隔时间|侦听器、BackendServer、BackendPool、BackendHttpSetting|
|BackendResponseStatus|是|后端响应状态|Count|总计|后端成员生成的 HTTP 响应代码数。 这不包括应用程序网关生成的任何响应代码。|BackendServer、BackendPool、BackendHttpSetting、HttpStatusGroup|
|BlockedCount|是|Web 应用程序防火墙阻止的请求规则分发|计数|总计|Web 应用程序防火墙阻止的请求规则分发|RuleGroup、RuleId|
|BlockedReqCount|是|Web 应用程序防火墙阻止的请求计数|Count|总计|Web 应用程序防火墙阻止的请求计数|无维度|
|BytesReceived|是|接收的字节数|字节|总计|应用程序网关从客户端收到的字节总数|侦听器|
|BytesSent|是|发送的字节数|字节|总计|应用程序网关向客户端发送的字节总数|侦听器|
|CapacityUnits|否|当前容量单位数|Count|平均值|已消耗的请求单位数|无维度|
|ClientRtt|否|客户端 RTT|毫秒|平均值|客户端与应用程序网关之间的平均往返时间。 此指标指示建立连接和返回确认所用的时间|侦听器|
|ComputeUnits|否|当前计算单位数|Count|平均值|消耗的计算单位数|无维度|
|CpuUtilization|否|CPU 使用率|百分比|平均值|应用程序网关的当前 CPU 利用率|无维度|
|CurrentConnections|是|当前连接数|Count|总计|使用应用程序网关建立的当前连接计数|无维度|
|EstimatedBilledCapacityUnits|否|预估计费容量单位数|计数|平均值|预估收费容量单位数|无维度|
|FailedRequests|是|失败的请求数|Count|总计|应用程序网关已提供服务的失败请求计数|BackendSettingsPool|
|FixedBillableCapacityUnits|否|固定的可计费容量单位|计数|平均值|最小收费容量单位数|无维度|
|HealthyHostCount|是|正常的主机计数|Count|平均值|正常的后端主机数|BackendSettingsPool|
|MatchedCount|是|Web 应用程序防火墙规则分发总数|Count|总计|传入流量的 Web 应用程序防火墙规则分发总数|RuleGroup、RuleId|
|NewConnectionsPerSecond|否|每秒新连接数|每秒计数|平均值|每秒使用应用程序网关建立的新连接数|无维度|
|ResponseStatus|是|响应状态|Count|总计|应用程序网关返回的 Http 响应状态|HttpStatusGroup|
|吞吐量|否|吞吐量|每秒字节数|平均值|应用程序网关每秒提供的字节数|无维度|
|TlsProtocol|是|客户端 TLS 协议|计数|总计|与应用程序网关建立了连接的客户端发起的 TLS 和非 TLS 请求数。 若要查看 TLS 协议分布，请按“TLS 协议”维度进行筛选。|侦听器，TlsProtocol|
|TotalRequests|是|请求总数|Count|总计|应用程序网关已提供服务的成功请求计数|BackendSettingsPool|
|UnhealthyHostCount|是|不正常的主机计数|Count|平均值|不正常的后端主机数|BackendSettingsPool|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ApplicationRuleHit|是|应用程序规则命中次数|Count|总计|应用程序规则的命中次数|Status、Reason、Protocol|
|DataProcessed|是|已处理的数据|字节|总计|此防火墙处理的数据总量|无维度|
|FirewallHealth|是|防火墙运行状况|百分比|平均值|指示此防火墙的总体运行状况|状态，原因|
|NetworkRuleHit|是|网络规则命中次数|Count|总计|网络规则的命中次数|Status、Reason、Protocol|
|SNATPortUtilization|是|SNAT 端口利用率|百分比|平均值|当前正在使用的出站 SNAT 端口的百分比|协议|
...
|RecordSetCount|是|记录集计数|Count|最大值|DNS 区域中的记录集数|无维度|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ArpAvailability|是|Arp 可用性|百分比|平均值|从 MSEE 到所有对等方的 ARP 可用性。|PeeringType、对等|
|BgpAvailability|是|Bgp 可用性|百分比|平均值|从 MSEE 到所有对等方的 BGP 可用性。|PeeringType、对等|
...
|RxLightLevel|是|RxLightLevel|Count|平均值|Rx 光能级（以 dBm 为单位）|链接，通道|
|TxLightLevel|是|TxLightLevel|Count|平均值|Tx 光能级（以 dBm 为单位）|链接，通道|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|AllocatedSnatPorts|否|分配的 SNAT 端口|Count|平均值|时间段内已分配的 SNAT 端口总数|FrontendIPAddress、BackendIPAddress、ProtocolType、 |
...
|UsedSnatPorts|否|已使用 SNAT 端口|计数|平均值|时间段内 SNAT 端口的总数|FrontendIPAddress、BackendIPAddress、ProtocolType、 |
...
|ChecksFailedPercent|是|检查失败百分比（预览版）|百分比|平均值|连接监视检查失败百分比|SourceAddress、、SourceResourceId、SourceType、Protocol、DestinationAddress、DestinationName、DestinationResourceId、DestinationType、DestinationPort、TestGroupName、TestConfigurationName|
|ProbesFailedPercent|是|失败的探测百分比|百分比|平均值|失败的连接监视探测百分比|无维度|
|RoundTripTimeMs|是|往返时间（毫秒）（预览版）|毫秒|平均值|连接监视检查的往返时间（以毫秒为单位）|SourceAddress、、SourceResourceId、SourceType、Protocol、DestinationAddress、DestinationName、DestinationResourceId、DestinationType、DestinationPort、TestGroupName、TestConfigurationName|
...
|ThrottledRequests|否|限制的请求数。|计数|总计|Microsoft.ServiceBus 限制的请求数。|EntityName、 |
...
|dw_backup_size_gb|是|数据存储大小(GB)|Count|总计|数据存储大小由数据和事务日志的大小组成。 此指标将计入帐单的 "存储" 部分。 仅适用于数据仓库。|无维度|
|dw_geosnapshot_size_gb|是|灾难恢复存储大小 (GB) |Count|总计|灾难恢复存储大小在帐单中反映为 "灾难恢复存储"。 仅适用于数据仓库。|无维度|
|dw_snapshot_size_gb|是|快照存储大小 (GB) |Count|总计|快照存储大小是快照捕获的增量更改的大小，用于创建用户定义的还原点和自动还原点。 此指标将计入帐单的 "存储" 部分。 仅适用于数据仓库。|无维度|
...
|storage|是|已用数据空间|字节|最大值|已用数据空间。 不适用于数据仓库。|无维度|
...
|---|---|---|---|---|---|---|
|可用性|是|可用性|百分比|平均值|存储服务或指定的 API 操作的可用性百分比。 可用性通过由 TotalBillableRequests 值除以适用的请求数（其中包括引发意外错误的请求）计算得出。 所有意外错误都会导致存储服务或指定的 API 操作的可用性下降。|GeoType, ApiName, Authentication|
|流出量|是|流出量|字节|总计|流出数据量。 此数量包括从 Azure 存储传出到外部客户端，以及在 Azure 中传出。 因此，此数字不反映计费的流出量。|GeoType, ApiName, Authentication|
|流入量|是|流入量|字节|总计|流入的数据量（以字节为单位）。 此数字包括从外部客户端到 Azure 存储流入的数据量，以及流入 Azure 中的数据量。|GeoType, ApiName, Authentication|
...
|事务|是|事务|Count|总计|向存储服务或指定的 API 操作发出的请求数。 此数值包括成功和失败的请求数，以及引发错误的请求数。 针对不同类型的响应数使用 ResponseType 维度。|ResponseType, GeoType, ApiName, Authentication|
|UsedCapacity|否|已用容量|字节|平均值|存储帐户使用的存储量。 对于标准存储帐户，该指标是 Blob、表、文件和队列使用的容量总和。 对于高级存储帐户和 Blob 存储帐户，该帐户与 Blobcapacity 相同或 FileCapacity 相同。|无维度|
...
|BlobProvisionedSize|否|Blob 预配大小|字节|平均值|存储帐户的 Blob 服务中预配的存储量（以字节为单位）。|BlobType、Tier|
...
|---|---|---|---|---|---|---|
|可用性|是|可用性|百分比|平均值|存储服务或指定的 API 操作的可用性百分比。 可用性通过由 TotalBillableRequests 值除以适用的请求数（其中包括引发意外错误的请求）计算得出。 所有意外错误都会导致存储服务或指定的 API 操作的可用性下降。|GeoType、ApiName、Authentication、文件共享|
|流出量|是|流出量|字节|总计|流出数据量。 此数量包括从 Azure 存储传出到外部客户端，以及在 Azure 中传出。 因此，此数字不反映计费的流出量。|GeoType、ApiName、Authentication、文件共享|
|FileCapacity|否|文件容量|字节|平均值|存储帐户使用的文件存储量。|FileShare|
|FileCount|否|文件计数|Count|平均值|存储帐户中的文件数目。|FileShare|
|FileShareCapacityQuota|否|文件共享容量配额|字节|平均值|Azure 文件服务可使用的存储量上限（以字节为单位）。|FileShare|
|FileShareCount|否|文件共享计数|Count|平均值|存储帐户中的文件共享数目。|无维度|
|FileShareProvisionedIOPS|否|文件共享预配 IOPS|字节|平均值|高级文件存储帐户中高级文件共享的预配 IOPS 的基线数。 此数字是基于预配的大小 (配额) 共享容量计算得出的。|FileShare|
|FileShareSnapshotCount|否|文件共享快照计数|Count|平均值|存储帐户文件服务的共享中存在的快照数。|FileShare|
|FileShareSnapshotSize|否|文件共享快照大小|字节|平均值|存储帐户的文件服务中的快照所用存储量（以字节为单位）。|FileShare|
|流入量|是|流入量|字节|总计|流入的数据量（以字节为单位）。 此数字包括从外部客户端到 Azure 存储流入的数据量，以及流入 Azure 中的数据量。|GeoType、ApiName、Authentication、文件共享|
|SuccessE2ELatency|是|成功 E2E 延迟|毫秒|平均值|向存储服务或指定的 API 操作发出的成功请求的平均端到端延迟（以毫秒为单位）。 此值包括在 Azure 存储中读取请求、发送响应和接收响应确认所需的处理时间。|GeoType、ApiName、Authentication、文件共享|
|SuccessServerLatency|是|成功服务器延迟|毫秒|平均值|Azure 存储处理成功请求所用的平均时间。 此值不包括 SuccessE2ELatency 中指定的网络延迟。|GeoType、ApiName、Authentication、文件共享|
|事务|是|事务|Count|总计|向存储服务或指定的 API 操作发出的请求数。 此数值包括成功和失败的请求数，以及引发错误的请求数。 针对不同类型的响应数使用 ResponseType 维度。|ResponseType、GeoType、ApiName、Authentication、文件共享|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|OrchestrationActivityRunsEnded|否|已结束活动运行数|计数|总计|成功、失败或取消的业务流程活动计数|Result，FailureType，Activity，ActivityType，管道|
|OrchestrationPipelineRunsEnded|否|已结束管道运行数|计数|总计|成功、失败或取消的业务流程管道运行计数|Result、FailureType、管道|
...
|SparkJobsEnded|是|已结束应用程序数|计数|总计|已结束的应用程序计数|JobType、Jobresult.txt|
...
|HttpQueueLength|是|Http 队列长度|Count|平均值|Http 队列长度|实例|
|MemoryPercentage|是|内存百分比|百分比|平均值|内存百分比|实例|
|SocketInboundAll|是|SocketInboundAll|Count|平均值|SocketInboundAll|实例|
|SocketLoopback|是|SocketLoopback|Count|平均值|SocketLoopback|实例|
|SocketOutboundAll|是|SocketOutboundAll|Count|平均值|SocketOutboundAll|实例|
|SocketOutboundEstablished|是|SocketOutboundEstablished|Count|平均值|SocketOutboundEstablished|实例|
|SocketOutboundTimeWait|是|SocketOutboundTimeWait|Count|平均值|SocketOutboundTimeWait|实例|
...
|AverageResponseTime|是|平均响应时间** (弃用) **|秒|平均值|平均响应时间|实例|
|BytesReceived|是|数据输入|字节|总计|数据输入|实例|
|BytesSent|是|数据输出|字节|总计|数据输出|实例|
