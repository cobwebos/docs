...
|---|---|---|---|---|---|
|CPU 百分比|CPU 百分比|百分比|平均值|当前虚拟机正在使用的已分配计算单元百分比|无维度|
|网络传入|在可计费的网络|字节|总计|所有网络接口上收到的虚拟机 （传入流量） 的可计费字节数|无维度|
|网络传出|扩展可计费的网络|字节|总计|通过虚拟机 （传出流量） 的所有网络接口上的可计费字节数|无维度|
|磁盘读取字节数|磁盘读取字节数|字节|总计|监视期间从磁盘读取字节数|无维度|
|磁盘写入字节数|磁盘写入字节数|字节|总计|磁盘监视期间写入的字节数|无维度|
|磁盘读取操作次数/秒|磁盘读取操作次数/秒|每秒计数|平均值|磁盘读取 IOPS|无维度|
|磁盘写入操作次数/秒|磁盘写入操作次数/秒|每秒计数|平均值|磁盘写入 IOPS|无维度|
|剩余 CPU 信用额度|剩余 CPU 信用额度|Count|平均值|可用来集中使用的总信用点数|无维度|
|已用 CPU 信用额度|已用 CPU 信用额度|Count|平均值|虚拟机使用的总信用点数|无维度|
|每磁盘读取字节数/秒|数据磁盘读取字节数/秒 （已弃用）|每秒计数|平均值|监视期间从单个磁盘读取的字节数/秒|SlotId|
|每磁盘写入字节数/秒|数据磁盘写入字节数/秒 （已弃用）|每秒计数|平均值|监视期间写入单个磁盘的字节数/秒|SlotId|
|每磁盘读取操作数/秒|数据磁盘读取操作数/秒 （已弃用）|每秒计数|平均值|从单个磁盘监视期间的读取 IOPS|SlotId|
|每磁盘写入操作数/秒|数据磁盘写入操作数/秒 （已弃用）|每秒计数|平均值|监视期间从单个磁盘写入 IOPS|SlotId|
|每磁盘 QD|数据磁盘 QD （已弃用）|Count|平均值|数据磁盘队列深度(或队列长度)|SlotId|
|OS 每磁盘读取字节数/秒|OS 磁盘读取字节数/秒 （已弃用）|每秒计数|平均值|在 OS 磁盘监视期间从单个磁盘读取的字节数/秒|无维度|
|OS 每磁盘写入字节数/秒|OS 磁盘写入字节数/秒 （已弃用）|每秒计数|平均值|在 OS 磁盘监视期间写入单个磁盘的字节数/秒|无维度|
|OS 每磁盘读取操作数/秒|OS 磁盘读取操作数/秒 （已弃用）|每秒计数|平均值|从单个磁盘监视期间 OS 磁盘读取 IOPS|无维度|
|OS 每磁盘写入操作数/秒|OS 磁盘写入操作数/秒 （已弃用）|每秒计数|平均值|在 OS 磁盘监视期间从单个磁盘写入 IOPS|无维度|
|OS 每磁盘 QD|OS 磁盘 QD （已弃用）|Count|平均值|OS 磁盘队列深度(或队列长度)|无维度|
|数据磁盘读取字节数/秒|数据磁盘读取字节数/秒（预览版）|每秒计数|平均值|监视期间从单个磁盘读取的字节数/秒|LUN|
|数据磁盘写入字节数/秒|数据磁盘写入字节数/秒（预览版）|每秒计数|平均值|监视期间写入单个磁盘的字节数/秒|LUN|
|数据磁盘读取操作数/秒|数据磁盘读取操作数/秒（预览版）|每秒计数|平均值|从单个磁盘监视期间的读取 IOPS|LUN|
|数据磁盘写入操作数/秒|数据磁盘写入操作数/秒（预览版）|每秒计数|平均值|监视期间从单个磁盘写入 IOPS|LUN|
|数据磁盘队列深度|数据磁盘队列深度 （预览版）|Count|平均值|数据磁盘队列深度(或队列长度)|LUN|
|OS 磁盘读取字节数/秒|OS 磁盘读取字节数/秒（预览版）|每秒计数|平均值|在 OS 磁盘监视期间从单个磁盘读取的字节数/秒|无维度|
|OS 磁盘写入字节数/秒|OS 磁盘写入字节数/秒（预览版）|每秒计数|平均值|在 OS 磁盘监视期间写入单个磁盘的字节数/秒|无维度|
|OS 磁盘读取操作数/秒|OS 磁盘读取操作数/秒（预览版）|每秒计数|平均值|从单个磁盘监视期间 OS 磁盘读取 IOPS|无维度|
|OS 磁盘写入操作数/秒|OS 磁盘写入操作数/秒（预览版）|每秒计数|平均值|在 OS 磁盘监视期间从单个磁盘写入 IOPS|无维度|
|OS 磁盘队列深度|OS 磁盘队列深度 （预览版）|Count|平均值|OS 磁盘队列深度(或队列长度)|无维度|
|入站的流|入站的流 （预览版）|Count|平均值|入站的流的数字的当前流中的入站方向 （转到 VM 的流量）|无维度|
|出站流|出站流 （预览版）|Count|平均值|出站流的数字的当前流中的出站方向 （传出 VM 的流量）|无维度|
|入站的流最大的创建速率|入站的流最大的创建速率 （预览版）|每秒计数|平均值|入站流量 （流量进入虚拟机） 的最大的创建速率|无维度|
|出站流最大的创建速率|出站流最大的创建速率 （预览版）|每秒计数|平均值|最大的创建速率的出站流 （传出 VM 的流量）|无维度|
|高级数据磁盘缓存读取命中|高级数据磁盘缓存读取命中 （预览版）|Percent|平均值|高级数据磁盘缓存读取命中|LUN|
|高级数据磁盘缓存读取未命中|高级数据磁盘缓存读取未命中 （预览版）|Percent|平均值|高级数据磁盘缓存读取未命中|LUN|
|高级 OS 磁盘缓存读取命中|高级 OS 磁盘缓存读取命中 （预览版）|Percent|平均值|高级 OS 磁盘缓存读取命中|无维度|
|高级 OS 磁盘缓存读取未命中|高级 OS 磁盘缓存读取未命中 （预览版）|Percent|平均值|高级 OS 磁盘缓存读取未命中|无维度|
|总共网络|总共网络|字节|总计|虚拟机在所有网络接口上收到的字节数（传入流量）|无维度|
...
|网络传入|在可计费的网络|字节|总计|所有网络接口上收到的虚拟机 （传入流量） 的可计费字节数|VMName|
|网络传出|扩展可计费的网络|字节|总计|通过虚拟机 （传出流量） 的所有网络接口上的可计费字节数|VMName|
|磁盘读取字节数|磁盘读取字节数|字节|总计|监视期间从磁盘读取字节数|VMName|
|磁盘写入字节数|磁盘写入字节数|字节|总计|磁盘监视期间写入的字节数|VMName|
...
|OS 每磁盘写入操作数/秒|OS 磁盘写入操作数/秒 （已弃用）|每秒计数|平均值|在 OS 磁盘监视期间从单个磁盘写入 IOPS|无维度|
|OS 每磁盘 QD|OS 磁盘 QD （已弃用）|Count|平均值|OS 磁盘队列深度(或队列长度)|无维度|
|数据磁盘读取字节数/秒|数据磁盘读取字节数/秒（预览版）|每秒计数|平均值|监视期间从单个磁盘读取的字节数/秒|LUN VMName|
|数据磁盘写入字节数/秒|数据磁盘写入字节数/秒（预览版）|每秒计数|平均值|监视期间写入单个磁盘的字节数/秒|LUN VMName|
|数据磁盘读取操作数/秒|数据磁盘读取操作数/秒（预览版）|每秒计数|平均值|从单个磁盘监视期间的读取 IOPS|LUN VMName|
|数据磁盘写入操作数/秒|数据磁盘写入操作数/秒（预览版）|每秒计数|平均值|监视期间从单个磁盘写入 IOPS|LUN VMName|
|数据磁盘队列深度|数据磁盘队列深度 （预览版）|Count|平均值|数据磁盘队列深度(或队列长度)|LUN VMName|
|OS 磁盘读取字节数/秒|OS 磁盘读取字节数/秒（预览版）|每秒计数|平均值|在 OS 磁盘监视期间从单个磁盘读取的字节数/秒|VMName|
|OS 磁盘写入字节数/秒|OS 磁盘写入字节数/秒（预览版）|每秒计数|平均值|在 OS 磁盘监视期间写入单个磁盘的字节数/秒|VMName|
|OS 磁盘读取操作数/秒|OS 磁盘读取操作数/秒（预览版）|每秒计数|平均值|从单个磁盘监视期间 OS 磁盘读取 IOPS|VMName|
|OS 磁盘写入操作数/秒|OS 磁盘写入操作数/秒（预览版）|每秒计数|平均值|在 OS 磁盘监视期间从单个磁盘写入 IOPS|VMName|
|OS 磁盘队列深度|OS 磁盘队列深度 （预览版）|Count|平均值|OS 磁盘队列深度(或队列长度)|VMName|
|入站的流|入站的流 （预览版）|Count|平均值|入站的流的数字的当前流中的入站方向 （转到 VM 的流量）|VMName|
|出站流|出站流 （预览版）|Count|平均值|出站流的数字的当前流中的出站方向 （传出 VM 的流量）|VMName|
|入站的流最大的创建速率|入站的流最大的创建速率 （预览版）|每秒计数|平均值|入站流量 （流量进入虚拟机） 的最大的创建速率|VMName|
|出站流最大的创建速率|出站流最大的创建速率 （预览版）|每秒计数|平均值|最大的创建速率的出站流 （传出 VM 的流量）|VMName|
|高级数据磁盘缓存读取命中|高级数据磁盘缓存读取命中 （预览版）|Percent|平均值|高级数据磁盘缓存读取命中|LUN VMName|
|高级数据磁盘缓存读取未命中|高级数据磁盘缓存读取未命中 （预览版）|Percent|平均值|高级数据磁盘缓存读取未命中|LUN VMName|
|高级 OS 磁盘缓存读取命中|高级 OS 磁盘缓存读取命中 （预览版）|Percent|平均值|高级 OS 磁盘缓存读取命中|VMName|
|高级 OS 磁盘缓存读取未命中|高级 OS 磁盘缓存读取未命中 （预览版）|Percent|平均值|高级 OS 磁盘缓存读取未命中|VMName|
|总共网络|总共网络|字节|总计|虚拟机在所有网络接口上收到的字节数（传入流量）|VMName|
...
|d2c.endpoints.egress.builtIn.events|路由：发送到消息/事件的消息数|Count|总计|IoT 中心路由成功将消息发送到内置终结点（消息/事件）的次数。 此指标只启动时启用了路由 (https://aka.ms/iotrouting) 的 IoT 中心。|无维度|
|d2c.endpoints.latency.builtIn.events|路由：消息/事件的消息延迟|毫秒|平均值|消息进入 IoT 中心与遥测消息进入内置终结点（消息/事件）之间的平均延迟时间（毫秒）。 此指标只启动时启用了路由 (https://aka.ms/iotrouting) 的 IoT 中心。|无维度|
...
|EventGridDeliveries|事件网格传送（预览版）|Count|总计|IoT 中心事件数发布到事件网格。 使用结果维度的成功和失败请求数。 事件类型维度显示事件的类型 (https://aka.ms/ioteventgrid) 。|Result、EventType|
|EventGridLatency|从在 Iot 中心事件时已发布到事件网格事件的生成到的平均延迟 （毫秒）。 此数值是所有事件类型的平均。 若要查看特定事件类型的延迟，请使用 EventType 维度。|EventType|
...
|performanceCounters/processorCpuPercentage|处理器时间|Percent|平均值|处理器在非空闲线程上所花费的时间百分比。|cloud/roleInstance|
...
|失败的运行次数|失败的运行次数|Count|总计|此工作区失败的运行数|场景|
...
|storage|已用数据空间|字节|最大值|数据库总大小|无维度|
...
|workers_percent|辅助角色百分比|Percent|平均值|辅助角色百分比|无维度|
...
|storage_used|已用数据空间|字节|平均值|已用的存储量|无维度|
|xtp_storage_percent|内存中 OLTP 存储百分比|Percent|平均值|内存中 OLTP 存储百分比|无维度|
|cpu_limit|CPU 限制|Count|平均值|CPU 限制|无维度|
|cpu_used|已用 CPU|Count|平均值|已用 CPU|无维度|
|allocated_data_storage|已分配的数据空间|字节|平均值|已分配的数据空间|无维度|
...
|BlobCount|Blob 计数|Count|总计|存储帐户的 Blob 服务中的 Blob 数。|/BlobType|       |BlobCount|Blob 计数|Count|平均值|存储帐户的 Blob 服务中的 Blob 数。|BlobType、Tier|
