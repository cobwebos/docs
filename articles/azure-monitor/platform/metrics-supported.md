...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|CoreCount|专用核心计数|Count|总计|批处理帐户中的专用核心总数|None|
|TotalNodeCount|专用节点计数|Count|总计|批处理帐户中的专用节点总数|None|
|LowPriorityCoreCount|低优先级核心计数|Count|总计|批处理帐户中的低优先级核心总数|None|
|TotalLowPriorityNodeCount|低优先级节点计数|Count|总计|批处理帐户中的低优先级节点总数|None|
|CreatingNodeCount|正在创建的节点计数|Count|总计|正在创建的节点数目|None|
|StartingNodeCount|正在启动的节点计数|Count|总计|正在启动的节点数目|None|
|WaitingForStartTaskNodeCount|正在等待启动任务的节点计数|Count|总计|正在等待启动任务完成的节点数目|None|
|StartTaskFailedNodeCount|启动任务失败的节点计数|Count|总计|启动任务失败的节点数目|None|
|IdleNodeCount|空闲节点计数|Count|总计|空闲节点数目|None|
|OfflineNodeCount|脱机节点计数|Count|总计|脱机节点数目|None|
|RebootingNodeCount|正在重新启动的节点计数|Count|总计|正在重新启动的节点数目|None|
|ReimagingNodeCount|正在重置映像的节点计数|Count|总计|正在重置映像的节点数目|None|
|RunningNodeCount|正在运行的节点计数|Count|总计|正在运行的节点数目|None|
|LeavingPoolNodeCount|正在退出池的节点计数|Count|总计|正在退出池的节点数目|None|
|UnusableNodeCount|不可用的节点计数|Count|总计|不可用的节点数目|None|
|PreemptedNodeCount|已占用节点计数|Count|总计|已占用节点数|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|CPU 百分比|CPU 百分比|百分比|平均值|当前虚拟机正在使用的已分配计算单元百分比。|None|
|网络传入|网络传入|字节|总计|虚拟机在所有网络接口上收到的字节数（传入流量）。|None|
|网络传出|网络传出|字节|总计|虚拟机在所有网络接口上发出的字节数（传出流量）。|None|
|Disk Read Bytes/Sec|磁盘读取|每秒字节数|平均值|监视期间从磁盘读取的平均字节数。|None|
|Disk Write Bytes/Sec|磁盘写入|每秒字节数|平均值|监视期间向磁盘写入的平均字节数。|None|
...
|ContainerCount|Blob 容器计数|Count|平均值|存储帐户的 Blob 服务中的容器数。|None|
...
|---|---|---|---|---|---|
|CPU 百分比|CPU 百分比|百分比|平均值|当前虚拟机正在使用的已分配计算单元百分比|None|
|网络传入|可计费网络（已弃用）|字节|总计|虚拟机在所有网络接口上收到的可计费字节数（传入流量）（已弃用）|None|
|网络传出|可计费网络流出量（已弃用）|字节|总计|通过虚拟机传出的所有网络接口上的可计费字节数（传出流量）（已弃用）|None|
|磁盘读取字节数|磁盘读取字节数|字节|总计|监视期间从磁盘读取的字节数|None|
|磁盘写入字节数|磁盘写入字节数|字节|总计|监视期间向磁盘写入的字节数|None|
|磁盘读取操作次数/秒|磁盘读取操作次数/秒|每秒计数|平均值|磁盘读取 IOPS|None|
|磁盘写入操作次数/秒|磁盘写入操作次数/秒|每秒计数|平均值|磁盘写入 IOPS|None|
...
|OS 每磁盘读取字节数/秒|OS 磁盘读取字节/秒[（已弃用）](portal-disk-metrics-deprecation.md)|每秒计数|平均值|OS 磁盘监视期间每秒从单个磁盘读取的字节数|None|
|OS 每磁盘写入字节数/秒|OS 磁盘写入字节/秒[（已弃用）](portal-disk-metrics-deprecation.md)|每秒计数|平均值|OS 磁盘监视期间每秒写入到单个磁盘的字节数|None|
|OS 每磁盘读取操作数/秒|OS 磁盘读取操作/秒[（已弃用）](portal-disk-metrics-deprecation.md)|每秒计数|平均值|OS 磁盘监视期间从单个磁盘执行的读取 IOPS|None|
|OS 每磁盘写入操作数/秒|OS 磁盘写入操作/秒[（已弃用）](portal-disk-metrics-deprecation.md)|每秒计数|平均值|OS 磁盘监视期间从单个磁盘执行的写入 IOPS|None|
|OS 每磁盘 QD|OS 磁盘[QD（已弃用）](portal-disk-metrics-deprecation.md)|Count|平均值|OS 磁盘队列深度(或队列长度)|None|
...
|OS 磁盘读取字节数/秒|OS 磁盘读取字节数/秒（预览版）|每秒计数|平均值|OS 磁盘监视期间每秒从单个磁盘读取的字节数|None|
|OS 磁盘写入字节数/秒|OS 磁盘写入字节数/秒（预览版）|每秒计数|平均值|OS 磁盘监视期间每秒写入到单个磁盘的字节数|None|
|OS 磁盘读取操作次数/秒|OS 磁盘读取操作数/秒（预览版）|每秒计数|平均值|OS 磁盘监视期间从单个磁盘执行的读取 IOPS|None|
|OS 磁盘写入操作次数/秒|OS 磁盘写入操作数/秒（预览版）|每秒计数|平均值|OS 磁盘监视期间从单个磁盘执行的写入 IOPS|None|
|OS 磁盘队列深度|OS 磁盘队列深度（预览版）|Count|平均值|OS 磁盘队列深度(或队列长度)|None|
|入站流数|入站流数|Count|平均值|入站流数是入站方向的当前流（传入 VM 的流量）的数目|None|
|出站流数|出站流数|Count|平均值|出站流数是出站方向的当前流（传出 VM 的流量）的数目|None|
|入站流最大创建速率|入站流最大创建速率|每秒计数|平均值|入站流（传入 VM 的流量）的最大创建速率|None|
|出站流最大创建速率|出站流最大创建速率|每秒计数|平均值|出站流（传出 VM 的流量）的最大创建速率|None|
|高级数据磁盘缓存读取命中|高级数据磁盘缓存读取命中（预览版）|百分比|平均值|高级数据磁盘缓存读取命中|LUN|
|高级数据磁盘缓存读取未命中|高级数据磁盘缓存读取未命中（预览版）|百分比|平均值|高级数据磁盘缓存读取未命中|LUN|
|高级 OS 磁盘缓存读取命中|高级 OS 磁盘缓存读取命中（预览版）|百分比|平均值|高级 OS 磁盘缓存读取命中|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|TotalPullCount|提取总数|Count|平均值|映像提取总次数|None|
|SuccessfulPullCount|成功提取计数|Count|平均值|成功的映像提取次数|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|kube_node_status_allocatable_cpu_cores|托管群集中可用 CPU 内核的总数|Count|平均值|托管群集中可用 CPU 内核的总数|None|
...
|TotalCapacity|总容量|字节|平均值|总容量|None|
|AvailableCapacity|可用容量|字节|平均值|报告期间可用容量(以字节为单位)。|None|
...
|具有分类的资产数量|具有至少一个分类的资产数量|Count|平均值|指示具有至少一个标记分类的资产数。|None|
|扫描取消|扫描已取消|Count|总计|指示已取消的扫描数。|None|
|扫描完成|扫描已完成|Count|总计|指示成功完成的扫描数。|None|
|扫描失败|扫描失败|Count|总计|指示扫描失败数。|None|
|扫描时间|扫描时间|秒|总计|指示总扫描时间（以秒为单位）。|None|
|目录活动用户|每日活跃用户|Count|总计|每日活动用户数|None|
...
|ResourceCount|实体总数|Count|最大值||None|
|FactorySizeInGbUnits|总工厂大小（以 GB 为单位）|Count|最大值||None|
...
|作业阶段|阶段中的作业|Count|总计|每个阶段的作业数。|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|TotalStorage|总存储|字节|最大值|帐户中存储的数据总量。|None|
|DataWritten|写入的数据量|字节|总计|写入帐户的数据总量。|None|
|DataRead|读取的数据量|字节|总计|从帐户中读取的数据总量。|None|
|WriteRequests|写入请求数|Count|总计|帐户的数据写入请求计数。|None|
|ReadRequests|读取请求数|Count|总计|帐户的数据读取请求计数。|None|
...
|成功共享同步|已发送的共享成功快照|Count|Count|帐户中已发送的已发送共享成功快照数|None|
|失败的共享同步|发送的共享失败快照|Count|Count|帐户中发送的共享失败快照数|None|
|成功共享订阅同步|已接收的共享成功快照|Count|Count|帐户中已接收的已接收共享成功快照数|None|
|失败的共享订阅同步|收到的共享失败快照|Count|Count|帐户中已接收的共享失败快照数|None|
...
|backup_storage_used|已用的备份存储|字节|平均值|已用的备份存储|None|
...
|network_bytes_ingress|网络传入|字节|总计|跨活动连接数的网络传入|None|
|connections_failed|失败的连接数|Count|总计|失败的连接数|None|
|connections_succeeded|成功连接|Count|总计|成功连接|None|
|maximum_used_transactionIDs|最大已使用事务 ID|Count|平均值|最大已使用事务 ID|None|
...
|c2d.methods.failure|失败的直接方法调用数|Count|总计|所有失败的直接方法调用的计数。|None|
...
|c2d.twin.read.success|后端的成功克隆读取数|Count|总计|由后端发起的所有成功的克隆读取的计数。|None|
|c2d.twin.read.failure|后端的失败克隆读取数|Count|总计|由后端发起的所有失败的克隆读取的计数。|None|
|c2d.twin.read.size|后端的克隆读取的响应大小|字节|平均值|由后端发起的所有成功的克隆读取的平均大小、最小大小和最大大小。|None|
|c2d.twin.update.success|后端的成功克隆更新数|Count|总计|由后端发起的所有成功的克隆更新的计数。|None|
|c2d.twin.update.failure|后端的失败克隆更新数|Count|总计|由后端发起的所有失败的克隆更新的计数。|None|
...
|UpdateAccountNetworkSettings|更新的帐户网络设置数|Count|Count|更新的帐户网络设置数|None|
|UpdateAccountReplicationSettings|更新的帐户复制设置数|Count|Count|更新的帐户复制设置数|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|TransactionCount|事务计数|Count|Count|事务总数|TransactionCount|
|SuccessCount|成功计数|Count|Count|成功的事务数|SuccessCount|
|FailureCount|失败计数|Count|Count|失败的事务数|FailureCount|
...
|OUTMSGS|传出的消息数（已过时）（已弃用）|Count|总计|命名空间的传出消息总数。 此指标已弃用。 请改用传出消息指标（已弃用）|None|
...
|NumActiveWorkers|活动辅助角色数目|Count|最大值|活动辅助角色数目|MetricName|
...
|ObservedCapacity|观察到的容量|Count|平均值|自动缩放执行时报告的容量。|None|
...
|exceptions/count|异常|Count|Count|所有未捕获异常的已合并计数。|cloud/roleName,cloud/roleInstance,client/type|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|connectedDeviceCount|连接设备总数|Count|平均值|连接到 IoT 中心的设备数量|None|
|c2d.属性.阅读.成功|成功的设备属性从 IoT 中心读取|Count|总计|从 IoT 中央启动的所有成功属性读取的计数|None|
|c2d.属性.读取.失败|从 IoT 中心读取失败的设备属性|Count|总计|从 IoT 中央启动的所有失败属性读取的计数|None|
|d2c.属性.阅读.成功|成功的设备属性从设备读取|Count|总计|从设备启动的所有成功属性读取的计数|None|
|d2c.属性.读取.失败|从设备读取失败的设备属性|Count|总计|从设备启动的所有失败属性读取的计数|None|
|c2d.属性.更新.成功|从 IoT 中心成功更新设备属性|Count|总计|从 IoT 中央启动的所有成功属性更新的计数|None|
|c2d.属性.更新.失败|来自 IoT 中心的设备属性更新失败|Count|总计|从 IoT 中央启动的所有失败属性更新的计数|None|
|d2c.属性.更新.成功|成功从设备更新设备属性|Count|总计|从设备启动的所有成功属性更新的计数|None|
|d2c.属性.更新.失败|设备更新失败|Count|总计|从设备启动的所有失败属性更新的计数|None|
...
|ContinuousExportMaxLatenessMinutes|连续导出最大延迟|Count|最大值|群集中连续导出作业报告的延迟（以分钟）为|None|
...
|StreamingIngestDataRate|流引入数据速率|Count|平均值|流引入数据速率（MB/秒）|None|
...
|TotalNumberOfExtents|盘区总数|Count|总计|数据盘区总数|None|
|InstanceCount|实例计数|Count|平均值|实例计数总数|None|
...
|正在退出的核心数|正在退出的核心数|Count|平均值|正在退出的核心数|Scenario,ClusterName|
|配额利用率百分比|配额利用率百分比|Count|平均值|已利用的配额百分比|Scenario,ClusterName,VmFamilyName,VmPriority|
|CpuUtilization|CpuUtilization|Count|平均值|CPU（预览）|Scenario,runId,NodeId,CreatedTime|
|GpuUtilization|GpuUtilization|Count|平均值|GPU（预览版）|Scenario,runId,NodeId,CreatedTime,DeviceId|
...
|VolumeLogicalSize|体积消耗大小|字节|平均值|卷的逻辑大小（所用字节数）|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|VolumePoolAllocatedUsed|分配给卷大小的池|字节|平均值|分配池已使用的大小|None|
...
|新连接数秒|每秒新连接数|每秒计数|平均值|使用应用程序网关建立每秒新连接|None|
|CpuUtilization|CPU 使用率|百分比|平均值|应用程序网关的当前 CPU 利用率|None|
|CapacityUnits|当前容量单位数|Count|平均值|已消耗的请求单位数|None|
|固定可计费容量单位|固定计费容量单位|Count|平均值|将收取的最低容量单位|None|
|估计已计费容量单位|估计计费容量单位|Count|平均值|将收取的估计容量单位|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|RxLightLevel|RxLightLevel|Count|平均值|Rx 光能级（以 dBm 为单位）|Link,Lane|
|TxLightLevel|TxLightLevel|Count|平均值|Tx 光能级（以 dBm 为单位）|Link,Lane|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|QueryVolume|查询量|Count|总计|为专用 DNS 区域服务的查询数|None|
|RecordSetCount|记录集计数|Count|最大值|专用 DNS 区域中的记录集数|None|
|RecordSetCapacityUtilization|记录集容量使用率|百分比|最大值|专用 DNS 区域使用的记录集容量百分比|None|
|虚拟网络链接计数|虚拟网络链路计数|Count|最大值|链接到专用 DNS 区域的虚拟网络数|None|
|虚拟网络链接容量利用率|虚拟网络链路容量利用率|百分比|最大值|专用 DNS 区域使用的虚拟网络链路容量百分比|None|
|虚拟网络与注册链接计数|虚拟网络注册链接计数|Count|最大值|连接到启用自动注册的专用 DNS 区域的虚拟网络数|None|
|具有注册容量的虚拟网络|虚拟网络注册链路容量利用率|百分比|最大值|具有专用 DNS 区域使用自动注册容量的虚拟网络链路百分比|None|
...
|incoming.scheduled|已发送的已安排推送通知数|Count|总计|已取消的已安排推送通知数|None|
...
|outgoing.mpns.pnserror|MPNS 错误数|Count|总计|因为与 MPNS 通信时发生错误而失败的推送的计数。|None|
|outgoing.mpns.authenticationerror|MPNS 身份验证错误数|Count|总计|因为 PNS 未接受所提供的凭据或者凭据被阻止而失败的推送的计数。|None|
|notificationhub.pushes|所有传出通知|Count|总计|通知中心的所有传出通知|None|
|incoming.all.requests|所有传入请求数|Count|总计|通知中心的传入的请求数总计|None|
|incoming.all.failedrequests|所有传入的失败请求数|Count|总计|通知中心的传入的失败请求数总计|None|
...
|sqlserver_process_core_percent|SQL Server 进程核心百分比|百分比|最大值|SQL Server 进程的 CPU 使用率百分比，由操作系统测量。|None|
|sqlserver_process_memory_percent|SQL Server 进程内存百分比|百分比|最大值|SQL Server 进程的内存使用百分比，由操作系统测量。|None|
|tempdb_data_size|Tempdb 数据文件大小 (KB)|Count|最大值|Tempdb 数据文件大小 (KB)。 不适用于数据仓库。|None|
|tempdb_log_size|Tempdb 日志文件大小 (KB)|Count|最大值|Tempdb 日志文件大小 (KB)。 不适用于数据仓库。|None|
|tempdb_log_used_percent|Tempdb 日志已用百分比|百分比|最大值|Tempdb 日志已用百分比。 不适用于数据仓库。|None|
...
|dw_backup_size_gb|数据存储大小|Count|总计|数据存储大小由数据的大小和事务日志组成。 指标计入账单的"存储"部分。 仅适用于数据仓库。|None|
|dw_snapshot_size_gb|快照存储大小|Count|总计|快照存储大小是快照捕获的增量更改的大小，用于创建用户定义和自动还原点。 指标计入账单的"存储"部分。 仅适用于数据仓库。|None|
...
|queued_queries|排队查询|Count|总计|跨所有工作负荷组的排队查询。 仅适用于数据仓库。|None|
...
|log_backup_size_bytes|日志备份存储大小|字节|最大值|累积的日志备份存储大小。 适用于基于 vCore 的超大规模数据库。|None|
|snapshot_backup_size_bytes|快照备份存储大小|字节|最大值|累积快照备份存储大小。 应用于超大规模数据库。|None|
|base_blob_size_bytes|基本 Blob 存储大小|字节|最大值|基本 blob 存储大小。 应用于超大规模数据库。|None|
...
|sqlserver_process_core_percent|SQL Server 进程核心百分比|百分比|最大值|SQL Server 进程的 CPU 使用率百分比，由操作系统测量。 适用于弹性池。|None|
|sqlserver_process_memory_percent|SQL Server 进程内存百分比|百分比|最大值|SQL Server 进程的内存使用百分比，由操作系统测量。 适用于弹性池。|None|
|tempdb_data_size|Tempdb 数据文件大小 (KB)|Count|最大值|Tempdb 数据文件大小 (KB)|None|
|tempdb_log_size|Tempdb 日志文件大小 (KB)|Count|最大值|Tempdb 日志文件大小 (KB)|None|
|tempdb_log_used_percent|Tempdb 日志已用百分比|百分比|最大值|Tempdb 日志已用百分比|None|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|SyncGroupSyncSessionAppliedFilesCount|已同步的文件数|Count|总计|已同步的文件计数|SyncGroupName,ServerEndpointName,SyncDirection|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|ServerEndpointSyncSessionAppliedFilesCount|已同步的文件数|Count|总计|已同步的文件计数|ServerEndpointName,SyncDirection|
...
|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|ServerHeartbeat|服务器联机状态|Count|最大值|每当已注册的服务器在云终结点中成功记录检测信号时，记录值 1 的指标|ServerResourceId,ServerName|
...
|SQLOnDemand 查询处理字节|已处理的数据|字节|总计|查询处理的数据量|None|
...
|内存容量GB|内存容量 （GB）|Count|最大值|内存容量 （GB）|None|

## <a name="microsoftsynapseworkspacessqlpools"></a>微软.Synapse/工作区/sqlPools

|指标|指标显示名称|单位|聚合类型|描述|维度|
|---|---|---|---|---|---|
|DWU 限制|DWU 限制|Count|最大值|SQL 池的服务级别目标|None|
|DWUused|已用的 DWU|Count|最大值|表示整个 SQL 池使用的高级表示形式。 按 DWU 限制 = DWU 百分比衡量|None|
|DWUused百分比|DWU 使用百分比|百分比|最大值|表示整个 SQL 池使用的高级表示形式。 通过在 CPU 百分比和数据 IO 百分比之间获取最大值来测量|None|
|连接被防火墙阻止|被防火墙阻止的连接|Count|总计|防火墙规则阻止的连接计数。 重新访问 SQL 池的访问控制策略，并在计数较高时监视这些连接|None|
|自适应缓存百分比|自适应缓存命中百分比|百分比|最大值|测量工作负载使用自适应缓存的用量程度。 使用此指标与缓存命中百分比指标一起确定是为其他容量进行缩放还是重新运行工作负载以滋润缓存|None|
|自适应缓存使用百分比|自适应缓存使用百分比|百分比|最大值|测量工作负载使用自适应缓存的用量程度。 使用此指标与缓存使用的百分比指标一起使用，以确定是为其他容量进行缩放，还是重新运行工作负载以滋润缓存|None|
|本地温度百分比|本地天分贝使用百分比|百分比|最大值|跨所有计算节点的本地 tempdb 利用率 - 每五分钟发出一次值|None|
|内存使用百分比|内存使用百分比|百分比|最大值|SQL 池中所有节点的内存利用率|None|
...
|TcpSynSent|发送的 TCP Syn|Count|平均值|发送的 TCP Syn|实例|
|TcpSynReceived|收到的 TCP Syn|Count|平均值|收到的 TCP Syn|实例|
|TcpEstablished|已建立的 TCP|Count|平均值|已建立的 TCP|实例|
|TcpFinWait1|TCP Fin 等待 1|Count|平均值|TCP Fin 等待 1|实例|
|TcpFinWait2|TCP Fin 等待 2|Count|平均值|TCP Fin 等待 2|实例|
|TcpClosing|TCP 正在关闭|Count|平均值|TCP 正在关闭|实例|
|TcpCloseWait|TCP 关闭等待|Count|平均值|TCP 关闭等待|实例|
|TcpLastAck|TCP 上一次的 Ack|Count|平均值|TCP 上一次的 Ack|实例|
