...
|配额|内存: 配额|字节|Average|当前内存配额（字节）。 内存配额也称为内存授予或内存保留。|ServerResourceType|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|CPU 百分比|CPU 百分比|百分比|Average|虚拟机当前正在使用的已分配计算单元的百分比。|无维度|
|输入网络|网络传入|字节|总计|虚拟机在所有网络接口上收到的字节数(传入流量)。|无维度|
|输出网络|网络传出|字节|总计|通过虚拟机传出的所有网络接口上的字节数(传出流量)。|无维度|
|磁盘读取的字节数/秒|磁盘读取|每秒字节数|Average|监视期间从磁盘读取的平均字节数。|无维度|
|磁盘写入的字节数/秒|磁盘写入|每秒字节数|Average|监视期间写入磁盘的平均字节数。|无维度|
|磁盘读取操作数/秒|磁盘读取操作次数/秒|每秒计数|Average|磁盘读取的 IOPS。|无维度|
|磁盘写入操作数/秒|磁盘写入操作次数/秒|每秒计数|Average|磁盘写入的 IOPS。|无维度|

## <a name="microsoftclassiccomputedomainnamesslotsroles"></a>Microsoft.ClassicCompute/domainNames/slots/roles

|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|CPU 百分比|CPU 百分比|百分比|Average|虚拟机当前正在使用的已分配计算单元的百分比。|RoleInstanceId|
|输入网络|网络传入|字节|总计|虚拟机在所有网络接口上收到的字节数(传入流量)。|RoleInstanceId|
|输出网络|网络传出|字节|总计|通过虚拟机传出的所有网络接口上的字节数(传出流量)。|RoleInstanceId|
|磁盘读取的字节数/秒|磁盘读取|每秒字节数|Average|监视期间从磁盘读取的平均字节数。|RoleInstanceId|
|磁盘写入的字节数/秒|磁盘写入|每秒字节数|Average|监视期间写入磁盘的平均字节数。|RoleInstanceId|
|磁盘读取操作数/秒|磁盘读取操作次数/秒|每秒计数|Average|磁盘读取的 IOPS。|RoleInstanceId|
|磁盘写入操作数/秒|磁盘写入操作次数/秒|每秒计数|Average|磁盘写入的 IOPS。|RoleInstanceId|

## <a name="microsoftcognitiveservicesaccounts"></a>Microsoft.CognitiveServices/accounts

|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|TotalCalls|总调用数|Count|总计|调用的总数。|ApiName、OperationName、Region|
|SuccessfulCalls|成功调用数|Count|总计|成功的调用数。|ApiName、OperationName、Region|
|TotalErrors|错误总数|Count|总计|带有错误响应的调用总数(HTTP 响应代码为 4xx 或 5xx)。|ApiName、OperationName、Region|
|BlockedCalls|阻止的调用数|Count|总计|调用数超出速率或配额限制。|ApiName、OperationName、Region|
|ServerErrors|服务器错误数|Count|总计|出现服务内部错误的调用数(HTTP 响应代码为 5xx)。|ApiName、OperationName、Region|
|ClientErrors|客户端错误数|Count|总计|出现客户端错误的调用数(HTTP 响应代码为 4xx)|ApiName、OperationName、Region|
|DataIn|数据输入|字节|总计|传入数据的大小(以字节为单位)。|ApiName、OperationName、Region|
|DataOut|数据输出|字节|总计|传出数据的大小(以字节为单位)。|ApiName、OperationName、Region|
|延迟|延迟|毫秒|Average|延迟(以毫秒为单位)。|ApiName、OperationName、Region|
...
|TotalTokenCalls|标记调用总数|Count|总计|标记调用的总数。|ApiName、OperationName、Region|
...
|数据磁盘读取字节数/秒|数据磁盘读取字节数/秒(预览)|每秒计数|Average|监视期间每秒从单个磁盘读取的字节数|LUN|
|数据磁盘写入字节数/秒|数据磁盘写入字节数/秒(预览)|每秒计数|Average|监视期间每秒写入到单个磁盘的字节数|LUN|
|数据磁盘读取操作数/秒|数据磁盘读取操作数/秒(预览)|每秒计数|Average|监视期间从单个磁盘执行的读取 IOPS|LUN|
|数据磁盘写入操作数/秒|数据磁盘写入操作数/秒(预览)|每秒计数|Average|监视期间从单个磁盘执行的写入 IOPS|LUN|
|数据磁盘队列深度|数据磁盘队列深度（预览版）|Count|Average|数据磁盘队列深度(或队列长度)|LUN|
|OS 磁盘读取字节数/秒|OS 磁盘读取字节数/秒(预览)|每秒计数|Average|OS 磁盘监视期间每秒从单个磁盘读取的字节数|无维度|
|OS 磁盘写入字节数/秒|OS 磁盘写入字节数/秒(预览)|每秒计数|Average|OS 磁盘监视期间每秒写入到单个磁盘的字节数|无维度|
|OS 磁盘读取操作次数/秒|OS 磁盘读取操作数/秒(预览)|每秒计数|Average|OS 磁盘监视期间从单个磁盘执行的读取 IOPS|无维度|
|OS 磁盘写入操作次数/秒|OS 磁盘写入操作数/秒(预览)|每秒计数|Average|OS 磁盘监视期间从单个磁盘执行的写入 IOPS|无维度|
...
|网络传出流量总计|网络传出流量总计|字节|总计|通过虚拟机传出的所有网络接口上的字节数(传出流量)|无维度|
...
|磁盘读取操作数/秒|磁盘读取操作次数/秒|每秒计数|Average|磁盘读取的 IOPS|VMName|
|磁盘写入操作数/秒|磁盘写入操作次数/秒|每秒计数|Average|磁盘写入的 IOPS|VMName|
|剩余 CPU 信用额度|剩余 CPU 信用额度|Count|Average|可用来集中使用的总信用点数|无维度|
|已用 CPU 信用额度|已用 CPU 信用额度|Count|Average|虚拟机使用的总信用点数|无维度|
|每磁盘读取字节数/秒|数据磁盘读取字节数/秒（已弃用）|每秒计数|Average|监视期间每秒从单个磁盘读取的字节数|槽位 ID|
|每磁盘写入字节数/秒|数据磁盘写入字节数/秒（已弃用）|每秒计数|Average|监视期间每秒写入到单个磁盘的字节数|槽位 ID|
...
|OS 每磁盘写入操作数/秒|OS 磁盘写入操作数/秒（已弃用）|每秒计数|Average|OS 磁盘监视期间从单个磁盘执行的写入 IOPS|无维度|
|OS 每磁盘 QD|OS 磁盘 QD（已弃用）|Count|Average|OS 磁盘队列深度(或队列长度)|无维度|
|数据磁盘读取字节数/秒|数据磁盘读取字节数/秒(预览)|每秒计数|Average|监视期间每秒从单个磁盘读取的字节数|LUN、VMName|
|数据磁盘写入字节数/秒|数据磁盘写入字节数/秒(预览)|每秒计数|Average|监视期间每秒写入到单个磁盘的字节数|LUN、VMName|
|数据磁盘读取操作数/秒|数据磁盘读取操作数/秒(预览)|每秒计数|Average|监视期间从单个磁盘执行的读取 IOPS|LUN、VMName|
|数据磁盘写入操作数/秒|数据磁盘写入操作数/秒(预览)|每秒计数|Average|监视期间从单个磁盘执行的写入 IOPS|LUN、VMName|
|数据磁盘队列深度|数据磁盘队列深度（预览版）|Count|Average|数据磁盘队列深度(或队列长度)|LUN、VMName|
|OS 磁盘读取字节数/秒|OS 磁盘读取字节数/秒(预览)|每秒计数|Average|OS 磁盘监视期间每秒从单个磁盘读取的字节数|VMName|
|OS 磁盘写入字节数/秒|OS 磁盘写入字节数/秒(预览)|每秒计数|Average|OS 磁盘监视期间每秒写入到单个磁盘的字节数|VMName|
|OS 磁盘读取操作次数/秒|OS 磁盘读取操作数/秒(预览)|每秒计数|Average|OS 磁盘监视期间从单个磁盘执行的读取 IOPS|VMName|
|OS 磁盘写入操作次数/秒|OS 磁盘写入操作数/秒(预览)|每秒计数|Average|OS 磁盘监视期间从单个磁盘执行的写入 IOPS|VMName|
...
|网络传出流量总计|网络传出流量总计|字节|总计|通过虚拟机传出的所有网络接口上的字节数(传出流量)|VMName|

## <a name="microsoftcontainerinstancecontainergroups"></a>Microsoft.ContainerInstance/containerGroups

|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|CpuUsage|CPU 使用率|Count|Average|所有核心上的 CPU 使用量(以millicore为单位)。|containerName|
...
|Size|Size|字节|Average|事件中心的大小(字节)。|EntityName|
...
|ActiveConnections|ActiveConnections（预览版）|Count|Average|Microsoft.EventHub 的活动连接总数。 (预览)|无维度|
|ConnectionsOpened|打开的连接数。 (预览)|Count|Average|Microsoft.EventHub 打开的连接数。 (预览)|无维度|
|ConnectionsClosed|已关闭的连接数。 (预览)|Count|Average|Microsoft.EventHub 已关闭的连接数。 (预览)|无维度|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|ObservedMetricValue|观察到的指标值|Count|Average|自动缩放执行时由它计算的值|MetricTriggerSource|
|MetricThreshold|指标阈值|Count|Average|自动缩放运行时的已配置自动缩放阈值。|MetricTriggerRule|
|ObservedCapacity|观察到的容量|Count|Average|自动缩放执行时报告给它的容量。|无维度|
...
|browserTimings/receiveDuration|正在接收响应时间|毫秒|Average|第一个字节和最后一个字节之间的时间，或直至断开连接的时间。|无维度|
|browserTimings/sendDuration|发送请求的时间|毫秒|Average|连接到网络和接收到第一个字节之间的时间。|无维度|
|browserTimings/totalDuration|浏览器网页加载时间|毫秒|Average|从用户请求直到 DOM、样式表、脚本和图像加载完毕的时间。|无维度|
...
|dependencies/duration|依赖项持续时间|毫秒|Average|应用程序对外部资源所进行的调用的持续时间。|dependency/type, dependency/performanceBucket, dependency/success, operation/synthetic, cloud/roleInstance, cloud/roleName|
...
|performanceCounters/requestExecutionTime|HTTP 请求执行时间|毫秒|Average|最新请求的执行时间。|cloud/roleInstance|
|performanceCounters/requestsInQueue|应用程序队列中的 HTTP 请求|Count|Average|应用程序请求队列的长度。|cloud/roleInstance|
|performanceCounters/requestsPerSecond|HTTP 请求速率|每秒计数|Average|来自 ASP.NET 的针对应用程序的所有请求的每秒速率。|cloud/roleInstance|
...
|performanceCounters/processorCpuPercentage|处理器时间|Percent|Average|处理器执行非空闲线程所用时间的百分比。|cloud/roleInstance|
|performanceCounters/memoryAvailableBytes|可用内存|字节|Average|可立刻分配给进程或系统使用的物理内存。|cloud/roleInstance|
|performanceCounters/processPrivateBytes|进程专用字节数|字节|Average|专门分配给受监视应用程序的进程的内存。|cloud/roleInstance|
|requests/duration|服务器响应时间|毫秒|Average|接收 HTTP 请求和完成发送响应之间的时间。|request/performanceBucket, request/resultCode, operation/synthetic, cloud/roleInstance, request/success, cloud/roleName|
...
|exceptions/count|Exceptions|Count|Count|所有未捕获异常的组合计数。|cloud/roleName, cloud/roleInstance, client/type|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|CacheUtilization|缓存使用率|Percent|Average|群集范围内的使用率级别|None|
...
|EventProcessedForEventHubs|处理的事件数（针对事件中心）|Count|总计|从事件中心引入时，由群集处理的事件数|无|
|IngestionResult|引入结果|Count|Count|引入操作的数量|状态|
|CPU|CPU|Percent|Average|CPU 使用率级别|无|
| ContinuousExportNumOfRecordsExported | 连续导出中导出的记录数 | Count | 总计 | 导出操作期间写入的每个存储项目的已导出记录数  | None |
| ExportUtilization | 导出利用率 | Percent | 最大值 | 导出利用率 | 无 |
| ContinuousExportPendingCount | 连续导出挂起计数 | Count | 最大值 | 准备执行的挂起连续导出作业的数目 | None |
...
|RunLatency|运行延迟|秒|Average|已完成的工作流运行的延迟情况。|无维度|
|RunSuccessLatency|运行成功延迟|秒|Average|成功的工作流运行的延迟情况。|无维度|
...
|ActionLatency|操作延迟 |秒|Average|已完成的工作流操作的延迟情况。|无维度|
|ActionSuccessLatency|操作成功延迟 |秒|Average|成功的工作流操作的延迟情况。|无维度|
...
|TriggerLatency|触发器延迟 |秒|Average|已完成的工作流触发器的延迟情况。|无维度|
|TriggerFireLatency|触发器激发延迟 |秒|Average|已激发的工作流触发器的延迟情况。|无维度|
|TriggerSuccessLatency|触发器成功延迟 |秒|Average|成功的工作流触发器的延迟情况。|无维度|
...
|Average_Disk Read Bytes/sec|磁盘读取的字节数/秒|Count|Average|Average_Disk Read Bytes/sec|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|Average_Disk Write Bytes/sec|磁盘写入的字节数/秒|Count|Average|Average_Disk Write Bytes/sec|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|Average_Bytes Received/sec|已接收字节/秒|Count|Average|Average_Bytes Received/sec|Computer、ObjectName、InstanceName、CounterPath、SourceSystem|
...
|Size|大小（预览版）|字节|Average|队列/主题的大小(字节)。 (预览)|EntityName|
|消息|队列/主题中的消息计数。 (预览)|Count|Average|队列/主题中的消息计数。 (预览)|EntityName|
|活动消息|队列/主题中的活动消息计数。 (预览)|Count|Average|队列/主题中的活动消息计数。 (预览)|EntityName|
|DeadletteredMessages|队列/主题中的死信消息计数。 (预览)|Count|Average|队列/主题中的死信消息计数。 (预览)|EntityName|
|ScheduledMessages|队列/主题中的计划消息计数。 (预览)|Count|Average|队列/主题中的计划消息计数。 (预览)|EntityName|
...
|dtu_consumption_percent|DTU 百分比|Percent|Average|DTU 百分比|无维度|
...
|dtu_limit|DTU 限制|Count|Average|DTU 限制|无维度|
|dtu_used|已用的 DTU|Count|Average|已用的 DTU|无维度|
|cpu_limit|CPU 限制|Count|Average|CPU 限制|无维度|
|cpu_used|已用 CPU|Count|Average|已用 CPU|无维度|
...
|local_tempdb_usage_percent|本地 tempdb 百分比|Percent|Average|本地 tempdb 百分比|无维度|
|app_cpu_billed|计费的应用 CPU|Count|总计|计费的应用 CPU|无维度|
|app_cpu_percent|应用 CPU 百分比|Percent|Average|应用 CPU 百分比|无维度|
|app_memory_percent|已用应用内存百分比|Percent|Average|已用应用内存百分比|无维度|
...
|workers_percent|工作线程百分比|Percent|Average|工作线程百分比|无维度|
|sessions_percent|会话百分比|Percent|Average|会话百分比|无维度|
|eDTU_limit|eDTU 限制|Count|Average|eDTU 限制|无维度|
|storage_limit|数据的最大大小|字节|Average|存储限制|无维度|
|eDTU_used|已用的 eDTU|Count|Average|已用的 eDTU|无维度|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|DiskReadBytesPerSecond|磁盘读取的字节数/秒|每秒字节数|Average|由于读取操作超过采样期限而限制的平均磁盘吞吐量。|无维度|
|DiskWriteBytesPerSecond|磁盘写入的字节数/秒|每秒字节数|Average|由于写入操作超过采样期限而限制的平均磁盘吞吐量。|无维度|
...
|磁盘读取操作数/秒|磁盘读取操作次数/秒|每秒计数|Average|上一个采样周期的平均 IO 读取操作次数。 请注意，这些操作的大小可能是可变的。|无维度|
|磁盘写入操作数/秒|磁盘写入操作次数/秒|每秒计数|Average|上一个采样周期的平均 IO 写入操作次数。 请注意，这些操作的大小可能是可变的。|无维度|
