...
ms.locfileid: "87132010"
---
# <a name="supported-metrics-with-azure-monitor"></a>Azure 监视器支持的指标

> [!NOTE]
> 此列表主要通过 Azure Monitor Metrics REST API 自动生成。 通过 GitHub 对此列表所做的任何修改都可以进行编写，而不会发出警告。 请与本文的作者联系，以获取有关如何进行永久更新的详细信息。
...
2. 使用[诊断设置](diagnostic-settings.md)将平台指标路由到 
    - Azure 存储
    - Azure Monitor 日志（因此 Log Analytics）
...
> [!WARNING]
> 此处**未**列出在 Azure 虚拟机、Service Fabric 和云服务中运行的来宾操作系统（来宾 OS）的指标。 必须通过在或来宾操作系统中运行的一个或多个代理收集来宾 OS 指标。  来宾 OS 指标包括跟踪来宾 CPU 百分比或内存使用率的性能计数器，两者都经常用于自动缩放或警报。 
>
...
|Requests|是|Requests|计数|总计|包含多个维度的网关请求指标|Location、Hostname、LastErrorReason、BackendResponseCode、GatewayResponseCode、BackendResponseCodeCategory、GatewayResponseCodeCategory|
...
|jvm。 total. count|是|jvm。 total. count|计数|总计|GC 暂停计数|部署、AppName、Pod|
...
|tomcat. 错误|是|tomcat. 错误|计数|总计|Tomcat 全局错误|部署、AppName、Pod|
...
|tomcat. total. count|是|tomcat. total. count|计数|总计|Tomcat 请求总数|部署、AppName、Pod|
|tomcat. 总计. 时间|是|tomcat. 总计. 时间|毫秒|总计|Tomcat 请求总时间|部署、AppName、Pod|
|tomcat. 已发送|是|tomcat. 已发送|字节|总计|Tomcat 发送的总字节数|部署、AppName、Pod|
|tomcat 会话。当前|是|tomcat 会话。当前|计数|总计|Tomcat 会话活动计数|部署、AppName、Pod|
|tomcat。最大值|是|tomcat。最大值|计数|总计|Tomcat 会话最大活动计数|部署、AppName、Pod|
|tomcat。最大值|是|tomcat。最大值|毫秒|最大值|Tomcat 会话最大活动时间|部署、AppName、Pod|
|tomcat。已创建|是|tomcat。已创建|计数|总计|Tomcat 创建的会话数|部署、AppName、Pod|
|tomcat 会话。已过期|是|tomcat 会话。已过期|计数|总计|Tomcat 过期的会话数|部署、AppName、Pod|
|tomcat 会话。已拒绝|是|tomcat 会话。已拒绝|计数|总计|Tomcat 拒绝的会话数|部署、AppName、Pod|
|tomcat.threads.config 最大值|是|tomcat.threads.config 最大值|计数|总计|Tomcat 配置最大线程计数|部署、AppName、Pod|
|tomcat。当前|是|tomcat。当前|计数|总计|Tomcat 当前线程计数|部署、AppName、Pod|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|活动核心数|是|活动核心数|计数|平均值|活动核心数|方案，ClusterName|
|活动节点数|是|活动节点数|计数|平均值|正在运行的节点数目|方案，ClusterName|
|空闲核心数|是|空闲核心数|计数|平均值|空闲核心数|方案，ClusterName|
|空闲节点数|是|空闲节点数|计数|平均值|空闲节点数目|方案，ClusterName|
|已完成的作业|是|已完成的作业|计数|总计|已完成的作业数|方案，ClusterName，ResultType|
|提交的作业|是|提交的作业|计数|总计|提交的作业数|方案，ClusterName|
|正在退出的核心数|是|正在退出的核心数|计数|平均值|正在退出的核心数|方案，ClusterName|
|正在退出的节点数|是|正在退出的节点数|计数|平均值|正在退出的节点数|方案，ClusterName|
|占用的核心数|是|占用的核心数|计数|平均值|占用的核心数|方案，ClusterName|
|占用的节点数|是|占用的节点数|计数|平均值|已占用节点数|方案，ClusterName|
|配额利用率百分比|是|配额利用率百分比|计数|平均值|已利用的配额百分比|方案、ClusterName、VmFamilyName、VmPriority|
|核心总数|是|核心总数|计数|平均值|核心总数|方案，ClusterName|
|节点总数|是|节点总数|计数|平均值|节点总数|方案，ClusterName|
|不可用的核心数|是|不可用的核心数|计数|平均值|不可用的核心数|方案，ClusterName|
|不可用的节点数|是|不可用的节点数|计数|平均值|不可用的节点数目|方案，ClusterName|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|BroadcastProcessedCount|是|广播处理计数|计数|平均值|处理的事务数|节点、通道、类型、状态|
|ConnectionAccepted|是|已接受的连接数|计数|总计|已接受的连接数|节点|
|ConnectionActive|是|活动连接数|计数|平均值|活动连接数|节点|
|ConnectionHandled|是|已处理连接数|Count|总计|已处理的连接数|节点|
...
|EndorserEndorsementFailures|是|Endorser 认可失败|计数|平均值|签署失败的次数。|Node、信道、chaincode、chaincodeerror|
|GossipLeaderElectionLeader|是|Gossip 领袖选举负责人|计数|总计|对等机为领导（1）或从动者（0）|Node、信道|
|GossipMembershipTotalPeersKnown|是|Gossip 成员资格总数已知|计数|平均值|已知对等方总数|Node、信道|
|GossipStateHeight|是|Gossip 状态高度|计数|平均值|当前分类帐高度|Node、信道|
|IOReadBytes|是|IO 读取字节数|字节|总计|IO 读取字节数|节点|
|IOWriteBytes|是|IO 写入字节数|字节|总计|IO 写入字节数|节点|
|LedgerTransactionCount|是|帐目交易计数|计数|平均值|处理的事务数|Node、信道、transaction_type、chaincode、validation_code|
...
|ProcessedBlocks|是|已处理的块|计数|总计|已处理的块数|节点|
|ProcessedTransactions|是|已处理事物数|Count|总计|已处理的事务数|节点|
|QueuedTransactions|是|已排队事务数|计数|平均值|已排队的事务数|节点|
...
|setcommands|是|集 (Sets)|计数|总计||ShardId|
...
|事务|是|事务|计数|总计|向存储服务或指定的 API 操作发出的请求数。 此数值包括成功和失败的请求数，以及引发错误的请求数。 针对不同类型的响应数使用 ResponseType 维度。|ResponseType、GeoType、ApiName、Authentication、文件共享|
...
|磁盘写入操作次数/秒|是|磁盘写入操作次数/秒|每秒计数|平均值|磁盘写入 IOPS|无维度|
|入站流|是|入站流量|计数|平均值|入站流数是入站方向的当前流（传入 VM 的流量）的数目|无维度|
...
|出站流数|是|出站流|计数|平均值|出站流数是出站方向的当前流（传出 VM 的流量）的数目|无维度|
...
|OS 每磁盘写入字节数/秒|是|OS 磁盘写入字节数/秒(已弃用)|每秒计数|平均值|OS 磁盘监视期间每秒写入到单个磁盘的字节数|无维度|
|OS 每磁盘写入操作数/秒|是|OS 磁盘写入操作数/秒(已弃用)|每秒计数|平均值|OS 磁盘监视期间从单个磁盘执行的写入 IOPS|无维度|
|出站流数|是|出站流|计数|平均值|出站流数是出站方向的当前流（传出 VM 的流量）的数目|VMName|
...
|RoutingDataSizeInBytesDelivered|是|路由传送消息大小（以字节为单位）（预览版）|字节|总计|IoT 中心发送到终结点的消息的总大小（以字节为单位）。 您可以使用 "终结点" 和 "EndpointType" 维度来查看传递到不同终结点的消息大小（以字节为单位）。 对于每个传递的消息，指标值都会增加，包括如果消息传递到多个终结点，或者消息传递到同一终结点多次。|EndpointType、终结点、RoutingSource|
...
|CassandraKeyspaceDelete|否|已删除 Cassandra 密钥空间|Count|Count|已删除 Cassandra 密钥空间|ResourceName |
|CassandraKeyspaceThroughputUpdate|否|已更新 Cassandra 密钥空间吞吐量|Count|计数|已更新 Cassandra 密钥空间吞吐量|ResourceName |
|CassandraKeyspaceUpdate|否|已更新 Cassandra 密钥空间|计数|Count|已更新 Cassandra 密钥空间|ResourceName |
...
|CassandraRequests|否|Cassandra 请求数|Count|Count|发出的 Cassandra 请求数|DatabaseName、CollectionName、Region、OperationType、ResourceType、ErrorCode|
|CassandraTableCreate|否|已创建 Cassandra 表|Count|Count|已创建 Cassandra 表|Context.resourcename、ChildResourceName、 |
|CassandraTableDelete|否|Cassandra 表已删除|Count|Count|Cassandra 表已删除|Context.resourcename、ChildResourceName、 |
|CassandraTableThroughputUpdate|否|Cassandra 表吞吐量已更新|Count|Count|Cassandra 表吞吐量已更新|Context.resourcename、ChildResourceName、 |
|CassandraTableUpdate|否|Cassandra 表已更新|计数|计数|Cassandra 表已更新|Context.resourcename、ChildResourceName、 |
|CreateAccount|是|创建的帐户数|Count|Count|创建的帐户数|无维度|
|DataUsage|否|数据使用情况|字节|总计|按 5 分钟粒度报告的总数据用量|CollectionName、DatabaseName、Region|
|DeleteAccount|是|删除的帐户数|Count|计数|删除的帐户数|无维度|
...
|GremlinDatabaseCreate|否|已创建 Gremlin 数据库|Count|计数|已创建 Gremlin 数据库|ResourceName |
|GremlinDatabaseDelete|否|已删除 Gremlin 数据库|Count|计数|已删除 Gremlin 数据库|ResourceName |
|GremlinDatabaseThroughputUpdate|否|已更新 Gremlin 数据库吞吐量|Count|计数|已更新 Gremlin 数据库吞吐量|ResourceName |
|GremlinDatabaseUpdate|否|已更新 Gremlin 数据库|Count|计数|已更新 Gremlin 数据库|ResourceName |
|GremlinGraphCreate|否|已创建 Gremlin 图|Count|计数|已创建 Gremlin 图|Context.resourcename、ChildResourceName、 |
|GremlinGraphDelete|否|已删除 Gremlin 图形|Count|计数|已删除 Gremlin 图形|Context.resourcename、ChildResourceName、 |
|GremlinGraphThroughputUpdate|否|已更新 Gremlin 图形吞吐量|Count|计数|已更新 Gremlin 图形吞吐量|Context.resourcename、ChildResourceName、 |
...
|MongoCollectionCreate|否|已创建 Mongo 收集|Count|计数|已创建 Mongo 收集|Context.resourcename、ChildResourceName、 |
|MongoCollectionDelete|否|已删除 Mongo 集合|Count|计数|已删除 Mongo 集合|Context.resourcename、ChildResourceName、 |
|MongoCollectionThroughputUpdate|否|已更新 Mongo 集合吞吐量|Count|计数|已更新 Mongo 集合吞吐量|Context.resourcename、ChildResourceName、 |
|MongoCollectionUpdate|否|Mongo 集合已更新|Count|计数|Mongo 集合已更新|Context.resourcename、ChildResourceName、 |
...
|MongoDBDatabaseCreate|否|已创建 Mongo 数据库|Count|计数|已创建 Mongo 数据库|ResourceName |
|MongoDBDatabaseUpdate|否|已更新 Mongo 数据库|Count|计数|已更新 Mongo 数据库|ResourceName |
...
|RegionFailover|是|故障转移的区域数|Count|计数|故障转移的区域数|无维度|
|RemoveRegion|是|删除的区域数|Count|计数|删除的区域数|区域|
...
|SqlContainerCreate|否|已创建 Sql 容器|Count|计数|已创建 Sql 容器|Context.resourcename、ChildResourceName、 |
|SqlContainerDelete|否|Sql 容器已删除|Count|计数|Sql 容器已删除|Context.resourcename、ChildResourceName、 |
|SqlContainerThroughputUpdate|否|已更新 Sql 容器吞吐量|Count|计数|已更新 Sql 容器吞吐量|Context.resourcename、ChildResourceName、 |
|SqlContainerUpdate|否|已更新 Sql 容器|Count|计数|已更新 Sql 容器|Context.resourcename、ChildResourceName、 |
|SqlDatabaseCreate|否|已创建 Sql 数据库|Count|计数|已创建 Sql 数据库|ResourceName |
|SqlDatabaseDelete|否|Sql 数据库已删除|Count|计数|Sql 数据库已删除|ResourceName |
...
|TableTableCreate|否|已创建 AzureTable 表|Count|计数|已创建 AzureTable 表|ResourceName |
|TableTableDelete|否|AzureTable 表已删除|Count|计数|AzureTable 表已删除|ResourceName |
|TableTableThroughputUpdate|否|AzureTable 表吞吐量已更新|Count|计数|AzureTable 表吞吐量已更新|ResourceName |
|TableTableUpdate|否|已更新 AzureTable 表|Count|计数|已更新 AzureTable 表|ResourceName |
...
|UpdateAccountKeys|是|更新的帐户密钥数|Count|计数|更新的帐户密钥数|KeyType|
|UpdateAccountNetworkSettings|是|更新的帐户网络设置数|Count|计数|更新的帐户网络设置数|无维度|
|UpdateAccountReplicationSettings|是|更新的帐户复制设置数|Count|计数|更新的帐户复制设置数|无维度|
...
|NumActiveWorkers|是|活动工作线程数|计数|最大值|活动辅助角色数目|MetricName|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|BatchBlobCount|是|批处理 Blob 计数|计数|平均值|聚合批次中用于引入的数据源数。|数据库|
|BatchDuration|是|批处理持续时间|秒|平均值|引入流中聚合阶段的持续时间。|数据库|
|BatchesProcessed|是|处理的批处理|计数|平均值|为引入聚合的批处理数。 批处理完成原因：批处理是否已达到批处理时间、数据大小或批处理策略设置的文件数限制|Database、SealReason|
...
|ContinuousExportResult|是|连续导出结果|Count|计数|指示连续导出是成功还是失败|ContinuousExportName，结果，数据库|
...
|---|---|---|---|---|---|---|
|活动核心数|是|活动核心数|计数|平均值|活动核心数|方案，ClusterName|
|活动节点数|是|活动节点数|计数|平均值|活动节点数。 这些节点是正在运行作业的节点。|方案，ClusterName|
...
|错误|是|错误|计数|总计|此工作区中的运行错误数。 每当运行遇到错误时，计数就会更新。|方案|
|失败的运行次数|是|失败的运行次数|计数|总计|此工作区的运行失败次数。 当运行失败时，将更新计数。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
...
|空闲核心数|是|空闲核心数|计数|平均值|空闲核心数|方案，ClusterName|
|空闲节点数|是|空闲节点数|计数|平均值|空闲节点数。 空闲节点是指未运行任何作业，但可以接受新作业（如果可用）的节点。|方案，ClusterName|
|正在退出的核心数|是|正在退出的核心数|计数|平均值|正在退出的核心数|方案，ClusterName|
|正在退出的节点数|是|正在退出的节点数|计数|平均值|正在退出的节点数。 正在退出的节点是指刚刚完成了作业处理，将进入空闲状态的节点。|方案，ClusterName|
|失败的模型部署数|是|失败的模型部署数|计数|总计|此工作区中失败的模型部署数|方案，StatusCode|
|已启动的模型部署数|是|已启动的模型部署数|计数|总计|此工作区中已启动的模型部署数|方案|
|成功的模型部署数|是|成功的模型部署数|计数|总计|此工作区中成功的模型部署数|方案|
|失败的模型注册数|是|失败的模型注册数|计数|总计|此工作区中失败的模型注册数|方案，StatusCode|
...
|占用的核心数|是|占用的核心数|计数|平均值|占用的核心数|方案，ClusterName|
|占用的节点数|是|占用的节点数|计数|平均值|占用的节点数。 这些节点是已从可用节点池中取出的低优先级节点。|方案，ClusterName|
...
|配额利用率百分比|是|配额利用率百分比|计数|平均值|已利用的配额百分比|方案、ClusterName、VmFamilyName、VmPriority|
|已启动的运行数|是|已启动的运行数|Count|总计|为此工作区运行的运行数。 当运行开始在所需资源上运行时，将更新计数。|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|启动运行数|是|启动运行数|计数|总计|为此工作区启动的运行数。 在请求创建运行并运行信息（如运行 Id）后，将更新计数|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|核心总数|是|核心总数|计数|平均值|核心总数|方案，ClusterName|
|节点总数|是|节点总数|计数|平均值|节点总数。 此总数包括一些活动节点、空闲节点、不可用的节点、占用的节点和正在退出的节点|方案，ClusterName|
|不可用的核心数|是|不可用的核心数|计数|平均值|不可用的核心数|方案，ClusterName|
|不可用的节点数|是|不可用的节点数|计数|平均值|不可用的节点数。 不可用的节点是由于存在某种不可解决的问题而无法正常运行的节点。 Azure 将回收这些节点。|方案，ClusterName|
|警告|是|警告|计数|总计|此工作区中的运行警告数。 每当运行遇到警告时，计数就会更新。|方案|
...
|用法|否|使用情况|计数|计数|API 调用计数|ApiCategory、ApiName、ResultType、ResponseCode|
...
|请求|是|Requests|计数|总计|对流式处理终结点的请求。|OutputFormat，HttpStatusCode，ErrorCode|
...
|CbsVolumeBackupActive|是|卷备份活动状态|计数|平均值|当前卷的备份是否处于挂起状态。|无维度|
|CbsVolumeLogicalBackupBytes|是|已备份的逻辑字节|字节|平均值|已为此卷备份 Toatl 压缩/未加密的字节数。|无维度|
|CbsVolumeOperationComplete|是|操作状态|计数|平均值|上次备份/还原操作成功。|无维度|
|CbsVolumeOperationTransferredBytes|是|为操作传输的字节数|字节|平均值|上次备份/还原操作传输的总字节数。|无维度|
|CbsVolumeProtected|是|卷保护状态|计数|平均值|受云备份服务保护的卷。|无维度|
...
|XregionReplicationHealthy|是|卷复制状态是否正常|计数|平均值|关系的条件为1或0。|无维度|
...
|RxLightLevel|是|RxLightLevel|计数|平均值|Rx 光能级（以 dBm 为单位）|链接，通道|
|TxLightLevel|是|TxLightLevel|计数|平均值|Tx 光能级（以 dBm 为单位）|链接，通道|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|AllocatedSnatPorts|否|分配的 SNAT 端口|计数|平均值|时间段内已分配的 SNAT 端口总数|FrontendIPAddress、BackendIPAddress、ProtocolType、 |
|ByteCount|是|字节计数|计数|总计|时间段内传输的字节总数|FrontendIPAddress, FrontendPort, Direction|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ByteCount|是|字节计数|计数|总计|时间段内传输的字节总数|Port、Direction|
...
|qpu_high_utilization_metric|是|QPU 高利用率|计数|总计|最后一分钟内 QPU 高利用率，1 为高 QPU 利用率，反之为 0|无维度|
|QueryDuration|是|查询持续时间（数据集）|毫秒|平均值|上一个间隔的 DAX 查询持续时间|无维度|
|QueryPoolJobQueueLength|是|查询池作业队列长度（数据集）|计数|平均值|查询线程池队列中的作业数。|无维度|
...
|BytesTransferred|是|BytesTransferred|计数|总计|Microsoft.Relay 的已传输字节总数。|EntityName|
|ListenerConnections-ClientError|否|ListenerConnections-ClientError|计数|总计|关于 Microsoft.Relay 的侦听程序连接的客户端错误。|EntityName、 |
|ListenerConnections-ServerError|否|ListenerConnections-ServerError|Count|总计|关于 Microsoft.Relay 的侦听程序连接的服务器错误。|EntityName、 |
|ListenerConnections-Success|否|ListenerConnections-Success|计数|总计|Microsoft.Relay 的侦听程序成功连接数。|EntityName、 |
...
|SenderConnections-ClientError|否|SenderConnections-ClientError|计数|总计|关于 Microsoft.Relay 的发送方连接的客户端错误。|EntityName、 |
|SenderConnections-ServerError|否|SenderConnections-ServerError|计数|总计|关于 Microsoft.Relay 的发送方连接的服务器错误。|EntityName、 |
|SenderConnections-Success|否|SenderConnections-Success|计数|总计|Microsoft.Relay 的发送方成功连接数。|EntityName、 |
...
|ServerErrors|否|服务器错误数。|计数|总计|Microsoft.ServiceBus 的服务器错误数。|EntityName、 |
|大小|否|大小|字节|平均值|队列/主题的大小（以字节为单位）。|EntityName|
|SuccessfulRequests|否|成功的请求数|计数|总计|命名空间的成功请求总数|EntityName、 |
|ThrottledRequests|否|限制的请求数。|计数|总计|Microsoft.ServiceBus 限制的请求数。|EntityName、 |
|UserErrors|否|用户错误数。|计数|总计|Microsoft.ServiceBus 的用户错误数。|EntityName、 |
...
|dw_backup_size_gb|是|数据存储大小(GB)|计数|总计|数据存储大小由数据和事务日志的大小组成。 此指标将计入帐单的 "存储" 部分。 仅适用于数据仓库。|无维度|
|dw_geosnapshot_size_gb|是|灾难恢复存储大小（GB）|计数|总计|灾难恢复存储大小在帐单中反映为 "灾难恢复存储"。 仅适用于数据仓库。|无维度|
|dw_snapshot_size_gb|是|快照存储大小（GB）|计数|总计|快照存储大小是快照捕获的增量更改的大小，用于创建用户定义的还原点和自动还原点。 此指标将计入帐单的 "存储" 部分。 仅适用于数据仓库。|无维度|
...
|tempdb_data_size|是|Tempdb 数据文件大小 (KB)|计数|最大值|Tempdb 数据文件大小 (KB)。 不适用于数据仓库。|无维度|
...
|cpu_percent|是|CPU 百分比|百分比|平均值|CPU 百分比|无维度|
|cpu_used|是|使用的 CPU|计数|平均值|已用 CPU。 适用于基于 vCore 的弹性池。|无维度|
...
|database_cpu_used|否|使用的 CPU|计数|平均值|已用 CPU|DatabaseResourceId|
...
|tempdb_data_size|是|Tempdb 数据文件大小 (KB)|计数|最大值|Tempdb 数据文件大小 (KB)|无维度|
|tempdb_log_size|是|Tempdb 日志文件大小 (KB)|计数|最大值|Tempdb 日志文件大小 (KB)|无维度|
...
|FileShareProvisionedIOPS|否|文件共享预配 IOPS|字节|平均值|高级文件存储帐户中高级文件共享的预配 IOPS 的基线数。 此数字基于共享容量的预配大小（配额）来计算。|FileShare|
|FileShareSnapshotCount|否|文件共享快照计数|计数|平均值|存储帐户文件服务的共享中存在的快照数。|FileShare|
|FileShareSnapshotSize|否|文件共享快照大小|字节|平均值|存储帐户的文件服务中的快照所用存储量（以字节为单位）。|FileShare|
|流入量|是|流入量|字节|总计|流入的数据量（以字节为单位）。 此数字包括从外部客户端到 Azure 存储流入的数据量，以及流入 Azure 中的数据量。|GeoType、ApiName、Authentication、文件共享|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|ActiveRequests|是|活动请求数|计数|总计|活动请求数|实例|
...
|DiskQueueLength|是|磁盘队列长度|计数|平均值|磁盘队列长度|实例|
|Http101|是|Http 101|计数|总计|Http 101|实例|
|Http2xx|是|Http 2xx|计数|总计|Http 2xx|实例|
|Http3xx|是|Http 3xx|计数|总计|Http 3xx|实例|
|Http401|是|Http 401|计数|总计|Http 401|实例|
|Http403|是|Http 403|计数|总计|Http 403|实例|
|Http404|是|Http 404|计数|总计|Http 404|实例|
|Http406|是|Http 406|计数|总计|Http 406|实例|
|Http4xx|是|Http 4xx|计数|总计|Http 4xx|实例|
|Http5xx|是|Http 服务器错误|计数|总计|Http 服务器错误|实例|
|HttpQueueLength|是|Http 队列长度|计数|平均值|Http 队列长度|实例|
...
|TcpClosing|是|正在关闭的 TCP|计数|平均值|TCP 正在关闭|实例|
...
|指标|是否可通过诊断设置导出？|指标显示名称|计价单位|聚合类型|说明|维度|
|---|---|---|---|---|---|---|
|AppConnections|是|连接|计数|平均值|连接|实例|
|AverageMemoryWorkingSet|是|平均内存工作集|字节|平均值|平均内存工作集|实例|
|AverageResponseTime|是|平均响应时间 **（不推荐使用）**|秒|平均值|平均响应时间|实例|
|BytesReceived|是|数据输入|字节|总计|数据输入|实例|
|BytesSent|是|数据输出|字节|总计|数据输出|实例|
|CpuTime|是|CPU 时间|秒|总计|CPU 时间|实例|
|CurrentAssemblies|是|当前程序集|计数|平均值|当前程序集|实例|
|FileSystemUsage|是|文件系统使用情况|字节|平均值|文件系统使用情况|无维度|
|Gen0Collections|是|第 0 代垃圾回收|计数|总计|第 0 代垃圾回收|实例|
|Gen1Collections|是|第 1 代垃圾回收|计数|总计|第 1 代垃圾回收|实例|
|Gen2Collections|是|第 2 代垃圾回收|计数|总计|第 2 代垃圾回收|实例|
|句柄数|是|句柄计数|计数|平均值|句柄计数|实例|
...
|PrivateBytes|是|专用字节|字节|平均值|专用字节数|实例|
|Requests|是|Requests|计数|总计|请求|实例|
|RequestsInApplicationQueue|是|应用程序队列中的请求数|计数|平均值|应用程序队列中的请求数|实例|
|线程数|是|线程计数|计数|平均值|线程计数|实例|
|TotalAppDomains|是|应用程序域总数|计数|平均值|应用程序域总数|实例|
|TotalAppDomainsUnloaded|是|卸载的应用程序域总数|计数|平均值|卸载的应用程序域总数|实例|
...
|FunctionExecutionUnits|是|函数执行单位数|计数|总计|[函数执行单位数](https://github.com/Azure/Azure-Functions/wiki/Consumption-Plan-Cost-Billing-FAQ#how-can-i-view-graphs-of-execution-count-and-gb-seconds)|实例|
|Gen0Collections|是|第 0 代垃圾回收|计数|总计|第 0 代垃圾回收|实例|
|Gen1Collections|是|第 1 代垃圾回收|计数|总计|第 1 代垃圾回收|实例|
|Gen2Collections|是|第 2 代垃圾回收|计数|总计|第 2 代垃圾回收|实例|
