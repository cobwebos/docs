...
ms.locfileid: "79275381"
---
# <a name="supported-metrics-with-azure-monitor"></a>Azure Monitor 支持的指标

Azure Monitor 提供多种方式来与指标交互，包括在门户中制作指标图表、通过 REST API 访问指标，或者使用 PowerShell 或 CLI 查询指标。 下面是目前可在 Azure Monitor 的指标管道中使用的完整指标列表。 其他指标可在门户或旧版 API 中使用。 下面的此列表仅包含可以通过合并的 Azure Monitor 指标管道使用的指标。 度量值按命名空间组织。 有关服务及其所属命名空间的列表，请参阅[Azure 服务的资源提供程序](../../azure-resource-manager/management/azure-services-resource-providers.md)。 若要以编程方式查询和访问这些指标，请使用[2018-01-01 api 版本](https://docs.microsoft.com/rest/api/monitor/metricdefinitions)
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|TotalRequests|网关请求总数（不推荐使用）|Count|总计|网关请求数-请改用包含 GatewayResponseCodeCategory 维度的多维度请求度量值|位置，主机名|
|SuccessfulRequests|成功的网关请求数（已弃用）|Count|总计|成功的网关请求数-将多维度请求指标与 GatewayResponseCodeCategory 维度一起使用|位置，主机名|
|UnauthorizedRequests|未经授权的网关请求（不推荐使用）|Count|总计|未经授权的网关请求数-将多维度请求指标与 GatewayResponseCodeCategory 维度一起使用|位置，主机名|
|FailedRequests|失败的网关请求数（已弃用）|Count|总计|网关请求失败数-请改用具有 GatewayResponseCodeCategory 维度的多维度请求度量值|位置，主机名|
|OtherRequests|其他网关请求（不推荐使用）|Count|总计|其他网关请求数-将多维度请求指标与 GatewayResponseCodeCategory 维度一起使用|位置，主机名|
...
|EventHubTotalEvents|EventHub 事件总数|Count|总计|发送到 EventHub 的事件数|位置|
|EventHubSuccessfulEvents|成功的 EventHub 事件|Count|总计|成功的 EventHub 事件数|位置|
|EventHubTotalFailedEvents|故障 EventHub 事件|Count|总计|故障 EventHub 事件数|位置|
|EventHubRejectedEvents|拒绝 EventHub 事件|Count|总计|拒绝的 EventHub 个事件数（错误的配置或未授权）|位置|
|EventHubThrottledEvents|限制 EventHub 事件|Count|总计|已限制的 EventHub 事件数|位置|
|EventHubTimedoutEvents|已超时 EventHub 事件|Count|总计|超时 EventHub 事件数|位置|
|EventHubDroppedEvents|已丢弃 EventHub 事件|Count|总计|由于达到队列大小限制而跳过的事件数|位置|
|EventHubTotalBytesSent|EventHub 事件的大小|字节|总计|EventHub 事件的总大小（以字节为单位）|位置|
|Requests|Requests|Count|总计|具有多个维度的网关请求度量值|Location、Hostname、LastErrorReason、BackendResponseCode、GatewayResponseCode、BackendResponseCodeCategory、GatewayResponseCodeCategory|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|HttpIncomingRequestCount|HttpIncomingRequestCount|Count|Count|传入 http 请求总数。|无|
|FailedHttpRequestCount|FailedHttpRequestCount|Count|Count|Http 请求失败。|无|
|HttpIncomingRequestDuration|HttpIncomingRequestDuration|Count|平均值|Http 请求的延迟。|无|
...
|TomcatSessionCreatedCount|Tomcat 会话创建计数|Count|总计|Tomcat 会话创建计数|AppName、Pod|
|TomcatSessionExpiredCount|Tomcat 会话过期计数|Count|总计|Tomcat 会话过期计数|AppName、Pod|
|TomcatSessionRejectedCount|Tomcat 会话已拒绝计数|Count|总计|Tomcat 会话已拒绝计数|AppName、Pod|
...
|TotalUpdateDeploymentRuns|更新部署运行总数|Count|总计|软件更新部署的运行总计|SoftwareUpdateConfigurationName，状态|
|TotalUpdateDeploymentMachineRuns|运行更新部署计算机总数|Count|总计|在软件更新部署运行中运行的软件更新部署计算机总数|SoftwareUpdateConfigurationName，Status，TargetComputer，SoftwareUpdateConfigurationRunId|
...
|TaskStartEvent|任务启动事件数|Count|总计|已启动的任务总数|无|
|TaskCompleteEvent|任务完成事件数|Count|总计|已完成的任务总数|无|
|TaskFailEvent|任务失败事件数|Count|总计|处于失败状态的已完成任务总数|无|
|PoolCreateEvent|池创建事件数|Count|总计|已创建的池总数|无|
|PoolResizeStartEvent|池调整大小启动事件数|Count|总计|已启动的池调整大小活动总数|无|
|PoolResizeCompleteEvent|池调整大小完成事件数|Count|总计|已完成的池调整大小活动总数|无|
|PoolDeleteStartEvent|池删除启动事件数|Count|总计|已启动的池删除活动总数|无|
|PoolDeleteCompleteEvent|池删除完成事件数|Count|总计|已完成的池删除活动总数|无|
|JobDeleteCompleteEvent|作业删除完成事件数|Count|总计|已成功删除的作业总数。|无|
|JobDeleteStartEvent|作业删除启动事件数|Count|总计|已请求删除的作业总数。|无|
|JobDisableCompleteEvent|作业禁用完成事件数|Count|总计|已成功禁用的作业总数。|无|
|JobDisableStartEvent|作业禁用启动事件数|Count|总计|已请求禁用的作业总数。|无|
|JobStartEvent|作业启动事件数|Count|总计|已成功启动的作业总数。|无|
|JobTerminateCompleteEvent|作业终止完成事件数|Count|总计|已成功终止的作业总数。|无|
|JobTerminateStartEvent|作业终止启动事件数|Count|总计|已请求终止的作业总数。|无|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|已提交作业|已提交作业|Count|总计|已提交的作业数|方案，ClusterName|
|作业已完成|作业已完成|Count|总计|已完成的作业数|方案，ClusterName，ResultType|
|节点总数|节点总数|Count|平均值|节点总数|方案，ClusterName|
|活动节点|活动节点|Count|平均值|正在运行的节点数目|方案，ClusterName|
|空闲节点|空闲节点|Count|平均值|空闲节点数目|方案，ClusterName|
|不可用节点|不可用节点|Count|平均值|不可用的节点数目|方案，ClusterName|
|抢占节点|抢占节点|Count|平均值|已占用节点数|方案，ClusterName|
|退出节点|退出节点|Count|平均值|离开节点的数目|方案，ClusterName|
|内核总数|内核总数|Count|平均值|内核总数|方案，ClusterName|
|活动核心|活动核心|Count|平均值|活动核心数|方案，ClusterName|
|空闲内核数|空闲内核数|Count|平均值|空闲内核数|方案，ClusterName|
|不可用内核|不可用内核|Count|平均值|无法使用的内核数|方案，ClusterName|
|抢先核心|抢先核心|Count|平均值|被占用的内核数|方案，ClusterName|
|正在离开内核|正在离开内核|Count|平均值|正在离开的内核数|方案，ClusterName|
|配额使用率百分比|配额使用率百分比|Count|平均值|使用的配额百分比|方案、ClusterName、VmFamilyName、VmPriority|
...
|ConnectionAccepted|接受的连接|Count|总计|接受的连接|节点|
|ConnectionHandled|处理的连接|Count|总计|处理的连接|节点|
|ConnectionActive|活动连接数|Count|平均值|活动连接数|节点|
|RequestHandled|处理的请求|Count|总计|处理的请求|节点|
|ProcessedBlocks|处理的块|Count|总计|处理的块|节点|
|ProcessedTransactions|处理的事务|Count|总计|处理的事务|节点|
|PendingTransactions|挂起的事务|Count|平均值|挂起的事务|节点|
|QueuedTransactions|排队事务|Count|平均值|排队事务|节点|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|WebApplicationFirewallRequestCount|Web 应用程序防火墙请求计数|Count|总计|Web 应用程序防火墙所处理的客户端请求数|PolicyName，RuleName，操作|
...
|BlobCount|Blob 计数|Count|平均值|存储帐户的 Blob 服务中的 Blob 数。|BlobType，层|
...
|FileCount|文件计数|Count|平均值|存储帐户的文件服务中的文件数。|文件共享|
|FileShareCount|文件共享计数|Count|平均值|存储帐户的文件服务中的文件共享数。|无|
|FileShareSnapshotCount|文件共享快照计数|Count|平均值|存储帐户的文件服务中的共享上存在的快照数。|文件共享|
...
|事务|事务|Count|总计|向存储服务或指定的 API 操作发出的请求数。 此数值包括成功和失败的请求数，以及引发错误的请求数。 针对不同类型的响应数使用 ResponseType 维度。|ResponseType、GeoType、ApiName、Authentication、文件共享|
|流入量|流入量|字节|总计|流入的数据量（以字节为单位）。 此数字包括从外部客户端到 Azure 存储流入的数据量，以及流入 Azure 中的数据量。|GeoType、ApiName、Authentication、文件共享|
...
|CharactersTrained|训练的字符|Count|总计|训练的字符总数。|ApiName，OperationName，Region|
...
|OS 每磁盘 QD|OS Disk QD （已弃用）|Count|平均值|OS 磁盘队列深度(或队列长度)|无|
...
|OS 磁盘队列深度|OS 磁盘队列深度（预览）|Count|平均值|OS 磁盘队列深度(或队列长度)|无|
|入站流|入站流|Count|平均值|入站流是入站方向的当前流数（传入 VM 的流量）|无|
|出站流|出站流|Count|平均值|出站流是以出站方向（从 VM 传出的流量）为当前流的数目|无|
...
|入站流|入站流|Count|平均值|入站流是入站方向的当前流数（传入 VM 的流量）|VMName|
|出站流|出站流|Count|平均值|出站流是以出站方向（从 VM 传出的流量）为当前流的数目|VMName|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|TotalPullCount|请求总数|Count|平均值|映像提取总数|无|
|SuccessfulPullCount|成功的请求计数|Count|平均值|成功的图像提取次数|无|
|TotalPushCount|推送计数总计|Count|平均值|映像推送数总计|无|
|SuccessfulPushCount|推送计数成功|Count|平均值|成功的映像推送数|无|
...
|kube_pod_status_ready|就绪状态下的 Pod 数|Count|平均值|就绪状态下的 Pod 数|命名空间，pod|
|kube_node_status_condition|各种节点条件的状态|Count|平均值|各种节点条件的状态|条件，状态，status2，节点|
|kube_pod_status_phase|依据阶段的 Pod 数|Count|平均值|依据阶段的 Pod 数|阶段、命名空间、pod|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|SuccessfullRequests|成功的请求数|Count|总计|自定义提供程序发出的成功请求数|HttpMethod、CallPath、StatusCode|
...
|PipelineCancelledRuns|已取消的管道运行指标|Count|总计||FailureType、Name|
...
|ActivityCancelledRuns|已取消的活动运行指标|Count|总计||ActivityType、PipelineName、FailureType、Name|
|TriggerFailedRuns|失败的触发器运行数指标|Count|总计||Name、FailureType|
|TriggerSucceededRuns|成功的触发器运行数指标|Count|总计||Name、FailureType|
|TriggerCancelledRuns|已取消的触发器运行指标|Count|总计||Name、FailureType|
...
|IntegrationRuntimeAvailableNodeNumber|Integration runtime 可用节点计数|Count|平均值||IntegrationRuntimeName|
|MaxAllowedResourceCount|允许的最大实体计数|Count|最大值||无|
|MaxAllowedFactorySizeInGbUnits|允许的最大工厂大小（GB 单位）|Count|最大值||无|
|ResourceCount|实体总数|Count|最大值||无|
|FactorySizeInGbUnits|总工厂大小（GB 单位）|Count|最大值||无|
...
|network_bytes_egress|网络传出|字节|总计|跨活动连接数的网络传出|无|
|network_bytes_ingress|网络传入|字节|总计|跨活动连接数的网络传入|无|
|pg_replica_log_delay_in_seconds|副本滞后时间|秒|最大值|副本滞后时间（秒）|无|
...
|cpu_percent|CPU 百分比|百分比|平均值|CPU 百分比|无|
|memory_percent|内存百分比|百分比|平均值|内存百分比|无|
|iops|IOPS|Count|平均值|每秒 IO 操作数|无|
|storage_percent|存储空间百分比|百分比|平均值|存储空间百分比|无|
|storage_used|已用的存储量|字节|平均值|已用的存储量|无|
|active_connections|活动连接数|Count|平均值|活动连接数|无|
|network_bytes_egress|网络传出|字节|总计|跨活动连接数的网络传出|无|
...
|EventGridDeliveries|事件网格传递（预览）|Count|总计|发布到事件网格的 IoT 中心事件数。 对于成功和失败的请求数，请使用结果维度。 事件类型维度显示事件的类型（ https://aka.ms/ioteventgrid)。|ResourceId，Result，事件 Id|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|AddRegion|添加的区域|Count|Count|添加的区域|区域|
|AvailableStorage|可用存储|字节|总计|5分钟粒度报告的可用存储总数|CollectionName、DatabaseName、Region|
|CassandraConnectionClosures|Cassandra 连接关闭数|Count|总计|在1分钟的粒度报告的已关闭的 Cassandra 连接数|APIType、Region、ClosureReason|
|CassandraRequestCharges|Cassandra 请求费用|Count|总计|发出的 Cassandra 请求所使用的 ru 数|APIType、DatabaseName、CollectionName、Region、OperationType、ResourceType|
...
|CreateAccount|帐户已创建|Count|Count|帐户已创建|无|
|DataUsage|数据使用情况|字节|总计|在5分钟粒度内报告的总数据用量|CollectionName、DatabaseName、Region|
|DeleteAccount|帐户已删除|Count|Count|帐户已删除|无|
|DocumentCount|文档计数|Count|总计|5分钟粒度报告的总文档数|CollectionName、DatabaseName、Region|
...
|RegionFailover|区域故障转移|Count|Count|区域故障转移|无|
|RemoveRegion|删除区域|Count|Count|删除区域|区域|
...
|UpdateAccountKeys|帐户密钥已更新|Count|Count|帐户密钥已更新|KeyType|
|UpdateAccountNetworkSettings|已更新帐户网络设置|Count|Count|已更新帐户网络设置|无|
|UpdateAccountReplicationSettings|已更新帐户复制设置|Count|Count|已更新帐户复制设置|无|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|TransactionCount|事务计数|Count|Count|事务计数总计|TransactionCount|
|成功计数|成功计数|Count|Count|成功的事务计数|成功计数|
|FailureCount|失败计数|Count|Count|失败的事务计数|FailureCount|
...
|PublishFailCount|发布失败事件|Count|总计|未能发布到此主题的事件总数|主题，ErrorType，错误|
|PublishSuccessLatencyInMs|发布成功延迟|毫秒|总计|发布成功延迟时间（毫秒）|无|
|MatchedEventCount|匹配的事件数|Count|总计|与此事件订阅匹配的事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName|
|DeliveryAttemptFailCount|发送失败的事件数|Count|总计|未能发送到此事件订阅的事件总数|主题，EventSubscriptionName，DomainEventSubscriptionName，Error，ErrorType|
|DeliverySuccessCount|发送的事件数|Count|总计|发送到此事件订阅的事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName|
|DestinationProcessingDurationInMs|目标处理持续时间|毫秒|平均值|目标处理持续时间（毫秒）|主题、EventSubscriptionName、DomainEventSubscriptionName|
|DroppedEventCount|删除的事件数|Count|总计|与此事件订阅匹配的已删除事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName、DropReason|
|DeadLetteredCount|死信事件数|Count|总计|与此事件订阅匹配的死信事件总数|主题、EventSubscriptionName、DomainEventSubscriptionName、DeadLetterReason|
...
|PublishFailCount|发布失败事件|Count|总计|未能发布到此主题的事件总数|ErrorType，错误|
...
|DeliveryAttemptFailCount|发送失败的事件数|Count|总计|未能发送到此事件订阅的事件总数|错误，ErrorType|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的请求数|Count|总计|Microsoft.EventHub 成功请求数。|EntityName，OperationResult|
|ServerErrors|服务器错误数。|Count|总计|Microsoft.EventHub 的服务器错误数。|EntityName，OperationResult|
|UserErrors|用户错误数。|Count|总计|Microsoft.EventHub 用户错误数。|EntityName，OperationResult|
|QuotaExceededErrors|超过限额错误。|Count|总计|Microsoft.EventHub 的超过限额错误数。|EntityName，OperationResult|
|ThrottledRequests|限制的请求数。|Count|总计|Microsoft.EventHub 限制的请求数。|EntityName，OperationResult|
|IncomingRequests|传入请求数|Count|总计|Microsoft.EventHub 传入的请求数。|EntityName|
|IncomingMessages|传入消息数|Count|总计|Microsoft.EventHub 传入的消息数。|EntityName|
...
|INREQS|传入请求（已弃用）|Count|总计|命名空间的传入发送请求总数（不推荐使用）|无|
|SUCCREQ|成功的请求数（已弃用）|Count|总计|命名空间的成功请求总数（不推荐使用）|无|
|FAILREQ|失败的请求数（已弃用）|Count|总计|命名空间的失败请求总数（不推荐使用）|无|
|SVRBSY|服务器忙错误（已弃用）|Count|总计|命名空间的服务器繁忙错误总数（不推荐使用）|无|
|INTERR|内部服务器错误（已弃用）|Count|总计|命名空间的内部服务器错误总数（不推荐使用）|无|
|MISCERR|其他错误（不推荐使用）|Count|总计|命名空间的失败请求总数（不推荐使用）|无|
|INMSGS|传入消息（已过时）（不推荐使用）|Count|总计|命名空间的传入消息总数。 此指标已弃用。 请改用传入消息度量值（不推荐使用）|无|
|EHINMSGS|传入消息（已弃用）|Count|总计|命名空间的传入消息总数（不推荐使用）|无|
|OUTMSGS|传出消息（已过时）（不推荐使用）|Count|总计|命名空间的传出消息总数。 此指标已弃用。 请改用传出消息度量值（不推荐使用）|无|
|EHOUTMSGS|传出消息（已弃用）|Count|总计|命名空间的传出消息总数（不推荐使用）|无|
...
|EHABL|存档积压消息（已弃用）|Count|总计|命名空间的积压工作（backlog）中的事件中心存档消息（不推荐使用）|无|
|EHAMSGS|存档消息（已弃用）|Count|总计|命名空间中的事件中心存档消息（不推荐使用）|无|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的请求数|Count|总计|Microsoft.EventHub 成功请求数。|OperationResult|
|ServerErrors|服务器错误数。|Count|总计|Microsoft.EventHub 的服务器错误数。|OperationResult|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|GatewayRequests|网关请求数|Count|总计|网关请求数|HttpStatus|
|CategorizedGatewayRequests|已分类的网关请求数|Count|总计|按类别（1xx/2xx/3xx/4xx/5xx）统计的网关请求数|HttpStatus|
|NumActiveWorkers|活动工作线程数|Count|最大值|活动工作线程数|MetricName|
...
|dependencies/count|依赖项调用|Count|Count|应用程序对外部资源所进行的调用计数。|依赖项/类型、依赖项/performanceBucket、依赖关系/成功、依赖项/目标、操作/合成、云/roleInstance、云/工作项|
...
|dependencies/failed|依赖项调用失败|Count|Count|应用程序对外部资源所进行的依赖项调用失败的计数。|依赖项/类型、依赖项/performanceBucket、依赖关系/成功、依赖项/目标、操作/合成、云/roleInstance、云/工作项|
|pageViews/count|页面视图|Count|Count|页面视图计数。|操作/综合，云/工作群|
...
|requests/count|服务器请求数|Count|Count|已完成的 HTTP 请求计数。|request/performanceBucket，request/resultCode，operation/合成，cloud/roleInstance，request/success，云/工作群|
|requests/failed|失败的请求|Count|Count|标记为失败的 HTTP 请求的计数。 在大多数情况下这些请求的响应代码 >= 400 且不等于 401。|request/performanceBucket，request/resultCode，request/success，操作/综合，cloud/roleInstance，云/工作群|
...
|exceptions/count|例外|Count|Count|所有未捕获异常的已合并计数。|云/工作群、云/roleInstance、客户端/类型|
|exceptions/browser|浏览器异常|Count|Count|浏览器中所引发未捕获异常的计数。|client/isServer、cloud/|
|exceptions/server|服务器异常|Count|Count|服务器应用程序中引发的未捕获的异常计数。|client/isServer、cloud/、cloud/roleInstance|
|traces/count|跟踪|Count|Count|跟踪文档计数|trace/severityLevel、operation/合成、cloud/、cloud/roleInstance|
...
|ServiceApiResult|服务 API 结果总计|Count|Count|服务 API 结果总数|ActivityType，ActivityName，StatusCode，StatusCodeClass|
...
|IngestionVolumeInMB|引入卷（MB）|Count|总计|已引入群集的数据总量 (MB)|数据库|
|IngestionLatencyInSeconds|引入延迟（秒）|秒|平均值|从源（例如消息位于事件中心）到群集的引入时间（秒）|无|
|EventsProcessedForEventHubs|处理的事件数（适用于事件/IoT 中心）|Count|总计|引入 from 事件/IoT 中心时，群集处理的事件数|EventStatus|
|IngestionResult|引入结果|Count|Count|引入操作的数量|IngestionResultDetails|
|CPU|CPU|百分比|平均值|CPU 使用率级别|无|
|ContinuousExportNumOfRecordsExported|连续导出–已导出记录的数目|Count|总计|导出操作期间写入的每个存储项目的已导出记录数|无|
|ExportUtilization|导出利用率|百分比|最大值|导出利用率|无|
|ContinuousExportPendingCount|连续导出挂起计数|Count|最大值|准备执行的挂起连续导出作业的数目|无|
|ContinuousExportMaxLatenessMinutes|连续导出最大迟到分钟数|Count|最大值|等待并准备执行的所有连续导出的最大迟到（分钟）|无|
|ContinuousExportResult|连续导出结果|Count|Count|指示连续导出是成功还是失败|ContinuousExportName，结果，数据库|
|StreamingIngestDuration|流摄取持续时间|毫秒|平均值|流引入持续时间（毫秒）|无|
|StreamingIngestDataRate|流引入数据速率|Count|平均值|流引入数据速率（MB/秒）|无|
|SteamingIngestRequestRate|流式处理引入请求速率|Count|RateRequestsPerSecond|流式处理引入请求速率（每秒请求数）|无|
|StreamingIngestResults|流式处理引入结果|Count|平均值|流式处理引入结果|结果|
|TotalNumberOfConcurrentQueries|并发查询总数|Count|总计|并发查询总数|无|
|TotalNumberOfThrottledQueries|限制查询总数|Count|总计|限制查询总数|无|
|TotalNumberOfThrottledCommands|阻止的命令总数|Count|总计|阻止的命令总数|CommandType|
|TotalNumberOfExtents|总区数|Count|总计|数据区总数|无|
...
|RunStartThrottledEvents|运行启动限制事件|Count|总计|工作流运行启动限制事件的数目。|无|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|已完成运行|已完成运行|Count|总计|已成功为此工作区完成的运行数|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|已开始运行|已开始运行|Count|总计|为此工作区启动的运行数|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|失败的运行次数|失败的运行次数|Count|总计|此工作区的运行失败次数|方案、RunType、PublishedPipelineId、ComputeType、PipelineStepType|
|模型注册成功|模型注册成功|Count|总计|此工作区中成功的模型注册数|场景|
|模型注册失败|模型注册失败|Count|总计|此工作区中失败的模型注册数|方案，StatusCode|
|模型部署已启动|模型部署已启动|Count|总计|在此工作区中启动的模型部署数|场景|
|模型部署成功|模型部署成功|Count|总计|此工作区中成功的模型部署数|场景|
|模型部署失败|模型部署失败|Count|总计|此工作区中失败的模型部署数|方案，StatusCode|
|节点总数|节点总数|Count|平均值|节点总数。 此总数包括某些活动节点、空闲节点、不可用的节点、已抢占的节点、保留节点|方案，ClusterName|
|活动节点|活动节点|Count|平均值|Acitve 节点数。 这些节点是当前正在运行作业的节点。|方案，ClusterName|
|空闲节点|空闲节点|Count|平均值|空闲节点数。 空闲节点是指未运行任何作业的节点，但可以接受新作业（如果可用）。|方案，ClusterName|
|不可用节点|不可用节点|Count|平均值|无法使用的节点数。 由于某些无法解析的问题，无法使用的节点不起作用。 Azure 将回收这些节点。|方案，ClusterName|
|抢占节点|抢占节点|Count|平均值|已抢占节点数。 这些节点是从可用节点池中删除的低优先级节点。|方案，ClusterName|
|退出节点|退出节点|Count|平均值|正在离开的节点数。 保留节点是指刚完成了作业处理并将进入空闲状态的节点。|方案，ClusterName|
|内核总数|内核总数|Count|平均值|内核总数|方案，ClusterName|
|活动核心|活动核心|Count|平均值|活动核心数|方案，ClusterName|
|空闲内核数|空闲内核数|Count|平均值|空闲内核数|方案，ClusterName|
|不可用内核|不可用内核|Count|平均值|无法使用的内核数|方案，ClusterName|
...
|正在离开内核|正在离开内核|Count|平均值|正在离开的内核数|方案，ClusterName|
|配额使用率百分比|配额使用率百分比|Count|平均值|使用的配额百分比|方案、ClusterName、VmFamilyName、VmPriority|
|CpuUtilization|CpuUtilization|Count|平均值|CPU（预览）|方案、runId、CreatedTime|
|GpuUtilization|GpuUtilization|Count|平均值|GPU （预览）|方案、runId、、CreatedTime、DeviceId|
...
|Requests|Requests|Count|总计|对流式处理终结点的请求。|OutputFormat，HttpStatusCode，ErrorCode|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|AssetQuota|资产配额|Count|平均值|当前 media service 帐户允许的资产数量|无|
|AssetCount|资产计数|Count|平均值|当前 media service 帐户中已创建的资产数量|无|
|AssetQuotaUsedPercentage|资产配额已用百分比|百分比|平均值|当前媒体服务帐户中的资产已用百分比|无|
|ContentKeyPolicyQuota|内容密钥策略配额|Count|平均值|当前 media service 帐户允许多少内容密钥策略|无|
|ContentKeyPolicyCount|内容密钥策略计数|Count|平均值|当前 media service 帐户中已创建了多少内容密钥策略|无|
|ContentKeyPolicyQuotaUsedPercentage|内容密钥策略配额已用百分比|百分比|平均值|当前 media service 帐户中使用的内容密钥策略百分比|无|
|StreamingPolicyQuota|流式处理策略配额|Count|平均值|当前 media service 帐户允许多少流式处理策略|无|
|StreamingPolicyCount|流式处理策略计数|Count|平均值|当前 media service 帐户中已创建了多少个流式处理策略|无|
...
|AllocatedSnatPorts|已分配的 SNAT 端口数（预览）|Count|总计|时间段内已分配的 SNAT 端口总数|FrontendIPAddress、BackendIPAddress、ProtocolType、IsAwaitingRemoval|
|UsedSnatPorts|已使用的 SNAT 端口数（预览）|Count|总计|时间段内 SNAT 端口的总数|FrontendIPAddress、BackendIPAddress、ProtocolType、IsAwaitingRemoval|
...
|ByteCount|字节计数|Count|总计|时间段内传输的字节总数|端口，方向|
|PacketCount|数据包计数|Count|总计|时间段内传输的数据包总数|端口，方向|
|SynCount|SYN 计数|Count|总计|时间段内传输的 SYN 数据包总数|端口，方向|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|ApplicationRuleHit|应用程序规则命中计数|Count|总计|应用程序规则命中次数|状态、原因、协议|
|NetworkRuleHit|网络规则命中计数|Count|总计|命中网络规则的次数|状态、原因、协议|
...
|AvgRequestCountPerHealthyHost|每分钟每个正常主机的请求数|Count|平均值|池中每个正常的后端主机每分钟平均请求计数|BackendSettingsPool|
...
|CapacityUnits|当前容量单位|Count|平均值|使用的容量单位|无|
|ComputeUnits|当前计算单元|Count|平均值|使用的计算单位|无|
|BackendResponseStatus|后端响应状态|Count|总计|后端成员生成的 HTTP 响应代码的数目。 这不包括任何由应用程序网关生成的响应代码。|BackendServer、BackendPool、BackendHttpSetting、HttpStatusGroup|
|TlsProtocol|客户端 TLS 协议|Count|总计|与应用程序网关建立连接的客户端启动的 TLS 和非 TLS 请求数。 若要查看 TLS 协议分发，请按维度 TLS 协议进行筛选。|侦听器，TlsProtocol|
...
|MatchedCount|Web 应用程序防火墙规则分发总数|Count|总计|Web 应用程序防火墙传入流量的规则分配总数|RuleGroup、RuleId|
|BlockedCount|Web 应用程序防火墙阻止的请求规则分发|Count|总计|Web 应用程序防火墙阻止的请求规则分发|RuleGroup、RuleId|
|BlockedReqCount|Web 应用程序防火墙阻止的请求计数|Count|总计|Web 应用程序防火墙阻止的请求计数|无|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|RxLightLevel|RxLightLevel|Count|平均值|Rx 轻型级别（dBm）|链接，通道|
|TxLightLevel|TxLightLevel|Count|平均值|按 dBm 的 Tx 轻型级别|链接，通道|
|AdminState|AdminState|Count|平均值|端口的管理状态|链接|
|LineProtocol|LineProtocol|Count|平均值|端口的线路协议状态|链接|
...
|BackendRequestCount|后端请求计数|Count|总计|从 HTTP/S 代理发送到后端的请求数|HttpStatus、HttpStatusGroup、后端|
...
|更新|更新|Count|平均值|更新|计算机，产品，分类，UpdateState，可选，已批准|
...
|SenderConnections-ServerError|SenderConnections-ServerError|Count|总计|关于 Microsoft.Relay 的发送方连接的服务器错误。|EntityName，OperationResult|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|SuccessfulRequests|成功的请求数|Count|总计|命名空间的成功请求总数|EntityName，OperationResult|
|ServerErrors|服务器错误数。|Count|总计|Microsoft.ServiceBus 的服务器错误数。|EntityName，OperationResult|
|UserErrors|用户错误数。|Count|总计|Microsoft.ServiceBus 的用户错误数。|EntityName，OperationResult|
|ThrottledRequests|限制的请求数。|Count|总计|Microsoft.ServiceBus 限制的请求数。|EntityName，OperationResult|
...
|ScheduledMessages|队列/主题中计划消息的计数。|Count|平均值|队列/主题中计划消息的计数。|EntityName|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|AllocatedCpu|AllocatedCpu|Count|平均值|在 milli 核心中分配给此容器的 Cpu|ApplicationName、ServiceName、CodePackageName、ServiceReplicaName|
|AllocatedMemory|AllocatedMemory|字节|平均值|分配给此容器的内存（MB）|ApplicationName、ServiceName、CodePackageName、ServiceReplicaName|
|ActualCpu|ActualCpu|Count|平均值|Milli 核心中的实际 CPU 使用率|ApplicationName、ServiceName、CodePackageName、ServiceReplicaName|
...
|ApplicationStatus|ApplicationStatus|Count|平均值|Service Fabric 网格应用程序的状态|ApplicationName，状态|
|ServiceStatus|ServiceStatus|Count|平均值|Service Fabric 网格应用程序中服务的运行状况状态|ApplicationName、Status、ServiceName|
|ServiceReplicaStatus|ServiceReplicaStatus|Count|平均值|Service Fabric 网格应用程序中服务副本的运行状况状态|ApplicationName，Status，ServiceName，ServiceReplicaName|
|ContainerStatus|ContainerStatus|Count|平均值|Service Fabric 网格应用程序中容器的状态|ApplicationName、ServiceName、CodePackageName、ServiceReplicaName、Status|
|RestartCount|RestartCount|Count|平均值|Service Fabric 网格应用程序中的容器重启计数|ApplicationName、Status、ServiceName、ServiceReplicaName、CodePackageName|

## <a name="microsoftsignalrservicesignalr"></a>Microsoft.SignalRService/SignalR

|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|ConnectionCount|连接计数|Count|最大值|用户连接量。|端点|
...
|deadlock|死锁数|Count|总计|死锁. 不适用于数据仓库。|无|
...
|tempdb_data_size|Tempdb 数据文件大小（Kb）|Count|最大值|Tempdb 数据文件大小（Kb）。 不适用于数据仓库。|无|
|tempdb_log_size|Tempdb 日志文件大小（Kb）|Count|最大值|Tempdb 日志文件大小（Kb）。 不适用于数据仓库。|无|
...
|app_cpu_billed|应用 CPU 计费|Count|总计|应用 CPU 计费。 适用于无服务器数据库。|无|
...
|eDTU_used|已用的 eDTU|Count|平均值|使用的 eDTU。 适用于基于 DTU 的弹性池。|无|
...
|cpu_used|使用的 CPU|Count|平均值|使用的 CPU。 适用于基于 vCore 的弹性池。|无|
|database_cpu_used|使用的 CPU|Count|平均值|使用的 CPU|DatabaseResourceId|
...
|tempdb_data_size|Tempdb 数据文件大小（Kb）|Count|最大值|Tempdb 数据文件大小（Kb）|无|
|tempdb_log_size|Tempdb 日志文件大小（Kb）|Count|最大值|Tempdb 日志文件大小（Kb）|无|
...
|dtu_used|已用的 DTU|Count|平均值|已用的 DTU|DatabaseResourceId|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|ClientIOPS|客户端 IOPS 总数|Count|平均值|缓存处理的客户端文件操作的速率。|无|
...
|StorageTargetIOPS|StorageTarget IOPS 总数|Count|平均值|缓存发送到特定 StorageTarget 的所有文件操作的速率。|StorageTarget|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|ServerSyncSessionResult|同步会话结果|Count|平均值|每次服务器终结点成功完成与云终结点的同步会话时记录值1的指标|SyncGroupName,ServerEndpointName,SyncDirection|
|StorageSyncSyncSessionAppliedFilesCount|已同步文件|Count|总计|同步的文件计数|SyncGroupName,ServerEndpointName,SyncDirection|
|StorageSyncSyncSessionPerItemErrorsCount|未同步的文件|Count|总计|未能同步的文件计数|SyncGroupName,ServerEndpointName,SyncDirection|
|StorageSyncBatchTransferredFileBytes|同步的字节数|字节|总计|为同步会话传输的总文件大小|SyncGroupName,ServerEndpointName,SyncDirection|
|StorageSyncServerHeartbeat|服务器联机状态|Count|最大值|每次 resigtered 服务器成功使用云终结点记录检测信号时记录值1的指标|ServerName|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|SyncGroupSyncSessionAppliedFilesCount|已同步文件|Count|总计|同步的文件计数|SyncGroupName,ServerEndpointName,SyncDirection|
|SyncGroupSyncSessionPerItemErrorsCount|未同步的文件|Count|总计|未能同步的文件计数|SyncGroupName,ServerEndpointName,SyncDirection|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|ServerEndpointSyncSessionAppliedFilesCount|已同步文件|Count|总计|同步的文件计数|ServerEndpointName,SyncDirection|
|ServerEndpointSyncSessionPerItemErrorsCount|未同步的文件|Count|总计|未能同步的文件计数|ServerEndpointName,SyncDirection|
...
|指标|指标显示名称|单位|聚合类型|说明|维度|
|---|---|---|---|---|---|
|ServerHeartbeat|服务器联机状态|Count|最大值|每次 resigtered 服务器成功使用云终结点记录检测信号时记录值1的指标|ServerResourceId、ServerName|
...
|InputEvents|输入事件数|Count|总计|输入事件数|LogicalName、PartitionId|
|InputEventBytes|输入事件字节数|字节|总计|输入事件字节数|LogicalName、PartitionId|
|LateInputEvents|延迟输入事件数|Count|总计|延迟输入事件数|LogicalName、PartitionId|
|OutputEvents|输出事件数|Count|总计|输出事件数|LogicalName、PartitionId|
|ConversionErrors|数据转换错误数|Count|总计|数据转换错误数|LogicalName、PartitionId|
|错误|运行时错误|Count|总计|运行时错误|LogicalName、PartitionId|
|DroppedOrAdjustedEvents|失序事件数|Count|总计|失序事件数|LogicalName、PartitionId|
|AMLCalloutRequests|函数请求数|Count|总计|函数请求数|LogicalName、PartitionId|
|AMLCalloutFailedRequests|失败的函数请求数|Count|总计|失败的函数请求数|LogicalName、PartitionId|
|AMLCalloutInputEvents|函数事件数|Count|总计|函数事件数|LogicalName、PartitionId|
|DeserializationError|输入反序列化错误|Count|总计|输入反序列化错误|LogicalName、PartitionId|
|EarlyInputEvents|早期输入事件数|Count|总计|早期输入事件数|LogicalName、PartitionId|
|OutputWatermarkDelaySeconds|水印延迟|秒|最大值|水印延迟|LogicalName、PartitionId|
|InputEventsSourcesBacklogged|积压的输入事件数|Count|最大值|积压的输入事件数|LogicalName、PartitionId|
...
|IngressReceivedMessagesCountLag|入口收到的消息数计数延迟|Count|平均值|事件源分区中最后排队消息的序列号与传入正在处理的消息序列号之间的差异|无|
|WarmStorageMaxProperties|热存储最大属性|Count|最大值|用于 S1/S2 SKU 的环境允许的最大属性数，以及 PAYG SKU 的热商店允许的最大属性数|无|
|WarmStorageUsedProperties|热存储使用属性 |Count|最大值|用于 S1/S2 SKU 的环境使用的属性数，以及用于 PAYG SKU 的热商店使用的属性数|无|
...
|DiskReadOperations|磁盘读取操作|Count|总计|上一个采样期间内的 IO 读取操作数。 请注意，这些操作可能会改变大小。|无|
|DiskWriteOperations|磁盘写入操作|Count|总计|上一个采样期间内的 IO 写入操作数。 请注意，这些操作可能会改变大小。|无|
...
|TcpSynSent|TCP Syn 已发送|Count|平均值|TCP Syn 已发送|实例|
|TcpSynReceived|已收到 TCP Syn|Count|平均值|已收到 TCP Syn|实例|
|TcpEstablished|已建立 TCP|Count|平均值|已建立 TCP|实例|
|TcpFinWait1|TCP Fin 等待1|Count|平均值|TCP Fin 等待1|实例|
|TcpFinWait2|TCP Fin 等待2|Count|平均值|TCP Fin 等待2|实例|
|TcpClosing|TCP 关闭|Count|平均值|TCP 关闭|实例|
|TcpCloseWait|TCP 关闭等待|Count|平均值|TCP 关闭等待|实例|
|TcpLastAck|TCP 上次确认|Count|平均值|TCP 上次确认|实例|
