---
title: Azure 监视器常见问题 |微软文档
description: 回答有关 Azure 监视器的常见问题的解答。
...
此 Microsoft 常见问题解答是关于 Azure 监视器的常见问题列表。

## <a name="general"></a>常规

### <a name="what-is-azure-monitor"></a>说明是 Azure Monitor？
[Azure 监视器](overview.md)是 Azure 中的一项服务，它为 Azure、其他云环境或本地中的应用程序和服务提供性能和可用性监视。 Azure 监视器将数据从多个源收集到一个公共数据平台中，可以分析其趋势和异常。 Azure 监视器中的丰富功能可帮助您快速识别和响应可能影响应用程序的关键情况。

### <a name="whats-the-difference-between-azure-monitor-log-analytics-and-application-insights"></a>Azure 监视器、日志分析和应用程序见解之间的区别是什么？
2018 年 9 月，Microsoft 将 Azure 监视器、日志分析和应用程序见解合并到单个服务中，以便对应用程序及其所依赖的组件提供强大的端到端监视。 日志分析和应用程序见解中的功能没有改变，尽管某些功能已重命名为 Azure 监视器，以便更好地反映其新范围。 日志分析的日志数据引擎和查询语言现在称为 Azure 监视器日志。 请参阅[Azure 监视器术语更新](terminology.md)。

### <a name="what-does-azure-monitor-cost"></a>Azure 监视器的成本是多少？
自动启用的 Azure 监视器功能（如收集指标和活动日志）不收取任何费用。 与其他功能（如日志查询和警报）相关联的成本。 有关详细的定价信息，请参阅[Azure 监视器定价页](https://azure.microsoft.com/pricing/details/monitor/)。

### <a name="how-do-i-enable-azure-monitor"></a>如何启用 Azure 监视器？
创建新 Azure 订阅的瞬间启用 Azure 监视器，并自动收集[活动日志](platform/activity-logs-overview.md)和平台[指标](platform/data-platform-metrics.md)。 创建[诊断设置](platform/diagnostic-settings.md)以收集有关 Azure 资源操作的更多详细信息，并添加[监视解决方案](insights/solutions.md)和[见解](insights/insights-overview.md)，以便对特定服务收集的数据提供其他分析。 

### <a name="how-do-i-access-azure-monitor"></a>如何访问 Azure 监视器？
从 Azure 门户中的 **"监视器"** 菜单访问所有 Azure 监视器功能和数据。 不同 Azure 服务的菜单的 **"监视**"部分提供对相同工具的访问，并筛选到特定资源的数据。 Azure 监视器数据对于使用 CLI、PowerShell 和 REST API 的各种方案也可访问。

### <a name="is-there-an-on-premises-version-of-azure-monitor"></a>是否有本地版本的 Azure 监视器？
不是。 Azure 监视器是一种可扩展的云服务，用于处理和存储大量数据，尽管 Azure 监视器可以监视本地和其他云中的资源。

### <a name="can-azure-monitor-monitor-on-premises-resources"></a>Azure 监视器是否可以监视本地资源？
是的，除了从 Azure 资源收集监视数据外，Azure 监视器还可以从其他云和本地中的虚拟机和应用程序收集数据。 请参阅[Azure 监视器的监视数据源](platform/data-sources.md)。

### <a name="does-azure-monitor-integrate-with-system-center-operations-manager"></a>Azure 监视器是否与系统中心操作管理器集成？
您可以将现有的系统中心操作管理器管理组连接到 Azure 监视器，以便从代理收集数据到 Azure 监视器日志中。 这允许您使用日志查询和解决方案来分析从代理收集的数据。 还可以配置现有的系统中心操作管理器代理，将数据直接发送到 Azure 监视器。 请参阅[将操作管理器连接到 Azure 监视器](platform/om-agents.md)。

### <a name="what-ip-addresses-does-azure-monitor-use"></a>Azure 监视器使用哪些 IP 地址？
有关代理和其他外部资源访问 Azure 监视器所需的 IP 地址和端口的列表，请参阅[应用程序见解和日志分析使用的 IP 地址](app/ip-addresses.md)。 

## <a name="monitoring-data"></a>监视数据

### <a name="where-does-azure-monitor-get-its-data"></a>Azure 监视器从何处获取其数据？
Azure 监视器从各种源收集数据，包括来自 Azure 平台和资源的日志和指标、自定义应用程序和在虚拟机上运行的代理。 其他服务（如 Azure 安全中心和网络观察程序）将数据收集到日志分析工作区中，以便可以使用 Azure 监视器数据对其进行分析。 您还可以使用 REST API 将自定义数据发送到 Azure 监视器，用于日志或指标。 请参阅[Azure 监视器的监视数据源](platform/data-sources.md)。

### <a name="what-data-is-collected-by-azure-monitor"></a>Azure 监视器收集了哪些数据？ 
Azure 监视器将数据从各种源收集到[日志](platform/data-platform-logs.md)或[指标中](platform/data-platform-metrics.md)。 每种类型的数据都有其各自的相对优势，并且每种数据都支持 Azure 监视器中的特定功能集。 每个 Azure 订阅都有一个指标数据库，同时您可以创建多个日志分析工作区来收集日志，具体取决于您的要求。 请参阅[Azure 监视器数据平台](platform/data-platform.md)。

### <a name="is-there-a-maximum-amount-of-data-that-i-can-collect-in-azure-monitor"></a>是否可以在 Azure 监视器中收集最大数量的数据？
您可以收集的指标数据量没有限制，但此数据存储最多为 93 天。 请参阅[保留指标](platform/data-platform-metrics.md#retention-of-metrics)。 您可以收集的日志数据量没有限制，但可能会受您为日志分析工作区选择的定价层的影响。 请参阅[定价详细信息](https://azure.microsoft.com/pricing/details/monitor/)。

### <a name="how-do-i-access-data-collected-by-azure-monitor"></a>如何访问 Azure 监视器收集的数据？
见解和解决方案为处理存储在 Azure 监视器中的数据提供了自定义体验。 您可以使用用 Kusto 查询语言 （KQL） 编写的日志查询直接处理日志数据。 在 Azure 门户中，可以使用日志分析编写和运行查询并交互式分析数据。 使用指标资源管理器分析 Azure 门户中的指标。 请参阅[在 Azure 监视器中分析日志数据](log-query/log-query-overview.md)，然后[开始使用 Azure 指标资源管理器](platform/metrics-getting-started.md)。

## <a name="solutions-and-insights"></a>解决方案和见解

### <a name="what-is-an-insight-in-azure-monitor"></a>Azure 监视器中的见解是什么？
见解为特定的 Azure 服务提供自定义的监视体验。 它们使用与 Azure 监视器中的其他功能相同的指标和日志，但可能会收集其他数据并在 Azure 门户中提供独特的体验。 请参阅[Azure 监视器 中的见解](insights/insights-overview.md)。

要查看 Azure 门户中的见解，请参阅 **"监视器"** 菜单的 **"见解"** 部分或服务菜单的 **"监视"** 部分。

### <a name="what-is-a-solution-in-azure-monitor"></a>Azure 监视器中的解决方案是什么？
监视解决方案是打包的逻辑集，用于根据 Azure 监视器功能监视特定应用程序或服务。 它们在 Azure 监视器中收集日志数据，并使用 Azure 门户中的常见体验为其分析提供日志查询和视图。 请参阅[Azure 监视器 中的监视解决方案](insights/solutions.md)。

要查看 Azure 门户中的解决方案，请在 **"监视器"** 菜单的 **"见解"** 部分中单击 **"更多**"。 单击"**添加**"以向工作区添加其他解决方案。

## <a name="logs"></a>日志

### <a name="whats-the-difference-between-azure-monitor-logs-and-azure-data-explorer"></a>Azure 监视器日志和 Azure 数据资源管理器之间的区别是什么？
Azure 数据资源管理器是一项快速且高度可缩放的数据探索服务，适用于日志和遥测数据。 Azure 监视器日志构建在 Azure 数据资源管理器之上，使用相同的 Kusto 查询语言 （KQL），但存在一些细微差异。 请参阅[Azure 监视器日志查询语言差异](log-query/data-explorer-difference.md)。

### <a name="how-do-i-retrieve-log-data"></a>如何检索日志数据？
使用使用 Kusto 查询语言 （KQL） 编写的日志查询从日志分析工作区检索所有数据。 您可以编写自己的查询或使用包含特定应用程序或服务的日志查询的解决方案和见解。 请参阅[Azure 监视器 中的日志查询概述](log-query/log-query-overview.md)。

### <a name="what-is-a-log-analytics-workspace"></a>Log Analytics 工作区是什么？
Azure 监视器收集的所有日志数据都存储在日志分析工作区中。 工作区本质上是一个容器，用于从各种源收集日志数据。 您可能具有用于所有监视数据的单个日志分析工作区，或者可能具有多个工作区的要求。 请参阅[设计 Azure 监视器日志部署](platform/design-logs-deployment.md)。

### <a name="can-you-move-an-existing-log-analytics-workspace-to-another-azure-subscription"></a>是否可以将现有的日志分析工作区移动到另一个 Azure 订阅？
可以在资源组或订阅之间移动工作区，但不能移动到其他区域。 请参阅[将日志分析工作区移动到不同的订阅或资源组](platform/move-workspace.md)。

### <a name="why-cant-i-see-query-explorer-and-save-buttons-in-log-analytics"></a>为什么我无法在日志分析中看到查询资源管理器和保存按钮？

[当查询范围](log-query/scope.md)设置为特定资源时，**查询资源管理器**、**保存**和**新建警报规则**按钮不可用。 要创建警报、保存或加载查询，日志分析必须限定为工作区。 要在工作区上下文中打开日志分析，请从**Azure 监视器**菜单中选择 **"日志**"。 选择的是上次使用的工作区，但可以选择任何其他的工作区。 请参阅[Azure 监视器日志分析中的日志查询范围和时间范围](log-query/scope.md)

### <a name="why-am-i-getting-the-error-register-resource-provider-microsoftinsights-for-this-subscription-to-enable-this-query-when-opening-log-analytics-from-a-vm"></a>为什么在从 VM 打开日志分析时，我会收到错误："注册此订阅的资源提供商'Microsoft.Insights'以启用此查询"？ 
许多资源提供程序都将自动注册，但您可能需要手动注册某些资源提供程序。 注册的作用域始终是订阅。 有关详细信息，请参阅[资源提供程序和类型](../azure-resource-manager/management/resource-providers-and-types.md#azure-portal)。

### <a name="why-am-i-am-getting-no-access-error-message-when-opening-log-analytics-from-a-vm"></a>为什么从 VM 打开日志分析时没有访问错误消息？ 
若要查看 VM 日志，需获得存储 VM 日志的工作区的读取权限。 在这些示例中，管理员必须在 Azure 中为你授予相关权限。

## <a name="alerts"></a>警报

### <a name="what-is-an-alert-in-azure-monitor"></a>Azure 监视器中的警报是什么？
在监视数据中发现重要情况时，警报会以主动的方式通知你。 有了警报，你就可以在系统的用户注意到问题之前确定和解决这些问题。 警报有多种类型：

- 指标 - 指标值超过阈值。
- 日志查询 - 日志查询的结果与定义的条件匹配。
- 活动日志 - 活动日志事件与定义的条件匹配。
- Web 测试 - 可用性测试与定义的条件匹配的结果。


请参阅[Microsoft Azure 中的警报概述](platform/alerts-overview.md)。


### <a name="what-is-an-action-group"></a>什么是行动组？
操作组是警报可以触发的通知和操作的集合。 多个警报可以使用单个操作组，允许您利用常见的通知和操作集。 请参阅[在 Azure 门户中创建和管理操作组](platform/action-groups.md)。


### <a name="what-is-an-action-rule"></a>什么是操作规则？
操作规则允许您修改一组符合特定条件的警报的行为。 这允许您执行维护时段内禁用警报操作等要求。 还可以将操作组应用于一组警报，而不是将其直接应用于警报规则。 请参阅[操作规则](platform/alerts-action-rules.md)。

## <a name="agents"></a>代理

### <a name="does-azure-monitor-require-an-agent"></a>Azure 监视器是否需要代理？
只需代理即可从虚拟机中的操作系统和工作负载收集数据。 虚拟机可以位于 Azure、另一个云环境或本地。 请参阅[Azure 监视器代理的概述](platform/agents-overview.md)。


### <a name="whats-the-difference-between-the-azure-monitor-agents"></a>Azure 监视器代理之间的区别是什么？
Azure 诊断扩展适用于 Azure 虚拟机，并将数据收集到 Azure 监视器指标、Azure 存储和 Azure 事件中心。 日志分析代理适用于 Azure 中的虚拟机、另一个云环境或本地虚拟机，并将数据收集到 Azure 监视器日志。 依赖项代理需要日志分析代理并收集进程详细信息和依赖项。 请参阅[Azure 监视器代理的概述](platform/agents-overview.md)。


### <a name="does-my-agent-traffic-use-my-expressroute-connection"></a>我的代理流量是否使用我的 ExpressRoute 连接？
到 Azure 监视器的流量使用 Microsoft 对等快速路由电路。 有关不同类型的快速路由流量的说明，请参阅[ExpressRoute 文档](../expressroute/expressroute-faqs.md#supported-services)。 

### <a name="how-can-i-confirm-that-the-log-analytics-agent-is-able-to-communicate-with-azure-monitor"></a>如何确认日志分析代理能够与 Azure 监视器通信？
从代理计算机上的控制面板中，选择 **"安全&设置**"，**微软监控代理**。 在**Azure 日志分析 （OMS）** 选项卡下，绿色复选标记图标确认代理能够与 Azure 监视器通信。 黄色警告图标表示代理出现问题。 一个常见原因是**Microsoft 监视代理**服务已停止。 请使用服务控制管理器重启该服务。

### <a name="how-do-i-stop-the-log-analytics-agent-from-communicating-with-azure-monitor"></a>如何阻止日志分析代理与 Azure 监视器通信？
对于直接连接到日志分析的代理，打开控制面板并选择 **"安全&设置**"，**即 Microsoft 监视代理**。 在**Azure 日志分析 （OMS）** 选项卡下，删除列出的所有工作区。 在 System Center Operations Manager 中，从 Log Analytics 托管的计算机列表中删除该计算机。 Operations Manager 会更新该代理的配置以便不再向 Log Analytics 进行报告。 
...
有关详细信息，请参阅[使用 Azure 监视器日志管理使用情况和成本](platform/manage-cost-storage.md)。
...
### <a name="how-much-network-bandwidth-is-used-by-the-microsoft-management-agent-mma-when-sending-data-to-azure-monitor"></a>向 Azure 监视器发送数据时，Microsoft 管理代理 （MMA） 使用了多少网络带宽？
带宽是已发送数据量的一个函数。 数据通过网络发送时会被压缩。


### <a name="how-can-i-be-notified-when-data-collection-from-the-log-analytics-agent-stops"></a>当日志分析代理的数据收集停止时，如何通知我？

当数据收集停止时，可以使用[新建日志警报](platform/alerts-metric.md)中所述的步骤收到通知。 对警报规则使用以下设置：

- **定义警报条件**：将日志分析工作区指定为资源目标。
- **警报条件** 
   - **信号名称**：*自定义日志搜索*
   - **搜索查询**：`Heartbeat | summarize LastCall = max(TimeGenerated) by Computer | where LastCall < ago(15m)`
   - **警报逻辑**：**基于***结果数*，**条件***大于*，**阈值** *0*
   - **评估基于**：**期间 （以分钟）** *30*，**频率 （以分钟）** *10*
- **定义警报详细信息** 
   - **名称**：*数据收集已停止*
   - **严重性**：*警告*

指定现有或新的[操作组](platform/action-groups.md)，以便在日志警报与条件匹配时，如果检测信号丢失超过 15 分钟，则会通知您。
...
### <a name="why-cant-i-see-view-designer"></a>为什么我看不到视图设计器？

视图设计器仅适用于在日志分析工作区中分配具有参与者权限或更高版本的用户。
...
* 将项目插入：
  * Web.config
  * packages.config
* （仅限新项目 - 如果将[应用程序见解添加到现有项目][start]，则必须手动执行此操作。将代码段插入到客户端和服务器代码中，以便使用应用程序见解资源 ID 初始化它们。 例如，在 MVC 应用中，代码插入到母版页视图/共享/\_布局.cshtml 中。
...
* [性能计数器](app/performance-counters.md)- 如果使用[状态监视器](app/monitor-performance-live-website-now.md)、[应用服务的 Azure 监视](app/azure-web-apps.md)[、VM 或虚拟机缩放集的 Azure 监视](app/azure-vm-vmss-apps.md)，或者[应用程序见解收集编写器](app/java-collectd.md)。
...
### <a name="what-happens-to-application-insights-telemetry-when-a-server-or-device-loses-connection-with-azure"></a>当服务器或设备与 Azure 失去连接时，应用程序洞察的遥测会发生什么情况？

我们所有的 SDK，包括 Web SDK，都包括"可靠的传输"或"强大的传输"。 当服务器或设备与 Azure 失去连接时，遥测数据将存储在文件系统（服务器 SDK）或 HTML5 会话存储 （Web SDK）[中。](https://docs.microsoft.com/azure/azure-monitor/app/data-retention-privacy#does-the-sdk-create-temporary-local-storage) SDK 将定期重试发送此遥测数据，直到我们的引入服务认为它"过时"（日志为 48 小时，指标为 30 分钟）。 过时的遥测数据将被删除。 在某些情况下，例如当本地存储已满时，不会发生重试。
...
Azure 警报仅出现在指标上。 创建一个每当事件发生时都跨越值阈值的自定义指标。 然后在该指标上设置警报。 每当指标在任一方向上超过阈值时，您都会收到通知;无论初始值是高还是低，直到第一次交叉，您都得不到通知;总是有几分钟的延迟。
...
> [!NOTE]
> 应用程序 IdProvider 在 v2.6.0 中开始可用。
...
### <a name="what-does-other-processes-represent-under-the-node-view"></a>"其他*进程*"视图下表示什么？

**其他进程**旨在帮助您清楚地了解节点上高资源使用的根本原因。 这使您能够区分容器化流程和非容器化流程的使用情况。
...
> [!NOTE]
> 对于具有 50 个以上节点的大型群集，不建议使用第二个选项，因为它从群集中的每个节点生成 API 服务器调用以执行此扩充。 此选项还会增加收集的每个日志行的数据大小。
...
容器监视解决方案不支持 RBAC，但容器的 Azure 监视器支持它。 在显示这些群集的数据的边栏选项卡上，解决方案详细信息页可能不会显示正确的信息。
...
默认情况下，kube-system 命名空间中的容器的日志收集被禁用。 可以通过在 omsagent 上设置一个环境变量来启用日志收集。 有关详细信息，请参阅容器 GitHub 页的[Azure 监视器](https://github.com/helm/charts/tree/master/incubator/azuremonitor-containers)。 
...
目前，容器的 Azure 监视器不支持多行日志记录，但有可用的解决方法。 你可以将所有服务配置为以 JSON 格式进行写入，然后 Docker/Moby 会在单个行上写入它们。
...
您可能会看到以下错误：**请求中指定的回复 URL 与为应用程序配置的回复 URL 不匹配："<应用程序 ID"。\>** 解决方法可以在"[如何使用容器 Azure 监视器实时查看容器数据](insights/container-insights-livedata-setup.md#configure-ad-integrated-authentication)"一文中找到。 
...
有关使用 Azure、Azure US 政府和 Azure 中国 21Vianet 云的容器化代理所需的代理和防火墙配置信息，请参阅[网络防火墙要求](insights/container-insights-onboard.md#network-firewall-requirements)。
...
如果选择使用基于脚本的方法，这些步骤将[包含在使用 Azure PowerShell 或资源管理器模板的"为 VM 启用 Azure 监视器"模板](insights/vminsights-enable-at-scale-powershell.md)一文中。 

### <a name="what-do-i-do-if-my-vm-is-already-reporting-to-an-existing-workspace"></a>如果 VM 已开始向现有工作区报告，我该怎么做？
如果你已开始从虚拟机收集数据，则可能已将虚拟机配置为向现有的 Log Analytics 工作区报告数据。  只要该工作区在某个受支持区域中，就可以在该现有工作区中启用用于 VM 的 Azure Monitor。  如果您已在使用的工作区不在我们支持的区域之一，此时您将无法将 VM 的 Azure 监视器载板。  我们将力求支持更多的区域。
...
在载入过程中，我们会检查上述每个步骤的状态，并在门户中返回通知状态。 配置工作区和安装代理通常需要 5 到 10 分钟。 在门户中查看监视数据需要额外 5 到 10 分钟。  

如果在启动载入后有消息指出 VM 需要载入，则 VM 最长需要花费 30 分钟来完成该过程。 


### <a name="i-dont-see-some-or-any-data-in-the-performance-charts-for-my-vm"></a>我的 VM 的性能图表中看不到部分或全部数据
我们的性能图表已更新，以使用存储在*InsightsMetrics*表中的数据。  要查看这些图表中的数据，您需要升级才能使用新的 VM 见解解决方案。  有关其他信息，请参阅我们的[GA 常见问题解答](insights/vminsights-ga-release-faq.md)。

如果在磁盘表或某些性能图表中看不到性能数据，则性能计数器可能无法在工作区中配置。 若要解决此问题，请运行以下 [PowerShell 脚本](insights/vminsights-enable-at-scale-powershell.md#enable-with-powershell)。
...
Azure VM 的概述页基于来宾 VM 中的活动的主机度量值显示图表。  Azure VM“概述”中的网络图表仅显示计费的网络流量。  这不包括虚拟网络间流量。  为 VM Azure 监视器显示的数据和图表基于来自来宾 VM 的数据，网络图表显示该 VM 入站和出站的所有 TCP/IP 流量，包括虚拟间网络。
...
在这种情况下，当您打开 VM 并从左侧窗格中选择 **"见解"** 时，即使已在 VM 上安装"**立即试用"** 选项，系统也会提示您。  但是，如果此 VM 未加入到用于 VM 的 Azure Monitor，则不会像通常情况下那样通过选项向你发出提示。 
