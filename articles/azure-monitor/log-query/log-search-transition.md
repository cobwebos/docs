...
若要在 Azure Monitor 日志中保存当前查询，请选择“保存”。 将“另存为”更改为“函数”，并提供一个**函数别名**以创建[函数](functions.md)。

![保存日志查询](media/log-search-transition/save-query-logs.png)

## <a name="saved-searches"></a>保存的搜索
在日志搜索中，可以通过操作栏中的“保存的搜索”项使用已保存的搜索。 在 Azure Monitor 日志中，可以从“查询资源管理器”访问已保存的查询。
