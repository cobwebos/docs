...
在日志搜索中，可以从“列表”视图提取[自定义字段](../platform/custom-fields.md)，该视图中的字段菜单包括“从表中提取字段”操作。__

![在日志搜索中提取字段](media/log-search-transition/extract-fields-log-search.png)

在 Azure Monitor 日志中，可以从表视图提取自定义字段。 若要展开记录，可以单击该记录左侧的箭头，然后单击省略号访问“提取字段”操作。__
...
若要在 Azure Monitor 日志中保存当前查询，请选择“保存”。**** 将“另存为”更改为“函数”，并提供一个**函数别名**以创建[函数](functions.md)。****__ 若要使用[计算机组](../platform/computer-groups.md)的函数别名，请选择“将此查询另存为计算机组”__。
...
在 Azure Monitor 日志中，必须修改查询才能返回这些记录。 展开结果中的一行，然后单击值**+** 的下一个以将其添加到查询。 然后注释掉 **summarize** 命令并再次运行查询。
