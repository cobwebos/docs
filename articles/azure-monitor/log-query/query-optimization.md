...
除了在查询处理节点中所花费的时间以外，Azure Monitor 日志还需要额外的时间来执行以下操作：对用户进行身份验证，并验证它们是否允许访问此数据、查找数据存储、分析查询和分配查询处理节点。 此时间不包括在查询总 CPU 时间内。

### <a name="early-filtering-of-records-prior-of-using-high-cpu-functions"></a>使用较高的 CPU 功能之前提前筛选记录
...
### <a name="avoid-using-evaluated-where-clauses"></a>避免使用计算 where 子句
...
[联接](/azure/kusto/query/joinoperator?pivots=azuremonitor)和[汇总](/azure/kusto/query/summarizeoperator)命令在处理大量数据时可能会导致 CPU 使用率较高。 它们的复杂性直接与将用作`by`汇总或联接属性的列的可能值数（称为*基数*）相关。 有关联接和汇总的说明和优化，请参阅其文档文章和优化提示。
...
### <a name="avoid-full-xml-and-json-parsing-when-string-parsing-works"></a>当字符串分析工作时，请避免使用完整的 XML 和 JSON 分析
完全分析 XML 或 JSON 对象可能会消耗大量 CPU 和内存资源。 在许多情况下，如果只需要一个参数或两个参数，且 XML 或 JSON 对象很简单，则使用[parse 运算符](/azure/kusto/query/parseoperator)或其他[文本分析技术](/azure/azure-monitor/log-query/parse-text)将它们分析为字符串更容易。 当 XML 或 JSON 对象中的记录数增加时，性能提升将更重要。 当记录数达到数百万时，这一点很重要。

例如，下面的查询将返回与上述查询完全相同的结果，而不执行完整的 XML 分析。 请注意，它对 XML 文件结构做出一些假设，例如，FilePath 元素出现在 Get-filehash 之后，并且它们都没有属性。 
...
增加正在处理的数据的另一个因素是使用大量的表。 使用和`union *`命令时`search *`通常会发生这种情况。 这些命令强制系统对工作区中的所有表的数据进行评估和扫描。 在某些情况下，工作区中可能有数百个表。 使用 "搜索" 或任何搜索，尽量避免使用 "搜索" 或任何搜索，而无需将其限定到特定的表。
...
减少数据量的另一种方法是在查询的早期[使用条件。](/azure/kusto/query/whereoperator) Azure 数据资源管理器平台包含一个缓存，可让它知道哪些分区包含与特定 where 条件相关的数据。 例如，如果查询包含`where EventID == 4624` ，它会将查询仅分发给处理包含匹配事件的分区的节点。
...
### <a name="reduce-the-number-of-columns-that-is-retrieved"></a>减少检索到的列数
...
### <a name="make-sure-all-sub-queries-have-timegenerated-filter"></a>请确保所有子查询都具有 TimeGenerated 筛选器
