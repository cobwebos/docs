...
* 如果只有少量用户，则发送自定义事件或指标，使用感兴趣的数据作为指标值或事件名称，并将用户 ID 设置为属性。 若要分析页面视图，可以替换标准的 JavaScript trackPageView 调用。 若要分析服务器端遥测数据，请使用遥测初始值设定项将用户 ID 添加到所有服务器遥测。 然后，你可以筛选并分段指标，并搜索用户 ID。
