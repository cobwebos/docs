...
（如果系统是 Azure 云服务，有[另一种方法可以设置单独 ikey](./cloudservices.md)。）
...
通过[从快速启动窗格获取的脚本](./javascript.md)，iKey 也在应用的网页中使用。 从服务器状态生成它，而不是逐字将其编码到脚本中。 例如，在 ASP.NET 应用中：
...
可以设置“应用程序版本”属性，这样便可以筛选[搜索](./diagnostic-search.md)和[指标资源管理器](../platform/metrics-charts.md)结果。
...
* 在[遥测初始值设定项](./api-custom-events-metrics.md#defaults)中将该行换行，确保以一致的方式设置所有 TelemetryClient 实例。
...
当它具有内部信息时，Application Insights Web 模块自动将**应用程序版本**作为属性添加到每个遥测项。 这样，便可以在执行[诊断搜索](./diagnostic-search.md)或[浏览指标](../platform/metrics-charts.md)时按版本进行筛选。

但请注意，内部版本号只能由 Microsoft 生成引擎生成，而不能由 Visual Studio 中的开发人员生成引擎生成。

### <a name="release-annotations"></a>版本注释
如果使用 Azure DevOps，则可以在每次发布新版本时将[批注标记](./annotations.md)添加到图表中。 

## <a name="next-steps"></a>后续步骤

* [多个角色的共享资源](./app-map.md)
* [创建遥测初始值设定项来区分 A|B 变体](./api-filtering-sampling.md#add-properties)
