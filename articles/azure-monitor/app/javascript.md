...
大多数配置字段的名称都可以默认为 false。 除 `instrumentationKey` 外，所有字段都是可选的。
...
| enableAutoRouteTracking | false | 在单页面应用程序（SPA）中自动跟踪路由更改。 如果为 true，则每个路由更改会将新的 Pageview 发送到 Application Insights。 哈希路由更改（`example.com/foo#bar`）还会记录为新的页面视图。
...
| 扩展 |
|---------------|
| [回应](https://github.com/microsoft/ApplicationInsights-JS/blob/17ef50442f73fd02a758fbd74134933d92607ecf/extensions/applicationinsights-react-js/README.md)|
...
```
此版本提供最少的功能和功能，并根据你的需要，根据需要进行构建。 例如，它不执行任何 autocollection （未捕获的异常、AJAX，等等）。 用于发送某些遥测类型的 Api （例如 `trackTrace`、`trackException` 等）未包含在此版本中，因此你将需要提供自己的包装。 唯一可用的 API 为 `track`。 下面是一个[示例](https://github.com/Azure-Samples/applicationinsights-web-sample1/blob/master/testlightsku.html)。
...
- npm 方案：调用 `downloadAndSetup` 可从 CDN 下载完整的 Applicationinsights.config 脚本并将其初始化为检测密钥：
