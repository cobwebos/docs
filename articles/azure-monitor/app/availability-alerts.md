...
> [!NOTE]
> 通过此体验创建的可用性警报基于状态。 这意味着，当满足警报条件时，当检测到站点不可用时，将生成单个警报。 如果下次评估警报条件时站点仍然关闭，这不会生成新的警报。 因此，如果您的网站关闭了一个小时，并且您设置了电子邮件警报，则只有在网站关闭时才会收到一封电子邮件，并在网站备份时收到后续电子邮件。 您不会收到连续警报，提醒您该网站仍然不可用。
