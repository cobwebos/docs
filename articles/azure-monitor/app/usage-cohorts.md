...
3. 选择“参与用户 -- 按使用天数”。****
    ...
    * **Period**：月份的定义。
    * **使用"最小自定义"，** 用户需要在一个时间段内使用某种东西以计数为参与的次数。

4. 将 **"已使用"至少自定义更改为** **5 天以上**，并在默认值 28 天时保留**期限**。
...
这些筛选器支持更复杂的问题，而通过查询生成器无法表达这样的问题。 例如 _，在过去 28 天中从事工作的人员。在过去60天里，这些人是怎样的？_

## <a name="example-events-cohort"></a>示例：事件队列

还可以创建事件队列。 在本部分，我们将定义一个事件队列和页面视图， 然后了解如何从其他工具使用它们。 此队列可以定义一组被团队视为活动用法的事件，或者定义一组与特定新功能相关的事件。__
...
_我定义了来自某个国家/地区的用户队列。当我比较"用户"工具中的此队列只是在该国家/地区设置筛选器时，我看到的结果不同。为什么？_
