...
* [通过在](#debug-data-from-the-sdk)应用程序 Insights.xml`<SDKLogger />`文件中的根节点下添加元素（在项目中的资源文件夹中），打开日志记录，并检查以 AI 开头的条目：INFO/WARN/ERROR 是否存在任何可疑日志。 
...
* 打开“使用情况和预估成本”页面并检查[采样](../../azure-monitor/app/sampling.md)是否正在进行。 （100% 传输意味着采样未运行。可以将应用程序见解服务设置为仅接受从应用到达的遥测数据的一小部分。 这有助于保持在每月的遥测配额范围内。
...
**我正在查看 Azure[门户](https://portal.azure.com)。地图是否告诉我有关我的应用的内容？**
