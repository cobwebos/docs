...
* 打开“使用情况和预估成本”页面并检查[采样](../../application-insights/app-insights-sampling.md)是否正在进行。 （如果传输百分比为 100%，表示当前未执行采样。）可将 Application Insights 服务设置为只接受来自应用的一部分遥测数据。 这有助于保持在每月的遥测配额范围内。
