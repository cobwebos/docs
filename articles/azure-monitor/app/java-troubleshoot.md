...
* 要[启用日志记录](#debug-data-from-the-sdk)，请在 applicationinsights.config 文件（位于项目的 resources 文件夹）中的根节点下添加一个 `<SDKLogger />` 元素，并检查是否有任何可疑日志以 AI： INFO/警告/ERROR 开头的条目。 
...
### <a name="spring-boot-starter"></a>春季 Boot 入门

若要使用 Application Insights 春季 Boot 入门版的弹簧 Boot 应用启用 SDK 日志记录，请将以下内容添加到 `application.properties` 文件：
