...
ms.locfileid: "54121373"
---
# <a name="application-insights-for-java-web-apps-that-are-already-live"></a>适用于活动 Java Web 应用的 Application Insights

如果有一个 Web 应用程序在 J2EE 服务器上运行，无需更改代码或重新编译项目，就能开始使用 [Application Insights](../../azure-monitor/app/app-insights-overview.md) 来监视该应用程序。 使用此选项可以获取发送到服务器的 HTTP 请求、未经处理的异常和性能计数器的相关信息。

需要订阅 [Microsoft Azure](https://azure.com)。

> [!NOTE]
> 此页上的过程在运行时会将 SDK 添加到 Web 应用。 如果不想要更新或重建源代码，此运行时检测非常有用。 但是，我们建议尽量改为[将 SDK 添加到源代码](java-get-started.md)。 这样可以提供更多选项，包括编写代码来跟踪用户活动。
> 
> 
...
2. 创建新的 Application Insights 资源并将应用程序类型设置为 Java Web 应用程序。
    ...
    ![填写名称，选择 Java Web 应用，并单击“创建”](./media/java-live/02-create.png)

    仅需数秒便可创建资源。

4. 打开新资源并获取其检测密钥。 稍后需要将此密钥粘贴到代码项目中。
   
    ![在新资源概述中，单击“属性”，并复制检测密钥](./media/java-live/03-key.png)
...
* 事件关联是对 HTTP 请求组件的补充。 它将标识符分配到服务器收到的每个请求，并将此标识符添加为遥测的每个项的“Operation.Id”属性。 使用它可以通过在[诊断搜索](../../azure-monitor/app/diagnostic-search.md)中设置筛选器，来关联与每个请求关联的遥测。
...
可能需要[设置例外才能发送传出数据](../../azure-monitor/app/ip-addresses.md)。
...
![示例数据](./media/java-live/5-results.png)
...
[了解有关指标的详细信息。](../../azure-monitor/app/metrics-explorer.md)

## <a name="next-steps"></a>后续步骤
* [将遥测添加到网页](javascript.md)，用于监视页面视图和用户指标。
* [设置 Web 测试](../../azure-monitor/app/monitor-web-app-availability.md)，确保应用程序处于活动状态且能够做出响应。
* [捕获日志跟踪](java-trace-logs.md)
* [搜索事件和日志](../../azure-monitor/app/diagnostic-search.md)帮助诊断问题。
