...
连续导出不**支持**以下 Azure 存储功能/配置：

* 在 Azure Blob 存储中结合使用[VNET/Azure 存储防火墙](https://docs.microsoft.com/azure/storage/common/storage-network-security)。
...
有关更大的代码示例，请参阅[使用辅助角色][exportasa]。

## <a name="delete"></a>删除旧数据
如果需要，你需要负责管理存储容量并删除旧数据。
...
* *存储中应会出现多少个 Blob？*
  ...
  * 此外，对于高流量应用程序，将分配额外的分区单元。 在这种情况下，每个单位每分钟创建一个 blob。
