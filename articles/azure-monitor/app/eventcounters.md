...
ms.locfileid: "82187089"
---
# <a name="eventcounters-introduction"></a>EventCounters 简介

`EventCounter`用于发布和使用计数器或统计信息的 .NET/.NET Core 机制。 [本](https://github.com/dotnet/runtime/blob/master/src/libraries/System.Diagnostics.Tracing/documentation/EventCounterTutorial.md)文档概述了有关如何`EventCounters`发布和使用它们的示例。 所有 OS 平台（Windows、Linux 和 macOS）都支持 EventCounters。 它可以被视为只在 Windows 系统中受支持的[PerformanceCounters](https://docs.microsoft.com/dotnet/api/system.diagnostics.performancecounter)的跨平台等效项。
...
下面的示例演示如何添加/删除计数器。 使用`AddApplicationInsightsTelemetry()`或`AddApplicationInsightsWorkerService()`启用 Application Insights 遥测收集后`ConfigureServices` ，将在应用程序的方法中完成此自定义。 下面是 ASP.NET Core 应用程序的示例代码。 对于其他类型的应用程序，请参阅[本](worker-service.md#configuring-or-removing-default-telemetrymodules)文档。
...
与其他遥测一样， **customMetrics**还具有一个`cloud_RoleInstance`列，该列指示运行应用的主机服务器实例的标识。 上面的查询显示每个实例的计数器值，并可用于比较不同服务器实例的性能。
