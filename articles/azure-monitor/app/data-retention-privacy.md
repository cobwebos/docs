...
* 可以编写发送其他自定义遥测数据的代码，帮助进行诊断与监视使用情况。 （此扩展性是 Application Insights 的一项强大功能。）这可能会错误地编写此代码，使其包含个人和其他敏感数据。 如果你的应用程序处理此类数据，则应对你编写的所有代码应用全面的审核过程。
...
*   查看收集的数据，因为这可能包括某些情况下允许但不是其他情况的数据。  设备名称就是一个很好的例子。 服务器中的设备名称不会对隐私产生任何影响，但也很有用，但手机或便携式计算机的设备名称可能会受到隐私影响，并且不太有用。 最初开发的 SDK 是目标服务器，将默认收集设备名称，这可能需要在正常事件和异常中被覆盖。
...
* [Web 服务器遥测数据](../../azure-monitor/app/asp-net.md) - HTTP 请求。  URI、处理请求花费的时间、响应代码、客户端 IP 地址。 `Session id` 列中的一个值匹配。
...
数据存储在 Microsoft Azure 服务器中。 对于 Azure 门户中的帐户， [Azure 安全性、隐私和合规性文档](https://go.microsoft.com/fwlink/?linkid=392408)中介绍了帐户限制。
...
利用本地存储的遥测通道在 TEMP 或 APPDATA 目录中创建临时文件，这些目录仅限于运行应用程序的特定帐户。 当终结点暂时不可用或达到限制值时，可能会发生这种情况。 解决此问题后，遥测通道便会恢复发送所有新数据和暂留数据。

不会在本地加密此永久性数据。 如果这是一个问题，请查看数据并限制私有数据的收集。 （有关详细信息，请参阅[如何导出和删除私有数据](https://docs.microsoft.com/azure/application-insights/app-insights-customer-data#how-to-export-and-delete-private-data)。）

如果客户需要将此目录配置为具有特定的安全要求，则可以针对每个框架进行配置。 请确保运行应用程序的进程对此目录拥有写入权限，并确保此目录受保护，以免遥测数据遭用户意外读取。
...
下面的代码片段展示了如何在 `ServerTelemetryChannel.StorageFolder` 类的 `ConfigureServices()` 方法中设置 `Startup.cs`：
...
你可以通过在所使用的导出程序的构造函数中传入 `storage_path` 参数来更改存储文件的位置。
...
[PCI 安全标准委员会](https://www.pcisecuritystandards.org/)已将[截止时间设置为2018年6月30日，](https://www.pcisecuritystandards.org/pdfs/PCI_SSC_Migrating_from_SSL_and_Early_TLS_Resource_Guide.pdf)以禁用旧版本的 TLS/SSL 并升级到更安全的协议。 在 Azure 放弃旧版支持后，如果应用程序/客户端无法通过最低版本 TLS 1.2 进行通信，则你无法将数据发送到 Application Insights。 测试和验证应用程序对 TLS 的支持的方法根据操作系统/平台以及应用程序使用的语言/框架而异。

我们不建议将应用程序显式设置为仅使用 TLS 1.2，除非需要这样做，因为这可能会中断平台级别的安全功能，使你能够在可用时自动检测并利用新的更安全的协议，如TLS 1.3。 我们建议针对应用程序代码执行全面的审核，以检查特定 TLS/SSL 版本的硬编码。
...
|Node.js |  受支持，在 v10.5.0 中可能需要配置。 | 对于任何特定于应用程序的配置，请使用[官方 NODE.JS TLS/SSL 文档](https://nodejs.org/api/tls.html)。 |
...
若要运行初步测试来查看 Linux 系统是否可以通过 TLS 1.2 进行通信，请打开终端并运行：
...
| --- | --- |
| **属性** |**任何数据 - 由代码确定** |
| DeviceContext |`Id`，IP，区域设置，设备型号，网络，网络类型，OEM 名称，屏幕分辨率，角色实例，角色名称，设备类型 |
