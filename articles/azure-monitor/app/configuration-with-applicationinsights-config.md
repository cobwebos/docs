...
从 IIS 安装[收集系统性能计数器](../../azure-monitor/app/performance-counters.md)，如 CPU、内存和网络负载。 可以指定要收集哪些计数器，包括自己设置的性能计数器。
...
* [应用程序见解窗口服务器](https://www.nuget.org/packages/Microsoft.ApplicationInsights.WindowsServer/)NuGet 包。
...
Microsoft.ApplicationInsights 包提供 SDK 的[核心 API](https://msdn.microsoft.com/library/mt420197.aspx)。 其他遥测模块也使用它[来定义自己的遥测](../../azure-monitor/app/api-custom-events-metrics.md)数据。
...
[遥测通道](telemetry-channels.md)管理遥测的缓冲和传输到应用程序见解服务。
...
* `WebTestTelemetryInitializer`为来自[可用性测试](../../azure-monitor/app/monitor-web-app-availability.md)的 HTTP 请求设置用户 ID、会话 ID 和合成源属性。
  `<Filters>` 设置请求的标识属性。
...
您可以[编写自己的遥测处理器](../../azure-monitor/app/api-filtering-sampling.md#filtering)。
...
还有一个标准[采样遥测处理器](../../azure-monitor/app/api-filtering-sampling.md)（从 2.0.1）：
