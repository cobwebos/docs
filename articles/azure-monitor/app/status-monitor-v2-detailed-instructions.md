...
- 说明：默认情况下，禁用正在运行的 PowerShell 脚本。 我们建议仅允许在当前范围运行 RemoteSigned 脚本。
...
- 说明：1.0.1.0 的基本版本（v）无法解压缩 nupkg 文件。 使用 .zip 扩展名重命名文件。
...
Application Insights SDK 需要将应用的遥测数据发送给 Microsoft。 建议在 web.config 文件中配置应用的代理设置。 有关详细信息，请参阅[APPLICATION INSIGHTS FAQ： Proxy passthrough](https://docs.microsoft.com/azure/azure-monitor/app/troubleshoot-faq#proxy-passthrough)。
