...
1. `MaxTransmissionSenderCapacity`：将同时发送到 Application Insights 的 `Transmission` 实例的最大数量。 默认值为10。 此设置可配置为更大的数字，如果生成的遥测数据量较大，则建议使用此设置。 大容量通常在负载测试过程中或在关闭采样时出现。
...
1. 遥测在网络问题的长时间内丢失。 在网络中断期间或在 Application Insights 后端出现问题时，遥测数据将存储到本地磁盘。 但是，超过24小时的项目将被丢弃。
