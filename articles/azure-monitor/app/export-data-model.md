...
| context.device |对象 (object) |客户端设备 |
...
| context.operation.id |字符串 |具有相同 `operation id` 的项在门户中显示为“相关项”。 通常是 `request id`。 |
...
| internal.data.id |字符串 | 将项引入到 Application Insights 时分配的 `Unique id` |
