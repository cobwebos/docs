---
title: 用于 .NET 应用的 Azure 应用程序见解快照调试器
...
发生异常时，可自动从实时 Web 应用程序收集调试快照。 快照显示发生异常时源代码和变量的状态。 [Azure 应用程序见解](../../azure-monitor/app/app-insights-overview.md)中的快照调试器监视 Web 应用中的异常遥测。 它可收集常出现的异常的调试快照，为诊断生产中的问题提供所需信息。 在应用程序中包括[快照收集器 NuGet 包](https://www.nuget.org/packages/Microsoft.ApplicationInsights.SnapshotCollector)，并在[应用程序 Insights.config](../../azure-monitor/app/configuration-with-applicationinsights-config.md)中选择配置集合参数。快照显示在应用程序见解门户中的[异常](../../azure-monitor/app/asp-net-exceptions.md)上。

可在门户中查看调试快照，查看调用堆栈并检查每个调用堆栈帧中的变量。 若要获取更强大的调试体验与源代码，请在 Visual Studio 2019 Enterprise 中打开快照。 在 Visual Studio 中，您还可以[将 Snappoints 设置为交互式拍摄快照](https://aka.ms/snappoint)，而无需等待异常。
...
* 在 Windows 服务器 2012 R2 或更高版本上运行的[Azure 服务交换矩阵服务](snapshot-debugger-vm.md?toc=/azure/azure-monitor/toc.json)
* 运行 Windows Server 2012 R2 或更高版本的 [Azure 虚拟机和虚拟机规模集](snapshot-debugger-vm.md?toc=/azure/azure-monitor/toc.json)
* 运行 Windows Server 2012 R2 或更高版本或 Windows 8.1 或更高版本的[本地虚拟或物理计算机](snapshot-debugger-vm.md?toc=/azure/azure-monitor/toc.json)
