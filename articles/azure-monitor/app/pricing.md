...
若要了解有关数据卷源的详细信息，可以 `_BilledSize` 使用引入属性，该属性存在于每个事件上。

例如，若要查看在过去30天内哪些操作生成的数据量最多，可以将所有依赖项事件 `_BilledSize` 求和：
...
查看 Application Insights 使用情况和估计成本，了解数据引入趋势以及要定义的每日容量上限。 应谨慎考虑，因为在达到限制后，你将无法监视资源。
