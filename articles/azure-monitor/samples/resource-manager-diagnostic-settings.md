...
若要创建诊断设置，请将 <resource namespace>/providers/diagnosticSettings 类型的资源添加到模板。 本文为两种不同资源类型提供了示例，但相同模式可以应用于其他资源类型。 每种资源类型允许的日志和指标的收集将有所不同。

[!INCLUDE [azure-monitor-samples](../../../includes/azure-monitor-resource-manager-samples.md)]


## <a name="diagnostic-setting-for-azure-key-vault"></a>Azure 密钥保管库的诊断设置 
下面的示例将两个日志查询添加到一个 Azure 密钥保管库。
...
下面的示例将两个日志查询添加到一个 Azure SQL 数据库。
### <a name="template-file"></a>模板文件
