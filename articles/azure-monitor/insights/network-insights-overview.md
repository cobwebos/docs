...
description: 简要概述了针对网络的 Azure Monitor，为所有部署的网络资源提供了运行状况和指标的综合视图，无需任何配置。
ms.subservice: ''
ms.topic: conceptual
author: bwren
ms.author: bwren
...
应用程序网关的“依赖项”视图提供了前端 IP 如何连接到侦听器、规则和后端池的简化视图。 连接边缘采用颜色编码，并根据后端池运行状况提供其他详细信息。 此视图还提供了应用程序网关指标和指标的详细视图，适用于所有相关的后端池，如虚拟机规模集和虚拟机实例。

![“依赖项”视图](media/network-insights-overview/dependency-view.png)

通过依赖项图可轻松导航到配置设置。 右键单击后端池可以访问其他功能。 例如，如果后端池是虚拟机，则可以直接访问虚拟机 insights 和网络观察程序连接，以排除故障并识别连接问题。
