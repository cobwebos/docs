...
- **通过管理组发送 Operations Manager 代理数据** - 如果代理[连接到 SCOM 管理组](../../log-analytics/log-analytics-om-agents.md)，则数据将从管理服务器发送到 Log Analytics。 在这种情况下，代理不需要直接连接到 Log Analytics。 如果未选中此框，则数据将从代理直接发送到 Log Analytics，即使代理已连接到一个 SCOM 管理组。 它还需要能够通过 [Log Analytics 网关](../../azure-monitor/platform/gateway.md)与 Log Analytics 进行通信。
