...
用于密钥保管库的 Azure Monitor 结合了日志和指标来提供全局监视解决方案。 所有用户都可以访问基于指标的监视数据，但包含基于日志的可视化效果可能要求用户[对其 Azure 密钥保管库启用日志记录](https://docs.microsoft.com/azure/key-vault/key-vault-logging)。
...
4. 有关如何为密钥保管库服务启用诊断日志的其他帮助，请参阅[完整指南](https://docs.microsoft.com/azure/key-vault/key-vault-logging)。
...
为了更好地理解每个状态代码的含义，建议通读有关 [Azure 密钥保管库状态和响应代码](https://docs.microsoft.com/azure/key-vault/authentication-requests-and-responses)的文档。
...
>[!NOTE]
> 请注意，用户必须启用了诊断设置才能查看此工作簿。 要详细了解如何启用诊断设置，请阅读有关 [Azure 密钥保管库日志记录](https://docs.microsoft.com/azure/key-vault/general/logging)的详细信息。
...
要帮助解决使用用于密钥保管库的 Azure Monitor（预览版）标识的任何密钥保管库相关问题，请参阅 [Azure 密钥保管库文档](https://docs.microsoft.com/azure/key-vault/)。
...
查看[使用 Azure Monitor 工作簿创建交互式报表](https://docs.microsoft.com/azure/azure-monitor/platform/workbooks-overview)，了解工作簿旨在支持的方案、创作新报表和自定义现有报表的方式，以及更多信息。
