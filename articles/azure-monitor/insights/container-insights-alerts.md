---
title: 使用容器 Azure Monitor 创建性能警报 |Microsoft Docs
description: 本文介绍如何使用容器 Azure Monitor 根据内存和 CPU 使用率的日志查询来创建自定义警报。
...
```
>[!IMPORTANT]
>以下查询使用占位符值 \<your-cluster > 和 \<your-name > 来表示群集和控制器。 在设置警报时，将它们替换为特定于您的环境的值。
...
按照以下步骤，使用之前提供的一个日志搜索规则在 Azure Monitor 中创建日志警报。  
...
5. 在 "**日志**" 页上，选择 " **+ 新建警报规则**"。
6. 在 "**条件**" 部分中，选择 "**只要自定义日志搜索 \<logic 未定义 >** 预定义的自定义日志条件"。 **自定义日志搜索**信号类型是自动选择的，因为我们是直接从 "Azure Monitor 日志" 页创建警报规则。  
