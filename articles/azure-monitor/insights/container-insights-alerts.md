...
在 Azure Monitor 中，遵循以下步骤使用前面提供的日志搜索规则之一创建日志警报。 要使用 ARM 模板创建，请参阅[使用 Azure 资源模板创建示例日志警报](../platform/alerts-log.md#sample-log-alert-creation-using-azure-resource-template)。
...
2. 在 Azure 门户中，搜索并选择**日志分析工作区**。
3. 在日志分析工作区列表中，选择支持容器 Azure 监视器的工作区。 
4. 在左侧的窗格中，选择 **"日志"** 以打开 Azure 监视器日志页。 使用此页编写并执行 Azure Log Analytics 查询。
5. 在 **"日志"** 页上，将前面提供的[查询](#resource-utilization-log-search-queries)之一粘贴到 **"搜索"查询**字段中，然后选择 **"运行"** 以验证结果。 如果不执行此步骤，则无法选择 **_New 警报**选项。
6. 选择 **"新建警报**"以创建日志警报。
...
9. 按如下所述配置警报：
    ...
    1. 从 **"基于**下拉列表"中，选择 **"指标度量**"。 指标度量将为查询中其值超过指定阈值的每个对象创建一个警报。
...
- 要了解有关 Azure 监视器以及如何监视库伯奈斯群集的其他方面，请参阅[查看库伯奈斯群集性能](container-insights-analyze.md)和[查看库伯奈斯群集运行状况](container-insights-health.md)。
