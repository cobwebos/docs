...
本文将帮助您了解 Azure 监视存储（预览）提供的经验，以便获得有关存储帐户的运行状况和性能的可操作知识，并能够专注于热点并诊断延迟、限制和可用性问题。
...
* **可用性**打开**可用性**工作簿。 它显示 Azure 存储服务的当前运行状况状态、显示由存储帐户中定义的数据服务分类的每个对象的可用运行状况状态，显示趋势线表示所选时间范围，以及帐户中每个数据服务的可用性趋势图。  
