...
description: 管理解决方案通常包括保存的日志查询，以分析解决方案收集的数据。 本文介绍如何在资源管理器模板中定义日志分析保存的搜索。
ms.subservice: ''
ms.topic: conceptual
author: bwren
ms.author: bwren
...
> [!NOTE]
> 本文中的示例使用管理解决方案需要或通用的参数和变量，并在["设计"中描述并在 Azure 中构建管理解决方案](solutions-creating.md)
...
工作区的名称包含在每个 Log Analytics 资源的名称中。 这在使用**工作区**参数的解决方案中完成，如以下"已保存搜索"资源示例。
...
存在两类由 **Type** 属性指定的操作资源。 计划需要一个**警报**操作，该操作定义警报规则的详细信息以及创建警报时执行的操作。 操作资源的类型为 `Microsoft.OperationalInsights/workspaces/savedSearches/schedules/actions`。
...
| `severity` | 是 | 警报记录的严重等级包括以下值：<br><br> **严重**<br>**warning**<br>**信息**
