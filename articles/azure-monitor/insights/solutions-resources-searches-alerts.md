...
> [!NOTE]
> 本文中的示例使用管理解决方案需要或通用的参数和变量，在[Azure 中设计和构建管理解决方案](solutions-creating.md)中进行了介绍。
...
- **已保存搜索。** 定义运行的日志搜索。 多个警报规则可共享一个保存的搜索。
- **编制.** 定义运行日志搜索的频率。 每个警报规则有且仅有一个计划。
...
存在两类由 **Type** 属性指定的操作资源。 计划需要一个**警报**操作，该操作可定义警报规则的详细信息以及创建警报时采取的操作。 操作资源的类型为 `Microsoft.OperationalInsights/workspaces/savedSearches/schedules/actions`。
...
| `severity` | 是 | 警报记录的严重等级包括以下值：<br><br> **来说**<br>**出现**<br>**条**
