...
与 Azure Monitor 日志集成可提供强大的聚合和筛选功能，使用于 VM 的 Azure Monitor 能够分析一段时间内的数据趋势。 你可以直接从虚拟机的单个 VM 中查看此数据，也可以使用 Azure Monitor 提供 Vm 的聚合视图，其中视图支持 Azure 资源上下文或工作区上下文模式。 有关详细信息，请参阅[访问模式概述](../platform/design-logs-deployment.md#access-mode)。
...
日志大小因性能计数器的字符串长度而异，并且可能会随分配给 VM 的逻辑磁盘和网络适配器的数量而增大。 如果你已有一个工作区并且正在收集这些计数器，则不会重复收费。 如果已在使用服务映射，则将看到的唯一更改是发送给 Azure Monitor 的其他连接数据。
