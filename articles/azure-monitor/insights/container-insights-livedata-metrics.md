...
此性能图表映射到等效的，它`kubectl get nodes`对**状态**列进行调用并将其映射到按状态类型分组的图表。
...
此性能图表映射到等效的调用`kubectl get pods –all-namespaces` ，并将 "**状态**" 列映射为按状态类型分组的图表。
