...
在开始前，请务必查看[先决条件](vminsights-enable-overview.md)，并验证订阅和资源是否满足相关要求。 查看 [Log Analytics Linux 和 Windows 代理](../../log-analytics/log-analytics-agent-overview.md)的要求与部署方法。
...
3. 计算机是否将日志和性能数据发送到 Azure Monitor 日志？ 对计算机执行以下查询: 
    ...
    它是否返回了一个或多个结果？ 是否为最新数据？ 如果是这样, 则 Log Analytics 代理正在正常运行并与服务进行通信。 如果不是，请检查服务器上的代理：[适用于 Windows 的 Log Analytics 代理故障排除](../platform/agent-windows-troubleshoot.md)或[适用于 Linux 的 Log Analytics 代理故障排除](../platform/agent-linux-troubleshoot.md)。
