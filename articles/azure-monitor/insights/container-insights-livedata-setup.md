...
启用 Kubernetes RBAC 授权时，将使用两个用户： **clusterUser**和**CLUSTERADMIN**访问 Kubernetes API。 这类似于无需管理选项即可运行 `az aks get-credentials -n {cluster_name} -g {rg_name}`。 这意味着需要向**clusterUser**授予对 Kubernetes API 中终结点的访问权限。
