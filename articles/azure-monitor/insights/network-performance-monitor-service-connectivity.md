...
要打开网络性能监视器的配置，请打开[网络性能监视器解决方案](network-performance-monitor.md)并选择"**配置**"。
...
3. 选择测试类型：<br>
    ...
    * 例如：要创建 Blob 存储帐户的 Web 测试，请选择**Web**并将目标输入为*存储帐户*.blob.core.windows.net。 同样，可以使用[此链接](https://docs.microsoft.com/azure/storage/common/storage-account-overview#storage-account-endpoints)为其他表存储、队列存储和 Azure 文件存储创建测试。
