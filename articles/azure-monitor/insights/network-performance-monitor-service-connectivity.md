---
title: 网络性能监视器解决方案服务连接-Azure Log Analytics
...
1. 选择“服务连接监视器”选项卡。
2. 选择“添加测试”并输入测试名称和说明。 每个工作区最多可以创建450个测试。 
3. 选择测试类型：<br>
    ...
    * 例如：若要创建对 blob 存储帐户的 web 测试，请选择 " **web** "，并将 target 输入为*yourstorageaccount*. blob.core.windows.net。 同样，你可以使用[此链接](https://docs.microsoft.com/azure/storage/common/storage-account-overview#storage-account-endpoints)为其他表存储、队列存储和 Azure 文件创建测试。
...
8. 选择要从中监视服务网络连接的节点。 确保每个测试添加的代理数小于150。 任何代理都可以测试最多150终结点/代理。
...
对于美国政府弗吉尼亚州，只有 DOD Url 内置 NPM。 使用 GCC Url 的客户需要创建自定义测试，并分别添加每个 URL。
