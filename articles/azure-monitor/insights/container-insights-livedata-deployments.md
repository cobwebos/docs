...
对于容器 Azure Monitor，"查看部署（预览版）" 功能通过公开`kubeclt get deployments`和`kubectl describe deployment {your deployment}`命令实时模拟对 Kubernetes 部署对象的直接访问。 
...
若要查看与等效的部署的详细信息， `kubectl describe deployment`请执行以下步骤。
...
从列表中选择部署时，会自动在页面右侧显示一个 "属性" 窗格。 其中显示了在运行命令`kubectl describe deployment {deploymentName}`时将查看的所选部署的相关信息。 您可能已注意到说明信息缺少一些详细信息。 最值得注意的是，缺少**模板**。 选择 "**原始**" 选项卡可以导航到未分析的描述详细信息。  
