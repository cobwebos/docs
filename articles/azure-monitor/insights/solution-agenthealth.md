---
title: Azure 监视器中的代理运行状况解决方案 |微软文档
...
| `IsGatewayInstalled` | 如果安装了日志分析网关，则值*为 true，* 否则值为*false*。|
| `ComputerIP` | 计算机的公共 IP 地址。 在 Azure VM 上，如果公共 IP 可用，则将显示公共 IP。 对于使用专用 IP 的 VM，这将显示 Azure SNAT 地址（而不是专用 IP 地址）。 |
