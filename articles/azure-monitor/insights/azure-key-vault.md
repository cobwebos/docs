...
description: 你可以使用 Azure Monitor 中的 Azure Key Vault 解决方案查看 Azure Key Vault 日志。
...
你可以使用 Azure Monitor 中的 Azure Key Vault 解决方案查看 Azure Key Vault AuditEvent 日志。
...
1. 使用[从解决方案库中添加 Azure Monitor 解决方案](../../azure-monitor/insights/solutions.md)中所述的过程，将 Azure Key Vault 解决方案添加到 Log Analytics 工作区。
...
[安装解决方案](https://azuremarketplace.microsoft.com/en-usrketplace/marketplace/apps/Microsoft.KeyVaultAnalyticsOMS?tab=Overview)后，可通过单击 Azure Monitor**概述**"页中的**Key Vault 分析**磁贴来查看 Key Vault 数据。 通过点击“见解”部分下的“更多”，从“Azure Monitor”菜单打开此页面。 
...
+ 日志直接写入 Log Analytics 的工作区，而无需使用存储帐户
...
2. 使用[从解决方案库添加 Azure Monitor 解决方案](../../azure-monitor/insights/solutions.md)中所述的过程，启用 Azure Key Vault 解决方案
...
* 使用[Azure Monitor 中的日志查询](../../azure-monitor/log-query/log-query-overview.md)来查看详细的 Azure Key Vault 数据。
