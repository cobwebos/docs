...
* 若要链接，Log Analytics 工作区和自动化帐户必须位于同一订阅中，但可以位于部署到同一区域的不同资源组中。 美国东部区域的工作区以及美国东部 2 区的自动化帐户除外。
...
1. 如果**工作区**已链接到自动化帐户，则此页将列出它链接到的工作区。 如果你选择列出的工作区的名称，你会被重定向到该工作区的 "概述" 页。
