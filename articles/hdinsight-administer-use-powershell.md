<properties 
	pageTitle="使用 Azure PowerShell 管理 HDInsight 中的 Hadoop 群集 | Azure" 
...
- Azure 订阅。Azure 是基于订阅的平台。HDInsight 的 Azure PowerShell cmdlet 使用订阅执行任务。有关获取订阅的详细信息，请参阅[购买选项][azure-purchase-options]和[免费试用][azure-trial]。

- 配备 Azure PowerShell 的工作站。有关说明，请参阅[安装和配置 Azure PowerShell][powershell-install-configure]。
...
有关使用 Azure 门户创建 Azure 存储帐户的信息，请参阅[创建、管理或删除存储帐户](/documentation/articles/storage-create-storage-account/)。
...
有关开发和运行 MapReduce 作业的详细信息，请参阅将 [MapReduce 与 HDInsight 配合使用][hdinsight-use-mapreduce]。
...
请参阅[提交 MapReduce 作业](#mapreduce) 部分中的表格。
