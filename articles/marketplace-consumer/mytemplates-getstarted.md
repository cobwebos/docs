<properties
   pageTitle="专用模板入门 | Microsoft Azure"
...
可以将 [Azure 门户](https://portal.azure.com)中新的**模板**功能与作为 [Azure 应用商店](https://azure.microsoft.com/marketplace/)扩展的 **Microsoft.Gallery** 资源提供程序一起使用，使用户能够从个人库创建、管理和部署专用模板。
...
### 方法 1：从正在运行的资源组新建模板资源

1. 导航到 Azure 门户中的现有资源组。在“设置”中选择“导出模板”。
2. 导出 Resource Manager 模板后，使用“保存模板”按钮将其保存到“模板”存储库。可在[此处](../resource-manager-export-template.md)找到有关导出模板的完整详细信息。<br /><br />![导出资源组](media/rg-export-portal1.PNG)<br />

3. 选择“保存到模板”命令按钮。<br /><br />
...
5. 单击“保存”。
    ...
    > [AZURE.NOTE] 当导出的 Resource Manager 模板有错误时，“导出模板”边栏选项卡将显示通知，但用户仍能够将此 Resource Manager 模板保存到“模板”。在重新部署导出的 Resource Manager 模板之前，请确保检查并修复任何 Resource Manager 模板问题。

### B.方法 2：通过浏览新增模板资源

还可以使用“浏览”>“模板”中的“+添加”命令按钮从头开始新增一个**模板**。需要提供名称、说明和 Resource Manager 模板 JSON。

![添加模板](media/add-template-portal1.PNG) <br />

> [AZURE.NOTE] Microsoft.Gallery 是基于租户的 Azure 资源提供程序。模板资源与创建它的用户相关联，而不与任何特定订阅相关联。仅在部署模板时，才需要选择订阅。

## 查看模板资源

在“浏览”>“模板”中可以看到用户可用的所有**模板**。这包括用户已创建的**模板**以及已与用户共享具有不同级别权限的模板。在下面的[访问控制](#access-control-for-a-tenant-resource-provider)部分中提供更多详细信息。
...
> [AZURE.NOTE] 只能与同一 Azure Active Directory 租户中的用户和组共享模板。如果与不在同一租户中的电子邮件地址共享模板，将发送邀请请求该用户以来宾身份加入该租户。
