...
在 Azure CLI 上，缓存状态报表的末尾包含新的软件信息。 （使用[az hpc-cache show](/cli/azure/ext/hpc-cache/hpc-cache#ext-hpc-cache-az-hpc-cache-show)检查。）在消息中查找字符串 "upgradeStatus"。

如果有更新，请使用[az hpc-缓存升级固件](/cli/azure/ext/hpc-cache/hpc-cache#ext-hpc-cache-az-hpc-cache-upgrade-firmware)来应用更新。

如果没有可用的更新，则此操作不起作用。

此示例显示缓存状态（无可用更新）和升级固件命令的结果。
