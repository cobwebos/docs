...
创建缓存后，NFS 客户端可以使用简单`mount`的命令来访问它。 命令将 Azure HPC 缓存上的特定存储目标路径连接到客户端计算机上的本地目录。
...
在客户端上使用该命令之前，请确保客户端满足先决条件，并且具有使用 NFS `mount`命令所需的软件（如上文在[准备客户端](#prepare-clients)中所述）。
...
1. 选择要用于客户端的**虚拟命名空间路径**。 这些路径链接到后端存储系统上的导出。
   ...
   可以在 "存储目标门户" 页上查看和更改虚拟命名空间路径。 若要了解如何操作，请阅读[添加存储目标](hpc-cache-add-storage.md)。
...
1. 步骤3中的 "**装载命令**" 字段自动使用自定义装载命令填充，该命令使用在前面的字段中设置的装入地址、虚拟命名空间路径和客户端路径。
   ...
   !["命名空间路径" 字段的屏幕截图，其中打开了选择器](media/mount-command-copy.png)
...
此命令成功之后，存储导出的内容将显示在客户端上的``hpccache``目录中。
...
如果要在不使用 "**装载说明**" 页的情况下创建装载命令，可以在 "缓存**概述**" 页上找到 "装入地址"，在 "**存储目标**" 页上找到虚拟命名空间路径。
