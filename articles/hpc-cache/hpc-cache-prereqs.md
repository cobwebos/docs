...
* 如果使用多个缓存实例，则每个实例都需要其自己的子网。

最佳做法是为缓存创建新的子网。 创建缓存时，可以创建新的虚拟网络和子网。

### <a name="dns-access"></a>DNS 访问

Azure HPC 缓存需要 DNS 来访问其虚拟网络外部的资源。 根据所使用的资源，可能需要设置自定义 DNS 服务器并配置该服务器与 Azure DNS 服务器之间的转发： 
...
缓存支持 Azure Blob 容器或 NFS 硬件存储导出。 你可以在创建缓存时定义存储目标，但也可以在以后添加。 
...
如果要将 Azure Blob 存储用于 Azure HPC 缓存，需要一个兼容的存储帐户，以及一个使用 Azure HPC 缓存格式化数据填充的空 Blob 容器或容器，如[将数据移动到 Azure Blob 存储](hpc-cache-ingest.md)中所述。
