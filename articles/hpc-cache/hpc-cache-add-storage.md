...
> [!NOTE]
> 在创建 NFS 存储目标之前，请确保可从 Azure HPC 缓存访问存储系统并满足权限要求。 如果缓存无法访问存储系统，则存储目标创建将会失败。 有关详细信息，请阅读[NFS 存储要求](hpc-cache-prereqs.md#nfs-storage-requirements)和[排查 NAS 配置和 NFS 存储目标问题](troubleshoot-nas.md)。
