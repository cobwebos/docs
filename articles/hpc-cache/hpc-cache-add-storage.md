...
*存储目标是*通过 Azure HPC 缓存访问的文件的后端存储。 您可以添加 NFS 存储（如本地硬件系统），或在 Azure Blob 中存储数据。
...
1. 在 **"选择"** 字段中，搜索"hpc"。  此字符串应匹配一个服务主体，称为"HPC 缓存资源提供程序"。 单击该主体以将其选中。
   ...
   > [!NOTE]
   > 如果搜索"hpc"不起作用，请尝试改用字符串"存储缓存"。 参与预览的用户（在 GA 之前）可能需要为服务主体使用较旧的名称。
...
![定义 NFS 目标的添加存储目标页的屏幕截图](media/add-nfs-target.png)

> [!NOTE]
> 在创建 NFS 存储目标之前，请确保存储系统可从 Azure HPC 缓存访问并满足权限要求。 如果缓存无法访问存储系统，则存储目标创建将失败。 有关详细信息，请阅读[NFS 存储要求](hpc-cache-prereqs.md#nfs-storage-requirements)和[故障排除 NAS 配置和 NFS 存储目标问题](troubleshoot-nas.md)。
...
当您创建指向 NFS 存储系统的存储目标时，您需要为该目标选择使用模型。 此模型确定数据的缓存方式。
...
* **大于 15% 写入**- 此选项可加快读取和写入性能。 使用此选项时，所有客户端都必须通过 Azure HPC 缓存访问文件，而不是直接安装后端存储。 缓存的文件将具有未存储在后端的最新更改。
  ...
  在此使用模型中，不会对照后端存储上的文件检查缓存中的文件。 假定文件的缓存版本更最新。 缓存中修改后的文件在缓存中已写入一小时后写入后端存储系统，无需其他更改。
