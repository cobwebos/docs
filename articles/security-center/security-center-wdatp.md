...
- **自动载入**：为载入 Azure 安全中心的 windows 服务器自动启用 MICROSOFT Defender ATP 传感器（运行 Windows Server 2019 的那些服务除外）。
...
2. 如果看不到所有工作区，原因可能是缺少权限，请确保工作区设置为 "Azure 安全标准层"。 有关详细信息，请参阅[升级到安全中心的标准层以增强安全性](security-center-pricing.md)。
    
3. 选择 "**添加服务器**"，查看有关如何安装 Log Analytics 代理的说明。 

4. 载入后，可以在 "**计算和应用**" 下监视计算机。
...
1. 如果此命令成功，你将在 Azure 安全中心仪表板和 Microsoft Defender ATP 门户上看到新警报。 此警报可能要在几分钟之后才显示。

1. 若要查看安全中心的警报，请参阅**安全警报** > **可疑 PowerShell 命令行**。

1. 从调查窗口中，选择要前往 Microsoft Defender ATP 门户的链接。
