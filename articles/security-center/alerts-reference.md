...
description: 本文列出了 Azure 安全中心的 Azure Defender 仪表板中显示的安全警报。
...
|警报|说明|Intent<br> ([了解更多](#intentions)) |严重性|
|----|----|:----:|--|
...
|**成功的暴力攻击**|从同一源检测到多个登录尝试。 一些已成功通过主机的身份验证。<br>这就是一种突发攻击，在这种攻击中，攻击者执行许多身份验证尝试来查找有效的帐户凭据。|-|中/高|
...
| **预览–存储帐户上托管的仿冒内容**    | 在网络钓鱼攻击中使用的 URL，它指向你的 Azure 存储帐户。 此 URL 是影响 Microsoft 365 用户的网络钓鱼攻击的一部分。<br>通常，在此类页面上承载的内容旨在诱使访问者将其企业凭据或财务信息输入到看似合法的 Web 窗体中。<br>此警报由 Microsoft 威胁智能提供支持。<br>详细了解 [Microsoft 威胁智能功能](https://go.microsoft.com/fwlink/?linkid=2128684)。<br>适用于：Azure Blob 存储、Azure 文件存储  | 集合                         | 高     |
...
| 警报                                                        | 说明                                                                                                                                                                                                                                                                                                                            | Intent<br> ([了解更多](#intentions))  | 严重性 |
|--------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------:|----------|
| **有人从 Tor 退出节点访问密钥保管库**               | 有人在访问密钥保管库时使用 Tor IP 匿名系统隐藏其位置。 尝试在未经授权的情况下访问连接到 Internet 的资源时，恶意执行组件通常会尝试隐藏其位置。                                                                                                | 凭据访问                 | 中型   |
| **密钥保管库中存在大量操作**                 | 与历史数据相比，对密钥保管库执行的操作较多。 随着时间推移，密钥保管库活动的数量通常是稳定的。  这可能是活动的合法更改。 也可能是基础结构遭到入侵，并且需要进一步的调查。                                           | 凭据访问               | 中型   |
| **密钥保管库中存在可疑的策略更改和机密查询** | 密钥保管库策略已被更改，然后还执行了列出和/或获取机密的操作。 此外，用户通常不会对此保管库执行这样的操作。 这极有可能指示密钥保管库遭到入侵，恶意执行组件窃取了其中的机密。                     | 凭据访问                     | 中型   |
| **密钥保管库中存在可疑的机密列出和查询活动**       | 机密列出操作之后有多次机密获取操作。 而且用户通常不会对此保管库执行这样的操作。 这指示有人可能在转储存储在密钥保管库中的机密，以实现潜在的恶意目的。                                                              | 凭据访问                    | 中型   |
| **异常应用程序访问了密钥保管库**                 | 某个通常不访问密钥保管库的应用程序访问了它。 这可能是合法的访问尝试（例如在基础结构或代码更新后）。 但也可能指示基础结构遭到入侵，恶意执行组件正在尝试访问密钥保管库。             | 凭据访问                  | 中型   |
| **密钥保管库中存在异常操作模式**                 | 与历史数据相比，有人执行了一组异常的密钥保管库操作。 随着时间推移，密钥保管库活动的数量通常是稳定的。  这可能是活动的合法更改。 也可能是基础结构遭到入侵，并且需要进一步的调查。                                            | 凭据访问                     | 中型   |
| **异常用户访问了密钥保管库**                        | 某个通常不访问密钥保管库的用户访问了它。 这可能是合法的访问尝试（例如，需要访问权限的新用户加入了组织）。 但也可能指示基础结构遭到入侵，恶意执行组件正在尝试访问密钥保管库。          | 凭据访问                    | 中型   |
| **异常用户-应用程序对访问了密钥保管库**       | 某个通常不访问密钥保管库的用户-应用程序对访问了它。 这可能是合法的访问尝试（例如在基础结构或代码更新后）。 但也可能指示基础结构遭到入侵，恶意执行组件正在尝试访问密钥保管库。 | 凭据访问                    | 中型   |
| **用户访问了大量密钥保管库**                  | 与历史数据相比，某个用户或应用程序访问的保管库数量发生了变化。 随着时间推移，密钥保管库活动的数量通常是稳定的。 这可能是活动的合法更改。 也可能是基础结构遭到入侵，并且需要进一步的调查。                                 | 凭据访问                   | 中型   |
|                                                              |                                                                                                                                                                                                                                                                                                                                        |                                    |          |
