...
|----|----|:----:|--|
|**检测到来自恶意 IP 的登录**|已成功为帐户“tristan.schleining”和进程“Advapi”进行远程身份验证，但登录 IP 地址 (x.x.x.x) 之前被报告为恶意或高度异常。 可能已受到攻击。|-|高|
|**检测到来自恶意 IP 的登录。[出现多次]**|已成功为帐户“IUSR_10001”和进程“Advapi”进行远程身份验证，但登录 IP 地址 (x.x.x.x) 之前被报告为恶意或高度异常。 可能已受到攻击。 扩展名为 .scr 的文件是屏幕保护程序文件，通常位于 Windows 系统目录中并从该目录执行。|-|高|
...
|**检测到 Petya 勒索软件指标**|%{Compromised Host} 上的主机数据分析检测到与 Petya 勒索软件关联的指标。 有关详细信息，请参阅 https://blogs.technet.microsoft.com/mmpc/2017/06/27/new-ransomware-old-techniques-petya-adds-worm-capabilities/ 。 查看此警报中关联的命令行，并将此警报上报给安全团队。|-|高|
...
|**检测到可疑的新防火墙规则**|主机数据分析检测到，有人通过 netsh.exe 添加了新的防火墙规则，以允许来自可疑位置的某个可执行文件的流量。|-|中型|
...
|**检测到可疑的 FTP -s 开关使用方式**|%{Compromised Host} 中的进程创建数据分析检测到，有人使用 FTP 的“-s:filename”开关。 此开关用于指定客户端要运行的 FTP 脚本文件。 我们知道，恶意软件或恶意进程会使用此 FTP 开关 (-s:filename) 指向配置为连接到远程 FTP 服务器的脚本文件，并下载其他恶意二进制文件。|-|中型|
...
|**Azure Web 应用上托管了网络钓鱼内容**|在 Azure 应用服务网站上发现用于网络钓鱼攻击的 URL。 此 URL 是发送给 O365 客户的网络钓鱼攻击的一部分。 相关内容通常引诱访问者在看似合法的网站中输入其企业凭据或财务信息。|集合|高|
...
|**可能的恶意软件已上传到存储帐户**|指示包含可能的恶意软件的 blob 已上传到存储帐户。 此警报基于利用 Microsoft 威胁情报强大功能的哈希信誉分析，其中包括病毒、特洛伊木马、间谍软件和勒索软件的哈希。 可能的原因包括：攻击者故意上传恶意软件、合法用户无意间上传了潜在恶意的 blob。 在此处了解有关 Microsoft 威胁情报功能的详细信息： https://go.microsoft.com/fwlink/?linkid=2128684 |横向移动|高|
