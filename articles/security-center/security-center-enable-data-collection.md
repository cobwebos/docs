...
> - 在 Log Analytics 中存储数据，无论你是使用新的或现有工作区中，可能会产生附加费用数据存储。 有关更多详细信息，请参阅[定价页](https://azure.microsoft.com/pricing/details/security-center/)。

## 启用 Microsoft Monitoring Agent 的自动预配 <a name="auto-provision-mma"></a>

若要从计算机收集数据，应已安装 Microsoft Monitoring Agent。  可以自动完成代理安装 （推荐） 或可以手动安装代理。  

>[!NOTE]
> 默认情况下自动设置处于关闭状态。 若要将安全中心设置为默认安装自动预配，请将自动预配设置为“打开”。 
>

在自动预配时，安全中心预配 Microsoft Monitoring agent 上所有受支持的 Azure 虚拟机和创建任何新的。 强烈建议进行自动预配，但也可以手动代理安装。 [了解如何安装 Microsoft Monitoring Agent 扩展](#manualagent)。



启用 Microsoft Monitoring Agent 自动设置的步骤：
1. 在下，安全中心主菜单中，选择**定价和设置**。
2. 单击适用的订阅
...
> 在 Log Analytics 中存储数据可能会产生附加费用数据存储。 有关更多详细信息，请参阅[定价页](https://azure.microsoft.com/pricing/details/security-center/)。
...
> 将数据存储在 log analytics 中可能会产生附加费用数据存储。 有关更多详细信息，请参阅[定价页](https://azure.microsoft.com/pricing/details/security-center/)。
...
在 Azure 安全中心选择数据收集层只会影响 Log Analytics 工作区中安全事件的存储。 Log Analytics 代理仍将收集和分析所需的 Azure 安全中心威胁检测，而不考虑的安全事件层您选择 （如果有） 将存储在 Log Analytics 工作区中的安全事件。 选择在工作区中存储安全事件将允许在工作区中调查、搜索和审核这些事件。 
> [!NOTE]
> 将数据存储在 log analytics 中可能会产生附加费用数据存储。 有关更多详细信息，请参阅[定价页](https://azure.microsoft.com/pricing/details/security-center/)。
> 
...
-  最小 - 确保此集只涵盖可能指示成功违反的事件以及数量很少的重要事件。 例如，此集包含用户成功和失败的登录 （事件 Id 4624 和 4625），但它不包含登录这是重要的审核但未对检测毫无意义，相对较多的扩展。 此集的大多数数据量是登录事件和进程创建事件（事件 ID 4688）。
-  通用 - 提供此集中的完整用户审核跟踪。 例如，此集包含用户登录名和用户登录超时值 （事件 ID 4634）。 我们加入审核操作，如安全组更改、关键域控制器 Kerberos 操作以及行业组织建议的其他事件。
...
- Microsoft Monitoring Agent 已安装的计算机上，但不是作为扩展 （直接代理）<br>
...
如果配置的工作区是用户工作区 （不安全中心的默认工作区中），则将需要安装"安全 /"securityFree"解决方案，它为安全中心从虚拟机和计算机启动处理事件到该工作区报告。<br>
<br>
...
- 存在现有的 VM 扩展<br>
    - 将 Monitoring Agent 作为扩展安装时，扩展配置仅允许向单个工作区报告。 安全中心不会覆盖用户工作区的现有连接。 安全中心将已连接，在工作区中存储 VM 的安全数据，前提是已安装在其上的"安全性"或"securityFree"解决方案。 在此过程中，安全中心可将扩展版本升级到最新版本。  
    - 若要查看现有扩展将数据发送到哪个工作区，请运行测试来[验证与 Azure 安全中心的连接](https://blogs.technet.microsoft.com/yuridiogenes/2017/10/13/validating-connectivity-with-azure-security-center/)。 或者，可以打开 Log Analytics 工作区中，选择一个工作区、 选择 VM，并查看 Log Analytics 代理连接。 
    - 如果有一个环境，其中 Log Analytics 代理安装在客户端工作站上，并向现有的 Log Analytics 工作区报告，查看的列表[通过 Azure 安全中心支持的操作系统](security-center-os-coverage.md)以确保你的操作系统支持，并查看[现有 log analytics 客户](security-center-faq.md#existingloganalyticscust)有关详细信息。
