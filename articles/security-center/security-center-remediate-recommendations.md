...
1. 按照“修正步骤”部分**** 中的说明进行操作。 每个建议都有其自己的一组指令。 以下屏幕截图显示了用于将应用程序配置为仅允许通过 HTTPS 的流量的补救步骤。
...
|CORS 不应允许所有资源访问 API 应用|此操作阻止其他域访问 API 应用程序。 若要允许特定的域，请在 "允许的来源" 字段中输入（用逗号分隔）。 <br>**注意**：如果将此字段留空，则将阻止所有跨源调用。 "参数字段标题：" 允许的来源 "|
