...
Azure 安全中心将建议启用网络安全组 (NSG)（如果尚未启用）。 NSG 包含一系列访问控制列表 (ACL) 规则，这些规则可以允许或拒绝虚拟网络中流向 VM 实例的网络流量。 NSG 可以与子网或该子网中的各个 VM 实例相关联。 当 NSG 与某个子网相关联时，ACL 规则适用于该子网中的所有 VM 实例。 另外，可以进一步通过将 NSG 直接关联到单个 VM 对流向该 VM 的流量进行限制。 有关详细信息，请参阅 [What is a Network Security Group (NSG)?](../virtual-network/virtual-networks-nsg.md)（什么是网络安全组 (NSG)？）
...
2. 这将打开子网或虚拟机的“配置缺少的网络安全组”边栏选项卡，具体取决于所选的建议。 选择一个用于配置 NSG 的子网或虚拟机。
   ...
   ![配置 VM 的 NSG][3]
3. 在“选择网络安全组”边栏选项卡中，选择一个现有的 NSG 或选择创建一个新的 NSG。
   
   ![选择网络安全组][4]

如果创建一个新的 NSG，请按照[如何使用 Azure 门户管理 NSG](../virtual-network/virtual-networks-create-nsg-arm-pportal.md) 中的步骤创建 NSG 并设置安全规则。
