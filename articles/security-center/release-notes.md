...
- [已弃用预览 AKS 建议“应在 Kubernetes 服务上定义 Pod 安全策略”](#deprecated-preview-aks-recommendation-pod-security-policies-should-be-defined-on-kubernetes-services)
- [改进了 Azure 安全中心的电子邮件通知](#email-notifications-from-azure-security-center-improved)
...
当选择建议并启用“包括安全发现结果”选项后，便可通过连续导出功能将安全发现结果导出。
...
以下与网络安全组相关的安全建议已得到改进，以减少一些误报的情况。
...
### <a name="deprecated-preview-aks-recommendation-pod-security-policies-should-be-defined-on-kubernetes-services"></a>已弃用预览 AKS 建议“应在 Kubernetes 服务上定义 Pod 安全策略”

如 [Azure Kubernetes 服务](https://docs.microsoft.com/azure/aks/use-pod-security-policies)文档所述，将弃用预览建议“应在 Kubernetes 服务上定义的 Pod 安全策略”。
...
### <a name="email-notifications-from-azure-security-center-improved"></a>改进了 Azure 安全中心的电子邮件通知

电子邮件中有关安全警报的以下方面得到了改进： 

- 添加了针对所有严重级别的警报发送电子邮件通知的功能
- 添加了在订阅上通知具有不同 RBAC 角色的用户的功能
- 默认情况下，我们会主动向订阅所有者通知高严重性警报（这些警报有很大可能反映了真正的违规行为）
- 我们已从电子邮件通知配置页中删除了“电话号码”字段

有关详细信息，请参阅[针对安全警报设置电子邮件通知](security-center-provide-security-contact-details.md)。
...
发现新的威胁后，安全中心会通过新建议功能提供新的安全建议。 为了避免意外更改你的安全分数，以及为了提供一个可以在分数受影响前探索新建议的宽限期，标记为“预览”的建议不再纳入安全分数的计算中。 但仍应尽可能按这些建议进行修正，这样在预览期结束时，它们会有助于提升分数。
...
### <a name="recommendations-now-include-a-severity-indicator-and-the-freshness-interval"></a>建议现在包含严重性指标和新鲜度间隔

建议的详细信息页现在包含一个新鲜度间隔指标（每当相关时）并清楚地显示了建议的严重性。
...
安全中心已添加对[安全默认值](https://docs.microsoft.com/azure/active-directory/fundamentals/concept-fundamentals-security-defaults)的完全支持，即 Microsoft 的免费标识安全保护。

安全默认值提供预先配置的标识安全设置，以保护组织免受与标识相关的常见攻击。 安全默认值总共已保护五百多万个租户；安全中心还保护了五万个租户。

安全中心每当标识未启用安全默认值的 Azure 订阅时，都会提供安全建议。 到目前，安全中心仍建议使用 Azure Active Directory (AD) 高级许可证中的条件访问启用多重身份验证。 对于免费使用 Azure AD 的客户，我们现在建议启用安全默认值。 

我们的目标是鼓励更多的客户通过 MFA 保护其云环境，并降低最高的一项风险，同时也是对[安全分数](secure-score-security-controls.md)影响最大的风险。
...
“应使用服务主体而不是管理证书来保护订阅”这一建议鼓励使用服务主体或 Azure 资源管理器来更安全地管理订阅。 

有关详细信息，请参阅 [Azure Active Directory 中的应用程序对象和服务主体对象](https://docs.microsoft.com/azure/active-directory/develop/app-objects-and-service-principals#service-principal-object)。


### <a name="vulnerability-assessment-on-vms---recommendations-and-policies-consolidated"></a>VM 上的漏洞评估 - 已合并建议和策略

安全中心会检查 VM 是否正在运行漏洞评估解决方案。 如果未找到漏洞评估解决方案，安全中心将提供建议以简化部署。

如果发现漏洞，安全中心会建议汇总发现结果，以便你在必要时进行调查和修正。

为了确保所有用户无论使用哪种扫描程序类型都能获得一致体验，我们已将四条建议统一为以下两条：

|统一的建议|更改描述|
|----|:----|
|**应在虚拟机上启用漏洞评估解决方案**|替换以下两条建议：<br> **•** 在虚拟机上启用内置漏洞评估解决方案（由 Qualys（现已弃用）提供支持）（标准层提供）<br> **•** 漏洞评估解决方案应安装在虚拟机上（现已弃用）（标准层和免费层）|
...
另外，当发现漏洞并报告给安全中心时，无论发现漏洞的是哪种漏洞评估解决方案，单一建议都会提醒你注意这些发现结果。

#### <a name="updating-dependencies"></a>正在更新依赖项

如果你的脚本、查询或自动化程序引用了以前的建议或策略键/名称，请使用下表更新引用：
...
##### <a name="from-august-2020"></a>从 2020 年 8 月起

|建议|范围|
|----|:----|
|**应在虚拟机上启用漏洞评估解决方案**<br>注册表项：ffff0522-1e88-47fc-8382-2a80ba848f5d|内置 + BYOL|
...
此项目的早期阶段包括个人预览版，并将新的（默认禁用）策略添加到 ASC_default 计划。

你可以放心地忽略这些策略，环境不会受到任何影响。 如果想要启用这些策略，请在 https://aka.ms/SecurityPrP 注册预览版，并从以下选项中进行选择：
...
- [容器安全性改进 - 提高注册表扫描速度并更新文档](#container-security-improvements---faster-registry-scanning-and-refreshed-documentation)
- [使用新建议更新自适应应用程序控制，并支持在路径规则中使用通配符](#adaptive-application-controls-updated-with-a-new-recommendation-and-support-for-wildcards-in-path-rules)
...
虽然现在可以在更多台计算机上部署集成的漏洞评估扩展（由 Qualys 提供支持），但只有在使用[将集成漏洞扫描程序部署到标准层 VM](deploy-vulnerability-assessment-vm.md#deploy-the-integrated-vulnerability-scanner-to-your-vms)列出的操作系统时，才可以使用支持
...
Azure 存储的威胁防护会检测 Azure 存储帐户上可能存在的有害活动。 安全中心在检测到有人试图访问或利用你的存储帐户时，会显示警报。 
...
添加了八条新建议以提供一种简单的方法来为以下资源类型启用 Azure 安全中心的威胁防护功能：虚拟机、应用服务计划、Azure SQL 数据库服务器、计算机上的 SQL 服务器、Azure 存储帐户、Azure Kubernetes 服务群集、Azure 容器注册表的注册表和 Azure Key Vault 保管库。
...
这些新建议属于“启用高级威胁防护”安全控件。

建议还包括快速修复功能。 

> [!IMPORTANT]
> 修正其中的任何建议将产生相关资源的保护费用。 如果当前订阅中有相关资源，则会立即开始计费。 如果以后再添加这些资源，则会在后来计费。
> 
> 例如，如果订阅中没有任何 Azure Kubernetes 服务群集，而你启用了威胁防护，则不会产生任何费用。 如果将来在同一订阅中添加群集，它将自动受到保护，届时将开始计费。

有关详细信息，请参阅[安全建议参考页面](recommendations-reference.md)。
...
### <a name="container-security-improvements---faster-registry-scanning-and-refreshed-documentation"></a>容器安全性改进 - 提高注册表扫描速度并更新文档

作为容器安全领域连续投资的一部分，我们极大地改进了安全中心对 Azure 容器注册表中存储的容器映像的动态扫描性能，现在很高兴分享这项改进。 现在，扫描通常在大约两分钟内完成。 在某些情况下，最多可能需要 15 分钟。

为了更清楚地说明 Azure 安全中心的容器安全功能，我们还更新了容器安全文档页面。 

有关安全中心的容器安全性，请参阅以下文章：
...
- [Azure Kubernetes 服务群集的威胁防护功能发出的安全警报](alerts-reference.md#alerts-akscluster)
- [Azure Kubernetes 服务主机的威胁防护功能发出的安全警报](alerts-reference.md#alerts-containerhost)
- [有关容器的安全性建议](recommendations-reference.md#recs-containers)



### <a name="adaptive-application-controls-updated-with-a-new-recommendation-and-support-for-wildcards-in-path-rules"></a>使用新建议更新自适应应用程序控制，并支持在路径规则中使用通配符

自适应应用程序控制功能已收到两个重要更新：

* 新建议确定以前未被允许的可能合法的行为。 “应更新自适应应用程序控制策略中的允许列表规则”这项新建议提示你向现有策略添加新规则以减少自适应应用程序控制违规警报中的误报数。

* 路径规则现在支持通配符。 从本次更新起，可以使用通配符配置允许的路径规则。 有两个支持的方案：

    * 在路径末尾使用通配符可允许此文件夹和子文件夹中的所有可执行文件

    * 在路径中间使用通配符可启用文件夹名称变动的已知可执行文件名称（例如，使用已知可执行文件的个人用户文件夹、自动生成的文件夹名称等）。
...
- 应在 SQL 服务器的高级数据安全设置中将“高级威胁防护类型”设置为“全部”
...
详细了解[内置策略](security-center-policy-definitions.md)。
...
- [用于启用威胁防护和高级数据安全的新策略](#new-policies-for-enabling-threat-protection-and-advanced-data-security)



### <a name="secure-score-api-preview"></a>安全分数 API（预览）

现在可以通过[安全分数 API](https://docs.microsoft.com/rest/api/securitycenter/securescores/)（当前为预览版）访问分数。 通过 API 方法，可灵活地查询数据，久而久之构建自己的安全功能分数报告机制。 例如，可以使用安全功能分数 API 来获取特定订阅的分数。 此外，还可以使用安全功能分数控件 API 列出安全控件和订阅的当前分数。

有关使用安全分数 API 可以实现的外部工具的示例，请参阅 [GitHub 社区安全分数区](https://github.com/Azure/Azure-Security-Center/tree/master/Secure%20Score)。
...
添加了两条新建议来帮助你将 [Log Analytics 代理](https://docs.microsoft.com/azure/azure-monitor/platform/log-analytics-agent)部署到 Azure Arc 计算机，并确保它们受 Azure 安全中心保护：

- Log Analytics 代理应安装在基于 Windows 的 Azure Arc 计算机上(预览)
- Log Analytics 代理应安装在基于 Linux 的 Azure Arc 计算机上(预览)

这些新建议将出现在与现有（相关）建议“应在计算机上安装监视代理”相同的以下四项安全控件中：修正安全配置、应用自适应应用程序控制、应用系统更新和启用终结点保护。

建议还包括快速修复功能，可帮助加速部署进程。 

有关这两条新建议的详细信息，请查看[计算和应用推荐](recommendations-reference.md#recs-computeapp)表。

有关 Azure 安全中心如何使用代理的详细信息，请参阅[什么是 Log Analytics 代理？](faq-data-collection-agents.md#what-is-the-log-analytics-agent)。
...
自动执行组织的监视和事件响应过程可以极大地缩短调查和缓解安全事件所花的时间。

要在组织中部署自动化配置，请使用这些内置的“不存在时部署”Azure 策略来创建和配置[连续导出](continuous-export.md)和[工作流自动化](workflow-automation.md)过程：
...
有关使用这两个导出策略的详细信息，请参阅[通过 Policy 持续导出 Azure 安全中心的警报和建议](https://techcommunity.microsoft.com/t5/azure-security-center/continuously-export-azure-security-center-alerts-and/ba-p/1440745)。
...
“实施安全最佳做法”安全控件现在包括以下新建议：

- **应使用网络安全组来保护非面向 Internet 的虚拟机**

现有建议“使用网络安全组来保护面向 Internet 的虚拟机”并不区分面向 Internet 的虚拟机和非面向 Internet 的虚拟机。 对于这两种情况，如果未将 VM 分配到网络安全组，则会生成高严重性建议。 此新建议分离非面向 Internet 的计算机，以减少误报并避免不必要的高严重性警报。

有关详细信息，请查看[网络建议](recommendations-reference.md#recs-network)表。




### <a name="new-policies-for-enabling-threat-protection-and-advanced-data-security"></a>用于启用威胁防护和高级数据安全的新策略

下面的新策略已添加到 ASC 默认计划中，旨在帮助为相关资源类型启用威胁防护或高级数据安全。
...
安全分数增强版引入的一个安全控件是“实现安全最佳做法”。 为订阅创建的所有自定义建议已自动放入该控件中。 
...
安全分数增强版引入的一个安全控件是“实现安全最佳做法”。 如果建议在此控件中显示，则不影响安全功能分数。 
...
- 应在 Windows 虚拟机上安装来宾配置扩展（预览）- 如果使用 [Azure Policy 来宾配置](https://docs.microsoft.com/azure/governance/policy/concepts/guest-configuration)，则可在虚拟机中查看服务器和应用程序设置（仅限 Windows）。

- 应在计算机上启用 Windows Defender 攻击防护（预览）- Windows Defender 攻击防护采用 Azure Policy 来宾配置代理。 攻击防护服务具有 4 个组件，旨在锁定设备来阻隔各种攻击途径，并阻止恶意软件攻击中常用的行为，同时让企业能够平衡其安全风险和生产力要求（仅限 Windows）。
...
要详细了解安全控件，请参阅[安全分数增强版（预览版）](secure-score-security-controls.md)。
...
- **主动及时检测恶意软件** - 使用 CDA 方法时，会等到故障发生后再运行分析来查找恶意工件。 使用无文件攻击检测后，可在内存中威胁正在运行时主动识别它们。 
