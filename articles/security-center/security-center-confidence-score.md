...
检测到有可疑的进程在虚拟机上运行时，会生成警报。 安全中心针对 Azure 中运行的 Windows 虚拟机评审并分析这些警报。 它会使用高级算法，针对组织中的多个实体和数据源以及所有的 Azure 资源执行自动检查与关联，并提供一个置信度评分，该评分表示安全中心有多大的把握认为该警报符合事实且需要调查。
