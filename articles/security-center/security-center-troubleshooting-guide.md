...
安全中心使用 Microsoft 监视代理（这是 Azure 监视器服务使用的代理）从 Azure 虚拟机收集安全数据。 启用数据收集并在目标计算机中正确安装代理后，应执行以下进程：
