...
要为 Azure 应用服务计划启用高级威胁保护，您必须：

* 订阅 Azure 安全中心的标准定价层
* 启用应用服务计划，如下所示。 安全中心与应用服务本机集成，无需部署和载入 - 集成是透明的。
* 制定与专用计算机关联的应用服务计划。 支持的计划包括：基本、标准、高级、隔离或 Linux。 安全中心不支持免费、共享或消费计划。 有关详细信息，请参阅[应用服务计划](https://azure.microsoft.com/pricing/details/app-service/plans/)。

启用应用服务计划后，安全中心会评估应用服务计划涵盖的资源并根据其发现生成安全建议。 安全中心保护运行应用服务的 VM 实例以及管理接口。 此外，它还监视向/从应用服务中运行的应用发送的请求和响应。

安全中心利用云的规模和 Azure 作为云提供商拥有的可见性来监视常见的 Web 应用攻击。 安全中心可能会发现对你的应用程序的攻击并识别正在出现的攻击，即使攻击者处于侦测阶段，正在通过扫描来识别多个 Azure 托管应用程序的漏洞。 作为 Azure 本机服务，安全中心还在独特的立场提供基于主机的安全分析（涵盖此 PaaS 的底层计算节点），因此，安全中心可以检测已遭恶意利用的 Web 应用程序受到的攻击。 有关详细信息，请参阅[Azure 应用服务的威胁保护](threat-protection.md#app-services)。
...
>
>要验证计数，请前往 Azure 门户中的"应用服务计划"，您可以在其中查看每个计划使用的计算实例数。 
