...
由于 Judy 和 Sam 的角色不同，他们需要在安全中心的不同领域获取日常活动的相关信息。 Judy 在日常监视中将使用**安全警报**。

![安全警报](./media/security-center-incident-response/security-center-incident-response-fig3.png)

Judy 需要在检测和评估阶段使用安全警报。 Judy 完成初始评估后, 如果需要进一步调查, 他们可能会将问题上报给 Sam。 此时，Sam 会利用安全中心提供的信息，有时结合其他数据源，进入到诊断阶段。
...
Judy 登录到 Azure 门户，现在位于安全中心控制台中。 作为每个日常监视活动的一部分, 他们开始通过执行以下步骤来查看高优先级的安全警报:
...
阅读说明后, Judy 确信这不是误报, 并且应将此案例升级到 Sam。
