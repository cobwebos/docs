...
通过监视标识活动，你可以在事件发生之前采取主动措施，或者采取反应操作来停止攻击尝试。 例如，安全中心可能会标记不推荐使用的帐户（不再需要的帐户，并将其阻止 Azure Active Directory）进行删除。 

你可能会在 Azure 安全中心的 "**身份和访问**资源安全性" 一节中看到的建议示例包括：
...
有关这些建议的详细信息以及你可能在此处看到的建议的完整列表，请参阅[身份验证和访问建议](recommendations-reference.md#recs-identity)。
...
- 如果有 AD 高级版，请使用[条件性访问](https://docs.microsoft.com/azure/active-directory/conditional-access/overview)启用 MFA。
