...
- 安全读者：属于此角色的用户能够查看安全中心的权限，包括建议、警报、策略和运行状况，但无法进行更改。
...
安全策略定义一组控件，这些控件是针对指定订阅中的资源建议的。 在安全中心定义策略时，需考虑到公司的安全要求和应用程序类型或数据的敏感性。

在订阅级别启用的策略会自动传播到该订阅中的所有资源组，所下图所示：

![安全策略](./media/security-center-planning-and-operations-guide/security-center-planning-and-operations-guide-fig2-newUI.png)
...
在配置安全策略之前，请查看每项 [安全建议](security-center-recommendations.md)，确定这些策略是否适合各种订阅和资源组。 此外，务必了解解决[安全建议](https://docs.microsoft.com/en-us/azure/security-center/security-center-recommendations)应采取的操作和组织中负责监视新建议并采用必要步骤的人员。
...
Azure 安全中心使用 Microsoft Monitoring Agent（Operations Management Suite 和 Log Analytics 服务同样适用此代理）从虚拟机中收集安全数据。 通过此代理收集的数据存储于 Log Analytics 工作区中。

### <a name="agent"></a>代理

在安全策略中启用数据收集后，Microsoft Monitoring Agent（适用于 [Windows](https://docs.microsoft.com/azure/log-analytics/log-analytics-windows-agents) 或 [Linux](https://docs.microsoft.com/azure/log-analytics/log-analytics-linux-agents)）会安装在所有支持的 Azure VM 和新建的任何 VM 上。  如果 VM 已安装 Microsoft Monitoring Agent，Azure 安全中心将利用当前安装的代理。 代理的过程设计为非入侵性，对 VM 性能的影响非常小。
...
对于 Azure 安全中心创建的工作区，数据将保留 30 天。 对于现有工作区，保留期取决于工作区定价层。
...
若要从 Azure 门户访问安全中心，可单击“浏览”，然后在“筛选器”字段中键入“安全中心”。 用户获取的视图取决于应用的筛选器，下例显示了具有许多待处理问题的环境：

![仪表板](./media/security-center-planning-and-operations-guide/security-center-planning-and-operations-guide-fig6.png)
