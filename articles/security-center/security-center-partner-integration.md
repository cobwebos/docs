...
要了解有关从 Qualys 集成漏洞扫描工具（包括可供标准层客户使用的内置扫描仪）的更多详细信息，请参阅： 

- [虚拟机的集成漏洞扫描器](built-in-vulnerability-assessment.md)。
- [部署合作伙伴漏洞扫描解决方案](partner-vulnerability-assessment.md)。

安全中心还为您的：

* SQL 数据库 - 请参阅[漏洞评估仪表板中浏览漏洞评估报告](security-center-iaas-advanced-data.md#explore-vulnerability-assessment-reports)
* Azure 容器注册表映像 - 请参阅[Azure 容器注册表与安全中心集成（预览）](azure-container-registry-integration.md)
...
1. 从[Azure 门户](https://azure.microsoft.com/features/azure-portal/)打开**安全中心**。

1. 从安全中心的菜单中，选择**安全解决方案**。

在 **"安全解决方案**"页中，您可以看到集成 Azure 安全解决方案的运行状况并运行基本管理任务。
