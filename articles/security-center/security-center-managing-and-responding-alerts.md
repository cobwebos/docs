...
> [!NOTE]
> 如果你使用的是**2015-06-01-preview** API 版本, 则警报严重性类型将应用于上述方案的情况有所不同。  
...
在 "**说明**" 字段中, 可以找到有关此事件的更多详细信息。 通过这些额外的详细信息，可深入了解是什么触发了该安全警报、其目标资源、源 IP 地址（如果适用）以及修复建议。  在某些情况下，源 IP 地址为空（不可用），因为并不是所有 Windows 安全事件日志都包含 IP 地址。

安全中心提供的修正建议因安全警报而异。 在某些情况下，可能需要使用其他 Azure 功能来实现建议的修复操作。 例如, 对此攻击的补救措施是不允许使用[网络 ACL](../virtual-network/virtual-networks-acl.md)或[网络安全组](../virtual-network/security-overview.md#security-rules)规则生成此攻击的 IP 地址。 有关不同类型的警报的详细信息, 请参阅[安全警报类型](security-center-alerts-overview.md#security-alert-types)。
