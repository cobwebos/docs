...
1. 根据已启用的安全策略，安全中心会视需要提供一组安全建议。 一开始应查看虚拟机和计算机建议。 在“安全中心”仪表板中选择“概览”，然后选择“计算与应用”。
...
2. 解决所有相关的计算建议以后，应转到下一工作负荷：网络。 在“安全中心”仪表板中单击“概览”，然后单击“网络”。
  ...
  网络建议页有一个列表，其中包含网络配置、面向 Internet 的终结点和网络拓扑方面的问题。 就像“计算与应用”一样，某些网络建议会提供集成的修正，而另一些则不会。

3. 解决所有相关的网络建议以后，应转到下一工作负荷：存储与数据。 在“安全中心”仪表板中单击“概览”，然后单击“数据与存储”。

  ![数据资源](./media/tutorial-security-policy/tutorial-security-policy-fig4.png)

  “数据资源”页包含的建议涉及启用 Azure SQL 服务器和数据库审核、启用 SQL 数据库加密，以及启用 Azure 存储帐户加密。 如果没有这些工作负荷，则不会看到任何建议。 就像“计算与应用”一样，某些数据与存储建议会提供集成的修正，而另一些则不会。

4. 解决所有相关的数据与存储建议以后，应转到下一工作负荷：标识和访问。 在“安全中心”仪表板中单击“概览”，然后单击“标识和访问”。

  ![标识和访问](./media/tutorial-security-policy/tutorial-security-policy-fig5.png)
