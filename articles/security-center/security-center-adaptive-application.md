...
自适应应用程序控制是 Azure 安全中心提供的智能的自动化端到端应用程序白名单解决方案。 可以通过自适应应用程序控制来控制哪些应用程序能够在 Azure 中的 VM 上运行，这样有很多好处，能够增强 VM 对恶意软件的抵抗力。 安全中心使用机器学习分析在 VM 上运行的应用程序，有助于运用此智能服务应用特定的白名单规则。 此功能大大简化配置和维护应用程序白名单策略的过程，让你可以：
...
3. 在“选择 VM”中查看建议的 VM 的列表，取消选中不需向其应用应用程序白名单策略的 VM。 接下来，你将看到两个列表：
...
4. 复查每个列表中的应用程序，并取消选中不希望应用规则的任何应用程序。 每个列表包括：
  ...
  - **可利用**：一个警告图标，表示攻击者可能会利用特定应用程序绕过应用程序白名单解决方案。 建议在这些应用程序审批前查看它们。
  - **用户**：建议允许其运行应用程序的用户

5. 选择完以后，请选择“创建”。 <br>
选择“创建”后，Azure 安全中心会自动基于 Windows 服务器 (AppLocker) 上可用的内置应用程序白名单解决方案创建相应的规则。
...
1. 若要编辑和监视使用应用程序白名单策略配置的组，请返回到“自适应应用程序控制”页，在“VM 组”下选择“已配置”：
...
4. 可以在“策略扩展”下添加需要允许的任何应用程序路径。 添加这些路径后，除了已有的规则之外，安全中心还会更新所选 VMS 组中 VM 上的应用程序白名单策略，并为这些应用程序创建适当的规则。

5. 查看“最近警报”部分中列出的当前冲突。 单击要重定向到 Azure 安全中心内“警报”页面的每一行，并查看 Azure 安全中心在关联的 VM 上检测到的所有警报。
  - **警报**：已记录的任何冲突。
  - **VM 数量**：带有此类警报的虚拟机数量。

6. 在“发布者白名单规则”、“路径白名单规则”和“哈希白名单规则”下，可以根据规则集合类型，查看当前在组内的 VM 上配置了哪些应用程序白名单规则。 对于每条规则，可以查看：

  - **规则**：AppLocker 根据此参数检查应用程序是否能运行。
  - **文件类型**：特定规则涵盖的文件类型。 可以是以下任意类型：EXE、脚本、MSI 或这些类型的任意排列。
  - **用户**：允许运行应用程序白名单规则所涵盖的应用程序的用户的名称或数量。
...
安全中心只为运行稳定的一组应用程序的虚拟机建议应用程序白名单规则。 如果已关联 VM 上的应用程序始终变化不定，则不会创建建议。
...
Azure 安全中心还可以在非推荐的 VM 组上定义应用程序白名单策略。 遵循与前述相同的原则，在这些组上配置应用程序白名单策略。
