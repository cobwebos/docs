...
description: 本文档可帮助你在 Azure 安全中心使用自适应应用程序控制，将 Azure 计算机中运行的应用程序列入允许列表。
...
自适应应用程序控制是 Azure 安全中心的一个智能、自动化的端到端解决方案，可帮助你控制哪些应用程序可以在 Azure 和非 Azure 计算机（Windows 和 Linux）上运行。 除此之外，这有助于强化计算机免受恶意软件的侵害。 安全中心使用机器学习来分析计算机上运行的应用程序，并通过此智能创建一个允许列表。 此功能极大地简化了配置和维护应用程序允许列表策略的过程，使你能够：
...
> [!NOTE]
> 对于非 Azure 和 Linux 计算机，仅在审核模式下支持自适应应用程序控件。

## <a name="how-to-enable-adaptive-application-controls"></a>如何启用自适应应用程序控制？

自适应应用程序控件可帮助您定义一组允许在已配置的计算机上运行的应用程序。 此功能适用于 Azure 和非 Azure Windows （所有版本、经典或 Azure 资源管理器）和 Linux 计算机。 使用以下步骤配置应用程序允许列表：
...
1. 若要编辑和监视使用应用程序允许列表策略配置的组，请返回到 "**自适应应用程序控制**" 页，然后选择 " **vm 组**" 下**配置**的：
   ...
   - **组名称**：订阅和组的名称
   - **Vm 和计算机**：组中的虚拟机数
   - **Mode**：审核模式将记录运行不在允许列表上的应用程序的尝试;强制执行将不允许应用程序运行，除非它们位于允许列表中
   - **警报**：任何当前的冲突
...
## <a name="move-a-vm-from-one-group-to-another"></a>将 VM 从一个组移到另一个组
 ...
 将 VM 从一个组移到另一个组时，应用到它的应用程序控制策略将更改为你将其移动到的组的设置。 你还可以将 VM 从配置的组移动到未配置的组，这将导致删除以前应用于 VM 的任何应用程序控制策略。

 1. 从 "**自适应应用程序控制**" 页上的 "**配置**" 选项卡中，单击要移动的 VM 当前所属的组。
1. 单击 "**配置的 vm 和计算机**"。
1. 单击 VM 行中的三个点以移动，并单击 "**移动**"。 将打开 "**将计算机移动到不同组**" 窗口。

    ![保护](./media/security-center-adaptive-application/adaptive-application-move-group.png)

 1. 选择要将 VM 移到的组，单击 "**移动计算机**"，然后单击 "**保存**"。

    ![保护](./media/security-center-adaptive-application/adaptive-application-move-group2.png)

 > [!NOTE]
> 单击 "**移动计算机**" 后，请确保单击 "**保存**"。 如果不单击 "**保存**"，则不会移动计算机。

## <a name="next-steps"></a>后续步骤
本文档介绍了如何在 Azure 安全中心使用自适应应用程序控制将在 Azure 中运行的应用程序和非 Azure Vm 列入允许列表。 若要了解更多有关 Azure 安全中心的详细信息，请参阅以下内容：
