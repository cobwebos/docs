...
description: 本文介绍如何通过 Azure 站点恢复添加和管理 VMware vCenter，以便 VMware VM 的灾难恢复到 Azure。
...
为本地 VMware VM 设置灾难恢复时，站点恢复需要访问 vCenter 服务器/vSphere 主机。 然后，站点恢复过程服务器可以自动发现 VM，并根据需要将其故障转移。 默认情况下，进程服务器在站点恢复配置服务器上运行。 为配置服务器添加一个帐户以连接到 vCenter Server/vSphere 主机，如下所示：

1. 登录到配置服务器。
1. 使用桌面快捷方式打开配置服务器工具 （_cspsconfigtool.exe）。_
1. 在“管理帐户”选项卡上，单击“添加帐户”********。

   ![add-account](./media/vmware-azure-manage-vcenter/addaccount.png)

1. 提供帐户详细信息并单击“确定”以添加该帐户****。 帐户应具有帐户权限表中汇总的权限。
...
|**复制/故障转移** | 至少一个只读用户帐户。 | 数据中心对象 –> 传播到子对象、角色=只读 | 在数据中心级别分配的对数据中心内所有对象具有访问权限的用户。<br/><br/> 要限制访问，请将 **"传播"到子**对象（vSphere 主机、数据存储、虚拟机和网络）的 **"无访问"** 角色分配给子对象。<br/><br/> 用于迁移，但不用于完全复制、故障转移和故障回复。|
...
1. 在保管库>**站点恢复基础结构** > **配置服务器**中，打开配置服务器。
...
1. 选择用于连接 VMware vCenter 或 vSphere ESXi 服务器的帐户。 然后单击“确定”****。

## <a name="modify-credentials"></a>修改凭据

如有必要，您可以修改用于连接到 vCenter 服务器/vSphere 主机的凭据，如下所示：
...
1. 提供新帐户的详细信息并单击“确定”****。 帐户需要[帐户权限](#account-permissions)表中列出的权限。
1. 在保管库>**站点恢复基础结构** > **配置服务器**中，打开配置服务器。
1. 在“详细信息”中，单击“刷新服务器”********。
1. “刷新服务器”作业完成后，选择 vCenter Server。
1. 在**摘要**中，在**vCenter 服务器/vSphere 主机帐户**中选择新添加的帐户，然后单击"**保存**"。
...
1. 在**站点恢复基础结构** > **配置服务器**>保管库中，单击 vCenter 服务器添加到的配置服务器。
...
如果要迁移所有 VM 以使用新 vCenter Server，只需更新分配给该 vCenter Server 的 IP 地址。 不要添加其他 VMware 帐户，因为这可能导致重复的条目。 按照下面的方式更新地址：

1. 在**站点恢复基础结构** > **配置服务器**>保管库中，单击 vCenter 服务器添加到的配置服务器。
1. 在“vCenter Server”部分，单击要从中迁移的 vCenter Server。****
1. 在“摘要”**** 中，将 IP 地址更新为新 vCenter Server 的 IP 地址，并保存更改。
1. 更新 IP 地址后，站点恢复将开始从新的 vCenter 服务器接收 VM 发现信息。 这不会影响正在进行的复制活动。

## <a name="migrate-a-few-vms-to-a-new-server"></a>将数个 VM 迁移到新服务器

如果只想将一些复制 VM 迁移到新的 vCenter 服务器，请执行以下操作：
...
如果要迁移到新 vCenter 服务器的 VM 数高于将保留在原始 vCenter 服务器上的 VM 数，则执行以下操作：

1. 在配置服务器设置中将分配给 vCenter Server 的 [IP 地址更新](#modify-the-ip-address-and-port)为新 vCenter Server 的地址。
1. [禁用](site-recovery-manage-registration-and-protection.md#disable-protection-for-a-vmware-vm-or-physical-server-vmware-to-azure)保留在旧服务器上的少数 VM 的复制。
...
如果有任何问题，请参阅[排除故障 vCenter 服务器发现失败](vmware-azure-troubleshoot-vcenter-discovery-failures.md)。
