...
- **添加/删除/排除磁盘**：启用复制后，无法添加/删除/排除磁盘进行复制。 如果想要添加/删除或排除磁盘，需要禁用 VM 保护，并重新启用保护。
- **故障转移**：故障转移后，如果故障转移应用需要排除磁盘才能正常工作，则需要手动创建这些磁盘。 或者，可将 Azure 自动化集成到恢复计划中，以便在故障转移计算机期间创建磁盘。
- **故障转移**：故障转移后故障回本地站点时，在 Azure 中手动创建的磁盘不会失败。 例如，如果在 Azure VM 中故障转移三个磁盘并直接创建两个磁盘，则只会故障回复完成故障转移的三个磁盘。 在故障回复中或 VM 的反向复制中，不能包括手动创建的磁盘。
