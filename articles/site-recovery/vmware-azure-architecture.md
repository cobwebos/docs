...
1. 启用 VM 复制时，对 Azure 存储的初始复制将开始（使用指定复制策略）。 注意以下事项：
    ...
    - 应用任何复制策略设置：
        ...
        - **应用一致的快照**。 应用一致的快照可以每隔1到12小时执行一次，具体取决于你的应用需求。 快照是标准的 Azure Blob 快照。 VM 上运行的移动代理在复制流中请求对应此设置的 VSS 快照和作为应用程序一致点的时间点书签。
...
3. 初始复制操作可确保将启用复制时计算机上的整个数据发送到 Azure。 完成初始复制后，开始将增量更改复制到 Azure。 对虚拟机的跟踪更改将发送到进程服务器。
4. 通信按如下方式发生：
...
1. 有时，在初始复制期间或传输增量更改时，源计算机与进程服务器或进程服务器之间可能存在网络连接问题。 其中的任何一种都可能导致数据传输到 Azure 暂时失败。
2. 为避免出现数据完整性问题，并将数据传输成本降到最低，Site Recovery 将计算机标记为重新同步。
3. 在以下情况下，还可以将计算机标记为进行重新同步，以维持源计算机与 Azure 中存储的数据之间的一致性
    - 如果计算机强制关机
    - 如果一台计算机经历了磁盘大小调整的配置更改（将磁盘大小从 2 TB 修改为 4 TB）
4. 重新同步仅向 Azure 发送增量数据。 通过在源计算机和 Azure 中存储的数据之间计算数据校验和来最大程度地减少在本地与 Azure 之间的数据传输。
5. 默认情况下，重新同步安排为在非工作时间自动运行。 如果你不希望等待默认非工作时间的重新同步，可手动重新同步 VM。 为此，请在 Azure 门户中选择 VM > 重新**同步**。
6. 如果默认重新同步在办公时间之外失败并且需要手动干预，则会在 Azure 门户中的特定计算机上生成错误。 可以解决此错误，并手动触发重新同步。
7. 重新同步完成后，会恢复增量更改的复制。
