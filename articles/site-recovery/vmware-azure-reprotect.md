...
故障回复到本地站点之前，需要 Azure 中的进程服务器：

- 进程服务器从 Azure 中受保护的虚拟机接收数据，然后将数据发送到本地站点。
- 在进程服务器与受保护虚拟机之间需要配置低延迟网络。 因此，建议在 Azure 中部署进程服务器。 如果进程服务器离复制虚拟机（Azure 中进行故障转移的计算机）较近，则复制性能较高。 
- 如需概念证明，可以将本地进程服务器与 ExpressRoute 一起用于专用对等互连。
