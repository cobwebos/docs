...
* 登录到部署它的 VM，然后从桌面快捷方式开始**Azure Site Recovery Configuration Manager** 。
...
1. [登录](#access-configuration-server)后，选择 "**管理虚拟机凭据**"
2. 选择要修改的帐户，单击“编辑” 
3. 输入新凭据，然后选择“确定”****。
...
1. [登录](#access-configuration-server)到配置服务器后，选择 "**管理连接**"。
...
配置服务器具有一个内置的 web 服务器，该服务器协调所有受保护计算机上的移动代理活动、内置/横向扩展进程服务器以及连接到它的主目标服务器。 Web 服务器使用 SSL 证书对客户端进行身份验证。 该证书在三年后到期，并可随时续订。
...
### <a name="if-certificates-are-yet-to-expire"></a>证书是否过期

1. 若要续订，请在保管库中打开**Site Recovery 基础结构** > **配置服务器**。 选择所需的配置服务器。
2. 确保所有组件的横向扩展进程服务器、主目标服务器和所有受保护计算机上的移动代理都处于最新版本并处于连接状态。
3. 现在，选择 "**续订证书**"。
4. 仔细按照本页上的说明进行操作，并单击 "确定" 以续订选定配置服务器上的证书及其关联组件。

### <a name="if-certificates-have-already-expired"></a>如果证书已过期

1. 过期后，**无法从 Azure 门户续订**证书。 在继续之前，请确保所有组件的横向扩展进程服务器、主目标服务器和所有受保护计算机上的移动代理处于最新版本，并且处于连接状态。
2. **仅当证书已过期时才执行此过程。** 登录到配置服务器，导航到 C 驱动器 > 程序数据 > Site Recovery > home > svsystems > bin 并以管理员身份执行 "Renewcerts.exe" 执行程序工具。
3. 将弹出一个 PowerShell 执行窗口，并触发证书续订。 此过程最长需要 15 分钟。 直到续订完成后才关闭窗口。
...
1. 在保管库中，请参阅**管理** > **Site Recovery 基础结构** > **配置服务器**。
...
2. 从配置服务器中取消[关联](vmware-azure-set-up-replication.md#disassociate-or-delete-a-replication-policy)和[删除](vmware-azure-set-up-replication.md#disassociate-or-delete-a-replication-policy)所有复制策略。
...
1. 在 Azure 门户中，导航到 **"恢复服务保管库** > " "**管理** > **VMware & 物理计算机** > **配置服务器****Site Recovery 基础结构** > "
2. 单击要刷新的配置服务器。
3. 在包含所选配置服务器详细信息的边栏选项卡上，单击 "**更多** > **刷新服务器**"。
4. 监视**Site Recovery 作业**下的**恢复服务保管库** > **监视** > 下的作业进度。
