---
title: 使用 Azure 站点恢复设置物理本地服务器的灾难恢复
...
[Azure 站点恢复](site-recovery-overview.md)服务通过管理和协调本地计算机和 Azure 虚拟机 （VM） 的复制、故障转移和故障倒退，从而有助于灾难恢复策略。
...
- 查看所有组件[的支持要求](vmware-physical-secondary-support-matrix.md)。
...
获取微软[Azure 帐户](https://azure.microsoft.com/)。
...
- 可以使用域或本地帐户
- 对于 Windows VM，如果使用的不是域帐户，则在本地计算机上禁用远程用户访问控制。 为此，在**HKEY_LOCAL_MACHINE[软件]微软[视窗][当前版本]策略_系统**下的寄存器中，添加 DWORD 条目**本地帐户令牌筛选策略**，其值为 1。
...
1. 单击“恢复服务保管库”****> 保管库。
2. 在"资源"菜单中，单击 **"站点恢复** > **准备基础结构** > **保护目标**"。
...
注册完成后，配置服务器将显示在保管库中的 **"设置** > **服务器**"页上。
...
1. 要创建新的复制策略，请单击**站点恢复基础结构** > **复制策略** > **+复制策略**。
2. 在“创建复制策略”**** 中指定策略名称。
...
5. 在 **"目标**"中，选择要在故障转移后创建 Azure VM 的订阅和资源组。 选择要在 Azure 中使用的部署模型（经典或资源管理）。
6. 选择要用于复制数据的 Azure 存储帐户。 
7. 选择 Azure VM 在故障转移后创建时所要连接的 Azure 网络和子网。
8. 选择“立即为选定的计算机配置”****，将网络设置应用到选择保护的所有计算机。 **稍后选择"配置"** 以选择每台计算机的 Azure 网络。 
9. 在“物理计算机”中，单击“+物理计算机”********。 指定名称和 IP 地址。 选择要复制的计算机的操作系统。 发现和列出服务器需要几分钟的时间。 
10. 在**属性** > **配置属性**中，选择进程服务器将用于自动在计算机上安装移动服务的帐户。
11. 在**复制设置** > **中 配置复制设置**，验证是否选择了正确的复制策略。 
12. 单击**启用复制**。 您可以在 **"设置** > **作业** > **网站恢复作业**"中跟踪**启用保护**作业的进度。 在“完成保护”作业运行之后，计算机就可以进行故障转移了。****


要监视添加的服务器，可以在**配置服务器** > **上次联系时**检查上次发现的时间。 若要添加计算机而不想要等待计划的发现时间，请突出显示配置服务器（不要单击它），并单击“刷新”****。
