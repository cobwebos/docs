---
title: 分析 Azure 站点恢复中的 Hyper-V 部署规划器报告
...
**开始日期**和**结束日期**：为生成报表而考虑的分析数据的开始日期和结束日期。 默认情况下，开始日期是开始分析的日期，结束日期是停止分析的日期。 如果报表是使用这些参数生成的，此信息可以是“StartDate”和“EndDate”值。
...
**所需的 RPO（分钟）：** 默认恢复点目标或在生成报表时为"希望RPO"参数传递的值，以估计所需的带宽。
...
**VM 兼容性**：值为“是”和“是\*”。 ******** **是**\*，对于 VM 适合[高级 SSD](../virtual-machines/windows/disks-types.md)的实例。 在这里，所分析的高变动量磁盘或 IOPS 磁盘适合的高级磁盘大小大于映射到磁盘的大小。 存储帐户决定了根据大小对磁盘分类时，可将磁盘归入哪种高级存储磁盘类型： 
* <128 GB 为 P10。
...
例如，如果磁盘的工作负载特征将其置于 P20 或 P30 类别中，但大小将其映射到较低的高级存储磁盘类型，该工具将该 VM 标记为 **"是**\*"。 该工具还建议根据建议的高级存储磁盘类型更改源磁盘大小，或者在故障转移后更改目标磁盘类型。
...
* 源 VM 平均数据改动超出了支持站点恢复数据改动限制 20 MB/s 的平均 I/O 大小。
...
**磁盘大小 （GB）**：VM 所有磁盘的总设置大小。 该工具还会显示 VM 中各个磁盘的磁盘大小。
...
高级存储 | 16 KB 或更高| 每个 VM 20 MB/s | 每个 VM 1684 GB
...
**卷 （GB） 上的可用空间**：VM 卷上的可用磁盘空间。 在计算卷上的可用空间时，会考虑由以前批次的 VM（其 VHD 位于同一卷上）用来进行增量复制的磁盘空间。 
...
**增量复制 （GB） 卷上所需的存储**：增量复制 VM 卷上所需的可用存储空间。
...
**批处理 （HH：MM） 的估计初始复制时间**：估计的初始复制时间（以小时为单位：分钟）。
