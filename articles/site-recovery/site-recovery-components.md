...
- **复制 Hyper-V VM（不包括 VMM）**：可将不由 VMM 管理的 Hyper-V VM 复制到 [Azure](/documentation/articles/site-recovery-hyper-v-site-to-azure/)。
- **迁移 VM**：可使用 Site Recovery 在区域之间[迁移 Azure IaaS VM](/documentation/articles/site-recovery-migrate-azure-to-azure/)，或者[将 AWS Windows 实例迁移](/documentation/articles/site-recovery-migrate-aws-to-azure/)到 Azure IaaS VM。目前仅支持迁移，这意味着你可以故障转移这些 VM，但无法故障回复。
...
- [旧体系结构](/documentation/articles/site-recovery-vmware-to-azure-classic-legacy/)：此体系结构不应用于新部署。
- [增强版体系结构](/documentation/articles/site-recovery-vmware-to-azure-classic/)：这是最新的体系结构，应用于所有的新部署。如果你已使用旧体系结构部署此方案，请[了解如何迁移](/documentation/articles/site-recovery-vmware-to-azure-classic-legacy/#migrate-to-the-enhanced-deployment)到增强型部署。
