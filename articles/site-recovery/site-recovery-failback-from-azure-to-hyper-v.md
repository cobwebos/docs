---
title: "在 Azure Site Recovery 中针对 Hyper-v 虚拟机进行故障回复 | Microsoft 文档"
...
在门户上，没有名为故障回复的显式手势。 故障回复是一个用来复原到主站点的步骤。 根据定义，故障转移是指将虚拟机从主（本地）站点故障转移到恢复 (Azure) 站点，故障回复是指将虚拟机从恢复站点故障转移回主站点。

启动故障转移时，边栏选项卡会显示关于作业方向的信息。 如果方向是从 Azure 到本地，则这是故障回复。

## <a name="why-is-there-only-a-planned-failover-gesture-to-failback"></a>为何只有一个已计划的故障转移手势可用来进行故障回复？
Azure 是具有高可用性的环境，你的虚拟机将始终可用。 故障回复是一个已计划活动，在该活动中，你决定进行短暂的停机以便工作负荷可以重新开始在本地运行。 这应当没有数据丢失。 因此，只有一个已计划的故障转移手势可用，这将关闭 Azure 中的 VM，下载最新更改并确保没有数据丢失。
...
在**提交**后，你可以启动*反向复制*。 这将通过从本地复制回 Azure 的方式开始保护虚拟机。 请注意，这只会复制在 Azure 中关闭 VM 后发生的更改，因此只会发送差异更改。
