...
* 如果要在经典门户中设置 Site Recovery，请按照[本文](site-recovery-vmware-to-azure-classic.md)中的详细说明进行操作。 
...
* **配置服务器**：运行 Windows Server 2012 R2 作为配置服务器的本地 VM。 在此 VM 上也安装其他 Site Recovery 组件（包括进程服务器和主目标服务器）。 有关详细信息，请参阅[方案体系结构](site-recovery-components.md#replicate-vmware-vmsphysical-servers-to-azure)和[配置服务器先决条件](site-recovery-vmware-to-azure.md#prerequisites)。
