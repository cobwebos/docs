---
title: 使用 Azure 站点恢复安装 Linux VM 故障恢复的主目标服务器
...
1.  在管理系统升级相应选项中选择“不自动更新”选项，再按 Enter。********
    ...
    > [!WARNING]
    > 由于 Azure Site Recovery 主目标服务器需要非常特定的 Ubuntu 版本，因此需确保已为虚拟机禁用内核升级。 如果启用，任意常规升级都会导致主目标服务器无法正常工作。 请确保选择 **"无自动更新"** 选项。
...
4. 在左侧窗格中，选择 **"高级** > **常规**"，然后选择屏幕右下角部分的 **"配置参数"** 按钮。
...
2. 从进程服务器复制所需的安装程序文件，并将其保存为主目录中**的最新linuxmobsvc.tar.gz。**
...
3. 格式化驱动器，然后在新驱动器上创建文件系统 **：mkfs.ext4 /dev/映射器/\<保留磁盘的多路径 id>**。
...
> [!NOTE]
> 在安装主目标服务器之前，请检查虚拟机上的 **/etc/主机**文件是否包含将本地主机名映射到与所有网络适配器关联的 IP 地址的条目。
...
等到脚本执行完成。 如果主目标成功注册，则主目标将列在门户的**站点恢复基础结构**页上。
...
2. 运行以下命令安装主目标服务器并将它注册到配置服务器。
    ...
    ```
     ...
     等到脚本执行完成。 如果成功注册主目标，则主目标将列在门户的**站点恢复基础结构**页上。
...
运行安装程序。 它会自动检测是否在 master 目标服务器上安装了代理。 要升级，请选择**Y**。 设置完成后，使用以下命令检查安装的主目标的版本：
...
主目标的安装和注册完成后，您可以看到主目标显示在**站点恢复基础结构****的主目标**部分，在配置服务器概述下。

您现在可以继续[重新保护](vmware-azure-reprotect.md)，然后是故障恢复。
