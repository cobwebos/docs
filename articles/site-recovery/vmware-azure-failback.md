---
title: 如何使用 Azure Site Recovery 从 Azure 故障回复到 VMware | Microsoft Docs
...
本文介绍如何将虚拟机从 Azure 虚拟机故障回复到本地 VMware 环境。 根据[在 Site Recovery 中故障转移](site-recovery-failover.md)教程将 VMware 虚拟机或 Windows/Linux 物理服务器从本地站点故障转移到 Azure 以后，请按本文中的说明进行故障回复。
...
- 故障回复期间，虚拟机必须位于配置服务器数据库中，否则故障回复不会成功。 因此，请确保定期执行计划的服务器备份。 如果发生灾难，需要使用原始 IP 地址还原服务器，让故障回复正常工作。
...
1. [重新保护](vmware-azure-reprotect.md)虚拟机，使其开始复制到本地站点中运行的 VMware 虚拟机。 在此过程中，还需要：
    1. 设置本地主目标：Windows 虚拟机的 Windows 主目标和 Linux 虚拟机的 [Linux 主目标](vmware-azure-install-linux-master-target.md)。
    2. 设置[进程服务器](vmware-azure-set-up-process-server-azure.md)。
    3. 启动[重新保护](vmware-azure-reprotect.md)。 这会关闭本地虚拟机，并将 Azure 虚拟机的数据与本地磁盘同步。

1. 将 Azure 中的虚拟机复制到本地站点后，启动从 Azure 到本地站点的故障转移。

1. 对数据进行故障回复以后，对本地虚拟机进行重新保护，使之开始复制到 Azure。
...
> [!IMPORTANT]
> 启动故障回复之前，请确保已完成重新保护虚拟机。 虚拟机在其运行状况为“正常”时应处于受保护状态。 若要重新保护虚拟机，请参阅[如何重新保护](vmware-azure-reprotect.md)。

1. 在“复制的项”页中选择该虚拟机，右键单击并选择“非计划的故障转移”。
2. 在“确认故障转移”中，验证故障转移方向（从 Azure 故障转移），并选择要用于故障转移的恢复点（最新恢复点，或最新的应用一致性恢复点）。 应用一致性点会在最新的时间点之后，并将导致丢失部分数据。
3. 故障转移期间，Site Recovery 会关闭 Azure 中的虚拟机。 检查故障回复是否按预期完成后，可以检查 Azure 中的虚拟机是否已关闭。
4. 需要**提交**才能从 Azure 中删除故障转移的虚拟机。请右键单击受保护项，然后单击“提交”。 某个作业将删除 Azure 中已故障转移的虚拟机。
...
对于 Windows 虚拟机，Azure Site Recovery 在故障转移过程中禁用 VMware 工具。 在 Windows 虚拟机故障回复期间，会重新启用 VMware 工具。 
...
1. 在“保管库” > “设置” > “已复制的项”中，选择已故障回复的虚拟机，并单击“重新保护”。
