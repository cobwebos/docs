...
- 对于 Hyper-V 环境，如果应用程序使用分布式事务，建议你部署[包含 SAN 复制的 Site Recovery](/documentation/articles/site-recovery-vmm-san)。

- 对于 VMware 环境，建议你部署 [VMware 到 VMware](/documentation/articles/site-recovery-vmware-to-vmware) 保护。
...
下一步是使用虚拟机和可用性组创建恢复计划。选择在步骤 1 中所用的相同 VMM 服务器作为源，并选择 Microsoft Azure 作为目标。
