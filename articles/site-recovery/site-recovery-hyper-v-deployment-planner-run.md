...
|-TargetRegion|（可选）充当复制目标的 Azure 区域。 由于 Azure 的成本因区域而异，因此可使用此参数来生成特定目标 Azure 区域的报表。 默认值为 WestUS2 或上次使用的目标区域。 请参阅[支持的目标区域](site-recovery-hyper-v-deployment-planner-cost-estimation.md#supported-target-regions)的列表。|
|-OfferId|（可选）与订阅关联的产品/服务。 默认值为 MS-AZR-0003P（即用即付）。|
|-Currency|（可选）在生成的报表中显示的成本所采用的货币。 默认为美元 ($) 或上次使用的货币。 请参阅[支持的货币](site-recovery-hyper-v-deployment-planner-cost-estimation.md#supported-currencies)的列表。|
...
* [VM-粗糙年位置](site-recovery-hyper-v-deployment-planner-analyze-report.md#vm-storage-placement-recommendation)
