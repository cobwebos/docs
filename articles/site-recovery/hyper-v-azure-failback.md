...
3. 选择**受保护的项** -> **保护组** -> \<保护组名称 >-> \<VirtualMachineName > 你想要故障回复，然后选择**计划的故障转移**。
...
7. 单击复选标记开始故障转移（故障回复）。
8. 完成初始数据同步并且已准备好关闭 Azure 中的虚拟机中，单击后**作业** > \<计划的故障转移作业 >>**完成故障转移**. 这会关闭 Azure 计算机，将最新更改传输到本地虚拟机，然后启动虚拟机。
