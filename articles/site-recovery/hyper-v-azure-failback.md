...
5. 完成数据同步和启动本地 VM 所需的时间将取决于多个因素。 要加快数据下载速度，可以配置 Microsoft 恢复服务代理以使用更多线程并行化下载。 [了解详情](https://support.microsoft.com/help/3056159/how-to-manage-on-premises-to-azure-protection-network-bandwidth-usage)。
...
1. 在保管库中选择“复制的项”，然后选择 VM****。 右键单击 VM >“计划的故障转移”****。 如果恢复计划失败，请选择计划名称，然后单击**故障转移** > **计划故障转移**。
...
3. 在“数据同步”中，选择一个选项****：
    - **在故障转移之前同步数据(仅同步增量更改)** - 此选项可最大程度地减少 VM 的停机时间，因为它可在不关闭 VM 的情况下执行同步。
        - **阶段 1：** 拍摄 Azure VM 的快照并将其复制到本地 Hyper-V 主机。 计算机将继续在 Azure 中运行。
...
3. 在**受保护项目** > **保护组保护组** > \<保护组名称> \<->虚拟机名称>中，选择要故障恢复的 VM，然后选择 **"计划故障转移**"。
4. 在“确认计划的故障转移”中，选择“如果本地虚拟机不存在，则创建它”********。
5. 在“主机名”中，选择要在其上放置 VM 的新 Hyper-V 主机服务器****。
6. 在 **"数据同步**"中，我们建议您选择在故障转移之前同步数据的选项。 此选项可以最大程度地减少 VM 的停机时间，因为它可以在不关闭 VM 的情况下执行同步。 它执行以下操作：
    - **阶段 1：** 拍摄 Azure VM 的快照并将其复制到本地 Hyper-V 主机。 计算机将继续在 Azure 中运行。
    - **第 2 阶段**：关闭 Azure VM，以便那里不发生新的更改。 最终的更改集将传输到本地服务器，本地虚拟机会启动。
    
7. 单击复选标记开始故障转移（故障回复）。
8. 初始同步完成后，您已准备好关闭 Azure VM，请单击**作业** > \<计划故障转移作业> > **完成故障转移**。 这将关闭 Azure 计算机，将最新更改传输到本地 VM，并启动 VM。
