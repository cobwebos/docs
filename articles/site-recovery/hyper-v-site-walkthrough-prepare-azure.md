---
title: "使用 Azure Site Recovery 准备 Azure 资源以将 Hyper-V VM（不含 System Center VMM）复制到 Azure | Microsoft Docs"
description: "介绍开始使用 Azure Site Recovery 将 Hyper-V VM（不含 VMM）复制到 Azure 之前需在 Azure 中准备好的资源"
...
使用本文中的说明来准备 Azure 资源，以便可以使用 [Azure Site Recovery](site-recovery-overview.md) 服务将本地 Hyper-V VM（不含 System Center VMM）复制到 Azure。
...
请确保已了解[先决条件](hyper-v-site-walkthrough-prerequisites.md)
...
- 该网络应位于与恢复服务保管库相同的区域
- Azure 门户中的 Site Recovery 可以使用在 [Resource Manager](../resource-manager-deployment-model.md) 中或经典模式下设置的网络。
