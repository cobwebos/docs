...
可以。 除了使用 Site Recovery 设置 VMware VM 的完全灾难恢复以外，还可以使用 Site Recovery 将本地 VMware VM 迁移到 Azure。 在此场景中，可将本地 VMware VM 复制到 Azure 存储。 然后从本地故障转移到 Azure。 故障转移后，应用和工作负荷可供使用并在 Azure VM 上运行。 该过程类似于设置完全灾难恢复，不过，在迁移时，无法从 Azure 故障回复。
...
不能。 从 2019 年 3 月开始，在 Azure 门户中只能复制到 Azure 托管磁盘。
...
可以。 请下载 MySQL 安装程序并将其置于 C:\Temp\ASRSetup 文件夹中。 设置配置服务器 VM 时，请接受条款，然后选择“下载并安装”。 门户将使用你添加的安装程序来安装 MySQL。
