...
**存储指南** | 确保按照[存储指南](../storage/storage-scalability-targets.md#scalability-targets-for-virtual-machine-disks)配置源 Azure 虚拟机，以免产生任何性能问题。 如果遵从默认设置，Site Recovery 会基于源配置创建所需的存储帐户。 如果自定义和选择自己的设置，请确保遵从[虚拟机磁盘的可伸缩性目标](../storage/storage-scalability-targets.md#scalability-targets-for-virtual-machine-disks)。
**网络指南** | 需要将特定 URL 或 IP 范围的 Azure VM 出站连接列入白名单。 有关更多详细信息，请参阅 [Azure 虚拟机复制网络指南](site-recovery-azure-to-azure-networking-guidance.md)一文。
