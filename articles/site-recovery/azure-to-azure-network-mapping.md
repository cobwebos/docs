...
ms.locfileid: "78396915"
---
# <a name="set-up-network-mapping-and-ip-addressing-for-vnets"></a>设置 VNet 的网络映射和 IP 寻址

本文介绍如何映射不同 Azure 区域中的两个 Azure 虚拟网络 (VNet) 实例，以及如何设置网络之间的 IP 寻址。 网络映射为启用复制时基于源网络的目标网络选择提供默认行为。
...
- 如果已经为源网络进行了网络映射，则在为更多 Vm 启用复制时，映射的目标网络将始终为默认网络。 通过从下拉列表中选择其他可用选项，可以选择更改目标虚拟网络。 
- 若要更改新复制的默认目标虚拟网络，需要修改现有的网络映射。
- 如果要修改从区域 A 到区域 B 的网络映射，请确保先删除从区域 B 到区域 A 的网络映射。删除反向映射后，修改从区域 A 到区域 B 的网络映射，然后创建相关的反向映射。

>[!NOTE]
>* 修改网络映射仅会更改新 VM 复制的默认值。 它不会影响现有复制的目标虚拟网络选择。 
>* 如果要修改现有复制的目标网络，请参阅复制项的 "计算和网络" 设置。
