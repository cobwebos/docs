---
title: VMware VM 多租户灾难恢复与 Azure 站点恢复
...
[Azure 站点恢复](site-recovery-overview.md)支持租户订阅的多租户环境。 它还支持适用于租户订阅的多租户，这些租户订阅通过 Microsoft 云解决方案提供商 (CSP) 计划创建和管理。
...
1. 通过克隆预定义的*只读*角色来创建新角色，然后为它指定一个方便的名称（如Azure_Site_Recovery，如本示例所示）。
2. 将以下权限分配给该角色：
...
3. VM 现在可以进行复制了。 验证是否仅在**复制** > **选择虚拟机**中显示租户的 VM。
