...
2. 删除由需要删除的 VMM 服务器上的云使用的任何网络映射。 在**系统中心 VMM** > **网络映射****站点恢复基础结构** > 中，右键单击网络映射>**删除**。
3. 记下 VMM 服务器的 ID。
4. 取消复制策略与要删除的 VMM 服务器上的云的关联。  在**系统中心 VMM** >  **复制策略****的站点恢复基础结构** > 中，双击关联的策略。 右键单击云“取消关联”****。
5. 删除 VMM 服务器或主动节点。 在**系统中心 VMM VMM** > **服务器****的网站恢复基础结构** > 中，右键单击服务器>**删除**。
6. 如果 VMM 服务器处于“已断开连接”状态，请对 VMM 服务器下载并运行[清理脚本](https://aka.ms/asr-cleanup-script-vmm)。 使用 **"以管理员身份运行"** 选项打开 PowerShell，以更改默认（本地计算机）作用域的执行策略。 在脚本中，指定要删除的 VMM 服务器的 ID。 脚本会从服务器中删除注册和云配对信息。
...
2. 取消关联 Hyper-V 站点的策略。 在**超 V 站点** >  **复制策略****的站点恢复基础结构** > 中，双击关联的策略。 右键单击站点 >“取消关联”****。
3. 删除 Hyper-V 主机。 在**超 V 站点** > **超 V 主机****的网站恢复基础结构** > 中，右键单击服务器>**删除**。
4. 从 Hyper-V 站点中删除所有主机后，将该站点删除。 在**超V网站** > **超V网站****的网站恢复基础设施** > 中，右键单击网站>**删除**。
...
2. 在“禁用复制”**** 页中，选择下列选项之一：
    - **禁用复制并删除（建议）** - 此选项从 Azure 站点恢复中删除复制的项目，并且计算机的复制将停止。 此外，还将清理配置服务器上的复制配置，并停止对这个受保护的服务器收取 Site Recovery 费用。 请注意，此选项仅在配置服务器处于连接状态时使用。
