...
* **未连接 VMM 服务器**：如果 VMM 服务器未连接，当你删除该服务器时，需要手动运行一个脚本才能执行清理操作。 该脚本位于 [Microsoft 库](http://aka.ms/asr-cleanup-script-vmm)中。 请记下服务器的 VMM ID，以完成手动清理过程。
* **群集中的 VMM 服务器**：如果你需要取消注册在群集中部署的 VMM 服务器，请执行以下操作：
...
## <a name="unregister-a-hyper-v-server-in-a-hyper-v-site"></a>取消注册 Hyper-V 站点中的 Hyper-V 服务器
...
## <a name="stop-protecting-a-hyper-v-virtual-machine"></a>停止保护 Hyper-V 虚拟机
如果您想要停止保护 HYPER-V 虚拟机，将需要删除对它的保护。 根据删除保护的方式，您可能需要在虚拟机上手动清除保护设置。 

### <a name="remove-protection"></a>删除保护
1. 在云属性的“**虚拟机**”选项卡中选择虚拟机，然后选择“**删除**”。
2. 在“**确认删除虚拟机**”页上有几个选项：
   
   * **禁用保护** — 如果你启用并保存此选项，虚拟机将不再受 Site Recovery 的保护。 系统将自动清理虚拟机的保护设置。
   * **从保管库删除** — 如果你选择此选项，将只从 Site Recovery 保管库中删除虚拟机。 不会影响虚拟机的本地保护设置。 你将需要手动清理设置来删除保护设置，从 Azure 订阅中删除虚拟机，以及删除保护设置（你将需要使用下面的说明手动清理这些设置）。

如果你选择删除虚拟机及其硬盘，将从目标位置删除它们。

### <a name="clean-up-protection-settings-manually-between-vmm-sites"></a>手动清理保护设置（在 VMM 站点之间）
...
### <a name="clean-up-protection-settings-manually-between-on-premises-vmm-sites-and-azure"></a>手动清理保护设置（在本地 VMM 站点与 Azure 之间）
...
### <a name="clean-up-protection-settings-manually-between-hyper-v-sites-and-azure"></a>手动清理保护设置（在 Hyper-V 站点与 Azure 之间）
...
## <a name="stop-protecting-a-vmware-virtual-machine-or-a-physical-server"></a>停止保护 VMware 虚拟机或物理服务器
...
2. 在“**删除虚拟机**”中选择下列选项之一：
   ...
   * **禁用保护(用于恢复深化和重设卷大小)** — 如果你已经执行了以下操作，则将只能查看和启用此选项：
     
     * **调整了虚拟机卷的大小** — 当你调整卷大小时，虚拟机将进入临界状态。 如果发生这种情况请选择此选项。 它将禁用保护，同时保留 Azure 中的恢复点。 在重新启用对虚拟机的保护时，调整过大小的卷的数据将被传输到 Azure。
     * **运行故障转移** — 在你已通过运行从本地 VMware 虚拟机或物理服务器故障转移到 Azure 以测试环境后，选择此选项以开始再次保护你的本地虚拟机。 此选项会禁用每个虚拟机，然后您将需要重新启用对它们的保护。 请注意：
       * 禁用虚拟机的此项设置不会影响 Azure 中的副本虚拟机。
       * 您不能从虚拟机卸载移动服务。
   * **禁用保护** — 如果你启用并保存此选项，虚拟机将不再受 Site Recovery 的保护。 系统将自动清理虚拟机的保护设置。
   * **从保管库删除** — 如果你选择此选项，将只从 Site Recovery 保管库中删除虚拟机。 不会影响虚拟机的本地保护设置。 要删除计算机上的设置并从 Azure 订阅中删除虚拟机，你将需要通过卸载移动服务来清理这些设置。
