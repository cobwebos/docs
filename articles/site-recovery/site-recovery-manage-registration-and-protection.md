...
可以通过在 Azure Site Recovery 门户中的“服务器”选项卡上删除服务器，从保管库中取消注册 VMM 服务器。注意以下事项：

-  建议在 VMM 服务器连接到 Azure 之后取消注册该服务器。这样可确保正确清理本地 VMM 服务器上的设置以及与之关联的 VMM 服务器（包含的云映射到要删除的服务器上的云的 VMM 服务器）。建议只在连接出现永久性问题时才删除未连接的服务器。
- 如果 VMM 服务器未连接，当你删除该服务器时，需要手动运行一个脚本才能执行清理操作。该脚本位于 [Microsoft 库](https://gallery.technet.microsoft.com/scriptcenter/Cleanup-Script-for-Windows-95101439)中。请记下服务器的 VMM ID，以完成手动清理过程。
- 如果你需要取消注册在群集中部署的 VMM 服务器，请执行以下操作：
...
如果你想要停止保护某个虚拟机，请执行以下操作：

1. 在云属性的“虚拟机”选项卡中选择虚拟机，然后选择“删除”。
2. “确认删除虚拟机”页上有几个选项：

	- **对虚拟机禁用保护** — 如果你启用并保存此选项，虚拟机将不再受 Site Recovery 的保护。系统将自动清理虚拟机的保护设置。
	- **停止管理虚拟机** — 如果你选择此选项，虚拟机将只从 Azure Site Recovery 服务中删除。虚拟机的本地保护设置不受影响，你需要根据以下说明手动清理这些设置。
...
### 手动清理保护设置（在 VMware 站点与 Azure 之间）
