...
3. 如果虚拟机已加入域，请确保域控制器正确运行。 可以按照下面给出的步骤执行此操作：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在同一网络中创建新的虚拟机。

    b.保留“数据库类型”设置，即设置为“共享”。  确保能够加入其中应启动故障转移的虚拟机的同一域。
...
4. 如果使用自定义 DNS 服务器，请确保可以访问该服务器。 可以按照下面给出的步骤执行此操作：
    ...
    b.保留“数据库类型”设置，即设置为“共享”。 检查虚拟机是否能够使用自定义 DNS 服务器解析名称
