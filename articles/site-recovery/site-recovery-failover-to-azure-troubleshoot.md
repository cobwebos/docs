...
1. 转到**虚拟机** > **网络**，单击所需的网络接口的名称。  ![network-interface](media/site-recovery-failover-to-azure-troubleshoot/network-interface.PNG)
...
4. 单击"**配置所需设置** > **创建新**"。 ![新建](media/site-recovery-failover-to-azure-troubleshoot/Create-New-Public-IP.png)
...
## <a name="unable-to-open-serial-console-after-failover-of-a-uefi-based-machine-into-azure"></a>故障转移基于 UEFI 的计算机到 Azure 后无法打开串行控制台

如果能够使用 RDP 连接到计算机，但无法打开串行控制台，请按照以下步骤操作：

* 如果计算机操作系统是红帽或 Oracle Linux 7.*/8.0，则在具有根权限的故障转移 Azure VM 上运行以下命令。 在命令之后重新启动 VM。

        grub2-mkconfig -o /boot/efi/EFI/redhat/grub.cfg

* 如果计算机操作系统为 CentOS 7.*，则在具有根权限的故障转移 Azure VM 上运行以下命令。 在命令之后重新启动 VM。
...
有关重新保护虚拟机的详细信息，请参阅[在故障转移到 Azure 后重新保护计算机并将其故障退到本地站点](vmware-azure-reprotect.md)。
...
## <a name="linux-master-target-registration-with-cs-fails-with-a-tls-error-35"></a>与 CS 的 Linux 主目标注册失败，TLS 错误 35 
