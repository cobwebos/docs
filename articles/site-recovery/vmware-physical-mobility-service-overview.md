...
- [推送安装](#push-installation)：当通过 Azure 门户启用保护时，站点恢复在服务器上安装移动代理。
- 手动安装：您可以通过[UI](#install-mobility-agent-through-ui)或[命令提示符](#install-mobility-agent-through-command-prompt)在每台计算机上手动安装移动服务。
- [自动部署](vmware-azure-mobility-install-configuration-mgr.md)：您可以使用配置管理器 等软件部署工具自动安装。

> [!NOTE]
> 移动代理在源计算机上使用大约 6%-10% 的内存用于 VMware VM 或物理计算机。
...
4. 如果代理安装成功但 VSS 提供程序安装失败，则会将作业状态标记为“警告”。 这不影响崩溃一致性点的生成。
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 若要生成应用程序一致性点，请参阅[我们的指南](vmware-physical-manage-mobility-service.md#install-site-recovery-vss-provider-on-source-machine)，了解如何手动完成 Site Recovery VSS 提供程序的安装。 </br>
    b.保留“数据库类型”设置，即设置为“共享”。  如果不希望生成应用程序一致性点，请[修改复制策略](vmware-azure-set-up-replication.md#create-a-policy)，将应用程序一致性点关闭。
...
5. 在**配置服务器详细信息**中，指定您配置的 IP 地址和密码。
...
**设置** | **详细信息**
--- | ---
使用情况 | 统一代理.exe \</Role MS/MT \<> /安装位置安装位置> /平台"VmWare"/静音
