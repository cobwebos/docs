...
**最佳做法** | **详细信息**
--- |---
**使用** | 确保只将配置服务器/独立进程服务器用于目标用途。 不要在该计算机上运行任何其他组件。
...
## <a name="step-1-troubleshoot-process-server-health-alerts"></a>第 1 步：对进程服务器运行状况警报进行故障排除
...
![警告][yellow] | 指定的服务未运行。 | 1. 检查服务正在运行。<br/> 2. 如果服务按预期运行，请按照以下说明[解决连接和复制问题](#check-connectivity-and-replication)。
![警告][yellow]  | 过去 15 分钟的 CPU 利用率超过 80%。 | 1. 不要添加新计算机。<br/>2. 检查使用进程服务器的 VM 数量是否符合[定义的限制](site-recovery-plan-capacity-vmware.md#capacity-considerations)，并考虑设置[额外的进程服务器](vmware-azure-set-up-process-server-scale.md)。<br/>3. 按照以下说明[解决连接和复制问题](#check-connectivity-and-replication)。
![严重][red] |  过去 15 分钟的 CPU 利用率超过 95%。 | 1. 不要添加新计算机。<br/>2. 检查使用进程服务器的 VM 数量是否符合[定义的限制](site-recovery-plan-capacity-vmware.md#capacity-considerations)，并考虑设置[额外的进程服务器](vmware-azure-set-up-process-server-scale.md)。<br/>3. 按照以下说明[解决连接和复制问题](#check-connectivity-and-replication)。<br/> 4. 如果问题仍然存在，运行 VMware/物理服务器复制[的部署计划](https://aka.ms/asr-v2a-deployment-planner)。
![警告][yellow] | 过去 15 分钟的内存使用率超过 80%。 |  1. 不要添加新计算机。<br/>2. 检查使用进程服务器的 VM 数量是否符合[定义的限制](site-recovery-plan-capacity-vmware.md#capacity-considerations)，并考虑设置[额外的进程服务器](vmware-azure-set-up-process-server-scale.md)。<br/>3. 按照与警告相关的任何说明进行操作。<br/> 4. 如果问题仍然存在，请按照以下说明[解决连接和复制问题](#check-connectivity-and-replication)。
![严重][red] | 过去 15 分钟的内存使用率超过 95%。 | 1. 不要添加新计算机，并考虑设置[额外的进程服务器](vmware-azure-set-up-process-server-scale.md)。<br/> 2. 按照与警告相关的任何说明进行操作。<br/> 3. 4. 如果问题持续出现，请遵照以下说明[排查连接和复制问题](#check-connectivity-and-replication)。<br/> 4. 如果问题仍然存在，运行 VMware/物理服务器复制问题的[部署规划器](https://aka.ms/asr-v2a-deployment-planner)。
![警告][yellow] | 过去 15 分钟的缓存文件夹可用空间小于 30%。 | 1. 不要添加新计算机，并考虑设置[额外的进程服务器](vmware-azure-set-up-process-server-scale.md)。<br/>2. 检查使用进程服务器的 VM 数量是否符合[准则](site-recovery-plan-capacity-vmware.md#capacity-considerations)。<br/> 3. 按照以下说明[解决连接和复制问题](#check-connectivity-and-replication)。
![严重][red] |  过去 15 分钟的可用空间小于 25%。 | 1. 按照与此问题的警告相关的说明进行操作。<br/> 2. 3. 遵照以下说明[排查连接和复制问题](#check-connectivity-and-replication)。<br/> 3. 如果问题仍然存在，运行 VMware/物理服务器复制[的部署计划](https://aka.ms/asr-v2a-deployment-planner)。
![严重][red] | 进程服务器有 15 分钟或更长时间未发出检测信号。 Tmansvs 服务未与配置服务器通信。 | 1) 检查进程服务器是否已启动并运行。<br/> 2. 检查进程服务器上是否运行了 tmassvc。<br/> 3. 按照以下说明[解决连接和复制问题](#check-connectivity-and-replication)。


![表键](./media/vmware-physical-azure-troubleshoot-process-server/table-key.png)


## <a name="step-2-check-process-server-services"></a>第 2 步：检查进程服务器服务
...
**部署** | **正在运行的服务**
--- | ---
**配置服务器上的进程服务器** | 进程服务器;进程服务器监视器;cx进程服务器;在 Mage 推送安装;日志上传服务（日志上传）;内奇童子军申请服务;微软 Azure 恢复服务代理（obengine）;Inage Scout VX 代理-Sentinel/前哨（svagent）;特曼斯夫夫;万维网出版服务（W3SVC）;MySQL;微软 Azure 站点恢复服务 （德拉）
...
## <a name="step-3-check-the-process-server-heartbeat"></a>第 3 步：检查进程服务器检测信号
...
## <a name="step-4-verify-time-sync-on-source-machine"></a>第 4 步：验证源计算机上的时间同步

确保复制计算机的系统日期/时间同步。[了解更多](https://docs.microsoft.com/windows-server/networking/windows-time-service/accurate-time)

## <a name="step-5-check-anti-virus-software-on-source-machine"></a>第 5 步：检查源计算机上的防病毒软件
...
## <a name="step-6-check-connectivity-from-source-machine"></a>第 6 步：检查源计算机的连接
...
## <a name="step-7-troubleshoot-an-unreachable-process-server"></a>第 7 步：对无法访问的进程服务器进行故障排除
...
## <a name="step-8-check-whether-the-process-server-is-pushing-data"></a>第 8 步：检查进程服务器是否推送数据

检查进程服务器是否主动将数据推送到 Azure。

  1. 在进程服务器上打开任务管理器（按 Ctrl+Shift+Esc）。
  2. 选择“性能”选项卡> “打开资源监视器”。********
  3. 在 **"资源监视器"** 页中，选择"**网络**"选项卡。在 **"具有网络活动的进程**"下，检查 cbengine.exe 是否主动发送大量数据。
...
1. 对于基于 IP 地址的防火墙规则：
    ...
    a） 下载[微软 Azure 数据中心 IP 范围](https://www.microsoft.com/download/details.aspx?id=41653)的完整列表。

    b) 将 IP 地址范围添加到防火墙配置，以确保防火墙允许与 Azure（以及默认的 HTTPS 端口 443）通信。

    c） 允许订阅的 Azure 区域和 Azure 西美国区域（用于访问控制和标识管理）的 IP 地址范围。
...
## <a name="step-12-verify-process-server-proxy-settings"></a>第 12 步：验证进程服务器代理设置 
...
2. 确保 Azure Site Recovery 代理使用相同的设置发送数据。
    ...
    b）**打开微软 Azure 备份**，并选择**操作** > **更改属性**。

    c) 在“代理配置”选项卡上，代理地址应与注册表设置中显示的代理地址相同。**** 如果不同，请将其更改为相同的地址。

## <a name="step-13-check-bandwidth"></a>第 13 步：检查带宽
