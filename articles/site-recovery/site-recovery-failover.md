...
故障转移后**** | **位置** | **操作**
--- | --- | ---
**运行 Windows 的 Azure VM** | 故障转移之前的本地计算机 | 若要通过 Internet 访问 Azure VM，请启用 RDP，并确保已针对“公共”**** 添加 TCP 和 UDP 规则，并在“Windows 防火墙”**** > “允许的应用”**** 中针对所有配置文件允许 RDP。<br/><br/> 要通过站点到站点连接访问 Azure VM，请使用计算机上的 RDP，并确保**在域和专用**网络的**Windows 防火墙** -> **允许的应用和功能**中允许 RDP。<br/><br/> <br/><br/> 删除所有静态永久性路由和 WinHTTP 代理。 确保操作系统 SAN 策略已设置为 **OnlineAll**。 [了解详情](https://support.microsoft.com/kb/3031135)。<br/><br/> 在触发故障转移时，请确保 VM 上没有处于挂起状态的 Windows 更新。 Windows 更新可能会在故障转移时启动，在更新完成之前，无法登录到 VM。
**运行 Linux 的 Azure VM** | 故障转移之前的本地计算机 | 确保 VM 上的安全外壳服务已设置为在系统引导时自动启动。<br/><br/> 确保防火墙规则允许 SSH 连接。


## <a name="run-a-failover"></a>运行故障转移

本过程介绍如何根据[恢复计划](site-recovery-create-recovery-plans.md)运行故障转移。 如果要为单个 VM 运行故障转移，请遵循适用于 [VMware VM](vmware-azure-tutorial-failover-failback.md)、[物理服务器](physical-to-azure-failover-failback.md)或 [Hyper-V VM](hyper-v-azure-failover-failback-tutorial.md) 的说明。


运行恢复计划故障转移，如下所示：

1. 在站点恢复保管库中，选择**恢复计划** > *recoveryplan_name*。
2. 单击 **“故障转移”**。

    ![故障转移](./media/site-recovery-failover/Failover.png)

3. 在**故障转移** > **故障转移方向**中，如果要复制到 Azure，请保留默认值。
4. 在“故障转移”中，选择要故障转移到的恢复点********。

    - **最新**：使用最新点。 此时会处理发送到 Site Recovery 服务的所有数据，并为每台计算机创建一个恢复点。 此选项提供最低 RPO（恢复点目标），因为故障转移后创建的 VM 具有触发故障转移时复制到 Site Recovery 的所有数据。
   - **最新处理**：使用此选项将 VM 故障转移到站点恢复已处理的最新恢复点。 你可在 VM 的“最新恢复点”中查看最新处理的恢复点****。 此选项提供较低的 RTO，因为无需费时处理未经处理的数据
   - **最新的应用一致性**：使用此选项将 VM 故障转移到站点恢复处理的最新应用程序一致性恢复点。
   - **已处理的最新多 VM：** 使用此选项的 VM，这些 VM 是复制组故障转移到最新常见多 VM 一致性恢复点的 VM。 其他虚拟机会故障转移到其最新处理的恢复点。 此选项仅可用于至少有一个 VM 已启用多 VM 一致性的恢复计划。
   - **最新的多 VM 应用一致性**：使用此选项，作为复制组一部分的 VM 故障转移到最新的常见多 VM 应用程序一致性恢复点。 其他虚拟机将故障转移到其最新的应用程序一致恢复点。 仅可用于至少有一个 VM 已启用多 VM 一致性的恢复计划。
   - **自定义**：不适用于恢复计划。 此选项仅可用于单个 VM 的故障转移。
...
- **故障转移**：处理数据，以便可以从中创建 Azure VM。 如果选择了**最新的**恢复点，则会基于发送到服务的数据创建恢复点。
- **开始**：使用上一步骤中处理的数据创建 Azure VM。

> [!WARNING]
> **不要取消正在进行的故障转移**：在启动故障转移之前，VM 的复制已停止。 如果取消正在进行的作业，故障转移会停止，但 VM 将不会开始复制。 无法再次启动复制。
...
- [了解如何](site-recovery-runbook-automation.md)将 Azure 自动化运行簿添加到恢复计划。
...
- **内部 IP 地址**：要在故障转移后设置 Azure VM 的内部 IP 地址，有几个选项：
...
- **外部 IP 地址**：您可以在故障转移时保留公共 IP 地址。 必须将故障转移过程中创建的 Azure VM 分配给 Azure 区域中可用的 Azure 公共 IP 地址。 可手动分配公共 IP 地址，也可通过恢复计划自动化公共 IP 地址分配过程。 [了解详情](concepts-public-ip-address-with-site-recovery.md)。
...
- [准备](vmware-azure-reprotect.md)VMware 重新保护和故障恢复。
- [故障恢复](hyper-v-azure-failback.md)超 VM。
