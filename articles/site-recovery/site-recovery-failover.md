...
| --- | --- | --- | --- |
| **故障转移到 Azure — 无网络** |不要选择目标 Azure 网络 |故障转移将检查是否在 Azure 中按预期启动测试虚拟机 |恢复计划中的所有测试虚拟机都已添加到单个云服务中并可互相连接<br/><br/>计算机在故障转移后不会连接到 Azure 网络。<br/><br/>用户可以连接到具有公共 IP 地址的测试计算机 |
| **故障转移到 Azure — 有网络** |选择目标 Azure 网络 |故障转移将检查测试计算机是否已连接到网络 |创建与 Azure 生产网络隔离的 Azure 网络，并按预期设置复制的虚拟机的基础结构使其工作。<br/><br/>测试虚拟机的子网基于某个子网，如果发生计划内或计划外故障转移，则故障转移虚拟机预期连接到该子网。 |
...
### <a name="run-a-test-failover-from-on-premises-to-azure"></a>运行从本地到 Azure 的测试故障转移
...
7. 在“**说明**”中，记录并保存与测试性故障转移相关联的任何观测结果。
...
### <a name="run-a-test-failover-from-a-primary-on-premises-site-to-a-secondary-on-premises-site"></a>运行从本地主站点到本地辅助站点的测试性故障转移
