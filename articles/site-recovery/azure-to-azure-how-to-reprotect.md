...
ms.locfileid: "77209250"
---
# <a name="reprotect-failed-over-azure-vms-to-the-primary-region"></a>重新保护已故障转移到主要区域的 Azure VM

使用[Azure Site Recovery](site-recovery-overview.md)将 Azure vm 从一个区域[故障转移](site-recovery-failover.md)到另一个区域时，将在次要区域中启动 vm，并处于不**受保护**状态。 如果要将 Vm 故障回复到主要区域，则需要执行以下操作：
...
4. 仅同步源磁盘与目标磁盘之间的更改。 将通过比较两个磁盘来计算差异，然后传输这些差异。 查看下面的，找到完成重新保护所要完成的估计时间。
5. 同步完成后，增量复制将开始，并使用复制策略创建恢复点。
...
4. 同步完成后，增量复制将开始，并使用复制策略创建恢复点。

#### <a name="estimated-time-to-do-the-reprotection"></a>完成重新保护的估计时间 

在大多数情况下，Azure Site Recovery 不会将完整的数据复制到源区域。 以下条件确定要复制的数据量：

1.  如果由于某种原因（例如资源组更改/删除）删除、损坏或无法访问源 VM 数据，则在重新保护过程中，将会发生完整 Site Recovery，因为在源区域中没有可供使用的数据。
2.  如果源 VM 数据可访问，则只会通过比较两个磁盘并进行传输来计算差异。 请查看下表以获取估计的时间。

|示例情况 | 重新保护所需的时间 |
|---|---|
|源区域有1个具有 1 TB 标准磁盘的 VM<br/>-仅使用 127 GB 数据，磁盘的其余部分为空<br/>-磁盘类型是标准版，具有 60 MiB/S 的吞吐量<br/>-故障转移后无数据更改| 大概时间：45分钟–1.5 小时<br/> -在重新保护期间，Site Recovery 将填充所有数据的校验和，将需要 127 GB/45 Mb ~ 45 分钟<br/>-Site Recovery 自动缩放需要一定的开销时间：约20-30 分钟<br/>-无出口费用 |
|源区域有1个具有 1 TB 标准磁盘的 VM<br/>-仅使用 127 GB 数据，磁盘的其余部分为空<br/>-磁盘类型是标准版，具有 60 MiB/S 的吞吐量<br/>-45 在故障转移后更改数据| 大概时间：1小时-2 小时<br/>-在重新保护期间，Site Recovery 将填充所有数据的校验和，将需要 127 GB/45 Mb ~ 45 分钟<br/>-用于应用 45 45 GB/45 MBps ~ 17 分钟的更改的传输时间<br/>-出口费用将用于 45 GB 数据更改，而不是用于校验和 |
 
## <a name="next-steps"></a>后续步骤

保护 VM 后，可以启动故障转移。 故障转移会关闭次要区域中的 VM，在主要区域中创建和启动 VM，在此过程中会出现一些短暂的停机。 建议为此过程选择合适的时间，并在启动到主站点的完全故障转移之前运行测试故障转移。 [详细了解](site-recovery-failover.md)Azure Site Recovery 故障转移。
