...
|源区域中有 1 个包含 1 TB 高级磁盘的 VM。<br/>仅使用了 127 GB 数据，磁盘的剩余存储是空的。<br/>磁盘类型为“高级”，吞吐量为 200 MBps。<br/>故障转移后未发生数据更改。| 大约时间：45-60 分钟。<br/>在重新保护期间，Site Recovery 会填充所有数据的校验和。 此操作以 80MBps 的速率运行，因此总共需要 127 GB/80 MBps 的时间（大约 27 分钟）。<br/>Site Recovery 进行自动缩放需要一些开销时间，大约为 20-30 分钟。 |
|源区域中有 1 个包含 1 TB 高级磁盘的 VM。<br/>仅使用了 127 GB 数据，磁盘的剩余存储是空的。<br/>磁盘类型为“高级”，吞吐量为 200 MBps。<br/>故障转移后有 45 GB 的数据更改。| 大约时间：1.5-2 小时。<br/>在重新保护期间，Site Recovery 会填充所有数据的校验和。 此操作以 80MBps 的速率运行，因此总共需要 127 GB/80 MBps 的时间（大约 27 分钟）。</br>传输速度约为吞吐量的 16%（即 32MBps）。 因此，应用 45 GB 的更改所需的传输时间为 45 GB/32 MBps（大约 24 分钟）。<br/>Site Recovery 进行自动缩放需要一些开销时间，大约为 20-30 分钟。 |
