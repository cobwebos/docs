...
|示例场景 | 重新保护所需的时间 |
|---|---|
|源区域中有 1 个包含 1 TB 标准磁盘的 VM<br/>仅使用了 127 GB 数据，磁盘的剩余存储是空的。<br/>磁盘类型为“标准”，吞吐量为 60 MBps。<br/>故障转移后未发生数据更改。| 大约时间：60-90 分钟。<br/> 在重新保护期间，Site Recovery 会填充所有数据的校验和。 此操作以 45MBps 的速率运行，因此总共需要 127 GB/45 MBps 的时间（大约 45 分钟）。<br/>Site Recovery 进行自动缩放需要一些开销时间，大约为 20-30 分钟。 |
|源区域中有 1 个包含 1 TB 标准磁盘的 VM<br/>仅使用了 127 GB 数据，磁盘的剩余存储是空的。<br/>磁盘类型为“标准”，吞吐量为 60 MBps。<br/>故障转移后有 45 GB 的数据更改。| 大约时间：2.5-3 小时。<br/> 在重新保护期间，Site Recovery 会填充所有数据的校验和。 此操作以 45MBps 的速率运行，因此总共需要 127 GB/45 MBps 的时间（大约 45 分钟）。<br/>传输速度约为吞吐量的 16%（即 9.6MBps）。 因此，应用 45 GB 的更改所需的传输时间为 45 GB/9.6 MBps（大约 80 分钟）。<br/>Site Recovery 进行自动缩放需要一些开销时间，大约为 20-30 分钟。 |
|源区域中有 1 个包含 1 TB 标准磁盘的 VM<br/>仅使用了 20 GB 数据，磁盘的剩余存储为空。<br/>磁盘类型为“标准”，吞吐量为 60 MBps。<br/>故障转移后，磁盘上的初始数据为 15 GB。 故障转移后有 5 GB 数据更改。 因此填充的数据总量为 20 GB。| 大约时间：1-1.5 小时。<br/>由于磁盘中填充的数据小于磁盘大小的 10%，因此我们执行完整的初始复制。<br/> 传输速度约为吞吐量的 16%（即 9.6MBps）。 因此，应用 20 GB 的更改所需的传输时间为 20 GB/9.6 MBps（大约 36 分钟）。<br/>Site Recovery 进行自动缩放需要一些开销时间，大约为 20-30 分钟。 |
|源区域中有 1 个包含 1 TB 高级磁盘的 VM。<br/>仅使用了 127 GB 数据，磁盘的剩余存储是空的。<br/>磁盘类型为“高级”，吞吐量为 200 MBps。<br/>故障转移后未发生数据更改。| 估计时间：2小时。<br/>在重新保护期间，Site Recovery 会填充所有数据的校验和。 这会在 (25MBps 上运行，最多可) 磁盘吞吐量的16%，因此所需的总时间为 127 GB/25 MBps，约87分钟。<br/>Site Recovery 进行自动缩放需要一些开销时间，大约为 20-30 分钟。 |
|源区域中有 1 个包含 1 TB 高级磁盘的 VM。<br/>仅使用了 127 GB 数据，磁盘的剩余存储是空的。<br/>磁盘类型为“高级”，吞吐量为 200 MBps。<br/>故障转移后有 45 GB 的数据更改。| 大约时间：2.5-3 小时。<br/>在重新保护期间，Site Recovery 会填充所有数据的校验和。 这会在 (25MBps 上运行，最多可) 磁盘吞吐量的16%，因此所需的总时间为 127 GB/25 MBps，约87分钟。</br>传输速度约为吞吐量的 16%（即 32MBps）。 因此，应用 45 GB 的更改所需的传输时间为 45 GB/32 MBps（大约 24 分钟）。<br/>Site Recovery 进行自动缩放需要一些开销时间，大约为 20-30 分钟。 |
