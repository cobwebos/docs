---
title: 用 Azure Monitor 日志监视 Azure Site Recovery （Log Analytics） |Microsoft Docs
...
4. 在切换中选择**Azure 诊断**。
5. 从日志列表中，选择前缀为**AzureSiteRecovery**的所有日志。 然后单击“确定”。
...
此查询绘制特定 Azure VM （ContosoVM123）的趋势图，该图跟踪数据更改率（每秒写入的字节数）和数据上传速率。 
