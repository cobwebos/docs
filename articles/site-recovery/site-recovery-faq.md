...
通过协调和自动化 Azure VM 复制（本地区域虚拟机和物理服务器到 Azure；本地计算机到辅助数据中心），Site Recovery 可帮助实现业务连续性与灾难恢复 (BCDR) 策略。 [了解详情](site-recovery-overview.md)。
...
缓存不会产生额外的费用。 复制到标准存储帐户时，此缓存存储是同一目标存储帐户的一部分。
...
### <a name="how-can-i-enforce-tls-12-on-all-on-premises-azure-site-recovery-components"></a>如何在所有本地 Azure 站点恢复组件上强制实施 TLS 1.2？
安装在复制项上的移动代理仅在 TLS 1.2 上与进程服务器通信。 但是，从配置服务器到 Azure 以及从进程服务器到 Azure 的通信可能位于 TLS 1.1 或 1.0 上。 请按照[指南](https://support.microsoft.com/en-us/help/3140245/update-to-enable-tls-1-1-and-tls-1-2-as-default-secure-protocols-in-wi)对您设置的所有配置服务器和处理服务器强制实施 TLS 1.2。
...
复制到 Azure 时，复制流量将进入 Azure 存储的公共终结点。 因此，您只能通过公共互联网或通过 ExpressRoute（微软对等互连或现有公共对等互连）进行复制。

### <a name="can-i-use-riverbed-steelheads-for-replication"></a>是否可以使用 Riverbed SteelHeads 进行复制？

我们的合作伙伴 Riverbed 会提供有关如何使用 Azure Site Recovery 的详细指导。 请查看他们的[解决方案指南](https://community.riverbed.com/s/article/DOC-4627)。
...
- Azure Site Recovery 通过公共终结点将数据复制到 Azure 存储。 您需要设置 Microsoft[对等互连](../expressroute/expressroute-circuit-peerings.md#microsoftpeering)或使用现有的[公共对等互连](../expressroute/about-public-peering.md)（用于新电路的弃用）才能使用 ExpressRoute 进行站点恢复复制。
...
* **Hyper-V：** Hyper-V VM 可以每 30 秒复制一次（高级存储除外）、5 分钟或 15 分钟。
* **Azure VM、VMware VM、物理服务器：** 此处与复制频率无关。 复制是连续的。
...
可以通过安全的 Internet 连接或者站点到站点 VPN 或 Azure ExpressRoute 访问 Azure VM。 在连接之前需要做许多准备。 [了解详情](site-recovery-test-failover-to-azure.md#prepare-to-connect-to-azure-vms-after-failover)。
...
* [适用于超 V 虚拟机](hyper-v-azure-failback.md#fail-back-to-an-alternate-location)
...
[了解](site-recovery-whats-new.md)新更新，并[获取汇总信息](service-updates-how-to.md)。
