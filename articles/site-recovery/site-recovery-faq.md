...
本文包含有关站点恢复的常见问题。如果在阅读常见问题解答后仍有问题，请在 [Azure 恢复服务论坛](https://social.msdn.microsoft.com/Forums/zh-cn/home?forum=hypervrecovmgr)上发布你的问题。
...
不支持这样做。你应该[将物理客户端计算机复制](/documentation/articles/site-recovery-vmware-to-azure)到 Azure 或辅助数据中心。


### 我可以使用站点恢复来保护哪些工作负荷？

可以使用站点恢复来保护在虚拟机或物理服务器上运行的大多数工作负荷。站点恢复可以帮助你部署应用程序感知的 DR。它除了与 Microsoft 应用程序（包括 SharePoint、Exchange、Dynamics、SQL Server 及 Active Directory）集成之外，还能与行业领先的供应商（包括 Oracle、SAP、IBM 及 Red Hat）紧密配合。你可以针对每个特定的应用程序自定义灾难恢复解决方案。[详细了解](/documentation/articles/site-recovery-workload)工作负荷保护。


### 一定需要 System Center VMM 服务器才能保护 Hyper-V 虚拟机吗？ 

不是。如果是复制到 Azure，无论 Hyper-V 主机服务器是否位于 VMM 云，都可以复制该主机服务器上的 Hyper-V VM。如果要复制到辅助数据中心，Hyper-V 主机服务器就必须在 VMM 云中托管。[了解详细信息](/documentation/articles/site-recovery-hyper-v-site-to-azure)
...
可以将运行 Windows 或 Linux 的物理服务器复制到 Azure 或辅助站点来进行保护。有关操作系统要求，请阅读[我需要什么？](/documentation/articles/site-recovery-vmware-to-azure#what-do-i-need)无论是将物理服务器复制到 Azure 还是辅助站点，都存在相同的限制。

请注意，如果本地服务器关闭，物理服务器将在 Azure 中以 VM 的形式运行。目前不支持故障回复到本地物理服务器，但可以故障回复到在 Hyper-V 或 VMware 上运行的虚拟机。

### 我可以保护哪些 VMware VM？

对于此方案，你需要一个 VMware vCenter 服务器、一个 vSphere Hypervisor 以及若干个运行 VMware 工具的虚拟机。有关确切的要求，请参阅[我需要什么？](/documentation/articles/site-recovery-vmware-to-azure#what-do-i-need)无论是将物理服务器复制到 Azure 还是辅助站点，都存在相同的限制。

### 将虚拟机复制到 Azure 需要满足任何先决条件吗？

要复制到 Azure 的虚拟机应该要符合 [Azure 要求](/documentation/articles/site-recovery-best-practices#virtual-machines)。
...
是的。可以使用 Rest API、PowerShell 或 Azure SDK 将站点恢复工作流自动化。请在[使用 PowerShell 部署站点恢复](/documentation/articles/site-recovery-deploy-with-powershell)中了解详细信息。
...
将需要一个使用[标准异地冗余存储](/documentation/articles/storage-redundancy#geo-redundant-storage)的存储帐户。只有在将 VMware 虚拟机或 Windows/Linux 物理服务器复制到 Azure 时，才支持使用[高级存储帐户](/documentation/articles/storage-premium-storage-preview-portal)。

对标准本地冗余存储的支持正在开发，请在[反馈论坛](http://feedback.azure.com/forums/256299-site-recovery/suggestions/7204469-local-redundant-type-azure-storage-support)中向我们发送有关此功能的反馈。。

### 我可以多久复制数据一次？
- **Hyper-V：**可以每隔 30 秒、5 分钟或 15 分钟复制 Hyper-V VM 一次。如果你已设置 SAN 复制，则复制将是同步的。
- **VMware 和物理服务器：**复制频率无关紧要。复制将是连续的。 

### 我可以将复制从现有的恢复站点扩展到其他站点吗？
不支持扩展扩展或链式复制。请在[反馈论坛](http://feedback.azure.com/forums/256299-site-recovery/suggestions/6097959-support-for-exisiting-extended-replication)中向我们发送有关此功能的反馈。


### 在首次复制到 Azure 时可以进行脱机复制吗？ 

不支持此操作。请在[反馈论坛](http://feedback.azure.com/forums/256299-site-recovery/suggestions/6227386-support-for-offline-replication-data-transfer-from)中向我们发送有关此功能的反馈。


### 可以从复制中排除特定的磁盘吗？

不支持此操作。请在[反馈论坛](http://feedback.azure.com/forums/256299-site-recovery/suggestions/6418801-exclude-disks-from-replication)中向我们发送有关此功能的反馈。

### 可以使用动态磁盘来复制虚拟机吗？

复制 Hyper-V 虚拟机时，支持使用动态磁盘。复制 VMware 虚拟机或物理服务器时则不支持。请在[反馈论坛](http://feedback.azure.com/forums/256299-site-recovery)中向我们发送有关此功能的反馈。

### 在故障转移到 Azure 之后，如何访问 Azure 虚拟机？ 

可以通过安全的 Internet 连接或者站点到站点 VPN 或 Azure ExpressRoute（如果有）访问 Azure VM。基于 VPN 连接的通信将发往 VM 所在的 Azure 网络上的内部端口。基于 Internet 的通信将映射到 Azure 云服务上 VM 的公共终结点。[了解详细信息](/documentation/articles/site-recovery-network-design#connectivity-after-failover)
...
故障转移不是自动的。你可以在门户中单击一下来启动故障转移，或者使用[站点恢复 PowerShell](https://msdn.microsoft.com/zh-cn/library/dn850420.aspx) 来触发故障转移。在站点恢复门户中也可以轻松进行故障回复。若要自动化，你可以使用本地 Orchestrator 或 Operations Manager 来检测虚拟机故障，然后使用 SDK 来触发故障转移。
