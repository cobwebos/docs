...
不能。 Azure 直接与服务提供商保持计费关系。 服务提供商责任为其租户生成特定的帐单。
...
可以。 在前 31 天内，每个受保护的实例不会产生任何 Azure Site Recovery 费用。 例如，你在过去 6 个月内保护了 10 个实例，然后你将第 11 个实例连接到 Azure Site Recovery，则在连接后的前 31 天内，第 11 个实例不会产生任何费用。 而前 10 个实例将继续产生 Azure Site Recovery 费用，因为它们受到保护的时间已超过 31 天。
...
复制到 Azure 时，复制流量将进入 Azure 存储的公共终结点。 因此，你只能通过 ExpressRoute （Microsoft 对等互连或现有的公共对等互连）通过公共 internet 进行复制，并且 VPN 不起作用。
...
- Azure Site Recovery 通过公共终结点将数据复制到 Azure 存储。 你需要设置 Microsoft 对[等互连](../expressroute/expressroute-circuit-peerings.md#microsoftpeering)或使用现有的[公共对等互连](../expressroute/expressroute-circuit-peerings.md#publicpeering)（不推荐用于新线路）来使用 ExpressRoute 进行 Site Recovery 复制。
...
### <a name="if-i-fail-over-to-azure-how-does-azure-make-sure-my-data-is-resilient"></a>如果故障转移到 Azure，Azure 如何确保我的数据可恢复？
