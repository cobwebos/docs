---
title: 通过 Azure 站点恢复规划超 V 灾难恢复的容量
...
Hyper-V 到 Azure 部署的[Azure 站点恢复部署规划器]（site-recovery-hyper-v-deployment-planner.md）提供以下内容：
...
3. 在“Capacity Planner”工作表中输入所需信息。**** 必须填写以下屏幕快照中标有红圈的所有字段：
   ...
   a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“选择方案”中，选择“从 Hyper-V 到 Azure”或“从 VMware/物理设备到 Azure”。************

   b.保留“数据库类型”设置，即设置为“共享”。 在“平均每日数据更改率(%)”中输入使用 [Hyper-V 容量计划工具](site-recovery-capacity-planning-for-hyper-v-replication.md)或 [Site Recovery Deployment Planner](./site-recovery-deployment-planner.md) 收集的信息。****

   c. 将 Hyper-V VM 复制到 Azure 时，不使用“压缩”设置。**** 对于压缩，请使用第三方应用程序（例如 Riverbed）。

   d.单击“下一步”。 在“保留天数”中，指定副本应保留的天数。****

   e.在“新建 MySQL 数据库”边栏选项卡中，接受法律条款，然后单击“确定”。 在“完成成批虚拟机的初始复制所需的小时数”和“初始复制批处理的虚拟机数”中，可以输入用于计算初始复制要求的设置********。 部署 Site Recovery 时，应上传整个初始数据集。
...
3. 在“工作负荷限定”工作表中输入所需的信息。**** 必须填写所有标记的字段。
   ...
   a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“处理器内核数”中指定源服务器上的内核总数。****

   b.保留“数据库类型”设置，即设置为“共享”。 在“内存分配(MB)”中指定源服务器的 RAM 大小。****

   c. **NIC 数**：指定源服务器上的网络适配器数。

   d.单击“下一步”。 在“总存储空间(GB)”中指定 VM 存储空间的总大小。**** 例如，如果源服务器有 3 个磁盘，每个磁盘的大小为 500 GB，则总的存储空间大小为 1500 GB。

   e.在“新建 MySQL 数据库”边栏选项卡中，接受法律条款，然后单击“确定”。 在“附加磁盘数”中指定源服务器的磁盘总数。****
...
* 在示例输出中，请注意以下问题：
  ...
  * VM5 和 VM6 需要高级存储帐户，并且都可以使用单个帐户。
    ...
    > [!NOTE]
    > 标准存储和高级存储的 IOPS 在 VM 级别而非磁盘级别进行计算。 标准 VM 最多可处理每个磁盘 500 IOPS。 如果磁盘的 IOPS 大于 500，则需要高级存储。 如果某个磁盘的 IOPS 超出 500 但所有 VM 磁盘的 IOPS 仍在标准 Azure VM 的支持范围内，则规划器会选取标准 VM 而不是 DS 或 GS 系列。 （Azure VM 限制是 VM 大小、磁盘数、适配器数、CPU 和内存。您需要使用相应的 DS 或 GS 系列 VM 手动更新映射 Azure 大小单元格。
