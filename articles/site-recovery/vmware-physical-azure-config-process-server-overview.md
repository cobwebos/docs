---
title: 关于 Azure 站点恢复配置/进程/主目标服务器
...
本文介绍了[站点恢复](site-recovery-overview.md)服务用于将 VMware VM 和物理服务器复制到 Azure 的配置、进程和主目标服务器。

## <a name="configuration-server"></a>配置服务器

对于本地 VMware VM 和物理服务器的灾难恢复，部署本地站点恢复配置服务器。
...
**作用** | 配置服务器在本地和 Azure 之间协调通信并管理数据复制。 | 详细了解将 [VMware](vmware-azure-architecture.md) 和[物理服务器](physical-azure-architecture.md)灾难恢复到 Azure 所需的体系结构。
...
进程服务器在故障转移和故障转移期间处理复制数据，并为本地 VMware VM 和物理服务器安装移动服务。

**设置** | **详细信息** | **链接**
--- | --- | ---
**部署**  | 默认情况下，部署配置服务器时，将安装进程服务器。 <br/><br/> 需要本地进程服务器来恢复和复制本地 VMware VM 和物理服务器。 | [了解详情](vmware-azure-architecture.md#architectural-components)。
**角色（本地**） | 从启用用于复制的计算机接收复制数据。 <br/><br/> 使用缓存、压缩和加密优化复制数据，并将其发送到 Azure 存储。 <br/><br/> 在要复制的本地 VMware VM 和物理服务器上执行站点恢复移动服务的推送安装。 <br/><br/> 执行本地计算机的自动发现。 | [了解详情](vmware-azure-enable-replication.md)。
**角色（从 Azure 故障回复）** | 从本地站点故障转移后，在 Azure 中将进程服务器设置为 Azure VM，以便处理故障回复到本地位置的事项。<br/><br/> Azure 中的进程服务器是临时的。 故障回复完成后，即可删除 Azure VM。 | [了解详情](vmware-azure-set-up-process-server-azure.md)。
**缩放** | 对于更大型的部署，可以在本地设置更多的横向扩展进程服务器。 更多的服务器可以处理更多数量的复制计算机以及更大的复制流量，从而横向扩展容量。<br/><br/> 可以在两个进程服务器之间移动计算机，以便对复制流量进行负载均衡。 | [了解详情](vmware-azure-set-up-process-server-scale.md)。
