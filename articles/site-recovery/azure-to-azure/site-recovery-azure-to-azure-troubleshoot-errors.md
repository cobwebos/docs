...
如果启用复制时未看到可供选择的 Azure VM，原因可能是 Azure VM 上留下了过时的 Site Recovery 配置。 在以下情况中，过时配置可能会留在 Azure VM 上：
