...
本文介绍如何将 Azure 虚拟机从 Azure 重新保护到本地站点。 根据[此教程](site-recovery-failover.md)将 VMware 虚拟机或 Windows/Linux 物理服务器从本地站点故障转移到 Azure 以后，请按本文中的说明进行故障回复。
...
如果要恢复到备用位置（删除了本地 VM 时），请选择针对主目标服务器配置的保留驱动器和数据存储。 故障回复到本地站点时，故障回复保护计划中的 VMware VM 将使用与主目标服务器相同的数据存储，并将在 vCenter 中创建新 VM。
如果要将 Azure VM 恢复到现有本地 VM，则应使用读/写访问权限将本地 VM 的数据存储装载在主目标服务器的 ESXi 主机上。
