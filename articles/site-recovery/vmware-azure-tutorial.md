...
> [!TIP]
> 本教程使用 OVF 模板创建配置服务器 VMware VM。 如果无法执行此操作，可以[手动设置配置服务器](physical-manage-configuration-server.md)。 

> [!TIP]
> 在本教程中，Site Recovery 将 MySQL 下载并安装到配置服务器。 如果不希望 Site Recovery 这样做，可以手动进行设置。 [了解详细信息](vmware-azure-deploy-configuration-server.md#prepare-for-mysql-installation)。
...
4. 下载配置服务器的 OVF 模板。 我们随模板提供了一个有效期为 100 天的评估许可证。 该时间过后，需获取许可证。
...
1. 选择“完成配置”以完成注册。 
2. 注册完成后，请在 Azure 门户中确认配置服务器和 VMware 服务器是否已在保管库中的“源”页上列出。 然后，选择“确定”以配置目标设置。
...
1. Site Recovery 会检查是否有一个或多个兼容的 Azure 存储帐户和网络。 在此教程系列的[第一个教程](tutorial-prepare-azure.md)中设置 Azure 组件时，你应该已经有了这些。
...
2. 在“虚拟机” > “选择虚拟机”中，选择要复制的每个虚拟机。 只能选择可以启用复制的计算机。 然后选择“确定”。
3. 在“属性” > “配置属性”中，选择进程服务器在计算机上自动安装移动服务时使用的帐户。
4. 在“复制设置” > “配置复制设置”中，检查是否选择了正确的复制策略。
5. 选择“启用复制”。 为 VM 启用复制后，Site Recovery 会安装移动服务。
6. 可以在“设置” > “作业” > “Site Recovery 作业”中，跟踪“启用保护”作业的进度。 在“完成保护”作业运行之后，计算机就可以进行故障转移了。
