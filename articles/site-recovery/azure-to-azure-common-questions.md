...
### <a name="how-does-the-free-tier-for-azure-site-recovery-work"></a>Azure 站点恢复的免费级别的工作原理是什么？
每个使用 Azure 站点恢复保护的实例在其保护期的前 31 天内均享受免费。 从第 32 天起，将按以上收费率对实例的保护进行计费。
...
Site Recovery 团队与 Azure 容量管理团队合作，计划充足的基础结构容量，并帮助确保在启动故障转移时，能够成功地将受 Site Recovery 保护的 Vm 部署到目标区域。
...
是的，可以在保护时使用 PowerShell 排除磁盘。 有关详细信息，请参阅[文章](azure-to-azure-exclude-disks.md)

### <a name="can-i-add-new-disks-to-replicated-vms-and-enable-replication-for-them"></a>能否向复制的 Vm 添加新磁盘并为其启用复制？

是的，具有托管磁盘的 Azure Vm 支持这种情况。 将新磁盘添加到启用了复制的 Azure VM 时，VM 的复制运行状况将显示警告，并显示一个说明，指出 VM 上的一个或多个磁盘可用于保护。 可以为添加的磁盘启用复制。
- 如果为添加的磁盘启用保护，则在初始复制后，该警告将消失。
- 如果选择不启用磁盘复制，则可以选择关闭该警告。
- 当你对要为其添加磁盘并为其启用复制的 VM 进行故障转移时，复制点将显示可用于恢复的磁盘。 例如，如果 VM 有一个磁盘，你添加了一个新的磁盘，则在添加磁盘之前创建的复制点会显示复制点包含“1 个磁盘（共 2 个）”。
...
例如，如果你有一个应用程序，并且每层的应用、数据库和 Web 位于不同的资源组中，则必须三次单击[复制向导](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-how-to-enable-replication#enable-replication)来保护所有层。 Site Recovery 将在三个不同的资源组中复制这三个层。
...
Site Recovery 可以在1小时内创建与应用程序一致的恢复点，最小频率为1小时。
...
可以。 如果将保留期从 24 小时增大到 72 小时，则 Site Recovery 将保存额外 48 小时的恢复点。 增加的时间会产生存储费用。 例如，如果单个恢复点包含 10 GB 的增量更改，每 GB 费用为 0.16 美元/月，则每月会产生 1.6 美元 * 48 的额外费用。
...
### <a name="how-is-capacity-assured-in-target-region-for-azure-vms"></a>Azure Vm 的目标区域中有哪些容量保证？
Site Recovery 团队与 Azure 容量管理团队合作来规划足够的基础结构容量，以帮助确保在启动故障转移时，将在目标区域中成功部署启用了灾难恢复的 Vm。
...
故障转移后无法保留生产应用程序的公共 IP 地址。
- 必须为故障转移过程中启动的工作负载分配一个在目标区域中可用的 Azure 公共 IP 资源。
- 可以手动执行此操作，也可以使用恢复计划自动执行此操作。
- 了解如何[在故障转移后设置公共 IP 地址](concepts-public-ip-address-with-site-recovery.md#public-ip-address-assignment-using-recovery-plan)。  

### <a name="can-i-retain-a-private-ip-address-during-failover"></a>在故障转移期间是否可以保留专用 IP 地址？
是的，你可以保留专用 IP 地址。 默认情况下，为 Azure VM 启动灾难恢复时，Site Recovery 将根据源资源设置创建目标资源。 -对于使用静态 IP 地址配置的 Azure Vm，Site Recovery 会尝试为目标 VM 设置相同的 IP 地址（如果未使用）。
了解如何[在故障转移期间保留 IP 地址](site-recovery-retain-ip-azure-vm-failover.md)。

### <a name="after-failover-why-is-the-server-assigned-a-new-ip-address"></a>故障转移后，服务器为什么会为其分配新的 IP 地址？

故障转移时，Site Recovery 会尽量提供 IP 地址。 如果该地址被另一个虚拟机占用，则 Site Recovery 会将下一个可用 IP 地址设为目标。
详细了解如何[为 vnet 设置网络映射和 IP 寻址](azure-to-azure-network-mapping.md#set-up-ip-addressing-for-target-vms)。
...
### <a name="what-is-a-rto-of-a-vm-failover-"></a>VM 故障转移的 RTO 是什么？
...
重新保护之后，故障回复的时间通常类似于从主要区域故障转移到次要区域所需的时间。

## <a name="capacity"></a>功能

### <a name="how-is-capacity-assured-in-target-region-for-azure-vms"></a>Azure Vm 的目标区域中有哪些容量保证？
Site Recovery 团队与 Azure 容量管理团队合作来规划足够的基础结构容量，以帮助确保在启动故障转移时能够在目标区域中成功部署启用了灾难恢复的 Vm。
...
不可以，Site Recovery 不会截获复制的数据，也不会有任何有关 Vm 上运行内容的信息。 只有协调复制与故障转移所需的元数据将发送到站点恢复服务。  
...
是的，支持在 Azure 中加密传输和[静态加密](https://docs.microsoft.com/azure/storage/storage-service-encryption)。
