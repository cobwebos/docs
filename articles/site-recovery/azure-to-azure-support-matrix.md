...
**在受支持的地理群集内跨区域迁移虚拟机（订阅内和跨订阅）** | 在“资源管理器部署模型”VM 的同一 Azure Active Directory 租户中受支持。 “经典部署模型”VM 不支持。
**在同一区域内迁移 VM** | 不支持。
...
- Oracle Enterprise Linux 6.4、6.5、6.6、6.7（运行 Red Hat 兼容内核或 Unbreakable Enterprise Kernel Release 3 (UEK3)）
