...
联系 [Azure 计费支持](https://docs.microsoft.com/azure/azure-portal/supportability/resource-manager-core-quotas-request)启用订阅，以便在目标位置中创建所需大小的 VM。 然后重试失败的操作。
...
由于 SUSE Linux 使用符号链接（或*符号链接*）来维护证书列表，请按照以下步骤操作：
...
要使站点恢复复制正常工作，需要 VM 与特定 URL 的出站连接。 如果 VM 位于防火墙后或使用网络安全组 (NSG) 规则来控制出站连接，则可能会遇到以下问题之一。 请注意，虽然我们继续支持通过 URL 进行出站访问，但不再支持使用允许的 IP 范围列表。
...
- **分辨率**
  - Azure Site Recovery 需要具有对 Office 365 IP 范围的访问权限来进行身份验证。
    如果使用 Azure 网络安全组 （NSG） 规则/防火墙代理来控制 VM 上的出站网络连接，请确保使用基于[Azure 活动目录 （AAD） 服务标记的](../virtual-network/security-overview.md#service-tags)NSG 规则允许访问 AAD。 我们不再支持基于 IP 地址的 NSG 规则。
...
- **分辨率**
  - 如果使用 Azure 网络安全组 （NSG） 规则/防火墙代理来控制 VM 上的出站网络连接，请确保使用服务标记。 我们不再支持使用通过 NSG 的 IP 地址允许列表进行 Azure 站点恢复 （ASR）。
...
要[将所需的 URL](azure-to-azure-about-networking.md#outbound-connectivity-for-urls)添加到允许列表中，请按照[网络指导文档中](site-recovery-azure-to-azure-networking-guidance.md)的步骤操作。
...
- **窗口**：[附加和初始化新磁盘](https://docs.microsoft.com/azure/virtual-machines/windows/attach-managed-disk-portal)。

- **Linux**：[在Linux中初始化一个新的数据磁盘](https://docs.microsoft.com/azure/virtual-machines/linux/add-disk)。
...
> - 如果尝试使用**虚拟机** > **设置** > **灾难恢复**来保护 VM，则操作将失败，消息为"由于 VM 上现有的陈旧资源链接而无法启用复制"。
...
### <a name="cause-1-the-resource-group-and-source-virtual-machine-are-in-different-locations"></a>原因 1：资源组和源虚拟机位于不同位置
...
### <a name="cause-3-stale-configuration"></a>原因 3：陈旧配置
...
### <a name="cause-1-the-virtual-machine-has-an-extension-installed-in-a-failed-or-unresponsive-state"></a>原因 1：虚拟机的扩展安装处于故障或无响应状态
...
### <a name="cause-1-your-vm-is-attached-to-a-network-thats-already-mapped-to-a-target-network"></a>原因 1：您的 VM 已连接到已映射到目标网络的网络
...
### <a name="cause-2-you-previously-protected-the-vm-by-using-site-recovery-and-then-you-disabled-the-replication"></a>原因 2：您以前使用站点恢复保护 VM，然后禁用复制
...
> 无法为虚拟机启用保护，因为它具有大小*为 DiskSize*的*DiskName，* 其大小小于受支持的最低大小 1024 MB。
...
## <a name="protection-was-not-enabled-because-the-grub-configuration-includes-the-device-name-instead-of-the-uuid-error-code-151126"></a><a name="enable-protection-failed-as-device-name-mentioned-in-the-grub-configuration-instead-of-uuid-error-code-151126"></a>由于 GRUB 配置包含设备名称而不是 UUID，因此未启用保护（错误代码 151126）
...
1. 将设备名称替换为其 UUID，格式为**root_UUID**=*UUID，* 然后**恢复_UUID**=*UUID*。 例如，在替换后，/boot/grub/menu.lst 中的代码行（前面已讨论）如下所示：
