...
description: 了解如何使用 Site Recovery 在 System Center VMM 云中设置本地 Hyper-v Vm 到 Azure 的灾难恢复。
...
本文介绍如何为 System Center Virtual Machine Manager （VMM）管理的本地 Hyper-v Vm 启用复制，以便通过使用[Azure Site Recovery](site-recovery-overview.md)服务实现到 Azure 的灾难恢复。 如果不使用 VMM，请[遵循本教程](hyper-v-azure-tutorial.md)。

本文是系列教程的第三篇文章，介绍如何为本地 VMware VM 设置到 Azure 的灾难恢复。 在上一教程中，我们已[准备好本地 hyper-v 环境](hyper-v-prepare-on-premises-tutorial.md)以实现到 Azure 的灾难恢复。
...
> * 设置网络映射，在 VMM VM 网络和 Azure 虚拟网络之间进行映射。
> * 创建复制策略。
> * 为 VM 启用复制。
...
6. 在 "**是否使用 System CENTER VMM 管理 hyper-v 主机？** " 中，选择 **"是"** 。
7.  选择“确定”。
...
设置源环境时，请在 VMM 服务器上安装 Azure Site Recovery 提供程序，并在保管库中注册服务器。 在每个 Hyper-v 主机上安装 Azure 恢复服务代理。

1. 在 "**准备基础结构**" 中，选择 "**源**"。
2. 在 "**准备源**" 中，选择 " **+ vmm** " 以添加 VMM 服务器。 在“添加服务器”中，检查“服务器类型”中是否已显示“System Center VMM 服务器”。
...
7. 在“同步云元数据”中，选择“将云元数据同步到 Site Recovery 门户”。 此操作在每个服务器上只需发生一次。 然后，选择“注册”。
8. 在保管库中注册服务器后，选择 "**完成**"。
...
1. 在 Microsoft Azure 恢复服务代理安装向导 ">**必备项检查**" 下，选择 "**下一步**"。 将自动安装任何缺少的先决条件。
2. 在 "**安装设置**" 中，接受安装位置和缓存位置。 缓存驱动器需要至少 5 GB 的存储空间。 我们建议使用 600 GB 或更多可用空间的驱动器。 然后，选择“安装”。
3. 安装**完成**后，请选择 "**关闭**" 以完成向导。
...
1. 在**Site Recovery 基础结构** > **网络**映射 > **网络**映射 "中，选择" **+ 网络映射**"图标。
