...
* 只能故障回复到虚拟机文件系统 (VMFS) 数据存储。 如果有 vSAN 或 RDM，重新保护和故障回复将不起作用。
...
在 Windows 虚拟机故障回复期间，VMware 工具会在故障回复时重新启用。 同样，对于 linux 虚拟机，VMware 工具会在故障回复期间重新安装在计算机上。
...
重新保护作业完成后，虚拟机将复制回 Azure，然后可以执行故障转移。
