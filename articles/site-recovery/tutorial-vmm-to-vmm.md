...
本教程介绍如何为 System Center Virtual Machine Manager (VMM) 云中托管的本地 Hyper-V VM 设置到辅助站点的灾难恢复。 本教程介绍如何执行下列操作：
...
[网络映射](site-recovery-network-mapping.md)在源和目标云中的本地 VMM VM 网络之间映射。 映射可执行以下操作：
...
5. 下载注册密钥。 安装提供程序时需要此密钥。 生成的密钥有效期为 5 天。
    ...
    ![设置源](./media/tutorial-vmm-to-vmm/source-settings.png)
...
5. 安装完成后，单击“注册”，以便在保管库中注册服务器。
    ...
    ![安装位置](./media/tutorial-vmm-to-vmm/provider-register.png)
...
2. 此时会显示与 Site Recovery 同步的 VMM 云。 选择目标云。
   ...
   ![目标](./media/tutorial-vmm-to-vmm/target-vmm.png)
...
6. 如果要通过网络进行复制，请在“初始复制方法”中指定是要启动还是计划初始复制。 要节省网络带宽，可以将它计划在非高峰时间运行。 然后单击“确定”。
     ...
     ![复制策略](./media/tutorial-vmm-to-vmm/replication-policy.png)
