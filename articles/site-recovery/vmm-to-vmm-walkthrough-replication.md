---
title: "HYPER-V 复制到辅助站点与 Azure Site Recovery 的复制策略设置 |Microsoft 文档"
description: "描述如何为 HYPER-V 虚拟机复制到辅助 VMM 站点与 Azure Site Recovery 的策略设置。"
...
配置后[网络映射](vmm-to-vmm-walkthrough-network-mapping.md)，使用本文章可设置为 HYPER-V 虚拟机 (VM) 复制到辅助站点上，复制策略使用[Azure Site Recovery](site-recovery-overview.md)。
...
- 在创建复制策略时，使用策略的所有主机必须都具有相同的操作系统。 VMM 云可包含运行不同 Windows Server 版本的 Hyper-V 主机，但此情况下需要多个复制策略。
- 可脱机执行初始复制。
