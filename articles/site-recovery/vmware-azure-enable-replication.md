...
本文介绍如何将本地 VMware 虚拟机 （VM） 复制到 Azure。
...
- 每个磁盘的大小应小于 4 TB。
- 操作系统磁盘应该是基本磁盘，而不是动态磁盘。
- 对于启用 UEFI 的第 2 代虚拟机，操作系统系列应为 Windows，引导磁盘应小于 300 GB。

## <a name="before-you-start"></a>准备工作

复制 VMware 虚拟机时，请记住此信息：
...
- 系统会每隔 15 分钟发现 VMware VM 一次。 发现后，VM 可能需要 15 分钟或更长时间才能显示在 Azure 门户中。 添加新 vCenter 服务器或 vSphere 主机时，发现可能需要 15 分钟或更长时间。
- 在门户中更新虚拟机上的环境更改可能需要 15 分钟或更长时间。 例如，VMware 工具安装。
- 您可以检查 VMware VM 的最后发现时间：请参阅**vCenter 服务器**/vSphere 主机配置服务器页面上**的最后一个联系人"在"处"** 字段。
- 若要添加要复制的虚拟机且不想要等待执行计划的发现完成，请突出显示配置服务器（不要单击它），然后选择“刷新”。****
- 启用复制时，如果已准备好虚拟机，进程服务器将自动在 VM 上安装 Azure 站点恢复移动服务。

## <a name="enable-replication"></a>启用复制

在执行此操作之前，请查看以下信息：

- Azure Site Recovery 现在会将所有新复制项直接复制到托管磁盘。 进程服务器将复制日志写入到目标区域中的缓存存储帐户。 这些日志用于在具有 命名约定的副本`asrseeddisk`托管磁盘中创建恢复点。
- PowerShell 支持复制到托管磁盘，从[Az.RecoveryServices 模块版本 2.0.0](https://www.powershellgallery.com/packages/Az.RecoveryServices/2.0.0-preview)开始提供
- 故障转移时，所选的恢复点将用于创建目标托管磁盘。
- 以前配置为复制到目标存储帐户的 VM 不受影响。
- 新虚拟机的存储帐户复制只能通过表示状态传输 （REST） API 和 PowerShell 获得。 使用 Azure REST API 版本 2016-08-10 或 2018-01-10 复制到存储帐户。
...
1. 选择进程服务器。 如果没有创建其他进程服务器，则配置服务器的内置进程服务器将在下拉菜单中可用。 每个进程服务器的运行状况根据建议的限制和其他参数指示。 选择一个正常运行的进程服务器。 无法选择[关键](vmware-physical-azure-monitor-process-server.md#process-server-alerts)进程服务器。 你可以[进行故障排除并解决](vmware-physical-azure-troubleshoot-process-server.md)错误**或者**设置一个[横向扩展进程服务器](vmware-azure-set-up-process-server-scale.md)。

   :::image type="content" source="./media/vmware-azure-enable-replication/ps-selection.png" alt-text="“启用复制源”窗口":::

   > [!NOTE]
   > 从[版本 9.24](site-recovery-whats-new.md)开始，引入了其他警报，以增强进程服务器的运行状况警报。 将站点恢复组件升级到版本 9.24 或以上，以便生成所有警报。

1. 对于**Target**，选择要在虚拟机上创建失败的订阅和资源组。 选择要在 Azure 中用于故障的 VM 的部署模型。
...
1. 对于**虚拟机** > **选择虚拟机**，请选择要复制的每个虚拟机。 只能选择可以启用复制的虚拟机。 然后选择“确定”。  如果无法查看或选择任何特定的虚拟机，请参阅[源计算机未在 Azure 门户中列出](vmware-azure-troubleshoot-replication.md#step-3-troubleshoot-source-machines-that-arent-available-for-replication)以解决此问题。

   :::image type="content" source="./media/vmware-azure-enable-replication/enable-replication5.png" alt-text="“启用复制”&gt;“选择虚拟机”窗口":::

1. 对于**属性** > **配置属性**，选择进程服务器用于在 VM 上自动安装站点恢复移动服务的帐户。 此外，根据数据改动模式选择要用于复制的目标托管磁盘的类型。
...
1. 从**复制设置** > **配置复制设置**，验证是否选择了正确的复制策略。 您可以在 **"设置** > **复制策略** > "策略_名称_ > **"编辑设置**"中修改复制策略设置。 应用于策略的更改也适用于复制和新的虚拟机。
1. 如果要将虚拟机收集到复制组中，则启用多 VM**一致性**。 指定组的名称，然后选择“确定”****。
...
1. 选择“启用复制”。**** 可以在“设置” > “作业” > “Site Recovery 作业”中，跟踪“启用保护”作业的进度****************。 **完成保护**作业运行后，虚拟机已准备好进行故障转移。
...
1. 转到 **"设置** > **复制项目**"，然后选择虚拟机。 “概要”**** 页显示有关 VM 设置和状态的信息。
...
1. 在**计算和网络** > **计算属性**中，可以更改多个 VM 属性。
   ...
   - **Azure VM 名称**：如有必要，修改名称以满足 Azure 要求。
   - **目标 VM 大小或 VM 类型**：默认 VM 大小是根据目标 Azure 区域中的磁盘计数、NIC 计数、CPU 核心计数、内存和可用 VM 角色大小等参数选择的。 Azure 站点恢复选择满足所有条件的第一个可用 VM 大小。 在故障转移之前，随时可以根据需要选择不同的 VM 大小。 VM 磁盘大小也基于源磁盘大小，并且只能在故障转移后更改。 在 [Windows 上的 VM 磁盘的可伸缩性和性能目标](/azure/virtual-machines/windows/disk-scalability-targets)中了解磁盘大小和 IOPS 速率。
   - **资源组**：您可以选择[一个资源组](/azure/azure-resource-manager/management/overview#resource-groups)，从该组中，虚拟机将成为故障转移后的一部分。 在故障转移之前，随时可以更改此设置。 故障转移之后，如果将虚拟机迁移到其他资源组，则会中断该虚拟机的保护设置。
   - **可用性集**：如果您的虚拟机需要是故障转移后的一部分，则可以选择[可用性集](/azure/virtual-machines/windows/tutorial-availability-sets)。 选择可用性集时，请注意以下信息：
     - 仅会列出属于指定资源组的可用性集。
     - 位于不同虚拟网络中的 VM 不能属于同一个可用性集。
     - 仅大小相同的虚拟机可以属于同一可用性集。

1. 还可以添加有关目标网络、子网和分配给 Azure VM 的 IP 地址的信息。
1. 在**磁盘中**，您可以看到要复制的 VM 上的操作系统和数据磁盘。

### <a name="configure-networks-and-ip-addresses"></a>配置网络和 IP 地址

您可以设置目标 IP 地址：

- 如果不提供地址，则 VM 故障使用 DHCP。
...
Microsoft 软件保障客户可以使用 Azure 混合权益来节省迁移到 Azure 的 Windows 服务器计算机的许可成本。 该权益也适用于 Azure 灾难恢复。 如果符合条件，你可以在故障转移时，将该权益分配到 Azure Site Recovery 创建的虚拟机。
...
- [详细了解如何](site-recovery-manage-registration-and-protection.md)清理注册和保护设置以禁用复制。
