...
组织需要业务连续性和灾难恢复（BCDR）策略，使工作负荷和数据在计划内和计划外停机期间保持安全和可用。 而且，恢复到正常工作条件。
...
- 近乎同步的复制，其恢复点目标（RPO）为低到30秒，以满足大多数关键业务应用的需求。
- 针对单层或多层应用程序的应用一致性快照。
- 与其他应用程序级复制技术集成 SQL Server AlwaysOn 和合作关系。 例如，Active Directory 复制、SQL AlwaysOn 和 Exchange 数据库可用性组（Dag）。
- 灵活的恢复计划，使你只需要单击一次即可恢复整个应用程序堆栈，并在计划中包括外部脚本和手动操作。
- Site Recovery 和 Azure 中的高级网络管理可以简化应用程序网络要求。 网络管理功能，例如保留 IP 地址、配置负载均衡，以及与 Azure 流量管理器集成，以实现较低的恢复时间目标（RTO）网络切换。
...
- 使用 Site Recovery 为高峰负载缩放 SQL Server，方法是在 Azure 中将_它们分解为_更大的 IaaS 虚拟机大小。
...
- 启用对 Citrix XenApp 和 XenDesktop 部署的保护。 将不同的部署层复制到 Azure： Active Directory、DNS 服务器、SQL 数据库服务器、Citrix 传递控制器、店面服务器、XenApp Master （VDA）、Citrix XenApp 许可证服务器。
