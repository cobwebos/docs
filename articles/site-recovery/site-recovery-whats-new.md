...
ms.locfileid: "76896031"
---
# <a name="whats-new-in-site-recovery"></a>Site Recovery 中的新增功能

[Azure Site Recovery](site-recovery-overview.md)服务不断更新和改进。 为了帮助你保持最新状态，本文为你提供了有关最新版本、新功能以及新内容的信息。 此页将定期更新。
...
**时更新** |  **统一安装** | **配置服务器 .ova** | **移动服务代理** | **Site Recovery 提供程序** | **恢复服务代理**
--- | --- | --- | --- | --- | ---
...
**功能** | **세부 정보**
--- | ---
**测试故障转移设置** | 设置测试故障转移时，你现在可以为测试故障转移 VM 和网络配置设置，包括 IP 地址、NSG、内部负载均衡，以及每个计算机 NIC 的公共 IP 地址。 这些设置是可选的，不更改当前行为。 如果未配置这些设置，则可在测试故障转移时选择 Azure VNet。 [了解更多](https://azure.microsoft.com/blog/customize-networking-for-dr-drills-azure-site-recovery/)。
...
**功能** | **세부 정보**
--- | ---
新进程服务器警报 | 我们添加了新的进程服务器警报。 [了解更多](vmware-physical-azure-monitor-process-server.md)。
...
**功能** | **세부 정보**
--- | ---
**复制添加的磁盘** | 为已启用灾难恢复的 Azure VM 添加的数据磁盘进行复制。 [了解更多](azure-to-azure-enable-replication-added-disk.md)。
**自动更新** | 为在启用了灾难恢复的 Azure Vm 上运行的移动服务扩展配置自动更新时，现在可以选择要使用的现有自动化帐户，而不是使用 Site Recovery 创建的默认帐户。 [了解更多](azure-to-azure-autoupdate.md)。
...
**功能** | **세부 정보**
--- | ---
**进程服务器监视** | 对于本地 VMware Vm 和物理服务器的灾难恢复，请监视服务器运行状况报告和警报的进程服务器问题并对其进行故障排除。 [了解更多](vmware-physical-azure-monitor-process-server.md)。
...
加速网络为 VM 启用单一根 i/o 虚拟化（SR-IOV），从而提高网络性能。 为 Azure VM 启用复制时，Site Recovery 会检测是否已启用加速网络。 如果是，则在故障转移后 Site Recovery 会自动在目标副本 Azure VM 上为[Windows](https://docs.microsoft.com/azure/virtual-network/create-vm-accelerated-networking-powershell#enable-accelerated-networking-on-existing-vms)和[Linux](https://docs.microsoft.com/azure/virtual-network/create-vm-accelerated-networking-cli#enable-accelerated-networking-on-existing-vms)配置加速网络。

[了解更多](azure-vm-disaster-recovery-with-accelerated-networking.md)。
...
**子目录** | 为这些目录添加的支持被设置为单独的分区或不在同一系统磁盘上的文件系统：<br/><br/> /（root）、/boot、/usr、/usr/local、/var、/etc。
...
**可用性区域中的 Azure Vm** | 可以为在可用性区域中部署的 Azure Vm 启用到另一个区域的复制。 你现在可以为 Azure VM 启用复制，并将故障转移的目标设置为单一 VM 实例、可用性集中的 VM 或可用性区域中的 VM。 此设置不会影响复制。 [阅读](https://azure.microsoft.com/blog/disaster-recovery-of-zone-pinned-azure-virtual-machines-to-another-region/)公告。
...
Site Recovery 添加了用于自动更新移动服务扩展的选项。 移动服务扩展安装在 Site Recovery 复制的每个 Azure VM 上。 启用复制时，可以选择是否允许 Site Recovery 管理对扩展的更新。

更新不需要重新启动 VM，也不会影响复制。 [了解更多](azure-to-azure-autoupdate.md)。

### <a name="pricing-calculator-for-azure-vm-disaster-recovery"></a>Azure VM 灾难恢复的定价计算器

Azure Vm 的灾难恢复产生了 VM 许可成本，并产生了网络和存储成本。 Azure 提供了一个[定价计算器](https://aka.ms/a2a-cost-estimator)，可帮助你确定这些成本。 Site Recovery 现在提供了一个基于三层应用的示例[定价估算示例](https://aka.ms/a2a-cost-estimator)，该应用使用六个包含12个标准 HDD 磁盘和6个高级 SSD 磁盘的虚拟机。

- 该示例假设数据更改为标准的一天 10 GB，高级版的数据更改为 20 GB。
- 对于特定的部署，可以更改变量来估算成本。
- 你可以指定 Vm 的数量、托管磁盘的数量和类型以及虚拟机上预期的预期总数据更改率。
- 此外，可以应用压缩因子来估算带宽成本。
...
**磁盘加密支持** | 添加了对通过 Azure AD 应用进行 Azure 磁盘加密（ADE）加密的 Azure Vm 的灾难恢复的支持。 [了解更多](azure-to-azure-how-to-enable-replication-ade-vms.md)。
...
### <a name="update-rollup-28"></a>更新汇总28
...
**跨订阅灾难恢复** | 支持将一个区域中的 Azure Vm 复制到不同订阅中的另一个区域，该区域位于同一个 Azure Active Directory 租户中。 [了解更多](https://aka.ms/cross-sub-blog)。
...
更新 " [Azure 更新](https://azure.microsoft.com/updates/?product=site-recovery)" 页上的更新。
