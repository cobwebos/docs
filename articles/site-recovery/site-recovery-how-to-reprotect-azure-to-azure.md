...
为了说明重新保护的工作流以及避免混淆，我们参考了东亚区域作为受保护计算机的主要站点，使用东南亚区域作为受保护计算机的恢复站点。 故障转移期间，虚拟机会在东南亚区域启动。 在执行故障回复前,，需要重新保护从东南亚区域回复到东亚区域的虚拟机。 本文介绍重新保护的步骤。
...
重新保护作业首先会在目标站点（在本例中为 SEA）中植入最新数据，并在该操作完成后，在故障转移回东南亚区域前复制增量。
