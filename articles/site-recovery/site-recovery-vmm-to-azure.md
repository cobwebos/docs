<properties 
	pageTitle="在 Azure 门户中使用 Azure Site Recovery 将 VMM 云中的 Hyper-V 虚拟机复制到 Azure | Azure"
...
# 在 Azure 门户中使用 Azure Site Recovery 将 VMM 云中的 Hyper-V 虚拟机复制到 Azure | Azure
...
- 你可以轻松运行从本地基础结构到 Azure 的故障转移，以及从 Azure 故障回复（还原）到本地站点中的 Hyper-V 主机服务器。 
...
**先决条件** | **详细信息**
--- | ---
**Azure 帐户**| 需要一个 [Azure](https://azure.cn/) 帐户。你可以从 [1rmb 试用版](/pricing/1rmb-trial/)开始。[详细了解](/home/features/site-recovery)站点恢复定价。 
**Azure 存储空间** | 需要使用标准 Azure 存储帐户来存储复制的数据。可以使用 LRS 或 GRS 存储帐户。我们建议使用 GRS，以便在发生区域性服务中断或无法恢复主要区域时，能够恢复数据。[了解详细信息](/documentation/articles/storage-redundancy)。此帐户必须位于与恢复服务保管库相同的区域中。<br/><br/>不支持高级存储。<br/><br/> 复制的数据存储在 Azure 空间，Azure VM 在发生故障转移时创建。<br/><br/>[详细了解](/documentation/articles/storage-introduction) Azure 存储空间。
**Azure 网络** | 你需要一个 Azure 虚拟网络，以便发生故障转移时 Azure VM 能够连接到其中。该网络必须位于与恢复服务保管库相同的区域中。
...
**Hyper-V** | Hyper-V 主机服务器必须至少运行具有 Hyper-V 角色且安装了最新更新的 Windows Server 2012 R2。<br/><br/> Hyper-V 服务器应包含一个或多个 VM。<br/><br/> 必须在 VMM 云中管理包含要复制的 VM 的 Hyper-V 主机服务器或群集。<br/><br/>Hyper-V 服务器应直接或通过代理连接到 Internet。<br/><br/>Hyper-V 服务器上应已安装文章 [2961977](https://support.microsoft.com/zh-cn/kb/2961977) 中所述的修复程序。<br/><br/>Hyper-V 主机服务器需有 Internet 访问权限，以便将数据复制到 Azure。 
**提供程序和代理** | 在部署 Azure Site Recovery 的过程中，你将在 VMM 服务器上安装 Azure Site Recovery 提供程序，并在 Hyper-V 主机上安装恢复服务代理。提供程序和代理需要通过 Internet 直接连接或通过代理连接到 Azure。请注意，不支持基于 HTTPS 的代理。VMM 服务器和 Hyper-V 主机上的代理服务器应该允许访问：<br/><br/> *.hypervrecoverymanager.windowsazure.cn <br/><br/> *.accesscontrol.chinacloudapi.cn <br/><br/> *.backup.windowsazure.cn <br/><br/> *.blob.core.chinacloudapi.cn <br/><br/> *.store.core.chinacloudapi.cn<br/><br/>如果 VMM 服务器上存在基于 IP 地址的防火墙规则，请检查这些规则是否允许与 Azure 通信。需要允许 [Azure 数据中心 IP 范围](https://www.microsoft.com/download/confirmation.aspx?id=41653)和 HTTPS (433) 协议。<br/><br/>允许订阅的 Azure 区域的 IP 地址范围。<br/><br/>此外，VMM 服务器上的代理服务器需有权访问 https://www.msftncsi.com/ncsi.txt
...
**先决条件** | **详细信息**
--- | ---
**受保护的 VM** | 在故障转移 VM 之前，需确保要分配到 Azure VM 的名称符合 [Azure 先决条件](/documentation/articles/site-recovery-best-practices#azure-virtual-machine-requirements)。可以在启用 VM 的复制后修改此名称。<br/><br/>受保护计算机上单个磁盘的容量不应超过 1023 GB。一个 VM 最多可以有 64 个磁盘（因此最大容量为 64 TB）。<br/><br/> 不支持共享的磁盘来宾群集。<br/><br/> 不支持统一可扩展固件接口 (UEFI)/可扩展固件接口 (EFI)。<br/><br/> 如果源 VM 有 NIC 组合，它在故障转移到 Azure 之后将转换成单个 NIC。<br/><br/>不支持保护运行 Linux 的、使用静态 IP 地址的 VM。

## 准备部署

若要准备部署，需要：

1. [设置 Azure 网络](#set-up-an-azure-network)，Azure 
2. VM 在故障转移后将位于其中。 
2. 针对复制的数据[设置 Azure 存储帐户](#set-up-an-azure-storage-account)。
4. [准备 VMM 服务器](#prepare-the-vmm-server)以便进行站点恢复部署。 
5. [准备网络映射](#prepare-for-network-mapping)。设置网络，以便在站点恢复部署期间配置网络映射。

### 设置 Azure 网络

需要一个 Azure 网络，以便在故障转移后创建的 Azure VM 进行连接。
...
- 建议在开始之前先设置网络。否则，需要在站点恢复部署期间执行此操作。


### 设置 Azure 存储帐户

- 需要使用标准 Azure 存储帐户来保存复制到 Azure 的数据。该帐户必须位于与恢复服务保管库相同的区域中。
- 根据用于故障转移 Azure VM 的资源模型，你将在 [ARM 模式](/documentation/articles/storage-create-storage-account)或[经典模式](/documentation/articles/storage-create-storage-account-classic-portal)下设置帐户。
- 建议在开始之前设置帐户。否则，需要在站点恢复部署期间执行此操作。

### 准备 VMM 服务器

- 确保 VMM 服务器符合[先决条件](#on-premises-prerequisites)。
- 在站点恢复部署期间，可以指定 VMM 服务器上的所有云应可在 Azure 门户中使用。如果只希望特定云出现在门户中，可以在 VMM 管理控制台中对此云启用该设置。 


### 准备网络映射

需要在站点恢复部署期间设置网络映射。网络映射将在源 VMM VM 网络与目标 Azure 网络之间映射，以便：
...
4. [创建新资源组](../resource-group-portal.md#create-resource-group)或选择现有的资源组。指定 Azure 区域。计算机将复制到此区域。若要查看受支持的区域，请参阅 [Azure Site Recovery 价格详细信息](/home/features/site-recovery/#price)中的“地域可用性”。
4. 若要从仪表板快速访问保管库，请单击“固定到仪表板”，然后单击“创建保管库”。
...
站点恢复提供的“快速启动”体验可帮助你尽快完成部署。“快速启动”将检查先决条件，并引导你以正确的顺序完成站点恢复部署步骤。

在“快速启动”中，选择要复制的计算机类型，以及要复制到的位置。设置本地服务器、Azure 存储帐户和网络。创建复制策略，并执行容量规划。设置好基础结构之后，可以启用 VM 复制。可对特定计算机运行故障转移，或创建恢复计划以故障转移多个计算机。
...
4. 安装完成后，单击“注册”以在保管库中注册 VMM 服务器。
5. 在“Internet 连接”中，指定在 VMM 服务器上运行的提供程序如何通过 Internet 连接到站点恢复。

	- 如果你希望提供程序直接进行连接，请选择“不使用代理直接连接”。
	- 如果你希望使用当前已在服务器上设置的代理进行连接，请选择“使用现有代理设置进行连接”。
	- 如果现有代理要求身份验证，或者你希望使用自定义代理，请选择“使用自定义代理设置进行连接”。
...
6. 在“保管库设置”页中，单击“浏览”以选择密钥文件。指定 Azure Site Recovery 订阅和保管库名称。

	![服务器注册](./media/site-recovery-vmm-to-azure/provider4.png)
...
7. 接受或修改用于保存为数据加密自动生成的 SSL 证书的位置。如果在 Azure Site Recovery 门户中为受 Azure 保护的云启用数据加密，则会使用此证书。请确保该证书安全。运行故障转移到 Azure 时，如果已启用数据加密，需要使用该证书来解密。
	...
	![服务器注册](./media/site-recovery-vmm-to-azure/provider5.png)

8. 在“服务器名称”中，指定一个友好名称以在保管库中标识该 VMM 服务器。在群集配置中，请指定 VMM 群集角色名称。
13. 如果要将 VMM 服务器上所有云的元数据与保管库同步，请启用“同步云元数据”。此操作在每个服务器上只需执行一次。如果你不希望同步所有云，可以将此设置保留为未选中状态并在 VMM 控制台中的云属性中分别同步各个云。单击“注册”完成此过程。

	![服务器注册](./media/site-recovery-vmm-to-azure/provider6.png)

9. 注册随即开始。注册完成后，服务器将显示在保管库的“设置”>“服务器”边栏选项卡中。
...
2. 在权限提升的命令提示符下，运行以下命令提取解压缩提供程序安装程序：
...
2. 在“先决条件检查”页上，单击“下一步”。将自动安装任何缺少的必备组件。
...
4.	如果尚未创建 Azure 网络并想要使用 ARM 创建一个网络，请单击“+网络”以内联方式执行该操作。在“创建虚拟网络”边栏选项卡上，指定网络名称、地址范围、子网详细信息、订阅和位置。该网络应位于与恢复服务保管库相同的位置。
...
- 确认 VMM 服务器上的虚拟机已连接到 VM 网络，并且至少创建了一个 Azure 虚拟网络。注意，可以将多个 VM 网络映射到单个 Azure 网络。
...
5. 在“目标网络”中，选择副本 Azure VM 启动后所在的 Azure 网络。然后，单击“确定”。

	![网络映射](./media/site-recovery-vmm-to-azure/network-mapping2.png)

下面是网络映射开始时发生的事情：

- 映射开始时，源 VM 网络上的现有 VM 将连接到目标网络。进行复制时，连接到源 VM 网络的新 VM 连接到映射的 Azure 网络。 
...
3. 在“初始复制开始时间”中，指定开始初始复制的时间。复制通过 Internet 带宽进行，因此你可能需要将它安排在非繁忙时间。
5. 在“加密 Azure 上存储的数据”中，指定是否加密 Azure 存储空间中的静态数据。然后，单击“确定”。

	![复制策略](./media/site-recovery-vmm-to-azure/gs-replication2.png)

6. 当创建新策略时，该策略自动与 VMM 云关联。单击“确定”。可以在“设置”>“复制”> 策略名称 >“关联 VMM 云”中，将其他 VMM 云（及其中的 VM）与此复制策略相关联。
...
1.	单击“下载”以下载该工具，然后将它运行。[阅读](/documentation/articles/site-recovery-capacity-planner)工具随附的文章。
...
1. 在注册表中导航到 **HKEY\_LOCAL\_MACHINE\\SOFTWARE\\Microsoft\\Microsoft Azure Backup\\Replication**。
	...
	- 修改 **UploadThreadsPerVM** 值（如果该项不存在，请创建该项），以控制用于磁盘复制的线程数。 
...
5. 选择 Azure VM 在故障转移后启动时所要启动的 Azure 网络和子网。选择“立即为选定的计算机配置”，将网络设置应用到选择保护的所有计算机。选择“稍后配置”以选择每个计算机的 Azure 网络。如果要使用与现有不同的网络，可以[创建一个](#set-up-an-azure-network)。若要使用 ARM 模型创建网络，请单击“新建”。若要使用经典模型创建网络，请[在 Azure 门户中](/documentation/articles/virtual-networks-create-vnet-classic-pportal)执行该操作。选择适用的子网。然后，单击“确定”。
6. 在“虚拟机”>“选择虚拟机”中，单击并选择要复制的每台计算机。只能选择可以启用复制的计算机。然后，单击“确定”。

	![启用复制](./media/site-recovery-vmm-to-azure/enable-replication5.png)

5. 在“属性”>“配置属性”中，为选定的 VM 选择操作系统和 OS 磁盘。然后，单击“确定”。可以稍后再设置其他属性。
...
3. 在“计算和网络”>“计算属性”中，可以指定 Azure VM 名称和目标大小。根据需要修改名称，使其符合 [Azure 要求](/documentation/articles/site-recovery-best-practices#azure-virtual-machine-requirements)。你还可以查看和修改目标网络、子网的相关信息，以及要分配到 Azure VM 的 IP 地址。注意以下事项：
...
4. 单击“确定”开始故障转移。若要跟踪进度，可以单击 VM 以打开其属性，或者在“设置”>“站点恢复作业”中选择“测试故障转移”作业。
...
3. 可以在“复制的项”、“恢复计划”和“站点恢复作业”磁贴中管理和监视复制。可以在“设置”->“作业”->“站点恢复作业”中深入到不同的作业。
