...
2. 在**故障转移**中选择要故障转移**的恢复点**。 可以使用以下选项之一：
...
3. 如果希望 Site Recovery 在触发故障转移之前尝试关闭源计算机，请选择“在开始故障转移前关闭计算机”****。 即使关机失败，故障转移也仍会继续。 您可以按照 **"作业**"页上的故障转移进度进行操作。
...
1. 在"**设置** > **复制项**>保管库中，右键单击在>**重新保护**上出现故障的计算机。
2. 在“重新保护”中，确保选择“Azure 到本地”********。
3. 指定本地主目标服务器和进程服务器。
4. 在“数据存储”中，选择要将本地磁盘恢复到的主目标数据存储****。
       - 如果本地 VM 已删除或不存在，并且需要创建新磁盘，请使用此选项。
       - 如果磁盘已存在，则忽略此设置，但确实需要指定值。
...
1. 在"**设置**>**复制项目**>保管库中，选择失败的回 VM，然后单击 **"重新保护**"。
