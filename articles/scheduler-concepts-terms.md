...
|**作业集合**|一个作业集合包含一组作业，并且维护该集合内各作业共享的设置、配额和中止值。一个作业集合由某一订阅所有者创建，并且基于使用情况或应用程序边界将作业组合在一起。它被约束到一个区域。通过作业集合，还可以强制执行配额以便约束对该集合中所有作业的使用；该配额包括 MaxJobs 和 MaxRecurrence。|
...
如下所示为具有计划程序作业的一个全面的示例。后面的部分中将提供详细信息。
...
1.  频率：分钟、小时、天、周、月、年之一  

2.  间隔：针对重复周期的按给定频率的间隔  

3.  规定的计划：指定重复周期的分钟、小时、星期几、月和月中第几天  

4.  计数：重复周期的计数  

5.  结束时间：在指定的结束时间后将不执行任何作业  
...
若要设置重试策略，可指定两个附加设置值：重试间隔 (**retryInterval**) 和重试次数 (**retryCount**)。

重试间隔使用 **retryInterval** 对象指定，表示两次重试之间的时间间隔。其默认值为 1 分钟，其最小值为 1 分钟，其最大值为 18 个月。它使用 ISO 8601 格式定义。同样，重试次数的值使用 **retryCount** 对象指定；它是尝试重试的次数。其默认值为 5，其最大值为 20。**retryInterval** 和 **retryCount** 都是可选的 - 如果 **retryType** 设为 **fixed** 并且未显式指定任何值，则为它们赋予默认值。
