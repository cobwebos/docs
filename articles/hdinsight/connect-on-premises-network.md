...
1. 配置虚拟网络以使用自定义 DNS 服务器，而不是默认 Azure 递归解析程序。
1. 配置自定义 DNS 服务器和本地 DNS 服务器之间的转发。

这些配置启用以下行为：

* 对虚拟网络具有 DNS 后缀的完全限定域名的请求已转发至自定义 DNS 服务器。____ 然后，自定义 DNS 服务器将这些请求转发至 Azure 递归解析程序，Azure 递归解析程序将返回 IP 地址。
...
1. 在“基本信息”选项卡中输入以下信息：____  
    ...
    |身份验证类型 | __密码__或__SSH 公钥__：SSH 帐户的身份验证方法。 我们建议使用公钥，因为它们更安全。 本示例使用**密码**。  有关详细信息，请参阅[创建并使用用于 Linux VM 的 SSH 密钥](../virtual-machines/linux/mac-create-ssh-keys.md)文档。|
...
4. 在“网络”选项卡中，输入以下信息：****
    ...
    | --- | --- |
    |虚拟网络 | 选择此前创建的虚拟网络。|
    |子网 | 选择前面创建的虚拟网络的默认子网。 请勿选择 VPN 网关所用的子网____。|
    |公共 IP | 使用自动填充的值。  |
...
创建虚拟机后，您将收到"**部署成功"** 通知，并带有 **"转到资源**"按钮。  选择“转到资源”****，转到新的虚拟机。  在新虚拟机的默认视图中，按照以下步骤确定关联的 IP 地址：
...
1. 使用 SSH 连接到虚拟机的公共 IP 地址____。 将 `sshuser` 替换为创建 VM 时指定的 SSH 用户帐户。 以下示例将在 40.68.254.142 连接到虚拟机：
...
4. 在 SSH 会话中，请使用以下命令：
    ...
    `icb0d0thtw0ebifqt0g1jycdxd.ex.internal.cloudapp.net` 文本是该虚拟网络的 DNS 后缀____。 保存此值，因为它在以后使用。
...
7. 若要验证该绑定是否可以解析本地网络中的资源名称，请使用以下命令：
    ...
    >
    > 将 `10.0.0.4` 替换为虚拟网络中自定义 DNS 服务器的内部 IP 地址____。
...
1. 从左侧菜单，导航到**所有服务** > **网络** > **虚拟网络**。

2. 从列表中选择虚拟网络，此时会打开虚拟网络的默认视图。  

3. 在默认视图中的“设置”下，选择“DNS 服务器”********。  

4. 选择“自定义”，然后输入自定义 DNS 服务器的专用 IP 地址____****。
...
有关如何配置 DNS 服务器的具体步骤，请参考 DNS 服务器软件文档。 查找如何配置条件转发器的步骤____。
...
配置本地 DNS 服务器后，可以使用`nslookup`本地网络验证是否可以解析虚拟网络中的名称。 以下示例 
...
2. 对于步骤 1 中确定的 IP 地址，允许该 IP 地址的入站流量。
   ...
   * 如果您使用的__是 NSG：__ 允许 IP 地址端口__443__上的__入站__流量。
   * 如果您使用的是__UDR__：为 IP 地址设置到__Internet__路由的下__一跃点__类型。
...
> * 在配置的高级设置部分，必须选择前面创建的虚拟网络和子网____。

## <a name="connecting-to-hdinsight"></a>连接到 HDInsight

关于 HDInsight 的大多数文档都假定能够通过 Internet 访问群集。 例如，可以通过 `https://CLUSTERNAME.azurehdinsight.net` 连接到该群集。 此地址使用公共网关，如果您使用 NSG 或 UDR 限制从 Internet 访问，则公共网关不可用。

一些文档在通过 SSH 会话连接到群集时还引用了 `headnodehost`。 此地址仅从群集中的节点可用，并且对于通过虚拟网络连接的客户端不可用。
