---
title: Apache Storm on HDInsight 简介 | Microsoft Docs
...
> [!NOTE]
> 你也可以使用 [Azure CLI](../xplat-cli-install.md) 或 [Azure PowerShell](../powershell-install-configure.md) 预配 Storm 群集。
> 
> 
...
### 易于使用
**对于基于 Linux 的 Storm on HDInsight 群集**，可以使用 SSH 连接到群集，并使用 `storm` 命令启动和管理拓扑。此外，还可以使用 Ambari 监视 Storm 服务和 Storm UI，从而监视和管理正在运行的拓扑。
...
**对于基于 Windows 的 Storm on HDInsight 群集**，HDInsight Tools for Visual Studio 允许你创建 C# 和混合 C#/Java 拓扑，然后将它们提交到 Storm on HDInsight 群集。
...
有关 HDInsight Tools for Visual Studio 的详细信息，请参阅 [HDInsight Tools for Visual Studio 入门](hdinsight-hadoop-visual-studio-tools-get-started.md)。
...
storm-starter 示例位于基于 Linux 的群集上的 **/usr/hdp/current/storm-client/contrib/storm-starter** 目录中，和基于 Windows 的群集上的 **%storm\_home%\\contrib\\storm-starter** 目录中。
