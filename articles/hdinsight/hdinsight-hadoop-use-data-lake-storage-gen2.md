...
有关使用数据湖存储 Gen2 的群集创建选项的完整比较，请参阅[比较存储选项以与 Azure HDInsight 群集一起使用](hdinsight-hadoop-compare-storage-options.md)。
...
1. 在左上角，单击“创建资源”****。
1. 在搜索框中，键入**分配的用户**，然后单击 **"用户分配托管标识**"。
1. 单击 **“创建”**。
1. 输入托管标识的名称，选择正确的订阅、资源组和位置。
1. 单击 **“创建”**。

有关托管标识在 Azure HDInsight 中的工作方式的详细信息，请参阅[Azure HDInsight 中的托管标识](hdinsight-managed-identities.md)。
...
1. 在“创建存储帐户”**** 屏幕上：
    1. 选择正确的订阅和资源组。
    1. 输入 Data Lake Storage Gen2 帐户的名称。 有关存储帐户命名约定的详细信息，请参阅为[Azure 资源命名约定](/azure/azure-resource-manager/management/resource-name-rules#microsoftstorage)。
    1. 单击“高级”选项卡****。
    1. 单击 **Data Lake Storage Gen2** 下的“分层命名空间”**** 旁边的“启用”****。
    1. 单击 **"审阅" = 创建**。
    1. 单击 **"创建"**

有关创建存储帐户期间的其他选项的详细信息，请参阅[快速入门：创建 Azure 数据湖存储 Gen2 存储帐户](../storage/blobs/data-lake-storage-quickstart-create-account.md)。
...
1. 在“添加角色分配”窗口中，选择“存储 Blob 数据所有者”角色********。 然后，选择具有托管标识和存储帐户的订阅。 接下来，搜索并找到之前创建的用户分配托管标识。 最后，选择托管标识，该标识将列在 **"选定成员**"下。
...
1. 此初始设置完成后，可通过门户创建群集。 群集必须与存储帐户位于同一 Azure 区域中。 在群集创建菜单的 **"存储"** 选项卡中，选择以下选项：
    ...
    * 在**主存储帐户**下，搜索并选择新创建的数据存储存储 Gen2 存储帐户。
    ...
    * 在 **"标识"** 下，选择新创建的用户分配的托管标识。
    ...
    > [!NOTE]
    > * 要添加辅助数据存储湖存储 Gen2 帐户，只需将之前创建的托管标识分配给要添加的新数据存储湖存储 Gen2 存储帐户即可。 请注意，不支持通过 HDInsight 上的"额外存储帐户"选项卡添加辅助数据存储库 Gen2 帐户。
    > * 您可以在 HDInsight 使用的 Azure 存储帐户上启用 RA-GRS 或 RA-ZRS。 但是，不支持针对 RA-GRS 或 RA-ZRS 辅助终结点创建群集。
...
## <a name="create-a-cluster-with-data-lake-storage-gen2-through-azure-powershell"></a>通过 Azure PowerShell 创建具有数据存储湖存储第 2 代的群集
...
有几种方法可以从 HDInsight 群集访问数据存储 Gen2 中的文件。
