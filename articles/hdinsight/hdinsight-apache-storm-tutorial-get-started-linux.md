...
2. 使用以下命令启动示例拓扑：
   ...
   > [!NOTE]
   > 由于 HDinsight 将与 Storm 的新版本一起更新，因此文件名的 `0.10.0.2.4.2.4-5` 部分可能会更改。
   > 
   > 
   ...
   > [!NOTE]
   > 将拓扑提交到群集时，必须先复制包含群集的 jar 文件，然后再使用 `storm` 命令。可以从文件所在的客户端使用 `scp` 命令实现此目的。例如：`scp FILENAME.jar USERNAME@CLUSTERNAME-ssh.azurehdinsight.net:FILENAME.jar`
   > 
...
1. 将 Web 浏览器打开到 https://CLUSTERNAME.azurehdinsight.net/stormui，其中 **CLUSTERNAME** 是群集的名称。此时将打开 Storm UI。
