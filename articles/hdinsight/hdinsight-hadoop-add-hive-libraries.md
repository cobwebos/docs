---
title: 群集创建期间的 Apache 蜂巢库 - Azure HDInsight
description: 了解如何在群集创建期间将 Apache Hive 库（jar 文件）添加到 HDInsight 群集。
...
了解如何在 HDInsight 上预加载 [Apache Hive](https://hive.apache.org/) 库。 本文档包含有关在群集创建过程中使用脚本操作预加载库的信息。 使用本文档中的步骤添加的库在 Hive 中具有全局性版本 - 无需使用[ADD JAR](https://cwiki.apache.org/confluence/display/Hive/LanguageManual+Cli)来加载它们。

## <a name="how-it-works"></a>工作原理

创建群集时，可以使用脚本操作在创建群集节点时对其进行修改。 本文档中的脚本接受一个参数，即库的位置。 此位置必须位于 Azure 存储帐户中，并且库必须作为 jar 文件存储。
...
使用本文中的脚本操作使库在使用适用于**WebHCat**的 Hive 客户端和**HiveServer2**时可用。
...
* 在创建期间，包含 jar 文件的库的存储帐户**必须**链接到 HDInsight 群集。 它必须是默认存储帐户，或者通过__存储帐户设置添加的帐户__。
...
1. 使用[Linux 上的预配 HDInsight 群集](hdinsight-hadoop-provision-linux-clusters.md)中的步骤开始预配群集，但不完成预配。 也可以使用 Azure PowerShell 或 HDInsight .NET SDK 来使用此脚本创建群集。 有关使用这些方法的详细信息，请参阅[使用脚本操作自定义 HDInsight 群集](hdinsight-hadoop-customize-cluster-linux.md)。 对于 Azure 门户，从 **"配置 + 定价**"选项卡中选择 **"添加脚本操作**"。

1. 对于**存储**，如果包含 jar 文件库的存储帐户与群集中使用的帐户不同，则完成**其他存储帐户**。
...
1. 对于**脚本操作**，请提供以下信息：
    ...
    > [!NOTE]
    > 对于 Apache Spark 2.1，使用此 bash`https://hdiconfigactions.blob.core.windows.net/linuxsetupcustomhivelibsv01/setup-customhivelibs-v00.sh`脚本 URI： 。

1. 继续按[预配 Linux 上的 HDInsight 群集](hdinsight-hadoop-provision-linux-clusters.md)中所述预配群集。

群集创建完成后，您可以使用通过 Hive 通过此脚本添加的 jar，而无需使用 语句`ADD JAR`。
