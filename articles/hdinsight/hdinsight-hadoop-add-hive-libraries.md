...
---
# <a name="add-hive-libraries-during-hdinsight-cluster-creation"></a>在 HDInsight 群集创建过程中添加 Hive 库
...
> 使用本文中的脚本操作使库可用于以下方案：
>
> * **基于 Linux 的 HDInsight** - 使用 **Hive 命令行**、**WebHCat** (Templeton) 和 **HiveServer2** 时。
> * **基于 Windows 的 HDInsight** - 使用 **Hive 命令行** 和 **WebHCat** (Templeton) 时。
>
>
...
* 在创建期间，包含 jar 文件的库的存储帐户**必须**链接到 HDInsight 群集。 可以通过两种方式之一来完成此操作：

  * 在群集的默认存储帐户上的容器中。
  * 在链接的存储容器上的容器中。 例如，在门户中可以使用“可选配置”、“链接存储帐户”添加额外的存储。
* 必须指定容器的 WASB 路径作为脚本操作的参数。 例如，如果 jar 存储在名为 **mystorage** 的存储帐户上名为 **libs** 的容器中，则该参数应为 **wasbs://libs@mystorage.blob.core.windows.net/**。
  ...
  >
  > 如果尚未创建存储帐户，可以通过 [Azure 门户](https://portal.azure.com)创建该帐户。 然后可以使用实用程序（如 [Azure 存储资源管理器](http://storageexplorer.com/)）在帐户中创建一个新容器并将文件上传到该容器。
  >
  >

## <a name="create-a-cluster-using-the-script"></a>使用脚本创建群集。
> [!NOTE]
> 以下步骤创建新的基于 Linux 的 HDInsight 群集。 若要创建新的基于 Windows 的群集，创建群集时请选择 **Windows** 作为群集 OS，并使用 Windows (PowerShell) 脚本而不是 bash 脚本。
>
...
在本文档中，你已学习如何在群集创建过程中将 jar 文件中包含的 Hive 库添加到 HDInsight 群集中。 有关使用 Hive 的详细信息，请参阅[将 Hive 与 HDInsight 配合使用](hdinsight-use-hive.md)
