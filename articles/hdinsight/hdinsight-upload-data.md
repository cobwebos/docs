...
**原因**：写入 Azure 存储时，HDInsight 群集上的 HBase 默认为块大小 256 KB。 尽管这对 HBase API 或 REST API 来说可良好运行，但使用 `hadoop` 或 `hdfs dfs` 命令行实用工具时则会导致错误。
...
1. 在浏览器中，转到群集的 Ambari Web UI。 这是 `https://CLUSTERNAME.azurehdinsight.net`的，其中 `CLUSTERNAME` 是群集的名称。
