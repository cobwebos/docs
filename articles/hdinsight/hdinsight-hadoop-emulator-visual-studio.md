...
确保 Hortonworks 沙盒正在运行，然后遵循 [Hadoop 生态系统入门](hdinsight-hadoop-emulator-get-started.md#set-passwords)中的步骤配置 SSH `root` 帐户和 Ambari `admin` 帐户的密码。 从 Visual Studio 连接到沙盒时，将使用这些密码。
