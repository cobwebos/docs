...
确保 Hortonworks 沙盒正在运行，然后按照 [Hadoop 生态系统入门](hdinsight-hadoop-emulator-get-started.md#set-sandbox-passwords)中的步骤进行操作。 这些步骤配置 SSH `root` 帐户和 Ambari `admin` 帐户的密码。 从 Visual Studio 连接到沙盒时，将使用这些密码。
...
3. 从“连接到 HDInsight Emulator”对话框中，输入为 Ambari 配置的密码。
...
5. 等待服务验证完成。 在某些情况下，验证可能失败，并提示更新配置。 如果验证失败，请选择“更新”按钮，然后等待服务的配置和验证完成。
...
3. 提交查询后，将显示作业状态。 作业状态显示了 Hadoop 处理作业时有关作业的信息。 “作业状态”项提供作业的状态。 状态会定期更新，也可以使用刷新图标手动刷新状态。
    ...
    > [!NOTE]
    > 使用基于 Linux 的 HDInsight 群集时，Tez 也是默认引擎。 它不是基于 Windows 的 HDInsight 上的默认引擎；若要这种群集上使用 Tez，必须在 Hive 查询的开头处添加 `set hive.execution.engine = tez;` 行。
...
还创建包含多个 Hive 脚本的项目。 如果需要将相关脚本保留在一起，或者使用版本控制系统维护脚本，则项目非常有用。
...
Hive 提供了类似 SQL 的语言用于处理结构化数据，Pig 通过对数据执行转换来工作。 Pig 提供了一种语言 (Pig Latin)，可用于开发转换管道。 执行以下步骤以在本地群集上使用 Pig：
...
可以选择查看数据库或表的“属性”。 选择“属性”会在属性窗口中显示选定项的详细信息。
