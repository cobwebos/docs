---
title: 用于视觉工作室&霍顿作品的数据湖工具 - Azure HDInsight
...
* [用于可视化工作室的 Azure 数据湖工具](https://www.microsoft.com/download/details.aspx?id=49504)。
...
2. 在“服务器资源管理器”**** 中，右键单击“HDInsight”**** 项，并选择“连接到 HDInsight Emulator”****。
    ...
    ![服务器资源管理器，突出显示连接到 HDInsight 仿真器](./media/hdinsight-hadoop-emulator-visual-studio/connect-hdinsight-emulator.png)
...
执行这些步骤后，您现在在"服务器资源管理器 **"部分下**在"服务器资源管理器"中具有**HDInsight 本地群集**条目。
...
1. 在“服务器资源管理器”中，右键单击前面添加的本地群集所对应的项，然后选择“编写 Hive 查询”********。
    ...
    ![突出显示“编写 Hive 查询”的服务器资源管理器屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/write-apache-hive-query.png)
...
2. 在“新建查询”窗口中，输入以下命令：
    ...
    ![突出显示“提交”按钮的查询窗口屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/query-window-submit-hive.png)
...
3. 提交查询后，会显示作业状态。 作业状态显示 Hadoop 处理作业时有关作业的信息。 “作业状态”**** 提供作业的状态。 状态会定期更新，也可以使用刷新图标手动刷新状态。
    ...
    ![突出显示“作业状态”的“作业视图”对话框屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/job-view-dialog-box1.png)
...
4. 还可以按交互方式运行 Hive 作业，方法是将“批处理”**** 字段更改为“交互”****。 然后选择“执行”****。
    ...
    ![HiveServer2 输出的屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/hiveserver2-output-box.png)
...
2. 在项目列表中，展开“模板”、“Azure Data Lake”，然后选择“HIVE (HDInsight)”************。 在模板列表中，选择“Hive 示例”****。 输入名称和位置，然后选择“确定”****。
    ...
    ![新项目窗口，包含 Azure 数据湖、蜂巢示例和"确定"](./media/hdinsight-hadoop-emulator-visual-studio/new-apache-hive-project.png)
...
1. 打开可视化工作室，然后选择 **"文件**"，**新建**，然后**选择"项目**"。 在项目列表中，展开“模板”、“Azure Data Lake”，然后选择“Pig (HDInsight)”************。 在模板列表中，选择“Pig 应用程序”****。 输入名称和位置，并选择“确定”****。
...
2. 输入以下文本作为使用此项目创建的 **script.pig** 文件内容。
    ...
    当 Pig 使用的语言与 Hive 不同时，通过 **"提交"** 按钮，操作作业的方式在两种语言之间是一致的。 选择“提交”**** 旁边的下拉列表会显示 Pig 的高级提交对话框。

    ![“提交脚本”对话框 pig 的屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/advanced-apache-pig1.png)

3. 显示的作业状态和输出也与 Hive 查询相同。

    ![已完成 Pig 作业的屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/completed-apache-pig.png)
...
1. 在“服务器资源管理器”中，右键单击本地群集，然后选择“查看作业”********。 此时会显示已提交到群集的作业列表。
    ...
    ![突出显示“查看作业”的服务器资源管理器屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/server-explorer-view-jobs.png)
...
2. 展开一个表可显示该表的列。 若要快速查看数据，请右键单击某个表并选择“查看前 100 行”****。
    ...
    ![服务器资源管理器，其中扩展了表并选择了查看前 100 行](./media/hdinsight-hadoop-emulator-visual-studio/hdi-view-top-100-rows.png)

### <a name="database-and-table-properties"></a>数据库和表属性

可以查看数据库或表的属性。 选择“属性”**** 会在属性窗口中显示选定项的详细信息。 有关示例，请参阅以下屏幕截图中显示的信息：

![“属性”窗口的屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/hdi-properties-window.png)

### <a name="create-a-table"></a>创建表

若要创建表，请右键单击某个数据库，然后选择“创建表”****。

![突出显示“创建表”的服务器资源管理器屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/server-explorer-create-table.png)

然后，可以使用表单创建表。 在以下屏幕截图的底部，可以看到用于创建表的原始 HiveQL。

![用于创建表的表单屏幕截图](./media/hdinsight-hadoop-emulator-visual-studio/create-table-form-box.png)
