...
使用 HDInsight 群集可以通过 Internet 访问 Apache Ambari Web UI，但某些功能需要 SSH 隧道。 例如，在没有 SSh 隧道的情况下，无法通过 internet 访问 Apache Oozie service 的 web UI。
...
4. 会显示类似于下图的页面：
    ...
    > [!NOTE]  
    > 请注意此页的 URL;它应该与 `http://hn1-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:8088/cluster`类似。 此 URL 使用了节点的内部完全限定的域名 (FQDN)，仅在使用 SSH 隧道时可以访问。
