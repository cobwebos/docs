...
Spark 应用程序 UI 是有关 Spark 执行器使用的资源的另一个信息源。  在 Spark UI 中，选择 **"执行器**"选项卡以显示执行器使用的配置和资源的摘要和详细信息视图。  借助这些视图可以确定是要更改整个群集的 Spark 执行器的默认值，还是更改一组特定的作业执行。
...
* [Jupyter](https://jupyter.org/)和[Apache Zepelin](https://zeppelin.apache.org/)笔记本 - 交互式基于浏览器的 UI，用于与 Spark 群集进行交互。
