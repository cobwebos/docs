...
| -q |`-q` |使用此参数可关闭单元可视化。 如果不想 autovisualize 单元格的内容，而只想将其捕获为数据帧，请使用 `-q -o <VARIABLE>`。 如果想要关闭可视化而不捕获结果（例如，运行诸如 `CREATE TABLE` 语句的 SQL 查询），请使用不带 `-o` 参数的 `-q`。 |
