...
6. 可以在输出窗口中查看应用程序 ID。
   ...
   ![远程运行结果](./media/apache-spark-intellij-tool-failure-debug/hdinsight-remotely-run-result.png)   
...
1. 打开 **Microsoft Azure 存储资源管理器**，找到已失败作业所在群集的 HDInsight 帐户，将已失败作业的资源从相应的位置 ( **\hdp\spark2-events\\.spark-failures\\\<应用程序 ID>** ) 下载到本地文件夹。 “活动”窗口会显示下载进度。
   ...
   ![下载失败 file1](./media/apache-spark-intellij-tool-failure-debug/hdinsight-find-spark-file-001.png)

   ![下载失败 file2](./media/apache-spark-intellij-tool-failure-debug/spark-on-cosmos-doenload-file-2.png)   
