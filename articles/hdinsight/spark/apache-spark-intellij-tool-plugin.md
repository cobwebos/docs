...
本文演示如何使用适用于 IntelliJ IDE 的**Azure 工具包**插件在 azure HDInsight 上开发 Apache Spark 应用程序。 [Azure HDInsight](../hdinsight-overview.md)是云中托管的开源分析服务。 服务允许使用开源框架，如 Hadoop、Apache Spark、Apache Hive 和 Apache Kafka。
...
* [Oracle Java 开发工具包](https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html)。  本文使用 Java 版本8.0.202。
...
6. 在“新建项目”窗口中提供以下信息：   
    ...
    |项目位置&nbsp;| 输入项目保存位置。|
    |项目 SDK| 在您首次使用主意时，此字段可能为空。  选择“新建...”并导航到 JDK。 |
...
9. 通过执行以下步骤添加应用程序源代码：
    ...
    a. 从 "项目" 中，导航到 " **myApp** > **src** > **main** > **scala**"。  
...
用户可以[登录到 Azure 订阅](#sign-in-to-your-azure-subscription)，也可以[链接到 HDInsight 群集](#link-a-cluster)。 使用 Ambari 用户名/密码或已加入域的凭据连接到 HDInsight 群集。
...
可以使用 Azure Toolkit for IntelliJ 执行各种操作。  大多数操作都从**Azure 资源管理器**启动。  在菜单栏中，导航到“视图” > “工具窗口” > “Azure 资源管理器”。   

### <a name="access-the-job-view"></a>访问作业视图

1. 在 Azure 资源管理器中，导航到**HDInsight** > \<> > **作业**。
...
3. 在“Spark 历史记录服务器”仪表板上，可以使用应用程序名称查找刚运行完的应用程序。 在上述代码中，已使用 `val conf = new SparkConf().setAppName("myApp")` 设置了应用程序名称。 Spark 应用程序名称为**myApp**。
...
8. 控制台应如下图所示。 在“控制台”窗口中键入 `sc.appName`，然后按 Ctrl+Enter。  系统将显示结果。 可以通过单击红色按钮结束本地控制台。
...
7. 控制台应如下图所示。 在“控制台”窗口中键入 `sc.appName`，然后按 Ctrl+Enter。  系统将显示结果。 可以通过单击红色按钮结束本地控制台。
...
可以通过将一些代码发送到本地控制台或 Livy 交互式会话控制台 (Scala) 来方便地预测脚本结果。 可以在 Scala 文件中突出显示一些代码，然后右键单击“向 Spark 控制台发送所选内容”  。 所选代码将发送到控制台。 结果将显示在控制台中的代码后面。 控制台将检查是否存在错误。  
...
可以按照常规步骤登录到 Azure 订阅，以连接到 ID 为 Broker （HIB）的 HDInsight ESP 群集。 登录后，你将在 Azure 资源管理器中看到群集列表。 有关详细信息，请参阅[连接到 HDInsight 群集](#connect-to-your-hdinsight-cluster)。

### <a name="run-a-spark-scala-application-on-an-hdinsight-esp-cluster-with-id-broker-hib"></a>在 ID 为 Broker （HIB）的 HDInsight ESP 群集上运行 Spark Scala 应用程序

可以按照常规步骤，将作业提交到 ID 为 Broker 的 HDInsight ESP 群集（HIB）。 有关更多说明，请参阅[在 HDInsight Spark 群集中运行 Spark Scala 应用程序](#run-a-spark-scala-application-on-an-hdinsight-spark-cluster)。

我们会将所需的文件上传到使用您的登录帐户命名的文件夹，并且您可以在配置文件中看到上传路径。

   ![配置中的上传路径](./media/apache-spark-intellij-tool-plugin/upload-path-in-the-configuration.png)

### <a name="spark-console-on-an-hdinsight-esp-cluster-with-id-broker-hib"></a>ID 为 Broker （HIB）的 HDInsight ESP 群集上的 Spark 控制台

可以在 ID 为 Broker （HIB）的 HDInsight ESP 群集上运行 Spark 本地控制台（Scala）或运行 Spark Livy Interactive 会话控制台（Scala）。 如需查看更详尽的说明，请参阅 [Spark 控制台](#spark-console)。
...
1. 对于通过 IntelliJ 创意创建的现有 Spark Scala 应用程序，打开关联`.iml`的文件。

2. 在根级别，是一个**module**元素，类似于以下文本：

        ```
        <module org.jetbrains.idea.maven.project.MavenProjectsManager.isMavenModule="true" type="JAVA_MODULE" version="4">
        ```

   编辑元素以添加`UniqueKey="HDInsightTool"` ，使**module**元素如下所示：
...
1. 在出现的 HDInsight 群集列表中，选择为此项目创建的群集旁边的 " **...** "。
...
本文介绍了如何使用 Azure Toolkit for IntelliJ 插件开发以[Scala](https://www.scala-lang.org/)编写的 Apache Spark 应用程序。 然后直接从 IntelliJ 集成开发环境（IDE）将它们提交到 HDInsight Spark 群集。 请转到下一篇文章，了解如何将在 Apache Spark 中注册的数据拉取到 Power BI 等 BI 分析工具中。
