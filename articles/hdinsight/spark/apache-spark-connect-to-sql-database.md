...
1. 我们要将数据从 **HVAC.csv** 流式传输到 hvactable。 中的群集`/HdiSamples/HdiSamples/SensorSampleData/HVAC/`上提供了 HVAC .csv 文件。 在以下代码片段中，我们先获取要流式传输的数据的架构。 然后，使用该架构创建流数据帧。 粘贴代码单元中的代码片段，然后按 **SHIFT + ENTER** 运行。
