...
1. 连接到 HDInsight 群集后，使用以下 **Hadoop** 命令来启动 MapReduce 作业：
    ...
    这会启动包含在 **hadoop-mapreduce-examples.jar** 文件中的 **wordcount** 类。 作为输入，它会使用 **wasbs://example/data/gutenberg/davinci.txt** 文档，输出将存储到：**wasbs:///example/data/WordCountOutput**。
