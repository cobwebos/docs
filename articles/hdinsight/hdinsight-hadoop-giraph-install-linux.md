---
title: "在基于 Linux 的 HDInsight (Hadoop) 上安装并使用 Giraph | Microsoft Docs"
...
可通过使用**脚本操作**自定义群集，在 Azure HDInsight 上 Hadoop 中的任何一种群集上安装 Giraph。

本主题介绍如何使用脚本操作安装 Giraph。 一旦你已安装 Giraph，你还将了解如何将 Giraph 用于大多数典型应用程序，也就是处理大型图形。

> [!NOTE]
> 本文中的信息特定于基于 Linux 的 HDInsight 群集。 有关使用基于 Windows 的群集的信息，请参阅[在 HDInsight Hadoop 群集 (Windows) 上安装 Giraph](hdinsight-hadoop-giraph-install.md)
> 
> 
...
6. 完成该作业后，结果将存储在 **wasb:///example/out/shotestpaths** 目录中。 创建的文件将以 **part-m-** 开头，结尾的数字表示第一个文件、第二个文件，依此类推。 使用以下命令查看输出：
