...
使用 **"从教程部署防火墙**[：使用 Azure 门户部署和配置 Azure 防火墙](../firewall/tutorial-firewall-deploy-portal.md#deploy-the-firewall)"中的步骤创建名为**Test-FW01**的防火墙。
...
1. 在“添加应用程序规则集合”屏幕上提供以下信息：****
    ...
    **目标 FQDN 部分**
    ...
    | Rule_4 | * | https:443,http:80 | storage_account_name.blob.core.windows.net | 请将 `storage_account_name` 替换为实际的存储帐户名称。 如果群集由 WASB 提供支持，则为 WASB 添加一个规则。 要仅使用 https 连接，请确保在存储帐户上启用[了"需要安全传输"。](../storage/common/storage-require-secure-transfer.md) |
...
* [来自运行状况和管理服务](../hdinsight/hdinsight-management-ip-addresses.md#health-and-management-services-all-regions)的所有 IP 地址：具有下一个跃点类型的**Internet**的所有区域。

* 从[运行状况和管理服务](../hdinsight/hdinsight-management-ip-addresses.md#health-and-management-services-specific-regions)创建群集的区域的两个 IP 地址：具有下一个跃点类型的**Internet**的特定区域。
