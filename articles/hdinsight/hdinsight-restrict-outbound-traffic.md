...
保护出站地址的解决方案是使用可基于域名控制出站流量的防火墙设备。 Azure 防火墙可以限制出站的 HTTP 和 HTTPS 通信，基于目标的 FQDN 或[FQDN 标记](https://docs.microsoft.com/azure/firewall/fqdn-tags)。
...
1. 六个地址，可在[此列表所需的 HDInsight 管理 IP 地址](../hdinsight/hdinsight-extend-hadoop-virtual-network.md#hdinsight-ip)使用的下一跃点**Internet**:
...
1. 上**子网关联**屏幕上，选择已创建你的群集的虚拟网络并**HDInsight 子网**用于 HDInsight 群集。
1. 单击“确定”。 
...
Azure 防火墙可将日志发送到一些不同的存储系统。 对于配置的说明日志记录你的防火墙，请按照中的步骤[教程：监视 Azure 防火墙日志和指标](../firewall/tutorial-diagnostics.md)。
