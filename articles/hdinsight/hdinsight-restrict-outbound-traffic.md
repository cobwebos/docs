...
1. 在 "**添加应用程序规则集合**" 屏幕上，提供以下信息：
    ...
    **FQDN 标记部分**
    ...
    | 名称 | 源地址 | FQDN 标记 | 说明 |
    | --- | --- | --- | --- |
    | Rule_1 | * | Windowsupdate.log 和 HDInsight | HDI services 必需的 |
...
1. 在 "**添加网络规则集合**" 屏幕上，提供以下信息：
    ...
    **IP 地址部分**
    ...
    | Rule_3 | TCP | * | Data Lake Storage 帐户的 IP 地址 | * | 如果使用 Azure Data Lake Storage，则可以在 "IP 地址" 部分中添加一个网络规则，以解决与 ADLS Gen1 和 Gen2 的 SNI 问题。 此选项会将流量路由到防火墙，这可能会导致较高的数据负载开销，但会在防火墙日志中记录和审核流量。 确定 Data Lake Storage 帐户的 IP 地址。 可以使用 powershell 命令（如 `[System.Net.DNS]::GetHostAddresses("STORAGEACCOUNTNAME.blob.core.windows.net")`）将 FQDN 解析为 IP 地址。|
...
* 一个虚拟设备路由 IP 地址 0.0.0.0/0，下一跃点是你的 Azure 防火墙专用 IP 地址。

例如，若要为 "美国东部" 区域中创建的群集配置路由表，请执行以下步骤：

1. 选择 Azure 防火墙**FW01**。 复制 "**概述**" 页上列出的**专用 IP 地址**。 在此示例中，我们将使用**10.0.2.4 的示例地址**。
...
若要了解 Azure 防火墙和请求增加的规模限制，请参阅[此](../azure-resource-manager/management/azure-subscription-service-limits.md#azure-firewall-limits)文档或参考[常见问题解答](../firewall/firewall-faq.md)。
