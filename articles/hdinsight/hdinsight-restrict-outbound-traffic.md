...
1. 向“目标 FQDN”部分添加以下规则：
   ...
   | Rule_4 | * | https:443,http:80 | < storage_account_name （.net） > | 如果群集由 WASB 提供支持，则为 WASB 添加一个规则。 若只使用 https 连接，请确保在存储帐户上启用 [需要安全传输](https://docs.microsoft.com/azure/storage/common/storage-require-secure-transfer)。 |
