...
*对于 HDInsight 群集，只有辅助存储帐户可以是 Blob 存储类型，页面 Blob 不是受支持的存储选项。
...
您可以使用用于主存储和可选辅助存储的不同服务组合创建群集。 下表总结了 HDInsight 中当前支持的群集存储配置：
...
* **与 Hadoop 兼容的访问：** 在 Azure 数据湖存储 Gen2 中，您可以像使用 Hadoop 分布式文件系统 （HDFS） 一样管理和访问数据。 Azure Blob 文件系统 (ABFS) 驱动程序可以在所有 Apache Hadoop 环境中使用（包括 Azure HDInsight 和 Azure Databricks）。 使用 ABFS 可以访问存储在 Data Lake Storage Gen2 中的数据。

* **POSIX 权限的超集：** 数据湖 Gen2 的安全模型支持 ACL 和 POSIX 权限，以及特定于数据存储第 2 代的一些额外粒度。 可以通过管理工具或 Apache Hive 和 Apache Spark 等框架来配置设置。

* **成本效益：** 数据存储湖存储 Gen2 提供低成本的存储容量和事务。 Azure Blob 存储生命周期等功能可以随数据在其生命周期中的移动来调整计费率，从而帮助降低成本。

* **与 Blob 存储工具、框架和应用兼容：** Data Lake 存储 Gen2 继续与各种工具、框架和应用程序一起用于 Blob 存储。

* **优化的驱动程序：** ABFS 驱动程序专为大数据分析进行了优化。 相应的 REST API 通过分布式文件系统 (DFS) 终结点 dfs.core.windows.net 进行显示。
...
有关详细信息，请参阅[Azure Blob 文件系统驱动程序 （ABFS）：Hadoop 的专用 Azure 存储驱动程序](../storage/blobs/data-lake-storage-abfs-driver.md)。
...
我们建议对默认群集存储和业务数据使用单独的存储容器，将 HDInsight 日志和临时文件与您自己的业务数据隔离开来。 我们还建议在每次使用后删除包含应用程序和系统日志的默认 Blob 容器，以降低存储成本。 请确保在删除该容器之前检索日志。
...
![HDInsight 存储架构](./media/hdinsight-hadoop-compare-storage-options/storage-architecture.png "HDInsight 存储体系结构")
...
* ***未*连接到群集的存储帐户中的公共容器或公共 Blob：** 您对容器中的 blob 具有只读权限。
...
* ***未*连接到群集的存储帐户中的专用容器：** 除非在提交 WebHCat 作业时定义存储帐户，否则无法访问容器中的 blob。 
...
多个 WebHCat 作业，包括 Apache Hive、MapReduce、Apache Hadoop 流和 Apache Pig，都可以带有存储帐户和元数据的说明。 （对于具有存储帐户的 Pig，而不是元数据，这一点目前为数。有关详细信息，请参阅[使用具有备用存储帐户和元存储的 HDInsight 群集](https://social.technet.microsoft.com/wiki/contents/articles/23256.using-an-hdinsight-cluster-with-alternate-storage-accounts-and-metastores.aspx)。
...
### <a name="benefits-of-azure-storage"></a><a id="benefits"></a>Azure 存储的优点
...
* **数据存储成本：** 长期在 DFS 中存储数据比将数据存储在 Azure 存储中的成本更高，因为计算群集的成本高于 Azure 存储的成本。 此外，由于数据无需在每次生成计算群集时重新加载，也节省了数据加载成本。

* **弹性向外扩展：** 尽管 HDFS 提供了向外扩展文件系统，但缩放由你为群集创建的节点数量决定。 与依靠自动获得的 Azure 存储的弹性缩放功能相比，更改规模可能会更复杂。
...
## <a name="data-security-in-data-lake-storage-gen1"></a><a name="DataLakeStoreSecurity"></a>Data Lake Storage Gen1 中的数据安全性
Data Lake Storage Gen1 使用 Azure Active Directory 进行身份验证，使用访问控制列表 (ACL) 管理对数据的访问。

| **功能** | **说明** |
| --- | --- |
| 身份验证 |Data Lake Storage Gen1 与 Azure Active Directory (Azure AD) 集成，可对 Data Lake Storage Gen1 中存储的所有数据进行标识与访问管理。 因为这种集成，Data Lake Storage Gen1 可受益于所有 Azure AD 功能。 这些功能包括多重身份验证、条件访问、基于角色的访问控制、应用程序使用监视、安全监视和警报等。 Data Lake Storage Gen1 支持在 REST 接口中使用 OAuth 2.0 协议进行身份验证。 参阅[使用 Azure Active Directory 进行 Azure Data Lake Storage Gen1 身份验证](../data-lake-store/data-lakes-store-authentication-using-azure-active-directory.md)。|
