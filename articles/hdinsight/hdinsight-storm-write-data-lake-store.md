...
Azure Data Lake 存储是与 HDFS 兼容的云存储服务，可为数据提供高吞吐量、可用性、持久性和可靠性。 在本文档中，将学习如何使用基于 Java 的 Storm 拓扑，通过 Apache Storm 中提供的 [HdfsBolt](http://storm.apache.org/releases/1.0.2/javadocs/org/apache/storm/hdfs/bolt/HdfsBolt.html) 组件将数据写入 Azure Data Lake Store。
...
在 HDInsight 看来，Data Lake Store 是一种兼容 HDFS 的文件系统，因此可以通过 Storm-HDFS Bolt 向其写入内容。 以下代码演示如何使用 Storm-HDFS Bolt 将数据写入名为 `MYDATALAKE` 的 Data Lake Store 帐户中名为 `/stormdata` 的目录。
...
> [!IMPORTANT]
> 通过 HDInsight 使用 Data Lake Store 时，请使用 `adl://` URI 方案。
...
* **从 [Azure 门户](https://portal.azure.com)**：在门户中，选择用于 HDInsight 的 Azure Data Lake Store。
