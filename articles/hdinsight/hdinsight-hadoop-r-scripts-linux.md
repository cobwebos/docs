---
title: "在基于 Linux 的 HDInsight 上安装 R | Microsoft Docs"
...
可以使用**脚本操作**群集自定义在 HDInsight 上 Hadoop 中的任何类型的群集上安装 R。 这样，数据科学家和分析人员便可使用 R 部署功能强大的 MapReduce/YARN 编程框架，以便在 HDInsight 中部署的 Hadoop 群集上处理大量数据。

> [!IMPORTANT]
> [HDInsight](https://azure.microsoft.com/pricing/details/hdinsight/) 产品/服务包括 R Server 作为 HDInsight 群集的一部分。 它允许 R 脚本使用 MapReduce 和 Spark 来运行分布式计算。 有关详细信息，请参阅 [开始使用 HDInsight 上的 R Server](hdinsight-hadoop-r-server-get-started.md)。 
> 
> 
...
* [r-base-dev](http://packages.ubuntu.com/precise/r-base-dev)：Auxilliary GNU R 程序包
...
2. 从 `username@hn0-CLUSTERNAME:~$` 提示符中，输入以下命令以启动交互式 R 会话：
        ...
        R
3. 输入以下 R 程序。 这会生成数字 1 到 100，然后将其乘以 2。
...
4. 接下来，请使用以下命令查看 MapReduce 输出存储到的临时路径：
    ...
    这应大致如 `/tmp/file5f615d870ad2` 所示。 若要查看实际输出，请使用以下命令：
