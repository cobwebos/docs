...
1. NameNode Java 堆大小从 2 GB 更改为 1 GB。
...
1.  若要限制要并行运行的作业数目，请修改`hive.exec.parallel.thread.number`属性。 默认值为 8。
...
1. 选择**保存**上要保存新值的窗口的顶部右侧。 某些属性可能需要重启服务才能生效。
