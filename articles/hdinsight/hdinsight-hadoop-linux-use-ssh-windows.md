---
title: "在 Windows 上的 PuTTY 中将 SSH 与 HDInsight 群集配合使用 | Microsoft 文档"
description: "了解如何创建和使用 SSH 密钥，在基于 Windows 的客户端上通过 PuTTY SSH 客户端向基于 Linux 的 HDInsight 群集进行身份验证。"
services: hdinsight
documentationcenter: 
author: Blackmist
manager: jhubbard
...
---
# <a name="use-ssh-with-hdinsight-hadoop-from-putty-on-windows"></a>在 Windows 上的 PuTTY 中将 SSH 与 HDInsight (Hadoop) 配合使用
...
> [!NOTE]
> 本文中的步骤假设使用的是装有 PuTTY SSH 的基于 Windows 的系统。 如果使用的是提供 `ssh` 命令的 Linux、Unix、OS X 或 Windows 系统，请参阅[在 Linux、Unix 或 OS X 中的 HDInsight 上将 SSH 与基于 Linux 的 Hadoop 配合使用](hdinsight-hadoop-linux-use-ssh-unix.md)。
...
Windows 以前一直都未有 SSH 客户端。 PuTTY 是可以安装在 Windows 上的图形 SSH 客户端。
...
## <a name="create-a-linux-based-hdinsight-cluster"></a>创建基于 Linux 的 HDInsight 群集
