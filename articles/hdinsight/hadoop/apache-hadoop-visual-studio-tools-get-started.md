...
即使没有启动 Tez 应用程序，也不会显示作业图形，即使将 Tez 指定为执行引擎。  出现这种情况的原因可能是作业不包含 DML 语句，或者 DML 语句可以返回而不启动 Tez 应用程序。 例如，`SELECT * FROM table1` 将不会启动 Tez 应用程序。
