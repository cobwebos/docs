...
`user-ratings.txt`中包含`userID`的数据具有、 `movieID`、 `userRating`和`timestamp`的结构，指示每个用户对电影进行评分的程度。 下面是数据的示例：
...
Mahout 作业不会删除在处理作业时创建的临时数据。 在示例作业中指定 `--tempDir` 参数，以将临时文件隔离到特定路径中轻松删除。 若要删除临时文件，请使用以下命令：
