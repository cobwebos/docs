...
* 运行 Windows 10、7、Windows Server 2008 R2 或更高版本操作系统的工作站。
* Office 2016、Office 2013 Professional Plus、Office 365 ProPlus、Excel 2013 独立版或 Office 2010 专业增强版。
...
1. 根据 Excel 版本执行以下步骤：
   ...
   * Excel 2016
     ...
     * 选择 >**数据** > 从 azure **HDInsight （HDFS）** **的 azure** > **获取数据** > 。

       ![HDI.PowerQuery. SelectHdiSource. 2016](./media/apache-hadoop-connect-excel-power-query/powerquery-selecthdisource-excel2016.png)

   * Excel 2013/2010

     * **从 Microsoft Azure HDInsight**中选择**Azure** > **Power Query** > 。

       ![HDI.PowerQuery.SelectHdiSource](./media/apache-hadoop-connect-excel-power-query/powerquery-selecthdisource.png)

       **注意：** 如果看不到 " **Power Query** " 菜单，请在页面底部的下拉 "**管理**" 框**中，选择**"**文件**" " >  > **选项**"，然后选择 " **COM 外接程序**"。 选择“转到...”按钮，并且确认已选中 Power Query for Excel 外接程序所对应的框。

       **注意：** Power Query 还允许通过**从其他源中**进行选择，从 HDFS 导入数据。

1. 从 " **Azure HDInsight （HDFS）** " 对话框的 "**帐户名称或 URL** " 文本框中，输入与群集关联的 Azure Blob 存储帐户的名称。 然后选择“确定”。 此帐户可以是默认存储帐户，也可以是链接的存储帐户。  格式为 `https://StorageAccountName.blob.core.windows.net/`。

1. 对于 "**帐户密钥**"，请输入 Blob 存储帐户的密钥，然后选择 "**连接**"。 （只有在第一次访问此存储时才需要输入帐户信息。）

1. 在 "查询编辑器" 左侧的 "**导航器**" 窗格中，双击与群集关联的 Blob 存储容器名称。 默认情况下，该容器名称与群集名称相同。

1. 在 "**名称**" 列中找到**hivesampledata.txt** （文件夹路径为 **.。/hive/warehouse/hivesampletable/** ），然后选择 hivesampledata.txt 左侧的 "**二进制**"。 HiveSampleData.txt 随所有群集提供。 （可选）可以使用自己的文件。

    ![HDI Excel power query 导入数据](./media/apache-hadoop-connect-excel-power-query/powerquery-importdata.png)

1. 如果需要，可以重命名列名称。 准备就绪后，选择 "**关闭" & 负载**。  数据已加载到工作簿：

    ![HDI Excel power query 导入表](./media/apache-hadoop-connect-excel-power-query/powerquery-importedtable.png)

## <a name="next-steps"></a>后续步骤

在本文中，已了解如何使用 Power Query 将数据从 HDInsight 检索到 Excel 中。 同样地，也可以将来自 HDInsight 的数据检索到 Azure SQL 数据库中。 也可以将数据上载到 HDInsight。 若要了解详细信息，请参阅以下文章：
