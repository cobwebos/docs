...
* 存储帐户已启用[安全转移](../../storage/common/storage-require-secure-transfer.md)，并且正在使用不正确的[URI 方案](../hdinsight-hadoop-linux-information.md#URI-and-scheme)。

* 使用*禁用*安全转移的存储帐户创建群集。 此后，存储帐户上启用了安全转移。

## <a name="resolution"></a>解决方法

如果为 Azure 存储或数据湖存储 Gen2 启用了安全传输，`wasbs://`则`abfss://`URI 将分别为 或 。  另请参阅[安全传输](../../storage/common/storage-require-secure-transfer.md)。

对于新群集，请使用已具有所需安全传输设置的存储帐户。 不要更改现有群集正在使用的存储帐户的安全转移设置。
