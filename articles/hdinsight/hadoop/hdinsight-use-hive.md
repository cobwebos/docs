...
[阿帕奇蜂巢](https://hive.apache.org/)是阿帕奇哈多普的数据仓库系统。 使用 Hive 可以汇总、查询和分析数据。 Hive 查询使用 HiveQL 编写，它是类似于 SQL 的查询语言。
...
HiveQL 语言参考可在[语言手册](https://cwiki.apache.org/confluence/display/Hive/LanguageManual)中提供。
...
* __外部__：数据存储在数据仓库外部。 数据可以存储在可由群集访问的任何存储中。
    ...
    * 数据也在 Hive 外部使用。 例如，数据文件由另一个进程（不锁定文件）更新。
...
|CREATE EXTERNAL TABLE|在 Hive 中创建新**的外部**表。 外部表只会在 Hive 中存储表定义。 数据以原始格式的形式保留在原始位置中。|
|行格式|让 Hive 知道数据的格式已如何进行了设置。 在此情况下，每个日志中的字段以空格分隔。|
|存储为文本文件位置|告诉 Hive 数据存储的位置（`example/data`目录）并将其存储为文本。 数据可以在一个文件中，也可以分散在目录的多个文件内。|
...
|语句 |描述 |
|---|---|
|创建表（如果不存在）|如果表不存在，请创建它。 由于不使用**外部**关键字，因此此语句将创建一个内部表。 该表存储在 Hive 数据仓库中，并完全由 Hive 管理。|
