...
2. 在下面的脚本中，将 `sshuser` 替换为群集的 SSH 用户帐户名。  将 `CLUSTERNAME` 替换为 HDInsight 群集名称。  从命令行，将你的工作目录更改为在前面的步骤中创建并输入以下命令以将文件从 HDInsight 群集复制：
...
5. 在“添加驱动程序”对话框中，添加以下信息：
    ...
    * **额外类路径**：使用**添加**按钮添加的所有 jar 文件之前下载
