...
---
# <a name="run-apache-sqoop-jobs-in-hdinsight-with-curl"></a>在 HDInsight 中通过卷运行 Apache Sqoop 作业

[!INCLUDE [sqoop-selector](../../../includes/hdinsight-selector-use-sqoop.md)]

了解如何使用 Curl 在 HDInsight 中的 Apache Hadoop 群集上运行 Apache Sqoop 作业。 本文介绍如何从 Azure 存储导出数据，并使用卷将数据导入到 SQL Server 数据库中。 本文介绍如何将[Apache Sqoop 与 HDInsight 中的 Hadoop 配合使用](./hdinsight-use-sqoop.md)。
...
* 用于查询 Azure SQL 数据库的客户端。 请考虑使用[SQL Server Management Studio](../../sql-database/sql-database-connect-query-ssms.md)或[Visual Studio Code](../../sql-database/sql-database-connect-query-vscode.md)。
...
1. 使用以下命令 sqoop 作业：
   ...
   * **-d** -由于未使用 `-G`，因此请求默认为 POST 方法。 `-d` 指定与请求一起发送的数据值。
     ...
     此命令将返回可用于检查作业状态的作业 ID。

       ```json
       {"id":"job_1415651640909_0026"}
       ```

1. 若要检查作业的状态，请使用以下命令。 将 `JOBID` 替换为上一步骤返回的值。 例如，如果返回值为 `{"id":"job_1415651640909_0026"}`，则 `JOBID` 将 `job_1415651640909_0026`。 根据需要修改 `jq` 的位置。
...
1. 在作业的状态更改为“SUCCEEDED”后，可以从 Azure Blob 存储中检索作业的结果。 随查询一起传递的 `statusdir` 参数包含输出文件的位置；在本例中，该位置为 `wasb:///example/data/sqoop/curl`。 此地址会将作业的输出存储在 HDInsight 群集所用的默认存储容器上的 `example/data/sqoop/curl` 目录中。

    可使用 Azure 门户访问 stderr 和 stdout blob。

1. 若要验证数据是否已导出，请在 SQL 客户端中使用以下查询来查看导出的数据：
