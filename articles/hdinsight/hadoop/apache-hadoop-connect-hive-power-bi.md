...
了解如何使用 ODBC 将 Microsoft Power BI Desktop 连接到 Azure HDInsight 并直观显示 Apache Hive 的数据。
...
本文介绍如何将 `hivesampletable` Hive 表中的数据加载到 Power BI 中。 Hive 表包含一些移动电话使用情况数据。 然后在世界地图上绘制使用情况数据：
...
1. 从左侧的 "**获取数据**" 对话框中，选择 "**其他**"，从右侧选择 " **ODBC** "，然后选择底部的 "**连接**"。

1. 在 "**从 ODBC** " 对话框中，从下拉列表中选择在上一部分中创建的数据源名称。 然后选择“确定”。

1. 第一次使用时， **ODBC 驱动程序**对话框将打开。 从左侧菜单中选择 "**默认" 或 "自定义**"。 然后选择 "**连接**" 以打开 **"导航器"** 。

1. 从 "**导航器**" 对话框中，展开 " **ODBC > HIVE > 默认**，选择" **hivesampletable**"，然后选择"**加载**"。
