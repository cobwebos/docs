...
在 Visual Studio 中，创建一个名为 "*映射*器" 的新 .NET Framework 控制台应用程序。 针对该应用程序使用以下代码：
...
创建应用程序后，生成它以在项目目录中生成 */bin/Debug/mapper.exe*文件。

## <a name="create-the-reducer"></a>创建化简器

在 Visual Studio 中，创建一个名为*化简器*的新 .NET Framework 控制台应用程序。 针对该应用程序使用以下代码：
...
创建应用程序后，生成它以在项目目录中生成 */bin/Debug/reducer.exe*文件。

## <a name="upload-to-storage"></a>上传到存储

接下来，需要将*映射*器和*化简器*应用程序上传到 HDInsight 存储。
...
1. 展开要将此应用程序部署到的 HDInsight 群集。 将列出带有文本“（默认存储帐户）”的条目。
   ...
   * 如果可以扩展 " **（默认存储帐户）** " 条目，则将使用**Azure 存储帐户**作为群集的默认存储。 若要查看群集的默认存储上的文件，请展开该条目，然后双击 " **（默认容器）** "。
...
3. 在 MapReduce 作业完成后，使用以下命令查看结果：
...
有关将 MapReduce 与 HDInsight 配合使用的详细信息，请参阅[在 hdinsight Apache Hadoop 中使用 mapreduce](hdinsight-use-mapreduce.md)。
