...
ms.locfileid: "82192053"
---
# <a name="manage-hdinsight-clusters-by-using-the-apache-ambari-web-ui"></a>使用 Apache Ambari Web UI 管理 HDInsight 群集

[!INCLUDE [ambari-selector](../../includes/hdinsight-ambari-selector.md)]

Apache Ambari 简化了 Apache Hadoop 群集的管理和监视。 这种简化是通过提供易于使用的 web UI 和 REST API 来实现的。 Ambari 包含在 HDInsight 群集上，用于监视群集和进行配置更改。
...
通过选择 "__管理通知__"，可以管理警报方法，并通过 "**操作**" 菜单创建警报通知。 所有当前通知都会显示。 从此处创建通知。 出现特定的警报/严重性组合时，可通过**电子邮件**或 **SNMP** 发送通知。 例如，可在“YARN 默认设置”组中的任何警报设为“严重”时发送电子邮件消息。  
...
某些服务会在页面顶部显示“快速链接”**** 链接。 此链接可用于访问特定于服务的 web Ui，例如：
...
使用已加入域的 HDInsight 群集时，支持使用用户、组和权限。 若要深入了解如何在已加入域的群集上使用 Ambari 管理 UI，请参阅[管理已加入域的 HDInsight 群集](./domain-joined/hdinsight-security-overview.md)。
...
2. 使用 "**操作**" 菜单选择要执行的操作：
    ...
    |启用维护模式|隐藏主机的警报。 如果执行的是生成警报的操作，则应启用此模式。 例如，停止和启动服务。|
    |关闭维护模式|使主机恢复正常警报。|
    |停止|停止主机上的 DataNode 或 NodeManagers。|
    |开始|启动主机上的 DataNode 或 NodeManagers。|
    |重启|停止然后启动主机上的 DataNode 或 NodeManagers。|
...
Ambari 视图允许开发人员使用 Apache Ambari 视图框架将 UI 元素插入 Ambari Web UI。 HDInsight 提供了以下视图和 Hadoop 群集类型：
...
* [Apache Ambari REST API](hdinsight-hadoop-manage-ambari-rest-api.md) HDInsight。
