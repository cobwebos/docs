...
Ambari Web UI 在 HDInsight 群集`https://CLUSTERNAME.azurehdinsight.net`上可用`CLUSTERNAME`，群集的名称位于此位置。

> [!IMPORTANT]  
> 连接到 HDInsight 上的 Ambari 需要 HTTPS。 当提示进行身份验证时，请使用在创建群集时提供的管理员帐户名称和密码。 如果未提示您输入凭据，请检查网络设置以确认客户端和 Azure HDInsight 群集之间没有连接问题。

## <a name="ssh-tunnel-proxy"></a>SSH 隧道（代理）

虽然您的群集的 Ambari 可通过 Internet 直接访问，但安巴里 Web UI（如 JobTracker）的某些链接不会在互联网上公开。 若要访问这些服务，必须创建一个 SSH 隧道。 有关详细信息，请参阅[将 SSH 隧道与 HDInsight 配合使用](hdinsight-linux-ambari-ssh-tunnel.md)。
...
连接到 Ambari Web UI 时，系统会提示您对页面进行身份验证。 请使用在创建群集过程中使用的群集管理员用户（默认 Admin）和密码。
...
![阿帕奇·安巴里提醒页面摘要](./media/hdinsight-hadoop-manage-ambari/hdinsight-alerts-page.png)
...
最后，在“操作”____ 菜单中选择“管理警报设置”____ 可设置发送通知前出现警报的次数。 可以使用此设置来防止针对暂时性错误发出通知。

有关使用免费[SendGrid 帐户](https://docs.microsoft.com/azure/sendgrid-dotnet-how-to-send-email)的警报通知的教程，请参阅[在 Azure HDInsight 中配置 Apache Ambari 电子邮件通知](./apache-ambari-email.md)。

### <a name="cluster"></a>群集

仪表板的“指标”**** 选项卡包含一系列小组件，可让你一目了然地轻松监视群集状态。 “CPU 使用率”**** 等多个 Widget 可在单击后提供更多信息。

![阿帕奇 Ambari 仪表板与指标](./media/hdinsight-hadoop-manage-ambari/hdi-metrics-dashboard.png)

“热图”**** 选项卡以绿色到红色的彩色热图显示度量值。

![阿帕奇安巴里仪表板与热图](./media/hdinsight-hadoop-manage-ambari/hdi-heatmap-dashboard.png)

若要了解群集内节点的详细信息，请选择“主机”。**** 然后选择您感兴趣的特定节点。

![阿帕奇·安巴里主机摘要详细信息](./media/hdinsight-hadoop-manage-ambari/ambari-host-details1.png)

### <a name="services"></a>Services

仪表板上的“服务”**** 边栏可让你快速了解群集上运行的服务的状态。 各种图标用来指示状态或应当采取的操作。 例如，如果某项服务需要再循环，则会显示一个黄色的再循环符号。

![阿帕奇安巴里服务侧酒吧](./media/hdinsight-hadoop-manage-ambari/apache-ambari-service-bar.png)
...
![阿帕奇·安巴里服务摘要信息](./media/hdinsight-hadoop-manage-ambari/ambari-service-details.png)
...
![阿帕奇·安巴里主机页面概述](./media/hdinsight-hadoop-manage-ambari/hdinsight-hosts-page.png)
...
2. 使用“操作”**** 菜单选择要执行的操作：
    ...
    |启用维护模式|隐藏主机的警报。 如果您正在执行生成警报的操作，则应启用此模式。 例如，停止和启动服务。|
    |关闭维护模式|使主机恢复正常警报。|
...
![阿帕奇·安巴里服务操作列表](./media/hdinsight-hadoop-manage-ambari/ambari-service-actions.png)
...
2. 选择"**配置"** 选项卡。将显示当前配置。 同时，还会显示以前的配置列表。
    ...
    ![阿帕奇·安巴里服务配置](./media/hdinsight-hadoop-manage-ambari/ambari-service-configs.png)
