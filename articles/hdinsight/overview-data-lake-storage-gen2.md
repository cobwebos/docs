...
description: HDInsight 中的 Data Lake Storage Gen2 的概述。
...
ms.locfileid: "82195122"
---
# <a name="azure-data-lake-storage-gen2-overview-in-hdinsight"></a>HDInsight 中的 Azure Data Lake Storage Gen2 概述

Azure Data Lake Storage Gen2 采用了 Azure Data Lake Storage Gen1 中的核心功能，并将它们集成到了 Azure Blob 存储中。 这些功能包括与 Hadoop 兼容的文件系统、Azure Active Directory (Azure AD) 和基于 POSIX 的访问控制列表 (ACL)。 这种组合可让你充分利用 Azure Data Lake Storage Gen1 的性能。 同时还使用 Blob 存储的分层和数据生命周期管理。
...
* **与 Hadoop 兼容的访问权限：** 在 Azure Data Lake Storage Gen2 中，你可以像使用 Hadoop 分布式文件系统（HDFS）一样管理和访问数据。 Azure Blob 文件系统 (ABFS) 驱动程序可以在所有 Apache Hadoop 环境中使用（包括 Azure HDInsight 和 Azure Databricks）。 使用 ABFS 可以访问存储在 Data Lake Storage Gen2 中的数据。

* **POSIX 权限的超集：** Data Lake Gen2 的安全模型支持 ACL 和 POSIX 权限，同时还支持特定于 Data Lake Storage Gen2 的一些额外粒度。 可以通过管理工具或 Apache Hive 和 Apache Spark 等框架来配置设置。

* **成本效益：** Data Lake Storage Gen2 提供低成本的存储容量和事务。 当数据在其生命周期中移动时，Azure Blob 存储生命周期有助于降低成本。

* **与 Blob 存储工具、框架和应用的兼容性：** Data Lake Storage Gen2 继续使用适用于 Blob 存储的各种工具、框架和应用程序。

* **优化驱动程序：** ABFS 驱动程序针对大数据分析进行了优化。 相应的 REST API 通过分布式文件系统 (DFS) 终结点 dfs.core.windows.net 进行显示。
...
Apache Hadoop 应用程序会以本机方式从本地磁盘存储读取和写入数据。 Hadoop 文件系统驱动程序（如 ABFS）可让 Hadoop 应用程序与云存储配合工作。 通过模拟常规 Hadoop 文件系统操作来工作。 驱动程序将从应用程序收到的这些命令转换为实际云存储平台可以理解的操作。

以前，Hadoop 文件系统驱动程序将所有文件系统操作转换为 Azure 存储，REST API 客户端上的调用。 然后调用 REST API。 但是，这种客户端转换会导致针对单个文件系统操作（例如文件重命名）执行多个 REST API 调用。 ABFS 已将 Hadoop 文件系统逻辑从客户端移到服务器端。 Azure Data Lake Storage Gen2 API 现在将与 Blob API 并行运行。 此迁移提高了性能，因为现在常用的 Hadoop 文件系统操作可以通过一个 REST API 调用来执行。

有关详细信息，请参阅[Azure Blob Filesystem 驱动程序（ABFS）：适用于 Hadoop 的专用 Azure 存储驱动程序](../storage/blobs/data-lake-storage-abfs-driver.md)。
