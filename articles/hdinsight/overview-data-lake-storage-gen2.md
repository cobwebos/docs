---
title: HDInsight 中的 Azure 数据存储第 2 代概述
description: HDInsight 中数据存储第 2 代的概述。
...
ms.locfileid: "81873337"
---
# <a name="azure-data-lake-storage-gen2-overview-in-hdinsight"></a>HDInsight 中的 Azure 数据存储第 2 代概述

Azure Data Lake Storage Gen2 采用了 Azure Data Lake Storage Gen1 中的核心功能，并将它们集成到了 Azure Blob 存储中。 这些功能包括与 Hadoop 兼容的文件系统、Azure Active Directory (Azure AD) 和基于 POSIX 的访问控制列表 (ACL)。 此组合允许您利用 Azure 数据存储第 1 代的性能。 同时使用 Blob 存储的分层和数据生命周期管理。
...
* **成本效益：** 数据存储湖存储 Gen2 提供低成本的存储容量和事务。 Azure Blob 存储生命周期有助于降低成本，因为数据在生命周期中移动时调整计费费率。
...
Apache Hadoop 应用程序会以本机方式从本地磁盘存储读取和写入数据。 Hadoop 文件系统驱动程序（如 ABFS）使 Hadoop 应用程序能够处理云存储。 的工作原理是模拟常规的 Hadoop 文件系统操作。 驱动程序将从应用程序收到的这些命令转换为实际云存储平台可以理解的操作。

以前，Hadoop 文件系统驱动程序将所有文件系统操作转换为客户端上的 Azure 存储 REST API 调用。 然后调用 REST API。 但是，这种客户端转换会导致针对单个文件系统操作（例如文件重命名）执行多个 REST API 调用。 ABFS 将 Hadoop 文件系统逻辑从客户端移动到服务器端。 Azure Data Lake Storage Gen2 API 现在将与 Blob API 并行运行。 此迁移提高了性能，因为现在常用的 Hadoop 文件系统操作可以通过一个 REST API 调用来执行。
