...
在本示例中，将使用映射器和化简器实现基本字数统计。 映射器会将句子分解成不同的单词，而化简器会汇总单词并统计字数以生成输出。
...
>
> * 使用 `scp` 会将文件放在群集的主头节点上。 此处假设稍后会连接到头节点并从 SSH 会话运行作业。
> * 使用 PowerShell 脚本会将文件放入群集的默认存储。 此处假定稍后会使用 PowerShell 脚本从远程客户端运行作业。
...
1. 在开发环境中，创建一个名为 `Put-FilesInHDInsight.ps1` 的新文件，并且使用以下代码作为该文件的内容：
...
2. 要使用此脚本上传文件，请在 Azure PowerShell 提示符下使用以下命令：
...
3. 重复此命令，将 `mapper.py` 替换为 `reducer.py`。 这会同时将 mapper 和 reducer 文件上传到群集的默认存储。
...
1. 通过使用 SSH 连接到群集：
   ...
   > [!NOTE]
   > 如果你使用了密码来保护 SSH 帐户，系统会提示你输入密码。 如果你使用了 SSH 密钥，则可能需要使用 `-i` 参数和私钥路径，例如 `ssh -i /path/to/private/key username@clustername-ssh.azurehdinsight.net`。
...
3. 使用以下命令启动 MapReduce 作业。
   ...
   * **-files**：告知 Hadoop 此 MapReduce 作业需要指定的文件，而且应将这些文件复制到所有辅助角色节点。
...
在开发环境中，按照以下步骤从 PowerShell 运行流式处理 MapReduce。 PowerShell 脚本使用 WebHCat 连接到 HDInsight 群集来运行作业。
...
作业的输出会存储在 `/example/wordcountout` 目录中。 可以从 SSH 会话查看此信息，也可以从本地下载并使用 PowerShell 查看。

若要从与群集的 SSH 会话查看数据，请使用以下命令：

`hdfs dfs -text /example/wordcountout/part-00000`

这会显示字词和字词出现次数的列表。 下面是输出数据的示例：
...
1. 创建一个名为 `Get-FilesInHDInsight.ps1` 的新文件，并使用以下代码作为该文件的内容：
...
2. 在 Azure PowerShell 提示符中，使用以下命令运行脚本并查看输出：
