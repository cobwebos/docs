...
1. 使用以下命令创建一个变量，其中包含用于主要群集的 Apache Zookeeper 主机。 如 `ZOOKEEPER_IP_ADDRESS1` 的字符串必须替换为之前记录的实际 IP 地址，如 `10.23.0.11` 和 `10.23.0.7`。 如果对自定义 DNS 服务器使用 FQDN 解析，请按照[以下步骤](apache-kafka-get-started.md#getkafkainfo)获取 broker 和 zookeeper 名称。：
...
1. 使用以下内容查看此（**主**）群集的 Zookeeper 主机信息：
...
1. 在配置与辅助群集通信的生成者之前，请为**辅助**群集的 broker IP 地址设置一个变量。 使用以下命令创建此变量：
    ...
    命令 `echo $SECONDARY_BROKERHOSTS` 应返回类似于以下文本的信息：
...
1. 使用以下命令创建一个环境变量，该变量具有辅助群集的 Zookeeper 主机的 IP 地址：
...
1. 通过 SSH 连接到**辅助**群集，请使用以下命令启动 MirrorMaker 进程：
    ...
    |--白名单|MirrorMaker 从主要群集复制到次要群集的主题列表。|
    |--流数|要创建的使用者线程数。|
...
3. 通过 SSH 连接到**辅助**群集，请使用**Ctrl + C**结束 MirrorMaker 进程。 它可能需要几秒钟时间结束进程。 若要验证是否已将消息复制到辅助数据库，请使用以下命令：
    ...
    主题列表现在包括 `testtopic`，在 MirrorMaster 将主题从主群集镜像到次要群集时创建。 从主题检索到的消息与在主群集上输入的消息相同。

## <a name="delete-the-cluster"></a>删除群集

[!INCLUDE [delete-cluster-warning](../../../includes/hdinsight-delete-cluster-warning.md)]

本文档中的步骤在不同的 Azure 资源组中创建了群集。 若要删除创建的所有资源，可以删除创建的两个资源组： " **kafka** " 和 " **secondary_rg kafka**"。 删除资源组会删除按照本文档创建的所有资源，包括群集、虚拟网络和存储帐户。
