...
若要轮换密钥，必须具有新密钥的完整 url （请参阅[设置 Key Vault 和密钥](#set-up-the-key-vault-and-keys)的步骤3）。 完成后，请转到门户中的 Kafka 群集属性部分，并单击 "**磁盘加密密钥 URL**" 下的 "**更改密钥**"。 输入新的密钥 url，并提交以旋转密钥。
