...
本文介绍如何在 Apache Kafka 客户端与 Apache Kafka 代理之间设置传输层安全性（TLS）加密（以前称为安全套接字层（SSL）加密）。 还介绍了如何设置客户端的身份验证（有时称为双向 TLS）。

> [!Important]
> 有两个客户端可用于 Kafka 应用程序：一个 Java 客户端和一个控制台客户端。 只有 Java 客户端`ProducerConsumer.java`可以使用 TLS 来生成和使用。 控制台生成方客户`console-producer.sh`端不能与 TLS 一起使用。
...
Kafka TLS broker 安装程序将按以下方式使用四个 HDInsight 群集 Vm：
...
## <a name="update-kafka-configuration-to-use-tls-and-restart-brokers"></a>更新 Kafka 配置以使用 TLS 并重新启动代理
...
1. 对于 HDI 版本3.6，请在 "**高级 kafka** " 和 " **kafka 模板**" 属性下，中转到 Ambari UI 并添加以下配置。
...
如果不需要身份验证，则仅设置 TLS 加密的步骤摘要为：

1. 登录到 CA （活动头节点）。
1. 从 CA 计算机将 CA 证书复制到客户端计算机（wn0）。
1. 登录到客户端计算机（hn1），并导航到该`~/ssl`文件夹。
...
1. 在客户端`client-ssl-auth.properties`计算机（hn1）上创建文件。 该文件应包含以下行：
...
1. 使用生成方和使用方选项启动管理客户端，以验证生成方和使用方是否在端口 9093 上运行。 请参阅下面的[验证](apache-kafka-ssl-encryption-authentication.md#verification)部分，了解使用控制台制造者/使用者验证设置所需的步骤。

## <a name="client-setup-with-authentication"></a>客户端设置（使用身份验证）

> [!Note]
> 仅当设置 TLS 加密**和**身份验证时，才需要执行以下步骤。 如果只是要设置加密，请参阅[客户端安装（不进行身份验证](apache-kafka-ssl-encryption-authentication.md#client-setup-without-authentication)）。
...
1. 转到客户端计算机（备用头节点）并导航到该`~/ssl`文件夹。 将已签名的证书复制到客户端计算机。
...
1. 创建带签名证书的客户端存储，并将 ca 证书导入到客户端计算机上的密钥存储和 truststore （hn1）：
...
> [!Note]
> 如果安装了 HDInsight 4.0 和 Kafka 2.1，则可以使用控制台制造者/使用者来验证你的设置。 如果没有，请在端口9092上运行 Kafka 生成程序并向主题发送消息，然后在使用 TLS 的端口9093上使用 Kafka 使用者。
...
1. 启动控制台制造者，并将路径`client-ssl-auth.properties`作为制造者的配置文件提供。

    ```bash
    /usr/hdp/current/kafka-broker/bin/kafka-console-producer.sh --broker-list <FQDN_WORKER_NODE>:9093 --topic topic1 --producer.config ~/ssl/client-ssl-auth.properties
    ```

1. 打开到客户端计算机的另一个 ssh 连接并启动控制台使用者， `client-ssl-auth.properties`并将路径作为使用者的配置文件提供。
