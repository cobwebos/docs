...
活动目录用户可以使用其域凭据登录到群集节点。 他们还可以使用域凭据来验证与其他已批准终结点（如 Hue、Ambari 视图[Hue](https://gethue.com/)、ODBC、JDBC、PowerShell 和 REST API）的群集交互。
...
![阿帕奇·安巴里仪表板管理](./media/hdinsight-authorize-users-to-ambari/manage-apache-ambari.png)
...
1. 从管理页中，选择 **"用户**"。
    ...
    ![阿帕奇·安巴里管理页面用户](./media/hdinsight-authorize-users-to-ambari/apache-ambari-management-page-users.png)

1. 选择 **= 创建本地用户**。

1. 提供**用户名和密码**。 **password** 选择 **"保存**"。

### <a name="add-users-through-powershell"></a>通过 PowerShell 添加用户

通过替换`CLUSTERNAME`和`NEWUSER``PASSWORD`以适当的值编辑下面的变量。
...
通过替换`CLUSTERNAME`、`ADMINPASSWORD``NEWUSER`和 以`USERPASSWORD`适当的值编辑下面的变量。 脚本设计为使用 bash 执行。 Windows 命令提示符需要稍作修改。
...
1. 在管理页中，选择左侧“视图”菜单标题下面的“视图”链接。********
    ...
    ![阿帕奇·安巴里视图链接](./media/hdinsight-authorize-users-to-ambari/apache-ambari-views-link.png)

2. 在“视图”页中，展开“HIVE”行。**** 有一个默认的 Hive 视图，它是在将 Hive 服务添加到群集时创建的。 还可以根据需要创建更多的 Hive 视图实例。 选择一个 Hive 视图：

    ![HDInsight 视图 - 阿帕奇蜂巢视图](./media/hdinsight-authorize-users-to-ambari/views-apache-hive-view.png)
...
1. 若要添加用户，请选择“添加用户”按钮。****
   ...
   * 开始键入用户名，随后会看到以前定义的名称的下拉列表。
     ...
     ![阿帕奇·安巴里用户自动完成](./media/hdinsight-authorize-users-to-ambari/ambari-user-autocomplete.png)

   * 选择或完成键入用户名。 若要将此用户名添加为新用户，请选择“新建”按钮。****

   * 若要保存更改，请选中**蓝色复选框**。

     ![阿帕奇·安巴里授予用户权限](./media/hdinsight-authorize-users-to-ambari/user-entered-permissions.png)
...
1. 若要添加组，请选择“添加组”按钮。****
   ...
   * 若要保存更改，请选中**蓝色复选框**。
     ...
     ![阿帕奇·安巴里授予权限](./media/hdinsight-authorize-users-to-ambari/ambari-group-entered.png)
...
![HDInsight 视图 - 阿帕奇 Tez 视图](./media/hdinsight-authorize-users-to-ambari/views-apache-tez-view.png)
...
![阿帕奇·安巴里角色菜单链接](./media/hdinsight-authorize-users-to-ambari/cluster-roles-menu-link.png)

若要查看授予每个角色的权限列表，请单击“角色”页上“角色”表标题旁边的蓝色问号。****

![阿帕奇·安巴里角色菜单链接权限](./media/hdinsight-authorize-users-to-ambari/roles-menu-permissions.png "阿帕奇·安巴里角色菜单链接权限")
...
![阿帕奇·安巴里角色块视图](./media/hdinsight-authorize-users-to-ambari/ambari-roles-block-view.png)
...
* “列表”视图的“用户”类别显示所有用户的列表，可让我们在下拉列表中选择每个用户的角色。
    ...
    ![阿帕奇·安巴里角色列表视图 - 用户](./media/hdinsight-authorize-users-to-ambari/roles-list-view-users.png)
...
* “列表”视图的“组”类别显示所有组，以及分配给每个组的角色。 在本示例中，组列表已从群集“域”设置的“访问用户组”属性中指定的 Azure AD 组同步。**** 请参阅[在启用了 ESP 的情况下创建 HDInsight 群集](./domain-joined/apache-domain-joined-configure-using-azure-adds.md#create-an-hdinsight-cluster-with-esp)。
    ...
    ![阿帕奇·安巴里角色列表视图 - 组](./media/hdinsight-authorize-users-to-ambari/roles-list-view-groups.png)
...
![阿帕奇·安巴里仪表板显示](./media/hdinsight-authorize-users-to-ambari/user-cluster-user-role.png)
