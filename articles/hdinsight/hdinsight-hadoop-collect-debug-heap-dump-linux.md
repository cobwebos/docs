...
---
# <a name="enable-heap-dumps-for-hadoop-services-on-linux-based-hdinsight-preview"></a>在基于 Linux 的 HDInsight（预览版）上为 Hadoop 服务启用堆转储
...
> [!IMPORTANT]
> 本文档中的步骤仅适用于使用 Linux 的 HDInsight 群集。 Linux 是 HDInsight 3.4 或更高版本上使用的唯一操作系统。 有关详细信息，请参阅 [HDInsight 在 Windows 上即将弃用](hdinsight-component-versioning.md#hdi-version-33-nearing-deprecation-date)。
...
### <a name="scripts"></a>脚本
还可以在发生 **OutOfMemoryError** 时触发一个脚本。 例如，可以触发一个通知，这样你就知道发生了错误。 这可以使用以下选项进行控制：
...
1. 打开群集的 Ambari Web UI。 URL 为 https://YOURCLUSTERNAME.azurehdinsight.net。
...
3. 使用“筛选...”条目，输入“opts”。 此时会筛选配置项目的列表，只保留那些包含该文本的项目。这是一种快速查找 shell 脚本的方式，也可用于快速查找可用于设置这些选项的**模板**。
...
6. 选择需要重启的每个服务，然后使用“服务操作”按钮以“打开维护模式”。 这可以防止重新启动时从该服务生成警报。
