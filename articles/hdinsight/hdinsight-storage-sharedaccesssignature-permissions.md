---
title: "使用共享访问签名限制对数据进行 HDInsight 访问"
...
---
# <a name="use-azure-storage-shared-access-signatures-to-restrict-access-to-data-with-hdinsight"></a>使用 Azure 存储空间共享访问签名来限制使用 HDInsight 访问数据
...
3. 在提示符下使用以下命令对 Azure 订阅进行身份验证：
    ...
    出现提示时，请使用 Azure 订阅的帐户进行登录。
   
    如果登录名与多个 Azure 订阅关联，可能需要使用 `Select-AzureRmSubscription` 来选择想要使用的订阅。
4. 在提示符下，将目录更改为包含 HDInsightSAS.ps1 文件的 `CreateCluster` 目录。 然后使用以下命令以运行该脚本
   
        .\HDInsightSAS.ps1
   
    当脚本运行时，在创建资源组和存储帐户时，它将记录输出到 PowerShell 命令提示符的内容。 然后它会提示你输入 HDInsight 群集的 HTTP 用户。 这是用于保护群集的 HTTP/s 访问的用户帐户。
   
    如果要创建基于 Linux 的群集，还会收到输入 SSH 用户帐户名和密码的提示。 这用于远程登录到群集。
...
### <a name="update-an-existing-cluster-to-use-the-sas"></a>更新现有群集以使用 SAS
...
5. 单击“添加”按钮以保存此密钥和值，然后单击“保存”按钮以保存配置更改。 出现提示时，请添加更改的说明（例如，“添加 SAS 存储访问”），然后单击“保存”。
   ...
   > [!IMPORTANT]
   > 尽管此操作会保存配置更改，但必须重启几个服务才能使更改生效。
   > 
   > 
6. 在 Ambari Web UI 中，从左侧的列表中选择“HDFS”，然后从右侧的“服务操作”下拉列表选择“全部重启”。 出现提示时，选择“打开维护模式”，然后选择“确认全部重启”。
   
    从页面左侧的列表中，为 MapReduce2 和 YARN 条目重复此过程。
7. 当这些服务重启后，选择每个服务，然后从“服务操作”下拉列表中禁用维护模式。
...
* 对于**基于 Linux** 的 HDInsight 群集，请使用 SSH 连接到群集。 有关如何将 SSH 与基于 Linux 的群集配合使用的详细信息，请参阅以下任一文档：
...
1. 在提示符下键入以下命令。 将 **SASCONTAINER** 替换为针对 SAS 存储帐户创建的容器名称。 将 **SASACCOUNTNAME** 替换为用于 SAS 的存储帐户名称：
