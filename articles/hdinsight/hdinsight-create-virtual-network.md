...
1. 若要使用配置，请重新启动 Bind。 例如，`sudo service bind9 restart` 。

1. 将一个条件转发器添加到本地 DNS 服务器。 配置条件转发器，以便将步骤 1 中 DNS 后缀的请求发送到自定义 DNS 服务器。
...
2. 使用以下文本作为自定义 DNS 服务器上 `/etc/bind/named.config.local` 文件的内容。 在这两个虚拟网络中的自定义 DNS 服务器上进行此更改。
    ...
    将 `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.cloudapp.net` 值替换为另一个____ 虚拟网络的 DNS 后缀。 此条目将远程网络 DNS 后缀的请求路由到该网络中的自定义 DNS。
