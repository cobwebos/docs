...
| [为 HDInsight 上的 Apache Storm 开发基于 Java 的拓扑][5797064f] |Maven |Java |
...
| [使用 Storm on HDInsight 从 Azure 事件中心处理事件 (Java)](https://github.com/Azure-Samples/hdinsight-java-storm-eventhub) |事件中心 |Java |
| [使用 Apache Storm on HDInsight 处理事件中心的车辆传感器数据][246ee964] |事件中心、Cosmos DB、Azure 存储 Blob (WASB) |C#、Java |
| [使用 HDInsight 上的 Apache Storm 从 Azure 事件中心提取、转换和加载（ETL）到 Apache HBase][b4b68194] |事件中心、HBase |C# |
| [用于C#从 HDInsight 上的 Apache Storm 使用 Azure 服务的模板风暴拓扑项目][ce0c02a2] |事件中心、Cosmos DB、SQL 数据库、HBase、SignalR |C#、Java |
| [使用 HDInsight 上的 Apache Storm 从 Azure 事件中心读取的可伸缩性基准][d6c540e3] |消息吞吐量、事件中心、SQL 数据库 |C#、Java |
