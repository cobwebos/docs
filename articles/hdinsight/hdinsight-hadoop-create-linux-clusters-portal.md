...
|群集类型|单击 "**选择群集类型**" 以打开列表。 从列表中选择所需的群集类型。 HDInsight 群集采用不同的类型。 这些类型与该群集进行优化的工作负荷或技术相对应。 没有任何方法支持创建组合多种类型的群集，|
...
从”主存储类型”的下拉列表中，选择默认存储类型。  稍后的字段将因你的选择而异。 对于 **Azure 存储**：
