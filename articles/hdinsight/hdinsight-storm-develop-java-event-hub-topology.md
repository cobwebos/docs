...
* Apache Storm on HDInsight 3.4 版群集。 参考下列其中一篇入门文章来创建群集：
  
  * HDInsight 群集。 有关创建新群集的信息，请参阅[在 HDInsight 群集中开始使用 Storm](hdinsight-apache-storm-tutorial-get-started-linux.md)。
...
* 文本编辑器或 Java 集成开发环境 (IDE)。
  ...
  * SSH 客户端。 有关如何将 SSH 与 HDInsight 配合使用的详细信息，请参阅以下文章之一：
    ...
    * [在 Linux、Unix、OS X 或 Bash on Windows 10 的 HDInsight 上结合使用 SSH 和基于 Linux 的 Hadoop](hdinsight-hadoop-linux-use-ssh-unix.md)。

    * [在 Windows 的 HDInsight 上结合使用 SSH (PuTTY) 和基于 Linux 的 Hadoop](hdinsight-hadoop-linux-use-ssh-windows.md)。

* 一个 SCP 客户端。 此客户端已随附所有的 Linux、Unix 和 OS X 系统。 对于 Windows 客户端，建议使用可从 [PuTTY download page](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html)（PuTTY 下载页面）中获取的 PSCP。
...
**com.microsoft.example.EventHubReader** 从事件中心读取数据（由 EventHubWriter 写入的数据），并将其存储到 /devicedata 目录的群集默认存储中。
...
> [!NOTE]
> 如果使用早期版本的 HDInsight（例如 3.2 版），则必须手动注册这些组件。 有关此示例以及早期版本的 HDInsight 群集所需的自定义位，请参阅示例存储库的 [Storm v0.9.3](https://github.com/Azure-Samples/hdinsight-java-storm-eventhub/tree/Storm_v0.9.3) 分支。
...
必须在 `EventHubs.properties` 文件中填充事件中心和 HDInsight 群集的相关信息。
...
2. 在“添加新事件中心”屏幕中，输入“事件中心名称”，选择“区域”以在其中创建中心，然后创建新的命名空间或选择现有命名空间。 单击“箭头”继续。
   ...
   > [!NOTE]
   > 应该选择与 Storm on HDInsight 服务器相同的**位置**，以降低延迟和成本。
...
2. 使用以下命令生成和打包项目：
    ...
    这会下载所需的依赖项，进行生成，然后打包项目。 输出将作为 **EventHubExample-1.0-SNAPSHOT.jar** 存储在 **/target** 目录中。
...
### <a name="if-using-a-linux-based-cluster"></a>如果使用基于 Linux 的群集
...
2. 上传文件完成后，可使用 SSH 连接到 HDInsight 群集。 将 **USERNAME** 替换为 SSH 登录名。 将 **CLUSTERNAME** 替换为 HDInsight 群集名：
   ...
   > [!NOTE]
   > 如果使用了 SSH 帐户密码，则系统将提示输入该密码。 如果将 SSH 密钥与帐户配合使用，则可能需要使用 `-i` 参数来指定密钥文件的路径。 以下示例将从 `~/.ssh/id_rsa` 加载私钥：
   > 
...
3. 使用以下命令启动拓扑：
    ...
    这会启动拓扑，并为它们提供“reader”和“writer”友好名称。

4. 拓扑需要一到两分钟的时间将事件写入事件中心或从中读取事件，然后使用以下命令验证 EventHubReader 是否正在将数据存储到 HDInsight 存储：
...
### <a name="if-using-a-windows-based-hdinsight-cluster"></a>如果使用基于 Windows 的 HDInsight 群集

> [!IMPORTANT]
> Linux 是 HDInsight 3.4 或更高版本上使用的唯一操作系统。 有关详细信息，请参阅 [HDInsight Deprecation on Windows](hdinsight-component-versioning.md#hdi-version-32-and-33-nearing-deprecation-date)（HDInsight 在 Windows 上即将弃用）。

1. 打开浏览器，并导航到 https://CLUSTERNAME.azurehdinsight.net。 出现提示时，输入 HDInsight 群集的管理员凭据。 将进入 Storm 仪表板。

2. 使用“Jar 文件”下拉列表在生成环境中浏览并选择 EventHubExample-1.0-SNAPSHOT.jar 文件。

3. 对于“类名”，请输入 `com.mirosoft.example.EventHubWriter`。

4. 对于“其他参数”，请输入 `writer`。 最后，单击“提交”上传 jar 并启动 EventHubWriter 拓扑。

5. 启动拓扑后，使用窗体来启动 EventHubReader：
   
   * **Jar 文件**：选择前面上传的 EventHubExample-1.0-SNAPSHOT.jar
...
6. 等待几分钟让拓扑生成事件并将其存储到 Azure 存储，然后选择“Storm 仪表板”页面顶部的“Hadoop 查询控制台”选项卡。
...
7. 在“查询控制台”上，选择“Hive 编辑器”并将默认的 `select * from hivesampletable` 替换为以下内容：
    ...
    单击“选择”以运行查询。 这会返回 10 行由 EventHubReader 写入群集存储的数据。 完成查询后，你应会看到类似于下面的数据：
...
8. 选择页面顶部的“Storm 仪表板”，然后选择“Storm UI”。 从“Storm UI”中选择 **reader** 拓扑的链接，然后使用“终止”按钮停止拓扑。 针对 **writer** 拓扑重复该过程。
...
可将持久性检查点导入和导出到 WASB（基于 Windows 的 HDInsight 群集所用的 Azure 存储。）用于执行此操作的脚本位于 Storm on HDInsight 群集上的 **c:\apps\dist\storm-0.9.3.2.2.1.0-2340\zkdatatool-1.0\bin** 中。
