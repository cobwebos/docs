...
1. 使用远程桌面连接到域控制器。
    ...
    1. 在 Azure 门户中，选择 "**资源组**" > **OnPremADVRG** > **adVM** > "**连接**"。
...
1. 创建两个新用户： **HDIAdmin**和**HDIUser**。 这两个用户将登录到 HDInsight 群集。
    ...
    1. 在 " **Active Directory 用户和计算机**" 页上，选择 "**操作**" > **New** > **User**"。
    ...
    1. 在出现的弹出窗口中，输入新帐户的密码。 选择 "**密码永不过期** **" > "确定"** 。
    1. 选择 "**完成**" 以创建新帐户。
...
1. 在 " **Active Directory 用户和计算机**" 页上，选择 "**操作**" > **新** > **组**。 添加名为*HDIUserGroup*的组。
...
![创建用户分配托管标识](./media/apache-domain-joined-create-configure-enterprise-security-cluster/hdinsight-image-0117.png)
