...
1. 创建两个新用户： **HDIAdmin**和**HDIUser**。 这两个用户将登录到 HDInsight 群集。
    ...
    1. 在 "**新建对象-用户**" 页上 **，为 "名字"** 和 "**用户登录名**" 输入 `HDIUser`。 其他字段将自动填充数据。 然后，选择“下一步”。
...
1. 创建全局安全组。
    ...
    1. 在**Active Directory 用户和计算机**"中，右键单击 `HDIFabrikam.com`，然后导航到"**新建** > **组**"。

    1. 在 "**组名称**" 文本框中输入 `HDIUserGroup`。
...
1. 登录到 Azure 门户。
1. 选择 "**创建资源**"，然后键入 `managed identity`。 选择**用户分配的托管身份** > **创建**。
...
1. 找到自定义 DNS 服务器的 IP 地址。
    1. 选择 Azure AD DS 资源的 `HDIFabrikam.com`。
    1. 在 "**管理**" 下选择 "**属性**"。
...
> [!NOTE]  
> 任何创建有效公钥加密标准（PKCS） \#10 请求的实用程序或应用程序都可用于形成 SSL 证书请求。
...
1. 为 Azure AD DS 托管域启用 LDAPS。
    1. 从 "Azure 门户中，选择" 域 `HDIFabrikam.com`"。
    1. 在 "**管理**" 下，选择**安全 LDAP**。
...
1. 在承载 Azure AD DS （`HDIFabrikam-AADDSVNET`）的虚拟网络与将托管启用了 ESP 的 HDInsight 群集（`HDIFabrikam-HDIVNet`）的虚拟网络之间创建对等关系。 使用以下 PowerShell 代码对这两个虚拟网络对等互连。
...
1. 在**HDIFabrikam-AADDSVNET**虚拟网络上设置自定义 DNS。
    1. 请参阅 Azure 门户 > 资源组 > **OnPremADVRG** ** >  > ** **DNS 服务器**的**资源组**。
