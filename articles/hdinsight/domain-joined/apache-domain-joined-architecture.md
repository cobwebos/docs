...
description: 了解如何使用企业安全包规划 Azure HDInsight 安全性。
...
- 一个 Active Directory 域（由 Azure AD DS 管理）。 **域名必须为 39 个字符或更少才能使用 Azure HDInsight。**
...
如果您正在与活动目录联合服务 （AD FS） 联合使用，则必须启用密码哈希同步。（有关建议的设置，请参阅[此视频](https://youtu.be/qQruArbu2Ew)。密码哈希同步有助于在 AD FS 基础结构发生故障时进行灾难恢复，它还有助于提供泄漏的凭据保护。 有关详细信息，请参阅[使用 Azure AD Connect 同步启用密码哈希同步](../../active-directory/hybrid/how-to-connect-password-hash-synchronization.md)。

在 IaaS VM 上单独使用本地 Active Directory 或 Active Directory 而不使用 Azure AD 和 Azure AD DS，这是使用 ESP 的 HDInsight 群集不支持的配置。

如果使用联合身份验证，并且密码哈希正确同步，但您出现身份验证失败，请检查是否为 PowerShell 服务主体启用了云密码身份验证。 如果没有，则必须为你的 Azure AD 租户设置[主领域发现 (HRD) 策略](../../active-directory/manage-apps/configure-authentication-for-federated-users-portal.md)。 若要检查和设置 HRD 策略，请执行以下操作：

1. 安装预览[Azure AD 电源外壳模块](https://docs.microsoft.com/powershell/azure/active-directory/install-adv2)。
...
- [使用 ESP 管理 HDInsight 群集](apache-domain-joined-manage.md)
