...
## <a name="integrate-hdinsight-with-ad-and-ad-on-iaas-vm"></a>将 HDInsight 与 AD 以及 IaaS VM 上的 AD 集成

将 HDInsight 与 Azure AD 或 IaaS VM 上的 AD 集成后，HDInsight 群集节点即可加入域。 HDInsight 为在群集上运行的 Hadoop 服务创建服务主体，并将其置于 Azure AD 或 IaaS VM 上的 AD 中的指定组织单位 (OU) 内。 HDInsight 还会针对已加入域的节点的 IP 地址，在域中创建反向 DNS 映射。

可以使用多个体系结构来实现这种设置。 可从以下体系结构中选择。

HDInsight 与 Azure IaaS 上运行的 AD 集成

这是将 HDInsight 与 Active Directory 集成的最简单体系结构。 AD 域控制器在 Azure 中的一个（或多个）虚拟机 (VM) 上运行。 这些 VM 通常位于虚拟网络中。 可为 HDInsight 群集设置另一个虚拟网络。 若要让 HDInsight 能否发现 Active Directory，需通过 [VNet 到 VNet 对等互连](../virtual-network/virtual-network-create-peering.md)让这些虚拟网络能够实现对等互连。 如果在 ARM 中创建 Active Directory，则可在同一 VNet 中创建 Active Directory 和 HDInsight，无需执行对等互连。 
...
对于仅限云的 Azure AD，请配置一个域控制器，使 HDInsight 能够与 Azure AD 集成。 此过程是使用 [Azure Active Directory 域服务](../active-directory-domain-services/active-directory-ds-overview.md) (Azure AD DS) 实现的。 Azure AD DS 在云中创建域控制器计算机并提供其 IP 地址。 它会创建两个域控制器，以确保高可用性。
