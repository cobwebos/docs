...
## <a name="notes-for-08302016-release-of-hdinsight"></a>HDInsight 08/30/2016 版发行说明
...
## <a name="notes-for-07072016-release-of-hdinsight"></a>HDInsight 07/07/2016 版发行说明
随此版本一起部署的基于 Linux 的 HDInsight 群集的所有版本号包括：
...
| 标题 | 说明 | 受影响区域（例如服务、组件或 SDK） | 群集类型（例如 Spark、Hadoop、HBase 或 Storm） | JIRA（如果适用） |
| --- | --- | --- | --- | --- |
| [用于 IntelliJ 的 HDInsight 工具](hdinsight-apache-spark-intellij-tool-plugin.md) |适用于 HDInsight Spark 群集的 IntelliJ IDEA 插件现与用于 IntelliJ 的 Azure 工具包集成。 它支持 Azure SDK v2.9.1、最新的 Java SDK 且包括适用于 IntelliJ 的独立 HDInsight 插件的所有功能。 |工具 |Spark |不适用 |
| [用于 Eclipse 的 HDInsight 工具](hdinsight-apache-spark-eclipse-tool-plugin.md) |用于 Eclipse 的 Azure 工具包现在支持 HDInsight Spark 群集。 它启用了以下功能： <ul><li>借助一流的创作支持（IntelliSense、自动套用格式、错误检查等），用 Scala 和 Java 轻松创建和写入 Spark 应用程序。</li><li>本地测试 Spark 应用程序。</li><li>将作业提交到 HDInsight Spark 群集并检索结果。</li><li>登录到 Azure 并访问与 Azure 订阅相关联的所有 Spark 群集。</li><li>导航 HDInsight Spark 群集的所有关联存储资源。</li></ul> |工具 |Spark |不适用 |

从此版本开始，我们已经为基于 Linux 的 HDInsight 群集更改了来宾 OS 修补策略。 新策略的目标是显著减少由于修补导致的重启次数。 新策略在 Linux 群集上修补虚拟机 (VM)，每周一或周四从 12AM UTC 开始以交错的方式在任意给定群集的节点上进行。 但是由于来宾 OS 修补，任何给定 VM 最多每 30 天重启一次。 此外，自新创建群集的创建日期起 30 天内，该群集不会发生第一次重启。

> [!NOTE]
> 这些更改仅适用于等同于或高于此发行版本的新创建群集。
>
>

## <a name="notes-for-06062016-release-of-hdinsight"></a>HDInsight 2016/06/06 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：

| HDP | HDI 版本 | Spark 版本 | Ambari 生成号 | HDP 生成号 |
| --- | --- | --- | --- | --- |
...
| 用于 IntelliJ 的 HDInsight 工具 |这是针对 HDInsight Spark 群集的 IntelliJ IDEA 插件。 它启用了以下功能：<ul><li>借助一流的创作支持（IntelliSense、自动套用格式、错误检查等），用 Scala 和 Java 轻松创建和写入 Spark 应用程序。</li><li>本地测试 Spark 应用程序。</li><li>将作业提交到 HDInsight Spark 群集并检索结果。</li><li>登录到 Azure 并访问与 Azure 订阅相关联的所有 Spark 群集。</li><li>导航 HDInsight Spark 群集的所有关联存储资源。</li><li>为 HDInsight Spark 群集导航所有作业历史记录和作业信息。</li><li>从台式计算机远程调试 Spark 作业。</li></ul> |工具 |Spark |不适用 |
...
| 标题 | 说明 | 受影响区域（例如服务、组件或 SDK） | 群集类型（例如 Spark、Hadoop、HBase 或 Storm） | JIRA（如果适用） |
| --- | --- | --- | --- | --- |
| Spark 版本更新和其他 Bug 修复 |此版本将 HDInsight 群集中的 Spark 版本更新至 1.6.1 并修复其他 Bug |服务 |Spark |不适用 |

## <a name="notes-for-04112016-release-of-hdinsight"></a>HDInsight 2016/04/11 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
| HDInsight 高级版 |HDInsight 现已推出两种类别：标准版和高级版。 HDInsight 高级版当前处于预览状态，仅可用于 Linux 上的 Hadoop 和 Spark 群集。 有关详细信息，请参阅[此文](hdinsight-component-versioning.md#hdinsight-standard-and-hdinsight-premium)。 |服务 |Linux 上的 Hadoop 和 Spark |不适用 |
| Microsoft R Server |HDInsight 高级版提供了可随附在 Linux 上的 Hadoop 和 Spark 群集中的 Microsoft R Server。 有关详细信息，请参阅 [HDInsight 上的 R Server 概述](hdinsight-hadoop-r-server-overview.md)。 |服务 |Linux 上的 Hadoop 和 Spark |不适用 |
| Spark 1.6.0 |HDInsight 3.4 群集现在包括 Spark 1.6.0 |服务 |Linux 上的 Spark 群集 |不适用 |
| Jupyter 笔记本增强功能 |Spark 群集随附的 Jupyter 笔记本现可提供其他 Spark 内核。 其中还包括了增强功能，例如使用 %%magic、自动可视化和与 Python 可视化库集成（如 matplotlib）。 有关详细信息，请参阅 [Jupyter 笔记本的可用内核](hdinsight-apache-spark-jupyter-notebook-kernels.md)。 |服务 |Linux 上的 Spark 群集 |不适用 |

## <a name="notes-for-03222016-release-of-hdinsight"></a>HDInsight 03/22/2016 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-03102016-release-of-hdinsight"></a>HDInsight 03/10/2016 发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-01272016-release-of-hdinsight"></a>HDInsight 01/27/2016 发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-12022015-release-of-hdinsight"></a>HDInsight 12/02/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-11302015-release-of-hdinsight"></a>HDInsight 11/30/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-10272015-release-of-hdinsight"></a>HDInsight 10/27/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
| 标题 | 说明 | 受影响区域（例如服务、组件或 SDK） | 群集类型（例如 Hadoop、HBase 或 STORM） | JIRA（如果适用） |
| --- | --- | --- | --- | --- |
| 更新了所有 HDInsight 群集（Windows 和 Linux）的 HDInsight 版本 |在此版本中，HDInsight 和 HDP 版本已更新 |服务 |全部 |不适用 |
| 修复了大写字母群集的 Windows Spark 群集的 Jupyter |在以大写字母形式指定 DNS 名称的群集中，由于原始请求检查，Jupyter 笔记本存在问题。 解决方法是将 Jupyter 的配置的 DNS 名称更改为小写形式。 |服务 |HDInsight Spark (Windows) |不适用 |

## <a name="notes-for-10202015-release-of-hdinsight"></a>HDInsight 10/20/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
| 标题 | 说明 | 受影响区域（例如服务、组件或 SDK） | 群集类型（例如 Hadoop、HBase 或 STORM） | JIRA（如果适用） |
| --- | --- | --- | --- | --- |
| 默认 HDP 版本已更改为 HDP 2.2 |HDInsight Windows 群集的默认版本已更改为 HDP 2.2。 2015 年 2 月已推出 HDInsight 版本 3.2 (HDP 2.2) 正式版。 此项更改只会在使用 Azure 门户、PowerShell cmdlet 或 SDK 群集预配的同时未进行明确选择时，才切换默认群集版本。 |服务 |全部 |不适用 |
| VM 名称格式进行了更改，以便在单个虚拟网络中的 Linux 群集上部署多个 HDInsight |此版本将添加在单个虚拟网络中部署多个 HDInsight Linux 群集的支持。 作为更新的一部分，群集中虚拟机名称的格式已从 headnode\*、workernode\* 和 zookeepernode\* 分别更改为 hn\*、wn\* 和 zk\*。 不建议直接依赖于虚拟机名称的格式，因为其可能发生变更。 在本机计算机或 Ambari API 上使用“hostname -f”以确定主机列表和组件到主机上的映射。 有关详细信息，请访问 [https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/hosts.md](https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/hosts.md) 和 [https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/host-components.md](https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/host-components.md)。 |服务 |Linux 上的 HDInsight 群集 |不适用 |
...
## <a name="notes-for-09092015-release-of-hdinsight"></a>HDInsight 09/09/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-07312015-release-of-hdinsight"></a>HDInsight 07/31/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-07072015-release-of-hdinsight"></a>HDInsight 07/07/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-06262015-release-of-hdinsight"></a>HDInsight 06/26/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-06182015-release-of-hdinsight"></a>HDInsight 06/18/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
<td>云服务现在会打开群集上的 5 个端口（8001 到 8005），例如 https://<clustername>.azurehdinsight.net:8001/。 对这些 URL 的请求使用相同的基本身份验证密码机制身份验证作为端口 443。 这些端口绑定到活动头节点上的相同端口。 使用脚本操作可让客户服务在头节点的这些端口上侦听并路由到群集外部。</td>
<td>云服务</td>
<td>全部</td>
<td>不适用</td>
</tr>
...
## <a name="notes-for-06042015-release-of-hdinsight"></a>HDInsight 06/04/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-06012015-release-of-hdinsight"></a>HDInsight 06/01/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-05272015-release-of-hdinsight"></a>HDInsight 05/27/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
<td>在此更新中，由节点管理器启动的 YARN 容器 (yarn.nodemanager.resource.memory-mb and yarn.scheduler.maximum-allocation-mb) 的默认可用内存已增加到 5632MB。 以前此内存量缩减为 4608MB，但根据不同的作业运行，新值必须为大多数作业提供更高的可靠性与性能，因此此默认值更好。 如往常一样，如果严重依赖于此内存配置，请在创建群集时显式设置内存。</td>
<td>HDP</td>
<td>所有群集类型</td>
<td>不适用</td>
</tr>
...
## <a name="notes-for-05202015-release-of-hdinsight"></a>HDInsight 05/20/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-04272015-release-of-hdinsight"></a>HDInsight 04/27/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
<tr>
<td>争用状态 Bug 修复</td>
<td>群集创建请求现在会等待 PUT 请求获得接受，才进行状态轮询</td>
<td>SDK 中 IsInRole 中的声明</td>
<td>Hadoop</td>
<td>不适用</td>
</tr>
...
## <a name="notes-for-04142015-release-of-hdinsight"></a>HDInsight 04/14/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
<tr>
<td>Tez Bug 修复</td>
<td>此版本的 HDI 3.2 包含 Apache TEZ 2214 和 TEZ 1923 的修复程序。 对于需要随机排列大量数据、对 Tez 执行的某些 Hive 查询而言，需要这些修复程序。
</td>
<td>HDP</td>
<td>Hadoop</td>
...
## <a name="notes-for-04012015-release-of-hdinsight"></a>HDInsight 04/01/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
<td>HDInsight 版本 3.2 对这一更新包含几个缺陷修补程序，并打包为 HDP 的一部分的几个内部 jar 已升级。 这些 JAR 文件是内部 HDP 包，而不是针对客户应用程序直接使用。 应用程序应打包自己版本的 Jar，以便升级到 HDP 内部 Jar 不会中断客户应用程序。</td>
<td>HDP</td>
<td>全部</td>
<td>不适用</td>
</tr>
...
## <a name="notes-for-03032015-release-of-hdinsight"></a>HDInsight 03/03/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-02182015-release-of-hdinsight"></a>HDInsight 02/18/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
<td>Hadoop 2.6/HDP2.2，同时提供 HDInsight 3.2 群集。 它包含对所有的开放源组件的重大更新。 有关详细信息，请参阅“HDInsight 中的新增功能”和 <a href ="http://docs.hortonworks.com/HDPDocuments/HDP2/HDP-2.2.0/HDP_2.2.0_Release_Notes_20141202_version/index.html" target="_blank">HDP 2.2.0.0 发行说明</a>。</td>
<td>开源软件</td>
<td>全部</td>
<td>不适用</td>
</tr>
...
## <a name="notes-for-02062015-release-of-hdinsight"></a>HDInsight 02/06/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-1292015-release-of-hdinsight"></a>HDInsight 1/29/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-152015-release-of-hdinsight"></a>HDInsight 1/5/2015 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
Twitter 等网站所提供的公共 API 是一类用于分析和了解流行趋势的有用数据源。 在此教程中，可以学习如何使用 Hive 获取发送包含特定文字的多数推文的 Twitter 用户列表。 </p>

<p><b>Mahout 电影推荐</b><br>
Apache Mahout 是 Apache Hadoop 的机器学习库。 Mahout 包含用于处理数据的算法（例如筛选、分类和群集）。 在本教程中，将要使用一个推荐引擎根据好友观看的电影生成电影推荐。</p></td>
<td>查询控制台</td>
<td>Hadoop</td>
<td>不适用</td>
</tr>
...
## <a name="notes-for-12232014-release-of-hdinsight"></a>HDInsight 12/23/2014 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-12182014-release-of-hdinsight"></a>HDInsight 12/18/2014 版发行说明
此版本包含以下组件更新：
...
## <a name="notes-for-12052014-release-of-hdinsight"></a>HDInsight 12/05/2014 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-11212014-release-of-hdinsight"></a>HDInsight 11/21/2014 版发行说明
随此版本一起部署的 HDInsight 群集的所有版本号包括：
...
## <a name="notes-for-11142014-release-of-hdinsight"></a>HDInsight 11/14/2014 版发行说明
...
## <a name="notes-for-11072014-release-of-hdinsight"></a>HDInsight 11/07/2014 版发行说明
...
也可通过 REST API（位于 https://{ClusterDnsName}）远程检索此信息。 azurehdinsight.net/ws/v1/applicationhistory/。
...
## <a name="notes-for-10152014-release"></a>10/15/2014 版发行说明
...
## <a name="notes-for-1072014-release"></a>10/7/2014 版发行说明

* 使用 Ambari 终结点“https://{clusterDns}.azurehdinsight.net/ambari/api/v1/clusters/{clusterDns}.azurehdinsight.net/services/{servicename}/components/{componentname}”时，*host_name* 字段会返回节点的完全限定域名 (FQDN)，而不只是主机名。 例如，会看到 FQDN“**headnode0.{ClusterDNS}.azurehdinsight.net**”，而不是返回“**headnode0**”。 需要这种改变促进实现可以在一个虚拟网络中部署多个群集类型（如 HBase 和 Hadoop）的方案。 例如，使用 HBase 作为 Hadoop 的后端平台时，会发生这种情况。
...
* 此错误是已知的[兼容性问题](https://social.msdn.microsoft.com/Forums/azure/a7de016d-8de1-4385-b89e-d2e7a1a9d927/hdinsight-powershellsdk-error-cluster-is-not-configured-for-http-services-access?forum=hdinsight)，起因于 HDInsight SDK 或 Azure PowerShell 版本和群集版本的差异。 8 月 15 日或之后创建的群集支持虚拟网络的新配置功能。 但旧版的 SDK 或 Azure PowerShell 无法正确解释此功能。 结果造成某些作业提交操作失败。 如果使用 HDInsight SDK API 或 Azure PowerShell cmdlet 来提交作业（**Use-AzureRmHDInsightCluster** 或 **Invoke-AzureRmHDInsightHiveJob**），这些作业可能失败并返回错误消息“群集 <clustername> 未配置 HTTP 服务访问”。 或者（根据具体的操作），可能会收到其他错误消息，例如“无法连接到群集”。
...
## <a name="notes-for-8212014-release"></a>8/21/2014 版本发行说明
* 我们正在添加以下 WebHCat 配置 (HIVE-7155)，该配置可将 Templeton 控制器作业的默认内存限制设置为 1 GB： （以前的默认值是 512 MB。）
  ...
  * 要恢复到旧默认值，可以在创建群集时使用以下命令通过 Azure PowerShell 将此配置值设置为 512：
...
由于 Azure PowerShell 或 HDInsight SDK 与群集之间的版本不同，可能会遇到 Azure PowerShell 或 HDInsight SDK 错误消息“群集 <clustername> 未配置 HTTP 服务访问”（或者根据操作，遇到其他错误消息，如：“无法连接到群集”）。 8 月 15 日或之后创建的群集支持虚拟网络的新配置功能。 旧版本 Azure PowerShell 或 HDInsight SDK 无法正确解释此功能，导致提交作业操作失败。 如果使用 HDInsight SDK API 或 Azure PowerShell cmdlet（例如 Use-AzureRmHDInsightCluster 或 Invoke-AzureRmHDInsightHiveJob）来提交作业，这些作业可能失败并返回上述其中一个错误消息。
...
## <a name="notes-for-6242014-release"></a>6/24/2014 版本发行说明
此版本包含 HDInsight 服务的几项新的增强功能：

* **HDP 2.1 可用性**：HDInsight 3.1（包含 HDP 2.1）已正式发布，并成为新群集的默认版本。
* **HBase - Azure 门户改进**：我们会在预览版中提供 HBase 群集。 只需单击几下鼠标，就能从门户创建 HBase 群集：
...
默认情况下，没有为 Hive 查询启用 Tez。 要使用 Tez，必须选择启用它。 可以通过运行以下代码段来启用 Tez：
...
有关详细信息，请参阅 [Tez 上的 Hive](https://cwiki.apache.org/confluence/display/Hive/Hive+on+Tez)。
...
与 HDInsight 3.1 群集一起部署的自定义 Oozie 元存储数据库无法在 HDInsight 2.1 群集上重复使用。 即使是源自 2.1 群集的元存储，也是如此。 不支持此方案，因为元存储架构与 3.1 群集一起使用时升级，所以与 2.1 群集所需的元存储就不再兼容。 尝试重复使用已在 HDInsight 3.1 群集上使用过的 Oozie 元存储会使 HDInsight 2.1 群集变得无用。
...
**前缀语法**：HDInsight 3.1 和 3.0 群集仅支持“wasb://”语法。 较早的“asv://”语法在 HDInsight 2.1 和 1.6 群集中受支持，但在 HDInsight 3.1 或 3.0 群集中不受支持。 这意味着提交到 HDInsight 3.1 或 3.0 群集的任何显式使用“asv://”语法的作业都将会失败。 应改用“wasb://”语法。 而且，提交到任何 HDInsight 3.1 或 3.0 群集的作业，如果是使用现有元存储创建的，而该元存储包含对使用“asv://”语法的资源的显式引用，则这些作业也会失败。 需要使用“wasb://”语法重新创建这些元存储，确定资源地址。
...
* HDInsight 版本 3.1 使用基于 [Hortonworks 数据平台 2.1.7][hdp-2-1-7] 的 Hadoop 分发版。 这是使用 11/7/2014 之后的 Azure 门户时创建的默认 Hadoop 群集。 创建于 2014/11/7 之前的 HDInsight 3.1 群集基于 [Hortonworks 数据平台 2.1.1][hdp-2-1-1]
