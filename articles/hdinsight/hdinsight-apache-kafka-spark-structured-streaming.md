...
1. 在 Web 浏览器中，导航到 `https://CLUSTERNAME.azurehdinsight.net/jupyter`，其中 `CLUSTERNAME` 是群集的名称。 出现提示时，输入创建群集时使用的群集登录名（管理员）和密码。
...
在本教程中，你已了解了如何使用 Apache Spark 结构化流式处理。 在 Apache Kafka on HDInsight 中写入和读取数据。 使用以下链接，了解如何将 Apache Storm 和 Kafka 结合使用。
