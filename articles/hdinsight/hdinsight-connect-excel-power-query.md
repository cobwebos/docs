...
4. 对于“帐户名称”，请输入与群集相关联的 Azure Blob 存储帐户名称，然后单击“确定”。 此帐户可以是[默认存储帐户](hdinsight-administer-use-management-portal.md#find-the-default-storage-account)或链接的存储帐户。  格式为 *https://&lt;StorageAccountName>.blob.core.windows.net/*。
5. 对于“帐户密钥”，请输入 Blob 存储帐户的密钥，然后单击“保存”。 （只有在第一次访问此存储时才需要输入帐户信息。）
