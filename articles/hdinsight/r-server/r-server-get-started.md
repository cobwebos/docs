...
可以使用 Azure 资源管理器模板、SDK 和 PowerShell 自动创建 HDInsight R Server。
...
4. 选择“群集类型”以打开“群集配置”边栏选项卡。 在“群集配置”边栏选项卡上选择以下选项：
    ...
    * 将其他选项保留为默认值，并使用“选择”按钮保存群集类型。
        ...
        ![群集类型边栏选项卡截图](./media/r-server-get-started/clustertypeconfig.png)
...
5. 输入“群集登录用户名”和“群集登录密码”。
    ...
    * 然后在分配 HDI 群集凭据时指定公钥文件 (*.pub)，最后确认资源组和区域并选择“下一步”。
        ...
        ![凭据边栏选项卡](./media/r-server-get-started/publickeyfile.png)  
...
6. 快速创建会转换到“存储”边栏选项卡以选择要用于群集使用的 HDFS 文件系统主位置的存储帐户设置。 选择新的或现有的 Azure 存储帐户或者现有的 Data Lake Storage 帐户。
    ...
    - 如果选择 Azure 存储帐户，可通过选择“选择存储帐户”，然后选择相关帐户来选择现有存储帐户。 使用“选择存储帐户”部分中的“新建”链接创建新帐户。
      ...
      如果选择了“新建存储帐户”选项，系统将提示选择“位置”，即选择要创建存储帐户的区域。  
         ...
         ![数据源边栏选项卡](./media/r-server-get-started/datastore.png)  
...
7. 随后会显示“摘要”边栏选项卡，验证所有设置。 在此处可以更改“群集大小”以修改群集中的服务器数，还可指定要运行的任何“脚本操作”。 除非确定需要更大的群集，否则请保留辅助角色节点数目的默认值 `4`。 该群集的预估成本将显示在边栏选项卡内。
   ...
   * R Server 分析的性能与所要分析的数据大小呈线性关系。 例如：  
     ...
     * 如果登录到边缘节点并运行 R 脚本，则将在边缘节点上本地<strong></strong>执行除 ScaleR rx-functions 外的所有函数。 因此，应相应调整边缘节点的内存和核心数。 如果通过便携式计算机使用 HDI 上的 R Server 作为远程计算上下文，则这一点同样适用。
     ...
     ![节点定价层边栏选项卡](./media/r-server-get-started/pricingtier.png)
...
2. 在 Azure 门户中打开群集的条目，选择“R Server 仪表板”快速链接，然后选择“R Studio 仪表板”：
     ...
     ![访问 R Studio 仪表板](./media/r-server-get-started/rstudiodashboard1.png)

     ![访问 R Studio 仪表板](./media/r-server-get-started/rstudiodashboard2.png)
...
![并发用户 1](./media/r-server-get-started/concurrent-users-1.png)
...
- 首先，通过 HDInsight 网关使用 HTTP 用户凭据登录： 
    ...
    ![并发用户 2a](./media/r-server-get-started/concurrent-users-2a.png)

- 然后，使用 SSH 用户凭据登录到 RStudio：
  
    ![并发用户 2b](./media/r-server-get-started/concurrent-users-2b.png)
...
![并发用户 3](./media/r-server-get-started/concurrent-users-3.png)
...
![并发用户 4](./media/r-server-get-started/concurrent-users-4.png)

请注意，RStudio 指示你在使用新用户（例如，此处为 sshuser6）登录到群集： 

![并发用户 5](./media/r-server-get-started/concurrent-users-5.png)
...
![并发用户 6](./media/r-server-get-started/concurrent-users-6.png)
...
2. 在“设置”边栏选项卡中，依次选择“脚本操作”、“提交新项”，即可提交新的脚本操作。
   ...
   ![脚本操作边栏选项卡的图像](./media/r-server-get-started/scriptaction.png)
...
![单机操作](./media/r-server-get-started/admin-util-one-box-.png)
...
![操作诊断](./media/r-server-get-started/admin-util-diagnostics.png)
...
* 选择（要解除授权的）辅助节点，单击“操作”>“选择主机”>“主机”> 单击“打开维护模式”。 例如，下图中，选择了对 wn3 和 wn4 解除授权。  
   ...
   ![解除辅助节点的授权](./media/r-server-get-started/get-started-operationalization.png)  
...
* 查看“URI”部分，并添加辅助节点的 IP 和端口详细信息。
    ...
    ![通过命令行解除辅助节点的授权](./media/r-server-get-started/get-started-op-cmd.png)
