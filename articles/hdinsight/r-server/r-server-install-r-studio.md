...
* 装有 R Server 的 Azure HDInsight 群集。 有关说明，请参阅 [Get started with R Server on HDInsight clusters](r-server-get-started.md)（HDInsight 群集上的 R Server 入门）。
   ...
   * 如果使用装有 PuTTY 的 Windows 客户端
     ...
     3. 提供以下有关“用于控制 SSH 端口转发的选项”窗体的信息：
        ...
        * **目标** - 必须映射到本地客户端计算机的目标。 例如 **localhost:8787**。
            ...
            ![创建 SSH 隧道](./media/r-server-install-r-studio/createsshtunnel.png "创建 SSH 隧道")
...
10. 系统将提示输入 SSH 用户名和密码以连接到群集。 如果在创建群集时使用了 SSH 密钥，则必须输入步骤 5 中所创建的密码。

    ![连接到 R Studio](./media/r-server-install-r-studio/connecttostudio.png "创建 SSH 隧道")
...
12. 在 RStudio 中，会看到下载的测试脚本。 双击该文件将它打开，选择文件的内容，然后单击“运行”。 “控制台”窗格中应会显示输出：

   ![测试安装](./media/r-server-install-r-studio/test-r-script.png "测试安装")

另一种方法是键入 `source(testhdi.r)` 或 `source(testhdi_spark.r)` 来执行脚本。

## <a name="see-also"></a>另请参阅

* [适用于 HDInsight 群集上的 R Server 的计算上下文选项](r-server-compute-contexts.md)
