---
title: 将 Azure 活动目录用户同步到 HDInsight 群集
description: 将经过身份验证的用户从 Azure 活动目录同步到 HDInsight 群集。
author: ashishthaps
ms.author: ashishth
ms.reviewer: jasonh
...
1. 从[Azure 门户](https://portal.azure.com)导航到与 ESP 群集关联的 Azure AD 目录。

2. 从左侧菜单选择“所有用户”，然后选择“新建用户”。********

    ![Azure 门户用户和组所有](./media/hdinsight-sync-aad-users-to-cluster/users-and-groups-new.png)
...
1. 此结果显示状态为 **"完成"，** 创建了一个新用户，并为用户提供了成员资格。 在此示例中，用户分配到“HiveUsers”同步的 LDAP 组，因为用户已在 Azure AD 中添加到该组。

    > [!NOTE]  
    > 前面的方法仅在群集创建期间同步在域设置的**Access 用户组**属性中指定的 Azure AD 组。 有关详细信息，请参阅[创建 HDInsight 群集](domain-joined/apache-domain-joined-configure.md)。

## <a name="verify-the-newly-added-azure-ad-user"></a>验证新添加的 Azure AD 用户

打开 [Apache Ambari Web UI](hdinsight-hadoop-manage-ambari.md)，验证新的 Azure AD 用户是否已添加。 通过浏览 访问**`https://CLUSTERNAME.azurehdinsight.net`** Ambari Web UI。 输入群集管理员用户名和密码。

1. 在 Ambari 仪表板中，选择“管理”菜单下的“管理 Ambari”。********

    ![阿帕奇·安巴里仪表板管理安巴里](./media/hdinsight-sync-aad-users-to-cluster/manage-apache-ambari.png)
...
* [使用 ESP 管理 HDInsight 群集](hdinsight-domain-joined-manage.md)
