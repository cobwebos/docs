---
title: 将 Azure Active Directory 用户同步到 HDInsight 群集
...
1. 在[Azure 门户](https://portal.azure.com)中，导航到与 ESP 群集关联的 Azure AD 目录。
...
1. 此结果显示状态为 "已**完成**"、"已创建一个新用户" 和 "用户已分配成员身份"。 在此示例中，用户分配到“HiveUsers”同步的 LDAP 组，因为用户已在 Azure AD 中添加到该组。

    > [!NOTE]  
    > 以前的方法只同步在群集创建过程中在域设置的 "**访问用户组**" 属性中指定的 Azure AD 组。 有关详细信息，请参阅[创建 HDInsight 群集](domain-joined/apache-domain-joined-configure.md)。

## <a name="verify-the-newly-added-azure-ad-user"></a>验证新添加的 Azure AD 用户

打开 [Apache Ambari Web UI](hdinsight-hadoop-manage-ambari.md)，验证新的 Azure AD 用户是否已添加。 通过浏览到来访问 Ambari Web UI **`https://CLUSTERNAME.azurehdinsight.net`** 。 输入群集管理员用户名和密码。
