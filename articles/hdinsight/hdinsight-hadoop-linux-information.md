...
* **Ambari (web)** - https://&lt;clustername>.azurehdinsight.net
    ...
    > [!IMPORTANT]
    > 某些 Web UI 可使用内部域名通过 Ambari 访问节点。 内部域名不可通过 Internet 公开访问。 在尝试通过 Internet 访问某些功能时，你可能会收到“找不到服务器”的错误。
    >
...
* 2.2.4.9-1：目录名称是 HDInsight 使用的 Hortonworks 数据平台的版本。 你群集上的数字可能与这里列出的有所不同。
* 当前：此目录包含 2.2.4.9-1 目录下的子目录的链接。 由于该目录存在，因此你无需记住版本号。
...
> [!NOTE]
> 这将返回应用到服务器的第一个配置 (`service_config_version=1`)，其中包含此信息。 你可能需要列出所有配置版本，以查找最新的版本。
