...
* **cURL**。 可通过程序包管理系统获取此实用工具，也可以从 [http://curl.haxx.se/](http://curl.haxx.se/) 下载此实用工具。
  ...
  > [!NOTE]
  > 如果使用 PowerShell 运行本文档中的命令，则必须先删除默认创建的 `curl` 别名。 从 PowerShell 提示符使用 `curl` 命令时，此别名使用 Invoke-WebRequest PowerShell cmdlet 而不是 cURL，这会造成本文档中使用的许多命令返回错误。
  >
...
Azure Resource Manager 模板是描述**资源组**及其包含的所有资源（例如 HDInsight）的 JSON 文档基于此模板的方法可在一个模板中定义 HDInsight 所需的所有资源，并通过可将更改应用到组的“部署”，来统一整体管理组的更改。
...
> [!NOTE]
> 这些步骤是[使用 Azure CLI 创建服务主体以访问资源](../azure-resource-manager/resource-group-authenticate-service-principal-cli.md#create-service-principal-with-password)文档的“使用密码创建服务主体”部分中提供的信息的缩减版本。 这些步骤将创建一个新的服务主体，使用该服务主体可以对用于创建 HDInsight 群集等 Azure 资源的 REST API 请求进行身份验证。
...
2. 使用以下命令在 Azure Active Directory 中创建新的应用程序。
   ...
   > [!NOTE]
   > 由于创建此应用程序的目的是通过服务主体进行身份验证，因此，`--home-page` 和 `--identifier-uris` 值无需引用托管在 Internet 上的实际网页；它们只需是唯一的 URI 即可。
