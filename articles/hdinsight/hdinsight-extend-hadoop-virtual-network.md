...
默认名称解析不  允许 HDInsight 解析加入到虚拟网络的网络中的资源名称。 例如，将本地网络加入到虚拟网络是很常见的。 仅通过默认名称解析，HDInsight 无法按名称访问本地网络中的资源。 反过来也是如此，本地网络中的资源也不能按名称访问虚拟网络中的资源。
...
3. 查找 Azure 为你的虚拟网络分配的 DNS 后缀。 该值类似于 `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.cloudapp.net`。 有关查找 DNS 后缀的信息，请参阅[示例：自定义 DNS](#example-dns) 部分。
...
> [!IMPORTANT]  
> 更改的 IP 地址`hdirule1`和`hdirule2`在此示例中以匹配你使用的 Azure 区域。 可以在[使用网络安全组和用户定义的路由的 HDInsight](#hdinsight-ip) 一节找到此信息。
...
本示例演示如何添加规则以允许所需 IP 地址的入站流量。 它不包含用于限制其他源的入站访问的规则。 下面的代码演示如何启用来自 Internet 的 SSH 访问：
...
下面的代码演示如何启用来自 Internet 的 SSH 访问：
...
2. 使用以下文本作为自定义 DNS 服务器上 `/etc/bind/named.config.local` 文件的内容。 在这两个虚拟网络中的自定义 DNS 服务器上进行此更改。
    ...
    将 `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.cloudapp.net` 值替换为另一个  虚拟网络的 DNS 后缀。 此条目将远程网络 DNS 后缀的请求路由到该网络中的自定义 DNS。
...
* 有关配置 Azure 虚拟网络中的 Apache HBase 群集，请参阅[在 Azure 虚拟网络中的 HDInsight 上创建 Apache HBase 群集](hbase/apache-hbase-provision-vnet.md)。
