---
title: HDInsight 中的 Azure 数据存储第 1 代概述
description: HDInsight 中数据存储第 1 代的概述。
...
ms.locfileid: "81873350"
---
# <a name="azure-data-lake-storage-gen1-overview-in-hdinsight"></a>HDInsight 中的 Azure 数据存储第 1 代概述

Azure Data Lake Storage Gen1 是一个企业范围的超大规模存储库，适用于大数据分析工作负载。 使用 Azure 数据湖，可以捕获任何大小、类型和引入速度的数据。 并在一个位置进行操作和探索性分析。
...
数据存储湖存储第 1 代是一个 Apache Hadoop 文件系统，兼容 HDFS 和 Hadoop 环境。  使用 WebHDFS API 的 HDInsight 应用程序或服务可以轻松地与数据存储第 1 代集成。 Data Lake Storage Gen1 还为应用程序公开了 WebHDFS 兼容的 REST 接口。

使用 Hadoop 分析框架可以轻松地分析存储在数据存储库第 1 代的数据。 框架，如映射减少或蜂巢。 可将 Azure HDInsight 群集预配和配置为直接访问 Data Lake Storage Gen1 中存储的数据。

## <a name="unlimited-storage-petabyte-files"></a>无限存储空间，PB 量级的文件

数据存储库 Gen1 提供无限的存储，适用于存储不同类型的数据进行分析。 它不对帐户大小或文件大小施加限制。 或可存储在数据湖中的数据量。 单个文件的大小范围从千字节到 PB，使数据存储 Gen1 成为存储任何类型的数据的绝佳选择。 通过进行多个副本来持久存储数据。 数据在数据湖中存储的时间没有限制。

## <a name="performance-tuning-for-big-data-analytics"></a>针对大数据分析优化了性能

数据存储第一代专为分析系统而设计。 需要大量吞吐量来查询和分析大量数据的系统。 Data Lake 将文件的各个部分散在多个独立的存储服务器中。 分数数据时，此设置可以改善并行读取文件时的读取吞吐量。

## <a name="readiness-for-enterprise-highly-available-and-secure"></a>企业准备：高度可用且安全
...
Data Lake Storage Gen1 可按本机格式（原样）存储任何数据，不需要事先经过转换。 加载数据之前，Data Lake Storage Gen1 不需要定义架构。 独立的分析框架在分析时会解释数据并定义架构。 数据存储第 1 代可以处理结构化数据。 和半结构化和非结构化数据。

Data Lake Storage Gen1 的数据容器本质上是文件夹和文件。 可以使用 SDK、Azure 门户和 Azure PowerShell 来操作存储的数据。 使用这些接口和容器放入存储的数据可以存储任何数据类型。 数据存储库 Gen1 不会根据数据类型对数据进行任何特殊处理。
...
| **功能** | **说明** |
| --- | --- |
| 身份验证 |Data Lake Storage Gen1 与 Azure Active Directory (Azure AD) 集成，可对 Data Lake Storage Gen1 中存储的所有数据进行标识与访问管理。 因为这种集成，Data Lake Storage Gen1 可受益于所有 Azure AD 功能。 这些功能包括：多重身份验证、条件访问和基于角色的访问控制。 此外，应用程序使用情况监视、安全监视和警报等。 Data Lake Storage Gen1 支持在 REST 接口中使用 OAuth 2.0 协议进行身份验证。 参阅[使用 Azure Active Directory 进行 Azure Data Lake Storage Gen1 身份验证](../data-lake-store/data-lakes-store-authentication-using-azure-active-directory.md)。|
...
数据存储源 Gen1 与 Hadoop 环境中的大多数开源组件兼容。 此外，还与其他 Azure 服务完美集成。  请按照以下链接了解有关如何将数据存储 Gen1 与开源组件和其他 Azure 服务一起使用。

* 请参阅[使用 Azure 数据湖存储 Gen1 的开源大数据应用程序](../data-lake-store/data-lake-store-compatible-oss-other-applications.md)。
...
* 有关[大数据要求，请参阅使用 Azure 数据存储第 1 代](../data-lake-store/data-lake-store-data-scenarios.md)。

## <a name="data-lake-storage-gen1-file-system-adl"></a>Data Lake Storage Gen1 文件系统 (adl://)

在 Hadoop 环境中，您可以通过新的文件系统 AzureDataLakeFile 系统 （adl://）访问 Data Lake 存储第 1 代。 可以使用`adl://`的应用程序和服务的性能可以以 WebHDFS 中当前不可用的方式进行优化。 因此，您可以灵活地使用推荐的adl://来获得最佳性能。 或者继续直接使用 WebHDFS API 来维护现有代码。 Azure HDInsight 充分使用 AzureDataLakeFilesystem 来提供 Data Lake Storage Gen1 的最佳性能。

使用以下 URI 访问数据存储第 1 代的数据：
