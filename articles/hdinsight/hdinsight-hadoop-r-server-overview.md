...
在这种情况下，可以在创建群集之后，使用脚本操作来安装其他包。 有关详细信息，请参阅[创建包含 R Server 的 HDInsight 群集](hdinsight-hadoop-r-server-get-started.md)。   
