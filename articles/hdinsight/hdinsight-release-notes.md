---
title: Azure HDInsight 的发行说明
description: Azure HDInsight 最新发行说明。 获取 Hadoop、Spark、R Server、Hive 等的开发技巧和详细信息。
...
本文提供有关**最新**Azure HDInsight 版本更新的信息。 有关更早版本的信息，请参阅[HDInsight 发行说明存档](hdinsight-release-notes-archive.md)。
...
### <a name="tls-12-enforcement"></a>TLS 1.2 强制
TLS(전송 계층 보안) 및 SSL(Secure Sockets Layer)은 컴퓨터 네트워크를 통해 통신 보안을 제공하는 암호화 프로토콜입니다. 详细了解[TLS](https://en.wikipedia.org/wiki/Transport_Layer_Security#SSL_1.0.2C_2.0_and_3.0)。 HDInsight 在公共 HTTPs 终结点上使用 TLS 1.2，但仍支持使用 TLS 1.1 来实现向后兼容性。 
...
HDInsight 中的所有托管磁盘都受 Azure 存储服务加密（SSE）保护。 默认情况下，这些磁盘上的数据是由 Microsoft 管理的密钥加密的。 从此版本开始，你可以创建自己的密钥（BYOK）进行磁盘加密，并使用 Azure Key Vault 来管理它。 BYOK 加密是在群集创建过程中进行的一步配置，无需额外付费。 只需使用 Azure Key Vault 将 HDInsight 注册为托管标识，并在创建群集时添加加密密钥。 有关详细信息，请参阅[客户托管的密钥磁盘加密](https://docs.microsoft.com/azure/hdinsight/disk-encryption)。
...
群集上 _version 的 py 文件已更新为，而不是 4.4. x. x. x. x. x. x. x. x. # # 或 Ambari 需要重新启动。
...
1. 如果 version_info 的值设置为： 
    ...
    如果 version_info 已设置为（4，4，0），则继续执行下一步，因为只需要重新启动 Ambari，不需要进行其他更改。
