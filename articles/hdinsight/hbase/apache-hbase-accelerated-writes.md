...
若要使用加速写入功能创建新的 HBase 群集，请按照在[HDInsight 中设置群集中](../hdinsight-hadoop-provision-linux-clusters.md)的步骤进行操作 **，直到达到步骤3的存储**。 在 "**元存储设置**" 下，单击 "**启用加速写入**" 旁边的复选框。 然后，继续执行群集创建的剩余步骤。

![为 HDInsight Apache HBase 启用加速写入选项](./media/apache-hbase-accelerated-writes/accelerated-writes-cluster-creation.png)
...
按比例缩小群集时执行类似步骤：刷新表并禁用表以停止传入的数据。 不能将群集缩小为小于三个节点。
