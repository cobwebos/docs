...
我们将计算得出需要一定数量新的工作节点来满足当前 CPU 和内存需求，然后通过添加相同数量的新工作节点来发出增加请求。
...
根据每个节点的 AM 容器数以及当前的 CPU 和内存需求，自动缩放将发出删除一定数量节点的请求，并指定可供删除的候选节点。 默认情况下，将在一个周期内删除两个节点。
