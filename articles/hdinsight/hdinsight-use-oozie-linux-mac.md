...
Oozie 希望将作业所需的资源存储在同一个目录中。 此示例使用 **wasbs:///tutorials/useoozie**。 请使用以下命令创建此目录，并创建一个数据目录用于保存此工作流所创建的新 Hive 表：
...
1. 使用以下命令获取默认存储的完整 WASB 地址。 稍后在配置文件中将要用到该地址：
    ...
    保存 **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** 值，后续步骤将会用到它。
...
4. 打开 nano 编辑器后，使用以下内容作为该文件的内容：
   ...
   * 将 **wasbs://mycontainer@mystorageaccount.blob.core.windows.net** 的所有实例替换为前面收到的值。
...
1. 使用以下命令获取 Oozie 服务的 URL：
    ...
    **http://hn0-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:11000/oozie** 部分是用于 Oozie 命令的 URL。
...
**症状**：作业状态将更改为“已暂停”。 作业详细信息中显示 RunHiveScript 状态为 **START_MANUAL**。 选择该操作会显示以下错误消息：
