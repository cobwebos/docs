---
title: "在基于 Linux 的 HDInsight 中使用 Hadoop Oozie 工作流 | Microsoft Docs"
...
使用以下步骤创建 HiveQL 脚本，用于定义将在本文档后部分的 Oozie 工作流中使用的查询。

1. 使用 SSH 连接到群集。 以下命令是 `ssh` 命令使用方法示例。 将 __USERNAME__ 替换为群集的 SSH 用户。 将 __CLUSTERNAME__ 替换为 HDInsight 群集的名称。
    ...
    * [在 Linux、OS X、Unix 或 Windows 中的 HDInsight 上将 SSH 与基于 Linux 的 Hadoop 配合使用](hdinsight-hadoop-linux-use-ssh-unix.md) - 本文档假设用户有权使用 `ssh` 命令。

    * [在装有 PuTTY 的 Windows 中的 HDInsight 上将 SSH 与基于 Linux 的 Hadoop 配合使用](hdinsight-hadoop-linux-use-ssh-windows.md) - 本文档假设用户正在使用 PuTTY SSH 客户端。
...
Oozie 工作流定义以 hPDL（一种 XML 过程定义语言）编写。 使用以下步骤定义工作流：
...
1. 使用以下命令获取默认存储的完整 WASB 地址。 稍后在配置文件中将使用该地址：
...
2. 使用以下命令获取群集头节点的 FQDN。 这用于群集的 JobTracker 地址。 稍后在配置文件中将使用该地址：
    ...
    用于 JobTracker 的端口是 8050，因此要用于 JobTracker 的完整地址是 `hn0-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:8050`。
...
Oozie Web UI 提供基于 Web 的视图来显示群集上 Oozie 作业的状态。 它可以让你查看作业状态、作业定义、配置、作业中操作的图形以及作业的日志。 还可以查看作业中操作的详细信息。
...
2. 创建隧道后，请在 Web 浏览器中打开 Ambari Web UI。 Ambari 的 URI 位于 **https://CLUSTERNAME.azurehdinsight.net** 上。 请将 **CLUSTERNAME**替换为基于 Linux 的 HDInsight 群集的名称。
...
1. 使用以下命令创建名为 **coordinator.xml** 的新文件：
    ...
    请注意 `${...}` 变量，这些变量在运行时会替换为作业定义中的值。 变量包括：
...
4. 使用以下命令修改 **job.xml** 文件：
   ...
   * 添加以下内容，这会将 coordinator.xml 中使用的变量设为指向 workflow.xml 的位置：
   ...
   * 添加以下内容，以定义 coordinator.xml 文件使用的开始时间、结束时间和频率：
       ...
       这些内容将开始时间设为 2017 年 2 月 7 日下午 12:00，结束时间为 2017 年 2 月 9 日，运行此作业的时间间隔为一天。 作业运行的频率以分钟为单位，因此是 24 小时 x 60 分钟 = 1440 分钟。 最后，将时区设置为 UTC。
...
7. 如果访问 Oozie Web UI 并选择“协调器作业”选项卡，会看到类似于下面的信息：
