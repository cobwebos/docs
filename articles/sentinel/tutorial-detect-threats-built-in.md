...
- 基于**合成技术**，Azure 中的高级多阶段攻击检测使用可缩放的机器学习算法，可将多个产品之间的许多低保真警报和事件关联到高保真且可操作的事件中。 默认情况下启用合成。 由于逻辑处于隐藏状态，因此不能将其用作模板来创建多个规则。
...
有关向导中的字段的详细信息，请参阅[教程：创建自定义分析规则以检测可疑的威胁](tutorial-detect-threats-custom.md)。
