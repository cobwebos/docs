...
| 多个 Azure 租户 | Azure Sentinel 支持仅在其自己的 Azure Active Directory （Azure AD）租户边界内从 Microsoft 和 Azure SaaS 资源收集数据。 因此，每个 Azure AD 租户都需要一个单独的工作区。 |  |
...
### <a name="managed-security-service-provider-mssp"></a>托管安全服务提供程序（MSSP）
...
正如上面的要求所隐含的那样，需要通过单个 SOC 集中监视和管理多个 Azure Sentinel 工作区（可能跨 Azure Active Directory （Azure AD）租户。
...
- 使用[工作区（）表达式](../azure-monitor/log-query/workspace-expression.md)引用其他工作区中的表。 
- 将[union 运算符](https://docs.microsoft.com/azure/data-explorer/kusto/query/unionoperator?pivots=azuremonitor)与工作区（）一起使用，以便跨多个工作区中的表应用查询。
...
| 方法  | 说明 | 何时应使用？ |
|---------|-------------|--------------------|
| 编写跨工作区查询 | 工作簿创建者可以在工作簿中编写跨工作区查询（如上所述）。 | 此选项使工作簿创建者能够完全从工作区结构中保护用户。 |
...
通过使用按工作区搜寻功能，您的威胁销售可以创建新的搜寻查询，也可以修改现有的搜寻查询，以涵盖多个工作区，如上文所示。
...
## <a name="whats-not-supported-across-workspaces"></a>工作区不支持哪些功能？

工作区不支持以下功能：

- 不能使用跨工作区查询跨工作区运行计划的警报规则。
