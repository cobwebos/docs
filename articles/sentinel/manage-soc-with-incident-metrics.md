...
作为安全操作中心（SOC）管理器，您需要获得整体效率指标和措施，以衡量您的团队的业绩。 你需要按多个不同的条件（如严重性、MITRE 战术、平均时间到会审、平均解决时间等）来查看一段时间内的事件操作。 Azure Sentinel 现在使此数据可供你使用 Log Analytics 中的新的**SecurityIncident**表和架构以及附带的**安全操作效率**工作簿。 随着时间的推移，你将能够直观显示团队的表现，并使用这种见解提高效率。 你还可以针对事件表编写和使用你自己的 KQL 查询，以创建符合特定审核需求和 Kpi 的自定义工作簿。
...
每次创建或更新事件时，都会向表中添加一个新的日志条目。 这使你可以跟踪对事件所做的更改，并允许更强大的 SOC 指标，但在为该表构造查询时需要注意这一点，因为可能需要删除事件的重复条目（依赖于所运行的确切查询）。 
