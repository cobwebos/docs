...
1. 打开 ICDx 管理控制台，将 Microsoft Azure Sentinel (添加 Log Analytics) 转发器。
2. 在 ICDx 导航栏上，单击 " **配置**"。 
3. 在 **配置** 屏幕的顶部，单击 " **转发器**"。
...
6. 在 " **事件**" 下，执行以下操作： 
    - **源**：选择一个或多个要从中转发事件的存档。 你可以选择活动收集器存档 (包括常见存档) 、孤立收集器存档 (也就是说，你已删除) 、ICDx 接收程序存档或系统存档的收集器的存档。 <br>默认值为常见存档。
      > [!NOTE]
      > ICDx 接收方存档按名称单独列出。 
