...
5. 在 "**高级设置**" 边栏选项卡中，选择 "**数据** > **系统日志**"。 然后，添加要收集的连接器的功能。
    ...
    添加 syslog 设备在其日志标头中包含的工具。 你可以在**syslog 中的** `/etc/rsyslog.d/security-config-omsagent.conf` syslog 设备上、在文件夹中以及从`/etc/syslog-ng/security-config-omsagent.conf` **r-syslog**中查看此配置。
