...
需要在专用 Linux 计算机（VM 或本地）上部署代理，以支持在 Cisco ASA 和 Azure Sentinel 之间进行通信。 下图说明了 Azure 中 Linux VM 的情况下的设置。
...
## <a name="step-2-forward-cisco-asa-logs-to-the-syslog-agent"></a>步骤2：将 Cisco ASA 日志转发到 Syslog 代理
...
若要在 Cisco 事件 Log Analytics 中使用相关架构，请搜索 `CommonSecurityLog`。
