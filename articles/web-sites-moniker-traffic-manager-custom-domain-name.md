...
#Configuring a custom domain name for an Azure Web Site using Traffic Manager (Moniker)
...
To associate your custom domain with an Azure Web Site, you must add a new entry in the DNS table for your custom domain by using tools provided by Moniker. Use the following steps to locate the DNS tools for Moniker.com
...
5. Then create a DNS record by first selecting the **Record Type**. Then fill in the **Hostname** and the **Address**. 
    ...
    * When adding a CNAME record, you must set the **Hostname** field to the sub-domain you wish to use. For example, **www**. You must set the **Address** field to the **.trafficmanager.net** domain name of the Traffic Manager profile you are using with your Azure Web Site. For example, **contoso.trafficmanager.net**.

	    > [WACN.NOTE] You must only use CNAME records when associating your custom domain name with a web site that is load balanced using Traffic Manager.
