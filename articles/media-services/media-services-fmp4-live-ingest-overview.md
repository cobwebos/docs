...
5. [1] 中的第 3.3.6 部分定义了名为 MovieFragmentRandomAccessBox (‘mfra’) 的框，此框可能会在实时引入结束时发送，表示通道 EOS（流式传输结束）。 Azure 媒体服务的引入逻辑使得 EOS（流式传输结束）的使用方式已过时，不应该发送实时引入的 ‘mfra’ 框。 如果已发送，Azure 媒体服务会无提示方式将其忽略。 建议使用[频道重置](https://docs.microsoft.com/rest/api/media/operations/channel#reset_channels)来重置引入点的状态，此外建议使用[节目停止](https://msdn.microsoft.com/library/azure/dn783463.aspx#stop_programs)来结束演播与流。
