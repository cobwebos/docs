...
## <a name="1-conformance-notation"></a>1. 符合性表示法
本文档中的关键字“必须”、“不得”、“需要”、“应”、“不应”、“应该”、“不应”、“建议”、“可以”和“可选”均根据 RFC 2119 中所述予以解释。

## <a name="2-service-diagram"></a>2. 服务图
...
## <a name="3-bitstream-format--iso-14496-12-fragmented-mp4"></a>3. 比特流格式 – ISO 14496-12 碎片 MP4
...
1. 第 3.3.2 节 [1] 定义了一个可选框，称为 **"流清单盒**"，用于实时摄用。 由于 Azure 负载均衡器的路由逻辑，此框已被弃用。 引入到媒体服务时不应出现此框。 如果存在此框，媒体服务会以无提示方式将其忽略。
...
### <a name="summary"></a>总结
此列表并不完整，只列出了此示例中部分可用的引入选项。 事实上，实时引入支持将轨迹以任何组合方式分组到流中。 客户和编码器供应商可以根据工程复杂性、编码器容量以及冗余与故障转移注意事项，来选择自己的实现。 但是，在大多数情况下，整个实时演播只有一个音频轨迹。 因此，确保包含音轨的引入流的健康性非常重要。这种考虑通常会导致将音轨放在自己的流中（如选项 2 中所示），或将其与最低比特率视频轨道捆绑在一起（如选项 3 所示）。 此外，要获得更好的冗余与容错效果，强烈建议针对嵌入到媒体服务的实时引入，将同一个音频轨迹通过两个不同的流（选项 2 中采用冗余音频轨迹）发送，或者将音频轨迹与至少两个比特率最低的视频轨迹捆绑在一起（选项 3 将音频与至少两个视频流捆绑在一起）。
...
1. 发生 TCP 错误后：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 必须关闭当前连接，并且必须为新的 HTTP POST 请求创建新的连接。

    b.保留“数据库类型”设置，即设置为“共享”。 新的 HTTP POST URL 必须与初始 POST URL 相同。
...
## <a name="11-special-types-of-ingestion-formats"></a>11. 特殊类型的摄入格式
...
1. 在 [1] 第 6 节中定义的 **"实时服务器清单框**"中，使用*父轨道名称*参数指定父轨道的名称。有关详细信息，请参阅 [1] 中第 4.2.1.2.1.2 节。
1. 在 **"实时服务器清单"框中**，**必须将清单输出**设置为**true**。
1. 根据信号事件的稀疏性质，建议如下：
   
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 直播活动开始时，编码器会将初始标头框发送给服务，使服务可以在客户端清单中注册稀疏轨道。
   
    b.保留“数据库类型”设置，即设置为“共享”。 未发送数据时，编码器应该终止 HTTP POST 请求。 不发送数据的长时间运行 HTTP POST 可以防止媒体服务在服务更新或服务器重新启动事件中很快与编码器断开连接。 在这些情况下，在套接字上的接收操作中，会暂时阻止媒体服务器。
   
    c. 在此期间如果没有可用的信号数据，编码器应该关闭 HTTP POST 请求。 当 POST 请求处于活动状态时，编码器应发送数据。

    d.单击“下一步”。 发送稀疏片段时，编码器可以设置显式 content-Length 标头（如果可用）。

    e.在“新建 MySQL 数据库”边栏选项卡中，接受法律条款，然后单击“确定”。 通过新连接发送稀疏片段时，编码器应从标头框开始发送，接着发送新片段。 这适用于在中途发生故障转移的情况，并会与先前从未看到稀疏轨迹的新服务器建立新的稀疏连接。
...
1. 使用独立的流发送两个最低视频比特率。 每个流还应包含每个唯一音轨的副本。例如，当支持多种语言时，这些流应包含每种语言的音轨。
