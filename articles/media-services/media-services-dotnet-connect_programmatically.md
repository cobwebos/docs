<properties 
	pageTitle="使用 .NET 连接到 Media Services 帐户" 
	description="本主题演示如何使用 .NET 连接到 Media Services。" 
...
若要以编程方式连接到 Media Services，你必须在以前设置了一个 Azure 帐户，并在该帐户上配置了 Media Services，然后设置一个 Visual Studio 项目，以便通过 Media Services SDK for .NET 进行开发。有关详细信息，请参阅“设置以使用 Media Services SDK for .NET 进行开发”。

在 Media Services 帐户设置过程结束时，你获得以下必需的连接值。请使用这些值以编程方式连接到 Media Services。

- 你的 Media Services 帐户名。

- 你的 Media Services 帐户密钥。
...
自 Media Services SDK 版本 3.0.0.0 起，可以重复使用 ACS 令牌。以 **MediaServicesCredentials** 作为参数的 **CloudMediaContext** 构造函数可实现在多个上下文之间共享 ACS 令牌。MediaServicesCredentials 类中封装有 Media Services 凭据。如果 ACS 令牌可用且其过期时间已知，则可以使用该令牌创建一个新的 MediaServicesCredentials 实例并将其传递给 CloudMediaContext 的构造函数。请注意，Media Services SDK 将在每次令牌过期时自动刷新令牌。有两种方法可重复使用 ACS 令牌，如以下示例中所示。
...
以下 App.config 文件包含了必需的连接值。<appSettings> 元素中的值是你从 Media Services 帐户设置过程中获取的必需值。
