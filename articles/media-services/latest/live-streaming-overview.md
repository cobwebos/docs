...
对于希望将内容传递给大型 internet 受众的客户，我们建议你在 [流式处理终结点](streaming-endpoint-concept.md)上启用 CDN。
...
借助媒体服务，可以利用[动态打包](dynamic-packaging-overview.md)，以便预览和广播正在发送到服务的贡献源中采用 [MPEG DASH、HLS 和平滑流式处理格式](https://en.wikipedia.org/wiki/Adaptive_bitrate_streaming)的实时传送流。 观看者可以使用任何与 HLS、DASH 或平滑流式处理兼容的播放器播放实时流。 你可以使用 web 或移动应用程序中的 [Azure Media Player](https://amp.azure.net/libs/amp/latest/docs/index.html) ，以这些协议中的任何一种来传送流。

## <a name="dynamic-encryption"></a>动态加密

动态加密可让你通过128或以下三种主要数字版权管理 (DRM) 系统中的任何一项（Microsoft PlayReady、Google Widevine 和 Apple FairPlay）动态加密你的实时或点播内容。 媒体服务还提供了用于向已授权客户端传送 AES 密钥和 DRM（PlayReady、Widevine 和 FairPlay）许可证的服务。 有关详细信息，请参阅[动态加密](content-protection-overview.md)。
...
Live 脚本是一项功能，可将其用于传递或实时编码的实时事件。 有关详细信息，请参阅 [实时](live-transcription.md)脚本。 启用此功能后，服务将使用认知服务的[语音转文本](../../cognitive-services/speech-service/speech-to-text.md)功能将传入音频中的口语转录为文本。 然后，可以在 MPEG-DASH 和 HLS 协议中将此文本连同视频和音频一起传输。
