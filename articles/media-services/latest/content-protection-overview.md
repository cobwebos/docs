...
在媒体服务 v3 中，内容密钥与流定位符相关联（参阅[此示例](protect-with-aes128.md)）。 如果使用媒体服务密钥传送服务，可让 Azure 媒体服务自动生成内容密钥。 如果你使用的是自己的密钥传送服务，或者如果你需要处理需要在两个数据中心具有相同内容密钥的高可用性方案，则应自行生成内容密钥。
...
若要成功完成内容保护系统，需要全面了解工作范围。 以下部分概述了需要实现的三个部件。
...
安全令牌服务 (STS) 颁发 JWT 作为用于访问后端资源的访问令牌。 可以使用 Azure 媒体服务许可证/密钥传送服务作为后端资源。 STS 必须定义以下内容：
...
可以通过 Azure 媒体服务传送使用 AES 明文密钥或 DRM 加密（利用 PlayReady、Widevine 或 FairPlay）动态加密的内容。 目前，可以加密 HLS、MPEG 破折号和平滑流式处理格式。 每个协议支持以下加密方法。
