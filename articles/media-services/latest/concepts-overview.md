...
本主题简要概述 Azure 媒体服务的术语和概念。 本文还提供了指向文章的链接，其中深入介绍了媒体服务 v3 的概念和功能。

在开始开发之前，应查看这些主题中所述的基本概念。

> [!NOTE]
> 目前，可以使用[Azure 门户](https://portal.azure.com/)：管理媒体服务 V3[实时事件](live-events-outputs-concept.md)，查看（不管理） v3[资产](assets-concept.md)，并[获取有关访问 api 的信息](access-api-portal.md)。
...
|流式处理终结点/打包/源|**流式处理终结点**表示动态（实时）打包和源服务，可以直接将您的实时内容和点播内容传递给客户端播放器应用程序。 它使用通用流式处理媒体协议（HLS 或破折号）之一。 此外，**流式处理终结点**提供对业界领先数字版权管理系统（drm）的动态（实时）加密。<br/><br/>在媒体流行业，此服务通常称为**打包器**或**来源**。  此功能行业中的其他常见术语包括 JITP （实时包装程序）或 JITE （实时加密）。
...
|内容保护|借助媒体服务，你可以通过高级加密标准（AES-128）或/以及以下三个主要 DRM 系统中的任何一种，提供动态加密的实时和点播内容： Microsoft PlayReady、Google Widevine 和 Apple FairPlay。 媒体服务还提供了用于向已授权客户端传送 AES 密钥和 DRM（PlayReady、Widevine 和 FairPlay）许可证的服务。 <br/><br/>若要针对流指定加密选项，请创建**内容密钥策略**并将其与**流定位符**相关联。 使用**内容密钥策略**，可以配置如何将内容密钥传送到终端客户端。<br/><br/> 在需要相同选项的情况下尝试重复使用策略。| [内容密钥策略](content-key-policy-concept.md)<br/><br/>[内容保护](content-protection-overview.md)|
...
|通过事件网格进行监视|若要查看作业的进度，请使用**事件网格**。 媒体服务还会发出实时事件类型。 使用事件网格，应用可以侦听和响应来自几乎所有 Azure 服务和自定义源的事件。 |[处理事件网格事件](reacting-to-media-services-events.md)<br/><br/>[架构](media-services-event-schemas.md)|
|监视 Azure Monitor|监视指标和诊断日志，以帮助你了解应用如何通过 Azure Monitor 执行。|[指标和诊断日志](media-services-metrics-diagnostic-logs.md)<br/><br/>[诊断日志架构](media-services-diagnostic-logs-schema.md)|
|播放器客户端|可以在各种浏览器和设备上使用 Azure Media Player 播放媒体服务流式传输的媒体内容。 Azure Media Player 使用 HTML5、媒体源扩展（MSE）和加密媒体扩展（EME）等行业标准提供丰富的自适应流式处理体验。 |[Azure Media Player 概述](use-azure-media-player.md)|
