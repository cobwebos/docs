...
ms.locfileid: "37133255"
---
# <a name="use-aes-128-dynamic-encryption-and-the-key-delivery-service"></a>使用 AES-128 动态加密和密钥传递服务

借助媒体服务，可以传送使用 AES 通过 128 位加密密钥加密的 HTTP Live Streaming (HLS)、MPEG-DASH 和平滑流。 媒体服务还提供密钥传送服务，将加密密钥传送给已授权的用户。 如果希望媒体服务加密某个资产，需要将加密密钥与 StreamingLocator 相关联，并配置内容密钥策略。 当播放器请求流时，媒体服务将使用指定的密钥通过 AES 加密来动态加密内容。 为解密流，播放器从密钥传送服务请求密钥。 为了确定用户是否被授权获取密钥，服务将评估你为密钥指定的授权策略。
...
使用以下命令，将包含本主题中所述完整 .NET 示例的 GitHub 存储库克隆到计算机：
...
输出[资产](https://docs.microsoft.com/rest/api/media/assets)会存储作业编码的结果。 完成编码后，使用 AES (ClearKey) 加密发布输出资产。  
...
## <a name="create-a-contentkey-policy"></a>创建 ContentKey 策略

内容密钥提供对资产的安全访问。 需要创建一个内容密钥策略，用于配置如何将内容密钥传送到终端客户端。 该内容密钥与 StreamingLocator 相关联。 媒体服务还提供密钥传送服务，将加密密钥传送给已授权的用户。 

当播放器请求流时，媒体服务将使用指定的密钥通过 AES 加密来动态加密内容（本例中使用 AES 加密）。为解密流，播放器从密钥传送服务请求密钥。 为了确定用户是否被授权获取密钥，服务将评估你为密钥指定的授权策略。
