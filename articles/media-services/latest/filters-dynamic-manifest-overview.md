...
### <a name="examples-urls-with-filters-in-query-string"></a>示例：查询字符串中具有筛选器的 URL

您可以将筛选器应用于 ABR 流协议：HLS、MPEG-DASH 和平滑流式处理。 下表显示了一些包含筛选器的 URL 示例：
...
以下示例使用编码器将夹层资产编码成七个 ISO MP4 视频再现内容（从 180p 到 1080p）。 编码的资产可以[动态打包](dynamic-packaging-overview.md)到以下任何流协议：HLS、MPEG DASH 和平滑。

下图的顶部显示了不包含筛选器的资产的 HLS 清单。 （它包含所有七个格式副本。 在左下角，该图显示了一个 HLS 清单，其中应用了名为"ott"的筛选器。 “ott”筛选器指定要删除所有低于 1 Mbps 的比特率，因此最差的两个质量级别已从响应中剥除。 右下角显示了名为“mobile”的筛选器已应用到 HLS 清单。 “mobile”筛选器指定要删除分辨率大于 720p 的再现内容，因此已剥除两个 1080p 再现内容。
...
开发此类播放器 SDK 相当有挑战性，因为各个设备特定的播放器框架之间需要不同的实现。 此外，在某些平台上，Player API 是有限的，不包括用户无法选择或更改默认音轨的音频选择功能。使用资产筛选器，您可以通过创建仅包含所需音频语言的筛选器来控制行为。

![使用动态清单的语言轨迹筛选][language_filter]

## <a name="trimming-the-start-of-an-asset"></a>修剪资产开头

在大多数实时流事件中，操作员必须在发生实际事件之前进行某些测试。 例如，在活动开始之前，它们可能包含这样的石板："程序将暂时启动。

如果节目正在存档，则测试和静态数据也会一并存档并包含在演播中。 但是，此信息不应向客户端显示。 通过动态清单，可以创建开始时间筛选器，并从清单中删除不需要的数据。
...
您可以单独发布这些虚拟程序，而无需对实时存档进行后期处理，也不会创建单独的资产（这不能从 CDN 中现有的缓存片段中获益）。 此类虚拟节目的示例包括足球或篮球比赛中的上下半场（第几节）、棒球比赛中的局，或者任何体育项目的单项赛事。
...
目前，Azure 媒体服务提供持续时间可设为 1 分钟到 25 小时的循环存档。 清单筛选可以基于存档创建循环 DVR 窗口存档，而不会删除媒体。 在许多情况下，广播者想要提供受限制的 DVR 窗口，此窗口可随着实时边缘移动，并同时保留更大的存档窗口。 广播者可能想要使用超出 DVR 窗口的数据来突出显示剪辑，或者想要为不同的设备提供不同的 DVR 窗口。 例如，大多数移动设备不处理大的 DVR 窗口（可以让移动设备有 2 分钟的 DVR 窗口，桌面客户端有 1 小时的 DVR 窗口）。
...
也可以在一个 URL 中组合多个筛选器。 以下方案演示为何需要组合多个筛选器：
...
若要组合筛选器，请在清单/播放列表 URL 中设置分号分隔格式的筛选器名称。 假设您有一个名为*MyMobileDevice*的筛选器来筛选质量，并且您有另一个名为*MyStartTime*来设置特定的开始时间。 最多可以组合 3 个筛选器。
...
- 如果更新筛选器，则流式处理终结点需要最多 2 分钟来刷新规则。 如果使用筛选器为内容提供服务（并将内容缓存在代理和 CDN 缓存中），则更新这些筛选器可能会导致播放器失败。 我们建议在更新筛选器之后清除缓存。 如果无法做到这一点，请考虑使用其他筛选器。
- 客户需要手动下载清单，并分析确切的开始时间戳和时间刻度。

    - 要确定资产中轨道的属性，[请获取并检查清单文件](#get-and-examine-manifest-files)。
