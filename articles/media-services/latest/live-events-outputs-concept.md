...
description: 本主题概述了 Azure 媒体服务 v3 中的实时事件和实时输出。
...
[实时事件](https://docs.microsoft.com/rest/api/media/liveevents)可以设置为*传递*（本地实时编码器发送多个比特率流）或*实时编码*（本地实时编码器发送单个比特率流）。 这些类型是在创建期间使用 [LiveEventEncodingType](https://docs.microsoft.com/rest/api/media/liveevents/create#liveeventencodingtype) 设置的：

* **LiveEvent编码类型.无**：本地实时编码器发送多个比特率流。 引入的流通过直播活动传递，而不会经过任何进一步的处理。 也称为直通模式。
* **LiveEvent编码类型.标准**：本地实时编码器向实时事件发送单个比特率流，媒体服务创建多个比特率流。 如果贡献源的分辨率为 720p 或更高，则 **Default720p** 预设将编码一组 6 分辨率/比特率对。
* **LiveEventEncodeType.Premium1080p**：本地实时编码器向实时事件发送单个比特率流，媒体服务创建多个比特率流。 Default1080p 预设指定分辨率/比特率对的输出集。
...
> [!NOTE]
> 在长时间执行多个事件且已投资于本地编码器时，使用直通方法是执行实时流式处理的最经济的方法。 请参阅[定价](https://azure.microsoft.com/pricing/details/media-services/)详细信息。
>
...
* 对引入和预览的 IP 限制。 可定义允许向该直播活动引入视频的 IP 地址。 允许的 IP 地址可以指定为单个 IP 地址（例如“10.0.0.1”）、使用一个 IP 地址和 CIDR 子网掩码的 IP 范围（例如“10.0.0.1/22”）或使用一个 IP 地址和点分十进制子网掩码的 IP 范围（例如“10.0.0.1(255.255.252.0)”）。<br/>如果未指定 IP 地址并且没有规则定义，则不会允许任何 IP 地址。 若要允许任何 IP 地址，请创建一个规则并设置 0.0.0.0/0。<br/>IP 地址必须采用以下格式之一：IpV4 地址，包含四个数字或 CIDR 地址范围。
...
* 创建事件时，您可以选择打开实时转录。 <br/> 默认情况下，实时转录被禁用。 在"实时事件"或其关联的实时输出运行时，无法更改此属性。 
...
* 非虚 URL
    ...
    如果客户端应用在创建实时事件之前不需要预先生成引入 URL，请让媒体服务自动生成实时事件的访问令牌。

* 虚 URL

    对于使用硬件广播编码器且不希望在启动实时事件时重新配置其编码器的大型媒体广播者来说，虚模式是首选。 这些广播公司想要一个预测性引入 URL，不会随时间而变化。
    ...
    |.NET|[LiveEvent.VanityUrl](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.media.models.liveevent.vanityurl?view=azure-dotnet#Microsoft_Azure_Management_Media_Models_LiveEvent_VanityUrl)|[实时事件输入.访问令牌](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.media.models.liveeventinput.accesstoken?view=azure-dotnet#Microsoft_Azure_Management_Media_Models_LiveEventInput_AccessToken)|
    
### <a name="live-ingest-url-naming-rules"></a>实时引入 URL 命名规则

* 下面的随机** 字符串是一个 128 位的十六进制数字（由 32 个 0-9 a-f 字符组成）。
* *访问令牌*：使用虚荣模式时设置的有效 GUID 字符串。 例如，`"1fce2e4b-fb15-4718-8adc-68c6eb4c26a7"` 。
* *流名称*：指示特定连接的流名称。 流名称值通常由使用的实时编码器添加。 您可以将实时编码器配置为使用任何名称来描述连接，例如："video1_audio1"、"video2_audio1"、"流"。
