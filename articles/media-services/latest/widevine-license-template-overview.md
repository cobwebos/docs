...
如果存在预先存在的策略，则不需要在内容密钥规范中指定任何值。与此内容关联的预先存在的策略用于确定输出保护，例如高带宽数字内容保护 (HDCP) 和 Copy General Management System (CGMS) 。 如果预先存在的策略未注册到 Widevine 许可证服务器，则内容提供程序可以在许可证请求中注入值。   
...
| content_key_specs <br/>key |Base64-<br/>编码的字符串 |用于此跟踪的内容密钥。如果已指定，则需要 track_type 或 key_id。 内容提供者可以使用此选项注入此跟踪的内容密钥，而不是让 Widevine 许可证服务器生成或查找密钥。 |
...
媒体服务提供了用于配置 Widevine 许可证的类。 若要构造许可证，请将 JSON 传递给 [WidevineTemplate](/dotnet/api/microsoft.azure.management.media.models.contentkeypolicywidevineconfiguration.widevinetemplate?view=azure-dotnet#Microsoft_Azure_Management_Media_Models_ContentKeyPolicyWidevineConfiguration_WidevineTemplate)。
...
使用上一部分中定义的类来创建用于配置 [WidevineTemplate](/dotnet/api/microsoft.azure.management.media.models.contentkeypolicywidevineconfiguration.widevinetemplate?view=azure-dotnet#Microsoft_Azure_Management_Media_Models_ContentKeyPolicyWidevineConfiguration_WidevineTemplate) 的 JSON：
