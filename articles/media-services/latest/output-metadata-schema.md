...
ms.locfileid: "84418517"
---
# <a name="output-metadata"></a>输出元数据

编码作业与要在其上执行某些编码任务的输入资产（或资产）相关联。 例如，将 MP4 文件编码为 H.264 MP4 自适应比特率集；创建缩略图；创建叠加。 完成任务后，会生成一个输出资产。  输出资产包含视频、音频、缩略图和其他文件。 输出资产还包含提供输出资产相关元数据的文件。 元数据 JSON 文件的名称采用以下格式： `<source_file_name>_manifest.json` （例如 `BigBuckBunny_manifest.json` ）。  
...
| **名称**<br />必需 |媒体资产文件名。 <br /><br />示例：`"Name": "Ignite-short_1280x720_AACAudio_3551.mp4"`|
| **大小**<br />必需 |资产文件的大小（以字节为单位）。 <br /><br />示例：`"Size": 32414631`|
| **Duration**<br />必需 |内容播放持续时间。 有关详细信息，请参阅[ISO8601](https://www.iso.org/iso-8601-date-and-time-format.html)格式。 <br /><br />示例：`"Duration": "PT1M10.315S"`|
...
| “属性” | 说明 |
| --- | --- |
| **Id**<br /> 必需 |此视频轨道的从零开始的索引。**注意：** 此**Id**不一定是在 TrackID 文件中使用的。 <br /><br />示例：`"Id": 1`|
| **FourCC**<br />必需 | Ffmpeg 报告的视频编解码器 FourCC 代码。  <br /><br />示例：`"FourCC": "avc1"`|
| **配置文件** |H264 配置文件（仅适用于 H264 编解码器）。  <br /><br />示例：`"Profile": "High"` |
...
| **Framerate**<br />必需 |采用 .3f 格式测量的视频帧速率。  <br /><br />示例：`"Framerate": 29.970`|
| **率**<br />必需 |从 AssetFile 计算的平均视频比特率（比特/秒）。 只计算基本流有效负载，并且不包括打包开销。  <br /><br />示例：`"Bitrate": 3551567`|
| **TargetBitrate**<br />必需 |通过编码预设请求的此视频轨道的目标平均比特率（以每秒位数为单位）。 <br /><br />示例：`"TargetBitrate": 3520000` |
...
| “属性”  | 说明 |
| --- | --- |
| **Id**<br />必需  |此音轨的从零开始的索引。**注意：** 这不一定是在 TrackID 文件中使用的。  <br /><br />示例：`"Id": 2`|
| **编解码器**  |音频轨道编解码器字符串。  <br /><br />示例：`"Codec": "aac"`|
| **语言**|示例：`"Language": "eng"`|
| 通道<br />必需|音频通道数。  <br /><br />示例：`"Channels": 2`|
| **SamplingRate**<br />必需 |音频采样速率（以采样数/秒或 Hz 为单位）。  <br /><br />示例：`"SamplingRate": 48000`|
| **率**<br />必需 |由 AssetFile 计算的平均音频比特率（以比特/秒为单位）。 只计算基本流有效负载，并且不包括打包开销。  <br /><br />示例：`"Bitrate": 128041`|
