...
在 Azure 媒体服务中，[资产](https://docs.microsoft.com/rest/api/media/assets)是一个核心概念。 它是您输入媒体（例如，通过上传或实时引入）、输出媒体（来自作业输出）和发布媒体（用于流式传输）的位置。 

资产映射到[Azure 存储帐户](storage-account-concept.md)中的 Blob 容器，资产中的文件存储为该容器中的块 Blob。 资产包含有关存储在 Azure 存储中的数字文件的信息（包括视频、音频、图像、缩略图集合、文本轨道和隐藏字幕文件）。

当帐户使用常规用途 v2 (GPv2) 存储时，媒体服务支持 Blob 层。 使用 GPv2 可将文件移到[冷存储或存档存储](https://docs.microsoft.com/azure/storage/blobs/storage-blob-storage-tiers)。 **存档**存储适合存档不再需要的源文件（例如，编码后的源文件）。

建议仅将**存档**存储用于已编码的，并且其编码作业输出已放入输出 Blob 容器中的极大型源文件。 要与资产关联并用于流式传输或分析内容的输出容器中的 blob 必须存在于**热**存储层或 **"酷"** 存储层中。
