...
description: 本文介绍使用 Azure 媒体服务对视频和音频进行编码。
...
> [!IMPORTANT]
> 媒体服务不为已取消或出错的作业收费。 例如，进度达到 50% 且已取消的作业不会以工作分钟数的 50% 计费。 您只收取已完成作业的费用。
...
要使用媒体服务 v3 进行编码，您需要创建[一个转换](https://docs.microsoft.com/rest/api/media/transforms)和一个[作业](https://docs.microsoft.com/rest/api/media/jobs)。 转换用于定义编码设置和输出的配方，作业则是该配方的一个实例。 有关详细信息，请参阅[转换和作业](transforms-jobs-concept.md)。

使用媒体服务进行编码时，可以使用预设来指示编码器应如何处理输入媒体文件。 在媒体服务 v3 中，您可以使用标准编码器对文件进行编码。 例如，可以在编码内容中指定所需的视频分辨率和/或音频信道数量。

可以使用行业最佳做法推荐的内置预设之一快速入门，也可以选择针对特定方案或设备要求生成自定义预设。 有关详细信息，请参阅[使用自定义转换编码](customize-encoder-presets-how-to.md)。

从 2019 年 1 月开始，当使用标准编码器进行编码以生成 MP4 文件时，将生成新的 .mpi 文件并将其添加到输出资产中。 此 MPI 文件旨在提高[动态打包](dynamic-packaging-overview.md)和流式处理方案的性能。
...
- **编码器命名预设.自适应流**（推荐）：有关详细信息，请参阅[自动生成比特率梯](autogen-bitrate-ladder.md)。
...
在媒体服务 v3 中，预设是 API 本身中的强类型化实体。 您可以在[Open API 规范（或 Swagger）](https://github.com/Azure/azure-rest-api-specs/tree/master/specification/mediaservices/resource-manager/Microsoft.Media/stable/2018-07-01)中找到这些对象的"架构"定义。 您还可以在[REST API](https://docs.microsoft.com/rest/api/media/transforms/createorupdate#standardencoderpreset) [、.NET SDK（](https://docs.microsoft.com/dotnet/api/microsoft.azure.management.media.models.standardencoderpreset?view=azure-dotnet)或其他媒体服务 v3 SDK 参考文档中）中查看预设定义（如**标准编码预设**）。
...
* [使用媒体服务上载、编码和流式传输](stream-files-tutorial-with-api.md)。
* [使用内置预设从 HTTPS URL 进行编码](job-input-from-http-how-to.md)。
* [使用内置预设对本地文件进行编码](job-input-from-local-file-how-to.md)。
* [构建自定义预设以定位您的特定方案或设备要求](customize-encoder-presets-how-to.md)。
