...
## <a name="example--one-image-at-a-specific-timestamp"></a>示例 - 在特定时间戳处生成一幅映像

使用以下 JSON 和 XML 预设可以在输入视频的 30 秒标记处生成单个 JPEG 图像。 此预设预期输入的持续时间超过 30 秒（否则作业会失败）。
...
## <a id="code_sample"></a>示例 - 编码视频并生成缩略图
