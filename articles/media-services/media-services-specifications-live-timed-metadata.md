...
ms.locfileid: "76905245"
---
# <a name="signaling-timed-metadata-in-live-streaming"></a>实时传送视频流中的超时元数据 

上次更新时间：2019-08-22

### <a name="conformance-notation"></a>一致性表示法

本文档中的关键字 "必须"、"不得"、"需要"、"应"、"不应"、"应该"、"不能"、"建议"、"可以" 和 "可选" 将被解释为 RFC 2119 中所述。
...
| 演示时间   | 向查看器显示事件的时间。 时间表示媒体时间线上查看器将看到事件的时刻。 例如，SCTE-35 splice_info （）命令消息的显示时间为 splice_time （）。 |
| 到达时间        | 事件消息到达的时间。 该时间通常与事件的显示时间不同，因为事件消息将在事件的显示时间之前发送。                                                    |
| 稀疏轨迹        | 不连续的媒体轨迹，并且时间与父或控制轨迹同步。                                                                                                                                                  |
| 格式              | Azure 媒体流服务                                                                                                                                                                                                             |
| 信道接收器        | Azure 媒体实时流式处理服务                                                                                                                                                                                                        |
| HLS                 | Apple HTTP Live Streaming 协议                                                                                                                                                                                                            |
| 此时                | 通过 HTTP 的动态自适应流式处理                                                                                                                                                                                                          |
| 光              | 平滑流式处理协议                                                                                                                                                                                                                     |
| MPEG2-TS            | MPEG 2 传输流                                                                                                                                                                                                                      |
| RTMP                | 实时多媒体协议                                                                                                                                                                                                                 |
| uimsbf              | 无符号整数，最高有效位。                                                                                                                                                                                                 |
...
| [SSTR]  | [Azure 媒体服务分段的运行中引入规范](https://docs.microsoft.com/azure/media-services/media-services-fmp4-live-ingest-overview)                                                      |
| [RFC8216]         | 迅驰. Pantos，Ed.;可能。 HTTP Live Streaming。 2017年8月。 条. [https://tools.ietf.org/html/rfc8216](https://tools.ietf.org/html/rfc8216)                                                            |
...
广告信号（提示消息）以 [AMF0] 提示消息的形式发送，该消息嵌入在 [RTMP] 流中。 在实际事件或 [SCTE35] ad 接合信号需要发生之前，可能会发送提示消息。 若要支持此方案，请在提示消息中发送事件的实际显示时间戳。 有关详细信息，请参阅 [AMF0]。
...
| 字段名称 | 字段类型 | 必填？ | 说明                                                                                                                                                                                                                                                                        |
| ---------- | ---------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| type       | String     | 必选  | 事件消息。  应为 "SpliceOut" 以指定简单模式拼接。                                                                                                                                                                                                         |
| 识别         | String     | 必选  | 描述接合或线段的唯一标识符。 标识消息的此实例                                                                                                                                                                                       |
| 持续时间   | 多种     | 必选  | 接合的持续时间。 单位为秒的小数部分。                                                                                                                                                                                                                           |
| 已用    | 多种     | 可选  | 在重复该信号以便支持在中进行调整时，此字段应为自接合开始后已经过去的呈现时间量。 单位为秒的小数部分。 使用简单模式时，此值不应超过拼接的原始持续时间。 |
| time       | 多种     | 必选  | 应为表示法中接合时间的时间。 单位为秒的小数部分。                                                                                                                                                                                                |
...
| 字段名称 | 字段类型 | 必填？ | 说明                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------- | ---------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 暗示        | String     | 必选  | 事件消息。  对于 [SCTE-35] 消息，这必须是 base64 编码的 [RFC4648] 二进制 splice_info_section （），以便将消息发送到 HLS、平滑和短信客户端。                                                                                                                                                                                                                               |
...
| 识别         | String     | 必选  | 描述接合或线段的唯一标识符。 标识消息的此实例。  具有等效语义的消息应具有相同的值。                                                                                                                                                                                                                                                       |
| 持续时间   | 多种     | 必选  | 事件或广告接合段的持续时间（如果已知）。 如果未知，则该值**应**为0。                                                                                                                                                                                                                                                                                                                    |
| 已用    | 多种     | 可选  | 在重复执行 [SCTE-35] ad 信号以便进行调整时，此字段应为自接合开始后已经过去的呈现时间量。 单位为秒的小数部分。 在 [SCTE-35] 模式下，此值可能会超出接合或分段的原始指定持续时间。                                                                                                                   |
| time       | 多种     | 必选  | 事件或 ad 接合的显示时间。  显示时间和持续时间**应**与类型为1或2的流访问点（SAP）一致，如 [ISO-14496-12] 附录 I 中所定义。对于 HLS 出口，时间和持续时间**应**与段边界对齐。 同一事件流中不同事件消息的显示时间和持续时间不得重叠。 单位为秒的小数部分。 |
...
可以通过发送多条具有相同呈现时间和 ID 的消息来取消或更新消息。 呈现时间和 ID 唯一地标识事件，并且在满足预掷限制的特定呈现时间收到的最后一条消息是所处理的消息。 更新的事件将替换以前接收的任何消息。 前滚约束为4秒。 将对在显示时间之前至少四秒钟收到的消息进行操作。

## <a name="22-fragmented-mp4-ingest-smooth-streaming"></a>2.2 碎片的引入（平滑流式处理）

有关实时流引入的要求，请参阅 [SSTR-摄取]。 以下各节提供有关引入计时表示元数据的详细信息。  计时演示元数据将引入为稀疏轨迹，这是在 "实时服务器清单" 框（请参阅 SSTR）和 "电影" 框（"moov"）中定义的。  
...
### <a name="221-live-server-manifest-box"></a>2.2.1 实时服务器清单框

**必须**在 "实时服务器清单" 框中使用 **\<textstream\>** 项声明稀疏轨迹，并且**必须**设置以下属性：

| **属性名称** | **字段类型** | **必填？** | **描述**                                                                                                                                                                                                              |
| ------------------ | -------------- | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| systemBitrate      | 多种         | 必选      | **必须**为 "0"，表示具有未知可变比特率的轨迹。                                                                                                                                                          |
| parentTrackName    | String         | 必选      | **必须**为父轨迹的名称，稀疏轨迹时间代码的时间刻度为该名称。 父轨迹不能为稀疏轨迹。                                                                             |
| manifestOutput     | Boolean        | 必选      | **必须**为 "true"，以指示将在平滑客户端清单中嵌入稀疏轨迹。                                                                                                                        |
| 类型            | String         | 必选      | **必须**是四个字符代码 "DATA"。                                                                                                                                                                                  |
| 机制             | String         | 必选      | **必须**为标识消息方案的 URN 或 URL。 对于 [SCTE-35] 消息，此项**必须**为 "urn： SCTE： scte35：2013： bin"，以便将消息发送到 HLS、平滑和短信客户端与 [SCTE-35] 兼容。 |
| trackName          | String         | 必选      | **必须**是稀疏轨迹的名称。TrackName 可用于区分具有相同方案的多个事件流。 每个唯一的事件流都**必须**有唯一的轨道名称。                                |
| 刻度          | 多种         | 可选      | **必须**为父轨迹的时间刻度。                                                                                                                                                                               |

---

### <a name="222-movie-box"></a>2.2.2 电影箱
...
| **字段名称** | **字段类型**          | **必填？** | **描述**                                                                                                    |
| -------------- | ----------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------ |
| 持续时间       | 64位无符号整数 | 必选      | **应**为0，因为跟踪框有零个样本，并且 "跟踪" 框中的样本的总持续时间为0。 |
...
| **字段名称** | **字段类型**          | **必填？** | **描述**       |
| -------------- | ----------------------- | ------------- | --------------------- |
| handler_type   | 32位无符号整数 | 必选      | **应**为 "meta"。 |

---

"Stsd" 框**应**包含带有编码名称的 MetaDataSampleEntry 框，如 [ISO-14496-12] 中所定义。  例如，对于 SCTE-35 消息，编码名称**应**为 "SCTE"。

### <a name="223-movie-fragment-box-and-media-data-box"></a>2.2.3 影片片段框和媒体 Data Box
...
| **字段名称**         | **字段类型**          | **必填？** | **描述**                                                                                           |
| ---------------------- | ----------------------- | ------------- | --------------------------------------------------------------------------------------------------------- |
| fragment_absolute_time | 64位无符号整数 | 必选      | **必须**是事件的到达时间。 此值将消息与父轨迹对齐。           |
| fragment_duration      | 64位无符号整数 | 必选      | **必须**是事件的持续时间。 持续时间可以为零，以指示持续时间未知。 |
...
| **字段名称**          | **字段类型**                   | **必填？** | **描述**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------------- | -------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| version                 | 32位无符号整数（uimsbf） | 必选      | 确定 "mdat" 框的内容的格式。 不可识别的版本将被忽略。 目前唯一支持的版本是1。                                                                                                                                                                                                                                                                                                                                                                      |
| 识别                      | 32位无符号整数（uimsbf） | 必选      | 标识消息的此实例。 具有等效语义的消息应具有相同的值;也就是说，处理具有相同 id 的任何一个事件消息框就已足够。                                                                                                                                                                                                                                                                                                                            |
| presentation_time_delta | 32位无符号整数（uimsbf） | 必选      | 在 TrackFragmentExtendedHeaderBox 中指定的 fragment_absolute_time 和 presentation_time_delta 的总和**必须**是事件的呈现时间。 显示时间和持续时间**应**与类型为1或2的流访问点（SAP）一致，如 [ISO-14496-12] 附录 I 中所定义。对于 HLS 出口，时间和持续时间**应**与段边界对齐。 同一事件流中不同事件消息的显示时间和持续时间**不得**重叠。 |
| 消息                 | 字节数组                       | 必选      | 事件消息。 对于 [SCTE-35] 消息，消息为二进制 splice_info_section （）。 对于 [SCTE-35] 消息，这**必须**是 splice_info_section （），以便将消息发送到 HLS、平滑和短信客户端与 [SCTE-35] 兼容。 对于 [SCTE-35] 消息，二进制 splice_info_section （）是 "mdat" 框的负载，并且**不**是 base64 编码的。                                                                                                                     |
...
可以通过发送多条具有相同呈现时间和 ID 的消息来取消或更新消息。  呈现时间和 ID 唯一地标识事件。 针对特定显示时间接收的、满足预掷约束的最后一条消息是所处理的消息。 更新的消息将替换以前接收的任何消息。  前滚约束为4秒。 将对在显示时间之前至少四秒钟收到的消息进行操作。 


## <a name="3-timed-metadata-delivery"></a>3计时的元数据传递

事件流数据对媒体服务不透明。 媒体服务只在引入终结点和客户端终结点之间传递三部分信息。 以下属性将传递给客户端，符合 [SCTE-35] 和/或客户端的流式处理协议：

1.  方案–标识消息方案的 URN 或 URL。
2.  呈现时间–媒体时间线上事件的显示时间。
3.  Duration –事件的持续时间。
4.  ID –事件的可选唯一标识符。
5.  Message –事件数据。
...
| **属性名称** | 类型                      | **必填？**                             | **描述**                                                                                                                                                                                                                                                                          |
| ------------------ | ----------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 暗示                | 带引号的字符串                 | 必选                                  | 编码为 base64 编码字符串的消息，如 [RFC4648] 中所述。 对于 [SCTE-35] 消息，这是 base64 编码的 splice_info_section （）。                                                                                                                                      |
| 类别               | 带引号的字符串                 | 必选                                  | 标识消息方案的 URN 或 URL。 对于 [SCTE-35] 消息，该类型采用特殊值 "scte35"。                                                                                                                                                                          |
| id                 | 带引号的字符串                 | 必选                                  | 事件的唯一标识符。 如果引入消息时未指定 ID，Azure 媒体服务将生成唯一 id。                                                                                                                                              |
| 持续时间           | 十进制浮点数 | 必选                                  | 事件的持续时间。 如果未知，则该值**应**为0。 单位为 factional 秒。                                                                                                                                                                                           |
| 已用            | 十进制浮点数 | 可选，但对于滑动窗口是必需的 | 当该信号重复以支持滑动显示窗口时，此字段**必须**为自事件开始后已经过去的呈现时间量。 单位为秒的小数部分。 此值可能会超出接合或分段的原始指定持续时间。 |
| TIME               | 十进制浮点数 | 必选                                  | 事件的呈现时间。 单位为秒的小数部分。                                                                                                                                                                                                                        |


HLS player 应用程序层将使用该类型来识别消息的格式、对消息进行解码、应用必要的时间转换，并处理事件。  根据事件的时间戳，事件时间在父级轨迹的片段播放列表中保持同步。  它们已插入在最近的片段（#EXTINF 标记）前面。
...
当启用滑动显示窗口时，将从 "段" 播放列表中删除外接程序标记，当它们引用的媒体时间已从滑动显示窗口中汇总时。
...
| **属性名称** | 类型                | **必填？** | **描述**                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------ | ----------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| scheme_id_uri      | string                  | 必选      | 标识消息的方案。 方案在 "实时服务器清单" 框中设置为 "方案" 特性的值。 该值**应**为标识消息方案的 URN 或 URL;支持的输出 schemeId 应为 "urn： scte： scte35：2014： xml + bin" （每个 [SCTE-214-1] sec 6.7.4 （MPD）），因为该服务此时仅支持 "xml + bin"，以便在 MPD 中简洁。 |
| value              | string                  | 可选      | 方案所有者用来自定义消息语义的其他字符串值。 为了区分具有相同方案的多个事件流，**必须**将该值设置为事件流的名称（[SSTR-摄取] 的 trackName 或 [RTMP] 摄取的 AMF 消息名称）。                                                                         |
| Timescale          | 32位无符号整数 | 必选      | 时间刻度，以每秒的刻度为单位。                                                                                                                                                                                                                                                                                                                                                     |
...
带内事件流要求 MPD 在适配集级别具有 InbandEventStream 元素。  此元素具有必需的 schemeIdUri 属性和可选时间刻度属性，该属性也会出现在事件消息框（"emsg"）中。  未在 MPD 中定义的方案标识符的事件消息框不**应**存在。
...
| **字段名称**          | **字段类型**          | **必填？** | **描述**                                                                                                                                                                                                                                                                                        |
| ----------------------- | ----------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| scheme_id_uri           | string                  | 必选      | 标识消息的方案。 方案在 "实时服务器清单" 框中设置为 "方案" 特性的值。 该值**必须**为标识消息方案的 URN。 对于 [SCTE-35] 消息，此项**必须**为 "urn： SCTE： scte35：2013： bin" （符合 [SCTE-214-3]）          |
| “值”                   | string                  | 必选      | 方案所有者用来自定义消息语义的其他字符串值。 为了区分具有相同方案的多个事件流，值将设置为事件流的名称（trackName 用于平滑引入或 RTMP 引入的 AMF 消息名称）。 |
| Timescale               | 32位无符号整数 | 必选      | "Emsg" 框中的 "时间" 和 "持续时间" 字段的时间刻度，以每秒刻度数为单位。                                                                                                                                                                                                            |
| Presentation_time_delta | 32位无符号整数 | 必选      | 事件显示时间的媒体呈现时间增量和此段中的最早表示时间。 显示时间和持续时间**应**与类型为1或2的流访问点（SAP）一致，如 [ISO-14496-12] 附录 I 中所定义。                                  |
| event_duration          | 32位无符号整数 | 必选      | 事件的持续时间，或为0xFFFFFFFF 表示未知的持续时间。                                                                                                                                                                                                                              |
| Id                      | 32位无符号整数 | 必选      | 标识消息的此实例。 具有等效语义的消息应具有相同的值。 如果引入消息时未指定 ID，Azure 媒体服务将生成唯一 id。                                                                                        |
| Message_data            | 字节数组              | 必选      | 事件消息。 对于 [SCTE-35] 消息，消息数据是与 [SCTE-214-3] 相容的二进制 splice_info_section （）                                                                                                                                                                        |
...
对于视频和音频轨迹，事件在 "emsg" 框中带内信号。  对于其 presentation_time_delta 为15秒或更少的所有段请求，会发出信号。 

当启用滑动显示窗口时，当事件消息的时间和持续时间的总和小于清单中的媒体数据时间时，将从 MPD 中删除事件消息。  换言之，当事件消息所引用的媒体时间从 "滑动演示" 窗口中汇总出来时，它们会从清单中删除。
...
[35 SCTE] 使用方案 **"urn： SCTE： scte35：2013： bin"** 作为 [引入] 的 [SSTR] 引入的类型 **"scte35"** ，以二进制格式消息。 为了促进 [SCTE-35] 计时的转换（基于 MPEG-2 传输流表示时间戳（PT）），时间（splice_time （））和媒体时间线之间的磅（pts_time + pts_adjustment 之间的映射由事件表示时间提供（用于平滑引入的 fragment_absolute_time 字段和 RTMP 引入的时间字段。 映射是必需的，因为33位磅值大约每26.5 小时滚动一次。
