...
description: 在本教程中，你将了解如何使用 IoT Edge 上的实时视频分析将基于事件的视频录制到云中并从云中播放。
ms.topic: tutorial
ms.date: 05/27/2020
...
ms.locfileid: "84300818"
---
# <a name="tutorial-event-based-video-recording-to-cloud-and-playback-from-cloud"></a>教程：将基于事件的视频录制到云中并从云中播放

在本教程中，你将了解如何使用 IoT Edge 上的实时视频分析，选择性地将实时视频源的部分内容录制到云中的媒体服务。 此用例在本教程中称为[基于事件的视频录制](event-based-video-recording-concept.md) (EVR)。 为实现此目的，你将使用对象检测 AI 模型在视频中查找对象，并仅在检测到某种类型的对象时录制视频剪辑。 你还将了解如何使用媒体服务播放录制的视频剪辑。 对于需要保留所需视频剪辑的各种方案，该功能非常有用。
...
* 开发计算机上的 [Visual Studio Code](https://code.visualstudio.com/)，带有 [Azure IoT Tools](https://marketplace.visualstudio.com/items?itemName=vsciot-vscode.azure-iot-tools) 扩展和 [C#](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp) 扩展。

    > [!TIP]
    > 系统可能会提示你安装 Docker。 你可以忽略该提示。
...
完成上述步骤后，会在 Azure 订阅中部署某些 Azure 资源，包括：
...
基于事件的视频录制 (EVR) 是指由事件触发的视频录制过程。 此事件可能因视频信号本身的处理（例如检测到视频中的移动对象时）而产生或产生自独立源（例如打开门）。 或者，可以仅在推理服务检测到特定事件发生时触发录制。  本教程将使用在高速公路上移动的车辆的视频，并在检测到卡车时录制视频剪辑。

![媒体图](./media/event-based-video-recording-tutorial/overview.png)

上图以图画形式呈现了[媒体图](media-graph-concept.md)以及用于完成所需方案的其他模块。 共涉及四个 IoT Edge 模块：
...
* 自定义模块，用于对要在本教程中生成和部署的对象进行计数和筛选（在上图中称为“对象计数器”）。
...
* 第一条路径的目的地是[帧速率筛选处理器](media-graph-concept.md#frame-rate-filter-processor)节点，该节点以指定（降低的）帧速率输出视频帧。 这些视频帧将发送到 HTTP 扩展节点，该节点随后将帧（作为图像）中继到 AI 模块（YOLO v3，这是一个对象探测器）并接收结果，结果是模型检测到的对象（交通流中的车辆）。 然后，HTTP 扩展节点会通过 IoT 中心消息接收器节点向 IoT Edge 中心发布结果。
* 设置了对象计数器模块，目的是从 IoT Edge 中心接收消息 - 其中包括对象检测结果（交通流中的车辆）。 它将检查这些消息，查找特定类型（通过设置进行配置）的对象。 当找到此类对象时，此模块将向 IoT Edge 中心发送消息。 然后系统将这些“找到对象”消息路由到媒体图的 IoT 中心源节点。 接收到此类消息后，媒体图中的 IoT 中心源节点会触发[信号入口处理器](media-graph-concept.md#signal-gate-processor)节点，导致后者在配置的时间内处于开启状态。 在此持续时间内，视频流会经过入口到达资产接收器节点。 然后，实时流的这一部分将通过[资产接收器](media-graph-concept.md#asset-sink)节点被记录到 Azure 媒体服务帐户中的[资产](terminology.md#asset)内。

## <a name="set-up-your-development-environment"></a>设置开发环境

在开始之前，请检查是否满足[先决条件](#prerequisites)中的第三个项目符号。 资源设置脚本完成后，单击大括号即可公开文件夹结构。 你将看到在 ~/clouddrive/lva-sample 目录下创建的几个文件。
...
* ~/clouddrive/lva-sample/appsetting.json - Visual Studio Code 用于运行示例代码。

以下步骤需要用到这些文件。

1. 从此处克隆存储库： https://github.com/Azure-Samples/live-video-analytics-iot-edge-csharp 。
1. 启动 Visual Studio Code，然后打开用于下载存储库的文件夹。
...
在上一步中，你将启动 Visual Studio Code 并打开包含示例代码的文件夹。
...
打开“src/edge/deployment.objectCounter.template.json”。 请注意，“模块”部分下有四个条目，对应于上面列出的项目（概念部分中）：

* lvaEdge - 这是 IoT Edge 上的实时视频分析模块
* yolov3 -这是使用 YOLO v3 模型构建的 AI 模块
* rtspsim - 这是 RTSP 模拟器
* objectCounter -这是在 yolov3 的结果中查找特定对象的模块

对于 objectCounter 模块，请参阅用于“image”值的字符串 (${MODULES.objectCounter}) - 这基于关于开发 IoT Edge 模块的[教程](https://docs.microsoft.com/azure/iot-edge/tutorial-develop-for-linux)。 Visual Studio Code 将自动识别对象计数器模块的代码位于“src/edge/modules/objectCounter”下。 

阅读[本](https://docs.microsoft.com/azure/iot-edge/module-composition#declare-routes)部分，了解如何在 IoT Edge 部署清单中声明路由，然后检查模板 JSON 文件中的路由。 请注意：
...
使用 Visual Studio Code，按照[这些](https://docs.microsoft.com/azure/iot-edge/tutorial-develop-for-linux#build-and-push-your-solution)说明登录到 Docker 和“生成并推送 IoT Edge 解决方案”，但在此步骤中使用 src/edge/deployment.objectCounter.template.json。

![生成并推送 IoT Edge 解决方案](./media/event-based-video-recording-tutorial/build-push.png)

这会生成用于对象计数的 objectCounter 模块，并将图像推送到 Azure 容器注册表 (ACR)。

* 检查确认已在 .env 文件中定义 CONTAINER_REGISTRY_USERNAME_myacr 和 CONTAINER_REGISTRY_PASSWORD_myacr 环境变量。

以上步骤将在 src/edge/config/deployment.objectCounter.amd64.json 创建 IoT Edge 部署清单。 右键单击该文件，然后单击“为单个设备创建部署”。

![为单个设备创建部署](./media/quickstarts/create-deployment-single-device.png)

如果在本教程中你是首次使用 IoT Edge 上的实时视频分析，Visual Studio Code 将提示你输入 IoTHub 连接字符串。 可以从 appsettings.json 文件中复制字符串。

接下来，Visual Studio Code 会要求你选择 IoT 中心设备。 选择 IoT Edge 设备（应为“lva-sample-device”）。

在此阶段，将启动将边缘模块部署到 IoT Edge 设备的过程。
大约 30 秒后，刷新 Visual Studio Code 中左下方的 Azure IoT 中心，应会看到已部署了 4 个模块（请再次注意名称是否为：lvaEdge、rtspsim、yolov3 和 objectCounter）。

![部署的 4 个模块](./media/event-based-video-recording-tutorial/iot-hub.png)

## <a name="prepare-for-monitoring-events"></a>准备监视事件

若要从对象计数器模块和 IoT Edge 上的实时视频分析模块中查看事件，请执行以下步骤：
...
1. Visual Studio Code，导航到“src/cloud-to-device-console-app/operations.json”

1. 在节点 GraphTopologySet 下，编辑以下内容：
...
1. 接下来，在节点 GraphInstanceSet 和 GraphTopologyDelete 下编辑，

    `"topologyName" : "EVRtoAssetsOnObjDetect"`
1. 启动调试会话（点击 F5）。 在 TERMINAL 窗口中，你将开始看到一些输出的消息。

1. operations.json 首先调用 GraphTopologyList 和 GraphInstanceList。 如果在先前的快速入门或教程后清理了资源，这将返回空列表，然后暂停以便点击 Enter，如下所示：
    ...
    ```
    1. 在 TERMINAL 窗口中按“Enter”键时，会进行下一组直接方法调用。
...
1. 图形实例将继续运行并录制视频 - RTSP 模拟器将继续循环源视频。 查看以下部分中所述的消息，之后，返回到 TERMINAL 窗口并按“Enter”，停止实例。 接下来会执行一系列调用，以清理资源：
...
运行媒体图时，IoT Edge 上的实时视频分析模块会将某些诊断和操作事件发送到 IoT Edge 中心。 Visual Studio Code 的 OUTPUT 窗口中显示的消息包含“body”部分和“applicationProperties”部分。 要了解这些部分表示的内容，请参阅[创建和读取 IoT 中心消息](https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-messages-construct)。
...
实例化媒体图后，RTSP 源节点尝试连接到在 RTSP 模拟器容器上运行的 RTSP 服务器。 如果成功，它将打印此事件。 请注意，事件类型为 Microsoft.Media.MediaGraph.Diagnostics.MediaSessionEstablished。
...
* “eventTime”指示事件发生的时间，该时间是指流量视频 (MKV file) 开始以实时流的形式进入模块的时间。
...
applicationProperties 包含 eventTime，这是“对象计数器”模块观察到 YOLO v3 模块的结果开始包含所需对象（卡车）的时间。

随着视频中检测到其他卡车，所显示的事件也会随之增加。

### <a name="recordingstarted-event"></a>RecordingStarted 事件

在对象计数器发送事件后，紧接着会看到一个类型为 Microsoft.Media.Graph.Operational.RecordingStarted 的事件
...
ApplicationProperties 中的“subject”引用图形中的资产接收器节点，该节点生成此消息。 正文包含有关输出位置的信息，在本例中是用于录制视频的 Azure 媒体服务资产的名称。 应记下此值。
...
此事件指示已向资产写入足够的数据，以便播放机/客户端启动视频播放。 ApplicationProperties 中的“subject”引用图形中的资产接收器节点，该节点生成此消息。 正文包含有关输出位置的信息，在本例中是用于录制视频的 Azure 媒体服务资产的名称。

### <a name="recordingstopped-event"></a>RecordingStopped 事件

如果检查[拓扑](https://github.com/Azure/live-video-analytics/tree/master/MediaGraph/topologies/evr-hubMessage-assets/topology.json)中信号入口处理器节点的激活设置 (maximumActivationTime)，将会看到“入口”设置为在 30 秒视频流经过后关闭入口。 RecordingStarted 事件后的大约 30 秒，应会看到一个类型为 Microsoft.Media.Graph.Operational.RecordingStopped 的事件，指示资产接收器节点已停止将视频录制到资产。
...
此事件指示录制已停止。 ApplicationProperties 中的“subject”引用图形中的资产接收器节点，该节点生成此消息。 正文包含有关输出位置的信息，在本例中是用于录制视频的 Azure 媒体服务资产的名称。

## <a name="media-services-asset"></a>媒体服务资产  

可以登录到 Azure 门户来检查图创建的媒体服务资产，并观看视频。

1. 打开 Web 浏览器，并转到 [Azure 门户](https://portal.azure.com/)。 输入登录到门户所需的凭据。 默认视图是服务仪表板。
1. 在订阅中的资源中找到媒体服务帐户，并打开“帐户”边栏选项卡
1. 单击媒体服务列表中的“资产”。

    ![资产](./media/continuous-video-recording-tutorial/assets.png)
1. 会发现列出了一个名为 sampleAssetFromEVR-LVAEdge-{DateTime} 的资产，该名称是 RecordingStarted 事件的 outputLocation 属性中提供的名称。 拓扑中的 assetNamePattern 决定了此名称的生成方式。
1. 请单击该资产。
1. 在“资产详细信息”页上，单击“流式处理 URL”文本框下的“新建”。

    ![新建资产](./media/continuous-video-recording-tutorial/new-asset.png)

1. 在打开的向导中，接受默认选项，然后点击“添加”。 有关详细信息，请参阅[视频播放](video-playback-concept.md)。

    > [!TIP]
    > 请确保[流式处理终结点正在运行](../latest/streaming-endpoint-concept.md)。
1. 播放机应该会加载视频，你应该能够点击“播放”来观看。

> [!NOTE]
> 由于视频源是模拟相机源的容器，因此视频中的时间戳与激活图形实例的时间和停用该实例的时间有关。 如果使用[播放多日录制](playback-multi-day-recordings-tutorial.md)教程中内置的播放控件，可以看到屏幕上显示视频中的时间戳。

## <a name="clean-up-resources"></a>清理资源

如果你想要尝试其他教程，则应保留创建的资源。 否则，请转到 Azure 门户，浏览到资源组，选择运行本教程所用的资源组，并删除该资源组。
