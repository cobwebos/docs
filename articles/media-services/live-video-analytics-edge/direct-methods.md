...
IoT Edge 上的实时视频分析公开了多个可从 IoT 中心调用的直接方法。 直接方法表示与设备进行的请求-答复式交互，类似于会立即成功或失败（在用户指定的超时时间后）的 HTTP 调用。 此方法用于即时操作过程不同的情况，即时操作的不同取决于设备能否响应。 有关详细信息，请参阅[了解和调用 IoT 中心的直接方法](../../iot-hub/iot-hub-devguide-direct-methods.md)。
