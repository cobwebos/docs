---
title: 《将视频连续录制到云中并从云中播放》教程 - Azure
description: 在本教程中，你将了解如何使用 IoT Edge 上的实时视频分析将视频连续录制到云中并使用 Azure 媒体服务流式传输该视频的任何部分。
ms.topic: tutorial
ms.date: 05/27/2020
...
ms.locfileid: "84259983"
---
# <a name="tutorial-continuous-video-recording-to-cloud-and-playback-from-cloud"></a>教程：将视频连续录制到云中并从云中播放  

在本教程中，你将了解如何使用 IoT Edge 上的实时视频分析对云执行[连续视频录制](continuous-video-recording-concept.md) (CVR) 并使用 Azure 媒体服务流式传输该视频的任何部分。 这对于诸如安全性、合规性等方案很有用，因为这些方案需要将相机中的素材存档保存数天（或数周）。
...
完成上述步骤后，Azure 订阅中会部署某些 Azure 资源，包括：
...
正如[此处](media-graph-concept.md)所述，使用媒体图可以定义应从何处捕获媒体、应如何处理媒体以及应将结果交付到何处。 若要完成 CVR，需要从支持 RTSP 的相机中捕获视频，并将其连续记录到 [Azure 媒体服务资产](terminology.md#asset)。 下图显示了该媒体图的图形表示形式。
...
1. 复制 ~/clouddrive/lva-sample/appsettings.json 文件中的内容。 文本应如下所示：
    ...
    ```
    借助 IoT 中心连接字符串可以使用 Visual Studio Code 通过 Azure IoT 中心将命令发送到 Edge 模块。
...
在 Visual Studio Code 打开“src/edge/deployment.template.json”。 该模板定义了要部署到边缘设备 (Azure Linux VM) 的边缘模块。 请注意，“模块”部分下有两个条目，其名称如下：

* lvaEdge - 这是 IoT Edge 模块上的实时视频分析
* rtspsim - 这是 RTSP 模拟器
...
1. 通过单击左下角的 AZURE IOT HUB 窗格旁边的“更多操作”图标，设置 IoTHub 连接字符串。 可以复制 src/cloud-to-device-console-app/appsettings.json 文件中的字符串。 

    ![设置 IOT 连接字符串](./media/quickstarts/set-iotconnection-string.png)
1. 接下来，右键单击“src/edge/deployment.template.json”文件并单击“生成 IoT Edge 部署清单”。 Visual Studio Code 使用 .env 文件中的值来替换在部署模板文件中找到的变量。 这会在 src/edge/config 文件夹中创建一个名为“deployment.amd64.json”的清单文件。

   ![生成 IoT Edge 部署清单](./media/quickstarts/generate-iot-edge-deployment-manifest.png)
1. 右键单击“src/edge/config/deployment.amd64.json”，然后单击“为单个设备创建部署”。

   ![为单个设备创建部署](./media/quickstarts/create-deployment-single-device.png)
1. 然后，系统会要求你“选择 IoT 中心设备”。 从下拉列表中选择 lva-sample-device。
1. 在大约 30 秒内，刷新左下部分的 Azure IoT 中心，应会看到边缘设备已部署以下模块：
    * IoT Edge 模块上的实时视频分析（模块名称为“lvaEdge”）
...
1. 接下来，在节点 GraphInstanceSet 和 GraphTopologyDelete 下，确保 topologyName 的值与上述图拓扑中的“name”属性的值匹配：
...
1. operation.json 首先调用 GraphTopologyList 和 GraphInstanceList。 如果在先前的快速入门或教程后清理了资源，这会返回空列表，然后暂停以便你能够点击 Enter，如下所示：
...
1. 如果现在切换到 Visual Studio Code 中的 OUTPUT 窗口，会看到 IoT Edge 模块上的实时视频分析功能发送到 IoT 中心的消息。

     * 以下各部分将介绍这些消息
1. 图形实例将继续运行并录制视频 - RTSP 模拟器将继续循环源视频。 若要停止录制，请返回到 TERMINAL 窗口并点击“Enter”。 接下来会执行一系列调用，以清理资源：
...
运行媒体图时，IoT Edge 模块上的实时视频分析会将某些诊断和操作事件发送到 IoT Edge 中心。 这些事件是 Visual Studio Code 的 OUTPUT 窗口中显示的消息，其中包含正文”部分和“applicationProperties”部分。 若要理解这些部分的内容，请阅读[本文](https://docs.microsoft.com/azure/iot-hub/iot-hub-devguide-messages-construct)。
...
激活图形实例后，RTSP 源节点会尝试连接到在 rtspsim 模块中运行的 RTSP 服务器。 如果成功，它会输出此事件：
...
* ApplicationProperties 中的“使用者”引用生成消息的图形拓扑中的节点。 在本例中，该消息来自 RTSP 源节点。
...
正文包含有关输出位置的信息，在本例中是用于录制视频的 Azure 媒体服务资产的名称。 应记下此值。
...
此事件指示已向资产写入足够的数据，以便播放机/客户端启动视频播放。
...
停用图形实例后，资产接收器节点会停止将视频记录到资产，并发出类型为 Microsoft.Media.Graph.Operational.RecordingStopped 的此事件。
...
1. 你会看到其中列有一个资产，名为 sampleAsset-CVRToAMSAsset-Sample-Graph-1，这是在图形拓扑文件中选择的命名模式。
1. 请单击该资产。
1. 在“资产详细信息”页上，单击流 URL 文本框下的“新建”。

    ![新资产](./media/continuous-video-recording-tutorial/new-asset.png)

1. 在打开的向导中，接受默认选项，然后点击“添加”。 有关详细信息，请参阅[视频播放](video-playback-concept.md)。

    > [!TIP]
    > 请确保[流式处理终结点正在运行](../latest/streaming-endpoint-concept.md)。
1. 播放机会加载视频，可点击“播放”>** 观看。

> [!NOTE]
> 由于视频源是模拟相机源的容器，因此视频中的时间戳与激活图形实例的时间和停用该实例的时间有关。 可参阅[本](playback-multi-day-recordings-tutorial.md)教程，了解如何浏览多天的录制并查看该存档的部分内容。 在该教程中，还可以在屏幕上显示的视频中看到时间戳。
