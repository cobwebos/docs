...
| 名称 | 值 | 说明 |
| --- | --- | --- |
| policy_overrides。 can_play |boolean。 true 或 false |指示允许播放内容。 默认值为 false。 |
| policy_overrides。 can_persist |boolean。 true 或 false |指示可以将许可证保存到非易失性存储器供脱机使用。 默认值为 false。 |
