...
品牌检测在许多业务场景中非常有用，例如，内容存档和发现、上下文广告、社交媒体分析、零售竞争分析，等等。 通过视频索引器品牌检测，可以使用必应的品牌数据库以及自定义项通过为每个视频索引器帐户构建一个自定义品牌模型，为在语音和视觉文本中提到的品牌编制索引。 使用“自定义品牌模型”功能，可以选择是否让视频索引器检测必应品牌数据库中的品牌；可以排除某些品牌，不让其检测到（实质上是创建一个品牌的方块列表）；还可以添加一些品牌，这些品牌应该在你的模型中，但可能不在必应的品牌数据库中（实质上是创建一个品牌的允许列表）。
