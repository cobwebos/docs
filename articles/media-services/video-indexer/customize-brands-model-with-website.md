...
- 如果希望视频索引器排除检测到某些品牌（实质上是创建品牌的拒绝列表），请选择此。
- 如果希望视频索引器包含的品牌应为可能不在 Bing 品牌数据库中的模型的一部分（实质上是创建品牌的接受列表），请选择此项。
...
1. 选择 " **+ 添加品牌**"。
    ...
    "引用 URL" 字段可以是此品牌的任何参考网站（如其维基百科页面的链接）。
