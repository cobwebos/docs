...
ms.locfileid: "79499912"
---
# <a name="create-a-video-indexer-account-connected-to-azure"></a>创建连接到 Azure 的视频索引器帐户

创建视频索引器帐户时，您可以选择免费试用帐户（在其中获得一定数量的免费索引分钟数）或付费选项（不受配额限制）。 通过免费试用，视频索引器为网站用户提供长达 600 分钟的免费索引，为 API 用户提供长达 2400 分钟的免费索引。 使用付费选项，您可以创建连接到 Azure 订阅的视频索引器帐户和 Azure 媒体服务帐户。 您支付已编制索引的分钟数以及媒体帐户相关费用。
...
* Azure 活动目录 （Azure AD） 域。

    如果没有 Azure AD 域，请使用 Azure 订阅创建此域。 有关详细信息，请参阅在[Azure AD 中管理自定义域名](../../active-directory/users-groups-roles/domains-manage.md)

* 具有**应用程序管理员**角色的 Azure AD 域中的用户。 将视频索引器帐户连接到 Azure 时，将要使用此成员。

    此用户应是具有工作或学校帐户的 Azure AD 用户。 不要使用个人帐户，如outlook.com、live.com或hotmail.com。
...
> [!NOTE]
> 如果 Azure 订阅使用基于证书的多重身份验证，则必须在安装了所需证书的设备上执行以下步骤至关重要。
...
5. 在“Azure 媒体服务帐户”下，选择以下选项之一：****
    ...
    * 若要创建新的媒体服务帐户，请选择“创建新资源组”。**** 提供资源组的名称。
        ...
        Azure 会在订阅中创建新帐户，包括新的 Azure 存储帐户。 您的新媒体服务帐户具有默认的初始配置，具有流式处理终结点和 10 S3 保留单位。
    * 若要使用现有的媒体服务帐户，请选择“使用现有资源”。**** 从帐户列表中选择自己的帐户。

        媒体服务帐户必须与视频索引器帐户位于同一区域。

        > [!NOTE]
        > 为了最小化索引持续时间和低吞吐量，强烈建议将媒体服务帐户中的[预留单位](../previous/media-services-scale-media-processing-overview.md )的类型和数量调整为**10 S3 预留单位**。 请参阅[使用门户更改保留单位](../previous/media-services-portal-scale-media-processing.md)。

    * 要手动配置连接，请选择 **"切换到手动配置**"链接。
...
> [!NOTE]
> 强烈建议在同一区域中具有以下三个帐户：与媒体服务帐户连接的视频索引器帐户以及连接到同一媒体服务帐户的 Azure 存储帐户。
...
1. 使用 [Azure](https://portal.azure.com/) 门户创建 Azure 媒体服务帐户，如[创建帐户](../previous/media-services-portal-create-account.md)中所述。
    ...
    为媒体服务帐户创建存储帐户时，为复制字段选择 **"存储V2"** 科目类型和**异地冗余 （GRS）。**

    ![新的 AMS 帐户](./media/create-account/create-ams-account1.png)

    > [!NOTE]
    > 请务必记下媒体服务资源和帐户名。 在下一节中的步骤中，您需要它们。

2. 将[保留单位](../previous/media-services-scale-media-processing-overview.md )的类型和数量调整为您创建的媒体服务帐户中的**10 S3 预留单位**。 请参阅[使用门户更改保留单位](../previous/media-services-portal-scale-media-processing.md)。
3. 在视频索引器 Web 应用中播放视频之前，必须启动新媒体服务帐户的默认**流式处理终结点**。

    在新的媒体服务帐户中，选择**流式处理终结点**。 然后选择流终结点并按"开始"。

    ![新的 AMS 帐户](./media/create-account/create-ams-account2.png)

4. 对于使用媒体服务 API 进行身份验证的视频索引器，需要创建 AD 应用。 以下步骤引导你完成[通过 Azure 门户开始使用 Azure AD 身份验证](../previous/media-services-portal-get-started-with-aad.md)中所述的 Azure AD 身份验证过程：

    1. 在新的媒体服务帐户中，选择“API 访问”****。
    2. 选择[服务主体身份验证方法](../previous/media-services-portal-get-started-with-aad.md)。
    3. 获取客户端 ID 和客户端密码

        选择 **"设置**->**键**"后，添加 **"说明"，** 按 **"保存**"，然后填充键值。

        如果密钥过期，帐户所有者必须联系视频索引器支持以续订密钥。

        > [!NOTE]
        > 务必记下密钥值和应用程序 ID。 在下一节中的步骤中，您需要它。
...
|设置|描述|
|---|---|
|视频索引器帐户区域|视频索引器帐户区域的名称。 为了更好的性能和更低的成本，强烈建议指定 Azure 媒体服务资源和 Azure 存储帐户所在的区域的名称。 |
|Azure AD 租户|Azure AD 租户的名称，例如“contoso.onmicrosoft.com”。 可以在 Azure 门户中检索租户信息。 将光标悬停在右上角的登录用户名上。 找到“域”右侧的名称****。|
|订阅 ID|应在其下创建此连接的 Azure 订阅。 可以从 Azure 门户中检索订阅 ID。 选择左侧面板中的所有**服务**，然后搜索"订阅"。 选择“订阅”并从订阅列表中选择所需的 ID****。|
...
* 如果连接到现有媒体服务帐户，视频索引器不会更改现有媒体**预留单位**配置。

   您可能需要根据计划负载调整媒体保留单位的类型和数量。 请记住，如果负载较高，并且没有足够的单位或速度，则视频处理可能导致超时失败。

* 如果连接到新的媒体服务帐户，视频索引器会自动在其中启动默认的流式处理终结点****：

    ![媒体服务流式处理终结点](./media/create-account/ams-streaming-endpoint.png)

    流式处理终结点具有相当大的启动时间。 因此，从将帐户连接到 Azure 到在视频索引器 Web 应用中流式传输和观看视频可能需要几分钟时间。

* 如果连接到现有媒体服务帐户，视频索引器不会更改默认的流式处理终结点配置。 如果没有正在运行的**流式处理终结点**，则无法观看来自此媒体服务帐户或视频索引器的视频。

## <a name="next-steps"></a>后续步骤

您可以按照以下说明：[使用 API，](video-indexer-use-apis.md)以编程方式与试用帐户和/或与连接到 Azure 的视频索引器帐户进行编程交互。
