...
视频索引器支持自动语言标识（盖子），这是自动识别音频中的语言内容并发送以主要标识语言转录的媒体文件的过程。 

当前盖子支持：英语、西班牙语、法语、德语、意大利语、普通话中文、日语、俄语和葡萄牙语（巴西）。 
...
可以将 insights JSON 中的模型主导语言作为 `sourceLanguage` 属性（位于根/视频/见解下）提供。 属性下也提供了相应的置信度分数 `sourceLanguageConfidence` 。
...
* 尽管视频索引器支持阿拉伯语（新式标准和 Levantine）、印地语和韩语，但在盖子中不支持这些语言。
