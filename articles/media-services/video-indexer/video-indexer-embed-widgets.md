...
若要嵌入专用视频，必须在 iframe 的属性中传递访问令牌 `src` ：
...
- [获取视频访问令牌](https://api-portal.videoindexer.ai/docs/services/Operations/operations/Get-Video-Access-Token?)。 将其作为查询参数添加到 URL。 `src`如前面所示，将此 URL 指定为 iframe 的值。
...
1. 可以在时间线中编辑脚本。
1. 视频索引器获取这些更新，并将它们保存在语言模型的 " [记录编辑](customize-language-model-with-website.md#customize-language-models-by-correcting-transcripts) " 中。
1. 更新字幕：

    * 如果使用视频索引器的播放机小组件，它会自动更新。
    * 如果使用的是外部播放机，则会获得一个新的字幕文件 "用户 **获取视频标题** "。
...
默认情况下，播放机将开始播放视频。 您可以选择不传递到 `&autoplay=false` 前面的嵌入 URL。
