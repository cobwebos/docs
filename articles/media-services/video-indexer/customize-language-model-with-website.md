...
您可以通过选择语言模型右侧的**省略号（"..."**）按钮，然后选择 "**重命名**"，来更改语言模型的名称。
...
还可以通过选择语言模型右侧的**省略号（"..."**）按钮添加文本文件，然后选择 "**添加文件**"。

### <a name="delete-files"></a>删除文件

若要从语言模型删除文件，请选择文本文件右侧的省略号（**...**）按钮，然后选择 "**删除**"。 系统会弹出一个新窗口，告诉您无法撤消删除操作。 选择 "新建" 窗口中的 "**删除**" 选项。

此操作会从语言模型中彻底删除该文件。

## <a name="delete-a-language-model"></a>删除语言模型

若要从你的帐户中删除语言模型，请选择语言模型右侧的省略号（**"..."）按钮**，然后选择 "**删除**"。
...
1. 选择铅笔图标以编辑脚本的脚本。
    ...
    > [!NOTE]
    > 只有文本更正可用于自定义。 不包含实际字词的更正（例如标点符号或空格）不包括在内。
