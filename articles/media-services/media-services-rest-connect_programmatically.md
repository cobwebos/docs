...
访问 Microsoft Azure 媒体服务时需要以下两项内容：由 Azure 访问控制服务 (ACS) 提供的访问令牌和媒体服务本身的 URI。在创建这些请求时，可以使用任何想要的方法，前提是在调用 Media Services 时指定了正确的标头值，并且正确地传入了访问令牌。
...
>[AZURE.NOTE]获取新 URI 后，应该使用该 URI 来与 Media Services 通信。
