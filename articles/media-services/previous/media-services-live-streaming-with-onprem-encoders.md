...
### <a name="channel-input-ingest-configurations"></a><a id="channel_input"></a>频道输入（引入）配置
...
可以选择通过 TLS 连接引入分片 MP4（平滑流式处理）实时流。 要通过 TLS 进行引入，请确保将引入 URL 更新为 HTTPS。 当前，无法通过 TLS 引入 RTMP。
...
## <a name="suggestions-and-feedback"></a>建议和反馈
