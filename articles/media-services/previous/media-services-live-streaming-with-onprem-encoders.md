...
>
>有关推荐的本地编码器的信息，请参阅[建议在本地编码器](media-services-recommended-encoders.md)。
...
* **已停止**：这是频道在创建后的初始状态。 在这种状态下，可以更新频道属性，但不允许进行流式处理。
* **正在启动**：正在启动频道。 此状态不允许进行更新或流式处理。 如果发生错误，则频道会返回到“已停止”状态。
* **正在运行**：频道可以处理实时流。
