...
* **已停止**。 这是频道在其创建后的初始状态。 在这种状态下，可以更新频道属性，但不允许进行流式处理。
* **正在启动**。 正在启动频道。 此状态不允许进行更新或流式处理。 如果发生错误，频道将返回到“已停止”状态。
* **正在运行**。 频道可以处理实时流。
* **正在停止**。 正在停止频道。 此状态不允许进行更新或流式处理。
* **正在删除**。 正在删除频道。 此状态不允许进行更新或流式处理。
