...
description: 本文概述了 Azure 媒体服务流终结点。
...
Azure 媒体服务向流式处理终结点实体添加了以下属性 **：Cdn 提供程序** **、CdnProfile、****流式处理终结点版本**。 有关这些属性的详细概述，请参阅[本主题](https://docs.microsoft.com/rest/api/media/operations/streamingendpoint)。 
...
从 2017 年 1 月的媒体服务版本开始，您有两种流类型：**标准**（预览版）和**高级**版 。 这些类型属于流式处理终结点版本“2.0”。


|类型|说明|
|--------|--------|  
|**标准**|默认的流式处理终结点是“标准”**** 类型，可以通过调整流单元更改为“高级”类型。|
|**溢价** |此选项适用于需要更高级缩放或控制的专业方案。 通过调整流式处理单元，您将移动到**高级**类型。<br/>专用流式处理终结点存在于隔离环境中，不会争用资源。|
