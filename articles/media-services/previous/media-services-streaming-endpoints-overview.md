...
|Type|描述|
|--------|--------|  
|**标准**|默认流式处理终结点是**标准**类型，可以通过调整流式处理单位更改为高级类型。|
|**高级** |此选项适用于需要更高级缩放或控制的专业方案。 可以通过调整流式处理单位移动到**高级**类型。<br/>专用的流式处理终结点位于独立环境中，不会争用资源。|
