...
Function.json 文件定义函数绑定和其他配置设置。 运行时使用此文件确定要监视的事件，以及如何将数据传入函数执行和从函数执行返回数据。 有关详细信息，请参阅[Azure 函数 HTTP 和 Webhook 绑定](../../azure-functions/functions-reference.md#function-code)。
