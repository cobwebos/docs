...
如果存在预先存在的策略，则无需指定内容密钥规范中的任何值。与此内容关联的预先存在的策略用于确定输出保护，例如高带宽数字内容保护 (HDCP) 和 Copy General Management System (CGMS) 。 如果预先存在的策略未注册到 Widevine 许可证服务器，则内容提供者可以在许可证请求中注入值。   
