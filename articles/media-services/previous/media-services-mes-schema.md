...
## <a name="encoding"></a><a name="Encoding"></a>编码
包含以下元素序列：  
...
| “属性” | 类型 | 说明 |
| --- | --- | --- |
| **H264视频** |[H264视频](media-services-mes-schema.md#H264Video) |视频的 H.264 编码设置。 |
...
| **同步模式**<br/><br/> minOccurs=“0” | |将在未来版本中公开功能。 |
...
| “属性” | 类型 | 说明 |
| --- | --- | --- |
| **条件** |**xs:string** | 当输入没有视频时，您可能希望强制编码器插入单色视频轨道。为此，请使用条件="仅插入BlackIfNoVideo"（仅以最低比特率插入视频）或"插入BlackIfNoVideo"（在所有输出比特率处插入视频）。 有关详细信息，请参阅[此](media-services-advanced-encoding-with-mes.md#no_video)文章。|
...
| “属性” | 类型 | 说明 |
| --- | --- | --- |
| **配置 文件**<br/><br/> minOccurs=“0”<br/><br/> 默认值 =“自动” |**xs: string** |可能是以下 **xs: string** 值之一：**Auto**、**Baseline**、**Main**、**High**。 |
| **水平**<br/><br/> minOccurs=“0”<br/><br/> 默认值 =“自动” |**xs: string** | |
| **比特率**<br/><br/> minOccurs=“0” |**xs:int** |此视频层使用的比特率，以 kbps 为单位指定。 |
...
| “属性” | 类型 | 说明 |
| --- | --- | --- |
| **配置 文件**<br/><br/> minOccurs="0"<br/><br/> 默认值 =“AACLC” |**xs: string** |可能是以下值之一：**AACLC**、**HEAACV1** 或 **HEAACV2**。 |
...
## <a name="audiogroup"></a><a name="AudioGroup"></a>音频组
...
| “属性” | 类型 | 说明 |
| --- | --- | --- |
| **渠道**<br/><br/> minOccurs=“0” |**xs: int** |音频通道数。 以下是有效选项：1、2、5、6、8。<br/><br/> 默认值：2。 |
| **SamplingRate**<br/><br/> minOccurs=“0” |**xs: int** |音频采样率，以 Hz 为单位。 |
| **比特率**<br/><br/> minOccurs=“0” |**xs: int** |对音频进行编码时使用的比特率，以 kbps 为单位指定。 |
...
## <a name="clip"></a><a name="Clip"></a>剪辑
### <a name="attributes"></a>特性
...
| **时间** |**xs:duration** |指定演示文稿的持续时间（例如，视频中覆盖的外观）。 |
...
| “属性” | 类型 | 说明 |
| --- | --- | --- |
| **文件名** |**xs:string** |输出文件的名称。<br/><br/> 可以使用下表中描述的宏来生成输出文件名。 例如：<br/><br/> **"Outputs": [      {       "FileName": "{Basename}*{Resolution}*{Bitrate}.mp4",       "Format": {         "Type": "MP4Format"       }     }   ]** |
...
| **[代码]** |到视频的“H264”以及音频的“AAC”的映射。 |
| **[比特率]** |如果输出文件包含视频和音频，则为目标视频比特率；如果输出文件仅包含音频，则为目标音频比特率。 使用的值是以 kbps 为单位的比特率。 |
| **[通道]** |如果文件包含音频，则为音频通道计数。 |
| **[宽度]** |如果文件包含视频，则为输出文件中视频的宽度（以像素为单位）。 |
| **[高度]** |如果文件包含视频，则为输出文件中视频的高度（以像素为单位）。 |
| **[扩展]** |继承自输出文件的“类型”属性。 输出文件名具有以下扩展名之一：“mp4”、“ts”、“jpg”、“png”或“bmp”。 |
| **[索引]** |对于缩略图是必需的。 只应存在一次。 |
...
| **步骤** |**xs:string** | |
| **范围** |**xs:string** | |
| **旋转后保留分辨率** |**xs:boolean** |有关详细说明，请参阅以下部分：[PreserveResolutionAfterRotation](media-services-mes-schema.md#PreserveResolutionAfterRotation) |
...
或者，您可以使用 **"保留决议后旋转"** 标志并将其设置为"true"（默认值为"false"）。 因此，如果预设为宽度 = "100%"，高度 = "100%"，并将 PreserveResolutionAfterRotation 设置为“true”，则宽为 1280 像素、高为 720 像素，且 90 度旋转的输入视频将生成 0 度旋转、但宽为 720 像素、高为 1280 像素的输出视频。 参阅下图：  
