...
- 以下各项必须在 AMS 密钥传送端上设置：
	...
	- **应用证书密码 ID** - 客户必须以和上传其他 AMS 密钥类似的方式并使用 **ContentKeyType.FairPlayPfxPassword** 枚举值上传密码。在结果中，他们将得到 AMS ID，在密钥传送策略选项中，他们需要使用此 ID。
	- **iv** - 16 字节随机值必须与资产传送策略中的 iv 相匹配。客户生成 IV 并将其放入两个位置：资产传递策略和密钥传递策略选项。 
	...
	-  **ASK ID** - 在客户将 ASK 上传到 AMS 时获得。客户必须使用 **ContentKeyType.FairPlayASk** 枚举值上传 ASK。因此，将返回 AMS ID，在设置密钥传送策略选项时应使用此 ID。
...
1. 针对你要传送内容的“流式处理终结点”，获取至少一个流式处理单位。有关详细信息，
请参阅：[配置流式处理终结点](/documentation/articles/media-services-dotnet-get-started#configure-streaming-endpoint-using-the-portal)。
