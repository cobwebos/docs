<properties 
	pageTitle="使用多重 DRM 的 CENC 和访问控制：Azure 与 Azure 媒体服务的参考设计和实现" 
...
Azure Active Directory, Azure 媒体服务, Azure Media Player, 动态加密, 许可证传送, PlayReady, Widevine, FairPlay, 通用加密 (CENC), 多重 DRM, DASH, EME, MSE, JSON Web 令牌 (JWT), 声明, 现代浏览器, 密钥滚动更新, 对称密钥, 非对称密钥, OpenID Connect, X509 证书
...
1. 降低管理加密资产的成本，因为只需要一份加密资产；
1. 消除 DRM 客户端许可成本，因为原生 DRM 客户端在其原生平台上通常是免费提供的。

Microsoft 已成为 DASH 和 CENC 与其他一些主要行业播放器的积极推动者。Microsoft Azure 媒体服务为 DASH 和 CENC 提供支持。有关最新的通告，请参阅 Mingfei 的博客：[宣布已在 Azure 媒体服务中推出 Google Widevine 许可证传送服务](https://azure.microsoft.com/blog/announcing-general-availability-of-google-widevine-license-services/)和 [Azure 媒体服务已添加 Google Widevine 打包功能用于传送多重 DRM 流](https://azure.microsoft.com/blog/azure-media-services-adds-google-widevine-packaging-for-delivering-multi-drm-stream/)。
...
1. 提供 Microsoft Azure/Azure 媒体服务平台上的参考实现；
1. 讨论一些设计和实现主题。
...
1. Google Widevine
1. Apple FairPlay（尚不受 Azure 媒体服务支持）
...
**Windows 10 设备（Windows 电脑、Windows 平板电脑、Windows Phone、Xbox）**|PlayReady|MS Edge/IE11/EME<br/><br/><br/>UWP|DASH（HLS 不支持 PlayReady）<br/><br/>DASH、平滑流（HLS 不支持 PlayReady） 
**Android 设备（手机、平板电脑、电视）**|Widevine|Chrome/EME|DASH
**iOS（iPhone、iPad）、OS X 客户端和 Apple 电视**|FairPlay|Safari 8+/EME|HLS
**插件：Adobe Primetime**|Primetime Access|浏览器插件|HDS、HLS
...
1. 外围网络层（蓝色表示），包含所有面向公众的终结点；
1. 公共 Internet 层（浅蓝色表示），包含具有跨公共 Internet 流量的 CDN 和播放器。
...
1. 对用户进行身份验证；
1. 为用户创建授权令牌；
1. 将 DRM 保护的内容（清单）下载到播放器；
1. 播放器将许可证获取请求和密钥 ID 与授权令牌提交到许可证服务器。

在转到下一个主题之前，还需要注意一些与密钥管理设计相关的事项。
...
多对一|每个资产需要多个内容密钥。<br/><br/>例如，如果需要针对 MPEG-DASH 应用具有多重 DRM 的动态 CENC 保护，且针对 HLS 应用动态 AES-128 加密，需要两个不同的内容密钥，两者分别有自己的 ContentKeyType。（对于用于动态 CENC 保护的内容密钥，应该使用 ContentKeyType.CommonEncryption，而对于用于动态 AES-128 加密的内容密钥，应该使用 ContentKeyType.EnvelopeEncryption。）<br/><br/>另举一例，DASH 内容在 CENC 的保护下，理论上可以使用一个内容密钥来保护视频流，并使用另一个内容密钥来保护音频流。 
...
1. 每月订阅：使用非持久性许可证，以及内容密钥与资产之间的一对一映射。在这种情况下：
...
**DRM 许可证传送**|1\.Azure 媒体服务许可证传送（PlayReady、Widevine、FairPlay）或 <<br/>2.自定义 PlayReady 许可证服务器
**源**|Azure 媒体服务流式处理终结点
**密钥管理**|不需要参考实现
**内容管理**|一个 C# 控制台应用程序

换而言之，标识提供者 (IDP) 和安全令牌服务 (STS) 都将是 Azure AD。对于播放器，我们将使用 [Azure Media Player API](http://amp.azure.net/libs/amp/latest/docs/)。Azure 媒体服务和 Azure Media Player 都支持具有多重 DRM 的 DASH 和 CENC。
...
1. AssetDeliveryPolicy，包含有关流格式、DRM 保护和许可证获取 URL 的规范。
...
1. 播放器根据支持的浏览器/DRM 发出许可证获取请求。在许可证获取请求中，还会提交密钥 ID 和 JWT 令牌。许可证传送服务将验证 JWT 令牌，在发布所需的许可证之前包含声明。
...
1. 使用 [Azure Media Player API](http://amp.azure.net/libs/amp/latest/docs/) 创建播放器。[Azure Media Player 的 ProtectionInfo API](http://amp.azure.net/libs/amp/latest/docs/) 可让你指定要在不同的 DRM 平台上使用哪种 DRM 技术。
1. 测试矩阵：
...
George 还撰写了一篇相关的博客文章：[Azure 媒体服务和动态加密中的 JWT 令牌身份验证](http://gtrifonov.com/2015/01/03/jwt-token-authentication-in-azure-media-services-and-dynamic-encryption/)。以下是他发表的[将 Azure AD 与 Azure 媒体服务密钥传送集成的示例](https://github.com/AzureMediaServicesSamples/Key-delivery-with-AAD-integration/)。
...
1. **颁发者** URL 应以**“/”**结尾。  
	...
	**受众**应该是播放器应用程序客户端 ID，你也应该在颁发者 URL 的末尾添加**“/”**。
	...
	在 [JWT 解码器](http://jwt.calebb.net/)中，你应该可以在 JWT 令牌中看到 **aud** 和 **iss**，如下所示：
...
1. 客户端与 Azure AD 之间的 JWT 令牌交换（需要使用 HTTPS）；
1. 客户端的 DRM 许可证获取，如果许可证传送由 Azure 媒体服务提供（需要使用 HTTPS）。当然，PlayReady 产品套件不会针对许可证传送强制要求使用 HTTPS。如果 PlayReady 许可证服务器位于 Azure 媒体服务以外的地方，则可以使用 HTTP 或 HTTPS。
...
1. 应该避免混合内容：都使用 HTTP，或者都使用 HTTPS。播放混合内容时，silverlightSS 技术需要清除混合内容警告。flashSS 技术可处理混合内容，但不提供混合内容警告。
1. 如果流式处理终结点是在 2014 年 8 月之前创建的，则它不支持 HTTPS。在此情况下，请针对 HTTPS 创建并使用新的流式处理终结点。

在参考实现中，对于 DRM 保护内容，应用程序和流都使用 HTTTPS。对于打开的内容，播放器不需要身份验证或许可证，因此可以任意使用 HTTP 或 HTTPS。

### Azure Active Directory 签名密钥滚动更新
...
在此“应用程序标识”流程中，Web API 相信 Web 应用程序已对用户进行了身份验证。因此，此模式称为受信任的子系统。[此页面上的示意图](http://msdn.microsoft.com/zh-cn/library/azure/dn645542.aspx/)描绘了授权代码授予流程的工作方式。
...
1. 安全令牌服务需要颁发令牌，许可证服务器场可接受和验证这些令牌。例如，Axinom 提供的 Widevine 许可证服务器需要特定的 JWT 令牌，其中包含“权利消息”。因此，需要有 STS 来颁发此类 JWT 令牌。作者已完成此类实现，
1. 你不再需要在 Azure 媒体服务中配置许可证传送服务 (ContentKeyAuthorizationPolicy)。你需要做的就是，在设置使用多重 DRM 的 CENC 过程中配置 AssetDeliveryPolicy 时，提供许可证获取 URL（针对 PlayReady、Widevine 和 FairPlay）。
...
可以在[此处](https://openidconnectweb.chinacloudsites.cn/)找到播放器 Web 应用程序及其登录凭据。
...
Windows 10 的 Microsoft Edge 及 IE 11 中的 EME，允许支持 [PlayReady SL3000](https://www.microsoft.com/playready/features/EnhancedContentProtection.aspx/) 的 Windows 10 设备上的 PlayReady SL3000 调用。PlayReady SL3000 解锁增强型高级内容（4K、HDR 等）以及新的内容传送模型（增强型内容的早期窗口）流程。
