...
如果你运行`Azure Dev Spaces: Redirect an existing Kubernetes service to my machine [Preview]`，则会要求你选择现有的 Kubernetes 服务：
...
如果你运行`Azure Dev Spaces: Redirect an existing Kubernetes pod to my machine [Preview]`，则会要求你选择特定的 pod：

![选择 Pod](../media/how-to-connect/connect-choose-pod.png)

此选项连接到特定的 pod。 此选项可用于与不发送或接收流量以及终止复制盒的 pod 交互。 如果 pod 发送和接收流量，则此选项的行为方式类似于`Azure Dev Spaces: Redirect an existing Kubernetes service to my machine [Preview]` ，会将 AKS 群集中的所有流量重定向到与所选 pod 服务相关的所有 pod。
...
对于本示例，请`Azure Dev Spaces: Redirect an existing Kubernetes service to my machine [Preview]`选择并选择 "*自行车*" 服务。
...
单击 "*调试*"，然后*打开 "配置*"。 如果系统提示选择环境，请选择 *"node.js"。* 这将创建`.vscode/launch.json`一个文件。 将该文件的内容替换为以下内容：
...
单击 "Azure Dev Spaces" 状态栏，然后选择应用程序的 "公共 URL"，导航到 " *bikesharingweb* " 服务。 还可以从前面运行的`azds list-uris`命令中找到公共 URL。 如果未在群集上使用 Azure Dev Spaces，请使用应用程序的 IP 或 URL 作为正在使用的命名空间。 在以上示例中，*bikesharingweb* 服务的公共 URL 为 `http://dev.bikesharingweb.fedcab0987.eus.azds.io/`。 选择 " *Aurelia meets Briggs （客户）* " 作为用户，然后选择要出租的自行车。
...
通过将光标置于第233行`server.js`并达到*F9*来删除断点。
...
Azure Dev Spaces 无需任何其他配置即可处理路由流量和复制环境变量。 如果需要将装载到 AKS 群集中的容器的所有文件（例如 ConfigMap 文件）下载，可以创建`azds-local.env`将这些文件下载到开发计算机上。
