---
title: 配置 AKS Dev Spaces 群集以使用 Helm 3（预览版）
...
---
# <a name="configure-your-azure-dev-spaces-cluster-to-use-helm-3-preview"></a>配置 AKS Dev Spaces 群集以使用 Helm 3（预览版）
...
在具有此预览功能的区域中创建新的 AKS 群集。 以下命令将创建名为*MyResourceGroup*的资源组和名为*MYAKS*的新 AKS 群集：
...
使用 "*使用*AKS" 命令在你的群集上启用 dev 空间，并按照提示进行操作。 以下命令在 *MyResourceGroup* 组中的 *MyAKS* 群集上启用 Dev Spaces，并创建一个默认开发空间。
