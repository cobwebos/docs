...
* 执行任何形式的集成测试随之也会变得很困难。 集成测试只能在提交后以切实可行的方式进行，这意味着，在开发周期的后期才能发现问题。
    ...
    ![描述在开发微服务时面临的挑战的示意图。](media/common/microservices-challenges.png)
...
1. 选择 _dev_ 空间：`azds space select --name dev`。 当系统提示选择父 dev 空间时，请选择 \<none\>。
...
![显示简单 CI/CD 空间设置的关系图。](media/common/ci-cd-space-setup.png)
