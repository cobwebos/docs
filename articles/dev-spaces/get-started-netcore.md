---
title: 创建 Kubernetes 开发空间：Visual Studio Code 和 .NET Core
...
1. 下载并安装 [VS Azure Dev Spaces](https://marketplace.visualstudio.com/items?itemName=azuredevspaces.azds) 和 [C#](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp) 扩展。 对于每个扩展，在扩展的“市场”页上单击“安装”一次，然后再在 VS Code 中单击一次。
...
![a z d s trace 终端窗口](media/get-started-netcore/azds-trace.png)
...
![此屏幕截图显示“命令面板”窗口中选择的命令“Azure Dev Spaces：准备 Azure Dev Spaces 的配置文件”。](media/common/command-palette.png)
...
![屏幕截图显示了 Visual Studio Code 窗口的左上角。 调试图标突出显示，左侧面板标题为“调试”，标题右侧的下拉列表显示“dot NET Core Launch (A Z D S)”。](media/get-started-netcore/debug-configuration.png)
...
![屏幕截图显示“Visual Studio Code”窗口底部。 橙色状态栏为最后一行。 它包含一个用于打开网站的 URL。](media/common/vscode-status-bar-url.png)
...
![调试操作窗格是位于页面顶部中心的一个小窗格（就在页面标题下方）。 “重启”按钮突出显示。按钮的悬停图像为“重启(ctrl + shift + F5)”。](media/common/debug-action-refresh.png)
