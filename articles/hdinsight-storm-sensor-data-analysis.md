<properties
   pageTitle="使用 Apache Storm on HDInsight、事件中心和 HBase 分析传感器数据 | Azure"
   description="了解如何使用 HDInsight 上的 Apache Storm 和 HBase 处理来自 Azure 事件中心的数据。"
   services="hdinsight"
   documentationCenter=""
   authors="Blackmist"
   manager="paulettm"
...
> [AZURE.NOTE] Java、JDK、Maven 和 Git 也可通过 [Chocolatey NuGet](http://chocolatey.org/) 程序包管理器获得。
...
1. 在 [Azure 门户](https://manage.windowsazure.com)中，选择"新建"|"Service Bus"|"事件中心"|"自定义创建"。
...
5. 保存策略后，使用页面底部的"共享访问密钥生成器"检索**设备**和 **storm** 策略的密钥。保存这些密钥，因为稍后将要用到。
...
4. 在"群集详细信息"页面上，输入新群集的名称，并选择"Storm"作为"群集类型"。单击箭头继续。
...
打开 **Config.properties** 文件并添加先前创建事件中心时使用的信息。在添加此信息后，请保存该文件。
	...
	## 如果未提供，将使用 Storm 的 zookeeper 设置
...
1. 打开新的命令提示符或终端，并将目录更改为 将目录更改为 **hdinsight-eventhub-example/SendEvents/nodejs**，并使用以下命令安装应用程序所需的依赖项：
...
1. 若要将仪表板部署到 Azure 网站，请遵循[生成 Node.js 网站并将其部署到 Azure](web-sites-nodejs-develop-deploy-mac) 中的步骤。记下网站的 URL，它类似于 **mywebsite.azurewebsites.net**。
...
2. 请按照[部署和管理 Storm 拓扑](hdinsight-storm-deploy-monitor-topology)上载并使用在你 Storm 指在 HDInsight 群集上启动拓扑 **Storm 仪表板**。
...
3. 键入或选择以下值：
	...
	- **DNS 服务器**：本文使用 Azure 提供的内部 DNS 服务器，因此，你可以选择"无"。此外，也支持使用自定义 DNS 服务器的高级网络配置。有关详细指导，请参阅[名称解析 (DNS)](http://msdn.microsoft.com/zh-cn/library/azure/jj156088.aspx)。
...
9. 在页面底部，默认子网名称为 **Subnet-1**。使用"添加子网"按钮添加 **Subnet-2**。Storm 和 HBase 群集将位于这些子网中。
	...
	> [AZURE.NOTE] 在本文中，我们将使用只有一个节点的群集。如果你创建的是多节点群集，你必须为用于群集的子网验证其 **CIDR（地址数）**。地址数必须大于工作节点数加上七（网关：2，头节点：2，ZooKeeper：3）。例如，如果需要一个 10 节点 HBase 群集，子网的地址数必须大于 17 (10+7)。否则，部署将失败。
	>
	> 强烈建议为一个群集指定一个子网。
...
群集名称开头的域名称的部分是 DNS 后缀，例如 **mycluster.b1.cloudapp.net**。

### 启用 HBase bolt

1. 打开 **hdinsight-eventhub-example\TemperatureMonitor\conf\hbase-site.xml** 和替换 `suffix` 为 HBase 群集先前获取的 DNS 后缀的以下行中的条目。进行这些更改之后，保存该文件。
...
* 有关 HDinsight Storm 拓扑的更多示例，请参阅：
	...
	* [Storm on HDInsight 示例](https://github.com/hdinsight/hdinsight-storm-examples)
