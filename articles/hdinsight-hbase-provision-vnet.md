<properties 
   pageTitle="在 Azure 虚拟网络上设置 HBase 群集 | Azure" 
...
了解如何在 [Azure 虚拟网络][1]上创建 HDInsight HBase 群集。 
...
- **Azure 订阅**。Azure 是基于订阅的平台。有关获得订阅的详细信息，请参阅[购买选项][azure-purchase-options][试用版][azure-trial]。

- **安装并配置有 Azure PowerShell 的工作站**。有关说明，请参阅[安装和配置 Azure PowerShell](install-configure-powershell)。若要执行 Azure PowerShell 脚本，必须以管理员身份运行 Azure PowerShell 并将执行策略设为  *RemoteSigned*。请参阅[使用 Set-ExecutionPolicy cmdlet][2]。
...
3. 键入或选择以下值：
	...
	- **最大 VM 计数** - 选择最大虚拟机 (VM) 计数之一。此值确定可以在地址空间下创建的可能主机 (VM) 数。为了完成本教程，**4096 [CIDR:/20]** 就足矣。 
	- **位置** - 位置必须与你要创建的 HBase 群集相同。
	- **DNS 服务器** - 本教程使用 Azure 提供的内部域名系统 (DNS) 服务器，因此，你可以选择**"无"**。还支持将更高级的网络配置用于自定义 DNS 服务器。有关详细指导，请参阅[名称解析 (DNS)](http://msdn.microsoft.com/zh-cn/library/azure/jj156088.aspx)。
4. 单击右下角的**"创建虚拟网络"**。新虚拟网络名称将显示在列表中。等到状态列显示**"已创建"**。
5. 在主窗格中，单击刚创建的虚拟网络。
6. 单击该页顶部的**"仪表板"**。
7. 在**"速览"**下，记住虚拟网络 ID。在设置 HBase 群集时将要用到它。
8. 单击该页顶部的**"配置"**。
9. 在该页底部，默认子网名称为 **Subnet-1**。你可以选择重命名子网或者向 HBase 群集添加一个新子网。记下子网名称；在设置群集时将要用到它。
10. 验证要用于群集的子网的**"CIDR(地址计数)"**。地址计数必须大于从节点数加上七（网关：2 个，头节点：2 个，Zookeeper：3 个）。例如，如果需要一个 10 节点 HBase 群集，则子网的地址计数必须大于 17 (10+7)。否则，部署将失败。
...
> [AZURE.NOTE] HDInsight 群集使用 Azure Blob 存储来存储数据。有关详细信息，请参阅[在 HDInsight 中将 Azure Blob 存储与 Hadoop 配合使用](hdinsight-use-blob-storage)。你需要存储帐户和 Blob 存储容器。存储帐户位置必须与虚拟网络位置和群集位置匹配。
...
> [AZURE.NOTE] 有关通过使用 Azure PowerShell 设置新 HBase 群集的信息，请参阅[使用 Azure PowerShell 设置 HBase 群集](#powershell)。
...
6. 在**"存储帐户"**页上提供以下值：
	...
	<table border='1'>
		...
		<tr><td>帐户名</td>
			<td><ul>
				<li>如果选择了使用现有存储，请为<strong>"帐户名"</strong>选择现有的存储帐户。下拉列表仅列出你选择设置群集的相同数据中心内的存储帐户。</li>
...
若要开始使用新 HBase 群集，可以按照[开始在 HDInsight 中将 HBase 与 Hadoop 配合使用](hdinsight-hbase-get-started)中的步骤操作。
...
2.	使用 Java 应用程序远程连接到 HBase 时，必须使用完全限定域名 (FQDN)。若要确定这一点，你必须获取 HBase 群集的连接特定的 DNS 后缀。为此，请使用 Curl 查询 Ambari，或使用远程桌面来连接到群集。
	...
	* **Azure PowerShell** - 使用以下 Azure PowerShell 脚本注册 **Get-ClusterDetail** 函数，该函数可用于返回 DNS 后缀：
	...
	> [AZURE.NOTE] 你还可以使用远程桌面来连接到 HBase 群集（你将连接到头节点），并从命令提示符运行 **ipconfig** 来获得 DNS 后缀。有关启用远程桌面协议 (RDP) 并使用 RDP 连接到群集的说明，请参阅[使用 Azure 门户在 HDInsight 中管理 Hadoop 群集][hdinsight-admin-portal]。
	>
...
> [AZURE.NOTE] 有关 Azure 虚拟网络中的名称解析的详细信息，包括如何使用自己的 DNS 服务器，请参阅[名称解析 (DNS)](http://msdn.microsoft.com/zh-cn/library/azure/jj156088.aspx)。
...
[img-provision-cluster-page1]: ./media/hdinsight-hbase-provision-vnet/hbasewizard1.png "设置新 HBase 群集的详细信息"
