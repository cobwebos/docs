...
- **[Azure Private Link service](private-link-service-overview.md)** ： Azure 专用链接服务是服务提供商创建的服务。 目前，专用链接服务可附加到标准负载均衡器的前端 IP 配置。 
...
可以。 专用终结点可以跨 Azure 区域连接到 Azure PaaS 资源。
...
可以。 一个专用链接服务可以接收来自多个专用终结点的连接。 不过，一个专用终结点只能连接到一个专用链接服务。  
