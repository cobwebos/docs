...
If you want to create a web site, choose the ASP.NET Web Application project template, and then choose MVC. See [Get started with Azure and ASP.NET](http://www.windowsazure.com/zh-cn/documentation/articles/web-sites-dotnet-get-started/).
...
1. Now that you have a VSO team project with some source code in it, you are ready to connect your team project to Azure.  In the [Azure Portal](http://manage.windowsazure.cn), select your cloud service or web site, or create a new one by selecting the + icon at the bottom left and choosing **Cloud Service** or **Web Site** and then **Quick Create**. Choose the **Set up publishing with Visual Studio Online** link.<br/>
![][10]
...
or if there is no storage account, it creates one. The storage account provides a place in Azure for storage files and other data. For more information, see [What is a storage account?](http://www.windowsazure.cn/zh-cn/documentation/articles/storage-whatis-account).
...
13. In the [Azure Portal](http://manage.windowsazure.cn), you can view the associated deployment on the Deployments tab when the staging environment is selected.<br/>
![][30]
