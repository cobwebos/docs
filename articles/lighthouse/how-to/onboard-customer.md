...
|**缴费**     |租户中的用户/组/Spn 的**principalId**值，每个值都有一个**principalIdDisplayName** ，可帮助客户了解授权的目的，并将其映射到内置的**roleDefinitionId**值以指定访问级别。      |
...
下面的示例显示了一个已修改的**delegatedResourceManagement**文件，该文件可用于载入订阅。 资源组参数文件（位于 [rg-delegated-resource-management](https://github.com/Azure/Azure-Lighthouse-samples/tree/master/templates/rg-delegated-resource-management) 文件夹）很类似，但还带有一个 rgName 参数，它用于标识要载入的特定资源组****。
...
- 了解如何删除对以前载入[的委派的访问权限](remove-delegation.md)。
