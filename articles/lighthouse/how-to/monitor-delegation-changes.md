...
本主题说明监视租户（跨所有客户）的委派活动所需的权限，以及用于执行此操作的最佳实践。 它还包含一个示例脚本，其中显示了用于查询和报告此数据的一种方法。
...
若要访问租户级别的活动日志数据，必须在根范围（/）向帐户分配[监视读取器](../../role-based-access-control/built-in-roles.md#monitoring-reader)内置角色。 此分配必须由具有 "全局管理员" 角色的用户执行，并且具有其他提升的访问权限。

### <a name="elevate-access-for-a-global-administrator-account"></a>提升全局管理员帐户的访问权限

若要在根作用域（/）上分配角色，你将需要具有提升的访问权限的全局管理员角色。 仅当需要进行角色分配时才应添加此提升的访问权限，并在完成后将其删除。
...
提升访问权限后，可以为帐户分配适当的权限，以便它可以查询租户级别的活动日志数据。 此帐户需要在管理租户的根范围内分配[监视读取器](../../role-based-access-control/built-in-roles.md#monitoring-reader)内置角色。

> [!IMPORTANT]
> 在根范围授予角色分配意味着相同的权限将应用于租户中的每个资源。

由于这是一种广泛的访问级别，因此建议将此角色分配给服务主体帐户，而不是分配给单个用户或组。 此外，我们建议采用以下最佳做法：
...
- [使用证书进行身份验证](../../active-directory/develop/howto-create-service-principal-portal.md#certificates-and-secrets)，并[将其安全地存储在 Azure Key Vault 中](../../key-vault/general/best-practices.md)。
- 限制有权代表服务主体执行操作的用户。
...
[此 Azure PowerShell 脚本](https://github.com/Azure/Azure-Lighthouse-samples/tree/master/tools/monitor-delegation-changes)可用于查询过去1天的活动，并报告任何已添加或已删除的委派（或未成功的尝试）。 它将查询[租户活动日志](https://docs.microsoft.com/rest/api/monitor/TenantActivityLogs/List)数据，然后构造以下值以报告添加或删除的委托：
...
- 对以前的委派所做的更改（例如更新权限结构）将记录为添加的委派。
- 如上所述，帐户必须具有 "监视读取者" 内置角色（/），才能访问此租户级别的数据。
