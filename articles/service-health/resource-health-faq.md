...
## <a name="does-resource-health-differentiate-between-unavailability-caused-by-platform-problems-versus-something-i-did"></a>资源运行状况是否区分平台问题导致的不可用与我的操作是什么？
...
资源运行状况为基于活动日志的警报提供[预览支持](resource-health-alert-arm-template-guide.md)。 活动日志警报使用[操作组](https://docs.microsoft.com/azure/azure-monitor/platform/action-groups)来通知用户已触发警报。 操作组支持各种通知通道，例如电子邮件、短信、Webhook 和 ITSM 操作。
