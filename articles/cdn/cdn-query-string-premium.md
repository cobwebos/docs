...
- **唯一缓存**：在此模式下，包含唯一 URL 的每个请求（包括查询字符串）将视为具有其自己的缓存的唯一资产。 例如，源服务器对 `example.ashx?q=test1` 的请求做出的响应将缓存在 POP 节点，并为具有同一查询字符串的后续缓存返回该响应。 `example.ashx?q=test2` 的请求将作为具有其自己的生存时间设置的单独资产来缓存。
