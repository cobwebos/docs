...
* **唯一缓存**：此模式将具有查询字符串的每个请求视为具有其自己的缓存的唯一资产。  例如，来自于源对 *foo.ashx?q=bar* 的请求做出的响应将缓存在边缘节点，并为具有该查询字符串的后续缓存返回该响应。  对 *foo.ashx?q=somethingelse* 提出的请求将作为具有其自己的生存时间的单独资产来缓存。
