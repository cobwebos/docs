...
ms.locfileid: "79252111"
---
# <a name="optimize-azure-cdn-for-the-type-of-content-delivery"></a>针对内容分发类型优化 Azure CDN

向许多全球受众传送内容时，请务必优化内容传送。 [Azure 内容交付网络 （CDN）](cdn-overview.md)可以根据您拥有的内容类型优化交付体验。 内容可以是网站、实时传送视频流、视频或可供下载的大文件。 创建 CDN 终结点时，请在“优化对象”**** 选项中指定方案。 选择哪个方案决定了对通过 CDN 终结点传送的内容应用哪种优化。
...
* [一般网络交付](#general-web-delivery)。 此优化也用于媒体流式处理和大型文件下载。

> [!NOTE]
> 微软的动态站点加速是通过 Azure[前门服务](https://docs.microsoft.com/azure/frontdoor/front-door-overview)提供的。
...
2. 在"设置"下，选择 **"优化**"。 然后，从 **"优化"下**拉列表中选择一个类型。
...
> [!NOTE]
> 如果 CDN 终结点主要用于视频点播内容，请使用此优化类型。 此优化类型与一般媒体流优化类型之间的主要区别是连接重试超时。使用实时流式处理方案的超时时间要短得多。
>
...
> [!NOTE]
> 来自 Microsoft 的动态站点加速通过[Azure 前门服务](https://docs.microsoft.com/azure/frontdoor/front-door-overview)提供，该服务是利用 Microsoft 专用全球网络提供应用工作负载的全球[任意广播](https://en.wikipedia.org/wiki/Anycast)服务。
