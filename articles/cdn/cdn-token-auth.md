...
令牌身份验证是一种机制，允许阻止 Azure 内容交付网络 (CDN) 向未经授权的客户端提供资产。 进行令牌身份验证通常是为了防止内容的“热链接”，即不同的网站（如信息板）未经允许就使用资产。 热链接可能会影响内容交付费用。 在 CDN 上启用令牌身份验证，将先由 CDN 边缘服务器对请求进行身份验证，再由 CDN 交付内容。 
