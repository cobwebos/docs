...
2. [创建 CNAME DNS 记录](#step-2-create-the-cname-dns-records)
    ...
    - 选项 2：使用 cdnverify 子域将自定义域映射到 CDN 终结点 
...
如果使用 Azure 托管 [DNS 域](https://docs.microsoft.com/azure/dns/dns-overview)，必须将域提供商的 DNS 委托给 Azure DNS。 有关详细信息，请参阅 [向 Azure DNS 委派域](https://docs.microsoft.com/azure/dns/dns-delegate-domain-azure-dns)。
...
CNAME 记录映射特定的域和子域，例如 `www.contoso.com` 或 `cdn.contoso.com`；它无法将 CNAME 记录映射到根域，例如 `contoso.com`。 一个子域只能与一个 CDN 终结点相关联。 CNAME 记录会将寻址到子域的所有流量路由到指定的终结点。 例如，如果将 `www.contoso.com` 与 CDN 终结点关联在一起，则不能将它与其他 Azure 终结点（例如，存储帐户终结点或云服务终结点）相关联。 但是，可以将同一域中的不同子域用于不同的服务终结点。 还可以将不同子域映射到同一 CDN 终结点。

使用以下选项之一将自定义域映射到 CDN 终结点：

- 选项 1：将自定义域直接映射到 CDN 终结点。 如果自定义域上未运行生产流量，则可以直接将自定义域映射到 CDN 终结点。 将自定义域映射到 CDN 终结点的过程可能会导致在 Azure 门户中注册域时出现短暂的停机。 CNAME 映射条目应采用以下格式： 
...
- 选项 2：使用 cdnverify 子域将自定义域映射到 CDN 终结点。 如果自定义域上运行了不可中断的生产流量，可以创建 CNAME 到 CDN 终结点的临时映射。 借助此选项，可以使用 Azure **cdnverify** 子域来提供中间注册步骤，这样，在发生 DNS 映射时，用户仍可以持续访问你的域。
...
此步骤依赖于步骤 2 的选项 2：使用 cdnverify 子域将自定义域映射到 CDN 终结点。 如果使用的是 **cdnverify** 临时子域并确认该域可正常工作，则可以将永久自定义域映射到 CDN 终结点。
