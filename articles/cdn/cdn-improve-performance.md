...
> [!IMPORTANT]
> CDN 配置变更可能需要一些时间才能传播到整个网络。 对于**来自 Akamai 的 Azure CDN** 配置文件，传播通常在一分钟内完成。  对于**来自 Verizon 的 Azure CDN** 配置文件，传播通常在 90 分钟内完成。 如果首次为 CDN 终结点设置压缩，请考虑等待 1-2 个小时，确保将压缩设置传播到 POP 之后再排查问题。
> 
> 
...
### <a name="azure-cdn-from-verizon-profiles-both-standard-and-premium-tiers"></a>“来自 Verizon 的 Azure CDN”配置文件（标准和高级层）

对于**来自 Verizon 的 Azure CDN** 配置文件，仅压缩符合条件的文件。 要符合压缩条件，文件必须：
- 大于 128 个字节
- 小于 1 MB
...
如果对资产的请求指定了 brotli 压缩（`Accept-Encoding: br` HTTP 标头）且请求导致缓存未命中，Azure CDN 将对源服务器上的资产执行 brotli 压缩。 此后，将直接从缓存提供压缩的文件。

### <a name="azure-cdn-from-akamai-profiles"></a>“来自 Akamai 的 Azure CDN”配置文件
