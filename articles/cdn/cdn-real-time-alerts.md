...
1. 在 [Azure 门户](https://portal.azure.com)中，导航到 CDN 配置文件。
    ...
    ![CDN 配置文件边栏选项卡](./media/cdn-real-time-alerts/cdn-profile-blade.png)
2. 从 CDN 配置文件的边栏选项卡，单击“**管理**”按钮。
   
    ![CDN 配置文件边栏选项卡管理按钮](./media/cdn-real-time-alerts/cdn-manage-btn.png)
...
7. 在“**媒体类型**”下拉列表中，选择“**HTTP 大型对象**”。
   ...
   > [!IMPORTANT]
   > 你必须选择“**HTTP 大型对象**”作为“**媒体类型**”。  **来自 Verizon 的 Azure CDN** 会不使用其他选项。  如果不选择“**HTTP 大型对象**”， 警报将永远不会被触发。
   > 
   > 
8. 通过选择“**指标**”、“**运算符**”和“**触发值**”，创建一个“**表达式**”来进行监视。
   
   * 对于”**指标**”，请选择你要监视的条件类型。  “**带宽 Mbps**“是每秒兆位的带宽使用量。  ”**总连接数**“是到边缘服务器的并发 HTTP 连接数。  有关各种缓存状态和状态代码的定义，请参阅 [Azure CDN 缓存状态代码](https://msdn.microsoft.com/library/mt759237.aspx)和 [Azure CDN HTTP 状态代码](https://msdn.microsoft.com/library/mt759238.aspx)
   * “**运算符**”是在指标和触发值之间建立关系的数学运算符。
   * “**触发值**”是通知发送之前必须满足的阈值。
     
     在以下示例中，已创建的表达式表明希望在 404 状态代码数大于 25 时获得通知。
     
     ![实时警报示例表达式](./media/cdn-real-time-alerts/cdn-expression.png)
9. 对于”**间隔**“，输入希望表达式进行评估的频率。
10. 在”**通知时间**“下拉列表中，选择在表达式为 True 时你想要在何时获得通知。
    
    * “**条件开始**”指明在第一次检测到指定的条件时将发送通知。
    * ”**条件结束**“指明在未检测到指定的条件时将发送通知。 只有在网络监视系统检测到指定的条件发生后，才会触发此通知。
    * “**连续**”指明在网络监视系统每次检测到指定的条件时都将发送通知。 请记住，对于指定的条件，网络监视系统每个间隔将仅检查一次。
    * “**条件开始和结束**”指明会在第一次检测到指定的条件时发送一次通知，并在未再检测到条件时再次发送一次通知。
11. 如果想要通过电子邮件接收通知，请勾选“**通过电子邮件通知**”复选框。  
    
    ![通过电子邮件的形式通知](./media/cdn-real-time-alerts/cdn-notify-email.png)
    
    在“**收件人**”字段中，输入要将通知发送到的电子邮件地址。 对于“**主题**”和“**正文**”，你可以保留默认值，或者也可以使用“**可用关键字**”列表自定义消息，以在发送消息时动态插入警报数据。
...
12. 如果希望通知被发布到 Web 服务器，请勾选“**通过 HTTP Post 通知**”复选框。
    ...
    在“**Url**”字段中，输入要将 HTTP 消息发布到的 URL。 在“**标头**”文本框中，输入要在请求中发送的 HTTP 标头。  对于“**正文**”，你可以使用“**可用关键字**”列表自定义消息，以在发送消息时动态插入警报数据。  “**标头**”和“**正文**”默认为 XML 有效负载，类似于以下示例。
    ...
    > 
    > 
13. 单击“**保存**”按钮以保存警报配置。  如果在步骤 5 中勾选了“**启用警报**”，你的通知现在将处于活动状态。
