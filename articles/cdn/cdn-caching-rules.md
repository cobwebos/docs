...
- **覆盖**：忽略源提供的缓存持续时间;而是使用提供的缓存持续时间。 这不会覆盖缓存控制：无缓存。
...
设置这些规则后，_&lt;终结点主机名&gt;_.azureedge.net/home/index.html 的请求会触发自定义缓存规则#2，该规则设置为：**如果缺少，则设置为：如果缺少，则设置为**3 天。 因此，如果 *index.html* 文件包含 `Cache-Control` 或 `Expires` HTTP 标头，则会遵循这些标头；如果未设置这些标头，则会将文件缓存 3 天。
