...
为应用程序启用 CDN 后，可能希望监视 CDN 使用情况、检查交付内容的运行状况以及排除潜在问题。 Azure CDN 通过 [CDN 核心分析](cdn-analyze-usage-patterns.md)和[诊断日志](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs)提供了这些功能

## <a name="cdn-core-analytics"></a>CDN 核心分析
作为具有 Verizon 标准或高级配置文件的当前 Azure CDN 用户，可通过 Azure 门户的“管理”选项查看补充门户中的核心分析。 

## <a name="azure-diagnostic-logs"></a>Azure 诊断日志

Azure 通过此新功能，现可查看核心分析并将其保存到一个或多个目标，包括：
...
通过诊断日志，可将基本使用情况指标从 CDN 终结点导出到不同的源，以便以自定义方式使用它们。 例如，可执行以下数据导出类型：
...
以下演练介绍核心分析数据的架构，并介绍启用该功能并将其传递到不同目标以及从这些目标使用时所涉及的步骤。
...
要使用 Azure 存储来存储日志，请选择“存档到存储帐户”，选择保留天数，并单击“日志”下的“核心分析”。
...
1. 从“监视”下的“诊断日志”边栏选项卡中，选择“发送到 Log Analytics”。 

    ![门户 - 诊断日志](./media/cdn-diagnostics-log/05_Ready-to-Configure.png)    

2. 单击“配置”配置 Log Analytics 日志记录。 可在出现的对话框中选择上一个工作区或创建一个新的工作区。
...
4. 接下来必须选择新工作区名称、现有订阅、新的或现有的资源组、位置和定价层。 可选择将此配置固定到仪表板。 单击“确定”完成此配置。

    随即便可看到包含 OMS 工作区和资源组名称的工作区。 名称必须唯一，并且只能使用字母、数字和连字符。 不允许使用空格和下划线。 

    ![门户 - 诊断日志](./media/cdn-diagnostics-log/08_Workspace-resource.png)

5. 接下来会收到一条短消息，指出已创建工作区，并将返回到日志记录配置屏幕。 可确认 Log Analytics 工作区的名称。

    ![门户 - 诊断日志](./media/cdn-diagnostics-log/09_Return-to-logging.png)

    设置 Log Analytics 配置后，确保选中 CDN 日志记录的“核心分析”框。

6. 按需设置好一切后，请单击配置对话框顶部的“保存”按钮。

    ![门户 - 诊断日志](./media/cdn-diagnostics-log/10_Save-me.png)

    “保存”按钮不再有效，“开/关”按钮现在为“开”，但是蓝色而不是紫色。

7. 要查看新的 OMS 工作区，请前往 Azure 门户仪表板，单击 Log Analytics 工作区的名称。 接下来将看到工作区（确保 OMS 工作区在左侧突出显示）。 单击 OMS 门户磁贴以在 OMS 存储库中查看工作区。 
...
有关日志数据延迟的详细信息，请转到[此处](#log-data-delays)。

## <a name="enable-logging-with-powershell"></a>使用 PowerShell 启用日志记录

下例介绍了如何通过 Azure PowerShell cmdlet 启用并获取诊断日志。
...
首先需要一款可访问存储帐户内容的工具，才可从 Azure 存储帐户访问核心分析数据。 市场上有多种可用的工具，但推荐使用 Microsoft Azure 存储资源管理器。 可从[此处](http://storageexplorer.com/)下载工具。 下载并安装软件后，请将其配置为使用配置为 CDN 诊断日志目标的同一 Azure 存储帐户。
...
可使用“视图设计器”自定义数据视图。 转到 OMS 工作区，然后单击“视图设计器”磁贴开始设计。

![视图设计器](./media/cdn-diagnostics-log/27_Designer.png)

可从左侧拖放图表类型，并在左侧填写要分析的数据详细信息。
...
--- | ---
Verizon 日志数据延迟 1 小时，在终结点传播完成后长达 2 小时才开始显示。 | Akamai 日志数据延迟 24 小时，如果在 24 小时以前创建，则在长达 2 小时后才开始显示。 如果是最近创建的，则可能需要长达 25 个小时才能开始显示日志。
...
其中“time”表示报告统计信息的小时边界的开始时间。 如果 CDN 提供商不支持指标，此值将为 null 值，而不是 double 或 integer 值。 该 null 值表示不存在指标且与 0 值不同。 另请注意，在终结点上配置的每个域都有一组这些指标。
