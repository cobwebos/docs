...
[令牌身份验证拒绝代码](#token-auth-denial-code) | 确定以基于令牌的身份验证为由拒绝请求时为用户返回的响应类型。
...
[Cache-Key 查询字符串](#cache-key-query-string) | 确定 cache-key 是否会包括与请求关联的查询字符串参数。
[Cache-Key 重写](#cache-key-rewrite) | 重写与请求关联的 cache-key。
[完整缓存填充](#complete-cache-fill) | 确定当请求导致边缘服务器部分缓存未命中时会发生什么情况。
[压缩文件类型](#compress-file-types) | 定义会在服务器上压缩的文件的格式。
...
[遵循 No-Cache 请求](#honor-no-cache-request) | 确定是否会将 HTTP 客户端的 no-cache 请求转发到源服务器。
...
Name | 目的
-----|--------
[Age 响应标头](#age-response-header) | 确定是否会在发送给请求者的响应中包括 Age 响应标头。
...
Name | 目的
-----|--------
[自定义日志字段 1](#custom-log-field-1) | 确定将要在原始日志文件中分配给自定义日志字段的格式和内容。
[日志查询字符串](#log-query-string) | 确定是否会将查询字符串和 URL 一起存储在访问日志中。
...
--|--
 ...
 全部排除  |表示会从 cache-key 中排除所有查询字符串参数。 此配置复制“Query-String 缓存”页上名为“standard-cache”的默认缓存行为。 
...
cache-key 是一个相对路径，用于确定缓存的资产。 换言之，服务器会根据 cache-key 所定义的路径查看缓存版的资产。
...
- 此功能将只压缩大小不到 1 MB 的资产。 服务器不会压缩更大的资产。
- 某些类型的内容，例如图像、视频和音频媒体资产（例如 JPG、MP3、MP4 等）已经压缩。 再对这些类型的资产进行压缩不会显著减少文件大小。 因此，建议不要对这些类型的资产启用压缩。
- 不支持通配符，例如星号。
...
默认情况下，自定义日志字段称为“x-ec_custom-1”。 但是，可以在“原始日志设置”页中自定义该字段的名称。
...
- 此字段的有效字符包括：字母数字（0-9、a-z 和 A-Z）、短划线、冒号、分号、撇号、逗号、句点、下划线、等号、括号、方括号、空格。 百分比符号和大括号只能用于指定标头字段。
...
值|结果
-|-
Enabled|防止边缘服务器使用“416 无法满足请求的范围”状态代码响应无效的 byte-range 请求。 服务器会改为交付请求的资产并为客户端返回“200 正常”。
...
-|-
代码|选择会返回给请求者的响应代码。
源和模式| 这些设置定义的请求 URI 模式用于标识可重定向请求的类型。 只会重定向其 URL 同时满足下述两个条件的请求： <br/> <br/> **源（或内容访问点）：**选择用于标识源服务器的相对路径。 该路径是“/XXXX/”部分和终结点名。 <br/> **源（模式）：**必须定义一个可通过相对路径标识请求的模式。 此正则表达式模式必须定义一个路径，该路径直接开始于以前选择的内容访问点（见上）之后。 <br/> - 确保上面定义的请求 URI 条件（即源和模式）不与为此功能定义的任何匹配条件冲突。 <br/> - 指定模式；如果使用空白值作为模式，则匹配所有字符串。
