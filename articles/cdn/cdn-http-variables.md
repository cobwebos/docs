...
| 洲（申请者） | %{geo_continent} | 指示申请者所在的大洲（缩写形式） <br />有效值是： <br />AF非洲<br />方式亚洲<br />各欧洲<br />\N\N北美<br />OC大洋洲<br />成为南美<br /><br />**已弃用：** %{virt_dst_continent}。 <br />此变量已弃用, 以支持% {geo_continent}。 <br />虽然规则使用此已被弃用的变量后仍然有效，但还是应更新规则，以使用新变量。| 不可用 |
...
| 引用站点域 | %{referring_domain} | 指示在引用请求标头中定义的域。 | <www.google.com> |
...
| 请求 URI（不含查询字符串的相对路径） | %{uri} | 指示请求的内容的相对路径。 <br /><br/>重要信息：<br />- 此相对路径不包括查询字符串。<br />- 此相对路径反映 URL 重写。 在以下情况下，将重写 URL：<br />  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-URL 重写功能:此功能将重写在请求 URI 中定义的相对路径。<br />    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-边缘 CNAME URL:此类型的请求会重写到相应的 CDN URL。 |/800001/corigin/rewrittendir/foo.js |
...
| 字符或语法无效 | 包含无效字符或语法的变量将被视为文本值。 <br /><br />示例 #1:指定的值包含无效字符 (例如-)。 <br /><br />示例 #2:指定的值包含一组双大括号。 <br /><br />示例 #3:指定的值缺少右大括号。<br /> | 示例 #1：%{resp_user-agent} <br /><br />示例 #2：%{{host}} <br /><br />示例 #3：%{host |
...
| 条件 | 语法 | 示例 | 描述 |
| --------- | ------ | --------| ----------- |
| 当满足以下任何条件时，可将标头设置为默认值： <br /><br />- 缺少标头 <br /><br />- 标头值设置为 NULL。| %{Variable:=Value} | %{http_referrer:=unspecified} | 仅当引用站点的标头缺失或设置为 NULL 时, 才会将其设置为 "*未指定*"。 如果已设置，则无需执行任何操作。 |
| 如果标头未设置，请将其设置为默认值。 | %{Variable=Value} | %{http_referrer=unspecified} | 仅当找不到引用的标头时, 才会将其设置为*未指定*。 如果已设置，则无需执行任何操作。 |
| 如果不满足以下任何条件，请将标头设置为默认值： <br /><br />- 缺失<br /><br /> - 设置为 NULL。 | %{Variable:+Value} | %{http_referrer:+unspecified} | 向引用的标头分配值时, 将仅设置为 "*未指定*"。 如果该标头丢失或设置为 NULL，则无需执行任何操作。 |
