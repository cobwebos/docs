...
| 引用站点域 | %{referring_domain} | 指示引用器请求标头中定义的域。 | <www.google.com> |
...
| 请求 URI | %{request} | 描述请求。 <br />语法：HTTP &lt;&gt;&lt;方法相对&gt;&lt;路径 HTTP 协议&gt; | GET /marketing/foo.js?loggedin=true HTTP/1.1 |
...
| 如果不满足以下任何条件，请将标头设置为默认值： <br /><br />- 缺失<br /><br /> - 设置为 NULL。 | %{Variable:+Value} | %[http_referrer：[未指定] | 仅当向其分配了值时，引用者标头才会设置为*未指定*。 如果该标头丢失或设置为 NULL，则无需执行任何操作。 |
