...
> [!NOTE] 
> 缓存规则仅适用于 **Verizon 提供的标准 Azure CDN** 和 **Akamai 提供的标准 Azure CDN** 配置文件。 对于 **Verizon 提供的高级 Azure CDN** 配置文件，必须在[管理](cdn-rules-engine.md)门户中使用 **Azure CDN 规则引擎**来获得类似的功能。
...
1. 在“自定义缓存规则”下，创建两个匹配条件：
     ...
     答： 对于第一个匹配条件，将“匹配条件”设置为“路径”，对于“匹配值”输入 `/blobcontainer1/*`。 将“缓存行为”设置为“替代”，并在“小时”框中输入 4。
...
使用 [Azure 命令行接口](https://docs.microsoft.com/cli/azure) (CLI)，可以从命令行管理 Azure blob 资源。 若要在使用 Azure CLI 上传 Blob 时设置 cache-control 标头，可使用 *开关设置*cacheControl`-p` 属性。 以下示例说明如何将 TTL 设置为 1 小时（3600 秒）：
