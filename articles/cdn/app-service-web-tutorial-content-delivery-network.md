...
ms.locfileid: "34161238"
---
# <a name="tutorial-add-azure-cdn-to-an-azure-app-service-web-app"></a>教程：向 Azure 应用服务 Web 应用添加 Azure CDN

本教程介绍如何向 [Azure 应用服务中的 Web 应用](../app-service/app-service-web-overview.md)添加 Azure CDN。 
  [Azure 内容分发网络 (CDN)](cdn-overview.md) 将静态 Web 内容缓存在按特定策略布置好的位置，以便提供最大的吞吐量，方便将内容分发给用户。 Azure CDN 还会降低 Web 应用的服务器负载。 
...
如果还没有域名，请考虑遵照[应用服务域教程](../app-service/custom-dns-web-site-buydomains-web-app.md)使用 Azure 门户购买一个域。 
...
如果没有自定义域，请考虑遵照[应用服务域教程](../app-service/custom-dns-web-site-buydomains-web-app.md)使用 Azure 门户购买一个域。 
...
在“添加自定义域”页中，会显示创建 CNAME 记录时需要使用的终结点主机名。 主机名派生自 CDN 终结点 URL：**&lt;EndpointName>.azureedge.net**。 
...
导航到域注册机构的网站，找到创建 DNS 记录的部分。 例如，此部分可能叫做“域名”、“DNS”或“名称服务器管理”。
...
在浏览器中，使用自定义域（例如 cdn.contoso.com/index.html）导航到 index.html 文件，验证其结果是否与直接访问 &lt;endpointname&gt;azureedge.net/index.html 时的结果相同。
