...
直接在地图下方提供每个地区的详细的流量和带宽信息。 这样，可以查看命中总数、命中百分比、传输的总数据量（以千兆字节为单位）以及每个区域传输的数据的百分比。 查看每个指标的说明。 最后，将鼠标悬停在某个区域（即国家/地区、州或省）上时，该区域中发生的命中的名称和百分比会显示为工具提示。
...
还有两个额外的根据地理位置提供统计信息的报告，这两个报告分别是热门城市和热门国家/地区报告。 这些报告分别根据源自这些区域的命中数对城市和国家/地区进行排名。 生成此类型的报告后，条形图会显示在特定平台上请求内容的前 10 个城市或国家/地区。 此条形图，可快速评估生成的最大为你的内容的请求数的区域。
...
每日的摘要报表允许您查看的命中数和每日在特定平台上传输的数据的总数目。 此信息可用于快速识别 CDN 活动模式。 例如，此报表可以帮助你检测更高版本经验或低于预期流量哪几天。

在生成此类型的报告后，条形图将提供关于在报告所涵盖的时间段内每天经历的平台特定需求量的直观指示。 它将通过在报告中显示每一天的条形来实现此操作。 例如，选择时间段内调用"上周"将生成七条条形图。 每个条形都会显示当天的命中次数。
...
由小时报表允许您查看的命中数和每小时在特定平台上传输的数据的总数目。 此信息可用于快速识别 CDN 活动模式。 例如，此报表可以帮助你检测遇到更高版本低于预期流量或一天内的时间段。
...
通过浏览器报表，可查看已使用哪个浏览器请求内容。 生成此类型的报告后，饼图将表示前 10 个浏览器处理的请求的百分比。
...
通过 404 错误报表允许您标识的生成最多的 404 未找到的状态代码的内容的类型。 报告的顶部包含返回最多 404 Not Found 状态代码的前 10 个资产的条形图。 此条形图将请求总数与导致这些资产的 404 Not Found 状态代码的请求进行比较。 每个条形都进行了颜色编码。 黄色条用于表示该请求导致了 404 Not Found 状态代码。 红色条用于表示资产的请求总数。
