...
每个基于地图的报表都 (例如，国家/地区）对地理实体进行排名，提供一个地图来帮助您直观地显示您的内容请求位置。 它能够根据在该区域中经历的需求量，通过对每个区域进行颜色编码来这样做。 较浅的阴影区域表示内容的需求较低，而较暗的区域表示内容有较高的需求量。

直接在地图下方提供每个地区的详细的流量和带宽信息。 这样，可以查看命中总数、命中百分比、传输的总数据量（以千兆字节为单位）以及每个区域传输的数据的百分比。 查看每个指标的说明。 最后，当你将鼠标悬停在某个区域 (例如，国家/地区、州或省) 时，该区域中发生的命中的名称和百分比将显示为工具提示。
...
| 欧洲地图 |此报告允许查看欧洲对 CDN 内容的需求。 每个国家/地区都在此地图上进行了颜色编码，以表示源自该区域的命中的百分比。 |
| 亚太地图 |此报告允许查看亚洲对 CDN 内容的需求。 每个国家/地区都在此地图上进行了颜色编码，以表示源自该区域的命中的百分比。 |

## <a name="geography-reports-bar-charts"></a>地理报告（条形图）
还有两个额外的根据地理位置提供统计信息的报告，这两个报告分别是热门城市和热门国家/地区报告。 这些报表将分别根据源自这些国家/地区的命中数对城市和国家/地区进行排名。 生成此类型的报告后，条形图将指示通过特定平台请求内容的前10个城市或国家/地区。 此条形图可让你快速评估生成最多内容请求数的区域。
...
* 排名靠前的城市报表的工具提示按姓名、省/市/自治区和国家/地区缩写标识城市。
* 如果无法确定发起请求的城市或区域（即州/省），则它将指明它们是未知的。 如果国家/地区未知，则两个问号 (例如，？？将显示 ) 。
* 报告可能包含“欧洲”或“亚洲/太平洋地区”的指标。 这些项目并不意味着提供这些区域的所有 IP 地址的统计信息。 相反，它们仅适用于源自基于欧洲或亚洲/太平洋的 IP 地址而不是特定城市或国家/地区的请求。
