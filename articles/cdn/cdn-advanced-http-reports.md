...
每个基于地图的报告都会对地理实体（即国家/地区）进行排名，并为您提供地图，以帮助您可视化请求内容的位置。 它能够根据在该区域中经历的需求量，通过对每个区域进行颜色编码来这样做。 较浅的阴影区域表示内容的需求较低，而较暗的区域表示内容有较高的需求量。

直接在地图下方提供每个地区的详细的流量和带宽信息。 这样，可以查看命中总数、命中百分比、传输的总数据量（以千兆字节为单位）以及每个区域传输的数据的百分比。 查看每个指标的说明。 最后，当您将鼠标悬停在区域（即国家/地区、州或省）上时，该区域中发生的命中的名称和百分比将显示为工具提示。

下面为每种类型的基于地图的地理报告提供简要说明。

| 报表名称 | 描述 |
| --- | --- |
| 世界地图 |此报告允许查看全球对 CDN 内容的需求。 每个国家/地区在世界地图上都进行了颜色编码，以指示来自该区域的命中百分比。 |
| 美国地图 |此报告允许查看美国对 CDN 内容的需求。 每个州都在此地图上进行了颜色编码，以表示源自该区域的命中数的百分比。 |
| 加拿大地图 |此报告允许查看加拿大对 CDN 内容的需求。 每个省都在此地图上进行了颜色编码，以表示源自该区域的命中数的百分比。 |
| 欧洲地图 |此报告允许查看欧洲对 CDN 内容的需求。 此地图上对每个国家/地区进行颜色编码，以指示源自该区域的热门点击百分比。 |
| 亚太地图 |此报告允许查看亚洲对 CDN 内容的需求。 此地图上对每个国家/地区进行颜色编码，以指示源自该区域的热门点击百分比。 |

## <a name="geography-reports-bar-charts"></a>地理报告（条形图）
还有两个额外的根据地理位置提供统计信息的报告，这两个报告分别是热门城市和热门国家/地区报告。 这些报告根据来自这些国家/地区的点击次数分别对城市和国家/地区进行排名。 生成此类报告后，条形图将指示通过特定平台请求内容的前 10 个城市或国家/地区。 此条形图可让你快速评估生成最多内容请求数的区域。
...
* "热门城市"报表的工具提示按城市的名称、州/省和国家/地区缩写标识城市。
* 如果无法确定发起请求的城市或区域（即州/省），则它将指明它们是未知的。 如果国家/地区未知，则将显示两个问号（即？？？）。
* 报告可能包含“欧洲”或“亚洲/太平洋地区”的指标。 这些项目并不意味着提供这些区域的所有 IP 地址的统计信息。 相反，它们仅适用于源自分布在欧洲或亚太地区的 IP 地址的请求，而不是特定城市或国家/地区的请求。
...
| 热门国家/地区 |此报告根据源自该国/地区的点击次数对国家/地区进行排名。 |
