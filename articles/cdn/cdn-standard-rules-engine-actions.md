...
操作 | HTTP 标头名称 | “值”
-------|------------------|------
追加 | 如果选择此选项并且规则匹配，则 "**标头名称**" 中指定的标头将添加到具有指定值的请求。 如果标头已存在，则将值追加到现有值。 | String
Overwrite | 如果选择此选项并且规则匹配，则 "**标头名称**" 中指定的标头将添加到具有指定值的请求。 如果标头已存在，则指定的值将覆盖现有值。 | String
...
操作 | HTTP 标头名称 | “值”
-------|------------------|------
追加 | 如果选择此选项并且规则匹配，则使用指定的**值**将 "**标头名称**" 中指定的标头添加到响应中。 如果标头已存在，则将**值**追加到现有值。 | String
Overwrite | 如果选择此选项并且规则匹配，则使用指定的**值**将 "**标头名称**" 中指定的标头添加到响应中。 如果标头已存在，则**值**将覆盖现有值。 | String
...
Fragment | 定义要在重定向中使用的片段。 留空以保留传入片段。 
