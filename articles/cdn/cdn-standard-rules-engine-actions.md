...
替代 | 如果选择此选项并且规则匹配，则从你的来源返回的 TTL 值将被该操作中指定的值覆盖。
...
操作 | HTTP 标头名称 | 值
-------|------------------|------
附加 | 如果选择此选项并且规则匹配，则 "**标头名称**" 中指定的标头将添加到具有指定值的请求。 如果标头已存在，则将值追加到现有值。 | 字符串
Overwrite | 如果选择此选项并且规则匹配，则 "**标头名称**" 中指定的标头将添加到具有指定值的请求。 如果标头已存在，则指定的值将覆盖现有值。 | 字符串
删除 | 如果选择此选项，则规则匹配，并且在规则中指定的标头存在，则将从请求中删除标头。 | 字符串
...
操作 | HTTP 标头名称 | 值
-------|------------------|------
附加 | 如果选择此选项并且规则匹配，则使用指定的**值**将 "**标头名称**" 中指定的标头添加到响应中。 如果标头已存在，则将**值**追加到现有值。 | 字符串
Overwrite | 如果选择此选项并且规则匹配，则使用指定的**值**将 "**标头名称**" 中指定的标头添加到响应中。 如果标头已存在，则**值**将覆盖现有值。 | 字符串
删除 | 如果选择此选项，则规则匹配，并且在规则中指定的标头存在，则将从响应中删除标头。 | 字符串
