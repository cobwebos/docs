...
覆盖 | 如果选择此选项并且规则匹配，则 "**标头名称**" 中指定的标头将添加到具有指定值的请求。 如果标头已存在，则指定的值将覆盖现有值。 | 字符串
...
覆盖 | 如果选择此选项并且规则匹配，则使用指定的**值**将 "**标头名称**" 中指定的标头添加到响应中。 如果标头已存在，则**值**将覆盖现有值。 | 字符串
...
字段 | 描述 
------|------------
Type | 选择要返回到请求者的响应类型：找到（302）、已移动（301）、临时重定向（307）和永久重定向（308）。
协议 | 匹配请求、HTTP、HTTPS。
