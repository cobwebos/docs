...
Data Lake Store 的加密是在帐户创建过程中设置的，默认情况下始终启用。 你可以自行管理密钥，也可以让 Data Lake Store 为你进行管理（此为默认设置）。
...
|能否通过 Key Vault 检索 MEK？|不能。 存储在 Key Vault 中以后，MEK 就只能用于加密和解密。|不能。 存储在 Key Vault 中以后，MEK 就只能用于加密和解密。|
|谁拥有 Key Vault 实例和 MEK？|Data Lake Store 服务|你拥有 Key Vault 实例，该实例属于你自己的 Azure 订阅。 Key Vault 中的 MEK 可以通过软件或硬件进行管理。|
|你能否撤消对 Data Lake Store 服务的 MEK 的访问权限？|否|是的。 你可以管理 Key Vault 中的访问控制列表，删除 Data Lake Store 服务的服务标识的访问控制项。|
|你能否永久删除 MEK？|否|是的。 如果你从 Key Vault 中删除 MEK，则任何人（包括 Data Lake Store 服务）都不能解密 Data Lake Store 帐户中的数据。 <br><br> 如果你在将 MEK 从 Key Vault 中删除以前对其进行了显式备份，则可将 MEK 还原，然后对数据进行恢复。 但是，如果你在将 MEK 从 Key Vault 中删除以前未对其进行备份，则再也不能在以后解密 Data Lake Store 帐户中的数据。|
...
| 键                   | 缩写 | 关联项 | 存储位置                             | 类型       | 说明                                                                                                   |
|-----------------------|--------------|-----------------|----------------------------------------------|------------|---------------------------------------------------------------------------------------------------------|
| 主加密密钥 | MEK          | Data Lake Store 帐户 | 密钥保管库                              | 非对称 | 可以由 Data Lake Store 或你来管理。                                                              |
...
你在设置 Data Lake Store 帐户时已选择使用自己的密钥。 创建帐户后，无法更改此选项。 以下步骤假设你使用客户管理的密钥（即，你已从 Key Vault 选择自己的密钥）。

请注意，如果使用默认的加密选项，则始终通过 Data Lake Store 管理的密钥加密数据。 在该选项中，你不能轮换密钥，因为密钥是 Data Lake Store 管理的。
