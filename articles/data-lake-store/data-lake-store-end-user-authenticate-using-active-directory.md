...
如果希望最终用户能够登录到通过 Azure AD 应用程序，此身份验证机制是建议的方法。 然后，应用程序可访问 Azure 资源，且访问权限级别与已登录的最终用户相同。 最终用户需要定期提供凭据，以使应用程序可继续访问。

让最终用户登录的结果是你的应用程序授予访问令牌和刷新令牌。 访问令牌会附加到对 Data Lake Storage Gen1 或 Data Lake Analytics 作出的每个请求，默认情况下一小时内有效。 刷新令牌可用于获取新的访问令牌，默认情况下两周内有效。 最终用户登录，可以使用两种不同方法。
