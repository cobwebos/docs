...
|            |    文件     |   Folder |
|------------|-------------|----------|
| **读取 (R)** | 可以读取文件内容 | 需要**读取****和执行**来列出文件夹的内容|
...
如访问检查算法所示，掩码限制**命名用户**、**拥有组**和**命名组**的访问。  
...
创建文件或文件夹时，umask 用于修改默认 ACL 在子项上的设置方式。 umask 是父文件夹上的 9 位值，其中包含用于**拥有用户**、**拥有组****和其他**的 RWX 值。

Azure 数据存储第 1 代的 umask 是设置为 007 的常量值。 此值将转换为
...
> [!NOTE]
> 不需要有“写入”权限即可删除文件夹中的文件。 此外，**无法**删除根文件夹"/"。
>
>
