<properties 
   pageTitle="通过 REST API 开始使用 Data Lake Store | Microsoft Azure" 
...
# 通过 REST API 开始使用 Azure Data Lake Store
...
>[AZURE.NOTE] 有关 Data Lake Store 的 REST API 支持的详细信息，请参阅 [Azure Data Lake Store REST API Reference](https://msdn.microsoft.com/library/mt693424.aspx)（Data Lake Store REST API 参考）。

## 先决条件

- **一个 Azure 订阅**。请参阅[获取 Azure 免费试用版](https://azure.microsoft.com/pricing/free-trial/)。
- **创建 Azure Active Directory 应用程序**。使用 Azure Active Directory 进行身份验证的方法有两种：**交互式**和**非交互式**。根据身份验证方式的不同，有不同的先决条件。
	* **交互式身份验证**（本文中使用的身份验证方式）- 必须在 Azure Active Directory 中创建一个**本机客户端应用程序**。创建应用程序后，检索以下与应用程序相关的值。
		- 为应用程序获取**客户端 ID** 和**重定向 URI**
		- 设置委派权限

	* **非交互式身份验证** - 必须在 Azure Active Directory 中创建一个 **Web 应用程序**。创建应用程序后，检索以下与应用程序相关的值。
		- 为应用程序获取**客户端 ID**、**客户端密码**和**重定向 URI**
		- 设置委派权限
		- 将 Azure Active Directory 应用程序分配到某个角色。该角色可以在要对 Azure Active Directory 应用程序授予权限的范围级别。例如，可以将应用程序分配至订阅级别或资源组级别。有关说明，请参阅 [Assign application to a role](../resource-group-create-service-principal-portal.md#assign-application-to-role)（将应用程序分配到角色）。

	有关如何检索这些值、设置权限以及分配角色的说明，请参阅 [Create Active Directory application and service principal using portal](../resource-group-create-service-principal-portal.md)（使用门户创建 Active Directory 应用程序和服务主体）。
...
1. 通过应用程序将用户重定向至以下 URL：
	...
	>[AZURE.NOTE] <REDIRECT-URI> 需要经过编码才能在 URL 中使用。因此，对于 https://localhost，请使用 `https%3A%2F%2Flocalhost`）
...
2. 捕获响应中的授权代码。在本教程中，可以从 Web 浏览器的地址栏复制授权代码，然后在 POST 请求中将其传递给令牌终结点，如下所示：
	...
	>[AZURE.NOTE] 在此情况下，<REDIRECT-URI> 不需要编码。
...
- [Open Source Big Data applications compatible with Azure Data Lake Store](data-lake-store-compatible-oss-other-applications.md)（与 Azure Data Lake Store 兼容的开源大数据应用程序）
