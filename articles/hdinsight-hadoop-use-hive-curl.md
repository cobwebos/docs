...
> 
> REST API 通过<a href="http://en.wikipedia.org/wiki/Basic_access_authentication" target="_blank">基本身份验证</a>进行保护。你始终应该使用安全 HTTP (HTTPS) 来发出请求，以确保安全地将凭据发送到服务器。
...
2. 使用以下命令创建名为 **log4jLogs** 的新表：
    ...
    * **-d** - 由于未使用 `-G`，请求将默认为使用 POST 方法。`-d` 指定与请求一起发送的数据值。
...
4. 在作业的状态更改为 **SUCCEEDED** 后，你可以从 Azure Blob 存储中检索作业的结果。随查询一起传递的  `statusdir` 参数包含输出文件的位置；在这种情况下是 **wasb:///example/curl**。此地址会将作业的输出存储在 HDInsight 群集所用的默认存储容器的 **example/curl** 目录中。

    你可以使用 <a href="/documentation/articles/xplat-cli/" target="_blank">Azure 跨平台命令行界面 (xplat-cli)</a> 列出并下载这些文件。例如，若要列出 **example/curl** 中的文件，请使用以下命令：
	...
	> [AZURE.NOTE] 你必须使用 `-a` 和 `-k` 参数指定包含 Blob 的存储帐户名称，或者设置 **AZURE\_STORAGE\_ACCOUNT** 和 **AZURE\_STORAGE\_ACCESS\_KEY** 环境变量。请参阅<a href="/documentation/articles/hdinsight-upload-data/" target="_blank" for more information.
...
6. 使用以下语句创建名为  **errorLogs**: 的新"内部"表
    ...
    * **SELECT** - 从新的 **errorLogs** 表中选择所有行。

7. 使用返回的作业 ID 检查作业的状态。在成功后，按前面所述使用 xplat-cli 下载并查看结果。输出应包含三行，其中所有行都包含 **[ERROR]**.
