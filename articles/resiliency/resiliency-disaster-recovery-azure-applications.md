...
在下图中，用户连接到为流量管理器指定的 URL (__http://myATMURL.trafficmanager.net__)，后者将真实的站点 URL（__http://app1URL.cloudapp.net__ 和 __http://app2URL.cloudapp.net__)）进行了抽象化。当策略发出指令时，将根据配置何时路由用户的条件，将用户发送到正确的真实站点。策略选项为轮循机制、性能或故障转移。在本文中，我们只讨论故障转移选项。
