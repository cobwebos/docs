---
title: Azure VMware 解决方案（AVS）-监视 AVS 私有云活动
description: 介绍 AVS 环境的 Azure VMware 解决方案中的活动可用的信息，包括警报、事件、任务和审核。
...
---
# <a name="monitor-vmware-solutions-avs-activity"></a>监视 VMware 解决方案（AVS）活动

AVS 活动日志提供对 AVS 门户上完成的操作的见解。 此列表包括警报、事件、任务和审核。 使用活动日志来确定执行时间和执行哪些操作。 活动日志不包括用户完成的任何读取操作。
...
警报是你的 AVS 环境中任何重要活动的通知。 警报包括影响计费或用户访问的事件。
...
| 时间 | 警报发生的时间。 |
| 严重性 | 警报的重要性。|
| 资源名称 | 分配给资源的名称，如 AVS 私有云名称。 |
| 资源类型 | 资源类别： AVS 私有云、云架。 |
| 资源 ID | 资源的标识符。 |
| Description | 触发警报的说明。 |
| 已确认 | 指示是否确认了警报。 |
...
事件显示 AVS 门户上的用户和系统活动。 "事件" 页列出与特定资源关联的活动和影响的严重性。
...
任务是一种 AVS 私有云活动，预计需要30秒或更长时间才能完成。 （预计时间不到30秒的活动仅作为事件报告。）打开 "任务" 页，跟踪你的 AVS 私有云的任务进度。
