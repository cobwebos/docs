---
title: Azure VMware 解决方案（AVS）-使用 VMware Site Recovery Manager 将 AVS 私有云设置为灾难恢复站点
...
可以使用 AVS 私有云作为本地 VMware 工作负荷的灾难恢复（DR）站点。

DR 解决方案基于 vSphere 复制和 VMware Site Recovery Manager （SRM）。 可以遵循类似的方法，使你的 AVS 私有云成为受本地恢复站点保护的主站点。
...
* 在 AVS 私有云中安装、配置和管理 vSphere 复制和 SRM。
* 当 AVS 私有云为受保护的站点时，为 SRM 提供自己的许可证。 当 AVS 站点用作恢复站点时，无需任何额外的 SRM 许可证。

使用此解决方案，你可以完全控制 vSphere 复制和 SRM。 熟悉的 UI、API 和 CLI 接口允许使用现有的脚本和工具。

![Site Recovery 管理器部署](media/srm-deployment.png)

你可以使用与你的 AVS 私有云和本地环境兼容的任何 vRA 和 SRM 版本。 本指南中的示例使用 vRA 6.5 和 SRM 6.5。 这些版本与 AVS 支持的 vSphere 6.5 兼容。

## <a name="deploy-the-solution"></a>部署解决方案

以下部分介绍如何在你的 AVS 私有云中使用 SRM 部署灾难恢复解决方案。
...
3. [为环境创建 AVS 私有云](#create-an-avs-private-cloud-for-your-environment)
4. [为 SRM 解决方案设置 AVS 私有云网络](#set-up-avs-private-cloud-networking-for-the-srm-solution)
5. [设置本地网络与 AVS 私有云之间的站点到站点 VPN 连接并打开所需的端口](#set-up-a-site-to-site-vpn-connection-between-your-on-premises-network-and-the-avs-private-cloud-and-open-required-ports)
6. [在 AVS 私有云中设置基础结构服务](#set-up-infrastructure-services-in-your-avs-private-cloud)
7. [在本地环境中安装 vSphere 复制设备](#install-vsphere-replication-appliance-in-your-on-premises-environment)
8. [在 AVS 私有云环境中安装 vSphere 复制设备](#install-vsphere-replication-appliance-in-your-avs-private-cloud-environment)
9. [在本地环境中安装 SRM 服务器](#install-srm-server-in-your-on-premises-environment)
10. [在 AVS 私有云中安装 SRM 服务器](#install-srm-server-in-your-avs-private-cloud)

### <a name="verify-that-vmware-product-versions-are-compatible"></a>验证 VMware 产品版本是否兼容

本指南中的配置服从以下兼容性要求：

* 同一版本的 SRM 必须部署在你的 AVS 私有云和本地环境中。
* 相同版本的 vSphere 复制必须部署在你的 AVS 私有云和本地环境中。
* 你的 AVS 私有云和本地环境中的平台服务控制器（PSC）版本必须兼容。
* 你的 AVS 私有云和本地环境中的 vCenter 版本必须兼容。
...
若要查看你的 AVS 私有云中的 vCenter 和 PSC 版本，请打开 AVS 门户。 中转到 "**资源**"，选择你的 AVS 私有云，然后单击 " **vSphere 管理网络**" 选项卡。

![vCenter & AVS 私有云中的 PSC 版本](media/srm-resources.png)
...
3. 使用 AVS sizer 工具估算灾难恢复站点所需的资源，以保护本地环境。

### <a name="create-an-avs-private-cloud-for-your-environment"></a>为环境创建 AVS 私有云

按照[创建 Avs 私有云中](create-private-cloud.md)的说明和大小建议，从 AVS 门户创建 Avs 私有云。

### <a name="set-up-avs-private-cloud-networking-for-the-srm-solution"></a>为 SRM 解决方案设置 AVS 私有云网络

访问 AVS 门户，为 SRM 解决方案设置 AVS 私有云网络。

为 SRM 解决方案网络创建 VLAN，并为其分配一个子网 CIDR。 有关说明，请参阅[创建和管理 vlan/子网](create-vlan-subnet.md)。

### <a name="set-up-a-site-to-site-vpn-connection-between-your-on-premises-network-and-the-avs-private-cloud-and-open-required-ports"></a>设置本地网络与 AVS 私有云之间的站点到站点 VPN 连接并打开所需的端口

设置本地网络与 AVS 私有云之间的站点到站点连接。 有关说明，请参阅[配置与 AVS 私有云的 VPN 连接](set-up-vpn.md)。

### <a name="set-up-infrastructure-services-in-your-avs-private-cloud"></a>在 AVS 私有云中设置基础结构服务

在 AVS 私有云中配置基础结构服务，便于管理工作负荷和工具。

如果你想要执行以下任一操作，则可以添加一个外部标识提供者，如[使用 Azure AD 作为 AVS 私有云上的 vCenter 的标识提供者](azure-ad.md)中所述：

* 在你的 AVS 私有云中标识本地 Active Directory （AD）中的用户。
* 在你的 AVS 私有云中为所有用户设置一个广告。
* 使用 Azure AD。
...
有关说明，请参阅[配置与 AVS 私有云的 VPN 连接](set-up-vpn.md)。 步骤类似于本地安装的步骤。

在 vRA 和 SRM 安装期间，AVS 建议使用 Fqdn 而不是 IP 地址。 若要查看你的 AVS 私有云中的 vCenter 和 PSC 的 FQDN，请打开 AVS 门户。 中转到 "**资源**"，选择你的 AVS 私有云，然后单击 " **vSphere 管理网络**" 选项卡。

![查找 AVS 私有云中的 vCenter/PSC 的 FQDN](media/srm-resources.png)

AVS 要求您不要使用默认的 "cloudowner" 用户安装 vRA 和 SRM，而是创建一个新用户。 这样做是为了帮助确保你的 AVS 私有云 vCenter 环境的正常运行时间和可用性。 但是，AVS 私有云 vCenter 中的默认 cloudowner 用户没有足够的权限来创建具有管理权限的新用户。

在安装 vRA 和 SRM 之前，必须升级 cloudowner 用户的 vCenter 权限，然后在 vCenter SSO 域中创建具有管理权限的用户。 有关默认 AVS 私有云用户和权限模型的详细信息，请参阅[了解 AVS 私有云权限模型](learn-private-cloud-permissions.md)。
...
6. 在 AVS 私有云网站上，通过 vCenter 单一登录注册 AVS 私有 Cloud vRA。
...
#### <a name="vcenter-ui-create-a-user-in-the-avs-private-cloud-for-vra-and-srm-installation"></a>vCenter UI：在适用于 vRA 和 SRM 安装的 AVS 私有云中创建用户

从 AVS 门户提升权限后，使用 cloudowner 用户凭据登录到 vCenter。

在 vCenter 中创建新用户 `srm-soln-admin`，并将其添加到 vCenter 中的 administrators 组。
以 cloudowner 用户的身份注销 vCenter，并以*srm soln*用户身份登录。

#### <a name="avs-portal-configure-firewall-rules-for-vra"></a>AVS 门户：配置 vRA 的防火墙规则
...
### <a name="install-srm-server-in-your-avs-private-cloud"></a>在 AVS 私有云中安装 SRM 服务器
...
以下步骤介绍了 AVS 私有云 SRM 安装。

1. [vCenter UI：安装 SRM](#vcenter-ui-install-srm)
2. [AVS 门户：为 SRM 配置防火墙规则](#avs-portal-configure-firewall-rules-for-srm)
3. [vCenter UI：配置 SRM](#vcenter-ui-configure-srm)
4. [AVS 门户：取消提升权限](#avs-portal-de-escalate-privileges)
...
#### <a name="avs-portal-configure-firewall-rules-for-srm"></a>AVS 门户：为 SRM 配置防火墙规则

按照[设置防火墙表和规则](firewall.md)中所述配置防火墙规则，以允许在以下各项之间进行通信：

AVS 私有云中的 SRM 服务器和 vCenter/PSC。
两个站点上的 SRM 服务器
...
将 SRM 安装到 AVS 私有云中后，请按照 VMware Site Recovery 管理器安装和配置指南部分中所述执行以下任务。 对于 SRM 6.5，[安装 Site Recovery Manager](https://docs.vmware.com/en/Site-Recovery-Manager/6.5/com.vmware.srm.install_config.doc/GUID-437E1B65-A17B-4B4B-BA5B-C667C90FA418.html)的 VMware 文档中提供了相关说明。

1. 将 Site Recovery Manager 服务器实例连接到受保护的和恢复站点。
2. 建立与远程 Site Recovery Manager 服务器实例的客户端连接。
3. 安装 Site Recovery Manager 许可证密钥。

#### <a name="avs-portal-de-escalate-privileges"></a>AVS 门户：取消提升权限
...
你可以完全控制 AVS 私有云环境中的 vSphere 复制和 SRM 软件，并且应执行必要的软件生命周期管理。 在更新或升级 vSphere 复制或 SRM 之前，请确保软件的任何新版本都与 AVS 私有云 vCenter 和 PSC 兼容。

> [!NOTE]
> AVS 目前正在探索用于提供托管 DR 服务的选项。 

## <a name="multiple-replication-configuration"></a>多个复制配置

 [基于阵列的复制和 vSphere 复制技术可以同时与 SRM 一起使用](https://blogs.vmware.com/virtualblocks/2017/06/22/srm-array-based-replication-vs-vsphere-replication)。 但是，必须将它们应用到不同的 Vm 集（可以通过基于阵列的复制或 vSphere 复制来保护给定 VM，但不能同时使用两者）。 此外，可以将 AVS 网站配置为多个受保护站点的恢复站点。 有关多站点配置的详细信息，请参阅[SRM 多站点选项](https://blogs.vmware.com/virtualblocks/2016/07/28/srm-multisite/)。
