---
title: Azure VMware 解决方案（AVS）-为 Oracle RAC 优化 AVS 私有云
...
ms.locfileid: "77016011"
---
# <a name="optimize-your-avs-private-cloud-for-installing-oracle-rac"></a>优化你的 AVS 私有云以安装 Oracle RAC

可以在 AVS 私有云环境中部署 Oracle 真实应用程序群集（RAC）。 本指南介绍了如何部署新群集并优化 Oracle RAC 解决方案的 VM。 完成本主题中的步骤后，即可安装和配置 Oracle RAC。
...
以下部分介绍如何设置适用于 Oracle RAC 的 AVS 私有云环境。

1. 磁盘配置最佳做法
2. 部署 AVS 私有云 vSphere 群集
3. 为 Oracle RAC 设置网络
4. 设置 vSAN 存储策略
...
## <a name="deploy-avs-private-cloud-vsphere-cluster"></a>部署 AVS 私有云 vSphere 群集

若要在 AVS 私有云上部署 vSphere 群集，请遵循以下过程：

1. 在 AVS 门户中[创建一个 Avs 私有云](create-private-cloud.md)。 AVS 在新创建的 AVS 私有云中创建一个名为 "cloudowner" 的默认 vCenter 用户。 有关默认 AVS 私有云用户和权限模型的详细信息，请参阅[了解 AVS 私有云权限模型](learn-private-cloud-permissions.md)。 此步骤为你的 AVS 私有云创建主管理群集。

2. 在 AVS 门户中，使用新群集[展开 AVS 私有云](expand-private-cloud.md)。 此群集将用于部署 Oracle RAC。 根据所需的容错容错（最少三个节点）选择节点数。

## <a name="set-up-networking-for-oracle-rac"></a>为 Oracle RAC 设置网络

1. 在你的 AVS 私有云中，[创建两个 vlan](create-vlan-subnet.md)，一个用于 oracle 公用网络，另一个用于 oracle 专用网络并分配适当的子网 CIDRs。
2. 创建 Vlan 后，[在 AVS 私有云 vCenter 上创建分布式端口组](create-vlan-subnet.md#use-vlan-information-to-set-up-a-distributed-port-group-in-vsphere)。
3. 在管理群集上为 Oracle 环境设置[DHCP 和 DNS 服务器虚拟机](dns-dhcp-setup.md)。
4. 在安装在 AVS 私有云中[的 dns 服务器上配置 dns 转发](on-premises-dns-setup.md#create-a-conditional-forwarder)。

## <a name="set-up-vsan-storage-policies"></a>设置 vSAN 存储策略

vSAN 策略为 VM 磁盘上存储的数据定义允许和磁盘条带化失败。 创建 VM 时，必须在 VM 磁盘上应用创建的存储策略。

1. 登录到你的 AVS 私有云的[vSphere 客户端](https://docs.azure.cloudsimple.com/vsphere-access)。
2. 在顶部菜单中，选择 "**策略和配置文件**"。
...
1. 在 AVS 门户中，[升级](escalate-private-cloud-privileges.md)cloudowner 用户的权限。
2. 登录到你的 AVS 私有云的[vSphere 客户端](https://docs.azure.cloudsimple.com/vsphere-access)。
