---
title: Azure VMware 解决方案（AVS）-在适用于 vRealize 自动化的 AVS 私有云上设置 vCenter
description: 介绍如何在 AVS 私有云上将 VMware vCenter 服务器设置为 VMware vRealize Automation 的终结点
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/19/2019
ms.topic: article
...
---
# <a name="set-up-vcenter-on-your-avs-private-cloud-for-vmware-vrealize-automation"></a>在你的 AVS 私有云上为 VMware vRealize Automation 设置 vCenter

你可以将你的 AVS 私有云上的 VMware vCenter 服务器设置为 VMware vRealize Automation 终结点。

## <a name="before-you-begin"></a>开始之前

在配置 vCenter 服务器之前完成以下任务：

* 配置本地环境与 AVS 私有云之间的[站点到站点 VPN 连接](vpn-gateway.md#set-up-a-site-to-site-vpn-gateway)。
* [将本地 dns 请求的 dns 转发](on-premises-dns-setup.md)到你的 AVS 私有云的 dns 服务器。
...
2. 为 vRealize 自动化终结点部署 vSphere 代理。
    ...
    安装程序文件的命名约定是 setup_*vra*@5480.exe。
    ...
    7. 输入 IaaS 服务器登录信息，然后单击 "**下一步**"。 如果使用 Active Directory，请以**domain\user**格式输入用户名。 否则，请使用 **user@domain** 格式。
    ...
    10. 输入 AVS 私有云的 vCenter 服务器终结点的名称。 记录名称以供稍后在配置过程中使用。
...
2. 选择 > **终结点**的**基础结构** > **终结**点。
3. 选择 "**新建**" > **虚拟** > **vSphere**"。
4. 输入在前面的过程中指定的 vSphere 终结点名称。
5. 对于 "**地址**"，以 https://*vcenter-fqdn*/Sdk 格式输入 AVS 私有云 vCenter Server URL，其中*vcenter-fqdn*是 vcenter 服务器的名称。
6. 输入 AVS 支持为你创建的 vRealize Automation IaaS 管理用户的凭据。
...
输出如下所示。 "`managementEndpointName`" 字段的值为终结点名称。
