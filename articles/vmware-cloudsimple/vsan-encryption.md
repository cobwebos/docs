---
title: Azure VMware 解决方案（AVS）-配置适用于 AVS 私有云的 vSAN 加密
description: 介绍如何配置 vSAN 软件加密功能，以便你的 AVS 私有云可以使用 Azure 虚拟网络中运行的密钥管理服务器。
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/19/2019
ms.topic: article
...
---
# <a name="configure-vsan-encryption-for-avs-private-cloud"></a>配置 AVS 私有云的 vSAN 加密

你可以配置 vSAN 软件加密功能，使你的 AVS 私有云可以使用 Azure 虚拟网络中运行的密钥管理服务器。
...
* 为 KMS 工具提供自己的许可证。
* 使用 Azure 虚拟网络中运行的第三方 KMS 工具在 AVS 私有云中配置和管理 vSAN 加密。

## <a name="kms-deployment-scenario"></a>KMS 部署方案

KMS 服务器群集在 Azure 虚拟网络中运行，可通过配置的 Azure ExpressRoute 连接从 AVS 私有云 vCenter 访问 IP。
...
2. [AVS 门户：获取 ExpressRoute 对等互连信息](#avs-portal-obtain-expressroute-peering-information)
3. [Azure 门户：将虚拟网络连接到 AVS 私有云](#azure-portal-connect-your-virtual-network-to-the-avs-private-cloud)
...
### <a name="avs-portal-obtain-expressroute-peering-information"></a>AVS 门户：获取 ExpressRoute 对等互连信息

若要继续安装，需要提供 authorization 密钥和对等线路 URI，以便 ExpressRoute 以及对 Azure 订阅的访问权限。 此信息可在 AVS 门户的 "虚拟网络连接" 页面上找到。 有关说明，请参阅[设置到 AVS 私有云的虚拟网络连接](virtual-network-connection.md)。 如果在获取信息时遇到任何问题，请[提出支持请求](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade/newsupportrequest)。
...
2. 按照[使用门户将虚拟网络连接到 ExpressRoute 线路](../expressroute/expressroute-howto-linkvnet-portal-resource-manager.md)中的说明，将虚拟网络链接到 AVS expressroute 线路。
3. 使用来自 AVS 的欢迎电子邮件中收到的 AVS ExpressRoute 线路信息，将虚拟网络链接到 Azure 中的 AVS ExpressRoute 线路。
...
6. 当在 SSH shell 中出现提示时，选择 "`No`" 以将节点设置为初始 KeyControl 节点。
7. 通过重复此过程的步骤3-5 添加其他 KeyControl 节点，并在系统提示添加到现有群集时选择 `Yes`。
