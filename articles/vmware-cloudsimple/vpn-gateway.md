---
title: Azure VMware 解决方案（AVS）-设置 VPN 网关
description: 描述如何设置点到站点 VPN 网关和站点到站点 VPN 网关，并创建本地网络与 AVS 私有云之间的连接
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/14/2019
ms.topic: article
...
ms.locfileid: "77016759"
---
# <a name="set-up-vpn-gateways-on-avs-network"></a>在 AVS 网络上设置 VPN 网关

使用 VPN 网关，可以从本地网络和客户端计算机远程连接到 AVS 网络。 本地网络与 AVS 网络之间的 VPN 连接可用于访问 AVS 私有云上的 vCenter 和工作负荷。 AVS 支持站点到站点 VPN 和点到站点 VPN 网关。

## <a name="vpn-gateway-types"></a>VPN 网关类型

* 使用**站点到站点 VPN**连接，可以设置你的 AVS 私有云工作负荷以访问本地服务。 你还可以使用本地 Active Directory 作为标识源，以便向你的 AVS 私有云 vCenter 进行身份验证。 目前仅支持**基于策略的 VPN**类型。
* **点到站点 VPN**连接是从计算机连接到 AVS 私有云的最简单方法。 使用点到站点 VPN 连接远程连接到 AVS 私有云。 有关为点到站点 VPN 连接安装客户端的信息，请参阅[配置与 AVS 私有云的 VPN 连接](set-up-vpn.md)。

在某个区域中，你可以创建一个点到站点 VPN 网关和一个站点到站点 VPN 网关。

## <a name="automatic-addition-of-vlansubnets"></a>自动添加 VLAN/子网

AVS VPN 网关提供向 VPN 网关添加 VLAN/子网的策略。 策略允许你为管理 VLAN/子网和用户定义的 VLAN/子网指定不同的规则。 管理 VLAN/子网的规则适用于你创建的任何新的 AVS 私有云。 用户定义的 Vlan/子网规则允许自动向站点到站点 VPN 网关的现有或新的 AVS 私有云添加任何新的 VLAN/子网，并为每个连接定义策略。

向 VPN 网关添加 Vlan/子网的策略适用于站点到站点 VPN 和点到站点 VPN 网关。

## <a name="automatic-addition-of-users"></a>自动添加用户

点到站点 VPN 网关可为新用户定义自动添加策略。 默认情况下，订阅的所有所有者和参与者都可以访问 AVS 门户。 只有首次启动 AVS 门户时才会创建用户。 选择**自动添加**规则后，任何新用户都可以使用点到站点 VPN 连接访问 AVS 网络。

## <a name="set-up-a-site-to-site-vpn-gateway"></a>设置站点到站点 VPN 网关

1. [访问 AVS 门户](access-cloudsimple-portal.md)，然后选择 "**网络**"。
2. 选择**VPN 网关**。
3. 单击 "**新建 VPN 网关**"。
...
5. 创建来自本地网络的第一个连接，然后单击 "**下一步**"。
    ...
    * 对于**本地前缀**，请输入将访问 AVS 网络的本地 CIDR 前缀。 您可以在创建连接时添加多个 CIDR 前缀。

    ![创建站点到站点 VPN 网关连接](media/create-vpn-gateway-s2s-connection.png)

6. 在你的 AVS 私有云网络上启用将从本地网络访问的 VLAN/子网，然后单击 "**下一步**"。

    * 若要添加管理 VLAN/子网，请启用 "**添加到 AVS 私有云的管理 vlan/子网**"。 子网需要用于 vMotion 和 vSAN 子网。
    * 若要添加 vMotion 子网，请启用 "**添加 AVS 私有云的 vmotion 网络**"。
...
5. 对于 "**连接/用户**"，指定以下设置，然后单击 "**下一步**"。
    ...
    * 若要自动允许当前和未来的所有用户通过点到站点网关访问 AVS 私有云，请选择 "**自动添加所有用户**"。 选择该选项时，将自动选择用户列表中的所有用户。 您可以通过取消选择列表中的单个用户来覆盖 "自动" 选项。
...
若要使用点到站点 VPN 进行连接，请参阅[使用点到站点 Vpn 连接到 AVS](set-up-vpn.md#connect-to-avs-using-point-to-site-vpn)。
