...
在本教程中，你将学习如何使用 Azure 流分析来实时获得数据的深入见解。Azure 流处理服务可让开发人员通过结合数据流（例如点击流、日志、设备生成的事件及历史记录或引用数据）调整动态数据的空间，以快速轻松获取深入的业务见解。由 Microsoft Azure 托管的 Azure 流分析是可完全管理的实时流计算服务，它提供内置冗余、低延迟及可缩放性，可让你在几分钟之内就立刻上手。
...
若要完成本教程，需要 Microsoft Azure 订阅。Microsoft 提供免费试用版 Microsoft Azure 服务，如下所述。
...
有关详细信息，请参阅 MSDN 中的[时间管理](https://msdn.microsoft.com/library/azure/mt582045.aspx)，以及查询中所用的[窗口化](https://msdn.microsoft.com/library/azure/dn835019.aspx)构造。
...
尽管本教程简介的涵盖范围相当大，但它绝对不是完整的说明。可以在[此处](stream-analytics-stream-analytics-query-patterns.md)找到更多使用 SAQL 语言的查询模式。
