...
ms.locfileid: "78392582"
---
# <a name="tutorial-analyze-phone-call-data-with-stream-analytics-and-visualize-results-in-power-bi-dashboard"></a>教程：通过流分析分析电话呼叫数据并在 Power BI 仪表板中显示结果
...
3. 请在编辑器中将现有查询替换为以下查询，此查询针对 5 秒时间间隔的调用数据执行自联接：
   ...
   若要检查欺诈性呼叫，可根据 `CallRecTime` 值来自联接流数据。 然后，可以查找 `CallingIMSI` 值（始发号码）相同，但 `SwitchNum` 值（来源国家/地区）不同的呼叫记录。 当对流数据使用 JOIN 操作时，该联接必须对可以及时分隔匹配行的程度施加一定限制。 由于流数据是无限的，因此请使用 **DATEDIFF** 函数在联接的 [ON](https://docs.microsoft.com/stream-analytics-query/datediff-azure-stream-analytics) 子句中指定关系的时间限制。
...
若要设置应用程序，请访问 [PowerBI-Developer-Samples](https://github.com/Microsoft/PowerBI-Developer-Samples) GitHub 存储库，然后按照“用户拥有数据”部分的说明操作（请使用 **integrate-web-app** 子部分下的重定向 URL 和主页 URL）。 由于我们使用的是“仪表板”示例，因此请使用 **GitHub 存储库**中的 [integrate-web-app](https://github.com/Microsoft/PowerBI-Developer-Samples/tree/master/User%20Owns%20Data/integrate-web-app) 示例代码。
