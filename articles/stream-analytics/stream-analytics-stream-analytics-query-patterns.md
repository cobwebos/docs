...
description: 本文介绍了在 Azure 流分析作业中有用的几种常见查询模式和设计。
...
使用**CAST**语句指定其数据类型。 请参阅[数据类型（Azure 流分析）](/stream-analytics-query/data-types-azure-stream-analytics)上受支持的数据类型的列表。
...
有关详细信息，请参阅[大小写表达式](/stream-analytics-query/case-azure-stream-analytics)。
...
**DATEDIFF**是一个特定于日期的函数，用于比较和返回两个 DateTime 字段之间的时差，有关详细信息，请参阅[日期函数](https://docs.microsoft.com/stream-analytics-query/date-and-time-functions-azure-stream-analytics)。
...
有关详细信息，请参阅[跳槽窗口](/stream-analytics-query/hopping-window-azure-stream-analytics)。
...
有关详细信息，请参阅[时间时间。](/stream-analytics-query/timestamp-by-azure-stream-analytics#over-clause-interacts-with-event-ordering)
...
有关详细信息，请参阅[COUNT（时间）](/stream-analytics-query/count-azure-stream-analytics)。
...
有关详细信息，请参阅[Azure 流分析一文的地理围栏和地理空间聚合方案](geospatial-scenarios.md)。
