...
- **处理时间**：事件到达处理系统并观察到的时间。 例如，收费站传感器看到汽车时，计算机系统需要一些时间来处理数据。

- **水印**：一个事件时间标记，指示事件已进入流处理器的点。 通过水印，系统能够指示事件引入过程的明确进度。 流式传输的性质决定了事件数据的传入永远不会停止，因此水印指示的是流式传输中某个时点的进度。
...
2. **应用程序时间**（也称为事件时间）
   ...
   生成事件时，会分配应用程序时间。应用程序时间是事件有效负载的一部分。 要按应用程序时间处理事件，请在选择查询中使用**时间戳到**子句。 如果 **"时间戳"项**不存在，则事件由到达时间处理。
...
然后，流分析通过查询规范得到开始时间。 但是，由于输入事件中转站仅按到达时间编制索引，因此系统必须将开始事件时间转换为到达时间。 系统可以从输入事件中转站中的这一时点开始处理事件。 可通过提前到达时段限制来简化转换过程。 这是开始事件时间减去 5 分钟提前到达窗口。 此计算还意味着系统丢弃所有被视为具有事件时间比到达时间更长时间的事件。
...
| **早期输入事件数** | 表示从源处提前到达的事件的数目，这些事件或已删除，或提前超过 5 分钟已调整了时间戳。 |
...
1. 这些事件进展的水印图示：
   ...
   5. 处理第十二个事件 （device3） 时，到达时间（12：27）比活动时间（12：21）早 6 分钟。 适用延迟到达策略。 对事件时间进行了调整 (12:22)，该时间高于水印时间 (12:21)，因此不再进行进一步调整。
