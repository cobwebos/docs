...
电信公司的传入呼叫数据量很大。 公司希望实时检测欺诈呼叫，以便他们可以通知客户或关闭特定数目的服务。 有这样一种 SIM 欺诈，即在同一时间出现多个同一身份发起的呼叫，但这些呼叫却位于不同的地理位置。 若要检测这种类型的欺诈行为，该公司需要检查来电记录和查找特定模式-在这种情况下，为大约在同一时间在不同国家/地区中进行的调用。 任何属于此类别的电话记录都将写入存储，以供后续分析。
...
|`SwitchNum`|用于连接呼叫的电话交换机。 对于此示例中，交换机是源的表示国家/地区 （美国、 中国、 英国、 德国或澳大利亚） 的字符串。 |
|`CallingNum`|呼叫方的电话号码。 |
...
此转换需要一个不重叠的时间窗口序列，每个窗口将具有一组可对其进行分组和聚合的离散数据。 这种类型的窗口称为“翻转窗口”。 在翻转窗口中，可以查看按分组的传入呼叫的计数`SwitchNum`，它表示发起呼叫的国家/地区。 
...
1. 在代码编辑器中将查询更改为以下内容：
    ...
    若要指定你想要使用翻转窗口，请使用[TUMBLINGWINDOW](https://msdn.microsoft.com/library/dn835055.aspx)函数，在`GROUP BY`子句。 在函数中，可以指定时间单位（从微秒到一天的任意时间）和窗口大小（单位数）。 在此示例中，翻转窗口包含 5 秒时间间隔内，因此你会为每隔 5 秒钟的调用计数收到按国家/地区。
...
若要检查这些情况，可以使用流数据的自联接基于 `CallRecTime` 值将流联接到自身。 然后，可以查找调用记录`CallingIMSI`值 （始发号码） 是相同的但`SwitchNum`值 （国家/地区的原点） 并不相同。
