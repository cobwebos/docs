<properties
	pageTitle="流分析上的 Power BI 仪表板 | Azure"
...
*	**工作区** – 在你的 Power BI 租户中选择一个可在其下创建数据集的工作区。

>	[AZURE.NOTE] 不应在 Power BI 帐户中显式创建此数据集和表。它们将在你启动流分析作业并且该作业开始将输出抽取到 Power BI 时自动创建。如果你的作业查询未返回任何结果，则不会创建该数据集和表。

*	单击**“确定”**和**“测试连接”**。现在，你的输出配置已完成。

>	[AZURE.WARNING] 另请注意，如果 Power BI 已有与你在此流分析作业中提供的数据集和表同名的数据集和表，现有数据将被覆盖。
...
如需进一步了解如何配置 Power BI 输出以及如何利用 Power BI 组，请参阅[了解流分析输出](stream-analytics-define-outputs.md "了解流分析输出")的[“Power BI”部分](stream-analytics-define-outputs.md#power-bi)。若要详细了解如何通过 Power BI 来创建仪表板，请参阅另一有用的资源：[Power BI 中的仪表板](https://powerbi.microsoft.com/documentation/powerbi-service-dashboards/)。
