...
|EventsLateArrivalMaxDelayInSeconds|整数|否|最大可容忍延迟时间，以秒为单位。 支持的范围为-1 到1814399（20.23：59：59天），-1 用于指定无限期等待。 如果该属性不存在，则将其解释为具有值-1。|
|EventsOutOfOrderMaxDelayInSeconds|整数|否|可按顺序将有序事件调整为按顺序返回的最大可容忍延迟（以秒为单位）。|
|EventsOutOfOrderPolicy|字符串|否|指示要应用于输入事件流中未按顺序到达的事件的策略。 -调整或删除|
|StreamingUnits|整数|是|指定流式处理作业使用的流式处理单位数。|
...
*使用[CI/CD npm 包部署 Azure 流分析作业](setup-cicd-vs-code.md)，在本地针对实时流输入测试流分析查询