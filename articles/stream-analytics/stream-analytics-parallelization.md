...
利用分区，可根据[分区键](https://docs.microsoft.com/azure/event-hubs/event-hubs-scalability#partitions)将数据分为多个子集。 如果按某个键对输入（例如事件中心）进行分区，强烈建议在向流分析作业添加输入时指定此分区键。 缩放流分析作业时，可利用输入和输出中的分区。 流分析作业可以并行利用和写入不同的分区，从而增加吞吐量。 

### <a name="inputs"></a>输入
所有 Azure 流分析输入都可以利用分区：
-   EventHub（如果使用兼容性级别 1.1 或更低级别，需要使用 PARTITION BY 关键字显式设置分区键）
-   IoT 中心（如果使用兼容性级别 1.1 或更低级别，需要使用 PARTITION BY 关键字显式设置分区键）
-   Blob 存储
...
1. 如果查询逻辑取决于同一个查询实例正在处理的相同键，则必须确保事件转到输入的同一个分区。 对于事件中心或 IoT 中心，这意味着事件数据必须已设置 **PartitionKey** 值。 或者，可以使用已分区的发件人。 对于 Blob 存储，这意味着事件将发送到相同的分区文件夹。 例如，按 userID 聚合数据的查询实例使用 userID 作为分区键来对输入事件中心进行分区。 但是，如果查询逻辑不需要由同一个查询实例处理相同的键，则可忽略此要求。 举例来说，简单的选择项目筛选器查询就体现了此逻辑。  

2. 下一步是对查询进行分区。 对于兼容性级别为 1.2 或更高级别（推荐）的作业，可以在输入设置中将自定义列指定为分区键，并且将对作业进行并行化操作。 兼容性级别为 1.0 或 1.1 的作业要求在所有查询步骤中使用“PARTITION BY PartitionId”。 允许采用多个步骤，但必须使用相同的键对其进行分区。 

3. 流分析中支持的大部分输出可利用分区。 如果使用不支持分区的输出类型，则作业不易并行。 对于事件中心输出，请确保将“分区键列”设置为查询中使用的同一分区键。 有关详细信息，请参阅[输出部分](#outputs)。
...
* 输入：具有 8 个分区的事件中心
* 输出：具有 8 个分区的事件中心（“分区键列”必须设置为使用“PartitionId”）
...
此查询是一个简单的筛选器。 因此，无需担心对发送到事件中心的输入进行分区。 请注意，兼容性级别低于 1.2 的作业必须包含 PARTITION BY PartitionId 子句，因此它满足前面的要求 #2。 对于输出，需要在作业中配置事件中心输出，将分区键设置为“PartitionId”。 最后一项检查是确保输入分区数等于输出分区数。
...
如果输入分区数不等于输出分区数，则无论是什么查询，拓扑不易并行。 但是，我们仍然可以实现一定程度的并行化。
...
默认情况下，兼容性级别 1.2 或以上支持并行查询执行。 例如，只要将“TollBoothId”列设置为输入分区键，就可对上一部分中的查询进行分区。 不需要 PARTITION BY PartitionId 子句。
...
虽然[易并行](#embarrassingly-parallel-jobs)作业很有必要，但不足以大规模维持更高的吞吐量。 每个存储系统及其对应的流分析输出在如何尽量实现最大写入吞吐量的方面各有不同。 对于任何大规模方案，某些挑战都可以通过使用正确的配置得以解决。 本部分讨论几种常见输出的配置，并提供几个引入速率为每秒 1K、5K 和 10K 个事件的示例。

以下观测数据使用包含无状态 (passthrough) 查询的流分析作业，该查询是一个基本的 JavaScript UDF，可写入到事件中心、Azure SQL DB 或 Cosmos DB。
...
[事件中心](https://github.com/Azure-Samples/streaming-at-scale/tree/master/eventhubs-streamanalytics-eventhubs)解决方案在流单元 (SU) 和吞吐量方面可实现线性缩放，是分析和流式传输流分析数据最高效的方式。 作业可纵向扩展到 192 SU，这大致相当于处理速率高达 200 MB/秒，即每天可处理 19 万亿个事件。
...
[Azure SQL](https://github.com/Azure-Samples/streaming-at-scale/tree/master/eventhubs-streamanalytics-azuresql) 支持并行写入（称为继承分区），但此功能默认情况下未启用。 但是，结合使用继承分区和完全并行查询可能不足以实现更高的吞吐量。 SQL 写入吞吐量在很大程度上取决于 SQL Azure 数据库配置和表架构。 [SQL 输出性能](./stream-analytics-sql-output-perf.md)一文详细提供了可最大程度地提高写入吞吐量的参数。 如[从 Azure 流分析输出到 Azure SQL 数据库](./stream-analytics-sql-output-perf.md#azure-stream-analytics)一文中所述，此解决方案无法作为完全并行管道以线性方式扩展到 8 个以上的分区，并且可能需要在 SQL 输出之前重新分区（请参阅 [INTO](https://docs.microsoft.com/stream-analytics-query/into-azure-stream-analytics#into-shard-count)）。 需要高级 SKU 来维持高 IO 速率，同时，每隔几分钟就会产生日志备份的开销。
...
流分析的 [Cosmos DB](https://github.com/Azure-Samples/streaming-at-scale/tree/master/eventhubs-streamanalytics-cosmosdb) 输出已更新为对[兼容性级别 1.2](./stream-analytics-documentdb-output.md#improved-throughput-with-compatibility-level-12)使用本机集成。 与兼容性级别 1.1（新作业的默认兼容级别）相比，兼容性级别1.2 明显提高了吞吐量，并减少了 RU 消耗。 该解决方案使用根据 /deviceId 分区的 CosmosDB 容器，解决方案其余部分采用相同的配置。

所有 [Azure 大规模流式处理示例](https://github.com/Azure-Samples/streaming-at-scale)均通过加载模拟测试端作为输出来使用事件中心。 每个输入事件都是一个 1KB 的 JSON 文档，可轻松地将配置的引入速率转换为吞吐率（1 MB/秒、5 MB/秒和 10 MB/秒）。 事件可模拟 IoT 设备，为最多 1K 个设备发送如下所示的 JSON 数据（省略显示）：
...
> [!NOTE]
> 配置会因解决方案中所用的不同组件而发生改变。 若要获得更准确的估计值，请根据具体方案自定义示例。

### <a name="identifying-bottlenecks"></a>识别瓶颈

使用 Azure 流分析作业中的“指标”窗格识别管道中的瓶颈。 请通过“输入/输出事件”评估吞吐量、查看[水印延迟](https://azure.microsoft.com/blog/new-metric-in-azure-stream-analytics-tracks-latency-of-your-streaming-pipeline/)或查看积压工作事件，以了解作业是否跟得上输入速率 。 对于事件中心指标，请查看“限制的请求数”并相应地调整阈值单位。 对于 Cosmos DB 指标，请查看“吞吐量”下的“每个分区键范围所使用的最大 RU/秒”，以确保均匀使用分区键范围。 对于 Azure SQL DB，请监视“日志 IO”和“CPU” 。
