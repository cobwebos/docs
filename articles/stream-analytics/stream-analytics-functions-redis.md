...
1. 在 Azure 门户中，转到作业的“输出”边栏选项卡，并选择页面顶部的“添加”。
   
    ![添加输出](./media/stream-analytics-functions-redis/adding-outputs.png)
2. 选择“服务总线队列”作为“接收器”，并按照屏幕上的说明操作。 请务必选择在[服务总线队列入门][servicebus-getstarted]中创建的服务总线队列命名空间。 完成后，单击“向右箭头”按钮。
3. 指定以下值：
...
5. 在“查询”选项卡上，将当前查询替换为以下内容。 将*[服务总线名称]*替换为步骤 3 中创建的输出名称。 
...
完成后，会获得新的 Redis 缓存。 在“所有设置”下，选择“访问密钥”并记下“主要连接字符串”。
...
1. 在门户中，从左侧导航栏中选择“应用服务”，并单击 Azure 函数应用名称，获取函数的应用网站。
...
2. 依次单击“新建函数”>“ServiceBusQueueTrigger – C#”。 对于以下字段，请遵循以下说明：
   ...
   * **服务总线连接**：选择“添加连接字符串”。 要查找连接字符串，请转到经典门户，并依次选择“服务总线”、创建的服务总线，以及屏幕底部的“连接信息”。 请确保在此页的主屏幕上操作。 复制并粘贴连接字符串。 可以输入任意连接名称。
...
1. 选择新创建的 **ServiceBusQueueTrigger**，并单击右上角的“函数应用设置”。 依次选择“转到应用服务设置”>“设置”>“应用程序设置”
...
4. 现在，请返回到“应用服务设置”，并依次选择“工具”>“应用服务编辑器(预览版)”>“打开”>“转到”。
...
6. 将此文件上传到函数的根目录（不是 WWWROOT）。 应会看到名为 **project.lock.json** 的文件自动显示，确认 Nuget 包“StackExchange.Redis”和“Newtonsoft.Json”已导入。
...
3. 在出现的“启动作业”边栏选项卡中，选择“立即”，并单击屏幕底部的“启动”按钮。 作业状态将更改为“正在启动”，片刻之后会更改为“正在运行”。
...
要验证数据是否在 Redis 缓存中，请转到新门户中的 Redis 缓存页（如前面的[创建 Azure Redis 缓存](#Create-an-Azure-Redis-Cache)步骤中所示），并选择“控制台”。
...
如果是 Microsoft Azure 的新用户，我们将邀请你通过注册一个 [Azure 免费试用帐户](https://azure.microsoft.com/pricing/free-trial/)，来试用它。 如果不熟悉流分析，我们将邀请你[创建第一个流分析作业](stream-analytics-create-a-job.md)。
