...
这些输入资源可与流分析作业位于同一 Azure 订阅中，也可位于不同的订阅中。
...
流分析还支持称为“引用数据”的输入。 引用数据是完全静态的或更改缓慢。 它通常用于执行关联和查找。 例如，可以将数据流输入中的数据联接到引用数据中的数据，就像执行 SQL 联接以查找静态值一样。 当前支持将 azure Blob 存储和 Azure SQL 数据库用作引用数据的输入源。 引用数据源 blob 的大小限制为 300 MB，具体取决于查询复杂性和分配的流式处理单位（有关详细信息，请参阅引用数据文档的[大小限制](stream-analytics-use-reference-data.md#size-limitation)部分）。
