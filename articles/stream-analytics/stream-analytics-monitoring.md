...
Azure 门户会显示可用于监视和排查查询与作业性能问题的关键性能指标。 要查看这些指标，请浏览到感兴趣的想要查看其指标的流分析作业，并查看“概览”页面上的“监视”部分。  
...
| 输入事件数           | 从输入事件反序列化的记录数。 此计数不包括导致反序列化错误的传入事件。 在内部恢复和自联接等方案中，流分析可以多次引入相同的事件。 因此，如果作业包含简单的“传递”查询，则我们建议不要指望输入事件与输出事件的指标相匹配。 |
| 收到的输入源数       | 作业收到的消息数。 对于事件中心，一条消息是一个事件数据。 对于 Blob，一条消息是一个 Blob。 请注意，在反序列化之前，将会统计输入源的数目。 如果出现反序列化错误，则输入源数可能大于输入事件数。 否则，它可能小于或等于输入事件数，因为每条消息可能包含多个事件。 |
