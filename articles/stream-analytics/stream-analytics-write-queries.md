...
2. 新的作业有一个有助于入门的查询模板。 查询模板执行一种“传递”查询，将来自输入事件的所有字段投射到输出。  
   
   * 如果已经为作业定义了至少一个输入和一个输出，则可以用希望首先使用的输入和输出的别名代替占位符 "[YourOutputAlias]" 和 "[YourInputAlias]"。 此外，仍然可以在 Azure 经典门户中编写和测试查询，无需在作业上定义输入和输出。
...
1. 确保查询没有任何错误（否则“测试”按钮会被禁用），然后单击“测试”按钮。  
