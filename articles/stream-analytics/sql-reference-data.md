...
2. 创作快照查询。 
   ...
   使用**\@snapshotTime**参数指示流分析运行时从 SQL 数据库临时表获取在系统时间有效的引用数据集。 如果不提供此参数，可能会出于时钟偏差的原因获取不准确的基本参考数据集。 完整的快照查询示例如下所示：
   ```SQL
...
2. 创作增量查询。 
   ...
   此查询检索 SQL 数据库中在开始时间、**\@增量开始时间和**结束时间**\@delta EndTime**中插入或删除的所有行。 增量查询必须返回与快照查询以及列**操作**相同的列。__ 此列定义在**\@增量 StartTime**和**\@delta EndTime**之间插入或删除行。 如果插入了记录，则生成的行将标记为 **1**；如果删除了记录，则标记为 **2**。 
...
可以使用两个按“逻辑名称”筛选的指标（在指标 Azure 门户中）来监视 SQL 数据库参考数据输入的运行状况。
   ...
   * 输入事件：此指标衡量从 SQL 数据库引用数据集加载的记录数。
   * InputEvent 字节：此指标测量在流分析作业的内存中加载的引用数据快照的大小。 
