...
5. 如果使用的是 Visual Studio 2019，并且已安装 SQL Server Data tools，则可以通过单击 "**执行**" 来测试该查询。 此时会弹出一个向导窗口帮助你连接到 SQL 数据库；查询结果将显示在窗口底部。
...
2. 创作快照查询。 
   ...
   使用 **\@snapshotTime**参数指示流分析运行时从 SQL 数据库时态表中获取在系统时间有效的引用数据集。 如果不提供此参数，可能会出于时钟偏差的原因获取不准确的基本参考数据集。 完整的快照查询示例如下所示：
   ```SQL
...
2. 创作增量查询。 
   ...
   此查询检索 SQL 数据库中的所有行，这些行在开始时间内插入或删除、 **\@deltaStartTime**和结束时间 **\@deltaEndTime**。 增量查询必须返回与快照查询以及列**操作**相同的列。 此列定义在 **\@deltaStartTime**和 **\@deltaEndTime**之间插入还是删除行。 如果插入了记录，则生成的行将标记为 **1**；如果删除了记录，则标记为 **2**。 
...
有两个按逻辑名称筛选的指标（在 "指标 Azure 门户" 下）可用于监视 SQL 数据库引用数据输入的运行状况。
