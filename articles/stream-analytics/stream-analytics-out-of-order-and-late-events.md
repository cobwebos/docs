...
---
# <a name="azure-stream-analytics-event-order-consideration"></a>Azure 流分析事件顺序注意事项

## <a name="understand-arrival-time-and-application-time"></a>了解抵达时间和应用程序时间。
...
## <a name="azure-stream-analytics-handling-of-multiple-streams"></a>Azure 流分析对多个流的处理方式

在少数情况下，Azure 流分析作业合并了来自多个时间线的事件，包括：

* 从多个分区生成输出。 未显式指定“Partition by PartitionId”的查询必须合并来自所有分区的事件。
* 两个或更多个不同输入源的联合。
* 联接输入源。

在合并多个时间线的情况下，只有在合并的所有源都至少位于时间点 *t1* 之后，Azure 流分析才生成时间戳 *t1* 的输出。
例如，如果查询从某个事件中心分区读取数据，而该分区包含两个分区，其中一个分区 *P1* 包含时间点 *t1* 之前的事件，另一个分区 *P2* 包含时间点 *t1 + x* 之前的事件，那么，只有到达时间点 *t1* 时，才生成输出。
但是，如果显式指定了“Partition by PartitionId”子句，这两个分区会独立地继续运行。
...
按**应用程序时间**进行处理时，流分析可以容许延迟事件和无序事件。 Azure 门户中的“事件顺序”选项提供以下设置： 
...
如前所述，延迟到达容错是应用程序时间与到达时间之间的最大差。
另外，在按应用程序时间进行处理时，会在应用无序容错设置之前，调整比配置的延迟到达容错更晚的事件。 因此，有效的无序是延迟到达容错与无序容错中的最小值。
...
查询包含“Partition by PartitionId”子句，并且在单个分区中，事件是使用同步发送方法发送的。 在发送事件之前，同步发送方法一直阻塞。
在这种情况下，无序为零，因为事件是按顺序发送的，只有在显式确认后才发送下一个事件。 延迟到达时间是事件生成时间与事件发送时间之间的最大延迟 + 发送者与输入源之间的最大延迟

### <a name="example-2"></a>示例 2：
查询包含“Partition by PartitionId”子句，并且在单个分区中，事件是使用异步发送方法发送的。 异步发送方法可以同时启动多个发送操作，这可能导致事件无序。
在这种情况下，无序时间和延迟到达时间至少是事件生成时间与事件发送时间之间的最大延迟 + 发送者与输入源之间的最大延迟。

### <a name="example-3"></a>示例 3：
查询不包含“Partition by PartitionId”，并且至少有两个分区。
配置与示例 2 相同。 但是，如果一个分区中缺少数据，可能会按照额外的“延迟到达容错”时限将输出延迟。

## <a name="to-summarize"></a>总结
* 应该根据正确性和延迟要求配置延迟到达容错和无序时限，并且应考虑到事件发送方式。
* 建议将无序容错设置得比延迟到达容错更小。
