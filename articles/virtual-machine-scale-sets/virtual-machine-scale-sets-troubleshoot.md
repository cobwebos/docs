...
* 缩小和扩大阈值的差值是否足够大？
    ...
    假设设置了一个规则，该规则指示当平均 CPU 在 5 分钟内超过 50% 时扩大，当平均 CPU 低于 50% 则缩小。 在 CPU 使用率接近此阈值时，这会导致“摇摆”问题，缩放操作不断地增加和减少集的大小。 因此，自动缩放服务尝试防止“摇摆”的发生，这可能表现为不缩放。 因此请确保扩大和缩小阈值的差值足够大，以允许在缩放之间存在一些空间。
* 是否编写过自己的 JSON 模板？
...
* 诊断扩展运行正常且可发出性能数据吗？
  ...
  * 新 VM 的报告更可靠，并且启动更快。
    ...
    可能想要继续使用诊断扩展的唯一原因是如果需要内存诊断报告/缩放。 基于主机的指标不会报告内存。
