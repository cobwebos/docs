...
将区域 (非区域性) 规模集部署到 API 版本 *2017-12-01*之外的一个或多个区域时，可以使用以下可用性选项：
- 最大分配 (platformFaultDomainCount = 1) 
- 静态固定分配 (platformFaultDomainCount = 5) 
- 与存储磁盘容错域的分布 (platforFaultDomainCount = 2 或 3) 
...
部署规模集时，还可以选择为每个可用性区域部署单个[放置组](./virtual-machine-scale-sets-placement-groups.md)或多个放置组。 对于区域 (非区域性) 规模集，选择是在区域中使用单个放置组，或在区域中包含多个放置组。 对于大多数工作负荷，建议使用多个放置组，这样可以增大规模。 在 API 版本 *2017-12-01*中，对于单区域和跨区域规模集，规模集默认为多个放置组，但对于区域 (非区域性) 规模集，它们默认为单个放置组。
