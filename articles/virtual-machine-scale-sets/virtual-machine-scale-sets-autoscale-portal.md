...
5. 当平均 CPU 负载在十分钟内均超过 70% 时，请创建规则以增加规模集中的 VM 实例数。 触发规则时，VM 实例数增加 20%。 在 VM 实例数较少的规模集中，可将“操作”设置为“增加计数”，然后指定 1 或 2 作为实例计数************。 在有大量 VM 实例的规模集中，增加 10% 或 20% 的 VM 实例可能更合适。
    ...
    | *操作员*             | 用于比较指标数据和阈值的运算符。                                                     | 大于   |
    | *阈 值*            | 使自动缩放规则触发操作的百分比。                                                 | 70             |
    | *时间*             | 比较指标与阈值之前监视的时长。                                   | 10 分钟     |
...
2. 创建规则，在平均 CPU 负载在十分钟内下降至 30% 以下时减少规模集中的 VM 实例数。 触发规则后，VM 实例数减少 20%。
    ...
    | 参数              | 说明                                                                                                          | 值          |
    |------------------------|----------------------------------------------------------------------------------------------------------------------|----------------|
    | *操作员*             | 用于比较指标数据和阈值的运算符。                                                      | 小于   |
    | *阈 值*            | 使自动缩放规则触发操作的百分比。                                                 | 30             |
...
本文详细介绍了如何使用自动缩放规则来进行横向缩放，即增加或减少规模集中的 VM 实例数**。 还可进行纵向缩放，即增大或减小 VM 实例的大小**。 有关详细信息，请参阅[使用虚拟机缩放集的垂直自动缩放](virtual-machine-scale-sets-vertical-scale-reprovision.md)。
