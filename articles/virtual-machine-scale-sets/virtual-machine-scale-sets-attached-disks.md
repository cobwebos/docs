---
title: Azure 虚拟机缩放集附加数据磁盘
description: 了解如何通过特定用例的大纲将附加的数据磁盘与虚拟机规模集一起使用。
author: avirishuv
...
- [Azure 电源外壳](tutorial-use-disks-powershell.md)
...
在 Azure 中运行[的服务结构](/azure/service-fabric)群集中的每个[节点类型](../service-fabric/service-fabric-cluster-nodetypes.md)都由虚拟机缩放集支持。 可以使用 Azure 资源管理器模板将数据磁盘附加到组成 Service Fabric 群集的规模集。 可以使用[现有模板](https://github.com/Azure-Samples/service-fabric-cluster-templates)作为起点。 在模板中包括 _Microsoft.Compute/virtualMachineScaleSets_ 资源的 _storageProfile_ 中的 _dataDisks_ 节，然后部署模板。 以下示例附加一个 128 GB 的数据磁盘：
...
规模集模型中指定的数据磁盘始终为空。 但是，可以将现有数据磁盘附加到规模集中的特定 VM。 如果要将数据传播到比例集中的所有 VM 中，可以复制数据磁盘并将其附加到比例集中的每个 VM，或者创建包含数据的自定义映像，并在此自定义映像中预配比例集，或者您可以使用 Azure 文件或类似数据存储产品。
...
对规模集中附加数据磁盘的 Azure 门户支持是有限的。 可以根据需要使用 Azure 模板、CLI、PowerShell、SDK 和 REST API 管理附加磁盘。
