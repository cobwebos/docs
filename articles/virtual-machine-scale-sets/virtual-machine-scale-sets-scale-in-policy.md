...
可根据一系列指标（包括平台指标和用户定义的自定义指标）横向扩展或缩减虚拟机规模集部署。 虽然横向扩展基于规模集模型创建新的虚拟机，但扩展影响正在运行的虚拟机，这些虚拟机可能具有不同的配置和/或功能，因为规模集工作负载会发生变化。 
...
请注意，跨可用性区域或容错域的均衡不会在可用性区域或容错域之间移动实例。 平衡是通过从不平衡的可用性区域或容错域中删除虚拟机来实现的，直到虚拟机的分布达到平衡。
...
创建资源组，然后创建一个新规模集，其中放大缩小字体策略集为 *"最旧VM"。*
...
下面的示例在创建新的缩放集时添加扩展策略。 首先创建资源组，然后创建一个将扩展策略为 *"最旧VM"* 的新比例集。 
...
可以通过 Azure 门户修改现有规模集的放大缩小字体功能 放大缩小字体功能 
 
1. 在现有虚拟机缩放集中，从左侧的菜单中选择 **"缩放**"。
1. 选择"**放大策略**"选项卡。
1. 从下拉列表中选择"放大"策略。
1. 完成后，选择“保存”****。 
