...
**问：在门户、PowerShell 或 CLI 中，我没有看到任何计划维护的迹象。怎么了？**
...
**问：我的 VM 上看不到任何维护信息。出了什么问题？**

**答：** 有很多原因会导致在 VM 上看不到任何维护信息：
   - 使用的是标记为“Microsoft 内部”的订阅**。
   - VM 未计划进行维护。 可能是此次维护已结束、已取消或已修改，因此你的 VM 不再受其影响。
   - 您没有将 **"维护**"列添加到 VM 列表视图中。 虽然我们已向默认视图添加此列，但如果你将视图配置为查看非默认列，则仍需手动将“维护”列添加到 VM 列表视图****。

**问：我的 VM 计划第二次进行维护。为什么？**
