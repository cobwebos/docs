---
title: 处理 Azure 中虚拟机规模集的维护通知 | Microsoft Docs
...
ms.locfileid: "38506254"
---
# <a name="handling-planned-maintenance-notifications-for-virtual-machine-scale-sets"></a>处理虚拟机规模集的计划内维护通知
...
可以使用 Azure 门户、PowerShell、REST API 和 CLI 查询虚拟机规模集 VM 的维护时段并启动自助维护。
...
## <a name="view-virtual-machine-scale-sets-impacted-by-maintenance-in-the-portal"></a>在门户中查看受维护影响的虚拟机规模集

安排了大量计划内维护后，可以使用 Azure 门户查看受即将到来的大量维护影响的虚拟机规模集列表。 

1. 登录到 [Azure 门户](https://portal.azure.com)。
2. 选择左侧导航栏中的“所有服务”，然后选择“虚拟机规模集”。
3. 在“虚拟机规模集”页面上，选择顶部的“编辑列”选项以打开可用列的列表。
4. 在“可用列”部分中，选择“自助维护”项，然后使用箭头按钮将其移至“选定的列”列表中。 可以将“可用列”部分中的下拉列表从“全部”切换到“属性”，以便更轻松地查找“自助维护”项。 在“选定的列”部分中获得“自助维护”项后，选择页面底部的“应用”按钮。 

执行上述步骤后，“自助维护”列将显示在虚拟机规模集的列表中。 每个虚拟机规模集可以具有以下自助维护列的值之一：

| 值 | 说明 |
|-------|-------------|
| 是 | 虚拟机规模集中至少有一个虚拟机处于自助时段。 你可以在此自助时段随时启动维护。 | 
| 否 | 受影响的虚拟机规模集中的自助时段中没有任何虚拟机。 | 
| - | 你的虚拟机规模集不属于大量的计划内维护。| 

## <a name="notification-and-alerts-in-the-portal"></a>门户中的通知和警报

Azure 通过向订阅所有者和共有者组发送电子邮件来传达计划维护的安排。 可以通过创建 Azure 活动日志警报，为此通信添加其他收件人和频道。 有关详细信息，请参阅 [通过 Azure 活动日志监视订阅活动] (../monitoring-and-diagnostics/monitoring-overview-activity-logs.md)

1. 登录到 [Azure 门户](https://portal.azure.com)。
2. 在左侧菜单中选择“监视”。 
3. 在“监视 - 警报(经典)”窗格中，单击“+ 添加活动日志警报”。
4. 完成“添加活动日志警报”页面中的信息，并确保在“条件”中设置以下内容：
   - 事件类别：服务运行状况
   - 服务：虚拟机规模集和虚拟机
   - 类型：计划内维护 
    
若要详细了解如何配置活动日志警报，请参阅[创建活动日志警报](../monitoring-and-diagnostics/monitoring-activity-log-alerts.md)
    
    
## <a name="start-maintenance-on-your-virtual-machine-scale-set-from-the-portal"></a>从门户中开始维护虚拟机规模集

在查看虚拟机规模集概述时，将能够看到更多维护相关的详细信息。 如果至少一个虚拟机规模集中的虚拟机包括在大量的计划内维护中，则将会在页面顶部附近添加新的通知功能区。 可以单击通知功能区转到“维护”页面，以查看受计划内维护影响的虚拟机实例。 

从那里，可以通过选中与受影响的虚拟机对应的框，然后单击“开始维护”选项来启动维护。

启动维护后，虚拟机规模集中受影响的虚拟机将进行维护并暂时不可用。 如果错过了自助时段，则由 Azure 维护虚拟机规模集时，你将仍可以看到该时段。
...
### <a name="start-maintenance-on-your-vm-instance-using-powershell"></a>使用 PowerShell 在 VM 实例上启动维护

如果使用带 `-PerformMaintenance` 参数的 [Set-AzureRmVmss](/powershell/module/azurerm.compute/set-azurermvmss) cmdlet 将 **IsCustomerInitiatedMaintenanceAllowed** 设置为 true，则可以在 VM 上启动维护。

```powershell
Set-AzureRmVmss -ResourceGroupName rgName -VMScaleSetName vmssName -InstanceId id -PerformMaintenance 
```

## <a name="check-maintenance-status-using-cli"></a>使用 CLI 检查维护状态

可以使用 [az vmss list-instances](/cli/azure/vmss?view=azure-cli-latest#az-vmss-list-instances) 查看计划内维护信息。
 
仅当有计划内维护时，才会返回维护信息。 如果未计划任何影响 VM 实例的维护，该命令不返回任何维护信息。 
...
### <a name="start-maintenance-on-your-vm-instance-using-cli"></a>使用 CLI 在 VM 实例上启动维护
...
**答：** 设置计划时，我们定义了长达几天的时间窗口。 但是，服务器（和 VM）在此时间窗口内的确切排序是未知的。 想要知道其 VM 确切时间的客户可以使用[计划事件](../virtual-machines/windows/scheduled-events.md)并从虚拟机中查询，这样就会在 VM 重启前 15 分钟收到通知。
...
**答：** 有很多原因会导致在 VM 上看不到任何维护信息：
   ...
   - VM 未计划进行维护。 可能是这次维护已结束、已取消或已修改，因此你的 VM 不再受其影响。
   - 未将“维护”列添加到 VM 列表视图。 虽然我们已向默认视图添加此列，但配置为查看非默认列的客户必须手动将“维护”列添加到其 VM 列表视图。
