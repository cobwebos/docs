<properties
   pageTitle="Azure 批处理 PowerShell cmdlet 入门"
...
本文将简要介绍可用于管理批处理帐户的 Azure PowerShell cmdlet，并提供有关批处理工作项、作业和任务的信息。

有关详细的 cmdlet 语法，请键入 ```get-help <Cmdlet_name>```。


## 先决条件

* **批处理预览版** - 注册[批处理预览版](https://account.windowsazure.com/PreviewFeatures)（如果尚未获得）以使用该服务。
* **Azure PowerShell** - 有关先决条件以及下载和安装说明，请参阅[如何安装和配置 Azure PowerShell](powershell-install-configure.md)。0.8.10 和更高版本中引入了批处理 cmdlet。

## 使用批处理 cmdlet

使用标准过程来启动 Azure PowerShell，然后[连接到你的 Azure 订阅](powershell-install-configure.md#Connect)。此外：

* **选择 Azure 订阅** - 如果你有多个订阅，请选择添加了批处理预览版功能的订阅：

    ```
    Select-AzureSubscription -SubscriptionName <SubscriptionName>
    ```

* **切换到 AzureResourceManage 模式** - Azure 资源管理器模块随附了批处理 cmdlet。有关详细信息，请参阅[将 Windows PowerShell 与资源管理器配合使用](powershell-azure-resource-manager.md)。若要使用此模块，请运行 [Switch-AzureMode](https://msdn.microsoft.com/library/dn722470.aspx) cmdlet：
...
**New-AzureBatchAccount** 可在指定的资源组中创建新的批处理帐户。如果你没有资源组，可以通过运行[New-AzureResourceGroup](https://msdn.microsoft.com/library/dn654594.aspx) cmdlet 并在 **Location** 参数中指定一个 Azure 区域来创建资源组。（可以通过运行 [Get-AzureLocation](https://msdn.microsoft.com/library/dn654582.aspx) 查找不同 Azure 资源的可用区域。）例如：
...
使用 **Get-AzureBatchWorkItem**、**Get-AzureBatchJob**、**Get-AzureBatchTask** 和 **Get-AzureBatchPool** 等 cmdlet 来查询批处理帐户下创建的实体。
...
> [AZURE.NOTE] 默认情况下，帐户的主要密钥用于身份验证，但你可以通过更改 BatchAccountContext 对象的 **KeyInUse** 属性，显式选择要使用的密钥：
```$context.KeyInUse = "Secondary"```.


### 查询数据

例如，使用 **Get-AzureBatchWorkItem** 可查找工作项。假设你已将 BatchAccountContext 对象存储在 *$context* 中，则默认情况下，此 cmdlet 将查询帐户下的所有工作项：

```
Get-AzureBatchWorkItem -BatchContext $context
```

对于其他实体（例如池）也可以执行类似的操作：
...
你可以使用 **Filter** 参数提供一个 OData 筛选器，以便只查找所需的对象。例如，可以查找名称以"myWork"开头的所有工作项：
...
OData 筛选器的替代方法是使用 **Name** 参数。若要查询名为"myWorkItem"的特定项：
...
* [Azure cmdlet 参考](https://msdn.microsoft.com/library/jj554330.aspx)
