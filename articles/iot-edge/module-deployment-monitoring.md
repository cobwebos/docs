...
本文重点介绍和监视设备队列，统称为*IoT Edge 自动部署*。基本部署步骤如下：

1. 操作员定义描述一组模块和目标设备的部署。每个部署都有一个反映此信息的部署清单。
2. IoT 中心服务与所有目标设备进行通信，以便使用声明的模块配置它们。
3. IoT 中心服务从 IoT Edge 设备检索状态，然后将这些状态提供给操作员。例如，操作员可以看到 Edge 设备何时未成功配置，或者模块在运行时失败。
...
例如，您有一个具有目标条件标记的部署。 启动该部署时，共有 10 个生产设备。 这 10 个设备都成功安装了模块。 IoT Edge 代理状态显示 10 个总设备、10 个成功响应、0 个失败响应和 0 个挂起的响应。 现在，又添加 5 个 tags.environment = 'prod' 的设备。 该服务检测到更改，IoT Edge 代理状态变为总共 15 个设备、10 个成功响应、0 个失败响应和 5 个挂起响应，同时部署到 5 个新设备。

在设备孪生标记、设备孪生报告属性或设备 Id 上使用任何布尔条件来选择目标设备。 如果想将条件与标记结合使用，则需在设备孪生中与属性相同的级别下添加 "tags":{} 节。 [深入了解设备孪生中的标记](../iot-hub/iot-hub-devguide-device-twins.md)
...
* 在设备孪生中，只能使用标记、报告的属性或设备 Id 生成目标条件。
...
* **报告成功**显示已报告模块已成功部署的 IoT Edge 设备。
* **报告失败**显示已报告一个或多个模块未成功部署的 IoT Edge 设备。 若要进一步调查此错误，请通过远程方式连接到这些设备并查看日志文件。

此外，还可以定义自己的自定义指标来帮助监视和管理部署。

指标提供各种状态的摘要计数，设备可能在应用部署配置后报告这些状态。 指标可以查询[边缘Hub模块孪生报告的属性](module-edgeagent-edgehub.md#edgehub-reported-properties)，如*上次"希望状态"* 或 *"上次连接时间*"。 例如：
...
系统运行时模块 edgeAgent 和 edgeHub 未配置为分层部署的一部分。 分层部署所针对的任何 IoT Edge 设备都需要首先应用于它的标准自动部署。 自动部署提供了可以在此基础上添加分层部署的基础。
...
使用分层部署时，可能有意或无意地对针对设备的同一模块使用两个部署。 在这些情况下，您可以决定优先级较高的部署应覆盖模块孪生或附加模块。 例如，你可能有一个将同一模块应用于 100 个不同设备的部署。 但是，其中 10 个设备位于安全设施中，需要额外配置才能通过代理服务器进行通信。 你可以使用分层部署来添加模块孪生属性，以便这 10 个设备可安全通信，而不会覆盖基本部署中现有的模块孪生信息。

可以在部署清单中追加模块孪生所需属性。 在标准部署中，你可以在模块孪生的 properties.desired 部分中添加属性，而在层部署中，你可以声明所需属性的新子集****。

例如，在标准部署中，您可以添加具有以下所需属性的模拟温度传感器模块，这些属性告诉它以 5 秒的间隔发送数据：
...
在针对部分或全部相同设备的分层部署中，可以添加一个属性，告诉模拟传感器发送 1000 条消息，然后停止。 您不希望覆盖现有属性，因此在名为 的所需`layeredProperties`属性中创建一个新节，其中包含新属性：
...
应用了两个部署的设备将反映模拟温度传感器的模块孪生中的以下属性：
