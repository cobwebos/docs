...
4. 与 IoT 中心重新连接和重新同步
   ...
   一旦还原与 IoT 中心的连接，IoT Edge 设备会再次同步。 本地存储的消息立即传递到 IoT 中心，但依赖于连接速度、IoT 中心延迟以及相关因素。 这些消息按照它们存储的相同顺序传递。
...
对于将其扩展脱机功能扩展到 IoT 子设备的 IoT Edge 设备，需要完成以下两个步骤。 首先，在 Azure 门户中声明父子关系。 其次，在父设备和任何子设备之间创建信任关系，然后配置设备到云的通信，使其通过作为网关的父设备。

### <a name="assign-child-devices"></a>分配子设备

子设备可以是注册到同一个 IoT 中心的任何非 IoT Edge 设备。 父设备可以有多个子设备，但子设备只能有一个父设备。 有以下三个选项可用于将子设备设置为边缘设备：通过 Azure 门户、使用 Azure CLI 或使用 IoT 中心服务 SDK。

以下部分提供了有关如何在 IoT 中心声明现有 IoT 设备的父/子关系的示例。 如果要为子设备创建新的设备标识，请参阅[通过 Azure IoT 中心对下游设备进行身份验证](how-to-authenticate-downstream-device.md)以获取详细信息。

#### <a name="option-1-iot-hub-portal"></a>选项 1：IoT 中心门户

创建新设备时，可以声明父子关系。 或者，对于现有设备，可以从 IoT Edge 父设备或 IoT 子设备的设备详细信息页声明关系。
...
将 [Azure 命令行接口](https://docs.microsoft.com/cli/azure/?view=azure-cli-latest)与 [IoT 扩展](https://github.com/azure/azure-iot-cli-extension)（v0.7.0 或更高版本）配合使用，可以使用 [device-identity](https://docs.microsoft.com/cli/azure/ext/azure-iot/iot/hub/device-identity?view=azure-cli-latest) 子命令管理父子关系。 以下示例使用查询将中心内的所有非 IoT Edge 设备分配为 IoT Edge 设备的子设备。
...
可以修改“[查询](../iot-hub/iot-hub-devguide-query-language.md)”以选择其他部分设备。 如果指定大量设备，此命令可能需要几秒钟。
...
可以将父/子关系视为透明网关，其中，子设备在 IoT 中心具有自己的标识，但通过其父设备使用云进行通信。 为了安全地通信，子设备需要能够验证父设备是否来自受信任的源。 否则，第三方可能会设置恶意设备来模拟父设备和拦截通信。
...
为了提高可靠性，强烈建议指定在环境中使用的 DNS 服务器地址。 若要设置 IoT Edge 的 DNS 服务器，请参阅故障排除文章中的 [Edge 代理模块持续报告“配置文件为空”，且设备上不会启动任何模块](troubleshoot-common-errors.md#edge-agent-module-reports-empty-config-file-and-no-modules-start-on-the-device)的解决方法。

## <a name="optional-offline-settings"></a>可选脱机设置

如果设备处于脱机状态，则在重新建立连接之前，IoT Edge 父设备将存储所有设备到云的消息。 IoT Edge 中心模块管理脱机消息的存储和转发。 对于可能长时间内处于脱机状态的设备，可以通过配置两个 IoT Edge 中心设置来优化性能。

首先，增加生存时间设置，以便 IoT Edge 中心将消息保留足够长的时间以重新连接设备。 然后，为消息存储添加更多磁盘空间。

### <a name="time-to-live"></a>生存时间

生存时间设置是指在过期之前消息可以等待传递的时间量（以秒为单位）。 默认为 7200 秒（两个小时）。 最大值仅受整数变量的最大值限制，此值大约为 20 亿。
