...
6. 在第 35 行中，将 **\<sql connection string\>** 字符串替换为以下字符串。 **Data Source** 属性引用 SQL Server 容器，该容器尚不存在，但在下一部分，我们将使用名称 **SQL** 来创建该容器。 
...
9. 找到包引用所在的组，并添加一个新组来包含 SqlClient。 
