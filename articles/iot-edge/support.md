...
ms.locfileid: "85807394"
---
# <a name="azure-iot-edge-supported-systems"></a>Azure IoT Edge 支持的系统

本文提供了有关 IoT Edge 支持的系统和组件的详细信息，包括正式版和预览版。

如果使用 Azure IoT Edge 服务时遇到问题，可以通过多种方式寻求支持。 尝试以下支持渠道之一：
...
Azure IoT Edge 模块以容器形式实现，因此 IoT Edge 需要使用容器引擎启动模块。 Microsoft 提供了容器引擎 moby-engine 来满足此要求。 此容器引擎基于 Moby 开放源代码项目。 Docker CE 和 Docker EE 是其他常用的容器引擎。 它们也基于 Moby 开放源代码项目，并与 Azure IoT Edge 兼容。 Microsoft 对使用那些容器引擎的系统提供尽力而为的支持；但是，Microsoft 无法为其中的问题提供修复。 因此，Microsoft 建议在生产系统上使用 moby-engine。
...
* 支持第 1 层系统。 对于第 1 层系统，Microsoft：
  * 将此操作系统包括在自动化测试中
  * 为它们提供安装程序包
* 第 2 层系统与 Azure IoT Edge 兼容，并且可以相对容易地使用。 对于第 2 层系统：
  * Microsoft 已在这些平台上进行了非正式测试，或者知道合作伙伴已成功在平台上运行 Azure IoT Edge
...
Microsoft 支持下表中列出的系统，这些系统通常可用或处于公开预览版，每个新版本都会针对其进行测试。 
...
| [Windows 10 IoT Enterprise](https://docs.microsoft.com/windows/iot-core/windows-iot-enterprise)（内部版本 17763） | ![Windows 10 IoT Enterprise + AMD64](./media/tutorial-c-module/green-check.png) |  |  |
...
上面列出的 Windows 操作系统是在 Windows 上运行 Windows 容器的设备的要求，这是受支持的唯一生产配置。 适用于 Windows 的 Azure IoT Edge 安装包允许在 Windows 上使用 Linux 容器；但是，只能将此配置用于开发和测试。 有关详细信息，请参阅[在 Windows 上使用 IoT Edge 运行 Linux 容器](how-to-install-iot-edge-windows-with-linux.md)。
...
<sup>1</sup> Debian 10 系统（包括 Raspian Buster）使用 IoT Edge 不支持的 OpenSSL 版本。 在安装 IoT Edge 之前，请使用以下命令安装较低版本：

```bash
sudo apt-get install libssl1.0.2
```

<sup>2</sup> [Azure IoT Edge 版本](https://github.com/Azure/azure-iotedge/releases)存储库中的 Debian 9 包应与 Ubuntu 20.04 一起使用。

## <a name="releases"></a>发行版本

[azure-iotedge releases](https://github.com/Azure/azure-iotedge/releases) 页提供 IoT Edge 发行资产和发行说明。 本部分将介绍这些发行说明中的信息，以帮助你更轻松地直观显示每个版本的组件。

可以单独安装或更新 IoT Edge 组件，并与旧版本中的组件向后兼容。 下表列出了每个版本中包含的组件：
...
IoT Edge 使用 Microsoft.Azure.Device.Client SDK。 有关详细信息，请参阅 [Azure IoT C# SDK GitHub 存储库](https://github.com/Azure/azure-iot-sdk-csharp)或 [Azure SDK for .NET 参考内容](https://docs.microsoft.com/dotnet/api/overview/azure/iot/client?view=azure-dotnet)。 以下列表显示了测试每个版本所针对的客户端 SDK 版本：
...
在 Raspberry Pi3 等小型设备以及服务器级硬件上，Azure IoT Edge 都能非常顺利地运行。 为方案选择合适的硬件取决于想要运行的工作负荷。 在设备方面做出最终决策可能比较复杂；但是，可以在传统的便携式计算机或台式机上轻松开始设计解决方案的原型。
