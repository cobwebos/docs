...
ms.locfileid: "32311737"
---
# <a name="continuous-integration-and-continuous-deployment-to-azure-iot-edge---preview"></a>向 Azure IoT Edge 进行持续集成和持续部署 - 预览
本教程演示如何使用 Visual Studio Team Services (VSTS) 和 Microsoft Team Foundation Server (TFS) 的持续集成和持续部署功能，快速高效地向 Azure IoT Edge 生成、测试并部署应用程序。 
...
1. 在第一个 Azure IoT Edge 任务中，将“显示名称”更新为“模块生成和推送”，并在“操作”下拉列表中选择“生成并推送”。 在“Module.json 文件”文本框中添加下面的路径。 然后选择“容器注册表类型”，确保在代码中配置并选择相同的注册表。 此任务将生成并推送解决方案中的所有模块，并发布至指定的容器注册表。 
...
本教程演示如何使用 VSTS 或 TFS 的持续集成和持续部署功能。 

* 在[了解单设备 IoT Edge 部署或大规模 IoT Edge 部署](module-deployment-monitoring.md)中了解 IoT Edge 部署
* 完成[大规模部署和监视 IoT Edge 模块][how-to-deploy-monitor.md] 中创建、更新或删除部署的步骤。
