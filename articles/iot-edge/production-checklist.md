...
生产环境中的每个 IoT Edge 设备上需要安装设备证书颁发机构 (CA) 证书。 然后，在 config.yaml 文件中将该 CA 证书声明到 IoT Edge 运行时。 对于开发和测试方案，如果未在 yaml 文件中声明任何证书，则 IoT Edge 运行时将创建临时证书。 但是，这些临时证书将在三个月后过期，并且对于生产方案而言并不安全。
...
可以配置协议（确定所使用的端口），以便将 IoT Edge 代理和 IoT Edge 中心与 IoT 中心进行上游通信。 默认协议为 AMQP，但可以根据网络设置更改协议。
...
如果将**OptimizeForPerformance**设置为**TRUE**，则 MQTT 协议头将使用 PooledByteBufferAllocator，它具有更好的性能，但会分配更多内存。 分配器在32位操作系统或内存不足的设备上不能正常工作。 此外，如果针对性能进行了优化，RocksDb 会为其角色分配更多的内存作为本地存储提供程序。
