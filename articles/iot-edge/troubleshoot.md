...
description: 使用本文了解 Azure IoT Edge 的标准诊断技能，如检索组件状态和日志
author: kgremban
manager: philmea
ms.author: kgremban
ms.date: 04/27/2020
...
ms.locfileid: "82858595"
---
# <a name="troubleshoot-your-iot-edge-device"></a>排查 IoT Edge 设备问题

如果在您的环境中运行 Azure IoT Edge 时遇到问题，请使用本文作为故障排除和诊断指南。

## <a name="run-the-check-command"></a>运行 "检查" 命令

故障排除 IoT Edge 的`check`第一步应该是使用命令，该命令将运行一系列常见问题的配置和连接性测试。 `check` 命令在[版本 1.0.7](https://github.com/Azure/azure-iotedge/releases/tag/1.0.7) 及更高版本中提供。
...
故障排除工具运行多个检查，这些检查分为以下三个类别：

* *配置检查*将检查可能阻止 IoT Edge 设备连接到云的详细信息，包括*yaml*和容器引擎的问题。
* *连接检查*验证 IoT Edge 运行时是否可以访问主机设备上的端口，以及所有 IoT Edge 组件是否可以连接到 IoT 中心。 如果 IoT Edge 设备位于代理后面，这组检查将返回错误。
* *生产就绪状态检查*将查找建议的生产最佳实践，例如设备证书颁发机构（CA）证书和模块日志文件配置的状态。

有关此工具的每个诊断检查的信息，包括在出现错误或警告时要执行的操作，请参阅[IoT Edge 排查检查问题](https://github.com/Azure/iotedge/blob/master/doc/troubleshoot-checks.md)。

## <a name="gather-debug-information-with-support-bundle-command"></a>用 "support-束" 命令收集调试信息

需要从 IoT Edge 设备收集日志时，最方便的方法是使用`support-bundle`命令。 默认情况下，此命令收集模块、IoT Edge 安全管理器和容器引擎`iotedge check`日志、JSON 输出和其他有用的调试信息。 它将它们压缩为一个文件，以便于共享。 `support-bundle` [Release 1.0.9](https://github.com/Azure/azure-iotedge/releases/tag/1.0.9)和更高版本中提供了命令。

运行带有`support-bundle` `--since`标志的命令，指定要从过去获取日志的时间。 例如`6h` ，自过去六个小时以来，自过去`6d`六个小时以来会获得`6m`日志，自过去六分钟起，最后六分钟。 包含`--help`标志以查看选项的完整列表。
...
> [!WARNING]
> `support-bundle`命令的输出可以包含主机、设备和模块名称、模块记录的信息，等等。如果共享公共论坛中的输出，请注意这一点。

## <a name="check-your-iot-edge-version"></a>检查 IoT Edge 版本

如果你运行的是较旧版本的 IoT Edge，则升级可能会解决你的问题。 此`iotedge check`工具会检查 IoT Edge 安全守护程序是否为最新版本，但不检查 IoT Edge 中心和代理模块的版本。 若要检查设备上的运行时模块的版本，请使用命令`iotedge logs edgeAgent`和`iotedge logs edgeHub`。 当模块启动时，将在日志中声明版本号。

有关如何更新设备的说明，请参阅[更新 IoT Edge 安全守护程序和运行时](how-to-update-iot-edge.md)。

## <a name="check-the-status-of-the-iot-edge-security-manager-and-its-logs"></a>检查 IoT Edge 安全管理器及其日志的状态

[IoT Edge 安全管理器](iot-edge-security-manager.md)负责在启动和预配设备上初始化 IoT Edge 系统之类的操作。 如果 IoT Edge 未启动，则安全管理器日志可能会提供有用的信息。

在 Linux 上：

* 查看 IoT Edge 安全管理员的状态：

   ```bash
   sudo systemctl status iotedge
   ```

* 查看 IoT Edge 安全管理员的日志：
...
* 仅查看 IoT Edge security manager 日志的最后5分钟：
...
### <a name="if-the-iot-edge-security-manager-is-not-running-verify-your-yaml-configuration-file"></a>如果 IoT Edge 安全管理器未运行，请验证你的 yaml 配置文件
...
运行 IoT Edge 安全守护程序后，查看容器的日志来检测问题。 先查看你的已部署容器，然后查看构成 IoT Edge 运行时的容器：edgeAgent 和 edgeHub。 IoT Edge 代理日志通常提供有关每个容器的生命周期的信息。 IoT Edge 中心日志提供有关消息传送和路由的信息。
