...
1. 使用下拉菜单选择“重启策略”****。 从以下选项中选择：
   * **始终**- 如果模块因任何原因关闭，则始终重新启动。
   * **从不**- 模块永远不会重新启动，如果它因任何原因关闭。
   * **运行故障时** - 如果模块发生故障，它将重启，但如果完全关闭，则不重启。
   * **on 不正常**- 如果模块崩溃或返回不正常状态，则重新启动。 这取决于每个执行运行状况监控功能的模块。
1. 使用下拉菜单选择模块的所需状态****。 从以下选项中选择：
   * **正在运行** -“正在运行”是默认选项。 该模块在部署之后将立即开始运行。
   * **已停止**- 部署后，模块将保持空闲状态，直到您或其他模块调用该模块。
...
如果要创建分层部署，则可能需要配置针对相同设备的其他部署中的模块。 要更新模块孪生而不覆盖其他版本，请打开 **"模块孪生设置"** 选项卡。为模块孪生所需的属性中的子节创建一个新的**模块孪生属性**，该属性具有唯一的名称`properties.desired.settings`，例如 。 如果只在 `properties.desired` 字段中定义属性，它将覆盖在任何较低优先级部署中定义的模块的所需属性。
...
### <a name="step-4-metrics"></a>第 4 步：指标
...
### <a name="step-5-target-devices"></a>第 5 步：目标设备
...
选择 **"下一步"：查看 + 创建**以继续执行最后一步。

### <a name="step-6-review-and-create"></a>第 6 步：查看和创建
...
1. 检查部署列表。对于每个部署，可以查看以下详细信息：
   ...
   * **目标条件**- 用于定义目标设备的标记。
   * 优先级**** - 分配到部署的优先级编号。
   * **系统指标** - **"目标"** 指定 IoT 中心中与定位条件匹配的设备孪生数，**应用指定**已将部署内容应用于 IoT 中心中的模块孪生的设备数。
   * **设备指标** - 部署中报告 IoT Edge 客户端运行时成功或错误的 IoT Edge 设备数。
   * **自定义指标** - 部署报告数据中针对为部署定义的任何指标的 IoT Edge 设备数。
