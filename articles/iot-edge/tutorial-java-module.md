...
如果你是第一次创建 Java 模块，则下载 maven 包可能需要几分钟时间。 解决方案准备就绪后，VS Code 窗口会加载 IoT Edge 解决方案工作区。 解决方案工作区包含五个顶级组件：

* **modules** 文件夹包含模块的 Java 代码，以及用于将模块构建为容器映像的 Docker 文件。
* **\.env** 文件存储容器注册表凭据。
* **deployment.template.json** 文件包含 IoT Edge 运行时用于在设备上部署模块的信息。
* **deployment.debug.template.json** 文件包含模块的调试版本。
* 你不会在本教程中编辑 **\.vscode** 文件夹或 **\.gitignore** 文件。
...
目前，Visual Studio Code 可以开发适用于 Linux AMD64 和 Linux ARM32v7 设备的 Java 模块。 需要选择面向每个解决方案的体系结构，因为每种体系结构类型的容器的生成和运行方式均不相同。 默认值为 Linux AMD64。
...
2. 查看抵达 IoT 中心的消息。 消息到达可能需要一段时间。 IoT Edge 设备必须接收其新部署并启动所有模块。 然后，我们对 JavaModule 代码所做的更改将等到计算机温度达到 25 度时才发送消息。 IoT 中心还会将消息类型“警报”添加到达到该温度阈值的任何消息。 
...
在本教程中，你已创建一个 IoT Edge 模块用于筛选 IoT Edge 设备生成的原始数据。 准备好生成自己的模块时，可以详细了解如何[开发 IoT Edge 模块](module-development.md)或如何[使用 Visual Studio Code 开发模块](how-to-vs-code-develop-module.md)。 有关代码示例，包括模拟温度模块，请参阅 [IoT Edge 模块示例](https://github.com/Azure/iotedge/tree/master/edge-modules)。

继续学习后续教程，了解如何借助 Azure IoT Edge 部署 Azure 云服务，以便在边缘位置处理和分析数据。
