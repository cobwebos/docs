...
本文提供运行时模块孪生的所需属性和报告属性。 有关如何部署 IoT Edge 设备上的模块的详细信息，请参阅[了解如何部署模块和 IoT Edge 中建立路由](module-composition.md)。

模块孪生包括： 

* **所需属性**。 解决方案后端可以设置所需的属性，以及该模块可以读取它们。 该模块还可以接收更改通知所需的属性。 所需的属性和报告的属性中用于同步模块配置或条件。

* **报告属性**。 该模块可以设置报告的属性，以及解决方案后端可以读取和查询。 报告的属性以及所需属性中用于同步模块配置或条件。 
...
| modules.{moduleId}.restartPolicy | {"从不" \| "上失败" \| "上运行不正常" \| "始终"} | 是 |
...
最后一条信息，一份当前所需属性，可用于告知设备是否已应用最新的所需的属性或仍在运行以前的部署清单。
...
| lastDesiredStatus.code | 此状态代码是指由 IoT Edge 代理所看到的最后一个所需属性。 允许的值：`200` 成功、`400` 配置无效、`412` 架构版本无效、`417` 所需属性为空、`500` 失败 |
...
| systemModules.edgeHub.exitCode | 如果容器退出，IoT Edge 中心容器报告的退出代码 |
...
| modules.{moduleId}.exitCode | 如果容器退出模块容器报告的退出代码 |
...
| storeAndForwardConfiguration.timeToLiveSecs | 以秒为单位的时间的 IoT Edge 中心保留消息是否断开路由终结点，如果 IoT 中心或本地模块。 值可以是任何正整数。 | 是 |
...
| lastDesiredStatus.code | 状态代码，指由 IoT Edge 中心所看到的最后一个所需属性。 允许的值：`200` 成功、`400` 配置无效、`500` 失败 |
| lastDesiredStatus.description | 状态的文本说明。 |
| clients.{device or moduleId}.status | 此设备或模块的连接状态。 可能值：{"connected" \| "disconnected"}。 仅模块标识可以处于断开连接状态。 仅在连接时才会显示连接到 IoT Edge 中心的下游设备。 |
| clients.{device or moduleId}.lastConnectTime | 上次设备或模块连接。 |
| clients.{device or moduleId}.lastDisconnectTime | 最后一次设备或模块断开连接。 |
