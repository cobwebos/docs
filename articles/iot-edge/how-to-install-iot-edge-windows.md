...
检查过去 5 分钟的服务日志。 如果刚完成了 IoT Edge 运行时的安装，则可能会在运行**IoTEdge**和**IoTEdge**之间看到错误列表。 这些错误是预期的，因为服务在配置前尝试启动。 
...
| **RestartIfNeeded** | 无 | 此标志允许部署脚本在不提示的情况下重新启动计算机（如有必要）。 |
...
| 参数 | 接受的值 | 注释 |
| --------- | --------------- | -------- |
| **手动** | None | **开关参数**。 如果未指定设置类型，则手动为默认值。<br><br>声明你要提供设备连接字符串来手动预配设备 |
| **Dps** | None | **开关参数**。 如果未指定设置类型，则手动为默认值。<br><br>声明你要提供设备预配服务 (DPS) 范围 ID 和设备的注册 ID，以通过 DPS 进行预配。  |
...
### <a name="uninstall-iotedge"></a>卸载-IoTEdge

| 参数 | 接受的值 | 注释 |
| --------- | --------------- | -------- |
| **团队** | 无 | 此标志会强制卸载，以防上一次卸载尝试失败。 
| **RestartIfNeeded** | 无 | 此标志允许卸载脚本在不提示的情况下重新启动计算机（如有必要）。 |
