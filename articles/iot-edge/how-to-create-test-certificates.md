...
8. 验证是否已正确安装 OpenSSL，并确保现有证书不存在名称冲突。 如果出现问题，脚本输出应介绍如何在系统上修复这些问题。
...
可以通过两种方式使用 x.509 证书对 IoT 设备进行身份验证：使用自签名证书或使用证书颁发机构（CA）签名证书。 对于 x.509 自签名身份验证，有时称为指纹身份验证，则需要创建新的证书以放置在 IoT 设备上。
