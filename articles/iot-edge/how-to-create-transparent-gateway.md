...
充当网关的设备必须能够安全地连接到下游设备。 Azure IoT Edge 允许使用公钥基础结构 (PKI) 在设备之间建立安全连接。 在这种情况下，我们允许下游设备连接到用作透明网关的 IoT Edge 设备。 要维持合理的安全性，下游设备应确认网关设备的标识。 此标识检查可防止设备连接到潜在的恶意网关。
...
IoT Edge security daemon 使用 IoT Edge 设备 CA 证书对工作负荷 CA 证书进行签名，后者反过来为 IoT Edge hub 签署服务器证书。 在建立连接的过程中，网关将其服务器证书提供给下游设备。 下游设备将进行检查以确保服务器证书是汇总到根 CA 证书的证书链的一部分。 此过程允许下游设备确认网关是否来自受信任的源。 有关详细信息，请参阅[了解 Azure IoT Edge 如何使用证书](iot-edge-certs.md)。
...
利用此功能，本地模块或下游设备可以根据需要使用 IoT Edge 设备进行身份验证，并使用消息和方法彼此通信，甚至在从 IoT 中心断开连接时也是如此。 有关详细信息，请参阅[了解 IoT Edge 设备、模块和子设备的扩展脱机功能](offline-capabilities.md)。
