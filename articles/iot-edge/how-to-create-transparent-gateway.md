...
2. 使用以下命令创建 IoT Edge 设备 CA 证书和私钥。 提供 CA 证书的名称，例如 **MyEdgeDeviceCA**。 在生成证书的过程中，将使用该名称来为文件命名。 
...
1. 创建根 CA 证书和一个中间证书。 这些证书位于 *\<WRKDIR>* 中。
   ...
   如果已在此工作目录中创建了根证书和中间证书，请不要再次运行此脚本。 重新运行此脚本将覆盖现有证书。 而是，请继续下一步。 
