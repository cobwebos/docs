...
如果在载入用于 VM 的 Azure Monitor 之前，未为工作区中基于 Windows 或 Linux 的 VM [启用性能计数器收集](monitoring-vminsights-onboard.md?toc=/azure/azure-monitor/toc.json#performance-counters-enabled)，则需要遵循[此处](../log-analytics/log-analytics-data-sources-performance-counters.md?toc=/azure/azure-monitor/toc.json#configuring-performance-counters)所述的适用于 Windows 和 Linux 的步骤，来禁用这些规则。
