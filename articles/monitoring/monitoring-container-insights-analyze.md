---
title: 使用用于容器的 Azure Monitor（预览版）监视 AKS 群集性能 | Microsoft Docs
...
ms.locfileid: "50214148"
---
# <a name="understand-aks-cluster-performance-with-azure-monitor-for-containers-preview"></a>使用用于容器的 Azure Monitor（预览版）了解 AKS 群集性能
...
若要了解如何启用适用于容器的 Azure Monitor，请参阅[载入适用于容器的 Azure Monitor](monitoring-container-insights-onboard.md)。

Azure Monitor 提供一个多群集视图，显示在订阅中跨资源组部署的所有受监视 AKS 群集以及不受此解决方案监视的 AKS 群集的运行状况。  可以即时了解群集运行状况，并且可以从这里向下钻取到节点和控制器性能页，或者可以通过导航来查看群集的性能图表。  对于发现的标识为“不受监视”的 AKS 群集，可以随时选择为该群集启用监视功能。  

## <a name="sign-in-to-the-azure-portal"></a>登录到 Azure 门户
登录到 [Azure 门户](https://portal.azure.com)。 

## <a name="multi-cluster-view-from-azure-monitor"></a>从 Azure Monitor 获得的多群集视图 
若要查看已部署的所有 AKS 群集的运行状况，请在 Azure 门户的左窗格中选择“监视”。  在“见解”部分，选择“容器(预览)”。  

![Azure Monitor 多群集仪表板示例](./media/monitoring-container-insights-analyze/azmon-containers-multiview.png)

在“受监视的群集”选项卡上，可以了解以下情况：

1. 多少群集处于严重或不正常状态？多少群集处于正常或未报告状态（也称未知状态）？
2. 每个群集部署了多少节点、用户和系统 Pod？  

定义的运行状况如下： 

1. **正常** - VM 没有检测到任何问题，并且按要求运行。 
2. **严重** - 检测到一个或多个严重问题，需要解决这些问题才能按预期还原正常操作状态。
3. **警告** - 检测到一个或多个需要解决的问题，不解决这些问题可能会导致运行状况变得严重。
4. **未知** - 如果服务无法与节点或 Pod 建立连接，则状态将更改为未知状态。
...
可以直接从 AKS 群集访问适用于容器的 Azure Monitor，只需从左窗格中选择“见解(预览)”即可。 分四个视角查看有关 AKS 群集的信息：
...
单击“见解(预览)”时打开的默认页为“群集”，其中包括四个性能折线图，显示群集的主要性能指标。 

![“群集”选项卡上的性能图表示例](./media/monitoring-container-insights-analyze/containers-cluster-perfview.png)

性能图表显示四个性能指标：

- **节点 CPU 利用率 &nbsp;(%)**：从聚合视角反映整个群集的 CPU 利用率。 若要筛选出相关结果的时间范围，可通过图表上的百分位选择器选择“Avg”、“Min”、“Max”、“50th”、“90th”、“95th”（单选或复选均可）。 
- **节点内存利用率 &nbsp;(%)**：从聚合视角反映整个群集的内存利用率。 若要筛选出相关结果的时间范围，可通过图表上的百分位选择器选择“Avg”、“Min”、“Max”、“50th”、“90th”、“95th”（单选或复选均可）。 
...
![示例 Kubernetes 透视属性窗格](./media/monitoring-container-insights-analyze/perspectives-preview-pane-01.png)
...
![通过筛选器缩小结果范围的示例](./media/monitoring-container-insights-analyze/add-filter-option-01.png)
...
![用于数据筛选的百分位选择](./media/monitoring-container-insights-analyze/containers-metric-percentile-filter.png)

在以下示例中，请注意节点 *aks-nodepool1-*，其“容器”的值为 14，表示部署的容器总数。

![各节点容器数汇总示例](./media/monitoring-container-insights-analyze/containers-nodes-containerstotal.png)
...
![选择控制器视图](./media/monitoring-container-insights-analyze/containers-controllers-tab.png)
...
| 图标 | 状态 | 
|--------|-------------|
| ![“就绪”运行状态图标](./media/monitoring-container-insights-analyze/containers-ready-icon.png) | 正在运行（就绪）|
| ![“正在等待”或“已暂停”状态图标](./media/monitoring-container-insights-analyze/containers-waiting-icon.png) | “正在等待”或“已暂停”|
| ![“上次报告正在运行”状态图标](./media/monitoring-container-insights-analyze/containers-grey-icon.png) | 上次报告正在运行但已超过 30 分钟未响应|
| ![成功状态图标](./media/monitoring-container-insights-analyze/containers-green-icon.png) | 成功停止或无法停止|

状态图标显示的计数基于 Pod 提供的数据。 它显示最差的两个状态。将鼠标悬停在状态上时，它显示容器中所有 Pod 的汇总状态。 如果没有就绪状态，状态值会显示 **(0)**。 

在选择器中，选择“容器”。

![选择容器视图](./media/monitoring-container-insights-analyze/containers-containers-tab.png)

可查看 Azure Kubernetes 容器的性能运行状况。  
...
| ![“正在等待”或“已暂停”状态图标](./media/monitoring-container-insights-analyze/containers-waiting-icon.png) | “正在等待”或“已暂停”|  
| ![“上次报告正在运行”状态图标](./media/monitoring-container-insights-analyze/containers-grey-icon.png) | 上次报告正在运行但已超过 30 分钟未响应|  
| ![“已终止”状态图标](./media/monitoring-container-insights-analyze/containers-terminated-icon.png) | 成功停止或无法停止|  
| ![“已失败”状态图标](./media/monitoring-container-insights-analyze/containers-failed-icon.png) | “已失败”状态 |  
...
![在 Log Analytics 中分析数据](./media/monitoring-container-insights-analyze/container-health-log-search-example.png)   
