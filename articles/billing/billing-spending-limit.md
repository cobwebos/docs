...
Azure 支出限制用于限制你的 Azure 订阅支出额。 如果新客户注册包括几个月信用额度的试用版产品/服务，则这些客户都将默认开启支出限制。 支出限制为 $0。 无法进行更改。 支出限制不可用于即用即付订阅和承诺计划等订阅类型。 请参阅 [Azure 产品/服务以及支出限制可用性的完整列表](https://azure.microsoft.com/support/legal/offer-details/)。

## <a name="what-happens-when-i-reach-the-spending-limit"></a>达到支出限制后会发生什么情况？

因你的使用而产生的费用已经用完了产品/服务中包括的每月额度时，你部署的服务将在该计费月的剩余时间中被禁用。 例如，你部署的云服务会从生产中移除，你的 Azure 虚拟机将被停止并取消分配。 要阻止服务被禁用，可以选择删除支出限制。 当服务被禁用时，只能以只读模式向管理员提供存储帐户和数据库中的数据。 在下一计费月开始时，如果你的产品/服务包含多个月的额度，则将重新启用订阅。 然后就可重新部署云服务，并可完全访问存储帐户和数据库。

在免费试用版订阅达到支出限制后，可重新启用订阅，并使其在 90 天内自动[升级到标准的即用即付产品/服务](billing-upgrade-azure-subscription.md)。

点击产品/服务的支出限制时，即会收到通知。 登录 [Azure 帐户中心](https://account.windowsazure.com)，选择“帐户”，然后选择“订阅”。 你将看到已达到支出限制的订阅的相关通知。
...
支出限制会阻止你部署或使用某些应用商店及 Microsoft 服务。 有些情况应移除订阅上的支出限制。

- 计划部署第一方映像（如 Oracle）和服务（如 Visual Studio Team Services）。 此方案几乎立马导致你超过支出限制并导致订阅被禁。

- 你拥有不可中断的服务。

- 你的服务和资源包含你希望保留的设置，如虚拟 IP 地址。 这些设置将在取消分配服务和资源时丢失。


## <a name="remove-the-spending-limit"></a>移除支出限制

只要订阅关联了有效的付款方法，即可随时移除支出限制。 对于具有几个月信用额度的产品/服务，你可以在下个计费周期开始时重新启用支出限制。
