...
Azure 预订可帮助你通过提交到一年或三年的虚拟机、SQL 数据库计算容量、Azure Cosmos DB 吞吐量或其他 Azure 资源来节省资金。 提交可让你获取所用资源的折扣。 预订可将虚拟机、SQL 数据库计算、Azure Cosmos DB 或其他资源的成本大幅减少至即用即付价格的 72%。 预订提供计费折扣，并且不会影响资源的运行时状态。

你可以提前或按月为预留付费。 前期和月度预订的总成本相同，并且选择每月支付费用时，不会支付任何额外费用。 每月付款适用于 Azure 预留，而不是第三方产品。
...
- **SQL 数据仓库**-预订涵盖 cDWU 使用情况。 它不涉及与 SQL 数据仓库使用关联的存储或网络费用。
- **应用服务戳记费**-预订涵盖了邮票使用情况。 它不适用于工作人员，因此与 stamp 关联的任何其他资源将单独收费。
...
预留通过与订阅绑定的付款方式付款。 如果订阅为“企业”订阅，则虚拟机预留实例费用会从货币承诺余额中扣除。 当您的资金承诺余额不会涵盖预订费用时，您将对超额收费。 如果你有来自具有即用即付费率的单个计划的订阅，你的帐户中的信用卡将立即对提前购买付费。 月支付显示在发票上，信用卡按月收费。 当你通过发票进行计费时，你将看到下一张发票上的费用。
