...
您可以在 Azure 门户使用创建的预算[预算功能](../cost-management/tutorial-acm-create-budgets.md)在成本管理中。 或者，可以创建使用 REST Api、 Powershell cmdlet 的预算，也可以使用 CLI。 以下过程使用 REST API。 在调用 REST API 之前，需要授权令牌。 若要创建授权令牌，可使用 [ARMClient](https://github.com/projectkudu/ARMClient) 项目。 通过 ARMClient，可向 Azure 资源管理器进行身份验证，并获取令牌来调用 API  。
...
10. 选择“原始”按钮选项  。
11. 在文本框中，粘贴在下方示例预算定义中，但是您必须替换**subscriptionid**， **budgetname**，和**actiongroupname**参数替换你订阅 ID、 你的预算的唯一名称和 URL 和请求正文中创建的操作组名称：
