...
您可以使用成本管理中的[预算功能](../cost-management/tutorial-acm-create-budgets.md)在 Azure 门户中创建预算。 或者, 可以使用 REST Api、Powershell cmdlet 或使用 CLI 创建预算。 以下过程使用 REST API。 在调用 REST API 之前，需要授权令牌。 若要创建授权令牌，可使用 [ARMClient](https://github.com/projectkudu/ARMClient) 项目。 通过 ARMClient，可向 Azure 资源管理器进行身份验证，并获取令牌来调用 API。
...
10. 选择“原始”按钮选项。
11. 在文本框中, 粘贴以下示例预算定义, 但必须将**subscriptionid**、 **budgetname**和**actiongroupname**参数替换为你的订阅 ID、预算的唯一名称以及操作组名称同时在 URL 和请求正文中创建:
