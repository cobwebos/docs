...
可以使用成本管理中的[预算功能](../cost-management/tutorial-acm-create-budgets.md)在 Azure 门户中创建预算。 也可以使用 REST API、Powershell cmdlet 或 CLI 创建预算。 以下过程使用 REST API。 在调用 REST API 之前，需要授权令牌。 若要创建授权令牌，可使用 [ARMClient](https://github.com/projectkudu/ARMClient) 项目。 通过 ARMClient，可向 Azure 资源管理器进行身份验证，并获取令牌来调用 API  。
