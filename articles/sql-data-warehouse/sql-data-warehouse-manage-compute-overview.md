---
title: "管理 Azure SQL 数据仓库的计算能力（概述）| Microsoft 文档"
...
* 通过调整[数据仓库单位 (DWU)][数据仓库单位 (DWU)] 缩放计算能力
* 暂停或恢复计算资源

<a name="scale-performance-bk"></a>

## <a name="scale-performance"></a>缩放性能
在 SQL 数据仓库中，你可以快速地进行性能缩放，只需增加或减少 CPU 计算资源、内存和 I/O 带宽即可。 若要缩放性能，只需调整 SQL 数据仓库分配给数据库的[数据仓库单位 (DWU)][数据仓库单位 (DWU)] 数即可。 SQL 数据仓库可以快速地进行更改，并处理针对硬件或软件的所有基础更改。
...
若要弹性缩放计算能力，只需更改数据库的[数据仓库单位 (DWU)][数据仓库单位 (DWU)] 设置即可。 随着添加更多的 DWU，性能将呈线性增加。  在较高的 DWU 级别，若要显著改善性能，需添加 100 以上的 DWU。 为了方便你选择有意义的 DWU 增长数，我们提供了会给出最佳结果的 DWU 级别。

若要调整 DWU，可以使用以下任何单个方法。

* [通过 Azure 门户缩放计算能力][通过 Azure 门户缩放计算能力]
* [通过 PowerShell 缩放计算能力][通过 PowerShell 缩放计算能力]
* [通过 REST API 缩放计算能力][通过 REST API 缩放计算能力]
* [通过 TSQL 缩放计算能力][通过 TSQL 缩放计算能力]
...
* [通过 Azure 门户暂停计算][通过 Azure 门户暂停计算]
* [通过 PowerShell 暂停计算][通过 PowerShell 暂停计算]
* [ REST API 暂停计算][ REST API 暂停计算]
...
* [通过 Azure 门户恢复计算][通过 Azure 门户恢复计算]
* [通过 PowerShell 恢复计算][通过 PowerShell 恢复计算]
* [通过 REST API 恢复计算][通过 REST API 恢复计算]

## <a name="permissions"></a>权限
必须拥有 [ALTER DATABASE][ALTER DATABASE] 中所述的权限，才能缩放数据库。  必须拥有 [SQL DB 参与者][SQL DB 参与者]权限（具体说来，就是 Microsoft.Sql/servers/databases/action），才能暂停和恢复计算。
...
<!--Article references-->
[数据仓库单位 (DWU)]: ./sql-data-warehouse-overview-what-is.md#data-warehouse-units

[通过 Azure 门户缩放计算能力]: ./sql-data-warehouse-manage-compute-portal.md#scale-compute-bk
[通过 PowerShell 缩放计算能力]: ./sql-data-warehouse-manage-compute-powershell.md#scale-compute-bk
[通过 REST API 缩放计算能力]: ./sql-data-warehouse-manage-compute-rest-api.md#scale-compute-bk
[通过 TSQL 缩放计算能力]: ./sql-data-warehouse-manage-compute-tsql.md#scale-compute-bk

[容量限制]: ./sql-data-warehouse-service-capacity-limits.md

[通过 Azure 门户暂停计算]:  ./sql-data-warehouse-manage-compute-portal.md#pause-compute-bk
[通过 PowerShell 暂停计算]: ./sql-data-warehouse-manage-compute-powershell.md#pause-compute-bk
[ REST API 暂停计算]: ./sql-data-warehouse-manage-compute-rest-api.md#pause-compute-bk
...
[工作负荷和并发管理]: ./sql-data-warehouse-develop-concurrency.md
[表设计概述]: ./sql-data-warehouse-tables-overview.md
...
[表统计信息]: ./sql-data-warehouse-tables-statistics.md
[最佳实践]: ./sql-data-warehouse-best-practices.md 
[部署概述]: ./sql-data-warehouse-overview-develop.md

[SQL DB 参与者]: ../active-directory/role-based-access-built-in-roles.md#sql-db-contributor
