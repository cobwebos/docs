...
description: 用于开发解决方案的 Azure SQL 数据仓库中的 CREATE TABLE 为 SELECT （CTAS）语句的说明和示例。
...
选择 .。。INTO 不允许在操作过程中更改分布方法或索引类型。 使用 ROUND_ROBIN 的默认分布类型和聚集列存储索引的默认表结构创建 `[dbo].[FactInternetSales_new]`。
...
还可以使用 CTAS 来解决下面列出的一些不支持的功能。 此方法通常可以证明有用，因为您不仅能符合您的代码，而且在 SQL 数据仓库上运行速度会更快。 此性能是完全并行化设计的结果。 方案包括：
...
SQL 数据仓库在 `UPDATE` 语句的 `FROM` 子句中不支持 ANSI 联接，因此不能使用前面的示例而不进行修改。
...
有时，删除数据的最佳方法是使用 CTAS，尤其是对于使用 ANSI 联接语法的 `DELETE` 语句。 这是因为 SQL 数据仓库在 `DELETE` 语句的 `FROM` 子句中不支持 ANSI 联接。 选择要保留的数据，而不是删除数据。
...
请注意，列“result”沿用表达式的数据类型和可为 null 的值。 如果您不小心，则继续执行数据类型会导致值的细微差异。
...
这对于数据迁移非常重要。 尽管第二个查询可能更准确，但仍存在问题。 与源系统相比，数据会有所不同，这会在迁移中造成完整性问题。 这是“错误”答案其实是正确答案的极少见情况之一！
