...
SQL 数据仓库不支持在 `FROM` 语句的 `UPDATE` 子句中使用 ANSI Join，因此，只有在修改上述示例之后才能使用它。
...
有时，删除数据的最佳方法是使用 CTAS，特别是对于使用 ANSI Join 语法的 `DELETE` 语句。 这是因为，SQL 数据仓库不支持在 `FROM` 语句的 `DELETE` 子句中使用 ANSI Join。 无需删除数据，可以选择要保留的数据。
