---
title: 临时表
description: 在 Azure SQL 数据仓库中使用临时表的基本指南，重点介绍会话级别临时表的原则。
...
SQL 数据仓库在实现临时表时确实会施加一些限制。  目前，仅支持会话范围的临时表。  不支持全局临时表。  此外，不能在临时表上创建视图。  只能使用哈希分布或轮循机制分布来创建临时表。  不支持复制的临时表分布。 
