...
如果刚开始使用 Azure SQL 数据仓库，千万别让本文吓到。  主题的顺序主要按照重要性排列。  如果一开始就关注头几个概念，则效果应该不错。  如您获取更熟悉和适应了使用 SQL 数据仓库，请再回来看看其他概念。  融会贯通不需要很长时间。
...
另请参阅[管理表统计信息][Manage table statistics], [CREATE STATISTICS][CREATE STATISTICS]， [UPDATE STATISTICS][更新统计信息]
...
另请参阅[将数据加载][Load data], [Guide for using PolyBase][Guide for using PolyBase]， [Azure SQL 数据仓库加载模式和策略][Azure SQL 数据仓库加载模式和策略]， [使用 Azure 数据工厂加载数据][Load Data with Azure Data Factory], [Move data with Azure Data Factory][Move data with Azure Data Factory]， [CREATE EXTERNAL FILE FORMAT][CREATE EXTERNAL FILE FORMAT]，[创建表，如 select (CTAS)][Create table as select (CTAS)]
...
另请参阅[表概述][Table overview], [Table distribution][Table distribution]，[选择表分发][选择表分发]， [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
另请参阅[了解事务][Understanding transactions], [Optimizing transactions][Optimizing transactions]，[表分区][表分区]， [TRUNCATE TABLE][TRUNCATE TABLE] , [ALTER TABLE][ALTER TABLE]，[创建表，如 select (CTAS)][创建表，如 select (CTAS)]
...
另请参阅[表概述][Table overview], [Table data types][Table data types]， [CREATE TABLE][CREATE TABLE]
...
聚集列存储索引是将数据存储在 SQL 数据仓库中最有效率的方式之一。  默认情况下，SQL 数据仓库中的表将创建为聚集列存储。  为了让列存储表的查询获得最佳性能，良好的分段质量很重要。  当行在内存不足的状态下写入列存储表时，列存储分段质量可能降低。  压缩行组中的行数可以测量分段质量。  请参阅[索引质量不佳的列存储的原因][Causes of poor columnstore index quality]in the [Table indexes][Table indexes]一文，了解有关检测和改善聚集列存储表分段质量的分步说明。  由于高质量列存储段很重要，因此可以考虑使用用来加载数据的中型或大型资源类中的用户 ID。 使用较低的[数据仓库单位](what-is-a-data-warehouse-unit-dwu-cdwu.md)值意味着需要向加载用户分配较大的资源类。
...
另请参阅[表索引][Table indexes], [Columnstore indexes guide][Columnstore indexes guide]，[重新生成列存储索引][重新生成列存储索引]
