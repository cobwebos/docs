...
引入工作负荷组-LINK TBD 后，并发槽的概念将不再适用。  每个请求的资源按百分比分配，并在工作负荷组定义中指定。  但是，即使在删除并发槽后，每个查询所需的资源量最少取决于服务级别。  下表定义了各服务级别每个查询所需的最小资源量，以及可以实现的关联并发性。 

|服务级别|并发查询数上限|REQUEST_MIN_RESOURCE_GRANT_PERCENT 支持的最低%|
|---|---|---|
|DW100c|4|25%|
|DW200c|8|12.5%|
|DW300c|12|8|
