...
* 适用于 Visual Studio 的 SSDT。 如果你安装了 Visual Studio，则可能已有 SSDT for Visual Studio。 有关安装指说明和选项，请参阅 [安装 Visual Studio 和 SSDT](sql-data-warehouse-install-visual-studio.md)。
...
3. 将以下 T-sql 查询复制到查询窗口：
    ...
    ```
4. 通过单击绿色箭头来运行查询，或使用以下快捷方式： `CTRL`+`SHIFT`+`E`。
