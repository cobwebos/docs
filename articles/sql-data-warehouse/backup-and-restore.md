...
快照是创建还原点的服务的内置功能。 无需启用此功能。 但是，在创建还原点时，数据仓库应处于 "活动" 状态。 如果数据仓库定期暂停，可能无法创建自动还原点，因此请确保在暂停数据仓库之前创建用户定义的还原点。 用户当前无法删除自动还原点，因为服务使用这些还原点来维护要恢复的 Sla。
...
有关 SQL 数据仓库定价的详细信息，请参阅 [SQL 数据仓库定价](https://azure.microsoft.com/pricing/details/sql-data-warehouse/gen2/)。 跨区域还原时，不会对数据流出量收费。
