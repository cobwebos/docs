...
3. 编辑解压缩的 aw_create.bat 文件，并设置位于文件顶部的以下变量。  切勿在“=”和参数之间留有空格。  下面是编辑后的内容示例。
    ...
    ```
4. 从 Windows 命令提示符运行编辑过的 aw_create.bat。  确保所在的目录是保存了所编辑 aw_create.bat 版本的位置。
   此脚本将...
   
   * 删除所有 Adventure Works 表或所有已在数据库中的视图
...
将一些示例数据载入 SQL 数据仓库后，可以快速运行几个查询。  若要运行查询，请使用 Visual Studio 和 SSDT 连接到 Azure SQL DW 中新建的 Adventure Works 数据库，如[使用 Visual Studio 进行查询][query with Visual Studio]文档中所述。
