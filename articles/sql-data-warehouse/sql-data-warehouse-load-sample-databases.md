...
请遵循以下简单步骤，加载并查询 Adventure Works 示例数据库。 这些脚本首先使用 sqlcmd 运行 SQL，这会创建表和视图。 创建了表之后，脚本会使用 bcp 加载数据。  如果还没有安装 sqlcmd 和 bcp，请单击以下链接[安装 bcp][安装 bcp] 并 [安装 sqlcmd][安装 sqlcmd]。

## <a name="load-sample-data"></a>加载示例数据
1. 下载[适用于 SQL 数据仓库的 Adventure Works 示例脚本][适用于 SQL 数据仓库的 Adventure Works 示例脚本] zip 文件。
...
将一些示例数据载入 SQL 数据仓库后，你可以快速运行几个查询。  若要运行查询，请使用 Visual Studio 和 SSDT 连接到 Azure SQL DW 中新建的 Adventure Works 数据库，如[使用 Visual Studio 进行查询][使用 Visual Studio 进行查询]文档中所述。
...
SQL 数据仓库几乎支持 SQL Server 所能支持的所有 T-SQL 构造。  [迁移代码][迁移代码]文档中描述了两者的所有差别。

## <a name="next-steps"></a>后续步骤
现在你可以配合示例数据尝试一些查询，接下来请了解如何[开发][开发]、[加载][加载]或[迁移][迁移]到 SQL 数据仓库。
...
[安装 bcp]: sql-data-warehouse-load-with-bcp.md
[安装 sqlcmd]: sql-data-warehouse-get-started-connect-sqlcmd.md
