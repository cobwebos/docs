...
另请参阅[管理表统计信息][Manage table statistics], [CREATE STATISTICS][CREATE STATISTICS]、[更新统计]信息[更新统计信息]
...
另请参阅[表概述][Table overview], [Table distribution][Table distribution]、[选择表分布][选择表分布]、 [CREATE TABLE][CREATE TABLE], [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT]
...
另请参阅[了解事务][Understanding transactions], [Optimizing transactions][Optimizing transactions]、[表分区][表分区]、 [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE]、 [create table as select (CTAS)][create table as select (CTAS)]
...
聚集列存储索引是将数据存储在 SQL 数据仓库中最有效率的方式之一。  默认情况下，SQL 数据仓库中的表将创建为聚集列存储。  为了让列存储表的查询获得最佳性能，良好的分段质量很重要。  当行在内存不足的状态下写入列存储表时，列存储分段质量可能降低。  压缩行组中的行数可以测量分段质量。  有关检测和改善聚集列存储表的段质量的分步说明, 请参阅[列存储索引质量][Causes of poor columnstore index quality] in the [Table indexes][Table indexes]不佳的原因。  由于高质量列存储段很重要，因此可以考虑使用用来加载数据的中型或大型资源类中的用户 ID。 使用较低的[数据仓库单位](what-is-a-data-warehouse-unit-dwu-cdwu.md)值意味着需要向加载用户分配较大的资源类。
...
另请参阅[表索引][Table indexes], [Columnstore indexes guide][Columnstore indexes guide]、重新[生成列存储索引]重新[生成列存储索引]
