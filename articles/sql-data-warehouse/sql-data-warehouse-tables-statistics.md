<properties
   pageTitle="管理 SQL 数据仓库中表的统计信息 | Microsoft Azure"
...
有关详细信息，请参阅 MSDN 上的 [DBCC SHOW\_STATISTICS][]。若要了解详细信息，请参阅有关[表概述][Overview]、[表数据类型][Data Types]、[分布表][Distribute]、[为表编制索引][Index]、[将表分区][Partition]和[临时表][Temporary]的文章。有关最佳实践的详细信息，请参阅 [SQL 数据仓库最佳实践][]。
