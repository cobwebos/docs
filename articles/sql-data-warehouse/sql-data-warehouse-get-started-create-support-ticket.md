...
3. 选择您的 [Azure 支持计划][Azure 支持计划]。
   
   * **计费、配额和订阅管理**支持可用于所有支持级别。
   * **故障维修服务**支持通过[开发人员支持][开发人员支持]、[标准支持][标准支持]、[专业直接支持][专业直接支持]或[顶级支持][顶级支持]提供。故障维修服务问题是客户在使用 Azure 时遇到的确定是由 Microsoft 引起的问题。
   * 我们在[专业直接][专业直接]和[顶级][顶级]支持级别提供**开发人员辅导**与**咨询服务**。
     
     如果你购买了顶级支持计划，则还可以在 [Microsoft Premier 在线门户][Microsoft Premier 在线门户]中报告 SQL 数据仓库的相关问题。若要深入了解各种支持计划（包括范围、响应时间、定价等），请参阅 [Azure 支持计划][Azure support plan]。有关 Azure 支持的常见问题，请参阅 [Azure 支持常见问题][Azure 支持常见问题]。
...
6. 系统会在此支持票证中预先填充你的**联系信息**。请根据需要更新这些信息。
...
## 其他资源
此外，可在 [Stack Overflow][Stack Overflow] 或 [Azure SQL 数据仓库 MSDN 论坛][Azure SQL 数据仓库 MSDN 论坛]上与 SQL 数据仓库社区互动。
