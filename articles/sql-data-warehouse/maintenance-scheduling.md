...
使用维护计划可以选择一个方便接收新功能、升级和修补程序的时间范围。 请选择 7 天内的主要和辅助维护时段。 例如，主要时段为星期六 22:00 到星期日 01:00，辅助时段为星期三 19:00 到 22:00。 如果 SQL 数据仓库在主要维护时段无法执行维护，则它会尝试在辅助维护时段再次执行维护。 在主要时段和辅助时段期间都可能会进行服务维护。 若要确保所有维护操作快速完成，DW400(c) 和更低的数据仓库层无法完成指定的维护时段外的维护。
