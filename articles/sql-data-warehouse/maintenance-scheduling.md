...
使用维护计划可以选择一个方便接收新功能、升级和修补程序的时间范围。 请选择 7 天内的主要和辅助维护时段。 例如，主要时段为星期六 22:00 到星期日 01:00，辅助时段为星期三 19:00 到 22:00。 如果 SQL 数据仓库在主要维护时段无法执行维护，则它会尝试在辅助维护时段再次执行维护。 主和辅助 windows 过程中可能服务维护。
...
在执行所有维护事件之前，会提前 24 小时发送通知。 若要尽量减少实例停机时间，请确保数据仓库在选定的维护时限之前没有任何长时间运行的事务。 维护开始时，将取消所有活动会话。 非已提交读事务将回滚，并且您的数据仓库将会遇到短连接丢失。 针对数据仓库的维护完成后，你会立即收到通知。
