<properties
   pageTitle="Azure SQL Database 的定价层建议"
   description="在 Azure 门户中更改定价层时，提供的定价层建议将会推荐最适合用于运行现有 Azure SQL Database 负载的层。"
...
# SQL Database 定价层建议
 ...
 在 Azure 门户中更改定价层时，提供的定价层建议将会推荐最适合用于运行现有 Azure SQL Database 负载的层。
...
Azure 会通过评估 SQL Database 的历史资源使用量，来分析当前的性能和功能要求。此外，会根据数据库大小和启用的[业务连续性](https://msdn.microsoft.com/zh-cn/library/azure/hh852669.aspx)功能确定可接受的最低服务层。
...
根据具体数据库的详细信息，执行升级或降级通常不会立即发生。当数据库过渡到新层时，管理门户会提供通知；你也可以通过在 SQL Database 服务器的 master 数据库中查询 [sys.dm_operation_status (Azure SQL Database)](https://msdn.microsoft.com/zh-cn/library/dn270022.aspx) 视图，来监视升级状态。
