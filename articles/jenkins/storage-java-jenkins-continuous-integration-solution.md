...
* **URL 格式**：可使用以下 URL 格式对 Blob 寻址：
    ...
    在以上格式中，`storageaccount` 表示存储帐户的名称，`container_name` 表示容器的名称，而 `blob_name` 表示 Blob 的名称。 在容器名称中，可具有多个由正斜杠 **/** 分隔的路径。 用于本教程的示例容器是 MyJob，${BUILD**ID}/${BUILD**NUMBER} 用于通用虚拟路径，因此 Blob 具有以下格式的 URL **\_\_** ：
