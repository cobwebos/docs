...
keywords: 詹金斯， 蔚蓝， devops， 存储， cicd
ms.topic: article
ms.date: 08/13/2019
...
Jenkins 通过允许开发人员轻松地集成其代码更改以及自动和频繁地生成版本，实现了软件项目的持续集成，因此提高了开发人员的工作效率。 生成是版本控制的，并且可将生成项目上传到不同存储库中。 本文将演示如何将 Azure Blob 存储用作生成项目的存储库。 它还将演示如何从 Azure Blob 存储下载依赖项。
...
* 一个 Azure 存储帐户。 如果还没有存储帐户，可使用[创建存储帐户](../storage/common/storage-account-create.md)中的步骤创建一个。
...
3. 在“Microsoft Azure 存储帐户配置”**** 部分中：
   1. 输入存储帐户名称，可以从[Azure 门户](https://portal.azure.com)获取该名称。
   2. 输入存储帐户密钥，也可以从[Azure 门户](https://portal.azure.com)获取。
   3. 如果要使用全局 Azure 云，请对“BLOB 服务终结点 URL”使用默认值****。 如果使用其他 Azure 云，请使用存储帐户的[Azure 门户](https://portal.azure.com)中指定的终结点。 
   4. 选择“验证存储凭据”以验证存储帐户****。 
...
7. 对于“容器名称”****，请指定容器名称。 （如果上载生成项目时容器不存在，将创建该容器。您可以使用环境变量，因此对于此示例，请输入`${JOB_NAME}`为容器名称。
...
8. 对于此示例，请选择“默认将新容器设为公开”****。 （如果要使用私有容器，需要创建共享访问签名以允许访问，该内容超出本文讨论范围。 可在[使用共享访问签名 (SAS)](../storage/common/storage-sas-overview.md)中了解有关共享访问签名的详细信息。）
...
14. 成功完成此作业后，可通过打开公共 Blob 检查生成项目。
    ...
    4. 选择“容器”****。
    5. 选择名为 myjob 的容器，该名称是创建 Jenkins 作业时分配的作业名称的小写形式****。 在 Azure 存储中，容器名称和 Blob 名称都是小写的（并且区分大小写）。 在名为**myjob**的容器的 blob 列表中，您应该会看到**hello.txt**和**date.txt**。 复制这两项中任一项的 URL 并在浏览器中打开。 会看到已作为生成项目上传的文本文件。
...
4. 对于“Blob 名称”****，请指定 Blob 名称。 可以使用环境变量。 另外，在指定 Blob 名称的初始字母后，可以使用星号作为通配符。 例如 **，project\\*** 将指定名称以**项目**开头的所有 Blob。
...
* **URL 格式**：可使用以下 URL 格式对 Blob 寻址：
    ...
    （以上格式适用于全局 Azure 云。 如果使用其他 Azure 云，请使用[Azure 门户](https://portal.azure.com)中的终结点来确定 URL 终结点。
  
    在以上格式中，`storageaccount` 表示存储帐户的名称，`container_name` 表示容器的名称，而 `blob_name` 表示 Blob 的名称。 在容器名称中，可以有多个路径，由正向斜杠分隔。 **/** 用于本教程的示例容器是 MyJob，${BUILD\_ID}/${BUILD\_NUMBER} 用于通用虚拟路径，因此 Blob 具有以下格式的 URL********：
