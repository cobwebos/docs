---
title: 安全的 OPC UA 客户端和使用 Azure IoT OPC UA 证书管理的 OPC UA 服务器应用程序 |Microsoft Docs
description: 使用新的密钥对和证书使用 OPC 保管库保护 OPC UA 客户端和 OPC UA 服务器应用程序。
author: dominicbetts
ms.author: dobett
ms.date: 11/26/2018
ms.topic: conceptual
...
ms.locfileid: "58759378"
---
# <a name="secure-opc-ua-client-and-opc-ua-server-application"></a>保护 OPC UA 客户端和 OPC UA 服务器的应用程序 
Azure IoT OPC UA 证书管理，也称为 OPC 保管库是一个微服务，可以配置注册，然后管理证书生命周期的 OPC UA 服务器和客户端应用程序在云中。 本文介绍如何使用新密钥对安全 OPC UA 客户端和 OPC UA 服务器应用程序和证书使用 OPC 保管库。
...
OPC 保管库的脚本：
- 使用新的密钥对和证书使用 OPC 保管库保护 OPC UA 客户端和 OPC UA 服务器应用程序。
...
验证可以成功建立 OPC 客户端和 OPC PLC 之间的连接和 OPC 客户端可以成功地从 OPC PLC 读取数据。 应看到以下输出中 OPC 客户端日志输出：
```
...
```
如果看到此输出，然后 OPC PLC 是现在由于都有现在由 CA 和这两个信任证书所签名的此 CA 签名的证书信任 OPC 客户端，反之亦然。
...
- OPC 测试客户端将要使用 iot 中心直接方法调用和 OPC 方法调用来配置 OPC 发布服务器以发布/取消发布来自 OPC Testserver 的节点。
- 监视输出的错误消息。
