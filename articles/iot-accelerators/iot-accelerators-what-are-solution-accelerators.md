...
* [预测性维护](iot-accelerators-predictive-overview.md)
...
| [预测性维护](iot-accelerators-predictive-overview.md)   |是 |是 |-   |-   |是 |是 |是 |
...
> [!NOTE]
> 若要部署解决方案加速器并详细了解如何自定义该解决方案，请访问 [Microsoft Azure IoT 解决方案加速器](https://www.azureiotsolutions.com/)。
...
> [!NOTE]
> 若要详细了解在远程监视解决方案加速器中部署的资源，请参阅[远程监视体系结构选择](iot-accelerators-remote-monitoring-architectural-choices.md)。
...
* [Azure 事件中心](../active-directory-domain-services/index.yml)。 可以通过此服务将大量事件引入到云。 请参阅 [Azure IoT 中心与 Azure 事件中心的比较](../iot-hub/iot-hub-compare-event-hubs.md)。
...
* [Azure 流分析](../stream-analytics/index.yml)。 预测性维护解决方案加速器使用此服务来处理传入遥测、执行聚合并检测事件。 此预配置解决方案也会使用流分析来处理包含数据（例如元数据或来自设备的命令响应）的信息消息。
...
有关 IoT 解决方案体系结构的详细信息，请参阅 [Microsoft Azure IoT 服务：参考体系结构](https://aka.ms/iotrefarchitecture)。
