...
- **审批者**：审批者角色分配给用户以批准或拒绝证书申请。 该角色不包括任何其他角色。
...
- **Key Vault 标准**：用于存储 web 服务的机密和 Azure Cosmos DB 密钥。
- **Key Vault 高级**：承载颁发者 CA 密钥、用于签名服务以及保管库配置和应用程序私钥的存储。
- **Azure Cosmos DB**：应用程序和证书请求的数据库。 
...
- 证书生存期
  ...
  - 子 CA 或联机颁发者 CA 证书：处于联机状态且仅颁发订户证书的 Ca 的最大证书有效期不得超过6年。 对于这些 Ca，相关的专用签名密钥的使用时间不得超过3年，无法颁发新证书。<br>
    > [!IMPORTANT]
...
- **生存期**：处于联机状态且仅颁发订户证书的 Ca 的最大证书有效期不得超过6年。 对于这些 Ca，相关的专用签名密钥的使用时间不得超过3年，无法颁发新证书。
