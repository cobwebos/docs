...
方括号包含将在部署过程中计算的表达式。该模板中括在括号中的表达式用于在部署期间获取参数值。你可以使用更多表达式，你将在本文的后面部分看到其他表达式的示例。有关完整列表，请参阅 [Azure Resource Manager 模板函数](resource-group-template-functions.md)。
