...
如果在本文的任何位置需要更多帮助，可以联系 MSDN Azure 上的 Azure 开发测试实验室（DTL）专家[并 Stack Overflow 论坛](https://azure.microsoft.com/support/forums/)。 或者，也可以提出 Azure 支持事件。 转到[Azure 支持站点](https://azure.microsoft.com/support/options/)并选择 "获取支持"。   
...
- **检查默认实验室的存储帐户**（即创建实验室时创建的第一个存储帐户，其名称通常以字母 "a" 开头，以 "a" 开头，以 "a" 为一个数字，即 \<labname\> #）。
    1. 导航到实验室的资源组。
