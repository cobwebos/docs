...
- Internet Information Server （IIS）的[应用程序路由请求](/iis/extensions/planning-for-arr/using-the-application-request-routing-module)模块可用于将请求重定向 `https://{gateway-hostname}/api/host/{lab-machine-name}/port/{port-number}` 到 azure 函数，后者处理请求以获取用于身份验证的令牌。
