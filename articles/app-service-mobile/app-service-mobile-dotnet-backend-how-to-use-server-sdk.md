...
如果正在开始新项目，可以使用 [Azure 门户]或 Visual Studio 创建应用服务应用程序。本部分将帮助用户使用其中一种方式创建新的移动应用程序后端，用于托管简单的待办事项列表 API。可以在本地运行此操作，或将项目发布到基于云的应用服务移动应用。
