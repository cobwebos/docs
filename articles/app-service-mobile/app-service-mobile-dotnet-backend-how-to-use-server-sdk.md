...
## <a name="how-to-create-a-net-mobile-app-backend"></a><a name="create-app"></a>如何创建 .NET 移动应用后端
...
1. 打开“新建项目”**** 对话框（从“文件”**** > “新建”**** > “项目...”****）。
2. 展开**模板** > **可视化 C#，** 然后选择**Web**。
3. 选择“ASP.NET Web 应用程序”****。
4. 填写项目名称。 然后单击“确定”****。
5. 在“ASP.NET 4.5.2 模板”** 下，选择“Azure 移动应用”****。 选中“云中的主机”****，在云中创建移动后端（可在其中发布此项目）。
6. 单击“确定”。

## <a name="how-to-download-and-initialize-the-sdk"></a><a name="install-sdk"></a>如何下载并初始化 SDK
SDKNuGet.org[上可用]。此包包括开始使用 SDK 所需的基本功能。 若要初始化该 SDK，需要对 **HttpConfiguration** 对象执行操作。
...
初始化 .NET 后端服务器项目的方式类似其他 ASP.NET 项目，可通过包含 OWIN 启动类来完成。 确保已引用 NuGet 包 `Microsoft.Owin.Host.SystemWeb`。 要在 Visual Studio 中添加此类，请右键单击服务器项目并选择"**添加新** >
**项目**"，然后选择**Web** > **常规** > **OWIN 启动类**。  将生成具有以下属性的类：
...
## <a name="how-to-publish-the-server-project"></a><a name="publish-server-project"></a>如何发布服务器项目
...
3. 如果要求选择发布目标，请单击**Microsoft Azure 应用服务** > **下一步**，然后（如果需要）使用 Azure 凭据登录。
...
## <a name="how-to-define-a-table-controller"></a><a name="define-table-controller"></a>如何定义表控制器
...
### <a name="how-to-adjust-the-table-paging-size"></a><a name="adjust-pagesize"></a>如何调整表分页大小
...
1. 在 Visual Studio 中，右键单击"控制器"文件夹，然后单击"**添加** > **控制器**"，选择**Web API 2 控制器&mdash;为空**，然后单击"**添加**"。
...
* [如何：检索到经过身份验证的用户信息](#user-info)
* [如何限制已获授权用户的数据访问](#authorize)

### <a name="how-to-add-authentication-to-a-server-project"></a><a name="add-auth"></a>如何将身份验证添加到服务器项目
...
### <a name="how-to-use-custom-authentication-for-your-application"></a><a name="custom-auth"></a>如何对应用程序使用自定义身份验证
> [!IMPORTANT]
...
### <a name="how-to-retrieve-authenticated-user-information"></a><a name="user-info"></a>如何检索经过身份验证的用户信息
...
### <a name="how-to-restrict-data-access-for-authorized-users"></a><a name="authorize"></a>如何限制已获授权用户的数据访问
...
## <a name="how-to-enable-targeted-push-using-tags"></a><a name="tags"></a>如何使用标记启用目标推送
...
## <a name="how-to-send-push-notifications-to-an-authenticated-user"></a><a name="push-user"></a>如何将推送通知发送到经过身份验证的用户
...
在本地应用程序配置中使用*authSigningKey*参数的签名密钥。 移动后端现在已具备在本地运行时验证令牌的功能，客户端将从基于云的终结点获取令牌。
