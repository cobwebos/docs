...
---
# <a name="connect-to-azure-storage-in-your-xamarinforms-app"></a>在 Xamarin.Forms 应用中连接到 Azure 存储
...
本主题演示如何向移动应用待办事项列表快速入门项目中添加图像支持。 必须先完成[创建 Xamarin.Forms 应用]教程。
...
3. 导航到移动应用后端。 在“所有设置” -> “应用程序设置” -> “连接字符串”下，创建名为 `MS_AzureStorageAccountConnectionString` 的新密钥并使用从存储帐户复制的值。 使用“自定义”作为密钥类型。

## <a name="add-a-storage-controller-to-the-server"></a>向服务器添加存储控制器
...
6. 更新 Web API 配置，设置属性路由。 在 **Startup.MobileApp.cs** 中，将以下行添加到 `ConfigureMobileApp()` 方法，在 `config` 变量的定义后：
...
## <a name="update-your-client-app-to-add-image-support"></a>更新客户端应用，添加图像支持
