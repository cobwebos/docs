...
此时会列出计划的作业以及迁移之前指定的频率。  按需作业会被禁用。  若要运行按需作业，请执行以下操作：
...
* 对于基于 .NET 的服务器项目，新的 [Mobile Apps SDK NuGet packages](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Server/)（移动应用 SDK NuGet 包）在 NuGet 依赖关系上具有更大的灵活性。  这些包支持新的应用服务身份验证，并由任何 ASP.NET 项目组成。 要了解升级的详细信息，请参阅[将现有 .NET 移动服务升级为应用服务](app-service-mobile-net-upgrading-from-mobile-services.md)。
