...
以下代码将获取对表（包含 TodoItem 数据）的引用，你可以将该引用用于后续操作以便读取和更新数据表。你将需要 TodoItem 类，并将其属性设置为解释移动服务为响应你的查询而发送的 JSON。
...
如果你的表权限已设置为**"具有应用程序密钥的任何人"**，则此代码有效。如果您更改权限以保护您的移动服务，您将需要添加用户身份验证支持。请参阅[身份验证入门](http://azure.microsoft.com/zh-cn/documentation/articles/mobile-services-dotnet-backend-windows-universal-dotnet-get-started-users/)。
...
以下代码将查询表以获取所有项目。请注意，它仅返回数据的第一页，默认为 50 个项目。因为它是一个可选的参数，你可以传入所需的页大小。
...
更新数据表中的行。参数项是要更新的 TodoItem 对象。

	await todoTable.UpdateAsync(item);

#####删除条目

删除数据库中的行。参数项是要删除的 TodoItem 对象。
