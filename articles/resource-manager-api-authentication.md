...
请调用 [Resource Manager 角色定义 API](https://msdn.microsoft.com/library/azure/dn906879.aspx) 列出所有 Azure RBAC 角色，然后搜索并逐一查看结果，按名称找到所需的角色定义。
...
现已做好一切准备，可以使用 [Resource Manager 创建角色分配](https://msdn.microsoft.com/library/azure/dn906887.aspx) API 将相应的 RBAC 角色分配到服务主体。
...
若要检查应用程序是否具有 Azure 订阅上的所需访问权限，也可以调用 [Resource Manager 权限](https://msdn.microsoft.com/library/azure/dn906889.aspx) API。此方式类似于用于确定用户是否具有订阅访问管理权限的方式。不过，这次请使用上一步骤中收到的仅限应用的访问令牌来调用权限 API。
