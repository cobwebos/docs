<properties linkid="mobile-services-dotnet-backend-hybrid-connections-get-started" urlDisplayName="Connect to an on-premises SQL Server from an Azure mobile service using Hybrid Connections" pageTitle="使用混合连接从 Azure 移动服务连接到本地 SQL Server - Azure 移动服务" metaKeywords="" description="了解如何使用混合连接从 Azure 移动服务连接到本地 SQL Server" metaCanonical="" services="" documentationCenter="Mobile" title="Connect to an on-premises SQL Server from an Azure mobile service using Hybrid Connections" authors="yavorg" solutions="" manager="" editor="mollybos" />
...
有关如何配置 SQL Server 以使其符合上述条件的详细说明，请参阅[安装 SQL Server Express，启用 TCP/IP 并在本地创建一个 SQL Server 数据库](/zh-cn/documentation/articles/web-sites-hybrid-connection-connect-on-premises-sql-server#InstallSQL)。如果已在符合上述条件的配置和环境中安装了 SQL Server，则可以跳到后面的部分，[开始在本地创建 SQL Server 数据库](/zh-cn/documentation/articles/web-sites-hybrid-connection-connect-on-premises-sql-server#CreateSQLDB)。 
...
1. 在门户的"移动服务"选项卡中，选择现有的移动服务或创建一个新的移动服务。 
	...
	>[WACOM.NOTE]请务必选择使用 .NET 后端创建的服务，或者创建一个新的 .NET 后端移动服务。若要了解如何创建新的 .NET 后端移动服务，请参阅[移动服务入门](/zh-cn/documentation/articles/mobile-services-dotnet-backend-windows-store-dotnet-get-started/) 
...
3. 在 Visual Studio 2013 中，打开定义了基于 .NET 的移动服务的项目。 
	...
	若要了解如何下载 .NET 后端项目，请参阅[移动服务入门](/zh-cn/documentation/articles/mobile-services-dotnet-backend-windows-store-dotnet-get-started/) 。
