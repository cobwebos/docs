<properties
	pageTitle="在 Azure HDInsight 中将 Python 与 Hive 和 Pig 配合使用"
	description="了解如何在 Azure HDInsight 中通过 Hive 和 Pig 使用 Python 用户定义的函数 (UDF)。"
	services="hdinsight"
	documentationCenter=""
	authors="Blackmist"
	manager="paulettm"
...
3. 在处理完成后，必须将输出以单行形式写入到 STDOUT，并在每两个字段之间提供一个制表符。这可以通过使用  `print "\t".join([clientid, phone_label, hashlib.md5(phone_label).hexdigest()])` 来完成。

4. 这都出现在  `while` 循环中，该循环将重复到不读取 `line`为止，此时， `break` 将退出循环，并且脚本将终止。
...
请参阅[运行示例](#running) ，以了解有关如何在 HDInsight 群集上运行本示例的信息。
...
3. 示例数据 **sample.log** 基本上符合我们要返回的日期、时间、类名、级别和详细信息架构。但是，它还包含了一些以字符串"*java.lang.Exception*"开头的行，我们需要修改这些行，使之与架构匹配。<b>if</b> 语句将检查这些行，然后调整输入数据以将"<i>java.lang.Exception</i>"字符串移到末尾，使数据与预期的输出架构相一致。
...
1. 使用 Python 示例[streaming.py](#streamingpy) 和 [jython.py](#jythonpy)，在你的开发计算机上创建文件的本地副本。
...
4. 从 SSH 会话中，将以前上载的 Python 文件添加到云的 WASB 存储。
...
1. 使用  `hive` 命令来启动 hive shell。在 shell 加载后，你应该看到  `hive>` 提示符。
...
1. 使用  `pig` 命令来启动该 shell。在 shell 加载后，你应该看到  `grunt>` 提示符。

2. 在  `grunt>` 提示符处输入以下语句。
...
3. 输入以下行后，作业将会启动。最后，它将返回类似于以下内容的输出。
...
2. 使用以下 PowerShell 脚本将 **streaming.py** 和 **jython.py** 文件上载到服务器。更改 Azure HDInsight 群集的名称，并在脚本的前三行中更改 **streaming.py** 和 **jython.py** 文件的路径。
	...
	> [AZURE.NOTE] [在 HDInsight 中为 Hadoop 作业上载数据](hdinsight-upload-data) 文档中介绍了上载脚本的其他方法。
