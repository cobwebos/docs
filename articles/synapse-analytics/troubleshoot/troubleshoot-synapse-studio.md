...
## <a name="sql-on-demand-preview-service-connectivity-issue"></a>SQL 点播（预览版）服务连接问题
...
> [!NOTE] 
>    以下故障排除步骤适用于 Chromium Edge 和 Chrome。 您可以使用其他浏览器（如 FireFox）来执行相同的故障排除步骤，但 "开发人员工具" 窗口可能与此 TSG 中的屏幕截图具有不同的布局。 如果可能，请不要使用传统边缘进行故障排除，因为在某些情况下可能会显示不准确的信息。
...
![网络-面板](media/troubleshooting-synapse-studio/network-panel.png)
...
- 状态以 "（failed）" 开头，扩大 "Status" 列或将鼠标指针悬停在状态文本上，以查看完整的文本。 打开支持票证时，包括文本和/或屏幕截图。
...
- 状态为 "40 倍"、"50x" 或其他数字，请选择项以查看详细信息。 你应看到右侧的项详细信息。 找到 "响应标头" 部分;然后检查是否存在名为 "访问控制-允许-源" 的项。 如果是这样，请检查它是否具有以下值之一：
...
![项-详细信息](media/troubleshooting-synapse-studio/item-details.png)
...
![开发人员-工具-控制台](media/troubleshooting-synapse-studio/developer-tool-console.png)

如果要附加屏幕快照，请提供拍摄屏幕截图的时间（或估计时间范围）。 这将有助于我们查看问题。

某些浏览器支持在 "控制台" 选项卡中显示时间戳。对于 Chromium Edge/Chrome，请打开 "开发人员工具" 中的 "设置" 对话框，并选中 "首选项" 选项卡中的 "显示时间戳"。

![开发人员-工具-控制台设置](media/troubleshooting-synapse-studio/developer-tool-console-settings.png)
