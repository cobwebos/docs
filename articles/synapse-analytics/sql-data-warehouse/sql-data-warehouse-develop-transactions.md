...
正如您所料，SQL 池支持将事务作为数据仓库工作负荷的一部分。 但是，若要确保在大规模维护 SQL 池，与 SQL Server 相比，某些功能会受到限制。 本文重点介绍它们之间的差异。
...
> [!NOTE]
> XACT_STATE 函数使用 -2 表示失败的事务，以代表 SQL Server 中不同的行为。 SQL Server 使用值 -1 来代表无法提交的事务。 SQL Server 可以容忍事务内的某些错误，而无需将其标记为无法提交。 例如，会`SELECT 1/0`导致错误，但不强制事务进入不可提交状态。

SQL Server 还允许读取无法提交的事务。 但是，SQL 池不允许这样做。 如果在 SQL 池事务内部发生错误，它会自动进入-2 状态，并且在语句回滚之前，您将无法再执行任何 select 语句。

因此，请务必检查你的应用程序代码是否使用 XACT_STATE （），因为你可能需要进行代码修改。
...
Msg 111233, Level 16, State 1, Line 1 111233；当前事务已中止，所有挂起的更改都已回退。 出现此问题的原因是，在 DDL、DML 或 SELECT 语句之前，仅回滚状态中的事务没有显式回滚。
...
还值得注意的是，SQL 池不实现或支持 ERROR_LINE （）函数。 如果你在代码中具有此项，则需要将其删除才能符合 SQL 池。
...
若要了解有关优化事务的详细信息，请参阅[事务最佳做法](sql-data-warehouse-develop-best-practices-transactions.md)。 若要了解有关其他 SQL 池最佳实践的信息，请参阅[sql 池最佳实践](sql-data-warehouse-best-practices.md)。
