---
title: 定义数据类型
description: 用于在 Synapse SQL 池中定义表数据类型的建议。
...
如果使用 PolyBase 外部表加载表，则定义的表行长度不能超过 1 MB。 当数据长度可变的行超过 1 MB 时，可使用 BCP 而不是 PolyBase 加载行。

## <a name="identify-unsupported-data-types"></a>识别不支持的数据类型

如果要从另一个 SQL 数据库迁移数据库，则可能会发现 SQL 池不支持的数据类型。 使用以下查询在现有 SQL 架构中发现不受支持的数据类型：
...
下面的列表显示了 SQL 池不支持的数据类型，并提供了不支持的数据类型的有用方法。
