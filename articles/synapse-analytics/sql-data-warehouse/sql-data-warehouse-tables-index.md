...
| [COMPRESSED_rowgroup_rows_DELETED] |逻辑上被删除的列存储格式的行数。 如果该数值相对于表的大小来说是高的，请考虑重新创建分区或重新生成索引，因为这会以物理方式删除它们。 |
...
超出每个分区对齐分布区的 102,400 行批量阈值的批次更新和插入操作将直接写入列存储格式。 但是，假设在平均分布的情况下，需要在单个操作中修改超过 6.144 百万行才发生这种情况。 如果给定的分区对齐分布区的行数小于 102400，则这些行将转到增量存储并保留在那里，直到插入或修改了足够的行以关闭行组或重建了索引。
