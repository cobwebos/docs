...
**验证 DTU 容量。** 每个 SQL 池都由具有默认 DTU 配额的[逻辑 SQL server](../../azure-sql/database/logical-servers.md) （例如，myserver.database.windows.net）托管。  验证服务器是否有足够的剩余 DTU 配额来还原数据库。 若要了解如何计算所需 DTU 或请求更多的 DTU，请参阅[请求 DTU 配额更改](sql-data-warehouse-get-started-create-support-ticket.md)。

## <a name="restore-a-deleted-data-warehouse-through-powershell"></a>通过 PowerShell 还原已删除的数据仓库

若要还原已删除的 SQL 池，请使用 [Restore-AzSqlDatabase](/powershell/module/az.sql/restore-azsqldatabase?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json) cmdlet。 如果还删除了相应的服务器，则无法还原该数据仓库。
...
5. 获取特定的已删除数据仓库。
6. 还原已删除的 SQL 池
    1. 若要将已删除的 SQL 池还原到其他服务器，请确保指定其他服务器名称。  此服务器还可以位于不同的资源组和区域。
