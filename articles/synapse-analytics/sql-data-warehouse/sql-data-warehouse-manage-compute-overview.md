...
暂停计算资源会导致存储层从计算节点分离。 计算资源将从您的帐户中释放。 暂停计算资源时，不会产生计算费用。 恢复计算资源会将存储重新附加到计算节点，并且恢复计算费用。 暂停 SQL 池时：
...
缩放 SQL 池需要 ALTER[数据库](/sql/t-sql/statements/alter-database-azure-sql-data-warehouse)中描述的权限。  暂停和恢复需要 [SQL DB 参与者](../../role-based-access-control/built-in-roles.md#sql-db-contributor)权限，具体而言是 Microsoft.Sql/servers/databases/action 权限。
