...
Azure 同步分析使用服务器级 IP 防火墙规则。 不支持数据库级 IP 防火墙规则。 有关详细信息，请参阅 Azure [SQL 数据库防火墙规则](../../sql-database/sql-database-firewall-configure.md?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json)
...
如需详细了解这些额外的角色，以及如何在 SQL 数据库上进行身份验证，请参阅[在 Azure SQL 数据库中管理数据库和登录名](../../sql-database/sql-database-manage-logins.md?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json)。  有关使用 Azure 活动目录进行连接的详细信息，请参阅[使用 Azure 活动目录身份验证进行连接](sql-data-warehouse-authentication.md)。
...
有关使用不同协议连接到仓库的详细信息和示例，请参阅连接到 SQL[池](../sql/connect-overview.md)。
