---
title: 教程：加载纽约出租车数据
description: 教程使用 Azure 门户和 SQL Server Management Studio 从用于 Synapse SQL 的 Azure blob 加载纽约出租车数据。
...
ms.locfileid: "85207543"
---
# <a name="tutorial-load-the-new-york-taxicab-dataset"></a>教程：加载纽约出租车数据集

本教程使用[COPY 语句](https://docs.microsoft.com/sql/t-sql/statements/copy-into-transact-sql?view=azure-sqldw-latest)从 Azure blob 存储帐户加载纽约出租车数据集。 本教程使用 [Azure 门户](https://portal.azure.com)和 [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms?toc=/azure/synapse-analytics/sql-data-warehouse/toc.json&bc=/azure/synapse-analytics/sql-data-warehouse/breadcrumb/toc.json&view=azure-sqldw-latest) (SSMS) 执行以下操作：
...
2. 在后续的快速入门中，请复制此完全限定的服务器名称，将其用于连接到服务器及其数据库。 然后选择服务器名称以打开服务器设置。
...
你现在可以使用此 IP 地址连接到服务器及其数据仓库。 可从 SQL Server Management Studio 或另一种所选工具进行连接。 连接时，请使用之前创建的 ServerAdmin 帐户。  
...
已准备好开始将数据加载到新的数据仓库。 本教程的此部分介绍如何使用 COPY 语句从 Azure 存储 blob 加载纽约市出租车 cab 数据集。 若要了解如何将数据置于 Azure Blob 存储或直接从源加载数据以供将来参考，请参阅[加载概述](design-elt-data-loading.md)。
...
> [!NOTE]
> 本教程直接将数据加载到最终表。 通常会将加载到生产工作负荷的临时表中。 数据在临时表中时，可以执行任何必要的转换。 
...
在本教程中，已学习了如何创建数据仓库以及用于加载数据的用户。 你使用了简单的[COPY 语句](https://docs.microsoft.com/sql/t-sql/statements/copy-into-transact-sql?view=azure-sqldw-latest#examples)将数据加载到数据仓库中。
