---
title: 而不是 ETL，为突触 SQL 池设计 ELT |微软文档
...
---
# <a name="designing-a-polybase-data-loading-strategy-for-azure-synapse-sql-pool"></a>为 Azure 突触 SQL 池设计 PolyBase 数据加载策略
...
虽然 SQL 池支持许多加载方法，包括非多边形选项（如 BCP 和 SQL BulkCopy API），但加载日期最快、最可扩展的方式是通过 PolyBase。  PolyBase 是这样一种技术，可以通过 T-SQL 语言访问存储在 Azure Blob 存储或 Azure Data Lake Storage 中的外部数据。
...
实现 SQL 池的 PolyBase ELT 的基本步骤包括：

1. 将源数据提取到文本文件中。
2. 将数据移入 Azure Blob 存储或 Azure Data Lake Store。
3. 准备要加载的数据。
4. 使用 PolyBase 将数据加载到 SQL 池暂存表中。
5. 转换数据。
6. 将数据插入生产表。
...
- 设置文本文件中的数据的格式，以便与 SQL 池目标表中的列和数据类型对齐。 外部文本文件与数据仓库表中的数据类型不相符会导致在加载期间拒绝行。
...
## <a name="4-load-the-data-into-sql-pool-staging-tables-using-polybase"></a>4. 使用 PolyBase 将数据加载到 SQL 池暂存表中
...
- [具有 Azure 数据砖块的 PolyBase](../../azure-databricks/databricks-extract-load-sql-data-warehouse.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json)将数据从 SQL 数据仓库表传输到数据砖块数据框和/或使用 PolyBase 将数据从数据块数据帧写入 SQL 数据仓库表。

### <a name="non-polybase-loading-options"></a>非 PolyBase 加载选项

如果数据与 PolyBase 不兼容，可以使用 [bcp](/sql/tools/bcp-utility?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json&view=azure-sqldw-latest) 或 [SQLBulkCopy API](/dotnet/api/system.data.sqlclient.sqlbulkcopy?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json)。 bcp 直接加载到 SQL 池而不经过 Azure Blob 存储，并且仅用于小负载。 请注意，这些选项的加载性能明显低于 PolyBase。
