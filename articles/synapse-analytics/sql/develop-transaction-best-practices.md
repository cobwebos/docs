---
title: 优化 SQL 池的事务
description: 了解如何在 SQL 池（数据仓库）中优化事务代码的性能，同时将长期回滚的风险降至最低。
...
了解如何优化 SQL 池中的事务代码的性能，同时将长期回滚的风险降至最低。

## <a name="transactions-and-logging"></a>事务和日志记录

事务是关系数据库引擎的一个重要组成部分。 SQL 池在数据修改期间使用事务。 这些事务可以是显式或隐式。 单个 INSERT、UPDATE 和 DELETE 语句都是隐式事务的示例。 显式事务使用 BEGIN TRAN、COMMIT TRAN 或 ROLLBACK TRAN。 显式事务通常用于多个修改语句需要绑定在单个原子单元的时候。

SQL 池使用事务日志向数据库提交更改。 每个分布区都具有其自己的事务日志。 事务日志写入都是自动的。 无需任何配置。 尽管此过程可保证写入，但它确在系统中引入一项开销。 编写事务性高效的代码，可以尽量减少这种影响。 事务性高效的代码大致分为两类。
...
> [!IMPORTANT]
> SQL 池有 60 个分发。 因此，假设所有行均匀分布且处于单个分区中，批在写入到聚集列存储索引时会需有 6,144,000 行（或更多）要按最少记录的方式记入日志。 如果对表进行分区且正插入的行跨越分区边界，则每个分区边界都需 6,144,000 行，假定数据分布很均匀。 每个分布区的每个分区各自必须超过 102,400 行的阈值，从而使插入以最少记录的方式记录到分布区中。
...
> [!NOTE]
> 使用 SQL 池工作负载管理功能可以重新创建大型表。 有关详细信息，请参阅[用于工作负荷管理的资源类](../sql-data-warehouse/resource-classes-for-workload-management.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json)。
...
Azure 同步分析允许您按需[暂停、恢复和扩展](../sql-data-warehouse/sql-data-warehouse-manage-compute-overview.md?toc=/azure/synapse-analytics/toc.json&bc=/azure/synapse-analytics/breadcrumb/toc.json)SQL 池。 

当您暂停或扩展 SQL 池时，请务必了解任何在途事务都立即终止;导致回滚任何打开的事务。 

如果工作负荷在暂停或缩放操作前已发出数据修改在长时间运行之后仍未完成的指示，则需要撤消此项工作。 此撤消可能会影响暂停或缩放 SQL 池所需的时间。 
...
请参阅[SQL 池中的事务](develop-transactions.md)，了解有关隔离级别和事务限制的更多内容。  有关其他最佳做法的概述，请参阅[SQL 池最佳实践](best-practices-sql-pool.md)。
