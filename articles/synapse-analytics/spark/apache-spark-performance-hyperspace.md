...
超空间引入了 Apache Spark 用户为其数据集创建索引的能力 (例如 CSV、JSON、Parquet 等，) 并利用它们来实现潜在的查询和工作负荷加速。

在本文中，我们重点介绍了超空间的基本知识，重点介绍其简易性，并说明了它是如何由任何人使用的。

免责声明：超空间有助于在以下两种情况下加速工作负荷/查询：

* 查询包含对具有高选择性 (的谓词的筛选器，例如，你想要从一百万个候选行中选择100匹配行) 
* 查询包含需要洗牌的联接 (例如，你想要使用 10 GB 的数据集加入 100 GB 数据集) 

你可能想要认真监视工作负荷，并确定索引是否正在根据具体情况进行帮助。

此文档还适用于适用于 Scala 的笔记本窗体，适用于[Python](https://github.com/microsoft/hyperspace/blob/master/notebooks/python/Hitchhikers%20Guide%20to%20Hyperspace.ipynb)，适用于[c #](https://github.com/microsoft/hyperspace/blob/master/notebooks/csharp/Hitchhikers%20Guide%20to%20Hyperspace.ipynb)和[Scala](https://github.com/microsoft/hyperspace/blob/master/notebooks/scala/Hitchhikers%20Guide%20to%20Hyperspace.ipynb)

## <a name="setup"></a>设置

首先，启动新的 Spark 会话。 由于本文档只是说明超空间可以提供的内容的教程，因此你将进行配置更改，使我们能够突出显示在小型数据集上执行的超空间。 默认情况下，当联接的一侧的数据大小 (较小时，Spark 使用广播联接优化联接查询，这是我们在本教程中使用的示例数据) 的情况。 因此，我们禁用了广播联接，以便以后在运行联接查询时，Spark 使用排序合并联接。 这主要用于演示如何在大规模中使用超空间索引以加速联接查询。

运行下面的单元格的输出显示对已成功创建的 Spark 会话的引用，并输出 "-1" 作为已修改的联接配置的值，这表示已成功禁用广播联接。
...
要准备环境，需要创建示例数据记录并将其保存为 parquet 数据文件。 当 Parquet 用于说明时，可以使用其他格式（如 CSV）。 在后续单元中，你将了解如何在此示例数据集上创建多个超空间索引，以及如何在运行查询时使 Spark 使用这些索引。

示例记录对应于两个数据集：部门和员工。 应配置 "empLocation" 和 "deptLocation" 路径，以便在存储帐户上保存生成的数据文件。

下面的单元格下运行的输出会将我们的数据集的内容显示为 triplets 的列表，后跟 dataFrames 的引用，以将每个数据集的内容保存到首选位置。
...
让我们验证上面创建的 parquet 文件的内容，以确保它们包含正确格式的预期记录。 稍后我们将使用这些数据文件来创建超空间索引和运行示例查询。

运行下面的单元格时，输出将以表格形式显示员工和部门 dataFrames 中的行。 应该有14名员工和4个部门，每个部门都与您在上一单元中创建的 triplets 之一匹配。
...
超空间使你可以对从保存的数据文件扫描的记录创建索引。 成功创建后，与该索引对应的条目将添加到超空间的元数据中。 Apache Spark 的优化器 (将使用此元数据，在查询处理过程中，我们的扩展) 用于查找和使用适当的索引。

创建索引后，可以执行多个操作：

刷新如果基础数据发生更改，则可以刷新现有索引来捕获数据。
删除如果索引不是必需的，则可以执行软删除，即索引不会被物理删除，而是将其标记为 "已删除"，因此不会再用于工作负荷。
真空，如果不再需要索引，可以将其真空，这会强制从超空间的元数据中完全删除索引内容和关联元数据。
以下部分说明了如何在超空间中执行此类索引管理操作。

首先，需要导入所需的库，并创建超空间的实例。 稍后将使用此实例调用不同的超空间 Api 来创建示例数据的索引，并修改这些索引。
...
下面的代码演示如何在超空间实例中列出所有可用的索引。 它使用 "索引" API，该 API 将现有索引的相关信息作为 Spark 数据帧返回，以便您可以执行其他操作。 例如，可以调用此数据帧上的有效操作来检查其内容或进一步分析 (例如，筛选特定索引，或者根据所需的属性) 对它们进行分组。
...
您可以使用 "deleteIndex" API 并提供索引名称来删除现有索引。 删除索引需要软删除：它主要将超空间元数据中的索引状态从 "活动" 更新为 "已删除"。 这将从任何未来的查询优化中排除删除的索引，超空间不再为任何查询选择该索引。 但是，已删除索引的索引文件仍然仍可用 (因为它是软删除的) ，因此，如果用户请求，则可以还原索引。

下面的单元格删除名为 "deptIndex2" 的索引，并在之后列出超空间元数据。 输出应类似于 "列表索引" 的 "列表索引" 单元格，但 "deptIndex2" 除外，此时应将其状态更改为 "已删除"。
...
可以使用 "restoreIndex" API 还原已删除的索引。 这会将最新版本的索引恢复为活动状态，并使其再次可用于查询。 下面的单元格显示了 "restoreIndex" 用法的示例。 删除 "deptIndex1" 并将其还原。 输出显示在调用 "deleteIndex" 命令后，"deptIndex1" 首先进入 "已删除" 状态，并在调用 "restoreIndex" 后恢复为 "活动" 状态。
...
你可以使用 "vacuumIndex" 命令对已删除的索引执行硬删除（即，完全删除文件和元数据条目）。 完成后，此操作将是不可逆的，因为它会以物理方式删除所有索引文件 (这就是硬删除) 的原因。
...
## <a name="enabledisable-hyperspace"></a>启用/禁用超空间

超空间提供使用 Spark 启用或禁用索引使用的 Api。

通过使用 "enableHyperspace" 命令，无空间优化规则对 Spark 优化器可见，它们将利用现有超空间索引来优化用户查询。
通过使用 "disableHyperspace" 命令，在查询优化期间，超空间规则将不再适用。 应注意，禁用超空间不会影响已创建的索引，因为它们保持不变。
下面的单元格演示了如何使用这些命令来启用或禁用超空间。 输出只显示对其配置已更新的现有 Spark 会话的引用。
...
为了使 Spark 在查询处理过程中使用超空间索引，需要确保超空间已启用。

以下单元启用超空间，并创建两个 DataFrames，其中包含用于运行示例查询的示例数据记录。 对于每个数据帧，会打印几个示例行。
...
第一个示例查询将查找部门记录 (参见下面的单元) 。 在 SQL 中，此查询如下所示：
...
在查询计划中，计划底部的 "FileScan" 运算符显示从中读取记录的数据源。 此文件的位置指示最新版本的 "deptIndex1" 索引的路径。 这会根据查询和使用超空间优化规则显示，Spark 决定在运行时利用正确的索引。
...
第二个示例是针对部门记录的范围选择查询。 在 SQL 中，此查询如下所示：
...
与第一个示例类似，下面的单元格的输出显示 (两个部门的名称) 和查询计划的查询结果。 FileScan 运算符中的数据文件位置显示 "deptIndex1" 用于运行该查询。
...
运行下面的单元格的输出显示查询结果，这些结果是14名员工的姓名和每位员工的工作部门的名称。 查询计划也包含在输出中。 请注意，两个 FileScan 运算符的文件位置如何显示 Spark 使用 "empIndex" 和 "deptIndex1" 索引来运行查询。
...
索引很好，但你如何知道是否正在使用它们？ 超空间允许用户在运行查询之前，比较其原始计划与已更新索引相关计划。 您可以选择使用 html/纯文本/控制台模式来显示命令输出。
...
如果为其创建索引的原始数据发生更改，则该索引将不再捕获数据的最新状态。 您可以使用 "refreshIndex" 命令刷新此类陈旧索引。 这将导致索引完全重新生成并根据最新的数据记录进行更新 (不必担心，我们将向你展示如何在其他笔记本) 中以增量方式刷新索引。
