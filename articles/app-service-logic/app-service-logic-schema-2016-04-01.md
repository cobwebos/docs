...
此架构中的最大变化之一是添加了范围以及能够相互嵌套操作。  将一组操作分组在一起时，或是需要相互嵌套操作（例如一个条件可以包含其他条件）时，这会非常有用。  可以在[此处](app-service-logic-loops-and-scopes.md)找到有关范围语法的更多详细信息，不过可以在下面找到一个简单范围示例：
...
在以前版本的架构中，条件和循环是与单个操作关联的参数。  此限制已在此架构中解除，现在条件和循环会显示为一种操作类型。  可以在[本文中](app-service-logic-loops-and-scopes.md)找到详细信息，下面演示了条件操作的简单示例：
...
升级到新的 2016-06-01 架构只需几个步骤。  可以在[本文中](app-service-logic-schema-2016-04-01.md)找到有关架构中的更改的详细信息。  升级过程包括运行升级脚本、保存为新逻辑应用以及可能会覆盖旧逻辑应用（如果需要）。
...
在新架构中，可以使用筛选器操作重复每个项一个条件的以前 foreach 循环模式。  这应在升级时自动进行。  条件会变为 foreach 循环前的筛选器操作（用于仅返回与条件匹配的项数组），该数组会传递到 foreach 操作中。  可以在[本文中](app-service-logic-loops-and-scopes.md)查看此方面的示例
...
* [使用逻辑应用工作流定义](app-service-logic-author-definitions.md)
