---
title: "使用中继消息传送的服务总线 REST 教程 | Microsoft 文档"
...
---
# <a name="service-bus-wcf-relay-rest-tutorial"></a>服务总线 WCF 中继 REST 教程
...
## <a name="step-2-define-a-rest-based-wcf-service-contract-to-use-with-service-bus"></a>步骤 2：定义基于 REST 的 WCF 服务约定以用于服务总线
...
创建 REST 样式的服务总线服务首先需要你创建使用接口定义的约定。 下一步是实现该接口。 此步骤包括创建名为 **ImageService** 的类，该类用于实现用户定义的 **IImageContract** 接口。 实现约定后，即可使用 App.config 文件配置接口。 该配置文件包含应用程序所需的信息，如服务的名称、约定的名称，以及用来与服务总线通信的协议类型。 该过程后面的示例中提供了这些任务所用的代码。
...
2. `<system.serviceModel>` XML 元素是一个 WCF 元素，用于定义一个或多个服务。 在这里，它用于定义服务名称和终结点。 在 `<system.serviceModel>` 元素的下面（仍在 `<system.serviceModel>` 中）添加具有以下内容的 `<bindings>` 元素。 这样就定义了应用程序中使用的绑定。 你可以定义多个绑定，但在本教程中，你只要定义一个绑定。
    ...
    此步骤定义了一个服务总线 [WebHttpRelayBinding](https://msdn.microsoft.com/library/microsoft.servicebus.webhttprelaybinding.aspx) 绑定，其中 **relayClientAuthenticationType** 设置为 **None**。 此设置表明使用此绑定的终结点将不需要客户端凭据。
...
4. 在 `<services>` 元素的后面，使用以下内容创建 `<behaviors>` 元素，并将 “SAS_KEY” 替换为先前从 [Azure 门户][Azure 门户]获得的*共享访问签名* (SAS) 密钥。
...
## <a name="step-4-host-the-rest-based-wcf-service-to-use-service-bus"></a>步骤 4：托管基于 REST 的 WCF 服务以使用服务总线
