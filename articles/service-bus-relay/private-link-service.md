...
8. 在 "**资源**" 页上，执行以下步骤：
    ...
    2. 如果选择 "**按资源 ID 或别名连接到 Azure 资源**"，因为命名空间与专用终结点的目录不在同一目录下，请执行以下步骤：
        1. 输入**资源 ID**或**别名**。 它可以是某人与你共享的资源 ID 或别名。 获取资源 ID 的最简单方法是导航到 Azure 门户中的 Azure 中继命名空间，并从中`/subscriptions/`复制 URI 的一部分。 下面是一个示例：`/subscriptions/000000000-0000-0000-0000-000000000000000/resourceGroups/myresourcegroup/providers/Microsoft.Relay/namespaces/myrelaynamespace.` 
