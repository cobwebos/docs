...
* [视觉工作室 2015 或更高版本](https://www.visualstudio.com)。 本教程中的示例使用 Visual Studio 2019。
...
[Azure 中继](https://azure.microsoft.com/services/service-bus/)采用现有的 WCF Web 服务，并使这些服务安全地可供企业外围以外的解决方案访问，而无需对公司网络基础结构进行侵入性更改。 虽然此类中继服务仍托管在现有环境中，但它们会将侦听传入会话和请求这一任务委托给云托管的中继服务。 Azure 中继还通过使用[共享访问签名 （SAS）](../service-bus-messaging/service-bus-sas.md)身份验证保护这些服务免受未经授权的访问。
...
1. 在*Program.cs*中，将命名空间定义替换为以下代码，该代码为其添加了配置文件服务和主机。
...
1. 仍在`<appSettings>`元素中的*App.config*中，将连接字符串值替换为以前从门户获取的连接字符串。
...
1. 选择 Ctrl_Shift_B**Build** > 或选择**生成解决方案**来构建应用程序并验证到目前为止工作的准确性。
...
1. 在 Visual Studio 中的*Product.cs*文件中，将现有的命名空间定义替换为以下代码：
...
1. 确保 **ProductsPortal**是活动项目。 右键单击**解决方案资源管理器**中的项目名称，然后选择 **"设置为启动项目**"。
...
1. 与["创建本地服务器](#create-an-on-premises-server)"部分中的步骤类似，将 NuGet 包添加到项目引用中。 在“解决方案资源管理器”中，右键单击“ProductsPortal”项目并选择“管理 NuGet 包”。************
...
1. 选择 **"通用属性** > **启动项目**"并选择**多个启动项目**。 确保 **ProductsServer** 和 **ProductsPortal** 按此顺序显示，并且两者的操作为“启动”。********
...
若要在本地测试应用程序，请在 Visual Studio 中按 F5。 本地服务器**ProductsServer**应首先启动，然后**产品门户**应用程序应在浏览器窗口中启动。 这次，会看到产品清单列出了从产品服务本地系统中检索到的数据。
...
1. 在“解决方案资源管理器”中，右键单击“ProductsPortal”项目并选择“发布”************。 在 **"发布"** 页上，选择 **"发布**"。
...
<a name="set-productsportal-as-web-app"></a>在云中运行应用程序之前，必须确保**产品门户**从 Visual Studio 内部作为 Web 应用启动。
