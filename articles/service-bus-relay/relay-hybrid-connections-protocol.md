...
Azure 中继是 Azure 服务总线平台最重要的功能支柱之一。 中继的新“混合连接”功能是基于 HTTP 和 WebSocket 的安全、开放协议演化版__。 它取代了以前，同样命名的_BizTalk服务_功能，是建立在专有的协议基础。 将混合连接集成到 Azure 应用服务并不影响原有的运行方式。
...
| 参数        | 必选 | 描述
| ---------------- | -------- | -------------------------------------------
| `sb-hc-action`   | 是      | 对于侦听器角色，参数必须是**sb-hc-action_listen**
...
* **令牌**– 指定空间或混合连接授予**侦听**权的有效 URL 编码的服务总线共享访问令牌。
