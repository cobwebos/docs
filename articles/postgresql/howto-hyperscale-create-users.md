...
* [创建 PostgreSQL 扩展](concepts-hyperscale-extensions.md)（因为该角色是的成员 `azure_pg_admin` ）。
...
超大规模（Citus）通过整个群集传播单表 GRANT 语句，并将其应用于所有工作节点。 它还传播系统范围的授予（例如，对于架构中的所有表）：
