...
由于超大规模是托管的 PaaS 服务，因此只有 Microsoft 才能使用 `postgres` 超级用户角色登录。 对于有限的管理访问权限，超大规模提供 `citus` 角色。

`citus` 角色的权限：

* 读取所有配置变量，甚至是通常仅对超级
* 阅读所有 pg\_stat\_\* 视图，并使用各种统计信息相关的扩展，甚至视图或扩展通常仅对超级超级程序可见。
* 执行可能需要很长时间才能对表进行共享锁定的监视函数。
* [创建 PostgreSQL 扩展](concepts-hyperscale-extensions.md)（因为角色是 `azure_pg_admin`的成员）。

特别要注意的是，`citus` 角色具有一些限制：
...
如前所述，`citus` 管理员帐户无权创建其他用户。 若要添加用户，请使用 Azure 门户接口。
