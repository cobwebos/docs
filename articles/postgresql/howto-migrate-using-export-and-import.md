---
title: 迁移数据库 - 用于 PostgreSQL 的 Azure 数据库 - 单个服务器
...
```
例如，如果其中有一个本地服务器和一个称为**testdb**的数据库：
```bash
...
- 要使用转储和还原迁移 PostgreSQL 数据库，请参阅[使用转储迁移 PostgreSQL 数据库并还原](howto-migrate-using-dump-and-restore.md)。
