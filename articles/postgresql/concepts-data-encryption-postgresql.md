...
使用客户管理的密钥 Azure Database for PostgreSQL 单服务器数据加密可以为静态数据保护创建自己的密钥（BYOK）。 它还允许组织在密钥和数据的管理中实现职责分离。 通过客户托管的加密，你负责和完全控制密钥的生命周期、密钥使用权限，以及对密钥的操作进行审核。

对于 Azure Database for PostgreSQL 单一服务器，在服务器级别设置数据加密。 使用这种形式的数据加密，可以使用加密数据加密密钥（DEK）中的密钥。 DEK 是客户管理的非对称密钥，存储在客户拥有的和客户管理的[Azure Key Vault](../key-vault/key-Vault-secure-your-key-Vault.md)实例中。 本文稍后将详细介绍 DEK。
