...
通过客户托管的密钥进行数据加密 Azure Database for PostgreSQL 单个服务器，使你可以将 qwn 密钥（BYOK）用于静态数据保护。 它还允许组织在密钥和数据的管理中实现职责分离。 通过客户托管的加密，你负责和完全控制密钥的生命周期、密钥使用权限，以及对密钥的操作进行审核。
