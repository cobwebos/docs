---
title: 读取副本-Azure Database for PostgreSQL-单服务器
description: 本文介绍 Azure Database for PostgreSQL-单服务器中的读取副本功能。
author: rachel-msft
ms.author: raagyema
...
可以在任何[Azure Database for PostgreSQL 区域](https://azure.microsoft.com/global-infrastructure/services/?products=postgresql)中拥有主服务器。 主服务器可以在其配对区域或通用副本区域中拥有副本。 下图显示了哪些副本区域可用，具体取决于你的主区域。
...
* 区域可用性：在美国西部2、法国中部、阿拉伯联合酋长国北部和德国中部提供 Azure Database for PostgreSQL。 但是，它们的配对区域不可用。
...
可以使用主机名和有效的用户帐户连接到副本，就像在常规的 Azure Database for PostgreSQL 服务器上连接一样。 对于名为 " **my replica** " 的服务器，请使用管理员用户名**myadmin**连接到副本：
...
Azure Database for PostgreSQL 提供了两个用于监视复制的指标。 这两个指标是副本和**副本延迟**的**最大延迟**。 若要了解如何查看这些指标，请参阅[读取副本操作方法一文](howto-read-replicas-portal.md)中的**监视副本**部分。

**副本的最大延迟**指标显示主副本和最滞后副本之间的滞后时间（以字节为单位）。 此指标仅适用于主服务器。

**副本延迟**指标显示自上次重播事务以来的时间。 如果主服务器上未发生任何事务，则该指标会反映此滞后时间。 此指标仅适用于副本服务器。 副本滞后时间从 `pg_stat_wal_receiver` 视图计算得出：
...
当你停止复制时，副本将丢失到其上一个主副本和其他副本的所有链接。
...
如果尝试更新上面所述的服务器值，但不遵守限制，则会收到错误。

创建副本时或之后，防火墙规则、虚拟网络规则和参数设置不会从主服务器继承到副本。

### <a name="max_prepared_transactions"></a>max_prepared_transactions
[PostgreSQL 要求](https://www.postgresql.org/docs/current/runtime-config-resource.html#GUC-MAX-PREPARED-TRANSACTIONS)读取副本上的 `max_prepared_transactions` 参数的值大于或等于 master 值;否则，副本将无法启动。 如果要更改主节点上的 `max_prepared_transactions`，请先在副本上更改它。
