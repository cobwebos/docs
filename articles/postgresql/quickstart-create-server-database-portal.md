...
用于 PostgreSQL 的 Azure 数据库是一种托管服务，可用于在云中运行、管理和缩放具有高可用性的 PostgreSQL 数据库。 此快速入门介绍了如何使用 Azure 门户在大约五分钟内创建 Azure Database for PostgreSQL 服务器。
...
3.  如上图所示，在新服务器详细信息窗体中填写以下信息：
    ...
    设置|建议的值|说明
    ---|---|---
    服务器名称 |mypgserver-20170401|选择用于标识 Azure Database for PostgreSQL 服务器的唯一名称。 域名 postgres.database.azure.com 附加到可供应用程序连接到的服务器名称。 服务器名称只能包含小写字母、数字和连字符 (-) 字符，并且必须包含 3 到 63 个字符。
    订阅|用户的订阅|要用于服务器的 Azure 订阅。 如果有多个订阅，请选择要计费的资源所在的相应订阅。
    资源组|myresourcegroup| 可以创建新的资源组名称，也可以使用订阅中现有的。
    服务器管理员登录名 |mylogin| 创建自己的登录帐户，在连接到服务器时使用。 管理员登录名不能是“azure_superuser”、“azure_pg_admin”、“admin”、“administrator”、“root”、“guest”或“public”，也不能以“pg_”开头。
    密码 |*由用户选择* | 为服务器管理员帐户创建新密码。 必须包含 8 到 128 个字符。 密码必须含以下字符类别中的三类 – 英文大写字母、英文小写字母、数字(0-9)及非字母数字字符（!、$、#、% 等）。
    位置|
            *离用户最近的区域*| 选择最靠近用户的位置。
    PostgreSQL 版本|选择最新版本| 选择最新版本，有特定要求的除外。
    定价层 | 基本，50 个计算单元，50 GB | 单击“定价层”为新数据库指定服务层和性能级别。 在顶部的选项卡中选择“基本”层。 单击“计算单位”滑块的左端，将值调整为适用于本快速入门的最小值。 单击“确定”，保存所做的定价层选择。 请参阅下面的屏幕截图。
    | 固定到仪表板 | 勾选标记 | 选中“固定到仪表板”选项，以便轻松地在 Azure 门户的前端仪表板页上跟踪服务器。

  > [!IMPORTANT]
  > 在此处指定的服务器管理员登录名和密码是以后在本快速入门中登录到服务器及其数据库所必需的。 请牢记或记录此信息，以后会使用到它。

    ![Azure Database for PostgreSQL - 选取定价层](./media/quickstart-create-database-portal/2-service-tier.png)
...
1.  部署完成后，找到服务器。 可以根据需要进行搜索。 例如，单击左侧菜单中的“所有资源”并键入服务器名称（例如 mypgserver-20170401）即可搜索新创建的服务器。 单击搜索结果中列出的服务器名称。 服务器的“概述”页面随即打开，其中提供了用于进一步配置的选项。
 
    ![Azure Database for PostgreSQL - 搜索服务器名称](./media/quickstart-create-database-portal/4-locate.png)

2.  在服务器页中，选择“连接安全性”。 
    ![Azure Database for PostgreSQL - 创建防火墙规则](./media/quickstart-create-database-portal/5-firewall-2.png)
...
创建 Azure Database for PostgreSQL 服务器时，会创建名为“postgres”的默认数据库。 若要连接到数据库服务器，需牢记完整的服务器名称和管理员登录凭据。 你可能已在此前的快速入门文章中记下这些值。 如果没有记下这些值，可以在 Azure 门户的服务器“概览”页中轻松地找到服务器名称和登录信息。
...
3. 在 Cloud Shell 绿色提示符下键入 psql 命令行，连接到 Azure Database for PostgreSQL 服务器中的数据库。

    借助 [psql](https://www.postgresql.org/docs/9.6/static/app-psql.html) 实用工具可以使用以下格式连接到用于 PostgreSQL 的 Azure 数据库：
    ```bash
    ...
    psql 参数 |建议的值|说明
    ---|---|---
    --host | 服务器名称 | 指定此前在创建 Azure Database for PostgreSQL 时使用过的服务器名称值。 显示的示例服务器为 mypgserver-20170401.postgres.database.azure.com。 请使用完全限定的域名 (\*.postgres.database.azure.com)，如示例中所示。 如果记不起服务器名称，请按上一部分的步骤操作，以便获取连接信息。 
    --port | **5432** | 在连接到 Azure Database for PostgreSQL 时，始终使用端口 5432。 
    --username | 服务器管理员登录名 |键入此前在创建 Azure Database for PostgreSQL 时提供的服务器管理员登录用户名。 如果记不起用户名，请按上一部分的步骤操作，以便获取连接信息。  格式为 username@servername。
    --dbname | postgres | 将默认的系统生成的数据库名称 postgres 用于第一次连接。 以后则创建自己的数据库。

    使用你自己的参数值运行 psql 命令以后，系统会提示你键入服务器管理员密码。 此密码是在创建服务器时提供的密码。 

    psql 参数 |建议的值|说明
    ---|---|---
    password | 管理员密码 | 注意，键入的密码字符不会显示在 bash 提示符处。 在键入所有字符后按 Enter，以便进行身份验证和连接。

    连接后，psql 实用工具会显示 postgres 提示符，要求你在其中键入 sql 命令。 在初始连接输出中可能会显示警告，因为 Azure Cloud Shell 中的 psql 版本可能不同于 Azure Database for PostgreSQL 服务器版本。 
...
6.  键入 \q 后按 Enter 即可退出 psql。 完成后即可关闭 Azure Cloud Shell。

现在，你已连接到 Azure Database for PostgreSQL 并创建空白用户数据库。 请转到下一部分，使用另一常用工具 pgAdmin 进行连接。
...
2.  在“仪表板”页中心的“快速链接”部分，单击“添加新服务器”图标。
...
4.  在“创建 - 服务器”对话框的“连接”选项卡中，使用下图中指定的设置并单击“保存”。
   ![pgAdmin - 创建 - 服务器](./media/quickstart-create-database-portal/10-pgadmin-create-server.png)
    ...
    pgAdmin 参数 |建议的值|说明
    ---|---|---
    主机名/地址 | 服务器名称 | 指定此前在创建 Azure Database for PostgreSQL 时使用过的服务器名称值。 显示的示例服务器为 mypgserver-20170401.postgres.database.azure.com。 请使用完全限定的域名 (\*.postgres.database.azure.com)，如示例中所示。 如果记不起服务器名称，请按上一部分的步骤操作，以便获取连接信息。 
    端口 | **5432** | 在连接到 Azure Database for PostgreSQL 时，始终使用端口 5432。  
    维护数据库 | postgres | 使用系统生成的默认数据库名称 postgres。
    用户名 | 服务器管理员登录名 | 键入此前在创建 Azure Database for PostgreSQL 时提供的服务器管理员登录用户名。 如果记不起用户名，请按上一部分的步骤操作，以便获取连接信息。 格式为 username@servername。
    密码 | 管理员密码 |  之前在此快速入门中创建服务器时选择的密码。
    角色 | 留空 | 无需在此时提供角色名称。 此字段留空。
    SSL 模式 | 必需 | 默认情况下，所有 Azure PostgreSQL 服务器在创建时都会启用 SSL 强制。 若要关闭 SSL 强制，请参阅[强制 SSL](./concepts-ssl-connection-security.md) 中的详细信息。
    
5.  单击“保存” 。
6.  在“浏览器”左窗格中，展开“服务器”节点。 选择服务器，例如“Azure PostgreSQL 服务器”，然后通过单击进行连接。
7. 展开服务器节点，然后展开其下的“数据库”。 列表应包括现有的 postgres 数据库，以及任何新创建的用户数据库，例如在上一部分创建的 mypgsqldb。 注意，在使用 Azure Database for PostgreSQL 时，可以为单个服务器创建多个数据库。
8. 右键单击“数据库”，选择“创建”菜单，然后单击“数据库”。
9.  在“数据库”字段中键入所选的数据库名称，例如“mypgsqldb”，如示例所示。 
10. 从下拉框中选择数据库的“所有者”。 选择服务器管理员登录名，例如“mylogin”示例。
10. 单击“保存”创建新的空数据库。
11. 在“浏览器”窗格中，可以在服务器名称下的“数据库”列表中看到创建的数据库。
...
> [!TIP]
> 本教程系列中的其他快速入门教程是在本文的基础上制作的。 如果打算继续使用后续的快速入门，请不要清除在本快速入门中创建的资源。 如果不打算继续，请在 Azure 门户中执行以下步骤来删除此快速入门创建的资源。
...
![Azure Database for PostgreSQL - 删除服务器](./media/quickstart-create-database-portal/12-delete.png)
3.  确认要删除的服务器名称并显示其下受影响的数据库。 在文本框中键入服务器名称（例如 mypgserver-20170401 示例），然后单击“删除”。
