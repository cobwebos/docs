...
启用 pgAudit 会在服务器上生成大量日志记录，影响性能和日志存储。 建议使用 Azure 诊断日志服务，该服务提供较长期的存储选项以及分析和警报功能。 建议关闭标准日志记录，降低额外的日志记录带来的性能影响：
   ...
   1. 设置参数 `logging_collector` 到 OFF。 
   2. 重新启动服务器以应用此更改。
...
> [!NOTE]
> 将 @no__t 设置为 ON 时，会将日志重定向到客户端进程（如 psql），而不是写入到文件。 通常应让此设置保持禁用状态。

> [!NOTE]
> 仅当 `pgaudit.log_client` 处于启用状态时，@no__t 为0。 此外，在 Azure 门户中，当前有一个 @no__t 的 bug：显示了一个组合框，表示可以选择多个级别。 但是，只应选择一个级别。 

> [!NOTE]
> 在 Azure Database for PostgreSQL 中，无法使用 pgAudit 文档中所述的 `-` （减号）号快捷方式设置 @no__t。 所有必需的声明类（READ、WRITE 等）应该单独指定。

### <a name="audit-log-format"></a>审核日志格式
每个审核条目都由日志行开头附近 `AUDIT:` 表示。 [PgAudit 文档](https://github.com/pgaudit/pgaudit/blob/master/README.md#format)中详细说明了条目的其余部分的格式。
...
若要详细了解 `log_line_prefix`，请访问[PostgreSQL 文档](https://www.postgresql.org/docs/current/runtime-config-logging.html#GUC-LOG-LINE-PREFIX)。

### <a name="getting-started"></a>入门
若要快速开始，请将 @no__t 设置为 `WRITE`，并打开日志以查看输出。 
