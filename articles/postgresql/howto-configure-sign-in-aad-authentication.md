...
只有 Azure AD 管理员用户才能创建/启用基于 Azure AD 身份验证的用户。 对于常规数据库操作，我们建议不要使用 Azure AD 管理员，因为它具有提升的用户权限（如 CREATEDB）。

若要设置 Azure AD 管理员（可以使用用户或组），请执行以下步骤：
...
> 请注意，在 Azure Cloud Shell 中检索 Azure AD 访问令牌时，需要显式调用`az login`并再次登录（在单独的窗口中包含代码）。 后，该`get-access-token`命令将按预期方式工作。
...
使用`psql`命令行客户端时，需要通过`PGPASSWORD`环境变量传递访问令牌，因为访问令牌超出`psql`可直接接受的密码长度：
...
3. 在 Azure Database for PostgreSQL `<user>@yourtenant.onmicrosoft.com`中创建角色。
4. 使`<user>@yourtenant.onmicrosoft.com`角色 azure_ad_user 成为角色的成员。 必须仅向 Azure AD 用户提供此。
...
如果你的现有用户已与 Azure AD 用户名相匹配，则你可以向其授予`azure_ad_user`角色，以便对其进行 Azure AD 身份验证：
