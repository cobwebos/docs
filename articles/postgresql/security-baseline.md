---
title: 后 SQL 单服务器 Azure 数据库的 Azure 安全基线
description: 后 SQL 单服务器 Azure 数据库的 Azure 安全基线
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 03/23/2020
...
ms.locfileid: "81261154"
---
# <a name="azure-security-baseline-for-azure-database-for-postgresql-single-server"></a>后 SQL 单服务器 Azure 数据库的 Azure 安全基线

PostgreSQL 单一服务器 Azure 数据库的 Azure 安全基线包含一些建议，可帮助您改进部署的安全状态。
...
**指南**：为具有专用终结点的 PostgreSQL 配置 Azure 数据库的专用链接。 使用专用链接可以通过专用终结点连接到 Azure 中的各种 PaaS 服务。 Azure 专用链接实质上将 Azure 服务引入专用虚拟网络 （VNet）。 虚拟网络和 PostgreSQL 实例之间的流量将传输 Microsoft 骨干网络。

或者，您可以使用虚拟网络服务终结点来保护和限制对 Azure 数据库的网络访问，以便 PostgreSQL 实现。 虚拟网络规则是一种防火墙安全功能，用于控制是否允许 Azure Database for PostgreSQL 服务器接受从虚拟网络中的特定子网发送的通信。

您还可以使用防火墙规则保护 PostgreSQL 服务器的 Azure 数据库。 在指定哪些计算机具有权限之前，服务器防火墙会阻止对数据库服务器的所有访问。 要配置防火墙，请创建防火墙规则，以指定可接受的 IP 地址的范围。 可以在服务器级别创建防火墙规则。

如何为后数据库配置 Azure 数据库的专用链接：https://docs.microsoft.com/azure/postgresql/howto-configure-privatelink-portal

如何在 PostgreSQL 的 Azure 数据库中创建和管理 VNet 服务终结点和 VNet 规则：https://docs.microsoft.com/azure/postgresql/howto-manage-vnet-using-portal

如何为 PostgreSQL 防火墙规则配置 Azure 数据库：https://docs.microsoft.com/azure/postgresql/howto-manage-firewall-using-portal
...
**指南**：当用于 PostgreSQL 实例的 Azure 数据库被保护到专用终结点时，可以在同一虚拟网络中部署虚拟机。 您可以使用网络安全组 （NSG） 来降低数据泄露的风险。 启用 NSG 流日志并将日志发送到存储帐户以进行流量审核。 您还可以将 NSG 流日志发送到日志分析工作区，并使用流量分析提供有关 Azure 云中的流量流的见解。 流量分析的一些优点是能够可视化网络活动并识别热点、识别安全威胁、了解流量模式和查明网络错误配置。
...
**指南**：将高级威胁保护用于后格雷SQL的 Azure 数据库。 高级威胁防护可检测异常活动，指示访问或利用数据库的异常和潜在有害尝试。

在与 Azure 数据库关联的虚拟网络上启用 DDoS 保护标准，以防止 DDoS 攻击。 使用 Azure 安全中心集成威胁智能拒绝与已知恶意或未使用的 Internet IP 地址的通信。

如何为后格雷SQL 为 Azure 数据库配置高级威胁保护：https://docs.microsoft.com/azure/postgresql/howto-database-threat-protection-portal
...
**指南**：当用于 PostgreSQL 实例的 Azure 数据库被保护到专用终结点时，可以在同一虚拟网络中部署虚拟机。 然后，您可以配置网络安全组 （NSG）以降低数据泄露的风险。 启用 NSG 流日志并将日志发送到存储帐户以进行流量审核。 您还可以将 NSG 流日志发送到日志分析工作区，并使用流量分析提供有关 Azure 云中的流量流的见解。 流量分析的一些优点是能够可视化网络活动并识别热点、识别安全威胁、了解流量模式和查明网络错误配置。
...
**指南**：对于需要访问 Azure 数据库的 PostgreSQL 实例的资源，请使用虚拟网络服务标记在网络安全组或 Azure 防火墙上定义网络访问控件。 创建安全规则时，可以使用服务标记代替特定的 IP 地址。 通过指定服务标记名称（例如 SQL。WestUs） 在规则的相应源或目标字段中，您可以允许或拒绝相应服务的流量。 Microsoft 管理服务标记包含的地址前缀，并在地址更改时自动更新服务标记。

注意：用于 PostgreSQL 的 Azure 数据库使用"Microsoft.Sql"服务标记。

有关使用服务标记的详细信息：https://docs.microsoft.com/azure/virtual-network/service-tags-overview

了解 PostgreSQL Azure 数据库的服务标记使用情况：https://docs.microsoft.com/azure/postgresql/concepts-data-access-and-security-vnet#terminology-and-description
...
**指南**：使用 Azure 策略为 PostgreSQL 实例定义和实施与 Azure 数据库关联的网络设置和网络资源的标准安全配置。 在"Microsoft.DBforPostgreSQL"和"Microsoft.Network"命名空间中使用 Azure 策略别名来创建自定义策略，以审核或强制执行用于 PostgreSQL 实例的 Azure 数据库的网络配置。 您还可以使用与网络相关的内置策略定义或 PostgreSQL 实例的 Azure 数据库，例如：
...
**指南**：对与 Azure 数据库的 Azure 数据库的 PostgreSQL 实例相关的资源使用标记来提供元数据和逻辑组织。
...
**指南**：使用 Azure 活动日志监视网络资源配置，并检测与 PostgreSQL 实例的 Azure 数据库相关的网络资源的更改。 在 Azure 监视器中创建警报，这些警报将在对关键网络资源的更改时触发。
...
**指南**：Microsoft 维护用于 Azure 资源的时间源，例如日志中时间戳的 PostgreSQL 的 Azure 数据库。
...
**指南**：启用诊断设置和服务器日志和引入日志以聚合 Azure 数据库为 PostgreSQL 实例生成的安全数据。 在 Azure 监视器中，使用日志分析工作区来查询和执行分析，并使用 Azure 存储帐户进行长期/存档存储。 或者，您可以将数据启用到 Azure Sentinel 或第三方 SIEM。

如何为 PostgreSQL 配置和访问 Azure 数据库的服务器日志：https://docs.microsoft.com/azure/postgresql/howto-configure-server-logs-in-portal

如何为 PostgreSQL 配置和访问 Azure 数据库的审核日志：https://docs.microsoft.com/azure/postgresql/concepts-audit
...
**指南**：在 Azure 数据库上启用 PostgreSQL 实例的诊断设置，以访问审核、安全和诊断日志。 确保专门启用 PostgreSQL 审核日志。 活动日志自动可用，包括事件源、日期、用户、时间戳、源地址、目标地址和其他有用元素。 您还可以启用 Azure 活动日志诊断设置，并将日志发送到相同的日志分析工作区或存储帐户。
...
**指南**：在 Azure 监视器中，对于用于保存 Azure 数据库的 PostgreSQL 日志的日志分析工作区，请根据组织的合规性法规设置保留期。 使用 Azure 存储帐户进行长期/存档存储。
...
**指南**：分析和监视 Azure 数据库中的日志，以进行 PostgreSQL 实例的异常行为。 使用 Azure 监视器的日志分析查看日志并执行日志数据的查询。 或者，您可以将数据启用并车载到 Azure Sentinel 或第三方 SIEM。
...
**指南**：为后格雷SQL 启用 Azure 数据库的高级威胁保护。 高级威胁防护可检测异常活动，指示访问或利用数据库的异常和潜在有害尝试。

此外，您还可以为 PostgreSQL 启用服务器日志和诊断设置，并将日志发送到日志分析工作区。 将日志分析工作区添加到 Azure Sentinel 中，因为它提供安全编排自动响应 （SOAR） 解决方案。 这允许创建行动手册（自动解决方案），并用于修复安全问题。

如何为后格雷SQL 的 Azure 数据库启用高级威胁保护：https://docs.microsoft.com/azure/postgresql/howto-database-threat-protection-portal
...
**指导**：不适用;用于 PostgreSQL 的 Azure 数据库不处理或生成与恶意软件相关的日志。
...
**指导**：不适用;用于 PostgreSQL 的 Azure 数据库不处理或生成 DNS 相关日志。
...
**指南**：维护对 PostgreSQL 实例的 Azure 数据库的控制平面（例如 Azure 门户）具有管理访问权限的用户帐户的清单。 此外，维护有权访问 PostgreSQL 实例的 Azure 数据库的数据平面（在数据库本身内）的管理帐户的清单。 （创建 PostgreSQL 服务器时，为管理员用户提供凭据。 此管理员可用于创建其他 PostgreSQL 用户。

PostgreSQL 的 Azure 数据库不支持基于内置角色的访问控制，但可以基于特定资源提供程序操作创建自定义角色。

了解 Azure 订阅的自定义角色：https://docs.microsoft.com/azure/role-based-access-control/custom-roles 

了解 Azure 数据库，用于 PostgreSQL 资源提供程序操作：https://docs.microsoft.com/azure/role-based-access-control/resource-provider-operations#microsoftdbforpostgresql 

了解 PostgreSQL Azure 数据库的访问管理：https://docs.microsoft.com/azure/postgresql/concepts-security#access-management
...
**指南**：Azure 活动目录和用于 PostgreSQL 的 Azure 数据库没有默认密码的概念。

创建 PostgreSQL 资源本身的 Azure 数据库后，Azure 将强制创建具有强密码的管理用户。 但是，创建 PostgreSQL 实例后，您可以使用您创建的第一个服务器管理员帐户来创建其他用户并授予对它们的管理访问权限。 创建这些帐户时，请确保为每个帐户配置不同的强密码。

如何为 PostgreSQL 创建 Azure 数据库的其他帐户：https://docs.microsoft.com/azure/postgresql/howto-create-users
...
**指南**：围绕使用有权访问 Azure 数据库的 PostgreSQL 实例创建标准操作过程。 使用 Azure 安全中心标识和访问管理监视管理帐户数。 

了解 Azure 安全中心标识和访问：https://docs.microsoft.com/azure/security-center/security-center-identity-access 

了解如何在 Azure 数据库中为 PostgreSQL 创建管理员用户：https://docs.microsoft.com/azure/postgresql/howto-create-users#the-server-admin-account
...
**指南**：支持使用直接在数据库中配置的用户名/密码，以及使用 Azure 活动目录 （AD） 标识并使用 Azure AD 令牌进行连接，支持登录 Azure 数据库。 使用 Azure AD 令牌时，支持不同方法，例如 Azure AD 用户、Azure AD 组或连接到数据库的 Azure AD 应用程序。

另外，PostgreSQL 的控制平面访问可通过 REST API 获得，并支持 SSO。 要进行身份验证，请将请求的授权标头设置为从 Azure 活动目录获取的 JSON Web 令牌。

使用 Azure 活动目录使用 Azure 数据库进行 PostgreSQL 身份验证：https://docs.microsoft.com/azure/postgresql/howto-configure-sign-in-aad-authentication

了解 Azure 数据库，用于后格雷SQL REST API：https://docs.microsoft.com/rest/api/postgresql/
...
**指南**：使用特权访问工作站 （PAW） 配置为登录和配置 Azure 资源的多重身份验证 （MFA）。
...
**指南**：为 PostgreSQL 的 Azure 数据库启用高级威胁保护，以生成可疑活动的警报。

此外，当环境中发生可疑或不安全活动时，可以使用 Azure 活动目录 （AD） 特权标识管理 （PIM） 生成日志和警报。

使用 Azure AD 风险检测查看有关风险用户行为的警报和报告。

如何为后格雷SQL 设置 Azure 数据库的高级威胁保护：https://docs.microsoft.com/azure/postgresql/howto-database-threat-protection-portal
...
对于为 PostgreSQL 登录到 Azure 数据库，建议使用 Azure AD 并使用 Azure AD 令牌进行连接。 使用 Azure AD 令牌时，支持不同方法，例如 Azure AD 用户、Azure AD 组或连接到数据库的 Azure AD 应用程序。

Azure AD 凭据还可用于管理平面级别（例如 Azure 门户）的管理，以控制 PostgreSQL 管理帐户。
...
**指南**： 查看 Azure 活动目录日志，以帮助发现陈旧的帐户，其中可能包括具有 Azure 数据库的 PostgreSQL 管理角色。 此外，使用 Azure 标识访问审核可有效地管理组成员身份、访问可用于访问 PostgreSQL 的 Azure 数据库的企业应用程序以及角色分配。 用户访问权限应定期审查，例如每 90 天一次，以确保只有正确的用户才能继续访问。
...
**指南**：为 PostgreSQL 和 Azure 活动目录启用 Azure 数据库的诊断设置，将所有日志发送到日志分析工作区。 在日志分析中配置所需的警报（如身份验证尝试失败）。
...
**指南**：当前不可用;对于 PostgreSQL 的 Azure 数据库，尚未支持客户密码箱。
...
**指南**：使用标记来帮助跟踪 Azure 数据库的 PostgreSQL 实例或存储或处理敏感信息的相关资源。
...
**指导**：为开发、测试和生产实施单独的订阅和/或管理组。 使用专用链路、服务终结点和/或防火墙规则的组合来隔离和限制对 PostgreSQL 实例的 Azure 数据库的网络访问。
...
如何为 PostgreSQL 防火墙规则配置 Azure 数据库：https://docs.microsoft.com/azure/postgresql/concepts-firewall-rules
...
**指南**：当使用 Azure 虚拟机访问 Azure 数据库以进行 PostgreSQL 实例时，请使用专用链路、PostgreSQL 网络配置、网络安全组和服务标记来降低数据渗漏的可能性。

Microsoft 管理 PostgreSQL Azure 数据库的基础基础结构，并实施了严格的控制，以防止客户数据丢失或暴露。

如何缓解 PostgreSQL Azure 数据库的数据渗漏：https://docs.microsoft.com/azure/postgresql/concepts-data-access-and-security-private-link
...
**指南**： 用于 PostgreSQL 的 Azure 数据库支持使用安全套接字层 （SSL） 将 PostgreSQL 服务器连接到客户端应用程序。 通过在数据库服务器与客户端应用程序之间强制实施 SSL 连接，可以加密服务器与应用程序之间的数据流，有助于防止“中间人”攻击。 在 Azure 门户中，确保默认情况下为 PostgreSQL 实例的所有 Azure 数据库启用"强制 SSL 连接"。

目前，Azure Database for PostgreSQL 支持的 TLS 版本为 TLS 1.0、TLS 1.1、TLS 1.2。

如何为 PostgreSQL 配置 Azure 数据库的传输加密：https://docs.microsoft.com/azure/postgresql/concepts-ssl-connection-security
...
**指南**：数据识别、分类和损失预防功能尚未可用于 PostgreSQL 的 Azure 数据库。 如果需要符合性，则实施第三方解决方案。
...
**指导**：使用基于 Azure 角色的访问控制 （RBAC） 来控制对 PostgreSQL 控制平面（例如 Azure 门户）的 Azure 数据库的访问。 对于数据平面访问（在数据库本身中），使用 SQL 查询创建用户并配置用户权限。 RBAC 不会影响数据库中的用户权限。

如何在 Azure 中配置 RBAC：https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal

如何使用 Sql 为 PostgreSQL 配置用户访问：https://docs.microsoft.com/azure/postgresql/howto-create-users
...
**指南**： Azure 数据库用于 PostgreSQL 服务使用 FIPS 140-2 验证加密模块对静态数据进行存储加密。 数据（包括备份）在磁盘上加密，运行查询时创建的临时文件除外。 该服务使用包含在 Azure 存储加密中的 AES 256 位密码，并且密钥由系统进行管理。 存储加密始终处于启用状态，无法禁用。

使用用于 PostgreSQL 单个服务器的 Azure 数据库的与客户管理的密钥 （CMK） 进行数据加密，使您能够自带密钥 （BYOK） 以进行静态数据保护。 目前，必须请求访问权限才能使用此功能。 为此，请联系：

AskAzureDBforPostgreSQL@service.microsoft.com.

了解 PostgreSQL Azure 数据库的静态加密：https://docs.microsoft.com/azure/postgresql/concepts-security

使用客户管理的密钥了解 PostgreSQL Azure 数据库的静态加密：https://docs.microsoft.com/azure/postgresql/concepts-data-encryption-postgresql
...
**指南**：将 Azure 监视器与 Azure 活动日志一起用于创建警报，用于何时对 PostgreSQL 和其他关键或相关资源的 Azure 数据库的生产实例进行更改。
...
**指南**：当前不可用;Azure 安全中心尚不支持 PostgreSQL Azure 数据库的漏洞评估。
...
**指南**：Microsoft 在支持 PostgreSQL Azure 数据库的基础系统上执行漏洞管理。
...
**指南**：使用 Azure 资源图查询和发现订阅中的所有资源（包括用于 PostgreSQL 实例的 Azure 数据库）。 确保租户中具有适当的（读取）权限，并能够枚举订阅中的所有 Azure 订阅和资源。
...
**指南**：将标记应用于 Azure 数据库的 PostgreSQL 实例和其他相关资源，从而提供元数据，以逻辑方式将它们组织到分类中。
...
**指南**：在适当情况下，使用标记、管理组和单独的订阅来组织和跟踪 PostgreSQL 实例和相关资源的 Azure 数据库。 定期协调库存，确保及时从订阅中删除未经授权的资源。
...
**指南**：使用 Azure 条件访问通过为"Microsoft Azure 管理"应用配置"阻止访问"来限制用户与 Azure 资源管理器交互的能力。 这可以防止在高安全性环境中创建和更改资源，例如包含敏感信息的 PostgreSQL Azure 数据库实例。
...
**指南**：使用 Azure 策略为 PostgreSQL 实例定义和实现 Azure 数据库的标准安全配置。 在"Microsoft.DBforPostgreSQL"命名空间中使用 Azure 策略别名创建自定义策略，以审核或强制执行 PostgreSQL 实例的 Azure 数据库的网络配置。 您还可以使用与 PostgreSQL 实例的 Azure 数据库相关的内置策略定义，例如：
...
**指南**：如果对 PostgreSQL 实例和相关资源使用 Azure 数据库的自定义 Azure 策略定义，请使用 Azure 存储库安全地存储和管理代码。
...
**指南**：使用"Microsoft.DBforPostgreSQL"命名空间中的 Azure 策略别名创建自定义策略以警报、审核和强制执行系统配置。 此外，开发用于管理策略异常的流程和管道。
...
**指南**：使用"Microsoft.DBforPostgreSQL"命名空间中的 Azure 策略别名创建自定义策略以警报、审核和强制执行系统配置。 使用 Azure 策略 [审核]、[拒绝]和 [部署（如果不存在））自动为 PostgreSQL 实例和相关资源强制实施 Azure 数据库的配置。
...
**指南**：对于在 Azure 应用服务上运行的 Azure 虚拟机或 Web 应用程序，用于访问用于 PostgreSQL 实例的 Azure 数据库，请使用 Azure 密钥保管库的托管服务标识来简化和保护 Azure 数据库，以便 PostgreSQL 密钥管理。 确保启用密钥保管库软删除。
...
**指南**： 后格雷SQL 服务器的 Azure 数据库支持 Azure 活动目录身份验证（预览版）以访问数据库。  为 PostgreSQL 服务器创建 Azure 数据库时，可以为管理员用户提供凭据。 此管理员可用于创建其他数据库用户。  

对于在 Azure 应用服务上运行的 Azure 虚拟机或 Web 应用程序，用于访问 PostgreSQL 服务器的 Azure 数据库，请使用 Azure 密钥保管库的托管服务标识来存储和检索 PostgreSQL 服务器的 Azure 数据库凭据。 确保启用密钥保管库软删除。
...
**指南**：在支持 Azure 服务的基础主机上启用 Microsoft 反恶意软件（例如，用于 PostgreSQL 的 Azure 数据库），但它不在客户内容上运行。
...
在支持 Azure 服务的基础主机上启用 Microsoft 反恶意软件（例如，用于 PostgreSQL 的 Azure 数据库），但它不在客户内容上运行。
...
**指南**：用于 PostgreSQL 的 Azure 数据库获取数据文件和事务日志的备份。 根据支持的最大存储大小，我们要么进行完整备份和差分备份（最多 4 TB 存储服务器）或快照备份（最多 16 TB 存储服务器）。 可以通过这些备份将服务器还原到所配置的备份保留期中的任意时间点。 默认的备份保留期为七天。 可以选择将其配置为长达 35 天。 所有备份都使用 AES 256 位加密进行加密。

如何备份 Azure 数据库中的服务器，以便 PostgreSQL：https://docs.microsoft.com/azure/postgresql/howto-restore-server-portal

了解 Azure 数据库，用于 PostgreSQL 初始配置：https://docs.microsoft.com/azure/postgresql/tutorial-design-database-using-azure-portal
...
**指南**：用于 PostgreSQL 的 Azure 数据库根据用户的选择自动创建服务器备份并将其存储在本地冗余存储或异地冗余存储中。 备份可以用来将服务器还原到某个时间点。 备份和还原是任何业务连续性策略的基本组成部分，因为它们可以保护数据免遭意外损坏或删除。

如果使用 Azure 密钥保管库为 PostgreSQL 实例存储 Azure 数据库的凭据，请确保定期自动备份密钥。
...
**指南**：在 PostgreSQL 的 Azure 数据库中，执行还原会从原始服务器的备份创建新服务器。 有两种类型的还原可用：时间点还原和地理还原。 时间点还原：可以与任一备份冗余选项配合使用，所创建的新服务器与原始服务器位于同一区域。 异地还原：只能在已将服务器配置为进行异地冗余存储的情况下使用，用于将服务器还原到另一区域。

估计的恢复时间取决于若干因素，包括数据库大小、事务日志大小、网络带宽，以及在同一区域同时进行恢复的数据库总数。 恢复时间通常少于 12 小时。

定期测试 Azure 数据库的还原 PostgreSQL 实例。
...
**指南**： 用于 PostgreSQL 的 Azure 数据库需要完整、差异和事务日志备份。 可以通过这些备份将服务器还原到所配置的备份保留期中的任意时间点。 默认的备份保留期为七天。 可以选择将其配置为长达 35 天。 所有备份都使用 AES 256 位加密进行加密。

了解 PostgreSQL Azure 数据库中的备份和还原：https://docs.microsoft.com/azure/postgresql/concepts-backup
