...
使用 SSL/TLS 并验证根证书的所有应用程序都需要更新根证书才能连接到 Azure Database for PostgreSQL。 如果当前未使用 SSL/TLS (sslmode = disable) ，或者你不是验证服务器证书 (sslmode = allow、sslmode = 首选或 sslmode = 需要) ，则不会影响应用程序的可用性。 可以通过 [此处](concepts-ssl-connection-security.md#enforcing-tls-connections)) 验证客户端应用程序是否尝试使用具有预定义的受信任证书颁发机构的 SSL 模式 (CA。
...
如果在2020年11月30日之前未更新根证书，则通过 SSL/TLS 连接并对根证书进行验证的应用程序将无法与 PostgreSQL 数据库服务器通信，应用程序将遇到 PostgreSQL 数据库服务器的连接问题。
...
Azure Database for PostgreSQL 使用的这些证书由受信任的证书颁发机构 (CA) 提供。 因此，Azure Database for PostgreSQL 上对这些证书的支持与 CA 的支持这些证书相关联。 但是，在这种情况下，这些预定义的证书中可能出现意外的错误，这些错误需要尽快解决。
