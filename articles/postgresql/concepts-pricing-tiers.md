...
如果可用存储小于 512MB 或预配存储大小的 5%，则将标记为仅读 100 GB 预配存储的服务器。 对于预配存储超出 100 GB 的服务器，如果可用存储少于 5 GB，则会将其标记为只读。
...
存储自动增长可防止服务器耗尽存储空间并变为只读。 如果启用了存储自动增长，存储会在不影响工作负荷的情况下自动增长。 对于配置存储小于 100 GB 的服务器，一旦可用存储低于 1 GB 或预配存储的 10%， 则预配存储大小将增加 5 GB。 对于具有 100 GB 以上预配存储的服务器，当可用存储空间低于 10 GB 或预配存储大小的 5%时，预配存储大小将增加 5%。 适用上面指定的最大存储限制。
