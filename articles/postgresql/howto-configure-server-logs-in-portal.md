---
title: 管理日志 - Azure 门户 - 用于 PostgreSQL 的 Azure 数据库 - 单个服务器
description: 本文介绍如何在 Azure 数据库中配置和访问服务器日志 （.log 文件）， 用于 PostgreSQL - 从 Azure 门户获取单个服务器。
author: rachel-msft
ms.author: raagyema
...
- 请参阅[CLI 中的访问服务器日志](howto-configure-server-logs-using-cli.md)，了解如何以编程方式下载日志。
- 详细了解 Azure Database for PostgreSQL 中的[服务器日志](concepts-server-logs.md)。 
- 有关参数定义和 PostgreSQL 日志记录的详细信息，请参阅 PostgreSQL 有关[错误报告和日志记录](https://www.postgresql.org/docs/current/static/runtime-config-logging.html)的文档。
