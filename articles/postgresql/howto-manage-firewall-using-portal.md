...
description: 使用 Azure 门户创建和管理 Azure Database for PostgreSQL 单个服务器的防火墙规则
author: rachel-msft
ms.author: raagyema
...
ms.locfileid: "74770299"
---
# <a name="create-and-manage-firewall-rules-for-azure-database-for-postgresql---single-server-using-the-azure-portal"></a>使用 Azure 门户创建和管理 Azure Database for PostgreSQL 单个服务器的防火墙规则
服务器级防火墙规则可用于从指定的 IP 地址或 IP 地址范围管理对 Azure Database for PostgreSQL 服务器的访问。
...
4. 添加其他地址范围。 在 Azure Database for PostgreSQL 防火墙规则中，可以指定单个 IP 地址，也可以指定某个范围的地址。 如果希望将规则限制为单个 IP 地址，请在起始 IP 和结束 IP 字段中输入相同的地址。 打开防火墙后，管理员、用户和应用程序便可以访问其拥有有效凭据的 PostgreSQL 服务器上的任何数据库。
