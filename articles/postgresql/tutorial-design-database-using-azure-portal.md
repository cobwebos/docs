---
title: "使用 Azure 门户设计你的第一个 Azure Database for PostgreSQL | Microsoft Docs"
description: "本教程演示如何使用 Azure 门户设计你的第一个 Azure Database for PostgreSQL。"
services: postgresql
author: SaloniSonpal
ms.author: salonis
manager: jhubbard
...
---
# <a name="design-your-first-azure-database-for-postgresql-using-the-azure-portal"></a>使用 Azure 门户设计你的第一个 Azure Database for PostgreSQL
...
## <a name="create-an-azure-database-for-postgresql"></a>创建 Azure Database for PostgreSQL
...
1. 在 Azure 门户的左侧菜单中，单击“所有资源”，然后搜索刚创建的服务器 mypgserver-20170401。
...
4. 选择服务器的“概述”页面。 记下“服务器名称”和“服务器管理员登录名”。
 ...
 ![Azure Database for PostgreSQL - 服务器管理员登录名](./media/tutorial-design-database-using-azure-portal/6-server-name.png)


## <a name="connect-to-postgresql-database-using-psql-in-cloud-shell"></a>使用 Cloud Shell 中的 psql 连接到 PostgreSQL 数据库

现在使用 psql 命令行实用工具连接到 Azure Database for PostgreSQL 服务器。 
1. 通过顶部导航窗格中的终端图标启动 Azure Cloud Shell。

   ![Azure Database for PostgreSQL - Azure Cloud Shell 终端图标](./media/tutorial-design-database-using-azure-portal/7-cloud-shell.png)

2. Azure Cloud Shell 在浏览器中打开，支持键入 bash 命令。

   ![Azure Database for PostgreSQL - Azure Shell Bash 提示](./media/tutorial-design-database-using-azure-portal/8-bash.png)

3. 出现 Cloud Shell 提示时，请使用 psql 命令连接到 Azure Database for PostgreSQL 服务器。 借助 [psql](https://www.postgresql.org/docs/9.6/static/app-psql.html) 实用工具使用以下格式连接到 Azure Database for PostgreSQL 服务器：
   ```bash
   psql --host=<myserver> --port=<port> --username=<server admin login> --dbname=<database name>
   ```

   例如，以下命令使用访问凭据连接到 PostgreSQL 服务器 mypgserver-20170401.postgres.database.azure.com 上名为“postgres”的默认数据库。 出现提示时输入服务器管理员密码。
...
接下来了解如何使用 Azure CLI 执行类似的任务，请查看本教程：[使用 Azure CLI 设计你的第一个 Azure Database for PostgreSQL](tutorial-design-database-using-azure-cli.md)
