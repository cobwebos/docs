---
title: "在 Azure Database for PostgreSQL 中进行监视 | Microsoft Docs"
description: "本文介绍了用于对 Azure Database for PostgreSQL 进行监视并发出警报的指标，包括 CPU、限制、存储和连接统计信息。"
services: postgresql
author: rachel-msft
ms.author: raagyema
manager: jhubbard
...
|compute_limit|计算单位限制|计数|此服务器的最大计算单位数|
...
> [!NOTE]
> 计算单位由内存和 CPU 构成。 计算单位百分比为“最大值(内存%, CPU%)”。 检查内存和 CPU 图表，找出是哪项导致计算单位百分比发生更改。 有关详细信息，请参阅[计算单位](concepts-compute-unit-and-storage.md)。
