...
2.  如果应用程序使用完全限定的域名 (FQDN)，则使用[自定义域](manage-apps/application-proxy-configure-custom-domain.md)发布应用程序。 此功能允许相同 URL 同时在内部和外部使用。

    此选项确保应用程序内的链接可通过应用程序代理从外部访问，因为应用程序内指向内部 URL 的链接也可从外部识别。 所有链接仍需要属于已发布的应用程序。 但使用此选项，链接不需要属于同一应用程序，而是可以属于多个应用程序。

3.  如果这些选项都不可行，可预览进行 URL 转换/重写的新功能。 使用此功能，应用程序 HTML 正文中存在的内部 URL 或链接将转换为或“映射”到已发布的外部应用代理 URL。 此转换仅适用于 HTML 或 CSS 中的链接，不适用于通过 JS 生成的链接。 

因此，我们强烈建议尽可能使用[自定义域](manage-apps/application-proxy-configure-custom-domain.md)解决方案。 如果确实想要加入预览，请将电子邮件发送到 <aadapfeedback@microsoft.com>（随附 applicationId）。
