...
6.  选择 VM 大小。 若要查看更多的大小，请选择“全部查看”或更改“支持的磁盘类型”筛选器。 在“设置”页中保留默认值，然后单击“确定”。
    ...
    ![Alt 图像文本](../managed-service-identity/media/msi-tutorial-windows-vm-access-arm/msi-windows-vm.png)
...
5. 若要查看并确认在此 VM 上安装了哪些扩展，请单击“扩展”。 如果 MSI 已启用，列表中会显示现“ManagedIdentityExtensionforWindows”。
    ...
    ![Alt 图像文本](../managed-service-identity/media/msi-tutorial-windows-vm-access-arm/msi-windows-extension.png)

## <a name="grant-your-vm-access-to-a-database-in-an-azure-sql-server"></a>授予 VM 对 Azure SQL 服务器中的数据库的访问权限

现在，可以授予 VM 对 Azure SQL 服务器中的数据库的访问权限。  对于此步骤，可以使用现有的或创建新的 SQL 服务器。  若要使用 Azure 门户创建新的服务器和数据库，请遵循此 [Azure SQL 快速入门](~/articles/sql-database/sql-database-get-started-portal.md)。 [Azure SQL 文档](~/articles/sql-database/index.yml)中还提供了有关使用 Azure CLI 和 Azure PowerShell 执行这些操作的快速入门。
...
创建组并将 VM MSI 添加到成员身份后，可使用以下步骤[配置 SQL 服务器的 Azure AD 身份验证](~/articles/sql-database/sql-database-aad-authentication-configure.md#provision-an-azure-active-directory-administrator-for-your-managed-instance)：
...
- [使用 SQL 数据库和 SQL 数据仓库进行通用身份验证（MFA 的 SSMS 支持）](~/articles/sql-database/sql-database-ssms-mfa-authentication.md)
- [使用 SQL 数据库或 SQL 数据仓库配置和管理 Azure Active Directory 身份验证](~/articles/sql-database/sql-database-aad-authentication-configure.md)
...
- 详细了解 [Azure SQL 对 Azure AD 身份验证的支持](~/articles/sql-database/sql-database-aad-authentication.md)。
- 详细了解如何[配置 Azure SQL 对 Azure AD 身份验证的支持](~/articles/sql-database/sql-database-aad-authentication-configure.md)。
