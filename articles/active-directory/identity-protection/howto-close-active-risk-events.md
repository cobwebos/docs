---
title: 如何在 Azure Active Directory 标识保护中关闭活动风险事件 | Microsoft Docs
...
如果已自动检测到指定的用户风险级别, 则可以通过配置[用户风险条件性访问策略](howto-user-risk-policy.md)来要求更改密码。 
...
- **解决** - 如果在调查风险事件之后在“标识保护”外部采取适当的补救措施，并且应该将风险事件视为已关闭，请将事件标记为“已解决”。 解决的事件会将风险事件的状态设置为“已关闭”，此风险事件不再算作用户风险。
