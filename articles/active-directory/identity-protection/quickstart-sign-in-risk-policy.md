...
## <a name="create-your-conditional-access-policy"></a>创建条件性访问策略 

本快速入门中的方案使用 Tor 浏览器中的登录名生成检测到的“从匿名 IP 地址登录”风险事件。  此风险事件的风险级别为中等。 若要响应此风险事件，请将登录风险条件设置为中。 

本部分演示如何创建所需登录风险条件性访问策略。 在策略中，设置：
...
## <a name="test-your-conditional-access-policy"></a>测试条件性访问策略

若要测试策略，请尝试使用 Tor 浏览器以 **Alan Charon** 身份登录到 [Azure 门户](https://portal.azure.com)。 应通过条件性访问策略阻止你在登录尝试。
...
如果不再需要删除测试用户，Tor 浏览器并禁用登录风险条件性访问策略：
