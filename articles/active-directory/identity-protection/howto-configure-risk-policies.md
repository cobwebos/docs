...
当用户成功完成由登录风险策略触发的 MFA 提示符时, 它会向从合法用户进行登录的身份保护提供反馈。 因此, 触发 MFA 提示符的登录风险事件将自动关闭, 并且标识保护将阻止此事件引起用户风险提升。 启用登录风险策略可允许用户在出现 MFA 提示时自行更正, 并随后自动关闭关联的风险登录, 从而减少风险登录视图中的 noisiness。
...
Azure AD 会分析用户的每次登录。 分析的目的是检测伴随登录而来的可疑操作。 在 Azure AD 中，系统可以检测到的可疑操作也称为风险事件。 尽管某些风险事件可以实时检测到，但有些风险事件需要更长的时间才能检测到。 例如，若要检测异常位置的不可能行程，系统需要花费 14 天的初始学习期来学习用户的常规行为。 可通过多个选项解决检测到的风险事件。 例如, 你可以手动解决各个风险事件, 也可以使用登录风险或用户风险条件性访问策略来解决它们。
