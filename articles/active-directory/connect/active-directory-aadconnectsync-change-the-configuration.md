 ...
 * 如果之前配置 PreferredDataLocation 属性上现有在使用 Azure AD PowerShell 的 Azure AD 中同步用户对象，则必须**向后移植**本地 Active Directory 中的相应用户对象的属性值。
    ...
    > [!IMPORTANT]
    > 如果不向后移植到本地 Active Directory 中的相应用户对象的属性值，Azure AD Connect 将在启用同步 PreferredDataLocation 属性时的 Azure AD 中删除现有的属性值。
...
### 步骤 2：将源属性添加到本地 AD 连接器架构
并非所有 AD 属性都将导入本地 AD 连接器空间。 若要将源属性添加到导入属性的列表：

 1. 在 Synchronization Service Manager 中转到“连接器”选项卡。
 
 2. 右键单击“本地 AD 连接器”，然后选择“属性”。
...
### 步骤 3：将 PreferredDataLocation 添加到 Azure AD 连接器架构
默认情况下，PreferredDataLocation 属性不会导入 Azure AD 连接空间。 若要将 PreferredDataLocation 属性添加到导入属性的列表：
...
### 步骤 4：创建流从本地 Active Directory 的属性值的入站的同步规则
...
5. 转到“范围筛选器”选项卡，然后添加**包含以下子句的单个范围筛选器组**：
...
### 步骤 5：创建流到 Azure AD 的属性值的出站同步规则
...
4. 在“说明”选项卡下面提供以下配置：
    ...
    | 属性 | 值 | 详细信息 |
    | --- | --- | --- |
    | Name | *提供名称* | 例如，“Out to AAD – User PreferredDataLocation” |
    | 说明 | *提供说明* |
    | 连接的系统 | *选择 AAD 连接器* |
    | 连接的系统对象类型 | 用户 ||
    | Metaverse 对象类型 | **Person** ||
...
5. 转到“范围筛选器”选项卡，然后添加**包含两个子句的单个范围筛选器组**：
...
1. 在**本地 AD 连接器**上运行**完全导入**步骤：
   ...
   2. 右键单击“本地 AD 连接器”，然后选择“运行...”

   3. 在弹出对话框中，选择“完全导入”，然后单击“确定”。
...
2. 在**Azure AD 连接器**上运行**完全导入**步骤：
   ...
   2. 在弹出对话框中，选择“完全导入”，然后单击“确定”。
   
   3. 等待操作完成。

3. 验证现有 User 对象上的同步规则更改：

源属性已从本地 Active Directory 和 Azure AD 中的 PreferredDataLocation 导入相应的连接器空间。 然后再继续进行完全同步步骤，建议执行**预览**上现有用户对象中在本地 AD 连接器空间。 选择的对象应具有填充的源属性。 成功**预览**填充 Metaverse 中 PreferredDataLocation 是一个很好的指标，表示已配置同步规则正确。 有关如何执行**预览**的信息，请参阅[验证更改](#verify-the-change)部分。

4. 在**本地 AD 连接器**上运行**完全同步**步骤：

   1. 右键单击“本地 AD 连接器”，然后选择“运行...”
  
   2. 在弹出对话框中，选择“完全同步”，然后单击“确定”。
   
   3. 等待操作完成。

5. 验证 Azure AD 的**挂起的导出**：

   1. 右键单击“Azure AD 连接器”，然后选择“搜索连接器空间”。
