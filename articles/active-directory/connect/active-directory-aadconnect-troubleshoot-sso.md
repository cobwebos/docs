...
- 将 Azure AD 服务 URL（https://autologon.microsoftazuread-sso.com、https://aadg.windows.net.nsatc.net）添加到“受信任的站点”区域而非“本地 Intranet”区域。
...
- 在 PowerShell 中，调用 `New-AzureADSSOAuthenticationContext`。 这应当会提供一个弹出窗口，用于输入你的 Azure AD 租户管理员凭据。
...
- 调用 `New-AzureADSSOAuthenticationContext`。 这应当会提供一个弹出窗口，用于输入你的 Azure AD 租户管理员凭据。
