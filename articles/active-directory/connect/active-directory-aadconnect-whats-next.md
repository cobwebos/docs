...
将用户同步到云后，你需要向他们分配许可证，以便其可以继续使用 Office 365 等云应用。
...
<center>![Directory 同步时间](./media/active-directory-aadconnect-whats-next/verify.png)</center>

<a id="start-a-scheduled-synchronization-task" class="xliff"></a>

## 启动计划的同步任务
如果需要运行同步任务，可以通过再次运行 Azure AD Connect 向导来执行此操作。  需要提供 Azure AD 凭据。  在向导中，选择“自定义同步选项”任务，然后一直单击“下一步”完成向导。 最后，请确保已选中“初始配置完成后立即开始同步过程”框。
...
在完成 Azure AD Connect 的初始安装后，你随时可以从 Azure AD Connect 启动页或桌面快捷方式再次启动向导。  你会发现，再次运行向导会以其他任务的形式提供一些新选项。  
