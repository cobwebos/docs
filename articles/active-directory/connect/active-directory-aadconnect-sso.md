...
Azure Active Directory 无缝单一登录（Azure AD 无缝 SSO）为登录到企业网络中连接的企业计算机的用户提供真正的单一登录。 启用此功能后，用户无需键入其密码即可登录到 Azure AD；在大多数情况下，甚至无需键入其用户名。 此功能可让用户轻松访问基于云的服务，而无需使用其他任何本地组件。
...
在能够进行 Kerberos 身份验证的桌面（例如基于 Windows 的计算机）上，支持[新式身份验证](https://aka.ms/modernauthga)的基于 Web 浏览器的客户端和 Office 客户端支持无缝 SSO。 下表详细描述了各个操作系统上基于浏览器的客户端。
...
- 如果对 Azure AD 发出的登录请求包含 `domain_hint`（标识 Azure AD 租户，例如 contoso.onmicrosoft.com）或 `login_hint`（标识用户的用户名，例如 user@contoso.onmicrosoft.com 或 user@contoso.com）参数，将执行以下步骤。
- 如果 Azure AD 登录请求中不包含上述任何参数，则执行以下步骤后，系统将要求该用户提供其用户名。
...
无缝 SSO 是机会性功能，这表示如果因为某些原因无法使用该功能，用户只需和以往一样在登录页输入密码即可。
...
由于在 Azure AD 中用于无缝 SSO 的服务 URL 包含句点，因此需要将这些 URL 显式添加到每个用户的浏览器中的 Intranet 区域设置。 这样，浏览器便可以自动将当前登录用户的 Kerberos 票证发送到 Azure AD。 在每台计算机上执行此操作时，将所需 URL 添加到所有用户的 Intranet 区域的最简单方法是在 Active Directory 中创建组策略。
...
屏幕应如下所示：![单一登录](./media/active-directory-aadconnect-sso/sso7.png)
...
5. 确保已在企业网络中连接该桌面。
6. 确保桌面的时间与 Active Directory 同步，并且各域控制器的时间偏差不超过 5 分钟。
7. 从用户的桌面中清除现有的 Kerberos 票证。 为此，可以在命令提示符下运行 **klist purge** 命令。
8. 查看浏览器的控制台日志（在“开发人员工具”下）来帮助确定潜在问题。
