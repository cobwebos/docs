...
|/service/tenant/configuration/save/action|创建包含配置快照的、目标为储存库中的指定的分支的提交内容|
...
|/automationAccounts/configurations/readContent/action|获取 Azure Automation DSC 的内容|
...
|/operatingSystemFamilies/read|列出 Microsoft Azure 中可用的来宾操作系统系列，同时列出可用于每个系列的操作系统版本
...
|/virtualMachineScaleSets/scale/action|缩减/扩展现有虚拟机规模集的实例计数|
...
|/accounts/dataLakeStoreAccounts/read|获取 DataLakeAnalytics 帐户的 DataLakeStore 帐户。|
...
| 操作 | 说明 |
|---|---|
|/jitNetworkAccessPolicies/read|获取适时网络访问策略|
|/jitNetworkAccessPolicies/write|创建新的更新现有的适时网络访问策略|
|/jitNetworkAccessPolicies/initiate/action|启动适时网络访问策略|
...
|/servers/firewallRules/write|创建或更新用于控制可连接到服务器的 IP 地址范围的服务器防火墙规则|
...
|/servers/recommendedElasticPools/databases/read|检索应添加到为给定服务器建议的弹性数据库池中的数据库|
...
|/servers/databases/dataMaskingPolicies/read|检索在给定的数据库上配置的数据掩码策略的详细信息|
