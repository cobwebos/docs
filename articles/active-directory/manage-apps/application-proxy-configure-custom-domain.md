...
通过 Azure Active Directory 应用程序代理发布应用程序时，将为用户创建外部 URL。 此 URL 的默认域为 yourtenant.msappproxy.net**。 例如，如果在你的*租户中发布名为* *Contoso*的应用，则外部 URL 为*https：\//expenses-contoso.msappproxy.net*。 如果要使用自己的域名而不是*msappproxy.net*，则可为应用程序配置自定义域。 
...
只要通配符与外部 URL 匹配，就可以使用通配符证书。 必须为[通配符应用程序](application-proxy-wildcard.md)使用通配符证书。 如果要使用证书还访问子域，则必须在同一证书中将子域通配符作为使用者备用名称添加。 例如，除非你添加* \*apps.adventure-works.com*作为使用者备用名称，否则， * \*adventure-works.com*的证书不能用于* \*apps.adventure-works.com。* 
