...
2. **本地代理服务器基础结构**：此基础结构是一种能够进行传输层安全（TLS）检查的代理设备。 必须将代理配置为将包含允许租户列表的标头插入到发往 Azure AD 的流量。
...
下图演示了大致的流量流。 租户限制要求仅对要 Azure AD 的流量进行 TLS 检查，而不需要对 Office 365 云服务进行 TLS 检测。 这一区别很重要，因为对 Azure AD 进行身份验证的流量通常比 SaaS 应用程序（如 Exchange Online 和 SharePoint Online）的流量要小得多。
...
- 客户端必须信任代理提供的用于 TLS 通信的证书链。 例如，如果使用内部[公钥基础结构（PKI）](/windows/desktop/seccertenroll/public-key-infrastructure)中的证书，则必须信任内部颁发根证书颁发机构证书。
...
- 对于 "*限制访问-租户*"，请使用 " \<允许的租户列表\>" 值，它是要允许用户访问的租户的逗号分隔列表。 已注册到某个租户的任何域都可用于在此列表中标识该租户。 例如，若要允许访问 Contoso 和 Fabrikam 租户，名称/值对如下所示： `Restrict-Access-To-Tenants: contoso.onmicrosoft.com,fabrikam.onmicrosoft.com`
