...
- 只有一个连接器。 为了避免这种停机并改善高可用性，我们建议安装另一个连接器并[创建连接器组](application-proxy-connector-groups.md)。  
...
可在“服务”窗口中检查服务的状态。 连接器包含两个 Windows 服务：实际的连接器和更新程序。 二者都必须一直运行。
 ...
 ![AzureAD 本地服务](./media/application-proxy-connectors/aad-connector-services.png)
