...
- **安全**。 本地应用程序可以使用 Azure 的授权控制和安全分析功能。 例如，本地应用程序可以使用条件访问和两步验证。 应用程序代理不要求通过防火墙打开入站连接。
...
建议使用应用程序代理来允许远程用户访问内部资源。 应用程序代理取代了对 VPN 或反向代理的需求。 它不适用于公司网络上的内部用户。  这些不必要地使用应用程序代理的用户可能会带来意外和不受欢迎的性能问题。
