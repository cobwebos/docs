...
---
# <a name="how-to-deploy-the-access-panel-extension-for-internet-explorer-using-group-policy"></a>如何：使用组策略部署 Internet 资源管理器的访问面板扩展
...
首先，必须将安装程序包放在网络位置中，该位置可以由要在其上远程安装扩展的计算机访问。 为此，请按照下列步骤进行操作：
...
1. 转到"**共享"** 选项卡。然后单击 **"** > **任务"""新共享..."...**

    ![屏幕截图显示从"任务"屏幕中查找"新共享"的位置](./media/deploy-access-panel-browser-extension/shares.png)

1. 完成“新建共享向导”并设置权限，确保可以从用户的计算机访问该共享位置。**** [了解有关共享的详细信息。](https://technet.microsoft.com/library/cc753175.aspx)
1. 下载以下 Microsoft Windows 安装程序包（.msi 文件）：[Access Panel Extension.msi](https://account.activedirectory.windowsazure.com/Applications/Installers/x64/Access%20Panel%20Extension.msi)
...
## <a name="step-2-create-the-group-policy-object"></a>第 2 步：创建组策略对象

1. 登录到承载活动目录域服务 （AD DS） 安装的服务器。
1. 在服务器管理器中，转到**工具** > **组策略管理**。
...
1. 在“组策略管理”窗口的左窗格中，查看组织单位 (OU) 层次结构并确定想要应用组策略的范围。**** 例如，可以选择一个小型 OU 并将其部署到少量用户以进行测试，或者选择一个顶级 OU 并将其部署到整个组织。
   ...
   > [!NOTE]
   > 如果要创建或编辑组织单位 （O），请切换回服务器管理器，然后转到**工具** > **活动目录用户和计算机**。

1. 选择 OU 后，请右键单击它，并选择“在这个域中创建 GPO 并在此处链接它...”****

    ![屏幕截图显示创建新的 GPO 选项](./media/deploy-access-panel-browser-extension/create-gpo.png)
...
1. 转到包含[步骤 1：创建分发点](#step-1-create-the-distribution-point)中所述安装程序包的共享文件夹，选择 .msi 文件，并单击“打开”。****
...
## <a name="step-4-auto-enable-the-extension-for-internet-explorer"></a>第 4 步：自动启用 Internet 资源管理器的扩展
...
1. 右键单击“加载项列表”，并选择“编辑”。********
    ...
    ![右键单击"加载列表"并选择"编辑"](./media/deploy-access-panel-browser-extension/edit-add-on-list.png)
...
1. 在“显示内容”窗口中执行以下步骤：****
   ...
   1. 单击“确定”关闭“显示内容”窗口。********
      ...
      ![填写上一步中指定的值](./media/deploy-access-panel-browser-extension/show-contents.png)
...
![显示"您要存储密码..."提示](./media/deploy-access-panel-browser-extension/remember-password-prompt.png)
...
1. 查找名为“对表单上的用户名和密码打开自动完成功能”的设置。****
   ...
   > 旧版的 Active Directory 可能列出此设置，但其名称为“不允许自动完成功能保存密码”。**** 该设置的配置不同于本教程中所述的设置。
    ...
    ![请记住在"用户设置"下查找此内容](./media/deploy-access-panel-browser-extension/disable-auto-complete.png)

1. 右键单击上述设置，并选择“编辑”。****
1. 在标题为“对表单上的用户名和密码打开自动完成功能”的窗口中选择“禁用”。********

    ![为自动完成功能的"已禁用"选项选择"已禁用"选项](./media/deploy-access-panel-browser-extension/disable-passwords.png)
...
## <a name="step-6-testing-the-deployment"></a>第 6 步：测试部署
...
1. 组策略更改可能需要几个登录才能使用此计算机完全更新。 要强制更新，请打开“命令提示”窗口，并运行以下命令：****`gpupdate /force`
...
1. 在“管理加载项”窗口中，检查“访问面板扩展”是否已安装且其“状态”设置为“已启用”。****************
   ...
   ![验证"访问面板扩展"是否安装并启用](./media/deploy-access-panel-browser-extension/verify-install.png)
