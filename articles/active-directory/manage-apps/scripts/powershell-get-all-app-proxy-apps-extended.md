...
description: PowerShell 示例列出了所有 Azure 活动目录 （Azure AD） 应用程序代理应用程序以及应用程序 ID （AppId）、名称（显示名称）、外部 URL（外部 Url）、内部 URL（内部 Url）和身份验证类型（外部身份验证类型）。
...
---
# <a name="get-all-application-proxy-apps-and-list-extended-information"></a>获取所有应用程序代理应用和列表扩展信息

此 PowerShell 脚本示例列出了有关所有 Azure 活动目录 （Azure AD） 应用程序代理应用程序的信息，包括应用程序 ID （AppId）、名称（显示名称）、外部 URL（外部 URL）、内部 URL（内部 Url）和身份验证类型（外部身份验证类型）。
