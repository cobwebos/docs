...
2. 使用 web 应用程序的服务帐户（计算机或专用域用户帐户）的 `principalsallowedtodelegateto` 属性，启用应用程序代理（连接器）的 Kerberos 身份验证委托。 应用程序服务器在 `webserviceaccount` 的上下文中运行，并且 `connectorcomputeraccount`委派服务器。 在 `webserviceaccount`域中的域控制器（运行 Windows Server 2012 R2 或更高版本）上运行以下命令。 为两个帐户使用平面名称（非 UPN）。
