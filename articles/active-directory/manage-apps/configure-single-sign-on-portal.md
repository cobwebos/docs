...
1. 对于[配置基本 SAML 选项](#configure-basic-saml-options)中所述的信息，请询问应用程序供应商。
1. 使用非生产环境测试本教程中的步骤。 如果没有 Azure AD 非生产环境，可以[获取一个月的试用版](https://azure.microsoft.com/pricing/free-trial/)。
...
1. 在显示的“Azure Active Directory”导航面板中的“管理”下，选择“企业应用程序”。    此时会显示 Azure AD 租户中应用程序的随机示例。
1. 在“应用程序类型”菜单中选择“所有应用程序”，然后选择“应用”    。
1. 输入要为其配置单一登录的应用程序的名称。 例如，可以输入 **GitHub-test** 以配置在[添加应用程序](add-application-portal.md)快速入门中添加的应用程序。  

   ![显示应用程序搜索栏的屏幕截图](media/configure-single-sign-on-portal/azure-portal-application-search.png)

1. 选择要为其配置单一登录的应用程序。
1. 在“管理”部分选择“单一登录”。  
1. 选择“SAML”以配置单一登录。  此时会显示“设置 SAML 单一登录 - 预览”页。 
...
1. 若要编辑基本 SAML 配置选项，请选择“基本 SAML 配置”部分右上角的“编辑”图标（铅笔图标）。  
     ...
     ![编辑基本 SAML 配置选项](media/configure-single-sign-on-portal/basic-saml-configuration-edit-icon.png)

1. 在该页上的相应字段中，输入步骤 1 中由应用程序供应商提供的信息。
1. 在页面顶部，选择“保存”  。
...
1. 若要修改“名称标识符值”，请选择“名称标识符值”字段旁边的“编辑”图标（铅笔图标）。    根据需要对标识符格式和源进行相应的更改。 完成后保存更改。 有关自定义声明的详细信息，请参阅[自定义在 SAML 令牌中为企业应用程序颁发的声明](../develop/active-directory-saml-claims-customization.md)操作指南文章。
1. 若要添加声明，请选择页面顶部的“添加新声明”。  输入**名称**，然后选择相应的源。 如果选择“属性”源，则需要选择所要使用的“源属性”。   如果选择“转换”源，则需要选择所要使用的“转换”和“参数 1”。   
1. 选择“保存”。  新声明将显示在表中。
...
1. 在“SAML 签名证书”部分，选择“新建证书”。  
1. 在显示的新证书行中，设置“过期日期”。  有关可用配置选项的详细信息，请参阅[高级证书签名选项](certificate-signing-options.md)一文。
...
1. 在门户中打开应用程序（如果尚未打开）。
1. 在应用程序的左侧导航面板中，选择“用户和组”。 
1. 选择“添加用户”。 
1. 在“添加分配”部分，选择“用户和组”。  
1. 若要查找特定的用户，请在“选择成员或邀请外部用户”框中键入相应的用户名。  选择该用户的个人资料照片或徽标，然后选择“选择”。 
1. 在“添加分配”部分，选择“分配”。   完成后，选定的用户会显示在“用户和组”列表中。 
...
1. 复制此部分中每个行的值。 然后，将每个值粘贴到“基本 SAML 配置”部分的相应行中。  例如，复制“设置 GitHub-test”部分的“登录 URL”值，并将其粘贴到“基本 SAML 配置”部分的“登录 URL”字段中，等等。    
...
1. 打开应用程序的单一登录设置。
1. 滚动到“使用 \<applicationName> 验证单一登录”部分。  在本教程中，此部分名为“设置 GitHub-test”。 
1. 选择“测试”。  此时会显示测试选项。
1. 选择“以当前用户身份登录”。  通过此测试，可以先了解单一登录是否对你（管理员）有效。
