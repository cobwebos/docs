...
在排除应用程序代理问题时，我们建议您从检查故障排除流、[调试应用程序代理连接器问题](application-proxy-debug-connectors.md)开始，以确定应用程序代理连接器是否配置正确。 如果连接到应用程序时仍有问题，请按照[调试应用程序代理应用程序中](application-proxy-debug-apps.md)的故障排除流操作。

如果在访问已发布应用程序或发布应用程序时出现错误，请检查以下选项，查看 Microsoft Azure AD 应用程序代理是否正确工作：

* 打开 Windows 服务控制台。 验证**Microsoft AAD 应用程序代理连接器**服务是否启用并运行。 另请查看应用程序代理服务属性页，如下图所示：  
  ![Microsoft AAD 应用程序代理连接器属性窗口屏幕截图](./media/application-proxy-troubleshoot/connectorproperties.png)
* 打开事件查看器，并在**应用程序和服务日志** > **Microsoft** > **AadApplicationProxy** > **连接器** > **管理员**中查找应用程序代理连接器事件。
...
| 错误 | 建议的步骤 |
| ----- | ----------------- |
| 连接器注册已失败：确保已在 Azure 管理门户中启用应用程序代理，并且已正确输入 Active Directory 用户名和密码。 错误：“发生了一个或多个错误。” | 如果关闭了注册窗口但没有登录到 Azure AD，请再次运行连接器向导并注册连接器。 <br><br> 如果注册窗口打开，然后立即关闭，而不允许您登录，您可能会收到此错误。 当系统上存在网络错误时，可能出现此错误。 确保可以从浏览器连接到公共网站，并且端口在[应用程序代理先决条件](application-proxy-add-on-premises-application.md#prepare-your-on-premises-environment)中指定处于打开状态。 |
| 注册窗口中出现明确错误。 无法继续 | 如果看到此错误，然后窗口关闭，这意味着输入的用户名或密码错误。 重试。 |
| 连接器注册已失败：确保已在 Azure 管理门户中启用应用程序代理，并且已正确输入 Active Directory 用户名和密码。 错误：AADSTS50059: 未在请求中找到或所提供的任何凭据均未暗示任何租户识别信息，并且服务主体 URI 的搜索已失败。 | 您尝试使用 Microsoft 帐户登录，而不是使用您尝试访问的目录的组织 ID 的域登录。 确保管理员是租户域的相同域名的一部分，例如，如果 Azure AD 域为 contoso.com，则管理员应为 admin@contoso.com。 |
| 无法检索运行 PowerShell 脚本的当前执行策略。 | 如果连接器安装失败，请检查以确保未禁用 PowerShell 执行策略。 <br><br>1. 打开组策略编辑器。<br>2. 转到**计算机配置** > **管理模板** > **Windows 组件** > **Windows PowerShell**和双击**打开脚本执行**。<br>3. 执行策略可以设置为 **"未配置**"或 **"已启用**"。 如果设置为“已启用”****，请确保在“选项”下将“执行策略”设置为“允许本地脚本和远程签名脚本”**** 或“允许所有脚本”****。 |
...
| 连接器注册失败：请确保您是活动目录的应用程序管理员来注册连接器。 错误：“注册请求被拒绝。” | 尝试登录时所使用的别名不是此域上的管理员。 始终为拥有用户域的目录安装连接器。 确保尝试登录的管理员帐户至少具有 Azure AD 租户的应用程序管理员权限。 |
| 由于网络问题，连接器无法连接到服务。 连接器尝试访问以下 URL。 | 连接器无法连接到应用程序代理云服务。 如果防火墙规则阻止连接，则可能发生此情况。 请确保您已允许访问[应用程序代理先决条件](application-proxy-add-on-premises-application.md#prepare-your-on-premises-environment)中列出的正确端口和 URLS。 |
...
| 错误 | 建议的步骤 |
| ----- | ----------------- |
| 无法检索运行 PowerShell 脚本的当前执行策略。 | 如果连接器安装已失败，请检查确保 PowerShell 执行策略未禁用。<br><br>1. 打开组策略编辑器。<br>2. 转到**计算机配置** > **管理模板** > **Windows 组件** > **Windows PowerShell**和双击**打开脚本执行**。<br>3. 执行策略可以设置为 **"未配置**"或 **"已启用**"。 如果设置为“已启用”****，请确保在“选项”下将“执行策略”设置为“允许本地脚本和远程签名脚本”**** 或“允许所有脚本”****。 |
...
| 网站无法显示页面。 | 如果应用程序是 OWA 应用程序，用户可能在尝试访问已发布的应用时得到此错误。 这可能由以下原因之一导致：<br><li>为此应用程序定义的 SPN 不正确。 确保针对此应用程序配置的 SPN 正确。</li><li>尝试访问应用程序的用户正在使用 Microsoft 帐户（而不是正确的公司帐户）登录，或者该用户是来宾用户。 确保用户使用与已发布应用程序的域匹配的公司帐户登录。 Microsoft 帐户用户和来宾无法访问 IWA 应用程序。</li><li>尝试访问应用程序的用户未在本地端为此应用程序正确定义。 请确保此用户具有为本地计算机上的此后端应用程序定义的适当权限。 |
| 无法访问此公司应用。 无权访问此应用程序。 授权失败。 确保向用户分配此应用程序的访问权限。 | 如果用户使用 Microsoft 帐户而不是其公司帐户登录，则用户在尝试访问您发布的应用时可能会收到此错误。 来宾用户也可能得到此错误。 Microsoft 帐户用户和来宾无法访问 IWA 应用程序。 确保用户使用与已发布应用程序的域匹配的公司帐户登录。<br><br>可能未针对此应用程序分配用户。 转到“应用程序”**** 选项卡，然后在“用户和组”**** 下将此用户或用户组分配到此应用程序。 |
| 当前无法访问此公司应用。 请稍后重试…连接器已超时。 | 如果用户未在本地端正确定义此应用程序，则用户在尝试访问您发布的应用时可能会收到此错误。 确保用户具有为本地计算机上的此后端应用程序定义的适当权限。 |
| 无法访问此公司应用。 无权访问此应用程序。 授权失败。 确保用户具有 Azure 活动目录高级版许可证。 | 如果用户的管理员未使用高级许可证显式分配应用，则用户在尝试访问您发布的应用时可能会收到此错误。 转到订阅者的活动目录**许可证**选项卡，并确保为此用户或用户组分配了高级许可证。 |
| 找不到具有指定主机名的服务器。 | 如果应用程序的自定义域配置不正确，则用户在尝试访问您发布的应用时可能会收到此错误。 按照[Azure AD 应用程序代理中处理自定义域](application-proxy-configure-custom-domain.md)的步骤，确保已上载域的证书并正确配置 DNS 记录 |
