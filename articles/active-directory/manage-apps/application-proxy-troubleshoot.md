...
排除故障时应用程序代理问题，我们建议您开始查看故障排除的流中，[调试应用程序代理连接器颁发](application-proxy-debug-connectors.md)，以确定是否已正确配置应用程序代理连接器。 如果你仍无法连接到该应用程序，请按照中的故障排除流[调试应用程序代理应用程序问题](application-proxy-debug-apps.md)。
...
| 连接器无法连接到服务，原因是网络问题。 连接器尝试访问以下 URL。 | 连接器不能连接到应用程序代理云服务。 如果有防火墙规则阻止连接，则可能发生这种情况。 请确保已允许访问到正确的端口和 URL 列入[应用程序代理先决条件](application-proxy-add-on-premises-application.md#prepare-your-on-premises-environment)。 |
