...
*错误 AADSTS70001： 在目录中找不到具有标识符\/"https： /contoso.com"的应用程序*。
...
## <a name="the-reply-address-does-not-match-the-reply-addresses-configured-for-the-application"></a>答复地址与为应用程序配置的回复地址不匹配

*错误 AADSTS50011：回复地址"https：/contoso.com"\/与为应用程序配置的回复地址不匹配*

**可能的原因**

SAML 请求中`AssertionConsumerServiceURL`的值与 Azure AD 中配置的"回复 URL"值或模式不匹配。 SAML 请求中`AssertionConsumerServiceURL`的值是您在错误中看到的 URL。

**分辨率**

确保 SAML 请求中`AssertionConsumerServiceURL`的值与 Azure AD 中配置的"回复 URL"值匹配。 如果在 Azure 门户中使用"我的应用安全浏览器扩展"中的[测试体验](../azuread-dev/howto-v1-debug-saml-sso-issues.md)，则无需手动执行这些步骤。
...
1.  当应用程序加载以后，请打开“基本 SAML 配置”。**** 验证或更新"回复 URL"文本框中的值，以匹配`AssertionConsumerServiceURL`SAML 请求中的值。    
    
更新 Azure AD 中的"回复 URL"值后，该值与应用程序在 SAML 请求中发送的值匹配，您应该能够登录到应用程序。

## <a name="user-not-assigned-a-role"></a>未为用户分配角色

*错误 AADSTS50105： 登录的用户"brian\@contoso.com"不会分配给应用程序的角色*。
...
1.  从应用程序列表中选择要将用户分配给的应用程序。

1.  加载应用程序后，从应用程序的左侧导航菜单中选择 **"用户"和"组**"。
...
1. 将鼠标悬停在列表中**的用户**上以显示**复选框**。 单击用户个人资料照片或徽标旁边的复选框，将用户添加到 **"选定"** 列表。

1. **可选：** 如果要**添加多个用户**，请在 **"按姓名或电子邮件地址搜索"** 框中键入另一个全名或电子邮件地址，然后单击该复选框将用户添加到 **"选定"** 列表。

1. 选择完用户后，单击 **"选择**"按钮将其添加到要分配给应用程序的用户和组列表中。

1. **可选：** 单击"**添加分配"** 窗格中的 **"选择角色**选择器"以选择要分配给所选用户的角色。

1. 单击"**分配"** 按钮将应用程序分配给选定的用户。
...
1. 捕获 SAML 请求。 请按照教程["如何调试 Azure AD 中基于 SAML 的单一登录](../azuread-dev/howto-v1-debug-saml-sso-issues.md)"来了解如何捕获 SAML 请求。
...
1. 选择在**SAML 签名证书**部分下**创建新证书**。

1. 选择到期日期，然后单击"**保存**"。

1. 选中**使新证书处于活动状态**以覆盖活动证书。 然后，单击窗格顶部的“保存”**** 并选择接受以激活滚动更新证书。

1. 在**SAML 签名证书**部分下，单击 **"删除"** 以删除**未使用的**证书。
...
要了解如何自定义发送到应用程序的 SAML 属性声明，请参阅 Azure[活动目录中的声明映射](../develop/active-directory-claims-mapping.md)。
