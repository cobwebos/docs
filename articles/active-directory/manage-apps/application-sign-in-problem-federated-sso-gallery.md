...
*错误 AADSTS70001:在目录\/* 中找不到标识符为 "https:/contoso.com" 的应用程序。
...
确保 SAML 请求`AssertionConsumerServiceURL`中的值与 Azure AD 中配置的 "回复 URL" 值匹配。 如果在[Azure 门户中使用](../develop/howto-v1-debug-saml-sso-issues.md)"我的应用" 安全浏览器扩展, 则无需手动执行这些步骤。
...
*错误 AADSTS50105:已登录用户 "brian\@contoso.com" 未分配给应用程序*的角色。
...
Azure AD 不支持应用程序为单一登录发送的 SAML 请求。 常见问题如下：
...
1. 捕获 SAML 请求。 按照教程[如何在 Azure AD 中调试基于 SAML 的单一登录应用程序](../develop/howto-v1-debug-saml-sso-issues.md), 了解如何捕获 saml 请求。
