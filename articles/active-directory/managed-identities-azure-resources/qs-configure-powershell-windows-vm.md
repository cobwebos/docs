...
2. 首先，使用 `Get-AzVM` cmdlet 检索 VM 属性。 然后，若要启用系统分配的托管标识，请在 [Update-AzVM](/powershell/module/az.compute/update-azvm) cmdlet 中使用 `-IdentityType` 开关：
