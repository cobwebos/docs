...
|           | unauthorized_client | 客户端无权使用此方法请求访问令牌。 | 由未使用本地环回来调用扩展的请求引起，或在没有正确配置 Azure 资源的虚拟机上运行。 如需 VM 配置方面的帮助，请参阅[使用 Azure 门户在 VM 上配置 Azure 资源的托管标识](qs-configure-portal-windows-vm.md)。 |
