...
* 可以让用户使用其 Azure AD 帐户自动登录到 Trello 单一登录 (SSO)
...
若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 Trello 用户。 换句话说，需要在 Azure AD 用户与 Trello 中相关用户之间建立链接关系。

可以通过将 Azure AD 中“用户名”的值分配为 Trello 中“用户名”的值来建立此链接关系。 若要配置和测试 Trello 的 Azure AD SSO，需要完成以下构建基块：
...
4. 在“配置应用设置”对话框页上，如果要在“IDP 发起的模式”下配置应用程序，请执行以下步骤：
   ...
   1. 在“回复 URL”文本框中，使用以下模式键入 URL：`https://trello.com/auth/saml/consume/<enterprise>`
...
5. 如果要在“配置应用设置”对话框页上以“SP 发起的模式”配置应用程序，则单击“显示高级设置(可选)”，然后输入“登录 URL”：
   
    ![配置单一登录](./media/active-directory-saas-trello-tutorial/tutorial_trello_08.png)
   
   1. 在“登录 URL”文本框中，使用以下模式键入 URL：`https://trello.com/auth/saml/consume/<enterprise>`
   2. 单击“资源组名称” 的 Azure 数据工厂。

6. 在“配置 Trello 的单一登录”页上，单击“下载证书”，然后在计算机上保存该文件。
   
    ![配置单一登录](./media/active-directory-saas-trello-tutorial/tutorial_trello_09.png)

7. 若要为应用程序配置 SSO，请转到 [Trello 企业 SSO 配置](https://trello.com/sso-configuration)页，向 Trello 团队发送登录 URL 并附加下载的证书。
