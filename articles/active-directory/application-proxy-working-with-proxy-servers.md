...
如果环境中包含出站代理，请确保将执行安装的帐户正确配置为使用出站代理。 由于安装程序在执行安装的用户的上下文中运行，因此可以使用 Microsoft Edge 或其他 Internet 浏览器来检查配置。

使用 Microsoft Edge 配置代理设置：
...
默认情况下，由连接器用来发出出站请求的底层 OS 组件会自动尝试在网络上查找代理服务器。 这些组件使用 Web 代理自动发现 (WPAD)（如果已在环境中启用）。
...
下一部分将介绍需要在出站代理上执行哪些配置步骤才能让流量通过出站代理。 但首先让我们了解如何将连接器配置为绕过本地代理，确保它与 Azure 服务建立直接连接。 这是建议的方法（如果网络策略允许这样做），因为这样可以少维护一项配置。
...
如前所述，在某些客户环境中，要求所有出站流量通过出站代理且不发生异常。 因此，绕过代理是不可行的。
...
2. 将代理设置配置为允许与 Azure AD 应用程序代理服务建立连接。
...
然后，对 C:\Program Files\Microsoft AAD App Proxy Connector Updater\ApplicationProxyConnectorUpdaterService.exe.config 路径下的文件进行类似的更改，将连接器更新程序服务配置为使用代理。
...
可以使用自选的监视工具。 本文使用了 Microsoft 网络监视器 3.4。 可[从 Microsoft 下载](https://www.microsoft.com/en-us/download/details.aspx?id=4865)该工具。
