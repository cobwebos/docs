...
* 可以让用户使用其 Azure AD 帐户自动登录到 Front 单一登录 (SSO)
...
本部分旨在说明如何基于名为“Britta Simon”的测试用户配置和测试 Front 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 Front 用户。 换句话说，需要建立 Azure AD 用户与 Front 中相关用户之间的关联关系。

可以通过将 Azure AD 中“用户名”的值分配为 Front 中“用户名”的值来建立此关联关系。

若要配置和测试 Front 的 Azure AD SSO，需要完成以下构建基块：
...
在本部分中，将在经典门户中启用 Azure AD SSO 并在 Front 应用程序中配置单一登录。

**若要配置 Front 的 Azure AD SSO，请执行以下步骤：**
...
3. 在“配置应用设置”对话框页上，如果要在“IDP 发起的模式”下配置应用程序，请执行以下步骤，然后单击“下一步”：
  ...
  1. 在“标识符”文本框中，使用以下模式键入 URL：`https://<company name>.frontapp.com`。
  2. 在“回复 URL”文本框中，使用以下模式键入 URL：`https://<company name>.frontapp.com/sso/saml/callback`
...
4. 如果要在“配置应用设置”对话框页上以“SP 发起的模式”配置应用程序，则单击“显示高级设置(可选)”，然后输入“登录 URL”，并单击“下一步”。
  ...
  1. 在“登录 URL”文本框中，使用以下模式键入 URL：`https://<company name>.frontapp.com`
...
本部分旨在通过授予 Britta Simon 访问 Front 的权限，允许她使用 Azure SSO。
