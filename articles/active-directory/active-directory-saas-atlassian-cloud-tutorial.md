...
在本部分中，将在经典门户中启用 Azure AD 单一登录并在 Atlassian Cloud 应用程序中配置单一登录。
...
8. 创建新 SAML 配置并添加标识提供者配置。
    ...
    a. 从 Azure AD 中复制“实体 ID”值并将其粘贴到“标识提供者实体 ID”字段中。
    
    b.保留“数据库类型”设置，即设置为“共享”。 复制“SAML SSO URL”并将其粘贴到“标识提供者 SSO URL”框中。

    c. 在记事本中打开从 Azure AD 下载的证书并复制其中包含的值（不要复制 Begin 和 End 行），然后将其粘贴到“公共 X509 证书”框中。
...
9. 更新 Azure AD 设置，确保设置正确的标识符 URL
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 从 SAML 屏幕复制“SP 标识 ID”并将其作为“标识符”值粘贴到 Azure AD 中。
...
5. 在“告诉我们有关此用户的信息”对话框页中，执行以下步骤： ![创建 Azure AD 测试用户](./media/active-directory-saas-atlassian-cloud-tutorial/create_aaduser_05.png) 
...
6.  在“用户配置文件”对话框页面上，执行以下步骤：![创建 Azure AD 测试用户](./media/active-directory-saas-atlassian-cloud-tutorial/create_aaduser_06.png) 
...
在本部分中，将在 Atlassian Cloud 中创建一个名为 Britta Simon 的用户。 在执行单一登录之前，该用户必须在 Atlassian Cloud 中存在。 请使用管理员权限登录到 Atlassian Cloud 实例，然后执行以下步骤。
...
在本部分中，将通过向 Britta Simon 授予对 Atlassian Cloud 的访问权限使她能够使用 Azure 单一登录。
