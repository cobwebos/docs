...
ms.openlocfilehash: a34a4c38e3cb34984bc0e959cdd6f9ef3009464a
...
* 在 Azure AD Connect 向导中配置[目录扩展](active-directory-aadconnectsync-feature-directory-extensions.md)时，现在可选择“Teletex 字符串”类型的 AD 属性。
...
* 如果进行安装，并在安装向导结束时取消选择“启动同步过程”， 则重新运行安装向导将不启用计划程序。
...
* 使用安装向导中的 MFA 和 PIM 来提供全局管理员支持。
  * 如果使用 MFA，则需同意代理也允许指向 https://secure.aadcdn.microsoftonline-p.com 的流量。
  * 需将 https://secure.aadcdn.microsoftonline-p.com 添加到受信任的站点列表，这样 MFA 才能正常工作。
* 允许在初始安装之后更改用户的登录方法。
...
* 根据预览版客户的反馈，已暂时删除“用户写回”[](active-directory-aadconnect-feature-preview.md#user-writeback)预览版功能。 今后在解决所提供的反馈意见后，我们将重新添加此功能。
...
* 可[配置 ADFS](connect/active-directory-aadconnect-get-started-custom.md#configuring-federation-with-ad-fs)
* 可[从 DirSync 升级](connect/active-directory-aadconnect-dirsync-upgrade-get-started.md)
