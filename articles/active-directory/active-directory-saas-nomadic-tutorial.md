...
本部分中的步骤在 Azure 管理门户中启用 Azure AD 单一登录并在 Nomadic 应用程序中配置单一登录。
...
3. 在“Nomadic 域和 URL”部分中，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入 URL：`https://<company name>.nomadic.fm/signin`
    
    b. 在“标识符”文本框中，使用以下模式键入 URL：`https://<company name>.nomadic.fm/auth/saml2/sp`

    > [!NOTE] 
    > 请注意，这些不是实际值。 必须使用实际登录 URL 和标识符更新这些值。 若要获取这些值，请联系 [Nomadic 支持团队](mailto:help@nomadic.fm)。
...
### <a name="creating-a-nomadic-test-user"></a>创建 Nomadic 测试用户
...
本部分需授予 Britta Simon 访问 Nomadic 的权限，使之能够使用 Azure 单一登录。
