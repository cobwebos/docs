...
若要配置 Datahug 与 Azure AD 的集成，需要从库中将 Datahug 添加到托管 SaaS 应用列表。
...
4. 在搜索框中，键入 **Datahug**。
    ...
    ![创建 Azure AD 测试用户](./media/active-directory-saas-datahug-tutorial/tutorial_datahug_search.png)

5. 在结果窗格中，选择“Datahug”，然后单击“添加”按钮添加该应用程序。

    ![创建 Azure AD 测试用户](./media/active-directory-saas-datahug-tutorial/tutorial_datahug_addfromgallery.png)
...
3. **[创建 Datahug 测试用户](#creating-a-datahug-test-user)** - 在 Datahug 中创建 Britta Simon 的对应用户，将其链接到该用户的 Azure AD 表示形式。
4. **[分配 Azure AD 测试用户](#assigning-the-azure-ad-test-user)** - 让 Britta Simon 能够使用 Azure AD 单一登录。
5. **[测试单一登录](#testing-single-sign-on)** - 验证配置是否正常工作。

### <a name="configuring-azure-ad-single-sign-on"></a>配置 Azure AD 单一登录

在本部分中，将在 Azure 门户中启用 Azure AD 单一登录并在 Datahug 应用程序中配置单一登录。
...
3. 在“Datahug 域和 URL”部分中，如果要在“IDP 启动的模式”下配置应用程序，请执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“标识符”文本框中，使用以下模式键入 URL：`https://apps.datahug.com/identity/<uniqueID>`
...
4. 选中“显示高级 URL 设置”。 如果要在“SP”发起的模式下配置应用程序：
    ...
    > [!NOTE] 
    > 这些不是实际值。 使用实际标识符和回复 URL 更新这些值。 此处我们建议在“标识符”和“回复 URL”中使用字符串的唯一值。 请联系 [Datahug 客户端支持团队](http://datahug.com/about/contact-us/)获取这些值。 
...
9. 若要在 **Datahug** 端配置单一登录，需要将下载的“元数据 XML”、“SAML 实体 ID”和“SAML 单一登录服务 URL”发送到 [Datahug 支持](http://datahug.com/about/contact-us/)。 他们会将此应用程序设置为使用在两端正确设置的 SAML SSO 连接。
...
1. 在 **Azure 门户**的左侧导航窗格中，单击“Azure Active Directory”图标。
    ...
    ![创建 Azure AD 测试用户](./media/active-directory-saas-datahug-tutorial/create_aaduser_01.png) 

2. 若要显示用户列表，请转到“用户和组”，单击“所有用户”。
    
    ![创建 Azure AD 测试用户](./media/active-directory-saas-datahug-tutorial/create_aaduser_02.png) 

3. 若要打开“用户”对话框，请在对话框顶部单击“添加”。
 
    ![创建 Azure AD 测试用户](./media/active-directory-saas-datahug-tutorial/create_aaduser_03.png) 

4. 在“用户”对话框页上，执行以下步骤：
 
    ![创建 Azure AD 测试用户](./media/active-directory-saas-datahug-tutorial/create_aaduser_04.png) 
...
**若要将 Britta Simon 分配到 Datahug，请执行以下步骤：**
...
当在访问面板中单击 Datahug 磁贴时，应当会自动登录到 Datahug 应用程序。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://msdn.microsoft.com/library/dn308586)（访问面板简介）。 
