...
## <a name="adding-qprism-from-the-gallery"></a>从库中添加 QPrism
若要配置 QPrism 与 Azure AD 的集成，需要从库中将 QPrism 添加到托管 SaaS 应用列表。

**若要从库中添加 QPrism，请执行以下步骤：**
...
在本部分中，基于一个名为“Britta Simon”的测试用户使用 QPrism 配置和测试 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 QPrism 用户。 换句话说，需要在 Azure AD 用户与 QPrism 中的相关用户之间建立链接关系。

通过将 Azure AD 中“用户名”的值指定为 QPrism 中“用户名”的值来建立此链接关系。
...
1. 在 Azure 门户中的 **QPrism** 应用程序集成页上，单击“单一登录”。
...
3. 在“QPrism 域和 URL”部分中，执行以下步骤：
    ...
    > [!NOTE] 
    > 这些不是实际值。 使用实际标识符和登录 URL 更新这些值。 请联系 [QPrism 客户端支持团队](mailto:qsupport-ce@quatrro.com)获取这些值。 

4. 若要生成**元数据** URL，请执行以下步骤：

    a. 单击“应用注册”。
    
    ![配置单一登录 appreg](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_appregistrations.png)
   
    b. 单击“终结点”以打开“终结点”对话框。  
    
    ![配置单一登录 endpointcon](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_endpointicon.png)

    c. 单击复制按钮以复制**联合元数据文档** URL 并将其粘贴到记事本。
    
    ![配置单一登录终结点](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_endpoint.png)
     
    d.单击“下一步”。 现在，转到 **QPrism** 的属性页，使用“复制”按钮复制**应用程序 ID** 并将其粘贴到记事本中。
 
    ![配置单一登录 appid](./media/active-directory-saas-qprism-tutorial/tutorial_qprism_appid.png)
...
6. 若要在“QPrism”端配置单一登录，需要将**元数据 URL** 发送给 [QPrism 支持团队](mailto:qsupport-ce@quatrro.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
**若要将 Britta Simon 分配到 QPrism，请执行以下步骤：**
...
单击访问面板中的 QPrism 磁贴时，应自动登录到 QPrism 应用程序。
