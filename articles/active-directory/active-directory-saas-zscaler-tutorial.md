...
description: "了解如何使用 Zscaler 与 Azure Active Directory 以启用单一登录、自动化预配及更多内容！"
...
本教程旨在介绍 Azure 与 Zscaler 的集成。 在本教程中概述的方案假定您已具有以下各项：
...
### <a name="to-enable-the-application-integration-for-zscaler-perform-the-following-steps"></a>若要启用 Zscaler 的应用程序集成，请执行以下步骤：
...
此部分旨在概述如何使用基于 SAML 协议的联合身份验证，使用户通过其 Azure AD 中的帐户对 Zscaler 进行身份验证。  
在此过程中，需要将证书上传到 Zscaler。
...
3. 在“配置应用 URL”页的“Zscaler 登录 URL”文本框中，键入从 Zscaler 获取的登录 URL，然后单击“下一步”： 
    ...
    > [!NOTE]
    > 如果不清楚登录 URL 是什么，请与 Zscaler 支持团队联系。
    > 
    > 
...
9. 在“配置 SAML 单一登录参数”对话框页上，执行以下步骤，然后单击“完成”：
    ...
    3. 在“上传 SSL 公共证书”字段中，上传已从 Azure 经典门户下载的证书。
...
5. 在“代理服务器”部分中，执行以下步骤：
    ...
    2. 在“地址”文本框中，键入 **gateway.zscalertwo.net**。
...
为了使 Azure AD 用户能够登录到 Zscaler，必须将其预配到 Zscaler。  
对于 Zscaler，需要手动执行预配。
...
### <a name="to-assign-users-to-zscaler-perform-the-following-steps"></a>若要将用户分配到 Zscaler，请执行以下步骤：
