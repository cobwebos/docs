...
# Azure Active Directory Identity Protection 和 Microsoft Graph 入门
...
1. 使用客户端机密添加应用程序。 
2. 使用此密钥和其他一些信息片段在 Microsoft Graph 中进行身份验证，然后在其中接收身份验证令牌。 
...
7. 在“向我们说明你的应用程序”对话框中，执行以下步骤：
   
    ![创建应用程序](./media/active-directory-identityprotection-graph-getting-started/tutorial_general_05.png)
   
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“名称”文本框中，键入应用程序的名称（例如“AADIP 风险事件 API 应用程序”）。
   
    b.保留“数据库类型”设置，即设置为“共享”。 在“类型”中，选择“Web 应用程序和/或 Web API”。
...
8. 在“应用属性”对话框中，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，键入 `http://localhost`。
   
    b.保留“数据库类型”设置，即设置为“共享”。 在“应用 ID URI”文本框中，键入 `http://localhost`。
...
3. 上“针对其他应用程序的权限”对话框中执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 选择“Microsoft Graph”。
   
    b.保留“数据库类型”设置，即设置为“共享”。 单击“完成”。
4. 单击“应用程序权限: 0”，然后选择“读取所有标识风险事件信息”。
...
3. 在“密钥”部分中，复制新建密钥的值，然后将其粘贴到一个安全的位置。
   ...
   > [!NOTE]
   > 如果丢失此密钥，必须返回到此部分并创建新密钥。 将此密钥保密：持有该密钥的任何人都可以访问你的数据。
   > 
   > 
...
## 在 Microsoft Graph 中进行身份验证并查询标识风险事件 API
现在应已获得：
...
## 后续步骤
恭喜，你已向 Microsoft Graph 发出了第一个调用！  
