...
我们很高兴地宣布发布 Azure AD 域服务安全审核以进行公共预览。 安全审核通过使用 Azure AD 域服务将安全审核事件流式传输到目标资源（包括 Azure 存储、Azure 日志分析工作区和 Azure 事件中心），帮助您深入了解身份验证服务门户。
...
现在，您可以在 Azure AD Connect 中成功地将用户的 UPN 后缀从一个联合域更改为另一个联合域。 此修复意味着在同步周期期间不应再遇到联合域域更改错误错误消息，或收到通知电子邮件，指出"无法在 Azure 活动目录中更新此对象，因为属性 |联合用户.用户主体名称*，无效。 更新本地目录服务中的值"。
...
有关如何部署 Azure 活动目录 （Azure AD） 的新分步指南，包括自助服务密码重置 （SSPR）、单一登录 （SSO）、条件访问 （CA）、应用代理、用户预配、活动目录联合服务 （ADFS） 到直通身份验证 （PTA） 和 ADFS 到密码哈希同步 （PHS）。
...
此 B2B 协作功能适用于所有 Azure 活动目录客户，可与 Azure AD 高级功能（如条件访问和标识保护）结合使用，以便更精细地控制外部业务用户签署时和方式并获取访问权限。
...
作为客户反馈的一部分，使非管理员角色能够访问 Azure AD 活动日志，我们启用了处于"报表读取器"角色的用户访问 Azure 门户中的登录和审核活动以及使用 Microsoft 图形的能力Api。 
