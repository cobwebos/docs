...
1. 如果未通过单击"客户端应用"列>列来显示客户端应用列，请添加该列。
1. 按客户端应用筛选>选中显示的所有其他客户端选项，然后单击"应用"。
1. 按状态筛选>成功，然后单击"应用"。 
