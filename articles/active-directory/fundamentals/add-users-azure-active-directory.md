...
1. 选择 "**用户**"，然后选择 "**新建用户**"。
    ...
    ![通过用户（Azure AD 中的所有用户）添加用户](media/add-users-azure-active-directory/add-user-in-users-all-users.png)
...
1. 复制 "**密码**" 框中提供的自动生成的密码。 你需要为用户提供此密码以首次登录。
...
1. 搜索并选择要从 Azure AD 租户中删除的用户。 例如，Mary Parker。

1. 选择“删除用户”。

    ![用户 - 突出显示删除用户的所有用户页面](media/add-users-azure-active-directory/delete-user-all-users-blade.png)

该用户已删除并不再显示在“用户 - 所有用户”页上。 可在接下来的 30 天内于“已删除用户”页查看该用户，在此期间可将其还原。 有关还原用户的详细信息，请参阅[使用 Azure Active Directory 还原或删除最近删除的用户](active-directory-users-restore.md)。

删除用户时，用户使用的任何许可证都将可供其他用户使用。

>[!Note]
>你必须使用 Windows Server Active Directory 来更新证书颁发机构为 Windows Server Active Directory 的用户的标识、联系信息或作业信息。 完成更新后，必须等待下一个同步周期完成才能看到更改。
