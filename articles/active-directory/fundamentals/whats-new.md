...
若要访问此功能和详细信息，请参阅 [Azure Active Directory - 数据位于何处](http://aka.ms/AADDataMap)。
...
以下应用包含在[批准的客户端应用](https://docs.microsoft.com/azure/active-directory/conditional-access/technical-reference.md#approved-client-app-requirement)列表中：
...
从 2018 年 10 月 10 日起，Azure AD 不再允许对应用使用以前用过的身份验证代码。 此项安全变更有助于使 Azure AD 与 OAuth 规范保持一致，将在 v1 和 v2 终结点上强制实施。
...
>[!Note]
>为了尽量帮助将应用中断的可能性减到最小，我们将依赖于此模式且每天登录次数超过 10 次的应用列入了例外情况。
