...
数据层包含多个可提供读写功能的前端服务。 下图显示了单目录分区的组件在地理位置分散的数据中心内的传送方式。
...
#### <a name="secondary-replicas"></a>辅助副本

所有目录*读取*都是从位于物理位置位于不同地理位置的数据中心的*辅助副本*提供服务。 由于数据是以异步方式复制的，因此存在许多次要副本。 目录读取（例如身份验证请求）通过接近客户的数据中心提供服务。 次要副本负责提供读取可伸缩性。
...
可用性（或运行时间）是指系统无中断运行的能力。 Azure AD 高可用性的关键在于，服务可以快速将流量转移到多个地理位置分散的数据中心。 每个数据中心都是独立的，这将启用取消关联失败模式。 通过这种高可用性设计，Azure AD 维护活动无需停机。
...
#### <a name="data-durability"></a>数据持续性

在确认写入之前，持久至少将写入两个数据中心。 首先将写入操作提交到主副本，然后立即将写入操作复制到至少一个数据中心。 此写入操作可确保托管主副本的数据中心发生灾难性损失时不会导致数据丢失。
...
Azure AD 跨数据中心运行，具有以下特征：

* 身份验证、Graph 其他 AD 服务驻留在网关服务的后面。 网关管理这些服务的负载均衡。 如果使用事务运行状况探测检测到任何不正常的服务器，网关会自动故障转移。 根据这些运行状况探测，网关将流量动态路由到正常的数据中心。
* 对于*读取*，目录具有辅助副本以及在多个数据中心内运行的主动-主动配置中的相应前端服务。 当整个数据中心发生故障时，流量会自动路由到另一个数据中心。
 \* 对于*写入操作*，目录将通过计划（新的主副本同步到旧的主副本）或紧急故障转移过程跨数据中心故障转移主（主）副本。 通过将任何提交复制到至少两个数据中心来实现数据持久性。
