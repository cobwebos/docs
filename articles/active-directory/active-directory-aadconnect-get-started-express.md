...
“快速设置”是默认选项，用于最常见的部署方案。如果使用快速设置，Azure AD Connect 将为单林拓扑部署同步。[密码同步](active-directory-aadconnectsync-implement-password-synchronization.md)已启用，可让用户使用其本地密码来登录云。[自动升级](/documentation/articles/active-directory-aadconnect-feature-automatic-upgrade)已启用，可以简化维护工作。如果使用“快速设置”，在完成安装之后，将自动开始同步（不过你可以选择不执行此步骤）。
...
如果你收到错误消息并出现连接问题，请参阅 [Troubleshoot connectivity problems（排查连接问题）](active-directory-aadconnect-troubleshoot-connectivity.md)。
...
8. 在“已准备好配置”屏幕上，单击“安装”。
	- 在“已准备好配置”页上，可以取消选中“配置完成后立即开始同步过程”复选框。如果想要进行其他设置（例如[筛选](active-directory-aadconnectsync-configure-filtering.md)），应取消选中此复选框。如果取消选择此选项，向导将配置同步，但会保持禁用计划程序。在重新运行安装向导以手动启用计划程序之前，计划程序将不运行。
