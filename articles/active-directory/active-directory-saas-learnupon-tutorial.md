...
本部分的目的是基于名为“Britta Simon”的测试用户展示如何配置并测试 LearnUpon 的 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 LearnUpon 用户。 换句话说，需要在 Azure AD 用户与 LearnUpon 中的相关用户之间建立链接关系。  
...
本部分的目的是在 Azure 经典门户中启用 Azure AD 单一登录并在 LearnUpon 应用程序中配置单一登录。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
    ...
    a. 在“回复 URL”文本框中，使用以下模式键入断言使用者服务 URL：`https://\<companyname\>.learnupon.com/saml/consumer`

    b. 单击“下一步”。 


1. 在“在 LearnUpon 处配置单一登录”页上，执行以下步骤：
   
    ![配置单一登录](./media/active-directory-saas-learnupon-tutorial/tutorial_learnupon_05.png) 
   
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 单击“下载证书”，然后将文件保存在计算机上。 将需要使用此证书和元数据 URL（实体 ID、SSO 登录 URL 和注销 URL）在 LearnUpon 端设置 SSO。
   
    b. 单击“下一步”。
2. 打开另一个浏览器实例并使用管理员帐户登录到 LearnUpon。 
3. 单击“设置”选项卡。
   
    ![配置单一登录](./media/active-directory-saas-learnupon-tutorial/tutorial_learnupon_06.png) 
4. 单击“单一登录 - SAML”，然后单击“常规设置”以配置 SAML 设置。
...
5. 在“常规设置”部分中，执行以下步骤：
    ...
    f. 在“标识符提供程序位置”文本框中，键入相应的值以指明当用户在 Azure 经典门户登录屏幕上单击你已上载的图标时要将用户发送到何处。
   
    g.在 Azure 经典门户中，复制“单一注销服务 URL”值，然后将其复制到“注销 URL”文本框中。
   
    h. 单击“管理指纹”，然后上载所下载的证书的指纹。 
6. 单击“用户设置”，然后执行以下步骤：
   
    ![配置单一登录](./media/active-directory-saas-learnupon-tutorial/tutorial_learnupon_11.png) 
   
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“名字标识符格式”文本框中，键入相应的值以指明用户名字在 SAML 断言中所处的位置，例如：**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/    givenname**。
   
    b.保留“数据库类型”设置，即设置为“共享”。 在“姓氏标识符格式”文本框中，键入相应的值以指明用户姓氏在 SAML 断言中所处的位置，例如：**http://schemas.xmlsoap.org/ws/2005/05/identity/claims/    surname**。
...
本部分的目的是通过向 Britta Simon 授予对 LearnUpon 的访问权限，使她能够使用 Azure 单一登录。
