...
将身份验证外包给 Azure AD 的任何应用程序都必须在目录中进行注册。 此步骤涉及通知 Azure AD 应用程序，包括应用程序所在的 URL、身份验证后用于发送答复的 URL、用于标识应用程序的 URI，等等。 该信息是必需的，有以下几个重要原因：
...
与开发单租户应用程序相比，当开发多租户应用程序时，会出现一些额外的注意事项。 例如，如果你要使应用程序可供多个目录中的用户使用，则你需要一种机制来确定他们所在的租户。 单租户应用程序只需要在其自己的目录中查找用户，而多租户应用程序需要从 Azure AD 中的所有目录来识别特定用户。 为此，Azure AD 提供了一个任何多租户应用程序都可以在其中对登录请求进行定向的通用身份验证终结点，而不是提供特定于租户的终结点。 对于 Azure AD 中的所有目录，此终结点为 `https://login.microsoftonline.com/common` ，而特定于租户的终结点可能为 `https://login.microsoftonline.com/contoso.onmicrosoft.com` 。 开发应用程序时需要考虑公共终结点，因为在登录、注销和令牌验证期间需要必要的逻辑来处理多个租户。
...
本文档中列出的每个方案都包括一个小节，用以介绍其预配要求。 有关在 Azure AD 中预配应用程序以及单租户应用程序与多租户应用程序之间区别的更深入信息，请参阅[将应用程序与 Azure Active Directory 集成](../develop/single-and-multi-tenant-apps.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json)了解详细信息。 继续阅读以了解 Azure AD 中的常见应用程序方案。
