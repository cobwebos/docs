---
title: 使用 OAuth2.0 代表流的服务到服务身份验证 |微软文档
...
在使用 [OAuth 2.0 授权代码授权流](v1-protocols-oauth-code.md)的应用程序上对用户进行身份验证后，启动 OBO 流。 此时，应用程序向包含用户声明和同意访问 API A 的中间层 Web API （API A） 发送访问令牌 （令牌 A）。接下来，API A 向下游 Web API （API B） 发出经过身份验证的请求。

这些步骤构成"代表流"：![显示 OAuth2.0 代表流中的步骤](./media/v1-oauth2-on-behalf-of-flow/active-directory-protocols-oauth-on-behalf-of-flow.png)
...
| 参数 | 描述 |
| --- | --- |
| token_type |指示令牌类型值。 Azure AD 支持的唯一类型是**承载**。 有关持有者令牌的详细信息，请参阅 [OAuth2.0 授权框架：持有者令牌用法 (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.txt)。 |
| scope |令牌中授予的访问权限的范围。 |
...
| 参数 | 描述 |
| --- | --- |
| token_type |指示令牌类型值。 Azure AD 支持的唯一类型是**承载**。 有关持有者令牌的详细信息，请参阅 [OAuth 2.0 授权框架：持有者令牌用法 (RFC 6750)](https://www.rfc-editor.org/rfc/rfc6750.txt)。 |
| scope |令牌中授予的访问权限的范围。 |
| expires_in |访问令牌有效的时间长度（以秒为单位）。 |
| expires_on |访问令牌的过期时间。 该日期表示为自 1970-01-01T0:0:0Z UTC 至过期时间的秒数。 此值用于确定缓存令牌的生存期。 |
| resource |接收服务（受保护资源）的应用 ID URI。 |
