...
1. Web 应用程序需要获取访问令牌，以便通过 Web API 进行身份验证并检索所需的资源。 它向 Azure AD 的令牌终结点发出请求，提供凭据、应用程序 ID 和 Web API 的应用程序 ID URI。
...
1. 使用 Azure AD 颁发的授权代码，Web 应用程序向 Azure AD 的令牌终结点发送请求，其中包括授权代码、有关客户端应用程序的详细信息（应用程序 ID 和重定向 URI）以及所需的资源（应用程序 ID）用于 Web API 的 URI）。
...
1. Web 应用程序需要授权代码才能获取访问令牌，因此它通过浏览器向 Azure AD 的授权终结点发出请求，在成功身份验证后为 Web 应用程序提供应用程序 ID 并重定向 URI。 用户登录到 Azure AD。
...
* 单租户 - 对于应用程序标识和委托用户标识这两种情况，Web 应用和 Web API 都必须在 Azure AD 的同一个目录中进行注册。 可以将 Web API 配置为公开一组权限，这些权限用于限制 Web 应用程序对其资源的访问。 如果使用的是委托用户标识类型，则 Web 应用需要从 Azure 门户的“对其他应用程序的权限”**** 下拉菜单中选择所需的权限。 如果使用的是应用程序标识类型，则不需要此步骤。
