...
Web 应用是在 Web 浏览器到 Web 应用方案中对用户进行身份验证的应用程序。 在此方案中，web 应用程序指示用户的浏览器对其进行签名，以便 Azure AD。 Azure AD 通过用户的浏览器返回一个登录响应，该响应在一个安全令牌中包含了关于用户的声明。 此方案支持使用 OpenID Connect、SAML 2.0 和 WS 联合身份验证协议进行登录。
...
1. 用户在登录页面上进行登录。
1. 如果身份验证成功，Azure AD 会创建一个身份验证令牌并将登录响应返回到在 Azure 门户中配置的应用程序的回复 URL。 对于生产应用程序，此回复 URL 应当采用 HTTPS 格式。 返回的令牌包括应用程序对该令牌进行验证所需的关于用户和 Azure AD 的声明。
