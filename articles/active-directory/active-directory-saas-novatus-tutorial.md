...
* 可以让用户使用其 Azure AD 帐户自动登录到 Novatus 单一登录 (SSO)
...
## <a name="add-novatus-from-the-gallery"></a>从库中添加 Novatus
...
本部分旨在说明如何基于名为“Britta Simon”的测试用户配置和测试 Novatus 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 Novatus 用户。 换句话说，需要建立 Azure AD 用户与 Novatus 中相关用户之间的关联关系。
...
本部分旨在介绍如何在 Azure 经典门户中启用 Azure AD SSO 并在 Novatus 应用程序中配置 SSO。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
  ...
  * 在“登录 URL”文本框中，使用以下模式键入用户用于登录 Novatus 应用程序的 URL：**“https://sso.novatuscontracts.com/companyname”**。 引用泛型名称时，需将 **companyname** 替换为实际的名称。
4. 在“在 Novatus 中配置单一登录”页中，执行以下步骤：
   
    ![配置单一登录](./media/active-directory-saas-novatus-tutorial/tutorial_novatus_05.png)   
  1. 单击“下载证书”，然后将文件保存在计算机上。
  2. 单击“资源组名称” 的 Azure 数据工厂。
5. 若要为应用程序配置 SSO，请通过 jvinci@novatusinc.com 联系 Novatus 支持团队。 将下载的证书文件附加到邮件中，与 Novatus 团队共享元数据 URL（实体 ID、SSO 登录 URL 和注销 URL），以便在团队这边设置 SSO。
...
### <a name="create-a-novatus-test-user"></a>创建 Novatus 测试用户
...
本部分旨在通过授予 Britta Simon 访问 Novatus 的权限，允许她使用 Azure SSO。
