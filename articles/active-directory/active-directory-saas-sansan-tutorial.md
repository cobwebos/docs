---
title: "教程：Azure Active Directory 与 SanSan 集成 | Microsoft 文档"
...
在本教程中，了解如何将 SanSan 与 Azure Active Directory (Azure AD) 集成。

将 SanSan 与 Azure AD 集成提供以下优势：

* 可在 Azure AD 中控制谁有权访问 SanSan
* 可以让用户使用其 Azure AD 帐户自动登录到 SanSan（单一登录）
* 可以在一个中心位置（即 Azure 经典门户）管理帐户

如果要了解有关 SaaS 应用与 Azure AD 集成的更多详细信息，请参阅 [Azure Active Directory 的应用程序访问与单一登录是什么](active-directory-appssoaccess-whatis.md)。

## <a name="prerequisites"></a>先决条件
若要配置 Azure AD 与 SanSan 的集成，需备齐以下项目：
...
在本教程中，将在测试环境中测试 Microsoft Azure AD 单一登录。 本教程中概述的方案包括两个主要构建基块：
...
6. 在搜索框中，键入“SanSan”。
    ...
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/tutorial_sansan_01.png)
7. 在结果窗格中，选择“SanSan”，然后单击“完成”以添加该应用程序。
   
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/tutorial_sansan_06.png)

## <a name="configuring-and-testing-microsoft-azure-ad-single-sign-on"></a>配置和测试 Microsoft Azure AD 单一登录
在本部分中，将基于名为“Britta Simon”的测试用户配置和测试 SanSan 的 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 SanSan 用户。 换句话说，需要在 Azure AD 用户与 SanSan 中相关用户之间建立链接关系。
可以通过将 Azure AD 中“用户名”的值分配为 SanSan 中“用户名”的值来建立此链接关系。

若要配置和测试 SanSan 的 Microsoft Azure AD 单一登录，需要完成以下构建基块：
...
3. **[创建 SanSan 测试用户](#creating-an-sansan-test-user)** - 在 SanSan 中创建 Britta Simon 的对应用户，将其链接到她的 Azure AD 表示形式。
...
在本部分中，将在经典门户中启用 Microsoft Azure AD 单一登录并在 SanSan 应用程序中配置单一登录。

**若要配置 SanSan 的 Microsoft Azure AD 单一登录，请执行以下步骤：**

1. 在 Azure 经典门户中的“SanSan”应用程序集成页上，单击“配置单一登录”，打开“配置单一登录”对话框。
   
    ![配置单一登录](./media/active-directory-saas-sansan-tutorial/tutorial_general_05.png) 
2. 在“你希望用户如何登录 SanSan”页上，选择“Microsoft Azure AD 单一登录”，然后单击“下一步”。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入 URL：
   ...
   | 移动浏览器设置 |`https://ap.sansan.com/s/saml2/<company name>/acs` |
    ...
    b.保留“数据库类型”设置，即设置为“共享”。 在“标识符”文本框中，使用以下模式键入 URL： 

    | 环境             | 代码 |
    | :--                     | :-- |
    | 电脑 Web                  | `https://ap.sansan.com/v/saml2/<company name>`|
    | 本机移动应用       | `https://internal.api.sansan.com/saml2/<company name>` |
    | 移动浏览器设置 | `https://ap.sansan.com/s/saml2/<company name>` |


    c. 单击“下一步”。

1. 在“配置 SanSan 的单一登录”页上，执行以下步骤：
...
2. 若要为应用程序配置 SSO，请联系 SanSan 支持团队，他们将协助配置 SSO。 请向他们提供以下信息：
...
1. 在 **Azure 经典门户**中，在左侧导航窗格上，单击“Active Directory”。
    ...
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/create_aaduser_09.png) 
2. 在“目录”列表中，选择要启用目录集成的目录。
3. 若要显示用户列表，请在顶部菜单中，单击“用户”。
   
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/create_aaduser_03.png) 
4. 若要打开“添加用户”对话框，请在底部工具栏中单击“添加用户”。
   
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/create_aaduser_04.png) 
5. 在“告诉我们有关此用户的信息”对话框页上，执行以下步骤：
   
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/create_aaduser_05.png) 
...
6. 在“用户配置文件”对话框页上，执行以下步骤：
   ...
   ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/create_aaduser_06.png) 
...
7. 在“获取临时密码”对话框页上，单击“创建”。
    ...
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/create_aaduser_07.png) 
8. 在“获取临时密码”对话框页上，执行以下步骤：
   
    ![创建 Azure AD 测试用户](./media/active-directory-saas-sansan-tutorial/create_aaduser_08.png) 
   
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 写下“新密码”的值。
   
    b. 单击“完成”。   

### <a name="creating-an-sansan-test-user"></a>创建 SanSan 测试用户
在本部分中，将在 SanSan 中创建一个名为“Britta Simon”的用户。 在执行 SSO 前，SanSan 应用程序需要在应用程序中预配用户。 

> [!NOTE]
> 如果需要手动创建一个用户或一批用户，需要联系 SanSan 支持团队。
> 
> 
...
单击访问面板中的“SanSan”磁贴时，用户应自动登录到 SanSan 应用程序。
