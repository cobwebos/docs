...
|8|32|270|1190|
|16|64|245|1200*|
\* 此计算机的连接限制为 200。 对于所有其他计算机，我们使用了默认的 200 连接限制。
 
>[!NOTE]
>默认配置限制为 200（对于 2 核心、4 核心和 8 核心）。  在使用 16 个核心进行测试期间，连接限制已更改为 800。 在 4 核心、8 核心和 16 核心计算机之间，最大 TPS 没有多大区别。 主要区别体现在预期的延迟。  
...
连接器只会发送出站请求。 出站流量将依次发送到应用程序代理服务和发布的应用程序。 无需打开入站端口，因为一旦建立会话，流量就会双向流动。 无需在连接器之间设置负载均衡，或者通过防火墙配置入站访问。 
