...
建议不要在托管应用程序的同一服务器上安装连接器。 但是，你必须能够从安装连接器的服务器访问应用程序。
...
可将连接器安装在网络中允许连接器向应用程序代理服务和后端应用程序发送请求的任意位置。 如果将它们安装在公司网络中、外围安全区域 (DMZ) 内、甚至是云中运行的虚拟机中，它们都可正常工作。 重要的是，运行连接器的计算机还有权访问应用。
...
* 符合 FIPS 规范的计算机可能需要更改其配置才能允许连接器服务、连接器更新程序服务及其安装程序生成和存储证书。
