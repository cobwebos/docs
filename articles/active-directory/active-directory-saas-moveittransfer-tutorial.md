...
* 可以让用户使用其 Azure AD 帐户自动登录到 MOVEit Transfer（单一登录）
...
本部分的目的是基于名为“Britta Simon”的测试用户展示如何配置并测试 MOVEit 的 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 MOVEit 用户。 换句话说，需要在 Azure AD 用户与 MOVEit 中的相关用户之间建立链接关系。

可以通过将 Azure AD 中“用户名”的值分配为 MOVEit Transfer 中“用户名”的值来建立此链接关系。

若要配置并测试 MOVEit Transfer 的 Azure AD 单一登录，需要完成以下构建基块：
...
在本部分中，将在经典门户中启用 Azure AD 单一登录并在 MOVEit Transfer 应用程序中配置单一登录。
...
3. 在“配置应用设置”对话框页上，执行以下步骤，然后单击“下一步”：
    ...
    a. 在“登录 URL”文本框中，键入包含你自己的域的登录 URL。
   
    b. 在“标识符”文本框中，键入一个实体 ID URL。
   
    c. 在“回复 URL”文本框中，键入一个已启用的断言使用者界面 URL。
...
12. 在“编辑联合身份提供程序用户设置”页上，执行以下操作，然后单击“保存”。
    
    a. 对于“登录名”，选择“SAML NameID”。
    
    b. 对于“全名”，选择“其他”，并在“属性名称”文本框中输入以下值：http://schemas.microsoft.com/identity/claims/displayname。
    
    c. 对于“电子邮件”，选择“其他”，并在“属性名称”文本框中输入以下值：http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress。
...
### <a name="creating-a-moveit-transfer-test-user"></a>创建 MOVEit Transfer 测试用户
...
本部分的目的是通过向 Britta Simon 授予对 MOVEit Transfer 的访问权限，使她能够使用 Azure 单一登录。
