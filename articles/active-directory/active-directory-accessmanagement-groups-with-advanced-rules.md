...
# 使用属性创建高级规则
Azure 管理门户允许你灵活地设置更高级的规则来启用动态组成员身份。

**若要创建高级规则**，请在 Azure 管理门户的该组的“配置”选项卡下，选中“高级规则”单选按钮，然后在提供的文本框中键入你的高级规则。你可以使用以下信息创建高级规则。
...
高级规则正文的总长度不能超过 255 个字符。
> [AZURE.NOTE]
> 字符串和正则表达式运算不区分大小写。你还可以执行 Null 检查，使用 $null 作为常量，例如 user.department -eq $null。应该使用 ` 字符来转义包含引号 " 的字符串，例如，user.department -eq "Sa`"les"。
...
2. 使用以下语法键入规则：Direct Reports for *Direct Reports for {UserID\_of\_manager}*
