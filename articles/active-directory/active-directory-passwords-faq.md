...
## 密码重置注册
 ...
 > **答：**可以。你可以使用 DirSync 或 PowerShell 实现此操作，也可以通过 [Azure 管理门户](https://manage.windowsazure.com)或 Office 管理门户实现。可以通过博客文章“针对 Azure AD MFA 和密码重置电话号码的改进隐私功能”并阅读“了解密码重置如何使用数据”了解有关此功能的更多信息。
 ...
 > **答：**如果用户已经定义了至少 N 项身份验证信息（其中 N 是你在 [Azure 管理门户](https://manage.windowsazure.com)中设置的“所需身份验证方法数量”），则此用户即会被视为已注册用户。若要了解更多信息，请参阅“自定义用户密码重置策略”。
