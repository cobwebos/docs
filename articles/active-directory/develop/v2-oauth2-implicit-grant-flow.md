...
| id_token |应用程序请求的 id_token。 可以使用 id_token 验证用户的标识，并以用户身份开始会话。 有关 id_token 及其内容的更多详细信息，请参阅 [v2.0 终结点令牌参考](v2-id-and-access-tokens.md)。 |
...
可以选择验证客户端代码中的 `id_token`，但是常见的做法是将 `id_token` 发送到后端服务器，并在那里执行验证。 验证 id_token 的签名后，就有几项声明需要验证。 有关详细信息，请参阅 [v2.0 令牌参考](v2-id-and-access-tokens.md)，其中包括[验证令牌](v2-id-and-access-tokens.md#validating-tokens)和[有关签名密钥滚动更新的重要信息](v2-id-and-access-tokens.md#validating-tokens)。 我们建议利用库来分析和验证令牌 - 对于大多数语言和平台至少有一个可用。
...
有关 id_token 中声明的详细信息，请参阅 [v2.0 终结点令牌参考](v2-id-and-access-tokens.md)。
...
有关访问令牌中提供的声明的详细信息，请参阅 [v2.0 终结点令牌参考](v2-id-and-access-tokens.md)。
