...
| 参数     | 条件     | 说明                                                                               |
|--------------:|--------------:|:-----------------------------------------------------------------------------------------:|
| `tenant` | 必选 | 要向其请求权限的目录租户。 可以采用 GUID 或友好名称格式提供或使用 `organizations` 以一般方式引用，如示例所示。 不要使用"通用"，因为个人帐户不能提供管理员同意，除非是在租户的上下文中。 为确保与管理租户的个人帐户保持最佳兼容性，请使用租户 ID。 |
...
- 了解[Azure AD 应用程序同意体验](application-consent-experience.md)
