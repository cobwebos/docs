...
- 编写 ASP.NET 或 ASP.NET Core 应用程序时，如果未告知框架你已用授权代码兑换令牌，则可能会发生此错误。 为此，需要调用 `context.HandleCodeRedemption()` 事件处理程序的 `AuthorizationCodeReceived` 方法。
...
已针对 .NET Desktop、.NET Core 和 Windows 通用平台应用启用 IWA 流。 在 .NET Core 上，必须提供 IWA 的用户名，因为 .NET Core 无法从操作系统获取用户名。
