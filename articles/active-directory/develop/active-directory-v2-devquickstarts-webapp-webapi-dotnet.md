---
title: "Azure AD v2.0 .NET Web 应用调用 API 入门 | Microsoft Docs"
...
通过 v2.0 终结点，可以快速地将身份验证添加 Web 应用和 Web API，同时支持个人 Microsoft 帐户以及工作或学校帐户。  此外，我们会构建一个借助 Microsoft OWIN 中间件使用 OpenID Connect 让用户登录的 MVC Web 应用。  该 Web 应用程序会获取受 OAuth 2.0 保护的 Web API 的 OAuth 2.0 访问令牌，用于创建、读取和删除给定用户的“待办事项列表”。
...
本教程的代码 [在 GitHub 上](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet)维护。  若要遵照该代码，可以[下载 .zip 格式的应用骨架](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet/archive/skeleton.zip)，或克隆该骨架：

```git clone --branch skeleton https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet.git```

此外，也可以[以 .zip 格式下载已完成的应用](https://github.com/AzureADQuickStarts/AppModelv2-WebApp-WebAPI-OpenIdConnect-DotNet/archive/complete.zip)，或克隆已完成的应用：
...
* 创建 **Password** 类型的**应用密码**，并复制其值以备后用
* 为应用添加 **Web** 平台。
...
* 在同一个文件中，实现 `ConfigureAuth(...)` 方法。  在 `OpenIDConnectAuthenticationOptions` 中提供的参数会充当应用程序与 Azure AD 通信时使用的坐标。
...
在 `AuthorizationCodeReceived` 通知中，我们想要使用[与 OpenID Connect 串联的 OAuth 2.0](active-directory-v2-protocols.md)，以兑换待办事项列表服务的访问令牌的 authorization_code。  MSAL 可以简化此过程：
...
* 现在，添加一个新方法，即 `OnAuthorizationCodeReceived` 事件处理程序。  此处理程序使用 MSAL 获取待办事项列表 API 的访问令牌，并将 MSAL 令牌缓存中的令牌存储起来以供稍后使用：
...
* 此处可再次使用 MSAL，从 MSAL 缓存检索 access_tokens。  首先，将 MSAL 的 `using` 语句添加到此文件。
