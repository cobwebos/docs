...
---
# <a name="integrate-azure-ad-into-an-android-app"></a>将 Azure AD 集成到 Android 应用程序中
...
* [适用于 Node.js 的 Microsoft Azure Active Directory 示例 REST API 服务](active-directory-devquickstarts-webapi-nodejs.md)

## <a name="step-2-register-your-web-api-with-your-microsoft-azure-ad-tenant"></a>步骤 2：向 Microsoft Azure AD 租户注册 Web API
**我正在执行什么操作？**
...
此处我们假设你要注册上面引用的 TODO REST API，但这同样适用于你希望 Azure Active Directory 保护的任何 Web API。
...
6. 对于登录 URL，请输入示例的基 URL，默认情况下为 `https://localhost:8080`。
7. 对于应用程序 ID URI，请输入 `https://<your_tenant_name>/TodoListService`，并将 `<your_tenant_name>` 替换为你的 Azure AD 租户的名称。  单击“确定”完成注册。
8. 位于 Azure 门户中的同时，请转到应用程序页面，找到应用程序 ID 值，并将其复制到某个位置，之后配置应用程序时将需要此值。
...
此处我们假设你要注册上面引用的示例应用程序，但这同样适用于你正在开发的任何应用。
...
6. 对于“重定向 URI”，请输入 `http://TodoListClient`。  单击“完成”。
7. 从应用程序页面，找到应用程序 ID 值，并将其复制到某个位置，之后配置应用程序时将需要此值。
8. 在“设置”页上，依次选择“所需的权限”和“添加”。  查找并选择 TodoListService，在“委派的权限”下添加“访问 TodoListService”权限，然后选择“完成”。
...
* 根据[“先决条件”部分中的步骤设置适用于 Android 的 Maven](https://github.com/MSOpenTech/azure-activedirectory-library-for-android/wiki/Setting-up-maven-environment-for-Android)
* 使用 SDK 19 设置模拟器
* 转到存储库克隆到的根文件夹
...
1. 添加对项目的引用，并将其指定为 Android 库。 如果不确定如何执行此操作，请[单击此处了解详细信息](http://developer.android.com/tools/projects/projects-eclipse.html)
...
4. 在主要活动中创建 AuthenticationContext 的实例。 有关此调用的详细信息不在本自述文件的范围内，但你可以通过查看 [Android 本机客户端示例](https://github.com/AzureADSamples/NativeClient-Android)来快速入门。 以下示例使用 SharedPreferences 作为默认缓存，其中颁发机构采用 `https://login.windows.net/yourtenant.onmicrosoft.com` 的形式：
...
1. **代理**：Microsoft Intune 的公司门户应用将提供代理组件。 如果在验证器中创建了一个用户帐户并且开发人员选择不跳过代理帐户，ADAL 将使用代理帐户。 开发人员可以使用以下操作跳过代理用户：
   
   `AuthenticationSettings.Instance.setSkipBroker(true);`
   
   开发人员需要注册特殊的 redirectUri 供代理使用。 RedirectUri 的格式为 `msauth://packagename/Base64UrlencodedSignature`。 你可以使用脚本“brokerRedirectPrint.ps1”获取应用的 redirecturi，也可以使用 API 调用 mContext.getBrokerRedirectUri。 签名与签名证书相关。
   ...
   应用清单应有权使用 AccountManager 帐户：http://developer.android.com/reference/android/accounts/AccountManager.html
...
授权 URL 需要 STS 实例和租户名称：https://login.windows.net/yourtenant.onmicrosoft.com
