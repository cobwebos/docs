...
| `availableToOtherTenants` | Boolean | 如果应用程序与其他租户共享，则为 true；否则为 false。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `signInAudience`。_ | |
...
| `publicClient` | Boolean | 指定此应用程序是否为公共客户端（例如在移动设备上运行的已安装应用程序）。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `allowPublicClient`。_ | |
