...
| 密钥  | 值类型 | 描述  | 示例值 |
|---------|---------|---------|---------|
| `accessTokenAcceptedVersion` | 可为 Null 的 Int32 | 指定资源所需的访问令牌版本。 这会更改独立于用于请求访问令牌的终结点或客户端生成的 JWT 的版本和格式。<br/><br/>使用的端点 v1.0 或 v2.0 由客户端选择，仅影响 id_tokens 的版本。 资源需要显式配置 `accesstokenAcceptedVersion` 以指示受支持的访问令牌格式。<br/><br/>`accesstokenAcceptedVersion` 的可能值为 1、2 或为 null。 如果值为 null，则默认为 1，这对应于 v1.0 终结点。 <br/><br/>如果 `signInAudience` @no__t 为-1，则该值必须为 `2`  | `2` |
| `addIns` | 集合 | 定义使用服务在特定上下文中调用应用时可以使用的自定义行为。 例如，可以呈现文件流的应用程序可以为其 "FileHandler" 功能设置 addIns 属性。 这会使服务（如 Office 365）在用户正在处理的文档的上下文中调用该应用程序。 | <code>{<br>&nbsp;&nbsp;&nbsp;"id":"968A844F-7A47-430C-9163-07AE7C31D407"<br>&nbsp;&nbsp;&nbsp;"type": "FileHandler",<br>&nbsp;&nbsp;&nbsp;"properties": [<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{"key": "version", "value": "2" }<br>&nbsp;&nbsp;&nbsp;]<br>}</code>|
...
| `availableToOtherTenants` | 布尔 | 如果与其他租户共享应用程序，则为 true;否则为 false。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `signInAudience`。_ | |
| `appId` | 字符串 | 指定由 Azure AD 分配给应用的应用唯一标识符。 | `"601790de-b632-4f57-9523-ee7cb6ceba95"` |
...
| `displayName` | 字符串 | 应用的显示名称。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `name`。_ | `"MyRegisteredApp"` |
| `errorUrl` | 字符串 | 不支持. | |
...
| `homepage` | 字符串 | 应用程序主页的 URL。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `signInUrl`。_ | `"https://MyRegisteredApp"` |
| `objectId` | 字符串 | 应用在目录中的唯一标识符。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `id`。_ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `id` | 字符串 | 应用在目录中的唯一标识符。 此 ID 不是用于在任何协议事务中标识应用的标识符。 用于引用目录查询中的对象。 | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `oauth2AllowImplicitFlow` | 布尔 | 指定此 Web 应用是否可以请求 OAuth2.0 隐式流访问令牌。 默认值为 false。 此标志用于基于浏览器的应用，例如 Javascript 单页应用。 若要了解详细信息，请在目录中输入 `OAuth 2.0 implicit grant flow`，并查看有关隐式流的主题。 | `false` |
| `oauth2AllowIdTokenImplicitFlow` | 布尔 | 指定此 Web 应用是否可以请求 OAuth2.0 隐式流 ID 令牌。 默认值为 false。 此标志用于基于浏览器的应用，例如 Javascript 单页应用。 | `false` |
...
| `oauth2RequiredPostResponse` | 布尔 | 指定在 OAuth 2.0 令牌请求过程中，Azure AD 是否允许与 GET 请求相反的 POST 请求。 默认值为 false，即指定只允许 GET 请求。 | `false` |
...
| `publicClient` | 布尔 | 指定此应用程序是否为公共客户端（例如，在移动设备上运行的已安装应用程序）。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `allowPublicClient`。_ | |
| `publisherDomain` | 字符串 | 已验证的应用程序的发布者域。 只读。 | https://www.contoso.com |
| `replyUrls` | 字符串数组 | 此多值属性保存 Azure AD 在返回令牌时接受用作目标的已注册 redirect_uri 值列表。 <br><br> _注意：这仅适用于应用注册（旧）体验。替换为[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验中的 `replyUrlsWithType`。_ | |
...
应用程序清单具有多个称为集合的属性;例如，approles、keycredentials、knownClientApplications、identifierUris、rediretUris、requiredResourceAccess 和 oauth2Permissions。 在任何应用程序的完整应用程序清单中，所有集合中的条目总数已限制在1200。 如果你已在应用程序清单中指定了100重定向 Uri，则仅保留1100个剩余项，以在构成清单的所有其他集合中使用。
...
尝试上传先前下载的清单时，可能会出现以下错误之一。 这很可能是因为清单编辑器现在支持较新版本的架构，该架构与你尝试上传的架构不匹配。
...
- "**无法更新 xxxxxx 应用程序。错误详细信息：找不到类型名称的值，并且没有预期类型可用。指定模型时，负载中的每个值都必须具有一个类型，该类型可以通过调用方显式指定，也可以从父值隐式推断。[]** "
