...
你可以配置通过 Azure 门户或以编程方式使用应用程序的属性[REST API](https://docs.microsoft.com/previous-versions/azure/ad/graph/api/entity-and-complex-type-reference#application-entity)或[PowerShell](https://docs.microsoft.com/powershell/module/azuread/?view=azureadps-2.0#applications)。 但是，在某些情况下，需要编辑应用清单来配置应用的属性。 这些方案包括：
...
| `availableToOtherTenants` | Boolean | 如果应用程序共享与其他租户; 则为 true否则为 false。 <br><br> _注意：这是仅在应用注册 （旧版） 体验中可用。替换为`signInAudience`中[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验。_ | |
...
| `displayName` | String | 应用的显示名称。 <br><br> _注意：这是仅在应用注册 （旧版） 体验中可用。替换为`name`中[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验。_ | `"MyRegisteredApp"` |
...
| `homepage` | String | 应用程序主页的 URL。 <br><br> _注意：这是仅在应用注册 （旧版） 体验中可用。替换为`signInUrl`中[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验。_ | `"https://MyRegisteredApp"` |
| `objectId` | String | 应用在目录中的唯一标识符。 <br><br> _注意：这是仅在应用注册 （旧版） 体验中可用。替换为`id`中[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验。_ | `"f7f9acfc-ae0c-4d6c-b489-0a81dc1652dd"` |
...
| `publicClient` | Boolean | 指定此应用程序是否为公共客户端 （例如移动设备上运行的已安装应用程序）。 <br><br> _注意：这是仅在应用注册 （旧版） 体验中可用。替换为`allowPublicClient`中[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验。_ | |
| `publisherDomain` | String | 应用程序验证的发行者域。 只读的。 | https://www.contoso.com |
| `replyUrls` | 字符串数组 | 此多值属性保存 Azure AD 在返回令牌时接受用作目标的已注册 redirect_uri 值列表。 <br><br> _注意：这是仅在应用注册 （旧版） 体验中可用。替换为`replyUrlsWithType`中[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)体验。_ | |
...
应用程序清单在 Azure AD 中表示的基础的应用程序模型的架构。 随着基础架构的发展，将更新清单编辑器以不时反映新的架构。 因此，可能会注意到应用程序清单中显示的新属性。 在极少数情况下，可能会注意到现有属性的语法或语义修改或您可能会发现不再支持以前存在的属性。 例如，你将看到中的新特性[应用注册](https://go.microsoft.com/fwlink/?linkid=2083908)的已知包含在应用注册 （旧版） 体验中不同的名称。
...
1. 如果您的工作流需要将清单保存在源存储库中供以后使用，我们建议将变基中看到与在存储库中的已保存的清单**应用注册**体验。
