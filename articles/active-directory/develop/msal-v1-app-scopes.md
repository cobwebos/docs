...
若要使用 Azure AD Graph API (https://graph.windows.net/) 通过 MSAL.NET Azure Active Directory 进行读取和写入，需要如下所示创建范围列表：
...
- 对于要求资源访问令牌接受 v1.0 令牌的 MSAL（v2.0 终结点）（与上面的情况相同），Azure AD 将提取最后一个斜杠前面的所有内容并将其用作资源标识符，以分析请求的范围中的所需受众。 因此，如果 https://database.windows.net 预期的受众为“https://database.windows.net/”，则你需要请求 https://database.windows.net//.default 范围。 另请参阅 GitHub 问题 [#747：将省略资源 URL 的尾部斜杠，因为该斜杠会导致 SQL 身份验证失败](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/issues/747)。
