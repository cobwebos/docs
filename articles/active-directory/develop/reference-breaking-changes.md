...
一项安全更改已在 7 月 26 日上线，它会更改颁发仅限应用的令牌的方式（通过客户端凭据授予）。 以前，允许应用程序获取令牌来调用任何其他应用，无论该应用程序是否在租户中存在或者是否向其许可了角色。  此行为已更新，因此对于设置为单租户的资源（有时称为 Web API）（默认值），客户端应用程序必须存在于资源租户中。  请注意，仍不需要客户端与 API 之间的现有许可，应用应该仍会执行其自身的授权检查，以确保 `roles` 声明存在并且包含 API 所需的值。
...
1. 当客户端应用程序通过`response_type=id_token`请求 id_token 时，还会请求对上述`response_type=token`创建的 Web API 的访问令牌 （ ） 。 因此，使用 v2.0 终结点时，`scope` 参数应与 `api://GUID/SCOPE` 类似。 在 v1.0 终结点上，`resource` 参数应为 Web API 应用 URI。
