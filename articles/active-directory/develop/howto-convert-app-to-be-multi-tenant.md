...
description: 演示如何构建可以在任何 Azure Active Directory 租户中的用户登录的多租户应用程序。
...
> [!NOTE]
> 本机客户端注册，以及[Microsoft 标识平台应用程序](./active-directory-appmodel-v2-overview.md)是默认情况下的多租户。 不需要采取任何措施将这些应用程序注册转换为多租户。
...
Web 应用程序和 web Api 接收和验证 Microsoft 标识平台的令牌。

> [!NOTE]
> 尽管本机客户端应用程序请求和接收来自 Microsoft 标识平台的令牌，它们执行此操作以将其发送到 Api，进行验证。 本机应用程序不会验证令牌，并且必须将它们视为不透明。

让我们来看它接收从 Microsoft 标识平台时应用程序如何验证令牌。 单租户应用程序通常采用类似于下面的终结点值：
...
这种同意体验受应用程序请求的权限的影响。 Microsoft 标识平台支持两种类型的仅限应用的和委派权限。
...
租户管理员可以禁用普通用户同意应用程序的能力。 如果禁用此功能，则始终需要管理员同意，才能在租户中使用应用程序。 如果你想要测试你的应用程序禁用最终用户同意，您可以找到配置开关中的[Azure 门户][ AZURE-portal]中**[用户设置](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/UserSettings/menuId/)** 部分下**企业应用程序**。
...
> [!NOTE]
> 某些应用程序想要提供一种体验，让普通用户能够一开始即表示同意，然后应用程序可让管理员参与操作并请求需要管理员同意的权限。 没有办法在今天; Azure AD 中执行此操作使用 1.0 版应用程序注册但是，使用 Microsoft 标识平台 (v2.0) 终结点允许应用程序以请求权限在运行时，而不是在注册时，可以实现此方案。 有关详细信息，请参阅[Microsoft 标识平台终结点][AAD-V2-Dev-Guide]。
...
#### <a name="multiple-tiers-in-multiple-tenants"></a>在多个租户中的多个层
...
如果它是由 Microsoft 以外的其他组织生成的 API，API 开发人员将需要提供其客户同意应用程序添加到客户租户中的一种方法。 推荐的设计是第三方开发人员用来生成 API，以便它也可以作为 web 客户端来实现注册。 为此，请按以下步骤操作：

1. 根据前面各节的内容确保 API 实现多租户应用程序注册/代码要求。
2. 除了公开 API 的作用域/角色，请确保注册包括"登录并读取用户个人资料"（默认情况下提供） 的权限。
