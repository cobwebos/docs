---
title: Azure AD v2 .NET Core 守护程序 | Microsoft Docs
description: 了解 .NET Core 进程如何使用应用的自身标识获取访问令牌并调用受 Azure Active Directory v2.0 终结点保护的 API
...
![显示本快速入门生成的示例应用的工作原理](media/quickstart-v2-netcore-daemon/netcore-daemon-intro-updated.png)

## <a name="prerequisites"></a>先决条件

本快速入门需要 [.NET Core 2.1](https://www.microsoft.com/net/download/dotnet-core/2.1)。
...
如果尝试在此时运行应用程序，则会收到“HTTP 403 - 禁止访问”错误：`Insufficient privileges to complete the operation`。 之所以发生这种情况，是因为任何仅限应用的权限都需要管理员许可，这意味着目录的全局管理员必须提供应用程序的许可。 根据自己的角色选择下面的一个选项：
...
MSAL ([Microsoft.Identity.Client](https://www.nuget.org/packages/Microsoft.Identity.Client)) 库用于用户登录和请求用于访问由 Microsoft Azure Active Directory (Azure AD) 保护的 API 的令牌库。 如前所述，本快速入门请求令牌的方法是使用应用程序自身的标识而不是委托的权限。 在此示例中使用的身份验证流称为[客户端凭据 oauth 流](v2-oauth2-client-creds-grant-flow.md)。 若要详细了解如何将 MSAL.NET 与客户端凭据流配合使用，请参阅[此文](https://aka.ms/msal-net-client-credentials)。
...
> | 其中： ||
> |---------|---------|
> | `secret` | 是在 Azure 门户中为应用程序创建的客户端机密。 |
> | `clientId` | 是在 Azure 门户中注册的应用程序的**应用程序(客户端) ID**。 可以在 Azure 门户的应用的“概览”页中找到此值。 |
> | `Authority`    | （可选）用户要进行身份验证的 STS 终结点。 对于公有云，通常为 <https://login.microsoftonline.com/{tenant}>，其中 {tenant} 是租户名称或租户 ID。|
> | `redirectUri`  | 用于在身份验证后发送用户的 URL。 在此示例中，由于这是控制台/非交互式应用程序，因此未使用此参数 |
> | `clientCredentials`  | 客户端凭据对象，包含机密或证书 |
> | `userTokenCache`  | 适用于用户的令牌缓存的实例。 在此示例中，由于此应用在应用上下文而非用户上下文中运行，因此此值为 null|
> | `appTokenCache`  | 适用于应用的令牌缓存的实例|

有关详细信息，请参阅 [`ConfidentialClientApplication` 的参考文档](https://docs.microsoft.com/dotnet/api/microsoft.identity.client.confidentialclientapplication.-ctor?view=azure-dotnet)

### <a name="requesting-tokens"></a>请求令牌

若要通过应用的标识来请求令牌，请使用 `AcquireTokenForClientAsync` 方法：
