...
| 个人 Microsoft 帐户、工作和学校帐户 | 100 | 应用程序清单中的 `signInAudience` 字段设置为*AzureADandPersonalMicrosoftAccount* |
...
目前 Azure AD 应用程序模型支持在任何组织的 Azure Active Directory （Azure AD）租户中使用 Microsoft 工作或学校帐户登录的应用的 HTTP 和 HTTPS 方案。 这是应用程序清单中 `signInAudience` 字段设置为*AzureADMyOrg*或*AzureADMultipleOrgs*。 对于登录到个人 Microsoft 帐户和工作和学校帐户（@no__t 设置为*AzureADandPersonalMicrosoftAccount*）的应用，只允许使用 HTTPS 方案。
...
通配符 Uri （例如 `https://*.contoso.com`）很方便，但应避免这样做。 在重定向 URI 中使用通配符具有安全含义。 根据 OAuth 2.0 规范（[RFC 6749 的第3.1.2 部分](https://tools.ietf.org/html/rfc6749#section-3.1.2)），重定向终结点 uri 必须是绝对 uri。 
