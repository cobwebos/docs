...
| `sid`                      | 会话 ID，用于每个会话用户注销。 | JWT        |  个人帐户和 Azure AD 帐户。   |         |
...
| `xms_pdl`          | 首选数据位置   | JWT | | 对于多地域租户，首选数据位置是显示用户所在地理区域的三字母代码。 有关详细信息，请参阅[有关首选数据位置的 Azure AD 连接文档](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnectsync-feature-preferreddatalocation)。<br/>例如：`APC` 表示“亚太”。 |
...
| `nickname`    | 别名                        | 用户的其他名称。 昵称不同于名字或姓氏。 需要`profile`作用域。| 
| `family_name` | 姓氏                       | 根据用户对象中的定义提供用户的姓氏。 <br>"family_name":"Miller" | MSA 和 Azure AD 中支持。 需要`profile`作用域。   |
| `given_name`  | 名字                      | 根据用户对象中的设置提供用户的名字和“姓氏”。<br>"given_name": "Frank"                   | MSA 和 Azure AD 中受支持。  需要`profile`作用域。 |
| `upn`         | 用户主体名称 | 可以与 username_hint 参数一起使用的用户标识符。  不是用户的持久标识符，不应当用于关键数据。 | 有关声明配置，请参阅下面的[附加属性](#additional-properties-of-optional-claims)。 需要`profile`作用域。|
...
此可选声明对象会导致返回到客户端的 ID 令牌包含包含具有附加主租户和资源租户信息的 upn 声明。 仅当用户是租户中的来宾（使用不同的 IDP 进行身份验证）时，才会更改令牌中的 `upn` 声明。 

## <a name="configuring-optional-claims"></a>配置可选声明

> [!IMPORTANT]
> 访问令牌**始终**使用资源而非客户端的清单生成。  因此，在请求`...scope=https://graph.microsoft.com/user.read...`中，资源是 Microsoft 图形 API。  因此，访问令牌是使用 Microsoft 图形 API 清单创建的，而不是客户端的清单。  更改应用程序的清单绝不会导致 Microsoft 图形 API 的令牌看起来不同。  若要验证 `accessToken` 更改是否生效，需请求你的应用程序的令牌，而不是另一应用的令牌。  
...
除了标准的可选声明集以外，还可将令牌配置为包含扩展。 有关详细信息，请参阅[Microsoft Graph 扩展属性文档](https://docs.microsoft.com/graph/api/resources/extensionproperty?view=graph-rest-1.0)- 请注意，可选声明不支持架构和打开扩展，仅支持 AAD-Graph 样式目录扩展。 使用此功能可以附加应用可以使用的附加用户信息 – 例如，用户设置的附加标识符或重要配置选项。 参阅此页面底部的示例。
...
-   也可以编写使用[Microsoft 图形 API](https://docs.microsoft.com/graph/use-the-api?context=graph%2Fapi%2F1.0&view=graph-rest-1.0)更新应用程序的应用程序。 Microsoft 图形 API 参考指南中的[可选声明](https://docs.microsoft.com/graph/api/resources/optionalclaims?view=graph-rest-1.0)类型可以帮助您配置可选声明。

**示例：** 在下面的示例中，您将使用**令牌配置（预览）UI**和**清单**向用于应用程序的访问、ID 和 SAML 令牌添加可选声明。 会将不同的可选声明添加到应用程序可以接收的每种令牌。
