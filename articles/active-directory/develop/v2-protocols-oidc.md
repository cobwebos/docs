...
ms.locfileid: "83771581"
---
# <a name="microsoft-identity-platform-and-openid-connect-protocol"></a>Microsoft 标识平台和 OpenID Connect 协议

OpenID Connect (OIDC) 是一种在 OAuth 2.0 基础之上构建而成的身份验证协议，可用于确保用户安全地登录 Web 应用。 使用 Microsoft 标识平台终结点的 OpenID Connect 实现时，可以向基于 Web 的应用添加登录和 API 访问。 本文介绍如何独立于语言执行此操作，并介绍如何在不使用任何 Microsoft 开源库的情况下发送和接收 HTTP 消息。

[OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html) 扩展了 OAuth 2.0 *授权*协议，使其可用作*身份验证*协议，这样一来，用户可使用 OAuth 执行单一登录。 OpenID Connect 引入了 ID 令牌的概念，ID 令牌是一种可让客户端验证用户标识的安全令牌。 ID 令牌还可获取用户的基本个人资料信息。 由于 OpenID Connect 扩展了 OAuth 2.0，因此应用可安全获取访问令牌，访问令牌可用于访问[授权服务器](active-directory-v2-protocols.md#the-basics)保护的资源。 使用 Microsoft 标识平台终结点，已向 Azure AD 注册的第三方应用可以为安全资源（如 Web API）颁发访问令牌。 若要详细了解如何设置应用以颁发访问令牌，请参阅[如何向 Microsoft 标识平台终结点注册应用](quickstart-register-app.md)。 如果要构建在服务器上托管并通过浏览器访问的 [web 应用程序](v2-app-types.md#web-apps)，建议使用 OpenID Connect。
...
OpenID Connect 描述了元数据文档，其中包含应用执行登录所需的大部分信息。 这包括要使用的 URL 和服务的公共签名密钥的位置等信息。 对于 Microsoft 标识平台终结点，这是你应使用的 OpenID Connect 元数据文档：
...
只接收 id_token 不足以对用户进行身份验证；必须验证 id_token 的签名，并按照应用的要求验证令牌中的声明。 Microsoft 标识平台终结点使用 [JSON Web 令牌 (JWT)](https://self-issued.info/docs/draft-ietf-oauth-json-web-token.html) 和公钥加密对令牌进行签名，并验证其是否有效。

可以选择在客户端代码中验证 `id_token`，但通常的做法是将 `id_token` 发送到后端服务器，并在其中执行验证。 验证 id_token 的签名后，就需要验证几项声明。 有关详细信息，请参阅 [`id_token` 参考](id-tokens.md)，其中包括[验证令牌](id-tokens.md#validating-an-id_token)和[有关签名密钥滚动更新的重要信息](active-directory-signing-key-rollover.md)。 我们建议利用库来分析和验证令牌 - 对于大多数语言和平台至少有一个可用。
...
通过在请求中添加权限范围并使用 `response_type=id_token code`，Microsoft 标识平台终结点可确保用户已同意 `scope` 查询参数中指明的权限。 它将授权代码返回应用，以交换访问令牌。
