...
ms.locfileid: "89437762"
---
# <a name="the-new-azure-portal-app-registration-experience"></a>新的 Azure 门户应用注册体验

Azure 门户的新 [应用注册](https://go.microsoft.com/fwlink/?linkid=2083908) 体验有很多改进。 如果你更熟悉应用程序注册门户 (用于注册或管理应用程序的 apps.dev.microsoft.com) （此处称为 "旧经验"），本指南将帮助你开始使用新的体验。
...
- 你的应用程序和相关配置在新体验中按原样保留。 无需再次注册应用程序，而且应用程序的用户无需再次登录。
    ...
    >
    > 在某些情况下，特别是当你使用个人 Microsoft (帐户（例如 Outlook、Live、Xbox 等）登录时，如果 ) 使用 Azure AD 的电子邮件地址，我们发现，当你从旧体验中获得了 Azure 门户时，它会在你的 Azure AD 租户中使用同一电子邮件登录到不同的帐户。 如果你仍相信你的应用程序丢失，请注销并以正确的帐户登录。

- 在 Azure 门户中尚不支持使用个人 Microsoft 帐户创建的 Live SDK 应用，并将在不久的将来继续保留旧体验。

## <a name="key-changes"></a>关键更改

-   在过去的体验中，应用默认注册为 *汇聚* 应用-支持所有组织帐户 (多租户) 和个人 Microsoft 帐户的应用。 这不能通过旧体验进行修改，因此很难创建 (多租户或单租户) 仅支持组织帐户的应用。
...
-   新的应用列表显示了已通过 Azure 门户中的旧应用注册体验注册的应用程序 () 仅 Azure AD 帐户登录的应用程序，以及通过 [应用程序注册门户](https://apps.dev.microsoft.com/) 注册的应用 (Azure AD 和 Microsoft 个人帐户) 登录的应用。

-   新应用列表有两个附加列： **在** "列和 **证书" & "机密** " 列中创建，显示已在应用上注册的凭据 (当前状态、即将到期或过期) 。

## <a name="new-app-registration"></a>新应用注册

在过去的体验中，若要注册应用，只需提供一个名称。 已创建的应用已注册为 *汇聚* 应用-支持所有组织目录 (多租户) 和个人 Microsoft 帐户的应用。  这种情况无法通过旧体验进行修改，因此很难创建仅支持组织帐户 (单租户) 或多租户的应用。 [了解有关支持的帐户类型的详细信息](v2-supported-account-types.md)
...
在新体验中，可以在 "应用程序身份验证" 部分找到 "回复 Url" \' 。 此外，它们也称为重定向 Uri，重定向 Uri 的格式已更改。 它们需要与应用类型 (web 或公用客户端-移动和桌面) 相关联。 [了解详细信息](quickstart-register-app.md#add-a-redirect-uri)

Web Api 在公开 API 页中进行配置。

> [!NOTE]
> 尝试新的“身份验证设置”体验，在其中可以根据要面向的平台或设备配置应用程序的设置。  [了解详细信息](quickstart-register-app.md#configure-platform-settings)
...
-   向用户显示 [应用程序 \' 许可提示](application-consent-experience.md)) 上的发布者域 (，可以在 "品牌" 页面上找到。 有关如何配置发布者域的详细信息，请参阅[此操作指南](howto-configure-publisher-domain.md)。
