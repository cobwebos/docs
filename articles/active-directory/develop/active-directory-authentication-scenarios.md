...
* 单租户：如果在构建仅供你的组织使用的应用程序，则必须使用 Azure 门户在你公司的目录中注册该应用程序。
...
注册应用程序之后，必须将其配置为使用 OAuth 2.0 隐式授予协议。 默认情况下，应用程序禁用此协议。 若要为应用程序启用 OAuth2 隐式授予协议，从 Azure 门户中编辑该协议的应用程序清单，将“oauth2AllowImplicitFlow”值设置为 true。 有关详细说明，请参阅[为单页面应用程序启用 OAuth 2.0 隐式授权](active-directory-integrating-applications.md)。
...
1. 本机应用程序使用浏览器弹出窗口向 Azure AD 中的授权终结点发出请求。 此请求包括应用程序 ID 和本机应用程序的重定向 URI（与 Azure 门户中显示的相同），以及 Web API 的应用程序 ID URI。 如果用户尚未登录，同样将会提示他们登录
