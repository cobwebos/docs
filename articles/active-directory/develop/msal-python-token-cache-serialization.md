---
title: 自定义令牌缓存序列化 （MSAL Python） |蔚蓝
...
序列化令牌缓存的策略因您编写公共客户端应用程序（桌面）还是机密客户端应用程序（Web 应用、Web API 或守护进程应用程序）而异。
...
对于 Web 应用或 Web API，您可以使用会话、Redis 缓存或数据库来存储令牌缓存。 每个用户（每个帐户）应有一个令牌缓存，因此请确保按帐户序列化令牌缓存。

## <a name="next-steps"></a>后续步骤

有关如何为 Windows 或 Linux Web 应用或 Web API 使用令牌缓存的示例，请参阅[ms 身份-python-webapp。](https://github.com/Azure-Samples/ms-identity-python-webapp/blob/master/app.py#L64-L72) 该示例适用于调用 Microsoft Graph API 的 Web 应用。
