...
### <a name="app-topologies-impacted"></a>影响的应用拓扑
...
* 你正在构建多租户 Web 应用，该应用使用 Microsoft Graph 访问 Exchange 及其他服务。  采用此应用的企业客户设置了关于 SharePoint Online 的策略。  此 Web 应用请求 MS Graph 令牌时，将应用关于任意 Microsoft 服务的策略（尤其是可以通过 Graph 访问的服务）。  将提示该最终用户进行 MFA。 在此场景下，最终用户通过有效令牌登录，将向 Web 应用返回一个声明“质询”。  
* 你正在构建本机应用，该应用使用中间层服务访问 Microsoft Graph。  公司中使用此应用的企业客户对 Exchange Online 应用了策略。  最终用户登录时，本机应用将请求访问中间层并将发送令牌。  中间层执行代理流来请求访问 MS Graph。  此时，将向中间层发出一个声明“质询”。 中间层将质询发送回本机应用，本机应用需符合条件访问策略。
...
若要查看演示如何处理声明质询的代码示例，请参阅适用于 ADAL .NET 的 [.NET 桌面代码示例](https://github.com/Azure-Samples/active-directory-dotnet-native-desktop)或适用于 ADAL .NET 的[代理代码示例](https://github.com/Azure-Samples/active-directory-dotnet-webapi-onbehalfof-ca)。
