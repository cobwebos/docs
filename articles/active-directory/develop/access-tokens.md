...
| `azp` | 字符串，GUID | 仅在 v2.0 令牌中提供，替换为 `appid`。 使用令牌的客户端的应用程序 ID。 该应用程序可以自身名义或者代表用户进行操作。 应用程序 ID 通常表示应用程序对象，但它还可以表示 Azure AD 中的服务主体对象。 |
| `azpacr` | “0”、“1”或“2” | 仅在 v2.0 令牌中提供，替换为 `appidacr`。 表示对客户端进行身份验证的方式。 对于公共客户端，值为“0”。 如果使用客户端 ID 和客户端机密，则值为“1”。 如果使用客户端证书进行身份验证，则值为“2”。 |
...
| `wids` | [RoleTemplateID](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-assign-admin-roles#role-template-ids) guid 的数组 | 从["管理员角色" 页](https://docs.microsoft.com/azure/active-directory/users-groups-roles/directory-assign-admin-roles#role-template-ids)中的角色部分，表示分配给此用户的租户范围的角色。  此声明通过[应用程序清单](reference-app-manifest.md)的 `groupMembershipClaims` 属性，基于每个应用程序进行配置。  将其设置为 "All" 或 "DirectoryRole" 是必需的。  由于令牌长度方面的原因，通过隐式流获取的令牌中可能不存在。 |
