...
> [!IMPORTANT]
> 访问令牌是根据令牌的受众（即，拥有令牌中范围的应用程序）创建的。  如果在[应用清单](reference-app-manifest.md#manifest-reference)中将资源设置 `accessTokenAcceptedVersion` 指定为 `2`，则允许客户端调用 v1.0 终结点来接收 v2.0 访问令牌。  同样，这就是为什么更改客户端的访问令牌[可选声明](active-directory-optional-claims.md)不会更改为 `user.read` 请求令牌时收到的访问令牌的原因，该令牌归 MS Graph 资源所有。  
...
> [!组超额声明] 若要确保令牌大小不超过 HTTP 标头大小限制，Azure AD 会限制组声明中包含的对象 Id 的数量。 如果用户所属的组超过超额限制（150对于 SAML 令牌为，对于 JWT 令牌为200），则 Azure AD 不会在令牌中发出组声明。 相反，它在令牌中包含超额声明，该声明指示应用程序查询图形 API 以检索用户的组成员身份。
> { ..."_claim_names"： {"groups"： "src1"}，{"_claim_sources"： {"src1"： {"endpoint"： "[Graph Url，从此用户的组成员身份获取]"}}    
    ... }你可以使用 " `BulkCreateGroups.ps1` [应用创建脚本](https://github.com/Azure-Samples/active-directory-dotnet-webapp-groupclaims/blob/master/AppCreationScripts/)" 文件夹中提供的来帮助测试超额方案。
...
声明指示用于对令牌进行签名的算法， `kid`而声明表示用于验证令牌的特定公钥。 `alg`
