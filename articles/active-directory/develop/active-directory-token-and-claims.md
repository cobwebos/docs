...
如果应用是通过 Azure AD 保护的 API，需要 HTTP 请求中的访问令牌，则应验证和检查接收到的令牌。 应用应先对访问令牌执行验证，再用其访问资源。 有关验证的详细信息，请参阅[验证令牌](#validating-tokens)。 有关如何使用 .NET 执行此操作的详细信息，请参阅[使用 Azure AD 的持有者令牌保护 Web API](active-directory-devquickstarts-webapi-dotnet.md)。
