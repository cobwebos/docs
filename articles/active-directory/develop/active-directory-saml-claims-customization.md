...
3. 若要应用多个转换，请单击 "**添加转换**"。最多可将两个转换应用于一个声明。 例如，你可以先提取的电子邮件前缀`user.mail`。 然后，将字符串设为大写。
...
首先，Azure AD 验证 Britta 的用户类型是否为`All guests`。 从开始，Azure AD 将声明的源分配给`user.extensionattribute1`。 其次，Azure AD 验证 Britta 的用户类型是否为`AAD guests`，因为这也是 true，因此 Azure AD 将声明的源分配给`user.mail`。 最后，通过 Britta 的值`user.email`发出声明。
