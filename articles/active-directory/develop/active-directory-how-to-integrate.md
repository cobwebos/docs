...
**减少登录阻碍并降低支持成本。** 通过使用 Microsoft 标识平台登录到你的应用程序，你的用户将不会再有一个名称和密码要记住。 作为开发人员，可以减少要存储和保护的密码。 无需重置忘记的密码，单凭这一点就能节省不少的精力。 Microsoft 标识平台为某些世界上最受欢迎的云应用程序（包括 Office 365 和 Microsoft Azure）进行登录。 用户可能已登录到 Microsoft 标识平台，拥有数百万个组织的数百万用户。 详细了解如何[添加对 Microsoft 标识平台登录的支持](./authentication-vs-authorization.md)。
...
**Microsoft 标识平台是适用于 Office 365 和其他 Microsoft 业务服务的授权服务器。**  如果你支持使用 Microsoft 标识平台登录到你的应用程序，或者支持将当前用户帐户链接到使用 OAuth 2.0 Azure AD 用户帐户，则可以请求对用户配置文件、日历、电子邮件、联系人、文件和其他信息的读取和写入访问权限。 可以顺利地将事件写入用户日历，以及在其 OneDrive 中读取或写入文件。 深入了解如何[访问 Office 365 API](/previous-versions/office/office-365-api/)。
...
**用户注册应用程序后，该程序会在其 Azure AD 访问面板和 Office 365 应用启动器中显示。**  之后，用户能够快速轻松地返回应用程序，可提高用户参与度。 深入了解 [Azure AD 访问面板](../user-help/my-apps-portal-end-user-access.md)。
...
**异常登录检测。**  Microsoft 标识平台每天处理超过10亿次登录，同时使用机器学习算法来检测可疑活动，并通知 IT 管理员可能存在的问题。 通过支持 Microsoft 标识平台登录，你的应用程序将获得此保护的好处。 深入了解如何[查看 Azure Active Directory 访问报告](../reports-monitoring/overview-reports.md)。

**条件性访问。**  除了多重身份验证以外，管理员可以要求用户在登录应用程序之前满足特定的条件。 可设置的条件包括客户端设备的 IP 地址范围、指定的组中的成员资格，以及用于访问的设备的状态。 详细了解[Azure Active Directory 的条件性访问](../conditional-access/overview.md)。
