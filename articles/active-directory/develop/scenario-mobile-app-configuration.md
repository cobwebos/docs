---
title: 配置用于调用 web Api 的移动应用-Microsoft 标识平台 |Microsoft
...
还有其他一些*参数*方法用于设置 UI 父代、替代默认机构、指定客户端名称和版本（对于遥测）、指定重定向 URI、指定要使用的 Http 工厂（例如，处理代理、指定遥测和日志记录）。 这是以下段落的主题。
