...
若要启用其中一项功能，应用程序开发人员在`WithBroker()` `PublicClientApplicationBuilder.CreateApplication`调用方法时需要使用参数。 `.WithBroker()` 默认设置为 true。 开发人员还需要执行适用于[iOS](#brokered-authentication-for-ios)或[Android](#brokered-authentication-for-android)应用程序的步骤。
...
当适用于 .net 的 Microsoft 身份验证库（MSAL.NET）调用代理时，代理反过来会通过`OpenUrl` `AppDelegate`类的方法回调到你的应用程序。 由于 MSAL 会等待来自代理的响应，因此，你的应用程序需要通过合作来调用 MSAL.NET。 若要启用此协作，请`AppDelegate.cs`更新文件以重写以下方法。
...
MSAL.NET 使用 URL 调用中介，然后将中介响应返回到应用。 若要完成往返行程，请在`Info.plist`文件中为应用注册 URL 方案。
...
添加`msauthv2`到`Info.plist`文件`LSApplicationQueriesSchemes`的部分。
...
### <a name="step-7-make-sure-the-redirect-uri-is-registered-with-your-app"></a>步骤 7：确保重定向 URI 已注册到你的应用
