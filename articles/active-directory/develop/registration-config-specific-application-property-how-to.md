...
| 字段            | 描述                                                                              |
|------------------|------------------------------------------------------------------------------------------|
| “属性”             | 应用程序的名称。 它应至少具有四个字符。                |
| 支持的帐户类型| 选择您希望应用程序支持的帐户：仅此组织目录中的帐户、任何组织目录中的帐户或任何组织目录中的帐户以及个人 Microsoft 帐户中的帐户。  |
| 重定向 URI（可选） | 选择要生成的应用的类型：“Web”或“公共客户端(移动和桌面)”，然后输入应用程序的重定向 URI (或回复 URL)。******** 对于 Web 应用程序，请提供应用的基 URL。 例如，http://localhost:31544 可以是本地计算机上运行的 Web 应用的 URL。 用户将使用此 URL 登录到 Web 客户端应用程序。 对于公共客户端应用程序，请提供 Azure AD 返回令牌响应时所用的 URI。 输入特定于应用程序的值，如myapp://auth。要查看 Web 应用程序或本机应用程序的具体示例，请查看我们的[快速入门](https://docs.microsoft.com/azure/active-directory/develop)。|
...
| 应用 ID URI      | 这应该是一个唯一的 URI，通常https://**&lt;租户&gt;/&lt;\_名称应用程序\_名称&gt;** 的窗体。 这在权限授予流程中作为指定应对其颁发令牌的资源的唯一标识符使用。 也将成为已颁发访问令牌中的“aud”声明。 |
