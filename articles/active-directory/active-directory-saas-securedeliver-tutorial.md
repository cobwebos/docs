...
若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 SECURE DELIVER 用户。 换句话说，需要在 Azure AD 用户与 SECURE DELIVER 中相关用户之间建立链接关系。  
...
本部分的目的是在 Azure 经典门户中启用 Azure AD 单一登录并在 SECURE DELIVER 应用程序中配置单一登录。
...
3. 在“配置应用设置”对话框页上，执行以下步骤，然后单击“下一步”：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入用户用于登录 SECURE DELIVER 应用程序的 URL：**“https://i-securedeliver.jp/sd/\<公司名称\>/jsf/login/sso”**。
   
    b.保留“数据库类型”设置，即设置为“共享”。 如果不知道值，请通过 [iw-sd-support@fujifilm.com](mailto:iw-sd-support@fujifilm.com) 联系 SECURE DELIVER 支持团队以获取租户 URL。
...
本部分的目的是通过授予 Britta Simon 访问 SECURE DELIVER 的权限，允许她使用 Azure 单一登录。
