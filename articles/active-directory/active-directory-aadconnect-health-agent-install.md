...
| Azure AD Connect Health 代理已安装在每台目标服务器上 |Azure AD Connect Health 要求在目标服务器上安装代理，以提供可在门户中查看的数据。 </br></br>例如，若要从 AD FS 本地基础结构获取数据，必须将代理安装在 AD FS、AD FS 代理和 Web 应用程序代理服务器上。 同样，要获取 AD DS 本地基础结构的相关数据，必须将代理安装在域控制器上。 </br></br>**重要说明：**在安装代理时使用的帐户必须是工作帐户或学校帐户， 而不能是 Microsoft 帐户。 有关详细信息，请参阅 [Sign up for Azure as an organization](sign-up-organization.md)（以组织身份注册 Azure） |
| Azure 服务终结点的出站连接 |在安装期间和运行时，代理需要连接到 Azure AD Connect Health 服务终结点。 如果出站连接被阻止，请确保在允许列表中添加以下终结点： </br></br><li>&#42;.blob.core.windows.net </li><li>&#42;.queue.core.windows.net</li><li>adhsprodwus.servicebus.windows.net - Port: 5671 </li><li>https://management.azure.com </li><li>https://s1.adhybridhealth.azure.com/</li><li>https://policykeyservice.dc.ad.msft.net/</li><li>https://login.windows.net</li><li>https://login.microsoftonline.com</li><li>https://secure.aadcdn.microsoftonline-p.com</li> |
...
此时会启动一个命令提示符，其后面是执行 Register-AzureADConnectHealthADFSAgent 的一些 PowerShell 命令。 根据系统的提示登录到 Azure。
...
#### <a name="to-enable-auditing-for-ad-fs-20"></a>启用 AD FS 2.0 审核的步骤
...
> [!WARNING]
> 如果某个组策略正在禁用 AD FS 审核，Azure AD Connect Health 代理将无法收集信息。 请确保没有可能正在禁用审核的组策略。
> 
> 
...
## <a name="installing-the-azure-ad-connect-health-agent-for-ad-ds-on-server-core"></a>在 Server Core 上安装用于 AD DS 的 Azure AD Connect Health 代理。
