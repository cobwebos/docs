...
# 将自己的域名添加到 Azure Active Directory
...
你可以使用下列方式之一，将最多 900 个域名添加到 Azure AD 目录：
...
> [AZURE.NOTE]
> 是否使用 Office 365？ 设置域后，你可以开始创建使用自定义域名的电子邮件地址、Lync Online 帐户和分发组。还可以将你的域名用于 SharePoint Online 上托管的面向公众的网站。
...
2. 在“添加域”页上，键入你要添加的域名，然后执行以下操作之一：
    1. 如果你不打算将本地 Active Directory 与 Azure AD 集成，请执行以下操作：
        1. 保留“我计划配置此域为使用本地 Active Directory 进行单点登录”复选框的清除状态，然后单击“添加”按钮。
...
> [AZURE.NOTE]
> 是否使用 Microsoft Intune？ 不需要编辑 Microsoft Intune 云服务的 DNS 记录。
...
若要检查域的状态，请在“域”页上单击该域，然后在“域属性”页上单击“域故障排除”。
...
1. 在 Azure 门户上的“Azure Active Directory”中，单击“域”。
