...
* **信息保护管理员**：具有此角色的用户可以在 Azure 门户中访问 Azure 信息保护。 他们可以配置 Azure 信息保护策略的标签、管理保护模板，以及激活保护。
...
In | 有权执行的操作
-------- | ---------
Azure 信息保护 | * 配置全局策略、作用域内策略和标签<br>* 管理、配置、更新保护模板<br>* 启用保护激活以使用 
Privileged Identity Management | * 读取权限<br>* 不能管理 Azure AD 角色成员身份或设置。
