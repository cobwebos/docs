...
* 可以让用户使用其 Azure AD 帐户自动登录到 RightScale 单一登录 (SSO)
...
## <a name="add-rightscale-from-the-gallery"></a>从库中添加 RightScale
...
本部分旨在说明如何基于名为“Britta Simon”的测试用户配置和测试 RightScale 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 RightScale 用户。 换句话说，需要在 Azure AD 用户与 RightScale 中相关用户之间建立链接关系。  
...
本部分旨在介绍如何在经典门户中启用 Azure AD SSO 并在 RightScale 应用程序中配置 SSO。
...
3. 在“配置应用设置”对话框页上，如果要在“IDP 发起的模式”下配置应用程序，请执行以下步骤，然后单击“下一步”：
  ...
  1. 在“回复 URL”文本框中，使用以下模式键入 URL：`https://login.rightscale.com/login/saml2/consume`
...
1. 如果要在“配置应用设置”对话框页上以“SP 发起的模式”配置应用程序，则单击“显示高级设置(可选)”，然后输入“登录 URL”，并单击“下一步”。
  ...
  1. 在“登录 URL”文本框中，使用以下模式键入用户用于登录 RightScale 应用程序的 URL：`https://login.rightscale.com/`
  2. 单击“下一步”。
...
3. 若要为应用程序配置 SSO，需要以管理员身份登录 RightScale 租户。
  ...
  6. 将 Azure AD 中的实体 ID 复制到 RightScale 中的 **SAML EntityID**。
...
### <a name="create-a-rightscale-test-user"></a>创建 RightScale 测试用户
...
本部分旨在通过授予 Britta Simon 访问 RightScale 的权限，允许她使用 Azure SSO。
