...
本部分旨在说明如何基于名为“Britta Simon”的测试用户配置和测试 Workrite 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 Workrite 用户。 换言之，需要建立 Azure AD 用户与 Workrite 中相关用户之间的链接关系。  

通过将 Azure AD 中“用户名”的值分配为 Workrite 中“用户名”的值来建立此链接关系。

若要配置和测试 Workrite 的 Azure AD SSO，需要完成以下构建基块：
...
本部分旨在介绍如何在 Azure 经典门户中启用 Azure AD SSO 并在 Workrite 应用程序中配置 SSO。

**若要配置 Workrite 的 Azure AD SSO，请执行以下步骤：**
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
    ...
    ![Azure AD 单一登录][8] 
  1. 在“登录 URL”文本框中，键入用户登录到 Workrite 站点所使用的 URL（例如：*https://app.workrite.co.uk/securelogin/samlgateway.aspx?id=1a82b5aa-4dd6-4472-9721-7d0193f59e22*）。
...
4. 在“配置 Workrite 的单一登录”页上，执行以下步骤：
 ...
 2. 联系 Workrite 支持团队 [support@workrite.co.uk](mailto:support@workrite.co.uk)，向其提供下载的证书、**颁发者 URL**（实体 ID）、**单一登录服务 URL**、**单一注销 URL**，然后请求他们为 Workrite 应用设置 SSO。  
...
4. 在“创建用户”对话框中，执行以下步骤：
    ...
    ![分配用户][402]
 1. 键入要预配的有效 Azure AD 用户的**电子邮件**、**名字**和**姓氏**。  
 2. 对于“选择角色”选项，请选择“客户端管理员”。  
 3. 单击“保存” 。   

### <a name="assign-the-azure-ad-test-user"></a>分配 Azure AD 测试用户
本部分旨在通过授予 Britta Simon 访问 Workrite 的权限，允许她使用 Azure SSO。
