...
使用条件访问策略，可以控制用户如何访问你的[云应用](conditional-access/conditions.md#cloud-apps)。 配置条件访问策略时，至少需要一个云应用。 
...
在条件访问策略中，可以配置[客户端应用](conditional-access/conditions.md#client-apps)条件，以将策略绑定到发起访问尝试的客户端应用。 设置客户端应用条件，在用户尝试从以下类型的客户端应用进行访问时授予其访问权限或阻止访问：
...
- 如果已准备好在环境中配置条件访问策略，请参阅 [Azure Active Directory 中条件访问的推荐做法](conditional-access/best-practices.md)。
