...
在本部分中，将在 Azure 管理门户中启用 Azure AD 单一登录并在 Cezanne HR Software 应用程序中配置单一登录。
...
3. 在“Cezanne HR Software 域和 URL”部分中，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入 URL：`https://w3.cezanneondemand.com/cezannehr/-/<tenant id>`
    
    b.保留“数据库类型”设置，即设置为“共享”。 在“标识符”文本框中，键入：`https://w3.cezanneondemand.com/CezanneOnDemand/`

    > [!NOTE] 
    > 请注意，这些不是实际值。 必须使用实际登录 URL 和标识符更新这些值。 此处我们建议在“标识符”中使用 URL 的唯一值。 若要获取这些值，请联系 [Cezanne HR Software 支持团队](mailto:info@cezannehr.com)。
...
14. 在“SAML 2.0 标识提供者”部分中执行以下步骤。
    ...
    b.保留“数据库类型”设置，即设置为“共享”。 在“实体标识符”文本框中，放置 Azure AD 应用程序配置窗口中的“SAML 实体 ID”值。

    c. 将“SAML 绑定”更改为“POST”。

    d. 在“安全令牌服务终结点”文本框中，放置 Azure AD 应用程序配置窗口中的“SAML 单一登录服务 URL”值。

    e.在“新建 MySQL 数据库”边栏选项卡中，接受法律条款，然后单击“确定”。 在“用户 ID 属性名称”文本框中输入“http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name”。
...
4.  在“帐户信息”部分中，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“用户名”文本框中，键入 Britta Simon 的电子邮件地址。
