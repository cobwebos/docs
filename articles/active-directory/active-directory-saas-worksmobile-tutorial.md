...
- 可让用户使用其 Azure AD 帐户自动登录到 WORKS MOBILE 单一登录 (SSO)
...
## <a name="add-works-mobile-from-the-gallery"></a>从库中添加 WORKS MOBILE
...
在本部分中，将基于名为“Britta Simon”的测试用户配置并测试 WORKS MOBILE 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 WORKS MOBILE 用户。 换句话说，需要建立 Azure AD 用户与 WORKS MOBILE 中相关用户之间的链接关系。

可以通过将 Azure AD 中“用户名”的值分配为 WORKS MOBILE 中“用户名”的值来建立此链接关系。

若要配置和测试 WORKS MOBILE 的 Azure AD SSO，需要完成以下构建基块：
...
在本部分中，将在 Azure 管理门户中启用 Azure AD SSO，并在 WORKS MOBILE 应用程序中配置 SSO。

**若要配置 WORKS MOBILE 的 Azure AD SSO，请执行以下步骤：**
...
3. 在“WORKS MOBILE 域和 URL”部分中，执行以下步骤：
  ...
  1. 在“登录 URL”文本框中，使用以下模式键入 URL：`https://<your-subdomain>.worksmobile.com/jp/myservice`
  2. 在“标识符”文本框中，键入 `worksmobile.com` 值。

    >[!NOTE] 
    >这些不是实际值。 必须使用实际登录 URL 和标识符更新这些值。 此处我们建议在“标识符”中使用字符串的唯一值。 若要获取这些值，请联系 [WORKS MOBILE 支持团队](mailto:dl_ssoinfo@worksmobile.com)。 
    >
    >
...
10. 若要为应用程序配置 SSO，请联系 [WORKS MOBILE 支持团队](mailto:dl_ssoinfo@worksmobile.com)并向他们提供以下内容：• 下载的**证书文件** • **SAML 单一登录服务 URL** • **SAML 实体 ID** • **注销 URL**
...
### <a name="create-a-works-mobile-test-user"></a>创建 WORKS MOBILE 测试用户
...
在本部分中，通过授予 Britta Simon 访问 WORKS MOBILE 的权限，允许她使用 Azure SSO。
