...
**2.可公开访问 AD FS 联盟元数据。** 从公共 Internet（企业网络之外）上的计算机导航到以下 URL，查看你的联合身份验证元数据是否可以公开访问：
...
>[AZURE.NOTE] 如果你需要支持多个顶级域（例如 contoso.com 和 fabrikam.com），则必须将 **SupportMultipleDomain** 开关用于任何 cmdlet。有关详细信息，请参阅 [Support for Multiple Top Level Domains](active-directory-aadconnect-multiple-domains.md)（支持多个顶级域）。
...
有关详细信息，请阅读 [Repairing the trust](/documentation/articles/active-directory-aadconnect-federation-management/#repairing-the-trust)（修复信任）。
