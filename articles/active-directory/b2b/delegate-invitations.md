...
description: 了解如何启用 Active Directory B2B 外部协作并管理谁可以邀请来宾用户。 使用来宾邀请者角色委派邀请。
...
本文介绍如何启用 Azure 活动目录 （Azure AD） B2B 协作并确定谁可以邀请来宾。 默认情况下，目录中的所有用户和来宾都可以邀请来宾，即使他们未分配到管理员角色。 通过外部协作设置，您可以为组织中不同类型的用户打开或关闭来宾邀请。 您还可以通过分配允许其邀请来宾的角色，将邀请委托给单个用户。
...
### <a name="to-configure-external-collaboration-settings"></a>要配置外部协作设置：
...
4. 在 **"外部协作设置"** 页上，选择要启用的策略。
  ...
  - **来宾用户权限有限**：此策略确定目录中来宾的权限。 选择 **"是**"可阻止来宾访问某些目录任务，例如枚举用户、组或其他目录资源。 选择 **"否**"可使来宾与目录中的常规用户对目录数据具有相同的访问权限。
   - **来宾邀请者角色的管理员和用户可以邀请**：要允许管理员和用户在"来宾邀请者"角色中邀请来宾，请此策略设置为 **"是**"。
   - **会员可以邀请**：要允许目录中的非管理员成员邀请来宾，请此策略设置为 **"是**"。
   - **客人可以邀请**：为了允许客人邀请其他客人，请将此政策设置为 **"是**"。
...
使用来宾邀请者角色，您可以让单个用户能够邀请来宾，而无需为其分配全局管理员或其他管理员角色。 将来宾邀请者角色分配给个人。 然后，请确保设置**管理员和用户在来宾邀请者角色可以邀请**到**是**。
