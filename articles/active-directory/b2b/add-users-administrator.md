---
title: 在 Azure 门户 - Azure AD 中添加 B2B 协作用户
...
作为分配任何有限管理员目录角色的用户，可以使用 Azure 门户邀请 B2B 协作用户。 可将来宾用户邀请到目录、组或应用程序。 通过上述任一方法邀请用户后，受邀用户的帐户将添加到 Azure Active Directory (Azure AD)，其用户类型为“来宾”。** 然后，来宾用户必须兑换其邀请才能访问资源。 用户的邀请不会过期。
...
确保组织的外部协作设置已配置为允许您邀请来宾。 默认情况下，所有用户和管理员都可以邀请来宾。 但是，您的组织的外部协作策略可能配置为防止某些类型的用户或管理员邀请来宾。 要了解如何查看和设置这些策略，请参阅[启用 B2B 外部协作并管理谁可以邀请来宾](delegate-invitations.md)。

## <a name="add-guest-users-to-the-directory"></a>将来宾用户添加到目录

若要将 B2B 协作用户添加到目录，请执行以下步骤：

1. 以分配有限管理员目录角色或来宾邀请者角色的用户身份登录到[Azure 门户](https://portal.azure.com)。
...
5. 在“新建用户”页上，选择“邀请用户”，然后添加来宾用户的信息********。 
   ...
   - **组**：您可以将来宾用户添加到一个或多个现有组，也可以稍后执行此操作。
   - **目录角色**：如果需要用户的 Azure AD 管理权限，则可以将它们添加到 Azure AD 角色。 
...
如果需要手动将 B2B 协作用户添加到组，请按照以下步骤操作：
