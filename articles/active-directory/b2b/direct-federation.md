---
title: 与用于 B2B Azure AD 的标识提供者的直接联合
...
要与联合的域在 Azure AD 中***不得进行 DNS***验证。 允许你设置与非托管（电子邮件验证或 "病毒"） Azure AD 租户的直接联合，因为未对其进行 DNS 验证。
...
例如，为**fabrikam.com**设置直接联合时，`https://fabrikam.com/adfs` 的身份验证 URL 将通过验证。 同一域中的主机也会通过，例如 `https://sts.fabrikam.com/adfs`。 但是，同一域的身份验证 URL `https://fabrikamconglomerate.com/adfs` 或 `https://fabrikam.com.uk/adfs` 不会通过。
...
## <a name="step-1-configure-the-partner-organizations-identity-provider"></a>步骤1：配置合作伙伴组织的标识提供者
...
## <a name="step-2-configure-direct-federation-in-azure-ad"></a>步骤2：在 Azure AD 中配置直接联合 
...
## <a name="step-3-test-direct-federation-in-azure-ad"></a>步骤3：在 Azure AD 中测试直接联合
