...
在本教程中，将为混合环境启用密码写回。 密码写回用来将 Azure Active Directory (Azure AD) 中的密码更改同步回本地 Active Directory 域服务 (AD DS) 环境。 作为Azure AD Connect的一部分，可以启用密码写回，从而提供一种安全机制，将密码更改从Azure AD发送回现有的本地目录。 可以在[什么是密码写回](concept-sspr-writeback.md)一文中找到有关密码写回的内部工作的更多详细信息。
...
* 访问一个工作的Azure AD租户，至少要分配一个试用许可证。
...
在 Azure AD Connect 中启用密码写回功能只是操作的一半。 允许 SSPR 使用密码写回将完成整个循环，从而允许更改或重置密码的用户在本地设置密码。

1. 使用全局管理员帐户登录到 [Azure 门户](https://portal.azure.com)。
2. 浏览到“Azure 活动目录”，单击“密码重置，然后选择“本地集成”。
