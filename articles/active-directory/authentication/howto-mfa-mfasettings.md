---
title: 配置 Azure 多重身份验证 - Azure 活动目录
...
您可以通过浏览 Azure**活动目录** > **安全** > **MFA**从 Azure 门户访问与 Azure 多重身份验证相关的设置。
...
| [欺诈警报](#fraud-alert) | 配置与用户报告欺诈性验证请求的能力相关的设置 |
...
2. 浏览到**Azure 活动目录** > **安全** > **MFA** > **阻止/取消阻止用户**。
3. 选择“添加”以阻止用户****。
4. 选择“复制组”****。 输入被阻止用户的**用户名作为用户名\@domain.com**。 在“原因”字段输入注释****。
5. 选择“添加”，完成阻止用户操作****。
...
* **初始问候期间报告欺诈的代码** - 当用户接收电话呼叫来执行双重验证时，他们通常按 **#** 来确认其登录。 要报告欺诈，用户在按 之前**#** 输入代码。 此代码默认为 **0**，但可以自定义此代码。
...
在美国，如果您尚未配置 MFA 呼叫者 ID，则来自 Microsoft 的语音呼叫来自以下号码：+1 （866） 539 4191、+1 （855） 330 8653 和 +1 （877） 668 6536。 如果使用垃圾邮件过滤器，请确保排除这些数字。
...
1. 以管理员身份登录到 [Azure 门户](https://portal.azure.com)。
1. 浏览到**Azure 活动目录** > **安全** > **MFA** > **电话呼叫设置**。
1. 选择“添加问候语”****。
1. 选择问候的类型。
1. 选择语言。
...
| --- | --- |
| 身份验证成功 | 您的登录已成功验证。 再见。 |
| 扩展提示 | 感谢您使用微软的登录验证系统。 请按磅键继续。 |
| 欺诈确认 | 已提交欺诈警报。 要取消阻止您的帐户，请联系您公司的 IT 帮助台。 |
| 欺诈问候语（标准） | 感谢您使用微软的登录验证系统。 请按英镑键完成验证。 如果您未启动此验证，则可能有人试图访问您的帐户。 请按零磅提交欺诈警报。 这将通知公司的 IT 团队并阻止进一步的验证尝试。 |
| 欺诈报告 已提交欺诈警报。 | 要取消阻止您的帐户，请联系您公司的 IT 帮助台。 |
| 激活 | 感谢您使用微软的登录验证系统。 请按英镑键完成验证。 |
| 身份验证被拒绝重试 | 验证被拒绝。 |
| 重试（标准） | 感谢您使用微软的登录验证系统。 请按英镑键完成验证。 |
| 问候语（标准） | 感谢您使用微软的登录验证系统。 请按英镑键完成验证。 |
| 问候语 （PIN） | 感谢您使用微软的登录验证系统。 请输入您的 PIN，后跟英镑密钥以完成验证。 |
| 欺诈问候语 （PIN） | 感谢您使用微软的登录验证系统。  请输入您的 PIN，后跟英镑密钥以完成验证。 如果您未启动此验证，则可能有人试图访问您的帐户。 请按零磅提交欺诈警报。 这将通知公司的 IT 团队并阻止进一步的验证尝试。 |
| 重试（PIN） | 感谢您使用微软的登录验证系统。 请输入您的 PIN，后跟英镑密钥以完成验证。 |
| 数字后扩展提示 | 如果已在此扩展处，请按磅键继续。 |
| 身份验证被拒绝 | 对不起，我们现在不能签你。 请稍后重试。 |
| 激活问候语（标准） | 感谢您使用微软的登录验证系统。 请按英镑键完成验证。 |
| 激活重试（标准） | 感谢您使用微软的登录验证系统。 请按英镑键完成验证。 |
| 激活问候语 （PIN） | 感谢您使用微软的登录验证系统。 请输入您的 PIN，后跟英镑密钥以完成验证。 |
| 数字前的扩展提示 | 感谢您使用微软的登录验证系统。 请转接此呼叫到分机... |
...
Azure 多重身份验证的应用密码、受信任 IP、验证选项以及“记住多重身份验证”设置可以在服务设置中找到。 可以通过浏览 Azure**活动目录** > **安全** > **MFA** > **开始** > **配置** > **其他基于云的 MFA 设置**从 Azure 门户访问服务设置。

![Azure 多重身份验证服务设置](./media/howto-mfa-mfasettings/multi-factor-authentication-settings-service-settings.png)

受信任的 IP 地址范围可以是私有的，也可以是公共的。
...
>[!NOTE]
>应用密码不适用于基于条件访问的多重身份验证策略和现代身份验证。
...
> [!NOTE]
> MFA 受信任的 IP 和条件访问命名位置仅适用于 IPV4 地址。

如果你的组织部署 NPS 扩展，向本地应用程序提供 MFA，请注意源 IP 地址将始终显示为身份验证尝试流经的 NPS 服务器。

| Azure AD 租户类型 | 受信任的 IP 功能选项 |
|:--- |:--- |
| 托管 |**特定 IP 地址范围**：由管理员指定一组 IP 地址范围，以便为从公司 Intranet 登录的用户绕过双重验证。 最多可以配置 50 个受信任的 IP 范围。|
...
>[!IMPORTANT]
>请记住，如果帐户或设备遭到入侵，受信任的设备的多重身份验证可能影响安全性。 如果公司帐户被泄露或受信任设备丢失或被盗，则应[撤销 MFA 会话](howto-mfa-userdevicesettings.md)。
>
...
此功能可以减少 Web 应用的身份验证次数，而通常每次使用时都会提示验证。 此功能会增加新式身份验证客户端的身份验证次数（通常每隔 90 天提示一次）。 与条件访问策略结合使用时，还可以增加身份验证数。
...
>
>**记住多重身份验证**功能与 B2B 用户不兼容，在登录到受邀的租户时，B2B 用户将不可见。
>
