---
title: IIS 身份验证和 Azure MFA 服务器 - Azure 活动目录
...
2. 单击 **"基于窗体"** 选项卡。
3. 单击 **添加**。
4. 要自动检测用户名、密码和域变量，请在"自动配置基于表单的网站`https://localhost/contoso/auth/login.aspx`"对话框中输入登录 URL（如 ），然后单击"**确定**"。
...
10. 如果所有用户都已或即将导入服务器并接受多重身份验证，则选中 **"需要 Azure 多重身份验证"用户匹配**框。 如果大量用户尚未导入到该服务器并且/或者将免除 Multi-Factor Authentication，请使该框处于未选中状态。
11. 单击“高级”**** 以查看高级设置，包括：
...
2. 单击**HTTP**选项卡。
3. 单击 **添加**。
4. 在"添加基本 URL"对话框中，输入执行 HTTP 身份验证的网站的 URL（<http://localhost/owa>如 ），并提供应用程序名称（可选）。 应用程序名称将出现在 Azure 多重身份验证报告中，并可能会显示在短信或移动应用身份验证消息中。
...
1. 如果在 IIS 6 上运行，请单击**ISAPI**选项卡。选择 Web 应用程序运行下的网站（例如默认网站），以启用该站点的 Azure 多重身份验证 ISAPI 筛选器插件。
2. 如果在 IIS 7 或更高版本上运行，请单击**本机模块**选项卡。选择服务器、网站或应用程序以在所需级别启用 IIS 插件。
...
2. 单击 **添加**。
3. 当"添加受信任的 IP"对话框出现时，选择 **"单个 IP、IP****范围**"或 **"子网**"单选按钮。
4. 输入应允许的 IP 地址、IP 地址范围或子网。 如果输入子网，请选择适当的网络掩码，并单击“确定”****。
