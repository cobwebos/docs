...
2. 单击 "**基于窗体**" 选项卡。
3. 单击“添加”  。
4. 若要自动检测用户名、密码和域变量，请在 "自动配置`https://localhost/contoso/auth/login.aspx`基于窗体的网站" 对话框中输入登录 URL （例如），然后单击 **"确定"**。
...
10. 选中 "**需要 Azure 多重身份验证用户匹配**" 框（如果所有用户均已导入或将导入到服务器中），并受多重身份验证的限制。 如果大量用户尚未导入到该服务器并且/或者将免除 Multi-Factor Authentication，请使该框处于未选中状态。
11. 单击“高级”**** 以查看高级设置，包括：
...
1. 如果在 IIS 6 上运行，请单击 " **ISAPI** " 选项卡。选择运行 web 应用程序的网站（例如默认网站），以便为该网站启用 Azure 多重身份验证 ISAPI 筛选器插件。
2. 如果在 IIS 7 或更高版本上运行，请单击 "**本机模块**" 选项卡。选择要在所需级别启用 IIS 插件的服务器、网站或应用程序。
...
2. 单击“添加”  。
3. 显示 "添加受信任的 Ip" 对话框时，选择 "**单个 IP**"、" **ip 范围**" 或 "**子网**" 单选按钮。
