...
本部署指南介绍了如何规划和测试 Azure 多重身份验证。

若要快速查看 Azure 多重身份验证的操作，并返回以了解其他部署注意事项：
...
Microsoft 提供[通信模板](https://aka.ms/mfatemplates)和[最终用户文档](../user-help/security-info-setup-signin.md)来帮助草拟你的通信。 您可以通过选择该[https://myprofile.microsoft.com](https://myprofile.microsoft.com)页面上的**安全信息**链接，将用户发送到直接注册。
...
管理员必须确定用户注册其方法的方式。 组织应为 Azure MFA 和自助密码重置（SSPR）[启用新的组合注册体验](howto-registration-mfa-sspr-combined.md)。 SSPR 允许用户使用用于多重身份验证的相同方法，以安全的方式重置密码。 建议使用这种组合注册，因为这是用户的绝佳体验，能够为两个服务注册一次。 为 SSPR 和 Azure MFA 启用相同的方法将允许注册用户使用这两种功能。
...
在 ISE 窗口中运行此 PowerShell，或另存`.PS1`为要本地运行的文件。
...
选择在未注册 MFA 的用户尝试进行身份验证时所发生的情况。 使用注册表路径`HKLM\Software\Microsoft\AzureMFA`中`REQUIRE_USER_MATCH`的注册表设置来控制功能行为。 此设置包含一个配置选项。
...
以下指南详细介绍了如何管理 AD FS 服务器上的 Azure MFA 证书。 当你通过 Azure MFA 配置 AD FS 时，通过`New-AdfsAzureMfaTenantCertificate` PowerShell cmdlet 生成的证书的有效期为两年。 在过期之前，请续订并安装续订的证书，以在 MFA 服务中 ovoid 中断。
