---
title: 将 Azure MFA 服务器与 AD FS 2.0 - Azure 活动目录一起使用
...
4. 要自动检测用户名、密码和域变量，请在"自动配置基于表单的网站"`https://sso.contoso.com/adfs/ls`对话框中输入登录 URL（如 ），然后单击"**确定**"。
...
7. 在"添加基于表单的网站"对话框中，在"提交 URL"字段中输入 AD FS 登录页的`https://sso.contoso.com/adfs/ls`URL（如 ），然后输入应用程序名称（可选）。 应用程序名称将出现在 Azure 多重身份验证报告中，并可能会显示在短信或移动应用身份验证消息中。
...
10. 如果所有用户均已导入或将导入到该服务器并接受双重验证，请选中“需要 Azure 多重身份验证用户匹配”**** 框。 如果大量用户尚未导入到该服务器并且/或者将免除进行多重身份验证，请使该框处于未选中状态。
    ...
    ![将基于表单的网站添加到 MFA 服务器](./media/howto-mfaserver-adfs-2/manual.png)
...
12. 由于不可能将 AD FS 代理服务器加入到域，因此，可以使用 LDAP 连接到域控制器来进行用户导入和预身份验证。 在"基于表单的网站高级"对话框中，单击 **"主身份验证"** 选项卡，并为预身份验证类型选择**LDAP 绑定**。
...
![注册表编辑器用于配置公司设置](./media/howto-mfaserver-adfs-2/reg.png)
...
确保已将 Active Directory 中的用户导入服务器。 如果要允许内部 IP 地址，请参阅可信 IP 部分，以便在从这些位置登录网站时不需要两步验证。
...
![将受信任的 IP 配置为 MFA 服务器](./media/howto-mfaserver-adfs-2/trusted.png)
