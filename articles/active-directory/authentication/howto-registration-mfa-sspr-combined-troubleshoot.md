...
| User registered all required security info | 失败 | User canceled security info registration. | This event occurs when a user cancels registration from interrupt mode.|
...
| User reviewed security info | 失败 | User failed to review security info. | This event occurs when a user selects **Looks good** on the security info review page but something fails on the backend.|
...
| User deleted security info | 失败 | User failed to delete *method*. | This event occurs when a user tries to delete a method but the attempt fails for some reason. *Method* can be Authenticator app, Phone, Email, Security questions, App password, Alternate phone, and so on.|
...
1. Install the appropriate Azure AD PowerShell modules. 在 PowerShell 窗口中，运行以下命令以安装模块：
...
1. Save the list of affected user object IDs to your computer as a text file with one ID per line. 记下该文件的位置。
...
In a PowerShell window, run the following command, providing the script and user file locations. 出现提示时，输入全局管理员凭据。 该脚本将输出每个用户更新操作的结果。
