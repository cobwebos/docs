...
1. 用户选择“无法访问帐户”链接或直接转到 [https://aka.ms/sspr](https://passwordreset.microsoftonline.com)****。
   ...
   * 要以不同的本地化语言将"？mkt_"查看密码重置门户，请到密码重置 URL 的末尾，并附上本地化为西班牙语[https://passwordreset.microsoftonline.com/?mkt=es-us](https://passwordreset.microsoftonline.com/?mkt=es-us)。
2. 用户输入用户 ID 并传递验证码。
...
用户在注册从[https://aka.ms/ssprsetup](https://aka.ms/ssprsetup)重置自助服务密码时，没有注册其移动应用的选项。 用户可以在 注册其移动应用，[https://aka.ms/mfasetup](https://aka.ms/mfasetup)或在[https://aka.ms/setupsecurityinfo](https://aka.ms/setupsecurityinfo)。
...
> [!IMPORTANT]
> 在配置 1 门策略时，无法选择身份验证器应用作为唯一的身份验证方法。 同样，在配置双门策略时，无法选择身份验证器应用和仅选择一个附加方法。
> 然后，在配置包含身份验证器应用作为方法的 SSPR 策略时，在配置 1-gate 策略时至少应选择一个其他方法，并且在配置 2 个门策略时应至少选择两个其他方法。
> 提出此要求的原因是，当前的 SSPR 注册体验不包括注册身份验证器应用的选项。 注册身份验证器应用的选项包含在[新的合并注册中，用于自助服务密码重置和 Azure 多重身份验证（公共预览）。](concept-registration-mfa-sspr-converged.md)
> 允许仅使用身份验证器应用（对于 1 门策略）或身份验证器应用和仅一种附加方法（对于双门策略）的策略可能会导致用户被阻止注册 SSPR，直到它们配置为使用新的注册经验。
...
如果禁用了注册要求，用户可以手动注册。 他们可以访问[https://aka.ms/ssprsetup](https://aka.ms/ssprsetup)或选择"**注册密码重置**"链接，该链接位于"访问面板"中的 **"配置文件"** 选项卡下。
...
[Writeback]: ./media/concept-sspr-howitworks/troubleshoot-on-premises-integration-writeback.png "本地集成密码写回配置和故障排除信息"
