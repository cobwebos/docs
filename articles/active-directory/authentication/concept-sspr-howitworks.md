...
>
> 此要求是因为当前 SSPR 注册体验不包含用于注册验证器应用的选项。 用于注册验证器应用的选项包含在新的[组合注册体验](concept-registration-mfa-sspr-converged.md)中。
>
> 允许只使用验证器应用的策略（需要一个方法时），或验证器应用和另一个方法（如果需要两个方法时），则可能会导致阻止用户注册 SSPR，直到将其配置为使用新的组合注册体验。
...
* **B2b 用户**：通过使用新的[Azure AD b2b 功能](../b2b/what-is-b2b.md)创建的任何新 b2b 用户也可以使用他们在邀请过程中注册的电子邮件来重置其密码。
