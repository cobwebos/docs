---
title: 使用 NPS - Azure 活动目录提供 Azure MFA 功能
...
4. **Azure MFA**与 Azure 活动目录通信，以检索用户的详细信息，并使用配置到用户的验证方法执行辅助身份验证。
...
安装该扩展时，需要使用 Azure AD 租户的目录 ID 和管理员凭据。 可在 [Azure 门户](https://portal.azure.com)中找到该目录 ID。 以管理员身份登录。 搜索并选择**Azure 活动目录**，然后选择**属性**。 复制“目录 ID”**** 框中的 GUID 并保存。 安装 NPS 扩展时，使用此 GUID 作为租户 ID。
...
1. 在 RADIUS 客户端（VPN、Netscaler 服务器或其他客户端）与 NPS 服务器之间使用的密码加密算法。
   - **PAP**支持云中 Azure MFA 的所有身份验证方法：电话呼叫、单向短信、移动应用通知、OATH 硬件令牌和移动应用验证码。
...
3. 运行*安装程序.exe*并按照安装说明操作。 如果发生错误，请仔细检查先决条件部分的两个库是否已成功安装。

#### <a name="upgrade-the-nps-extension"></a>升级 NPS 扩展

升级现有 NPS 扩展安装时，为了避免重新启动基础服务器，请完成以下步骤：
...
如果以前的计算机证书已过期，并且已生成新证书，则应删除任何过期的证书。 证书过期可能会导致 NPS 扩展启动出现问题。

> [!NOTE]
> 如果使用自己的证书，而不是使用 PowerShell 脚本生成的证书，请确保它们符合 NPS 命名约定。 使用者名称必须为“CN=\<TenantID\>”，“OU=Microsoft NPS Extension”****。 

### <a name="microsoft-azure-government-additional-steps"></a>微软 Azure 政府其他步骤

对于使用 Azure 政府云的客户，每个 NPS 服务器上都需要执行以下其他配置步骤：
...
1. 重复前两个步骤，为每个 NPS 服务器设置注册表键值。
1. 重新启动每个 NPS 服务器的 NPS 服务。

    为了最小的影响，将每个 NPS 服务器从 NLB 旋转中一次拉出一个，并等待所有连接耗尽。

### <a name="certificate-rollover"></a>证书滚动更新

使用 NPS 扩展版本 1.0.1.32 时，现在支持读取多个证书。 此功能将有助于在滚动证书更新到期之前进行滚动更新。 如果您的组织正在运行 NPS 扩展的早期版本，则应升级到版本 1.0.1.32 或更高版本。

`AzureMfaNpsExtnConfigSetup.ps1`脚本创建的证书的有效期为 2 年。 IT 组织应监视证书的过期情况。 NPS 扩展证书放置在"个人"下的本地计算机证书存储中，并颁发给提供给脚本的租户 ID。

当证书接近到期日期时，应创建新证书以替换它。  此过程是通过`AzureMfaNpsExtnConfigSetup.ps1`再次运行 并在提示时保留相同的租户 ID 来完成的。 此过程应在环境中的每个 NPS 服务器上重复。
...
- NPS 扩展使用本地活动目录中的 UPN 来标识 Azure MFA 上的用户以执行辅助 Auth。可以将扩展配置为使用其他标识符，如备用登录 ID 或自定义活动目录字段（UPN） 以外的字段。 有关详细信息，请参阅[用于多重身份验证的 NPS 扩展的高级配置选项](howto-mfa-nps-extension-advanced.md)一文。
- 并非所有加密协议都支持所有验证方法。
...
在对 NPS 扩展进行故障排除时，可以使用以下脚本执行基本运行状况检查步骤。
...
由*AzureMfaNpsExttttttconfigSetup.ps1*脚本生成的自签名证书也有两年的有效性生存期。 验证是否安装了证书时，还应检查证书是否尚未过期。
...
运行此命令后，转到 C 驱动器，找到该文件并双击该文件。 转到详细信息并向下滚动到“指纹”，将服务器上安装的证书的指纹与此指纹进行比较。 证书指纹应匹配。

如果该命令返回了多个证书，可以使用 Valid-From 和 Valid-Until 时间戳（采用用户可读格式）来筛选出每个不相符的项。

---

### <a name="why-cannot-i-sign-in"></a>为什么我不能登录？

检查密码是否未过期。 NPS 扩展不支持在登录工作流中更改密码。 请与组织的 IT 员工联系以获得进一步帮助。
...
### <a name="why-is-authentication-not-working-despite-a-valid-certificate-being-present"></a>为什么身份验证不起作用，尽管存在有效的证书？

如果以前的计算机证书已过期，并且已生成新证书，则应删除任何过期的证书。 证书过期可能会导致 NPS 扩展启动出现问题。

要检查您是否具有有效的证书，请使用 MMC 检查本地计算机帐户的证书存储，并确保证书尚未超过其到期日期。 要生成新有效的证书，应重新运行"[运行 PowerShell 脚本](#run-the-powershell-script)"一节下的步骤
...
其他故障排除指南和可能的解决方案可以在"[解决来自 Azure 多重身份验证的 NPS 扩展名中的错误消息](howto-mfa-nps-extension-errors.md)"一文中找到。

## <a name="next-steps"></a>后续步骤

- [Windows 服务器网络策略服务器的概述和配置](https://docs.microsoft.com/windows-server/networking/technologies/nps/nps-top)
