---
title: Azure MFA 服务器和活动目录 - Azure 活动目录
...
可以根据需要自定义特定目录的属性。  这样，便可以添加自定义属性，优化为只同步所需的属性。 为每个属性字段的值使用目录架构中定义的属性的名称。 下表提供了有关每项功能的更多信息。
...
| 可分辨名称 |输入包含每条记录的可分辨名称的属性的属性名称。  在 Active Directory 中，它通常是 distinguishedName。 其他 LDAP 实现可能使用 entryDN 或类似值。  默认值为 distinguishedName。 <br><br>如果仅包含可分辨名称的属性不存在，则可以使用广告路径属性。  将自动剥除路径的“LDAP://\<server\>/”部分，仅留下对象的可分辨名称。 |
...
要编辑属性，请单击"**属性"选项卡上的"编辑**"。 这将打开一个窗口，您可以在其中编辑属性。 选择任一属性旁边的“...”会打开一个窗口，可在其中选择要显示的属性。****
...
![目录对象与 MFA 服务器同步](./media/howto-mfaserver-dir-ad/dirint5.png)
...
可以设置其他多重身份验证服务器作为备份 RADIUS 代理、LDAP 代理或 IIS 身份验证。 同步配置在所有代理之间共享。 但是，这些代理中只有一个可能运行多重身份验证服务器服务。 此选项卡允许您选择应启用同步的多重身份验证服务器。
