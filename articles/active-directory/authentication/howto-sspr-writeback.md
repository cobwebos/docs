...
* **支持使用[密码哈希同步](./../connect/active-directory-aadconnectsync-implement-password-hash-synchronization.md)的用户重置密码**：如果密码重置服务检测到已同步的用户帐户启用了密码哈希同步，我们会同时重置此帐户的本地密码和云密码。 密码回写不依赖于密码哈希同步。
