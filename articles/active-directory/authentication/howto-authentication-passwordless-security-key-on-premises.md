...
description: 启用无密码安全密钥登录到本地资源（预览版）
...
ms.locfileid: "73164856"
---
# <a name="enable-passwordless-security-key-sign-in-to-on-premises-resources-preview"></a>启用无密码安全密钥登录到本地资源（预览版）
...
将在本地 Active Directory 中创建 Azure AD Kerberos 服务器对象，并将其安全地发布到 Azure Active Directory。 对象不与任何物理服务器关联。 它只是 Azure Active Directory 用于为 Active Directory 域生成 Kerberos 票证授予票证（Tgt）的一种资源。

![从 Azure AD 和 AD DS 中获取 TGT 和 PRT](./media/howto-authentication-passwordless-on-premises/fido2-tgt-exchange-process.png)
...
在完成本文中的步骤之前，组织必须完成文章[启用无密码 security key sign To Windows 10 设备（预览版）](howto-authentication-passwordless-security-key.md)中的步骤。

组织必须满足以下软件要求。

- 设备必须运行 Windows 10 内部版本18945或更高版本
- 升级到最新版本的[Azure AD Connect](../hybrid/how-to-connect-install-roadmap.md#install-azure-ad-connect)
- 已充分修补 Windows Server 2016/2019 域控制器以处理本地身份验证请求负载。

### <a name="supported-scenarios"></a>支持的方案

- 此方案支持对两者进行单一登录（SSO）：
   - 云资源，如 Office 365 和其他启用 SAML 的应用程序。
   - 本地资源和对网站的 Windows 集成身份验证（包括需要 IIS 身份验证的网站和 SharePoint 站点）和/或使用 NTLM 身份验证的资源。

### <a name="unsupported-scenarios"></a>不支持的方案

不支持以下方案：

- Windows Server Active Directory 域服务（AD DS）已加入域（仅限本地设备）**不支持**部署。
...
管理员将使用其 Azure AD Connect 服务器中的 PowerShell 工具在其本地目录中创建 Azure AD 的 Kerberos 服务器对象。 需要在组织中包含 Azure AD 用户的每个域和林中运行这些步骤。

1. 升级到最新版本的 Azure AD Connect。 说明假设已将 Azure AD Connect 配置为支持混合环境。
1. 在 Azure AD Connect 服务器上，打开提升的 PowerShell 提示符，导航到 C:\Program Files\Microsoft Azure Active Directory Connect\AzureADKerberos\
...
此命令将输出 Azure AD Kerberos 服务器的属性。 您可以查看属性以验证所有内容是否都按正确的顺序进行。

| properties | 描述 |
| --- | --- |
| ID | AD 域控制器对象的唯一 ID。 此 ID 有时称为 "槽" 或它是 "分支 ID"。 |
| DomainDnsName | Active Directory 域的 DNS 域名。 |
| ComputerAccount | Azure AD Kerberos 服务器对象（DC）的计算机帐户对象。 |
| 用户帐户 | 已禁用的用户帐户对象，该对象保存 Azure AD Kerberos 服务器 TGT 加密密钥。 此帐户的 DN 将为： <br> CN = krbtgt_AzureAD，CN = Users，< Domain-DN > |
| KeyVersion | Azure AD Kerberos 服务器 TGT 加密密钥的密钥版本。 创建密钥时，将分配该版本。 然后，每次对密钥进行旋转时，版本都会递增。 增量是基于复制元数据的，可能大于1。 例如，初始 KeyVersion 可能为192272。 第一次轮换密钥时，版本可能会前进到212621。 要验证的重要一点是，本地对象的 KeyVersion 和云对象的 CloudKeyVersion 是相同的。 |
| KeyUpdatedOn | 更新/创建 Azure AD Kerberos 服务器 TGT 加密密钥的日期和时间。 |
| KeyUpdatedFrom | 上次更新 Azure AD Kerberos 服务器 TGT 加密密钥的域控制器。 |
...
例如，你的组织有一个包含两个域 contoso.com 和 fabrikam.com 的 Active Directory 林。 如果选择允许 Azure AD 为整个林颁发 Kerberos 票证授予票证（TGT），Azure AD 中将有两个 KerberosDomain 对象。 一个用于 contoso.com 的 KerberosDomain 对象，一个用于 fabrikam.com。 如果有多个 Active Directory 的林，则每个林中的每个域都有一个 KerberosDomain 对象。

你需要在包含 Azure AD 用户的组织中的每个域和林中运行 "[创建 Kerberos 服务器" 对象](#create-kerberos-server-object)中的步骤。

## <a name="known-behavior"></a>已知行为

如果密码已过期，则将阻止通过 FIDO 登录。 希望用户重置其密码，然后才能使用 FIDO 登录。
...
如果清理安装混合 Azure AD 联接的计算机，则必须使用密码登录，并等待策略同步，然后才能使用 FIDO 登录。

- 通过在命令窗口中键入 "`dsregcmd /status`" 来检查当前状态，并检查 `AzureAdJoined` 和 `DomainJoined` 是否都显示 `YES`。
- 此延迟是加入域的设备的已知限制，而不是 FIDO 特定的限制。
...
请确保修补了足够的域控制器来响应资源请求。 若要检查是否可以看到运行该功能的域控制器，请检查 nltest/dsgetdc： contoso/keylist/kdc. 的输出
