---
title: Azure AD 密码保护中的故障排除-Azure Active Directory
...
此问题可能会表现出各种症状, 但通常有一个常见的根本原因。

Azure AD 密码保护对 Microsoft 密钥分发服务提供的加密和解密功能非常重要, 此功能在运行 Windows Server 2012 和更高版本的域控制器上提供。 必须在域中的所有 Windows Server 2012 和更高版本的域控制器上启用并运行 KDS 服务。
...
1. 你未在域中的所有域控制器上安装 DC 代理软件。 在这种情况下, 很难确保远程 Windows 客户端在密码更改操作过程中以特定的域控制器为目标。 如果你认为已成功将安装了 DC 代理软件的特定 DC 作为目标, 则可以通过双重检查 DC 代理管理事件日志进行验证: 无论结果如何, 都至少要有一个事件来记录密码的结果检查. 如果用户的密码更改不存在任何事件, 则可能是由另一个域控制器处理密码更改。
