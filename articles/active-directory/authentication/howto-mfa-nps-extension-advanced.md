...
当发出请求的 IP 地址来自白名单时，将跳过双重验证。 将 IP 允许列表与 RADIUS 请求的 ratNASIPAddress 属性中的 IP 地址相比较。 如果收到的 RADIUS 请求不包含 ratNASIPAddress 属性，则将记录以下警告：“P_WHITE_LIST_WARNING::IP 白名单被忽略，因为 RADIUS 请求中的 NasIpAddress 属性缺少源 IP。”
