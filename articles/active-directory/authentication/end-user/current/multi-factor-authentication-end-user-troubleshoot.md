...
首先，请确保正确输入应用密码。 生成的应用密码会替换普通的密码，但这只适用于不支持双重验证的旧式桌面应用程序。 如果仍然不起作用，请尝试登录并[创建新的应用密码](multi-factor-authentication-end-user-app-passwords.md)。  如果还是不起作用，请联系公司支持人员，让其[删除现有应用密码](../../howto-mfa-userdevicesettings.md)，然后便可以新建密码。
