---
title: 使用 Microsoft Authenticator 应用 (预览版) 启用无密码登录-Azure Active Directory
...
创建这个新的强凭据的先决条件之一是, 该设备还必须在 Azure AD 租户内注册到单个用户。 由于当前设备注册限制, 只能在单个租户中注册设备。 此项限制意味着，只能为 Microsoft Authenticator 应用中的一个工作或学校帐户启用手机登录。
