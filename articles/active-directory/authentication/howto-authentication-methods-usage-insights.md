...
要直接访问使用情况&见解，请访问[https://portal.azure.com/#blade/Microsoft_AAD_IAM/AuthMethodsOverviewBlade](https://portal.azure.com/#blade/Microsoft_AAD_IAM/AuthMethodsOverviewBlade)。 单击此链接会转到注册概述。

“已注册用户”、“已启用用户”和“已赋能用户”磁贴显示用户的以下注册数据：

- 已注册：如果用户（或管理员）注册了足够的身份验证方法以满足组织的 SSPR 或多重身份验证策略，则被视为已注册用户。
- 已启用：如果用户在 SSPR 策略范围内，则视为已启用用户。 如果为某个组启用了 SSPR，而某个用户在该组中，则认为该用户是已启用的用户。 如果为所有用户启用了 SSPR，则认为租户中的所有用户（不包括来宾）都是已启用的用户。
- 支持：如果用户同时注册并启用，则认为该用户已启用。 此状态意味着该用户随时可按需执行 SSPR。
