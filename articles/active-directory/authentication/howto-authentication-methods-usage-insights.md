...
1. 浏览到**Azure Active Directory** > **密码重置** >  **& insights**。
...
- 已启用：如果用户处于 SSPR 策略范围内，则将其视为已启用。 如果为某个组启用了 SSPR，则如果用户在该组中，则认为该用户处于启用状态。 如果为所有用户启用了 SSPR，则租户中的所有用户（不包括来宾）都将被视为已启用。
- 功能：如果用户已注册并处于启用状态，则会将该用户视为能够使用。 此状态意味着它们可以在需要时随时执行 SSPR。
