...
在本教程中，您将在组织中启用 Azure AD 自助密码重置 (SSPR) 的试用版，并使用非管理员帐户进行测试。

任何自助服务密码重置的测试都必须使用非管理员帐户完成。 Microsoft 管理管理员帐户的密码重置策略，需要使用更严格的身份验证方法。 此策略不允许使用安全性问题和回答，要求使用两种方法进行重置。
...
1. 开始使用试点组时，请为组织中的部分用户启用自助服务密码。
   * 在“属性”页中，在“启用自助服务密码重置”选项下，选择“选定”，然后选取一个试验组。
...
1. 在“注册”页上
   * 针对“要求用户在登录时注册”选择“是”。
   * 将“要求用户重新确认其身份验证信息之前的天数”设置为“180”。
   * 单击“保存”
1. 在“通知”页上
   * 将“重置密码时通知用户”选项设置为“是”。
...
现在，已为试验组中的云用户配置了自助密码重置。

## <a name="test-sspr-as-a-user"></a>以用户身份测试 SSPR

使用非管理员测试用户（属于试点组的成员）测试自助密码重置。 **请注意，如果您使用的帐户具有分配给它的任何管理员角色，那么身份验证方法和编号可能与您选择的Microsoft管理员策略不同。**

1. 打开一个新的 隐私模式 或 匿名模式 浏览器窗口。
