...
本教程将介绍如何在组织中推出 Azure AD 自助密码重置 (SSPR) 的试用版，并使用非管理员帐户进行测试。

任何自助密码重置的测试都应使用非管理员帐户完成。 Microsoft 管理管理员帐户的密码重置策略，要求使用严格的身份验证方法。此策略不允许使用安全性问题和回答，而要求使用两种重置方法。
...
使用非管理员测试用户（属于试点组的成员）测试自助密码重置。 请注意，如果所使用的帐户具有任何管理员角色，那么身份验证方法和数量可能与所选的有所不同，因为由Microsoft管理管理员策略
