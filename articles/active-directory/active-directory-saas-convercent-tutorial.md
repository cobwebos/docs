...
- 可以让用户使用其 Azure AD 帐户自动登录到 Convercent 单一登录 (SSO)
...
## <a name="add-convercent-from-the-gallery"></a>从库中添加 Convercent
...
在本部分中，将基于名为“Britta Simon”的测试用户配置并测试 Convercent 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 Convercent 用户。 换句话说，需要在 Azure AD 用户与 Convercent 中的相关用户之间建立链接关系。

可以通过将 Azure AD 中“用户名”的值分配为 Convercent 中“用户名”的值来建立此链接关系。

若要配置和测试 Convercent 的 Azure AD SSO，需要完成以下构建基块：
...
在本部分中，将在 Azure 新门户中启用 Azure AD SSO 并在 Convercent 应用程序中配置单一登录。
...
3. 在“Convercent 域和 URL”部分，如果要在“IDP 发起的模式”下配置应用程序，请执行以下步骤：
  ...
  1. 在“标识符”文本框中，键入 `https://sts.convercent.com/`
  2. 单击“显示高级 URL 设置”。
  3. 在“中继状态”文本框中，键入：`https://app.convercent.com/`
...
4. 在“Convercent 域和 URL”部分，如果要在“SP 发起的模式”下配置应用程序，请执行以下步骤：
  ...
  * 在“登录 URL”文本框中，键入 `https://app.convercent.com/`
    >[!NOTE] 
...
### <a name="create-a-convercent-test-user"></a>创建 Convercent 测试用户
...
在本部分中，通过授予 Britta Simon 访问 Convercent 的权限，允许她使用 Azure SSO。
