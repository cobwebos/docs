...
* 可以让用户使用其 Azure AD 帐户自动登录到 eTouches 单一登录 (SSO)
...
在本部分中，将基于名为“Britta Simon”的测试用户配置并测试 eTouches 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 eTouches 用户。 换句话说，需要在 Azure AD 用户与 eTouches 中的相关用户之间建立链接关系。

可以通过将 Azure AD 中“用户名”的值分配为 eTouches 中“用户名”的值来建立此链接关系。

若要配置和测试 eTouches 的 Azure AD SSO，需要完成以下构建基块：
...
**若要配置 eTocuhes 的 Azure AD SSO，请执行以下步骤：**
...
2. 在“SAML 令牌属性”对话框中，对于下表中显示的每个行，执行以下步骤：
 ...
 1. 单击“添加用户属性”，打开“添加用户属性”对话框。
  ...
  4. 单击“完成”。    
3. 在经典门户中，在 **eTouches** 应用程序集成页上，单击“配置单一登录”，以打开“配置单一登录”对话框。
   
    ![配置单一登录][6] 
4. 在“你希望用户如何登录到 eTouches”页上，选择“Azure AD 单一登录”，然后单击“下一步”。
...
5. 在“配置应用设置”对话框页上，执行以下步骤：
  ...
  1. 在“登录 URL”文本框中，使用以下模式键入用户用来登录 eTouches 应用程序的 URL：**https://www.eiseverywhere.com/saml/accounts/?sso&accountid=\<accountid\>**。
  2. 单击“下一步”。
6. 在“在 eTouches 处配置单一登录”页上，执行以下步骤：
   
    ![配置单一登录](./media/active-directory-saas-etouches-tutorial/tutorial_etouches_05.png)  
  1. 单击“下载元数据”，然后在计算机上保存该文件。0
  2. 单击“资源组名称” 的 Azure 数据工厂。
7. 若要为应用程序配置 SSO，请在 eTouches 应用程序中执行以下步骤：  
  1. 使用管理权限登录到 **eTouches** 应用程序。 
  2. 转到 **SAML** 配置。
  3. 在“常规设置”部分中，将 Azure AD 联合元数据内容粘贴到文本框中。
  4. 单击“保存并停留”按钮。
  5. 在“SAML 元数据”部分中单击“更新元数据”按钮。 
  6. 这将打开该页面并执行 SSO。 SSO 开始运行后，便可设置用户名。
  7. 在“用户名”字段中，选择 **emailaddress**，如下图所示。 
  8. 复制“SSO URL / ACS”值，并将其粘贴到 Azure AD 应用程序配置向导中的“登录 URL”文本框中。
...
5. 在“告诉我们有关此用户的信息”对话框页上，执行以下步骤：
 ...
 ![创建 Azure AD 测试用户](./media/active-directory-saas-etouches-tutorial/create_aaduser_05.png) 
  1. 在“用户类型”中，选择“你的组织中的新用户”。
  2. 在“用户名”文本框中，键入“BrittaSimon”。
  3. 单击“资源组名称” 的 Azure 数据工厂。
6. 在“用户配置文件”对话框页上，执行以下步骤：

 ![创建 Azure AD 测试用户](./media/active-directory-saas-etouches-tutorial/create_aaduser_06.png)  
