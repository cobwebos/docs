...
- [快速入门：通过 Azure Active Directory 条件访问要求特定应用进行多重身份验证](active-directory-conditional-access-app-based-mfa.md) - 体验一下如何配置条件访问策略。 
...
此场景的挑战在于将“在从不受信任的位置尝试访问时”转换为条件访问条件。 在条件访问策略中，可以配置[位置条件](active-directory-conditional-access-locations.md)来应对与网络位置相关的场景。 使用位置条件，你可以选择[命名位置](active-directory-conditional-access-locations.md#named-locations)，这些位置表示 IP 地址范围、国家和地区的逻辑分组。  
...
使用本文中概述的方法，你现在可以针对不受信任位置配置条件访问策略。 在将策略推广到生产环境中之前，始终应当对策略进行测试以确保它按预期工作。 理想情况下，如果可能，应当在测试租户中执行初始测试。 有关详细信息，请参阅[应当如何部署新策略](active-directory-conditional-access-best-practices.md#how-should-you-deploy-a-new-policy)。 
