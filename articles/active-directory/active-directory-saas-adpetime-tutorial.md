...
* 可使用户通过其 Azure AD 帐户自动登录 ADP eTime 单一登录 (SSO)
...
## <a name="add-adp-etime-from-the-gallery"></a>从库中添加 ADP eTime
...
本部分旨在说明如何基于名为“Britta Simon”的测试用户配置和测试 ADP eTime 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 ADP eTime 用户。 换句话说，需要建立 Azure AD 用户与 ADP eTime 中相关用户之间的链接关系。  

通过将 Azure AD 中的 **user name** 值分配为 ADP eTime 中的 **Username** 值，建立此链接关系。

若要配置和测试 ADP eTime 的 Azure AD SSO，需要完成以下构建基块：
...
ADP eTime 应用程序需要特定格式的 SAML 断言，这要求将自定义属性映射添加到 SAML 令牌属性配置。 以下屏幕截图显示一个示例。 声明名称将始终是**“PersonImmutableID”**和映射到包含用户 EmployeeID 的 ExtensionAttribute2 的值。 

将用户从 Azure AD 映射到 ADP eTime 在此是在 EmployeeID 上完成，但可将其映射到同样基于应用程序设置的其他值。 所以，首先请与 ADP eTime 团队合作，使用用户的正确标识符，并将该值与**“PersonImmutableID”**声明一起映射。  
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
  ...
  1. 在“回复 URL”文本框中，使用以下模式键入用户用于登录 ADP eTime 应用程序的 URL：`https://<server name>.adp.com/affwebservices/public/saml2assertionconsumer`。
  2. 单击“资源组名称” 的 Azure 数据工厂。
4. 在“配置 ADP eTime 的单一登录”页面上，执行以下步骤：
...
6. 若要在 Azure AD 中配置 RelayState 值，请执行以下步骤： 
 ...
 9. 在“中继状态”文本框中，使用以下模式键入值，然后保存设置： 
...
### <a name="create-a-adp-etime-test-user"></a>创建 ADP eTime 测试用户
...
本部分旨在通过授予 Britta Simon 访问 ADP eTime 的权限，允许她使用 Azure SSO。
