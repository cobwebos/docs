...
要创建启用了系统分配的托管标识的 Azure VM，需要创建 VM 并检索访问令牌，使用 CURL 通过系统分配的托管标识类型值对资源管理器终结点进行调用。
...
要在现有 VM 上禁用系统分配的托管标识，需要获取访问令牌，然后使用 CURL 对资源管理器 REST 终结点进行调用以将标识类型更新为 `None`。
