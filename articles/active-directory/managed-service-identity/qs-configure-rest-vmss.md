...
### <a name="enable-system-assigned-managed-identity-on-a-existing-virtual-machine-scale-set"></a>在现有虚拟机规模集上启用系统分配的托管标识
...
3. 要在具有现有用户分配的托管标识的虚拟机规模集上启用系统分配的托管标识，需要将 `SystemAssigned` 添加到 `type` 值。  
   ...
   API 版本 `2018-06-01` 以字典格式将用户分配的托管标识存储在 `userAssignedIdentities` 值中，而 API 版本 `2017-12-01` 及早期版本以数组格式将其存储在 `identityIds` 值中。
...
3. 如果使用的是 **API 版本 2018-06-01**，若要从具有用户分配的托管标识的虚拟机规模集中删除系统分配的托管标识，请从 `{"identity":{"type:" "}}` 值中删除 `SystemAssigned`，同时保留 `UserAssigned` 值和 `userAssignedIdentities` 字典值。 如果使用的是 **API 版本 2017-12-01** 或早期版本，请保留 `identityIds` 数组。
