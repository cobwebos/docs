<properties
   pageTitle="教程：将 Salesforce 与 Azure Active Directory 集成 | Microsoft Azure"
...
2. 你在 [Salesforce.com](https://www.salesforce.com/) 中必须拥有有效的租户。

> [AZURE.IMPORTANT]如果你使用 Salesforce.com **试用**帐户，将无法配置自动化的用户预配。试用帐户没有所需的 API 访问权限，而只有在付费之后才拥有这种权限。
> 
> 你可以使用[免费的开发人员帐户](https://developer.salesforce.com/signup)克服这项限制，以完成本教程。

如果你使用 Salesforce 沙箱环境，请参阅 [Salesforce 沙箱集成教程](https://msdn.microsoft.com/zh-cn/library/azure/dn798671.aspx?f=255&MSPPError=-2147217396)。


##步骤 1：将 Salesforce 添加到你的目录

1. 在 [Azure 管理门户](https://manage.windowsazure.cn)的左侧导航窗格中，单击“Active Directory”。

	![从左侧导航窗格选择“Active Directory”。][0]

2. 从“目录”列表选择想要将 Salesforce 添加到的目标目录。
...
6. 在**搜索框**中键入 **Salesforce**。然后从结果中选择“Salesforce”，并单击“完成”以添加该应用程序。
...
7. 在“管理员”导航窗格下，单击“安全控件”以展开相关的部分。然后单击“单一登录设置”。
...
8. 在“单一登录设置”页上，单击“编辑”按钮。
	...
	> [AZURE.NOTE] 如果无法为 Salesforce 帐户启用单一登录设置，你可能需要联系 Salesforce 支持人员，让他们为你启用该功能。
...
11. 在“SAML 单一登录设置编辑”页上进行以下配置：
 ...
 - 在 Azure AD 中，复制“颁发者 URL”值，然后将它粘贴到 Salesforce 中的“颁发者”字段。

 - 在“实体 ID”文本框中，使用以下模式输入 Salesforce 域名：
     - 企业帐户：`https://<domain>.my.salesforce.com`
     - 开发人员帐户：`https://<domain>-dev-ed.my.salesforce.com`

 - 单击“浏览”或“选择文件”打开“选择要上载的文件”对话框，选择你的 Salesforce 证书，然后单击“打开”以上载该证书。

 - 对于“SAML 标识类型”，请选择“断言包含用户的 salesforce.com 用户名”。

 - 对于“SAML 标识位置”，请选择“标识位于 Subject 语句的 NameIdentifier 元素中”

 - 在 Azure 中，复制“远程登录 URL”值，然后将它粘贴到 Salesforce 中的“标识提供者登录 URL”字段。
...
14. 在“身份验证服务”部分中，选择 SAML SSO 配置的友好名称，然后单击“保存”。

	![选择你的 SSO 配置][17]

	> [AZURE.NOTE] 如果选择了多个身份验证服务，则当用户尝试向 Salesforce 环境发起单一登录时，系统将提示他们选择登录时要使用的身份验证服务。如果你不希望发生这种情况，应该**将其他所有身份验证服务保持未选中状态**。
...
2. 在“配置用户预配”对话框中，键入 Salesforce 管理员用户名和密码。
	...
	> [AZURE.NOTE] 如果正配置生产环境，最佳做法是专门为此步骤在 Salesforce 中创建新的管理员帐户。必须在 Salesforce 中为此帐户分配“系统管理员”配置文件。
...
7. 复制令牌，转到 Azure AD 窗口，然后将令牌粘贴到“用户安全令牌”字段中。然后，单击“下一步”。
...
3. 选择测试用户，然后单击屏幕底部的“分配”按钮：
 ...
 - 如果你已启用自动化用户预配，系统会提示你定义用户应具有的 Salesforce 配置文件类型。新预配的用户应在几分钟后显示在 Salesforce 环境中。
		...
		> [AZURE.IMPORTANT] 如果要预配为 Salesforce **开发人员**环境，那么每个配置文件的许可证数量将会非常有限。因此，最好为用户预配 **Chatter Free 用户**配置文件，该配置文件有 4,999 个可用的许可证。
