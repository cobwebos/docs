...
* 可以让用户使用其 Azure AD 帐户自动登录到 SkyDesk Email 单一登录 (SSO)
...
* Azure AD 订阅
* 已启用 SkyDesk Email 单一登录 (SSO) 的订阅
...
## <a name="add-skydesk-email-from-the-gallery"></a>从库中添加 SkyDesk Email
...
本部分的目的是说明如何基于名为“Britta Simon”的测试用户配置和测试 SkyDesk Email 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 SkyDesk Email 用户。 换句话说，需要在 Azure AD 用户与 SkyDesk Email 中相关用户之间建立链接关系。

通过将 Azure AD 中“用户名”的值指定为 SkyDesk Email 中“用户名”的值来建立此链接关系。

若要配置和测试 SkyDesk Email 的 Azure AD SSO，需要完成以下构建基块：
...
本部分的目的是在 Azure 经典门户中启用 Azure AD SSO 并在 SkyDesk Email 应用程序中配置单一登录。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
   ...
   1. 在“登录 URL”文本框中，使用以下模式键入用户用于登录 SkyDesk Email 应用程序的 URL：**“https://mail.skydesk.jp/portal/\<公司名称\>”**。
   2. 单击“下一步”。

4. 在“配置 SkyDesk Email 的单一登录”页上，执行以下步骤：
...
5. 若要在 **SkyDesk Email** 中启用 SSO，请执行以下步骤：
  1. 以管理员身份登录 SkyDesk Email 帐户。
  2. 在顶部菜单中，单击“设置”，然后选择“组织”。 
...
6. 在“SAML 身份验证”对话框页上，执行以下步骤：
   ...
   1. 在 Azure AD 经典门户中，复制“SAML SSO URL”值，然后将其粘贴到“登录 URL”文本框中。
   2. 在 Azure AD 经典门户中，复制“单一注销服务 URL”值，然后将其粘贴到“注销 URL”文本框中。
   3. **更改密码 URL** 是可选项，因此将其留空。
   4. 单击“从文件获取密钥”，选择已下载的 SkyDesk Email 证书，然后单击“打开”上载证书。
   5. 选择“RSA”作为**算法**。
   6. 单击“确定”以保存更改。
...
### <a name="create-a-skydesk-email-test-user"></a>创建 SkyDesk Email 测试用户
...
>[!NOTE] 
>如果需要批量创建用户，则需要联系 SkyDesk Email 支持团队。
>
