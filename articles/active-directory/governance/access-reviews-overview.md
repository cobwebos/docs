...
- **要维护策略的例外列表：** 在理想情况下，所有用户将都按照访问策略来保护组织的资源的访问权限。 但是，有时，某些业务案例要求例外处理。 IT 管理员可以管理此任务、避免忽视策略例外情况，为审核员提供定期评审这些例外情况的证明。
...
每个交互使用访问评审的用户必须具有付费的 Azure AD Premium P2 许可证。 示例包括：
...
还可以要求来宾用户评审自己的访问权限。 对于每个付费的 Azure AD Premium P2 许可证分配给你自己组织的用户之一，可以使用 Azure AD 企业到企业 (B2B) 来邀请外部用户限额下的最多 5 名来宾用户。 这些来宾用户也可以使用 Azure AD Premium P2 功能。 有关详细信息，请参阅[Azure AD B2B 协作许可指南](../b2b/licensing-guidance.md)。

以下是一些示例方案，可帮助您确定必须具有的许可证数量。

| 场景 | 计算 | 所需许可证数 |
| --- | --- | --- |
| 管理员使用 500 个用户创建组 A 的访问评审。<br/>将分配为审阅者的 3 组所有者。 | 一名管理员 + 3 组所有者 | 4 |
| 管理员使用 500 个用户创建组 A 的访问评审。<br/>可以自我审查。 | 一名管理员 + 500 个用户为自我审阅者 | 501 |
| 管理员使用 5 个用户和 25 个来宾用户创建组 A 的访问评审。<br/>可以自我审查。 | 一名管理员 + 5 个用户为自我审阅者<br/>（需要的 1:5 比率介绍来宾用户） | 6 |
| 管理员使用 5 个用户和 28 来宾用户创建组 A 的访问评审。<br/>可以自我审查。 | 一名管理员 + 5 个用户自我审阅者为 + 1 个用户以覆盖来宾用户以所需的 1:5 的比例 | 7 |
