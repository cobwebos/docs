...
1. 以全局管理员或用户管理员身份登录到要在其中使用访问评审的 [Azure 门户](https://portal.azure.com)。
...
| 方案 | 计算 | 所需许可证数 |
| --- | --- | --- |
| 管理员创建具有 500 个用户的组 A 的访问评审。 分配 3 个组所有者作为评审者。 | 1 个许可证用于管理员 + 3 个许可证用于每位充当审阅者的组所有者。 | 4 |
| 管理员创建具有 500 个用户的组 A 的访问评审。 将其设为自我评审。 | 1 个许可证用于管理员 + 500 个许可证用于每位充当自我审阅者的用户。 | 501 |
| 管理员创建具有 5 个用户和 25 个来宾用户的 B 组的访问评审。 将其设为自我评审。 | 1 个许可证用于管理员 + 5 个许可证用于每位充当自我审阅者的用户。<br/>（按所需的 1:5 比例包含来宾用户） | 6 |
| 管理员创建具有 5 个用户和 108 个来宾用户的 C 组的访问评审。 将其设为自我评审。 | 1 个许可证用于管理员 + 5 个许可证用于每位充当自我审阅者的用户+ 16 个额外的许可证用于涵盖所有 108 位来宾用户，按所要求的 1:5 的比例。<br/>1+5=6 个许可证，涵盖 5\*6=30 位来宾用户。 对于余下的 (108-5\*6)=78 位来宾用户，需要 78/5=16 个额外的许可证。 因此，总共需要 6+16=22 个许可证。 | 22 |
