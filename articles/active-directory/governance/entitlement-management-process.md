...
| 拒绝 | 审批者拒绝请求。 |
| 已批准 | 审批者批准请求。 |
| 传送 | **尚未为**用户分配对访问包中所有资源的访问权限。 如果这是外部用户，则用户可能尚未访问资源目录。 它们还可能未接受同意提示。 |
...
### <a name="approved-or-denied-emails"></a>批准或拒绝的电子邮件
 ...
 当审批者收到请求者提交的访问请求时，他们可以批准或拒绝该访问请求。 审批者需要为决策添加业务理由。 下面是批准请求后发送给审批者和备用审批者的电子邮件示例：
