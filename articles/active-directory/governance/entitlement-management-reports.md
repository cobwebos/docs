---
title: 在授权管理中查看&日志的报告 - Azure AD
description: 了解如何在 Azure 活动目录授权管理中查看用户分配报告和审核日志。
...
---
# <a name="view-reports-and-logs-in-azure-ad-entitlement-management"></a>在 Azure AD 授权管理中查看报表和日志

Azure AD 授权管理报告和 Azure AD 审核日志提供有关用户可以访问哪些资源的其他详细信息。 作为管理员，您可以查看用户的访问包和资源分配，并查看请求日志以进行审核或确定用户请求的状态。 本文介绍如何使用授权管理报告和 Azure AD 审核日志。

观看以下视频，了解如何查看用户在授权管理中有权访问的资源：

>[!VIDEO https://www.youtube.com/embed/omtNJ7ySjS0]

## <a name="view-access-packages-for-a-user"></a>查看用户的访问包

此报表使您能够列出用户可以请求的所有访问包以及当前分配给该用户的访问包。
...
1. 在列表中查找用户，然后单击"**选择**"。
    ...
    **"可以请求"** 选项卡显示用户可以请求的访问包的列表。 此列表由为访问包定义[的请求策略](entitlement-management-access-package-request-policy.md#for-users-in-your-directory)确定。 

    ![用户访问包](./media/entitlement-management-reports/access-packages-report.png)

1. 如果访问包有多个资源角色或策略，请单击资源角色或策略条目以查看选择详细信息。

1. 单击"**已分配"** 选项卡以查看当前分配给用户的访问包的列表。 将访问包分配给用户时，这意味着用户可以访问访问包中的所有资源角色。

## <a name="view-resource-assignments-for-a-user"></a>查看用户的资源分配

此报表使您能够列出当前分配给授权管理中的用户的资源。 请注意，此报告适用于使用授权管理管理的资源。 用户可能有权访问您目录中的授权管理之外的其他资源。
...
1. 在列表中查找用户，然后单击"**选择**"。
    ...
    将显示当前分配给用户的资源的列表。 该列表还显示了他们从中获取的资源角色的访问包和策略，以及访问的开始日期和结束日期。
    
    如果用户在两个或多个包中访问同一资源，则可以单击箭头以查看每个包和策略。
...
- [存档报告和日志](entitlement-management-logs-and-reporting.md)
