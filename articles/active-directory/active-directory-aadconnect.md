...
当今的用户希望在本地和云中都能访问应用程序。他们希望使用任何设备都能进行这种访问，包括笔记本电脑、智能手机或者平板电脑。为了实现这个目的，你与所在的组织需要设法让用户能够访问这些应用，然而，完全迁移到云中并不一定可行。

<center>![什么是 Azure AD Connect](./media/active-directory-aadconnect/arch.png)</center>

随着 Azure Active Directory Connect 的推出，访问应用和迁移到云的操作变得比以往容易得多。Azure AD Connect 具有以下优点：

- 无论是云中还是本地，用户都可以使用一个通用标识登录。他们无需记住多个密码或帐户，管理员也不必担心多个帐户可能带来额外的管理开销。
- 使用这一个工具，你就可以根据引导将本地目录连接到 Azure Active Directory。安装后，向导将部署并配置实现目录集成所需的所有组件，包括同步服务、密码同步或 AD FS，以及 Azure AD PowerShell 模块等必备组件。
...
Azure AD Connect 可以让你轻松实现这种集成，它还能够简化本地和云标识基础结构的管理工作。
...
若要开始使用 Azure AD Connect，可以通过以下链接下载最新版本：[下载 Azure AD Connect](http://go.microsoft.com/fwlink/?LinkId=615771)
...
- 同步 - 此部分由以前作为 DirSync 和 AAD Sync 发布的组件和功能组成。此部分负责创建用户和组。它还负责确保本地环境中有关用户和组的信息与云匹配。
- AD FS - 这是 Azure AD Connect 的可选部分，可用于使用本地 AD FS 基础结构设置混合环境。组织可以使用此部分来解决复杂的部署，包括域加入 SSO、实施 AD 登录策略和智能卡或者第三方 MFA 等方案。有关配置 SSO 的更多信息，请参阅[使用单一登录的 DirSync](https://msdn.microsoft.com/library/azure/dn441213.aspx)。
- 运行状况监视 - 对于使用 AD FS 的复杂部署，Azure AD Connect Health 能够密切监视联合服务器，并且让你可以在 Azure 门户的中心位置查看此活动。有关更多信息，请参阅 [Azure Active Directory Connect Health](https://msdn.microsoft.com/library/azure/dn906722.aspx)。


### Azure AD Connect 支持组件

下面列出了一些必备组件和支持组件，在你要设置 Azure AD Connect 的服务器上，Azure AD Connect 将安装下列组件。此列表用于基本快速安装。如果你在“安装同步服务”页上选择使用其他 SQL Server，则不会安装下列 SQL Server 2012 组件。
...
- Azure 订阅或 [Azure 试用版订阅](http://azure.microsoft.com/pricing/free-trial/) - 这只是用来访问 Azure 门户，而不是用于 Azure AD Connect。如果你使用的是 PowerShell 或 Office 365，则无需 Azure 订阅即可使用 Azure AD Connect。
- 要为将要集成的 Azure AD 租户准备 Azure AD 全局管理员帐户
- Azure AD Connect 必须安装在 Windows Server 2008 或更高版本上。此服务器可以是域控制器或者成员服务器。
- AD 架构版本与林级别必须是 Windows Server 2003 或更高版本。只要符合架构和林级别的要求，域控制器就能运行任何版本。
- 如果你部署的是 Active Directory 联合身份验证服务，则必须在 Windows Server 2012 R2 或更高版本上安装 AD FS。
- Azure AD Connect 要求使用 SQL Server 数据库来存储标识数据。默认情况下，将会安装 SQL Server 2012 Express LocalDB（轻量版 SQL Server Express），并在本地计算机上创建服务的服务帐户。SQL Server Express 有 10 GB 的大小限制，你可以管理大约 100000 个对象。
- 如果你要管理更多目录对象，则需要将安装过程指向其他版本的 SQL Server。Azure AD Connect 支持从 SQL Server 2008（装有 SP4）到 SQL Server 2014 的各种 Microsoft SQL Server。
- 本地 Active Directory 的企业管理员帐户
- 如果你使用的是出站代理，则必须在 **C:\\Windows\\Microsoft.NET\\Framework64\\v4.0.30319\\Config\\machine.config** 文件中添加以下设置，才能完成安装。<code>
...
“快速设置”是默认选项，也是最常用的设置方案之一。如果使用此方案，Azure AD Connect 在部署同步设置时，将使用密码哈希同步选项。这适用于单一林，它允许你的用户使用本地密码登录云。如果使用“快速设置”，在完成安装之后，将自动开始进行同步；不过你可以选择不在此时同步。使用此选项时，只需单击几下鼠标就能将本地目录扩展到云中。
...
1. 以企业管理员身份登录到要安装 Azure AD Connect 的服务器。这个服务器应该就是以后的同步服务器。
...
9. 在“已准备好配置”屏幕上，单击“安装”。
	...
	- 此外，你也可以通过选中对应的复选框，针对“Exchange 混合部署”配置同步服务。如果你不打算在云中和本地同时配置 Exchange 邮箱，则无需这样做。
...
下表概述了这些任务并提供了各个任务的简要描述。

<center>![联接规则](./media/active-directory-aadconnect-whats-next/addtasks.png) </center>
...
- [更改 Azure AD Connect 的默认配置](active-directory-aadconnect-whats-next-change-default-config)
- [使用 Azure AD Connect 同步规则编辑器](active-directory-aadconnect-whats-next-synch-rules-editor)
- [使用声明性设置](active-directory-aadconnect-whats-next-declarative-prov)

另外，为 Azure AD Sync 创建的某些文档仍有参考价值，并适用于 Azure AD Connect。尽管我们正在努力将这些文档发布到 Azure.com，但有些文档目前仍然只能在 MSDN 范围的库中提供。有关其他文档，请参阅 [MSDN 上的 Azure AD Connect](active-directory-aadconnect) 和 [MSDN 上的 Azure AD Sync](https://msdn.microsoft.com/library/azure/dn790204.aspx)。
