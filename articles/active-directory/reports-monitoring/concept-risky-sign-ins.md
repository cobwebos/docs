---
title: 门户中的 "有风险的登录" 报表 |Microsoft Docs
...
- **User** -在登录操作过程中使用的用户的名称。
- **Ip** -用于连接到 Azure Active Directory 的设备的 ip 地址。
- **Location** -用于连接到 Azure Active Directory 的位置。 这是基于跟踪、注册表数据、反向查询和其他信息的最佳近似值。
- **登录时间** - 进行登录的时间
- **状态** - 登录的状态
...
- 有关检测到的[风险检测类型](concept-risk-events.md)的聚合信息。 使用**Azure AD Premium P1 edition**时，许可证未涵盖的检测将显示为**检测到其他风险**检测的登录。 对于**Azure AD Premium P2 版本**，你可以获取有关所有基础检测的最详细信息。
