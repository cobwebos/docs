...
Azure Active Directory (Azure AD) 可以检测到与用户帐户相关的可疑操作。 对于每个检测到的操作，都会创建一个称为**风险检测**的记录。 有关详细信息，请参阅[Azure AD 风险检测](concept-risk-events.md)。 
...
- 在**Azure 活动目录免费版本中**，您将获得风险登录的列表。 

- 此外 **，Azure 活动目录高级 1**版允许您检查已检测到每个报表的某些基本风险检测。 

- **Azure Active Directory 高级版**为您提供了有关所有潜在风险检测的最详细信息，还使您能够配置自动响应配置的风险级别的安全策略。

## <a name="risky-sign-ins-report-for-azure-ad-free-edition"></a>Azure AD 免费版的有风险登录报告

Azure AD 免费版为您提供为用户检测到的危险登录的列表。 每个记录包含以下属性：

- **用户**- 登录操作期间使用的用户的名称。
- **IP** - 用于连接到 Azure 活动目录的设备的 IP 地址。
- **位置**- 用于连接到 Azure 活动目录的位置。 这是基于跟踪、注册表数据、反向查询和其他信息的最佳近似值。
- **登录时间** - 进行登录的时间
- **状态** - 登录的状态
...
- 有关已检测到[的风险检测类型的](concept-risk-events.md)聚合信息。 使用**Azure AD 高级 P1 版本**，许可证未涵盖的检测显示为风险检测**登录，并检测到其他风险**。 使用**Azure AD 高级 P2 版本**，您可以获取有关所有基础检测的最详细信息。

- 一个用于下载报表的选项

![有风险的登录](./media/concept-risky-sign-ins/456.png)

选择风险检测时，您将获得此风险检测的详细报告视图，使您能够：
...
要调查风险检测，请从列表中选择一个。  
这将打开此风险检测**的详细信息**边栏选项卡。 在 **"详细信息"** 边栏选项卡上，您可以选择手动关闭风险检测或重新激活手动关闭的风险检测。 
