...
| 由于大型数据集，刷新可能会失败。 | 目前，使用 Power BI 的 Azure AD 内容包可以支持仅小型数据集 （小于 500,00 行） 由于 Power BI 服务中的超时方面的一些限制。 如果您遇到的限制错误或者刷新由于超时问题而失败，这可能是由于尝试提取大型数据集。 减少时间段，在查询中的，然后重试。|
