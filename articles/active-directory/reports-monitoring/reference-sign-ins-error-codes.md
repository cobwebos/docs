...
|50078|提供的多因素身份验证已过期，你必须刷新多重身份验证才能访问。|
...
|50181|登录期间与 OTP 相关的故障。 |
|50194|应用程序 "{appId}" （{appName}）未配置为多租户应用程序。 对于在 "{time}" 之后创建的此类应用程序，不支持使用/common 终结点。 使用特定于租户的终结点，或将应用程序配置为多租户。|
...
|90094| 应用已请求登录用户不允许许可的权限，但用户被阻止。 |
|90095| 应用程序请求了不允许已登录用户同意的权限，并且用户显示了[管理员许可请求](../manage-apps/configure-admin-consent-workflow.md)窗体。 |
|130500|由于用户凭据策略，手机登录被阻止。|
|500011| 在名为 <tenant ID>的租户中找不到名为 <site address> 的资源主体。 如果应用程序尚未由租户管理员安装，或者尚未获得租户中的任何用户同意，则可能会发生这种情况。 可能将身份验证请求发送给了错误的租户。|
|500014|资源 "{identifier}" 已禁用。|
|500021| 租户受公司代理限制。 拒绝资源访问权限。|
|500121| 在强身份验证请求期间，身份验证失败。|
|500133| 断言不在其有效时间范围内。 确保访问令牌在用于用户断言或请求新令牌之前没有过期。|
|500172|"{Issuer}" 颁发的证书 "{name}" 无效。 当前时间： "{curTime}"。 证书 NotBefore： "{startTime}"。 证书 NotAfter： "{endTime}"。|
|501291|客户端应用程序为 Mam 应用，设备未注册，并使用 broker 发送请求。 需要完成工作位置联接才能访问应用程序。|
|530003|需要对设备进行管理才能访问此资源。|
...
|700051|没有为应用程序启用 Response_type "token"。 由于以下原因，应用程序请求了不受支持的响应类型：未为应用程序启用 response_type "token"。 应用程序所有者应进入 Azure 门户或调用 MS Graph 以启用隐式访问令牌授予。|
|900432|跨云请求不支持机密客户端。|
