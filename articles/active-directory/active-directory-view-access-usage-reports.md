---
title: "查看访问和使用情况报告 | Microsoft Docs"
...
> [!NOTE]
> * 仅当已启用 [Azure Active Directory Premium](active-directory-get-started-premium.md) 时，一些高级异常报告和资源使用情况报告才可用。 高级报告可帮助你提高访问安全性、应对潜在威胁以及获得对设备访问和应用程序使用情况进行分析。
...
| --- | --- |
| **异常活动报告** | |
| [来自未知源的登录](active-directory-reporting-sign-ins-from-unknown-sources.md) |可能表示未受跟踪的登录尝试。 |
| [多次失败后的登录](active-directory-reporting-sign-ins-after-multiple-failures.md) |可能表示成功的暴力破解攻击。 |
| [来自多个地理区域的登录](active-directory-reporting-sign-ins-from-multiple-geographies.md) |可能表示多个用户正在使用同一帐户登录。 |
| [来自具有可疑活动的 IP 地址的登录](active-directory-reporting-sign-ins-from-ip-addresses-with-suspicious-activity.md) |可能表示在经过持续入侵尝试后成功登录。 |
| [来自可能受感染的设备的登录](active-directory-reporting-sign-ins-from-possibly-infected-devices.md) |可能表示从已受感染的设备尝试登录。 |
| [异常的登录活动](active-directory-reporting-irregular-sign-in-activity.md) |可能表示对用户登录模式而言异常的事件。 |
| [具有异常登录活动的用户](active-directory-reporting-users-with-anomalous-sign-in-activity.md) |表示用户的帐户受到威胁。 |
...
<li>[异常的登录活动](active-directory-reporting-irregular-sign-in-activity.md)。</li>
...
| 登录 [Azure 管理门户](https://manage.windowsazure.com/) |是 |
| 登录 [Microsoft Azure 门户](https://portal.azure.com/) |是 |
...
> 注意：为了减少此报告中的干扰信息量，将不会显示 [Microsoft Online Services 登录助手](http://community.office365.com/en-us/w/sso/534.aspx)进行的登录。
> 
> 
...
2. 在“报告”页面上，单击你要查看和/或下载的报告。
   ...
   > [!NOTE]
   > 如果这是你第一次使用 Azure Active Directory 的报告功能，你将看到一条“选择加入”的消息。 如果同意，请单击复选标记图标以继续。
   > 
   > 
