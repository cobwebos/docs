...
* 可以让用户使用其 Azure AD 帐户自动登录到 RunMyProcess 单一登录 (SSO)
...
若要配置和测试 RunMyProcess 的 Azure AD SSO，需要完成以下构建基块：
...
在本部分中，将在经典门户中启用 Azure AD 单一登录并在 RunMyProcess 应用程序中配置 SSO。

**若要配置 RunMyProcess 的 Azure AD SSO，请执行以下步骤：**
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
  ...
  1. 在“登录 URL”文本框中，使用以下模式键入 URL：`https://live.runmyprocess.com/live/<tenant id>`。 
  2. 单击“下一步”。
    >[!NOTE]
...
7. 转到“身份验证方法”部分，并执行以下步骤：
  ...
  1. 对于“方法”，选择“使用 Samlv2 的 SSO”。 
  2. 在“SSO 重定向”文本框中放置 Azure AD 应用程序配置向导中 **SAML SSO URL** 的值。
  3. 在“注销重定向”文本框中放置 Azure AD 应用程序配置向导中**单一注销服务 URL** 的值。
  4. 在“名称 ID 格式”文本框中放置 Azure AD 应用程序配置向导中**名称标识符格式**的值。
  5. 复制已下载的证书文件的内容，然后将其粘贴到“证书”文本框中。 
  6. 单击“保存”图标。
...
6. 在“用户配置文件”对话框页上，执行以下步骤：
   ...
   ![创建 Azure AD 测试用户](./media/active-directory-saas-runmyprocess-tutorial/create_aaduser_05.png)  
...
### <a name="create-a-runmyprocess-test-user"></a>创建 RunMyProcess 测试用户
...
3. 在“用户设置”部分中执行以下步骤：
 ...
 1. 在相关文本框中键入要预配的有效 AAD 帐户的“名称”和“电子邮件”。 
 2. 选择 **IDE 语言**、**语言**和**配置文件**。 
 3. 选择“将帐户创建电子邮件发送给我”。 
 4. 单击“保存” 。
...
本部分旨在通过授予 Britta Simon 访问 RunMyProcess 的权限，允许她使用 Azure SSO。
