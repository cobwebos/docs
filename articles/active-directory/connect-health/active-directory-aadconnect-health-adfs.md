...
---
# <a name="using-azure-ad-connect-health-with-ad-fs"></a>在 AD FS 中使用 Azure AD Connect Health
...
若要选择其他指标、指定时间范围或更改分组，请右键单击使用情况分析图表并选择“编辑图表”。 然后可以指定时间范围、选择不同的指标以及更改分组。 可以查看基于不同“指标”的身份验证流量分布，并使用下表中所述的相关“分组依据”参数对每个指标进行分组。

| 度量值 | 分组依据 | 分组意味着什么，它为什么很有用？ |
| --- | --- | --- |
| 请求总数：联合身份验证服务所处理的请求总数 |全部 |显示未分组的请求总数。 |
...
| 用户计数：系统中处于活动状态的唯一用户数的平均数 |全部 |此指标提供所选时间段内使用联合身份验证服务的用户平均数。 不对用户进行分组。 <br>平均值取决于所选的时间段。 |
...
通过选择边栏选项卡顶部的“筛选器”选项，你可以按服务器进行筛选，以查看单个服务器的度量值。 若要更改指标，请右键单击监视边栏选项卡下的监视图表，然后选择“编辑图表”。 然后在打开的新边栏选项卡中，你可以从下拉列表中选择其他度量值，并指定查看性能数据的时间范围。
