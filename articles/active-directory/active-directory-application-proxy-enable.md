...
本文将指导你如何在 Azure AD 中为云目录启用 Microsoft Azure AD 应用程序代理。此过程涉及到在你的专用网络中安装应用程序代理连接器，它会使你的网络保持连接到代理服务。你还要使用 Microsoft Azure AD 租户订阅注册连接器。如果你不了解应用程序代理服务器可以做些什么，可了解有关[如何提供对安全远程访问本地应用程序](active-directory-application-proxy-get-started.md)的详细信息。

在完成启用 Azure AD 应用程序代理的演练后，你就可以准备发布用于远程访问的本地应用了。
...
- 可以安装应用程序代理连接器，运行 Windows Server 2012 R2 或 Windows 8.1 或更高版本的服务器。该服务器将 HTTPS 请求发送到云中的应用程序代理服务，需要与你打算发布的应用程序建立 HTTPS 连接。
- 如果防火墙位于路径中，请确保防火墙已开启，连接器能向应用程序代理发出 HTTPS (TCP) 请求。连接器将连同属于高级域的子域一起使用这些端口： *msappproxy.net* 和 *servicebus.windows.net*。确保打开以下用于**出站**流量的**所有**端口：
...
3. 从目录页上选择“配置”，并向下滚动至“应用程序代理”。
4. 将“为此目录启用应用程序代理服务”切换至“启用”状态。

	![启用应用程序代理](./media/active-directory-application-proxy-enable/app_proxy_enable.png)

5. 选择“立即下载”。你将转到“Azure AD 应用程序代理连接器下载”。阅读并接受许可条款，然后单击“下载”为应用程序代理连接器保存 Windows Installer 文件 (.exe)。

## 步骤 2： 安装并注册连接器
1. 在根据此前的先决条件准备的服务器上运行 *AADApplicationProxyConnectorInstaller.exe*。
2. 按照向导中的说明进行安装。
...
4. 安装完成后，服务器上会添加两个新的服务，如下所示。

 	- **Microsoft AAD 应用程序代理连接器**将会启用连接
	- **Microsoft AAD 应用程序代理连接器**是一项自动更新服务，它会定期检查连接器新版本并根据需要更新连接器。
...
为了获得高可用性，应该至少部署一个额外的连接器。若要部署更多连接器，请重复上面的步骤 2 和 3。每个连接器必须分开注册。
