...
| 什么是应用锁定功能，它如何帮助我更加安全？ | 若要使你的一次性密码、应用信息和应用设置更加安全，可以在 Microsoft Authenticator 应用中开启应用锁定功能。 从 Microsoft 身份验证器应用的 **"设置"** 屏幕打开应用锁定，这意味着每次打开 Microsoft 身份验证器应用时，系统都会要求您使用 PIN 或生物识别进行身份验证。 此功能提供了额外的保护，您在 Microsoft 身份验证器应用中批准通知的方式不会更改。<br><br>**注意**<br>由于设备注册可能发生在 Microsoft 身份验证器应用之外的其他位置（如公司门户应用中或 Android 帐户设置中）中，因此不能保证应用锁定会阻止用户访问 Microsoft 身份验证器应用。 |
...
| 我的一次性密码 （OTP） 代码不起作用。   应采取何种操作？ | 确保设备上的日期和时间正确且正在自动同步。 如果日期和时间错误或不同步，代码将无法工作。 |
