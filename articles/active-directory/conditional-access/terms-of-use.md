...
有关其他视频，请参阅：
- [如何在 Azure 活动目录中部署使用条款](https://www.youtube.com/embed/N4vgqHO2tgY)
- [如何在 Azure 活动目录中推出使用条款](https://www.youtube.com/embed/t_hA4y9luCY)
...
- 要求员工在 Azure AD 自助服务密码重置 （SSPR） 中注册安全信息之前接受您的使用条款。
- 显示针对组织中所有用户的常规使用条款。
...
- 使用[Microsoft 图形 API（](https://developer.microsoft.com/graph/docs/api-reference/beta/resources/agreement)当前预览版）创建和管理使用条款。
...
1. 对于**使用条款文档**，浏览到您最终确定的使用条款 PDF 并选择它。
1. 选择使用条款文档的语言。 可以通过语言选项上传多个版本的使用条款，每个版本的语言各不相同。 最终用户看到的使用条款版本取决于其浏览器首选项。
1. 要要求最终用户在接受之前查看使用条款，请使用 "**要求用户"将使用条款扩展到** **On**。
1. 要求最终用户接受您在从其访问的每个设备上的使用条款，请将 **"要求用户在每台设备上的同意"** 设置为**On**。 如果启用了此选项，用户可能需要安装其他应用程序。 有关详细信息，请参阅[每个设备的使用条款](#per-device-terms-of-use)。
1. 如果要在计划上过期使用条款同意，请使用 "**过期同意"** 设置为**On**。 设置为“打开”时，会显示另外两项计划设置。

   ![用于设置开始日期、频率和持续时间的“使同意状态过期”设置](./media/terms-of-use/expire-consents.png)

1. 使用 **"开始过期**"和 **"频率**"设置指定使用期限到期的计划。 下表显示了几项示例设置的结果：
...
1. 使用**重新接受要求（天）前的持续时间**设置指定用户必须重新接受使用条款前的天数。 这可以让用户遵照自己的计划。 例如，如果将持续时间设置为 **30** 天，则两个用户的计划如下：
...
1. 在 **"条件访问**"下，使用 **"使用条件访问强制"策略模板**列表来选择模板以强制执行使用条款。
   ...
   >条件访问策略控制（包括使用条款）不支持对服务帐户强制实施。 我们建议从条件访问策略中排除所有服务帐户。
    ...
    可以使用自定义条件访问策略将使用条款细化，向下细化到特定云应用程序或用户组。 有关详细信息，请参阅[快速入门：在访问云应用之前需要接受使用条款](require-tou.md)。
...
1. 有关使用条款，请单击 **"已接受**"或 **"已拒绝**"下的数字以查看用户的当前状态。
...
**通过要求用户在每台设备设置上同意**，您可以要求最终用户接受您在所访问的每个设备上的使用条款。 最终用户将需要在 Azure AD 中注册其设备。 注册设备时，设备 ID 用于强制执行每台设备的使用条款。
...
如果用户正在使用 iOS 设备，系统将提示他们安装 Microsoft[身份验证器应用](https://apps.apple.com/us/app/microsoft-authenticator/id983156458)。

### <a name="register-an-android-device"></a>注册 Android 设备

如果用户正在使用 Android 设备，系统将提示他们安装 Microsoft[身份验证器应用](https://play.google.com/store/apps/details?id=com.azure.authenticator)。
...
大多数组织都有一个流程，让员工同意其组织的使用条款和隐私声明。 但是，对于通过 SharePoint 或 Teams 添加的 Azure AD 企业到企业 (B2B) 来宾，如何强制要求他们同样表示同意？ 使用条件访问和使用条款，可以直接对 B2B 来宾用户强制实施策略。 在邀请兑换流程期间，向用户显示使用条款。 这种支持目前处于预览状态。

仅当用户在 Azure AD 中具有来宾帐户时，才显示使用条款。 SharePoint Online 目前具有[临时外部共享收件人体验](/sharepoint/what-s-new-in-sharing-in-targeted-release)，用于共享文档或不需要用户具有来宾帐户的文件夹。 在这种情况下，不会显示使用条款。

![用户和组窗格 - 包含选中所有来宾用户选项的选项卡](./media/terms-of-use/b2b-guests.png)
...
![选择 Microsoft Azure 信息保护应用的云应用窗格](./media/terms-of-use/cloud-app-info-protection.png)
...
> [!NOTE]
> [对于每台设备的使用条款](#per-device-terms-of-use)，不支持 Intune 注册应用。
...
**问：信息的存储时间为多长？**<br />
答：用户在使用条款报告中计数，接受/拒绝者在使用条款的生命周期内存储。 Azure AD 审核日志的存储时间为 30 天。

**问：为什么在使用条款报告与 Azure AD 审核日志不同时看到不同数量的同意？**<br />
答：使用条款报告存储为该使用条款的生存期，而 Azure AD 审核日志存储 30 天。 此外，使用条款报告仅显示用户当前的同意状态。 例如，如果用户拒绝然后接受，则使用条款报告仅显示该用户的接受。 如果需要查看历史记录，可以使用 Azure AD 审核日志。

**问：如果我编辑使用条款的详细信息，是否需要用户再次接受？**<br />
答：不，如果管理员编辑使用条款的详细信息（名称、显示名称、要求用户展开或添加语言），则不需要用户重新接受新术语。

**问：我可以更新现有的使用条款文档吗？**<br />
答：目前无法更新现有使用条款文档。 若要更改使用条款文档，必须创建新的使用条款实例。

**问：如果超链接在 PDF 文档的使用条款中，最终用户能否单击它们？**<br />
答：是的，最终用户可以选择指向其他页面的超链接，但不支持指向文档中部分的链接。
...
答：您可以使用现代身份验证在企业应用程序上创建条件访问策略。 有关详细信息，请参阅[企业应用程序](./../manage-apps/view-applications-portal.md)。

**问：是否可以向给定用户或应用添加多个使用条款？**<br />
答：是的，通过创建针对这些组或应用程序的多个条件访问策略。 如果用户处于多个使用条款的范围内，他们可一次接受一个使用条款。

**问：如果用户拒绝使用条款，会发生什么情况？**<br />
答：将阻止此用户访问该应用程序。 用户需要重新登录并接受条款才能获取访问权限。

**问：是否可以不接受以前接受的使用条款？**<br />
答：您可以[查看以前接受的使用条款](#how-users-can-review-their-terms-of-use)，但目前没有一种不接受的方法。

**问：如果我还使用 Intune 条款和条件，会发生什么情况？**<br />
答：如果已配置 Azure AD 使用条款和[Intune 条款和条件](/intune/terms-and-conditions-create)，则用户必须接受这两种条款和条件。 有关详细信息，请参阅[为组织博客文章选择合适的条款解决方案](https://go.microsoft.com/fwlink/?linkid=2010506&clcid=0x409)。

**问：使用服务条款用于身份验证的终结点是什么？**<br />
答：使用条款使用以下终结点进行身份验证：https://tokenprovider.termsofuse.identitygovernance.azure.com和https://account.activedirectory.windowsazure.com。 如果你的组织针对注册实施 URL 允许列表，则需要将这些终结点以及用于登录的 Azure AD 终结点添加到该允许列表。

## <a name="next-steps"></a>后续步骤

- [快速入门：在访问云应用之前要求接受使用条款](require-tou.md)
- [Azure 活动目录中的条件访问最佳做法](best-practices.md)
