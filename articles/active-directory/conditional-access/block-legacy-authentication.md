...
如何阻止使用旧身份验证的应用访问租户的资源？ 建议是只需使用条件性访问策略阻止它们。 如有必要，只允许某些用户和特定网络位置使用基于旧身份验证的应用程序。

第一个双因素身份验证完成后，会强制执行条件性访问策略。 因此，条件性访问不应作为用于方案，如拒绝服务 (DoS) 攻击，第一个行防御措施，但可以利用这些事件 （例如，登录风险级别，位置为请求中，依次类推） 从信号来确定访问权限。
...
在条件性访问策略中，可以设置绑定到用于访问你的资源的客户端应用的条件。 客户端应用条件使你可以通过为“移动应用和桌面客户端”选择“其他客户端”，将范围缩小到使用旧身份验证的应用程序   。
...
Azure 有一项安全功能，导致无法创建此类策略，因为此配置违反[最佳做法](best-practices.md)的条件性访问策略。
...
您可以选择为所有可用的授权控件**其他客户端**条件; 但是，最终用户体验始终是相同的-阻止访问。

如果阻止使用传统的身份验证**其他客户端**条件，您还可以设置的设备平台和位置条件。 例如，如果只想阻止移动设备的旧式身份验证，请通过选择以下项来设置**设备平台**条件：
...
- 如果您不熟悉尚未配置条件性访问策略，请参阅[使用 Azure Active Directory 条件性访问需要针对特定应用的 MFA](app-based-mfa.md)有关的示例。
