...
规划你的条件性访问部署是必须确保你的组织中实现应用程序和资源的所需的访问策略。 大部分您在规划阶段的部署，以设计各种策略授予或阻止对你的用户在你选择的条件下访问所需的时间。 本文档介绍应执行以实现安全、 高效的条件性访问策略的步骤。 在开始之前，请确保你已了解如何[条件性访问](overview.md)工作原理以及何时应使用它。
...
现在，可以决定策略的命名标准。 命名标准有助于查找策略及了解其用途，而无需在 Azure 管理门户中将其打开。 若要显示策略命名：
...
例如，以下名称指示此策略是带四个的 MFA 中断时是否应启用的第一个策略：
...
必须确保测试策略的排除条件。 例如，你可能从要求执行 MFA 的策略中排除了某个用户或组。 测试如果排除的用户提示进行 MFA，因为其他策略的组合可能会要求对这些用户进行 MFA。
