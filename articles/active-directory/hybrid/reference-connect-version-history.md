...
>[!IMPORTANT]
>注册为混合 Azure AD 联接的 Windows 计算机在 Azure AD 中表示为设备对象。 这些设备对象可用于条件性访问。 Windows 10 计算机通过 Azure AD Connect 同步到云，下一级 Windows 计算机使用 AD FS 或无缝单一登录直接注册。
>
>只有具有混合 Azure AD 联接配置的特定 userCertificate 属性值的 Windows 10 计算机才应 Azure AD Connect 同步到云。  在的早期版本中 Azure AD Connect 未严格实施此要求，导致 Azure AD 中不必要的设备对象。 Azure AD 中的此类设备始终处于 "挂起" 状态，因为这些计算机不打算注册到 Azure AD 中。
>
>此版本的 Azure AD Connect 将只同步正确配置为混合 Azure AD 加入的 Windows 10 计算机。 Azure AD Connect 不得同步[下层 Windows 设备](../../active-directory/devices/hybrid-azuread-join-plan.md#windows-down-level-devices)。  将从 Azure AD 中删除之前未正确同步 Azure AD 中的任何设备。  但是，此更改不会删除为混合 Azure AD 联接正确注册 Azure AD 的任何 Windows 设备。 
>
>有些客户可能会看到其部分或全部 Windows 设备从 Azure AD 中消失。 这不是问题的原因，因为在条件访问授权期间 Azure AD 不会使用这些设备标识。 某些客户可能需要重新访问[如何：计划混合 Azure Active Directory 联接实现](../../active-directory/devices/hybrid-azuread-join-plan.md) ，使其 Windows 计算机正确注册，并确保此类设备可以完全参与基于设备的条件访问。 如果 Azure AD Connect 尝试删除[下级 Windows 设备](../../active-directory/devices/hybrid-azuread-join-plan.md#windows-down-level-devices)，则该设备不是[Microsoft Workplace Join 为非 WINDOWS 10 计算机 MSI](https://www.microsoft.com/download/details.aspx?id=53554)创建的，它无法由任何其他 Azure AD 功能使用。  如果在 Azure AD 超过导出删除阈值的情况下看到计算机/设备对象的删除，则建议客户允许这些删除操作。
