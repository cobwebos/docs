...
**问：Azure AD 连接支持从两个域同步到 Azure AD 吗？**  
是的。支持此方案。 请参阅[多个域](how-to-connect-install-multiple-domains.md)。
 
**问：在 Azure AD 连接中，同一活动目录域是否可以有多个连接器？**  
不支持对同一个 AD 域使用多个连接器。 

**问：是否可以将 Azure AD Connect 数据库从本地数据库移动到远程 SQL Server 实例？**   
...
3. 针对现有的[远程 SQL 数据库](how-to-connect-install-existing-database.md)安装 Azure AD Connect。
   本文演示了如何改用本地 SQL 数据库。 如果改用远程 SQL 数据库，则在此过程的步骤 5 中，还必须输入用于运行 Windows 同步服务的现有服务帐户。 下面描述了此同步引擎服务帐户：
      ...
      **使用现有服务帐户**：默认情况下，Azure AD Connect 使用虚拟服务帐户来使用同步服务。 如果使用远程 SQL Server 实例或使用需要身份验证的代理，请使用托管服务帐户，或者使用域中的服务帐户并知道密码。 在这些情况下，请输入要使用的帐户。 确保运行安装的用户是 SQL 中的系统管理员，以便可以创建服务帐户的登录凭据。 有关详细信息，请参阅 [Azure AD Connect 帐户和权限](reference-connect-accounts-permissions.md#adsync-service-account)。 
...
以下是一份信息性文档，其中介绍了工程、支持和我们的顾问多年来开发的一些最佳实践。  这显示在可快速引用的项目符号列表中。  尽管此列表试图全面，但可能还有其他最佳做法可能尚未列入清单。

- 如果使用完整 SQL，则它应该保持本地与远程
    - 跳数更少
    - 更容易排除故障
    - 复杂性降低
    - 需要将资源指定为 SQL 并允许 Azure AD 连接和操作系统开销
- 如果可能，如果无法绕过代理，则需要确保超时值大于 5 分钟。
- 如果需要代理，则必须将代理添加到计算机。
- 了解本地 SQL 作业和维护及其如何影响 Azure AD 连接 - 尤其是重新索引
- 确保比 DNS 能够外部解决
- 无论您是使用物理服务器还是虚拟服务器，确保[服务器规格](how-to-connect-install-prerequisites.md#hardware-requirements-for-azure-ad-connect)符合建议
- 确保如果您使用的是虚拟服务器，所需的资源是专用的
- 确保磁盘和磁盘配置符合 SQL Server 的最佳做法
- 安装和配置 Azure AD 连接运行状况以进行监视
- 使用 Azure AD 连接中内置的删除阈值。
- 仔细检查发布更新，以便为可能添加的所有更改和新属性做好准备
- 备份一切
    - 备份密钥
    - 备份同步规则
    - 备份服务器配置
    - 备份 SQL 数据库
- 确保没有 SQL VSS 编写器备份 SQL 的第三方备份代理（在具有第三方快照的虚拟服务器中很常见）
- 限制在增加复杂性时使用的自定义同步规则的数量
- 将 Azure AD 连接服务器视为第 0 层服务器
- 在不了解影响和正确的业务驱动因素的情况下，对修改云同步规则持怀疑态度
- 确保为支持 Azure AD 连接和 Azure AD 连接运行状况打开了正确的 URL 和防火墙端口
- 利用云筛选属性排除故障并防止幻象对象
- 使用暂存服务器可确保使用 Azure AD 连接配置文档程序，以确保服务器之间的一致性
- 暂存服务器应位于单独的数据中心（物理位置）
- 暂存服务器不是高可用性解决方案，但您可以有多个暂存服务器
- 引入"延迟"暂存服务器可以减少一些潜在的停机时间，以防出错
- 首先测试和验证暂存服务器上的所有升级
- 在切换到暂存服务器之前，始终验证导出。  利用暂存服务器实现完全导入和完全同步，以减少业务影响
- 尽可能保持 Azure AD 连接服务器之间的版本一致性 

**问：是否可以允许 Azure AD 连接在工作组计算机上创建 Azure AD 连接器帐户？**
不是。  为了允许 Azure AD 连接到自动创建 Azure AD 连接器帐户，计算机必须加入域。  

## <a name="network"></a>网络
**问：我有防火墙、网络设备或其他限制连接在网络上保持打开时间的东西。使用 Azure AD 连接时，客户端超时阈值应该是什么？**  
...
**问：I具有多林环境，两个林之间的网络正在使用 NAT（网络地址转换）。是否支持在这两个林之间使用 Azure AD 连接？**</br>
...
**问：我为 Office 365 依赖方设置了"自动更新依赖方"。当我的令牌签名证书自动滚动时，我是否必须执行任何操作？**  
...
**问：支持 FIPS 的计算机上支持下一代加密 （NGC） 同步规则吗？**  
不是。  不支持。

**问：如果我在 Azure 门户中禁用了同步设备（例如：HAADJ），为什么要重新启用该设备？**<br>
可以在本地创作或掌控同步设备。 如果在本地启用了同步设备，即使管理员之前禁用了该设备，也可能会在 Azure 门户中重新启用它。 若要禁用同步设备，请使用本地 Active Directory 禁用计算机帐户。

**问：如果我在 Office 365 或 Azure AD 门户中为同步用户阻止用户登录，为什么在再次登录时将其解除阻止？**<br>
...
**问：Azure 登录页显示以前成功登录的用户的用户名。可以关闭此行为吗？**  
目前，不支持修改 **"密码**输入"字段的 HTML 属性，包括自动完成标记。 我们目前正在开发一种功能，它将允许使用自定义 JavaScript 向“密码”字段添加任何属性。****
...
**问：我收到一封电子邮件，告诉我我的自动升级不再工作，我需要安装新版本。为什么我需要这样做？**  
...
**问：我收到一封电子邮件，告诉我要升级到最新版本以重新启用自动升级。我使用的是版本 1.1.654.0。我需要升级吗？**  
需要。需要升级到 1.1.750.0 或更高版本才能重新启用自动升级。 [下载并升级到最新版本](https://www.microsoft.com/download/details.aspx?id=47594)。

**问：我收到一封电子邮件，告诉我要升级到最新版本以重新启用自动升级。如果我使用 PowerShell 启用自动升级，我还需要安装最新版本吗？**  
是的，仍需要升级到 1.1.750.0 或更高版本。 通过 PowerShell 启用自动升级服务不会解决在 1.1.750.0 之前的版本中发现的自动升级问题。

**问：我想升级到较新版本，但我不确定谁安装了 Azure AD Connect，并且我们没有用户名和密码。我们需要这个吗？**
不需要知道最初用来升级 Azure AD Connect 的用户名和密码。 可以使用任何具有全局管理员角色的 Azure AD 帐户。

**问：如何确定所用 Azure AD Connect 的版本？**  
要验证服务器上安装了哪个版本的 Azure AD Connect，请转到控制面板，并通过选择 > **"程序和功能**"来查找 Microsoft Azure **Programs**AD 连接的已安装版本，如下所示：
...
**问：我们去年已升级到最新版本的 Azure AD Connect。我们需要再次升级吗？**  
...
**问：我不确定升级到较新版本的 Azure AD Connect 时存在哪些风险。你能打电话给我帮我升级吗？**  
...
**问：为什么我看到事件 6311 和 6401 发生在同步步骤错误之后？**
