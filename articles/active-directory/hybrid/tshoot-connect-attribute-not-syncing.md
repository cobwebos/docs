...
* 从 AD 导入：活动目录对象将带入 AD CS。

* 从 AAD 导入：Azure 活动目录对象将引入 AAD CS。

* 同步：**入站同步规则**和**出站同步规则**按优先级数从低到高的顺序运行。 要查看同步规则，可以从桌面应用程序转到“同步规则编辑器”****。 入站同步规则**** 将数据从 CS 引入 MV。 出站同步规则**** 将数据从 MV 移动到 CS。

* 导出到 AD：运行同步后，对象将从 AD CS 导出到**活动目录**。

* 导出到 AAD：运行同步后，对象将从 AAD CS 导出到**Azure 活动目录**。
