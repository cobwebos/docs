...
- 无缝 SSO 支持 Kerberos 的 AES256_HMAC_SHA1、AES128_HMAC_SHA1 和 RC4_HMAC_MD5 加密类型。 建议将 AzureADSSOAcc $ 帐户的加密类型设置为 AES256_HMAC_SHA1，或将其中一个 AES 类型与RC4，提高安全性。 加密类型存储在 Active Directory 中的帐户的 Msds-supportedencryptiontypes 属性上。  如果 "AzureADSSOAcc $ 帐户加密类型" 设置为 "RC4_HMAC_MD5"，并且你想要将其更改为 AES 加密类型之一，请确保首先滚动 AzureADSSOAcc $ 帐户的 Kerberos 解密密钥，如 [FAQ 文档](how-to-connect-sso-faq.md)中所述。 在相关问题下，不会发生无缝 SSO。
