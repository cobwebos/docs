...
重复属性复原是 Azure Active Directory 中的一项功能，它可以在运行 Microsoft 的同步工具之一时消除**UserPrincipalName**和 SMTP **ProxyAddress**冲突导致的矛盾。
...
_**\<OriginalPrefix > +\<4DigitNumber >\@\<InitialTenantDomain >. onmicrosoft**_ 。

特性复原过程仅处理 UPN 和 SMTP **ProxyAddress**值。
...
可以在 Microsoft 365 管理中心查看目录同步错误。 Microsoft 365 管理中心中的报表只显示具有这些错误的**用户**对象。 它不显示有关 **Groups** 和 **Contacts** 之间的冲突的信息。

![活动用户](./media/how-to-connect-syncservice-duplicate-attribute-resiliency/1234.png "活动用户数")

有关如何在 Microsoft 365 管理中心查看目录同步错误的说明，请参阅[在 Office 365 中识别目录同步错误](https://support.office.com/article/Identify-directory-synchronization-errors-in-Office-365-b4fc07a5-97ea-4ca6-9692-108acab74067)。
...
ProxyAddress 冲突的电子邮件通知示例如下所示：  
    ![活动用户](./media/how-to-connect-syncservice-duplicate-attribute-resiliency/6.png "活动用户数")  
...
1. 具有特定属性配置的对象继续收到导出错误，而不是重复属性被隔离。  
   例如：
    ...
    a. 新用户在 AD 中创建，其 UPN 为**joe\@contoso.com**和 ProxyAddress **smtp： joe\@contoso.com**
   
    b. 此对象的属性与现有组冲突，其中 ProxyAddress 为**SMTP： Joe\@contoso.com**。
...
2. UPN 冲突的详细错误消息对已更改/隔离其 UPN 的用户显示不正确的 displayName。 例如：
    ...
    b. 在**UPN = user\@contoso.com**下，尝试同步**用户 B** 。
   
    c. **用户 B 的**UPN 更改为**User1234\@contoso.onmicrosoft.com** ，**用户\@Contoso.com**添加到**DirSyncProvisioningErrors**。
   
    d. **用户 b**的错误消息应指出**用户 A 已将** **用户\@contoso.com**作为 UPN，但它显示**用户 B 自己的**displayName。
