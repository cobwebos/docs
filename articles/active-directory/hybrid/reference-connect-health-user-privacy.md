...
具有活动**运行状况服务数据的服务器不是最**新的**错误**警报超过30天，建议在该时间范围内没有数据达到连接运行状况。 这些服务器将被禁用，并且不显示在 Connect Health 门户中。 若要重新启用这些服务器，必须卸载并[重新安装运行状况代理](how-to-connect-health-agent-install.md)。 请注意，这不适用于具有相同警报类型的**警告**。 警告指示向你发出警报的服务器中缺少部分数据。 
