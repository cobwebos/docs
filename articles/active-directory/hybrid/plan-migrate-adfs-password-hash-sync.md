---
title: Azure AD 连接：从联合迁移到 Azure AD 的 PHS |微软文档
...
> [!NOTE]
> 更改身份验证方法需要规划、测试和潜在的停机时间。 [分阶段推出](how-to-connect-staged-rollout.md)提供了一种使用密码哈希同步测试和逐步从联合迁移到云身份验证的替代方法。
...
要更新 Azure AD 连接，请完成[Azure AD 连接中的步骤：升级到最新版本](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-upgrade-previous-version)。

### <a name="password-hash-synchronization-required-permissions"></a>密码哈希同步所需的权限

可以使用快速设置或自定义安装配置 Azure AD Connect。 如果使用自定义安装选项，则密码哈希同步[所需的权限](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-accounts-permissions)可能未到位。
...
* **Azure AD 连接**。 如果最初使用 Azure AD Connect 配置了 AD FS，则必须使用 Azure AD Connect 向导更改为密码哈希同步。**
...
3. 在“查看解决方案”页上，记下“密码哈希同步”的状态。********<br /> 
   ...
   * 如果**密码哈希同步**设置为**启用**，则可以跳过本文中的步骤**1：启用密码哈希同步**部分。
4. 在“查看解决方案”页上，滚动到“Active Directory 联合身份验证服务(AD FS)”。********<br />

   * 如果此部分显示了 AD FS 配置，则可以肯定 AD FS 最初是使用 Azure AD Connect 配置的。 可以使用 Azure AD Connect 的“更改用户登录”选项将域从联合标识转换为托管标识。**** 该过程在**选项 A：使用 Azure AD 连接从联合切换到密码哈希同步**中进行了详细说明。
   * 如果当前设置中未列出 AD FS，则必须使用 PowerShell 手动将域从联合标识转换为托管标识。 有关此过程的详细信息，请参阅**选项 B：使用 Azure AD 连接和 PowerShell 从联合切换到密码哈希同步**。
...
> [!NOTE]
> 如果 **SupportsMfa** 设置为 **True**，则表示你正在使用本地多重身份验证解决方案将第二因素质询注入到用户身份验证流。 将此域从联合身份验证转换为托管身份验证后，此设置不再适用于 Azure AD 身份验证方案。 禁用联合后，将切断与本地联合的关系，这包括本地 MFA 适配器。 
>
...
| 使用 AD FS 阻止旧版身份验证客户端。| 请考虑使用[条件访问控件](https://docs.microsoft.com/azure/active-directory/conditional-access/conditions)和[交换联机客户端访问规则](https://aka.ms/EXOCAR)的组合替换阻止早期身份验证客户端版本的 AD FS 控件。 |
...
| 目前在 AD FS 中使用访问控制策略（AuthZ 规则）来控制对 Office 365 的访问。| 请考虑将策略替换为等效的 Azure AD[条件访问策略](https://docs.microsoft.com/azure/active-directory/active-directory-conditional-access-azure-portal)和[交换联机客户端访问规则](https://aka.ms/EXOCAR)。|
...
配置命名位置后，必须更新配置为包括或排除网络**的所有受信任位置**或**MFA 受信任的 IP**值的所有条件访问策略，以反映新的命名位置。

有关条件访问中**的位置**条件的详细信息，请参阅[活动目录条件访问位置](https://docs.microsoft.com/azure/active-directory/active-directory-conditional-access-locations)。

#### <a name="hybrid-azure-ad-joined-devices"></a>已加入混合 Azure AD 的设备

将设备加入 Azure AD 时，可以创建条件访问规则，强制该设备符合安全性和合规性的访问标准。 此外，用户可以使用组织的工作或学校帐户（而不是个人帐户）登录到设备。 使用已加入混合 Azure AD 的设备可将已加入 Active Directory 域的设备加入到 Azure AD。 你的联合环境可能已设置为使用此功能。

为确保混合联接继续适用于域转换为密码哈希同步后加入域的任何设备，对于 Windows 10 客户端，必须使用 Azure AD Connect 设备选项同步活动目录计算机到 Azure AD 的帐户。 
...
### <a name="step-1-enable-password-hash-synchronization"></a>第 1 步：启用密码哈希同步
...
### <a name="step-2-prepare-for-seamless-sso"></a>第 2 步：为无缝 SSO 做好准备

要让设备使用无缝 SSO，必须使用 Active Directory 中的组策略将一个 Azure AD URL 添加到用户的 Intranet 区域设置。

默认情况下，浏览器将自动从 URL 计算正确的区域（Internet 或 Intranet）。 例如 **，http：\/\/contoso/** 映射到 Intranet 区域，**然后\/\/http：intranet.contoso.com**映射到 Internet 区域（因为 URL 包含句点）。 仅当显式将 URL 添加到浏览器的 Intranet 区域时，浏览器才会将 Kerberos 票证发送到云终结点（例如 Azure AD URL）。
...
### <a name="step-3-change-the-sign-in-method-to-password-hash-synchronization-and-enable-seamless-sso"></a>第 3 步：将登录方法更改为密码哈希同步并启用无缝 SSO

可以使用两个选项将登录方法更改为密码哈希同步并启用无缝 SSO。

#### <a name="option-a-switch-from-federation-to-password-hash-synchronization-by-using-azure-ad-connect"></a>选项 A：使用 Azure AD 连接从联合切换到密码哈希同步
...
6. 在“已准备好进行配置”页上，确保已选中“配置完成后启动同步过程”复选框。******** 然后，选择 **"配置**"。
...
7. 在 Azure AD 门户中，选择**Azure 活动目录** > **Azure AD 连接**。
8. 验证以下设置：
   * “联合身份验证”设置为“已禁用”。********
   * “无缝单一登录”设置为“已启用”。********
   * “密码同步”设置为“已启用”。********<br /> 
...
跳到[测试和后续步骤](#testing-and-next-steps)。
   ...
   > [!IMPORTANT]
   > 跳过选项**B 部分：使用 Azure AD 连接和 PowerShell 从联合切换到密码哈希同步**。 如果已选择使用“选项 A”将登录方法更改为密码哈希同步并启用无缝 SSO，则“选项 B”部分中的步骤不适用。

#### <a name="option-b-switch-from-federation-to-password-hash-synchronization-using-azure-ad-connect-and-powershell"></a>选项 B：使用 Azure AD 连接和 PowerShell 从联合切换到密码哈希同步
...
4. 在“用户登录”页上，选择“密码哈希同步”按钮。******** 依次选择“启用单一登录”、“下一步”。********
   ...
   启用密码哈希同步之前：![在用户登录页面上显示"不配置"选项的屏幕截图](media/plan-migrate-adfs-password-hash-sync/migrating-adfs-to-phs_image12.png)<br />

   启用密码哈希同步后：![在用户登录页面上显示新选项的屏幕截图](media/plan-migrate-adfs-password-hash-sync/migrating-adfs-to-phs_image13.png)<br />
...
3. 在 Azure AD 门户中，选择**Azure 活动目录** > **Azure AD 连接**。
...
[排除 Azure 活动目录密码哈希同步故障](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnectsync-troubleshoot-password-hash-synchronization)
