...
> [!IMPORTANT]
> Microsoft 不支持在正式记录的这些操作之外修改或操作 Azure AD Connect 同步。 这些操作中的任何一个都可能导致 Azure AD 连接同步的不一致或不受支持的状态。因此，Microsoft 无法为此类部署提供技术支持。
...
在开始更改筛选之前，请确保[禁用计划的任务](#disable-the-scheduled-task)，以免意外导出尚未确认是否正确的更改。
...
为了保护您避免意外删除许多对象，默认情况下，功能"[防止意外删除](how-to-connect-sync-feature-prevent-accidental-deletes.md)"处于打开状态。 如果由于筛选而删除了许多对象（默认为 500 个），则需要遵循本文中的步骤来允许将删除结果传播到 Azure AD。

如果使用 2015 年 11 月 ([1.0.9125](reference-connect-version-history.md#1091250)) 之前的内部版本、更改筛选器配置或使用密码哈希同步，则在完成配置之后，需要触发所有密码的完全同步。 有关如何触发密码完全同步的步骤，请参阅[触发所有密码的完整同步](tshoot-connect-password-hash-synchronization.md#trigger-a-full-sync-of-all-passwords)。 如果使用内部版本 1.0.9125 或更高版本，则常规的**完全同步**操作也会计算是否应同步密码，因此不再需要执行这个额外的步骤。
...
3. 您现在可以进行配置更改，并从**同步服务管理器**控制台手动运行同步引擎。
...
更改基于域的筛选的首选方法是运行安装向导并更改[域和 OU 筛选](how-to-connect-install-custom.md#domain-and-ou-filtering)。 使用安装向导可以自动完成本主题中所述的所有任务。
...
2. 从 **"开始"** 菜单开始**同步服务**。
3. 选择**连接器**，在 **"连接器**"列表中，选择具有"**活动目录域服务"** 类型的连接器。 从“操作”中选择“属性”。********  
...
5.  要完成配置，您需要运行**完全导入**和**增量同步**。继续阅读"[应用"部分并验证更改](#apply-and-verify-changes)。
...
8. 要完成配置，您需要运行**完全导入**和**增量同步**。继续阅读"[应用"部分并验证更改](#apply-and-verify-changes)。
...
7. 单击“添加转换”，为“FlowType”选择“Constant”，为“目标属性”选择“cloudFiltered”。******************** 在“源”文本框中键入 **True**。**** 单击**添加**以保存规则。  
   ![入站 3 转换](./media/how-to-connect-sync-configure-filtering/inbound3.png)
8. 要完成配置，您需要运行**完全同步**。继续阅读"[应用"部分并验证更改](#apply-and-verify-changes)。
...
6. 将“联接”规则留空，然后单击“下一步”。********
7. 单击“添加转换”，为“FlowType”选择“Constant”，为“目标属性”选择“cloudFiltered”。******************** 在“源”框中键入 **False**。**** 单击**添加**以保存规则。  
...
11. 单击“添加转换”，为“FlowType”选择“Constant”，为“目标属性”选择“cloudFiltered”。******************** 在“源”框中键入 **True**。**** 单击**添加**以保存规则。  
    ![入站 3 转换](./media/how-to-connect-sync-configure-filtering/inbound3.png)  
12. 要完成配置，您需要运行**完全同步**。继续阅读"[应用"部分并验证更改](#apply-and-verify-changes)。
...
8. 单击“保存”。****
9. 要完成配置，您需要运行**完全同步**。继续阅读"[应用"部分并验证更改](#apply-and-verify-changes)。
...
在首次使用[自定义安装](how-to-connect-install-custom.md#sync-filtering-based-on-groups)安装 Azure AD Connect 时，可以配置基于组的筛选。 这种筛选专用于只需同步一小组对象的试验部署。 禁用基于组的筛选后，无法重新启用它。 *不支持*在自定义配置中使用基于组的筛选。 仅支持使用安装向导配置此功能。 完成试验后，请使用本主题所述的其他某个筛选选项。 将基于 OU 的筛选与基于组的筛选结合使用时，必须包含组和成员所在的 OU。
