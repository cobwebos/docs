...
使用 PowerShell 在 Active Directory 林中启用 *无缝 SSO* 。 如果有多个 Active Directory 林，请为每个林单独启用它。仅为选择进行分阶段推出的用户触发 *无缝 SSO* 。 这不会影响现有的联合身份验证设置。
...
4. 以管理员身份运行 PowerShell。 在 PowerShell 中， `New-AzureADSSOAuthenticationContext`调用。 此命令将打开一个窗格，可在其中输入租户的全局管理员凭据。
