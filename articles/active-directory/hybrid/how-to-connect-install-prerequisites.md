...
* 使用 "点线" (名称包含句点 "." 的本地林或域。 *不支持*) NetBIOS 名称。
...
* 证书的标识必须与联合身份验证服务名称（例如 sts.contoso.com）匹配。
  * 标识是 dNSName 类型的 SAN) 扩展 (使用者备用名称，或者，如果没有 SAN 条目，则将使用者名称指定为公用名。
...
* 不支持基于 CryptoAPI 下一代 (CNG) 密钥和密钥存储提供程序 (Ksp) 的证书。 因此，你必须使用基于加密服务提供程序的证书 (CSP) 而不是 KSP。
* 支持通配符证书。

### <a name="name-resolution-for-federation-servers"></a>联合服务器的名称解析
* 为 AD FS 名称设置 DNS 记录 (例如，intranet (内部 DNS 服务器的 sts.contoso.com) ) 和通过域注册机构 (公共 DNS) 公共 DNS。 对于 Intranet DNS 记录，请确保使用 A 记录而不是 CNAME 记录。 Windows 身份验证需要使用记录才能从已加入域的计算机正常工作。
* 如果要部署多个 AD FS 服务器或 Web 应用程序代理服务器，请确保已配置负载均衡器，并将 AD FS 名称的 DNS 记录 (例如，sts.contoso.com) 指向负载均衡器。
* 若要使用 Windows 集成身份验证在 intranet 中使用 Internet Explorer 的浏览器应用程序，请确保 AD FS 名称 (例如，sts.contoso.com) 添加到 Internet Explorer 中的 intranet 区域。 可以通过组策略控制此要求并将其部署到所有已加入域的计算机。
