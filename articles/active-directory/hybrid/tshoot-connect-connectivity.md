---
title: Azure AD 连接：解决 Azure AD 连接问题 |微软文档
...
安装向导使用两种不同的安全上下文。 在 **"连接到 Azure AD"** 页上，它使用当前登录的用户。 在"**配置**"页上，它更改为[运行同步引擎服务的帐户](reference-connect-accounts-permissions.md#adsync-service-account)。 如果出现问题，该问题很有可能已显示在向导中的“连接到 Azure AD”页上，因为代理配置是全局性的。****
...
如果无法访问终结点**https://secure.aadcdn.microsoftonline-p.com**，并且全局管理员启用了 MFA，则会出现此错误。  
...
如果收到**无法连接到远程服务器**，则 PowerShell 尝试在没有使用代理的情况下进行直接调用，或者未正确配置 DNS。 请确保正确配置了 **machine.config** 文件。
...
| 407 |需要代理身份验证 |代理服务器要求登录，但未提供任何登录信息。 如果代理服务器需要身份验证，请确保在计算机中配置了此设置。此外，请确保对运行向导的用户和服务帐户使用域帐户。 |
