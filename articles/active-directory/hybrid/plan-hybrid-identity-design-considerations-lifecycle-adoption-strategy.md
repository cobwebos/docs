...
Azure AD RBAC 允许公司限制个人可以在有权访问 Azure 门户后执行的操作的数量。 使用 RBAC 控制对门户的访问时，IT 管理员可通过下列访问管理方式委派访问权限：
