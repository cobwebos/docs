...
* 可以让用户通过其 Azure AD 帐户自动登录到 Nomadesk 单一登录 (SSO)
...
## <a name="add-nomadesk-from-the-gallery"></a>从库中添加 Nomadesk
...
本部分旨在说明如何基于名为“Britta Simon”的测试用户配置和测试 Nomadesk 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 Nomadesk 用户。 换句话说，需要建立 Azure AD 用户与 Nomadesk 中相关用户之间的关联关系。
...
本部分旨在介绍如何在 Azure 经典门户中启用 Azure AD SSO 并在 Nomadesk 应用程序中配置 SSO。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
  ...
  * 在“登录 URL”文本框中，使用以下模式键入用户用于登录 Nomadesk 应用程序的 URL：**“https://mynomadesk.com/logon/saml/TENANTID”**。 引用泛型名称时，需将 **TENANTID** 替换为实际的租户 ID。
...
### <a name="create-a-nomadesk-test-user"></a>创建 Nomadesk 测试用户
