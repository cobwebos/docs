...
如果你已更改默认的同步规则，这些规则将在系统升级完成之后重置为默认配置。 为了确保配置在每次升级之后得到保留，请务必按照[更改默认配置的最佳实践](active-directory-aadconnectsync-best-practices-changing-default-configuration.md)中所述的步骤来更改配置。
...
以下步骤也适用于从 Azure AD Sync 或者使用 FIM + Azure AD 连接器的解决方案迁移。 这些步骤不适用于 DirSync，但是，可以在[升级 Azure Active Directory 同步 (DirSync)](connect/active-directory-aadconnect-dirsync-upgrade-get-started.md)一文中找到适用于 DirSync 的相同交叉迁移（也称为并行部署）方法的步骤。

### <a name="swing-migration-steps"></a>交叉迁移步骤
