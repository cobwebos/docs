---
title: 延期或续订 PIM-Azure Active Directory 中的 Azure 资源角色分配 |Microsoft Docs
...
ms.locfileid: "67476306"
---
# <a name="extend-or-renew-azure-resource-role-assignments-in-pim"></a>在 PIM 中扩展或续订 Azure 资源角色分配

Azure Active Directory (Azure AD) Privileged Identity Management (PIM) 引入了新控件用于管理 Azure 资源的访问和分配生命周期。 管理员可以使用开始和结束日期时间属性分配成员身份。 当分配结束时间即将到来时，PIM 会向受影响的用户或组发送电子邮件通知。 此外，它还向资源管理员发送电子邮件通知，确保能够保持相应的访问权限。 即使访问权限未延期，分配也可以续订，并在长达 30 天内以过期状态保持可见。
...
![Azure 资源-我的角色页面上列出符合条件的角色与一个操作列](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-ui.png)

如果分配结束日期时间在 14 天内，则“延期”按钮将在用户界面中变为有效链接。  以下示例假设当前日期为 3 月 27 日。

![其中包含指向激活或扩展操作列](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-within-14.png)

若要请求延期此角色分配，请选择“延期”打开请求窗体。 

![扩展有一个原因框的角色分配窗格](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-role-assignment-request.png)
...
![使用分配的详细信息扩展来扩展角色分配窗格](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-form-complete.png)

片刻之后，资源管理员会收到一封电子邮件通知，要求他们审阅延期请求。 如果已提交延期请求，Azure 门户的顶部会显示一条 toast 通知，其中解释了错误。

![说明存在已存在挂起的角色分配扩展通知](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-failed-existing-request.png)

转到**挂起的请求**若要查看请求的状态或取消它的左窗格中的页。

![Azure 资源的挂起的请求页列出所有挂起的请求和取消的链接](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-cancel-request.png)
...
![Azure 资源的批准请求页的列表的请求和批准或拒绝的链接](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-admin-approve-grid.png)

当管理员选择“批准”或“拒绝”时，将显示请求的详细信息，同时会显示一个字段，让管理员提供审核日志的理由。  

![批准角色分配请求，请求者原因是分配类型、 开始时间、 结束时间和原因](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-admin-approve-blade.png)
...
![Azure 资源-成员页上列出符合条件的角色，其中包含用于扩展链接](media/pim-resource-roles-renew-extend/aadpim-rbac-extend-admin-extend.png)
...
![我角色页-过期的角色选项卡](media/pim-resource-roles-renew-extend/aadpim-rbac-renew-from-myroles.png)

显示的角色列表默认为“符合条件的角色”。  使用下拉菜单在“符合条件”与“活动”分配角色之间切换。

若要请求续订列表中的任何角色分配，请选择“续订”操作。  然后提供请求原因。 建议提供持续时间和任何附加的上下文，以帮助资源管理员做出批准或拒绝的决定。

![续订角色分配窗格显示原因框](media/pim-resource-roles-renew-extend/aadpim-rbac-renew-request-form.png)
...
![Azure 资源-列出链接续订过期的角色的成员页](media/pim-resource-roles-renew-extend/aadpim-rbac-renew-from-member-blade.png)
