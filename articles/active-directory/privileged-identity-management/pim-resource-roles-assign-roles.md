---
title: 在 Privileged Identity Management Azure Active Directory 中分配 Azure 资源角色 |Microsoft Docs
...
> [!NOTE]
> 默认情况下，分配给所有者或用户访问管理员订阅角色的组的用户或成员，以及在 Azure AD 中启用订阅管理 Azure AD 全局管理员。 这些管理员可以分配角色、配置角色设置，并使用 Azure 资源 Privileged Identity Management 来查看访问权限。 如果没有资源管理员权限，用户将无法管理资源 Privileged Identity Management。 查看 [Azure 资源的内置角色](../../role-based-access-control/built-in-roles.md)列表。
