...
本文分步介绍了如何在 Azure Active Directory (Azure AD 组织中计划 Privileged Identity Management (PIM) 的部署。 你会在可能的情况下将具有高特权角色的用户重新分配给不太强大的内置角色或自定义角色，并针对你的最特权角色规划实时角色分配。 在本文中，我们将针对部署规划和实现提出建议。
...
若要使用 Privileged Identity Management，你的目录必须具有以下付费或试用许可证之一。 有关详细信息，请参阅[使用 Privileged Identity Management 的许可要求](subscription-requirements.md)。
...
## <a name="how-pim-works"></a>PIM 的工作方式

本部分提供了有关 Privileged Identity Management 过程的相关部分的规划目的的评审。 有关详细信息，请参阅 [什么是 Azure AD Privileged Identity Management？](pim-configure.md)

1. 开始使用 Privileged Identity Management 以便用户有资格使用特权角色。
1. 当合格用户需要使用其特权角色时，他们将使用 Privileged Identity Management 激活该角色。
1. 用户在设置中可能需要：
...
1. 用户成功激活其角色后，他们将拥有设置持续时间的角色权限。
1. 管理员可以查看审核日志中所有 Privileged Identity Management 活动的历史记录。 他们还可以通过使用 Privileged Identity Management 功能（如访问评审和警报）进一步保护其 Azure AD 组织并满足合规性。

## <a name="roles-that-can-be-managed-by-pim"></a>可以使用 PIM 管理的角色

**Azure AD 角色** 全部在 Azure Active Directory (如全局管理员、Exchange 管理员和安全管理员) 中。 可通过 [Azure Active Directory 中的管理员角色权限](../users-groups-roles/directory-assign-admin-roles.md)，了解更多有关角色及其功能的信息。 如需帮助来确定向管理员分配哪个角色，请参阅[按任务划分的最小特权角色](../users-groups-roles/roles-delegate-by-task.md)。

**Azure 角色** 是指链接到 azure 资源、资源组、订阅或管理组的角色。 你可以使用 PIM 提供对内置 Azure 角色（如所有者、用户访问管理员、参与者以及 [自定义角色](../../role-based-access-control/custom-roles.md)）的实时访问。 有关 Azure 角色的详细信息，请参阅 [azure 基于角色的访问控制](../../role-based-access-control/overview.md)。

有关详细信息，请参阅 [Privileged Identity Management 中无法管理的角色](pim-roles.md)。

## <a name="deployment-plan"></a>部署计划

在你的组织中部署 Privileged Identity Management 之前，请按照说明进行操作并了解本节中的概念，以帮助你创建针对组织的特权标识要求定制的计划。

### <a name="identify-your-stakeholders"></a>标识利益干系人

以下部分可帮助你确定项目中涉及的所有利益干系人，并需要注销、查看或随时获得通知。 它包含单独的表，用于为 Azure 角色 Azure AD 角色和 PIM 部署 PIM。 根据组织情况，将利益干系者添加到下表中。
...
| 名称 | 角色 | 操作 |
| --- | --- | --- |
| 姓名和电子邮件 | **标识架构师或 Azure 全局管理员**<br/>标识管理团队提供的代表，负责定义如何将此更改与你的组织中的核心标识管理基础结构进行对齐。 | SO/R/I |
| 姓名和电子邮件 | **服务所有者/线路管理器**<br/>某项服务或一组服务的 IT 所有者的代表。 它们是制定决策和帮助向团队推出 Privileged Identity Management 的关键所在。 | SO/R/I |
| 姓名和电子邮件 | **安全所有者**<br/>安全团队的代表，可以签署该计划满足组织的安全要求。 | SO/R |
| 姓名和电子邮件 | **IT 支持管理员/支持人员**<br/>IT 支持组织的代表，他们可以从支持人员的角度为此更改的可支持性提供反馈。 | R/I |
| 试点用户的姓名和电子邮件 | **特权角色用户**<br/>要为其实施特权标识管理的用户组。 在实施 Privileged Identity Management 后，他们需要知道如何激活其角色。 | I |

#### <a name="stakeholders-privileged-identity-management-for-azure-roles"></a>利益干系人： Azure 角色 Privileged Identity Management
...
| 姓名和电子邮件 | **IT 支持管理员/支持人员**<br/>IT 支持组织的代表，他们可以从支持人员的角度为此更改的可支持性提供反馈。 | R/I |
| 试点用户的姓名和电子邮件 | **Azure 角色用户**<br/>要为其实施特权标识管理的用户组。 在实施 Privileged Identity Management 后，他们需要知道如何激活其角色。 | I |

### <a name="start-using-privileged-identity-management"></a>开始使用 Privileged Identity Management

在规划过程中，你应按照 [开始使用 Privileged Identity Management](pim-getting-started.md) 文章中的步骤准备 Privileged Identity Management。 Privileged Identity Management 使你能够访问一些旨在帮助你进行部署的功能。

如果你的目标是部署适用于 Azure 资源的 Privileged Identity Management，则应遵循我们的 [发现 azure 资源来管理 Privileged Identity Management](pim-resource-roles-discover-resources.md) 文章。 只有订阅和管理组的所有者才能 Privileged Identity Management 管理这些资源。 在管理下，PIM 功能可用于所有级别的所有者，包括管理组、订阅、资源组和资源。 如果你是尝试为 Azure 资源部署 Privileged Identity Management 的全局管理员，则可以 [提升访问权限，以管理所有 azure 订阅](../../role-based-access-control/elevate-access-global-admin.md?toc=%2fazure%2factive-directory%2fprivileged-identity-management%2ftoc.json) ，使你能够在目录中访问所有 azure 资源，以便发现。 但是，建议在使用 Privileged Identity Management 管理其资源之前先获取每个订阅所有者的批准。

### <a name="enforce-principle-of-least-privilege"></a>强制执行最低权限原则

请务必确保已在组织中针对 Azure AD 和 Azure 角色强制实施最低权限原则。

#### <a name="plan-least-privilege-delegation"></a>计划最低权限委托

对于 Azure AD 角色，组织通常会在大多数管理员只需要一个或两个特定且不太强大的管理员角色时，为多个管理员分配全局管理员角色。 如果有大量的全局管理员或其他高权限角色，则很难及时跟踪特权角色分配。

按照以下步骤实现 Azure AD 角色的最低权限原则。
...
1. 列出组织中有特权角色的人员。 你可以使用 Privileged Identity Management [发现和见解 (预览版) ](pim-security-wizard.md) 来降低曝光。

    ![发现和见解 (预览) 页面，通过特权角色降低曝光](./media/pim-deployment-plan/new-preview-page.png)

1. 对于组织中的所有全局管理员，找出他们需要该角色的原因。 然后，将其从全局管理员角色中删除，并将内置角色或自定义角色分配到 Azure Active Directory 中的较低权限。 仅供参考，Microsoft 目前只有大约10个具有全局管理员角色的管理员。 了解 [Microsoft 如何使用 Privileged Identity Management](https://www.microsoft.com/itshowcase/Article/Content/887/Using-Azure-AD-Privileged-Identity-Management-for-elevated-access)的详细信息。
...
将审阅者设置为 ** () 的成员 **。 角色中的所有用户都将收到一封电子邮件，询问他们是否需要访问权限。 此外，在高级设置中启用 " **需要出于批准原因** "，使用户必须指出需要角色的原因。 基于此信息，你可以从不必要的角色中删除用户，或将其委派给更精细的管理员角色。
...
对于 Azure 订阅和资源，可以设置类似的访问评审流程，用于评审每个订阅或资源中的角色。 此过程的目标是最大程度地减少附加到每个订阅或资源的所有者和用户访问管理员分配，并删除不必要的分配。 但是，组织通常会将此类任务委派给每个订阅或资源的所有者，因为他们对特定角色（尤其是自定义角色）有更深入的了解。

如果你在组织中尝试为 Azure 角色部署 PIM 的全局管理员角色，则可以 [提升访问权限来管理所有 azure 订阅](../../role-based-access-control/elevate-access-global-admin.md?toc=%2fazure%2factive-directory%2fprivileged-identity-management%2ftoc.json) ，以获取对每个订阅的访问权限。 然后可以找到每个订阅的所有者，与其协作，删除不必要的分配，最大程度减少所有者角色分配量。

拥有 Azure 订阅的 "所有者" 角色的用户也可以使用 [azure 资源的访问评审](pim-resource-roles-start-access-review.md) 来审核和删除不必要的角色分配，这与前面 Azure AD 角色所述的过程类似。
...
在组织中清理特权角色分配后，你需要决定要 Privileged Identity Management 保护哪些角色。

如果某个角色受 Privileged Identity Management 保护，则分配给它的符合条件的用户必须提升权限才能使用该角色授予的权限。 提升过程还可能包括获得批准、使用多重身份验证以及提供激活的原因。 Privileged Identity Management 还可以通过通知、Privileged Identity Management 和 Azure AD 审核事件日志来跟踪权限提升。

要选择使用 Privileged Identity Management 保护哪些角色可能存在一定困难，而且每个组织的情况都不一样。 本部分提供有关 Azure AD 和 Azure 角色的最佳实践建议。

#### <a name="azure-ad-roles"></a>Azure AD 角色

确定保护权限最多的角色 Azure AD 优先级非常重要。 基于 Privileged Identity Management 客户的使用模式，排在前 10 位的 Privileged Identity Management 管理的 Azure AD 角色为：
...
> [!TIP]
> ： heavy_check_mark： **Microsoft 建议** 你在第一步中使用 Privileged Identity Management 来管理所有全局管理员和安全管理员，因为他们是在受到威胁时可以执行最大危害的用户。

考虑哪些数据和权限对你的组织最为敏感是很重要的。 例如，某些组织可能希望使用 Privileged Identity Management 来保护其 Power BI 管理员角色或其团队管理员角色，因为他们可以访问数据和更改核心工作流。

如果有任何角色分配有来宾用户，则这些角色很容易受到攻击。

> [!TIP]
> :heavy_check_mark:**Microsoft 建议**使用 Privileged Identity Management 管理含有来宾用户的所有角色，以降低与来宾用户帐户相关的攻击风险。

像目录读者、消息中心读取器和安全读者这样的读者角色有时会视为不如其他角色更重要，因为它们没有写入权限。 但是，我们有一些客户也保护这些角色，因为有权访问这些帐户的攻击者可能能够读取敏感数据，如个人数据。 在决定是否要使用 Privileged Identity Management 来管理组织中的读者角色时，请考虑此风险。
...
如果你是全局管理员来确定哪些订阅和资源最重要，则应该联系你组织中的订阅所有者来收集每个订阅管理的资源列表。 然后，与订阅所有者合作，根据严重性级别对资源进行分组，以防它们受到 (低、中、高) 。 根据此严重级别 Privileged Identity Management 管理资源的优先级。
...
是否向组而不是单个用户分配角色是一种战略决定。 规划时，请考虑在以下情况下将角色分配给组来管理角色分配：

- 向多个用户分配了角色
- 要委派分配角色

#### <a name="many-users-are-assigned-to-a-role"></a>向多个用户分配了角色

跟踪谁分配了角色并根据他们需要的时间管理其分配，在手动完成操作时可能需要一段时间。 若要为某个角色分配一个角色，请首先 [创建一个角色可分配组](../users-groups-roles/roles-groups-create-eligible.md) ，然后为该组分配适用于角色的组。 此操作可将组中的每个人与有资格提升为角色的单个用户进行相同的激活过程。 组成员使用 Privileged Identity Management 的激活请求和审批过程，分别激活其对组的分配。 组未激活，只是用户的组成员身份。

#### <a name="you-want-to-delegate-assigning-the-role"></a>要委派分配角色

组所有者可以管理组的成员身份。 对于 Azure AD 角色分配的组，只有特权角色管理员、全局管理员和组所有者可以管理组成员身份。 通过向组中添加新成员，该成员将获得对分配了组的角色的访问权限，无论分配是符合条件还是处于活动状态。 使用组所有者委派分配角色的组成员身份管理，以减少所需的权限。 若要详细了解如何在创建组时为组分配所有者，请参阅 [在 Azure AD 中创建可分配角色的组](../users-groups-roles/roles-groups-create-eligible.md)。

> [!TIP]
> ： heavy_check_mark： **Microsoft 建议** 你通过 Privileged Identity Management 将 Azure AD 角色可分配的组置于管理之下。 在通过 PIM 管理角色分配的组后，该组被称为特权访问组。 使用 PIM 来要求组所有者先激活其所有者角色分配，然后才能管理组成员身份。 有关在 PIM 管理下引入组的详细信息，请参阅将 [特权访问组 (预览) 到 Privileged Identity Management 中](groups-discover-groups.md)。
...
> [!TIP]
> ： heavy_check_mark：对于除建议使用的[两个中断玻璃紧急访问帐户以外的其他两个中断玻璃紧急访问帐户](../users-groups-roles/directory-emergency-access.md)， **Microsoft 建议**你为 Azure AD 角色和 Azure 角色提供零个永久活动的分配，该帐户应具有永久全局管理员角色。
...
#### <a name="privileged-identity-management-settings-for-azure-roles"></a>Azure 角色 Privileged Identity Management 设置
...
| 事件工单 | 激活角色时符合条件的用户需要记录事件工单号。 此设置可帮助组织使用内部事件编号识别每次激活，以减少无效激活的次数。<br/><br/> :heavy_check_mark:**Microsoft 建议**利用事件工单号将 Privileged Identity Management 绑定到内部系统。 对于需要激活上下文的审批者，此方法非常有用。 |
...
| 审批者 | 如果符合条件的角色需要执行审批操作来进行激活，应列出批准请求的人员。 默认情况下，Privileged Identity Management 将审批者设置为身份是特权角色管理员的所有用户，无论他们是永久性角色还是合格角色。<br/><br/>**注意：** 如果用户同时有资格拥有 Azure AD 角色和该角色的审批者身份，则无法为自己执行审批操作。<br/><br/> ： heavy_check_mark： **Microsoft 建议** 你选择 "审批者"，使其成为最熟悉该角色的用户，而不是 "全局管理员"。 |
...
| 活动管理员 | 对于 Azure 资源，活动管理员是永远不必激活角色便可使用角色的用户的列表。 此列表并不像在 Azure AD 角色中一样是永久管理员，因为你可以设置当用户将丢失此角色时的过期时间。 |
| 活动期限 | Azure 角色的活动角色分配在配置的持续时间后过期。 可以选择 15 天、1 个月、3 个月、6 个月、1 年或永久活动。 |
| 合格期限 | Azure 角色的符合条件的角色分配将在此持续时间后过期。 可以选择 15 天、1 个月、3 个月、6 个月、1 年或永久符合条件。 |

## <a name="implementation-plan"></a>实现计划
...
1. 导航到 **Azure AD 角色**"，选择" **角色**"，然后选择所配置的角色。

1. 对于测试用户组，如果他们已是永久管理员，则可以通过从其行中选择三个点来搜索它们并将其从永久转换为符合条件。 如果他们还未分配有角色，可以[实施新的合格角色分配](pim-how-to-add-role-to-user.md#make-a-user-eligible-for-a-role)。

1. 为所有想要测试的角色重复步骤 1-3。

1. 测试用户一经设置，应向其发送有关如何[激活 Azure AD 角色](pim-how-to-activate-role.md)的链接。

#### <a name="configure-privileged-identity-management-for-azure-roles"></a>为 Azure 角色配置 Privileged Identity Management

1. 为想要测试的订阅或资源中的角色[配置 Azure 资源角色设置](pim-resource-roles-configure-role-settings.md)。

1. 导航到该订阅的 **Azure 资源** ，并选择 " **角色**"，然后选择你配置的角色。
...
1. 对于已配置的每个角色，为具有符合条件的分配的所有用户选择省略号 (**...**) 。
1. 选择 " **永久** 保留" 选项，以使角色分配成为永久分配。

#### <a name="azure-roles"></a>Azure 角色

1. 登录到 [Azure 门户](https://portal.azure.com/)。
1. 打开“Azure AD Privileged Identity Management”。
1. 选择 " **Azure 资源** "，然后选择要回滚的订阅或资源。
1. 选择“角色”。
1. 对于已配置的每个角色，为具有符合条件的分配的所有用户选择省略号 (**...**) 。
1. 选择 " **永久** 保留" 选项，以使角色分配成为永久分配。
...
访问评审是向分配有特权角色的用户或特定审阅者询问每个用户是否需要特权身份的最佳方式。 如果想要减少攻击面并保持符合性，访问评审非常有用。 有关开始访问评审的详细信息，请参阅 [Azure AD 角色访问](pim-how-to-start-security-review.md) 评审和 [Azure 角色访问评审](pim-resource-roles-start-access-review.md)。 某些组织为遵守法律和法规，需要执行定期访问评审，而对于其他组织，访问评审是在整个组织中实施最低权限原则的最佳方式。

> [!TIP]
> ： heavy_check_mark： **Microsoft 建议** 为所有 Azure AD 和 Azure 角色设置季度访问评审。

在大多数情况下，Azure AD 角色的审阅者是用户自身，而 Azure 角色的审阅者是该角色所在的订阅的所有者。 但是，通常情况下公司会拥有未与任何人员的电子邮件地址关联的特权帐户。 在这种情况下，无人审阅访问权限。
...
如果你是全局管理员或特权角色管理员，你可以访问审核日志。 有关详细信息，请参阅 [Azure AD 角色审核历史记录](pim-how-to-use-audit-log.md) 和 [Azure 角色审核历史记录](azure-pim-resource-rbac.md)。
...
> [!TIP]
> ： heavy_check_mark： **Microsoft 建议** 将 [azure 日志监视](../reports-monitoring/concept-activity-logs-azure-monitor.md) 设置为在 azure 存储帐户中存档审核事件，以获得更高的安全性和符合性。
