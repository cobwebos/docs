---
title: 部署特权标识管理 （PIM） - Azure AD |微软文档
...
> 本文中有许多带有以下标记的项：
> 
> ：heavy_check_mark：**微软推荐**
> 
> 这些是常规建议，应仅当其适用于企业需求时才实施。

## <a name="learn-about-privileged-identity-management"></a>了解 Privileged Identity Management

Azure AD Privileged Identity Management 可帮助跨 Azure AD、Azure 资源和其他 Microsoft 联机服务管理特权管理角色。 在分配和遗忘特权标识的世界中，特权标识管理提供实时访问、请求审批工作流和完全集成的访问审核等解决方案，以便您可以识别、发现和防止恶意特权角色的实时活动。 通过部署 Privileged Identity Management 来管理整个组织中的特权角色可极大降低风险，同时获取有关特权角色活动的有用见解。
...
1. 管理员可以查看审核日志中所有 Privileged Identity Management 活动的历史记录。 他们还可以使用特权标识管理功能（如访问审核和警报）进一步保护其 Azure AD 组织并满足合规性要求。

有关详细信息，请参阅[什么是 Azure AD Privileged Identity Management？](pim-configure.md)。

### <a name="roles-that-can-be-managed-by-privileged-identity-management"></a>可以通过 Privileged Identity Management 管理的角色

**Azure AD 角色**– 这些角色都在 Azure 活动目录中（如全局管理员、Exchange 管理员和安全管理员）。 可通过 [Azure Active Directory 中的管理员角色权限](../users-groups-roles/directory-assign-admin-roles.md)，了解更多有关角色及其功能的信息。 如需帮助来确定向管理员分配哪个角色，请参阅[按任务划分的最小特权角色](../users-groups-roles/roles-delegate-by-task.md)。
...
#### <a name="stakeholders-privileged-identity-management-for-azure-ad-roles"></a>利益干系人：Azure AD 角色的特权标识管理
...
#### <a name="stakeholders-privileged-identity-management-for-azure-resource-roles"></a>利益干系人：Azure 资源角色的特权标识管理
...
如果目标是为 Azure 资源部署 Privileged Identity Management，则应按照[发现要在 Privileged Identity Management 文档中管理的 Azure 资源](pim-resource-roles-discover-resources.md)一文中所述操作。 只有订阅和管理组的所有者才能发现这些资源并将其添加到特权标识管理中。 板载后，PIM 功能可用于所有级别的所有者，包括管理组、订阅、资源组和资源。 如果你是全局管理员并尝试为 Azure 资源部署 Privileged Identity Management，可以[提升访问权限以管理所有 Azure 订阅](../../role-based-access-control/elevate-access-global-admin.md?toc=%2fazure%2factive-directory%2fprivileged-identity-management%2ftoc.json)，从而让自己访问目录中的所有 Azure 资源以进行发现。 但是，建议在使用 Privileged Identity Management 管理其资源之前先获取每个订阅所有者的批准。
...
1. 对于组织中的所有全局管理员，找出他们需要该角色的原因。 根据阅读以前的文档，如果人员的工作可以由一个或多个粒度管理员角色执行，则应将其从全局管理员角色中删除，并在 Azure 活动目录内相应地分配（作为参考：Microsoft 目前只有大约 10 名具有全局管理员角色的管理员。 在 [Microsoft 如何使用 Privileged Identity Management](https://www.microsoft.com/itshowcase/Article/Content/887/Using-Azure-AD-Privileged-Identity-Management-for-elevated-access) 中了解详细信息）。

1. 对于所有其他 Azure AD 角色，查看分配列表，确定不再需要该角色的管理员，并在分配中将其删除。

要自动执行最后两个步骤，可以使用特权标识管理中的访问审核。 按照在[特权标识管理中启动 Azure AD 角色的访问审阅](pim-how-to-start-security-review.md)步骤，可以为每个具有一个或多个成员的 Azure AD 角色设置访问审阅。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您使用特权身份管理管理所有全局管理员和安全管理员作为第一步，因为他们在受到危害时可能危害最大。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您使用特权标识管理管理来宾用户管理所有角色，以减少与受攻击的来宾用户帐户相关的风险。

有时人们会认为目录读取者、消息中心读取者和安全读取者等读取者角色相比其他角色来说不那么重要，因为这些角色没有写入权限。 但是，我们看到一些客户也保护这些角色，因为已访问这些帐户的攻击者可能能够读取敏感数据，如个人数据。 在决定是否需要使用 Privileged Identity Management 管理组织中的读者角色时，应考虑到这一点。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您与关键服务的订阅/资源所有者合作，为敏感订阅/资源中的所有角色设置特权标识管理工作流。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您使用特权标识管理管理所有订阅/资源的所有者角色和用户访问管理员角色。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**对 Azure AD 角色和 Azure 资源角色具有零永久活动分配，但建议的[两个中断玻璃紧急访问帐户](../users-groups-roles/directory-emergency-access.md)应具有永久全局管理员角色。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您为具有永久角色分配的用户设置定期访问审核（如果您有任何）。 可通过此部署计划的最后一部分了解有关定期访问评审的详细信息
...
| --- | --- |
| 角色 | 要为其定义设置的角色的名称。 |
| 要求 MFA | 符合条件的用户是否需要在激活角色之前执行 MFA。<br/><br/> ：heavy_check_mark：Microsoft**建议**您对所有管理员角色强制实施 MFA，尤其是在角色具有来宾用户时。 |
| 通知 | 如果设置为 true，当符合条件的用户激活角色时，组织中的全局管理员、特权角色管理员和安全管理员会收到电子邮件通知。<br/><br/>**注：** 某些组织没有与其管理员帐户绑定的电子邮件地址，要获取这些电子邮件通知，应着手设置备用电子邮件地址，以便管理员将收到这些电子邮件。 |
| 事件工单 | 激活角色时符合条件的用户需要记录事件工单号。 此设置可帮助组织使用内部事件编号识别每次激活，以减少无效激活的次数。<br/><br/> ：heavy_check_mark：Microsoft**建议**利用事件票证编号将特权身份管理与内部系统联系起来。 这对于需要激活操作上下文的审批者而言特别有用。 |
| 需要审批 | 符合条件的用户是否需要获取批准才能激活角色。<br/><br/> ：heavy_check_mark：Microsoft**建议**您设置权限最大的角色审批。 基于全体 Privileged Identity Management 客户的使用模式，全局管理员、用户管理员、Exchange 管理员、安全管理员和密码管理员是最常应用审批操作的角色。 |
| 审批者 | 如果符合条件的角色需要执行审批操作来进行激活，应列出批准请求的人员。 默认情况下，Privileged Identity Management 将审批者设置为身份是特权角色管理员的所有用户，无论他们是永久性角色还是合格角色。<br/><br/>**注：** 如果用户既有资格享受 Azure AD 角色，又有资格担任该角色的审批者，则他们将无法自行批准。<br/><br/> ：heavy_check_mark：Microsoft**建议**您选择审批者为那些最了解特定角色及其频繁用户的人，而不是全局管理员。 |
| 激活持续时间 | 在角色到期之前，用户拥有该角色的有效期。 |
| 永久管理员 | 将成为某个角色的永久管理员的用户的列表（永远不必激活）。<br/><br/> ：heavy_check_mark：Microsoft**建议**除全局管理员外的所有角色都拥有零常设管理员。 请参阅本计划的“应向谁分配合格角色”和“应向谁分配永久活动角色”部分，了解更多相关信息。 |
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您将每个 Azure AD 角色的服务所有者作为测试用户，以便他们熟悉该过程并成为推出的内部倡导者。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您在帮助台/支持团队中设置时间，引导他们完成特权身份管理工作流（如果您的组织有内部 IT 支持团队）。 为他们提供合适的文档和联系信息。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您立即处理标记为高严重性的所有警报。 对于严重性为中等和低级别的警报，应随时获取最新相关信息并在认为存在安全威胁时采取措施。
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您为所有 Azure AD 和 Azure 资源角色设置季度访问审核。

在大多数情况下，Azure AD 角色的评审者是用户自己，而 Azure 资源角色的评审者是该角色所在订阅的所有者。 但是，通常情况下公司会拥有未与任何人员的电子邮件地址关联的特权帐户。 在这种情况下，无人审阅访问权限。

> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您为所有具有特权角色分配的帐户添加辅助电子邮件地址，这些帐户未链接到定期检查的电子邮件地址
...
> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您每周至少让一名管理员通读所有审核事件，并每月导出您的审核事件。

如果要在较长时间内自动存储审核事件，Privileged Identity Management 的审核日志将自动同步到 [Azure AD 审核日志](../reports-monitoring/concept-audit-logs.md)中。

> [!TIP]
> ：heavy_check_mark：Microsoft**建议**您设置[Azure 日志监视](../reports-monitoring/concept-activity-logs-azure-monitor.md)以在 Azure 存储帐户中存档审核事件，以进行安全性和合规性。
