...
| --- | --- |
| **严重性** | 低 |
| **为什么收到此警报？** | 如果不执行 MFA，则遭到入侵的用户可以激活特权角色。 |
...
| --- | --- |
| **严重性** | 低 |
| **为什么收到此警报？** | 当前租户没有 Azure AD Premium P2。 |
...
| --- | --- |
| **严重性** | 中型 |
| **为什么收到此警报？** | 具有特权角色但在过去 90 天内未更改密码的帐户。 这些帐户可能是未维护且易受攻击者攻击的服务帐户或共享帐户。 |
...
| --- | --- |
| **严重性** | 高 |
| **为什么收到此警报？** | 在 PIM 外部进行的特权角色分配未受到正确的监视，可能表示正遭到攻击。 |
...
| --- | --- |
| **严重性** | 低 |
| **为什么收到此警报？** | 全局管理员是特权最高的角色。 如果全局管理员遭到入侵，则攻击者可以获取其所有访问权限，使整个系统面临风险。 |
...
| **最小全局管理员数** | 此设置指定你认为不安全的全局管理员数量（2 到 100）。 |
| **全局管理员的百分比** | 此设置指定环境中不安全的全局管理员最小百分比（0% 到 100%）。 |
...
| --- | --- |
| **严重性** | 低 |
| **为什么收到此警报？** | 同一用户多次激活同一特权角色是受到攻击的迹象。 |
