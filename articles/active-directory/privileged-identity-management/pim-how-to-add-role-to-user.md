...
Azure AD 特权标识管理 （PIM） 服务还允许特权角色管理员进行永久管理员角色分配。 此外，特权角色管理员可以使用户**有资格**担任 Azure AD 管理员角色。 符合条件的管理员可在需要时激活角色，在完成任务后，其权限随即失效。
...
1. 在 **"成员资格设置"** 窗格中的 **"分配"类型**列表中，选择 **"合格**"或 **"活动**"。
...
1. 在角色列表中，选择刚刚分配的角色以查看成员列表。
     ...
     分配角色后，您选择的用户将显示在成员列表中，称为 **"符合该角色条件**"。
