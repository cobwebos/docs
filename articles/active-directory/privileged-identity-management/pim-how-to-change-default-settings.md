...
gt
1. 打开**Azure AD 特权标识管理**&gt; **Azure AD** &gt; **角色角色设置**。

    ![列出 Azure AD 角色的角色设置页](./media/pim-how-to-change-default-settings/role-settings.png)
...
在为角色配置设置时，可以选择每个分配类型的两个分配持续时间选项（合格和活动）。 在 Privileged Identity Management 中将用户分配到角色时，这些选项将成为默认的最大持续时间。

您可以选择以下**符合条件的**分配持续时间选项之一：

| | |
| --- | --- |
| **允许永久的合格分配** | 全局管理员和特权角色管理员可以分配永久符合条件的分配。 |
| **过期合格分配后** | 全局管理员和特权角色管理员可以要求所有符合条件的分配具有指定的开始日期和结束日期。 |
...
| **过期活动分配后** | 全局管理员和特权角色管理员可以要求所有活动分配具有指定的开始日期和结束日期。 |

> [!NOTE]
> 全局管理员和特权角色管理员可以续订具有指定结束日期的所有分配。 此外，用户也可启动自助服务请求来[扩展或续订角色分配](pim-resource-roles-renew-extend.md)。
...
在某些情况下，您可能希望将用户分配给角色的持续时间很短（例如，一天）。 在这种情况下，分配的成员不需要请求激活。 在这种情况下，Privileged Identity Management 无法在用户使用其角色分配时强制实施多重身份验证，因为从分配角色时起，用户就已经在角色中处于活动状态。

为了确保完成分配的管理员是他们说的，可以通过选中**活动赋值上的"多重身份验证"** 框，对活动分配强制实施多重身份验证。
...
要在激活之前需要多重身份验证，请在 **"编辑角色设置**的分配"选项卡中选中**激活时需要多重身份验证**框。
...
如果设置多个审批人，审批一旦其中一个审批或拒绝，审批即完成。 不能要求至少两个用户批准。 要需要批准才能激活角色，请按照以下步骤操作。
...
1. 选择至少一个用户，然后单击"**选择**"。 必须至少选择 1 个审批者。 没有默认的审批者。
