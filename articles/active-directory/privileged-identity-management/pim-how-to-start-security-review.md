---
title: 在 PIM-Azure AD 中创建 Azure AD 角色的访问评审Microsoft Docs
...
若要降低与过时角色分配相关的风险，应定期查看访问权限。 您可以使用 Azure AD Privileged Identity Management （PIM）为特权 Azure AD 角色创建访问评审。 还可以配置自动进行的定期访问评审。
...
指定访问评审的设置后，请选择 "**启动**"。 访问评审将显示在列表中，并带有其状态指示器。
