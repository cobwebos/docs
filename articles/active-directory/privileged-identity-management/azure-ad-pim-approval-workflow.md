...
从 2019 年 11 月开始，特权标识管理的 Azure AD 角色部分正在更新为与 Azure 角色体验相匹配的新版本。 这将创建附加功能以及[对现有 API 的更改](azure-ad-roles-features.md#api-changes)。 在推出新版本时，本文中遵循的过程取决于当前拥有的 Privileged Identity Management 版本。 按照本部分中的步骤确定所拥有的 Privileged Identity Management 的版本。 了解 Privileged Identity Management 版本之后，可以选择本文中与该版本匹配的过程。
...
1. 选择 **"批准请求**"。
    ...
    ![批准请求 - 显示查看 Azure AD 角色的请求的页面](./media/azure-ad-pim-approval-workflow/resources-approve-pane.png)
...
- 当被批准的用户在其角色中处于活动状态时，将通知全局管理员和特权角色管理员。

>[!NOTE]
>全局管理员或特权角色管理员认为已批准的用户不应处于活动状态，则可以删除特权标识管理中的活动角色分配。 尽管管理员不会收到挂起请求的通知，除非他们是审批者，否则他们可以通过在特权标识管理中查看挂起的请求来查看和取消所有用户的任何挂起请求。
