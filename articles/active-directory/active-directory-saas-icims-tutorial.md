...
本部分的目的是说明如何基于一个名为“Britta Simon”的测试用户使用 ICIMS 配置和测试 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 ICIMS 用户。 换句话说，需要在 Azure AD 用户与 ICIMS 中相关用户之间建立链接关系。  
...
本部分的目的是在 Azure 经典门户中启用 Azure AD 单一登录并在 ICIMS 应用程序中配置单一登录。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入用户用于登录 ICIMS 应用程序的 URL：`https://<tenant name>.icims.com`。

    b. 单击“资源组名称” 的 Azure 数据工厂。


1. 在“配置 ICIMS 的单一登录”页上，执行以下步骤：
    ...
    b. 单击“资源组名称” 的 Azure 数据工厂。
2. 若要为应用程序配置 SSO，请联系 ICIMS 支持团队，并将下载的元数据文件作为附件通过电子邮件发送。 此外，请提供颁发者 URL、SAML SSO URL 和注销 URL，以便可以针对 SSO 集成配置它们。
...
### <a name="creating-a-icims-test-user"></a>创建 ICIMS 测试用户
...
本部分的目的是通过授予 Britta Simon 访问 ICIMS 的权限，允许她使用 Azure 单一登录。
