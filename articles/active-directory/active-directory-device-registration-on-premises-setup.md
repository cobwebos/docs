...
1. 将 [media]\support\adprep 目录和子目录复制到其中一个 Active Directory 域控制器（其中“[media]”是 Windows Server 2012 R2 安装媒体的路径）。
1. 在命令提示符下，转到 adprep 目录并运行 adprep.exe /forestprep。 遵照屏幕说明完成架构升级。
...
1. 出现 **ServiceAccountName** 提示时，输入选择作为 AD FS 服务帐户的服务帐户名。 如果该帐户为 gMSA 帐户，请以 domain\accountname$ 格式输入该帐户。 对于域帐户，请使用 **domain\accountname** 格式。

### <a name="enable-device-authentication-in-ad-fs"></a>在 AD FS 中启用设备身份验证

1. 在联合服务器上，打开 AD FS 管理控制台，并转到“AD FS” > “身份验证策略”。
1. 从“操作”窗格中选择“编辑全局主要身份验证”。
1. 选中“启用设备身份验证”，然后选择“确定”。
1. 默认情况下，AD FS 会定期从 Active Directory 中删除未使用的设备。 使用 Azure Active Directory 设备注册服务时禁用此任务，以便能够在 Azure 中管理设备。
...
1. 以 UPN 格式输入你的用户名；例如，**dan@contoso.com**。
1. 选择“加入”。
1. 出现提示时，请使用凭据登录。 随后将加入该设备。
...
有关如何使用程序包的说明，请参阅[适用于非 Windows 10 计算机的 Windows Installer 程序包](devices/hybrid-azuread-join-control.md#control-windows-down-level-devices)。
...
1. 找到要向其应用此新访问规则的应用程序。 右键单击该应用程序，然后选择“编辑声明规则”。
1. 选择“颁发授权规则”选项卡，然后选择“添加规则”。
1. 从“声明规则”模板下拉列表中，选择“根据传入声明允许或拒绝用户”。 然后，选择“下一步”。
1. 在“声明规则名称”字段中，键入“允许从已注册的设备访问”。
1. 从“传入声明类型”下拉列表中，选择“为注册用户”。
1. 在“传入声明值”字段中，键入“true”。
1. 选中“允许访问具有此传入声明的用户”单选按钮。
1. 选择“完成”，然后选择“应用”。
1. 删除比你创建的规则更宽松的所有规则。 例如，删除默认规则“允许访问所有用户”。
...
现在，当用户通过未注册到 Azure Active Directory 设备注册服务的设备访问应用程序时，他们将看到错误。
