...
本文介绍 Azure Active Directory 应用程序代理如何提供安全服务来远程发布和访问应用程序。
...
使用[条件性访问](active-directory-conditional-access-azuread-connected-apps.md)可以针对允许哪些流量访问后端应用程序来定义限制。 可以基于位置、身份验证强度和用户风险配置文件，创建限制登录的策略。
...
2. 应用程序代理服务评估该令牌。 它确保用户是颁发的令牌所针对的租户中的公司管理员。 如果用户不是管理员，则终止此过程。
