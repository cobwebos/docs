...
> [!IMPORTANT]
> 短时间，来宾用户权限的新门户控件将仅在使用 URL 时可见 [https://aka.ms/AADRestrictedGuestAccess](https://aka.ms/AADRestrictedGuestAccess) 。 PowerShell 和 Microsoft Graph 仍可用于设置控件，而更改将由门户进行。

1. 用全局管理员权限登录到 [Azure AD 管理中心](https://aka.ms/AADRestrictedGuestAccess) 。
...
我们添加了新的 Microsoft Graph API 来配置 Azure AD 组织中的来宾权限。 可以执行以下 API 调用来分配任何权限级别。 此处使用的 guestUserRoleId 的值是说明最受限制的来宾用户设置。 有关使用 Microsoft Graph 设置来宾权限的详细信息，请参阅 [authorizationPolicy 资源类型](https://docs.microsoft.com/graph/api/resources/authorizationpolicy)。
...
问题 | Answer
-------- | ------
这些权限适用于何处？ | 这些目录级别权限在 Azure AD 服务和门户中强制执行，包括 Microsoft Graph、PowerShell v2、Azure 门户和我的应用门户。 利用 Office 365 组进行协作的 Microsoft 365 服务也会受到影响，特别是 Outlook、Microsoft 团队和 SharePoint。
此功能在 "我的应用" 门户中的哪些部分会受到影响？ | "我的应用" 门户中的组功能将服从这些新权限。 这包括在我的应用中查看组列表和组成员身份的所有路径。 未对组磁贴可用性进行任何更改。 组磁贴可用性仍由 Azure 管理门户中的现有组设置控制。
这些权限是否替代 SharePoint 或 Microsoft 团队来宾设置？ | 不是。 这些现有设置仍然会控制这些应用程序中的体验和访问权限。 例如，如果您在 SharePoint 中看到问题，请仔细检查您的外部共享设置。
...
这些权限是否默认设置？ | 不是。 现有的默认权限保持不变。 您可以选择将权限设置为更严格的权限。
