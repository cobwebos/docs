...
要查看有多少许可证可用，请转到 Azure**活动目录** > **许可证** > **所有产品**。
...
**问题：** 由于当地法律和法规方面的原因，某些 Microsoft 服务不能在所有位置使用。 必须先为用户指定“使用位置”属性，才能将许可证分配给用户。**** 您可以在 Azure 门户中的 **"用户** > **配置文件** > **设置"** 部分下指定位置。
...
**问题：** 在更新用户或组的许可证分配时，您可能会看到某些用户的 Azure AD 邮件和代理地址属性已更改。
...
如果尝试将此产品本身分配到组，门户将返回通知消息。 如果选择项详细信息，会显示以下错误消息：
  ...
  “许可证操作失败。 添加或删除从属服务前，请确保组具有必要的服务。 **微软工作场所分析服务要求启用在线交换（计划 2）。**
