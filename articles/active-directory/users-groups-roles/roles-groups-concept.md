...
Azure Active Directory （Azure AD）正在引入公共预览，可在其中将云组分配到 Azure AD 内置角色。 利用此功能，你可以使用组授予对 Azure AD 的管理员访问权限，从而最大程度地降低全局角色和特权管理员。
...
创建新的 Office 365 或 "isAssignableToRole" 属性设置为 "true" 的安全组。 在 Azure 门户中创建组时，还可以启用此属性，方法是打开**可分配给组 Azure AD 角色**。 无论采用哪种方式，你都可以将组分配给一个或多个 Azure AD 角色，方法与向用户分配角色时相同。 可在单个 Azure AD 组织（租户）中创建最多200个可分配角色的组。
...
- 只有全局管理员和特权角色管理员才能创建角色分配的组（启用了 "isAssignableToRole" 属性）。
...
- 将云组分配到管理单元上的 Azure AD 角色（内置或自定义）。
- 将本地组分配到 Azure AD 角色（内置或自定义）
