---
title: 管理紧急访问管理员帐户 - Azure AD |微软文档
...
- 紧急访问帐户使用的身份验证机制应该不同于其他管理帐户（包括其他紧急访问帐户）使用的机制。  例如，如果管理员通过本地 MFA 正常登录，则 Azure MFA 是不同的机制。  但是，如果 Azure MFA 是管理帐户身份验证的主要部分，请考虑针对这些方法采用其他方法，例如将条件访问与第三方 MFA 提供程序一起使用。
...
但是，至少应有一个紧急访问帐户的多重身份验证机制与其他非紧急帐户不同。 这包括第三方多重身份验证解决方案。 如果具有条件访问策略，要求 Azure AD 和其他连接的软件作为服务 （SaaS） 应用的每个[管理员进行多重身份验证](../authentication/howto-mfa-userstates.md)，则应从此要求中排除紧急访问帐户，并配置不同的机制。 此外，应确保这些帐户不使用按用户的多重身份验证策略。
...
1. [将 Azure AD 登录日志发送到](https://docs.microsoft.com/azure/active-directory/reports-monitoring/howto-integrate-activity-logs-with-log-analytics)Azure 监视器。

### <a name="obtain-object-ids-of-the-break-glass-accounts"></a>获取破碎玻璃帐户的对象号

1. 使用分配给用户管理员角色的帐户登录到[Azure 门户](https://portal.azure.com)。
1. 选择**Azure 活动目录** > **用户**。
1. 搜索碎玻璃帐户并选择用户名。
1. 复制并保存对象 ID 属性，以便以后可以使用它。
1. 对第二个碎玻璃帐户重复上述步骤。

### <a name="create-an-alert-rule"></a>创建警报规则

1. 使用分配给 Azure 监视器中的监视参与者角色的帐户登录到[Azure 门户](https://portal.azure.com)。
1. 选择 **"所有服务**"，在搜索中输入"日志分析"，然后选择**日志分析工作区**。
1. 选择工作区。
1. 在工作区中，选择 **"警报** > **新警报规则**"。
    1. 在 **"资源**"下，验证订阅是要与之关联的警报规则。
    1. 在 **"条件"下**，选择 **"添加**"。
    1. 选择 **"信号名称**"下的**自定义日志搜索**。
    1. 在 **"搜索"查询**下，输入以下查询，插入两个碎玻璃帐户的对象指示。
        > [!NOTE]
        > 对于要包括的每个附加中断玻璃帐户，向查询添加另一个"或 UserId = "ObjectGuid"。

        ![将破玻璃帐户的对象指示添加到警报规则](./media/directory-emergency-access/query-image1.png)
    ...
    1. **在"基于 计算"** 下，选择"**期间（以分钟"表示**您希望查询运行多长时间），选择 **"频率（以分钟"表示**您希望查询运行的频率）。 频率应小于或等于期间。

        ![警报逻辑](./media/directory-emergency-access/alert-image2.png)

    1. 选择 **"完成**"。 您现在可以查看此警报的估计每月成本。
1. 选择要由警报通知的用户的操作组。 如果要创建操作组，请参阅[创建操作组](#create-an-action-group)。
1. 要自定义发送到操作组成员的电子邮件通知，请在 **"自定义操作"** 下选择操作。
1. 在 **"警报详细信息"** 下，指定警报规则名称并添加可选说明。
1. 设置事件**的严重性级别**。 我们建议您将其设置为 **"关键"（Sev 0）。**
1. 在**启用规则创建时**，将其设置为 **"是**"。
1. 要关闭警报一段时间，请选择"**禁止警报**"复选框，然后输入等待时间，然后再再次发出警报，然后选择"**保存**"。
1. 单击“创建警报规则”。****
...
1. 选择 **"编辑详细信息"** 以选择要配置的通知方法并输入所需的联系信息，然后选择 **"确定"** 以保存详细信息。
1. 添加要触发的任何其他操作。
1. 选择“确定”。
