---
title: 使用组管理对 SaaS 应用的访问 - Azure AD |微软文档
...
ms.locfileid: "74026857"
---
# <a name="using-a-group-to-manage-access-to-saas-applications"></a>使用组来管理对 SaaS 应用程序的访问

将 Azure 活动目录 （Azure AD） 与 Azure AD 高级许可证计划一起使用，可以使用组来分配对与 Azure AD 集成的 SaaS 应用程序的访问。 例如，如果要为营销部分配使用五个不同 SaaS 应用程序所需的访问权限，则可以创建一个包含销售部用户的组，然后将该组分配给营销部所需的这五个 SaaS 应用程序。 这样就可以在一个位置管理营销部的成员身份，从而节省了时间。 之后，当将用户添加为营销组的成员时，这些用户将分配到应用程序；当将用户从营销组中删除时，也会从应用程序中删除为其分配的权限。 此功能适用于可从 Azure AD 应用程序库中添加的数百个应用程序。

> [!IMPORTANT]
> 只有在启动 Azure AD 高级试用版或购买 Azure AD 高级许可证计划后，才能使用此功能。
> 仅支持安全组基于组的分配。
