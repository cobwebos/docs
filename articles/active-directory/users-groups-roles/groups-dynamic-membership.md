---
title: Azure Active Directory 中基于属性的动态组成员资格规则 | Microsoft Docs
...
3. 选择“所有组”，然后选择“新组”。
   ...
   ![添加新组](./media/groups-dynamic-membership/new-group-creation.png)
...
4. 在“组”边栏选项卡上，输入新组的名称和说明。 根据是要为用户还是设备创建规则，在“成员身份类型”中选择“动态用户”或“动态设备”，并选择“添加动态查询”。 可以使用规则生成器生成一个简单的规则，或自己编写一个高级规则。 本文包含有关可用的用户和设备属性，以及高级规则示例的详细信息。
   ...
   ![添加动态成员身份规则](./media/groups-dynamic-membership/add-dynamic-group-rule.png)
...
## <a name="status-of-the-dynamic-rule"></a>动态规则的状态

可在动态组的“概述”页上查看成员资格处理状态和上次更新日期。
  
  ![动态组状态显示](./media/groups-dynamic-membership/group-status.png)
...
1. 在所选组的“属性”页中，选择“动态用户”作为“成员身份类型”，然后在说明对组成员身份所做更改的对话框中选择“是”，以便继续操作。 
   ...
   ![选择“动态用户”作为“成员身份类型”](./media/groups-dynamic-membership/select-group-to-convert.png)
  
2. 选择“添加动态查询”，然后提供规则。
  
   ![输入规则](./media/groups-dynamic-membership/enter-rule.png)
