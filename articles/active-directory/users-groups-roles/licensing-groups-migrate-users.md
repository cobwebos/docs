---
title: 添加具有直接许可证的用户 Azure AD 分组许可-|Microsoft Docs
...
可以通过直接分配将现有的许可证部署到组织中的用户;也就是说，使用 PowerShell 脚本或其他工具分配各个用户许可证。 在开始使用基于组的许可来管理组织中的许可证之前，可以使用此迁移计划将现有解决方案无缝地替换为基于组的许可。
...
请考虑删除原始的直接分配。 建议你逐步执行此操作，并首先在用户子集上监视结果。 如果可以保留用户的原始直接分配，但当用户离开其授权组时，他们将保留直接分配的许可证，这可能不是你所需的。
...
1. 使用 Azure 门户将 Office 365 E3 许可证分配给 Azure AD 中的 "**所有用户**" 组。

1. 确认所有用户的许可证分配已完成。 请在组的 "概述" 页上，选择 "**许可证**"，并检查 "**许可证**" 边栏选项卡顶部的 "处理状态"。

   - 查看“已向所有用户应用最新的许可证更改”，确认处理已完成。

   - 查看顶部的通知，了解可能未成功为其分配许可证的用户。 某些用户的许可证是否已用完？ 某些用户的许可证计划是否存在冲突，阻止他们继承组许可证？

1. 定点检查某些用户，确认是否为他们应用了直接许可证和组许可证。 前往用户的 "配置文件" 页，选择 "**许可证**"，并检查许可证的状态。
   ...
   - 选择每个许可证以查看启用了哪些服务。 若要验证直接许可证和组许可证是否为用户启用了完全相同的服务，请选择 "**分配**"。
