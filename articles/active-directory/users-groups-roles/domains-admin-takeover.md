---
title: 管理员接管非托管目录的 Azure Active Directory |Microsoft Docs
...
1. 打开[Microsoft 365 管理中心内](https://admin.microsoft.com)。
2. 选择**用户**选项卡，并创建新的用户帐户具有一个名称，如*用户\@fourthcoffeexyz.onmicrosoft.com*不使用自定义域名。 
3. 请确保新的用户帐户对 Azure AD 租户拥有全局管理员权限。
4. 打开**域**选项卡中的 Microsoft 365 管理中心，选择的域名，然后选择**删除**。 
  
   ![从 Office 365 删除域名](./media/domains-admin-takeover/remove-domain-from-o365.png)
  
5. 如果在 Office 365 中存在任何引用了已删除的域名的用户或组，必须将其重命名到 .onmicrosoft.com 域。 如果强制删除域名，所有用户自动重都命名，在此示例中为*用户\@fourthcoffeexyz.onmicrosoft.com*。
...
> [!NOTE]
> 如果删除了域名，则在 Office 365 租户中分配了许可证的任何 Power BI 或 Azure Rights Management 服务的用户必须保存其仪表板。 它们必须使用类似的用户名登录*用户\@fourthcoffeexyz.onmicrosoft.com*而非*用户\@fourthcoffee.xyz*。
