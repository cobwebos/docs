---
title: 创建或编辑动态组并获取状态 - Azure AD |微软文档
...
---
# <a name="create-or-update-a-dynamic-group-in-azure-active-directory"></a>在 Azure 活动目录中创建或更新动态组
...
1. 使用租户中的全局管理员、Intune 管理员或用户管理员角色中的帐户登录到[Azure AD 管理中心](https://aad.portal.azure.com)。
1. 搜索并选择**组**。
1. 选择“所有组”****，然后选择“新组”****。
...
1. 使用租户中的全局管理员、组管理员、Intune 管理员或用户管理员角色中的帐户登录到[Azure AD 管理中心](https://aad.portal.azure.com)。
1. 选择 **"所有组组** > **All groups**组"。
1. 选择要打开其配置文件的组。
1. 在组的配置文件页上，选择**动态成员资格规则**。 规则生成器支持最多五个表达式。 若要添加五个以上的表达式，必须使用文本框。

   ![为动态组添加成员身份规则](./media/groups-create-rule/update-dynamic-group-rule.png)

1. 要查看可用于成员资格规则的自定义扩展属性：
   1. 选择“获取自定义扩展属性”****
   1. 输入应用程序 ID，然后选择“刷新属性”****。
1. 更新规则后，选择 **"保存**"。
...
如果在处理特定组的成员资格规则时出错误，则该组的“概述”页顶部会显示警报****。 如果无法为租户中的所有组处理超过 24 小时的挂起的动态成员身份更新，则**所有组**的顶部都会显示警报。
