...
在本部分中，将在 Corptax 中创建一个名为 Britta Simon 的用户。 请与  [Corptax 支持团队](https://connect.corptax.com/)协作，将用户添加到 Corptax 平台中。 使用单一登录前，必须先创建并激活用户。
...
在“环境”文本框中键入相应环境，应自动登录到设置了 SSO 的 Corptax  。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
