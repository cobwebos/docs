...
* 可让用户使用其 Azure AD 帐户自动登录到 Zscaler Private Access (ZPA)（单一登录）。
...
4. 在搜索框中键入 Zscaler Private Access (ZPA)，从结果面板中选择“Zscaler Private Access (ZPA)”，然后单击“添加”按钮以添加该应用程序    。
...
要正常使用单一登录，需要建立 Azure AD 用户与 Zscaler Private Access (ZPA) 中相关用户之间的链接关系。

若要配置和测试 Zscaler Private Access (ZPA) 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. **[配置 Zscaler Private Access (ZPA) 单一登录](#configure-zscaler-private-access-zpa-single-sign-on)** - 在应用程序端配置单一登录。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 Zscaler Private Access (ZPA) 测试用户](#create-zscaler-private-access-zpa-test-user)** - 在 Zscaler Private Access (ZPA) 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的 Zscaler Private Access (ZPA) 应用程序集成页上，选择“单一登录”   。
...
4. 在“基本 SAML 配置”  部分中，按照以下步骤操作：
    ...
    ![Zscaler Private Access (ZPA) 域和 URL 单一登录信息](common/sp-identifier.png)

    a. 在“登录 URL”文本框中，使用以下模式键入 URL：`https://samlsp.private.zscaler.com/auth/login?domain=<your-domain-name>` 

    b. 在“标识符(实体 ID)”文本框中，键入 URL：`https://samlsp.private.zscaler.com/auth/metadata` 
...
6. 在“设置 Zscaler Private Access (ZPA)”部分，根据需要复制相应的 URL  。
...
1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”、“Zscaler Private Access (ZPA)”    。
...
在访问面板中单击“Zscaler Private Access (ZPA)”磁贴时，应会自动登录到设置了 SSO 的 Zscaler Private Access (ZPA) 中。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
