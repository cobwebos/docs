...
5. 在 "**管理员凭据**" 部分下，在 "**租户 URL**" 中输入 SCIM 终结点值。 租户 URL 应采用可在 Azure Databricks 主页`https://<region>.azuredatabricks.net/api/2.0/preview/scim` URL 中找到**区域**的格式。 例如， **westus**区域的 SCIM 终结点将为`https://westus.azuredatabricks.net/api/2.0/preview/scim`。 输入先前在**机密令牌**中检索的令牌值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到 Azure Databricks SCIM 连接器。 如果连接失败，请确保 Azure Databricks SCIM 连接器帐户具有管理员权限，然后重试。
...
* 当前针对用户的 Azure Databricks "SCIM API 获取请求是区分大小写的，因此，如果USER@contoso.com我们查询它，则会在将其存储为user@contoso.com时产生0个结果。
