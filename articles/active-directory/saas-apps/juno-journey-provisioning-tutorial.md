---
title: 教程：为 Juno 旅程配置 Azure Active Directory 的自动用户预配 |Microsoft Docs
...
---
# <a name="tutorial-configure-juno-journey-for-automatic-user-provisioning"></a>教程：为自动用户预配配置 Juno 旅程

本教程介绍了需要在 Juno 旅程和 Azure Active Directory （Azure AD）中执行的步骤，以配置自动用户预配。 配置后，Azure AD 使用 Azure AD 预配服务自动预配用户和组，并将其预配到[Juno 旅程](https://www.junojourney.com/)。 有关此服务的功能、工作原理以及常见问题的重要详细信息，请参阅[使用 Azure Active Directory 自动将用户预配到 SaaS 应用程序和取消预配](../manage-apps/user-provisioning.md)。 


## <a name="capabilities-supported"></a>支持的功能
> [!div class="checklist"]
> * 在 Juno 旅程中创建用户
> * 如果用户不再需要访问，请在 Juno 旅程中删除用户
> * 使用户属性在 Azure AD 和 Juno 旅程之间保持同步
> * [单一登录](https://docs.microsoft.com/azure/active-directory/saas-apps/juno-journey-tutorial)到 Juno 旅程（推荐）
...
*  [Juno 旅程租户](https://www.junojourney.com/getstartedwithjuno)。
...
3. 确定要[在 Azure AD 和 Juno 旅程之间映射](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)的数据。 

## <a name="step-2-configure-juno-journey-to-support-provisioning-with-azure-ad"></a>步骤 2。 配置 Juno 旅程以支持 Azure AD 的预配

1. 对于 "**机密令牌**" 和 "**租户 URL** "，请support@the-juno.com联系 Juno 旅程支持团队。 此值将分别在 Azure 门户的 Juno 旅程应用程序的 "预配" 选项卡中输入 "**机密令牌**" 和 "**租户 URL** " 字段。 

## <a name="step-3-add-juno-journey-from-the-azure-ad-application-gallery"></a>步骤 3. 从 Azure AD 应用程序库添加 Juno 旅程

从 Azure AD 应用程序库添加 Juno 旅程，开始管理预配到 Juno 旅程。 如果以前为 SSO 设置了 Juno 旅程，则可以使用相同的应用程序。 但建议您在最初测试集成时创建一个单独的应用程序。 在[此处](https://docs.microsoft.com/azure/active-directory/manage-apps/add-gallery-app)了解有关从库中添加应用程序的详细信息。 
...
* 将用户和组分配到 Juno 旅程时，必须选择 "**默认" 访问权限**以外的角色。 具有默认访问角色的用户将从预配中排除，并在预配日志中被标记为不有效。 如果应用程序上唯一可用的角色是默认访问角色，则可以[更新应用程序清单](https://docs.microsoft.com/azure/active-directory/develop/howto-add-app-roles-in-azure-ad-apps)来添加其他角色。 
...
## <a name="step-5-configure-automatic-user-provisioning-to-juno-journey"></a>步骤 5。 配置 Juno 旅程的自动用户预配 

本部分将指导你完成以下步骤：配置 Azure AD 预配服务，以便基于 Azure AD 中的用户和/或组分配在 TestApp 中创建、更新和禁用用户和/或组。

### <a name="to-configure-automatic-user-provisioning-for-juno-journey-in-azure-ad"></a>若要为 Azure AD 中的 Juno 旅程配置自动用户预配，请执行以下操作：
...
2. 在应用程序列表中，选择“Juno Journey”。****
    ...
    ![应用程序列表中的 Juno 旅程链接](common/all-applications.png)
...
5. 在 "**管理员凭据**" 部分下，输入之前在 "**租户 url**" 中检索到的租户 url 值。 输入先前在 "**机密令牌**" 中检索到的机密令牌值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到 Juno 旅程。 如果连接失败，请确保 Juno 旅程帐户具有管理员权限，然后重试。
...
8. 在 "**映射**" 部分下，选择 "**将 Azure Active Directory 用户同步到 Juno**"。

9. 在 "**属性映射**" 部分中，查看从 Azure AD 同步到 Juno 旅程的用户属性。 选为 "**匹配**" 属性的属性用于匹配 Juno 旅程中的用户帐户以执行更新操作。 如果选择更改[匹配的目标属性](https://docs.microsoft.com/azure/active-directory/manage-apps/customize-application-attributes)，将需要确保 JUNO 旅程 API 支持基于该属性筛选用户。 选择“保存”按钮以提交任何更改****。
   ...
   urn： ietf： params： scim：架构：扩展： enterprise：2.0： User： manager|字符串|
...
11. 若要为 Juno 旅程启用 Azure AD 预配服务，请在 "**设置**" 部分中将 "**预配状态**" 更改为 **"打开**"。

    ![预配状态已打开](common/provisioning-toggle-on.png)

12. 通过在 "**设置**" 部分的 "**范围**" 中选择所需的值，定义要预配到 Juno 旅程的用户和/或组。
