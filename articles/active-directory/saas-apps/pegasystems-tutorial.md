...
1. 在搜索框中，键入“Pega Systems”，在结果面板中选择“Pega Systems”，然后单击“添加”按钮添加该应用程序。
...
1. **[创建 Pega Systems 测试用户](#create-a-pega-systems-test-user)** - 在 Pega Systems 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 身份。
...
1. 在“Pega Systems 域和 URL”部分中，如果要在“IDP”发起的模式下配置应用程序，请执行以下步骤：
...
1. Pega Systems 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 这些声明是客户特定的，并取决于你的要求。 以下可选的声明只是你可以为应用程序配置的示例。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 
