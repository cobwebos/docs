...
* 将用户分配到 Looop 时，必须在分配对话框中选择任何特定于应用程序的有效角色（如果可用）。 将从设置中排除具有**默认访问**角色的用户。
...
5. 在 "**管理员凭据**" 部分中`https://<organisation_domain>.looop.co/scim/v2` ，输入 "**租户 URL**"。 例如，`https://demo.looop.co/scim/v2`。 输入先前从 Looop 中的 "**机密令牌**" 中检索并保存的值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到 Looop。 如果连接失败，请确保 Looop 帐户具有管理员权限，然后重试。
