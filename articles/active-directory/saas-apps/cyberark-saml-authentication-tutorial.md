---
title: 教程：Azure Active Directory 与 CyberArk SAML Authentication 的集成 | Microsoft Docs
...
ms.locfileid: "67112909"
---
# <a name="tutorial-integrate-cyberark-saml-authentication-with-azure-active-directory"></a>教程：将 CyberArk SAML Authentication 与 Azure Active Directory 集成
...
本教程在测试环境中配置并测试 Azure AD SSO。 CyberArk SAML Authentication 支持 **SP 和 IDP** 发起的 SSO
...
2. **[配置 CyberArk SAML Authentication](#configure-cyberark-saml-authentication)** ，以在应用程序端配置 SSO 设置。
...
5. **[创建 CyberArk SAML Authentication 测试用户](#create-cyberark-saml-authentication-test-user)** ，在 CyberArk SAML Authentication 中创建 B.Simon 的对应用户，并将其链接到其在 Azure AD 中的表示形式。
...
### <a name="configure-cyberark-saml-authentication"></a>配置 CyberArk SAML Authentication

若要在 **CyberArk SAML Authentication** 端配置单一登录，需要将下载的“证书(原始)”以及从 Azure 门户复制的相应 URL 发送给 [CyberArk SAML Authentication 支持团队](mailto:bizdevtech@cyberark.com)。  他们会对此进行设置，使两端的 SAML SSO 连接均正确设置
...
在本部分，我们通过授予 B.Simon 访问 CyberArk SAML Authentication 的权限，使其能够使用 Azure 单一登录。
...
在本部分，我们将在 CyberArk SAML Authentication 中创建名为 Britta Simon 的用户。 在  [CyberArk SAML Authentication 支持团队](mailto:bizdevtech@cyberark.com)的配合下，将用户添加到 CyberArk SAML Authentication 平台。 使用单一登录前，必须先创建并激活用户。

### <a name="test-sso"></a>测试 SSO

在访问面板中单击“CyberArk SAML Authentication”磁贴时，应会自动登录到设置了 SSO 的 CyberArk SAML Authentication。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
