---
title: 教程：配置自动用户预配 Azure Active Directory 与 Zscaler |Microsoft Docs
description: 了解如何配置 Azure Active Directory 自动预配和取消其预配到 Zscaler 用户帐户。
...
ms.locfileid: "58086269"
---
# <a name="tutorial-configure-zscaler-for-automatic-user-provisioning"></a>教程：将 Zscaler 配置自动用户预配

本教程的目的是为了演示在 Zscaler 和 Azure Active Directory (Azure AD) 若要配置 Azure AD 自动预配和取消其预配的用户和/或组到 Zscaler 中执行的步骤。
...
> [!NOTE]
> Azure AD 预配集成依赖于 Zscaler SCIM API，可供企业包使用的帐户的 Zscaler 开发人员。

## <a name="adding-zscaler-from-the-gallery"></a>从库中添加 Zscaler
配置自动用户预配 Azure AD 与 Zscaler 之前, 需要从 Azure AD 应用程序库将 Zscaler 添加到托管 SaaS 应用程序的列表。
...
3. 若要添加 Zscaler，请单击**新的应用程序**在对话框顶部的按钮。

    ![“新增应用程序”按钮][3]

4. 在“搜索框”中，键入 Zscaler。

    ![Zscaler 预配](./media/zscaler-provisioning-tutorial/app-search.png)

5. 在结果面板中，选择**Zscaler**，然后单击**添加**按钮将 Zscaler 添加到 SaaS 应用程序列表。

    ![Zscaler 预配](./media/zscaler-provisioning-tutorial/app-search-results.png)

    ![Zscaler 预配](./media/zscaler-provisioning-tutorial/app-creation.png)

## <a name="assigning-users-to-zscaler"></a>将用户分配到 Zscaler

Azure Active Directory 使用称为“分配”的概念来确定哪些用户应收到对所选应用的访问权限。 在自动用户预配的上下文中，只同步已分配到 Azure AD 中的应用程序的用户和/或组。

在配置和启用自动用户预配前, 应确定哪些用户和/或 Azure AD 中的组需要访问到 Zscaler。 确定后，您可以将这些用户和/或组到 Zscaler 分配按照此处的说明：
...
本部分将指导你完成步骤以配置 Azure AD 预配服务以创建、 更新和禁用用户和/或在 Azure AD 中基于用户和/或组分配在 Zscaler 中的组。

> [!TIP]
> 您还可以选择启用基于 SAML 的单一登录 zscaler 中的说明提供[Zscaler 单一登录教程](zscaler-tutorial.md)。 可以独立于自动用户预配配置单一登录，尽管这两个功能互相补充。

### <a name="to-configure-automatic-user-provisioning-for-zscaler-in-azure-ad"></a>若要配置自动用户在 Azure AD 中预配 zscaler:

1. 登录 [Azure 门户](https://portal.azure.com)，并浏览到“Azure Active Directory”>“企业应用程序”>“所有应用程序”。

2. 从 SaaS 应用程序列表中选择 Zscaler。

    ![Zscaler 预配](./media/zscaler-provisioning-tutorial/app-instance-search.png)
...
5. 下**管理员凭据**部分中，输入**租户 URL**并**机密令牌**Zscaler 帐户在步骤 6 中所述。

6. 若要获取**租户 URL**并**机密令牌**，导航到**管理 > 身份验证设置**在 Zscaler 门户用户界面上单击**SAML**下**身份验证类型**。 
...
7. 填入步骤 5 中所示的字段后，单击**测试连接**以确保 Azure AD 可以连接到 Zscaler。 如果连接失败，请确保你的 Zscaler 帐户具有管理员权限，然后重试。
...
10. 下**映射**部分中，选择**Azure Active Directory 用户同步到 Zscaler**。
...
12. 下**映射**部分中，选择**Azure Active Directory 组同步到 Zscaler**。
...
15. 若要启用 Azure AD 预配服务 zscaler，请更改**预配状态**到**上**中**设置**部分。

    ![Zscaler 预配](./media/zscaler-provisioning-tutorial/provisioning-status.png)

16. 选择所需的值中预配到 Zscaler 中定义的用户和/或组你想**作用域**中**设置**部分。
