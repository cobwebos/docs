...
1. 如果要在 SP 发起的模式下配置应用程序，请单击“设置其他 URL”，并执行以下步骤：
    ...
    > [!NOTE]
    > 这些不是实际值。 请使用实际的“标识符”、“回复 URL”和“登录 URL”更新这些值。 请联系 [Talentsoft 客户端支持团队](mailto:advancedservices@talentsoft.com)获取这些值。 还可以参考 Azure 门户中的“基本 SAML 配置”部分中显示的模式。
...
要在 Talentsoft 端配置单一登录，需要将下载的“证书 (Base64)”以及从 Azure 门户复制的相应 URL 发送给 [Talentsoft 支持团队](mailto:advancedservices@talentsoft.com) 。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。

### <a name="create-talentsoft-test-user"></a>创建 Talentsoft 测试用户

在本部分，你将在 Talentsoft 中创建名为 B.Simon 的用户。 在 [Talentsoft 支持团队](mailto:advancedservices@talentsoft.com)的配合下，将用户添加到 Talentsoft 平台。 使用单一登录前，必须先创建并激活用户。
