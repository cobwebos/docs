---
title: 教程：Azure Active Directory 与 IBM OpenPages 集成 | Microsoft Docs
...
1. 在搜索框中，键入“IBM OpenPages”，在结果面板中选择“IBM OpenPages”，然后单击“添加”按钮添加该应用程序。
...
1. **[创建 IBM OpenPages 测试用户](#create-an-ibm-openpages-test-user)** - 在 IBM OpenPages 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在“IBM OpenPages 域和 URL”部分中，执行以下步骤：
...
1. 若要在**IBM OpenPages** 端配置单一登录，需将下载的**元数据 XML** 发送给 [IBM OpenPages 支持团队](https://www.ibm.com/support/home/)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
在本部分中，将在 IBM OpenPages 中创建一个名为 Britta Simon 的用户。 与  [IBM OpenPages 支持团队](https://www.ibm.com/support/home/) 协作，将用户添加到 IBM OpenPages 平台中。 使用单一登录前，必须先创建并激活用户。
