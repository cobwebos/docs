...
本教程的目的是演示要在 Keeper 密码管理器 & 数字保管库和 Azure Active Directory (Azure AD 中执行的步骤，以将) 配置为自动将用户和/或组预配到 Keeper 密码管理器 Azure AD 数字保管库并取消其预配。
...
* 将用户分配到 & 数字保管库的 Keeper 密码管理器时，必须在分配对话框中选择任何特定于应用程序的有效角色 (（如果) 可用）。 具有“默认访问权限”角色的用户排除在预配之外。
