...
本教程介绍如何将 SafeConnect 与 Azure Active Directory (Azure AD) 集成。

将 SafeConnect 与 Azure AD 集成具有以下优势：

- 可在 Azure AD 中控制谁有权访问 SafeConnect。
- 可以让用户使用其 Azure AD 帐户自动登录到 SafeConnect（单一登录）。
...
若要配置 Azure AD 与 SafeConnect 的集成，需备齐以下项目：

- Azure AD 订阅
- 启用了 SafeConnect 单一登录的订阅
...
在本部分中，根据名为“Britta Simon”的测试用户的指示配置和测试 SafeConnect 的 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 SafeConnect 用户。 换句话说，需要建立 Azure AD 用户与 SafeConnect 中相关用户之间的关联关系。

若要配置和测试 SafeConnect 的 Azure AD 单一登录，需要完成以下构建基块：
...
在本部分中，将在 Azure 门户中启用 Azure AD 单一登录并在 SafeConnect 应用程序中配置单一登录。
...
在本部分中，请通过授予 Britta Simon 访问 SafeConnect 的权限，支持其使用 Azure 单一登录。
