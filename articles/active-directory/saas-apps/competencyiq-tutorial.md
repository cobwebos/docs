...
1. [创建 CompetencyIQ 测试用户](#creating-a-competencyiq-test-user) - 在 CompetencyIQ 中有一个与 Azure AD 中的 Britta Simon 相对应的关联用户。
...
1. 在“CompetencyIQ 域和 URL”部分中，执行以下步骤：
...
1. 在“CompetencyIQ Configuration”部分中，单击“配置 CompetencyIQ”打开“配置登录”窗口。 从“快速参考”部分中复制“SAML 实体 ID”和“SAML 单一登录服务 URL”。

    ![配置单一登录](./media/competencyiq-tutorial/tutorial_competencyiq_configure.png) 

1. 若要在 CompetencyIQ 端配置单一登录，需要将下载的元数据 XML、SAML 实体 ID 和 SAML 单一登录服务 URL 发送到 [CompetencyIQ 支持团队](https://www.competencyiq.com/)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
