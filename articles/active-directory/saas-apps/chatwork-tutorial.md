...
本教程介绍如何将 Chatwork 与 Azure Active Directory (Azure AD) 集成。 将 Chatwork 与 Azure AD 集成后，可以：
...
* 配置 Chatwork 后，即可强制实施会话控制，从而实时保护组织的敏感数据以防外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。
...
1. [配置 Chatwork SSO](#configure-chatwork-sso) - 在应用程序端配置单一登录设置。
    1. [创建 Chatwork 测试用户](#create-chatwork-test-user) - 在 Chatwork 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在“设置 Chatwork”部分中，根据要求复制相应的 URL。
...
在本部分中，将通过授予 B.Simon 访问 Chatwork 的权限，允许其使用 Azure 单一登录。
...
单击访问面板中的 Chatwork 磁贴时，应会自动登录到为其设置了 SSO 的 Chatwork。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
...
- [如何通过高级可见性和控制保护 Chatwork](https://docs.microsoft.com/cloud-app-security/proxy-intro-aad)
