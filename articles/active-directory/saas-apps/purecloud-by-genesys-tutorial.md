...
使用名为 **B.Simon** 的测试用户配置和测试 PureCloud by Genesys 的 Azure AD SSO。 若要运行 SSO，需要在 Azure AD 用户与 PureCloud by Genesys 相关用户之间建立链接关系。
...
1. **[配置 PureCloud by Genesys SSO](#configure-purecloud-by-genesys-sso)** - 在应用程序端配置单一登录设置。
...
1. 单击顶部的“管理员”并导航到“集成”下的“单一登录”    。

    ![配置单一登录](./media/purecloud-by-genesys-tutorial/configure01.png)

1. 切换到“ADFS/Azure AD(Premium)”选项卡，然后执行以下步骤  ：
