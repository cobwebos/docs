...
4. 在搜索框中，键入“Skytap”，选择结果面板中的“Skytap”，再单击“添加”按钮以添加应用程序。
...
3. **[创建 Skytap 测试用户](#create-a-skytap-test-user)** - 在 Skytap 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
3. 在“Skytap 域和 URL”部分中，若要在 IDP 发起模式下配置应用程序，请按照以下步骤操作：

    ![“Skytap 域和 URL”单一登录信息](./media/skytap-tutorial/tutorial_skytap_url.png)

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 在“标识符”文本框中，使用以下模式键入 URL：`http://pingone.com/<custom EntityID>`
...
7. 若要在 Skytap 端配置单一登录，需要将下载的元数据 XML 发送给 [Skytap 支持团队](mailto:support@skytap.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
