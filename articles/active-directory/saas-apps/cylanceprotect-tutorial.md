...
1. 在搜索框中，键入“CylancePROTECT”，在结果面板中选择“CylancePROTECT”，然后单击“添加”按钮添加该应用程序。
...
1. **[创建 CylancePROTECT 测试用户](#create-a-cylanceprotect-test-user)** - 在 CylancePROTECT 中创建 Britta Simon 的对应用户，并将其关联到用户的 Azure AD 表示形式。
...
1. 在“CylancePROTECT 域和 URL”部分执行以下步骤：
...
1. 在“SAML 签名证书”部分，单击“证书(Base64)”，然后在计算机上保存证书文件。
...
1. 在“CylancePROTECT 配置”部分，单击“配置 CylancePROTECT”打开“配置登录”窗口。 从“快速参考”部分中复制“注销 URL”、“SAML 实体 ID”和“SAML 单一登录服务 URL”。

    ![CylancePROTECT 配置](./media/cylanceprotect-tutorial/tutorial_cylanceprotect_configure.png) 

1. 若要在 CylancePROTECT 端配置单一登录，需要将下载的证书 (Base64)、注销 URL、SAML 实体 ID 和 SAML 单一登录服务 URL 发送给控制台管理员。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
