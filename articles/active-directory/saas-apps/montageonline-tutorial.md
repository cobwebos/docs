...
3. **[创建 Montage Online 测试用户](#create-a-montage-online-test-user)** - 在 Montage Online 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
3. 在“Montage Online 域和 URL”部分中，执行以下步骤：
...
6. 在“Montage Online 配置”部分，单击“配置 Montage Online”以打开“配置登录”窗口。 从“快速参考”部分中复制“注销 URL”、“SAML 实体 ID”和“SAML 单一登录服务 URL”。

    ![Montage Online 配置](./media/montageonline-tutorial/tutorial_montageonline_configure.png) 

7. 若要在 Montage Online 端配置单一登录，需要将下载的证书 (Base64)、注销 URL、SAML 实体 ID 和 SAML 单一登录服务 URL 发送给 [Montage Online 支持团队](https://www.montage.co.nz/contact-us/)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
