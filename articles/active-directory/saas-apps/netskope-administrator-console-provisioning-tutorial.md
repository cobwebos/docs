...
ms.locfileid: "88554543"
---
# <a name="tutorial-configure-netskope-administrator-console-for-automatic-user-provisioning"></a>教程：为 Netskope 管理员控制台配置自动用户预配

本教程的目的是演示要在 Netskope 管理员控制台和 Azure Active Directory (Azure AD 中执行的步骤，以将) 配置为自动将用户和/或组预配到 Netskope Azure AD 并取消其预配。
...
* 将用户分配到 Netskope 管理员控制台时，必须在分配对话框中选择任何特定于应用程序的有效角色 (（如果) 可用）。 具有“默认访问权限”角色的用户排除在预配之外。
