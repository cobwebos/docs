...
ms.locfileid: "54826507"
---
# <a name="tutorial-azure-active-directory-integration-with-deskradar"></a>教程：Azure Active Directory 与 Deskradar 集成

本教程介绍如何将 Deskradar 与 Azure Active Directory (Azure AD) 集成。
将 Deskradar 与 Azure AD 集成具有以下优势：

* 可在 Azure AD 中控制谁有权访问 Deskradar。
* 可让用户使用其 Azure AD 帐户自动登录到 Deskradar（单一登录）。
...
若要配置 Azure AD 与 Deskradar 的集成，需要准备好以下各项：

* 一个 Azure AD 订阅。 如果你没有 Azure AD 环境，可以在[此处](https://azure.microsoft.com/pricing/free-trial/)获取一个月的试用版。
* 已启用 Deskradar 单一登录的订阅
...
4. 在搜索框中，键入“Deskradar”，在结果面板中选择“Deskradar”，然后单击“添加”按钮添加该应用程序。

     ![结果列表中的 Deskradar](common/search-new-app.png)
...
2. **[配置 Deskradar 单一登录](#configure-deskradar-single-sign-on)** - 在应用程序端配置“单一登录”设置。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的“Deskradar”应用程序集成页上，选择“单一登录”。
...
4. 如果要在 **IDP** 发起的模式下配置应用程序，请在“基本 SAML 配置”部分中执行以下步骤：
    ...
    ![Deskradar 域和 URL 单一登录信息](common/idp-intiated.png)
...
5. 如果要在 SP 发起的模式下配置应用程序，请点击“设置其他 URL”，并执行以下步骤：
    ...
    ![Deskradar 域和 URL 单一登录信息](common/metadata-upload-additional-signon.png)
...
6. Deskradar 应用程序需要特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 在“使用 SAML 设置单一登录”页上，单击“编辑”按钮以打开“用户属性”对话框。
...
9. 在“设置 Deskradar”部分，根据要求复制相应 URL。
...
### <a name="configure-deskradar-single-sign-on"></a>配置 Deskradar 单一登录

1. 使用管理员帐户通过电子邮件和密码登录到 Deskradar 实例，该电子邮件和密码是你通过邀请链接注册时定义的。

2. 单击侧栏中的图标，打开“团队”面板。

3. 切换到“身份验证”选项卡。

4. 在“SAML 2.0”选项卡上执行以下步骤：
...
5. 使用文本编辑器打开已下载的“证书(Base64)”文件，将其内容复制并粘贴到 Deskradar 中的“公共证书”字段。
...
在本部分中，通过向 Britta Simon 授予 Deskradar 的访问权限支持她使用 Azure 单一登录。

1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”、“Deskradar”。

    ![“企业应用程序”边栏选项卡](common/enterprise-applications.png)

2. 在应用程序列表中，选择“Deskradar”。

    ![应用程序列表中的 Deskradar 链接](common/all-applications.png)
...
**若要在 SP 发起的模式中使用应用程序，请执行以下步骤：**

1. 启动 Deskradar 实例，方法是在浏览器中打开 URL：`https://YOURDOMAIN.deskradar.cloud`（将 `YOURDOMAIN` 替换为 Deskradar 实例域）。 
1. 选择“企业单一登录”。

    ![Deskradar](./media/deskradar-tutorial/16-sso-button.jpg)

- 如果已在 Active Directory 上登录，则会直接登录到 Deskradar。
- 如果尚未登录，则会重定向到 Active Directory 的“登录”窗体。 可以在该处输入企业帐户凭据来登录。
- 你会回到 Deskradar，在使用企业帐户凭据成功登录以后，即可登录到 Deskradar。

**若要在 IDP 发起的模式中使用应用程序，请执行以下步骤：**

单击访问面板中的 Deskradar 磁贴时，会自动登录到为其设置了 SSO 的 Deskradar。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
