...
1. **[创建 Atomic Learning 测试用户](#creating-an-atomic-learning-test-user)** - 在 Atomic Learning 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在“Atomic Learning 域和 URL”部分中，执行以下步骤：
...
1. 若要在“Atomic Learning”端配置单一登录，需要将下载的元数据 XML 发送给 [Atomic Learning 支持团队](mailto:cs@atomiclearning.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
