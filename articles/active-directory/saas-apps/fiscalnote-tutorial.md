...
1. 在搜索框中键入“FiscalNote”，在结果面板中选择“FiscalNote”，然后单击“添加”按钮添加该应用程序。
...
1. **[创建 FiscalNote 测试用户](#create-a-fiscalnote-test-user)** - 在 FiscalNote 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
1. 在“FiscalNote 域和 URL”部分中，执行以下步骤：
...
1. FiscalNote 应用程序需要特定格式的 SAML 断言，这要求向“SAML 令牌属性”配置添加自定义属性映射。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。
...
1. 在“FiscalNote 配置”部分，单击“配置 FiscalNote”打开“配置登录”窗口。 从“快速参考”部分中复制“注销 URL”、“SAML 实体 ID”和“SAML 单一登录服务 URL”。

    ![FiscalNote 配置](./media/fiscalnote-tutorial/tutorial_fiscalnote_configure.png) 

1. 若要在 **FiscalNote** 端配置单一登录，需要将下载的“证书(原始)”、“注销 URL”、“SAML 实体 ID”和“SAML 单一登录服务 URL”发送给 [FiscalNote 支持团队](mailto:support@fiscalnote.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
