...
3. [创建 Certain Admin SSO 测试用户](#create-a-certain-admin-sso-test-user) - 在Certain Admin SSO 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
3. 在“Certain Admin SSO 域和 URL”部分中，执行以下步骤：

    ![Certain Admin SSO 域和 URL 单一登录信息](./media/certainadminsso-tutorial/tutorial_certainadminsso_url.png)

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入 URL： `https://<YOUR DOMAIN URL>/svcs/sso_admin_login/handleRequest/<ID>`
...
6. 在“Certain Admin SSO 配置”部分，单击“配置 Certain Admin SSO”打开“配置登录”窗口。 从“快速参考”部分中复制“注销 URL”、“SAML 实体 ID”和“SAML 单一登录服务 URL”。

    ![Certain Admin SSO 配置](./media/certainadminsso-tutorial/tutorial_certainadminsso_configure.png) 

7. 若要在 Certain Admin SSO 端配置单一登录，需要将下载的证书（原始）、注销 URL、SAML 实体 ID 和 SAML 单一登录服务 URL 发送给 [Certain Admin SSO 支持团队](mailto:integrations@certain.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
