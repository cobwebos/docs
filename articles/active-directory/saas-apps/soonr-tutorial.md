...
1. 在搜索框中，键入“Soonr Workplace”。

    ![创建 Azure AD 测试用户](./media/soonr-tutorial/tutorial_soonr_search.png)

1. 在结果面板中，选择“Soonr Workplace”，然后单击“添加”按钮添加该应用程序。
...
1. **[创建 Soonr Workplace 测试用户](#creating-a-soonr-workplace-test-user)** - 在 Soonr Workplace 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在“Soonr Workplace 域和 URL”部分中，执行以下步骤：
...
1. 在“Soonr Workplace 域和 URL”部分中，如果要在“SP 发起的模式”下配置应用程序，请执行以下步骤：
...
1. 在“Soonr Workplace 配置”部分中，单击“配置 Soonr Workplace”打开“配置登录”窗口。 从“快速参考”部分中复制“注销 URL”、“SAML 实体 ID”和“SAML 单一登录服务 URL”。

    ![配置单一登录](./media/soonr-tutorial/tutorial_soonr_configure.png) 

1. 若要在 **Soonr Workplace** 端配置单一登录，需要将下载的**元数据 XML**、**注销 URL、SAML 实体 ID 和 SAML 单一登录服务 URL** 发送给 [Soonr Workplace 支持团队](https://awp.autotask.net/help/)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
