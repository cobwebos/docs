...
1. 在结果窗格中，选择“Lecorpio”，并单击“添加”按钮添加该应用程序。
...
1. **[创建 Lecorpio 测试用户](#creating-a-lecorpio-test-user)** - 在 Lecorpio 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在“Lecorpio 域和 URL”部分中，执行以下步骤：
...
1. 要在 **Lecorpio** 端配置单一登录，需要将下载的**元数据 XML** 发送给 [Lecorpio 支持团队](mailto:info@lecorpio.com)。
