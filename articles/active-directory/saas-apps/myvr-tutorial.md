---
title: 教程：Azure Active Directory 单一登录 (SSO) 与 MyVR 的集成 | Microsoft Docs
...
ms.locfileid: "75530113"
---
# <a name="tutorial-azure-active-directory-single-sign-on-sso-integration-with-myvr"></a>教程：Azure Active Directory 单一登录 (SSO) 与 MyVR 的集成
...
## <a name="configure-and-test-azure-ad-single-sign-on-for-myvr"></a>配置和测试 MyVR 的 Azure AD 单一登录
...
1. **[配置 MyVR SSO](#configure-myvr-sso)** - 在应用程序端配置单一登录设置。
    1. **[创建 MyVR 测试用户](#create-myvr-test-user)** - 在 MyVR 中创建 B.Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
1. MyVR 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 以下屏幕截图显示了默认属性的列表。

    ![image](common/edit-attribute.png)

1. 除了上述属性，MyVR 应用程序还要求在 SAML 响应中传递回更多的属性，如下所示。 这些属性也是预先填充的，但可以根据要求查看它们。
...
在本部分中，通过授予 B.Simon 访问 MyVR 的权限，允许其使用 Azure 单一登录。
...
单击访问面板中的“MyVR”磁贴时，应会自动登录到为其设置了 SSO 的 MyVR。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
...
- [通过 Azure AD 试用 MyVR](https://aad.portal.azure.com/)
