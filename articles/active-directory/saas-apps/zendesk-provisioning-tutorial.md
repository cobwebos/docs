...
5. 在 "**管理员凭据**" 部分下，输入你的 Zendesk 帐户的 "管理员用户名"、"机密令牌" 和 "域"。 这些值的示例如下：
   ...
   * 在 "**管理员用户名**" 框中，填写 Zendesk 租户上的管理员帐户的用户名。 示例为 admin@contoso.com。

   * 在 "**机密令牌**" 框中，填写步骤6中所述的机密令牌。

   * 在 "**域**" 框中，填写 Zendesk 租户的子域。 例如，对于 `https://my-tenant.zendesk.com`的租户 URL 的帐户，你的子域就是**我的租户**。

6. Zendesk 帐户的机密令牌位于**管理员** > **API** > **设置**。 请确保 "**令牌访问**" 设置为 "**已启用**"。
