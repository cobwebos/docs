...
description: 了解如何在 Azure AD 和 Amazon Web Services （AWS）（旧教程）之间配置单一登录。
...
在本教程中，了解如何将 Azure Active Directory （Azure AD）与 Amazon Web Services （AWS）（旧教程）集成。
...
1. 在另一个窗口中打开[Microsoft Graph 资源管理器](https://developer.microsoft.com/graph/graph-explorer)。
    ...
    f. 从提取的服务主体的列表中，获取需要修改的那一个。 还可使用 Ctrl+F 从列出的所有服务主体中搜索应用程序。 您可以使用从 Azure AD "属性" 页复制的 "**对象 ID** " 来使用以下查询，以转到相应的服务主体。
