---
title: 教程：Azure Active Directory 单一登录 (SSO) 与 AppNeta Performance Monitor 集成 | Microsoft Docs
...
本教程介绍如何将 AppNeta Performance Monitor 与 Azure Active Directory (Azure AD) 集成。 将 AppNeta Performance Monitor 与 Azure AD 集成后，可以：

* 在 Azure AD 中控制谁有权访问 AppNeta Performance Monitor。
...
* 已启用 AppNeta Performance Monitor 单一登录 (SSO) 的订阅。
...
1. 在“从库中添加”部分的搜索框中，键入“AppNeta Performance Monitor”   。
1. 从结果面板中选择“AppNeta Performance Monitor”，然后添加该应用  。 在该应用添加到租户时等待几秒钟。


## <a name="configure-and-test-azure-ad-single-sign-on-for-appneta-performance-monitor"></a>为 AppNeta Performance Monitor 配置和测试 Azure AD 单一登录

使用名为 B.Simon 的测试用户配置和测试 AppNeta Performance Monitor 的 Azure AD SSO  。 若要正常使用 SSO，需要在 Azure AD 用户与 AppNeta Performance Monitor 中的相关用户之间建立链接关系。

若要配置和测试 AppNeta Performance Monitor 的 Azure AD SSO，请完成以下构建基块：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. **[配置 AppNeta Performance Monitor SSO](#configure-appneta-performance-monitor-sso)** - 在应用程序端配置单一登录设置。
    1. **[创建 AppNeta Performance Monitor 测试用户](#create-appneta-performance-monitor-test-user)** - 在 AppNeta Performance Monitor 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的“AppNeta Performance Monitor”应用程序集成页上，找到“管理”部分并选择“单一登录”。   
...
1. AppNeta Performance Monitor 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 以下屏幕截图显示了默认属性的列表。

    ![image](common/edit-attribute.png)

1. 除了上述属性，AppNeta Performance Monitor 应用程序还要求在 SAML 响应中传递回更多的属性，如下所示。 这些属性也是预先填充的，但可以根据要求查看它们。
...
1. 在“设置 AppNeta Performance Monitor”部分，根据要求复制相应的 URL  。
...
在本部分，我们通过授予 B.Simon 访问 AppNeta Performance Monitor 的权限，使其能够使用 Azure 单一登录。

1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”。  
1. 在应用程序列表中，选择“AppNeta Performance Monitor”。 
...
## <a name="configure-appneta-performance-monitor-sso"></a>配置 AppNeta Performance Monitor SSO
