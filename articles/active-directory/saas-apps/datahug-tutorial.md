...
* 可让用户使用其 Azure AD 帐户自动登录到 Datahug（单一登录）。
...
4. 在搜索框中键入“Datahug”，在结果面板中选择“Datahug”，然后单击“添加”按钮添加应用程序。

     ![结果列表中的 Datahug](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

在本部分中，基于一个名为“Britta Simon”的测试用户使用 Datahug 配置和测试 Azure AD 单一登录。
若要使单一登录有效，需要在 Azure AD 用户与 Datahug 相关用户之间建立链接关系。

若要配置并测试 Datahug 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. **[配置 Datahug 单一登录](#configure-datahug-single-sign-on)** - 在应用程序端配置单一登录设置。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 Datahug 测试用户](#create-datahug-test-user)** - 在 Datahug 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的 **Datahug** 应用程序集成页上，选择“单一登录”。
...
5. 如果要在 SP 发起的模式下配置应用程序，请单击“设置其他 URL”，并执行以下步骤：
    ...
    > [!NOTE]
    > 这些不是实际值。 请使用实际标识符和回复 URL 更新这些值。 请联系 [Datahug 客户端支持团队](http://datahug.com/about/contact-us/)获取这些值。 还可以参考 Azure 门户中的“基本 SAML 配置”部分中显示的模式。
...
7. 在“SAML 签名证书”部分中，单击“编辑”按钮以打开“SAML 签名证书”对话框并执行以下步骤。
    ...
    ![Communifire 签名选项](./media/datahug-tutorial/tutorial_datahug_signingoption.png)
...
若要在 **Datahug** 端配置单一登录，需要将下载的“联合元数据 XML”以及从 Azure 门户复制的相应 URL 发送给 [Datahug 支持团队](http://datahug.com/about/contact-us/)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”和“Datahug”。

    ![“企业应用程序”边栏选项卡](common/enterprise-applications.png)

2. 在应用程序列表中，选择“Datahug”。

    ![应用程序列表中的 Datahug 链接](common/all-applications.png)
...
单击访问面板中的 Datahug 磁贴时，应当会自动登录到为其设置了 SSO 的 Datahug。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
