...
6. [创建 iProva 测试用户](#create-iprova-test-user) - 在 iProva 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式  。
...
6. 在“用户名”文本框中输入用户的用户名（如 `BrittaSimon@contoso.com`）  。

7. 在“全名”框中输入完整名称，例如“BrittaSimon”   。

8. 选择“无密码(使用单一登录)”选项  。

9. 在“电子邮件地址”框中，输入用户的电子邮件地址（例如 `BrittaSimon@contoso.com`）  。
