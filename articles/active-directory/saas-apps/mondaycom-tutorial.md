...
本教程在测试环境中配置并测试 Azure AD SSO。 monday.com 支持 **SP 和 IDP** 发起的 SSO，并支持**实时**用户预配。
...
2. **[配置 monday.com](#configure-mondaycom)** ，以在应用程序端配置 SSO 设置。
...
5. **[创建 monday.com 测试用户](#create-mondaycom-test-user)** ，在 monday.com 中创建 B.Simon 的对应用户，并将其关联到用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)的“monday.com”应用程序集成页上，找到“管理”部分，选择“单一登录”    。
...
1. monday.com 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 以下屏幕截图显示了默认属性的列表。 单击“编辑”图标以打开“用户属性”对话框  。
...
在访问面板中选择“monday.com”磁贴时，应会自动登录到为其设置了 SSO 的 monday.com。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
