...
* 可在 Azure AD 中控制谁有权访问 monday.com。
* 可让用户使用其 Azure AD 帐户自动登录到 monday.com（单一登录）。
...
4. 在搜索框中，键入“monday.com”，在结果面板中选择“monday.com”，然后单击“添加”按钮添加该应用程序。

    ![结果列表中的 monday.com](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

在本部分中，根据名为“Britta Simon”的测试用户的指示配置和测试 monday.com 的 Azure AD 单一登录。
若要使单一登录有效，需要在 Azure AD 用户与 monday.com 相关用户之间建立链接关系。

若要配置和测试 monday.com 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. **[配置 monday.com 单一登录](#configure-mondaycom-single-sign-on)** - 在应用程序端配置单一登录。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. [创建 monday.com 测试用户](#create-mondaycom-test-user) - 在 monday.com 中创建 Britta Simon 的对应用户，并将其关联到用户的 Azure AD 表示形式。
...
若要配置 monday.com 的 Azure AD 单一登录，请执行以下步骤：

1. 在 [Azure 门户](https://portal.azure.com/)中，在 monday.com 应用程序集成页上，选择“单一登录”。
...
5. 如果要在 SP 发起的模式下配置应用程序，请单击“设置其他 URL”，并执行以下步骤：
    ...
    > [!NOTE]
    > 登录 URL 值不是实际值。 使用实际登录 URL 更新此值。 请联系 [ 客户端支持团队](mailto:support@monday.com)获取此值。 还可以参考 Azure 门户中的“基本 SAML 配置”部分中显示的模式。

6. monday.com 应用程序需要特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 在“使用 SAML 设置单一登录”页上，单击“编辑”按钮以打开“用户属性”对话框。
...
9. 在“设置 monday.com”部分，根据要求复制相应 URL。
...
若要在 monday.com 端配置单一登录，需要将下载的“证书(Base64)”以及从 Azure 门户复制的相应 URL 发送给 [monday.com 支持团队](mailto:support@monday.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
在本部分中，通过授予 Britta Simon 访问 monday.com 的权限，以支持其使用 Azure 单一登录。

1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”和“monday.com”。
...
在本部分中，我们将在 monday.com 中创建一个名为 Britta Simon 的用户。 monday.com 支持默认启用的恰时用户预配。 此部分不存在任何操作项。 如果 monday.com 中不存在用户，则会在身份验证后创建一个新用户。
