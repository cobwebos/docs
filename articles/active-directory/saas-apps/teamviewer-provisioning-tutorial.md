...
5. 在 "**管理员凭据**" 部分中`ttps://webapi.teamviewer.com/scim/v2` ，在 " **Tentant URL** " 字段中输入，并输入之前在**机密令牌**中创建的脚本标记。 单击 "**测试连接**" 以确保 Azure AD 可以连接到 TeamViewer。 如果连接失败，请确保你的 TeamViewer 帐户具有管理员权限，然后重试。
