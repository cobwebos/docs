...
本教程的目的是演示要在 Oracle 合成 ERP 和 Azure Active Directory (Azure AD) 中执行的步骤，以将 Azure AD 配置为自动将用户和/或组预配到 Oracle 合成 ERP 并取消其预配。
...
* 将用户分配到 Oracle 合成 ERP 时，必须在分配对话框中选择任何特定于应用程序的有效角色 (如有) 。 具有“默认访问权限”角色的用户排除在预配之外。
