...
1. 在搜索框中，键入“Boxcryptor”，在结果面板中选择“Boxcryptor”，然后单击“添加”按钮以添加该应用程序。
...
1. **[创建 Boxcryptor 测试用户](#create-a-boxcryptor-test-user)** - 在 Boxcryptor 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在“Boxcryptor 域和 URL”部分中，执行以下步骤：
...
1. 在“Boxcryptor 配置”部分中，单击“配置 Boxcryptor”以打开“配置登录”窗口。 从“快速参考”部分中复制“SAML 单一登录服务 URL”和“SAML 实体 ID”。

    ![Andromeda SCM 配置](./media/boxcryptor-tutorial/tutorial_boxcryptor_configure.png)

1. 若要在 **Boxcryptor** 端配置单一登录，需要将下载的“证书 (Base64)”、“SAML 单一登录服务 URL”和“SAML 实体 ID”发送给 [Boxcryptor 支持团队](mailto:support@boxcryptor.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
在本部分中，将在 Boxcryptor 中创建一个名为 Britta Simon 的用户。 与  [Boxcryptor 支持团队](mailto:support@boxcryptor.com) 协作，在 Boxcryptor 平台中添加需要加入允许列表的用户或域。 如果该团队添加了域，则用户将自动预配到 Boxcryptor 平台。 使用单一登录前，必须先创建并激活用户。
