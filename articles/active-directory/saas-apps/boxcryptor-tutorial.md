...
在本部分中，将在 Boxcryptor 中创建一个名为 Britta Simon 的用户。 请与  [Boxcryptor 客户端支持团队](mailto:support@boxcryptor.com) 协作，将必须添加到允许列表的用户或域添加到 Boxcryptor 平台中。 如果该团队添加了域，则用户将自动预配到 Boxcryptor 平台。 使用单一登录前，必须先创建并激活用户。

### <a name="test-single-sign-on"></a>测试单一登录 

在本部分中，使用访问面板测试 Azure AD 单一登录配置。

单击访问面板中的 Boxcryptor 磁贴时，应会自动登录到为其设置了 SSO 的 Boxcryptor。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
