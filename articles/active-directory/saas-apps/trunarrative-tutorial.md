...
* 配置 TruNarrative 后，可以强制实施会话控制，从而实时保护组织的敏感数据免于外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。
...
在本部分，你将在 TruNarrative 中创建名为 B.Simon 的用户。 在 TruNarrative 支持团队的配合下，将用户添加到 TruNarrative 平台。 使用单一登录前，必须先创建并激活用户。
...
在访问面板中单击“TruNarrative”磁贴时，应会自动登录到设置了 SSO 的 TruNarrative。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
