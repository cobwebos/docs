...
1. 在结果窗格中，选择“Workpath”，然后单击“添加”按钮添加该应用程序。
...
1. [创建 Workpath 测试用户](#creating-a-workpath-test-user) - 在 Workpath 中创建 Britta Simon 的对应用户，将其链接到该用户的 Azure AD 表示形式。
...
1. 在“Workpath 域和 URL”部分，如果要在“IDP 发起的模式”下配置应用程序，请执行以下步骤：
...
1. Workpath 应用程序需要特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 以下屏幕截图显示了此配置的示例。 
...
1. 在“Workpath 配置”部分上，单击“配置 Workpath”，打开“配置登录”窗口。 从“快速参考”部分中复制“注销 URL”、“SAML 实体 ID”和“SAML 单一登录服务 URL”。

    ![配置单一登录](./media/workpath-tutorial/tutorial_workpath_configure.png) 

1. 若要在 Workpath 端配置单一登录，需要将下载的元数据 XML、注销 URL、SAML 实体 ID 和 SAML 单一登录服务 URL 发送给 [Workpath 支持团队](https://help.workpath.com)。 
