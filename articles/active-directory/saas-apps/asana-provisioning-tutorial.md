...
Azure AD 使用称为*分配*的概念来确定哪些用户应接收对选定应用的访问权限。 在自动用户帐户预配的上下文中，只同步已分配到 Azure AD 中的应用程序的用户和组。
...
1. 在“管理员凭据”部分中，按照以下说明生成令牌并将其输入到“机密令牌”**** 文本框中：****
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 使用管理员帐户登录到 [Asana](https://app.asana.com)。

    b.保留“数据库类型”设置，即设置为“共享”。 单击顶部栏中的个人资料照片，然后选择当前的组织名称设置。

    c. 导航到“服务帐户”选项卡。****

    d.单击“下一步”。 单击“添加服务帐户”。****

    e.在“新建 MySQL 数据库”边栏选项卡中，接受法律条款，然后单击“确定”。 根据需要更新“姓名”、“关于”和个人资料照片。******** 复制中的令牌**令牌**，并选择在**保存更改**。

1. 在 Azure 门户中，选择“测试连接”确保 Azure AD 可连接到 Asana 应用****。 如果连接失败，请确保 Asana 帐户具有管理员权限，并重试“测试连接”**** 步骤。

1. 在**通知电子邮件**中输入要接收预配错误通知的个人或组的电子邮件地址。 然后选中复选框。

1. 选择“保存”。****

1. 在 **"映射"** 部分下，选择**将 Azure 活动目录用户同步到 Asana**。
