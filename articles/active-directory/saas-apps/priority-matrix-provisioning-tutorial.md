...
* 将用户分配到优先级矩阵时，必须在分配对话框中选择任何特定于应用程序的有效角色（如果可用）。 将从设置中排除具有**默认访问**角色的用户。
...
5. 在 "**管理员凭据**" 部分中`https://sync.appfluence.com/scim/v2/` ，输入 "**租户 URL**"。 输入先前从**机密令牌**中的优先级矩阵检索并保存的值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到优先级矩阵。 如果连接失败，请确保你的优先级矩阵帐户具有管理员权限，然后重试。
