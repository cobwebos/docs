...
ms.locfileid: "68931829"
---
# <a name="tutorial-integrate-iwellnessnow-with-azure-active-directory"></a>教程：将 iWellnessNow 与 Azure Active Directory 集成

本教程介绍如何将 iWellnessNow 与 Azure Active Directory (Azure AD) 进行集成。 将 iWellnessNow 与 Azure AD 集成后，可以：

* 在 Azure AD 中控制谁有权访问 iWellnessNow。
...
* 启用了 iWellnessNow 单一登录 (SSO) 的订阅。

## <a name="scenario-description"></a>方案描述

本教程在测试环境中配置并测试 Azure AD SSO。

* iWellnessNow 支持 **SP 和 IDP** 发起的 SSO
...
1. 在“从库中添加”部分的搜索框中，键入“iWellnessNow”   。
1. 从结果面板中选择“iWellnessNow”，然后添加该应用  。 在该应用添加到租户时等待几秒钟。

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

使用名为 B. Simon 的测试用户配置和测试 iWellnessNow 的 Azure AD SSO  。 若要运行 SSO，需要在 Azure AD 用户与 iWellnessNow 相关用户之间建立链接关系。

若要配置和测试 iWellnessNow 的 Azure AD SSO，请完成以下构建基块：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
2. **[配置 iWellnessNow SSO](#configure-iwellnessnow-sso)** - 在应用程序端配置单一登录设置。
...
5. **[创建 iWellnessNow 测试用户](#create-iwellnessnow-test-user)** - 在 iWellnessNow 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)的“iWellnessNow”应用程序集成页上，找到“管理”部分，选择“单一登录”    。
...
1. 如果要在 SP  发起的模式下配置应用程序，请单击“设置其他 URL”  ，并执行以下步骤：
    ...
    > [!NOTE]
    > 这些不是实际值。 使用实际登录 URL、标识符和回复 URL 更新这些值。 请联系 [iWellnessNow 客户端支持团队](mailto:info@iwellnessnow.com)来获取这些值。 还可以参考 Azure 门户中的“基本 SAML 配置”  部分中显示的模式。

1. 在“设置 SAML 单一登录”页的“SAML 签名证书”部分中找到“元数据 XML”，选择“下载”以下载该证书并将其保存在计算机上     。

    ![证书下载链接](common/metadataxml.png)

1. 在“设置 iWellnessNow”部分中，根据要求复制相应 URL  。

    ![复制配置 URL](common/copy-configuration-urls.png)

### <a name="configure-iwellnessnow-sso"></a>配置 iWellnessNow SSO

若要在 iWellnessNow  端配置单一登录，需要将下载的“联合元数据 XML”  以及从 Azure 门户复制的相应 URL 发送给 [iWellnessNow 支持团队](mailto:info@iwellnessnow.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
在本部分，我们将通过授予 B.Simon 访问 iWellnessNow 的权限，使其能够使用 Azure 单一登录。
...
### <a name="create-iwellnessnow-test-user"></a>创建 iWellnessNow 测试用户
