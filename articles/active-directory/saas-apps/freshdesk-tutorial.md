...
4. 在搜索框中，键入“FreshDesk”。 在结果窗格中，选择“FreshDesk”，然后选择“添加”按钮添加该应用程序。
...
3. 在“FreshDesk 域和 URL”部分中，执行以下步骤：
...
4. 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 以下屏幕截图显示一个示例。 “用户标识符”的默认值是“user.userprincipalname”，但 **FreshDesk** 要求通过用户的电子邮件地址映射此项。 为此，可以使用列表中的 **user.mail** 属性，或使用基于组织配置的相应属性值。

    ![配置单一登录](./media/freshdesk-tutorial/tutorial_attribute.png)
...
5. 在“SAML 签名证书”部分中，单击“证书(base64)”，并在计算机上保存证书文件。
    ...
    > [!NOTE]
    > 如果有任何问题，请参考此[链接](https://support.freshdesk.com/support/discussions/topics/317543)。

6. 单击“保存”按钮。

    ![配置单一登录](./media/freshdesk-tutorial/tutorial_general_400.png)

7. 如果尚未安装 **OpenSSL**，请在系统中安装它。
...
8. 打开**命令提示符**并运行以下命令：
    ...
    b. 在命令提示符中输入 `openssl x509 -noout -fingerprint -sha256 -inform pem -in certificate.crt` 值。 
    
    > [!NOTE]
    > 此处，**certificate.crt** 是在上一步中生成的输出证书。

    c. 复制**指纹**值并将其粘贴到记事本中。 从指纹中删除冒号并获得最终指纹值。

9. 在“FreshDesk 配置”部分中，单击“配置 FreshDesk”打开“配置登录”窗口。 从“快速参考”部分中复制“SAML 单一登录服务 URL”和“注销 URL”。
...
11. 选择**设置图标**，在“安全性”部分中，执行以下步骤：
    ...
    e. 在“安全证书指纹”文本框中，粘贴你之前删除冒号后获得的**指纹**值。
