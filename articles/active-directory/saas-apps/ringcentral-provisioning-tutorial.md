...
3.  请联系 RingCentral 支持团队matthew.hunt@ringcentral.com ，以获取**SCIM 身份验证令牌**。 此值将在 Azure 门户的 RingCentral 应用程序的 "预配" 选项卡的 "机密令牌" 字段中输入。
...
5. 在 "**管理员凭据**" 部分中`https://platform.ringcentral.com/scim/v2` ，输入 "**租户 URL**"。 输入先前在**机密令牌**中检索到的**SCIM Authentication 令牌**值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到 RingCentral。 如果连接失败，请确保 RingCentral 帐户具有管理员权限，然后重试。
