...
4. 在搜索框中，键入“Replicon”。
    ...
    ![Replicon 应用程序搜索](./media/replicon-provisioning-tutorial/RepliconAppSearch.png)

5. 在结果面板中，选择“Replicon”，然后单击“添加”按钮将 Replicon 添加到 SaaS 应用程序列表。

    ![Replicon 搜索结果](./media/replicon-provisioning-tutorial/RepliconAppSearchResults.png)

    ![Replicon 创建应用程序](./media/replicon-provisioning-tutorial/RepliconAppCreate.png)
...
> [!TIP]
> 还可选择按照 [Replicon 单一登录教程](replicon-tutorial.md)中提供的说明为 Replicon 启用基于 SAML 的单一登录。 可以独立于自动用户预配配置单一登录，尽管这两个功能互相补充。
...
2. 从 SaaS 应用程序列表中选择 Replicon。
    ...
    ![Replicon 预配](./media/replicon-provisioning-tutorial/Replicon2.png)

3. 选择“预配”选项卡。

    ![Replicon 预配](./media/replicon-provisioning-tutorial/RepliconProvisioningTab.png)

4. 将“预配模式”设置为“自动”。

    ![Replicon 预配](./media/replicon-provisioning-tutorial/Replicon1.png)
...
5. 在“管理员凭据”部分下，输入 Replicon 帐户的“管理员用户名”、“管理员密码”、“CompanyId”和“域”。 这些值的示例如下：
    ...
    *   在“CompanyId”字段中，填入 Replicon 租户的 CompanyId。 示例：基于以下登录名的 CompanyID 是 Contoso。
    ...
    ![Replicon 登录](./media/replicon-provisioning-tutorial/RepliconLogin.png)
...
6. 按照 [Replicon 服务帮助](https://www.replicon.com/help/determining-the-url-for-your-service-calls)中所述的步骤，获取 Replicon 租户帐户的“serviceEndpointRootURL”。 获取 URL 后，“域”将突出显示为“serviceEndpointRootURL”的子域。 
    ...
    ![Replicon 预配](./media/replicon-provisioning-tutorial/RepliconEndpoint.png)

7. 填入步骤 5 中所示的字段后，单击“测试连接”以确保 Azure AD 可以连接到 Replicon。 如果连接失败，请确保 Replicon 帐户具有管理员权限，然后重试。

    ![Replicon 预配](./media/replicon-provisioning-tutorial/RepliconTestConnection.png)

8. 在“通知电子邮件”字段中，输入应接收预配错误通知的个人或组的电子邮件地址，并选中复选框“发生故障时发送电子邮件通知”。

    ![Replicon 预配](./media/replicon-provisioning-tutorial/RepliconNotificationEmail.png)

9. 单击“ **保存**”。

10. 在“映射”部分下，选择“将 Azure Active Directory 用户同步到 Replicon”。
    
    ![Replicon 预配](./media/replicon-provisioning-tutorial/RepliconUserMapping.png)

11. 在“属性映射”部分中，查看从 Azure AD 同步到 Replicon 的用户属性。 选为“匹配”属性的特性用于匹配 Replicon 中的用户帐户以执行更新操作。 选择“保存”按钮以提交任何更改。

    ![Replicon 预配](./media/replicon-provisioning-tutorial/RepliconUserMappingAtrributes.png)
...
13. 若要为 Replicon 启用 Azure AD 预配服务，请在“设置”部分中将“预配状态”更改为“启用”。
    ...
    ![Replicon 预配](./media/replicon-provisioning-tutorial/RepliconProvisioningStatus.png)

14. 通过在“设置”部分的“范围”中选择所需的值，定义要预配到 Replicon 的用户和/或组。

    ![Replicon 预配](./media/replicon-provisioning-tutorial/UserGroupSelection.png)

15. 已准备好预配时，单击“保存”。

    ![Replicon 预配](./media/replicon-provisioning-tutorial/SaveProvisioning.png)
