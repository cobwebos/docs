...
本教程介绍如何将 ICIMS 与 Azure Active Directory (Azure AD) 集成。 将 ICIMS 与 Azure AD 集成后，可以：

* 在 Azure AD 中控制谁有权访问 ICIMS。
...
* ICIMS 支持 SP 发起的 SSO 
* 配置 ICIMS 后，可以强制实施会话控制，实时防止组织的敏感数据外泄和渗透。 会话控制从条件访问扩展而来。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-any-app)。
...
1. 在“从库中添加”部分的搜索框中，键入 ICIMS。  
1. 在结果面板中选择“ICIMS”，然后添加该应用。  在该应用添加到租户时等待几秒钟。

## <a name="configure-and-test-azure-ad-single-sign-on-for-icims"></a>为 ICIMS 配置并测试 Azure AD 单一登录

使用名为 B.Simon 的测试用户配置并测试 ICIMS 的 Azure AD SSO。  若要使 SSO 有效，需要在 Azure AD 用户与 ICIMS 相关用户之间建立关联。

若要配置并测试 ICIMS 的 Azure AD SSO，请完成以下构建基块：

1. **[配置 Azure AD SSO](#configure-azure-ad-sso)** - 使用户能够使用此功能。
    * **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 B. Simon 测试 Azure AD 单一登录。
    * **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 B. Simon 能够使用 Azure AD 单一登录。
1. **[配置 ICIMS SSO](#configure-icims-sso)** - 在应用程序端配置单一登录设置。
    * **[创建 ICIMS 测试用户](#create-icims-test-user)** - 在 ICIMS 中创建 B.Simon 的对应用户，并将其关联到用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的“ICIMS”应用程序集成页上，找到“管理”部分并选择“单一登录”。   
...
在本部分，你将通过授予 B.Simon 访问 ICIMS 的权限，使其能够使用 Azure 单一登录。
...
## <a name="configure-icims-sso"></a>配置 ICIMS SSO
...
在本部分，你将在 ICIMS 中创建名为 B.Simon 的用户。 在 [ICIMS 支持团队](https://www.icims.com/contact-us)的配合下，将用户添加到 ICIMS 平台。 使用单一登录前，必须先创建并激活用户。
...
- [在 Azure AD 中试用 ICIMS](https://aad.portal.azure.com/)
