...
3. **[创建 Autotask Endpoint Backup 测试用户](#create-a-autotask-endpoint-backup-test-user)** - 在 Autotask Endpoint Backup 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
3. 在“Autotask Endpoint Backup 域和 URL”部分中，执行以下步骤：
...
6. 若要在“Autotask Endpoint Backup”端配置单一登录，需要将下载的元数据 XML 发送给 [Autotask Endpoint Backup 支持团队](https://backup.autotask.net/help/Content/0_HOME/Support_for_End_Clients.htm)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
