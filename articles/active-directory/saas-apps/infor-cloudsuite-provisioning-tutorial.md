...
* 将用户分配到 Infor CloudSuite 时，必须在分配对话框中选择任何特定于应用程序的有效角色（如果可用）。 将从设置中排除具有**默认访问**角色的用户。
...
5. 在 "**管理员凭据**" 部分中`https://mingle-t20b-scim.mingle.awsdev.infor.com/INFORSTS_TST/v2/scim` ，输入 "**租户 URL**"。 输入先前在**机密令牌**中检索的持有者令牌值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到 Infor CloudSuite。 如果连接失败，请确保 Infor CloudSuite 帐户具有管理员权限，然后重试。
