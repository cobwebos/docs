...
4. 在搜索框中，键入“LiquidFiles”，在结果面板中选择“LiquidFiles”，然后单击“添加”按钮添加该应用程序    。

    ![结果列表中的 LiquidFiles](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

在本部分中，基于一个名为“Britta Simon”的测试用户使用 LiquidFiles 配置和测试 Azure AD 单一登录  。
若要运行单一登录，需要在 Azure AD 用户与 LiquidFiles 相关用户之间建立链接关系。

若要配置并测试 LiquidFiles 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. **[配置 LiquidFiles 单一登录](#configure-liquidfiles-single-sign-on)** - 在应用程序端配置单一登录。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 LiquidFiles 测试用户](#create-liquidfiles-test-user)** - 在 LiquidFiles 中创建 Britta Simon 的对应用户，将其链接到用户的 Azure AD 表示形式。
...
要配置 LiquidFiles 的 Azure AD 单一登录，请执行以下步骤：

1. 在 [Azure 门户](https://portal.azure.com/)中，在“LiquidFiles”应用程序集成页上，选择“单一登录”   。
...
4. 在“基本 SAML 配置”  部分中，按照以下步骤操作：
    ...
    > [!NOTE]
    > 这些不是实际值。 使用实际登录 URL 和标识符更新这些值。 请联系 [LiquidFiles 客户端支持团队](https://www.liquidfiles.com/support.html)获取这些值。 还可以参考 Azure 门户中的“基本 SAML 配置”  部分中显示的模式。
...
### <a name="configure-liquidfiles-single-sign-on"></a>配置 LiquidFiles 单一登录
...
1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”和“LiquidFiles”    。

    ![“企业应用程序”边栏选项卡](common/enterprise-applications.png)

2. 在应用程序列表中，选择“LiquidFiles”  。

    ![应用程序列表中的 LiquidFiles 链接](common/all-applications.png)
...
单击访问面板中的 LiquidFiles 磁贴时，应当会自动登录到为其设置了 SSO 的 LiquidFiles。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
