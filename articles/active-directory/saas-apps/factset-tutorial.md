...
3. **[创建 FactSet 测试用户](#create-a-factset-test-user)** - 在 FactSet 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
3. 在“FactSet 域和 URL”部分，执行以下步骤：

    ![FactSet 域和 URL 单一登录信息](./media/factset-tutorial/tutorial_factset_url.png)

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入 URL： `https://login.factset.com/services/auth-test?idpid=<GUID>`
...
6. 若要在“FactSet”端配置单一登录，需要将下载的“元数据 XML”发送给 [FactSet 支持团队](mailto:support@factset.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
