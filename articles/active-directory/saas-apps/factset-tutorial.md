...
若要在 **FactSet** 端配置单一登录，需要将下载的“联合元数据 XML”以及从 Azure 门户复制的相应 URL 发送给你所联系的 FactSet 支持人员  。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
本部分将在 FactSet 中创建一个名为 Britta Simon 的用户。 请与 FactSet 客户支持代表协作，将用户添加到 FactSet 平台中。 使用单一登录前，必须先创建并激活用户。
