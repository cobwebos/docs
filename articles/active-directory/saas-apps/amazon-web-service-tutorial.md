---
title: 教程： Azure Active Directory 与 Amazon Web Services （AWS）的单一登录（SSO）集成 |Microsoft Docs
...
ms.locfileid: "78668852"
---
# <a name="tutorial-azure-active-directory-single-sign-on-sso-integration-with-amazon-web-services-aws"></a>教程： Azure Active Directory 与 Amazon Web Services （AWS）的单一登录（SSO）集成
...
1. 在“用户”属性中执行以下步骤：
   ...
   1. 在“用户名”字段中输入username@companydomain.extension。 例如，`B.Simon@contoso.com` 。
...
8. 在“创建角色”页中，执行以下步骤：  
    ...
    b. 在 "**选择 saml 2.0 提供程序**" 下，选择之前创建的**SAML 提供程序**（例如： *WAAD*）。
...
> [!NOTE]
> 预配服务仅从 AWS 导入角色到 Azure AD。 服务不会将 Azure AD 中的用户和组设置为 AWS。

> [!NOTE]
> 保存预配凭据之后，必须等待初始同步周期运行。 同步通常大约需要40分钟才能完成。 你可以在 "**设置**" 页底部的 "**当前状态**" 下查看状态。
 ...
 * “预配”部分只支持一次为一个 AWS 租户输入一组凭据。 所有导入的角色都写入到 AWS 租户的 Azure AD `appRoles`[ 对象`servicePrincipal`的 ](https://docs.microsoft.com/graph/api/resources/serviceprincipal?view=graph-rest-beta) 属性。
   ...
   若要解决此问题，可以使用 [Microsoft Graph API](https://docs.microsoft.com/graph/api/resources/serviceprincipal?view=graph-rest-beta) 来提取所有已导入每个 AWS `appRoles`（已在其中配置预配）中的 `servicePrincipal`。 然后，可将这些角色字符串添加到 AWS `servicePrincipal`（已在其中配置 SSO）。
