...
本教程的目的是演示要在动态信号和 Azure Active Directory (Azure AD) 中执行的步骤，以将 Azure AD 配置为自动将用户和/或组预配到动态信号并取消其预配。
...
* 将用户分配到动态信号时，必须在分配对话框中选择任何特定于应用程序的有效角色 (如有) 。 具有“默认访问权限”角色的用户排除在预配之外。
