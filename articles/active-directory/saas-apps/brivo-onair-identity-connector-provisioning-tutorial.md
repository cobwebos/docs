...
本教程的目的是演示要在 Brivo Onair Identity Connector 和 Azure Active Directory （Azure AD）中执行的步骤，以将 Azure AD 自动预配和取消预配到 Brivo Onair 标识连接器。
...
* Azure AD 租户
* [Brivo Onair 标识连接器租户](https://www.brivo.com/quote)
...
5. 在 "**管理员凭据**" 部分下的 "**租户 URL**" 中输入 `https://scim.brivo.com/ActiveDirectory/v2/`。 输入先前在**机密令牌**中检索到的**SCIM Authentication 令牌**值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到 Brivo Onair 标识连接器。 如果连接失败，请确保 Brivo Onair 标识连接器帐户具有管理员权限，然后重试。
