...
* 可让用户使用其 Azure AD 帐户自动登录到 Zscaler Three（单一登录）。
...
本部分将基于名为“Britta Simon”的测试用户配置并测试 Zscaler Three 的 Azure AD 单一登录  。
若要运行单一登录，需要在 Azure AD 用户与 Zscaler Three 中的相关用户之间建立链接关系。
...
5. **[创建 Zscaler Three 测试用户](#create-zscaler-three-test-user)** - 在 Zscaler Three 中创建 Britta Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的 Zscaler Three 应用程序集成页上，选择“单一登录”   。
...
4. 在“基本 SAML 配置”  部分中，按照以下步骤操作：
    ...
    ![Zscaler Three 域和 URL 单一登录信息](common/sp-intiated.png)
...
2. 将扩展添加到浏览器后，单击“安装 Zscaler Three”  会将你定向到 Zscaler Three 应用程序。 在此处，请提供管理员凭据以登录到 Zscaler Three。 浏览器扩展会自动配置应用程序，并自动执行第 3 步到第 6 步。

    ![安装 sso](common/setup-sso.png)
...
1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”和“Zscaler Three”    。

    ![“企业应用程序”边栏选项卡](common/enterprise-applications.png)

2. 在应用程序列表中，选择“Zscaler Three”  。

    ![应用程序列表中的 Zscaler Three 链接](common/all-applications.png)
...
在本部分，我们将在 Zscaler Three 中创建一个名为 Britta Simon 的用户。 Zscaler Three 支持在默认情况下启用的实时预配。 此部分不存在任何操作项。 尝试访问 Zscaler Three 时，如果 Zscaler Three 中尚不存在用户，则系统会创建一个新用户。
