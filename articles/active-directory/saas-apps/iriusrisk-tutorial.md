...
2. **[配置 IriusRisk 单一登录](#configure-iriusrisk-single-sign-on)** - 在应用程序端配置单一登录。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 IriusRisk 测试用户](#create-iriusrisk-test-user)** - 在 IriusRisk 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
4. 在“基本 SAML 配置”  部分中，按照以下步骤操作：
    ...
    > [!NOTE]
    > 登录 URL 值不是实际值。 使用实际登录 URL 更新此值。 请联系 [IriusRisk 客户端支持团队](mailto:info@continuumsecurity.net)来获取此值。 还可以参考 Azure 门户中的“基本 SAML 配置”  部分中显示的模式。
...
### <a name="configure-iriusrisk-single-sign-on"></a>配置 IriusRisk 单一登录

若要在“IriusRisk”端配置单一登录，需要将下载的“联合元数据 XML”以及从 Azure 门户复制的相应 URL 发送给 [IriusRisk 支持团队](mailto:info@continuumsecurity.net)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
### <a name="create-iriusrisk-test-user"></a>创建 IriusRisk 测试用户

在本部分，我们会在 IriusRisk 中创建一个名为 Britta Simon 的用户。 IriusRisk 支持默认已启用的实时用户预配。 此部分不存在任何操作项。 如果 IriusRisk 中尚不存在用户，身份验证后会创建一个新用户。

### <a name="test-single-sign-on"></a>测试单一登录

在本部分中，使用访问面板测试 Azure AD 单一登录配置。

单击访问面板中的 IriusRisk 磁贴时，应会自动登录到为其设置了 SSO 的 IriusRisk。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
