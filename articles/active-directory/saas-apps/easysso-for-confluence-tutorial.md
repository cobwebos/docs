...
* 在 Azure AD 中控制谁有权访问 EasySSO for Confluence。
* 让用户使用其 Azure AD 帐户自动登录到 EasySSO for Confluence。
...
1. 如果要在 SP 发起的模式下配置应用程序，请单击“设置其他 URL”，并执行以下步骤：
    ...
    > [!NOTE]
    > 这些不是实际值。 请使用实际的“标识符”、“回复 URL”和“登录 URL”更新这些值。 请联系 [EasySSO for Confluence 客户端支持团队](mailto:support@techtime.co.nz)来获取这些值。 还可以参考 Azure 门户中的“基本 SAML 配置”部分中显示的模式。
...
1. 使用管理员权限登录到 EasySSO for Confluence 实例，并导航到“管理应用”部分。

    ![管理应用](./media/easysso-for-confluence-tutorial/jira-admin-1.png)
...
1. 选择顶部的“证书”选项卡，系统将显示以下屏幕。找到在之前的 Azure AD SSO 配置步骤中保存的“证书(Base64)”或“元数据文件”。 对于如何继续操作，可使用以下选项：
    ...
    ![元数据 URL](./media/easysso-for-confluence-tutorial/jira-admin-4.png)
...
1. 单击页面底部的“保存”按钮。 你将看到元数据文件或证书文件的内容分析为配置字段。 EasySSO for Confluence 配置已完成。
...
1. 若要获得最佳测试体验，请导航到“外观和感受”选项卡，并选中“SAML 登录按钮”选项。 这将在 Jira 登录屏幕上启用单独的按钮，专门用于测试端到端 Azure AD SAML 集成。 对于生产模式，也可以让此按钮保留启用状态，并配置其位置、颜色和转换。
    ...
    ![外观和感受](./media/easysso-for-confluence-tutorial/jira-admin-5.png)
...
在本部分中，将在 EasySSO for Confluence 中创建一个名为 B.Simon 的用户。 EasySSO for Confluence 支持默认禁用的实时用户预配。 若要启用用户预配，必须在 EasySSO 插件配置的“常规”部分中显式选中“成功登录后创建用户”选项。 如果 EasySSO for Confluence 中尚不存在用户，则会在身份验证后创建一个新用户。

但是，如果不希望在用户首次登录时启用自动用户预配，则用户必须存在于 EasySSO for Confluence 实例使用的后端用户目录（如 LDAP 或 Atlassian Crowd）中。

![用户预配](./media/easysso-for-confluence-tutorial/jira-admin-6.png)

## <a name="test-sso"></a>测试 SSO

在本部分中，使用访问面板测试 Azure AD 单一登录配置。

单击访问面板中的 EasySSO for Confluence 磁贴时，应当会自动登录到为其设置了 SSO 的 EasySSO for Confluence。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
