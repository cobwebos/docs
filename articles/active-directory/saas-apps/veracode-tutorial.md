...
使用名为 **B.Simon** 的测试用户配置和测试 Veracode 的 Azure AD SSO。 若要运行 SSO，需要在 Azure AD 用户与 Veracode 相关用户之间建立链接关系。
...
1. **[配置 Veracode SSO](#configure-veracode-sso)** - 在应用程序端配置单一登录设置。
    * **[创建 Veracode 测试用户](#create-veracode-test-user)** - 在 Veracode 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. Veracode 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 以下屏幕截图显示了默认属性的列表。

    ![image](common/default-attributes.png)

1. 除了上述属性，Veracode 应用程序还要求在 SAML 响应中传递回更多的属性，如下所示。 这些属性也是预先填充的，但可以根据要求查看它们。
