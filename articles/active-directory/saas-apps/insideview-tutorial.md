...
* 可以在 Azure AD 中控制谁有权访问 InsideView。
...
4. 在搜索框中键入 **InsideView**，在结果面板中选择“InsideView”，然后单击“添加”按钮添加该应用程序。

    ![结果列表中的“InsideView”](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

在本部分，我们基于名为 **Britta Simon** 的测试用户来配置并测试 InsideView 的 Azure AD 单一登录。
若要正常使用单一登录，需要在 Azure AD 用户与 InsideView 相关用户之间建立链接关系。

若要配置和测试 InsideView 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. **[配置 InsideView 单一登录](#configure-insideview-single-sign-on)** - 在应用程序端配置单一登录设置。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 InsideView 测试用户](#create-insideview-test-user)** - 在 InsideView 中创建 Britta Simon 的对应用户，并将其关联到其在 Azure AD 中的表示形式。
...
4. 在“基本 SAML 配置”部分中，按照以下步骤操作：
    ...
    ![InsideView 域和 URL 单一登录信息](common/idp-reply.png)

    在“回复 URL”文本框中，使用以下模式键入 URL：`https://my.insideview.com/iv/<STS Name>/login.iv`

    > [!NOTE]
    > 此值不是真实值。 请使用实际回复 URL 更新此值。 请联系 [InsideView 客户端支持团队](mailto:support@insideview.com)获取该值。 还可以参考 Azure 门户中的“基本 SAML 配置”部分中显示的模式。
...
1. 在“添加新 SAML”部分中，执行以下步骤：
    ...
    b. 在“SamlP/WS-Fed 自主提供的终结点”文本框中，粘贴从 Azure 门户复制的“登录 URL”值 ****。
...
2. 在应用程序列表中，选择“InsideView”。
    ...
    ![“应用程序”列表中的“InsideView”链接](common/all-applications.png)
...
### <a name="create-insideview-test-user"></a>创建 InsideView 测试用户

要使 Azure AD 用户能够登录到 InsideView，必须将这些用户预配到 InsideView 中。 对于 InsideView，预配是一项手动任务。
...
在访问面板中单击“InsideView”磁贴时，应会自动登录到设置了 SSO 的 InsideView。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
