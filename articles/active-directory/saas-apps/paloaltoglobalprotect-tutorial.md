...
3. **[创建 Palo Alto Networks - GlobalProtect 测试用户](#create-a-palo-alto-networks---globalprotect-test-user)** - 在 Palo Alto Networks - GlobalProtect 中创建 Britta Simon 的对应用户，以链接到该用户的 Azure AD 表示形式。
...
3. 在“Palo Alto Networks - GlobalProtect 域和 URL”部分中，执行以下步骤：
...
4. Palo Alto Networks - GlobalProtect 应用程序需要采用特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 以下屏幕截图显示一个示例。
...
2. 在应用程序列表中，选择“Palo Alto Networks - GlobalProtect”。
