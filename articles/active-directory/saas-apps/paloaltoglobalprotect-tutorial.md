---
title: 教程：Azure Active Directory 与 Palo Alto Networks - GlobalProtect 集成 | Microsoft 文档
...
1. 从库添加 Palo Alto Networks - GlobalProtect
...
1. 在搜索框中，键入“Palo Alto Networks - GlobalProtect”，从结果面板选择“Palo Alto Networks - GlobalProtect”，然后单击“添加”按钮来添加应用程序。

    ![结果列表中的 Palo Alto Networks - GlobalProtect](./media/paloaltoglobalprotect-tutorial/tutorial_paloaltoglobal_addfromgallery.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

在本部分中，根据名为“Britta Simon”的测试用户，配置和测试 Palo Alto Networks - GlobalProtect 的 Azure AD 单一登录。

为使单一登录能正常工作，Azure AD 需要知道与 Azure AD 用户相对应的 Palo Alto Networks - GlobalProtect 用户。 换句话说，需要建立 Azure AD 用户与 Palo Alto Networks - GlobalProtect 中相关用户之间的关联关系。

可通过将 Azure AD 中“用户名”的值指定为 Palo Alto Networks - GlobalProtect 中“用户名”的值来建立此关联关系。

若要配置和测试 Palo Alto Networks - GlobalProtect 的 Azure AD 单一登录，需要完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
1. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
1. **[创建 Palo Alto Networks - GlobalProtect 测试用户](#create-a-palo-alto-networks---globalprotect-test-user)** - 在 Palo Alto Networks - GlobalProtect 中创建 Britta Simon 的对应用户，以链接到该用户的 Azure AD 表示形式。
1. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
1. **[测试单一登录](#test-single-sign-on)** - 验证配置是否正常工作。

### <a name="configure-azure-ad-single-sign-on"></a>配置 Azure AD 单一登录

在本部分中，可以在 Azure 门户中启用 Azure AD 单一登录，并在 Palo Alto Networks - GlobalProtect 应用程序中配置单一登录。

若要配置 Palo Alto Networks - GlobalProtect 的 Azure AD 单一登录，请执行以下步骤：

1. 在 Azure 门户中的“Palo Alto Networks - GlobalProtect”应用程序集成页上，单击“单一登录”。

    ![配置单一登录链接][4]

1. 在“单一登录”对话框中，选择“基于 SAML 的单一登录”作为“模式”以启用单一登录。
 
    ![“单一登录”对话框](./media/paloaltoglobalprotect-tutorial/tutorial_paloaltoglobal_samlbase.png)

1. 在“Palo Alto Networks - GlobalProtect 域和 URL”部分中，执行以下步骤：
    ...
    > [!NOTE] 
    > 这些不是实际值。 必须使用实际登录 URL 和标识符更新这些值。 若要获取这些值，请联系 [Palo Alto Networks - GlobalProtect 客户端支持团队](https://support.paloaltonetworks.com/support)。 
 
1. Palo Alto Networks - GlobalProtect 应用程序需要采用特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 以下屏幕截图显示一个示例。
    
    ![配置单一登录](./media/paloaltoglobalprotect-tutorial/tutorial_paloaltoglobal_attribute.png)
...
1. 在“单一登录”对话框的“用户属性”部分中，按上图所示配置 SAML 令牌属性，并执行以下步骤：
    ...
    ![配置单一登录](./media/paloaltoglobalprotect-tutorial/tutorial_attribute_05.png)
    
    b. 在“名称”文本框中，键入为该行显示的属性名称。
    
    c. 在“值”列表中，选择为该行显示的属性值。 我们使用 user.userprincipalname 的值作为一个映射示例，你也可以使用适当的值进行映射。 
...
1. 在“SAML 签名证书”部分中，单击“元数据 XML”，并在计算机上保存元数据文件。
    ...
    ![证书下载链接](./media/paloaltoglobalprotect-tutorial/tutorial_paloaltoglobal_certificate.png) 

1. 单击“保存”按钮。

    ![配置单一登录“保存”按钮](./media/paloaltoglobalprotect-tutorial/tutorial_general_400.png)
...
1. 在 Azure 门户的左窗格中，单击“Azure Active Directory”按钮。
    ...
    ![“Azure Active Directory”按钮](./media/paloaltoglobalprotect-tutorial/create_aaduser_01.png)

1. 若要显示用户列表，请转到“用户和组”，然后单击“所有用户”。

    ![“用户和组”以及“所有用户”链接](./media/paloaltoglobalprotect-tutorial/create_aaduser_02.png)

1. 若要打开“用户”对话框，在“所有用户”对话框顶部单击“添加”。

    ![“添加”按钮](./media/paloaltoglobalprotect-tutorial/create_aaduser_03.png)
...
### <a name="create-a-palo-alto-networks---globalprotect-test-user"></a>创建 Palo Alto Networks - GlobalProtect 测试用户

Palo Alto Networks - GlobalProtect 支持实时用户设置，因此，在成功进行身份验证后，如果用户不存在，将会在系统中自动进行创建。 无需在此处执行任何操作。 
...
若要将 Britta Simon 指定到 Palo Alto Networks - GlobalProtect，请执行以下步骤：
...
1. 在应用程序列表中，选择“Palo Alto Networks - GlobalProtect”。
    ...
    ![应用程序列表中的 Palo Alto Networks - GlobalProtect 链接](./media/paloaltoglobalprotect-tutorial/tutorial_paloaltoglobal_app.png)  
