...
1. 需要将元数据证书导入到 F5 (Kerberos) 中，该证书将在稍后的设置过程中使用。 转到“系统”>“证书管理”>“流量证书管理”>>“SSL 证书列表”  。 单击右下角的“导入” 

    ![F5 (Kerberos) 配置](./media/kerbf5-tutorial/configure01.png) 

1. 此外，还需要将 **SSL 证书**用于主机名 (`Kerbapp.superdemo.live`)，在本示例中我们使用了通配符证书。

    ![F5 (Kerberos) 配置](./media/kerbf5-tutorial/configure02.png) 
 
1. 转到“F5 BIG-IP”，单击“访问”>“引导式配置”>“联合身份验证”>“SAML 服务提供程序”  。

    ![F5 (Kerberos) 配置](./media/kerbf5-tutorial/configure03.png) 

    ![F5 (Kerberos) 配置](./media/kerbf5-tutorial/configure04.png)

1. 指定“实体 ID”（与在“Azure AD 应用程序配置”中配置的 ID 相同） 

    ![F5 (Kerberos) 配置](./media/kerbf5-tutorial/configure05.png) 

1. 创建新的虚拟服务器，并指定“目标地址”。  选择我们之前上传的**通配符证书**（或者为应用程序上传的**证书**）和**关联的私钥**。
...
1. 在“单一登录设置”下选择“Kerberos”，然后选择“高级设置”。    需要在 `user@domain.suffix` 中创建请求。

1. 在“用户名来源”下指定 `session.saml.last.attr.name. http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname`。  有关变量和值的完整列表，请参阅附录。
帐户名是创建的 F5 委托帐户（请查看 F5 文档）。

    ![F5 (Kerberos) 配置](./media/kerbf5-tutorial/configure09.png)   

1. 请联系 [F5 (Kerberos) 客户端支持团队](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)获取“终结点检查属性”文档详细信息。 

    ![F5 (Kerberos) 配置](./media/kerbf5-tutorial/configure10.png) 

1. 请联系 [F5 (Kerberos) 客户端支持团队](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45)获取“会话管理属性”文档详细信息。 
