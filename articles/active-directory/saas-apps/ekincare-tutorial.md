...
* 可让用户使用其 Azure AD 帐户自动登录到 eKincare（单一登录）。
...
4. 在搜索框中，键入“eKincare”，在结果面板中选择“eKincare”，然后单击“添加”按钮添加该应用程序    。

     ![结果列表中的 eKincare](common/search-new-app.png)

## <a name="configure-and-test-azure-ad-single-sign-on"></a>配置和测试 Azure AD 单一登录

在本部分中，基于名为“Britta Simon”的测试用户使用 eKincare 配置和测试 Azure AD 单一登录。 
若要运行单一登录，需要在 Azure AD 用户与 eKincare 相关用户之间建立链接关系。

若要配置和测试 eKincare 的 Azure AD 单一登录，需完成以下构建基块：

1. **[配置 Azure AD 单一登录](#configure-azure-ad-single-sign-on)** - 使用户能够使用此功能。
2. **[配置 eKincare 单一登录](#configure-ekincare-single-sign-on)** - 在应用程序端配置单一登录设置。
3. **[创建 Azure AD 测试用户](#create-an-azure-ad-test-user)** - 使用 Britta Simon 测试 Azure AD 单一登录。
4. **[分配 Azure AD 测试用户](#assign-the-azure-ad-test-user)** - 使 Britta Simon 能够使用 Azure AD 单一登录。
5. **[创建 eKincare 测试用户](#create-ekincare-test-user)** - 在 eKincare 中创建 Britta Simon 的对应用户，将其链接到该用户的 Azure AD 表示形式。
...
1. 在 [Azure 门户](https://portal.azure.com/)中的 **eKincare** 应用程序集成页上，选择“单一登录”。 
...
5. eKincare 应用程序需要特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。  在“使用 SAML 设置单一登录”  页上，单击“编辑”  按钮以打开“用户属性”  对话框。
...
6. 在“用户属性”对话框的“用户声明”部分中，通过使用“编辑图标”编辑声明或使用“添加新声明”添加声明，按上图所示配置 SAML 令牌属性，并执行以下步骤     ： 
    ...
    ![显示“用户声明”对话框的屏幕截图，其中已选择“添加新声明”和“保存”按钮。](common/new-save-attribute.png)
...
8. 在“设置 eKincare”部分中，根据要求复制相应的 URL  。
...
### <a name="configure-ekincare-single-sign-on"></a>配置 eKincare 单一登录

若要在 **eKincare** 端配置单一登录，需要将下载的“联合元数据 XML”以及从 Azure 门户复制的相应 URL 发送给 [eKincare 支持团队](mailto:tech@ekincare.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
...
1. 在 Azure 门户中，依次选择“企业应用程序”、“所有应用程序”和“eKincare”    。

    ![“企业应用程序”边栏选项卡](common/enterprise-applications.png)

2. 在应用程序列表中，选择“eKincare”  。

    ![应用程序列表中的 eKincare 链接](common/all-applications.png)
...
单击访问面板中的 eKincare 磁贴时，应当会自动登录到为其设置了 SSO 的 eKincare。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction)（访问面板简介）。
