...
1. 在结果窗格中，选择“eKincare”，然后单击“添加”按钮添加该应用程序。
...
1. [创建 eKincare 测试用户](#creating-a-ekincare-test-user) - 在 eKincare 中创建 Britta Simon 的对应用户，将其链接到该用户的 Azure AD 表示形式。
...
1. 在“eKincare 域和 URL”部分中，执行以下步骤：
...
1. eKincare 应用程序需要特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 以下屏幕截图显示了此配置的一个示例。
...
1. 若要在 eKincare 端配置单一登录，需将下载的 元数据 XML 发送给 [eKincare 支持团队](mailto:tech@ekincare.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
