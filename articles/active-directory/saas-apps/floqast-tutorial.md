...
3. [创建 FloQast 测试用户](#create-a-floqast-test-user) - 在 FloQast 中创建 Britta Simon 的对应用户，并将其链接到用户的 Azure AD 表示形式。
...
3. 在“FloQast 域和 URL”部分，如果要在 IDP 发起的模式下配置应用程序，请执行以下步骤：
...
5. FloQast 应用程序需要特定格式的 SAML 断言。 请为此应用程序配置以下声明。 可以在应用程序集成页的“用户属性”部分管理这些属性的值。 以下屏幕截图显示一个示例。
...
7. 在“SAML 签名证书”部分中，单击“元数据 XML”，并在计算机上保存元数据文件并执行以下步骤：

    ![证书下载链接](./media/floqast-tutorial/tutorial_floqast_certificate.png)

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 选中“显示高级证书签名设置”。
...
9. 若要在 FloQast 端配置单一登录，需要将下载的元数据 XML 发送给 [FloQast 支持团队](mailto:support@floqast.com)。 他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。
