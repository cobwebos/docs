...
1. 记录[支持案例](https://help.joinblink.com/hc/requests/new)或电子邮件**闪烁支持**support@joinblink.com 以请求 SCIM 令牌。 .
...
5. 在 "**管理员凭据**" 部分下的 "**租户 URL**" 中输入 `https://api.joinblink.com/scim`。 输入先前在**机密令牌**中检索到的**SCIM Authentication 令牌**值。 单击 "**测试连接**" 以确保 Azure AD 可以连接到闪烁。 如果连接失败，请确保闪烁帐户具有管理员权限，然后重试。
