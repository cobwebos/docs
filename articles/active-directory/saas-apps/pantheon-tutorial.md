...
1. 在结果面板中，选择“Pantheon”，然后单击“添加”按钮添加该应用程序。
...
1. [创建 Pantheon 测试用户](#creating-a-pantheon-test-user) - 在 Pantheon 中有一个与 Azure AD 中的 Britta Simon 相对应的关联用户。
...
1. Pantheon 应用程序需要特定格式的 SAML 断言，这就需要使用用户的电子邮件地址设置 UserIdentifier 属性值。 默认情况下，Azure AD 将 UserPrincipalName 用于 UserIdentifier 属性。 但为了成功集成，需要将此值调整为与用户的电子邮件地址匹配。 只有在完成相应的映射后，集成才有效。
...
1. 在“Pantheon 配置”部分中，单击“配置 Pantheon”打开“配置登录”窗口。 从“快速参考”部分中复制“SAML 单一登录服务 URL”

    ![配置单一登录](./media/pantheon-tutorial/tutorial_pantheon_configure.png) 

1. 若要在 Pantheon 端配置单一登录，需要将下载的证书和 SAML 单一登录服务 URL 发送给 [Pantheon 支持团队](https://pantheon.io/docs/getting-support/)。
