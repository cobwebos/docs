...
* 配置 Netvision Compas 后，可以强制实施会话控制，从而实时保护组织的敏感数据免于外泄和渗透。 会话控制扩展自条件访问。 [了解如何通过 Microsoft Cloud App Security 强制实施会话控制](https://docs.microsoft.com/cloud-app-security/proxy-deployment-aad)
...
1. **[配置 Netvision Compas SSO](#configure-netvision-compas-sso)** - 在应用程序端配置单一登录设置。
    1. **[创建 Netvision Compas 测试用户](#create-netvision-compas-test-user)** - 在 Netvision Compas 中创建 B.Simon 的对应用户，并将其链接到该用户的 Azure AD 表示形式。
...
1. 在“设置 Netvision Compas”部分，根据要求复制相应的 URL。 
...
若要在 **Netvision Compas** 端配置单一登录，需要将下载的“证书(Base64)”以及从 Azure 门户复制的相应 URL 发送给 [Netvision Compas 支持团队](mailto:contact@net.vision)。  他们会对此进行设置，使两端的 SAML SSO 连接均正确设置。

### <a name="create-netvision-compas-test-user"></a>创建 Netvision Compas 测试用户

在本部分，你将在 Netvision Compas 中创建名为 B.Simon 的用户。 在 [Netvision Compas 支持团队](mailto:contact@net.vision)的配合下，将用户添加到 Netvision Compas 平台。 使用单一登录前，必须先创建并激活用户。
