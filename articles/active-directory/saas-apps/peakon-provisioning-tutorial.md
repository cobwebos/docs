...
本教程的目的是演示要在 Peakon 和 Azure Active Directory (Azure AD) 中执行的步骤，以将 Azure AD 自动预配和取消预配到 Peakon。
...
* 将用户分配到 Peakon 时，必须在分配对话框中选择任何特定于应用程序的有效角色 (如有) 。 具有“默认访问权限”角色的用户排除在预配之外。
...
* 必须从 Peakon 的自定义 SCIM 用户扩展中扩展 Peakon 中的所有自定义用户属性 `urn:ietf:params:scim:schemas:extension:peakon:2.0:User` 。
