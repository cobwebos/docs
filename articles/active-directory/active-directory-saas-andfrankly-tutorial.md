...
本部分的目的是基于名为“Britta Simon”的测试用户展示如何配置并测试 &frankly 的 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 &frankly 用户。 换句话说，需要建立 Azure AD 用户与 &frankly 中相关用户之间的关联关系。
...
在本部分中，在经典门户中启用 Azure AD 单一登录，并在 &frankly 应用程序中配置单一登录。
...
4. 如果要在“配置应用设置”对话框页上以“SP 发起的模式”配置应用程序，则单击“显示高级设置(可选)”，然后输入“登录 URL”，并单击“下一步”。
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，使用以下模式键入 URL：`https://andfrankly.com/saml/okta/?saml_sso=<tenant id>`
...
本部分的目的是通过授予 Britta Simon 访问 &frankly 的权限，允许她使用 Azure 单一登录。
