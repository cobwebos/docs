...
* 可以让用户使用其 Azure AD 帐户自动登录到 CloudPassage 单一登录 (SS0)
...
* 不应使用生产环境，除非有此必要。
* 如果没有 Azure AD 测试环境，可以获取免费的[一个月试用版订阅](https://azure.microsoft.com/pricing/free-trial/)。 
...
## <a name="add-cloudpassage-from-the-gallery"></a>从库中添加 CloudPassage
...
本部分的目的是说明如何基于名为“Britta Simon”的测试用户配置和测试 CloudPassage 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 CloudPassage 用户。 换句话说，需要在 Azure AD 用户与 CloudPassage 中的相关用户之间建立链接关系。  

可以通过将 Azure AD 中“用户名”的值分配为 CloudPassage 中“用户名”的值来建立此链接关系。

若要配置和测试 CloudPassage 的 Azure AD SSO，需要完成以下构建基块：
...
本部分旨在介绍如何在 Azure AD 经典门户中启用 Azure AD SSO 并在 CloudPassage 应用程序中配置 SSO。  

CloudPassage 应用程序需要特定格式的 SAML 断言，这要求向 SAML 令牌属性配置添加自定义属性映射。 
...
3. 在“配置应用设置”对话框页上，执行以下步骤： 
    ...
    ![配置应用设置][9]   
  1. 在“登录 URL”文本框中，键入用户用来登录 CloudPassage 应用的 URL（例如：*https://portal.cloudpassage.com/saml/init/accountid*）。  
  2. 在“回复 URL”文本框中，键入 AssertionConsumerService URL（例如：*https://portal.cloudpassage.com/saml/consume/accountid*）。 可以通过在 CloudPassage 门户的“单一登录设置”部分中单击“SSO 设置文档”来获取此属性的值。
...
8. 在“单一登录设置”部分中，执行以下步骤： 
    ...
    ![配置单一登录][14]
  1. 在 Azure 经典门户中，在“在 CloudPassage 处配置单一登录”对话框页上，复制“颁发者 URL”值，然后将其粘贴到“SAML 颁发者 URL”文本框中。
  2. 在 Azure 经典门户中，在“在 CloudPassage 处配置单一登录”对话框页上，复制“服务提供商(SP)启动的终结点”值，然后将其粘贴到“SAML 终结点 URL”文本框中。
  3. 在 Azure 经典门户中，在“在 CloudPassage 处配置单一登录”对话框页上，复制“注销 URL”值，然后将其粘贴到“注销登陆页面”文本框中。
  4. 基于下载的证书创建一个 **base-64** 编码的文件。 

    >[AZURE.TIP] 有关详细信息，请参阅 [How to convert a binary certificate into a text file](http://youtu.be/PlgrzUZ-Y1o)（如何将二进制证书转换为文本文件）。
  5. 在记事本中打开 base-64 编码的证书，将其内容复制到剪贴板，然后再粘贴到“x 509 证书”文本框中。
  6. 单击“保存” 。
...
4. 若要添加必需的用户属性，请对下表中的每一行执行以下步骤： 
  ...
  1. 单击“添加用户属性”。 
    ...
    ![配置单一登录][18]
  2. 在“属性名称”文本框中，键入为该行显示的属性名称，对于“属性值”，选择为该行显示的属性值。 
...
### <a name="create-a-cloudpassage-test-user"></a>创建 CloudPassage 测试用户
...
4. 在“添加新用户”部分中，执行以下步骤： 
  ...
  1. 在“名字”文本框中，键入“Britta”。 
  2. 在“姓氏”文本框中，键入“Simon”。
  3. 在“用户名”文本框、“电子邮件”文本框和“重新键入电子邮件”文本框中，键入 Britta 在 Azure AD 中的用户名。
  4. 对于“访问类型”，选择“启用 Halo 门户访问”。
  5. 单击“添加”。

### <a name="assign-the-azure-ad-test-user"></a>分配 Azure AD 测试用户
本部分的目的是通过向 Britta Simon 授予对 CloudPassage 的访问权限，使她能够使用 Azure SSO。
