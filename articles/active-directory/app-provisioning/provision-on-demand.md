...
* 确保向目标应用程序提供了有效凭据，如机密令牌和租户 URL。 所需的凭据因应用程序而异。 有关详细的配置教程，请参阅[教程列表](https://docs.microsoft.com/azure/active-directory/saas-apps/tutorial-list)。 
* 请确保目标应用程序支持对 "**属性映射**" 窗格中定义的匹配属性进行筛选。 你可能需要检查应用程序开发人员提供的 API 文档，以了解支持的筛选器。
* 对于跨域标识管理（SCIM）应用程序的系统，可以使用 Postman 等工具。 此类工具可帮助你确保应用程序以 Azure Active Directory （Azure AD）预配服务预期的方式响应授权请求。 查看[示例请求](https://docs.microsoft.com/azure/active-directory/app-provisioning/use-scim-to-provision-users-and-groups#request-3)。
...
接下来，预配服务确定用户是否处于预配的[范围](https://docs.microsoft.com/azure/active-directory/app-provisioning/how-provisioning-works#scoping)内。 该服务会考虑以下几个方面：
...
* 请确保定义了有效的范围角色。 例如，避免将[Greater_Than 运算符](https://docs.microsoft.com/azure/active-directory/app-provisioning/define-conditional-rules-for-provisioning-user-accounts#create-a-scoping-filter)用于非整数值。
* 如果用户没有必要的角色，请查看[有关设置分配给默认访问角色的用户的提示](https://docs.microsoft.com/azure/active-directory/app-provisioning/application-provisioning-config-problem-no-users-provisioned#provisioning-users-assigned-to-the-default-access-role)。
...
* 导出更改失败可能会有很大差异。 请查看[文档，](https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-provisioning-logs#error-codes)了解常见故障的设置日志。
