...
| 任务 | 引用 |
| --- | --- |
| 在 Azure Active Directory 租户中启用设备注册，以允许设备加入工作区。 默认情况下，不会为该服务启用多重身份验证。 但是，在注册设备时，建议启用多重身份验证。 在 ADRS 中启用多重身份验证之前，请确保为多重身份验证提供程序配置了 AD FS。 |[启用 Azure Active Directory 设备注册](active-directory-conditional-access-device-registration-overview.md) |
...
| 可以使用管理员门户查看和启用/禁用已注册的设备。 在此任务中，你将使用管理员门户查看一些已注册的设备。 |[Azure Active Directory 设备注册概述](active-directory-conditional-access-device-registration-overview.md) |
