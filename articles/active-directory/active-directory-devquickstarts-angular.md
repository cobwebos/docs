...
2. 通过在页面右上角单击你的帐户选择你的 Azure AD 租户。
3. 在左侧的导航窗格中，单击“Azure Active Directory”。
4. 单击“应用注册”并单击“添加”。
5. 为应用程序输入一个友好的名称，例如“TodoListService”，选择“Web 应用程序和/或 Web API”作为应用程序类型。
6. 对于登录 URL，请输入示例的基 URL，例如 `https://localhost:8080`。
7. 完成注册后，Azure AD 将为应用程序分配一个唯一的客户端标识符，即应用程序 ID。 在后面的部分中将会用到此值，因此，请从应用程序页复制此值。
8. 在设置页中，配置“回复 URL”，这是 AAD 返回令牌的位置。 本示例的默认位置是 `https://localhost:44326/`

* Adal.js 使用 OAuth 隐式流来与 Azure AD 通信。  你必须通过执行以下操作，为应用程序启用隐式流：
  * 从应用程序页单击“清单”，以打开清单编辑器并找到 `oauth2AllowImplicitFlow` 属性。 将其值设置为 `true`。
  * 保存清单。
