...
1. 在事件查看器中打开 "**用户设备注册**" 事件日志。 位于 "**应用程序和服务日志**" 下 > **Microsoft** > **Windows** > **用户设备注册**
...
- 未能连接并从发现终结点提取发现元数据。
   - 设备应能够访问系统上下文中 `https://enterpriseregistration.windows.net`，以发现注册和授权终结点。 
   - 如果本地环境需要出站代理，则 IT 管理员必须确保设备的计算机帐户能够发现并以无提示方式向出站代理进行身份验证。
- 未能连接到用户领域终结点并执行领域发现。 （仅限 Windows 10 版本1809及更高版本）
   - 设备应能够访问系统上下文中 `https://login.microsoftonline.com`，以便为验证的域执行领域发现，并确定域类型（托管/联合）。
...
- **DSREG_AUTOJOIN_DISC_WAIT_TIMEOUT** （0x801c001f/-2145648609）
   - 原因：执行发现时操作超时。
   - 解决方法：确保在系统上下文中可访问 `https://enterpriseregistration.windows.net`。 有关详细信息，请参阅[网络连接要求](hybrid-azuread-join-managed-domains.md#prerequisites)部分。
...
- **ERROR_ADAL_INTERNET_TIMEOUT** （0xcaa82ee2/-894947614）
   - 原因：常规网络超时。
   - 解决方法：确保在系统上下文中可访问 `https://login.microsoftonline.com`。 确保本地标识提供程序可在系统上下文中访问。 有关详细信息，请参阅[网络连接要求](hybrid-azuread-join-managed-domains.md#prerequisites)。
...
- **ERROR_ADAL_INTERNET_CANNOT_CONNECT** （0xcaa82efd/-894947587）
   ...
   - 解决方法：检查与 `https://login.microsoftonline.com`的网络连接。
...
2. 执行步骤来重现问题。
3. 通过执行 `stop_ngc_tracing_public.cmd`停止运行日志记录脚本。
4. 压缩并发送 `%SYSTEMDRIVE%\TraceDJPP\*` 下的日志以进行分析。
