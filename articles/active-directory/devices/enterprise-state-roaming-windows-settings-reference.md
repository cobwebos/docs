---
title: Windows 10 漫游设置参考 - Azure 活动目录
description: 将在 Windows 10 中使用 ESR 漫游或备份的设置
...
最终用户可以使用以下设置组在 Windows 10 设备上启用/禁用设置同步。
...
在下表中，"设置组"列中的其他条目引用可以通过转到"设置>帐户>同步设置>其他 Windows 设置来禁用的设置。 
...
| **帐户**：帐户图片 |sync |X |主题 |
| **帐户**：其他帐户设置 |X |X | |
| **高级移动宽带**：互联网连接共享网名（通过蓝牙自动发现移动 Wi-Fi 热点） |X |X |密码 |
...
| **Microsoft Edge 浏览器**：热门网站 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：键入的 URL <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：收藏夹栏设置 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：显示主页按钮 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：阻止弹出窗口 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：询问如何处理每个下载 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：提出保存密码 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：发送 Do Not Track 请求 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：保存窗体项 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：键入时显示搜索和站点建议 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：cookie 首选项 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：允许站点保存设备中受保护的媒体许可证 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
| **Microsoft Edge 浏览器**：屏幕阅读器设置 <sup>[[1]](#footnote-1)</sup> |sync |sync |internal |
