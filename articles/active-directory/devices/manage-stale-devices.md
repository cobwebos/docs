...
description: 了解如何从 Azure 活动目录中的已注册设备的数据库中删除陈旧设备。
...
---
# <a name="how-to-manage-stale-devices-in-azure-ad"></a>如何：在 Azure AD 中管理陈旧设备
...
- 设备数量的增加会产生不必要的设备回写，从而增加 Azure AD 连接同步的时间。
...
- 已触发需要[托管设备](../conditional-access/require-managed-devices.md)或[已批准客户端应用](../conditional-access/app-based-conditional-access.md)的条件访问策略。
- 已加入 Azure AD 或已加入混合 Azure AD 的 Windows 10 设备在网络中处于活动状态。 
- Intune 受管理设备已签入服务。

如果活动时间戳的现有值和当前值之间的增量超过 14 天（+/-5 天差异），则现有值将替换为新值。
...
定义作为陈旧设备指标的时间范围。 定义时间帧时，将用于将活动时间戳更新到值中的窗口进行分解。 例如，不应将小于 21 天的时间戳（包括方差）视为陈旧设备的指示器。 在某些情况下，某个设备看似是陈旧设备，但实际上并不是。 例如，相关设备的所有者可能正在度假或请了病假。  超过陈旧设备的时间范围。
...
不要删除系统管理的设备。 这些通常是设备，如自动驾驶仪。 删除后，无法重新预配这些设备。 默认情况下，新的 `get-msoldevice` cmdlet 可以排除系统管理的设备。 
...
- **Windows 7/8** - 首先禁用或删除本地 AD 中的 Windows 7/8 设备。 无法使用 Azure AD Connect 在 Azure AD 中禁用或删除 Windows 7/8 设备。 相反，在本地进行更改时，必须在 Azure AD 中禁用/删除。

> [!NOTE]
>* 删除本地 AD 或 Azure AD 中的设备不会删除客户端上的注册。 它只会阻止使用设备作为标识（例如条件访问）访问资源。 阅读有关如何[删除客户端注册](faq.md#hybrid-azure-ad-join-faq)的其他信息。
>* 仅在 Azure AD 中删除 Windows 10 设备将使用 Azure AD 连接从本地重新同步设备，但作为处于"挂起"状态的新对象。 设备上需要重新注册。
>* 从 Windows 10/Server 2016 设备的同步作用域中删除设备将删除 Azure AD 设备。 将其重新添加到同步作用域将使新对象处于"挂起"状态。 需要重新注册设备。
>* 如果不对 Windows 10 设备使用 Azure AD 连接进行同步（例如，仅使用 AD FS 进行注册），则必须管理类似于 Windows 7/8 设备的生命周期。
...
> [!NOTE]
>* 删除 Azure AD 设备不会删除客户端上的注册。 它只会阻止使用设备作为标识（例如条件访问）访问资源。 
>* 阅读有关如何[在 Azure AD 上取消联接的更多内容](faq.md#azure-ad-join-faq) 
...
> [!NOTE]
>* 删除 Azure AD 注册的设备不会删除客户端上的注册。 它只会阻止使用设备作为标识（例如条件访问）访问资源。
>* 阅读有关如何[删除客户端注册](faq.md#azure-ad-register-faq)的详细信息

## <a name="clean-up-stale-devices-in-the-azure-portal"></a>在 Azure 门户中清理陈旧的设备  

可以在 Azure 门户中清理陈旧的设备，但使用 PowerShell 脚本可以更高效地处理此过程。 使用最新的 PowerShell V1 模块使用时间戳筛选器并筛选出系统管理的设备（如自动驾驶仪）。 目前不建议使用 PowerShell V2。
...
如果目录中有大量设备，请使用时间戳筛选器缩小返回的设备数量。 获取时间戳超过特定日期的所有设备并将返回的数据存储在 CSV 文件中： 
...
### <a name="why-should-i-worry-about-windows-autopilot-devices"></a>为什么我应该担心 Windows 自动驾驶仪设备？

当 Azure AD 设备与 Windows 自动驾驶仪对象关联时，如果将来将重新使用该设备，则可能会出现以下三种情况：
- 使用 Windows 自动驾驶仪用户驱动的部署时不使用白手套，将创建新的 Azure AD 设备，但不会使用 ZTDID 对其进行标记。
- 使用 Windows 自动驾驶仪自部署模式部署时，它们将失败，因为找不到关联的 Azure AD 设备。  （这是一种安全机制，可确保没有"冒名顶替"设备尝试加入没有凭据的 Azure AD。故障将指示 ZTDID 不匹配。
- 使用 Windows 自动驾驶仪白手套部署时，它们将失败，因为找不到关联的 Azure AD 设备。 （在幕后，白手套部署使用相同的自部署模式过程，因此它们强制实施相同的安全机制。
