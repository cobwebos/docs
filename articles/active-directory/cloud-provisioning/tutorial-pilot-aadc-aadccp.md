...
ms.locfileid: "74814456"
---
# <a name="pilot-cloud-provisioning-for-an-existing-synced-ad-forest"></a>试运行现有已同步 AD 林的云预配 
...
3. 确保为试运行范围中的对象填充 ms-ds-consistencyGUID，使云预配与对象硬性匹配。 请务必注意，Azure AD Connect sync 默认不会为组对象填充 ms-ds-consistencyGUID。
...
至少应安装 [Azure AD Connect](https://www.microsoft.com/download/details.aspx?id=47594) 1.4.32.0。 若要更新 Azure AD Connect sync，请完成 [Azure AD Connect：升级到最新版本](../hybrid/how-to-upgrade-previous-version.md)中的步骤。  如果测试环境中没有最新版本的 Azure AD Connect，则会提供此步骤。
...
## <a name="scope-azure-ad-connect-sync-to-exclude-the-pilot-ou"></a>限定 Azure AD Connect sync 的范围以排除试运行 OU
现在，请将 Azure AD Connect 配置为排除前面创建的试运行 OU。  云预配代理将处理这些用户的同步。  使用以下步骤来限定 Azure AD Connect 的范围。
