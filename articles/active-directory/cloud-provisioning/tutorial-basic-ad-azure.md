---
title: 教程 - 本地基本活动目录和 Azure AD 环境。
...
---
# <a name="tutorial-basic-active-directory-environment"></a>教程：基本活动目录环境

本教程将指导您创建基本的活动目录环境。 

![创建](media/tutorial-single-forest/diagram1.png)

您可以使用本教程中创建的环境来测试混合标识方案的各个方面，并且将成为某些教程的先决条件。  如果您已有现有的活动目录环境，则可以使用它作为替代。  此信息是为我从无开始的个人提供的。
...
- [微软 .NET 框架 4.7.1](https://www.microsoft.com/download/details.aspx?id=56115)

> [!NOTE]
> 本教程使用 PowerShell 脚本，以便可以在最短的时间内创建教程环境。  每个脚本都使用在脚本开头声明的变量。  你可以而且应该更改变量以反映你的环境。
>
>在安装 Azure AD Connect 云预配代理之前，使用的脚本创建了一般活动目录环境。  它们与所有教程相关。
>
> GitHub [此处](https://github.com/billmath/tutorial-phs)提供了本教程中使用的 PowerShell 脚本的副本。

## <a name="create-a-virtual-machine"></a>创建虚拟机
为了启动和运行我们的混合标识环境，您需要做的第一件事是创建一个虚拟机，该虚拟机将用作我们的本地活动目录服务器。  请执行以下操作：
...
现在，您已经安装了虚拟机，在安装 Active Directory 之前，您需要执行一些操作。  也就是说，您需要重命名虚拟机、设置静态 IP 地址和 DNS 信息，以及安装远程服务器管理工具。   请执行以下操作：
...
现在，您已经创建了 VM，并且它已重命名，并且具有静态 IP 地址，您可以继续安装和配置活动目录域服务。  请执行以下操作：
...
现在，您已经拥有了我们的活动目录环境，您需要一个测试帐户。  此帐户将在我们的本地 AD 环境中创建，然后同步到 Azure AD。  请执行以下操作：
...
现在，您需要创建 Azure AD 租户，以便可以将我们的用户同步到云。  要创建新的 Azure AD 租户，请执行以下操作。
...
现在，您拥有了 Azure AD 租户，您将创建一个全局管理员帐户。  要创建全局管理员帐户，请执行以下操作。
...
下面是一个可选部分，它提供了创建其他服务器和或林的步骤。  这可用于一些更高级的教程，如[Azure AD 连接到云预配的试验](tutorial-pilot-aadc-aadccp.md)。

如果只需要一个额外的服务器，则可以在 -**创建虚拟机**步骤后停止，并将服务器加入到上面创建的现有域。  
...
现在，您有一个可用于现有教程的环境，并测试云资源调配提供的其他功能。
