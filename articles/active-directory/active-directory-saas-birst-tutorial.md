...
本部分旨在展示如何根据名为“Britta Simon”的测试用户的指令配置和测试 Birst Agile Business Analytics 的 Azure AD 单一登录。

对于单一登录到工作帐户，Azure AD 需要知道 Azure AD 用户在 Birst Agile Business Analytics 中的对应用户是谁。 换句话说，需要建立 Azure AD 用户与 Birst Agile Business Analytics 中相关用户之间的链接关系。
...
本部分要在 Azure 经典门户中启用 Azure AD 单一登录，并且在 Birst Agile Business Analytics 应用程序中配置单一登录。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，键入用户用于登录 Birst Agile Business Analytics 应用程序的 URL，所用模式如下：**“https://login.bws.birst.com/SAMLSSO/Services.aspx?birst.idpid=TENANTIDPID”**。
...
1. 在“配置 Birst Agile Business Analytics 的单一登录”页面上，执行以下步骤：
...
1. 在 Azure 经典门户中，选择单一登录配置确认，然后单击“下一步”。
...
本部分旨在向 Britta Simon 授予访问 Birst Agile Business Analytics 的权限，使她能够使用 Azure 单一登录。
