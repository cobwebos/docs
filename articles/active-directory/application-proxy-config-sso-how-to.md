...
有关预身份验证方法的详细信息，请参阅[应用发布文档](manage-apps/application-proxy-publish-azure-portal.md)的步骤 4。
...
-   **集成 Windows 身份验证**：对于使用集成 Windows 身份验证 (IWA) 的应用程序，通过 Kerberos 约束委派 (KCD) 启用单一登录。 此方法在 Active Directory 中赋予应用程序代理连接器权限来模拟用户，并代表用户发送和接收令牌。 关于配置 KCD 的详细信息，可以在[通过 KCD 启用单一登录文档](manage-apps/application-proxy-configure-single-sign-on-with-kcd.md)中找到。

-   **基于标头的登录**：通过合作关系启用基于标头的登录，需要提供一些其他配置。 有关为使用标头进行身份验证的应用程序配置单一登录的合作关系和分步说明的详细信息，请参阅 [Azure AD 的 PingAccess 文档](manage-apps/application-proxy-configure-single-sign-on-with-ping-access.md)。
