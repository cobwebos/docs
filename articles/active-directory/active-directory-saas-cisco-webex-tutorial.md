...
完成本教程后，分配到 Cisco Webex 的 Azure AD 用户能够在 Cisco Webex 公司站点上（服务提供商启动的登录）或者使用[访问面板简介](active-directory-saas-access-panel-introduction.md)以单一登录方式登录到应用程序。
...
7. 在结果窗格中，选择“Cisco Webex”，并单击“完成”以添加该应用程序。
...
2. 在“你希望用户如何登录到 Cisco Webex”页上，选择“Microsoft Azure AD 单一登录”，并单击“下一步”。
...
3. 在“配置应用 URL”页上，执行以下步骤，并单击“下一步”。
   ...
   1. 在“登录 URL”文本框中，键入 Cisco Webex 租户 URL（例如：*http://contoso.webex.com*）。
   2. 在“Cisco Webex 回复 URL”文本框中，键入 **Cisco Webex AssertionConsumerService URL**（例如：*https://company.webex.com/dispatcher/SAML2AuthService?siteurl=company*）。
4. 在“在 Cisco Webex 处配置单一登录”页上，要下载证书，请单击“下载证书”，然后将证书文件保存在计算机上。
   
   ![配置单一登录](./media/active-directory-saas-cisco-webex-tutorial/IC777620.png "配置单一登录")
5. 在另一个 Web 浏览器窗口中，以管理员身份登录到 Cisco Webex 公司站点。
6. 在顶部菜单中，单击“网站管理”。
...
8. 在“联合 Web SSO 配置”部分中，执行以下步骤：
   ...
   2. 基于下载的证书创建一个 **Base-64 编码**的文件。  
    ...
    >  
   3. 在记事本中打开 Base-64 编码的证书，并复制其内容。
   4. 单击“导入 SAML 元数据”，然后粘贴 base-64 编码的证书。
   ...
   10. 单击“更新”。
9. 在 Azure 经典门户中，在“在 Cisco Webex 处配置单一登录”对话框页上，选择“单一登录配置确认”，并单击“完成”。
