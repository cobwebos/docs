...
本部分的目的是说明如何基于名为“Britta Simon”的测试用户配置和测试 Small Improvements 的 Azure AD 单一登录。

若要运行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 Small Improvements 用户。 换句话说，需要在 Azure AD 用户与 Small Improvements 中相关用户之间建立链接关系。  
...
本部分的目的是在 Azure 经典门户中启用 Azure AD 单一登录并在 Small Improvements 应用程序中配置单一登录。
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
   ...
   > [!NOTE]
   > 通过 [Support@small-improvements.com](mailto:support@small-improvements.com) 联系 Small Improvements 支持团队，以便为帐户配置域名。 要使单一登录正常工作，这是必需的。
   > 
   > 

    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在“登录 URL”文本框中，键入用户用于登录 Small Improvements 应用程序的 URL。  
    b.保留“数据库类型”设置，即设置为“共享”。 单击“资源组名称” 的 Azure 数据工厂。


1. 在“配置 Small Improvements 的单一登录”页上，执行以下步骤：
...
5. 在“SSO 设置”页上，执行以下步骤：
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在 Azure 经典门户的“配置 Small Improvements 的单一登录”对话框页上，复制“SAML SSO URL”值，然后将其粘贴到“HTTP 终结点”文本框中。
   
    b.保留“数据库类型”设置，即设置为“共享”。 在记事本中打开下载的证书，复制其内容，然后将其粘贴到“x509 证书”文本框中。
   
    c. 如果想要让 SSO 和登录表单身份验证选项可供用户使用，则选中“也启用通过登录名/密码访问”选项。 
   
    d.单击“下一步”。 在“SAML 提示”文本框中输入用于命名“SSO 登录”按钮的相应值。 
...
本部分的目的是通过授予 Britta Simon 访问 Small Improvements 的权限，允许她使用 Azure 单一登录。
