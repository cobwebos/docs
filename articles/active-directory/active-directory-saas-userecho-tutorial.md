...
* 可让用户通过其 Azure AD 帐户自动登录到 UserEcho 单一登录 (SSO)
...
## <a name="add-userecho-from-the-gallery"></a>从库中添加 UserEcho
...
本部分旨在说明如何基于名为“Britta Simon”的测试用户配置和测试 UserEcho 的 Azure AD SSO。

若要运行 SSO，Azure AD 需要知道与 Azure AD 用户相对应的 UserEcho 用户。 换句话说，需要在 Azure AD 用户与 UserEcho 中的相关用户间建立链接关系。  

通过将 Azure AD 中“用户名”的值指定为 UserEcho 中 **Username** 的值，建立此链接关系。

若要配置和测试 UserEcho 的 Azure AD SSO，需要完成以下构建基块：
...
本部分旨在介绍如何在 Azure 经典门户中启用 Azure AD SSO 并在 UserEcho 应用程序中配置 SSO。 
...
3. 在“配置应用设置”对话框页上，执行以下步骤：
  ...
  1. 在“登录 URL”文本框中，键入用户用来登录 UserEcho 应用程序的 URL（例如：*https://fabrikam.UserEcho.com/*）。
  2. 单击“下一步”。
...
9. 在“单一登录 (SAML)”页上，执行以下步骤：
  ...
  1. 至于“SAML 启用”，请选择“是”。 
  2. 在 Azure 经典门户的“配置 UserEcho 的单一登录”对话框页上，复制“单一登录服务 URL”值，然后将其粘贴到“SAML SSO URL”文本框中。
  3. 在 Azure 经典门户的“配置 UserEcho 的单一登录”对话框页上，复制“远程注销 URL”值，然后将其粘贴到“远程注销 URL”文本框中。 
  4. 在记事本中打开下载的证书，复制其内容，然后将其粘贴到“X.509 证书”文本框中。    
  5. 单击“保存” 。
...
### <a name="create-a-userecho-test-user"></a>创建 UserEcho 测试用户
...
6. 在“邀请新用户”对话框中，执行以下步骤：
  ...
  1. 在“名称”文本框中，键入 **Britta Simon**。
  2. 在“电子邮件”文本框中，键入 Britta 在 Azure 经典门户中的电子邮件地址。
  3. 单击“邀请”。

将向 Britta 发送邀请，通过此邀请她可开始使用 UserEcho。 

### <a name="assign-the-azure-ad-test-user"></a>分配 Azure AD 测试用户
本部分旨在通过授予 Britta Simon 访问 UserEcho 的权限，允许她使用 Azure SSO。
