...
本教程介绍了如何将 Bambu by Sprout Social 与 Azure Active Directory (Azure AD) 集成。

将 Bambu by Sprout Social 与 Azure AD 集成具有以下优势：
...
若要配置 Azure AD 与 Bambu by Sprout Social 的集成，需要以下项：
...
4. 在搜索框中，键入“Bambu by Sprout Social”。
...
本部分介绍了使用名为“Britta Simon”的测试用户配置和测试 Bambu by Sprout Social 与 Azure AD 的单一登录。

若要进行单一登录，Azure AD 需要知道与 Azure AD 用户相对应的 Bambu by Sprout Social 用户。 换句话说，需要在 Azure AD 用户与 Bambu by Sprout Social 中的相关用户之间建立链接关系。

建立此链接关系的方法是将 Azure AD 中的**用户名**值分配为 Bambu by Sprout Social 中的**用户名**值。
...
本部分介绍了在 Azure 门户中启用 Azure AD 单一登录，并在 Bambu by Sprout Social 应用程序中配置单一登录。

**若要配置 Bambu by Sprout Social 与 Azure AD 的单一登录，请执行以下步骤：**

1. 在 Azure 门户中的 **Bambu by Sprout Social** 应用程序集成页上，单击“单一登录”。

    ![配置单一登录][4]

2. 在“单一登录”对话框中，选择“基于 SAML 的登录”作为“模式”以启用单一登录。
 
    ![配置单一登录](./media/active-directory-saas-bambubysproutsocial-tutorial/tutorial_bambubysproutsocial_samlbase.png)

3. 在“Bambu by Sprout Social 域和 URL”部分中，用户不必执行任何步骤，因为该应用已经与 Azure 预先集成。 
...
5. 单击“保存”按钮。
    ...
    ![配置单一登录](./media/active-directory-saas-bambubysproutsocial-tutorial/tutorial_general_400.png)
    
6. 在“Bambu by Sprout Social 配置”部分，单击“配置 Bambu by Sprout Social”，以打开“配置登录”窗口。 从“快速参考”部分中复制“SAML 单一登录服务 URL”
...
1. 在 **Azure 门户**的左侧导航窗格中，单击“Azure Active Directory”图标。
    ...
    ![创建 Azure AD 测试用户](./media/active-directory-saas-bambubysproutsocial-tutorial/create_aaduser_01.png) 

2. 转到“用户和组”，单击“所有用户”显示用户列表。
    
    ![创建 Azure AD 测试用户](./media/active-directory-saas-bambubysproutsocial-tutorial/create_aaduser_02.png) 

3. 在对话框顶部单击“添加”，打开“用户”对话框。
 
    ![创建 Azure AD 测试用户](./media/active-directory-saas-bambubysproutsocial-tutorial/create_aaduser_03.png) 

4. 在“用户”对话框页上，执行以下步骤：
 
    ![创建 Azure AD 测试用户](./media/active-directory-saas-bambubysproutsocial-tutorial/create_aaduser_04.png) 
...
本部分中将授予 Britta Simon 访问 Bambu by Sprout Social 的权限，使她能够使用 Azure 单一登录。
...
2. 在应用程序列表中选择“Bambu by Sprout Social”。

    ![配置单一登录](./media/active-directory-saas-bambubysproutsocial-tutorial/tutorial_bambubysproutsocial_app.png) 
...
单击访问面板中的“Bambu by Sprout Social”磁贴时，应该会自动登录到“Bambu by Sprout Social”应用程序。 有关访问面板的详细信息，请参阅 [Introduction to the Access Panel](https://msdn.microsoft.com/library/dn308586)（访问面板简介）。 
