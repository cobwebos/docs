...
description: "了解如何使用 Zscaler Two 与 Azure Active Directory 以启用单一登录、自动化预配及更多内容！"
...
本教程旨在介绍 Azure 与 Zscaler Two 的集成。  
在本教程中概述的方案假定您已具有以下各项：

* 一个有效的 Azure 订阅
* 已启用 ZScaler Two 单一登录的订阅

完成本教程后，已分配到 ZScaler Two 的 Azure AD 用户可在 ZScaler Two 公司站点（服务提供商发起的登录）或使用[访问面板简介](active-directory-saas-access-panel-introduction.md)单一登录到应用程序

在本教程中概述的方案由以下构建基块组成：

1. 启用 ZScaler Two 的应用程序集成
2. 配置单一登录
3. 配置代理设置
4. 配置用户设置
5. 分配用户
...
## <a name="enabling-the-application-integration-for-zscaler-two"></a>启用 ZScaler Two 的应用程序集成
本部分旨在概述如何启用 ZScaler Two 的应用程序集成。

### <a name="to-enable-the-application-integration-for-zscaler-two-perform-the-following-steps"></a>若要启用 ZScaler Two 的应用程序集成，请执行以下步骤：
...
5. 在“要执行什么操作”对话框中，单击“从库中添加应用程序”。
   ...
   ![从库中添加应用程序](./media/active-directory-saas-zscaler-two-tutorial/IC749322.png "Add an application from gallerry")
6. 在“搜索框”中，键入“ZScaler Two”。
   
   ![应用程序库](./media/active-directory-saas-zscaler-two-tutorial/IC800200.png "Application Gallery")
7. 在“结果”窗格中，选择“ZScaler Two”，然后单击“完成”以添加该应用程序。
   
   ![ZScaler Two](./media/active-directory-saas-zscaler-two-tutorial/IC800201.png "ZScaler Two")

## <a name="configuring-single-sign-on"></a>配置单一登录
此部分旨在概述如何使用基于 SAML 协议的联合身份验证，使用户通过其 Azure AD 中的帐户对 ZScaler Two 进行身份验证。  
在此过程中，需要将 Base-64 编码的证书上传到 ZScaler Two 租户。  
如果不熟悉此过程，请参阅[如何将二进制证书转换为文本文件](http://youtu.be/PlgrzUZ-Y1o)

### <a name="to-configure-single-sign-on-perform-the-following-steps"></a>若要配置单一登录，请执行以下步骤：
1. 在 Azure 经典门户的“ZScaler Two”应用程序集成页上，单击“配置单一登录”打开“配置单一登录”对话框。
   
   ![配置单一登录](./media/active-directory-saas-zscaler-two-tutorial/IC800202.png "Configure Single Sign-On")
2. 在“你希望用户如何登录 ZScaler Two”页上，选择“Microsoft Azure AD 单一登录”，然后单击“下一步”。
   
   ![配置单一登录](./media/active-directory-saas-zscaler-two-tutorial/IC800203.png "Configure Single Sign-On")
3. 在“配置应用 URL”页的“ZScaler Two 登录 URL”文本框中，键入用户登录到 ZScaler Two 应用程序时使用的 URL，然后单击“下一步”。
   
   ![配置应用 URL](./media/active-directory-saas-zscaler-two-tutorial/IC800204.png "Configure App URL")
   
   > [!NOTE]
   > 如果需要，可以从 ZScaler Two 支持团队获取环境的实际值。
   > 
   > 
4. 在“配置 ZScaler Two 的单一登录”页上，若要下载证书，请单击“下载证书”，然后将该证书文件保存到计算机上。
   
   ![配置单一登录](./media/active-directory-saas-zscaler-two-tutorial/IC800205.png "Configure Single Sign-On")
5. 在其他 Web 浏览器窗口中，以管理员身份登录到 ZScaler Two 公司站点。
6. 在顶部菜单中，单击“管理”。
...
9. 在“配置 SAML 单一登录参数”对话框页上，执行以下步骤，然后单击“完成”：
   ...
   1. 在 Azure 经典门户中，在“配置 ZScaler Two 的单一登录”对话框页上，复制“身份验证请求 URL”值，然后将其粘贴到“用户发送的用于身份验证的 SAML 门户 URL”文本框中。
...
10. 在“配置用户身份验证”对话框页上，执行以下步骤：
    ...
    1. 单击“保存” 。
    2. 单击“立即激活”。
11. 在 Azure 经典门户中的“配置 ZScaler Two 的单一登录”对话框页面上，选择“单一登录配置确认”，然后单击“完成”。
...
为了使 Azure AD 用户能够登录到 ZScaler Two，必须将其预配到 ZScaler Two。  
对于 ZScaler Two，需要手动执行预配。
...
> [!NOTE]
> 可以使用 Zscaler Two 提供的任何其他 ZScaler Two 用户帐户创建工具或 API 来预配 AAD 用户帐户。
> 
> 
...
### <a name="to-assign-users-to-zscaler-two-perform-the-following-steps"></a>若要将用户分配到 ZScaler Two，请执行以下步骤：
1. 在 Azure 经典门户中，创建测试帐户。
2. 在“ZScaler Two”应用程序集成页上，单击“分配用户”。
