...
**我们做了什么？** 我们此问题没有出现在依赖项中，并且不经常出现，则可能是最近的某项更改导致。 指标和事件图提供的历史角度使将任何突然更改与部署联系变得简单。 这可缩小问题的搜索范围。 若要确定应用程序代码中的哪些行降低了性能，请启用 Application Insights Profiler。 请参阅[使用 Application Insights 分析实时 Azure Web 应用](./app-insights-profiler.md)。 启用 Profiler 后，将看到如下跟踪。 在此示例中，显而易见是 *GetStorageTableData* 方法导致了问题。  

![App Insights Profiler 跟踪](./media/app-insights-detect-triage-diagnose/AppInsightsProfiler.png)
