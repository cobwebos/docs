---
title: "适用于 Azure 云服务的 Application Insights"
...
此选项可在运行时检测应用，提供监视请求所需的全部遥测数据、Web 角色中的异常和依赖项，以及辅助角色中的性能计数器。 应用生成的所有诊断跟踪也将发送到 Application Insights。

如果这就是你所需的全部信息，Application Insights 完全可以满足你的要求。 后续步骤是[通过应用查看指标](app-insights-metrics-explorer.md)、[使用 Analytics 查询数据](app-insights-analytics.md)，以及设置[仪表板](app-insights-dashboards.md)。 可以设置[可用性测试](app-insights-monitor-web-app-availability.md)并[将代码添加到网页](app-insights-javascript.md)，以便在浏览器中监视性能。
...
备选方案是将多个角色中的遥测数据发送到同一个资源，但[将维度属性添加到标识其源角色的每个遥测项](app-insights-api-filtering-sampling.md#add-properties-itelemetryinitializer)。 在此方案中，异常等指标图表通常显示不同角色的聚合计数，但你可以根据需要按角色标识符将图表分段。 此外，还可以按同一个维度筛选搜索结果。 使用这种备选方法可以方便一次性查看所有信息，但同时可能导致在角色之间产生一定的混淆。
...
如果在前一个版本仍然有效时为下一个功能开发自定义事件，可以将开发遥测数据发送到独立的 Application Insights 资源。 否则，很难在实时站点的所有流量中找到你的测试遥测数据。
...
2. 请注意，每个资源都由检测密钥标识。 以后若要手动配置或验证 SDK 的配置，可能需要使用该检测密钥。
   ...
   ![单击“属性”，选择密钥，然后按 Ctrl+C](./media/app-insights-cloudservices/02-props.png) 

## <a name="set-up-azure-diagnostics-for-each-role"></a>为每个角色设置 Azure 诊断
设置此选项可以使用 Application Insights 监视应用。 对于 Web 角色，此选项可提供性能监视、警报和诊断以及使用情况分析。 对于其他角色，可以搜索和监视 Azure 诊断信息，例如重新启动、性能计数器和对 System.Diagnostics.Trace 的调用。 
...
* 应用程序日志、ETW 日志和任何诊断基础结构日志显示为跟踪。

若要查看性能计数器和事件计数，请打开“指标资源管理器”并添加新图表：[](app-insights-metrics-explorer.md)

![Azure 诊断数据](./media/app-insights-cloudservices/23-wad.png)

使用“[搜索](app-insights-diagnostic-search.md)”在 Azure 诊断发送的各种跟踪日志中进行搜索。 例如，如果角色中有未经处理的异常造成该角色崩溃和回收，该信息会显示在“Windows 事件日志”的“应用程序”通道中。 你可以使用搜索功能查看 Windows 事件日志错误并获取异常的完整堆栈跟踪，以便找出问题的根本原因。
...
在 Web 角色中，请求模块将自动收集有关 HTTP 请求的数据。 有关如何覆盖默认收集行为的示例，请参阅[示例 MVCWebRole](https://github.com/Microsoft/ApplicationInsights-Home/tree/master/Samples/AzureEmailService/MvcWebRole)。 

可以像跟踪 HTTP 请求一样捕获辅助角色调用的性能。 在 Application Insights 中，请求遥测类型用于度量一个单位的、可以计时并且可以独立成功或失败的命名服务器端工作。 当 SDK 自动捕获 HTTP 请求时，你可以插入自己的代码来跟踪针对辅助角色发出的请求。
...
该示例 Web 角色包含 MVC5 和 Web API 2 控制器。 可以使用以下方式捕获这 2 个控制器的未经处理的异常：
...
* TrackException(ex)
* 如果已添加 Application Insights 跟踪侦听器 NuGet 包，可以使用 System.Diagnostics.Trace 来记录异常。 [代码示例](https://github.com/Microsoft/ApplicationInsights-Home/blob/master/Samples/AzureEmailService/WorkerRoleA/WorkerRoleA.cs#L107)
...
可以指定其他自定义计数器或其他 Windows 性能计数器，如[此处](https://github.com/Microsoft/ApplicationInsights-Home/blob/master/Samples/AzureEmailService/WorkerRoleA/ApplicationInsights.config#L14)所示
...
* 添加自定义 TelemetryInitializer 实现，用于将 Operation.Id 设置为前面设置的 correlationId。 如此处所示：[ItemCorrelationTelemetryInitializer](https://github.com/Microsoft/ApplicationInsights-Home/blob/master/Samples/AzureEmailService/WorkerRoleA/Telemetry/ItemCorrelationTelemetryInitializer.cs#L13)
...
就这么简单！ 门户中的体验随时可以准备帮助你一目了然地查看所有关联的遥测数据：
...
如果你在使用客户端移动应用，可以插入一些代码来发送有关重要用户操作的自定义事件，并创建 [HockeyApp 桥](app-insights-hockeyapp-bridge-app.md)。 在 [Analytics](app-insights-analytics.md) 中创建查询来显示事件计数，并将事件固定到仪表板。
