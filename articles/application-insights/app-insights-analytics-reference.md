...
* *Threshold*：范围 {0..1} 内的值。 默认值为 0.001。 对于大型输入，阈值应较小。 
...
* `withsource=`*ColumnName*：如果指定，输出将包含名为 *ColumnName* 的列，该列的值指示哪个源表提供了哪一行。
...
过去一天已生成 `exceptions` 事件或 `traces` 事件的不同用户数。 在结果中，“SourceTable”列指示“Query”或“Command”：
