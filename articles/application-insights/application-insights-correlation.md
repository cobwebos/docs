...
在微服务的世界中，每次逻辑操作都需要在服务的不同组件中完成工作。 UI 组件与身份验证提供程序组件通信以验证用户凭据，并与 API 组件通信以获取要可视化的数据。 而 API 组件又可以从其他服务查询数据，并使用缓存提供程序组件以及通知有关此调用的计费组件。 Application Insights 支持分布式遥测关联。 它允许检测哪个组件要对 UI 故障或性能下降问题负责。

本文介绍 Application Insights 使用的数据模型。 其中阐述了上下文传播技术和协议， 以及如何在不同的语言和平台上实现相关的概念。

## <a name="telemetry-correlation-data-model"></a>遥测关联数据模型

Application Insights 定义了用于分配遥测关联的[数据模型](/application-insights-data-model.md)。 若要将遥测与逻辑操作相关联，每个项都应包含名为 `operation_Id` 的上下文字段。 此标识符由分布式跟踪中的每个遥测项共享。 因此，即使单个层失去了遥测功能，你也仍可以关联其他组件报告的遥测。
...
Application Insights 数据模型定义了两个字段（`request.source` 和 `dependency.target`）来解决此问题。 第一个字段定义发起请求的组件，第二个字段定义哪个组件返回了依赖项调用的响应。


## <a name="example"></a>示例

我们以 STOCKS API 应用程序为例。该应用程序使用名为 STOCKS API 的外部 API 显示某只股票的当前市价。 STOCK PRICES 应用程序包含一个 `Stock page` 页面，用于对处理 ajax 调用 `GET /Home/Stock` 的服务器发出此调用。 为了返回结果，应用程序将使用 http 调用 `GET /api/stock/value` 查询 STOCK API。
