<properties
	pageTitle="监视任何网站的可用性和响应能力 | Microsoft Azure"
...
如果已经为此应用程序[设置 Application Insights 资源][start]，并且想要在同一位置中查看可用性报告，请跳过本步骤。
...
- 默认情况下，如果五分钟内有三个位置发生失败，则会发出**警报**。某个位置的失败很可能是网络问题，而不是站点问题。但是，可以将阈值更改为更敏感或更不敏感，也可以更改要将电子邮件发送给哪个人。

    可以设置在引发警报时调用的 [webhook](../azure-portal/insights-webhooks-alerts.md)。（但请注意，查询参数不会以“属性”的形式传递。）
...
#### 2\.将 Web 测试上载到 Application Insights
 ...
 * 参数化令牌，设置参数来指定从验证器返回令牌的时间，并在站点查询中使用该参数。（Visual Studio 会尝试参数化测试，但无法正常参数化令牌。）
...
在“概述”边栏选项卡中，打开“设置”、“性能测试”。创建测试时，系统会请求连接或创建 Visual Studio Team Services 帐户。
...
* 自动[使用 PowerShell 脚本设置 Web 测试](https://azure.microsoft.com/blog/creating-a-web-test-alert-programmatically-with-application-insights/)。
* 设置在引发警报时调用的 [webhook](../azure-portal/insights-webhooks-alerts.md)。
...
* *如何在防火墙后面运行的内部服务器上使用可用性测试？*
    ...
    请将防火墙配置为允许从 [Web 测试代理 IP 地址](app-insights-ip-addresses.md#availability)发出的请求。
