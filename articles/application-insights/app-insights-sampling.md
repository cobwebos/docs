...
* 采样保留 1  *n* 录制并丢弃其余。 例如，它可能会保留 5 件事件中的 1 件，采样率为 20%。 
...
* 采样除数 *n* 属性中的每个记录中报告`itemCount`，后者在搜索中显示的友好名称"请求计数"或"事件计数"下。 如果采样不在运行，则 `itemCount==1`。
...
* **引入采样**在 Azure 门户中正常工作。 则会丢弃一些从你的应用，你设置的速率到达的遥测数据。 它不会减少遥测流量，但有助于使你每月配额中。 引入采样的大优点是，无需重新部署应用就可设置它，并且它统一适用于所有服务器和客户端。 
...
如果你的应用程序通常会超过其每月配额，但你没有使用基于 SDK 的采样类型之一的选项，请使用此类型的采样。 
...
客户端 (JavaScript) SDK 与服务器端 SDK 一同参与固定速率采样。 检测到的页才会从为其服务器端做出"中的示例"。 其决定的相同用户发送客户端遥测 此逻辑的设计目的是维护跨客户端和服务器端的用户会话的完整性。 因此，通过 Application Insights 中的任何特定遥测项，可查找此用户或会话的所有其他遥测项。 
...
* 一种方法开始自适应采样，了解新增功能对其进行评分结算 （请参阅上述问题），然后切换到固定速率采样使用该速率。 
  
    否则，就必须猜测。 分析 AI 中当前遥测使用量、观察出现的任何限制，并估计所收集的遥测量。 这些三个输入，以及你所选的定价层建议多少你可能想要减少的收集的遥测数据量。 但是，用户数量的增加或遥测量中的某些其他变化可能导致估计无效。
