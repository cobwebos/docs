<properties
	pageTitle="适用于 JavaScript Web 应用的 Application Insights | Microsoft Azure"
...
了解网页或应用的性能和使用情况。如果将 Visual Studio Application Insights 添加到页面脚本，可以获取页面加载的时间和 AJAX 调用、计数、浏览器异常与 AJAX 失败详细信息，以及用户和会话计数。所有这些信息可按页面、客户端 OS 和浏览器版本、地理位置和其他维度细分。还可以针对失败计数或页面加载缓慢情况设置警报。
...
首先需要订阅 [Microsoft Azure](https://azure.com)。如果团队拥有组织订阅，请咨询所有者将 Microsoft 帐户添加到其中。提供了免费定价层，因此开发和小规模的使用不会产生任何费用。


## 设置适用于网页的 Application Insights

首先，是否需要将 Application Insights 添加到网页？ 也许已经这样做了。如果在 Visual Studio 的“新建项目”对话框中选择向 Web 应用添加 Application Insights，则已经添加了脚本。在此情况下，不再需要执行任何操作。
...
* 在 ASP.NET MVC 项目中，请将脚本放在 `View\Shared\_Layout.cshtml` 中
* 在 SharePoint 站点中，在控制台上打开“站点设置/母版页”。[](app-insights-sharepoint.md)
...
*还没有数据？ 单击页面顶部的“刷新”。**还是没有？ 请参阅[故障排除](app-insights-troubleshoot-faq.md)。*

“浏览器”边栏选项卡是一个[指标资源管理器边栏选项卡](app-insights-metrics-explorer.md)，其中包含预设筛选器和图表选项。如果需要，可以编辑时间范围、筛选器和图表配置，将结果另存为收藏项目。单击“还原默认值”可恢复原始边栏选项卡设置。
...
> [AZURE.NOTE] 如果删除边栏选项卡上的“浏览器”筛选器，服务器和 AJAX 依赖项将包含在这些图表中。单击“还原默认值”可以重新配置筛选器。
...
> [AZURE.NOTE] 如果使用“搜索”，请注意，必须匹配整个字词：“Abou”和“bout”与“About”并不匹配。[](app-insights-diagnostic-search.md)
