...
2. 单击![“应用程序映射”图标](media/app-insights-website-monitoring/006.png)“Analytics”。  这将打开“Analytics”，该软件提供丰富的查询语言，可用于分析 Application Insights 收集的所有数据。 若要查看与客户端浏览器请求相关的数据，请运行以下查询：
   ...
   ![一段时间内用户请求的分析图](./media/app-insights-website-monitoring/analytics-query.png)

3. 返回到“概览”页。 在“调查”标题下单击“浏览器”，然后选择“性能”。在此处可找到与网站性能相关的指标。 另外还有一个用于分析网站中故障和异常的相应视图。 可以单击“示例”，深入了解各个事务的详细信息。 在这里，可以访问[端到端事务详细信息](app-insights-transaction-diagnostics.md)体验。

   ![“服务器指标”图](./media/app-insights-website-monitoring/browser-performance.png)

4. 若要开始探索[用户行为分析工具](app-insights-usage-overview.md)，请在 Application Insights 主菜单的“使用情况”标题下选择[**用户**](app-insights-usage-segmentation.md)。 由于我们是在单台计算机中进行测试，因此只会看到一个用户的数据。 对于实时网站，用户的分布可能如下所示：

     ![用户图](./media/app-insights-website-monitoring/usage-users.png)

5. 如果我们已检测较复杂的包含多个页面的网站，则可使用另一有用的工具：[**用户流**](app-insights-usage-flows.md)。 使用**用户流**时，可以跟踪用户在访问网站不同部分时采用的路径。

  ![用户流可视化](./media/app-insights-website-monitoring/user-flows.png)
