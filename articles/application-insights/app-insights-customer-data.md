...
最终将由你和你的公司来决定使用哪项策略来处理私人数据（如有），下文提供的只是一些可以借鉴的方法。 我们从技术角度出发，按偏好顺序，从最偏好的到最不偏好的列出这些方法：
...
虽然绝大多数清除操作完成起来会比 SLA 快得多，但由于其对 Application Insights 使用的数据平台造成的严重影响，因此将完成清除操作所需的正式 SLA 设置为 30 天。
