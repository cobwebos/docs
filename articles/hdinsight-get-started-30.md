...
> [WACN.NOTE] 本教程介绍如何在 HDInsight 上使用 Hadoop 2.2 群集。有关在 HDInsight 上使用 Hadoop 1.2 群集的教程，请参阅[开始使用 Azure HDInsight][开始使用 Azure HDInsight]。

> [WACN.NOTE] HDInsight 群集版本 3.0 及将来的更高版本都不支持 *asv://* 语法。应改用 *wasb://* 语法。

Microsoft 还发布了 HDInsight Emulator for Azure（以前称作 Microsoft HDInsight 开发者预览版），与 Azure HDInsight 的通用版本结合使用。该产品针对开发人员方案并因此仅支持单节点部署。有关如何使用 HDInsight Emulator 的信息，请参阅 [HDInsight Emulator 入门][HDInsight Emulator 入门]。

**先决条件：**

在开始阅读本教程前，你必须具有：

-   Azure 订阅。有关获取订阅的详细信息，请参阅[购买选项][购买选项]、[免费试用][免费试用]。
...
-   [为运行的 PowerShell 设置本地环境][为运行的 PowerShell 设置本地环境]
-   [设置 HDInsight 群集][设置 HDInsight 群集]
-   [运行 WordCount MapReduce 程序][运行 WordCount MapReduce 程序]
-   [连接到 Microsoft 商业智能工具][连接到 Microsoft 商业智能工具]
-   [后续步骤][后续步骤]

## 为运行 PowerShell 设置本地环境

有多种方法可将 MapReduce 作业提交到 HDInsight。在本教程中，你将使用 Azure PowerShell。若要安装 Azure PowerShell，请运行 [Microsoft Web 平台安装程序][Microsoft Web 平台安装程序]。出现提示时，依次单击“运行” 、“安装” ，然后按照说明进行操作。有关详细信息，请参阅[安装和配置 Azure PowerShell][安装和配置 Azure PowerShell]。

PowerShell cmdlet 需要使用你的订阅信息来管理你的服务。

**使用 Azure AD 连接到你的订阅**

1.  打开 Azure PowerShell 控制台，按照[如何：安装 Azure PowerShell][如何：安装 Azure PowerShell] 中的说明进行操作。
2.  运行以下命令：
...
连接到你的订阅的另一个方法是使用证书方法。有关说明，请参阅[安装和配置 Azure PowerShell][安装和配置 Azure PowerShell]。
...
有关详细说明，请参阅
[如何创建存储帐户][如何创建存储帐户]和[将 Azure Blob 存储与 HDInsight 配合使用][将 Azure Blob 存储与 HDInsight 配合使用]。
...
5.  单击右下角的右箭头以配置群集用户。
6.  在“配置群集用户”选项卡中，输入 HDInsight 群集用户帐户的**用户名**和**密码**。除了此帐户外，你还可以在设置群集后创建一个 RDP 用户帐户，以便通过远程桌面访问群集。有关说明，请参阅[使用管理门户管理 HDInsight][使用管理门户管理 HDInsight]
7.  单击右下角的右箭头以配置存储帐户。
...
-   MapReduce 程序。在本教程中，你将使用 HDInsight 群集分发随附的 WordCount 示例，因此，你无需编写自己的 MapReduce 程序。该示例位于 */example/jars/hadoop-mapreduce-examples.jar*。有关编写自己的 MapReduce 作业的说明，请参阅[为 HDInsight 开发 Java MapReduce 程序][为 HDInsight 开发 Java MapReduce 程序]。

-   一个输入文件。你将使用 */example/data/gutenberg/davinci.txt* 作为输入文件。有关上载文件的信息，请参见[将数据上载到 HDInsight][将数据上载到 HDInsight]。
...
> [WACN.NOTE] 默认情况下，用于默认文件系统的 Blob 容器与 HDInsight 群集同名。
...
有关详细信息，请参阅[将 Azure Blob 存储与 HDInsight 配合使用][将 Azure Blob 存储与 HDInsight 配合使用]。
...
> [WACN.NOTE] 如果你在记事本中打开 *./example/data/WordCountOutput/part-r-00000*（MapReduce 作业的多行输出），将会注意到断行未正确呈现。这是正常情况。
...
-   从 [Microsoft 下载中心][Microsoft 下载中心]下载 Microsoft Power Query for Excel 并安装它。
...
9.  重复该过程以便将“Column1.2” 重命名为“Count” 。
    ...
    ![HDI.GettingStarted.PowerQuery.ImportData3][HDI.GettingStarted.PowerQuery.ImportData3]
