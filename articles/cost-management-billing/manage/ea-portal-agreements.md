...
LSP 在 EA 门户中提供一个百分数。  门户中的所有商业信息都将按 LSP 提供的百分比 uplifted。 示例：
...
- 以下示例说明客户将看到的商业信息：
    - 货币余额： USD 110000。
    - 服务 A 的计量速率： USD 11/小时。
    - 用于100小时的服务 A 的使用情况/托管信息： USD 1100。
    - 资金余额适用于服务的客户后扣缴： USD 108900。
...
若要检查注册是否有标记发布，请单击左侧导航栏上的 "**管理**"，并单击 "**注册**" 选项卡。选择要检查的注册框，然后在 "_注册详细信息_" 下查看标记状态。 它会将该 EA 的加价功能的当前状态显示为“已禁用”、“预览”或“已发布”。
...
| Microsoft Azure 计算实例 v2 VM | EA：350核心 | GA IaaS v2 VM：<ul><li> A0\_A7 系列 - 350 个核心 </li><li> B\_A0\_A4 系列 - 350 个核心 </li><li> A8\_A9 系列 - 350 个核心 </li><li> DF 系列 - 350 个核心</li><li> GF - 350 个核心 </li></ul>|
...
预购额在“包含的单位数量”列中显示为额度，通过该额度使用的使用量不会产生任何使用费，直到耗尽包含的单位数量。 耗尽后，额外的使用量按普通 EA 客户价目表中的协定费率计费。

基于每月某类计算机的数量购买额度。 例如：如果你在美国东部 2 区域每年购买 70 台 A2 VM，EA 门户中的显示方式与你的购买方式不太相同。

每月虚拟机算作每月运行 744 小时。 转换一下，70 台乘以 744 小时，得出每月购买的 A2 VM 的时数为 52,080 小时。 由于 EA 门户的使用情况摘要中以 100 小时的增量为单位报告 A2 VM 的时数，你看到的额度就是每月 520.8 (52,080/100) 个包含的单位数量，还会看到当月已使用的单位数量。 在已使用的单位数量超过每月包含的单位数量之前，使用费显示为零。
...
1. 单击“帐户”页以查看该页时，新添加的帐户将显示为“挂起”状态。 帐户所有者首次登录 EA 门户后，它会变为“可用”状态。
...
添加的所有新订阅使用默认订阅名称“Microsoft Azure Enterprise”。 务必更新订阅名称以将其与企业合约中的其他订阅区分开来，并确保能在企业级的报表上识别出此名称。
...
首次向合约添加新的帐户所有者 (AO) 时，这些帐户所有者将始终显示为“挂起”状态。 AO 收到激活欢迎电子邮件时，可以登录以激活其帐户。 此激活会将其帐户的状态从“挂起”更新为“可用”。

**购买计划 SKU 后对使用量收费**

如果客户在错误的合约编号下部署了服务或选择了错误的服务，就会出现这种情况。

如果要验证是否是在正确的合约下进行的部署，可以通过价目表核对包含的单位数量信息。 请以企业管理员身份登录，然后在左侧导航窗格中单击 "**报表**"，然后选择 "**价目表**" 选项卡。单击右上角的 "下载" 图标，然后使用 "已包含数量" 列中的筛选器查找相应的计划 SKU 部件号，并选择大于 "0" 的值。
...
如果价目表上的服务的包含单位与已部署的内容不匹配（例如，已分析的 Operational Insights 高级数据分析与 Operational Insights 标准数据分析），则表示你可能已部署了计划未涵盖的服务，请联系[https://aka.ms/AzureEntSupport](https://aka.ms/AzureEntSupport)上的 Azure Enterprise Portal 支持人员，以便我们可以进一步提供帮助。
