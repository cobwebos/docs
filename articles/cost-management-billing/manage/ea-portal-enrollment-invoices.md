...
在**5 月1日之前，直接注册开始日期早于 2018**：直销企业 AZURE （EA）客户已设置为 Azure 服务的年度计费周期（Marketplace 服务除外）。 计费周期基于周年日。 周年日即协议生效日。 只有超过货币承诺 (MC) 阈值的 150%，才能收到第一张 Azure 服务超额发票。  一旦服务使用量超过 MC 阈值的 150%，系统就会根据周年日自动将你的计费周期转换为季度。  如果未超过 MC 阈值的 150%，则合约仍然采用按年计费的周期，并且你将于承诺的年末收到超额发票。

**开始日期在5月1日之后直接注册，2018**：客户的 Azure 消耗和计费单独发票将按月计费。  Azure 货币承诺未涵盖的费用均被计入超额付款。  

**使用在5月1日之前开始的注册进行的间接**注册（2018：间接企业 AZURE （EA）客户设置为季度计费周期。  通道合作伙伴 (CP) 将直接向客户开票。  

**开始日期在5月1日之后的间接注册为 2018**：开始日期大于或等于5月 1 2018 日的任何间接协议将按月计费。  
...
自 2019 年 4 月这个计费周期起，客户将开始收到一张 Azure 发票，它将所有 Azure 和 Azure 市场的费用合并到一张发票中（而不是分别使用两张单独的发票）。 （此更改不会影响澳大利亚、日本或新加坡的客户。）过渡到合并发票时，你将收到部分 Marketplace 发票。 这张单独的最终发票显示计费更新日之前的市场费用。 该日以后的市场费用显示在 Azure 发票上。 过渡期结束后，所有 Azure 费用和市场费用均显示在合并发票上。  
...
1. 在此行中，你将看到应用于你的注册的信用额度和简短说明，例如服务级别协议信用额度。
...
| --- | --- | --- | --- |
| 小时数 | 24 | 0,0535960591133005 | 12,863,054,187,192,100,000,000 |

应会看到：

| ServiceResource | ResourceQtyConsumed | ResourceRate | ExtendedCost |
| --- | --- | --- | --- |
| 小时数 | 24 | 0,0535960591133005 | 1,2863054187192120000000 |

### <a name="root-cause"></a>根本原因

Excel 将所有字段作为 "常规" 文本导入，并假定数字以数学标准分隔： "1000.00"。  于是，将句点 (.) 用作为千分位符、将逗号用作小数分隔符 (,) 的欧洲货币（比如 1.000,00）的显示就不正确。 这可能会根据你的区域语言设置而异。
...
- **基于使用情况：** 以事务性单位度量的产品。  例如，虚拟机按小时收费，必应 API 搜索按搜索数量收费。
- **每月费用：** 根据使用情况/访问权限每月计费。
...
新计量方式于 2016 年 1 月 27 日到 1 月 28 日之间生效。 在此转换期间部署的 .csv 下载中，你将看到两个行项目：一个用于在 A1 指示器上发出的使用情况，另一个用于在与部署大小相对应的新专用计量器上发出的使用情况。
...
| 区域 2 | 东亚 <br> 东南亚 <br> 日本东部 <br> 日本西部 <br> 澳大利亚东部 <br> 澳大利亚东南部 | 亚太 <br> 日本 <br> 拉丁美洲 <br> 中东/非洲 <br> 澳大利亚东部 <br> 澳大利亚东南部 |
| 区域 3 | 巴西南部 |   |
...
- **使用情况：** 产品是以事务性单位度量的。  例如，虚拟机按小时收费，必应 API 搜索按搜索数量收费。
- **不基于使用情况：** 一次计费或每月定期收费，与使用情况无关。
...
- **常规用途**： "Microsoft Azure Enterprise" 内容包试用版产品/服务（"产品/服务"）的扩展 Power BI Pro 可用于产品/服务的现有合格用户，以允许他们通过使用特定的 Power BI 内容包来访问与 Microsoft Azure 消费相关的某些见解。
- **资格**：如果企业协议（EA）下的用户具有与组织 Microsoft Azure 计费、服务、服务和/或成本管理相关的功能，则他们可能参与该产品/服务。
- **排除项**：
  ...
  - 术语：产品/服务将于2017年6月1日开始，结束于2018年5月31日结束。  在这 12 个月期间，用户随时可以获取该套餐；不过无论获取时间是哪一天，该套餐均将于 2018 年 5 月 31 日终止。
...
有关如何为合约生成 API 密钥的详细信息，请访问 [Enterprise Portal](https://ea.azure.com/?WT.mc_id=azurebg_email_Trans_33675_1378_Service_Notice_EA_Customer_Power_BI_EA_Content_Pack_Apr26) 上的 API 报表帮助文件。 有关新内容包的详细信息，请参阅[Microsoft Azure 使用见解](/power-bi/desktop-connect-azure-cost-management)。
...
资源标记托管在 Azure 门户上。 你可以通过 [https://portal.azure.com](https://portal.azure.com) 联系 Azure 订阅团队。 请按照[本链接](https://docs.microsoft.com/azure/azure-portal/supportability/how-to-create-azure-support-request)（关于提出支持请求）中的步骤操作。
...
**原始单位总数：** 在详细使用情况报表中使用的数量。
**每单位 MOCP 资源：** 上游使用系统以不同的单位为每个服务发出使用情况。 预设**每单位消耗：** EA 度量单位。 预设**价格：** Azure EA 门户中的单价。
**总成本：** 从 Azure EA 门户的详细使用情况报表或承诺使用情况扩展成本。
