...
AdditionalInfo | 所有 | 服务特定的元数据。 例如，虚拟机的映像类型。
BillingAccountId<sup>1</sup> | 所有 | 根计费帐户的唯一标识符。
BillingAccountName | 所有 | 计费帐户的名称。
BillingCurrency | 所有 | 与计费帐户关联的货币。
BillingPeriod | EA、PAYG | 费用的计费周期。
BillingPeriodEndDate | 所有 | 计费周期的结束日期。
BillingPeriodStartDate | 所有 | 计费周期的开始日期。
BillingProfileId<sup>1</sup> | 所有 | EA 注册、PAYG 订阅、MCA 计费对象信息或 AWS 合并帐户的唯一标识符。
BillingProfileName | 所有 | EA 注册、PAYG 订阅、MCA 计费对象信息或 AWS 合并帐户的名称。
ChargeType | 所有 | 指示费用表示使用情况 (**Usage**)、购买 (**Purchase**) 还是退款 (**Refund**)。
ConsumedService | 所有 | 与费用相关联的服务的名称。
CostCenter<sup>1</sup> | EA、MCA | 为订阅定义的用于跟踪成本的成本中心（仅在启用计费周期适用于 MCA 帐户）。
费用 | EA、PAYG | 参见 CostInBillingCurrency。
...
日期<sup>1</sup> | 所有 | 费用项目的使用或购买日期。
EffectivePrice | 所有 | 计费周期的混合单价。 混合价格以求平均值的方式消除了单价中出现的任何波动，例如，使用渐变分层时，某段时间的价格会随数量的增加而降低。
ExchangeRateDate | MCA | 确定汇率的日期。
ExchangeRatePricingToBilling | MCA | 将以定价货币计算的成本转换为以计费货币计算的成本时所使用的汇率。
频率 | 所有 | 指示某项费用是否会重复收取。 费用可以一次性收取 (**OneTime**)、按月或按年重复收取 (**Recurring**)，或者按使用情况收取 (**UsageBased**)。
...
IsAzureCreditEligible | 所有 | 指示是否有资格使用 Azure 额度支付费用（值： True、False）。
位置 | MCA | 资源运行时所在的数据中心位置。
MeterCategory | 所有 | 计量的分类类别的名称。 例如，“云服务”和“网络”。
MeterId<sup>1</sup> | 所有 | 计量的唯一标识符。
MeterName | 所有 | 计量的名称。
MeterRegion | 所有 | 按位置定价的服务的数据中心位置的名称。 参见“位置”。
MeterSubCategory | 所有 | 计量子分类类别的名称。
OfferId<sup>1</sup> | 所有 | 购买的套餐的名称。
...
ProductOrderId | 所有 | 产品订单的唯一标识符。
ProductOrderName | 所有 | 产品订单的唯一名称。
PublisherName | 所有 | 市场服务的发布者。
PublisherType | 所有 | 发布者类型（值： **Azure**、 **AWS**、 **Marketplace**）。
数量 | 所有 | 购买或消耗的单位数。
...
SubscriptionId<sup>1</sup> | 所有 | Azure 订阅的唯一标识符。
SubscriptionName | 所有 | Azure 订阅的名称。
标记<sup>1</sup> | 所有 | 分配给资源的标记。 不包括资源组标记。 可以用来将内部退款的成本进行分组或分发。 有关详细信息，请参阅[使用标记来组织 Azure 资源](https://azure.microsoft.com/updates/organize-your-azure-resources-with-tags/)。
条款 | 所有 | 显示套餐的有效期限。 例如：在保留实例的情况下，它将显示12个月作为术语。 对于一次性购买或定期购买，“期限”为 1 个月（SaaS、市场支持）。 这不适用于 Azure 消耗。
UnitOfMeasure | 所有 | 用于服务计费的度量单位。 例如，计算服务按小时计费。
