...
| --- | --- | --- | --- |
| 可用性区域 | lineitem/AvailabilityZone | 所有 |   |
| 位置 | 产品/区域 | 所有 |   |
| 计量 |   | 所有 |   |
| 计量类别 | lineItem/ProductCode | 所有 |   |
| 计量子类别 | lineitem/UsageType | 所有 |   |
| 操作 | lineItem/Operation | 所有 |   |
| 资源 | lineItem/ResourceId | 所有 |   |
| 资源类型 | product/instanceType | 所有 | 如果 product/instanceType 为 null，则使用 lineItem/UsageType。 |
| ResourceGuid | N/A | 所有 | Azure 计量器 GUID。 |
| 服务名称 | product/ProductName | 所有 | 如果 product/ProductName 为 null，则使用 lineItem/ProductCode。 |
| 服务层 |   |   |   |
| 订阅 ID | lineItem/UsageAccountId | 合并的帐户和管理组 |   |
| 订阅名称 | N/A | 合并的帐户和管理组 | 帐户名称是使用 AWS 组织 API 收集的。 |
| 标记 | resourceTags/\* | 所有 | 将从用户定义的标记中删除_用户：_ 前缀以允许跨云标记。 _Aws：_ 前缀保持不变。 |
| 计费帐户 ID | bill/PayerAccountId | 管理组 |   |
| 计费帐户名称 | N/A | 管理组 | 帐户名称是使用 AWS 组织 API 收集的。 |
| 提供商 | N/A | 管理组 | AWS 或 Azure。 |
