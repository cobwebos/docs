...
若要创建自定义成本分摊模型，请在报告菜单上选择“成本”  &gt;  “成本管理”&gt;“成本分摊 360°”  。
...
成本模型列表将显示新成本模型的“处理状态”  。 在使用成本模型更新 Cloudyn 数据库之前，可能需要一些时间。 处理完成后，状态更新为“已完成”  。 然后，可以在“扩展筛选器”  &gt;“成本模型”  下的“成本分析”报告中查看来自成本模型的数据。

### <a name="category-manager"></a>类别管理器

类别管理器是一项数据清理工具，可帮助你将多个类别（标记）的值合并以创建新类别。 它是一个简单的基于规则的工具，可以在其中选择一个类别并创建规则以合并现有的值。 例如，你可能有现有的 R&amp;D  和开发人员  类别，其中二者都表示开发组。
