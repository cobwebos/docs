...
Azure 文件存储根据行业标准[服务器消息块 (SMB) 协议](https://msdn.microsoft.com/library/windows/desktop/aa365233.aspx)和 [Samba/通用 Internet 文件系统 (CIFS)](https://technet.microsoft.com/library/cc939973.aspx) 在云中提供网络文件共享。 Azure 文件共享可以由客户端（例如本地部署的 Windows、macOS、Linux）或 Azure 虚拟机同时装载。 通用存储帐户允许使用单个帐户访问 Azure 文件存储以及 Blob、Azure 虚拟机磁盘、队列之类的其他服务。
