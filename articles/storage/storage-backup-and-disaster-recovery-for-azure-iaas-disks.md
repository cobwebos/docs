...
[Azure 备份服务](https://azure.microsoft.com/services/backup/)可用于备份和 DR，适用于[托管磁盘](storage-managed-disks-overview.md)和[非托管磁盘](storage-about-disks-and-vhds-windows.md#unmanaged-disks)。 可以创建备份作业，其中包含基于时间的备份、VM 轻松还原和备份保留策略。 

如果将[高级存储磁盘](storage-premium-storage.md)、[托管磁盘](storage-managed-disks-overview.md)或其他类型磁盘与[本地冗余存储 (LRS)](storage-redundancy.md#locally-redundant-storage) 选项结合使用，请务必利用定期 DR 备份。 Azure 备份将数据存储到恢复服务保管库中，以供长期保留。 对备份恢复服务保管库选择[异地冗余存储 (GRS)](storage-redundancy.md#geo-redundant-storage) 选项。 这可确保将备份复制到其他 Azure 区域，以免受到区域灾难影响。

对于[非托管磁盘](storage-about-disks-and-vhds-windows.md#unmanaged-disks)，可以将 LRS 存储类型用于 IaaS 磁盘，但要确保为 Azure 备份恢复服务保管库启用 GRS 选项。
...
如果无法使用 Azure 备份服务，可以使用快照实现自己的备份机制。 复杂的是，需要为 VM 使用的所有磁盘创建一致性快照，再将这些快照复制到另一个区域。 因此，Azure 认为相对于生成自定义解决方案，利用备份服务是更好的选择。 如果将 RA-GRS/GRS 存储用于磁盘，快照会自动复制到备用数据中心。 如果将 LRS 存储用于磁盘，需要自行复制数据。 有关详细信息，请参阅[使用增量快照备份 Azure 非托管 VM 磁盘](storage-incremental-snapshots.md)。

快照表示处于特定时间点的对象。 快照因保留数据的大小递增而产生费用。 有关详细信息，请参阅[创建 blob 快照](storage-blob-snapshots.md)。
...
3.  然后，为所有磁盘[创建 blob 快照](storage-blob-snapshots.md)
...
若要有效复制 DR 增量快照，请参阅[使用增量快照备份 Azure 非托管 VM 磁盘](storage-incremental-snapshots.md)中的说明。
