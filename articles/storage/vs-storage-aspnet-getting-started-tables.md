...
1. 在“解决方案资源管理器”中展开“视图”文件夹，右键单击“表”，然后从上下文菜单中选择“添加”->“视图”。

1. 在“添加视图”对话框中，输入 **CreateTable** 作为视图名称，然后选择“添加”。

1. 打开 `CreateTable.cshtml`，然后按以下代码片段所示对其进行修改：
...
1. 运行应用程序，然后选择“创建表”，查看结果是否与以下屏幕截图类似：
...
*实体*使用派生自 **TableEntity** 的自定义类映射到 C\# 对象。 若要将实体添加到表，请创建用于定义实体的属性的类。 本部分介绍如何定义可将客户的名字和姓氏分别用作行键和分区键的实体类。 实体的分区键和行键共同唯一地标识表中的实体。 查询分区键相同的实体的速度快于查询分区键不同的实体的速度，但使用不同的分区键可实现更高的并行操作可伸缩性。 对于应存储在表服务中的任何属性，该属性必须是公开设置和检索值的受支持类型的公共属性。
实体类*必须*声明公共无参数构造函数。
...
1. 打开 `AddEntity.cshtml`，然后按以下代码片段所示对其进行修改：
...
1. 运行应用程序，然后选择“添加实体”，查看结果是否与以下屏幕截图类似：
...
1. 打开 `GetSingle.cshtml`，然后按以下代码片段所示对其进行修改：
...
1. 运行应用程序，然后选择“获取单个实体”，查看结果是否与以下屏幕截图类似：
...
1. 打开 `GetPartition.cshtml`，然后按以下代码片段所示对其进行修改：
...
1. 运行应用程序，然后选择“获取分区”，查看结果是否与以下屏幕截图类似：
...
1. 打开 `DeleteEntity.cshtml`，然后按以下代码片段所示对其进行修改：
...
1. 运行应用程序，然后选择“删除实体”，查看结果是否与以下屏幕截图类似：
