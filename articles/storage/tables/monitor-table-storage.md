---
title: 监视 Azure 表存储 |Microsoft Docs
description: 了解如何监视 Azure 表存储的性能和可用性。 监视 Azure 表存储数据、了解配置以及分析指标和日志数据。
author: normesta
services: storage
ms.service: storage
ms.topic: conceptual
...
如果你有依赖 Azure 资源的关键应用程序和业务流程，则需要监视这些资源的可用性、性能和操作。 本文介绍 Azure 表存储生成的监视数据，以及如何使用 Azure Monitor 的功能来分析此数据的警报。
...
每个表存储资源的 Azure 门户中的 " **概述** " 页包含资源使用情况的简要视图，例如请求和每小时计费。 这些信息非常有用，但只提供少量监视数据。 其中的某些数据是自动收集的，并且可在创建资源后立即进行分析。 你可以使用某些配置启用其他数据收集类型。

## <a name="what-is-azure-monitor"></a>说明是 Azure Monitor？
Azure 表存储使用 [Azure Monitor](../../azure-monitor/overview.md)，这是 Azure 中的一种完整的堆栈监视服务来创建监视数据。 Azure Monitor 提供了一整套用于监视 Azure 资源以及其他云和本地资源的功能。 
...
Azure 表存储会收集与其他 Azure 资源相同的监视数据，如 [监视 Azure 资源的数据](../../azure-monitor/insights/monitor-azure-resource.md#monitoring-data)中所述。 

有关 Azure 表存储创建的指标和日志指标的详细信息，请参阅 [Azure 表存储监视数据参考](monitor-table-storage-reference.md) 。
...
![在 Azure 门户中访问指标的屏幕截图](./media/monitor-table-storage/access-metrics-portal.png)

对于支持维度的指标，可使用所需的维度值筛选指标。 以下示例演示了如何通过选择“API 名称”维度的值，在特定操作上查看帐户级别的“事务” 。

![在 Azure 门户中访问包含维度的指标的屏幕截图](./media/monitor-table-storage/access-metrics-portal-with-dimension.png)

有关 Azure 存储支持的维度的完整列表，请参阅[指标维度](monitor-table-storage-reference.md#metrics-dimensions)。

Azure 表存储的指标位于以下命名空间中： 

- Microsoft.Storage/storageAccounts
- Microsoft.Storage/storageAccounts/tableServices

有关包括 Azure 表存储的所有 Azure Monitor 支持指标的列表，请参阅 [Azure Monitor 支持的指标](https://docs.microsoft.com/azure/azure-monitor/platform/metrics-supported)。
...
可以列出存储帐户或表存储服务的指标定义。 请使用 [Get-AzMetricDefinition](https://docs.microsoft.com/powershell/module/az.monitor/get-azmetricdefinition) cmdlet。
...
您可以读取存储帐户或表存储服务的帐户级别指标值。 使用 [Get-AzMetric](https://docs.microsoft.com/powershell/module/Az.Monitor/Get-AzMetric) cmdlet。
...
可以列出存储帐户或表存储服务的指标定义。 使用 [az monitor metrics list-definitions](https://docs.microsoft.com/cli/azure/monitor/metrics#az-monitor-metrics-list-definitions) 命令。
...
您可以读取存储帐户或表存储服务的指标值。 请使用 [az monitor metrics list](https://docs.microsoft.com/cli/azure/monitor/metrics#az-monitor-metrics-list) 命令。
...
有关这些日志中显示的字段的详细参考信息，请参阅 [Azure 表存储监视数据参考](monitor-table-storage-reference.md)。
...
仅在针对服务终结点发出请求时才会创建日志条目。 例如，如果存储帐户在其表终结点中具有活动，而在其 blob 或队列终结点中没有活动，则仅创建与表服务相关的日志。 Azure 存储日志包含有关成功和失败的存储服务请求的详细信息。 可以使用该信息监视各个请求和诊断存储服务问题。 将最大程度地记录请求。
...
表存储服务（如创建或删除日志）发出的请求不会被记录。 若要查看所记录数据的完整列表，请参阅[存储记录的操作和状态消息](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages)和[存储日志格式](monitor-table-storage-reference.md)。
...
不会记录所有其他失败的匿名请求。 若要查看所记录数据的完整列表，请参阅[存储记录的操作和状态消息](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages)和[存储日志格式](monitor-table-storage-reference.md)。
...
![审核日志](media/monitor-table-storage/event-hub-log.png)
...
- 有关 Azure 表存储创建的日志和指标的参考，请参阅 [Azure 表存储监视数据参考](monitor-table-storage-reference.md)。
