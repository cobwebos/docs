...
ms.locfileid: "67269365"
---
# <a name="overview-of-azure-files-azure-active-directory-domain-service-aad-ds-authentication-support-for-smb-access-preview"></a>Azure 文件 Azure Active Directory 域服务 (AAD DS) 身份验证支持的 SMB 访问权限 （预览版） 概述
[!INCLUDE [storage-files-aad-auth-include](../../../includes/storage-files-aad-auth-include.md)]

若要了解如何启用 AAD DS 身份验证用于 Azure 文件，请参阅[启用 Azure Active Directory 域服务身份验证通过 Azure 文件 （预览版） 的 SMB](storage-files-active-directory-enable.md)。

## <a name="glossary"></a>术语表 
它是有助于您了解 SMB 上的 Azure 文件与 Azure AD 域服务身份验证的一些关键术语：
...
用于 Azure 文件的 azure AD 域服务身份验证通过使用共享密钥身份验证提供以下几个好处：

-   **扩展传统的基于标识的文件共享访问体验到在云中使用 Azure AD 和 Azure AD 域服务**  
    如果你打算"提升和转移"到云中，使用 Azure 文件替换传统文件服务器，则你可能希望你的应用程序进行身份验证与 Azure AD 应用程序的凭据来访问文件数据。 Azure 文件支持使用 Azure AD 凭据来访问 Azure 文件通过 SMB 从 AAD DS 已加入域的 Windows Vm。 还可选择将所有本地 Active Directory 对象同步到 Azure AD，以保留用户名、密码和其他组分配。

-   **对 Azure 文件共享强制实施粒度访问控制**  
    可以授予对特定的标识在共享、 目录或文件级别的权限。 例如，假设多个团队使用一个 Azure 文件共享进行项目协作。 你可以向所有团队授予访问非敏感目录的权限，而仅授权财务团队访问含敏感财务数据的目录。 
...
### <a name="enable-azure-ad-domain-service-authentication-for-smb-access"></a>启用 SMB 访问权限的 Azure AD 域服务身份验证
您可以在 2018 年 9 月 24 日之后创建的新的和现有存储帐户上的 Azure 文件的 Azure AD 域服务身份验证。 

启用此功能前，验证是否已部署 Azure AD 域服务的主存储帐户与之关联的 Azure AD 租户。 如果尚未设置 Azure AD 域服务，请按照[使用 Azure 门户启用 Azure Active Directory 域服务](../../active-directory-domain-services/create-instance.md)中提供的分步指导进行操作。

部署 Azure AD 域服务通常需要 10 至 15 分钟。 部署 Azure AD 域服务后，即可启用通过 SMB 为 Azure 文件进行 Azure AD 身份验证这一功能。 有关详细信息，请参阅[启用 Azure Active Directory 域服务的 Azure 文件 （预览版） 身份验证通过 SMB](storage-files-active-directory-enable.md)。 

### <a name="configure-share-level-permissions-for-azure-files"></a>为 Azure 文件配置共享级别权限
一旦启用 Azure AD 域服务身份验证，可以配置为 Azure AD 标识的自定义 RBAC 角色并分配到的存储帐户中的任何文件共享的访问权限。

在已加入域的 VM 上运行的应用程序尝试装载 Azure 文件共享或者访问目录或文件时，系统会验证应用程序的 Azure AD 凭据以确保共享级别权限和 NTFS 权限正确。 有关配置共享级别权限的信息，请参阅[SMB （预览版） 上的启用 Azure Active Directory 域服务身份验证](storage-files-active-directory-enable.md)。
...
Azure 文件现在支持将数据复制到 Azure 文件共享时保留目录或文件的 Acl。 可以将一个目录或文件的 Acl 复制到 Azure 文件。 例如，可使用带 `/copy:s` 标志的 [robocopy](https://docs.microsoft.com/windows-server/administration/windows-commands/robocopy) 将数据和 ACL 复制到 Azure 文件共享。 ACL 保留启用默认情况下，并且不需要显式启用你的存储帐户上的 Azure AD 域服务身份验证功能。 
