...
ms.locfileid: "82176084"
---
# <a name="planning-for-an-azure-file-sync-deployment"></a>规划 Azure 文件同步部署

:::row:::
    :::column:::
        [![访谈 Azure 文件同步的访谈和演示，单击即可玩！](./media/storage-sync-files-planning/azure-file-sync-interview-video-snapshot.png)](https://www.youtube.com/watch?v=nfWLO7F52-s)
    :::column-end:::
    :::column:::
        Azure 文件同步是一项服务，可用于在本地 Windows Server 或云 VM 上缓存多个 Azure 文件共享。 
        
        本文介绍 Azure 文件同步概念和功能。 熟悉 Azure 文件同步后，请按照[Azure 文件同步的部署指南](storage-sync-files-deployment-guide.md)来试用此服务。        
    :::column-end:::
:::row-end:::

文件将存储在[Azure 文件共享](storage-files-introduction.md)中的云中。 可以通过两种方式使用 azure 文件共享：直接安装这些无服务器 Azure 文件共享（SMB），或使用 Azure 文件同步在本地缓存 Azure 文件共享。你选择哪种部署选项会更改你在规划部署时需要考虑的事项。 
...
若要在 Windows Server 上启用同步功能，你必须安装 Azure 文件同步可下载的代理。 Azure 文件同步代理提供了两个主要组件`FileSyncSvc.exe`：、负责监视服务器终结点上的更改和启动同步会话的后台 Windows 服务，以及`StorageSync.sys`支持云分层和快速灾难恢复的文件系统筛选器。  
...
| 版本 | 支持的 SKU | 支持的部署选项 |
|---------|----------------|------------------------------|
| Windows Server Standard 2012 R2 | Datacenter、Standard 和 IoT | 完整和核心 |
...
\*目前不建议同步超过100000000个文件 & 目录。 这是基于我们测试的阈值的软限制。 有关详细信息，请参阅[Azure 文件可伸缩性和性能目标](storage-files-scale-targets.md#azure-file-sync-scale-targets)。
...
Azure 文件同步适用于基于 AD 的标准标识，无需进行任何特殊设置，只需设置同步。使用 Azure 文件同步时，一般情况下，大多数访问都是通过 Azure 文件同步缓存服务器而不是通过 Azure 文件共享来完成的。 由于服务器终结点位于 Windows Server 上，并且 Windows Server 长时间支持 AD 和 Windows 样式 Acl，因此除了确保注册到存储同步服务的 Windows 文件服务器已加入域外，不需要任何操作。 Azure 文件同步将 Acl 存储在 Azure 文件共享中的文件上，并将其复制到所有服务器终结点。
...
类似于 BitLocker 的第三方产品的工作方式类似于 BitLocker，在 NTFS 卷下，也应该与 Azure 文件同步完全透明地配合使用。 
...
Azure 文件同步不会与 NTFS 加密文件系统（NTFS EFS）或位于文件系统但文件数据流下面的第三方加密解决方案交互。 

### <a name="encryption-in-transit"></a>传输中加密

> [!NOTE]
> Azure 文件同步服务将删除对8月2020的 TLS 1.0 和1.1 的支持。 默认情况下，所有支持的 Azure 文件同步代理版本都已使用 TLS 1.2。 如果在服务器上禁用了 TLS 1.2 或使用了代理，则可能会发生使用较早版本的 TLS。 如果使用代理，则建议检查代理配置。 5/1/2020 之后添加的 Azure 文件同步服务区域将仅支持 TLS 1.2，并将从8月2020中的现有区域中删除 TLS 1.0 和1.1 支持。  有关详细信息，请参阅[故障排除指南](storage-sync-files-troubleshoot.md#tls-12-required-for-azure-file-sync)。
...
- 仅在新文件服务器上创建服务器终结点，并使用`robocopy`将数据从旧文件共享复制到中。 根据新服务器上文件共享的拓扑（每个卷上有多少共享、每个卷的可用空间是多少，等等），可能需要临时预配其他存储，因为从旧服务器到本地数据`robocopy`中心内的新服务器，在本地数据中心内的新服务器中，将其完成的速度要快于 Azure 文件同步会将数据移动到 Azure。
