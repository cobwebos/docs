...
如果一切成功进行，则应会看到以下输出，其中 `192.168.0.5` 是虚拟网络中专用终结点的专用 IP 地址。 请注意，仍应使用 storageaccount.file.core.windows.net 来对文件共享进行计数，而不是 privatelink 路径。
