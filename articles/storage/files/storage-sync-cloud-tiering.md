...
Windows 使用的所有文件系统，根据群集大小（也称为 "分配单元大小"）来组织硬盘。 群集大小表示可用于保存文件的最小磁盘空间量。 如果文件大小不能超过群集大小的偶数倍，则必须使用额外的空间来保存到群集大小的下一个多个空间。
...
创建卷时，可以使用其他群集大小手动格式化该卷。 如果卷源自较早版本的 Windows，则默认群集大小也可能不同。 [本文详细介绍了默认群集大小。](https://support.microsoft.com/help/140365/default-cluster-size-for-ntfs-fat-and-exfat) 即使选择了小于 4 KB 的群集大小，也可以分层的最小文件大小的 8 KB 限制。 （即使在技术上，2 KB 的群集大小将等于小于 8 KB。）
