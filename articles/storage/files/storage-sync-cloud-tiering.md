...
对于代理版本1.x 和更高版本，对于要分层的文件，最小文件大小基于文件系统群集大小（两个文件系统群集大小）。 例如，如果 NTFS 文件系统群集大小为4KB，则生成的文件的最小文件大小为8KB。 对于代理版本3.x 和更早版本，文件到层级的最小文件大小为64KB。
...
云分层使用 "上次访问时间" 来确定应分层哪些文件。 云分层筛选器驱动程序（storagesync.sys）跟踪上次访问时间，并记录云分层热商店中的信息。 你可以使用本地 PowerShell cmdlet 查看热度商店。
