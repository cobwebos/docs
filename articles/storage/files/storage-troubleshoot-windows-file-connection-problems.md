...
ms.locfileid: "79136899"
---
# <a name="troubleshoot-azure-files-problems-in-windows"></a>在 Windows 中排查 Azure 文件问题

本文列出了从 Windows 客户端进行连接时，与 Microsoft Azure 文件相关的常见问题。 此外，还提供了这些问题的可能原因和解决方法。 除了本文中的故障排除步骤外，您还可以使用[AzFile诊断](https://gallery.technet.microsoft.com/Troubleshooting-tool-for-a9fa1fe5) 来确保 Windows 客户端环境具有正确的先决条件。 AzFileDiagnostics 会自动检测本文中提及的大多数症状，并帮助设置环境，以实现最佳性能。 还可以在 [Azure 文件共享疑难解答](https://support.microsoft.com/help/4022301/troubleshooter-for-azure-files-shares)中找到此信息，该疑难解答提供相关步骤来帮助解决在连接/映射/装载 Azure 文件共享时遇到的问题。
...
### <a name="cause-2-virtual-network-or-firewall-rules-are-enabled-on-the-storage-account"></a>原因 2：在存储帐户上启用虚拟网络或防火墙规则 
...
### <a name="cause-3-share-level-permissions-are-incorrect-when-using-identity-based-authentication"></a>原因 3：使用基于标识的身份验证时，共享级权限不正确

如果用户使用活动目录 （AD） 或 Azure 活动目录域服务 （Azure AD DS） 身份验证访问 Azure 文件共享，则如果共享级别的权限不正确，则访问文件共享失败，出现"拒绝访问"错误。 

### <a name="solution-for-cause-3"></a>原因 3 的解决方案

要更新共享级别权限，请参阅[向标识分配访问权限](https://docs.microsoft.com/azure/storage/files/storage-files-identity-auth-active-directory-domain-service-enable#assign-access-permissions-to-an-identity)。
...
Azure 文件同步可以将本地 Windows 服务器转换为 Azure 文件共享的快速缓存。 可以使用 Windows Server 上可用的任意协议本地访问数据，包括 SMB、NFS 和 FTPS。 Azure 文件同步在端口 443 上工作，因此可用作从端口 445 被阻止的客户端访问 Azure 文件的解决方法。 [了解如何设置 Azure 文件同步](https://docs.microsoft.com/azure/storage/files/storage-sync-files-extend-servers)。

#### <a name="solution-2---use-vpn"></a>解决方案 2 - 使用 VPN
通过设置 VPN 到您的特定存储帐户，流量将经过一个安全隧道，而不是通过互联网。 按照[说明设置 VPN](storage-files-configure-p2s-vpn-windows.md)从 Windows 访问 Azure 文件。
...
### <a name="cause-2-ntlmv1-is-enabled"></a>原因 2： 启用了 NTMv1
...
### <a name="cause-1-virtual-network-or-firewall-rules-are-enabled-on-the-storage-account"></a>原因 1：在存储帐户上启用虚拟网络或防火墙规则

### <a name="solution-for-cause-1"></a>原因 1 的解决方案

验证是否已在存储帐户上正确配置虚拟网络和防火墙规则。 若要测试虚拟网络或防火墙规则是否导致此问题，请将存储帐户上的设置临时更改为“允许来自所有网络的访问”****。 若要了解详细信息，请参阅[配置 Azure 存储防火墙和虚拟网络](https://docs.microsoft.com/azure/storage/common/storage-network-security)。

### <a name="cause-2-your-user-account-does-not-have-access-to-the-storage-account"></a>原因 2：您的用户帐户无法访问存储帐户
...
- 使用 cmdkey 命令将凭据添加到凭据管理器中。 通过交互式登录或使用 从服务帐户上下文下的命令行执行此操作`runas`。
...
## <a name="error-aaddstenantnotfound-in-enabling-azure-active-directory-domain-service-aad-ds-authentication-for-azure-files-unable-to-locate-active-tenants-with-tenant-id-aad-tenant-id"></a>在为 Azure 文件启用 Azure 活动目录域服务 （AAD DS） 身份验证时未找到错误 AadDstenant"无法查找具有租户 ID ad-租户 id 的活动租户"

### <a name="cause"></a>原因

当您尝试在存储帐户[上的 Azure 文件上启用 Azure 文件上的 Azure 活动目录域服务 （Azure AD DS） 身份验证](storage-files-identity-auth-active-directory-domain-service-enable.md)时，将发生错误 AadDsTenantNotFound，该存储帐户上未在关联订阅的 AAD 租户上创建[AAD 域服务 （AAD DS）。](https://docs.microsoft.com/azure/active-directory-domain-services/active-directory-ds-overview)  
...
## <a name="error-system-error-1359-has-occurred-an-internal-error-received-over-smb-access-to-file-shares-with-azure-active-directory-domain-service-aad-ds-authentication-enabled"></a>错误 '系统错误 1359 已发生。 通过启用 Azure 活动目录域服务 （AAD DS） 身份验证的 SMB 访问文件共享而收到的内部错误

### <a name="cause"></a>原因

错误 '系统错误 1359 已发生。 当您尝试通过启用 AAD DS 身份验证连接到文件共享时，将发生内部错误，该身份验证针对具有域 DNS 名称的 AAD DS 以数字字符开头。 例如，如果您的 AAD DS 域 DNS 名称是"1域"，则在尝试使用 AAD 凭据装载文件共享时，将出现此错误。 
