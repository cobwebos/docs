...
* **问：如何才能开始使用 Azure 文件存储？**  
    ...
    * [如何使用门户创建 Azure 文件共享](storage-how-to-create-file-share.md#create-file-share-through-the-portal)
...
* **问：如何为 Azure 文件存储启用服务器端加密？**
    ...
    Azure 文件存储的[服务器端加密](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)通常适用于所有区域以及公有云和国内云。 可以使用 [Azure 门户](https://portal.azure.com/)、[Microsoft Azure 存储资源提供程序 API](/rest/api/storagerp/storageaccounts)、[Azure Powershell](https://msdn.microsoft.com/library/azure/mt607151.aspx) 或 [Azure CLI](../common/storage-azure-cli.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json) 为 Azure 文件存储启用 SSE。
    
    为 Azure 文件存储启用 SSE 后，写入存储帐户中文件存储的任何新数据都会自动进行加密。 此功能可用于写入到现有或新存储帐户中的现有或新共享的所有新数据。 启用此功能不需额外付费。 详细了解[如何为 Azure 文件存储启用 SSE](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)。
...
* **问：Azure 文件存储存在哪些缩放限制？**
    若要了解 Azure 文件存储的可伸缩性和性能目标，请参阅 [Azure 存储可伸缩性和性能目标](../common/storage-scalability-targets.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json#scalability-targets-for-blobs-queues-tables-and-files)。
...
* **问：如何排查 Azure 文件存储错误？**
    ...
    有关端到端疑难解答指南，可以参阅 [Azure 文件存储疑难解答文章](storage-troubleshoot-windows-file-connection-problems.md)。 
...
* [排查 Azure 文件存储问题](storage-troubleshoot-linux-file-connection-problems.md)
