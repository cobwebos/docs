...
在部署 Azure 文件同步之前，需为本教程进行一些设置。除了创建 Azure 存储帐户和文件共享，还需创建 Windows Server 2016 Datacenter VM 并针对 Azure 文件同步准备该服务器。
...
1. 当 Azure 存储帐户部署完成以后，请单击“转到资源”。
1. 在存储帐户窗格中单击“文件”。

    ![单击“文件”](./media/storage-sync-files-extend-servers/click-files.png)

1. 单击“+ 文件共享”。

    ![单击”添加文件共享”按钮](./media/storage-sync-files-extend-servers/create-file-share-portal2.png)

1. 将新文件共享命名为 *afsfileshare*，输入“1”作为“配额”，然后单击“创建”。 配额最高可以为 5 TiB，但本教程只需 1 GB。

    ![为新文件共享提供名称和配额](./media/storage-sync-files-extend-servers/create-file-share-portal3.png)

1. 选择新文件共享，然后在文件共享位置单击“上传”。

    ![上传文件](./media/storage-sync-files-extend-servers/create-file-share-portal5.png)

1. 浏览到在其中创建了 .txt 文件的 *FilesToSync* 文件夹，选择 *mytestdoc.txt*，然后单击“上传”。

    ![浏览文件共享](./media/storage-sync-files-extend-servers/create-file-share-portal6.png)

目前已在 Azure 中创建 Azure 存储帐户和包含一个文件的文件共享。 现在需创建 Azure VM，其中的 Windows Server 2016 Datacenter 代表本教程中的本地服务器。
...
1. 在“基本信息”选项卡的“项目详细信息”下，选择为本教程创建的资源组。

   ![在门户边栏选项卡中输入 VM 的基本信息](./media/storage-sync-files-extend-servers/vm-resource-group-and-subscription.png)

1. 在“实例详细信息”下提供一个 VM 名称，例如 *myVM*。
...
1. 在“数据磁盘”下，单击“创建并附加新磁盘”。

1. 在本教程中，请将“大小(GiB)”更改为“1 GB”，其余保留默认设置。
...
1. 单击“创建”。
   ...
   可以单击“通知”图标，观察**部署进度**。 创建新的 VM 需要数分钟才能完成。

1. VM 部署完成以后，请单击“转到资源”。
...
1. 在 Azure 门户中，单击虚拟机属性页上的“连接”。

   ![从门户连接到 Azure VM](./media/storage-sync-files-extend-servers/connect-vm.png)

1. 在“连接到虚拟机”页面中，保留默认选项，以使用 IP 地址通过端口 3389 进行连接，然后单击“下载 RDP 文件”。

   ![下载 RDP 文件](./media/storage-sync-files-extend-servers/download-rdp.png)

1. 打开下载的 RDP 文件，然后在出现提示时单击“连接”。
1. 在“Windows 安全性”窗口中，依次选择“更多选择”、“使用其他帐户”。 以 *localhost\username* 形式键入用户名，输入为虚拟机创建的密码，然后单击“确定”。

   ![更多选择](./media/storage-sync-files-extend-servers/local-host2.png)

1. 你可能会在登录过程中收到证书警告。 单击“是”或“继续”以创建连接。
...
1. 仍在 **Windows Server 2016 Datacenter** VM 中时，请单击“文件和存储服务” > “卷” > “磁盘”。

    ![数据磁盘](media/storage-sync-files-extend-servers/your-disk.png)

1. 右键单击名为“Msft 虚拟磁盘”的 1 GB 磁盘，然后单击“新建卷”。
1. 在保留默认设置的情况下完成向导，记下分配的驱动器号，然后单击“创建”。
1. 单击“关闭”。

   目前已将磁盘联机并创建一个卷。 可以确认添加数据磁盘的操作是否成功，方法是在 VM 上打开资源管理器，然后确认新驱动器是否存在。

1. 在 VM 的资源管理器中展开“此电脑”，然后双击新驱动器。 在此示例中，它是 F: 驱动器。
1. 右键单击并选择“新建” > “文件夹”。 将文件夹命名为 *FilesToSync*。
1. 双击 **FilesToSync** 文件夹。
...
部署 Azure 文件同步时，首先会将一个“存储同步服务”资源放入所选订阅的资源组中。 存储同步服务从部署到其中的订阅和资源组继承访问权限。

1. 在 Azure 门户中单击“创建资源”，然后搜索“Azure 文件同步”。
...
1. 选择“创建”，打开“部署存储同步”选项卡。
   ...
   | 值 | 说明 |
   | ----- | ----- |
   | **名称** | 存储同步服务的唯一名称（按订阅）。<br><br>在本教程中，我们将使用 *afssyncservice02*。 |
   | **订阅** | 用于本教程的订阅。 |
   | **资源组** | 用于本教程的资源组。<br><br>在本教程中，我们将使用 *afsresgroup101918*。 |
   | **位置** | 美国东部 |
...
1. 切换回 **Windows Server 2016 Datacenter** VM，打开 **Internet Explorer**。
1. 访问 [Microsoft 下载中心](https://go.microsoft.com/fwlink/?linkid=858257)。 向下滚动到“Azure 文件同步代理”部分并单击“下载”。

   ![同步代理下载](media/storage-sync-files-extend-servers/sync-agent-download.png)

1. 勾选 **StorageSyncAgent_V3_WS2016.EXE** 对应的框，单击“下一步”。

   ![选择代理](media/storage-sync-files-extend-servers/select-agent.png)

1. 单击“允许一次” > “运行” > “打开”，打开该文件。
1. 如果尚未关闭 PowerShell 窗口，请将其关闭。
1. 接受**存储同步代理设置向导**中的默认设置。
1. 单击“安装”。
1. 单击“完成”。

你已在 **Windows Server 2016 Datacenter** VM 上部署 Azure 同步服务并安装代理。 现在需将 VM 注册到**存储同步服务**。
...
1. 提供以下信息：
   ...
   | **资源组** | 包含本教程的存储同步服务的资源组。 在本教程中，我们一直使用 *afsresgroup101918*。 |
   | **存储同步服务** | 用于本教程的存储同步服务的名称。 在本教程中，我们一直使用 *afssyncservice02*。 |

1. 单击“注册”，完成服务器注册。
1. 在注册过程中，系统会提示进行其他登录。 登录并单击“下一步”。
1. 单击“确定”。

## <a name="create-a-sync-group"></a>创建同步组
同步组定义一组文件的同步拓扑。 同步组中必须包含一个表示 Azure 文件共享的云终结点，以及一个或多个服务器终结点。 服务器终结点表示已注册的服务器上的路径。

1. 若要创建同步组，请在 [Azure 门户](https://portal.azure.com/)的为本教程创建的存储同步服务中选择“+ 同步组”。 在本教程中，我们使用了 *afssyncservice02* 作为示例。

   ![在 Azure 门户中创建新的同步组](media/storage-sync-files-extend-servers/add-sync-group.png)

1. 在打开的窗格中输入以下信息，创建具有云终结点的同步组：

   | 值 | Description |
   | ----- | ----- |
   | **同步组名称** | 此名称在存储同步服务内必须是唯一的，但可以是符合逻辑的任何名称。 在本教程中，我们将使用 *afssyncgroup*。|
   | **订阅** | 在其中部署了本教程的存储同步服务的订阅。 |
   | **存储帐户** |单击“选择存储帐户”。 在显示的窗格中，选择包含为本教程创建的 Azure 文件共享的存储帐户。 我们使用了 *afsstoracct101918*。 |
   | **Azure 文件共享** | 为本教程创建的 Azure 文件共享的名称。 我们使用了 *afsfileshare*。 |
...
服务器终结点代表已注册服务器上的特定位置，例如服务器卷中的文件夹。

1. 若要添加服务器终结点，请选择新创建的同步组，然后选择“添加服务器终结点”。

   ![在“同步组”窗格中添加一个新的服务器终结点](media/storage-sync-files-extend-servers/add-server-endpoint.png)

1. 在“添加服务器终结点”窗格中，输入以下信息，创建服务器终结点：

   | | |
   | ----- | ----- |
   | 值 | Description |
   | **已注册的服务器** | 为本教程创建的服务器的名称。 在本教程中，我们使用了 *afsvm101918* |
   | **路径** | 为本教程创建的驱动器的 Windows Server 路径。 在示例中，该路径为 *f:\filestosync*。 |
   | **云分层** | 在本教程中，保留为禁用状态。 |
   | **卷可用空间** | 在本教程中，保留为空白。 |
...
本教程介绍了使用 Azure 文件同步扩展 Windows Server 存储容量的基本步骤。单击以下链接可以更详尽地了解如何针对 Azure 文件同步部署进行规划。
