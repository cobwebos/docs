...
本文将演示如何使用 Microsoft Azure Blob 存储执行常见任务。示例是用 Objective-C 编写的，并使用了[用于 iOS 的 Azure 存储客户端库](https://github.com/Azure/azure-storage-ios)。涉及的任务包括**上载**、**列出**、**下载**和**删除** Blob。有关 Blob 的详细信息，请参阅[后续步骤](#next-steps)部分。你也可以下载[示例应用](https://github.com/Azure/azure-storage-ios/tree/master/BlobSample)以快速了解如何在 iOS 应用程序中使用 Azure 存储空间。
...
- **帐户名** - 存储帐户的名称
- **帐户密钥** - 如果你在使用 [Azure 门户](https://portal.azure.cn)，请导航到你的存储帐户，然后单击“密钥”图标以查看此信息。如果使用 [Azure 经典门户](https://manage.windowsazure.cn)，请在该门户中导航到你的存储帐户，然后单击“管理访问密钥”。
...
如你所见，使用 SAS 令牌时，不会在 iOS 应用程序中公开你的帐户名和帐户密钥。你可以通过查阅 [Shared Access Signature s: Understanding the SAS model](/documentation/articles/storage-dotnet-shared-access-signature-part-1/)（共享访问签名：了解 SAS 模型）了解有关 SAS 的详细信息。
