...
- 有关以编程方式操作 Blob 的详细信息，请参阅 [Get started with Azure Blob storage using .NET](/documentation/articles/storage-dotnet-how-to-use-blobs/)（通过 .NET 开始使用 Azure Blob 存储）。
...
> [AZURE.NOTE] 在 ASP.NET 中执行 Azure 存储空间调用的一些 API 是异步的。有关详细信息，请参阅[使用 Async 和 Await 进行异步编程](http://msdn.microsoft.com/zh-cn/library/hh191443.aspx)。以下示例中的代码假定正在使用异步编程方法。
...
Azure 存储空间支持块 Blob 和页 Blob。大多数情况下，推荐使用块 Blob。
