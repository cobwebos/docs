...
7.  **支持的操作系统：**在递交到 Azure 之前，客户可以使用以下操作系统之一，通过 Azure 导入/导出工具准备硬盘驱动器 - Windows 7、Windows 8、Windows 8.1、Windows 10*、Windows Server 2008 R2、Windows Server 2012 和 Windows Server 2012 R2。  

  > [AZURE.IMPORTANT]
  >  
  >  - 如果使用 Windows 10 计算机来准备硬盘驱动器，请参阅[常见问题部分](#frequently-asked-questions)来获取特殊说明。
  >  
...
**如何在 Windows 10 上准备 HDD？**
  
使用 Windows 10 准备 HDD 时，请遵循以下说明：
 
- 在为 Azure 导入/导出服务准备的 HDD 上手动启用 BitLocker 加密。在文件资源管理器中右键单击 HDD 驱动器号并选择“打开 BitLocker”，然后根据向导操作。  
- 请务必记得恢复密钥。  
- 当系统要求你选择加密模式时，请选择“兼容模式(最适合可从此设备移动的驱动器)”  
- 加密完成之后，运行 Azure 导入/导出工具将数据复制到此 HDD。   
- 使用 */bk* 参数，并在此参数值中指定恢复密钥。   
- 请不要使用 */format* 和 */encrypt* 参数。  

**导入/导出服务的定价是什么？**

- 有关定价信息，请参阅[定价页](/pricing/overview/)。
