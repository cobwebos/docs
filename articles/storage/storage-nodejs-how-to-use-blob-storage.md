---
title: "如何通过 Node.js 使用 Blob 存储 | Microsoft Docs"
...
有关如何创建 Node.js 应用程序的说明，请参阅[在 Azure App Service 中创建 Node.js Web 应用]、[生成 Node.js 应用程序并将其部署到 Azure 云服务] 、或[使用 Web Matrix 构建 Node.js Web 应用并将其部署到 Azure]。
...
若要了解在 [Azure 门户](https://portal.azure.com)中为 Azure Web 应用设置环境变量的示例，请参阅[使用 Azure 表服务的 Node.js Web 应用]。
...
> [!NOTE]
> 可匿名访问 Blob，只需使用 **createBlobServiceAnonymous** 并提供主机地址即可。 例如，使用 `var blobSvc = azure.createBlobServiceAnonymous('https://myblob.blob.core.windows.net/');`。
>
>
...
如果该容器是新建的，则 `result.created` 为 true。 如果该容器已存在，则 `result.created` 为 false。 `response` 包含有关操作的信息，其中包括容器的 ETag 信息。

### <a name="container-security"></a>容器安全性
默认情况下，新容器是私有的，不能匿名访问。 若要使容器公开，以便能对其进行匿名访问，可将容器的访问级别设置为“Blob”或“容器”。
...
下面的代码示例会将 **test.txt** 文件的内容上传到 **myblob** 中。
...
若要列出容器中的 Blob，请使用 **listBlobsSegmented** 方法。 如果要返回带特定前缀的 Blob，请使用 **listBlobsSegmentedWithPrefix**。
...
新的租约可使用 **acquireLease** 方法获取，只需指定要获取其租约的 Blob 或容器即可。 例如，以下代码将获取 **myblob** 的租约。
...
[使用 Web Matrix 构建 Node.js Web 应用并将其部署到 Azure]: ../app-service-web/web-sites-nodejs-use-webmatrix.md
