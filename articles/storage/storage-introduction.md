...
Azure 高级存储现在发布了预览版。Azure 高级存储提供高性能、低延迟的磁盘支持，适合在 Azure 虚拟机上运行的 I/O 密集型工作负载。有了 Azure 高级存储，你就可以将多个持久性数据磁盘附加到虚拟机，并根据性能要求对其进行配置。每个数据磁盘在 Azure 高级存储中都有一个后备 SSD 磁盘，以确保最高的 I/O 性能。有关详细信息，请参阅[高级存储：适用于 Azure 虚拟机工作负荷的高性能存储](../storage-premium-storage-preview-portal)。
...
- 高级存储帐户当前仅支持 Azure 虚拟机磁盘。Azure 高级存储可通过 [Azure 预览版页](/services/preview/)请求提供。
...
Azure 存储空间通过一个 [REST API](http://msdn.microsoft.com/zh-cn/library/azure/dd179355.aspx) 来公开存储资源，任何可以发出 HTTP/HTTPS 请求的语言都可以调用该 REST API。另外，Azure 存储空间还为多种主流语言提供了编程库。这些库通过对细节进行处理简化了使用 Azure 存储空间的许多方面，这些细节包括同步和异步调用、操作的批处理、异常管理、自动重试、操作行为，等等。这些库当前可供下列语言和平台以及正在筹备的其他语言和平台使用：

- [.NET](http://go.microsoft.com/fwlink/?LinkID=390731)
- [本机代码](http://msdn.microsoft.com/zh-cn/library/azure/dn495438.aspx)
...
- [Azure 存储服务 REST API 参考](http://msdn.microsoft.com/zh-cn/library/azure/dd179355.aspx)
- [AzCopy 命令行工具参考](/documentation/articles/storage-use-azcopy)
