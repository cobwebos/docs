...
3.	在“新建”下面：选择“数据 + 存储”。在此处单击“存储帐户”。选择部署模型。为新部署使用 Resource Manager。单击“创建”。

4.	在“存储帐户”边栏中，键入存储帐户名称。选择所需的部署位置。

5.	单击“类型”。在“选择存储帐户类型”边栏选项卡中，选择“高级本地冗余”。单击“选择”。在单击“选择”后，“类型”将显示为“Premium-LRS”。

6.	在“存储帐户”边栏选项卡上创建新的“资源组”，或选择现有的资源组（如果已有资源组）。单击“创建”。

	![定价层][Image1]

#### II.通过 Azure 门户创建 Azure 虚拟机

只有创建了 DS 或 GS 系列 VM 才能使用高级存储。请遵照 [Create a Windows virtual machine in the Azure portal（在 Azure 门户中创建 Windows 虚拟机）](/documentation/articles/virtual-machines-windows-tutorial)中的步骤创建新的 DS 或 GS 虚拟机。

#### III.通过 Azure 门户附加高级存储数据磁盘

1. 在 Azure 门户中找到新的或现有的 DS 或 GS VM。
2. 在 VM 的“所有设置”中，转到“磁盘”，然后单击“附加新磁盘”。
3. 输入数据磁盘的名称，然后在“类型”中选择“高级”。选择所需的“大小”和“主机缓存”设置。

	![高级磁盘][Image2]

请参阅 [How to attach a data disk in Azure Portal（如何在 Azure 门户中附加数据磁盘）](/documentation/articles/virtual-machines-attach-disk-preview)中的详细步骤。
