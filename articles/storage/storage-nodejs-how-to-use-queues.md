...
创建一个空的 Node.js 应用程序。 有关创建 Node.js 应用程序的说明，请参阅[在 Azure App Service 中创建 Node.js Web 应用]、[生成 Node.js 应用程序并将其部署到 Azure 云服务]或[使用 Web Matrix 构建 Node.js Web 应用并将其部署到 Azure]。
...
> [!NOTE]
> 默认情况下，一条消息只会隐藏 30 秒，然后其他客户端就可以看见它。 可将 `options.visibilityTimeout` 与 **getMessages** 配合使用，以指定其他值。
> 
> [!NOTE]
