...
有关如何使用 **CloudTable** 对象的详细信息，请参阅 [Get started with Azure Table storage using .NET](storage-dotnet-how-to-use-tables.md)（通过 .NET 开始使用 Azure 表存储）。

## <a name="related-topics-covered-by-the-queues-how-to-article"></a>队列操作指南文章涵盖的相关主题
有关如何处理队列消息触发的表处理，或者不特定于表处理的 WebJobs SDK 方案的信息，请参阅 [Getting started with Azure Queue storage and Visual Studio connected services (WebJob Projects)](vs-storage-webjobs-getting-started-queues.md)（开始使用 Azure 队列存储和 Visual Studio 连接服务（Web 作业项目））。
