---
title: 为传入请求强制执行所需的最低版本的传输层安全性（TLS）
titleSuffix: Azure Storage
description: 为请求 Azure 存储的客户端配置存储帐户以要求最小版本的传输层安全性（TLS）。
services: storage
author: tamram
ms.service: storage
ms.topic: how-to
...
ms.locfileid: "87275771"
---
# <a name="enforce-a-minimum-required-version-of-transport-layer-security-tls-for-requests-to-a-storage-account"></a>为存储帐户请求强制执行最低要求的传输层安全性（TLS）版本

客户端应用程序和 Azure 存储帐户之间的通信使用传输层安全性（TLS）进行加密。 TLS 是一种标准的加密协议，通过 Internet 确保客户端和服务之间的隐私和数据完整性。 有关 TLS 的详细信息，请参阅[传输层安全性](https://en.wikipedia.org/wiki/Transport_Layer_Security)。
...
本文介绍如何使用拖动（检测-修正-管理）框架持续管理存储帐户的安全 TLS。

有关如何在从客户端应用程序发送请求时指定 TLS 的特定版本的信息，请参阅[配置客户端应用程序的传输层安全性（TLS）](transport-layer-security-configure-client-version.md)。
...
若要将请求记录到 Azure 存储帐户并确定客户端使用的 TLS 版本，可以在 Azure Monitor （预览版）中使用 Azure 存储日志记录。 有关详细信息，请参阅[监视 Azure 存储](monitor-storage.md)。
...
有关 Azure Monitor 中的 Azure 存储日志中可用的字段的参考，请参阅[资源日志（预览）](monitor-storage-reference.md#resource-logs-preview)。
...
1. 选择 "**模板部署（使用自定义模板部署"）**，选择 "**创建**"，然后选择 **"在编辑器中生成自己的模板"**。
...
若要测试存储帐户所需的最小 TLS 版本是否禁止使用较旧版本发出的调用，可以将客户端配置为使用较旧版本的 TLS。 有关将客户端配置为使用特定 TLS 版本的详细信息，请参阅[配置客户端应用程序的传输层安全性（TLS）](transport-layer-security-configure-client-version.md)。
...
下图显示了在以下情况下发生的错误：尝试使用设置为 TLS 1.0 的最小 TLS 版本（新帐户的默认值）创建存储帐户时，如果策略具有拒绝影响，则需要将最低 TLS 版本设置为 TLS 1.2。
