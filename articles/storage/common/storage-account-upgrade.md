...
从常规用途 v1 或 Blob 存储帐户升级到常规用途 v2 存储帐户非常简单。 可以使用 Azure 门户、PowerShell 或 Azure CLI 进行升级。 升级到常规用途 v2 存储帐户时，不会造成数据丢失或数据丢失的风险。 帐户升级通过简单的 Azure 资源管理器操作进行，此操作可更改帐户类型。
