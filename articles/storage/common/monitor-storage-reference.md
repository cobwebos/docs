...
description: 有关 Azure 存储中的监视数据的日志和指标参考。
author: normesta
...
ms.locfileid: "87077853"
---
# <a name="azure-storage-monitoring-data-reference"></a>Azure 存储监视数据参考

有关收集和分析 Azure 存储监视数据的详细信息，请参阅[监视 Azure 存储](monitor-storage.md)。
...
| FileCount   | 存储帐户中的文件数目。 <br/><br/> 单元：计数 <br/> 聚合类型：平均值 <br/> 值示例：1024 |
| FileShareCount | 存储帐户中的文件共享数目。 <br/><br/> 单位：Count <br/> 聚合类型：平均值 <br/> 值示例：1024 |
...
| 流出量 | 流出数据量。 此数字包括从外部客户端到 Azure 存储流出的数据量，以及流出 Azure 中的数据量。 因此，此数字不反映计费的流出量。 <br/><br/> 单位：字节 <br/> 聚合类型：总计 <br/> 适用维度：GeoType、ApiName 和 Authentication（[定义](#metrics-dimensions)） <br/> 值示例：1024 |
| SuccessServerLatency | Azure 存储处理成功请求所用的平均时间。 此值不包括 SuccessE2ELatency 中指定的网络延迟。 <br/><br/> 单位：毫秒 <br/> 聚合类型：平均值 <br/> 适用维度：GeoType、ApiName 和 Authentication（[定义](#metrics-dimensions)） <br/> 值示例：1024 |
| SuccessE2ELatency | 向存储服务或指定的 API 操作发出的成功请求的平均端到端延迟。 此值包括在 Azure 存储中读取请求、发送响应和接收响应确认所需的处理时间。 <br/><br/> 单位：毫秒 <br/> 聚合类型：平均值 <br/> 适用维度：GeoType、ApiName 和 Authentication（[定义](#metrics-dimensions)） <br/> 值示例：1024 |
| 可用性 | 存储服务或指定的 API 操作的可用性百分比。 可用性通过由“计费请求总数”值除以适用的请求数（包括引发意外错误的请求）计算得出。 所有意外错误都会导致存储服务或指定的 API 操作的可用性下降。 <br/><br/> 单位：百分比 <br/> 聚合类型：平均值 <br/> 适用维度：GeoType、ApiName 和 Authentication（[定义](#metrics-dimensions)） <br/> 值示例：99.99 |
...
| **BlobTier** | Azure 存储提供了不同的访问层，允许以最具成本效益的方式存储 Blob 对象数据。 请参阅 [Azure 存储 Blob 层](../blobs/storage-blob-storage-tiers.md)中的详细信息。 支持的值包括： <br/> <li>**Hot**：热层</li> <li>**Cool**：冷层</li> <li>**存档**：存档层</li> <li>**Premium**：块 Blob 的高级层</li> <li>**P4/P6/P10/P15/P20/P30/P40/P50/P60**：高级页 Blob 的层类型</li> <li>**标准**：标准页 Blob 的层类型</li> <li>**Untiered**：常规用途 v1 存储帐户的层类型</li> |
| **GeoType** | 来自主要或辅助群集的事务。 可用值包括 **Primary** 和 **Secondary**。 从辅助租户读取对象时，该维度会应用到读取访问异地冗余存储 (RA-GRS)。 |
| **ResponseType** | 事务响应类型。 可用的值包括： <br/><br/> <li>**ServerOtherError**：除描述的错误以外的其他所有服务器端错误 </li> <li>**ServerBusyError**：返回了 HTTP 503 状态代码的已经过身份验证的请求。 </li> <li>**ServerTimeoutError**：返回了 HTTP 500 状态代码的超时已经过身份验证的请求。 由于服务器错误而发生超时。 </li> <li>**AuthorizationError**：由于未经授权访问数据或者授权失败，经过身份验证的请求失败。 </li> <li>**NetworkError**：由于网络错误，经过身份验证的请求失败。 往往发生于客户端在超时失效之前提前关闭了连接时。 </li><li>**ClientAccountBandwidthThrottlingError**：请求由于超过了[存储帐户可伸缩性限制](scalability-targets-standard-account.md)而受到带宽限制。</li><li>**ClientAccountRequestThrottlingError**：请求由于超过了[存储帐户可伸缩性限制](scalability-targets-standard-account.md)而受到请求速率限制。<li>**ClientThrottlingError**：其他客户端限制错误。 不包括 ClientAccountBandwidthThrottlingError 和 ClientAccountRequestThrottlingError。</li> <li>**ClientTimeoutError**：返回了 HTTP 500 状态代码的超时已经过身份验证的请求。 如果将客户端的网络超时或请求超时设置为比存储服务预期值更小的值，则预期会发生此超时。 否则，会报告为 ServerTimeoutError。 </li> <li>**ClientOtherError**：除描述的错误以外的其他所有客户端错误。 </li> <li>**成功**：请求成功</li> <li> **SuccessWithThrottling**：在第一次（前几次）尝试时 SMB 客户端受到限制，但重试后成功的成功请求。</li> |
...
下表列出了在 Azure Monitor 日志或 Azure 存储中收集的 Azure 存储资源日志的属性。 这些属性描述了操作、服务，以及用于执行操作的授权类型。
...
|**category** | 所请求操作的类别。 例如：`StorageRead`、`StorageWrite` 或 `StorageDelete`。|
|**operationName** | 执行的 REST 操作类型。 <br> 有关完整的操作列表，请参阅[存储分析记录的操作和状态消息](https://docs.microsoft.com/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages)主题。 |
|**operationVersion** | 发出请求时指定的存储服务版本。 这等效于 **x-ms-version** 标头的值。 例如： `2017-04-17`。|
|**schemaVersion** | 日志的架构版本。 例如： `1.0`。|
|**statusCode** | 请求的 HTTP 状态代码。 如果请求被中断，那么此值可能设置为 `Unknown`。 <br> 例如：`206` |
|**statusText** | 所请求操作的状态。  有关完整的状态消息列表，请参阅[存储分析记录的操作和状态消息](https://docs.microsoft.com/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages)主题。 在 2017-04-17 版和更高版本中，未使用状态消息 `ClientOtherError`， 而是在此字段中包含一个错误代码。 例如：`SASSuccess`  |
|**durationMs** | 执行所请求操作花费的总时间，以毫秒表示。 这包括读取传入请求和向请求者发送响应所花费的时间。 例如：`12`。|
...
| properties | 说明 |
|:--- |:---|
|**identity / type** | 用于发出请求的身份验证类型。 例如： `OAuth`、`SAS Key`、`Account Key` 或 `Anonymous` |
|**identity / tokenHash**|这是仅供内部使用的保留字段。 |
...
|**requester / objectId** | 请求者的 OAuth 对象 ID。 对于 Kerberos 身份验证，此属性表示 Kerberos 验证的用户的对象标识符。 例如： `0e0bf547-55e5-465c-91b7-2873712b249c`。 |
...
|**requester / userName** | 这是仅供内部使用的保留字段。|
...
|**userAgentHeader** | **User-Agent 标头**值，括在引号中。 例如： `WA-Storage/6.2.0 (.NET CLR 4.0.30319.42000; Win32NT 6.2.9200.0)`。|
...
|**etag** | 所返回对象的 ETag 标识符，括在引号中。 例如： `0x8D101F7E4B662C4`。  |
|**serverLatencyMs** | 执行所请求操作花费的总时间，以毫秒表示。 此值不包括网络延迟（读取传入请求和向请求者发送响应所花费的时间）。 例如： `22`。 |
|**serviceType** | 与此请求关联的服务。 例如：`blob`、`table`、`files` 或 `queue`。 |
|**operationCount** | 请求中涉及的每个已记录操作的数目。 此计数从 `0` 索引处开始。 某些请求（例如复制 Blob 的请求）需要多个操作。 大多数请求仅执行一个操作。 例如： `1`。 |
|**requestHeaderSize** | 请求标头的大小，以字节表示。 例如： `578`。 <br>如果请求不成功，此值可能为空。 |
|**requestBodySize** | 存储服务读取的请求数据包的大小，以字节表示。 <br> 例如： `0`。 <br>如果请求不成功，此值可能为空。  |
|**responseHeaderSize** | 响应标头的大小，以字节表示。 例如： `216`。 <br>如果请求不成功，此值可能为空。  |
|**responseBodySize** | 由存储服务写入的响应数据包的大小，以字节为单位。 如果请求不成功，此值可能为空。 例如： `216`。  |
...
|**serverMd5** | 由存储服务计算的 MD5 哈希值。 例如： `3228b3cf1069a5489b298446321f8521`。 <br>此字段可为空。  |
|**lastModifiedTime** | 所返回对象的上次修改时间 (LMT)。  例如： `Tuesday, 09-Aug-11 21:13:26 GMT`。 <br>对于可返回多个对象的操作，此字段为空。 |
|**conditionsUsed** | 表示条件的键值对（以分号分隔）列表。 条件可以是下列任何一项： <li> If-Modified-Since <li> If-Unmodified-Since <li> If-Match <li> If-None-Match  <br> 例如： `If-Modified-Since=Friday, 05-Aug-11 19:11:54 GMT`。 |
|**contentLengthHeader** | 发送到存储服务的请求的 Content-Length 标头值。 如果请求成功，则此值等于 requestBodySize。 如果请求不成功，则此值可能不等于 requestBodySize，或者可能为空。 |
|**tlsVersion** | 在请求连接中使用的 TLS 版本。 例如：`TLS 1.2`。 |
|**smbTreeConnectID** | 在连接树时建立的服务器消息块 (SMB) **treeConnectId**。 例如：`0x3` |
|**smbPersistentHandleID** | 在重新连接网络后仍可正常发挥作用的 SMB2 CREATE 请求中的持久性句柄 ID。  在 [MS-SMB2](https://docs.microsoft.com/openspecs/windows_protocols/ms-smb2/f1d9b40d-e335-45fc-9d0b-199a31ede4c3) 2.2.14.1 中引用为 **SMB2_FILEID.Persistent**。 例如：`0x6003f` |
|**smbVolatileHandleID** | 在重新连接网络时回收的 SMB2 CREATE 请求中的易失性句柄 ID。  在 [MS-SMB2](https://docs.microsoft.com/openspecs/windows_protocols/ms-smb2/f1d9b40d-e335-45fc-9d0b-199a31ede4c3) 2.2.14.1 中引用为 **SMB2_FILEID.Volatile**。 例如：`0xFFFFFFFF00000065` |
|**smbMessageID** | 连接的相对 **MessageId**。 例如： `0x3b165` |
|**smbCreditsConsumed** | 请求使用的流入量或流出量，以 64k 为单位。 例如：`0x3` |
|**smbCommandDetail** | 有关此特定请求（而不是常规类型的请求）的详细信息。 例如：`0x2000 bytes at offset 0xf2000` |
|**smbFileId** | 与文件或目录关联的 **FileId**。  大致类似于 NTFS FileId。 例如：`0x9223442405598953` |
|**smbSessionID** | 设置会话时建立的 SMB2 **SessionId**。 例如：`0x8530280128000049` |
|**smbCommandMajor uint32** | **SMB2_HEADER.Command** 中的值。 目前，这是一个 0 到 18 之间（包括 0 和 18）的数字。 例如：`0x6` |
|**smbCommandMinor** | **SmbCommandMajor** 的子类（如果适用）。 例如： `DirectoryCloseAndDelete` |

## <a name="see-also"></a>另请参阅

- 有关如何监视 Azure 存储，请参阅[监视 Azure 存储](monitor-storage.md)。
