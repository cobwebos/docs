...
* [地理区域冗余存储 （GZRS） （预览）](storage-redundancy.md)为需要高可用性和最大持久性的方案提供复制。 使用区域冗余存储 （ZRS） 在主区域的三个 Azure 可用性区域中同步复制数据，然后异步复制到辅助区域。 对于对辅助区域中数据的读取访问，启用读取访问地理区域冗余存储 （RA-GZRS）。
...
本文介绍如何设计应用程序以应对主要区域中发生的服务中断。 如果主要区域不可用，应用程序可以调整为对次要区域执行读取操作。 在开始之前，请确保您的存储帐户已配置为 RA-GRS 或 RA-GZRS。
...
在为 RA-GRS 或 RA-GZRS 设计应用程序时，请记住以下要点：
...
> [!NOTE]
> 客户托管的帐户故障转移（预览）在支持 GZRS/RA-GZRS 的区域尚不可用，因此客户当前无法使用 GZRS 和 RA-GZRS 帐户管理帐户故障转移事件。 在预览期间，Microsoft 将管理任何影响 GZRS/RA-GZRS 帐户的故障转移事件。
...
要了解如何检查上次同步时间，请参阅[检查存储帐户的最后同步时间属性](last-sync-time-get.md)。
...
* 有关演示如何在主终结点和辅助终结点之间来回切换的完整示例，请参阅 Azure 示例 —[使用具有 RA-GRS 存储的断路器模式](https://github.com/Azure-Samples/storage-dotnet-circuit-breaker-pattern-ha-apps-using-ra-grs)。
