...
description: 了解如何使用读取访问异地冗余存储来构建高度可用的应用程序，该应用程序的灵活性足以处理中断。
services: storage
author: tamram
ms.service: storage
ms.topic: conceptual
...
例如，假设客户提交更新成功，但在更新传播到次要区域前主要区域发生故障。 当客户请求读取数据时，他们将从次要区域（而不是更新的数据）接收过时数据。 在设计应用程序时，必须确定这是否可接受，如果可接受，如何告知客户。 
...
下表显示了在更新员工的详细信息以使其成为*管理员*角色成员时可能发生的情况的示例。 此示例要求更新**员工**条目实体和**管理员角色**实体的管理员总数。 请注意更新如何以无序方式在次要区域中应用。
