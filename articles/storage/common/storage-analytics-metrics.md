...
> 存储分析指标目前为经典指标。 Microsoft 建议[在 Azure 监视器中使用存储指标](storage-metrics-in-azure-monitor.md)，而不是存储分析指标。
...
|每小时度量值，主位置|-   $MetricsHourPrimaryTransactionsBlob<br />-   $MetricsHourPrimaryTransactionsTable<br />-   $MetricsHourPrimaryTransactionsQueue<br />-   $MetricsHourPrimaryTransactionsFile|所有版本。 只有 2015-04-05 和更高版本才支持文件服务指标。|  
|分钟度量值，主位置|-   $MetricsMinutePrimaryTransactionsBlob<br />-   $MetricsMinutePrimaryTransactionsTable<br />-   $MetricsMinutePrimaryTransactionsQueue<br />-   $MetricsMinutePrimaryTransactionsFile|所有版本。 只有 2015-04-05 和更高版本才支持文件服务指标。|  
...
按照以下步骤在[Azure 门户](https://portal.azure.com)中启用指标：
...
您可以使用本地计算机上的 PowerShell 配置存储帐户中的存储指标，使用 Azure PowerShell cmdlet **Get-AzStorageServiceMetrics 属性**来检索当前设置，以及 cmdlet **Set-AzStorageServiceMetrics 属性**来更改当前设置。  
...
* **MetricsLevel**，可能的值为：
* **无**：关闭监视。
* **服务**：收集指标，如入口/出口、可用性、延迟和成功百分比，这些指标被聚合为 Blob、队列、表和文件服务。
* **ServiceAndApi**： 除了服务指标之外，为 Azure 存储服务 API 中的每个存储操作收集相同的指标集。
...
**CloudBlobClient**、**CloudQueueClient**、**CloudTableClient** 和 **CloudFileClient** 类都包含 **SetServiceProperties** 和 **SetServicePropertiesAsync** 等方法，这些方法采用 **ServiceProperties** 对象作为参数。 你可以使用 **ServiceProperties** 对象配置存储指标。 例如，以下 C# 代码片段演示如何更改小时队列指标的指标级别和保留天数：  
...
在将存储分析度量值配置为监视存储帐户后，存储分析将使用存储帐户在一组已知表中记录度量值。 您可以将图表配置为在[Azure 门户](https://portal.azure.com)中查看每小时指标：
...
||||||||||||  
|-|-|-|-|-|-|-|-|-|-|-|  
|**PartitionKey**|**RowKey**|**时间 戳**|**TotalRequests**|**TotalBillableRequests**|**TotalIngress**|**TotalEgress**|**可用性**|**AverageE2ELatency**|**AverageServerLatency**|**PercentSuccess**|  
