...
删除虚拟目录中部分的 blob （例如：仅删除 jpg 和 pdf 文件，或者 blob 名称为 `exactName` ）：
...
从具有分层命名空间（包含/排除不支持）的 Blob 存储帐户中删除单个目录：
...
**--日志级别**字符串定义日志文件的日志详细级别。 可用级别包括： `INFO` （所有请求/响应）、（ `WARNING` 响应缓慢）、 `ERROR` （仅失败的请求）和 `NONE` （无输出日志）。 （默认值 `INFO` ）（默认值 `INFO` ）
