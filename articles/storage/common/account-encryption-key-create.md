...
若要验证新存储帐户是否正在使用帐户加密密钥，请调用 Azure CLI [az storage account](/cli/azure/storage/account#az-storage-account-show)命令。 此命令返回存储帐户属性及其值的列表。 查找 `keyType` 属性，并验证其是否设置为 `Account`。
