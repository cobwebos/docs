...
> [!NOTE]
> 默认情况下，AzCopy 将数据上载为块 blob。 要将文件上载为"追加 Blob"或"页面 Blob"，请使用 标志`--blob-type=[BlockBlob|PageBlob|AppendBlob]`。
> 默认情况下，AzCopy 会上传您的数据以继承帐户访问层。 要将文件上载到特定的[访问层](../blobs/storage-blob-storage-tiers.md)，请使用 标志`--block-blob-tier=[Hot|Cool|Archive]`。
...
> [!NOTE]
> 当前版本的 AzCopy 不会在其他源和目标之间同步（例如：文件存储或 Amazon Web 服务 （AWS） S3 存储桶）。
