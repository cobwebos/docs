---
title: 使用 Azure 门户来管理对容器和队列使用 RBAC 的 Azure 存储的 Azure AD 访问权限 |Microsoft Docs
...
ms.locfileid: "58368232"
---
# <a name="grant-access-to-azure-containers-and-queues-with-rbac-in-the-azure-portal"></a>授予对 Azure 容器和 Azure 门户中使用 RBAC 的队列的访问权限
...
1. 分配相应的 Azure 存储 RBAC 角色，授予对容器或队列访问权限。 读取访问权限，将分配**Blob 数据读取器**或**队列数据读取器**角色。 对于读取、 写入和删除访问权限，将分配**Blob 数据参与者**或**队列数据参与者**角色。 此外可以分配自定义角色。
