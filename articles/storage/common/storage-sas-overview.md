...
- **用户委托 SAS。** 用户委托 SAS 使用 Azure Active Directory (Azure AD) 凭据以及为 SAS 指定的权限进行保护。 用户委托 SAS 仅适用于 Blob 存储。
    ...
    有关用户委托 SAS 的详细信息，请参阅 [创建用户委派 sas (REST API) ](/rest/api/storageservices/create-user-delegation-sas)。
...
> [!NOTE]
> Microsoft 建议尽可能使用 Azure AD 凭据作为最佳安全方案，而不是使用帐户密钥，这样可以更容易地受到威胁。 当应用程序设计要求使用共享访问签名来访问 Blob 存储时，请使用 Azure AD 凭据创建用户委托 SAS （如果可能）以实现高级安全性。 有关详细信息，请参阅 [使用 Azure Active Directory 授予对 blob 和队列的访问权限](storage-auth-aad.md)。

共享访问签名可以采取以下两种形式的一种：

- **即席 SAS：** 创建即席 SAS 时，如果) 中省略开始时间，则会在 SAS (URI 中指定该 SAS 的开始时间、到期时间和权限。 任何类型的 SAS 都可以用作临时 SAS。
...
可以通过以下两种方式之一对 SAS 令牌进行签名：

- 使用 Azure Active Directory (创建的 *用户委托密钥* Azure AD) 凭据。 用户委托 SAS 使用用户委托密钥进行签名。

    若要获取用户委托密钥并创建 SAS，必须为 Azure AD 安全主体分配一个包含 storageAccounts **//blobServices/generateUserDelegationKey** 操作的 Azure 角色。 有关具有获取用户委派密钥权限的 Azure 角色的详细信息，请参阅 [创建用户委派 SAS (REST API) ](/rest/api/storageservices/create-user-delegation-sas)。

- 通过存储帐户密钥)  (共享密钥。 服务 SAS 和帐户 SAS 都是使用存储帐户密钥签名的。 若要创建使用帐户密钥签名的 SAS，应用程序必须有权访问该帐户密钥。

当请求包含 SAS 令牌时，将根据该 SAS 令牌的签名方式对该请求进行授权。 Azure 存储还使用用于创建 SAS 令牌的访问密钥或凭据来授予对拥有 SAS 的客户端的访问权限。

下表总结了每种类型的 SAS 令牌在对 Azure 存储的请求中包括时的授权：

| SAS 类型 | 授权类型 |
|-|-|
| 仅 (Blob 存储的用户委托 SAS)  | Azure AD |
| 服务 SAS | 共享密钥 |
| 帐户 SAS | 共享密钥 |
...
- **要注意 SAS 开始时间。** 如果将 SAS 的开始时间设置为当前时间，则可能会在前几分钟间歇发生故障，因为不同的计算机的当前时间 (称为时钟倾斜) 。 通常，将开始时间至少设置为 15 分钟前。 或者根本不设置，这会使它在所有情况下都立即生效。 同样此原则也适用于到期时间 - 请记住，对于任何请求，在任一方向你可能会观察到最多 15 分钟的时钟偏移。 对于使用 2012-02-12 之前的 REST 版本的客户端，未参照某一存储访问策略的 SAS 的最大持续时间是 1 小时，指定超过 1 小时持续时间的任何策略都会失败。
- **请注意 SAS 日期时间格式。** 如果为 SAS 设置开始时间和/或过期时间，则对于某些实用程序 (例如，命令行实用程序 AzCopy) 需要日期时间格式为 "+% Y-% m-% dT% H:%M：% SZ"，特别包括使用 SAS 令牌进行工作所需的秒数。  
