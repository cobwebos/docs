...
- 要注意 SAS 开始时间。  如果将 SAS 的开始时间设置为当前时间，则可能会在前几分钟间歇发生故障，因为不同的计算机的当前时间略有不同（称为时钟偏差）。 通常，将开始时间至少设置为 15 分钟前。 或者根本不设置，这会使它在所有情况下都立即生效。 同样原则也适用于到期时间 - 请记住，对于任何请求，在任一方向可能会观察到最多 15 分钟的时钟偏移。 对于使用 2012-02-12 之前的 REST 版本的客户端，未参照某一存储访问策略的 SAS 的最大持续时间是 1 小时，指定超过 1 小时持续时间的任何策略都会失败。
- **请注意 SAS 日期/时间格式。** 如果为 SAS 设置了开始时间和/或到期时间，则对于某些实用程序（例如，命令行实用程序 AzCopy），需要将日期/时间格式设置为“+%Y-%m-%dT%H:%M:%SZ”，特别是包括秒数，以便它使用 SAS 令牌工作。  
