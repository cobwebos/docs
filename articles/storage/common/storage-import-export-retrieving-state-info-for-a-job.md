...
|`Transferring`|服务开始将数据从驱动器传输到 Windows Azure 存储时，驱动器将转换为 `Transferring` 状态。|
...
当作业或驱动器未能正常完成其预期生命周期时，作业或驱动器将转换为 `Faulted` 状态。 此时，运营团队通过电子邮件或电话联系客户。 解决问题后，出错的作业或驱动器将退出 `Faulted` 状态并转换为适当的状态。
