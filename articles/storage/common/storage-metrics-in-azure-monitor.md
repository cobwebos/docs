...
Azure Monitor 提供多种访问指标的方法。 你可以从[Azure 门户](https://portal.azure.com)、Azure Monitor API （REST 和 .net）和分析解决方案（例如事件中心）访问它们。 有关详细信息，请参阅 [Azure Monitor 指标](../../monitoring-and-diagnostics/monitoring-overview-metrics.md)。
...
Azure Monitor 提供[.NET SDK](https://www.nuget.org/packages/Microsoft.Azure.Management.Monitor/)来读取指标定义和值。 [示例代码](https://azure.microsoft.com/resources/samples/monitor-dotnet-metrics-api/)演示如何通过不同的参数来使用 SDK。 对于存储指标，需使用 `0.18.0-preview` 或更高版本。 资源 ID 在 .NET SDK 中使用。 有关详细信息，请阅读“了解存储中服务的资源 ID”。

下面的示例演示如何使用 Azure Monitor .NET SDK 来读取存储指标。

#### <a name="list-account-level-metric-definition-with-the-net-sdk"></a>通过 .NET SDK 列出帐户级别指标定义
...
#### <a name="read-metric-values-with-the-net-sdk"></a>通过 .NET SDK 读取指标值
...
#### <a name="read-multi-dimensional-metric-values-with-the-net-sdk"></a>通过 .NET SDK 读取多维指标值
...
| 入口 | 流入数据量。 此数字包括从外部客户端到 Azure 存储流入的数据量，以及流入 Azure 中的数据量。 <br/><br/> 单元：字节 <br/> 聚合类型：总计 <br/> 适用维度：GeoType、ApiName 和 Authentication（[定义](#metrics-dimensions)） <br/> 值示例：1024 |
