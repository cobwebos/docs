...
Azure Monitor 提供多种访问指标的方法。 可以从[Azure 门户](https://portal.azure.com)、Azure 监视器 API（REST 和 .NET）和分析解决方案（如事件中心）访问它们。 有关详细信息，请参阅 [Azure Monitor 指标](../../monitoring-and-diagnostics/monitoring-overview-metrics.md)。
...
Azure 监视器提供[.NET SDK](https://www.nuget.org/packages/Microsoft.Azure.Management.Monitor/)来读取指标定义和值。 [示例代码](https://azure.microsoft.com/resources/samples/monitor-dotnet-metrics-api/)演示如何通过不同的参数来使用 SDK。 对于存储指标，需使用 `0.18.0-preview` 或更高版本。 资源 ID 用在 .NET SDK 中。 有关详细信息，请阅读“了解存储中服务的资源 ID”。
...
| 标准名称 | 描述 |
| ------------------- | ----------------- |
| BlobCapacity | 存储帐户中使用的 Blob 存储总计。 <br/><br/> 单元：字节 <br/> 聚合类型：平均 <br/> 值示例：1024 <br/> 尺寸 **：Blob 类型**和**BlobTier** （[定义](#metrics-dimensions)） |
| BlobCount    | 在存储帐户中存储的 Blob 对象数。 <br/><br/> 单位：计数 <br/> 聚合类型：平均 <br/> 值示例：1024 <br/> 尺寸 **：Blob 类型**和**BlobTier** （[定义](#metrics-dimensions)） |
...
| 流出量 | 流出数据量。 此数字包括从 Azure 存储向外部客户端的出口以及 Azure 中的出口。 因此，此数字不反映计费的流出量。 <br/><br/> 单元：字节 <br/> 聚合类型：总计 <br/> 适用维度：GeoType、ApiName 和 Authentication（[定义](#metrics-dimensions)） <br/> 值示例：1024 |
...
| 维度名称 | 描述 |
| ------------------- | ----------------- |
| **Blob 类型** | 仅限 Blob 指标的 Blob 类型。 支持的值为 **BlockBlob**、**PageBlob** 和 **Azure Data Lake Storage**。 BlockBlob 中包含追加 Blob。 |
| **BlobTier** | Azure 存储提供了不同的访问层，允许以最具成本效益的方式存储 Blob 对象数据。 请在 [Azure 存储 Blob 层](../blobs/storage-blob-storage-tiers.md)中查看详细信息。 支持的值包括： <br/> <li>**热门**： 热层</li> <li>**酷**： 酷层</li> <li>**存档**： 存档层</li> <li>**高级**：块 Blob 的高级层</li> <li>**P4/P6/P10/P15/P20/P30/P40/P50/P60**：高级页面 blob 的层类型</li> <li>**标准**：标准页面 Blob 的层类型</li> <li>**未分层**：通用 v1 存储帐户的层类型</li> |
| **GeoType** | 来自主要或辅助群集的事务。 可用值包括**主**值和**辅助**值 。 从辅助租户读取对象时，该维度会应用到读取访问异地冗余存储 (RA-GRS)。 |
| **响应类型** | 事务响应类型。 可用的值包括： <br/><br/> <li>**服务器其他错误**：除所述错误外，所有其他服务器端错误 </li> <li>**服务器忙错误**：已验证的请求，返回 HTTP 503 状态代码。 </li> <li>**服务器超时错误**：已超时身份验证的请求，该请求返回了 HTTP 500 状态代码。 由于服务器错误而发生超时。 </li> <li>**授权错误**：由于未经授权访问数据或授权失败而失败的已身份验证请求。 </li> <li>**网络错误**：由于网络错误而失败的已身份验证请求。 往往发生于客户端在超时失效之前提前关闭了连接时。 </li>  <li>**客户端帐户带宽限制错误**： 请求被限制的带宽超过[存储帐户可伸缩性限制](scalability-targets-standard-account.md).</li><li>**客户端帐户请求限制错误**： 请求被限制的请求速率超过[存储帐户可伸缩性限制](scalability-targets-standard-account.md).<li>**客户端限制错误**：其他客户端限制错误。 排除客户端帐户带宽限制错误和客户端帐户请求限制错误。</li> <li>**客户端超时错误**：已超时身份验证的请求，该请求返回了 HTTP 500 状态代码。 如果将客户端的网络超时或请求超时设置为比存储服务预期值更小的值，则预期会发生此超时。 否则，会报告为 ServerTimeoutError。</li> </li> <li>**客户端其他错误**：除所述错误外，所有其他客户端错误。 </li> <li>**成功**：成功请求</li> <li> **成功与限制**： 成功请求时，SMB 客户端在第一次尝试中受到限制，但在重试后成功。</li> |
| **阿皮名称** | 操作的名称。 例如： <br/> <li>**创建容器**</li> <li>**DeleteBlob**</li> <li>**获取Blob**</li> 有关所有操作名称，请参阅[文档](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages)。 |
| **身份验证** | 事务中所用的身份验证类型。 可用的值包括： <br/> <li>**帐户密钥**：使用存储帐户密钥对事务进行身份验证。</li> <li>**SAS**： 事务使用共享访问签名进行身份验证。</li> <li>**OAuth**：事务使用 OAuth 访问令牌进行身份验证。</li> <li>**匿名**：交易是匿名请求的。 它不包括预检请求。</li> <li>**匿名预检**：交易是预检请求。</li> |
