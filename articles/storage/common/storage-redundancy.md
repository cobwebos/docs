...
你的存储帐户中的所有数据都将都复制，包括块 blob 和追加 blob、 页 blob、 队列、 表和文件。 复制所有类型的存储帐户，尽管 ZRS 需要一个常规用途 v2 存储帐户。
