...
启用客户管理的密钥后，可以指定要与存储帐户关联的密钥。 还可以指示 Azure 存储应将客户托管的密钥自动更新到最新版本，还是手动更新密钥版本。

### <a name="specify-a-key-from-a-key-vault"></a>从 Key Vault 指定密钥

从密钥保管库选择客户管理的密钥时，将启用密钥版本的自动更新。 若要手动管理密钥版本，请改为指定密钥 URI，并包含密钥版本。 有关详细信息，请参阅将 [密钥指定为 URI](#specify-a-key-as-a-uri)。
...
指定密钥 URI 时，请从 URI 中省略密钥版本，以启用自动更新到最新密钥版本。 如果在密钥 URI 中包括密钥版本，则不会启用 "自动更新"，并且你必须自行管理密钥版本。 有关更新密钥版本的详细信息，请参阅 [手动更新密钥版本](#manually-update-the-key-version)。
...
指定密钥后，Azure 门户指示是否启用密钥版本的自动更新，并显示当前用于加密的密钥版本。

:::image type="content" source="media/storage-encryption-keys-portal/portal-auto-rotation-enabled.png" alt-text="显示启用密钥版本自动更新的屏幕截图":::

## <a name="manually-update-the-key-version"></a>手动更新密钥版本

默认情况下，在 Key Vault 中创建客户托管密钥的新版本时，Azure 存储会自动使用新版本进行加密，如前一部分中所述。 如果选择自行管理密钥版本，则每次创建密钥的新版本时，必须更新与存储帐户关联的密钥版本。

若要将存储帐户更新为使用新的密钥版本，请执行以下步骤：

1. 导航到你的存储帐户，并显示“加密”设置。
1. 输入新版本的密钥的 URI。 或者，可以再次选择 Key Vault 和密钥以更新版本。
1. 保存所做更改。
