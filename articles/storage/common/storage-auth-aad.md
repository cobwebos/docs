---
title: 授予对 Azure blob 和队列使用 Azure Active Directory 访问权限 |Microsoft Docs
description: 授予对 Azure blob 和队列使用 Azure Active Directory 访问权限。
services: storage
author: tamram
ms.service: storage
ms.topic: article
...
ms.locfileid: "67302421"
---
# <a name="authorize-access-to-azure-blobs-and-queues-using-azure-active-directory"></a>授予对 Azure blob 和队列使用 Azure Active Directory 访问权限

使用 Azure Active Directory (AD) 与存储 Blob 和队列存储的请求进行授权的 azure 存储支持。 与 Azure AD，可以使用基于角色的访问控制 (RBAC) 向安全主体授予权限可能是用户、 组或应用程序服务主体。 安全主体是由 Azure AD 返回 OAuth 2.0 令牌进行身份验证。 标记用于授权访问队列或 Blob 存储中的资源的请求。

用户或应用程序使用 OAuth 2.0 令牌由 Azure AD 返回授权通过共享密钥的授权提供卓越的安全性和易于使用并共享访问签名 (SAS)。 与 Azure AD，没有必要进行存储帐户访问密钥与你的代码和风险潜在的安全漏洞。 虽然可以继续为应用程序使用共享密钥授权，但是，使用 Azure AD 不需要将帐户访问密钥与代码存储在一起。 也可以继续使用共享访问签名 (SAS) 授予对存储帐户中的资源的精细访问权限，但 Azure AD 提供了类似的功能，并且不需要管理 SAS 令牌，也不需要担心吊销已泄露的 SAS。 Microsoft 建议使用 Azure AD 授权与 Azure 存储应用程序在可能的情况。

使用 Azure AD 的授权是可用于所有通用和 Blob 中的所有公共区域和国家/地区云的存储帐户。 仅通过 Azure 资源管理器部署模型创建的存储帐户支持 Azure AD 授权。
...
身份验证步骤需要应用程序请求在运行时的 OAuth 2.0 访问令牌。 如果应用程序正在运行从 Azure VM、 虚拟机规模集或 Azure Functions 应用等 Azure 实体中，它可以使用[托管标识](../../active-directory/managed-identities-azure-resources/overview.md)访问 blob 或队列。 若要了解如何对请求由托管标识对 Azure Blob 或队列服务进行授权，请参阅[Azure 资源的授权访问的 blob 和队列与 Azure Active Directory 和管理的标识](storage-auth-aad-msi.md)。
...
本机应用程序和对 Azure Blob 或队列服务发出请求的 web 应用程序也可以授权与 Azure AD 的访问权限。 若要了解如何请求访问令牌并使用它来授权对 blob 或队列数据的请求，请参阅[从 Azure 存储应用程序到 Azure 存储使用 Azure AD 授权访问](storage-auth-aad-app.md)。
...
## <a name="azure-ad-authorization-over-smb-for-azure-files"></a>SMB 上的 Azure 文件的 azure AD 授权

Azure 文件支持与 Azure AD 的授权基于 SMB 的已加入域的仅限 Vm （预览版）。 若要了解如何使用 Azure AD 通过 SMB 进行 Azure 文件，请参阅[SMB 上的 Azure 文件 （预览版） 的概述的 Azure Active Directory 授权](../files/storage-files-active-directory-overview.md)。

## <a name="next-steps"></a>后续步骤

- [Azure 资源的授权访问的 blob 和队列与 Azure Active Directory 和管理的标识](storage-auth-aad-msi.md)
