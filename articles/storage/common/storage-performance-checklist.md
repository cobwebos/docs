...
* 如果某个应用程序肯定会超出伸缩性目标之一，则应创建多个存储帐户并将应用程序数据跨多个这样的存储帐户进行分区。 如果使用这种模式，则在设计应用程序时，必须确保能够在以后添加更多的存储帐户，以便进行负载均衡。 撰写本文时, 每个 Azure 订阅最多可以有250个区域的存储帐户 (使用 Azure 资源管理器模型进行部署时)。  存储帐户除了用于数据存储、事务处理或数据传输之外，并无其他开销。
