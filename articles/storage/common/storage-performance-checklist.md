...
可以使用 **UpdateMessage** 来增加不可见性超时或更新消息的状态信息。 虽然此功能很强大，但请记住，每项“UpdateMessage”  操作都会计入可伸缩性目标。 不过，与作业每完成一步就将其从一个队列传到下一个队列的工作流相比，此方法可能要高效得多。 使用**UpdateMessage**操作使应用程序能够将作业状态保存到消息，然后再继续工作，而不是每次一步完成排队的消息的作业的下一步。  
