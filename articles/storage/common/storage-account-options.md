...
Azure 存储提供三个不同的帐户选项，支持不同的定价和功能。 用户在确定最适合应用程序的选项时，应考虑到这些差异。  三个不同的选项如下所示：

* **常规用途 v2 (GPv2)** 帐户提供所有最新功能，支持 Blob、文件、队列和表。 目前，这些最新功能包括 Blob 级别分层、存档存储、更大规模的帐户限制和存储事件。 设计定价的目的是为了提供最低的单 GB 价格和具有行业竞争优势的事务价格。

* **Blob 存储**帐户提供块 Blob 的所有最新功能，但只支持块 Blob。  定价大部分与常规用途 v2 帐户类似。 建议大多数用户使用常规用途 v2 帐户而非 Blob 存储帐户。

* **常规用途 v1 (GPv1)** 帐户允许使用所有 Azure 存储服务，但可能没有最新功能，其单 GB 定价也可能不是最低的。 例如，GPv1 中不支持冷存储和存档存储。  事务定价较低，因此改动率或读取率高的工作负荷适合此帐户类型。

### <a name="changing-account-kind"></a>更改帐户类型
用户随时可以通过门户、CLI 或 PowerShell 将 GPv1 帐户或 Blob 存储帐户升级为 GPv2 帐户。 此更改无法撤消，不允许其他更改。

## <a name="general-purpose-v2"></a>常规用途 v2
**常规用途 v2 (GPv2)** 存储帐户支持所有存储服务（包括 Blob、文件、队列和表）的所有功能。 对于块 Blob，可以根据访问模式在帐户级别选择热或冷存储层，或者在 Blob 级别选择热、冷或存档层。 可以将经常访问的、不常访问的和很少访问的数据分别存储在热、冷和存档存储层中，以便优化成本。 重要的是，可以在门户、CLI 或 PowerShell 中将任何 GPv1 帐户升级到 GPv2 帐户。 GPv2 帐户支持 Blob 存储帐户和 GPv1 帐户所支持的所有 API 和功能，并且也具有这些帐户类型所具有的良好持续性、可用性、可伸缩性和性能特点。

Blob 存储帐户在帐户级别公开“访问层”属性，可以将默认存储帐户层指定为“热”或“冷”。 默认存储帐户层可以应用到任何 Blob，前提是该 Blob 没有在 Blob 级别设置任何显式层。 如果数据的使用模式有所更改，也可以随时在这些存储层之间切换。 **存档层**仅适用于 Blob 级别。
...
**Blob 存储帐户**支持 GPv2 帐户所支持的所有块 Blob 功能，但其局限性是只支持块 Blob。 客户应先了解 Blob 存储帐户和 GPv2 帐户的定价差异，然后再考虑是否升级到 GPv2。 请注意，此升级不能撤消。

> [!NOTE]
> Blob 存储帐户仅支持块 Blob 和追加 Blob，不支持页 Blob。

## <a name="general-purpose-v1"></a>常规用途 v1
**常规用途 v1 (GPv1)** 是最早的存储帐户，也是能够在经典部署模型中使用的唯一帐户类型。 GPv1 不提供冷存储和存档存储之类的功能。 通常情况下，与 GPv2 或 Blob 存储帐户相比，GPv1 的单 GB 存储成本较高，但事务成本较低。

## <a name="recommendations"></a>建议

有关存储帐户的详细信息，请参阅 [关于 Azure 存储帐户](../common/storage-create-storage-account.md?toc=%2fazure%2fstorage%2fblobs%2ftoc.json) 。

对于仅需块 Blob 或追加 Blob 存储的应用程序，建议使用 GPv2 存储帐户，充分利用分层存储的差异化定价模型。 但是，我们知道这在某些需要使用 GPv1 存储帐户的情况下是不可能的，例如：

* 仍需使用经典部署模型。 Blob 存储帐户只能通过 Azure 资源管理器部署模型使用。
...
> [!NOTE]
> 有关 Blob 存储帐户定价模型的更多详细信息，请参阅 [Azure 存储定价](https://azure.microsoft.com/pricing/details/storage/)页。 有关出站数据传输收费的更多详细信息，请参阅[数据传输定价详细信息](https://azure.microsoft.com/pricing/details/data-transfers/)页。
...
4. 选择 **Resource Manager** 作为部署模型。
    ...
    分层存储只能用于 Resource Manager 存储帐户；建议对新资源使用该部署模型。 有关详细信息，请参阅 [Azure 资源管理器概述](../../azure-resource-manager/resource-group-overview.md)。  

5. 在“帐户类型”下拉列表中，选择“常规用途 v2”。

    可以在这里选择存储帐户的类型。 分层存储不适用于常规用途存储，只适用于 Blob 存储类型帐户。     

    选择此项时，性能层设置为“标准”。 分层存储不适用于“高级”性能层。
...
5. 此时会在右侧显示用于确认的新边栏选项卡。 在“确认升级”下键入帐户名称。 
...
> [!NOTE]
> Blob 存储帐户公开表服务终结点的目的只是为了存储和访问该帐户的指标数据。 GPv1 ZRS 存储帐户不支持指标数据。
...
> [!NOTE]
> 如果有一个常规用途存储帐户，在其中存储了页 Blob、虚拟机磁盘、队列、文件、表以及块 Blob 数据和追加 Blob 数据，则不适合使用此估算过程。 这是因为容量数据不区分块 Blob 与其他类型，因此不会给出其他数据类型的容量数据。 如果使用这些类型，则也可查看最新账单上的数量，作为一种替代方法。
...
若要估算 Blob 存储帐户的事务成本，需将事务细分成三组，因为这些事务价格不一样。
...
在使用 GRS 或 RA-GRS 存储帐户时，也可以通过所写入数据量的估算值来计算 Blob 存储帐户的异地复制数据传输成本。
...
可以轻松地将 GPv1 或 Blob 存储帐户升级为 GPv2，不会造成停机或 API 更改，也不需移动数据。 这是 GPv2 相对于Blob 存储帐户的主要优势之一。

但是，如果需要迁移到 Blob 存储帐户，可以按下面的说明操作。

Blob 存储帐户专用于存储块 Blob 和追加 Blob。 现有的常规用途存储帐户（用于存储表、队列、文件、磁盘、Blob）无法转换为 Blob 存储帐户。 若要使用存储层，需创建新的 Blob 存储帐户并将现有的数据迁移到新创建的帐户中。

可以使用以下方法，从本地存储设备、第三方云存储提供商或 Azure 中现有的常规用途存储帐户将现有数据迁移到 Blob 存储帐户：

### <a name="azcopy"></a>AzCopy

AzCopy 是一个 Windows 命令行实用程序，旨在实现高性能地将数据复制到 Azure 存储和从 Azure 存储中复制。 可以使用 AzCopy 从现有的常规用途存储帐户将数据复制到 Blob 存储帐户，或从本地存储设备将数据上传到 Blob 存储帐户。
...
可以创建自定义应用程序，使用一个 Azure 客户端库或 Azure 存储服务 REST API 将数据迁移到 Blob 存储帐户。 Azure 存储对多种语言和平台（如 .NET、Java、C++、Node.JS、PHP、Ruby 和 Python）提供了内容丰富的客户端库。 客户端库提供高级功能，如重试逻辑、日志记录和并行上传。 也可以直接针对可以由发出 HTTP/HTTPS 请求的任何语言调用的 REST API 进行开发。

有关更多详细信息，请参阅 [Azure Blob 存储入门](../blobs/storage-dotnet-how-to-use-blobs.md)。
...
是的。 可以在门户中轻松地将 GPv1 或 Blob 存储帐户升级为 GPv2。
...
**能否将页 Blob 和虚拟机磁盘存储在 Blob 存储帐户中？**

不能。 Blob 存储帐户仅支持块 Blob 和追加 Blob，不支持页 Blob。 Azure 虚拟机磁盘由页 Blob 支持，因此不能使用 Blob 存储帐户来存储虚拟机磁盘。 但是，可以在 Blob 存储帐户中将虚拟机磁盘的备份存储为块 Blob。 如果需要考虑使用 GPv2 来代替 Blob 存储帐户，则这是原因之一。
...
[Azure Blob 存储入门](../blobs/storage-dotnet-how-to-use-blobs.md)
