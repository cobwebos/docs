...
可以从 Azure 门户跟踪导入或导出作业的状态。 单击 "**导入/导出**" 选项卡。作业的列表将显示在该页上。
...
| 作业状态 | 描述 |
|:--- |:--- |
| Creating | 创建作业后，其状态将设置为 "**正在创建**"。 当作业处于 "**正在创建**" 状态时，导入/导出服务假设驱动器尚未寄送到数据中心。 作业可保持此状态最多两周，之后服务会自动将其删除。 |
...
| 非公开 | 在将所有驱动器寄回给你后，如果处理作业期间出现任何错误，作业状态会设置为“Closed”****。 作业会在保持“Closed”状态 90 天后自动删除****。 |
...
| Received | 导入/导出服务为导入作业处理从货运公司收到的驱动器后，驱动器将转换为“Received”状态****。 对于导出作业，初始驱动器状态为 "已**接收**" 状态。 |
...
| 驱动器状态 | 事件 | 解决方法/后续步骤 |
|:--- |:--- |:--- |
| NeverReceived | 标记为 " **NeverReceived** " 的驱动器 (因为作业的发货中未收到该驱动器) 会收到另一个发货。 | 运营团队将驱动器转换为“Received”状态****。 |
