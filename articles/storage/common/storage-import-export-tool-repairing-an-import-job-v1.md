...
|||  
|-|-|  
|**/r：**<RepairFile\>|**必需。** 修复文件的路径。该文件用于跟踪修复进度，以及恢复已中断的修复。 每个驱动器都必须有且仅有一个修复文件。 当你开始对给定驱动器进行修复时，请将路径传递到尚不存在的修复文件。 若要恢复已中断的修复，应该传入现有修复文件的名称。 请始终指定与目标驱动器对应的修复文件。|  
|**/logdir:**<LogDirectory\>|**可选。** 日志目录。 详细日志文件将写入此目录。 如果未指定任何日志目录，将使用当前目录作为日志目录。|  
|**/d:**<TargetDirectories\>|**必需。** 包含已导入的原始文件的一个或多个目录（以分号分隔）。 还可以使用导入驱动器，但如果原始文件的备用位置可用，则不需要该驱动器。|  
|**/bk：**<BitLockerKey\>|**可选。** 如果希望该工具解锁原始文件所在的已加密驱动器，请指定 BitLocker 密钥。|  
...
|**/PathMapFile:**<DrivePathMapFile\>|**可选。** 用于在同一作业中导入多个同名文件的文本文件的文件的路径。 工具首次运行时，会在此文件中填充所有不明确的名称。 以后运行该工具时，使用此文件来解决歧义。|  
  
## <a name="using-the-repairimport-command"></a>使用 RepairImport 命令  
若要通过网络流式传输导入数据来修复这些数据，必须使用 `/d` 参数指定包含导入的原始文件的目录。 还要指定从存储帐户下载的复制日志文件。 下面显示了一个用于修复部分失败的导入作业的典型命令行：  
  
```  
WAImportExport.exe RepairImport /r:C:\WAImportExport\9WM35C2V.rep /d:C:\Users\bob\Pictures;X:\BobBackup\photos /sn:bobmediaaccount /sk:VkGbrUqBWLYJ6zg1m29VOTrxpBgdNOlp+kp0C9MEdx3GELxmBw4hK94f7KysbbeKLDksg7VoN1W/a5UuM2zNgQ== /CopyLogFile:C:\WAImportExport\9WM35C2V.log  
```  
  
在以下复制日志文件的示例中，为导入作业寄送的驱动器上某个文件的 64K 内容已损坏。 由于此故障是唯一一个指示，因此作业中的其余 blob 已成功导入。  
...
在某些情况下，由于以下原因之一，该工具可能无法找到或打开所需的文件：找不到该文件、文件不可访问、文件名不明确或文件内容不再正确。  
  
如果该工具尝试查找 `\animals\koala.jpg` 且在和中有一个具有该名称的文件，则可能出现不明确的错误 `C:\Users\bob\pictures` `X:\BobBackup\photos` 。 也就是说，`C:\Users\bob\pictures\animals\koala.jpg` 和 `X:\BobBackup\photos\animals\koala.jpg` 都位于导入作业驱动器上。  
  
使用 `/PathMapFile` 选项可以解决这些错误。 您可以指定文件的名称，该文件包含工具无法正确识别的文件的列表。 以下命令行示例填充 `9WM35C2V_pathmap.txt`：  
...
```
 ...
 对于列表中的每个文件，应尝试找到并打开该文件，以确保它可用于该工具。 如果希望此工具显式通知您查找文件的位置，请修改路径映射文件，并在同一行上添加每个文件的路径，并以制表符分隔：  
