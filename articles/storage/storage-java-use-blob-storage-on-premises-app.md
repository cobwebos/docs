---
title: "使用 Blob 存储的本地应用程序 (Java) | Microsoft Docs"
...
在 **StorageSample** 类中，声明一个字符串变量，其中将包含默认终结点协议以及在 Azure 存储帐户中指定的存储帐户名和存储访问密钥。 将占位符值 **your\_account\_name** 和 **your\_account\_key** 分别替换为你自己的帐户名和帐户密钥。
...
使用 **File** 构造函数获取对将上传的本地创建文件的引用。 确保在运行代码之前已创建此文件。

```java
File fileReference = new File ("c:\\myimages\\image1.jpg");
```

通过调用 **CloudBlockBlob.upload** 方法，上传该本地文件。 **CloudBlockBlob.upload** 方法的第一个参数是 **FileInputStream** 对象，表示将上传到 Azure 存储的本地文件。 第二个参数是此文件的大小（以字节为单位）。
...
创建一个名为 **MakeHTMLPage** 的方法来创建一个基本 HTML 页面。 此方法具有一个 **CloudBlobContainer** 类型的参数，该参数将用于循环访问已上传 Blob 的列表。 此方法将引发 **FileNotFoundException** 类型的异常（可由 **FileOutputStream** 构造函数引发）以及 **URISyntaxException**（可由 **ListBlobItem.getUri** 方法引发）。 包括左大括号 **{**。
...
由于存储是收费的，因此建议在完成对此示例的试验之后，删除 **gettingstarted** 容器。 若要删除容器，请使用 **CloudBlobContainer.delete** 方法。
