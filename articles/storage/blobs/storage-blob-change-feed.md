...
更改源的可用小时段在清单文件中进行了描述，该文件指定了该段的更改源文件的路径。 `$blobchangefeed/idx/segments/`虚拟目录的列表显示按时间排序的这些段。 段的路径描述段所表示的每小时时间范围的开始时间。 您可以使用该列表来筛选出感兴趣的日志段。
...
> [!NOTE]
> `$blobchangefeed/idx/segments/1601/01/01/0000/meta.json`当启用更改源时，将自动创建。 可以放心地忽略此文件。 它是一个始终为空的初始化文件。 

段清单文件（`meta.json`）显示`chunkFilePaths`属性中该段的更改源文件的路径。 下面是段清单文件的示例。
...
> [!NOTE]
> 只有`$blobchangefeed`在你的帐户上启用了 "更改源" 功能之后，才会显示此容器。 必须先等待几分钟，再启用更改源，然后才能列出容器中的 blob。 
...
更改源文件在`$blobchangefeed/log/`虚拟目录中存储为[追加 blob](https://docs.microsoft.com/rest/api/storageservices/understanding-block-blobs--append-blobs--and-page-blobs#about-append-blobs)。 每个路径下的第一个更改源文件`00000`将具有文件名（例如`00000.avro`）。 添加到该路径的每个后续日志文件的名称将递增1（例如： `00001.avro`）。
...
- 更改其值`Control`为内部`eventType`系统记录并且不反映帐户中对象的更改的事件记录。 您可以放心地忽略这些记录。
...
- 由于日志流的内部分区，每`chunkFilePaths`个段可以具有不同的数量，以管理发布吞吐量。 每个`chunkFilePath`中的日志文件都保证包含互斥的 blob，并且可以并行使用和处理，而不会违反迭代期间每个 blob 的修改顺序。

- 段在状态中`Publishing`开始。 将记录追加到段完成后，它将是`Finalized`。 应用程序不应使用在`LastConsumable` `$blobchangefeed/meta/Segments.json`文件中的属性日期之后的任何段中的日志文件。 下面是`LastConsumable` `$blobchangefeed/meta/Segments.json`文件中属性的一个示例：
...
- 日志`url`文件的属性当前始终为空。
- 段`LastConsumable` json 文件的属性未列出更改源所完成的第一段。 仅在第一段完成后才会出现此问题。 第一小时之后的所有后续段都将在`LastConsumable`属性中精确捕获。
