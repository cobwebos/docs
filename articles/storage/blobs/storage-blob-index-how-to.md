...
在 Azure 门户中，"Blob 索引标记" 筛选器会`@container`自动应用参数以限定所选容器的范围。 如果希望在整个存储帐户中筛选和查找标记的数据，请使用我们的 REST API、Sdk 或工具。
...
5. 选择 "查看" " **+ 添加**" 以![查看带有 blob 索引标记的规则设置生命周期管理规则筛选器示例](media/storage-blob-index-concepts/blob-index-lifecycle-management-example.png)
