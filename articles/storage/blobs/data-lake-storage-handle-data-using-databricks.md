...
> * 创建文件系统并将数据上传到 Azure Data Lake Storage Gen2。
> * 创建服务主体。
...
> * 创建 Azure Data Lake Storage Gen2 帐户。 请参阅[创建 Azure Data Lake Storage Gen2 帐户](data-lake-storage-quickstart-create-account.md)。
...
## <a name="create-a-file-system-and-upload-sample-data"></a>创建文件系统并上传示例数据

首先，在 Data Lake Storage Gen2 帐户中创建一个文件系统。 然后就可以将示例数据文件上传到 Data Lake Store。 稍后需在 Azure Databricks 中使用此文件来运行一些转换操作。

1. 将 [small_radio_json.json](https://github.com/Azure/usql/blob/master/Examples/Samples/Data/json/radiowebsite/small_radio_json.json) 示例数据文件下载到本地文件系统。

2. 从 [Azure 门户](https://portal.azure.com/)导航到 Data Lake Storage Gen2 帐户，该帐户是你为了遵循本教程的先决条件而创建的。

3. 在存储帐户的“概览”页中，选择“在资源管理器中打开”。

   ![打开存储资源管理器](./media/data-lake-storage-handle-data-using-databricks/data-lake-storage-open-storage-explorer.png "打开存储资源管理器")

4. 选择“打开 Azure 存储资源管理器”，打开存储资源管理器。

   ![打开存储资源管理器第二个提示符](./media/data-lake-storage-handle-data-using-databricks/data-lake-storage-open-storage-explorer-2.png "打开存储资源管理器第二个提示符")

   此时会打开存储资源管理器。 可以根据本主题中的指导创建文件系统并上传示例数据：[快速入门：使用 Azure 存储资源管理器在 Azure Data Lake Storage Gen2 帐户中管理数据](data-lake-storage-explorer.md)。
...
## <a name="extract-data-from-the-data-lake-store"></a>从 Data Lake Store 提取数据

在此部分，请先在 Azure Databricks 工作区中创建一个笔记本，然后运行代码片段，以便将数据从 Data Lake Store 提取到 Azure Databricks 中。
...
5. 在此代码块中，请将 `application-id`、`authentication-id`、`tenant-id` 占位符的值替换为你在完成[保存存储帐户配置](#config)中的步骤时收集的值。 将 `storage-account-name` 占位符值替换为存储帐户的名称。

6. 按 **SHIFT + ENTER** 键，运行此块中的代码。

7. 现在可以将示例 json 文件加载为 Azure Databricks 中的数据帧。 将以下代码粘贴到新单元格中。 将括号中显示的占位符替换为你的值。

   ```scala
   val df = spark.read.json("abfss://<file-system-name>@<storage-account-name>.dfs.core.windows.net/small_radio_json.json")
   ```

   * 将 `file-system-name` 占位符值替换为在存储资源管理器中提供给文件系统的名称。

   * 将 `storage-account-name` 占位符替换为存储帐户的名称。

8. 按 **SHIFT + ENTER** 键，运行此块中的代码。
...
4. 提供连接到 Azure SQL 数据仓库实例所需的值。 必须已经将 SQL 数据仓库作为先决条件创建。
