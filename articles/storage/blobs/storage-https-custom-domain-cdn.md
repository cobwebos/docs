...
可以利用此功能通过 HTTPS 访问使用自定义域的存储 Blob。 要执行此操作，首先需要在 Blob 终结点上启用 Azure CDN，并将 CDN 映射到一个自定义域名。 执行这些步骤后，借助一键式启用、完善的证书管理，为自定义域启用 HTTPS 的流程得以简化，并且所有这些都包含在常规 CDN 定价中，不会额外增加成本。
...
“Cdn-endpoint-name”指的是为 CDN 终结点配置的名称。 可以从下拉列表中选择此值。 “Origin-path”指的是原始存储帐户内静态内容所在的路径。
如果将所有静态内容承载在单个容器中，请将“origin-path”替换为该容器的名称。
