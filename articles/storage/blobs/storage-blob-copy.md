...
ms.locfileid: "72600275"
---
# <a name="copy-a-blob-with-net"></a>使用 .NET 复制 blob

本文演示如何将 blob 复制到 Azure 存储帐户。 它还演示了如何中止异步复制操作。 示例代码使用适用于[.net 的 Azure 存储客户端库](/dotnet/api/overview/azure/storage/client)。
...
复制操作的源 blob 可以是块 blob、追加 blob、页 blob 或快照。 如果目标 blob 已存在，则它必须与源 blob 具有相同的 blob 类型。 将覆盖任何现有的目标 blob。 
...
对于所有 blob 类型，可以检查目标 blob 上的[CopyState](/dotnet/api/microsoft.azure.storage.blob.copystate.status?view=azure-dotnet)属性，以获取复制操作的状态。 完成复制后，最终的 blob 将被提交。

复制操作可以采用以下任何形式：

  - 可以将源 blob 复制到具有不同名称的目标 blob。 目标 blob 可以是相同 blob 类型（块、追加或页）的现有 blob，也可以是复制操作创建的新 blob。
