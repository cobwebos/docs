...
> [!NOTE]
> 如果为存储帐户启用了防火墙规则，生命周期管理请求可能会被阻止。 可以通过提供例外来取消阻止这些请求。 所需的回避为`Logging,  Metrics,  AzureServices`:。 有关详细信息，请参阅[配置防火墙和虚拟网络](https://docs.microsoft.com/azure/storage/common/storage-network-security#exceptions)中的“例外”部分。
...
以下 PowerShell 脚本可用于将策略添加到存储帐户。 必须用资源组名称初始化变量。`$rgname` 必须用您的存储帐户名称初始化变量。`$accountName`
...
## <a name="azure-resource-manager-template-with-lifecycle-management-policy"></a>具有生命周期管理策略的 Azure 资源管理器模板
...
| `type`         | 枚举值 | 当前有效的类型为`Lifecycle`。 | True |
...
下面的示例规则筛选帐户, 以对中`container1`的对象和以`foo`开头的对象运行操作。  
...
某些数据在云中保持空闲状态，并且在存储后很少（如果有）被访问。 以下生命周期策略配置为在引入数据后对其进行存档。 此示例将容器`archivecontainer`内存储帐户中的 blob 转换为存档层。 通过在上次修改时间之后的 blob 0 天执行转换:
...
将 blob 从一个访问层移到另一个访问层时, 其上次修改时间将不会更改。 如果将存档的 blob 手动解除冻结到 "热" 层, 则生命周期管理引擎会将该 blob 移回存档层。 禁用影响此 blob 的规则, 以防止它再次存档。 如果 blob 需要始终处于热层中, 请将其复制到另一个位置。 当 blob 可以安全地移回存档层时, 重新启用规则。 
