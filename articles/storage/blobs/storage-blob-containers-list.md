...
默认情况下，列出操作每次最多返回 5000 条结果。 若要返回更少的结果，请在调用某个 `maxresults`ListContainerSegmented**方法时为** 参数提供非零值。
...
若要连同结果一起返回容器元数据，请指定 **ContainerListingDetails** 枚举的 [Metadata](/dotnet/api/microsoft.azure.storage.blob.containerlistingdetails) 值。 Azure 存储包含每个返回的容器的元数据，因此无需同时调用 **FetchAttributes** 方法之一即可检索容器元数据。
