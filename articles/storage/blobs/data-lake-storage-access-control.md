...
ms.locfileid: "79255569"
---
# <a name="access-control-in-azure-data-lake-storage-gen2"></a>Azure Data Lake Storage Gen2 中的访问控制

Azure Data Lake Storage Gen2 实现一个支持 Azure 基于角色的访问控制（RBAC）和类似于 POSIX 的访问控制列表（Acl）的访问控制模型。 本文汇总了 Data Lake Storage Gen2 访问控制模型的基本知识。

<a id="azure-role-based-access-control-rbac" />

## <a name="role-based-access-control"></a>基于角色的访问控制

RBAC 使用角色分配有效地将权限集应用于*安全主体*。 *安全主体*是一个对象，表示用户、组、服务主体或在请求访问 Azure 资源的 AZURE ACTIVE DIRECTORY （AD）中定义的托管标识。

通常，这些 Azure 资源会被限制为顶级资源（例如： Azure 存储帐户）。 对于 Azure 存储，进而 Azure Data Lake Storage Gen2，此机制已扩展到容器（文件系统）资源。
...
### <a name="the-impact-of-role-assignments-on-file-and-directory-level-access-control-lists"></a>角色分配对文件和目录级别访问控制列表的影响

尽管使用 RBAC 角色分配是一种强大的机制来控制访问权限，但它是相对于 Acl 的相当粒度机制。 RBAC 的最小粒度为容器级别，其计算优先级高于 Acl。 因此，如果将角色分配到容器作用域中的安全主体，则无论 ACL 分配如何，该安全主体都具有与该容器中的所有目录和文件相关联的授权级别。

通过[内置角色](https://docs.microsoft.com/azure/storage/common/storage-auth-aad?toc=%2fazure%2fstorage%2fblobs%2ftoc.json#built-in-rbac-roles-for-blobs-and-queues)或自定义角色向安全主体授予 RBAC 数据权限时，将首先评估请求的授权。 如果请求的操作由安全主体的 RBAC 分配授权，则会立即解决授权，并且不执行任何其他 ACL 检查。 或者，如果安全主体没有 RBAC 分配，或请求的操作与分配的权限不匹配，则会执行 ACL 检查来确定是否已授权安全主体执行请求的操作。

> [!NOTE]
> 如果已为安全主体分配了存储 Blob 数据所有者内置角色分配，则会将安全主体视为*超级用户*，并向其授予对所有变异操作的完全访问权限，包括设置目录或文件的所有者，以及它们不是其所有者的目录和文件的 acl。 超级用户访问是唯一获准的更改资源所有者的方式。

## <a name="shared-key-and-shared-access-signature-sas-authentication"></a>共享密钥和共享访问签名（SAS）身份验证

Azure Data Lake Storage Gen2 支持用于身份验证的共享密钥和 SAS 方法。 这些身份验证方法的特征是没有与调用方关联的标识，因此不能执行安全主体基于权限的授权。
...
可以将安全主体与文件和目录的访问级别相关联。 这些关联在*访问控制列表（ACL）* 中捕获。 存储帐户中的每个文件和目录都具有访问控制列表。

如果在存储帐户级别为安全主体分配了角色，则可以使用访问控制列表授予该安全主体对特定文件和目录的更高访问权限。

不能使用访问控制列表提供低于角色分配授予的级别的访问级别。 例如，如果将 "[存储 Blob 数据参与者](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#storage-blob-data-contributor)" 角色分配到安全主体，则不能使用 "访问控制列表" 来防止安全主体写入目录。

### <a name="set-file-and-directory-level-permissions-by-using-access-control-lists"></a>使用访问控制列表设置文件和目录级别权限

若要设置文件和目录级别权限，请参阅以下任意文章：
...
> [!IMPORTANT]
> 如果安全主体是*服务*主体，则必须使用服务主体的对象 id，而不是相关应用程序注册的对象 id。 若要获取服务主体的对象 ID，请打开 Azure CLI，然后使用以下命令： `az ad sp show --id <Your App ID> --query objectId`。 请确保将 `<Your App ID>` 占位符替换为应用注册的应用 ID。

### <a name="types-of-access-control-lists"></a>访问控制列表的类型

有两种类型的访问控制列表：*访问 acl*和*默认 acl*。

访问 ACL 控制对某个对象的访问权限。 文件和目录都具有访问 ACL。

默认 Acl 是与目录关联的 Acl 的模板，用于确定在该目录下创建的任何子项的访问 Acl。 文件没有默认 ACL。
...
> [!NOTE]
> 如果你只使用 Acl （无 RBAC）来授予权限，则若要授予安全主体对文件的读取或写入访问权限，你将需要为容器以及导致文件的文件夹层次结构中的每个文件夹授予安全主体**执行**权限。
...
下表列出了一些常见方案，可帮助您了解对存储帐户执行特定操作所需的权限。
...
在 POSIX ACL 中，每个用户都与“主组”关联。 例如，用户 "Alice" 可能属于 "财务" 组。 Alice 还可能属于多个组，但一个组始终被指定为其主要组。 在 POSIX 中，当 Alice 创建文件时，该文件的拥有组设置为她的主组，在本例中为“finance”。 否则，所有者组的行为类似于为其他用户/组分配的权限。
...
> [!NOTE]
> 拥有组无法更改某个文件或目录的 ACL。  当拥有组设置为创建帐户的用户时，**如果**是在根目录的情况下创建的，则单个用户帐户对于通过拥有组提供权限是无效的。 可以将此权限分配给有效的用户组（如果适用）。
...
> [!NOTE]
> 对于新的 Data Lake Storage Gen2 容器，根目录（"/"）的访问 ACL 的掩码默认为750（对于目录），而640用于文件。 文件不接收 X 位，因为它与只存储系统中的文件无关。
>
...
不是。 只要启用了分层命名空间（HNS）功能，就会为存储帐户启用通过 Acl 的访问控制。
...
### <a name="how-do-i-set-acls-correctly-for-a-service-principal"></a>如何实现为服务主体正确设置 Acl？

当你为服务主体定义 Acl 时，必须为你创建的应用注册使用*服务主体*的对象 ID （OID）。 需要特别注意的是，已注册的应用在特定 Azure AD 租户中具有单独的服务主体。 已注册的应用的 OID 在 Azure 门户中可见，但该*服务主体*有另一个（不同的） oid。

若要获取与应用注册对应的服务主体的 OID，可以使用 `az ad sp show` 命令。 指定应用程序 ID 作为参数。 下面是一个示例，介绍如何获取与应用注册（应用 ID = 18218b12-1895-43e9-ad80-6e8fc1ea88ce）对应的服务主体的 OID。 在 Azure CLI 中运行以下命令：
...
如果有服务主体的正确 OID，请访问存储资源管理器**管理访问权限**"页，添加 oid 并为 oid 分配适当的权限。 请确保选择 "**保存**"。
