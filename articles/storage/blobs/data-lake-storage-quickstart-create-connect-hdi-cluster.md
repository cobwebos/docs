...
虽然 Hadoop 的本地安装使用 Hadoop 分布式文件系统 (HDFS) 作为群集上的存储，但在云中，会使用连接到群集的存储终结点。 HDInsight 群集使用 [Data Lake Storage Gen2](data-lake-storage-abfs-driver.md) 或 [Azure 存储中的 Blob](../../hdinsight/hdinsight-hadoop-use-blob-storage.md)。 使用 Azure 存储或 Data Lake Storage 意味着可以安全地删除用于计算的 HDInsight 群集，同时仍保留数据。
