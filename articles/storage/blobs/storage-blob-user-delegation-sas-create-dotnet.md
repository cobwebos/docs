...
将以下`using`指令添加到代码, 以使用 azure 标识和 azure 存储客户端库的预览版本。
...
若要对服务主体进行身份验证, 请创建[DefaultAzureCredential](/dotnet/api/azure.identity.defaultazurecredential)类的实例。 `DefaultAzureCredential`构造函数读取您之前创建的环境变量。
