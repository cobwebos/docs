...
可以通过基于角色的访问控制 (RBAC) ，将 blob 数据的权限分配给 Azure AD 安全主体。 有关 Azure 存储中 Azure 角色的详细信息，请参阅[通过 RBAC 管理 Azure 存储数据访问权限](../common/storage-auth-aad-rbac.md)。

## <a name="supported-operations"></a>支持的操作

对 blob 数据的操作支持 Azure 存储扩展。 可调用的操作取决于向 Azure AD 安全主体授予的权限，此安全主体用于登录 PowerShell。 通过 RBAC 分配对 Azure 存储容器的权限。 例如，如果已将 **Blob 数据读取器** 角色分配给你，则可以运行脚本命令，从容器中读取数据。 如果已为你分配了 " **Blob 数据参与者** " 角色，则可以运行脚本命令来读取、写入或删除容器或其包含的数据。
...
1. 创建容器之前，请向自己分配[存储 Blob 数据参与者](../../role-based-access-control/built-in-roles.md#storage-blob-data-contributor)角色。 即使你是帐户所有者，也需要显式权限才能针对存储帐户执行数据操作。 有关如何分配 Azure 角色的详细信息，请参阅[在 Azure 门户中使用 RBAC 授予对 Azure Blob 和队列数据的访问权限](../common/storage-auth-aad-rbac.md)。
