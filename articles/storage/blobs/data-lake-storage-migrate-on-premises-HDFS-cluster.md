...
7. 将数据从 Hadoop HDFS 复制到你之前创建的容器中 Data Box Blob 存储。 如果找不到要复制到的目录，则该命令会自动创建它。
    ...
    * 将 `<destination_directory>` 占位符替换为要将数据复制到的目录的名称。
    ...
    @No__t-0 选项用于将 `hadoop-azure*.jar` 和相关 `azure-storage*.jar` 文件提供给 `distcp`。 对于某些群集，可能已发生此情况。

    下面的示例演示如何使用 `distcp` 命令复制数据。
...
* 若要使用基于 Azure 的 Hadoop 群集，请运行以下 DistCp 命令：
    ...
    * 将 @no__t 0 和 @no__t 占位符替换为源和目标存储帐户的名称。

    * 将 @no__t 0 和 @no__t 占位符替换为源和目标容器的名称。

    * 将 @no__t 0 和 @no__t 占位符替换为源目录和目标目录路径。
...
若要创建服务主体，请参阅 [How to：使用门户创建可访问资源的 Azure AD 应用程序和服务主体](https://docs.microsoft.com/azure/active-directory/develop/howto-create-service-principal-portal)。
...
1. 下载 @no__t 的脚本。 请参阅本文中的[下载帮助器脚本并设置边缘节点以运行它们](#download-helper-scripts)。
...
* 用在创建服务主体时收集的应用程序 ID 和客户端机密替换 `<application-id>` 和 @no__t 1 占位符。
