...
完成项目导入后，打开 AzureApp.java（位于 src/main/java 内的 blobQuickstart.blobAzureApp 中），并替换 **字符串中的** 和`accountname``accountkey``storageConnectionString`。 然后运行应用程序。 以下部分是有关如何完成这些任务的具体说明。
