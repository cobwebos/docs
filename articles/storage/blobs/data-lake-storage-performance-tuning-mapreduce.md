---
title: 调整性能：MapReduce、HDInsight& Azure 数据存储第 2 代 |微软文档
...
---
# <a name="tune-performance-mapreduce-hdinsight--azure-data-lake-storage-gen2"></a>调整性能：MapReduce、HDInsight& Azure 数据存储第 2 代
...
* Azure Data Lake Storage Gen2 帐户****。 有关如何创建一个存储帐户的说明，请参阅[快速入门：创建 Azure 数据存储湖存储 Gen2 存储帐户](data-lake-storage-quickstart-create-account.md)。
...
* Data Lake Storage Gen2 的性能优化指南****。  有关一般性能概念，请参阅[数据存储湖存储第 2 代性能调优指南](data-lake-storage-performance-tuning-guidance.md)
...
要优化 mapreduce.job.maps/mapreduce.job.reduces，应考虑可供使用的总 YARN 内存量。  该信息在 Ambari 中提供。  导航到 YARN 并查看"配置"选项卡。 YARN 内存将显示在此窗口中。  应将 YARN 内存量与群集中的节点数相乘，获得总 YARN 内存量。
