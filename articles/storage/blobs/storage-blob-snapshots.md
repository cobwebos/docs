...
以下代码示例演示如何使用[用于 .NET 的 Azure 存储客户端库](/dotnet/api/overview/azure/storage/client)创建快照。 本示例在创建快照时为其指定了其他元数据。
...
* 可以将快照复制到其基本 Blob 上。 通过将快照提升到基本 Blob 的位置，可还原早期版本的 Blob。 快照将保留，但基本 Blob 会被快照的可写副本覆盖。
