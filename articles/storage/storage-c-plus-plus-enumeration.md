<properties 
    pageTitle="使用用于 C++ 的 Microsoft Azure 存储空间客户端库列出 Azure 存储空间资源 | Microsoft Azure" 
    description="了解如何在用于 C++ 的 Microsoft Azure 存储空间客户端库中使用列表 API 来枚举容器、blob、队列、表和实体。" 
...
使用 Azure 存储空间进行开发时，很多情况下列表操作很重要。本文介绍如何使用用于 C++ 的 Microsoft Azure 存储空间客户端库中提供的列表 API 最有效率地枚举 Azure 存储空间中的对象。
