...
本指南说明如何使用适用于 Azure 移动服务的 iOS 客户端执行常见任务。这些示例用 Objective-C 编写，需要安装[移动服务 SDK][移动服务 SDK]。本教程还需要安装 [iOS SDK][iOS SDK]。所述的任务包括：查询数据，插入、更新和删除数据，对用户进行身份验证和处理错误。如果你是第一次使用移动服务，最好先完成[移动服务快速入门][移动服务快速入门]。快速入门教程可帮助你配置帐户并创建第一个移动服务。
...
本指南假设你已创建一个移动服务和一个表。有关详细信息，请参阅[创建表][创建表]。本主题中的示例使用名为 `ToDoItem` 的表，该表包含以下列：
...
-   *totalCount*：查询的所有页中的总项数，而不仅仅是在当前页中返回的项数。除非你在请求中显式请求了总计数，否则此值将设置为 -1。有关详细信息，请参阅[在页中返回数据][在页中返回数据]。
...
有关详细信息，请参阅[如何：访问自定义参数][如何：访问自定义参数]。

<a name="inserting"></a>
## 插入数据如何：在移动服务中插入数据

若要在表中插入新行，可以创建一个新的 [NSDictionary 对象][NSDictionary 对象]并将该对象传递给 insert 函数。以下代码将在表中插入一个新的 todo 项：
...
启用动态架构后，移动服务将基于 insert 或 update 请求中对象的字段自动生成新列。有关详细信息，请参阅[动态架构][动态架构]。
...
有关配置标识提供者的详细信息，请参阅[身份验证入门][身份验证入门]。
...
如果身份验证成功，则返回包含分配的用户 ID 值和身份验证令牌的用户对象。你可以在服务器脚本中使用此用户 ID 来验证或修改请求。有关详细信息，请参阅[使用脚本为用户授权][使用脚本为用户授权]。你可以安全缓存该令牌本身，以便在后续登录时使用。

你还可以在表中设置权限，以便将特定操作的访问权限限制给已经过身份验证的用户。有关详细信息，请参阅[权限][权限]。
...
以下示例使用 [Live Connect SDK][Live Connect SDK] 为 iOS 应用程序启用单一登录。
...
使用缓存的令牌时，在该令牌过期之前，用户不需要重新登录。如果用户尝试使用过期的令牌登录，系统将返回 401 未授权响应。此时，该用户必须重新登录以获取新令牌，而该令牌同样会得到缓存。如果使用筛选器，则不需要在应用程序调用移动服务时编写代码来处理过期的令牌。筛选器可让你截获对移动服务的调用以及来自移动服务的响应。筛选器中的代码将测试 401 响应，如果令牌已过期，则触发登录进程，然后重试执行生成 401 响应的请求。有关详细信息，请参阅[处理过期的令牌][处理过期的令牌]。
