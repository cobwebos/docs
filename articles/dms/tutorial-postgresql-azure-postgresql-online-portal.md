---
title: 教程：通过 Azure 门户将 PostgreSQL 迁移到 Azure 数据库，以便联机发布
titleSuffix: Azure Database Migration Service
description: 通过 Azure 数据库迁移服务，了解如何通过 Azure 数据库迁移服务执行从本地 PostgreSQL 到 Azure 数据库的联机迁移。
services: dms
author: HJToland3
ms.author: jtoland
manager: craigg
...
可以使用 Azure 数据库迁移服务将数据库从本地 PostgreSQL 实例迁移到[用于 PostgreSQL 的 Azure 数据库](https://docs.microsoft.com/azure/postgresql/)，而停机时间将降至最低。 本教程介绍如何在 Azure 数据库迁移服务中使用联机迁移活动将 **DVD Rental** 示例数据库从 PostgreSQL 9.6 的本地实例迁移到 Azure Database for PostgreSQL。
...
* 确保虚拟网络的网络安全组 （NSG） 规则不会阻止以下到 Azure 数据库迁移服务的入站通信端口：443、53、9354、445、12000。 有关虚拟网络 NSG 流量筛选的更多详细信息，请参阅[使用网络安全组筛选网络流量](https://docs.microsoft.com/azure/virtual-network/virtual-network-vnet-plan-design-arm)一文。
...
> [!IMPORTANT]
> 现有数据库中的所有表都需要一个主键，以确保更改可以同步到目标数据库。
...
2. 在目标环境中创建一个空数据库，即 Azure Database for PostgreSQL。
    ...
    > [!NOTE]
    > 后缩放- 超大规模 （Citus） 的 Azure 数据库的实例只有一个数据库 **：citus**。
...
4. 要提取删除外键脚本并将其添加到目标（用于 PostgreSQL 的 Azure 数据库）、PgAdmin 或 psql 中，请参阅以下脚本。
...
6. 要禁用目标数据库中的触发器，请运行下面的脚本。

   > [!IMPORTANT]
   > 数据中的触发器（插入或更新）在从源复制的数据之前，在目标中强制数据完整性。 因此，建议您在迁移期间禁用**目标所有**表中的触发器，然后在迁移完成后重新启用触发器。
...
3. 在 **"创建迁移服务"** 屏幕上，指定名称、订阅、新资源组或现有资源组以及服务的位置。

4. 选择现有虚拟网络或新建一个。

    虚拟网络为 Azure 数据库迁移服务提供了对源 PostgreSQL 服务器和 PostgreSQL 实例的目标 Azure 数据库的访问权限。
...
2. 在**Azure 数据库迁移服务**屏幕上，搜索您创建的 Azure 数据库迁移服务实例的名称，选择实例，然后选择 "**新的迁移项目**"。

3. 在 **"新建迁移项目"** 屏幕上，指定项目的名称，在 **"源服务器类型**文本"框中，在 **"目标服务器类型**文本"框中选择**PostgresSQL，** 为**PostgreSQL**选择 Azure 数据库。
...
5. 选择 **"保存**"，注意成功使用 Azure 数据库迁移服务迁移数据的要求，然后选择"**创建并运行活动**"。
...
1. 在 **"目标详细信息**"屏幕上，指定目标超大规模 （Citus） 服务器的连接详细信息，该服务器是使用pg_dump部署**DVD 租赁**架构的预预配超量程 （Citus） 实例。
...
2. 选择“保存”，然后在“映射到目标数据库”屏幕上，映射源和目标数据库以进行迁移。********
    ...
    ![映射到目标数据库屏幕](media/tutorial-postgresql-to-azure-postgresql-online-portal/dms-map-target-databases.png)

3. 选择 **"保存**"，然后在 **"迁移设置"** 屏幕上接受默认值。

    ![迁移设置屏幕](media/tutorial-postgresql-to-azure-postgresql-online-portal/dms-migration-settings.png)
...
* 选择“运行迁移”****。
    ...
    将显示迁移活动窗动**状态**应更新以显示为**正在进行备份**。

## <a name="monitor-the-migration"></a>监视迁移

1. 在迁移活动屏幕上选择“刷新”****，以便更新显示，直到迁移的“状态”**** 显示为“完成”****。

     ![监控迁移过程](media/tutorial-postgresql-to-azure-postgresql-online-portal/dms-monitor-migration.png)

2. 迁移完成后，在 **"数据库名称**"下，选择特定数据库以达到 **"完全数据加载**"和 **"增量数据同步**操作"的迁移状态。
...
2. 等待挂起**的更改**计数器显示**0**以确保停止到源数据库的所有传入事务，选择"**确认**"复选框，然后选择"**应用**"。

    ![完整的剪切屏幕](media/tutorial-postgresql-to-azure-postgresql-online-portal/dms-complete-cutover.png)

3. 当数据库迁移状态显示 **"已完成"** 时，将应用程序连接到 PostgreSQL Azure 数据库的新目标实例。
