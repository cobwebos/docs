...
* SSMS 17.2 版本或更高版本。
* 用于托管 SSISDB 的目标数据库服务器的实例。 如果还没有数据库服务器，请通过导航到 SQL Server（仅限逻辑服务器）[表单](https://ms.portal.azure.com/#create/Microsoft.SQLServer)，使用 Azure 门户创建一个 Azure SQL Database 服务器（不带数据库）。
* SSIS 必须在包含 Azure-SSIS Integration Runtime (IR) 的 Azure 数据工厂 (ADF) 中进行预配，并且目标 SSISDB 由 Azure SQL Database 服务器实例托管（如文章[在 Azure 数据工厂中预配 Azure-SSIS Integration Runtime](https://docs.microsoft.com/azure/data-factory/tutorial-deploy-ssis-packages-azure) 中所述）。
...
9. 在“选择目标”页上，指定项目目标****。
    ...
    a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，然后单击“添加引用”。 在"服务器名称文本"框中，输入完全限定的 Azure SQL 数据库服务器名称（<server_name>.database.windows.net）。
...
12. 在“查看”页上，查看部署设置****。
    ...
    > [!NOTE]
    > 您可以通过选择 **"上一步"** 或选择左侧窗格中的任何步骤链接来更改设置。
