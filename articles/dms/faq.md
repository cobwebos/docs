...
**问：使用 Azure 数据库迁移服务时，脱机迁移和联机迁移之间的区别是什么？**
可以使用 Azure 数据库迁移服务执行脱机和联机迁移。 使用*脱机*迁移时，应用程序停机时间从迁移开始时开始。 使用*联机*迁移时，停机时间仅限于在迁移结束时缩短的时间。 建议对脱机迁移进行测试，以便确定其停机时间是否可以接受；如果不能接受，请进行联机迁移。
...
* 通过使用 Azure 资源管理器部署模型为 Azure 数据库迁移服务创建 Microsoft Azure 虚拟网络，该模型通过使用[ExpressRoute](https://docs.microsoft.com/azure/expressroute/expressroute-introduction)或[VPN](https://docs.microsoft.com/azure/vpn-gateway/vpn-gateway-about-vpngateways)提供到本地源服务器的站点到站点的连接。
...
有关使用 Azure 数据库迁移服务竞争特定迁移方案所需的所有先决条件的列表，请参阅有关docs.microsoft.com的 Azure 数据库迁移服务[文档中](https://docs.microsoft.com/azure/dms/dms-overview)的相关教程。

**问：如何查找 Azure 数据库迁移服务的 IP 地址，以便为用于访问源数据库进行迁移的防火墙规则创建允许列表？**
...
虽然多个 Microsoft 教程可以引导您完成设置虚拟网络的过程，但官方文档将显示在[文章 Azure 虚拟网络](https://docs.microsoft.com/azure/virtual-network/virtual-networks-overview)中。
...
**问：我在 DMS 中设置迁移项目，并且很难连接到源数据库。我该怎么办？**
