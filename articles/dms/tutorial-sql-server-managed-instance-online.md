---
title: 教程：将 SQL 服务器联机迁移到 SQL 托管实例
...
ms.locfileid: "80298906"
---
# <a name="tutorial-migrate-sql-server-to-an-azure-sql-database-managed-instance-online-using-dms"></a>教程：使用 DMS 在线将 SQL 服务器迁移到 Azure SQL 数据库托管实例

可以使用 Azure 数据库迁移服务将数据库从本地 SQL Server 实例迁移到 [Azure SQL 数据库托管实例](../sql-database/sql-database-managed-instance.md)，且几乎不用停机。 有关可能需要一些手动工作的其他方法，请参阅有关[SQL Server 实例迁移到 Azure SQL 数据库托管实例](../sql-database/sql-database-managed-instance-migrate.md)的文章。
...
1. 在**迁移目标详细信息**屏幕上，指定 DMS 实例可用于连接到 Azure SQL 数据库托管实例和 Azure 存储帐户的目标实例**的应用程序 ID**和**密钥**。
...
2. 选择包含 Azure SQL 数据库托管实例的目标实例的**订阅**，然后选择目标实例。

    如果尚未预配 Azure SQL 数据库托管实例，请选择[该链接](https://aka.ms/SQLDBMI)以帮助您预配实例。 Azure SQL 数据库托管实例准备就绪后，返回到此特定项目以执行迁移。
...
5. 当数据库迁移状态显示 **"已完成"** 时，将应用程序连接到 Azure SQL 数据库托管实例的新目标实例。
