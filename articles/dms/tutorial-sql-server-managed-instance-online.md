...
* 确保虚拟网络安全组规则不会阻止以下到 Azure 数据库迁移服务的入站通信端口：443、53、9354、445、12000。 有关虚拟网络 NSG 流量筛选的更多详细信息，请参阅[使用网络安全组筛选网络流量](https://docs.microsoft.com/azure/virtual-network/virtual-networks-nsg)一文。
...
1. 在 "**迁移目标详细信息**" 屏幕上，指定 DMS 实例可用于连接到 Azure SQL 数据库托管实例和 Azure 存储帐户的目标实例的**应用程序 ID**和**密钥**。
...
2. 选择包含 Azure SQL 数据库托管实例的目标实例的**订阅**，然后选择目标实例。
    ...
    如果尚未预配 Azure SQL 数据库托管实例，请选择[链接](https://aka.ms/SQLDBMI)以帮助你预配该实例。 Azure SQL 数据库托管实例准备就绪后，返回到此特定项目以执行迁移。
...
5. 数据库迁移状态显示为 "**已完成**" 时，将应用程序连接到 Azure SQL 数据库托管实例的新目标实例。

    ![交接完成](media/tutorial-sql-server-to-managed-instance-online/dms-cutover-complete.png)

## <a name="next-steps"></a>后续步骤

* 有关演示如何使用 T-sql RESTORE 命令将数据库迁移到托管实例的教程，请参阅[使用 restore 命令将备份还原到托管实例](../sql-database/sql-database-managed-instance-restore-from-backup-tutorial.md)。
