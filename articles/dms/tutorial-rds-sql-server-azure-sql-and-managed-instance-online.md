...
本文介绍从 RDS SQL Server 到 SQL 数据库或 SQL 托管实例的联机迁移。
...
* 在 Azure 门户中的[AZURE SQL 数据库中创建数据库](https://docs.microsoft.com/azure/sql-database/sql-database-get-started-portal)，或[在 SQL 托管实例中创建数据库](https://docs.microsoft.com/azure/sql-database/sql-database-managed-instance-get-started)，然后创建一个名为**AdventureWorks2012**的空数据库。 
...
* 对于 SQL 数据库，创建服务器级[防火墙规则](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure)以允许 Azure 数据库迁移服务访问目标数据库。 提供用于 Azure 数据库迁移服务的虚拟网络子网范围。
* 确保用于连接到源 RDS SQL Server 实例的凭据与属于“Processadmin”服务器角色的帐户相关联，并与属于要迁移的所有数据库上的“db_owner”数据库角色的帐户相关联。
* 如果迁移到 SQL 托管实例中的数据库，请确保用于连接到目标数据库的凭据对 SQL 数据库中的目标数据库拥有 CONTROL DATABASE 权限和 sysadmin 角色的成员。
...
> [!NOTE]
> 在 DMA 中创建迁移项目之前，请确保已在 SQL 数据库或 SQL 托管实例中预配了数据库，如先决条件中所述。 在本教程中，假定数据库的名称为**AdventureWorks2012**，但可以提供任何所需的名称。
...
2. 指定项目名称，在“源服务器类型”文本框中，选择“SQL Server”，然后在“目标服务器类型”文本框中，选择“Azure SQL 数据库”。
    ...
    > [!NOTE]
    > 对于 "目标服务器类型"，请选择 " **AZURE Sql 数据库**" 以迁移到 Azure sql DATABASE 和 SQL 托管实例。
...
6. 选择 "**下一步**"，在 "**连接到目标服务器**" 下，指定 sql 数据库或 sql 托管实例中数据库的目标连接详细信息，选择 "**连接**"，然后选择预配的**AdventureWorksAzure**数据库。

    ![数据迁移助手目标连接详细信息](media/tutorial-rds-sql-to-azure-sql-and-managed-instance/dma-target-connect.png)

7. 选择 "**下一步**" 以转到 "**选择对象**" 屏幕，您可以在该屏幕上指定需要部署的**AdventureWorks2012**数据库中的架构对象。
...
9. 选择 "**部署架构**" 以部署架构，然后在部署架构后，检查目标是否存在异常。
...
5. 选择现有虚拟网络或新建一个。
    ...
    虚拟网络提供 Azure 数据库迁移服务，可访问源 SQL Server 和目标 SQL 数据库或 SQL 托管实例。
...
4. 在“新建迁移项目”屏幕上指定项目名称，在“源服务器类型”文本框中选择“AWS RDS for SQL Server”，在“目标服务器类型”文本框中选择“Azure SQL 数据库”。**** **** **** **** ****
    ...
    > [!NOTE]
    > 对于 "目标服务器类型"，选择 " **AZURE Sql 数据库**" 以迁移到 sql 数据库和 sql 托管实例。
...
3. 在“选择表”屏幕上选择“保存”，展开表列表，如何查看受影响字段的列表**** ****。
    ...
    Azure 数据库迁移服务自动选择目标数据库上存在的所有空源表。 若要重新迁移已包含数据的表，需要在此屏幕上显式选择表。
...
3. 选择“确认”****，然后选择“应用”****。
4. 数据库迁移状态显示为 "**已完成**" 时，将应用程序连接到新的目标数据库。

    ![活动状态 - 已完成](media/tutorial-rds-sql-to-azure-sql-and-managed-instance/dms-activity-completed.png)

## <a name="next-steps"></a>后续步骤

* 有关执行到 Azure 的联机迁移的已知问题和限制的信息，请参阅文章[联机迁移的已知问题和解决方法](known-issues-azure-sql-online.md)。
...
* 有关 SQL 托管实例的信息，请参阅[什么是 sql 托管实例](https://docs.microsoft.com/azure/azure-sql/managed-instance/sql-managed-instance-paas-overview)一文。
