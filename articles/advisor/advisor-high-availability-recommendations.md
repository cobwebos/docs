...
在主动-主动配置中，VPN 网关的两个实例都将建立 S2S VPN 隧道到本地 VPN 设备。 当一个网关实例上发生计划内维护事件或计划外事件时，流量将自动切换到其他活动 IPsec 隧道。 Azure 顾问将识别未配置为主动-主动的 VPN 网关并建议将它们配置为高可用性。

## <a name="use-production-vpn-gateways-to-run-your-production-workloads"></a>使用生产 VPN 网关运行生产工作负载

Azure 顾问将检查任何作为基本 SKU 的 VPN 网关，并建议您改用生产 SKU。 基本 SKU 专为开发和测试目的而设计。 生产 SKU 提供数量更高的隧道、BGP 支持、主动-主动配置选项、自定义 Ipsec/IKE 策略以及更高的稳定性和可用性。
...
Azure Cosmos DB 容器配置了延迟索引模式可能会影响查询结果的新鲜度。 顾问将检测以这种方式配置的容器，并建议切换到一致模式。 [了解有关 Cosmos DB 中索引策略的更多详细信息](https://aka.ms/cosmosdb/how-to-manage-indexing-policy)

## <a name="configure-your-azure-cosmos-db-containers-with-a-partition-key"></a>为 Azure Cosmos DB 容器配置分区键

Azure 顾问将标识 Azure Cosmos DB 正在接近其预配存储配额的非分区集合。 它将建议将这些集合迁移到具有分区密钥定义的新集合，以便服务可以自动将其横向扩展。 [了解有关选择分区键的更多详细信息](https://aka.ms/cosmosdb/choose-partitionkey)
...
Azure Advisor 将标识使用旧版本的 .NET SDK 的 Azure Cosmos DB 帐户，并建议从 Nuget 升级到最新版本，以获得最新的修复、性能改进和新功能功能。 [了解有关宇宙 DB .NET SDK 的更多](https://aka.ms/cosmosdb/sql-api-sdk-dotnet)
...
Azure Advisor 将标识使用旧版本的 Java SDK 的 Azure Cosmos DB 帐户，并建议从 Maven 升级到最新版本，以进行最新修复、性能改进和新功能。 [了解有关宇宙 DB Java SDK 的更多](https://aka.ms/cosmosdb/sql-api-sdk-dotnet)
...
Azure Advisor 将标识使用旧版本的 Cosmos DB Spark 连接器的 Azure Cosmos DB 帐户，并建议从 Maven 升级到最新版本，以进行最新修复、性能改进和新功能功能。 [了解有关宇宙 DB 火花连接器的更多](https://aka.ms/cosmosdb/spark-connector)

## <a name="enable-virtual-machine-replication"></a>启用虚拟机复制
未将复制启用到另一个区域的虚拟机无法抵御区域中断。 复制虚拟机可减少 Azure 区域中断期间的任何不利影响。 Advisor 将检测未启用复制的 VM，并建议启用复制，以便在发生中断时，您可以在远程 Azure 区域中快速启动虚拟机。 [了解有关虚拟机复制的更多](https://docs.microsoft.com/azure/site-recovery/azure-to-azure-quickstart)
