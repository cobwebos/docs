...
你可以使用 Azure 流量管理器配置文件上的[生存时间（TTL）设置](../traffic-manager/traffic-manager-performance-considerations.md)来指定给定终结点停止响应查询时切换终结点的速度。 如果减少 TTL 值，客户端将路由到正常运行的终结点。
