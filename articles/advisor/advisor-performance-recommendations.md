...
### <a name="fix-the-cpu-pressure-of-your-azure-mysql-azure-postgresql-and-azure-mariadb-servers-with-cpu-bottlenecks"></a>修复 Azure MySQL、Azure PostgreSQL 和 Azure MariaDB 服务器的 CPU 压力，并设置 CPU 瓶颈
长时间 CPU 的利用率非常高可能会导致工作负载的查询性能降低。 增加 CPU 大小将有助于优化数据库查询的运行时并提高整体性能。 Azure Advisor 将标识 CPU 利用率高且可能运行 CPU 受限工作负载的服务器，并建议扩展计算。

### <a name="reduce-memory-constraints-on-your-azure-mysql-azure-postgresql-and-azure-mariadb-servers-or-move-to-a-memory-optimized-sku"></a>减少 Azure MySQL、Azure PostgreSQL 和 Azure MariaDB 服务器上的内存限制，或移动到内存优化的 SKU
较低的缓存命中率可能会导致查询性能降低和 IOPS 增加。 这可能是由于查询计划错误或运行内存密集型工作负载造成的。 修复查询计划或增加 PostgreSQL 数据库服务器、Azure MySQL 数据库服务器或 Azure MariaDB 服务器 Azure 数据库的 [内存](https://docs.microsoft.com/azure/postgresql/concepts-pricing-tiers)将有助于优化数据库工作负载的执行。 Azure Advisor 可识别由于此高缓冲区池改动而受影响的服务器，并建议修复查询计划、移动到具有更多内存的更高 SKU 或增加存储大小以获取更多 IOPS。

### <a name="use-a-azure-mysql-or-azure-postgresql-read-replica-to-scale-out-reads-for-read-intensive-workloads"></a>使用 Azure MySQL 或 Azure PostgreSQL 读取副本扩展读取以进行读取密集型工作负荷
Azure Advisor 利用基于工作负载的启发式方法，例如过去七天在服务器上读取与写入的比率，以识别读取密集型工作负载。 用于 PostgreSQL 资源的 Azure 数据库或具有非常高读取/写入比率的 MySQL 资源的 Azure 数据库可能会导致 CPU 和/或内存争用，从而导致查询性能降低。 添加 [副本](https://docs.microsoft.com/azure/postgresql/howto-read-replicas-portal)将有助于向副本服务器扩展读取，防止主服务器上的 CPU 和/或内存限制。 Advisor 将标识具有如此高读取密集型工作负载的服务器，并建议添加 [读取副本](https://docs.microsoft.com/azure/postgresql/concepts-read-replicas) 以卸载某些读取工作负载。
...
与数据库服务器的每个新连接都会占用一些内存。 如果由于内存中的 [上限](https://docs.microsoft.com/azure/postgresql/concepts-limits)而与服务器的连接失败，数据库服务器的性能会降低。 Azure Advisor 将识别运行在许多连接失败的服务器，并建议升级服务器的连接限制，通过扩展计算或使用内存优化 SKU（每个内核的计算更多）为服务器提供更多内存。

## <a name="scale-your-cache-to-a-different-size-or-sku-to-improve-cache-and-application-performance"></a>将缓存缩放为不同的大小或 SKU 以提高缓存和应用程序性能

当缓存实例在高内存压力、服务器负载或高网络带宽下未运行时性能最佳，这可能导致它们无响应、数据丢失或不可用。 Advisor 将识别这些条件下的缓存实例，并建议应用最佳实践以降低内存压力、服务器负载或网络带宽，或扩展到不同大小或容量更大的 SKU。

## <a name="add-regions-with-traffic-to-your-azure-cosmos-db-account"></a>将具有流量的区域添加到 Azure Cosmos 数据库帐户

Advisor 将检测具有当前未配置区域流量的 Azure Cosmos DB 帐户，并建议添加该区域。 这将改善来自该区域的请求的延迟，并确保在区域中断时提供可用性。 [了解有关使用 Azure Cosmos DB 进行全局数据分发的更多情况](https://aka.ms/cosmos/globaldistribution)

## <a name="configure-your-azure-cosmos-db-indexing-policy-with-customer-included-or-excluded-paths"></a>使用客户包含或排除的路径配置 Azure Cosmos DB 索引策略

Azure Advisor 将标识使用默认索引策略但可以从基于工作负载模式的自定义索引策略中受益的 Cosmos DB 容器。 默认索引策略对所有属性进行索引，但使用自定义索引策略，具有查询筛选器中使用的显式包含或排除路径，可以减少用于索引的 R 和存储。 [了解有关修改索引策略的更多](https://aka.ms/cosmosdb/modify-index-policy)

## <a name="configure-your-azure-cosmos-db-query-page-size-maxitemcount-to--1"></a>将 Azure Cosmos DB 查询页大小 (MaxItemCount) 配置为 -1 

Azure Advisor 将标识使用查询页大小为 100 的 Azure Cosmos DB 容器，并建议使用 -1 的页面大小来加快扫描速度。 [了解有关最大项目计数的更多](https://aka.ms/cosmosdb/sql-api-query-metrics-max-item-count)
