...
如果可以在较小的 SKU （位于同一 SKU 系列中）或更小的实例（例如）中容纳当前负载，则顾问会考虑调整虚拟机的大小。
...
顾问会分析你的使用情况，并评估你的 MariaDB、MySQL 或 PostgreSQL 数据库服务器资源在过去7天内是否被广泛利用。 资源利用率较低会导致不需要的支出，而不会对性能产生重大影响。 为了降低成本并有效管理资源，建议将计算大小（Vcore）减少一半。
...
### <a name="synapse-analytics-formerly-sql-data-warehouse-reserved-capacity"></a>Synapse Analytics （以前称为 SQL 数据仓库）保留容量
