...
**子网**流量路由方法允许你将一个最终用户 IP 地址范围集映射到配置文件中的特定终结点。 此后，如果流量管理器收到针对该配置文件的 DNS 查询，则它将检查该请求的源 IP 地址（大多数情况下，这是调用方使用的 DNS 解析程序的传出 IP 地址），确定它映射到哪个终结点，并在查询响应中返回该终结点。 可以将要映射到终结点的 IP 地址指定为 CIDR 范围（例如 1.2.3.0/24）或地址范围（例如 1.2.3.4-5.6.7.8）。 与某个终结点关联的 IP 范围在该配置文件中必须是唯一的，并且不能与同一配置文件中另一个终结点的 IP 地址集重叠。
如果不存在该 IP 地址可以映射到的终结点，则流量管理器将发送 NODATA 响应。 因此，强烈建议你确保在终结点中指定所有可能的 IP 范围。
