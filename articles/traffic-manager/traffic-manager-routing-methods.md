...
对终结点优先级的配置在 ARM API（以及新的 Azure 门户）和 ASM API（以及经典门户）中是按不同方式进行的：
...
- 在 ASM API 中，终结点优先级是根据终结点在配置文件定义中的排列顺序显式配置的。你还可以在 Azure“经典”门户的配置文件的“配置”页上配置故障转移顺序。
...
“加权”流量路由可以通过新的 Azure 门户进行配置，但权重不能通过“经典”门户进行配置。它还可以使用 Azure PowerShell、Azure CLI 和 Azure REST API 通过 ARM 和 ASM 进行配置。
