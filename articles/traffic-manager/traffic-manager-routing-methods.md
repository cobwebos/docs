...
* **[优先级](#priority-traffic-routing-method)：** 如果要对所有流量使用主服务终结点，请选择**优先级**，并在主终结点或备份终结点不可用时提供备份。
* **[加权](#weighted)：** 如果要在一组终结点之间分配流量时选择 **"加权"，** 这些终结点是均匀的，也可以根据您定义的权重进行分配。
* **[性能](#performance)：** 当您在不同地理位置有终结点，并且希望最终用户在最低网络延迟方面使用"最近"终结点时，请选择 **"性能**"。
...
流量管理器读取 DNS 查询的源 IP 地址，并确定它所源自的地理区域。 随后，它会查找并查看是否存在此地理区域所映射到的终结点。 此查找从最低粒度级别（支持它的国家/省，其他在国家/区域级别）开始，并一直持续到最高级别，即**世界**。 使用此遍历找到的第一个匹配项将指定为要在查询响应中返回的终结点。 当与“嵌套”类型终结点匹配时，将基于其路由方法返回该子配置文件中的终结点。 以下几点适用于此行为：
...
* [为什么在尝试将现有配置文件的路由方法更改为地理路由时会出现错误？](https://docs.microsoft.com/azure/traffic-manager/traffic-manager-faqs#why-am-i-getting-an-error-when-i-try-to-change-the-routing-method-of-an-existing-profile-to-geographic)
...
了解如何使用[流量管理器端点监视](traffic-manager-monitoring.md)开发高可用性应用程序
