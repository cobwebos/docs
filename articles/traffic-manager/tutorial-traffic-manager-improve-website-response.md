...
- 在不同的 Azure 区域（**美国东部**和**西欧**）运行的两个基本网站实例。
- 用于测试流量管理器的两个测试 Vm-**美国东部**和第二个 vm 中的一个 vm**西欧**。 测试 VM 用于演示流量管理器如何将用户流量路由到同一区域中运行的网站，并提供最低的延迟。
...
2. 在“创建虚拟机”中，在“基本信息”选项卡中键入或选择以下值：
   ...
   - **实例详细信息** > **区域**：选择 "**美国东部**"。
   - **管理员帐户** > **用户名**：输入所选的用户名。
   - **管理员帐户** > **密码**：输入所选的密码。 密码必须至少 12 个字符长，且符合[定义的复杂性要求](../virtual-machines/windows/faq.md?toc=%2fazure%2fvirtual-network%2ftoc.json#what-are-the-password-requirements-when-creating-a-vm)。
   -  > **公用入站**端口的**入站端口规则**：选择 "**允许所选端口**"。
   - **入站端口规则** > **选择入站端口**：在下拉框中选择 " **RDP**和**HTTP** "。

3. 选择 "**管理**" 选项卡，选择 "**下一步：磁盘**"，然后选择 "下一步：**网络**"，然后单击 "**管理**" 在“监视”下，将“启动诊断”设置为“关闭”。
4. 选择“查看 + 创建”。
5. 查看设置，并单击“创建”。  
...
在本部分中，将在两个 Vm *myIISVMEastUS*和*MYIISVMWESTEUROPE*上安装 IIS 服务器，然后更新默认网站页面。 自定义网站页面显示从 Web 浏览器访问网站时要连接到的 VM 的名称。
...
9. 通过在*myResourceGroupTM2*资源组中创建与 VM *myIISVMWestEurope*的 RDP 连接来重复步骤1-8，以便安装 IIS 并自定义其默认网页。
...
添加运行 IIS 服务器的两个 Vm- *myIISVMEastUS* & *myIISVMWestEurope*将用户流量路由到最靠近用户的终结点。
...
4. 重复步骤2和3，为与名为*myIISVMWestEurope*的 IIS 服务器 VM 关联的公共 IP 地址*myIISVMWestEurope*添加另一个名为 " *myWestEuropeEndpoint* " 的终结点。
...
2. 接下来，使用步骤 1-5 连接到位于“西欧”的 VM *myVMWestEurope*，然后从此 VM 浏览到流量管理器配置文件域名。 由于位于**西欧**的 VM，你现在将被路由到以最接近**西欧**中的 IIS 服务器*myIISVMWestEurope*为宿主的网站。
