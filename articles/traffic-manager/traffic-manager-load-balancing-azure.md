...
1. 在 Azure 门户中，单击"**创建资源** > **网络** > **流量管理器"配置文件** > **"创建**"。
2. 输入以下基本信息：
...
1. 在 Azure 门户中，在左侧窗格中，单击"**创建资源** > **网络** > **应用程序网关**"。
2. 输入有关应用程序网关的下述基本信息：
...
1. 在 Azure 门户中，在左侧窗格中，单击"**创建资源** > **网络** > **负载均衡器**"。
2. 为负载均衡器选择一个名称。
3. 将 **"类型"** 设置为**内部**，并选择适当的虚拟网络和子网，以便负载均衡器驻留在其中。
4. 在**IP 地址分配**下，选择**动态**或**静态**。
5. 在“资源组”下面，选择负载均衡器的资源组****。
6. 在 **"位置**"下，为负载均衡器选择适当的区域。
7. 单击“创建”生成负载均衡器****。
...
4. 输入访问探测时使用的数据库**端口**。
5. 在 **"间隔**"下，指定探测应用程序的频率。
6. 在**不正常阈值**下，指定后端 VM 被视为不正常时必须发生的连续探测失败数。
7. 单击 **"确定"** 以创建探测器。
...
2. 输入负载均衡规则的“名称”****。
3. 选择负载均衡器、**协议**和**端口**的**前端 IP 地址**。
4. 在**后端端口**下，指定要在后端池中使用的端口。
5. 选择在前面步骤中创建的**后端池**和**探测**，对其应用规则。
6. 在 **"会话持久性"** 下，选择您希望会话持久化的方式。
7. 在 **"空闲超时"** 下，指定空闲超时前的分钟数。
