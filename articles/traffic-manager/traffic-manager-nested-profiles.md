...
嵌套式流量管理器配置文件既可以使用 Azure Resource Manager (ARM) 配置，也可以使用 Azure 服务管理 (ASM) REST API、PowerShell cmdlet 和跨平台 Azure CLI 命令来配置。也可以通过 Azure 门户来使用这些配置文件，但“经典”门户除外。
...
流量管理器计费分两部分：终结点运行状况检查以及数百万的 DNS 查询（如需完整的详细信息，请参阅我们的[定价页](/home/features/traffic-manager/#price)）。 嵌套式配置文件的计费方式具体如下：
