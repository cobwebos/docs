<properties 
	pageTitle="访问 Azure Active Directory Graph 信息（Windows 应用商店）| 移动开发人员中心" 
...
8. 在 GetUserInfoController.cs 中，将 `Get` 方法替换为以下方法。此方法将使用 Graph REST API 从 Azure Active Directory 返回用户实体，并要求已授权用户调用该 API。
...
[自定义 API 教程]: mobile-services-dotnet-backend-windows-store-dotnet-call-custom-api
[调用自定义 API]: mobile-services-dotnet-backend-windows-store-dotnet-call-custom-api
[Store Server Scripts]: mobile-services-store-scripts-source-control
[注册以使用 Azure Active Directory 登录名]: mobile-services-how-to-register-active-directory-authentication
...
[在移动服务中配合 AAD 进行基于角色的访问控制]: mobile-services-dotnet-backend-windows-store-dotnet-aad-rbac
