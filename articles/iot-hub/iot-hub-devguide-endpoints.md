---
title: "开发人员指南 - IoT 中心终结点 | Microsoft Docs"
...
## <a name="list-of-iot-hub-endpoints"></a>IoT 中心终结点列表
...
* **设备终结点**。 对于标识注册表中预配的每个设备，IoT 中心会公开设备可用来发送和接收消息的一组终结点：
  ...
  * *检索和更新设备克隆属性*。 设备使用此终结点访问其[设备克隆][lnk-twins]的属性。
  * *接收直接方法请求*。 设备使用此终结点侦听[直接方法][lnk-methods]的请求。
    ...
    设备克隆终结点和方法终结点只能通过 [MQTT v3.1.1][lnk-mqtt] 使用。
* **服务终结点**。 每个 IoT 中心公开应用程序后端可用来与设备通信的一组终结点。 这些终结点目前只能通过 [AMQP][lnk-amqp] 协议公开，通过 HTTP 1.1 公开的方法调用终结点除外。
  
  * *接收设备到云的消息*。 此终结点与 [Azure 事件中心][lnk-event-hubs]兼容。 后端服务可用它来读取由设备发送的所有[设备到云的消息][lnk-d2c]。
  * *发送云到设备的消息和接收送达确认*。 这些终结点可让应用程序后端发送可靠的[云到设备的消息][lnk-c2d]，以及接收相应的传送或过期确认。
...
在 IoT 解决方案中，*现场网关*位于你的设备和 IoT 中心终结点之间。 它通常位于靠近设备的位置。 设备使用设备支持的协议，直接与现场网关通信。 现场网关使用 IoT 中心支持的协议连接到 IoT 中心终结点。 现场网关可以是高度专业化的硬件，也可以是运行完成网关所针对的端到端方案的软件的低能耗计算机。

可以使用 [Azure IoT 网关 SDK][lnk-gateway-sdk]实现现场网关。 此 SDK 提供特定功能，例如从多个设备向 IoT 中心的同一连接上传输数据的多路通信。
