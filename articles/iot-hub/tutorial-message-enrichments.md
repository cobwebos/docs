...
ms.locfileid: "72809693"
---
# <a name="tutorial-using-azure-iot-hub-message-enrichments-preview"></a>教程：使用 Azure IoT 中心消息根据（预览版）
...
在本教程中，你将使用 Azure CLI 来设置资源，其中包括指向两个不同存储容器的两个终结点 **： "** 已设置" 和 "**原始**"。 然后，使用[Azure 门户](https://portal.azure.com)将 message 根据配置为仅应用于使用已扩充存储容器发送到终结点**的消息**。 向 IoT 中心发送消息，该消息路由到两个存储容器。 仅对已**扩充存储容器的终结**点发送的消息进行扩充。

以下是完成本教程所要执行的任务：

**使用 IoT 中心消息根据**
> [!div class="checklist"]
> * 使用 Azure CLI 创建资源--一个 IoT 中心、一个包含两个终结点的存储帐户和路由配置。
> * 使用 Azure 门户配置 message 根据。
...
下载[IoT 设备模拟](https://github.com/Azure-Samples/azure-iot-samples-csharp/archive/master.zip)并将其解压缩。 此存储库包含多个应用程序，其中包括将用于向 IoT 中心发送消息的应用程序。

此下载还包含用于创建用于测试消息根据的资源的脚本。 脚本位于/azure-iot-samples-csharp/iot-hub/Tutorials/Routing/SimulatedDevice/resources/iothub_msgenrichment_cli.azcli. 中。 现在，你可以查看并使用该脚本。 还可以直接从项目复制脚本。

准备好开始测试时，将使用此下载中的设备模拟应用程序将消息发送到 IoT 中心。

## <a name="set-up-and-configure-resources"></a>设置并配置资源

除了创建必要的资源以外，Azure CLI 脚本还会将两个路由配置为单独存储容器的终结点。 有关配置路由的详细信息，请参阅[路由教程](tutorial-routing.md)。 设置资源后，可以使用[Azure 门户](https://portal.azure.com)为每个终结点配置 message 根据，然后继续执行测试步骤。
...
如果尚未执行此操作，请打开[Cloud Shell 窗口](https://shell.azure.com)，并确保将其设置为 Bash。 在解压缩的存储库中打开脚本，使用 Ctrl + A 选择所有该脚本，然后按 Ctrl-c 复制它。 或者，你可以复制以下 CLI 脚本或直接在 Cloud Shell 中打开它。 右键单击命令行并选择 "**粘贴**"，将该脚本粘贴到 "Cloud Shell" 窗口中。 脚本每次运行一条语句。 脚本停止运行后，选择**Enter**以确保它运行最后一个命令。 下面的代码块显示了所使用的脚本，其中注释说明了所执行的操作。
...
此时，将设置所有资源并配置路由。 可以在门户中查看消息路由配置，并设置消息根据，以获取转到已**扩充存储容器的消息**。

### <a name="view-routing-and-configure-the-message-enrichments"></a>查看路由和配置消息根据

1. 通过选择 "**资源组**" 切换到 IoT 中心，然后选择为本教程设置的资源组（**ContosoResources_MsgEn**）。 在列表中找到 IoT 中心并将其选中。 为 Iot 中心选择 "**消息路由**"。
...
2. 将这些值添加到 ContosoStorageEndpointEnriched 终结点的列表。
   ...
   | Msds-devicelocation | $twin 标记。位置 | AzureStorageContainers > ContosoStorageEndpointEnriched |
...
运行控制台应用程序。 稍等几分钟。 正在发送的消息将显示在应用程序的控制台屏幕上。
...
**使用 IoT 中心消息根据**
> [!div class="checklist"]
> * 使用 Azure CLI 创建资源--一个 IoT 中心、一个包含两个 enendpoints 的存储帐户和路由配置。
> * 使用 Azure 门户配置 message 根据。
> * 运行模拟 IoT 设备向中心发送消息的应用。
> * 查看结果，并验证消息根据是否按预期方式工作。
