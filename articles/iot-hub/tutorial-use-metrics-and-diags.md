...
>[!NOTE]
>创建设备标识时，可能会出现以下错误：*未找到 IoT 中心 ContosoTestHub 的策略 iothubowner 的密钥*。 若要修正此错误，请更新 Azure CLI IoT 扩展，然后再次运行脚本中的最后两个命令。 
>
...
3. 确保订阅和资源组正确。 在“资源类型”下取消选中“全选”，然后查找并勾选 IoT 中心”。 （它将复选标记置于再次*选择*"下一步"，只需忽略它。）在 "**资源**" 下，选择中心名称。 屏幕应该如下图所示： 
...
5. 现在针对“已使用的消息总数”设置另一警报。 当使用的消息数接近 IoT 中心的配额时，如果需要系统发送警报来通知你该中心很快就会开始拒绝消息，则可使用此指标。
   ...
   **名称**：提供警报规则的名称，如*使用的消息数*。

   **说明**：提供警报的描述，例如，*在接近配额时发出警报*。
