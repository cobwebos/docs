...
在本课程中，将使用脚本在网关中自动配置和执行 BLE 示例应用程序。 此类应用程序使用模块集合聚合和转换数据，处理命令或执行任意数量的相关任务。 模块通过消息中转站互相通信。 该示例应用程序具有 BLE 模块和 IoT 中心模块。 BLE 模块从 BLE SensorTag 接收数据。 IoT 中心模块通过 Azure IoT 网关 SDK 中提供的网关框架打包接收的数据并将其发送到 IoT 中心。
