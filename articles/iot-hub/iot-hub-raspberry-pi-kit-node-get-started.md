...
1. 下载 Raspbian。
   ...
   a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 [Raspbian Buster with desktop](https://www.raspberrypi.org/downloads/raspbian/)（.zip 文件）。

   b. 将 Raspbian 映像解压缩到计算机的某个文件夹中。

2. 将 Raspbian 安装到 microSD 卡。

   a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 [下载并安装 Etcher SD 卡刻录机实用工具](https://etcher.io/)。
...
1. 从主计算机使用下列 SSH 客户端之一连接到 Raspberry Pi：
   ...
   **Windows 用户**
   ...
   a.在“解决方案资源管理器”中，右键单击项目文件夹下的“引用”文件夹，并单击“添加引用”。 下载并安装 [PuTTY](https://www.putty.org/) for Windows。
...
1. 通过运行以下命令，打开配置文件：
   ...
   *注意：默认情况下，本教程中使用的0x77。根据你的配置，它也可能是0x76 的：如果遇到 i2c 错误，请尝试将此值更改为118，并查看其是否更好。若要查看传感器使用的地址，请在 raspberry pi 上的 shell 中运行 `sudo i2cdetect -y 1`*
