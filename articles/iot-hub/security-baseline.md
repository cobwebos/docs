---
title: Azure IoT 中心的 azure 安全基线
...
Microsoft Azure IoT 中心的 Azure 安全基线包含的建议可帮助你提高部署的安全状况。 此服务的基线摘自 [Azure 安全基准版本 1.0](../security/benchmarks/overview.md)，其中提供了有关如何根据我们的最佳做法指导保护 Azure 上的云解决方案的建议。 有关详细信息，请参阅 [Azure 安全基线概述](../security/benchmarks/security-baselines-overview.md)。
...
**指南**：默认情况下，IoT 中心的主机名映射到一个公共终结点，该终结点具有 internet 上可公开路由的 IP 地址。 不同的客户将共享此 IoT 中心公共终结点，广域网和本地网络中的 IoT 设备均可对其进行访问。

IoT 中心的功能，包括消息路由、文件上传和批量设备导入/导出，还需要配置有从 IoT 中心到客户拥有的 Azure 资源的连接（通过其公共终结点）。 这些连接路径上的流量共同构成了从 IoT 中心到客户资源的出口流量。

建议限制与 azure 资源的连接 (包括 Azure IoT 中心) 通过你拥有和操作的虚拟网络在隔离的网络中减少连接性，并直接启用到 Azure 主干网络的本地网络连接。 使用 Azure 专用链接和 Azure 专用终结点（如果可行），以便能够从其他虚拟网络对服务进行专用访问。

将设备中的开放硬件端口保持在最低限度，以避免不需要的访问。 此外，还构建了阻止或检测设备物理篡改的机制。
...
**指南**：阻止具有 IOT 中心 IP 筛选规则的已知恶意 ip。 还会通过用于 IoT 的 Azure 安全中心来记录和发出恶意尝试。

Azure DDoS 保护基本版已启用，并且在 IoT 中心中不会产生额外的成本。 始终在流量监视和常见网络级别攻击的实时缓解措施下，提供 Microsoft 联机服务所利用的相同防御。 整个 Azure 全球网络的规模可用于跨区域分散和缓解攻击流量。
...
**指南**：不适用;此建议适用于生成可由客户记录和查看的网络数据包的产品。 IoT 中心不会生成面向客户的网络数据包，并且不会直接部署到 Azure 虚拟网络。
...
**指南**：对于需要访问 Azure IoT 中心的资源，可以使用虚拟网络服务标记来定义网络安全组或 Azure 防火墙上的网络访问控制。 创建安全规则时，可以使用服务标记代替特定的 IP 地址。 通过在规则的相应 "源" 或 "目标" 字段中指定服务标记名称 (例如 AzureIoTHub) ，可以允许或拒绝相应服务的流量。 Microsoft 会管理服务标记包含的地址前缀，并会在地址发生更改时自动更新服务标记。
...
**指南**：定义和实现与 Azure IoT 中心命名空间和 azure 策略关联的网络资源的标准安全配置。 使用 "Microsoft 设备" 和 "Microsoft 网络" 命名空间中的 Azure 策略别名创建自定义策略，以便审核或强制实施机器学习命名空间的网络配置。 
...
**指南**：对与 Azure IoT 中心部署关联的网络资源使用标记，以便以逻辑方式将它们组织到分类。
...
**指南**： Microsoft 在日志中维护用于 Azure IoT 中心等 azure 资源时间戳的时间源。
...
**指南**：通过 Azure Monitor 引入日志来聚合由 Azure IoT 中心生成的安全数据。 在 Azure Monitor 中，使用 Log Analytics 工作区查询和执行分析，并使用存储帐户进行长期/存档存储。 或者，你可以将和机载数据启用到 Azure Sentinel 或第三方安全事件和事件管理 (SIEM) 。
...
**指南**：在 azure 资源上启用 azure IoT 诊断设置，以便访问审核、安全和诊断日志。 活动日志自动可用，包括事件源、日期、用户、时间戳、源地址、目标地址和其他有用元素。
...
**指南**：在 Azure Monitor 中，根据组织的符合性规定，设置与 Azure IoT 中心实例相关联的 Log Analytics 工作区的日志保持期。
...
**指南**：将 Azure 安全中心用于 IoT 与 Log Analytics 工作区，用于监视和警报安全日志和事件中的异常活动。 或者，你可以将和机载数据启用到 Azure Sentinel。 你还可以使用可能会对安全产生影响的 Azure Monitor （例如，流量意外下降时）定义操作警报。
...
**指南**：不适用;Azure IoT 中心不会处理或产生与反恶意软件相关的日志。
...
**指南**：不适用;Azure IoT 中心不会处理或产生与 DNS 相关的日志。
...
**指南**： azure RBAC) 的 azure 基于角色的访问控制 (允许通过角色分配来管理对 Azure IoT 中心的访问。 你可以将这些角色分配给用户、组服务主体和托管标识。 某些资源有预定义的内置角色，可以通过 Azure CLI、Azure PowerShell 或 Azure 门户等工具来清点或查询这些角色。 
...
**指南**：对 Azure IoT 中心资源的访问管理是通过) Azure Active Directory (Azure AD 控制的。 Azure AD 没有默认密码。
...
**指南**：启用 Azure AD MFA，以保护你的整体 Azure 租户，并受益于所有服务。 IoT 中心服务不支持 MFA。  
...
**指南**：对于访问 IoT 中心的用户，不支持条件访问。 若要缓解这种情况，请使用 Azure AD 命名位置，以仅允许从 IP 地址范围或国家/地区的特定逻辑分组访问整个 Azure 租户，受益所有服务（包括 IoT 中心）。 
...
**指南**：若要获取用户对 IoT 中心的访问权限，请使用 Azure Active Directory (Azure AD) 作为中心身份验证和授权系统。 Azure AD 通过对静态数据和传输中数据使用强加密来保护数据。 Azure AD 还会对用户凭据进行加盐、哈希处理和安全存储操作。
...
用户 Azure Monitor 诊断日志来监视连接类别中的未经授权的连接尝试。
...
**指南**：在 Microsoft 需要访问客户数据的支持方案中，将直接从客户请求该数据。 
...
**指南**：对于控制面用户访问 IoT 中心，请使用 Azure RBAC 来控制访问权限。 对于 iot 中心的数据平面访问权限，请为 IoT 中心使用共享访问策略。
...
Microsoft 管理 Azure IoT 中心的底层基础结构，并实施了严格控制以防止客户数据丢失或泄露。
...
**指南**： IoT 中心支持静态数据加密与客户托管的密钥 (CMK) ，也称为 "自带密钥" (BYOK) 。

Azure IoT 中心提供静态数据和传输中的数据加密，因为它是在数据中心内编写的，并在你访问数据时对其进行解密。 默认情况下，IoT 中心使用 Microsoft 托管的密钥来加密静态数据。
...
**指南**：将 Azure Monitor 与 azure 活动日志结合使用，为 Azure IoT 中心的生产实例以及其他关键或相关资源的发生更改创建警报。
...
**指南**：不适用;Microsoft 对支持 Azure IoT 中心的基础系统执行漏洞管理。
...
**指南**：不适用;Microsoft 对支持 Azure IoT 中心的基础系统执行修补程序管理。 
...
**指南**：将标记应用于 Azure 资源 (并非所有资源都支持标记，但大多数情况下) 以逻辑方式将它们组织到分类。
...
每个 IoT 中心都有一个标识注册表，可用于在服务中创建每设备资源。 可将单个或一组设备标识添加到允许列表或方块列表，以便完全控制设备访问。
...
**指南**：如果对 Azure IoT 中心或相关资源使用自定义 azure 策略定义，请使用 Azure Repos 安全地存储和管理代码。
...
**指南**：使用 "Microsoft 设备" 命名空间中的 Azure 策略别名创建自定义策略，以对系统配置进行警报、审核和强制执行。 另外，开发一个用于管理策略例外的流程和管道。
...
**指南**：在支持 azure 服务的基础主机上启用了 Microsoft 反恶意软件 (例如，Azure IoT 中心) ，但它不会在客户内容上运行。
...
**指南**：在 Key Vault 中启用软删除和清除保护，以防止意外或恶意删除密钥。 如果将 Azure 存储用于存储备份，请启用软删除以在 blob 或 blob 快照被删除时保存和恢复数据。
...
- [ 使用标记来组织 Azure 资源](/azure/azure-resource-manager/resource-group-using-tags)
