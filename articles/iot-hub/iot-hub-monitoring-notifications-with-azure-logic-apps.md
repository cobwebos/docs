...
到达 IoT 中心的消息如下所示, 正文中包含的遥测数据和应用程序属性中包含的`temperatureAlert`属性 (系统属性未显示):
...
若要了解有关 IoT 中心消息格式的详细信息, 请参阅[创建和读取 Iot 中心消息](iot-hub-devguide-messages-construct.md)。

在本主题中, 将在 IoT 中心设置路由, 以便将`temperatureAlert` `true`属性发送到服务总线终结点的消息。 然后, 设置一个逻辑应用, 该应用在到达服务总线终结点的消息上触发, 并向你发送电子邮件通知。
