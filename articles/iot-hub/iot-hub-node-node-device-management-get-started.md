...
* 创建一个具有直接方法的模拟设备应用，以便可以通过云调用该方法进行重启。
* 创建一个控制台应用程序，以便通过 IoT 中心调用模拟设备应用中的重启直接方法。
...
2. 在 **triggerrebootondevice** 文件夹的命令提示符处，运行下述命令以安装 **azure-iothub** 设备 SDK 包和 **azure-iot-device-mqtt** 包：
    ...
    ```
3. 在 **triggerrebootondevice** 文件夹中，使用文本编辑器创建新的 **dmpatterns_getstarted_service.js** 文件。
...
在本教程中，使用直接方法来触发设备的远程重启，使用报告的属性来报告设备上次重启时间，并查询设备克隆，从云中发现设备上次重启的时间。
