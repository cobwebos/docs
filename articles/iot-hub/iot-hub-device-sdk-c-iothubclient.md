...
当你调用 **IoTHubClient\_CreateFromConnectionString** 时，基础库将创建在后台运行的新线程。 此线程将事件发送到 IoT 中心以及从 IoT 中心接收消息。 使用 **LL** API 时不会创建此类线程。 创建后台线程是为了给开发人员提供方便。 您不必担心显式发送事件和接收来自 IoT 中心的消息 -- 它会自动在后台发生。 相比之下，**LL** API 可让你根据需要显式控制与 IoT 中心的通信。
...
**LL** API 不创建后台线程。 必须调用一个新 API 明确地与 IoT 中心之间相互发送和接收数据。 以下示例就此进行演示。

SDK 中包含的**iothub\_\_客户端示例\_http**应用程序演示了较低级别的 API。 在该示例中，使用如下代码将事件发送到 IoT 中心：
...
**\_IoTHubClient\_LL DoWork**经常在一个循环中调用的原因是，每次调用它时，它都会向 IoT 中心发送*一些*缓冲事件，并检索为设备排队的下*一*条消息。 不能保证每个调用发送所有缓冲事件或检索所有排队的消息。 如果想要发送缓冲区中的所有事件，并继续进行其他处理，可以使用如下代码来替换此循环：
...
此代码将一直调用 **IoTHubClient\_LL\_DoWork**，直到缓冲区中的所有事件都发送到 IoT 中心为止。 请注意，这并不表示收到所有已排队的消息。 部分原因在于，检查"所有"消息并不是确定性操作。 如果检索了“所有”消息，但随即将另一个消息发送到设备，会发生什么情况？ 更好的处理方法是使用设定的超时。 例如，消息回调函数可以在每次调用计时器时重置计时器。 例如，如果在过去 *X* 秒内未收到任何消息，可以接着编写逻辑来继续处理。

完成入侵事件和接收消息后，请确保调用相应的函数来清理资源。

```C
IoTHubClient_LL_Destroy(iotHubClientHandle);
```

基本上，只有一组 API 用于使用后台线程发送和接收数据，而另一组 API 在没有后台线程的情况下执行相同的工作。 许多开发人员可能更喜欢使用非 LL API，但是当他们想要明确控制网络传输时，较低级别 API 就很有用。 例如，有些设备会收集各时间段的数据，并且只按指定的时间间隔引入事件（例如，每小时一次或每天一次）。 较低级别 API 可以在与 IoT 中心之间发送和接收数据时提供明确控制的能力。 还有一些人纯粹偏好较低级别 API 提供的简单性。 所有操作都发生在主线程上，而不是有些操作在后台发生。
...
在前面的示例中，我们将属性附加到发送到 IoT 中心的事件。 属性也可以附加到从 IoT 中心接收的消息。 如果想要从消息检索属性，可以在消息回调函数中使用如下所示的代码：
...
如前所述，当 IoT 中心发出的消息抵达时， **IoTHubClient** 库将调用注册的回调函数来做出响应。 有必要进一步了解此函数的一个返回参数。 下面是**iothub\_\_客户端示例\_http**示例应用程序中回调函数的摘录：
...
在最后一个案例中，消息也会发送到 IoT 中心，但指示应重新传送消息。 通常，如果遇到一些错误，但希望再次尝试处理该消息，则会放弃邮件。 相反，当您遇到不可恢复的错误（或者如果您只是决定不想处理消息）时，拒绝消息是合适的。
...
此字符串中有四条信息：IoT 中心名称、IoT 中心后缀、设备 ID 和共享访问密钥。 当在 Azure 门户中创建 IoT 中心实例时，可以获取 IoT 中心的完全限定域名 (FQDN) - 它提供了 IoT 中心名称（FQDN 的第一个部分）和 IoT 中心后缀（FQDN 的其余部分）。 当您向 IoT 中心注册设备时，您将获得设备 ID 和共享访问密钥（如[上一篇文章中](iot-hub-device-sdk-c-intro.md)所述）。
...
batching 选项非常重要。 默认情况下，库将逐个引入事件（单个事件是你传递给 **IoTHubClient\_LL\_SendEventAsync** 的任何内容）。 如果 batching 选项为 **true**，库将尽可能多地从缓冲区中收集事件（上限为 IoT 中心接受的消息大小上限）。  事件批在单个 HTTPS 调用中发送到 IoT 中心（单个事件已捆绑到 JSON 数组中）。 启用批处理通常会导致性能的大幅提升，因为您减少了网络往返。 不过，这也会明显减少带宽，你要在事件批中发送一组 HTTPS 标头，而不是针对每个事件发送一组标头。 除非您有特定原因这样做，否则通常需要启用批处理。
