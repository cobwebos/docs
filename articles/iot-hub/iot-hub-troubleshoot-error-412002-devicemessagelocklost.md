...
当设备从队列收到云到设备的消息时（例如，使用[`ReceiveAsync()`](https://docs.microsoft.com/dotnet/api/microsoft.azure.devices.client.deviceclient.receiveasync?view=azure-dotnet)），该消息由 IoT 中心锁定，锁定超时持续时间为一分钟。 如果在锁定超时过期后设备尝试完成消息，则 IoT 中心会引发此异常。
