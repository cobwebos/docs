...
通过面向服务的终结点 */消息/设备绑定*，发送云到设备的消息。 然后，设备通过特定于设备的终结点 */设备/[设备 Id]/消息/设备绑定*接收消息。
...
当 IoT 中心服务向设备发送消息时，服务将消息状态设置为 *"排队*"。 当设备想要接收某条消息时，IoT 中心会通过将状态设置为“不可见”来锁定该消息。****** 这种状态使得设备上的其他线程可以开始接收其他消息。 当设备线程完成消息的处理后，会通过完成消息来通知 IoT 中心**。 随后 IoT 中心会将状态设置为“已完成”**。
...
| StatusCode         | 必需的字符串，在 IoT 中心生成的反馈消息中使用： <br/> *成功* <br/> *过期* <br/> *超过交货数量* <br/> *拒绝* <br/> *清除* |
| 描述        | *状态代码*的字符串值 |
...
| properties                  | 描述 | 范围和默认值 |
| ------------------------- | ----------- | ----------------- |
| defaultTtlAsIso8601       | 云到设备消息的默认 TTL | ISO_8601间隔可达2天（至少1分钟）;默认值： 1 小时 |
| maxDeliveryCount          | 每个设备队列的云到设备最大传送计数 | 1 到 100;默认值： 10 |
| feedback.ttlAsIso8601     | 服务绑定反馈消息的保留时间 | ISO_8601间隔可达2天（至少1分钟）;默认值： 1 小时 |
| feedback.maxDeliveryCount | 反馈队列的最大传送计数 | 1 到 100;默认值： 10 |
| 反馈.锁持续时间Iso8601 | 反馈队列的最大传送计数 | ISO_8601间隔从 5 秒到 300 秒（至少 5 秒）;默认值：60 秒。 |

您可以通过以下方式之一设置配置选项：

* **Azure 门户**：在 IoT 中心的 **"设置"** 下，选择**内置终结点**并将**云扩展到设备消息传送**。 （设置**反馈.maxDeliveryCount**和**反馈. lockDurationAsIso8601**属性当前不支持 Azure 门户中。

    ![为门户中的云到设备消息设置配置选项](./media/iot-hub-devguide-messages-c2d/c2d-configuration-portal.png)
...
若要尝试接收云到设备的消息，请参阅[发送云到设备](iot-hub-csharp-csharp-c2d.md)教程。
