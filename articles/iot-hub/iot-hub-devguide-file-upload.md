...
要使用文件上传功能，必须首先将 Azure 存储帐户链接到 IoT 中心。 您可以通过 Azure 门户或通过[IoT 中心资源提供程序 REST API](/rest/api/iothub/iothubresource)以编程方式完成此任务。 将 Azure 存储帐户与 IoT 中心关联后，当设备启动文件上传请求时，此服务将向该设备返回 SAS URI。
...
您可以使用 Azure 门户、Azure CLI 或 PowerShell 在 IoT 中心设置这些属性。 要了解如何，请参阅[配置文件上载](iot-hub-configure-file-upload.md)下的主题。
