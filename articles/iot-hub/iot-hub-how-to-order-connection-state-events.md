---
title: 订购设备连接事件 fr Azure IoT 中心（带 Azure 宇宙数据库）
...
* 有效的 Azure 帐户。 如果没有，您可以创建[一个免费帐户](https://azure.microsoft.com/pricing/free-trial/)。
...
1. 在 Cosmos DB SQL API 中，选择**数据资源管理器** > **项** > **"新的存储过程**"。
...
4. 填写**事件订阅详细信息**：提供描述性名称并选择**事件网格架构**。
...
可以使用 Azure CLI 完成 IoT 中心步骤，而不是使用[Azure 门户](https://portal.azure.com)。 有关详细信息，请参阅有关使用 Azure CLI [创建事件订阅](https://docs.microsoft.com/cli/azure/eventgrid/event-subscription)和[创建 IoT 设备](/cli/azure/ext/azure-cli-iot-ext/iot/hub/device-identity#ext-azure-cli-iot-ext-az-iot-hub-device-identity-create)的网页。
