...
对的调用 `CompleteAsync()` 通知 IoT 中心消息已成功处理，并且可以安全地从设备队列中删除该消息。 当设备的处理成功完成，而不考虑它使用的协议时，设备应调用此方法。
...
> [!NOTE]
> 使用 HTTPS 而不使用 MQTT 或 AMQP 作为传输时，`ReceiveAsync` 方法将立即返回。 使用 HTTPS 的云到设备消息的支持模式是间歇性连接的设备，这些设备在每隔25分钟) 不经常检查消息 (。 发出更多 HTTPS 接收会导致 IoT 中心限制请求。 有关 MQTT、AMQP 和 HTTPS 支持之间的差异的详细信息，请参阅 [云到设备通信指南](iot-hub-devguide-c2d-guidance.md) 和 [选择通信协议](iot-hub-devguide-protocols.md)。
>
...
1. 按 **F5** 运行应用。 应会看到两个应用程序都会启动。 选择“SendCloudToDevice”窗口并按“Enter” 。 应会看到设备应用正在接收的消息，几秒钟后，**SendCloudToDevice** 应用程序将收到反馈消息。
   ...
   ![接收消息和服务应用接收反馈的设备应用](./media/iot-hub-csharp-csharp-c2d/sendc2d2.png)
