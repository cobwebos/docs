...
对 `CompleteAsync()` 的调用将通知 IoT 中心，指出已成功处理消息。 可以安全地从设备队列中删除该消息。 如果因故导致设备应用无法完成消息处理作业，IoT 中心将再传递一次。 设备应用中的消息处理逻辑必须是幂等的，以便多次接收相同的消息会生成相同的结果。
