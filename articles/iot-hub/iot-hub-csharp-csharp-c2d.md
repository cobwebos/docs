...
本教程的基础[是将遥测数据从设备发送到 IoT 中心](quickstart-send-telemetry-dotnet.md)。 它展示了如何执行以下步骤：
...
* **SimulatedDevice**, 在[将遥测从设备发送到 iot 中心时](quickstart-send-telemetry-dotnet.md)创建的应用的修改版本, 它连接到 iot 中心并接收云到设备的消息。

* **SendCloudToDevice**, 它通过 IoT 中心将云到设备的消息发送到设备应用, 然后接收其传递确认。
...
在本部分中, 将修改在[将遥测从设备发送到 iot 中心](quickstart-send-telemetry-dotnet.md)中创建的设备应用, 以接收来自 iot 中心的云到设备消息。
...
在本文中, 你将创建一个后端服务, 通过在将[遥测从设备发送到 iot 中心](quickstart-send-telemetry-dotnet.md)中创建的 iot 中心发送云到设备的消息。 若要发送云到设备的消息, 服务需要**服务连接**权限。 默认情况下, 每个 IoT 中心都创建有一个名为 "**服务**" 的共享访问策略, 该策略将授予此权限。
...
4. 在 Program.cs 文件`using`的顶部添加以下语句。
...
6. 将以下方法添加到 **Program** 类。 在将[遥测数据从设备发送到 IoT 中心](quickstart-send-telemetry-dotnet.md)时, 将设备名称设置为所用的名称。
...
2. 在**Main**方法中, 在行的`serviceClient = ServiceClient.CreateFromConnectionString(connectionString)`后面添加以下方法。

   ``` csharp
   ReceiveFeedbackAsync();
   ```

3. 若要请求针对传递云到设备消息的反馈，必须在 **SendCloudToDeviceMessageAsync** 方法中指定一个属性。 在行的`var commandMessage = new Message(...);`后面添加以下行。
