---
title: "IoT 中心开发人员指南主题 | Microsoft Docs"
description: "Azure IoT 中心开发人员指南，其中介绍了 IoT 中心终结点、安全性、标识注册表、设备管理和消息传送"
services: iot-hub
...
Azure IoT 中心是一项完全托管的服务，有助于在数百万个设备和一个应用程序后端之间实现安全可靠的双向通信。

Azure IoT 中心提供：

* 使用每个设备的安全凭据和访问控制来保护通信安全。
* 多个从设备到云和从云到设备的超大规模通信选项。
* 每个设备的状态信息和元数据的可查询存储。
* 通过最流行语言和平台的设备库来方便建立设备连接。

本 IoT 中心开发人员指南包括以下文章：

* [使用 IoT 中心发送和接收消息][devguide-messaging]说明 IoT 中心公开的消息传送功能（从设备到云和从云到设备）。 此文章还包括以下主题的相关信息：消息格式和支持的通信协议以及这些协议使用的端口号等。
* [从设备到云通信指南][lnk-d2c-guidance]可帮助用户在从设备到云的消息、设备克隆的报告属性和文件上传之间做出选择。
* [从云到设备通信指南][lnk-c2d-guidance]可帮助用户在直接方法、设备克隆的所需属性和从云到设备的消息之间做出选择。
...
* [使用设备克隆同步状态和配置][devguide-device-twins]说明*设备克隆*概念，以及它公开的功能，如使用设备克隆来同步设备。 此文章包括有关设备克隆中存储的数据的信息。
...
* [参考 - 设备克隆和作业的 IoT 中心查询语言][devguide-query]说明了可用于从中心检索设备克隆和作业信息的 IoT 中心查询语言。
...
* [参考 - 设备和服务 SDK][devguide-sdks] 列出了在开发与 IoT 中心交互的设备和服务应用程序时可以使用的 Azure IoT SDK。 此文章包括指向联机 API 文档的链接。
