...
* 设备到云的消息分区存放到创建时设置的一组固定数据分区中（请参阅[Device-to-cloud configuration options（设备到云的配置选项）](#d2cconfiguration)。
* 与事件中心类似，读取设备到云消息的客户端必须处理数据分区和检查点。请参阅 [Event Hubs - Consuming events（事件中心 - 使用事件）][lnk-event-hubs-consuming-events]。
...
* IoT 中心的缩放方式与事件中心稍有不同。有关详细信息，请参阅 [Scaling IoT Hub（缩放 IoT 中心）][lnk-guidance-scale]。

请注意，这并不表示在所有情况下都能使用 IoT 中心代替事件中心。例如，在某些事件处理计算中，可能需要在分析数据流之前，根据不同属性或字段重新分区事件。在这种情况下，你可以使用事件中心来减少流处理管道的两个部分。有关详细信息，请参阅 [Azure Event Hubs Overview（Azure 事件中心概述）][lnk-eventhub-partitions]中的 Partitions（分区）。

有关如何使用设备到云的消息传送的详细信息，请参阅 [IoT Hub APIs and SDKs（IoT 中心 API 和 SDK）][lnk-apis-sdks]。
...
有关这类消息的最佳处理方式，请参阅 [Device-to-cloud processing（设备到云的处理）][lnk-guidance-d2c-processing]。
...
有关云到设备消息的教程，请参阅 [Azure IoT 中心云到设备的消息入门][lnk-getstarted-c2d-tutorial]。有关不同 API 和 SDK 如何公开云到设备功能的参考主题，请参阅 [IoT Hub APIs and SDKs（IoT 中心 API 和 SDK）][lnk-apis-sdks]。
