...
Azure IoT 中心是一项完全托管的服务，有助于在数百万台设备和单个解决方案后端之间实现安全可靠的双向通信。 [向 IoT 中心从设备发送遥测数据](quickstart-send-telemetry-node.md)快速入门介绍如何创建 IoT 中心、 预配设备标识，以及编写模拟的设备应用发送设备到云消息。

[!INCLUDE [iot-hub-basic](../../includes/iot-hub-basic-whole.md)]

本教程基于[向 IoT 中心从设备发送遥测数据](quickstart-send-telemetry-node.md)。 其中了说明了如何：
...
* **SimulatedDevice**，在中创建的应用的修改的版本[向 IoT 中心从设备发送遥测数据](quickstart-send-telemetry-node.md)，它连接到 IoT 中心并接收云到设备的消息。

* **SendCloudToDeviceMessage**，它将云到设备消息发送到模拟的设备应用通过 IoT 中心，然后接收其传送确认。
...
在本部分中，会修改中创建的模拟的设备应用[向 IoT 中心从设备发送遥测数据](quickstart-send-telemetry-node.md)以接收来自 IoT 中心设备到云消息。
...
2. 修改 **connectCallback** 函数以处理从 IoT 中心发送的消息。 在本示例中，设备始终调用 **complete** 函数，以通知 IoT 中心它已处理消息。 **connectCallback** 函数的新版本类似于以下片段：
   ...
   > [!NOTE]
   > 如果使用 HTTPS（而不使用 MQTT 或 AMQP）作为传输，则 DeviceClient  实例将不会频繁（频率低于每 25 分钟一次）检查 IoT 中心发来的消息。 有关 MQTT、 AMQP 和 HTTPS 支持，以及 IoT 中心限制之间的差异的详细信息，请参阅[IoT 中心开发人员指南](iot-hub-devguide-messaging.md)。
   >

## <a name="send-a-cloud-to-device-message"></a>发送云到设备的消息

在本部分中，创建一个 Node.js 控制台应用程序，它将云到设备的消息发送到模拟设备应用程序。 需要在添加的设备的设备 ID[向 IoT 中心从设备发送遥测数据](quickstart-send-telemetry-node.md)快速入门。 还需要中心的 IoT 中心连接字符串（位于 [Azure 门户](https://portal.azure.com)）。
...
5. 将以下代码添加到 **SendCloudToDeviceMessage.js** 文件。 "{Iot hub connection string}"占位符值替换为中创建的中心的 IoT 中心连接字符串[向 IoT 中心从设备发送遥测数据](quickstart-send-telemetry-node.md)快速入门。 "{Device id}"占位符替换为在添加的设备的设备 ID[向 IoT 中心从设备发送遥测数据](quickstart-send-telemetry-node.md)快速入门：
