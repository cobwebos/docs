...
* **模拟设备**，在[将遥测数据从设备发送到 IoT 中心](quickstart-send-telemetry-node.md)中创建的应用程序的修改版本，该中心连接到 IoT 中心并接收云到设备的消息。
...
1. 使用文本编辑器，打开**模拟设备.js**文件。 此文件位于 **iot-hub\Quickstarts\simulated-device** 文件夹中，该文件夹位于在[从设备将遥测数据发送到 IoT 中心](quickstart-send-telemetry-node.md)快速入门中下载的 Node.js 示例代码的根文件夹中。
...
2. 将一个处理程序注册到设备客户端，用于接收从 IoT 中心发送的消息。 将对 `client.on` 的调用直接添加到创建设备客户端的行后面，如以下代码片段所示：
   ...
   > [!NOTE]
   > 如果使用 HTTPS（而不使用 MQTT 或 AMQP）作为传输，则 DeviceClient**** 实例将不会频繁（频率低于每 25 分钟一次）检查 IoT 中心发来的消息。 有关 MQTT、AMQP 和 HTTPS 支持之间的差异，以及 IoT 中心限制的详细信息，请参阅 [IoT 中心开发人员指南](iot-hub-devguide-messaging.md)。
   >
...
1. 在**模拟设备**文件夹中的命令提示符下，运行以下命令以向 IoT 中心发送遥测数据并侦听云到设备的消息：
