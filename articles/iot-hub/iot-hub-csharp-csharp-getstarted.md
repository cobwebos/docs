...
> [!NOTE]
> 有关各种 SDK 的信息（你可以使用这些 SDK 构建可在设备和解决方案后端运行的应用程序），请参阅 [IoT 中心 SDK][lnk-hub-sdks]。
> 
> 
...
## 创建设备标识
在本部分中，将创建一个 Windows 控制台应用程序，用于在 IoT 中心的标识注册表中创建设备标识。设备无法连接到 IoT 中心，除非它在设备标识注册表中具有条目。有关详细信息，请参阅 [IoT 中心开发人员指南][lnk-devguide-identity]中的“设备标识注册表”部分。当你运行此控制台应用时，它将生成唯一的设备 ID 和密钥，当设备向 IoT 中心发送设备到云的消息时，可以用于标识设备本身。
...
## 接收设备到云的消息
在本部分中，将创建一个 Windows 控制台应用程序，用于读取来自 IoT 中心的设备到云消息。IoT 中心公开与 [Azure 事件中心][lnk-event-hubs-overview]兼容的终结点，以让你读取设备到云的消息。为了简单起见，本教程创建的基本读取器不适用于高吞吐量部署。若要了解如何大规模处理设备到云的消息，请参阅[处理设备到云的消息][lnk-process-d2c-tutorial]教程。有关如何处理来自事件中心消息的更多信息，请参阅[事件中心入门][lnk-eventhubs-tutorial]教程。（本教程适用于 IoT 中心上与事件中心兼容的终结点）。
...
6. 将以下方法添加到 **Program** 类：
    ...
    此方法使用 **EventHubReceiver** 实例接收来自所有 IoT 中心设备到云接收分区的消息。请注意在创建 **EventHubReceiver** 对象时传递 `DateTime.Now` 参数的方式，使它只收到它启动后发送的消息。此筛选器在测试环境中非常有用，因为这样可以看到当前的消息集。在生产环境中，代码应确保处理所有消息。有关详细信息，请参阅[如何处理 IoT 中心设备到云的消息][lnk-process-d2c-tutorial]教程。
7. 最后，在 **Main** 方法中添加以下行：
...
3. 在“**NuGet 包管理器**”窗口中，选择“**浏览**”，搜索 **Microsoft.Azure.Devices.Client**，选择“**安装**”以安装 **Microsoft.Azure.Devices.Client** 包，然后接受使用条款。该过程将下载、安装 [Azure IoT - 设备 SDK NuGet 包][lnk-device-nuget]及其依赖项并添加对它的引用。
4. 在 **Program.cs** 文件顶部添加以下 `using` 语句：
...
## 后续步骤
在本教程中，你已在门户中配置了 IoT 中心，并随后在中心的标识注册表中创建了设备标识。你已使用此设备标识来让模拟设备应用向中心发送设备到云的消息，并创建了用于显示中心所接收消息的应用。
