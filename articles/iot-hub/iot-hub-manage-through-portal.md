...
可以从 4 个层中做选择：**免费**、**标准 1**、**标准 2** 和**标准 S3**。免费层只允许 500 台设备连接到 IoT 中心，并且每天最多传输 8,000 条信息。
...
- 单击“添加”打开边栏选项卡，可在其中输入新的策略名称以及想要与此策略关联的权限，如下图所示。
 	...
 	**服务连接**策略可授予访问云端终结点的权限（例如，连接到 IoT 中心的服务使用者组），而**设备连接**策略可授予在 IoT 中心的设备端终结点上发送和接收消息的权限。
...
**SAS TTL**：此设置是 IoT 中心返回给设备的 SAS URI 生存时间。默认设置为 1 小时，但可以使用滑块自定义为其他值。

**文件通知设置默认 TTL**：文件上传通知到期前的生存时间。默认设置为 1 天，但可以使用滑块自定义为其他值。
