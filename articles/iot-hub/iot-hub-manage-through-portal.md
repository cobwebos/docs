...
可以从 3 个层中做选择：**免费**、**标准 1** 和**标准 2**。免费层只允许 10 个设备连接到 IoT 中心。

**S1（低频率）**：IoT 中心 S1（低频率）版专为具有大量设备，但每台设备生成的数据量相对较少的 IoT 解决方案而设计。每个单位的 S1（低频率）版可连接最多 500 台设备或每天在所有连接的设备上传输最多 50,000 条消息。

**S2（高频率）**：IoT 中心 S2（高频率）版专为设备生成大量数据的 IoT 解决方案而设计。每个单位的 S2（高频率）版可连接最多 500 台设备或每天在所有连接的设备之间传输最多 150 万条消息。
...
一个 IoT 中心单位包含 500 个设备，因此选择 IoT 单位数目表示此单位支持的设备总数等于单位数乘以 500。例如，如果想要 IoT 中心支持 1000 个设备，请选择 2 个单位。
...
只有在当天发送的消息数目不冲突时，才允许从较高层 (S2) 转移较低层 (S1)。例如，如果每天的消息数目超过 50,000，则 IoT 中心无法从 S2 层更改为 S1。
