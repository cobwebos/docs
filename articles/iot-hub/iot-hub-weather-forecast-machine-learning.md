---
title: 使用 Azure 机器学习与 IoT 中心数据进行天气预报
...
- [Azure 机器学习工作室（经典）](https://studio.azureml.net/)帐户。

## <a name="deploy-the-weather-prediction-model-as-a-web-service"></a>将天气预测模型部署为 Web 服务

在本节中，您将从 Azure AI 库获取天气预报模型。 然后，向模型添加 R 脚本模块以清洁温度和湿度数据。 最后，将模型部署为预测 Web 服务。

### <a name="get-the-weather-prediction-model"></a>获取天气预报模型

在本节中，您将从 Azure AI 库获取天气预报模型，并在 Azure 机器学习工作室（经典）中打开它。

1. 转到[天气预测模型页](https://gallery.cortanaintelligence.com/Experiment/Weather-prediction-model-1)。

   ![在 Azure AI 库中打开天气预报模型页面](media/iot-hub-weather-forecast-machine-learning/weather-prediction-model-in-azure-ai-gallery.png)

1. 单击 **"在工作室中打开"（经典）** 以在 Microsoft Azure 机器学习工作室（经典）中打开模型。

   ![在 Azure 机器学习工作室中打开天气预报模型（经典）](media/iot-hub-weather-forecast-machine-learning/open-ml-studio.png)

### <a name="add-an-r-script-module-to-clean-temperature-and-humidity-data"></a>添加 R 脚本模块以清洁温度和湿度数据

要使模型正确运行，温度和湿度数据必须转换为数值数据。 在本节中，您将 R 脚本模块添加到天气预报模型中，该模型将删除具有温度或湿度数据值的任何行，这些值无法转换为数值。

1. 在 Azure 机器学习工作室窗口的左侧，单击箭头展开工具面板。 在搜索框中输入"执行"。 选择**执行 R 脚本**模块。

   ![选择执行 R 脚本模块](media/iot-hub-weather-forecast-machine-learning/select-r-script-module.png)

1. 在关系图上拖动"**清除缺失数据**"模块和现有执行**R 脚本**模块附近的**执行 R 脚本**模块。 删除 **"清除缺失数据****"与执行 R 脚本**模块之间的连接，然后按所示连接新模块的输入和输出。

   ![添加执行 R 脚本模块](media/iot-hub-weather-forecast-machine-learning/add-r-script-module.png)

1. 选择新的**执行 R 脚本**模块以打开其属性窗口。 复制以下代码并将其粘贴到**R 脚本**框中。
   ...
   ![将代码添加到执行 R 脚本模块](media/iot-hub-weather-forecast-machine-learning/add-code-to-module.png)

### <a name="deploy-predictive-web-service"></a>部署预测性 Web 服务

在本节中，您可以验证模型，根据模型设置预测 Web 服务，然后部署 Web 服务。

1. 单击 **"运行**"以验证模型中的步骤。 此步骤可能需要几分钟才能完成。

   ![运行实验以验证步骤](media/iot-hub-weather-forecast-machine-learning/run-experiment.png)

1. 单击 **"设置 WEB 服务** > **预测 Web 服务**"。 将打开预测实验图。

   ![在 Azure 机器学习工作室中部署天气预报模型（经典）](media/iot-hub-weather-forecast-machine-learning/predictive-experiment.png)

1. 在预测实验图中，删除**Web 服务输入**模块和顶部**的天气数据集**之间的连接。 然后拖动在**分数模型**模块附近的 Web**服务输入**模块，然后如图所示连接它：

   ![连接 Azure 机器学习工作室中的两个模块（经典）](media/iot-hub-weather-forecast-machine-learning/13_connect-modules-azure-machine-learning-studio.png)
...
1. 在模型的仪表板上，下载“Excel 2010 或更早版本的工作簿”**** 用于“请求/响应”****。
   ...
   > [!Note]
   > 请确保您下载**Excel 2010 或更早的工作簿**，即使您在计算机上运行了更高版本的 Excel。
...
在"流分析"作业中**Start** > ，单击"**立即** > **开始**"。 成功启动作业后，作业状态将从“已停止”**** 更改为“正在运行”****。
