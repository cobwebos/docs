...
使用 IoT 中心将[遥测数据从设备发送到 iot 中心](quickstart-send-telemetry-dotnet.md)快速入门教程和[使用 iot 中心发送云到设备的消息](iot-hub-csharp-csharp-c2d.md)教程显示 iot 中心的基本设备到云和云到设备的消息传送功能。 [使用 IoT 中心配置消息路由](tutorial-routing.md)教程介绍了一种在 Microsoft Azure Blob 存储中可靠地存储设备到云消息的方法。 但是，在某些情况下，无法轻松地将设备发送的数据映射到 IoT 中心接受的相对较小的设备到云消息。 例如：
...
1. 在**Main**方法中，在 `Console.ReadLine()`之前添加以下行：
...
1. 在 "**通用属性** > **启动项目**" 中，选择 "**多个启动项目**"，然后选择 " **ReadFileUploadNotification** " 和 " **SimulatedDevice**" 的**启动**操作。 选择“确定”保存更改。
