...
| 文件上传 | 向 Azure 存储传输文件时，不通过 IoT 中心计费。 文件传输时的启动消息和完成消息按 4KB 增量数计费。 例如，传输 10 MB 的文件时，除了 Azure 存储成本之外，还需要对两条消息收费。 |
| 直接方法 | 成功的方法请求按 4 KB 的块计费，并以 4 KB 作为附加消息的块的形式对响应进行收费。 向已断开连接的设备发出的请求被视作消息，以 4KB 块大小为单位收费。 例如，如果某方法的正文为 4 KB，则会产生一条消息作为两条消息的正文。 而正文为 6KB 的方法产生的来自设备的响应若为 1KB，则按 2 条请求消息收费，另加一条响应的消息费用。 |
...
某个设备每分钟将一条 1KB 的设备到云的消息发送到 IoT 中心，该消息随后由 Azure 流分析读取。 解决方案后端每10分钟调用一次设备上的方法（具有512字节的有效负载），以触发特定操作。 设备以 200 字节大小的结果响应方法。
