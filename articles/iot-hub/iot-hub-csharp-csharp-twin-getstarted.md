...
* CreateDeviceIdentity。 此应用创建设备标识和关联的安全密钥以连接模拟设备应用。
...
1. 将以下字段添加到 Program 类。 将 `{device connection string}` 替换为在[在 IoT 中心内注册新设备](#register-a-new-device-in-the-iot-hub)中记下的设备连接字符串。
