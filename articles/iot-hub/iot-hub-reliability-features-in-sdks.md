---
title: 通过设备 Sdk 管理 IoT 中心连接 & 可靠消息传送
...
* **自定义重试**：对于某些 SDK 语言，可以设计更适合你的方案的自定义重试策略，然后将其注入 RetryPolicy。 自定义重试在 C SDK 上不可用，并且 Python SDK 当前不支持此功能。 Python SDK 会按需重新连接。
