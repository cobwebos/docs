...
| Operation | 限制 |
| --------- | ----- |
| 设备 | 可注册到单个 IoT 中心的设备和模块的总数上限为 1,000,000。 增加此限制的唯一方法是联系[微软支持](https://azure.microsoft.com/support/options/)。|
...
| IoT Edge 自动部署<sup>1</sup> | 每次部署 50 个模块。 每个付费 SKU 中心 100 个部署（包括分层部署）。 每个免费 SKU 中心 10 个部署。 |
...
在任意给定时间，您可以通过[增加 IoT 中心中的预配单位数](iot-hub-upgrade.md)来增加配额或限制。
