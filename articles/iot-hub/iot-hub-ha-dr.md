...
<sup>1</sup>在此功能的预览版中，手动故障转移期间无法恢复云到设备的消息和父作业。
...
>
> - 当路由到 blob 存储，我们建议登记 blob，然后循环访问它们，以确保所有容器都读取而无需进行任何假设分区。 在 Microsoft 启动故障转移或手动故障转移期间，分区范围可能会发生更改。 若要了解如何枚举的 blob，请参阅列表[路由到 blob 存储](iot-hub-devguide-messages-d2c.md#azure-blob-storage)。
