...
description: 本文显示消息扩充，使 IoT 中心能够在消息发送到指定的终结点之前用附加信息标记消息。
author: robinsh
manager: philmea
ms.service: iot-hub
services: iot-hub
...
---
# <a name="message-enrichments-for-device-to-cloud-iot-hub-messages"></a>设备到云 IoT 中心消息的消息扩充
...
您可以通过选择终结点作为事件网格，向发布到事件网格的消息添加扩充。 我们根据您的事件网格订阅在 IoT 中心中创建设备遥测的默认路由。 此单一路由可以处理你的所有事件网格订阅。 在创建事件网格订阅到设备遥测后，可以为事件网格终结点配置扩充。 有关详细信息，请参阅 [IoT 中心和事件网格](iot-hub-event-grid.md)。
...
| **方法** | **命令** |
| ----- | -----| 
| 门户 | [Azure 门户](https://portal.azure.com) | 请参阅[消息扩充教程](tutorial-message-enrichments.md) | 
| Azure CLI   | [az iot 中心消息扩充](https://docs.microsoft.com/cli/azure/iot/hub/message-enrichment?view=azure-cli-latest) |
...
要尝试消息扩充，请参阅[消息扩充教程](tutorial-message-enrichments.md)
...
* 消息丰富不适用于数字孪生更改事件[（IoT 即插即用公共预览](../iot-pnp/overview-iot-plug-and-play.md)的一部分）。
...
* [消息丰富教程](tutorial-message-enrichments.md)
