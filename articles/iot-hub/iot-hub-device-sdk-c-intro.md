...
SDK 已在广泛的平台上经过测试（有关详细信息，请参阅 [SDK 文档](https://github.com/Azure/azure-iot-sdks/tree/master/c)）。尽管本文包含在 Windows 平台上运行的示例代码的演练，但请记住，本文所述的代码在各种支持的平台上都完全相同。
...
在存储库的 **common**、**iothub\_client** 和 **serializer** 文件夹中可找到 SDK 的核心实现。**common** 文件夹包含所有库使用的共享代码（通常你不会直接使用 **common** 文件夹中的代码）。不过，**iothub\_client** 和 **serializer** 文件夹包含两个将使用于代码中的不同 SDK 层实现。

-   **IoTHubClient** - **iothub\_client** 文件夹包含 SDK（**IoTHubClient** 库）中最低 API 层的实现。**IoTHubClient** 库包含用于将事件发送到 IoT 中心以及从 IoT 中心接收消息的 API。如果你使用此库，就需负责实现消息序列化，但与 IoT 中心通信的其他细节则由系统为你处理。

-   **serializer** - **serializer** 文件夹包含 **序列化程序**库的实现。**序列化程序**库在 **IoTHubClient** 提供的功能之上添加建模功能。如果你使用**序列化程序**库，首先需要定义一个模型，以指定要发送到 IoT 中心的事件以及预期要从 IoT 中心接收的消息。定义此模型后，SDK 将提供一个 API 图面，让你轻松处理事件和消息，而无需担心序列化细节。

查看示例代码可以更轻松地了解所有这些知识。以下部分将演练 SDK 中包含的几个示例应用程序。这应可让你轻松了解 SDK 体系结构层的各种功能以及 API 工作原理的简介。

## 运行示例之前

只有在先完成以下两个任务之后，才可以在适用于 C 语言的 Azure IoT 设备 SDK 中运行示例：准备开发环境和获取设备凭据。SDK 随附的[自述文件](https://github.com/Azure/azure-iot-sdks/tree/master/c)提供了这两个任务的说明。以下部分包含有关这些说明的一些额外注释。

### 准备开发环境

首先，需要从 GitHub 获取 SDK 的副本，然后构建源。应该从 [GitHub 存储库](https://github.com/Azure/azure-iot-sdks)的 **master** 分支获取源文件的副本：

下载源文件副本后，必须完成 SDK 文章[准备开发环境](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md)中所述的步骤。为 Windows 执行的大多数操作都涉及到[准备 QPID Proton 库](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md#preparing-qpid-proton-libraries-in-windows)和[验证环境](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md#verify-your-environment)。以下是一些提示，可帮助你完成准备指南中所述的过程：

-   当你创建 **PROTON\_PATH** 环境变量时，请将它设为**系统**环境变量，如下所示：
...
-   务必安装正确的 Python 版本。通常，在 Windows 中，应该安装 x86 版本：

  ![](media/iot-hub-device-sdk-c-intro/09-Python.PNG)

可以使用安装程序中的默认选项。

- 将 Python 目录添加到系统 **PATH** 环境变量。在 Windows 上，屏幕如下所示：
- 
...
当你从命令提示符运行 **build\_proton.cmd** 脚本时，该脚本将从 Proton 的 GitHub 存储库下载其源文件，然后构建 Proton 库。

如果你想要运行 MQTT 示例，请根据 SDK 文档中有关[构建 Apache MQTT 库](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md#building-the-apache-mqtt-library-in-windows)的说明操作。无论如何，最后都必须运行 **build.cmd**。

完成上述步骤后，可以开始编译示例应用程序。

### 获取设备凭据

现在你已设置好开发环境，最后一个操作就是获取一组设备凭据。若要使设备能够访问 IoT 中心，必须先将该设备添加到 IoT 中心设备注册表。添加设备时，你将获取一组所需的设备凭据，以便设备能够连接到 IoT 中心。下一部分中所述的示例应用程序预期这些凭据的格式为**设备连接字符串**。

[设备资源管理器](https://github.com/Azure/azure-iot-sdks/tree/master/tools/DeviceExplorer)工具（Azure IoT 设备 SDK 已随附）使用 Azure IoT 服务库在 IoT 中心执行各种功能（包括添加设备）。如果你使用设备资源管理器来添加设备，将会获得相应的连接字符串。需要此连接字符串才能运行示例应用程序。

如果你不熟悉上述过程，以下过程描述了如何使用设备资源管理器来添加设备和获取设备连接字符串。

在 **Visual Studio 2015** 中打开 **[DeviceExplorer.sln](https://github.com/Azure/azure-iot-sdks/blob/master/tools/DeviceExplorer/DeviceExplorer.sln)** 并构建解决方案。运行该程序时，你将看到此界面：
...
在使用 SDK 时始终需要 **Microsoft.Azure.IoTHub.Common** 包。由于此示例依赖于 AMQP，因此你还必须包含 **Apache.QPID.Proton.AzureIoT** 和 **Microsoft.Azure.IoTHub.AmqpTransport** 包（HTTP 有对应的包）。由于此示例使用 **IoTHubClient** 库，因此你还必须在解决方案中包含 **Microsoft.Azure.IoTHub.IoTHubClient** 包。

可以在 **iothub\_client\_sample\_amqp.c** 源文件中找到示例应用程序的实现：

  ![](media/iot-hub-device-sdk-c-intro/13-iothub_client_sample_amqp_c.PNG)

我们将使用此示例应用程序来演练使用 **IoTHubClient** 库时所需的项目。
...
请注意，我们要将设备连接字符串副本（从设备资源管理器获取的连接字符串）传递给此函数。我们还需指定要使用的协议。本示例使用 AMQP，但也可以选择 HTTP。
...
处理完消息时，请注意对 **IoTHubMessage\_Destroy** 函数的调用。必须进行此调用，才能释放在创建消息时分配的资源。
...
这释放 **IoTHubClient\_CreateFromConnectionString** 函数以前分配的资源。

如你所见，使用 **IoTHubClient** 库可以轻松发送事件和接收消息。该库将处理与 IoT 中心通信的详细信息，包括要使用哪个协议（从开发人员的立场来看，这是一个简单的配置选项）。

在如何对设备发送到 IoT 中心的事件进行序列化方面，**IoTHubClient** 库也可提供精确的控制。在某些情况下，这是一项优点，但在其他情况下，这是你不想要看到的实现细节。如果是这样，可以考虑使用下一部分中说明的**序列化程序**库。

## 序列化程序

从概念上讲，**序列化程序库**位于 SDK 中的 **IoTHubClient** 库之上。它使用 **IoTHubClient** 库来与 IoT 中心进行基本通信，但添加了建模功能，以减轻开发人员处理消息序列化的负担。我们将通过一个示例充分演示此库的工作原理。

azure-iot-sdks 存储库的 **serializer** 文件夹中有一个包含 **simplesample\_amqp** 应用程序的 **samples** 文件夹。此示例的 Windows 版本包含以下 Visual Studio 解决方案：
...
对 **serializer\_init** 函数进行的调用是单次调用，可将基础库初始化。然后，需调用 **IoTHubClient\_CreateFromConnectionString** 函数，这是 **IoTHubClient** 示例中的同一 API。此调用将设置设备连接字符串（也可用于选择要使用的协议）。请注意，本示例使用 AMQP 作为传输方式，但也可以使用 HTTP。
...
**序列化程序**库中的模型定义设备可发送到 IoT 中心的事件以及可接收的消息（在模块化语言中称为*操作*）。可以使用 **simplesample\_amqp** 示例应用程序中的一组 C 宏来定义模型：
...
在本示例中，有一个名为 **ContosoAnemometer** 的模型。此模型定义设备可以发送到 IoT 中心的两个事件：**DeviceId** 和 **WindSpeed**。它还定义了设备可以接收的三个操作（消息）：**TurnFanOn**、**TurnFanOff** 和 **SetAirResistance**。每个事件都有一个类型，而每个操作都有一个名称（以及一组可选参数）。
...
此代码非常类似于 **iothub\_client\_sample\_amqp** 应用程序中的代码，其中从某个字节数组创建了一条消息，然后使用 **IoTHubClient\_SendEventAsync** 将它发送到 IoT 中心。此后，只需释放以前分配的消息句柄和已序列化的数据缓冲区。

**IoTHubClient\_SendEventAsync** 的倒数第二个参数是对成功发送数据时所调用的回调函数的引用。下面是回调函数的示例：
