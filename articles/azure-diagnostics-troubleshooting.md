...
| -6 |来宾代理插件启动器尝试使用无效的命令行启动诊断。<p><p>这是一个内部错误，仅当在 VM 上不正确地手动调用了来宾代理插件启动器时才会发生该错误。 |
| -10 |Diagnostics 插件退出并返回未处理的异常。 |
| -11 |来宾代理程序无法创建负责启动和监视监视代理的进程。<p><p>解决方案：验证是否有足够的系统资源用于启动新进程。<p> |
| -101 |调用诊断插件时参数无效。<p><p>这是一个内部错误，仅当在 VM 上不正确地手动调用了来宾代理插件启动器时才会发生该错误。 |
...
| -105 |诊断插件无法打开诊断配置文件。<p><p>这是一个内部错误，仅当在 VM 上不正确地手动调用了诊断插件时才会发生该错误。 |
| -106 |无法读取诊断配置文件。<p><p>解决方案：这是配置文件未通过架构验证的结果。 因此，解决方案是提供符合架构的配置文件。 可以在 VM 上的文件夹 *%SystemDrive%/WindowsAzure/Config* 中，找到提供给诊断扩展的 XML。 打开相应的 XML 文件并搜索 **Microsoft.Azure.Diagnostics**，然后搜索 **xmlCfg** 字段。 数据采用 base64 编码，因此需要[将其解码](http://www.bing.com/search?q=base64+decoder)才能查看诊断加载的 XML。<p> |
| -107 |传递给监视代理的资源目录无效。<p><p>这是一个内部错误，仅当在 VM 上不正确地手动调用了监视代理时才会发生该错误。</p> |
| -108 |无法将诊断配置文件转换为监视代理配置文件。<p><p>这是一个内部错误，仅当使用无效的配置文件手动调用了诊断插件时才会发生该错误。 |
| -110 |常规诊断配置错误。<p><p>这是一个内部错误，仅当使用无效的配置文件手动调用了诊断插件时才会发生该错误。 |
