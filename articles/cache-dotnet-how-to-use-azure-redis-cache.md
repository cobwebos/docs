...
Microsoft Azure Redis 缓存目前以预览版提供，基于流行的开放源代码 Redis 缓存。它让您访问 Microsoft 管理的安全专用的 Redis 缓存。使用 Azure Redis 缓存创建的缓存可从 Microsoft Azure 内的任何应用程序进行访问。
...
若要创建缓存，首先请登录到 [Microsoft Azure 门户][]，然后单击“新建”、“数据 + 存储”、“Redis 缓存”。
