...
### <a name="create-an-azure-ad-key-vault-application"></a>创建 Azure AD Key Vault 应用程序

Blockchain Workbench 部署要求注册 Azure AD Key Vault 应用程序。

1. 在 Azure 门户的左侧导航窗格中，选择“Azure Active Directory”服务。 选择“应用注册” > “新建应用程序注册”。
2. 提供应用程序的“名称”和“登录 URL”。 可以使用占位符值，因为稍后在部署期间会更改这些值。

    ![创建 Key Vault 应用注册](media/blockchain-workbench-deploy/key-vault-app-create.png)

    | 设置  | 值  |
    |---------|---------|
    | 名称 | `Blockchain Key Vault app` |
    | 应用程序类型 | Web 应用/API |
    | 登录 URL | `https://keyvaultclient |

5. 选择“创建”以注册 Azure AD Key Vault 应用程序。
...
11. 完成“Azure Monitor”设置。
    ...
    | 设置 | 说明  |
    |---------|--------------|
    | 监视 | 选择是否要使用 Azure Monitor 来监视区块链网络。 |
    | 连接到现有的 OMS 实例 | 选择是要使用现有的 Operations Management Suite 实例，还是创建新实例。 
...
部署 Azure Blockchain Workbench 后，下一步是[管理 Azure Blockchain Workbench 中的用户](blockchain-workbench-manage-users.md)。