...
会话具有基于时段的流量控制模型；创建会话时，每一方声明它愿意在接收时段内接受的帧数。 当各方交换帧时，已传输的帧将填满该时段，传输在时段已满时停止，直到该时段使用流程行为原语** 进行重置或扩展为止（行为原语** 是 AMQP 术语，表示在双方之间交换的协议级别手势）。
...
通过 TCP 使用 AMQP 连接的客户端需要在本地防火墙中打开端口 5671 和 5672。 除了这些端口，如果启用[启用了启用链接重定向](https://docs.microsoft.com/dotnet/api/microsoft.servicebus.messaging.amqp.amqptransportsettings.enablelinkredirect?view=azure-dotnet)功能，则可能需要打开其他端口。 `EnableLinkRedirect`是一种新的消息传递功能，可帮助在接收消息时跳过一跳，从而有助于提高吞吐量。 客户端将开始直接与端口范围 104XX 上的后端服务通信，如下图所示。 

![目标端口列表][4]

如果防火墙阻止这些端口，则 .NET 客户端将失败，并且使用套接字Exception（"尝试以其访问权限禁止的方式访问套接字"）。 可以通过在 connectg 字符串`EnableAmqpLinkRedirect=false`中设置该功能来禁用该功能，该字符串强制客户端通过端口 5671 与远程服务通信。
...
在链接上，传输只有在发件人有足够的*链接信用*时才能发生。 链接信用额度是接收者使用*流程*行为原语所设置的计数器，其范围是链接。 将链接信用额度分配给发送者时，将通过传递消息来尝试用完该信用额度。 每个消息传递使剩余的链接信用额度减 1。 当链接信用额度用完时，便会停止传递。
...
| 客户端 | 服务总线 |
| --- | --- |
| --> detach(<br/>handle={numeric handle},<br/>闭合=**真**<br/>) |无操作 |
| 无操作 |<-- detach(<br/>handle={numeric handle},<br/>闭合=**真**<br/>) |
...
| durable |- |- |
| priority |- |- |
| ttl |消息生存时间 |[时间到生活](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| first-acquirer |- |- |
...
| 字段名称 | 使用情况 | API 名称 |
| --- | --- | --- |
| message-id |应用程序为此消息定义的自由格式标识符。 用于重复检测。 |[消息 Id](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
...
| correlation-id |应用程序定义的相关性标识符，服务总线无法进行解释。 |[相关性 Id](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
| content-type |应用程序定义的内容类型指示符，服务总线无法进行解释。 |[内容类型](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
...
| group-id |应用程序为相关的消息集定义的标识符。 用于服务总线会话。 |[会话 Id](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage) |
...
| x-opt-via-partition-key | 应用程序定义的分区键值，指示某个事务在何时用于通过传输队列发送消息。 | [维亚分区键](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.viapartitionkey?view=azure-dotnet) |
...
| x-opt-sequence-number | 服务定义的唯一编号，用于分配给消息。 | [序列号](/dotnet/api/microsoft.servicebus.messaging.brokeredmessage.sequencenumber?view=azure-dotnet) |
...
|  | <------ | disposition( <br/> first=0, last=0, <br/>state=**Declared**(<br/>**txn-id**[事务 ID]<br/>))|
...
| | <------ | disposition( <br/> first=57, last=57, <br/>状态=**已接受（）**|

#### <a name="sending-a-message-in-a-transaction"></a>在事务中发送消息

所有事务性工作都使用携带 txn-id`transactional-state`的事务传递状态完成。在发送消息的情况下，事务状态由消息的传输帧进行。 
...
令牌赋予权限。 服务总线识别三个基本权限：“发送”允许发送、“侦听”允许接收，“管理”允许操作实体。 AAD/ACS 颁发的 SWT 令牌明确将这些权限包含为声明。 服务总线 SAS 令牌引用在命名空间或实体上配置的规则，这些规则是使用权限配置的。 使用与该规则关联的密钥来签名令牌，以此方式让令牌表达各自的权限。 与使用 put-token ** 的实体关联的令牌将允许已连接的客户端根据每个令牌权限来与实体交互。 客户端在发送*者*角色上占据的链接需要"发送"权利;承担*接收者*角色需要"倾听"的权利。
...
| 客户端 | | 服务总线 |
| --- | --- | --- |
| attach(<br/>name={link name},<br/>role=sender,<br/>source={client link ID},<br/>目标 =**[通过实体]**，<br/>**properties=map [(<br/>com.microsoft:transfer-destination-address=<br/>{destination-entity} )]** ) | ------> | |
