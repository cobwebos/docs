...
**服务总线高级消息传送**在 CPU 和内存级别提供资源隔离，以便每个客户工作负荷以隔离方式运行。 此资源容器称为*消息传递单元*。 每个高级命名空间至少会分配一个消息传送单元。 可以为每个服务总线高级命名空间购买 1、2、4 或 8 个消息传送单元。 单一工作负荷或实体可以跨多个消息传送单元，可以随意更改消息传送单元数。 从而为基于服务总线的解决方案提供可预测和稳定的性能。
...
## <a name="premium-messaging-resource-usage"></a>高级消息资源使用情况
通常，实体上的任何操作都可能导致 CPU 和内存使用。 下面是一些操作： 

- 管理操作，如 CRUD（创建、检索、更新和删除）队列、主题和订阅的操作。
- 运行时操作（发送和接收消息）
- 监视操作和警报

但是，额外的 CPU 和内存使用量不会另加定价。 对于高级消息传递层，消息单元只有一个价格。

CPU 和内存使用情况被跟踪并显示给您，原因如下： 

- 为系统内部提供透明度
- 了解所购买资源的能力。
- 帮助您决定向上/缩小规模的容量规划。

## <a name="messaging-unit---how-many-are-needed"></a>消息传递单元 - 需要多少？

预配 Azure 服务总线高级命名空间时，必须指定分配的消息传递单元数。 这些消息传递单元是分配给命名空间的专用资源。

分配给服务总线高级命名空间的消息传递单元数可以**动态调整**，以考虑工作负载的变化（增加或减少）。

在决定体系结构的消息传递单元数时，需要考虑许多因素：

- 从分配给命名空间的***1 个或 2 个消息传递单元***开始。
- 研究命名空间的资源使用指标中的 CPU[使用率指标](service-bus-metrics-azure-monitor.md#resource-usage-metrics)。
    - 如果 CPU 使用率***低于 20%，*** 则可以***缩减***分配给命名空间的消息传递单元数。
    - 如果 CPU 使用率***超过 70%，*** 则应用程序将受益于***扩展***分配给命名空间的消息传递单元数。

可以使用[Azure 自动化 Runbook](../automation/automation-quickstart-create-runbook.md)自动扩展分配给服务总线命名空间的资源的过程。

> [!NOTE]
> **分配给**命名空间的资源的缩放可以是先发制人的，也可以是被动的。
>
>  * **抢占**：如果预期会增加工作负载（由于季节性或趋势），则可以在工作负载命中之前将更多消息传递单元分配给命名空间。
>
>  * **反应：** 如果通过研究资源使用情况指标来识别其他工作负荷，则可以将其他资源分配给命名空间以合并不断增长的需求。
>
> 服务总线的计费表每小时一次。 在放大的情况下，您只需为使用这些资源的小时数支付额外资源。
>
