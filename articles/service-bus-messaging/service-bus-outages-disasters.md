---
title: "使服务总线应用程序免受服务总线中断和灾难影响 | Microsoft 文档"
...
要针对 ACS 中断进行保护，请使用共享访问签名 (SAS) 令牌。 在这种情况下，客户端使用密钥对自创令牌进行签名，从而与服务总线直接进行身份验证。 不再需要调用 ACS。 有关 SAS 令牌的详细信息，请参阅[服务总线身份验证][服务总线身份验证]。

## <a name="protecting-queues-and-topics-against-messaging-store-failures"></a>保护队列和主题免受消息存储故障的影响
将未分区的队列或主题分配到一个消息存储空间。 如果此消息存储空间不可用，则针对该队列或主题的所有操作将都失败。 另一方面，分区的队列包括多个片段。 每个片段存储在不同的消息存储空间中。 当向分区的队列或主题发送消息时，服务总线会将该消息分配到其中一个片段。 如果相应的消息存储空间不可用，则服务总线会将消息写入另一片段（如有可能）。 有关分区实体的详细信息，请参阅[分区消息传送实体][分区消息传送实体]。
...
有关详细信息，请参阅[异步消息传送模式和高可用性][异步消息传送模式和高可用性]中的“Azure 数据中心内的服务总线故障”部分。
...
[使用服务总线中继消息进行异地复制][使用服务总线中继消息进行异地复制]示例演示了如何复制中继。
...
[Geo-replication with Service Bus Brokered Messages][Geo-replication with Service Bus Brokered Messages]示例演示了消息传送实体的主动复制。
...
[Geo-replication with Service Bus Brokered Messages][Geo-replication with Service Bus Brokered Messages]示例演示了消息传送实体的被动复制。

## <a name="next-steps"></a>后续步骤
若要了解有关灾难恢复的详细信息，请参阅这些文章：

* [Azure SQL 数据库业务连续性][Azure SQL 数据库业务连续性]
* [Azure 复原技术指南][Azure 复原技术指南]
...
[Geo-replication with Service Bus Brokered Messages]: http://code.msdn.microsoft.com/Geo-replication-with-f5688664（使用服务总线中转消息进行异地复制）
