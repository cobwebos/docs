...
3. 在主要命名空间与次要命名空间之间创建配对以获取“别名”。
    ...
    >[!NOTE] 
    > 如果有[迁移到 Azure 服务总线高级版 Azure 服务总线标准级别命名空间](service-bus-migrate-standard-premium.md)，则必须使用预先存在的别名 （即在服务总线标准级别命名空间连接字符串） 创建灾难恢复通过配置**PS/CLI**或**REST API**。
    >
    >
    > 这是因为，在迁移期间，Azure 服务总线标准级别命名空间连接字符串/DNS 名称本身成为你的 Azure 服务总线高级命名空间的别名。
    >
    > 客户端应用程序必须使用此别名 （即标准的 Azure 服务总线命名空间连接字符串） 以连接到高级命名空间的灾难恢复配对已设置。
    >
    > 如果你使用门户设置灾难恢复配置，然后门户将抽象此你需要注意的地方。
