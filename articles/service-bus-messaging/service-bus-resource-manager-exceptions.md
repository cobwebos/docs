...
| 错误代码 | 错误子代码 | 错误消息 | 说明 | 建议 |
| ---------- | ------------- | ------------- | ----------- | -------------- |
| 未授权 | 无 | 辅助命名空间上的操作无效。 辅助命名空间为只读。 | 操作是针对辅助命名空间执行的，辅助命名空间作为 readonly 命名空间进行设置。 | 针对主命名空间重试该命令。 了解有关[辅助命名空间](service-bus-geo-dr.md)的详细信息 |
| 未授权 | 无 | MissingToken：找不到授权标头。 | 当授权的值为 null 或不正确时，将发生此错误。 | 请确保授权标头中提到的令牌值正确且不为 null。 |