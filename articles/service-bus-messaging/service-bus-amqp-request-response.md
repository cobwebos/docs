...
请求消息正文必须包含包含以下条目的**映射**的**amqp 值**部分：  
...
响应消息正文必须包含包含以下条目的**映射**的**amqp 值**部分：  
...
响应消息正文必须包含包含以下条目的映射的**amqp 值**部分：  
...
**规则描述**映射必须包含以下条目，其中 sql**筛选器****和关联筛选器**是互斥的：  
...
|receiver-settle-mode|ubyte|是|在 AMQP 内核 v1.0 中指定的**接收器稳定**模式。|  
