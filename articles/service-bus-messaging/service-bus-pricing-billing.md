...
> [!NOTE]
> 有关当前服务总线定价的详细信息，请参阅 [Azure 服务总线定价页](https://azure.microsoft.com/pricing/details/service-bus/)和[服务总线常见问题解答](service-bus-faq.md#service-bus-pricing)。
>
>
...
请注意，每个 Azure 订阅每月仅需支付一次标准基础费用。 这意味着创建单个标准或高级层服务总线命名空间后，可在同一 Azure 订阅下创建其他任意数目的标准或高级层命名空间，而不会产生附加的基础费用。

在 2014 年 11 月 1 日之前创建的所有现有服务总线命名空间会自动放入标准层。 这可确保你继续有权访问服务总线当前可用的所有功能。 随后，可以根据需要使用 [Azure 经典门户][Azure 经典门户]降级到基本层。
...
先前，队列和主题/订阅每个 URL 限制为 100 个并发连接。 当前计费方案移除队列和主题/订阅的每个 URL 限制，在服务总线命名空间和 Azure 订阅级别实施基于中转连接数的配额和计费方法。

基本层包括并严格限制为每个服务总线命名空间 100 个中转连接。 连接数目大于此数字将被拒绝用于基本层中。 标准层移除每个命名空间的限制，在 Azure 订阅上计算总共的中转连接使用量。 标准层中，每个 Azure 订阅的 1,000 中转连接无需额外费用（不包括在基础费用内）。 在 Azure 中的标准层服务总线命名空间中使用的中转连接总数超过 1000 时会依据分级方案计费，如下表所示。
...
WCF 中继仅适用于标准层命名空间。 另外，中继的定价和连接配额保持不变。 这意味着中继将继续根据消息数（而不是操作数）和中继小时数计费。

| WCF 中继定价 | 成本 |
| --- | --- |
| WCF 中继小时数 |每 100 中继小时数收取 0.10 美元 |
| 消息 |每 10,000 条消息收取 0.10 美元 |

## <a name="faq"></a>常见问题
### <a name="how-is-the-wcf-relay-hours-meter-calculated"></a>如何计算 WCF 中继小时数？
请参阅[此主题](service-bus-faq.md)。
...
* 有关服务总线定价的更多详细信息，请参阅 [Azure 服务总线定价页](https://azure.microsoft.com/pricing/details/service-bus/)。
* 有关服务总线和计费的一些常见问题解答，请参阅[服务总线定价常见问题解答](service-bus-faq.md#service-bus-pricing)。
