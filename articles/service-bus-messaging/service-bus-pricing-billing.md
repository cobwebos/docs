...
> [!NOTE]
> 计费基于的峰值并发连接数和按比例分配费用每小时每月 730 个小时。
>
>
...
超出附送数量（标准层为 1,000）的峰值并发中转连接数的服务总线收费。 峰值进行测量按小时计费，在一个月的 730 个小时按比例分配费用和每月计费周期内累加。 在计费周期结束时，会针对按比例分配的小时峰值的总和应用附送数量（每月 1,000 个中转连接）。

例如：

1. 每个设备（共 10000 个）通过单个 AMQP 连接进行连接，并从服务总线主题中接收命令。 设备将遥测事件发送至事件中心。 如果所有设备每天连接 12 小时，则需支付以下连接费用（未包括任何其他服务总线主题费用）：10000 个连接 * 12 小时 * 30 天 / 730 = 4,931 中转连接。 在扣除每月 1,000 个中转连接的限额后，需要按每个中转连接 0.03 美元的费率支付 4,000 个中转连接的费用，总共 120 美元。
2. 10,000 台设备通过 HTTP 从服务总线队列接收消息，超时不为零。 如果所有设备每天连接 12 小时，则需支付以下连接费用（未包括任何其他服务总线费用）：10000 个 HTTP 接收连接 * 12 小时 / 天 * 30 天/730 个小时 = 4,931 中转连接。
