...
*事务*将两个或多个操作组合到*执行作用域中*。 就本质而言，此类事务必须确保所有操作属于给定的操作组，无论联合成功还是失败。 在这方面，事务作为一个单元进行操作，通常称为原子性**。
...
要启用将数据从队列到处理器，然后到另一个队列的事务性移交，服务总线支持传输**。 在传输操作中，发送方首先将消息发送到*传输队列*，传输队列使用自动转发功能所依赖的相同健壮的传输实现将消息立即移动到预期目标队列。 消息永远不会以对传输队列的使用者可见的方式提交到传输队列的日志中。
