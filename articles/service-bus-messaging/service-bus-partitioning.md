...
对非分区实体的速览操作始终返回最早的消息，但不返回分区实体上的消息。 相反，它会将最早的消息返回到其消息代理首先响应的分区之一。 不保证返回的消息在所有分区中是最早的消息。 
...
* **高一致性功能**：如果实体使用会话、重复检测或显式控制分区键等功能，则消息传送操作始终路由到特定分区。 如果任何分区遇到过高的流量，或基础存储处于不正常状态，这些操作将失败，可用性会降低。 整体来说，一致性仍然远高于非分区实体；只有一部分流量会遇到问题，而不是所有流量。 有关详细信息，请参阅此处[对可用性和一致性的讨论](../event-hubs/event-hubs-availability-and-consistency.md)。
* **管理**：必须对实体的所有分区执行 "创建"、"更新" 和 "删除" 等操作。 如果任何分区处于不正常状态，可能会导致这些操作失败。 以“获取”操作来说，必须汇总来自所有分区的信息，例如消息计数。 如果任何分区处于不正常状态，则实体可用性状态会报告为受限制。
