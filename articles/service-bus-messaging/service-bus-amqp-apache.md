---
title: "如何在 Linux VM 上安装 Apache Qpid Proton-C | Microsoft 文档"
...
---
# <a name="install-apache-qpid-proton-c-on-an-azure-linux-vm"></a>在 Azure Linux VM 上安装 Apache Qpid Proton-C
...
第一步是使用 [Azure 经典门户][Azure 经典门户]执行的。 以下屏幕截图显示了如何创建名为“scott-centos”的 CentOS VM：
...
若要登录到计算机，必须知道 SSH 的终结点端口。 可以通过 [Azure 经典门户][Azure 经典门户]选择新创建的 VM 并单击“**终结点**”选项卡从门户中获取此值。 以下屏幕截图显示此计算机的公共 SSH 端口为 57146。
...
1. 编辑 yum 配置文件 (/ etc/yum.conf) 并注释对内核标头更新的排除 (\# exclude=kernel\*)。 这是安装 gcc 编译器所必需的。
2. 安装必备组件包：
...
3. 下载 Proton 库：
   ...
   ```
4. 从分发存档中提取 Proton 代码：
   
   ```
   tar xvfz qpid-proton-0.9.tar.gz
   ```
5. 使用从自述文件中获取的以下步骤生成并安装代码：
