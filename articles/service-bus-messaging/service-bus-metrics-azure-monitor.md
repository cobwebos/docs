...
| 计划的消息| 队列/主题中的计划消息计数。 <br/><br/> 单位：计数 <br/> 聚合类型：平均  <br/> 维度：EntityName |

> [!NOTE]
> 以下指标的值是时间点值。 在时间点之后立即使用的传入消息可能不会反映在这些指标中。 
> - 消息
> - 活动消息 
> - 死信消息 
> - 计划的消息 
...
> 以下指标仅适用于**** 高级层。 
> 
> 监视高级层命名空间的任何中断的重要指标是：**每个命名空间的 CPU 使用率**和**每个命名空间的内存大小**。 使用 Azure 监视器设置这些指标的[警报](../azure-monitor/platform/alerts-metric.md)。
> 
> 可以监视的另一个指标是：**限制请求**。 只要命名空间保持在其内存、CPU 和代理连接限制内，这不应该是问题。 有关详细信息，请参阅[Azure 服务总线高级层中的限制](service-bus-throttling.md#throttling-in-azure-service-bus-premium-tier)
