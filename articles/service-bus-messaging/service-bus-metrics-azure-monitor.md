...
> 以下指标仅适用于  高级层。 
> 
> 用于监视高级层命名空间的任何中断的重要指标是：每个命名空间的**CPU 使用率**和**每个命名空间的内存大小**。 使用 Azure Monitor 为这些指标[设置警报](../azure-monitor/platform/alerts-metric.md)。
> 
> 您可以监视的另一个指标为：**限制请求**。 不过，只要命名空间保持在其内存、CPU 和中转连接限制内，就不会出现问题。 有关详细信息，请参阅[Azure 服务总线高级层中的限制](service-bus-throttling.md#throttling-in-azure-service-bus-premium-tier)
