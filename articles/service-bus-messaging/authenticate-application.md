...
将 RBAC 角色分配到 Azure AD 安全主体后，Azure 会向该安全主体授予对这些资源的访问权限。 访问权限可以局限到订阅、资源组或服务总线命名空间级别。 Azure AD 安全主体可以是 Azure 资源的用户、组、应用程序服务主体或[托管标识](../active-directory/managed-identities-azure-resources/overview.md)。
...
- [Azure 服务总线数据接收器](../role-based-access-control/built-in-roles.md#azure-service-bus-data-receiver)：使用此角色授予对服务总线命名空间及其实体的接收访问权限。 
...
- **服务总线命名空间**：角色分配跨越命名空间下服务总线的整个拓扑以及与其关联的使用者组。
- **资源组**：角色分配应用于资源组下的所有服务总线资源。
- **订阅**：角色分配应用于订阅中的所有资源组中的所有服务总线资源。
...
1. 在 [Azure 门户](https://portal.azure.com/)中，导航到你的服务总线命名空间。 选择左侧菜单上的“访问控制(标识和访问管理)”，显示命名空间的访问控制设置****。 如果需要创建服务总线命名空间，请按照本文的说明操作：[创建服务总线消息命名空间](service-bus-create-namespace-portal.md)。
...
如果应用程序是一个控制台应用程序，则必须注册一个本机应用程序并将 **Microsoft.ServiceBus** 的 API 权限添加到“必需的权限”集。**** 本机应用程序还需要 Azure AD 中的**重定向 URI，** 该 URI 充当标识符;URI 不需要是网络目标。 对于此示例请使用 `https://servicebus.microsoft.com`，因为示例代码已使用了该 URI。
...
请参阅 GitHub 上的以下示例：[服务总线的角色基访问控制](https://github.com/Azure/azure-service-bus/tree/master/samples/DotNet/Microsoft.ServiceBus.Messaging/RoleBasedAccessControl)。 

使用“客户端机密登录”选项，而不是“交互用户登录”选项。******** 使用客户端机密选项时，看不到弹出窗口。 应用程序会使用租户 ID 和应用 ID 进行身份验证。 

### <a name="run-the-sample"></a>运行示例

在运行示例之前，请编辑**app.config**文件，并根据方案设置以下值：
