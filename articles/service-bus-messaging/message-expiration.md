---
title: Azure 服务总线-消息过期
description: 本文介绍 Azure 服务总线消息的到期时间和生存时间。 完成此类截止时间之后，将不再传递该消息。
...
必须通过 Azure 资源管理器操作或 .NET Framework 客户端 [NamespaceManager](/dotnet/api/microsoft.servicebus.namespacemanager) API 设置此autoDeleteOnIdle 属性。 不能在门户中设置此属性。
