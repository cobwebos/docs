...
> [!NOTE]
> 只能为**高级**命名空间使用服务标记。 如果使用的是**标准**命名空间，请使用运行以下命令时看到的 IP 地址： `nslookup <host name for the namespace>` 。 例如：`nslookup contosons.servicebus.windows.net`。 
