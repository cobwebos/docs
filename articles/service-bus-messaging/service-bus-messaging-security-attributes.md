...
| 传输中的加密 (如 ExpressRoute 加密、VNet 加密和 VNet-VNet 加密)| 是 | 支持标准的 HTTPS/TLS 机制。 |
| 加密密钥处理（CMK、BYOK 等）| 否 |   |
| 列级加密（Azure 数据服务）| 不可用 | |
...
| Authorization| 是 | 支持通过 [RBAC](service-bus-role-based-access-control.md)（预览版）和 SAS 令牌进行授权；请参阅[服务总线身份验证和授权](service-bus-authentication-and-authorization.md)。 |
