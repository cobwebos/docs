...
description: 用于评估 Azure 服务总线消息传递的安全控制清单
...
| 控制和管理平面日志记录和审核| 是 | 操作日志可用。  | [服务总线诊断日志](service-bus-diagnostic-logs.md) |
| 数据平面日志记录和审核| 否 |  |
...
| 安全控制 | Yes/No | 说明| 文档 |
|---|---|--|--|
| 身份验证| 是 | 通过[Azure 活动目录托管服务标识进行管理](service-bus-managed-service-identity.md)。| [服务总线身份验证和授权](service-bus-authentication-and-authorization.md)。 |
...
| 安全控制 | Yes/No | 说明 | 文档 |
|---|---|--|--|
| 服务器端静态加密：微软管理的密钥 |  默认情况下，服务器端静态加密为“是”。 |  |  |
| 服务器端静态加密：客户管理的密钥 (BYOK) | 是的。 | Azure KeyVault 中的客户托管密钥可用于加密服务总线命名空间上静态的数据。 | [配置客户管理密钥，以便使用 Azure 门户加密静态 Azure 服务总线数据](configure-customer-managed-key.md)  |
| 列级加密（Azure 数据服务）| 空值 | |   |
