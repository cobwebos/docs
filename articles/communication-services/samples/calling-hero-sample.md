...
下面将详细介绍先决条件、示例设置步骤以及帮助你熟悉其各个组件的分步教程。
...
2. `git clone`
3. 转到 Calling/ClientApp 文件夹并运行 `npm run setup`
   1. 如果看到错误 1，请在上面的输出中查找一个 URL，你需要转到该 URL 以对客户端授权。 （URL 类似于这种形式：`app.vssps.visualstudio.com/oauth2/authorize?clientid=...`）在浏览器中访问该 URL 后，从浏览器窗口复制命令并运行它。
   2. 完成上一步后，再次运行命令 `npm run setup-vsts-auth`。
4. 从 Azure 门户获取 `Connection String`。 有关连接字符串的详细信息，请参阅[创建 Azure 通信资源](../quickstarts/create-communication-resource.md)
5. 获取连接字符串后，将连接字符串添加到 Service .NET 文件夹下的 Calling/appsetting.json 文件中。 在变量 `ResourceConnectionString` 中输入连接字符串。

### <a name="local-run"></a>本地运行

1. 转到 Calling 文件夹
2. 在 Visual Studio 中打开 `Calling.csproj` 解决方案
2. 运行 `Calling` 项目*

*浏览器将打开 `localhost:5000`（节点用来部署客户端应用的位置）。 Internet Explorer 不支持该应用。
...
3. 发布之前，请使用`Edit App Service Settings`添加连接字符串，并填写 `ResourceConnectionString` 作为键，提供连接字符串（从 appsettings.json 复制）作为值。

## <a name="clean-up-resources"></a>清理资源

如果想要清理并删除通信服务订阅，可以删除资源或资源组。 删除资源组同时也会删除与之相关联的任何其他资源。 详细了解如何[清理资源](../quickstarts/create-communication-resource.md#clean-up-resources)。
...
- [Azure 通信预览版](https://github.com/Azure/communication-preview) - 了解有关呼叫 Web SDK 的详细信息
- [Redux](https://redux.js.org/) - 客户端状态管理
- [FluentUI](https://developer.microsoft.com/fluentui#/) - Microsoft 支持的 UI 库
- [React](https://reactjs.org/) - 用于生成用户界面的库
- [ASP.NET Core](https://docs.microsoft.com/aspnet/core/introduction-to-aspnet-core?view=aspnetcore-3.1&preserve-view=true) - 用于生成 Web 应用程序的框架
