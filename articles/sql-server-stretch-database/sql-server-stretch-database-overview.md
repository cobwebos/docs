---
title: "Stretch Database 概述 | Microsoft Docs"
...
---
# <a name="stretch-database-overview"></a>延伸数据库概述
...
使用 SQL Server Stretch Database 将冷暖事务数据从 SQL Server 动态延伸到 Microsoft Azure。 与典型的冷数据存储不同，你的数据将始终联机且可供查询。 你可以提供更长的数据保留时限，而不会破坏大型表（如客户订单历史记录）的存储库。 受益于 Azure 的低成本，你无需扩展价格不菲的本地存储。 你可以在 Azure 门户中选择定价层和配置设置，以掌控费用。 根据需要扩展或缩减。 有关详细信息，请访问 [SQL Server Stretch Database 定价](https://azure.microsoft.com/pricing/details/sql-server-stretch-database/) 页。
...
如果使用 SQL Server 2016 的临时表功能，则可以使用Stretch Database 将关联的所有或部分历史记录表迁移到 Azure 上的高性价比存储中。 有关详细信息，请参阅 [管理版本由系统控制的临时表中历史数据的保留期](https://msdn.microsoft.com/library/mt637341.aspx)。
