---
title: "将冷数据存档到 Azure - Stretch Database | Microsoft 文档"
...
---
# <a name="introduction-to-stretch-database"></a>Stretch Database 简介
...
## <a name="what-are-the-benefits-of-stretch-database"></a>延伸数据库的优势是什么？
延伸数据库提供以下优势：

### <a name="provides-cost-effective-availability-for-cold-data"></a>以经济高效的方式为冷数据提供可用性
使用 SQL Server Stretch Database 将冷暖事务数据从 SQL Server 动态延伸到 Microsoft Azure。 与典型的冷数据存储不同，你的数据将始终联机且可供查询。 你可以提供更长的数据保留时限，而不会破坏大型表（如客户订单历史记录）的存储库。 充分利用 Azure 的低成本，无需扩展价格不菲的本地存储。 你可以在 Azure 门户中选择定价层和配置设置，以掌控费用。 根据需要扩展或缩减。 有关详细信息，请访问 [SQL Server Stretch Database 定价](https://azure.microsoft.com/pricing/details/sql-server-stretch-database/) 页。

### <a name="doesnt-require-changes-to-queries-or-applications"></a>无需对查询或应用程序进行更改
无缝访问 SQL Server 数据，无论该数据在本地还是已延伸到云。  你可以设置策略来确定数据的存储位置，SQL Server 将在后台处理数据移动。 整个表始终会联机且可供查询。 此外，延伸数据库不需要对现有查询或应用程序进行任何更改 – 数据位置对应用程序完全透明。

### <a name="streamlines-on-premises-data-maintenance"></a>简化本地数据维护
减少数据的本地维护量和存储。 本地数据的备份运行更快，在维护时段内即可完成。 云中数据的备份会自动运行。 对本地存储的需求大大降低。 与在本地 SSD 中添加数据相比，使用 Azure 存储可将费用降低 80%。

### <a name="keeps-your-data-secure-even-during-migration"></a>即使在迁移期间也确保数据安全
...
## <a name="is-stretch-database-for-you"></a>延伸数据库适合你吗？
...
如果使用 SQL Server 2016 的临时表功能，则可以使用 Stretch Database 将关联的所有或部分历史记录表迁移到 Azure 上的高性价比存储中。 有关详细信息，请参阅 [管理版本由系统控制的临时表中历史数据的保留期](https://msdn.microsoft.com/library/mt637341.aspx)。
...
## <a name="test-drive-stretch-database"></a>体验延伸数据库
**借助 AdventureWorks 示例数据库体验延伸数据库。** 若要获取 AdventureWorks 示例数据库，请从 [此处](https://www.microsoft.com/download/details.aspx?id=49502)。 将示例数据库还原到 SQL Server 2016 实例后，解压缩示例文件，然后从 Stretch DB 文件夹打开 Stretch DB Samples 文件。 运行此文件中的脚本可以查看启用 Stretch Database 之前和之后数据所用的空间，跟踪数据迁移的进度，以及确认在数据迁移期间和之后是否可以继续查询现有数据和插入新数据。
