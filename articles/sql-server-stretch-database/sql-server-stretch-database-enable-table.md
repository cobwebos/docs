---
title: "为表启用 Stretch Database | Microsoft 文档"
description: "了解如何为 Stretch Database 配置表。"
...
---
# <a name="enable-stretch-database-for-a-table"></a>为表启用 Stretch Database
若要为 Stretch Database 配置某个表，请在 SQL Server Management Studio 中选择该表对应的“**延伸 | 启用**”，打开“**为表启用延伸**”向导。 也可以使用 Transact\-SQL 对现有表启用 Stretch Database，或创建一个启用了 Stretch Database 的新表。

* 如果在单独的某个表中存储了冷数据，你可以迁移整个表。
* 如果表同时包含热数据和冷数据，你可以指定筛选器函数来选择要迁移的行。

**先决条件**。 如果针对某个表选择了“**延伸 | 启用**”但尚未为数据库启用 Stretch Database，该向导将先为 Stretch Database 配置数据库。 请遵循[通过运行“为数据库启用延伸向导”开始操作](sql-server-stretch-database-wizard.md)中的步骤，而不要遵循本主题中的步骤。

**权限**。 对数据库或表启用 Stretch Database 需要有 db\_owner 权限。 对某个表启用 Stretch Database 还要求对该表拥有 ALTER 权限。
...
## <a name="a-nameenablewizardtableause-the-wizard-to-enable-stretch-database-on-a-table"></a><a name="EnableWizardTable"></a>使用向导来对表启用 Stretch Database
**启动向导**
...
查看在向导中输入的值和选择的选项。 然后选择“完成”即可启用延伸。

**结果**

查看结果。

## <a name="a-nameenabletsqltableause-transact-sql-to-enable-stretch-database-on-a-table"></a><a name="EnableTSQLTable"></a>使用 Transact\-SQL 对表启用 Stretch Database
...
### <a name="enable-stretch-database-for-an-existing-table"></a>为现有表启用 Stretch Database
...
有关详细信息，请参阅 [ALTER TABLE (Transact-SQL)](https://msdn.microsoft.com/library/ms190273.aspx)。

### <a name="create-a-new-table-with-stretch-database-enabled"></a>使用启用的 Stretch Database 创建一个新表
...
有关详细信息，请参阅 [CREATE TABLE (Transact-SQL)](https://msdn.microsoft.com/library/ms174979.aspx)。
