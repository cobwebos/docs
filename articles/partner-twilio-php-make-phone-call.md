...
4.  安装 Azure SDK for PHP。有关该 SDK 的概述及其安装说明，请参阅[设置 Azure SDK for PHP][]。
...
-   你可以使用 Azure 存储 Blob 或 SQL Database 存储电话号码和呼叫文本，而不使用 Web 窗体。有关通过 PHP 使用 Azure 存储 Blob 的信息，请参阅[在 PHP 应用程序中使用 Azure 存储空间][]。有关通过 PHP 使用 SQL Database 的信息，请参阅[在 PHP 应用程序中使用 SQL Database][]。
